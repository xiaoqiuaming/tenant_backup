...
int ObSqlReadStrategy::compare_range(const sql::ObSqlReadStrategy::RowKey_Objs objs1,const sql::ObSqlReadStrategy::RowKey_Objs objs2) const
{
    ...
    else
    {
    	...
    	YYSYS_LOG(ERROR, "rowkey_info_shoud not be null, ret= %d", ret);
    }
...
int ObSqlReadStrategy::eraseDuplicate(pRowKey_Objs start_rowkeys, pRowKey_Objs end_rowkeys, bool forward, int64_t position)
{
	...
	{
		ret = OB_ERR_UNEXPECTED ;
		YYSYS_LOG(ERROR, "param error   range_count_= %ld, ret =  %d",range_count_, ret);
	}
	else if ( position < range_count_ && position >=1 && !forward )
	{
		...
		{
			ret = OB_ERR_UNEXPECTED ;
			YYSYS_LOG(ERROR, "IN duplicate ranges , range_count_ = %ld error  %d", range_count_,ret);
		}
	}
	else
	{
		ret = OB_ERR_UNEXPECTED ;
		YYSYS_LOG(WARN, "UNEXPECTED ,check your param !  ret =  %d", ret);
	}
	return ret;
}
//add:e
...
//  	 qsort( (void*)tmp_rowkeys, range_count_ ,sizeof(RowKey_Objs), compare_range);
  	 for ( i = range_count_ - 1; i >= 1 ;i-- )
  	 {
  		 if ( 0 == compare_range( *(start_rowkeys+i), *(start_rowkeys+i-1) )  && 0 == compare_range( *(end_rowkeys+i), *(end_rowkeys+i-1) ) )
  		 {
  			...
  			{
  				YYSYS_LOG(ERROR, "fail to  erase duplicate ranges  %d", ret);
  				break;
  			}
  			else
  			{
  				YYSYS_LOG(DEBUG, "erase duplicate ranges success  %d", ret);
  			}
  		 }
  	 }
   }
   ...
   {
		ret = OB_ERR_UNEXPECTED ;
		YYSYS_LOG(WARN, "check your param ! range_count_=%ld, ret =  %d",range_count_, ret);
   }
   return ret ;
}
//add:e
void ObSqlReadStrategy::clear_simple_cond_filter_list()
{
    simple_cond_filter_list_.clear();
}
...
int ObSqlReadStrategy::find_scan_range(bool &found, bool single_row_only)
{
  ...
  if(NULL != rowkey_info_)
  {
	  for (idx = 0; idx < rowkey_info_->get_size(); idx++)
	  {
		...
		{
		  YYSYS_LOG(WARN, "fail to get column id ret=%d, idx=%ld, column_id=%ld", ret, idx, column_id);
		  break;
		}
		else
		{
		  ...
		  {
			YYSYS_LOG(WARN, "fail to find closed column range for column %lu", column_id);
			break;
		  }
		}
	 ...
	 //TODO
	  ...
	  if ( OB_SUCCESS == ret )
	  {
		  ...
		  {
			  YYSYS_LOG(ERROR, "print ranges error! ret = %d",ret);
		  }
		  else if ( ( ret = sort_mutiple_range() )!= OB_SUCCESS)
		  {
			  YYSYS_LOG(ERROR, "sorting ranges error! ret = %d",ret);
		  }
	  ...
	  else
	  {
		  ...
		  {
			  this ->range_count_ = 1;
			  YYSYS_LOG(WARN, "construct ranges  failed,the error can be ignored !,ret=%d",ret);
			  ret = OB_SUCCESS;
		  }
		  else
		  {
			  YYSYS_LOG(ERROR, "construct ranges  failed, ob_malloc may failed!,ret=%d",ret);
		  }
  ...
  {
	  ret = OB_ERR_UNEXPECTED ;
	  YYSYS_LOG(ERROR, "rowkey_info should not be null,ret=%d",ret);
  }
  return ret;
}
//add:e
...
int ObSqlReadStrategy::get_next_scan_range(ObNewRange &range,bool &has_next)
{
	...
	{
		YYSYS_LOG(WARN, "range count should not be negative or zero ,range count =%ld!", range_count_);
		idx_key_ = -1;
		has_next = false;
		ret = OB_ERR_UNEXPECTED;
	}
  ...
  for (i = 0; i < simple_cond_filter_list_.count(); i++)
  {
          ...
          (NULL != (column = rowkey_info_->get_column(idx))))
      {
        ...
        if (target_type == ObVarcharType && source_type != ObVarcharType)//比较输入类型和定义类型 fyd 2014 .3.12
        {
          if (NULL == (varchar_buff = (char*)ob_malloc(OB_MAX_VARCHAR_LENGTH, ObModIds::OB_SQL_READ_STRATEGY)))
          {
            ...
            YYSYS_LOG(WARN, "ob_malloc %ld bytes failed, ret=%d", OB_MAX_VARCHAR_LENGTH, ret);
          }
          else
          {
            ...
            {
                YYSYS_LOG(WARN, "failed to cast object, ret=%d, from_type=%d to_type=%d", ret, source_type, target_type);
              ...
              switch (cond_op)
              {
                ...
                case T_OP_IS:
                  ...
                  else if (start_key_objs_[idx] == end_key_objs_[idx])
                  {
                    ...
                    {
                      YYSYS_LOG(WARN, "two different equal condition on the sanme column, column_id=%lu", column_id);
                    }
                    ob_free(varchar_buff);
                    varchar_buff = NULL;
                  }
                ...
                default:
                  ob_free(varchar_buff);
                  varchar_buff = NULL;
                  YYSYS_LOG(WARN, "unexpected cond op: %ld", cond_op);
                  ret = OB_ERR_UNEXPECTED;
                  break;
              }
            }
            ...
            switch (cond_op)
            {
              ...
              case T_OP_IS:
                ...
                else if (start_key_objs_[idx] == end_key_objs_[idx])
                {
                  if (*p_promoted_obj != start_key_objs_[idx])
                  {
                    YYSYS_LOG(WARN, "two different equal condition on the same column, column_id=%lu"
    ...
    else if ((!single_row_only) && simple_cond_filter_list_.at(i).is_simple_between(real_val, cond_cid, cond_op, cond_start, cond_end))
    {
      if (cond_cid == column_id)
      {
        ...
        if (column == NULL)
        {
          YYSYS_LOG(WARN, "get column from rowkey_info failed, column = NULL");
        }
        else
        {
          ...
          if (target_type == ObVarcharType && start_source_type != ObVarcharType)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to cast object, ret=%d, from_type=%d to_type=%d", ret, start_source_type, target_type);
                ob_free(varchar_buff);
                varchar_buff = NULL;
                break;
              }
          ...
          if (OB_SUCCESS == ret)
          {
            if (target_type == ObVarcharType && end_source_type != ObVarcharType)
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(WARN, "failed to cast object, ret=%d, from_type=%d to_type=%d", ret, end_source_type, target_type);
                  ob_free(varchar_buff);
                  varchar_buff = NULL;
                  break;
                }
 ...
 */
int ObSqlReadStrategy::find_closed_column_range_simple_con(bool real_val, 
                                                           const ObRowkeyColumn *column, 
														   int64_t column_idx, 
														   int64_t idx_of_ranges, 
														   int64_t cond_op, 
														   ObObj cond_val, 
														   bool &found_start, 
														   bool &found_end, 
														   bool single_row_only)
{
    int ret = OB_SUCCESS;
    UNUSED(real_val);
    ObObjType target_type ;
    ...
    {
  	  ret = OB_ERR_UNEXPECTED;
  	  YYSYS_LOG(ERROR, "column should not be null,ret=%d",ret);
    }
    else if ( idx < 0 || idx >= range_count_ * OB_MAX_ROWKEY_COLUMN_NUMBER)
    {
  	  ret = OB_ERR_UNEXPECTED;
  	  YYSYS_LOG(ERROR, " obj index of ranges out of range ,column idx =%ld,range idx =%ld,ret=%d",column_idx, idx,ret);
    }
    else
    {
  	   target_type = column->type_;
            ...
            case T_OP_IS:
             // not set value
              ...
              else if (mutiple_start_key_objs_[idx] == mutiple_end_key_objs_[idx])
              {
                ...
                {
                  YYSYS_LOG(WARN, "two different equal condition on the same column, column_id=%lu", column->column_id_);
	  ...
	  {
		  YYSYS_LOG(WARN, "error, ret=%d", ret);
	  }
    ////////////////
    }
 ...
 */
int ObSqlReadStrategy::find_closed_column_range_simple_btw(bool real_val, 
                                                           const ObRowkeyColumn *column, 
														   int64_t column_idx, 
														   int64_t idx_of_ranges,/* int64_t cond_op, ObObj cond_val,*/ 
														   ObObj cond_start, 
														   ObObj cond_end, 
														   bool &found_start, 
														   bool &found_end, 
														   bool single_row_only)
{
   ...
   bool need_store_space  = false ;
   ObObjType target_type ;
  // YYSYS_LOG(DEBUG, "column idx =%ld,range idx =%ld,obj start:%s,end:%s",column_idx, idx_of_ranges,to_cstring(mutiple_start_key_objs_[idx]), to_cstring(mutiple_end_key_objs_[idx]));
   ///
  if ( NULL == column )
  {
	  ret = OB_ERR_UNEXPECTED;
...
int ObSqlReadStrategy::find_closed_column_range_ex(bool real_val, int64_t idx, uint64_t column_id, bool &found_start, bool &found_end, bool single_row_only)
{
  ...
  if ( NULL != rowkey_info_)
  {
	  ...
	  if (simple_in_filter_list_.count() && !single_row_only )
	  {
		  ...
		  {
			  ret = OB_ERR_UNEXPECTED;
			  YYSYS_LOG(WARN, "fail to get rowkey  from rowkey info ,ret=%d", ret);
		  } 
		  else if (  column->column_id_ != column_id )
		  {
			  ret = OB_ERR_UNEXPECTED;
			  YYSYS_LOG(WARN, "column_id do not match idx  ,ret=%d", ret);
		  }
		  else
		  {
			  for ( i = 0; i < simple_in_filter_list_.count() && ret == OB_SUCCESS; i++ )
			  {
				 if ( simple_in_filter_list_.at(i).is_in_expr_with_ex_rowkey(real_val, *rowkey_info_, rowkey_array, rowkey_objs_allocator))
				 {
						...
						{
							YYSYS_LOG(WARN, "fail to get rowkey prefix info   from in expr  ,ret=%d", ret);
							continue;
						}
						else if ( range_count_ <= 0  )
						{
						  ...
						  {
							  YYSYS_LOG(ERROR, "fail to malloc space for rowkey ranges ret=%d", ret);
							  break;
						  }
						  else
						  {
			  ...
			  if ( OB_SUCCESS == ret && rowkey_array.count() > 0 && rowkey_array.at(0).get_obj_cnt() > idx )
			  {
				  for ( int rowkey_i = 0;rowkey_i < rowkey_array.count() && ret == OB_SUCCESS; rowkey_i++ )
				  {
						...
						YYSYS_LOG(DEBUG, "column idx=%ld, row idx = %d,write position = %ld,val=%s,prefx rk size = %ld,row num = %ld,current range idx = %d",
...
//						YYSYS_LOG(INFO, "range idx = %d,ret=%d", rowkey_i, ret);
				 }
				...
				if (ret != OB_SUCCESS)
				{
					YYSYS_LOG(WARN, "resolve  in expr for ranges error, ret=%d", ret);
				} 
				else
				{
					YYSYS_LOG(DEBUG,
	...
	//////
	  if (range_count_  <= 0 )
	  {
		 ...
		 {
			 YYSYS_LOG(WARN, "malloc  range space failed!,repeat = %d,ret =%d",repeat, ret);
		 }
		 if ( OB_SUCCESS == ret )
		 {
			 ...
			 YYSYS_LOG(DEBUG, "malloc  range space successed!");
		 }
		 else
		 {
	...
	///////
	  ...
	  {
		  ret = OB_ERR_UNEXPECTED;
		  YYSYS_LOG(WARN, "rowkey column is null,the parameter should be checked! cid=%ld,ret=%d", column_id, ret );
	  }
	  else
	  {
		  for (i = 0; i < simple_cond_filter_list_.count() && ret == OB_SUCCESS; i++)
		  {
			if (simple_cond_filter_list_.at(i).is_simple_condition(real_val, cond_cid, cond_op, cond_val) && cond_cid == column_id )
			{
				  for ( int64_t range_index = 0; range_index < range_count_ && ret == OB_SUCCESS ;range_index ++)
				  {
					 ...
					 {
						 YYSYS_LOG(WARN, "construct range = %ld,column = %ld  failed!", range_index, idx);
					 }
				  }
			}
...
//					 }
  ...
  {
	  ret = OB_ERR_UNEXPECTED;
	  YYSYS_LOG(WARN, "rowkey_info_ should not be null, ret = %d!",ret);
  }
  return ret;
}
//add:e
 ...
 *  如果当前列已经由了值，则忽略当前值
 */
int  ObSqlReadStrategy::resolve_close_column_range_in(bool real_val, 
                                                      const ObRowkeyColumn *column, 
													  ObObj cond_val, 
													  int64_t column_idx, 
													  int64_t idx_of_ranges, 
													  ...
													  bool single_row_only)
{
	...
	////////////////
	 bool need_store_space  = false ;
	  ...
	  {
		  ret = OB_ERR_UNEXPECTED;
		  YYSYS_LOG(ERROR, " obj index of ranges out of range ,column idx =%ld,range idx =%ld,ret=%d",column_idx, current_index,ret);
	  }
	  else
	  {
		   target_type = column->type_;
		  ...
		  if ( OB_SUCCESS == ret )
		  {
			...
			{
				 YYSYS_LOG(WARN, "duplicate value for the same column, ret=%d", ret);
				 if ( need_store_space )
				 {
					 ob_free(varchar_buff);
					 varchar_buff = NULL;
...
int ObSqlReadStrategy::add_filter(const ObSqlExpression &expr)
{
  ...
  {
    // YYSYS_LOG(DEBUG, "simple condition [%s]", to_cstring(expr));
    if (OB_SUCCESS != (ret = simple_cond_filter_list_.push_back(expr)))
    {
      YYSYS_LOG(WARN, "fail to add simple filter. ret=%d", ret);
    }
  }
  else if (expr.is_simple_between(false, cid, op, val1, val2))
  {
    // YYSYS_LOG(DEBUG, "simple between condition [%s]", to_cstring(expr));
    if (OB_SUCCESS != (ret = simple_cond_filter_list_.push_back(expr)))
    {
      YYSYS_LOG(WARN, "fail to add simple filter. ret=%d", ret);
    }
...
//    {
//      YYSYS_LOG(WARN, "fail to add in sub query filter. ret=%d", ret);
//    }
...
//    {
//      // YYSYS_LOG(DEBUG, "simple in expr [%s]", to_cstring(expr));
//      if (OB_SUCCESS != (ret = simple_in_filter_list_.push_back(expr)))
//      {
//        YYSYS_LOG(WARN, "fail to add simple filter. ret=%d", ret);
//      }
...
//add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160226:b
int ObSqlReadStrategy::reset_filter()
{
  int ret = OB_SUCCESS;
  simple_in_filter_list_.clear();
  simple_cond_filter_list_.clear();
...
//  }
  ...
  for (idx = 0; idx < rowkey_info_->get_size(); idx++)
  {
    if (OB_SUCCESS != (ret = rowkey_info_->get_column_id(idx, column_id)))
    {
      YYSYS_LOG(WARN, "fail to get column id. idx=%ld, ret=%d", idx, ret);
      break;
    }
    ...
    {
      YYSYS_LOG(WARN, "fail to find closed range for column %lu", column_id);
      break;
    }
    else if (!found)
    {
...
//    rowkey.assign(start_key_objs_, rowkey_info_->get_size());
    ...
    {
    	ret = OB_ERR_UNEXPECTED;
    	YYSYS_LOG(WARN, "range_count_ should be positive, ret=%d", ret);
    }
    //mod:e
    if (OB_SUCCESS != (ret = rowkey_array.push_back(rowkey)))
    {
      YYSYS_LOG(WARN, "fail to push rowkey to list. rowkey=%s, ret=%d", to_cstring(rowkey), ret);
    }
  }
  return ret;
}
...
int ObSqlReadStrategy::find_rowkeys_from_in_expr(bool real_val, ObIArray<ObRowkey> &rowkey_array, common::PageArena<ObObj,common::ModulePageAllocator> &objs_allocator)
{
  ...
  {
    YYSYS_LOG(DEBUG, "simple in filter count[%ld]", simple_in_filter_list_.count());
    ret = OB_SUCCESS;
  }
  else
  {
    for (i = 0; i < simple_in_filter_list_.count(); i++)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "fail to get rowkey(s) from in expression. ret=%d", ret);
      ...
      {
        YYSYS_LOG(DEBUG, "simple in expr rowkey_array count = %ld", rowkey_array.count());
      }
    }
  }
  ...
  if (OB_SUCCESS == ret && true == is_in_expr_with_rowkey)
  {
    ...
    for (rowkey_idx = 0; rowkey_idx < rowkey_array.count(); rowkey_idx++)
    {
      ...
      if (OB_LIKELY(OB_SUCCESS == ret))
      {
        ...
        {
          YYSYS_LOG(WARN, "rowkey has too much varchar. len=%ld", total_used_buf_len);
        }
...
int ObSqlReadStrategy::get_read_method(ObIArray<ObRowkey> &rowkey_array, PageArena<ObObj,common::ModulePageAllocator> &rowkey_objs_allocator, int32_t &read_method)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to find rowkeys in equal where operator. ret=%d", ret);
    }
...
int  ObSqlReadStrategy::release_rowkey_objs()
       {
		  ...
		  // step 2
		YYSYS_LOG(DEBUG, "release the mem ,range_count_ = %ld,range_count_cons_ = %ld", range_count_, range_count_cons_);
		//mod peiouya IN_EXPR  [PrefixKeyQuery_for_INstmt] 20140930:b
       	//if ( range_count_ >= 1 )
		if ( range_count_cons_ >= 1 )
		//mod 20140930:e
       	{
       		YYSYS_LOG(DEBUG, "release the mem ,range = %ld start%d", range_count_, ret);
			...
			//add 20140930:e
          	idx_key_ = 0;
          	YYSYS_LOG(DEBUG, "release the mem ,range = %ld end%d", range_count_, ret);
       	}
       	return ret ;
       ...
       int ObSqlReadStrategy::malloc_rowkey_objs_space( int64_t num)
       {
       	...
       	{
       		ret = OB_ERR_UNEXPECTED ;
       		YYSYS_LOG(ERROR, "malloc space twice,error, ret=%d", ret);
       	}
       	else if ( num <=0 )
       	{
       		ret = OB_ERR_UNEXPECTED ;
       		YYSYS_LOG(ERROR, "Wrong parameter,should not be negative, ret=%d", ret);
       	}
       	...
       	{
                      ret = OB_ALLOCATE_MEMORY_FAILED;
                      YYSYS_LOG(WARN, "ob_malloc %ld bytes failed, ret=%d",OB_MAX_ROWKEY_COLUMN_NUMBER*num*sizeof(common::ObObj), ret);
       	}
       	...
       	else if ( (mutiple_start_key_mem_hold_ = (char **)ob_malloc(sizeof(char*)*num*OB_MAX_ROWKEY_COLUMN_NUMBER, ObModIds::OB_SQL_READ_STRATEGY)) == NULL)
       	{
   			...
   			mutiple_end_key_objs_ = NULL;
               ret = OB_ALLOCATE_MEMORY_FAILED;
               YYSYS_LOG(WARN, "ob_malloc %ld bytes failed, ret=%d", sizeof(char*)*num*OB_MAX_ROWKEY_COLUMN_NUMBER, ret);
       	}
          ...
          if ( NULL != rowkey_info_)
          {
			  ...
			  for ( int i = 0;i < range_count_;i++)
			  {
			   for ( int j = 0; j < rowkey_num; j++)
			   {
				...
				YYSYS_LOG(DEBUG, "ranges:line %d  column %d start is=  %s", i, j, to_cstring(*(mutiple_start_key_objs_+ i*OB_MAX_ROWKEY_COLUMN_NUMBER+j)));
				YYSYS_LOG(DEBUG, "ranges:line %d  column %d end is=  %s", i, j, to_cstring(*(mutiple_end_key_objs_+ i*OB_MAX_ROWKEY_COLUMN_NUMBER+j)));
