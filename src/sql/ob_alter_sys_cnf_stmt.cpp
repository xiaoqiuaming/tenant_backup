...
int64_t ObSysCnfItemKey::hash() const
{
  ...
  if (cluster_id_ != common::OB_ALL_CLUSTER_FLAG)
  {
...
int64_t ObSysCnfItem::to_string(char* buf, const int64_t buf_len) const
{
  ...
  {
    case OB_ROOTSERVER:
      databuff_printf(buf, buf_len, pos, "SERVER TYPE ::= YAOADMINSVR, ");
      break;
    case OB_UPDATESERVER:
      databuff_printf(buf, buf_len, pos, "SERVER TYPE ::= YAOTXNSVR, ");
      break;
    case OB_CHUNKSERVER:
      databuff_printf(buf, buf_len, pos, "SERVER TYPE ::= YAODATASVR, ");
      break;
    case OB_MERGESERVER:
      databuff_printf(buf, buf_len, pos, "SERVER TYPE ::= YAOSQLSVR, ");
      break;
    default:
  ...
  if (cluster_id_ != common::OB_ALL_CLUSTER_FLAG)
  {
    databuff_printf(buf, buf_len, pos, "CLUSTER ::= %ld\n", cluster_id_);
  }
  else
  {
