...
int ObSQLQueryCacheItemHolder::Buffer::init(int64_t capacity)
{
  ...
  if (NULL == ptr_)
  {
    YYSYS_LOG(ERROR, "ob_malloc failed");
...
int ObSQLQueryCacheItemHolder::Buffer::write(const char* bytes, int64_t size)
{
  ...
  {
    YYSYS_LOG(WARN, "expect more space than capacity, capacity_: %ld length_: %ld",
        capacity_, length_);
    ret = OB_ERROR;
  }
  else
  ...
  : save_flag_(false), initialized_(false)
{
  ...
  {
    YYSYS_LOG(ERROR, "Buffer init failed, err: %d", err);
  }
  else
  {
...
void ObSQLQueryCacheItemHolder::reuse()
{
  if (!initialized_)
  {
    YYSYS_LOG(ERROR, "ObSQLQueryCacheItemHolder has not been intialized");
  }
  else
  {
    key_.reuse();
...
int ObSQLQueryCacheItemHolder::set(int64_t sql_id, const ObIArray<ObObj> & array)
{
  ...
  {
    YYSYS_LOG(WARN, "do not support params larger than %ld", max_param_num_);
    ret = OB_ERROR;
  }
  else
  {
...
int ObSQLQueryCache::init(const mergeserver::ObMergeServerConfig * config)
{
  ...
  if (initialized_)
  {
    YYSYS_LOG(ERROR, "ObSQLQueryCache has been initialized");
    ret = OB_INIT_TWICE;
  }
  else if (NULL == config)
  {
    YYSYS_LOG(ERROR, "ObSQLQueryCache arguments error");
...
int ObSQLQueryCache::get(const ObSQLQueryCacheKey & key, ObResWrapper & res)
{
  ...
  if (!initialized_)
  {
    YYSYS_LOG(ERROR, "ObSQLQueryCache has not been initialized");
    ret = OB_NOT_INIT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "cache_ get error, ret: %d", ret);
    }
    else if (OB_SUCCESS == ret && yysys::CTimeUtil::getTime() - res.value.store_ts
        > config_->query_cache_expiration_time)
    {
      int err = remove(key, res);
      if (OB_SUCCESS != err && OB_ENTRY_NOT_EXIST != err)
      {
        YYSYS_LOG(ERROR, "remove stale result cache error, err: %d", err);
      }
...
int ObSQLQueryCache::revert(ObResWrapper & res)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "cache_ revert error, ret: %d", ret);
    }
  }
  return ret;
}
...
int ObSQLQueryCache::remove(const ObSQLQueryCacheKey & key, ObResWrapper & res)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret && OB_ENTRY_NOT_EXIST != ret)
    {
      YYSYS_LOG(ERROR, "cache_ erase error, ret: %d", ret);
    }
    else
    {
      ret = revert(res);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "revert error, ret: %d", ret);
      }
    }
  }
  return ret;
    ...
    const ObIArray<ObObj> & params, ObResWrapper & res)
{
  ...
  else
  {
    ...
    if (NULL == holder)
    {
      YYSYS_LOG(ERROR, "ObTlStore get return NULL");
      ret = OB_ERROR;
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "ObSQLQueryCacheItemHolder set error, ret: %d", ret);
      }
      else
      {
        ...
        do
        {
          ...
          if (OB_ENTRY_NOT_EXIST == ret)
          {
            ...
            YYSYS_LOG(DEBUG, "fake node inserted");
          }
          else if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "KeyValueCache get error, ret: %d", ret);
          }
          else if (yysys::CTimeUtil::getTime() - res.value.store_ts
              > config_->query_cache_expiration_time)
          {
            YYSYS_LOG(INFO, "out_dated");
...
int ObSQLQueryCache::append_ongoing_res(easy_buf_t * res)
{
  ...
  else
  {
    ...
    else if (holder->has_save_flag())
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "buffer_write error, ret: %d", ret);
...
int ObSQLQueryCache::finish_ongoing_res()
{
  ...
  else
  {
    ...
    if (NULL == holder)
    {
      YYSYS_LOG(ERROR, "ObTlStore returns NULL");
      ret = OB_ERROR;
    }
    else if (holder->has_save_flag())
    {
      ObSQLQueryCacheValue value;
      value.store_ts = yysys::CTimeUtil::getTime();
      value.res      = holder->buffer_ptr();
      value.res_len  = holder->buffer_length();
      ret = cache_.put(holder->get_key(), value);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "KeyValueCache put error, ret: %d", ret);
      }
    }
  }
  return ret;
