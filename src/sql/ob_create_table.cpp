...
int ObCreateTable::open()
{
  ...
  {
    YYSYS_LOG(WARN, "create table is not allowed in transaction, err=%d", ret);
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "create table succ, table_name=%s if_not_exists=%c",
...
void ObCreateTable::grant_owner_privilege()
{
  ...
  else
  {
    ...
    if (i == retry_times + 1)
    {
      ...
      /*YYSYS_LOG(ERROR, "User: %.*s create table %s success, but grant all privileges, grant option on %s to '%.*s' failed, ret=%d",
    ...
    //add liumz, [multi_database.create_table]20150708:b
    //YYSYS_LOG(ERROR, "test::lmz, is_regrant_priv[%d]", is_regrant_priv);
