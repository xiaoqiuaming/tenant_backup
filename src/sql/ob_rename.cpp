...
int ObRename::set_table(const uint64_t table_id, const uint64_t base_table_id)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid id: table_id=%ld, base_table_id=%ld", table_id, base_table_id);
...
int ObRename::add_column_id(const uint64_t& column_id)
{
  ...
  {
    YYSYS_LOG(WARN, "Add column id failed, column_id=%lu, ret=%d", column_id, ret);
  }
  return ret;
}
...
int ObRename::open()
{
  ...
  {
    YYSYS_LOG(WARN, "invalid id: table_id=%ld, base_table_id=%ld", table_id_, base_table_id_);
    ret = OB_NOT_INIT;
  }
...
int ObRename::cons_row_desc()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to get original row desc, err=%d", ret);
  }
  else if (column_ids_.count() != 0 && column_ids_.count() != org_row_desc_->get_column_num())
  {
    YYSYS_LOG(ERROR, "Column ids are not initiated well");
    ret = OB_NOT_INIT;
  }
  else
  {
    ...
    for (int64_t idx = 0; idx < org_row_desc_->get_column_num(); idx++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to add tid and cid to row desc. idx=%ld, tid=%lu, ret=%d",
                  idx, table_id_, ret);
        break;
      }
    }
...
int ObRename::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    YYSYS_LOG(ERROR, "not init, call open() first");
    ret = OB_NOT_INIT;
  }
  else
  {
...
DEFINE_SERIALIZE(ObRename)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to encode table_id_:ret[%d]", ret);
  }
  else if (OB_SUCCESS != (ret = encode_vi64(buf, buf_len, pos, static_cast<int64_t>(base_table_id_))))
  {
    YYSYS_LOG(WARN, "fail to encode base_table_id_:ret[%d]", ret);
  }
  if (OB_SUCCESS != (ret = encode_vi64(buf, buf_len, pos, column_ids_.count())))
  {
    YYSYS_LOG(WARN, "fail to encode renamed columns count:ret[%d]", ret);
  }
  else
  {
...
DEFINE_DESERIALIZE(ObRename)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to decode table_id:ret[%d]", ret);
  }
  else if (OB_SUCCESS != (ret = decode_vi64(buf, data_len, pos, &base_table_id)))
  {
    YYSYS_LOG(WARN, "fail to decode base_table_id:ret[%d]", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "decode columns_count fail:ret[%d]", ret);
    }
    else
    {
      for (int64_t i = 0; OB_SUCCESS == ret && i < columns_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "decode column id failed : ret[%d]", ret);
        }
        else if (OB_SUCCESS != (ret = column_ids_.push_back(static_cast<uint64_t>(column_id))))
        {
          YYSYS_LOG(WARN, "fail to add column id to array:ret[%d]", ret);
        }
      }
    }
  }
