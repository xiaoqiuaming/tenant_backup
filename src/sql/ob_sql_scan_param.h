...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      inline void set_version(const int64_t version)
      {version_ = version;}
      ...
      { scan_flag_.read_mode_ = mode & ScanFlag::SF_MASK_READ_MODE; }
      inline ScanFlag::SyncMode get_read_mode() const
      { return static_cast<ScanFlag::SyncMode>(scan_flag_.read_mode_); }

      inline void set_scan_direction(const ScanFlag::Direction dir)
      { scan_flag_.direction_ = dir & 0x3; }
      inline ScanFlag::Direction get_scan_direction() const
      { return static_cast<ScanFlag::Direction>(scan_flag_.direction_); }

      inline void set_not_exit_col_ret_nop(const bool nop)
      { scan_flag_.not_exit_col_ret_nop_ = nop; }
      inline bool is_not_exit_col_ret_nop() const
      { return scan_flag_.not_exit_col_ret_nop_; }

      inline void set_daily_merge_scan(const bool full)
      { scan_flag_.daily_merge_scan_ = full; }
      inline bool is_daily_merge_scan() const
      { return scan_flag_.daily_merge_scan_; }

      inline void set_full_row_scan(const bool full)
      { scan_flag_.full_row_scan_ = full; }
      inline bool is_full_row_scan() const
      { return scan_flag_.full_row_scan_; }

      inline void set_rowkey_column_count(const int16_t count)
      { scan_flag_.rowkey_column_count_ = count; }
      inline int16_t get_rowkey_column_count() const
      { return scan_flag_.rowkey_column_count_; }
