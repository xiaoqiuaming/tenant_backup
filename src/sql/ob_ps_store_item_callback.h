      ...
      public:
        ...
        virtual void operator()(common::hash::HashMapPair<ObPsStoreKey, ObPsStoreItem*> entry)
        {
          ...
          if (common::OB_SUCCESS == rc_)
          {
            ...
            item_ = entry.second;
            YYSYS_LOG(DEBUG, "rlock succ ps ref count is %ld item is %p", item_->get_ps_count(), entry.second);
          }
          else if (EBUSY == rc_)
          {
          ...
          {
            YYSYS_LOG(ERROR, "ps debug rlock failed ret=%d item is %p stmtid=%lu", rc_, entry.second, entry.second->get_sql_id());
            rc_ = common::OB_ERROR;
          }
        }
    };
    ...
    class ObPsStoreItemDecRef : public ObPsStoreAtomicOp
    {
      public:
        ...
        virtual void operator()(common::hash::HashMapPair<ObPsStoreKey, ObPsStoreItem*> entry)
        {
          ...
          YYSYS_LOG(DEBUG, "item is %p stmtid=%ld, ps ref count is %ld", entry.second, entry.second->get_sql_id(),pscount);
          if (0 == pscount)
          {
          ...
          YYSYS_LOG(DEBUG, "ps ref count is %ld", pscount);
        }
    };
  }
}
