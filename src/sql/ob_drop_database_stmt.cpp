...
ObDropDbStmt& ObDropDbStmt::operator =(const ObDropDbStmt &other)
{
  ...
  else
  {
    ...
    for(i = 0; i< db->count(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get db name from drop database statement, ret [%d]",ret);
        break;
      }
      else if(OB_SUCCESS != (ret = this->add_database(db_name)))
      {
        YYSYS_LOG(WARN, "failed to set db name to drop db statement, ret[%d]", ret);
        break;
      }
    }
  }
...
int ObDropDbStmt::add_database(const ObString &database)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to add database, err = %d", ret);
  }
  return ret;
}
...
void ObDropDbStmt::print(FILE *fp, int32_t level, int32_t index)
{
  ...
  for(int64_t i =0; i < database_.count(); i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "get string from database failed, index[%ld]", i);
      break;
    }
    if(0 == i)
    {
