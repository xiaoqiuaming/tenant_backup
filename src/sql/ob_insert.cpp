...
int ObInsert::open()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to insert by mutator. ret=%d", ret);
    }
  }
  return ret;
}
...
inline bool ObInsert::is_rowkey_column(uint64_t column_id)
{
  ...
  for (int64_t i = 0; i < rowkey_info_.get_size(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to get column id, err=%d", ret);
      break;
    }
...
int ObInsert::insert_by_mutator()
{
        ...
        && OB_SUCCESS == (ret = child_op_->get_next_row(row)))
  {
    ...
    if (rowkey_info_.get_size() >= row->get_column_num())
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to insert cell, err=%d", ret);
        break;
      }
      else
      {
        YYSYS_LOG(DEBUG, "insert cell, rowkey=%s cell=%s", to_cstring(rowkey), to_cstring(cell_nop));
      }
      continue;
    }
    // other columns
    for (int64_t i = 0; i < row->get_column_num(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get cid from row desc, err=%d", ret);
        break;
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "insert cell, rowkey=%s cell=%s", to_cstring(rowkey), to_cstring(*res_cell));
        }
      }
    } // end for
  }   // end while
