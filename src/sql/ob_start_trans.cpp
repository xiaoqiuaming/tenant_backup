...
int ObStartTrans::open()
{
  int ret = OB_SUCCESS;
  OB_ASSERT(rpc_);
  //common::ObTransID trans_id;//del peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150724
  ...
  {
    YYSYS_LOG(USER_ERROR, "already in transaction");
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "ups start transaction failed, err=%d", ret);
    }
    else
    {
      session->set_trans_id(trans_id);
      session->set_curr_trans_start_time(yysys::CTimeUtil::getTime());
      OB_STAT_INC(OBMYSQL, SQL_MULTI_STMT_TRANS_COUNT);
      OB_STAT_INC(OBMYSQL, SQL_MULTI_STMT_TRANS_STMT_COUNT);
    }
    YYSYS_LOG(DEBUG, "start transaction, ret=%d req=%s trans_id=%s ret=%d",
        ret, to_cstring(req_), to_cstring(trans_id), ret);
  }
   */
  if (session->get_trans_start() || session->get_trans_id().is_valid())
  {
    YYSYS_LOG(USER_ERROR, "already in transaction");
    // no update trans id, keep last trans id
    ret = OB_ERR_TRANS_ALREADY_STARTED;
