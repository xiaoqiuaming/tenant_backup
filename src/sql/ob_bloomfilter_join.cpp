...
ObBloomFilterJoin::ObBloomFilterJoin()
    :get_next_row_func_(NULL),
       ...
       is_right_iter_end_(false), hashmap_num_(0)
{
...
int ObBloomFilterJoin::set_join_type(const ObJoin::JoinType join_type)
{
  ...
  switch(join_type)
  {
    ...
    case LEFT_OUTER_JOIN:
      ...
      //YYSYS_LOG(ERROR,"left_outer_get_next_row");
      use_bloomfilter_ = true;
      break;
...
int ObBloomFilterJoin::open()
{
  ...
  if (con_length_ <= 0)
  {
    YYSYS_LOG(WARN, "bloom filter join can not work without equijoin conditions");
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    for (int64_t i = 0; i < con_length_; ++i)
    {
      ...
      if (expr.is_equijoin_cond(c1, c2))
      {
        ...
        if(use_bloomfilter_)
        {
		  ...
		  {
            /* Never Mind, it's OK*/
            YYSYS_LOG(WARN, "Now, we only support TableRpcScan operator!");
          }
          else
          {
            ...
            YYSYS_LOG(DEBUG, "%d, %lu", use_index_back, index_tid);
            if(use_index_back)
            {
              if(OB_INVALID_ID == index_tid)
              {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "WTF, use index scan,but index tid is invalid");
              }
              else
            ...
            YYSYS_LOG(DEBUG, "dem1 has been changed from %ld to %ld", c2.tid, dem1.value_.cell_.tid);
          }
        }

        //c1是左表, c2是右表
        YYSYS_LOG(DEBUG, "Dragon says: %ld/%ld c1 is left table while c2 is right table c1[%lu, %lu] c2[%lu, %lu]",
    ...
    {
      //YYSYS_LOG(ERROR, "load data from left table, row=%s", to_cstring(*row));
      ...
      //YYSYS_LOG(ERROR,"DHC bucket_num=%d",(int)bucket_num);
      //sort--part1 维护前表数据
    ...
    YYSYS_LOG(ERROR,"max_size=%d second_size=%d",(int)max_size,(int)second_size);
    */
  }
  if(OB_SUCCESS == ret)
  {
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::compare_equijoin_cond()");
  int ret = OB_SUCCESS;
  cmp = 0;
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::left_row_compare_equijoin_cond()");
  int ret = OB_SUCCESS;
  cmp = 0;
  const ObObj *res1 = NULL;
  const ObObj *res2 = NULL;
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::right_row_compare_equijoin_cond()");
  int ret = OB_SUCCESS;
  cmp = 0;
  const ObObj *res1 = NULL;
  const ObObj *res2 = NULL;
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::curr_row_is_qualified()");
  int ret = OB_SUCCESS;
  is_qualified = true;
...
int ObBloomFilterJoin::inner_get_next_row(const common::ObRow *&row)
{
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    {
      //  YYSYS_LOG(ERROR,"DHC etch the next right row number=[%d]",number++);
      ...
      else
      {
        ...
        //YYSYS_LOG(ERROR,"DHC etch the next right row ret=[%d]",ret);
        if (OB_SUCCESS != ret)
        {
          if (OB_ITER_END == ret)
          {
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::full_outer_get_next_row() [%d]",(int)left_cache_.is_empty());
  int ret = OB_SUCCESS;
  const ObRow *right_row = NULL;
  // fetch the next left row
  if (NULL != last_left_row_)
...
{
  //YYSYS_LOG(ERROR,"DHC last_hash_i=%d",last_left_hash_id_);
  int ret = OB_SUCCESS;
  for(;last_left_hash_id_<HASH_BUCKET_NUM;)
  {
    ...
    else 
	{
      ...
      //YYSYS_LOG(ERROR,"left_rows i=%d %s %d",last_left_hash_id_,to_cstring(curr_cached_left_row_),curr_cached_left_row_.get_raw_row().get_reserved2());
...
{
  //YYSYS_LOG(ERROR, "dhc ObBloomFilterJoin::compare_hash_equijoin()");
  int ret = OB_SUCCESS;
  uint64_t bucket_num = 0;
  const ObObj *res1 = NULL;
  const ObObj *res2 = NULL;
...
int ObBloomFilterJoin::inner_hash_get_next_row(const common::ObRow *&row)
{
  ...
  while(OB_SUCCESS == ret)
  {
    if (left_hash_id_cache_valid_)
    {
      ...
      if (0 == cmp)
      {
        ...
        else if (is_qualified)
        {
          ...
          //YYSYS_LOG(ERROR,"curr_hash_row %s",to_cstring(*left_row));
          break;
        }
      }
      else
    ...
    else
    {
      // fetch the next right row
      ret = right_op_->get_next_row(right_row);
      //YYSYS_LOG(ERROR, "get_next_row %d",numi++);
      if (OB_SUCCESS != ret)
      {
        if (OB_ITER_END == ret)
        {
      ...
      //YYSYS_LOG(ERROR,"DHC etch the next right row ret=[%d] row=%s",ret,to_cstring(*right_row));
      OB_ASSERT(right_row);
      int cmp = -10;
...
int ObBloomFilterJoin::left_hash_outer_get_next_row(const common::ObRow *&row)
{
  ...
  if(last_left_hash_id_ != -1)
  {
    ...
    else
    {
      ...
      {
         //YYSYS_LOG(ERROR,"dhc right_row=%s  row=%s",to_cstring(*left_row),to_cstring(curr_row_));
         row = &curr_row_;
      }
    }
  }
