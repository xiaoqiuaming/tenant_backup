...
#define DEF_OP(type) \
    ...
    const char* ob_phy_operator_type_str(ObPhyOperatorType type)
    {
      ...
      switch(type)
      {
        ...
        DEF_OP(PHY_READ_ATOMIC_RPC_SCAN);
        //add duyr 20151229:e
        //add hongchen [add_miss_op] 20170810:b
        DEF_OP(PHY_ALTER_GROUP);
        DEF_OP(PHY_CUR_TIME);
        DEF_OP(PHY_MULTI_UPS_MULTI_GET);
        DEF_OP(PHY_MULTI_UPS_SCAN);
        DEF_OP(PHY_REPLACE_COMPLETE_ROW_FUSE);
        DEF_OP(PHY_REPLACE_SEMANTIC_FILTER);
        DEF_OP(PHY_UPDATE_SEMANTIC_FILTER);
        DEF_OP(PHY_INCOMPLETE_ROW_FILTER);
        DEF_OP(PHY_UPDATE_DB_SEM_FILTER);
        //add hongchen [add_miss_op] 20170810:e
        default:
          break;
      }
      return ret;
    ...
    void ob_print_phy_operator_stat()
    {
      for (int32_t t = PHY_INVALID; t < PHY_END; ++t)
      {
        if (0 < PHY_OP_STAT[t])
        {
          YYSYS_LOG(INFO, "[PHY_OP_STAT] %s id=%d num=%lu",
    ...
    void ob_inc_phy_operator_stat(ObPhyOperatorType type)
    {
      ...
      YYSYS_LOG(DEBUG, "ob_inc_phy_operator_stat %s num=%lu",
    ...
    {
      OB_ASSERT(type >= PHY_INVALID && type < PHY_END);
      YYSYS_LOG(DEBUG, "ob_dec_phy_operator_stat %s num=%lu",
          ob_phy_operator_type_str(type), PHY_OP_STAT[type]);
