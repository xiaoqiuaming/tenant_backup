...
ObPhyOperator *ObPhyOperatorFactory::get_one(ObPhyOperatorType phy_operator_type, common::ModuleArena &allocator)
{
  ...
  switch(phy_operator_type)
  {
    ...
    case PHY_MULTIPLE_SCAN_MERGE:
      ...
      break;
    //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160122:b
    case PHY_REPLACE_COMPLETE_ROW_FUSE:
      ret =  tc_rp_alloc(ObReplaceCompleteRowFuse);
      break;
    case PHY_REPLACE_SEMANTIC_FILTER:
      ret =  tc_rp_alloc(ObReplaceSemanticFilter);
      break;
    case PHY_UPDATE_SEMANTIC_FILTER:
      ret = tc_rp_alloc(ObUpdateSemanticFilter);
      break;
  ...
  if (NULL != opt)
  {
    switch (opt->get_type())
    {
      ...
      case PHY_MULTIPLE_SCAN_MERGE:
        ...
        break;
      case PHY_REPLACE_COMPLETE_ROW_FUSE:
        tc_rp_free(dynamic_cast<ObReplaceCompleteRowFuse*>(opt));
        break;
      case PHY_REPLACE_SEMANTIC_FILTER:
        tc_rp_free(dynamic_cast<ObReplaceSemanticFilter*>(opt));
        break;
//      case PHY_INCOMPLETE_ROW_FILTER:
//        tc_rp_free(dynamic_cast<ObIncompleteRowFilter*>(opt));
//        break;
      case PHY_UPDATE_SEMANTIC_FILTER:
        tc_rp_free(dynamic_cast<ObUpdateSemanticFilter*>(opt));
        break;
      default:
        opt->~ObPhyOperator();
        break;
