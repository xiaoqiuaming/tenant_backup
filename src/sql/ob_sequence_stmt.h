  ...
  namespace sql
  {
    class ObSequenceStmt
    {
      public:
        ...
        //add 20150623:e
        bool has_sequence()
        {return (sequence_names_no_dup_.count() > 0);}

        bool is_next_type()
        {return is_next_type_;}

         void set_is_next_type(bool is_next_type)
         {is_next_type_ = is_next_type;}

        void set_current_expr_has_sequence(bool is_has_sequence)
        {current_expr_has_sequence_ = is_has_sequence;}

        bool current_expr_has_sequence()
        {return current_expr_has_sequence_;}
    ...
    inline int ObSequenceStmt::init_sequence_option()
    {
      ...
      {
        YYSYS_LOG(ERROR, "Default value of minvalue covert to decimal failed.");
      }
      else if (OB_SUCCESS != (ret = max_value_.from(OB_SEQUENCE_DEFAULT_MAX_VALUE)))
      {
        YYSYS_LOG(ERROR, "Default value of maxvalue covert to decimal failed.");
      }
      else if (OB_SUCCESS != (ret = start_with_.from(OB_SEQUENCE_DEFAULT_START_CHAR)))
      {
        YYSYS_LOG(ERROR, "Default value of start with covert to decimal failed.");
      }
      else if(OB_SUCCESS != (ret = increment_by_.from(OB_SEQUENCE_DEFAULT_START_CHAR)))
      {
        YYSYS_LOG(ERROR, "Default value of increment by covert to decimal failed.");
      }
      return ret;
    }
    ...
    inline int ObSequenceStmt::generate_decimal_value(uint8_t datatype, ObDecimal &res, int option)
    {
      ...
      if(0 == datatype)
      {
        if (MinValue == option)
        {
          ...
          {
            YYSYS_LOG(ERROR, "Convert default min value to deciaml failed.");
          }
        }
        else if(OB_SUCCESS != (ret = res.from(OB_SEQUENCE_DEFAULT_MAX_VALUE)))
        {
          YYSYS_LOG(ERROR, "Convert default max value to deciaml failed.");
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "Convert bound value to deciaml failed.");
        }
      }
      return ret;
    }
