...
int ObMergeDistinct::open()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to open child_op, ret=%d", ret);
  }
  else if (NULL == (last_row_buf_ = (char *)ob_malloc(OB_ROW_BUF_SIZE, 0)))
  {
    YYSYS_LOG(WARN, "fail to alloc %lu bytes memory", OB_ROW_BUF_SIZE);
...
int ObMergeDistinct::close()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to close child_op, ret=%d", ret);
  }
  return ret;
}
...
int ObMergeDistinct::get_next_row(const common::ObRow *&row)
{
  ...
  if (NULL == child_op_)
  {
    YYSYS_LOG(WARN, "child_op_ is not set");
    ...
    if (got_first_row_)
    {
      while(OB_SUCCESS == ret && !got_distinct_row)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to compare this_row and last_row_. ret=%d", ret);
          }
          else if (!equal)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to convert row to compact row. ret=%d" ,ret);
              break;
            }
            //YYSYS_LOG(DEBUG, "find a distinct row");
            break;
          }
          else
          {
            //YYSYS_LOG(DEBUG, "equal of this & last row. skip");
          }
        }
      }/* end while */
    }
  ...
  {
    YYSYS_LOG(WARN, "fail to get next row. ret=%d, got_first_row_=%d, got_distinct_row=%d",
...
int ObMergeDistinct::compare_equal(const common::ObRow *this_row, const common::ObRow *last_row, bool &result) const
{
  ...
  {
    YYSYS_LOG(WARN, "compared row invalid. this_row=%p, last_row=%p", this_row, last_row);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    for (i = 0; i < column_count; i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to get column from distinct_column_ array. ret=%d, i=%ld, column_count=%ld",
            ret, i, column_count);
        break;
      }
      else if (OB_SUCCESS != (ret = this_row->get_cell(column.table_id_, column.column_id_, this_obj)))
      {
        YYSYS_LOG(WARN,"fail to get cell. ret=%d, table_id=%lu, column_id=%lu",
      ...
      {
        YYSYS_LOG(WARN, "cell is null. this_obj=%p, last_obj=%p", this_obj, last_obj);
        ret = OB_ERROR;
        break;
      }
      else
...
int ObMergeDistinct::add_distinct_column(const uint64_t tid, const uint64_t cid)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to add new column(%lu, %lu) to distinct_columns_. ret=%d", tid, cid, ret);
  }
  return ret;
}
