...
int ObChangeObi::open()
{
  ...
  {
    YYSYS_LOG(WARN, "temporarily set ob query timeout failed, ret=%d", ret);
  }
  ...
  {
    YYSYS_LOG(WARN, "get rs failed, ret=%d", ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    if (!got_rs_)
    {
      ...
      YYSYS_LOG(ERROR, "get cluster failed, ret=%d", ret);
    }
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (role_.get_role() == ObiRole::MASTER)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check addr failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = change_cluster_to_master(target_server_)))
      {
        YYSYS_LOG(WARN, "change cluster to master cluster failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = check_if_cluster_is_master(target_server_)))
      {
        YYSYS_LOG(WARN, "cluster is not yet master, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = broadcast_master_cluster()))
      {
        YYSYS_LOG(WARN, "set new master cluster addr to all cluster failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = post_check()))
      {
        YYSYS_LOG(WARN, "post_check failed, ret=%d", ret);
      }
      if(OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "set master success!");
      }
      else
      {
        YYSYS_LOG(ERROR, "set master failed!");
      }
    }
    // SET_SLAVE
    else if (role_.get_role() == ObiRole::SLAVE)
    {
      if (OB_SUCCESS != (ret = check_addr()))
      {
        YYSYS_LOG(ERROR, "check_addr failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = change_cluster_to_slave(target_cluster_index_)))
      {
        YYSYS_LOG(WARN, "change cluster to slave failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = check_if_cluster_is_slave(target_cluster_index_)))
      {
        YYSYS_LOG(WARN, "cluster is not yet slave, ret=%d", ret);
      }
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "set slave success!");
      }
      else
      {
        YYSYS_LOG(ERROR, "set slave failed!");
      }
    }
    else // 主备切换
    {
      ...
      if (multi_master_cluster_)
      {
        ...
        YYSYS_LOG(ERROR, "mutiple master clusters exist, ret=%d, use ALTER SYSTEM SET_SLAVE SLAVE='' to change master to slave", ret);
      }
      else if (OB_SUCCESS != (ret = check_new_master_addr()))
      {
        YYSYS_LOG(WARN, "new master addr[%s:%d] invalid, ret=%d", target_server_ip_, target_server_port_, ret);
      }
      else if (OB_SUCCESS != (ret = check_ups_log()))
      {
        YYSYS_LOG(WARN, "master ups and slave ups not sync,ret=%d", ret);
      }
      else if (master_cluster_exists_)
      {
        if (OB_SUCCESS != (ret = change_cluster_to_slave(master_cluster_index_)))
        {
          YYSYS_LOG(WARN, "change master to slave failed, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = check_if_cluster_is_slave(master_cluster_index_)))
        {
          YYSYS_LOG(WARN, "old master is not yet slave, ret=%d", ret);
        }
      }
      ...
      {
        YYSYS_LOG(ERROR, "waiting old slave ups sync failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = change_cluster_to_master(target_server_)))
      {
        YYSYS_LOG(WARN, "change slave cluster to master cluster failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = check_if_cluster_is_master(target_server_)))
      {
        YYSYS_LOG(WARN, "old slave is not yet master, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = broadcast_master_cluster()))
      {
        YYSYS_LOG(WARN, "set new master rs addr to all cluster failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = post_check()))
      {
        YYSYS_LOG(WARN, "post_check failed, ret=%d", ret);
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "change obi role success!");
      }
      else
      {
        YYSYS_LOG(ERROR, "change obi role failed, ret=%d", ret);
      }
    }
  }
...
int ObChangeObi::close()
{
  ...
  {
    YYSYS_LOG(WARN, "restore ob query timeout failed, ret=%d", ret);
  }
  return ret;
}
void ObChangeObi::get_master_ups()
{
  ...
  for (int64_t i = 0;i < cluster_count_; ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "get master ups from rootserver[%s] failed, ret=%d", to_cstring(all_rs_ups_[i].master_rs_), ret);
    }
    else
    {
...
int ObChangeObi::get_rs()
{
  ...
  {
    YYSYS_LOG(WARN, "direct execute sql=%.*s failed, ret=%d", get_cluster.length(), get_cluster.ptr(), ret);
  }
  ...
  else
  {
    ...
    while ((OB_SUCCESS == ret) && (OB_SUCCESS == (ret = result.get_next_row(row))))
    {
      ...
      {
        YYSYS_LOG(WARN, "pcell->get_int failed, ret=%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "pcell->get_varchar failed, ret=%d", ret);
      }
      ...
      else
      {
        ...
        if (cluster_id >= 0)
        {
          ...
          if (cluster_id == 0)
          {
            YYSYS_LOG(WARN, "cluster id == 0, it should be greater than 0");
          }
          ...
          if (!strncmp(ip_str, target_server_ip_, strlen(ip_str)))
          {
            ...
            if (static_cast<int32_t>(rootserver_port) != target_server_port_)
            {
              ...
              YYSYS_LOG(WARN, "input cluster addr[%s:%d] is not a valid cluster,ret=%d", target_server_ip_, target_server_port_, ret);
            }
          }
        }
        else
        {
          ...
          YYSYS_LOG(WARN, "cluster_role = %ld, not expected", cluster_role);
        }
      }
    }// while
    if (ret == OB_ITER_END)
    ...
    {
      YYSYS_LOG(WARN, "failed to close result set, ret=%d", ret);
    }
    result.reset();
  }
...
int ObChangeObi::get_master_rs_by_rpc(int64_t &master_index)
{
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < cluster_count_; i++)
  {
    ...
    if (OB_SUCCESS != ret2)
    {
      YYSYS_LOG(WARN, "get obi role from [%s] failed, ret %d",
          to_cstring(all_rs_ups_[i].master_rs_), ret);
    }
    else
    {
      YYSYS_LOG(INFO, "rootserver %s role %s", to_cstring(all_rs_ups_[i].master_rs_),
          role.get_role_str());
      if (role.get_role() == ObiRole::MASTER)
      {
        ...
        {
          YYSYS_LOG(WARN, "multi master cluster %s and %s",
...
int ObChangeObi::check_new_master_addr()
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "master_cluster_index_=%ld cluster_count_=%ld", master_cluster_index_, cluster_count_);
  }
  else
  {
    if (force_)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(ERROR, "input new master addr[%s:%d] is not a valid cluster, ret=%d",
                  target_server_ip_, target_server_port_, ret);
      }
    }
    else
    {
      ...
      if (master_cluster_index_ == -1)
      {
        ...
        YYSYS_LOG(ERROR, "master cluster not exist, force is false, normal change obi role failed, ret=%d", ret);
      }
      else
      {
        //普通切，并且存在主集群
            ...
            && all_rs_ups_[master_cluster_index_].master_rs_.get_port() == target_server_port_)
        {
          ...
          YYSYS_LOG(WARN, "new_master_ip[%s] is already master cluster, ret=%d", target_server_ip_, ret);
        }
        else
        {
          bool found = false;
          ...
          if (!found)
          {
            ...
            YYSYS_LOG(WARN, "new_master_ip[%s:%d] is not a slave cluster, ret=%d", target_server_ip_, target_server_port_, ret);
          }
        }
      }
    }
...
int ObChangeObi::check_ups_log()
{
  ...
  else
  {
    ...
    {
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "master_cluster_index_=%ld cluster_count_=%ld ret=%d", master_cluster_index_, cluster_count_, ret);
    }
    else if (!all_rs_ups_[master_cluster_index_].got_master_ups_ || !all_rs_ups_[target_cluster_index_].got_master_ups_)
    {
      ...
      YYSYS_LOG(ERROR, "get master ups failed, get old master cluster master ups[%s], get target cluster master ups[%s], ret=%d",
    ...
    else
    {
      ...
      for (int64_t i = 0;i < RETRY_TIME; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "get master ups log seq from ups[%s] failed, ret=%d",
                    to_cstring(all_rs_ups_[master_cluster_index_].master_ups_), ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "get slave ups log seq from ups[%s] failed, ret=%d", to_cstring(all_rs_ups_[target_cluster_index_].master_ups_), ret);
          }
          else if (slave_ups_log_seq < master_ups_log_seq)
          {
            ret = OB_LOG_NOT_SYNC;
            YYSYS_LOG(WARN, "master ups and slave ups no sync, slave_ups_log_seq=%ld, master_ups_log_seq=%ld ret=%d",
                slave_ups_log_seq, master_ups_log_seq, ret);
          }
          else
          {
            YYSYS_LOG(INFO, "master ups and slave ups sync");
            break;
          }
        }
      }
...
int ObChangeObi::change_cluster_to_slave(int64_t cluster_index)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "cluster_index=%ld cluster_count_=%ld ret=%d", cluster_index, cluster_count_, ret);
  }
  else if (OB_SUCCESS != (ret = set_slave_in_inner_table(cluster_index)))
  {
    YYSYS_LOG(WARN, "set cluster to slave in __all_cluster table failed, ret %d", ret);
  }
  else
  {
    ObiRole obi_role;
    ...
    if (OB_SUCCESS != (ret = context_->rs_rpc_proxy_->set_obi_role(all_rs_ups_[cluster_index].master_rs_, timeout, obi_role)))
    {
      if (force_)
      {
        YYSYS_LOG(WARN, "set obi role[slave] to cluster[%s] failed, but force is true, continue", to_cstring(all_rs_ups_[cluster_index].master_rs_));
        ret = OB_SUCCESS;
      }
      else
      {
        YYSYS_LOG(WARN, "set obi role[slave] to cluster[%s] failed, ret=%d", to_cstring(all_rs_ups_[cluster_index].master_rs_), ret);
      }
    }
    else
    {
      YYSYS_LOG(INFO, "change cluster[%s] to slave finished, wait to check", to_cstring(all_rs_ups_[cluster_index].master_rs_));
    }
  }
  return ret;
}
...
int ObChangeObi::set_slave_in_inner_table(int64_t cluster_index)
{
  ...
  else
  {
    ...
    if (n < 0 || n >= static_cast<int>(sizeof(sql)))
    {
      ...
      YYSYS_LOG(WARN, "snprintf failed, n %d, errno %d", n, n < 0 ? errno : 0);
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "result set init failed, ret %d", ret);
      }
      else if (OB_SUCCESS != (ret = ObSql::direct_execute(sql_str, rs, *context_)))
      {
        YYSYS_LOG(WARN, "direct execute sql %s failed, ret %d", sql, ret);
      }
      ...
      {
        YYSYS_LOG(INFO, "execute %s success, affect row %ld", sql, rs.get_affected_rows());
      }
      int ret2 = rs.close();
      if (OB_SUCCESS != ret2)
      {
        YYSYS_LOG(WARN, "close result failed, ret %d", ret2);
      }
      rs.reset();
      context_->session_info_->set_current_result_set(result_set_out_);
      ...
      {
        ret = OB_SUCCESS;
        YYSYS_LOG(INFO, "ignore set cluster %ld to slave error %d for force operation",
...
int ObChangeObi::check_if_cluster_is_slave(int64_t cluster_index)
{
  ...
  {
    YYSYS_LOG(WARN, "get obi role from cluster[%s] failed, ret=%d", to_cstring(all_rs_ups_[cluster_index].master_rs_), ret);
  }
  else
  {
    if (obi_role.get_role() != ObiRole::SLAVE)
    {
      ...
      YYSYS_LOG(WARN, "cluster[%s] is not yet slave, role is[%s]", to_cstring(all_rs_ups_[cluster_index].master_rs_), obi_role.get_role() == ObiRole::MASTER ? "master" : "init");
    }
    else
    {
      YYSYS_LOG(INFO, "cluster[%s] is slave now", to_cstring(all_rs_ups_[cluster_index].master_rs_));
    }
  }
  if (OB_SUCCESS != ret)
  {
    ...
    {
      YYSYS_LOG(WARN,
                "cluster[%s] is not yet slave, role is[%s], but force is true, continue",
                to_cstring(all_rs_ups_[cluster_index].master_rs_),
                obi_role.get_role() == ObiRole::MASTER ? "master" : "init");
      ret = OB_SUCCESS;
    }
  }
  return ret;
...
int ObChangeObi::waiting_old_slave_ups_sync()
{
  ...
  {
    // do nothing
    YYSYS_LOG(INFO, "force change obi role, DO NOT CHECK UPS LOG");
  }
  else if (!all_rs_ups_[master_cluster_index_].got_master_ups_ || !all_rs_ups_[target_cluster_index_].got_master_ups_)
  {
    ret = OB_GET_CLUSTER_MASTER_UPS_FAILED;
  ...
  else
  {
    ...
    int64_t ed = yysys::CTimeUtil::getTime() + check_ups_log_interval_ * 1000 * 1000;
    int64_t timeout = TIMEOUT;
    int64_t master_ups_log_seq = -1;
    int64_t slave_ups_log_seq = -1;
    if (OB_SUCCESS != (ret = context_->merger_rpc_proxy_->get_ups_log_seq(all_rs_ups_[master_cluster_index_].master_ups_,
            timeout, master_ups_log_seq)))
    {
      YYSYS_LOG(WARN, "get master ups log seq from ups[%s] failed, ret=%d", to_cstring(all_rs_ups_[master_cluster_index_].master_ups_), ret);
    }
    else
    {
      st = yysys::CTimeUtil::getTime();
      while(st <= ed)
      {
        if (OB_SUCCESS != (ret = context_->merger_rpc_proxy_->get_ups_log_seq(all_rs_ups_[target_cluster_index_].master_ups_,
                timeout, slave_ups_log_seq)))
      ...
      if (st > ed)
      {
        ...
        YYSYS_LOG(ERROR, "master master ups and slave master ups not sync, ret=%d", ret);
      }
    }
  }
  return ret;
...
int ObChangeObi::change_cluster_to_master(const ObServer &target_server)
{
  ...
  {
    YYSYS_LOG(WARN, "set obi role [master] to cluster[%s] failed, ret=%d", to_cstring(target_server), ret);
  }
  else
  {
    YYSYS_LOG(INFO, "change cluster[%s] to master finished, wait to check", to_cstring(target_server));
  }
  return ret;
}
int ObChangeObi::check_if_cluster_is_master(const ObServer &target_server)
{
  ...
  {
    YYSYS_LOG(WARN, "get obi role from cluster[%s] failed, ret=%d", to_cstring(target_server), ret);
  }
  else
  {
    if (obi_role.get_role() != ObiRole::MASTER)
    {
      ...
      YYSYS_LOG(WARN, "cluster[%s] is not yet master, role is[%s]", to_cstring(target_server), obi_role.get_role() == ObiRole::SLAVE ? "slave" : "init");
    }
    else
    {
      YYSYS_LOG(INFO, "cluster[%s] is master now", to_cstring(target_server));
    }
  }
  return ret;
}
int ObChangeObi::set_master_rs_vip_port_to_all_cluster()
{
  ...
  if (OB_SUCCESS != (ret = context_->rs_rpc_proxy_->set_master_rs_vip_port_to_cluster(target_server_, timeout, target_server_ip_, target_server_.get_port())))
  {
    ...
    YYSYS_LOG(WARN, "set new master cluster vip[%s] and port[%d] to self cluster[%s] failed, ret=%d", target_server_ip_, target_server_.get_port(), to_cstring(target_server_), ret);
    YYSYS_LOG(ERROR, "set new master cluster vip[%s] and port[%d] to self cluster[%s] failed, ret=%d", target_server_ip_, target_server_.get_port(), to_cstring(target_server_), ret);
  }
  else
  {
    YYSYS_LOG(INFO, "set new master cluster addr[%s:%d] to server[%s] success", target_server_ip_, target_server_.get_port(), to_cstring(target_server_));
  }
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = 0;i < cluster_count_; ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "set new master cluster vip[%s] and port[%d] to cluster[%s] failed, ret=%d", target_server_ip_, target_server_.get_port(), to_cstring(all_rs_ups_[i].master_rs_), ret);
      }
      else
      {
        YYSYS_LOG(INFO, "set new master cluster addr[%s:%d] to server[%s] success", target_server_ip_, target_server_.get_port(), to_cstring(all_rs_ups_[i].master_rs_));
      }
    }
    if (OB_SUCCESS != ret)
    {
...
int ObChangeObi::broadcast_master_cluster()
{
  ...
  do
  {
    ...
    {
      YYSYS_LOG(WARN, "set_master_rs_vip_port_to_all_cluster failed, ret %d, master cluster rs [%s:%d",
  ...
  {
    // WARN log for mysql warnings.
    YYSYS_LOG(WARN, "broadcast master cluster rs vip:port [%s:%d] to all clusters failed, ret %d. "
    ...
    YYSYS_LOG(ERROR, "broadcast master cluster rs vip:port [%s:%d] to all clusters failed, ret %d. "
        "You can try to recover this by re-execute the SQL with force flag or "
        "execute the follow command on all master rootserver: "
        " rs_admin -r rs_ip -p rs_port set_config -o master_root_server_ip='%s',master_root_server_port='%d'",
        target_server_ip_, target_server_.get_port(), ret, target_server_ip_, target_server_.get_port());
...
int ObChangeObi::ms_renew_master_master_ups()
{
  ...
  {
    YYSYS_LOG(WARN, "renew new master master ups failed, ret=%d", ret);
  }
  else 
  {
    YYSYS_LOG(INFO, "ms renew new master master ups[%s] success", to_cstring(new_master_master_ups));
  }
  return ret;
}
int ObChangeObi::update_all_sys_config()
{
  ...
  int cnt = snprintf(str, sizeof(str), "alter system set master_yaoadminsvr_ip='%s' server_type=yaoadminsvr,master__port=%d server_type=yaoadminsvr",
                     target_server_ip_, target_server_.get_port());
  ObString alter_sql;
  ...
  {
    YYSYS_LOG(WARN, "direct execute sql=%.*s failed, ret=%d", alter_sql.length(), alter_sql.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = result.open()))
  {
    YYSYS_LOG(WARN, "open result failed, ret=%d", ret);
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "execute %.*s success", alter_sql.length(), alter_sql.ptr());
    int tmp_err = result.close();
    if (OB_SUCCESS != tmp_err)
    {
      YYSYS_LOG(WARN, "failed to close result set, ret=%d", ret);
...
int ObChangeObi::check_master_rs_vip_port()
{
  ...
  {
    YYSYS_LOG(WARN, "direct execute sql=%.*s failed, ret=%d", sql.length(), sql.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, ret=%d", ret);
  ...
  else
  {
    ...
    while ((OB_SUCCESS == ret) && (OB_SUCCESS == (ret = result.get_next_row(row))))
    {
      ...
      {
        YYSYS_LOG(WARN, "get varchar from ObObj failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = row->raw_get_cell(1, pcell, table_id, column_id)))
      {
        YYSYS_LOG(WARN, "raw get cell failed, ret=%d", ret);
      ...
      else
      {
        if (name == "master_root_server_ip")
        {
          if (value != target_server_ip_)
          {
            ...
            YYSYS_LOG(ERROR, "config item:name[%.*s] in __all_sys_config_stat not sync, it should be %s, but is %.*s ret=%d",
        ...
        else if (name == "master_root_server_port")
        {
          ...
          {
            ret = OB_CONFIG_NOT_SYNC;
            YYSYS_LOG(ERROR, "config item:name[%.*s] in __all_sys_config_stat not sync, ret=%d",
        ...
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "column value not expected, name=%.*s ret=%d", name.length(), name.ptr(), ret);
          break;
        }
      }
    }
...
int ObChangeObi::execute_sql(const ObString &sql)
{
  ...
  if (OB_SUCCESS != (ret = result.init()))
  {
    YYSYS_LOG(WARN, "result init failed, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = ObSql::direct_execute(sql, result, *context_)))
  {
    YYSYS_LOG(WARN, "direct execute %.*s failed, ret=%d", sql.length(), sql.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, ret=%d", ret);
...
int ObChangeObi::set_target_server_addr(const ObString &target_server)
{
  ...
  if (target_server.length() < MAX_IP_PORT_LENGTH)
  {
    ...
    {
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN, "input new master addr[%s] invalid, ret=%d", buf, ret);
    }
    else
    {
      ...
      else
      {
        ...
        {
          ret = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "set_ipv4_addr failed,ret=%d", ret);
        }
      }
    }
  }
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "length of %.*s invalid, ret=%d", target_server.length(), target_server.ptr(), ret);
  }
  return ret;
}
...
int ObChangeObi::check_addr()
{
  ...
  {
    ret = OB_IP_PORT_IS_NOT_CLUSTER;
    YYSYS_LOG(ERROR, "input cluster[%s:%d] is not a valid cluster, ret=%d", target_server_ip_, target_server_port_, ret);
  }
  return ret;
}
int ObChangeObi::post_check()
{
  ...
  while (start_time <= end_time)
  {
    ...
    {
      YYSYS_LOG(WARN, "ms renew master master ups failed, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = update_all_sys_config()))
    {
      YYSYS_LOG(WARN, "update __all_sys_config failed, ret=%d", ret);
    }
    ...
    else
    {
      ...
      if (!force_ && role_.get_role() == ObiRole::INIT)
      {
        ...
        {
          YYSYS_LOG(WARN, "check_master_rs_vip_port failed, ret=%d", ret);
        }
        else
        {
          break;
    ...
    sleep(2);
    start_time = yysys::CTimeUtil::getTime();
  }// while
  return ret;
}
