...
ObSemiJoin::ObSemiJoin()
  :get_next_row_func_(NULL),
    ...
    hashmap_num_(0),
    left_cache_for_store_(ObModIds::OB_SEMI_JOIN_LEFT_CACHE),
    right_cache_for_store_(ObModIds::OB_SEMI_JOIN_RIGHT_CACHE),
    ...
    second_index_table_id_(OB_INVALID_ID), left_row_desc_(NULL)
...
int ObSemiJoin::close()
{
  ...
  left_row_desc_ = NULL;
  if (!is_can_use_semi_join_ && NULL != right_table_filter_)
  {
      right_table_filter_->reset();
      right_table_filter_->~ObFilter();
      right_table_filter_ = NULL;
  }
  is_can_use_semi_join_ = true;
  id_ = 0;
...
//        {
//            YYSYS_LOG(WARN, "hash create failed ,ret=[%d]", ret);
//        }
//        else
//        {
...
int ObSemiJoin::set_alias_table (uint64_t tid, ObSelectStmt *&stmt, bool aliasT)
{
  ...
  {
    YYSYS_LOG(ERROR, "Select Stmt or tid has not been init");
    ret = OB_ERR_UNEXPECTED;
  }
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "table[%ld] not belong to this stmt[%p]", tid, stmt);
      ret = OB_ERR_UNEXPECTED;
    }
    ...
    {
      YYSYS_LOG(WARN, "can't be here! aliasT[%s]", aliasT ? "true" : "false");
      ret = OB_ERROR;
    }
  }
...
//  int filter_data_size = 0;
//  int64_t total_size = 0;
//  //Ŀǰ��ʱʹ�ÿ⺯��map����������ob�Դ���hashset��
//  map<string,bool> is_confilct;
//  //��δ�����ʹ��ob��hash map������ȥ�أ�Ŀǰ��������:b
//  //    int curr_buket = 0;
...
{
  if (NULL != right_table_filter_)
  {
      YYSYS_LOG(WARN, "right_table_filter_ has been inited.should release memory first!");
  }
...
int ObSemiJoin::add_filter_to_rpc_scan(int &index_param_count,ObSqlExpression *&table_filter_expr,ObSqlExpression *&src_table_filter_expr,const int flag)
{
          ...
          case PHY_TABLE_MEM_SCAN:
          {
            ...
            while(1)
            {
              ...
              else
              {
                ...
                YYSYS_LOG(WARN,"table rpc scan operator is null! ret=%d",OB_ERR_POINTER_IS_NULL);
              }
            }
          }
            break;
  ...
  else
  {
    ...
    ret = OB_ERR_POINTER_IS_NULL;
    //ret = OB_SUCCESS;//for UT
    YYSYS_LOG(WARN,"sort operator is null! ret=%d",OB_ERR_POINTER_IS_NULL);
...
int ObSemiJoin::add_sort_column(const uint64_t tid, const uint64_t cid, bool is_ascending_order)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to push back to array, err=%d", ret);
  }
  return ret;
}
void ObSemiJoin::set_mem_size_limit(const int64_t limit)
{
  //YYSYS_LOG(INFO, "sort mem limit=%ld", limit);
...
{
  UNUSED(filename);
  //YYSYS_LOG(INFO, "sort run file=%.*s", filename.length(), filename.ptr());
...
int ObSemiJoin::store_row_in_merge_sort(const common::ObRow *&input_row, bool &need_merge)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to set sort columns for in_mem_sort. ret=%d", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to add row, err=%d", ret);
    }
    else if (need_dump())
    {
      if (OB_SUCCESS != (ret = merge_sort_.dump_run(in_mem_sort_)))
      {
        YYSYS_LOG(WARN, "failed to dump, err=%d", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "need merge sort");
        in_mem_sort_.reset();
        need_merge = true;
    ...
    if (OB_SUCCESS == ret)
    {
      if (need_merge && 0 < in_mem_sort_.get_row_count())
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to build heap, err=%d", ret);
        }
      }
    }
  }
...
int ObSemiJoin::open()
{
  ...
  else
  {
      ...
      case LEFT_OUTER_JOIN:
        ...
        break;
      case RIGHT_OUTER_JOIN:
        //��ʱ�ر�right semi join�ӿڣ����޸�
        //ret=semi_join_open_right();
        ret = OB_ERR_FUNC_DEV;
        break;
      case LEFT_ANTI_SEMI_JOIN:
        //��ʱ�ر�left anti semi join�ӿڣ����޸�
        ret = OB_ERR_FUNC_DEV;
        break;
      case RIGHT_ANTI_SEMI_JOIN:
        //��ʱ�ر�right anti semi join�ӿڣ����޸�
        ret = OB_ERR_FUNC_DEV;
        break;
      default:
        ret = OB_ERR_UNEXPECTED;
...
int ObSemiJoin::semi_join_open_right()
{
  ...
  {
    YYSYS_LOG(WARN, "get table id and table column failed, err=%d", ret);
  }
      ...
      else
      {
        if(is_can_use_semi_join_)
        {
          if(OB_SUCCESS != (ret = cons_filter_for_left_table(table_filter_expr_,
                                                             ...
                                                             right_join_cid)))
          {
            YYSYS_LOG(WARN, "failed to construct filter for right table, err=%d", ret);
          }
          else
          {
            if(OB_SUCCESS != (ret = add_filter_to_rpc_scan(index_param_count,src_table_filter_expr_,table_filter_expr_,0)))
            {
              YYSYS_LOG(WARN, "failed to add filter to right table, err=%d", ret);
            }
          }
        }
        else
  ...
  if(OB_SUCCESS != ret)
  {
    if(OB_ERR_INDEX_NUM_IS_ZERO == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to open right, err=%d", ret);
      }
    ...
    {
      YYSYS_LOG(WARN, "failed to pass filter , err=%d", ret);
    }
  }
  else
  {
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(ret!=OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "out of time, err=%d", ret);
      }
      else
      {
...
int ObSemiJoin::semi_join_open()
{
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        is_row_empty = left_cache_for_store_.is_empty();
        if(ret == OB_ITER_END)
        {
          ret = OB_SUCCESS;
        }
        if(ret == OB_SUCCESS && is_can_use_semi_join_)
        {
          ...
          if(sort_query == NULL)
          {
            YYSYS_LOG(WARN,"get sort op failed!");
          }
          else
          {
            if(is_row_empty)
            {
                dynamic_cast<ObSort *>(sort_query)->set_is_left_table_empty(is_row_empty);
            }
            //Ŀǰֻ����sort�����¹ҵ���table rpc scan������������memory scan
            //������multi bind�Ȳ������Ͳ���ʹ��semi join
            main_query = dynamic_cast<ObTableRpcScan *>(sort_query->get_child(0));
            if(main_query == NULL)
            {
              is_can_use_semi_join_ = false;
              YYSYS_LOG(WARN,"[semi join]get ObTableRpcScan op failed!can not use semi join"\
                        ",this is just a warning! right table is memory table or sub query!phy_type=[%d]",
                        sort_query->get_child(0)->get_type());
            }
            else
            {
              ...
              {
                if(is_use_second_index_without_storing()) //�ر�
                {
                  main_query->get_rpc_scan ().set_left_table_id_column_id (
                        left_join_tid.at (id_),
                        left_join_cid.at (id_),
  ...
  if(ret == OB_SUCCESS)
  {
    //YYSYS_LOG(ERROR,"wanglei ::right_table_filter_:%s ",to_cstring(*right_table_filter_));
    ...
    if(ret == OB_SUCCESS)
    {
      ...
      //YYSYS_LOG(ERROR, "right op is %s", to_cstring(*right_op_));
...
int ObSemiJoin::curr_row_is_qualified(bool &is_qualified)
{
  ...
  //YYSYS_LOG(ERROR, "wl other_join_conds_ is=[%ld]",other_join_conds_.count());
  for (int64_t i = 0; i < other_join_conds_.count(); ++i)
  {
    ObSqlExpression &expr = other_join_conds_.at(i);
    //YYSYS_LOG(ERROR, "wl ObSqlExpression %ld ,is=[%s]",i,to_cstring(expr));
    //add tianz [SubQuery_for_Instmt] [JHOBv0.1] 20140610:b
