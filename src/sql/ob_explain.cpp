...
int ObExplain::open()
{
  ...
  if (child_op_ == NULL)
  {
    YYSYS_LOG(WARN, "explain operator must have a child operator");
    ret = OB_ERR_GEN_PLAN;
  }
...
int ObExplain::get_next_row(const common::ObRow *&row)
{
  ...
  else
  {
    ...
    if (verbose_)
    {
      ...
      // FIX ME
      databuff_printf(json_buf, OB_MAX_LOG_BUFFER_SIZE, pos, "\n");
      pos += strlen("\n");
    }
    pos += to_string(json_buf + pos, OB_MAX_LOG_BUFFER_SIZE - pos);
    ObString str(static_cast<int32_t>(pos), static_cast<int32_t>(pos), json_buf);
    ObString stored_str;
    if ((ret = str_buf_.write_string(str, &stored_str)) != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "failed to make space for explain string, err=%d", ret);
    }
    else
    {
