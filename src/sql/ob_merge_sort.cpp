...
void ObMergeSort::reset()
{
  ...
  if (run_file_.is_opened())
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to close run file, err=%d", ret);
    }
  }
  if ('\0' != run_filename_buf_[0])
  {
    ...
    if (0 == stat(run_filename_buf_, &stat_buf))
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to remove tmp run file, err=%s", strerror(errno));
      }
    }
  }
  heap_array_.clear();
...
int ObMergeSort::set_run_filename(const common::ObString &filename)
{
  ...
  {
    YYSYS_LOG(ERROR, "filename is too long, filename=%.*s", filename.length(), filename.ptr());
...
int ObMergeSort::dump_run(ObInMemorySort &rows)
{
  ...
  if (!run_file_.is_opened())
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to open run file, err=%d filename=%.*s",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to begin dump run, err=%d", ret);
    }
    else
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to end dump run, err=%d", ret);
        }
        else
        {
          if (NULL == row_desc_)
          ...
          YYSYS_LOG(INFO, "dump run, row_count=%ld", rows.get_row_count());
        }
      }
    }
  }
...
struct ObMergeSort::HeapComparer
{
  ...
  bool operator()(const RowRun &r1, const RowRun &r2) const
  {
    ...
    for (int32_t i = 0; i < sort_columns_.count(); ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get cell, err=%d", ret);
        break;
      }
...
int ObMergeSort::build_merge_heap()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to begin to read backet, err=%d", ret);
  }
  else
  {
    heap_array_.clear();
    ...
    for (int64_t i = 0; i < dump_run_count_; ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to read next row, err=%d i=%ld", ret, i);
        break;
      }
  ...
  if (OB_SUCCESS == ret)
  {
    if (NULL != final_run_ && 0 < final_run_->get_row_count())
    {
      ...
      {
        YYSYS_LOG(WARN, "failed th get the first row, err=%d", ret);
      }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(INFO, "build merge heap, size=%ld sort_columns_count=%ld",
...
int ObMergeSort::get_next_row(const common::ObRow *&row)
{
  ...
  if (0 >= heap_array_.count())
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to end read backet, err=%d", ret);
    }
    ret = OB_ITER_END;
    YYSYS_LOG(INFO, "end of merge sort");
  }
  else if (need_push_heap_)
  {
    ...
    if (run_idx < dump_run_count_)
    {
      if (OB_SUCCESS != (ret = run_file_.get_next_row(run_idx, last_pop_row->first)))
      {
        if (OB_ITER_END == ret)
        {
          ...
          YYSYS_LOG(INFO, "end of dumped run, run_idx=%ld run_count=%ld", run_idx, heap_array_.count());
        }
        else
        {
          YYSYS_LOG(WARN, "failed to read next row, err=%d i=%ld", ret, run_idx);
        }
      }
    }
    else
    {
      ...
      if (OB_SUCCESS != (ret = final_run_->get_next_row(last_pop_row->first)))
      {
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(INFO, "end of the in-memory run, run_idx=%ld run_count=%ld", run_idx, heap_array_.count());
          heap_array_.pop_back();
        }
        else
        {
          YYSYS_LOG(WARN, "failed to get next row, err=%d i=%ld", ret, run_idx);
        }
      }
    }
    // push heap
