...
void ObAggregateFunction::reset()
{
  if (NULL != aggr_columns_)
  {
    ...
    for (int64_t i = 0; i < aggr_columns_->count(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get aggr column, err=%d", ret);
      }
      else if (is_distinct)
      {
        dedup_sets_[i].clear();
...
int ObAggregateFunction::init(const ObRowDesc &input_row_desc, ObExpressionArray &aggr_columns, bool is_analytic)
{
  ...
  for (int64_t i = 0;i < row_desc_.get_column_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to add cell to aggre_cells, ret=%d", ret);
      break;
    }
    else if (OB_SUCCESS != (ret = aux_cells_.push_back(empty_expr_obj_)))
    {
      YYSYS_LOG(ERROR, "failed to add cell to aux_cells_, ret=%d", ret);
      break;
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to reset curr_row_:ret[%d]", ret);
    }
    else if (OB_SUCCESS != (ret = init_dedup_sets()))
    {
      YYSYS_LOG(WARN, "fail to init_dedup_sets:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObAggregateFunction::clone_expr_cell(const int64_t aggr_idx, const ObExprObj &cell, ObExprObj &cell_clone)
{
  ...
  if (ObVarcharType == cell.get_type())
  {
    ...
    if (varchar.length() > OB_MAX_VARCHAR_LENGTH)
    {
      ...
      YYSYS_LOG(ERROR, "varchar too long, length=%d", varchar.length());
    }
    ...
    else
    {
      ...
      if (varchar.length() != varchar_clone.write(varchar.ptr(), varchar.length()))
      {
        ...
        YYSYS_LOG(ERROR, "failed to write varchar, length=%d", varchar.length());
      }
      else
      {
    ...
    else if (ObDecimalType == cell.get_type())
  {
    ...
    {
      ret = OB_SIZE_OVERFLOW;
      YYSYS_LOG(ERROR, "vardecimal too long, length=%d", vardecimal.length());
    }
    else if (NULL == (buff = get_varchar_buff(aggr_idx)))
    {
      YYSYS_LOG(ERROR, "no memory");
    ...
    else
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "failed to write varchar, length=%d", vardecimal.length());
      }
      else
      {
...
int ObAggregateFunction::prepare(const ObRow &input_row)
{
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < input_row.get_column_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to clone cell, err=%d", ret);
    }
  } // end for
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < aggr_columns_->count(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to get calc cell, err=%d", ret);
    }
    ...
    {
      YYSYS_LOG(WARN, "failed to init cell, err=%d", ret);
    }
    else if (is_distinct)
    {
      has_distinct = true;
      ...
      {
        YYSYS_LOG(WARN, "failed to set row cell, err=%d tid=%lu cid=%lu", ret, tid, cid);
      }
    }
  } // end for
  if (OB_SUCCESS == ret && has_distinct)
  {
    ...
    else
    {
      for (int64_t i = 0; i < dedup_row.get_column_num(); ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to clear hash set, err=%d idx=%ld", ret, i);
        }
        else if (hash::HASH_INSERT_SUCC != (ret = dedup_sets_[i].set(&stored_row->reserved_cells_[i])))
        {
          YYSYS_LOG(WARN, "failed to insert into hash set, err=%d", ret);
        }
        else
        {
          ret = OB_SUCCESS;
  ...
  if (0 < dedup_row_desc_.get_column_num())
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < aggr_columns_->count(); ++i)
    {
      ...
      else if (is_distinct)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to calc cell, err=%d", ret);
        }
        ...
        {
          YYSYS_LOG(ERROR, "unexpected branch, hash_ret=%d", ret);
          ret = OB_ERR_UNEXPECTED;
        }
      }
    } // end for
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < aggr_columns_->count(); ++i)
  {
    ...
    else if (is_distinct)
    {
      ...
      if (is_qualified[dedup_cell_idx])
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get raw cell, err=%d", ret);
        }
        else if (OB_SUCCESS != (ret = calc_aggr_cell(i, aggr_fun, *input_cell, *aggr_cell, *aux_cell)))
        {
          YYSYS_LOG(WARN, "failed to calculate aggr cell, err=%d", ret);
        }
      }
      ++dedup_cell_idx;
    }
                                        ...
                                        const ObObj &oprand, ObExprObj &res1, ObExprObj &res2)
{
  ...
  switch(aggr_fun)
  {
    ...
    default:
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "unknown aggr function type, t=%d", aggr_fun);
      break;
  }
  return ret;
}
        ...
        case T_FUN_LISTAGG:
          ...
          else
          {
              if (OB_SUCCESS == ret)
              {
                  ...
                  if (listagg_param_delimeter_.count() == 0)
                  {
                    res1.to_string_v2(pre_value);
                    oprand_clone.to_string_v2(next_value);
                    temp_result = pre_value + next_value;
                  }
                  else
                  {
                    ...
                    res1.to_string_v2(pre_value);
                    oprand_clone.to_string_v2(next_value);
                    snprintf(delimter_temp,
                             (listagg_param_delimeter_.at(aggr_idx)).length()+1,
                             "%.*s",
                             (listagg_param_delimeter_.at(aggr_idx)).length()+1,
                             (listagg_param_delimeter_.at(aggr_idx)).ptr());
                    delimeter = delimter_temp;
                    temp_result = pre_value + delimeter + next_value;
                  }
        ...
        case T_FUN_LISTAGG:// add by gaojt
          ...
          else
          {
            ...
            if(ObDecimalType==aggr_cell->get_type())
            {
                ...
                TTInt *tt = od.get_words();
                src_cell.set_decimal(tt->ToUInt_v2(), od.get_precision(), od.get_scale(), od.get_vscale(), len);

                if(OB_SUCCESS!=(ret=ob_write_obj(curr_row_buff_, src_cell, *res_cell)))
                {
                    YYSYS_LOG(ERROR,"failed to get result obj to res_cell");
                }
             }
             else
             //add e
...
int ObAggregateFunction::init_dedup_sets()
{
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < aggr_columns_->count(); ++i)
  {
    ...
    {
      ret = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "too many aggr column, i=%ld max=%ld", i, common::OB_ROW_MAX_COLUMNS_COUNT);
    }
    else if (is_distinct)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to add reserved column, err=%d", ret);
      }
      else if (OB_SUCCESS != (ret = dedup_row_desc_.add_column_desc(tid, cid)))
      {
        YYSYS_LOG(WARN, "failed to add column desc, err=%d", ret);
          ...
          {
            YYSYS_LOG(WARN, "failed to create hash set, err=%d", ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "create dedup set, i=%ld dedup_cell_idx=%ld tid=%lu cid=%lu",
...
void ObAggregateFunction::destroy_dedup_sets()
{
  ...
  for (int64_t i = 0; i < dedup_row_desc_.get_column_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to destroy hashset, i=%ld", i);
    }
  }
  dedup_row_desc_.reset();
}
