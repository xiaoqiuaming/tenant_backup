...
void *parse_malloc(const size_t nbyte, void *malloc_pool)
{
  ...
  if (alloc_buf == NULL)
  {
    YYSYS_LOG(ERROR, "parse_malloc gets string buffer error");
  }
  else if (nbyte <= 0)
  {
    YYSYS_LOG(ERROR, "wrong size %ld of parse_malloc", nbyte);
  }
  else if (NULL == (ptr = alloc_buf->alloc(headlen + nbyte)))
  {
    YYSYS_LOG(ERROR, "alloc memory failed");
  }
  else
  {
    *((int64_t *)ptr) = nbyte;
...
char *parse_strndup(const char *str, size_t nbyte, void *malloc_pool)
{
  ...
  if (str)
  {
    ...
    {
      YYSYS_LOG(ERROR, "parse_strdup gets string buffer error");
    }
  }
  return new_str;
}
