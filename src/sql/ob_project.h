...
namespace oceanbase
{
  namespace sql
  {
    ...
    //add:e
        ...
        {return index_num_;}
        inline void set_index_num(int64_t index_num)
        {index_num_ = index_num;}
        //add e
        inline void set_row_store(ObRowStore* store_pre, ObRowStore* store_post)
        {
            row_store_pre_ = store_pre;
            row_store_post_ = store_post;
        }
        ...
        int add_nextval_for_update(uint64_t *nextval, uint32_t p, uint32_t s, uint32_t vs,uint32_t len);
