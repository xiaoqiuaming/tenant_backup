  ...
  namespace sql
  {
                                       ...
                                       updateserver::RWSessionCtx &session_ctx,const ObDmlType dml_type, ObRowStore* store)
    {
       ...
       // UNUSED(host);
        ...
        {
          YYSYS_LOG(WARN,"fail to get index table array for table[%ld]", main_table_id_);
        }
            ...
            for(uint64_t i=0;i<array_count;i++)   //对该表的每一张可用的索引表进行处理
            {
                ...
                if (NULL == (index_table_schema = schema_mgr->get_table_schema(tmp_index_tid)))
                {
                 // ret = OB_ERR_ILLEGAL_ID;
                  YYSYS_LOG(WARN,"fail to get table schema for table[%ld]", tmp_index_tid);
                  continue;
                }
                else
                {
               ...
               // YYSYS_LOG(ERROR,"test::fanqs,,tmp_index_tid=%ld", tmp_index_tid);
                ...
                {
                   YYSYS_LOG(WARN,"fail to handle_one_index_table for table[%ld]", tmp_index_tid);
                   break;
                }
            }
        }
                                               ...
                                               const ObDmlType dml_type, ObRowStore* store)
    {
       ...
       // UNUSED(rk_size);
        ...
        {
            YYSYS_LOG(ERROR,"fail to cons_index_row_desc for table[%ld]", index_tid);
        }
        //del hongchen [SKEW_INDEX_COLUMN_BUGFIX] 20170801:b
        /*
        else if(OB_SUCCESS != (ret = get_row_desc(data_desc)))
        {
          YYSYS_LOG(ERROR, "failed to get data desc,ret = %d", ret);
        }
        */
        //del hongchen [SKEW_INDEX_COLUMN_BUGFIX] 20170801:e
        else if(NULL != store)
        {
           // YYSYS_LOG(ERROR,"test::fanqs,index row_desc=%s", to_cstring(index_row_desc));
            ObRowStore row_store;
            const ObRowStore::StoredRow *stored_row = NULL;
            ObRow row_tmp;
            ObRow row_in_store;
            row_in_store.set_row_desc(index_row_desc);
            data_desc = row_dec;  //add hongchen [SKEW_INDEX_COLUMN_BUGFIX] 20170801
            ...
            while(OB_SUCCESS==(ret = store->get_next_row(row_tmp)))  //取得用户sql语句中输入的要插入原表的所有行
            {
                ...
                {
                    YYSYS_LOG(ERROR, "failed to do row_store.add_row faile,ret=%d,row=%s",ret,to_cstring(row_in_store));
                }
            }
            if(ret==OB_ITER_END)
                ret=OB_SUCCESS;
        ...
        {
          YYSYS_LOG(ERROR, "iterator pointer cannot be null");
        }
        return ret;
    }
    ...
    int ObIndexTrigger::cons_index_row_desc(uint64_t index_tid,const ObSchemaManagerV2 *schema_mgr,ObRowDesc &row_desc)
    {
        ...
        {
          ret = OB_ERR_ILLEGAL_ID;
          YYSYS_LOG(ERROR,"fail to get table schema for table[%ld]", index_tid);
        }
        else
        {
            const ObRowkeyInfo *rowkey_info = &index_table_schema->get_rowkey_info();
            ...
            if(OB_SUCCESS == ret && schema_mgr->is_index_has_storing(index_tid))
            {
              ...
              {
                YYSYS_LOG(WARN, "add index vitual column failed,ret = %d", ret);
              }
            }
        }
        return ret;
    ...
    {
      int ret = OB_SUCCESS;
      //YYSYS_LOG(ERROR, "test::fanqs, ObIndexTrigger::open,,main_table_id_=%ld,index_num_=%ld,cur_index_=%ld",main_table_id_, index_num_,cur_index_);
      //could_insert_ = false;
      if (NULL == child_op_)
      {
        ret = OB_NOT_INIT;
    ...
    DEFINE_DESERIALIZE(ObIndexTrigger)
    {
        ...
        {
          YYSYS_LOG(WARN, "serialize row_desc fail ret=%d buf=%p data_len=%ld pos=%ld", ret, buf, data_len, pos);
        }
