...
namespace oceanbase
{
  ...
  namespace sql
  {
    ...
    {
      int ret = OB_SUCCESS;
      //        YYSYS_LOG(ERROR, "[liumengzhan] index num = %ld", index_num_);
      for (int64_t i=0; i<index_num_ && OB_LIKELY(OB_SUCCESS == ret); i++)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to handle_one_index_table for table[%ld]", index_table_id_[i]);
          break;
        }
      }
      ...
      //        {
      //            ret = OB_SCHEMA_ERROR;
      //            YYSYS_LOG(WARN,"fail to get main table schema, tid=[%ld], ret=%d", main_table_id_, ret);
      //        }
      //        else
      //        {
      ...
      //            {
      //                ret = OB_ERROR;
      //                YYSYS_LOG(WARN,"fail to get rowkey info, tid=[%ld], ret=%d", main_table_id_, ret);
      //            }
      //            else
      //            {
      ...
      //                {
      //                   YYSYS_LOG(WARN,"fail to handle_one_index_table for table[%ld]", main_table_id_);
      //                }
      //            }
      //        }//end if
      return ret;
                                              ...
                                              const ObDmlType dml_type,ObRowStore* store)
    {
      ...
      if (NULL == schema_mgr || NULL == host || NULL == child_op_)
      {
        ...
        YYSYS_LOG(WARN,"invalid argument,  schema_mgr or host is NULL.");
      }
      else if (NULL == store)
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN,"child_op_ is NULL, ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = child_op_->get_row_desc(next_row_desc)))
      {
        YYSYS_LOG(WARN, "get child op desc failed,ret [%d]", ret);
      }
      ...
      while (OB_LIKELY(OB_SUCCESS == ret) && OB_SUCCESS == (ret = store->get_next_row(next_row)))
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to add row to row store, ret=%d, row=%s",ret,to_cstring(row_in_store));
        }
      }//end while
      if(OB_ITER_END == ret)
      {
    ...
    int ObDeleteIndex::get_row_desc(const common::ObRowDesc *&row_desc) const
    {
      ...
      {
        YYSYS_LOG(WARN, "child_op get_row_desc fail ret=%d", ret);
      }*/
      row_desc = &row_desc_;
      return ret;
    }
    ...
    PHY_OPERATOR_ASSIGN(ObDeleteIndex)
    {
      ...
      for (int64_t i=0;i<index_num_&&OB_LIKELY(OB_SUCCESS==ret);i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get index tid fail, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret=add_index_tid(i, tid)))
        {
          YYSYS_LOG(ERROR, "add index tid fail, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret=o_ptr->get_index_row_desc(i, index_row_desc)))
        {
          YYSYS_LOG(ERROR, "get index row desc fail, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret=add_index_row_desc(i, *index_row_desc)))
        {
          YYSYS_LOG(ERROR, "add index row desc fail, ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    DEFINE_SERIALIZE(ObDeleteIndex)
    {
      ...
      {
        YYSYS_LOG(ERROR, "serialize main_table_id fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, buf_len, pos);
      }
      else if (OB_SUCCESS != (ret=serialization::encode_vi64(buf, buf_len, pos, index_num_)))
      {
        YYSYS_LOG(ERROR, "serialize index_num fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, buf_len, pos);
      }
      else if (OB_SUCCESS != (ret = row_desc_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(ERROR, "serialize row_desc fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, buf_len, pos);
      }
      for (int64_t i=0;i<index_num_&&OB_LIKELY(OB_SUCCESS==ret);i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "serialize index_tid fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, buf_len, pos);
        }
        else if (OB_SUCCESS != (ret = index_row_desc.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(ERROR, "serialize index_row_desc fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, buf_len, pos);
        }
      }
      return ret;
    }
    ...
    DEFINE_DESERIALIZE(ObDeleteIndex)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize main_table_id fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, data_len, pos);
      }
      else if (OB_SUCCESS != (ret=serialization::decode_vi64(buf, data_len, pos, &index_num_)))
      {
        YYSYS_LOG(ERROR, "deserialize index_num fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, data_len, pos);
      }
      else if (OB_SUCCESS != (ret = row_desc_.deserialize(buf, data_len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize row_desc fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, data_len, pos);
      }
      if (OB_LIKELY(OB_SUCCESS==ret))
      {
      ...
      for (int64_t i=0;i<index_num_&&OB_LIKELY(OB_SUCCESS==ret);i++)
      {
        ...
        if (OB_SUCCESS != (ret=serialization::decode_vi64(buf, data_len, pos, &tid)))
        {
          YYSYS_LOG(ERROR, "deserialize index_tid fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, data_len, pos);
        }
        else if (OB_SUCCESS != (ret = index_row_desc.deserialize(buf, data_len, pos)))
        {
          YYSYS_LOG(ERROR, "deserialize index_row_desc fail ret=%d buf=%p buf_len=%ld pos=%ld", ret, buf, data_len, pos);
        }
