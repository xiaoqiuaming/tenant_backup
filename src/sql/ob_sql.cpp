...
#include "sql/ob_drop_view_stmt.h"
...
static int init_hook_env(ObSqlContext &context, ObPhysicalPlan *&phy_plan, OperatorT *&op)
{
  ...
  if (NULL == ptr1 || NULL == ptr2)
  {
    ...
    YYSYS_LOG(WARN, "fail to malloc memory for ObValues, op=%p", op);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to add operator, err=%d", ret);
      op->~OperatorT();
...
//mod gaojt 20160418:e
{
  ...
  {
    if (OB_UNLIKELY(YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_TRACE))
    {
      YYSYS_LOG(TRACE, "execute special sql statement success [%.*s]", stmt.length(), stmt.ptr());
    }
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "store cur query string failed ret=%d", ret);
    }
    else
    {
      //mod gaojt [Delete_Update_Function] [JHOBv0.1] 20160418:b
//      ret = generate_logical_plan(stmt, context, result_plan, result);
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "generate logical plan failed, ret=%d sql=%.*s", ret, stmt.length(), stmt.ptr());
      }
      else
      {
        YYSYS_LOG(DEBUG, "generate logical plan succ");
        ...
        if (!context.disable_privilege_check_)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            ...
            YYSYS_LOG(WARN, "no privilege,sql=%.*s ret=%d", stmt.length(), stmt.ptr(), ret);
          }
        }
        bool logical_plan_optimizer = true;
        bool query_optimizer_switch = true;
        ObObj val;
        context.session_info_->get_sys_variable_value(
                    ObString::make_string("yb_enable_query_optimizer"), val);
        val.get_bool(query_optimizer_switch);
        if (!query_optimizer_switch)
        {
            logical_plan_optimizer = false;
        }
        else if(context.is_prepare_protocol_)
        {
            logical_plan_optimizer = false;
        }
        else if(logical_plan_optimizer)
        {
            int32_t num = logic_plan->get_stmts_count();
            for(int32_t i = 0;i<num;i++)
            {
                ObSelectStmt *select_stmt = dynamic_cast<ObSelectStmt*>(logic_plan->get_stmt(i));
                if (select_stmt ==NULL)
                {
                    YYSYS_LOG(DEBUG, "QX obselectstmt [%d] is NULL!", i);
                }
                else if (select_stmt->get_query_hint().join_array_.size()>0)
                {
                    logical_plan_optimizer = false;
                    break;
                }
                else if (select_stmt->get_query_hint().no_query_opt_)
                {
                    logical_plan_optimizer = false;
                    break;
                }
                else if (select_stmt->get_query_hint().has_index_hint())
                {
                    logical_plan_optimizer = false;
                    break;
                }
        ...
        if(OB_SUCCESS == ret && logical_plan_optimizer)
        {
            ...
            {
                ret = ObOptimizer::optimizer(result_plan, result, context.schema_manager_);
                logic_plan->set_query_optimizer_open(true);
            }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fill result set failed,ret=%d", ret);
          }
          else if(OB_SUCCESS != (ret = generate_physical_plan(context, result_plan, multi_phy_plan, result)))
          {
            YYSYS_LOG(WARN, "generate physical plan failed, ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "generete physical plan success, sql=%.*s", stmt.length(), stmt.ptr());
            ObPhysicalPlan *phy_plan = multi_phy_plan.at(0);
            OB_ASSERT(phy_plan);
            if (OB_UNLIKELY(YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_TRACE))
            {
              YYSYS_LOG(TRACE, "ExecutionPlan: \n%s", to_cstring(*phy_plan));
            }
...
//    {
      ...
      {
        YYSYS_LOG(WARN, "parse: %p, %p, %p, msg=[%s], start_col_=[%d], end_col_[%d], line_[%d], yycolumn[%d], yylineno_[%d]",
        ...
        snprintf(ud_select_parse_result.error_msg_, MAX_ERROR_MSG,
            "You have an error in your SQL syntax; check the manual that corresponds to your YaoBase version for the right syntax to use near '%.*s' at line %d", static_cast<int32_t>(error_length), stmt.ptr() + ud_select_parse_result.start_col_ - 1, ud_select_parse_result.line_);
        result.set_message(ud_select_parse_result.error_msg_);
        ret = OB_ERR_PARSE_SQL;
      }
      else if (NULL == context.schema_manager_)
      {
        YYSYS_LOG(WARN, "context.schema_manager_ is null");
      ...
      else
      {
        ...
        else
        {
          ...
          else if(ud_select_parse_result.result_tree_->children_[0]->type_ == T_SEQUENCE_ALTER)
          {
            ...
            {
              YYSYS_LOG(ERROR, "Alter sequence falied.");
            }
          }
          //20150430:e
          else
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to generate logical plan, err=%d sql=%.*s result_plan.err_stat_.err_msg_=[%s]",
          ...
          else
          {
            ...
            ////TODO Ϊ��prepare stmtname from select from test a Ҳʹ��psstore ��Ҫ��¼�°󶨵����
            ...
            //  {
            //    //TODO
            //    //log sql to ob_prepare_stmt >>>> ob_prepare����ִ�мƻ���ʱ�����ȥ ObPsStore������ ��session�ϱ���һ��stmt_name->sql id��ӳ��
            //    //ob_prepare  ob_execute��������������Ҫ�޸�
            //  }
            //}
            if (OB_UNLIKELY(YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_DEBUG))
            {
  ...
  else
  {
      ...
      || NULL == parse_result.result_tree_)
    {
      ...
      snprintf(parse_result.error_msg_, MAX_ERROR_MSG,
          "You have an error in your SQL syntax; check the manual that corresponds to your YaoBase version for the right syntax to use near '%.*s' at line %d", static_cast<int32_t>(error_length), stmt.ptr() + parse_result.start_col_ - 1, parse_result.line_);
      YYSYS_LOG(WARN, "failed to parse sql=%.*s err=%s", stmt.length(), stmt.ptr(), parse_result.error_msg_);
      result.set_message(parse_result.error_msg_);
      ret = OB_ERR_PARSE_SQL;
    }
    //add wangjg [multi_statements] 20170804:b
    else if((parse_result.result_tree_->type_ == T_STMT_LIST) && (parse_result.result_tree_->num_child_ > 1))
    {
      YYSYS_LOG(WARN, "not support multi statements, the number of statements is %d.", parse_result.result_tree_->num_child_);
      result.set_message("not support multi statements");
      ret = OB_NOT_SUPPORT_MULTI_STATEMENTS;
    }
    //add wangjg :e
    else if (NULL == context.schema_manager_)
    {
...
int ObSql::generate_physical_plan(ObSqlContext & context, ResultPlan &result_plan, ObMultiPhyPlan & multi_phy_plan, ObResultSet & result)
{
  ...
  {
      YYSYS_LOG(WARN, "failed to transform to physical plan");
...
int ObSql::stmt_prepare(const common::ObString &stmt, ObResultSet &result, ObSqlContext &context)
{
  ...
  if (NULL == store)
  {
    YYSYS_LOG(ERROR, "ObPsStore in context is null");
    ret = OB_ERROR;
  }
  else
  {
    ret = store->get(stmt, item);  // rlock  ��ֹ��close�õ�д����ʱ�����߳̿��Եõ����item
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "stmt_prepare failed when get ObPsStoreItem ret=%d sql=\'%.*s\'", ret,
    ...
    else
    {
      ...
      {
        YYSYS_LOG(DEBUG, "Get ObPsStore success stmt is %.*s ref count is %ld", stmt.length(),
        ...
        if (need_rebuild_plan(context.schema_manager_, item))
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "can not rebuild prepare ret=%d", ret);
          }
        }
        else
        {
          YYSYS_LOG(DEBUG, "Latest table schema is same with phyplan in ObPsStore");
        }
        //END check schema
      }
      else if (PS_ITEM_INVALID == item->get_status())
      {
        YYSYS_LOG(DEBUG, "Get ObPsStore success but status PS_ITEM_INVALID stmt is %.*s ref count is %ld", stmt.length(),
                  stmt.ptr(), item->get_ps_count());
        //����ִ�мƻ� copy data into psstore  result����plan���ڴ滹����ԭ���ķ�ʽ����
        ret = do_real_prepare(stmt, result, context, item, do_prepare);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "do_real_prepare failed, stmt is %.*s ret=%d", stmt.length(), stmt.ptr(), ret);
        }
      }
    }
    ...
    if (!do_prepare && OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "never reach here ret=%d", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "Get plan succ sql_id is %ld, ref count is %ld",
        ...
        {
          YYSYS_LOG(WARN, "Store result set to session failed ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = copy_plan_from_store(&result, item, &context, new_stmt_id)))
        {
          YYSYS_LOG(WARN, "copy plan to ObPsStore failed ret=%d", ret);
        }
...
int ObSql::do_real_prepare(const common::ObString &stmt, ObResultSet &result, ObSqlContext &context, ObPsStoreItem *item, bool &do_prepare)
{
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "try wlock when do real prepare %ld times sql=\"%.*s\"stmt_id=%ld", trywcount,
    ...
    else
    {
      if (0 == item->try_wlock())
      {
        //do prepare  result���ڴ��ڿ�����ObPsStore ���������ݸ��ͻ��˺�����ͷŵ� ����ʹ�õ���pagearena
        YYSYS_LOG(DEBUG, "do real prepare %.*s stmt_id=%ld", stmt.length(), stmt.ptr(), item->get_sql_id());
        do_prepare = true;
        ...
        {
          YYSYS_LOG(WARN, "context.session_info_(null)");
          ret = OB_NOT_INIT;
        }
        else if (NULL == (allocator = context.session_info_->get_transformer_mem_pool_for_ps()))
        {
          YYSYS_LOG(WARN, "failed to get new allocator");
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "direct execute failed");
          }
          else if (OB_SUCCESS != (ret = context.session_info_->store_ps_session_info(result)))
          {
            YYSYS_LOG(WARN, "Store result set to session failed");
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          YYSYS_LOG(DEBUG, "ExecutionPlan in result: \n%s", to_cstring(*result.get_physical_plan()));
          //copy field/plan from resultset to item
          ret = copy_plan_to_store(&result, item);
          YYSYS_LOG(DEBUG, "ExecutionPlan in item: \n%s", to_cstring(item->get_physical_plan()));
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "copy phy paln from resultset to PbPsStoreItem failed ret=%d", ret);
                        ...
                        ObResultSet &result, ObSqlContext &context)
{
  ...
  {
    YYSYS_LOG(WARN, "can not get ObPsSessionInfo from id_psinfo_map stmt_id = %ld, ret=%d", stmt_id, ret);
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != context.ps_store_->get_plan(inner_stmt_id, item))
    {
      ...
      YYSYS_LOG(USER_ERROR, "statement not prepared, stmt_id=%lu ret=%d", inner_stmt_id, ret);
    }
    else
    {
      //check schema
      if (need_rebuild_plan(context.schema_manager_, item))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "rebuild plan failed stmt_id=%lu, schema_version=%ld", inner_stmt_id, item->get_schema_version());
          //ret = stmt_close(inner_stmt_id, context);
        }
        else
        {
          YYSYS_LOG(DEBUG, "rebuild plan success stmt_id=%lu, schema_version=%ld", inner_stmt_id, item->get_schema_version());
        }
      }
      //end check schema
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "Copy plan from ObPsStore to  result set failed ret=%d", ret);
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to store cur query string ret=%d", ret);
        }
        else if(OB_SUCCESS != context.session_info_->store_params_type(stmt_id, params_type))
        {
          ret = OB_ERR_PREPARE_STMT_UNKNOWN;
          YYSYS_LOG(USER_ERROR, "statement not prepared, stmt_id=%lu ret=%d", stmt_id, ret);
        }
		...
		//mod 20130306:e
        {
          YYSYS_LOG(WARN, "Incorrect arguments to EXECUTE ret=%d", ret);
        }
        else
        {
          if (OB_UNLIKELY(YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_TRACE))
          {
            if (NULL == (result.get_physical_plan()->get_main_query()))
            {
                YYSYS_LOG(TRACE, "ExecutionPlan: \nresult.get_physical_plan() is NULL!");
            }
            else
            {
        ...
        YYSYS_LOG(DEBUG, "stmt execute end print field info");
        //YYSYS_LOG(DEBUG, "%s", to_cstring(result.get_field_columns()));
...
int ObSql::stmt_close(const uint64_t stmt_id, ObSqlContext &context)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != (ret = context.ps_store_->remove_plan(inner_sql_id))) //��ȫ�ֳ�����ɾ��ִ�мƻ�
    {
      YYSYS_LOG(WARN, "close prepared statement failed, sql_id=%lu", inner_sql_id);
    }
    else if(OB_SUCCESS != (ret = context.session_info_->remove_ps_session_info(stmt_id)))//ɾ��session��stmtid��Ӧ��ObPsSessionInfo
    {
      YYSYS_LOG(WARN, "remove ps session info failed, stmt_id=%lu", stmt_id);
    }
    else
    {
      YYSYS_LOG(DEBUG, "close stmt, stmt_id=%lu sql_id=%ld", stmt_id, inner_sql_id);
    }
  }
  return ret;
}
  ...
  if (stmt.length() >= select_collation_len && 0 == strncasecmp(stmt.ptr(), select_collation, select_collation_len))
  {
    ...
    else
    {
      ...
      for (int i = 0; i < 6; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to add field column %d", i);
          break;
        }
      }
...
int ObSql::do_privilege_check(const ObString & username, const ObPrivilege **pp_privilege, ObLogicalPlan *plan)
{
    ...
    switch (stmt->get_stmt_type())
    {
      case ObBasicStmt::T_SELECT:
        {
          ...
          if (OB_UNLIKELY(NULL == select_stmt))
          {
            ...
            YYSYS_LOG(ERROR, "dynamic cast failed,err=%d", err);
          }
          else
          {
            ...
            else
            {
              ...
              //mod:e
              {
                ...
                {
                  YYSYS_LOG(WARN, "push table_privilege to array failed,err=%d", err);
                }
              }
            }
          }
      ...
      case ObBasicStmt::T_DROP_USER:
        {
          ...
          // create user��ȫ��Ȩ��
          // comment liumz: ����ȫ��Ȩ���� is_global_priv && OB_NOT_EXIST_TABLE_TID ��ʾ
      ...
      case ObBasicStmt::T_DROP_DATABASE:
       {
          ...
          {//add:e
            ...
            {
              YYSYS_LOG(WARN, "get db name from drop db stmt failed, err=%d", err);
            }
            //mod:e
            table_privilege.table_id_ = OB_NOT_EXIST_TABLE_TID;
      ...
      case ObBasicStmt::T_CREATE_VIEW:
        {
          ...
          ObCreateViewStmt *create_view_stmt = dynamic_cast<ObCreateViewStmt*>(stmt);
          if (OB_UNLIKELY(NULL == create_view_stmt))
          {
            err = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "dynamic cast failed,err=%d", err);
          }
          ...
          {
            table_privilege.db_name_ = create_view_stmt->get_db_name();
            //add:e
            table_privilege.table_id_ = OB_NOT_EXIST_TABLE_TID;
            OB_ASSERT(true == table_privilege.privileges_.add_member(OB_PRIV_CREATE));
            err = table_privileges.push_back(table_privilege);
      ...
      case ObBasicStmt::T_DROP_VIEW:
        {
          ...
          ObDropViewStmt *drop_view_stmt = dynamic_cast<ObDropViewStmt*>(stmt);
          if (OB_UNLIKELY(NULL == drop_view_stmt))
          {
            err = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "dynamic cast failed,err=%d", err);
          }
          else
          {
            ...
            for (int64_t i = 0;i < drop_view_stmt->get_view_id_size() && OB_LIKELY(OB_SUCCESS == err);i++)
            {
              ObPrivilege::TablePrivilege table_privilege;

              const ObString &db_table_name = drop_view_stmt->get_view_name(i);
              bool split_ret = db_table_name.split_two(db_name, table_name);
              if (!split_ret)
              {
                err = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "split db_name.table_name failed, db_name.table_name[%.*s], db_name[%.*s], table_name[%.*s]",
              ...
              {
                YYSYS_LOG(WARN, "write db_name to name_pool failed, err=%d", err);
              }
              if (drop_view_stmt->get_if_exists() && OB_INVALID_ID == drop_view_stmt->get_view_id(i))
              {
                //view not exists, need no privilege.
                continue;
              }
              else
              {
                table_privilege.table_id_ = drop_view_stmt->get_view_id(i);
              }
              OB_ASSERT(true == table_privilege.privileges_.add_member(OB_PRIV_DROP));
              err = table_privileges.push_back(table_privilege);
              if (OB_UNLIKELY(OB_SUCCESS != err))
      ...
      case ObBasicStmt::T_DROP_TABLE:
        {
          ...
          else
          {
            ...
            //mod:e
            {
              ...
              //add:e
              //add liumz, priv_bugfix[drop table if exists]20170828:b
              if (drop_table_stmt->get_if_exists() && OB_INVALID_ID == drop_table_stmt->get_table_id(i))
              {
                //table not exists, need no privilege.
                continue;
              }
              //add:e
              else
      ...
      case ObBasicStmt::T_GATHER_STATISTICS:
      {
        ...
        else
        {
            ...
            db_name.assign_buffer(db_buf, OB_MAX_DATBASE_NAME_LENGTH);
            table_name.assign_buffer(table_buf, OB_MAX_TABLE_NAME_LENGTH);
            const ObString &db_table_name = gather_statistics_stmt->get_table_name();
            bool split_ret = db_table_name.split_two(db_name, table_name);
            if (!split_ret)
            {
                err = OB_ERR_UNEXPECTED;
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "username %.*s don't have enough privilege,err=%d", username.length(), username.ptr(), err);
  }
  else
  {
    YYSYS_LOG(DEBUG, "%.*s do privilege check success", username.length(), username.ptr());
  }
  return err;
}
...
bool ObSql::no_enough_memory()
{
  ...
  if (OB_UNLIKELY(ret))
  {
    YYSYS_LOG(WARN, "not enough memory, limit=%ld used=%ld",
...
int ObSql::copy_plan_from_store(ObResultSet *result, ObPsStoreItem *item, ObSqlContext *context, uint64_t sql_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid argument result is %p, item is %p", result, item);
  ...
  else
  {
    ...
    if (NULL != value)
    {
      ...
      if (NULL == phy_plan)
      {
        YYSYS_LOG(ERROR, "can not alloc mem for ObPhysicalPlan");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        YYSYS_LOG(DEBUG, "copy from store ob_malloc plan is %p store item %p, store plan %p", phy_plan, item, &item->get_physical_plan());
        ...
        if (OB_SUCCESS != ret)
        {
          ...
          YYSYS_LOG(ERROR, "Copy Physical plan from ObPsStoreItem to Current ResultSet failed ret=%d", ret);
        }
        else
        {
          result->set_physical_plan(phy_plan, true);
          ...
          {
            YYSYS_LOG(WARN, "Get ps session info failed sql_id=%lu, ret=%d", sql_id, ret);
          }
          else if (OB_SUCCESS != (ret = result->from_store(value, info)))
          {
            YYSYS_LOG(WARN, "Assemble Result from ObPsStoreItem(%p) and ObPsSessionInfo(%p) failed, ret=%d",
                      value, info, ret);
          }
        }
      }
    ...
    {
      YYSYS_LOG(ERROR, "ObPsStoreItemValue is null");
      ret = OB_ERROR;
    }
  }
  return ret;
    ...
    if (NULL != plan && NULL != value)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int i = 0; OB_SUCCESS == ret && i < result->get_field_columns().count(); ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "deep copy field failed, ret=%d", ret);
          }
        }
        ...
        for (int i = 0; OB_SUCCESS == ret && i < result->get_param_columns().count(); ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "deep copy parma failed, ret=%d", ret);
          }
		  //add peiouya  [NotNULL_check] [JHOBv0.1] 20140306:b
		  bool flag = true;
          ...
          {
            YYSYS_LOG(WARN, "Failed to store column NULL info, ret=%d", ret);
          }
		  //add 20140306:e
        }
        if (OB_SUCCESS == ret)
...
//        value->stmt_type_ = result->get_stmt_type();
      ...
      {
        storeplan->clear();
        YYSYS_LOG(ERROR, "Copy Physical Plan from ResultSet to ObPsStoreItem failed");
        ret = OB_ERROR;
      }
    }
    else
    {
      YYSYS_LOG(ERROR, "PhysicalPlan is null in ObPsStroreItemg");
    }
  }
  return ret;
}
...
int ObSql::try_rebuild_plan(const common::ObString &stmt, ObResultSet &result, ObSqlContext &context, ObPsStoreItem *item, bool &flag, bool substitute)
{
  int ret = OB_SUCCESS;
  flag = false;
  YYSYS_LOG(DEBUG, "call try rebuild plan");
  if (0 == item->wlock())//wait for wlock
  {
    YYSYS_LOG(DEBUG, "get wlock");
    if (item->is_valid() && !need_rebuild_plan(context.schema_manager_, item))
    {
      ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "other thread rebuild plan already");
    }
    else
    {
      flag = true;
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "direct execute failed");
          YYSYS_LOG(WARN, "try rebuild plan failed ret is %d", ret);
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (!substitute)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "store ps session info failed ret=%d", ret);
            }
          }
    ...
    {
      YYSYS_LOG(DEBUG, "new physical plan is %s", to_cstring(*result.get_physical_plan()));
    }
    item->unlock();//unlock whatever happen
  }
  YYSYS_LOG(DEBUG, "end call try rebuild plan");
  return ret;
}
...
int ObSql::copy_physical_plan(ObPhysicalPlan& new_plan, ObPhysicalPlan& old_plan, ObSqlContext *context)
{
  ...
  {
    YYSYS_LOG(WARN, "Assign physical plan failed, ret=%d", ret);
  }
  else if (context && (ret = set_execute_context(new_plan, *context)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Set execute context failed, ret=%d", ret);
  }
  return ret;
}
...
int ObSql::set_execute_context(ObPhysicalPlan& plan, ObSqlContext& context)
{
  ...
  for (int64_t i = 0; ret == OB_SUCCESS && i < plan.get_operator_size(); i++)
  {
    ...
    YYSYS_LOG(DEBUG, "plan %ld op is type=%s %p", i, ob_phy_operator_type_str(op->get_type()), op);
    if (!op)
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "Wrong physical plan, ret=%d, operator idx=%ld", ret, i);
    }
    switch (op->get_type())
    {
      case PHY_UPS_EXECUTOR:
      {
        ...
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "Empty inner plan of ObUpsExecutor, ret=%d", ret);
        }
        else
        {
...
bool ObSql::need_rebuild_plan(const common::ObSchemaManagerV2 *schema_manager, ObPsStoreItem *item)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument schema_manager=%p, item=%p", schema_manager, item);
  }
  else
  {
    ObPhysicalPlan &plan = item->get_physical_plan();
    int64_t tcount = plan.get_base_table_version_count();
    YYSYS_LOG(DEBUG, "table_version_count=%ld", tcount);
    for (int64_t idx = 0; idx < tcount; ++idx)
    {
      ...
      YYSYS_LOG(DEBUG, "table_id=%ld schema_version=%ld", tversion.table_id_, tversion.version_);
      if (NULL == ts)
      {
        YYSYS_LOG(DEBUG, "table(id=%ld) no longer exist in schema manager", tversion.table_id_);
        ret = true;
        break;
      }
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "table(id=%ld) version not match ObPsStore version=%ld, SchemaManager version=%ld",
    ...
    ResultPlan  &result_plan, ObResultSet &result)
{
  ...
  {
    YYSYS_LOG(ERROR,"Can not malloc space for select stmt.");
  }
  else if(OB_SUCCESS != (ret = direct_execute(sequence_select,result,context)))
  {
    YYSYS_LOG(ERROR,"Sequence select direct execute failed.");
  }
  else
  {
    ...
    */
    ObDecimal increment_by;
    ObDecimal min_value;
    ObDecimal max_value;
    ObDecimal const_start_with;
    if (OB_SUCCESS != (ret = result.open()))
    {
      YYSYS_LOG(ERROR, "failed to execute plan,ret=%d", ret);
    }
    else if(OB_SUCCESS != (ret = result.get_next_row(row)))
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(USER_ERROR,"sequence %s does not exist", parse_node->children_[0]->str_value_);
      }
      else
      {
        YYSYS_LOG(ERROR,"sequence call failed to get sequence row information, ret = %d",ret);
      }
    }
    else if(NULL == row)
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR,"get next row failed");
    }
    else if(OB_SUCCESS != (ret = row->raw_get_cell((DATA_TYPE_ID - OB_APP_MIN_COLUMN_ID),row_column_val)))
    {
      YYSYS_LOG(ERROR,"sequence call failed to get sequence field information, ret = %d",ret);
    }
    else if(OB_SUCCESS != (ret = row_column_val->get_int(data_type)))
    {
      YYSYS_LOG(ERROR,"sequence call failed to get field data_type,ret = %d",ret);
    }
    ...
    {
      YYSYS_LOG(ERROR,"sequence call failed to get field increment_by,ret = %d",ret);
    }
    ...
    {
      YYSYS_LOG(ERROR,"sequence call failed to get field min_value,ret = %d",ret);
    }
    ...
    {
      YYSYS_LOG(ERROR,"sequence call failed to get field max_value,ret = %d",ret);
    }
    ...
    {
      YYSYS_LOG(ERROR,"sequence call failed to get field const_start,ret = %d",ret);
    }
    if(OB_SUCCESS != (temp_select_ret = result.close()))
    {
      YYSYS_LOG(ERROR,"result close failed");
    }
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN,"result reset failed");
      }
      else
      {
    ...
    if (OB_SUCCESS == ret && parse_node->children_[1]->type_ == T_SEQUENCE_LABEL_LIST)
    {
      ...
      for (int8_t i = 0; i < lable_list->num_child_ && ret == OB_SUCCESS; i++)
      {
        ...
        case T_SEQUENCE_RESTART_WITH:
          if (1 == option_node->num_child_ && NULL != option_node->children_[0])
          {
            ...
            {
              YYSYS_LOG(ERROR, "Construction decimal value with RestartWith failed.");
              break;
            }
            else
            {
      ...
      for (int8_t i = 0; i < lable_list->num_child_ && ret == OB_SUCCESS; i++)
      {
        ...
        case T_SEQUENCE_QUICK:
          ...
          break;
        default:
          YYSYS_LOG(ERROR, "Unknow node type of alter sequence");
          ret = OB_ERR_UNEXPECTED;
          break;
        }//end switch
        if (OB_SUCCESS == ret)
    ...
    if (OB_SUCCESS == ret && OB_SUCCESS == (ret = seq_alter_stmt->sequence_option_validity(T_SEQUENCE_ALTER)))
    {
      ...
      if(OB_SUCCESS != (ret = ob_write_string(*name_pool,
                                ...
                                sequence_update)))
      {
        YYSYS_LOG(ERROR,"Can not malloc space for update stmt");
      }
      else if(OB_SUCCESS != (ret = generate_logical_plan(sequence_update,context,result_plan,result)))
      {
        YYSYS_LOG(ERROR,"sequence update generate_logical_plan failed");
        YYSYS_LOG(DEBUG,"update stmt: %.*s", sequence_update.length(), sequence_update.ptr());
      }
    }
  }
  return ret;
