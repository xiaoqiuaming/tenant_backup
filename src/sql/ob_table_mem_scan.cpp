...
namespace oceanbase
{
  namespace sql
  {
    ...
    int ObTableMemScan::open()
    {
      ...
      else if (!plan_generated_)
      {
        if (OB_SUCCESS == ret && has_rename_)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to set rename child. ret=%d", ret);
          }
          else
          {
            child_op_ = &rename_;
        ...
        {
          ret = OB_NOT_INIT;
          YYSYS_LOG(WARN, "must call set_table() before call open(). ret=%d", ret);
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to open table scan in mem operator. ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObTableMemScan::add_output_column(const ObSqlExpression& expr, bool change_tid_for_storing)//mod liumz, [optimize group_order by index]20170419
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to add project. ret=%d", ret);
      }
      else
      {
        has_project_ = true;
    ...
    int ObTableMemScan::set_table(const uint64_t table_id, const uint64_t base_table_id)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to rename table id. ret=%d", ret);
      }
      else
      {
        has_rename_ = true;
    ...
    PHY_OPERATOR_ASSIGN(ObTableMemScan)
    {
      ...
      {
        YYSYS_LOG(WARN, "Assign rename_ failed, ret=%d", ret);
      }
      else if ((ret = project_.assign(&o_ptr->project_)) != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "Assign project_ failed, ret=%d", ret);
      }
