...
int ObScalarAggregate::set_child(int32_t child_idx, ObPhyOperator &child_operator)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set single child, err=%d", ret);
  }
...
int ObScalarAggregate::get_next_row(const ObRow *&row)
{
  ...
  if (OB_UNLIKELY(is_first_row_))
  {
    ...
    //add:e
    {
      ...
      YYSYS_LOG(DEBUG, "scalar aggregate returns single row for the empty input set");
...
DEFINE_SERIALIZE(ObScalarAggregate)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to serialize ObScalarAggregate. ret=%d", ret);
  }
  return ret;
}
...
DEFINE_DESERIALIZE(ObScalarAggregate)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to deserialize ObScalarAggregate. ret=%d", ret);
  }
  return ret;
}
