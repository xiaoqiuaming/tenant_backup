...
bool ObTabletScan::has_incremental_data() const
{
  ...
  switch (plan_level_)
  {
    ...
    case UPS_DATA:
      ...
      ret = !op_multi_ups_scan_.is_result_empty();
      //mod 20160122:e
      break;
    case JOIN_DATA:
      ret = true;
    ...
    )
{
  ...
  ObMultiUpsScan *op_multi_ups_scan = NULL;
  //mod 20160105:e
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    op_multi_ups_scan = &op_multi_ups_scan_;
    //mod 20160105:e
    ...
    //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20160105:b
    //op_ups_scan->set_version_range(version_range);//����data_version
    op_multi_ups_scan->set_version_range(version_range);
    YYSYS_LOG(DEBUG, "need inc data,version range is[%s]",to_cstring(version_range));
    //mod 20160105:e
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    //{
    //  YYSYS_LOG(WARN, "ups scan set ups rpc stub fail:ret[%d]", ret);
    //}
    //else if(OB_SUCCESS != (ret = op_ups_scan->set_ts_timeout_us(ts_timeout_us_)))
    //{
    //  YYSYS_LOG(WARN, "set ups scan timeout fail:ret[%d]", ret);
    //}
    //else
    //{
    //  op_ups_scan->set_is_read_consistency(sql_scan_param_->get_is_read_consistency());
    //}
    if(OB_SUCCESS != (ret = op_multi_ups_scan->set_ups_rpc_proxy(rpc_proxy_)))
    {
      YYSYS_LOG(WARN, "ups scan set ups rpc stub fail:ret[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = op_multi_ups_scan->set_ts_timeout_us(ts_timeout_us_)))
    {
      YYSYS_LOG(WARN, "set ups scan timeout fail:ret[%d]", ret);
    }
    else
    {
      op_multi_ups_scan->set_is_read_consistency(sql_scan_param_->get_is_read_consistency());
    }
    //mod 20160105:e
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to set ups scan range");
    }
    for (int64_t i=0;OB_SUCCESS == ret && i<basic_columns.count();i++)
    {
      //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20160105:b
      //if(OB_SUCCESS != (ret = op_ups_scan->add_column(basic_columns.at(i))))
      if(OB_SUCCESS != (ret = op_multi_ups_scan->add_column(basic_columns.at(i))))
      //mod 20160105:e
      {
        YYSYS_LOG(WARN, "op ups scan add column fail:ret[%d]", ret);
      }
    }
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151223:b
    if (OB_SUCCESS == ret )
    {
      op_multi_ups_scan->set_read_atomic_param(sql_scan_param_->get_read_atomic_param());
    }
    //add duyr 20151223:e
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "set sstable scan fail:ret[%d]", ret);
    }
    //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20160105:b
    //else if(OB_SUCCESS != (ret = op_tablet_scan_fuse->set_incremental_scan(op_ups_scan)))
    else if(OB_SUCCESS != (ret = op_tablet_scan_fuse->set_incremental_scan(op_multi_ups_scan)))
    //mod 20160105:e
    {
      YYSYS_LOG(WARN, "set incremental scan fail:ret[%d]", ret);
    }
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151223:b
    else if (OB_SUCCESS != (ret = op_tablet_scan_fuse->set_read_atomic_param(sql_scan_param_->get_read_atomic_param())))
    {
  ...
  if(OB_SUCCESS == ret)
  {
    if(table_join_info.join_column_.count() > 0)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to set rpc proxy:ret[%d]", ret);
        }
        else
        {
  ...
  if(OB_SUCCESS == ret && renamed_table_id != table_id)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "op_rename set table fail:ret[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = op_rename->set_child(0, *op_root_)))
      {
        YYSYS_LOG(WARN, "op_rename set child fail:ret[%d]", ret);
      }
      else
      {
        op_root_ = op_rename;
  ...
  if(ret == OB_SUCCESS)
  {
     ...
     op_multi_ups_scan_.set_trans_id(const_cast<ObSqlScanParam *>(sql_scan_param_)->get_trans_id());//add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
  }
  //add:e
  return ret;
}
  ...
  int64_t data_version, ObVersionRange & new_range)
{
  ...
  {
    op_multi_ups_scan= &op_multi_ups_scan_;
    op_multi_ups_scan->set_version_range(new_range);
  }
  ...
  if(OB_SUCCESS == ret)
  {
    if (OB_SUCCESS != (ret = set_ups_scan_range(*(sql_scan_param_->get_range()))))
    {
      YYSYS_LOG(WARN, "fail to set ups scan range, ret [%d]", ret);
    }
    for (int64_t i=0;OB_SUCCESS == ret && i<basic_columns.count();i++)
    {
      //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20160105:b
    ...
    if (OB_SUCCESS == ret )
    {
      YYSYS_LOG(DEBUG, "op_multi_ups_scan  set read_atomic_param:%s",to_cstring(sql_scan_param_->get_read_atomic_param()));
      op_multi_ups_scan->set_read_atomic_param(sql_scan_param_->get_read_atomic_param());
      op_multi_ups_scan->set_meger_row_desc();
    }
    //add duyr 20151223:e
	  //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
    if (OB_SUCCESS == ret)
    {
        YYSYS_LOG(DEBUG, "op_multi_ups_scan  set read_atomic_param:%s",to_cstring(sql_scan_param_->get_data_mark_param()));
        op_multi_ups_scan->set_data_mark_param(sql_scan_param_->get_data_mark_param());
    }
    //add duyr 20160531:e
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "start_version_ [%ld], data_version[%ld]", int64_t(new_range.start_version_),data_version);
      if (OB_SUCCESS != (ret = op_tablet_scan_fuse->set_sstable_scan(&op_sstable_scan_)))
      {
        YYSYS_LOG(WARN, "set sstable scan fail:ret[%d]", ret);
      }
    ...
    else
    {
      ...
      is_truncated = true;
      YYSYS_LOG(DEBUG, "version changed_, start_version_ [%ld], data_version[%ld]", int64_t(new_range.start_version_),data_version);
      ObVersionRange version_range;
      int64_t end_data_version = sql_scan_param_->get_data_version();
      version_range.start_version_ = ObVersion(data_version);
      version_range.border_flag_.unset_min_value();
      version_range.border_flag_.set_inclusive_start();
      ...
      op_multi_ups_scan->set_version_range(version_range);
      YYSYS_LOG(DEBUG, "version_range:%s",to_cstring(version_range));
    }
  }

  if (OB_SUCCESS == ret)
  {
    if(!is_truncated && OB_SUCCESS != (ret = op_tablet_scan_fuse->set_sstable_scan(&op_sstable_scan_)))
    {
      YYSYS_LOG(WARN, "set sstable scan fail:ret[%d]", ret);
    }
      //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20160105:b
...
bool ObTabletScan::check_inner_stat() const
{
  ...
  if (!ret)
  {
    YYSYS_LOG(WARN, "join_batch_count_[%ld], "
    "sql_scan_param_[%p], "
    "network_timeout_[%ld], "
    "rpc_proxy_[%p]",
    join_batch_count_,
...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(DEBUG, "sql_scan_param=%s", to_cstring(sql_scan_param));
  ...
  for (int64_t i=0; OB_SUCCESS == ret && i<basic_columns.count(); i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "scan param add column fail:ret[%d]", ret);
    }
  }
...
int ObTabletScan::create_plan(const ObSchemaManagerV2 &schema_mgr)
{
  ...
  if (OB_SUCCESS != (ret = get_basic_column_and_join_info(
                               ...
                               basic_columns,
                               table_join_info
                               //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151214:b
                               ,sql_scan_param_
                               //add duyr 20151214:e
                               )))
  {
    YYSYS_LOG(WARN, "fail to get basic column and join info:ret[%d]", ret);
  }

  //add duyr [MultiUPS] [READ_ATOMIC] [read_atomic::debug] 20151223:b
  const ObReadAtomicParam& tmp_param = sql_scan_param_->get_read_atomic_param();
  if (tmp_param.is_valid())
  {
     YYSYS_LOG(DEBUG,"read_atomic::debug,CS real read atomic param from MS scan param!param=[%s]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if (sql_scan_param_->get_data_version() != OB_NEWEST_DATA_VERSION)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "invalid version");
        }
        else if (sql_scan_param_->get_data_version() < data_version)
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "The request version is not exist: request version[%ld], sstable version[%ld]", sql_scan_param_->get_data_version(), data_version);
        }
        else if (sql_scan_param_->get_data_version() == data_version)
        {
          last_rowkey_op_ = &op_sstable_scan_;
...
//                             sql_scan_param_->get_data_version())))
      ...
      {
        YYSYS_LOG(WARN, "fail to check incremental data :ret[%d]", ret);
      }
      ...
      //mod:e
      {
        YYSYS_LOG(WARN, "fail to add ups operator:ret[%d]", ret);
      }
    }
    else
    {
      ...
      && sql_scan_param_->has_project())
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to set project child. ret=%d", ret);
      }
      else
      {
        op_root_ = op_project;
  ...
  if (OB_SUCCESS == ret && sql_scan_param_->has_filter())
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      op_filter->set_read_atomic_param(sql_scan_param_->get_read_atomic_param());
      //add duyr 20160121:e
      if (OB_SUCCESS != (ret = op_filter->set_child(0, *op_root_)))
      {
        YYSYS_LOG(WARN, "fail to set filter child. ret=%d", ret);
      }
      else
      {
        op_root_ = op_filter;
    ...
    && (sql_scan_param_->has_scalar_agg() || sql_scan_param_->has_group()))
  {
    ...
    {
      ret = OB_ERR_GEN_PLAN;
      YYSYS_LOG(WARN, "Group operator and scalar aggregate operator"
    ...
    else if (sql_scan_param_->has_scalar_agg())
    {
        ...
        if(sql_scan_param_->has_listagg())
        {
            ...
            {
              YYSYS_LOG(WARN, "Fail to set child of scalar aggregate operator. ret=%d", ret);
            }
    ...
    else if (sql_scan_param_->has_group())
    {
      ...
      {
        ret = OB_ERR_GEN_PLAN;
        YYSYS_LOG(WARN, "Physical plan error, group need a sort operator. ret=%d", ret);
      }
      else
      {
      ...
      {
        YYSYS_LOG(WARN, "Fail to set child of sort operator. ret=%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "Fail to set child of group operator. ret=%d", ret);
      }
      else
      {
        op_root_ = &op_group_;
  ...
  if (OB_SUCCESS == ret && sql_scan_param_->has_limit())
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to set limit child. ret=%d", ret);
      }
      else
      {
        op_root_ = op_limit;
  ...
  if (OB_SUCCESS != ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to close op sstable scan:ret[%d]", err);
    }
  }
...
int ObTabletScan::set_ups_scan_range(const ObNewRange &scan_range)
{
  ...
  {
    YYSYS_LOG(WARN, "get tablet range fail, rc %d", rc);
  }
  else
  {
    ...
    rc = op_multi_ups_scan_.set_range(range);
    //mod 20160105:e
  }
  return rc;
}
...
int ObTabletScan::check_incremental_data_range(int64_t table_id, int64_t start_data_version, int64_t end_data_version, ObVersionRange &range)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    bool is_strong_consistency = sql_scan_param_->get_is_read_consistency();
    if (OB_SUCCESS != (ret = rpc_proxy_->check_incremental_data_range(table_id, version_range, range,is_strong_consistency)))
    {
      YYSYS_LOG(WARN, "failed to check incremental data range");
    }
  }
  return ret;
}
