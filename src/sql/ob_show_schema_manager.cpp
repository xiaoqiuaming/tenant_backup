...
yysys::CThreadMutex ObShowSchemaManager::mutex_;
...
const ObSchemaManagerV2* ObShowSchemaManager::get_show_schema_manager()
{
  if (!show_schema_mgr_)
  {
    ...
    if (!show_schema_mgr_)
    {
      ...
      {
        YYSYS_LOG(WARN, "Construct ObShowSchemaManager failed");
        ob_free(ptr);
      }
      else if (add_show_schemas(*schema_mgr) != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "Load show tables' schemas failed");
...
int ObShowSchemaManager::add_show_tables_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_TABLES_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
...
int ObShowSchemaManager::add_show_databases_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_DATABASES_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
...
int ObShowSchemaManager::add_show_index_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_INDEX_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
//add:e

int ObShowSchemaManager::add_show_all_index_schema(ObSchemaManagerV2 &schema_mgr)
{
    int ret = OB_SUCCESS;
    TableSchema table_schema;
    table_schema.init_as_inner_table();
    strcpy(table_schema.table_name_, OB_ALL_INDEX_SHOW_TABLE_NAME);
    table_schema.table_id_ = OB_ALL_INDEX_SHOW_TID;
    table_schema.rowkey_column_num_ = 1;
    ...
    ADD_COLUMN_SCHEMA("db_name", //column_name
                      ...
                      128, //column length
                      false); //is nullable
    ADD_COLUMN_SCHEMA("data_table_name", //column_name
                      column_id++, //column_id
                      0, //rowkey_id
                      ObVarcharType, //column_type
                      128, //column length
    ...
    if (ret == OB_SUCCESS
            && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
    {
        YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_ALL_INDEX_SHOW_TABLE_NAME, ret);
    }
    return ret;
}

int ObShowSchemaManager::add_show_create_index_schema(ObSchemaManagerV2 &schema_mgr)
{
    int ret = OB_SUCCESS;
    TableSchema table_schema;
    table_schema.init_as_inner_table();
    strcpy(table_schema.table_name_, OB_CREATE_INDEX_SHOW_TABLE_NAME);
    table_schema.table_id_ = OB_CREATE_INDEX_SHOW_TID;
    table_schema.rowkey_column_num_ = 1;
    table_schema.max_used_column_id_ = OB_APP_MIN_COLUMN_ID + 1;
    table_schema.max_rowkey_length_ = 128;
    int column_id = OB_APP_MIN_COLUMN_ID;

    ADD_COLUMN_SCHEMA("index_definition", //column_name
                      column_id++, //column_id
                      1, //rowkey_id
                      ObVarcharType, //column_type
                      4096, //column length
    ...
    if (ret == OB_SUCCESS
            && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
    {
        YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_CREATE_INDEX_SHOW_TABLE_NAME, ret);
    }
    return ret;
}
...
int ObShowSchemaManager::add_show_variables_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_VARIABLES_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
...
int ObShowSchemaManager::add_show_columns_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_COLUMNS_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
...
int ObShowSchemaManager::add_show_create_table_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_CREATE_TABLE_SHOW_TABLE_NAME, ret);
  }
  return ret;
}

int ObShowSchemaManager::add_show_create_view_schema(ObSchemaManagerV2& schema_mgr)
{
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  table_schema.init_as_inner_table();
  strcpy(table_schema.table_name_, OB_CREATE_VIEW_SHOW_TABLE_NAME);
  table_schema.table_id_ = OB_CREATE_VIEW_SHOW_TID;
  table_schema.rowkey_column_num_ = 1;
  table_schema.max_used_column_id_ = OB_APP_MIN_COLUMN_ID + 1;
  table_schema.max_rowkey_length_ = 128;
  int column_id = OB_APP_MIN_COLUMN_ID;
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_CREATE_VIEW_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
...
int ObShowSchemaManager::add_show_parameters_schema(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_PARAMETERS_SHOW_TABLE_NAME, ret);
  }
  return ret;
}

//add liu jun.[MultiUPS] [sql_api] 20150325:b
int ObShowSchemaManager::add_show_functions_schema(ObSchemaManagerV2 &schema_mgr)
{
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  table_schema.init_as_inner_table();
  strcpy(table_schema.table_name_, OB_FUNCTIONS_SHOW_TABLE_NAME);
  table_schema.table_id_ = OB_FUNCTIONS_SHOW_TID;
  table_schema.rowkey_column_num_ = 1;
  table_schema.max_used_column_id_ = OB_APP_MIN_COLUMN_ID + 4;
  table_schema.max_rowkey_length_ = 128;
  int column_id = OB_APP_MIN_COLUMN_ID;
  ADD_COLUMN_SCHEMA("rule_name", //column_name
      ...
      false); //is nullable
  ADD_COLUMN_SCHEMA("rule_par_num",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObIntType,
      sizeof(int32_t), //column length
      false); //is nullable)
  ADD_COLUMN_SCHEMA("rule_par_list",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObVarcharType,
      OB_MAX_TABLE_NAME_LENGTH, //column length
      false); //is nullable)
  ADD_COLUMN_SCHEMA("rule_body", //column_name
      column_id++, //column_id
      0, //rowkey_id
      ObVarcharType,  //column_type
      OB_MAX_ROW_NUMBER_PER_QUERY, //column length
      false); //is nullable
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_FUNCTIONS_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
//add 20150325:e
//add wuna[MultiUPS] [sql_api] 20160223:b
int ObShowSchemaManager::add_show_table_rules_schema(ObSchemaManagerV2 &schema_mgr)
{
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  table_schema.init_as_inner_table();
  strcpy(table_schema.table_name_, OB_TABLE_RULES_SHOW_TABLE_NAME);
  table_schema.table_id_ = OB_TABLE_RULES_SHOW_TID;
  ...
  ADD_COLUMN_SCHEMA("table_id",//column_name
      ...
      sizeof(int32_t), //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("start_version",//column_name
      column_id++, //column_id
      2, //rowkey_id
      ObIntType,
      sizeof(int32_t), //column length
  ...
  ADD_COLUMN_SCHEMA("partition_type",//column_name
      ...
      sizeof(int32_t), //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("prefix_name",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObVarcharType,
      OB_MAX_VARCHAR_LENGTH, //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("rule_name",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ...
      OB_MAX_TABLE_NAME_LENGTH, //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("par_list",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObVarcharType,
      OB_MAX_VARCHAR_LENGTH, //column length
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_TABLE_RULES_SHOW_TABLE_NAME, ret);
  }
  return ret;
}
int ObShowSchemaManager::add_show_groups_schema(ObSchemaManagerV2 &schema_mgr)
{
  int ret = OB_SUCCESS;
  TableSchema table_schema;
  table_schema.init_as_inner_table();
  strcpy(table_schema.table_name_, OB_PARTITION_GROUPS_SHOW_TABLE_NAME);
  table_schema.table_id_ = OB_PARTITION_GROUPS_SHOW_TID;
  table_schema.rowkey_column_num_ = 2;
  table_schema.max_used_column_id_ = OB_APP_MIN_COLUMN_ID + 4;
  table_schema.max_rowkey_length_ = sizeof(int64_t);
  int column_id = OB_APP_MIN_COLUMN_ID;
  ...
  ADD_COLUMN_SCHEMA("start_version",//column_name
      ...
      sizeof(int32_t), //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("paxos_id",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObIntType,
      sizeof(int32_t), //column length
      false); //is nullable
  ADD_COLUMN_SCHEMA("reserver_field1",//column_name
      column_id++, //column_id
      0, //rowkey_id
      ObVarcharType,
      OB_MAX_TABLE_NAME_LENGTH, //column length
  ...
  if (ret == OB_SUCCESS
    && (ret = schema_mgr.add_new_table_schema(table_schema)) != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "Add schema of %s faild, ret=%d", OB_PARTITION_GROUPS_SHOW_TABLE_NAME, ret);
  }
  return ret;
}

int ObShowSchemaManager::add_show_paxos_id_schema(ObSchemaManagerV2 &schema_mgr)
{
    int ret = OB_SUCCESS;
    TableSchema table_schema;
    table_schema.init_as_inner_table();
    strcpy(table_schema.table_name_, OB_PAXOS_ID_SHOW_TABLE_NAME);
    table_schema.table_id_ = OB_PAXOS_ID_SHOW_TID;
    table_schema.rowkey_column_num_ = 1;
    table_schema.max_used_column_id_ = OB_APP_MIN_COLUMN_ID;
    table_schema.max_rowkey_length_ = sizeof(int32_t);
    int column_id = OB_APP_MIN_COLUMN_ID;
...
int ObShowSchemaManager::add_show_schemas(ObSchemaManagerV2& schema_mgr)
{
  ...
  if (ret == OB_SUCCESS)
  {
    if (OB_SUCCESS != (ret = schema_mgr.sort_table()))
    {
        YYSYS_LOG(WARN, "failed to sort tables in schema manager, err=%d",ret);
    }
    else if(OB_SUCCESS != (ret = schema_mgr.sort_column()))
    {
...
int ObShowSchemaManager::add_show_schema(ObSchemaManagerV2& schema_mgr, int32_t stmt_type)
{
  ...
  switch (stmt_type)
  {
    ...
    case ObBasicStmt::T_SHOW_ALL_INDEX:
      ret = add_show_all_index_schema(schema_mgr);
      break;
    case ObBasicStmt::T_SHOW_CREATE_INDEX:
      ret = add_show_create_index_schema(schema_mgr);
      break;
    ...
    case ObBasicStmt::T_SHOW_CREATE_VIEW:
      ret = add_show_create_view_schema(schema_mgr);
      break;
    ...
    case ObBasicStmt::T_SHOW_FUNCTIONS:
      ret = add_show_functions_schema(schema_mgr);
      break;
    //add 20150325:e
    //add wuna[MultiUPS] [sql_api] 20160223:b
    case ObBasicStmt::T_SHOW_TABLE_RULES:
      ret = add_show_table_rules_schema(schema_mgr);
      break;
    case ObBasicStmt::T_SHOW_GROUPS:
      ret = add_show_groups_schema(schema_mgr);
      break;
    //add 20160223:e
    case ObBasicStmt::T_SHOW_CURRENT_PAXOS_ID:
      ret = add_show_paxos_id_schema(schema_mgr);
      break;
