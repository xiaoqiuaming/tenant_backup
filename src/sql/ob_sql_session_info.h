...
//      common::ObObj* cur_time_;
    ...
    class ObSQLSessionInfo: public common::ObVersionProvider
    {
      ...
      public:
        ...
        const char* get_peer_addr() const
        {return addr_;}
        ...
        void set_current_result_set(ObResultSet *cur_result_set)
        {cur_result_set_ = cur_result_set;}
        void set_query_start_time(int64_t time)
        {cur_query_start_time_ = time;}
        int store_query_string(const common::ObString &stmt);
        const yysys::WarningBuffer& get_warnings_buffer() const
        {return warnings_buf_;}
        uint64_t get_new_stmt_id()
        {return (common::atomic_inc(&next_stmt_id_));}
        IdPlanMap& get_id_plan_map()
        {return id_plan_map_;}
        SysVarNameValMap& get_sys_var_val_map()
        {return sys_var_val_map_;}
        ObResultSet* get_current_result_set()
        {return cur_result_set_;}
        const common::ObString get_current_query_string() const;
        int64_t get_query_start_time() const
        {return cur_query_start_time_;}
        const ObSQLSessionState get_session_state() const
        {return state_;}
        easy_connection_t *get_conn() const
        {return conn_;}
        const char* get_session_state_str()const;
        void set_session_state(ObSQLSessionState state)
        {state_ = state;}
        const common::ObString& get_user_name()
        {return user_name_;}
        common::ObStringBuf& get_parser_mem_pool()
        {return parser_mem_pool_;}
        common::StackAllocator& get_transformer_mem_pool()
        {return transformer_mem_pool_;}
        ...
        int32_t try_read_lock()
        {return pthread_rwlock_tryrdlock(&rwlock_);}
        int32_t try_write_lock()
        ...
        void set_trans_id(const common::ObTransID &trans_id)
        {trans_id_ = trans_id;};
        //mod peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150804:b
        //const common::ObTransID& get_trans_id(){return trans_id_;};
        const common::ObTransID& get_trans_id()
        {return participant_trans_id_[0];};
        //mod 20150804:e
        ...
        {return config_provider_->is_regrant_priv();};
        //add:e
        //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20151201:b
        void set_retry_plan_timeout(int64_t retry_timeout);
        int64_t get_retry_plan_timeout(void)
        {return retry_plan_timeout;}
        void reset_retry_info(void);
        //add 20151201:e
        // add lqc [multiups 1.0][create rule without trans]
        void set_trans_info(common::ObEndTransReq& req);
        // add e
        /**
         * @pre 系统变量存在的情况下
        ...
        { last_active_time_ = yysys::CTimeUtil::getTime(); }
        bool is_timeout();
...
//            trans_id_.read_times_ = read_times ;
            ...
            {
                participant_trans_id_[i].read_times_ = read_times;
            }
        }
   //add by maosy  20161210
        ...
        void set_trans_start(bool trans_start)
        {is_trans_start_ = trans_start;}
        bool get_trans_start() const
        {return is_trans_start_;}
        int get_participant_num()
        { return participant_num_;}
        /**
         * @brief get_participant_trans_info
         * @param [in]paxos_id participant paxos_id
         * @param [out]part_trans_id
         * @return
         */
        int get_participant_trans_info(const int32_t paxos_id, ObTransID &part_trans_id);
        /**
         * @brief check_mem_version
         * @param [in]version
         * @return match or not match previous mem version
         */
        int check_mem_version(const int64_t version);
        int add_participant_info(const int32_t part_paxos_id, const ObTransID &part_trans_id);
        /**
         * @brief get trans info, get all participant info
         * @param [out]req
         */
        void get_trans_info(common::ObEndTransReq& req);
        void reset_trans_info();
        //add 20150701:e
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150804:b
        const common::ObTransID& get_current_trans_id()
        {return trans_id_;}
        //add 20150804:e
        //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
        void set_master_ups_transid(int64_t paxos_id,int64_t transid)
        {
            master_ups_transids_[paxos_id] = transid;
        }
        int64_t get_master_ups_transid(const int32_t paxos_id)
        {
            return master_ups_transids_[paxos_id];
        }
        void reset_master_ups_transids()
        {
            for(int i = 0 ; i < MAX_UPS_COUNT_ONE_CLUSTER ;i++)
            {
               master_ups_transids_[i]=OB_INVALID_DATA;
            }
        }
        // add by maosy e
        const int32_t get_paxos_id()
        {  return participant_paxos_id_[0]; };
      private:
      ...
      private:
        ...
        yysys::CThreadMutex mutex_;//互斥同一个session上的多次query请求
        uint64_t session_id_;
        yysys::WarningBuffer warnings_buf_;
        uint64_t user_id_;
        ...
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        bool is_trans_start_;  /*是否已经 start transation*/
        int32_t participant_num_;//default 0, after end trans , reset 0
        int64_t memtable_version_;
        int32_t participant_paxos_id_[MAX_UPS_COUNT_ONE_CLUSTER];/*参与分布式事务 ups信息*/
        ObTransID participant_trans_id_[MAX_UPS_COUNT_ONE_CLUSTER];
        //add 20150701:e
        //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20151201:b
        int64_t retry_plan_timeout;
        bool has_set_retry_plan_time_out_;
        //add 20151201:e
        //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
        int64_t master_ups_transids_[MAX_UPS_COUNT_ONE_CLUSTER];
		//add by maosy e
    };
