...
//add wangyao b [SEQUENCE OPTIMIZATION] 20171011:b
...
int ObSequence::add_sequence_info_to_map(ObString &sequence_name, SequenceInfo &s_info)
{
  ...
  {
    YYSYS_LOG(ERROR,"set (%.*s) to coiumn map failed!",sequence_name.length(),sequence_name.ptr());
    ret = OB_ERROR;
  }
  else if ((hash::HASH_EXIST == ret))//||( hash::HASH_INSERT_SUCC == ret))//
  {
    YYSYS_LOG(ERROR, "overwrite!");
    if (-1 == (ret = sequence_info_map_.set(sequence_name, s_info, 1)))//overWrite
    {
      YYSYS_LOG(ERROR,"Over write sequence info failed!");
    }
  }
...
int ObSequence::get_next_row(const common::ObRow *&row)
{
  ...
  {
    YYSYS_LOG(DEBUG,"lock get next row!");
...
//        YYSYS_LOG(ERROR,"the row is::[%s]",to_cstring(*(srow.get_inner_row())));
      }
    }
  }
  //quick path use
  else
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "get the first child ::ObValues op of sequence failed!");
    }
    else
    {
...
int ObSequence::cons_sequence_row_desc(ObRowDesc& row_desc, const ObRowkeyInfo *&rowkey_info)
{
  ...
  if (NULL == (table_schema = sql_context_->schema_manager_->get_table_schema(OB_ALL_SEQUENCE_TID)))
  {
    ...
    YYSYS_LOG(ERROR, "fail to get table schema for table[%lu]", OB_ALL_SEQUENCE_TID);
  }
  else
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey_col_num; ++i) // for each primary key
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to add sequence row desc, err=%d", ret);
      }
    }// end for
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "the sequence row desc is::[%s]",to_cstring(row_desc));
  }
  return ret;
}
...
int ObSequence::init_sequence_info_map()
{
  ...
  if (!sequence_info_map_inited_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "create sequence_info_map_ fail:ret[%d]", ret);
    }
    else
    {
 ...
 *
 */
int ObSequence::get_prevval_by_name(ObString *sequence_name, uint64_t *&prevval, uint32_t &p, uint32_t &s, uint32_t &vs,
                                    uint32_t &len, bool &can_use_prevval)
{
  int ret = OB_SUCCESS;
  SequenceInfo s_info;
  YYSYS_LOG(DEBUG, "sequence name :[%.*s]",sequence_name->length(),sequence_name->ptr());
  if (-1 == (ret = sequence_info_map_.get(*sequence_name, s_info)))
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "get sequence info failed!");
  }
  else if (hash::HASH_NOT_EXIST == ret)
  {
    YYSYS_LOG(ERROR, "get sequence [%.*s] failed! ret::[%d]",sequence_name->length(),sequence_name->ptr(),ret);
  }
  else
  {
    ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret)
  {
    if (1 == s_info.can_use_prevval_)
    {
      ...
      if (OB_SUCCESS != (ret =  s_info.prevval_.get_ttint(prevval)))
      {
        YYSYS_LOG(ERROR,"get decimal pointer from s_info.prevval_ error,ret = %d",ret);
      }
      else
      {
          p = s_info.prevval_.get_precision();
          s = s_info.prevval_.get_scale();
          vs = s_info.prevval_.get_vscale();
          len = s_info.prevval_.get_nwords();
      }
    }
  }
  return ret;
 ...
 *
 */
int ObSequence::get_nextval_by_name(ObString *sequence_name, uint64_t *&nextval, uint32_t &p, uint32_t &s, uint32_t &vs,
                                    uint32_t &len, bool &can_use_nextval)
{
  int ret = OB_SUCCESS;
  SequenceInfo s_info;
  if (-1 == (ret = sequence_info_map_.get(*sequence_name, s_info)))
  ...
  if (OB_SUCCESS == ret)
  {
    if (1 == s_info.valid_)
    {
      ...
      if (OB_SUCCESS != (ret =  s_info.current_value_.get_ttint(nextval)))
      {
        YYSYS_LOG(ERROR,"get decimal pointer from s_info.current_value_ error,ret = %d",ret);
      }
      else
      {
          p = s_info.current_value_.get_precision();
          s = s_info.current_value_.get_scale();
          vs = s_info.current_value_.get_vscale();
          len = s_info.current_value_.get_nwords();
      }
    }
  }
  return ret;
...
int ObSequence::calc_nextval_for_sequence(SequenceInfo &s_value)
{
  ...
  {
    //do nothing
    YYSYS_LOG(DEBUG,"new created,the nextval can be used directly");
  }
  //current sequence has been used at least once,we need to build the nextval by calculating
  else
  {
    s_value.current_value_.get_decimal(dec_current_value);
    s_value.min_value_.get_decimal(dec_min_value);
    s_value.max_value_.get_decimal(dec_max_value);
    s_value.inc_by_.get_decimal(dec_inc_by);
    ...
    {
      YYSYS_LOG(WARN,"cast sequence info from varchar to decimal failed!");
    }
    else
    {
      cycle = s_value.cycle_;
    ...
    */
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "caculate next value of sequence faliled!");
      }
      ...
      else
      {
        ...
        */
        ...
        {
            YYSYS_LOG(ERROR, "write the new prevval to pool failed,ret::[%d]",ret);
        }
        //5. set to the SequenceInfo
        if (OB_SUCCESS == ret)
        {
          s_value.current_value_.set_precision(dec_result.get_precision());
          s_value.current_value_.set_scale(dec_result.get_scale());
          s_value.current_value_.set_vscale(dec_result.get_vscale());
          s_value.current_value_.set_nwords(len);
          s_value.current_value_.set_ttint(new_result);
          /*
          if (OB_SUCCESS != (ret = s_value.current_value_.set_decimal(new_result)))
          {
            YYSYS_LOG(ERROR, "set next valid value failed!");
          }
          */
        }
        YYSYS_LOG(DEBUG, "the  new value is ::%s", to_cstring(s_value.current_value_));
      }//end else
    }
  }//end else
   return ret;
...
int ObSequence::calc_prevval_for_sequence(SequenceInfo &s_value)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS != (ret = name_pool_.write_decimal(current_value.get_words()->ToUInt_v2(), new_result, s_value.current_value_.get_nwords())))
    {
      YYSYS_LOG(ERROR, "write the new prevval to pool failed,ret::[%d]",ret);
    }
    else
    {
        s_value.prevval_.set_precision(current_value.get_precision());
        s_value.prevval_.set_scale(current_value.get_scale());
        s_value.prevval_.set_vscale(current_value.get_vscale());
        s_value.prevval_.set_nwords(s_value.current_value_.get_nwords());
        s_value.prevval_.set_ttint(new_result);
        YYSYS_LOG(DEBUG, "the new sequence prevval value is::%s", to_cstring(s_value.prevval_));
    }
    //YYSYS_LOG(DEBUG, "the new sequence prevval value is::%.*s",new_result.length(),new_result.ptr());
//    YYSYS_LOG(ERROR, "the new sequence prevval value is ::[%.*s]",new_result.length(),new_result.ptr());
  }
  return ret;
}
...
int ObSequence::build_next_value_for_sequence(ObString &sequence_name, SequenceInfo &s_value)
{
  ...
  if (sequence_info_map_inited_)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "get [%.*s] sequence info error!",sequence_name.length(),sequence_name.ptr());
    }
    else if (hash::HASH_NOT_EXIST == ret)//not in hash map
    {
      YYSYS_LOG(ERROR, "the sequence [%.*s] not in map!",sequence_name.length(),sequence_name.ptr());
    }
    else if (hash::HASH_EXIST == ret)//succeed
    {
      if (OB_SUCCESS !=(ret = calc_nextval_for_sequence(s_value)))
      {
        YYSYS_LOG(WARN, "calc nextval for sequence [%.*s] failed!",sequence_name.length(),sequence_name.ptr());
      }
      if (OB_SUCCESS == ret)
      {
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "sequnece column map not inited!");
  }
  return ret;
}
...
int ObSequence::get_only_update_nextval_sequence_info(const ObRow *&sequence_row, SequenceInfo &s_info)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "the row is not inited!");
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get sequence row cell failed!");
    }
    else
    {
        /*
      ...
      {
        YYSYS_LOG(ERROR , "get sequence obj_current_value  failed!");
      }
      else */
      if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.current_value_)))
      {
        YYSYS_LOG(ERROR, "store start with falied!");
      }
      //            YYSYS_LOG(ERROR, "@@@@@@@@the cell start with is::[%s]@@@@@@@",to_cstring(*cell));
    }
    //valid
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR , "get sequence valid failed!");
      }
      else
      {
        s_info.valid_ = valid;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if ((!only_update_prevval))//!only_update_prevval
      {
        ...
        {
          YYSYS_LOG(ERROR , "get sequence data_type failed!");
        }
        else
        {
          s_info.data_type_ = data_type;
          //            YYSYS_LOG(ERROR, "@@@@@@@@the cell data type is::[%s]@@@@@@@",to_cstring(*cell));
        }
      }
    }
  }
 ...
 *
 */
int ObSequence::get_sequence_current_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval, ObDecimal &obj_current_value)
{
  int ret = OB_SUCCESS;
  if (NULL == sequence_row)
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if ((OB_SUCCESS == ret) && (!only_update_prevval))//!only_update_prevval
      {
        if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.current_value_)))
        {
          YYSYS_LOG(ERROR, "store current value to name pool falied!");
        }
//            YYSYS_LOG(ERROR, "@@@@@@@@the cell start with is::[%s]@@@@@@@",to_cstring(*cell));
      }
    }
 ...
 *
 */
int ObSequence::get_sequence_inc_by_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval, ObDecimal &obj_increment_by)
{
  int ret = OB_SUCCESS;
  if (NULL == sequence_row)
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR , "get sequence increment_by failed!");
      }
      if ((OB_SUCCESS == ret) && (!only_update_prevval) && (!has_checked_))//!only_update_prevval
      {
        if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.inc_by_)))
        {
          YYSYS_LOG(ERROR, "store increment by falied!");
        }
      }
//                   YYSYS_LOG(ERROR, "@@@@@@@@the cell increment is::[%s]@@@@@@@",to_cstring(*cell));
    }
  }
  return ret;
}
...
int ObSequence::get_sequence_min_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ObDecimal obj_min_value;
      if (OB_SUCCESS != (ret = cell->get_decimal(obj_min_value)))
      {
        YYSYS_LOG(ERROR , "get sequence obj_min_value failed!");
      }
      if ((OB_SUCCESS == ret) && (!only_update_prevval) && (!has_checked_))//!only_update_prevval
      {
        if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.min_value_)))
        {
          YYSYS_LOG(ERROR, "store min_value falied!");
        }
        YYSYS_LOG(DEBUG, "@@@@@@@@the cell min valued is::[%s]@@@@@@@",to_cstring(*cell));
      }
      //          YYSYS_LOG(ERROR, "@@@@@@@@the cell min valued is::[%s]@@@@@@@",to_cstring(*cell));
    }
  }
  return ret;
}
...
int ObSequence::get_sequence_max_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ObDecimal obj_max_value;
      if (OB_SUCCESS != (ret = cell->get_decimal(obj_max_value)))
      {
        YYSYS_LOG(ERROR , "get sequence obj_max_value failed!");
      }
      if ((OB_SUCCESS == ret) && (!only_update_prevval) && (!has_checked_))//!only_update_prevval
      {
        if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.max_value_)))
        {
          YYSYS_LOG(ERROR, "store max_value falied!");
        }
        YYSYS_LOG(DEBUG, "@@@@@@@@the cell max value is::[%s]@@@@@@@",to_cstring(*cell));
      }
      //          YYSYS_LOG(ERROR, "@@@@@@@@the cell max value is::[%s]@@@@@@@",to_cstring(*cell));
    }
  }
  return ret;
}
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if ((!only_update_prevval))//!only_update_prevval
      {
        ...
        {
          YYSYS_LOG(ERROR , "get sequence cycle failed!");
        }
        else
        {
          s_info.cycle_ = cycle;
          //          YYSYS_LOG(ERROR, "@@@@@@@@the cell cycle  is::[%s]@@@@@@@",to_cstring(*cell));
        }
      }
    }
  }
 ...
 *
 */
int ObSequence::get_sequence_can_use_prev_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool update_prevval, ObDecimal &obj_current_value)
{
  int ret = OB_SUCCESS;
  if (NULL == sequence_row)
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR , "get sequence present value failed!");
      }
      else
      {
        ...
        if (update_prevval)
        {
          ...
          //ObString prevval;
          uint64_t *prevval = NULL;
          if (OB_SUCCESS == ret)
          {
            int len;
            if (obj_current_value.get_words()->table[1] == 0)
            {
                len = 1;
            }
            else
            ...
            if (OB_SUCCESS != (ret = name_pool_.write_decimal(obj_current_value.get_words()->ToUInt_v2(), prevval, len)))
            {
              YYSYS_LOG(ERROR, "write current value to name_pool failed!");
            }
            if (OB_SUCCESS == ret)
            {
              s_info.prevval_.set_precision(obj_current_value.get_precision());
              s_info.prevval_.set_scale(obj_current_value.get_scale());
              s_info.prevval_.set_vscale(obj_current_value.get_vscale());
              s_info.prevval_.set_nwords(len);
              s_info.prevval_.set_ttint(prevval);
              //YYSYS_LOG(ERROR, "set prev value of sequecen faliled!");
            }
            //                YYSYS_LOG(ERROR,"^^^^^the [%.*s]prevval is[%.*s]^^^^^",sequence_name.length(),sequence_name.ptr(), prevval.length(),prevval.ptr());
          }
        }//end if
      }
    }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if ((!only_update_prevval))//!only_update_prevval
      {
        ...
        {
          YYSYS_LOG(ERROR , "get sequence use quick path failed!");
        }
        else
        {
...
int ObSequence::update_sequence_info_map(bool update_prevval,bool update_nextval)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR,"U must update at least one type value of sequence!");
  }
  ...
  while(OB_SUCCESS == ret)//for each row
  {
    ...
    {
//      YYSYS_LOG(ERROR, "@@@@@@@@the row is::[%s]@@@@@@@",to_cstring(*tem_row));
      YYSYS_LOG(DEBUG, "@@@@@@@@the row is::[%s]@@@@@@@",to_cstring(*tem_row));
      const ObObj *cell = NULL;
      ObObjType cell_type;
      ...
      if (OB_SUCCESS == ret && has_checked_)//(only_update_nextval) || (only_update_prevval))//only_update_nextval and only_update_prevval
      {
        ...
        {
          ret = OB_SUCCESS;
          YYSYS_LOG(DEBUG, "reget!");
        }
        if (ret == hash::HASH_NOT_EXIST)
        {
          YYSYS_LOG(ERROR,"do not happen in this place!");
        }
      }
      //============================================
      ...
      if (only_update_nextval && OB_SUCCESS == ret)//only_update_nextval
      {
        ...
        {
          YYSYS_LOG(WARN, "get only update nextval sequence info failed!");
        }
      }
      else//only_update_prevval and update_both
      {
        //YYSYS_LOG(ERROR, "@@@@@@@@the cell name is::[%s]@@@@@@@",to_cstring(*cell));
        //data type
        if (OB_SUCCESS == ret)
        {
          if (OB_SUCCESS != (ret = get_sequence_data_type_info(tem_row, col_info, only_update_prevval)))
          {
            YYSYS_LOG(WARN, "get sequence data type info failed,ret::[%d]",ret);
          }
        }
        //start with

        ObDecimal obj_current_value;
        if (OB_SUCCESS == ret)
        {
          if (OB_SUCCESS != (ret = get_sequence_current_value_info(tem_row, col_info, only_update_prevval, obj_current_value)))
          {
            YYSYS_LOG(WARN, "get sequence current value info failed,ret::[%d]",ret);
          }
        }

        //increment by
        ObDecimal obj_increment_by;
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence min value info failed,ret::[%d]",ret);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence max value info failed,ret::[%d]",ret);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence cycle value info failed,ret::[%d]",ret);
          }
        }
        //catch 7
        //order 8
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence valid value info failed,ret::[%d]",ret);
          }
        }
        //restart with 10
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence can use prev  value info failed,ret::[%d]",ret);
          }
        }
        //use_quick_path 12
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get sequence use quick path value info failed,ret::[%d]",ret);
          }
        }
      }//end else
      //============================================
      ...
      if (OB_SUCCESS == ret && sequence_info_map_inited_)
      {
        ...
        {
          YYSYS_LOG(ERROR,"set (%.*s) to coiumn map failed!",stored_sequence_name.length(),stored_sequence_name.ptr());
          ret = OB_ERROR;
        }
        else if ((hash::HASH_EXIST == ret))//||( hash::HASH_INSERT_SUCC == ret))//
        {
//          YYSYS_LOG(ERROR, "overwrite!");
          YYSYS_LOG(DEBUG, "overwrite!");
  ...
  if (!has_checked_)
  {
    for (int64_t i = 0; OB_SUCCESS == ret && i < sequence_name_no_dup_.count(); i++)
    {
      ...
      else if (hash::HASH_NOT_EXIST == ret)
      {
        ...
        YYSYS_LOG(USER_ERROR, "the sequence::[%.*s] you input is not exist,please check!",sequence_name_no_dup_.at(i).length(),sequence_name_no_dup_.at(i).ptr());
        break;
      }
      else
      {
...
int ObSequence::update_sequence_info_map_for_prevval(ObString &sequence_name, bool update_prevval)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (-1 == (ret = sequence_info_map_.get(sequence_name,s_info)))
    {
      YYSYS_LOG(ERROR,"get (%.*s) sequence_info_map failed!",sequence_name.length(),sequence_name.ptr());
      ret = OB_ERROR;
    }
    else if (ret == hash::HASH_NOT_EXIST)//
    {
      YYSYS_LOG(WARN,"not exist! should't happen here,ret::[%d]",ret);
    }
    else if (hash::HASH_EXIST == ret)
    {
      ret = OB_SUCCESS;
...
int ObSequence::prepare_sequence_prevval()
{
  ...
  {
    YYSYS_LOG(ERROR, "failed get static data of sequences!");
  }
  // 2. init map
  else if (OB_SUCCESS == ret && OB_SUCCESS != (ret = init_sequence_info_map()))//init map
  {
    YYSYS_LOG(ERROR, "init sequence name column map failed!");
  }
  // 3.upate map
  else if (OB_SUCCESS != (ret = update_sequence_info_map(true, true)))
  {
    YYSYS_LOG(WARN, "set all sequence rows to map failed! [ret:%d]",ret);
  }
  return ret;
}
...
int ObSequence::prepare_lock_row(ObString &sequence_name)
{
  ...
  if (!has_cons_lock_row_desc_ &&  OB_SUCCESS != (ret = lock_row_desc_.add_column_desc(OB_ALL_SEQUENCE_TID,
                                                    NAME_ID)))
  {
    YYSYS_LOG(ERROR, "failed to add row key row desc, err=%d", ret);
  }
//  YYSYS_LOG(ERROR,"cccccccccccsequence is::[%.*s]",sequence_name.length(),sequence_name.ptr());
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "set sequence row key cell failed!");
    }
  }
  return ret;
}
...
int ObSequence::start_sequence_trans()
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR,"sql context not inited!");
  }
  else
  {
...
//if (session->get_trans_id().is_valid())
  if (session->get_trans_id().is_valid() || session->get_trans_start())//already in transaction
// add by maosy e
  {
    has_client_trans_ = true;
    YYSYS_LOG(DEBUG, "already in transaction");
    ret = OB_SUCCESS;
  }
  else
  {
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR,"fail to do direct execute,ret = %d",ret);
      }
      //open
      if(OB_SUCCESS == ret && OB_SUCCESS != (ret = result_set.open()))
      {
        YYSYS_LOG(ERROR,"fail to open lock sql,ret = %d",ret);
      }
      //close
      else if (OB_SUCCESS != (ret = result_set.close()))
      {
        YYSYS_LOG(WARN,"close failed!");
      }
    }
  }
...
int ObSequence::end_sequence_trans()
{
  ...
  {
    //do noting
    YYSYS_LOG(DEBUG,"U start transaction from the client");
  }
  else
  {
    YYSYS_LOG(DEBUG,"end transaction!");
    if (OB_SUCCESS == ret)
    {
      ObResultSet result_set;
...
//add wangyao [sequence for optitmize]:20171109
/**
 *
 * @brief we use this function to parse the row to the SequenceInfo,expecially when the first use nextval
 * we can use the current_value to the prevval_value
 * @param val_row [in]
 * @param s_info [in]
 * @param not_use_prevval [in]
 * @return if OB_SUCCESS
 *
 */
int ObSequence::parse_row(const ObRow *val_row, SequenceInfo &s_info,bool not_use_prevval)
{
    int ret = OB_SUCCESS;
    const ObObj *cell = NULL;
    //YYSYS_LOG(ERROR,"to_cstring val_row is %s",to_cstring(*val_row));
    if (NULL != val_row)
    {
       if (OB_SUCCESS == (ret = val_row->raw_get_cell((DATA_TYPE_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
             int64_t data_type_ = OB_INVALID_INDEX;
             if (OB_SUCCESS != (ret = cell->get_int(data_type_)))
             {
                YYSYS_LOG(ERROR,"faild to get data_type_ from cell ret=%d",ret);
             }
             else
             {
                s_info.data_type_ = data_type_;
             }
           }
           else
           {
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((CURRENT_VALUE_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
          if(NULL != cell)
          {
             if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.current_value_)))
             {
               YYSYS_LOG(ERROR,"faild to get current_value_ from cell ret=%d",ret);
             }
             else
             {
                if(not_use_prevval)
                {
                  if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.prevval_)))
                  {
                    YYSYS_LOG(ERROR,"faild to get prevval_value_ from cell ret=%d",ret);
                  }
                }
              }
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((INC_BY_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
          if(NULL !=cell)
          {
              if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.inc_by_)))
              {
                YYSYS_LOG(ERROR,"faild to get inc_by_ from cell ret=%d",ret);
              }
          }
          else
          {
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((MIN_VALUE_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.min_value_)))
               {
                   YYSYS_LOG(ERROR,"faild to get min_value_ from cell ret=%d",ret);
               }
           }
           else
           {
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((MAX_VALUE_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               if (OB_SUCCESS != (ret = name_pool_.write_obj(*cell, &s_info.max_value_)))
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((CYCLE_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               int64_t cycle = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(cycle)))
               {
                   YYSYS_LOG(ERROR,"faild to get cycle from cell ret=%d",ret);
               }
               else
               {
                   s_info.cycle_ = cycle;
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((CACHE_NUM_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               int64_t cache_num = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(cache_num)))
               {
                   YYSYS_LOG(ERROR,"faild to get cache_num from cell ret=%d",ret);
               }
               else
               {
                   s_info.cache_ = cache_num;
               }
           }
           else
           {
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((ORDER_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               int64_t order = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(order)))
               {
                   YYSYS_LOG(ERROR,"faild to get order from cell ret=%d",ret);
               }
               else
               {
                   s_info.order_ = order;
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((VALID_ID - OB_APP_MIN_COLUMN_ID), cell)))
       {
           if(NULL != cell)
           {
               int64_t valid = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(valid)))
               {
                   YYSYS_LOG(ERROR,"faild to get valid from cell ret=%d",ret);
               }
               else
               {
                   s_info.valid_ = valid;
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((CAN_USE_PREVVAL_ID - OB_APP_MIN_COLUMN_ID -1), cell)))
       {
           if(NULL != cell)
           {
               int64_t can_use_prevval = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(can_use_prevval)))
               {
                   YYSYS_LOG(ERROR,"faild to get can_use_prevval from cell ret=%d",ret);
               }
               else
               {
                    s_info.can_use_prevval_=can_use_prevval;
       ...
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = val_row->raw_get_cell((USE_QUICK_PATH_ID - OB_APP_MIN_COLUMN_ID -1), cell)))
       {
           if(NULL != cell)
           {
               int64_t use_quick_path = OB_INVALID_INDEX;
               if (OB_SUCCESS != (ret = cell->get_int(use_quick_path)))
               {
                   YYSYS_LOG(ERROR,"faild to get use_quick_path from cell ret=%d",ret);
               }
               else
               {
                   s_info.use_quick_path_ = use_quick_path;
...
//add e
//add wangyao [sequence for optimize]:20171203
/**
 *
 * @brief we use this function to parse the row to the SequenceInfo in the UPS,the row that we construct in order to deliver the
 * new prevval_value
 * @param tmp_row [in]
 * @param s_info [in]
 * @return if OB_SUCCESS
 *
 */
int ObSequence::parse_row_prevval(const ObRow *tmp_row, SequenceInfo &s_info)
{
    int ret = OB_SUCCESS;
    const ObObj *cell = NULL;
    if(NULL != tmp_row)
    {
       if (OB_SUCCESS == ret && OB_SUCCESS == (ret = tmp_row->raw_get_cell(1, cell)))
       {
...
//add e
//add wangyao [sequence for optimize]:20171115
/**
 *
 * @brief we use this function to parse the row to the SequenceInfo in the MS,the row is we construct in the UPS,
 * in order to deliver the new SequenceInfo to MS
 * @param val_row [in]
 * @param s_info [in]
 * @return if OB_SUCCESS
 *
 */
int ObSequence::parse_row(const ObRow *val_row, SequenceInfo &s_info)
{
    int ret = OB_SUCCESS;
    const ObObj *cell = NULL;
    //YYSYS_LOG(ERROR,"to_cstring val_row is %s",to_cstring(*val_row));
 ...
 * whose name is "sequence_name" with new nextval(s_info.current_value_),in this function
 * we break the current transaction,this contain two case:
 * 1.When the client displays the open transaction, the sequence statement is first called
 * 2.When the client displays the open transaction,he ordinary statement is first called,
 * and the sequnence statement is invoked.
 * @param sequence_name [in]
 * @param s_info [in]
 * @param update_prevval [in]
 * @return if OB_SUCCESS
 *
 */
int ObSequence::update_by_dml_special_for_optimize(ObString &sequence_name, SequenceInfo &s_info, bool update_prevval)
{
    int ret = OB_SUCCESS;
    char update_sql[OB_SQL_LENGTH];
    ObString update_sql_s;
    int64_t pos = 0;
    bool is_empty_req=false;//add wangyao [sequence for break transaction]:20171109
    if (NULL == sql_context_)
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR,"sql context not inited!");
    ...
    {
        //mod by wangyao [bug fix for sequence]:20171027
        /*
        databuff_printf(update_sql, 1024, pos,
                        "update __all_sequence set data_type=data_type, current_value=current_value, increment_by=increment_by, min_value=min_value, max_value=max_value, is_cycle=is_cycle, is_order=is_order, is_valid=is_valid, use_quick_path=use_quick_path where sequence_name = \'%.*s\'",
                        sequence_name.length(), sequence_name.ptr());

                        */

        databuff_printf(update_sql, OB_SQL_LENGTH, pos,
                        "update __all_sequence set data_type=data_type, \
                        current_value=current_value, increment_by=increment_by, \
                        min_value=min_value, max_value=max_value, is_cycle=is_cycle,\
                        cache_num=cache_num,is_order=is_order, is_valid=is_valid,\
                        can_use_prevval=can_use_prevval,use_quick_path=use_quick_path \
        ...
        {
          update_sql_s.assign_ptr(update_sql, static_cast<ObString::obstr_size_t>(pos));
        }
        if (OB_SUCCESS == ret)
        {
            ObResultSet result_set;
            ...
            else if (OB_SUCCESS != (ret = ObSql::direct_execute(update_sql_s, result_set, *sql_context_)))
            {
              YYSYS_LOG(ERROR,"fail to do direct execute,ret = %d",ret);
            }
            else
            {
               ...
               ObUpsExecutor* ups_executor = dynamic_cast<ObUpsExecutor*>(result_set.get_physical_plan()->get_main_query()->get_child(0));
            //ups_modify = dynamic_cast<ObUpsModifyWithDmlType*>(result_set.get_physical_plan()->get_main_query()->get_child(0)->get);
                 if(NULL == ups_executor)
                 {
                   ret=OB_ERROR;
                 }
                 else
                 {
                    ups_modify =dynamic_cast<ObUpsModifyWithDmlType*>(ups_executor->get_inner_plan()->get_main_query());
                    ...
                    {
                      ups_modify->set_is_sequence(true);
                      ups_modify->set_update_prevval(update_prevval);//add wangyao [sequence for get prevval value]:20171206

                      //do update
                      //add wangyao [sequence for break transaction]:20171109
                       ObEndTransReq req;
                       result_set.get_session()->get_trans_info(req);
                       bool is_auto_commit=result_set.get_session()->get_autocommit();
                       bool is_start_trans = result_set.get_session()->get_trans_start();
                       if(0 == req.participant_num_)
                       {
                         is_empty_req=true;
                       }
                       result_set.get_session()->set_autocommit(true);
                       result_set.get_session()->reset_trans_info();
                       //add e
                      if(OB_SUCCESS != (ret = result_set.open()))
                      {
                        YYSYS_LOG(ERROR,"fail to open lock sql,ret = %d",ret);
                      }
                      else if (OB_SUCCESS == ret)
                      {
                         const ObRow *val_row = NULL;
                         const ObRow *tmp_row = NULL;
                        if(OB_SUCCESS !=(ret =ups_executor->get_next_row_for_sequence(val_row)))
                        {
                          YYSYS_LOG(ERROR,"get sequence_row info faild,ret=%d",ret);
                        }
                        else if(OB_SUCCESS !=(ret = parse_row(val_row, s_info)))
                        {
                          YYSYS_LOG(ERROR,"parse row to sequence info faild,ret=%d",ret);
                        }
                       //add wangyao[sequence for prevval value parse]:20171206
                        if(OB_SUCCESS == ret && update_prevval)
                        {
                         if(OB_SUCCESS !=(ret =ups_executor->get_next_row_for_prevval(tmp_row)))
                         {
                           YYSYS_LOG(ERROR,"get sequence_row info faild,ret=%d",ret);
                         }
                         else if(OB_SUCCESS !=(ret = parse_row_prevval(tmp_row, s_info)))
                         {
                           YYSYS_LOG(ERROR,"parse row to sequence info faild,ret=%d",ret);
                         }
                        }
                        //add e
                        //result_set.close();
                        if (OB_SUCCESS == ret && OB_SUCCESS != (ret = result_set.close()))
                        {
                         YYSYS_LOG(WARN,"close failed!");
                        }
                     }
                      ...
                      if(OB_SUCCESS == ret)
                      {
                        if(is_empty_req)
                        {
                          result_set.get_session()->set_trans_start(is_start_trans);
                        }
                        else
                        {
                          result_set.get_session()->set_trans_info(req);
                          result_set.get_session()->set_autocommit(is_auto_commit);
                        }
                      }
                      //add e
                    }
...
//add e
//mod by wangyao [sequence for optimize]:20171120
/**
 *
 * @brief we use this function construct a dml SQL(update statement) update the sequence
...
int ObSequence::update_by_dml(ObString &sequence_name, SequenceInfo &s_info)
{
  ...
  int ret = OB_SUCCESS;
  bool is_empty_req=false;//add wangyao:20171120
  if (NULL == sql_context_)
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR,"sql context not inited!");
  ...
  {
    ObResultSet result_set;
    /*ObString*/ObDecimal tmp_current_value;
    YYSYS_LOG(DEBUG,"DML UPDATE");
    if (OB_SUCCESS != (ret = s_info.current_value_.get_decimal(tmp_current_value)))
    {
      YYSYS_LOG(ERROR,"get start with failed!");
    }
    else
    {
      //name
      ...
      //start
      char current_value[MAX_PRINTABLE_SIZE];
      memset(current_value, 0, MAX_PRINTABLE_SIZE);
      int64_t tmp_len = tmp_current_value.to_string(current_value, MAX_PRINTABLE_SIZE);
      std::string new_value;
      new_value.assign(current_value,tmp_len);
      ...
      YYSYS_LOG(DEBUG,"the sql is::[%.*s]",excute_sql.length(),excute_sql.ptr());
      if (OB_SUCCESS != (ret = result_set.init()))
      {
        YYSYS_LOG(ERROR, "init result set failed, ret = %d", ret);
      }
      ...
      //do update
      //add wangyao [sequence for break transaction]:20171120
      else
      {
        ObEndTransReq req;
        result_set.get_session()->get_trans_info(req);
...
int ObSequence::do_simple_fill(ObSEArray<ObObj,64> &post_expr_array, int64_t idx, int dml_type, bool is_const)
{
  ...
  YYSYS_LOG(DEBUG, "&&&&&&&the obj is::[%s]&&&&&&&",to_cstring(*expr_array_obj));
  //1.get the const obj
  if (ObVarcharType != (obj_type = expr_array_obj->get_type()))
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "the type must be ObVarcharType!");
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get the expr array obj failed!");
    }

    //mod wangyao [sequence for bug fix]
    else
    {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "get the sequence values error!");
        }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else if (OB_SEQUENCE_SELECT_LIMIT == dml_type)
      {
        ...
        //mod by wangyao [sequence for fault-tolerant processing]:20171212
      ...
      //add wangyao [sequence for bug fix]
      else
      {
          ret=OB_ERROR;
          YYSYS_LOG(DEBUG,"the dml type is wrong ");
      }
      //add e
      YYSYS_LOG(DEBUG, "the current sequence type is::[%s]",is_next_type?"next":"prev");
        ...
        if (!can_use_current_nextval(sequence_name))//本地当前seuquence不可用
        {
          ...
          else//for normal sequence or client transaction
          {
            ...
            if ((OB_SUCCESS == ret) && ((has_client_trans_ && can_lock_current_sequence(sequence_name)) || (!has_client_trans_)))
            {
              ...
              {
                YYSYS_LOG(ERROR,"prepare the lock row failed!");
              }
              else if (OB_SUCCESS != (ret = check_lock_condition()))
              {
                YYSYS_LOG(WARN,"not inited!");
              }
...
//              YYSYS_LOG(ERROR,"the lock row is[%s]",to_cstring(lock_row_));
              if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = lock_by_row.lock_by_for_update(&lock_row_,  OB_ALL_SEQUENCE_TID, true, my_result_set_))))
              {
                YYSYS_LOG(ERROR,"lock sequence row failed!");
              }
              //(3)取当前sequence值并更新到本地
              YYSYS_LOG(DEBUG,"update next::[%s],update prev::[%s]",update_nextval == true? "true":"false",update_prevval==true? "true":"false");
              if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = this->update_sequence_info_map(update_prevval, update_nextval))))
              {
                YYSYS_LOG(WARN, "set current sequence row to map failed! [ret:%d]",ret);
              }
              ...
              {
                YYSYS_LOG(ERROR, "update the sequence info failed!");
              }
              */
              //del:e
              //add wangyao [SEQUENCE FOR OPTIMIZE]:20171024
              if ((OB_SUCCESS == ret) && !has_client_trans_)
             {
               YYSYS_LOG(DEBUG,"has_client_trans_ is %d",has_client_trans_);
               if (hash::HASH_EXIST != (ret = sequence_info_map_.get(sequence_name, s_value)))
               {
                   ret = OB_ERROR;
                   YYSYS_LOG(ERROR, "get the sequence values error!");
               }
               else if (OB_SUCCESS != (ret = update_by_dml_special_for_optimize(sequence_name, s_value,update_prevval)))
               {
                  YYSYS_LOG(ERROR, "update the sequence info to sequence_all_sysetem failed!");
               }
               else if (hash::HASH_OVERWRITE_SUCC != sequence_info_map_.set(sequence_name, s_value, 1))
               {
                  ret = OB_ERROR;
                  YYSYS_LOG(ERROR, "update the sequence info to sequence_info_map failed!");
               }
             }
             //del by wangyao [SEQUENCE FOR optimize]:20171124
 //            if (OB_SUCCESS == ret)
 //            {
 ...
 //            }
            ...
            //(6)结束事务
              ...
              {
                 YYSYS_LOG(WARN,"commit sequence trans failed!");
              }
              */
               //add e
          }//end else
        ...
        if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = get_nextval_by_name(&sequence_name,value,p,s,vs,len,can_use_nextval))))
        {
            YYSYS_LOG(ERROR, "get nextval failed!");
        }
        if (OB_SUCCESS == ret && !can_use_nextval)//the sequence can't be used any more
        {
          ret = OB_ERROR;
          YYSYS_LOG(USER_ERROR, "the sequence is out of range");
        }
        //c.使用完一个 next sequence 后，需要更新本地 map 中的prevval值，这里做记录，在一行结束后进行更新prevval值
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(DEBUG,"has client_trans::[%s]",true == has_client_trans_?"yes":"no");
          if (OB_SUCCESS != (ret = add_single_row_sequence_name(sequence_name)))
          {
            YYSYS_LOG(ERROR, "add single row failed!");
          }
        }
      }
      else //if (!is_next_type) is preval type,
      {
        if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = get_prevval_by_name(&sequence_name, value,p,s,vs,len, can_use_prevval))))
        {
            YYSYS_LOG(ERROR, "get the sequence prevval error!");
        }
        if (OB_SUCCESS == ret && (!can_use_prevval))//can't use
        {
          ret = OB_ERROR;
          //A PREVIOUS VALUE expression cannot be used before the NEXT VALUE
          YYSYS_LOG(USER_ERROR, "THE PREVVAL expression of  sequence [%.*s] can't be used before using NEXTVAL",sequence_name.length(),sequence_name.ptr());
        }
      }
      //YYSYS_LOG(DEBUG,"set decimal::[%.*s]!",value.length(),value.ptr());
      if(OB_SUCCESS == ret && (can_use_nextval ||can_use_prevval))
      {
          if (value!=NULL)
          {
              TTInt tmp_tt;
              tmp_tt.FromUInt_v2(value, len);
              if (s == 0 && vs == 0)
              {
                  uint32_t tmp_p = 0;
                  TTInt BASE(10);
              ...
              {
                  ObDecimal tmp_dec, od;
                  tmp_dec.from(p, s, vs, tmp_tt);
                  const char *tmp_s = to_cstring(tmp_dec);
                  ret = od.from(tmp_s);
                  p = od.get_precision();
                  s = od.get_scale();
                  vs = od.get_vscale();
          ...
          {
              ret = OB_ERROR;
              YYSYS_LOG(ERROR, "replace the value for current column failed!");
          }
          if (OB_SUCCESS == ret && (OB_SUCCESS != (ret = expr_array_obj->set_decimal(value, p, s, vs, len))))
          {
              YYSYS_LOG(ERROR, "replace the value for current column failed!");
          }
      }
      ...
      */
      ...
      if (OB_SUCCESS == ret && OB_SEQUENCE_UPDATE_LIMIT == dml_type && is_next_type && can_use_nextval)
      {
        ...
        if (OB_SUCCESS != (ret = add_nextval_to_project_op(value, p, s, vs, len)))
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR,"failed to add the nextval to project op,ret=[%d]",ret);
        }
        //b.store the value_idx of current expr
        else if (OB_SUCCESS != (ret = add_idx_of_next_value_to_project_op(idx)))
        {
          YYSYS_LOG(ERROR, "failed to add idx of next value,ret=[%d]",ret);
        }
        ...
        {
          YYSYS_LOG(ERROR, "failed to add expr idx to project op,ret=[%d]",ret);
        }
        YYSYS_LOG(DEBUG,/*"the values [%.*s],*/ "the indx of valueis[%ld],the expr idxis[%ld]",
                  /*value.length(),value.ptr(),*/idx,out_put_columns_idx_);
      }
    }//end if
  }//end if
  return ret;
...
int ObSequence::do_complex_fill(ObSEArray<ObObj,64> &post_expr_array, int dml_type)
{
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < post_expr_array.count(); i++)//for eache array item
  {
    ...
    if (ObIntType == post_expr_array[i].get_type())
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to get int value.err=%d", ret);
      }
      else if ((int64_t)ObPostfixExpression::CONST_OBJ == expr_type)//const obj
      {
        if (OB_SUCCESS != (ret = this->do_simple_fill(post_expr_array, i+=1, dml_type)))
        {
          ...
          {
            YYSYS_LOG(DEBUG, "not a varchar const_obj [ret::%d]",ret);
            ret = OB_SUCCESS;
            continue;
          }
          else
          {
            YYSYS_LOG(WARN, "do complex fill error![ret::%d]",ret);
          }
        }
      }
    }
...
{
//  YYSYS_LOG(DEBUG, "Pair array count:[%ld], sequence_idx_in_expr_.count:[%ld]",names_types_array.count(), sequence_idx_in_expr_.count());
...
void ObSequence::need_to_update_prevval(ObString &sequence_name, bool &update_prevval)
{
  ...
  {
    YYSYS_LOG(ERROR,"get index failed!");
  }
}
...
bool ObSequence::get_cur_seuqnece_type_for_insert()
{
  ...
  {
    YYSYS_LOG(ERROR,"won't be here!");//won't be here!
  }
  return is_next;
}
...
bool ObSequence::get_cur_seuqnece_type_for_select(int dml_type, int &ret)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "dml type is wrong.");
  }
  if (OB_SUCCESS == ret)
  {
...
int ObSequence::check_column_cast_validity(ObSqlExpression& expr, const int64_t &column_idx, bool is_update)
{
  int ret = OB_SUCCESS;
  if (out_row_desc_.get_column_num() <= 0)
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR,"the row_desc is not inited");
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS == ret)
    {
        if(is_update)
        {
            if (OB_INVALID_ID == (table_id = expr.get_table_id()))
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Failed to get table_id, err=%d", ret);
            }
            else if(OB_INVALID_ID == (column_id = expr.get_column_id()))
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Failed to get column_id, err=%d", ret);
            }
            else if (OB_SUCCESS != (ret = out_row_desc_ext_.get_by_id(table_id, column_id, idx, data_type)))
            {
              ret = OB_ERR_UNEXPECTED;
        ...
        {
            if (OB_SUCCESS != (ret = out_row_desc_ext_.get_by_idx(column_idx, table_id, column_id, data_type)))
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Failed to get column, err=%d",ret);
            }
        }
        if (OB_SUCCESS == ret && (OB_SUCCESS!=(ret = obj_cast(*single_value, data_type, casted_cell, single_value))))
        {
            YYSYS_LOG(WARN, "failed to cast obj, table_id=%lu, column_id=%lu,err=%d, ",table_id,column_id,ret);
        }
    }
    //else
    //{
