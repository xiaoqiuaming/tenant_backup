...
ObRowCount::ObRowCount()
  : table_id_(OB_INVALID_ID), column_id_(OB_INVALID_ID), when_func_index_(OB_INVALID_INDEX), already_read_(false)
{
}
...
int ObRowCount::set_child(int32_t child_idx, ObPhyOperator &child_operator)
{
      ...
      && child_operator.get_type() != PHY_UPS_MODIFY_WITH_DML_TYPE)
  {
    ...
    YYSYS_LOG(ERROR, "ObRowCount child must be ObUpsModify or ObUpsModifyWithDmlType type");
  }
  else
  {
...
int ObRowCount::cons_row_desc()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "Table Id/Column Id must be set");
  }
...
int ObRowCount::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "ObRowCount must be opened first");
  }
  else
  {
    row_desc = &row_desc_;
...
int ObRowCount::get_next_row(const common::ObRow *&row)
{
  ...
  else if (!already_read_)
  {
    ...
    {
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "ObRowCount child must be ObAffectedRows type");
    }
    else if ((ret = ups_affected_rows->get_affected_rows(row_count)) != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "Get affected row failed, ret=%d", ret);
    }
    else
    {
      ObObj row_num;
      ...
      {
        YYSYS_LOG(WARN, "Failed to set row cell ret=%d", ret);
      }
      else
      {
        row = &row_;
...
DEFINE_SERIALIZE(ObRowCount)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to serialize table_id_. ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = encode_vi64(buf, buf_len, pos, static_cast<int64_t>(column_id_))))
  {
    YYSYS_LOG(WARN, "fail to serialize column_id_. ret=%d", ret);
  }
  return ret;
}
...
DEFINE_DESERIALIZE(ObRowCount)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize table_id. ret=%d", ret);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS != (ret = decode_vi64(buf, data_len, pos, &val)))
    {
      YYSYS_LOG(WARN, "fail to deserialize column_id. ret=%d", ret);
    }
    else
    {
