...
bool ObTabletJoin::check_inner_stat()
{
  ...
  {
    ret = false;
    YYSYS_LOG(WARN, "check inner stat fail:fused_scan_[%p], batch_count_[%ld]", fused_scan_, batch_count_);
  }
  return ret;
}
  ...
  switch(child_idx)
  {
    case 0:
      ...
      if(NULL == fused_scan_)
      {
        ...
        YYSYS_LOG(WARN, "child operator is not ObTabletFuse");
      }
      break;
    default:
...
int ObTabletJoin::open()
{
  ...
  {
    YYSYS_LOG(WARN, "open fused scan fail:ret[%d]", ret);
  }
  else if(OB_SUCCESS != (ret = gen_ups_row_desc()))
  {
    YYSYS_LOG(WARN, "gen ups row desc:ret[%d]", ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "table_join_info_[%s]", to_cstring(table_join_info_));
  }
...
int ObTabletJoin::close()
{
  ...
  if(NULL != fused_scan_)
  {
    ...
    {
      YYSYS_LOG(WARN, "close fused scan fail:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObTabletJoin::compose_get_param(uint64_t table_id, const ObRowkey &rowkey, ObGetParam &get_param)
{
  ...
  else
  {
    for(int64_t i=0; (OB_SUCCESS == ret) && i<table_join_info_.join_column_.count();i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "get join info fail:ret[%d], i[%ld]", ret, i);
      }
      else
      {
        ...
        if (OB_SIZE_OVERFLOW == ret)
        {
          if (0 != i)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to rollback get param:ret[%d], i[%ld]", ret, i);
            }
            else
            {
          ...
          YYSYS_LOG(INFO, "batch count is too large[%ld], get param capability[%ld]", batch_count_, get_param.get_row_size());
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "add cell info to get_param fail:ret[%d]", ret);
        }
      }
    }
  }
...
int ObTabletJoin::get_right_table_rowkey(const ObRow &row, ObRowkey &rowkey, ObObj *rowkey_obj) const
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "rowkey_obj is null");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    for(int32_t i=0;OB_SUCCESS == ret && i<count;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "get join condition fail:ret[%d], i[%d]", ret, i);
      }
      else if(OB_SUCCESS != (ret = row.get_cell(table_join_info_.left_table_id_, left_column_id, cell)))
      {
        YYSYS_LOG(WARN, "row get cell fail:ret[%d]", ret);
      }
      else if( i >= OB_MAX_ROWKEY_COLUMN_NUMBER )
      {
        ret = OB_SIZE_OVERFLOW;
        YYSYS_LOG(WARN, "join condition array length is great than right table rowkey obj length:i[%d]", i);
      }
      else
      {
        rowkey_obj[i] = *cell;
...
int ObTabletJoin::fetch_fused_row(ObGetParam *get_param)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "get param is null");
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fetch fused row prepare fail:ret[%d]", ret);
    }
  }
  ...
  for(int64_t i=0;(OB_SUCCESS == ret) && i < batch_count_;i++)
  {
    if(NULL == fused_row_)
    {
      ...
      {
        YYSYS_LOG(WARN, "get next row from fused scan fail:ret[%d]", ret);
      }
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SIZE_OVERFLOW == ret)
      {
        YYSYS_LOG(DEBUG, "fused row store size overflow");
        ret = OB_SUCCESS;
        break;
      }
      else if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add row to row store fail:ret[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = fused_row_array_.push_back(stored_row)))
      {
        YYSYS_LOG(WARN, "add item to fused row array fail:ret[%d]", ret);
      }
      else
      {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "gen get param fail:ret[%d]", ret);
      }
      else
      {
        fused_row_ = NULL;
  ...
  YYSYS_LOG(DEBUG, "fetch fused row count[%ld]", fused_row_array_.count());
...
int ObTabletJoin::fetch_next_batch_row(ObGetParam *get_param)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != (ret = fetch_fused_row(get_param)))
    {
      if(OB_ITER_END != ret)
      {
        YYSYS_LOG(ERROR, "fetch_fused_row fail :ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "fused row iterate end.");
...
int ObTabletJoin::get_next_row(const ObRow *&row)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "fuse_scan_ is null");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    if (fused_row_idx_ >= valid_fused_row_count_)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "no more data in fused_row_.");
        ret = OB_ITER_END;
      }
      else
      {
        if(NULL == (get_param = GET_TSI_MULT(ObGetParam, common::TSI_SQL_GET_PARAM_1)))
        {
          ...
          YYSYS_LOG(WARN, "get tsi get_param fail:ret[%d]", ret);
        }
        else
        {
          get_param->reset(true);
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fetch next batch row fail:ret[%d]", ret);
          }
    ...
    {
      YYSYS_LOG(WARN, "get next row from fused row store fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "get item from fused row array fail:ret[%d], fused_row_idx_[%ld], array_count[%ld]", ret, fused_row_idx_, fused_row_array_.count());
    }
    else if(OB_SUCCESS != (ret = ObRowUtil::convert(stored_row->get_compact_row(), tmp_row)))
    {
      YYSYS_LOG(WARN, "convert ob row fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "get rowkey from tmp_row fail:ret[%d]", ret);
    }
    else if (OB_SUCCESS != (ret = get_ups_row(rowkey, tmp_ups_row, *get_param)))
    {
      YYSYS_LOG(WARN, "get ups row fail:rowkey[%s], ret[%d]", to_cstring(rowkey), ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "fuse_row: incrow:%s, sstable row:%s",
        to_cstring(tmp_ups_row), to_cstring(tmp_row));
    if(OB_SUCCESS != (ret = ObRowFuse::join_row(&tmp_ups_row, &tmp_row, &curr_row_)))
    {
      YYSYS_LOG(WARN, "fused ups row to row fail:ret[%d]", ret);
    }
    else
    {
      row = &curr_row_;
    }
    YYSYS_LOG(DEBUG, "fuse_row dest row:%s", to_cstring(curr_row_));
  }
...
int ObTabletJoin::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "fused_scan_ is null");
  }
  else if (OB_SUCCESS != (ret = fused_scan_->get_row_desc(row_desc)))
  {
    YYSYS_LOG(WARN, "fail to get row desc:ret[%d]", ret);
