...
int ObMergeExcept::distinct_get_next_row(const common::ObRow *&row)
{
        ...
        if (OB_SUCCESS == left_ret_ && OB_SUCCESS == right_ret_)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            if (cmp < 0)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to save cur_first_query_row_ to last row, ret=%d",ret);
              }
              break;
            }
            else if (cmp == 0)
  ...
  {
    YYSYS_LOG(WARN, "failed to do distinct on left_op, ret=%d", ret);
  }
  return ret;
}
int ObMergeExcept::all_get_next_row(const common::ObRow *&row)
{
  ...
  if (OB_SUCCESS == ret)
  {
    while (true)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "failed to get next row, ret=%d, left_ret_=%d, right_ret_=%d", ret, left_ret_, right_ret_);
          break;
        }
  ...
  {
    YYSYS_LOG(WARN, "failed to get next row  on left_op, ret=%d", ret);
  }
  return ret;
}
