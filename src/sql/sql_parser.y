...
%token READ_STATIC REAL RENAME REPLACE RESTRICT PRIVILEGES REVOKE RIGHT
       ROLLBACK KILL READ_CONSISTENCY  RULE RESTART /*liuzy [sequence] add "RESTART"*/
//add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151224:b
%token <ival> HINTINTNUM
%token READ_ATOMIC_WEAK READ_ATOMIC_STRONG  TIMELIMIT
//add duyr 20151224:e
...
%token WITHIN OVER
/*add 20141130:e*/
/*add liu jun.[Multi_UPS_alpha] 20150416:b*/  /*PARTITION*/
%token  FUNCTION FUNCTIONS
/*add liu jun.20150416:e*/
%token PAXOS_IDX
%token MOVE
/*add wuna [Multi_UPS_beta] 20151126:b*/ /*MAXVALUE */ 
%token LESS THAN HASH ENUM LIST RULES GROUPS PREFIX
/*add 20151126:e*/
%token HARDCODE
...
%token <non_reserved_keyword>
       ...
       UNLOCKED UPDATESERVER USE_BLOCK_CACHE USE_BLOOM_FILTER VARIABLES VERBOSE WARNINGS
...
/*  add 20150324:e*/
%type <node> alter_group_stmt group_name
...
%type <node> alter_table_option_actions alter_table_option_action
...
%type <node> opt_hint opt_hint_list hint_option //opt_force //del peiouya [MultiUPS] [UPS_Manage_Function] 20150504
...
/*add liu jun.[Multi_UPS_alpha] 20150416:b*/
%type <node> func_name prefix_group_name part_func_list table_func_list
%type <node> create_part_func_stmt parameter_list parameters function_body
%type <node> drop_part_func_stmt factor_list
/*add liu jun.20150416:e*/
/*add wuna [Multi_UPS_beta] 20151126:b*/
/*%type <node> part_range_def_list part_range_def range_value_list range_value hash_or_enum*/
%type <node> part_range_def_list part_range_def range_value_list range_value hash_or_enum_or_range_hardcode
%type <node> part_list_def_list part_list_def alter_part_rule range_list_prefix
/*add 20151126:e*/
%type <node> gather_statistics_stmt opt_gather_columns
%type <ival> opt_stat
%token INDEXS INVALID VALID INIT
%token PARTITION_CALC
%type <ival> opt_version

%type <node> create_view_stmt opt_with_check_option drop_view_stmt alter_view_stmt
%token VIEW CHECK CASCADED
%start sql_stmt
%%
...
stmt:
  ...
  | rollback_stmt {$$ = $1;}
  | kill_stmt {$$ = $1;}
  | create_part_func_stmt {$$ = $1;}/*add liu jun.[Multi_UPS_alpha] 20150416*/
  | drop_part_func_stmt { $$ = $1; } /*add by liu jun.[MultiUps_Live] 20150416.*/
...
//        || strcasecmp($1->str_value_, "current_timestamp") == 0
  ...
  | TIME '(' expr ')'
    ...
    //add 20151228:e
  | PARTITION_CALC '(' relation_factor ',' '(' insert_vals ')' opt_version ')'
    {
      ParseNode* func = NULL;
      malloc_terminal_node(func, result->malloc_pool_, T_IDENT);
      char *str = "partition_calc";
      func->str_value_ = parse_strdup(str, result->malloc_pool_);
      func->value_ = strlen(func->str_value_);
      ParseNode* version_flag = NULL;
      malloc_terminal_node(version_flag, result->malloc_pool_, T_INT);
      version_flag->value_ = $8;
      ParseNode *val_list = NULL;
      merge_nodes(val_list, result->malloc_pool_, T_VALUE_VECTOR, $6);
      malloc_non_terminal_node($$, result->malloc_pool_, T_PARTITION_CALC, 4, func, $3, version_flag, val_list);
    }
  ;
...
opt_version:
  ...
  | ',' INTNUM
  {
    if($2->value_ > 1)
    {
        yyerror(&@1, result, "partition calc func's version flag only be int type(-1,0,1)");
        YYABORT;
    }
    $$ = $2->value_;
  }
 ...
 * modify by LiuJun
 *0:'(' table_hash_list ')'
 *1:ParseNode *table_hashes = NULL;
 *2:merge_nodes(table_hash_list, result->malloc_pool_, T_TABLE_HASH_LIST, $10);
 *3:malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 5,
              $3,                   if not exists 
              $4,                   table name
              table_elements,       columns or primary key
              table_options,        table option(s) 
              table_hashes          added by LiuJun
              );
 *modify by wuna
 *****************************************************************************/
...
create_table_stmt:
    ...
    opt_table_option_list table_func_list
    {
      ...
      if($9 != NULL && $9->type_ == T_LINK_NODE)
      {
        ParseNode *table_funcs = NULL;
        merge_nodes(table_funcs, result->malloc_pool_, T_TABLE_HASH_LIST, $9);
        malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_TABLE, 5,
                $3,                   /* if not exists */
                $4,                   /* table name */
                table_elements,       /* columns or primary key */
                table_options ,        /* table option(s) */
                table_funcs          /*add liu jun.*/
                );
      }
      else
      {
...
table_element_list:
  ...
  *added by LiuJun.
  */
/*mod peiouya [Multi_UPS_alpha] 20141117:b*/
/*add wuna [Multi_UPS_beta] 20151126:b*/
table_func_list:
    PARTITION BY  '(' prefix_group_name part_func_list ')'
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $4, $5);
    }
  | PARTITION BY RANGE '(' expr ')' '(' part_range_def_list ')' range_list_prefix
    {
      ParseNode* range_list = NULL;
      merge_nodes(range_list, result->malloc_pool_, T_RANGE_DEF_LIST, $8);
      malloc_non_terminal_node($$, result->malloc_pool_, T_RANGE_PARTITION, 3, $5, range_list, $10);
    }
  | PARTITION BY RANGE COLUMNS '(' column_list ')' '(' part_range_def_list ')' range_list_prefix
    {
      ParseNode* col_list = NULL;
      ParseNode* range_list = NULL;
      merge_nodes(col_list, result->malloc_pool_, T_COLUMN_LIST, $6);
      merge_nodes(range_list, result->malloc_pool_, T_RANGE_DEF_LIST, $9);
      malloc_non_terminal_node($$, result->malloc_pool_, T_RANGE_COLUMNS_PARTITION, 3, col_list, range_list, $11);
    }
  | PARTITION BY LIST '(' expr ')' '(' part_list_def_list ')' range_list_prefix
    {
     ParseNode* def_list = NULL;
     merge_nodes(def_list, result->malloc_pool_, T_LIST_DEF_LIST, $8);
     malloc_non_terminal_node($$, result->malloc_pool_, T_LIST_PARTITION, 3, $5, def_list, $10);
    }
  | PARTITION BY LIST COLUMNS '(' column_list ')' '(' part_list_def_list ')' range_list_prefix
   {
    ParseNode* col_list = NULL;
    ParseNode* def_list = NULL;
    merge_nodes(col_list, result->malloc_pool_, T_COLUMN_LIST, $6);
    merge_nodes(def_list, result->malloc_pool_, T_LIST_DEF_LIST, $9);
    malloc_non_terminal_node($$, result->malloc_pool_, T_LIST_COLUMNS_PARTITION, 3, col_list, def_list, $11);
   }
  | /* EMPTY */
    { $$ = NULL; }
    ;
part_list_def_list:
    part_list_def {$$=$1;}
  | part_list_def_list ',' part_list_def
   {
     malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $3);
   }
   ;
part_list_def:
   PARTITION func_name VALUES IN '(' expr_list ')'
   {
     ParseNode* list_val_list = NULL;
     merge_nodes(list_val_list, result->malloc_pool_, T_EXPR_LIST, $6);
     malloc_non_terminal_node($$, result->malloc_pool_, T_LIST_DEF, 2, $2, list_val_list);
   }
   ;
part_range_def_list:
    part_range_def {$$=$1;}
  | part_range_def_list ',' part_range_def
   {
     malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $3);
   }
   ;
...
part_range_def:
   PARTITION func_name VALUES LESS THAN '(' range_value_list ')'
   {
     ParseNode* range_val_list = NULL;
     merge_nodes(range_val_list, result->malloc_pool_, T_RANGE_VAL_LIST, $7);
     malloc_non_terminal_node($$, result->malloc_pool_, T_RANGE_DEF, 2, $2, range_val_list);
   }
 | PARTITION func_name VALUES LESS THAN MAXVALUE
   {
     ParseNode* maxval = NULL;
     ParseNode* range_val = NULL;
     malloc_terminal_node(maxval, result->malloc_pool_, T_TYPE_MAXVALUE);
     malloc_non_terminal_node(range_val, result->malloc_pool_, T_RANGE_VAL_LIST, 1, maxval);
     malloc_non_terminal_node($$, result->malloc_pool_, T_RANGE_DEF, 2, $2, range_val);
   }
   ;

range_value_list:
   range_value {$$=$1;}
 | range_value_list ',' range_value
   {
     malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $3);
   }
   ;
...
range_value:
 ...
 | MAXVALUE
   {
     malloc_terminal_node($$, result->malloc_pool_, T_TYPE_MAXVALUE);
   }
   ;

range_list_prefix:
   WITH PREFIX '(' func_name ')'
   {
     $$=$4;
   }
 | /* EMPTY */
...
part_func_list:
    ',' func_name ',' column_list
    {
      ...
      malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $2, col_list);
    }
  | /* EMPTY */
    { $$ = NULL; }
  ;
...
table_option:
  ...
  | EXPIRE_INFO
    {
      malloc_terminal_node($$, result->malloc_pool_, T_EXPIRE_INFO);
    }
  ...
  | USE_BLOCK_CACHE opt_equal_mark BOOL
    {
      (void)($2) ; /* make bison mute */
      malloc_non_terminal_node($$, result->malloc_pool_, T_USE_BLOCK_CACHE, 1, $3);
    }
...
/*****************************************************************************
 *
 *	gather statistics grammar //add by zhangeb[statistics info] 20200228
 *
 *****************************************************************************/
...
hint_option:
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151224:b
  | READ_ATOMIC_WEAK  weak_hint_num
    {
      malloc_terminal_node($$, result->malloc_pool_, T_READ_ATOMIC_WEAK);
      $$->value_ = $2;
    }
  | READ_ATOMIC_STRONG
    {
      malloc_terminal_node($$, result->malloc_pool_, T_READ_ATOMIC_STRONG);
    }
  //add duyr 20151224:e
  | NOT_USE_INDEX
    {
...
//add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151224:b
weak_hint_num:
    TIMELIMIT  '=' HINTINTNUM
    {
        if ($3 < 0)
        {
            yyerror(&@1, result, "TIMELIMIT must not be less than 0!");
            YYABORT;
        }
        $$ = $3;
    }
...
join_type:
  ...
  | MERGE
    {
      $$ = NULL;
      yyerror(&@1, result, "Merge join can not be the join type!");
      YYABORT;
    }
  | BLOOMFILTER
    {
      $$ = NULL;
      yyerror(&@1, result, "Bloomfilter join can not be the join type!");
      YYABORT;
    }
  | SEMI
    {
      $$ = NULL;
      yyerror(&@1, result, "Semi join can not be the join type!");
      YYABORT;
    }
  | LEFT SEMI
    {
      $$ = NULL;
      yyerror(&@1, result, "Left semi join can not be the join type!");
      YYABORT;
    }
  | RIGHT SEMI
    {
      $$ = NULL;
      yyerror(&@1, result, "Right semi join can not be the join type!");
      YYABORT;
    }
  ;
...
show_stmt:
  ...
  | SHOW opt_stat INDEXS opt_show_condition
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_SHOW_ALL_INDEX, 1, $4);
      $$->value_ = $2;
    }
  | SHOW CREATE INDEX ON relation_factor
    { malloc_non_terminal_node($$, result->malloc_pool_, T_SHOW_CREATE_INDEX, 1, $5); }
  ...
  | SHOW PARTITION FUNCTIONS opt_show_condition
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_SHOW_FUNCTIONS, 1, $4);
    }
    /*add 20150325:e*/
    /*add wuna [MultiUps] 20160223:b*/
  | SHOW TABLE RULES opt_show_condition
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_SHOW_TABLE_RULES, 1, $4);
    }
  | SHOW PARTITION GROUPS opt_show_condition
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_SHOW_GROUPS, 1, $4);
    }
    /*add 20160223:e*/
  | SHOW CURRENT PAXOS_IDX
    {
      malloc_terminal_node($$, result->malloc_pool_, T_SHOW_CURRENT_PAXOS_ID);
    }
  ;
...
/*****************************************************************************
 *
 *	create and drop partition function grammar
 *
 *****************************************************************************/

/*add by liu jun. [Multi_UPS_alpha] 20150416:b*/
/*mod by wuna [MultiUps_beta] 20151225:b*/
create_part_func_stmt:
/* mod by maosy 2017/02/28 b*/
/*CREATE hash_or_enum PARTITION FUNCTION relation_factor parameter_list function_body*/
    /*CREATE hash_or_enum PARTITION FUNCTION relation_name parameter_list function_body*/
    CREATE hash_or_enum_or_range_hardcode PARTITION FUNCTION relation_name parameter_list function_body
    /* mod 2017/02/28 e */
    {
      ParseNode *func_body = NULL;
      merge_nodes(func_body, result->malloc_pool_, T_FUNC_ENTITY,$7);
      malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_PART_FUNC, 4,
              $2,                   /*partition type*/
              $5,                   /*fuction name */
              $6,                   /*parameters*/
              func_body              /*operation expression*/
              );
    }
  ;

//hash_or_enum:
hash_or_enum_or_range_hardcode:
   HASH {malloc_terminal_node($$, result->malloc_pool_, T_TYPE_HASH);}
  |ENUM {malloc_terminal_node($$, result->malloc_pool_, T_TYPE_ENUM);}
  |RANGE {malloc_terminal_node($$, result->malloc_pool_, T_TYPE_RANGE);}
  |HARDCODE {malloc_terminal_node($$, result->malloc_pool_, T_TYPE_HARDCODE);}
  ;
/*mod 20151225:e*/
...
parameter_list:
    '(' parameters ')'
    {
      merge_nodes($$, result->malloc_pool_, T_PARAMETER_LIST, $2);
    }
  ;
...
drop_part_func_stmt:
    DROP PARTITION FUNCTION factor_list
    {
      ParseNode *funcs = NULL;
      merge_nodes(funcs, result->malloc_pool_, T_FUNC_LIST, $4);
      malloc_non_terminal_node($$, result->malloc_pool_, T_DROP_PART_FUNC, 1, funcs);
    }
  ;
...
/* mod by maosy 2017/02/28 b*/
    ...
    /* mod by maosy 2017/02/28 b*/
  /*  | factor_list ',' relation_factor*/
  | factor_list ',' relation_name
  /* mod 2017/02/28 e */
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $3);
    }
...
/*add 20150428:e*/
alter_group_stmt:
    ALTER GROUP group_name MOVE TO PAXOS_IDX COMP_EQ INTNUM
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_GROUP, 2, $3, $8);
    }
    ;
group_name:
    relation_name
...
var_and_val_list:
  ...
  | OPTION SQL_SELECT_LIMIT COMP_EQ DEFAULT
    {
      ParseNode *node1 = NULL;
      malloc_terminal_node(node1, result->malloc_pool_, T_SYSTEM_VARIABLE);
      node1->str_value_ = "sql_select_limit";
      ParseNode *node2 = NULL;
      malloc_terminal_node(node2, result->malloc_pool_, T_INT);
      node2->value_ = 1000000;
      malloc_non_terminal_node($$,result->malloc_pool_, T_VAR_VAL, 2, node1, node2);
    }
  | OPTION SQL_SELECT_LIMIT COMP_EQ INTNUM
    {
      ParseNode *node1 = NULL;
      malloc_terminal_node(node1, result->malloc_pool_, T_SYSTEM_VARIABLE);
      node1->str_value_ = "sql_select_limit";
      malloc_non_terminal_node($$,result->malloc_pool_, T_VAR_VAL, 2, node1, $4);
    }
  ;
...
alter_table_stmt:
  ...
  | ALTER TABLE relation_factor RENAME TO relation_factor
   ...
   //add.e
    //mod wuna [MultiUps] [sql_api] 20160106:b
    //add liu jun. [MultiUPS] [sql_api] 20150530:b
  | ALTER TABLE relation_factor PARTITION RULE TO '(' alter_part_rule ')'
    {
      ParseNode *alter_actions = NULL;
      merge_nodes(alter_actions, result->malloc_pool_, T_RULE_MODIFY, $8);
      malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_ACTION_LIST, 1, alter_actions);
      malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_TABLE, 2, $3, $$);
    }
  | ALTER TABLE relation_factor alter_table_option_actions
    {
      ParseNode *alter_table_option_actions = NULL;
      merge_nodes(alter_table_option_actions, result->malloc_pool_, T_ALTER_TABLE_OPTION_LIST, $4);
      malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_TABLE, 2, $3, alter_table_option_actions);
    }
 /* | ALTER GROUP relation_factor TO */
    //20150530:e
    //20160106:e
  ;
//mod wuna [MultiUps] [sql_api] 20160106:b
alter_part_rule:
    prefix_group_name part_func_list
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $2);
    }
  ;
//20160106:e
alter_table_option_actions:
    alter_table_option_action
    {
        $$ = $1;
    }
  | alter_table_option_actions ',' alter_table_option_action
    {
        malloc_non_terminal_node($$, result->malloc_pool_, T_LINK_NODE, 2, $1, $3);
    }
  ;
alter_table_option_action:
    ADD CURRENT table_option
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_TABLE_OPTION_ADD, 1, $3);
    }
  | DROP CURRENT table_option
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_TABLE_OPTION_DROP, 1, $3);
    }
  | ALTER CURRENT table_option
    {
      malloc_non_terminal_node($$, result->malloc_pool_, T_TABLE_OPTION_ALTER, 1, $3);
    }
  ;
...
alter_column_behavior:
  ...
  | SET NAME NAME VARCHAR '('INTNUM')'
    {
      if (strcasecmp($2->str_value_, "data") != 0 || strcasecmp($3->str_value_, "type") != 0)
      {
        yyerror(&@1, result, "You have an error in your SQL syntax!");
        YYABORT;
      }
      else
      {
        malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_VARCHAR_LENGTH, 1, $6);
      }
    }
  | SET NAME NAME DECIMAL '('INTNUM ',' INTNUM ')'
    {
      if (strcasecmp($2->str_value_, "data") != 0 || strcasecmp($3->str_value_, "type") != 0)
      {
        yyerror(&@1, result, "You have an error in your SQL syntax!");
      ...
      {
        malloc_non_terminal_node($$, result->malloc_pool_, T_ALTER_DECIMAL_PRECISION, 2, $6, $8);
      }
    }
  ;
...
//del peiouya [MultiUPS] [UPS_Manage_Function] 20150429:b
//    |
...
create_view_stmt:
    CREATE opt_or_replace VIEW relation_factor opt_insert_columns AS select_stmt opt_with_check_option
    {
        malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_VIEW, 5,
                                 $2, //or replace
                                 $4, //relation_factor
                                 $5, //(a,b,c,...)
                                 $7, //select stmt
...
opt_with_check_option:
    ...
    {
        malloc_terminal_node($$, result->malloc_pool_, T_INT);
        $$->value_ = 1; //default: with cascaded check option
    }
...
drop_view_stmt:
    DROP VIEW opt_if_exists table_list
    {
        ParseNode *view_list = NULL;
        merge_nodes(view_list, result->malloc_pool_, T_TABLE_LIST, $4);
        malloc_non_terminal_node($$, result->malloc_pool_, T_DROP_VIEW, 2, $3, view_list);
    }
    ;

alter_view_stmt:
    ALTER VIEW relation_factor opt_insert_columns AS select_stmt opt_with_check_option
    {
        ParseNode *or_replace = NULL;
        malloc_terminal_node(or_replace, result->malloc_pool_, T_SEQUENCE_REPLACE);
        malloc_non_terminal_node($$, result->malloc_pool_, T_CREATE_VIEW, 5,
