...
int ObLimit::get_int_value(const ObSqlExpression& in_val, int64_t& out_val) const
{
  ...
  else if (result->get_type() != ObIntType)
  {
    ...
    {
      YYSYS_LOG(ERROR, "Cast limit/offset value failed, err=%d", ret);
    }
  }
  if (ret == OB_SUCCESS)
  {
...
int ObLimit::get_limit(int64_t &limit, int64_t &offset) const
{
  ...
  {
    YYSYS_LOG(WARN, "Get limit/offset value failed, err=%d", ret);
  }
  else if (limit < -1 || offset < 0)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "Invalid arguments, limit=%ld offset=%ld", limit_, offset_);
  }
  return ret;
}
...
int ObLimit::open()
{
  ...
  {
    YYSYS_LOG(WARN, "Failed to instantiate limit/offset, err=%d", ret);
  }
  else
  {
...
int ObLimit::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "child_op_ is NULL or invalid offset=%ld", offset_);
  }
  else
  {
...
int ObLimit::get_next_row(const common::ObRow *&row)
{
  ...
  else
  {
    while (input_count_ < offset_)
    {
      ...
      {
        YYSYS_LOG(WARN, "child_op failed to get next row, err=%d", ret);
        break;
      }
      else
      {
    ...
    if (OB_SUCCESS == ret)
    {
      if (output_count_ < limit_ || 0 > limit_)
      {
        if (OB_SUCCESS != (ret = child_op_->get_next_row(input_row)))
        {
          if (OB_ITER_END != ret)
          {
            YYSYS_LOG(WARN, "child_op failed to get next row, err=%d, limit_=%ld, offset_=%ld, input_count_=%ld, output_count=%ld",
...
#define ENCODE_EXPRESSION(has_part, expr) \
  ...
  { \
    ...
    { \
      YYSYS_LOG(WARN, "fail to encode " #expr ":ret[%d]", ret); \
    } \
    else if (has_part && OB_SUCCESS != (ret = expr.serialize(buf, buf_len, pos))) \
    { \
      YYSYS_LOG(WARN, "fail to serialize " #expr ":ret[%d]", ret); \
    } \
  }
...
#define DECODE_EXPRESSION(expr) \
  ...
  { \
    ...
    { \
      YYSYS_LOG(WARN, "fail to decode Limit/Offset expression :ret[%d]", ret); \
    } \
    else if (has_part && OB_SUCCESS != (ret = expr.deserialize(buf, data_len, pos))) \
    { \
      YYSYS_LOG(WARN, "fail to deserialize " #expr ":ret[%d]", ret); \
    } \
  }
  org_limit_.reset();
