...
int ObPrivExecutor::commit()
{
  ...
  if(OB_SUCCESS == ret && context_->session_info_->get_autocommit()==false)
  {
      ret = execute_stmt_no_return_rows(start_thx);
  }
  return ret;
...
int ObPrivExecutor::open()
{
  ...
  //add liu jun. [MultiUPS] [part_cache] 20150626:b
  ObMsPartitionManager *temp = dynamic_cast<ObMsPartitionManager *>(context_->part_monitor_->get_partition_manager());
  if(NULL == temp)
  {
    ret = OB_ERROR;
  ...
  {
    ret = OB_SCHEMA_ERROR;
    YYSYS_LOG(WARN, "get schema error, ret=%d", ret);
  }
  else
  {
    switch(stmt_->get_stmt_type())
    {
      ...
      default:
        ...
        YYSYS_LOG(DEBUG, "not a privilege-related sql, ret=%d", ret);
        break;
    };
    ...
    if (OB_SUCCESS != err)
    {
      ...
      //add liu jun. [MultiUPS] [part_cache] 20150626:b
      if(context_->partition_mgr_ != NULL)
      {
        if(OB_SUCCESS != (err = context_->part_monitor_->release_manager(context_->partition_mgr_)))
        {
...
int ObPrivExecutor::get_all_columns_by_user_name(const ObString &user_name, UserInfo &user_info)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", select_user.length(), select_user.ptr(), ret);
      result_set_out_->set_message(result2.get_message());
    }
    else if (OB_SUCCESS != (ret = result2.open()))
    {
      YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", select_user.length(), select_user.ptr(), ret);
    }
    else
    {
      ...
      else if (OB_SUCCESS == ret)
      {
        ...
        for (i = 0;i < field_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "raw get cell failed, ret=%d", ret);
            break;
          }
          else if (pcell->get_type() != ObIntType)
          {
            ...
            {
              ret = OB_ERR_UNEXPECTED;
              YYSYS_LOG(WARN, "type is not expected, type=%d", pcell->get_type());
              break;
            }
          }
          else if (OB_SUCCESS != (ret = pcell->get_int(field_value)))
          {
            YYSYS_LOG(WARN, "get value from cell failed, ret=%d", ret);
            break;
          }
          else if (OB_SUCCESS != (ret = user_info.field_values_.push_back(field_value)))
          {
            YYSYS_LOG(WARN, "push field value to array failed, ret=%d", ret);
            break;
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          else if (pcell->get_type() != ObVarcharType)
          {
            ...
            {
              ret = OB_ERR_UNEXPECTED;
              YYSYS_LOG(WARN, "type is not expected, type=%d, ret=%d", pcell->get_type(), ret);
            }
          }
          ...
          {
            YYSYS_LOG(WARN, "ob_write_string failed, ret=%d", ret);
          }
...
int ObPrivExecutor::revoke_all_priv_from_table_privs_by_user_name(const ObString &user_name)
{
  ...
  {
    YYSYS_LOG(ERROR, "get user id by user name failed, user_name=%.*s, ret=%d", user_name.length(), user_name.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = get_table_ids_by_user_id(user_id, table_ids)))
  {
    YYSYS_LOG(ERROR, "get table ids from user id failed, user_id=%ld, ret=%d", user_id, ret);
  }
  else
  {
    int i = 0;
    for (i = 0;i < table_ids.count();++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "revoke privilege from __all_table_privilege failed, user_id=%lu, table_id=%lu, ret=%d", user_id, tid, ret);
        break;
      }
    }
  }
...
int ObPrivExecutor::revoke_all_priv_from_db_privs_by_user_name(const ObString &user_name)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get db ids from user id failed, user_id=%ld, ret=%d", user_id, ret);
    }
    else
    {
      for (int64_t db_index = 0;db_index < db_ids.count() && OB_LIKELY(OB_SUCCESS == ret);db_index++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "revoke privilege from __all_database_privilege failed, user_id=%ld, db_id=%ld, ret=%d", user_id, db_id, ret);
        }
        else if (OB_SUCCESS != (ret = get_table_ids_by_user_db_id(user_id, db_id, table_ids)))
        {
          YYSYS_LOG(ERROR, "get table ids from user id and db id failed, user_id=%ld, db_id=%ld ret=%d", user_id, db_id, ret);
        }
        else
        {
          for (int64_t table_index = 0; table_index < table_ids.count() && OB_LIKELY(OB_SUCCESS == ret);table_index++)
          {
            ...
            {
              YYSYS_LOG(ERROR, "revoke privilege from __all_table_privilege failed, user_id=%ld, db_id=%ld, table_id=%lu, ret=%d", user_id, db_id, tid, ret);
            }
          }//table for
        }
      }//db for
...
int ObPrivExecutor::revoke_priv_from_db_privs_by_user_db_id(int64_t user_id, int64_t db_id, const ObArray<ObPrivilegeType> *privileges)
{
  ...
  {
    YYSYS_LOG(ERROR, "revoke database privs from user_id=%ld failed, db_id=%ld, ret=%d", user_id, db_id, ret);
  }
  else if (OB_SUCCESS != (ret = get_table_ids_by_user_db_id(user_id, db_id, table_ids)))
  {
    YYSYS_LOG(ERROR, "get table ids from __all_table_privilege by user id and db_id failed, user_id=%ld, db_id=%ld, ret=%d", user_id, db_id, ret);
  }
  for(int64_t table_index = 0; table_index < table_ids.count() && OB_LIKELY(OB_SUCCESS == ret); table_index++)
  {
    uint64_t tid = table_ids.at(table_index);
    if (OB_SUCCESS != (ret = reset_table_privilege_v2(user_id, db_id, tid, privileges)))
    {
      YYSYS_LOG(ERROR, "revoke table privs from user_id=%ld failed, db_id=%ld, tid=%lu, ret=%d", user_id, db_id, tid, ret);
    }
  }
  return ret;
}
...
int ObPrivExecutor::get_db_ids_by_user_id(int64_t user_id, ObArray<int64_t> &db_ids)
{
  ...
  else if (OB_SUCCESS != (ret = ObSql::direct_execute(select_db_id, tmp_result, *context_)))
  {
    ...
    YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", select_db_id.length(), select_db_id.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", select_db_id.length(), select_db_id.ptr(), ret);
  }
  else
  {
    ...
    while (true)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          if (pcell->get_type() == ObIntType)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "push db id=%ld to array failed, ret=%d", db_id, ret);
              }
            }
          }
          else
        ...
        {
          YYSYS_LOG(WARN, "raw get cell(db_id) failed, ret=%d", ret);
        }
      }
    }// while
  }
...
int ObPrivExecutor::get_table_ids_by_user_db_id(int64_t user_id, int64_t db_id, ObArray<uint64_t> &table_ids)
{
  ...
  {
    context_->session_info_->get_current_result_set()->set_message(tmp_result.get_message());
    YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", select_table_id.length(), select_table_id.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", select_table_id.length(), select_table_id.ptr(), ret);
  }
  else
  {
    OB_ASSERT(tmp_result.is_with_rows() == true);
    ...
    while (true)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          if (pcell->get_type() == ObIntType)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "push table id=%lu to array failed, ret=%d", table_id, ret);
              }
            }
          }
          else
        ...
        {
          YYSYS_LOG(WARN, "raw get cell(table_id) failed, ret=%d", ret);
        }
      }
    }// while
  }
...
int ObPrivExecutor::execute_update_user(const ObString &update_user)
{
  ...
  else if (OB_SUCCESS != (ret = ObSql::direct_execute(update_user, tmp_result, *context_)))
  {
    ...
    YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", update_user.length(), update_user.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", update_user.length(), update_user.ptr(), ret);
  }
  else
  {
    ...
    if (affected_rows == 0)
    {
      YYSYS_LOG(WARN, "user not exist, sql=%.*s", update_user.length(), update_user.ptr());
      result_set_out_->set_message("user not exist");
      ret = OB_ERR_USER_NOT_EXIST;
    }
...
int ObPrivExecutor::execute_delete_database(const ObString &delete_db)
{
  ...
  {
    result_set_out_->set_message(tmp_result.get_message());
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d",delete_db.length(),delete_db.ptr(), ret);
  }
  else if(OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", delete_db.length(), delete_db.ptr(), ret);
  ...
  else
  {
    ...
    if(affected_rows == 0)
    {
      YYSYS_LOG(WARN,"delete db failed,sql = %.*s", delete_db.length(), delete_db.ptr());
...
int ObPrivExecutor::execute_delete_user(const ObString &delete_user)
{
  ...
  {
    result_set_out_->set_message(tmp_result.get_message());
    YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", delete_user.length(), delete_user.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", delete_user.length(), delete_user.ptr(), ret);
  }
  else
  {
    OB_ASSERT(tmp_result.is_with_rows() == false);
    ...
    if (affected_rows == 0)
    {
      YYSYS_LOG(WARN, "delete user failed, sql=%.*s", delete_user.length(), delete_user.ptr());
...
int ObPrivExecutor::get_user_id_by_user_name(const ObString &user_name, int64_t &user_id)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", select_user_id.length(), select_user_id.ptr(), ret);
      result_set_out_->set_message(result2.get_message());
    }
    else if (OB_SUCCESS != (ret = result2.open()))
    {
      YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", select_user_id.length(), select_user_id.ptr(), ret);
    }
    else
    {
      OB_ASSERT(result2.is_with_rows() == true);
      ...
      if (OB_SUCCESS != ret)
      {
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(WARN, "user_name: %.*s not exists, ret=%d", user_name.length(), user_name.ptr(), ret);
          ret = OB_ERR_USER_NOT_EXIST;
          result_set_out_->set_message("user not exists");
        }
        else
        {
          YYSYS_LOG(WARN, "next row from ObResultSet failed,ret=%d", ret);
        }
      }
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN, "got type of %d cell from __all_user row, expected type=%d", pcell->get_type(), ObIntType);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "raw get cell(user_id) from __all_user failed, ret=%d", ret);
        }
      }
      int err = result2.close();
      if (OB_SUCCESS != err)
  ...
  else
  {
    ...
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(WARN, "db_name: %.*s not exists, ret=%d", db_name.length(), db_name.ptr(), ret);
          ret = OB_ERR_DATABASE_NOT_EXSIT;
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN, "got type of %d cell from __all_database row, expected type=%d", pcell->get_type(), ObIntType);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "raw get cell(db_id) from __all_database failed, ret=%d", ret);
        }
      }
      int err = result2.close();
      if (OB_SUCCESS != err)
...
int ObPrivExecutor::do_revoke_privilege(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast revoke stmt failed, ret=%d", ret);
  }
  else
  {
    // start transaction
    ...
    else
    {
      ...
      for (i = 0;i < user_count;++i)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get user from grant stmt failed, ret=%d", ret);
          break;
        }
        else
        {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if ((OB_NOT_EXIST_TABLE_TID == table_id) || (table_id != OB_INVALID_ID))
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(ERROR, "revoke privilege from __all_table_privilege failed, user_id=%ld, table_id=%lu, ret=%d", user_id, table_id, ret);
                }
              }
            }
          }
          ...
          else if (OB_INVALID_ID == table_id)
          {
            ...
            {
              YYSYS_LOG(ERROR, "revoke all users priv from username=%.*s failed, ret=%d", user_name.length(), user_name.ptr(), ret);
            }
            else if (OB_SUCCESS != (ret = revoke_all_priv_from_table_privs_by_user_name(user_name)))
            {
              YYSYS_LOG(ERROR, "revoke all table privs from username=%.*s failed, ret=%d", user_name.length(), user_name.ptr(), ret);
            }
          }
        }
      }// for (i = 0;i < user_count;++i)
      ...
      for (i = 0;i < user_count && OB_LIKELY(OB_SUCCESS == ret);++i)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (table_id != OB_INVALID_ID)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              if (revoke_stmt->is_global_priv() && (OB_NOT_EXIST_TABLE_TID == table_id))
              {
                ...
                if (OB_SUCCESS != (ret = get_db_ids_by_user_id(user_id, db_ids)))
                {
                  YYSYS_LOG(ERROR, "get db ids from __all_database_privilege by user id failed, username=%.*s, ret=%d", user_name.length(), user_name.ptr(), ret);
                }
                for(int64_t db_index = 0; db_index < db_ids.count() && OB_LIKELY(OB_SUCCESS == ret); db_index++)
                {
                  int64_t db_id = db_ids.at(db_index);
                  ...
                  {
                    YYSYS_LOG(ERROR, "revoke db&table privs from username=%.*s failed, db_id=%ld, ret=%d", user_name.length(), user_name.ptr(), db_id, ret);
                  }
                }//outer for
				//add liumz, [bugfix: grant table level priv, revoke global level priv]20150902:b
                if (OB_SUCCESS == ret && OB_SUCCESS != (ret = revoke_priv_from_table_privs_by_user_name_v2(user_name, privileges)))
                {
                  YYSYS_LOG(ERROR, "revoke table privs from username=%.*s failed, ret=%d", user_name.length(), user_name.ptr(), ret);
                }
				//add:e
              }
              else if (!revoke_stmt->is_global_priv())
              {
                ...
                if (OB_NOT_EXIST_TABLE_TID == table_id)
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "get db_id by db_name failed, ret=%d", ret);
                  }
                ...
                else if (OB_INVALID_ID != table_id)
                {
                  ...
                  if (!IS_SYS_TABLE_ID(table_id))
                  {
                    ...
                    {
                      YYSYS_LOG(ERROR, "revoke database privs from username=%.*s failed, db_id=%ld, ret=%d", user_name.length(), user_name.ptr(), db_id, ret);
                    }
                  }
                  if (OB_SUCCESS == ret)
                  {
                    ...
                    {
                      YYSYS_LOG(ERROR, "revoke table privs from username=%.*s failed, db_id=%ld, tid=%lu, ret=%d", user_name.length(), user_name.ptr(), db_id, table_id, ret);
                    }
                  }
                }
                else
          ...
          else if (OB_INVALID_ID == table_id)
          {
            ...
            {
              YYSYS_LOG(ERROR, "revoke all database privs and table privs from username=%.*s failed, ret=%d", user_name.length(), user_name.ptr(), ret);
            }
			//add liumz, [bugfix: grant table level priv, revoke global level priv]20150902:b
...
int ObPrivExecutor::do_grant_privilege(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast grant stmt failed, ret=%d", ret);
  }
  else
  {
    // start transaction
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (grant_stmt->is_global_priv() && (OB_NOT_EXIST_TABLE_TID == table_id))
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "grant global privs to [%.*s] failed ,ret=%d", user_name.length(), user_name.ptr(), ret);
              }
              else if (OB_SUCCESS != (ret = grant_db_privs_by_user_id(user_id, privileges)))
              {
                YYSYS_LOG(WARN, "grant privs on all databases to [%.*s] failed ,ret=%d", user_name.length(), user_name.ptr(), ret);
              }
            }
          }
          else if (!grant_stmt->is_global_priv())
          {
            ...
            {              
              ...
              if (OB_SUCCESS != (ret = get_db_id_by_db_name(db_name, db_id)))
              {
                YYSYS_LOG(ERROR, "get db_id by db_name[%.*s] failed, ret=%d", db_name.length(), db_name.ptr(), ret);
              }
              else if (OB_SUCCESS != (ret = grant_db_privs_by_user_id_db_name(user_id, db_name, privileges)))
              {
                YYSYS_LOG(WARN, "grant privs on [%.*s.*] to [%.*s] failed, ret=%d",
            ...
            else if (OB_INVALID_ID != table_id)
            {
              ...
              {
                YYSYS_LOG(WARN, "grant table privs to [%.*s] failed, table_id=%lu, ret=%d", user_name.length(), user_name.ptr(), table_id, ret);
              }
            }
            else
            {
...
int ObPrivExecutor::get_all_db_names(ObStrings &db_names)
{
  ...
  {
    context_->session_info_->get_current_result_set()->set_message(tmp_result.get_message());
    YYSYS_LOG(WARN, "direct_execute failed, sql=%.*s ret=%d", select_db_name.length(), select_db_name.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", select_db_name.length(), select_db_name.ptr(), ret);
  }
  else
  {
    OB_ASSERT(tmp_result.is_with_rows() == true);
        ...
        if (OB_SUCCESS == ret)
        {
          if (pcell->get_type() == ObVarcharType)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "push db_name[%.*s] to array failed, ret=%d", db_name.length(), db_name.ptr(), ret);
              }
            }
          }
          else
...
int ObPrivExecutor::grant_db_privs_by_user_id(int64_t user_id, const ObArray<ObPrivilegeType> *privileges)
{
  ...
  {
    YYSYS_LOG(ERROR, "get all db names from __all_database failed, ret=%d", ret);
  }
  for(int64_t db_index = 0; db_index < db_names.count() && OB_LIKELY(OB_SUCCESS == ret); db_index++)
  {
    ...
    {
      YYSYS_LOG(WARN, "get_string() failed, ret=%d", ret);
    }
    //for one db & all tables under the db
    else if (OB_SUCCESS != (ret = grant_db_privs_by_user_id_db_name(user_id, db_name, privileges)))
    {
      YYSYS_LOG(ERROR, "grant db&table privs to user_id=%ld failed, db_name=%.*s, ret=%d", user_id, db_name.length(), db_name.ptr(), ret);
    }
  }
  return ret;
}
...
int ObPrivExecutor::grant_db_privs_by_user_id_db_name(int64_t user_id, const ObString &db_name, const ObArray<ObPrivilegeType> *privileges)
{
  ...
  if (OB_SUCCESS != (ret = get_db_id_by_db_name(db_name, db_id)))
  {
    YYSYS_LOG(ERROR, "get db id from __all_database by db_name failed, db_name=%.*s, ret=%d", db_name.length(), db_name.ptr(), ret);
  }
  else if (-1 != db_id/*ignore sys db*/ && OB_SUCCESS != (ret = set_db_privilege(user_id, db_id, privileges)))
  {
    YYSYS_LOG(ERROR, "grant database privs to user_id=%ld failed, db_id=%ld, ret=%d", user_id, db_id, ret);
  }
  else if (OB_SUCCESS != (ret = get_all_table_ids_by_db_name(db_name, table_ids)))
  {
    YYSYS_LOG(ERROR, "get table ids from __first_tablet_entry by db_name failed, db_name=%.*s, ret=%d", db_name.length(), db_name.ptr(), ret);
  }
  for(int64_t table_index = 0; table_index < table_ids.count() && OB_LIKELY(OB_SUCCESS == ret); table_index++)
  {
    uint64_t tid = table_ids.at(table_index);
    ...
    {
      YYSYS_LOG(ERROR, "grant table privs to user_id=%ld, db_id=%ld, tid=%lu, ret=%d", user_id, db_id, tid, ret);
    }
  }
  return ret;
}
...
int ObPrivExecutor::do_drop_user(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast drop user stmt failed, ret=%d", ret);
  }
  else
  {
    // START TRANSACTION
    ...
    else
    {
      ...
      for (i = 0;i < user_count; ++i)
      {
        ...
        if (OB_SUCCESS != (ret = users->get_string(i, user_name)))
        {
          YYSYS_LOG(ERROR, "get user from drop user stmt failed, ret=%d", ret);
          break;
        }
        else if (user_name == OB_ADMIN_USER_NAME)
        {
          ...
          YYSYS_LOG(ERROR, "drop admin failed, can't drop admin");
          break;
        }
        else
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(INFO,"cannot find user");
              err = OB_ERROR;
            }
            ...
            if(OB_SUCCESS == err)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to get list of user[%ld]",user.user_id_);
              }
              else if(OB_SUCCESS != (err = clean_user_privilege(user.user_id_,db_id_list,table_id_list)))
              {
                YYSYS_LOG(WARN, "failed to clean user privilege,ret = %d", ret);
              }
              else
              {
                YYSYS_LOG(INFO, "clean user privileges");
              }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      for(int64_t i = 0; i < user_id_list.count();i++)
      {
        ...
        {
          YYSYS_LOG(WARN,"clean database privilege failed user_id[%ld],ret[%d]",user_id, ret);
          break;
        }
        else
        {
          YYSYS_LOG(INFO,"clean privilege for user[%ld]",user_id_list.at(i));
        }
      }
      if(OB_SUCCESS == ret)
      {
    ...
    for (int i = 0;i < rename_infos->count(); i = i + 2)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "insert_user=%.*s", insert_user.length(), insert_user.ptr());
...
int ObPrivExecutor::do_set_password(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast failed,err=%d", ret);
  }
  else
  {
    ...
    else
    {
      ...
      if (pwd.length() == 0)
      {
        ...
        YYSYS_LOG(WARN, "password must not be empty");
      }
      else
      {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      //{
        ...
        //YYSYS_LOG(WARN, "user %.*s not exists, ret=%d", user.length(), user.ptr(), ret);
      //}
    }
  }
  context_->session_info_->set_current_result_set(result_set_out_);
...
int ObPrivExecutor::do_lock_user(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast lock user stmt failed, ret=%d", ret);
  }
  else
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "get user from lock user stmt failed, ret=%d", ret);
      }
      else
      {
        // step 2
...
int ObPrivExecutor::do_create_user(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast create user stmt failed, ret=%d", ret);
  }
  else
  {
    ...
    {
      ObString select_user_id = ObString::make_string("select value from __all_sys_stat where cluster_id = 0 and name = 'yb_max_user_id'");
      ObResultSet result2;
      context_->session_info_->set_current_result_set(&result2);
      if (OB_SUCCESS != (ret = result2.init()))
      {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "next row from ObResultSet failed, sql=%.*s ret=%d", select_user_id.length(), select_user_id.ptr(), ret);
        }
        else
        {
          ObExprObj in;
          ...
          {
            YYSYS_LOG(WARN, "raw get cell(yb_max_user_id) from row failed, ret=%d", ret);
          }
        }
        err = result2.close();
        if (OB_SUCCESS != err)
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for (i = 0;i < count; i = i + 2)
      {
        ...
        if (OB_SUCCESS != (ret = users->get_string(i, user_name)))
        {
          YYSYS_LOG(WARN, "get user from create user stmt failed, ret=%d", ret);
          break;
        }
        else if (OB_SUCCESS != (ret = users->get_string(i + 1, pass_word)))
        {
          YYSYS_LOG(WARN, "get password from create user stmt failed, ret=%d", ret);
          break;
        }
        ...
        else
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              ...
              YYSYS_LOG(WARN, "create user failed, user:%.*s exists",user_name.length(), user_name.ptr());
              break;
            }
          }
          ...
          {
              int64_t user_id;
              ObString db_name = ObString::make_string("TANG");
              ObPrivilegeType priv = OB_PRIV_SELECT;
              ObArray<ObPrivilegeType> privileges;
              if(OB_SUCCESS != (ret = privileges.push_back(priv)))
              {
                  YYSYS_LOG(WARN,"failed to push to array privileges, err=%d",ret);
              }
              else if(OB_SUCCESS != (ret = get_user_id_by_user_name(user_name,user_id)))
              {
                  YYSYS_LOG(ERROR,"get user_id by user_name[%.*s] failed, ret =%d",user_name.length(),user_name.ptr(),ret);
              }
              else if(OB_SUCCESS != (ret = grant_db_privs_by_user_id_db_name(user_id,db_name,&privileges)))
              {
                  YYSYS_LOG(WARN,"grant privs on [%.*s.*] to [%.*s] failed, ret=%d",
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      databuff_printf(update_max_user_id_buff, 512, pos, "UPDATE __all_sys_stat set value = '%ld' \
          where cluster_id = 0 and name = 'yb_max_user_id'", current_user_id);
      if (pos >= 511)
      {
        // overflow
        ret = OB_BUF_NOT_ENOUGH;
...
int ObPrivExecutor::do_create_db(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast create db stmt failed!");
  }
  else
  {
    if(OB_SUCCESS != (ret = start_transaction()))
    ...
    {
      ObString select_db_id = ObString::make_string("select value from __all_sys_stat where cluster_id = 0 and name = 'yb_max_database_id'");
      ObResultSet result2;
      context_->session_info_->set_current_result_set(&result2);
      if (OB_SUCCESS != (ret = result2.init()))
      {
      ...
      else
      {
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "next row from ObResultSet failed, sql = %.*s ret = %d", select_db_id.length(), select_db_id.ptr(), ret);
        }
        else
        {
          ObExprObj in;
          ...
          {
            YYSYS_LOG(WARN, "raw get cell yb_max_database_id from row failed, ret = %d", ret);
          }
        }
      }
      err = result2.close();
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "get database from create db stmt failed, ret = %d", ret);
    }
    ...
    else
    {
      ...
      {
        ret = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(WARN, "privilege buff overflow, ret = %d", ret);
      }
      else
      {
        ...
        if(OB_SUCCESS != ret)
        {
          ...
          YYSYS_LOG(WARN, "create db failed,db:%.*s exists",db_name.length(), db_name.ptr());
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      databuff_printf(update_max_db_id_buff, 512, pos, "UPDATE __all_sys_stat set value = '%ld' \
                      where cluster_id = 0 and name = 'yb_max_database_id'", current_db_id);
      if(pos >= 511)
      {
        ret = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(WARN, "privilege buffer overflow, ret = %d", ret);
  ...
  if(OB_SUCCESS == ret && context_->session_info_->is_regrant_priv())
  {
    ...
    {
      YYSYS_LOG(WARN, "grant al db priv failed,ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObPrivExecutor::grant_all_db_priv(const int64_t &db_id)
{
  ...
  {
    YYSYS_LOG(WARN, "check user exist failed, ret [%d]", ret);
  }
  else
  {
    insert_db_priv.assign_buffer(insert_db_priv_buff, 512);
    databuff_printf(insert_db_priv_buff, 
	512, 
	pos,
	"REPLACE INTO __all_database_privilege(user_id,db_id,priv_all,priv_alter,priv_create_user,priv_create,priv_delete,"
    ...
    else
    {
      ...
      if(OB_SUCCESS != ret)
      {
        ...
        YYSYS_LOG(WARN, "grant owner database failed");
      }
    }
  }
  return ret;
...
int ObPrivExecutor::grant_all_db_priv_global_user(const ObString &db_name)
{
  ...
  for(; iter != user_pirv_map->end(); ++ iter)
  {
    ...
    if(!user.privileges_.is_empty())
    {
      ...
      if(pos >= 511)
      {
        YYSYS_LOG(WARN, "buff for grant sql is not enough");
        ret = OB_BUF_NOT_ENOUGH;
      }
      else
      {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "execute grant sql failed,sql = %.*s, ret[%d]", grant_sql.length(), grant_sql.ptr(),ret);
          break;
        }
      }
    }
...
int ObPrivExecutor::do_use_db(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast use db stmt failed!") ;
  }
  else
  {
    ...
    else
    {
      ...
      if(OB_SUCCESS != (ret = database->get_string(0,db_name)))
      {
        YYSYS_LOG(WARN, "get database name failed, ret = [%d]", ret);
      }
      else if(OB_SUCCESS != (ret = db_can_be_connect(user, db_name)))
      {
        YYSYS_LOG(WARN, "user [%.*s] has no priveleges to use db [%.*s] ret [%d]", user.length(), user.ptr(), db_name.length(), db_name.ptr(), ret);
      }
      else if(OB_SUCCESS != (ret = context_->session_info_->set_db_name(db_name)))
      {
        YYSYS_LOG(WARN, "change database failed, ret [%d]", ret);
      }
    }
  }
  if(OB_SUCCESS == ret)
...
int ObPrivExecutor::do_drop_db(const ObBasicStmt *stmt)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "dynamic cast drop db stmt failed!");
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "get db name from statemen failed, ret[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = (*(context_->pp_privilege_))->is_db_exist(db_name,db_id)))
    {
      YYSYS_LOG(WARN, "database cannot be droped, ret[%d]", ret);
    }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN,"clean database privilege failed,db_id[%ld],ret[%d]",db_id,ret);
    }
  }
  /*ObString null_db;
  if(OB_SUCCESS == ret)
...
int ObPrivExecutor::db_can_be_drop(const ObString &db_name)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "direct_execute failed, sql = %.*s ret = %d",sql_get_db.length(), sql_get_db.ptr(), ret);
      result_set_out_->set_message(result.get_message());
    }
    else if(OB_SUCCESS != (ret = result.open()))
    {
      YYSYS_LOG(WARN, "open result set failed, sql =%.*s ret = %d", sql_get_db.length(), sql_get_db.ptr(), ret );
    }
...
int ObPrivExecutor::db_can_be_connect(const ObString &user_name, const ObString &db_name)
{
  ...
  if(OB_ERR_NO_ACCESS_TO_DATABASE == ret)
  {
	...
	512, 
	pos, 
	"SELECT /*+read_consistency(WEAK) */dp.user_id,dp.db_id,dp.priv_all,dp.priv_alter,dp.priv_create,dp.priv_create_user,dp.priv_delete,dp.priv_drop,dp.priv_grant_option,dp.priv_insert,dp.priv_update,dp.priv_select,dp.priv_replace,dp.is_locked FROM __all_database_privilege dp WHERE dp.user_id=%ld AND dp.db_id = %ld ", 
	user.user_id_, 
	db_id);
    if(pos >= 511)
    {
      YYSYS_LOG(WARN, "privilege buff over flow");
      ret = OB_BUF_NOT_ENOUGH;
    }
    else
    {
      ...
      if(OB_SUCCESS != (ret = direct_execute_sql_db_priv_is_valid(sql_get_db_priv)))
      {
        ...
        pos = 0;
        databuff_printf(select_table_priv_buff, 
		512, 
		pos, 
		"SELECT /*+read_consistency(WEAK) */t.user_id,t.table_id,t.priv_all,t.priv_alter,t.priv_create,t.priv_create_user,t.priv_delete,t.priv_drop,t.priv_grant_option,t.priv_insert,t.priv_update,t.priv_select, priv_replace FROM __all_table_privilege t WHERE t.user_id = %ld AND t.db_id = %ld", 
		user.user_id_, 
		db_id);
        if(pos >= 512)
        {
          YYSYS_LOG(WARN, "privilege buff overflow");
          ret = OB_BUF_NOT_ENOUGH;
        }
        else
        {
          ...
          //YYSYS_LOG(ERROR, "test::whx exec ret[%d],sql[%.*s]", ret, sql_get_table_priv.length(),sql_get_table_priv.ptr());
        }
      }
    }
  }
...
int ObPrivExecutor::sql_get_db_id(const ObString &db_name, uint64_t &db_id) const
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "direct execute failed, sql = %.*s", sql_select_database.length(), sql_select_database.ptr());
      result_set_out_->set_message(result.get_message());
    }
    else if(OB_SUCCESS != (ret = result.open()))
    {
      YYSYS_LOG(WARN, "open result set failed, sql = %.*s, ret = %d", sql_select_database.length(), sql_select_database.ptr(), ret);
    }
    else if(result.is_with_rows())
    {
      const ObRow* row = NULL;
      ...
      {
        YYSYS_LOG(WARN, "database not exist,db_name = %.*s", db_name.length(), db_name.ptr());
        ret = OB_ERR_DATABASE_NOT_EXSIT;
      }
    }
    int err = OB_SUCCESS;
...
int ObPrivExecutor::direct_execute_sql_has_result(const ObString &sql)
{
  ...
  {
    YYSYS_LOG(WARN, "direct_execute failed, sql = %.*s", sql.length(),sql.ptr());
    result_set_out_->set_message(result.get_message());
  }
  else if(OB_SUCCESS != (ret = result.open()))
  {
    YYSYS_LOG(WARN, "open result set failed, sql =%.*s ret = %d", sql.length(), sql.ptr(), ret );
  }
  else if(result.is_with_rows())
  {
    //const ObRow* row = NULL;
  ...
  else if(result.is_with_rows())
  {
    const ObRow* row = NULL;
    {
      ...
      while(OB_SUCCESS == result.get_next_row(row))
      {
        ...
        {
          YYSYS_LOG(WARN, "db_result_to_priv_empty func exec failed, err[%d]", err);
          break;
        }
        else if(!is_empty)
        {
...
int ObPrivExecutor::direct_execute_sql_table_priv_is_valid(const ObString &sql)
{
  ...
  else if(result.is_with_rows())
  {
    ...
    {
      ObPrivilege priv;
      //YYSYS_LOG(ERROR, "test::whx row [%s]", to_cstring(*row));
  ...
  for( ObPrivilege::UserDbPriMap::const_iterator iter = db_pri_map->begin();iter != db_pri_map->end();++iter)
  {
    ...
    if(id_left == id_right)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "direct execute failed, sql = %.*s",sql_delete_priv.length(),sql_delete_priv.ptr());
          result_set_out_->set_message(result.get_message());
          break;
        }
        else if(OB_SUCCESS != (ret = result.open()))
        {
          YYSYS_LOG(WARN, "open result set failed, sql=%.*s ret=%d", sql_delete_priv.length(), sql_delete_priv  .ptr(), ret);
          break;
        }
        else
        {
          ...
          YYSYS_LOG(INFO," clean database privileges affect [%ld] rows", affected_rows);
        }
        int err = OB_SUCCESS;
        err = result.close();
        if(OB_SUCCESS != err)
...
int ObPrivExecutor::clean_user_privilege(const uint64_t &user_id, const ObArray<int64_t> &db_list, const ObArray<uint64_t> &table_list)
{
  ...
  if(OB_SUCCESS == ret)
  {
    for(int64_t i = 0; i < db_list.count();i++)
    {
      ...
      {
        sql_delete_priv.assign_ptr(sql_buf,static_cast<ObString::obstr_size_t>(pos));
        YYSYS_LOG(WARN, "test::whx, clean user table privilege, sql = %.*s",sql_delete_priv.length(),sql_delete_priv.ptr());
        if(OB_SUCCESS != (ret = result.init()))
        {
          YYSYS_LOG(WARN, "init result set failed,%d",ret);
        }
        ...
        {
          YYSYS_LOG(WARN, "open result set failed,sql = %.*s",sql_delete_priv.length(),sql_delete_priv.ptr());
          break;
        }
        else
        {
          int64_t affected_rows = result.get_affected_rows();
          YYSYS_LOG(INFO," clean user privileges affect [%ld] rows", affected_rows);
        }
        int err = OB_SUCCESS;
        err = result.close();
        if(OB_SUCCESS != err)
