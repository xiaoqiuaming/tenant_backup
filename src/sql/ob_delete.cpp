...
int ObDelete::open()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to delete by mutator. ret=%d", ret);
    }
  }
  return ret;
}
...
int ObDelete::delete_by_mutator()
{
        ...
        && OB_SUCCESS == (ret = child_op_->get_next_row(row)))
  {
    ...
    for (int64_t i = 0; i < rowkey_info_.get_size(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get data type from row desc, err=%d cid=%lu", ret, cid);
        break;
      }
      else
      {
    ...
    {
      YYSYS_LOG(WARN, "failed to delete cell, err=%d", ret);
      break;
    }
    else
    {
      YYSYS_LOG(DEBUG, "delete cell, rowkey=%s", to_cstring(rowkey));
    }
  }   // end while
  if (OB_ITER_END != ret)
  {
