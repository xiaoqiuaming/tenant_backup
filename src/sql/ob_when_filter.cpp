...
int ObWhenFilter::open()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "ObWhenFilter has no child operator, ret=%d", ret);
  }
  else
  {
    for (int32_t i = 1; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
    {
      ...
      {
        ret = OB_ERR_GEN_PLAN;
        YYSYS_LOG(WARN, "Can not get %dth child of ObWhenFilter ret=%d", i, ret);
        break;
      }
      else if ((ret = op->open()) != OB_SUCCESS)
      {
        if (!IS_SQL_ERR(ret))
        {
          YYSYS_LOG(WARN, "Open the %dth child of ObWhenFilter failed, ret=%d", i, ret);
        }
        break;
      }
      else if ((ret = op->get_next_row(child_row)) != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "Get nex row of %dth child failed ret=%d", i, ret);
        break;
      }
      switch (op->get_type())
      {
        case PHY_ROW_COUNT:
        {
          ...
          {
            YYSYS_LOG(WARN, "Wrong index of ObRowCount, idx=%d ret=%d", 0, ret);
          }
          else if ((ret = tmp_row_desc.add_column_desc(table_id, column_id)) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "Add column to ObRowDesc failed, ret=%d", ret);
          }
          else if ((ret = tmp_row.set_cell(table_id, column_id, *cell)) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "Add cell to ObRow failed, ret=%d", ret);
          }
          break;
        }
        default:
        {
          ...
          YYSYS_LOG(WARN, "Unknown child type of ObWhenFilter, index=%d, ret=%d", i, ret);
        }
      }
    }
    ...
    for (int32_t i = 1; ret == OB_SUCCESS && i < ObMultiChildrenPhyOperator::get_child_num(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "Close the %dth child of ObWhenFilter failed, ret=%d", i, ret);
      }
    }
    if (ret == OB_SUCCESS)
    {
      ...
      else
      {
        if (!IS_SQL_ERR(ret))
        {
          YYSYS_LOG(WARN, "Open the first child of ObWhenFilter failed, ret=%d", ret);
        }
      }
    }
  }
...
int ObWhenFilter::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "children_ops_[0] is NULL");
  }
  else
  {
...
int ObWhenFilter::get_next_row(const common::ObRow *&row)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "ObWhenFilter has no child, ret=%d", ret);
  }
...
DEFINE_SERIALIZE(ObWhenFilter)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to serialize child_num_. ret=%d", ret);
  }
  else if(OB_SUCCESS != (ret = encode_vi64(buf, buf_len, pos, when_number_)))
  {
    YYSYS_LOG(WARN, "fail to serialize when_number_. ret=%d", ret);
  }
...
DEFINE_DESERIALIZE(ObWhenFilter)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to deserialize child_num_. ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = decode_vi64(buf, data_len, pos, &when_number_)))
  {
    YYSYS_LOG(WARN, "fail to deserialize when_number_. ret=%d", ret);
  }
  ...
  else
  {
    ...
    //    {
    //      YYSYS_LOG(WARN, "fail to add expression to filter.ret=%d, buf=%p, data_len=%ld, pos=%ld",
