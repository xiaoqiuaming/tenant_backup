...
int ObCSCreatePlan::get_basic_column_and_join_info(
  ...
  ,const ObSqlReadParam * sql_read_param
  //add duyr 20151214:e
  )
{
  int ret = OB_SUCCESS;
  ...
  bool is_cid = false;
   ...
   //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151214:b
  const ObReadAtomicParam* read_atomic_param = NULL;
  if (NULL != sql_read_param
      && sql_read_param->get_read_atomic_param().is_valid())
  {
    read_atomic_param = &(sql_read_param->get_read_atomic_param());
  }
  //add duyr 20151214:e
  // add by maosy [MultiUps 1.0] [batch_udi] 20170420 b
  const ObDataMarkParam * data_mark_param =NULL;
  uint64_t datamark_cid = OB_INVALID_ID;
  if(NULL != sql_read_param
          && sql_read_param->get_data_mark_param().is_valid())
  {
      data_mark_param = &(sql_read_param->get_data_mark_param());
      datamark_cid = data_mark_param->modify_time_cid_ ;
  }
  ...
  for(int32_t i=0;OB_SUCCESS == ret && i<project.get_output_columns().count();i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "get expression from out columns fail:ret[%d] i[%d]", ret, i);
    }
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_INVALID_ID == expr.get_table_id()
        ...
        || renamed_table_id == expr.get_table_id()
              || column_id == datamark_cid)// add by maosy [MultiUps 1.0] [batch_udi] 20170427
      {
      ...
      else
      {
        ...
        YYSYS_LOG(WARN, "Unknown expression id, table_id[%lu], renamed_table_id[%lu], expr_table_id[%lu]", 
     ...
     if(OB_SUCCESS == ret)
     {
      ...
      if ( (OB_SUCCESS == (ret = expr.get_column_index_expr(tid, column_id, is_cid))) && is_cid && common::OB_ACTION_FLAG_COLUMN_ID != column_id)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "duplicate column [%lu]", column_id);
          continue; //如果发现重复列，就直接跳过
        }
        else if (!column_ids_.add_member((int32_t)column_id))
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "fail to add member[%lu]", column_id);
        }
        //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151125:b
        else if (NULL != read_atomic_param
                 && common::ObReadAtomicHelper::is_read_atomic_special_cid(*read_atomic_param,
                                                                         column_id))
        {
          if(OB_SUCCESS != (ret = basic_columns.push_back(column_id)))
          {
            YYSYS_LOG(WARN, "fail to push item:ret[%d],tid[%lu], column_id[%lu]",
        ...
        {
            YYSYS_LOG(INFO,"INFO::debug,data mark cid=%ld",column_id);
            if(OB_SUCCESS != (ret = basic_columns.push_back(column_id)))
            {
              YYSYS_LOG(WARN, "fail to push item:ret[%d],tid[%lu], column_id[%lu]",
                        ret,table_id, column_id);
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "get column schema fail:tid[%lu], table_id[%lu] column_id[%lu]", tid, table_id, column_id);
        }
        else
        {
          ...
          if(OB_SUCCESS != (ret = basic_columns.push_back(column_id)))
          {
            YYSYS_LOG(WARN, "fail to push item:ret[%d], column_id[%lu]", ret, column_id);
          }
        }
        ...
        if(OB_SUCCESS == ret)
        {
          ...
          if(NULL != join_info)
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN, "left table id cannot change:t1[%lu], t2[%lu]", table_join_info.left_table_id_, table_id);
            }
            ...
            if(OB_SUCCESS == ret)
            {
              if(table_join_info.join_condition_.count() == 0)
              {
                ...
                for(uint64_t left_col_idx=0;left_col_idx<join_info->left_column_count_;left_col_idx++)
                {
                  ...
                  {
                    YYSYS_LOG(WARN, "get left rowkey column id fail:ret[%d], col_idx[%ld]", ret, col_idx);
                  }
                  else if(OB_SUCCESS != (ret = table_join_info.join_condition_.push_back(left_rowkey_column_id)))
                  {
                    YYSYS_LOG(WARN, "push left rowkey column id fail:ret[%d], left_rowkey_column_id[%ld]", ret, left_rowkey_column_id);
                  }
                }
              }
              ...
              {
                ret = OB_ERROR;
                YYSYS_LOG(WARN, "join info count change");
              }
            }
            ...
            if(OB_SUCCESS == ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "add join info item fail:ret[%d]", ret);
              }
            }
          }
        }
    ...
    if(table_join_info.join_column_.count() > 0)
    {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        for(int32_t i=0;OB_SUCCESS == ret && i<table_join_info.join_condition_.count();i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "get join cond fail:ret[%d] i[%d]", ret, i);
          }
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to push item:ret[%d], column_id[%lu]", ret, left_column_id);
            }
          }
        }
      }
