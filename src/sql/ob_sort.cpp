...
int ObSort::exist_sort_column(const uint64_t tid, const uint64_t cid,bool& is_exist)
{
	...
	for(int32_t idx = 0; idx < sort_columns_.count(); idx++)
	{
		...
		{
			YYSYS_LOG(WARN, "failed to get array element, err=%d", ret);
			break;
                }
...
int ObSort::open()
{
  ...
  else if(is_left_table_empty_)
  {
      ret = OB_SUCCESS;
  }
  else if (OB_SUCCESS != (ret = do_sort()))
  {
    YYSYS_LOG(WARN, "failed to sort input data, err=%d", ret);
  }
  return ret;
}
...
int ObSort::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    YYSYS_LOG(ERROR, "child op is NULL");
    ret = OB_NOT_INIT;
  }
  else
  {
  ...
  else
  {
        ...
        && OB_SUCCESS == (ret = child_op_->get_next_row(input_row)))
    {
      ...
      if (1 == is_set_op_flag_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "row_columns_number is not equal result_type_array_size");
        ...
        else
        {
          ...
          if (dec_index_array.count() > 0)
              tmp_mem = new uint64_t[dec_index_array.count()][2];
          for(int64_t i = 0; i < dec_index_array.count(); i ++)
          {
                     ...
                     ObVarcharType == tmp_cell->get_type())
            {
              ...
              if (OB_SUCCESS != (ret = expr_from.cast_to(ObDecimalType, expr_to, *tmp_mem_buf)))
              {
                ...
                YYSYS_LOG(INFO,"column[%ld] value can not cast to ObDecimalType", dec_index_array.at(i) + 1);
                YYSYS_LOG(USER_ERROR,
              ...
              {
                expr_to.to(*tmp_cell);
                if (expr_to.get_type() == ObDecimalType)
                {
                    uint64_t *tt = tmp_mem[i];
                    memset(tt, 0, sizeof(uint64_t) * 2);
                    ObDecimal dec = expr_to.get_decimal();
                    memcpy(tt, dec.get_words()->ToUInt_v2(), 8 * expr_to.get_len());
                    tmp_cell->set_ttint(tt);
                    YYSYS_LOG(DEBUG, "xsl,cast_cells_to_res_type,except cast to decimal!");
                }
              }
            }
          }
      ...
      else if (need_dump())
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to sort, err=%d", ret);
        }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      const  int buf_length=1024*1024;
      char  sort_buf[buf_length];
      memset(sort_buf,0,buf_length);
      int64_t  pos=in_mem_sort_.to_string(sort_buf,buf_length);
      YYSYS_LOG(DEBUG, "SORT_ROWS{%.*s}, err=%d",(int32_t)pos,sort_buf ,ret);
      //add shili e
    }
  }
  return ret;
...
int ObSort::get_next_row(const common::ObRow *&row)
{
  ...
  {
    YYSYS_LOG(WARN, "execution timeout, ts=%ld", my_phy_plan_->get_timeout_timestamp());
...
void ObSort::set_is_left_table_empty(bool is_empty)
{
    is_left_table_empty_ = is_empty;
}
...
DEFINE_SERIALIZE(ObSort)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to encode sort columns count:ret[%d]", ret);
  }
  else
  {
    for (int64_t i=0;OB_SUCCESS == ret && i<sort_columns_.count();i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to serialize sort column:ret[%d]", ret);
      }
    }
  }
  if (OB_SUCCESS == ret)
...
DEFINE_DESERIALIZE(ObSort)
{
  ...
  {
    YYSYS_LOG(WARN, "decode sort_columns_count fail:ret[%d]", ret);
  }
  else
  {
    sort_columns_.clear();
    for (int64_t i=0;OB_SUCCESS == ret && i<sort_columns_count;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to add sort column to array:ret[%d]", ret);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to decode mem_size_limit_:ret[%d]", ret);
    }
  }
  //add qianzm [set_operation] 20151222 :b
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to decode is_set_op_flag_:ret[%d]", ret);
    }
  }
  //add e
  return ret;
