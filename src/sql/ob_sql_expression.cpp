...
DEFINE_SERIALIZE(ObSqlExpression)
{
  ...
  {
	YYSYS_LOG(WARN, "fail to serialize basic param. ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = post_expr_.serialize(buf, buf_len, pos)))
  {
	YYSYS_LOG(WARN, "fail to serialize postfix expression. ret=%d", ret);
  }
  else
  {
	//add tianz [SubQuery_for_Instmt] [JHOBv0.1] 20140504:b
	...
	{
	  YYSYS_LOG(WARN, "fail to serialize sub query num. ret=%d", ret);
	}
	else if(use_bloom_filter_)
	{
	  if(OB_SUCCESS != (ret = bloom_filter_.serialize(buf, buf_len, pos)))
	  {
		YYSYS_LOG(WARN, "fail to eserialize bloomfilter. ret=%d", ret);
	  }
	  YYSYS_LOG(DEBUG, "serialize bloonfilter success"); //mod peiouya 20151225
	}
	//add 20140504:e
	// success
	//YYSYS_LOG(INFO, "success serialize one ObSqlExpression. pos=%ld", pos);
  }
  return ret;
}
...
DEFINE_DESERIALIZE(ObSqlExpression)
{
  ...
  else
  {
	...
	{
	 YYSYS_LOG(WARN, "fail to deserialize sub query num. ret=%d", ret);
	}
	...
	else if(use_bloom_filter_)
	{
	 ...
	 {
	  YYSYS_LOG(WARN, "fail to serialize bloomfilter. ret=%d", ret);
	 }
	 else
	 {
...
int ObSqlExpression::serialize_basic_param(char* buf, const int64_t buf_len, int64_t& pos) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to serialize obj. ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
...
int ObSqlExpression::deserialize_basic_param(const char* buf, const int64_t data_len, int64_t& pos)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to get int value. ret=%d, column_id_=%lu", ret, column_id_);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != (ret = obj.get_int(val)))
    {
      YYSYS_LOG(WARN, "fail to get int value. ret=%d, table_id_=%lu", ret, table_id_);
    }
    else
    {
      table_id_ = val;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to get int value. ret=%d, is_aggr_func_=%d", ret, is_aggr_func_);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to get int value. ret=%d, is_distinct_=%d", ret, is_distinct_);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != (ret = obj.get_int(val)))
    {
      YYSYS_LOG(WARN, "fail to get int value. ret=%d, aggr_func_=%d", ret, aggr_func_);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to get varchar value. ret=%d, listagg_delimeter_=%.*s", ret, listagg_delimeter_.length(),listagg_delimeter_.ptr());
    }
  }
  /* add 20150104:e */
  return ret;
...
DEFINE_GET_SERIALIZE_SIZE(ObSqlExpression)
{
  ...
  if(use_bloom_filter_)
  {
    ...
    YYSYS_LOG(WARN, "add bloonfilter size in get serialize size %ld",bloom_filter_.get_serialize_size());
  }
    //add 20140504:e
  return size;
}
...
int ObSqlExpressionUtil::make_column_expr(const uint64_t tid, const uint64_t cid, ObSqlExpression &expr)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to add expr item. ret=%d", ret);
  }
...
ObSqlExpression* ObSqlExpression::alloc()
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to allocate expression object");
  }
  else
  {
  ...
  {
    YYSYS_LOG(INFO, "[EXPR] alloc %p, times=%ld cached=%d alloc_num=%d",
...
void ObSqlExpression::free(ObSqlExpression* ptr)
{
  ...
  {
    YYSYS_LOG(INFO, "[EXPR] free %p, times=%ld cached=%d alloc_num=%d",
