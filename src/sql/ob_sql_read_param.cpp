...
namespace oceanbase
{
  namespace sql
  {
    ObSqlReadParam::ObSqlReadParam() :
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151201:b
      ,read_atomic_param_()
      //add duyr 20151201:e
    ...
    int ObSqlReadParam::add_group_column(const uint64_t tid, const uint64_t cid)
    {
      ...
      {
        ret = OB_ERR_GEN_PLAN;
        YYSYS_LOG(WARN, "Can not adding group column after adding aggregate function(s). ret=%d", ret);
      }
      else
      {
        if (NULL == group_)
        ...
        if (NULL == group_)
        {
          ...
          YYSYS_LOG(WARN, "failed to alloc memory");
        }
        ...
        {
          YYSYS_LOG(WARN, "Add sort column of ObSqlReadParam sort operator failed. ret=%d", ret);
        }
        else if ((ret = group_->add_group_column(tid, cid)) != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "Add group column of ObSqlReadParam group operator failed. ret=%d", ret);
        }
        else if (!has_group_)
        {
          has_group_columns_sort_ = true;
    ...
    int ObSqlReadParam::add_sort_column_for_listagg(const uint64_t tid, const uint64_t cid,bool is_asc)
    {
        ...
        {
           YYSYS_LOG(ERROR, "Add sort column of ObSqlReadParam sort operator failed. ret=%d", ret);
        }
        if(OB_SUCCESS == ret)
        {
            is_listagg_ = true;
    ...
    /* add 20150104:e */
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151201:b
    void ObSqlReadParam::set_read_atomic_param(const ObReadAtomicParam &read_atomic_param)
    {
      read_atomic_param_ = read_atomic_param;
      if (read_atomic_param.is_valid())
        YYSYS_LOG(DEBUG,"read_atomic::debug,sql param read atomic param=[%s]",
                  to_cstring(read_atomic_param_));
    }

    ObReadAtomicParam& ObSqlReadParam::get_read_atomic_param()
    {
      return read_atomic_param_;
    }

    const ObReadAtomicParam& ObSqlReadParam::get_read_atomic_param() const
    {
      return read_atomic_param_;
    }

    int ObSqlReadParam::reset_filter()
    {
      int ret = OB_SUCCESS;
      filter_.reuse();
      return ret;
    ...
    int ObSqlReadParam::add_aggr_column(const ObSqlExpression& expr)
    {
      ...
      if (has_group_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Add aggregate function of ObSqlReadParam group operator failed. ret=%d", ret);
        }
      }
      else
      {
        ...
        else if ((ret = scalar_agg_->add_aggr_column(expr)) != OB_SUCCESS)
        {
          ...
          YYSYS_LOG(WARN, "Add aggregate function of ObSqlReadParam scalar aggregate operator failed. ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObSqlReadParam::serialize_basic_param(char * buf, const int64_t buf_len, int64_t & pos) const
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize basic param. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      }
      // result cached
      if (ret == OB_SUCCESS)
      ...
      if (OB_SUCCESS == ret)
      {
        if (OB_INVALID_ID != table_id_)
        {
          ...
          if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "fail to serialize table id = %lu", table_id_);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "Invalid table_id_. table_id_=%ld", table_id_);
      ...
      if (OB_SUCCESS == ret)
      {
        if (OB_INVALID_ID != renamed_table_id_)
        {
          ...
          if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "fail to serialize renamed table id = %lu", renamed_table_id_);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "Invalid renamed_table_id_. renamed_table_id_=%ld", renamed_table_id_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "fail to serialize bool:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObSqlReadParam::serialize_flag(char* buf, const int64_t buf_len, int64_t& pos,
    ...
    int ObSqlReadParam::serialize_stmt_type(char* buf, const int64_t buf_len,int64_t& pos,ObBasicStmt::StmtType stmt_type) const
    {
      int ret = OB_SUCCESS;
      ...
      {
        //serialize stmt_type
        if (OB_SUCCESS != (ret = serialize_flag(buf, buf_len, pos, ObActionFlag::SQL_STMT_TYPE)))
        {
          YYSYS_LOG(WARN, "fail to serialize flag. ret=%d", ret);
        }
        else
        {
          ret = serialize_int(buf,buf_len,pos,(int64_t)stmt_type);
          YYSYS_LOG(DEBUG, "serialize stmt type =%ld",(int64_t)stmt_type);
          YYSYS_LOG(DEBUG, "buf_len:%ld   pos=%ld", buf_len,pos);
          if(OB_SUCCESS!=ret)
          {
            YYSYS_LOG(WARN, "serialize stmt type  error!!!, ret=%d",ret);
          }
        }
      }
      return ret;
    }
    int ObSqlReadParam::deserialize_stmt_type(const char* buf, const int64_t data_len,int64_t& pos,ObBasicStmt::StmtType &stmt_type)
    {
      int ret=OB_SUCCESS;
      int64_t stmt_type_int= 0;
      ...
      {
        if (OB_SUCCESS != (ret = deserialize_int(buf, data_len, pos, stmt_type_int)))
        {
          YYSYS_LOG(WARN, "fail to deserialize_int(rowkey size). buf=%p, data_len=%ld, pos=%ld, ret=%d",
                    buf, data_len, pos, ret);
        }
        else
        {
          stmt_type =(ObBasicStmt::StmtType)stmt_type_int;
          YYSYS_LOG(DEBUG, " stmt_type=%d, ret=%d", stmt_type, ret);
        }
      }
      return ret;
    }
    int64_t ObSqlReadParam::get_obj_serialize_size(const int64_t value, bool is_ext) const
    {
      ObObj obj;
      if (is_ext)
      {
    ...
    int ObSqlReadParam::serialize_int(char* buf, const int64_t buf_len, int64_t& pos,
    ...
    int ObSqlReadParam::deserialize_int(const char* buf, const int64_t data_len, int64_t& pos,
        int64_t& value) const
    {
      int ret         = OB_SUCCESS;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "expected deserialize int type, but get type=%d", type);
          ret = OB_ERROR;
        }
      }
      return ret;
    ...
    int ObSqlReadParam::deserialize_basic_param(const char * buf, const int64_t data_len, int64_t & pos)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to get int. obj type=%d", obj.get_type());
          }
        }
        else
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get bool:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    */
    ...
    DEFINE_SERIALIZE(ObSqlReadParam)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "fail to serialize obj. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      ...
      if (OB_SUCCESS == ret && has_project_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize project param. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret && has_scalar_agg_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize scalar aggregation param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
              buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret && has_group_columns_sort_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize group column sort param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
              buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret && is_listagg_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize listagg sort param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
              buf, buf_len, pos, ret);
        }
      }
      
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151125:b
      //READ_ATOMIC_PARAM_FIELD
      if (OB_SUCCESS == ret && read_atomic_param_.is_valid() && read_atomic_param_.is_inited())
      {
        obj.set_ext(ObActionFlag::READ_ATOMIC_PARAM_FIELD);
        if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "fail to serialize obj. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
        else if (OB_SUCCESS != (ret = read_atomic_param_.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "fail to serialize read atomic param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
              buf, buf_len, pos, ret);
        }
      }
      //add duyr 20151125:e
      ...
      if (OB_SUCCESS == ret && data_mark_param_.is_valid())
      {
          ...
          if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "fail to serialize data mark param field obj. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
          }
          else if (OB_SUCCESS != (ret = data_mark_param_.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "fail to serialize data_mark param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
                buf, buf_len, pos, ret);
          }
      }
      //add duyr 20160531:e
...
if(OB_SUCCESS == ret && trans_id_.count() > 0)
      {
          YYSYS_LOG(DEBUG, "trans_id before serialize %s",to_cstring(trans_id_));
          int64_t count = trans_id_.count();
          obj.set_ext(ObActionFlag::TRANS_ID_FIELD);
          if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "fail to serialize data mark param field obj. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
          }
          else if(OB_SUCCESS !=(ret = serialize_int(buf,buf_len,pos,count)))
          {
              YYSYS_LOG(WARN, "fail to deserialize trans id count. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                        buf, buf_len, pos, ret);
          }
          for(int64_t i =0 ; i < count && OB_SUCCESS==ret; i++)
          {
              if(OB_SUCCESS != (ret = trans_id_.at(i).serialize(buf,buf_len,pos)))
              {
                  YYSYS_LOG(WARN, "fail to serialize trans id . buf=%p, data_len=%ld, pos=%ld, ret=%d",
                            buf, buf_len, pos, ret);
              }
          }
      }
      ...
      if (OB_SUCCESS == ret && has_group_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize group param. buf=%p, buf_len=%ld, pos=%ld, ret=%d",
              buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret && has_limit_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize limit param. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret && has_filter_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize filter param. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize end param. buf=%p, buf_len=%ld, pos=%ld, ret=%d", buf, buf_len, pos, ret);
        }
      }
      return ret;
    }
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret && ObActionFlag::END_PARAM_FIELD != obj.get_ext())
        {
            ...
            case ObActionFlag::BASIC_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to dersialize basic param. ret=%d", ret);
                }
                break;
              }
            case ObActionFlag::SQL_PROJECT_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize project. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
            ...
            case ObActionFlag::SQL_SCALAR_AGG_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize scalar aggregation. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
            ...
            case ObActionFlag::SQL_GROUP_SORT_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize group column sort. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
          ...
          case ObActionFlag::SORT_FOR_LISTAGG_FIELD:
            ...
            /* add 20150104:e*/
            //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151125:b
            case ObActionFlag::READ_ATOMIC_PARAM_FIELD:
              {
                if (OB_SUCCESS != (ret = read_atomic_param_.deserialize(buf, data_len, pos)))
                {
                  YYSYS_LOG(WARN, "fail to deserialize read atomic param. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                break;
              }
          ...
          //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
            case ObActionFlag::DATA_MARK_PARAM_FIELD:
            {
              ...
              {
                YYSYS_LOG(WARN, "fail to deserialize data mark param. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                    buf, data_len, pos, ret);
              }
              break;
            }
          //add duyr 20160531:e
          //add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
          case ObActionFlag::TRANS_ID_FIELD:
          {

              int64_t count = 0;
              if(OB_SUCCESS !=(ret = deserialize_int(buf,data_len,pos,count)))
              {
                  YYSYS_LOG(WARN, "fail to deserialize trans id count. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                            buf, data_len, pos, ret);
              }
              for(int64_t i =0 ; i < count && OB_SUCCESS==ret; i++)
              {
                  ...
                  if(OB_SUCCESS != (ret = par_transid.deserialize(buf,data_len,pos)))
                  {
                      YYSYS_LOG(WARN, "fail to deserialize trans id . buf=%p, data_len=%ld, pos=%ld, ret=%d",
                                buf, data_len, pos, ret);
                  }
                  else if(OB_SUCCESS !=(ret = trans_id_.push_back(par_transid)))
                  {
                      YYSYS_LOG(WARN," failed to pusb back par ,ret = %d",ret);
                  }
              }
              YYSYS_LOG(DEBUG, "trans_id after serialize %s",to_cstring(trans_id_));
              break;
          }
              // add by maosy e
            case ObActionFlag::SQL_GROUP_BY_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize group by. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
            ...
            case ObActionFlag::SQL_LIMIT_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize limit. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
            ...
            case ObActionFlag::SQL_FILTER_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize filter. buf=%p, data_len=%ld, pos=%ld, ret=%d",
                      buf, data_len, pos, ret);
                }
                else
                {
    ...
    DEFINE_GET_SERIALIZE_SIZE(ObSqlReadParam)
    {
      ...
      /* add 20150104:e */
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151201:b
      if (read_atomic_param_.is_valid()&& read_atomic_param_.is_inited())
      {
        obj.set_ext(ObActionFlag::READ_ATOMIC_PARAM_FIELD);
        total_size += obj.get_serialize_size();
      ...
      if(trans_id_.count() >0)
      {
          ...
          obj.set_ext(ObActionFlag::TRANS_ID_FIELD);
          total_size += obj.get_serialize_size();
          total_size += get_obj_serialize_size(count);
...
//      total_size += trans_id_.get_serialize_size();
    ...
    ObSqlReadParam& ObSqlReadParam::operator=(const ObSqlReadParam &other)
    {
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151130:b
      read_atomic_param_ = other.read_atomic_param_;
      //add duyr 20151130:e
      //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
      data_mark_param_ = other.data_mark_param_;
      //add duyr 20160531:e
      trans_id_ = other.trans_id_;//add by maosy [MultiUPS 1.0] [read uncommit]20170525 
    ...
    int64_t ObSqlReadParam::to_string(char *buf, const int64_t buf_len) const
    {
      ...
      {
        databuff_print_obj(buf,buf_len,pos,read_atomic_param_);
        pos = pos - 1;
      }
      //add duyr 20151130:e
      //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
    ...
    int ObSqlReadParam::assign(const ObSqlReadParam* other)
    {
      ...
      if (ret == OB_SUCCESS && o_ptr->has_project_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign ObProject failed, ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS && o_ptr->has_scalar_agg_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign ObScalarAggregate failed, ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS && o_ptr->has_group_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign ObMergeGroupBy failed, ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS && o_ptr->has_group_columns_sort_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign ObSort failed, ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS && o_ptr->has_group_columns_sort_)
      {
      ...
      if (OB_SUCCESS == ret)
      {
        read_atomic_param_ = o_ptr->read_atomic_param_;
      }
      //add duyr 20151130:e
      //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
      if (OB_SUCCESS == ret)
      ...
      if (ret == OB_SUCCESS && o_ptr->has_limit_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign ObLimit failed, ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS && o_ptr->has_filter_)
      {
        ...
        {
          YYSYS_LOG(WARN, "Assign filter_ failed, ret=%d", ret);
        }
      }
      return ret;
    }
