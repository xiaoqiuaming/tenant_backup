...
ObVariableSet::ObVariableSet()
  : variable_nodes_(), rpc_(NULL), table_id_(OB_INVALID_ID), rowkey_info_(), name_cid_(OB_INVALID_ID), value_cid_(OB_INVALID_ID), value_type_(ObMinType), mutator_()
{
}
...
int ObVariableSet::open()
{
  ...
  {
    ret = OB_ERR_GEN_PLAN;
    YYSYS_LOG(WARN, "Variable set statement is not initiated");
  }
  else
  {
    mutator_.use_db_sem();
...
int ObVariableSet::process_variables_set()
{
  ...
  for (int64_t i = 0; ret == OB_SUCCESS && i < variable_nodes_.count(); i++)
  {
    ...
    else if (!node.is_system_variable_)
    {
      ...
      {
        YYSYS_LOG(WARN, "set variable to session plan failed. ret=%d, name=%.*s",
    ...
    else if (!node.is_global_)
    {
      ...
      else if (node.variable_name_ == ObString::make_string("autocommit"))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to cast to int, obj=%s", to_cstring(*value_obj));
        }
        else
        {
    ...
    else
    {
      ...
      for (int64_t j = 0; ret == OB_SUCCESS && j < rowkey_info_.get_size(); ++j)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "Get rowkey error");
          break;
        }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "Failed to update cell, err=%d", ret);
          break;
        }
        else
        {
...
int ObVariableSet::set_autocommit()
{
  ...
  {
    // do nothing
    YYSYS_LOG(DEBUG, "already in autocommit=1");
  }
  else
  {
    ...
    if (session->get_trans_id().is_valid())
    {
      ...
      ObEndTransReq req;
      //mod peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150702:b
      ...
      else
      {
        ...
        {
          int64_t trans_time = yysys::CTimeUtil::getTime() - session->get_curr_trans_start_time();
          OB_STAT_INC(OBMYSQL, SQL_MULTI_STMT_TRANS_TIME, trans_time);
          session->set_curr_trans_start_time(0);
        }
        YYSYS_LOG(WARN, "autocommit unfinished transaction when set autocommit=1, trans_id=%s",
...
int ObVariableSet::clear_autocommit()
{
  ...
  {
    // do nothing
    YYSYS_LOG(DEBUG, "in autocommit=0");
  }
  else
  {
