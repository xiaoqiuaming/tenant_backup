...
int ObSequenceInsert::copy_sequence_info_from_insert_stmt(const ObSEArray<int64_t, 64>& row_desc_map)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "the insert stmt is not init! ret::[%d]",ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < row_size; i++)
    {
      for (int64_t j = 0; OB_SUCCESS == ret && j < column_size; j++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "copy value row col failed!");
          break;
        }
      }//end for
      if (OB_SUCCESS != (ret = sequence_columns_mark_.push_back(value_row)))
      {
        YYSYS_LOG(ERROR, "copy value row failed!");
        break;
      }
      value_row.clear();
    }//end for
    ...
    for (int64_t i = 0 ;  OB_SUCCESS == ret && i < insert_stmt->get_next_prev_ids_size(); i++)
    {
      ...
      for (int64_t j = 0; OB_SUCCESS == ret && j < ids.count(); j++)
      {
        ...
        {
          YYSYS_LOG(ERROR,"copy next prev values error!");
        }
      }
...
//        YYSYS_LOG(ERROR, "====the val expr is::[%s]======",to_cstring(val_expr));
        ...
        {
          YYSYS_LOG(ERROR, "get is const expr failed!");
        }
        if (is_const)//do replace
        {
          if (OB_SUCCESS == ret)
          {
//            YYSYS_LOG(ERROR,"currnet expr [%s] is const expr, do simple fill!",to_cstring(val_expr));
            if (OB_SUCCESS != (ret = this->do_simple_fill(post_expr_array, int64_t(1), OB_SEQUENCE_INSERT_LIMIT, true)))
            {
              YYSYS_LOG(WARN, "fill the sequence value to expr error!");
            }
          }
        }
        ...
        else
        {
          if (OB_SUCCESS == ret)
          {
//            YYSYS_LOG(ERROR,"currnet expr [%s] is complex expr, do complex fill!",to_cstring(val_expr));
...
//        YYSYS_LOG(ERROR, "====the val expr after filling is::[%s]======",to_cstring(val_expr));
      //check if the current the column with sequence value is valid ,we do obj_cast() for check
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN,"current column with sequence do obj cast failed, ret=%d",ret);
          }
        }
      }//end if
    }//end for
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < single_row_sequence_names_.count(); i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "update sequence info for quick or client transaction failed!");
        }
      }
    }
    if (OB_SUCCESS == ret)
  ...
  {
      //mod by wangyao [sequence for optimize]:20171126
      // if (is_quick_path_sequence(get_sequence_name_no_dup(i)) || has_client_trans_)
      if (is_quick_path_sequence(get_sequence_name_no_dup(i)))
      //mod e
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "get the sequence values failed!");
      }
...
int ObSequenceInsert::fill_sequence_info_to_input_values(ObExprValues *input_values, ObValues *sequence_values)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "input_values and sequence_values are not inited!");
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fill input values with the sequence info failed!");
    }
  }//end else
  return ret;
}
...
//  ObPhyOperator *op = NULL;
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "the sql_context and table_id are not inited!");
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "get the table scan op failed!");
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    if (NULL == (table_schema = sql_context_->schema_manager_->get_table_schema(table_id_)))
    {
      ...
      YYSYS_LOG(ERROR, "fail to get table schema for table[%ld]", table_id_);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get input table desc failed!");
    }
    ...
    if (OB_LIKELY(OB_SUCCESS == ret))
    {
      ...
      if (OB_SUCCESS != (ret = rows_filter->add_expr_item(expr_item)))
      {
        YYSYS_LOG(ERROR,"Failed to add expr item, err=%d", ret);
      }
    }
    if (OB_LIKELY(OB_SUCCESS == ret))
    {
    ...
    if (OB_LIKELY(OB_SUCCESS == ret))
    {
      ...
      for (int64_t i = 0; ret == OB_SUCCESS && i < row_num; i++) // for each row
      {
        for (int64_t j = 0; ret == OB_SUCCESS && j < col_num; j++)//for each column
        {
          ...
          {
            YYSYS_LOG(ERROR,"Failed to get tid cid, err=%d", ret);
          }
          // success
          else if (rowkey_info.is_rowkey_column(column_id))
          {
            ...
            {
//              YYSYS_LOG(ERROR, "the duplication obj is ::[%s]",to_cstring(val_expr.get_expr_array()[k]));
              if (OB_SUCCESS != (ret = rows_filter->add_expr_obj(val_expr.get_expr_array()[k])))
              {
                YYSYS_LOG(ERROR, "add input key obj to rows filter failed!");
                break;
              }
            }//end for
          }
...
//        YYSYS_LOG(ERROR, "!!!the row num is::%ld",row_num);
        ExprItem expr_in;
        ...
        {
          YYSYS_LOG(ERROR,"Failed to add expr item end, err=%d", ret);
        }
        else if (OB_SUCCESS != (ret = table_scan->add_filter(rows_filter)))
        {
          YYSYS_LOG(ERROR,"Failed to add filter, err=%d", ret);
        }
      }
      YYSYS_LOG(DEBUG, "the duplicating expr is ::[%s]",to_cstring(*rows_filter));//add lijianqiang [sequecne insert]
    }
...
int ObSequenceInsert::open()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "sub operator(s) is/are NULL");
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed get static data of sequence!");
    }
