      ...
      else
      {
        ...
        if (OB_SUCCESS != (ret = child_op_->get_next_row(row)))
        {
          if (OB_ITER_END != ret)
          {
            if (!IS_SQL_ERR(ret))
            {
              YYSYS_LOG(ERROR, "child_op get_next_row fail but not OB_ITER_END, ret=%d", ret);
            }
          }
          else
          {
            YYSYS_LOG(DEBUG, "iterate nothing from child_op=%p type=%d will insert rows", child_op_, child_op_->get_type());
            could_insert_ = true;
            ret = OB_SUCCESS;
          }
        }
        ...
        {
          YYSYS_LOG(DEBUG, "iterate row from child_op=%p type=%d %s will not insert anything",
                    child_op_, child_op_->get_type(), (NULL == row) ? "nil" : to_cstring(*row));
          uint64_t t_id = OB_INVALID_ID;
          uint64_t c_id = OB_INVALID_ID;
          row->get_row_desc()->get_tid_cid(0, t_id, c_id);
          if(t_id != OB_USERS_TID)
          {
              YYSYS_LOG(USER_ERROR, "Duplicate entry \'%s\' for key \'PRIMARY\'", (NULL == row) ? "nil" : to_cstring(*row));
          }
          else
          {
              YYSYS_LOG(USER_ERROR, "Create user failed, user exists!");
          }
