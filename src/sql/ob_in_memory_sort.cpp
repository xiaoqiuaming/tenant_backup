...
int ObInMemorySort::add_row(const common::ObRow &row)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to add row into row_store, err=%d", ret);
  }
  ...
  else if (NULL == row_desc_)
  {
    ...
    YYSYS_LOG(DEBUG, "set row desc=%p col_num=%ld", row_desc_, row_desc_->get_column_num());
  }
  return ret;
}
...
int ObInMemorySort::sort_rows()
{
  ...
  {
    YYSYS_LOG(DEBUG, "sort rows, count=%ld", sort_array_.count());
...
int ObInMemorySort::get_next_row(common::ObRow &row)
{
  ...
  {
    ret = OB_ITER_END;
    YYSYS_LOG(DEBUG, "end of the in-memory run");
  }
  else
  {
    OB_ASSERT(row_desc_);
    ...
    {
      YYSYS_LOG(WARN, "failed to convert row, err=%d", ret);
    }
    else
    {
      ++sort_array_get_pos_;
...
int ObInMemorySort::get_next_compact_row(ObString &compact_row)
{
  ...
  {
    ret = OB_ITER_END;
    YYSYS_LOG(INFO, "end of the in-memory run");
  }
  else
  {
...
DEFINE_SERIALIZE(ObSortColumn)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to encode column_id_:ret[%d]", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_ascending_)))
  {
    YYSYS_LOG(WARN, "fail to encode is_ascending_:ret[%d]", ret);
  }
  return ret;
}
