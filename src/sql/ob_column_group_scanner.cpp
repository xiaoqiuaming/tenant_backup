...
namespace oceanbase
{
  namespace sql
  {
        ...
        const uint64_t column_id, const int64_t offset)
    {
      ...
      {
        YYSYS_LOG(WARN, "add_column_desc ret=%d, table_id=%ld, column_id=%ld, offset=%ld",
            ret, table_id, column_id, offset);
      }
      else if (OB_SUCCESS != (ret = current_scan_column_indexes_.add_column(offset)))
      {
        YYSYS_LOG(WARN, "add_column_offset ret=%d, table_id=%ld, column_id=%ld, offset=%ld",
            ret, table_id, column_id, offset);
      }
      return ret;
    }
        ...
        const uint64_t table_id, const Group& group, const ObSSTableSchema& schema)
    {
      ...
      if (group.seq_ == 0)
      {
        ...
        if (schema.is_binary_rowkey_format(table_id))
        {
          ...
          {
            YYSYS_LOG(ERROR, "binary rowkey sstable NOT set rowkey info, table=%ld", table_id);
            iret = OB_ERROR;
          }
          else
          {
        ...
        {
          iret = OB_ERROR;
          YYSYS_LOG(INFO, "rowkey column group not exist:table:%ld", table_id);
        }
        else
        {
      ...
      if (OB_SUCCESS != iret)
      {
        YYSYS_LOG(ERROR, "add rowkey columns error, iret=%d", iret);
      }
      else if (NULL == (def_array = schema.get_group_schema(table_id, group.id_, column_size)))
      {
        iret = OB_ERROR;
        YYSYS_LOG(ERROR, "find column group def array error.");
      }
      else
      {
        ...
        const int64_t column_id_size, const Group& group, const ObSSTableSchema& schema)
    {
      ...
      {
        YYSYS_LOG(WARN, "get_rowkey_column_count ret=%d, table_id=%ld", iret, table_id);
      }
      ...
      for (int32_t i = 0; i < column_id_size && OB_SUCCESS == iret; ++i)
      {
        ...
        {
          YYSYS_LOG(ERROR, "input column id =%ld (i=%d) is invalid.", 
              current_column_id, i);
        ...
        const ObSSTableReader* const sstable_reader)
    {
      ...
      if (NULL != sstable_reader)
      {
        ...
        {
          YYSYS_LOG(ERROR, "match_rowkey_desc table=%ld, ret=%d", table_id, ret);
        }
        ...
        const sstable::ObSSTableReader *sstable_reader)
    {
      ...
      if (NULL == schema)
      {
        YYSYS_LOG(ERROR, "schema of table =%ld not exist.", table_id);
      ...
      {
        YYSYS_LOG(ERROR, "cannot get rowkey column of table=%ld", table_id);
      }
        ...
        const ObSSTableReader *sstable_reader)
    {
      ...
      {
        iret = OB_ERROR;
        YYSYS_LOG(ERROR, "internal error, schema=%p is null "
      ...
      if (OB_SUCCESS != iret)
      {
        YYSYS_LOG(ERROR, "trans input param error, iret=%d,"
            "sstable id=%ld, table id=%ld, group id =%ld, group seq=%ld",
            iret, sstable_file_id, table_id, group.id_, group.seq_);
      }
      else
      {
        YYSYS_LOG(DEBUG, "trans input param succeed,sstable id=%ld, "
    ...
    int ObColumnGroupScanner::search_block_index(const bool first_time)
    {
      ...
      if (!first_time)
      {
        ...
        {
          YYSYS_LOG(ERROR, "cursor=%ld, block count=%ld error, cannot search next batch blocks.", 
      ...
      {
        YYSYS_LOG(DEBUG, "search block index out of range.");
      ...
      else if (OB_SUCCESS != iret)
      {
        ...
        YYSYS_LOG(ERROR, "search block index error, iret=%d, info=%ld,%ld,%ld,"
        ...
        const common::ObRowkeyInfo* rowkey_info)
    {
                ...
                uncompressed_data_buffer_, uncompressed_data_bufsiz_))) )
      {
        ...
        YYSYS_LOG(ERROR, "thread local page arena hold memory usage,"
      ...
      {
        YYSYS_LOG(ERROR, "allocate block internal data buffer failed, sz=%ld",
        ...
        const ObSSTableReader* const sstable_reader)
    {
      ...
      {
        iret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "set invalid scan param =%p or reader=%p",
      ...
      {
        YYSYS_LOG(ERROR, "trans_input_column_id error, iret=%d", iret);
      }
      else if (OB_SUCCESS != (iret = consummate_row_desc(scan_param->get_table_id(), sstable_reader)))
      {
        YYSYS_LOG(ERROR, "consummate_row_desc error, iret=%d", iret);
      }
      else
      {
        // save parameters
        ...
        {
          YYSYS_LOG(ERROR, "search in block index error, iret:%d.", iret);
        }
        else if ( OB_SUCCESS != (iret = fetch_next_block()) )
        {
          YYSYS_LOG(ERROR, "error in first fetch_next_block, iret:%d", iret);
        }
        ...
        const char* &block_data_ptr, int64_t &block_data_size)
    {
      ...
      if (OB_SUCCESS == iret)
      {
        ...
        {
          iret = OB_IO_ERROR;
          YYSYS_LOG(ERROR, "read sstable block data in cache failed, iret=%d", iret);
        }
      }
      ...
      if (OB_SUCCESS == iret)
      {
        ...
        if (OB_SUCCESS != iret)
        {
          YYSYS_LOG(ERROR, "get record header error, iret=%d, block data size=%ld", 
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "block data length=%d > fixed length=%ld, "
      ...
      if (OB_SUCCESS == iret)
      {
        ...
        if (header.is_compress())
        {
          ...
          if (NULL != dec)
          {
            ...
            if (iret != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "decompress failed, iret=%d, real_size=%ld", iret, real_size);
            }
            else
            {
      ...
      {
        YYSYS_LOG(ERROR, "read_current_block_data error, input param:"
    ...
    int ObColumnGroupScanner::load_current_block_and_advance() 
    {
      ...
      if (OB_SUCCESS == iret && is_forward_status())
      {
        ...
        {
          YYSYS_LOG(DEBUG, "current batch block scan over, "
      ...
      if (OB_SUCCESS == iret && is_forward_status())
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS == iret && NULL != block_data_ptr && block_data_size > 0)
          {
            ...
            {
              YYSYS_LOG(DEBUG, "current cursor = %ld, out of range, need_looking_forward=%d", 
              ...
              else
              {
                ...
                YYSYS_LOG(DEBUG, "current block has no data, but maybe in next block."
            ...
            {
              iterate_status_ = ITERATE_IN_ERROR;
              YYSYS_LOG(ERROR, "block scaner initialize error, iret=%d," 
          ...
          {
            iterate_status_ = ITERATE_IN_ERROR;
            YYSYS_LOG(ERROR, "get current block data error, iret=%d, cursor_=%ld, block count=%ld,"
    ...
    void ObColumnGroupScanner::free_buffer()
    {
      ...
      {
        arena->free();
        YYSYS_LOG(DEBUG, "test::whx free arean!");
      }
    }
    //add e
