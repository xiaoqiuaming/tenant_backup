...
int ObSequenceUpdate::copy_sequence_info_from_update_stmt()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "the update stmt is not init! ret::[%d]",ret);
  }
  if (OB_SUCCESS == ret)
  {
...
int ObSequenceUpdate::fill_the_sequence_info_to_cond_expr(ObSqlExpression *filter, uint64_t cond_expr_id)
{
  ...
  YYSYS_LOG(DEBUG,"the size of name type is[%ld] the boundary is[%ld]",sequence_name_type_pairs_.count(),condition_types_names_idx_);
      ...
      if (ObIntType == post_expr_array[i].get_type())
      {
        ...
        else if ((int64_t)ObPostfixExpression::CONST_OBJ == expr_type)//const obj
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(DEBUG,"not a sequence varchar");
              continue;//not a sequence varchar
            }
            else//find
            {
              if (OB_SUCCESS != (ret = get_prevval_by_name(&sequence_name, preval_val, p, s, vs, len, can_use_prevval)))
              {
                YYSYS_LOG(ERROR, "get the info of sequence::[%.*s] failed",sequence_name.length(),sequence_name.ptr());
              }
              if (OB_SUCCESS == ret && can_use_prevval)
              {
                expr_array_obj->set_decimal(preval_val, p, s, vs, len);
                YYSYS_LOG(DEBUG,"set decimal,the obj is::[%s]",to_cstring(post_expr_array[i]));
              }
              else
              {
                if (!can_use_prevval)
                {
                  ret = OB_ERROR;
                  YYSYS_LOG(USER_ERROR, "THE PREVVAL expression of  sequence [%.*s] can't be used before using NEXTVAL",names_types_array.at(0).first.length(),names_types_array.at(0).first.ptr());
                }
              }
            }
          }
...
{
  YYSYS_LOG(INFO, "handle the sequence for update set clause");
  int ret = OB_SUCCESS;
  ...
  YYSYS_LOG(DEBUG,"the out put columns count[%ld]   the exprs ids count[%ld]!",out_put_columns.count(), (sequecne_expr_ids_.count()));
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG,"the update row num is::[%ld]",row_num);
    for (int64_t num = 0; OB_SUCCESS == ret && num < row_num; num++)//for each row
    {
      for (int64_t i = 0; OB_SUCCESS == ret && i < out_put_columns.count(); i++)
      {
        out_put_columns_idx_ = i;
        if (0 != sequecne_expr_ids_.at(i))
        {
          ...
          if (is_const)//do replace
          {
            if (OB_SUCCESS == ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "fill the sequence value to expr error! ret=%d", ret);
              }
            }
          }
          ...
          if (OB_SUCCESS == ret)
          {
            if (OB_SUCCESS != (ret = check_column_cast_validity(*sql_expr, i, true)))
            {
              YYSYS_LOG(USER_ERROR,"value out of range");
          ...
          {
              col_sequence_types_idx_ = col_sequence_types_idx_ < 1 ? 1 : col_sequence_types_idx_ ;
          ...
          YYSYS_LOG(DEBUG, "the column after fill is::[%s]",to_cstring(*sql_expr));
        }//end if
      ...
      if (num == row_num -1 && OB_SUCCESS == ret && is_column_hint_index_)
      {
        ...
        for (int64_t j = 0; OB_SUCCESS == ret && j < out_put_columns.count(); j++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "add set column to index trigger failed, ret=%d",ret);
          }
        }
      }
      if (OB_SUCCESS == ret)
    ...
    for (int64_t i = 0; ret == OB_SUCCESS && i < get_sequence_name_no_dup_size(); i++)
    {
         ...
         //mod e
      {
        ...
        else if (hash::HASH_EXIST == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR,"update quick sequence failed, ret=%d",ret);
          }
        }
      }//end if
    }//end for
  ...
  {
    YYSYS_LOG(ERROR,"the construct sequence info failed!,ret=[%d]",ret);
  }
  return ret;
}
