...
int ObDropTableStmt::add_table_name_id(ResultPlan& result_plan, const ObString& table_name)
{
  ...
  if (ret == OB_SUCCESS /*&& !if_exists_*/)
  {
    ...
    if (OB_INVALID_ID == table_id && !if_exists_)
    {
    ...
    //add:e
    /*del liumz, priv_bugfix[drop table if exists]20170828
    ...
    if (table_schema != NULL && table_schema->get_type() != ObTableSchema::NORMAL)
    {
