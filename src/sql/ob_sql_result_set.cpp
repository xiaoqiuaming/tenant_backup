                              ...
                              const int64_t len, int64_t &pos)
{
  ...
  {
    YYSYS_LOG(ERROR, "encode errno error, ret = [%d]", ret);
  }
  else if (OB_SUCCESS != (ret = sqlstr_.serialize(buf, len, pos)))
  {
    YYSYS_LOG(ERROR, "encode sqlstr error, ret = [%d]", ret);
  }
  else if (OB_SUCCESS != errno_)
  {
    YYSYS_LOG(WARN, "execute sqlstr error, errno: [%d], sql[%.*s]",
              errno_, sqlstr_.length(), sqlstr_.ptr());
  }
  else if (OB_SUCCESS != (ret = encode_vi64(buf, len, pos, fields.count())))
  {
    YYSYS_LOG(ERROR, "encode fields count error, ret = [%d]", ret);
  }
  else
  {
    ...
    if (OB_SUCCESS == ret && select_stmt)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get row desc error, ret = [%d]", ret);
      }
      else if (NULL == row_desc)
      {
        YYSYS_LOG(ERROR, "row or row_desc is NULL");
        ret = OB_SERIALIZE_ERROR;
      }
      else if (OB_SUCCESS != (ret = row_desc->serialize(buf, len, pos)))
      {
        YYSYS_LOG(ERROR, "serilize row desc error, ret = [%d]", ret);
      }
      ...
      if (OB_SUCCESS == ret || OB_ITER_END == ret)
      {
        ...
        do
        {
          if (NULL != row)
          {
            ...
            {
              YYSYS_LOG(DEBUG, "add new row to ob_new_scanner "
            ...
            {
              YYSYS_LOG(ERROR, "add new row to ob_new_scanner error "
        ...
        {
          YYSYS_LOG(ERROR, "fail to serialize scanner, ret = [%d]", ret);
        }
      }
      else
      {
                                ...
                                const int64_t len, int64_t &pos)
{
  ...
  {
    YYSYS_LOG(ERROR, "deserialize errno error! ret = [%d]", ret);
  }
  else if (OB_SUCCESS != (ret = sqlstr_.deserialize(buf, len, pos)))
  {
    YYSYS_LOG(ERROR, "deserialize sqlstr error! ret = [%d]", ret);
  }
  else if (OB_SUCCESS != errno_)
  {
    YYSYS_LOG(WARN, "execute sql [%.*s] error, errno: [%d]",
              sqlstr_.length(), sqlstr_.ptr(), errno_);
  }
  else if (OB_SUCCESS != (ret = decode_vi64(buf, len, pos, &field_cnt)))
  {
    YYSYS_LOG(ERROR, "deserialize fields count error! ret = [%d]", ret);
  }
  else
  {
    for (int64_t i = 0; OB_SUCCESS == ret && i < field_cnt; ++i)
    {
          ...
          || OB_SUCCESS != field.org_cname_.deserialize(buf, len, pos))
      {
        ...
        YYSYS_LOG(ERROR, "deserialize fields error, ret = [%d]", ret);
        break;
      }
      else if (OB_SUCCESS != (ret = fields_.push_back(field)))
      {
        YYSYS_LOG(ERROR, "push field into array error, ret = [%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret && field_cnt > 0)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize row desc error, ret = [%d]", ret);
      }
      else if (OB_SUCCESS != (ret = scanner_.deserialize(buf, len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize new scanner data error, ret = [%d]", ret);
      }
      else
      {
        scanner_.set_default_row_desc(&row_desc_);
        YYSYS_LOG(DEBUG, "deserialize sql_proxy successfully!");
      }
    }
  }
  return ret;
