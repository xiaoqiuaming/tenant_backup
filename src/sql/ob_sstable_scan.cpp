...
int ObSSTableScan::init_sstable_scanner()
{
  ...
  else if ((sstable_version_ = tablet->get_sstable_version()) < SSTableReader::COMPACT_SSTABLE_VERSION)
  {
    ...
    {
      YYSYS_LOG(ERROR, "cannot find sstable with scan range: %s, sstable version: %ld",
    ...
    {
      YYSYS_LOG(ERROR, "set_scan_param to scanner error, range: %s, sstable version: %ld",
          to_cstring(scan_param_.get_range()), sstable_version_);
    }
    else
    {
...
//add duyr [MultiUPS] [READ_ATOMIC] [read_atomic::debug] 20151223:b
int ObSSTableScan::init_data_mark()
{
  int ret = OB_SUCCESS;
...
int ObSSTableScan::open()
{
  ...
  {
    YYSYS_LOG(WARN, "fail to set scan param for scanner");
  }
  //add duyr [MultiUPS] [READ_ATOMIC] [read_atomic::debug] 20151223:b
...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(DEBUG, "sstable scan row count=%ld", row_counter_);
  scanner_.cleanup();
...
int ObSSTableScan::open_scan_context_local_idx(const sstable::ObSSTableScanParam &param, const ScanContext &context, ObNewRange &fake_range)
{
  ...
  {
    YYSYS_LOG(WARN, "cannot acquire tablet with scan range: %s, version: %ld",
  ...
  {
    YYSYS_LOG(WARN, "tablet is null point!");
    ret = OB_ERROR;
  }
  //add e
  ...
  {
    YYSYS_LOG(WARN, "tablet is removed or tablet_range.start_key > scan_range.start_key, "
...
int ObSSTableScan::init_sstable_scanner_for_local_idx(ObNewRange &fake_range)
{
  ...
  {
   // YYSYS_LOG(ERROR, "test::whx scan_param = (%s)",to_cstring(scan_param_));
...
int ObSSTableScan::get_next_row(const ObRowkey* &row_key, const ObRow *&row_value)
{
  ...
  if (NULL == iterator_)
  {
    YYSYS_LOG(ERROR, "internal error, iterator_ not set, call init_sstable_scanner");
    ret = OB_NOT_INIT;
  }
  else
  {
...
int ObSSTableScan::get_tablet_data_version(int64_t &version)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "invoke this func after open");
  }
  else
  {
