...
int ObExecute::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  if (OB_UNLIKELY(NULL == child_op_))
  {
    ...
    {
      ret = OB_NOT_INIT;
      YYSYS_LOG(ERROR, "Stored session plan can not be fount or not correct");
    }
    else
    {
...
int ObExecute::open()
{
  ...
  {
    ret = OB_ERR_GEN_PLAN;
    YYSYS_LOG(WARN, "Prepare statement is not initiated, stmt_id=%lu", stmt_id_);
  }
  else
  {
...
int ObExecute::fill_execute_items()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "Stored session plan can not be fount or not correct, result_set=%p main_query=%p phy_plan=%p",
              result_set, main_query, physical_plan);
  }
  else if ((ret = set_child(0, *main_query)) != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "Find stored executing plan failed");
  }
  //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160223:b
  else
  {
    ...
    /**
     * for insert,update,delete,replace,the main query is ObUpsExecutor
     */
    //mod by maosy [MultiUPS 1.0 ] [batch and prepare] 20170516 b
    /*?????update delete insert???iud loop???????*/
    //    if (PHY_UPS_EXECUTOR == main_query->get_type())
    //    {
    //    if (NULL != (ups_executor = dynamic_cast<ObUpsExecutor *>(main_query)))
    if (PHY_UPS_EXECUTOR == main_query->get_type() || PHY_IUD_LOOP_CONTROL == main_query->get_type())
    {
        if (NULL != (ups_executor = dynamic_cast<ObUpsExecutor *>(main_query))
                ||NULL != (ups_executor = dynamic_cast<ObUpsExecutor *>(main_query->get_child(0))))
            //mod by maosy 20170516 e
      {
        if (NULL == sql_context_)
        {
          ret = OB_NOT_INIT;
          YYSYS_LOG(WARN, "sql context is not inited");
        }
        else
        {
          YYSYS_LOG(INFO, "prepare for update dml");
          ups_executor->set_sql_context(sql_context_);
        }
      }
      else
      {
        YYSYS_LOG(INFO, "prepare for select");
      }
    }
  }
  //add 20160223:e
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (param_values.count() != param_names_.count())
    {
      ...
      YYSYS_LOG(USER_ERROR, "Incorrect arguments number to EXECUTE, need %ld arguments", param_values.count());
    }
    for (int64_t i = 0; ret == OB_SUCCESS && i < param_names_.count(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "Get variable %.*s faild. ret=%d", param_name.length(), param_name.ptr(), ret);
      }
      else
      {
            YYSYS_LOG(DEBUG, "execute using variables, i=%ld val=%s stmt_id=%ld",
  ...
  {
    YYSYS_LOG(WARN, "failed to open result set, err=%d", ret);
  }
  return ret;
}
...
int ObExecute::get_next_row(const common::ObRow *&row)
{
  ...
  if (NULL == child_op_)
  {
    ...
    YYSYS_LOG(ERROR, "child_op_ must not NULL");
  }
  else
  {
