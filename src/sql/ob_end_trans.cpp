...
int ObEndTrans::open()
{
  ...
  session->get_trans_info(req_);
  //mod 20150702:e
  int64_t begin_time_us = yysys::CTimeUtil::getTime();
  if (!req_.trans_id_.is_valid())
  {
    YYSYS_LOG(DEBUG, "session_id[%ld] not in transaction",session->get_session_id());
  }
  ...
  */
  ...
  if (0 < session->get_curr_trans_start_time())
  {
    ...
    YYSYS_LOG(DEBUG, "trans finished, start=%ld trans=%ld", session->get_curr_trans_start_time(), trans_time);
