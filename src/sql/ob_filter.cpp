...
int ObFilter::add_filter(ObSqlExpression* expr)
{
  ...
  if (!filters_.add_last(expr))
  {
    ...
    YYSYS_LOG(ERROR, "failed to add column");
  }
  return ret;
}
...
int ObFilter::remove_last_filter()
{
  ...
  {
    YYSYS_LOG(ERROR, "wtf,you shouldn't see this");
    ret = OB_ERR_UNEXPECTED;
  }
  else
  {
    ...
    if(NULL == node)
    {
      YYSYS_LOG(WARN, "you cann't remove DList header!");
      ret = OB_ERR_UNEXPECTED;
    }
    else if(NULL == expr)
    {
      YYSYS_LOG(INFO, "this is not a ObSqlExpression type");
    }
    else
    {
  ...
  YYSYS_LOG(DEBUG, "[semi join]::after remove last filters_ ,size[%d],is_empty[%d]",
            filters_.get_size(), filters_.is_empty());
  return ret;
}
//add e
...
int ObFilter::open()
{
  ...
  if (OB_SUCCESS != (ret = ObSingleChildPhyOperator::open()))
  {
    ...
    {
      YYSYS_LOG(WARN,"failed to open child op!ret=%d",ret);
    }
  }
  else if (need_handle_sub_query_
           && (OB_SUCCESS != (ret = process_sub_query())))
  {
    YYSYS_LOG(WARN,"failed to process sub query!ret=%d",ret);
  }
  return ret;
  //add tianz 20151016:e
}
...
int ObFilter::get_next_row(const common::ObRow *&row)
{
  ...
  //YYSYS_LOG(DEBUG,"TEST FILTER:%s", to_cstring(*this));
  if (OB_UNLIKELY(NULL == child_op_))
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "child_op_ is NULL");
  ...
  else
  {
    ...
    {
      //YYSYS_LOG(DEBUG,"TEST input_row:%s", to_cstring(*input_row));
      did_output = true;
      ...
      dlist_for_each(ObSqlExpression, p, filters_)
      {
        ...
        //mod duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160121:b   conflict_level B
        //if (OB_SUCCESS != (ret = p->calc(*input_row, result)))
        if (read_atomic_param_.is_valid())
        {
          if (OB_SUCCESS != (ret = filter_read_atomic_row_(input_row,p,did_output)))
          {
            YYSYS_LOG(WARN,"fail to filter read atomic row!ret=%d",ret);
            break;
          }
          else if (!did_output)
          {
         ...
         //mod duyr 20160121:e
      else if (OB_SUCCESS != (ret = p->calc(*input_row, result, sub_hash,is_hashmap_contain_null, p_data_type_desc, use_hash_map)))        
      {
...
//add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160121:b
int ObFilter::filter_read_atomic_row_(const common::ObRow *input_row,
                                      ObSqlExpression *filter,
                                      bool &result)
{
  int ret = OB_SUCCESS;
  const ObObj *filter_result = NULL;
  result = false;

  if (NULL == input_row || NULL == input_row->get_row_desc() || NULL == filter)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR,"invalid argument!input_row=%p,filter=%p,ret=%d",
              input_row,filter,ret);
  }
  else if (!read_atomic_param_.is_valid())
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR,"invalid read atomic param!parm=[%s],ret=%d",
              to_cstring(read_atomic_param_),ret);
  }
  else if (!read_atomic_param_.is_need_real_data()
           && read_atomic_param_.is_need_extend_data())
  {//just has  extend values,can't filter this row!
    result = true;
  }
  else if (OB_SUCCESS != (ret = filter->calc(*input_row,filter_result)))
  {
    YYSYS_LOG(WARN,"fail to calc expr!input_row=[%s],filter=[%s],ret=%d",
              to_cstring(*input_row),to_cstring(*filter),ret);
  }
  else if (filter_result->is_true())
  {
    result = true;
  }
  else if (read_atomic_param_.need_prepare_data_)
  {
    bool is_prepare_row_exist = false;
    common::ObRow prepare_row;
    prepare_row.set_row_desc(*(input_row->get_row_desc()));
    if (OB_SUCCESS != (ret = common::ObReadAtomicHelper::split_commit_preapre_row_(input_row,
                                                                                   read_atomic_param_,
                                                                                   NULL,NULL,
                                                                                   &prepare_row,
                                                                                   &is_prepare_row_exist)))
    {
      YYSYS_LOG(WARN,"fail to get prepare row!prepare_row=[%s],is_prepare_row_exist=%d,ret=%d",
                to_cstring(prepare_row),is_prepare_row_exist,ret);
    }
    else if (is_prepare_row_exist
             && OB_SUCCESS != (ret = filter->calc(prepare_row,filter_result)))
    {
      YYSYS_LOG(WARN,"fail to calc filter!prepare_row=[%s],filter=[%s],filter_result=%p,ret=%d",
                to_cstring(prepare_row),to_cstring(*filter),filter_result,ret);
    }
    else if (is_prepare_row_exist)
    {
      result = filter_result->is_true();
    }

    YYSYS_LOG(DEBUG,"read_atomic::debug,finish filter prepare row!prepare_row=[%s],is_prepare_row_exist=%d,result=%d,ret=%d",
              to_cstring(prepare_row),is_prepare_row_exist,result,ret);
  }

  if (NULL != input_row && NULL != filter)
    YYSYS_LOG(DEBUG,"read_atomic::debug,finish filter row!orig_filter_row=[%s],filter=[%s],result=%d,ret=%d",
              to_cstring(*input_row),to_cstring(*filter),result,ret);
...
DEFINE_SERIALIZE(ObFilter)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to serialize filter expr count. ret=%d", ret);
  }
  else
  {
    dlist_for_each_const(ObSqlExpression, p, filters_)
    {
      ...
      {
        YYSYS_LOG(WARN, "filter expr serialize fail. ret=%d", ret);
        break;
      }
    } // end for
  }
...
DEFINE_DESERIALIZE(ObFilter)
{
  ...
  else
  {
    for (i = 0; i < expr_count; i++)
    {
      ...
      {
        ObSqlExpression::free(expr);
        YYSYS_LOG(WARN, "fail to deserialize expression. ret=%d i=%ld count=%ld", ret, i, expr_count);
        break;
      }
      else if (OB_SUCCESS != (ret = add_filter(expr)))
      {
        YYSYS_LOG(WARN, "fail to add expression to filter.ret=%d, buf=%p, data_len=%ld, pos=%ld", ret, buf, data_len, pos);
        break;
      }
    }
  }
  ...
  dlist_for_each_const(ObSqlExpression, p, o_ptr->filters_)
  {
    ...
    else
    {
      *expr = *p;
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "Add expression to ObFilter failed");
        break;
      }
    }
  }
