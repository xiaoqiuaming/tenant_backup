   ...
   public:
    ...
    struct ObPostfixInAdaptor
    {
        ...
        ObPostfixInAdaptor()
            :cdim(0),
              ...
              is_not(false),
              inhash(false),
              has_null_left_(false),
              rhv(2048,ModulePageAllocator(ObModIds::OB_IN_POSTFIX_EXPRESSION)),
              rhm(1024,ModulePageAllocator(ObModIds::OB_IN_POSTFIX_EXPRESSION))
        {
        }
    ...
    struct ObInRowOperator
    {
          ...
          else
          {
            ...
            while (i > 0)
            {
              ...
              if(E_obj.get_type()==ObDecimalType)
              {
                ...
                uint32_t len;
                len = E_obj.get_len();
                TTInt *tt = dec.get_words();
                obj.set_decimal(tt->ToUInt_v2(),dec.get_precision(),dec.get_scale(),dec.get_vscale(),len);
                ...
                if(type != ObNullType )
                {
                  ...
                  if(OB_SUCCESS != (ret = obj_cast(obj,ex_type,res[i],temp)))
                  {
                    char b[256];
                    obj.to_string(b,255);
                    YYSYS_LOG(WARN,"failed to cast %s to type %d",b,type);
                  }
                  else
                  {
              ...
              else
              {
                  ...
                  if(type != ObNullType )
                  {
                      ...
                      else
                      {
                         ...
                         {
                             YYSYS_LOG(WARN,"failed to copy obobj ret=%d",ret);
                             return ret;
                         }
                      }
                  }
              ...
              else
              {
                  ...
                  if(has_null)
                  {
                      ...
                      if(ne)
                      {
                          ...
                          {
                              YYSYS_LOG(WARN, "fail to deep copy column,%d",ret);
                              return ret;
                          }
                          else
                          {
                              if(OB_SUCCESS != (ret = ip->in_null_.push_back(inr)))
                                  YYSYS_LOG(WARN,"failed to push null value into in_null vector for not in postexprssion");
                              return ret;
            ...
            int hash_err = 0;
              ...
              {
                  YYSYS_LOG(WARN, "fail to deep copy value,%d",ret);
                  return ret;
              }
               hash_err = ip->right_hash_.set(columns2,cvals,1);
                ...
                {
                  ret = OB_ERROR;
                  YYSYS_LOG(WARN,"fail to set hash value of current group [err:%d]", hash_err);
                }



          }
          //YYSYS_LOG(ERROR,"dolphin:: exit out push_right_hash idx_i:%d,dim:%ld,f=%d",top,count,f);
      ...
      int push_row(const ObExprObj *stack, int &top, const int count)
      {
        ...
        if (top < count)
        {
          YYSYS_LOG(WARN, "not enough row elements in stack. top=%d, count=%d", top, count);
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
          for (i = 0; i < count; i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to push element to IN vector. ret=%d", ret);
            }
      ...
      int get_result(ObExprObj *stack, int &top, ObExprObj &result,bool is_in)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get_int from stack. top=%d, ret=%d", top, ret);
        }
        ...
        else
        {
          ...
          switch (dim)
          {
            case 1:
              ...
              {
                YYSYS_LOG(WARN, "fail to check element in row. left_start_idx=%ld, right_start_idx=%ld, elem_count=%ld, width=%ld",
            ...
            case 2:
              ...
              break;
            default:
              YYSYS_LOG(WARN, "invalid dim. dim=%ld", dim);
          ...
          const int64_t right_start_idx, const int64_t right_elem_count, const int64_t width,bool in)
      {
            ...
            right_idx += width)
        {
          ...
          for (i = 0; OB_SUCCESS == ret && i < width; i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to get element from array. ret=%d. vec.count=%ld, width[%ld], i[%ld], left_idx[%ld], right_idx[%ld]" ,
    ...
    class ObPostfixExpression
    {
     ...
     public:
        ...
        bool is_op_is_null();
        //add dragon e
        bool is_not_equal_cond(uint64_t &table_id);
      ...
      int check_in_hash(ObExprObj& result,int t = 0,int64_t v = T_OP_IN);
		...
		*/
        void set_has_bloomfilter(ObBloomFilterV1 * bloom_filter)
      ...
      private:
        ...
        //add wanglei:e
        static inline int is_month(char *tmp_month, char *month);
        //add wanglei [to_timestamp] 20160311:b
    ...
    public:
        ...
        //add e
         ...
         int check_partition_body_expr(int& op_row_num);
         //add 20160105:e
         //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160122:b
      private:
         friend class ObReadAtomicRpcScan;
         int get_next_tid_cid(ObObj *&tid_val,ObObj *&cid_val,bool &is_finish,int64_t &next_expr_idx);
      //add duyr 20160122:e
      private:
        ExprArray expr_;
        ObExprObj *stack_;
