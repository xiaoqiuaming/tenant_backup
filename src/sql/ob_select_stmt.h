...
namespace oceanbase
{
  namespace sql
  {
    ...
    struct JoinedTable
    {
      ...
      void reset()
      {
        ...
        expr_ids_.clear();
        join_types_.clear();
        expr_nums_per_join_.clear();
        optimized_join_operator_.clear();
      }
      void destroy()
      {
        table_ids_.~ObArray();
        expr_ids_.~ObArray();
        join_types_.~ObArray();
        expr_nums_per_join_.~ObArray();
        optimized_join_operator_.~ObArray();
      }
      enum JoinType
      {
        T_FULL,
      ...
      { return join_types_.push_back(type); }
      int add_expr_id(uint64_t eid)
      { return expr_ids_.push_back(eid); }
      ...
      common::ObArray<int64_t>& get_expr_nums_per_join() {return expr_nums_per_join_;}
      int64_t get_expr_ids_index(int64_t index);
      ...
      common::ObArray<int64_t>   expr_nums_per_join_;
       ...
       ObOptimizerRelation* from_item_rel_opt_;
    };
    ...
    struct JoinedTableInfo
    {
        ...
        {
            expr_ids_.clear();
            joined_table_id_ = OB_INVALID_ID;
            type_ = JoinedTable::T_INNER;
            tid_[0] = OB_INVALID_ID;
            tid_[1] = OB_INVALID_ID;
            if(NULL != child_[0])
            {
                child_[0]->~JoinedTableInfo();
                child_[0] = NULL;
            }
            if(NULL != child_[1])
            {
                child_[1]->~JoinedTableInfo();
                child_[1] = NULL;
            }
        }
        void destroy()
        ...
        int add_join_exprs(ObVector<uint64_t> &exprs)
        {
          ...
          {
            ret = expr_ids_.push_back(exprs.at(idx));
          }
          return ret;
        }

        uint64_t tid_[2];
        JoinedTable::JoinType type_;
        JoinedTableInfo *child_[2];
        uint64_t joined_table_id_;
        common::ObArray<uint64_t> expr_ids_;
    };
  }
  ...
  namespace sql
  {
    ...
    public:
      ...
      int32_t get_partition_expr_size() const
      { return partition_expr_ids_.size(); }
      int32_t get_order_item_for_rownum_size() const
      { return order_items_for_rownum_.size(); }
      int32_t get_anal_fun_size() const
      { return anal_func_ids_.size(); }
      //add:e
      ...
      const SelectItem *get_select_item(uint64_t expr_id) const;
      ...
      int32_t get_order_item_for_listagg_size() const
      { return order_items_for_listagg_.size(); }
      //add 20150104:e
      ...
      int add_raw_column(const uint64_t &expr_id,
                         const common::ObString &column_name);
      const common::ObString *get_raw_column(const uint64_t expr_id) const;

      int64_t get_raw_column_size()
      {
        return raw_column_names_.count();
      }

      int add_joined_table_info(JoinedTableInfo *joined_tables_info)
      {
        return joined_tables_info_.push_back(joined_tables_info);
      }

      JoinedTableInfo *get_joined_table_info(int32_t idx)
      {
        OB_ASSERT(0 <= idx && idx < joined_tables_info_.size());
        return joined_tables_info_.at(idx);
      }

      int32_t get_joined_table_info_size()
      {
        return joined_tables_info_.size();
      }
      ...
      common::ObArray<common::ObObjType> & get_output_columns_dsttype()
      {return output_columns_dsttype_;}
      //add 20151225:e
      void print(FILE* fp, int32_t level, int32_t index = 0);

      void print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level=0);
      ...
      inline int64_t get_range_vector_count()
      {return range_vectors_.count();}
      inline void set_start_is_min()
      { start_is_min_ = true;}
      inline bool start_is_min()
      { return start_is_min_;}
      inline void set_end_is_max()
      { end_is_max_ = true;}
      inline bool end_is_max()
      { return end_is_max_;}
      inline void set_has_range(bool has_range)
      ...
      bool select_clause_has_sequence()
      {return select_clause_has_sequence_;}
      void set_select_clause_has_sequence(bool is_has_seq)
      {select_clause_has_sequence_ = is_has_seq;}
      //pair
      ...
      */
      ...
      bool is_support_read_atomic()
      {
        return (0 >= joined_tables_.size() && (1 == get_table_size())
                && (0 >= agg_func_ids_.size()) && (!IS_SYS_TABLE_ID(table_items_[0].ref_id_)));
      }
      //add hongchen [FIX_SUPPORT_READ_ATOMIC] 20170612:e
      inline bool is_partition_calc_func()
      { return is_partition_calc_func_;}
      inline void set_is_partition_calc_func(bool is_partition_calc_func)
      {
        is_partition_calc_func_ = is_partition_calc_func;
      }
      inline uint64_t get_partition_cal_table_id()
      {
        return partition_calc_table_id_;
      }
      inline void set_partition_cal_table_id(uint64_t table_id)
      {
        partition_calc_table_id_ = table_id;
      }
      inline int64_t get_partition_cal_version_flag()
      {
        return partition_cal_version_flag_;
      }
      inline void set_partition_cal_version_flag(int64_t partition_cal_version_flag)
      {
        partition_cal_version_flag_ = partition_cal_version_flag;
      }
      inline common::ObArray<uint64_t> get_partition_cal_values_()
      {
        return partition_cal_values_;
      }
      inline int add_partition_cal_values(uint64_t expr_id)
      {
        return partition_cal_values_.push_back(expr_id);
      }
      inline int64_t get_partition_cal_values_size() const
      {
        return partition_cal_values_.count();
      }
      inline uint64_t get_partition_cal_value_id(int64_t index) const
      {
        OB_ASSERT(0<= index && index < partition_cal_values_.count());
        return partition_cal_values_.at(index);
      }
      ...
      int remove_agg_func_id(const int32_t index)
      {
          int ret = OB_SUCCESS;
          ret = agg_func_ids_.remove(index);
          return ret;
      }
      bool is_max_min_eliminate()
      {
          return is_max_min_eliminate_;
      }

      void set_is_max_min_eliminate(bool is_max_min_eliminate)
      {
          is_max_min_eliminate_ = is_max_min_eliminate;
      }

      uint64_t get_eliminate_index_col_id()
      {
          return eliminate_idnex_col_id_;
      }

      void set_eliminate_index_col_id(uint64_t eliminate_index_col_id)
      {
          eliminate_idnex_col_id_ = eliminate_index_col_id;
      }
    private:
      ...
      bool is_max_min_eliminate_;
      uint64_t eliminate_idnex_col_id_;
      ...
      common::ObArray<common::ObArray<std::pair<common::ObString,uint64_t> > > sequence_name_type_pairs_;//��������е�sequence��Ϣ
      common::ObArray<std::pair<common::ObString,uint64_t> >                   single_name_type_pair_;  //���һ�е�sequence��Ϣ
      common::ObArray<uint64_t>  sequence_where_clause_expr_id_;            //�洢where�����е�sequence��expr_id����push��expr_id��û��push��0
      common::ObArray<bool>      sequence_select_clause_has_sequence_;      //�洢select��ѯ�����Ƿ���sequence����push��true��û��push��false
      bool                       select_clause_has_sequence_;               //���ڱ��select clause���Ƿ�ʹ�ù�sequence
      int64_t                    column_has_sequence_count_;                //���ڼ�¼select clause���ж����а���sequence
      //add 20150525:e
      //delete by xionghui [subquery_final] 20160216 :b
      /*
      //add zhujun [fix equal-subquery bug] 20151013:b
      ...
      */
      ...
      //add 20160107:e
      bool is_partition_calc_func_;
      uint64_t partition_calc_table_id_;
      int64_t partition_cal_version_flag_;
      common::ObArray<uint64_t> partition_cal_values_;

      oceanbase::common::ObList<ObOptimizerRelation*> rel_opt_list;
      common::hash::ObHashMap<uint64_t,ObBaseRelStatInfo*, common::hash::NoPthreadDefendMode> table_id_statInfo_map_;
      oceanbase::common::ObList<ObOptimizerRelation*> subquery_rel_opt_list_;
      oceanbase::common::ObList<ObFromItemJoinMethodHelper*> from_item_method_list_;
      oceanbase::common::ObList<int> from_item_appear_order_list_;
      oceanbase::common::ObList<ObOptimizerRelation*> unaryRef_expr_rel_info_list_;
      oceanbase::common::ObList<ObOptimizerRelation*> joined_from_item_rel_info_list_;
      ObOptimizerRelation* select_stmt_rel_info_;
      common::ObArray<std::pair<common::ObString, uint64_t> > raw_column_names_;
      common::ObVector<JoinedTableInfo *> joined_tables_info_;
    ...
    inline int ObSelectStmt::cons_sequence_single_name_type_pair(common::ObString &sequence_name)
    {
      ...
      {
        YYSYS_LOG(ERROR,"add sequence name no dup error!");
      }
      if (common::OB_SUCCESS == ret)
      {
        name_type.first.assign_ptr(sequence_name.ptr(),sequence_name.length());
        YYSYS_LOG(DEBUG,"clone name is[%.*s]",name_type.first.length(),name_type.first.ptr());
        if (is_next_type())//ture nextval
        {
          name_type.second = common::NEXT_TYPE;
          YYSYS_LOG(DEBUG,"push next");
        }
        else//false  prevval
        {
          name_type.second = common::PREV_TYPE;
          YYSYS_LOG(DEBUG, "push preval");
        }
        if (OB_SUCCESS != (ret = single_name_type_pair_.push_back(name_type)))
        {
          YYSYS_LOG(ERROR,"push back single_name_type_pair_ failed:[%.*s]", sequence_name.length(), sequence_name.ptr());
        }
      }
      return ret;
    }
