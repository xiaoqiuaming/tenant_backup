...
int ObSQLIdMgr::init()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObHashMap create error, ret: %d", ret);
  }
  else if (OB_SUCCESS != (ret = id2sql_map_.create(SQL_ID_MAX_BUCKET_NUM)))
  {
    YYSYS_LOG(WARN, "id2sql_map_ init error, err=%d", ret);
  }
  return ret;
}
...
int ObSQLIdMgr::get_sql_id(const ObString & sql, int64_t & sql_id)
{
  ...
  if (HASH_EXIST == err)
  {
    YYSYS_LOG(DEBUG, "got sql_id from sqlidmgr sql(%.*s), sqlid=%ld", sql.length(), sql.ptr(), sql_id);
  }
  else if (HASH_NOT_EXIST == err)
  {
    ...
    else if (HASH_NOT_EXIST == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "ObStringBufT write_string error, err: %d", err);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (HASH_INSERT_SUCC == err)
        {
          ...
          {
            YYSYS_LOG(ERROR, "This should not be reached");
            sql_id = 0;
          ...
          else
          {
            ...
            stat.create_timestamp_ = yysys::CTimeUtil::getTime();
            if (HASH_INSERT_SUCC != (err = id2sql_map_.set(sql_id, stat)))
            {
              // rollback
              sql2id_map_.erase(stored_sql);
              YYSYS_LOG(ERROR, "failed to insert id2sql map, err=%d", ret);
              ret = OB_ERROR;
            }
            else
            {
        ...
        {
          YYSYS_LOG(ERROR, "insert into sql2id failed ret is %d", err);
          ret = OB_ERR_UNEXPECTED;
        }
      }
    }
  ...
  {
    YYSYS_LOG(ERROR, "ObSQLIdMgr get error, sql(%.*s) err: %d", sql.length(), sql.ptr(), err);
    ret = OB_ERROR;
  }
  return ret;
}
...
struct ObStatPrepareOp
{
  void operator() (oceanbase::common::hash::HashMapPair<SQLIdValue, SQLStat> &entry)
  {
    ...
    entry.second.last_active_timestamp_ = yysys::CTimeUtil::getTime();
  };
};
...
int ObSQLIdMgr::stat_prepare(int64_t sql_id)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to get sql_id=%ld err=%d", sql_id, err);
    ret = OB_ERR_UNEXPECTED;
  }
  return ret;
}
...
{
  int64_t count = 0;
  YYSYS_LOG(INFO, "================sql statistics begin================");
  IdSqlMap::const_iterator it = id2sql_map_.begin();
  for (; it != id2sql_map_.end(); ++it)
  {
    int64_t avg_execute_us = 0;
    ...
    YYSYS_LOG(INFO, "[SQL] idx=%ld id=%ld stmt=`%.*s' prepared_count=%ld executed_count=%ld "
  ...
  YYSYS_LOG(INFO, "================sql statistics end================");
}
