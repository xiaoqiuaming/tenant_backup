...
int ObTabletCacheJoin::open()
{
  ...
  {
    YYSYS_LOG(WARN, "not inited: table_id_[%lu], ups_row_cache_[%p]",
      table_id_, ups_row_cache_);
  }
  else if (OB_SUCCESS != (ret = ObTabletJoin::open()))
  {
    YYSYS_LOG(WARN, "ObTabletJoin open fail:ret[%d]", ret);
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(NULL == tmp)
    {
      ...
      YYSYS_LOG(WARN, "allocate mem for cache handle fail");
    }
    else
    {
...
int ObTabletCacheJoin::close()
{
  ...
  {
    YYSYS_LOG(WARN, "ObTabletJoin close fail:ret[%d]", ret);
  }

  if(OB_SUCCESS != (ret = revert_cache_handle()))
  {
    YYSYS_LOG(WARN, "revert cache handle fail:ret[%d]", ret);
  }
...
int ObTabletCacheJoin::get_ups_row(const ObRowkey &rowkey, ObUpsRow &ups_row, const ObGetParam &get_param)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_ENTRY_NOT_EXIST == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "fetch next ups row fail:ret[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = ups_row_cache_->get(cache_key, value)))
      {
        YYSYS_LOG(WARN, "get rowkey %s from cache not exist.ret[%d]", to_cstring(rowkey), ret);
      }
    }
    else if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get rowkey %s from cache not exist.", to_cstring(rowkey));
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "convert to ups row fail:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObTabletCacheJoin::fetch_ups_row(const ObGetParam &get_param)
{
  ...
  YYSYS_LOG(DEBUG, "fetch join ups_row param: %ld, %s", get_param.get_cell_size(),
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "get thread specific buffer fail:ret[%d]", ret);
    }
    else
    {
      buffer->reset();
  ...
  if(OB_SUCCESS == ret && get_param.get_cell_size() > 0)
  {
    ...
    {
      YYSYS_LOG(WARN, "open ups multi get fail:ret[%d]", ret);
    }
    ...
    while(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "ups multi get next row fail:ret[%d]", ret);
      }
      else
      {
        if (row_count++ < 10)
        {
          YYSYS_LOG(DEBUG, "fetch join ups row(%ld): key:%s, row:%s",
        ...
        if(NULL == ups_row)
        {
          ...
          YYSYS_LOG(WARN, "shoud be ObUpsRow");
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "convert ups row to compact row fail:ret[%d]", ret);
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        if(handle_count_ >= batch_count_)
        {
          ...
          YYSYS_LOG(WARN, "handle_count_ is overflow[%ld]", handle_count_);
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        if(OB_BUF_NOT_ENOUGH == ret)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN, "row put in join cache [%ld]", row_in_cache_num);
          }
          else
          {
            YYSYS_LOG(DEBUG, "join cache size overflow [%ld] row add to cache", row_in_cache_num);
            ret = OB_SUCCESS;
          }
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "join cache put fail:ret[%d], rowkey[%s]", ret, to_cstring(*rowkey));
        }
        else
        {
          row_in_cache_num ++;
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "close ups multi get fail:ret[%d]", ret);
      }
    }
  }
...
int ObTabletCacheJoin::compose_next_get_param(ObGetParam &next_get_param, int64_t fused_row_idx)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    for(int64_t i=fused_row_idx;OB_SUCCESS == ret && i<fused_row_array_.count();i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "get stored row from fused row array fail:ret[%d], i[%ld]", ret, i);
      }
      else if(OB_SUCCESS != (ret = ObRowUtil::convert(stored_row->get_compact_row(), row)))
      {
        YYSYS_LOG(WARN, "convert row fail:ret[%d], i[%ld], fused_row_array_count[%ld]", ret, i, fused_row_array_.count());
      }
...
int ObTabletCacheJoin::revert_cache_handle()
{
  ...
  if(OB_SUCCESS == ret)
  {
    for(int64_t i=0;OB_SUCCESS == ret && i<handle_count_;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "revert cache handle fail:ret[%d], [%ld]", ret, i);
      }
    }
  }
...
int ObTabletCacheJoin::fetch_next_ups_row(const ObGetParam &get_param, int64_t fused_row_idx)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "compose next get param fail:ret[%d]", ret);
        }
        else
        {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fetch_ups_row fail:ret[%d]", ret);
    }
  }
  return ret;
}
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_ENTRY_NOT_EXIST == ret)
    {
      if(OB_SUCCESS != (ret = compose_get_param(table_join_info_.right_table_id_, rowkey, get_param)))
      {
        ...
        {
          YYSYS_LOG(WARN, "compose get param fail:ret[%d]", ret);
        }
      }
    }
    else if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "join cache get fail:ret[%d]", ret);
    }
    else
    {
      handle_count_ ++;
