...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      { replace_values_subquery_ = subquery; }
      enum ObPhyOperatorType get_type() const
      { return PHY_INDEX_TRIGGER_REP; }
      //bool is_first_replace();
      //add wenghaixing [secondary index replace.fix_replace_bug]20150422
      inline void set_data_max_cid(uint64_t cid)
      {data_max_cid_ = cid;}
      inline uint64_t get_data_max_cid()
      {return data_max_cid_;}
      //add e
      //add wenghaixing [secondary index static_index_build.consistency]20150424
      inline virtual int64_t get_index_num()const
      {return index_num_;}
      inline void set_main_desc(ObRowDesc &desc)
      {main_row_desc_ = desc;}
      inline void get_main_desc(ObRowDesc &desc)
      {desc = main_row_desc_;}
      //add e
      //add hongchen [secondary index optimize] 20170803:b
      inline void set_real_replace_column_desc(ObRowDesc* desc)
      {real_replace_column_desc_ = *desc;}
      //add hongchen [secondary index optimize] 20170803:e
      DECLARE_PHY_OPERATOR_ASSIGN;
      NEED_SERIALIZE_AND_DESERIALIZE;
    //add hongchen [REPLACE_INDEX_OPT] 20170804:b
    private:
      int cons_del_row_and_save(const int64_t &index_idx, ObRowStore& delete_row_store);
      int cons_ins_row_and_save(const int64_t &index_idx, ObRowStore& insert_row_store, ObRowStore* store);
    ...
    private:
      ...
      common::ObRowDesc real_replace_column_desc_;  //add hongchen [REPLACE_INDEX_OPT] 20170804
      ...
      bool       is_data_row_iter_end_;             //add hongchen [REPLACE_INDEX_OPT] 20170804
      //add by maosy [MultiUps 1.0] [secondary index optimize] 20170622 b:
      ObRow static_data_row_;
      int64_t last_index_;
      ObSEArray<int64_t,100> tevalues_index_;
      bool need_get_next_row_ ;
      //add by maosy e
