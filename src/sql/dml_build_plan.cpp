...
int resolve_partition_calc_func(
    ResultPlan * result_plan,
    ObSelectStmt* select_stmt,
    ParseNode* node);
...
int resolve_joined_table(
    ...
    JoinedTableInfo *joined_table_info = NULL);
...
int resolve_default_node(
    ResultPlan * result_plan,
    ParseNode* node,
    ObObj default_value,
    ObStringBuf *name_pool);

int resolve_insert_default(
    ResultPlan * result_plan,
...
static int add_all_rowkey_columns_to_stmt(ResultPlan* result_plan, uint64_t table_id, ObStmt *stmt)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument. stmt=%p, result_plan=%p", stmt, result_plan);
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      for (rowkey_idx = 0; rowkey_idx < rowkey_info.get_size(); rowkey_idx++)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get table %lu column %ld. ret=%d", table_id, rowkey_idx, ret);
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
        ...
        else if (NULL == (rowkey_column_schema = schema_checker->get_column_schema(table_id, rowkey_column_id)))
        {
          ...
          YYSYS_LOG(WARN, "fail to get table %lu column %lu", table_id, rowkey_column_id);
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
        ...
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "fail to add column item '%s' to table %lu", rowkey_column_schema->get_name(), table_id);
            break;
          }
        }
      }/* end for */
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        else if (expr_scope_type == T_WHERE_LIMIT && stmt->get_stmt_type() == ObStmt::T_SELECT)
        {
          ...
          if (select_stmt->current_expr_has_sequence())
          {
            ...
            {
              YYSYS_LOG(ERROR, "add sequence expr_id failed");
            }
            else if(OB_SUCCESS != (ret = select_stmt->add_sequence_single_name_type_pairs()))
            {
              YYSYS_LOG(ERROR, "add sequence pair failed");
            }
            else
            {
                 ...
                 ObColumnInfo &col_info)
{
  ...
  YYSYS_LOG(DEBUG, "node->type_ = %d", node->type_);
  ...
  switch (node->type_)
  {
    case T_BINARY:
    {
      ...
      if (logical_plan->get_is_show_view())
      {
        c_expr->set_length(static_cast<uint32_t>(node->value_));
      }
    ...
    case T_STRING:
    {
      ...
      {
        //str_val.assign_ptr(const_cast<char*>(node->str_value_), static_cast<int32_t>(node->value_));
        str_val.assign_ptr(const_cast<char*>(node->str_value_), static_cast<int32_t>(strlen(node->str_value_)));
      ...
      {
        YYSYS_LOG(WARN, "string is too long, exceed varchar limit");
        // snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
        //        "Varchar is too long");
        if (OB_ERR_VARCHAR_TOO_LONG == ret)
        {
          ...
          const ObTableSchema *table_schema = NULL;
          if (NULL == (table_schema = static_cast<ObSchemaChecker*>(result_plan->schema_checker_)->get_table_schema(col_info.table_id_)))
          {
            snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "table schema is null");
            ret = OB_ERR_UNEXPECTED;
          }
          else if (NULL != (col_sche = static_cast<ObSchemaChecker*>(result_plan->schema_checker_)->get_column_schema(col_info.table_id_, col_info.column_id_)))
          {
            int64_t length = col_sche->get_size() - col_info.varchar_size_;
            snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                     "Varchar is too long, tname[%s], cname[%s], clength[%ld]", table_schema->get_table_name(), col_sche->get_name(), length);
          }
          else
          {
            snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "col_schema is null");
            ret = OB_ERR_UNEXPECTED;
          }
        }
        break;
      ...
      if (logical_plan->get_is_show_view())
      {
        c_expr->set_length(val.get_val_len());
      }
      expr = c_expr;
      if (node->type_ == T_TEMP_VARIABLE)
      {
        YYSYS_LOG(INFO, "resolve tmp variable, name=%.*s", str.length(), str.ptr());
      }
      break;
    }
    ...
    */
    ...
    case T_DECIMAL: // set as string
    {
      ...
      ObObj val;
      if(OB_SUCCESS != (ret = src.from(str.ptr(),str.length())))
      {
        YYSYS_LOG(ERROR, "fail to do from varchar to decimal, ret=%d", ret);
        break;
      }
      else
      {
        ...
        if (OB_SUCCESS != (ret = ob_write_decimal(*name_pool, src_val, len, dst_val)))
        {
          YYSYS_LOG(WARN, "out of memory");
          break;
        }
      ...
      if (logical_plan->get_is_show_view())
      {
        c_expr->set_precision(val.get_precision());
        c_expr->set_scale(val.get_scale());
      }
      expr = c_expr;
      break;
    }
    ...
    case T_OP_NAME_FIELD:
    {
      ...
      else
      {
        ...
        if (logical_plan->get_is_show_view())
        {
          b_expr->set_length(column_item->data_length_);
          b_expr->set_precision(column_item->data_precision_);
          b_expr->set_scale(column_item->data_scale_);
        }
        expr = b_expr;
    ...
    case T_IDENT:
    {
      ...
      else
      {
        ...
        if (expr == NULL)
        {
          ...
          YYSYS_LOG(DEBUG, "column_name:%.*s", column_name.length(), column_name.ptr());
        ...
        if (!expr && stmt->get_stmt_type() == ObStmt::T_SELECT)
        {
          ...
          if (expr_id != OB_INVALID_ID)
          {
            ...
            else
            {
              ...
              if (logical_plan->get_is_show_view())
              {
                b_expr->set_length(alias_expr->get_expr()->get_length());
                b_expr->set_precision(alias_expr->get_expr()->get_precision());
                b_expr->set_scale(alias_expr->get_expr()->get_scale());
              }
              expr = b_expr;
    ...
    case T_OP_NOT:
    {
                   ...
                   ))
      {
        ...
        switch (sub_expr->get_expr_type())
        {
          ...
          case T_DOUBLE:
          {
            ...
            /*
               *����������decimal��ֵ�ǰ���varcahr��ģ���������Ĵ���������varcahr����ǰ�����һ������
               */
          case T_DECIMAL:
          {
            uint64_t *t1 = NULL;
            ...
            if (OB_SUCCESS != (ret = src_obj.get_ttint(t1)))
            {
              YYSYS_LOG(ERROR, "get decimal pointer from src_obj error, ret=%d", ret);
            }
            else
            {
              ...
              if(logical_plan->get_is_show_view())
              {
                const_expr->set_precision(new_val.get_precision());
                const_expr->set_scale(new_val.get_scale());
              }
            }
            break;
          }
      ...
      else
      {
        ...
        if(logical_plan->get_is_show_view())
        {
          u_expr->set_length(sub_expr->get_length());
          u_expr->set_precision(sub_expr->get_precision());
          u_expr->set_scale(sub_expr->get_scale());
        }
        expr = u_expr;
      }
      break;
    ...
    case T_OP_CNN:
    {
      ...
      ret = resolve_expr(result_plan, stmt, node->children_[0], sql_expr, sub_expr1, expr_scope_type, true, col_info);
      if (ret != OB_SUCCESS)
        break;
      ObRawExpr* sub_expr2 = NULL;
      ret = resolve_expr(result_plan, stmt, node->children_[1], sql_expr, sub_expr2, expr_scope_type, true, col_info);
      if (ret != OB_SUCCESS)
        break;
      ...
      else if (node->type_ == T_OP_POW)
      {
        ...
        if(logical_plan->get_is_show_view())
        {
          b_expr->set_length(sub_expr1->get_length());
          b_expr->set_precision(sub_expr1->get_precision());
          b_expr->set_scale(sub_expr1->get_scale());
        }
      }
      ...
      else if (node->type_ == T_OP_CNN)
      {
        ...
        if (logical_plan->get_is_show_view())
        {
          ObObjType res_type1 = sub_expr1->get_result_type(),res_type2 = sub_expr2->get_result_type();
          if(T_INT <= sub_expr1->get_expr_type() && sub_expr1->get_expr_type() <= T_BOOL)
          {
            ObConstRawExpr *c_expr = dynamic_cast<ObConstRawExpr *>(sub_expr1);
            ObObj value = c_expr->get_value();
            char buf[OB_MAX_VARCHAR_LENGTH] = {0};
            int64_t pos = value.value_to_string(buf, OB_MAX_VARCHAR_LENGTH);
            sub_expr1->set_length(static_cast<uint32_t>(pos));
            res_type1 = ObVarcharType;
          }
          if (T_INT <= sub_expr2->get_expr_type() && sub_expr2->get_expr_type() <= T_BOOL)
          {
            ObConstRawExpr *c_expr = dynamic_cast<ObConstRawExpr *>(sub_expr2);
            ObObj value = c_expr->get_value();
            char buf[OB_MAX_VARCHAR_LENGTH] = {0};
            int64_t pos = value.value_to_string(buf, OB_MAX_VARCHAR_LENGTH);
            sub_expr2->set_length(static_cast<uint32_t>(pos));
            res_type2 = ObVarcharType;
          }
          ObExprObj expr_obj;
          TypeDesc type[2];
          type[0].init(sub_expr1->get_precision(), sub_expr1->get_scale(), sub_expr1->get_length(), res_type1);
          type[1].init(sub_expr2->get_precision(), sub_expr2->get_scale(), sub_expr2->get_length(), res_type2);
          TypeDesc result_type;
          ret = expr_obj.calc(type, OP_CONCAT, result_type);
          if (ret == OB_SUCCESS)
          {
            b_expr->set_length(result_type.length_);
          }
        }
      }
      else
      ...
      if (logical_plan->get_is_show_view())
      {
        if (b_expr->get_result_type() == ObDecimalType &&
            (T_OP_ADD <= node->type_ && node->type_ <= T_OP_DIV))
        {
          oceanbase::common::OpType op_type = static_cast<oceanbase::common::OpType> (node->type_);
          TypeDesc type[2];
          TypeDesc result_type;
          type[0].init(sub_expr1->get_precision(), sub_expr1->get_scale(), sub_expr1->get_length(), sub_expr1->get_result_type());
          type[1].init(sub_expr2->get_precision(), sub_expr2->get_scale(), sub_expr2->get_length(), sub_expr2->get_result_type());
          ObExprObj expr_obj;
          ret = expr_obj.calc(type, op_type, result_type);
          if (ret == OB_SUCCESS)
          {
            b_expr->set_result_type(result_type.type_);
            b_expr->set_precision(result_type.precision_);
            b_expr->set_scale(result_type.scale_);
          }
        }
      }
              ...
              && sub_expr2->get_expr_type()==T_REF_QUERY)
      {
        ...
        if(stmt->get_stmt_type()==ObBasicStmt::T_SELECT)
        {
          ...
          YYSYS_LOG(INFO, "is equal sub-query");
        }
      }
      ...
      if((node->type_ == T_OP_LIKE ||node->type_ == T_OP_NOT_LIKE )&& sub_expr2->get_expr_type() == T_REF_QUERY)
      {
        if(stmt->get_stmt_type()==ObBasicStmt::T_SELECT)
        {
          ...
          //b_expr->set_expr_type(T_OP_IN);
          YYSYS_LOG(INFO, "is like-subquery");
        }
      }
      //add e:
      */
    ...
    case T_CASE:
    {
      ...
      for (int32_t i = 0; ret == OB_SUCCESS && i < node->children_[1]->num_child_; i++)
      {
        ...
        //YYSYS_LOG(ERROR,"then_type is %d,when type is %d,tmp_type is %d",then_type,when_expr->get_result_type(),tmp_type);
      ...
      //YYSYS_LOG(ERROR,"case result_type is %d",tmp_type);
    ...
    case T_FUN_ROW_NUMBER://add liumz, [ROW_NUMBER]20150825
    {
      ...
      //[735]
      if (logical_plan->use_for_view())
      {
        col_expr->set_aggr_expr(ret_sql_expr->get_expr());
        col_expr->set_contain_aggr(true);
      }

      if(logical_plan->get_is_show_view())
      {
        col_expr->set_length(ret_sql_expr->get_expr()->get_length());
        if (node->type_ == T_FUN_SUM)
        {
          col_expr->set_precision(MAX_DECIMAL_DIGIT);
          col_expr->set_scale(ret_sql_expr->get_expr()->get_scale());
        }
        else
        {
          col_expr->set_precision(ret_sql_expr->get_expr()->get_precision());
          col_expr->set_scale(ret_sql_expr->get_expr()->get_scale());
        }
      }
    ...
    case T_FUN_SYS:
    {
      ...
      if (node->num_child_ > 1)
      {
        ...
        if(node->children_[0]->type_ == T_DATE_ADD || node->children_[0]->type_ == T_DATE_SUB)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR,"resolve expr error.ret=%d",ret);
          }
          else if (OB_SUCCESS != (ret = func_expr->add_param_expr(para_datetime_expr)))
          {
            YYSYS_LOG(ERROR,"add param expr error.");
          }
          else
          {
          ...
          //[735]
          if(logical_plan->use_for_view() && node->children_[1] != NULL)
          {
            func_expr->set_date_op_type(node->children_[1]->type_);
            if (T_CUR_TIME <= node->children_[1]->type_ && node->children_[1]->type_ <= T_CUR_TIME_HMS)
            {
              //do nothing
            }
            else if(node->children_[1]->type_ != T_NULL && node->children_[1]->str_value_ != NULL)
            {
              ObString date_time;
              ret = ob_write_string(*logical_plan->get_name_pool(), ObString::make_string(node->children_[1]->str_value_), date_time);
              if(ret != OB_SUCCESS)
              {
                ret = OB_ERR_PARSER_MALLOC_FAILED;
                YYSYS_LOG(WARN, "out of memory");
                snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Malloc function name failed");
              }
              func_expr->set_datetime(date_time);
              func_expr->set_param_is_sys_func(true);//[735]
            }
          }
        }
        else
          ...
          */
            ...
            if(node->children_[1]->children_[i]->type_==T_TYPE_DECIMAL)
            {
              ...
              if(num_child==2)
              {
                ...
                {
                  ret = OB_INVALID_ARGUMENT;
                  YYSYS_LOG(WARN, "invalid argument of decimal. precision = %ld,  scale = %ld. ret=%d", pre, scale,ret);
                }
                if (logical_plan->get_is_show_view())
                {
                  if (ret == OB_SUCCESS)
                  {
                    func_expr->set_precision(static_cast<uint32_t>(pre));
                    func_expr->set_scale(static_cast<uint32_t>(scale));
                  }
                }
              }
            }
          ...
          //[735]
          if ((logical_plan->get_is_show_view() || logical_plan->use_for_view()) &&
              0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_CAST), func_name.ptr(), func_name.length()))
          {
            if(node->children_[1] != NULL && node->children_[1]->children_[1] != NULL
               && node->children_[1]->children_[1]->num_child_ == 1)
            {
              ParseNode *type_node = node->children_[1]->children_[1]->children_[0];
              if(type_node != NULL)
              {
                uint32_t length = static_cast<uint32_t>(type_node->value_);
                func_expr->set_length(length);
              }
            }
          }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_SUBSTR), func_name.ptr(), func_name.length()))
        {
          ...
          if(logical_plan->get_is_show_view())
          {
            TypeDesc type[2];
            bool is_null = false;
            if (func_expr->get_param_size() >= 1)
            {
              ObRawExpr *sub_expr = func_expr->get_param_expr(0);
              if (sub_expr->get_expr_type() == T_NULL)
              {
                is_null = true;
              }
              else
              {
                type[0].init(sub_expr->get_precision(), sub_expr->get_scale(), sub_expr->get_length(), sub_expr->get_result_type());
              }
            }
            bool is_use = false;
            int64_t pos = 0;
            if (func_expr->get_param_size() >= 2)
            {
              ObRawExpr *sub_expr = func_expr->get_param_expr(1);
              if (sub_expr->get_expr_type() == T_NULL)
              {
                is_null = true;
              }
              else if(sub_expr->get_expr_type() == T_INT)
              {
                ObConstRawExpr *c_expr = dynamic_cast<ObConstRawExpr *>(sub_expr);
                const ObObj &value = c_expr->get_value();
                value.get_int(pos);
                is_use = true;
              }
            }
            int64_t len = 0;
            if (func_expr->get_param_size() >= 3)
            {
              ObRawExpr *sub_expr = func_expr->get_param_expr(2);
              if (sub_expr->get_expr_type() == T_NULL)
              {
                is_null = true;
              }
            ...
            if (is_null)
            {
              func_expr->set_length(4);
            }
            else if (!is_use)
            {
              func_expr->set_length(1);
            }
            else
            {
              ObExprObj expr_obj;
              TypeDesc result_type;
              ret = expr_obj.calc(type, OP_SUBSTR, result_type, 0, pos, len);
              if (ret == OB_SUCCESS)
              {
                func_expr->set_length(result_type.length_);
              }
            }
          }
        }
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_CAST), func_name.ptr(), func_name.length()))
        {
          ...
          if (num == 2)
          {
            ...
            if (NULL != param_expr)
            {
              ...
              {
                YYSYS_LOG(WARN, "fail to get int val. obj.get_type()=%d", obj.get_type());
                break;
              }
            }
            else
            {
              YYSYS_LOG(WARN, "fail to get param expression");
              break;
            }
          }
          else
          {
            YYSYS_LOG(WARN, "CAST function must only take 2 params");
          }
        }
        ...
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_TRIM), func_name.ptr(), func_name.length()))
        {
          ...
          if(logical_plan->get_is_show_view())
          {
            int32_t index = func_expr->get_param_size() - 1;
            ObRawExpr *sub_expr = NULL;
            sub_expr = func_expr->get_param_expr(index);
            if(sub_expr)
            {
              func_expr->set_length(sub_expr->get_length());
            }
          }
        }
        ...
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_ROUND), func_name.ptr(), func_name.length()))
        {
          ...
          if(logical_plan->get_is_show_view())
          {
            if (func_expr->get_param_size() == 2)
            {
              ObRawExpr *expr1 = func_expr->get_param_expr(0);
              ObRawExpr *expr2 = func_expr->get_param_expr(1);
              OB_ASSERT(expr1 && expr2);
              if(expr2->get_expr_type() == T_INT)
              {
                int64_t digit = 0;
                ObConstRawExpr *c_expr = dynamic_cast<ObConstRawExpr *>(expr2);
                const ObObj &val = c_expr->get_value();
                val.get_int(digit);
                TypeDesc type[2];
                TypeDesc result_type;
                type[0].init(expr1->get_precision(), expr1->get_scale(), expr1->get_length(), expr1->get_result_type());
                ObExprObj expr_obj;
                ret = expr_obj.calc(type, OP_ROUND, result_type, digit);
                if(ret == OB_SUCCESS)
                {
                  func_expr->set_precision(result_type.precision_);
                  func_expr->set_scale(result_type.scale_);
                }
              }
            }
          }
        ...
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_HEX), func_name.ptr(), func_name.length()))
        {
          ...
          if(logical_plan->get_is_show_view())
          {
            func_expr->set_length(16); //max value:8000 0000 0000 0001/7fff ffff ffff ffff
          }
        }
        ...
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_INT_TO_IP), func_name.ptr(), func_name.length()))
        {
          ...
          if(logical_plan->get_is_show_view())
          {
            func_expr->set_length(15);//255.255.255.255
          }
        }
        else if (0 == strncasecmp(oceanbase::sql::ObPostfixExpression::get_sys_func_name(SYS_FUNC_GREATEST), func_name.ptr(), func_name.length()))
        {
...

        {
          ...
          //            {
          //              ret = OB_ERR_PARSER_SYNTAX;
          //              YYSYS_LOG(WARN,"Fail to get column item!");
          //            }
          //            else
          //            {
    ...
    case T_SEQUENCE_EXPR:
    {
      ...
      if (expr_scope_type == T_INSERT_LIMIT && stmt->get_stmt_type() == ObStmt::T_INSERT)//insert
      {
        ...
        else//normal
        {
          ...
          //          YYSYS_LOG(ERROR, "the value is ::[%ld]",node->children_[0]->value_);
          if (oceanbase::common::PREV_TYPE == node->children_[0]->value_)//prvval set false
          {
            is_next_type = false;
            //            YYSYS_LOG(ERROR,"current is preval!");
          }
    ...
    uint64_t& expr_id)
{
  ...
  {
    YYSYS_LOG(WARN, "Logical Plan is empty");
  }
  ...
  if (node != NULL)
  {
    if ((sql_expr = create_middle_sql_raw_expr(
           ...
           *node,
           expr_id)) == NULL)
    {
      YYSYS_LOG(WARN, "Create middle sql raw expr failed");
    }
    ...
    else
    {
      ...
      {
        ret = OB_ERR_PARSER_MALLOC_FAILED;
        YYSYS_LOG(WARN, "Create row_count expression failed");
      }
      else
      {
  ...
  if (node != NULL)
  {
      ...
      if(node->type_ == T_FUN_LISTAGG)
      {
        if(node->num_child_>=2)
        {
          if(1 == node->children_[1]->num_child_)
          {
            ...
            {
              YYSYS_LOG(ERROR, "resolve expr_list failed");
            }
          }
          else if(2 == node->children_[1]->num_child_)
          {
            if(T_STRING == node->children_[1]->children_[1]->type_)
            {
              if(OB_SUCCESS != (ret = resolve_expr(result_plan, select_stmt, node->children_[1]->children_[0], sql_expr, sub_expr, T_AGG_LIMIT)))
              {
                YYSYS_LOG(ERROR, "resolve first parameter failed,ret=%d",ret);
              }
              ...
              {
                YYSYS_LOG(ERROR, "resolve expr_list failed,ret=%d",ret);
              }
              if(OB_SUCCESS == ret)
              {
            ...
            else
            {
              ...
              YYSYS_LOG(ERROR, "the delimeter's type must T_STRING',ret=%d",ret);
            }
          }
          else
          {
            ret = OB_ERR_PARAM_SIZE;
            YYSYS_LOG(ERROR, "wrong param number for listagg");
          }
          if(OB_SUCCESS == ret&& node->num_child_==3)
          {
            ...
            {
              YYSYS_LOG(ERROR, "resolve order clause for listagg failed");
            }
          }
        }
        else
    ...
    if (ret == OB_SUCCESS && CREATE_RAW_EXPR(agg_expr, ObAggFunRawExpr, result_plan) != NULL)
    {
      ...
      if(logical_plan->get_is_show_view())
      {
        ...
        {
          agg_expr->set_length(sub_expr->get_length());
          agg_expr->set_precision(sub_expr->get_precision());
          agg_expr->set_scale(sub_expr->get_scale());
        }
        if(node->type_ == T_FUN_SUM || node->type_ == T_FUN_LISTAGG || node->type_ == T_FUN_AVG)
        {
          agg_expr->set_precision(38);
          agg_expr->set_length(0);
        }
      }
...
{
  int& ret = result_plan->err_stat_.err_code_ = OB_SUCCESS;
  OB_ASSERT(node->num_child_ == 4);

  if (ret == OB_SUCCESS && node->children_[1])
  {
    ...
    if (table_node->type_ != T_RELATION)
    {
      ret = OB_ERR_PARSER_SYNTAX;
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
               "Only single base table is supported for partition calc");
    }
    else
    {
      ret = resolve_table(result_plan, select_stmt, table_node, table_id);
      if (ret == OB_SUCCESS)
      {
        select_stmt->set_partition_cal_table_id(table_id);
      }
    }
  }
  ...
  {
    ParseNode *version_node = node->children_[2];
    if (version_node->type_ != T_INT)
    {
      ret = OB_ERR_PARSER_SYNTAX;
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
               "partition calc func's version flag only be int type(-1, 0, 1)");
    }
    else
    {
      select_stmt->set_partition_cal_version_flag(version_node->value_);
    }
  }

  if (ret == OB_SUCCESS && node->children_[3])
  {
    ParseNode *val_node = node->children_[3];
    uint64_t expr_id = OB_INVALID_ID;
    if (val_node->type_ != T_VALUE_VECTOR)
    {
      ret = OB_ERR_PARSER_SYNTAX;
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
               "check partition calc func's partition value");
    }
    else
    {
      for (int64_t i = 0; ret == OB_SUCCESS && i < val_node->num_child_; i++)
      {
        ret = resolve_independ_expr(result_plan, select_stmt, val_node->children_[i], expr_id, T_NONE_LIMIT);
        if (ret == OB_SUCCESS && (ret = select_stmt->add_partition_cal_values(expr_id)) != OB_SUCCESS)
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "Can not add expr_id to select_stmt's partition_expr");
        }
      }
    }
  }
...
int resolve_joined_table(
    ...
    ParseNode* node,
    JoinedTable& joined_table,
    JoinedTableInfo *joined_table_info)
{
  int& ret = result_plan->err_stat_.err_code_ = OB_SUCCESS;
  ...
  for (uint64_t i = 1; ret == OB_SUCCESS && i < 3; i++)
  {
    ...
    switch (table_node->type_)
    {
      ...
      case T_ALIAS:
        ...
        if(ret == OB_SUCCESS)
        {
          joined_table_info->tid_[i-1] = tid;
        }
        break;
      case T_JOINED_TABLE:
        if(ret == OB_SUCCESS)
        {
          JoinedTableInfo *mem_ptr = NULL;
          if (NULL == (mem_ptr = (JoinedTableInfo *) parse_malloc(sizeof(JoinedTableInfo), result_plan->name_pool_)))
          {
            ret = OB_ERR_PARSER_MALLOC_FAILED;
            YYSYS_LOG(WARN, "out of memory");
            snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Can not malloc space for JoinedTableInfo");
            break;
          }
          joined_table_info->child_[i-1] = new (mem_ptr) JoinedTableInfo();
        }
        ret = resolve_joined_table(result_plan, select_stmt, table_node, joined_table, joined_table_info->child_[i-1]);
        break;
      default:
        /* won't be here */
        ret = OB_ERR_PARSER_MALLOC_FAILED;
  ...
  if (ret == OB_SUCCESS)
  {
    switch (node->children_[0]->type_)
    {
      case T_JOIN_FULL:
        ...
        joined_table_info->type_ = JoinedTable::T_FULL;
        break;
      case T_JOIN_LEFT:
        ret = joined_table.add_join_type(JoinedTable::T_LEFT);
        joined_table_info->type_ = JoinedTable::T_LEFT;
        break;
      case T_JOIN_RIGHT:
        ret = joined_table.add_join_type(JoinedTable::T_RIGHT);
        joined_table_info->type_ = JoinedTable::T_RIGHT;
        break;
      case T_JOIN_INNER:
        ret = joined_table.add_join_type(JoinedTable::T_INNER);
        joined_table_info->type_ = JoinedTable::T_INNER;
        break;
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        if (OB_SUCCESS != (ret = joined_table_info->add_join_exprs(join_exprs)))
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "add join condition error");
        }
      }
    }
  }
    ...
    switch (table_node->type_)
    {
      ...
      case T_RELATION:
      {
        ...
        if (logical_plan == NULL)
        {
          ...
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Wrong invocation of YaoStmt::add_table_item, logical_plan must exist!!!");
        }
        else
        {
          is_create_view = logical_plan->get_is_create_view();
        }
        if(ret == OB_SUCCESS && !is_create_view)
        {
          ObSchemaChecker *schema_checker = NULL;
          if (NULL == (schema_checker = static_cast<ObSchemaChecker *>(result_plan->schema_checker_)))
          {
          ...
          else
          {
            ...
            ObString dt_name;
            int64_t buf_len = OB_MAX_COMPLETE_TABLE_NAME_LENGTH;
            dt_name.assign_buffer(buf, static_cast<int32_t>(buf_len));

            if(OB_SUCCESS != (ret = write_db_table_name(db_name, table_name, dt_name)))
            {
              snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Table name too long");
            }
            const ObTableSchema *table_schema = schema_checker->get_table_schema(dt_name);
            if(table_schema != NULL && table_schema->get_type() == ObTableSchema::VIEW)
            {
              if(stmt->get_stmt_type() != ObBasicStmt::T_SELECT)
              {
                ret = OB_NOT_SUPPORTED;
                snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "not support");
                break;
              }

              const char *text = table_schema->get_text();
              YYSYS_LOG(DEBUG, "view[%s], view_difinition=[%s]", table_schema->get_table_name(), text);
              const char *view_name = table_schema->get_table_name();
              if ((ret = logical_plan->get_is_recursive_view(view_name)) == true)
              {
                ret = OB_ERR_INCLUDE_RECURSION_VIEW;
                snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                         "'%.*s' contains view recursion", dt_name.length(), dt_name.ptr());
              }
              else if((ret = logical_plan->add_view_info(view_name, stmt->get_query_id())) != OB_SUCCESS)
              {
                snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "add view info failed");
              }
              ...
              else if (0 != (ret = parse_init(&parse_result)))
              {
                ret = OB_ERR_PARSER_INIT;
                YYSYS_LOG(WARN, "parse init err, err=%s", strerror(errno));
              }
              ...
              else
              {
                ...
                if (alias_node)
                {
                  alias_name.assign_ptr((char *)(alias_node->str_value_),
                  static_cast<int32_t>(strlen(alias_node->str_value_)));
                }
                ...
                if(ret == OB_SUCCESS)
                {
                  ret = logical_plan->remove_view_info(view_name, stmt->get_query_id());
                  if(ret != OB_SUCCESS)
                  {
                    snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "remove view info failed");
                  }
                }
              }
              break;
      ...
      case T_SELECT:
      {
        ...
        if (ret == OB_SUCCESS)
        {
          ...
          if (alias_node->value_ == 0 && OB_SUCCESS == ret)
          {
            select_stmt->set_real_alias_flag(table_id);
          }
        }
        break;
      }
      case T_JOINED_TABLE:
      {
        ...
        JoinedTableInfo *joined_table_info = NULL;
        if (NULL == (joined_table_info = (JoinedTableInfo *) parse_malloc(sizeof(JoinedTableInfo), result_plan->name_pool_)))
        {
          ret = OB_ERR_PARSER_MALLOC_FAILED;
          YYSYS_LOG(WARN, "out of memory");
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Can not malloc space for JoinedTableInfo");
        ...
        joined_table_info = new (joined_table_info) JoinedTableInfo();
        joined_table_info->set_joined_tid(table_id);
        ret = select_stmt->add_joined_table_info(joined_table_info);
        if(ret != OB_SUCCESS)
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "can not add JoinedTableInfo");
        }
        ret = resolve_joined_table(result_plan, select_stmt, table_node, *joined_table, joined_table_info);
        if (ret != OB_SUCCESS)
          break;
    ...
    ParseNode* node)
{
  ...
  if (node)
  {
    ...
    {
      ret = OB_ERR_LOGICAL_PLAN_FAILD;
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "logical_plan is null");
    }
    else
    {
      logical_plan->set_is_create_view(false);
    }
  }
  return ret;
}
    ...
    int64_t num_columns)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      const ObTableSchema *table_schema = schema_checker->get_table_schema(table_item.ref_id_);
      if(table_schema != NULL && table_schema->get_type() == ObTableSchema::VIEW)
      {
        ObSQLSessionInfo *session_info = static_cast<ObSQLSessionInfo *>(result_plan->session_info_);
        if (session_info == NULL)
    ...
    else if (table_item.type_ == TableItem::GENERATED_TABLE || is_view)
    {
      if (!is_view)
      {
        ref_id = table_item.ref_id_;
      }
      ObSelectStmt* sub_select = static_cast<ObSelectStmt*>(logical_plan->get_query(ref_id));
      if (sub_select == NULL)
      {
        ret = OB_ERR_ILLEGAL_ID;
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
      ...
      else
      {
        ...
        for (int32_t i = 0; ret == OB_SUCCESS && i < num && (num_columns <= 0 || i < num_columns); i++)
        {
          ...
          if (stmt->get_stmt_type() == ObStmt::T_SELECT && num_columns <= 0)
          {
            ...
            {
              snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                       "Can not add YaoSqlRawExpr to logical plan");
              break;
            }
      ...
      if (NULL != column && column_size > 0)
      {
        ...
        for (int32_t i = 0; ret == OB_SUCCESS && i < column_size && (num_columns <= 0 || i < num_columns); i++)
        {
          ...
          if (column_item == NULL)
          {
            ...
            if (logical_plan->get_is_show_view())
            {
              new_column_item.data_precision_ = column[i].get_precision();
              new_column_item.data_scale_ = column[i].get_scale();
              new_column_item.data_length_ = static_cast<uint32_t>(column[i].get_size());
            }
            ret = stmt->add_column_item(new_column_item);
            if (ret != OB_SUCCESS)
            {
          ...
          if (stmt->get_stmt_type() == ObStmt::T_SELECT && num_columns <= 0)
          {
            ...
            if (logical_plan->get_is_show_view())
            {
              expr->set_length(column_item->data_length_);
              expr->set_precision(column_item->data_precision_);
              expr->set_scale(column_item->data_scale_);
            }
            ObSqlRawExpr* sql_expr = (ObSqlRawExpr*)parse_malloc(sizeof(ObSqlRawExpr), result_plan->name_pool_);
            if (sql_expr == NULL)
            {
    ...
    ParseNode* node)
{
  ...
  for (int32_t i = 0; ret == OB_SUCCESS &&i < node->num_child_; i++)
  {
    ...
    const int64_t buf_len = OB_MAX_COMPLETE_TABLE_NAME_LENGTH + OB_MAX_COLUMN_NAME_LENGTH +10;
    char buf[buf_len];
    ObString raw_column_name;
    raw_column_name.assign_buffer(buf, static_cast<int32_t>(buf_len));
    if(project_node->type_ == T_ALIAS)
    {
      raw_column_name.clone(expr_name);
    }
    ...
    if (project_node->type_ == T_ALIAS)
    {
      ...
      // {
      //   YYSYS_LOG(ERROR, "alias name %.s is ambiguous", alias_node->str_value_);
      //   return false;
      // }
    }
    ...
    if (OB_SUCCESS == ret && project_node->type_ == T_PARTITION_CALC)
    {
      if (node->num_child_ != 1 || project_node->num_child_ != 4)
      {
        YYSYS_LOG(WARN, "failed to resolve partition_calc function ");
        ret = OB_ERR_PARSE_SQL;
      }
      else if (project_node->children_[0] && strcasecmp(project_node->children_[0]->str_value_, "partition_calc") == 0)
      {
        select_stmt->set_is_partition_calc_func(true);
        if ((ret = resolve_partition_calc_func(result_plan, select_stmt, project_node)) != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "failed to resolve partition_calc function ");
        }
      }
      else
      {
        ret = OB_ERR_PARSER_SYNTAX;
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                 "Invalid func name of partition_calc function ");
      }
      break;
    }
    ...
    else if (is_real_alias == true &&
             OB_SUCCESS != (ret = select_stmt->add_raw_column(expr_id, raw_column_name)))
    {
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "add raw column error");
      break;
    }
    alias_node = NULL;
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS && node->children_[1] != NULL)
  {
    bool is_partition_calc = false;
    ParseNode* project_node = node->children_[1];
    ParseNode* project_child_node = NULL;
    for(int32_t i = 0; i < project_node->num_child_; i++)
    {
      project_child_node = project_node->children_[i]->children_[0];
      if (project_child_node->type_ == T_PARTITION_CALC)
      {
        is_partition_calc = true;
        break;
      }
    }
    if (is_partition_calc)
    {
      if (project_node->num_child_ > 1)
      {
        ret = OB_ERR_PARSE_SQL;
        YYSYS_LOG(WARN, "sql parser error, only support select partition_cal()");
      }
      ...
      for(int32_t i = 2; ret == OB_SUCCESS && i < node->num_child_; i++)
      {
  ...
  else if (node->children_[6] != NULL)
  {
    ...
    bool is_create_view = logical_plan->get_is_create_view();
    UNUSED(is_create_view);
    // assign first query
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      else
      {
        ...
        for (int32_t i = 0; i < left_select->get_select_item_size(); i ++)
        {
          ...
          {
            YYSYS_LOG(ERROR,"columns[%d] type are not compatible", i + 1);
  ...
  else
  {
    ...
    if (ret == OB_SUCCESS && select_stmt->get_joined_table_size()>0)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR,"join filter ERROR!");
      }
    }
    //add 20140409:e
  }
  ...
  if (node)
  {
    ...
    for (int32_t i = 0; i < node->num_child_; i++)
    {
      ...
      switch (hint_node->type_)
      {
        ...
        case T_READ_ATOMIC_WEAK:
          query_hint.read_atomic_level_ = READ_ATOMIC_WEAK;
          break;
        case T_READ_ATOMIC_STRONG:
          query_hint.read_atomic_level_ = READ_ATOMIC_STRONG;
          break;
          //add duyr 20151224:e
        ...
        case T_READ_CONSISTENCY:
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "unknown hint value, ret=%d", ret);
          }
          break;
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151224:b
    if (OB_SUCCESS == ret && NO_READ_ATOMIC_LEVEL != query_hint.read_atomic_level_)
    {
      //must read data from master ups!
      query_hint.read_consistency_  = STRONG;
    }
    //add duyr 20151224:e
  }
    ...
    ParseNode* hint_node)
{
  ...
  /*if(query_hint.use_index_array_.size() >= 1)
  {
    ...
    YYSYS_LOG(ERROR, "too much index hint, ret=[%d]", ret);
    return ret;
  }
 */
  ...
  else
  {
    ...
    YYSYS_LOG(ERROR, "unknown table name '%s', ret=[%d]", hint_node->children_[0]->children_[1]->str_value_, ret);
    return ret;
  }
  // ������������table_id���ж����Ƿ�������������������жϴ���������ԭ���Ƿ��������ԭ����һ�µ�
  ...
  if(NULL != (index_table_schema = schema_checker->get_table_schema(t_index_name)))
  {
    ...
    if(OB_INVALID_ID == index_src_tid || pair.src_table_id_ != index_src_tid)
    {
      ...
      YYSYS_LOG(ERROR, "unknown index name '%s' of '%s', index table name '%s', ret=[%d]",
  ...
  else
  {
    ...
    YYSYS_LOG(ERROR, "unknown index name '%s', index table name '%s', ret=[%d]",
    ...
    ParseNode* hint_node)
{
  ...
  if(query_hint.join_array_.size() >= 1)
  {
    ...
    YYSYS_LOG(ERROR, "too much join hint, ret=[%d]", ret);
    ...
    ObUpdateStmt *update_stmt)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument. update_stmt=%p, result_plan=%p", update_stmt, result_plan);
    ret = OB_INVALID_ARGUMENT;
  }
  ...
  if (node)
  {
    ...
    for (int32_t i = 0; ret == OB_SUCCESS && i < node->num_child_; i++)
    {
      ...
      for (int32_t j = 0; ret == OB_SUCCESS && j < vector_node->num_child_; j++)
      {
        ...
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "Can not add expr_id to YaoArray");
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        if ((ret = select_stmt->add_value_row(value_row)) != OB_SUCCESS)
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "Add value-row to YaoInsertStmt error");
      }
      value_row.clear();
    }
  }
...
int resolve_insert_values(
    ...
    ObInsertStmt* insert_stmt,
    ParseNode* node,
    ObSchemaChecker *schema_checker, ObStringBuf *name_pool)
{
  ...
  for (int32_t i = 0; ret == OB_SUCCESS && i < insert_stmt->get_column_size(); i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "column item not found!index[%d]", i);
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "array push back failed! column_id_ = %ld", col_item->column_id_);
      }
    }
  }
  //add e

  //node->num_child_: values����()�Ķ���
  for (int32_t i = 0; ret == OB_SUCCESS && i < node->num_child_; i++)
  {
    ParseNode* vector_node = node->children_[i];
    uint64_t expr_id = OB_INVALID_ID;
    //vector_node->num_child_: values����()�б����ĸ���
    int32_t value_num = vector_node->num_child_;
    int32_t col_num = static_cast<int32_t>(col_id.count());
    if (col_num != value_num)
    {
      ret = OB_ERR_COLUMN_SIZE;
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "too %s value, value num[%d], column num[%d]",
               col_num > value_num ? "few" : "many", col_num, value_num);
    }
    ...
    for (int32_t j = 0; ret == OB_SUCCESS && j < vector_node->num_child_; j++)
    {
      ...
      const ObColumnSchemaV2 *col_sche = NULL;
      schema_checker = static_cast<ObSchemaChecker*>(result_plan->schema_checker_);
      if (NULL != (col_sche = schema_checker->get_column_schema(table_id, cid)))
      {
        if (ObVarcharType == (col_sche->get_type()))
        {
          col_info.varchar_size_ = col_sche->get_size();
        }
      }

      if (vector_node->children_[j]->type_ == T_DEFAULT)
      {
        ObColumnSchemaV2 *column_schema =
            const_cast<ObColumnSchemaV2 *>(schema_checker->get_column_schema(table_id, (uint64_t)cid));
        if (column_schema == NULL)
        {
          ret = OB_ERROR;
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "Get column schema error");
        }
        else
        {
          ...
          {
            ObObj default_obj;
            column_schema->calc_default_val_obj(default_obj);
            ParseNode *default_value_node = (ParseNode *) parse_malloc(sizeof(ParseNode), name_pool);
            memset(default_value_node, 0, sizeof(ParseNode));
            default_value_node->num_child_ = 0;
            if (OB_SUCCESS == (ret = resolve_default_node(result_plan, default_value_node, default_obj, name_pool)))
            {
              ret = resolve_independ_expr(result_plan, insert_stmt, default_value_node,
                                          expr_id, T_INSERT_LIMIT);
            }
            else
            {
              YYSYS_LOG(ERROR, "resolve default node error, table_id=[%ld], column_id=[%ld]", table_id, cid);
            }
          }
          else
          {
      ...
      //add lijianqiang [sequence] 20150402 :b
      if (insert_stmt->current_expr_has_sequence())//��sequence��������õ���Ϣ�洢����
      {
        if (ret == OB_SUCCESS && (ret = insert_stmt->add_next_prev_ids()))
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "Can not add next_pre_ids to YaoArray");
        }
        insert_stmt->set_current_expr_has_sequence(false);//���ã��´�ʹ��
    ...
    ObStringBuf *name_pool)
{
  ...
  char temp2[OB_MAX_DEBUG_MSG_LEN];
  ...
  switch (default_value.get_type())
  {
    case ObIntType:
      node->type_ = T_INT;
      default_value.get_int(value);
      node->value_ = value;
      break;
    case ObPreciseDateTimeType:
      default_value.get_varchar_for_precise_datetime(str);

      format_ret = varchar_timestamp_format(str.ptr(), 6, "%4d-%2d-%2d %2d:%2d:%2d", 1);

      if (format_ret != OB_SUCCESS)
      {
        node->type_ = T_DATE;
        node->value_ = atol(str.ptr());
      }
      else
      {
        node->type_ = T_STRING;
        node->str_value_ = str.ptr();
      }
      break;
    case ObDateType:
      default_value.get_varchar_for_date(str);
      format_ret = varchar_timestamp_format(str.ptr(), 3, "%4d-%2d-%2d", 2);
      if (format_ret != OB_SUCCESS)
      {
        node->type_ = T_DATE_NEW;
        node->value_ = atol(str.ptr());
      }
      else
      {
      ...
      break;
    case ObTimeType:
      default_value.get_varchar_for_time(str);
      format_ret = varchar_timestamp_format(str.ptr(), 3, "%2d:%2d:%2d", 3);
      if (format_ret != OB_SUCCESS)
      {
        node->type_ = T_TIME;
        node->value_ = atol(str.ptr());
    ...
    case ObVarcharType:
      node->type_ = T_STRING;
      default_value.get_varchar(str);
      node->str_value_ = str.ptr();
      break;
    case ObDecimalType:
      node->type_ = T_DECIMAL;
      default_value.get_decimal(str);
      node->str_value_ = str.ptr();
      break;
    case ObFloatType:
      node->type_ = T_FLOAT;
      default_value.get_float(fval);
      snprintf(temp, sizeof(temp), "%f", fval);
      if ((ret = ob_write_string(*name_pool,
                                 ObString::make_string(temp),
                                 str)) != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "Can not malloc space for default value");
        break;
      }
      node->str_value_ = str.ptr();
      break;
    case ObDoubleType:
      node->type_ = T_DOUBLE;
      default_value.get_double(dval);
      snprintf(temp2, sizeof(temp2), "%lf", dval);
      if ((ret = ob_write_string(*name_pool,
                                 ObString::make_string(temp2),
                                 str)) != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "Can not malloc space for default value");
        break;
    ...
    ObStringBuf *name_pool)
{
  ...
  uint64_t insert_query_id = insert_stmt->get_insert_query_id();
  bool has_insert_query = false;
  if (insert_query_id != OB_INVALID_ID)
  {
    has_insert_query = true;
  }

  uint64_t table_id = insert_stmt->get_table_id();
  int32_t column_size = insert_stmt->get_column_size();
  ObVector<ColumnItem> &column_items = insert_stmt->get_column_items();

  if (ret == OB_SUCCESS)
  {
    ObColumnSchemaV2 *columns = NULL;
    int32_t size = 0;
    columns = const_cast<ObColumnSchemaV2 *>(schema_checker->get_table_columns(table_id, size));
    if (column_size >= size)
    {
    }
    else if (NULL == columns)
    {
      YYSYS_LOG(WARN, "can not get column schema");
    }
    else if (size > 0)
    {
      for (int32_t i = 0; i < size; i++)
      {
        ...
        {
          uint64_t column_id = columns[i].get_id();
          if (column_id == OB_INVALID_ID)
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "column id in column schema is invalid");
            break;
          }
          ...
          {
            ColumnItem column_item = column_items.at(j);
          ...
          {
            ObObj default_obj;
            columns[i].calc_default_val_obj(default_obj);

            ObString column_name;
            column_name.assign_ptr(
                  (char *)(columns[i].get_name()),
                  static_cast<int32_t>(strlen(columns[i].get_name())));
            if ((ret = insert_stmt->add_column_item(*result_plan, column_name)) != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "add column item failed, ret=[%d]", ret);
              break;
            }

            if (ret == OB_SUCCESS)
            {
              int64_t value_row_size = insert_stmt->get_value_row_size();
              ObObj default_value = default_obj;
              uint64_t expr_id = OB_INVALID_ID;

              ParseNode *node = (ParseNode *) parse_malloc(sizeof(ParseNode), name_pool);
              memset(node, 0, sizeof(ParseNode));
              node->num_child_ = 0;

              ret = resolve_default_node(result_plan, node, default_value, name_pool);
              if (ret != OB_SUCCESS)
              {
                YYSYS_LOG(ERROR, "get default value error");
              }
              else
              {
                if ((ret = resolve_independ_expr(
                       result_plan, insert_stmt, node, expr_id, T_INSERT_LIMIT)) == OB_SUCCESS)
                {
                  if (has_insert_query == false)
                  {
                    for (int64_t k = 0; k < value_row_size; ++k)
                    {
                      ObArray<uint64_t> &value_row =
                          const_cast<ObArray<uint64_t> &>(insert_stmt->get_value_row(k));
                      if (ret == OB_SUCCESS && (ret = value_row.push_back(expr_id)) != OB_SUCCESS)
                      {
                        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                                 "Can not add expr_id to YaoArray");
                      ...
                      if (insert_stmt->get_column_size() != static_cast<int32_t>(value_row.count()))
                      {
                        ret = OB_ERR_COLUMN_SIZE;
                        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                                 "Column count doesn't match value count");
                  ...
                  {
                    ObSelectStmt* select_stmt = static_cast<ObSelectStmt*>(logical_plan->get_query(insert_query_id));
                    if (select_stmt == NULL)
                    {
                      ret = OB_ERR_ILLEGAL_ID;
                      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                    ...
                    new_select_item.expr_id_ = expr_id;
                    new_select_item.expr_name_ = ObString::make_string(columns[i].get_default_val());
                    new_select_item.type_ = default_value.get_type();
                    new_select_item.is_real_alias_ = false;
                    select_items.push_back(new_select_item);
                  }
                }
                else
                {
                  YYSYS_LOG(ERROR, "add default value error");
                  break;
                }
              }
            }
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                 "Can not add YaoInsertStmt to logical plan");
      }
      else
      {
        ObSchemaChecker* schema_checker = static_cast<ObSchemaChecker*>(result_plan->schema_checker_);
        ...
        else
        {
          ...
          if (ret == OB_SUCCESS)
          {
            ...
            if (node->children_[2])
            {
              ret = resolve_insert_values(result_plan, insert_stmt, node->children_[2], schema_checker, name_pool);
              if (ret == OB_SUCCESS)
              {
                ret = resolve_insert_default(result_plan, logical_plan, insert_stmt, schema_checker, name_pool);
              }
              else
              {
                YYSYS_LOG(WARN, "resolve insert values error, ret[%d]", ret);
              }
            }
            else
            {
              ...
              if (ret == OB_SUCCESS)
              {
                ...
                {
                  YYSYS_LOG(WARN, "resolve insert select stmt error");
                }
              }
            }
          }
    ...
    uint64_t& query_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR, "out of memory for insert_stmt");
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
               "Can not malloc YaoInsertStmt");
    }
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR, "out of memory for seq_create_stmt");
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
               "Can not malloc YaoSequenceStmt");
    }
    else
    {
      insert_stmt = new(insert_stmt) ObInsertStmt(name_pool);
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "Can not add expr_id to YaoArray");
      ...
      {
        YYSYS_LOG(ERROR, "Create sequence failed.");
      }
      if (OB_SUCCESS == ret
      ...
      {
        ret = OB_ERR_COLUMN_SIZE;
        YYSYS_LOG(ERROR, "Column count doesn't match value count.");
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Create sequence failed.");
      }
      if (OB_SUCCESS == ret)
      {
    ...
    ParseNode* node)
{
  ...
  {
    YYSYS_LOG(ERROR, "Initialize default value of sequence failed.");
  }
      ...
      switch (opt_node->type_)
      {
        ...
        case T_TYPE_BIG_INTEGER:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Resolve datatype expr failed.");
            }
          }
          else
          {
      ...
      switch (opt_node->type_)
      {
        ...
        case T_SEQUENCE_START_WITH:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Construction decimal value with StartWith failed.");
              break;
            }
            ...
            {
              YYSYS_LOG(ERROR, "Resolve START WITH failed, ret = %d", ret);
            }
          }
          else
          {
        ...
        case T_SEQUENCE_INCREMENT_BY:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Construction decimal value with IncrementBy failed.");
              break;
            }
            else if (OB_SUCCESS == (ret = resolve_independ_expr(result_plan, insert_stmt, opt_node->children_[0], expr_id, T_INSERT_LIMIT)))
            {
            ...
            {
              YYSYS_LOG(ERROR, "Resolve INCREMENT BY failed, ret = %d", ret);
            }
          }
          else
          {
        ...
        case T_SEQUENCE_MINVALUE:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            if (1 == opt_node->num_child_ && NULL != opt_node->children_[0])
            {
              ...
              {
                YYSYS_LOG(ERROR, "Construction decimal value with Minvalue failed.");
                break;
              }
              else if (OB_SUCCESS == (ret = resolve_independ_expr(result_plan, insert_stmt, opt_node->children_[0], expr_id, T_INSERT_LIMIT)))
              {
              ...
              {
                YYSYS_LOG(ERROR, "Resolve MINVALUE failed, ret = %d", ret);
              }
            }
          }
          else
        ...
        case T_SEQUENCE_MAXVALUE:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            if (1 == opt_node->num_child_ && NULL != opt_node->children_[0])
            {
              ...
              {
                YYSYS_LOG(ERROR, "Construction decimal value with Maxvalue failed.");
                break;
              }
              else if (OB_SUCCESS == (ret = resolve_independ_expr(result_plan, insert_stmt, opt_node->children_[0], expr_id, T_INSERT_LIMIT)))
              {
              ...
              {
                YYSYS_LOG(ERROR, "Resolve MAXVALUE failed, ret = %d", ret);
              }
            }
          }
          else
        ...
        case T_SEQUENCE_CYCLE:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Resolve CYCLE failed, ret = %d", ret);
            }
          }
          else
          {
        ...
        case T_SEQUENCE_CACHE:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            if (1 == opt_node->num_child_ && NULL != opt_node->children_[0])
            {
              ...
              {
                YYSYS_LOG(ERROR, "Construction decimal value with Cache failed.");
              }
              else if (opt_node->children_[0]->type_ == T_OP_NEG)
              {
                ret = OB_VALUE_OUT_OF_RANGE;
                YYSYS_LOG(USER_ERROR, "The value of Cache is out of range.");
              }
              ...
              {
                YYSYS_LOG(ERROR, "Resolve CACHE failed, ret = %d", ret);
              }
            }
            else
            {
        ...
        case T_SEQUENCE_ORDER:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Resolve ORDER failed, ret = %d", ret);
            }
          }
          else
          {
        ...
        case T_SEQUENCE_QUICK:
          ...
          if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(column))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Resolve QUICK failed, ret = %d", ret);
            }
          }
          else
          {
        ...
        default:
          ret = OB_ERR_PARSER_SYNTAX;
          YYSYS_LOG(ERROR, "Wrong node type, ret = %d", ret);
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "WRONG OPTION %d", i);
          break;
      }//switch
  ...
  {
    YYSYS_LOG(ERROR, "Convert zero to decimal failed.");
  }
  for (int8_t i = 0; OB_SUCCESS == ret && i < ObSequenceStmt::OptionFlag; ++i)
  {
    if (ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(i))
    {
      ...
      switch (i)
      {
        ...
        case ObSequenceStmt::CurrentValue:
                   ...
                   && ObSequenceStmt::SequenceOption::NoSet == sequence_stmt->is_set_option(ObSequenceStmt::MaxValue))
          {
            ...
            {
              YYSYS_LOG(ERROR, "Convert default value of start with to decimal failed.");
              break;
            }
          }
      ...
      {
        YYSYS_LOG(ERROR, "Wrong enum : %d", i);
      }
      else if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "Convert to decimal failed, option number = %d", i);
      }
      ...
      {
        YYSYS_LOG(ERROR, "Resolve default value of option[%d] failed, ret = %d", i, ret);
      }
      if (OB_SUCCESS != ret)
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG, "Create sequence failed.");
    ...
    uint64_t& query_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                 "Can not add YaoSequenceStmt to logical plan");
      }
    }
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG (ERROR, "Resolve drop sequence where caluse failed.");
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    uint64_t& query_id)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      {
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                 "Can not add YaoTransStmt to logical plan");
      }
      else
      {
        ParseNode* table_node = node->children_[0];
        ...
        if (ret == OB_SUCCESS)
        {
          ...
          for (int32_t i = 0; ret == OB_SUCCESS && i < assign_list->num_child_; i++)
          {
            ...
            if (ret == OB_SUCCESS)
            {
              ...
              /* ---new: ����cinfo�����ж�varchar�ĳ����Ƿ�����schema��Ҫ��---*/
              ObColumnInfo col_info;
              col_info.table_id_ = column_item->table_id_;
              col_info.column_id_ = column_item->column_id_;
              ObSchemaChecker* schema_checker = NULL;
              const ObColumnSchemaV2 *col_sche = NULL;
              schema_checker = static_cast<ObSchemaChecker*>(result_plan->schema_checker_);
              if (NULL != (col_sche = schema_checker->get_column_schema(column_item->table_id_, column_item->column_id_)))
              {
                if (ObVarcharType == (col_sche->get_type()))
                {
                  col_info.varchar_size_ = col_sche->get_size();
              ...
              if (expr->type_ == T_DEFAULT)
              {
                ...
                {
                  ObColumnSchemaV2 *column_schema = const_cast<ObColumnSchemaV2 *>(
                                                      schema_checker->get_column_schema(column_item->table_id_, column_item->column_id_));
                  if (column_schema == NULL)
                  {
                    ret = OB_ERROR;
                    snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                  ...
                  {
                    if (column_schema->get_default_val_len() > 0)
                    {
                      ObObj default_obj;
                      column_schema->calc_default_val_obj(default_obj);
                      ParseNode *default_value_node = (ParseNode *) parse_malloc(sizeof(ParseNode), name_pool);
                      ...
                      if (OB_SUCCESS == (ret = resolve_default_node(result_plan, default_value_node, default_obj, name_pool)))
                      {
                        ret = resolve_independ_expr(result_plan, update_stmt, default_value_node,
                                                    ref_id, T_UPDATE_LIMIT);
                      }
                      else
                      {
                        YYSYS_LOG(ERROR, "resolve default node error, table_id=[%ld], column_id=[%ld]",
              ...
              if(OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "Varchar is too long! Column item:index[%d] tid[%ld], cid[%ld]",
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            YYSYS_LOG(DEBUG,"the exprs count is[%ld]",(update_stmt->get_sequence_expr_ids_size()));
            YYSYS_LOG(DEBUG,"the boundary is[%ld]",update_stmt->get_set_where_boundary());
          }
          //add 20150910:e
        }
        if (ret == OB_SUCCESS)
...
/*@author   ������
 *@brief    ���������Ż�select��ѯ�еĸ���join����
 *@brief    ��������Ϊֻ�ܱ�resolve_select_stmt��������
 *@param    result_plan �����߼��ƻ��ͱ��汨����Ϣ,result_plan->plan_tree_ ����Ϊ ObLogicalPlan*,����Ϊ ObMultiLogicPlan*
 *@param    select_stmt ����select���߼��ƻ���sql�����Ϣ
 *@return   �����Ƿ�ɹ�����Ϣ
 *@retval   OB_SUCCESS ��ʾ�ɹ�������ֵ��Ϊʧ��
 */
int optimize_joins_filter(
    ResultPlan * result_plan,
    ObSelectStmt* select_stmt)
{
  ...
  {
    YYSYS_LOG(ERROR,"function param error");
    snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
  ...
  {
    YYSYS_LOG(ERROR,"stmt type should be T_SELECT");
    snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
  ...
  for(int32_t i = 0; OB_SUCCESS == ret && i < num_table; i++)
  {
    ...
    if (from_item.is_joined_ == true)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR,"optimize join where expr error!");
        break;
      }
    }
  }
 ...
 *@author   ������
 *@brief    ��������һ������join���Ż�
 *@brief    ������ʱ����ֻ������optimize_joins_filter��������
 *@param    result_plan �����߼��ƻ��ͱ��汨����Ϣ
 *@param    select_stmt ����select���߼��ƻ���sql�����Ϣ
 *@param    joined_table ����һ������join��������Ϣ
 *@return   ���������Ƿ�ɹ�����Ϣ
 *@retval   OB_SUCCESS ��ʾ�ɹ�������ֵ��Ϊʧ��
 */
int optimize_join_where_expr(
    ...
    JoinedTable* joined_table)
{
  ...
  if (logical_plan == NULL)
  {
    ...
    YYSYS_LOG(ERROR,"logical_plan must exist!!!");
  }
  ...
  {
    /****first for******/
    //ע�������select_stmt->get_condition_size()�Ǹ���̬�仯��ֵ
    for(int32_t it = 0; OB_SUCCESS == ret && it < select_stmt->get_condition_size(); it++)
    {
      ...
      for (int64_t j = 0; OB_SUCCESS == ret && j < joined_table->join_types_.count(); j++)
      {
        ...
        for(int64_t join_index = 0; OB_SUCCESS == ret && join_index < join_expr_num; join_index++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN,"where expr and join expr don't meet optimize cnd");
            break;
          }
          ...
          {//�����Ż�����ʱ�������±���ʽ
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR,"gen new where expr failed!");
              break;
            }
          }
...
/*
 *@author   ������
 *@brief    �����ж�where_expr��join_expr�Ƿ������Ż�����
 *@brief    ��������Ϊֻ�ܱ�optimize_join_where_expr����ʹ��
 *@param    where_expr �����ѯ����
 *@param    join_expr  ������������
 *@param    old_where_exprid_store ����WHERE�Ӿ������еĲ�ѯ����
 *@param    is_meet_opti_cnd �����Ƿ������Ż�����
 *@param    ref_expr_from_join join_expr��׼�������²�ѯ�����Ĳ�����
 *@return   ���������Ƿ�ɹ�����Ϣ
 *@retval   OB_SUCCESS ��ʾ�ɹ�������ֵ��Ϊʧ��
 */
int optimize_meet_condition(
    ...
    ObBinaryRefRawExpr*& ref_expr_from_join)
{
  ...
  if (OB_SUCCESS == ret && is_meet_opti_cnd)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN,"split join_expr failed");
    }
  ...
  if (OB_SUCCESS == ret && is_meet_opti_cnd)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN,"split where_expr failed");
    }
  ...
  //fouth
  //where expr��join expr������һ�����������ڱ����������϶����
  //����join expr: A.id=B.id,ͬʱwhere expr A.id > 100 ===> ��ref_expr_from_join=B.id
  if ( OB_SUCCESS == ret && is_meet_opti_cnd )
  {
  ...
  //fifth
  //WHERE�Ӿ��У�ԭ��û��join_ref_exprʱ�ż����Ż�����
  if (OB_SUCCESS == ret && is_meet_opti_cnd && ref_expr_from_join != NULL)
  {
    ...
    for(int32_t i=0; OB_SUCCESS==ret && i< store_size; i++)
    {
      ...
      {//����ͬһ�ű��ı���ʽ
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN,"split old where_expr failed");
          break;
        }
        else if (sub_ref_exprs_store.size() >=1)
        {
...
/*
 *@author   ������
 *@brief    �������ݴ����where_expr��ref_expr_of_new_expr����һ���µı���ʽ
 *@brief    ����Ŀǰ����Ϊֻ�ܱ�optimize_join_where_expr��������
 *@param    where_expr  ����where����ʽָ��
 *@param    ref_expr_of_new_expr   �±���ʽ���е�ObBinaryRefRawExpr���͵Ľڵ�
 *@param    new_expr_id_store ���������ɵı���ʽid
 *@param    new_expr_store ���������ɵı���ʽָ��
 *@return   ���������Ƿ�ɹ�����Ϣ
 *@retval   OB_SUCCESS ��ʾ�ɹ�������ֵ��Ϊʧ��
 */
int optimize_gen_new_where_expr(
    ...
    ObVector<ObSqlRawExpr*>& new_expr_store)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {/*won't be here*/
      YYSYS_LOG(ERROR,"where expr don't meet optimize condition");
      snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (is_triple_expr)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR,"gen triple expr error");
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR,"gen binary expr error");
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
  ...
  if (OB_SUCCESS == ret && new_where_expr != NULL )
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR,"add expr into logical plan failed!");
        snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                 "Add YaoSqlRawExpr error");
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else
      {
        ...
        new_sql_expr->set_column_id(logical_plan->generate_column_id());//ϵͳ���ɵ�column_id����ͬ��schema�е�column_id
      }
      ...
      {
        YYSYS_LOG(ERROR, "negative bitmap values");
      }
    }
  }
  ...
  if (OB_SUCCESS == ret && expr_id != OB_INVALID_ID && new_sql_expr != NULL)
  {
    ...
    if (OB_SUCCESS != ( ret = new_expr_id_store.push_back(expr_id) ) )
    {
      ...
      YYSYS_LOG(ERROR,"and optimize expr into where exprs error!");
    }
    //���±���ʽ����ָ�뱣�浽new_expr_store����
    else if (OB_SUCCESS != (ret = new_expr_store.push_back(new_sql_expr)) )
    {

      snprintf(result_plan->err_stat_.err_msg_,MAX_ERROR_MSG,
               "and optimize expr into new expr stores error!");
      YYSYS_LOG(ERROR,"and optimize expr into new expr stores error!");
    }
    else
    {
      //���where expr�����������Ż�����jon����
...
/*
 *@author   ������
 *@brief    ������������ObTripleOpRawExpr���͵ı���ʽ
 *@brief    ����������ʱֻ������optimize_gen_new_where_expr��������
 *@param    old_main_triple_expr ����ʽ���ĸ��ڵ�����
 *@param    old_sub_first_ref_expr����ʽ���е�ObBinaryRefRawExpr���͵��ӽڵ�
 *@param    new_expr ���������ɵı���ʽ��ָ��
 */
    ...
    ObRawExpr*& new_expr)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"can't create ObBinaryRefRawExpr");
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"can't create ObConstRawExpr");
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"create ObTripleOpRawExpr failed");
    }
    else
    {
...
/*
 *@author   ������
 *@brief    ������������ObBinaryOpRawExpr���͵ı���ʽ
 *@brief    ����������ʱֻ������optimize_gen_new_where_expr��������
 *@param    old_main_triple_expr ����ʽ���ĸ��ڵ�����
 *@param    old_sub_ref_expr ����ʽ���е�ObBinaryRefRawExpr���͵��ӽڵ�
 *@param    sub_ref_expr_is_frist ����ʽ����ObBinaryRefRawExpr���͵Ľڵ��Ƿ�Ϊ��һ���ӽڵ�
 *@param    new_expr ���������ɵı���ʽ��ָ��
 */
int optimize_gen_binary_expr(
    ResultPlan * result_plan,
    ...
    ObRawExpr*& new_expr)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"create ObBinaryRefRawExpr failed");
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"create ObConstRawExpr failed");
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_PARSER_MALLOC_FAILED;
      YYSYS_LOG(ERROR,"create ObBinaryOpRawExpr failed");
    }
    else
    {
 ...
 *@author  ������
 *@brief   ��һ������ʽ�����зֽ⣬�����ؽڵ���Ϣ
 *@param   source_expr ���ֽ�ı���ʽ
 *@param   sub_ref_exprs_store �������ʽ����ObBinaryRefRawExpr���͵��ӱ���ʽ��ָ��
 *@param   sub_const_exprs_store �������ʽ����ObConstRawExpr���͵��ֱ���ʽ��ָ��
 *@param   is_triple_expr  ����source_expr�Ƿ�ΪObTripleOpRawExpr����
 *@param   sub_ref_expr_is_first ����source_expr��ObBinaryRefRawExpr���͵��ӱ���ʽ�Ƿ�Ϊ��һ���ӱ���ʽ
 *@return   ���������Ƿ�ɹ�����Ϣ
 *@retval   OB_SUCCESS ��ʾ�ɹ�������ֵ��Ϊʧ��
 */
int optimize_split_expr(
    ...
    if (source_expr->get_expr()->get_expr_type() == T_OP_BTW)
    {
      ...
      if (main_triple_expr->get_first_op_expr()->get_expr_type() == T_REF_COLUMN)
      {
        ...
        else if (OB_SUCCESS != (ret = sub_ref_exprs_store.push_back(sub_ref_expr)) )
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "optimize split expr error");
          YYSYS_LOG(ERROR,"Add sub_ref_expr into Vector error");
        }
        else if (OB_SUCCESS != (ret = sub_const_exprs_store.push_back(sub_first_const_expr)) )
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "optimize split expr error");
          YYSYS_LOG(ERROR,"Add sub_first_const_expr into Vector error");
        }
        else if (OB_SUCCESS != (ret = sub_const_exprs_store.push_back(sub_second_const_expr)) )
        {
          snprintf(result_plan->err_stat_.err_msg_, MAX_ERROR_MSG,
                   "optimize split expr error");
          YYSYS_LOG(ERROR,"Add sub_second_const_expr into Vector error");
        }
      }
    }
    else
    {//����ʽ��������ΪObBinaryOpRawExpr
      is_triple_expr = false;
...
//add dyr [HugeTable_Join_Filter] [JHOBv0.1] 20140512:b
//�ж�����ObBinaryRefRawExpr���͵ı���ʽ�Ƿ�Ϊͬһ�ű���ͬһ��
bool optimize_ref_expr_equal(
    ObBinaryRefRawExpr* ref_expr_1,
    ObBinaryRefRawExpr* ref_expr_2
    )
{
  ...
  {
    YYSYS_LOG(WARN,"can't use NULL expr to decide if equal");
    ret = false;
  }
...
int generate_index_table_name(ObString index_name, uint64_t src_tid, char *out_buff, int64_t& str_len)
{
  ...
  {
    YYSYS_LOG(WARN,"buff is not enouth to gen index table name");
    ret=OB_ERROR;
  }
  else
  {
...
int generate_expire_col_list(ObString input, ObStrings &out)
{
  ...
  {
    YYSYS_LOG(ERROR,"generate_expire_col_list parse error,ret[%d]",ret);
  }
  else
  {
