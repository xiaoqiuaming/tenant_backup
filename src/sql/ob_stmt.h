...
namespace oceanbase
{
  ...
  namespace sql
  {
    struct ObQueryHint
    {
      ObQueryHint()
      {
        ...
        read_atomic_level_= common::NO_READ_ATOMIC_LEVEL;
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151224:b
      common::ObReadAtomicLevel     read_atomic_level_;
    ...
    struct ObColumnInfo
    {
      uint64_t table_id_;
      uint64_t column_id_;
      int64_t varchar_size_;
      ObColumnInfo()
      {
        table_id_ = common::OB_INVALID_ID;
        column_id_ = common::OB_INVALID_ID;
        varchar_size_ = INT64_MAX;
      }
    };
  }
  ...
  namespace sql
  {
    ...
    public:
      ...
      int32_t get_when_fun_size() const
      { return when_func_ids_.size(); }
      int32_t get_when_expr_size() const
      { return when_expr_ids_.size(); }
      ...
      ParseNode* get_ud_where_parse_tree()
      {return ud_where_parse_tree_;};
      void set_ud_where_parse_tree(ParseNode* ud_where_parse_tree)
      {ud_where_parse_tree_ = ud_where_parse_tree;};
      ...
      int check_table_column(
          ...
          uint32_t &data_precision_,
          uint32_t &data_scale_,
          bool is_logical_optimizer = false);
      int add_table_item(
          ResultPlan& result_plan,
          ...
          bool is_logical_optimizer = false
          ,const uint64_t view_id = OB_INVALID_ID
              );
      ...
      int change_table_item_tid(uint64_t& id)
      {
        ...
        else
        {
          ...
          if ((ret = tables_hash_.add_column_desc(table_item->table_id_, OB_INVALID_ID)) != OB_SUCCESS)
            YYSYS_LOG(ERROR, "Can not add table_id to hash table");
        }
        return ret;
      }
      int add_column_item(
      ...
      int set_condition_id(uint64_t index)
      {
          return where_expr_ids_.push_back(index);
      }

      int set_real_alias_flag(uint64_t table_id);
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] 20160120:b
      ...
      void print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level);
