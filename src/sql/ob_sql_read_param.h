...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      virtual int serialize_stmt_type(char* buf, const int64_t buf_len,int64_t& post,ObBasicStmt::StmtType stmt_type) const;
      virtual int deserialize_stmt_type(const char* buf, const int64_t data_len,int64_t& pos,ObBasicStmt::StmtType &stmt_type);
      virtual int serialize_flag(char* buf, const int64_t buf_len, int64_t& pos,
          const int64_t flag) const;
      virtual int64_t get_obj_serialize_size(const int64_t value, bool is_ext=false) const;
      virtual int serialize_int(char* buf, const int64_t buf_len, int64_t& pos,
          const int64_t value) const;
      virtual int deserialize_int(const char* buf, const int64_t data_len, int64_t& pos,
          int64_t& value) const;
      ...
      void set_is_listagg(bool is_listagg)
      ...
      */
      ...
      {return filter_.get_sub_query_expr(idx,expr,query_index);};
      ...
      */
      ...
      virtual void set_read_atomic_param(const ObReadAtomicParam& read_atomic_param);
      virtual ObReadAtomicParam &get_read_atomic_param();
      virtual const ObReadAtomicParam &get_read_atomic_param()const;
      virtual int reset_filter();
      //add duyr 20151130:e
     ...
     //add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
      ...
      {
          trans_id_ = trans_id;
          YYSYS_LOG(DEBUG," par trans id = %s",to_cstring(trans_id_));
          return OB_SUCCESS;
      }
      inline ObSEArray< ObPartitionTransID,MAX_UPS_COUNT_ONE_CLUSTER>& get_trans_id()
      { return trans_id_;}
      //add by maosy e
    protected:
      // RESERVE_PARAM_FIELD
    ...
    protected:
      ...
      //add duyr 20160531:e
      ObSEArray< ObPartitionTransID,MAX_UPS_COUNT_ONE_CLUSTER> trans_id_;//add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
    };
