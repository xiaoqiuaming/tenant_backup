...
//add:e
  //add wuna [MultiUps][sql_api] 20160105:b
   int ObPostfixExpression::check_partition_body_expr(int& op_row_num)
   {
     int ret = OB_SUCCESS;
     op_row_num = 0;
     for(int64_t idx=0; OB_SUCCESS==ret && idx<expr_.count(); idx++)
     {
       int64_t type = 0;
       int64_t value = 0;
       if(OB_SUCCESS != (ret=(expr_[idx].get_int(type))))
       {
         YYSYS_LOG(WARN, "fail to get expr type.unexpected! ret=%d idx=%ld", ret, idx);
         break;
       }
       ...
       else
       {
         ...
         case OP:
           ...
           {
             YYSYS_LOG(WARN,"get operator type failed [err:%d]", ret);
           }
           else
           {
             switch(value)
             ...
             default:
               ...
               YYSYS_LOG(WARN,"unexpected [value:%ld]", value);
               break;
             }
             if(OB_SUCCESS == ret)
             {
...
//        }
        if(OB_SUCCESS != (ret = od.modify_value(p,s)))
        {
            YYSYS_LOG(WARN,"fix varchar decimal error,err=[%d]", ret);
        }
        else
        {
            ObObj obj2;
...
int ExprItem::assign(const common::ObObj &obj)
{
    ...
    case ObDecimalType:
        ...
        /*
           *�ú����ǽ�����Obj�����ֵ�浽ExprItem����
           *����obj����׺����ʽ�����ֵ��ExprItem�����ֵ����д����׺����ʽ��??         *
           *��Ϊ��׺����ʽ���Ұ�decimal��ֵ����varchar��ģ�����������浽ExprItem�����ֵҲ�ǰ���varcahr���
           *
           *get_varchar_d�����������Լ�д�ģ���Ҫ���ܸ�ObObj��get_varchar������ͬ����֮ͬ����get_varchar������type??           *ObVarcharType��obj���ܵ��á�������get_varchar_d������ObObj������������??           */
...
op_call_func_t ObPostfixExpression::call_func[T_MAX_OP - T_MIN_OP - 1] = {
    /*   WARNING: �����˳�򲻿��Ե���??           *   ��Ҫ��(ob_item_tpye.h) ExprType enum�����Ӧ
           */
...
ObPostfixExpression::ObPostfixExpression()
    :expr_(64*1024, ModulePageAllocator(ObModIds::OB_SQL_EXPR)),
      ...
      owner_op_(NULL),
      calc_buf_(ObModIds::OB_SQL_EXPR_CALC, DEF_STRING_BUF_SIZE),temp_expr_(64*1024, ModulePageAllocator(ObModIds::OB_SQL_EXPR)),//add tianz [SubQuery_for_Instmt] [JHOBv0.1] 20140408
...
ObPostfixExpression& ObPostfixExpression::operator=(const ObPostfixExpression &other)
{
 ...
 free_post_in();
  //add:e
    ...
    {
        YYSYS_LOG(WARN, "fail to reset string buffer");
    }
    else if (&other != this)
    {
        ObObj obj;
        ...
        for (i = 0; i < other.expr_.count(); i++)
        {
            if (ObVarcharType == other.expr_[i].get_type())
            {
                ...
                {
                    YYSYS_LOG(ERROR, "fail to write object to string buffer. ret=%d", ret);
                }
                else if (OB_SUCCESS != (ret = expr_.push_back(obj)))
                {
                    YYSYS_LOG(WARN, "failed to add item, err=%d", ret);
                }
            }
...
int ObPostfixExpression::add_expr_obj(const ObObj &obj)
{
    ...
    if (obj.get_type() == ObVarcharType)
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to write object to string buffer. err=%d", ret);
        }
        else if (OB_SUCCESS != (ret = expr_.push_back(obj2)))
        {
            YYSYS_LOG(WARN, "fail to push object to expr array. err=%d", ret);
        }
    }
    else if (OB_SUCCESS != (ret = expr_.push_back(obj)))
    {
...
int ObPostfixExpression::add_expr_item(const ExprItem &item)
{
    ...
    case T_DECIMAL:
        //modify fanqiushi DECIMAL OceanBase_BankCommV0.3 2014_7_9:b
        /*
          *     ���ɺ�׺����ʽ����ֵ�浽obj2���棬�ٰ�obj2�ŵ���׺����ʽ������expr_??          *     ����obj2��������ObDecimalType.
          */
        item_type.set_int(CONST_OBJ);
        obj.set_decimal(item.dec);
        obj.set_nwords(item.len);
        //obj.set_varchar(item.string_);
        if (OB_SUCCESS != (ret = str_buf_.write_obj(obj, &obj2)))
        {
            YYSYS_LOG(WARN, "fail to write object to string buffer. err=%d", ret);
    ...
    case T_TEMP_VARIABLE:
        ...
        if (OB_SUCCESS != (ret = str_buf_.write_obj(obj, &obj2)))
        {
            YYSYS_LOG(WARN, "fail to write variable name to string buffer. err=%d", ret);
        }
        else if (OB_SUCCESS != (ret = expr_.push_back(item_type)))
        {}
        else if (OB_SUCCESS != (ret = expr_.push_back(obj2)))
    ...
    case T_REF_QUERY:
        ...
        //  YYSYS_LOG(ERROR, "TODO... not implement yet");
        //  ret = OB_NOT_SUPPORTED;
    ...
    case T_CUR_TIME_OP:
        ...
        if (OB_SUCCESS != (ret = expr_.push_back(item_type)))
        {
            YYSYS_LOG(WARN, "failed to push item_type, ret=%d", ret);
        }
        break;
    case T_FUN_SYS:
        item_type.set_int(OP);
        ...
        {
            YYSYS_LOG(WARN, "unknown system function=%.*s", item.string_.length(), item.string_.ptr());
        }
        else if (OB_SUCCESS != (ret = expr_.push_back(item_type)))
        {}
        else if (OB_SUCCESS != (ret = expr_.push_back(obj)))
    ...
    case T_CASE_END:
        ...
        //add wanglei :e
    default:
        YYSYS_LOG(WARN, "unknown expr item type=%d", item.type_);
...
int ObPostfixExpression::add_expr_item_end()
{
    ...
    if (OB_SUCCESS != (ret = expr_.push_back(obj)))
    {
        YYSYS_LOG(WARN, "failed to add END, err=%d", ret);
    }
    return ret;
}
...
int ObPostfixExpression::merge_expr(const ObPostfixExpression &expr1, const ObPostfixExpression &expr2, const ExprItem &op)
{
    ...
    if (ret != OB_SUCCESS)
    {
        YYSYS_LOG(ERROR, "Generate new expression faild, ret=%d", ret);
    }
    return ret;
}
            ...
            bool second_check )
{
    ...
    {
        YYSYS_LOG(ERROR, "no memory for postfix expression extra params and stack");
  ...
  if( NULL != post_in_ada_) //add dolphin [in optimization] @20160608
    ...
    {
       // if(stack_[idx]!=NULL)
       // YYSYS_LOG(ERROR,"wanglei case when test:expr[%d]={%s}",idx,to_cstring(expr_[idx]));
        // �����������:��id�����֡���������������ʶ��
      if (OB_SUCCESS != (ret = expr_[idx++].get_int(type)))
        {
            YYSYS_LOG(WARN, "fail to get int value. unexpected! ret=%d idx=%d", ret, idx-1);
            ret = OB_ERR_UNEXPECTED;
            break;
        }
      else if(type <= BEGIN_TYPE || type >= END_TYPE)
        {
            YYSYS_LOG(WARN, "unsupported operand type [type:%ld],idx=%d", type,idx-1);
            ret = OB_ERR_UNEXPECTED;
            break;
        }
        else if (END == type)   // expr_����END���ű�ʾ����
        {
            if (idx_i != 1)
            {
                YYSYS_LOG(WARN, "calculation stack must be empty. check the code for bugs. idx_i=%d,idx=%d,type=%ld", idx_i,idx-1,type);
                ret = OB_ERR_UNEXPECTED;
            }
            else
            {
...
//                    result_.set_decimal(os2);
                ...
                else
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "failed to convert exprobj to obj, err=%d", ret);
                    }
                    else
                    {
                        composite_val = &result_;
        ...
        else
        {
            ...
            case COLUMN_IDX:
                ...
                if (OB_SUCCESS != (ret = expr_[idx++].get_int(value)))
                {
                    YYSYS_LOG(WARN,"get_int error [err:%d]", ret);
                }
                ...
                else
                {
                    if (4 == expr_.count())
                    {
                        ...
                        {
                            YYSYS_LOG(WARN, "fail to get cell from row. err=%d tid=%ld cid=%ld",
                                      ret, value, value2);
                        }
                        quick_path = true;
                    }
            ...
            case CUR_TIME_HMS_OP:
                ...
                {
                    YYSYS_LOG(WARN, "Can not get value ObObj. err=%d", ret);
                }
                break;
            case OP:
                ...
                else
                {
                    ...
                    if (OB_UNLIKELY(T_FUN_SYS == value))
                    {
                        ...
                        {
                            YYSYS_LOG(WARN, "failed to get sys func, err=%d", ret);
                        }
                        else if (0 > sys_func || sys_func >= SYS_FUNC_NUM)
                        {
                            YYSYS_LOG(WARN, "invalid sys function type=%ld", sys_func);
                            ret = OB_ERR_UNEXPECTED;
                        }
                        else if (OB_SUCCESS != (ret = SYS_FUNCS_TAB[sys_func](stack_, idx_i, result, *extra_params)))
                        {
                            YYSYS_LOG(WARN, "failed to call sys func, err=%d func=%ld", ret, sys_func);
                            break;
                        }
                        else
                        {
                            if(result.get_type() == ObDecimalType)
                            {
                                result.set_len(1);
                                if(const_cast<ObDecimal &>(result.get_decimal()).get_words()->table[1] !=0)
                                    result.set_len(2);
                            }
                            stack_[idx_i++] = result;
                        }
                  ...
                  */
                        ...
                        if ((T_OP_IN == value ||T_OP_NOT_IN == value) && (NULL != bloom_filter_ || NULL != hash_map))
                        {
                            ...
                            {
                                YYSYS_LOG(WARN, "fail to get_int from stack. top=%d, ret=%d", idx_i, ret);
                            }
                            else if(-1 == right_elem_count)
                            {
                                ...
                                if (OB_LIKELY(OB_SUCCESS == (ret = this->in_sub_query_func(stack_, 
																							idx_i, 
																							result, 
																							*extra_params, 
																							bloom_filter_, 
																							hash_map,
																							is_hashmap_contain_null,  
																							hash_map_pair_type_desc,
																							sub_query_idx, 
																							second_check))))
                                ////mod 20151225:e
                                //mod 20160518:e
                                ...
                                {
                                    YYSYS_LOG(WARN, "call calculation function error [value:%ld, idx_i:%d, err:%d]",
                                              value, idx_i, ret);
                                }
                            }
                        ...
                        if(!special_process)
                        {
                            ...
                            if (value == T_OP_LEFT_PARAM_END)
                            {
                                if(NULL == post_in_ada_)
                                {
                                    ...
                                    {
                                      ret = OB_ALLOCATE_MEMORY_FAILED;
                                        YYSYS_LOG(WARN,"alloc memory for in post-expression in adaptor failed");
                                        break;
                                    }
                                    else
                                    {
                                      ...
                                      if(NULL == post_in_ada_)
                                        {
                                            ret = OB_ALLOCATE_MEMORY_FAILED;
                                            YYSYS_LOG(WARN,"alloc memory for new in post-expression in adaptor object failed");
                                            break;
                                        }
                                    }
                                }
                                ...
                                {
                                    YYSYS_LOG(WARN, "failed to get cdim, err=%d", ret);
                                    break;
                                }
                                ...
                                if(NULL != post_in_ada_ && post_in_ada_->map_index + 1 <= post_in_ada_->rhm.count())
                                {
                                    if(0 <=  post_in_ada_->rhm.at(post_in_ada_->map_index))
                                    {
                                        ...
                                        {
                                            YYSYS_LOG(ERROR,"check in hash error,ret =%d",ret);
                                            return ret;
                                        }
                                        else
                                        {
                                ...
                                else if(!post_in_ada_->has_null_left_)
                                {
                                    ...
                                    for(;k<expr_.count();++k)
                                    {
                                        ...
                                        else if(OB_SUCCESS == ret)
                                        {
                                            if(T_OP_IN == post_in_ada_->right_count || T_OP_NOT_IN == post_in_ada_->right_count)
                                            {
                                                ...
                                                {
                                                    YYSYS_LOG(WARN, "failed to get sys right_count, err=%d,k=%d,right_count=%ld", ret,k,post_in_ada_->right_count);
                                                    break;
                                                }
                                                else
                                                {
                                                ...
                                                {
                                                    //YYSYS_LOG(ERROR,"right_count is %ld",right_count);
                                    ...
                                    if(post_in_ada_->inhash)
                                    {
                                      ...
                                      if(NULL != inbuf)
                                            ...
                                            {
                                                ret = OB_ALLOCATE_MEMORY_FAILED;
                                                YYSYS_LOG(ERROR,"alloc memory for in post-expression hash map failed");
                                                break;
                                            }
                                      ...
                                      if(NULL == var_cast)
                                            {
                                                ret = OB_ALLOCATE_MEMORY_FAILED;
                                                YYSYS_LOG(WARN,"alloc memory for in post-expression hash values failed");
                                                break;
                                            }
                                            ...
                                            {
                                                ret = OB_ALLOCATE_MEMORY_FAILED;
                                                YYSYS_LOG(WARN,"alloc memory for in post-expression pointer post_ind_ada_  failed");
                                                break;
                                            }
                            ...
                            {
                                //YYSYS_LOG(ERROR,"T_OP_ROW,inhash=%d,t_dim+1=%d,right_count=%ld,idx=%d,idx_i=%d",inhash,t_dim+1,right_count,idx,idx_i);
                                if(NULL != post_in_ada_ && post_in_ada_->inhash && ++post_in_ada_->t_dim <= post_in_ada_->right_count )
                                {
                                  if (OB_SUCCESS !=
                                    ...
                                    {
                                        YYSYS_LOG(WARN, "fail to push row into push_right_hash. ret=%d,idx_i=%d", ret,idx_i);
                                        break;
                                    }
                                    else
                                    {
                                ...
                                if(NULL == post_in_ada_ || (NULL != post_in_ada_ && !post_in_ada_->inhash))
                                {
                                    ...
                                    {
                                        ret = OB_ALLOCATE_MEMORY_FAILED;
                                        YYSYS_LOG(WARN, "call calculation function error [value:%ld, idx_i:%d, err:%d]",value, idx_i, ret);
                                        break;
                                    }
                                }
                            }
                                    ...
                                    case T_OP_NOT_IN:
                                    {
                                        if(!post_in_ada_->inhash)
                                        {
                                                                                                         ...
                                                                                                         *extra_params))))
                                            {
                                                ...
                                                stack_[idx_i++] = result;
                                                //YYSYS_LOG(ERROR,"dolphin::  call_func idx_i=%d,value=%ld",idx_i,value);
                                                ret = OB_SUCCESS;
                                            }
                                            else if (OB_NO_RESULT == ret)
                                            {
...
//                                            }
                                    ...
                                    /*if(ObDecimalType == result.get_type() && result.get_varchar().ptr())
                                                                                {
                                                                                    ...
                                                                                    YYSYS_LOG(ERROR,"dolphin::test ns is %.*s",ns.length(),ns.ptr());
            ...
            case UPS_TIME_OP:
                ...
                {
                    YYSYS_LOG(WARN, "failed to call sys_func_current_timestamp, err=%d", ret);
                }
                else
                {
                    stack_[idx_i++] = result;
...
//mod 20151225:e
{
    ...
    {
        //YYSYS_LOG(ERROR,"dolphin::test extra_params->in_row_operator_.vec_.count() is %ld",extra_params->in_row_operator_.vec_.count());
        stack_ = stack->stack_;
        extra_params->did_int_div_as_double_ = did_int_div_as_double_;
        //add dolphin[coalesce return type]@20151126:b
        extra_params->expr_type_ =  get_expr_type();
        //YYSYS_LOG(ERROR,"dolphin::test post expression return type is:%d",get_expr_type());
        calc_buf_.reuse();
    }
    bool quick_path = false;
    const ObObj *var = NULL;
        ...
        else if (END == type)   // expr_����END���ű�ʾ����
        {
            if (idx_i != idx_i_flag)
            {
                for(int i = 0;i <idx_i;++i)
                {
                    ...
                    char buf[256];
                    o.to_string(buf,256);
                    YYSYS_LOG(ERROR,"stack[%d] is:%s",i,buf);
                }
            ...
            case OP:
                ...
                else
                {
                  ...
                  */
                        ...
                        {
                            //add 20140508:e
                            //YYSYS_LOG(ERROR,"dolphin:: enter not sub query process flow");
                            //add dolphin[in post-expression optimization]@20150920:b
                            if (value == T_OP_LEFT_PARAM_END)
                            {
                                //print_expr();
                              ...
                              if(NULL != post_in_ada_ && post_in_ada_->map_index + 1 <= post_in_ada_->rhm.count())
                              {
                                ...
                                else if(!post_in_ada_->has_null_left_)
                                {
                                  ...
                                  if(post_in_ada_->inhash)
                                  {
                                    ...
                                    {
                                      ret = OB_ALLOCATE_MEMORY_FAILED;
                                      YYSYS_LOG(WARN,"alloc memory for in post-expression hash map failed");
                                      break;
                                    }
                                    char *var_cast = (char *) ob_malloc(post_in_ada_->right_count * post_in_ada_->cdim * MAX_PRINTABLE_SIZE, ObModIds::OB_IN_POSTFIX_EXPRESSION);
                                    if(NULL == var_cast)
        ...
        const ObObj*& val) const
{
    ...
    else
    {
        ...
        {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN, "Can not get result set.err=%d", ret);
        }
        else if (type == PARAM_IDX)
        {
            ...
            {
                YYSYS_LOG(ERROR, "Can not get param index, ret=%d", ret);
            }
            else if (param_idx < 0 || param_idx >= result_set->get_params().count())
            {
                ret = OB_ERR_ILLEGAL_INDEX;
                YYSYS_LOG(ERROR, "Wrong index of question mark position, pos = %ld\n", param_idx);
            }
            else
            {
        ...
        else if (type == SYSTEM_VAR || type == TEMP_VAR)
        {
            ...
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Can not get session info.err=%d", ret);
            }
            else if ((ret = expr_node.get_varchar(var_name)) != OB_SUCCESS)
            {
                YYSYS_LOG(ERROR, "Can not get variable name");
            }
            else if (type == SYSTEM_VAR
                     && (val = session_info->get_sys_variable_value(var_name)) == NULL)
            {
                ret = OB_ERR_VARIABLE_UNKNOWN;
                YYSYS_LOG(USER_ERROR, "System variable %.*s does not exists", var_name.length(), var_name.ptr());
            }
            else if (type == TEMP_VAR
                     && (val = session_info->get_variable_value(var_name)) == NULL)
            {
                ret = OB_ERR_VARIABLE_UNKNOWN;
                YYSYS_LOG(USER_ERROR, "Variable %.*s does not exists", var_name.length(), var_name.ptr());
            }
        }
        else if (type == CUR_TIME_OP)
        {
            ...
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Can not get current time. err=%d", ret);
            }
        }
        //add liuzy [datetime func] 20150910:b
        else if (type == CUR_DATE_OP)
        {
            ...
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Can not get current date. err=%d", ret);
            }
        }
        else if (type == CUR_TIME_HMS_OP)
        {
            ...
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "Can not get current time for HMS. err=%d", ret);
            }
        }
        //add 20150910:e
    }
...
int ObPostfixExpression::check_expr_type(const int64_t type_val, bool &is_type, const int64_t stack_size) const
{
    ...
    if (expr_.count() == stack_size)
    {
        if (ObIntType == expr_[0].get_type())
        {
            ...
            {
                YYSYS_LOG(WARN, "fail to get int value.err=%d", err);
            }
...
//add fanqiushi_index
int ObPostfixExpression::change_tid(uint64_t& array_index) //��ñ���ʽ�м�¼�е�tid��ObObj��ObObj��������±�
{
...
int ObPostfixExpression::get_cid(uint64_t& cid)  //��ñ���ʽ���е�cid���������ʽ���ж���У���??
 {
    int ret = OB_SUCCESS;
    int64_t type = 0;
    int64_t count = expr_.count();
    ...
    else if(column_count == 0 && ret == OB_SUCCESS)
...
int ObPostfixExpression::delete_sub_query_info(int sub_query_index)
{
    ...
    while(OB_SUCCESS == ret && sub_query_index>0)
    {
        ...
        if (OB_SUCCESS != (ret = expr_[idx].get_int(type)))
        {
            YYSYS_LOG(WARN, "Fail to get type. unexpected! ret=%d", ret);
            ret = OB_ERR_UNEXPECTED;
            break;
        }
...
int ObPostfixExpression::get_sub_query_column_num(int sub_query_index,int64_t &num,bool &special_sub_query)
{
    ...
    {
        YYSYS_LOG(WARN, "Fail to get sub query column num. unexpected! ret=%d", ret);
        ret = OB_ERR_UNEXPECTED;
    }
    ...
    else if(OP == type)
    {
        if(OB_SUCCESS != (ret = expr_[++idx].get_int(type)))
        {
            YYSYS_LOG(WARN, "Fail to get operator type. unexpected! ret=%d", ret);
            ret = OB_ERR_UNEXPECTED;
        }
...
int ObPostfixExpression::add_expr_in_obj(ObObj &obj_value)
{
    ...
    if (obj_value.get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to write object to string buffer.ret=%d",ret);
      }
      else if (OB_SUCCESS != (ret = expr_.push_back(item_type)))
      {
        YYSYS_LOG(WARN,"fail to push back item value.ret=%d",ret);
      }
      else if (OB_SUCCESS != (ret = expr_.push_back(obj2_value)))
      {
        YYSYS_LOG(WARN,"fail to push object to expr array.ret=%d",ret);
      }
    }
        ...
        if (expr_.count() == (3+2+3+1)) /*cid(3) + const_operand(2) + operator(3) + end(1) */
        {
            ...
            else
            {
                if (real_val)
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "Fail to get real value.err=%d", err);
                        break;
                    }
                }
                else
...
//add fanqiushi_index
int ObPostfixExpression::get_all_cloumn(ObArray<uint64_t> &column_index)  //��ñ���ʽ�������еĴ�tid��ObObj��ObObj��������±�
{
...
{
    //�жϸñ���ʽ�Ƿ��ܹ�ʹ������������ñ���ʽֻ��һ�У������Ǹ���ֵ��in����ʽ�����Ҹñ���ʽ���е�cid������main_tid��ĳһ���������ĵ�һ��������ñ���ʽ�ܹ�ʹ������
    int ret = OB_SUCCESS;
    bool return_ret = false;
    int64_t type = 0;
    int64_t tmp_type = 0;
...
bool ObPostfixExpression::is_all_expr_cid_in_indextable(uint64_t index_tid,const ObSchemaManagerV2 *sm_v2)  //�жϸñ���ʽ���������Ƿ���������index_tid??
{
    bool return_ret=true;
    int ret = OB_SUCCESS;
    //bool return_ret = false;
...
int ObPostfixExpression::find_cid(uint64_t &cid)  //��ñ���ʽ���е�cid
{
    int ret = OB_SUCCESS;
    //bool return_ret = false;
    int64_t type = 0;
	...
	{
        ret=OB_ERROR;
        YYSYS_LOG(WARN,"too many columns in one expr");
    }
    return ret;
}
//add wanglei [second index fix] 20160425:b
                                            ...
                                            ObObj &cond_start, ObObj &cond_end) const
{
    ...
    do
	{
        ...
        {
            YYSYS_LOG(DEBUG, "not a valid simple between function");
...
DEFINE_SERIALIZE(ObPostfixExpression)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to serialize postfix expression size. ret=%d", ret);
    }
    else
    {
        for (i = 0; i < expr_.count(); i++)
        {
            ...
            {
                YYSYS_LOG(WARN, "Fail to get type. unexpected! ret=%d idx=%d", ret, i);
                ret = OB_ERR_UNEXPECTED;
                break;
            }
                     ...
                     )
            {
                ...
                {
                    YYSYS_LOG(WARN, "Fail to serialize type CONST_OBJ");
                    break;
                }
                ...
                {
                    ret = OB_ERR_UNEXPECTED;
                    YYSYS_LOG(WARN,"Get value ObObj failed [err:%d]", ret);
                    break;
                }
                else if (OB_SUCCESS != (ret = val->serialize(buf, buf_len, pos)))
                {
                    YYSYS_LOG(WARN, "fail to serialize expr[%d]. ret=%d", i, ret);
                    break;
                }
                continue;
            }
            else if (type == OP)
            {
                ...
                {
                    YYSYS_LOG(WARN, "Fail to get op type. unexpected! ret=%d idx=%d", ret, i+1);
                    ret = OB_ERR_UNEXPECTED;
                    break;
                }
                else if (T_FUN_SYS == op_type)
            ...
            for (int64_t j = 0; j < num; j++)
            {
                ...
                {
                    ret = OB_ERR_UNEXPECTED;
                    YYSYS_LOG(WARN, "Fail to serialize expr[%d]. ret=%d count=%ld", i, ret, expr_.count());
                    break;
                }
                else if (j < num - 1)
                {
    ...
    {
        YYSYS_LOG(WARN, "fail to serialize postfix expression type. ret=%d", ret);
    }
    //add:e
    return ret;
}
                                            ...
                                            common::PageArena<ObObj,common::ModulePageAllocator> &rowkey_objs_allocator) const
{
    ...
    //        {
    //          // YYSYS_LOG(DEBUG, "not simple in expr. len=%ld. %d, %d, %d, %d", len,
    ...
    //        {
    //          // YYSYS_LOG(DEBUG, "not simple in expr. len=%ld", len);
    //        }
    ...
    //          {
    //            // YYSYS_LOG(DEBUG, "not simple in expr. len=%ld.%ld, %d, %d", len,
    //            //    dim2, ExprUtil::is_op_of_type(expr_.at(len-5), T_OP_ROW), ExprUtil::is_op(expr_.at(len-6)));
    //          }
    //          else if (dim != dim2)
    //          {
    //            // YYSYS_LOG(DEBUG, "not simple in expr. len=%ld. dim=%ld", len, dim);
    //          }
    ...
    //                {
    //                  YYSYS_LOG(ERROR, "get rowkey column fail. index=%ld, size=%ld", index, size);
    //                }
    //                else
    //                {
    //                  if (OB_SUCCESS != expr_.at(index * 3 + 2).get_int(cid)) // 3=COLUMN_REF, TID, CID
    //                  {
    //                    YYSYS_LOG(ERROR, "fail to get int value from expr_.at(2)");
    //                  }
    ...
    //                      {
    //                        YYSYS_LOG(ERROR, "Can not get value type. err=%d", err);
    //                      }
    //                      else if (real_val)
    //                      {
    //                        if (OB_SUCCESS != (err = get_var_obj(static_cast<ObPostExprNodeType>(type), expr_.at(val_offset), val)))
    //                        {
    //                          YYSYS_LOG(ERROR, "Can not get value. err=%d", err);
    //                        }
    //                        else
    //                        {
    ...
    //                      }
    //                      // YYSYS_LOG(DEBUG, "index=%ld, at=%ld, val=%s", index, offset, to_cstring(rowkey_objs[index]));
    //                    }
    //                    rowkey.assign(rowkey_objs, rowkey_column_count);
    //                    if (OB_SUCCESS!= (err = rowkey_array.push_back(rowkey)))
    //                    {
    //                      YYSYS_LOG(ERROR, "fail to push rowkey to array. err=%d", err);
    //                    }
    ...
    //                else
    //                {
    //                  YYSYS_LOG(ERROR, "fail to alloc memory");
...
// ע���ʱ rowkey ���ص�??��������������ǰ׺�е����ݣ������ǰ�������˳���  fyd
//  �˺����������������ǰ׺����ȫ���������᷵��true
bool ObPostfixExpression::is_in_expr_with_ex_rowkey(bool real_val, 
													const ObRowkeyInfo &info, 
													ObIArray<ObRowkey> &rowkey_array, 
													common::PageArena<ObObj,common::ModulePageAllocator> &rowkey_objs_allocator) const
{
    int err = OB_SUCCESS;
    int64_t rowkey_column_count = 0;
    ...
    ObRowkeyColumn rowkey_column;
    int64_t cid = OB_INVALID_ID;
    int64_t rowkey_index_map[OB_MAX_ROWKEY_COLUMN_NUMBER]={0};//���ڱ��水˳��������������������ʽ�е�λ��
    if (len > 16)
    {
        // check 1: (OP, T_OP_IN, 2), (END)
        if (!ExprUtil::is_end(expr_.at(len-1)) || !ExprUtil::is_value(expr_.at(len-2), 2L) ||
        ...
        {
            const int64_t single_row_len = dim * 2 + 3;
            // dim ��ʾ�ұ߾���һ����1,2,3,...������ʽ��һ���м�����������??��ʾ�������ռ����expr??��ʾ�������������һ��ռ3��expr��OP�Ӿ�  fyd  2014.3.20
            len = (len - 7) - (row_count * single_row_len);  // support multi row, 'dim' columns, each column takes two objects
            // len -7 ��Ϊ7��expr ��ȫ??(OP, T_OP_ROW, row_count) (OP, T_OP_IN, 2), (END)  ���ｫ��Щ��Ϣȥ?? fyd 2014.3.20
            //  row_count * single_row_len ����??�ұ��Ӿ�??�й��о�����Ϣ����������Ϣ�ĳ���  fyd  2014.3.20
            val_idx = len;//val_idx ��ʾ �����У�����in�Ӿ��ұ��������ݣ�expr1-1��ʼ�ĵط�  fyd
            OB_ASSERT(dim > 0);
            OB_ASSERT(len > 6);
            if (dim <= 0 || len <= 6)
            {
            ...
            // check 5: (OP, T_OP_ROW, ?)
                     ...
                     !ExprUtil::is_op(expr_.at(len-6)))//dim2 ��ʾ in ����е�?? fyd  2014.3.20
            {
                // YYSYS_LOG(DEBUG, "not simple in expr. len=%ld.%ld, %d, %d", len,
                //    dim2, ExprUtil::is_op_of_type(expr_.at(len-5), T_OP_ROW), ExprUtil::is_op(expr_.at(len-6)));
            }
            ...
            else if (/*dim >= info.get_size()*/dim>0)// ��ʾin�Ӿ��и�����??  �϶����� true
            {
                len = len - (dim * 3 + 6); // 3 = COLUMN_IDX, TID, CID
                // 6��ʾ����??OP, T_OP_LEFT_PARAM_END, 2)(OP, T_OP_ROW, ?) ����������ʽ��??fyd  2014.3.20
                if (len == 0)// len ���� 0 ��ʾ �������ʽ��������ȷ?? fyd  2014.3.20
                {
                    // extra values from expression
                    // extra 1: rowkey columns
                    //  ���������б��������е����� ���ӵ�һ������??����������rowkey_column_count  ����������ǰ׺������Ŀ  fyd
                    int64_t size = info.get_size();
                    for (index = 0; index < size  && OB_SUCCESS == err; index++)
                    {
                        ...
                        else
                        {
                            ...
                            for( tmp_index=0; tmp_index < dim; tmp_index++ )
                            {
                                ...
                                if (rowkey_column.column_id_ == static_cast<uint64_t>(cid))
                                {
                                    rowkey_index_map[index] = tmp_index;// ��¼��ÿ�������еĴ洢˳??
                                    rowkey_column_count++;
                                    break;
                                }
                            }
          ...
          {
            if(OB_SUCCESS != err)
              YYSYS_LOG(WARN, "fail to push rowkey_types  to array. err=%d", err);
          }*/
                                rowkey_objs += rowkey_column_count;
                            }//
                        }
    ...
    YYSYS_LOG(DEBUG, "current simple in expr is simple with ex rowkey. ret=%d. isSmple=%d", err, is_simple_expr);
    return is_simple_expr;
}
//add:e
DEFINE_DESERIALIZE(ObPostfixExpression)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to deserialize obj. ret=%d. buf=%p, data_len=%ld, pos=%ld",
                  ret, buf, data_len, pos);
    }
    else if (ObIntType != obj.get_type())
    {
        YYSYS_LOG(WARN, "unexpected obj type. actual type:%d, expected:%d", obj.get_type(), ObIntType);
        ret = OB_ERR_UNEXPECTED;
    }
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to get int value. ret=%d, expr_.count()%ld", ret, val);
        }
        else
        {
            ...
            for (i = 0; i < expr_count; i++)
            {
                ...
                else if(ObDecimalType==obj.get_type())
				{
                    ...
                    {
                        // YYSYS_LOG(WARN, "test::fan2=,,,,,p=%d,,,s=%d",obj.get_precision() ,obj.get_scale());
                    ...
                    //YYSYS_LOG(ERROR,"DEFINE_DESERIALIZE");
                }
                //add e
                else
                {
            ...
            if (OB_SUCCESS != (ret = obj.deserialize(buf, data_len, pos)))
            {
                YYSYS_LOG(WARN, "fail to deserialize obj expr_type_. ret=%d. buf=%p, data_len=%ld, pos=%ld",
                          ret, buf, data_len, pos);
            }
            else
            {
...
DEFINE_GET_SERIALIZE_SIZE(ObPostfixExpression)
{
    ...
    for (int i = 0; i < expr_.count(); i++)
    {
        ...
        {
            YYSYS_LOG(WARN, "Fail to get type. unexpected! idx=%d", i);
            break;
        }
        else if (type == PARAM_IDX || type == SYSTEM_VAR || type == TEMP_VAR || type == CUR_TIME_OP
                 //add liuzy [datetime func] 20150914:b
                 ...
                 )
        {
            ...
            {
                YYSYS_LOG(WARN,"Get value ObObj failed ");
                break;
            }
        ...
        else if (type == OP)
        {
            ...
            {
                YYSYS_LOG(WARN, "Fail to get op type. unexpected! idx=%d", i+1);
                break;
            }
            else if (T_FUN_SYS == op_type)
            {
        ...
        for (int64_t j = 0; j < num; j++)
        {
            if (i >= expr_.count())
            {
                YYSYS_LOG(WARN, "Fail to serialize expr[%d]. count=%ld", i, expr_.count());
                break;
            }
            else
            {
...
inline int ObPostfixExpression::nop_func(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    UNUSED(idx_i);
    UNUSED(result);
    YYSYS_LOG(WARN, "function not implemented!");
...
inline int ObPostfixExpression::sys_func_substr(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    {
        //YYSYS_LOG(INFO, "params.operand_count_ = %d, [3]=%s, [2]=%s, [1]=%s",
        ...
        else
        {
            ...
            YYSYS_LOG(WARN, " unexpected operand count %d", params.operand_count_);
        }
...
inline int ObPostfixExpression::left_param_end_func(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to push row into in_row_operator_. ret=%d", ret);
    }
    else
    {
...
inline int ObPostfixExpression::row_func(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    {
        YYSYS_LOG(WARN, "no enough operand in the stack. current size:%d, row_count=%d", idx_i, row_count);
        ret = OB_INVALID_ARGUMENT;
    }
    //add tianz [SubQuery_for_Instmt] [JHOBv0.1] 20140508:b
    else if (-1 == row_count)
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to push row to row operator. ret=%d", ret);
        }
        else
        {
            //YYSYS_LOG(INFO, "idx_i=%d, row_count=%d", idx_i, row_count);
            result.set_int((int64_t)row_count);
        }
    }
    return ret;
                                                  ...
                                                  bool second_check)
{
    ...
    else
    {
        ...
        else if(-1 == right_elem_count)
        {
            ...
            //mod peiouya [IN_AND NOT_IN_WITH_NULL_BUG_FIX] 20160518:e
            {
                YYSYS_LOG(WARN, "fail to get IN operation result. ret=%d", ret);
            }
        ...
        {
            //should not be here
            YYSYS_LOG(WARN, "invialid argument");
            ret = OB_INVALID_ARGUMENT;
        }
    }
    return ret;
...
bool ObPostfixExpression::is_not_equal_cond(uint64_t &table_id)
{
    ...
    if(expr_.count()>3)
    {
        ...
        if(test_expr_int(expr_,3,COLUMN_IDX))
        {
            ret = true;
        }
    }
                                    ...
                                    bool second_check)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to get width_obj from array. vec_top=%ld, ret=%d", vec_top, ret);
        }
        ...
        else
        {
                ...
                //mod 20151225:e
            {
                ...
                else if (ObDecimalType == temp_obj.get_type ())
                {
                    ...
                    int len = temp_obj.get_len();
                    uint64_t *tt = reinterpret_cast<uint64_t *>(varchar_buff + OB_MAX_VARCHAR_LENGTH * i);
                    memcpy(tt, od.get_words()->ToUInt_v2(),sizeof(uint64_t) *len);
                    temp_value.set_decimal(tt,od.get_precision(),od.get_scale(),od.get_vscale(),len);
...
//                    temp_value.set_decimal (os, od.get_precision (),od.get_scale (),od.get_vscale ());
                ...
                {
                    YYSYS_LOG(WARN, "fail to chang ObExprObj to ObObj. ");
                }
            ...
            if(OB_SUCCESS!=ret )
            {
                YYSYS_LOG(WARN,"get result ,get data error");
            }
            ...
            {
                YYSYS_LOG(WARN,"bloom filter should not be null");
            }
            else
            {
                //mod peiouya [IN_TYPEBUG_FIX] 20151225:b
...
inline int ObPostfixExpression::in_func(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    // ??    // 2 IN (3, 4) �ĺ�׺����ʽΪ
    // [ջ��] 2, 3, 4, Row(2), IN(2)
    // in_row_operator_�����ݵ�layoutΪ��
    // 2 3 4
    // width = 1
    //
    // ??    // (1, 3) IN ((3, 4), (1, 2))�ĺ�׺����ʽΪ
    // [ջ��] 1, 3, Row(2), 3, 4, Row(2), 1, 2, Row(2), Row(2), IN(2)
    // in_row_operator_�����ݵ�layoutΪ��
    // 1 3 3 4 1 2
    // width = 2
    //
    // ��Ȼ������width��ȡ��ǰwidth�������������Ƚϼ��ɼ���õ�in�Ľ�??    // note: 1. �ú�׺����ʽ�����У���������ȳ�ջ 2. ���һ��T_OP_ROW������ʵ���Ϻ���
    //
    int ret = OB_SUCCESS;
    if (NULL == stack_i)
    {
...
const char * ObPostfixExpression::get_sys_func_name(enum ObSqlSysFunc func_id)
{
    ...
    {
        ret = "";
        YYSYS_LOG(ERROR, "unexpected sql sys func type %d", func_id);
    }
    return ret;
}
...
int ObPostfixExpression::get_sys_func_param_num(const ObString& name, int32_t& param_num)
{
    ...
    else
    {
        ...
        YYSYS_LOG(WARN, "Unknown function '%.*s', ret=%d", name.length(), name.ptr(), ret);
    }
    return ret;
}
...
inline int ObPostfixExpression::sys_func_cast(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    /*int err = OB_SUCCESS;
      ...
      {
        YYSYS_LOG(WARN, "stack_i=%p, str_buf_=%p.", stack_i, params.str_buf_);
        err = OB_INVALID_ARGUMENT;
      }
      ...
      else
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to cast data from type %d to type %d. err=%d", stack_i[idx_i-2].get_type(), dest_type, err);
            }
          }
        }
        idx_i -= 2;
    ...
    else
    {
        ...
        if(params.operand_count_==4)
        {
            ...
            {
                YYSYS_LOG(WARN, "fail to get int value. actual type = %d. err=%d", stack_i[idx_i-3].get_type(), err);
            }
            else
            {
                // convert literal data type to inner data type
                ...
                if (OB_SUCCESS == err)
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "fail to cast data from type %d to type %d. err=%d", stack_i[idx_i-4].get_type(), dest_type, err);
                    }
                }
            }
            idx_i -= 4;
...
inline int ObPostfixExpression::sys_func_current_timestamp(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    result.set_precise_datetime(yysys::CTimeUtil::getTime());
    return OB_SUCCESS;
}
...
inline int ObPostfixExpression::sys_func_trim(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to trim value. err=%d", err);
        }
        idx_i -= 3;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_lower(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to get lower value. err=%d", err);
        }
        idx_i -= 1;
    }
    return err;
        ...
        for (int i = params.operand_count_; i > 0; i--)
        {
            if (!stack_i[idx_i - i].is_null())
            {
                ...
                if(E_obj.get_type()==ObDecimalType)
                {
                    ...
                    {
                        err = OB_INVALID_ARGUMENT;
                        YYSYS_LOG(WARN, "invalid cast specification for coalesce, type=%d but expr_type is %d,ret=%d", E_obj.get_type(), params.expr_type_, err);
                        break;
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "fail to copy object. err=%d", err);
                        break;
                    }
                    ObObj ex_type;
                    ex_type.set_type(params.expr_type_);
                    if(params.expr_type_ == ObDecimalType)
                    {
                        ex_type.set_precision(38);
                        ex_type.set_scale(0);
                    }
                    ObObj res;
                    //char buf[MAX_PRINTABLE_SIZE];
                    res.set_varchar(ObString(MAX_PRINTABLE_SIZE,MAX_PRINTABLE_SIZE, params.varchar_buf_));
                    const ObObj *temp;
                    if(OB_SUCCESS != (err = obj_cast(obj,ex_type,res,temp, true)))
                    {
                        char bf[256];
                        obj.to_string(bf,255);
                        YYSYS_LOG(WARN,"failed to cast %s to type %d,ret=%d",bf,params.expr_type_,err);
                        break;
                    }
                    else
                    {
...
inline int ObPostfixExpression::sys_func_year(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err!=OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the year func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_month(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err!=OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the month func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_day(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err!=OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the day func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_days(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err!=OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the days func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
static int type_skew_by_time_unit(ObExprObj& obj, ObInterval& value, time_unit tm_unit, int64_t op_type)
{
    ...
    case microseconds:
        break;
    default:
        YYSYS_LOG(WARN, "time unit out of range.");
        ret = OB_ERROR;
    }
    if (OB_SUCCESS != ret)
    {
    ...
    else if (ObTimeType == obj.get_type ())
    {
        ...
        case seconds:
            ...
            break;
        default:
            YYSYS_LOG(WARN, "time artimatic only support hours, minutes, seconds.");
    ...
    else if (ObDateType == obj.get_type ())
    {
        ...
        {
            YYSYS_LOG(WARN, "date artimatic not support hours, minutes, seconds, microseconds.");
            ret = OB_OBJ_TYPE_ERROR;
        }
    }
    else if (ObDateTimeType == obj.get_type ())
    {
        ...
        {
            YYSYS_LOG(WARN, "datetime artimatic not support microseconds.");
            ret = OB_OBJ_TYPE_ERROR;
        }
    }
    ...
    {
        YYSYS_LOG(WARN, "obj type is %d.", obj.get_type ());
        ret = OB_OBJ_TYPE_ERROR;
    }
    return ret;
}
...
int ObPostfixExpression::datetime_calc(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params,int64_t op_type)
{
    ...
    {
        err = OB_ERROR;
        YYSYS_LOG(USER_ERROR,"param is invalid,must be date,time,timestamp.");
    }
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "call calculation datetime function error [value:%ld, idx_i:%d, err:%d]",
                      op_type, idx_i, err);
        }
    }
    return err;
...
int ObPostfixExpression::calc_year_month(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params, int64_t value ,bool is_year)
{
    ...
    if(OB_SUCCESS != (err = stack_i[idx_i-1].get_int(interval_value)))
    {
        ...
        else if(stack_i[idx_i-1].get_type() == ObDecimalType)
        {
            ...
            if(err!= OB_SUCCESS)
            {
                YYSYS_LOG(WARN, "decimal cast_to_int64 failed.err=%d",err);
            }
            else
            {
        ...
        {
            YYSYS_LOG(WARN, "get_int failed.err = %d.", err);
        }
    }
    if(OB_SUCCESS == err)
    {
        ...
        else
        {
            ...
            {
                YYSYS_LOG(WARN, " type_skew failed.err = %d.", err);
            }
            else
            {
                if ( T_OP_MINUS == value)
                ...
                {
                    YYSYS_LOG(ERROR,"calc calc_year_month func failed.ret = %d",err);
                }
            }
        }
    }
                                                                 ...
                                                                 const ObPostExprExtraParams &params, int64_t value,time_unit tm_unit)
{
    ...
    if(OB_SUCCESS == err)
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc type_skew_by_time_unit failed.err=%d.",err);
        }
        ...
        else
        {
            ...
            if(OB_SUCCESS != (err = datetime_calc(stack_i,idx_i,result,params,value)))
            {
                YYSYS_LOG(ERROR,"calc sys_func_adddate func failed.ret = %d",err);
            }
        }
    }
    return err;
...
inline int ObPostfixExpression::sys_func_date_add_day(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_add_day failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_sub_day(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_day_hour_minute_second_microsecond(stack_i,idx_i,result,params,value,days)))
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_sub_day failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_add_hour(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_add_hour failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_sub_hour(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_day_hour_minute_second_microsecond(stack_i,idx_i,result,params,value,hours)))
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_sub_hour failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_add_minute(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_add_minute failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_sub_minute(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_day_hour_minute_second_microsecond(stack_i,idx_i,result,params,value,minutes)))
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_sub_minute failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_add_second(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_add_second failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_sub_second(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_day_hour_minute_second_microsecond(stack_i,idx_i,result,params,value,seconds)))
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_sub_second failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_add_microsecond(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_add_microsecond failed,err = %d",err);
        }
...
inline int ObPostfixExpression::sys_func_date_sub_microsecond(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_day_hour_minute_second_microsecond(stack_i,idx_i,result,params,value,microseconds)))
        {
            YYSYS_LOG(ERROR,"calc sys_func_date_sub_microsecond failed,err = %d",err);
        }
    }
    return err;
}
inline int ObPostfixExpression::sys_func_date_add_year(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "sys_func_date_add_year failed.err = %d",err);
        }
    }
    return err;
}
inline int ObPostfixExpression::sys_func_date_sub_year(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_year_month(stack_i,idx_i,result,params,value,true)))
        {
            YYSYS_LOG(WARN, "sys_func_date_sub_year failed.err = %d",err);
        }
    }
    return err;
}
inline int ObPostfixExpression::sys_func_date_add_month(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "sys_func_date_add_month failed.err = %d",err);
        }
    }
    return err;
}
...
inline int ObPostfixExpression::sys_func_floor(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        case ObVarcharType:
            ...
            {
                //YYSYS_LOG(ERROR, "varchar cast failed ret is %d",err);
				YYSYS_LOG(INFO, "varchar cast to decimal failed ret is %d",err);
            }
            else if(expr_to.is_null())
            {
                YYSYS_LOG(USER_ERROR, "Invalid character found in a character string argument");
                err = OB_INVALID_ARGUMENT;
            }
			else
            {
           ...
           case ObNullType:
            ...
            //xionghui e:
        default:
            YYSYS_LOG(USER_ERROR, "No authorized routine named FLOOR of type FUNCTION having compatible arguments was found type is %d", type);
            err = OB_INVALID_ARGUMENT;
            break;
        }
        if(err == OB_SUCCESS )
...
inline int ObPostfixExpression::sys_func_ceil(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        case ObVarcharType:
            ...
            {
                //YYSYS_LOG(ERROR, "varchar cast failed ret is %d",err);
				YYSYS_LOG(ERROR, "varchar cast to decimal failed ret is %d",err);
            }
            else if(expr_to.is_null())
            {
                YYSYS_LOG(USER_ERROR, "Invalid character found in a character string argument");
           ...
           case ObNullType:
            ...
            //xionghui e:
        default:
            YYSYS_LOG(USER_ERROR, "No authorized routine named CEIL of type FUNCTION having compatible arguments was found type is %d", type);
            err = OB_INVALID_ARGUMENT;
            break;
        }
        if(err == OB_SUCCESS )
    ...
    else
    {
        ...
        if(type1 == ObIntType || type1 == ObInt32Type)
        {
            ...
            case ObDoubleType:
                ...
                {
                    err = OB_INVALID_ARGUMENT;
                    YYSYS_LOG(WARN, "varchar cast failed ret is %d",err);
                }
            ...
            case ObNullType:
             ...
             //xionghui e:
            default:
                YYSYS_LOG(USER_ERROR, "the first argument is invalid type is %d", type2);
                err = OB_INVALID_ARGUMENT;
                break;
            }
            mem_buf.clear();
        ...
        {
            YYSYS_LOG(USER_ERROR, "the second argument is invalid type is %d", type1);
            err = OB_INVALID_ARGUMENT;
        }
        idx_i -= 2;
    }
...
inline int ObPostfixExpression::sys_func_date_sub_month(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        if(OB_SUCCESS != (err = calc_year_month(stack_i,idx_i,result,params,value,false)))
        {
            YYSYS_LOG(WARN, "sys_func_date_sub_month failed.err = %d",err);
        }
    }
    return err;
}
...
inline int ObPostfixExpression::sys_func_hour(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err != OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the hour func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_minute(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err != OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the minute func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_second(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(err != OB_SUCCESS)
        {
            YYSYS_LOG(USER_ERROR, "the second func param is not corrected");
        }
        idx_i -= 1;
    }
    return err;
...
inline int ObPostfixExpression::sys_func_replace(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    if (OB_UNLIKELY(NULL == stack_i || NULL == params.str_buf_))
    {
        YYSYS_LOG(WARN, "stack pointer=%p, str_buf_=%p", stack_i, params.str_buf_);
        err = OB_INVALID_ARGUMENT;
    }
    else if (idx_i < 2 || params.operand_count_ < 2)
    {
        YYSYS_LOG(WARN, "no enough operand in the stack. current size:%d, current op count:%d", idx_i, params.operand_count_);
        err = OB_INVALID_ARGUMENT;
    }
    else
    {
        ...
        if (2 == params.operand_count_)
        {
            ...
            {
                YYSYS_LOG(WARN, "cast source string to varchar failed, err=[%d]",err);
            }
            else if (OB_SUCCESS != (err = stack_i[idx_i-1].cast_to(ObVarcharType, search_obj, *params.str_buf_)))
            {
                YYSYS_LOG(WARN, "cast search string to varchar failed, err=[%d]",err);
            }
        }
        else if (3 == params.operand_count_)
        {
            ...
            {
                YYSYS_LOG(WARN, "cast repalce string to varchar failed, err=[%d]",err);
            }
        }
        else//other
        {
            err = OB_ERR_PARAM_SIZE;
            YYSYS_LOG(USER_ERROR, "the replace func param is not corrected, ret=[%d]",err);
        }
        if (OB_SUCCESS == err && ( source_obj.is_null() || search_obj.is_null() || (replace_obj.is_null() && (3 == params.operand_count_))))
        {
            ...
            {
                err = OB_ERROR;
                YYSYS_LOG(USER_ERROR, "last two params of repalce function can't null");
            }
        ...
        else
        {
            ...
            //        YYSYS_LOG(ERROR,"the soruce is[%.*s],the search is[%.*s], the replace is[%.*s]",
                    ...
                    while (true)
                    {
                        while (ptr < end)
                        {
                            if (*ptr == *search)
                            {
                                ...
                                while (j != search_end)
                                {
                                    //YYSYS_LOG(DEBUG,"not search end");
                                    if (*i++ != *j++)
                                    {
                                        ptr ++;
                                        break;//for next char
            ...
            if (!return_source_no_change)
            {
                ...
                {
                    YYSYS_LOG(WARN, "fail to allocate memory for string. ret=%d", err);
                }
                temp_result.set_varchar(source_str);
                //          YYSYS_LOG(ERROR,"the final result is[%.*s]",source_str.length(),source_str.ptr());
            }
...
inline int ObPostfixExpression::sys_func_last_day(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    if(err != OB_SUCCESS)
    {
        YYSYS_LOG(WARN, "no support type!");
    }
    else
    {
        struct tm gtm;
...
inline int ObPostfixExpression::sys_func_to_char(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    {
        YYSYS_LOG(USER_ERROR, "no enough operand in the stack. current size:%d. expect more than:%d", idx_i, params.operand_count_);
        ret = OB_INVALID_ARGUMENT;
    }
    else
    {
        ...
        if(obj_type== ObPreciseDateTimeType)
        {
            ...
            {
                YYSYS_LOG(WARN, "failed to cast object");
            }
            if(ret == OB_SUCCESS)
                result = promoted_obj;
        }
        ...
        {
            YYSYS_LOG(USER_ERROR, "no support type!");
            ret = OB_INVALID_ARGUMENT;
        }
    }
    if (OB_SUCCESS == ret)
...
int ObPostfixExpression::is_month(char *month, char *tmp_month)
{
    int ret = OB_SUCCESS;
    ...
    {
        ret = OB_INVALID_ARGUMENT;
		YYSYS_LOG(INFO, "the month abbreviation is not correct");
    }
    return ret;
}
int ObPostfixExpression::parse_date(char * date,char * date_str,int date_length,int date_str_length,ObExprObj &ob_date)
{
    ...
    int index = -1;
    char year[10],month[10],day[10],hour[10],min[10],second[10];
    char tmp_month[10];
    //char day[10];
    ...
    sscanf(date, "%[^-^/^.^,^;^ ] %*[-/.,; ] %[^-^/^.^,^;^ ] %*[-/.,; ] %[^-^/^.^,^;^ ] %[^:] %*[:] %[^:]%*[:]%[^:]", year, month, day, hour, min,second);
//    int split[2] ={0,0};
//    for(int i=0,j=0;i<date_length&&j<2;i++){if(date[i] == '-' || date[i] == '/'){split[j]= i;j++;}}
	...
	{
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(USER_ERROR,"not suport format type!");
    }
	break;
    case 2:
    case 4:
    case 6:
        {
            if(0 == strlen(hour) || 0 == strlen(min) || 0 == strlen(second))
            {
                ret = OB_INVALID_ARGUMENT;
				YYSYS_LOG(USER_ERROR, "this date:%s incorrect parameter input", date);
            }
            else if(4 == index || 6 == index)
            {
                std::swap(year,day);
            }
        }
    ...
    case 7:
    {
            ...
            else
            {
                ...
                case 0:
                {
                    if(0 == strlen(month) && 0 == strlen(day))
                    {
                        if(7!= strlen(year) && 8!= strlen(year))
                        {
                            ret = OB_INVALID_ARGUMENT;
							YYSYS_LOG(USER_ERROR, "this date:%s incorrect parameter input", date);
                        }
                    ...
                    else if(0 == strlen(day) && 0 != strlen(month))
                    {
                        if(2>= strlen(month) || 4< strlen(month))
                        {
                            ret = OB_INVALID_ARGUMENT;
							YYSYS_LOG(USER_ERROR, "this date:%s incorrect parameter input", date);
                        }
                        ...
                        {
                            memcpy(tmp_month,month,2);
                            memcpy(day,month+2,strlen(month)-2);
                            memset(month,'\0',sizeof(month));
                            memcpy(month,tmp_month,strlen(tmp_month));
                        }
                    }
				}
				break;
                ...
                case 1:
                    {
                        ...
                        else if(1 == index || 5 == index || 7 == index)
                        {
                            bool is_has_month = false;
                            int index_month;
                            for(int i=0;i< static_cast<int>(strlen(month));i++)
                            {
                                if(strcmp(&month[i],"��") ==0)
                                {
                                    is_has_month = true;
                            ...
                            if(is_has_month)
                            {
                                memcpy(tmp_month,month,index_month);
                            }
                            else
                            {
                                ret = is_month(month,tmp_month);
                            }
                            if(OB_SUCCESS == ret)
                            {
                                memset(month,'\0',sizeof(month));
    ...
    if(OB_SUCCESS == ret)
    {
        ...
        case 5:
        case 7:
                sprintf(buf, "%s%c%s%c%s", year, '-', month, '-', day);
            break;
        case 2:
            case 4:
            case 6:
                sprintf(buf, "%s%c%s%c%s%c%s%c%s%c%s", year, '-', month, '-', day, ' ',hour, ':',min,':',second);
            break;
        ...
        test_str.assign_ptr(buf,static_cast<ObString::obstr_size_t>(strlen(buf)));
        date_new.set_varchar(test_str);
        if(OB_SUCCESS != (ret = check_format_and_content(date_new,TIMESTAMP_CHECK)))
        {
			YYSYS_LOG(WARN, "the date %s is not match", date);
        }
        else
        {
            int year_int = 0;
            ...
            {
                YYSYS_LOG(INFO,"the date param is not corrected");
            }
		}
    }
    return ret;
...
inline int ObPostfixExpression::sys_func_to_date(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        {
            YYSYS_LOG(USER_ERROR, "fomart must be varchar type");
            ret = OB_INVALID_ARGUMENT;
        }
...
inline int ObPostfixExpression::parse_timestamp(char * date,char * date_str,int date_length,int date_str_length,ObExprObj &ob_date)
{
    ...
    int index = -1;
    char year[10], month[10], day[10], minutes[10], hour[10], second[10], microseconds[10];
    char date_t[20], time_tmp[30], PM[10];
    memset(year,'\0',sizeof(year));
    memset(month,'\0',sizeof(month));
    memset(day,'\0',sizeof(day));
    ...
    {
       ret = OB_INVALID_ARGUMENT;
	   YYSYS_LOG(USER_ERROR, "not support format type!");
    }
    ...
    case 1:
    {
        if(0 == strlen(microseconds) || 6<strlen(microseconds))
        {
            ret = OB_INVALID_ARGUMENT;
			YYSYS_LOG(USER_ERROR, "this date:%s incorrect parameter input", date);
        }
        ...
        {
            for(int i = static_cast<int>(strlen(microseconds));i<6;i++)
            {
                microseconds[i] = '0';
            ...
            case 0:
            {
               ...
               if(OB_SUCCESS == ret && (100 <= atoi(day)))
               {
                   ret = OB_INVALID_ARGUMENT;
				   YYSYS_LOG(USER_ERROR, "this date:%s incorrect parameter input", date);
               }
    ...
    if(ret == OB_SUCCESS)
    {
        ...
        if(0 == index || 1 == index)
        {
            sprintf(buf,"%s%c%s%c%s%c%s%c%s%c%s%c%s", year,'-',month,'-',day,' ',hour,':',minutes,':',second,'.',microseconds);

        }
        else if(2 == index)
        {
            sprintf(buf,"%s%c%s%c%s%c%s%c%s%c%s", year,'-',month,'-',day,' ',hour,':',minutes,':',second);
        }
        ObExprObj date_time_new;
        common::ObString test_str(static_cast<ObString::obstr_size_t>(data_len),
                                  static_cast<ObString::obstr_size_t>(0),buf);
        test_str.assign_ptr(buf,static_cast<ObString::obstr_size_t>(strlen(buf)));
        date_time_new.set_varchar(test_str);
        if(OB_SUCCESS != (ret = check_format_and_content(date_time_new,TIMESTAMP_CHECK)))
        {
			YYSYS_LOG(WARN, "this date:%s format is not match", date);
        }
        else
        {
            int year_int = 0;
...
inline int ObPostfixExpression::sys_func_decode(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else if (params.operand_count_ < 3 || idx_i < params.operand_count_)
    {
        ...
        YYSYS_LOG(WARN, "no enough operand in the stack. current size:%d. expect more than:%d", idx_i, params.operand_count_);
        ret = OB_INVALID_ARGUMENT;
    }
    else
    {
...
//add :e
// uncertainty chunk�еĸ����˺����߼� ��ǣ��multiups�Ĺ���
int ObPostfixExpression::arg_case_func(ObExprObj *stack_i, int &idx_i, ObExprObj &result, const ObPostExprExtraParams &params)
{
    ...
    else
    {
        ...
        if(result.get_type() != params.expr_type_)
        {
            ...
            result.to(obj1);
            ex_type.set_type(params.expr_type_);
            if(ex_type.get_type() == ObDecimalType)
            {
                ex_type.set_precision(38);
                ex_type.set_scale(0);
            }
            ...
            if(OB_SUCCESS != (ret = obj_cast(obj1,ex_type,res,temp)))
            {
                ...
                YYSYS_LOG(WARN,"failed to cast %s to type %d,ret=%d",bf,params.expr_type_,ret);
            }
            else
            {
                result.assign(*temp);
...
//            //YYSYS_LOG(DEBUG, "i=%d,arg_pos=%d,cmp_case_when(%s,%s)", i, arg_pos, to_cstring(stack_i[arg_pos]), to_cstring(stack_i[i]));
...
int ObPostfixExpression::check_in_hash(ObExprObj& result,int t,int64_t v)
{
    ...
    if(NULL == post_in_ada_)
    {
        ...
        YYSYS_LOG(WARN,"post expression check in hash but post_in_ada_ ptr is NULL");
        return ret;
    }
        ...
        if(value != NULL)
        {
            ...
            for(int j = 0;j<cnt;++j)
            {
                ...
                if(OB_SUCCESS != (ret = exp1.compare(exp2,val)))
                {
                    ...
                    YYSYS_LOG(WARN,"ObExprObj %s and %s compare error",buf1,buf2);
                    break;
                }
                else
                {
...
/*int ObPostfixExpression:: print_in_hash_param(int t)
{
    ...
    if(NULL != post_in_ada_)
    {
        ...
        YYSYS_LOG(ERROR,"dolphin::test left_param_ is %s",buf);
        ...
        for(;iter != post_in_ada_->rhv.at(t)->right_hash_.end();++iter)
        {
            ...
            YYSYS_LOG(ERROR,"dolphin::test hash value is %s",buf);
        ...
        for(;it != post_in_ada_->rhv.at(t)->in_null_.end();++it)
        {
            ...
            YYSYS_LOG(ERROR,"dolphin::test hash null value is %s",buf);
...
/*int ObPostfixExpression::print_expr(int j)
{
    ...
    for(int i = j;i < expr_.count();++i)
    {
        ...
        YYSYS_LOG(WARN,"expr_[%d] is %s:",i,b);
        ...
        case OP:
            ...
            else
            {
                if (OB_UNLIKELY(T_FUN_SYS == value))
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "failed to get sys func, err=%d", err);
                    }
                    else if (0 > sys_func || sys_func >= SYS_FUNC_NUM)
                    {
                        YYSYS_LOG(WARN, "invalid sys function type=%ld", sys_func);
...
bool ObPostfixExpression::is_rows_filter()
{
    ...
    while(OB_SUCCESS == err && END != type)
    {
        ...
        {
            YYSYS_LOG(WARN, "Fail to get type. unexpected! ret=%d", err);
            err = OB_ERR_UNEXPECTED;
            break;
        }
        else if(QUERY_ID == type)
        {
            ...
            {
                YYSYS_LOG(WARN, "Fail to get sub query column num. unexpected! ret=%d", err);
                err = OB_ERR_UNEXPECTED;
            }
            else if(-1 == mark)
            {
...
bool ObPostfixExpression::is_in_sub_query_expr(const ObRowkeyInfo &info) const
{
    ...
    if(len<0)
    {
        YYSYS_LOG(INFO, "should not be here. len=%ld", len);
    }
    else if(len>14)
    {
        if (!ExprUtil::is_end(expr_.at(len-1)) || !ExprUtil::is_value(expr_.at(len-2), 2L) ||
        ...
        else if (OB_SUCCESS != expr_.at(len-5).get_int(row_count) || !ExprUtil::is_sub_query(expr_.at(len-6)))
        {
            ...
            //YYSYS_LOG(INFO, "not simple in expr. len=%ld", len);
            //add :e
        }
        ...
        else
        {
            ...
            {
                YYSYS_LOG(ERROR, "fail to get left value of in expr. err=%d", err);
            }
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160122:b
    int ObPostfixExpression::get_next_tid_cid(ObObj *&tid_val, ObObj *&cid_val, bool &is_finish, int64_t &next_expr_idx)
    {
      int ret = OB_SUCCESS;
      int64_t idx      = next_expr_idx;
      int64_t type     = 0;
      int64_t value    = 0;
      int64_t value2   = 0;
      int64_t sys_func = 0;
      ...
      while(idx < expr_.count() && OB_SUCCESS == ret && is_finish)
      {
        if (!is_finish)
        {//means has get tid and cid!
          break;
        }
        ...
        {
          case COLUMN_IDX:
            tid_val = &(expr_.at(idx++));
            cid_val = &(expr_.at(idx++));
            is_finish = false;
            break;
          case CONST_OBJ:
          case CUR_TIME_OP:
          ...
          case OP:
            ...
            else
            {
              if (OB_UNLIKELY(T_FUN_SYS == value))
              {
                if(OB_SUCCESS != (ret = expr_[idx++].get_int(sys_func)))
                {
                  YYSYS_LOG(WARN, "failed to get sys func, ret=%d", ret);
                }
                else if (0 > sys_func || sys_func >= SYS_FUNC_NUM)
                {
                  YYSYS_LOG(WARN, "invalid sys function type=%ld", sys_func);
          ...
          default:
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN,"unexpected [type:%ld],ret=%d", type,ret);
            break;
        }
      } // end while
