...
int ObMultiColumnGroupScanner::add_row_iterator(ObRowkeyIterator* iterator)
{
  ...
  {
    YYSYS_LOG(WARN, "current iter num=%ld >= MAX_ITER_NUM=%ld", iter_num_, MAX_ITER_NUM);
    ret = OB_SIZE_OVERFLOW;
  }
  else if (OB_SUCCESS != (ret = cons_row_desc(iterator)))
  {
    YYSYS_LOG(WARN, "cons iterator's row desc error.");
  }
  else
  {
...
int ObMultiColumnGroupScanner::cons_row_desc(ObRowkeyIterator* iterator)
{
  ...
  {
    YYSYS_LOG(WARN, "get_row_desc ret=%d, row_desc=%p error.", ret, row_desc);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    for (int64_t i = 0; i < row_desc->get_column_num() && OB_SUCCESS == ret; ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot get [%ld]th tid cid, row desc=%s", i, to_cstring(*row_desc));
      }
      else if (OB_SUCCESS != (ret = row_desc_.add_column_desc(table_id, column_id)))
      {
        YYSYS_LOG(WARN, "cannot add [%ld]th [%lu,%lu] to column desc.", i, table_id, column_id);
      }
    }
  }
  return ret;
...
int ObMultiColumnGroupScanner::cons_row(int64_t& cur_cell_num, const common::ObRow& row)
{
  ...
  for (int64_t i = 0; i < row.get_column_num() && OB_SUCCESS == ret; ++i, ++cur_cell_num)
  {
    ...
    {
      YYSYS_LOG(WARN, "cannot get [%ld]th cell from row, ret=%d", i, ret);
    }
    else if (OB_SUCCESS != (ret = current_row_.raw_set_cell(cur_cell_num, *cell)))
    {
      YYSYS_LOG(WARN, "cannot set [%ld]th cell from row, cel_num=%ld, ret=%d", i, cur_cell_num, ret);
    }
  }
  ...
  else
  {
    ...
    for (int64_t i = 0; i < iter_num_; ++i)
    {
      if (0 == i) 
      {
        ...
        {
          YYSYS_LOG(WARN, "cons first row error=%d, cur_cell_num=%ld", ret, cur_cell_num);
        }
      }
      else
      {
        ...
        if (OB_ITER_END == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "iterator[%ld] got result[%d] <> [%d]", i, ret, first_result);
            ret = OB_ERROR;
            break;
          }
          continue;
