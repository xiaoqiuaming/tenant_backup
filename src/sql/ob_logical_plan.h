...
namespace oceanbase
{
  namespace sql
  {
    ...
    typedef std::pair<const char *, uint64_t> ViewInfo;
    ...
    public:
      ...
      int add_query(ObBasicStmt* stmt)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to allocate space for stmt. %p", stmt);
          ret = common::OB_ERROR;
        }
        return ret;
      }
      ...
      int add_expr(ObSqlRawExpr* expr)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to allocate space for expr. %p", expr);
          ret = common::OB_ERROR;
        }
        return ret;
      }
      ...
      int add_raw_expr(ObRawExpr* expr)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to allocate space for raw expr. %p", expr);
          ret = common::OB_ERROR;
        }
        return ret;
      }
...
//        cur_time_fun_type_ = CUR_TIME_UPS;
      ...
      inline bool get_query_optimizer_open()
      {
        return query_optimizer_open_;
      }

      inline void set_query_optimizer_open(bool query_optimizer_open)
      {
        query_optimizer_open_ = query_optimizer_open;
      }
      ...
      oceanbase::common::ObVector<ObBasicStmt*> &get_stmts()
      {
        return stmts_;
      }

      int32_t get_stmt_index_by_query_id(uint64_t query_id) const;

      int add_view_info(const char *view_name, uint64_t query_id);
      int remove_view_info(const char *view_name, uint64_t query_id);

      bool get_is_recursive_view(const char *view_name);

      void reset_view_list();

      void set_is_create_view(bool create_view)
      { is_create_view_ = create_view; }
      bool get_is_create_view() const
      { return is_create_view_;}
      void set_is_show_view(bool show_view)
      {   is_show_view_ = show_view; }
      bool get_is_show_view() const
      {  return is_show_view_; }

      //[735]
      void set_use_for_view(bool use_for_view)
      { use_for_view_ = use_for_view; }
      bool use_for_view() const
      { return use_for_view_; }
...
//      ObCurTimeType cur_time_fun_type_;
      ...
      bool query_optimizer_open_;
      int64_t new_gen_aid_;

      ObList<ViewInfo> view_list_;
      bool is_create_view_;
      bool is_show_view_;
      bool use_for_view_; //[735]
    };
  }
}
