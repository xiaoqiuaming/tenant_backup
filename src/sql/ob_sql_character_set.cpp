  ...
  namespace sql
  {
    ...
    int32_t get_char_number_from_name(const common::ObString& name)
    {
      ...
      if (n <= 0 || n >= common::OB_CHAR_SET_NAME_LENGTH)
      {
        ...
        YYSYS_LOG(ERROR, "failed to dump charset[%.*s]", name.length(), name.ptr());
      }
      else
      {
    ...
    int32_t get_char_number_from_name(const char* name)
    {
      int32_t ret = 0;
      int32_t low = 0;
      int32_t high = static_cast<int32_t>(ARRAYSIZEOF(charset) - 1);
      int cret = 0;
      while (low <= high)
      {
        ret = low + (high - low)/2;
    ...
    const char* get_char_name_from_number(int32_t number)
    {
      ...
      int32_t ret = 0;
      int32_t low = 0;
      int32_t high = static_cast<int32_t>(ARRAYSIZEOF(charsetR) - 1);
      while (low <= high)
      {
        ret = low + (high - low)/2;
