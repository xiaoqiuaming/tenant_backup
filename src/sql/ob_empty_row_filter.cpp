...
int ObEmptyRowFilter::open()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "empty row filter row desc[%s] child[%s]", to_cstring(*row_desc), to_cstring(*child_op_));
    }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "empty row filter cur row desc[%s]", to_cstring(cur_row_desc_));
  }
  return ret;
}
