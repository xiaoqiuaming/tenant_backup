...
bool ObTabletScanFuse::check_inner_stat()
{
  ...
  {
    ret = false;
    YYSYS_LOG(WARN, "check inner stat fail:sstable_scan_[%p], incremental_scan_[%p]", sstable_scan_, incremental_scan_);
  }
  //add zhaoqiong [Truncate Table]:20160318:b
  if(NULL == sstable_scan_)
  {
    YYSYS_LOG(DEBUG, "sstable_scan_ is null");
  }
  //add:e
  return ret;
}
...
int ObTabletScanFuse::set_sstable_scan(ObSSTableScan *sstable_scan)
{
  ...
  if(NULL == sstable_scan)
  {
    ...
    YYSYS_LOG(WARN, "argument sstable_scan is null");
...
int ObTabletScanFuse::set_incremental_scan(ObMultiUpsScan *incremental_scan)
//mod 20160107:e
{
  int ret = OB_SUCCESS;
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "argument incremental_scan is null");
  }
  else
  {
...
int ObTabletScanFuse::open()
{
  ...
  //mod:e
  {
    YYSYS_LOG(WARN, "open sstable scan fail:ret[%d]", ret);
  }
  //mod zhaoqiong [Truncate Table]:20160318:b
  ...
  //mod:e
  {
    YYSYS_LOG(WARN, "fail to get sstable scan row desc:ret[%d]", ret);
  }
  //add zhaoqiong [Truncate Table]:20160318:b
  else if (OB_SUCCESS != (ret = incremental_scan_->get_row_desc(row_desc)))
  {
    YYSYS_LOG(WARN, "fail to get incremental scan row desc:ret[%d]", ret);
  }
  //add:e
  else
  {
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "open incremental scan fail:ret[%d]", ret);
    }
    else
    {
      last_incr_row_ = NULL;
...
int ObTabletScanFuse::close()
{
  ...
  if(NULL != sstable_scan_)
  {
    ...
    {
      YYSYS_LOG(WARN, "close sstable scan fail:ret[%d]", ret);
    }
  }
  ...
  if(NULL != incremental_scan_)
  {
    ...
    {
      YYSYS_LOG(WARN, "close incremental scan fail:ret[%d]", ret);
    }
  }
...
int ObTabletScanFuse::get_next_row(const ObRow *&row)
{
  ...
  //add duyr [read_atomic::debug]
  if (read_atomic_param_.is_valid())
  {
    YYSYS_LOG(DEBUG,"read_atomic::debug,begin scan fuse!param=[%s],ret=%d",
  ...
  do
  {
    ...
    if(OB_SUCCESS == ret)
    {
      if (NULL == last_incr_row_)
      {
        ...
        if (read_atomic_param_.is_valid())
        {
          if (NULL != tmp_last_incr_row)
          {
            YYSYS_LOG(DEBUG,"read_atomic::debug,scan fuse!orig_incr_row[%s],ret=%d",
                      to_cstring(*tmp_last_incr_row),ret);
          }
        }
        //add duyr end
        if(OB_SUCCESS == ret)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "incremntal scan should return ObUpsRow");
          }
        }
        ...
        {
          YYSYS_LOG(WARN, "incremental scan get next row fail:ret[%d]", ret);
        }
      }
        ...
        //mod:e
      {
        ...
        if (read_atomic_param_.is_valid())
        {
          ...
          {
            YYSYS_LOG(DEBUG,"read_atomic::debug,scan fuse!orig_sstable_row[%s],ret=%d",
                      to_cstring(*last_sstable_row_),ret);
          }
        }
        //add duyr end
        if(OB_ITER_END == ret)
        ...
        {
          YYSYS_LOG(WARN, "sstable scan get next row fail:ret[%d]", ret);
        }
      }
      //add duyr [read_atomic::debug]
      if (read_atomic_param_.is_valid())
      {
        if (NULL != last_incr_row_)
        {
          YYSYS_LOG(DEBUG,"read_atomic::debug,scan fuse!final_incr_row[%s],ret=%d",
                    to_cstring(*last_incr_row_),ret);
        }
        if (NULL != last_sstable_row_)
        {
          YYSYS_LOG(DEBUG,"read_atomic::debug,scan fuse!final_sstable_row[%s],ret=%d",
                    to_cstring(*last_sstable_row_),ret);
        }
      }
      //add duyr end
        ...
        if(NULL != last_sstable_row_)
        {
          if(NULL != last_incr_row_)
          {
            ...
            if (cmp_result < 0)
            {
              ...
              {
                YYSYS_LOG(WARN, "assign last incr row to curr_row fail:ret[%d]", ret);
              }
              //add hongchen [MEM_EARLY_RELEASE_BUGFIX] 20170901:b
              else if(OB_SUCCESS != (ret = deep_copy_incr_rowkey()))
              {
                YYSYS_LOG(WARN, "fail to deep copy incremental rowkey!");
              }
              //add hongchen [MEM_EARLY_RELEASE_BUGFIX] 20170901:es
              else
              {
                row = &curr_row_;
...
//              {
//                YYSYS_LOG(WARN, "fuse row fail:ret[%d]", ret);
//              }
              else if (read_atomic_param_.is_valid()
                       && OB_SUCCESS != (ret = ObReadAtomicHelper::fuse_row(last_incr_row_,
                                                                            last_sstable_row_,
                                                                            read_atomic_param_,
                                                                            &curr_row_)))
              {
                YYSYS_LOG(WARN,"fail to fuse read atomic row!ret=%d",ret);
                      ...
                      && OB_SUCCESS != (ret = ObRowFuse::fuse_row(last_incr_row_, last_sstable_row_, &curr_row_)))
              {
                YYSYS_LOG(WARN, "fuse row fail:ret[%d]", ret);
              }
              //add hongchen [MEM_EARLY_RELEASE_BUGFIX] 20170901:b
...
int ObTabletScanFuse::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  if(NULL == sstable_scan_)
  {
    //mod zhaoqiong [Truncate Table]:20160318:b
    //ret = OB_ERROR;
    //YYSYS_LOG(WARN, "sstable scan is null");
...
//add hongchen [MEM_EARLY_RELEASE_BUGFIX] 20170901:b
int ObTabletScanFuse::deep_copy_incr_rowkey()
{
  int ret = OB_SUCCESS;
  if (NULL == incremental_rowkey_)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN,"incremental_rowkey_ == null, ret=%d", ret);
  }
  else
  {
    str_buf_.reset ();
    void* buff = NULL;
    if (NULL == (buff = str_buf_.alloc(sizeof(ObRowkey))))
    {
    ...
    {
      ObRowkey* tmp_rowkey = NULL;
      tmp_rowkey = new(buff) ObRowkey();
      if (OB_SUCCESS != (ret = incremental_rowkey_->deep_copy(*tmp_rowkey, str_buf_)))
      {
        YYSYS_LOG(WARN, "fail to deep copy rowkey, ret=%d", ret);
      }
      else
      {
        incremental_rowkey_ = tmp_rowkey;
      }
    }
  }
  return ret;
