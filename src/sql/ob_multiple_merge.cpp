...
ObMultipleMerge::ObMultipleMerge()
  ...
  ,is_return_current_fused_row_(false)
  //add 20160122:e
  //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
  ,data_mark_param_(),data_mark_final_row_(),data_mark_final_row_desc_(),stmt_start_time_(INT64_MAX)
  //add duyr 20160531:e
{
...
int ObMultipleMerge::get_row_desc(const ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "not child setted");
  }
  else
  {
...
int ObMultipleMerge::set_child(int32_t child_idx, ObPhyOperator &child_operator)
{
  ...
  if (child_idx >= MAX_CHILD_OPERATOR_NUM)
  {
    ...
    YYSYS_LOG(WARN, "child_idx[%d] is overflow", child_idx);
  }
...
int ObMultipleMerge::set_data_mark_param(const ObDataMarkParam &param)
{
    ...
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"invaid data mark param[%s]!ret=%d",
...
PHY_OPERATOR_ASSIGN(ObMultipleMerge)
{
  ...
  is_return_current_fused_row_ = o_ptr->is_return_current_fused_row_;
  //add 20160316:e
  //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
  data_mark_param_ = o_ptr->data_mark_param_;
  //add duyr 20160531:e
  stmt_start_time_ = o_ptr->stmt_start_time_;//add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 
  return ret;
}
...
DEFINE_SERIALIZE(ObMultipleMerge)
{
 ...
 {
   YYSYS_LOG(WARN, "fail to encode is_ups_row_:ret[%d]", ret);
 }
 //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160122:b
 else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_return_current_fused_row_)))
 {
   YYSYS_LOG(WARN, "fail to encode is_return_current_fused_row_:ret[%d]", ret);
 }
 //add 20160122:e
 //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
 else if (OB_SUCCESS != (ret = data_mark_param_.serialize(buf,buf_len,pos)))
 {
   YYSYS_LOG(WARN,"fail to serialize data mark param!ret=%d",ret);
 }
 //add duyr 20160531:e
 //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
 else if(OB_SUCCESS !=(ret = serialization::encode_i64(buf,buf_len,pos,stmt_start_time_)))
 {
     YYSYS_LOG(WARN,"fail to serialize stmt_start_time_!ret=%d",ret);
 }
 //add by maosy e
 return ret;
}
...
DEFINE_DESERIALIZE(ObMultipleMerge)
{
  ...
  {
    YYSYS_LOG(WARN, "faio to decode is_ups_row_:ret[%d]", ret);
  }
  //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160122:b
  else if (OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_return_current_fused_row_)))
  {
    YYSYS_LOG(WARN, "fail to decode is_return_current_fused_row_:ret[%d]", ret);
  }
  //add 20160122:e
  //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
  else if (OB_SUCCESS != (ret = data_mark_param_.deserialize(buf,data_len,pos)))
  {
      YYSYS_LOG(WARN,"fail to deserialize data mark param!ret=%d",ret);
  }
  //add duyr 20160531:e
  //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
  else if(OB_SUCCESS != (ret = serialization::decode_i64(buf,data_len,pos,&stmt_start_time_)))
  {
      YYSYS_LOG(WARN,"fail to deserialize stmt_start_time_!ret=%d",ret);
  }
  //add by maosy e
  return ret;
}
