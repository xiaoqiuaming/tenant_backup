...
int ObMultiChildrenPhyOperator::set_child(int32_t child_idx, ObPhyOperator &child_operator)
{
  ...
  {
    ret = OB_INIT_TWICE;
    YYSYS_LOG(ERROR, "The %dth operator already init, ret=%d", child_idx, ret);
  }
  else if (child_idx > children_ops_.count())
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "Child physical operator must be set in order, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = children_ops_.push_back(&child_operator)))
  {
    YYSYS_LOG(ERROR, "Add child physical operator failed, ret=%d", ret);
  }
  return ret;
}

int ObMultiChildrenPhyOperator::alter_child(int32_t child_idx, ObPhyOperator &child_operator)
{
  int ret = OB_SUCCESS;
  if (child_idx < children_ops_.count())
  {
  ...
  {
    ObPhyOperator *& op = children_ops_.at(child_idx);
    op = &child_operator;
  }
  return ret;
}
ObPhyOperator *ObMultiChildrenPhyOperator::get_child(int32_t child_idx) const
{
  ...
  {
    YYSYS_LOG(WARN, "Invalid child index, index=%d, count=%ld", child_idx, children_ops_.count());
  }
  return op;
}
