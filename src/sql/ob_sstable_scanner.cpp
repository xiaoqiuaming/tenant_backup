...
namespace oceanbase
{
  namespace sql
  {
        ...
        const ObRowkeyInfo* rowkey_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, group_array=%p, group_size=%ld",
        ...
        const common::ObRowkeyInfo* rowkey_info)
    {
      ...
      {
        iret = OB_NOT_FREE;
        YYSYS_LOG(WARN, "internal_scanner_obj_ptr_ must be freed before used:ret[%d]", iret);
      }
      else
      {
        ...
        if (NULL == internal_scanner_obj_ptr_)
        {
          YYSYS_LOG(ERROR, "alloc memory for column group scanner failed,"
        ...
        else
        {
          ...
          for (group_seq = 0; group_seq < group_size && OB_SUCCESS == iret; ++group_seq)
          {
            ...
            {
              YYSYS_LOG(ERROR, "column group scanner initialize , iret=%d,"
            ...
            {
              YYSYS_LOG(ERROR, "column group scanner set scan parameter error, iret=%d,"
                  "current_group_id=%ld, group_seq=%ld", iret, current_group_id, group_seq);
            }
            else if (OB_SUCCESS != (iret = merger_.add_row_iterator(column_group_scanner)) )
            {
              YYSYS_LOG(ERROR, "add iterator to ObMerger error, iret=%d,"
                  "current_group_id=%ld, group_seq=%ld", iret, current_group_id, group_seq);
            }
            else
            {
              YYSYS_LOG(DEBUG, "add %ldth group id=%ld to scan, ret = %d", 
        ...
        const common::ObRowkeyInfo* rowkey_info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "single column group scanner initialize , ret=%d,"
      ...
      {
        YYSYS_LOG(ERROR, "single column group scanner set scan parameter error, ret=%d,"
            "column_group_id=%lu, group_seq=%d", ret, column_group_id, 0);
      }
        ...
        const ScanContext* scan_context)
    {
      ...
      {
        YYSYS_LOG(ERROR, "initialize column_group_scanner error ret=%d", iret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "old fashion table(%ld) binary rowkey format, "
      ...
      {
        YYSYS_LOG(ERROR, "set_column_group_scanner error, ret=%d", iret);
      }
      return iret;
    }
        ...
        uint64_t* group_array, int64_t& group_size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid param, schema=%p, table_id=%ld, group_array=%p, group_size=%ld.", 
        ...
        uint64_t *group_array, int64_t& group_size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid arguments, sstable schema "
        ...
        for (int64_t i = 0; i < column_id_size && OB_SUCCESS == iret; ++i)
        {
          ...
          else if (!column_group_exists(group_array, current_group_size, column_group_id))
          {
            if (current_group_size > group_size)
            {
              ...
              YYSYS_LOG(ERROR, "input group size =%ld not "
    ...
    int ObSSTableScanner::trans_input_scan_range(const ObSSTableScanParam &scan_param)
    {
      ...
      {
        YYSYS_LOG(ERROR, "input scan parmeter invalid, cannot scan any data.");
        iret = OB_INVALID_ARGUMENT;
      }
      ...
      if (OB_SUCCESS == iret && input_range.start_key_.is_min_row())// only end_key valid
      {
        ...
        {
          YYSYS_LOG(ERROR, "invalid end key, rang=%s", to_cstring(input_range));
          iret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == iret && input_range.end_key_.is_max_row())// only start_key valid
      {
        ...
        {
          YYSYS_LOG(ERROR, "invalid start key, range=%s", to_cstring(input_range));
          iret = OB_INVALID_ARGUMENT;
        }
      }
    ...
    int ObSSTableScanner::get_row_desc(const common::ObRowDesc *&row_desc) const
    {
      ...
      if (is_empty_sstable())
      {
        ...
        {
          YYSYS_LOG(WARN, "row_desc maybe not set:row_desc_[%ld]", row_desc_.get_column_num());
          ret = OB_NOT_INIT;
        }
        else
        {
    ...
    int ObSSTableScanner::build_row_desc_from_scan_param(const sstable::ObSSTableScanParam& scan_param)
    {
      ...
      for (int64_t i = 0; i < column_id_size && OB_SUCCESS == ret; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "add_column_desc table=%ld, column=%ld, ret=%d", 
