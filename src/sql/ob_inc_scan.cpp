  ...
  namespace sql
  {
    ...
    int ObIncScan::set_data_mark_param(const ObDataMarkParam &param)
    {
        ...
        if (!param.is_valid())
        {
          ...
          YYSYS_LOG(ERROR,"invalid data mark param[%s]!ret=%d",to_cstring(param),ret);
        }
        else
        {
    ...
    int ObIncScan::fill_scan_param_with_data_mark_param()const
    {
        ...
        {
            ret = OB_NOT_INIT;
            YYSYS_LOG(ERROR,"scan param can't be NULL!ret=%d",ret);
        }
        else
        {
    ...
    int ObIncScan::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      // add by maosy [MultiUps 1.0] [#20] 20170324 b
      else if (OB_SUCCESS != (err = serialization::encode_bool(buf, buf_len, new_pos, is_hot_update_)))
      {
        YYSYS_LOG(ERROR, "serialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, buf_len, err);
      }
      // add by maosy e
      ...
      else if (ST_MGET == scan_type_)
      {
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(ERROR, "values is invalid");
        }
        else if (NULL == get_param_)
        {
          if (NULL == const_cast<ObIncScan*>(this)->get_get_param())
          {
            YYSYS_LOG(WARN, "failed to get get_param");
        ...
        if (OB_LIKELY(OB_SUCCESS == err))
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to create get param, err=%d", err);
          }
          else
          {
...
//            }
            ...
            //add duyr 20160531:e
            YYSYS_LOG(DEBUG, "get_param=%s", to_cstring(*get_param_));
            FILL_TRACE_LOG("inc_version=%s", to_cstring(version_range));
            if (OB_SUCCESS != (err = get_param_->serialize(buf, buf_len, new_pos)))
            {
              YYSYS_LOG(ERROR, "get_param.serialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, buf_len, err);
            }
            else
            {
              get_param_->reset(true);
      ...
      else if (ST_SCAN == scan_type_)
      {
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(ERROR, "scan_param == NULL");
        }
        //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
        else if (OB_SUCCESS != (err = fill_scan_param_with_data_mark_param()))
        {
          YYSYS_LOG(WARN,"fail to fill scan param with data mark param!ret=%d",err);
        }
        //add duyr 20160531:e
        else if (OB_SUCCESS != (err = scan_param_->serialize(buf, buf_len, new_pos)))
        {
          YYSYS_LOG(ERROR, "scan_param.serialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, buf_len, err);
        }
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObIncScan::deserialize(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      // add by maosy [MultiUps 1.0] [#20] 20170324 b
      else if (OB_SUCCESS != (err = serialization::decode_bool(buf, data_len, new_pos, &is_hot_update_)))
      {
          YYSYS_LOG(ERROR, "deserialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, data_len, err);
      }
      // add by maosy e
      ...
      else if (ST_MGET == scan_type_)
      {
        ...
        {
          err = OB_MEM_OVERFLOW;
          YYSYS_LOG(ERROR, "get_param == NULL");
        }
        else if (OB_SUCCESS != (err = get_param_->deserialize(buf, data_len, new_pos)))
        {
          YYSYS_LOG(ERROR, "get_param.deserialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, data_len, err);
        }
      }
      else if (ST_SCAN == scan_type_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "scan_param.deserialize(buf=%p[%ld-%ld])=>%d", buf, new_pos, data_len, err);
        }
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObIncScan::create_get_param_from_values(common::ObGetParam* get_param)
    {
      ...
      {
        YYSYS_LOG(WARN, "values is invalid");
        ret = OB_NOT_INIT;
      }
      else if (NULL == (values = dynamic_cast<ObExprValues*>(my_phy_plan_->get_phy_query_by_id(values_subquery_id_))))
      {
        YYSYS_LOG(ERROR, "subquery not exist");
        ret = OB_ERR_UNEXPECTED;
      }
      //del shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150714:b conflict_level A
//      else if (OB_SUCCESS != (ret = values->open()))
//      {
//        YYSYS_LOG(WARN, "failed to open values, err=%d", ret);
//      }
      //del 20150714:e
      ...
      else
      {
        ...
        while (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get rowkey, err=%d", ret);
            break;
          }
          else
          {
            ...
            for (int64_t i = 0; i < cell_num; ++i)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to get cell, err=%d i=%ld", ret, i);
                break;
              }
              else
              {
                ...
                {//finish add real cid,begin and data mark cids!
                    ...
                    if (OB_SUCCESS == ret && data_mark_param_.need_modify_time_)
                    {
                        ...
                        if (OB_SUCCESS != (ret = get_param->add_cell(cell_info)))
                        {
                          YYSYS_LOG(WARN, "failed to add modify_time_cid into get param, err=%d", ret);
                        }
                    }
                    ...
                    if (OB_SUCCESS == ret && data_mark_param_.need_major_version_)
                    {
                        ...
                        if (OB_SUCCESS != (ret = get_param->add_cell(cell_info)))
                        {
                          YYSYS_LOG(WARN, "failed to add major_version_cid_ into get param, err=%d", ret);
                        }
                    }
                    ...
                    if (OB_SUCCESS == ret && data_mark_param_.need_minor_version_)
                    {
                        ...
                        if (OB_SUCCESS != (ret = get_param->add_cell(cell_info)))
                        {
                          YYSYS_LOG(WARN, "failed to add minor_ver_start_cid_ into get param, err=%d", ret);
                        }
                    }
                    ...
                    if (OB_SUCCESS == ret && data_mark_param_.need_minor_version_)
                    {
                        ...
                        if (OB_SUCCESS != (ret = get_param->add_cell(cell_info)))
                        {
                          YYSYS_LOG(WARN, "failed to add minor_ver_end_cid_ into get param, err=%d", ret);
                        }
                    }
                    ...
                    if (OB_SUCCESS == ret && data_mark_param_.need_data_store_type_)
                    {
                        ...
                        if (OB_SUCCESS != (ret = get_param->add_cell(cell_info)))
                        {
                          YYSYS_LOG(WARN, "failed to add data_store_type_cid_ into get param, err=%d", ret);
                        }
                    }
                }
                //add duyr 20160531:e
        ...
        //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160422:b
        /**
         * nomater what happened, reset the row store iterator for other op use
         */
        values->reset_row_store_iterator();
        //add 20160422:e
      }
      return ret;
    }
