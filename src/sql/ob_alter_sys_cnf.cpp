...
int ObAlterSysCnf::open()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(USER_ERROR, "No param to be changed, ret=%d", ret);
  }
  else if ((local_context_.schema_manager_ =
                local_context_.merger_schema_mgr_->get_user_schema(0)) == NULL)
  {
    ret = OB_ERROR;
    YYSYS_LOG(USER_ERROR, "Fail to get schema manager, ret=%d", ret);
  }
  //add liu jun.[MultiUPS] [part_cache] 20150626:b
  else if(NULL == (temp = dynamic_cast<ObMsPartitionManager *>
  ...
  else
  {
    for (int64_t i = 0; i < sys_cnf_items_.count(); i++)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(USER_ERROR, "Invalid server ip: %.*s",
 ...
 */
      ...
      databuff_printf(sql_buff, OB_MAX_VARCHAR_LENGTH, pos,
                      ...
                      "values(%ld, '%s', '%.*s', %lu, '%.*s', 'unknown', '%s', '",
                      OB_ALL_SYS_CONFIG_TABLE_NAME,
                      cnf_item.cluster_id_ != OB_ALL_CLUSTER_FLAG ? cnf_item.cluster_id_ : common::OB_ALL_CLUSTER_FLAG,
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "Fail to cast obj, err=%d", ret);
          break;
        }
        else if ((ret = res_value->get_varchar(val)) != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "Get casted value failed, err=%d", ret);
          break;
        }
      ...
      {
          YYSYS_LOG(INFO, "check the valid of tablet_replicas_num");
          int64_t out_val = 0;
          ObObj casted_cell;
          const ObObj * res_value= NULL;
          ObObj date_type;
          ...
          {
              YYSYS_LOG(ERROR, "get_int error [err:%d]", ret);
              break;
          }
          else if (out_val <= 0 || out_val > OB_MAX_COPY_COUNT)
          {
              YYSYS_LOG(WARN, "illegal value, err=%d", ret);
      ...
      {
        YYSYS_LOG(WARN, "Init temp result set failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = ObSql::direct_execute(sql_string, tmp_result, local_context_)))
      {
        YYSYS_LOG(USER_ERROR, "Direct_execute failed, sql=%.*s ret=%d",
                  sql_string.length(), sql_string.ptr(), ret);
        break;
      }
      else if (OB_SUCCESS != (ret = tmp_result.open()))
      {
        YYSYS_LOG(WARN, "Open result set failed, sql=%.*s ret=%d",
                  sql_string.length(), sql_string.ptr(), ret);
        break;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "Fail to update param %.*s, sql=%.*s",
                    cnf_item.param_name_.length(), cnf_item.param_name_.ptr(),
        ...
        */
        ...
        {
          YYSYS_LOG(WARN, "Failed to close temp result set, sql=%.*s ret=%d",
                    sql_string.length(), sql_string.ptr(), ret);
          break;
        }
      }
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      {
        YYSYS_LOG(WARN, "Commit transaction failed, ret=%d", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "fail to refresh new config. ret=%d", ret);
        }
        // to release resouce, call open & close
        else if (OB_SUCCESS != (ret = tmp_result.open()))
        {
          YYSYS_LOG(WARN, "fail to open tmp result. ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = tmp_result.close()))
        {
          YYSYS_LOG(WARN, "fail to close tmp result. ret=%d", ret);
        }
      }
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "Rollback transaction failed");
      }
    }
  }
  ...
  //add liu jun.[MultiUPS] [part_cache] 20150626:b
  if(local_context_.partition_mgr_ != NULL)
  {
    local_context_.part_monitor_->release_manager(local_context_.partition_mgr_);
    local_context_.partition_mgr_ = NULL;
  }
  //20150626:e
  return ret;
...
int ObAlterSysCnf::execute_transaction_stmt(const int& trans_type)
{
  ...
  switch (trans_type)
  {
    ...
    case ROLLBACK:
      ...
      break;
    default:
      YYSYS_LOG(WARN, "Wrong transaction type, trans_type=%d", trans_type);
      ret = OB_ERROR;
      break;
  }
  if (ret != OB_SUCCESS)
  ...
  {
    YYSYS_LOG(USER_ERROR, "Execute transaction stmt failed, sql=%.*s ret=%d",
              trans_string.length(), trans_string.ptr(), ret);
  }
  else if (OB_SUCCESS != (ret = tmp_result.open()))
  {
    YYSYS_LOG(WARN, "Open temp result set failed, sql=%.*s ret=%d",
              trans_string.length(), trans_string.ptr(), ret);
  }
  else
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "Transaction stmt should not return rows, ret=%d", ret);
    }
    if (tmp_result.close() != OB_SUCCESS)
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "Failed to close tmp result set, ret=%d", ret);
    }
  }
  return ret;
}
