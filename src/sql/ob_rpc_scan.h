...
//add  by fyd  [PrefixKeyQuery_for_INstmt]2014.4.1:b
  ...
  namespace sql
  {
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151230:b
    class ObReadAtomicRpcScan;
    //add duyr 20151230:e
    // 用于MS进行全表扫描
    class ObRpcScan : public ObPhyOperator
    {
      public:
        ...
        void set_read_consistency_param();
        //add:e
        ...
        int get_cur_row_desc(const common::ObRowDesc *&row_desc) const;
...
//                                                            ObSqlReadStrategy &sql_read_strategy);
        ...
        int set_left_table_id_column_id(int64_t table_id,int64_t column_id,
                                        int64_t right_column_id,int64_t right_table_id);
        ...
        //用于重复主键的过滤
        int get_data_for_distinct(const common::ObObj *cell, ObObjType type, char *buff, int buff_size);
        ...
        {return read_param_->add_sort_column_for_listagg(tid,cid,is_asc);}
        //add 20150104:e
        /**
         * 指定limit/offset
         *
		...
		*/
        int get_param_sub_query_expr(int32_t idx,ObSqlExpression* &expr,int &query_index)
        {return read_param_->get_param_sub_query_expr(idx,expr,query_index);};
		...
		* @return read_method 
		*/
        int32_t get_read_method()
        {return hint_.read_method_;};
		//add 20140409:e
        ...
        int set_trans_id(const ObTransID trans_id,int64_t paxos_id,int64_t published_transid = OB_INVALID_DATA);
        //add:e
        //add by maosy  [MultiUps 1.0] [#210]20170517 b:
        uint64_t get_main_table_id()
        {
            if(OB_INVALID_ID != main_table_id_)
            {
      ...
      protected:
        ...
        {return cur_row_desc_.get_rowkey_cell_count();}
        inline uint64_t get_base_table_id()const
        {return base_table_id_;}
        inline uint64_t get_table_id()const
        {return table_id_;}
        inline const common::ObRowkeyInfo &get_rowkey_info()const
        {return rowkey_info_;}
        inline common::ObReadAtomicParam *get_read_atomic_param()
        {
          common::ObReadAtomicParam *ret = NULL;
          if (NULL != read_param_)
          {
            ObReadAtomicParam &param = read_param_->get_read_atomic_param();
            ret = &param;
          }
          return ret;
        }
        inline const common::ObReadAtomicParam *get_read_atomic_param()const
        {
          const common::ObReadAtomicParam *ret = NULL;
          if (NULL != read_param_)
          {
            ret = &(read_param_->get_read_atomic_param());
          }
          return ret;
        }
        inline const ObSqlReadParam *get_sql_read_param()const
        {return read_param_;}
        inline ObSqlReadParam *get_sql_read_param()
        {return read_param_;}
        int reset_filter();
      private:
        friend  class ObReadAtomicRpcScan;
      //add duyr 20151230:e
      private:
        // disallow copy
        ...
        //add:e
        int set_is_result_cached(ObSqlReadParam &dest_param, const uint64_t table_id);
      ...
      private:
        ...
        int add_raw_filter(ObSqlExpression &expr);
        int add_right_table_filter(const ObObj *cell_temp, ObSqlExpression &table_filter_expr);
        int cons_scan_request(bool *can_break = NULL);
        int cons_right_table_filter(ObSqlExpression &table_filter_expr,bool &is_in_expr_empty);


      private:
        static const int64_t REQUEST_EVENT_QUEUE_SIZE = 8192;
        static const int64_t MAX_IN_EXPR_LIMIT_NUM = 50;
        // 等待结果返回的超时时间
        int64_t timeout_us_;
...
//add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
        ...
        ObSEArray<common::ObPartitionTransID,MAX_UPS_COUNT_ONE_CLUSTER> trans_id_;
		// add by maosy e
        //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
