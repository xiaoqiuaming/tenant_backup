...
void ObGetCurTimePhyOperator::reset()
{
  ...
  /*Exp: ��cur_time_fun_type_�޸�ΪObArray<ObCurTimeType>����*/
...
int ObGetCurTimePhyOperator::open()
{
  ...
  {
    YYSYS_LOG(DEBUG, "open: idx = [%ld], size = [%ld]", idx, size);
  //add 20150909:e
    if (CUR_TIME_UPS == cur_time_fun_type_.at(idx))
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to get_cur_time_from_ups, ret=%d", ret);
      }
    }
    //add wuna [datetime func] 20150902:b
    else if (CUR_DATE == cur_time_fun_type_.at(idx))
    {
      YYSYS_LOG(DEBUG, "CUR_DATE, cur_time_fun_type_=%d", cur_time_fun_type_.at(idx));
      ret = get_cur_time(CUR_DATE);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to get_cur_time_from_date, ret=%d", ret);
      }
    }
    //add 20150902:e
    ...
    {
      YYSYS_LOG(DEBUG, "CUR_TIME_HMS, cur_time_fun_type_=%d", cur_time_fun_type_.at(idx));
      ret = get_cur_time(CUR_TIME_HMS);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to get_cur_time_from_time, ret=%d", ret);
      }
    }
    //add 20150907:e
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to get_cur_time, ret=%d", ret);
      }
    }
...
int ObGetCurTimePhyOperator::get_cur_time_from_ups()
{
  ...
  if (NULL == rpc_ || NULL == ups_plan_ || NULL == my_phy_plan_)
  {
    ...
    YYSYS_LOG(ERROR, "rpc_=%p, ups_plan_=%p and my_phy_plan_=%p must not be NULL",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "execute ups plan on updateserver exit with error code: %d", ret);
    }
  }
  // 2. set result set
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to add column desc, ret=%d", ret);
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "ups plan should contain current_time row, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = row.get_cell(OB_INVALID_ID, OB_MAX_TMP_COLUMN_ID, cur_time)))
      {
        YYSYS_LOG(WARN, "failed to get curren time cell:ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = outer_result_set->set_cur_time(*cur_time)))
      {
        YYSYS_LOG(WARN, "failed to set cur_time_ups of result_set:cur_time: %s ret=%d", to_cstring(*cur_time), ret);
      }
    }
  }
  return ret;
...
//mod 20150902:b
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "my_phy_plan_ must not null");
  }
  ...
  {
    time_t t = static_cast<time_t>(yysys::CTimeUtil::getTime()/1000000L);//seconds
    struct tm gtm;
    localtime_r(&t, &gtm);
  ...
  {
  //add 20150902:e
    cur_time.set_precise_datetime(yysys::CTimeUtil::getTime());
  }

  if(OB_SUCCESS != (ret = outer_result_set->set_cur_time(cur_time)))
  {
    YYSYS_LOG(WARN, "failed to set cur_time of result_set:cur_time: %s ret=%d", to_cstring(cur_time), ret);
  }
  return ret;
}
...
PHY_OPERATOR_ASSIGN(ObGetCurTimePhyOperator)
{
  ...
  if (NULL != o_ptr->ups_plan_)
  {
    if ((ups_plan_ = ObPhysicalPlan::alloc()) == NULL)
    {
      ...
      YYSYS_LOG(WARN, "Can not generate new ups physical plan, ret=%d", ret);
    }
    else if ((ret = ups_plan_->assign(*o_ptr->ups_plan_)) != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "Assign ups physical plan, ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
