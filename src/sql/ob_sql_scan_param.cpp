...
//      {
//        YYSYS_LOG(WARN, "fail to serialize trans_id. ret=%d", ret);
//      }
      //add:e
	  // add by maosy e
      ...
      {
        YYSYS_LOG(WARN, "fail to serialize tablet_version. ret=%d", ret);
      }
      //add:e
      return ret;
    }
...
// add by maosy e
      ...
      {
        YYSYS_LOG(WARN, "fail to deserialize table_id_ obj. ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = get_int_obj_value(buf, data_len, pos, border_flag)))
      {
        YYSYS_LOG(WARN, "fail to deserialize border_flag_ obj. ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = get_rowkey_obj_array(buf, data_len, pos,
              start_rowkey_obj_array, start_key_len)))
      {
        YYSYS_LOG(WARN, "fail to get start rowkey.ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = get_rowkey_obj_array(buf, data_len, pos,
              end_rowkey_obj_array, end_key_len)))
      {
        YYSYS_LOG(WARN, "fail to get end rowkey. ret=%d", ret);
      }
...
//      {
//        YYSYS_LOG(WARN, "fail to deserialize trans_id. ret=%d", ret);
//      }
      //add:e
	  // add by maosy e
      //add zhaoqiong [fixbug:tablet version info lost when query from ms] 20170228:b
...
// add by maosy e
    ...
    DEFINE_SERIALIZE(ObSqlScanParam)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize ObSqlReadParam. ret=%d", ret);
        }
      }
      // BASIC_PARAM_FIELD
      if (OB_SUCCESS == ret)
      ...
      if (OB_SUCCESS == ret)
      { //���л� stmt_type
        ret = serialize_stmt_type(buf, buf_len, pos,stmt_type_);
      }
      //add 20150612:e
      // END_PARAM_FIELD
      if (OB_SUCCESS == ret)
    ...
    DEFINE_DESERIALIZE(ObSqlScanParam)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to deserialize ObSqlReadParam. ret=%d", ret);
      }
      while (OB_SUCCESS == ret)
      {
        do
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to deserialize meta type. ret=%d. data_len=%ld,pos=%ld", ret, data_len, pos);
          }
            ...
            case ObActionFlag::BASIC_PARAM_FIELD:
              {
                ...
                {
                  YYSYS_LOG(WARN, "fail to deserialize basic param. ret=%d", ret);
                }
                break;
              }
            //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150628:e
            case ObActionFlag::SQL_STMT_TYPE:
              {
                if(OB_SUCCESS != (ret = deserialize_stmt_type(buf,data_len,pos,stmt_type_)))
                {
                  YYSYS_LOG(WARN, "fail to deserialize stmt type . ret=%d", ret);
                }
                break;
              }
            //add 20150628:e
    ...
    DEFINE_GET_SERIALIZE_SIZE(ObSqlScanParam)
    {
      ...
      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150629:b
      total_size += get_obj_serialize_size(ObActionFlag::SQL_STMT_TYPE,true);
      total_size += get_obj_serialize_size((int64_t)stmt_type_);
      //add 20150629:e
      return total_size;
    }
    ...
    int ObSqlScanParam::assign(const ObSqlReadParam* other)
    {
      ...
      {
        YYSYS_LOG(WARN, "Assign ObSqlReadParam failed, ret=%d", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "Add ObRowkey failed, ret=%d", ret);
        }
        //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150629:b
        stmt_type_=o_ptr->get_stmt_type();
        //add 20150629:e
      }
      return ret;
    }
