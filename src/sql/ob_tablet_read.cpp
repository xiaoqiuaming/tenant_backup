...
int ObTabletRead::open()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "should create plan before open");
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "open tablets scan child operator fail. ret=%d", ret);
    }
  }
  return ret;
}
...
int ObTabletRead::close()
{
  ...
  if(NULL != op_root_)
  {
    ...
    {
      YYSYS_LOG(WARN, "close op_root fail:ret[%d]", ret);
    }
  }
...
int ObTabletRead::get_next_row(const ObRow *&row)
{
  ...
  if(OB_UNLIKELY(NULL == op_root_))
  {
    ...
    YYSYS_LOG(WARN, "op root is null");
  }
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      YYSYS_LOG(DEBUG, "tablet read row[%s]", to_cstring(*row));
    }
  }
  return ret;
}
...
int ObTabletRead::get_last_rowkey(const ObRowkey *&rowkey)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "last_rowkey_op_ is null");
  }
  return ret;
}
