...
int ObDoubleChildrenPhyOperator::set_child(int32_t child_idx, ObPhyOperator &child_operator)
{
  ...
  if (0 == child_idx)
  {
    ...
    {
      ret = OB_INIT_TWICE;
      YYSYS_LOG(ERROR, "left operator already init");
    }
    else
    {
  ...
  else if (1 == child_idx)
  {
    ...
    {
      ret = OB_INIT_TWICE;
      YYSYS_LOG(ERROR, "right operator already init");
    }
    else
    {
  ...
  else
  {
    ...
    YYSYS_LOG(ERROR, "invalid child idx=%d", child_idx);
  }
  return ret;
}
...
int ObDoubleChildrenPhyOperator::close()
{
  ...
  if (NULL != left_op_ && NULL != right_op_)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to close child(ren) operator(s), err=%d", ret);
    }
  }
  return ret;
}
