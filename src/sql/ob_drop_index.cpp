...
void ObDropIndex::set_sql_context(const ObSqlContext &context)
{
...
int ObDropIndex::open()
{
  ...
  //{
  //  ret=OB_NOT_INIT;
  //  YYSYS_LOG(ERROR,"not init,prc_=%p",rpc_);
  //}
  //在没有索引表可以删除情况下返回信息
  if(NULL == rpc_)
  {
  ...
  else if(0 >= indexs_.count())
  {
    ...
    YYSYS_LOG(WARN,"not index to drop");
  }
  //modify e
  else if (local_context_.session_info_->get_trans_start() || local_context_.session_info_->get_trans_id().is_valid() )
  {
    YYSYS_LOG(WARN, "drop index is not allowed in transaction, err=%d", ret);
    ret = OB_ERR_TRANS_ALREADY_STARTED;
  }
  else if(OB_SUCCESS != (ret = rpc_->drop_index(indexs_)))
  {
    YYSYS_LOG(WARN,"failed to drop index,err=%d",ret);
  }
  else
  {
    YYSYS_LOG(INFO,"drop index succ,tables=[%s]",
                  to_cstring(indexs_));
  }
  return ret;
}
