...
namespace oceanbase
{
  namespace sql
  {
    ...
    {
      YYSYS_LOG(INFO, "ObSqlGetParam use memory:%s, allocator:used:%ld,total:%ld,",
    ...
    int ObSqlGetParam::add_rowkey(const ObRowkey& rowkey, bool deep_copy_args)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid row key, key_len=%ld, key_ptr=%p",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (rowkey_list_.count() < max_row_capacity_)
        {
            ...
            {
              YYSYS_LOG(WARN, "fail to push rowkey to list. ret=%d", ret);
            }
        }
        else
        {
          YYSYS_LOG(INFO, "get param is full, can't add rowkey anymore, row_size=%ld, max=%d",
    ...
    int ObSqlGetParam::copy_rowkey(const ObRowkey &rowkey, ObRowkey &stored_rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to copy rowkey to local buffer [err:%d].[rowkey=%s]", err, to_cstring(rowkey));
      }
      return err;
    }
    ...
    /////////////////////////////////////////////////////////////////////////////
      ...
      //del shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150806:b
...
//      if(OB_SUCCESS == ret)
//      {
//          //YYSYS_LOG(INFO, "trans_id before serialize %s",to_cstring(trans_id_));
...
//              //add end
//              //YYSYS_LOG(INFO, "trans_id_.deserialize %s",to_cstring(trans_id_));
//          }
//          //add:e
// add by maosy e
      }
...
//      total_size += trans_id_.get_serialize_size();
      ...
      int64_t& pos, const RowkeyListType & rowkey_list) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize rowkey size. ret=%d", ret);
        }
        else
        {
          if (OB_SUCCESS == ret)
          {
            for (i = 0; OB_SUCCESS == ret && i < rowkey_list.count(); i++)
            {
              ...
              // YYSYS_LOG(DEBUG, "serialize rowkey[%ld]: %s. ret=%d", i, to_cstring(rowkey_list.at(i)), ret);
            }
          }
        }
      }
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to serialize rowkey list. buf=%p, buf_len=%ld, pos=%ld, rowkey list size=%ld",
      ...
      else
      {
        ...
        else
        {
          ...
          for (i = 0; i < size; i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to deserialize rowkey. buf=%p, data_len=%ld, pos=%ld, i=%ld, size=%ld",
                  buf, data_len, pos, i, size);
            }
            else if (OB_SUCCESS != (ret = add_rowkey(rowkey, true)))
            {
              YYSYS_LOG(WARN, "fail to add deserialized rowkey to get param. ret=%d, i=%ld, size=%ld", ret, i, size);
            }
          }
        }
      }
    ...
    DEFINE_SERIALIZE(ObSqlGetParam)
    {
      ...
      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150612:e
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG,"get_stmt_type:%ld",(int64_t)stmt_type_);
          ...
          if (ObExtendType == type)
          {
            ...
            case ObActionFlag::FORMED_ROW_KEY_FIELD:
              ...
              case ObActionFlag::SQL_STMT_TYPE:
                ret= deserialize_stmt_type(buf,data_len,pos,stmt_type_);
                break;
              //add 20150612:e
