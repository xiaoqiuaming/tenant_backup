  ...
  namespace sql
  {
    ...
    int ObTableRpcScan::open()
    {
      ...
      if (child_op_ == NULL)
      {
        //add zhujun [transaction read uncommit]2016/3/22
          ...
          if(my_phy_plan_ != NULL && my_phy_plan_->get_result_set() != NULL)
          {
              ...
              {
                  //                int64_t paxid = OB_INVALID_PAXOS_ID ;
                  //                int tmp_ret = OB_SUCCESS ;
                  for(int32_t i=0 ; i < MAX_UPS_COUNT_ONE_CLUSTER ;i++ )
                  {
                      ObTransID trans_id ;
                      session->get_participant_trans_info(i,trans_id);
                      int64_t published_transid = OB_INVALID_DATA;
                      if(my_phy_plan_->get_insert_select_flag())
                      {
                          published_transid = session->get_master_ups_transid(i);
                      }
                      if(trans_id.is_valid()|| published_transid!=OB_INVALID_DATA)
                      {
                          rpc_scan_.set_trans_id(trans_id,i,published_transid);
                      }
        ...
        {
          ret = OB_NOT_INIT;
          YYSYS_LOG(WARN, "must call init() before call open(). ret=%d", ret);
        }
        ...
        if (OB_SUCCESS == ret && (has_group_ || has_scalar_agg_))
        {
          ...
          {
            ret = OB_ERR_GEN_PLAN;
            YYSYS_LOG(WARN, "Group operator and scalar aggregate operator can not appear in TableScan at the same time. ret=%d", ret);
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to open table scan. ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObTableRpcScan::get_rpc_row_desc(const common::ObRowDesc *&row_desc) const
    {
        int ret = rpc_scan_.get_cur_row_desc(row_desc);
        return ret;
    }
    ...
    int ObTableRpcScan::init(ObSqlContext *context, const common::ObRpcScanHint *hint)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to init table rpc scan. params(null)");
    ...
    int ObTableRpcScan::add_main_output_column(const ObSqlExpression& expr)
    {
        ...
        if (OB_SUCCESS == ret)
        {
         ...
         // {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to add column to rpc scan operator. ret=%d", ret);
          }
        }
        return ret;
    }
    ...
    int ObTableRpcScan::add_main_filter(ObSqlExpression* expr)
    {
        ...
        else
        {
            *expr_clone = *expr;
            ...
            if (OB_SUCCESS != ret)
            {
                YYSYS_LOG(WARN, "add main filter failed.ret=%d",ret);
                if (NULL != expr_clone)
                {
        ...
        else
        {
          ...
          *expr_clone = *expr;
          ...
          if (OB_SUCCESS != (ret = select_get_filter_.add_filter(expr_clone)))
          {
            ...
            YYSYS_LOG(WARN, "fail to add filter to filter for select get. ret=%d", ret);
          }
        }
        return ret;
    }
    ...
    int ObTableRpcScan::add_index_filter_ll(ObSqlExpression* expr)
    {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to add filter to rpc scan operator. ret=%d", ret);
          }
        }
        return ret;
    }
    ...
    int ObTableRpcScan::add_group_column(const uint64_t tid, const uint64_t cid, bool change_tid_for_storing)//mod liumz, [optimize group_order by index]20170419
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if ((ret = group_columns_sort_.add_sort_column((is_use_index_for_storing&&change_tid_for_storing)?main_tid_:tid, cid, true))
                  != OB_SUCCESS)//mod liumz, [optimize group_order by index]20170419
          {
            YYSYS_LOG(WARN, "Add sort column of TableRpcScan sort operator failed. ret=%d", ret);
          }
          ...
          {
            YYSYS_LOG(WARN, "Add group column of TableRpcScan group operator failed. ret=%d", ret);
          }
          else
          {
            has_group_ = true;
    ...
    int ObTableRpcScan::add_sort_column_for_listagg(const uint64_t tid, const uint64_t cid,bool is_asc)
    {
        ...
        {
          YYSYS_LOG(ERROR,"Add sort column to sort plan failed");
        }
        if(OB_SUCCESS == ret)
        {
    ...
    int ObTableRpcScan::add_aggr_column(const ObSqlExpression& expr)
    {
      ...
      {
        YYSYS_LOG(WARN, "Get aggregate function type failed. ret=%d", ret);
      }
      else if (is_distinct)
      {
        ret = OB_ERR_GEN_PLAN;
        YYSYS_LOG(WARN, "Distinct aggregate function can not be processed in TableRpcScan. ret=%d", ret);
      }
      else if (aggr_type == T_FUN_AVG)
      {
        ...
        ret = OB_ERR_GEN_PLAN;
        YYSYS_LOG(WARN, "Avg() aggregate function can not appears in TableRpcScan. ret=%d", ret);
      }
      else
      {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "Generate local aggregate function of TableRpcScan failed. ret=%d", ret);
        }
      ...
      if (ret == OB_SUCCESS)
      {
        if (has_group_)
        {
          ...
          {
            YYSYS_LOG(WARN, "Add aggregate function to TableRpcScan group operator failed. ret=%d", ret);
          }
        }
        else
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "Add aggregate function to TableRpcScan scalar aggregate operator failed. ret=%d", ret);
            }
          }
        }
      }
    ...
    int ObTableRpcScan::set_table(const uint64_t table_id, const uint64_t base_table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to add table id to rpc scan operator. table_id=%lu, ret=%d", base_table_id, ret);
      }
      return ret;
    }
    ...
    int ObTableRpcScan::set_limit(const ObSqlExpression& limit, const ObSqlExpression& offset)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to set limit. ret=%d", ret);
      }
      else
      {
        has_limit_ = true;
        ...
        if (!is_use_index_rpc_scan_)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Fail to add limit/offset to rpc scan operator. ret=%d", ret);
          }
        }
      }
      return ret;
    ...
    int64_t ObTableRpcScan::to_string(char* buf, const int64_t buf_len) const
    {
      ...
      databuff_printf(buf,
                      buf_len,
                      pos,
                      "TableRpcScan(read_method=%s, is use index for storing=%d,index tid=%ld; is use index without storing=%d,index tid=%ld;is_indexed_group_=%d; ",
                      ...
                      is_indexed_group_);//add liumz, [optimize group_order by index]20170419
      //modify e
      if (has_limit_)
      {
    ...
    */
	...
	{
	  YYSYS_LOG(WARN, "fail to remove sub query expr from rpc scan. ret=%d", ret);
	}
	else if(OB_SUCCESS != (ret = select_get_filter_.remove_or_sub_query_expr()))
	{
	  YYSYS_LOG(WARN, "fail to remove sub query expr filter. ret=%d", ret);
	}
	return ret;
    }
    ...
    PHY_OPERATOR_ASSIGN(ObTableRpcScan)
    {
      ...
      else
      {
        if (o_ptr->scalar_agg_)
        {
          ...
          {
            YYSYS_LOG(WARN, "Assign scalar_agg_ failed. ret=%d", ret);
          }
        }
        if (o_ptr->group_)
        {
          ...
          {
            YYSYS_LOG(WARN, "Assign group_ failed. ret=%d", ret);
          }
        }
...
#define ENCODE_OP(has_op, op) \
      ...
      { \
        ...
        { \
          YYSYS_LOG(WARN, "fail to encode " #has_op ":ret[%d]", ret); \
        } \
        else if (has_op) \
        { \
          if (OB_SUCCESS != (ret = op.serialize(buf, buf_len, pos))) \
          { \
            YYSYS_LOG(WARN, "fail to serialize " #op ":ret[%d]", ret); \
          } \
        } \
      }
...
#define DECODE_OP(has_op, op) \
      ...
      { \
        ...
        { \
          YYSYS_LOG(WARN, "fail to decode " #has_op ":ret[%d]", ret); \
        } \
        else if (has_op) \
        { \
          if (OB_SUCCESS != (ret = op.deserialize(buf, data_len, pos))) \
          { \
            YYSYS_LOG(WARN, "fail to deserialize " #op ":ret[%d]", ret); \
          } \
        } \
      }
