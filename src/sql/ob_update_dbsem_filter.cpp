  ...
  namespace sql
  {
    ...
    int ObUpdateDBSemFilter::open()
    {
      ...
      else
      {
        ...
        while(OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to get duplicated row:ret[%d]", ret);
          }
          */
          else
          { // uncertainty 整个文件的作用是单行更新检查主键重复，去除updaterowkey
            if(NULL != row /*&& row->is_new_row()*/)
            {
              ...
              YYSYS_LOG(USER_ERROR, "Duplicate entry \'%s\' for key \'PRIMARY\'", rowkey_buf);
    ...
    int ObUpdateDBSemFilter::close()
    {
      ...
      if (OB_SUCCESS != (ret = ObSingleChildPhyOperator::close()))
      {
        if (!IS_SQL_ERR(ret))
        {
          YYSYS_LOG(WARN, "failed to close child_op, err=%d", ret);
        }
      }
      else
      {
