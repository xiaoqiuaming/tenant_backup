...
int ObUpdate::open()
{
  ...
  if (OB_INVALID_ID == table_id_
      ...
      || NULL == rpc_)
  {
    YYSYS_LOG(WARN, "not init, tid=%lu, rpc=%p, row column num[%ld]",
              table_id_, rpc_, cols_desc_.get_column_num());
  }
  else if (OB_SUCCESS != (ret = ObSingleChildPhyOperator::open()))
  {
    YYSYS_LOG(WARN, "failed to open child op, err=%d", ret);
  }
  else
  {
    mutator_.use_ob_sem();
    ...
    {
      YYSYS_LOG(WARN, "fail to reset mutator. ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = update_by_mutator()))
    {
      YYSYS_LOG(WARN, "fail to update by mutator. ret=%d", ret);
    }
  }
  return ret;
}
...
int ObUpdate::add_update_expr(const uint64_t column_id, const ObSqlExpression &expr)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to add column id to ObUpate operator. ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = update_column_exprs_.push_back(expr)))
  {
    YYSYS_LOG(WARN, "fail to add column expression to ObUpate operator. ret=%d", ret);
  }
  else
  {
...
int ObUpdate::update_by_mutator()
{
        ...
        && OB_SUCCESS == (ret = child_op_->get_next_row(row)))
  {
    ...
    for (i = 0; i < update_column_ids_.count(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get data type from row desc, err=%d column_id=%lu", ret, column_id);
        break;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to calc %ld column value. ret=%d", i, ret);
        }
        else if (NULL == cell)
        {
          YYSYS_LOG(WARN, "null cell");
          ret = OB_ERR_UNEXPECTED;
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "fail to add table %lu item %s to update mutator. ret=%d", table_id_, to_cstring(rowkey), ret);
          break;
        }
        if (0)
        {
          YYSYS_LOG(DEBUG, "mutator.update() rowkey=%s, column_id=%lu, cell=%s, i=%ld",
  ...
  if (OB_ITER_END != ret)
  {
    YYSYS_LOG(WARN, "failed to cons mutator, err=%d", ret);
  }
  else
  {
    ret = OB_SUCCESS;
    ...
    ObRowkey &rowkey)
{
  ...
  {
    YYSYS_LOG(WARN, "rowkey_obj array is too small. size=%ld, expect %ld", obj_size, rowkey_info.get_size());
    ret = OB_INVALID_ARGUMENT;
  }
  else if (NULL == rowkey_objs)
  {
    YYSYS_LOG(WARN, "invalid argument. rowkey_objs=null");
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    for (int64_t i = 0; i < rowkey_info.get_size(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get primary key column, err=%d cid=%lu", ret, cid);
        break;
      }
      else if (OB_SUCCESS != (ret = row.get_cell(table_id, cid, cell)))
      {
        YYSYS_LOG(WARN, "failed to get rowkey cell, err=%d tid=%lu cid=%lu", ret, table_id, cid);
        break;
      }
      else if (cell->is_null())
      {
        YYSYS_LOG(WARN, "rowkey column cannot be null, tid=%lu cid=%lu", table_id, cid);
        ret = OB_ERR_UNEXPECTED;
        break;
      }
      else
