...
int ObResultSet::Field::deep_copy(ObResultSet::Field &other, ObStringBuf *str_buf)
{
  ...
  if (NULL == str_buf)
  {
    YYSYS_LOG(WARN, "invalid argument str_buf = %p", str_buf);
  }
  else
  {
    if (OB_SUCCESS != (ret = str_buf->write_string(other.tname_, &tname_)))
    {
      YYSYS_LOG(WARN, "ObStringBuf write Fileld.tname_ string error, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = str_buf->write_string(other.org_tname_, &org_tname_)))
    {
      YYSYS_LOG(WARN, "ObStringBuf write Fileld.org_tname_ string error, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = str_buf->write_string(other.cname_, &cname_)))
    {
      YYSYS_LOG(WARN, "ObStringBuf write Fileld.cname_ string error, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = str_buf->write_string(other.org_cname_, &org_cname_)))
    {
      YYSYS_LOG(WARN, "ObStringBuf write Fileld.org_cname_ string error, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = str_buf->write_string(other.org_tname_, &org_tname_)))
    {
      YYSYS_LOG(WARN, "ObStringBuf write Fileld.tname_ string error, ret=%d", ret);
...
ObResultSet::~ObResultSet()
{
  ...
  {
    YYSYS_LOG(DEBUG, "destruct physical plan, addr=%p", physical_plan_);
  ...
  {
    YYSYS_LOG(DEBUG, "destruct physical plan cons from assign, addr=%p", physical_plan_);
...
int ObResultSet::open()
{
  ...
  if (OB_UNLIKELY(NULL == physical_plan_))
  {
    ...
    {
      YYSYS_LOG(WARN, "physical_plan not init, stmt_type=%d", stmt_type_);
  ...
  else
  {
    ...
    if (my_session_->get_trans_id().is_valid())
    {
      ...
      int64_t stmt_interval = yysys::CTimeUtil::getTime() - my_session_->get_curr_trans_last_stmt_time();
      OB_STAT_INC(OBMYSQL, SQL_MULTI_STMT_TRANS_STMT_INTERVAL, stmt_interval);
      OB_STAT_INC(OBMYSQL, SQL_MULTI_STMT_TRANS_STMT_COUNT);
      YYSYS_LOG(DEBUG, "stmt begin in transaction, start=%ld last_stmt=%ld interval=%ld",
  ...
  //mod:e
	...
	//add 20140713 :e
    physical_plan_->set_timeout_timestamp(plan_timeout + yysys::CTimeUtil::getTime());
    ...
    /**
     * for retry,if the plan execute failed for the reason that the versionrange is not serial.remember reset.
     */
    my_session_->set_retry_plan_timeout(physical_plan_->get_timeout_timestamp());
    //add 20160316:e
    ...
    if (NULL != pre_query)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to open pre_query, ret=%d", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      if (NULL == main_query)
      {
        ...
        YYSYS_LOG(ERROR, "main query must not be NULL");
      }
      else
      {
        ret = main_query->open();
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to open main query, ret=%d", ret);
        }
      }
    }
  }
...
int ObResultSet::close()
{
  ...
  else
  {
    ...
    if (NULL != pre_query)
    {
      ...
      {
        ret = tmp_ret;
        YYSYS_LOG(WARN, "failed to close pre_query, ret=%d", ret);
      }
    }
    ...
    {
      ret = tmp_ret;
      YYSYS_LOG(WARN, "failed to close main_query, ret=%d", ret);
    }
    ...
    if (my_session_->get_trans_id().is_valid())
    {
      ...
      YYSYS_LOG(DEBUG, "stmt end in transaction, start=%ld now=%ld", my_session_->get_curr_trans_start_time(), now);
    }
  }
  set_errcode(ret);
  return ret;
...
int ObResultSet::pre_assign_params_room(const int64_t& size, common::ObIAllocator &alloc)
{
  ...
  YYSYS_LOG(DEBUG, "stmt_id=%lu param_count=%ld param_msize=%ld paramc_c=%ld paramc_s=%ld",
...
//mod 20140306:e
{
  ...
  else
  {
    if (0 < types.count())
    {
      ...
      // bound types
      params_type_ = types;
      YYSYS_LOG(DEBUG, "stmt_id=%lu paramt_count=%ld paramt_msize=%ld",
	...
	//add 20140306:e
    //YYSYS_LOG(DEBUG, "execute with params=%s", to_cstring(values));
    for (int64_t i = 0; ret == OB_SUCCESS && i < params_.count(); i++)
    {
      *params_.at(i) = values.at(i); // shallow copy
      YYSYS_LOG(DEBUG, "params is %p execute with params=%s", params_.at(i), to_cstring(*params_.at(i)));
    }
...
int ObResultSet::from_store(ObPsStoreItemValue *value, ObPsSessionInfo *info)
{
  ...
  if (NULL == value)
  {
    YYSYS_LOG(ERROR, "invalid argument value is %p", value);
