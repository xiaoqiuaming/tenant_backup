  ...
  namespace sql
  {
    ...
    class ObSqlExpression: public common::DLink
    {
      public:
        ...
        void set_listagg_delimeter(ObString &listagg_delimeter)
        {listagg_delimeter_ = listagg_delimeter;}//add gaojt [ListAgg][JHOBv0.1]20150104
        ObString get_listagg_delimeter()const
        {return listagg_delimeter_;}//add gaojt [ListAgg][JHOBv0.1]20150104
        void reset();
        ...
        //add dragon e
        inline bool is_not_equal_cond(uint64_t &table_id);
        /**
         * 根据表达式语义对row的值进行计算
         *
         * @param row [in] 输入行
     ...
     //add 20150414:e
	   ...
	   inline bool set_post_expr( ObPostfixExpression &other)
	   {
		...
		if ( other.is_empty() )
		{
			YYSYS_LOG(WARN, "post_expr should not be empty!");
			ret = false ;
		}
		else
		{
		...
		*/
        ...
        void add_sub_query_num()
        {sub_query_num_++;};

		/*Exp:get sub_query num in current expression
		* @return sub_query_num
		*/
        int64_t get_sub_query_num()
        {return sub_query_num_;};
		...
		*/
        int32_t get_sub_query_idx(int32_t idx)
        {return sub_query_idx_[idx] ;};
		...
		*/
        ...
        {return post_expr_.add_expr_in_obj(obj_value);};
		...
		*/
        ...
        {return post_expr_.complete_sub_query_info();};

		/*Exp: get address of bloomfilter
		* @param bloom_filter[out]
		*/
        int get_bloom_filter(ObBloomFilterV1 *& bloom_filter)
        {
		...
		*/
        ...
        {return post_expr_.get_sub_query_column_num(sub_query_index-direct_bind_query_num_,num,special_sub_query);};
        ...
        */
        ...
        {return post_expr_.get_sub_query_column_num_new(sub_query_index-direct_bind_query_num_,num,special_sub_query,special_case);};
        //add e:
        /*Exp:check whether current expression satisfy the condition of use bloomfilter */
        bool is_satisfy_bloomfilter()
        {return post_expr_.is_satisfy_bloomfilter();};// add 20140523
		...
		*/
        ...
        void set_is_update(bool is_update)
        {is_update_ = is_update;};
        bool get_is_update()
        {return is_update_;};
        void add_question_num(int64_t step)
        {questionmark_num_in_update_assign_list_ += step;};
        int64_t get_question_num()
        {return questionmark_num_in_update_assign_list_;};
        void reset_question_num()
    ...
    inline int ObSqlExpression::get_aggr_column(ObItemType &aggr_fun, bool &is_distinct) const
    {
      ...
      if (!is_aggr_func_)
      {
        ...
        YYSYS_LOG(ERROR, "this expression is not an aggr function");
      }
      else
      {
        aggr_fun = aggr_func_;
    ...
    //add dragon e
    inline bool ObSqlExpression::is_not_equal_cond(uint64_t &table_id)
    {
        return post_expr_.is_not_equal_cond(table_id);
    }
