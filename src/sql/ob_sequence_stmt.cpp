                                                ...
                                                const char* const node_name, uint8_t datatype, int option)
{
  ...
  if (node->num_child_ <= 0)
  {
    if (MinValue == option)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Generate %.*s deciaml value failed.", name.length(), name.ptr());
      }
    }
    else
    {
  ...
  else
  {
    ...
    switch (middle_node->type_)
    {
      ...
      default:
        ...
        YYSYS_LOG(USER_ERROR, "An unexpected token was found in '%.*s'.", name.length(), name.ptr());
    }
    if (OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_ERR_PARSE_SQL;
        YYSYS_LOG(USER_ERROR, "The scale of the decimal number must be zero. In '%.*s'.", name.length(), name.ptr());
      }
      else if (OB_SUCCESS != (ret = res.from(middle_node->str_value_)))
      {
        YYSYS_LOG(ERROR, "Fail to convert %.*s to deciaml.", name.length(), name.ptr());
      }
...
int ObSequenceStmt::sequence_option_validity(int64_t dml_type)
{
  ...
  if (T_SEQUENCE_CREATE == dml_type)
  {
    for (int8_t i = 0; OB_SUCCESS == ret && i < OptionFlag; ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Option [%d] of sequence is not set.", i);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (0 == data_type_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Convert boundary value of int32 to decimal failed.");
      }
    }
    else if (64 == data_type_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Convert boundary value of int64 to decimal failed.");
      }
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "Generate boundary value of decimal failed.");
      }
    }
    if (start_with_ < lower_bound || start_with_ > upper_bound)
    {
      YYSYS_LOG(USER_ERROR, "The value of startwith out of range of data type!");
      ret = OB_ERR_PARSE_SQL;
    }
    else if (increment_by_ < lower_bound || increment_by_ > upper_bound)
    {
      YYSYS_LOG(USER_ERROR, "The value of incrementby out of range of data type!");
      ret = OB_ERR_PARSE_SQL;
    }
    else if (min_value_ < lower_bound || min_value_ > upper_bound)
    {
      YYSYS_LOG(USER_ERROR, "The value of minvalue out of range of data type!");
      ret = OB_ERR_PARSE_SQL;
    }
    else if (max_value_ < lower_bound || max_value_ > upper_bound)
    {
      YYSYS_LOG(USER_ERROR, "The value of maxvalue out of range of data type!");
      ret = OB_ERR_PARSE_SQL;
    }
  }
  if (OB_SUCCESS == ret && min_value_ > max_value_)
  {
    YYSYS_LOG(USER_ERROR, "Minvalue must equal or small than maxvalue.");
    ret = OB_ERR_PARSE_SQL;
  }
  return ret;
}
