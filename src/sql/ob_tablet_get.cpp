...
void ObTabletGet::reset()
{
  ...
  //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20151231:b
  //op_ups_multi_get_.reset();
  op_multi_ups_multi_get_.reset();
  //mod 20151231:e
...
//  {
//      YYSYS_LOG(DEBUG,"mul_del::debug,CS get real data mark param=[%s]",
...
//  }
  ...
  {
    YYSYS_LOG(DEBUG, "cs select basic column ids [%ld], project[%s]", basic_columns.count(),
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    get_param_.set_read_atomic_param(sql_get_param_->get_read_atomic_param());
    if (get_param_.get_read_atomic_param().is_valid())
    {
      YYSYS_LOG(DEBUG,"read_atomic::debug,CS real read atomic param from MS get param!param=[%s]",
              to_cstring(get_param_.get_read_atomic_param()));
    }
    //add duyr 20151202:e
    for (int64_t i = 0; OB_SUCCESS == ret && i < sql_get_param_->get_row_size(); i ++)
    {
      ...
      for (int64_t col_id_idx = 0; OB_SUCCESS == ret && col_id_idx < basic_columns.count(); col_id_idx ++)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to add cell to get param:ret[%d]", ret);
        }
      }
    }
    YYSYS_LOG(DEBUG, "cs get param row size[%ld] sql_get_param row size[%ld]", get_param_.get_row_size(),
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    op_sstable_get_.set_read_atomic_param(sql_get_param_->get_read_atomic_param());
    //add duyr 20160506:e
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if (sql_get_param_->get_data_version() != OB_NEWEST_DATA_VERSION)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "The request version is not exist: request version[%ld], sstable version[%ld]", sql_get_param_->get_data_version(), data_version);
        }
        else if (sql_get_param_->get_data_version() == data_version)
        {
          last_rowkey_op_ = &op_sstable_get_;
...
//                             sql_get_param_->get_data_version())))
  ...
  if (OB_SUCCESS == ret && sql_get_param_->has_project())
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "cs get project desc[%s]", to_cstring(*op_project));
        op_root_ = op_project;
      }
    }
  }
  ...
  if (OB_SUCCESS != ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to close op sstable get:err[%d]", err);
    }
  }
    ...
    )
{
  ...
  ObMultiUpsMultiGet *op_multi_ups_multi_get = NULL;
  //mod 20151231:e
  ...
  if(OB_SUCCESS == ret)
  {
      ...
      {
          YYSYS_LOG(DEBUG,"gaojt test start version is = %ld",start_data_version);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    op_multi_ups_multi_get = &op_multi_ups_multi_get_;
    //mod 20151231:e
    ups_mget_row_desc_.reset();
    //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20151231:b
    ...
    //}
    if(OB_SUCCESS != (ret = op_multi_ups_multi_get->set_rpc_proxy(rpc_proxy_)))
    {
      YYSYS_LOG(WARN, "ups get set ups rpc stub fail:ret[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = op_multi_ups_multi_get->set_ts_timeout_us(ts_timeout_us_)))
    {
      YYSYS_LOG(WARN, "set ups get timeout fail:ret[%d]", ret);
    }
    //mod 20151231:e
    else if (OB_SUCCESS != (ret = ObNewScannerHelper::get_row_desc(get_param_, false, ups_mget_row_desc_)))
    {
      YYSYS_LOG(WARN, "fail to get row desc:ret[%d] get_param=%s", ret, to_cstring(get_param_));
    }
    else
    {
      get_param_.set_is_read_consistency(sql_get_param_->get_is_read_consistency());
      ...
      ////add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150806:b
      //op_ups_multi_get->set_stmt_type(sql_get_param_->get_stmt_type());
      //op_ups_multi_get->set_schema_manager(&schema_mgr);
      ////add 20150806::e
      op_multi_ups_multi_get->set_row_desc(ups_mget_row_desc_);
      op_multi_ups_multi_get->set_get_param(get_param_);
      op_multi_ups_multi_get->set_stmt_type(sql_get_param_->get_stmt_type());
      op_multi_ups_multi_get->set_schema_manager(&schema_mgr);
      //mod 20151231:e
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else if(OB_SUCCESS != (ret = op_tablet_get_fuse->set_incremental_get(op_multi_ups_multi_get)))
    //mod 20151231:e
    {
      YYSYS_LOG(WARN, "set incremental scan fail:ret[%d]", ret);
    }
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151223:b
    else if (OB_SUCCESS != (ret = op_tablet_get_fuse->set_read_atomic_param(sql_get_param_->get_read_atomic_param())))
    {
      YYSYS_LOG(WARN,"fail to set read atomic param to tablet_get_fuse!ret=%d",ret);
    }
    else if(OB_SUCCESS != (ret = op_tablet_get_fuse->set_get_param(get_param_)))
    {
      YYSYS_LOG(WARN, "fail to set get_param to tablet_get_fuse!ret=%d", ret);
    }
    //add duyr 20151223:e
    else
    {
  ...
  if(ret == OB_SUCCESS)
  {
    YYSYS_LOG(DEBUG, "get_param.get_trans_id():%s",to_cstring(const_cast<ObSqlGetParam *>(sql_get_param_)->get_trans_id()));
    op_multi_ups_multi_get_.set_trans_id(const_cast<ObSqlGetParam *>(sql_get_param_)->get_trans_id());
  }
  //add:e
  return ret;
}
...
bool ObTabletGet::check_inner_stat() const
{
  ...
  {
    ret = false;
    YYSYS_LOG(WARN, "tablet_manager_ [%p], sql_get_param_[%p]",
...
int ObTabletGet::set_tablet_manager(chunkserver::ObTabletManager *tablet_manager)
{
  ...
  if (NULL == tablet_manager)
  {
    ...
    YYSYS_LOG(WARN, "tablet_manager is null");
  }
  else
  {
