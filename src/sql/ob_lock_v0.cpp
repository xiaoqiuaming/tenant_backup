...
namespace oceanbase
{
  namespace sql
  {
                                     ...
                                     )
    {
      ...
      {
        ret = OB_SCHEMA_ERROR;
        YYSYS_LOG(ERROR,"can not get the table schema");
      }
      else
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN,"the name of table locked must not be NULL");
        }
        else
        {
      ...
      if(OB_SUCCESS==ret)
      {
        ...
        for (rowkey_idx = 0; OB_SUCCESS == ret && rowkey_idx < rowkey_num; rowkey_idx++)
        {
          ...
          else
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN, "fail to get the %ld th cell",rowkey_column_id);
            }
            ...
            else
            {
              ...
              memset(column_value, 0, temp_column_value.length()+1);
              ...
              delete []column_value;
              YYSYS_LOG(INFO,"column_expr=%s",column_expr.data());
            }
          }
        }
        if(OB_SUCCESS == ret)
        {
          if(rowkey_idx>0)
          {
            ...
            YYSYS_LOG(INFO,"where part=%s",where_part.data());
          }
          else
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR,"there must be at least one column");
          }
        }
      }
      //生成加锁语句的物理计划
      if(OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO,"TEMP LOCK SQL=%s",temp_lock_sql.data());
        YYSYS_LOG(INFO,"LOCK SQL=%.*s",lock_sql.length(),lock_sql.ptr());
        if (OB_SUCCESS == ret)
        {
        ...
        else
        {
          ...
          else
          {
            ...
            if(OB_SUCCESS == ret)
            {
              ...
              {
                ret = OB_NOT_INIT;
                YYSYS_LOG(ERROR, "Stored session plan cursor can not be found or not correct, physical_plan=%p main_query=%p"\
