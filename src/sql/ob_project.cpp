...
int ObProject::add_nextval_for_update(uint64_t *nextval, uint32_t p, uint32_t s, uint32_t vs, uint32_t len)
{
  int ret = OB_SUCCESS;
  ObObj nextval_obj;
  if (OB_SUCCESS != (ret = nextval_obj.set_decimal(nextval,p,s,vs,len)))
  {
    YYSYS_LOG(ERROR, "set nextval failed!");
  }
  else if (OB_SUCCESS != (ret = sequence_values_.push_back(nextval_obj)))
  {
    YYSYS_LOG(ERROR, "push nextval to sequence values array failed,ret=[%d]",ret);
  }
  return ret;
}
...
int ObProject::check_the_sequence_validity()
{
      ...
      && (idx_of_expr_with_sequence_.count() == idx_of_out_put_columns_with_sequence_.count()))
  {
    for (int64_t i=0; i < sequence_values_.count(); i++)//FOR DEBUG
    {
      ...
      YYSYS_LOG(DEBUG,
                "the value is::[%s], the seq idx is::[%ld], the  expr idx is::[%ld]",
                to_cstring(values),
                idx_of_expr_with_sequence_.at(i),
                idx_of_out_put_columns_with_sequence_.at(i));
    }
    YYSYS_LOG(DEBUG, "the sequence count int one row is::[%ld]",sequence_count_in_one_row_);
    //do nothing
  }
  else
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "the sequence info is not correct!");
  }
  return ret;
}
...
void ObProject::dump()
{
  for(int64_t i=0;i<sequence_values_.count();i++)
  {
    YYSYS_LOG(INFO, "sequence_values_[%ld]:%s",i,to_cstring(sequence_values_.at(i)));
  }
}
...
int ObProject::fill_sequence_info(common::ObSEArray<ObSqlExpression, OB_PREALLOCATED_NUM, common::ModulePageAllocator, ObArrayExpressionCallBack<ObSqlExpression> > &out_put_columns)
{
  ...
  if (OB_SUCCESS == ret && sequence_values_.count() > 0)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "row_num_idx_:%ld",row_num_idx_);
      ...
      uint64_t *sequence_value = NULL;
      uint32_t p,s,vs,len;
      if (OB_SUCCESS != (ret = sequence_values_.at(idx).get_ttint(sequence_value)))
      {
        YYSYS_LOG(WARN, "get sequence value failed, ret=%d",ret);
        break;
      }
      else
      {
          p = sequence_values_.at(idx).get_precision();
          s = sequence_values_.at(idx).get_scale();
          vs = sequence_values_.at(idx).get_vscale();
          len = sequence_values_.at(idx).get_nwords();
      }
      if (OB_SUCCESS != (ret = expr_array_obj->set_decimal(sequence_value,p,s,vs,len)))
      {
        YYSYS_LOG(WARN, "set sequence value failed, ret=%d",ret);
        break;
      }
    }
    row_num_idx_++;
...
int ObProject::cons_row_desc()
{
  ...
  if(0 != row_desc_.get_column_num())
  {
    ...
    YYSYS_LOG(WARN, "row desc should be empty");
  }
  ...
  if (0 >= columns_.count())
  {
    ...
    YYSYS_LOG(ERROR, "no column for output");
  }
  else
  {
...
int ObProject::open()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to construct row desc, err=%d", ret);
  }
  else
  {
...
int ObProject::get_next_row(const common::ObRow *&row)
{
  ...
  else
  {
    ...
    if(NULL != row_store_pre_)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to add row store,ret = %d", ret);
        return ret;
      }
    }
    //add e
    ...
    {
      YYSYS_LOG(ERROR, "fill sequence info failed!, ret=%d",ret);
    }
    //add 20150911:e
    else
    {
      YYSYS_LOG(DEBUG, "PROJECT ret=%d op=%p type=%d %s",
                ret, child_op_, child_op_->get_type(), (NULL == input_row) ? "nil" : to_cstring(*input_row));
      const ObObj *result = NULL;
      for (uint32_t i = 0;i < columns_.count(); ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to set row cell, err=%d", ret);
          break;
        }
      } // end for
    }
...
int64_t ObProject::to_string(char* buf, const int64_t buf_len) const
{
  ...
  databuff_printf(buf, buf_len, pos, "])(sequence_values_=[");
  for(int64_t i=0;i<sequence_values_.count();i++)
  {
    int64_t pos2 = sequence_values_.at(i).to_string(buf+pos, buf_len-pos);
    pos += pos2;
  }
  databuff_printf(buf, buf_len, pos, "])\n");
  if (NULL != child_op_)
...
DEFINE_SERIALIZE(ObProject)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "serialize fail index_num_. ret=%d", ret);
    }
  }
  //add e
  //add lijianqiang [sequence update] 20150911:b
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to serialize sequence values count. ret=%d", ret);
    }
    else
    {
      for (int64_t i = 0; i < sequence_value_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize sequence_values fail. ret=%d", ret);
          break;
        }
      }
      for (int64_t i = 0; i < sequence_value_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize idx in expr fail. ret=%d", ret);
          break;
        }
      }
      for (int64_t i = 0; i < sequence_value_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize idx in out put columns fail. ret=%d", ret);
          break;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize sequence count in one row fail. ret=%d", ret);
        }
      }
    }
  }
...
DEFINE_DESERIALIZE(ObProject)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to deserialize expr count. ret=%d", ret);
  }

  else if (OB_SUCCESS != (ret = obj.get_int(expr_count)))
  {
    YYSYS_LOG(WARN, "fail to get expr_count. ret=%d", ret);
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize sequence values count. ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = sequence_values.get_int(sequence_value_count)))
    {
      YYSYS_LOG(WARN, "fail to get sequence values count. ret=%d", ret);
    }
    if (OB_SUCCESS == ret)
    {
      //sequence_values_
      for (int64_t i = 0; OB_SUCCESS == ret && i < sequence_value_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to deserialize sequence value. ret=%d", ret);
        }
        if(OB_SUCCESS == ret)
        {
            if (OB_SUCCESS != (ret = next_value_obj.get_ttint(nextval)))
            {
              YYSYS_LOG(WARN, "fail to get sequence value. ret=%d", ret);
            }
            else
            {
                p = next_value_obj.get_precision();
                s = next_value_obj.get_scale();
                vs = next_value_obj.get_vscale();
                len = next_value_obj.get_nwords();
                if (OB_SUCCESS != (ret = add_nextval_for_update(nextval,p,s,vs,len)))
                {
                  YYSYS_LOG(WARN, "fail to deserialize add nextval for update,ret=%d",ret);
                }
            }
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < sequence_value_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, " fail deserialize idx in expr. ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = add_idx_of_next_value(idx)))
        {
          YYSYS_LOG(WARN, "fail to deserialize add idx of next value. ret=%d",ret);
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < sequence_value_count; i++)
      {
        ...
        if(OB_SUCCESS != (ret = serialization::decode_i64(buf, data_len, pos, &idx)))
        {
          YYSYS_LOG(WARN, " fail deserialize idx in out put columns. ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = add_expr_idx_of_out_put_columns(idx)))
        {
          YYSYS_LOG(WARN, "fail to deserialize add idx out put columns. ret=%d",ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "deserialize sequence count in one row fail. ret=%d", ret);
        }
      }
    }
  }
