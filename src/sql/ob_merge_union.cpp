...
int ObMergeUnion::all_get_next_row(const ObRow *&row)
{
  ...
  if (OB_SUCCESS == left_ret_)
  {
    ...
    else if (OB_ITER_END == left_ret_)
    {
      ...
      {
        ret = right_ret_;
        YYSYS_LOG(WARN, "failed to get_next_row from right_op,ret=%d",ret);
      }
    }
    else
    {
      ret = left_ret_;
      YYSYS_LOG(WARN, "failed to get_next_row from left_op, ret=%d", ret);
    }
  }
  else if (OB_ITER_END == left_ret_)
  {
...
int ObMergeUnion::distinct_get_next_row(const ObRow *&row)
{
  ...
  if(OB_SUCCESS == ret)
  {
    if (OB_SUCCESS == left_ret_ && OB_SUCCESS == right_ret_)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (last_cmp_ < 0)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to save current row to last row, ret=%d",ret);
          }
        }
        else if (last_cmp_ == 0)
        {
    ...
    else
    {
      ...
      YYSYS_LOG(WARN, "failed to get next row, err=%d left_err=%d right_err=%d",
