...
{
  YYSYS_LOG(INFO, "groupby mem limit=%ld", limit);
...
DEFINE_SERIALIZE(ObGroupBy)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to group column count. ret=%d", ret);
  }
  else
  {
    for (int64_t i = 0; ret == OB_SUCCESS && i < group_columns_.count(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "serialize group column fail. ret=%d", ret);
      }
    } // end for
  }
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      for (int64_t i = 0; ret == OB_SUCCESS && i < aggr_columns_.count(); ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize aggregate column fail. ret=%d", ret);
        }
      } // end for
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "serialize memory size limit fail. ret=%d", ret);
  }
  return ret;
}
...
DEFINE_DESERIALIZE(ObGroupBy)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to get group column count. ret=%d", ret);
  }
  else
  {
    int64_t table_id = 0;
    ...
    for (int64_t i = 0; ret == OB_SUCCESS && i < count; i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to deserialize group column. ret=%d", ret);
      }
      else if ((ret = add_group_column(static_cast<uint64_t>(table_id),
         static_cast<uint64_t>(column_id))) != OB_SUCCESS)
      {
        YYSYS_LOG(DEBUG, "fail to add group column, table_id=%lu, column_id=%lu. ret=%d",
  ...
  if (OB_SUCCESS != (ret = obj.deserialize(buf, data_len, pos)))
  {
    YYSYS_LOG(WARN, "fail to deserialize aggregate function count. ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = obj.get_int(count)))
  {
    YYSYS_LOG(WARN, "fail to get aggregate function count. ret=%d", ret);
  }
  else
  {
    for (int64_t i = 0; ret == OB_SUCCESS && i < count; i++)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "fail to add aggregate function,ret=%d. buf=%p, data_len=%ld, pos=%ld",
  ...
  {
    YYSYS_LOG(WARN, "fail to deserialize memory size limit. ret=%d", ret);
  }
  return ret;
}
