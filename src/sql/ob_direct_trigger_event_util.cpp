...
int ObDirectTriggerEventUtil::refresh_new_config(ObResultSet &result, ObSqlContext &context)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to format sql. err=%d", err);
  }
  else if (OB_SUCCESS != (err = ObSql::direct_execute(stmt, result, context)))
  {
    YYSYS_LOG(ERROR, "fail to trigger refresh_new_config event. err[%d]", err);
  }
  else
  {
    YYSYS_LOG(INFO, "trigger refresh_new_config event succ: err[%d]", err);
  }
  return err;
}
