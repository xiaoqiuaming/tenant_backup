...
int ObMergeIntersect::open()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to open double child operators, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = cons_row_desc()))
  {
    YYSYS_LOG(WARN, "failed to construct row description, ret=%d", ret);
  }
  else if (NULL == (left_last_row_buf_ = (char*)ob_malloc(OB_ROW_BUF_SIZE, 0)))
  {
    YYSYS_LOG(ERROR, "failed to ob_malloc %lu bytes memory", OB_ROW_BUF_SIZE);
...
int ObMergeIntersect::close()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to close child op,ret=%d", ret);
  }
  row_desc_ = NULL;
  cur_first_query_row_ = NULL;
  cur_second_query_row_ = NULL;
...
int ObMergeIntersect::do_distinct(ObPhyOperator *op, const ObRow *other, const ObRow *&row)
{
  ...
  while (true)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "failed to compare two row, ret=%d", ret);
        break;
      }
    }
    else if (OB_ITER_END == ret)
    {
      YYSYS_LOG(DEBUG, "reach the end of op when do distinct");
      break;
    }
    else
    {
      YYSYS_LOG(WARN, "failed to get_next_row,ret=%d",ret);
      break;
    }
  }
  return ret;
...
int ObMergeIntersect::distinct_get_next_row(const common::ObRow *&row)
{
  ...
  if (got_first_row_)
  {
    ...
    {
      ret = left_ret_;
      YYSYS_LOG(WARN, "failed to do_distinct on left_op, ret=%d", left_ret_);
    }
    ...
    {
      ret = right_ret_;
      YYSYS_LOG(WARN, "failed to do_distinct on right_op, ret=%d", right_ret_);
    }
  }
  else
  {
    ...
    {
      ret = left_ret_;
      YYSYS_LOG(WARN, "failed to get_next_row on left_op, ret=%d", left_ret_);
    }
    ...
    {
      ret = right_ret_;
      YYSYS_LOG(WARN, "failed to get_next_row on right_op, ret=%d", right_ret_);
    }
  }
  if (OB_SUCCESS == ret)
  {
    while (true)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (OB_SUCCESS == left_ret_ && OB_SUCCESS == right_ret_)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            if (cmp < 0)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to save cur_first_query_row_ to left last row, ret=%d",ret);
                break;
              }
              else
              {
            ...
            else if (cmp == 0)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to save cur_second_query_row_ to right last row, ret=%d",ret);
              }
              break;
            }
            else
          ...
          {
            YYSYS_LOG(WARN, "failed to compare two row, ret=%d", ret);
            break;
          }
        }
        ...
        else// unexpected branch
        {
          ...
          YYSYS_LOG(ERROR, "failed to do distinct, ret=%d, left_ret_=%d, right_ret_=%d", ret, left_ret_, right_ret_);
          break;
        }
      }
      else
  ...
  else
  {
    if (got_first_row_)
    {
      YYSYS_LOG(WARN, "failed to do_distinct, left_ret_=%d, right_ret_=%d", left_ret_, right_ret_);
    }
    else
    {
      YYSYS_LOG(WARN, "failed to get next row , left_ret_=%d, right_ret_=%d", left_ret_, right_ret_);
    }
  }
  return ret;
}
...
int ObMergeIntersect::cons_row_desc()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to get row desc of left op, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = right_op_->get_row_desc(right_row_desc)))
  {
    YYSYS_LOG(WARN, "failed to get row desc of right op, ret=%d", ret);
  }
  else
  {
      ...
      else if (!tmp_cell->is_null())
      {
        ...
        if (this_type != res_type)
        {
          ...
          if (ObDecimalType == res_type)
          {
            if (ObVarcharType == this_type)
            {
              if (OB_SUCCESS != (ret = expr_from.cast_to(ObDecimalType, expr_to, *tmp_string)))
              {
                ...
                YYSYS_LOG(INFO,"column[%ld] value fail to cast to ObDecimalType", i + 1);
                break;
              }
              ...
              else
              {
                ...
                if(expr_to.get_type() == ObDecimalType)
                {
                    ...
                    tmp_string->write_decimal(dec.get_words()->ToUInt_v2(),tt,expr_to.get_len());
            ...
            else
            {
              if (OB_SUCCESS != (ret = expr_from.cast_to(ObVarcharType, expr_tmp, *tmp_string)))
              {
                ...
                YYSYS_LOG(INFO,"column[%ld] value fail to cast to ObVarcharType", i + 1);
              }
              ...
              {
                ret = OB_ERR_COLUMN_TYPE_NOT_COMPATIBLE;
                YYSYS_LOG(INFO,"column[%ld] value can not cast to ObDecimalType", i + 1);
                break;
              }
              else
              {
          ...
          else if (ObDecimalType != res_type)
          {
            ...
            {
              ret = OB_ERR_COLUMN_TYPE_NOT_COMPATIBLE;
              YYSYS_LOG(INFO,"column[%ld] value can not cast to dest_type[%d]", i + 1, res_type);
              break;
            }
            else if (expr_to.is_null())
            {
