...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      const ObItemType get_expr_type() const
      { return type_; }
      ...
      virtual int optimize_sql_expression(
              ...
              int type)= 0;
      virtual int alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt * subquery_stmt) = 0;
      void print_indentation_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level)const
      {
      ...
      virtual void print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level =0)
      {
    ...
    private:
      ...
      uint32_t data_precision_;
      uint32_t data_scale_;
    };
    ...
    public:
      ...
      virtual int optimize_sql_expression(
              ...
              std::map<uint64_t, uint64_t> alias_table_hashmap,
              int type);
      virtual int alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt);
    private:
    ...
    public:
      ...
      ObBinaryRefRawExpr(uint64_t first_id, uint64_t second_id, ObItemType expr_type = T_INVALID)
          : ObRawExpr(expr_type), first_id_(first_id), second_id_(second_id),
            contain_aggr_(false), aggr_expr_(NULL)
      {
      }
      ...
      void set_aggr_expr(ObRawExpr *aggr_expr)
      {
        aggr_expr_ = aggr_expr;
      }
      bool is_contain_aggr() const
      {
        return contain_aggr_;
      }
      ObRawExpr *get_aggr_expr()
      {
        return aggr_expr_;
      }
    ...
    public:
      ...
      { return when_exprs_.push_back(expr); }
      int add_then_op_expr(ObRawExpr *expr)
      { return then_exprs_.push_back(expr); }
    ...
    public:
      ...
      { param_expr_ = expr; }
      void set_listagg_param_delimeter(ObRawExpr *expr)
      { listagg_param_delimeter_ = expr; }//add gaojt [ListAgg][JHOBv0.1]20150104
      bool is_param_distinct() const
      { return distinct_; }
      void set_param_distinct()
      { distinct_ = true; }

      ObRawExpr *get_listagg_param_delimeter()
      {
        return listagg_param_delimeter_;
      }
    ...
    public:
      ObSysFunRawExpr()
        :date_op_type_(T_NULL), param_is_sys_func_(false) //[735]
      {}
      ...
      { func_name_ = name; }
      const common::ObString& get_func_name()
      { return func_name_; }
      ...
      virtual int alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt);
      //[735]
      void set_param_is_sys_func(bool param_is_sys_func)
      { param_is_sys_func_ = param_is_sys_func; }
      bool param_is_sys_func() const
      { return param_is_sys_func_; }

      void set_date_op_type(const ObItemType &op_type)
      {
        date_op_type_ = op_type;
      }
      void set_datetime(const ObString &datetime)
      {
        datetime_ = datetime;
      }
      const ObItemType &get_date_op_type() const
      {
        return date_op_type_;
      }
      const common::ObString &get_datetime()
      {
        return datetime_;
      }
    ...
    private:
      ...
      ObItemType date_op_type_;
      common::ObString datetime_;
      bool param_is_sys_func_; //[735]
    };
    ...
    public:
      ...
      { expr_ = expr; }
      void set_tables_set(const common::ObBitSet<> tables_set)
      { tables_set_ = tables_set; }
      ...
      { return expr_; }
      const common::ObBitSet<>& get_tables_set() const
      { return tables_set_; }
      common::ObBitSet<>& get_tables_set()
      { return tables_set_; }
      const common::ObObjType get_result_type() const
...
//          ObPhysicalPlan *physical_plan = NULL);
      int fill_sql_expression(
          ...
          ObLogicalPlan *logical_plan = NULL,
          ObPhysicalPlan *physical_plan = NULL,
          bool is_for_partition = false);
      //mod 20160113:e
      void print(FILE* fp, int32_t level, int32_t index = 0) const;

      void print_json(char *buf,const int64_t buf_len, int64_t &pos, int32_t level =0) const;

      void print_indentation_json(char *buf,const int64_t buf_len, int64_t &pos, int32_t level) const;

      //add dyr [HugeTable_Join_Filter] [JHOBv0.1] 20140417:b
      void set_join_optimized(bool is_join_optimize)
      { is_join_optimized_ = is_join_optimize; }
      //返回true表示当前表达式已经在某个JOIN优化中被使用过了
      bool is_join_optimized() const
      { return is_join_optimized_; }
      //add 20140417:e

      //add duyr [join_without_pushdown_is_null] 20151214:b
      void set_push_down_with_outerjoin(bool val)
      {can_push_down_with_outerjoin_ = val;}
      bool can_push_down_with_outerjoin() const
      {return can_push_down_with_outerjoin_;}
      //add duyr 20151214:e
      ...
      int alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt);
