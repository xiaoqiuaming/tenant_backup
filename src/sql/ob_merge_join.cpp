   ...
   ,hashmap_num_(0)//add tianz [SubQuery_for_Instmt] [JHOBv0.1] 20140610
{
    ...
    has_print_right_cached_row_end_ = false;
    has_reset_right_cached_row_ = false;
    is_equal_to_last_join_left_row_ = false;
}
...
int ObMergeJoin::open()
{
  ...
  {
    YYSYS_LOG(WARN, "merge join can not work without equijoin conditions");
    ret = OB_NOT_SUPPORTED;
  }
  else*/ if (OB_SUCCESS != (ret = ObJoin::open()))
  {
    YYSYS_LOG(WARN, "failed to open child ops, err=%d", ret);
  }
               ...
               else
               {
                   ...
                   if (ret!=OB_SUCCESS)
                   {
                     if(OB_ITER_END == ret)
                     {
                       ...
                       //YYSYS_LOG(USER_ERROR, "Empty set");
    ...
    if (right_cache_is_valid_)
    {
      ...
      if (0 == cmp)
      {
        ...
        if (OB_SUCCESS != (ret = right_cache_.get_next_row(curr_cached_right_row_)))
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get next row from right_cache, err=%d", ret);
          }
          else
          {
    ...
    else
    {
      ...
      if (0 == cmp)
      {
        if (right_cache_.is_empty())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to store left row, err=%d", ret);
            break;
          }
        }
        bool is_qualified = false;
        ...
        {
          YYSYS_LOG(WARN, "failed to store the row, err=%d", ret);
        }
...
//        }
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          if (OB_ITER_END == ret)
          {
            YYSYS_LOG(INFO, "end of right child op");
            is_right_iter_end_ = true;
            if (!right_cache_.is_empty())
            {
              // no more right rows and the right cache is not empty, we SHOULD look at the next left row
    ...
    if (left_cache_is_valid_)
    {
      ...
      if (0 == cmp)
      {
        ...
        if (OB_SUCCESS != (ret = left_cache_.get_next_row(curr_cached_left_row_)))
        {
          if (OB_UNLIKELY(OB_ITER_END != ret))
          {
            YYSYS_LOG(WARN, "failed to get next row from left_cache, err=%d", ret);
          }
          else
          {
    ...
    else
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          if (OB_ITER_END == ret)
          {
            YYSYS_LOG(INFO, "end of left child op");
          ...
          {
            YYSYS_LOG(WARN, "failed to get next row from left child, err=%d", ret);
            break;
          }
        }
      }
      ...
      if (0 == cmp)
      {
        if (left_cache_.is_empty())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to store right row, err=%d", ret);
            break;
          }
        }
        bool is_qualified = false;
...
int ObMergeJoin::full_outer_get_next_row(const common::ObRow *&row)
{
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    {
      OB_ASSERT(!right_cache_.is_empty());
      if(!is_equal_to_last_join_left_row_)
      {
        int cmp = 0;
        if (OB_SUCCESS == (ret = left_row_compare_equijoin_cond(*left_row, last_join_left_row_, cmp)))
        {
            if(cmp ==0)
            {
                is_equal_to_last_join_left_row_ = true;
            }
        }
        else
        {
      ...
      if(is_equal_to_last_join_left_row_)
      {
          ...
          ret = join_with_right_cache_rows(left_row,row,if_continue);
          if(if_continue)
          {
              ret = OB_SUCCESS;
                          continue;
      ...
      {
          ret = check_right_cache_print(row);
          if(OB_SUCCESS == ret)
          {
              last_left_row_ = left_row;
              last_left_row_has_printed_ = false;
          ...
          {
              OB_ASSERT(!right_cache_is_valid_);
              ret = OB_SUCCESS;
              continue;
          }
      }
    ...
    {
        OB_ASSERT(left_row);
        bool if_compare = false,if_continue = false,if_return = false;
        ret = get_next_right_row(left_row,right_row,row,if_compare,if_continue,if_return);
        if(OB_SUCCESS == ret)
        {
            if(!if_compare)
            {
        ...
        OB_ASSERT(right_row);
        OB_ASSERT(!if_continue);
        ret = join_with_right_row(left_row,right_row,row,if_continue);
        if(OB_SUCCESS == ret)
        {
            if(if_continue)
            {
  ...
  if(OB_ITER_END == ret)
  {
      if(!right_cache_.is_empty())
      {
          ret = check_right_cache_print(row);
          if(OB_ITER_END == ret){
              if(!is_right_iter_end_)
              {
                  ret = check_right_rows_print(right_row,row);
              }
          }
      }
...
int ObMergeJoin::join_with_right_row(const ObRow *&left_row, const ObRow *&right_row, const ObRow *&row, bool &if_continue)
{
    int ret =OB_SUCCESS,cmp=0;
    if(OB_SUCCESS == (ret = compare_equijoin_cond(*left_row,*right_row,cmp)))
    {
        if(cmp ==0)
        {
            bool if_satisfy_unequal_cond = false;
            bool if_join_with_right_cache = false;
            const ObRowStore::MergejoinStoredRow *stored_row = NULL;
            if(right_cache_.is_empty())
            {
                last_join_left_row_store_.assign_buffer(last_join_left_row_store_.ptr(),MAX_SINGLE_ROW_SIZE);
                if(OB_SUCCESS != (ret = ObRowUtil::convert(*left_row,last_join_left_row_store_,last_join_left_row_)))
                {
                    YYSYS_LOG(WARN, "failed to store left row to last_join_left_row_, err=%d", ret);
                }
                else
                {
                    ret = join_and_check_unequal_cond(left_row,*right_row,row,stored_row,if_satisfy_unequal_cond,if_join_with_right_cache);
                }
            }
            else
            {
        ...
        else if(cmp <0)
        {
            if(!right_cache_.is_empty())
            {
                right_cache_is_valid_ = true;
                last_right_row_ = right_row;
                bool if_get_next_left_row = false;
                ret = check_left_row_print(left_row,row,if_get_next_left_row);
                if(OB_SUCCESS == ret)
                {
                    if(!last_left_row_has_printed_)
                    {
            ...
            {
                bool if_left_row_join  = true;
                ret = do_left_or_right_join_rows(*left_row,row,if_left_row_join);
                if(OB_SUCCESS == ret)
                {
                    last_right_row_ =  right_row;
                }
        ...
        {
            bool if_left_row_join  = false;
            ret = do_left_or_right_join_rows(*right_row,row,if_left_row_join);
            if(OB_SUCCESS == ret)
            {
                last_left_row_ =  left_row;
                last_left_row_has_printed_ = false;
...
int ObMergeJoin::join_with_right_cache_rows(const ObRow *&left_row, const ObRow *&row, bool& if_continue)
{
    int ret= OB_SUCCESS;
    const ObRowStore::MergejoinStoredRow* stored_row = NULL;
    ret = right_cache_.get_next_mergejoin_stored_row(curr_cached_right_row_,stored_row);
    if(OB_SUCCESS == ret)
    {
        bool if_join_with_right_cache = true;
        bool if_satisfy_unequal_cond = false;
        ret = join_and_check_unequal_cond(left_row,curr_cached_right_row_,row,stored_row,if_satisfy_unequal_cond,if_join_with_right_cache);
        if(OB_SUCCESS == ret)
        {
            if(!if_satisfy_unequal_cond)
            {
    ...
    {
        right_cache_.reset_iterator();
        has_reset_right_cached_row_ = true;
        bool if_get_next_left_row = false;
        ret = check_left_row_print(left_row,row,if_get_next_left_row);
        if(OB_SUCCESS == ret)
        {
...
int ObMergeJoin::get_next_right_row(const ObRow *&left_row, const ObRow *&right_row, const ObRow *&row, bool &if_compare, bool &if_continue, bool &if_return)
{
    int ret = OB_SUCCESS;
    if(is_right_iter_end_)
    {
        ret = do_left_or_right_join_rows(*left_row,row,true);
        if_return = true;
    }
    else if(NULL != last_right_row_)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                YYSYS_LOG(WARN, "failed to get right row, err=%d", ret);
                if_return = true;
            }
        }
    }
...
int ObMergeJoin::do_left_or_right_join_rows(const ObRow& join_row, const ObRow *&join_result_row, bool is_left_row)
{
    int ret =OB_SUCCESS;
    if(is_left_row)
    {
        if(OB_SUCCESS == (ret =left_join_rows(join_row)))
        {
            join_result_row = &curr_row_;
            last_left_row_has_printed_ = true;
        }
        else
        {
            YYSYS_LOG(WARN, "failed to join left rows, err=%d", ret);
        }
    }
    else
    {
        if(OB_SUCCESS == (ret = right_join_rows(join_row)))
        {
            join_result_row = &curr_row_;
        }
        else
        {
            YYSYS_LOG(WARN,"failed to join right rows, err=%d",ret);
        }
    }
    return ret;
}

int ObMergeJoin::join_and_check_unequal_cond(const ObRow *&left_row, const ObRow& right_row, const ObRow *&join_result_row, const ObRowStore::MergejoinStoredRow *&stored_row, bool& if_satisfy_unequal_cond, bool& if_join_with_right_cache)
{
    int ret = OB_SUCCESS;
    if(!if_join_with_right_cache)
    {
        if(OB_SUCCESS != (ret = right_cache_.add_mergejoin_row(right_row,stored_row)))
        {
                        YYSYS_LOG(WARN, "failed to store the row, err=%d", ret);
        }
    }
    ...
    {
        if(OB_SUCCESS ==(ret = curr_row_is_qualified(if_satisfy_unequal_cond)))
        {
            if(if_satisfy_unequal_cond)
            {
                join_result_row = &curr_row_;
                const_cast<ObRowStore::MergejoinStoredRow*>(stored_row)->join_status_ = static_cast<int32_t>(ObRowStore::SUC);
                last_left_row_ = left_row;
                last_left_row_has_printed_ = true;
            }
        }
        ...
        {
            YYSYS_LOG(WARN,"failed to test whether satisfy unequal conditions, err=%d", ret);
        }
    }
    else
    {
...
int ObMergeJoin::check_left_row_print(const ObRow *&left_row, const ObRow *&join_result_row, bool &if_get_next_left_row)
{
    int ret = OB_SUCCESS;
    if(!last_left_row_has_printed_)
    {
        ret = do_left_or_right_join_rows(*left_row,join_result_row,true);
    }
    else
    {
        ret = left_op_->get_next_row(left_row);
        ...
        if(ret == OB_SUCCESS)
        {
            if_get_next_left_row = true;
        }
    }
    return ret;
}

int ObMergeJoin::check_right_rows_print(const ObRow *&right_row, const ObRow *&row)
{
    int ret = OB_SUCCESS;
    do
    {
        ...
        if(OB_SUCCESS == ret)
        {
            ret = do_left_or_right_join_rows(*right_row,row,false);
            break;
        }
        else
        {
...
int ObMergeJoin::check_right_cache_print(const ObRow *&row)
{
    int ret = OB_SUCCESS;
    if(!has_print_right_cached_row_end_)
    {
        if(!has_reset_right_cached_row_)
        {
            right_cache_.reset_iterator();
            has_reset_right_cached_row_ = true;
        }
        do
        {
            const ObRowStore::MergejoinStoredRow* mergejoin_row =NULL;
            bool has_convert = false;
            if(OB_SUCCESS != (ret = right_cache_.get_next_mergejoin_stored_row(curr_cached_right_row_,mergejoin_row,has_convert,false)))
            {
                if(OB_UNLIKELY(OB_ITER_END == ret))
                {
                    YYSYS_LOG(INFO, "end of right cache");
                    has_print_right_cached_row_end_ = true;
                    break;
                }
                else
                {
            ...
            else if(OB_UNLIKELY(mergejoin_row == NULL))
            {
                ret = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN,"can not get merge join row from right_cache, err=%d",ret);
                break;
            }
            else if(mergejoin_row->join_status_ == ObRowStore::FAIL)
            {
                if(!has_convert)
                {
                    ret = OB_ERR_UNEXPECTED;
                    YYSYS_LOG(WARN, "curr_cached_right_row has convert failed, ret=%d", ret);
                }
                else if(OB_SUCCESS != (ret = right_join_rows(curr_cached_right_row_)))
                {
                    YYSYS_LOG(WARN,"failed to join rows, err=%d",ret);
                }
                else
...
/*
  ...
  while(���������������������û�е�����)
  {
    if (left_row�ڵ�ֵ�����ϵ���last_join_left_row_
        && right_cache_is_valid_)
    {
      ��right_cache_��ȡ��һ�����ݵ�right_row;
      if (right_cache_ȫ��ȡ����һ��)
      {
        ����right_cache_�еĵ���������һ��;
        left_op_->get_next_row(left_row);
      }
      ...
      {
        join left_row��right_row�������ݣ������curr_row_;
        row = &curr_row_;
        break;
      }
    }
    else
    {
      ...
      if (left_row��right_row�ڵ�ֵjoin���������)
      {
        join�������ݣ������curr_row_;
        if (left_row�ڵ�ֵ�����ϲ�����last_join_left_row_)
        {
          ���right_cache_��last_join_left_row_;
          right_cache_is_valid_ = false; // ��ʼʱΪfalse
          ����left_row��last_join_left_row_;
        }
        right_row�������ӵ�right_cache_��;
        last_left_row_ = left_row;
        row = &curr_row_;
        break;
      }
      else if(left_row�ڵ�ֵjoin������ < right_row)
      {
        if (LEFT_OUTER_JOIN == join_type_ || FULL_OUTER_JOIN == join_type_)
        {
          ��left_row����curr_row_�������ֵ��NULL����;
          last_right_row_ = right_row;
          row = &curr_row_;
          break;
        }
...
/*
  ...
  while(�������������������û�е�����)
  {
    ...
    else if(right_row�ڵ�ֵjoin������ < left_row)
    {
      right_op_->get_next_row(right_row);
    }
    else
    {
      // right_row�ڵ�ֵjoin������ > left_row
      left_op_->get_next_row(left_row);
    }
  }
*/
...
/*
  ...
  while(�������������������û�е����꣬��left_row��right_row����Ч)
  {
    if (left_row��right_row�ڵ�ֵjoin���������)
    {
      left_op_->get_next_row(left_row);
...
PHY_OPERATOR_ASSIGN(ObMergeJoin)
{
  ...
  {
    YYSYS_LOG(WARN, "Fail to set join type, ret=%d", ret);
  }
  else
  {
