...
int ObRunFile::open(const ObString &filename)
{
  ...
  {
    ret = OB_INIT_TWICE;
    YYSYS_LOG(ERROR, "file is opened");
  }
  else if (OB_SUCCESS != (ret = file_appender_.open(filename, true,
                                                    true, true)))
  {
    YYSYS_LOG(WARN, "failed to open file, err=%d name=%.*s",
              ret, filename.length(), filename.ptr());
  }
  else if (OB_SUCCESS != (ret = file_reader_.open(filename, true)))
  {
    file_appender_.close();
    YYSYS_LOG(WARN, "reader failed to open file, err=%d name=%.*s",
              ret, filename.length(), filename.ptr());
  }
  else
  {
    YYSYS_LOG(INFO, "open run file, name=%.*s", filename.length(), filename.ptr());
  }
  return ret;
}
...
int ObRunFile::begin_append_run(const int64_t bucket_idx)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "run file not opened");
  }
  else if (OB_SUCCESS != (ret = find_last_run_trailer(bucket_idx, curr_run_trailer_)))
  {
    ...
    else
    {
      ...
      YYSYS_LOG(INFO, "begin append run for bucket=%ld", bucket_idx);
    }
  }
  else
  {
...
int ObRunFile::append_row(const common::ObString &compact_row)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to append file, err=%d", ret);
  }
  else
  {
    ++curr_run_row_count_;
...
int ObRunFile::end_append_run()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to fsync run file, err=%d", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to write run trailer, err=%d", ret);
    }
    ...
    {
      YYSYS_LOG(INFO, "end append run, bucket=%ld row_count=%ld, run_trailer_pos=%ld run_size=%ld",
...
int ObRunFile::begin_read_bucket(const int64_t bucket_idx, int64_t &run_count)
{
  ...
  if (0 != run_blocks_.count())
  {
    ...
    YYSYS_LOG(ERROR, "run blocks is not empty");
  }
  else if (OB_SUCCESS != (ret = find_last_run_trailer(bucket_idx, run_trailer)))
  {
    YYSYS_LOG(ERROR, "no run for this bucket=%ld", bucket_idx);
  }
  else
  {
    // read the first block of every run
    int64_t run_trailer_pos = run_trailer->prev_run_trailer_pos_;
    YYSYS_LOG(INFO, "last run trailer, bucket_idx=%ld pos=%ld", bucket_idx, run_trailer_pos);
    RunBlock run_block;
    int64_t read_size = 0;
           ...
           && 0 < run_trailer_pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to alloc block, err=%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to read run file, err=%d offset=%ld read_size=%ld",
      ...
      else
      {
        ...
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "invalid run trailer, data corrupted, magic=%lX", run_trailer->magic_number_);
        }
        else if (bucket_idx != run_trailer->bucket_idx_)
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "invalid bucket idx=%ld expected=%ld", run_trailer->bucket_idx_, bucket_idx);
        }
        else if (0 > run_trailer_pos - run_trailer->curr_run_size_)
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "invalid run size, trailer_pos=%ld run_size=%ld", run_trailer_pos,
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "run trailer, bucket_idx=%ld prev_trailer_pos=%ld curr_run_size=%ld",
          ...
          {
            YYSYS_LOG(WARN, "failed to read run file, err=%d offset=%ld read_size=%ld data_size=%ld",
          ...
          {
            YYSYS_LOG(INFO, "read run first block, offset=%ld size=%ld", run_block.block_offset_, read_size);
            ++run_count;
            run_block.reset();
          }
        }
...
int ObRunFile::get_next_row(const int64_t run_idx, common::ObRow &row)
{
  ...
  if (run_idx >= run_count || 0 >= run_count)
  {
    ...
    YYSYS_LOG(WARN, "invalid run idx=%ld count=%ld", run_idx, run_count);
  }
  else
  {
    ...
    {
      YYSYS_LOG(INFO, "reach end of run, run_idx=%ld", run_idx);
      ret = OB_ITER_END;
    }
    else if (OB_SUCCESS != (ret = block_get_next_row(run_block, row)))
    {
      YYSYS_LOG(WARN, "failed to get the next row from the block, err=%d run_idx=%ld", ret, run_idx);
    }
  }
  return ret;
}
...
int ObRunFile::block_get_next_row(RunBlock &run_block, common::ObRow &row)
{
  ...
  if (run_block.next_row_pos_ >= run_block.block_data_size_)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to read next block, err=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
          ...
          || OB_ITER_END == ret)
      {
        if (OB_SUCCESS != (ret = read_next_run_block(run_block)))
        {
          YYSYS_LOG(WARN, "failed to read next block, err=%d", ret);
                                                ...
                                                compact_row, row)))
        {
          YYSYS_LOG(WARN, "failed to get next row after read new block, err=%d", ret);
        }
      }
      else
      {
...
int ObRunFile::parse_row(const char* buf, const int64_t buf_len, ObString &compact_row, common::ObRow &row)
{
  ...
  if (OB_SUCCESS == ret && is_row_finished)
  {
    ...
    {
      YYSYS_LOG(ERROR, "corrupted row data, col_num=%ld cell_num=%ld", row.get_column_num(), cell_idx);
      ret = OB_ERR_UNEXPECTED;
    }
  }
  return ret;
...
int ObRunFile::read_next_run_block(RunBlock &run_block)
{
  ...
  {
    YYSYS_LOG(DEBUG, "read block, size=%ld offset=%ld", read_size, run_block.block_offset_);
  }
  return ret;
}
