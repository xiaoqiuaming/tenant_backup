...
//    src_filter_expr_(NULL),
  ...
  //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
    ,is_data_mark_cids_added_(false), is_data_mark_param_valid_(false)
  //add duyr 20160531:e
{
...
//    }
    ...
    //YYSYS_LOG(INFO,"wanglei::~ObRpcScan OB_SEMI_JOIN_SQL_EXPRESSION clear malloc usage = %ld",usage);
    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
    //YYSYS_LOG(INFO,"wanglei::~ObRpcScan OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER free malloc usage = %ld",usage);
...
//    }
    ...
    //int64_t usage = 0;
    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION);
    //YYSYS_LOG(INFO,"wanglei::reset OB_SEMI_JOIN_SQL_EXPRESSION clear malloc usage = %ld",usage);
    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
    //YYSYS_LOG(INFO,"wanglei::reset OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER free malloc usage = %ld",usage);
    //for memory leak check:e
    //add wanglei [semi join update] 20160601:e
	    //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
    ...
    //add duyr 20160531:e
    trans_id_.reset();//add by maosy [MultiUPS 1.0] [read uncommit]20170525 
}
...
int ObRpcScan::init(ObSqlContext *context, const common::ObRpcScanHint *hint)
{
    ...
    {
        YYSYS_LOG(WARN, "must set table_id_ first. table_id_=%ld", table_id_);
        ret = OB_NOT_INIT;
    }
    else
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to get table schema. table_id[%ld]", base_table_id_);
            ret = OB_ERROR;
        }
        else
        {
    ...
    if (hint)
    {
        ...
        if (hint_.read_method_ == ObSqlReadStrategy::USE_SCAN)
        {
            ...
            if (OB_SUCCESS == ret && hint_.is_get_skip_empty_row_ && is_use_index_)
            {
                ...
                {
                    YYSYS_LOG(WARN, "fail to create column expression. ret=%d", ret);
                }
                else if (OB_SUCCESS != (ret = main_project.add_output_column(special_column)))
                {
                    YYSYS_LOG(WARN, "fail to add special is-row-empty-column to project. ret=%d", ret);
                }
                else
                {
                    YYSYS_LOG(DEBUG, "add special column to read param");
                }
            }
            //add:e
        }
        ...
        {
            ret = OB_INVALID_ARGUMENT;
            YYSYS_LOG(WARN, "read method must be either scan or get. method=%d", hint_.read_method_);
        }
    }
    return ret;
}
...
int ObRpcScan::set_is_use_index_for_storing(uint64_t main_tid,common::ObRowDesc &row_desc)
{
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < row_desc.get_column_num(); i ++)
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to get tid cid:ret[%d]", ret);
        }
...
int ObRpcScan::cast_range(ObNewRange &range)
{
    ...
    if (OB_LIKELY(OB_SUCCESS == ret))
    {
        ...
        {
            YYSYS_LOG(WARN, "failed to cast rowkey, err=%d", ret);
        }
    }
    if (OB_LIKELY(OB_SUCCESS == ret))
    {
...
int ObRpcScan::create_scan_param(ObSqlScanParam &scan_param)
{
    ...
    if(OB_SUCCESS == ret)
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to construct scan range. ret=%d", ret);
        }
        // TODO: lide.wd 将range 深拷贝到ObSqlScanParam内部的buffer_pool_中
        else if (OB_SUCCESS != (ret = scan_param.set_range(range)))
        {
            YYSYS_LOG(WARN, "fail to set range to scan param. ret=%d", ret);
        }
        //add zhujun [transaction read uncommit]2016/3/24
        else if (OB_SUCCESS != (ret = scan_param.set_trans_id(trans_id_)))
        {
            YYSYS_LOG(WARN, "fail to set trans_id to scan param. ret=%d", ret);
        }
        //add:e
        set_read_consistency_param();
    }
    //modify:e
    YYSYS_LOG(DEBUG, "scan_param=%s", to_cstring(scan_param));
    YYSYS_LOG(TRACE, "dump scan range: %s", to_cstring(range));
    return ret;
}
...
int ObRpcScan::init_next_scan_param(ObNewRange &range)
{
    ...
    if(OB_SUCCESS != ret)
    {
        YYSYS_LOG(WARN, "fail to fill read param to scan param. ret=%d", ret);
    }
    //modify:e
    ...
    {
        YYSYS_LOG(WARN, "fail to set request param. max_parallel=%ld, ret=%d",hint_.max_parallel_count, ret);
    }
    else
    {
        YYSYS_LOG(DEBUG, "next scan_param=%s", to_cstring(*scan_param_));
    }
    YYSYS_LOG(TRACE, "dump scan range: %s", to_cstring(range));
    return ret;
}
...
int ObRpcScan::create_get_param(ObSqlGetParam &get_param)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to set trans_id to get param. ret=%d", ret);
    }
    //add:e

    YYSYS_LOG(DEBUG, "get_param=%s", to_cstring(get_param));
    return ret;
}
...
int ObRpcScan::reset_read_param_for_index()  //重新设置一下read_param。因为回表情况下，第一次scan索引表的时候以及设置了一个read_param。在第二次get原表的时候，要把这个read_param清空，再重新赋值。
{
    ...
    //YYSYS_LOG(ERROR, "test::fanqs,,main_project4444=%s", to_cstring(main_project));
    read_param_->set_filter(main_filter_);
    //YYSYS_LOG(ERROR, "test::fanqs,,read_param_15662=%s", to_cstring(main_filter_));
    return ret;
}
...
{
    int ret = OB_SUCCESS;
    // YYSYS_LOG(ERROR, "test::fanqs,,enter create_get_param_for_index,,read_param_->has_filter()=%d,,read_param_=%s",read_param_->has_filter(),to_cstring(*read_param_));
    if (OB_SUCCESS != (ret = cons_get_rows_for_index(get_param)))  //构造主键的范围
  {
    ...
    if (OB_SUCCESS == ret)
    {
        //YYSYS_LOG(ERROR, "test::fanqs,,,scan_param=%s", to_cstring(*scan_param_));
		//add by maosy  [MultiUps 1.0] [secondary index optimize]20170508 b:
        ObDataMarkParam data_mark;
        if(read_param_->get_data_mark_param().is_valid())
        {
            data_mark = read_param_->get_data_mark_param();
        }
		//add by maosy e
//        read_param_->reset();
        ...
        else
        {
            ...
            {
                read_param_->set_data_mark_param(data_mark);
                YYSYS_LOG(DEBUG,"read_parm = %s",to_cstring(*read_param_));
            }
			//add by maosy e 
        }
    }
...
//        dest_param.set_is_result_cached(false);
        set_is_result_cached(dest_param, main_table_id_);
        if (OB_SUCCESS != (ret = dest_param.set_table_id(table_id_, main_table_id_)))
        {
            YYSYS_LOG(WARN, "fail to set table id and scan range. ret=%d", ret);
        }
        //add fanqiushi_index
        //YYSYS_LOG(ERROR, "test::fanqs,,table_id_=%ld,,base_table_id_=%ld,main_table_id_=%ld", table_id_,base_table_id_,main_table_id_);
        //add:e
    }
...
int ObRpcScan::fill_read_param_for_first_scan(ObSqlReadParam &dest_param)
{
    ...
    {
//        dest_param.set_is_result_cached(false);
        set_is_result_cached(dest_param, base_table_id_);
        if (OB_SUCCESS != (ret = dest_param.set_table_id(base_table_id_, base_table_id_)))
        {
            YYSYS_LOG(WARN, "fail to set table id and scan range. ret=%d", ret);
        }
        //add fanqiushi_index
        //YYSYS_LOG(ERROR, "test::fanqs,,table_id_=%ld,,base_table_id_=%ld", table_id_,base_table_id_);
        //add:e
    }
...
int ObRpcScan::set_is_result_cached(ObSqlReadParam &dest_param, const uint64_t table_id)
{
    int ret = OB_SUCCESS;
    const ObSchemaManagerV2 *user_schema = NULL;
    const ObTableSchema *table_schema = NULL;

    if(NULL == (user_schema = merge_service_->get_schema_mgr()->get_user_schema(0)))
    {
        YYSYS_LOG(WARN,"get latest but local schema failed.");
    }
    else if(NULL == (table_schema = user_schema->get_table_schema(table_id)))
    {
        YYSYS_LOG(WARN,"get_table_schema error,table_id:%lu.",table_id);
    }
    else
    {
        if(table_schema->get_table_type() == ObTableSchema::SSTABLE_IN_RAM)
        {
    ...
    if(NULL != user_schema)
    {
        if(OB_SUCCESS != (merge_service_->get_schema_mgr()->release_schema(user_schema)))
        {
...
void ObRpcScan::set_read_consistency_param()
{
    ...
    else if(NULL != session_info_ 
	&& session_info_->get_trans_start())
    {
        read_param_->set_is_read_consistency(true);
    }
    else
...
int ObRpcScan::open()
{
    ...
    {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(WARN, "alloc scan request from scan request pool failed, ret=%d", ret);
    }
    else if (NULL == (sql_get_request_ = ObMergeServerMain::get_instance()->get_merge_server().get_get_req_pool().alloc()))
    {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(WARN, "alloc get request from get request pool failed, ret=%d", ret);
    }
    //add duyr [Delete_Update_Function_isolation] [JHOBv0.1] 20160531:b
    ...
    {
        YYSYS_LOG(WARN,"fail to add data mark cids!ret=%d",ret);
    }
    //add duyr 20160531:e
    else
    {
        ...
        if (NULL != session_info_)
        {
            ...
            if (OB_SUCCESS == session_info_->get_sys_variable_value(ObString::make_string(OB_QUERY_TIMEOUT_PARAM), val))
            {
                ...
                {
                    YYSYS_LOG(WARN, "fail to get query timeout from session, ret=%d", ret);
            ...
            if (OB_SUCCESS == session_info_->get_sys_variable_value(ObString::make_string(OB_MAX_REQUEST_PARALLEL_COUNT), val))
            {
                ...
                {
                    YYSYS_LOG(WARN, "fail to get parallel count from session, ret=%d", ret);
            ...
            if (parallel_count > 0)
            {
                ...
                if (my_phy_plan_->get_has_range())
                {
                  hint_.max_parallel_count = merge_service_->get_config().export_tool_parallel;
                }
                //add hongchen [EXPORT_TOOL_OPTI] 20170821:e
                //mod liumz, [adjust parallel count]20170809:b
                if (is_use_semi_join_)
                {
                  hint_.max_parallel_count = parallel_count * 5;
                }
                //mod liumz, [adjust parallel count]20170809:e
            }
            else
            {
                // use default hint_.max_parallel_count
            }
            YYSYS_LOG(DEBUG, "query timeout is %ld, parallel is %ld", hint_.timeout_us, hint_.max_parallel_count);
        }

        timeout_us_ = hint_.timeout_us;
        OB_ASSERT(my_phy_plan_);
        this->set_read_consistency_param();
//        FILL_TRACE_LOG(" hint.read_consistency=%s ", get_consistency_level_str(hint_.read_consistency_));
//        if (hint_.read_consistency_ == common::FROZEN)
//        {
//            ObVersion frozen_version = my_phy_plan_->get_curr_frozen_version();
...
//        FILL_TRACE_LOG("only_static = %c", read_param_->get_is_only_static_data() ? 'Y' : 'N');
        ...
        if (OB_SUCCESS == ret)
        {
            YYSYS_LOG(DEBUG, "read_method_ [%s]", hint_.read_method_ == ObSqlReadStrategy::USE_SCAN ? "SCAN" : "GET");
            ...
            if(is_use_index_for_storing_)
            {
                ...
                //YYSYS_LOG(ERROR,"test::fanqs,,11,cur_row_desc_for_storing=%s",to_cstring(cur_row_desc_for_storing));
            }
            //add:e
//            FILL_TRACE_LOG("open %s", to_cstring(cur_row_desc_));
        ...
        //add wangyao  [sequence for optimize]:20171009
        ObFrozenDataCache & sequence_frozen_data_cache = ObMergeServerMain::
                get_instance()->get_merge_server().get_sequence_frozen_data_cache();
        //add e
        if (frozen_data_cache.get_in_use() && need_cache_frozen_data_ && (hint_.read_consistency_ == FROZEN))
        {
            ...
            {
                YYSYS_LOG(ERROR, "set_table_id error, ret: %d", ret);
            }
            else
            {
              //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150717:b
              //update delete sql语句设置 stmt_type
              ObBasicStmt::StmtType  stmt_type=my_phy_plan_->get_result_set()->get_inner_stmt_type();
              read_param_->set_stmt_type(stmt_type);
              //add 20150717:e
            ...
            {
                YYSYS_LOG(ERROR, "ObFrozenDataKeyBuf alloc_buf error, ret: %d", ret);
            }
            ...
            {
                YYSYS_LOG(ERROR, "ObSqlReadParam serialize error, ret: %d", ret);
            }
            else
            {
                ...
                // add wangyao  [sequence for optimize]:20171009
                if (OB_ALL_SEQUENCE_TID == table_id_)
                {
                  ret = sequence_frozen_data_cache.get(frozen_data_key_, cached_frozen_data_);
                }
                else
                {
                ...
                if (OB_SUCCESS != ret)
                {
                    YYSYS_LOG(ERROR, "ObFrozenDataCache get_frozen_data error, ret: %d", ret);
                }
        ...
        if (!cached_frozen_data_.has_data())
        {
            ...
            if (OB_SUCCESS == ret && hint_.read_method_ == ObSqlReadStrategy::USE_SCAN)
            {
                ...
                {
                    YYSYS_LOG(WARN, "initialize sql_scan_request failed, ret=%d", ret);
                }
                else
                {
                    ...
                    //          {
                    //            YYSYS_LOG(WARN, "fail to init sql_scan_event. ret=%d", ret);
                    //          }
                    //del:e
                    ...
                    {
                        YYSYS_LOG(WARN, "fail to create scan param. ret=%d", ret);
                    }
                }

                if (OB_SUCCESS == ret)
                {
                  //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150612:b
                  ObBasicStmt::StmtType  stmt_type=my_phy_plan_->get_result_set()->get_stmt_type();
                  scan_param_->set_stmt_type(stmt_type);
                  //add 20150612:e
                    ...
                    {
                        YYSYS_LOG(WARN, "fail to set request param. max_parallel=%ld, ret=%d",
            ...
            if (OB_SUCCESS == ret && hint_.read_method_ == ObSqlReadStrategy::USE_GET)
            {
                        ...
                        if (OB_SUCCESS == (err = cache_proxy_->get_tablet_location(get_param_->get_table_id(), *rowkey, loc_list)))
                        {
                            ...
                            else if (OB_ENTRY_NOT_EXIST == err)
                            {
                                ...
                                if (buff == NULL)
                                {
                                    ...
                                    YYSYS_LOG(ERROR, "malloc failed, ret=%d", err);
                                }
                                else
                                {
                                    ...
                                    {
                                        YYSYS_LOG(ERROR, "deep copy rowkey failed, err=%d", err);
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            YYSYS_LOG(ERROR, "push task to bloom_filter_task_queue failed,ret=%d", err);
                                        }
                                        else if (OB_TOO_MANY_BLOOM_FILTER_TASK == err)
                                        {
                                            YYSYS_LOG(DEBUG, "push task to bloom_filter_task_queue failed, ret=%d", err);
                                        }
                                        else
                                        {
                                            YYSYS_LOG(DEBUG, "PUSH TASK SUCCESS");
                                        }
                                    }
                                    if (OB_SUCCESS != err)
                                    {
                            ...
                            {
                                //go on
                                YYSYS_LOG(ERROR, "get from insert cache failed, err=%d", err);
                            }
                        }
                        else
                        {
                            //go on
                            YYSYS_LOG(ERROR, "get tablet location failed, table_id=%lu,rowkey=%s, err=%d", get_param_->get_table_id(), to_cstring(*rowkey), err);
                        }
                    }
                }
                if (!rowkey_not_exists_)
                {
                    ...
                    {
                        YYSYS_LOG(WARN, "fail to get row desc:ret[%d]", ret);
                    }
                    else if (OB_SUCCESS != (ret = sql_get_request_->set_row_desc(get_row_desc_)))
                    {
                        YYSYS_LOG(WARN, "fail to set row desc:ret[%d]", ret);
                    }
                    ...
                    if (OB_SUCCESS == ret)
                    {
                      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150612:b
                      ObBasicStmt::StmtType  stmt_type=my_phy_plan_->get_result_set()->get_stmt_type();
                      //            YYSYS_LOG(WARN, "stmt_type  =%ld",(int64_t)stmt_type);
                      sql_get_request_->set_stmt_type(stmt_type);
                      //add 20150612:e
                      ...
                      {
                          YYSYS_LOG(WARN, "fail to open get request. ret=%d", ret);
                      }
                      YYSYS_LOG(DEBUG, "get_row_desc_:  %s", to_cstring(get_row_desc_));
                    }
                }
            }//Get
        }
    ...
    if(ret == OB_SUCCESS && is_use_semi_join_)
    {
      ...
      YYSYS_LOG(DEBUG, "be4 we change, right_table_id = %ld", right_table_id_);
      ...
      //add 2016-9-6e
        YYSYS_LOG(DEBUG,"wanglei::table_id_ = %ld",table_id_);
        YYSYS_LOG(DEBUG,"wanglei::base_table_id_ = %ld",base_table_id_);
        YYSYS_LOG(DEBUG,"wanglei::main_table_id_ = %ld",main_table_id_);
        YYSYS_LOG(DEBUG,"wanglei::right_table_id = %ld",right_table_id_);
        YYSYS_LOG(DEBUG,"wanglei::main_filter_ = %s",to_cstring(main_filter_));
        YYSYS_LOG(DEBUG,"wanglei::index_filter_ = %s",to_cstring(index_filter_));
        //for memory leak check:b
        //int64_t usage = 0;
        //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION);
        //YYSYS_LOG(INFO,"wanglei::OB_SEMI_JOIN_SQL_EXPRESSION before malloc usage = %ld",usage);
        //for memory leak check:e
        //mod hongchen [SQL_EXPR_MEM_LEAK_FIX] 20170811:b
        //hint_.max_parallel_count = 500;
        hint_.max_parallel_count = OB_DEFAULT_MAX_PARALLEL_COUNT;
...
//add by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
//add zhujun [transaction read uncommit]2016/3/24
int ObRpcScan::set_trans_id(const ObTransID trans_id, int64_t paxos_id,int64_t published_transid) 
{
//    trans_id_ = trans_id;
    int ret = OB_SUCCESS ;
    ...
    {
        par_transid.transid_ = trans_id;
    }
    par_transid.published_transid_ = published_transid;
    par_transid.paxos_id_ = paxos_id;
    YYSYS_LOG(DEBUG," par trans id = %s",to_cstring(par_transid));
    if(trans_id_.count() >= MAX_UPS_COUNT_ONE_CLUSTER)
    {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR,"one cluster has max nine ups,count = %ld",trans_id_.count());
    }
    else if(OB_SUCCESS !=(ret = trans_id_.push_back(par_transid)))
    {
        YYSYS_LOG(WARN,"failed to pusb back trans id ,ret = %d",ret );
    }
    return ret ;
// add by maosy e
}
//add:e
//add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160226:b   // uncertainty 需要添加过来
int ObRpcScan::reset_filter()
{
  int ret = OB_SUCCESS;
  if (NULL != read_param_ && OB_SUCCESS != (ret = read_param_->reset_filter()))
  {
    YYSYS_LOG(WARN,"fail to reset filters of read param!ret=%d",ret);
  }
  else if (OB_SUCCESS != (ret = sql_read_strategy_.reset_filter()))
  {
    YYSYS_LOG(WARN,"fail to reset filters of read strategy!ret=%d",ret);
  }
  return ret;
}
...
int ObRpcScan::close()
{
    ...
    if (cached_frozen_data_.has_data())
    {
        ...
        if (OB_SUCCESS != ret)
        {
            YYSYS_LOG(ERROR, "ObFrozenDataCache revert error, ret: %d", ret);
        }
    }
    else if (frozen_data_cache.get_in_use() && need_cache_frozen_data_ && (hint_.read_consistency_ == FROZEN))
    {
        if (!is_rpc_failed_)
        {
            ...
            if (OB_ALL_SEQUENCE_TID == table_id_)
            {
              err = sequence_frozen_data_cache.put(frozen_data_key_, frozen_data_);
            }
            else
            {
             err = frozen_data_cache.put(frozen_data_key_, frozen_data_);
            }
            //int err = frozen_data_cache.put(frozen_data_key_, frozen_data_);
            //add e
            if (OB_SUCCESS != err)
            {
                YYSYS_LOG(ERROR, "ObFrozenDataCache put error, err: %d", err);
            }
        }
        frozen_data_.reuse();
    }
    ...
    if (is_get_||is_use_index_)  //modify by fanqiushi_index
    {
        ...
        if (insert_cache_need_revert_)
        {
            ...
            {
                YYSYS_LOG(WARN, "revert bloom filter failed, ret=%d", ret);
            }
            insert_cache_need_revert_ = false;
        }
...
//    }
    ...
    //int64_t usage = 0;
    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION);
    //YYSYS_LOG(INFO,"wanglei::CLOSE OB_SEMI_JOIN_SQL_EXPRESSION clear malloc usage = %ld",usage);
    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
    //YYSYS_LOG(INFO,"wanglei::CLOSE OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER free malloc usage = %ld",usage);
    //for memory leak check:e
    cleanup_request();
    return ret;
}
void ObRpcScan::cleanup_request()
{
    ...
    if (sql_scan_request_ != NULL)
    {
        ...
        {
            YYSYS_LOG(WARN, "free scan request back to scan req pool failed, ret=%d", tmp_ret);
        }
        sql_scan_request_ = NULL;
    }
    if (sql_get_request_ != NULL)
    {
        ...
        {
            YYSYS_LOG(WARN, "free get request back to get req pool failed, ret=%d", tmp_ret);
        }
        sql_get_request_ = NULL;
    }
}
...
int ObRpcScan::get_cur_row_desc(const common::ObRowDesc *&row_desc) const
{
    int ret = OB_SUCCESS;
    if(OB_UNLIKELY(base_table_id_ <= 0 || 0 >= cur_row_desc_.get_column_num()))
    {
		YYSYS_LOG(ERROR, "not init, tid=%lu, column_num=%ld", base_table_id_, cur_row_desc_.get_column_num());
        ret = OB_NOT_INIT;
    }
    else
    {
...
int ObRpcScan::get_row_desc(const common::ObRowDesc *&row_desc) const
{
    ...
    /*else
  {
    ...
    else if(is_use_index_for_storing_)
    {
        ...
        //YYSYS_LOG(ERROR,"test::fanqs,,row_desc=%s",to_cstring(*row_desc));
    }
    else
    {
        row_desc = &cur_row_desc_;
...
int ObRpcScan::get_next_row(const common::ObRow *&row)
{
    ...
    if (rowkey_not_exists_)
    {
        if (insert_cache_iter_counter_ == 0)
        {
            ...
            {
                YYSYS_LOG(ERROR, "set cell failed, err=%d", err);
            }
            else
            {
                row = &row_not_exists_;
    ...
    else
    {
        if (cached_frozen_data_.has_data())
        {
            ...
            {
                YYSYS_LOG(ERROR, "ObCachedFrozenData get_next_row error, ret: %d", ret);
            }
        }
        else
        {
            ...
            else if (ObSqlReadStrategy::USE_GET == hint_.read_method_)
            {
                ...
                //        if(ret == OB_SUCCESS)
                //            YYSYS_LOG(ERROR,"wanglei::sql_get_request_ get_next_row= %s",to_cstring(cur_row_));
            }
            else
            {
                YYSYS_LOG(WARN, "not init. read_method_=%d", hint_.read_method_);
                ret = OB_NOT_INIT;
            }
                    ...
                    && need_cache_frozen_data_ && (hint_.read_consistency_ == FROZEN) && OB_SUCCESS == ret)
            {
                ...
                if (OB_SUCCESS != ret)
                {
                    YYSYS_LOG(ERROR, "ObRowStore add_row error, ret: %d", ret);
                }
            }
        }
    }
    ...
    if (NULL != row && is_data_mark_param_valid_)
        YYSYS_LOG(DEBUG,"mul_del::debug,final_rpc_row=[%s],ret=%d",
                  to_cstring(*row),ret);
    //add duyr 20160531:e
...
int ObRpcScan::get_next_compact_row_for_index(const common::ObRow *&row)  //第一次scan索引表，返回索引表的数据
{
    ...
    //YYSYS_LOG(ERROR, "test::fanqs,,enter this,,tmp_desc=%s,,cur_row_desc=%s",buf,to_cstring(cur_row_desc_));
    cur_row_.set_row_desc(cur_row_desc_);
    //add:e
    int ret = OB_SUCCESS;
    ...
    else
    {
        ...
        do
        {
            ...
            else if (OB_LIKELY(OB_SUCCESS == (ret = sql_scan_request_->get_next_row(cur_row_))))
            {
                ...
                //add fanqiushi_index
                //YYSYS_LOG(ERROR, "test::fanqs,,cur_row_= %s,,", to_cstring(cur_row_));
                //add:e
                can_break = true;
            }
            ...
            else if (OB_ITER_END == ret)
            {
                ...
                //add fanqiushi_index
                //YYSYS_LOG(ERROR, "test::fanqs,,hello,,,timeout_us_=%ld,,remain_us=%ld",timeout_us_,remain_us);
                //add:e
                can_break = false;
                ...
                if( OB_SUCCESS != (ret = sql_scan_request_->wait_single_event(timeout_us_)))
                {
                    if (timeout_us_ <= 0)
                    {
                        YYSYS_LOG(WARN, "wait timeout. timeout_us_=%ld", timeout_us_);
                    }
              ...
              {
                  ret = OB_ITER_END;
                  YYSYS_LOG(WARN, "wanglei::test ret=%d,wait_single_event faild",ret);
              }*/
                    }
                    //add e
                }
                ...
                {
                    YYSYS_LOG(DEBUG, "got a scan event. timeout_us_=%ld", timeout_us_);
                }
            }
            else
            {
                // encounter an unexpected error or
                YYSYS_LOG(WARN, "Unexprected error. ret=%d, cur_row_desc[%s], read_method_[%d]", ret, to_cstring(cur_row_desc_), hint_.read_method_);
                can_break = true;
            }
        ...
        if (OB_SUCCESS == ret)
        {
            row = &cur_row_;
            //add fanqiushi_index
            //YYSYS_LOG(ERROR, "test::fanqs,,row=%s",to_cstring(cur_row_) );
            //add:e
        }
        //add fanqiushi_index
        //YYSYS_LOG(ERROR, "test::fanqs ret=%d",ret );
        //add:e
...
int ObRpcScan::get_next_compact_row(const common::ObRow *&row)
{
    int ret = OB_SUCCESS;
    //modify by fanqiushi_index
    //YYSYS_LOG(ERROR,"test::fanqs,,is_use_index_=%d",is_use_index_);
    ...
    else
    {
        ...
        // YYSYS_LOG(ERROR,"test::fanqs,,is_use_index_=%d,,get_next_row_count_=%ld",is_use_index_,get_next_row_count_);
...
//            OB_ASSERT(NULL == get_param_);
        ...
        else
        {
            ...
            //add fanqiushi_index
            //YYSYS_LOG(ERROR,"test::fanqs get_next_row_count_=%ld",get_next_row_count_);
            //YYSYS_LOG(ERROR,"test::fanqs cur_row_=%s",to_cstring(cur_row_));
            //add:e
        }
    }
    ...
    if (OB_SUCCESS == ret)
    {
        //YYSYS_LOG(ERROR,"test::fanqs,1110,cur_row_=%s,cur_row_desc_=%s",to_cstring(cur_row_),to_cstring(cur_row_desc_));
        if(is_use_index_for_storing_)
        {
            uint64_t tid=OB_INVALID_ID;
            uint64_t cid=OB_INVALID_ID;
            const ObObj *obj_tmp=NULL;
            // YYSYS_LOG(ERROR,"test::fanqs,,cur_row_=%s,cur_row_desc_=%s",to_cstring(cur_row_),to_cstring(cur_row_desc_));
            for(int64_t i=0;i<cur_row_.get_column_num();i++)   //根据索引表的一行构造原表的一行
            {
                cur_row_desc_.get_tid_cid(i,tid,cid);
                //YYSYS_LOG(ERROR,"test::fanqs,,tid=%ld,,main_table_id_=%ld,base_table_id_=%ld",tid,main_table_id_,base_table_id_);
            ...
            //YYSYS_LOG(ERROR, "test::fanqs,,cur_row_for_storing_=%s,,desc=%s,,cur_row_desc_for_storing=%s",to_cstring(cur_row_for_storing_),to_cstring(*desc),to_cstring(cur_row_desc_for_storing) );
        }
        else
            row = &cur_row_;
        //add fanqiushi_index
        //YYSYS_LOG(ERROR, "test::fanqs,,row=%s",to_cstring(*row) );
        //add:e
    }
    //modify:e
    return ret;
...
int ObRpcScan::add_output_column(const ObSqlExpression& expr, bool change_tid_for_storing)
{
    ...
    {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "must call set_table() first. base_table_id_=%lu",
                  base_table_id_);
    }
    else if ((OB_SUCCESS == (ret = expr.is_column_index_expr(is_cid)))  && (true == is_cid))
    {
        ...
        {
            YYSYS_LOG(WARN, "fail to add output column ret=%d", ret);
        }
    }
    else
    {
    ...
    {
        YYSYS_LOG(WARN, "fail to add column to scan param. ret=%d, tid_=%lu, cid=%lu", ret, expr.get_table_id(), expr.get_column_id());
    }
...
int ObRpcScan::set_table(const uint64_t table_id, const uint64_t base_table_id)
{
    ...
    if (0 >= base_table_id)
    {
        YYSYS_LOG(WARN, "invalid table id: %lu", base_table_id);
        ret = OB_INVALID_ARGUMENT;
    }
    else
    {
...
int ObRpcScan::cons_get_rows(ObSqlGetParam &get_param)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to find rowkeys in IN operator. ret=%d", ret);
    }
    else if (get_rowkey_array_.count() > 0)
    {
        ...
        for (idx = 0; idx < get_rowkey_array_.count(); idx++)
        {
            ...
            {
                YYSYS_LOG(WARN, "fail to add rowkey to get param. ret=%d", ret);
                break;
            }
        }
    }
    ...
    {
        YYSYS_LOG(WARN, "fail to find rowkeys from where equal condition, ret=%d", ret);
    }
    else if (get_rowkey_array_.count() > 0)
    {
        for (idx = 0; idx < get_rowkey_array_.count(); idx++)
...
int ObRpcScan::cons_get_rows_for_index(ObSqlGetParam &get_param)  //根据第一次scan索引表返回的数据，构造第二次get原表的主键的范围
{
    ...
    {
        if(NULL == index_row_ || NULL == index_row_->get_row_desc())
        {
            YYSYS_LOG(ERROR, "index_row_ is null or get index row_desc failed, ret=%d", ret);
            ret = OB_NOT_INIT;
            break;
        }
        //YYSYS_LOG(ERROR,"test::fanqs,,enter while,,index_row_=%s,,key_count=%d",to_cstring(*index_row_),key_count);
        ...
        {
            //YYSYS_LOG(ERROR,"test::fanqs,,enter if");
        ...
        {
            ret = OB_ITER_END;
            //YYSYS_LOG(ERROR,"test::fanqs,,key_count=%d",key_count);
            break;
        }
        //index_row_desc->
        //YYSYS_LOG(ERROR, "test::fanqs,,index_row_=%s,,index_row_desc=%s,,main_rowkey_info=%s,main_rowkey_info_.get_size()=%ld,ret=%d",to_cstring(*index_row_),to_cstring(*index_row_desc),to_cstring(main_rowkey_info_),main_rowkey_info_.get_size(),ret);
    }
    //YYSYS_LOG(ERROR,"test::fanqs,,ret=%d",ret);
    if(ret!=OB_ITER_END)
    {
        YYSYS_LOG(WARN,"faild to get index row to construct roweky,ret=%d",ret);
    }
    ...
    /*else
  {
      ...
      {
         // YYSYS_LOG(ERROR,"test::fanqs,,get_rowkey_array_.count()=%ld",get_rowkey_array_.count());
        for (idx = 0; idx < get_rowkey_array_.count(); idx++)
        {
          //深拷贝，从rowkey_objs_allocator 拷贝到了allocator_中
            //YYSYS_LOG(ERROR,"test::fanqs,,get_rowkey_array_.at(idx)=%s,,idx=%ld",to_cstring(get_rowkey_array_.at(idx)),idx);
...
int ObRpcScan::add_index_filter_ll(ObSqlExpression* expr)
{
    ...
    {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(ERROR,"[semi join] allocate memory failed! no memory!");
    }
    else
    {
//        tmp_expr->copy (expr);
...
int ObRpcScan::cons_scan_range(ObNewRange &range)
{
    ...
    //  {
    //    YYSYS_LOG(WARN, "fail to find range %lu", base_table_id_);
    //  }
    //del:e
    if (OB_SUCCESS != (ret = sql_read_strategy_.find_scan_range(found,false)))//single_row_only ==false
    {
        YYSYS_LOG(WARN, "construct  range %lu failed", base_table_id_);
    }
    ...
    }/*else if ( has_next )
  {
    ...
    else if (my_phy_plan_->get_has_range())
    {
        ...
        YYSYS_LOG(INFO, "DEBUG:HINT, start_key: %s, end_key: %s",to_cstring(start_key),to_cstring(end_key));
    ...
    //add 20141120:e
    YYSYS_LOG(DEBUG, "get range %lu ,has_next=%d", base_table_id_,has_next);
    //  sql_read_strategy_.has_next_range(has_next);
    // YYSYS_LOG(INFO, "get range %lu ,has_next=%d", base_table_id_,has_next);
    //mod: end
    return ret;
}
...
int ObRpcScan::get_min_max_rowkey(const ObArray<ObRowkey> &rowkey_array, ObObj *start_key_objs_, ObObj *end_key_objs_, int64_t rowkey_size)
{
    ...
    {
        YYSYS_LOG(WARN, "only support single insert row for scan optimization. rowkey_array.count=%ld", rowkey_array.count());
...
int ObRpcScan::add_filter(ObSqlExpression* expr)
{
    ...
    {
        YYSYS_LOG(WARN, "fail to add filter to sql read strategy:ret[%d]", ret);
    }
    if (OB_SUCCESS == ret && OB_SUCCESS != (ret = read_param_->add_filter(expr)))
    {
        YYSYS_LOG(WARN, "fail to add composite column to scan param. ret=%d", ret);
    }
    return ret;
}
...
int ObRpcScan::add_aggr_column(const ObSqlExpression& expr)
{
    ...
    {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "Output column(s) of ObRpcScan must be set first, ret=%d", ret);
    }
    ...
    {
        YYSYS_LOG(WARN, "Failed to add aggregate column desc, err=%d", ret);
    }
    return ret;
}
...
int ObRpcScan::cons_row_desc(const ObSqlGetParam &sql_get_param, ObRowDesc &row_desc)
{
    ...
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "should has project");
    }
    ...
    if (OB_SUCCESS == ret)
    {
        if ( sql_get_param.get_row_size() <= 0 )
        {
            ...
            YYSYS_LOG(WARN, "should has a least one row");
        }
        else
        {
...
int ObRpcScan::remove_or_sub_query_expr()
{
    ...
    {
        YYSYS_LOG(WARN, "fail to remove sub query expr from readparam. ret=%d", ret);
    }
    return ret;
}
...
PHY_OPERATOR_ASSIGN(ObRpcScan)
{
    ...
    {
        YYSYS_LOG(WARN, "Assign ObSqlReadStrategy failed, ret=%d", ret);
    }
    else if (o_ptr->scan_param_)
    {
        ...
        else
        {
            ...
            {
                YYSYS_LOG(WARN, "Assign Scan param failed, ret=%d", ret);
            }
        }
    }
    else if (o_ptr->get_param_)
    {
        ...
        else
        {
            ...
            {
                YYSYS_LOG(WARN, "Assign Get param failed, ret=%d", ret);
            }
        }
    }
    sql_read_strategy_.set_rowkey_info(rowkey_info_);
...
//          {
//            YYSYS_LOG(WARN, "get cell failure , ret=[%d]", ret);
//            break;
//          }
//          else
//          {
...
//                                                      sizeof(buff))))
//            {
//              YYSYS_LOG(WARN, "get data faliure , ret=[%d]", ret);
//              break;
//            }
//            else
//            {
...
//              {
//                last_row = null_row;
//                YYSYS_LOG(INFO,"[semi join]::first row = %s, skip %ld null row",to_cstring(null_row), null_row_counter);
//                const common::ObObj *cell_temp=NULL;
//                if(OB_SUCCESS!=(ret=null_row.get_cell(left_table_id_,left_column_id_,cell_temp)))
//                {
//                  YYSYS_LOG(WARN, "get cell failure , ret=[%d]", ret);
...
//                  {
//                    YYSYS_LOG(ERROR, "failed to set column value, err=%d", ret);
//                  }
//                  else
//                  {
...
//                    {
//                      YYSYS_LOG(ERROR,"Add cell value failed");
//                    }
//                    else
//                    {
//                      // 获取到第一行就break
...
//              // YYSYS_LOG(INFO, "[semi join]::str_temp =%s", buff);
//            }
//          }
//        }
...
//            {
//                last_row = row_temp;
//                YYSYS_LOG(DEBUG,"[semi join]::first row = %s",to_cstring(row_temp));
//                const common::ObObj *cell_temp=NULL;
...
//                        }
//                        //string str_temp(buff);
//                        YYSYS_LOG(DEBUG, "[semi join]::str_temp  =%s", buff);
//                    }
//                }
...
//            if(ret == OB_ITER_END)
//            {
//                YYSYS_LOG(DEBUG,"[semi join]::last_row = %s",to_cstring(last_row));
//                ret = OB_SUCCESS;
//                const common::ObObj *cell_temp=NULL;
...
//                YYSYS_LOG(DEBUG,"[semi join]::table_filter_expr_ = %s",to_cstring(*table_filter_expr_));
//                is_in_expr_empty_ = false;
...
//            {
//                if(OB_SUCCESS!=ret)
//                    YYSYS_LOG(WARN, "no more rows , sret=[%d]", ret);
//                else
//                {
//                    YYSYS_LOG(DEBUG,"[semi join]::row_temp = %s",to_cstring(row_temp));
//                    const common::ObObj *cell_temp=NULL;
//                    if(OB_SUCCESS!=(ret=row_temp.get_cell(left_table_id_,left_column_id_,cell_temp)))
//                    {
//                        YYSYS_LOG(WARN, "get cell failure , ret=[%d]", ret);
...
////                                    default:
////                                      ret = OB_NOT_SUPPORTED;
////                                      YYSYS_LOG(WARN, "obj type not support, type=%d", cell_temp->get_type());
////                                      break;
////                                  }
////                                  if (OB_LIKELY(OB_SUCCESS == ret))
////                                  {
...
////                                            break;
////                                          default:
////                                            YYSYS_LOG(WARN, "unexpected expression type %d", item.type_);
...
int ObRpcScan::get_table_row_with_more_scan(const common::ObRow *&row)
{
	...
	if(is_first_cons_scan_param_)
	{
		...
		if(OB_SUCCESS != (ret = cons_scan_request()))
		{
			if(OB_ITER_END != ret)
				YYSYS_LOG(WARN, "cons_first_scan_request failed, ret =%d", ret);
		}
//    if(is_first_cons_scan_param_)
//    {
//        is_first_cons_scan_param_ = false;
...
//        //for memory leak check:b
//        //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
//        //YYSYS_LOG(INFO,"wanglei:: OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER before malloc usage = %ld",usage);
//        //for memory leak check:e
...
//            //for memory leak check:b
//            //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
//            //YYSYS_LOG(INFO,"wanglei:: OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER after malloc usage = %ld",usage);
//            //for memory leak check:e
...
//                    YYSYS_LOG(WARN, "add filter to filter_raw error, ret =%d", ret);
//                  }
//                }
...
//                  {
//                    YYSYS_LOG(WARN, "push filter into sql_read_strategy_ failed.ret[%d]", ret);
//                  }
//                }
//                //add 2016-12-21 e
...
//                if(ret == OB_SUCCESS)
//                {
//                    YYSYS_LOG(DEBUG,"[semi join]::first>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
//                    YYSYS_LOG(DEBUG,"[semi join]::filter_expr_ = %s",to_cstring(*filter_expr_));
//                    YYSYS_LOG(DEBUG,"[semi join]::filter_raw = %s",to_cstring(filter_raw));
//                    YYSYS_LOG(DEBUG,"[semi join]::sql_read_strategy_ = %s",to_cstring(sql_read_strategy_));
//                    YYSYS_LOG(DEBUG,"[semi join]::first<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
...
//                    //for memory leak check:b
//                    //usage = ob_get_mod_memory_usage(ObModIds::OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER);
//                    //YYSYS_LOG(INFO,"wanglei:: OB_SEMI_JOIN_SQL_EXPRESSION_CONS_FILTER free malloc usage = %ld",usage);
//                    //for memory leak check:e
//                }
//            }
//            //防止内存泄露
        ...
        else
        {
            do
            {
                ...
                else if (OB_ITER_END == ret && sql_scan_request_->is_finish())
                {
                  //mod dragon [type_err] 2016-12-6 b
                  int err = OB_SUCCESS;
                  YYSYS_LOG(DEBUG, "[semi join]::start cons second scan param");
                  if(OB_SUCCESS != (err = sql_read_strategy_.remove_last_inexpr ()))
                  {
                    YYSYS_LOG(WARN, "sql_read_strategy_.remove_last_inexpr failed. err=%d", err);
                  }
                  else if(OB_SUCCESS != (err = filter_raw.remove_last_filter ()))
                  {
                    YYSYS_LOG(WARN, "remove last filter failed. err=%d", err);
                  }
                  else if(OB_SUCCESS != (err = cons_scan_request(&can_break)))
                  {
                      if(OB_ITER_END != err)
                          YYSYS_LOG(WARN, "cons scan_request failed, ret=%d", err);
                  }
...
//                    }
                  ...
                  {
                    YYSYS_LOG(WARN, "Unexprected error.construct next scan request failed, ret=%d", err);
                    ret =err;
                    can_break = true;
                  }
                  if(use_btw_expr_ && can_break == false)
                  {
                      YYSYS_LOG(DEBUG, "semi join::should not happen");
                      break;
                  }
...
//                                else
//                                {
//                                    YYSYS_LOG(DEBUG,"[semi join]::second>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
//                                    YYSYS_LOG(DEBUG,"[semi join]::filter_expr_ = %s",to_cstring(*filter_expr_));
//                                    YYSYS_LOG(DEBUG,"[semi join]::filter_raw = %s",to_cstring(filter_raw));
//                                    YYSYS_LOG(DEBUG,"[semi join]::sql_read_strategy_ = %s",to_cstring(sql_read_strategy_));
//                                    YYSYS_LOG(DEBUG,"[semi join]::second<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
...
//            }
    ...
    if(OB_UNLIKELY(row == NULL && OB_SUCCESS == ret))
    {
        YYSYS_LOG(ERROR, "can not happen,ret = %d", ret);
    }
...
//    UNUSED(args);
//    int ret = OB_SUCCESS;
//    YYSYS_LOG(ERROR,"wanglei::thread start=================================");
//    while(true)
//    {
...
//                            {
//                                YYSYS_LOG(WARN, "wait timeout. timeout_us_=%ld", rpc_scan->get_timeout_us());
//                            }
//                            can_break = true;
//                        }
//                        else
//                        {
//                            YYSYS_LOG(DEBUG, "got a scan event. timeout_us_=%ld", rpc_scan->get_timeout_us());
//                        }
//                    }
//                    else
//                    {
...
//    if(is_empty)
//         YYSYS_LOG(ERROR,"wanglei:one_cache is empty");
//     YYSYS_LOG(ERROR,"wanglei::thread stop=================================");
//    return 0;
//}
...
//    int ret = OB_SUCCESS;
    ...
    //    else
    //    {
    //        YYSYS_LOG(DEBUG, "next scan_param=%s", to_cstring(*scan_param));
    //    }
    //    YYSYS_LOG(TRACE, "dump scan range: %s", to_cstring(range));
//    return ret;
//}
...
//                    // got a row without block,
//                    // no need to check timeout, leave this work to upper layer
//                    YYSYS_LOG(ERROR,"wanglei::thread row = %s",to_cstring(cur_row));
//                   // const ObRowStore::StoredRow *stored_row = NULL;
//                    //rpc_scan->get_one_cache().add_row(rpc_scan->get_cur_row_ (), stored_row);
//                    can_break = true;
//                }
...
//            {
//                YYSYS_LOG(WARN, "free scan request back to scan req pool failed, ret=%d", ret);
//            }
...
int ObRpcScan::set_data_mark_param(const ObDataMarkParam &param)
{
    ...
    {
        ret = OB_NOT_INIT;
        YYSYS_LOG(ERROR,"read param is NULL!ret=%d",ret);
    }
...
//    //mod gaojt [Delete_Update_Function_isolation] [JHOBv0.1] 20161214:b
    ...
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"orig_tid[%ld] != param_tid[%ld]!ret=%d",
...
//        data_mark_ = param;
//        YYSYS_LOG(DEBUG,"gjt-ud:data_mark_[%s]",to_cstring(data_mark_));
        //add 20161214:e
    }
    return ret;
}
int ObRpcScan::get_data_mark_param(const ObDataMarkParam *&param)
{
    ...
    {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR,"shouldn't get null param!ret=%d",ret);
    }
    return ret;
}
int ObRpcScan::add_data_mark_cids_()
{
    ...
    {
        YYSYS_LOG(WARN,"fail to get data mark param!ret=%d",ret);
    }
    else if (NULL == param)
    {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR,"data mark param is NULL!ret=%d",ret);
    }
    else if (!param->is_valid())
    {
        ret = OB_INVALID_ARGUMENT;
    ...
    {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR,"param should be valid!ret=%d",ret);
    }
//add by maosy  [MultiUps 1.0] [#210]20170215 b:
    else if(is_use_index_for_storing_)
    {
        //todo
        if(param->need_modify_time_ &&
                OB_SUCCESS != (ret = cur_row_desc_for_storing.add_column_desc(param->table_id_,
                                                                              param->modify_time_cid_)))
        {
            YYSYS_LOG(WARN,"fail to add modify time cid!ret=%d",ret);
        }
        else
        {
            read_param_->get_data_mark_param().table_id_ = base_table_id_;
        }
    }
    if(OB_SUCCESS !=ret )
    {}
//add by maosy 20170517 e
    ...
    {
        YYSYS_LOG(WARN,"fail to add major_ver cid!ret=%d",ret);
    }
    else if (param->need_minor_version_
             && OB_SUCCESS != (ret = add_special_output_column_(param->table_id_,
                                                                param->minor_ver_start_cid_)))
    {
        YYSYS_LOG(WARN,"fail to add minor_ver_start_cid!ret=%d",ret);
    }
    else if (param->need_minor_version_
             && OB_SUCCESS != (ret = add_special_output_column_(param->table_id_,
                                                                param->minor_ver_end_cid_)))
    {
        YYSYS_LOG(WARN,"fail to add minor_ver_end_cid_!ret=%d",ret);
    }
    else if (param->need_data_store_type_
             && OB_SUCCESS != (ret = add_special_output_column_(param->table_id_,
                                                                param->data_store_type_cid_)))
    {
        YYSYS_LOG(WARN,"fail to add data store type cid!ret=%d",ret);
    }
	//add by maosy  [MultiUps 1.0] [secondary index optimize]20170401 b:
    else if(is_use_index_)
    {
        uint64_t tid = param->table_id_;
        uint64_t cid = param->modify_time_cid_;
        ObSqlExpression special_column;
        ...
        {
            YYSYS_LOG(WARN,"failed to add ,ret = %d",ret);
        }
        tid = read_param_->get_table_id();
        ObSqlExpression index_special_column;
        index_special_column.set_tid_cid(tid, cid);
        if (OB_SUCCESS != (ret = ObSqlExpressionUtil::make_column_expr(tid, cid, index_special_column)))
        {
          YYSYS_LOG(WARN, "fail to create column expression. ret=%d,tid=%ld,cid=%ld",
                    ret,tid,cid);
        }
        else if(OB_SUCCESS !=(ret = index_project_.add_output_column(index_special_column)))
        {
            YYSYS_LOG(WARN,"failed to add ,ret = %d",ret);
        }
    }
...
int ObRpcScan::add_special_output_column_(const uint64_t tid, const uint64_t cid)
{
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to add special output column. ret=%d,tid=%ld,cid=%ld",
                  ret,tid,cid);
      }
    }
...
//add duyr 20160531:e
int ObRpcScan::add_raw_filter(ObSqlExpression &expr)
{
    int ret = OB_SUCCESS;
    ObSqlExpression *expr_clone = ObSqlExpression::alloc();
    if(NULL == expr_clone)
...
int ObRpcScan::add_right_table_filter(const common::ObObj *cell_temp, ObSqlExpression &table_filter_expr)
{
    int ret = OB_SUCCESS;
    ObConstRawExpr col_val;
    if(NULL == cell_temp)
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "cell is null , ret =[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = col_val.set_value_and_type(*cell_temp)))
    {
        YYSYS_LOG(WARN,"failed to set column value, err=%d", ret);
    }
    else if(OB_SUCCESS !=(ret = col_val.fill_sql_expression(table_filter_expr)))
    {
        YYSYS_LOG(WARN, "Add cell value failed");
    }
    return ret;
}

int ObRpcScan::cons_scan_request(bool *can_break)
{
    int ret = OB_SUCCESS;
    ...
    else
    {
        ...
        bool is_in_expr_empty = true;
        if(OB_SUCCESS != (ret = cons_right_table_filter(filter_expr,is_in_expr_empty)))
        {
            if(OB_ITER_END != ret)
            {
                need_break = true;
                YYSYS_LOG(WARN,"construct right table filter failed ret=%d",ret);
            }
        }
        else if(!is_in_expr_empty)
        {
            sql_read_strategy_.reset();
            sql_read_strategy_.set_rowkey_info(rowkey_info_);
            if(OB_SUCCESS != (ret = sql_read_strategy_.add_filter(filter_expr)))
            {
                YYSYS_LOG(WARN,"push filter into sql_read_strategy_ failed.ret[%d]",ret);
            }
            else if(OB_SUCCESS != (ret = add_raw_filter(filter_expr)))
            {
                YYSYS_LOG(WARN,"add raw filter failed,ret=%d",ret);
            }
            else if(OB_UNLIKELY(NULL == sql_scan_request_))
            {
                ret = OB_NOT_INIT;
                YYSYS_LOG(WARN, "sql_scan_request_ is not inited");
            }
            if(OB_SUCCESS == ret)
            {
                if(NULL == scan_param_){
    ...
    if(NULL != can_break)
    {
        *can_break = need_break;
    }
    return ret;
}

int ObRpcScan::cons_right_table_filter(ObSqlExpression &table_filter_expr, bool &is_in_expr_empty)
{
    int ret = OB_SUCCESS;
    is_in_expr_empty = true;
    table_filter_expr.reset();
    int32_t null_row_counter =0;

    table_filter_expr.set_tid_cid(right_table_id_,right_column_id_);
    ExprItem ref_item;
    ref_item.type_ = T_REF_COLUMN;
    ref_item.value_.cell_.tid = right_table_id_;
    ref_item.value_.cell_.cid = right_column_id_;
    if(OB_SUCCESS != (ret = table_filter_expr.add_expr_item(ref_item)))
    {
        YYSYS_LOG(WARN,"table filter add T_REF_COLUMN expr item failed,ret=%d",ret);
    }
    if(OB_SUCCESS == ret && use_btw_expr_)
    {
        common::ObObj first_join_obj;
        common::ObObj last_join_obj;
        ObRow cur_row;
        bool get_first_row = false;
        bool get_last_row = false;
        cur_row.set_row_desc(*left_row_desc_);

        while (OB_SUCCESS == ret)
        {
            const common::ObObj *cell_temp = NULL;
            if(OB_SUCCESS != (ret = semi_join_left_table_row_->get_next_row(cur_row)))
            {
                if(OB_ITER_END != ret)
                {
                    YYSYS_LOG(WARN,"get next row from semi_join_left_table_row_ failed,ret=%d",ret);
                }
            }
            else if(OB_SUCCESS != (ret = cur_row.get_cell(left_table_id_,left_column_id_, cell_temp)))
            {
                YYSYS_LOG(WARN,"get cell failure, ret=[%d]",ret);
            }
            else if(NULL != cell_temp)
            {
                if(cell_temp->get_type() == ObNullType)
                {
                    null_row_counter++;
                    if(get_first_row)
                    {
                ...
                else if(!get_first_row)
                {
                    get_first_row = true;
                    first_join_obj = *cell_temp;
                }
                else
                {
                    if(get_last_row)
        ...
        if(OB_ITER_END == ret)
        {
            ...
            if(get_first_row)
            {
                if(OB_SUCCESS != (ret = add_right_table_filter(&first_join_obj, table_filter_expr)))
                {
                    YYSYS_LOG(WARN, "add first join cell into right table filter failed,ret=%d",ret);
                }
                else if(get_last_row)
                {
                    if(OB_SUCCESS != (ret = add_right_table_filter(&last_join_obj,table_filter_expr)))
                    {
                        YYSYS_LOG(WARN, "add last row cell into right table filter failed,ret=%d",ret);
                    }
                }
                else if(OB_SUCCESS != (ret = add_right_table_filter(&first_join_obj, table_filter_expr)))
                {
                    YYSYS_LOG(WARN, "add first join cell into right table filter failed,ret=%d",ret);
                }
            }
        ...
        if(ret == OB_SUCCESS)
        {
            ...
            table_filter_expr.add_expr_item_end();
            is_in_expr_empty = false;
        }
        is_left_row_cache_complete_ = true;
        first_join_obj.reset();
        last_join_obj.reset();
    }
    else if(OB_SUCCESS == ret && use_in_expr_)
    {
        int index_param_count = 0;
        ExprItem dem2,dem3,dem5,dem6,dem7;
        dem2.type_ = T_OP_ROW;
        dem2.data_type_ = ObMinType;
        dem2.value_.int_ = 1;
        table_filter_expr.add_expr_item(dem2);
        ...
        dem3.data_type_ = ObMinType;
        dem3.value_.int_ = 2;
        table_filter_expr.add_expr_item(dem3);
        ObRow row_temp;
        row_temp.set_row_desc(*left_row_desc_);
        while (OB_SUCCESS ==ret)
        {
            char buff[300];
            const common::ObObj *cell_temp = NULL;
            if(OB_SUCCESS != (ret = semi_join_left_table_row_->get_next_row(row_temp)))
            {
                if(OB_ITER_END != ret)
                {
                    YYSYS_LOG(WARN, "get next row from semi_join_left_table_row_ failed,ret=%d",ret);
            ...
            else if(OB_SUCCESS != (ret = row_temp.get_cell(left_table_id_, left_column_id_, cell_temp)))
            {
                YYSYS_LOG(WARN, "get cell failure,ret=[%d]",ret);
            }
            else if(OB_SUCCESS !=
                    (ret = get_data_for_distinct(cell_temp,cell_temp->get_type(),buff,sizeof(buff))))
            {
                YYSYS_LOG(WARN, "get data failure,ret=[%d]",ret);
            }
            else
            {
				string str_temp(buff);
				...
				{
					is_confilct_.insert(make_pair<string,bool>(str_temp,tmp));
					if(OB_SUCCESS != (ret = add_right_table_filter(cell_temp, table_filter_expr)))
					{
						YYSYS_LOG(WARN, "add join cell into right table filter failed,ret=%d",ret);
					}
					else
					{
						dem5.type_ = T_OP_ROW;
						dem5.data_type_ = ObMinType;
						dem5.value_.int_ = 1;
						table_filter_expr.add_expr_item(dem5);
						index_param_count++;
					}
				}
			}
            if(index_param_count >= MAX_IN_EXPR_LIMIT_NUM)
            {
                break;
            }
        }
        ...
        if(OB_SUCCESS == ret)
        {
            ...
            dem6.data_type_ = ObMinType;
            dem6.value_.int_ = index_param_count;
            table_filter_expr.add_expr_item(dem6);
            dem7.type_ = T_OP_IN;
            dem7.data_type_ = ObMinType;
            dem7.value_.int_ = 2;
            table_filter_expr.add_expr_item(dem7);
            table_filter_expr.add_expr_item_end();
            is_in_expr_empty = false;
        }
    }
