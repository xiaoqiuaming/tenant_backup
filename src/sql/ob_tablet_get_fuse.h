      ...
      public:
        ...
        inline int set_read_atomic_param(const ObReadAtomicParam &read_atomic_param)
        {
          ...
          if (read_atomic_param_.is_valid())
            YYSYS_LOG(DEBUG,"read_atomic::debug,get fuse read atomic param=[%s],ret=%d",
                    to_cstring(read_atomic_param_),ret);

          return ret;
        }

        inline int set_get_param(const ObGetParam &get_param)
        {
          int ret = OB_SUCCESS;
