...
int ObSequenceDelete::copy_sequence_info_from_delete_stmt()
{
  ...
  if (NULL != delete_stmt)
  {
    ...
    for (int32_t i = 0; OB_SUCCESS == ret && i < sequence_names.count(); i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "copy sequence names failed!");
      }
    }
    for (int32_t i = 0; OB_SUCCESS == ret && i < sequence_expr_ids.count(); i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "copy sequence expr ids failed!");
      }
    }
  }
  else
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR,"the delete stmt is not inited! ret::[%d]",ret);
  }
  return ret;
}
...
int ObSequenceDelete::fill_the_sequence_info_to_cond_expr(ObSqlExpression *filter, uint64_t cond_expr_id)
{
  int ret = OB_SUCCESS;
  UNUSED(cond_expr_id);
  YYSYS_LOG(DEBUG, "fill sequence info to DELETE condition");
          ...
          else
          {
            ...
            else//find
            {
              ...
              if (OB_SUCCESS == ret && can_use_prevval)
              {
                ...
                YYSYS_LOG(DEBUG,"set decimal,the obj is::[%s]",to_cstring(post_expr_array[4]));
              }
              else
              {
                if (OB_SUCCESS == ret && !can_use_prevval)
