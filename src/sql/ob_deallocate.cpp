...
int ObDeallocate::open()
{
  ...
  {
    ret = OB_ERR_GEN_PLAN;
    YYSYS_LOG(WARN, "Deallocate statement is not initiated, stmt_id=%lu", stmt_id_);
  }
  else
  {
...
int ObDeallocate::delete_plan_from_session()
{
  ...
  {
    YYSYS_LOG(WARN, "remove statment from session failed. ret=%d", ret);
  }
  ...
  */
  {
      ...
      YYSYS_LOG(DEBUG,"gaojt-test-ud: stmt-name=%.*s",stmt_name.length(),stmt_name.ptr());
      ...
      if (my_phy_plan_->get_result_set()->get_session()->plan_exists(stmt_name, &stmt_id) != false)
      {
          ...
          {
              YYSYS_LOG(DEBUG,"fail to remove plan, ret=%d",ret);
          }
          else
          {
              YYSYS_LOG(DEBUG,"Deallocate-ud-success");
          }
      }
  }
