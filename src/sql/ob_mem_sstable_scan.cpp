...
int ObMemSSTableScan::open()
{
  ...
  if (!from_deserialize_)
  {
    ...
    YYSYS_LOG(WARN, "this operator can only open after deserialize");
  }
  else
  {
...
int ObMemSSTableScan::get_next_row(const common::ObRow *&row)
{
  ...
  if (OB_ITER_END == ret)
  {
    YYSYS_LOG(DEBUG, "end of iteration");
  }
  else if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "fail to get next row from row store:ret[%d]", ret);
  }
  else
  {
    row = &cur_row_;
    YYSYS_LOG(DEBUG, "[MemSSTableScan] %s", to_cstring(cur_row_));
  }
  return ret;
}
...
int ObMemSSTableScan::get_row_desc(const common::ObRowDesc *&row_desc) const
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "cur_row_desc_ is empty");
  }
  else
  {
...
DEFINE_DESERIALIZE(ObMemSSTableScan)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to deserialize row desc:ret[%d]", ret);
  }
  else if (OB_SUCCESS != (ret = row_store_.deserialize(buf, data_len, pos)))
  {
    YYSYS_LOG(WARN, "fail to deserialize row_store:ret[%d]", ret);
  }
  else
  {
