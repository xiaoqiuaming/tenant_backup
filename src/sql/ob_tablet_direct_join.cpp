...
int ObTabletDirectJoin::open()
{
  ...
  {
    YYSYS_LOG(WARN, "ObTabletJoin open:ret[%d]", ret);
  }
  else
  {
    last_ups_rowkey_ = NULL;
...
int ObTabletDirectJoin::get_ups_row(const ObRowkey &rowkey, ObUpsRow &ups_row, const ObGetParam &get_param)
{
  ...
  if (NULL == last_ups_rowkey_)
  {
    ...
    else
    {
      ...
      YYSYS_LOG(WARN, "get param is empty");
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    if (NULL == last_ups_rowkey_ || *last_ups_rowkey_ != rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "ups multi get next row fail:ret[%d], rowkey[%s]", ret, to_cstring(rowkey));
      }
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "cannot find rowkey:rowkey[%s], last_ups_rowkey_[%s]", to_cstring(rowkey), to_cstring(*last_ups_rowkey_));
      }
    }
  }
