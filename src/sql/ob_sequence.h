  ...
  namespace sql
  {
    class ObSequence : public ObMultiChildrenPhyOperator
    {
      public:
        ...
        void set_stmt(ObSequenceStmt* stmt)
        {sequence_stmt_ = stmt;}
        void set_use_lock_row_result_set(bool flag)
        {use_lock_row_result_set_ = flag;}
        void set_sql_context(ObSqlContext *sql_context)
        {sql_context_ = sql_context;}
        void set_result_set( ObResultSet * rs)
        {this->out_result_set_ = rs;}
        ...
        int get_sequence_current_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval, ObDecimal &obj_current_value);
        int get_sequence_inc_by_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool only_update_prevval, ObDecimal &obj_increment_by);
        ...
        int get_sequence_can_use_prev_value_info(const ObRow *&sequence_row, SequenceInfo &s_info, bool update_prevval, ObDecimal &obj_current_value);
        ...
        int get_prevval_by_name(ObString *sequence_name, uint64_t *&prevval, uint32_t &p, uint32_t &s, uint32_t &vs, uint32_t &len,
                                bool &can_use_prevval);
        //int get_nextval_by_name(ObString *sequence_name, ObString &nextval, bool &can_use_nextval);
        int get_nextval_by_name(ObString *sequence_name, uint64_t *&nextval, uint32_t &p, uint32_t &s, uint32_t &vs, uint32_t &len,
                                bool &can_use_nextval);
        ...
        //add wangyao  [SEQUENCE OPTIMIZATION] 20171011:b
        int update_by_dml_special_for_optimize(ObString &sequence_name, SequenceInfo &s_info,bool update_prevval);
        int parse_row(const ObRow *val_row, SequenceInfo &s_info);
        int parse_row_prevval(const ObRow *val_row, SequenceInfo &s_info);
        int parse_row(const ObRow *val_row, SequenceInfo &s_info, bool not_use_prevval);
         //add e
        ...
        void set_project_for_update(ObProject * project)
        {project_for_update_ = project;}
        int add_nextval_to_project_op(uint64_t *nextval, uint32_t p, uint32_t s, uint32_t vs, uint32_t len);
        ...
        int check_column_cast_validity(ObSqlExpression& expr, const int64_t &column_idx, bool is_update = false);
    ...
    inline int ObSequence::add_sequence_names_no_dup()
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(ERROR,"the sequence_stmt_ is not init!");
      }
      else
      {
        ...
        {
//          YYSYS_LOG(ERROR,"add sequence[%.*s]",sequence_stmt_->get_sequence_no_dup_name(i).length(),sequence_stmt_->get_sequence_no_dup_name(i).ptr());
    ...
    inline int ObSequence::check_lock_condition()
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "the lock conditon not inited!");
      }
      return ret;
    }
    ...
    inline bool ObSequence::is_quick_path_sequence(ObString &sequence_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "sequence map not init!");
      }
      else
      {
        if (-1 == (err = sequence_info_map_.get(sequence_name,s_info)))
        {
          YYSYS_LOG(ERROR,"get (%.*s)  map failed!",sequence_name.length(),sequence_name.ptr());
        }
   ...
   inline int ObSequence::add_nextval_to_project_op(uint64_t *nextval, uint32_t p, uint32_t s, uint32_t vs, uint32_t len)
   {
     int ret = OB_SUCCESS;
     if (NULL != project_for_update_)
     {
       ret = project_for_update_->add_nextval_for_update(nextval, p, s, vs, len);
     }
     else
     {
       ret = OB_NOT_INIT;
       YYSYS_LOG(ERROR,"the project is not init!ret=[%d]",ret);
     }
     return ret;
   }
