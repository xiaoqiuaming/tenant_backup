...
ObStmt::ObStmt(ObStringBuf* name_pool, StmtType type)
  : ObBasicStmt(type), name_pool_(name_pool), when_number_(OB_INVALID_INDEX), ud_where_parse_tree_(NULL)//add gaojt [Delete_Update_Function] [JHOBv0.1] 20160425
{
        ...
        )
{
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "logical stmts count: %d", logical_plan->get_stmts_count());
  }
  ...
  if (ret == OB_SUCCESS)
  {
    switch (type)
    {
      ...
      case TableItem::GENERATED_TABLE:
        ...
        item.ref_id_ = ref_id;
        item.table_id_ = logical_plan->generate_table_id();
        if (view_id != OB_INVALID_ID)
        {
          item.table_id_ = view_id;
        }
        break;
      default:
        /* won't be here */
...
int ObStmt::set_real_alias_flag(uint64_t table_id)
{
  int ret = OB_ERROR;
  if(table_items_.size() > 0)
  {
    TableItem *item = table_items_.last();
    if(item != NULL && item->table_id_ == table_id)
    {
      item->is_real_alias_ = false;
      ret = OB_SUCCESS;
    }
  }
  return ret;
    ...
    bool is_logical_optimizer)
{
  ...
  column_item.data_length_ = 0;
  column_item.data_precision_ = 0;
  column_item.data_scale_ = 0;
  ...
  if (column_item.table_id_ != OB_INVALID_ID)
  {
    ret = check_table_column(
            ...
            *table_item,
            ...
            column_item.data_length_ ,
            column_item.data_precision_,
            column_item.data_scale_,
            is_logical_optimizer);
    if (ret != OB_SUCCESS)
    {
      return ret;
    }
  ...
  else
  {
    ...
    for (int32_t i = 0; i < num; i++)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        table_item.has_scan_columns_ = true;
        column_item.data_length_ = data_length;
        column_item.data_precision_ = data_precision;
        column_item.data_scale_ = data_scale;
      }
    ...
    bool is_logical_optimizer)
{
  ...
  if (logical_plan == NULL)
  {
    ret = OB_ERR_LOGICAL_PLAN_FAILD;
    snprintf(result_plan.err_stat_.err_msg_, MAX_ERROR_MSG,
             "Wrong invocation of YaoStmt::check_table_column, logical_plan must exist!!!");
  }
  ObSchemaChecker* schema_checker = NULL;
  if (ret == OB_SUCCESS)
  {
    ...
    switch(table_item.type_)
    {
      ...
      case TableItem::ALIAS_TABLE:
      {
        ...
        if (col_schema != NULL)
        {
          ...
          data_length = static_cast<uint32_t>(col_schema->get_size());
          data_precision = col_schema->get_precision();
          data_scale = col_schema->get_scale();
        }
              ...
              for(int32_t i = 0; i < num; i++)
              {
                ...
                if(column_name == select_item.alias_name_)
                {
                  if(column_id == OB_INVALID_ID)
                  {
                    ...
                    ObSqlRawExpr *sql_expr = logical_plan->get_expr(select_item.expr_id_);
                    OB_ASSERT(sql_expr);
                    ObRawExpr *expr = sql_expr->get_expr();
                    OB_ASSERT(expr);
                    data_length = expr->get_length();
                    data_precision = expr->get_precision();
                    data_scale = expr->get_scale();
                  }
                  else
                  {
                    ret = OB_ERR_COLUMN_DUPLICATE;
...
void ObStmt::print_table_items () const
{
  if (table_items_.size() > 0)
  {
    YYSYS_LOG(INFO, "<TableItemList Begin>\n");
    for (int i = 0; i < table_items_.size(); ++i)
    {
      TableItem& item = table_items_[i];
      YYSYS_LOG(INFO, "{Num %d, TableId:%lu, TableName:%.*s, ",
                i, item.table_id_, item.table_name_.length(), item.table_name_.ptr());
      if (item.alias_name_.length() > 0)
        YYSYS_LOG(INFO, "AliasName:%.*s, ", item.alias_name_.length(), item.alias_name_.ptr());
      else
        YYSYS_LOG(INFO, "AliasName:NULL, ");
      if (item.type_ == TableItem::BASE_TABLE)
        YYSYS_LOG(INFO, "Type:BASE_TABLE, ");
      else if (item.type_ == TableItem::ALIAS_TABLE)
        YYSYS_LOG(INFO, "Type:ALIAS_TABLE, ");
      else if (item.type_ == TableItem::GENERATED_TABLE)
        YYSYS_LOG(INFO, "Type:GENERATED_TABLE, ");
      YYSYS_LOG(INFO,"RefId: %lu}\n", item.ref_id_);
    }
    YYSYS_LOG(INFO, "<TableItemList End>\n");
  }
}
//add e

void ObStmt::print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level)
{
  print_indentation_json(buf, buf_len, pos, level);
  databuff_printf(buf, buf_len, pos, "\"TableItemList\":[\n");
  int32_t i;
  if (table_items_.size() > 0)
  {
    for (i = 0;i<table_items_.size();++i)
    {
      ...
      databuff_printf(buf, buf_len, pos, "{\"Num\":%d,",i);
      databuff_printf(buf, buf_len, pos, "\"TableId\":%ld,", item.table_id_);
      databuff_printf(buf, buf_len, pos, "\"TableName\":\"%.*s\",", item.table_name_.length(), item.table_name_.ptr());
      if (item.alias_name_.length()>0)
        databuff_printf(buf, buf_len, pos, "\"AliasName\":\"%.*s\",", item.alias_name_.length(), item.alias_name_.ptr());
      else
        databuff_printf(buf, buf_len, pos, "\"AliasName\":\"NULL\",");
      if (item.type_ == TableItem::BASE_TABLE)
        databuff_printf(buf, buf_len, pos, "\"Type\":\"BASE_TABLE\",");
      else if (item.type_ == TableItem::ALIAS_TABLE)
        databuff_printf(buf, buf_len, pos, "\"Type\":\"ALIAS_TABLE\",");
      else if (item.type_ == TableItem::GENERATED_TABLE)
        databuff_printf(buf, buf_len, pos, "\"Type\":\"GENERATED_TABLE\",");
      databuff_printf(buf, buf_len, pos, "\"RefId\":%lu}", item.ref_id_);

      if (i < table_items_.size() - 1)
  ...
  print_indentation_json(buf, buf_len, pos, level);
  databuff_printf(buf, buf_len, pos, "\"ColumnItemList\":[\n");
  if (column_items_.size() > 0)
  {
    for (i = 0;i<column_items_.size();++i)
    {
      ...
      print_indentation_json(buf, buf_len, pos, level + 1);
      databuff_printf(buf, buf_len, pos, "{\"Num\":%d,",i);
      databuff_printf(buf, buf_len, pos, "\"ColumnId\":%lu,", item.column_id_);
      databuff_printf(buf, buf_len, pos, "\"ColumnName\":\"%.*s\",", item.column_name_.length(), item.column_name_.ptr());
      databuff_printf(buf, buf_len, pos, "\"TableRef\":%lu}", item.table_id_);

      if (i < column_items_.size() - 1)
        databuff_printf(buf, buf_len, pos, ",");
      databuff_printf(buf, buf_len, pos, "\n");
    }
  }
