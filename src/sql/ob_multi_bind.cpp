...
int ObMultiBind::get_next_row(const ObRow *&row)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "ObMultiBind has no child, ret=%d", ret);
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    else
    {
	  ...
	  { 
	    ...
	    {
	      YYSYS_LOG(WARN, "fail to get sub query expr");
	    }
	    else
	    {
...
int ObMultiBind::open()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "main operator(s) is/are NULL");
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "wrong get table_rpc_scan_op or table_mem_scan_op, can't dynamic cast");
    ...
    else if (rpc_query != NULL)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to load sub query result, err=%d", ret);
      }
    }
    else
    {
    ...
    if (ret == OB_SUCCESS )
    {
      ...
      {
      YYSYS_LOG(WARN, "failed to open main query, err=%d", ret);
      }
    }
  }
  return ret;  
    ...
    if(special_sub_query)
    {
        ...
        if(special_case)
        {
            ...
            else
            {
                ...
                //YYSYS_LOG(INFO, "aaaaaaaaaaaaaaaaa=%s", to_cstring(*table_filter_expr));
                ...
                else
                {
                    ...
                    {
                        YYSYS_LOG(ERROR, "failed to fill_sql_expression expr item, err=%d", ret);
                    }
