                             ...
                             uint64_t& query_id)
{
  ...
  else
  {
    ...
    for (int i = 0; i < node->num_child_; ++i)
    {
      ...
      if(OB_SUCCESS != (ret = ObVerifyCipherStrength(password)))
      {
        PARSER_LOG("Your password must contain letters, numbers and special characters, more than 8 digits");
        break;
      }
    } // end for
  }
...
int ObVerifyCipherStrength(const ObString &pass_word)
{
  int ret = OB_SUCCESS;
  char pattern[100];
  sprintf(pattern, "^(?![\\d]+$)(?![a-zA-Z]+$)(?![\\da-zA-Z]+$)(?![\\d\\W]+$)(?![a-zA-Z\\W]+$)[\\dA-Za-z\\W]{8,%ld}$", OB_MAX_PASSWORD_LENGTH);
  boost::regex e(pattern);
  if(boost::regex_match(pass_word.ptr(), e))
  {
    return ret;
  }
  ret = OB_ERR_WEAK_PASSWORD;
  return ret;
}
                              ...
                              uint64_t& query_id)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else if (OB_SUCCESS != (ret = ObVerifyCipherStrength(password)))
      {
        PARSER_LOG("Your password must contain letters, numbers and special characters, more than 8 digits");
      }
