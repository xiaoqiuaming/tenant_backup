...
//       ,cur_time_(NULL)
    ...
    inline void ObResultSet::set_physical_plan(ObPhysicalPlan *physical_plan, bool did_own)
    {
      ...
      {
        YYSYS_LOG(WARN, "physical_plan_ is not NULL, %p", physical_plan_);
      }
...
//      if (NULL == cur_time_)
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "cur_time_ must not be NULL");
      }
      //add liuzy [datetime func] 20150909:b
      ...
      {
        YYSYS_LOG(ERROR, "obj type is wrong, ret = %d", ret);
      }
      //add 20150909:e
      else if (ObPreciseDateTimeType != cur_time.get_type())
      {
        ret = OB_OBJ_TYPE_ERROR;
        YYSYS_LOG(ERROR, "cur_time type[%d] is not ObPreciseDateTimeType[%d]",
