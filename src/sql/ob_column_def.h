...
namespace oceanbase
{
  namespace sql
  {
    enum AlterAction
    {
	  ...
	  //add 20140108:e
      RENAME_ACTION,
      ALTER_VARCHAR_LENGTH,
      ALTER_ACTION_DEFAULT,
      ALTER_DECIMAL_PRECISION
    ...
    struct ObColumnDef
    {
      ObColumnDef()
      {
        ...
        primary_key_id_ = 0;
        default_value_.set_type(common::ObNullType);
      }
    ...
    inline void ObColumnDef::print(FILE* fp, int32_t level)
    {
      ...
      switch (action_)
      {
        ...
        case ALTER_VARCHAR_LENGTH:
          print_indentation(fp, level);
          fprintf(fp, "Alter Action = ALTER_VARCHAR_LENGTH\n");
          print_indentation(fp, level);
          fprintf(fp, "datatype:varchar(%ld)", type_length_);
          break;
        case ALTER_DECIMAL_PRECISION:
          print_indentation(fp, level);
          fprintf(fp, "Alter Action = ALTER_DECIMAL_PRECISION\n");
          print_indentation(fp, level);
          fprintf(fp, "datatype:decimal(%ld, %ld)", precision_, scale_);
          break;
      default:
          break;
      }
        ...
        case common::ObDecimalType:
        {
          ...
          */
          //modify wenghaixing DECIMAL OceanBase_BankCommV0.3 2014_7_19:b
            ...
            default_value_.get_decimal(val);
            print_indentation(fp, level);
            char tmp[common::MAX_PRINTABLE_SIZE];
            int64_t length = val.to_string(tmp, common::MAX_PRINTABLE_SIZE);
            fprintf(fp, "default value : %.*s\n", static_cast<int32_t>(length), tmp);
          //modify e
          break;
        }
        default:
