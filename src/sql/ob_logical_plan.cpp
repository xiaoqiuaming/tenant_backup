...
//      cur_time_fun_type_ = NO_CUR_TIME;
      ...
      query_optimizer_open_ = false;
      is_create_view_ = false;
      is_show_view_ = false;
      use_for_view_ = false; //[735]
    }
    ...
    int ObLogicalPlan::add_view_info(const char *view_name, uint64_t query_id)
    {
      ViewInfo info(view_name, query_id);
      return view_list_.push_back(info);
    }

    int ObLogicalPlan::remove_view_info(const char *view_name, uint64_t query_id)
    {
      int ret = OB_SUCCESS;
      ObList<ViewInfo>::iterator iter;
      for (iter = view_list_.begin(); iter != view_list_.end(); iter++)
      {
        if(strcmp(view_name, iter->first) == 0 && iter->second == query_id)
        {
          ret = view_list_.erase(iter);
          break;
        }
      }
      return ret;
    ...
    bool ObLogicalPlan::get_is_recursive_view(const char *view_name)
    {
      bool ret = false;
      ObList<ViewInfo>::iterator iter;
      for (iter = view_list_.begin(); iter != view_list_.end(); iter++)
      {
        if(strcmp(view_name, iter->first) == 0)
        {
          ret = true;
          break;
        }
    ...
    void ObLogicalPlan::reset_view_list()
    {
      view_list_.reset();
    }

    int32_t ObLogicalPlan::get_stmt_index_by_query_id(uint64_t query_id) const
    {
      int32_t size = stmts_.size();
      int32_t index = -1;
      ...
      switch(stmts_[0]->get_stmt_type())
      {
        case ObStmt::T_PREPARE:
        {
          ...
          if (stmt == NULL)
          {
            ...
            YYSYS_LOG(WARN, "wrong prepared query id, id = %lu, ret = %d", prepare_stmt->get_prepare_query_id(), ret);
            break;
          }
        ...
        case ObStmt::T_SELECT:
        {
          ...
          if (select_stmt == NULL)
          {
            ...
            YYSYS_LOG(WARN, "logical plan of select statement error");
            break;
          }
          ...
          for (int32_t i = 0; ret == OB_SUCCESS && i < size; i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to alloc string[%.*s] ret=%d", select_item.expr_name_.length(), select_item.expr_name_.ptr(), ret);
              break;
            }
            ...
            if (sql_expr == NULL)
            {
              YYSYS_LOG(WARN, "fail to get expr. select_item.expr_id_=%lu", select_item.expr_id_);
              ret = OB_ERR_ILLEGAL_ID;
              break;
            }
            ...
            else if (expr->get_expr_type() == T_REF_COLUMN)
            {
              ...
              if (table_id != OB_INVALID_ID)
              {
                ...
                if (column_item == NULL)
                {
                  YYSYS_LOG(WARN, "fail to get column item by id. table_id=%lu column_id=%lu", table_id, column_id);
                  ret = OB_ERR_ILLEGAL_ID;
                  break;
                }
                if (OB_SUCCESS != (ret = ob_write_string(alloc, column_item->column_name_, field.org_cname_)))
                {
                  YYSYS_LOG(WARN, "fail to alloc string[%.*s] ret=%d", column_item->column_name_.length(), column_item->column_name_.ptr(), ret);
                  break;
                }
                TableItem *table_item = select_stmt->get_table_item_by_id(table_id);
                if (table_item == NULL)
                {
                  YYSYS_LOG(WARN, "fail to get table item by id. table_id=%lu", table_id);
                  ret = OB_ERR_ILLEGAL_ID;
                  break;
                }
                if (table_item->alias_name_.length() > 0)
                {
                  ...
                  {
                    YYSYS_LOG(WARN, "fail to alloc string[%.*s] ret=%d", table_item->alias_name_.length(), table_item->alias_name_.ptr(), ret);
                    break;
                  }
                }
                else
                {
                  ...
                  {
                    YYSYS_LOG(WARN, "fail to alloc string[%.*s] ret=%d", table_item->table_name_.length(), table_item->table_name_.ptr(), ret);
                    break;
                  }
                }
            ...
            {
              YYSYS_LOG(WARN, "fail to add field column to result_set. ret=%d", ret);
              break;
            }
          ...
          if (select_stmt->is_partition_calc_func())
          {
            ObString  tname = ObString::make_string("select partition_cal");
            ObSelectStmt* select_stmt = static_cast<ObSelectStmt*>(stmts_[0]);
            if (select_stmt == NULL)
            {
              YYSYS_LOG(WARN, "fail to get select statement");
              ret = OB_ERR_PARSE_SQL;
              break;
            }
            else
            ...
            if (select_stmt->get_partition_cal_version_flag() == 2)
            {
              ObString cname[1];
              cname[0] = ObString::make_string("paxos_id");
              field.cname_ = cname[0];
              field.org_cname_ = cname[0];
            ...
            {
              ObString cname[7];
              cname[0] = ObString::make_string("table_id");
              cname[1] = ObString::make_string("table_name");
              cname[2] = ObString::make_string("cur_version");
              cname[3] = ObString::make_string("valid_version");
              cname[4] = ObString::make_string("partition_key");
              cname[5] = ObString::make_string("group_name");
              cname[6] = ObString::make_string("paxos_id");

              for (int32_t i = 0; ret == OB_SUCCESS && i < 7; i++)
              {
        ...
        //add:e
        case ObStmt::T_SHOW_ALL_INDEX:
        case ObStmt::T_SHOW_CREATE_INDEX:
        case ObStmt::T_SHOW_VARIABLES:
        case ObStmt::T_SHOW_COLUMNS:
        case ObStmt::T_SHOW_SCHEMA:
        case ObStmt::T_SHOW_CREATE_TABLE:
        case ObStmt::T_SHOW_CREATE_VIEW:
        case ObStmt::T_SHOW_TABLE_STATUS:
        case ObStmt::T_SHOW_SERVER_STATUS:
        case ObStmt::T_SHOW_PARAMETERS:
        //add liu jun. [MultiUPS] [sql_api] 20150527:b
        case ObStmt::T_SHOW_FUNCTIONS:
        //20150527:e
        case ObStmt::T_SHOW_TABLE_RULES://add wuna. [MultiUPS] [sql_api] 20160223
        case ObStmt::T_SHOW_GROUPS://add wuna. [MultiUPS] [sql_api] 20160223
        case ObStmt::T_SHOW_CURRENT_PAXOS_ID:
        {
          ObShowStmt *show_stmt = static_cast<ObShowStmt*>(stmts_[0]);
          if (show_stmt == NULL)
          {
            YYSYS_LOG(WARN, "fail to get Show statement");
            ret = OB_ERR_PARSE_SQL;
            break;
          }
          ...
          if (table_item == NULL)
          {
            YYSYS_LOG(WARN, "fail to get table item by id. table_id=%lu", show_stmt->get_sys_table_id());
            ret = OB_ERR_ILLEGAL_ID;
            break;
          }
          ObString tname;
          ...
          for (int32_t i = 0; ret == OB_SUCCESS && i < show_stmt->get_column_size(); i++)
          {
            ...
            if (col_item != NULL)
            {
              ...
              {
                YYSYS_LOG(WARN, "fail to alloc string \"%.*s\" ret=%d", col_item->column_name_.length(), col_item->column_name_.ptr(), ret);
                break;
              }
              field.cname_ = cname;
              field.org_cname_ = cname;
        ...
        case ObStmt::T_EXECUTE:
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "Session Info is needed. ret=%d", ret);
          }
          else if (execute_stmt == NULL)
          {
            ret = OB_ERR_PARSE_SQL;
            YYSYS_LOG(WARN, "fail to get Execute statement");
          }
          else if ((stored_plan = session_info->get_plan(execute_stmt->get_stmt_name())) == NULL)
          {
            ret = OB_ERR_PREPARE_STMT_UNKNOWN;
            YYSYS_LOG(USER_ERROR, "statement %.*s not prepared",
          ...
          {
            YYSYS_LOG(WARN, "fail to fill result set, ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "get physical plan, addr=%p", stored_plan->get_physical_plan());
            YYSYS_LOG(DEBUG, "StoredPlan: %s", to_cstring(*stored_plan->get_physical_plan()));
          }
          break;
        }
        default:
...
//      if (ret == OB_SUCCESS && NO_CUR_TIME != cur_time_fun_type_)
      ...
      //mod 20150909:e
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to assign cur_time room, ret=%d", ret);
        }
      }
