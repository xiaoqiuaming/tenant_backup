  ...
  : ObBasicStmt(ObBasicStmt::T_CREATE_TABLE)
{
  ...
  part_type_ = OB_WITHOUT_PARTITION;
  //add 20150424:e
  //add wuna [MultiUps][sql_api] 20151207:b
  partition_form_ = OB_UNKNOWN_FORM_PARTITION;
  part_func_query_id_ = OB_INVALID_ID;
  //add 20151207:e
}
...
void ObCreateTableStmt::print(FILE* fp, int32_t level, int32_t index)
{
  ...
  {
    print_indentation(fp, level + 1);
    fprintf(fp, "use_block_cache = TRUE\n");
  }
...
//add peiouya [MultiUps_live] 20141118:b
int ObCreateTableStmt::set_func_name(ResultPlan& result_plan, const ObString& func_name)/*Add By Liu Jun*/
{
  int & ret = result_plan.err_stat_.err_code_ = OB_SUCCESS;
  
  if(ret == OB_SUCCESS && ((ret = ob_write_string(*name_pool_, func_name, func_name_)) != OB_SUCCESS ))
  {
      snprintf(result_plan.err_stat_.err_msg_, MAX_ERROR_MSG,
        "allocate memory for table name failed");
  }
...
int ObCreateTableStmt::add_col_list(const common::ObString& column_name)
{
  int ret=OB_SUCCESS;
  ObString part_col;
  if(OB_SUCCESS != (ret = ob_write_string(*name_pool_, column_name, part_col)))
  {
    YYSYS_LOG(WARN,"allocate memory for partition column name failed.");
  }
  else if(OB_SUCCESS != (ret = col_list_.push_back(part_col)))
  {
    YYSYS_LOG(WARN, "can not add partition column '%.*s'", column_name.length(), column_name.ptr());
  }
  return ret;
}

int ObCreateTableStmt::add_col_list(ResultPlan& result_plan, const ObString& column_name)
{
  int& ret = result_plan.err_stat_.err_code_ = OB_SUCCESS;
/* mod wuna [MultiUps] [sql_api] 20151203:b*/
//  ObColumnDef *col = NULL;
//  for(int64_t i = 0;i < col_list_.count();i++)
//  {
//    const ObString part_col_name = col_list_.at(i);
//    if(part_col_name == column_name)
//    {
//      ret = OB_ERR_ALREADY_EXISTS;
//      snprintf(result_plan.err_stat_.err_msg_, MAX_ERROR_MSG,
...
//    }
//  }
  ObObjType data_type = ObMinType;//unused
  if (OB_SUCCESS != (ret = check_column_name_and_get_data_type(result_plan,column_name,data_type)))
  {
    YYSYS_LOG(ERROR," check_column_name failed,ret=%d",ret);
  }
  else if(!is_valid_data_type(data_type))
  {
  ...
  else if(OB_SUCCESS != add_col_list(column_name))
  {
    YYSYS_LOG(ERROR," add_col_list failed,ret=%d",ret);
  }
/*mod 20151203:e*/
  return ret;
}
...
int ObCreateTableStmt::set_group_name_prefix(ResultPlan& result_plan, const ObString& group_name_prefix)
{
  int & ret = result_plan.err_stat_.err_code_ = OB_SUCCESS;
  
  if(ret == OB_SUCCESS && ((ret = ob_write_string(*name_pool_, group_name_prefix, group_name_prefix_)) != OB_SUCCESS ))
  {
      snprintf(result_plan.err_stat_.err_msg_, MAX_ERROR_MSG,
        "allocate memory for table name failed");
  }
...
bool ObCreateTableStmt::is_column_name(const ObString& column_name)
{
   int ret = false;
   for (int64_t i = 0; i < columns_.count(); i++)
   {
...
bool ObCreateTableStmt::is_duplicate_column_name(const ObString& column_name)
{
  int ret = false;
  for(int64_t i = 0;i < col_list_.count();i++)
  {
...
int ObCreateTableStmt::get_column_type_by_name(ResultPlan& result_plan, const ObString& column_name, ObObjType& data_type)
{
    int& ret = result_plan.err_stat_.err_code_ = OB_SUCCESS;
    data_type = ObMinType;
    for (int64_t i = 0; i < columns_.count(); i++)
    {
      ...
      if (cur_col.column_name_ == column_name)
      {
        data_type = cur_col.data_type_;
        break;
      }
    }
    if(ObMinType == data_type)
    {
        ret = OB_ERROR;
        snprintf(result_plan.err_stat_.err_msg_, MAX_ERROR_MSG,
...
int ObCreateTableStmt::check_column_name_and_get_data_type(ResultPlan& result_plan, const ObString& column_name, ObObjType& data_type)
{
  int& ret = result_plan.err_stat_.err_code_ = OB_SUCCESS;
  data_type = ObMinType;
  ...
  else if(OB_SUCCESS != (ret = get_column_type_by_name(result_plan,column_name,data_type)))
  {
...
void ObCreateTableStmt::set_col_list(const ObArray<common::ObString>& col_list)
{
  col_list_ = col_list;
}

void ObCreateTableStmt::set_partition_form(common::ObPartitionForm form)
{
  partition_form_ = form;
}

bool ObCreateTableStmt::is_valid_data_type(common::ObObjType& data_type)
{
  //return (ObIntType == data_type || ObVarcharType == data_type || ObDateTimeType == data_type);
  // uncertainty add for partion
