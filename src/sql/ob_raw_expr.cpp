...
{
  return ((type_ >= T_INT && type_ <= T_NULL) || type_ == T_DATE_NEW || type_ == T_TIME );
}
...
bool ObRawExpr::is_join_cond_opt() const
{
    ...
    if(type_ >= T_OP_EQ && type_ <= T_OP_NE)
    {
        ...
        if(binary_expr == NULL)
        {
            YYSYS_LOG(WARN,"dynamic_cast is fail, type = %d", this->get_expr_type());
        }
...
int ObConstRawExpr::set_value_and_type(const common::ObObj& val)
{
  ...
  switch(val.get_type())
  {
    ...
    default:
      ...
      YYSYS_LOG(WARN, "obj type not support, type=%d", val.get_type());
      break;
  }
    ...
    ObPhysicalPlan *physical_plan) const
{
  ...
  YYSYS_LOG(DEBUG, "fill_sql_expression: item.type_ = [%d], item.data_type_ = [%d], idx = [%ld]", item.type_, item.data_type_, idx);
  //mod 20150909:e
  ret = inter_expr.add_expr_item(item);
    ...
    ObPhysicalPlan *physical_plan) const
{
  ...
  {
    YYSYS_LOG(ERROR, "transformer error");
    ret = OB_ERROR;
  }
  else
  {
	...
	*/
    ...
    {
      YYSYS_LOG(ERROR, "set sub query idx error");
    }
    else
    {
...
void ObBinaryRefRawExpr::print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level)
{
    print_indentation_json(buf, buf_len, pos, level+1);
    if(first_id_ == OB_INVALID_ID)
        databuff_printf(buf, buf_len, pos, "{\"RefTableId\":0, \"RefColumnId\":%lu}\n",second_id_);
    else
        databuff_printf(buf, buf_len, pos, "{\"RefTableId\":%lu, \"RefColumnId\":%lu}\n",first_id_,second_id_);
}
...
void ObBinaryOpRawExpr::print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level)
{
    first_expr_->print_json(buf, buf_len, pos, level);
    print_indentation_json(buf, buf_len, pos, level+1);
    databuff_printf(buf, buf_len, pos, ",\n");
    second_expr_->print_json(buf, buf_len, pos, level);
}
	...
	//mod 20140404:e
    {
         ...
         && second_expr_->get_expr_type() == T_REF_QUERY)
      {
        ...
        switch (first_expr_->get_expr_type ())
        {
          ...
          case T_REF_QUERY:
          {
             YYSYS_LOG(USER_ERROR,"Not support sql like : subquery in (...)");
             ret =OB_NOT_SUPPORTED;
             break;
           }
           default:
         ...
         {
           YYSYS_LOG(WARN,"Fail to acquire subquery plan");
         ...
         else if (right_output_columns_dsttype.count () == left_output_columns_dsttype.count ())
         {
           ...
           for (int64_t idx = 0; idx < right_output_columns_dsttype.count (); idx ++)
           {
             ...
             {
              YYSYS_LOG(USER_ERROR,"%s can not compare with %s ",
         ...
         {
           YYSYS_LOG(USER_ERROR,"In operands contain different column(s)");
         ...
         {
           YYSYS_LOG(USER_ERROR,"Not support the sql");
           ret = OB_NOT_SUPPORTED;
         }
       }
     //add 20151225:e
    ...
    ObPhysicalPlan *physical_plan) const
{
  ...
  else
  {
      ...
      for (int32_t i = 0; ret == OB_SUCCESS && i < when_exprs_.size() && i < then_exprs_.size(); i++)
      {
          ...
          {
              YYSYS_LOG(ERROR,"add arg_expr_ failure,ret=%d",ret);
              break;
          }
          else if(OB_SUCCESS !=(ret =when_exprs_[i]->fill_sql_expression(inter_expr, transformer, logical_plan, physical_plan)))
          {
          ...
          {
              YYSYS_LOG(ERROR,"add equal failure,ret=%d",ret);
              break;
          }
          else if(OB_SUCCESS !=(ret = inter_expr.add_expr_item (item_case_then)))
          {
    ...
    ObPhysicalPlan *physical_plan) const
{
  ...
  if(strcmp(function_name,"to_date")==0)
  {
      ...
      ObExprObj result;
      result.set_null ();
      if(exprs_.size() == 2&&!exprs_[0]->is_column() && !exprs_[1]->is_column())
      {
          ...
          {
              YYSYS_LOG(WARN,"[to date optimization] fill first expr faild!");
          }
          else if((ob_sys_fun_expr != NULL && se_temp->is_optimization())||
                  (ob_sys_fun_expr == NULL))
          {
              ObPostfixExpression &ops = se_temp->get_decoded_expression_v2();
              ObArray<uint64_t> column_array;
              bool has_no_column = false;
              if(OB_SUCCESS == ops.get_all_cloumn(column_array))
              {
                  if(column_array.count() == 0)
                  {
                      has_no_column = true;
                  }
              }
              ...
              {
                  YYSYS_LOG(WARN,"[to date optimization] fill second expr faild!");
              }
              else
              {
                  if(se_temp!=NULL)
                  {
                      ...
                      {
                          YYSYS_LOG(WARN, "[to date optimization] faild to add expr to sql expression");
                      }
                      ...
                      {
                          ret = OB_SUCCESS;
                          YYSYS_LOG(WARN, "[to date optimization] faild to calc the sql expression");
                      }
                      else
                      {
                          ...
                          {
                              YYSYS_LOG(WARN,"[to date optimization]can not optimization for to date!");
                          }
                      }
                  }
                  else
                  {
                      ...
                      YYSYS_LOG(WARN,"[to date optimization] no pointer of ObSqlExpression!");
                  }
              }
          }
          }
  ...
  else if(strcmp(function_name,"to_timestamp")==0)
  {
      ...
      if(exprs_.size() == 2&&!exprs_[0]->is_column() &&!exprs_[1]->is_column())
      {
          ...
          if(OB_SUCCESS != (ret = exprs_[0]->fill_sql_expression(*se_temp, transformer, logical_plan, physical_plan)))
          {
              YYSYS_LOG(WARN,"[to timestamp optimization] fill first expr faild!");
          }
          else if((ob_sys_fun_expr != NULL && se_temp->is_optimization())||
                  (ob_sys_fun_expr == NULL))
          {
              ...
              if(OB_SUCCESS != (ret = exprs_[1]->fill_sql_expression(*se_temp, transformer, logical_plan, physical_plan)))
              {
                  YYSYS_LOG(WARN,"[to timestamp optimization] fill second expr faild!");
              }
              else
              {
                  if(se_temp!=NULL)
                  {
                      if (OB_SUCCESS != (ret =  se_temp->add_expr_item(item)))
                      {
                          YYSYS_LOG(WARN, "[to timestamp optimization] faild to add expr to sql expression");
                      }
                      else if(OB_SUCCESS != (ret = se_temp->add_expr_item_end ()))
                      {
                          YYSYS_LOG(WARN, "[to timestamp optimization] faild to add expr to sql expression");
                      ...
                      {
                          ret = OB_SUCCESS;
                          YYSYS_LOG(WARN, "[to timestamp optimization] faild to calc the sql expression");
                      }
                      else
                      {
                          se_temp->get_result (result);
                          ...
                          {
                              YYSYS_LOG(WARN,"[to timestamp optimization]can not optimization for to date!");
                          }
                      }
                  }
                  else
                  {
                      ret = OB_ERR_POINTER_IS_NULL;
                      YYSYS_LOG(WARN,"[to timestamp optimization] no pointer of ObSqlExpression!");
                  }
              }
          }
          }
  ...
  else if(strcmp(function_name,"date_add_day")==0)
  {
      ...
      ObExprObj result;
      result.set_null();
      bool having_column_ref = false;
      for(int i =0;i<exprs_.size();i++)
      {
          if(exprs_[i]->is_column())
          {
            having_column_ref = true;
                  break;
          }

      }
      if(!having_column_ref)
  ...
  if(!is_can_optimization)
  {
      ...
      for (int32_t i = 0; ret == OB_SUCCESS && i < exprs_.size(); i++)
      {
          ...
          if (ret != OB_SUCCESS)
          {
              YYSYS_LOG(WARN, "Add parameters of system function failed, param %d", i + 1);
              break;
          }
      }
      if (ret == OB_SUCCESS && (ret = inter_expr.add_expr_item(item)) != OB_SUCCESS)
      {
          YYSYS_LOG(WARN, "Add system function %.*s failed", func_name_.length(), func_name_.ptr());
      }
      //add dolphin[coalesce return type]@20151126:b
      inter_expr.set_expr_type(get_result_type());
      //YYSYS_LOG(ERROR,"dolphin::test ObSysFunRawExpr return type is:%d",get_result_type());
      // 原流程：e
  }
...
//mod wuna [MultiUps][sql_api] 20160113:b
...
//    && !(transformer != NULL && logical_plan != NULL && physical_plan != NULL))
  ...
  //mod 20160113:e
  {
    YYSYS_LOG(WARN,"(ObTransformer, ObLogicalPlan, ObPhysicalPlan) should be set together");
  }
...
void ObSqlRawExpr::print_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level) const
{
    print_indentation_json(buf, buf_len, pos, level);
    databuff_printf(buf, buf_len, pos, "{\n");
    print_indentation_json(buf, buf_len, pos, level +1);
    databuff_printf(buf, buf_len, pos, "\"ExprId\":%ld,", expr_id_);
    if(table_id_ == OB_INVALID_ID)
        databuff_printf(buf, buf_len, pos, "\"TableId\":\"NULL\",\"ColumnId\":%lu,",column_id_);
    else
        databuff_printf(buf, buf_len, pos, "\"TableId\":\"%lu\",\"ColumnId\":%lu,",table_id_,column_id_);
    databuff_printf(buf, buf_len, pos, "\n");

    print_indentation_json(buf, buf_len, pos, level +1);
    databuff_printf(buf, buf_len, pos, "\"RawExprs\":[\n");
    expr_->print_json(buf, buf_len, pos, level+1);
    print_indentation_json(buf, buf_len, pos, level +1);
    databuff_printf(buf, buf_len, pos, "]\n");
...
void ObSqlRawExpr::print_indentation_json(char *buf, const int64_t buf_len, int64_t &pos, int32_t level) const
{
...
int ObSqlRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    expr_->alter_sql_expression(old_id, new_id, subquery_stmt);
    return ret;
}
...
int ObBinaryOpRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    first_expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    second_expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    return ret;
}
...
int ObBinaryRefRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    if(new_id == 0)
    {
        if(first_id_ == old_id)
        {
            uint64_t column_idx = second_id_-16;
            const ColumnItem *column_item = subquery_stmt->get_column_item(uint32_t(column_idx));
            set_first_ref_id(column_item->table_id_);
            set_second_ref_id(column_item->column_id_);
        }
    }
    else
    {
...
int ObConstRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
    UNUSED(subquery_stmt);
    return ret;
}
...
int ObCurTimeExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
...
int ObUnaryRefRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
...
int ObUnaryOpRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    return ret;
...
int ObTripleOpRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    first_expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    second_expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    third_expr_->alter_sql_expression(old_id,new_id,subquery_stmt);
    return ret;
}
...
int ObMultiOpRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
...
int ObCaseOpRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
...
int ObAggFunRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
...
int ObSysFunRawExpr::alter_sql_expression(uint64_t old_id, uint64_t new_id, ObSelectStmt *subquery_stmt)
{
    int ret = OB_SUCCESS;
    UNUSED(old_id);
    UNUSED(new_id);
