...
namespace oceanbase
{
  namespace sql
  {
    ...
    public:
      ...
      void set_index_status(int64_t index_status);
      ...
      const int64_t &get_index_status() const;
      const common::ObString& get_like_pattern() const;
      int set_like_pattern(const common::ObString like_pattern);

      int add_column_def(ResultPlan &result_plan, const ObColumnDef &column_def);
      void set_is_show_view(bool is_show_view);
    ...
    private:
      ...
      int64_t index_status_;
      common::ObArray<ObColumnDef> column_list_;
      bool is_show_view_;
    ...
    //add:e
    inline void ObShowStmt::set_index_status(int64_t index_status)
    {
    ...
    //add:e
    inline const int64_t &ObShowStmt::get_index_status() const
    {
        return index_status_;
    }

    inline void ObShowStmt::set_is_show_view(bool is_show_view)
    {
      is_show_view_ = is_show_view;
    }

    inline bool ObShowStmt::get_is_show_view() const
    {
      return is_show_view_;
    }
    inline int64_t ObShowStmt::get_column_count()const
    {
      return column_list_.count();
    }
    inline const ObColumnDef &ObShowStmt::get_column_def(int64_t index) const
    {
      OB_ASSERT(0 <= index && index < column_list_.count());
      return column_list_.at(index);
    }
  }
}
