...
namespace oceanbase
{
  namespace compactsstablev2
  {
    int ObSSTableTable::switch_table_reset()
    {
      ...
      {
        YYSYS_LOG(ERROR, "can not go herer");
      }
    ...
    int ObSSTableTable::switch_sstable_reset()
    {
      ...
      {
        YYSYS_LOG(ERROR, "can not go here");
      }
              ...
              cur_range.table_range_.end_key_, cur_end_key_allocator)))
      {
        ...
        YYSYS_LOG(WARN, "old range table range end key deep copy error:"
              ...
              cur_start_key_allocator)))
      {
        ...
        YYSYS_LOG(WARN, "block endkey deep copy error:ret=%d,"
        ...
        const int64_t bloomfilter_nbyte)
    {
      ...
      {
        YYSYS_LOG(WARN, "init error:ret=%d", ret);
      }
        ...
        const bool not_table_first_row)
    {
      ...
      {
        YYSYS_LOG(WARN, "row get rowkey error:ret=%d,row=%s,rowkey=%p",
      ...
      else
      {
        if (not_table_first_row)
        {
          ...
          {
            YYSYS_LOG(WARN, "cur rowkey > the end_key_ of range:"
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "cur rowkey > the end_key_ of range or "
    ...
    int ObSSTableTable::set_table_range(const common::ObNewRange& range)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed copy range");
      }
      else
      {
    ...
    int ObSSTableTable::set_block_index(const int64_t data_offset)
    {
      ...
      if (data_offset < 0)
      {
        YYSYS_LOG(WARN, "invalid argument:data_offset=%ld", data_offset);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "add entry error:ret=%d", ret);
        }
      }
    ...
    int ObSSTableTable::finish_table_range(const bool is_table_finish)
    {
      ...
      if (OB_SUCCESS != (ret = table_range_builder_.add_item_border_flag(
              border_flag)))
      {
        YYSYS_LOG(WARN, "table range builder add item border flag:"
      ...
      if (OB_SUCCESS == ret)
      {
                ...
                range.table_range_.start_key_)))
        {
          ...
          YYSYS_LOG(WARN, "table range builder add_item error:ret=%d,"
        ...
        int64_t& startkey_length, int64_t& endkey_length)
    {
      ...
      if (OB_SUCCESS != (ret = table_range_builder_.build_table_range(
              buf, buf_size, length)))
      {
        YYSYS_LOG(WARN, "table range builder build table range error:ret=%d,"
        ...
        int64_t& startkey_length, int64_t& endkey_length)
    {
      ...
      if (OB_SUCCESS != (ret = table_range_builder_.build_table_range(
              buf, length)))
      {
        YYSYS_LOG(WARN, "table range builder build table range error:"
