...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        const char* sstable_fname, const bool use_dio)
    {
      ...
      {
        YYSYS_LOG(WARN, "argument error:NULL==sstable_fname");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!use_dio && -1 == (fd_ = ::open(sstable_fname,
              FILE_OPEN_NORMAL_RFLAG)))
      {
        YYSYS_LOG(ERROR, "open file error:sstable_fname=%s", sstable_fname);
        ret = OB_IO_ERROR;
      }
      else if (use_dio && -1 == (fd_ = ::open(sstable_fname,
      ...
      {
        YYSYS_LOG(INFO, "open file:fd_=%d", fd_);
      }
    ...
    int ObCompactSSTableReader::init(const uint64_t sstable_id)
    {
      ...
      if (is_inited_)
      {
        YYSYS_LOG(WARN, "init twice");
        ret = OB_INIT_TWICE;
      }
      else if (OB_INVALID_ID == sstable_id)
      {
        YYSYS_LOG(WARN, "OB_INVALID_ID == sstable_id");
        ret = OB_INVALID_ARGUMENT;
      }
    ...
    ObCompressor* ObCompactSSTableReader::get_decompressor()
    {
      ...
      if (NULL == compressor_ && strlen(compressor_name) > 0)
      {
        ...
        if (NULL == compressor_)
        {
          YYSYS_LOG(WARN, "create compressor error:NULL==compressor_");
        }
      }
      return compressor_;
    }
        ...
        int64_t& sstable_size)
    {
      ...
      if (sstable_size < 0)
      {
        YYSYS_LOG(WARN, "get file size error:ret=%ld", sstable_size);
        ret = OB_IO_ERROR;
      }
        ...
        ObSSTableTrailerOffset& trailer_offset)
    {
      ...
      {
        YYSYS_LOG(WARN, "read recrod error:ret=%d", ret);
      }
        ...
        const int64_t sstable_size)
    {
      ...
      if (offset < 0 || size <= 0)
      {
        YYSYS_LOG(WARN, "argument error:offset=%ld,size=%ld", offset, size);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = read_record(file_info,
              offset, size, record_buf)))
      {
        YYSYS_LOG(WARN, "read record error:offset=%ld, size=%ld", offset, size);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(ERROR, "check record error:ret=%d", ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "don not allow compress");
          ret = OB_ERROR;
        }
      }
        ...
        ObSSTableSchema*& schema)
    {
      ...
      {
        YYSYS_LOG(WARN, "ARGUMENT ERROR:record_offset=%ld, record_size=%ld, version=%ld",
      ...
      else
      {
        ...
        {
          //YYSYS_LOG(WARN, "schema cache get schema error");
          //ret = OB_ERROR;
        }
        else
        {
          ...
          if (NULL == result_buf)
          {
            YYSYS_LOG(WARN, "NULL == result_buf");
          ...
          else
          {
            ...
            {
              ob_free(result_buf);
              YYSYS_LOG(WARN, "new ObSSTableSchema error");
              ret = OB_MEM_OVERFLOW;
            }
          }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "read record error:ret=%d", ret);
            }
            else if (NULL == record_buf)
            {
              YYSYS_LOG(WARN, "NULL == record_buf");
              ret = OB_ERROR;
            }
          }
          ...
          if (OB_SUCCESS == ret)
          {
            if (OB_SUCCESS != (ret = ObRecordHeaderV2::check_record(
                    ...
                    record_header, payload_ptr, payload_size)))
            {
              YYSYS_LOG(WARN, "check record error");
            }
            else
            {
              if (!record_header.is_compress())
              {
                ...
                if (OB_SUCCESS != (ret = trans_to_sstable_schema(*result_schema,
                        tmp_buf, tmp_count)))
                {
                  YYSYS_LOG(WARN, "trans to sstable schema error");
                }
              }
              else
              {
                YYSYS_LOG(WARN, "do not support compress");
                ret = OB_ERROR;
              }
            }
          }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (OB_ENTRY_EXIST == ret)
            {
              ...
              if (NULL == sstable_schema_)
              {
                YYSYS_LOG(WARN, "get schema error");
                ret = OB_ERROR;
              }
              else
              {
            ...
            {
              YYSYS_LOG(WARN, "schema cache add schema error:ret=%d", ret);
            }
          }
        }
      }
        ...
        ObSSTableTableIndex*& table_index)
    {
      ...
      {
        YYSYS_LOG(WARN, "argument error:record_offset=%ld, record_size=%ld",
      ...
      else if (NULL != record_buf)
      {
        if (OB_SUCCESS != (ret = ObRecordHeaderV2::check_record(
                ...
                record_header, payload_ptr, payload_size)))
        {
          YYSYS_LOG(WARN, "check record error:ret=%d,record_buf=%p,"
        ...
        else
        {
          if (!record_header.is_compress())
          {
            ...
            if (NULL == tmp_buf)
            {
              YYSYS_LOG(WARN, "failed to alloc memory for arena");
              ret = OB_ALLOCATE_MEMORY_FAILED;
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "faile to new ObSSTableTableIndex");
                ret = OB_MEM_OVERFLOW;
              }
              else
              {
      ...
      {
        YYSYS_LOG(WARN, "can not go to here");
        ret = OB_ERROR;
      }
        ...
        ObBloomFilterV1*& bloom_filter)
    {
      ...
      if (NULL == table_index)
      {
        YYSYS_LOG(WARN, "table_index==NULL");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(WARN, "failed to alloce memory for arena");
        }
        else
        {
          for (int64_t i = 0; i < table_count; i ++)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to new ObBloomFilterV1");
              ret = OB_ERROR;
              break;
            }
            else
            ...
            if (OB_SUCCESS == ret)
            {
              if (!record_header.is_compress())
              {
                if (OB_SUCCESS != (ret = bloom_filter_ptr->init(
                    ...
                    payload_size)))
                {
                  YYSYS_LOG(WARN, "bloomfilter init error:ret=%d", ret);
                  break;
                }
                ...
                if (OB_SUCCESS == ret)
                {
                  ...
                  if (OB_SUCCESS != (ret = bloom_filter_ptr->set_buffer(tmp_ptr,
                          payload_size)))
                  {
                    YYSYS_LOG(WARN, "bloomfilter reinit error:ret=%d", ret);
                    break;
                  }
                }
              }
              ...
              {
                YYSYS_LOG(WARN, "check record error:ret=%d", ret);
                break;
              }
            }
          }
        ...
        ObNewRange*& table_range)
    {
      ...
      if (NULL == table_index)
      {
        YYSYS_LOG(WARN, "invalid argument:NULL==table_index");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(WARN, "faile to alloc memory");
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
          for (int64_t i = 0; i < table_count; i ++)
          {
            ...
            if (NULL == range_ptr)
            {
              YYSYS_LOG(WARN, "new ObNewRange error");
              ret = OB_MEM_OVERFLOW;
              break;
            }
            else
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < table_count; i ++)
        {
          ...
          if (OB_SUCCESS != (ret = read_record(file_info,
                  record_offset, record_size, record_buf)))
          {
            YYSYS_LOG(WARN, "read record error");
            break;
          }
          else
          {
                ...
                else
                {
                  ...
                  if (OB_SUCCESS != (ret = trans_to_table_range(
                          *range_ptr, store_type, tmp_buf2,
                          ...
                          table_index_ptr->range_end_key_length_)))
                  {
                    YYSYS_LOG(WARN, "trans_to_table_range error");
                    break;
                  }
                  else
                  {
        ...
        const int64_t offset, const int64_t size, const char*& out_buf)
    {
      ...
      if (NULL == file_buf)
      {
        YYSYS_LOG(WARN, "get tsi mult error");
        ret = OB_ERROR;
      }
        ...
        int64_t item_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument:item_array=%p, item_count=%ld", item_array, item_count);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
          ...
          const int64_t start_key_length, const int64_t end_key_length)
    {
      ...
      if (NULL == buf)
      {
        YYSYS_LOG(WARN, "invald argument:buf==NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        else
        {
          ...
          while(true)
          {
            ...
            {
              YYSYS_LOG(WARN, "row next cell error");
              break;
            }
            else
            {
              if (DENSE_SPARSE == store_type || DENSE_DENSE == store_type)
              {
                ...
                {
                  YYSYS_LOG(WARN, "row get cell error");
                  break;
                }
                else
                {
              ...
              {
                YYSYS_LOG(WARN, "can not go here");
                break;
              }
            }
          }
