  ...
  namespace compactsstablev2
  {
            ...
            const bool reverse_scan)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "too many block to read, buf_to_read_=%ld, "
        ...
        const int64_t offset, const int64_t size, char*& buffer)
    {
      ...
      else //READY == state_
      {
        ...
        {
          YYSYS_LOG(WARN, "get block not in order, sstable_id=%lu, "
                                          ...
                                          const bool reverse_scan)
    {
      ...
      else
      {
        ...
        if (buf_to_read_ > buf_size_)
        {
          YYSYS_LOG(WARN, "too many blocks to read, buf_to_read_=%ld, "
              "buf_size=%ld", buf_to_read_, buf_size_);
          buf_to_read_ = 0;
          ret = OB_ERROR;
        }
        ...
        const bool reverse_scan)
    {
      ...
      {
        YYSYS_LOG(WARN, "two aio buffer aren't in FREE state, "
      ...
      if (OB_SUCCESS == ret && inited_)
      {
            ...
            && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid block info, block_offset=%ld, "
          ...
          else if (!can_fit_aio_buffer(block_infos.position_info_[i].size_))
          {
            ...
            YYSYS_LOG(WARN, "block size is too big, block_size=%ld, "
          ...
          {
            reset();
            YYSYS_LOG(WARN, "blocks are not continuous, pre_offset=%ld, "
                                               ...
                                               const bool check_cache)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid aio buffer index, aio_buf_idx=%ld", 
            aio_buf_idx);
        ret = OB_ERROR;
      }
      else if (check_cache)
