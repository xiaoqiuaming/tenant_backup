...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        const int64_t min_split_sstable_size/*=0*/)
    {
      ...
      if (sstable_inited_)
      {
        YYSYS_LOG(WARN, "sstable reinited");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (DENSE_SPARSE != row_store_type
          && DENSE_DENSE != row_store_type)
      {
        YYSYS_LOG(WARN, "invalid row_stroe_type:row_store_type=%d",
            row_store_type);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!version_range.is_legal(row_store_type))
      {
        YYSYS_LOG(WARN, "version_range is invalid:row_store_type=%d,"
            "version_range=%s",
      ...
      {
        YYSYS_LOG(WARN, "table count is error:table_count=%ld",
            table_count);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (blocksize <= 0)
      {
        YYSYS_LOG(WARN, "block size is error:block_size=%ld", blocksize);
        ret = OB_INVALID_ARGUMENT;
      }
          ...
          || (def_sstable_size != 0 && min_split_sstable_size == 0))
      {
        YYSYS_LOG(WARN, "split size set error:def_sstable_size=%ld,"
      ...
      else
      {
        ...
        if (DENSE_SPARSE == row_store_type)
        {
          ...
          {
            YYSYS_LOG(WARN, "do not support the split of multible table:"
        ...
        else if (DENSE_DENSE == row_store_type)
        {
          if (1 != table_count)
          {
            YYSYS_LOG(ERROR, "DENSE_DENSE do not support the mult table");
            ret = OB_ERROR;
          }
        ...
        {
          YYSYS_LOG(ERROR, "invalid store type:row_store_type=%d",
              row_store_type);
          ret = OB_ERROR;
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          else if (NULL == compressor_)
          {
            ...
            if (NULL == (compressor_ = create_compressor(
                    compressor_name.ptr())))
            {
              YYSYS_LOG(ERROR, "create commpressor error:"
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "ObCompactSSTableWriter set sstable param"
            "success");
        ...
        const ObSSTableSchema& schema, const ObNewRange& table_range)
    {
      ...
      if (!sstable_inited_)
      {
        YYSYS_LOG(WARN, "sstable inited donot inited:sstable_inited_=%d",
            sstable_inited_);
        ret = OB_NOT_INIT;
      }
      ...
      {//switch table
        if (split_flag_)
        {
          YYSYS_LOG(WARN, "don not support the split of mult"
          ...
          ret = OB_ERROR;
                ...
                is_sstable_split)))
        {
          YYSYS_LOG(WARN, "finish the current block error:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = table_.add_table_bloomfilter(
                sstable_writer_buffer_.get_list_bloomfilter())))
        {
          YYSYS_LOG(WARN, "add table bloomfilter error:ret=%d", ret);
        }
        else
        {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "finish current table error:ret=%d", ret);
          }
          else
          {//switch table
        ...
        const ObString& filepath)
    {
      ...
      if (!sstable_inited_)
      {
        YYSYS_LOG(WARN, "sstable has not been inited");
        ret = OB_NOT_INIT;
      }
      else if(!table_inited_)
      {
        YYSYS_LOG(WARN, "table has not been inited");
        ret = OB_NOT_INIT;
      }
      else if (sstable_file_inited_)
      {
        YYSYS_LOG(WARN, "sstable file inited twice");
        ret = OB_INIT_TWICE;
      }
      else if (0 == filepath.length())
      {
        YYSYS_LOG(ERROR, "filepath is null");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = filesys_->create(filepath, dio_flag_)))
      {
        YYSYS_LOG(ERROR, "create file appender error:ret=%d,filepath=%s,"
            "dio_flag_=%d", ret, to_cstring(filepath), dio_flag_);
      }
      else if (OB_SUCCESS != (ret = flush_mem_block_list()))
      {
        YYSYS_LOG(WARN, "flush mem block list error:ret=%d", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "set sstable file path success");
        ...
        const ObRow& row_value, bool& is_sstable_split)
    {
      ...
      {
        YYSYS_LOG(WARN, "sstable file has not been inited");
        ret = OB_NOT_INIT;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "check row error:row_key=%s,row_value=%s",
      ...
      {//check the rowkey in the table range
        ...
        {//todo:the output of error meesage
          YYSYS_LOG(WARN, "check rowkey range error:row_key=%s,"
      ...
      if (OB_SUCCESS == ret)
      {
        if (not_table_first_row_)
        {
          if (OB_SUCCESS != (ret = sstable_writer_buffer_.check_rowkey(
                  row_key)))
          {
            YYSYS_LOG(WARN, "sstable writer buffer check row error:"
      ...
      if (OB_SUCCESS == ret)
      {
        if (need_switch_block())
        {
          ...
          {
            YYSYS_LOG(WARN, "finish current block error:ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {//add the row
          YYSYS_LOG(WARN, "add row error:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = sstable_writer_buffer_.set_cur_key(row_key)))
        {//cache the current key
          YYSYS_LOG(WARN, "soft copy the cur_key error:ret=%d", ret);
        }
        else
        {
          if (!not_table_first_row_)
          ...
          {
            YYSYS_LOG(WARN, "update list bloom filter error:ret=%d", ret);
          }
        }
      }
        ...
        bool& is_sstable_split)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "sstable check row error:ret=%d,row=%s",
              ret, to_cstring(row));
        }
      }
#endif
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "check rowkey range error:row=%s,"
    ...
    int ObCompactSSTableWriter::finish()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (OB_SUCCESS != (ret = finish_current_table(true)))
        {
          YYSYS_LOG(WARN, "finish current talbe errot:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = finish_current_sstable()))
        {
          YYSYS_LOG(WARN, "finish_current_sstable error:ret=%d", ret);
        }
      }
        ...
        const ObNewRange*& table_range, const int64_t flag)
    {
      ...
      {
        YYSYS_LOG(WARN, "get table range error:flag=%ld,"
    ...
    int ObCompactSSTableWriter::flush_mem_block_list()
    {
      ...
      while (cur_iter != end_iter)
      {
        ...
        {
          YYSYS_LOG(WARN, "write record header error:ret=%d", ret);
          break;
        }
                ...
                (*cur_iter)->data_len_)))
        {
          YYSYS_LOG(WARN, "write record body error:ret=%d", ret);
          break;
        }
        else
        {
    ...
    int ObCompactSSTableWriter::finish_current_block(bool& is_sstable_split)
    {
      ...
      if (0 != block_.get_row_count())
      {
        ...
        {
          YYSYS_LOG(WARN, "build block error:ret=%d", ret);
        }
        else
        {
        ...
        if (OB_SUCCESS == ret)
        {
          if (OB_SUCCESS != (ret = sstable_writer_buffer_.compress(
                  compressor_)))
          {//compress
            YYSYS_LOG(WARN, "compress error:ret=%d", ret);
          }
          else
          {//select data
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {//allow split
            if (OB_SUCCESS != (ret = finish_current_block_split(
                    is_sstable_split)))
            {
              YYSYS_LOG(WARN, "finish current block split error:ret=%d", ret);
            }
          }
          else
          {//don not allow split
            ...
            {
              YYSYS_LOG(WARN, "finish current block nosplit error:"
                  "ret=%d", ret);
            }
          }
        }
        ...
        bool& is_sstable_split)
    {
      ...
      if (split_buf_inited_)
      {
        ...
        {
          YYSYS_LOG(WARN, "cur node prepare error:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = sstable_writer_buffer_.build_cur_node()))
        {
          YYSYS_LOG(WARN, "build current node error:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = sstable_writer_buffer_.push_cur_node()))
        {
          YYSYS_LOG(WARN, "push cur node error:ret=%d", ret);
        }
        ...
        {//reach the condition of split
          ...
          {
            YYSYS_LOG(WARN, "finish current sstable error:ret=%d", ret);
          }
          else
          {
    ...
    int ObCompactSSTableWriter::finish_current_table(const bool finish_flag)
    {
      ...
      {
        YYSYS_LOG(WARN, "finish current block index error:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_block_endkey()))
      {
        YYSYS_LOG(WARN, "finish current block endkey error:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_table_range(finish_flag)))
      {
        YYSYS_LOG(WARN, "finish current table range error:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_table_bloomfilter()))
      {
        YYSYS_LOG(WARN, "finish current table bloomfilter error:ret=%d", ret);
      }
      else
      {
    ...
    {//ok
      ...
      else
      {
        if (OB_SUCCESS != (ret = table_.finish_last_block(
                cur_offset_ - cur_table_offset_)))
        {//最后一个block index
          YYSYS_LOG(WARN, "finish last block error:ret=%d", ret);
        }
        else
        {//block index的总大小
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {//需要分配空间
            ...
            if (OB_SUCCESS != (ret = sstable_writer_buffer_.ensure_uncomp_buf(
                    index_len)))
            {//分配空间
              YYSYS_LOG(WARN, "ensure space error:ret=%d", ret);
            }
            else
            {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              {//build block index
                YYSYS_LOG(WARN, "build block index error:ret=%d", ret);
              }
              else
              {//设置block index len
                sstable_writer_buffer_.set_uncomp_buf(buf_ptr, data_len);
    ...
    int ObCompactSSTableWriter::finish_current_block_endkey()
    {
      ...
      if (0 != endkey_len)
      {
        if (table_.is_alloc_block_endkey_buf())
        {
          ...
          {
            YYSYS_LOG(WARN, "sstable writer buffer ensure uncomp buf error:"
                "ret=%d", ret);
          }
          else
          {
          ...
          if (OB_SUCCESS == ret)
          {
            if (OB_SUCCESS != (ret = table_.build_block_endkey(
                    buf_ptr, buf_size, data_len)))
            {
              YYSYS_LOG(WARN, "build block endkey error:ret=%d", ret);
            }
        ...
        else
        {
          ...
          if (OB_SUCCESS != (ret = table_.build_block_endkey(
                  buf_ptr, data_len)))
          {
            YYSYS_LOG(WARN, "block block endkey error:ret=%d", ret);
          }
          else
          {
            sstable_writer_buffer_.set_uncomp_buf(buf_ptr, data_len);
        ...
        const bool finish_flag)
    {
      ...
      {
        YYSYS_LOG(WARN, "table finish table range error:ret=%d", ret);
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        if (table_.is_alloc_table_range_buf())
        {
          ...
          {
            YYSYS_LOG(WARN, "ensure uncomp buf error:ret=%d", ret);
          }
          else
          {
            buf_ptr = sstable_writer_buffer_.get_uncomp_buf_ptr();
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "build table range error:ret=%d", ret);
            }
            else
            {
    ...
    int ObCompactSSTableWriter::finish_current_sstable()
    {
      ...
      {
        YYSYS_LOG(WARN, "finish current table index error:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_table_schema()))
      {
        YYSYS_LOG(WARN, "finish current table schema:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_sstable_header()))
      {
        YYSYS_LOG(WARN, "finish current sstable header:ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_current_sstable_trailer_offset()))
      {
        YYSYS_LOG(WARN, "finish current sstable trailer offset:ret=%d", ret);
      }
      else
      {
        switch_sstable_reset();
        YYSYS_LOG(DEBUG, "finish_current_sstable:ret=%d", ret);
      }
      return ret;
    }
    ...
    int ObCompactSSTableWriter::finish_current_table_index()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (sstable_.is_alloc_table_index_buf())
        {
          ...
          {
            YYSYS_LOG(WARN, "sstable writer buffer ensure uncomp buf"
                "error:ret=%d", ret);
          }
          else
          {
          ...
          if (OB_SUCCESS == ret)
          {
            if (OB_SUCCESS != (ret = sstable_.build_table_index(
                    buf_ptr, buf_size, data_len)))
            {
              YYSYS_LOG(WARN, "build table index error:ret=%d", ret);
            }
            else
            {
              sstable_writer_buffer_.set_uncomp_buf(buf_ptr, data_len);
        ...
        else
        {
          ...
          if (OB_SUCCESS != (ret = sstable_.build_table_index(
                  buf_ptr, data_len)))
          {
            YYSYS_LOG(WARN, "sstable build table index error:ret=%d", ret);
          }
          else
          {
            sstable_writer_buffer_.set_uncomp_buf(buf_ptr, data_len);
    ...
    int ObCompactSSTableWriter::finish_current_table_schema()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (sstable_.is_alloc_table_schema_buf())
        {
          ...
          {
            YYSYS_LOG(WARN, "ensure_uncomp_buf error:ret=%d", ret);
          }
          else
          {
            buf_ptr = sstable_writer_buffer_.get_uncomp_buf_ptr();
          ...
          if (OB_SUCCESS == ret)
          {
            if (OB_SUCCESS != (ret = sstable_.build_table_schema(
                    buf_ptr, buf_size, data_len)))
            {
              YYSYS_LOG(WARN, "build table schema error:ret=%d", ret);
            }
            else
            {
              sstable_writer_buffer_.set_uncomp_buf(buf_ptr, data_len);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != (ret = write_record_header(record_header)))
        {
          YYSYS_LOG(WARN, "write record header:ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = write_record_body(buf, len)))
        {
          YYSYS_LOG(WARN, "write record body error:ret=%d", ret);
    ...
    int ObCompactSSTableWriter::finish_current_sstable_trailer_offset()
    {
      ...
      {
        YYSYS_LOG(WARN, "writer record body error:ret=%d", ret);
      }
      else
      {
        ...
        const int64_t block_buf_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "block buf == NULL or block buf len <= 0:block_buf_len=%ld", block_buf_len);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (0 == block_buf_len)
      {
        //do nothing
              ...
              block_buf_len, false)))
      {
        YYSYS_LOG(WARN, "file append error:ret=%d", ret);
      }
      else
      {
