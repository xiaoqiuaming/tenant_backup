  ...
  namespace compactsstablev2
  {
    int ObSSTable::finish_one_table()
    {
      ...
      if (NULL == (def = table_schema_.get_table_schema(table_id, 
              true, size)))
      {
        YYSYS_LOG(WARN, "get table schema error");
        ret = OB_ERROR;
      }
      else
      {
        for (int64_t i = 0; i < size; i ++)
        {
          ...
          {
            YYSYS_LOG(WARN, "add item error:ret=%d", ret);
            break;
          }
          else
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "table index builder add item error:ret=%d", ret);
        }
        else
        {
    ...
    int ObSSTable::check_row(const ObRowkey& rowkey, const ObRow& rowvalue)
    {
      ...
      if (DENSE_DENSE == sstable_header_.row_store_type_)
      {
                ...
                rowkey, rowvalue)))
        {
          ...
          YYSYS_LOG(WARN, "table schema check row fail:table_id=%lu,"
    ...
    int ObSSTable::check_row(const common::ObRow& row)
    {
      ...
      if (DENSE_DENSE == sstable_header_.row_store_type_)
      {
        ...
        if (OB_SUCCESS != (ret = table_schema_.check_row(table_id, row)))
        {
          ...
          YYSYS_LOG(WARN, "table schema check row fail:table_id=%lu,row=%s", 
              table_id, buf);
        }
      }
