...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        const common::ObCompactStoreType& row_store_type)
    {
      ...
      if (!data.available())
      {
        YYSYS_LOG(WARN, "invalid BlockData:data.internal_buf_=%p," \
      ...
      {
        YYSYS_LOG(WARN, "invalid row_store_type:row_store_type=%d",
            row_store_type);
      }
      else
      {
        ...
        if (index_item_length > data.internal_buf_size_)
        {
          ...
          if (arena == NULL)
          {
            YYSYS_LOG(WARN, "GET_TSI_MULT error:ModuleArena, " \
                "TSI_COMPACTSSTABLEV2_MODULE_ARENA_1");
          }
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "failed to alloc memrory for " \
        ...
        common::ObCompactCellIterator& row) const
    {
      ...
      {
        YYSYS_LOG(WARN, "row init error:ret=%d, row_buf=%p,row_store_type_=%d",
        ...
        common::ObRowkey& key) const
    {
      ...
      {
        YYSYS_LOG(WARN, "get row error:ret=%d,index.offset_=%d," \
            "index.size_=%d", ret, index->offset_, index->size_);
      }
      else if (OB_SUCCESS != (ret = get_row_key(row, key)))
      {
        YYSYS_LOG(WARN, "ger row key error:ret=%d", ret); 
      }
      else
      {
        //do nothing
          ...
          sstable::ObSSTableRowCacheValue& row_value) const
    {
      ...
      if (NULL == index)
      {
        YYSYS_LOG(WARN, "argument error:NULL==index");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == data_begin_)
      {
        YYSYS_LOG(WARN, "context error:NULL==data_begin_");
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = get_row_key(index, rowkey)))
      {
        YYSYS_LOG(WARN, "get row key error:ret=%d,index.offset_=%d" \
            "index.size_=%d", ret, index->offset_, index->size_);
      }
      else
      {
        ...
        ObRowkey& key) const
    {
      ...
      do
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "row store type error:row_store_type_=%d",
                row_store_type_);
            ret = OB_ERROR;
            break;
          }
