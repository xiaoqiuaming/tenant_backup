...
namespace oceanbase
{
  namespace compactsstablev2
  {
    ...
    int ObSSTableBuffer::add_item(const void* buf, const int64_t length)
    {
      ...
      if (0 >= length)
      {
        YYSYS_LOG(WARN, "invalid argument:item size=%ld", length);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "data is larger than the block size ret=%d", ret);
          }
          else
          {
        ...
        int64_t& length) const
    {
      ...
      {
        YYSYS_LOG(WARN, "buf is no enought:length_=%ld,buf_size=%ld",
            length_, buf_size);
        ret = OB_ERROR;
      }
      else
    ...
    int ObSSTableBuffer::get_data(const char*& buf, int64_t& length) const
    {
      ...
      {
        YYSYS_LOG(WARN, "the block count must be 1");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSSTableBuffer::get_free_buf(char*&buf, int64_t& size)
    {
      ...
      if (mem_block_list_.empty())
      {
        if (OB_SUCCESS != (ret = alloc_block()))
        {
          YYSYS_LOG(ERROR, "alloc block error:ret=%d", ret);
        }
      }
    ...
    */
    ...
    int ObSSTableBuffer::alloc_block()
    {
      ...
      {
        YYSYS_LOG(WARN, "alloc mem error:ret=%d,buf=%p,block_size_=%ld",
            ret, buf, block_size_);
      }
      else
      {
