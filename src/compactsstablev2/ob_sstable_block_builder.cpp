  ...
  namespace compactsstablev2
  {
    int ObSSTableBlockBuilder::reset()
    {
      ...
      if (NULL == row_buf_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "faild to alloc memory:ret=%d,row_buf_=%p," \
      ...
      if (NULL == row_index_buf_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "faild to alloc memory:ret=%d,row_index_buf_=%p," \
        ...
        const ObRow& row_value)
    {
      ...
      while(true)
      {
        ...
        if (NULL == cur_row_ptr)
        {
          YYSYS_LOG(WARN, "context error:cur_row_ptr=NULL");
          ret = OB_ERROR;
        }
        if (row_remain_size <= 0)
        {
          YYSYS_LOG(WARN, "context error:row_remain_size=%ld", row_remain_size);
          ret = OB_ERROR;
        }
        else if (OB_SUCCESS != (ret = row.init(cur_row_ptr, row_remain_size,
                row_store_type_)))
        {
          YYSYS_LOG(WARN, "row init error:ret=%d,cur_row_ptr=%p," \
        ...
        else if (OB_SUCCESS != (ret = row.append_row(row_key, row_value)))
        {
          ...
          {
            YYSYS_LOG(WARN, "alloc mem error:ret=%d,new_buf=%p,new_size=%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "add row index error:ret=%d, index.row_offset_=%d", 
    ...
    int ObSSTableBlockBuilder::build_block(char*& buf, int64_t& size)
    {
      ...
      {
        YYSYS_LOG(WARN, "row_buf_==NULL or row_index_buf_==NULL:row_buf_=%p" \
      ...
      else
      {
        ...
        while (true)
        {
          if (new_size - row_length_ < row_index_length_)
          {
            ...
            if (10 == cnt)
            {
              YYSYS_LOG(WARN, "error? while cnt is so large");
              ret = OB_ERROR;
            }
          }
          else
        ...
        if (OB_SUCCESS == ret)
        {
          if (use_new_buf)
          {
            if (OB_SUCCESS != (ret = alloc_mem(new_buf, new_size)))
            {
              YYSYS_LOG(ERROR, "alloc mem error:ret=%d,new_buf=%p,new_size=%ld",
                  ret, new_buf, new_size);
            }
            else
            {
          ...
          else
          {
            ...
            if (NULL == buf_ptr)
            {
              YYSYS_LOG(WARN, "context error:buf_ptr==NULL");
              ret = OB_ERROR;
            }
            else
            {
    ...
    int ObSSTableBlockBuilder::alloc_mem(char*& buf, const int64_t size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to alloc memory,size=%ld," \
