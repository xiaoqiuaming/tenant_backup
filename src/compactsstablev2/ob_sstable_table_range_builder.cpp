...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        const ObBorderFlag& flag)
    {
      ...
      {
        YYSYS_LOG(WARN, "buf add item error:ret=%d,flag=%d", ret, data);
      }
    ...
    int ObSSTableTableRangeBuilder::add_item(const common::ObRowkey& row_key)
    {
      ...
      while(true)
      {
        ...
        {
          YYSYS_LOG(WARN, "buf get free buf error:ret=%d,buf=%p,size=%ld",
              ret, buf, size);
          break;
        }
        else if (OB_SUCCESS != (ret = row.init(buf, size, DENSE)))
        {
          ...
          {
            YYSYS_LOG(WARN, "buf alloc block error:ret=%d", ret);
            break;
          }
          else
          {
        ...
        {
          YYSYS_LOG(WARN, "buf set writer size error:ret=%d,row.size()=%ld",
              ret, row.size());
          break;
        }
        else
