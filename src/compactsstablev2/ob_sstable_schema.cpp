...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        int64_t& size) const
    {
      ...
      {
        YYSYS_LOG(WARN, "table id error:table_id =%lu", table_id);
      }
      else if (0 == get_column_count())
      {
        YYSYS_LOG(WARN, "column count = 0");
      }
      else if (hash_init_)
      {
        TableKey key;
        ...
        else if (hash::HASH_EXIST == hash_get)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get offset error");
          }
        ...
        {
          YYSYS_LOG(ERROR, "can not go there");
        }
      }
      else
      {
        YYSYS_LOG(ERROR, "hash is not init");
      }
    ...
    int ObSSTableSchema::add_column_def(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      {
        YYSYS_LOG(WARN, "is largert than the column count");
        ret = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (offset >= 0)
        {
          YYSYS_LOG(WARN, "find column offset error");
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret && !hash_init_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "create hashmap error");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
            ...
            && column_def_ == column_def_array_)
        {
          ...
          {
            YYSYS_LOG(WARN, "malloc memory error");
            ret = OB_ERROR;
          }
          else
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "update hashmaps error:ret=%d"
        ...
        const uint64_t table_id, const int64_t rowkey_seq) const
    {
      ...
      if (OB_INVALID_ID == table_id || 0 == table_id)
      {
        YYSYS_LOG(ERROR, "table id error");
      }
      else
      {
        ...
        if (NULL == def)
        {
          YYSYS_LOG(ERROR, "get table schema error");
        }
        else
        {
        ...
        const uint64_t table_id, const int64_t rowvalue_seq) const
    {
      ...
      else
      {
        ...
        if (NULL == def)
        {
          YYSYS_LOG(ERROR, "def == NULL");
        }
        else if (rowvalue_seq > size)
        {
          YYSYS_LOG(WARN, "the rowvalue_seq error:rowvalue_seq=%ld,size=%ld",
              rowvalue_seq, size);
        }
        else
        {//从1开始算
    ...
    int ObSSTableSchema::operator=(const ObSSTableSchema& schema)
    {
      ...
      for (int64_t i = 0; i < column_num; i ++)
      {
        ...
        {
          YYSYS_LOG(WARN, "get oclumn def error");
          break;
        }
        ...
        const ObRowkey& rowkey, const ObRow& rowvalue)
    {
      ...
      {
        YYSYS_LOG(WARN, "get table schema error:table_id=%lu,size=%ld",
            table_id, size);
        ret = OB_ERROR;
      }
      else if (size != rowkey_obj_cnt)
      {
        YYSYS_LOG(WARN, "rowkey obj cnt is not equal:size=%ld,"
      ...
      else
      {
        ...
        for (int64_t i = 0; i < rowkey_obj_cnt; i ++)
        {
          ...
          {
            YYSYS_LOG(WARN, "value type is not match:i=%ld,"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "rowvalue obj cnt not match:size=%ld,"
        ...
        else
        {
          ...
          for (int64_t i = 0; i < rowvalue_obj_cnt; i ++)
          {
            ...
            {
              YYSYS_LOG(WARN, "rowvalue raw get cell error:ret=%d,i=%ld",
                  ret, i);
              break;
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "rowvalue column value type not match:"
      ...
      else
      {
        for (i = 0; i < rowkey_obj_cnt; i ++)
        {
                  ...
                  t_table_id, column_id)))
          {
            ...
            YYSYS_LOG(WARN, "row raw get cell error:ret=%d,i=%ld,obj=%s"
          ...
          else
          {
            if (def[i].column_value_type_ != obj->get_type())
            {
              YYSYS_LOG(WARN, "rowkey type is not match:i=%ld,"
        ...
        const uint64_t column_id) const
    {
      ...
      {
        YYSYS_LOG(WARN, "column id error");
      }
      else
      {
        int hash_get = 0;
        ...
        if (-1 == hash_get)
        {
         // YYSYS_LOG(INFO, "hash get failed:hash_get=%d, key.table_id_=%lu,"
    ...
    int ObSSTableSchema::create_hashmaps()
    {
      ...
      {
        YYSYS_LOG(WARN, "create schema hashmap error");
      }
      else if (OB_SUCCESS != (ret = table_column_schema_hashmap_.create(
              hash::cal_next_prime(512))))
      {
        YYSYS_LOG(WARN, "create table column schema hashmap error");
      }
      else
      {
        hash_init_ = true;
    ...
    int ObSSTableSchema::update_hashmaps(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      {
        YYSYS_LOG(WARN, "update table schema hashmap error");
      }
      else if (OB_SUCCESS != (ret = update_table_column_schema_hashmap(column_def)))
      {
        YYSYS_LOG(WARN, "update table column schema hashmpa error");
      }
    ...
    int ObSSTableSchema::update_table_schema_hashmap(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      if (!hash_init_)
      {
        YYSYS_LOG(WARN, "hash is not init");
        ret = OB_NOT_INIT;
      }
      else
      {
                ...
                || column_def.is_rowkey_column() != last->is_rowkey_column())))
        {
          ...
          if (-1 == ret_hash)
          {
            YYSYS_LOG(WARN, "table schema hash map set error:ret_hash=%d,"
              ...
              && column_def.is_rowkey_column() == last->is_rowkey_column()))
        {
          ...
          {
            YYSYS_LOG(ERROR, "table schema hashmap get errror:ret_hash=%d,"
          ...
          else if (hash::HASH_EXIST == ret_hash)
          {
            ...
            if (-1 == ret_hash)
            {
              YYSYS_LOG(WARN, "table schema hashmap set error:ret_hash=%d,"
                  "key.table_id_=%lu,key.is_rowkey_=%d,value.offset_=%ld",
                  ret_hash, key.table_id_, key.is_rowkey_, value.offset_);
              ret = OB_ERROR;
            }
    ...
    int ObSSTableSchema::update_table_column_schema_hashmap(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      else
      {
        ...
        else if (hash::HASH_NOT_EXIST == ret_hash)
        {
          ...
          if (-1 == ret_hash)
          {
            YYSYS_LOG(ERROR, "hash not exist");
            ret = OB_ERROR;
          }
        }
        else
        ...
        const common::ObSchemaManagerV2 & schema, ObSSTableSchema& sstable_schema)
    {
      ...
      else
      {
        ...
        for (col_index = 0; col_index < rowkey_info.get_size() && OB_SUCCESS == ret; ++col_index)
        {
          ...
          {
            YYSYS_LOG(ERROR,"add column_def(%lu,%lu,%ld) failed col_index : %ld",column_def.table_id_,
