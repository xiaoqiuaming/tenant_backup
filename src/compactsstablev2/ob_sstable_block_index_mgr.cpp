...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        ObBlockPositionInfos& pos_info) const
    {
      ...
      {//基准key
        YYSYS_LOG(ERROR, "trans range to search key error");
      }
      ...
      else
      {
        ...
        if (OB_SUCCESS != (ret = store_block_position_info(
                find_it, bound, mode, end_key, pos_info)))
        {
          YYSYS_LOG(WARN, "stroe block position info error:ret=%d",
              ret);
        }
      }
        ...
        ObBlockPositionInfos& pos_info) const
    {
      ...
      {
        YYSYS_LOG(WARN, "get bound error:ret=%d", ret);
      }
      else
      {
        ...
        {//check right border
          //YYSYS_LOG(WARN, "check border error");
        }
        else
        {
          switch (mode)
        ...
        const Bound& bound, const_iterator& find) const
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "get row key error:ret=%d", ret);
          }
          else
          {
            switch (mode)
        ...
        ObBlockPositionInfos& pos_info) const
    {
      ...
      {
        YYSYS_LOG(WARN, "beyond the range");
        ret = OB_ERROR;
      }
      else
      {
