...
namespace oceanbase
{
  namespace compactsstablev2
  {
    ...
    ObCompactSSTableScanner::~ObCompactSSTableScanner()
    {
      YYSYS_LOG(DEBUG, "destruct ObCompactSSTableScanner");
    }
    ...
    int ObCompactSSTableScanner::set_scan_param(const ObSSTableScanParam* scan_param, const ScanContext* scan_context)
    {
      ...
      if (NULL == scan_param)
      {
        YYSYS_LOG(WARN, "scan_param is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == scan_context)
      {
        YYSYS_LOG(WARN, "scan_context is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!scan_context->is_valid())
      {
        YYSYS_LOG(WARN, "invalid scan_context: block_cache_=[%p], block_index_cache_=[%p]",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "table id is invalid: table_id=[%lu]", table_id);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "initialize error: ret=[%d], scan_param=[%s]", ret, to_cstring(*scan_param));
        }
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid row store type: row_store_type=[%d]", row_store_type);
            ret = OB_INVALID_ARGUMENT;
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "build row desc error: ret=[%d], row_store_type=[%d], table_id=[%lu]", ret, row_store_type, table_id);
        }
        else if (OB_SUCCESS != (ret = build_column_index(row_store_type, table_id)))
        {
          YYSYS_LOG(WARN, "build column index error: ret=[%d], row_store_type=[%d]", ret, row_store_type);
        }
        else if (OB_SUCCESS != (ret = match_rowkey_desc(row_count_flag, table_id)))
        {
          YYSYS_LOG(WARN, "match rowkey desc error: ret=[%d], row_count_flag=[%d]", ret, row_count_flag);
        }
      }
      ...
      if (OB_SUCCESS == ret && ITERATE_END != iterate_status_)
      {
        ...
        {
          YYSYS_LOG(WARN, "serarh block index error: ret=[%d]", ret);
        }
        else if (OB_SUCCESS != (ret = fetch_next_block()))
        {
          YYSYS_LOG(WARN, "fetch next block error: ret=[%d]", ret);
        }
      }

      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "sstable scanner set scan param success");
      }
    ...
    int ObCompactSSTableScanner::get_next_row(const common::ObRowkey*& row_key, const common::ObRow*& row_value)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "deserialize row error: ret=[%d]", ret);
        }
        else if (OB_SUCCESS != (ret = store_and_advance_row()))
        {
          YYSYS_LOG(WARN, "store and advance row error: ret=[%d]", ret);
        }
        else
        {
          row_key = &row_key_;
    ...
    int ObCompactSSTableScanner::initialize(const ScanContext* scan_context, const ObSSTableScanParam* scan_param)
    {
      ...
      {
        YYSYS_LOG(WARN, "alloc buffer error:ret=[%d], uncomp_buf_size_=[%ld]", ret, uncomp_buf_size_);
      }
      else if (OB_SUCCESS != (ret = alloc_buffer(internal_buf_, internal_buf_size_)))
      {
        YYSYS_LOG(WARN, "alloc buffer error:ret=[%d], internal_buf_size_=[%ld]", ret, internal_buf_size_);
      }
      else
      {
        YYSYS_LOG(DEBUG, "sstable scanner initialize success");
      }
        ...
        const int64_t buf_size)
    {
      ...
      if (buf_size <= 0)
      {
        YYSYS_LOG(ERROR, "invalid buf size: buf_size=[%ld]", buf_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == arena)
      {
        YYSYS_LOG(ERROR, "failed to get tsi mult arena");
        ret = OB_ERROR;
      }
      else if (NULL == (buf = arena->alloc(buf_size)))
      {
        YYSYS_LOG(ERROR, "failed to alloc memory for buf:buf_size=%ld", buf_size);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
        YYSYS_LOG(DEBUG, "alloc buf success: buf_size=%ld", buf_size);
      }
    ...
    int ObCompactSSTableScanner::convert_column_id(const uint64_t table_id)
    {
      ...
      if (NULL == schema)
      {
        YYSYS_LOG(WARN, "schema ptr is NULL");
        ret = OB_ERROR;
      }
      else if (NULL == scan_column_array_ptr)
      {
        YYSYS_LOG(WARN, "scan_column_array_ptr is NULL");
        ret = OB_ERROR;
      }
      else if (scan_column_array_size <= 0)
      {
        YYSYS_LOG(WARN, "scan_column_array_size <= 0");
        ret = OB_ERROR;
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < scan_column_array_size; i ++)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid column id: column_id=[%lu]", cur_column_id);
          ret = OB_ERROR;
        }
        else if (NULL != (def = schema->get_column_def(table_id, cur_column_id)))
        {
          ...
          {//rowkey
            ...
            {
              YYSYS_LOG(WARN, "scan column indexes add column id error: i=[%ld], ret=[%d], scan_column_indexes_=[%s], "
        ...
        {//not exist;
          ...
          {
            YYSYS_LOG(WARN, "scan column indexes add column id error: i=[%ld], ret=[%d], scan_column_indexes_=[%s], cur_column_id=[%lu]",
    ...
    int ObCompactSSTableScanner::match_rowkey_desc(const RowCountFlag row_count_flag, const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "scan column array ptr is NULL");
        ret = OB_ERROR;
      }
      else if (scan_rowkey_column_cnt < 0)
      {
        YYSYS_LOG(WARN, "scan rowkey column cnt < 0: scan_rowkey_column_cnt=[%ld]", scan_rowkey_column_cnt);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
            ...
            || DENSE_DENSE_ROW_NULL == row_count_flag)
        {
          ...
          {
            YYSYS_LOG(WARN, "schema is null");
            ret = OB_ERROR;
          }
          else if (NULL == (def_array_ptr = schema->get_table_schema(table_id, true, def_array_size)))
          {
            YYSYS_LOG(WARN, "schema get table schema error: schema=[%s], table_id=[%lu]", to_cstring(*schema), table_id);
            ret = OB_ERROR;
          }
          else if (0 == def_array_size)
          {
            YYSYS_LOG(WARN, "def_array_size must > 0: def_array_size=[%lu]", def_array_size);
            ret = OB_ERROR;
          }
        ...
        {
          YYSYS_LOG(WARN, "can not go here: scan_flag_=[%d], row_count_flag=[%d], is_daily_merge_scan=[%d]",
    ...
    int ObCompactSSTableScanner::build_row_desc(const ObCompactStoreType row_store_type, const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "column_id_array_ptr is NULL");
        ret = OB_ERROR;
      }
      else if (0 == column_id_array_size)
      {
        YYSYS_LOG(WARN, "column_id_array_size==0");
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (DENSE_DENSE == row_store_type || DENSE_SPARSE == row_store_type)
        {
          for (int64_t i = 0; OB_SUCCESS == ret && i < column_id_array_size; i ++)
          {
            ...
            {
              YYSYS_LOG(WARN, "row desc add column desc error: ret=[%d], table_id=[%lu], column_id=[%lu]",
        ...
        if (DENSE_SPARSE == row_store_type)
        {
          ...
          {
            YYSYS_LOG(WARN, "row desc add column desc error: ret=[%d], table_id=[%lu]", ret, table_id);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (DENSE_DENSE == row_store_type)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "convert column id error: ret=[%d]", ret);
            }
            else
            {
    ...
    int ObCompactSSTableScanner::deserialize_row(ObCompactCellIterator& row)
    {
      ...
      {//rowkey
        ...
        {
          YYSYS_LOG(WARN, "row get next cell error: i=[%ld], ret=[%d]", rowkey_cnt, ret);
        }
        else
        {
          if (is_row_finished)
      ...
      if (DENSE_SPARSE == row_store_type)
      {
        for (int64_t i = 0; OB_SUCCESS == ret; i ++)
        {
          ...
          {
            YYSYS_LOG(WARN, "row get next cell error: i=[%ld], ret=[%d]", i, ret);
          }
          else
          {
            if (is_row_finished)
        ...
        const bool first_time)
    {
      ...
      {
        YYSYS_LOG(WARN, "sstable header is NULL");
        ret = OB_ERROR;
      }
      else if (0 == table_id || OB_INVALID_ID == table_id)
      {
        YYSYS_LOG(WARN, "invalid table id: table_id=[%lu]", table_id);
        ret = OB_ERROR;
      }
      else if (NULL == table_index)
      {
        YYSYS_LOG(WARN, "table index is null: table_id=[%lu]", table_id);
        ret = OB_ERROR;
      }
      else
      {
      ...
      {//block index array(if not find, return OB_BEYOND_THE_RANGE)
        ...
        if (first_time)
        {
          if (OB_SUCCESS != (ret = scan_context_->block_index_cache_
                ->get_block_position_info(
                  ...
                  index_array_)))
          {
            YYSYS_LOG(DEBUG, "block index cache get block position info"
        ...
        else
        {
          ...
          {
            YYSYS_LOG(DEBUG, "block index cache next offset:"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "prepare read blocks error:ret=%d", ret);
        }
      }
      ...
      {
        iterate_status_ = ITERATE_END;
        YYSYS_LOG(DEBUG, "OB_BEYOND_THE_RANGE");
        ret = OB_SUCCESS;
      }
      else if (OB_SUCCESS != ret)
      {
        iterate_status_ = ITERATE_IN_ERROR;
        YYSYS_LOG(WARN, "ITERATE ERROR:ret=%d", ret);
      }
    ...
    int ObCompactSSTableScanner::fetch_next_block()
    {
      ...
      do
      {
        ...
        {
          YYSYS_LOG(WARN, "load current block and advance error:ret=%d",
              ret);
          break;
        }
        ...
        const char*& buf, int64_t& buf_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable file id");
        ret = OB_ERROR;
      }
      else if (0 == table_id || OB_INVALID_ID == table_id)
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {//sync read
          ...
          {
            YYSYS_LOG(ERROR, "IO ERROR:ret=%d,sstable_file_id=%lu,"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "get record header error:ret=%d,"
        ...
        else if (header.data_length_ > uncomp_buf_size_)
        {
          ...
          if (OB_SUCCESS != (ret = alloc_buffer(uncomp_buf_,
                  uncomp_buf_size_)))
          {
            YYSYS_LOG(ERROR, "failed to alloc buffer:ret=%d,"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (header.is_compress())
        {
          ...
          if (NULL != dec)
          {
            ...
            {
                YYSYS_LOG(WARN, "decompress error:ret=%d,"
          ...
          {
            YYSYS_LOG(ERROR, "compressor is NULL");
          }
        }
        else
        {
    ...
    int ObCompactSSTableScanner::load_current_block_and_advance()
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid row store type:row_stroe_type=%d",
            row_store_type);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret && is_forward_status())
      {
        ...
        {//last block is the end, no need to looking forward
          iterate_status_ = ITERATE_END;
          YYSYS_LOG(DEBUG, "iterate_status==ITERATE_END");
        }
        ...
        {//current batch block scan over, begin fetch next batch blocks
          ...
          {
            YYSYS_LOG(WARN, "load block index error:ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret && is_forward_status())
      {
        ...
        else
        {
              ...
              block_data_size > 0)
          {
            ...
            {
              YYSYS_LOG(WARN, "block scanner set scan param error: ret=[%d], range=[%s], is_reverse_scan=[%d], "
          ...
          {
            YYSYS_LOG(WARN, "read current block data error: ret=[%d], block_data_ptr=[%p], block_data_size=[%ld]",
                ret, block_data_ptr, block_data_size);
          }
        }
      }
      ...
      {
        iterate_status_ = ITERATE_IN_ERROR;
        YYSYS_LOG(ERROR, "iterate_staus_ == ITERATE_IN_ERROR");
      }
    ...
    int ObCompactSSTableScanner::prepare_read_blocks()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (!sstable_scan_param_->is_sync_read())
        {
          ...
          {
            YYSYS_LOG(WARN, "block cache advise error:ret=%d,"
        ...
        ObSSTableScanColumnIndexes::Column& column) const
    {
      ...
      if (DENSE_DENSE != row_store_type
          && DENSE_SPARSE != row_store_type)
      {
        YYSYS_LOG(ERROR, "invalid row store type:row_store_type=%d",
            row_store_type);
      }
      else if (cursor < 0 || INVALID_CURSOR == cursor)
      {
        YYSYS_LOG(ERROR, "invalid cursor:cursor=%ld", cursor);
      }
      else if (OB_SUCCESS != (ret = scan_column_indexes_.get_column(cursor, column)))
      {
        YYSYS_LOG(ERROR, "scan column indexes get clumn error: ret=[%d], scan_column_indexes_=[%s], cursor=[%ld]",
    ...
    int ObCompactSSTableScanner::store_and_advance_row()
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid column cnt: column_cnt=[%ld]", scan_column_cnt);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (DENSE_DENSE == row_store_type)
        {
          ...
          {
            YYSYS_LOG(WARN, "dense dense store row error: ret=[%d], scan_column_cnt=[%ld]", ret, scan_column_cnt);
          }
        }
        else if (DENSE_SPARSE == row_store_type)
        {
          ...
          {
            YYSYS_LOG(WARN, "dense sparse store row error: ret=[%d], scan_column_cnt=[%ld]", ret, scan_column_cnt);
          }
        }
      }
    ...
    int ObCompactSSTableScanner::dense_dense_store_row(const int64_t scan_column_cnt)
    {
      ...
      if (DENSE_DENSE_FULL_ROW_SCAN == scan_flag_)
      {
        for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey_column_cnt_; i ++, index ++)
        {
          ...
          {
            YYSYS_LOG(WARN, "row value raw set cell error: i=[%ld], ret=[%d], index=[%ld], obj=[%s]",
      ...
      else if (DENSE_DENSE_NORMAL_ROW_SCAN == scan_flag_)
      {
        for (; OB_SUCCESS == ret && index < scan_column_cnt; index ++)
        {
          ...
          {
              YYSYS_LOG(WARN, "get column index:ret=[%d], index=[%ld]", ret, index);
          }
          else if (ObSSTableScanColumnIndexes::Rowkey == column.type_)
          {
            ...
            {
              YYSYS_LOG(WARN, "row value set cell error:ret=[%d], index=[%ld], column.index_=[%ld], obj=[%s], row_value=[%s]",
          ...
          else if (ObSSTableScanColumnIndexes::NotExist == column.type_)
          {
            if (not_exit_col_ret_nop)
            {
              ...
              {
                YYSYS_LOG(WARN, "row value raw set cell error: ret=[%d], index=[%ld], row_value=[%s]",
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "row value raw set cell error: ret=[%d], index=[%ld], row_value_=[%s]",
                    ret, index, to_cstring(row_value_));
              }
            }
          }
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "row value raw set cell error:ret=[%d], row_value=[%s], index=[%ld], column.index=[%ld], obj=[%s]",
    ...
    int ObCompactSSTableScanner::dense_sparse_store_row(const int64_t scan_column_cnt)
    {
      ...
      {
        YYSYS_LOG(WARN, "def array is NULL");
        ret = OB_ERROR;
      }
      else if (0 == size)
      {
        YYSYS_LOG(WARN, "the rowkey count of table is 0");
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "row value reset error: ret=[%d], row_value_=[%s]", ret, to_cstring(row_value_));
        }
        else if (OB_ACTION_FLAG_COLUMN_ID == column_ids_[0])
        {
          if (OB_SUCCESS != (ret = row_value_.raw_set_cell(scan_column_cnt, column_objs_[0])))
          {
            YYSYS_LOG(WARN, "row value raw set cell error: ret=[%d], scan_column_cnt=[%ld], obj=[%s]",
                ret, scan_column_cnt, to_cstring(column_objs_[0]));
          }
        }
        else if (OB_SUCCESS != (ret = row_value_.raw_set_cell(scan_column_cnt, valid_obj)))
        {
          YYSYS_LOG(WARN, "row value raw set cell error: ret=[%d], scan_column_cnt=[%ld]", ret, scan_column_cnt);
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey_column_cnt_; i ++)
      {
        if (OB_INVALID_INDEX != (cell_idx = row_desc->get_idx(table_id, def_array[i].column_id_)))
        {
          ...
          {
            YYSYS_LOG(WARN, "row value raw set cell error: ret=[%d], cell_idx=[%ld], obj=[%s]",
