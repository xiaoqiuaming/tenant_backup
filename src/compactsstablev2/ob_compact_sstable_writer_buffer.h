  ...
  namespace compactsstablev2
  {
    ...
    struct BlockListNode
    {
      ...
      inline int set_block_endkey(common::ObRowkey& rowkey)
      {
        ...
        if (common::OB_SUCCESS != (rowkey.deep_copy(block_endkey_, 
                allocator)))
        {
          YYSYS_LOG(WARN, "rowkey deep copy error");
        }
    ...
    public:
      ...
      inline int push_cur_node()
      {
        ...
        {
          YYSYS_LOG(WARN, "mem block list push back error:ret=%d", ret);
        }
        else
        {
        ...
        const common::ObRowkey& rowkey)
    {
      ...
      {
        YYSYS_LOG(ERROR, "the inserted rowkey is not larger than"
        ...
        const common::ObRow& row)
    {
      ...
      {
        YYSYS_LOG(ERROR, "row get rowkey error:ret=[%d],row=[%s]",
            ret, to_cstring(row));
      }
      else if (!(*rowkey > cur_rowkey_))
      {
        YYSYS_LOG(ERROR, "the rowkey of inserted row is not larger than"
            "current rowkey:*rowkey=[%s],cur_rowkey_=[%s]",
            to_cstring(*rowkey), to_cstring(cur_rowkey_));
        ret = common::OB_INVALID_ARGUMENT;
      }
