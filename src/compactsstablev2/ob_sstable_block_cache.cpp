...
namespace oceanbase
{
  namespace compactsstablev2
  {
    int ObSSTableBlockCache::init(const int64_t cache_mem_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "kv cache init error:ret=%d,"
      ...
      {
        inited_ = true;
        YYSYS_LOG_US(DEBUG, "init blockcache succ cache_mem_size=%ld, ", cache_mem_size);
      }
        ...
        const bool reverse_scan/*=false*/)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "block cache is not inited");
        ret = OB_NOT_INIT;
      }
      else if (NULL == fileinfo_cache_)
      {
        YYSYS_LOG(WARN, "fileinfo_cache_ is null");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_INVALID_ID == sstable_id || OB_INVALID_ID == table_id
          || 0 == table_id)
      {
        YYSYS_LOG(WARN, "table id is invalid:sstable_id=%lu, table_id=%lu", 
            sstable_id, table_id);
        ret = OB_INVALID_ARGUMENT;
      }
              ...
              table_id, true)))
      {
        YYSYS_LOG(WARN, "get aio buf mgr error:sstable_id=%ld,table_id=%ld",
            sstable_id, table_id);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "aio buf mgr advise error:ret=%d,sstable_id=%ld,"
        ...
        ObSSTableBlockBufferHandle& buffer_handle)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument:sstable_id=%lu, "
        ...
        else
        {
          ...
          if (OB_SUCCESS == status && NULL != buffer)
          {
            ...
            if (check_crc)
            {
              ...
              {
                YYSYS_LOG(WARN, "check record error:input_value.buffer_=%p,"
            ...
            if (OB_SUCCESS == status)
            {
              ...
              {
                YYSYS_LOG(WARN, "put and fetch error:key.sstable_id_=%lu,"
          ...
          {
            YYSYS_LOG(WARN, "read record error:status=%d,fileinfo_cache=%p,"
        ...
        const bool check_crc/* =true */)
    {
      ...
      {
        YYSYS_LOG(WARN, "INVALID argument:cursor=%ld,"
        ...
        else
        {
          ...
          if (!is_reverse)
          {
                ...
                && readahead_size < MAX_READ_AHEAD_SIZE; ++ pos)
            {
              ...
              {
                YYSYS_LOG(WARN, "next offset error:next_offset=%ld,"
          ...
          if (OB_SUCCESS == status && NULL != record_buf)
          {
            for (int64_t i = start_cursor; i <= end_cursor; ++ i)
            {
              ...
              if (check_crc)
              {
                ...
                if (OB_SUCCESS != status)
                {
                  YYSYS_LOG(WARN, "check record error:status=%d,"
              ...
              if (cursor == i)
              {
                ...
                {
                  YYSYS_LOG(WARN, "put and fetch error:status=%d,"
        ...
        const bool check_crc/*ret= true*/)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param:inited_=%d, fileinfo_cache_=%p",
      ...
      {
        YYSYS_LOG(WARN, "get aio buf mgr error:aio_buf_mgr=%p,"
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get block error:status=%d,sstable_id=%lu,"
