...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        bool& need_looking_forward)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid block data:block_internal_buf_=%p," \
      ...
      {
        YYSYS_LOG(WARN, "initialize error:ret=%d,is_reverse_scan=%d",
            ret, is_reverse_scan);
      }
      else if (OB_SUCCESS != (ret = block_reader_.init(
              block_data, row_store_type)))
      {//init block reader
        YYSYS_LOG(WARN, "block reader init error:ret=%d,"
              ...
              start_iterator, need_looking_forward)))
      {
        ...
        YYSYS_LOG(WARN, "locate start pos error:ret=%d,range=%s," \
      ...
      {
        YYSYS_LOG(DEBUG, "locate end pos error:ret=%d,range=%s," \
    ...
    int ObSSTableBlockScanner::get_next_row(ObCompactCellIterator*& row)
    {
      ...
      if (!is_inited_)
      {
        YYSYS_LOG(WARN, "block scanner is not inited");
      ...
      {
        YYSYS_LOG(WARN, "invalid argument:row_cursor_=%p," \
      ...
      {
        YYSYS_LOG(WARN, "load current row error:row_cursor_->offset_=%d"
        ...
        bool& need_looking_forward)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get row key error:ret=%d," \
        ...
        ObSSTableBlockReader::const_iterator row_index)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid row index:row_index=%p,"
            "row_start_index_=%p,row_last_index_=%p",
        ...
        ret = OB_ERROR;
              ...
              row_index, row_)))
      {
        YYSYS_LOG(WARN, "block reader get row error:ret=%d,"
