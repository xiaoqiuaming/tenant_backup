  ...
  namespace compactsstablev2
  {
    int ObSSTableSchemaCache::clear()
    {
      ...
      if (0 < schema_cnt_)
      {
        for (int64_t i = 0; i < schema_cnt_; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "schema array ref cnt !=0");
            ret = OB_ERROR;
          }
        ...
        const uint64_t table_id, const int64_t version)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (index >= 0)
        {
          YYSYS_LOG(INFO, "find schema node index error");
          ret = OB_ENTRY_EXIST;
        }
        ...
        const int64_t version)
    {
      ...
      else
      {
        ...
        if (index < 0)
        {
          YYSYS_LOG(WARN, "find schema node index error");
          ret = OB_ERROR;
        }
        else
        {
