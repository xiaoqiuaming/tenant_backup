...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        const ObRowkey& rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "rowkey deep copy error:ret=%d", ret);
      }
    ...
    int ObCompactSSTableWriterBuffer::set_cur_key(const ObRow& row)
    {
      ...
      if (OB_SUCCESS != (ret = row.get_rowkey(rowkey)))
      {
        ...
        YYSYS_LOG(WARN, "row get rowkey error:ret=%d,rowkey=%s", ret, buf);
      }
      else if (OB_SUCCESS != (ret = rowkey->deep_copy(cur_rowkey_, allocator)))
      {
        char buf[1024];
        cur_rowkey_.to_string(buf, 1024);
        YYSYS_LOG(WARN, "rowkey deep copy error:ret=%d,"
        ...
        const int64_t buf_size, const bool ptr_flag/*=true*/)
    {
      ...
      if (NULL == uncomp_buf_)
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(ERROR, "malloc memorry error");
          ret = OB_MEM_OVERFLOW;
        }
        else
        {
          ...
          if (NULL == uncomp_buf_)
          {
            YYSYS_LOG(WARN, "new struct error");
            ret = OB_MEM_OVERFLOW;
            ob_free(tmp_buf);
          }
          else
        ...
        const int64_t buf_size)
    {
      ...
      if (NULL == comp_buf_)
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(ERROR, "malloc memory error");
          ret = OB_MEM_OVERFLOW;
        }
        else
        {
    ...
    int ObCompactSSTableWriterBuffer::cur_node_prepare()
    {
      ...
      if (NULL == cur_node_)
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(ERROR, "alloc mem error");
          ret = OB_MEM_OVERFLOW;
        }
        else
        {
          ...
          if (NULL == cur_node_)
          {
            YYSYS_LOG(WARN, "create BlockListNode error");
            ret = OB_MEM_OVERFLOW;
            ob_free(tmp_buf);
          }
        }
    ...
    int ObCompactSSTableWriterBuffer::build_cur_node()
    {
      ...
      {//uncomp
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "set block endkey error:ret=%d", ret);
          }
        ...
        uint64_t table_id, const ObRowkey& row_key)
    {
      ...
      {
        YYSYS_LOG(WARN, "list bloomfilter insert error:ret=%d", ret);
      }
    ...
    int ObCompactSSTableWriterBuffer::compress(ObCompressor* compressor)
    {
      ...
      else 
      {
        ...
        {
          YYSYS_LOG(WARN, "ensure after compress buf space error:ret=%d", ret);
        }
