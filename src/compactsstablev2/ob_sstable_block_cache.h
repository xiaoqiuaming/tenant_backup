    ...
    public:
      ...
      inline int destroy()
      {
        ...
        if (inited_)
        {
          ...
          {
            YYSYS_LOG(WARN, "kv cache destroy error:ret=%d", ret);
          }
          else
          {
            inited_ = false;
      ...
      inline int clear()
      {
        ...
        if (!inited_)
        {
          YYSYS_LOG(WARN, "class not inited");
          ret = common::OB_NOT_INIT;
        }
        else if (common::OB_SUCCESS != (ret = kv_cache_.clear()))
        {
          YYSYS_LOG(WARN, "kv cache clear error:ret=%d", ret);
          ret = common::OB_ERROR;
        }
        else
        {
    ...
    public:
      ...
      ObSSTableBlockBufferHandle& operator = (const ObSSTableBlockBufferHandle& other)
      {
        ...
        if (NULL != other.block_cache_)
        {
          ...
          {
            YYSYS_LOG(WARN, "kv cache dup handle error");
            block_cache_ = NULL;
            buffer_ = NULL;
          }
        }
