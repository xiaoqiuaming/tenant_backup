...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        bool not_exit_col_ret_nop/*=false*/)
    {
      ...
      {
        YYSYS_LOG(WARN, "get_param error:%p, %ld", 
      ...
      {
        YYSYS_LOG(WARN, "initialize fail:ret=%d", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get cur param cell error:cell==NULL");
          ret = OB_ERROR;
        }
        else
        {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "load current block error:ret=%d", ret);
          }
        }
      }
          ...
          const ObCompactSSTableReader& sstable_reader)
    {
      ...
      {
        YYSYS_LOG(WARN, "alloc uncomp buffer error:uncomp_buf_=%p, uncomp_buf_size_=%ld",
            uncomp_buf_, uncomp_buf_size_);
      }
      else if (OB_SUCCESS != (ret = alloc_buffer(internal_buf_, 
              internal_buf_size_)))
      {
        YYSYS_LOG(WARN, "alloc uncomp buffer error:internal_buf_=%p, internal_buf_size_=%ld",
        ...
        const int64_t buf_size)
    {
      ...
      if (NULL == arena)
      {
        YYSYS_LOG(WARN, "GET TSI MULT:ModuleArena, " 
            "TSI_COMPACTSSTABLEV2_MODULE_ARENA_2");
    ...
    int ObCompactSSTableGetter::load_cur_block()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fectch block error:ret=%d", ret);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "init column mask error:ret=%d", ret);
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "can not reach here:ret=%d", ret);
      }
    ...
    int ObCompactSSTableGetter::fetch_block()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret && OB_SEARCH_NOT_FOUND != ret)
        {
          YYSYS_LOG(WARN, "block getter init error:ret=%d", ret);
        }
      }
      ...
      if (NULL != row_cache_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get caceh row value error:ret=%d", ret);
        }
        else if (row_cache_val.size_ <= 0 || NULL == row_cache_val.buf_)
        {
          YYSYS_LOG(WARN, "row_cache_val error:row_cache_val.size_=%ld,"
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "row cache put row error:ret=%d", ret);
          }
          ret = OB_SUCCESS;
        }
    ...
    int ObCompactSSTableGetter::init_column_mask()
    {
      ...
      if (NULL == schema)
      {
        YYSYS_LOG(WARN, "schema==NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_INVALID_ID == sstable_file_id)
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "sstable_file_id==OB_INVALID_ID");
      }
      else if (start >= cell_size || end > cell_size)
      {
        YYSYS_LOG(WARN, "cell size error:cell_size=%ld, start=%ld, end=%ld", 
            cell_size, start, end);
        ret = OB_ERROR;
      }
      else
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "get table schema error:table_id=%lu, is_rowkey=%d, column_cnt=%ld",
              table_id_, is_rowkey, column_cnt);
        }
        else
        {
          ...
          {//add rowkey column id
            ...
            {
              YYSYS_LOG(WARN, "get column indexes error:ret=%d", ret);
              break;
            }
          }
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {//query whole row
          ...
          {
            YYSYS_LOG(WARN, "get table schema error:table_id_=%lu, is_rowkey=%d, column_cnt_=%ld",
                table_id_, is_rowkey, column_cnt_);
          }
          else
          {
            for (int64_t i = 0; i < column_cnt_; i ++)
            {
              if (OB_SUCCESS != (ret = get_column_indexes_.add_column_id(
                      ...
                      def_array[i].column_id_)))
              {
                YYSYS_LOG(WARN, "add column id error:i=%ld, offset=%ld, clumn_id_=%lu",
        ...
        {//not query whole row
          ...
          for (int64_t i = 0; i < get_column_cnt; i ++)
          {
            ...
            {
              YYSYS_LOG(WARN, "invalid pram:current_column_id=%lu", current_column_id);
              ret = OB_INVALID_ARGUMENT;
            }
            ...
            {//exist
              ...
              {//rowkey column
                ...
                {
                  YYSYS_LOG(WARN, "add rowkey column id error:offset=%ld, current_column_id=%lu",
                      def->offset_, current_column_id);
                }
              }
              else
              {//rowvalue column
                if (OB_SUCCESS != (ret = get_column_indexes_.add_column_id(
                        ...
                        def->offset_, current_column_id)))
                {
                  YYSYS_LOG(WARN, "add rowvalue column id error:offset=%ld, current_column_id=%lu",
                      def->offset_, current_column_id);
                }
              }
            }
            ...
            {//not exist
              if (OB_SUCCESS != (ret = get_column_indexes_.add_column_id(
                      ...
                      0, current_column_id)))
              {
                YYSYS_LOG(ERROR, "add noexist column id error:offset=0, current_column_id=%lu",
                    current_column_id);
              }
            }
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (!bloom_filter->may_contain(cell->row_key_))
          {
            ...
            {
              YYSYS_LOG(WARN, "init column mask error");
            }
          }
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "load cur block error:ret=%d", ret);
            }
            bloomfilter_hit_ = true;
          }
        }
    ...
    int ObCompactSSTableGetter::get_row(ObRowkey*& rowkey, ObRow*& rowvalue)
    {
      ...
      if (bloomfilter_hit_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "block getter get row error:ret=%d", ret);
        }
        else
        {
          while(true)
          {//解析,得到obj数组
            ...
            {
              YYSYS_LOG(WARN, "row next cell error:ret=%d", ret);
              break;
            }
              ...
              while(true)
              {
                if (OB_SUCCESS != (ret = row->next_cell()))
                {
                  ...
                  break;
                        ...
                        row_obj, &is_row_finished)))
                {
                  YYSYS_LOG(WARN, "row get cell error:ret=%d", ret);
                }
                else
                {
                  column_objs_[rowvalue_cnt] = *row_obj;
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          while (column_cursor_ < get_column_indexes_.get_column_count())
          {
            if (OB_SUCCESS != (ret = get_column_index(column_cursor_,
                    column)))
            {
              YYSYS_LOG(WARN, "get column index error:ret=%d, column_cursor_=%lu", 
                  ret, column_cursor_);
              break;
            }
            else if (OB_SUCCESS != (ret = store_current_cell(column)))
            {
              YYSYS_LOG(WARN, "store current cell error:ret=%d", ret);
              break;
            }
            else
            {
        ...
        ObSSTableScanColumnIndexes::Column& column) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "get column indexes get column error:ret=%d", ret);
      }
        ...
        const ObSSTableScanColumnIndexes::Column& column)
    {
      ...
      if (DENSE_DENSE == row_store_type)
      {
        ...
        {
          YYSYS_LOG(WARN, "store dense column error:ret=%d", ret);
        }
      }
      else if (DENSE_SPARSE == row_store_type)
      {
        ...
        {
          YYSYS_LOG(WARN, "store sparse column error:ret=%d", ret);
        }
      }
      else
      {
        YYSYS_LOG(WARN, "can not reach here");
      }
        ...
        const ObSSTableScanColumnIndexes::Column& column)
    {
      ...
      if (OB_SUCCESS != (ret = row_desc_.add_column_desc(table_id_,
              column.id_)))
      {
        YYSYS_LOG(WARN, "row desc add column desc error:ret=%d", ret);
      }
      else if (ObSSTableScanColumnIndexes::Rowkey == column.type_)
      {
        ...
        {
          YYSYS_LOG(WARN, "row value set cell error:ret=%d", ret);
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "column.index >= column_cnt_:column.index_=%ld, column_cnt_=%ld",
        ...
        int64_t& buf_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "block cache get block error:ret=%d", ret);
        ret = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get record header error:ret=%d", ret);
        }
        else
        {
          if (header.data_length_ > uncomp_buf_size_)
          {
            YYSYS_LOG(WARN, "header.data_length_ > uncomp_buf_size_");
        ...
        if (header.is_compress())
        {
          ...
          if (NULL != dec)
          {
                    ...
                    uncomp_buf_, header.data_length_, real_size)))
            {
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "decompress error:ret=%d", ret);
              }
              else
              {
                buf = uncomp_buf_;
      ...
      {
        YYSYS_LOG(WARN, "ret!=OB_SUCCESS, NULL!=sstable_reader_,ret=%d,sstable_reader_=%p",
            ret, sstable_reader_);
      }
