...
namespace oceanbase
{
  namespace compactsstablev2
  {
        ...
        ObBlockPositionInfos& pos_info)
    {
      ...
      if (revert_handle && OB_SUCCESS != kv_cache_.revert(handle))
      {
        YYSYS_LOG(WARN, "kv cache revert error");
      }
        ...
        const uint64_t table_id, Handle& handle)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "read index record error");
        }
        else 
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "dont support compress");
            ret = OB_ERROR;
          }
        }
        else
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "read endkey recored error");
        }
        else 
        {
#ifndef _SSTABLE_NO_STAT_
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "don not support compress");
            ret = OB_ERROR;
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "check record error");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "new ObSSTableBlockIndexMgr error");
            ret = OB_MEM_OVERFLOW;
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (OB_SUCCESS != (ret = kv_cache_.put_and_fetch(
                ...
                handle, false, false)))
        {
          YYSYS_LOG(WARN, "kv cache put and fetch error");
        }
      }
      ...
      else
      {
        ...
        else
        {
          if (OB_SUCCESS != (ret = read_sstable_block_index(
                  block_index_info, block_index, table_id, handle)))
          {
            YYSYS_LOG(WARN, "read sstable block index error");
          }
          else
          {
#ifndef _SSTABLE_NO_STAT_
        ...
        const uint64_t table_id)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "ObSSTableBlockIndexCache is not init");
        ret = OB_NOT_INIT;
      }
      else if (NULL == fileinfo_cache_)
      {
        ...
        ret = OB_ERROR;
          ...
          || 0 == table_id)
      {
        YYSYS_LOG(WARN, "block index_info error");
        ret = OB_ERROR;
      }
