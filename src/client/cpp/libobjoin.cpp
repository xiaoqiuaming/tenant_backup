...
void st_ob_join::st_res_join_desc::set_res_columns(const char* new_res_columns)
{
  ...
  {
    YYSYS_LOG(ERROR, "argument is invalid, new_res_columns=NULL");
  }
  else
  {
    ...
    if (NULL == res_columns)
    {
      YYSYS_LOG(ERROR, "ob_malloc error");
    }
    else
    {
      ...
      {
        YYSYS_LOG(DEBUG, "foreign key column: %s", iter->c_str());
      }
    }
  }
}
...
void st_ob_join::st_res_join_desc::set_join_table(const char* new_join_table)
{
  ...
  {
    YYSYS_LOG(ERROR, "argument is invalid, new_res_table=NULL");
  }
  else
  {
...
void st_ob_join::st_res_join_desc::set_join_column(const char* new_join_column)
{
  ...
  {
    YYSYS_LOG(ERROR, "argument is invalid, new_join_column=NULL");
  }
  else
  {
                             ...
                             OB_RES* &final_res)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (NULL == final_res)
    {
      YYSYS_LOG(ERROR, "ob_acquire_res_st error");
      err_code = ob_errno();
    }
    else
    {
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (NULL == get_st)
    {
      YYSYS_LOG(ERROR, "ob_acquire_get_st error");
      err_code = ob_errno();
    }
  }
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "prepare_join_get_st error, err_code=%d", err_code);
    }
    else
    {
      ...
      if (NULL == join_res)
      {
        YYSYS_LOG(ERROR, "ob_exec_get error, err_code=%d", ob_errno());
        err_code = ob_errno();
      }
      else
      {
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "join_res error, err_code=%d", err_code);
        }
        else
        {
          final_res->cur_scanner = final_res->scanner.begin();
                                ...
                                sbuffer<> &rowkey)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    for (; col != res_column_list.end(); col++)
    {
        ...
        if (0 == col_found)
        {
          ...
          switch (row->cell[i].v.type)
          {
            case OB_INT_TYPE:
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(ERROR, "encode_i64 error, err=%d", err);
              }
              else
              {
                YYSYS_LOG(DEBUG, "Extrace foreign key(\"%s\" = %ld), "
                    "extend row key to %s",
            ...
            case OB_INT32_TYPE:
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(ERROR, "encode_i32 error, err=%d", err);
              }
              else
              {
                YYSYS_LOG(DEBUG, "Extrace foreign key(\"%s\" = %d), "
                    "extend row key to %s",
            ...
            case OB_VARCHAR_TYPE:
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(ERROR, "rowkey.assign error, err=%d", err);
              }
              else
              {
                YYSYS_LOG(DEBUG, "Extrace foreign key(\"%s\" = \"%s\"), "
                    "extend row key to %s",
            ...
            default:
              err = OB_ERROR;
              YYSYS_LOG(ERROR, "Unsupported join column type: %d",
                  row->cell[i].v.type);
          }
          if (OB_SUCCESS != err)
          {
      ...
      if (-1 == col_found)
      {
        YYSYS_LOG(ERROR, "Error column name: %s", col->c_str());
    ...
    if (OB_ERR_SUCCESS == err_code)
    {
      if (rowkey.length() == 0)
      {
        YYSYS_LOG(ERROR, "do not match any foreign keys");
        err_code = OB_ERR_INVALID_PARAM;
      }
    }
  }
                                ...
                                const char* join_column)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "ob_get_cell error, err_code=%d", err_code);
    }
    else
    {
      YYSYS_LOG(DEBUG, "JOIN GET Table=%s rowkey=%s Column=%s",
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    while (NULL != cur_row)
    {
      ...
      for (; join_iter != join_st->join_list.end(); join_iter++)
      {
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "extract_foreign_key error, err=%d", err_code);
        }
        else
        {
          ...
          if (OB_ERR_SUCCESS != err_code)
          {
            YYSYS_LOG(ERROR, "push_to_join_get_st error, err=%d", err_code);
          }
        }
      }
                          ...
                          const OB_VALUE* v, OB_RES* res)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    else
    {
      switch (v->type)
      {
        ...
        case OB_DOUBLE_TYPE:
          ...
          break;
        default:
          YYSYS_LOG(ERROR, "Unknown type, type=%d", v->type);
    ...
    if (OB_ERR_SUCCESS == err_code)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "add_cell error, err=%d", err);
        err_code = err_code_map(err);
        ob_set_err_with_default(err_code);
      }
      else
      {
        YYSYS_LOG(DEBUG, "add_cell Table=%.*s Row_key=%s Column=%.*s",
                                    ...
                                    OB_RES* res)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    if (NULL == cell)
    {
      YYSYS_LOG(ERROR, "join_res error");
      err_code = OB_ERR_ERROR;
      ob_set_err_with_default(err_code);
    }
    ...
    if (OB_ERR_SUCCESS == err_code)
    {
      if (NULL == cell)
      {
        ...
        ob_set_err_with_default(err_code);
          ...
          || (!cell->is_row_not_exist
              && memcmp(cell->column, join_it->join_column, cell->column_len)))
      {
        YYSYS_LOG(ERROR, "join cell not match get_st");
        YYSYS_LOG(ERROR, "join cell Table=%.*s Row_key=%s Column=%.*s Row_not_exist=%d",
            (int)cell->table_len, cell->table,
            get_display_row_key(cell->row_key, cell->row_key_len),
            (int)cell->column_len, cell->column,
            cell->is_row_not_exist);
        YYSYS_LOG(ERROR, "join st Table=%s Row_key=%s Column=%s",
            join_it->join_table,
            get_display_row_key(rowkey.ptr(), rowkey.length()),
            join_it->join_column);
        err_code = OB_ERR_ERROR;
      ...
      else
      {
        ...
        if (NULL != v)
        {
          YYSYS_LOG(DEBUG, "v.type = %d", v->type);
        }
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "append_to_res error");
        }
      }
    }
  }
                            ...
                            OB_RES* final_res)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "ob_res_seek_to_begin_row error, err_code=%d", err_code);
    }
  }
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    while (NULL != res_row && OB_ERR_SUCCESS == err_code)
    {
      ...
      if (OB_ERR_SUCCESS != err_code)
      {
        YYSYS_LOG(ERROR, "append_row_to_res error, err_code=%d", err_code);
      }
      else
      {
        OB_JOIN::JoinDescList::iterator join_iter = join_st->join_list.begin();
        for (; join_iter != join_st->join_list.end(); join_iter++)
        {
          ...
          else
          {
            ...
            if (OB_ERR_SUCCESS != err_code)
            {
              YYSYS_LOG(ERROR, "append_join_cell_to_res error, err_code=%d",
                  err_code);
            }
          }
        }
