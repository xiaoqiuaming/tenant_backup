    ...
    int32_t refresh_interval)
{
  ...
  else
  {
    ...
    initialized_ = true;
    YYSYS_LOG(INFO, "ServerCache initialized succ, rs=%s client=%p",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "update for the first time failed");
    }
  }
...
int MsList::update_(bool manual)
{
  ...
  if (!manual || yysys::CTimeUtil::getTime() - last_update_time_ > refresh_interval_)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      //mod pangtianze [MultiUPS] [merge with paxos] [conflict]
      ret = client_->send_request(rs_, OB_GET_MS_LIST, MY_VERSION, timeout, data_buff);
      //ret = client_->send_request(rs_, OB_GET_MASTER_MS_LIST, MY_VERSION, timeout, data_buff);
      //mod:e
      if (OB_SUCCESS != ret)
      {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else if (OB_SUCCESS != res.result_code_)
      {
        ...
        YYSYS_LOG(WARN, "OB_GET_MS_LIST error, result_code_=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize ms_num error, ret=%d", ret);
        }
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for (int32_t i = 0; i < ms_num && OB_SUCCESS == ret; i++)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize %dth MS error, ret=%d", i, ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "Get %dth Mergeserver %s", i, ob_server_to_string(new_ms[i]));
          int64_t reserve;
          ret = serialization::decode_vi64(data_buff.get_data(), data_buff.get_capacity(),
              data_buff.get_position(), &reserve);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "deserialize reserve field error, ret=%d", ret);
          }
        }
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      if (manual)
      {
        last_update_time_ = yysys::CTimeUtil::getTime();
      }
      has_updated = true;
    }
  }
  ...
  if (OB_SUCCESS == ret && has_updated)
  {
    ...
    {
      YYSYS_LOG(INFO, "Mergeserver List is modified, get the most updated, "
...
void MsList::list_copy_(const std::vector<ObServer> &list)
{
  ...
  for (iter = list.begin(); iter != list.end(); iter++)
  {
    ...
    YYSYS_LOG(INFO, "Add Mergeserver %s", ob_server_to_string(*iter));
  }
}
...
int UpsMaster::update_(bool manual)
{
  ...
  if (!manual || yysys::CTimeUtil::getTime() - last_update_time_ > refresh_interval_)
  {
    ...
    {
      ret = OB_NOT_INIT;
      YYSYS_LOG(ERROR, "UpsMaster has not been initialized, "
          "this should not be reached");
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        ret = res.result_code_;
        YYSYS_LOG(WARN, "OB_GET_UPS error, result_code_=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize ups_list error, ret=%d", ret);
        }
      }
    }
  ...
  if (OB_SUCCESS == ret && has_updated)
  {
    ...
    {
      ups_ = new_ups_master;
      YYSYS_LOG(INFO, "UpdateServer Master is modified, new one is %s",
          ob_server_to_string(ups_));
    }
    else
    {
      YYSYS_LOG(DEBUG, "UpdateServer Master does not change, addr=%s",
          ob_server_to_string(ups_));
    }
    rwlock_.unlock();
  }
                      ...
                      const char* user, const char* passwd)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "MsList init error, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = ups_master_.init(rs_, &client_,
            cntl_.refresh_interval)))
    {
      YYSYS_LOG(WARN, "UpsMaster init error, ret=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "init_free_req error, ret=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObTimer init error");
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "ObTimer schedule error");
      }
      else
      {
...
int ObClient::submit(int reqs_num, OB_REQ *reqs[])
{
  ...
  {
    YYSYS_LOG(WARN, "Parameter is invalid, reqs_num=%d reqs=%p",
        reqs_num, reqs);
    ret = OB_INVALID_ARGUMENT;
  }
  ...
  if (OB_SUCCESS == ret)
  {
    for (int i = 0; OB_SUCCESS == ret && i < reqs_num; ++i)
    {
      if (NULL == reqs[i])
      {
        YYSYS_LOG(WARN, "The %dth req is NULL", i);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = check_req_integrity(reqs[i])))
      {
        YYSYS_LOG(WARN, "The %dth req is not legal", i);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = dispatch_req(reqs[i])))
      {
        YYSYS_LOG(WARN, "dispatch req error, ret=%d", ret);
      }
    }
  }
                          ...
                          int64_t &num, OB_REQ* reqs[])
{
  ...
  int64_t start_time = yysys::CTimeUtil::getMonotonicTime();
  ...
  while (num < max_num)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ReqList pop_front error, ret=%d, "
    ...
    else if (NULL == i_req)
    {
      ...
      {
        if (yysys::CTimeUtil::getMonotonicTime() - start_time < timeout)
        {
          usleep(1000);
        }
        else
...
OB_RES* ObClient::scan(const ObScanParam& scan)
{
  ...
  if (NULL == res)
  {
    YYSYS_LOG(ERROR, "acquire OB_RES error");
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get_thread_data_buffer error, err_code=%d", err_code);
    }
    else
    {
      ...
      if (OB_ERR_SUCCESS != err_code)
      {
        YYSYS_LOG(ERROR, "serialize_scan error, err_code=%d", err_code);
      }
      else
      {
        int retry_counter = 0;
        ...
        {
          YYSYS_LOG(WARN, "MergeServer(%s) returned OB_ERR_NOT_MASTER, "
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "scan_ms error, MS=%s err_code=%d",
...
OB_RES* ObClient::get(const ObGetParam& get)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (OB_ERR_SUCCESS != err_code)
      {
        YYSYS_LOG(ERROR, "serialize_get error, err_code=%d", err_code);
      }
      else
      {
        int retry_counter = 0;
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "get_ms error, MS:%s err_code=%d",
              ob_server_to_string(ms_server), err_code);
        }
        else
        {
...
OB_ERR_CODE ObClient::set(const ObMutator& mut)
{
  ...
  else
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "serialize_set error, err_code=%d", err_code);
    }
    else
    {
      int retry_counter = 0;
      ...
      while (OB_ERR_NOT_MASTER == err_code
          && ++retry_counter <= OB_DEFAULT_RETRY)
      {
        YYSYS_LOG(WARN, "UpdateServer(%s) returned OB_ERR_NOT_MASTER, "
      ...
      if (OB_ERR_SUCCESS != err_code)
      {
        YYSYS_LOG(ERROR, "set_us error, US:%s err_code=%d",
...
OB_REQ* ObClient::acquire_get_req()
{
  ...
  if (NULL != req)
  {
    ...
    if (NULL == req->ob_get)
    {
      ...
      if (NULL == req->ob_get)
      {
        YYSYS_LOG(ERROR, "new OB_REQ error, NOT ENOUGH MEMORY");
...
OB_REQ* ObClient::acquire_scan_req()
{
  ...
  if (NULL != req)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "new OB_REQ error, this should not be reached");
        release_req(req, true);
        req = NULL;
      }
    }
...
void ObClient::release_req(OB_REQ* req, bool free_mem)
{
  if (NULL != req)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ReqList push_back error, err=%d, "
...
OB_GET* ObClient::acquire_get_st()
{
  ...
  if (NULL == get_st)
  {
    ...
    YYSYS_LOG(ERROR, "new OB_GET error, NOT ENOUGH MEMORY");
  }
  return get_st;
}
...
OB_SCAN* ObClient::acquire_scan_st()
{
  ...
  {
    YYSYS_LOG(ERROR, "new OB_SCAN error, NOT ENOUGH MEMORY");
    ob_set_errno(OB_ERR_MEM_NOT_ENOUGH);
    if (NULL != ob_scan)
    {
      SAFE_DELETE(ob_scan);
...
OB_SET* ObClient::acquire_set_st()
{
  ...
  {
    YYSYS_LOG(ERROR, "new OB_SET error, NOT ENOUGH MEMORY");
    ob_set_errno(OB_ERR_MEM_NOT_ENOUGH);
    if (NULL != ob_set)
    {
      SAFE_DELETE(ob_set);
...
OB_RES* ObClient::acquire_res_st()
{
  ...
  if (NULL == ob_res)
  {
    YYSYS_LOG(ERROR, "new OB_RES error, NOT ENOUGH MEMORY");
    ob_set_errno(OB_ERR_MEM_NOT_ENOUGH);
  }
  return ob_res;
}
...
OB_ERR_CODE ObClient::api_cntl(int32_t cmd, va_list args)
{
  ...
  switch (cmd)
  {
    ...
    case OB_S_TIMEOUT:
      tmp = va_arg(args, int32_t);
      atomic_exchange((uint32_t*)&(cntl_.timeout_get), static_cast<uint32_t>(tmp)); //mod support for arm platform by wangd 202106
      atomic_exchange((uint32_t*)&(cntl_.timeout_scan), static_cast<uint32_t>(tmp));//mod support for arm platform by wangd 202106
      atomic_exchange((uint32_t*)&(cntl_.timeout_set), static_cast<uint32_t>(tmp));//mod support for arm platform by wangd 202106
      break;
    case OB_S_TIMEOUT_GET:
	  //mod support for arm platform by wangd 202106
      atomic_exchange((uint32_t*)&(cntl_.timeout_get), static_cast<uint32_t>(va_arg(args, int32_t)));
      break;
    ...
    case OB_S_TIMEOUT_SCAN:
	  //mod support for arm platform by wangd 202106      
	  atomic_exchange((uint32_t*)&(cntl_.timeout_scan), static_cast<uint32_t>(va_arg(args, int32_t)));
      break;
    ...
    case OB_S_TIMEOUT_SET:
      //mod support for arm platform by wangd 202106      
	  atomic_exchange((uint32_t*)&(cntl_.timeout_set), static_cast<uint32_t>(va_arg(args, int32_t)));
      break;
    ...
    case OB_S_READ_MODE:      
	  //mod support for arm platform by wangd 202106      
	  atomic_exchange((uint32_t*)&(cntl_.read_mode), static_cast<uint32_t>(va_arg(args, int32_t)));
      break;
    case OB_G_READ_MODE:
    ...
    case OB_S_REFRESH_INTERVAL:      
	  ...
	  atomic_exchange((uint32_t*)&(cntl_.refresh_interval),
	      static_cast<uint32_t>(va_arg(args, int32_t)));
      break;
...
int ObClient::get_ms_list(ObScanner &scanner)
{
  ...
  for (; iter != ms_list.end() && OB_SUCCESS == ret; iter++)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "add MergeServer to result error, ret=%d", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "get_ms_list add a ms %s:%d", buf, iter->get_port());
    }
  }
...
int ObClient::init_reqs()
{
  ...
  if (NULL == reqs_)
  {
    YYSYS_LOG(ERROR, "new req_list_node error, NOT ENOUGH MEMORY");
    ret = OB_ERROR;
  }
  else
  {
...
OB_REQ* ObClient::acquire_req()
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(ERROR, "ReqList pop_front error, err=%d, "
        "this should not happened", err);
  }
  else if (NULL == i_req)
  {
    YYSYS_LOG(INFO, "no req left");
  }
  else
  {
    req = (OB_REQ*)i_req;
...
int ObClient::submit_set(const common::ObServer &server, OB_REQ *req)
{
  ...
  if (NULL == req || NULL == req->ob_set)
  {
    ...
    YYSYS_LOG(WARN, "Parameter is invalid, req=%p ob_set=%p", req, ob_set);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObGetParam serialize error, err=%d", ret);
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to send request to server=%s, ret=%d",
...
int ObClient::submit_scan(const common::ObServer &server, OB_REQ *req)
{
  ...
  if (NULL == req || NULL == req->ob_scan)
  {
    ...
    YYSYS_LOG(WARN, "Parameter is invalid, req=%p ob_scan=%p", req, ob_scan);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObScanParam serialize error, err=%d", ret);
    }
...
int ObClient::submit_get(const common::ObServer &server, OB_REQ *req)
{
  ...
  if (NULL == req || NULL == req->ob_get)
  {
    ...
    YYSYS_LOG(WARN, "Parameter is invalid, req=%p ob_get=%p", req, ob_get);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
...
yynet::IPacketHandler::HPRetCode
    ObClient::handlePacket(yynet::Packet* packet, void * args)
{
  OB_ERR_CODE err_code = OB_ERR_SUCCESS;
  ...
  if (NULL != packet)
  {
    ...
    {
      err_code = OB_ERR_ERROR;
      YYSYS_LOG(ERROR, "Unexpected: receive a packet with "
  ...
  else
  {
    ...
    if (NULL == creq)
    {
      YYSYS_LOG(ERROR, "Unexpected: receive a NULL packet with "
  ...
  if (OB_ERR_SUCCESS == err_code && REGULAR_PACKET == packet_type)
  {
    ...
    {
      YYSYS_LOG(WARN, "response packet deserialize error, err=%d", err);
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "deserialize result_code failed: "
          ...
          && (OB_OP_GET == creq->opcode || OB_OP_SCAN == creq->opcode))
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "deserialize scanner from buff failed, "
  ...
  {
    yynet::ControlPacket *cpkt = static_cast<yynet::ControlPacket*>(packet);
    int cmd = cpkt->getCommand();
    const char *cmd_str = NULL;
    switch (cmd)
    {
      case yynet::ControlPacket::CMD_BAD_PACKET:
        cmd_str = "BAD_PACKET";
        break;
      case yynet::ControlPacket::CMD_TIMEOUT_PACKET:
        cmd_str = "TIMEOUT_PACKET";
        break;
      case yynet::ControlPacket::CMD_DISCONN_PACKET:
        cmd_str = "DISCONN_PACKET";
        break;
      default:
        cmd_str = "UNKNOWN_PACKET";
    }
    YYSYS_LOG(WARN, "control packet: %s", cmd_str);
  }

  return yynet::IPacketHandler::FREE_CHANNEL;
}
                           ...
                           OB_REQ *req)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "packet serialize error");
      packet->free();
      packet = NULL;
    }
    else
    {
      if (NULL != req)
      {
        ((ObInnerReq*)req)->submit_time = yysys::CTimeUtil::getMonotonicTime();
      }
      ...
      if (!send_ok)
      {
        ...
        YYSYS_LOG(INFO, "cannot send packet, disconnected or queue is full");
        packet->free();
      }
    }
  }
...
int ObClient::check_req_integrity(OB_REQ *req)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "req's opcode is not supported, opcode=%d", req->opcode);
      ret = OB_ERROR;
    }
    else if (req->timeout == 0)
    {
      YYSYS_LOG(WARN, "req's timeout cannot be zero");
      ret = OB_ERROR;
    }
    else
    {
      switch (req->opcode)
      {
        case OB_OP_GET:
          if (NULL == req->ob_get)
          {
            YYSYS_LOG(WARN, "req's ob_get is NULL");
            ret = OB_ERROR;
          }
          break;
        case OB_OP_SCAN:
          if (NULL == req->ob_scan)
          {
            YYSYS_LOG(WARN, "req's ob_scan is NULL");
            ret = OB_ERROR;
          }
          break;
        case OB_OP_SET:
          if (NULL == req->ob_set)
          {
            YYSYS_LOG(WARN, "req's ob_set is NULL");
            ret = OB_ERROR;
          }
          break;
      }
  ...
  else
  {
    ...
    switch (req->opcode)
    {
      case OB_OP_GET:
        ...
        {
          YYSYS_LOG(WARN, "get_ms error, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = submit_get(server, req)))
        {
          YYSYS_LOG(WARN, "submit_get req error, ret=%d", ret);
        }
        break;
      case OB_OP_SCAN:
        if (OB_SUCCESS != (ret = get_ms(req, server)))
        ...
        {
          YYSYS_LOG(WARN, "submit_scan req error, ret=%d", ret);
        }
        break;
      case OB_OP_SET:
        if (OB_SUCCESS != (ret = get_us(req, server)))
        {
          YYSYS_LOG(WARN, "get_us error, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = submit_set(server, req)))
        {
          YYSYS_LOG(WARN, "submit_set req error, ret=%d", ret);
        }
        break;
      default:
        YYSYS_LOG(WARN, "req's opcode is not supported, opcode=%d",
            req->opcode);
        ret = OB_ERROR;
    }
  }
                                     ...
                                     ObDataBuffer& data_buff)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObScanParam serialize error, ret=%d", ret);
    err_code = OB_ERR_ERROR;
  }
  else
  {
    YYSYS_LOG(DEBUG, "scan ObScanParam table_id=%lu table_name=%.*s",
                                       ...
                                       ObScanner& scanner)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "deserialize result_code failed, ret=%d", ret);
    err_code = OB_ERR_ERROR;
  }
  else if (OB_SUCCESS == res_code.result_code_)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize scanner failed, ret=%d", ret);
      err_code = OB_ERR_ERROR;
    }
  }
                             ...
                             OB_RES::ScannerList& scanner)
{
  ...
  else
  {
    ...
    if (OB_ERR_SUCCESS == err_code)
    {
      ...
      if (OB_ERR_SUCCESS == err_code)
      {
        YYSYS_LOG(DEBUG, "get_ms receives a scanner, row_num=%ld",
                              ...
                              OB_RES::ScannerList& scanner)
{
  ...
  else
  {
    ...
    if (OB_ERR_SUCCESS == err_code)
    {
      ...
      if (OB_ERR_SUCCESS == err_code)
      {
        YYSYS_LOG(DEBUG, "scan_ms receives a scanner, row_num=%ld",
            scanner.back().get_row_num());
      }
    }
  }
...
OB_ERR_CODE ObClient::clear_active_memtable()
{
  ...
  else
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "clear_active_memtable error, err_code=%d", err_code);
    }
  }
...
OB_ERR_CODE ObClient::get_thread_data_buffer(ObDataBuffer &data_buffer)
{
  ...
  {
    ret = OB_ERR_ERROR;
    YYSYS_LOG(ERROR, "thread buffer return null, ret=%d", ret);
  }
  else
  {
