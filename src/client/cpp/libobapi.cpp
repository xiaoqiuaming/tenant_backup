...
ObAggregateFuncType get_ob_aggregation_type(OB_AGGREGATION_TYPE o)
{
  switch (o)
  {
    ...
    case OB_MIN:
      return MIN;
    case OB_LISTAGG:
      return LISTAGG;//add gaojt [ListAgg][JHOBv0.1]20150104
    default:
      return AGG_FUNC_MIN;
  }
}
    ...
    if (ob_res->cur_scanner != ob_res->scanner.end())
    {
      if (OB_SUCCESS == err)
      {
          ...
          if (ObExtendType == cell_info->value_.get_type())
          {
            switch (cell_info->value_.get_ext())
            {
              ...
              default:
                ...
                YYSYS_LOG(ERROR, "ObScanner get an unknow extend type cell, "
          ...
          else
          {
            switch (cell_info->value_.get_type())
            {
              ...
              default:
                err_code = OB_ERR_ERROR;
                ob_set_err_with_default(err_code);
                YYSYS_LOG(ERROR, "ObScanner get an unknow type cell, "
        ...
        else
        {
          ...
          YYSYS_LOG(ERROR, "ObScanner get_cell returned error, err=%d", err);
        }
      }
      ...
      else
      {
        ret = NULL;
        ob_set_err_with_default(OB_ERR_ERROR);
        YYSYS_LOG(ERROR, "ObScanner next_cell returned error, err=%d", err);
      }
    }
    else
    {
...
OB_ROW* ob_fetch_row(OB_RES* ob_res)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "reach end of a scanner");
      ob_res->cur_scanner++;
    }
    if (ob_res->cur_scanner != ob_res->scanner.end())
    {
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ret = NULL;
          ob_set_err_with_default(OB_ERR_ERROR);
          YYSYS_LOG(ERROR, "ObScanner get_row returned error, err=%d", err);
        }
      }
      else if (OB_ITER_END == err)
      {
      ...
      else
      {
        ret = NULL;
        ob_set_err_with_default(OB_ERR_ERROR);
        YYSYS_LOG(ERROR, "ObScanner next_row returned error, err=%d", err);
      }
    }
  }
  else
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
        ...
        OB_ERR_SUCCESS == err_code && i != ob_res->scanner.end(); i++)
    {
      ...
      if (OB_SUCCESS != err)
      {
        err_code = OB_ERR_ERROR;
        ob_set_err_with_default(err_code);
        YYSYS_LOG(ERROR, "ObScanner reset_iter returned error, err=%d", err);
      }
    }
    ...
    const char* column)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObGetParam add_cell returned error, "
    ...
    int is_add)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObMutator update returned error, "
          "err=%d table=%s row_key=%s column=%s",
          err, table, get_display_row_key(row_key, row_key_len), column);
      err_code = err_code_map(err);
      ob_set_err_with_default(err_code);
    ...
    const OB_VALUE* v)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObMutator insert returned error, "
          "err=%d table=%s row_key=%s column=%s",
          err, table, get_display_row_key(row_key, row_key_len), column);
      err_code = err_code_map(err);
      ob_set_err_with_default(err_code);
    ...
    const char* row_key, int64_t row_key_len)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObMutator del_row returned error, "
    ...
    const OB_VALUE* v)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObUpdateCondition add_cond returned error, "
    ...
    int is_exist)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObUpdateCondition update returned error, "
    ...
    int is_return)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObScanParam add_column returned error, "
    ...
    const char* expression)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObScanParam add_where_cond returned error, "
    ...
    const OB_VALUE* v)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObSimpleFilter add_cond returned error, "
    ...
    OB_ORDER order)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObScanParam add_orderby_column returned error, "
    ...
    int64_t count)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObScanParam set_limit_info returned error, "
    ...
    int64_t reserved_count)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObScanParam set_topk_precision returned error, "
    ...
    int is_return)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObGroupByParam add_groupby_column returned error, "
          "err=%d column=%s column_len=%d is_return=%d",
          err, column, column_len, is_return);
      err_code = err_code_map(err);
      ob_set_err_with_default(err_code);
    ...
    int is_return)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObGroupByParam add_aggregate_column returned error, "
    ...
    int is_return)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObGroupByParam add_complex_column returned error, "
    ...
    int is_return)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObGroupByParam add_return_column returned error, "
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    else
    {
      if (ob_fetch_row_num(scan_res) > 0 && ob_scan->join_st.join_list.size() > 0)
      {
        ...
        if (OB_ERR_SUCCESS != err_code)
        {
          YYSYS_LOG(ERROR, "ob_exec_res_join error");
          if (NULL != res)
          {
...
OB_ERR_CODE ob_exec_set(OB* ob, OB_SET* ob_set)
{
  ...
  YYSYS_LOG(DEBUG, "ob_exec_set err_code=%d", err_code);
  return err_code;
}
...
OB_ERR_CODE ob_api_cntl(OB* ob, int32_t cmd, ...)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "api_cntl error");
    }
    va_end(ap);
  }
  return err_code;
...
OB_ERR_CODE ob_debug(OB* ob, int32_t cmd, ...)
{
  ...
  if (OB_ERR_SUCCESS == err_code)
  {
    ...
    if (OB_ERR_SUCCESS != err_code)
    {
      YYSYS_LOG(ERROR, "ob_debug error");
    }
    va_end(ap);
  }
  return err_code;
