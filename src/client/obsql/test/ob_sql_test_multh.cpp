...
int prepare_env()
{
  ...
  {
    YYSYS_LOG(INFO, "connect to mysql error %s\n", mysql_error(mysql));
  }
  else
  {
    int ret = 10;
    ret = mysql_ping(mysql);
    YYSYS_LOG(INFO, "ping ret is %d\n", ret);
    mysql_close(mysql);
    mysql = mysql_init(NULL);
    if (mysql_query(mysql, "drop table if exists test"))
    {
      YYSYS_LOG(INFO, "DROP TABLE failed\n");
      YYSYS_LOG(INFO, "%s\n", mysql_error(mysql));
      exit(0);
    }
    ...
    {
      YYSYS_LOG(INFO, "CREATE TABLE failed\n");
      YYSYS_LOG(INFO, "%s\n", mysql_error(mysql));
      exit(0);
    }
    mysql_close(mysql);
    ...
    {
      YYSYS_LOG(INFO, "INSERT TABLE failed\n");
      YYSYS_LOG(INFO, "%s\n", mysql_error(mysql));
      exit(0);
    }
    mysql_close(mysql);
...
static void * task(void *arg)
{
  ...
  if (!stmt)
  {
    YYSYS_LOG(INFO, " mysql_stmt_init(), out of memory\n");
    exit(0);
  }

  if (mysql_stmt_prepare(stmt, SELECT_SAMPLE, (unsigned long)strlen(SELECT_SAMPLE)))
  {
    YYSYS_LOG(INFO, " mysql_stmt_prepare(), SELECT failed\n");
    YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
    exit(0);
  }
  YYSYS_LOG(INFO, "prepare statement ok");
  param_count= mysql_stmt_param_count(stmt);
  YYSYS_LOG(INFO, " total parameters in SELECT: %lu", param_count);

  if (param_count != 1) 
  {
    YYSYS_LOG(INFO, " invalid parameter count returned by MySQL\n");
    exit(0);
  }
  ...
  YYSYS_LOG(INFO, " total columns in SELECT statement: %d", column_count);

  if (column_count != 3) 
  {
    YYSYS_LOG(INFO, " invalid column count returned by MySQL\n");
    exit(0);
  }
  ...
  {
    YYSYS_LOG(INFO, " mysql_stmt_bind_param() failed\n");
    YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
    exit(0);
  }
  ...
  while(idx < 10000)
  {
    ...
    {
      YYSYS_LOG(INFO, " mysql_stmt_execute(), failed\n");
      YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
      exit(0);
    }
    ...
    {
      YYSYS_LOG(INFO, " mysql_stmt_bind_result() failed\n");
      YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
      exit(0);
    }
    ...
    {
      YYSYS_LOG(INFO, " mysql_stmt_store_result() failed\n");
      YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
      exit(0);
    }
    ...
    {
      row_count++;
      YYSYS_LOG(INFO, "  row %d\n", row_count);

      YYSYS_LOG(INFO, "   column1 (integer)  : ");
      if (is_null[0])
        YYSYS_LOG(INFO, " NULL\n");
      else
        YYSYS_LOG(INFO, " %ld(%ld)\n", (long int)int_data[0], length[0]);

      YYSYS_LOG(INFO, "   column2 (integer)   : ");
      if (is_null[1])
        YYSYS_LOG(INFO, " NULL\n");
      else
        YYSYS_LOG(INFO, " %ld(%ld)\n", (long int)int_data[1], length[1]);

      YYSYS_LOG(INFO, "   column3 (integer) : ");
      if (is_null[2])
        YYSYS_LOG(INFO, " NULL\n");
      else
    ...
    YYSYS_LOG(INFO, " total rows fetched: %d\n", row_count);
    if (row_count != 1)
    {
      YYSYS_LOG(INFO, " MySQL failed to return all rows\n");
      exit(0);
    }
  ...
  {
    YYSYS_LOG(INFO, " failed while closing the statement\n");
    YYSYS_LOG(INFO, " %s\n", mysql_stmt_error(stmt));
    exit(0);
  }
  mysql_close(mysql);
...
int main(int argc, char* argv[])
{
  ...
  int idx = 0;
  prepare_env();
  YYSYS_LOG(INFO, "table test created, 3 records was inserted");
  pthread_t pids[20];
  for(; idx < 20; ++idx)
  {
    pthread_create(pids+idx, NULL, task, NULL);
    YYSYS_LOG(DEBUG, "start %d thread", idx);
  }
  idx = 0;
  for(; idx < 20; ++idx)
  {
