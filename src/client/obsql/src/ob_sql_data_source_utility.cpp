...
//  {
//    YYSYS_LOG(ERROR, "invalid argument dest is %p, src is %p", dest, src);
//    ret = OB_SQL_ERROR;
//  }
//  else
//  {
...
int create_real_connection(ObDataSource *pool)
{
  ...
  if (NULL == pool)
  {
    YYSYS_LOG(ERROR, "invalid data source argument: pool=%p", pool);
    ret = OB_SQL_ERROR;
  }
  else
  {
    ...
    if (NULL == node)
    {
      YYSYS_LOG(ERROR, "alloc mem for ObSQLConn list node failed");
      ret = OB_SQL_ERROR;
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR,"can not get server ip address server(%u, %u)", pool->server_.ip_, pool->server_.port_);
      }
      else if (NULL == (conn->mysql_ = CALL_REAL(mysql_init, conn->mysql_)))
      {
        YYSYS_LOG(ERROR, "init mysql handler failed");
        ret = OB_SQL_ERROR;
      }
      else if (OB_SQL_SUCCESS != (ret = set_mysql_options(conn->mysql_)))
      {
        YYSYS_LOG(ERROR, "set_mysql_options fail, ret=%d, mysql=%p", ret, conn->mysql_);
      }
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "create new connection success: MS=%s, node=%p, conn=%p, mysql=%p, ds=%p, free=%d, used=%d",
  ...
  if (OB_SQL_SUCCESS != ret)
  {
    ...
    YYSYS_LOG(ERROR, "create new connection fail, ds=%p, MS=%s",
...
int add_data_source(int32_t conns, ObServerInfo server, ObClusterInfo *cluster)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid arguments: conns=%d, cluster=%p", conns, cluster);
    ret = OB_SQL_INVALID_ARGUMENT;
  }
  else
  {
    YYSYS_LOG(INFO, "add data source MS=%s: cluster={id=%d,addr=%s}, MS NUM=%d, conns=%d",
    ...
    if (NULL == ds)
    {
      YYSYS_LOG(ERROR, "alloc memory for ObDataSource fail");
      ret = OB_SQL_ERROR;
    }
    else
    {
      ...
      for (int32_t index = 0; index < conns && OB_SQL_SUCCESS == ret; ++index)
      {
        ...
        if (OB_SQL_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "create real connection to %s failed", get_server_str(&server));
      ...
      if (OB_SQL_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(DEBUG, "after add data source MS=%s: cluster={id=%d,addr=%s}, MS NUM=%d, conns=%d",
            get_server_str(&server), cluster->cluster_id_,
...
void recycle_data_source(ObDataSource *ds)
{
  if (NULL != ds)
  {
    ...
    YYSYS_LOG(INFO, "recycle data source: ds=%p, MS=%s, free=%d, used=%d",
...
void destroy_data_source(ObDataSource *ds)
{
  if (NULL != ds)
  {
    ...
    YYSYS_LOG(INFO, "destroy data source: ds=%p, MS=%s, free=%d, used=%d",
        ds, get_server_str(&ds->server_), ds->conn_list_.free_list_.size_, ds->conn_list_.used_list_.size_);
...
{
  YYSYS_LOG(INFO, "delete data source: cluster=%p, index=%d", cluster, sindex);
  ...
  if (NULL != cluster && 0 <= sindex && sindex < cluster->size_)
  {
    ...
    if (NULL == ds)
    {
      YYSYS_LOG(WARN, "cluster %p has NULL ObDataSource, cluster={id=%d,addr=%s}, MS NUM=%d",
          cluster, cluster->cluster_id_, get_server_str(&cluster->cluster_addr_), cluster->size_);
    }
    else
    {
      YYSYS_LOG(INFO, "delete data source: cluster={id=%d,addr=%s}, MS=%s, MS NUM=%d",
    ...
    YYSYS_LOG(DEBUG, "after delete_data_source: cluster=[%d,%s], size=%d",
...
int update_data_source(ObDataSource &ds)
{
  ...
  {
    YYSYS_LOG(INFO, "MS=%s conn size (used=%d,free=%d) < max_conn_size %d, try to create new connection",
    ...
    for (int index = conn_size; index < g_config_using->max_conn_size_; index++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "create real connection to %s failed", get_server_str(&ds.server_));
        break;
      }
    }
    ...
    if (OB_SQL_SUCCESS == ret)
    {
      YYSYS_LOG(INFO, "Add new connection to %s success, used=%d, free=%d, max_conn_size=%d. Delete it from black list",
          get_server_str(&ds.server_), ds.conn_list_.used_list_.size_,
          ds.conn_list_.free_list_.size_, g_config_using->max_conn_size_);
