			      ...
			      char **groups __attribute__((unused)))
{
  ...
  {
    YYSYS_LOG(ERROR, "mysql_server_init fail: libobsql has not been inited");
    return 1;//error
  }
  else
  {
...
my_bool STDCALL MYSQL_FUNC(mysql_thread_init)(void)
{
  ...
  {
    YYSYS_LOG(ERROR, "mysql_thread_init fail: libobsql has not been inited");
    ret = 0;
  }
...
MYSQL_TYPE * STDCALL MYSQL_FUNC(mysql_init)(MYSQL_TYPE *mysql)
{
  ...
  YYSYS_LOG(DEBUG, "mysql init called");
  if (0 == g_inited) //not inited
  {
    YYSYS_LOG(INFO, "not inited");
    return NULL;
  }
  ...
  {
    YYSYS_LOG(ERROR, "mysql_server_init fail, real_mysql_server_init=%p", g_func_set.real_mysql_server_init);
    flag = OB_SQL_ERROR;
  }
  else
  {
    if (NULL == mysql)
    {
      ...
      if (NULL == mysql)
      {
        YYSYS_LOG(ERROR, "alloc mem for MYSQL failed");
        flag = OB_SQL_ERROR;
      }
      else
      {
...
MYSQL_TYPE * STDCALL MYSQL_FUNC(mysql_real_connect)(MYSQL_TYPE *mysql,
                                                    const char *host,
                                                    const char *user,
                                                    const char *passwd,
                                                    const char *db,
                                                    ...
                                                    unsigned long clientflag)
{
  ...
  if (NULL != mysql && OB_SQL_MAGIC == ((ObSQLMySQL*)mysql)->magic_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "fail to connect to \"%s:%u\", can not find a valid connection, ob_mysql=%p", host, port, mysql);
      ret = NULL;
    }
    else
    {
...
static void clear_connection_(ObSQLMySQL *ob_mysql)
{
  ...
  if (ob_mysql->rconn_  != ob_mysql->wconn_)
  {
    if (NULL != ob_mysql->rconn_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "release_conn fail, ob_mysql=%p, rconn=%p, MS=%s, node=%p, "
    ...
    if (NULL != ob_mysql->wconn_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "release_conn fail, ob_mysql=%p, wconn=%p, MS=%s, node=%p, "
            "real mysql=%p, pool=%p, is_conn_pool_valid=%s",
...
static ObClusterInfo *get_next_slave_cluster_(ObClusterInfo *base_cluster, int32_t &index)
{
  ...
  YYSYS_LOG(DEBUG, "GET_SLAVE_CLUSTER: BASE={id=%u,addr=%s,type=%d}, index=%d",
  ...
  if (NULL == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "GET_SLAVE_CLUSTER: ret=NULL, index=%d", index);
  }
  else
  {
    YYSYS_LOG(DEBUG, "GET_SLAVE_CLUSTER: ret={id=%u,addr=%s,type=%d}, index=%d",
...
static ObSQLConn *choose_conn_(ObSQLMySQL *ob_mysql, const char *q, unsigned long length)
{
  ...
  if (0 == pthread_rwlock_rdlock(&g_config_rwlock))
  {
    ...
    {
      YYSYS_LOG(ERROR, "select_cluster fail, no valid cluster avaliable");
    }
    ...
    else if (NULL == (ret = acquire_conn(cluster, q, length, ob_mysql)) && 1 < g_group_ds.size_)
    {
      ...
      YYSYS_LOG(INFO, "CHOOSE_CLUSTER: BASE={id=%u,addr=%s,type=%d}, SIZE=%d",
      ...
      if (2 == g_group_ds.size_)
      {
        ...
        if (NULL != cluster)
        {
          YYSYS_LOG(INFO, "CHOOSE_CLUSTER: NEXT={id=%u,addr=%s,type=%d}",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "CHOOSE_CLUSTER: SLAVE={id=%u,addr=%s,type=%d}, index=%d",
        ...
        if (NULL == ret && MASTER_CLUSTER != base_cluster->cluster_type_)
        {
          ...
          if (NULL != cluster)
          {
            YYSYS_LOG(INFO, "CHOOSE_CLUSTER: MASTER={id=%u,addr=%s,type=%d}",
                cluster->cluster_id_, get_server_str(&cluster->cluster_addr_), cluster->cluster_type_);
            ret = acquire_conn(cluster, q, length, ob_mysql);
          }
          else
          {
            YYSYS_LOG(WARN, "CHOOSE_CLUSTER: no master cluster available, cluster_size=%d", g_group_ds.size_);
          }
        }
      }
    }
    ...
    if (NULL == ret)
    {
      YYSYS_LOG(WARN, "fail to choose connection from all clusters, cluster size=%d", g_config_using->cluster_size_);
    }
  ...
  {
    YYSYS_LOG(ERROR, "fail to choose connection: pthread_rwlock_rdlock failed on g_config_rwlock");
  }
...
static MYSQL* select_connection(ObSQLMySQL *ob_mysql, const char *q, unsigned long length, ObSQLType *stype)
{
  ...
  YYSYS_LOG(DEBUG, "select_connection: ob_mysql=%p, conn=%p, wconn=%p, rconn=%p, MS=%s",
  ...
  if (! (ob_mysql->has_stmt_))
  {
    ...
    if (OB_SQL_BEGIN_TRANSACTION == sql_type)
    {
      ...
      YYSYS_LOG(INFO, "set consistence %p", ob_mysql);
    }
  ...
  {
    YYSYS_LOG(DEBUG, "query to master cluster");
  ...
  {
    YYSYS_LOG(DEBUG, "ordinary query");
  ...
  YYSYS_LOG(DEBUG, "after select_connection: real_mysql=%p, ob_mysql=%p, conn=%p, wconn=%p, rconn=%p, MS=%s",
...
{
  YYSYS_LOG(DEBUG, "mysql_send_query call mysql_real_query");
...
int STDCALL MYSQL_FUNC(mysql_real_query)(MYSQL_TYPE *mysql, const char *q, unsigned long length)
{
  ...
  MYSQL* real_mysql = NULL;
  YYSYS_LOG(DEBUG, "mysql_real_query: mysql=%p, query is %s", mysql, q);
  if (NULL != mysql && OB_SQL_MAGIC == ((ObSQLMySQL*)mysql)->magic_)
  {
    ...
    if (NULL != real_mysql)
    {
      ...
      if (0 != ret && need_retry((ObSQLMySQL*)mysql))
      {
        ...
        YYSYS_LOG(DEBUG, "mysql_real_query fail, ret=%d, retry. mysql=%p, query=%s", ret, mysql, q);
        real_mysql = select_connection((ObSQLMySQL *)mysql, q, length, &stype);
        if (NULL != real_mysql)
        {
          ret = CALL_REAL(mysql_real_query, real_mysql, q, length);
        ...
        {
          YYSYS_LOG(WARN, "can not find a valid connection on retry");
        }
      }
    }
    else
    {
      YYSYS_LOG(WARN, "can not find a valid connection");
      ret = OB_SQL_ERROR;
    }
        ...
        OB_SQL_END_TRANSACTION == stype)
    {
      ...
      YYSYS_LOG(INFO, "unset consistence %p", mysql);
    }
...
MYSQL_STMT * STDCALL MYSQL_FUNC(mysql_stmt_init)(MYSQL_TYPE *mysql)
{
  ...
  if (NULL != mysql && OB_SQL_MAGIC == ((ObSQLMySQL*)mysql)->magic_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "mysql_stmt_init fail, no valid connection, ob_mysql=%p, conn=%p, wconn=%p, rconn=%p",
          ob_mysql, ob_mysql->conn_, ob_mysql->wconn_, ob_mysql->rconn_);
    }
  }
  else
...
int STDCALL MYSQL_FUNC(mysql_stmt_prepare)(MYSQL_STMT *stmt, const char *query, unsigned long length)
{
  ...
  {
    YYSYS_LOG(DEBUG, "stmt prepare mysql is %p query is %s", stmt->mysql, query);
  }
...
int STDCALL MYSQL_FUNC(mysql_stmt_execute)(MYSQL_STMT *stmt)
{
  if (NULL != stmt)
  {
    YYSYS_LOG(DEBUG, "stmt execute mysql is %p", stmt->mysql);
  }
...
my_bool STDCALL MYSQL_FUNC(mysql_stmt_close)(MYSQL_STMT * stmt)
{
  ...
  if (NULL != stmt)
  {
    YYSYS_LOG(DEBUG, "before stmt close: mysql=%p, errno=%d, sqlstate=%s, error=%s",
  ...
  if (NULL != real_mysql && 0 != errcode)
  {
    ...
    YYSYS_LOG(DEBUG, "after stmt close: mysql=%p, errno=%d, sqlstate=%s, error=%s",
...
void STDCALL MYSQL_FUNC(mysql_close)(MYSQL_TYPE *mysql)
{
  if (NULL != mysql && OB_SQL_MAGIC == ((ObSQLMySQL*)mysql)->magic_)
  {
    ...
    YYSYS_LOG(DEBUG, "mysql close: mysql=%p, mysql->conn_=%p, real mysql=%p, errno=%d, error=%s",
...
void MYSQL_FUNC(mysql_set_local_infile_handler)(MYSQL_TYPE *mysql,
