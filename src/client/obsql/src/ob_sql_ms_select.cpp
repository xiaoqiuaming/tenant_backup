...
static int init_ms_table()
{
  ...
  int16_t index = 0;
  YYSYS_LOG(DEBUG, "g_config_using cluster size is %d", g_config_using->cluster_size_);
...
static int update_ms_table()
{
  ...
  for (index = 0; index < g_config_using->cluster_size_; ++index)
  {
    ...
    for (cindex = 0; cindex < g_group_ds.size_; ++cindex)
    {
      ...
      if (NULL == scluster)
      {
        YYSYS_LOG(ERROR, "group data source has NULL cluster at index=%d, size=%d",
      ...
      {
        YYSYS_LOG(DEBUG, "update ms select table: cluster=%p, cluster id=%d, MS NUM=%d",
            scluster, scluster->cluster_id_, scluster->size_);

        for (sindex = 0; sindex < scluster->size_ ; ++sindex)
        {
          ssource = scluster->dslist_[sindex];
          YYSYS_LOG(DEBUG, "cluster[%d] ds[%d]=%p", scluster->cluster_id_, sindex, ssource);
  ...
  for (index = 0; index < g_config_using->cluster_size_; ++index)
  {
    YYSYS_LOG(DEBUG, "dump cluster ms table %d", index);
    ObSQLClusterConfig *config = g_config_using->clusters_ + index;
    for (int64_t num = 0; num < config->table_->slot_num_; ++num)
    {
      YYSYS_LOG(DEBUG, "cluster[%d]: offset=%ld, hash=%u, ds=%p",
...
int update_ms_select_table()
{
  ...
  {
    YYSYS_LOG(ERROR, "reinit ms select table failed");
  }
  return ret;
}
...
void destroy_ms_select_table(ObSQLGlobalConfig *cconfig)
{
  if (NULL != cconfig)
  {
    for (int16_t index = 0; index < cconfig->cluster_size_; ++index)
    {
      ...
      if (NULL != config->table_)
      {
        YYSYS_LOG(INFO, "destroy cluster(%s) ms select table in config %p",
...
ObDataSource * find_ds_by_val(ObSQLHashItem *first, int64_t num, const uint32_t val)
{
  ...
  YYSYS_LOG(INFO, "hash value is %u", val);
  if (NULL == item)
  {
    YYSYS_LOG(INFO, "item value is %u", first->hashvalue_);
    ds = first->server_;
  }
  else
  {
    ...
    {
      YYSYS_LOG(INFO, "item value is %u", item->hashvalue_);
      ds = item->server_;
    }
  }

  YYSYS_LOG(INFO, "data source is %p, %s", ds, get_server_str(&ds->server_));
  return ds;
}
#endif
