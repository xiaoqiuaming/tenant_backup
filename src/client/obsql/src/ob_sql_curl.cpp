...
int get_rs_list(const char *url)
{
  ...
  while(!url_feof(handle))
  {
    ...
    if (OB_SQL_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "There are more than %d cluster was added", OB_SQL_MAX_CLUSTER_NUM);
        break;
      }
      g_rslist[g_rsnum].ip_ = trans_ip_to_int(ip);
      g_rslist[g_rsnum].port_ = port;
      YYSYS_LOG(DEBUG, "get rootserver %s", get_server_str(g_rslist + g_rsnum));
      g_rsnum++;
    }
    else
    {
      YYSYS_LOG(WARN, "parse server ip/port failed url(%s)", url);
    }
  }
  //rsnum = num;
  url_fclose(handle);
