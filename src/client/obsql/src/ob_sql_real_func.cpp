...
int init_func_set(FuncSet *set)
{
  ...
  if (NULL != handle)
  {
    YYSYS_LOG(WARN, "init_func_set twice");
    ret = OB_SQL_ERROR;
  }
  else
  {
    ...
    if (!handle)
    {
      YYSYS_LOG(ERROR, "dlopen \"%s\" fail, %s", LIBMYSQLCLIENT_FILE, dlerror());
      ret = OB_SQL_ERROR;
    }
  ...
  if (OB_SQL_CLIENT_VERSION_55_MIN < native_client_version)
  {
    ...
    if (0 == ret)
    {
      set->real_mysql_set_local_infile_handler =
              reinterpret_cast<void (*)(MYSQL *, int (*)(void **, const char *, void *), int (*)(void *, char *, unsigned int), void (*)(void *), int (*)(void *, char*, unsigned int), void *)>(dlsym(
                handle,
  ...
  {
    YYSYS_LOG(INFO, "load MYSQLCLIENT \"%s\" success", LIBMYSQLCLIENT_FILE);
  }
...
void destroy_func_set(FuncSet *set)
{
  if (NULL != handle)
  {
    YYSYS_LOG(INFO, "unload MYSQLCLIENT \"%s\" begin", LIBMYSQLCLIENT_FILE);
    (void)memset(set, 0, sizeof(FuncSet));
    dlclose(handle);
    handle = NULL;
    YYSYS_LOG(INFO, "unload MYSQLCLIENT \"%s\" end", LIBMYSQLCLIENT_FILE);
  }
}
