...
void print_connection_info(ObGroupDataSource *gds)
{
  ...
  {
    YYSYS_LOG(WARN, "%s() acquire read lock on g_config_rwlock fail, ret=%d", __FUNCTION__, lock_ret);
  }
  else
  {
    ...
    if (NULL != gds)
    {
      ...
      for (int32_t index = 0; index < gds->size_; ++index)
      {
        ...
        YYSYS_LOG(INFO, "[CONN INFO] CLUSTER[%d]=[%s], MS=%d, FLOW=%d",
        ...
        for (int32_t i = 0; i < scluster->size_; i++)
        {
          ...
          YYSYS_LOG(INFO, "[CONN INFO] MS[%d][%d]=[%s] FREE=%d USED=%d",
    ...
    YYSYS_LOG(INFO, "[BLACK LIST] MS BLACK LIST SIZE=%d", g_ms_black_list.size_);
    ...
    for (int32_t index = 0; index < g_ms_black_list.size_; index++)
    {
      ...
      YYSYS_LOG(INFO, "[BLACK LIST] MS[%d]=%s", index + 1, get_server_str(&ms));
    }
...
static ObDataSource* select_ds(ObClusterInfo *cluster, const char* sql, unsigned long length, ObSQLMySQL *mysql)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument: cluster=%p, mysql=%p", cluster, mysql);
  }
  else
  {
    ...
    if (NULL == ds)
    {
      YYSYS_LOG(WARN, "no valid MS available in cluster={id=%d,addr=%s,type=%d}, MS NUM=%d",
          cluster->cluster_id_, get_server_str(&cluster->cluster_addr_), cluster->cluster_type_, cluster->size_);
    }
    else
    {
      YYSYS_LOG(DEBUG, "selected MS=%s. free=%d, used=%d, is_black=%s, retry=%s, last_ds=%s",
...
static ObSQLConn* select_conn(ObDataSource *server)
{
  ...
  if (NULL == server)
  {
    YYSYS_LOG(WARN, "invalid argument server is %p", server);
  }
  else
  {
  ...
  if (NULL == conn)
  {
    YYSYS_LOG(WARN, "MS[%s] has no free connections, used=%d, free=%d",
...
ObSQLConn* acquire_conn(ObClusterInfo *cluster, const char* sql, unsigned long length, ObSQLMySQL *mysql)
{
  ...
  else
  {
    ...
    if (NULL != server)
    {
      ...
      if (NULL == real_conn)
      {
        YYSYS_LOG(WARN, "cluster(id=%u,addr=%s) has no free connections, MS NUM=%d",
            cluster->cluster_id_, get_server_str(&cluster->cluster_addr_), cluster->size_);
      }
      else
      {
        YYSYS_LOG(DEBUG, "acquired real conn=%p, MS=%s, cluster={ID=%d,LMS=%s}, used=%d, free=%d",
...
ObSQLConn* acquire_conn_random(ObGroupDataSource *gds, ObSQLMySQL* mysql)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument gds=%p, mysql=%p", gds, mysql);
  }
  else if (0 != (lock_ret = pthread_rwlock_rdlock(&g_config_rwlock)))
  {
    YYSYS_LOG(ERROR, "%s() acquire read lock on g_config_rwlock fail, ret=%d", __FUNCTION__, lock_ret);
  }
  else
  {
    pthread_mutex_lock(&pool_mutex);
    for (index = 0; index < gds->size_; ++index)
    {
      ...
      if (NULL != ds)
      {
        ...
        if (NULL == real_conn)
        {
          YYSYS_LOG(WARN, "cluster={id=%d,LMS=%s} has no avaliable connections",
              scluster->cluster_id_, get_server_str(&scluster->cluster_addr_));
        }
        else
        {
          YYSYS_LOG(INFO, "random selected connection: MS=%s, conn=%p, mysql=%p",
      ...
      {
        YYSYS_LOG(WARN, "get ms failed from cluster={id=%d,LMS=%s}",
            scluster->cluster_id_, get_server_str(&scluster->cluster_addr_));
      }
    }
    pthread_mutex_unlock(&pool_mutex);
...
static int close_conn_(ObSQLConn *conn)
{
  ...
  YYSYS_LOG(DEBUG, "close connection: MS=%s, conn=%p, real mysql=%p, node=%p, "
  ...
  if (conn->is_conn_pool_valid_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "delete connection node from used list fail, used_list=%p, node=%p, ret=%d",
...
static int recycle_conn_(ObSQLConn *conn)
{
  ...
  YYSYS_LOG(DEBUG, "recycle connection: MS=%s, conn=%p, real mysql=%p, node=%p, "
      "is_conn_pool_valid=%s, pool=%p, error=%d %s",
      get_server_str(&conn->server_), conn, conn->mysql_, conn->node_,
      conn->is_conn_pool_valid_ ? "true" : "false", conn->pool_,
      CALL_REAL(mysql_errno, conn->mysql_), CALL_REAL(mysql_error, conn->mysql_));
  ...
  {
    YYSYS_LOG(ERROR, "ob_sql_list_del fail, ret=%d, node=%p, used_list=%p, size=%d, MS=[%s], real mysql=%p",
  ...
  {
    YYSYS_LOG(ERROR, "ob_sql_list_add_tail fail, ret=%d, node=%p, used_list=%p, size=%d, MS=[%s], real mysql=%p",
...
int release_conn(ObSQLConn *conn)
{
  ...
  if (NULL == conn)
  {
    YYSYS_LOG(ERROR, "invalid arguments: conn=%p", conn);
    ret = OB_SQL_INVALID_ARGUMENT;
  }
  else if (! is_conn_valid_(conn))
  {
    YYSYS_LOG(ERROR, "invalid connection: conn=%p, MS=%s, node=%p, real mysql=%p, pool=%p, is_conn_pool_valid=%s",
  ...
  {
    YYSYS_LOG(ERROR, "%s() acquire read lock on g_config_rwlock fail, ret=%d, "
  ...
  else
  {
    ...
    YYSYS_LOG(DEBUG, "release conn: MS=[%s], conn=%p, is_conn_pool_valid=%s, pool=%p, node=%p, "
    ...
    if (! is_conn_pool_valid || 0 != mysql_eno)
    {
      ...
      {
        YYSYS_LOG(ERROR, "close_conn_ fail, conn=%p, MS=%s, ret=%d", conn, ms_addr, ret);
      }
      else
      {
        conn = NULL;
      ...
      {
        YYSYS_LOG(INFO, "create new connection to %s, free=%d, used=%d, old errno=%d",
        ...
        {
          YYSYS_LOG(WARN, "fail to add new connection to pool=%p, MS=%s, free=%d, used=%d",
          ...
          if (! g_ms_black_list.exist(ds->server_.ip_))
          {
            ...
            YYSYS_LOG(INFO, "add MS=%s to BLACK LIST, size=%d", ms_addr, g_ms_black_list.size_);
        ...
        {
          YYSYS_LOG(DEBUG, "create connection success for MS=%s after release the old. free=%d, used=%d",
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "fail to recycle connection, conn=%p, MS=%s, ret=%d", conn, ms_addr, ret);
      }
      else
      {
        conn = NULL;
    ...
    YYSYS_LOG(DEBUG, "after release_conn: MS=[%s], free=[%d], used=[%d], ret=%d",
