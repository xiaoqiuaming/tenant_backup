...
static int update_flow_weight(ObGroupDataSource *gds)
{
  ...
  if (NULL == gds)
  {
    YYSYS_LOG(ERROR, "invalid argument gds is %p", gds);
    ret = OB_SQL_ERROR;
  }
  else
  {
    ...
    YYSYS_LOG(INFO, "update flow weight: flow weight sum is %d", slot);
    for (index = 0; index < gds->size_; ++index)
    {
      ...
      YYSYS_LOG(INFO, "cluster(id=%u,addr=%s) weight is %d",
...
int update_select_table(ObGroupDataSource *gds)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "update_flow_weigth failed");
    }
    else
    {
      if (NULL == g_table)
      {
        ...
        if (NULL == g_table)
        {
          YYSYS_LOG(ERROR, "alloc mem for ObSQLSelectTable failed");
          ret = OB_SQL_ERROR;
        }
        else
        {
          g_table->master_count_ = 0;
          YYSYS_LOG(DEBUG, "g_table->master_count_ is %u", g_table->master_count_);
        }
      }
      ...
      if (OB_SQL_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "update cluster select table: get master cluster=%p, id=%d, addr=%s",
            master_cluster,
        ...
        int slot_num = static_cast<int>(sizeof(g_table->clusters_) / sizeof(g_table->clusters_[0]));
...
{
  YYSYS_LOG(INFO, "destroy select table %p", g_table);
...
ObClusterInfo* select_cluster(ObSQLMySQL* mysql)
{
  ...
  if (NULL == g_table)
  {
    YYSYS_LOG(ERROR, "cluster select table is NULL, not inited");
  }
  else if (NULL == mysql)
  {
    YYSYS_LOG(ERROR, "invalid argument: mysql=%p", mysql);
  }
  else if (0 > g_group_ds.size_)
  {
    YYSYS_LOG(WARN, "no valid cluster, cluster size=%d", g_group_ds.size_);
  }
  else if (is_consistence(mysql))
  {
    //一致性请求
    YYSYS_LOG(DEBUG, "consistence g_table->master_count_ is %u", g_table->master_count_);
    cluster = g_table->master_cluster_;
    YYSYS_LOG(DEBUG, "consistence cluster=%p, id = %u flow weight is %d ",
        cluster,
  ...
  else // g_group_ds.cluster_size_ > 1
  {
    while (1)
    {
      ...
      YYSYS_LOG(DEBUG, "select g_table->master_count_ is %u, index=%ld", g_table->master_count_, cluster_index);

      if (NULL == cluster)
      {
        YYSYS_LOG(ERROR, "cluster in select table (g_table) is NULL, index=%ld", cluster_index);
        break;
      }
    ...
    if (NULL == cluster)
    {
      ...
      {
        YYSYS_LOG(WARN, "select cluster from g_table (%p) fail, use first cluster %p in g_group_ds %p instead, size=%d",
  ...
  if (NULL != cluster)
  {
    YYSYS_LOG(DEBUG, "select cluster: ID=%u FLOW=%d, CSIZE=%d, LMS=[%s], TYPE=%d, RDST=%d",
  ...
  {
    YYSYS_LOG(WARN, "select cluster fail, cluster size=%d", g_group_ds.size_);
  }
