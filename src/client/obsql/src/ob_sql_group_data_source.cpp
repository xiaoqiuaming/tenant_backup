...
static void delete_cluster_(ObGroupDataSource *gds, const int32_t cindex)
{
  if (NULL != gds && 0 <= cindex && cindex < gds->size_ && NULL != gds->clusters_[cindex])
  {
    ...
    YYSYS_LOG(INFO, "delete cluster: ID=%d, LMS=%s, MS NUM=%d, cluster number=%d",
    ...
    for (int32_t index = 0; index < cluster->size_; ++index)
    {
      ...
      {
        YYSYS_LOG(WARN, "cluster %p has NULL ObDataSource at index=%d: cluster=[id=%d,addr=%s], MS NUM=%d",
...
static void delete_ms_from_config_(ObSQLClusterConfig &config, const int32_t index)
{
  ...
  {
    YYSYS_LOG(INFO, "delete MS %s from ObSQLClusterConfig, MS NUM=%d",
    ...
    YYSYS_LOG(DEBUG, "after delete MS %s from ObSQLClusterConfig, MS NUM=%d",
        get_server_str(&config.merge_server_[index]), config.server_num_);
  }
}
...
static int add_cluster_(ObSQLClusterConfig *cluster_conf, ObGroupDataSource *gds)
{
  ...
  {
    YYSYS_LOG(ERROR, "can not add new cluster, current cluster number=%d, max cluster number=%d",
        gds->size_, OB_SQL_MAX_CLUSTER_NUM);
    ret = OB_SQL_ERROR;
  }
  else
  {
    YYSYS_LOG(INFO, "add cluster: ID=%d, LMS=%s, MS NUM=%d, TYPE=%d, FLOW=%d, RDST=%d",
    ...
    if (NULL == cluster)
    {
      YYSYS_LOG(ERROR, "alloc memory for ObClusterInfo fail");
      ret = OB_SQL_ERROR;
    }
    else
    {
      ...
      for (int index = 0; index < cluster_conf->server_num_; ++index)
      {
        ...
        {
          YYSYS_LOG(ERROR, "init ObDataSource %s for cluster %d failed",
      ...
      if (0 < cluster->size_)
      {
        ...
        ret = OB_SQL_SUCCESS;
        YYSYS_LOG(INFO, "add cluster success: ID=%d, LMS=%s, MS NUM=%d, cluster number=%d",
            cluster->cluster_id_, get_server_str(&cluster->cluster_addr_), cluster->size_, gds->size_);
      }
      else
      {
    ...
    {
      YYSYS_LOG(ERROR, "add cluster fail, ID=%d, LMS=%s",
          cluster_conf->cluster_id_, get_server_str(&cluster_conf->server_));
    }
  }
...
static int update_cluster_(ObSQLClusterConfig &config, ObClusterInfo *cluster)
{
  ...
  YYSYS_LOG(INFO, "update_cluster: ID=%u, LMS=%s, MS NUM=(old=%d,new=%d), FLOW={old=%d,new=%d}, RDST={old=%d,new=%d}",
  ...
  for (sindex = 0; sindex < cluster->size_; ++sindex)
  {
    ...
    if (NULL == sds)
    {
      YYSYS_LOG(WARN, "cluster %p has NULL ObDataSource at index=%d: cluster=[id=%d,addr=%s], MS NUM=%d, delete it",
    ...
    for (index = 0; index < config.server_num_; ++index)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "update_cluster: MS[%d]=%s, config.server_num_=%d, cluster->size_=%d",
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "update data source fail, MS=%s, cluster={id=%d,addr=%s}, delete MS from cluster and config",
  ...
  for (index = 0; index < config.server_num_; ++index)
  {
    ...
    if (0 == exist)
    {
      ...
      {
        YYSYS_LOG(INFO, "add_data_source fail, MS=%s, cluster={id=%d,addr=%s}, delete it from configuration",
  ...
  {
    YYSYS_LOG(INFO, "cluster {id=%d,addr=%s} has no valid MS after updating",
  ...
  {
    YYSYS_LOG(ERROR, "cluster {id=%d,addr=%s} ObSQLClusterConfig::server_num_(%d) != ObClusterInfo::size_(%d)",
        config.cluster_id_, get_server_str(&config.server_), config.server_num_, cluster->size_);
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "update cluster {id=%d,addr=%s} success. MS NUM=%d",
...
int update_group_ds(ObSQLGlobalConfig *config, ObGroupDataSource *gds)
{
  ...
  for (index = 0; index < gds->size_; ++index)
  {
    ...
    if (NULL == scluster)
    {
      YYSYS_LOG(WARN, "gds %p has NULL cluster at index=%d, cluster number=%d, delete it", gds, index, gds->size_);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "delete fail-updated cluster(id=%d,addr=%s) from group data source and configuration",
  ...
  for (cindex = 0; cindex < config->cluster_size_; ++cindex)
  {
    ...
    if (0 == exist)
    {
      ...
      {
        YYSYS_LOG(WARN, "delete fail-add cluster (id=%d,addr=%s) from configuration",
  ...
  {
    YYSYS_LOG(ERROR, "after update group data source: config cluster size (%d) != group data source cluster size (%d)",
...
void destroy_group_ds(ObGroupDataSource *gds)
{
  if (NULL != gds)
  {
    YYSYS_LOG(INFO, "destroy group datasource: %p", gds);
    ...
    for (int32_t cindex = 0; cindex < gds->size_; ++cindex)
    {
      ...
      if (NULL != cluster)
      {
        YYSYS_LOG(INFO, "destroy cluster %p: ID=%d, LMS=%s, MS NUM=%d, cluster number=%d",
