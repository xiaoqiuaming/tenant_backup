...
void delete_cluster_from_config(ObSQLGlobalConfig &config, const int index)
{
  ...
  {
    YYSYS_LOG(INFO, "delete cluster={id=%d,addr=%s} from config",
...
static void insert_rs_list(uint32_t ip, uint32_t port)
{
  ...
  if (0 == exist && g_rsnum < rslist_array_size)
  {
    ...
    YYSYS_LOG(INFO, "insert cluster address: %s, size=%d", get_server_str(g_rslist + g_rsnum - 1), g_rsnum);
  }
}
...
static int update_cluster_info_(MYSQL *mysql, ObSQLGlobalConfig *gconfig, ObServerInfo *server)
{
  ...
  {
    YYSYS_LOG(ERROR, "do query '%s' on server %s fail, err=%d, %s",
  ...
  {
    YYSYS_LOG(ERROR, "store result failed, query is %s, errno=%u, %s", OB_SQL_QUERY_CLUSTER,
        CALL_REAL(mysql_errno, mysql), CALL_REAL(mysql_error, mysql));
    ret = OB_SQL_ERROR;
  }
  else if (0 >= (cluster_num = CALL_REAL(mysql_num_rows, results)))
  {
    YYSYS_LOG(WARN, "get empty cluster information from server %s", get_server_str(server));
  }
  else
  {
    ...
    while (OB_SQL_MAX_CLUSTER_NUM > cluster_index && NULL != (record = CALL_REAL(mysql_fetch_row, results)))
    {
      ...
      {
        YYSYS_LOG(WARN, "cluster info are not complete: cluster_id=%s, cluster_role=%s, cluster_vip=%s, "
            "cluster_port=%s",
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "add cluster: cluster={id=%d,addr=%s}, flow={old=%d,new=%d}, type=%d, read_strategy=%u",
    ...
    YYSYS_LOG(INFO, "update cluster info from %s success, valid cluster num=%ld/%ld",
...
static int update_mslist_(MYSQL *mysql, ObSQLClusterConfig *cluster, ObServerInfo *server)
{
  ...
  {
    YYSYS_LOG(WARN, "do query '%s' from %s failed, errno=%d, %s",
        querystr, get_server_str(server), CALL_REAL(mysql_errno, mysql), CALL_REAL(mysql_error, mysql));
    ret = OB_SQL_ERROR;
  }
  else if (NULL == (results = CALL_REAL(mysql_store_result, mysql)))
  {
    YYSYS_LOG(WARN, "store result failed, query is '%s' errno=%u, %s", querystr,
        CALL_REAL(mysql_errno, mysql), CALL_REAL(mysql_error, mysql));
    ret = OB_SQL_ERROR;
  }
  else if (0 >= (ms_num = CALL_REAL(mysql_num_rows, results)))
  {
    YYSYS_LOG(WARN, "cluster (ID=%u, LMS=%s) mslist is empty", cluster->cluster_id_, get_server_str(&(cluster->server_)));
  }
  else
  {
    int64_t index = 0;
    ...
    while (OB_SQL_MAX_MS_NUM > index && NULL != (record = CALL_REAL(mysql_fetch_row, results)))
    {
      ...
      {
        YYSYS_LOG(WARN, "queried meergeserver info is not complete: ip=%p, port=%p", record[0], record[1]);
      }
      else
      {
    ...
    YYSYS_LOG(INFO, "update cluster mslist: cluster={ID=%u,LMS=%s}, valid ms num=%d/%lu",
...
static int get_cluster_config(ObServerInfo *server)
{
  ...
  {
    YYSYS_LOG(ERROR, "set_mysql_options fail, ret=%d, mysql=%p", ret, mysql);
  }
  else if (NULL == (mysql = CALL_REAL(mysql_real_connect, mysql, ip, user, passwd, OB_SQL_DB, port, NULL, 0)))
  {
    YYSYS_LOG(ERROR, "failed to connect to server %s, errno=%d, %s",
  ...
  {
    YYSYS_LOG(ERROR, "update_cluster_info_ fail, server=%s, mysql=%p, ret=%d",
  ...
  else
  {
    for (int index = 0; index < g_config_update->cluster_size_ && OB_SQL_SUCCESS == ret; ++index)
    {
      ...
      {
        YYSYS_LOG(WARN, "update mslist from %s fail, mysql=%p, cluster=[ID=%u, LMS=%s], index=%d, cluster_size=%d",
      ...
      {
        YYSYS_LOG(WARN, "no mergeserver avaliable in cluster %d, addr=%s. Delete it from configuration",
  ...
  {
    YYSYS_LOG(WARN, "no valid cluster updated from %s, total cluster address num=%d", get_server_str(server), g_rsnum);
    ret = OB_SQL_ERROR;
  }

  YYSYS_LOG(INFO, "update cluster config from %s, ret=%d, cluster size=%d, LMS num=%d, "
...
static int rebuild_tables()
{
  ...
  {
    YYSYS_LOG(INFO, "update data source: mslist_changed=%s, cluster_changed=%s, force_update=%s, black list size=%d",
    ...
    {
      YYSYS_LOG(ERROR, "update group data source fail, ret=%d", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "update data source success");
    }
    ...
    {
      YYSYS_LOG(ERROR, "update cluster select table failed");
    }
    else
    {
      YYSYS_LOG(INFO, "update cluster select table success");
    }
...
// wanhong.wwh 2014-04-09
  ...
  if (OB_SQL_SUCCESS == ret && mslist_changed)
  {
    ...
    if (OB_SQL_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "update ms select table failed");
    }
    else
    {
      YYSYS_LOG(DEBUG, "update ms select table success");
    }
  }
#endif
...
int get_ob_config()
{
  ...
  YYSYS_LOG(DEBUG, "get_ob_config: rsnum is %d", g_rsnum);

  // NOTE: 对g_rslist_rwlock加写锁，保护g_rslist和g_rsnum
  int lock_ret = 0;
  if (0 != (lock_ret = pthread_rwlock_wrlock(&g_rslist_rwlock)))
  {
    YYSYS_LOG(ERROR, "%s acquire write lock on g_rslist_rwlock fail, ret=%d", __FUNCTION__, lock_ret);
    ret = OB_SQL_ERROR;
  }
  else
  {
    if (0 >= g_rsnum)
    {
      YYSYS_LOG(ERROR, "no cluster address available, g_rsnum=%d", g_rsnum);
      ret = OB_SQL_ERROR;
    }
    else
    {
      ...
      for (int index = 0; index < g_rsnum; ++index)
      {
        ...
        {
          YYSYS_LOG(WARN, "update cluster config from %s failed, index=%d, cluster address num=%d",
...
{
  YYSYS_LOG(INFO, "destroy g_config_update %p", g_config_update);
#ifdef ENABLE_SELECT_MS_TABLE
  ...
  YYSYS_LOG(INFO, "destroy g_config_using %p", g_config_using);
#ifdef ENABLE_SELECT_MS_TABLE
...
int do_update()
{
  ...
  {
    YYSYS_LOG(ERROR, "%s() acquire write lock on g_config_rwlock fail, ret=%d", __FUNCTION__, lock_ret);
    ret = OB_SQL_ERROR;
  }
  else
  {
    ...
    if (OB_SQL_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "do_update fail: fail to rebuild table");
    }
    else
    {
      YYSYS_LOG(INFO, "update global config success");
    }
...
int update_cluster_address_list(const char *cluster_address)
{
  ...
  {
    YYSYS_LOG(WARN, "%s() try to acquire write lock on g_rslist_rwlock fail, ret=%d", __FUNCTION__, lock_ret);
    ret = OB_SQL_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "snprintf cluster_address(%s) to buffer fail, buffer length=%ld",
    ...
    else
    {
      ...
      while (NULL != (ipaddr = strtok_r(buf_ptr, CLUSTER_ADDRESS_DELIMITOR, &saveptr)))
      {
        ...
        {
          YYSYS_LOG(WARN, "ObServer.parse_from_cstring(ipaddr=%s) fail, ret=%d", ipaddr, ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "libobsql cluster address array is full, size=%d, ignore the left. cluster_address='%s'",
        ...
        YYSYS_LOG(INFO, "LIBOBSQL: refresh cluster address '%s' from '%s', size=%d", ipaddr, cluster_address, g_rsnum);
      ...
      if (! cluster_address_parsed)
      {
        YYSYS_LOG(ERROR, "no valid cluster address in '%s'", cluster_address);
        ret = OB_SQL_ERROR;
      }
      else
      {
        YYSYS_LOG(INFO, "LIBOBSQL: refresh cluster address count=%d, clusterAddress=%s", g_rsnum, cluster_address);
        ret = OB_SQL_SUCCESS;
      }
    }
