...
ObDataSource* random_mergeserver_select(ObClusterInfo *cluster, ObSQLMySQL *mysql)
{
  ...
  {
    YYSYS_LOG(WARN, "no MS in cluster[id=%d,addr=%s,type=%d], MS NUM=%d",
  ...
  else
  {
    ...
    YYSYS_LOG(DEBUG, "SELECT_MS: random offset=%ld", r);
    ...
    {
      YYSYS_LOG(INFO, "SELECT_MS: MS=%s invalid. free=%d, used=%d, is_black=%s, retry=%s, last_ds=%s",
      ...
      if (1 < cluster->size_)
      {
        for (int32_t i = 0; i < cluster->size_; i++)
        {
          ...
          YYSYS_LOG(INFO, "SELECT_MS: MS=%s %s. free=%d, used=%d, is_black=%s, retry=%s, last_ds=%s, index=%d",
...
ObDataSource* consishash_mergeserver_select(ObClusterInfo *pool, const char* sql, unsigned long length)
{
  ...
  YYSYS_LOG(INFO, "hashval of this query is %u", hashval);
  int32_t index = 0;
...
ObSQLConn* random_conn_select(ObDataSource *pool)
{
  ...
  YYSYS_LOG(DEBUG, "before random_conn_select: MS=%s, free=%d, used=%d",
  ...
  if (NULL != node)
  {
    ...
    {
      YYSYS_LOG(ERROR, "lock pool->mutex_ %p failed, code id %d, mesg is %s", &pool->mutex_, errno, strerror(errno));
    }
  }
  else
  {
    YYSYS_LOG(DEBUG, "ob_sql_list_get_first(pool(%p) free conn list) is null", pool);
  }

  YYSYS_LOG(DEBUG, "after random_conn_select: MS=%s, free=%d, used=%d, node=%p",
