...
{
  YYSYS_LOG(DEBUG, "wakeup update worker begin, g_update_flag=%s", g_update_flag ? "true" : "false");
  pthread_mutex_lock(&g_update_mutex);
  g_update_flag = true;
  pthread_cond_signal(&g_update_cond);
  pthread_mutex_unlock(&g_update_mutex);
  YYSYS_LOG(INFO, "wakeup update worker, g_update_flag=%s", g_update_flag ? "true" : "false");
}

void clear_update_flag()
{
  YYSYS_LOG(DEBUG, "clear update flag begin, g_update_flag=%s", g_update_flag ? "true" : "false");
  pthread_mutex_lock(&g_update_mutex);
  g_update_flag = false;
  pthread_mutex_unlock(&g_update_mutex);
  YYSYS_LOG(INFO, "clear update flag, g_update_flag=%s", g_update_flag ? "true" : "false");
}
...
static void *update_global_config(void *arg)
{
  ...
  while (g_inited)
  {
    last_update_time_us = yysys::CTimeUtil::getTime();
    ...
    pthread_mutex_lock(&g_update_mutex);
    while (! g_update_flag && yysys::CTimeUtil::getTime() < end_time)
    {
    ...
    OB_SQL_LOG(INFO, "update worker startup, g_update_flag=%s, update interval=%ld us",
        ...
        yysys::CTimeUtil::getTime() - last_update_time_us);
