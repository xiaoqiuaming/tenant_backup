...
void dump_table()
{
  if (NULL != g_table)
  {
    ...
    for (int idx = 0; idx < OB_SQL_SLOT_NUM; idx++)
    {
      ...
      YYSYS_LOG(DEBUG, "g_table[%d]=cluster(id=%d,addr=%s)",
...
int set_mysql_options(MYSQL *mysql)
{
  ...
  YYSYS_LOG(DEBUG, "set_mysql_options: CONNECT_TIMEOUT=%ld  READ_TIMEOUT=%ld  WRITE_TIMEOUT=%ld",
      connect_timeout, read_timeout, write_timeout);

  if (NULL == mysql || NULL == g_func_set.real_mysql_options)
  {
    YYSYS_LOG(ERROR, "invalid argument: mysql=%p, g_func_set.real_mysql_options=%p",
  ...
  {
    YYSYS_LOG(WARN, "mysql_options(MYSQL_OPT_CONNECT_TIMEOUT) fail, ret=%d, mysql=%p, timeout=%ld",
        ret, mysql, connect_timeout);
  }
  else if (0 != (ret = g_func_set.real_mysql_options(mysql, MYSQL_OPT_READ_TIMEOUT, &read_timeout)))
  {
    YYSYS_LOG(WARN, "mysql_options(MYSQL_OPT_READ_TIMEOUT) fail, ret=%d, mysql=%p, timeout=%ld",
        ret, mysql, read_timeout);
  }
  else if (0 != (ret = g_func_set.real_mysql_options(mysql, MYSQL_OPT_WRITE_TIMEOUT, &write_timeout)))
  {
    YYSYS_LOG(WARN, "mysql_options(MYSQL_OPT_WRITE_TIMEOUT) fail, ret=%d, mysql=%p, timeout=%ld",
