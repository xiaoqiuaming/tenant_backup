...
int ob_sql_list_init(ObSQLList *list, ObSQLListType type)
{
  ...
  if (NULL == list)
  {
    YYSYS_LOG(ERROR, "invalide argument list is %p", list);
    ret = OB_SQL_ERROR;
  }
  else
  {
...
int ob_sql_list_add_before(ObSQLList *list, ObSQLListNode *pos, ObSQLListNode *node)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalide argument list is %p, node is %p", list, node);
    ret = OB_SQL_ERROR;
  }
  else
  {
...
int ob_sql_list_add_head(ObSQLList *list, ObSQLListNode *node)
{
  ...
  if (NULL == list || NULL == node)
  {
    YYSYS_LOG(ERROR, "invalid argument list is %p, node is %p", list, node);
    ret = OB_SQL_ERROR;
  }
  else
  {
