...
#include "ob_root_async_task_queue.h"//add pangtianze [Paxos inner table revise] 20170826

namespace oceanbase
{
  namespace rootserver
  {
    // shutdown����ִ�еĲ���:shutdown, restart
    ...
    struct ObServerStatus
    {
      ...
      int64_t cluster_id_; //add peiouya [MultiUPS] [SYS_Table_Modify] 20150507   /*mod liuqc [add multi_ups to trunk] 2016/9/27 16:40*/
    ...
    class ObChunkServerManager
    {
      public:
        ...
        const_iterator get_serving_ms(const int64_t cluster_id = OB_ALL_CLUSTER_FLAG) const;
        // regist or receive heartbeat
		/*mod liuqc [add multi_ups to trunk] 2016/9/27 16:40*/
        ...
        int receive_hb(int & res, const common::ObServer& server, const int64_t time_stamp,const bool is_merge_server = false,
            const bool is_listen_ms = false, const int32_t sql_port = 0, const bool is_regist = false
			//add pangtianze [Paxos rs_election] 20170420:b
        ...
        int serialize_cs_info(const ObServerStatus *it, char* buf, const int64_t buf_len, int64_t& pos) const;
        ...
        int serialize_ms_list(char* buf, const int64_t buf_len, int64_t& pos, int32_t ms_cluster
      ...
      private:
        mutable yysys::CRWLock migrate_infos_lock_;
