...
class ObRsStressRunnable : public yysys::CDefaultRunnable
{
  public:
...
void ObRsStressRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  int ret = OB_SUCCESS;
...
int main(int argc, char *argv[])
{
  ...
  MyMain my_main;
  MyArguments args;
  YYSYS_LOGGER.setFileName("rs_stress.log", true);
  ...
  else
  {
    ...
    else
    {
      ...
      threads.wait();
      int64_t finish = yysys::CTimeUtil::getTime();
