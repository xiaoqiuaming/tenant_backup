  ...
  namespace rootserver
  {
    ...
    int ObRootLogWorker::sync_schema(const int64_t timestamp)
    {
      ...
      if (log_data == NULL)
      {
        ...
        YYSYS_LOG(ERROR, "allocate memory failed, size: %ld", OB_MAX_PACKET_LENGTH);
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "open schema file failed when sync");
        }
        else
        {
          ...
          if (ret == OB_SUCCESS)
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(ERROR, "read from schema file failed");
            }

            if (rl == OB_MAX_PACKET_LENGTH)
            {
              YYSYS_LOG(ERROR, "schema file too large, size: %ld", OB_MAX_PACKET_LENGTH);
            }
          }
    ...
    int ObRootLogWorker::regist_cs(const ObServer& server, const char* server_version, const int64_t timestamp, const int64_t cluster_id)
    {
      //return log_server_with_ts(OB_RT_CS_REGIST, server, server_version, timestamp);
      return log_server_with_ts(OB_RT_CS_REGIST, server, server_version, timestamp,cluster_id);
    }
    //mod:e

    //mod zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150528:b
    //int ObRootLogWorker::regist_lms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp)
    int ObRootLogWorker::regist_lms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp, const int64_t cluster_id)
    //mod:e
    {
      int ret = OB_SUCCESS;
      ...
      if (ret == OB_SUCCESS)
      {
        ret = serialization::encode_vi64(log_data, OB_MAX_PACKET_LENGTH, pos, cluster_id);
      }
      //add:e
    ...
    //mod zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150528:
    //int ObRootLogWorker::regist_ms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp)
    int ObRootLogWorker::regist_ms(const ObServer& server, int32_t sql_port, const char* server_version, const int64_t timestamp, const int64_t cluster_id)
    //mod:e
    {
      int ret = OB_SUCCESS;
    ...
    int ObRootLogWorker::server_is_down(const ObServer& server, const int64_t timestamp, const int64_t cluster_id)
    {
      // return log_server_with_ts(OB_RT_SERVER_DOWN, server, "", timestamp);
      return log_server_with_ts(OB_RT_SERVER_DOWN, server, "", timestamp,cluster_id);
    }
    //mod:e


    //add zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150528:b
    int ObRootLogWorker::server_is_down_ms(const ObServer& server, const int32_t sql_port, const int64_t timestamp, const int64_t cluster_id)
    {
      int ret = OB_SUCCESS;
      ...
      if (ret == OB_SUCCESS)
      {
        ret = flush_log(OB_RT_MS_DOWN, log_data, pos);
      }
    ...
    int ObRootLogWorker::log_server_with_ts(const LogCommand cmd, const ObServer& server, const char* server_version, const int64_t timestamp, const int64_t cluster_id)
    // mod:e
    {
      int ret = OB_SUCCESS;
        ...
        const int64_t occupy_size, const uint64_t crc_sum, const uint64_t row_checksum, const int64_t row_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize migrate done, ret=%d", ret);
      }
...
int ObRootLogWorker::add_range_for_load_data(const common::ObTabletReportInfoList& tablets)
    ...
    int ObRootLogWorker::set_load_table_status(const uint64_t table_id, const int32_t status, const int64_t end_time)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize status=%d, ret=%d", status, ret);
      }
      else if (OB_SUCCESS != (ret = serialization::encode_vi64(log_data, OB_MAX_PACKET_LENGTH, pos, end_time)))
      {
        YYSYS_LOG(WARN, "failed to serialize end_time=%ld, ret=%d", end_time, ret);
      }
    ...
    int ObRootLogWorker::add_new_tablet(const ObTabletInfo tablet, const ObArray<int32_t> &chunkservers, const int64_t mem_version)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to encode chunkserver count. ret=%d", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to serialize tablet, ret=%d", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        for (int32_t i=0; i < chunkservers.count(); ++i)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "serialize failed");
            break;
          }
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to encode mem_version. ret=%d", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to flush log to buf. ret=%d", ret);
        }
      }
      if (log_data != NULL)
      {
                                              ...
                                              int** server_indexs, int* count, const int64_t mem_version)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; OB_SUCCESS == ret && i < index; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "p_table_info should not be NULL");
            ret = OB_ERROR;
          }
          if (ret == OB_SUCCESS)
          {
...
int ObRootLogWorker::clean_root_table()
{
  ...
  {
    YYSYS_LOG(ERROR, "Seriliaze config version failed! ret: [%d]", ret);
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to flush log for cmd clean_root_table, ret=%d", ret);
    }
  }
  ...
  return ret;
    ...
    int ObRootLogWorker::got_config_version(int64_t config_version)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Flush log failed! ret: [%d]", ret);
      }
    ...
    int ObRootLogWorker::flush_log(const LogCommand cmd, const char* log_data, const int64_t& serialize_size)
    {
      ...
      //add:e
      {
        YYSYS_LOG(DEBUG, "flush update log, cmd type: %d", cmd);

      yysys::CThreadGuard guard(log_manager_->get_log_sync_mutex());
      ret = log_manager_->write_and_flush_log(cmd, log_data, serialize_size);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "write and flush log failed:cmd[%d], log[%s], size[%ld], ret[%d]",
    ...
    int ObRootLogWorker::apply(common::LogCommand cmd, const char* log_data, const int64_t& data_len)
    {
      ...
      YYSYS_LOG(INFO, "start replay log, cmd type: %d", cmd);
      switch (cmd)
      {
        ...
        case OB_LOG_SWITCH_LOG:
          YYSYS_LOG(INFO, "apply: switch_log");
          break;
        ...
        //add zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150710 b:
        case OB_RT_MS_DOWN:
          ret = do_ms_down(log_data, data_len);
          break;
        //add:e
        default:
          YYSYS_LOG(WARN, "unknow log command [%d]", cmd);
    ...
    int ObRootLogWorker::do_check_point(const char* log_data, const int64_t& log_length)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "this is master, may have lost checkpointing, ckpt=%ld", ckpt_id);
        }
        else
        {
    ...
    int ObRootLogWorker::do_cs_regist(const char* log_data, const int64_t& log_length)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "decode vstr error");
        }
      }
      //add zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150528:b
      if (ret == OB_SUCCESS)
      {
        ret = serialization::decode_vi64(log_data, log_length, pos, &cluster_id);
      }
      //add:e
 ...
 //add zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150713:b
      if (ret == OB_SUCCESS)
      {
        server_version = serialization::decode_vstr(log_data, log_length, pos, &server_version_length);
        if (NULL == server_version)
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (it != NULL)
        {
          ...
          //add:e
          yysys::CRLockGuard guard(root_server_->root_table_rwlock_);
    ...
    //add zhaoqiong [MultiUPS] [MS_CS_Manage_Function] 20150529:b
    int ObRootLogWorker::do_ms_down(const char* log_data, const int64_t& log_length)
    {
      int ret = OB_SUCCESS;
    ...
    int ObRootLogWorker::do_cs_migrate_done(const char* log_data, const int64_t& log_length)
    {
      ...
      int32_t type = 0;
       ...
       {
         YYSYS_LOG(WARN, "failed to deserialize part1 of migrate done info, ret=%d", ret);
       }

       if (log_length == pos)
       {
         YYSYS_LOG(INFO, "for compatible old commit log");
       }
       ...
       {
         YYSYS_LOG(WARN, "failed to deserialize part2 of migrate done, ret=%d", ret);
       }
...
int ObRootLogWorker::do_add_range_for_load_data(const char* log_data, const int64_t& log_length)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to report tablet for load data. ret=%d", ret);
    }
  }
  return ret;
}
    ...
    int ObRootLogWorker::do_add_new_tablet(const char* log_data, const int64_t& log_length)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to deserialize tablet. ret=%d", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS && count > 0 )
      {
		...
		//add e
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "fail to decode cs_id. ret=%d", ret);
            break;
          }
          else if (OB_SUCCESS != (ret = chunkservers.push_back(cs_id)))
          {
            YYSYS_LOG(ERROR, "failed to push back into array, err=%d", ret);
            break;
          }
        }
      }
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to decode mem_version. ret=%d", ret);
        }
      }
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to replay create new table. err = %d", ret);
        }
      }
      else
      {
        YYSYS_LOG(WARN, "fail to serialize data to buff. ret=%d", ret);
      }
    ...
    int ObRootLogWorker::do_batch_add_new_tablet(const char* log_data, const int64_t& log_length)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "new tablet_info fail.");
      }
      for (int64_t i = 0; OB_SUCCESS == ret && i < index; i++)
      {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "do batch add new tablet success.");
      }
      return ret;
    }
    ...
    int ObRootLogWorker::do_remove_replica(const char* log_data, const int64_t& log_length)
    {
      ...
      if (OB_SUCCESS != (ret = tablet.deserialize(log_data, log_length, pos)))
      {
        YYSYS_LOG(WARN, "deserialize tablet failed:ret[%d]", ret);
      }
      else
      {
        // replay log
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "replay removed replica failed:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObRootLogWorker::do_delete_replicas(const char* log_data, const int64_t& log_length)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "deserialize server failed:ret[%d]", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "replay remove replica error, err=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObRootLogWorker::do_remove_table(const char* log_data, const int64_t& log_length)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize table count");
      }
      else
      {
        ...
        for (int64_t i = 0; i < table_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to serialize table id");
            break;
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "delete table from root table in replay commit log");
    ...
    int ObRootLogWorker::do_add_load_table(const char* log_data, const int64_t& log_length)
    {
      ...
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "add load table failed:table_name=%.*s table_id=%lu uri=%.*s",
              table_name.length(), table_name.ptr(), table_id, uri.length(), uri.ptr());
        }
        else
        {
          YYSYS_LOG(INFO, "add load table: table_name=%.*s table_id=%lu uri=%.*s from replay commit log",
              table_name.length(), table_name.ptr(), table_id, uri.length(), uri.ptr());
        }
      }
      return ret;
    ...
    int ObRootLogWorker::do_set_load_table_status(const char* log_data, const int64_t& log_length)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialize status, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = serialization::decode_vi64(log_data, log_length, pos, &end_time)))
      {
        YYSYS_LOG(WARN, "failed to deserialize end_time, ret=%d", ret);
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "failed to set load table status: table_id=%lu status=%d from replay commit log",
              table_id, status);
        }
        else
        {
          YYSYS_LOG(INFO, "set load table task status: table_id=%lu status=%s endtime=%ld from replay commit log",
...
int ObRootLogWorker::do_init_first_meta_row(const char* log_data, const int64_t& log_length)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to replay init first meta file. err=%d", ret);
    }
    else
    {
...
int ObRootLogWorker::do_clean_root_table(const char* log_data, const int64_t& log_length)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to decode unused_value. ret=%d", ret);
        }
      }
      ret = root_server_->slave_clean_root_table();
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to clean root table for slave_rootserver, ret=%d", ret);
      }
      return ret;
    }
    ...
    int ObRootLogWorker::do_got_config_version(const char* log_data, const int64_t& log_length)
    {
      ...
      {
        YYSYS_LOG(ERROR, "root server is NULL!");
      }
      else if (NULL == (config_mgr = root_server_->get_config_mgr()))
      {
        YYSYS_LOG(ERROR, "Config manager is NULL!");
      }
   ...
   int ObRootLogWorker::do_set_bypass_version(const char* log_data, const int64_t& log_length)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize bypass version error");
      }
      else
      {
