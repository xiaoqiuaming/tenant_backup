...
namespace oceanbase
{
  namespace rootserver
  {
    ...
    struct ObRootElectionNode
    {
        ...
        char paxos_ups_quorum_scales_[OB_MAX_PAXOS_UPS_QUORUM_SCALES_LENGTH];
        ...
        /// @brief ��ȡRole
        inline ObRsNodeRole get_role() const
        { return rs_election_role_; }

        /// @brief �޸�Role
        inline void set_role(const ObRsNodeRole role)
        {
          YYSYS_LOG(INFO, "before set_rs_node_role=%s", get_rs_role_str());
          atomic_exchange(reinterpret_cast<uint32_t*>(&rs_election_role_), static_cast<uint32_t>(role));//mod support for arm platform by wangd 202106
          YYSYS_LOG(INFO, "after set_rs_node_role=%s", get_rs_role_str());
        }
        ...
        inline void print()
        {
          YYSYS_LOG(INFO, "addr[%s], votefor[%s], role[%s], alive[%d], has_leader[%d], lease[%ld], "
                    "term[%ld], hb_time[%ld], paxos_num[%d]",
                    my_addr_.to_cstring(), votefor_.to_cstring(), get_rs_role_str()
                    , is_alive_, has_leader_, lease_, current_rs_term_, hb_resp_timestamp_
                    , paxos_num_);
        }
        //add:e
        void set_paxos_ups_quorum_scales(const char *inputstr)
        {
          if (inputstr != NULL)
          {
            common::ObSpinLockGuard guard(change_paxos_quorum_scale_lock_);
            memcpy(paxos_ups_quorum_scales_, inputstr, strlen(inputstr) + 1);
            YYSYS_LOG(DEBUG, "new paxos_ups_quorum_scales is %s", paxos_ups_quorum_scales_);
          }
          else
          {
            YYSYS_LOG(WARN, "input string is NULL");
          }
        }
    };
    ...
    class ObRootElectionNodeMgr
    {
      ...
      public:
        ...
        inline const char * get_my_role_str() const
        {
          return my_node_.get_rs_role_str();
        }
        ...
        {
          int32_t new_priority = 1; //mod 20170911
        ...
        void get_all_alive_rs_by_cluster_id(ObServer *servers, int32_t & server_count, const int64_t cluster_id);
        //add 20160726:e
        ...
        void if_all_quorum_in_rs_changed(char *buf, const int64_t buf_len, int64_t& pos) const;
        ...
        void set_paxos_ups_quorum_scales(const char *inputstr)
        {
          my_node_.set_paxos_ups_quorum_scales(inputstr);
        }
        inline void set_my_paxos_num(const int32_t paxos_num)
        {
           atomic_exchange((uint32_t*)&my_node_.paxos_num_,  static_cast<uint32_t>(paxos_num));//mod support for arm platform by wangd 202106
        }
        //add:e
        ...
        void set_lease_params(const int64_t lease_duration_us,
                              const int64_t leader_lease_duration_us,
                              const int64_t no_hb_response_duration_us);


        int set_server_alive_stat(const common::ObServer &addr, const bool alive);
      ...
      private:
        ...
        inline void set_local_node_role(const ObRootElectionNode::ObRsNodeRole &role)
        {
          ...
          if (my_node_.get_role() == ObRootElectionNode::OB_LEADER && role != ObRootElectionNode::OB_LEADER)
          {
            YYSYS_LOG(ERROR, "rs[%s] abandon leader role", my_node_.my_addr_.to_cstring());
          }
          else if (my_node_.get_role() != ObRootElectionNode::OB_LEADER && role == ObRootElectionNode::OB_LEADER)
          {
            YYSYS_LOG(ERROR, "rs[%s] change to leader role, old role[%s]",
                      my_node_.my_addr_.to_cstring(), my_node_.get_rs_role_str());
          }
          //add:e
        ...
        {
          atomic_exchange(reinterpret_cast<uint8_t*>(&my_node_.has_leader_), static_cast<uint8_t>(has_leader));
        }
        inline void set_election_state(const ObRsElectStat state)
        {
          YYSYS_LOG(INFO, "before set_election_stat=%s role=%s", get_election_stat_str(), my_node_.get_rs_role_str());
          atomic_exchange(reinterpret_cast<uint32_t*>(&election_state_), static_cast<uint32_t>(state));//mod support for arm platform by wangd 202106
          YYSYS_LOG(INFO, "after set_election_stat=%s role=%s", get_election_stat_str(), my_node_.get_rs_role_str());
        }
        inline const char* get_election_stat_str() const
        {
          switch (election_state_)
          {
            case INIT:
              return "INIT";
            case RS_ELECTION_DOING:
              return "RS_ELECTION_DOING";
            case RS_ELECTION_DONE:
              return "RS_ELECTION_DONE";
            default:
              return "UNKNOWN";
          }
        }
                                     ...
                                     const int64_t rs_term)
        {
          ...
          {
            YYSYS_LOG(WARN, "update lease failed, ret=%d, sender=%s", ret, addr.to_cstring());
          }
      ...
      private:
        ...
        mutable yysys::CThreadMutex nodes_mutex_; //lock for rs nodes
        ...
        int64_t no_hb_response_duration_us_;
        int64_t lease_duration_us_;
        int64_t leader_lease_duration_us_;
