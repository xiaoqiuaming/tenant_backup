...
{   
  ...
  set_vote_for(addr);
  set_has_leader(true);
  reset_hb_timestamps();//add pangtianze [Paxos rs_election] 20170905
}
...
int ObRootElectionNodeMgr::grant_lease()
{
  ...
  yysys::CThreadGuard guard(&nodes_mutex_);
  ...
  for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; ++i)
  {
    if (other_nodes_[i].is_alive_)
    {
      ...
      msg.quorum_scale_ = (int32_t)root_server_->get_config().ups_quorum_scale;
      //add:e

      const char *temp_str = root_server_->get_config().paxos_ups_quorum_scales;
      snprintf(msg.paxos_ups_quorum_scales_, OB_MAX_PAXOS_UPS_QUORUM_SCALES_LENGTH, "%s", temp_str);

      msg.master_cluster_id_ = root_server_->get_master_cluster_id();
      //add liuzy [MultiUPS] [take_cluster_offline_interface] 20160606:b
      msg.has_offline_ = false;
      if (root_server_->get_ups_manager()->is_cluster_offline(other_nodes_[i].my_addr_.cluster_id_))
      {
        msg.has_offline_ = true;
      }
      //add 20160606:e
      ...
      YYSYS_LOG(DEBUG, "send lease to rs, rs=%s rs_term=%ld lease=%ld", to_cstring(other_nodes_[i].my_addr_), msg.current_term_, msg.lease_);
      if (OB_SUCCESS != ret2)
      {
        YYSYS_LOG(WARN, "grant lease to rs err, err=%d, ups=%s",
...
int ObRootElectionNodeMgr::handle_rs_heartbeat(const ObMsgRsHeartbeat &hb)
{
  ...
  if (hb.current_term_ >= get_my_term())
  {
    if (OB_UNLIKELY(ObRootElectionNode::OB_LEADER == get_my_role()))
    {
      ...
      set_local_node_role(ObRootElectionNode::OB_FOLLOWER);
      YYSYS_LOG(ERROR, "leader received heartbeat with higher term, change to follower, rs sender=%s, sender term=%ld, local_term=%ld, err=%d",
    ...
    else if (ObRootElectionNode::OB_CANDIDATE == get_my_role())
    {
      ...
      {
        YYSYS_LOG(WARN, "Dump to file failed! ret=%d.", ret);
      }
      //add:e
    ...
    else if (ObRootElectionNode::OB_FOLLOWER == get_my_role())
    {
        ...
        //mod:e
      {
        ...
        {
          YYSYS_LOG(DEBUG, "follower recevie heartbeat from leader[%s]", hb.addr_.to_cstring());
          set_my_node_info(hb.lease_, true, hb.addr_, hb.current_term_);
          root_server_->set_rt_master(hb.addr_);
          //add wangdonghui [paxos daily merge] 20170510 :b
        ...
        /*if (1 == hb.paxos_num_)
        {
           YYSYS_LOG(INFO, "rootserver paxos number is set as 1, so slave should stop");
        ...
        if ((int64_t)hb.quorum_scale_ != (int64_t)root_server_->get_config().ups_quorum_scale)
        {
          root_server_->set_quorum_scale(hb.quorum_scale_);
        }

        if (OB_LIKELY(0 != strcmp(hb.paxos_ups_quorum_scales_, "version_2")))
        {
          if (0 != ObString::make_string(my_node_.paxos_ups_quorum_scales_).compare(hb.paxos_ups_quorum_scales_))
          {
            set_paxos_ups_quorum_scales(hb.paxos_ups_quorum_scales_);
            ret = root_server_-> set_ups_quorum_scale_from_str(hb.paxos_ups_quorum_scales_);
            root_server_->set_paxos_quorum_scale(hb.paxos_ups_quorum_scales_);

            root_server_->get_config_mgr()->dump2file();
            root_server_->get_config_mgr()->get_update_task().write2stat();
            YYSYS_LOG(INFO, "new paxos_ups_quorum_scales is [%s].", hb.paxos_ups_quorum_scales_);
          }
        }
        //           root_server_->set_quorum_scale(hb.quorum_scale_);
        //}
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "another leader occured, another leader=%s, my leader=%s",
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "unknow rootserver election role");
    }
  ...
  else
  {
    ...
    //YYSYS_LOG(WARN, "receive invalid msg!");
    if (OB_LIKELY(ObRootElectionNode::OB_LEADER != get_my_role()))
    {
      set_local_node_role(ObRootElectionNode::OB_FOLLOWER);
      if (OB_SUCCESS != (ret = rs_update_lease(hb.lease_)))
      {
        YYSYS_LOG(WARN, "update lease failed, ret=%d, sender=%s", ret, hb.addr_.to_cstring());
      }
      YYSYS_LOG(WARN, "receive heartbeat with lower term, term=%ld, sender=%s", hb.current_term_, hb.addr_.to_cstring());
    }
    else
    {
      //rarely enter here
      ret = OB_LEADER_EXISTED;
      YYSYS_LOG(ERROR, "leader received heartbeat with lower term, ignore it, rs sender=%s, sender term=%ld, local_term=%ld, err=%d",
                to_cstring(hb.addr_), hb.current_term_, get_my_term(), ret);
    }
    //mod:e
  }
...
int ObRootElectionNodeMgr::rs_update_lease(const int64_t new_lease) //rs ������Լ
{
  int ret = OB_SUCCESS;
  ...
  else
  {
    ...
    //YYSYS_LOG(WARN, "received lease is earlier than self lease, don't need update self lease");
    YYSYS_LOG(WARN, "received lease[%ld] is earlier than self lease[%ld], don't need update self lease", new_lease, my_node_.lease_);
    //mod:e
  }
  return ret;
}
int32_t ObRootElectionNodeMgr::find_rs_index(const common::ObServer &addr)
{
  ...
  {
    YYSYS_LOG(DEBUG, "find rs index failed, rs[%s] is invalid!", addr.to_cstring());
  }
  return index;
}
...
int ObRootElectionNodeMgr::slave_init_first_meta(const ObTabletMetaTableRow &first_meta_row)
{
  ...
  if (rs_count == get_rs_paxos_number())
  {
    for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
    {
      if (other_nodes_[i].is_alive_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to init first meta in slave rootserver, err=%d, rootserver=%s",
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "real rootserver number not equal with the num needed, real num=%d, need num=%d",
  ...
  for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
  {
    if (other_nodes_[i].is_alive_)
    {
      ...
      if (!is_in_received_array)
      {
        ...
        YYSYS_LOG(INFO, "the rootserver has been not alive, rs=%s", other_nodes_[i].my_addr_.to_cstring());
      }
    }
  }
  //can add rs
...
int32_t ObRootElectionNodeMgr::add_rs(const common::ObServer &rs)
{
  ...
  else if(OB_INVALID_INDEX == (idx = find_rs_info_index(rs))) //mod pangtianze [Paxos rs_election] 20170710:b
  {
    for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
    {
      if (!other_nodes_[i].my_addr_.is_valid())
      {
        ...
        YYSYS_LOG(INFO, "add new rs info into rs_array, rs=%s", rs.to_cstring());
        count++;
        break;
      }
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "all rs count has been up to MAX[%d], can't add new rs", OB_MAX_RS_COUNT);
      }
    }
  }
  //add pangtianze [Paxos rs_election] 20170710:b
  else if (idx >= 0 && !other_nodes_[idx].is_alive_)
  {
    ...
    YYSYS_LOG(INFO, "add new rs info into rs_array again, rs=%s", rs.to_cstring());
    ...
    if(old_cluster_id!= rs.cluster_id_)  //mod pangtianze [Paxos rs_election] 20170713
    {
      ...
      YYSYS_LOG(INFO, "the rootserver info with old cluster_id has been deleted, rs=%s", other_nodes_[idx].my_addr_.to_cstring());
    }
    //add:e
  }
  else if (idx >= 0 && other_nodes_[idx].is_alive_)  //register rs still alive
  {
    ...
    if(other_nodes_[idx].my_addr_.cluster_id_ != rs.cluster_id_)
    {
      //   set_node_alive_stat(other_nodes_[idx], false);
      other_nodes_[idx].my_addr_.cluster_id_ = rs.cluster_id_;
      YYSYS_LOG(INFO, "rs cluster id has changed, rs=%s, cluster_id[%d->%d]",
...
int32_t ObRootElectionNodeMgr::add_rs_register_info(const common::ObServer &rs)
{
  ...
  int32_t count = 0;
  yysys::CThreadGuard guard(&nodes_mutex_);
  YYSYS_LOG(INFO, "will add reigster rs[%s]", rs.to_cstring());
  if (my_node_.my_addr_ == rs)
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "self regist to self, weird erro! rs=%s", rs.to_cstring());
  }
  else if(OB_INVALID_INDEX == (idx = find_rs_info_index(rs)))
  {
    for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
    {
      ...
      {
        other_nodes_[i].my_addr_ = rs;
        other_nodes_[i].hb_resp_timestamp_ = yysys::CTimeUtil::getTime();
        set_node_alive_stat(other_nodes_[i], true);
        YYSYS_LOG(INFO, "add new rs info into rs_array, rs=%s", rs.to_cstring());
        count++;
        break;
  ...
  else if (idx >= 0 && !other_nodes_[idx].is_alive_)
  {
    other_nodes_[idx].reset();
    other_nodes_[idx].my_addr_ = rs;
    other_nodes_[idx].hb_resp_timestamp_ = yysys::CTimeUtil::getTime();
    set_node_alive_stat(other_nodes_[idx], true);
    YYSYS_LOG(INFO, "add new rs info into rs_array again, rs=%s", rs.to_cstring());
  }
  else if (idx >= 0 && other_nodes_[idx].is_alive_)  //register rs still alive
  {
    ...
    {
      YYSYS_LOG(INFO, "rs[%s] is still alive, ignore reigst", rs.to_cstring());
    }
  }
  //add:e
                                                   ...
                                                   const int64_t rs_term, const bool force)
{
  int ret = OB_SUCCESS;
  OB_ASSERT(NULL != root_server_); //add pangtianze [Paxos] 20170717
  YYSYS_LOG(INFO, "receive leader broadcast, rs sender=%s, rs term=%ld", to_cstring(addr), rs_term);
  yysys::CThreadGuard guard(&nodes_mutex_);
  if (OB_UNLIKELY(force && rs_term >= get_my_term()))
  {
    ...
    YYSYS_LOG(INFO, "follow a new leader, rs leader=%s, rs term=%ld", addr.to_cstring(), rs_term);
  }
  else if(rs_term >= get_my_term())
  {
    ...
    if (OB_UNLIKELY(ObRootElectionNode::OB_LEADER == get_my_role()))
    {
      ...
      YYSYS_LOG(ERROR, "leader received leader broadcast, rs sender=%s, err=%d", to_cstring(addr), ret);
    }
    else if (ObRootElectionNode::OB_CANDIDATE == get_my_role())
    {
      if (!get_has_leader())
      ...
      else
      {
        ...
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "candidate maybe in broadcast phase, refuse other leader boradcast package! err = %d", ret);
      }
    }
    else if (ObRootElectionNode::OB_FOLLOWER == get_my_role())
    {
      ...
      else
      {
        ...
        ret = OB_LEADER_EXISTED;
        YYSYS_LOG(WARN, "leader is exist, ret = %d", ret);
      }
    }
    else
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "unknow rs election role, err = %d", ret);
    }
  }
  else
  {
...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(INFO, "receive vote request, rs sender=%s, rs term=%ld", to_cstring(addr), rs_term);

  yysys::CThreadGuard guard(&nodes_mutex_);
  if (ObRootElectionNode::OB_LEADER == get_my_role())
  {
    ret = OB_LEADER_EXISTED;
    YYSYS_LOG(WARN, "leader received vote request, rs sender=%s, err=%d", to_cstring(addr), ret);
    if (-1 == find_rs_index(addr))
    {
      ret = OB_ENTRY_NOT_EXIST;
      YYSYS_LOG(WARN, "rs has been removed from rs list, need re-regist, rs=%s, err=%d", addr.to_cstring(), ret);
    }
  }
  else if (ObRootElectionNode::OB_CANDIDATE == get_my_role())
  {
    if (!get_has_leader() && get_my_term() < rs_term)
    {
      ...
      YYSYS_LOG(INFO, "grant vote to a candidate, rs candidate=%s, rs term=%ld", addr.to_cstring(), rs_term);
    }
    else
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "refuse other candidate vote request package! has_leader=%s, rs candidate=%s, rs term=%ld",
                get_has_leader()?"true":"false", addr.to_cstring(), rs_term);
    }
  }
  else if (ObRootElectionNode::OB_FOLLOWER == get_my_role())
  {
    ...
    else if (!get_has_leader() && get_my_term() < rs_term)
    {
      //grant sender vote
      set_my_node_info(lease, false, addr, rs_term);
      YYSYS_LOG(INFO, "grant vote to a candidate, rs candidate=%s, rs term=%ld", addr.to_cstring(), rs_term);
    ...
    {
      ret = OB_INVALID_TERM;
      YYSYS_LOG(WARN, "vote msg is invalid, ret = %d", ret);
    }
  }
  else
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "unknow rs election role, ret = %d", ret);
  }
  return ret;
}
...
int ObRootElectionNodeMgr::leader_broadcast(bool force)
{
  ...
  int64_t now = yysys::CTimeUtil::getTime();
  int ret2 = OB_SUCCESS;
  yysys::CThreadGuard guard(&nodes_mutex_); //add pangtianze [Paxos rs_election] 20170905
  if (force)
  {
    if (ObRoleMgr::MASTER == role_mgr_->get_role()
        || ObRootElectionNode::OB_LEADER == get_my_role() )
    {
      ret2 = OB_ERROR;
      YYSYS_LOG(WARN, "slave cannot force leader broadcast, role[%s], election_role[%s]",
                role_mgr_->get_role_str(), my_node_.get_rs_role_str());
    }
    else
    {
      start_election(); //add pangtianze [Paxos rs_election] 20170905
  ...
  if (OB_SUCCESS == ret2)
  {
    ...
    for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
    {
      if (other_nodes_[i].is_alive_)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to send leader broadcast msg, err=%d, targer rootserver=%s",
                    ret, node.my_addr_.to_cstring());
        }
        else
        {
          YYSYS_LOG(INFO, "send leader broadcast msg succ, target rootserver=%s", node.my_addr_.to_cstring());
        }
      }
    }
  }
...
int ObRootElectionNodeMgr::request_votes()
{
  ...
  raise_my_term();
  YYSYS_LOG(INFO, "new rs term=%ld", get_my_term());
  vote_count_ = 1;
  ...
  for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
  {
    if (other_nodes_[i].is_alive_)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to send vote request msg, err=%d, target rootserver=%s",
                  ret, node.my_addr_.to_cstring());
      }
      else
      {
        YYSYS_LOG(INFO, "send vote request msg succ, target rootserver=%s", node.my_addr_.to_cstring());
      }
    }
  }
  return ret;
...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(INFO, "receive leader broadcast response, rs sender=%s, rs term=%ld",
            lb_resp.addr_.to_cstring(), lb_resp.term_);
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "rs not exist, rs=%s", lb_resp.addr_.to_cstring());
  }
  if (OB_SUCCESS == ret)
  {
    broadcast_count_ = 1;
    ...
    if (broadcast_count_ > (get_rs_paxos_number() / 2))
    {
      ...
      YYSYS_LOG(INFO, "candidate has get most broadcast confirm from all rootservers, confirm count=%d, paxos number=%d",
...
int ObRootElectionNodeMgr::handle_vote_request_resp(const ObMsgRsRequestVoteResp &rv_resp)
{
  int ret = OB_SUCCESS;
  OB_ASSERT(NULL != root_server_); //add pangtianze [Paxos] 20170717
  YYSYS_LOG(INFO, "receive vote request response, rs sender=%s, rs term=%ld",
            rv_resp.addr_.to_cstring(), rv_resp.term_);
  yysys::CThreadGuard guard(&nodes_mutex_);
  ...
  {
    YYSYS_LOG(INFO, "candidate finds leader, leader rs_term=%ld, local rs_term=%ld, switch to follower", rv_resp.term_, get_my_term());
    set_local_node_role(ObRootElectionNode::OB_FOLLOWER);
  ...
  else if (!rv_resp.is_granted_ && get_my_term() < rv_resp.term_)
  {
    ...
    set_local_node_role(ObRootElectionNode::OB_FOLLOWER);
    set_my_term(rv_resp.term_);
    YYSYS_LOG(INFO, "candidate finds higher rs_term=%ld, rs sender=%s, switch to follower", rv_resp.term_, rv_resp.addr_.to_cstring());
  }
  if (OB_SUCCESS == ret)
  {
    vote_count_ = 1;
    ...
    if (vote_count_ > (get_rs_paxos_number() / 2))
    {
      ...
      YYSYS_LOG(INFO, "candidate has get most votes from all rootservers, vote count=%d, paxos number=%ld",
...
int ObRootElectionNodeMgr::start_leader_broadcast(bool force)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "worker_ is null");
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "failed to submit leader broadcast task, is_force=%d, err=%d", force, ret);
    }
  }
  return ret;
}
int ObRootElectionNodeMgr::start_request_votes()
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "failed to submit vote request task, err=%d", ret);
    }
  }
  return ret;
}
...
int ObRootElectionNodeMgr::get_leader(common::ObServer &leader)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "no leader exist, rs term=%ld, election state=%s", get_my_term(), get_election_stat_str());
  }
  return ret;
}          
...
bool ObRootElectionNodeMgr::need_leader_change_role(const int64_t now)
{
  ...
  if (ret && get_rs_paxos_number() > 1)
  {
    ...
    {
      ret = false;
      YYSYS_LOG(WARN, "invalid rs heartbeat timestamp, hb=%ld", ts);
    }
    else if (ts + no_hb_response_duration_us_ > now)
    {
      ...
      for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1 ; i++)
      {
        if(other_nodes_[i].is_alive_ && (other_nodes_[i].hb_resp_timestamp_ == 0 || (other_nodes_[i].hb_resp_timestamp_ + lease_duration_us_ < now)))
        {
          ...
          YYSYS_LOG(INFO, "rootserver[%s] -> offline",other_nodes_[i].my_addr_.to_cstring());
        }
      }
      //add:e
    }
    ...
    {
      ret = true;
      YYSYS_LOG(WARN, "leader has failed to keep contact with most rs, middle_ts=%ld", ts);
    }
  }
  ...
  {
    ret = false;
    YYSYS_LOG(WARN, "unexpected paxos number: [%d]" ,get_rs_paxos_number());
  }
  else
  {
    ret = false;
...
int64_t ObRootElectionNodeMgr::get_middle_timestamp(const int alive_rs_count)
{
  ...
  {
    YYSYS_LOG(WARN, "no other rs alive");
  }
  return ts;
}
  ...
  else
  {
    ...
    for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
    {
      if (other_nodes_[i].is_alive_ && other_nodes_[i].paxos_num_ != local_paxos_num)
      {
        ...
        index = i;
        //add:e
        YYSYS_LOG(WARN, "paxos num not be changed, rs[%s], paxos_num[%d]",
...
void ObRootElectionNodeMgr::if_all_quorum_in_rs_changed(char *buf, const int64_t buf_len, int64_t& pos) const
{
  //add chujiajia [Paxos rs_election] 20151230:b
  int32_t index = OB_INVALID_INDEX;
  //add:e
  ...
  else
  {
    yysys::CThreadGuard guard(&nodes_mutex_);
    bool all_changed = true;
    const char * local_paxos_quorum_scales_str = root_server_->get_config().paxos_ups_quorum_scales;
    for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
    {
      ObString local_str = ObString::make_string(local_paxos_quorum_scales_str);
      //if (other_nodes_[i].is_alive_ && other_nodes_[i].quorum_scale_ != local_quorum_scale)
      if (other_nodes_[i].is_alive_ && (local_str.compare((char *)other_nodes_[i].paxos_ups_quorum_scales_)))
      {
        all_changed = false;
        //add chujiajia [Paxos rs_election] 20151230:b
        index = i;
        //add:e
        //YYSYS_LOG(WARN, "quorum scale not be changed, rs[%s], quorum_scale[%d]",
        //        other_nodes_[index].my_addr_.to_cstring(), other_nodes_[index].quorum_scale_);

        YYSYS_LOG(WARN, "quorum scale not be changed, rs[%s], paxos_ups_quorum_scales[%s]"
                  "but the current config in master rs: paxos_ups_quorum_scales[%s]",
                  other_nodes_[index].my_addr_.to_cstring(), other_nodes_[index].paxos_ups_quorum_scales_,
                  local_paxos_quorum_scales_str);
        break;
      }
    }
    if (all_changed)
    {
      ...
      databuff_printf(buf, buf_len, pos, "quorum scale in all rs are same, current paxos_ups_quorum_scales=[%s]\n", local_paxos_quorum_scales_str);
    }
    else
    {
      ...
      databuff_printf(buf, buf_len, pos,
                      "quorum scale in rootservers [%s], quorum_scale is [%s]; "
                      "but the current config in master rs are quorum scale[%s], Please do change_command again!\n",
                      other_nodes_[index].my_addr_.to_cstring(), other_nodes_[index].paxos_ups_quorum_scales_,
                      local_paxos_quorum_scales_str);
    }
  }
...
void ObRootElectionNodeMgr::start_election()
{
  ...
  YYSYS_LOG(INFO, "start rootserver election, rs term=%ld, start time=%ld", get_my_term(), begin_time_);
  //add:e
}
void ObRootElectionNodeMgr::end_election()
{
  set_election_state(RS_ELECTION_DONE);
  end_time_ = yysys::CTimeUtil::getTime();
  ObServer leader;
  if (OB_SUCCESS == get_leader(leader))
  {
    YYSYS_LOG(INFO, "end rootserver election, rs term=%ld, leader rootserver=%s, start time=%ld, end time=%ld, duration time=[%ld]ms",
              get_my_term(), leader.to_cstring(), begin_time_, end_time_, (end_time_ - begin_time_)/1000);
  }
  //add chujiajia [Paxos rs_election] 20151111:b
  else
  {
    YYSYS_LOG(WARN, "end rootserver election, but can not get leader info!");
  }
  //add:e
}
...
void ObRootElectionNodeMgr::get_broadcast_result(int64_t &term, bool &result)
{
  ...
  term = broadcast_result_.term_;
  result = broadcast_result_.result_;
}
...
{
  common::ObSpinLockGuard guard(broadcast_result_lock_);
  broadcast_result_.term_ = term;
  broadcast_result_.result_ =result;
}
...
int ObRootElectionNodeMgr::excute_election_vote(const int64_t now)
{
  ...
  if (election_vote_lease_us_ < now)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "start vote request failed, err=%d", ret);
    }
  }
  return ret;
}
int ObRootElectionNodeMgr::excute_election_broadcast(const int64_t now)
{
  ...
  if (election_broadcast_lease_us_ < now)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "start leader broadcast failed, err=%d", ret);
    }
  }
  return ret;
}
...
bool ObRootElectionNodeMgr::success_vote()
{
  ...
  {
    YYSYS_LOG(INFO, "request vote success");
    is_success = true;
  }
  return is_success;
}
...
bool ObRootElectionNodeMgr::success_broadcast()
{
  ...
  {
    YYSYS_LOG(INFO, "leader broadcast success");
    is_success = true;
  }
  return is_success;
}
...
int ObRootElectionNodeMgr::check_lease()
{
  ...
  if (ObRootElectionNode::OB_LEADER == get_my_role())
  {
    ...
    else
    {
      ...
      if (now > my_node_.lease_)
      {
        ...
        YYSYS_LOG(ERROR, "leader lease expired, will switch to follower");
        set_local_node_role(ObRootElectionNode::OB_FOLLOWER);
        reset_vote_for();
        set_has_leader(false);
        //reset_all_other_nodes(); //no need to reset
        YYSYS_LOG(INFO, "leader begin to switch to follower");
  ...
  else if (ObRootElectionNode::OB_CANDIDATE == get_my_role())
  {
    ...
    {
      YYSYS_LOG(WARN, "election duration is too long, begin time=%ld, now=%ld, will switch to follower",
                begin_time_, now);
    ...
    if (get_rs_paxos_number() > 0)
    {
      yysys::CThreadGuard guard(&mutex_);
      if (vote_request_doing_ && !broadcast_doing_)
      {
        //if (success_vote())
        if (success_vote() || 1 == get_rs_paxos_number())
        {
          YYSYS_LOG(INFO, "candidate request vote successed, will begin leader broadcast.");
          set_has_leader(true);
          ret = excute_election_broadcast(now);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "leader broadcast error, err=%d", ret);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "excute election failed, then will start a new election");
          ret = excute_election_vote(now);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "vote request error, err=%d", ret);
          }
        }
      }
      if (!vote_request_doing_ && broadcast_doing_)
      {
        if (success_broadcast() || 1 == get_rs_paxos_number())
        {
          broadcast_doing_ = false;
          YYSYS_LOG(INFO, "candidate broadcast successed, will change to leader.");
          ...
          end_election();
          YYSYS_LOG(INFO, "elecion success!");
        }
        else
        {
          broadcast_doing_ = false;
  ...
  else if (ObRootElectionNode::OB_FOLLOWER == get_my_role())
  {
    ...
    if (ObRoleMgr::MASTER == role_mgr_->get_role())
    {
      ...
      {
        role_mgr_->set_state(ObRoleMgr::STOP);
        YYSYS_LOG(ERROR, "rootserver switch to slave failed, will stop, err = %d", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "rootserver switch to slave success");
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "unknow rs election role in rs check thread");
  }
  return ret;
}
//add pangtianze [Paxos bugfix:old leader still in __all_server after it is down] 20161108:b
...
void ObRootElectionNodeMgr::print_lease_info(const bool is_leader)
{
  if (is_leader)
  {
    YYSYS_LOG(INFO, "leader lease=%ld", my_node_.lease_);
    for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
    {
      if (other_nodes_[i].my_addr_.is_valid() && other_nodes_[i].is_alive_)
      {
        YYSYS_LOG(INFO, "follower=%s, lease=%ld", other_nodes_[i].my_addr_.to_cstring(), other_nodes_[i].lease_);
      }
      else if (other_nodes_[i].my_addr_.is_valid())
      {
        YYSYS_LOG(INFO, "follower=%s, lease=%ld, not alive", other_nodes_[i].my_addr_.to_cstring(), other_nodes_[i].lease_);
      }
    }
  }
  else
  {
    YYSYS_LOG(INFO, "my lease=%ld", my_node_.lease_);
  }
}
...
int64_t ObRootElectionNodeMgr::get_random_time()
{
  ...
  //mod pangtianze [Paxos rs_election_priority] 20170911
  //random_time = rand()%100000 + 100000; //100ms~200ms
  int64_t base_time = 150000 * ((int64_t)root_server_->get_config().rs_election_priority - 1);
  random_time = rand()%100000 + base_time;
  //mod:e
  YYSYS_LOG(INFO, "election random %ld ms, prioriry=%ld", random_time/1000, (int64_t)root_server_->get_config().rs_election_priority);
  return random_time;
}
...
void ObRootElectionNodeMgr::get_all_alive_servers(ObServer *servers, int32_t &server_count)
{
  ...
  if (NULL == servers)
  {
    YYSYS_LOG(ERROR, "servers array ptr is null");
  }
  //add:e
  else
  {
...
//add liuzy [MultiUPS] [take_cluster_offline_interface] 20160726:b
void ObRootElectionNodeMgr::get_all_alive_rs_by_cluster_id(ObServer *servers, int32_t & server_count,
                                                           const int64_t cluster_id)
{
  server_count = 0;
  for (int32_t i = 0; i< OB_MAX_RS_COUNT - 1; i++)
  {
    if (other_nodes_[i].is_alive_ && other_nodes_[i].my_addr_.is_valid() && cluster_id == other_nodes_[i].my_addr_.cluster_id_)
    {
      servers[server_count] = other_nodes_[i].my_addr_;
      server_count++;
    }
...
int ObRootElectionNodeMgr::handle_rs_heartbeat_resp(const common::ObMsgRsHeartbeatResp &hb_resp, const int64_t time)
{
  ...
  if (get_my_term() < hb_resp.current_term_)
  {
    ...
    YYSYS_LOG(WARN, "find higher term from heartbeat response, set leader term as %ld", get_my_term());
  }
  if (OB_INVALID_INDEX == (idx = find_rs_index(hb_resp.addr_)))
  {
    YYSYS_LOG(WARN, "rs not exist, addr=%s", hb_resp.addr_.to_cstring());
    ret = OB_ENTRY_NOT_EXIST;
  }
  else
  {
     ...
     atomic_exchange((uint32_t*)&other_nodes_[idx].paxos_num_, static_cast<uint32_t>(hb_resp.paxos_num_));//mod support for arm platform by wangd 202106
     YYSYS_LOG(DEBUG, "update rs heartbeat response time and paxos num, rs addr=%s, timestamp=%ld, paxos num=%d", hb_resp.addr_.to_cstring(), time, hb_resp.paxos_num_);
     //atomic_exchange((uint32_t*)&other_nodes_[idx].quorum_scale_, hb_resp.quorum_scale_);
     //YYSYS_LOG(DEBUG, "update rs heartbeat response time and quorum scale, rs addr=%s, timestamp=%ld, quorum scale=%d", hb_resp.addr_.to_cstring(), time, hb_resp.quorum_scale_);
     other_nodes_[idx].set_paxos_ups_quorum_scales(hb_resp.paxos_ups_quorum_scales_);
  }
  return ret;
}
...
int ObRootElectionNodeMgr::handle_change_config_request(const common::ObServer &addr, const int64_t rs_term, const int32_t new_paxos_num)
{
  ...
  {
    ret = OB_LEADER_EXISTED;
    YYSYS_LOG(WARN, "leader received change config request, rs sender=%s, err=%d", to_cstring(addr), ret);
  }
  else if (ObRootElectionNode::OB_CANDIDATE == get_my_role()
           || ObRootElectionNode::OB_FOLLOWER == get_my_role())
  {
    ...
    if(rs_term >= get_my_term())
    {
      ...
      else
      {
        ...
        root_server_->set_paxos_num(new_paxos_num);
        YYSYS_LOG(INFO, "Rootserver save new paxos num, new_paxos_num = %d, rs_paxos_number = %d", new_paxos_num, get_rs_paxos_number());
      }
      //add:e
    ...
    {
      ret = OB_INVALID_TERM;
      YYSYS_LOG(WARN, "change_config_request is msg invalid!, err = %d", ret);
    }
  }
  else
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "unknow rs election role");
  }
  //add pangtianze [Paxos rs_election] 20170712:b
  {
    YYSYS_LOG(INFO, "slave remove invalid rootserver info");
...
void ObRootElectionNodeMgr::is_cluster_alive_with_rs(bool *is_cluster_alive) const
{
  ...
  {
    YYSYS_LOG(ERROR, "is_cluster_aliave array ptr is null");
  }
  //add:e
  else
  {
...
void ObRootElectionNodeMgr::remove_invalid_info()
{
  ...
  for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
  {
    ...
    if (other_nodes_[i].my_addr_.is_valid() && !other_nodes_[i].is_alive_)
    {
      ...
      YYSYS_LOG(INFO, "the rootserver info has been deleted, rs=%s", rs.to_cstring());
    }
  }
}
//add:e

void ObRootElectionNodeMgr::set_lease_params(const int64_t lease_duration_us,
                                             const int64_t leader_lease_duration_us,
                                             const int64_t no_hb_response_duration_us)
{
  if (lease_duration_us <= leader_lease_duration_us + no_hb_response_duration_us)
  {
    YYSYS_LOG(ERROR, "some rs lease params about rootserver need satify 'lease_duration_us > leader_lease_duration_us + no_hb_response_duration_us', "
              "follower_lease=%ld, leader_lease=%ld, no_hb_response_duration_us=%ld",
              lease_duration_us, leader_lease_duration_us, no_hb_response_duration_us);
    lease_duration_us_ = lease_duration_us;
    leader_lease_duration_us_ = 1 * 1000 * 1000;
    no_hb_response_duration_us_ = lease_duration_us - 2 * 1000 * 1000;
    YYSYS_LOG(INFO, "rs lease params has been set default, "
              "follower_lease=%ld, leader_lease=%ld, no_hb_response_duration_us=%ld",
              lease_duration_us_, leader_lease_duration_us_, no_hb_response_duration_us_);
  }
  else
  {
    lease_duration_us_ = lease_duration_us;
...
int ObRootElectionNodeMgr::set_server_alive_stat(const common::ObServer &addr, const bool alive)
{
  int ret = OB_ENTRY_NOT_EXIST;
  yysys::CThreadGuard guard(&nodes_mutex_);
  if (my_node_.my_addr_ == addr)
  {
    my_node_.is_alive_ = alive;
    ret = OB_SUCCESS;
  }
  else
  {
    for (int32_t i = 0; i < OB_MAX_RS_COUNT - 1; i++)
    {
      if (other_nodes_[i].my_addr_ == addr && other_nodes_[i].my_addr_.is_valid())
      {
        other_nodes_[i].is_alive_ = alive;
        ret = OB_SUCCESS;
        break;
      }
    }
