...
int ObRootClusterManager::set_replicas_num(const int32_t *replicas_num)
{
  ...
  if(NULL == replicas_num)
  {
    ...
    YYSYS_LOG(WARN, "Invalid arguments");
  }
  else
  {
    int32_t total = 0;
    ...
    {
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "total replica num is large than OB_MAX_COPY_COUNT, total=%d", total);
    }
    else
    {
      for(int32_t cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
                                                        ...
                                                        const int32_t total_replicas_num)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "Invalid arguments for calculate tablet replicas num, total_replicas_num=%d, ret=%d",
              total_replicas_num, ret);
  }
  //add bingo [Paxos all zero replica num handle] 20170811:b
  else if(0 == total_num)
  {
    for(int32_t cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
    {
      replicas_num[cluster_id] = 1;
    }
    YYSYS_LOG(ERROR,"total cluster replica num is zero, for insurance to set each cluster with 1");
  }
  //add:e
...
int ObRootClusterManager::get_cluster_tablet_replicas_num(int32_t *replicas_num) const
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(INFO, "Invalid arguments for get tablet replicas num, ret=%d", ret);
  }
  else
  {
    for(int32_t cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
...
void ObRootClusterManager::reset_cluster_tablet_replicas_num()
{
  for(int32_t cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
  {
    replicas_num_for_each_cluster_[cluster_id] = 0;
...
{
  int ret = OB_SUCCESS;
  if(cluster_id < 0 || cluster_id > OB_MAX_CLUSTER_ID || replicas_num < 0)
  {
    YYSYS_LOG(ERROR, "invalid cluster_id or replica_num, cluster_id=%d, replica_num=%d", cluster_id, replicas_num);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "total replica num is larger than OB_MAX_COPY_COUNT, total_num=%d", total_num);
...
{
  int ret = OB_SUCCESS;
  if(master_cluster_id < 0 || master_cluster_id > OB_MAX_CLUSTER_ID)
  {
    ret = OB_CLUSTER_ID_ERROR;
    YYSYS_LOG(WARN, "fail to set master cluster id, master_cluster_id=%d, ret=%d", master_cluster_id, ret);
  }
  else
  {
    //add pangtianze [Paxos Cluster.Balance] 20161019
      YYSYS_LOG(INFO, "set master cluster id, old=%d, new=%d", master_cluster_id_, master_cluster_id);
    //add:e
...
int ObRootClusterManager::split_config_str(const char* str, int32_t* values)
{
    ...
    if (NULL == str)
    {
      ...
      YYSYS_LOG(ERROR, "config string is NULL");
    }
    //add pangtianze [Paxos] 20170717:b
    else if (NULL == values)
    {
        rc = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "values array ptr is NULL");
    }
    //add:e
    else if (static_cast<int64_t>(strlen(str)) >= OB_MAX_VARCHAR_LENGTH)
    {
      rc = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "config str length >= OB_MAX_VARCHAR_LENGTH: length=[%d], ret=%d", static_cast<int>(strlen(str)), rc);
    }
    else
    {
...
int ObRootClusterManager::parse_config_value(const char* cluster_replica_num_param, int32_t* replicas_num)
{
    ...
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "cluster_replica_num_param array ptr is null");
    }
    else if (NULL == replicas_num)
    {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "replicas_num array ptr is null");
    }
    else
    {
    //add:e
      ...
      {
          YYSYS_LOG(ERROR, "parse cluster replica number config error, param=%s", cluster_replica_num_param);
      }
      else
      {
          for(int32_t cluster_id = 0;  cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
          {
            replicas_num[cluster_id] = values[cluster_id];
          }
      }
    }
    return ret;
...
int ObRootClusterManager::check_total_replica_num(int32_t *replica_num)
{
    ...
    {
    //add:e
    for (int cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
       {
           ...
           {
               ret = OB_ERROR;
               YYSYS_LOG(ERROR, "eplica num is invalird, cluster_id=%d, replica_num=%d",
       ...
       {
           ret = OB_REPLICA_NUM_OVERFLOW;
           YYSYS_LOG(ERROR, "total replica num is invalid, total_num=%d, ret=%d", total_num, ret);
       }
    }
    return ret;
}
