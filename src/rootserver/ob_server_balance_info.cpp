  ...
  namespace rootserver
  {
    ...
    double ObServerDiskInfo::get_utilization_ratio() const
    {
      ...
      if (capacity_ > 0)
      {
        ...
        {
          ret = -1;
          YYSYS_LOG(WARN, "invalid utilization, used=%ld capacity=%ld ratio=%f", used_, capacity_, ret);
        }
        else if (100 < ret)
        {
          ret = 100;
    ...
    {
      YYSYS_LOG(INFO, "disk info capacity = %ld used = %ld ratio=%d", capacity_, used_, get_percent());
    }
