...
int ObRestartServer::restart_servers()
{
  ...
  {
    ret = common::OB_ERROR;
    YYSYS_LOG(WARN, "ObRestartServer check inner stat fail");
  }
  ...
  if(common::OB_SUCCESS == ret && has_wait_restart)
  {
    ...
    for(it = server_manager_->begin(); it != server_manager_->end(); ++it)
    {
      if(it->wait_restart_ && it->can_restart_)
      {
        ...
        if(common::OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "restart cs fail:ret[%d], chunkserver[%s]", ret, chunk_server.to_cstring());
        }
        else
        {
          shutdown_cs(it);
...
void ObRestartServer::shutdown_cs(ObServerStatus* it)
{
  ...
  YYSYS_LOG(INFO,"shutdown cs, now:%ld, lease_duration:%s", now, config_->cs_lease_duration_time.str());
  ...
  log_worker_->server_is_down(it->server_, now,it->cluster_id_);
  //mod:e
  /*mod:e*/
  ...
  {
    YYSYS_LOG(ERROR, "root_table_for_query_ = NULL, server_index=%ld", it - server_manager_->begin());
  }
}
  ...
  for (it = root_table_->begin(); it != root_table_->sorted_end(); ++it)
  {
    ...
    for (int i = 0; i < expected_replicas_num; ++i)
    {
      ...
      if (common::OB_INVALID_INDEX != server_info_index)
      {
        ...
        {
          YYSYS_LOG(WARN, "get server status fail:server index[%d]", server_info_index);
        }
      }
    }
