  ...
  namespace rootserver
  {
    ...
    class ObIndexMonitor
    {
      ...
      public:
        ...
        void set_start_version(const int64_t& start_version)
        { start_version_ = start_version; }
        //add e
        //add liumz, [secondary index version management]20160413:b
        int64_t get_start_version()
        { return start_version_; }
        bool is_last_finished()
        { return last_finished_; }
        //add:e
        IndexPhase get_index_phase()
        { return index_phase_; }

        int serialize_rs_index_merge_stat(char *buf, const int64_t buf_len, int64_t &pos) const;

        void get_is_cluster_complete(bool *is_cluster_complete) const;
      ...
      private:
        ...
        int check_tablet_complete_in_clusters(uint64_t index_id);

      private:
        yysys::CThreadMutex monitor_mutex_;
        yysys::CThreadMutex mission_mutex_;
        ...
        bool last_finished_;
        volatile int64_t index_merge_start_time_;

        bool is_cluster_complete_[OB_MAX_CLUSTER_COUNT];
    };
