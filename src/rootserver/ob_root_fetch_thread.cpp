  ...
  namespace rootserver
  {
    ...
    int ObRootFetchThread::wait_recover_done()
    {
      ...
      while (!is_recover_done_ && !_stop)
      {
        ...
        if ((count % 100) == 0)
        {
          YYSYS_LOG(INFO, "wait recover from check point used %d seconds", (count/10));
        }
        usleep(WAIT_INTERVAL);
      }
      YYSYS_LOG(INFO, "wait recover done:recover_ret[%d], recover_done[%d], stop[%d]", recover_ret_, is_recover_done_, _stop);
      return recover_ret_;
    }
    ...
    int ObRootFetchThread::got_ckpt(uint64_t ckpt_id)
    {
      ...
      YYSYS_LOG(DEBUG, "fetch got checkpoint %lu", ckpt_id);

      if (log_manager_ == NULL)
      {
        recover_ret_ = common::OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "log manager is not set");
      }
      else
      {
      ...
      YYSYS_LOG(INFO, "recover from checkpoint %lu return %d", ckpt_id, recover_ret_);
