                               ...
                               ObTabletReportInfoList &delete_list, const int64_t timeout)
{
  ...
  static yysys::CThreadMutex delete_msg_lock;
  int ret = OB_SUCCESS;

  yysys::CThreadGuard guard(&delete_msg_lock);
  ...
  do
  {
    ...
    for (int i = 0; i < delete_list.get_tablet_size(); ++i)
    {
      if (-1 != tablets[i].tablet_location_.chunkserver_.get_port())
      {
            ...
            || (-1 != server_idx && server_idx == tablets[i].tablet_location_.chunkserver_.get_port()))
        {
          ...
          {
            YYSYS_LOG(ERROR, "add tablet error");
            break;
          }
    ...
    if (0 < delete_msg.get_tablet_size())
    {
      ...
      {
        YYSYS_LOG(INFO, "try to delete tablet replica, cs=%s tablet=%s version=%ld row_count=%ld size=%ld crc=%ld i=%d", 
      ...
      if ( server_status == NULL || 
          (server_status->status_ != ObServerStatus::STATUS_SERVING && server_status->status_ != ObServerStatus::STATUS_SHUTDOWN))
      {
        YYSYS_LOG(WARN, "failed to delete tablets msg, cs=%s, server_status is not serving or shut_down ", to_cstring(cs));
      }
      //add:e
      else if (OB_SUCCESS != (ret = rpc_stub.delete_tablets(cs, delete_msg, timeout)))
      {
        YYSYS_LOG(WARN, "failed to send delete tablets msg, err=%d cs=%s", ret, to_cstring(cs));
      }
      else
      {
        YYSYS_LOG(INFO, "delete tablet replicas from cs, count=%ld cs=%s", 
