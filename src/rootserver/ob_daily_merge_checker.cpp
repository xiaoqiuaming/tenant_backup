...
//add  peiouya [MultiUPS] [UPS_Manage_Function] 20150426:b
...
bool ObRowChecksum::is_equal(const ObRowChecksum &slave) const
{
  ...
  {
    YYSYS_LOG(ERROR, "unequal data_version, master_data_version:%ld slave_data_version:%ld", data_version_, slave.data_version_);
    ret = false;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "unequal table num, master_table_num:%ld, slave_table_num:%ld", rc_array_.count(), slave.rc_array_.count());
      ret = false;
    }
    ...
    for(; i < rc_array_.count(); i++)
    {
      for(j = 0; j < slave.rc_array_.count(); j++)
      {
        if(rc_array_[i].table_id_ == slave.rc_array_[j].table_id_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "unequal table row_checksum, table_id=%ld, master_checksum=%ld, slave_checksum=%ld", rc_array_[i].table_id_, rc_array_[i].row_checksum_, slave.rc_array_[j].row_checksum_);
            ret = false;
          }
          break;
        }
      ...
      {
        YYSYS_LOG(ERROR, "unequal table num, master table_id:%ld not found in slave", rc_array_[i].table_id_);
        ret = false;
      }
    }
  }
...
void ObDailyMergeChecker::MonitorRowChecksumTask::runTimerTask()
{
      ...
      root_server_->get_config().monitor_row_checksum)
  {
    ...
    {
      YYSYS_LOG(WARN, "slave cluster get row_checksum failed ret:%d", ret);
    }
    else if(OB_ENTRY_NOT_EXIST == ret)
    {
      YYSYS_LOG(INFO, "slave cluster get row_checksum failed, is merging");
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SUCCESS != ret && OB_ENTRY_NOT_EXIST != ret)
      {
        YYSYS_LOG(WARN, "master cluster get row_checksum failed, ret:%d", ret);
      }
      else if(OB_ENTRY_NOT_EXIST == ret)
      {
        YYSYS_LOG(INFO, "master cluster get row_checksum failed, is merging");
      }
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(INFO, "==========> check row_checksum OKay <==========");
      }
      else
      {
        YYSYS_LOG(WARN, "==========> check row_checksum failed! <==========");
      }

      YYSYS_LOG(INFO, "==========> master row_checksum <==========\n%s", to_cstring(master_));
      YYSYS_LOG(INFO, "==========> slave row_checksum <==========\n%s", to_cstring(slave_));
    }
  }
  else
  {
    YYSYS_LOG(INFO, "do not check row_checksum now, for obi_role:%s or switch:%s", obi_role.get_role_str(), config.monitor_row_checksum.str());
  }
  ...
  {
    YYSYS_LOG(ERROR, "timer schedule fail, ret: [%d]", ret);
  }
}

void ObDailyMergeChecker::run(yysys::CThread * thread, void * arg)
{
  UNUSED(thread);
  UNUSED(arg);
  int err = OB_SUCCESS;
  YYSYS_LOG(INFO, "[NOTICE] merge checker thread start");
  ...
  {
    YYSYS_LOG(ERROR, "init timer fail, ret: [%d]", err);
  }
  else if(OB_SUCCESS != (err = timer_.schedule(check_task_,
          root_server_->get_config().monitor_row_checksum_interval, false)))
  {
    YYSYS_LOG(ERROR, "timer schedule fail, ret: [%d]", err);
  }
  SET_THD_NAME_ONCE("rs-dailymerg");
  ...
  while (!_stop)
  {
    ...
    {
      //mod peiouya [MultiUPS] [UPS_Manage_Function] 20150426:b
      /*
      ...
      */
      common::ObArray<ObServer> paxos_array;
      root_server_->get_all_master_ups_info(paxos_array, false); 
      if (0 == paxos_array.count())
      {
        YYSYS_LOG(INFO, "no all master ups right now, sleep for next round");
        sleep(1);
        continue;
      }
      //mod 20150426:e
      ...
      {
        YYSYS_LOG(WARN, "failed to get frozen version, err=%d", err); 
      }
      else
      {
    ...
    {
      YYSYS_LOG(ERROR, "daily merge error!!, message info is: %s", root_server_->get_daily_merge_error_msg());
    }
    else
    {
      ...
      if (root_server_->is_master() && (root_server_->last_frozen_time_ > 0))
      {
        ...
        if (root_server_->get_obi_role().get_role() == ObiRole::MASTER)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to modify init index status. ret=%d", err);
          }
        }
        //add:e*/
        ...
        if (!root_server_->check_all_tablet_safe_merged())
        {
...

          {
            ...
            if (count > 0)
            {
              YYSYS_LOG(WARN, "delete dropped tables' tablets in root table:count[%ld], err[%d]", count, err);
            }
          ...
          if (root_server_->last_frozen_time_ != 1)
          {
            ...
            // mod e
            {
              YYSYS_LOG(ERROR, "merge is too slow, start at:%ld, now:%ld, used_time:%ld, max_merge_duration_:%ld",
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "build new root table ok:last_version[%ld]", frozen_version);
          ...
          root_server_->handle_orphan_index();
          root_server_->set_if_orphan_index_exist(false);
          ...
          if (NULL == out_schema)
          {
            ...
            YYSYS_LOG(WARN, "fail to new schema_manager. ret=%d", err);
          }//add:e
          ...
          {
            YYSYS_LOG(WARN,"failed to get index list");
          }
          else if (root_server_->config_.monitor_column_checksum)
          {
            for(int64_t i=0;i < index_id_list.count();i++)
            {
             /*
              ...
              root_server_->get_cluster_tablet_replicas_num(replica_num_each_cluster);
              bool is_right = true;
              ...
              for (int cluster_idx = 0; cluster_idx < OB_MAX_COPY_COUNT ; cluster_idx++)
              {
                if (is_cluster_alive[cluster_idx] && replica_num_each_cluster[cluster_idx] > 0)
                {
                  ...
                  //bool is_right = true;
                  YYSYS_LOG(INFO, "check index table tid:%ld", index_id_list.at(i));
                  if(OB_SUCCESS != (err = root_server_->check_column_checksum(cluster_idx, index_id_list.at(i), frozen_version, is_right)))
                  {
                    YYSYS_LOG(ERROR, "check column checksum failed, index id[%ld], cluster_id[%d]", index_id_list.at(i), cluster_idx);
                  }
                  else if (is_right)
                  {
                    break;
              ...
              if (!is_right)
              {
                for (int cluster_idx = 0; cluster_idx < OB_MAX_COPY_COUNT ; cluster_idx++)
                {
                  if (is_cluster_alive[cluster_idx] && replica_num_each_cluster[cluster_idx] > 0)
                  {
                    ...
                    if (!is_right)
                    {
                      if(OB_SUCCESS != (err = root_server_->modify_index_process_info(cluster_idx, index_id_list.at(i), ERROR)))
                      {
                        YYSYS_LOG(WARN, "failed to change idx table status:%ld, cluster_id[%d]", index_id_list.at(i), cluster_idx);
                      }
                    }
                  }
                }
                ...
                bool is_right = true;
                YYSYS_LOG(INFO, "check index table tid:%ld", index_id_list.at(i));
                if(OB_SUCCESS != (err = root_server_->check_column_checksum(OB_ALL_CLUSTER_FLAG, index_id_list.at(i), frozen_version, is_right)))
                {
                    YYSYS_LOG(ERROR, "check column checksum failed, index id[%ld], ret[%d]", index_id_list.at(i), err);
                }
                if(!is_right)
                {
                    if(OB_SUCCESS != (err = root_server_->check_column_checksum(OB_ALL_CLUSTER_FLAG, index_id_list.at(i), frozen_version, is_right)))
                    ...
                    if(!is_right)
                    {
                        for(int cluster_idx = 0; cluster_idx < root_server_->get_config().use_cluster_num; cluster_idx++)
                        {
                            if(OB_SUCCESS != (err = root_server_->modify_index_process_info(cluster_idx, index_id_list.at(i), ERROR)))
                            {
                                YYSYS_LOG(WARN, "failed to change idx table status:%ld, cluster_id[%d], ret[%d]", index_id_list.at(i), cluster_idx, err);
                            }
                        }
                    }
                }
          ...
          if(root_server_->get_obi_role().get_role() == ObiRole::MASTER && frozen_version > 3)
          {
            int clean_ret = OB_SUCCESS;
              ...
              if(OB_SUCCESS != (clean_ret = root_server_->clean_old_checksum_v2(frozen_version - 1)))
              {
                YYSYS_LOG(WARN, "clean __all_cchecksum_info failed  %d", clean_ret);
              }
              else
              {
                YYSYS_LOG(INFO, "clean __all_cchecksum_info success");
              }
          }
          //add e
          ...
          if (root_server_->get_obi_role().get_role() == ObiRole::MASTER)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to modify index status after merge done. ret=%d", err);
            }
          }
          //add:e
        }
      ...
      {        
        ...
        {
          YYSYS_LOG(WARN, "fail to modify staging index status in __index_process_info. ret=%d", err);
        }
        if (root_server_->get_obi_role().get_role() == ObiRole::MASTER)
        {
  ...
  YYSYS_LOG(INFO, "[NOTICE] merge checker thread exit");
}
