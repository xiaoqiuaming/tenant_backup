...
namespace oceanbase
{
  namespace rootserver
  {
    ...
    //add:e
    //add zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b  //uncertainty    ?????????
  ...
  else
  {
    ...
    */
    ...
    else
    {
      ...
      //[628]
      ups_manager_->set_ups_config((int32_t)config_.read_master_master_ups_percent, (int32_t)config_.merge_master_master_ups_percent);
      //mod 20150427:e
...
int ObRootServer2::check_tablet_complete_in_clusters(const uint64_t table_id, const int64_t tablet_version, bool is_complete[OB_MAX_CLUSTER_COUNT])
{
    int ret = OB_SUCCESS;
    yysys::CRLockGuard guard(root_table_rwlock_);
    if(NULL != root_table_)
    {
        ret = root_table_->check_tablet_complete_in_clusters(table_id, tablet_version, is_complete);
    }
    else
    {
        ret = OB_NOT_INIT;
      ...
      else
      {
        ...
        else
        {
          ...
          else if (config_.is_ups_flow_control)
          {
            ...
            //ups_manager_->set_ups_config((int32_t)config_.read_master_master_ups_percent,
            //(int32_t)config_.read_slave_master_ups_percent);
              ups_manager_->set_ups_config((int32_t)config_.read_master_master_ups_percent, (int32_t) config_.merge_master_master_ups_percent);//[628]
            //mod 20150427:e
          }
          else
          {
    ...
    else
    {
      ...
      else
      {
        ...
        if (OB_ITER_END == ret)
        {
          ret = OB_SUCCESS;
          //[view]
          if(only_core_tables == true)
          {
              tid_name->table_name_.assign_ptr(const_cast<char*>(OB_ALL_VIEW_TABLE_NAME),
                                               static_cast<int32_t>(strlen(OB_ALL_VIEW_TABLE_NAME)));
              tid_name->table_id_ = OB_ALL_VIEW_TID;
              tid_name->dbname_.assign(const_cast<char*>(OB_DEFAULT_DATABASE_NAME), static_cast<int32_t>(strlen(OB_DEFAULT_DATABASE_NAME)));
              TableSchema table_schema;
              if(OB_SUCCESS != (ret = schema_service_->get_table_schema(tid_name->table_name_, table_schema, tid_name->dbname_)))
              {
                  YYSYS_LOG(WARN, "failed to get table schema, err=%d, table_name=%.*s", ret,
...
int ObRootServer2::check_create_local_index_done(const uint64_t index_tid, bool &is_finished)
{
  ...
  else
  {
    ...
    bool is_cluster_has_complete_range[OB_CLUSTER_ARRAY_LEN];
    worker_->get_monitor()->get_is_cluster_complete(is_cluster_has_complete_range);
    ...
    for (ObRootTable2::const_iterator it = begin_pos; it <= end_pos && OB_LIKELY(OB_SUCCESS == ret); it++)
    {
      ...
      //mod liumz, [paxos static index]20170606:b
      for (int i = 0; i < config_.use_cluster_num && OB_SUCCESS == ret; i++)
      {
        yysys::CThreadGuard hist_mutex_gard(&hist_table_mutex_);
        //if (is_cluster_alive[i] && NULL != hist_table_[i])
        if (ups_manager_->is_cluster_online(i) && is_cluster_has_complete_range[i] && NULL != hist_table_[i])
        {
...
//    get_cluster_tablet_replicas_num(replica_num_each_cluster);
    ...
    for (ObRootTable2::const_iterator it = begin_pos; it <= end_pos && OB_SUCCESS == ret; it++)
    {
      ...
      for (int cluster_idx = 0; cluster_idx < config_.use_cluster_num; cluster_idx++)
      {
        if (ups_manager_->is_cluster_online(cluster_idx) && is_cluster_has_complete_range[cluster_idx])
        {
...
//    get_alive_cluster_with_cs(is_cluster_alive);
    ...
    for (int cluster_idx = 0; cluster_idx < config_.use_cluster_num && OB_SUCCESS == ret; cluster_idx++)
    {
      if (ups_manager_->is_cluster_online(cluster_idx) && is_cluster_has_complete_range[cluster_idx] && NULL != hist_table_[cluster_idx])
      {
        ...
        {//block
          ...
          while (hist_table_[cluster_idx]->next_srw() && OB_SUCCESS == ret)
          {
            ...
            if(0 == copy_count)
            {
                copy_count = static_cast<int32_t>(config_.tablet_replicas_num);
            }
    ...
    for (ObRootTable2::const_iterator it = begin_pos; it <= end_pos && OB_LIKELY(OB_SUCCESS == ret); it++)
    {
      for (int32_t idx = 0; idx < OB_MAX_COPY_COUNT; idx++)
      {
        if (OB_INVALID_INDEX != it->server_info_indexes_[idx] && OB_INVALID_VERSION == it->tablet_version_[idx])
        {
          ...
          atomic_exchange((uint32_t*) &(it->server_info_indexes_[idx]), static_cast<uint32_t>(OB_INVALID_INDEX));//mod support for arm platform by wangd 202106
        }
      }
    }
  }
...
/*
 * �����㱨��Ϣ, ֱ��д����ǰ��root table??
 * ������ֻ㱨��Ϣ���жԵ�ǰroot table��tablet�ķ��ѻ��ߺ�??
 * Ҫ���ò���д�������ƵĴ�����??
 */
  ...
  else
  {
    for (ObRootTable2::const_iterator it = first; it <= last; it++)
    {
      ...
      if (found_it_index != OB_INVALID_INDEX)
      {
        ...
        //over write
        //mod support for arm platform by wangd 202106:b
        atomic_exchange((uint32_t*) &(it->server_info_indexes_[found_it_index]), static_cast<uint32_t>(server_index));
        atomic_exchange((uint64_t*) &(it->tablet_version_[found_it_index]), static_cast<uint64_t>(tablet_version));
        if (p_tablet_write != NULL)
        {
          atomic_exchange((uint64_t*) &(p_tablet_write->row_count_), static_cast<uint64_t>(tablet_info.row_count_));
          atomic_exchange((uint64_t*) &(p_tablet_write->occupy_size_), static_cast<uint64_t>(tablet_info.occupy_size_));
        }
        //mod support for arm platform by wangd 202106:e
      }
    }
  }
  return ret;
...
int ObRootServer2::do_add_new_paxos_group(const int64_t new_paxos_num)
{
  ...
  else
  {
    ...
    //[492]
    change_paxos_new_stat_set(new_config, new_paxos_num);
    if(OB_SUCCESS != (ret = refresh_sys_new_info((get_new_frozen_version() + 1), new_config,
                                                 paxos_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),UPS_PAXOS_SCALE)))
    {
        YYSYS_LOG(WARN, "refresh paxos group new info failed, ret=%d", ret);
    }
    else if(OB_SUCCESS != (ret = refresh_sys_new_info((get_new_frozen_version() + 2), new_config,
                                                      paxos_new_set_next_version_.get_bitset_word(OB_BITSET_INDEX),UPS_PAXOS_SCALE)))
    {
        YYSYS_LOG(WARN, "refresh paxos group new info failed, ret=%d", ret);
    }
...
{
  int ret = OB_SUCCESS;
  int64_t new_use_paxos_num = config_.use_paxos_num - 1; //[699]
  yysys::CWLockGuard guard(bit_set_rwlock_);
  ...
  else
  {
    ...
    rs_node_mgr_->set_paxos_ups_quorum_scales(config_.paxos_ups_quorum_scales);
    //[492]
    paxos_offline_set_cur_version_.del_member((int32_t)paxos_idx);
    paxos_offline_set_next_version_.del_member((int32_t)paxos_idx);
    paxos_new_set_cur_version_.del_member((int32_t)paxos_idx);
    paxos_new_set_next_version_.del_member((int32_t)paxos_idx);
    if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version()+1, new_use_paxos_num,
                                                     paxos_offline_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),
                                                     UPS_PAXOS_SCALE)))
    {
        YYSYS_LOG(ERROR, "revert current version offline stat info failed when delete paxos, ret=%d", ret);
    }
    else if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version()+2, new_use_paxos_num,
                                                          paxos_offline_set_next_version_.get_bitset_word(OB_BITSET_INDEX),
                                                          UPS_PAXOS_SCALE)))
    {
        YYSYS_LOG(ERROR, "revert next version offline stat info failed when delete paxos, ret=%d", ret);
    }
    else
    {
    ...
    else
    {
      ...
      else
      {
        if (ups_manager_->get_paxos_status(paxos_idx) == UPS_NODE_STAT_NEW)
        {
          ...
          //[492]
          paxos_offline_set_cur_version_.add_member((int32_t)paxos_idx);
          if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version() + 1, config_.use_paxos_num,
      ...
      if (OB_SUCCESS != ret && (is_new_stat == false))
      {
      ...
      //[492]
      if(OB_SUCCESS != ret && (is_new_stat == true))
      {
          int tmp_ret = OB_SUCCESS;
          if(!paxos_new_set_cur_version_.add_member((int32_t)paxos_idx))
          {
              YYSYS_LOG(ERROR, "recover paxos group[%ld] new set failed", paxos_idx);
          }
          else if(!paxos_offline_set_cur_version_.del_member((int32_t)paxos_idx))
          {
              YYSYS_LOG(ERROR, "recover paxos group[%ld] offline set failed", paxos_idx);
          }
          else if(!paxos_offline_set_next_version_.del_member((int32_t)paxos_idx))
          {
              YYSYS_LOG(ERROR, "recover paxos group[%ld] offline set failed", paxos_idx);
          }
          ups_manager_->set_paxos_status(paxos_idx, UPS_NODE_STAT_NEW);
          if(OB_SUCCESS != (tmp_ret = refresh_sys_new_info(get_new_frozen_version() + 1, config_.use_paxos_num,
                                                           paxos_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),
                                                           UPS_PAXOS_SCALE)))
          {
              YYSYS_LOG(ERROR, "recover paxos group:[%ld] new stat failed, err=%d", paxos_idx, tmp_ret);
          }else if(OB_SUCCESS != (tmp_ret = recover_paxos_group_stat(paxos_idx, offline_version)))
          {
              YYSYS_LOG(ERROR, "recover paxos group:[%ld] offline stat failed, err=%d", paxos_idx, tmp_ret);
          }
          else
          {
              YYSYS_LOG(INFO, "recover paxos group:[%ld] stat succeed", paxos_idx);
...
void ObRootServer2::renew_cur_version_offline_set()
{
  ...
  //[492]
  paxos_new_set_cur_version_ = paxos_new_set_next_version_;
  cluster_new_set_cur_version_ = cluster_new_set_next_version_;
...
void ObRootServer2::set_online_paxos_stat()
{
  for (int32_t paxos_idx = 0; paxos_idx < MAX_UPS_COUNT_ONE_CLUSTER; ++paxos_idx)
  {
    ...
    {
      if ((!paxos_offline_set_cur_version_.has_member(paxos_idx))&&
              (!paxos_new_set_cur_version_.has_member(paxos_idx)))
      {
...
int ObRootServer2::do_add_new_cluster(const int64_t new_cluster_num)
{
  ...
  else
  {
    ...
    //[492]
    change_cluster_new_stat_set(new_config, new_cluster_num);
    if(OB_SUCCESS != (ret = refresh_sys_new_info((get_new_frozen_version() + 1), new_config,
                                                 cluster_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),UPS_CLUSTER_SCALE)))
    {
        YYSYS_LOG(WARN, "refresh cluster new info failed, ret=%d", ret);
    }
    else if(OB_SUCCESS != (ret = refresh_sys_new_info((get_new_frozen_version() + 2), new_config,
                                                      cluster_new_set_next_version_.get_bitset_word(OB_BITSET_INDEX),UPS_CLUSTER_SCALE)))
    {
        YYSYS_LOG(WARN, "refresh cluster new info failed, ret=%d", ret);
    }
...
{
  int ret = OB_SUCCESS;
  int64_t new_use_cluster_num = config_.use_cluster_num - 1; //[699]
  yysys::CWLockGuard guard(bit_set_rwlock_);
  bool is_master = this->is_master();
  ...
  else
  {
      ...
      if(is_master && OB_SUCCESS != (ret_tmp = del_index_of_cluster(cluster_idx)))
      {
          YYSYS_LOG(WARN, "update __index_process_info failed, ret=%d", ret_tmp);
          YYSYS_LOG(WARN, "please do sql on client: delete from __index_process_info where cluster_id=%ld", cluster_idx);
      }
      if(is_master && OB_SUCCESS != (ret_tmp = del_all_cluster(cluster_idx)))
      {
          YYSYS_LOG(WARN, "del __all_cluster failed, ret=%d", ret_tmp);
          YYSYS_LOG(WARN, "please do sql on client: delete from __all_cluster where cluster_id=%ld;", cluster_idx);
      }
  }
  ...
  if(ret == OB_SUCCESS)
  {
      cluster_offline_set_cur_version_.del_member((int32_t)cluster_idx);
      cluster_offline_set_next_version_.del_member((int32_t)cluster_idx);
      cluster_new_set_cur_version_.del_member((int32_t)cluster_idx);
      cluster_new_set_next_version_.del_member((int32_t)cluster_idx);

      if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version() + 1, new_use_cluster_num,
                                                       cluster_offline_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),
                                                       UPS_CLUSTER_SCALE)))
      {
          YYSYS_LOG(ERROR, "refresh current version offline stat info failed when delete cluster, ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version() + 2, new_use_cluster_num,
                                                            cluster_offline_set_next_version_.get_bitset_word(OB_BITSET_INDEX),
                                                            UPS_CLUSTER_SCALE)))
      {
          YYSYS_LOG(ERROR, "refresh next version offline stat info failed when delete cluster, ret=%d", ret);
      }
      else
      {
          YYSYS_LOG(INFO, "delete the last cluster succeed, ret=%d", ret);
      }
  }
  return ret;
}
  ...
  else
  {
    ...
    else
    {
      if (ups_manager_->get_cluster_status(cluster_idx) == UPS_NODE_STAT_NEW)
      {
        ...
        //[492]
        cluster_offline_set_cur_version_.add_member((int32_t)cluster_idx);
        if(OB_SUCCESS != (ret = refresh_sys_offline_info(get_new_frozen_version() + 1, config_.use_cluster_num,
    ...
    {
        int tmp_ret = OB_SUCCESS;
        if(!cluster_new_set_cur_version_.add_member((int32_t)cluster_idx))
        {
            YYSYS_LOG(ERROR, "recover cluster[%ld] new set failed.", cluster_idx);
        }
        else if(!cluster_offline_set_cur_version_.del_member((int32_t)cluster_idx))
        {
            YYSYS_LOG(ERROR, "recover cluster[%ld] offline set failed.", cluster_idx);
        }
        else if(!cluster_offline_set_next_version_.del_member((int32_t)cluster_idx))
        {
            YYSYS_LOG(ERROR, "recover cluster[%ld] offline set failed.", cluster_idx);
        }
        ups_manager_->set_cluster_status(cluster_idx, UPS_NODE_STAT_NEW);
        if(OB_SUCCESS != (tmp_ret = refresh_sys_new_info(get_new_frozen_version() + 1, config_.use_cluster_num,
                                                         cluster_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX),
                                                         UPS_CLUSTER_SCALE)))
        {
            YYSYS_LOG(ERROR, "recover cluster:[%ld] new stat failed, err=%d", cluster_idx, tmp_ret);
        }
        else if(OB_SUCCESS != (tmp_ret = recover_cluster_stat(cluster_idx, offline_version)))
        {
            YYSYS_LOG(ERROR, "recover cluster:[%ld] stat failed, err=%d", cluster_idx, tmp_ret);
        }
        else
...
//[492]
int ObRootServer2::get_cluster_and_paxos_stat_info(int64_t &paxos_new_stat, int64_t &cluster_new_stat, int64_t &paxos_offline_stat, int64_t &cluster_offline_stat, const int64_t version)
{
    int ret = OB_SUCCESS;
    yysys::CRLockGuard guard(bit_set_rwlock_);
    ...
    databuff_printf(buf, OB_MAX_SQL_LENGTH, pos,
                    "select name, use_num,stat_info,new_stat_info from %s where version = %ld;",
                    OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, version);
    ...
    if(ret == OB_SUCCESS)
    {
        ...
        else
        {
            ...
            ObString name;
            int64_t use_num;
            int64_t offline_stat_info;
            int64_t new_stat_info;
            ...
            while(OB_SUCCESS == res.get_new_scanner().get_next_row(row))
            {
                ...
                else if(OB_SUCCESS != (ret = column->get_int(offline_stat_info)))
                {
                    YYSYS_LOG(WARN, "get stat_info value failed, ret=%d", ret);
                }
                else if(OB_SUCCESS != (ret = row.raw_get_cell(3, column)))
                {
                    YYSYS_LOG(WARN, "get new_stat_info cell failed, ret=%d", ret);
                }
                else if(OB_SUCCESS != (ret = column->get_int(new_stat_info)))
                {
                    YYSYS_LOG(WARN, "get new_stat_info value failed, ret=%d", ret);
                }
                else
                {
                    if(ObString::make_string(PAXOS_STAT) == name)
                    {
                        paxos_offline_stat = offline_stat_info;
                        paxos_new_stat = new_stat_info;
                    }
                    else if(ObString::make_string(CLUSTER_STAT) == name)
                    {
                        cluster_offline_stat = offline_stat_info;
                        cluster_new_stat = new_stat_info;
                    }
                    else
                    {
                ...
                if(OB_SUCCESS != ret)
                {
                    YYSYS_LOG(ERROR, "get cluster stat info failed, ret=%d", ret);
                    break;
                }
            }
        }
...
int ObRootServer2::get_cur_data_version(int64_t &version)
{
    int ret = OB_SUCCESS;
    char buf[OB_MAX_SQL_LENGTH];
    memset(buf, 0, OB_MAX_SQL_LENGTH);
    ...
    if(this->get_ups_manager()->get_initialized())
    {
        version = this->get_last_frozen_version() + 1;
    }
    else
    {
        ms_provider_.get_ms(ms_server, true);
        databuff_printf(buf, OB_MAX_SQL_LENGTH, pos,
                        ...
                        OB_ALL_SYS_STAT_TABLE_NAME, ObString::make_string("yb_cur_data_version").ptr());
        ...
        else
        {
            ...
            if(OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, res)))
            {
                YYSYS_LOG(ERROR, "select cur sys version info failed, ret=%d", ret);
            }
            else if(1 != res.get_new_scanner().get_row_num())
            {
                ret = OB_ERROR;
                YYSYS_LOG(ERROR, "can't get cur sys version , ret=%d", ret);
            }
            else
            {
                const common::ObObj *column = NULL;
                ObString temp_version_str;
                int temp_version;
                while(OB_SUCCESS == res.get_new_scanner().get_next_row(row))
                {
                    if(OB_SUCCESS != (ret = row.raw_get_cell(0, column)))
                    ...
                    else if(OB_SUCCESS != (ret = column->get_varchar(temp_version_str)))
                    {
                        YYSYS_LOG(WARN, "get cur version value failed, ret=%d", ret);
                    }
                    if(OB_SUCCESS != ret)
                    {
                        YYSYS_LOG(WARN, "get cur version value failed, ret=%d", ret);
                    ...
                    {
                        temp_version = atoi(temp_version_str.ptr());
                        version = static_cast<int64_t>(temp_version);
                    }
                }
            }
        }
...
int ObRootServer2::revert_cluster_and_paxos_stat_info(int64_t active_version)
{
    int ret = OB_SUCCESS;
    char buf[OB_MAX_SQL_LENGTH];
    memset(buf, 0, OB_MAX_SQL_LENGTH);
    YYSYS_LOG(INFO, "cur sys data version=%ld,begin revert cluster and paxos stat", active_version);

    //current version stat
    int64_t cur_paxos_new_stat = 0;
    int64_t cur_cluster_new_stat = 0;
    int64_t cur_paxos_offline_stat = 0;
    int64_t cur_cluster_offline_stat = 0;

    //next version stat
    int64_t next_paxos_new_stat = -1;
    int64_t next_cluster_new_stat = -1;
    int64_t next_paxos_offline_stat = -1;
    int64_t next_cluster_offline_stat = -1;


    //get and revert info of current verison
    if(OB_SUCCESS != (ret = get_cluster_and_paxos_stat_info(cur_paxos_new_stat, cur_cluster_new_stat, cur_paxos_offline_stat,cur_cluster_offline_stat, active_version)))
    {
        YYSYS_LOG(WARN, "get current version paxos and cluster new stat failed, ret=%d", ret);
    }
    else
    {
        if(OB_SUCCESS != (ret = paxos_new_set_cur_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                           (uint32_t)cur_paxos_new_stat)))
        {
            YYSYS_LOG(WARN, "renew paxos cur version new stat bit set failed, ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = cluster_new_set_cur_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                                (uint32_t)cur_cluster_new_stat)))
        {
            YYSYS_LOG(WARN, "renew cluster cur version new stat bit set failed, ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = paxos_offline_set_cur_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                                (uint32_t)cur_paxos_offline_stat)))
        {
            YYSYS_LOG(WARN, "renew paxos cur version new stat bit set failed, ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = cluster_offline_set_cur_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                                (uint32_t)cur_cluster_offline_stat)))
        {
            YYSYS_LOG(WARN, "renew cluster cur version new stat bit set failed, ret=%d", ret);
        }
    }

    //get and revert info of next versoin
    if(ret == OB_SUCCESS)
    {
        if(OB_SUCCESS != (ret = get_cluster_and_paxos_stat_info(next_paxos_new_stat, next_cluster_new_stat, next_paxos_offline_stat,next_cluster_offline_stat, active_version + 1)))
        {
            YYSYS_LOG(WARN, "get next version paxos and cluster new stat failed, ret=%d", ret);
        }
        next_paxos_new_stat = 0;
        next_cluster_new_stat = 0;

        //[724]
        if(next_cluster_offline_stat == -1)
        {
            next_cluster_offline_stat = cur_cluster_offline_stat;
        }
        if(next_paxos_offline_stat == -1)
        {
            next_paxos_offline_stat = cur_paxos_offline_stat;
        }
        ...
        else if (OB_SUCCESS != (ret = paxos_new_set_next_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                                 (uint32_t)next_paxos_new_stat)))
        {
            YYSYS_LOG(WARN, "renew paxos next version new stat bit set failed, ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = cluster_new_set_next_version_.set_bitset_word(OB_BITSET_INDEX,
                                                                                (uint32_t)next_cluster_new_stat)))
        {
            YYSYS_LOG(WARN, "renew cluster next version new stat bit set failed, ret=%d", ret);
        }

        if(ret == OB_SUCCESS)
        {
            if(OB_SUCCESS != (ret = paxos_offline_set_next_version_.set_bitset_word(OB_BITSET_INDEX, (uint32_t)next_paxos_offline_stat)))
            {
                YYSYS_LOG(WARN, "renew paxos next version offline stat bit set failed, ret=%d", ret);
            }
            else
            {
                for(int32_t paxos_idx = 0; paxos_idx < config_.use_paxos_num && ret == OB_SUCCESS; paxos_idx++)
                {
                    if(paxos_offline_set_next_version_.has_member(paxos_idx) && !paxos_offline_set_cur_version_.has_member(paxos_idx))
                    {
                        if(OB_SUCCESS != (ret = ups_manager_->set_paxos_ups_offline_version(paxos_idx, active_version + 1)))
                        {
                            YYSYS_LOG(ERROR, "set ups offline version of paxos[%d] failed, ret=%d", paxos_idx, ret);
                        }
                    }
                }
            }
        ...
        if(ret == OB_SUCCESS)
        {
            if(OB_SUCCESS != (ret = cluster_offline_set_next_version_.set_bitset_word(OB_BITSET_INDEX, (uint32_t)next_cluster_offline_stat)))
            {
                YYSYS_LOG(WARN, "renew cluster next version offline stat bit set failed, ret=%d", ret);
            }
            else
            {
                for(int32_t cluster_idx = 0; cluster_idx < config_.use_cluster_num && ret == OB_SUCCESS; cluster_idx++)
                {
                    if(cluster_offline_set_next_version_.has_member(cluster_idx) && !cluster_offline_set_cur_version_.has_member(cluster_idx))
                    {
                        if(OB_SUCCESS != (ret = ups_manager_->set_cluster_ups_offline_version(cluster_idx, active_version + 1)))
                        {
                            YYSYS_LOG(ERROR, "set ups offline version of cluster[%d] failed, ret=%d", cluster_idx, ret);
                        }
                    }
                }
            }
    ...
    if(ret == OB_SUCCESS)
    {
        set_new_cluster_stat();
        set_offline_cluster_stat();
        set_online_cluster_stat();

        set_new_paxos_stat();
        set_offline_paxos_stat();
        set_online_paxos_stat();
    }
    return ret;
}
...
int ObRootServer2::init_cluster_stat_info()
{
  ...
  else
  {
    ...
    else if (2 > res.get_new_scanner().get_row_num())
    {
      ...
      else if (OB_SUCCESS != (ret = refresh_sys_new_info(active_version, config_.use_paxos_num,
                                                         paxos_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX), UPS_PAXOS_SCALE)))
      {
        YYSYS_LOG(ERROR, "refresh current paxos group new info failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = refresh_sys_new_info(active_version, config_.use_cluster_num,
                                                             cluster_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX), UPS_CLUSTER_SCALE)))
      {
        YYSYS_LOG(ERROR, "refresh current cluster new info failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = refresh_sys_cur_data_version_info()))
      {
          YYSYS_LOG(WARN, "refresh current sys version info failed");
      }
        ...
        else
        {
          if (ObString::make_string(PAXOS_STAT) == name)
          {
            if (use_num != get_worker()->get_restart_param().restart_paxos_num_)
            {
              ...
              kill_self();
              YYSYS_LOG(ERROR, "cmd use_paxos_num=%ld is not equal to inner table %s use_paxos_num_=%ld,will kill self.", get_worker()->get_restart_param().restart_paxos_num_, OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, use_num);
            }
...
//            }
          ...
          else if (ObString::make_string(CLUSTER_STAT) == name)
          {
            if (use_num != get_worker()->get_restart_param().restart_clu_num_)
            {
              ret = OB_RS_INVALID_CLUSTER_OR_PAXOS_NUM;
              kill_self();
              YYSYS_LOG(ERROR, "cmd use_cluster_num=%ld is not equal to inner table %s use_cluster_num_=%ld, will kill self", get_worker()->get_restart_param().restart_clu_num_, OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, use_num);
            }
...
//            }
      ...
      if(ret == OB_SUCCESS)
      {
          ...
          if(OB_SUCCESS != (ret = this->refresh_sys_cur_data_version_info()))
          {
              YYSYS_LOG(WARN, "refresh current sys version info failed");
          }
          else if(OB_SUCCESS != (ret = this->get_cur_data_version(active_version)))
          {
              YYSYS_LOG(WARN, "get cur data version failed,ret=%d", ret);
          }
          else if(OB_SUCCESS != (ret = this->revert_cluster_and_paxos_stat_info(active_version)))
          {
              YYSYS_LOG(WARN, "revert cluster and paxos group stat info failed,ret=%d", ret);
          }
      }
    }
  }
...
void ObRootServer2::set_new_cluster_stat()
{
    yysys::CRLockGuard guard(bit_set_rwlock_);
    for(int32_t cluster_idx = 0; cluster_idx < OB_MAX_CLUSTER_COUNT; ++cluster_idx)
    {
        if(cluster_new_set_cur_version_.has_member(cluster_idx))
        {
            ups_manager_->change_cluster_stat(cluster_idx, UPS_NODE_STAT_NEW);
        }
    }
}
void ObRootServer2::set_new_paxos_stat()
{
    yysys::CRLockGuard guard(bit_set_rwlock_);
    for(int32_t paxos_idx = 0; paxos_idx < MAX_UPS_COUNT_ONE_CLUSTER; ++paxos_idx)
    {
        if(paxos_new_set_cur_version_.has_member(paxos_idx))
        {
...
void ObRootServer2::set_online_cluster_stat()
{
  for (int32_t cluster_idx = 0; cluster_idx < OB_MAX_CLUSTER_COUNT; ++cluster_idx)
  {
    ...
    {
      if((!cluster_offline_set_cur_version_.has_member(cluster_idx)) &&
              (!cluster_new_set_cur_version_.has_member(cluster_idx)))
      {
...
int ObRootServer2::update_use_num_of_cluster_stat_info(int64_t version, int64_t use_num, ScaleType type)
{
  ...
  {
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos, "UPDATE %s SET use_num=%ld WHERE version=%ld AND name='%s';",
...
//[492]
void ObRootServer2::resolve_new_and_offline_stat_conflict(int64_t version, ScaleType type, StateType stat_type)
{
    if(stat_type == NEW_STAT)
    {
        if(version == (get_last_frozen_version() + 1))
        {
            if(UPS_PAXOS_SCALE == type)
            {
                for(int32_t paxos_idx = 0; paxos_idx < config_.use_paxos_num; paxos_idx++)
                {
                    if(paxos_new_set_cur_version_.has_member(paxos_idx))
                    {
                        paxos_offline_set_cur_version_.del_member(paxos_idx);
                        paxos_offline_set_next_version_.del_member(paxos_idx);
                    }
                }
            }
            else if(UPS_CLUSTER_SCALE == type)
            {
                for(int32_t cluster_idx = 0; cluster_idx < config_.use_cluster_num; cluster_idx++)
                {
                    if(cluster_new_set_cur_version_.has_member(cluster_idx))
                    {
                        cluster_offline_set_cur_version_.del_member(cluster_idx);
                        cluster_offline_set_next_version_.del_member(cluster_idx);
                    }
                }
            }
        }
        ...
        {
            uint32_t bit_word = 0;
            paxos_new_set_next_version_.set_bitset_word(OB_BITSET_INDEX, bit_word);
            cluster_new_set_next_version_.set_bitset_word(OB_BITSET_INDEX, bit_word);
        }
    }
    else if (stat_type == OFFLINE_STAT)
    {
        if(version == (get_last_frozen_version() + 1))
        {
            if(UPS_PAXOS_SCALE == type)
            {
                for(int32_t paxos_idx = 0; paxos_idx < config_.use_paxos_num; paxos_idx++)
                {
                    if(paxos_offline_set_cur_version_.has_member(paxos_idx))
                    {
                        paxos_new_set_cur_version_.del_member(paxos_idx);
                    }
                }
            }
            else if(UPS_CLUSTER_SCALE == type)
            {
                for(int32_t cluster_idx = 0; cluster_idx < config_.use_cluster_num; cluster_idx++)
                {
                    if(cluster_offline_set_cur_version_.has_member(cluster_idx))
                    {
                        cluster_new_set_cur_version_.del_member(cluster_idx);
                    }
                }
            }
        }
...
int ObRootServer2::refresh_sys_offline_info(int64_t version, int64_t use_num, int32_t offline_info, ScaleType type)
{
  ...
  //[492]
  resolve_new_and_offline_stat_conflict(version, type, OFFLINE_STAT);
  int32_t new_paxos_info = 0;
  int32_t new_cluster_info = 0;
  if(version == (get_last_frozen_version()+1))
  {
      new_paxos_info = paxos_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX);
      new_cluster_info = cluster_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX);
  }
  else if(version == (get_last_frozen_version()+2))
  {
      new_paxos_info = 0;
      new_cluster_info = 0;
  }
  ...
  if (UPS_PAXOS_SCALE == type)
  {
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos,
                    "REPLACE INTO %s (version, name, use_num, stat_info,new_stat_info)VALUES(%ld, \'%s\', %ld, %d, %d);",
                    OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, version, PAXOS_STAT, use_num, offline_info, new_paxos_info);
  }
  else
  {
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos,
                    "REPLACE INTO %s (version, name, use_num, stat_info,new_stat_info)VALUES(%ld, \'%s\', %ld, %d, %d);",
                    OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, version, CLUSTER_STAT, use_num, offline_info, new_cluster_info);
  }
  if (pos >= OB_MAX_SQL_LENGTH)
  {
    ret = OB_BUF_NOT_ENOUGH;
...
//[492]
int ObRootServer2::refresh_sys_new_info(int64_t version, int64_t use_num, int32_t new_info, ScaleType type)
{
    int ret = OB_SUCCESS;
    ...
    char sql_char[OB_MAX_SQL_LENGTH];
    memset(sql_char, 0, OB_MAX_SQL_LENGTH);
    resolve_new_and_offline_stat_conflict(version, type, NEW_STAT);

    int32_t offline_paxos_info = 0;
    int32_t offline_cluster_info = 0;
    if(version == (get_last_frozen_version()+1))
    {
        offline_paxos_info = paxos_offline_set_cur_version_.get_bitset_word(OB_BITSET_INDEX);
        offline_cluster_info = cluster_offline_set_cur_version_.get_bitset_word(OB_BITSET_INDEX);
    }
    else if(version == (get_last_frozen_version()+2))
    {
        offline_paxos_info = paxos_offline_set_next_version_.get_bitset_word(OB_BITSET_INDEX);
        offline_cluster_info = cluster_offline_set_next_version_.get_bitset_word(OB_BITSET_INDEX);
    }
    ...
    if (UPS_PAXOS_SCALE == type)
    {
      databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos,
                      "REPLACE INTO %s (version, name, use_num, stat_info,new_stat_info)VALUES(%ld, \'%s\', %ld, %d, %d);",
                      OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, version, PAXOS_STAT, use_num, offline_paxos_info, new_info);
    }
    else
    {
      databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos,
                      "REPLACE INTO %s (version, name, use_num, stat_info,new_stat_info)VALUES(%ld, \'%s\', %ld, %d, %d);",
                      OB_ALL_CLUSTER_STAT_INFO_TABLE_NAME, version, CLUSTER_STAT, use_num, offline_cluster_info, new_info);
    }
    if (pos >= OB_MAX_SQL_LENGTH)
    {
      ret = OB_BUF_NOT_ENOUGH;
    ...
    else
    {
      ...
      for (int32_t i = 0; i < config_.retry_times; ++i)
      {
        if (OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, timeout)))
        {
          YYSYS_LOG(WARN, "refresh system new stat info failed, ret=%d", ret);
        }
        else
        {
          break;
    ...
    if (OB_SUCCESS != ret)
    {
      if (UPS_PAXOS_SCALE == type)
      {
        YYSYS_LOG(ERROR, "renew paxos group new stat failed, ret=%d", ret);
      }
      else
      {
        YYSYS_LOG(ERROR, "renew cluster new stat failed, ret=%d", ret);
      }
    }
    return ret;
}
...
int ObRootServer2::refresh_all_cluster_stat_info(int64_t version)
{
  ...
  else if (OB_SUCCESS != (ret = refresh_sys_new_info(version, config_.use_paxos_num,
                                                     paxos_new_set_next_version_.get_bitset_word(OB_BITSET_INDEX),
                                                     UPS_PAXOS_SCALE)))
  {
      YYSYS_LOG(ERROR, "refresh all paxos group new stat info failed, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = refresh_sys_new_info(version, config_.use_cluster_num,
                                                         cluster_new_set_next_version_.get_bitset_word(OB_BITSET_INDEX),
                                                         UPS_CLUSTER_SCALE)))
  {
    YYSYS_LOG(ERROR, "refresh all subcluster new stat info failed, ret=%d", ret);
  }
  else
  {
...
//[492]
int ObRootServer2::refresh_sys_cur_data_version_info()
{
    int ret = OB_SUCCESS;
    ...
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos,
                    "REPLACE INTO %s (cluster_id, name, data_type, value , info )VALUES(%ld, \'%s\', %d, %ld,\'%s\');",
                    OB_ALL_SYS_STAT_TABLE_NAME, SYS_TABLE_PAXOS_ID, ObString::make_string("yb_cur_data_version").ptr(), ObIntType,
                    get_last_frozen_version()+1, ObString::make_string("current data version").ptr());
    ...
    else
    {
      sql.assign(sql_char, static_cast<ObString::obstr_size_t>(pos));
      YYSYS_LOG(INFO, "sql_str=[%.*s]", sql.length(), sql.ptr());
      for (int32_t i = 0; i < config_.retry_times*3; ++i)
      {
        if (OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, timeout)))
        {
          YYSYS_LOG(WARN, "refresh system current data version failed, ret=%d", ret);
        }
        else
        {
          break;
...
int ObRootServer2::refresh_all_change_cluster_paxos(int64_t op_time_int, const char *op_client, int64_t version, ObChangePaxosClusterType op_type, const char *op_param, int64_t num, int op_result)
{
  int ret = OB_SUCCESS;
  ObString sql;
  int64_t pos = 0;
  ...
  else
  {
    ...
    for (int32_t i = 0; i < config_.retry_times; ++i)
    {
      if (OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, timeout)))
      {
        YYSYS_LOG(WARN, "refresh system _all_change_cluster_paxos failed, ret=%d", ret);
      }
      else
      {
        break;
...
//[492]
void ObRootServer2::change_cluster_new_stat_set(int64_t cur_config_num, int64_t new_cluster_num)
{
    int64_t cluster_idx = OB_INVALID_INDEX;
    for (int64_t idx = new_cluster_num; 0 < idx; --idx)
    {
        cluster_idx = cur_config_num - idx;
        cluster_new_set_cur_version_.add_member((int32_t)cluster_idx);
    }
}

void ObRootServer2::change_paxos_new_stat_set(int64_t cur_config_num, int64_t new_paxos_num)
{
    int64_t paxos_idx = OB_INVALID_INDEX;
    for (int64_t idx = new_paxos_num; 0 < idx; --idx)
    {
        paxos_idx = cur_config_num - idx;
        paxos_new_set_cur_version_.add_member((int32_t)paxos_idx);
    }
}
...
int ObRootServer2::check_cluster_can_delete(const int64_t cluster_idx)
{
  ...
  if (!is_cluster_offline(cluster_idx))
  {
    ...
    YYSYS_LOG(INFO, "cluster[%ld] is online, can not be deleted, ret=%d", cluster_idx, ret);
  }
  else if (!cluster_offline_set_cur_version_.has_member((int32_t)cluster_idx))
  {
    ret = OB_CLUSTER_NOT_OFFLINE_CURRENT_VERSION;
    YYSYS_LOG(INFO, "cluster[%ld] is online in current version, can not be deleted, ret=%d", cluster_idx, ret);
  }
  ...
  else
  {
    ...
    else
    {
      ...
      if (UPS_NODE_STAT_OFFLINE == ups_manager_->get_paxos_status(paxos_idx))
      {
        ...
        {
            paxos_new_set_cur_version_.add_member((int32_t)paxos_idx);
            if(OB_SUCCESS != (ret = refresh_sys_new_info((get_last_frozen_version() + 1), config_.use_paxos_num,
                                                         paxos_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX), UPS_PAXOS_SCALE)))
            {
                YYSYS_LOG(WARN, "refresh paxos group new info failed, ret=%d", ret);
            }
  ...
  else
  {
    ...
    else
    {
      ...
      if (UPS_NODE_STAT_OFFLINE == ups_manager_->get_cluster_status(cluster_idx))
      {
        ...
        {
            cluster_new_set_cur_version_.add_member((int32_t)cluster_idx);
            if(OB_SUCCESS != (ret = refresh_sys_new_info((get_last_frozen_version() + 1), config_.use_cluster_num,
                                                         cluster_new_set_cur_version_.get_bitset_word(OB_BITSET_INDEX), UPS_CLUSTER_SCALE)))
            {
                YYSYS_LOG(WARN, "refresh paxos group info failed, ret=%d", ret);
            }
        }
      }
    }
...
int ObRootServer2::create_table(bool if_not_exists, const common::TableSchema &tschema)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_ILLEGAL_VALUE;
      YYSYS_LOG(USER_ERROR, "ILLEGAL replica num=[%d] < use_cluster_num[%d] or tablet_replicas_num[%d]", tschema.replica_num_, static_cast<int32_t> (config_.use_cluster_num), static_cast<int32_t> (config_.tablet_replicas_num));
    }
  }
  //add:e
  if(OB_SUCCESS == ret)
...
int ObRootServer2::alter_group(const common::ObString &group_name, const int64_t paxos_idx)
{
  ...
  {
    databuff_printf(select_groups, OB_SQL_LENGTH, pos, "select /*+read_consistency(strong) */ group_name, start_version, paxos_id, reserver_field1 from __all_all_group where group_name = '%.*s'",
        ...
        else
        {
          while(true)
          {
            ...
            else
            {
                //[623]
                const common::ObObj *column = NULL;
                if(OB_SUCCESS != row.raw_get_cell(3,column))
                {
                    YYSYS_LOG(WARN, "get reserver_field1 cell failed, ret=%d", ret);
                }
                else if (OB_SUCCESS != column->get_varchar(reserver_field1))
                {
                    YYSYS_LOG(WARN, "get reserver_field1 value failed, ret=%d", ret);
                }
    ...
    else
    {
      ...
      else
      {
        ...
        int64_t pos = 0;
        //[632]
        if(reserver_field1.ptr() !=NULL && reserver_field1.length()>0)
        {
            databuff_printf(insert_group, OB_SQL_LENGTH, pos, "replace into __all_all_group (group_name, start_version, paxos_id, reserver_field1) VALUES('%.*s', %ld, %ld, '%.*s')", group_name.length(),
                            group_name.ptr(), last_frozen_mem_version_ + 2, paxos_idx, reserver_field1.length(), reserver_field1.ptr());

        }
        else
        {
            databuff_printf(insert_group, OB_SQL_LENGTH, pos, "replace into __all_all_group (group_name, start_version, paxos_id) VALUES('%.*s', %ld, %ld)", group_name.length(),
                            group_name.ptr(), last_frozen_mem_version_ + 2, paxos_idx);
        ...
        {
          ret = OB_BUF_NOT_ENOUGH;
          YYSYS_LOG(WARN, "insert group buffer not enough, ret=%d", ret);
        }
        else
        {
...
int ObRootServer2::check_paxos_can_offline(const int64_t offline_version, const int64_t paxos_idx, bool &can_offline)
{
  ...
  else
  {
    ...
    databuff_printf(select_groups, OB_SQL_LENGTH, pos, "select /*+read_consistency(strong)*/ paxos_id, group_name from __all_all_group");
...
int ObRootServer2::modify_init_index()
{
  ...
  else
  {
    ...
    for (int64_t i = 0; i < init_indexes.count() && OB_LIKELY(OB_SUCCESS == ret); i++)
    {
      ...
      if(OB_SUCCESS == (ret = schema_service_->get_index_stat(tid, config_.use_cluster_num, stat)))
        //mod:e
      {
        if (ERROR == stat)
        {
  ...
  else if (true == ret)
  {
    ...
    {//mutex block
      ...
      if (true == ret)
      {
        ...
        // else if(last_frozen_mem_version == get_last_frozen_version()) //uncertainty ups ??��
...
int ObRootServer2::report_frozen_memtable(const int64_t frozen_version, const int64_t last_frozen_time, bool did_replay)
{
  ...
  if (OB_SUCCESS == ret)
  {
    atomic_exchange((uint64_t*) &last_frozen_mem_version_, static_cast<uint64_t>(frozen_version));//mod support for arm platform by wangd 202106
...
int ObRootServer2::major_freeze()
{
  ...
  */
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to init partition duty, ret=%d", ret);
      ret = OB_SUCCESS;
    }
    else if ((OB_SUCCESS == ret) && OB_SUCCESS != (ret = timer_.schedule(partition_duty_, 1000 * 10L, false)))
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(INFO, "update report a new froze version:version[%ld]", to_be_frozen_version_);
  }
  else
  {
    //BUG fix
...
//[677]
void ObRootServer2::do_stat_rs_link(char *buf, const int64_t buf_len, int64_t &pos)
{
    ObUpsNode *link_head = ups_manager_->get_link_head();
    if(NULL == link_head)
    {
        databuff_printf(buf, buf_len, pos, "  The link of RS[%s] is NULL.\n", to_cstring(get_self()));
    }
    else
    {
        databuff_printf(buf, buf_len, pos, "  The UpsNode of LinkList:\n");
        ...
        do{
            ...
            do{
                ...
                int64_t newStat = 0;
                int64_t offline = 0;
                for(int64_t i = 0; i < cluster->ups_array_->count(); ++i)
                {
                    if(UPS_STAT_OFFLINE == cluster->ups_array_->at(i).stat_)
                    {
                        offline++;
                    }
                    else if(ups_manager_->get_paxos_status(cluster->paxos_idx_) == UPS_NODE_STAT_NEW || ups_manager_->get_cluster_status(cluster->cluster_idx_) == UPS_NODE_STAT_NEW)
                    {
                        newStat++;
                    }
                    else
                ...
                databuff_printf(buf, buf_len, pos,"   [paxos%ld,cluster%ld]:(total %ld,active %ld,new %ld, offline %ld) ", cluster->paxos_idx_,cluster->cluster_idx_, cluster->ups_array_->count(), alive, newStat, offline);
                cluster = cluster->next_cluster_node();

            }while(cluster != paxos && NULL != cluster);
            databuff_printf(buf, buf_len, pos,"\n");
            paxos = paxos->next_paxos_group_node();
        }while(paxos != link_head && NULL != paxos);
    }
...
int ObRootServer2::do_stat(int stat_key, char *buf, const int64_t buf_len, int64_t& pos)
{
  ...
  switch(stat_key)
  {
    ...
    case OB_RS_STAT_CLUSTER_INFO:
      ...
      break;
      //20160425:e
    case OB_RS_STAT_LINK:  //[677]
      do_stat_rs_link(buf, buf_len, pos);
      break;
                                  ...
                                  )
{
  ...
  //[634]
  if ((0 > read_master_master_ups_percentage)
      ||(100 < read_master_master_ups_percentage)
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS == ret)
    {
        int32_t merge_master_master_ups_percentage = 0;//[628]
      ret = ups_manager_->set_ups_config(read_master_master_ups_percentage
                                         , merge_master_master_ups_percentage
                                         //del lbzhong [Paxos Cluster.Flow.UPS] 201607025:b
...
//[492]
int ObRootServer2::force_get_sys_table_ups_list(common::ObUpsList &ups_list)
{
    int ret = OB_SUCCESS;
    if (NULL == ups_manager_)
    {
    ...
    {
      ups_manager_->force_get_online_sys_table_ups_list(ups_list);
    }
    return ret;
}
      ...
      if (it->status_ != ObServerStatus::STATUS_DEAD && it->port_cs_ != 0)
      {
        ...
        */
        ret = worker_->get_rpc_stub().heartbeat_to_cs_with_index(tmp_server,
                                                                 ...
                                                                 get_index_beat(),
                                                                 get_build_index_version(),//[637]
                                                                 is_last_finished()); //[637]

        //mod:e
        //uincertainty ?????withindex
        if (OB_SUCCESS == ret)
        {
...
int ObRootServer2::start_bypass_process(ObBypassTaskInfo &table_name_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    //��¼��ǰ��frozen_version����ֹRS������״̬��ʧ�����CS??ʼÿ�պ�??
    yysys::CThreadGuard mutex_guard(&frozen_version_mutex_);
...
int ObRootServer2::start_import(const ObString& table_name, const uint64_t table_id, ObString& uri)
{
  ...
  {
    ret = OB_CURRENT_SYSTEM_MERGE_DOING;
    YYSYS_LOG(ERROR, "can not load data when merge doing, ret=%d",ret);
  }
...
void ObRootServer2::handle_orphan_index()
{
  ...
  {
    err = OB_ALLOCATE_MEMORY_FAILED;
    YYSYS_LOG(WARN, "fail to new schema manager. ret=%d", err);
  }
  else if (OB_SUCCESS != (get_schema(false, false, *schema_manager)))
  {
    YYSYS_LOG(WARN, "failed to get schema to get index schema");
  }
...
/*
 *����__all_cchecksum_infoϵͳ??
 *�����ڵ�ǰ��??3���汾��column checksumn��Ϣɾ��
 */
...
int ObRootServer2::receive_new_quorum_scale_request(const ObMsgRsNewQuorumScale new_quorum_scale)
{
  ...
  if(new_quorum_scale.term_ >= rs_node_mgr_->get_my_term())
  {
    ...
    if (-1 < new_quorum_scale.paxos_group_id_)
    {
      ...
      config_.paxos_ups_quorum_scales.set_value(temp_str);
      rs_node_mgr_->set_paxos_ups_quorum_scales(config_.paxos_ups_quorum_scales);
      YYSYS_LOG(INFO, "receive new quorum scale [%d] of paxos group [%d]; and after change, the string paxos_ups_quorum_scales is [%s]", new_quorum_scale.new_ups_quorum_scale_, new_quorum_scale.paxos_group_id_, (const char *)(config_.paxos_ups_quorum_scales));
    }
    else
    {
      ret = OB_INVALID_ARGUMENT;
  ...
  if (this->is_master())
  {
    ...
    for ( ; OB_SUCCESS == err && it != this->server_manager_.end() ;++it)
    {
      if (it->status_ != ObServerStatus::STATUS_DEAD && it->port_cs_ != 0)
      {
        ...
        {
          YYSYS_LOG(INFO, "the cs[%s] finish the collection task, progress is [%6.2f\%%]",
                    to_cstring(tmp_server), progress);
        }
        else
        {
          ret = false;
          YYSYS_LOG(INFO, "the cs[%s] don't finish the collection task, progress is [%6.2f\%%]",
                    to_cstring(tmp_server), progress);
        }
      }
    }
...
int ObRootServer2::start_get_statistics_task_operation(char *db_name)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS !=
            (ret = sql_str.serialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position())))
    {
      YYSYS_LOG(WARN, "failed to serialize sql, ret=%d", ret);
    }
    else if (OB_SUCCESS !=
             (ret = client_mgr->send_request(ms, OB_SQL_EXECUTE, DEFAULT_VERSION, timeout, msgbuf, session_id)))
    {
    ...
    for (; it != schema_mgr->column_end() && OB_SUCCESS == ret ; it++)
    {
      ...
      if (table_id >= 3001)
      {
        ...
        else if (strcmp(db_name, null.ptr()) != 0)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "get table name failed , table name is null,ret[%d]", ret);
          }
          else
          {
        ...
        if (OB_SUCCESS == ret && table_schema->get_index_helper().tbl_tid != OB_INVALID_ID)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get column id from index table failed,ret[%d]", ret);
          }
...
int ObRootServer2::fetch_range_collection_list(const int64_t version, int64_t start, int64_t end, ObArray<uint64_t> *list)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR,"root worker can't be null");
  }
  if (OB_SUCCESS == ret)
  {
...
int32_t ObRootServer2::set_ups_quorum_scale_of_paxos(int32_t paxos_id, int32_t quorum_scale)
{
  ...
  {
    ret = OB_PAXOS_IDX_OUT_RANGE;
    YYSYS_LOG(WARN, "sorry, the paxos_idx [%d] is out of range, which should be between 0 and %ld, ret=%d", paxos_id, OB_MAX_PAXOS_GROUP_COUNT - 1, ret);
  }
  else
  {
...
int32_t ObRootServer2::get_ups_quorum_scale_of_paxos(int32_t paxos_id, int32_t &quorum_scale)
{
  ...
  {
    ret = OB_PAXOS_IDX_OUT_RANGE;
    YYSYS_LOG(ERROR, "sorry, the paxos_idx [%d] is out of range, which should be between 0 and %ld, ret=%d", paxos_id, OB_MAX_PAXOS_GROUP_COUNT - 1, ret);
  }
  else
  {
...
bool ObRootServer2::is_paxos_quorum_scales_valid(const char *ups_quorum_scales)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR,"sorry, the input config string is out range. the input string is [%s], and its length is [%d]",
  ...
  if (is_paxos_quorum_scales_valid(ups_quorum_scales))
  {
    ...
    while (NULL != ptr)
    {
      if (count < config_.use_paxos_num)
      {
        ...
        {
          ret = OB_INVALID_ARGUMENT;
          YYSYS_LOG(ERROR, "invalid ups_quorum_scale occured when parse the config string, "
                    "paxos_id is [%d], it's scale is [%d]", count, temp_scale);
          break;
        }
    ...
    if (OB_SUCCESS == ret && 1 == count && config_.use_paxos_num > 1)
    {
      ...
      YYSYS_LOG(INFO, "only 1 ups quorum scales specified, but there are [%ld] paxos groups, so we use this quorum scale [%d] to "
                "initialize all paxos groups. ", (int64_t)config_.use_paxos_num, paxos_ups_quorum_scales_[0]);
    }
...
//[624]
int ObRootServer2::del_all_cluster(const int64_t cluster_idx)
{
    int ret = OB_SUCCESS;
    ObString sql;
    int64_t pos = 0;
    ...
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos, "delete from __all_cluster where cluster_id=%ld;", cluster_idx);
    if (pos >= OB_MAX_SQL_LENGTH)
    {
      ret = OB_BUF_NOT_ENOUGH;
      YYSYS_LOG(WARN, "buffer not enough, ret=%d", ret);
    ...
    else
    {
      ...
      for (int32_t i = 0; i < config_.retry_times; ++i)
      {
        if (OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, timeout)))
        {
          YYSYS_LOG(WARN, "delete from __all_cluster failed, ret=%d", ret);
        }
        else
        {
          break;
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "delete from __all_cluster failed, ret=%d", ret);
    }
    return ret;
}


int ObRootServer2::del_index_of_cluster(const int64_t cluster_idx)
{
    int ret = OB_SUCCESS;
    ObString sql;
    int64_t pos = 0;
    ...
    databuff_printf(sql_char, OB_MAX_SQL_LENGTH, pos, "delete from __index_process_info where cluster_id=%ld;", cluster_idx);
    if (pos >= OB_MAX_SQL_LENGTH)
    {
      ret = OB_BUF_NOT_ENOUGH;
      YYSYS_LOG(WARN, "buffer not enough, ret=%d", ret);
    ...
    else
    {
      ...
      for (int32_t i = 0; i < config_.retry_times; ++i)
      {
        if (OB_SUCCESS != (ret = get_rpc_stub().execute_sql(ms_server, sql, timeout)))
        {
          YYSYS_LOG(WARN, "delete from __index_process_info failed, ret=%d", ret);
        }
        else
        {
          break;
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "delete from __index_process_info failed, ret=%d", ret);
    }
    return ret;
}
...
//[index]
int ObRootServer2::create_view(const bool do_replace, const common::TableSchema &tschema)
{
    int ret = OB_SUCCESS;
    YYSYS_LOG(INFO, "create view, do_replace=%c view_name=%s",
              do_replace?'Y':'N', tschema.table_name_);
    uint64_t old_table_id = tschema.table_id_;
    if (OB_INVALID_ID == old_table_id)
    {
      const_cast<TableSchema &> (tschema).table_id_ = OB_APP_MIN_TABLE_ID - 1;
    ...
    {
      YYSYS_LOG(USER_ERROR, "create view failed, while table_id[%ld] less than %ld, and drop system table switch is %s",
                tschema.table_id_, OB_APP_MIN_TABLE_ID, config_.ddl_system_table_switch?"true":"false");
      ret = OB_OP_NOT_ALLOW;
    }
    ...
    if (OB_SUCCESS == ret)
    {
        ...
        ObString dt_name;
        dt_name.assign_buffer(buf, OB_MAX_COMPLETE_TABLE_NAME_LENGTH);
        dt_name.concat(tschema.dbname_,tschema.table_name_);
        bool exist = false;
        bool is_view = false;
      ...
      // LOCK BLOCK
      {
        ...
        if(ret == OB_SUCCESS)
        {
            if (OB_SUCCESS != (ret = check_view_exist(dt_name, exist, is_view)))
            {

            }
            else if(exist && !do_replace)
            {
                ret = OB_ENTRY_EXIST;
                YYSYS_LOG(INFO, "check view already exist:viewname[%s]", tschema.table_name_);
            }
            else if(exist && do_replace && !is_view) //the same name table(not view) is exist
            {
                ret = OB_ENTRY_EXIST;
                YYSYS_LOG(INFO, "check table already exist:table_name[%s]", tschema.table_name_);
            }
            else if(exist && is_view && do_replace) //the same name view is exist
            {
                if(OB_SUCCESS != (ret = ddl_tool_.drop_table(dt_name)))
                {
                    YYSYS_LOG(WARN, "drop view through ddl tool failed:tname[%s], err[%d]", tschema.table_name_, ret);
                }
            }
        }
        if(ret == OB_SUCCESS)
        {
            ...
            if (OB_SUCCESS != (ret = ddl_tool_.create_view(tschema)))
            {
              YYSYS_LOG(WARN, "create view throuth ddl tool failed:tname[%s], err[%d]", tschema.table_name_, ret);
            }
        }
...
int ObRootServer2::check_view_exist(const ObString &table_name, bool &exist, bool &is_view)
{
    int ret = OB_SUCCESS;
    exist = false;
    is_view = false;
    ...
    {
        const ObTableSchema *table = NULL;
        if(NULL == (table = schema_manager_for_cache_->get_table_schema(table_name)))
        {
            exist = false;
            is_view = false;
        }
        else if(table->get_type() == ObTableSchema::VIEW)
        {
            exist = true;
            is_view = true;
        }
...
int ObRootServer2::drop_one_view(bool if_exists, const common::ObString &view_name, bool &refresh)
{
    refresh = false;
    bool exist = false;
    bool is_view = false;

    int ret = check_view_exist(view_name, exist, is_view);
    if (OB_SUCCESS == ret)
    {
      if (!exist && !if_exists)
      {
        ret = OB_ENTRY_NOT_EXIST;
        YYSYS_LOG(WARN, "check view not exist:viewname[%.*s]", view_name.length(), view_name.ptr());
      }
      else if (exist && !is_view)
      {
          ret = OB_ERR_OBJECT_TYPE;
        YYSYS_LOG(WARN, "[%.*s] is not a VIEW", view_name.length(), view_name.ptr());
      }
    }
    ...
    if ((OB_SUCCESS == ret) && exist)
    {
      ...
      {
        table = schema_manager_for_cache_->get_table_schema(view_name);
        if (NULL == table)
        {
          ret = OB_ENTRY_NOT_EXIST;
          YYSYS_LOG(WARN, "check view not exist:viewname[%.*s]", view_name.length(), view_name.ptr());
        ...
        else
        {
          ...
          if (table_id < OB_APP_MIN_TABLE_ID && !config_.ddl_system_table_switch)
          {
            YYSYS_LOG(USER_ERROR, "drop view failed, while view_id[%ld] less than %ld, and drop system table switch is %s",
                      table_id, OB_APP_MIN_TABLE_ID, config_.ddl_system_table_switch?"true":"false");
            ret = OB_OP_NOT_ALLOW;
    ...
    {
      refresh = true;
      ret = ddl_tool_.drop_table(view_name);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "drop view throuth ddl tool failed:viewname[%.*s], ret[%d]",
                  view_name.length(), view_name.ptr(), ret);
      }
      else
      {
        YYSYS_LOG(INFO, "drop view succ:tname[%.*s]", view_name.length(), view_name.ptr());
      }
    }
    return ret;
}

int ObRootServer2::drop_views(bool if_exists, const common::ObStrings &views)
{
    YYSYS_LOG(INFO, "drop view, if_exists=%c views=[%s]",
              if_exists?'Y':'N', to_cstring(views));
    ObString view_name;
    bool force_update_schema = false;
    bool is_all_merged = false;
    ...
    else if (true == is_all_merged)
    {
      ...
      {
        yysys::CThreadGuard guard(&mutex_lock_);
        for (int64_t i = 0; i < views.count(); ++i)
        {
          ret = views.get_string(i, view_name);
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "get table failed:index[%ld], ret[%d]", i, ret);
            break;
          ...
          {
            bool refresh = false;
            ret = drop_one_view(if_exists, view_name, refresh);
            if (true == refresh && OB_SUCCESS == ret)
            {
              force_update_schema = true;
            }
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(WARN, "drop this view failed:index[%ld], tname[%.*s], ret[%d]",
                        i, view_name.length(), view_name.ptr(), ret);
              break;
            }
            else
            {
              YYSYS_LOG(INFO, "drop this view succ:index[%ld], tname[%.*s]",
                        i, view_name.length(), view_name.ptr());
            }
          }
        }
