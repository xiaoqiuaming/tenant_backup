...
namespace oceanbase
{
  namespace rootserver
  {
        ...
        const ObChunkServerManager* server_manager)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid arugment, root_server=%p, config=%p, rpc_stub=%p, server_manager=%p",
        ...
        const int64_t frozen_version)
    {
      ...
      if (NULL == schema_mgr)
      {
        YYSYS_LOG(WARN, "INVALID ARGUMENT. schema_mgr=%p, table_name_id.count()=%ld", schema_mgr, table_name_id.count());
        ret = OB_INVALID_ARGUMENT;
      }
      OperationType type;
      if (OB_SUCCESS == ret)
      {
        ...
        switch (type)
        {
          case CLEAN_ROOT_TABLE:
            {
              YYSYS_LOG(INFO, "receive order to clean old roottable.");
              ret = clean_root_table(schema_mgr);
              break;
            }
          case IMPORT_ALL:
            {
              YYSYS_LOG(INFO, "start to import all table. for cluster dulplicate.");
              ret = import_all_table(frozen_version, schema_mgr, table_name_id);
              break;
            }
          case IMPORT_TABLE:
            {
              YYSYS_LOG(INFO, "start to import table. for bypass process");
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to start operation. operation_type=%d, ret=%d", type, ret);
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "start to clean root table.");
      if (NULL == schema_mgr)
      {
        YYSYS_LOG(WARN, "invalid argument. schema_mgr=%p", schema_mgr);
        ret = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to generate new data. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        start_time_us_ = yysys::CTimeUtil::getTime();
        ret = root_server_->request_cs_report_tablet();
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to request cs report tablet. ret=%d", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "root server enter report_tablet process");
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "start to import all table.");
      if (NULL == schema_mgr)
      {
        YYSYS_LOG(WARN, "invalid argument. schema_mgr=%p", schema_mgr);
        ret = OB_INVALID_ARGUMENT;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to request cs load bypass table. ret=%d", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "root server enter load_sstable process");
        ...
        const ObBypassTaskInfo &table_name_id)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to init schema_mgr while bypass proces. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < table_name_id.count(); i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to generate new schema. ret=%d", ret);
            break;
          }
          else
          {
            YYSYS_LOG(INFO, "bypass process, table_name=%s, new_table_id=%ld", table_name, new_table_id);
          }
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to generate new roottable. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
    ...
    int ObRootOperationHelper::check_delete_tables_process()
    {
      ...
      YYSYS_LOG(INFO, "delete index=%ld, delete table count=%ld", delete_index_, delete_table.count());
      if (delete_index_ >= delete_table.count())
      {
        if (done_count_ >= total_count_)
        {
          ret = OB_SUCCESS;
          YYSYS_LOG(INFO, "delete process is done. done_count=%ld, total_count=%ld", done_count_, total_count_);
          done_count_ = 0;
          total_count_ = 0;
          ret = OB_SUCCESS;
        }
      ...
      else
      {
        ...
        for (; it != server_manager_->end(); ++it)
        {
          if (it->status_ != ObServerStatus::STATUS_DEAD)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to request cs to delete table,table_id=%lu. cs_addr=%s", table_id, it->server_.to_cstring());
            }
            else
            {
              total_count_ ++;
        ...
        YYSYS_LOG(INFO, "request cs to delete table. table_id=%ld, total_count=%ld",
      ...
      if (OB_SUCCESS != ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "cs delete table timeout. start_time=%ld, timeout=%ld, now=%ld",
    ...
    {
      YYSYS_LOG(INFO, "cs_index=%d  delete table finished. table_id=%lu, is_succ=%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          for (int64_t i = 0; i < table_name_id.count(); i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to add table. ret=%d, i = %ld", ret, i);
              break;
            }
            else
            {
              YYSYS_LOG(INFO, "bypass proces: request cs load tablet, table_id=%lu", const_cast<ObBypassTaskInfo*>(&table_name_id)->at(i).second);
            }
          }
        }
      }
        ...
        const common::ObTableImportInfoList &table_list, const bool is_load_succ)
    {
      ...
      if (!is_load_succ)
      {
        YYSYS_LOG(WARN, "cs_index %d fail to load tablet.", cs_index);
      }
      else
      {
        YYSYS_LOG(INFO, "chunkserver %d finish load sstable, table_list=%s", cs_index, to_cstring(table_list));
        ObServerStatus* status = server_manager_->get_server_status(cs_index);
        if (NULL == status)
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "invalid status.  status=%p, cs_index=%d", status, cs_index);
        }
        else
        {
        ...
        const int32_t server_index, const int64_t frozen_mem_version)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "bypass not in report process. check the stat. process=%s", print_process());
      }
      else
      {
    ...
    int ObRootOperationHelper::waiting_job_done(const int cs_index)
    {
      ...
      {
        YYSYS_LOG(INFO, "cs finish report for new root table. cs_index=%d", cs_index);
    ...
    int ObRootOperationHelper::set_delete_table(common::ObArray<uint64_t> &delete_tables)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument. table count=%ld", delete_tables.count());
      }
      else
      {
    ...
    int ObRootOperationHelper::check_load_sstable_process()
    {
      ...
      for (; it != server_manager_->end(); ++it)
      {
        ...
        {
          YYSYS_LOG(TRACE, "cs have not finish load. server=%s", to_cstring(it->server_));
          ret = OB_ERROR;
          break;
        }
      }
      if (OB_SUCCESS != ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "cs load sstable timeout. start_time=%ld, max_timeout=%ld, now=%ld",
              start_time_us_, timeout, now);
          ret = OB_BYPASS_TIMEOUT;
        }
      }
    ...
    int ObRootOperationHelper::check_report_tablet_process()
    {
      ...
      for (; it != server_manager_->end(); ++it)
      {
        ...
        {
          YYSYS_LOG(TRACE, "cs have not report tablet. server=%s", to_cstring(it->server_));
          ret = OB_ERROR;
          break;
        }
      }
      if (OB_SUCCESS != ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "cs report tablet timeout. start_time=%ld, max_timeout=%ld, now=%ld",
              start_time_us_, timeout, now);
          ret = OB_BYPASS_TIMEOUT;
        }
      }
    ...
    int ObRootOperationHelper::check_root_table()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "cs report some useless tablet. please check it");
        }
        YYSYS_LOG(INFO, "check root table success.");
      }
      else
      {
        int64_t now = yysys::CTimeUtil::getTime();
    ...
    int ObRootOperationHelper::check_process(OperationType &type)
    {
      ...
      if (process_ == LOAD_SSTABLE)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "check cs load sstable success. enter the next process:report tablet");
          process_ = REPORT_TABLET;
          start_time_us_ = yysys::CTimeUtil::getTime();
        ...
        {
          YYSYS_LOG(WARN, "load sstable timeout. request cs report new tablet. try to build roottable, process:report tablet");
          process_ = REPORT_TABLET;
          start_time_us_ = yysys::CTimeUtil::getTime();
          root_server_->request_cs_report_tablet();
        }
      ...
      if (REPORT_TABLET == process_)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "check cs report tablet success.enter the next processi:sort root table");
          ...
          || (OB_BYPASS_TIMEOUT == ret && REPORT_TABLET == process_))
      {
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "check root table success. notity switch root_table");
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to finish bypass meta data. err=%d", ret);
            bypass_data_.destroy_data();
            end_bypass_process();
          }
          else if (type == IMPORT_TABLE)
          {
            YYSYS_LOG(INFO, "bypass process new root table ok, try to delete old tablets");
            start_time_us_ = yysys::CTimeUtil::getTime();
        ...
        {
          YYSYS_LOG(WARN, "check roottable timeout. end the bypass process");
          bypass_data_.destroy_data();
          end_bypass_process();
        }
      }
      ...
      if (DELETE_TABLE == process_)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "check delete table success. finish the all process");
        }
      }
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "operation process is all finished.");
        end_bypass_process();
      }
      return ret;
    }
