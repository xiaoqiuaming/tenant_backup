...
namespace oceanbase
{
  namespace rootserver
  {
    ...
    void ObRootLogReplay::wait_replay(ObLogCursor& end_cursor)
    {
      ...
      {
        YYSYS_LOG(ERROR, "end_cursor[%s] is invalid", to_cstring(end_cursor));
      }
    ...
    int ObRootLogReplay::replay(common::LogCommand cmd, uint64_t seq, const char* log_data, const int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "fatal error, replay log cmd:[%d], log_data[%.*s], seq[%lu], failed, err=[%d]. Quit...",
    ...
    void ObRootLogReplay::run(yysys::CThread* thread, void* arg)
    {
      int ret = OB_SUCCESS;
      ...
      if (!is_initialized_)
      {
        YYSYS_LOG(ERROR, "ObLogReplayRunnable has not been initialized");
        ret = OB_NOT_INIT;
      }
      else
      {
        while (!_stop)
        {
          ...
          if (OB_READ_NOTHING == ret)
          {
            if (ObRoleMgr::MASTER == role_mgr_->get_role() && master_end_cursor_.is_valid())
            {
              ...
              {
                YYSYS_LOG(ERROR, "get_replayed_cursor(master_end_cursor[%s])=>%d",
              ...
              {
                // YYSYS_LOG(WARN, "wait_replay_retry_num[%ld], replay_cursor[%s] != master_end_cursor[%s], need wait replay",
                //           wait_replay_retry_num, to_cstring(replayed_cursor), to_cstring(master_end_cursor_));
                if (0 == (++wait_replay_retry_num % 1000))
                {
                  ret = OB_LOG_NOT_SYNC;
                  YYSYS_LOG(ERROR, "wait_replay_retry too many times[%ld], [%s->%s]",
                            wait_replay_retry_num, to_cstring(replayed_cursor), to_cstring(master_end_cursor_));
                }
              }
            }
          ...
          {
            YYSYS_LOG(ERROR, "ObLogReader read_data error[ret=%d]", ret);
            break;
          }
          else
          {
            if (OB_LOG_NOP != cmd)
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(ERROR, "replay log error[ret=%d]", ret);
                hex_dump(log_data, static_cast<int32_t>(data_len), false, YYSYS_LOG_LEVEL_ERROR);
                break;
              }
            }
          }
      ...
      if (NULL != role_mgr_) // double check
      {
        ...
        {
          YYSYS_LOG(WARN, "role_mgr_=%p", role_mgr_);
          role_mgr_->set_state(ObRoleMgr::ERROR);
        }
      }
      YYSYS_LOG(INFO, "ObLogReplayRunnable finished[stop=%d ret=%d]", _stop, ret);
    }
