...
bool ObRootTable2::internal_check() const
{
  ...
  {
    YYSYS_LOG(ERROR, "tablet_info_manager_ not inited");
    ret = false;
  }
//  else if (!has_been_sorted())
//  {
//    YYSYS_LOG(INFO, "not sorted");
//    ret = false;
//  }
  return ret;
}
    ...
    const_iterator& last) const
{
  ...
  {
    YYSYS_LOG(INFO, "root table is empty");
    first = begin();
    last = begin();
  ...
  else
  {
    ...
    {
      YYSYS_LOG(DEBUG, "find start pos error find_start_pos = %p end = %p", start_pos, end());
      ret = OB_FIND_OUT_OF_RANGE;
      first = end();
      last = end();
    }
    ...
    {
      YYSYS_LOG(DEBUG, "get_table_info null");
    ...
    else
    {
          ...
          && !range.border_flag_.inclusive_start())
      {
        ...
        if (first >= end())
        {
          YYSYS_LOG(DEBUG, "first[%p] >= end[%p] ", first, end());
          ret = OB_FIND_OUT_OF_RANGE;
        }
        else if (NULL == (tablet_info = get_tablet_info(first)))
        {
          YYSYS_LOG(DEBUG, "get_tablet_info null");
          ret = OB_ERROR_OUT_OF_RANGE;
        }
      }
      // find the last range
      if (OB_NOT_INIT == ret)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "last[%p] == end[%p] ", last, end());
          last = end();
          ret = OB_FIND_OUT_OF_RANGE;
        }
        else
      ...
      {
        YYSYS_LOG(DEBUG, "first[%p] > last[%p] ", first, last);
        ret = OB_ERROR_OUT_OF_RANGE;
      }
    }
  }
  ...
  {
    ret = OB_ERROR_OUT_OF_RANGE;
    YYSYS_LOG(WARN, "no proper range found");
  }
  return ret;
}
    ...
    const_iterator& ptr) const
{
  ...
  else
  {
   ...
   //   assert(false);
    ...
    {
      YYSYS_LOG(WARN, "not find. find_range=%s, find_pos=%p, begin()=%p, end()=%p",
    ...
    {
      ret = OB_ERROR_OUT_OF_RANGE;
      YYSYS_LOG(WARN, "get tablet_info fail. err =%d", ret);
    }
    else
    {
      ptr = find_pos;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == tablet_info || tablet_info->range_.compare_with_startkey(start_range) > 0)
        {
          ...
          YYSYS_LOG(WARN, "find range not exist.key=%s, find_pos.range=%s", start_key, rowkey);
          ret = OB_FIND_OUT_OF_RANGE;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (last < first)
        {
          YYSYS_LOG(WARN, "table not exist in root_table. table_id=%lu", table_id);
          ret = OB_FIND_OUT_OF_RANGE;
        }
      }
    }
...
int ObRootTable2::get_table_row_checksum(const int64_t tablet_version, const uint64_t table_id, uint64_t &row_checksum)
{
  ...
  if (start_pos != end())
  {
    ...
    for(const_iterator it = start_pos; it != end() && OB_SUCCESS == ret; ++it)
    {
      ...
      if(NULL != tablet_info)
      {
        ...
        {
          YYSYS_LOG(WARN, "bug, table_id:%lu miss some range, last_range:%s now_range:%s", table_id, to_cstring(last_tablet_info->range_), to_cstring(tablet_info->range_));
        }
        else
        {
          ...
          if(NULL != crc_helper)
          {
            ...
            {
              YYSYS_LOG(WARN, "range:%s tablet_version:%ld get row_checksum fail ret:%d", to_cstring(tablet_info->range_), tablet_version, ret);
              break;
            }
            else
            {
  ...
  {
    YYSYS_LOG(WARN, "not found tablet_version:%ld table_id:%lu", tablet_version, table_id);
  }
...
bool ObRootTable2::table_is_exist(const uint64_t table_id) const
{
  ...
  if (start_pos != end())
  {
    ...
    if (tablet_info == NULL)
    {
      YYSYS_LOG(ERROR, "get_tablet_info NULL");
    }
    else
    {
  ...
  for (int32_t i = 0; i < (int32_t)meta_table_.get_array_index(); ++i)
  {
    ...
    //add e
    {
      if (data_holder_[i].server_info_indexes_[j] == server_index)
      {
        ...
        if (time_stamp > 0 ) atomic_exchange((uint64_t*) &(data_holder_[i].last_dead_server_time_), static_cast<uint64_t>(time_stamp));//mod support for arm platform by wangd 202106
        count++;
      }
    }
  }
  YYSYS_LOG(INFO, "delete replicas, server_index=%d count=%ld", server_index, count);
  return;
}
...
{
  YYSYS_LOG(INFO, "meta_table_.size():%d", (int32_t)meta_table_.get_array_index());
  if (tablet_info_manager_ != NULL)
  {
    for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
    {
      YYSYS_LOG(DEBUG, "dump the %d-th tablet info, index=%d", i, data_holder_[i].tablet_info_index_);
      tablet_info_manager_->hex_dump(data_holder_[i].tablet_info_index_, YYSYS_LOG_LEVEL_INFO);
      YYSYS_LOG(DEBUG, "dump the %d-th tablet info end", i);
      data_holder_[i].dump();
      YYSYS_LOG(DEBUG, "dump the %d-th server info end", i);
    }
  }
  YYSYS_LOG(INFO, "dump meta_table_ complete");
  return;
}
...
void ObRootTable2::dump(const int32_t index) const
{
  if (tablet_info_manager_ != NULL)
  {
    ...
    {
      tablet_info_manager_->hex_dump(data_holder_[index].tablet_info_index_, YYSYS_LOG_LEVEL_INFO);
...
void ObRootTable2::dump_unusual_tablets(int64_t current_version, int32_t replicas_num, int32_t &num) const
{
  num = 0;
  YYSYS_LOG(INFO, "meta_table_.size():%d", (int32_t)meta_table_.get_array_index());
  YYSYS_LOG(INFO, "current_version=%ld replicas_num=%d", current_version, replicas_num);
  if (tablet_info_manager_ != NULL)
  {
    for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
    {
      ...
      if (new_num < replicas_num || is_merging)
      {
  ...
  YYSYS_LOG(INFO, "dump meta_table_ complete, num=%d", num);
  return;
}
...
int ObRootTable2::check_tablet_version_merged(const int64_t tablet_version, const int64_t safe_count, bool &is_merged) const
{
  ...
  if (NULL == tablet_info_manager_)
  {
    YYSYS_LOG(WARN, "tablet_info_manager is null");
    is_merged = false;
    err = OB_ERROR;
  }
  else
    ...
    for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
    {
	  ...
	  //add e
      {
        ...
        if (data_holder_[i].server_info_indexes_[j] != OB_INVALID_INDEX)
        {
          ...
          {
            YYSYS_LOG(TRACE, "root_table[%p] server[%d]'s tablet version[%ld] less than required[%ld]",
      ...
      if (succ_count < safe_count)
      {
        is_merged = false;
        ++fail_count;
        YYSYS_LOG(TRACE, "check tablet not all safe copy merged to this version:succ[%d], safe[%ld]",
  ...
  if (!is_merged)
  {
    YYSYS_LOG(INFO, "there are %d tablet not satisfy the version[%ld] and safe_copy_count[%ld], the merging tablet count=%d",
    ...
    for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
    {
      ...
      if (NULL != tablet_info)
      {
        ...
        if (NULL != table_schema
            && OB_INVALID_ID != (src_table_id = table_schema->get_index_helper().tbl_tid) 
            && NULL == schema_mgr.get_table_schema(src_table_id))
        {
          root_server_->set_if_orphan_index_exist(true);
          YYSYS_LOG(TRACE, "orphan index, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
          continue;
        }
        if (NULL == table_schema)
        {
          YYSYS_LOG(TRACE, "get table schema failed, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
          continue;
        }
        ...
        {
          YYSYS_LOG(TRACE, "unavailable index, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
          continue;
        }
      }
      else
      {
        YYSYS_LOG(WARN, "tablet_info is NULL, tablet info index = [%d]", data_holder_[i].tablet_info_index_);
        continue;
      }
      //add:e
      //add zhaoqiong[roottable tablet management]20150127:b
...
int ObRootTable2::check_tablet_version_merged_v3(const uint64_t table_id, const int64_t tablet_version, const int64_t safe_count, bool &is_merged)
{
  ...
  for (const_iterator it = begin_pos; it <= end_pos && OB_LIKELY(OB_SUCCESS == ret); it++)
  {
    ...
    if (succ_count < safe_count)
    {
      is_merged = false;
      ++fail_count;
      YYSYS_LOG(TRACE, "check tablet not all safe copy merged to this version:succ[%d], safe[%ld], tid[%lu]",
          succ_count, safe_count, table_id);
    }
  }//end outer for
  if (!is_merged)
  {
    YYSYS_LOG(INFO, "table[%lu] has %d tablet not safity the version[%ld] and safe_copy_count[%ld], the merging tablet count=%d",
...
int ObRootTable2::check_tablet_complete_in_clusters(const uint64_t table_id, const int64_t tablet_version, bool is_complete[OB_MAX_CLUSTER_COUNT])
{
    int ret = OB_SUCCESS;
    int64_t unique_tablet_num_each_cluster[OB_MAX_CLUSTER_COUNT];
    int64_t cur_tablet_num = 0;
    int32_t copy_cluster_id = -1;
    ...
    for(int32_t i = 0; i < OB_MAX_CLUSTER_COUNT; ++i)
    {
        is_complete[i] = true;
        unique_tablet_num_each_cluster[i] = 0;
    }
    ...
    for(const_iterator it = begin_pos; it <= end_pos && OB_LIKELY(OB_SUCCESS == ret); it++)
    {
        ...
        for(int32_t j = 0; j < OB_MAX_COPY_COUNT; ++j)
        {
            if(it->server_info_indexes_[j] != OB_INVALID_INDEX && it->tablet_version_[j] == tablet_version)
            {
                copy_cluster_id = root_server_->get_server_manager().get_cs(it->server_info_indexes_[j]).cluster_id_;
                unique_tablet_num_each_cluster[copy_cluster_id]++;
            }
        }
        for(int32_t j = 0; j < OB_MAX_CLUSTER_COUNT; ++j)
        {
            if(is_complete[j])
            {
                if(unique_tablet_num_each_cluster[j] > cur_tablet_num)
                {
                    unique_tablet_num_each_cluster[j] = cur_tablet_num;
                }
                else if(unique_tablet_num_each_cluster[j] < cur_tablet_num)
                {
                    is_complete[j] = false; //tablet has no copy, not complete
                }
            }
        }
    }
...
const ObTabletInfo* ObRootTable2::get_tablet_info(const int32_t meta_index) const
{
  ...
  if (meta_index < meta_table_.get_array_index() && meta_index >=0 && NULL != tablet_info_manager_)
  {
    ...
    if (NULL == tablet_info)
    {
      YYSYS_LOG(ERROR, "no tablet info found in tablet_info_manager_ with tablet_index=%d", tablet_index);
    }
  }
  else
  {
    YYSYS_LOG(WARN, "no tablet info found, meta_index=%d rt_size=%ld tablet_info_manager_=%p",
...
const ObTabletInfo* ObRootTable2::get_tablet_info(const const_iterator& it) const
{
  ...
  {
    YYSYS_LOG(WARN, "no tablet info found it=%p end()=%p begin()=%p tablet_info_manager_=%p",
...
bool ObRootTable2::move_back(const int32_t from_index_inclusive, const int32_t move_step)
{
  ...
  if (!ret)
  {
    YYSYS_LOG(ERROR, "too much data, not enought sapce");
  }
  return ret;
...
int ObRootTable2::get_root_meta(const ObTabletInfo &tablet_info, const_iterator &root_meta)
{
  ...
  {
    ret = OB_ERROR_OUT_OF_RANGE;
    YYSYS_LOG(WARN, "tablet info is not equal. err =%d", ret);
  }
  else
  {
    root_meta = find_pos;
...
int ObRootTable2::remove(const const_iterator& it, const int32_t safe_count, const int32_t server_index)
{
  ...
  if (safe_count <= 0)
  {
    ...
    YYSYS_LOG(WARN, "check safe count failed:safe_count[%d]", safe_count);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "not find the server or safe count failed:server[%d], safe[%d], current[%d]",
    ...
    const int64_t tablet_version, const int64_t seq)
{
  ...
  if (tablet_info_manager_ != NULL)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "add new tablet error have no enough space");
        ret = OB_ERROR;
      }
    }
  }
...
int ObRootTable2::create_table(const ObTabletInfo &tablet, const ObArray<int32_t> &chunkservers, const int64_t tablet_version)
{
  ...
  if (tablet_info_manager_ != NULL)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      YYSYS_LOG(DEBUG, "insert pos = %d", insert_pos);
      if(!move_back(insert_pos, 1))
      {
        YYSYS_LOG(ERROR, "too many tablet!!!");
        ret = OB_ERROR;
      }
    }
    int32_t tablet_index = OB_INVALID_INDEX;
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "too many tablet info!!!");
      }
      else
      {
        YYSYS_LOG(INFO, "add tablet info succ. tablet_index=%d", tablet_index);
        ObTabletCrcHistoryHelper *crc_helper = NULL;
        crc_helper = tablet_info_manager_->get_crc_helper(tablet_index);
        if (crc_helper != NULL)
        {
...
bool ObRootTable2::add_lost_range()
{
  ...
  while (i < (int32_t)meta_table_.get_array_index())
  {
    ...
    if (tablet_info == NULL)
    {
      YYSYS_LOG(ERROR, "invalid tablet info index is %d", i);
      ret = false;
      break;
    }
    ...
    if (tablet_info->range_.table_id_ != OB_INVALID_ID)
    {
      if (tablet_info->range_.table_id_ != last_range.table_id_ )
      {
        ...
        {
          YYSYS_LOG(ERROR, "table %lu not end correctly", last_range.table_id_);
        ...
        {
          YYSYS_LOG(ERROR, "table %lu not start correctly", tablet_info->range_.table_id_);
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "table %lu have a hole last range is ", last_range.table_id_);
          //last_range.hex_dump(YYSYS_LOG_LEVEL_INFO);
          static char row_key_dump_buff[OB_MAX_ROW_KEY_LENGTH * 2];
          last_range.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
          YYSYS_LOG(INFO, "%s", row_key_dump_buff);
          YYSYS_LOG(INFO, "now is table %lu  range is ", tablet_info->range_.table_id_);
          //tablet_info->range_.hex_dump(YYSYS_LOG_LEVEL_INFO);
...
bool ObRootTable2::check_lost_data(const int64_t server_index)const
{
  ...
  while (i < meta_table_.get_array_index())
  {
    ...
    if (have_hole)
    {
      ...
      if (NULL != tablet_info)
      {
        YYSYS_LOG(ERROR, "lost data, range=%s", to_cstring(tablet_info->range_));
      }
      else
      {
        YYSYS_LOG(WARN, "can't be here. i = %ld, it=%p", i, it);
      }
    }
    else
    {
  ...
  if (count > 0)
  {
    YYSYS_LOG(ERROR, "check lost data over, there are %ld range are lost.", count);
  }
  else
  {
    YYSYS_LOG(INFO, "check lost data over, all data are exist expect cs_index=%ld.", server_index);
  }
  //add zhaoqiong[roottable tablet management]20150127:b
  //YYSYS_LOG(INFO, "travel around roottable. there are %ld single_copy tablet, %ld double_copy tablet, %ld three_copy tablet",
     // single_copy_count, double_copy_count, three_copy_count);
  YYSYS_LOG(INFO, "travel around roottable. there are %ld single_copy tablet",
      copy_count_array[0]);
  //add e
  return ret;
} 
  ...
  while (i < (int32_t)meta_table_.get_array_index())
  {
    ...
    if (tablet_info->range_.table_id_ != OB_INVALID_ID)
    {
      ...
      if (NULL != schema_mgr)
      {
        ...
        {
          root_server_->set_if_orphan_index_exist(true);
          YYSYS_LOG(WARN, "orphan index, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
        ...
        if (NULL == table_schema)
        {
          YYSYS_LOG(WARN, "get table schema failed, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
        }
        else if (OB_INVALID_ID != table_schema->get_index_helper().tbl_tid
                 && AVALIBALE != table_schema->get_index_helper().status)
        {
          YYSYS_LOG(WARN, "invalid index, ignore it, table_id = [%lu]", tablet_info->range_.table_id_);
          is_invalid_index = true;
          ++i;
          continue;
        }
      ...
      if (tablet_info->range_.table_id_ != last_range.table_id_ )
      {
        if (!last_range.end_key_.is_max_row())
        {
          YYSYS_LOG(WARN, "table %lu not end correctly", last_range.table_id_);
          ret = false;
          //break;
        }
        if (!tablet_info->range_.start_key_.is_min_row())
        {
          YYSYS_LOG(WARN, "table %lu not start correctly", tablet_info->range_.table_id_);
          ret = false;
          //break;
        }
      }
      else
      {
        if (tablet_info->range_.start_key_.compare(last_range.end_key_) != 0)
        {
          YYSYS_LOG(WARN, "table not complete, table_id=%lu last_range=%s",
              last_range.table_id_, to_cstring(last_range));
          YYSYS_LOG(WARN, "cur_table_id=%lu  curr_range=%s",
...
int ObRootTable2::shrink_to(ObRootTable2* shrunk_table, ObTabletReportInfoList &delete_list)
{
      ...
      end() == sorted_end())
  {
    ...
    while (OB_SUCCESS == ret && it < end() )
    {
      ...
      if (NULL == tablet_info)
      {
        YYSYS_LOG(ERROR, "should never reach this bug occurs");
        ret = OB_ERROR;
        break;
      }
      last_range = tablet_info->range_;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "too much tablet");
          ret = OB_ERROR;
          break;
        }
      }
      ...
      {
        YYSYS_LOG(DEBUG, "shrink idx=%ld range=%s", it-begin(), to_cstring(tablet_info->range_));
        ...
        {
          YYSYS_LOG(ERROR, "crc check error,  crc=%lu last_crc=%lu last_cs=%d",
              tablet_info->crc_sum_, it->tablet_version_[0], it->server_info_indexes_[0]);
          YYSYS_LOG(ERROR, "error tablet is %s", to_cstring(last_range));
          YYSYS_LOG(ERROR, "tabletinfo in shrink table is");
          shrunk_table->data_holder_[last_tablet_index].dump();
          //last_range.hex_dump(YYSYS_LOG_LEVEL_ERROR);
        }
        else
        {
          ...
          if (0 != to_delete.tablet_location_.chunkserver_.get_port())
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to add delete tablet");
            }
          }
        }
        it++;
      ...
      if (OB_SUCCESS == ret)
      {
        for (it2 = it; it2 < it + OB_SAFE_COPY_COUNT * 2 && it2 < end() && OB_SUCCESS == ret; it2++)
        {
          ...
          {
            last_range.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
            YYSYS_LOG(ERROR, "lost some tablet, last_range=%s", row_key_dump_buff);
            tablet_info->range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
            YYSYS_LOG(ERROR, "this_range=%s", row_key_dump_buff);
            ret = OB_ERROR;
            break;
          }
          if (cmp_ret < 0)
          ...
          if (cmp_ret > 0)
          {
            YYSYS_LOG(ERROR, "you have not sorted it? bugs!!");
            ret = OB_ERROR;
          }
          if (cmp_ret == 0)
          {
            YYSYS_LOG(ERROR, "we should skip this , bugs!!");
            last_range.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
            YYSYS_LOG(ERROR, "last_range=%s", row_key_dump_buff);
            tablet_info->range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
            YYSYS_LOG(ERROR, "this_range=%s", row_key_dump_buff);
          ...
          //split it2;
          YYSYS_LOG(ERROR, "in this version we sould not reach this init data have some error");
          last_range.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
          YYSYS_LOG(ERROR, "last_range=%s", row_key_dump_buff);
          tablet_info->range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
          YYSYS_LOG(ERROR, "this_range=%s", row_key_dump_buff);
...
int32_t ObRootTable2::find_suitable_pos(const const_iterator& it, const int32_t server_index, const int64_t tablet_version, common::ObTabletReportInfo *to_delete/*=NULL*/)
{
  ...
  int32_t found_it_index = find_suitable_pos2(it, server_index, tablet_version, cluster_id, to_delete);
  return found_it_index;
}
...
int ObRootTable2::check_tablet_version(const int64_t tablet_version, int safe_copy_count) const
{
  ...
  for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
  {
    ...
    if (tablet_ok < safe_copy_count)
    {
      ...
      if (tablet_info != NULL)
      {
        YYSYS_LOG(WARN, "tablet version error now version is %ld, tablet is", tablet_version);
        static char row_key_dump_buff[OB_MAX_ROW_KEY_LENGTH * 2];
        tablet_info->range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
        YYSYS_LOG(INFO, "%s", row_key_dump_buff);
        //tablet_info->range_.hex_dump(YYSYS_LOG_LEVEL_INFO);
        ...
        //add e
        {
          YYSYS_LOG(INFO, "root table's tablet version is %ld", data_holder_[i].tablet_version_[j]);
        }
        ret = OB_ERROR;
      }
    }
...
bool ObRootTable2::check_tablet_copy_count(const int32_t copy_count) const
{
  ...
  for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
  {
    ...
    if (tablet_ok < copy_count)
    {
      ...
      if (tablet_info != NULL)
      {
        YYSYS_LOG(WARN, "tablet copy count less than %d", copy_count);
        static char row_key_dump_buff[OB_MAX_ROW_KEY_LENGTH * 2];
        tablet_info->range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
        YYSYS_LOG(INFO, "%s", row_key_dump_buff);
        //tablet_info->range_.hex_dump(YYSYS_LOG_LEVEL_INFO);
...
void ObRootTable2::merge_one_tablet(ObRootTable2* shrunk_table, int32_t last_tablet_index, const_iterator it, ObTabletReportInfo &to_delete)
{
  ...
  //add e
  {
    ...
    YYSYS_LOG(DEBUG, "delete replica, cs_idx=%d", to_delete.tablet_location_.chunkserver_.get_port());
...
/*
 * �õ�range���root table������Ӱ��
 * 1��range���ܵ���root table���� �ϲ� ��Ӱ���
 */
int ObRootTable2::get_range_pos_type(const common::ObNewRange& range, const const_iterator& first, const const_iterator& last) const
{
  ...
  if (tablet_info == NULL)
  {
    ...
    YYSYS_LOG(DEBUG, "maybe a new range, range=%s", row_key_dump_buff);
  }
  else if (first == end())
  {
    ret = POS_TYPE_ADD_RANGE;
    YYSYS_LOG(DEBUG, "add in the last, range=%s", row_key_dump_buff);
  }
  else if (last == end())
  {
    ...
    {
      YYSYS_LOG(WARN, "can't decide what type this range is[%s]", row_key_dump_buff);
    }
  }
  else
  {
    if (range.compare_with_startkey(tablet_info->range_) < 0 )
    {
      ...
      {
        ret = POS_TYPE_ADD_RANGE;
        YYSYS_LOG(DEBUG, "add new range %s", row_key_dump_buff);
      }
      ...
      if (POS_TYPE_ADD_RANGE == ret)
      {
        ...
        if (first != begin())
        {
          ...
          {
            ret = POS_TYPE_ERROR;
            YYSYS_LOG(ERROR, "no tablet_info found in table_info_manager.");
          }
          else
          {
            if (range.table_id_ == tablet_info->range_.table_id_)
            {
              ...
              {
                ret = POS_TYPE_ERROR;
                YYSYS_LOG(ERROR, "wrong range position");
              }
            }
          }
        }
    ...
    else
    {
      if (range.compare_with_startkey(tablet_info->range_) == 0)
      {
        ...
        {
          ret = POS_TYPE_ERROR;
          YYSYS_LOG(ERROR, "failed to get tablet info");
        }
    ...
    {
      YYSYS_LOG(ERROR, "No proper type is found");
      ret = POS_TYPE_ERROR;
    }
    if (ret == POS_TYPE_ERROR)
    {
      YYSYS_LOG(ERROR, "error range found");
      YYSYS_LOG(INFO, "tablet range is %s", to_cstring(tablet_info->range_));
      //tablet_info->range_.hex_dump(YYSYS_LOG_LEVEL_INFO);
      YYSYS_LOG(INFO, "reported range is %s", to_cstring(range));
      //range.hex_dump(YYSYS_LOG_LEVEL_INFO);
    }
  }
    ...
    const int64_t tablet_version, const int32_t server_index)
{
  ...
  {
    YYSYS_LOG(ERROR, "pos_type[%d] is not POS_TYPE_SPLIT_RANGE[%d]", pos_type, POS_TYPE_SPLIT_RANGE);
    ret = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "tablet_info[%p] or tablet_info_manager_[%p] should not be NULL",
    ...
    else
    {
      ...
      if (split_type == SPLIT_TYPE_TOP_HALF || split_type == SPLIT_TYPE_BOTTOM_HALF)
      {
        if (from_pos_inclusive <  meta_table_.get_array_index() )
        {
          ...
          {
            YYSYS_LOG(ERROR, "move back error");
            ret = OB_ERROR;
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "why you reach this bugss!!");
          ret = OB_ERROR;
        }
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(ERROR, "found_index should not be OB_INVALID_INDEX");
            }
          }
        }
      }
      else if (split_type == SPLIT_TYPE_MIDDLE_HALF)
      {
        ...
        {
          YYSYS_LOG(ERROR, "why we got this bugss!");
          ret = OB_ERROR;
        }
        if (OB_SUCCESS == ret)
        {
...
int ObRootTable2::write_to_file(const char* filename)
{
  ...
  if (tablet_info_manager_ == NULL)
  {
    YYSYS_LOG(ERROR, "tablet_info_manager should not be NULL bugs!!");
    ret = OB_ERROR;
  }
  if (ret == OB_SUCCESS)
  {
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        for(int64_t i=0; i<total_count; ++i)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "serialize tablet failed:index[%ld], total[%ld], buff_size[%ld], buff_pos[%ld]",
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "write data into [%s] failed", filename);
        }
        else
        {
          YYSYS_LOG(DEBUG, "root table has been write into [%s]", filename);
        }
      }
      fu.close();
    }
...
int ObRootTable2::read_from_file(const char* filename)
{
  ...
  YYSYS_LOG(DEBUG, "read root table from [%s]", filename);
  int64_t header_length = sizeof(common::ObRecordHeader);
  common::ObRecordHeader header;
  if (ret == OB_SUCCESS)
  {
  ...
  YYSYS_LOG(DEBUG, "header read success ret: %d", ret);
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (cr != OB_SUCCESS)
    {
      ...
      YYSYS_LOG(ERROR, "data check failed, rc: %d", cr);
    }
  }
...
int ObRootTable2::shrink()
{
  ...
  YYSYS_LOG(INFO, "tablet num after shrink, num=%d", sorted_count_);
  return ret;
}
...
//int ObRootTable2::get_deleted_table(const common::ObSchemaManagerV2 & schema, common::ObArray<uint64_t>& deleted_tables) const
// mod e
{
  int ret = OB_SUCCESS;
  ...
  for (it = this->begin(); it != this->end(); ++it)
  {
    ...
    //mod e
    {
      ...
      {
        YYSYS_LOG(ERROR, "delete table id is less than OB_APP_MIN_TABLE_ID:table_id[%lu]", cur_table_id);
        break;
      }
      else if (cur_table_id == OB_INVALID_ID)
      {
        YYSYS_LOG(ERROR, "table_id must not be OB_INVALID_ID=%lu", OB_INVALID_ID);
        break;
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "push deleted table failed:table_id[%lu], ret[%d]", cur_table_id, ret);
          break;
        }
        else
        {
          last_deleted_table_id = cur_table_id;
          YYSYS_LOG(INFO, "find a table not in schema and not a loading table:table_id[%lu]", cur_table_id);
        }
      }
    }
  }
...
int ObRootTable2::delete_tables(const common::ObArray<uint64_t> &deleted_tables)
{
  ...
  for (it = this->begin(); it != this->end(); ++it)
  {
    ...
    for (int j = 0; j < deleted_tables.count(); ++j)
    {
      ...
      if (tablet->range_.table_id_ == deleted_tables.at(j))
      {
        ...
        YYSYS_LOG(INFO, "delete tablets from roottable, range=%s", buf);
        break;
      }
    }
  } // end for
  ...
  {
    YYSYS_LOG(WARN, "failed to shrink table, err=%d", ret);
  }
  return ret;
}

int32_t ObRootTable2::find_suitable_pos2(const const_iterator& it, const int32_t server_index, const int64_t tablet_version, int32_t &cluster_id, ObTabletReportInfo *to_delete/*=NULL*/)
{
  int32_t found_it_index = OB_INVALID_INDEX;
  const ObChunkServerManager *server_manager = NULL;
  if (!check_root_server()
      || NULL == (server_manager = &root_server_->get_server_manager()))
  {
    YYSYS_LOG(ERROR, "inner stat error");
    found_it_index = OB_NOT_INIT;
  }
  else
  {
    int32_t replica_num[OB_CLUSTER_ARRAY_LEN];
    for (int32_t i = 0; i < OB_MAX_COPY_COUNT; i++)
    {
      if (it->server_info_indexes_[i] == server_index)
      {
    ...
    if (OB_INVALID_INDEX == found_it_index)
    {
      cluster_id = server_manager->get_server_status(server_index)->server_.cluster_id_;
      int ret = OB_SUCCESS;
      int32_t valid_replicas_num[OB_CLUSTER_ARRAY_LEN] = {0};
      if (root_server_->get_total_cluster_tablet_replicas_num() == 0)
      {
        for (int32_t i = 0; i < OB_CLUSTER_ARRAY_LEN; i++)
        {
          replica_num[i] = 1;
        }
      }
      else if (OB_SUCCESS != (ret = root_server_->get_cluster_tablet_replicas_num(replica_num)))
      {
        YYSYS_LOG(WARN, "fail to get cluster tablet replias num, ret=%d",ret);
      }

      for (int32_t i = 0; i < OB_MAX_COPY_COUNT; i++)
      {
        int32_t tmp_cluster_id = server_manager->get_server_status(it->server_info_indexes_[i])->server_.cluster_id_;
        if (OB_INVALID_INDEX != it->server_info_indexes_[i])
        {
          valid_replicas_num[tmp_cluster_id] ++;
        }
      }

      if (valid_replicas_num[cluster_id] < replica_num[cluster_id])
      {
        for (int32_t i = 0; i < OB_MAX_COPY_COUNT; i++)
        {
          int32_t tmp_cluster_id = server_manager->get_server_status(it->server_info_indexes_[i])->server_.cluster_id_;
          if (valid_replicas_num[tmp_cluster_id] > replica_num[tmp_cluster_id])
          {
      ...
      {
        if (it->tablet_version_[i] < min_tablet_version
          && server_manager->get_server_status(it->server_info_indexes_[i])->server_.cluster_id_ == cluster_id)
        {
          min_tablet_version = it->tablet_version_[i];
          found_it_index = i;
        }
