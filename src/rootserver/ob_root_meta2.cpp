...
namespace oceanbase
{
  ...
  namespace rootserver
  {
    ...
    void ObRootMeta2::dump() const
    {
      ...
      //add e
      {
        YYSYS_LOG(INFO, "server_info_index = %d, tablet_version = %ld",
            server_info_indexes_[i], tablet_version_[i]);
      }
      YYSYS_LOG(INFO, "last_dead_server_time = %ld last_migrate_time = %ld",
    ...
    void ObRootMeta2::dump(const int server_index, int64_t &tablet_num) const
    {
      ...
      //add e
      {
        ...
        {
          tablet_num ++;
          YYSYS_LOG(INFO, "tablet_number = %ld, server_info_index = %d, tablet_version = %ld",
    ...
    {
      last_migrate_time_ = yysys::CTimeUtil::getTime();
    }
    ...
    ObRootMeta2CompareHelper::ObRootMeta2CompareHelper(ObTabletInfoManager* otim):tablet_info_manager_(otim)
    {
      if (NULL == otim)
      {
        YYSYS_LOG(ERROR, "tablet_info_manager_ is NULL");
      }
    }
    ...
    int ObRootMeta2CompareHelper::compare(const int32_t r1, const int32_t r2) const
    {
      ...
      if (tablet_info_manager_ != NULL)
      {
        ...
        if (p_r1 != p_r2)
        {
          if (p_r1 == NULL)
          {
            YYSYS_LOG(WARN, "no tablet info, idx=%d", r1);
            ret = -1;
          }
          else if (p_r2 == NULL)
          {
            YYSYS_LOG(WARN, "no tablet info, idx=%d", r2);
            ret = 1;
          }
          else
          {
      ...
      {
        YYSYS_LOG(WARN, "NULL info manager");
      }
      return ret;
    }
    ...
    bool ObRootMeta2RangeLessThan::operator() (const ObRootMeta2& r1, const ObNewRange& r2) const
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "tablet_info not exist");
        }
        else
        {
