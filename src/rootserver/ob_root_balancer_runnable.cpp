...
void ObRootBalancerRunnable::run(yysys::CThread *thread, void *arg)
{
  UNUSED(thread);
  UNUSED(arg);
  YYSYS_LOG(INFO, "[NOTICE] balance worker thread start, waiting [%s]",
  ...
  SET_THD_NAME_ONCE("rs-balanc");

  YYSYS_LOG(INFO, "[NOTICE] balance working");
  while (!_stop)
  {
    if (is_master() || role_mgr_.get_role() == ObRoleMgr::STANDALONE)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to load cluster replicas num after switch to master.");
      }
      else
      {
        YYSYS_LOG(INFO, "load cluster replicas num after switch to master succ");
      }
      //add:e
      if (balancer_.get_root_server()->get_total_cluster_tablet_replicas_num() <=0)
      {
        YYSYS_LOG(ERROR, "find total cluster replica count is valid, don't do balance");
      }
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "is building index, do not balance");
        }
        //modify e
      }
    }
    ...
    {
      YYSYS_LOG(DEBUG, "not the master");
    }

    int64_t sleep_us = config_.balance_worker_idle_time;
    YYSYS_LOG(TRACE, "balance worker idle, sleep [%s]", config_.balance_worker_idle_time.str());
    int sleep_ms = static_cast<int32_t>(sleep_us/1000);
    balance_worker_sleep_cond_.wait(sleep_ms);
  } // end while
  YYSYS_LOG(INFO, "[NOTICE] balance worker thread exit");
}
