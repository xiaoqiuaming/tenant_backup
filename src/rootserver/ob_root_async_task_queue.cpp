...
void ObRootAsyncTaskQueue::ObSeqTask::print_info(void) const
{
  ...
  //  YYSYS_LOG(INFO, "seq task info:timestamp[%ld], timeout[%ld], remain_times[%ld], task_type[%d], server[%s], cluster_role[%d] status[%d]",
  //            timestamp_, max_timeout_, remain_times_, type_, server_.to_cstring(), cluster_role_, server_status_);
  YYSYS_LOG(INFO, "seq task info:timestamp[%ld], timeout[%ld], remain_times[%ld], task_type[%d], server[%s], cluster_role[%d], \
                   status[%d], cluster_id[%ld], paxos_id[%ld]",
                  timestamp_, max_timeout_, remain_times_, type_, server_.to_cstring(), cluster_role_, server_status_, cluster_id_, paxos_id_);
  //mod 20150423:e
}
...
{
  const_cast<ObSeqTask &>(task).timestamp_ = yysys::CTimeUtil::getTime();
...
void ObRootAsyncTaskQueue::print_info(void) const
{
  if (size() > 0)
  {
    ObSeqTask task;
    int ret = head(task);
    YYSYS_LOG(INFO, "print queue info:id[%lu], size[%ld], first_timestamp[%ld], cur_timestamp[%ld]",
        id_allocator_, size(), task.timestamp_, yysys::CTimeUtil::getTime());
    if (OB_SUCCESS == ret)
    {
      task.print_info();
    }
