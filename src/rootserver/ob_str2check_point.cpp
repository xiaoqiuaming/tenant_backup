...
int main(int argc, char* argv[])
{
  if (argc != 3)
  {
    YYSYS_LOG(INFO, "usage %s str_file_name checkpoint_file_name\n", argv[0]);
    return 0;
  }
  ob_init_memory_pool();
  ...
  {
    YYSYS_LOG(INFO, "new object error\n");
    return 0;
  }
  FILE *stream = fopen(argv[1], "r");
  if (stream == NULL)
  {
    YYSYS_LOG(INFO, "open str file error %s\n", argv[1]);
    return 0;
  }
  ...
  {
    YYSYS_LOG(INFO, "write file error %s\n", argv[2]);
    return 0;
  }
  return 0;
