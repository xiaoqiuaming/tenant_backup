...
int ObRootServerState::init(ObRootServer2 *root_server, yysys::CRWLock *server_manager_lock, const ObChunkServerManager* server_manager)
{
  int ret = OB_SUCCESS;
      ...
      || NULL == root_server)
  {
    ...
    YYSYS_LOG(WARN, "invalid argument. lock=%p, server_manager=%p, root_server=%p",
...
int ObRootServerState::set_daily_merge_error(const char* err_msg, const int64_t length)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument. err_msg=%p, len=%ld", err_msg, length);
    ret = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "already have daily_merge error.");
    }
    else
    {
