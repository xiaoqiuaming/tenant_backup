        ...
        for (int i = 0; i < MAX_KEEP_HIS_COUNT; i++)
        {
          if (version_[i] == version)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "crc check error version %ld crc sum %lu, %lu", version_[i], crc_sum_[i], crc_sum);
                ret = OB_ERROR;
              }
              ...
              {
                YYSYS_LOG(WARN, "row_checksum check error version %ld row_checksum %lu, %lu", version_[i], row_checksum_[i], row_checksum);
              }
            }
          }
        }
...
////////////////////////////////////////////////////////////////
    ...
    int ObTabletInfoManager::add_tablet_info(const common::ObTabletInfo& tablet_info, int32_t& out_index)
    {
      ...
      {
        YYSYS_LOG(ERROR, "too many tablet max is %ld now is %ld", MAX_TABLET_COUNT, tablet_infos_.get_array_index());
        ret = OB_ERROR;
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "copy tablet error");
        }
    ...
    void ObTabletInfoManager::dump_as_hex(FILE* stream) const
    {
      ...
      for (int64_t i = 0; i < tablet_infos_.get_array_index(); i++)
      {
        ...
        {
          data_holder_[i].range_.end_key_.to_string(end_key_buff, OB_MAX_ROW_KEY_LENGTH * 2);
        }
    ...
    void ObTabletInfoManager::read_from_hex(FILE* stream)
    {
      ...
      for (int64_t i = 0; i < size; i++)
      {
        ...
        if (out_index != index)
        {
          YYSYS_LOG(ERROR, "index %ld != out_index %d", index, out_index);
          break;
        }
      }
      return;
    ...
    DEFINE_DESERIALIZE(ObTabletInfoManager)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; OB_SUCCESS == ret && i < total_size; i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to deserialize tablet info, ret=%d, buf=%p, data_len=%ld, tmp_pos=%ld",
          ...
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to deserialize crc info, out_index=%d, ret=%d, buf=%p, data_len=%ld, "
