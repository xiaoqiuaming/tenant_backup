...
int ObRootMsProvider::get_ms(ObServer &server, const bool query_master_cluster)
{
  ...
  if (init_)
  {
    ...
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to get ms. ret=%d", ret);
      }
    }
    if (OB_SUCCESS == ret)
    {
      YYSYS_LOG(TRACE, "get ms addr, ms_addr=%s", to_cstring(server));
    }
  }
  else
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "ms provide need init first");
  }
  return ret;
}
...
int ObRootMsProvider::get_ms(const ObServer & master_rs, ObServer & server)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "fail to get ms list, ret=%d", ret);
  }
  else
  {
    ...
    if (server_size > 0)
    {
      ...
      ObServer temp_ms;
      temp_ms = server;
      int32_t temp_index = last_index;
      for(int32_t i = 0; i < server_size;i++)
      {
        if (temp_ms.cluster_id_ == local_rs.cluster_id_)
        {
        ...
        {
          temp_index = (temp_index + 1) % server_size;
          temp_ms = ms_array.at(temp_index);
        }
      }

      YYSYS_LOG(TRACE, "get alive ms succ:index[%d], server[%s]", last_index, server.to_cstring());
    }
    else
    {
      ret = OB_MS_NOT_EXIST;
      YYSYS_LOG(ERROR, "fail to get master instance ms. ms_list.count=0");
    }
  }
  return ret;
}
...
int ObRootMsProvider::get_ms(ObServer & server)
{
  ...
  if (init_)
  {
    ...
    if (server_size > 0)
    {
      ...
      {
        YYSYS_LOG(WARN, "there is no master ups at this moment, ret[%d], then use master_cluster_id", ret);
      }
      else
      {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get next alive ms failed:index[%d], ret[%d]", last_index, ret);
      }
      else
      {
        YYSYS_LOG(TRACE, "get alive ms succ:index[%d], server[%s]", last_index, server.to_cstring());
    ...
    else
    {
     ...
     //ret = OB_MS_NOT_EXIST;
     //YYSYS_LOG(WARN, "no merge server find in server manager");
     ObServer master_master_rs;
	 config_->get_master_root_server(master_master_rs);
     ret = get_ms(master_master_rs, server);
     if (OB_SUCCESS != ret)
