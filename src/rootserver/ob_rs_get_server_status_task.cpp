   ...
   int ObRsGetServerStatusTask::init(ObRootWorker *worker, common::ObSchemaService *schema_service)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument. worker = %p, schema_service = %p", worker, schema_service);
      ...
      else
      {
        ...
        last_check_server_time_ = yysys::CTimeUtil::getTime();
        //add:e
      }
      return ret;
    }
    void ObRsGetServerStatusTask::runTimerTask(void)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, worker = %p", worker_);
      }
    }
    ...
    {
      int64_t ret = 0;
      yysys::CThreadGuard guard(&svr_status_mutex_);
    ...
    int ObRsGetServerStatusTask::get_server_status()
    {
      ...
      {
          YYSYS_LOG(WARN, "get merge server list failed, ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = get_cs_list()))
      {
          YYSYS_LOG(WARN, "get chunk server list failed, ret=%d", ret);
      }
      else
      {
          if (!array_compare())
          {
            ...
            //add:e
            YYSYS_LOG(INFO, "server status array changed");
            ret = fill_server_manager();
            if (OB_SUCCESS == ret)
            {
               YYSYS_LOG(INFO, "update server manager succ");
            }
            for (int32_t i = 0; i < server_status_array_.count(); i++)
            {
              server_status = server_status_array_.at(i);
              YYSYS_LOG(INFO,"server_type=%d, server_addr=%s, ms_sql_port=%d, svr_role =%d",server_status_array_.at(i).svr_type_,
    ...
    int ObRsGetServerStatusTask::check_server_alive(const int64_t &lease_duration_time)
    {
        ...
        else if (lease_duration_time < 0)
        {
            ...
            YYSYS_LOG(WARN, "cs_lease_duraion_time is invalid, value=%ld", lease_duration_time);
        }        
        else
        {
           ...
           for (int32_t i = 0; i < array_count; i++)
           {
              ...
              else if (it != worker_->get_root_server().get_server_manager().end())
              {
                  ...
                  if (OB_CHUNKSERVER == status.svr_type_)
                  {
                    ...
                    {
                       YYSYS_LOG(INFO, "chunkserver %s maybe down, remove it from inner table", status.addr_.to_cstring());
                  ...
                  else if (OB_MERGESERVER == status.svr_type_)
                  {
                      ...
                      {
                          YYSYS_LOG(INFO, "mergeserver %s maybe down, remove it from inner table", status.addr_.to_cstring());
                  ...
                  {
                    YYSYS_LOG(WARN, "unknow server type, server=%s, type=%d", status.addr_.to_cstring(), status.svr_type_);
                    ret = OB_ERROR;
                  }
              }
              else if (it == worker_->get_root_server().get_server_manager().end())
    ...
    int ObRsGetServerStatusTask::fill_server_manager()
    {
      ...
      {
        ServerStatus status;
        worker_->get_root_server().reset_server_manager_info();
        int res = OB_SUCCESS;
        for (unsigned i = 0; i < server_status_array_.count(); i++)
        {
          ...
          {
             //fill cs
            worker_->get_root_server().get_server_manager().receive_hb(res, status.addr_, 0, false, false, 0, true);
          }
          else if (status.svr_role_ == OB_MERGESERVER)
          {
            //fill ms
            worker_->get_root_server().get_server_manager().receive_hb(res, status.addr_, 0, true, false, status.ms_sql_port_, true);
          }
        }
      }
      return ret;
    ...
    int ObRsGetServerStatusTask::get_ms_list()
    {
      ...
      for(int32_t i = 0;i<ms_num && OB_SUCCESS == ret;i++)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to push back ms server status, err=%d", ret);
            break;
          }
        }
      }
    ...
    int ObRsGetServerStatusTask::get_cs_list()
    {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "decode cs num fail:ret[%d]", ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "cs server number[%d]", cs_num);
          }
        }
        ...
        for(int32_t i = 0;i<cs_num && OB_SUCCESS == ret;i++)
        {
          if (OB_SUCCESS != (ret = cs.deserialize(data_buff.get_data(),
                                                  ...
                                                  data_buff.get_position())))
          {
            YYSYS_LOG(WARN, "deserialize chunk server fail, ret: [%d]", ret);
          }
          else if (OB_SUCCESS !=
                   (ret = serialization::decode_vi64(data_buff.get_data(),
                                                     data_buff.get_capacity(),
                                                     ...
                                                     &reserved)))
          {
            YYSYS_LOG(WARN, "deserializ chunk server"
                      " reserver int64 fail, ret: [%d]", ret);
          }
          else
          {
        ...
        for (unsigned i = 0; i < new_cs.size(); i++)
        {
            ...
            if (OB_SUCCESS != (ret = tmp_server_status_array_.push_back(server_status)))
            {
              YYSYS_LOG(WARN, "failed to push back cs server status, err=%d", ret);
              break;
            }
        }
        return ret;
