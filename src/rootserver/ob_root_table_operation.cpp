  ...
  namespace rootserver
  {
    ...
    int ObRootTableOperation::generate_root_table()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "operation helper generate roottable, addr=%p", root_table_tmp_);
        }
      }
      return ret;
    }
        ...
        const int32_t server_index, const int64_t frozen_mem_version)
    {
      ...
      YYSYS_LOG(INFO, "cs (cs_index=%d) report new tablet.", server_index);
      if (NULL == schema_manager_)
      {
        YYSYS_LOG(WARN, "inner stat error. check it. schema_manager=%p", schema_manager_);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for(int64_t i = 0; i < index; ++i)
        {
          ...
          if (p_table_info != NULL)
          {
            ...
            {
              YYSYS_LOG(WARN, "cs reported illegal tablet, server=%d crc=%lu range=%s",
            ...
            {
              YYSYS_LOG(DEBUG, "report table is %lu, refuse to add", p_table_info->tablet_info_.range_.table_id_);
              continue;
            }
            YYSYS_LOG(DEBUG, "add a tablet, server=%d crc=%lu version=%ld seq=%ld range=%s",
                server_index, p_table_info->tablet_info_.crc_sum_,
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "fail to add tablet. range=%s", to_cstring(p_table_info->tablet_info_.range_));
              break;
            }
          }
        }
    ...
    int ObRootTableOperation::check_root_table(common::ObTabletReportInfoList &delete_list)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "inner error. new_root_table_ = null,  root_table_tmp = null");
      }
      delete_list.reset();
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "root table begin=%p, end=%p, size=%ld",
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to shrink. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "check root table failed we will wait for another %ld seconds", timeout);
          ret = OB_ERROR;
        }
      }
      if (OB_SUCCESS == ret)
      {
        if (NULL == schema_manager_)
        {
          YYSYS_LOG(WARN, "inner stat error. schema_manager=%p", schema_manager_);
          ret = OB_ERROR;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        for (const ObTableSchema* it = schema_manager_->table_begin(); it != schema_manager_->table_end(); ++it)
        {
          if (it->get_table_id() != OB_INVALID_ID
            && (it->get_index_helper().tbl_tid == OB_INVALID_ID
              || (it->get_index_helper().tbl_tid != OB_INVALID_ID && (it->get_index_helper().status == AVALIBALE || it->get_index_helper().status == NOT_AVALIBALE ))
                )
              )
          {
            if(!root_table_tmp_->table_is_exist(it->get_table_id()))
            {
              YYSYS_LOG(ERROR, "table_id = %lu has not been reported integrity", it->get_table_id() );
              ret = OB_ERROR;
              break;
            }
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "tablet copy count are not satify, never mind");
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        root_table_tmp_ = NULL;
        tablet_manager_tmp_ = NULL;
        YYSYS_LOG(INFO, "check root table success. new root table addr=%p", new_root_table_);
      }
      return ret;
    }
  }
