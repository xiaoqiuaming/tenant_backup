...
ObDataSourceProxyList::~ObDataSourceProxyList()
{
  for (ObVector<ObDataSourceProxy*>::const_iterator it = proxys_.begin(); it != proxys_.end(); ++it)
  {
    ...
    if (NULL == cur)
    {
      YYSYS_LOG(ERROR,"cur must not null");
    }
    else
    {
...
int ObDataSourceProxyList::add_proxy(ObServer& server, ObString& support_types)
{
  ...
  if (NULL == buf)
  {
    ...
    YYSYS_LOG(ERROR, "failed to alloc new proxy info");
  }
  else
  {
    ...
    if (n<0 || n >=MAX_DATA_SOURCE_SUPPORT_NAME_STRING_LENGTH)
    {
      ...
      YYSYS_LOG(WARN, "failed to copy support_types[%.*s], its length[%u] must less than buf[%ld]",
    ...
    else
    {
      ...
      while (NULL != (token = strsep(&remain, "|")))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to copy string %s, ret=%d", token, ret);
          break;
        }
        else if (OB_SUCCESS != (ret = proxy->support_types_.push_back(type)))
        {
          YYSYS_LOG(WARN, "failed to add type to proxy %s, ret=%d", to_cstring(server), ret);
          break;
        }
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to add proxy %s", to_cstring(*proxy));
      }
    }
  }
...
{
  yysys::CRLockGuard guard(data_source_lock_);
  int64_t i = 0;
  if (NULL != proxy_list_)
  {
    YYSYS_LOG(INFO, "print proxy list");
        ...
        it != proxy_list.end(); ++it)
    {
      ...
      YYSYS_LOG(INFO, "proxy %ld: %s", ++i, to_cstring(*proxy));
    }
  }
  else
  {
    YYSYS_LOG(INFO, "empty proxy list");
  }
}
...
int ObDataSourceMgr::update_data_source_proxy()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "server_manager_=%p, rpc_stub_=%p and config_=%p must not null",
        server_manager_, rpc_stub_, config_);
  }
  else if (yysys::CTimeUtil::getTime() > UPDATE_DATA_SOURCE_PERIOD  + last_update_time_)
  {
    ObServer ms_server;
    ...
    {
      YYSYS_LOG(ERROR, "no serving ms found, failed to do update_data_source_proxy");
      ret = OB_MS_NOT_EXIST;
    }
    else
    {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(WARN, "failed to malloc ObDataSourceProxyList");
      }
      else
      {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to fetch_proxy_list, table_name=%.*s cluster_id=%ld, ret=%d",
            table_name.length(), table_name.ptr(), cluster_id, ret);
      }
      else if (proxy_list->proxys_.size() == 0)
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "proxy_list size must larger than 0");
      }
    }

    if (OB_SUCCESS == ret)
    {
      yysys::CWLockGuard guard(data_source_lock_);
...
void ObDataSourceMgr::inc_failed_count(const ObServer& server)
{
  ...
  if (NULL == proxy_list_)
  {
    YYSYS_LOG(ERROR, "can't get proxy_list_ yet");
  }
  else if (NULL == config_)
  {
    YYSYS_LOG(ERROR, "config must not null");
  }
  else
  {
    bool found = false;
        ...
        it != proxy_list.end() && !found; ++it)
    {
      ...
      else if (cur->server_ == server)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "data source server %s is failed more than data_source_max_fail_count %ld",
    ...
    if (!found)
    {
      YYSYS_LOG(WARN, "data source server %s is not in proxy list", to_cstring(server));
    }
  }
}
    ...
    ObServer& dest_server, ObServer& data_source_server)
{
  ...
  if (ObDataSourceDesc::DATA_SOURCE_PROXY != type)
  {
    ...
    YYSYS_LOG(ERROR, "unsupported data source type, %d", type);
  }
  else if (NULL == config_)
  {
    ret = OB_NOT_INIT;
  ...
  else
  {
    ...
    else
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "unknown uri %.*s", uri.length(), uri.ptr());
      }
      else
      {
  ...
  if (NULL == proxy)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "failed to select data source, ret=%d", ret);
    }
  }
    ...
    ObServer& data_source_server)
{
  ...
  else if (ObDataSourceDesc::DATA_SOURCE_PROXY == type)
  {
    ...
    else
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "no proxy in proxy list yet");
      }
      else
      {
        data_source_name.assign_ptr(uri.ptr(), static_cast<ObString::obstr_size_t>(ptr - uri.ptr()));
        ...
        while (seq >= 0 && OB_SUCCESS == ret)
        {
          if (it == proxy_list.end())
          {
            if (found == false)
            {
              ...
              {
                ret = OB_EAGAIN;
                YYSYS_LOG(WARN, "no availble proxy support type %.*s", data_source_name.length(), data_source_name.ptr());
              }
              else
              {
                ret = OB_INVALID_ARGUMENT;
                YYSYS_LOG(ERROR, "no proxy support type %.*s", data_source_name.length(), data_source_name.ptr());
              }
              print_data_source_info();
              break;
            }
...
/*mod liuqc [add multi_ups to trunk] 2016/9/27 16:40 :b*/
    ...
    ObServer& data_source_server)
{
  ...
  else if (ObDataSourceDesc::DATA_SOURCE_PROXY == type)
  {
    ...
    {// first time call this method
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to random select data source, ret=%d", ret);
      }
      else
      {
        found = true;
  ...
  {
    ret = OB_ERR_SYS;
    YYSYS_LOG(ERROR, "unkown ObDataSourceType %d", type);
  }
...
int ObDataSourceMgr::get_proxy_it(const ObServer& server, ObVector<ObDataSourceProxy*>::const_iterator& it)
{
  ...
  {
    ret = OB_ENTRY_EXIST;
    YYSYS_LOG(WARN, "server %s is not in proxy list", to_cstring(server));
  }
  return ret;
}
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (!found)
        {
          YYSYS_LOG(WARN, "no other proxy server found, use old previous %s", to_cstring(data_source_server));
        }
      }
    }
  }
