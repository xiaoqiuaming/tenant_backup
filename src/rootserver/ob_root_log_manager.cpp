...
namespace oceanbase
{
  namespace rootserver
  {
    ...
    int ObRootLogManager::init(ObRootServer2* root_server, common::ObSlaveMgr* slave_mgr, const common::ObServer* server)
    {
      ...
      if (is_initialized_)
      {
        YYSYS_LOG(ERROR, "rootserver's log manager has been initialized");
        ret = OB_INIT_TWICE;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (NULL == log_dir)
        {
          YYSYS_LOG(ERROR, "commit log directory is null");
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "Argument is invalid[log_dir_len=%d log_dir=%s]", log_dir_len, log_dir);
            ret = OB_INVALID_ARGUMENT;
          }
          else
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        // {
        //   YYSYS_LOG(ERROR, "ObLogDirScanner init error");
        // }
        ...
        else
        {
          if (!scanner.has_ckpt())
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(ERROR, "ObLogDirScanner get_max_log_id error[ret=%d]", ret);
              }
            }
            if (OB_SUCCESS == ret)
            {
              YYSYS_LOG(INFO, "check point does not exist, take replay_point[replay_point_=%lu][ckpt_id=%lu]", replay_point_, ckpt_id_);
            }
          }
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "get_min_log_id error[ret=%d]", ret);
              ret = OB_ERROR;
            }
            else
            {
              ...
              {
                YYSYS_LOG(ERROR, "missing log file[min_log_id=%lu check_point=%lu", min_log_id, ckpt_id_);
                ret = OB_ERROR;
              }
              else
              {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(ERROR, "get_max_log_id error[ret=%d]", ret);
                ret = OB_ERROR;
              }
            }
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "ObRootLogMgr[this=%p] init succ[replay_point_=%lu max_log_id_=%lu]", this, replay_point_, max_log_id_);
        }
      }
                                         ...
                                         const char* log_data, const int64_t len)
    {
      ...
      if (len > 0)
      {
        ...
        {
          YYSYS_LOG(WARN, "last_disk_elapse_[%ld] > disk_warn_threshold_us[%ld], cursor=[%s,%s], len=%ld",
        ...
        {
          YYSYS_LOG(WARN, "last_net_elapse_[%ld] > net_warn_threshold_us[%ld]", last_net_elapse_, net_warn_threshold_us_);
        }
      }
      return err;
    }
    ...
    int ObRootLogManager::add_slave(const common::ObServer& server, uint64_t &new_log_file_id)
    {
      ...
      if (slave_mgr == NULL)
      {
        YYSYS_LOG(ERROR, "slave manager is NULL");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "add slave manager failed [ret=%d]", ret);
        }
        else
        {
          yysys::CThreadGuard log_guard(get_log_sync_mutex());
          ret = switch_log_file(new_log_file_id);
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "switch log file error [ret=%d]", ret);
          }
        }
      }
    ...
    {
      YYSYS_LOG(INFO, "[NOTICE] after recovery checkpointing");
      root_server_->start_threads();
      YYSYS_LOG(INFO, "[NOTICE] background threads started and finished init");
      return OB_SUCCESS;
    }
    ...
    int ObRootLogManager::replay_log()
    {
      ...
      YYSYS_LOG(INFO, "begin replay log");
      ...
      if (ret == OB_SUCCESS && !is_log_dir_empty_)
      {
        if (ckpt_id_ > 0)
        {
          ...
          if (ret == OB_SUCCESS)
          {
            ...
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "recover from check point failed, err=%d", ret);
            }
          }
          else
          {
            YYSYS_LOG(ERROR, "load server status failed, so recover from check point failed");
          }
        }
        if (ret == OB_SUCCESS)
        {
        ...
        if (ret == OB_SUCCESS)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "ObLogReader init failed, ret=%d", ret);
          }
          else
          {
            ...
            while (ret == OB_SUCCESS)
            {
              if (OB_LOG_NOP != cmd)
              {
                ...
                if (ret != OB_SUCCESS)
                {
                  YYSYS_LOG(ERROR, "apply log failed:command[%d], ret[%d]", cmd, ret);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != (ret = log_reader.get_next_cursor(start_cursor))
            && OB_READ_NOTHING != ret)
        {
          YYSYS_LOG(ERROR, "get_cursor(%s)=>%d", to_cstring(start_cursor), ret);
        }
        else if (OB_SUCCESS == ret)
        {}
        else
        ...
        {
          YYSYS_LOG(ERROR, "ObLogWriter start_log[%s]", to_cstring(start_cursor));
        }
        else
        {
          YYSYS_LOG(INFO, "start log [%s] done", to_cstring(start_cursor));
        }
      }
      return ret;
    }
    ...
    int ObRootLogManager::recover_checkpoint(const uint64_t checkpoint_id)
    {
      ...
      if (ckpt_id_ > 0)
      {
        ...
        if (ret == OB_SUCCESS)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "recover from check point [%lu] failed, err=%d", ckpt_id_, ret);
          }
          else
          {
            YYSYS_LOG(INFO, "recover from check point [%lu] done", ckpt_id_);
          }
        }
        else
        {
    ...
    int ObRootLogManager::do_check_point(const uint64_t checkpoint_id/* = 0 */)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(ERROR, "generate check point file name failed, error: %s", strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
          ...
          if (err < 0)
          {
            YYSYS_LOG(ERROR, "open check point file[%s] failed: %s", filename, strerror(errno));
            ret = OB_ERROR;
          }
          else
          {
            ...
            if (st_str_len < 0)
            {
              YYSYS_LOG(ERROR, "snprintf st_str error[%s][server_status=%d]", strerror(errno), server_status);
              ret = OB_ERROR;
            }
            else if (st_str_len >= UINT64_MAX_LEN)
            {
              YYSYS_LOG(ERROR, "st_str is too long[st_str_len=%d UINT64_MAX_LEN=%d", st_str_len, UINT64_MAX_LEN);
              ret = OB_ERROR;
            }
            else
            {
              ...
              if (err < 0)
              {
                YYSYS_LOG(ERROR, "write error[%s]", strerror(errno));
                ret = OB_ERROR;
              }
            }
      ...
      if (ret == OB_SUCCESS)
      {
        YYSYS_LOG(INFO, "do check point success, check point id: %lu => %lu", ckpt_id_, ckpt_id);
        ckpt_id_ = ckpt_id;
    ...
    int ObRootLogManager::load_server_status()
    {
      ...
      if (err < 0 || err >= OB_MAX_FILE_NAME_LENGTH)
      {
        YYSYS_LOG(ERROR, "generate check point file name [%s] failed, error: %s", ckpt_fn, strerror(errno));
        ret = OB_ERROR;
      }

      if (ret == OB_SUCCESS && !FileDirectoryUtils::exists(ckpt_fn))
      {
        YYSYS_LOG(INFO, "check point file[\"%s\"] does not exist", ckpt_fn);
      ...
      else
      {
        ...
        if (err < 0)
        {
          YYSYS_LOG(ERROR, "open file[\"%s\"] error[%s]", ckpt_fn, strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
          st_str_len = static_cast<int32_t>(ckpt_file.read(st_str, UINT64_MAX_LEN));
          YYSYS_LOG(DEBUG, "st_str_len = %d", st_str_len);
          st_str[st_str_len] = '\0';
          if (st_str_len < 0)
          {
            YYSYS_LOG(ERROR, "read file error[%s]", strerror(errno));
            ret = OB_ERROR;
          }
          else if ((st_str_len >= UINT64_MAX_LEN) || (st_str_len == 0))
          {
            YYSYS_LOG(ERROR, "data contained in ckeck point file is invalid[ckpt_str_len=%d]", st_str_len);
            ret = OB_ERROR;
          }
          else
          {
            ...
            if (rc != 1)
            {
              YYSYS_LOG(ERROR, "server status load failed.");
              ret = OB_ERROR;
            }
          }
        }
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "load server status succ[server_status=%d] from file[\"%s\"]", rt_server_status_, ckpt_fn);
      }
