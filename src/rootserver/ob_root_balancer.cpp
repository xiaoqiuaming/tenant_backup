...
int ObLoadDataInfo::clone(ObLoadDataInfo& info)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to write table name, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = buffer_.write_string(info.uri_, &uri_)))
  {
    YYSYS_LOG(WARN, "failed to write uri_, ret=%d", ret);
  }
  else
  {
    ...
    const int64_t tablet_version, const int64_t start_time, const ObLoadDataStatus status)
{
  ...
  {
    ObString tmp;
           //mod by maosy [multi ups balance] 20150518 b:
    ...
    if (uri.length() > oceanbase_uri_prefix_len &&
    ...
    else
    {
      ...
      YYSYS_LOG(ERROR, "wrong uri format[%.*s] for load table %.*s %lu",
  ...
  if (tablet_version < 0)
  {
    YYSYS_LOG(ERROR, "load data tablet version must %ld >= 0", tablet_version);
   ...
   balancer_thread_(NULL)
{
  ...
  //add:e
  memset(balance_per_cluster_dest_start_pos_, 0, OB_MAX_CLUSTER_COUNT * sizeof(int32_t));
...
void ObRootBalancer::do_balance_or_load()
{
  ...
  // check timeout migrate task
  {
          ...
          config_->load_data_max_timeout_per_range, src_server, dest_server, type))
    {
      ...
      if (NULL != dest_cs)
      {
        ...
        {
          YYSYS_LOG(ERROR, "dest cs %s cur_in_=%d must not less than 0", to_cstring(dest_server), dest_cs->balance_info_.cur_in_);
        }
      }

      if (ObDataSourceDesc::OCEANBASE_INTERNAL == type
          || ObDataSourceDesc::OCEANBASE_OUT == type)
      {
        if (src_server.is_same_ip(dest_server))
        {
          YYSYS_LOG(ERROR, "OCEANBASE_INTERNAL must not happen in same cs src:%s dest=%s",
        ...
        if (NULL != src_cs)
        {
          ...
          {
            YYSYS_LOG(ERROR, "src cs %s cur out =%d must not less than 0", to_cstring(src_server), src_cs->balance_info_.cur_out_);
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "src_cs %s not found in server_manager", to_cstring(src_server));
        }
      }
      src_server.reset();
      dest_server.reset();
  ...
  {
    YYSYS_LOG(ERROR, "daily merge process is error. blance or loading_data are delayed. error msg is %s",
  ...
  {
    do_load_data();
      //YYSYS_LOG(ERROR,"no loading data");
  ...
  YYSYS_LOG(DEBUG,"balance_next_table_seq_ = %d", balance_next_table_seq_);
  //add:e
}
...
uint64_t ObRootBalancer::nb_get_next_table_id(int32_t table_count, int32_t seq/*=-1*/)
{
  ...
  int err = OB_SUCCESS;
  YYSYS_LOG(DEBUG, "get next table id. table_count=%d, seq=%d", table_count, seq);
  if (0 >= table_count)
  {
    // no table
  }
  else
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "fail to get schema manager. ret=%d", err);
      }
    }
    if (OB_SUCCESS == err)
    {
      ...
      for (it = schema_manager->table_begin(); schema_manager->table_end() != it; ++it)
      {
        if (seq % table_count == idx)
        {
          ...
          if (config_->enable_rereplication_in_table_level)
          {
            ...
            //mod:e
            {
              YYSYS_LOG(DEBUG, "get table id. table_id=%ld, replica_num=%d", ret, total_replica_num);
            }
            else
            {
              //del lbzhong [Paxos Cluster.Balance] 20160707:b
              //YYSYS_LOG(ERROR, "get table id. table_id=%ld, replica_num=%d less than min_replica_num=%d", ret, balance_replica_num_ ,min_replica_num);
              //del:e
            }
            //add e
          }
                                    ...
                                    )
{
  ...
  if (0 < cs_num)
  {
    ...
    it += balance_per_cluster_dest_start_pos_[cluster_id] % static_cast<int32_t>(server_manager_->size());
    ...
    while(true)
    {
          ...
          )
      {
        ...
        if (!meta->did_cs_have(cs_idx))
        {
          dest_it = it;
          dest_cs_idx = cs_idx;
          YYSYS_LOG(DEBUG, "find dest cs, start_pos=%ld cs_idx=%d",
                    it_start_pos-server_manager_->begin(), dest_cs_idx);
          //balance_select_dest_start_pos_ = dest_cs_idx + 1;
          balance_per_cluster_dest_start_pos_[cluster_id] = dest_cs_idx + 1;
          if (balance_per_cluster_dest_start_pos_[cluster_id] >= (static_cast<int32_t>(server_manager_->size())))
          {
            balance_per_cluster_dest_start_pos_[cluster_id] = 0;
          }
          ret = OB_SUCCESS;
          break;
        }
      ...
      if (it == it_start_pos)
      {
        balance_per_cluster_dest_start_pos_[cluster_id] = static_cast<int32_t>(it - server_manager_->begin());
        break;
      }
    } // end while
  }
                                           ...
                                           )
{
      ...
      0 == valid_replicas_num)
  {
    ...
    if (NULL != tablet)
    {
      ...
      //YYSYS_LOG(ERROR, "no valid replica found for %s", to_cstring(tablet->range_));
      YYSYS_LOG(DEBUG, "no valid replica found for %s in cluster[%d], need copy from external cluster",
                to_cstring(tablet->range_), cluster_id);
      //mod:e
    }
    else
    {
      YYSYS_LOG(ERROR, "no valid replica found for %p", it);
    }
  }
  //add zhaoqiong[roottable tablet management]20150302:b
  ...
  if (valid_replicas_num < tablet_replicas_num)
  {
    ...
    if (NULL == tablet)
    {
      YYSYS_LOG(ERROR, "no tablet info in roottable");
    }
    else
    {
      ...
      else if (-1 == did_need_copy)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "copy delayed, now=%ld lost_replica_time=%ld safe_log_time=%s",
...
int ObRootBalancer::nb_modify_index_stat(const uint64_t table_id)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to get schema manager. err=%d", err);
  }
  else
  {
    ...
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(WARN, "get table schema failed, table_id = [%lu]", table_id);
    }
    else if (OB_INVALID_ID != table_schema->get_index_helper().tbl_tid && NOT_AVALIBALE == table_schema->get_index_helper().status)
    {
      ...
      {
        YYSYS_LOG(WARN, "modify index stat [%d->%d] failed, table_id = [%lu]", NOT_AVALIBALE, AVALIBALE, table_id);
      }
      else
      {
        YYSYS_LOG(INFO, "modify index stat [%d->%d] success, table_id = [%lu]", NOT_AVALIBALE, AVALIBALE, table_id);
      }
    }
  }
  if (schema_mgr != NULL)
                                ...
                                )
{
      ...
      || OB_INVALID_INDEX == dest_cs_idx)
  {
        ...
        || OB_INVALID_INDEX == dest_cs_idx)
    {
      ...
      is_replicas_safe = true;
      //add:e
      YYSYS_LOG(DEBUG, "cannot find dest cs, cluster_id:%d",cluster_id);
    }
  }
  if (OB_INVALID_INDEX != dest_cs_idx)
  {
    ...
    {
      YYSYS_LOG(WARN, "cluster[%d] lost tablet=%s", cluster_id, to_cstring(tablet->range_));
    }
    ...
    { // fetch data from oceanbase cluster internal
      ...
      {
        YYSYS_LOG(DEBUG, "cannot find src cs");
      }
      else
      {
        //add lbzhong [Paxos Cluster.Balance] 20160705:b
    ...
    { // fetch data from data source
      ...
      { // this range has copy count=0, but not a data load task...
        YYSYS_LOG(ERROR, "no tablet exist in roottable for range %s, but it doesn't belong to a data load task. ret=%d",
            to_cstring(tablet->range_), ret);
      }
      else if (NULL == uri)
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "uri must not null");
      }
      else if (OB_SUCCESS != (ret = data_source_mgr_.select_data_source(data_source_type, *uri, dest_it->server_, src_server)))
      {
        if (OB_EAGAIN != ret)
        {
          YYSYS_LOG(ERROR, "no data source found for range=%s, ret=%d", to_cstring(tablet->range_), ret);
        }
      }
      else
      {
    ...
    if (OB_SUCCESS == ret && found_src)
    {
      ...
      {
        balance_per_cluster_dest_start_pos_[cluster_id] = dest_cs_idx;
        // is in migrating
        YYSYS_LOG(DEBUG, "OB_ROOT_MIGRATE_INFO_EXIST %s %s", to_cstring(dest_server), to_cstring(data_source_desc));
      }
      else if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to add migrate %s from %s to cs %s, ret=%d",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to send msg migrate %s from %s to cs %s, ret=%d",
            to_cstring(data_source_desc.range_), to_cstring(src_server), to_cstring(dest_server), ret);
        }
      }
    }
              ...
              || OB_INVALID_INDEX == dest_cs_idx)
          {
            if (src_cs->status_ == ObServerStatus::STATUS_SHUTDOWN)
            {
              if (OB_SUCCESS != nb_find_dest_cs(it, INT64_MAX, cs_num, dest_cs_idx, dest_it
                  ...
                  || OB_INVALID_INDEX == dest_cs_idx)
              {
                YYSYS_LOG(WARN, "cannot find dest cs");
              }
            }
          }
          if (OB_INVALID_INDEX != dest_cs_idx)
      ...
      && -1 != delete_idx)
  {
    ...
    else
    {
      ...
      if (role_mgr_->is_master())
      {
        ...
        {
          YYSYS_LOG(ERROR, "write log error");
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to delete tablet %s on cs %s, ret=%d",
                to_cstring(to_delete.tablet_info_.range_), to_cstring(cs), tmp_ret);
          }
        }
      }
      YYSYS_LOG(DEBUG, "delete replica, version=%ld cs_idx=%d",
                                     ...
                                     )
{
  ...
  {
    YYSYS_LOG(ERROR, "not find the table in root table:table_id[%lu], root_table[%p]", table_id, root_table_);
  }
}
...
void ObRootBalancer::check_table_rereplication(const uint64_t table_id,
    ...
    , const int32_t cluster_id
    //add:e
    , int64_t &tablet_count, bool &tablet_lost_flag
    ...
    if (NULL != tablet)
    {
      if (tablet->range_.table_id_ == table_id)
      {
        ...
        //mod:e
        {
          ...
          if (copy_from_external_cluster)
          {
            tablet_lost_flag = true;
          }
          if (RA_COPY == ract)
          {
            YYSYS_LOG(DEBUG, "RA_COPY: %s", to_cstring(tablet->range_));
            //mod liumz, [secondary index static_index_build] 20150526:b
...
bool ObRootBalancer::check_not_ini_table(const uint64_t table_id) const
{
  ...
  if (NULL == schema_manager)
  {
    YYSYS_LOG(ERROR, "schema_manager is NULL");
  }
  else
  {
...
int ObRootBalancer::do_rereplication_by_table(const uint64_t table_id, bool &scan_next_table)
{
  ...
  int64_t tablet_count[OB_CLUSTER_ARRAY_LEN] = {0};
  bool tablet_lost_flag = false;
  //del lbzhong [Paxos Cluster.Balance] 20160704:b
  ...
  for(int32_t cluster_id = 0; OB_SUCCESS == ret && cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
  {
    ...
    {
      int64_t tablet_count_tmp = 0;
      ret = do_rereplication_by_table(table_id, scan_next_table, need_replicate,
                                      found_table, total_count, safe_count,
                                      tablet_count_tmp, tablet_lost_flag
                                      ,cluster_id);
      tablet_count[cluster_id] = tablet_count_tmp;
    }
  }
  if (tablet_lost_flag)
  {
    for(int32_t cluster_id = 0; OB_SUCCESS == ret && cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
    {
      if(is_cluster_has_alive_cs(cluster_id))
      {
        YYSYS_LOG(ERROR, "cluster[%d] has table_id[%ld] tablet_count[%ld]", cluster_id, table_id, tablet_count[cluster_id]);
      }
    }
  }
  return ret;
    ...
    )
{
  ...
  {
    YYSYS_LOG(WARN, "calculate table size error, err=%d", ret);
  }
  else if (0 < cs_num)
  {
    if (!table_found)
    {
      ...
      {
        YYSYS_LOG(ERROR, "system table not find:table_id[%lu]", table_id);
      ...
      else
      {
        ...
        else
        {
          ...
          {
            invalid_tid = false;
            YYSYS_LOG(WARN, "find unavailiable index table not in root table, ignore it. role[%d], master[%d], table_id[%lu]",
        ...
        {
          YYSYS_LOG(ERROR, "find table not in root table:role[%d], master[%d], table_id[%lu]",
              root_server_->get_obi_role().get_role(), root_server_->is_master(), table_id);
        }
        if (schema_mgr != NULL)
        {
  ...
  {
    YYSYS_LOG(ERROR, "no cs found , no need to do replicate");
  }
  return ret;
}
...
void ObRootBalancer::check_shutdown_process()
{
  ...
  for (it = server_manager_->begin(); it != server_manager_->end(); ++it)
  {
    if (ObServerStatus::STATUS_SHUTDOWN == it->status_)
    {
      ...
      if (0 == count)
      {
        YYSYS_LOG(INFO, "shutdown chunkserver[%s] is finished", to_cstring(it->server_));
      }
    }
  }
}
                                        ...
                                        )
{
  ...
  {
    YYSYS_LOG(WARN, "can't find table_id %lu in root table, no balance need", table_id);
  }
  else if (0 < cs_num)
  {
    ...
    if (!is_curr_table_balanced)
    {
      ...
      //YYSYS_LOG(DEBUG, "balance table, table_id=%lu avg_count=%ld", table_id, avg_count);
      YYSYS_LOG(INFO, "balance table, table_id=%lu avg_count=%ld", table_id, avg_count);
      //mod:e
                                               ...
                                               )
{
  ...
  table_found = false;
  {
    ...
    YYSYS_LOG(DEBUG, "reset balance info. cs_count=%d, shutdown_count=%d", cs_count, shutdown_count);
  } // end lock
  {
    // calculate sum
    ObRootTable2::const_iterator it;
          ...
          //add e
          {
            if (OB_INVALID_INDEX != it->server_info_indexes_[i])
            {
              ...
              if (NULL != cs && ObServerStatus::STATUS_DEAD != cs->status_
                  //add lbzhong [Paxos Cluster.Balance] 20160705:b
                  && (cluster_id == OB_ALL_CLUSTER_FLAG || cs->server_.cluster_id_ == cluster_id || cluster_id == OB_EXTERNAL_CLUSTER_ID)
                  //add:e
                  )
              {
  ...
  if (0 < cs_count)
  {
    ...
    for (it = server_manager_->begin(); it != server_manager_->end(); ++it)
    {
      if (it->status_ != ObServerStatus::STATUS_DEAD
          //add lbzhong [Paxos Cluster.Balance] 20160705:b
          && (cluster_id == OB_ALL_CLUSTER_FLAG || it->server_.cluster_id_ == cluster_id || cluster_id == OB_EXTERNAL_CLUSTER_ID)
          //add:e
          )
      {
    ...
    if (0 < out_cs && out_cs < cs_count && migrate_out_per_cs > server_manager_->get_max_migrate_num() / out_cs)
    {
      ...
      { // avoid an old bug: when migrate_out_per_cs < 0, balance and migrate will not work
        YYSYS_LOG(WARN, "max_migrate_num(%ld)/out_cs_count(%d) is 0, use migrate_out_per_cs =1",
            server_manager_->get_max_migrate_num(), out_cs);
        migrate_out_per_cs = 1;
      }
    }
    YYSYS_LOG(DEBUG, "sstable distribution, table_id=%lu total_size=%ld total_count=%ld "
    ...
    ) const
{
  ...
  for (; it != server_manager_->end(); ++it)
  {
        ...
        )
    {
      ...
      YYSYS_LOG(DEBUG, "cs=%s sstables_count=%ld sstables_size=%ld cur_in_=%d cur_out_=%d",
...
int ObRootBalancer::send_msg_migrate(const ObServer &dest, const ObDataSourceDesc& data_source_desc)
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "migrate tablet: type=%s, tablet=%s tablet_version=%ld src=%s dest=%s keep_src=%c",
  ...
  {
    YYSYS_LOG(WARN, "failed to send migrate tablet: err=%d: type=%s, tablet=%s tablet_version=%ld src=%s dest=%s keep_src=%c",
...
{
  YYSYS_LOG(INFO, "print migrate infos:");
  if (NULL == server_manager_)
  {
    YYSYS_LOG(ERROR, "server_manager_ must no null");
  }
  else
  {
    int32_t total_in = 0;
    ...
    for (int64_t i = 0; i < MAX_LOAD_INFO_CONCURRENCY; ++i)
    {
      ...
      {
        YYSYS_LOG(INFO, "load table info[%ld]: table_name=%.*s table_id=%lu start_time=%s uri=%.*s status=%s",
    ...
    for (it = server_manager_->begin(); it != server_manager_->end(); ++it)
    {
      if (it->status_ != ObServerStatus::STATUS_DEAD)
      {
        ...
        YYSYS_LOG(INFO, "migrate info: cs=%s in=%d out=%d",
            to_cstring(it->server_), it->balance_info_.cur_in_,
            it->balance_info_.cur_out_);

      }
    }
    YYSYS_LOG(INFO, "migrate info: total_in=%d total_out=%d", total_in, total_out);
...
{
  YYSYS_LOG(INFO, "balance batch migrate infos, start_us=%ld timeout_us=%ld done=%d total=%d",
...
int ObRootBalancer::nb_trigger_next_migrate(const ObDataSourceDesc& desc, int32_t result)
{
...

  {
    ...
    if (OB_SUCCESS == free_migrate_info_ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "desc_cs %s not found in server_manager", to_cstring(dest_server));
      }

      if (ObDataSourceDesc::OCEANBASE_INTERNAL == desc.type_
          || ObDataSourceDesc::OCEANBASE_OUT == desc.type_)
      {
        ...
        if (NULL != src_cs)
        {
          ...
          if (src_cs->balance_info_.cur_out_ < 0)
          {
            YYSYS_LOG(ERROR, "src cs %s cur out =%d must not less than 0", to_cstring(desc.src_server_), src_cs->balance_info_.cur_out_);
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "src_cs %s not found in server_manager", to_cstring(desc.src_server_));
        }
      }
    }
    else
    {
      YYSYS_LOG(WARN, "not a record migrate task: result=%d, dest_cs=%s, desc=%s",
    ...
    if (NULL == dest_cs)
    {
      YYSYS_LOG(ERROR, "invalid arg, dest_cs=%p ip=%s, result = %d",
    ...
    {
      YYSYS_LOG(ERROR, "dest cs %s is offline", to_cstring(dest_server));
      ret = OB_ENTRY_NOT_EXIST;
    }
  }
  ...
  //mod by maosy [MultiUPS] [Balance_Modify] 20150518 b:
    ...
    // mod e
      if (OB_SUCCESS == result)
      {
        ...
        balance_last_migrate_succ_time_ = yysys::CTimeUtil::getTime();
...
void ObRootBalancer::do_balance()
{
  ...
  {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    YYSYS_LOG(WARN, "fail to new schema_manager. ret=%d", ret);
  }//add:e
  else if(OB_SUCCESS != (ret = root_server_->get_schema(false,false,*schemav2)))
  {
    YYSYS_LOG(WARN, "failed to get schema for migrate, ret=%d", ret);
  }
  //add e
  ...
  {
    YYSYS_LOG(WARN, "failed to restart servers, ret=%d", tmp_ret);
  }

  {
    //mod by maosy [MultiUPS] [Balance_Modify] 20150518 b:
    yysys::CWLockGuard guard(load_data_lock_);
    // mod e
    if (!is_loading_data_ && 0 == server_manager_->get_migrate_num() && 0 == balance_batch_migrate_count_)   // unncertainty 亻逻辑修改
//    if (0 == server_manager_->get_migrate_num() && 0 == balance_batch_migrate_count_)
    {
      balance_batch_migrate_done_num_ = 0;
      balance_last_migrate_succ_time_ = 0;
      balance_start_time_us_ = yysys::CTimeUtil::getTime();
      server_manager_->reset_balance_info();
      YYSYS_LOG(INFO, "rereplication begin");
    }
  }
  ...
  balance_batch_delete_count_ = 0;
  balance_batch_copy_count_ = 0;
  YYSYS_LOG(DEBUG, "table_count = %d", table_count);
  ...
  for (int32_t i = 0; i < table_count && scan_next_table && config_->enable_rereplication; ++i) // for each table
  {
    ...
    if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count)) && NULL != (index_schema = schemav2->get_table_schema(table_id)))
    {
      ...
      if(OB_INVALID_ID == index_schema->get_index_helper().tbl_tid)
      {
        ...
        YYSYS_LOG(DEBUG, "rereplication table, table_id=%lu table_count=%d copy_count=%d delete_count=%d",
  ...
  for (int32_t i = 0; i < table_count && scan_next_table && config_->enable_balance; ++i) // for each table
  {
    ...
    /*if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count)))
    {
      ...
      YYSYS_LOG(DEBUG, "balance table, table_id=%lu table_count=%d migrate_count=%d",
    ...
    if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count)) && NULL != (index_schema = schemav2->get_table_schema(table_id)))
    {
        ...
        //[view]
        if (index_schema->get_type() == ObTableSchema::VIEW)
        {
            continue;
        }
  ...
  {
    YYSYS_LOG(INFO, "batch migrate begin, count=%d(copy=%d) delete=%d",
  ...
  else
  {
   ...
   //if (!is_loading_data_ && 0 == server_manager_->get_migrate_num() && 0 == balance_batch_migrate_count_)
    ...
    {
      int64_t mnow = yysys::CTimeUtil::getTime();
      YYSYS_LOG(INFO, "balance batch migrate done, elapsed_us=%ld done=%d",
...
void ObRootBalancer::do_load_data()
{
  ...
  {
    ret = OB_NOT_MASTER;
    YYSYS_LOG(ERROR, "can not do load table on not master rs");
  }
  else if (receive_stop_)
  {
    ret = OB_IN_STOP_STATE;
    YYSYS_LOG(WARN, "rs is in receive_stop_, stop do load data");
  }
  else if (0 == get_loading_data_infos_count())
  {
    ...
    { // lock
      ...
      if (0 == get_loading_data_infos_count())
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to unlock frozen version");
        }
        root_server_->set_bypass_flag(false);
        need_make_checkpoint = true;
        is_loading_data_ = false;
        server_manager_->reset_balance_info();
        YYSYS_LOG(INFO, "all load table task is done, exit loading data status. current frozen verion=%ld",
  ...
  else
  {
    ...
    if (root_server_->is_master() && root_server_->get_obi_role().get_role() == ObiRole::MASTER)
    {
      ...
      stop_failed_load_table_task(is_finished); // check every 30s
    }
    ...
    while (scan_next_table && !receive_stop_)
    {
      { // find next load table id
        ...
        if (idx < 0)
        {
          YYSYS_LOG(DEBUG, "no loading table found");
          break;
        }
        // YYSYS_LOG(INFO, "do_rereplication_by_table table_name=%.*s table_id=%lu status=%s",
        //     info->table_name_.length(), info->table_name_.ptr(), info->table_id_, info->get_status());
        YYSYS_LOG(INFO, "do_rereplication_by_table table_name=%.*s table_id=%lu status=%s, fail_count=%ld",
            info->table_name_.length(), info->table_name_.ptr(), info->table_id_, info->get_status(), info->fail_count_);
      }
      ...
      for(int32_t cluster_id = 0; OB_SUCCESS == ret && cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
      {
        if(is_cluster_has_alive_cs(cluster_id))
        {
          int64_t tablet_count_temp = 0;
          ret = do_rereplication_by_table(table_id, scan_next_table, need_replicate,found_table,
                                          total_tablet_count, safe_tablet_count, tablet_count_temp,
                                          tablet_lost_flag, cluster_id);
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to do_rereplication_by_table table_id %lu", table_id);
      }*/
      //else if (!need_replicate && found_table)
      YYSYS_LOG(INFO, "need_replicate=%d, found_table=%d, is_finished=%d. It will %s",
                      need_replicate, found_table, is_finished,
                      (found_table && is_finished)? "handle load table done" : "continue");
      if (found_table && is_finished)
      {
        if(root_server_->get_obi_role().get_role() == ObiRole::MASTER)
        {
          if (OB_SUCCESS != (ret = handle_load_table_done(table_id)))
          {
            YYSYS_LOG(ERROR, "failed to finish loading table %lu", table_id);
          }
        }
        else
        {
          YYSYS_LOG(INFO, "all tablets is copied, table_id=%lu safe_tablet_count=%ld,total_tablet_count=%ld"
      ...
      {
        YYSYS_LOG(INFO, "do_rereplication_by_table table_id=%lu safe_tablet_count=%ld, "
            "total_tablet_count=%ld: cost time %ldus",
            table_id, safe_tablet_count, total_tablet_count, yysys::CTimeUtil::getTime() - start_time);
      }
    }
  }
}
  ...
  for (int32_t i = 0; i < table_count; ++i) // for each table
  {
    ...
    if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count, i)))
    {
      ...
      if(OB_SUCCESS == nb_calculate_sstable_count(table_id, avg_size, avg_count, cs_num, out_per_cs, shutdown_num))
      {
        ...
        if (!ret)
        {
          YYSYS_LOG(DEBUG, "table not balanced, id=%lu", table_id);
          break;
        }
      }
      else
...
bool ObRootBalancer::nb_is_all_tablets_replicated(int32_t expected_replicas_num)
{
  ...
  for (it = root_table_->begin(); it != root_table_->end(); ++it)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "tablet not replicated, num=%d expected=%d",
  ...
  ObArray<balancer::ObShutDownProgress> shutdown_servers;
  {
    ...
    for (it = server_manager_->begin(); it != server_manager_->end(); ++it)
    {
      if (ObServerStatus::STATUS_SHUTDOWN == it->status_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "array push error, err=%d", ret);
          break;
        }
      }
    }
...
int ObRootBalancer::add_load_table(const ObString& table_name, const uint64_t table_id, ObString& uri, const int64_t start_time)
{
  ...
  {
    ret = OB_NOT_MASTER;
    YYSYS_LOG(ERROR, "can not load table on not master rs");
  }
  else if (OB_MAX_TABLE_NAME_LENGTH < table_name.length())
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "table name max length is %lu, but input table_name=%.*s, length=%d",
        OB_MAX_TABLE_NAME_LENGTH, table_name.length(), table_name.ptr(), table_name.length());
  }
  else if (OB_MAX_URI_LENGTH < uri.length())
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "uri length=%d, but OB_MAX_URI_LENGTH=%ld", uri.length(), OB_MAX_URI_LENGTH);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (!is_loading_data_)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to init load data env, ret=%d", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to add_load_table_task_info, table_name=%.*s table_id=%lu uri=%.*s, ret=%d",
...
int ObRootBalancer::handle_load_table_done(const uint64_t table_id)
{
  ...
  {
    ret = OB_NOT_MASTER;
    YYSYS_LOG(ERROR, "this rs is the master rs of the master cluster, should not do handle_load_table_done");
  }
  else
  { // lock load_data_lock_ and get table_info
    ...
    if (NULL == info_ptr)
    {
      ...
      YYSYS_LOG(ERROR, "table %lu is not loading", table_id);
    }
    else if (info_ptr->last_check_status_time_ + interval > cur_time)
    {
      need_check = false;
      YYSYS_LOG(INFO, "no need to check status now, table_id=%lu last_check_status_time_=%ld"
    ...
    {
      YYSYS_LOG(WARN, "failed to dup import table info with table name=%.*s, table_id=%lu, ret=%d",
  ...
  if (OB_SUCCESS == ret && need_check)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "failed to check_import_status_of_all_clusters, with table_name=%.*s table_id=%lu, ret=%d",
          info.table_name_.length(), info.table_name_.ptr(), info.table_id_, ret);
    }
    else if (is_finished)
    {
      YYSYS_LOG(INFO, "[import] all clusters have copied table: table_name=%s table_id=%lu",
          to_cstring(info.table_name_), info.table_id_);
    }
    else if (is_failed)
    {
      ret = OB_DATA_LOAD_TABLE_STATUS_ERROR;
      YYSYS_LOG(ERROR, "status of loading table on cluster is failed, need to stop it, info_version=%ld, system_version=%ld",
                info.tablet_version_, root_server_->get_to_be_frozen_version());
      int tmp_ret = start_set_import_status(info.table_name_, info.table_id_, ObLoadDataInfo::FAILED);
      if (OB_SUCCESS != tmp_ret)
      {
        YYSYS_LOG(WARN, "failed to start_set_import_status, ret=%d", tmp_ret);
      }
    }
  }

  if (info.tablet_version_ == root_server_->get_to_be_frozen_version()
    ||  -1  == root_server_->get_to_be_frozen_version())
  {
  }
  else
  {
    ret = OB_DATA_SOURCE_TABLET_VERSION_ERROR;
    YYSYS_LOG(ERROR, "status of loading table on cluster is failed, need to stop it.");
    int tmp_ret = start_set_import_status(info.table_name_, info.table_id_, ObLoadDataInfo::FAILED);
    if (OB_SUCCESS != tmp_ret)
    {
      YYSYS_LOG(WARN, "failed to start_set_import_status, ret=%d", tmp_ret);
  ...
  if (OB_SUCCESS == ret && is_finished)
  {
    ...
    {
      YYSYS_LOG(ERROR, "[import] failed to do load table done, table_name=%.*s table_id=%lu, ret=%d",
          info.table_name_.length(), info.table_name_.ptr(), table_id, ret);
    }
    else
    {
      YYSYS_LOG(INFO, "[import] load table success, table_name=%.*s table_id=%lu",
  ...
  if (OB_SUCCESS == ret && is_finished)
  {
    ...
    if (OB_SUCCESS != tmp_ret)
    {
      YYSYS_LOG(ERROR, "[import] failed to notify_switch_schema, table_name=%.*s table_id=%lu old_table_id=%lu ret=%d",
          info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_, tmp_ret);
    }
    else
    {
      YYSYS_LOG(INFO, "[import] succeed to notify_switch_schema, table_name=%.*s table_id=%lu old_table_id=%lu",
  ...
  if (OB_SUCCESS == ret && is_finished)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "[import] failed to set import status, table_name=%.*s table_id=%lu old_table_id=%lu, ret=%d",
          info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_, ret);
    }
    else
    {
      YYSYS_LOG(INFO, "[import] succeed to set import status, table_name=%.*s table_id=%lu old_table_id=%lu",
          info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_);
    }
  }
  ...
  if (OB_SUCCESS != ret)
  {
    ...
    info.end_time_ = yysys::CTimeUtil::getTime();
    YYSYS_LOG(WARN, "[import] failed to handle_load_table_done, table_name=%.*s table_id=%lu old_table_id=%lu, ret=%d",
        info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_, ret);
    if (root_server_->is_master() && root_server_->get_obi_role().get_role() == ObiRole::MASTER)
    {
  ...
  {
    YYSYS_LOG(INFO, "[import] succeed to handle_load_table_done, table_name=%.*s table_id=%lu old_table_id=%lu",
        info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_);
    info.end_time_ = yysys::CTimeUtil::getTime();
    if (root_server_->is_master() && root_server_->get_obi_role().get_role() == ObiRole::MASTER)
    {
  ...
  {
    YYSYS_LOG(DEBUG, "[import] not all clusters copied, table_name=%.*s table_id=%lu old_table_id=%lu",
        info.table_name_.length(), info.table_name_.ptr(), info.table_id_, info.old_table_id_);
  }
...
void ObRootBalancer::handle_load_table_failed(const uint64_t table_id)
{
  ...
  {
    //ret = OB_NOT_DATA_LOAD_TABLE;
    YYSYS_LOG(WARN, "table %lu is not loading, nothing to clean", table_id);
  }
  else
  {
...
{ //TODO: use config table name
  ...
  if (!info.is_working())
  {
    ...
    n = snprintf(sql_buf, sizeof(sql_buf),
      "replace into bypass_import.load_data_history(start_time, end_time, table_name, table_id, status, uri)"
      " values(%ld, %ld, '%.*s', %lu, '%s', '%.*s')",
      info.start_time_, info.end_time_, info.table_name_.length(), info.table_name_.ptr(),
      info.table_id_, info.get_status(), info.uri_.length(), info.uri_.ptr());
  }
  ...
  if (n<0 || n >= OB_MAX_SQL_LENGTH)
  {
    ...
    YYSYS_LOG(ERROR, "can't generate sql: table_name=%.*s, table_id=%ld, uri=%.*s, status=%s",
        info.table_name_.length(), info.table_name_.ptr(), info.table_id_,
        info.uri_.length(), info.uri_.ptr(), info.get_status());
  }
  else
  {
    ...
    YYSYS_LOG(INFO, "[import] update load table history:%.*s", sql.length(), sql.ptr());
  }

  //ObChunkServerManager::const_iterator it = server_manager_->get_serving_ms();
  ObChunkServerManager::const_iterator it = server_manager_->get_serving_ms(config_->cluster_id);
  if (server_manager_-> end() == it)
  {
    YYSYS_LOG(ERROR, "no serving ms found, failed to do sql: %.*s", sql.length(), sql.ptr());
    ret = OB_MS_NOT_EXIST;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "update load table result failed: sql=%.*s, ret=%d",
    ...
    const uint64_t old_table_id, ObString& uri, const int64_t start_time, const int64_t tablet_version)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (root_table_->table_is_exist(table_id))
    {
      ...
      YYSYS_LOG(ERROR, "table id %lu is exist in root table, can't load it again", table_id);
    }
  }
  ...
  if (!is_loading_data_)
  {
    ...
    balance_last_migrate_succ_time_ = 0;
    server_manager_->reset_balance_info();
    YYSYS_LOG(INFO, "entry load table stat");
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY && OB_SUCCESS == ret; ++i)
    {
          ...
          && (load_data_infos_[i].table_id_ == table_id || load_data_infos_[i].table_name_ == table_name))
      {
        ...
        YYSYS_LOG(ERROR, "table %.*s %lu is loading, can't load table %.*s %lu again",
    ...
    if (NULL == info)
    {
      ...
      YYSYS_LOG(ERROR, "max load info conrurrency is %d, no new load table task is allowed", MAX_LOAD_INFO_CONCURRENCY);
    }
    else if (OB_SUCCESS != (ret = info->set_info(table_name, table_id, old_table_id, uri, tablet_version, start_time, ObLoadDataInfo::NEW)))
    {
      YYSYS_LOG(WARN, "failed to add load table table_name=%.*s table_id=%lu uri=%.*s, ret=%d",
          table_name.length(), table_name.ptr(), table_id, uri.length(), uri.ptr(), ret);
    }
  }
  return ret;
...
int ObRootBalancer::set_load_table_status_from_log(const uint64_t table_id, const int32_t status, const int64_t end_time)
{
  ...
  {
    ret = OB_NOT_DATA_LOAD_TABLE;
    YYSYS_LOG(ERROR, "table %lu is not loading, cannot change status", table_id);
  }
  else
  {
...
{ // load_data_lock_ must be locked by caller
  ...
  else
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(ERROR, "failed to malloc data buffer");
    }
    ...
    {
      YYSYS_LOG(WARN, "failed to  encode count of load info list, count=%ld", count);
    }
    else
    {
      for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY && OB_SUCCESS == ret; ++i)
      {
        if (load_data_infos_[i].is_working())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to encode table_name=%.*s, capacity=%ld, pos=%ld, ret=%d",
                load_data_infos_[i].table_name_.length(), load_data_infos_[i].table_name_.ptr(),
          ...
          {
            YYSYS_LOG(WARN, "failed to encode table_id=%ld, capacity=%ld, pos=%ld, ret=%d",
          ...
          {
            YYSYS_LOG(WARN, "failed to encode old_table_id=%ld, capacity=%ld, pos=%ld, ret=%d",
          ...
          {
            YYSYS_LOG(WARN, "failed to encode start_time=%ld, capacity=%ld, pos=%ld, ret=%d",
          ...
          {
            YYSYS_LOG(WARN, "failed to encode tablet_version=%ld, capacity=%ld, pos=%ld, ret=%d",
                load_data_infos_[i].tablet_version_, buffer.get_capacity(), buffer.get_position(), ret);
                  ...
                  buffer.get_data(), buffer.get_capacity(), buffer.get_position())))
          {
            YYSYS_LOG(WARN, "failed to encode uri=%.*s, capacity=%ld, pos=%ld, ret=%d",
    ...
    if (OB_SUCCESS == ret)
    {
       ...
       {
         ret = OB_ERR_SYS;
         YYSYS_LOG(WARN, "encode length should be %ld, but indeed is %ld", encode_length, buffer.get_position());
       }
    }
...
int ObRootBalancer::read_from_file(const char* filename)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "read data info from [%s] failed, expect size=%ld, indeed read size=%ld",
          filename, size, read_size);
    }
    else if (OB_SUCCESS != (ret = serialization::decode_i64(data_buffer, size, pos, &count)))
    {
      YYSYS_LOG(WARN, "failed to decode count, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
    }
    else
    {
      ObString table_name;
      ...
      for(int64_t i=0; i<count && OB_SUCCESS == ret; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to decode table_name, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        else if (OB_SUCCESS != (ret = serialization::decode_i64(data_buffer, size, pos, reinterpret_cast<int64_t*>(&table_id))))
        {
          YYSYS_LOG(WARN, "failed to decode table_id, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        else if (OB_SUCCESS != (ret = serialization::decode_i64(data_buffer, size, pos, reinterpret_cast<int64_t*>(&old_table_id))))
        {
          YYSYS_LOG(WARN, "failed to decode old_table_id, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        ...
        {
          YYSYS_LOG(WARN, "failed to decode start_time, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        else if (OB_SUCCESS != (ret = serialization::decode_i64(data_buffer, size, pos, &tablet_version)))
        {
          YYSYS_LOG(WARN, "failed to decode tablet_version, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        else if (OB_SUCCESS != (ret = uri.deserialize(data_buffer, size, pos)))
        {
          YYSYS_LOG(WARN, "failed to decode uri, capacity=%ld, pos=%ld, ret=%d", size, pos, ret);
        }
        else if (OB_SUCCESS != (ret = add_load_table_from_log(table_name, table_id, old_table_id, uri, start_time, tablet_version)))
        {
          YYSYS_LOG(WARN, "failed to add load table info, table_name=%.*s table_id=%lu old_table_id=%lu"
    ...
    ObList<ObNewRange*>& range_table, ModuleArena& allocator)
{
  ...
  do {
    ...
    {
      YYSYS_LOG(WARN, "failed to get data source proxy, table_name=%.*s table_id=%lu"
    ...
    {
      YYSYS_LOG(WARN, "failed to fetch range table from data source server=%s, "
    ...
    {
      YYSYS_LOG(WARN, "range list must not empty, data source server=%s, "
    ...
    else
    {
          ...
          it != range_table.end() && OB_SUCCESS == ret; ++it)
      {
        ...
        {
          ret = OB_ERR_SYS;
          YYSYS_LOG(ERROR, "range pointer must not null");
        }
        else if (table_id != (*it)->table_id_)
        {
          ret = OB_DATA_SOURCE_TABLE_NOT_EXIST;
          YYSYS_LOG(ERROR, "the tablet on uri=%.*s's table id is %lu, but request table_id is %lu",
        ...
        else if (first)
        {
          ...
          {
            YYSYS_LOG(WARN, "first range is not start with min, range: %s",  to_cstring(*(*it)));
        ...
        {
          YYSYS_LOG(WARN, "last row key %s not same with the start key of range %s", to_cstring(last_row_key), to_cstring(*(*it)));
          ret = OB_DATA_SOURCE_SYS_ERROR;
          //flag = false;
        }
        else
      ...
      {
        //flag = false;
        YYSYS_LOG(WARN, "last range is not end with max, range: %s",  to_cstring(last_row_key));
        ret = OB_DATA_SOURCE_SYS_ERROR;
      }
    }
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "failed to fetch range table from data source server=%s, "
  ...
  {
    YYSYS_LOG(INFO, "succeed to fetch range table from data source server=%s, "
    ...
    const uint64_t table_id, bool& is_finished, bool& is_failed)
{
  ...
  { // check import status on all clusters
    ...
    {
      ret = tmp_ret;
      YYSYS_LOG(WARN, "failed to get import status, master_rs=%s, table_name=%.*s, table_id=%lu, ret=%d",
    ...
    else
    {
      ...
      {
        is_finished = true;
        YYSYS_LOG(INFO, "got import status, master_rs=%s, table_name=%.*s, table_id=%lu status=COPIED",
            to_cstring(master_rs), table_name.length(), table_name.ptr(), table_id);
      }
      else if (ObLoadDataInfo::DOING == status)
      {
        is_finished = false;
        YYSYS_LOG(INFO, "got import status, master_rs=%s, table_name=%.*s, table_id=%lu status=DOING",
            to_cstring(master_rs), table_name.length(), table_name.ptr(), table_id);
      }
      else if (ObLoadDataInfo::PREPARE == status)
      {
        is_finished = false;
        YYSYS_LOG(INFO, "got import status, master_rs=%s, table_name=%.*s, table_id=%lu status=PREPARE.",
            to_cstring(master_rs), table_name.length(), table_name.ptr(), table_id);
      }
      else if (ObLoadDataInfo::FAILED == status)
      {
        is_failed = true;
        is_finished = false;
        YYSYS_LOG(ERROR, "got import status, master_rs=%s, table_name=%.*s, table_id=%lu status=FAILED."
      ...
      else
      {
        is_finished = false;
        ret = OB_DATA_LOAD_TABLE_STATUS_ERROR;
        YYSYS_LOG(ERROR, "got import status, master_rs=%s table_name=%.*s table_id=%lu"
    ...
    /*for (int64_t i = 0; i < slave_count && OB_SUCCESS == ret; ++i)
    {
      ...
      if (OB_SUCCESS != tmp_ret)
      {
        ret = tmp_ret;
        is_finished = false;
        YYSYS_LOG(WARN, "failed to get import status, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu, ret=%d",
      ...
      else
      {
        ...
        if (ObLoadDataInfo::COPIED == status)
        {
          YYSYS_LOG(INFO, "got import status, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu status=COPIED",
              i, to_cstring(slave_rs[i]), table_name.length(), table_name.ptr(), table_id);
        }
        else if (ObLoadDataInfo::DOING == status)
        {
          is_finished = false;
          YYSYS_LOG(INFO, "got import status, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu status=DOING",
              i, to_cstring(slave_rs[i]), table_name.length(), table_name.ptr(), table_id);
        }
        else if (ObLoadDataInfo::PREPARE == status)
        {
          is_finished = false;
          YYSYS_LOG(INFO, "got import status, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu status=PREPARE",
              i, to_cstring(slave_rs[i]), table_name.length(), table_name.ptr(), table_id);
        }
        else if (ObLoadDataInfo::FAILED == status)
        {
          is_finished = false;
          is_failed = true;
          YYSYS_LOG(ERROR, "got import status, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu status=FAILED",
              i, to_cstring(slave_rs[i]), table_name.length(), table_name.ptr(), table_id);
        }
        else
        {
          is_finished = false;
          ret = OB_DATA_LOAD_TABLE_STATUS_ERROR;
          YYSYS_LOG(ERROR, "got import status, slave_rs[%ld]=%s table_name=%.*s table_id=%lu"
  ...
  {
    is_finished = false;
    YYSYS_LOG(WARN, "failed to check import status, set is_finished = false");
  }
    ...
    const ObLoadDataInfo::ObLoadDataStatus& status)
{
  ...
  YYSYS_LOG(INFO, "start to set import table status: table_name=%.*s table_id=%lu, status=%s(%d)",
      table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status), status);

  if (!root_server_->is_master() || root_server_->get_obi_role().get_role() != ObiRole::MASTER)
  {
    ret = OB_NOT_MASTER;
    YYSYS_LOG(WARN, "this rs is not master of marster cluster, cannot set import task status");
  }
  ...
  { // set import status on all clusters
    ...
    if (OB_SUCCESS == tmp_ret)
    {
      YYSYS_LOG(INFO, "succeed to set import, master_rs=%s, table_name=%.*s, table_id=%lu, status=%s",
          to_cstring(master_rs), table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status_32));
    }
    else
    {
      ret = tmp_ret;
      YYSYS_LOG(WARN, "failed to set import, master_rs=%s, table_name=%.*s, table_id=%lu, status=%s, ret=%d",
    ...
    //   if (OB_SUCCESS == tmp_ret)
    //   {
    //     YYSYS_LOG(INFO, "succeed to set import, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu, status=%s",
    //         i, to_cstring(slave_rs[i]), table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status_32));
    //   }
    //   else
    //   {
    //     ret = tmp_ret;
    //     YYSYS_LOG(WARN, "failed to set import, slave_rs[%ld]=%s, table_name=%.*s, table_id=%lu, status=%s, ret=%d",
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "succeed to set all import table: table_name=%.*s table_id=%lu status=%s",
        table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status));
  }
  else
  {
    YYSYS_LOG(WARN, "failed to set all import table: table_name=%.*s table_id=%lu status=%s",
        table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status));
  }

  if (status == ObLoadDataInfo::FAILED)
  {
    YYSYS_LOG(ERROR, "fatal error: load table table_name=%.*s, table_id=%lu failed", table_name.length(), table_name.ptr(), table_id);
  }
    ...
    const uint64_t table_id, ObLoadDataInfo::ObLoadDataStatus& status)
{
  ...
  {
    yysys::CRLockGuard guard(load_data_lock_);
    for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY; ++i)
    {
      if (load_data_infos_[i].table_id_ == table_id &&
          load_data_infos_[i].table_name_ == table_name)
      {
        ...
        {
          status = ObLoadDataInfo::FAILED;
          // YYSYS_LOG(INFO, "table_id=%lu with tablet_version=%ld is in FAILED status",
          //     table_id, load_data_infos_[i].tablet_version_);
          YYSYS_LOG(INFO, "table_id=%lu, fail_count=%ld with tablet_version=%ld is in FAILED status",
              table_id, load_data_infos_[i].fail_count_, load_data_infos_[i].tablet_version_);
        }
        else if (status == ObLoadDataInfo::DOING)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to check if table_name=%.*s table_id=%lu with tablet_version=%ld is replicated, ret=%d",
                load_data_infos_[i].table_name_.length(), load_data_infos_[i].table_name_.ptr(),
        ...
        {
          YYSYS_LOG(INFO, "table_name=%.*s table_id=%lu with tablet_version=%ld is in PREPARE status",
              table_name.length(), table_name.ptr(),
              table_id, load_data_infos_[i].tablet_version_);
        }
        else if (ObLoadDataInfo::FAILED == status)
        {
          YYSYS_LOG(INFO, "table_name=%.*s table_id=%lu with tablet_version=%ld is in FAILED status",
              table_name.length(), table_name.ptr(),
              table_id, load_data_infos_[i].tablet_version_);
        }
        else
        {
          YYSYS_LOG(ERROR, "the status of table %.*s with table_id=%lu and uri=%.*s is not DOING",
  ...
  {
    ret = OB_NOT_DATA_LOAD_TABLE;
    YYSYS_LOG(WARN, "table %.*s with table_id=%lu is not loading", table_name.length(), table_name.ptr(), table_id);
  }
  return ret;
}
    ...
    const uint64_t table_id, const ObLoadDataInfo::ObLoadDataStatus& status)
{
    ...
    for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY; ++i)
    {
          ...
          && load_data_infos_[i].table_name_ == table_name)
      {
            ...
            && status == ObLoadDataInfo::DONE)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to do load data done, table_name=%s, table_id=%lu",
        ...
        else if (status == ObLoadDataInfo::FAILED || status == ObLoadDataInfo::KILLED)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to do load data fail, table_name=%s, table_id=%lu",
                to_cstring(table_name), table_id);
          }
          if (root_server_->get_obi_role().get_role() == ObiRole::MASTER)
          {
        ...
        {
          ret = OB_DATA_LOAD_TABLE_STATUS_ERROR;
          YYSYS_LOG(ERROR, "import status of table %.*s with table id=%lu is %s(%d), can't set to be %s(%d)",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "succeed to set table %.*s with table_id=%lu as status=%s",
          table_name.length(), table_name.ptr(), table_id, ObLoadDataInfo::trans_status(status));
    }
  }
  else
  {
    YYSYS_LOG(WARN, "failed to set table %.*s with table_id=%lu as status=%d, ret=%d",
...
int ObRootBalancer::is_table_replicated(const uint64_t table_id, const int64_t tablet_version, bool& is_replicated)
{
  ...
  int64_t safe_count = 0;
  int64_t total_count = 0;
  int64_t load_data_replicas_num = config_->load_data_replicas_num;
  int32_t chunk_server_count = server_manager_->get_alive_server_count(true);
  if (chunk_server_count > 0 && chunk_server_count < load_data_replicas_num)
  {
    YYSYS_LOG(INFO, "check chunkserver count less than replica num:server[%d], replica[%ld]",
            chunk_server_count, load_data_replicas_num);
    load_data_replicas_num = chunk_server_count;
  }
      ...
      if (tablet->range_.table_id_ == table_id)
      {
        ...
        //add e
        {
          if (OB_INVALID_INDEX != it->server_info_indexes_[i])
          {
            ...
            {
              is_replicated = false;
              YYSYS_LOG(WARN, "%s has tablet_verion[%ld] not equals with need tablet version[%ld]",
        ...
        //if (valid_replicas_num < tablet_replicas_num)
        if (valid_replicas_num < load_data_replicas_num)
        {
          is_replicated = false;
          break;
        }
  ...
  {
    YYSYS_LOG(INFO, "table with table_id = %lu is replicated, tablet_version=%ld, tablet_replicas_num=%ld total_count=%ld",
        table_id, tablet_version, tablet_replicas_num, total_count);
  }
  else
  {
    YYSYS_LOG(INFO, "table with table_id = %lu is not replicated, tablet_version=%ld,"
...
int ObRootBalancer::get_ms(ObServer& ms_server)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "no serving ms found, ret=%d", ret);
    ret = OB_MS_NOT_EXIST;
  }
  return ret;
}
...
{ // load_data_lock_ must be locked by caller
  ...
  { // this flag will also be set by "build root table", to change name of this method in 0.5
    ret = OB_EAGAIN;
    YYSYS_LOG(WARN, "root server is building root table, can't start load table");
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to lock frozen version");
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "load table could not start:  failed to check_replica_count_for_import, ret=%d", ret);
      if (OB_SUCCESS != root_server_->unlock_frozen_version())
      {
        YYSYS_LOG(ERROR, "failed to unlock frozen version");
      }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "entry load table stat, set tablet version as %ld",
        root_server_->get_frozen_version_for_cs_heartbeat());
    balance_batch_migrate_count_ = 0;
    balance_batch_copy_count_ = 0;
    balance_batch_migrate_done_num_ = 0;
...
{ // load_data_lock_ must be locked by caller
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "fail to get table schema. table_name=%s, ret=%d", to_cstring(table_name), ret);
    }
    else if (old_table_id == table_id)
    {
      ret = OB_ERR_SYS;
      YYSYS_LOG(ERROR, "old table id must not same as current table id, cant do add load table. table_name=%s, table_id=%lu",
          to_cstring(table_name), table_id);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (NULL == info)
    {
      ...
      for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY; ++i)
      {
        YYSYS_LOG(INFO, "%ld: status=%s table_name=%.*s table_id=%lu start_time=%ld", i, load_data_infos_[i].get_status(),
    ...
    {
      YYSYS_LOG(WARN, "failed to add load table table_name=%.*s table_id=%lu old_table_id=%lu uri=%.*s, ret=%d",
          table_name.length(), table_name.ptr(), table_id, old_table_id, uri.length(), uri.ptr(), ret);
    }
    else
    {
      YYSYS_LOG(INFO, "add load table task info: table_name=%.*s table_id=%lu old_table_id=%lu uri=%.*s, start_time=%ld, status=NEW",
...
int ObRootBalancer::check_replica_count_for_import(int64_t tablet_version)
{
  ...
  {
    // ret = OB_ERR_SYS;
    // YYSYS_LOG(ERROR, "check chunkserver count less than replica num:server[%d], replica[%ld]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "check_tablet_version_merged_v2() failed. ret=%d", ret);
      }
      if (out_schema != NULL)
      {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to check_tablet_version_merged, tablet_version=%ld min_replica_count=%ld, ret=%d",
      ...
      {
        ret = OB_RS_STATE_NOT_ALLOW;
        YYSYS_LOG(INFO, "root table does not satisfy tablet_version=%ld min_replica_count=%ld",
...
int ObRootBalancer::prepare_new_load_table()
{
  ...
  while(OB_SUCCESS == ret && found)
  {
    ...
    // find loading table in prepare stat
    {
      ...
      for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY; ++i)
      {
            ...
            && load_data_infos_[i].fail_count_ < ObLoadDataInfo::MAX_FAIL_COUNT)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to clone info, table_name=%.*s table_id=%lu, ret=%d",
                load_data_infos_[i].table_name_.length(), load_data_infos_[i].table_name_.ptr(),
    ...
    YYSYS_LOG(INFO, "changing table_name=%.*s table_id=%lu uri=%.*s from PREPARE -> DOING status",
        info.table_name_.length(), info.table_name_.ptr(), info.table_id_,
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to fetch range list, table_name=%.*s table_id=%lu uri=%.*s, ret=%d",
            info.table_name_.length(), info.table_name_.ptr(), info.table_id_,
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY; ++i)
      {
            ...
            load_data_infos_[i].table_name_ == info.table_name_)
        {
          ...
          if (load_data_infos_[i].status_ != ObLoadDataInfo::PREPARE)
          {
            info_ptr = NULL;
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "status must be ObLoadDataInfo::PREPARE, but current status is %s",
          ...
          else if (load_data_infos_[i].start_time_ != info.start_time_)
          {
            info_ptr = NULL;
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "find wrong load data info, start_time should be %ld, but indeed start_time=%ld"
                " table_name=%.*s table_id=%lu uri=%.*s",
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "info_ptr must not null");
        nb_print_migrate_infos();
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ROOT_TABLE_ID_EXIST;
          YYSYS_LOG(ERROR, "table id %lu is exist in root table, can't load it again", info.table_id_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_IN_STOP_STATE;
          YYSYS_LOG(WARN, "rs is in receive_stop_, stop prepare_new_load_table");
        }
        else
        {
          need_roll_back = true;
          ...
          if (OB_SUCCESS != (ret = root_server_->add_range_for_load_data(range_table)))
          {
            ...
            YYSYS_LOG(ERROR, "failed to insert range table to root table, ret=%d", ret);
          }
          if (root_server_->is_master() && root_server_->get_obi_role().get_role() == ObiRole::MASTER)
          {
    ...
    if (OB_SUCCESS != ret && info.is_working())
    {
      ...
      if (fail_count >= ObLoadDataInfo::MAX_FAIL_COUNT || need_roll_back)
      {
        if (root_server_->get_obi_role().get_role() == ObiRole::MASTER)
        {
          YYSYS_LOG(ERROR, "init load table failed count=%ld(limit %ld), kill it! table_name=%.*s table_id=%lu",
              fail_count, ObLoadDataInfo::MAX_FAIL_COUNT,
              info.table_name_.length(), info.table_name_.ptr(), info.table_id_);
          int tmp_ret = start_set_import_status(info.table_name_, info.table_id_, ObLoadDataInfo::FAILED);
          if (OB_SUCCESS != tmp_ret)
          {
            YYSYS_LOG(WARN, "failed to kill load table, ret=%d", tmp_ret);
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "failed to init table_name=%.*s table_id=%lu, wait for master rs to stop it.",
              info.table_name_.length(), info.table_name_.ptr(), info.table_id_);
        }
      }
    }
...
void ObRootBalancer::stop_failed_load_table_task(bool &is_finished)
{
  int ret = OB_SUCCESS;
  bool found = false;
  // bool is_finished = false;
    ...
    for(int64_t i=0; i<MAX_LOAD_INFO_CONCURRENCY && !receive_stop_; ++i)
    {
      ...
      if (load_data_infos_[i].is_working())
      {
        ...
        if (load_data_infos_[i].is_working())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to dup load table info with table name=%.*s, table_id=%lu, ret=%d",
                load_data_infos_[i].table_name_.length(), load_data_infos_[i].table_name_.ptr(),
                load_data_infos_[i].table_id_, ret);
            break;
          }
      ...
      if (found)
      {
        ...
        {
          YYSYS_LOG(INFO, "table_name=%.*s table_id=%lu is copied on all clusters, wait master rs to set it DONE",
              info.table_name_.length(), info.table_name_.ptr(), info.table_id_);
        }
...
void ObRootBalancer::print_balancer_info(const int64_t table_id)
{
  ...
  */
  ...
  {
    YYSYS_LOG(ERROR, "allocate memory for dump balancer info failed.");
  }
  else
  {
  //mod:e
...
int ObRootBalancer::print_sstable_info()
{
  ...
  for (int32_t i = 0; i < table_count && scan_next_table && config_->enable_balance; ++i) // for each table
  {
    ...
    if (OB_INVALID_ID != (table_id = nb_get_next_table_id(table_count)))
    {
      if(table_id > 0) //mod pangtianze [Paxos sstable info to rs log] 20170723
      {
        for(int32_t cluster_id = 0; OB_SUCCESS == ret && cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++) // mod lqc [multiusp] 20170702
        {
          if(is_cluster_has_alive_cs(cluster_id))
          {
            int64_t avg_size = 0;
            ...
            if (OB_SUCCESS != (ret = nb_calculate_sstable_count(table_id, avg_size, avg_count,
                    cs_num, migrate_out_per_cs, shutdown_num, need_replicate, table_found, total_count, safe_count, cluster_id
                    )))
            {
              YYSYS_LOG(WARN, "calculate table size error, err=%d, table_id=%ld", ret, table_id);
            }
            else if (!table_found)
            {
              YYSYS_LOG(WARN, "can't find table_id %lu in root table, no balance need", table_id);
            ...
            {
              YYSYS_LOG(INFO, "=================== Balance ===================\n");
              ObServerStatus *it = NULL;
              for (it = server_manager_->begin(); it != server_manager_->end(); ++it)
              {
                if (it->status_ != ObServerStatus::STATUS_DEAD && it->server_.cluster_id_ == cluster_id)
                {
                    YYSYS_LOG(INFO, "[%s] sstable:%ld\n", to_cstring(it->server_), it->balance_info_.table_sstable_count_);
                }
              }
            }
            YYSYS_LOG(INFO, "[table_id=%lu] tablet_num=%ld, cs_num=%d, avg=%ld, cluster_id=%d",
...
int ObRootBalancer::calculate_table_replica_num(const int64_t table_id, int32_t *replicas_num)
{
  ...
  if (schema_mgr == NULL)
  {
    YYSYS_LOG(ERROR, "cannot get mem for schema");
    ret = OB_ERROR;
  }
  ...
  {
    YYSYS_LOG(WARN, "table=%lu not exist.", table_id);
    ret = OB_ERROR;
  }
  else
  {
    ...
    if (cluster_mgr_->get_total_tablet_replicas_num() <= 0 )
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "total cluster replica count is invalid, count=%d, table_id=%ld",schema_replica_num, table_id);
    }
    else
    {
    //add:e
      yysys::CRLockGuard guard(*cluster_mgr_rwlock_);
      if (OB_SUCCESS != (ret = cluster_mgr_->get_cluster_tablet_replicas_num(replicas_num, schema_replica_num)))
      {
        YYSYS_LOG(WARN, "fail to get replicas_num, ret=%d", ret);
      }
      else
      {
        for(int32_t cluster_id = 0; cluster_id < OB_CLUSTER_ARRAY_LEN; cluster_id++)
