...
void ObMigrateInfos::set_size(int64_t size)
{
  ...
  {
    YYSYS_LOG(ERROR, "max migrate concurrency is %lu, can't set it as %lu, set it as %lu.",
  ...
  YYSYS_LOG(INFO, "current max migrate concurrency is %lu", size_);
}
    ...
    const common::ObServer& dest_server, const common::ObDataSourceDesc& data_source_desc)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (NULL == minfo)
    {
      ...
      YYSYS_LOG(WARN, "no free migrate info found, failed to add migrate info, max_size=%ld", size_);
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "clone start key error, ret=%d", ret);
        minfo->reuse();
      }
      ...
      else
      {
        ...
        minfo->start_time_ = yysys::CTimeUtil::getTime();
        minfo->stat_ = ObMigrateInfo::STAT_USED;
        YYSYS_LOG(DEBUG, "new migrate info: %s %s", to_cstring(dest_server), to_cstring(data_source_desc));
      }
    }
  }
    ...
    const common::ObServer& dest_server)
{
  ...
  {
    YYSYS_LOG(WARN, "no migrate info found for range=%s src_server=%s dest_server=%s",
            ...
            common::ObServer& dest_server, common::ObDataSourceDesc::ObDataSourceType& type)
{
  ...
  for (int64_t i=0; i< MAX_MIGRATE_INFO_NUM; ++i)
  {
    if (infos_[i].stat_ == ObMigrateInfo::STAT_USED && infos_[i].start_time_ + timeout < cur_time)
    {
      ...
      has_timeout = true;
      YYSYS_LOG(WARN, "loading data timeout: type=%s range=%s src_server=%s"
...
void ObMigrateInfos::print_migrate_info()
{
  ...
  for (int64_t i=0; i< MAX_MIGRATE_INFO_NUM; ++i)
  {
    if (infos_[i].stat_ == ObMigrateInfo::STAT_USED)
    {
      ...
      YYSYS_LOG(INFO, "migrate info[%ld], start_time=%s dest_cs=%s src_cs=%s range=%s keep_src=%c tablet_version=%ld",
  ...
  YYSYS_LOG(INFO, "migrate info: total concurrency migrate task is %ld", count);
}
