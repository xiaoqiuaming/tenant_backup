...
#include "rootserver/ob_root_major_freeze_task.h"  //add peiouya [MultiUPS] [PARTITION_LOCK_FUCTION] 20150609
#include "sql/ob_sql_result_set.h"  //add liuzy [MultiUPS] [add_paxos_interface] 20160127
#include "rootserver/ob_root_scale_task.h" //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160304
...
namespace oceanbase
{
  ...
  namespace rootserver
  {
    ...
    class ObRootWorker;
    class ObRootServer2;
    class ObSysTableReviseRunnable;
    // �μ���OceanBase�Ծ����̡�
    class ObBootState
    {
      public:
    ...
    /// @brief define module of cluster scale
    //add liuzy [MultiUPS] [add_cluster_interface] 20160315:b
    enum ScaleType
    {
      UPS_PAXOS_SCALE = 0,
      UPS_CLUSTER_SCALE = 1,
    ...
    class ObRootServer2
    {
      ...
      public:
        ...
        int check_tablet_complete_in_clusters(const uint64_t table_id, const int64_t tablet_version, bool is_complete[OB_MAX_CLUSTER_COUNT]);
        ...
        inline void set_paxos_quorum_scale(const char *paxos_quorum_scale)
        {
          common::ObSpinLockGuard guard(change_paxos_quorum_scale_lock_);
          config_.paxos_ups_quorum_scales.set_value(paxos_quorum_scale);
        }
        ...
        inline yysys::CRWLock * get_sys_table_revise_lock()
        {
          return &sys_table_revise_rwlock_;
        }
        //add:e
        ...
        int add_hardcode_partition_rule(void);
        // oceanbase bootstrap
        int boot_strap();
        ...
        int check_column_checksum(const int32_t cluster_id, const int64_t index_table_id, const int64_t version, bool &is_right);
        ...
        void set_if_orphan_index_exist(bool is_exist);
        bool check_if_have_orphan_index() const;
        void handle_orphan_index();
        ...
        int notify_partition_unchanged_version(int64_t &mem_version);
        ...
        //mod by maosy [MultiUPS] [Balance_Modify] 20150518 b:  //[uncertainty 
...
//        int force_drop_table(const uint64_t table_id);
        ...
        bool get_cluster_sys_stat()
        { return update_cluster_stat_; }
        ...
        /*
         * �л�������, update server�����ڴ�� ����chunk server ����merge�Ⱥ�ʱ�������
         * ������Ϣ���ô˺���
         */
       ...
       virtual int migrate_over(const int32_t result, const ObDataSourceDesc& desc,
        ...
        int find_root_table_key(const common::ObGetParam& get_param, common::ObScanner& scanner
                                //add lbzhong [Paxos Cluster.Flow.CS] 201607026:b
                                , const int32_t cluster_id = OB_ALL_CLUSTER_FLAG
                                //add:e
                                );
        ...
        int fill_tablet_info_list_by_server_index(ObIndexTabletInfoList *tablet_info_list[], 
        ...
        int check_create_global_index_done(const uint64_t index_tid, bool &is_finished);
        ...
        //add 20141017:e
       // common::ObServer get_update_server_info(bool use_inner_port) const; //uncertainty  trunk �Ѿ��������
        //add peiouya [MultiUPS] [UPS_Manage_Function] 20150429:b
        common::ObServer get_update_server_info_by_paxos(const int64_t paxos_id, bool use_inner_post) const;
        //add 20150429:e
        void get_all_master_ups_info(common::ObArray<ObServer>& paxos_array, bool use_inner_port) const;
        //mod 20150426:e
        //mod 20150426:e
        ...
        void dump_unusual_tablets(const int64_t tablet_version) const;
        ...
        int register_ups(const common::ObServer &addr, int32_t inner_port, int64_t log_seq_num,
                     int64_t lease, const char *server_version_, int64_t cluster_id,
                     int64_t paxos_id, int64_t last_frozen_version);
        //mod 20150427:e

        //mod peiouya [MultiUPS] 20150701:b
        ...
        int get_all_ups_list(common::ObUpsList &ups_list);

		 //add peiouya [MultiUPS] [UPS_Manage_Function] 20150528:e
        int get_master_ups_list(common::ObUpsList &ups_list);
        //add 20150528:e

        //[492]
        int force_get_sys_table_ups_list(common::ObUpsList &ups_list);
        ...
        int serialize_cs_info_list(char* buf, const int64_t buf_len, int64_t& pos) const;
        ...
        void reset_server_manager_info();
        //add:e
        ...
        //add:e
        int alter_group(const common::ObString &group_name, const int64_t paxos_idx);
        int check_paxos_can_offline(const int64_t offline, const int64_t paxos_idx, bool &can_offline);
        ...
        void set_index_phase(IndexPhase index_phase);//add liumz, [static_index range_intersect]20170915
        //add e
        ...
        int64_t get_to_be_frozen_version() const;

        int64_t get_last_frozen_version() const;

        //[view]
        int create_view(const bool do_replace, const common::TableSchema &tschema);
        int check_view_exist(const ObString &table_name, bool &exist, bool &is_view);
        int drop_views(bool if_exists, const common::ObStrings &views);
        int drop_one_view(bool if_exists, const common::ObString &view_name, bool &refresh);

        //add peiouya [MultiUPS] [UPS_Manage_Function] 20150527:b
        void set_last_frozen_version(const int64_t last_frozen_version, const bool did_force = false);
        void kill_self();
        //add 20150527:e

        //add peiouya [MultiUPS] [PARTITION_LOCK_FUCTION] 20150601:b
        bool get_partition_lock_flag() const;
        void set_partition_lock_flag_for_ups(const bool partition_lock_flag, const int64_t last_frozen_version);
        void set_partition_lock_flag_for_rs();
        //add 20150601:e

        bool get_minor_freeze_flag() const;
        void set_minor_freeze_flag(const bool minor_freeze_flag);

        //mod peiouya [MultiUPS] [STAT_MERGE_BUG_FIX] 20150725:b
        void set_last_frozen_time(int64_t frozen_time);
        //mod 20150725:e
        //add peiouya [MultiUPS] [PARTITION_LOCK_FUCTION] 20150609:b
        bool get_partition_lock_flag_for_ups() const;
        int64_t get_new_frozen_version();
        void set_new_frozen_version(const int64_t to_be_version, const bool is_cmd_set);
        //add 20150609:e
        ...
        ObRootMsProvider & get_ms_provider()
        { return ms_provider_; }
        ...
        int32_t get_total_cluster_tablet_replicas_num() const;
        void    reset_cluster_tablet_replicas_num();
        //add pangtianze [Paxos Cluster.Balance] 20170905:e
        ...
        //add:e
        //int64_t get_alive_and_have_replia_cluster_num_with_cs();
        ...
        int do_add_new_paxos_group(const int64_t new_paxos_num);
        int renew_config_use_num(const int64_t new_config, ScaleType type);
        //add 20160112:e

        int do_del_last_offline_paxos_group(const int64_t paxos_idx);

        //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160223:b
        int  do_paxos_group_offline(const int64_t paxos_idx);
        bool is_merge_done();
        void set_offline_paxos_stat();
        void renew_cur_version_offline_set();
        void get_cur_paxos_offline_set(ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> &bit_set);
        //add 20160223:e
        //add liuzy [MultiUPS] [add_cluster_interface] 20160311:b
        int do_add_new_cluster(const int64_t new_cluster_num);
        int sync_ups_manager_config(const int64_t new_num, ScaleType type);

        int sync_ups_manager_config_delete(const int64_t id, ScaleType type);

        int sync_slave_rs_config(const int64_t config_version);
        
        int do_del_last_offlined_cluster(const int64_t cluster_idx);
        //add 20160311:e
        //add liuzy [MultiUPS] [take_cluster_offline_interface] 20160325:b
        int  init_cluster_stat_info();

        //[492]
        int get_cluster_and_paxos_stat_info(int64_t &paxos_new_stat, int64_t &cluster_new_stat,
                                            int64_t &paxos_offline_stat, int64_t &cluster_offline_stat, const int64_t version);
        int get_cur_data_version(int64_t &version);
        int revert_cluster_and_paxos_stat_info(int64_t version);

        int  get_rs_offline_count_next_version();
        int  do_cluster_offline(const int64_t cluster_idx);
        int  refresh_all_cluster_stat_info(int64_t version);
        
        int refresh_all_change_cluster_paxos(int64_t op_time_int, const char *op_client, int64_t version, ObChangePaxosClusterType op_type, const char *op_param, int64_t num, int op_result);
        int update_use_num_of_cluster_stat_info(int64_t version, int64_t use_num, ScaleType type);

        //[624]
        int del_index_of_cluster(const int64_t cluster_idx);
        int del_all_cluster(const int64_t cluster_idx);
        
        int  refresh_sys_offline_info(int64_t version, int64_t use_num, int32_t offline_info, ScaleType type);

        //[492]
        int refresh_sys_new_info(int64_t version, int64_t use_num, int32_t new_info, ScaleType type);
        void resolve_new_and_offline_stat_conflict(int64_t version, ScaleType type, StateType stat_type);
        int refresh_sys_cur_data_version_info();


        bool is_initialized();
        void set_offline_cluster_stat();

        //[492]
        void set_new_cluster_stat();
        void set_new_paxos_stat();

        bool is_cluster_offline(const int64_t cluster_idx);

        //[691]
        bool is_paxos_group_offline_next_version(const int32_t paxos_idx)
        {
            return paxos_offline_set_next_version_.has_member(paxos_idx);
        }

        bool is_rs_offline_count_valid(const int64_t cluster_idx, int32_t &permit_count);
        //add 20160325:e
        int check_cluster_can_delete(const int64_t cluster_idx);
        //add liuzy [MultiUPS] [take_online_interface] 20160418:b
        int do_paxos_group_online(int64_t paxos_idx);
        int do_cluster_online(int64_t cluster_idx);
        //add 20160418:e
        int renew_config_replicas_num(int32_t *replicas_nums);
        int do_get_ups_memory(int64_t &max_used_mem);

        int start_gather_operation();
        int stop_gather_operation();
        int start_range_collection(int64_t start, int64_t end);
        int start_get_statistics_task_operation(char *db_name);
        int fetch_collection_list(const int64_t version, ObArray<uint64_t> *list);
        int fetch_range_collection_list(const int64_t version, int64_t start, int64_t end, ObArray<uint64_t> *list);

        int32_t set_ups_quorum_scale_of_paxos(int32_t paxos_id, int32_t quorum_scale);
        int32_t get_ups_quorum_scale_of_paxos(int32_t paxos_id, int32_t &quorum_scale);
        bool is_paxos_quorum_scales_valid(const char *ups_quorum_scales);
        int32_t set_ups_quorum_scale_from_str(const char *ups_quorum_scales);
        int32_t get_ups_quorum_scales_from_array(char *outstring);
      ...
      private:
         ...
         * ������ֻ㱨��Ϣ���жԵ�ǰroot table��tablet�ķ��ѻ��ߺϲ�
         * Ҫ���ò���д�������ƵĴ�������
         */
        ...
        void do_stat_paxos_group_info(char* buf, const int64_t buf_len, int64_t &pos);
        void do_stat_cluster_info(char* buf, const int64_t buf_len, int64_t &pos);
        //20160425:e

        //[677]
        void do_stat_rs_link(char *buf, const int64_t buf_len, int64_t &pos);
        ...
        //int select_cs(const int64_t min_count, ObArray<std::pair<ObServer, int32_t> > &chunkservers)
        int select_cs(const int64_t select_num, const int64_t cluster_id, int64_t& select_pos, common::ObArray<std::pair<common::ObServer, int32_t> > &chunkservers,
        int64_t &results_num
                      );
        //mod:e
        ...
        int select_cs_from_all_cluster(const int64_t select_num, common::ObArray<std::pair<common::ObServer, int32_t> > &chunkservers);
        //add 20150722:e
      private:
		...
		//add peiouya [MultiUPS] [Schema_sync] 20150425:b
        int force_sync_schema_all_master_ups(const common::ObSchemaManagerV2  &schema);
        //add 20150425:e
        //add zhaoqiong [Schema Manager] 20150327:b
        /**
         * @brief send schema_mutator to all ms and cs
        ...
        //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160223:b
        int  recover_paxos_group_stat(const int64_t paxos_idx, const int64_t offline_version);
        void set_online_paxos_stat();
        void change_paxos_stat(int64_t cur_config_num, int64_t new_paxos_num, ObPaxosClusterStatus new_stat);
        bool has_paxos_group_offline_next_verison();
        bool is_paxos_group_offline(const int64_t paxos_idx);
        bool mark_paxos_group_offline(const int64_t paxos_idx);
        //add 20160223:e
        int check_paxos_group_can_delete(const int64_t paxos_idx);
        //add wenghaixing [secondary index static_index_build]20150528
        ...
        int  recover_cluster_stat(const int64_t cluster_idx, const int64_t offline_version);
        bool has_cluster_offline_next_version();
        bool mark_cluster_offline(const int64_t cluster_idx);
        void set_online_cluster_stat();
        void change_cluster_stat(int64_t cur_config_num, int64_t new_cluster_num, ObPaxosClusterStatus new_stat);
        //add 20160405:e

        //[492]
        void change_cluster_new_stat_set(int64_t cur_config_num, int64_t new_cluster_num);
        void change_paxos_new_stat_set(int64_t cur_config_num, int64_t new_paxos_num);

        //add liuzy [MultiUPS] [take_online_interface] 20160418:b
        bool mark_paxos_group_online(int64_t paxos_idx);
        bool mark_cluster_online(int64_t cluster_idx);
        //add 20160418:e
        void do_stat_gather(char *buf, const int64_t buf_len, int64_t &pos);
        bool check_all_collection_tasks();
      private:
        ...
        mutable yysys::CThreadMutex root_table_build_mutex_; //any time only one thread can modify root_table
        ...
        mutable yysys::CThreadMutex hist_table_mutex_;
        //add:e
        mutable yysys::CRWLock root_table_rwlock_; //every query root table should rlock this
        common::ObTabletReportInfoList delete_list_;
        bool have_inited_;
        bool first_cs_had_registed_;
        volatile bool receive_stop_;
        volatile bool have_orphan_indexs_;

        mutable yysys::CThreadMutex frozen_version_mutex_;
        ...
        //add pangtianze [Paxos inner table revise] 20170826
        mutable yysys::CRWLock sys_table_revise_rwlock_;
        ObSysTableReviseRunnable *sys_table_revise_thread_;
        //add:e
        //add pangtianze [Paxos rs_election] 20150612:b
        ...
        //add:e
        common::ObSpinLock change_paxos_quorum_scale_lock_;
        // balance related
        ...
        mutable yysys::CThreadMutex rt_service_wmutex_;
        ...
        mutable yysys::CRWLock schema_manager_rwlock_;

        ObRootServerState state_;  //RS state
        int64_t bypass_process_frozen_men_version_; //旁路导入状�?�下，可以广播的frozen_version
        ...
        mutable yysys::CRWLock index_beat_rwlock_;
        //add e        
        //add lbzhong [Paxos Cluster.Balance] 201607014:b
        ObRootClusterManager cluster_mgr_;
        mutable yysys::CRWLock cluster_mgr_rwlock_;
        //add:e
        //add peiouya [MultiUPS] [PARTITION_LOCK_FUCTION] 20150601:b
        ...
        ObRootMajorFreezeDuty major_freeze_duty_;
        //add 20150609:e
        //add hongchen [CLUSTER_STAT_BUGFIX] 20170821:b
        ObRootClusterStatDuty cluster_stat_duty_;
        ...
        mutable yysys::CRWLock bit_set_rwlock_;
        ObRootPaxosExpansionDuty paxos_expansion_duty_;
        common::ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> paxos_offline_set_cur_version_;
        common::ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> paxos_offline_set_next_version_;
        //add 20160226:e
        //add liuzy [MultiUPS] [take_cluster_offline_interface] 20160325:b
        ObRootClusterExpansionDuty cluster_expansion_duty_;
        common::ObBitSet<OB_MAX_CLUSTER_COUNT> cluster_offline_set_cur_version_;
        common::ObBitSet<OB_MAX_CLUSTER_COUNT> cluster_offline_set_next_version_;
        ObRootInitClusterStat init_cluster_stat_duty_;
        //add 20160325:e

        //[492]
        common::ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> paxos_new_set_cur_version_;
        common::ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> paxos_new_set_next_version_;
        common::ObBitSet<OB_MAX_CLUSTER_COUNT> cluster_new_set_cur_version_;
        common::ObBitSet<OB_MAX_CLUSTER_COUNT> cluster_new_set_next_version_;

        bool update_cluster_stat_;
        bool minor_freeze_flag_;
        int32_t paxos_ups_quorum_scales_[OB_MAX_PAXOS_GROUP_COUNT];
        bool partition_change_flag_;
        ObRootPartitionDuty partition_duty_;
    };
  }
}
