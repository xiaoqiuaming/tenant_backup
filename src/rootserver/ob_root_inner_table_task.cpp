   ...
   )
{
  ...
  if (cluster_id < 0)
  {
    YYSYS_LOG(WARN, "check init param failed:cluster_id[%d]", cluster_id);
...
int ObRootInnerTableTask::modify_all_server_table(const ObRootAsyncTaskQueue::ObSeqTask & task)
{
  ...
  else
  {
    switch (task.type_)
    {
      case SERVER_ONLINE:
      {
        //del hongchen 20170717:b
        ////add wangdonghui [ups state exception in __all_server] 20170511 :b
        //if(task.server_status_ == ObRoleMgr::MASTER && task.role_ == OB_UPDATESERVER)
        //{
        ...
        //    if (OB_SUCCESS == ret)
        //    {
        //      YYSYS_LOG(INFO, "process inner task succ:task_id[%lu], timestamp[%ld], sql[%s]",
        //          task.get_task_id(), task.get_task_timestamp(), buf);
        //    }
        //}
        ////add :e
        //del hongchen 20170717:e
        if(OB_SUCCESS == ret)
        {
        const char * sql_temp = "REPLACE INTO __all_server(cluster_id, paxos_id, svr_type,"
          ...
          " VALUES(%d,%d,\'%s\',\'%s\',%u,%u,%d,\'%s\');";
        snprintf(buf, sizeof (buf), sql_temp, task.server_.cluster_id_, static_cast<int32_t>(task.paxos_id_),
                 print_role(task.role_), ip_buf, server_port, task.inner_port_, task.server_status_, task.server_version_);
        }
        break;
      }
      case SERVER_OFFLINE:
      {
        const char * sql_temp = "DELETE FROM __all_server WHERE svr_type=\'%s\' AND"
          " svr_ip=\'%s\' AND svr_port=%d AND cluster_id=%d AND paxos_id=%d;";
        snprintf(buf, sizeof (buf), sql_temp, print_role(task.role_),
            ip_buf, server_port, task.server_.cluster_id_, static_cast<int32_t>(task.paxos_id_));
        break;
      }
      case ROLE_CHANGE:
      {
        ...
        {
        const char * sql_temp = "REPLACE INTO __all_server(cluster_id, paxos_id, svr_type,"
          " svr_ip, svr_port, inner_port, svr_role) VALUES(%d,%d,\'%s\',\'%s\',%u,%u,%d);";
        snprintf(buf, sizeof (buf), sql_temp, task.server_.cluster_id_, static_cast<int32_t>(task.paxos_id_),
                 print_role(task.role_), ip_buf, server_port, task.inner_port_, task.server_status_);
        }
        break;
      }
...
int ObRootInnerTableTask::clean_other_masters(void)
{
  ...
  {
    YYSYS_LOG(WARN, "fetch master cluster id list failed, ret %d, ms %s", ret, to_cstring(ms));
  }
  else
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < cluster_ids.count(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "sql buffer not enough, n %d, errno %d", n, n < 0 ? errno : 0);
        ret = OB_BUF_NOT_ENOUGH;
      }
      else if (OB_SUCCESS != (ret = proxy_->query(
              query_master, RETRY_TIMES, TIMEOUT, ObString::make_string(sql))))
      {
        YYSYS_LOG(WARN, "execute sql [%s] failed, ret %d", sql, ret);
      }
    }
  }
  return ret;
...
void ObRootInnerTableTask::runTimerTask(void)
{
  ...
  else
  {
    ...
    while ((OB_SUCCESS == ret) && (end_time > cur_time))
    {
      ...
      {
        YYSYS_LOG(WARN, "process head inner task failed:ret[%d]", ret);
      }
      else
      {
...
int ObRootInnerTableTask::process_head_task(void)
{
  ...
  {
    ret = OB_NOT_MASTER;
    YYSYS_LOG(WARN, "slave can not process inner task, ignore it");
  }
  //add:e
  if (OB_SUCCESS == ret)
  {
    ...
    default:
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "not supported task right now");
        break;
      }
    }
  }
  ...
  if (OB_SUCCESS == ret || (OB_NOT_MASTER == ret && root_server_->is_slave())) //mod pangtianze [Paxos]  20170708:b
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "pop the succ task failed:ret[%d]", ret);
      pop_task.print_info();
      task.print_info();
    }
  }
...
int ObRootInnerTableTask::check_cluster_alive(const ObRootAsyncTaskQueue::ObSeqTask &task)
{
  ...
  if(SERVER_ONLINE == task.type_) //insert
  {
    ...
    {
      YYSYS_LOG(WARN, "check cluster exist failed, ret %d", ret);
    }
    else if(!is_exist)
    {
  ...
  else if(SERVER_OFFLINE == task.type_ || LMS_OFFLINE == task.type_) //delete
  {
    ...
    if(!is_alive)
    {
      ...
      {
        YYSYS_LOG(ERROR,"process offline cluster faild! ret %d", ret);
      }
      //add:e
    }
            ...
            (!root_server_->get_server_manager().is_cluster_alive_with_cs(cluster_id)) ) //all ms offline or all cs offline
    {
      ...
      {
        YYSYS_LOG(ERROR,"process cluster while all mscs offline faild! ret %d", ret);
      }
      //add:e
    }
    //add:e
  ...
  if(OB_SUCCESS == ret && need_execute_sql)
  {
    ...
    {
      YYSYS_LOG(INFO, "process inner task succ:sql[%s]", buf);
    }
  }
  return ret;
}
...
int ObRootInnerTableTask::process_cluster_while_all_mscs_offline(int32_t cluster_id)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    else if(0 != cluster_flows[cluster_id] || master_cluster_id == cluster_id)
    {
      ...
      int32_t max_flow_cluster_id = OB_ALL_CLUSTER_FLAG;  //mod pangtianze [MultiUPS] [merge with paxos] 20170714:b
	  //add pangtianze [MultiUPS] [merge with paxos] 20170714:b
	  int32_t max_id = 0;
	  //add:e
      for(int32_t cid = 0; cid < OB_CLUSTER_ARRAY_LEN; cid++)
      {
