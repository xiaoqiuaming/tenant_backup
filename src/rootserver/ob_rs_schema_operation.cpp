  ...
  namespace rootserver
  {
    ...
    int ObRsSchemaOperation::generate_schema(const common::ObSchemaManagerV2 *schema_mgr)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (NULL != schema_manager_)
        {
          YYSYS_LOG(WARN, "schema_manager is already exist. schema_manager=%p", schema_manager_);
          ret = OB_ERROR;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == schema_manager_)
        {
          ...
          if (NULL == schema_manager_)
          {
            ...
            YYSYS_LOG(WARN, "fail to allocate memory for schemaManager.");
          }
          else
          {
            *schema_manager_ = *schema_mgr;
            YYSYS_LOG(INFO, "new schema version=%ld", now);
    ...
    const char* table_name, const uint64_t new_table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "INVALID ARGUMENT. schema_mgr=%p, table_name=%p, table_id=%lu",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "error happen. not table exist. table_name=%s", table_name);
          ret = OB_ERROR;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(TRACE, "new max table id is =%ld", new_table_id);
          schema_manager_->set_max_table_id(new_table_id);
        }
        ret = schema_manager_->change_table_id(old_table_id, new_table_id);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to change table id. ret=%d, old_table_id=%ld, new_table_id=%ld", ret, old_table_id, new_table_id);
        }
      }
      return ret;
    }
