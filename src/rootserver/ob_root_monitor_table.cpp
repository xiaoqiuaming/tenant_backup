...
static const char * cluster_id = "1_cluster_id";
//add:e
...
//add zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b
static const ObString CLUSTER_ID(static_cast<int32_t>(strlen(cluster_id)), static_cast<int32_t>(strlen(cluster_id)), cluster_id);
//add:e
...
int ObRootMonitorTable::get(const ObRowkey & rowkey, ObScanner & scanner)
{
  ...
  if (count <= 0)
  {
    YYSYS_LOG(WARN, "after init all servers:count[%ld]", count);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fill result failed:rowkey[%s], ret[%d]", to_cstring(rowkey), ret);
    }
    else if ((scanner.get_row_num() != row_count) || row_count <= 1)
    {
      ret = OB_INNER_STAT_ERROR;
      YYSYS_LOG(ERROR, "check fill row count error:result[%ld], fill[%ld]",
          scanner.get_row_num(), row_count);
    }
    else
    {
      YYSYS_LOG(DEBUG, "fill result succ:rowkey[%s], count[%ld]",
...
int ObRootMonitorTable::insert_all_meta_servers(ServerVector & servers) const
{
  ...
  //add zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b
  server.cluster_id = OB_MAX_CLUSTER_COUNT;
  //add:e
  int ret = servers.push_back(server);
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "add root server failed:ret[%d]", ret);
  }
  else
  {
    YYSYS_LOG(DEBUG, "add root server succ:vip[%s]", server.addr.to_cstring());
  }
  return ret;
}
...
int ObRootMonitorTable::insert_all_query_servers(ServerVector & servers) const
{
  ...
  if (cs_manager_ != NULL)
  {
    for (it = cs_manager_->begin(); it != cs_manager_->end(); ++it)
    {
      if (ObServerStatus::STATUS_DEAD != it->status_)
      {
        ...
        //add zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b
        server.cluster_id = static_cast<int32_t>(it->cluster_id_);
        //add:e
        ret = servers.push_back(server);
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "insert chunk server failed:ret[%d]", ret);
          break;
        }
        ++cs_count;
      }
      if (ObServerStatus::STATUS_DEAD != it->ms_status_)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "insert merge server failed:ret[%d]", ret);
          break;
        }
        ++ms_count;
      }
  ...
  YYSYS_LOG(DEBUG, "insert all chunk & merge server:ret[%d], cs[%ld], ms[%ld]",
...
int ObRootMonitorTable::insert_all_merge_servers(ServerVector & servers) const
{
  ...
  YYSYS_LOG(DEBUG, "insert all merge server:ret[%d], ms[%ld]",
      ret, ms_count);
  return ret;
}
...
int ObRootMonitorTable::insert_all_update_servers(ServerVector & servers) const
{
  ...
  if (ups_manager_ != NULL)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get ups list failed:ret[%d]", ret);
    }
    else
    {
  ...
  YYSYS_LOG(DEBUG, "insert all update server:ret[%d], count[%ld]", ret, count);
  return ret;
}
...
void ObRootMonitorTable::init_all_servers(ServerVector & servers, int64_t & count) const
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "insert all root server failed:ret[%d]", ret);
  }
  ret = insert_all_query_servers(servers);
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "insert all chunk server failed:ret[%d]", ret);
  }
  ret = insert_all_update_servers(servers);
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "insert all update server failed:ret[%d]", ret);
  }
  count = servers.size();
}
...
int ObRootMonitorTable::find_first_tablet(const ObRowkey & rowkey, const ServerVector & servers) const
{
  ...
  else
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "convert rowkey to role server failed:ret[%d]", err);
    }
    else
    {
      Compare comp;
    ...
    int64_t & row_count, ObScanner & result)
{
  ...
  {
    ret = OB_INNER_STAT_ERROR;
    YYSYS_LOG(WARN, "check find pos failed:pos[%d], size[%d]", pos, servers.size());
  }
  ...
  else if ((0 == pos) || (servers.size() == 1))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add first row failed:ret[%d]", ret);
    }
    else
    {
      ++row_count;
      YYSYS_LOG(TRACE, "add the first row succ:pos[%d], server[%s], count[%ld]",
  ...
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = index - 1; (i >= 0) && (i < servers.size() - 1) && (row_count < MAX_FILL_COUNT); ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add row failed:ret[%d], pos[%ld], count[%ld]", ret, i, row_count);
        break;
      }
      else
      {
        ++row_count;
        YYSYS_LOG(TRACE, "add normal row succ:pos[%ld], server[%s], count[%ld]",
  ...
  if ((OB_SUCCESS == ret) && (row_count < MAX_FILL_COUNT))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add last row failed:ret[%d]", ret);
    }
    else
    {
      ++row_count;
      YYSYS_LOG(TRACE, "add the last row succ:pos[%d], server[%s], count[%ld]",
...
int ObRootMonitorTable::add_normal_row(const int64_t index, const ServerVector & servers, ObScanner & result)
{
  ...
  if ((index < 0) || (index >= servers.size() - 1))
  {
    ...
    YYSYS_LOG(WARN, "check normal server index failed:index[%ld], count[%d]",
        index, servers.size());
  }
  else
  {
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "convert server to cell rowkey failed:ret[%d]", ret);
      }
    }
    if (OB_SUCCESS == ret)
    {
      ...
      ret = add_server_columns(server, cell, result);
      //mod:e
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add server columns for normal tablet failed:ret[%d]", ret);
      }
    }
  }
  return ret;
...
int ObRootMonitorTable::add_first_row(ObScanner & result)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "add fake row cell failed:ret[%d]", ret);
  }
  return ret;
}
...
int ObRootMonitorTable::add_server_columns(const ObClusterServer & server, ObCellInfo & cell, ObScanner & result)
//mod:e
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "add fake port cell failed:ret[%d]", ret);
  }
  else
  {
    // add 1_port column
    ...
    cell.value_.set_int(server.addr.get_port());
    //mod:e
    ret = result.add_cell(cell);
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add port cell failed:ret[%d]", ret);
    }
    else
    {
      // add 1_ipv2 column
      ...
      cell.value_.set_int(server.addr.get_ipv4());
      //mod:e
      ret = result.add_cell(cell);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add ip cell failed:ret[%d]", ret);
      }
      //add zhaoqiong [fixbug:tablet version info lost when query from ms] 20170316:b
      else
      {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "add fake version cell failed:ret[%d]", ret);
          }
      }
      //add:e
    }
    ...
    if (ret == OB_SUCCESS)
    {
      // add 1_cluster_id column
      cell.column_name_ = CLUSTER_ID;
      cell.value_.set_int(server.cluster_id);
      ret = result.add_cell(cell);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add cluster_id failed:ret[%d]", ret);
      }
      //add:e
    }
...
int ObRootMonitorTable::add_last_row(const ServerVector & servers, ObScanner & result)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check last server index failed:index[%ld]", index);
  }
  else
  {
    ObCellInfo cell;
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add server columns for last tablet failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObRootMonitorTable::convert_server_2_rowkey(const ObClusterServer & server, ObRowkey & rowkey)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "convert int ip to string host failed:server[%s], ret[%d]",
...
int ObRootMonitorTable::convert_rowkey_2_server(const ObRowkey & rowkey, ObClusterServer & server) const
{
  ...
  {
    YYSYS_LOG(WARN, "check rowkey failed:length[%ld], ptr[%p]", rowkey.length(), keys);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "convert role failed:ret[%d]", ret);
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "convert addr failed:ret[%d]", ret);
      }
    }
  }
  return ret;
...
int ObRootMonitorTable::convert_role(const ObObj & obj, ObRole & role) const
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get varchar type failed:ret[%d]", ret);
    }
    else
    {
      role = OB_INVALID;
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "check role error:role[%.*s]", str_role.length(), str_role.ptr());
      }
    }
  }
  return ret;
...
int ObRootMonitorTable::convert_addr(const ObServer & server, ObString & addr)
{
  ...
  {
    ret = OB_INNER_STAT_ERROR;
    YYSYS_LOG(WARN, "ip to string failed:server[%s]", server.to_cstring());
  }
  else
  {
...
int ObRootMonitorTable::convert_addr(const ObObj & ip, const ObObj & port, ObServer & server) const
{
  ...
  else if (!ip.is_min_value())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get varchar for ip failed:ret[%d]", ret);
    }
    else
    {
    ...
    if ((OB_SUCCESS == ret) && !port.is_min_value())
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get int for port failed:ret[%d]", ret);
      }
    }
    if (OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "set addr failed:addr[%s], port[%ld], ret[%d]",
            addr, nport, ret);
      }
    }
  }
...
void ObRootMonitorTable::print_info(const bool sort) const
{
  ...
  YYSYS_LOG(TRACE, "print servers in the cluster:count[%ld], sort[%d]", count, sort);
  for (int32_t i = 0; i < count; ++i)
  {
    YYSYS_LOG(INFO, "index:%d, role:%d, server:%s",
