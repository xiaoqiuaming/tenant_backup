  ...
  namespace lsync
  {
    ...
    static int seek_within_one_file_(ObSingleLogReader& reader, uint64_t last_log_seq_id)
    {
      ...
      while(seq < last_log_seq_id)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "seek_within_on_file:read_log()=>%d", ret);
          break;
        }
      }
    ...
    static int seek_may_need_retry(ObSingleLogReader& reader, uint64_t last_log_file_id, uint64_t last_log_seq_id)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "close() failed");
      }
      else if (OB_SUCCESS != (ret = reader.open(last_log_file_id, 0)))
      {
        YYSYS_LOG(DEBUG, "open log failed: log_file_id = %ld, err=%d", last_log_file_id, ret);
      }
      else if (OB_SUCCESS != (ret = seek_within_one_file_(reader, last_log_seq_id)))
      {
        YYSYS_LOG(DEBUG, "seek within one file failed: log_seq_id = %ld", last_log_seq_id);
      }
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "seek_(log_file_id=%lu, log_seq_id=%lu)", last_log_file_id, last_log_seq_id);
    ...
    int ObSeekableLogReader:: seek(uint64_t last_log_file_id, uint64_t last_log_seq_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "seek() invalid argument: last_log_file_id=%ld", last_log_file_id);
      }
      ...
      {
        YYSYS_LOG(DEBUG, "seek() failed: file_id=%ld, log_id=%ld, ret=%d", last_log_file_id, last_log_seq_id, ret);
      }
      if (OB_SUCCESS == ret)
      {
    ...
    int ObSeekableLogReader:: get_next(LogCommand& cmd, uint64_t& seq, char*& log_data, int64_t& data_len)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "get_next:reader_.is_opened() == false");
      }
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(INFO, "switch log: last_log_file_id = %ld", last_log_file_id_);
            last_log_file_id_ = 0;
          }
          else
          {
 ...
 */
                                  ...
                                  LogCommand& cmd, uint64_t& seq, char*& log_data, int64_t& data_len)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "seek(log_file_id=%ld log_seq_id=%ld)=>%d", log_file_id, log_seq_id, ret);
      }
      else if (OB_SUCCESS != (ret = get_next(cmd, seq, log_data, data_len)))
      {
        YYSYS_LOG(DEBUG, "get_log_entries_buf(log_file_id=%ld, log_seq_id=%ld)=>%d",
                                 ...
                                 LogCommand& cmd, uint64_t& seq, char*& log_data, int64_t& data_len)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "last_log.get(log_file_id=%ld log_seq_id=%ld)=>%d", log_file_id, log_seq_id, ret);
      }
      else if (OB_SUCCESS != (ret = get_(log_file_id, log_seq_id, cmd, seq, log_data, data_len)))
      {
        YYSYS_LOG(DEBUG, "get_(log_file_id=%ld log_seq_id=%ld)=>%d", log_file_id, log_seq_id, ret);
      }
      else
      {
                                 ...
                                 LogCommand& cmd, uint64_t& seq, char*& log_data, int64_t& data_len, int64_t timeout)
    {
      ...
      while(!stop_)
      {
        ...
        if (OB_READ_NOTHING == ret && (yyutil::Time::now() - now).toMicroSeconds() < timeout)
        {
          usleep(10000);
          continue;
        }
                                                ...
                                                char* buf, int64_t limit, int64_t& pos, int64_t timeout)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "get_log_entry(log_file_id=%ld, log_seq_id=%ld)=>%d",
                  log_file_id, log_seq_id, ret);
      }
      else if (OB_SUCCESS != (ret = log_entry.serialize(buf, limit, pos)))
      {
        YYSYS_LOG(DEBUG, "log_entry.serialize(buf=%p, limit=%ld, pos=%ld)=>%d",
                  buf, limit, pos, ret);
      }
      else if (OB_SUCCESS != (ret = mem_chunk_serialize(buf, limit, pos, log_data, log_entry.get_log_data_len())))
      {
        YYSYS_LOG(DEBUG, "copy_log_data(buf=%p,limit=%ld,pos=%ld,data_len=%d)=>%d",
    ...
    int ObSeekableLogReader:: get_log_entry(uint64_t& log_file_id, uint64_t& log_seq_id, ObLogEntry& log_entry, char*& log_data, int64_t timeout)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "get_log_entry(log_file_id=%lu, log_seq_id=%lu)=>%d",
                  log_file_id, log_seq_id, ret);
      }
      else
      {
