...
namespace oceanbase
{
  ...
  namespace lsync
  {
    ...
    int ObSeekableLogReader::init(const char* log_dir, const bool is_read_old_log, const int64_t lsync_retry_wait_time_us)
    {
      ...
      {
        YYSYS_LOG(ERROR, "located_log_reader.init(log_dir=%s)=>%d", log_dir, err);
      }
      else
      {
        log_dir_ = log_dir;
    ...
    int get_log_file_offset_func(const char* log_dir, const int64_t file_id, const int64_t log_id, int64_t& offset, bool is_read_old_log)
    {
      ...
      {
        YYSYS_LOG(ERROR, "log_reader.init(log_dir=%s)=>%d", log_dir, err);
      }
      else if (OB_SUCCESS != (err = log_reader.open(file_id)))
      {
        if (OB_FILE_NOT_EXIST != err)
        {
          YYSYS_LOG(ERROR, "log_reader.open(file_id=%ld)=>%d", file_id, err);
        }
        else
        {
                        ...
                        int64_t& start_id, int64_t& end_id, bool& is_file_end, bool is_old_log)
    {
      ...
      {
        YYSYS_LOG(WARN, "offset[%ld] is not aligned[bits=%ld]", offset, align_bits);
      }
      while (OB_SUCCESS == err && pos <= len)
      {
        ...
        {
          YYSYS_LOG(WARN, "log_entry.check_data_integrity()=>%d", err);
        }
        else if (is_old_log)
        {
      ...
      {
        YYSYS_LOG(WARN, "parse log buf error:");
        hex_dump(log_data, static_cast<int32_t>(len), true, YYSYS_LOG_LEVEL_DEBUG);
      }
      ...
      YYSYS_LOG(DEBUG, "trim_log_buffer(offset=%ld, align_bits=%ld, pos=%ld, aligned_pos=%ld, log=[%ld,%ld])=>%d",
    ...
    int ObSeekableLogReader::get_location(const int64_t file_id, const int64_t start_id, ObLogLocation& start_location)
    {
      ...
      else if (start_location.is_valid() && start_location.file_id_ == file_id && start_location.log_id_ == start_id)
      {
        ...
        //YYSYS_LOG(DEBUG, "start_location hit: file_id=%ld, log_id=%ld, offset=%ld", start_location.file_id_, start_location.log_id_, start_location.offset_);
      }
      ...
      {
        YYSYS_LOG(ERROR, "get_log_file_offset_func(log_dir=%s, file_id=%ld, start_id=%ld)=>%d", log_dir_, file_id, start_id, err);
      }
      else if (OB_ENTRY_NOT_EXIST == err)
      {
        YYSYS_LOG(DEBUG, "get_location(file_id=%ld, log_id=%ld): OB_ENTRY_NOT_EXIST", file_id, start_id);
      }
      if (OB_SUCCESS == err)
      {
      ...
      YYSYS_LOG(DEBUG, "get_location(file_id=%ld, start_id=%ld, start_location=[%ld+%ld:%ld])=>%d",
    ...
    int ObSeekableLogReader::get_log(const int64_t file_id, const int64_t start_id, ObLogLocation& start_location, ObLogLocation& end_location, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      else if (OB_SUCCESS != (err = get_location(file_id, start_id, start_location)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "get_location(start_id=%ld)=>%d", start_location.log_id_, err);
        }
        else
        {
          err = OB_READ_NOTHING;
      ...
      {
        YYSYS_LOG(ERROR, "read_log_file_by_location(log_dir=%s, file=%ld:+%ld, start_id=%ld, buf=%p[%ld])=>%d",
      ...
      {
        YYSYS_LOG(WARN, "trim_log_buffer()=>%d", err);
      }
      else if (0 >= read_count)
      {
        err = OB_READ_NOTHING;
        YYSYS_LOG(DEBUG, "located_log_reader.get_log(file_id=%ld, log_id=%ld, offset=%ld): READ_NOTHING",
                  start_location.file_id_, start_location.log_id_, start_location.offset_);
      }
      else if (start_id != 0 && real_start_id != start_id)
      {
        err = OB_DISCONTINUOUS_LOG;
        YYSYS_LOG(ERROR, "log_not_continuous: real_start_id[%ld] != start_id[%ld]", real_start_id, start_id);
      }
      else if (is_file_end)
      {
      ...
      {
        YYSYS_LOG(WARN, "get_log(start_id=%ld): err=%d, reset to OB_READ_NOTHING", start_id, err);
        err = OB_READ_NOTHING;
      }
      YYSYS_LOG(DEBUG, "get_log(file_id=%ld, start_id=%ld, offset=%ld, read_count=%ld)=>%d",
    ...
    int ObSeekableLogReader::get_log(const int64_t file_id, const int64_t start_id, char* buf, const int64_t len, int64_t& read_count, const int64_t timeout_us)
    {
      ...
      YYSYS_LOG(DEBUG, "get_log(timeout_us=%ld)", timeout_us);
      ...
      while(!stop_ && OB_READ_NOTHING == err)
      {
        ...
        if (OB_SUCCESS != err && OB_READ_NOTHING != err)
        {
          YYSYS_LOG(ERROR, "get_log(file_id=%ld, log_id=%ld)=>%d", file_id, start_id + file_id -1, err);
        }
        else if (OB_READ_NOTHING == err && yysys::CTimeUtil::getTime() + reserved_time_to_send_packet_ + lsync_retry_wait_time_us_ < start_us + timeout_us)
        {
      ...
      {
        YYSYS_LOG(ERROR, "conver_log_seq(file_id=%ld, buf=%p[%ld])=>%d", file_id, buf, read_count, err);
      }
      else
      {
      ...
      {
        YYSYS_LOG(WARN, "last_end_id[%ld] != start_id[%ld] + file_id[%ld]", last_end_id, start_id, file_id);
      }
