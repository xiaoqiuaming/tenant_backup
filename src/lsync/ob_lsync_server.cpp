...
namespace oceanbase
{
  ...
  namespace lsync
  {
    ...
    static int get_thread_buffer(ThreadSpecificBuffer& thread_buffer, ObDataBuffer& data_buff)
    {
      ...
      if (NULL == my_buffer)
      {
        YYSYS_LOG(ERROR, "get thread buff failed:buffer[%p].", my_buffer);
        err = OB_ERROR;
      }
      else
      {
    ...
    static int new_data_buffer(ObDataBuffer& buf, int64_t len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "new_data_buffer: new char[%ld]=>NULL", len);
      }
      else
      {
    ...
    int ObLsyncServer:: get_log(ObFetchLogRequest& req, char* buf, int64_t limit, int64_t& pos, int64_t timeout) //TODO:
    {
      ...
      YYSYS_LOG(DEBUG, "get_log(log_file_id=%ld, log_seq_id=%ld)", log_file_id, log_seq_id);
      if (OB_SUCCESS != (err = reader_.get_log(log_file_id, log_seq_id, buf + pos, limit - pos, read_count, timeout))
          && OB_READ_NOTHING != err)
      {
        YYSYS_LOG(ERROR, "get_log(file_id=%ld, seq_id=%ld)=>%d", log_file_id, log_seq_id, err);
      }
      else if (OB_READ_NOTHING == err)
      {
        err = OB_SUCCESS;
        YYSYS_LOG(DEBUG, "read_nothing, reset errno to OB_SUCCESS.");
      }
      else
      {
        pos += read_count;
    ...
    int ObLsyncServer::initialize(const char* log_dir, uint64_t log_start, const char* dev, int port, int64_t timeout, int64_t convert_switch_log, int64_t lsync_retry_wait_time_us)
    {
      ...
      {
        YYSYS_LOG(ERROR, "new_data_buffer(log_buffer, len=%ld)=>%d", OB_MAX_LOG_BUFFER_SIZE, err); 
     }
      else if (OB_SUCCESS != (err = reader_.init(log_dir, 0 == convert_switch_log, lsync_retry_wait_time_us)))
      {
        YYSYS_LOG(ERROR, "reader_.initialize(log_dir='%s', retry_wait_time_us=%ld)=>%d", log_dir, lsync_retry_wait_time_us, err);
      }
                                            ...
                                            ObDataBuffer* buf)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "RegisterPacket deserialize failed: %d", ret);
        }
      }
      YYSYS_LOG(DEBUG, "SlaveRegisterRequest{file_id: %ld, log_id: %ld, slave: %s}",
      ...
      {
        YYSYS_LOG(DEBUG, "ups_slave_register:send_packet()=>%d", ret);
      }
      return ret;
    }
                                ...
                                int packet_code, ObDataBuffer* buf, int64_t timeout)
    {
      ...
      {
        YYSYS_LOG(WARN, "receive reg packet when expected FETCH_LOG.");
        ret = OB_NEED_RETRY;
      }
      else if (packet_code != OB_LSYNC_FETCH_LOG)
      {
        YYSYS_LOG(WARN, "unexpected packet: %d", packet_code);
        ret = OB_ERR_UNEXPECTED;
      }
      else if(!is_registered(convert_addr_to_server(request->ms->c->addr)))
      {
        YYSYS_LOG(WARN, "not registered: %s", get_peer_ip(request));
      ...
      {
        YYSYS_LOG(WARN, "ObRequestLogPacket.deserialize() error: %d.", ret);
      }
      else
      {
        ret = send_log_(req, request, channel_id, timeout);
        if(OB_SUCCESS != ret && OB_READ_NOTHING != ret)
          YYSYS_LOG(WARN, "send_log:send_packet=>%d", ret);
      }
      return ret;
    }
    ...
    int ObLsyncServer::send_log_(ObFetchLogRequest& req, easy_request_t* request, const uint32_t channel_id, int64_t timeout)
    {
      ...
      else
      {
        if (OB_SUCCESS != ret) // 其他的错误重设为OB_NEED_RETRY
        {
          ...
          {
            ret = OB_NEED_RETRY;
            YYSYS_LOG(ERROR, "get_log(file_id=%ld, seq_id=%ld)=>%d", req.log_file_id, req.log_seq_id, ret);
          }
        }
        ObFetchLogResponse response(ret, log_buffer_);
    ...
    //yynet::IPacketHandler::HPRetCode ObLsyncServer::handlePacket(yynet::Connection *conn, yynet::Packet *packet)
    //{
    //  yynet::IPacketHandler::HPRetCode rc = yynet::IPacketHandler::KEEP_CHANNEL;
    //  int err = OB_SUCCESS;
    //
    //  if (!packet->isRegularPacket()) // Maybe caused by `Timeout', so reset state => WAIT_REGISTER
    //  {
    //    rc = yynet::IPacketHandler::FREE_CHANNEL;
    //    state_ = WAIT_REGISTER;
    //    err = OB_ERR_UNEXPECTED;
    //    YYSYS_LOG(WARN, "control packet, packet code: %d", ((yynet::ControlPacket*)packet)->getCommand());
    //  }
    //
    //  if (OB_SUCCESS == err)
    //  {
    ...
    //    {
    //      YYSYS_LOG(WARN, "packet deserialize error: packet_code=%d, err=%d", req->get_packet_code(), err);
    //    }
    //    else if (OB_SUCCESS != (err = (req->get_api_version() == MY_VERSION)? OB_SUCCESS: OB_ERROR_FUNC_VERSION))
    //    {
    //      YYSYS_LOG(ERROR, "api version mismatch: request version %d, support version %d", req->get_api_version(), MY_VERSION);
    //    }
    //    else if (OB_SUCCESS != (err = handleRequest(conn, req->getChannelId(), req->get_packet_code(), req->get_buffer(),
    //                                                req->get_source_timeout())))
    //    {
    //      YYSYS_LOG(DEBUG, "handleRequest(packet_code=%d)=>%d", req->get_packet_code(), err);
    //    }
    //  }
    //  return rc;
    //}
    ...
    int ObLsyncServer::handlePacket(ObPacket *packet)
    {
      ...
      if (0 >= (remain_time = req->get_receive_ts() + req->get_source_timeout() - yysys::CTimeUtil::getTime()))
      {
        YYSYS_LOG(WARN, "packet wait too long time: receive[%ld] + timeout[%ld] < now[%ld]",
                  req->get_receive_ts(), req->get_source_timeout(), yysys::CTimeUtil::getTime());
      }
    ...
    int ObLsyncServer::handleRequestMayNeedRetry(easy_request_t* req, const uint32_t channel_id, int packet_code, ObDataBuffer* buf, int64_t timeout)
    {
      ...
      case WAIT_REGISTER:
        ...
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(INFO, "slave register ok:%s", inet_ntoa_r((conn->addr)));
          state_ = WAIT_LOG_REQ;
        }
        else
        {
          YYSYS_LOG(WARN, "slave register failed.");
          state_ = WAIT_REGISTER;
        }
        break;
      case WAIT_LOG_REQ:
      ...
      case STOP:
      default:
        YYSYS_LOG(ERROR, "unexpected state: %d", state_);
        err = OB_ERR_UNEXPECTED;
        break;
      }
                                   ...
                                   easy_request_t* req, const uint32_t channel_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "send_response()=>%d", err);
      }
      else
      {
        YYSYS_LOG(DEBUG, "send response succ size=%ld src=[%s]", out_buff.get_position(), inet_ntoa_r(conn->addr));
      }
      return err;
    }
