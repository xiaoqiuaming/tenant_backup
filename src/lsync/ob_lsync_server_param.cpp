...
#define get_lsync_config(name) YYSYS_CONFIG.getString(LSYNC_SECTION, name##_KEY, NULL)
#define merge_lsync_config(other, name) if(other.name##_is_set()) set_##name(other.get_##name())
    ...
    int ObLsyncServerParam::load_from_file(const char* config_file)
    {
      int err = OB_SUCCESS;
      const char* val = NULL;
      if(OB_SUCCESS != (err = YYSYS_CONFIG.load(config_file)))
      {
        YYSYS_LOG(WARN, "YYSYS_CONFIG.load('%s')=>%d", config_file, err);
      }
      else
      {
        if((val = get_lsync_config(DEV_NAME)))
          set_dev_name(_strncpy(dev_name_buf_, val, static_cast<int>(ARRAYSIZEOF(dev_name_buf_))));
        ...
        if((val = get_lsync_config(COMMIT_LOG_DIR)))
          set_commit_log_dir(_strncpy(commit_log_dir_buf_, val, static_cast<int>(ARRAYSIZEOF(commit_log_dir_buf_))));
    ...
    {
      YYSYS_LOG(INFO, "%s: ObLsyncServerParam{ log_dir: '%s', convert_switch_log: %d, log_file_start: %ld, timeout: %ld, dev: '%s', port: %d, lsync_retry_wait_time_us: %ld}\n",
