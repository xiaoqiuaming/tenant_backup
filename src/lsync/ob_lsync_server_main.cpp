...
namespace oceanbase
{
  namespace lsync
  {
    ...
    // {
    //   if (level < YYSYS_LOG_LEVEL_ERROR)
    //     level = YYSYS_LOG_LEVEL_ERROR;
    //   if (level > YYSYS_LOG_LEVEL_DEBUG)
    //     level = YYSYS_LOG_LEVEL_DEBUG;
    //   YYSYS_LOGGER._level = level;
    // }
    ...
    int ObLsyncServerMain::do_work()
    {
      ...
      {
        YYSYS_LOG(WARN, "load_from_file('%s')=>%d", config_, err);
      }
      else
      {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        err = server_.initialize(param_.get_commit_log_dir(),
                                 param_.get_log_file_start_id(),
                                 param_.get_dev_name(),
                                 param_.get_port(),
                                 param_.get_timeout(),
                                 param_.get_convert_switch_log(),
                                 param_.get_lsync_retry_wait_time_us());
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "ObLsyncServer.initialize()=>%d", err);
        }
      }
      if (OB_SUCCESS == err)
      {
