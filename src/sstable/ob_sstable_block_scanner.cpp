...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObSSTableBlockScanner::store_and_advance_column()
    {
      ...
      {
        YYSYS_LOG(ERROR, "get column index error, ret = %d, cursor=%ld, id= %lu, index=%d", 
            ret, column_cursor_, column.id_, column.index_);
      }
      else if (OB_SUCCESS != (ret = store_current_cell(column)))
      {
        YYSYS_LOG(ERROR, "store current cell error, ret = %d, cursor=%ld, type=%d, index=%d, id=%lu", 
    ...
    int ObSSTableBlockScanner::next_cell()
    {
      ...
      {
        YYSYS_LOG(ERROR, "not initialized, cursor=%p, start=%p, last=%p",
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "load current row error, ret=%d , cursor=%d,%d",
        ...
        const int64_t cursor, ObScanColumnIndexes::Column& column) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "query_column_indexes_ cursor=%ld not exist.", cursor);
      }
    ...
    int ObSSTableBlockScanner::load_current_row(const_iterator row_index)
    {
      ...
      {
        YYSYS_LOG(ERROR, "internal error, current row cursor=%p < start=%p, >last=%p.",
      ...
      {
        YYSYS_LOG(ERROR, "read current row error, store style=%ld, current row cursor=%d,%d",
    ...
    int ObSSTableBlockScanner::store_sparse_column(const ObScanColumnIndexes::Column &column)
    {
      ...
      if (0 == column_cursor_ && !handled_del_row_)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get column id from obj");
        }
      }
    ...
    int ObSSTableBlockScanner::store_dense_column(const ObScanColumnIndexes::Column & column)
    {
      ...
      {
          YYSYS_LOG(ERROR, "column_index=%d > current_column_count_=%ld", 
      ...
      YYSYS_LOG(DEBUG, "store dense column:(type:%d, index:%d, value:%s)", 
        ...
        bool not_exit_col_ret_nop) 
    {
      ...
      {
        YYSYS_LOG(ERROR, "block_data,desc invalid, bd=(%p,%ld, %p, %ld), desc=(%p,%ld,%ld) ",
      ...
      {
        YYSYS_LOG(ERROR, "deserialize error, ret=%d, data bufsiz=%ld", 
            ret, block_data.data_bufsiz_);
      }
      else if (OB_SUCCESS != (ret = 
            locate_start_pos(range, start_iterator, need_looking_forward)))
      {
        YYSYS_LOG(DEBUG, "locate start key out of range or error=%d", ret);
      }
      else if (OB_SUCCESS != (ret =
            locate_end_pos(range, last_iterator, need_looking_forward)))
      {
        YYSYS_LOG(DEBUG, "locate end key out of range or error=%d", ret);
      }
      else  if (start_iterator > last_iterator)
      {
        YYSYS_LOG(DEBUG, "query key not exist in this block, start_iterator > last_iterator."
      ...
      if (OB_SUCCESS != ret && OB_BEYOND_THE_RANGE != ret) 
      {
        ...
        YYSYS_LOG(ERROR, "set_scan_param error, ret = %d, cursor=%p,%p,%p, range=%s", 
        ...
        const_iterator& start_iterator, bool& need_looking_forward)
    {
      ...
      else
      {
        ...
        if (start_iterator == reader_.end())
        {
          ...
          YYSYS_LOG(DEBUG, "query start_key not in block's range.");
          ret = OB_BEYOND_THE_RANGE;
        ...
        {
          YYSYS_LOG(ERROR, "get start key of block error, start_iterator=%d,%d",
        ...
        else
        {
          ...
          if (start_iterator == reader_.end())
          {
            YYSYS_LOG(DEBUG, "query start_key equal last key in block, but query"
        ...
        const_iterator& last_iterator, bool& need_looking_forward)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "get last key of block error, last_iterator=%d,%d",
