...
{ 
  namespace sstable 
  {
    ...
    int ObAIOEventMgr::init(io_context_t ctx)
    {
      ...
      if (NULL == ctx)
      {
        YYSYS_LOG(WARN, "invalid parameter, ctx=%p", ctx);
        ret = OB_ERROR; 
      }
      else
      {
                                  ...
                                  const int64_t size, ObAIOBufferInterface& aio_buf)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "aio event manager doesn't init");
        ret = OB_ERROR;
      }
      else if (fd < 0 || offset < 0 || size <= 0)
      {
        YYSYS_LOG(WARN, "Invalid parameter, fd=%d, offset=%ld, size=%ld",
                  fd, offset, size);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (1 != ret)
        {
          YYSYS_LOG(WARN, "io_submit failed, ret=%d, error: %s", 
                    ret, strerror(-ret));
          ret = OB_ERROR;
        }
        else
    ...
    int ObAIOEventMgr::aio_wait(int64_t& timeout_us)
    {
      ...
      else
      {
        while (OB_CS_EAGAIN == inner_ret)
        {
          ...
          if (0 == event_nr)
          {
            YYSYS_LOG(WARN, "AIO read timeout, event_nr=%ld, timeout_us=%ld", 
                      event_nr, timeout_us);
            timeout_us = 0;
            ret = OB_AIO_TIMEOUT;
            break;
          ...
          else
          {
            for (int64_t i = 0; i < event_nr; ++i)
            {
              ...
              {
                YYSYS_LOG(WARN, "aio return invalid result, ret_code=%ld, error=%s", 
            ...
            {
              cur_timeout_us = start_time + timeout_us - yysys::CTimeUtil::getTime();
              if (OB_SUCCESS == ret)
              {
