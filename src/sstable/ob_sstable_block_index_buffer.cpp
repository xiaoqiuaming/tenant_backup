...
namespace oceanbase
{
  namespace sstable
  {
     ...
     DEFINE_SERIALIZE(ObSSTableBlockIndexItem)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie block index item, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObSSTableBlockIndexItem)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie block index item, buf=%p, "
    ...
    int ObSSTableBlockIndexBuffer::clear()
    {
      ...
      while (NULL != block_head_)
      {
        ...
        if (OB_ERROR == ret)
        {
          YYSYS_LOG(WARN, "failed to free mem, ptr=%p", block_head_);
        }
        block_head_ = tmp;
      }
      block_head_ = NULL;
    ...
    int ObSSTableBlockIndexBuffer::reset()
    {
      ...
      if (NULL != block_head_)
      {
        ...
        while (NULL != block_head_->next_)
        {
          ...
          if (OB_ERROR == ret)
          {
            YYSYS_LOG(WARN, "failed to free mem, ptr=%p", tmp);
          }
        }
        block_tail_ = block_head_;
        block_head_->next_ = NULL;
        block_head_->cur_pos_ = 0;
        block_head_->block_size_ = static_cast<int32_t>(DEFAULT_MEM_BLOCK_SIZE - sizeof(MemBlock));
    ...
    int ObSSTableBlockIndexBuffer::add_key(const ObRowkey& key)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument key_len=%ld, key_ptr=%p", 
      ...
      if (OB_SUCCESS == ret)
      {
            ...
            (NULL != block_tail_ && block_tail_->block_size_ - block_tail_->cur_pos_ <= rowkey_len))
        {
          ...
          if (OB_ERROR == ret)
          {
            YYSYS_LOG(WARN, "failed to alloc mem block ret=%d",ret);
          }
        }
    ...
    int ObSSTableBlockIndexBuffer::add_key(const ObString& key)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument key_len=%d, key_ptr=%p", key.length(), key.ptr());
        ret = OB_ERROR;
      }
    ...
    int ObSSTableBlockIndexBuffer::add_index_item(const ObSSTableBlockIndexItem& item)
    {
      ...
      if (0 >= serialize_size)
      {
        YYSYS_LOG(WARN, "invalid argument item seriallize_size=%ld", serialize_size);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          else
          {
            ...
            if (OB_ERROR == ret)
            {
              YYSYS_LOG(WARN, "failed to serialize item to mem block,");
            }
    ...
    int ObSSTableBlockIndexBuffer::get_data(char*& ptr, const int64_t ptr_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument ptr=%p, buf_size=%ld, data_size_=%ld", ptr, ptr_size, data_size_);
        ret = OB_ERROR;
      }
      int64_t pos = 0;
    ...
    int ObSSTableBlockIndexBuffer::alloc_block()
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to alloc mem size=%ld, ret=%d", DEFAULT_MEM_BLOCK_SIZE, ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        new_block->block_size_ = static_cast<int32_t>(DEFAULT_MEM_BLOCK_SIZE - sizeof(MemBlock));
        if (NULL == block_tail_)
        {
    ...
    int ObSSTableBlockIndexBuffer::alloc_mem(const int64_t size, void*& ptr)
    {
      ...
      if (0 >= size)
      {
        YYSYS_LOG(WARN, "invalid argument size=%ld",size);
      ...
      else
      {
        ...
        if (NULL == ptr)
        {
          YYSYS_LOG(WARN, "failed to alloc memory, size=%ld", size);
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObSSTableBlockIndexBuffer::free_mem(void* ptr)
    {
      ...
      if (NULL == ptr)
      {
        YYSYS_LOG(WARN, "invalid argument ptr=%p", ptr);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
