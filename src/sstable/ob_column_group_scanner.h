  ...
  namespace sstable
  {
    ...
    public:
      ...
      inline int get_cell(common::ObCellInfo** cell, bool* is_row_changed)
      {
        ...
        if (NULL == cell)
        {
          YYSYS_LOG(ERROR, "invalid arguments, cell=%p.", cell);
          ret = common::OB_INVALID_ARGUMENT;
        }
        else if (ITERATE_NOT_START == iterate_status_) 
        {
          YYSYS_LOG(ERROR, "iterate not start.");
          ret = common::OB_NOT_INIT;
        }
        else if (ITERATE_IN_ERROR == iterate_status_)
        {
          YYSYS_LOG(ERROR, "iterate in error.");
          ret = common::OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "block scanner get cell failed, ret=%d", ret);
          }
        }
