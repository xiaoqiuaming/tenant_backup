...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObSSTableReader::ObFileInfo::init(const char* sstable_fname, const bool use_dio)
    {
      ...
      {
        YYSYS_LOG(WARN, "file name is NULL");
        ret = OB_ERROR;
      }
      else if (!use_dio && -1 == (fd_ = ::open(sstable_fname, FILE_OPEN_NORMAL_RFLAG)))
      {
        YYSYS_LOG(WARN, "normal open sstable file fail errno=%u, error_str=%s, "
                        "filename=%s",
      ...
      {
        YYSYS_LOG(WARN, "direct open sstable file fail errno=%u, error_str=%s, "
                        "filename=%s",
                  errno, strerror(errno), sstable_fname);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(INFO, "open file succ fd=%d, sstable_filename=%s",
                  fd_, sstable_fname);
      }
    ...
    ObSSTableReader::ObSSTableReader(common::ModuleArena &arena,
                                     ...
                                     yysys::CThreadMutex* external_arena_mutex)
    ...
    int ObSSTableReader::open(const ObSSTableId& sstable_id, const int64_t version)
    {
      ...
      if (is_opened_)
      {
        YYSYS_LOG(WARN, "SSTableReader already opened");
        ret = OB_INIT_TWICE;
      }
      else if (OB_INVALID_ID == sstable_id.sstable_file_id_
               || sstable_id.sstable_file_offset_ < 0)
      {
        YYSYS_LOG(WARN, "open sstable file error. sstable_id=%lu, offset pos(%ld) < 0",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == file_info)
        {
          YYSYS_LOG(WARN, "get file info fail sstable_id=%lu",
                                            ...
                                            int64_t& sstable_size)
    {
      ...
      if (sstable_size < 0)
      {
        YYSYS_LOG(ERROR, "cannot get sstable size, sstable_size=%ld, fd=%d",
                                     ...
                                     const char*& out_buffer)
    {
      ...
      if (NULL == file_buf)
      {
        YYSYS_LOG(WARN, "get thread file read buffer failed, file_buf=NULL");
        ret = OB_ERROR;
      }
      else
      {
        ...
        const int64_t trailer_size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "trailer_offset:%ld, trailer_size:%ld is illegal",
            trailer_offset, trailer_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid trailer buffer size, trailer_buf=%p, trailer_buf_size=%ld,"
                          "trailer_size=%ld",
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            else
            {
              ...
              YYSYS_LOG(ERROR, "Trailer is compressed, don't know how to uncompress it");
              ret = OB_ERROR;
            }
          }
        }
    ...
    int ObSSTableReader::load_bloom_filter(const IFileInfo& file_info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "filter_offset:%ld, filter_size:%ld is illegal",
      ...
      {
        YYSYS_LOG(WARN, "cannot create bloom filter.");
        ...
        if (OB_SUCCESS == ret && NULL != record_buf)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            else
            {
              ...
              YYSYS_LOG(ERROR, "bloom filter is in compressed format, "
    ...
    int ObSSTableReader::load_schema(const IFileInfo& file_info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "schema_offset:%ld, schema_size:%ld, version:%ld is illegal",
      ...
      else
      {
        if (NULL == (schema_ = schema_cache.get_schema(first_table_id, version)))
        {
          ...
          if (NULL == schema_buf)
          {
            YYSYS_LOG(ERROR,"alloc schema buf error");
            ret = OB_ALLOCATE_MEMORY_FAILED;
          }
          ...
          if (OB_SUCCESS == ret && NULL != schema_buf)
          {
            ...
            if (NULL == schema_)
            {
              YYSYS_LOG(WARN,"new sstable schema instance error");
              ret = OB_ERROR;
            }
          }
          ...
          if (OB_SUCCESS == ret && NULL != record_buf)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              {
                // compressed schema
                YYSYS_LOG(ERROR, "schema is in compressed format, "
                                 "but expect uncompressed format");
                ret = OB_ERROR;
              }
            }
          ...
          if (OB_SUCCESS == ret && NULL != schema_)
          {
            ...
            if (OB_ENTRY_EXIST == ret)
            {
              ...
              YYSYS_LOG(INFO, "schema exist with table id =%ld, version=%ld, and refetch from cache:%p",
                  first_table_id, version, schema_);
              if (NULL == schema_)
              {
                YYSYS_LOG(WARN, "get schema from sstable schema cache failed, version=%ld",
                          version);
                ret = OB_ERROR;
              }
              else
            ...
            else if (OB_SUCCESS != ret)
            {
              ...
              YYSYS_LOG(WARN, "add schema to sstable schema cache failed, version=%ld",
                        version);
            }
          }
        }
        ...
        common::ObNewRange& range, common::ObObj* start_key_obj_array, common::ObObj* end_key_obj_array)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get_global_schema_rowkey_info, ret=%d", ret);
      }
      else if (OB_SUCCESS !=
          (ret = serialization::decode_vi64(payload_ptr, payload_size,
                                            pos, reinterpret_cast<int64_t*>(&table_id))))
      {
        YYSYS_LOG(ERROR, "failed to decode table id, payload_ptr=%p, payload_size=%ld, pos=%ld,"
            " ret=%d", payload_ptr, payload_size, pos, ret);
      }
      else if (OB_SUCCESS !=
          (ret = serialization::decode_i8(payload_ptr, payload_size, pos, &flag)))
      {
        YYSYS_LOG(ERROR, "failed to decode flag, payload_ptr=%p, payload_size=%ld, pos=%ld,"
            " ret=%d", payload_ptr, payload_size, pos, ret);
      }
      else if (OB_SUCCESS !=
          (ret = start_key.deserialize(payload_ptr, payload_size, pos)))
      {
        YYSYS_LOG(ERROR, "failed to deserialize start key, payload_ptr=%p, payload_size=%ld, pos=%ld,"
            " ret=%d", payload_ptr, payload_size, pos, ret);
      }
      else if (OB_SUCCESS !=
          (ret = end_key.deserialize(payload_ptr, payload_size, pos)))
      {
        YYSYS_LOG(ERROR, "failed to deserialize end key, payload_ptr=%p, payload_size=%ld, pos=%ld,"
            " ret=%d", payload_ptr, payload_size, pos, ret);
      }
      else
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to trans start binary rowkey to obj array, ret=%d", ret);
          }
          else
          {
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to trans end binary rowkey to obj array, ret=%d", ret);
          }
          else
          {
    ...
    int ObSSTableReader::load_range(const IFileInfo& file_info)
    {
      ...
      { // compat for SSTABLEV2
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to set range of trailer, ret=%d", ret);
        }
      }
        ...
        if (OB_SUCCESS == ret && NULL != record_buf)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            if (header.is_compress())
            {
              ...
              YYSYS_LOG(ERROR, "range buf is in compressed format, "
                        "but expect uncompressed format");
              ret = OB_ERROR;
            }
            else
            {
              ...
              { // compat for SSTABLEV2
                ...
                if (OB_SUCCESS != ret)
                {
                  YYSYS_LOG(ERROR, "failed to deserialize range of sstable v2: payload_ptr %p,"
              ...
              if (OB_SUCCESS == ret)
              {
                ...
                if (OB_SUCCESS != ret)
                {
                  YYSYS_LOG(ERROR, "failed to deep copy range,ret=%d", ret);
                }
                else
                {
          ...
          {
            YYSYS_LOG(ERROR, "failed to check record");
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "failed to read_record of range");
        }
      }
    ...
    int ObSSTableReader::serialize_bloom_filter(const int64_t bf_version, char* &bf_buffer, int64_t &bf_serialize_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "need bf version=%ld <> filter's version=%ld", 
      ...
      {
        YYSYS_LOG(WARN, "allocator memory failed. size=%ld", bf_serialize_size);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS != (ret = bloom_filter_->serialize(bf_buffer, bf_serialize_size, pos)))
      {
        YYSYS_LOG(WARN, "serialize filter ret=%d, size=%ld, pos=%ld", ret, bf_serialize_size, pos);
      }
      bf_mutex_.unlock();
      return ret;
    }
    ...
    ObCompressor* ObSSTableReader::get_decompressor()
    {
      ...
      if (NULL == compressor_ && strlen(compressor_name) > 0)
      {
        ...
        if (NULL == compressor_)
        {
          YYSYS_LOG(ERROR, "Problem create compressor");
        }
      }
    ...
    bool ObSSTableReader::check_sstable(const char* sstable_fname, uint64_t *sstable_checksum)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to init file info, sstable_fname=%s", sstable_fname);
      }
      else if (OB_SUCCESS != fetch_sstable_size(file_info, sstable_size))
      {
        YYSYS_LOG(WARN, "failed to fetch sstable size, sstable_fname=%s", sstable_fname);
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to load trailer offset, sstable_fname=%s", sstable_fname);
      }
      else if (OB_SUCCESS != load_trailer(file_info, trailer, trailer_buf,
               trailer_buf_size, trailer_offset, trailer_size))
      {
        YYSYS_LOG(WARN, "failed to load trailer, sstable_fname=%s", sstable_fname);
      }
      else
      {
