...
namespace oceanbase
{
  namespace sstable
  {
        ...
        bool not_exit_col_ret_nop)
    {
      ...
      {
        YYSYS_LOG(ERROR , "trans_input_scan_range error, ret=%d,", iret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "old fashion binary rowkey format, MUST set rowkey schema.");
        iret = OB_ERROR;
      }
    ...
    int reset_query_thread_local_buffer()
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "thread local page arena hold memory over limited,"
              "total=%ld,used=%ld,pages=%ld", internal_buffer_arena->total(),
              internal_buffer_arena->used(), internal_buffer_arena->pages());
