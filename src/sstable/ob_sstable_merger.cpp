...
namespace oceanbase
{
  namespace sstable
  {
      ...
      const ObNewRange& new_range)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable file path, path_ptr=%p, path_len=%d",
                  sstable_path.ptr(), sstable_path.length());
        ret = OB_INVALID_ARGUMENT;
      }
      else if (new_range.empty())
      {
        YYSYS_LOG(WARN, "invalid empty range for sstable merge");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = check_readers_param(readers, reader_size)))
      {
        YYSYS_LOG(WARN, "failed to check reader param, reader_size=%ld, ret=%d", 
          reader_size, ret);
      }
      else
      {
      ...
      ObSSTableReader* readers[], const int64_t reader_size)
    {
      ...
      if (reader_size <= 0)
      {
        YYSYS_LOG(WARN, "invalid param, reader_size=%ld", reader_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else 
      {
        for (int64_t i = 0; i < reader_size; ++i)
        {
          if (NULL == readers[i])
          {
            YYSYS_LOG(WARN, "reader is NULL, index=%ld", i);
            ret = OB_INVALID_ARGUMENT;
            break;
          }
        }
      ...
      ObSSTableReader* readers[], const int64_t reader_size, const ObNewRange& new_range)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable_schema=%p or trailer=%p",
      ...
      {
        YYSYS_LOG(WARN, "get column groups failed, ret=%d", ret);
      }
      else 
      {
        ...
        {
          YYSYS_LOG(WARN, "close sstable failed, ret=%d", ret);
        }
      ...
      const int64_t reader_size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "prepare scan param failed, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = seq_scanner_.set_scan_param(scan_param_,
              *block_cache_, *block_index_cache_)))
      {
        YYSYS_LOG(ERROR, "set request param for sequence sstable scanner "
                         "failed, ret=%d",ret);
        scan_param_.reset();
      }
      ...
      while(OB_SUCCESS == ret)
      {
            ...
            OB_SUCCESS != (ret = seq_scanner_.get_cell(&cell_, &is_row_changed)))
        {
          ...
          {
            YYSYS_LOG(ERROR, "get data error, ret=%d", ret);
          }
        ...
        else if (cell_ != NULL)
        {
          ...
          if (OB_SUCCESS == ret && ROW_GROWING == row_status)
          {
            ...
            {
              YYSYS_LOG(ERROR, "add_cell_to_row failed, ret=%d", ret);
            }
          }
        }
        else
    ...
    int ObSSTableMerger::fill_scan_param(const uint64_t column_group_id) 
    {
      ...
      if (NULL == col || size <= 0)
      {
        YYSYS_LOG(WARN, "cann't find this column group=%lu", column_group_id);
        ret = OB_ERROR;
      }
      else
      {
        for(int64_t i = 0; i < size; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "add column id [%u] to scan_param_ error[%d]",
    ...
    int ObSSTableMerger::create_new_sstable()
    {
      ...
      {
        YYSYS_LOG(WARN, "create sstable failed, ret=%d", ret);
      }
    ...
    int ObSSTableMerger::save_current_row()
    {
      ...
      if (OB_SUCCESS != (ret = writer_.append_row(row_, current_sstable_size_)))
      {
        ...
        YYSYS_LOG(WARN, "append row failed ret=%d, this row_, obj count=%ld, "
    ...
    int ObSSTableMerger::check_row_count_in_column_group()
    {
      ...
      if (0 != pre_column_group_row_num_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "the row num between two column groups is difference, "
