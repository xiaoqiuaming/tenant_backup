...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObSSTableBlockGetter::load_current_row(const_iterator row_index)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "read current row error, store style=%ld, current row cursor=%d,%d",
    ...
    inline int ObSSTableBlockGetter::store_and_advance_column()
    {
      ...
      {
        YYSYS_LOG(ERROR, "store current cell error, ret = %d, cursor=%ld, id=%lu, index=%d", 
            ret, column_cursor_, column.id_, column.index_);
      }
    ...
    int ObSSTableBlockGetter::next_cell()
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "ObSSTableBlockGetter doesn't initialize");
        ret = OB_NOT_INIT;      
      }
      else if ((!is_row_cache_data_ && NULL == row_cursor_) 
               || query_column_indexes_.get_column_count() == 0)
      {
        YYSYS_LOG(WARN, "not initialized, is_row_cache_data_=%d, "
                                   ...
                                   bool not_exit_col_ret_nop)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, buf=%p, data_len=%ld, "
      ...
      else 
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "deserialize error, ret=%d", ret);
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (row_cursor_ != reader_.end())
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "load row error, ret=%d", ret);
              inited_ = false;
            }
          }
          else
          {
            YYSYS_LOG(DEBUG, "not find the row key:, ret=%d, rowkey:%s", ret, to_cstring(row_key));
      ...
      const int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalide row data, row_buf=%p, data_len=%ld",
          row_buf, data_len);
        ret = OB_INVALID_ARGUMENT;
      }
      else
