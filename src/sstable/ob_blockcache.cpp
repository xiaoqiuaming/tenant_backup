...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObBlockCache::enlarg_cache_size(const int64_t cache_mem_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "enlarge total block cache size of kv cache fail");
      }
      else
      {
        YYSYS_LOG(INFO, "success enlarge block cache size to %ld", cache_mem_size);
      }
    ...
    int ObBlockCache::destroy()
    {
      ...
      if (inited_)
      {
        ...
        {
          YYSYS_LOG(WARN, "destroy cache fail");
          ret = OB_ERROR;
        }
        else
        {
        ...
        const bool check_crc /*= true*/)
    {
      ...
      {
        YYSYS_LOG(WARN, "read sstable[%ld] block[%ld,%ld] from disk error=%d",
        ...
        else
        {
          ...
          if (OB_SUCCESS == status && NULL != buffer)
          {
            ...
            if (OB_SUCCESS == status)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to get block from block cached after put "
          ...
          {
            YYSYS_LOG(WARN, "read block fail sstable_id=%lu offset=%ld nbyte=%ld",
                      sstable_id, offset, nbyte);
          }
          if (OB_SUCCESS != status && OB_SUCCESS != kv_cache_.erase(data_index))
          {
            YYSYS_LOG(WARN, "failed to erase fake node from kvcache, sstable_id=%lu, "
                            "offset=%ld nbyte=%ld",
                      sstable_id, offset, nbyte);
          }
        }
        ...
        else
        {
          ...
          if (!is_reverse)
          {
            for (pos = cursor; pos < block_infos.block_count_ && readahead_size < MAX_READ_AHEAD_SIZE; ++pos)
            {
              ...
              {
                YYSYS_LOG(ERROR, "asc: block not continious, current =%ld, acc =%ld, pos=%ld",
          ...
          else
          {
            for (pos = cursor; pos >= 0 && readahead_size < MAX_READ_AHEAD_SIZE ; --pos)
            {
              ...
              if (next_offset != block_infos.position_info_[pos].offset_)
              {
                YYSYS_LOG(ERROR, "dsc: block not continious, current =%ld, acc =%ld, pos=%ld",
                    block_infos.position_info_[pos].offset_, next_offset, pos);
                status = OB_ERROR;
                break;
              }
                             ...
                             const bool reverse_scan)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, sstable_id=%lu, table_id=%lu, "
                        "column_group_id=%lu",
      ...
      {
        YYSYS_LOG(WARN, "get thread local aio buffer manager failed");
        ret = OB_IO_ERROR;
      }
      else
      {
                                        ...
                                        const bool check_crc)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, table_id=%lu, column_group_id=%lu",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get block aio failed");
        }
