...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObSSTableRowCache::init(const int64_t max_mem_size)
    {
      ...
      {
        inited_ = true;
        YYSYS_LOG_US(DEBUG, "init sstble row cache succ, cache_mem_size=%ld,",
                     max_mem_size);
      }
    ...
    int ObSSTableRowCache::enlarg_cache_size(const int64_t cache_mem_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "enlarge total sstable row cache size of kv cache fail");
      }
      else
      {
        YYSYS_LOG(INFO, "success enlarge sstable row cache size to %ld", cache_mem_size);
      }
      ...
      ObSSTableRowCacheValue& row_value, ObMemBuf& row_buf)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable row cache get_row param,key=%s", to_cstring(key.row_key_));
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          if (row_cache_val.size_ > 0)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "can't allocate enough space to store the row data, "
      ...
      const ObSSTableRowCacheValue& row_value)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable row cache key, key=%s", to_cstring(key.row_key_));
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
