...
namespace oceanbase
{
  namespace sstable
  {
                                                      ...
                                                      const int64_t version)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_id=%lu, version=%ld, schema_cnt=%ld",
      ...
      else
      {
        ...
        if (index >= 0)
        {
          ...
          {
            YYSYS_LOG(WARN, "sstable schema is NULL in sstable schema cache, index=%ld, "
    ...
    int ObSSTableSchemaCache::ensure_schema_buf_space(const int64_t size)
    {
      ...
      if (size <= 0)
      {
        YYSYS_LOG(WARN, "invalid sstable schema size, size=%ld", size);
      ...
      else if (NULL == schema_array_ || (NULL != schema_array_ && size > reamin_size))
      {
        ...
        if (NULL == new_buf)
        {
          YYSYS_LOG(ERROR, "Problem allocating memory for sstable schema buffer");
        ...
        else
        {
          ...
          {
            YYSYS_LOG(INFO, "expand sstable schema cache, schema_cnt=%ld, "
                                         ...
                                         const int64_t version)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, schema=%p, table_id=%lu, version=%ld, schema_cnt=%ld",
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (index >= 0)
          {
            YYSYS_LOG(INFO, "sstable schema existent, table_id=%lu, version=%ld, schema_cnt=%ld",
                      table_id, version, schema_cnt_);
            ret = OB_ENTRY_EXIST;
          }
        }
    ...
    int ObSSTableSchemaCache::revert_schema(const uint64_t table_id, const int64_t version)
    {
      ...
      else
      {
        ...
        if (index < 0)
        {
          YYSYS_LOG(WARN, "sstable schema is non-existent, table_id=%lu, "
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "wrong schema or reference count, schema=%p, table_id=%lu, "
    ...
    int ObSSTableSchemaCache::clear()
    {
      ...
      if (schema_cnt_ > 0)
      {
        for (int64_t i = 0; i < schema_cnt_; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "schema reference count is not 0, ref_cnt=%ld, index=%ld,"
