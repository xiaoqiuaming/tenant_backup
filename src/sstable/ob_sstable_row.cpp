...
{ 
  namespace sstable 
  {
                                    ...
                                    const bool sparse_format)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, obj_count=%ld, sparse_format=%d",
    ...
    int ObSSTableRow::set_rowkey_info(const ObRowkeyInfo* rowkey_info)
    {
      ...
      if (NULL == rowkey_info)
      {
        YYSYS_LOG(WARN, "invalide argument rowkey_info=%p", rowkey_info);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSSTableRow::set_table_id(const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument table_id=%lu", table_id);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSSTableRow::set_column_group_id(const uint64_t column_group_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument column_group_id=%lu", column_group_id);
        ret = OB_ERROR;
      }
      else
      {
    ...
    const ObObj* ObSSTableRow::get_obj(const int32_t index) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%d", index);
      }
 ...
 //add zhuyanchao [secondary index static data build]20150321
    ...
    ObObj ObSSTableRow::get_row_obj(const int64_t index)
    {
        ...
        {
          YYSYS_LOG(WARN, "invalid param, index=%ld", index);
        }
        return obj;
        //modify e
    }
//add e
    ...
    int ObSSTableRow::ensure_key_buf_space(const int64_t size)
    {
      ...
      if (size <= 0)
      {
        YYSYS_LOG(WARN, "invalid key length, size=%ld", size);
        ret = OB_ERROR;
      }
               ...
               || (NULL != row_key_buf_ && size > row_key_buf_size_))
      {
        ...
        if (NULL == new_buf)
        {
          YYSYS_LOG(WARN, "Problem allocating memory for row key buffer");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObSSTableRow::set_rowkey(const ObRowkey& rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid row key, len=%ld ptr=%p", length, ptr);
        ret = OB_ERROR;
      }
      //add zhaoqiong [bugfix::trim rowkey in varchar type in case of memtable checksum mismatch] 20160612:b
      else if (OB_SUCCESS != (ret = const_cast<ObSSTableRow*>(this)->ensure_key_string_buf_space(rowkey.get_varchar_length())))
      {
        YYSYS_LOG(WARN, "cannot alloc key length=%ld ", rowkey.get_varchar_length());
      }
      //add:e
      else
      {
        for (int i=0; common::OB_SUCCESS == ret && i< length; i++)
        {
          ...
          if (obj.get_type() == common::ObVarcharType)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to trim rowkey[%d] in varchar type", i);
            }
          }
          //add:e
          if (OB_SUCCESS == ret)
    ...
    int ObSSTableRow::set_binary_rowkey(const common::ObString& binary_rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid row key, ken_len=%d ptr=%p",
            binary_rowkey.length(), binary_rowkey.ptr());
        ret = OB_ERROR;
      }
      else if (NULL == binary_rowkey_info_)
      {
        YYSYS_LOG(WARN, "must set rowkey info when you use old binary rowkey format.");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (obj_count_ > 0)
      {
        YYSYS_LOG(WARN, "set binary_rowkey must before add obj.");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = ObRowkeyHelper::binary_rowkey_to_obj_array(
              *binary_rowkey_info_, binary_rowkey, obj_array, array_size)))
      {
        YYSYS_LOG(WARN, "translate binary rowkey to object array error.");
      }
      else
      {
    ...
    int ObSSTableRow::get_binary_rowkey(common::ObString& binary_rowkey) const
    {
      ...
      {
        YYSYS_LOG(WARN, "obj_count_=%d < rowkey size=%ld", obj_count_, binary_rowkey_info_->get_size());
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = const_cast<ObSSTableRow*>(this)->ensure_key_buf_space(
              binary_rowkey_info_->get_binary_rowkey_length())))
      {
        YYSYS_LOG(WARN, "cannot alloc key length=%ld ", binary_rowkey_info_->get_binary_rowkey_length());
      }
      else 
      {
      ...
      const common::ObRowkey& rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument, table_id=%lu, "
    ...
    int ObSSTableRow::obj_varchar_trim(common::ObObj & obj)
    {
      ...
      if (NULL == row_key_string_buf_)
      {
        YYSYS_LOG(WARN, "row_key_string_buf_ is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (obj.get_type() != common::ObVarcharType)
      {
        YYSYS_LOG(WARN, "obj must be ObVarcharType");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (row_key_string_buf_used_ + obj.get_varchar_trim_length() > row_key_string_buf_size_)
      {
        YYSYS_LOG(WARN, "row_key_string_buf_used_=%d + VARCHAR TRIM LEN[%d] > row_key_string_buf_size_ =%d", row_key_string_buf_used_, obj.get_val_len(), row_key_string_buf_size_);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObSSTableRow::check_schema(const ObSSTableSchema& schema) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid column count in scheam or obj count in row,"
      ...
      {
        YYSYS_LOG(WARN, "get rowkey column count of table=%ld error.", table_id_);
      }
      else if (OB_SUCCESS != (ret = schema.get_rowkey_columns(table_id_, rowkey_column_defs, rowkey_column_count)))
      {
        YYSYS_LOG(WARN, "get rowkey column defs of table=%ld error.", table_id_);
      }
      else
      {
        // compare rowkey objs;
        ...
        for (; def_index < rowkey_column_count; ++def_index)
        {
          ...
          {
            YYSYS_LOG(ERROR, "seq=%d not legal, rowkey column count=%ld", 
          ...
          {
              YYSYS_LOG(WARN, "rowkey obj[i=%ld,seq=%d,id=%d] type is inconsistent "
        ...
        while (obj_index < obj_count_ && def_index < column_count)
        {
          ...
          {
            YYSYS_LOG(WARN, "problem get column def, column_def=%p", column_def);
            ret = OB_ERROR;
            break;
          }
          ...
          {
            YYSYS_LOG(WARN, "value obj[i=%ld,id=%d] type is inconsistent with column type,"
        ...
        {
          YYSYS_LOG(WARN, "column count not match, obj_index=%ld, obj_count_=%d,"
        ...
        const int64_t row_serialize_size) const
    {
      ...
      else
      {
        ...
        for (int64_t i = start_obj_index; i < obj_count_; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to serialzie obj, current obj index=%ld,"
    ...
    DEFINE_DESERIALIZE(ObSSTableRow)
    {
      ...
      if (OB_SUCCESS == ret && obj_count_ > 0)
      {
        ...
        for (i = 0; OB_SUCCESS == ret && i < obj_count_; ++i)
        {
          if (obj_count_ < DEFAULT_MAX_COLUMN_NUMBER)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to deserialzie obj, current obj index=%ld,"
                              "obj_count=%d", i, obj_count_);
            }
          }
        }
        ...
        if (i != obj_count_)
        {
          YYSYS_LOG(WARN, "expect deserialize obj_count=%d, but only "
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "deserialize sstable row failed, objcount = %d, binary_rowkey_info_ = %p",
