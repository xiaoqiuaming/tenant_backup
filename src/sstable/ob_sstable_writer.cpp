...
namespace oceanbase
{
  namespace sstable
  {
                                        ...
                                        const int64_t element_count)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(WARN, "sstable writer is inited, please close sstable first");
        ret = OB_ERROR;
      }
      else if (NULL == path.ptr() || path_len <= 0 || path_len > MAX_SSTABLE_NAME_SIZE)
      {
        YYSYS_LOG(WARN, "Wrong sstable file path, path_ptr=%p, path_len=%ld",
                  path.ptr(), path_len);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "Wrong store type or block size or bloom filter element count, "
      ...
      {
        YYSYS_LOG(WARN, "invalid arguments table_version= %ld", table_version);
        ret = OB_ERROR;
      }
      else if (0 >= schema.get_column_count())
      {
        YYSYS_LOG(WARN, "Wrong schema with column def count =%ld", schema.get_column_count());
        ret = OB_ERROR;
      }
      else if (NULL == compressor_name.ptr() || compressor_name.length() <= 0)
      {
        YYSYS_LOG(WARN, "Wrong compressor name, name_ptr=%p, name_len=%d",
      ...
      {
        YYSYS_LOG(WARN, "copy schema failed");
      }
      ...
      {
        YYSYS_LOG(WARN, "Problem allocate memory for block builder");
      }
      ...
      {
        YYSYS_LOG(WARN, "Problem initialize bloom filter");
      }
      ...
      if (OB_SUCCESS == ret && NULL == compressor_)
      {
        ...
        if (NULL == compressor_)
        {
          YYSYS_LOG(WARN, "Problem create compressor");
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Problem open sstable file='%s', error=%s",
                                        ...
                                        const int64_t element_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid frozen time, frozen_time=%ld", 
                                    ...
                                    int64_t& approx_space_usage)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "sstable writer isn't inited, please create sstable first");
        ret = OB_ERROR;
      }
      else if (0 >= rowkey_column_count || OB_SUCCESS != (ret = row.get_rowkey(row_key)))
      {
        YYSYS_LOG(WARN, "current row does not contains rowkey column count=%ld", rowkey_column_count);
        ret = OB_ERROR;
      }
      else if (row.get_obj_count() == 0)
      {
        YYSYS_LOG(WARN, "There is no objects in the row to append");
        ret = OB_ERROR;
      }
      else if (table_id == OB_INVALID_ID || table_id == 0)
      {
        YYSYS_LOG(WARN, "invalid row, table_id=%lu", table_id);
        ret = OB_ERROR;
      }
      else if (column_group_id == OB_INVALID_ID)
      {
        YYSYS_LOG(WARN, "invalid row, column_group_id=%lu", column_group_id);
        ret = OB_ERROR;
      }
      else if (NULL == row_key.get_obj_ptr() || row_key.get_obj_cnt() <= 0)
      {
        YYSYS_LOG(WARN, "The row to append with NULL row key, key_ptr=%p, "
                        "key_len=%ld", row_key.get_obj_ptr(), row_key.get_obj_cnt());
        ret = OB_ERROR;
      }
      else if (is_dense_format() && OB_SUCCESS != (ret = row.check_schema(schema_)))
      {
        YYSYS_LOG(WARN, "The objects in the row are inconsistent with the schema");
      }
      //add zhaoqiong [varchar extend] 20151130:b
      else if (!first_row_ && is_duplicated_row_key(table_id, column_group_id,row_key))
      {
        YYSYS_LOG(INFO, "Current row is duplicated,row(table_id=%ld, column_group_id =%ld)",table_id,column_group_id);
        YYSYS_LOG(WARN, "Dump key of last row(%s) in buffer and current row(%s)",
      ...
      else if (!first_row_ && is_invalid_row_key(table_id, column_group_id, row_key))
      {
         ...
         */
        YYSYS_LOG(WARN, "Current row is not in order. last_row(table_id=%ld, "
      ...
      {
        YYSYS_LOG(WARN, "use binary rowkey in first row, but current row(%s) "
      ...
      if (OB_SUCCESS == ret && need_switch_block(table_id, column_group_id, row_size))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to write current block to disk, file_name=%s, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "Problem add row into block builder, file_name=%s, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                    filename_, table_id_, row_count_, offset_);
        }
    ...
    int ObSSTableWriter::append_row(const ObRow& row, int64_t& approx_space_usage)
    {
      ...
      {
        YYSYS_LOG(WARN, "There is no objects in the row[%s] to append", to_cstring(row));
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == row.get_row_desc())
      {
        YYSYS_LOG(WARN, "row desc of row[%s] is NULL", to_cstring(row));
      }
      else if (OB_SUCCESS != (ret = row.get_row_desc()->get_tid_cid(0LL, table_id, column_id)))
      {
        YYSYS_LOG(WARN, "get_tid_cid ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = row.get_rowkey(row_key)) || NULL == row_key)
      {
        YYSYS_LOG(WARN, "current row[%s] does not contains rowkey.", to_cstring(row));
        ret = OB_INVALID_ARGUMENT;
      }
      else if (table_id == OB_INVALID_ID || table_id == 0)
      {
                                       ...
                                       int64_t& sstable_size)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(INFO, "sstable writer isn't inited, needn't close sstable");
      }
      ...
      if (row_count_ > 0)
      {
        ...
        if (OB_SUCCESS == ret && block_builder_.get_row_count() > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem write the last block, file_name=%s, "
                            "table_id=%lu, row_count=%ld, offset=%ld",
                    filename_, table_id_, row_count_, offset_);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem write block index, file_name=%s, "
                            "table_id=%lu, row_count=%ld, offset=%ld",
                    filename_, table_id_, row_count_, offset_);
          }
        }
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "empty sstable, just write key stream and trailer, "
      ...
      if (OB_SUCCESS == ret && enable_bloom_filter_ && row_count_ > 0)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write bloom filter, file_name=%s, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                  filename_, table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write schema, file_name=%s, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                  filename_, table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write range, file_name=%s, "
              "table_id=%lu, row_count=%ld, offset=%ld",
              filename_, table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write trailer, file_name=%s, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                  filename_, table_id_, row_count_, offset_);
        }
        else
      ...
      {
        YYSYS_LOG(WARN, "fsync sstable failed, filename_=%s", filename_);
      }

      if (OB_SUCCESS == ret && offset_ != filesys_->get_file_pos())
      {
        YYSYS_LOG(WARN, "sstable size in non-cocnsistent writer_offset=%ld, "
                                             ...
                                             int64_t& wrote_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, comp_data=%p, comp_size=%ld, uncomp_size=%ld",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to serialize checksum of block_");
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem serialize block header to sstable '%s'",
                      filename_);
          }
        }
                                            ...
                                            const bool need_compress)
    {
      ...
      {
        YYSYS_LOG(WARN, "Can't write NULL input, input=%p, input_len=%ld, "
            ...
            compress_buf_len, ObModIds::OB_SSTABLE_WRITER)))
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(INFO, "The compressed size is larger than original data size,"
        ...
        {
          YYSYS_LOG(WARN, "failed to compress, sstable='%s', input=%p, "
    ...
    int ObSSTableWriter::write_current_block()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem add entry to block index builder, "
    ...
    int ObSSTableWriter::write_block_index()
    {
      ...
      {
        YYSYS_LOG(ERROR, "block index is too large, can't write, "
    ...
    int ObSSTableWriter::write_schema()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize schema for sstable='%s'", filename_);
        }
      }
    ...
    int ObSSTableWriter::write_trailer()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize trailer for sstable='%s'", filename_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize trailer offset for sstable='%s'",
                    filename_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Problem write trailer offset to sstable='%s', error=%s",
                    filename_, strerror(errno));
        }
        else
        {
          offset_ += trailer_offset_size;
          YYSYS_LOG(INFO, "close sstable='%s', checksum=%lu, row_checksum=%lu, table_id=%lu, "
    ...
    int ObSSTableWriter::copy_schema(const ObSSTableSchema& schema)
    {
      ...
      for (int32_t i=0; i< table_trun_count; ++i)
      {
        ...
        if ( NULL != tab_def)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "add table def failed, offset=%d", i);
            break;
          }
        }
        else
        {
          YYSYS_LOG(WARN, "got null table def from schema, offset=%d", i);
          ret = OB_ERROR;
          break;
        }
      }
      ...
      for(int32_t i = 0; i < column_num && OB_SUCCESS == ret; ++i)
      {
        ...
        if (NULL != column_def)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "add column def failed column_count=%ld curr_ofset=%d", 
                      column_num, i);
            break;
          }
        }
        ...
        {
          YYSYS_LOG(WARN, "got null column from scheam, column_def=%p", column_def);
          ret = OB_ERROR;
          break;
        }
      }
                                              ...
                                              const int64_t block_size)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (compressor_name.length() <= OB_MAX_COMPRESSOR_NAME_LENGTH)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem set compressor name");
          }
        }
        else
        {
          YYSYS_LOG(WARN, "Compressor name length is too large, "
    ...
    int ObSSTableWriter::check_row_count()
    {
      ...
      if (is_dense_format())
      {
        ...
        {
          YYSYS_LOG(WARN, "current column group row count isn't consistent "
