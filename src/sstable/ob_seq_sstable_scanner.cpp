...
{ 
  namespace sstable 
  {
        ...
        bool not_exit_col_ret_nop)
    {
      ...
      {
        YYSYS_LOG(ERROR, "input scan param range is empty, cannot scan.");
      ...
      else
      {
        if (YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_DEBUG)
        {
          ...
          YYSYS_LOG(DEBUG, "scan input param range, range=%s", range_buf);
        }
    ...
    int ObSeqSSTableScanner::add_sstable_reader(ObSSTableReader* reader)
    {
      ...
      if (NULL == reader)
      {
        YYSYS_LOG(ERROR, "add sstable reader error reader is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (sstable_array_.get_array_index() >= MAX_SSTABLE_SCANNER_NUM)
      {
        YYSYS_LOG(ERROR, "add sstable reader error, sstable reader array full, size=%ld",
            sstable_array_.get_array_index());
        ret = OB_SIZE_OVERFLOW;
      }
      else if (!sstable_array_.push_back(reader))
      {
        YYSYS_LOG(ERROR, "add sstable reader push to sstable array error, size=%ld",
            sstable_array_.get_array_index());
        ret = OB_SIZE_OVERFLOW;
      }
    ...
    int ObSeqSSTableScanner::next_cell()
    {
      ...
      {
        YYSYS_LOG(ERROR, "cur_iter_idx_ = %d > sstable array size=%ld",
      ...
      else
      {
        switch (cur_iter_status_)
        {
          case NOT_START:
            ...
            else 
            {
              ...
              YYSYS_LOG(DEBUG, "begin to scan first sstable id =%ld, ret=%d",
        ...
        if (OB_ITER_END == ret)
        {
              ...
              && cur_iter_idx_ < sstable_array_.get_array_index())
          {
            ...
            ret = sstable_scanner_.set_scan_param(*scan_param_, 
                *sstable_array_.at(cur_iter_idx_), *block_cache_, *block_index_cache_);
            YYSYS_LOG(DEBUG, "begin to scan %d th sstable id =%ld, ret=%d", cur_iter_idx_,
                (*sstable_array_.at(cur_iter_idx_))->get_sstable_id().sstable_file_id_, ret);
            if (OB_SUCCESS == ret)
              ret = sstable_scanner_.next_cell();
          }
