...
{ 
  namespace sstable 
  {
    ...
    DEFINE_SERIALIZE(ObSSTableBlockHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie block header, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObSSTableBlockHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie block header, buf=%p, "
    ...
    int ObSSTableBlockBuilder::init(const int64_t block_size)
    {
      ...
      if (block_size <= 0)
      {
        YYSYS_LOG(WARN, "invalid block size, block_size=%ld", block_size);
        ret = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == ret && NULL == block_buf_)
      {
        ...
        if (NULL == block_buf_)
        {
          YYSYS_LOG(ERROR, "Problem allocating memory for block buf");
          ret = OB_ERROR; 
        }
      }
    ...
    int ObSSTableBlockBuilder::ensure_space(const int64_t size)
    {
      ...
      else if (index_remain < 0 || block_remain - size < 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "alloc memory failed new_block_buffer=%p", new_block_buffer);
          ret = OB_ERROR;
        }
        else
        {
    ...
    {
      int ret             = OB_SUCCESS;
      int32_t row_offset  = static_cast<int32_t>(get_block_data_size());
      ...
      {
        YYSYS_LOG(WARN, "invalid param, row column_count=%ld, row table id=%lu,"
      ...
      {
        YYSYS_LOG(WARN, "disargument  row (table_id=%lu, column_group_id=%lu)," 
          ...
          && OB_SUCCESS == (ret = ensure_space(row_size)))
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to serialize row index");
          }
        }
        else
        {
          YYSYS_LOG(WARN, "failed to serialize row");
        }
      }
    ...
    int ObSSTableBlockBuilder::build_block() 
    {
      ...
      {
        YYSYS_LOG(WARN, "there is not enough space to store the 'row_count + 1' index");
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to serialize 'row_count + 1' row index");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        block_header_.row_index_array_offset_ = static_cast<int32_t>(get_block_data_size());
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize block header");
        ret = OB_ERROR;
      }
