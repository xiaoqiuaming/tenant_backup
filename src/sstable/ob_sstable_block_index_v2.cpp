...
namespace oceanbase
{
  namespace sstable
  {
      ...
      if (NULL != ret)
      {
        ...
        else
        {
          ...
          if (ret->base_length_ > 0)
          {
            ...
            {
              YYSYS_LOG(WARN, "sstable block index deserialize failed, ret=%d",
                        status);
              ret = NULL;
            }
            else 
          ...
          {
            YYSYS_LOG(WARN, "sstable block index get_deserialize_size failed, ret=%ld",
                      ret->base_length_);
            ret = NULL;
          }
        }
        ...
        ObBlockPositionInfos& pos_info) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid arguments, table_id=%ld, mode=%d, key=%p",
            table_id, mode, key.ptr());
        iret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (iret = get_bound(bound)))
      {
        YYSYS_LOG(ERROR, "get position error, iret=%d", iret);
      }
        ...
        ObBlockPositionInfos& pos_info) const
    {
      ...
      if ( table_id <= 0 )
      {
        YYSYS_LOG(ERROR, "invalid arguments, table_id=%ld", table_id);
        iret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (iret = get_bound(bound)))
      {
      ...
      {
        YYSYS_LOG(ERROR, "trans range error,iret=%d", iret);
      }
      else if (OB_SUCCESS == ( iret = 
        ...
        const char* buf, const int64_t data_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid buf =%p, data_len=%ld, pos=%ld",
            buf, data_len, pos);
        iret = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(ERROR, "record header error, buf =%p, data_len=%ld, pos=%ld",
            buf, data_len, pos);
      }
      else if (record_header.is_compress())
      {
        YYSYS_LOG(ERROR, "compressed block index stream not support now.");
        iret = OB_ERROR;
      }
      else if (OB_SUCCESS != (iret = block_index_header.deserialize(
              payload_ptr, payload_size, payload_pos)))
      {
        YYSYS_LOG(ERROR, "deserialize block_index_header error, "
      ...
      else 
      {
        ...
        for (int64_t i = 0; i < block_index_header.sstable_block_count_ && OB_SUCCESS == iret; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize element of block index array error."
          ...
          else
          {
            ...
            if (block_index_header.rowkey_flag_ == 0)
            {
              ...
              {
                YYSYS_LOG(ERROR, "old binary rowkey format index, cannot get rowkey info.");
              }
        ...
        const char* base, int64_t base_length)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize record header error, "
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "block index cache buffer size =%ld not enough (%ld),"
          ...
          for (i = 0; i < block_index_count_ && current_obj_array_ptr < obj_array_end && OB_SUCCESS == iret; ++i)
          {
            ...
            {
              iret = OB_SIZE_OVERFLOW;
              YYSYS_LOG(ERROR, "key out of range, current_key_stream_offset=%d,"
            ...
            else
            {
              ...
              if (block_index_header.rowkey_flag_ == 0)
              {
                ...
                {
                  YYSYS_LOG(ERROR, "cannot translate binary rowkey to object array.");
                }
                else
                {
              ...
              {
                YYSYS_LOG(ERROR, "deserialize rowkey object array error.");
              }
          ...
          {
            YYSYS_LOG(ERROR, "traverse block index not to end, "
        ...
        const bool is_start, const_iterator& find_it) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "internal error, table=%ld,group=%ld",
