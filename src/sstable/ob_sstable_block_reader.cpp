...
{ 
  namespace sstable 
  {
    ...
    int ObSSTableBlockReader::deserialize(const BlockDataDesc& desc, const BlockData& data)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, input block data, desc error.");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = header_.deserialize(data.data_buffer_, data.data_bufsiz_, pos))) 
      {
        YYSYS_LOG(ERROR, "deserialize header error, databuf=%p,bufsiz=%ld", 
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "block need size = %ld > input internal bufsiz=%ld, row count=%d, realloc", 
        ...
        if (NULL != internal_buffer_ptr)
        {
          ...
          for (int i = 0; i < (header_.row_count_ + 1) && OB_SUCCESS == ret; ++i)
          {
            ...
            {
              YYSYS_LOG(ERROR, "analysis of the index data failed,"
        ...
        {
          YYSYS_LOG(ERROR, "block internal buffer allocate error,sz=%ld", index_entry_length);
    ...
    int ObSSTableBlockReader::get_row_key(const_iterator index, common::ObRowkey& key, int64_t &pos) const
    {
      ...
      else
      {
        ...
        if (rowkey_count == 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize binary rowkey error, offset=%d, size=%d",
          ...
          {
            YYSYS_LOG(ERROR, "cannot translate binary rowkey into ObObj array.");
          }
          else
          {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "cannot get rowkey columns ret =%d, count=%ld, rowkey_count=%ld",
        ...
        common::ObObj* values, int64_t& column_count, bool is_scan /*=true*/) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "get row key error, ret=%d, format=%d, index=%d,%d", 
        ...
        common::ObRowkey& key, common::ObRow& value) const
    {
      ...
      if (rowkey_count == 0)
      {
        ...
        if (OB_SUCCESS != (ret = get_row_key(index, key, pos)))
        {
          YYSYS_LOG(WARN, "get_binary_rowkey error, pos=%ld, ret=%d", pos, ret);
        }
        else
        {
          rowkey_count = block_data_desc_.rowkey_info_->get_size();
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        while (OB_SUCCESS == ret && row_start + pos < row_end)
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize column value object error, ret=%d, "
          ...
          {
            YYSYS_LOG(ERROR, "cannot get obj at cell:%ld", result_index);
          }
          else
          {
        ...
        common::ObRowkey& key, common::ObRow& value) const
    {
      ...
      if (OB_SUCCESS != (ret = get_row_key(index, key, pos)))
      {
        YYSYS_LOG(ERROR, "get row key error, ret=%d, index=%d,%d", 
        ...
        common::ObRowkey& key, common::ObRow& value) const
    {
          ...
          && filled < not_null_col_num)
      {
        ...
        {
          YYSYS_LOG(ERROR, "sparse format, obj(%s) not column_id", to_cstring(obj));
          ret = OB_UNKNOWN_OBJ;
        }
        ...
        common::ObObj* ids, common::ObObj* values, int64_t& count) const
    {
      ...
      else
      {
        while (row_start + pos < row_end && object_index < count)
        {
          ...
          if (OB_SUCCESS != ret) 
          {
            YYSYS_LOG(ERROR, "deserialize id object error, ret=%d, "
