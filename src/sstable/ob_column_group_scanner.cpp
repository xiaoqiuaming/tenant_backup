  ...
  namespace sstable
  {
        ...
        const ObSSTableReader *sstable_reader)
    {
      ...
      else if (column_id_size == 1 && *column_id_begin == OB_FULL_ROW_COLUMN_ID)
      {
        ...
        {
          iret = OB_ERROR;
          YYSYS_LOG(ERROR, "find column group def array error table=%ld, group=%ld", table_id, group_id);
        }
        else
        {
      ...
      else
      {
        ...
        for (int64_t i = 0; i < column_id_size && OB_SUCCESS == iret; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "input column id =%ld (i=%ld) is invalid.", 
                current_column_id, i);
        ...
        const ObSSTableReader* const sstable_reader)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "cg scanner begin to scan table id=%ld, group id=%ld, "
