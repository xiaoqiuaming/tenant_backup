...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObSSTableGetter::check_internal_member()
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "ObSSTableGetter doesn't initialize");
        ret = OB_ERROR;
      }
      else if (NULL == get_param_ || NULL == readers_ || readers_size_ <= 0)
      {
        YYSYS_LOG(WARN, "get_param or or readers is NULL, get_param=%p, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid get_param, cell_size=%ld row_index=%p, "
                          "row_size=%ld",
        ...
        {
          YYSYS_LOG(WARN, "current reader index overflow, cur_reader_idx_=%ld, "
    ...
    int ObSSTableGetter::switch_row()
    {
      ...
      else
      {
        ...
        if (need_load_block)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to load current block, ret=%d", ret);
          }
        }
      }
    ...
    inline const ObCellInfo* ObSSTableGetter::get_cur_param_cell() const
    {
      ...
      if (NULL == cell)
      {
        YYSYS_LOG(WARN, "crrent cell in get param is NULL, get_param=%p, "
    ...
    int ObSSTableGetter::handle_row_not_found()
    {
      ...
      if (NULL == cell)
      {
        YYSYS_LOG(WARN, "crrent cell in get param is NULL");
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "row key doesn't exist in current column group, but it "
               ...
               && OB_SUCCESS == (ret = check_internal_member()) )
      {
        ...
        else
        {
          ...
          case ROW_NOT_FOUND:
             ...
             */
              if(get_param_->get_table_truncate())
              {
                  obj.set_ext(ObActionFlag::OP_DEL_ROW);
                  del_cell_info_.column_id_ = 0;
                  del_cell_info_.table_id_ = cell->table_id_;
                  del_cell_info_.row_key_ = cell->row_key_;
                  del_cell_info_.value_ = obj;
                  *cell_info = &del_cell_info_;
              }
              else
              {
          ...
          case ITERATE_NORMAL:
            ...
            break;
          case GET_NEXT_ROW:
            YYSYS_LOG(WARN, "invalid internal state(GET_NEXT_ROW) in get_cell()");
          case ITERATE_END:
          default:
                              ...
                              ObSSTableRowCache* row_cache)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, readers=%p, size=%ld", readers, reader_size);
        ret = OB_INVALID_ARGUMENT;
      }
        ...
        const int64_t store_style, ObSSTableRowCacheValue& row_cache_val)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid cached row buf=%p, row_size=%ld", 
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "block getter initialize error, ret=%d", ret);  
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)  //load block index success
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (OB_SUCCESS != ret && OB_SEARCH_NOT_FOUND != ret)
            {
              YYSYS_LOG(WARN, "fetch_block error, ret=%d", ret);
            }
          }
          else
          {
            YYSYS_LOG(WARN, "failed to init column group column mask");
          }
        }
        else if (ret == OB_BEYOND_THE_RANGE)
        {
          YYSYS_LOG(DEBUG, "Not find block for rowkey: %s", to_cstring(look_key));
          ret = OB_SEARCH_NOT_FOUND;
        }
        else
        {
          YYSYS_LOG(WARN, "failed to apply block index, ret=%d", ret);
        }
      }
    ...
    int ObSSTableGetter::init_column_mask()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL != reader)
        {
          ...
          if (NULL == schema)
          {
            YYSYS_LOG(WARN, "schema is NULL, cur_reader_idx_=%ld, reader=%p", 
        ...
        {
          YYSYS_LOG(WARN, "reader is NULL, cur_reader_idx_=%ld", cur_reader_idx_);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = start; i < end && OB_SUCCESS == ret; ++i)
        {
          ...
          if (OB_FULL_ROW_COLUMN_ID == column_id)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to get column group schema, column_def=NULL,"
                                        ...
                                        const char*& block_data, int64_t& block_data_size)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (get_param_->get_is_result_cached())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to advise, table_id=%lu, sstable_id=%lu, "
        ...
        {
          YYSYS_LOG(WARN, "load block from obcache failed, ret=%d", ret);
          ret = OB_IO_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "invalid block data, block_size=%ld, sstable_id=%lu, table_id=%lu",
      ...
      if (OB_SUCCESS == ret)
      {
        if (header.is_compress())
        {
          ...
          if (NULL != compressor)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "decompress failed, data_length=%d data_zlength=%d",
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "the uncompressed length=%ld, the expected length=%d",
          ...
          {
            YYSYS_LOG(WARN, "failed to get compressor");
            ret = OB_ERROR;
          }
        }
        else
    ...
    int ObSSTableGetter::fetch_block() 
    {
      ...
      if (NULL != sstable_row_cache_)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to get sstable row cache value");
          }
          else if (row_cache_val.size_ <= 0 || NULL == row_cache_val.buf_)
          {
            YYSYS_LOG(WARN, "after sstable block getter reader row data, it "
          ...
          else 
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed put existent row into sstble row "
                              "cache, buf=%p, size=%ld",
                row_cache_val.buf_, row_cache_val.size_);
            }
            ret = OB_SUCCESS;
        ...
        else if (OB_SEARCH_NOT_FOUND == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed put non-existent row into sstble row "
                            "cache, buf=%p, size=%ld",
                row_cache_val.buf_, row_cache_val.size_);
          }
          ret = OB_SEARCH_NOT_FOUND;
                                             ...
                                             const uint64_t column_id)
    {
      ...
      if (column_index < 0 || OB_INVALID_ID == column_group_id)
      {
        ...
        YYSYS_LOG(DEBUG, "can't find column group id from sstable schema, "
    ...
    int ObSSTableGetter::filter_column_group()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL != reader)
        {
          ...
          if (NULL == schema)
          {
            YYSYS_LOG(WARN, "schema is NULL, cur_reader_idx_=%ld, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "table doesn't exist, table_id=%lu", table_id);
          ret = OB_SEARCH_NOT_FOUND;
        }
        ...
        for (int64_t i = start; i < end && OB_SUCCESS == ret; ++i)
        {
          ...
          if (OB_FULL_ROW_COLUMN_ID == column_id)
          {
            ...
            if (OB_SUCCESS == ret && column_group_size > 0)
            {
              ...
              {
                YYSYS_LOG(WARN, "more than one cell with same rowkey and "
