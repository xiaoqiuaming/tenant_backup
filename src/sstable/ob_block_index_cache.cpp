...
namespace oceanbase
{
  namespace sstable
  {
    ...
    int ObBlockIndexCache::init(const int64_t cache_mem_size)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(INFO, "have inited");
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != kv_cache_.init(cache_mem_size))
      {
        YYSYS_LOG(WARN, "init kv cache fail");
        ret = OB_ERROR;
      }
      else
      {
        inited_ = true;
        YYSYS_LOG_US(DEBUG, "init block index cache succ, cache_mem_size=%ld,", cache_mem_size);
      }
    ...
    int ObBlockIndexCache::enlarg_cache_size(const int64_t cache_mem_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "enlarge total block index cache size of kv cache fail");
      }
      else
      {
        YYSYS_LOG(INFO, "success enlarge block index cache size to %ld", cache_mem_size);
      }
        ...
        Handle& handle)
    {
      ...
      {
        YYSYS_LOG(WARN, "have not inited, fileinfo_cache_=%p", fileinfo_cache_);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "pread block index from sstable failed, sstable_id=%lu, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == bci_tmp)
        {
          YYSYS_LOG(WARN, "placement new block index instance failed");
          ret = OB_ERROR;
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to put and fetch block index from kvcache, table_id=%lu",
                      table_id);
          }
        }
        YYSYS_LOG(INFO, "load sstable block index, table_id=%lu, sstable_id=%lu, "
                  ...
                  bci_tmp->get_deserialize_size(), yysys::CTimeUtil::getTime() - start_time,
        ...
        Handle& handle)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to read sstable block index, table_id=%lu, "
            ...
            {
              YYSYS_LOG(WARN, "failed to delete fake node from kvcache, table_id=%lu, "
                  "sstable_id=%lu, offset=%ld, size=%ld",
                  table_id, block_index_info.sstable_file_id_, block_index_info.offset_,
                  block_index_info.size_);
            }
        ...
        const uint64_t column_group_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param sstable_id=%lu, offset=%ld, nbyte=%ld, "
        ...
        ObBlockPositionInfos& pos_info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_param error, table_id=%ld, column_group_id=%ld",
            table_id, column_group_id);
      }
      else if ( is_regular_mode(search_mode)
          && (NULL == key.ptr() || key.length() <= 0) )
      {
        YYSYS_LOG(WARN, "invalid param, key_ptr=%p, key_len=%ld",
                  key.ptr(), key.length());
      ...
      {
        YYSYS_LOG(ERROR, "load block index error, ret=%d, table_id=%ld"
      ...
      if (revert_handle && OB_SUCCESS != kv_cache_.revert(handle))
      {
        ...
        YYSYS_LOG(WARN, "failed to revert  block index cache handle");
      }
      ...
      const uint64_t table_id, ObRowkey& row_key)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_param error, table_id=%ld", table_id);
      }
      else if ( OB_SUCCESS != (ret =
          load_block_index(block_index_info, block_index, table_id, handle)) )
      {
