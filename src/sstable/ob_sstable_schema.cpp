...
namespace oceanbase
{
  ...
  namespace sstable
  {
        ...
        const ObSchemaManagerV2* &schema, const ObTableSchema* &table_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "old fashion sstable, NOT set SCHEMA compatible info, table=%ld", table_id);
        iret = OB_ERROR;
      }
      else if (NULL == (schema = global_sstable_schema_manager->get_schema(table_id)))
      {
        YYSYS_LOG(ERROR, "old fashion sstable, can NOT get SCHEMA compatible info, table=%ld", table_id);
        iret = OB_ERROR;
      }
      else if (NULL == (table_schema = schema->get_table_schema(table_id)))
      {
        YYSYS_LOG(ERROR, "old fashion sstable, can NOT get table %ld SCHEMA ", table_id);
        iret = OB_ERROR;
      }
      return iret;
    }
    ...
    int release_global_sstable_schema_manager(const ObSchemaManagerV2* schema)
    {
      ...
      if (NULL == global_sstable_schema_manager)
      {
        YYSYS_LOG(WARN, "old fashion sstable, NOT set SCHEMA compatible info.");
        iret = OB_ERROR;
      }
      else if (OB_SUCCESS != (iret =
            global_sstable_schema_manager->release_schema(schema)))
      {
        YYSYS_LOG(WARN, "old fashion sstable, release schema :%ld error: %d.",
    ...
    int get_global_schema_rowkey_info(const uint64_t table_id, ObRowkeyInfo& rowkey_info)
    {
      ...
      if (NULL != schema)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "release global sstable schema failed");
          if (OB_SUCCESS == iret)
          {
            iret = err;
          }
  ...
  namespace sstable
  {
    ...
    int ObSSTableSchema::get_rowkey_column_count(const uint64_t table_id, int64_t& rowkey_column_count) const
    {
      ...
      if (!hash_init_)
      {
        YYSYS_LOG(WARN, "hashmap not init yet");
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "Get (table_id=%lu) from table rowkey hashmap failed", table_id);
          ret = OB_ERROR;
        }
        else
        {
        ...
        const uint64_t table_id, const uint64_t rowkey_seq) const
    {
      ...
      if ( hash::HASH_EXIST != ret_hash )
      {
        YYSYS_LOG(WARN, "Get (table_id=%lu) from table rowkey hashmap failed", table_id);
      }
      else
      {
        int32_t offset = -1;
      ...
      const int32_t index) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%d, column_count=%ld",
    ...
    DEFINE_SERIALIZE(ObSSTableSchemaHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize schema header");
      }
    ...
    DEFINE_DESERIALIZE(ObSSTableSchemaHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialize schema header");
      }
    ...
    DEFINE_SERIALIZE(ObSSTableSchemaTableDef)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie schema def, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObSSTableSchemaTableDef)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie schema column def, buf=%p, "
    ...
    void ObSSTableSchema::dump() const
    {
      ...
      for (int64_t i = 0; i < column_count; ++i)
      {
        ...
        YYSYS_LOG(INFO, "column %ld: table:%d, group:%d, rowkey seq:%d, id:%d, type:%d, offset:%d",
    ...
    int ObSSTableSchema::update_hashmaps(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      {
        YYSYS_LOG(WARN, "update table schema hashmap failed. table_id=%u, column_group_id=%u,"
      ...
      {
        YYSYS_LOG(WARN, "update group schema hashmap failed. table_id=%u, column_group_id=%u,"
                  "column_name_id=%u", column_def.table_id_,
                  column_def.column_group_id_, column_def.column_name_id_);
      }
      else if (OB_SUCCESS != (ret = update_group_index_hashmap(column_def)))
      {
        YYSYS_LOG(WARN, "update group index  hashmap failed. table_id=%u, column_group_id=%u,"
                  "column_name_id=%u", column_def.table_id_,
                  column_def.column_group_id_, column_def.column_name_id_);
      }
      else if (OB_SUCCESS != (ret = update_table_rowkey_hashmap(column_def)))
      {
        YYSYS_LOG(WARN, "update rowkey hashmap failed. table_id=%u, column_group_id=%u,"
                  "column_name_id=%u", column_def.table_id_,
                  column_def.column_group_id_, column_def.column_name_id_);
      }
      return ret;
      ...
      else
      {
        ...
        else if (column_def.rowkey_seq_ > 0)
        {
          ...
          else
          {
            ...
            if (hash::HASH_EXIST != ret_hash)
            {
              YYSYS_LOG(ERROR, "get rowkey hashmap table=%d, column group =%d error",
        ...
        if (-1 == ret_hash)
        {
          YYSYS_LOG(ERROR, "insert table_rowkey_hashmap_ error,"
    ...
    int ObSSTableSchema::update_table_schema_hashmap(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (0 == size || (NULL != last && column_def.table_id_ != last->table_id_))
        {
          ...
          if (-1 == ret_hash)
          {
            YYSYS_LOG(ERROR, "Insert table schema hashmap faied"
        ...
        else if (NULL != last && column_def.column_group_id_ != last->column_group_id_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "Get (table_id=%u) from table schema hashmap failed", column_def.table_id_);
            ret = OB_ERROR;
          }
          else if (hash::HASH_EXIST == ret_hash)
          {
            ...
            if (-1 == ret_hash)
            {
              YYSYS_LOG(ERROR, "insert table_schema_hashmap_ failed table_id=%u", column_def.table_id_);
              ret = OB_ERROR;
            }
          }
        }
    ...
    int ObSSTableSchema::update_group_schema_hashmap(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (-1 == ret_hash)
          {
            YYSYS_LOG(ERROR, "Get key(table_id=%lu, column_group_id=%lu)"
          ...
          else if (hash::HASH_NOT_EXIST == ret_hash)
          {
            ...
            if (-1 == ret_hash)
            {
              YYSYS_LOG(ERROR, "insert group_schemma_hashmap_ failed "
    ...
    int ObSSTableSchema::update_group_index_hashmap(const ObSSTableSchemaColumnDef& column_def)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (-1 == ret_hash)
          {
            YYSYS_LOG(ERROR, "Get key(table_id=%lu, column_name_id=%lu)"
    ...
    int ObSSTableSchema::add_column_def(const ObSSTableSchemaColumnDef& def)
    {
      ...
      int32_t size = static_cast<int32_t>(get_column_count());
      ...
      {
        YYSYS_LOG(WARN, "Can not add any more column_def, column_count=%d", size);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "column def must be order in (table_id, group_id, column_id, rowkey_seq),"
      ...
      if ( OB_SUCCESS == ret)
      {
        ...
        if (column_def.column_group_id_ == ObSSTableSchemaColumnDef::ROWKEY_COLUMN_GROUP_ID)
        {
          if (offset >= 0)
          {
            YYSYS_LOG(WARN, "rowkey column(%u,%hu,%u,%u) duplicate with offset=%ld",
                  column_def.table_id_, column_def.column_group_id_,
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "normal column(%u,%hu,%u,%u) duplicate with rowkey column offset=%ld",
                  column_def.table_id_, column_def.column_group_id_,
                  column_def.column_name_id_, column_def.rowkey_seq_, offset);
            ret = OB_ERROR;
          }
      ...
      if (OB_SUCCESS == ret && !hash_init_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "create hashmaps failed");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
            ...
            && column_def_ == column_def_array_)
        {
          ...
          if (NULL == column_def_)
          {
            YYSYS_LOG(WARN, "failed to alloc memory column_def_=%p", column_def_);
            ret = OB_ERROR;
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "update hashmaps failed ret=%d", ret);
          }
        }
...
//add zhaoqiong [Truncate Table]:20160318:b
    int ObSSTableSchema::add_table_def(const ObSSTableSchemaTableDef& table_def)
    {
      ...
      {
        YYSYS_LOG(WARN, "is largert than the table count");
        ret = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(DEBUG, "add table truncate def succ, trun_table_count= %d", schema_header_.trun_table_count_);
      }
      return ret;
    }
//add:e
                                                    ...
                                                    uint64_t* group_ids, int64_t & size)const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid arguments table_id = %lu", table_id);
        ret = OB_ERROR;
      }
      else if (0 != get_column_count())
      {
        if (hash_init_)
        {
          ...
          if (-1 == hash_get)
          {
            YYSYS_LOG(ERROR, "Get table_id=%lu failed", table_id);
            ret = OB_ERROR;
          }
          ...
          else if (hash::HASH_EXIST == hash_get)
          {
            for (int index = 0; index < group_id_set.size_; ++index)
            {
              ...
              if (offset < 0 || offset >= get_column_count())
              {
                YYSYS_LOG(ERROR, "offset overflosw offset=%d total column count=%ld",
          ...
          {
            YYSYS_LOG(INFO, "hashmap not init yet");
            size = 0;
            ret = OB_ERROR;
          }
        }
        ...
        int64_t& size) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument table_id=%lu, group_id=%lu", table_id, group_id);
      }
      else if ( 0 != get_column_count() )
      {
        if (hash_init_)
                                              ...
                                              uint64_t *array, int64_t &size) const
    {
      ...
      {
           YYSYS_LOG(WARN, "invalid argument table_id = %lu, column_id = %lu",
                  table_id, column_id);
        ret = OB_ERROR;
      }
                                                                         ...
                                                                         uint64_t & column_group_id) const
    {
      ...
      if (OB_INVALID_ID == table_id || 0 == table_id
          || OB_INVALID_ID == column_id)
      {
        YYSYS_LOG(WARN, "invalid argument table_id=%lu, column_id=%lu", table_id, column_id);
      }
        ...
        if (hash_init_)
        {
          ...
          else if (hash::HASH_EXIST == hash_get)
          {
            if (0 < group_set.size_)
            {
              ...
              {
                YYSYS_LOG(ERROR, "offset overflosw offset=%d max_offset=%lu",
                          group_set.head_offset_, get_column_count());
                ret = OB_ERROR;
              }
              else
                                                                   ...
                                                                   const uint64_t column_id) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument table_id = %lu, column_group_id = %lu"
        ...
        if (hash_init_)
        {
          ...
          else if (hash::HASH_EXIST == hash_get)
          {
            ...
            for (int32_t index = 0; index < group_set.size_; ++index)
            {
              ...
              if (offset < 0 || offset >= get_column_count())
              {
                YYSYS_LOG(ERROR, "offset overflosw offset=%ld total colum count=%ld",
                          offset, get_column_count());
                ret = OB_ERROR;
                break;
              }
    ...
    DEFINE_SERIALIZE(ObSSTableSchema)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to serialize schema version_, buf=%p, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to serialize schema header, buf=%p, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; OB_SUCCESS == ret && i < schema_header_.trun_table_count_; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to serialize schema column def, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObSSTableSchema)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to deserialize sstable schema version, buf=%p, "
      ...
      if (OB_SUCCESS == ret )
      {
        if (version_ == OB_SSTABLE_SCHEMA_VERSION_TWO)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to deserialize schema header, buf=%p, "
        ...
        else if (version_ == OB_SSTABLE_SCHEMA_VERSION_ONE)
        {
          ...
          else
          {
            ...
            trun_table_count = 0;
            column_count = get_column_count();
            YYSYS_LOG(INFO, "compatiable sstable version buf=%p, "
        ...
        {
          YYSYS_LOG(WARN, "unsupported sstable schema version(%d)", version_);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; OB_SUCCESS == ret && i < trun_table_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to deserialize schema table def, buf=%p, "
          ...
          {
              YYSYS_LOG(WARN, "add table def failed table_def(table_id=%d, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; OB_SUCCESS == ret && i < column_count; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to deserialize schema column def, buf=%p, "
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "add column def failed cocolumn_def(table_id=%u, column_group_id=%u"
                        ", column_name_id=%u", column_def.table_id_,
                        column_def.column_group_id_, column_def.column_name_id_);;
            }
          }
    ...
    DEFINE_GET_SERIALIZE_SIZE(ObSSTableSchema)
    {
      ...
      int32_t column_count = static_cast<int32_t>(get_column_count());
        ...
        ObSSTableSchema& sstable_schema, bool binary_format)
    {
      ...
      if (NULL == col || size <= 0)
      {
        YYSYS_LOG(ERROR,"cann't find this table:%lu in schema", schema_table_id);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for (col_index = 0; col_index < rowkey_info.get_size() && OB_SUCCESS == ret && !binary_format; ++col_index)
        {
          ...
          if (NULL == column)
          {
            YYSYS_LOG(ERROR, "schema internal error, rowkey column %ld not exist.", col_index);
            ret = OB_ERROR;
            break;
          }
          ...
          {
            YYSYS_LOG(ERROR,"add column_def(%u,%u,%u,%u) failed col_index : %ld",column_def.table_id_,
        ...
        ObSSTableSchema& sstable_schema, bool binary_format)
    {
      ...
      for (; item != helper.end(); ++item)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "build_sstable_schema error item=%p, table_schema=%p", item, table_schema);
          break;
        }
      }
      return ret;
