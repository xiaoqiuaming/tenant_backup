...
{ 
  namespace sstable 
  {
    ...
    DEFINE_SERIALIZE(ObSSTableBlockIndexHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie block index header, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObSSTableBlockIndexHeader)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie block index header, buf=%p, "
                                              ...
                                              const int32_t record_size) 
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_id=%lu, key_len=%ld,"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret) 
        {
          ...
          if (OB_ERROR == ret)
          {
            YYSYS_LOG(WARN, "failed to add end key");
          }
          else
          {
        ...
        {
          YYSYS_LOG(WARN, "failed to add index item");
          ret = OB_ERROR;
        }
      }
      return ret;
                                              ...
                                              const int32_t record_size) 
    {
      ...
      if (record_size < 0 || key.length() <= 0 || NULL == key.ptr()
          || table_id == OB_INVALID_ID || table_id == 0 || OB_INVALID_ID == column_group_id)
      {
        YYSYS_LOG(WARN, "invalid param, table_id=%lu, key_len=%d,"
                        "key_ptr=%p, record_size=%d, column_group_id=%lu", 
        ...
        char* index_block, const int64_t buffer_size, int64_t& index_size)
    {
      ...
      if (NULL == index_block)
      {
        YYSYS_LOG(WARN, "invalid param, index_block=%p", index_block); 
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to serialize index block header");
          ret = OB_ERROR;
        }
      }
