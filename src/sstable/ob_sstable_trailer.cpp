...
namespace oceanbase
{
  namespace sstable
  {
    ...
    DEFINE_SERIALIZE(ObTrailerOffset)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie trailer offset, buf=%p, "
    ...
    DEFINE_DESERIALIZE(ObTrailerOffset)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie trailer offset, buf=%p, "
    ...
    int ObSSTableTrailer::set_range_record_offset(const int64_t offset)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, offset=%ld", offset);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_range(const ObNewRange& range)
    {
      ...
      {
        YYSYS_LOG(WARN, "table id[%ld] should not be OB_INVALID_ID", range.table_id_);
    ...
    DEFINE_SERIALIZE(ObSSTableTrailer)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie the fourth part of trailer, buf=%p, "
                  "buf_len=%ld, pos=%ld", buf, buf_len, pos);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; i < RESERVED_LEN && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to serialzie trailer reserved, buf=%p, "
        ...
        {
          YYSYS_LOG(WARN, "failed to serialzie trailer comprssor name, buf=%p, "
                          "buf_len=%ld, pos=%ld", buf, buf_len, pos);
          ret = OB_ERROR;
        }
    ...
    DEFINE_DESERIALIZE(ObSSTableTrailer)
    {
          ...
          && (OB_SUCCESS == (ret = decode_i32(buf, data_len, pos, &size_))))
      {
        if (size_ > data_len)
        {
          YYSYS_LOG(WARN, "data length is less than expected, buf=%p, "
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie trailer size, buf=%p, "
                        "buf_len=%ld, pos=%ld", buf, data_len, pos);
      }
          ...
          && (OB_SUCCESS == (ret = decode_i64(buf, data_len, pos, &range_record_size_))))
      {
        for (int64_t i = 0; i < RESERVED_LEN && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to deserialzie trailer reserved, buf=%p, "
                            "buf_len=%ld, pos=%ld, index=%ld", 
                      buf, data_len, pos, i);
            break;
          }
        ...
        {
          YYSYS_LOG(WARN, "failed to deserialzie trailer compressor name, buf=%p, "
                          "buf_len=%ld, pos=%ld", buf, data_len, pos);
          ret = OB_ERROR;
        }
        ...
        {
          YYSYS_LOG(WARN, "failed to deserialzie the fourth part of trailer, buf=%p, "
                        "buf_len=%ld, pos=%ld", 
                        buf, data_len, pos);
            }
          }
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie first part of trailer, buf=%p, "
                        "buf_len=%ld, pos=%ld", buf, data_len, pos);
      }   
    ...
    int ObSSTableTrailer::set_table_version(const int64_t version)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, version=%ld", version);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_block_count(const int64_t count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, block_count=%ld", count);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_bloom_filter_hash_count(const int64_t count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, hash_count=%ld", count);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_block_size(const int64_t size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, block_size=%ld", size);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_row_count(const int64_t count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, row_count=%ld", count);
        ret = OB_ERROR;
      }
    ...
    { 
      ...
      {
        YYSYS_LOG(WARN, "compessor name length is too big, name_length=%d,"
    ...
    int ObSSTableTrailer::set_row_value_store_style(const int16_t style)
    {
      ...
      {
        YYSYS_LOG(WARN, "unknow store style, style=%d", style);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailer::set_first_table_id(const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_id=%ld", table_id);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSSTableTrailer::set_frozen_time(const int64_t frozen_time)
    {
      ...
      if (frozen_time < 0)
      {
        YYSYS_LOG(WARN, "invalid param, frozen_time=%ld", frozen_time);
        ret = OB_ERROR;
      }
      else
      {
    ...
    bool ObSSTableTrailer::is_valid()
    {
      ...
      if (size_ <= 0)
      {
        YYSYS_LOG(WARN, "size error, size_=%d", size_);
      }
      else if (trailer_version_ < 0)
      {
        YYSYS_LOG(WARN, "trailer version error, trailer_version_=%d", trailer_version_);
      }
      else if (table_version_ < 0)
      {
        YYSYS_LOG(WARN, "table version error, table_version_=%ld", table_version_);
      }
      else if (first_block_data_offset_ != 0)
      {
        YYSYS_LOG(WARN, "first block data offset error, first_block_data_offset_=%ld", 
                  first_block_data_offset_);
      }
      else if (block_count_ < 0 )
      {
        YYSYS_LOG(WARN, "block_count_ should be greater than 0, block_count_=%ld",block_count_);
      }
      else if (block_index_record_offset_ < 0)
      {
        YYSYS_LOG(WARN, "block_index_record_offset_ should be greater than 0 [real:%ld]", 
                  block_index_record_offset_);
      }
      else if (block_index_record_size_ < 0)
      {
        YYSYS_LOG(WARN, "block_index_record_size_ should be greater than 0 [real:%ld]", 
                  block_index_record_size_);
      }
      else if (bloom_filter_hash_count_ < 0)
      {
        YYSYS_LOG(WARN, "bloom_filter_hash_count_ should be greater than 0 [real:%ld]", 
                  bloom_filter_hash_count_);
      }
      else if (bloom_filter_record_offset_ < 0)
      {
        YYSYS_LOG(WARN, "bloom_filter_record_offset_ should be greater than 0 [real:%ld]", 
                  bloom_filter_record_offset_);
      }
      else if (bloom_filter_record_size_ < 0)
      {
        YYSYS_LOG(WARN, "bloom_filter_record_size_ should be greater than 0 [real:%ld]", 
                  bloom_filter_record_size_);
      }
      else if (schema_record_offset_ < 0)
      {
        YYSYS_LOG(WARN, "schema_record_offset_ should be greater than 0 [real:%ld]", 
                  schema_record_offset_);
      }
      else if (schema_record_size_ < 0)
      {
        YYSYS_LOG(WARN, "schema_record_size_ should be greater than 0 [real:%ld]", 
                  schema_record_size_);
      }
      else if (block_size_ <= 0)
      {
        YYSYS_LOG(WARN, "blocksize_ should be greater than 0 [real:%ld]", 
                  block_size_);
      }
      else if (row_count_ < 0)
      {
        YYSYS_LOG(WARN, "row_count_ should be greater than 0 [real:%ld]", 
                  row_count_);
      }
      else if (OB_INVALID_ID == first_table_id_)
      {
        YYSYS_LOG(WARN, "first_table_id_ is invalid [real:%ld]", 
                  first_table_id_);
      }
      else if (frozen_time_ < 0)
      {
        YYSYS_LOG(WARN, "frozen_time_ should be greater than or equal to 0 [real:%ld]", 
                  frozen_time_);
      }
      else if (strlen(compressor_name_) == 0)
      {
        YYSYS_LOG(WARN, "compressor_name_ is NULL, length=%lu", strlen(compressor_name_));
      }
      else if (OB_SSTABLE_STORE_DENSE != row_value_store_style_ 
               && OB_SSTABLE_STORE_SPARSE != row_value_store_style_)
      {
        YYSYS_LOG(WARN, "row_value_store_style_ error [real:%hd,exp:%hd]", 
                  row_value_store_style_, OB_SSTABLE_STORE_DENSE);
      }
      else if (reserved_ != 0)
      {
        YYSYS_LOG(WARN, "reserved_ should be 0 [real:%d]", reserved_);
      }
      else
      {
        for (i = 0; i < RESERVED_LEN; ++i)
        {
          if (reserved64_[i] != 0)
          {
            YYSYS_LOG(WARN, "reserved64_ should be 0 [real:%ld,index:%ld]", reserved64_[i], i);
            break;
          }
        }
        if (RESERVED_LEN == i)
