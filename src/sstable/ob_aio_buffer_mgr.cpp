  ...
  namespace sstable 
  {
    ...
    int ObAIOBuffer::init()
    {
      ...
      if (NULL == misalign_buf_)
      {
        ...
        if (NULL == misalign_buf_)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for aio read buffer");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObAIOBuffer::reset()
    {
      ...
      if (FREE != state_)
      {
        YYSYS_LOG(WARN, "state is not FREE, can't reset aio buffer");
        ret = OB_ERROR;
      }
      else
      {
    ...
    void ObAIOBuffer::aio_finished(const int64_t ret_size, const int ret_code)
    {
      ...
      {
        buf_read_ = 0;
        YYSYS_LOG(WARN, "failed aio read, ret_size=%ld, buf_to_read_=%ld, "
                                   ...
                                   const bool reverse_scan)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "aio buffer manager doesn't init");
        ret = OB_ERROR;
      }
      else if (OB_INVALID_ID == sstable_id || NULL == block || block_count <= 0)
      {
        YYSYS_LOG(WARN, "invalid parameter, sstable_id=%lu, "
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "too many block to read, buf_to_read_=%ld, buf_size=%ld",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "get file info fail sstable_id=%lu", sstable_id);
            ret = OB_ERROR;
          }
          else
          {
                               ...
                               const int64_t size, char*& buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "aio buffer isn't ready, state=%d", state_);
        ret = OB_ERROR;
      }
      else //READY == state_
      {
        ...
        {
          YYSYS_LOG(WARN, "current block is out of range in aio buffer, "
        ...
        {
          YYSYS_LOG(WARN, "get block not in order, sstable_id=%lu, expected_sstable_id=%lu, "
                                     ...
                                     const bool reverse_scan)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, buffer=%p, size=%ld", buffer, size);
        ret = OB_ERROR;
      }
      else
      {
                                          ...
                                          const bool reverse_scan)
    {
      ...
      else
      {
        ...
        if (buf_to_read_ > buf_size_)
        {
          YYSYS_LOG(WARN, "too many blocks to read, buf_to_read_=%ld, buf_size=%ld",
                    buf_to_read_, buf_size_);
          buf_to_read_ = 0;
          ret = OB_ERROR;
        }
        ...
        {
          YYSYS_LOG(WARN, "buf_read_ is inconsistent with buf_to_read_, "
    ...
    io_context_t ObAIOBufferMgr::get_io_context()
    {
      ...
      if (NULL == ctx)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to setup io context, ctx=%p, error:%s", 
                    ctx, strerror(errno));
          ctx = NULL;
        }
      }
    ...
    int ObAIOBufferMgr::ensure_block_buf_space(const int64_t size)
    {
      ...
      if (size <= 0)
      {
        YYSYS_LOG(WARN, "invalid block info buffer size, size=%ld", size);
        ret = OB_ERROR;
      }
      else if (NULL == block_ || (NULL != block_ && size > block_buf_size_))
      {
        ...
        if (NULL == new_buf)
        {
          YYSYS_LOG(ERROR, "Problem allocating memory for block info buffer");
          ret = OB_ERROR;
        }
        else
        {
                               ...
                               const bool reverse_scan)
    {
      ...
      {
        YYSYS_LOG(WARN, "Invalid parameter, sstable_id=%lu, block_count=%ld", 
      ...
      {
        YYSYS_LOG(WARN, "two aio buffer aren't in FREE state, cur_aio_state=%s",
      ...
      if (OB_SUCCESS == ret && inited_)
      {
        ...
        for (int64_t i = 0; i < block_infos.block_count_ && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid block info, block_offset=%ld, block_size=%ld,"
                            ", block_idx=%ld", 
          ...
          else if (!can_fit_aio_buffer(block_infos.position_info_[i].size_))
          {
            ...
            YYSYS_LOG(WARN, "block size is too big, block_size=%ld, block_idx=%ld,"
                            "max_block_size=%ld", 
                      block_infos.position_info_[i].size_, i, 
          ...
          {
            reset();
            YYSYS_LOG(WARN, "blocks are not continuous, pre_offset=%ld, pre_size=%ld, "
                                               ...
                                               const bool check_cache)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid aio buffer index, aio_buf_idx=%ld", aio_buf_idx);
        ret = OB_ERROR;
      }
      else if (check_cache)
      {
      ...
      const bool reverse_scan)
    {
      ...
      if (!cur_range_assign && !preread_range_assign)
      {
        ...
        {
          YYSYS_LOG(WARN, "unexpected stauts, cur_range_assign=%d, "
                                                 ...
                                                 const bool reverse_scan)
    {
      ...
      for (i = start; reverse_scan ? (i >= end) : (i < end) && OB_SUCCESS == ret;)
      {
        ...
        {
          YYSYS_LOG(WARN, "get block from block cache failed, block_size=%d, "
    ...
    int ObAIOBufferMgr::init_read_range()
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "aio buffer manager doesn't initialize");
        ret = OB_ERROR;
      }
      else if(block_count_ <= 0 || NULL == block_)
      {
        YYSYS_LOG(WARN, "block of aio buffer manager doesn't initialize, "
    ...
    {
      YYSYS_LOG(DEBUG, "switch state from %s to %s", 
    ...
    {
      YYSYS_LOG(DEBUG, "stop state machine, state: %s, ret=%d", 
                                          ...
                                          const bool reverse_scan)
    {
      ...
      {
        YYSYS_LOG(WARN, "preread aio buffer isn't in FREE state, state=%s",
                  get_state_str(get_state()));
        ret = OB_ERROR;
      }
      else
                                        ...
                                        const int64_t aio_buf_idx, int64_t& timeout_us)
    {
      ...
      if (timeout_us <= 0)
      {
        YYSYS_LOG(WARN, "invalid parameter, timeout_us=%ld", timeout_us);
        ret = OB_ERROR;
      }
                                           ...
                                           char*& buffer, bool& stop)
    {
          ...
          && cur_aio_buf->is_data_valid(sstable_id, offset, size))
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "handle_ready_ready(), ready data is unavailable");
      ...
      else
      {
        ...
        preread_aio_buf->set_state(FREE);
        set_state(get_state());
        YYSYS_LOG(WARN, "handle_ready_ready(), both ready data is unavailable");
      }
                                          ...
                                          char*& buffer, bool& stop)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "handle_wait_ready(), ready data is unavailable");
          ...
          else
          {
            ...
            YYSYS_LOG(WARN, "handle_wait_ready(), wait and ready data is unavailable");
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            YYSYS_LOG(WARN, "handle_wait_free(), wait data is unavailable");
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            cur_aio_buf->set_state(FREE);
            set_state(get_state());
            YYSYS_LOG(WARN, "handle_ready_free(), ready data is unavailable");
          }
        }
      }
      else
                                                ...
                                                char*& buffer)
    {
      ...
      while (!stop)
      {
        ...
        default:
          YYSYS_LOG(WARN, "unexpected internal error");
          stop = true;
          break;
        }
      }
                                  ...
                                  const bool check_crc)
    {
      ...
      {
        YYSYS_LOG(WARN, "Invalid parameter, sstable_id=%lu, offset=%ld, size=%ld, "
                        "timeout_us=%ld", 
      ...
      if (OB_SUCCESS == ret && NULL != buffer)
      {
        if (check_crc && !from_cache)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to check block record, sstable_id=%lu "
                            "offset=%ld nbyte=%ld",
        ...
        if (OB_SUCCESS == status)
        {
          ...
          if (!cached_)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to copy block data to cache, status=%d", status);
                break;
              }
            }
          }
    ...
    ObAIOBufferMgr* ObThreadAIOBufferMgrArray::create_aio_buf_mgr()
    {
      ...
      if (NULL != buffer)
      {
        ...
        if (NULL == aio_buf_mgr)
        {
          YYSYS_LOG(WARN, "new aio buffer manager failed");
        }
      }
      else
      {
        YYSYS_LOG(ERROR, "failed to allocate memory for aio buffer manager");
        aio_buf_mgr = NULL;
      }
                                                               ...
                                                               const bool free_mgr)
    {
      ...
      for (i = 0; i < item_count_; ++i)
      {
            ...
            && (!free_mgr || (free_mgr && item_array_[i].aio_buf_mgr_->check_aio_buf_free())))
        {
          ...
          if (NULL == aio_buf_mgr)
          {
             ...
             * safe 
             */
            YYSYS_LOG(WARN, "aio buffer manager is NULL in thread aio buffer "
        ...
        for (i = 0; i < item_count_; ++i)
        {
              ...
              && FREE_FREE != item_array_[i].aio_buf_mgr_->get_state())
          {
                ...
                || FREE_FREE != item_array_[i].aio_buf_mgr_->get_state())
            {
              ...
              {
                YYSYS_LOG(WARN, "aio buffers of the disk are busy, disk_no=%ld, "
              ...
              if (yysys::CTimeUtil::getTime() - item_array_[i].timeout_time_ 
                  > DISK_WARN_AIO_TIMEOUT_US)
              {
                ...
                {
                  YYSYS_LOG(ERROR, "WARNING: maybe disk %ld is broken, wait %ldus "
                      "and it doesn't return",
                      item_array_[i].disk_no_, 
                      yysys::CTimeUtil::getTime() - item_array_[i].timeout_time_);
                }
              }
              err = OB_IO_ERROR;
            }
      ...
      if (free_mgr && NULL == aio_buf_mgr && OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "thread aio buffer manager count reach the maximum=%ld, "
                          "item_count=%ld", 
    ...
    int ObThreadAIOBufferMgrArray::wait_all_aio_buf_mgr_free(const int64_t timeout_us)
    {
           ...
           i < item_count_ && OB_SUCCESS == ret && cur_timeout_us > 0; ++i)
      {
        ...
        if (0 == item_array_[i].timeout_time_)
        {
          ...
          {
            item_array_[i].timeout_time_ = yysys::CTimeUtil::getTime();
          }
          else
          {
      ...
      {
        YYSYS_LOG(WARN, "wait all thread aio buffer manager free time out");
        ret = OB_AIO_TIMEOUT;
      }
      thread_aio_stat_.reset();
    ...
    int wait_aio_buffer()
    {
      ...
      {    
        YYSYS_LOG(WARN, "failed to wait all aio buffer manager free, "
                        "stop current thread");
        ret = OB_ERROR;
      }    
