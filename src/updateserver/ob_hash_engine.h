    ...
    class HashEngineTransHandle
    {
      ...
      private:
        bool is_valid_(const HashEngine *host) const
        {
          ...
          if (host_ != host)
          {
            YYSYS_LOG(WARN, "trans handle not match host=%p handle_host=%p", host, host_);
            bret = false;
          }
          return bret;
        };
        bool is_valid_(const HashEngine *host, const TETransType trans_type) const
        {
          ...
          {
            //YYSYS_LOG(WARN, "trans handle not match trans_type=%u handle_trans_type=%u", trans_type, trans_type_);
            bret = false;
          }
          return bret;
        };
