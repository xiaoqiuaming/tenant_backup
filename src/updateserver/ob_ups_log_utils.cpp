  ...
  namespace updateserver
  {
    ...
    int load_replay_point_func(const char* log_dir, const char* replay_point_file, int64_t& replay_point)
    {
               ...
               || rplpt_str_len >= (int)sizeof(rplpt_str))
      {
        ...
        YYSYS_LOG(ERROR, "read error[%s] or file contain invalid data[len=%d]", strerror(errno), rplpt_str_len);
      }
      else
      {
        ...
        {
          err = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "non-digit exist in replay point file[rplpt_str=%.*s]", rplpt_str_len, rplpt_str);
        }
        else if (ERANGE == errno)
        {
          err = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "replay point contained in replay point file is out of range");
        }
      }
      if (0 > open_ret)
      {
    ...
    int write_replay_point_func(const char* log_dir, const char* replay_point_file, const int64_t replay_point)
    {
      ...
      {
        err = OB_FILE_NOT_EXIST;
        YYSYS_LOG(ERROR, "open file[\"%s\"] error[%s]", rplpt_fn, strerror(errno));
      }
      else if ((rplpt_str_len = snprintf(rplpt_str, sizeof(rplpt_str), "%lu", replay_point)) < 0
               || rplpt_str_len >= (int64_t)sizeof(rplpt_str))
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "snprintf rplpt_str error[%s][replay_point=%lu]", strerror(errno), replay_point);
      }
      else if (0 > (rplpt_str_len = rplpt_file.write(rplpt_str, rplpt_str_len)))
      {
        err = OB_ERR_SYS;
        YYSYS_LOG(ERROR, "write error[%s][rplpt_str=%p rplpt_str_len=%ld]", strerror(errno), rplpt_str, rplpt_str_len);
      }
      if (0 > open_ret)
      {
        rplpt_file.close();
                          ...
                          int64_t& replay_point, int64_t& min_log_file_id, int64_t& max_log_file_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "load_replay_point(log_dir=%s)=>%d", log_dir, err);
      }
      else
      {
        ...
        if (min_log_file_id > replay_point || (max_log_file_id >0 && replay_point > max_log_file_id))
        {
          ...
          YYSYS_LOG(WARN, "min_log_file_id=%ld, max_log_file_id=%ld, replay_point=%ld", min_log_file_id, max_log_file_id, replay_point);
        }
      }
    ...
    int64_t set_counter(yysys::CThreadCond& cond, volatile int64_t& counter, const int64_t new_counter)
    {
    ...
    int64_t wait_counter(yysys::CThreadCond& cond, volatile int64_t& counter, const int64_t limit, const int64_t timeout_us)
    {
      ...
      {
        end_time_us = yysys::CTimeUtil::getTime() + timeout_us;
        ...
        while((wait_time_ms = (int)(wait_time_us/1000LL)) > 0)
        {
          ...
          else
          {
            ...
            wait_time_us = end_time_us - yysys::CTimeUtil::getTime();
          }
        }
        cond.unlock();
      }
    ...
    int get_local_max_log_cursor_func(const char* log_dir, const ObLogCursor& start_cursor, ObLogCursor& end_cursor)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid argument: log_dir=%s, log_cursor=%s", log_dir, start_cursor.to_str());
      }
      else if (OB_SUCCESS != (err = log_reader.init(&direct_reader, log_dir, start_cursor.file_id_, 0, false)))
      {
        YYSYS_LOG(ERROR, "ObLogReader init error[err=%d]", err);
      }
      while (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(ERROR, "ObLogReader read error[ret=%d]", err);
        }
        else if (OB_LAST_LOG_RUINNED == err)
        {
          YYSYS_LOG(WARN, "last_log[%s] broken!", end_cursor.to_str());
          err = OB_ITER_END;
        }
        ...
        {
          YYSYS_LOG(ERROR, "log_reader.get_cursor()=>%d",  err);
        }
      }
      if (OB_ITER_END == err)
      {
    ...
    int get_local_max_log_cursor_func(const char* log_dir, const uint64_t log_file_id_by_sst, ObLogCursor& log_cursor)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "max_log_file_id=%ld, log_file_id_by_sst=%ld", max_log_file_id, log_file_id_by_sst);
        ...
        {
          YYSYS_LOG(ERROR, "get_max_local_cursor(file_id=%ld)=>%d", log_cursor.file_id_, err);
        }
        else
        {
        ...
        if (OB_SUCCESS == err && log_cursor.log_id_ <= 0 && log_cursor.file_id_ - 1 > 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get_max_local_cursor(): try previous file[id=%ld]=>%d", log_cursor.file_id_, err);
          }
      ...
      YYSYS_LOG(INFO, "get_local_max_log_cursor(return_cursor=%s)=>%d", log_cursor.to_str(), err);
      return err;
    }
                              ...
                              )
    {
      ...
      else if (NULL == (buf = (char*)ob_malloc(len, ObModIds::OB_UPS_COMMON)))
      {
        ...
        YYSYS_LOG(ERROR, "ob_malloc(len=%ld) fail", len);
      }
      ...
      //add wangdonghui [ups_replication] 20170818 :b
      else if(end_cursor.log_id_ > clog_point)
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "commit_point(%ld) < log_replay_point(%ld); please modify commit_point or delete sst file", clog_point, end_cursor.log_id_);
      }
      //add :e
      else if (OB_ENTRY_NOT_EXIST == err)
      {
        YYSYS_LOG(WARN, "replay_local_log(start_file=%ld): NOT EXIST", start_location.file_id_);
      }
      else if (OB_SUCCESS != (err = reader.init(log_dir, true)))
      {
        YYSYS_LOG(ERROR, "located_log_reader init error[err=%d]", err);
      }
      else if (OB_SUCCESS != (err = replay_worker.start_log(end_cursor)))
      {
        YYSYS_LOG(ERROR, "replay_worker.start_log(%s)=>%d", end_cursor.to_str(), err);
      }
      else
      {
        YYSYS_LOG(INFO, "replay_local_log_func, start_log(%s)",end_cursor.to_str());
      ...
      while (!stop && OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = reader.get_log(start_location.log_id_, start_location, end_location,
                                                     buf, len, read_count)))
        {
          YYSYS_LOG(ERROR, "reader.get_log(log_id=%ld)=>%d", start_location.log_id_, err);
        }
        ...
        else if(clog_point == 0)
        {
          ...
          YYSYS_LOG(INFO, "commit point file was removed, replay_cursor(file_id:%ld,log_id:%ld,offset:%ld",
        ...
        if (end_location.log_id_ >= clog_point && start_location.log_id_ < clog_point)
        {
          ...
          {
            YYSYS_LOG(ERROR, "fail to find log from buf(log_id=%ld)=>%d", clog_point, err);
          }
          ...
          {
            YYSYS_LOG(ERROR, "WTF(log_id=%ld)=>%d", clog_point, err);
          }
        }
        ...
        {
          YYSYS_LOG(ERROR, "replay_worker.submit_batch(end_id=%ld, buf=%p[%ld])=>%d", end_id, buf, len, err);
        }
        else
        {
          start_location = end_location;
          ...
          {
            //YYSYS_LOG(INFO, "WJH_INFO replay_CMT_LOG_() submit_end_id=%ld", end_id);
            need_replay = false;
          }
          //add :e
        }
      ...
      if (!is_last_clog_batch_)
      {
        YYSYS_LOG(ERROR, "Replaying Local log, log with Commit_Point no find. end_id=%ld", end_id);
        err = OB_CLPT_NOFIND;
      }*/
      //del :e
      if (stop)
      ...
      {
        YYSYS_LOG(ERROR, "replay_worker.wait_task(end_id=%ld)=>%d", end_id, err);
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "replay_CMT_LOG_ tmp_log_cursor=%s", end_file_cursor.to_str());
        //mod :e
      }
      if (NULL != buf)
      {
      ...
      YYSYS_LOG(INFO, "replay_local_log_profile: duration=%ld", yysys::CTimeUtil::getTime() - start_time);
      return err;
    }
                            ...
                            ObLogReplayWorker& replay_worker)
    {
        ...
        else if (OB_SUCCESS != (err = replay_worker.get_real_replay_cursor(start_cursor)))
        {
            YYSYS_LOG(ERROR,"replay_worker_replay_worker_->get_real_replay_cursor=>%d", err);
        }
        else if (OB_SUCCESS != (err = reader.init(log_dir, true)))
        {
          YYSYS_LOG(ERROR, "located_log_reader init error[err=%d]", err);
        ...
        YYSYS_LOG(DEBUG, "replay_tmp_log_func start_cursor[%s] start_location.log_id_ = %ld", start_cursor.to_str(), start_location.log_id_);
        if (OB_SUCCESS != err || start_cursor.file_id_ <= 0)
        {}
        else
        {
            //YYSYS_LOG(INFO, "WJH_INFO replay_tmp_log_func start_cursor[%s] start_location.log_id_ = %ld", start_cursor.to_str(), start_location.log_id_);
            while (!stop && OB_SUCCESS == err)
            {
              if (OB_SUCCESS != (err = reader.get_log(start_location.log_id_, start_location, end_location,
                                                           buf, len, read_count)))
              ...
              //YYSYS_LOG(INFO, "WJH_INFO reader.get_log(log_id=%ld) readcount=%ld", start_location.log_id_, read_count);
              if (OB_SUCCESS != err)
              {}
              else if (OB_SUCCESS != (err = replay_worker.submit_batch(end_id, buf, read_count, RT_LOCAL)))
              {
              ...
              else
              {
                ...
                if (is_last_clog_batch_)
                {
                  //YYSYS_LOG(INFO, "WJH_INFO replay_TMP_LOG_func last_end_id=%ld", end_id);
                  break;
                }
              }
            }
            ...
            else
            {
              ...
              end_cursor.file_id_ = end_location.file_id_;
              end_cursor.offset_ = end_location.offset_;
              YYSYS_LOG(INFO, "last_clog_batch_submitted end_cursor=%s", end_cursor.to_str());
            }
            if (NULL != buf)
            {
              ob_free(buf);
            }
            YYSYS_LOG(INFO, "replay_tmp_log_profile: duration=%ld", yysys::CTimeUtil::getTime() - start_time);
       }
       return err;
    }
                               ...
                               const int64_t commit_id, const ReplayType replay_type)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "replay_single_log_func(table_mgr=%p, cmd=%d, log_data=%p, data_len=%ld)=>%d", table_mgr, cmd, log_data, data_len, err);
      }
      else
      {
        switch(cmd)
          ...
          case OB_LOG_UPS_MUTATOR:
            if (OB_SUCCESS != (err = mutator.deserialize(log_data, data_len, pos)))
            {
              YYSYS_LOG(ERROR, "UpsMutator deserialize error[ret=%d log_data=%p data_len=%ld]", err, log_data, data_len);
                     ...
                     && OB_MEM_OVERFLOW != err)
            {
              YYSYS_LOG(ERROR, "UpsTableMgr replay error[ret=%d]", err);
            }
            else if (OB_MEM_OVERFLOW == err)
            {
              YYSYS_LOG(WARN, "table_mgr->replay(log_cmd=%d, data=%p[%ld]):MEM_OVERFLOW", cmd, log_data, data_len);
            }
            break;
      ...
      if (pos != data_len)
      {
        ...
        //common::hex_dump(log_data, static_cast<int32_t>(data_len), false, YYSYS_LOG_LEVEL_WARN);
      }
      if (OB_SUCCESS != err && OB_MEM_OVERFLOW != err)
      {
        YYSYS_LOG(ERROR, "replay_log(cmd=%d, log_data=%p, data_len=%ld, commit_id=%ld)=>%d",
                              ...
                              const ObLogCursor& start_cursor, ObLogCursor& end_cursor, IObLogApplier* log_applier)
    {
      ...
      while (!stop && OB_SUCCESS == err)
      {
        //YYSYS_LOG(INFO, "log_cursor=%s", end_cursor.to_str());
        if (OB_SUCCESS != (err = log_reader.read_log(cmd, seq, log_data, data_len)) &&
            OB_FILE_NOT_EXIST != err && OB_READ_NOTHING != err && OB_LAST_LOG_RUINNED != err)
        {
          YYSYS_LOG(ERROR, "ObLogReader read error[ret=%d]", err);
        ...
        else if (NULL != log_applier)
        {
          ...
          while(!stop && OB_NEED_RETRY == err)
          {
            ...
            {
              YYSYS_LOG(ERROR, "replay_single_log()=>%d",  err);
            }
      ...
      if (NULL == log_applier)
      {
        YYSYS_LOG(DEBUG, "get_local_log_cursor(log_dir=%s, end_cursor=%s)=>%d", log_dir, end_cursor.to_str(), err);
      }
      else
      {
        YYSYS_LOG(DEBUG, "replay_local_log(log_dir=%s, end_cursor=%s)=>%d", log_dir, end_cursor.to_str(), err);
      }
      YYSYS_LOG(INFO, "replay_local_log_profile: duration=%ld", yysys::CTimeUtil::getTime() - start_time);
      return err;
    }
        ...
        else
        {
          ...
          while(OB_NEED_RETRY == err)
          {
            if (OB_SUCCESS != (err = log_applier->apply_log((LogCommand)log_entry.cmd_,
                ...
                && OB_NEED_RETRY != err && OB_CANCELED != err)
            {
              YYSYS_LOG(ERROR, "replay_log(cmd=%d, log_data=%p, data_len=%d)=>%d", log_entry.cmd_, log_data + pos, log_entry.get_log_data_len(), err);
            }
            else if (OB_NEED_RETRY == err)
            {
              usleep((useconds_t)retry_wait_time_us);
                            ...
                            const char* log_data, const int64_t data_len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "serialize_log_entry(buf=%p, len=%ld, pos=%ld, log_data=%p, data_len=%ld)=>%d",
                  buf, len, pos, log_data, data_len, err);
      }
      else if (pos + entry.get_serialize_size() + data_len > len)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(DEBUG, "pos[%ld] + entry.serialize_size[%ld] + data_len[%ld] > len[%ld]",
                  pos, entry.get_serialize_size(), data_len, len);
      }
      else if (OB_SUCCESS != (err = entry.serialize(buf, len, pos)))
      {
        YYSYS_LOG(ERROR, "entry.serialize(buf=%p, pos=%ld, capacity=%ld)=>%d",
                  buf, len, pos, err);
      }
      else
      {
                 ...
                 const char* log_data, const int64_t data_len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "generate_log(buf=%p, len=%ld, pos=%ld, log_data=%p, data_len=%ld, cursor=%s)=>%d",
                  buf, len, pos, log_data, data_len, cursor.to_str(), err);
      }
      else if (OB_SUCCESS != (err = cursor.next_entry(entry, cmd, log_data, data_len)))
      {
        YYSYS_LOG(ERROR, "cursor[%s].next_entry()=>%d", cursor.to_str(), err);
      }
      else if (OB_SUCCESS != (err = serialize_log_entry(buf, len, pos, entry, log_data, data_len)))
      {
        YYSYS_LOG(DEBUG, "serialize_log_entry(buf=%p, len=%ld, entry[id=%ld], data_len=%ld)=>%d",
                  buf, len, entry.seq_, data_len, err);
      }
      else if (OB_SUCCESS != (err = cursor.advance(entry)))
      {
        YYSYS_LOG(ERROR, "cursor[id=%ld].advance(entry.id=%ld)=>%d", cursor.log_id_, entry.seq_, err);
      }
      return err;
    }
    ...
    int set_entry(ObLogEntry& entry, const int64_t seq, const LogCommand cmd, const char* log_data, const int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "entry.fill_header(log_data=%p, data_len=%ld)=>%d", log_data, data_len, err);
      }
      return err;
    }
    ...
    int serialize_log_entry(char* buf, const int64_t len, int64_t& pos, const LogCommand cmd, const int64_t seq, const char* log_data, const int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "set_entry(seq=%ld, cmd=%d, log_data=%p, data_len=%ld)=>%d", seq, cmd, log_data, data_len, err);
      }
      else if (OB_SUCCESS != (err = serialize_log_entry(buf, len, pos, entry, log_data, data_len))
    ...
    int fill_log(char* buf, const int64_t len, int64_t& pos, ObLogCursor& cursor, const LogCommand cmd, const char* log_data, const int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "serialize_log_entry(buf[remain=%ld], entry[id=%ld], data_len=%ld)=>%d",
    ...
    int parse_log_buffer(const char* log_data, const int64_t len, int64_t& start_id, int64_t& end_id, int64_t& term)
    {
      ...
      {
        YYSYS_LOG(ERROR, "trim_log_buffer(log_data=%p[%ld])=>%d", log_data, len, err);
      }
      else if (end_pos != len)
      {
        err = OB_INVALID_LOG;
                        ...
                        )
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "trim_log_buffer(buf=%p[%ld]): invalid argument", log_data, len);
      }
      while (OB_SUCCESS == err && pos + log_entry.get_serialize_size() < len)
      {
        ...
        {
          YYSYS_LOG(ERROR, "log_entry.deserialize(log_data=%p, len=%ld, pos=%ld)=>%d", log_data, len, pos, err);
        }
        ...
        {
          err = OB_DISCONTINUOUS_LOG;
          YYSYS_LOG(WARN, "expected_id[%ld] != log_entry.seq[%ld]", real_end_id, log_entry.seq_);
        }
        //add wangjiahao [Paxos ups_replication_tmplog] 20150803 :b
        else if (last_term > 0 && last_term > log_entry.header_.get_reserved())
        {
          err = OB_DISCONTINUOUS_LOG;
          YYSYS_LOG(WARN, "last_term[%ld] != log_entry.header_.reserved[%ld]", last_term, log_entry.header_.get_reserved());
        }
        //add :e
        else
        {
      ...
      {
        YYSYS_LOG(ERROR, "parse log buf error:");
        //hex_dump(log_data, static_cast<int32_t>(len), true, YYSYS_LOG_LEVEL_WARN);
      }
                        ...
                        int64_t& start_id, int64_t& end_id, bool& is_file_end)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "trim_log_buffer(buf=%p[%ld], align_bits=%ld): invalid argument", log_data, len, align_bits);
      }
      if (!is_align(offset, align_bits))
      {
        YYSYS_LOG(WARN, "offset[%ld] is not aligned[bits=%ld], start_id[%ld]", offset, align_bits, start_id);
      }
      while (OB_SUCCESS == err && pos <= len)
      {
        ...
        {
          YYSYS_LOG(WARN, "read_eof: offset=%ld, pos=%ld, log_id=%ld", offset, pos, real_end_id);
          break;
        }
        else if (is_file_end)
        {
        ...
        {
          YYSYS_LOG(ERROR, "log_entry.check_data_integrity()=>%d, start_id[%ld], expected_id[%ld], log_entry.seq[%ld]",
                    err, start_id, real_end_id, log_entry.seq_);
        }
        else if (real_end_id > 0 && real_end_id != (int64_t)log_entry.seq_)
        {
          err = OB_DISCONTINUOUS_LOG;
      ...
      {
        YYSYS_LOG(WARN, "trim_log_buffer(offset=%ld, align=%ld, len=%ld, end_pos=%ld): not found aligned pos",
    ...
    int load_file_for_log_func(const char* file_dir, const char* file_name, int64_t& data)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "Arguments are invalid[file_dir=%p]", file_dir);
      }
      ...
      else if (!FileDirectoryUtils::exists(file_fn))
      {
        err = OB_FILE_NOT_EXIST;
        data = 0;
        YYSYS_LOG(WARN, "file_for_log file[\"%s\"] does not exist.", file_fn);
      }
      else if (0 > (open_ret = file_file.open(file_fn, O_RDONLY)))
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(WARN, "open file[\"%s\"] error[%s]", file_fn, strerror(errno));
      }
             ...
             || file_str_len >= (int)sizeof(file_str))
      {
        ...
        YYSYS_LOG(ERROR, "read error[%s] or file contain invalid data[len=%d]", strerror(errno), file_str_len);
      }
      else
      {
        ...
        {
          err = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "non-digit exist in file_for_log file[file_str=%.*s]", file_str_len, file_str);
        }
        else if (ERANGE == errno)
        {
          err = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "data contained in file_for_log file is out of range");
        }
      }
      if (0 > open_ret)
      {
    ...
    int write_file_for_log_func(const char* file_dir, const char* file_name, const int64_t data)
    {
      ...
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "snprintf file_str error[%s][data=%ld]", strerror(errno), data);
      }
    ...
    int ObLogTerm::init(const char* file_dir, const char* file_name)
    {
      ...
      {
        YYSYS_LOG(ERROR, "load_file_for_log_func(file_dir=%s, file_name=%s)=>%d", file_dir, file_name, err);
      }
    ...
    int ObLogTerm::write(const int64_t data)
    {
      ...
      else
      {
          ...
          {
            YYSYS_LOG(ERROR, "write_file_for_log_func(file_dir=%s, file_name=%s)=>%d", file_dir_, file_name_, err);
          }
          else
          {
            term_value_ = data;
...
int get_replay_cursor_from_file(const char* log_dir, ObLogCursor end_cursor, int64_t clog_point, ObLogCursor& replay_cursor)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid argument: log_dir=%s", log_dir);
      }
      else if (NULL == (buf = (char*)ob_malloc(len, ObModIds::OB_UPS_COMMON)))
      {
        err = OB_ALLOCATE_MEMORY_FAILED;
