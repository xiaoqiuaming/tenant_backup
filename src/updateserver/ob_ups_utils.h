    ...
    inline void rebind_cpu(const int64_t cpu, const char *file, const int line)
    {
          ...
          || bind_cpu != cpu)
      {
            ...
            && core_num > cpu)
        {
          ...
          YYSYS_LOG(INFO, "[%s][%d] rebind setaffinity tid=%ld ret=%d cpu=%ld",
    ...
    inline void rebind_cpu(const int64_t cpu_start, const int64_t cpu_end, volatile uint64_t &cpu, const char *file, const int line)
    {
          ...
          || bind_cpu_end != cpu_end)
      {
            ...
            && core_num > cpu_end)
        {
          ...
          CPU_SET(cpu2bind, &cpuset);
          int ret = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
          YYSYS_LOG(INFO, "[%s][%d] rebind setaffinity tid=%ld ret=%d cpu=%ld start=%ld end=%ld",
...
#define OB_UPS_MODIFY_TIME_COLUMN_ID(table_id) \
    ...
    class RowkeyInfoCache
    {
      ...
      public:
        ...
        virtual const common::ObRowkeyInfo *get_rowkey_info(const uint64_t table_id, const CommonSchemaManager *sm = NULL, int64_t query_version = 0) const;
        //mod:e
