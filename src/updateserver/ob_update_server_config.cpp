...
int ObUpdateServerConfig::init()
{
  ...
  trans_thread_num = calc_thread_num(get_cpu_num());
  replay_worker_num = calc_thread_num(get_cpu_num());
  distributed_trans_thread_num = calc_thread_num(get_cpu_num());
...
int64_t ObUpdateServerConfig::calc_thread_num(const int64_t cpu_num)
{
    return cpu_num <=64 ? cpu_num - 2 : 64;
}
...
int ObUpdateServerConfig::read_config()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "Reload server config fail! ret: [%d]", ret);
  }
...
int ObUpdateServerConfig::check_all()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "Check server config fail! ret: [%d]", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "{blockcache_size + blockindex_cache + table_memory_limit}"
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "active_mem_limit=%s cannot larger than table_memory_limit=%s",
      ...
      {
        YYSYS_LOG(WARN, "active_mem_limit=%s cannot less than MIN_ACTIVE_MEMTABLE_WITH_HASH=%ld",
    ...
    if (NULL == compressor)
    {
      YYSYS_LOG(ERROR, "cannot load compressor library name=[%s]",
...
void ObUpdateServerConfig::auto_config_memory(const int64_t total_memory_limit_gb)
{
  ...
  YYSYS_LOG(INFO, "[auto_config_memory] "
...
int64_t ObUpdateServerConfig::get_total_memory_limit(const int64_t phy_mem_size_gb)
{
  ...
  YYSYS_LOG(INFO, "phy_mem_size=%ld total_memory_limit=%ld", phy_mem_size_gb, ret);
