  ...
  namespace updateserver
  {
    ...
    struct ObLogLocationInfo
    {
      ...
      int restore_from_session(ObSessionBuffer& buf)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid session: maybe master changed!");
        }
        else
        {
                      ...
                      ObFetchLogReq& req, ObFetchedLog& fetched_log)
    {
      ...
      {
        YYSYS_LOG(ERROR, "location.deserialize()=>%d", err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "get_from_log_buffer(start_id=%ld)=>%d", req.start_id_, err);
      }
      ...
      {} // read from buf
               ...
               && OB_DATA_NOT_SERVE != err)
      {
        YYSYS_LOG(ERROR, "pos_log_reader.get_log(start_id=%ld)=>%d", req.start_id_, err);
      }
      else if (OB_SUCCESS == err)
      {
      ...
      {
        //mod pangtianze [Paxos ups_replication] 20170809:b
        //mod wangdonghui
        //YYSYS_LOG(WARN, "pos_log_reader.get_log(): DATA_NOT_SERVE, master has no such log(%ld). Logfile maybe be deleted.", req.start_id_);
        YYSYS_LOG(WARN, "pos_log_reader.get_log(): DATA_NOT_SERVE, master has no such log(%ld). \
        Logfile maybe be deleted, or Maybe slave has more recent log than master", req.start_id_);
      }
      ...
      YYSYS_LOG(DEBUG, "get_local_log(log_id=%ld, read_count=%ld)=>%d", req.start_id_, read_count, err);
      return err;
    }
    ...
    int ObCachedPosLogReader:: get_log(ObFetchLogReq& req, ObFetchedLog& result)
    {
      ...
      if (!is_inited())
      {
        err = OB_NOT_INIT;
               ...
               && OB_DATA_NOT_SERVE != err)
      {
        YYSYS_LOG(ERROR, "get_local_log(req.start_id=%ld)=>%d", req.start_id_, err);
      }
      return err;
    }
    ...
    int ObCachedRegionLogReader::get_log(int64_t start_id, int64_t end_id, char *data, int64_t& data_len, int64_t max_data_len, int64_t& old_pos, bool disk_only/*=false*/)
    {
      ...
      else if (!disk_only)
      {
          if (OB_SUCCESS != (err = get_from_log_buffer_2(log_buffer_, OB_DIRECT_IO_ALIGN_BITS, old_pos, new_pos, start_id, end_id, buf, read_count, max_data_len))
               && OB_DATA_NOT_SERVE != err)
          {
            YYSYS_LOG(ERROR, "get_from_log_buffer(start_id=%ld)=>%d", start_id, err);
          }
          else if (OB_SUCCESS == err)
          { // read from buf
              old_pos = new_pos;
              ...
              {
                  YYSYS_LOG(DEBUG,"read from buff [start_id,end_id]=[%ld,%ld], diff=%ld, date_len = %ld",
                            start_id, end_id, end_id - start_id, read_count);
              }
          }
          else if (OB_SUCCESS != err)
          {
              YYSYS_LOG(WARN, "read from buff [start_id,end_id]=[%ld,%ld], diff=%ld, date_len = %ld, err=%d",
                        start_id, end_id, end_id - start_id, read_count, err);
          }
      }
      ...
      {}
               ...
               && OB_DATA_NOT_SERVE != err)
      {
        YYSYS_LOG(ERROR, "pos_log_reader.get_log(start_id=%ld)=>%d", start_id, err);
      }
      else if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "wangjiahao_region_log Region_log_reader.get_log(start_id=%ld end_id=%ld): err=%d", start_id, end_id, err);
      }
      else
      { //read from disk
          data_len = read_count;
          ...
          if(REACH_TIME_INTERVAL(100 * 1000))
          {
              int64_t nextpos = 0;
              int64_t replayid = 0;
              if(OB_SUCCESS != log_buffer_->get_next_entry(log_buffer_->get_replay_pos(), nextpos, replayid))
              {
                  YYSYS_LOG(DEBUG,"read from disk [start_id,end_id]=[%ld,%ld], diff=%ld, date_len = %ld",
                            start_id, end_id, end_id - start_id, read_count);
              }
              else
              {
                  YYSYS_LOG(DEBUG,"read from disk [start_id,end_id]=[%ld,%ld], diff=%ld, date_len = %ld,"
                            " [replay_id,end_id]=[%ld,%ld]",
                            start_id, end_id, end_id - start_id, read_count,
                            replayid, log_buffer_->get_end_id());
              }
          }
      }
      return err;
