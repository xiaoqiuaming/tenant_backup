...
namespace oceanbase
{
  namespace updateserver
  {
    ObUpsMergerRpcProxy::ObUpsMergerRpcProxy(const int64_t retry_times, const int64_t timeout,
        ...
        ObUpsCache& ups_cache) : ObMergerRpcProxy(retry_times, timeout, root_server, merge_server), table_mgr_(table_mgr), ups_cache_(ups_cache)
    {
      // does nothing
    }
        ...
        ObMergerTabletLocation& addr, ObScanner& scanner, ObIterator*& it_out)
    {
      ...
      else
      {
        ...
        if (NULL == cell_info)
        {
          YYSYS_LOG(WARN, "invalid param, the first cell info is NULL");
          err = OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get from cache, err=%d", err);
          }
          else if (OB_ENTRY_NOT_EXIST == err)
          {
            // get cell data from chunk server
            YYSYS_LOG(DEBUG, "not exist in cache, get from cs");
          ...
          else
          {
            ...
            if (compare_row)
            {
              ...
              {
                YYSYS_LOG(WARN, "cell value is not extend type, %s", print_obj(value.value));
                err = OB_ERROR;
              }
              ...
              {
                YYSYS_LOG(WARN, "invalid value, ext_val=%ld", value.value.get_ext());
                err = OB_ERROR;
              }
            }
            else
        ...
        const ObGetParam& get_param, ObScanner& scanner)
    {
      ...
      if (NULL == cell)
      {
        YYSYS_LOG(ERROR, "check first cell failed:cell[%p]", cell);
      ...
      else
      {
        ...
        if (OB_INVALID_START_VERSION == err)
        {
          ...
          else
          {
            ...
            YYSYS_LOG(WARN, "invalid version, start_version=%c%ld,%ld%c, err=%d",
        ...
        {
          YYSYS_LOG(WARN, "failed to call table_mgr#scan, err=%d", err);
        }
      }

      if (OB_SUCCESS == err && get_param.get_cell_size() > 0 && scanner.is_empty())
      {
        YYSYS_LOG(WARN, "update server error, response result with zero cell");
