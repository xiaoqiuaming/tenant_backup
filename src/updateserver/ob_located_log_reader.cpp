  ...
  namespace updateserver
  {
                         ...
                         char* buf, const int64_t len, int64_t& read_count, const bool dio = true)
    {
      ...
      else if (OB_SUCCESS != (err = file_reader.open(fname, dio)))
      {
        ...
        {
          YYSYS_LOG(WARN, "file_reader.open(%s/%ld)=>%d", path, file_id, err);
        }
        else
        {
          err = OB_SUCCESS;
                                     ...
                                     char* buf, const int64_t len, int64_t& read_count, bool& is_file_end)
    {
      ...
      {
        YYSYS_LOG(ERROR, "read_log_file_by_location(%s/%ld, offset=%ld)=>%d", log_dir_, file_id, offset, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "trim_log_buffer()=>%d", err);
      }
      return err;
    }
