...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObRowDescPrepare::set_rowkey_size(ObUpsTableMgr* table_mgr, ObRowDesc* row_desc)
    {
      ...
      {
        YYSYS_LOG(WARN, "row_desc_.get_tid_cid(0)=>%d", err);
      }
      else if (NULL == (rki = get_rowkey_info(*table_mgr, table_id)))
      {
        YYSYS_LOG(WARN, "get rowkey info fail, table_id=%lu", table_id);
        err = OB_SCHEMA_ERROR;
      }
      else
      {
                           ...
                           ObPhyOperator*& result)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "open(session_ctx=%p, table_mgr=%p, get_param=%p):INVALID_ARGUMENT",
      ...
      {
        YYSYS_LOG(WARN, "open(%s)=>%d", to_cstring(*get_param), err);
      }
      else
      {
      ...
      else if (OB_SUCCESS != (err = ObNewScannerHelper::get_row_desc(*get_param, true, row_desc_)))
      {
        ...
        {
          YYSYS_LOG(USER_ERROR, "Duplicate entry for key \'PRIMARY\'");
          err = OB_ERR_PRIMARY_KEY_DUPLICATE;
        }
        else
        {
          YYSYS_LOG(WARN, "get_row_desc()=>%d", err);
        }
      }
      else if (OB_SUCCESS != (err = set_rowkey_size(table_mgr, &row_desc_)))
      {
        YYSYS_LOG(WARN, "set_rowkey_size()=>%d", err);
      }
      return err;
    }
    ...
    int ObIncGetIter::close()
    {
      ...
      if (NULL != get_param_)
      {
        ...
        {
          YYSYS_LOG(WARN, "result->close()=>%d", err);
        }
        if (OB_SUCCESS != (err = ObTableListQuery::close()))
        {
          YYSYS_LOG(WARN, "close()=>%d", err);
        }
    ...
    int get_last_cell_idx_of_row(const ObGetParam& get_param, const int64_t start_idx, int64_t& end_idx)
    {
      ...
      else if (NULL == (first_cell = get_param[cur_idx]))
      {
        ...
        YYSYS_LOG(WARN, "get_param[%ld]=>%d", cur_idx, err);
      }
      for(; OB_SUCCESS == err && cur_idx < get_param.get_cell_size(); cur_idx++)
      {
        ...
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "cellinfo null pointer idx=%ld", cur_idx);
        }
    ...
    int ObIncGetIter::get_next_row(const common::ObRow *&row)
    {
      ...
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(WARN, "table_mgr == NULL || sessio_ctx == NULL, maybe you don't init");
      }
      else if (OB_SUCCESS != (err = get_last_cell_idx_of_row(*get_param_, last_cell_idx_, end_cell_idx)))
      {
        if (OB_ITER_END != err)
        {
          YYSYS_LOG(WARN, "get_row(last_cell_idx=%ld)=>%d", last_cell_idx_, err);
        }
      }
      else
      {
      ...
      else if (OB_SUCCESS != (err = query(&get_query_, &row_desc_, result_)))
      {
        if (!IS_SQL_ERR(err))
        {
          YYSYS_LOG(WARN, "query()=>%d", err);
        }
      }
      else if (OB_SUCCESS != (err = result_->open()))
      {
        YYSYS_LOG(WARN, "result->open()=>%d", err);
      }
      else if (OB_SUCCESS != (err = result_->get_next_row(row)))
      {
        YYSYS_LOG(WARN, "result->get_next_row()=>%d", err);
      }
      else
      {
      ...
      {
          YYSYS_LOG(DEBUG,"mul_del::debug,ups inc row=[%s],param=[%s],ret=%d",
                            ...
                            const ObScanParam* scan_param, ObPhyOperator*& result)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "open(session_ctx=%p, table_mgr=%p, scan_param=%p): INVALID_ARGUMENT",
      ...
      {
        YYSYS_LOG(WARN, "open(%s)=>%d", to_cstring(*scan_param), err);
      }
      else
      {
    ...
    int ObUpsIncScan::open()
    {
      ...
      else if (ObIncScan::ST_MGET == scan_type_)
      {
        ...
        {
          YYSYS_LOG(WARN, "get_iter_.open()=>%d", err);
        }
      }
      else
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(WARN, "open(): result == NULL");
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObUpsIncScan::close()
    {
      ...
      if (NULL == result_)
      {
        //YYSYS_LOG(WARN, "result == NULL, no need to close");
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "scan_iter.close()=>%d", err);
        }
        if (ObIncScan::ST_MGET == scan_type_
            && OB_SUCCESS != (err = get_iter_.close()))
        {
          YYSYS_LOG(WARN, "get_iter.close()=>%d", err);
        }
        result_ = NULL;
      }
      return err;
    ...
    int ObUpsIncScan::get_next_row(const ObRow *&row)
    {
      ...
      // add by maosy [MultiUps 1.0] [#20] 20170324 e
      else if(is_hot_update_ &&OB_ITER_END != err )
      {
          int64_t row_action_flag = 0;
          if(OB_SUCCESS != (err = row->get_action_flag (row_action_flag)))
          {
              YYSYS_LOG(WARN,"failed to get action flag cell ,ret = %d",err);
          }
          else if ( row_action_flag ==ObActionFlag::OP_ROW_DOES_NOT_EXIST)
          {
              err= OB_ITER_END;
              row = NULL ;
          }
    ...
    int ObUpsIncScan::get_row_desc(const common::ObRowDesc *&row_desc) const
    {
      ...
      {
        YYSYS_LOG(WARN, "result->get_row_desc()=>%d", err);
      }
      return err;
    }
