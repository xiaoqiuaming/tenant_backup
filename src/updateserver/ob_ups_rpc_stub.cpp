  ...
  namespace updateserver
  {
        ...
        CommonSchemaManagerWrapper& schema_mgr, const bool only_core_tables, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "serialize timestamp failed:timestamp[%ld], err[%d].",
              timestamp, err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "serialize fail. err=%d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "send request to root server for fetch schema failed"
        ...
        const int64_t table_start_pos,const int64_t column_start_pos, CommonSchemaManager& schema_mgr)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "serialize timestamp failed:timestamp[%ld], err[%d].",timestamp, err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        ObSlaveSyncType slave_type, ObUpsFetchParam& fetch_param, uint64_t &max_log_seq, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "ObSlaveInfo serialize error, err=%d", err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "slave_type serialize error, err=%d", err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to send slave register to master. err=%d", err);
        }
      }
      int64_t pos = 0;
      if (OB_SUCCESS == err)
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to deserialize fetch_param. err=%d", err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to decode max_log_seq, err=%d", err);
        }
        else
        {
          YYSYS_LOG(INFO, "slave_register_followed get master max log_id = %ld, master_ups=%s", max_log_seq, master.to_cstring());
        }
      }
      return err;
    }
        ...
        ObUpsFetchParam& fetch_param, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send request to register failed err[%d].", err);
        }
      }
        ...
        uint64_t &log_id_res, uint64_t &log_seq_res, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "log_id serialize error, err=%d", err);
        }
        ...
        {
          YYSYS_LOG(WARN, "log_seq serialize error, err=%d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "send request to register failed err[%d]. lsync_addr=%s", err, master.to_cstring());
        }
      }
        ...
        const ObServer& ups_master, const int64_t schema_timestamp, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "send freeze memtable failed, err[%d].", err);
        }
      }
    ...
    */
        ...
        char* &log_data, int64_t &log_len, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = serialization::encode_i64(data_buff.get_data(), data_buff.get_capacity(),
                data_buff.get_position(), (int64_t)log_id)))
        {
          YYSYS_LOG(WARN, "Serialize log_id error, err=%d", err);
        }
        else if (OB_SUCCESS != (err = serialization::encode_i64(data_buff.get_data(), data_buff.get_capacity(),
                data_buff.get_position(), (int64_t)log_seq)))
        {
          YYSYS_LOG(WARN, "Serialize log_seq error, err=%d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        int64_t send_bgn_time = yysys::CTimeUtil::getMonotonicTime();
        ...
        {
          YYSYS_LOG(ERROR, "send request to lsync failed err[%d].", err);
        }
        else if (OB_RESPONSE_TIME_OUT == err)
        {
          int64_t send_end_time = yysys::CTimeUtil::getMonotonicTime();
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(ERROR, "result_code is error: result_code_=%d", result_code.result_code_);
        }
        else if (OB_SUCCESS != err)
        {
          YYSYS_LOG(DEBUG, "fetch_lsync()=>%d", err);
        }
        else
        {
          if (OB_SUCCESS != (err = serialization::decode_i64(data_buff.get_data(), data_buff.get_position(), pos, (int64_t*)&log_len)))
          {
            YYSYS_LOG(WARN, "Deserialize log_len error, err=%d", err);
          }
          else
          {
            log_data = data_buff.get_data() + pos;
            YYSYS_LOG(DEBUG, "log_data=%p log_len=%ld", log_data, log_len);
          }
        }
      }
    ...
    int ObUpsRpcStub:: fetch_log(const ObServer &master, const ObFetchLogReq& req, ObFetchedLog& fetched_log, const int64_t timeout_us)
    {
      ...
      {
        YYSYS_LOG(ERROR, "req.serialize()=>%d", err);
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        //YYSYS_LOG(DEBUG, "send_request(client_mgr->send_request()=>%d", err);
        if (OB_SUCCESS != err && OB_RESPONSE_TIME_OUT != err)
        {
          YYSYS_LOG(ERROR, "send request to master failed err[%d].", err);
        }
        else if (OB_RESPONSE_TIME_OUT == err)
        {
          int64_t send_end_time = yysys::CTimeUtil::getMonotonicTime();
    ...
    int ObUpsRpcStub ::fill_log_cursor(const ObServer &master, ObLogCursor& log_cursor, const int64_t timeout_us)
    {
      ...
      {
        YYSYS_LOG(ERROR, "log_cursor.serialize()=>%d", err);
      }
                                                              ...
                                                              timeout_us, data_buff)))
      {
        ...
        {
          YYSYS_LOG(WARN, "send request to master failed err[%d].", err);
        }
      }
      else if (OB_SUCCESS != (err = result_code.deserialize(data_buff.get_data(), data_buff.get_position(), pos)))
      {
...
//      if (NULL == obj)
//      {
//        YYSYS_LOG(ERROR, "get ObUpdateServerMain instance failed.");
//      }
//      else
//      {
...
//        rpc_buffer = server.get_rpc_buffer();
    ...
    int ObUpsRpcStub :: ups_report_slave_failure(const common::ObServer &slave_server, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          err = OB_ERROR;
          YYSYS_LOG(WARN, "fail to get ups_main");
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to serialize failure_info, err = %d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to send request. err = %d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "deserialize result_code failed:pos[%ld], err[%d].", pos, err);
        }
        else
        {
          err = result_code.result_code_;
    ...
    //mod peiouya [MultiUPS] [UPS_Manage_Function] 20150429:b
    ...
    int ObUpsRpcStub :: get_inst_master_ups(const common::ObServer &root_server, common::ObServer &ups_master,
                                            const int64_t paxos_id, const int64_t timeout_us)
    //mod 20150429:e
    {
      int err = OB_SUCCESS;
      ObDataBuffer data_buff;
      if (NULL == client_mgr_)
      {
        YYSYS_LOG(WARN, "INVALID argument. client_mgr_=%p", client_mgr_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      if (OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = get_master_ups_info_by_paxos_id(master_inst_rs, ups_master, paxos_id, timeout_us)))
        {
                                        ...
                                        const int64_t timeout)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to send request. ret: [%d]", ret);
        }
      }
      int64_t pos = 0;
      if (OB_SUCCESS == ret)
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to deserialize the response. ret: %d", ret);
        }
      }
    ...
    {
    YYSYS_LOG(WARN, "serialize slave_failure_info err. err= %d", err);
    }
    }
    ...
    if (OB_SUCCESS != err)
    {
    YYSYS_LOG(WARN, "fail to send request to rootserver. err = %d", err);
    }
    }
                                     ...
                                     int64_t &cluster_id, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "Serialize msg_register error, err=%d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "send request  failed, err[%d], rootserver addr = %s", err, root_server.to_cstring());
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to decode lease_renew_time. err=%d", err);
        }
        else
        {
          YYSYS_LOG(INFO, "get lease_renew_time = %ld", lease_renew_time);
        }
      }
      if (OB_SUCCESS == err && pos < data_buff.get_position())
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to decode cluster id. err=%d", err);
        }
        else
        {
          YYSYS_LOG(INFO, "get cluster id = %ld", cluster_id);
        }
      }
    ...
    int ObUpsRpcStub::send_keep_alive(const common::ObServer &slave_node)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to post request to slave [%s], err = %d", slave_node.to_cstring(), err);
        }
      }
      return err;
    }
    ...
    int ObUpsRpcStub::renew_lease(const common::ObServer &rootserver, const common::ObMsgUpsHeartbeatResp& msg)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to post request to rootserver [%s], err = %d", rootserver.to_cstring(), err);
        }
      }
      return err;
    }
        ...
        bool &merged, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "serialize version failed:version[%lu], err[%d].",
              version, err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send request to root server for check table merged failed"
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "deserialize result code from buff failed"
    ...
    int ObUpsRpcStub::notify_rs_trigger_event(const ObServer &rootserver, const ObTriggerMsg &msg, const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "fail to send request to rootserver [%s], err = %d", rootserver.to_cstring(), err);
        }
      }
      return err;
    }
