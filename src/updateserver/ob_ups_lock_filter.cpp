...
namespace oceanbase
{
  namespace updateserver
  {
    int ObUpsLockFilter::open()
    {
      ...
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "child_op == NULL");
      }
      else if (OB_SUCCESS != (err = child_op_->open()))
      {
        YYSYS_LOG(ERROR, "child_op->open()=>%d", err);
      }
      return err;
    }
    ...
    int ObUpsLockFilter::get_row_desc(const common::ObRowDesc *&row_desc) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "child_op->get_row_desc()=>%d", err);
      }
      return err;
    }
    ...
    int ObUpsLockFilter::lock_row(const uint64_t table_id, const common::ObRowkey& rowkey)
    {
      ...
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "lock_row(session_descriptor=%u, memtable=%p, lock_info=%p): not init",
      ...
      {
        YYSYS_LOG(ERROR, "memtable->rdlock_row()=>%d", err);
      }
      else if (NULL != value && value->get_cur_version() > session_.get_trans_id())
      {
        err = OB_ROW_MODIFIED;
        YYSYS_LOG(DEBUG, "row_version[%ld] > session.version[%ld]", value->get_cur_version(), session_.get_trans_id());
      }
      return err;
    }
