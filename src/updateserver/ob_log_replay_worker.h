...
namespace oceanbase
{
  ...
  {
    class ObLogReplayWorker: public yysys::CDefaultRunnable
    {
      public:
        ...
        class ApplyWorker: public common::S2MQueueThread
        {
          public:
            ...
            virtual ~ApplyWorker()
            {}
            void *on_begin() { SET_THD_NAME("ups-logreplay"); return NULL;};
            virtual void handle(void *task, void *pdata)
            {
              ...
              {
                YYSYS_LOG(WARN, "replay_worker_->handle_apply()=>%d", err);
              }
            }
      ...
      public:
        ...
        int init(IAsyncLogApplier* log_applier,
                 ...
                 const bool replay_queue_rebalance, //[bugfix replay queue rebalance
      ...
      public:
        ...
        int get_real_replay_cursor(ObLogCursor &cursor) const; //[bugfix for replay tmp log]

        //add wangdonghui [ups_replication] 20161221 :b
        int reset_replay_cursor() const;
        //add :e

        int set_real_replay_cursor(const ObLogCursor &cursor);//[bugfix for replay tmp log]
        int set_replay_cursor();//[bugfix for replay tmp log]
        int reset_real_replay_cursor();//[bugfix for replay tmp log]
        ...
        //add :e
      public:
        int64_t get_next_submit_log_id() const
        { return next_submit_log_id_; }
        int64_t get_next_commit_log_id() const
        { return next_commit_log_id_; }
        int64_t get_next_flush_log_id() const
        { return next_flush_log_id_; }
        int64_t get_last_barrier_log_id() const
        { return last_barrier_log_id_; }
      protected:
      ...
      private:
        ...
        ObLogCursor real_replay_cursor_;

        yysys::CThreadCond commit_log_id_cond_;
        yysys::CThreadCond flush_log_id_cond_;
