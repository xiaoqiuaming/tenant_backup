...
int ObSlaveInfo::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
{
  ...
  if (err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "ObSlaveInfo serialize error, err=%d buf=%p pos=%ld buf_len=%ld serialize_size=%ld",
...
int ObSlaveInfo::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "ObSlaveInfo deserialize error, err=%d buf=%p buf_len=%ld pos=%ld",
        err, buf, buf_len, pos);
  }
...
void ObUpsFetchRunnable::run(yysys::CThread* thread, void* arg)
{
  int ret = OB_SUCCESS;
  ...
  YYSYS_LOG(INFO, "ObUpsFetchRunnable finished[stop=%d ret=%d]", _stop, ret);
}
...
int ObUpsFetchRunnable::init(const ObServer& master, const char* log_dir, const ObUpsFetchParam &param, ObRoleMgr *role_mgr, common::ObLogReplayRunnable *replay_thread, SSTableMgr *sstable_mgr)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObFetchRunnable init error, ret=%d log_dir=%s role_mgr=%p replay_thread=%p",
