...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    //add by maosy [MultiUps 1.0][secondary index optimize]20170401 b:
        ...
        virtual int apply(RWSessionCtx &session_ctx, common::ObIterator &iter, const common::ObDmlType dml_type, const ObApplyState is_index = NO_INDEX) = 0;
		//add by maosy e
    ...
    class ObUpsTableMgr : public ObIUpsTableMgr
    {
      ...
      public:
        ...
        //int init();
        int init(uint64_t start_major_version);
        //mod 20150521:e
      ...
      public:
        ...
        int rollback(RWSessionCtx &session_ctx); //add peiouya [secondary_index_OPTI_CHECKSUNxxxx] 20180314
        //add by maosy [MultiUps 1.0][secondary index optimize]20170401 b:
        //int apply(RWSessionCtx &session_ctx, common::ObIterator &iter, const common::ObDmlType dml_type);
        int apply(RWSessionCtx &session_ctx, common::ObIterator &iter, const common::ObDmlType dml_type, const ObApplyState is_index =NO_INDEX);
		// add by maosy e
        ...
        {return schema_mgr_.get_version();}

        /**
         * @brief reference of tmp_schema_mgr_
         */
        CommonSchemaManagerWrapper &get_tmp_schema()
        {return tmp_schema_mgr_;}
        ...
        bool lock_tmp_schema()
        {return __sync_bool_compare_and_swap(&tmp_schema_mutex_,0,1);}
        bool unlock_tmp_schema()
        {return __sync_bool_compare_and_swap(&tmp_schema_mutex_,1,0);}
        //add:e
        int get_active_memtable_version(uint64_t &version);

        int get_last_truncate_timestamp_in_active(uint32_t uid, uint64_t table_id, int64_t &timestamp);//add zhaoqiong[244_bugfix_]
        ...
        uint64_t get_cur_major_version();
        //add 20160229:e

        //[499]
        uint64_t get_merged_version()
        { return table_mgr_.get_merged_version();}
        ...
        {
          YYSYS_LOG(INFO, "replay checksum flag switch from %s to %s", STR_BOOL(check_checksum_), STR_BOOL(if_check));
      ...
      public:
        ...
        int freeze_memtable(const TableMgr::FreezeType freeze_type, uint64_t &frozen_version, bool &report_version_changed,
                            const common::ObPacket *resp_packet = NULL, const uint64_t limit_version = 0);
        //mod 20150521:e
        ...
        void log_table_info();
        void get_ups_used_mem(int64_t &mem_size);//add wangyao [nmon_ups_memory]
        ...
        int fill_prepare_log(ObUpsMutator &ups_mutator,const common::ObTransID& coo_trans_id, TraceLog::LogBuffer &tlog_buffer);
        int fill_stat_log(const PartCommitInfo& commit_info, const bool commit);
        //add 20150701:e
      ...
      private:
        ...
        /**
         * overload ,add a boolean param to judge if there is a ObMerger object,
         * each iter has a version for it's table_item,in MultiUPS,each row has a version
         */
        template<class T>
        int add_to_scanner_(common::ObIterator& ups_merger, T& scanner,
            int64_t& row_count, const int64_t start_time, const int64_t timeout, bool has_merger_iter, const int64_t result_limit_size = UINT64_MAX);
        //add 20151228:e
      private:
        ...
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        int fill_prepare_log_(ObUpsMutator &ups_mutator,const common::ObTransID& coo_trans_id, TraceLog::LogBuffer &tlog_buffer);
        //add 20150701:e
