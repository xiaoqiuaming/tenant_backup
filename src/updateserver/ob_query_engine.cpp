...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int QueryEngine::set(const TEKey &key, TEValue *value)
    {
      ...
      {
        YYSYS_LOG(INFO, "value null pointer");
        ret = OB_ERROR;
      }
      //add zhaoqiong [Truncate Table]:20160318:b
      else if (0 == key.row_key.length())
      {
        ...
        {
          YYSYS_LOG(WARN, "put to keybtree fail btree_ret=%d [%s] [%s]",
        ...
        else
        {
          ...
          YYSYS_LOG(DEBUG, "insert to btree succ %s %s", key.log_str(), value->log_str());
        }
      }
      //add:e
      else if (OB_SUCCESS != (hash_ret = keyhash_.insert(hash_key, value)))
      {
        ...
        {
          YYSYS_LOG(WARN, "put to keyhash fail hash_ret=%d value=%p [%s] [%s]",
      ...
      else
      {
        ...
        {
          value->index_stat |= IST_BTREE_INDEX;
          YYSYS_LOG(DEBUG, "insert to hash and btree succ %s %s", key.log_str(), value->log_str());
        }
      }
      return ret;
    }

    //add peiouya [bug fix secondary index opti when rollback]:b
    int QueryEngine::update_tevalue_keybtree_and_keyhash(const TEKey &key, TEValue *value)
    {
        int ret = OB_SUCCESS;
        int btree_ret = ERROR_CODE_OK;
        int hash_ret = OB_SUCCESS;
        ...
        else if(OB_SUCCESS != (hash_ret = keyhash_.update_value(hash_key, value)))
        {
            YYSYS_LOG(ERROR,"failed to update hash_key[%s]'s value ptr, ret=%d", to_cstring(key), hash_ret);
            ret = hash_ret;
        }
        else
        {
            ...
            if(ERROR_CODE_OK != (btree_ret = keybtree_.put(key, value, true)))
            {
                YYSYS_LOG(WARN,"update value to keybtree fail btree_ret=%d [%s] [%s]",
                          btree_ret, key.log_str(), value->log_str());
                ret = (ERROR_CODE_ALLOC_FAIL == btree_ret) ? OB_MEM_OVERFLOW : OB_ERROR;
            }
            else
    ...
    TEValue *QueryEngine::get(const TEKey &key)
    {
      ...
      else if (0 == key.row_key.length())
      {
            ...
            || NULL == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get from table_keybtree fail btree_ret=%d %s", btree_ret, key.log_str());
          }
        }
      }
      //add:e
      else if (g_conf.using_hash_index)
      {
        ...
        while (true)
        {
              ...
              || NULL == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "get from keyhash fail hash_ret=%d %s", hash_ret, key.log_str());
            }
            break;
          }
...
//add support for arm platform by wangd 202106:e
      ...
      else
      {
            ...
            || NULL == ret)
        {
          if (ERROR_CODE_NOT_FOUND != btree_ret)
          {
            YYSYS_LOG(WARN, "get from keybtree fail btree_ret=%d %s", btree_ret, key.log_str());
          }
        }
      }
      return ret;
                          ...
                          QueryEngineIterator &iter)
    {
      ...
      {
        YYSYS_LOG(WARN, "btree get scan handle fail btree_ret=%d", btree_ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (0 != min_key)
        {
          ...
          {
            YYSYS_LOG(WARN, "get min key from btree fail btree_ret=%d", btree_ret);
        ...
        if (0 != max_key)
        {
          ...
          if (ERROR_CODE_OK != btree_ret)
          {
            YYSYS_LOG(WARN, "get max key from btree fail btree_ret=%d", btree_ret);
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          YYSYS_LOG(DEBUG, "[BTREE_SCAN_PARAM] [%s] %d [%s] %d",
                    scan_start_key_ptr->log_str(), start_exclude_, scan_end_key_ptr->log_str(), end_exclude_);
          if (ERROR_CODE_OK != btree_ret)
          {
            YYSYS_LOG(WARN, "set key range to btree scan handle fail btree_ret=%d", btree_ret);
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
      ...
      else
      {
        while (true)
        {
              ...
              || NULL == pvalue_)
          {
            if (ERROR_CODE_NOT_FOUND != btree_ret)
            {
              YYSYS_LOG(WARN, "get_next from keybtree fail btree_ret=%d pvalue_=%p", btree_ret, pvalue_);
            }
            ret = OB_ITER_END;
            break;
          }
          //mod duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160121:b
//          if (pvalue_->is_empty())
          if (pvalue_->is_empty(is_read_atomic_))
          //mod duyr 20160221:e
          {
            // 数据链表为空时 直接跳过这记录
