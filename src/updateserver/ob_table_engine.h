...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    struct ObCellInfoNode
    {
      ...
      //FIXME:[READ_ATOMIC] should I use ObTransVersion ranther than char buf?
      char trans_version_[common::ObReadAtomicHelper::TRANS_VERSION_BUF_SIZE];//coordinator's trans_id as distributed trans version
      //add duyr 20151117:e
      int64_t modify_time;
      ObCellInfoNode *next;
      char buf[0];
    ...
    class ObCellInfoNodeIterableWithCTime : public ObCellInfoNodeIterable, public RowkeyInfoCache
    {
      ...
      public:
        ...
        void reset();
      public:
        bool get_rdel_cell();//add zhaoqiong [306]
        void reset_all();//add zhaoqiong [306]
        void set_rdel();//add zhaoqiong [306]
        ...
        void set_rowkey_column(const uint64_t table_id, const common::ObRowkey &row_key, const CommonSchemaManager *sm = NULL
                , int64_t query_version = 0
                );
        void set_nop();
        bool is_cur_cell_from_te_kay()
        {return is_from_te_key_;}  //add hongchen[SECONDARY_INDEX_OPTI_BUGFIX] 20170809
      private:
        int inner_err;
        bool is_iter_end_;
        ...
        bool append_trun_; //add zhaoqiong [306]
        CtrlNode *trun_node_;//add zhaoqiong [306]
        CtrlNode rdel_node_;//add zhaoqiong [306]
...
//      };
      inline bool is_empty(const bool is_read_atomic = false) const
      {
        return ((is_read_atomic && NULL != cur_uc_info) ? false:(NULL == list_head));
      }
      //mod duyr 20160121:e
      ...
      int  set_able_force_unlock()
      {
          ...
          {
              YYSYS_LOG(ERROR,"unkown lock flag = %d",lock_flag);
              ret = OB_ERROR;
          }
          return ret;
      }
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [write_part] 20151117:b
    /**EXPR:used to backfill the distributed trans version,
      *use coordinator's trans id as trans version!
      **/
    class TEValueTransverCallback : public ISessionCallback
    {
      public:
        TEValueTransverCallback()
        {};
        ~TEValueTransverCallback()
        {};
      public:
