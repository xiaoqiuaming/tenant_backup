...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int precise_sleep(const int64_t microsecond)
    {
      ...
      if (0 < microsecond)
      {
        int64_t end_time = yysys::CTimeUtil::getMonotonicTime() + microsecond;
        ...
        do
        {
          ...
          {
            int64_t cur_time = yysys::CTimeUtil::getMonotonicTime();
        ...
        }while (end_time > yysys::CTimeUtil::getMonotonicTime());
      }
      return ret;
    }
      ...
      else
      {
        ...
        else
        {
          ...
          if (table_available_error_size >= mem_size_available)
          {
            ...
            {
              YYSYS_LOG(ERROR, "available memory reach the error-size=%ld, will try to free a memtable",
                        mem_size_available);
            }
          }
          else
        ...
        if (REACH_TIME_INTERVAL(MIN_PROC_INTERVAL))
        {
          YYSYS_LOG(INFO, "available memory reach the warn-size=%ld, will try to free a memtable",
                    mem_size_available);
        }
      }
      return bret;
    ...
    {
      if (YYSYS_LOG_LEVEL_DEBUG == YYSYS_LOGGER._level
          && NULL != scanner)
      {
        int64_t i = 0;
        ObCellInfo *ci = NULL;
        while (OB_SUCCESS == scanner->next_cell())
        {
          if (OB_SUCCESS == scanner->get_cell(&ci)
              && NULL != ci)
          {
            YYSYS_LOG(DEBUG, "[LOG_SCANNER][%ld] %s", i++, print_cellinfo(ci));
          }
          else
          {
            break;
    ...
    uint64_t get_create_time_column_id(const uint64_t table_id)
    {
          ...
          || CACHED_SCHEMA_VERSION != g_conf.global_schema_version)
      {
        ...
        inited = true;
        CACHED_SCHEMA_VERSION = g_conf.global_schema_version;
        YYSYS_LOG(INFO, "create time column id cache is cleared");
      }
          ...
          || OB_INVALID_ID == ret)
      {
        ...
        else
        {
          ...
          if (small_table_id)
          {
            ...
            YYSYS_LOG(INFO, "table_id=%lu create time column_id=%lu add to cache", table_id, ret);
          }
        }
      }
      return ret;
    ...
    uint64_t get_modify_time_column_id(const uint64_t table_id)
    {
          ...
          || CACHED_SCHEMA_VERSION != g_conf.global_schema_version)
      {
        ...
        inited = true;
        CACHED_SCHEMA_VERSION = g_conf.global_schema_version;
        YYSYS_LOG(INFO, "modify time column id cache is cleared");
      }
          ...
          || OB_INVALID_ID == ret)
      {
        ...
        else
        {
          ...
          {
            COLUMN_ID_CACHE[table_id - OB_APP_MIN_TABLE_ID] = ret;
            YYSYS_LOG(INFO, "table_id=%lu modify time column_id=%lu add to cache", table_id, ret);
          }
        }
      }
      return ret;
    ...
    int get_ups_schema_mgr(CommonSchemaManagerWrapper& schema_mgr)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_schema_mgr()=>%d", err);
      }
      return err;
    }
    ...
    //mod wangyao[bug fix checksum]:20180821:b
    //const ObRowkeyInfo *RowkeyInfoCache::get_rowkey_info(const uint64_t table_id) const
    const ObRowkeyInfo *RowkeyInfoCache::get_rowkey_info(const uint64_t table_id, const CommonSchemaManager *csm, int64_t query_version) const
    {
    //mod:e
      const ObRowkeyInfo *rki = NULL;
      ObUpdateServerMain *ups_main = NULL;

      //add wangyao[bug fix checksum]:20180821:b
      if (NULL != csm)
      {
          if(NULL != csm->get_table_schema(table_id))
          {
              rkinfo_table_id_ = table_id;
              rkinfo_ = csm->get_table_schema(table_id)->get_rowkey_info();
              rki = &rkinfo_;
          }
          else
          {
      ...
      //add:e
          ...
          else
          {
              ...
              //[bugfix 544]
              else if(query_version >0)
              {
                  CommonSchemaManagerWrapper commonsm;
                  const CommonSchemaManager *smm = NULL;
                  if(OB_SUCCESS != (ups_main->get_update_server().get_table_mgr().get_schema(query_version, commonsm)))
                  {
                      YYSYS_LOG(WARN, "get CommonSchemaManagerWrapper error");
                  }
                  else if(NULL != (smm = commonsm.get_impl()) && NULL != (tm = smm->get_table_schema(table_id)))
                  {
                      rkinfo_table_id_ = table_id;
                      rkinfo_ = tm->get_rowkey_info();
                      rki = &rkinfo_;
      ...
      YYSYS_LOG(DEBUG, "cached_table_id=%lu table_id=%lu cached_rowkey_info=[%s] rowkey_info=%p",
    ...
    uint64_t get_table_id(const ObGetParam& get_param)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_param[0] == NULL");
      }
      else
      {
        ret = cell->table_id_;
    ...
    int64_t get_time(const char *str)
    {
      ...
      if (NULL == str || '\0' == str[0])
      {
        YYSYS_LOG(ERROR, "invalid param, str=%p", str);
      }
      else
      {
        char* p_unit = NULL;
        ...
        if (0 > value)
        {
          YYSYS_LOG(ERROR, "invalid time, str=[%s]", str);
          value = -1;
        }
        ...
        {
          YYSYS_LOG(ERROR, "invalid unit, str=[%s]", str);
          value = -1;
        }
      }
      return value;
    ...
    int64_t split_string(const char *str, int64_t *values, str_to_int_pt *callbacks, const int64_t size, const char d)
    {
      ...
      if (NULL != str)
      {
        ...
        {
          deli = strchr(iter, d);
          YYSYS_LOG(DEBUG, "split deli=[%s] iter=[%s]", deli, iter);
          if (NULL != deli)
          {
            *deli = '\0';
          }
      ...
      YYSYS_LOG(INFO, "str=[%s] num=%ld values=[%s]", str, idx, print_array(values, idx));
      return idx;
    }
  }
}
...
void memory_limit_callback()
{
  ...
  if (!callback_running)
  {
    ...
    YYSYS_LOG(INFO, "ups memory_limit_callback");
