...
namespace oceanbase
{
  ...
  namespace updateserver
  {
    ObPhyOperator *ObUpsPhyOperatorFactory::get_one(ObPhyOperatorType type, common::ModuleArena &allocator)
    {
      ...
      {
        YYSYS_LOG(ERROR, "param not set: session_ctx=%p, table_mgr=%p", session_ctx_, table_mgr_);
      }
      else
      {
        switch(type)
        {
          ...
          {
            ObUpsIncompleteRowFilter * icr = tc_rp_alloc(ObUpsIncompleteRowFilter);
            icr->set_table_mgr(table_mgr_);
            ret = icr;
            break;
          }
          //add 20160425:e
      ...
      if (NULL != opt)
      {
        switch (opt->get_type())
        {
          case PHY_INC_SCAN:
            ...
            //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20160425:b
            case PHY_INCOMPLETE_ROW_FILTER:
            tc_rp_free(dynamic_cast<ObUpsIncompleteRowFilter*>(opt));
            break;
            //add 20160425:e
          default:
