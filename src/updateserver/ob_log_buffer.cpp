...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int64_t ObLogBuffer::to_string(char* buf, const int64_t len) const
    {
      ...
      databuff_printf(buf, len, pos, "LogBuffer(log_id=[%ld,%ld], pos=[%ld,%ld],read_pos = %ld, read_block =%ld, start_block=%ld, end_block =%ld",
                      get_start_id(), get_end_id(), start_pos_, end_pos_, replay_pos_,
                      get_block_id(replay_pos_), get_block_id(start_pos_), get_block_id(end_pos_));
      return pos;
    }
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "reset(%s)", to_cstring(*this));
    ...
    int64_t ObLogBuffer::get_start_id() const
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_next_entry(pos=%ld)=>%d", start_pos_, err);
      }
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "pos=[%ld, %ld), id=[%ld, %ld)", start_pos_, end_pos_, get_start_id(), end_id_);
    ...
    int ObLogBuffer::append_log(const int64_t start_id, const int64_t end_id, const char* buf, const int64_t len)
    {
      ...
      else if (NULL == buf || 0 > len || 0 >= start_id || start_id > end_id)
      {
        ...
        YYSYS_LOG(ERROR, "append_log(range=[%ld,%ld], buf=%p[%ld]): invalid argument", start_id, end_id, buf, len);
      }
      else if (end_id_ > 0 && end_id_ != start_id)
      {
        err = OB_DISCONTINUOUS_LOG;
        YYSYS_LOG(INFO, "append_log(start_id[%ld] != end_id_[%ld]): DISCONTINUOUS_LOG", start_id, end_id_);
      }
      else if (OB_SUCCESS != (err = append(buf, len)))
      {
        YYSYS_LOG(ERROR, "append(pos=%ld, buf=%p[%ld])=>%d", get_end_pos(), buf, len, err);
      }
      else
      {
        end_id_ = end_id;
    ...
    int ObLogBuffer::append_log(const int64_t start_id, const int64_t end_id, const char *buf, const int64_t len, bool &is_append)
    {
        int err = OB_SUCCESS;
        if (OB_SUCCESS != (err = check_state()))
        {
        ...
        else if (OB_SUCCESS != (err = append(buf, len, is_append)) && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "append(pos=%ld, buf=%p[%ld])=>%d", get_end_pos(), buf, len, err);
        }
        else if(OB_EAGAIN == err)//buf is full
        {
            YYSYS_LOG(WARN, "append(pos=%ld, buf=%p[%ld])=>%d", get_end_pos(), buf, len, err);
        }
        else
        {
          end_id_ = end_id;
    ...
    int ObLogBuffer:: get_next_entry(const int64_t pos, int64_t& next_pos, int64_t& log_id) const
    {
      ...
      else if (OB_SUCCESS != (err = read(pos, real_pos, log_entry_buf, sizeof(log_entry_buf), copy_count)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "read(pos=%ld, buf=%p[%ld])=>%d", pos, log_entry_buf, sizeof(log_entry_buf), err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "read for next_entry(pos=%ld): OB_DATA_NOT_SERVE", pos);
        }
      }
      else if (copy_count < log_entry.get_serialize_size())
      {
        err = OB_DATA_NOT_SERVE;
        YYSYS_LOG(DEBUG, "read for next_entry(pos=%ld): not read a full header", pos);
      }
      else if (OB_SUCCESS != (err = log_entry.deserialize(log_entry_buf, copy_count, log_entry_buf_pos)))
      {
        YYSYS_LOG(ERROR, "log_entry.deserialize(%p[%ld])=>%d", log_entry_buf, sizeof(log_entry_buf), err);
      }
      ...
      //YYSYS_LOG(DEBUG, "get_next_entry(log_id=%ld)=>%d", log_id, err);
      return err;
    }
    ...
    int ObLogBuffer::seek(const int64_t log_id, const int64_t advised_pos, int64_t& real_pos) const
    {
      ...
      while(OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = get_next_entry(real_pos, next_pos, cur_id)))
        {
          if (OB_DATA_NOT_SERVE != err)
          {
            YYSYS_LOG(ERROR, "get_next_entry(pos=%ld)=>%d", real_pos, err);
          }
        }
    ...
    int ObLogBuffer::get_log(const int64_t align_bits, const int64_t pos, int64_t& real_pos, int64_t& start_id, int64_t& end_id, char* buf, const int64_t len, int64_t& read_count) const
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "get_log(start_id=%ld, buf=%p[%ld]): invalid argument", start_id, buf, len);
      }
      ...
      else if (OB_SUCCESS != (err = read(pos, real_pos, buf, len, copy_count)))
      {
        if (OB_DATA_NOT_SERVE != err)
        {
          YYSYS_LOG(ERROR, "copy_to_buf(%p[%ld], read_count=%ld)=>%d", buf, len, read_count, err);
        }
      }
      else if (OB_SUCCESS != (err = trim_log_buffer(real_pos, align_bits, buf, copy_count, read_count, start_id, end_id, is_file_end)))
      {
        YYSYS_LOG(WARN, "parse_log_buffer(buf=%p[%ld], start_id=%ld)=>%d",
                  buf, copy_count, start_id, err);
      }
      else if (copy_count > 0 && read_count <= 0)
      {
        err = OB_DATA_NOT_SERVE;
        YYSYS_LOG(ERROR, "get_log(start_id=%ld, copy_count=%ld, read_count=%ld): NOT find align pos",
                  start_id, copy_count, read_count);
      }
      return err;
    }

    int get_from_log_buffer(ObLogBuffer* log_buf,
                            const int64_t align_bits,
                            const int64_t advised_pos,
                            int64_t& real_pos,
                            ...
                            int64_t& read_count)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "get_from_log_buffer(log_buf=%p, buf=%p, start_id=%ld, len=%ld):INVALID_ARGUMENT",
      ...
      else if (OB_SUCCESS != (err = log_buf->seek(start_id, advised_pos, real_pos)))
      {
        if (OB_DATA_NOT_SERVE != err)
        {
          YYSYS_LOG(ERROR, "log_buf->seek(start_id=%ld, advised_pos=%ld)=>%d", start_id, advised_pos, err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "log_buf->seek(start_id=%ld, advised_pos=%ld): OB_DATA_NOT_SERVE", start_id, advised_pos);
        }
      }
      else if (OB_SUCCESS != (err = log_buf->get_log(align_bits, real_pos, real_pos, real_start_id, end_id, buf, len, read_count)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "log_buf->get_log(pos=%ld, buf=%p[%ld])=>%d", real_pos, buf, len, err);
        }
      }
      else if (start_id != real_start_id)
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "start_id[%ld] != real_start_id[%ld]", start_id, real_start_id);
      }
      return err;
    }
                              ...
                              const int64_t start_id, int64_t end_id, char* buf, int64_t& read_count, const int64_t max_data_len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "get_from_log_buffer(log_buf=%p, buf=%p, start_id=%ld):INVALID_ARGUMENT",
      ...
      else if (OB_SUCCESS != (err = log_buf->get_log(align_bits, real_start_pos, real_start_pos, real_start_id, end_id, buf, len, read_count)))
      {
        if (OB_DATA_NOT_SERVE != err && OB_DISCONTINUOUS_LOG != err)
        {
          YYSYS_LOG(ERROR, "log_buf->get_log(pos=%ld, buf=%p[%ld])=>%d", real_start_pos, buf, len, err);
        }
      }
      else if (start_id != real_start_id)
      {
      ...
      else
      {
        ...
        log_buf->set_replay_pos(real_pos, read_count);
      }
      return err;
    }
    //add :e
                             ...
                             const int64_t start_id, const int64_t end_id, const char* buf, const int64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "append_to_log_buffer(log_buf=%p, buf=%p[%ld], id=[%ld,%ld]): invalid argument",
                  log_buf, buf, len, start_id, end_id);
      }
      else if (OB_SUCCESS != (err = log_buf->append_log(start_id, end_id, buf, len))
               && OB_DISCONTINUOUS_LOG != err)
      {
        YYSYS_LOG(ERROR, "log_buf->append_log(pos=%ld, range=[%ld, %ld], buf=%p[%ld])=>%d",
      ...
      {
        YYSYS_LOG(WARN, "append_to_log_buffer(%s, log_id=[%ld,%ld], buf=%p[%ld])", to_cstring(*log_buf), start_id, end_id, buf, len);
        if (OB_SUCCESS != (err = log_buf->reset()))
        {
          YYSYS_LOG(ERROR, "reset()=>%d", err);
        }
        else if (OB_SUCCESS != (err = log_buf->append_log(start_id, end_id, buf, len)))
        {
          YYSYS_LOG(ERROR, "log_buf->append_log(pos=%ld, range=[%ld, %ld], buf=%p[%ld])=>%d",
    ...
    int append_to_log_buffer_2(ObLogBuffer *log_buf,
                             const int64_t start_id, const int64_t end_id, const char *buf, const int64_t len, bool &is_append)
    {
        int err = OB_SUCCESS;
        if (NULL == log_buf || NULL == buf || start_id <= 0 || end_id < start_id || len < 0)
        {
        ...
        else if (OB_SUCCESS != (err = log_buf->append_log(start_id, end_id, buf, len, is_append))
                 && OB_DISCONTINUOUS_LOG != err && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "log_buf->append_log(pos=%ld, range=[%ld, %ld], buf=%p[%ld])=>%d",
                    log_buf->get_end_pos(), start_id, end_id, buf, len, err);
        }
        ...
        {
            YYSYS_LOG(WARN, "log_buf->append_log(pos=%ld, range=[%ld, %ld], buf=%p[%ld])=>%d",
                      log_buf->get_end_pos(), start_id, end_id, buf, len, err);
        }
        else if (OB_SUCCESS == err)
        {
