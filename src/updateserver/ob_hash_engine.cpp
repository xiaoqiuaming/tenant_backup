...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int HashEngine::init(MemTank *allocer, int64_t hash_item_num, int64_t index_item_num)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param hash_item_num=%ld index_item_num=%ld",
                  hash_item_num, index_item_num);
        ret = OB_ERROR;
      }
      else if (0 != hashmap_.create(hash_item_num))
      {
        YYSYS_LOG(WARN, "create hashmap fail hash_item_num=%ld", hash_item_num);
        ret = OB_ERROR;
      }
      else if (0 != indexmap_.create(index_item_num))
      {
        YYSYS_LOG(WARN, "create indexmap fail index_item_num=%ld", index_item_num);
        ret = OB_ERROR;
        hashmap_.destroy();
      }
      else
    ...
    int HashEngine::clear()
    {
      ...
      {
        YYSYS_LOG(WARN, "there is one transaction started");
        ret = OB_ERROR;
      }
      else if (mutation_started_)
      {
        YYSYS_LOG(WARN, "there is one mutation started");
        ret = OB_ERROR;
      }
      else
      {
    ...
    bool HashEngine::update_map_(const TEKey &key, const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "get key prefix fail %s", key.log_str());
      }
      else
      {
        ...
        else if (HASH_INSERT_SUCC == hash_ret && HASH_NOT_EXIST == index_ret)
        {
          ...
          {
            YYSYS_LOG(DEBUG, "set to indexmap succ %s", index_key.log_str());
            bret = true;
          }
          if (!bret)
          {
        ...
        {
          YYSYS_LOG(WARN, "updatemap fail hash_ret=%d index_ret=%d %s %s",
                        ...
                        const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "fill write trans buffer fail %s %s", key.log_str(), value.log_str());
        ret = OB_ERROR;
      }
      else
      {
        //YYSYS_LOG(INFO, "fill write trans buffer succ %s %s", key.log_str(), value.log_str());
      }
      return ret;
    }
    ...
    int HashEngine::set(const TEKey &key, const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "update map fail %s %s", key.log_str(), value.log_str());
        ret = OB_ERROR;
      }
      else
      {
        //YYSYS_LOG(INFO, "update map succ %s %s", key.log_str(), value.log_str());
      }
      return ret;
    }
    ...
    int HashEngine::query_map_(TEKey &key, TEValue &value)
    {
      ...
      {
        //YYSYS_LOG(INFO, "get from hashmap fail hash_ret=HASH_NOT_EXIST %s", key.log_str());
        ret = OB_ENTRY_NOT_EXIST;
      }
      else
      {
        YYSYS_LOG(WARN, "get from hashmap fail hash_ret=%d %s", hash_ret, key.log_str());
        ret = OB_ERROR;
      }
      return ret;
    }
                                ...
                                HashEngineIterator &iter)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "start_key must larger than or equal end_key start_key[%s], end_key[%s]",
        ...
        {
          YYSYS_LOG(WARN, "search index fail start_key[%s] min_key=%d start_exclude=%d end_key[%s] max_key=%d end_exclude=%d",
                                ...
                                HashEngineIterator &iter)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot scan by min_key or max_key for unindexed hash engine start_key[%s]",
                  start_key.log_str());
      ...
      {
        YYSYS_LOG(WARN, "get key prefix fail start_key[%s], end_key[%s]",
                  start_key.log_str(), end_key.log_str());
        ret = OB_ERROR;
      }
      else if (start_key_prefix != end_key_prefix)
      {
        YYSYS_LOG(WARN, "start_key_prefix must equal to end_key_prefix for unindexed hash engine start_key[%s], end_key[%s]",
                  start_key.log_str(), end_key.log_str());
        ret = OB_NOT_SUPPORTED;
      }
      else
      {
            ...
            && filt_ret)
        {
          iter.unsorted_set_(list_head, list_tail);
                ...
                || !filt_ret)
        {
          YYSYS_LOG(INFO, "get from indexmap fail hash_ret=%d filt_ret=%s %s", hash_ret, STR_BOOL(filt_ret), start_key_prefix.log_str());
          //ret = OB_ENTRY_NOT_EXIST;
        }
        else
        {
          YYSYS_LOG(WARN, "get from indexmap fail hash_ret=%d %s", hash_ret, start_key_prefix.log_str());
          ret = OB_ERROR;
        }
      }
      return ret;
    ...
    int HashEngine::create_index()
    {
      ...
      {
        YYSYS_LOG(INFO, "have already created index");
        ret = OB_SUCCESS;
      }
      else if (0 == hashmap_.size())
      {
        YYSYS_LOG(INFO, "not data, need not create index");
        ret = OB_SUCCESS;
      }
      ...
      {
        YYSYS_LOG(WARN, "new tmp index fail size=%ld", hashmap_.size());
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "hashmap_size=%ld index_pos=%ld hash_key=%s ptr=%p", hashmap_.size(), index_pos, iter->first.log_str(), &(iter->first));
      ...
      else
      {
        switch (type)
        {
          ...
          case WRITE_TRANSACTION:
            ...
            {
              YYSYS_LOG(WARN, "there is one transaction have been started");
              ret = OB_UPS_TRANS_RUNNING;
            }
            else if (OB_SUCCESS != trans_buffer_.start_mutation())
            {
              YYSYS_LOG(WARN, "start mutation fail");
              ret = OB_ERROR;
            }
            else
            {
      ...
      else
      {
        switch (handle.trans_type_)
        {
          case WRITE_TRANSACTION:
            if (!write_transaction_started_)
            {
              YYSYS_LOG(WARN, "there is no transaction started");
              ret = OB_ERROR;
            }
            else if (mutation_started_)
            {
              YYSYS_LOG(WARN, "mutation have not end");
              ret = OB_ERROR;
            }
            else if (OB_SUCCESS != trans_buffer_.end_mutation(rollback))
            {
              YYSYS_LOG(WARN, "end transaction fail");
              ret = OB_ERROR;
            }
            else
            {  
              if (!rollback)
              {
                ...
                for (iter = trans_buffer_.begin(); iter != trans_buffer_.end(); ++iter)
                {
                  ...
                  {
                    YYSYS_LOG_US(DEBUG, "commit trans succ %s %s", iter->first.log_str(), iter->second.log_str());
                  }
                }
              }
              trans_buffer_.clear();
    ...
    int HashEngine::start_mutation(HashEngineTransHandle &handle)
    {
      ...
      {
        YYSYS_LOG(WARN, "there is one mutation have been started");
        ret = OB_ERROR;
      }
      else if (!handle.is_valid_(this, WRITE_TRANSACTION))
      {
      ...
      {
        YYSYS_LOG(WARN, "trans buffer start mutation fail");
      }
      else
      {
    ...
    int HashEngine::end_mutation(HashEngineTransHandle &handle, bool rollback)
    {
      ...
      {
        YYSYS_LOG(WARN, "no mutation have been started");
        ret = OB_ERROR;
      }
      else if (!handle.is_valid_(this, WRITE_TRANSACTION))
      {
    ...
    int SortedIterator::next()
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid internal pointer cur_hash_kv=%p", cur_index_item_);
        ret = OB_ITER_END;
      }
      return ret;
    }
    ...
    int UnsortedIterator::next()
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid internal pointer cur_hash_kv=%p", cur_hash_kv_);
        ret = OB_ITER_END;
      }
      return ret;
    }
