...
namespace oceanbase
{
  namespace updateserver
  {
                                    ...
                                    ObIterator* iterator)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "transfer sstable query has not inited");
        ret = OB_ERROR;
      }
      else if (cell_size <= 0 || NULL == row_index || row_size <= 0
               || row_size > OB_MAX_GET_ROW_NUMBER)
      {
        YYSYS_LOG(WARN, "invalid get param, cell_size=%ld, row_index=%p, row_size=%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to init sstable getter, ret=%d", ret);
        }
        else
        {
          // do nothing
                                     ...
                                     ObIterator* iterator)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to dynamic cast iterator to sstable scanner, sstable_scanner=%p", 
                  sstable_scanner);
      ...
      const ObSSTableReader& reader, const uint64_t table_id, ObRowkey& row_key)
    {
      ...
      else 
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to get sstable end key, table_id=%lu", table_id);
        }
      }
