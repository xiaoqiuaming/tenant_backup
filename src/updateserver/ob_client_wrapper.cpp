   ...
   int ObClientWrapper::init(ObMergerRpcStub * rpc_stub,
       ...
       ObMergerLocationCacheProxy* cache)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "input param error, rpc=%p, schema=%p, cache=%p",
        ...
        else
        {
          if((OB_SUCCESS == rpc_proxy_.init(rpc_stub, schema, cache)))
          {
            ...
            if(NULL == ups_stream_)
            {
              YYSYS_LOG(WARN,"new ups_stream failed");
              ret = OB_ERROR;
            }
            else
            {
              ...
              if(NULL == ups_join_stream_)
              {
                YYSYS_LOG(WARN,"new ups_join_stream failed");
                ret = OB_ERROR;
              }
              else
              {
    ...
    int ObClientWrapper::get(const ObGetParam& get_param, const ObSchemaManagerV2 & schema_mgr)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "check get param failed:size[%ld], cell[%p]",
        ...
        else
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN,"ups rpc agent set request param failed,ret=%d",ret);
          }
        }
      }
      return ret;
