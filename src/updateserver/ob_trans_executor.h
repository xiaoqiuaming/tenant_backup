...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    //thread pool used to process distributed transaction,
    //contant is same with TransHandlePool
    class DistributedTransHandlePool : public common::S2MQueueThread
    {
      public:
        DistributedTransHandlePool() : affinity_start_cpu_(-1), affinity_end_cpu_(-1)
        {}
        virtual ~DistributedTransHandlePool()
        {}
      public:
        void handle(void *ptask, void *pdata)
        {
    ...
    class TransCommitThread : public SeqQueueThread
    {
      ...
      public:
        ...
        void *on_begin()
        {
            SET_THD_NAME_ONCE("ups-trxcommit");
    ...
    class TransExecutor : public TransHandlePool, public TransCommitThread, public CommitEndHandlePool, public DistributedTransHandlePool, public IObAsyncClientCallback
    //mod 20150701:e
    {
      ...
      public:
        ...
        //add:e
        inline int64_t get_flush_queue_size()
        {
        ...
        {return session_mgr_;};
        LockMgr &get_lock_mgr()
        {return lock_mgr_;};
        void log_trans_info() const;
        int &thread_errno();
        int64_t &batch_start_time();
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
       // common::FIFOAllocator& get_allocator() {return allocator_;}
        void process_unsettled_trans();
        bool check_session_exist(const ObTransID& trans_id);
        //add 20150701:e
      private:
        ...
        bool handle_get_publised_transid_(Task &task, ObDataBuffer &buffer);
        // add by e
        //mod peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        //int get_session_type(const ObTransID& sid, SessionType& type, const bool check_session_expired);
        int get_session_type(const ObTransID& sid, const ObTransID& coordinator_sid,SessionType& type, const bool check_session_expired);
        //mod:e
        ...
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        /**
         * @brief handle end distributed trans, coordinator process
         */
        bool handle_end_trans_(Task &task, ObDataBuffer &buffer);
        /**
         * @brief handle participant response prepare sucess, coordinator process
         */
        bool handle_prepare_ack_(Task &task, common::ObDataBuffer &buffer);
        /**
         * @brief handle system ups response write trans stat success, coordinator process
         */
        bool handle_write_stat_ack_(Task &task, common::ObDataBuffer &buffer);
        /**
         * @brief prepare distributed trans, send prepare to every participant
         * @param participant_tid array of all participant
         * @param participant_num participant_tid length
         * @return
         */
        int prepare_distributed_trans(const ObTransID* participant_tid, const int32_t participant_num);
        /**
         * @brief rollback distributed trans, send rollback to every participant
         * @param participant_tid array of all participant
         * @param participant_num participant_tid length
         * @return
         */
        void rollback_distributed_trans(const ObTransID* participant_tid, const int32_t participant_num);
        /**
         * @brief commit distributed trans, send commit to every participant
         * @param participant_tid array of all participant
         * @param participant_num participant_tid length
         * @return
         */
        void commit_distributed_trans(const ObTransID* participant_tid, const int32_t participant_num);

        int write_trans_stat(RWSessionCtx& ctx, const ObTransID& trans_id, bool commit);
        int end_sub_trans(RWSessionCtx &ctx, const common::ObTransID& trans_id, const bool commit);
        int handle_unsettled_participant_trans(RWSessionCtx &ctx, const common::ObTransID& trans_id);
        //int construct_trans_record(const ObTransID& trans_id, bool commit, ObMutator* mutator);
        bool handle_rollback_trans_(Task &task, common::ObDataBuffer &buffer);
        bool handle_commit_trans_(Task &task, common::ObDataBuffer &buffer);
        bool handle_check_session_exist(Task &task, common::ObDataBuffer &buffer);
        //add 20150701:e
        ...
        //[449]
        void handle_update_partition_stat(ObPacket &pkt);
        bool partition_change_between_versions(uint64_t active_mem_table_version, uint64_t curr_frozen_version);
        ...
        */
        int handle_write_commit_(Task &task, const bool prepare = false);
        int fill_log_(Task &task, RWSessionCtx &session_ctx, const bool prepare);
        //mod 20150731:e
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        int handle_write_trans_end_(Task &task, const bool commit);
        int fill_trans_end_log_(Task &task, RWSessionCtx &session_ctx, const bool commit);
        /**
         * @brief fetch distributed trans stat from system table
         * @param trans_id coordinator trans id
         * @param [out]find_trans_stat return system table has record of this trans
         * @param [out]commit trans stat in system table record
         * @return
         */
        int fetch_trans_stat(const common::ObTransID &trans_id, bool& find_trans_stat, bool& commit);
        /**
         * @brief fetch trans commit stat from system table response scanner
         * @param [out]commit
         * @param [in]scanner
         * @return
         */
        int fill_trans_stat(bool& commit, ObNewScanner& scanner);
        //add 20150701:e
        int commit_log_();
        ...
        int fill_session_log_(Task &task, RWSessionCtx &session_ctx,const bool prepare);//uncertainty
        int fill_normal_log_(Task &task, RWSessionCtx &session_ctx,const bool prepare);//uncertainty
      int fill_big_log_(Task &task, RWSessionCtx &session_ctx, const bool prepare);
        //add e
      ...
      private:
        ...
        //static void phandle_check_sstable_checksum(ObPacket &pkt, ObDataBuffer &buffer);//del chensy[363]
      private:
        ...
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        static bool thandle_end_trans(TransExecutor &host, Task &task, TransParamData &pdata);
        static bool thandle_prepare_ack(TransExecutor &host, Task &task, TransParamData &pdata);
        static bool thandle_rollback_trans(TransExecutor &host, Task &task, TransParamData &pdata);
        static bool thandle_commit_trans(TransExecutor &host, Task &task, TransParamData &pdata);
        static bool thandle_write_stat_ack(TransExecutor &host, Task &task, TransParamData &pdata);
        static bool thandle_check_session_exist(TransExecutor &host, Task &task, TransParamData &pdata);
        //add 20150701:e
        //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
        static bool thandle_get_publised_transid(TransExecutor &host, Task &task, TransParamData &pdata);
		//add by maosy e

        static bool thandle_update_partition_stat(TransExecutor &host, Task &task, TransParamData &pdata);//[449]
      ...
      private:
        ...
        static bool chandle_check_sstable_checksum(TransExecutor &host, Task &task, CommitParamData &pdata);//add chensy [363]
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        static bool chandle_write_prepare(TransExecutor &host, Task &task, CommitParamData &data);
        static bool chandle_write_commit_trans(TransExecutor &host, Task &task, CommitParamData &data);
        static bool chandle_write_rollback_trans(TransExecutor &host, Task &task, CommitParamData &data);
        //add 20150701:e
      ...
      private:
        ...
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        friend int handle_unsettled_trans(const void *arg);
        friend int handle_unsettled_trans_for_replay(const void *arg);
        //add 20150701:e
        ...
        bool mutex_flag_for_unsettled_trans_; //[521]
        hash::ObHashMap<int64_t, int, hash::ReadWriteDefendMode> partition_unchanged_version_;//[449]
