...
#define UPS ObUpdateServerMain::get_instance()->get_update_server()
    ...
    {
      public:
        SessionGuard(SessionMgr& session_mgr, LockMgr& lock_mgr, int& ret, const bool end_session_flag_ = true):
          ret_(ret),
          session_descriptor_(INVALID_SESSION_DESCRIPTOR), session_ctx_(NULL),
            session_mgr_(session_mgr), lock_mgr_(lock_mgr), end_session_flag_(end_session_flag_)
        {}
        ~SessionGuard()
        {
          revert();
        ...
        void revert()
        {
          if (NULL != session_ctx_)
          {
            if (OB_SUCCESS == ret_)
            {
              session_ctx_->set_last_active_time(yysys::CTimeUtil::getTime());
            }
            session_ctx_->unlock_session();
            session_mgr_.revert_ctx(session_descriptor_);
            if (OB_SUCCESS != ret_ && end_session_flag_)
            {
              bool rollback = true;
        ...
        static SessionType get_session_type(TransType type)
        {
          ...
          switch(type)
          {
            ...
            case REPLAY_TRANS:
              ...
              break;
            default:
              YYSYS_LOG(ERROR, "unknown trans_type=%d", type);
              break;
          }
          return session_type;
        }
        ...
        //pcode'value can only be set with OB_WRITE_TRANS_STAT
        int start_session(const ObTransReq& req, ObTransID& sid, SessionT*& ret_ctx, const common::PacketCode pcode = common::OB_PACKET_NUM)
        //mod 20150811:e
        {
          int ret = OB_SUCCESS;
          ...
          {
            ret = OB_INIT_TWICE;
            YYSYS_LOG(ERROR, "sid[%s] is valid or session_ctx[%p] != NULL", to_cstring(sid), session_ctx_);
          }
          ...
          {
            YYSYS_LOG(ERROR, "init_lock_info fail ret=%d %s", ret, to_cstring(sid));
          }
          ...
          {
            YYSYS_LOG(ERROR, "add_publish_callback fail ret=%d %s", ret, to_cstring(*session_ctx_));
          }
          ...
          {
            YYSYS_LOG(ERROR, "add_publish_callback(%s)=>%d", to_cstring(*session_ctx_), ret);
          }
          else if (NULL == (ret_ctx = dynamic_cast<SessionT*>(session_ctx_)))
          {
            ret = OB_TRANS_NOT_MATCH;
            YYSYS_LOG(USER_ERROR, "session=%p type=%d is not read-write session", session_ctx_, session_ctx_->get_type());
          }
          else
          {
        ...
        int fetch_session(const ObTransID& sid, SessionT*& session_ctx, const bool check_session_expired = false)
        {
          ...
          if (!sid.is_valid())
          {
            ...
            YYSYS_LOG(WARN, "sid[%s] is invalid", to_cstring(sid));
          }
          else if (NULL != session_ctx_)
          {
            ret = OB_INIT_TWICE;
            YYSYS_LOG(ERROR, "session_ctx_[%p] != NULL", session_ctx_);
          }
          else if (NULL == (session_ctx = session_mgr_.fetch_ctx<SessionT>(sid.descriptor_)))
          {
            YYSYS_LOG(WARN, "fetch ctx fail session_descriptor=%u", sid.descriptor_);
            ret = OB_TRANS_ROLLBACKED;
          }
          ...
          {
            YYSYS_LOG(WARN, "session not match: id=%u, start_time=%ld, ups=%s",
