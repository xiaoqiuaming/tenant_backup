...
int ClogMonitor::initialize(const ObServer& server)
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to initialize client_, ret=%d", ret);
  }
  else
  {
    master_ups_ = server;
...
int ClogMonitor::refresh_moitor()
{
  ...
  while(true)
  {
    ...
    snprintf(buff, sizeof(buff),
             ...
             "+--------+-----------------------+---------+-----------------------+-----------------------+-----------------+---------+\n"
             "|  Role  |      Host:Port        |  State  |       Master AS       |       Master TS      |      Lease      | Remain  |\n"
             "|--------+-----------------------+---------+-----------------------+-----------------------+-----------------|---------|\n%s"
             "+--------+-----------------------+---------+-----------------------+-----------------------+-----------------+---------+\n"
             "\n"
             "+-----------------------+-----------------+--------------+---------------+--------------+--------------+---------------+\n"
...
int ClogMonitor::get_ups_list(const ObServer &server)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "ups_list_str.deserialize()=>%d", ret);
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "parse_ups_list() fail, msg=%s, ret=%d", ups_list_str.ptr(), ret);
      }
      else
      {
        if(!is_master)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get_ups_list() fail, master_ups=%s, ret=%d", to_cstring(master_ups), ret);
          }
        }
      }
    }
...
int ClogMonitor::get_ups_clog_status(const ObServer &server, ClogStatus &status)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "stat.deserialize()=>%d", ret);
    }
    else
    {
      //printf("->%s\n", to_cstring(status));
...
int ClogMonitor::parse_ups_list(const char* str, bool &is_master, char* master_ip, int32_t &master_port)
{
  ...
  if(str[0] == 's')
  {
    ...
    while(start_pos < len)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "add ups list fail, server.ip=%s, server.port=%d, ups_num=%d", ip, port, ups_num_);
        break;
      }
      start_pos = end_pos + 1;
      i++;
  ...
  else if(str[0] == 'm')
  {
    ...
    {
      char port_str[10];
      memset(master_ip, 0, MONITOR_MAX_IP_LEN);
      memset(port_str, 0, sizeof(port_str));
...
int main(int argc, char** argv)
{
  ...
  YYSYS_LOGGER.setFileName("clog_monitor.log", true);
  YYSYS_LOGGER.setMaxFileSize(256 * 1024L * 1024L);  //256M
  YYSYS_LOG(INFO,"-------------------------------------------");
  YYSYS_LOG(INFO,"        Oceanbase Commit Log Monitor");
  YYSYS_LOG(INFO,"-------------------------------------------");
  ob_init_memory_pool();
