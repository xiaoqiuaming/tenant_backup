...
namespace oceanbase
{
  namespace updateserver
  {
                              ...
                              const int64_t n_blocks, const int64_t block_size_shift)
    {
      ...
      {
        YYSYS_LOG(ERROR, "remote_log_src.init()=>%d", err);
      }
      else if (OB_SUCCESS != (err = prefetch_log_buffer_.init(n_blocks, block_size_shift)))
      {
        YYSYS_LOG(ERROR, "prefetch_log_buffer_.init()=>%d", err);
      }
      else
      {
    ...
    int ObReplayLogSrc::reset_prefetch_log_buffer2()
    {
        int err = OB_SUCCESS;
        SpinWLockGuard guard(pc_lock_);
        ...
        {
            YYSYS_LOG(ERROR,"prefetch_log_buffer.reset()=>%d", err);
        }
        return err;
    }
    ...
    {
      int err = OB_SUCCESS;
      //YYSYS_LOG(DEBUG, "submit_prefetch_log_task(start_cursor=%s)", start_cursor.to_str());
      if (prefetch_log_buffer_.get_remain() <= prefetch_log_buffer_.get_block_size())
      {
        err = OB_SUCCESS;
        YYSYS_LOG(DEBUG, "no enough buf, need not submit prefetch task.");
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "submit_prefetch_task()=>%d", err);
        }
      }
      return err;
    }
    ...
    int ObReplayLogSrc::get_log(const ObLogCursor& start_cursor, int64_t& end_id, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_log_from_buffer()=>%d", err);
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "prefetch_log_buffer.get_log()=>%d", err);
        }
        else if (OB_DISCONTINUOUS_LOG == err && OB_SUCCESS != (prefetch_err = reset_prefetch_log_buffer()))
        {
          err = prefetch_err;
          YYSYS_LOG(ERROR, "prefetch_log_buffer.reset()=>%d", prefetch_err);
        }
        else if (OB_SUCCESS != (prefetch_err = submit_prefetch_log_task(start_cursor)))
        {
          err = prefetch_err;
          YYSYS_LOG(ERROR, "submit_prefetch_log_task()=>%d", prefetch_err);
        }
      ...
      YYSYS_LOG(DEBUG, "get_log_for_replay(start_cursor=%s, end_id=%ld, read_count=%ld, get_from_cache=%s)=>%d",
    ...
    int ObReplayLogSrc::prefetch_log()
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_prefetch_cursor()=>%d", err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "remote_log_src.get_log()=>%d", err);
      }
      ...
      {
        //YYSYS_LOG(DEBUG, "prefetch_log(start_cursor=%s): NEED_RETRY", start_cursor.to_str());
      }
      ...
      else
      {
          ...
          {
            YYSYS_LOG(ERROR, "prefetch_log_buffer_.append_log(log=[%ld,%ld], read_count=%ld)=>%d",
          ...
          else if (OB_DISCONTINUOUS_LOG == err)
          {
            ...
            {
              YYSYS_LOG(ERROR, "prefetch_log_buffer_.reset()=>%d", err);
            }
          }
          else if (OB_SUCCESS == err)
          {
      ...
      YYSYS_LOG(DEBUG, "prefetch_log(start_cursor=%s)=>%d", start_cursor.to_str(), err);
      return err;
    }
