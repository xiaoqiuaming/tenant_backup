...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    TransBuffer::TransBuffer() : trans_map_(), patch_map_()
    {
      ...
      {
        YYSYS_LOG(WARN, "create trans map fail size=%ld", DEFAULT_SIZE);
      }
      if (0 != patch_map_.create(DEFAULT_SIZE))
      {
        YYSYS_LOG(WARN, "create patch map fail size=%ld", DEFAULT_SIZE);
      }
    }
    ...
    int TransBuffer::push(const TEKey &key, const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "set to patch map fail %s %s", key.log_str(), value.log_str());
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int TransBuffer::commit_()
    {
      ...
      for (iterator iter = patch_map_.begin(); iter != patch_map_.end(); iter++)
      {
        ...
        if (HASH_INSERT_SUCC != hash_ret
            && HASH_OVERWRITE_SUCC != hash_ret)
        {
          YYSYS_LOG(ERROR, "set to trans map fail %s %s", iter->first.log_str(), iter->second.log_str());
          ret = OB_ERROR;
          break;
        }
      }
