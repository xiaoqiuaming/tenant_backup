...
#define DEF_SET_FUNC(name) \
      ...
      { \
        YYSYS_LOG(INFO, "%p set %s from %ld to %ld", this, #name, name##_, name); \
        name##_ = name;\
      };
      ...
      bool warn(const int64_t value)
      {
        ...
        //[bugfix 331]
        if(0 >= window_count_ || 0 >= max_value_limit_ || 0>= stb_value_limit_ ||
                0 >= gt_stb_count_limit_ || stb_value_limit_ > max_value_limit_)
        {
            ret = common::OB_INVALID_ARGUMENT;
          bret = false;
        }
        else if (total_count_ >= window_count_)
        {
            total_count_ = 0;
            gt_stb_count_ = 0;
        }
        ...
        else if(value <= stb_value_limit_)
        {
            bret = false;
            total_count_ += 1;
        }
...
//          }
    ...
    struct ObClogStat
    {
      ...
      ~ObClogStat()
      {}
      void set_disk_warn_us(int64_t us)
      { disk_warn_us_ = us; }
      void set_net_warn_us(int64_t us)
      { net_warn_us_ = us; }
