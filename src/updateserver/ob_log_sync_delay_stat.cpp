  ...
  namespace updateserver
  {
    int set_state_as_replaying_log()
    {
      ...
      else
      {
        ...
        if (role_mgr.get_state() == ObUpsRoleMgr::FATAL || role_mgr.get_state() == ObUpsRoleMgr::STOP)
        {
          YYSYS_LOG(ERROR, "can not change state to REPLAYING_LOG, current stat=%d", role_mgr.get_state());
          ret = OB_ERROR;
        }
        else
        //add :e
        if (role_mgr.get_state() != ObUpsRoleMgr::REPLAYING_LOG)
        {
          ...
          {
              YYSYS_LOG(ERROR,"ups log delay exceed tolerable time, will be set replaying state");
            YYSYS_LOG(INFO, "slave lagged, set state to REPLAYING_LOG");
          }
        }
      }
      return ret;
    ...
    int set_state_as_active()
    {
      ...
      else
      {
        ...
        if(ObUpsRoleMgr::SLAVE != role_mgr.get_role())
        {
            YYSYS_LOG(WARN,"can not change state to ACTIVE, current role=%d", role_mgr.get_role());
        }

        //add wangdonghui [ups_replication] 20170816 :b
        else if (role_mgr.get_state() == ObUpsRoleMgr::FATAL || role_mgr.get_state() == ObUpsRoleMgr::STOP)
        {
          YYSYS_LOG(ERROR, "can not change state to ACTIVE, current stat=%d", role_mgr.get_state());
          ret = OB_ERROR;
        }
        else
        //add :e
        if (role_mgr.get_state() != ObUpsRoleMgr::ACTIVE)
        {
          ...
          if (REACH_TIME_INTERVAL(10 * 1000 * 1000))
          {
            YYSYS_LOG(INFO, "slave catchup, set state to ACTIVE");
          }
        }
      }
      return ret;
    ...
    DEFINE_SERIALIZE(ObEventAccumulator)
    {
      ...
      if (NULL == buf || (buf_len < pos))
      {
        ...
        YYSYS_LOG(ERROR, "ObEventAccumulator.serialize(buf=%p, len=%ld, pos=%ld)=>%d",
                  buf, buf_len, pos, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "ObEventAccumulator.serialize()=>%d", err);
      }
      else
      {
        pos = new_pos;
    ...
    DEFINE_DESERIALIZE(ObEventAccumulator)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "ObEventAccumulator.deserialize(buf=%p, len=%ld, pos=%ld)=>%d",
      ...
      {
        YYSYS_LOG(ERROR, "ObEventAccumulator.deserialize()=>%d", err);
      }
      else
      {
        pos = new_pos;
    ...
    ObLogSyncDelayStat::ObLogSyncDelayStat(): have_ever_catchup_(false), delay_accumulator_(), delay_warn_time_us_(-1), delay_tolerable_time_us_(-1),
                                            ...
                                            ,con_nop_num_(0) //add [active_replaying_switch]
    {}
    ...
    int ObLogSyncDelayStat::add_log_replay_event(const int64_t log_id, const int64_t mutation_time, const int64_t master_log_id, ReplayType replay_type, common::LogCommand cmd)
    {
      int err = OB_SUCCESS;
      ...
      //add [active_replaying_switch]:b [dduuhhtt]
      if(replay_type == RT_APPLY && delay_tolerable_time_us_ > 0 && mutation_time + delay_tolerable_time_us_ > now_us
              && log_id + 100 > master_log_id)//could be set to ACTIVE when no more than 100 log behind
      {
          if(cmd == OB_LOG_NOP)
          {
              con_nop_num_++;
      ...
      {
        YYSYS_LOG(WARN, "mutator sync delay warn:"
        ...
        {
          YYSYS_LOG(ERROR, "[PER_5_MIN] mutator sync delay warn:"
                    "delay_warn_time=%ld us, n_lagged_mutator[%ld] - n_lagged_mutator_since_last_report[%ld] = %ld "
                    "log_id=%ld, master_log_id=%ld",
                    delay_warn_time_us_, n_lagged_mutator_,
                    n_lagged_mutator_since_last_report_, n_lagged_mutator_-n_lagged_mutator_since_last_report_,
    ...
    DEFINE_SERIALIZE(ObLogSyncDelayStat)
    {
      ...
      {
        YYSYS_LOG(ERROR, "delay_stat.serialize(buf=%p, data_len=%ld, new_pos=%ld)=>%d", buf, buf_len, new_pos, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "ObLogSyncDelayStat.serialize()=>%d", err);
      }
      else
      {
        pos = new_pos;
    ...
    DEFINE_DESERIALIZE(ObLogSyncDelayStat)
    {
      ...
      {
        YYSYS_LOG(ERROR, "delay_stat.deserialize(buf=%p, data_len=%ld, new_pos=%ld)=>%d", buf, data_len, new_pos, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "ObLogSyncDelayStat.deserialize()=>%d", err);
      }
      else
      {
        pos = new_pos;
