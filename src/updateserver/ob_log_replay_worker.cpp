...
#include "ob_trans_executor.h"//add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701
...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObLogReplayWorker::init(IAsyncLogApplier* log_applier,
                                ...
                                const bool replay_queue_rebalance,
                                const int64_t replay_thread_start_cpu,
                                const int64_t replay_thread_end_cpu)
    {
      int err = OB_SUCCESS;
      bool queue_rebalance = replay_queue_rebalance;
      ...
      {
        YYSYS_LOG(ERROR, "allocator.init(limit=%ld, page_size=%ld)=>%d", log_buf_limit, OB_MAX_PACKET_LENGTH, err);
      }
      else if (OB_SUCCESS != (err = apply_worker_.init(n_worker, queue_len/n_worker, queue_rebalance, dynamic_rebalance)))
      {
        YYSYS_LOG(ERROR, "apply_worker_.init(n_thread=%d, queue_len=%ld)=>%d", n_worker, queue_len, err);
      }
      else if (OB_SUCCESS != (err = commit_queue_.init(queue_len)))
      {
        YYSYS_LOG(ERROR, "commit_queue_.init(%ld)=>%d", queue_len, err);
      }
      else
      {
        n_worker_ = n_worker;
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "log_replay_worker.init(log_applier=%p, n_worker=%d): %d", log_applier, n_worker, err);
      }
      else
      {
        YYSYS_LOG(INFO, "log_replay_worker.init(log_applier=%p, n_worker=%d): success", log_applier, n_worker);
      }
      return err;
    }
    ...
    {
    YYSYS_LOG(WARN, "replay_worker set_master_max_commit_log_id waiting for __sync");
    }
    }
    //add:e
    ...
    void ObLogReplayWorker::on_error(ObLogTask* task, int err)
    {
      ...
      YYSYS_LOG(ERROR, "replay error: task=%s, err=%d, will kill self", NULL == task? "NULL": to_cstring(*task), err);
      if (__sync_bool_compare_and_swap(&killed, 0, 1))
      {
        kill(getpid(), SIGTERM);
        YYSYS_LOG(ERROR, "kill self, pid=%d", getpid());
      }
    }
    ...
    int ObLogReplayWorker::get_real_replay_cursor(ObLogCursor &cursor) const
    {
        int err = OB_SUCCESS;
        if(replay_cursor_.newer_than(real_replay_cursor_))
        {
            cursor = replay_cursor_;
        }
        else
        {
            cursor = real_replay_cursor_;
        }
        return err;
    }
    ...
    int ObLogReplayWorker::set_real_replay_cursor(const ObLogCursor &cursor)
    {
        int err = OB_SUCCESS;
        real_replay_cursor_ = cursor;
        return err;
    }

    int ObLogReplayWorker::set_replay_cursor()
    {
        int err = OB_SUCCESS;
        if(real_replay_cursor_.newer_than(replay_cursor_))
        {
            err = update_replay_cursor(real_replay_cursor_);
        }
        return err;
    }

    int ObLogReplayWorker::reset_real_replay_cursor()
    {
        int err = OB_SUCCESS;
        ((ObLogCursor &) real_replay_cursor_).reset();
        return err;
    }
    ...
    {
      int err = OB_SUCCESS;
      //YYSYS_LOG(INFO, "update_cursor(%s)", to_cstring(cursor));
      if (replay_cursor_.newer_than(cursor))
      {
        err = OB_DISCONTINUOUS_LOG;
        YYSYS_LOG(ERROR, "update_replay_cursor(replay_cursor[%ld:%ld+%ld] > curosr[%ld:%ld+%ld])",
      ...
      {
        YYSYS_LOG(ERROR, "commit_queue_.update(%ld)=>%d", cursor.log_id_, err);
      }
      else
      {
    ...
    int ObLogReplayWorker::start_log(const ObLogCursor& log_cursor)
    {
      ...
      else if (replay_cursor_.is_valid())
      {
        ...
        {
          YYSYS_LOG(WARN, "start_log(log_cursor=%s): ALREADY STARTED.", log_cursor.to_str());
        }
        else
        {
          YYSYS_LOG(ERROR, "start_log(log_cursor=%s): INIT_TWICE. replay_cursor=(%s)", log_cursor.to_str(), replay_cursor_.to_str());
          err = OB_INIT_TWICE;
        }
      }
      else if (OB_SUCCESS != (err = update_replay_cursor(log_cursor)))
      {
        YYSYS_LOG(ERROR, "update_replay_cursor(%s)=>%d", to_cstring(log_cursor), err);
      }
      return err;
    }
    ...
    void ObLogReplayWorker::run(yysys::CThread* thread, void* arg)
    {
      int err = OB_SUCCESS;
      int64_t id = (int64_t)arg;
      UNUSED(thread);
      YYSYS_LOG(INFO, "log replay worker[%ld] start", id);
      SET_THD_NAME_ONCE("ups-logcommit");
      if (!is_inited())
      {
        err = OB_NOT_INIT;
      }
      else if (OB_SUCCESS != (err = do_work(id)))
      {
        ...
        YYSYS_LOG(ERROR, "do_work(id=%ld)=>%d", id, err);
      }
      YYSYS_LOG(INFO, "log replay worker[%ld] exit: err=%d", id, err);
    }
    ...
    int ObLogReplayWorker::do_work(int64_t thread_id)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "thread_id=%ld", thread_id);
      }
      else if (0 == thread_id)
      {
        if (OB_SUCCESS != (err = do_commit(thread_id)))
        {
          YYSYS_LOG(ERROR, "do_commit(%ld)=>%d", thread_id, err);
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "do_replay(%ld)=>%d", thread_id, err);
        }
      }
      return err;
    }
    ...
    int ObLogReplayWorker::do_commit(int64_t thread_id)
    {
      ...
      while (!_stop && OB_SUCCESS == err && OB_SUCCESS == err_)
      {
        if (OB_SUCCESS != (err = commit_queue_.get(seq, (void*&)task, wait_time_us))
            && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "commit_queue_.get()=>%d", err);
        }
        else if (OB_EAGAIN == err)
        {
          //del peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
          //err = OB_SUCCESS;
          //del 20150701:e
        }
        ...
        else if (OB_UPS_MUTATOR_PREPARE == task->log_entry_.cmd_
                 || OB_PART_TRANS_ROLLBACK == task->log_entry_.cmd_)
        {
          //do nothing
        }
        //add 20150701:e
        ...
        {
          YYSYS_LOG(ERROR, "log_applier->end_transaction(task=%p)=>%d", task, err);
        }
        //add shili [LONG_TRANSACTION_LOG]  20160926:b
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        if (OB_EAGAIN == err)
        ...
        else
        {
          ...
          if (task->is_last_log_of_batch())
          {
            ...
            {
              // 失败之后不释放
              YYSYS_LOG(ERROR, "flush(end_id=%ld, %p[%ld])=>%d", task->log_id_,
    ...
    int ObLogReplayWorker::handle_apply(ObLogTask* task)
    {
      ...
      else
      {
        ...
        {
          err_ = err;
          YYSYS_LOG(ERROR, "replay()=>%d", err);
        }
        while(!_stop && OB_SUCCESS == err_ && OB_EAGAIN == (err = commit_queue_.add(task->log_id_, (void*)task)));
        if (OB_SUCCESS != err && OB_EAGAIN != err)
        {
          err_ = err;
          YYSYS_LOG(ERROR, "commit_queue.add(%ld, %p)=>%d", task->log_id_, task, err);
        }
      }
      if (OB_SUCCESS != err)
      {
    ...
    bool ObLogReplayWorker::is_all_task_finished() const
    {
      ...
      //YYSYS_LOG(INFO, "WJH_INFO check_all_task_finished next_submit_log_id=%ld next_commit_log_id=%ld", next_submit_log_id_, next_commit_log_id_);
    ...
    {
      int err = OB_SUCCESS;
      //YYSYS_LOG(INFO, "replay(task.log_id[%ld], next_submit_log_id[%ld], next_commit_log_id[%ld])", task.log_id_, next_submit_log_id_, next_commit_log_id_);
      if (task.log_id_ == 0)
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "task.log_id[%ld] is invalid", task.log_id_);
      }
      ...
      {
        YYSYS_LOG(ERROR, "log_applier->start_transaction()=>%d", err);
      }
      else if (OB_SUCCESS != (err = log_applier_->apply(task)))
      {
        if (OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "log_applier->apply()=>%d", err);
        }
      }
      return err;
    }
    ...
    static int is_barrier_log(bool& is_barrier, const LogCommand cmd, const char* buf, const int64_t len)
    {
      ...
      else if (OB_LOG_NOP == cmd || OB_LOG_SWITCH_LOG == cmd || OB_UPS_MUTATOR_PREPARE == cmd
               || OB_PART_TRANS_COMMIT == cmd || OB_PART_TRANS_ROLLBACK == cmd) //[629]mod
      {
        is_barrier = false;
      }
      else if (OB_LOG_UPS_MUTATOR == cmd)
      {
        ...
        {
          YYSYS_LOG(ERROR, "mutator.deserialize(buf=%p[%ld])=>%d", buf, len, err);
        }
    ...
    int ObLogReplayWorker::submit(ObLogTask& task, int64_t& log_id, const char* buf, int64_t len, int64_t& pos, const ReplayType replay_type)
    {
      ...
      //YYSYS_LOG(INFO, "submit(task.log_id[%ld], next_submit_log_id[%ld], next_commit_log_id[%ld])", task.log_id_, next_submit_log_id_, next_commit_log_id_);
      if (_stop)
      {
        err = OB_CANCELED;
      }
      ...
      {
        YYSYS_LOG(ERROR, "task.log_entry.deserialize()=>%d", err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "log_entry.check_header_integrity()=>%d", err);
      }
      ...
      {
        err = OB_DISCONTINUOUS_LOG;
        YYSYS_LOG(ERROR, "next_submit_log_id[%ld] != task.log_id[%ld], replay_cursor=%s",
      ...
      {
        err = OB_EAGAIN;
        YYSYS_LOG(DEBUG, "next_commit_id[%ld] + flying_limit[%ld] <= cur_seq[%ld]",
      ...
      {
        YYSYS_LOG(ERROR, "is_barrier_log()=>%d", err);
      }
      else
      {
        ...
        log_id = task.log_id_;
        task.profile_.enable_ = (TraceLog::get_log_level() <= YYSYS_LOG_LEVEL_INFO);
        ...
        if (OB_SUCCESS != (err = apply_worker_.push(&task))
            && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "queue_.push(log_id=%ld)=>%d", log_id, err);
        }
        else if (OB_EAGAIN == err)
        {
          //YYSYS_LOG(WARN, "queue.push(log_id=%ld): EAGAIN", log_id);
        }
        else if (OB_SUCCESS != (err = replay_cursor_.advance(entry)))
        {
          YYSYS_LOG(ERROR, "replay_cursor.advance()=>%d", err);
        }
        else
        {
          if (is_barrier)
    ...
    int ObLogReplayWorker::submit(int64_t& log_id, const char* buf, int64_t len, int64_t& pos, const ReplayType replay_type)
    {
      ...
      {
        err = err_;
        //YYSYS_LOG(ERROR, "replay_worker err_=%d", err);
      }
      else if (NULL == (task = task_pool_.alloc()))
      {
        err = OB_EAGAIN;
        YYSYS_LOG(ERROR, "task_pool.alloc(free=%ld)=>NULL", task_pool_.get_free_num());
      }
      else if (OB_SUCCESS != (err = submit(*task, log_id, buf, len, pos, replay_type))
               && OB_EAGAIN != err)
      {
        YYSYS_LOG(ERROR, "submit(next_submit_id=%ld)=>%d", next_submit_log_id_, err);
      }
      else if (OB_EAGAIN == err)
      {
        task_pool_.free(task);
    ...
    int ObLogReplayWorker::submit_batch(int64_t& log_id, const char* buf, int64_t len, const ReplayType replay_type)
    {
      ...
      char* new_buf = NULL;
      int64_t pos = 0;
      allocator_.set_mod_id(ObModIds::OB_UPS_REPLAY_LOG_TASK);
      if (NULL == (new_buf = (char*)allocator_.alloc(len)))
      {
        err = OB_EAGAIN;
        YYSYS_LOG(ERROR, "alloc(%ld) failed", len);
      }
      else
      {
      ...
      while(OB_SUCCESS == err && pos < len)
      {
        if (OB_SUCCESS != (err = submit(log_id, new_buf, len, pos, replay_type))
            && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "submit(log_id=%ld, buf=%p[%ld], pos=%ld,rt=%d)=>%d",
    ...
    //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
    int ObLogReplayWorker::handle_unsettled_trans()
    {
      return log_applier_->handle_unsettled_trans();
    }
    //add 20150701:e
