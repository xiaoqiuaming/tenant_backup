...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int set_state_as_fatal()
    {
      ...
      if (NULL == main)
      {
        YYSYS_LOG(ERROR, "get updateserver main null pointer");
        ret = OB_ERROR;
      }
      else
      {
    ...
    ObUpsTableMgr :: ObUpsTableMgr(ObILogWriter &log_writer) : log_buffer_(NULL),
                                                               ...
                                                               has_started_(false),
                                                               last_bypass_checksum_(0),tmp_schema_mutex_(0),//add zhaoqiong [Schema Manager] 20150327:b
                                                               paxos_id_(-1) //[588]
    {
    }
    ...
    int ObUpsTableMgr :: init(uint64_t start_major_version)
    {
      int err = OB_SUCCESS;

      //err = table_mgr_.init();
      err = table_mgr_.init(start_major_version);
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to init memtable list, err=%d", err);
      }
      else if (NULL == (log_buffer_ = (char*)ob_malloc(LOG_BUFFER_SIZE, ObModIds::OB_UPS_COMMON)))
      {
        YYSYS_LOG(WARN, "malloc log_buffer fail size=%ld", LOG_BUFFER_SIZE);
        err = OB_ERROR;
      }
    ...
    uint64_t ObUpsTableMgr :: get_cur_major_version()
    {
      return table_mgr_.get_cur_major_version();
    }
    //add 20160229:e

    //add zhaoqiong [244_bugfix]:20190710:b
    int ObUpsTableMgr :: get_last_truncate_timestamp_in_active(uint32_t uid, uint64_t table_id, int64_t &timestamp)
    {
      ...
      if(NULL == item)
      {
         ...
         YYSYS_LOG(ERROR, "active_memtable_ is NULL");
      }
      else
      {
         TEKey key;
         ...
         {
             YYSYS_LOG(WARN, "scan_one_table table_id=%ld, ret=%d", table_id, ret);
         }
         else if (NULL == table_value)
         {
            //do nothing
         }
         else if((table_value !=NULL) && OB_SUCCESS != (ret = table_value->row_lock.try_shared_lock(uid)))
         {
            YYSYS_LOG(ERROR, "get share lock failed, key.table_id=%ld", key.table_id);
    ...
    int ObUpsTableMgr :: get_frozen_bloomfilter(const uint64_t version, TableBloomFilter &table_bf)
    {
      ...
      UNUSED(version);
      UNUSED(table_bf);
      YYSYS_LOG(WARN, "no get frozen bloomfilter impl now");
      return ret;
    }
    ...
    int ObUpsTableMgr :: start_transaction(const MemTableTransType type, UpsTableMgrTransHandle &handle)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to acquire active memtable");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObUpsTableMgr :: end_transaction(UpsTableMgrTransHandle &handle, bool rollback)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param cur_memtable null pointer");
      }
      else
      {
        if (rollback)
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "flush log or end transaction fail ret=%d, enter FATAL state", ret);
        set_state_as_fatal();
    ...
    bool ObUpsTableMgr :: need_lock_partition() const
    {
      return table_mgr_.need_lock_partition();
    }
    //add 20150601:e
    //mod peiouya [MultiUPS] [UPS_Manage_Function] 20150521:b
    ...
    int ObUpsTableMgr :: freeze_memtable(const TableMgr::FreezeType freeze_type, uint64_t &frozen_version, bool &report_version_changed,
                                        const ObPacket *resp_packet, const uint64_t limit_version)
    {
      int ret = OB_SUCCESS;
      ...
      {
        YYSYS_LOG(WARN, "get schema mgr fail ret=%d", ret);
      }
      else if (schema_manager.get_version() <= CORE_SCHEMA_VERSION)
      {
        ret = OB_EAGAIN;
        YYSYS_LOG(ERROR, "try freeze but schema not ready, version=%ld", schema_manager.get_version());
      }
      else if (OB_SUCCESS == (ret = table_mgr_.try_freeze_memtable(freeze_type, new_version, frozen_version,
                                                              new_log_file_id, freeze_time_stamp, report_version_changed, limit_version)))
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize freeze_param fail ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = schema_mgr_.get_schema_mgr(schema_manager)))
        {
          YYSYS_LOG(WARN, "get schema mgr fail ret=%d", ret);
        ...
        {
          YYSYS_LOG(WARN, "serialize schema manager fail ret=%d", ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "add cell to ups_mutator fail ret=%d", ret);
          }
          else
          {
        ...
        while(OB_SUCCESS == ret && schema->need_next_serialize())
        {
          ...
          {
            YYSYS_LOG(WARN, "prepare_for_next_serialize fail ret=%d", ret);
          }
          ...
          {
            YYSYS_LOG(WARN, "ups_mutator serialilze fail log_buffer=%p log_buffer_size=%ld serialize_size=%ld ret=%d",
          ...
          else
          {
            ...
            YYSYS_LOG(INFO, "ups table mgr write schema next.");
            ret = log_mgr.write_and_flush_log(OB_UPS_WRITE_SCHEMA_NEXT, log_buffer_, serialize_size);
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "write log fail log_buffer_=%p serialize_size=%ld ret=%d", log_buffer_, serialize_size, ret);
            }
            else if (OB_SUCCESS != (ret = log_mgr.flush_log(TraceLog::get_logbuffer())))
            {
              YYSYS_LOG(WARN, "flush log fail ret=%d", ret);
            }
          }
        }
        //add:e
        YYSYS_LOG(INFO, "write freeze_op log ret=%d new_version=%lu frozen_version=%lu new_log_file_id=%lu op_flag=%d",
                  ret, new_version, frozen_version, new_log_file_id, 0);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "enter FATAL state.");
          set_state_as_fatal();
          ret = OB_RESPONSE_TIME_OUT;
        }
        else
      ...
      //[588]
      if(SYS_TABLE_PAXOS_ID == paxos_id_)
      {
          table_mgr_.release_memtable_lock();
      }
      return ret;
    }
    ...
    int ObUpsTableMgr :: handle_freeze_log_(ObUpsMutator &ups_mutator, const ReplayType replay_type)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "next cell for freeze ups_mutator fail ret=%d", ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret
            || NULL == mutator_ci)
        {
          YYSYS_LOG(WARN, "get cell from freeze ups_mutator fail ret=%d", ret);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "get freeze_param from freeze ups_mutator fail ret=%d header=%p length=%d",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "there is a old clog version=%d follow a new clog version=%ld",
          ...
          case 1:
            {
              ...
              YYSYS_LOG(INFO, "replay freeze memtable using freeze_param_v1 active_version=%ld new_log_file_id=%ld op_flag=%lx ret=%d",
          ...
          case 2:
            {
              ...
              if (OB_SUCCESS == (ret = schema_manager.deserialize(data, length, pos)))
              {
                ...
                {
                  YYSYS_LOG(WARN, "schema version=%ld too old, will not use", schema_manager.get_version());
                }
              }
              if (OB_SUCCESS == ret)
              {
              ...
              YYSYS_LOG(INFO, "replay freeze memtable using freeze_param_v2 active_version=%ld after_trans=%ld "
          ...
          case 3:
            {
              ...
              YYSYS_LOG(INFO, "replay freeze memtable using freeze_param_v3 active_version=%ld after_trans=%ld "
                        "new_log_file_id=%ld op_flag=%lx ret=%d",
          ...
          case 4:
            {
              ...
              if (OB_SUCCESS == (ret = schema_manager.deserialize(data, length, pos)))
              {
                ...
                {
                  YYSYS_LOG(WARN, "schema version=%ld too old, will not use", schema_manager.get_code_version());
                }
              }
              //mod zhaoqiong [Schema Manager] 20150327:b
              /**
              ...
              //mod:e
              {
                ...
                if (OB_SUCCESS == ret)
                {
                          ...
                          || freeze_param_v4->op_flag & FLAG_MAJOR_LOAD_BYPASS))
                  {
                    ...
                    YYSYS_LOG(INFO, "replay load bypass ret=%d loader_num=%ld", tmp_ret, loaded_num);
                  }
                }
              }
              YYSYS_LOG(INFO, "replay freeze memtable using freeze_param_v4 active_version=%ld new_log_file_id=%ld op_flag=%lx ret=%d",
                        freeze_param_v4->active_version, freeze_param_v4->new_log_file_id, freeze_param_v4->op_flag, ret);
              break;
            }
          default:
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "freeze_param version error %d", header->version);
            break;
        }
        //mod zhaoqiong [Schema Manager] 20150327:b
        //exp:if schema no completion, do nothing
    ...
    int ObUpsTableMgr :: replay(ObUpsMutator &ups_mutator, const ReplayType replay_type)
    {
      ...
      if (ups_mutator.is_freeze_memtable())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to handle freeze log, ret=%d", ret);
        }
      }
      else if (ups_mutator.is_drop_memtable())
      {
        YYSYS_LOG(INFO, "ignore drop commit log");
      }
      else if (ups_mutator.is_first_start())
      {
        ...
        YYSYS_LOG(INFO, "handle first start flag log");
      }
      else if (ups_mutator.is_check_cur_version())
      {
        if (table_mgr_.get_cur_major_version() != ups_mutator.get_cur_major_version())
        {
          YYSYS_LOG(ERROR, "cur major version not match, table_major=%lu mutator_major=%lu table_minor=%lu mutator_minor=%lu",
        ...
        {
          YYSYS_LOG(ERROR, "last bypass checksum not match, local_checksum=%lu mutator_checksum=%lu",
                    last_bypass_checksum_, ups_mutator.get_last_bypass_checksum());
          ret = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "check cur version succ, cur_major_version=%lu cur_minor_version=%lu last_bypass_checksum=%lu",
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to set mutator, ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObUpsTableMgr :: flush_obj_to_log(const LogCommand log_command, T &obj)
    {
      ...
      else
      {
        ...
        if (NULL == log_buffer_)
        {
          YYSYS_LOG(WARN, "log buffer malloc fail");
          ret = OB_ERROR;
        }
        else if (OB_SUCCESS != (ret = ups_serialize(obj, log_buffer_, LOG_BUFFER_SIZE, serialize_size)))
        {
          YYSYS_LOG(WARN, "obj serialilze fail log_buffer=%p log_buffer_size=%ld serialize_size=%ld ret=%d",
                    log_buffer_, LOG_BUFFER_SIZE, serialize_size, ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "write log fail log_command=%d log_buffer_=%p serialize_size=%ld ret=%d",
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "write log fail ret=%d, enter FATAL stat", ret);
            set_state_as_fatal();
            ret = OB_RESPONSE_TIME_OUT;
          }
        }
    ...
    int ObUpsTableMgr :: write_start_log()
    {
      ...
      if (has_started_)
      {
        YYSYS_LOG(INFO, "system has started need not write start flag");
      }
      else
      {
        ObUpsMutator ups_mutator;
        ...
        ret = flush_obj_to_log(OB_LOG_UPS_MUTATOR, ups_mutator);
        YYSYS_LOG(INFO, "write first start flag ret=%d", ret);
      }
      return ret;
    }
    ...
    int ObUpsTableMgr :: set_mutator_(ObUpsMutator& mutator)
    {
      ...
      {
        YYSYS_LOG(WARN, "start transaction fail ret=%d", ret);
      }
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "set to memtable fail ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "replay mutator succ");
          }
    ...
    int ObUpsTableMgr :: apply(const bool using_id, UpsTableMgrTransHandle &handle, ObUpsMutator &ups_mutator, ObScanner *scanner)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "cellinfo do not pass valid check or trans name to id fail");
          ret = OB_SCHEMA_ERROR;
        }
        else if (OB_SUCCESS != p_active_memtable->start_mutation(handle.trans_descriptor))
        {
          YYSYS_LOG(WARN, "start mutation fail trans_descriptor=%lu", handle.trans_descriptor);
          ret = OB_ERROR;
        }
        else
        {
...
//		if (OB_SUCCESS != (ret = p_active_memtable->set(session_ctx, iter, dml_type))
        if (OB_SUCCESS != (ret = p_active_memtable->set(session_ctx, iter, dml_type,is_index))
		//add by maosy 
            && !IS_SQL_ERR(ret))
        {
          YYSYS_LOG(WARN, "set to memtable fail ret=%d", ret);
    ...
    //add peiouya [secondary_index_OPTI_CHECKSUNxxxx] 20180314:b
    int ObUpsTableMgr ::rollback(RWSessionCtx &session_ctx)
    {
      int ret = OB_SUCCESS;
      TableItem *table_item = NULL;
      if (NULL == (table_item = table_mgr_.get_active_memtable()))
      ...
      {
        MemTable *p_active_memtable = &(table_item->get_memtable());
        ret = p_active_memtable->rewrite_tevalue(session_ctx);
        table_mgr_.revert_active_memtable(table_item);
      } 
      return ret;
    }
                              ...
                              ObMutator &mutator)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          FILL_TRACE_BUF(session_ctx.get_tlog_buffer(), "set to memtable ret=%d", ret);
          //int64_t ts = yysys::CTimeUtil::getTime();
          //char fname[1024];
          //snprintf(fname, 1024, "./%ld", ts);
          //YYSYS_LOG(INFO, "wait for file %s", fname);
          //struct stat st;
    ...
    int ObUpsTableMgr :: add_memtable_uncommited_checksum(uint64_t *checksum)
    {
      ...
      if (NULL == checksum)
      {
        YYSYS_LOG(WARN, "invalid param, checksum null pointer");
    ...
    void ObUpsTableMgr :: log_memtable_memory_info()
    {
      ...
      if (1 == (ATOMIC_ADD(&counter, 1) % mod))
      {
        ...
        YYSYS_LOG(INFO, "DML total=%lu, TPS=%ld", counter, 1000000 * mod/(cur_ts - last_report_ts));
    ...
    int ObUpsTableMgr :: set_schemas(const CommonSchemaManagerWrapper &schema_manager)
    {
      ...
      //mod:e
        ...
        {
          YYSYS_LOG(WARN, "set_schema(new_version=%ld, cur_ups_version=%ld): NO need to switch",
                    schema_manager.get_version(), schema_mgr_.get_version());
        }
        else if (OB_SUCCESS != (ret = schema_mgr_.set_schema_mgr(schema_manager)))
        {
          YYSYS_LOG(ERROR, "set_schemas error, ret=%d schema_version=%ld", ret, schema_manager.get_version());
        }
        else
        {
          YYSYS_LOG(INFO, "set_schemas(version[%ld->%ld])", schema_mgr_.get_version(), schema_manager.get_version());
          YYSYS_LOG(INFO, "==========print schema start==========");
          schema_manager.print_info();
          YYSYS_LOG(INFO, "==========print schema end==========");
        }
      }
    ...
    int ObUpsTableMgr :: replay_mgt_next(const char* buf, const int64_t buf_len, int64_t& pos, const ReplayType replay_type)
    {
      ...
      {
        YYSYS_LOG(ERROR, "freeze_param deserialize fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = tmp_schema_mgr_.deserialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(ERROR, "schema_manager deserialize fail, ret=%d", ret);
      }
      else if (tmp_schema_mgr_.get_version() < schema_mgr_.get_version())
      {
        ret = OB_VERSION_NOT_MATCH;
        YYSYS_LOG(ERROR, "should not be here.set_schema(new_version=%ld, cur_ups_version=%ld): NO need to switch",
      ...
      */
      ...
      if (OB_SUCCESS == ret && tmp_schema_mgr_.get_impl()->is_completion())
      {
        if (major_version_changed)
        {
         // ups_main->get_update_server().submit_report_freeze(); //uncertainty  ups 中删除
        }
        ups_main->get_update_server().submit_handle_frozen();
      }
    ...
    int ObUpsTableMgr::set_schema_next(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "schema_manager deserialize error, ret=%d", ret);
      }
      else if (!tmp_schema_mgr_.get_impl()->is_completion())
      {
        YYSYS_LOG(INFO, "schema is not completion");
      ...
      else
      {
        ...
        {
          ret = OB_VERSION_NOT_MATCH;
          YYSYS_LOG(ERROR, "set_schema(new_version=%ld, cur_ups_version=%ld): NO need to switch",
                    tmp_schema_mgr_.get_version(), schema_mgr_.get_version());
        }
        else if (OB_SUCCESS != (ret = schema_mgr_.set_schema_mgr(tmp_schema_mgr_)))
        {
          YYSYS_LOG(ERROR, "set_schemas error, ret=%d schema_version=%ld", ret, tmp_schema_mgr_.get_version());
        }
        else
        {
          YYSYS_LOG(INFO, "set_schemas(version[%ld->%ld])", schema_mgr_.get_version(), tmp_schema_mgr_.get_version());
          YYSYS_LOG(INFO, "==========print schema start==========");
    ...
    int ObUpsTableMgr:: switch_schema_mutator(const ObSchemaMutator &schema_mutator)
    {
      ...
      {
        ret = OB_SUCCESS;
        YYSYS_LOG(WARN, "mutator contain refresh schema, skip this mutator");
      }
      else if (schema_mutator.get_end_version() <= schema_mutator.get_start_version())
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "schema mutator error, not a valid mutator, start_version:%ld, end_version:%ld",
      ...
      {
        YYSYS_LOG(ERROR, "set_schemas error, ret=%d schema_version=%ld", ret, schema_mutator.get_end_version());
      }
      else if (OB_SUCCESS != (ret = write_schema_mutator(schema_mutator)))
      {
        YYSYS_LOG(ERROR, "write_schema_mutator(version=%ld)=>%d", schema_mutator.get_end_version(), ret);
      }
      else
      {
        YYSYS_LOG(INFO, "set_schema mutator(version[%ld->%ld])", schema_mutator.get_start_version(), schema_mutator.get_end_version());
      }
      return ret;
    }
    ...
    int ObUpsTableMgr::write_schema_mutator(const ObSchemaMutator &schema_mutator)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "ups table mgr switch schema mutator.");
    ...
    int ObUpsTableMgr::switch_tmp_schemas()
    {
      ...
      {
        ret = OB_EAGAIN;
        YYSYS_LOG(ERROR, "unexpected error, schema should be locked");
      }
      else if (!tmp_schema_mgr_.get_impl()->is_completion())
      {
        ret = OB_INNER_STAT_ERROR;
        YYSYS_LOG(WARN, "tmp schema mgr is not completion");
      }
      else
      {
    ...
    int ObUpsTableMgr:: write_schema(const CommonSchemaManagerWrapper &schema_manager)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ObUpsLogMgr& log_mgr = main->get_update_server().get_log_mgr();
          YYSYS_LOG(INFO, "ups table mgr switch schemas.");
      ...
      while(OB_SUCCESS == ret && schema->need_next_serialize())
      {
        ...
        {
          ObUpsLogMgr& log_mgr = main->get_update_server().get_log_mgr();
          YYSYS_LOG(INFO, "ups table mgr write switch schemas next log.");
          ret = log_mgr.write_and_flush_log(OB_UPS_SWITCH_SCHEMA_NEXT, log_buffer_, serialize_size);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "write log for next schema part fail log_buffer_=%p serialize_size=%ld ret=%d", log_buffer_, serialize_size, ret);
          }
        }
      }
      //add:e
    ...
    int ObUpsTableMgr :: switch_schemas(const CommonSchemaManagerWrapper &schema_manager)
    {
      ...
      //mod:e
      {
        YYSYS_LOG(WARN, "switch_schema(new_version=%ld, cur_ups_version=%ld): NO need to switch",
                  schema_manager.get_version(), schema_mgr_.get_version());
      }
      else if (OB_SUCCESS != (ret = schema_mgr_.set_schema_mgr(schema_manager)))
      {
        YYSYS_LOG(ERROR, "set_schemas error, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = write_schema(schema_manager)))
      {
        YYSYS_LOG(ERROR, "write_schema(version=%ld)=>%d", schema_manager.get_version(), ret);
      }
      else
      {
        YYSYS_LOG(INFO, "switch_schemas(version[%ld->%ld])", schema_mgr_.get_version(), schema_manager.get_version());
        YYSYS_LOG(INFO, "==========print schema start==========");
        schema_manager.print_info();
        YYSYS_LOG(INFO, "==========print schema end==========");
      }
    ...
    int ObUpsTableMgr :: create_index()
    {
      ...
      YYSYS_LOG(WARN, "no create index impl now");
      return ret;
    }
                              ...
                              const sql::ObLockFlag lock_flag/* = sql::LF_NONE*/)
    {
      ...
      if (NULL == table_list)
      {
        YYSYS_LOG(WARN, "get tsi table_list fail");
        ret = OB_ERROR;
      }
          ...
          || 0 == table_list->size())
      {
        YYSYS_LOG(WARN, "acquire table fail version_range=%s", range2str(get_param.get_version_range()));
      ...
      else
      {
        ...
        for (iter = table_list->begin(); iter != table_list->end(); iter++, index++)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid table_entity version_range=%s", range2str(get_param.get_version_range()));
            ret = OB_ERROR;
            break;
          }
          if (NULL == (table_utils = table_entity->get_tsi_tableutils(index)))
          {
            YYSYS_LOG(WARN, "get tsi tableutils fail index=%ld", index);
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to get_ ret=%d", ret);
          }
          else
          {
            //del lijianqiang [MultiUPS] [SELECT_MERGE] 20151230:b
            /**
             * the last row will be added to scanner in ObCellNewScanner::finish(),if not overflow and
             * the last row is valid ,we need the real version,not the max valid version
             */
                              ...
                              const int64_t timeout)
    {
      ...
      uint64_t table_id = OB_INVALID_ID; //add zhaoqiong [truncate xxx]
      bool is_final_minor = false;
      if (NULL == table_list)
      {
        YYSYS_LOG(WARN, "get tsi table_list fail");
      ...
      else if (NULL == (scan_range = scan_param.get_range()))
      {
        ...
        ret = OB_ERROR;
              ...
              || 0 == table_list->size())
      {
        YYSYS_LOG(WARN, "acquire table fail version_range=%s", range2str(scan_param.get_version_range()));
        ret = (OB_SUCCESS == ret) ? OB_INVALID_START_VERSION : ret;
      }
      else
      {
        ...
        do
        {
          ...
          bool has_merger_iter = true;
          //add 20151228:e
          bool truncate_status = false; //add zhaoqiong [truncate xxx]
          for (iter = table_list->begin(); iter != table_list->end(); iter++, index++)
          {
            ITableEntity *table_entity = *iter;
            ITableUtils *table_utils = NULL;
            ITableIterator *table_iter = NULL;
            truncate_status = false; //add zhaoqiong [truncate xxx]
            if (NULL == table_entity)
            {
              YYSYS_LOG(WARN, "invalid table_entity version_range=%s", range2str(scan_param.get_version_range()));
              ret = OB_ERROR;
              break;
            }
            if (NULL == (table_utils = table_entity->get_tsi_tableutils(index)))
            ...
            {
              YYSYS_LOG(WARN, "alloc table iterator fai index=%ld", index);
              ret = OB_MEM_OVERFLOW;
              break;
            }
            ...
            {
              YYSYS_LOG(WARN, "table entity scan fail ret=%d scan_range=%s", ret, scan_range2str(*scan_range));
              break;
            }
            sst_id = (NULL == table_entity) ? 0 : table_entity->get_table_item().get_sstable_id();
            ...
            if (OB_SUCCESS == ret)
            {
              uint64_t current_table_item_version = OB_INVALID_VERSION;
              //mod wangyao [316_bugfix]:20190722b
//              if (ITableEntity::SSTABLE == table_entity->get_table_type())
//              {
//                YYSYS_LOG(DEBUG,"is SSTABLE");
//                current_table_item_version = table_entity->get_table_item().get_sstable_id();
//              }
//              else
//              {
//                current_table_item_version = table_entity->get_table_item().get_memtable().get_version();
//                YYSYS_LOG(DEBUG,"is MEMETABLE");
//              }
              current_table_item_version = table_entity->get_table_item().get_sstable_id();
              //mod:e
              table_iter->set_version(SSTableID::get_major_version(current_table_item_version));
              YYSYS_LOG(DEBUG, "current table version is[%ld]",table_iter->get_version());
            }
            //add 20151228:e
            if (1 == table_list->size())
            {
              ret_iter = table_iter;
              //add lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
              has_merger_iter = false;
              YYSYS_LOG(DEBUG,"has merger iter,table_list_size=%ld",table_list->size());
              //add 20151228:e
              break;
            }
//mod zhaoqiong [truncate]:b
            ...
            {
              YYSYS_LOG(WARN, "failed to get_table_truncate_stat=%d", ret);
              break;
            }
            else if (OB_SUCCESS != (ret = merger.add_iterator(table_iter, truncate_status)))
//mod:e
            {
              YYSYS_LOG(WARN, "add iterator to merger fail ret=%d", ret);
              break;
            }
          }
          ...
          {
            int64_t row_count = 0;
            //mod lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
            //ret = add_to_scanner_(*ret_iter, scanner, row_count, start_time, timeout, scan_param.get_scan_size());
            ret = add_to_scanner_(*ret_iter, scanner, row_count, start_time, timeout, has_merger_iter, scan_param.get_scan_size());
            //mod 20151228:e
            ...
            if (OB_SIZE_OVERFLOW == ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "memory is not enough to add even one row");
                ret = OB_ERROR;
              }
            }
            else if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to add data from ups_merger to scanner, ret=%d", ret);
            }
            else
            {
                              ...
                              const sql::ObLockFlag lock_flag)
    {
      ...
      if (cell_size <= 0)
      {
        YYSYS_LOG(WARN, "invalid param, cell_size=%ld", cell_size);
        err = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (cell = get_param[0]))
      {
        YYSYS_LOG(WARN, "invalid param first cellinfo");
        err = OB_ERROR;
      }
      else
      {
        ...
        while (OB_SUCCESS == err && cell_idx < cell_size)
        {
          ...
          {
            YYSYS_LOG(WARN, "cellinfo null pointer idx=%ld", cell_idx);
            err = OB_ERROR;
          }
                  ...
                  cell->table_id_ != last_table_id)
          {
            ...
            {
              YYSYS_LOG(WARN, "allocate memory failed, first_idx=%ld, last_idx=%ld",
                  last_cell_idx, cell_idx - 1);
            }
            else if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to get_row_, first_idx=%ld, last_idx=%ld, err=%d",
            ...
            else
            {
              if ((start_time + timeout) < g_cur_time)
              {
                if (last_cell_idx > 0)
                {
                  ...
                  YYSYS_LOG(WARN, "get or scan too long time, start_time=%ld timeout=%ld timeu=%ld row_count=%ld",
                      start_time, timeout, yysys::CTimeUtil::getTime() - start_time, last_cell_idx);
                  err = OB_FORCE_TIME_OUT;
                }
                else
                {
                  YYSYS_LOG(ERROR, "can't get any row, start_time=%ld timeout=%ld timeu=%ld",
                      start_time, timeout, yysys::CTimeUtil::getTime() - start_time);
                                  ...
                                  const sql::ObLockFlag lock_flag)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, first_cell_idx=%ld, last_cell_idx=%ld",
            first_cell_idx, last_cell_idx);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (cell_size <= last_cell_idx)
      {
        YYSYS_LOG(WARN, "invalid status, cell_size=%ld, last_cell_idx=%ld",
            cell_size, last_cell_idx);
        ret = OB_ERROR;
      }
      else if (NULL == (cell = get_param[first_cell_idx]))
      {
        YYSYS_LOG(WARN, "cellinfo null pointer idx=%ld", first_cell_idx);
        ret = OB_ERROR;
      }
      else if (NULL == (column_filter = ITableEntity::get_tsi_columnfilter()))
      {
        YYSYS_LOG(WARN, "get tsi columnfilter fail");
        ret = OB_ERROR;
      }
      else
      {
        ...
        //add lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
        bool has_merger_iter = true;  //是否对 多个版本的memtable 的数据进行合并
        //add 20151228:e
        //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
        column_filter->set_read_atomic_param(get_param.get_read_atomic_param());
        //add duyr 20151207:e
        ...
        for (iter = table_list.begin(), index = 0; iter != table_list.end(); iter++, index++)
        {
          ...
          {
            YYSYS_LOG(WARN, "table entity get fail ret=%d table_id=%lu row_key=[%s] columns=[%s]",
                      ret, table_id, to_cstring(row_key), column_filter->log_str());
            break;
          }
          YYSYS_LOG(DEBUG, "get row row_key=[%s] row_key_ptr=%p columns=[%s] iter=%p",
...
//mod:e
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          ret = add_to_scanner_(*ret_iter, scanner, row_count, start_time, timeout, has_merger_iter);
          //add 20151228:e
          if (OB_SIZE_OVERFLOW == ret)
          {
            // return ret
          ...
          {
            YYSYS_LOG(WARN, "failed to add data from merger to scanner, ret=%d", ret);
          }
          else
          {
      ...
      YYSYS_LOG(DEBUG, "[op=GET_ROW] [first_cell_idx=%ld] [last_cell_idx=%ld] [ret=%d]", first_cell_idx, last_cell_idx, ret);
      return ret;
    }
    ...
    int ObUpsTableMgr::check_permission_(ObMutator &mutator, const IToken &token)
    {
            ...
            && OB_SUCCESS == (ret = mutator.next_cell()))
      {
            ...
            && NULL != mutator_ci)
        {
          ...
          {
            YYSYS_LOG(WARN, "get_schema_handle fail ret=%d", ret);
          }
          else
          {
            if (NULL == (table_schema = schema_mgr_.get_table_schema(schema_handle, ci->table_name_)))
            {
              YYSYS_LOG(WARN, "get schema fail table_name=%.*s table_name_length=%d",
            ...
            else if (OB_SUCCESS != (ret = main->get_update_server().get_perm_table().check_table_writeable(token, table_schema->get_table_id())))
            {
              ...
              YYSYS_LOG(WARN, "check write permission fail ret=%d table_id=%lu table_name=%.*s user_name=%.*s",
            ...
            && OB_SUCCESS == (ret = mutator.next_cell()))
      {
            ...
            && NULL != mutator_ci)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "get column schema fail table_name=%.*s table_id=%lu column_name=%.*s column_name_length=%d",
            ...
            {
              YYSYS_LOG(WARN, "rowkey column cannot be update %s", print_cellinfo(ci));
              ret = OB_SCHEMA_ERROR;
            }
            // 数据类型与合法性检查
    ...
    int ObUpsTableMgr::fill_prepare_log(ObUpsMutator &ups_mutator, const common::ObTransID& coo_trans_id, TraceLog::LogBuffer &tlog_buffer)
    {
      return fill_prepare_log_(ups_mutator, coo_trans_id,tlog_buffer);
    }
    //add 20150701:e
    ...
    int ObUpsTableMgr::fill_commit_log_(ObUpsMutator &ups_mutator, TraceLog::LogBuffer &tlog_buffer)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "log buffer full");
          ret = OB_EAGAIN;
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write log fail ret=%d, enter FATAL state", ret);
          set_state_as_fatal();
          ret = OB_RESPONSE_TIME_OUT;
        }
      }
...
#endif
    //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
    int ObUpsTableMgr::fill_prepare_log_(ObUpsMutator &ups_mutator,const common::ObTransID& coo_trans_id, TraceLog::LogBuffer &tlog_buffer)
    {
      int ret = OB_SUCCESS;
      int64_t serialize_size = 0;
      ObUpdateServerMain *main = ObUpdateServerMain::get_instance();
      ...
      else
      {
        FILL_TRACE_BUF(tlog_buffer, "ups_mutator serialize");
        ObUpsLogMgr& log_mgr = main->get_update_server().get_log_mgr();
        if (OB_BUF_NOT_ENOUGH == (ret = log_mgr.write_log(OB_UPS_MUTATOR_PREPARE, ups_mutator, coo_trans_id)))
        {
          YYSYS_LOG(INFO, "log buffer full");
          ret = OB_EAGAIN;
        }
    ...
    int ObUpsTableMgr::fill_stat_log(const PartCommitInfo& commit_info, const bool commit)
    {
      int ret = OB_SUCCESS;
      ObUpdateServerMain *main = ObUpdateServerMain::get_instance();
      if (NULL == main)
      ...
      {
        LogCommand cmd = OB_PART_TRANS_COMMIT;
        if(!commit)
        {
          cmd = OB_PART_TRANS_ROLLBACK;
        }

        ObUpsLogMgr& log_mgr = main->get_update_server().get_log_mgr();
        if (OB_BUF_NOT_ENOUGH == (ret = log_mgr.write_log(cmd, commit_info)))
        {
          YYSYS_LOG(INFO, "log buffer full");
          ret = OB_EAGAIN;
        }
    ...
    int ObUpsTableMgr::flush_commit_log_(TraceLog::LogBuffer &tlog_buffer)
    {
      ...
      //if (OB_SUCCESS != ret)
      //{
      //  YYSYS_LOG(WARN, "flush log fail ret=%d, will kill self", ret);
      //  kill(getpid(), SIGTERM);
      //}
      return ret;
    };
    ...
    int ObUpsTableMgr::load_sstable_bypass(SSTableMgr &sstable_mgr, int64_t &loaded_num)
    {
      ...
      //add liyuanfei check-ups-sstable-continuity 20190418:b
      uint64_t last_clog_id = sstable_mgr.get_max_clog_id();
      if(OB_INVALID == last_clog_id)
      {
        YYSYS_LOG(WARN, "invalid param last_clog_id=%lu", last_clog_id);
        last_clog_id = 0;
      }
      //add:e

      uint64_t checksum = 0;
      ret = sstable_mgr.load_sstable_bypass(major_version, minor_version_start, minor_version_end, clog_id, last_clog_id,/*add liyuanfei*/
    ...
    int ObUpsTableMgr::check_cur_version()
    {
      ...
      {
        YYSYS_LOG(WARN, "fill commit log fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = flush_commit_log_(TraceLog::get_logbuffer())))
      {
        YYSYS_LOG(WARN, "flush commit log fail ret=%d", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "write check cur version log succ, cur_major_version=%lu cur_minor_version=%lu",
    ...
    int ObUpsTableMgr::commit_check_sstable_checksum(const uint64_t sstable_id, const uint64_t checksum)
    {
      ...
      {
        YYSYS_LOG(INFO, "write check sstable checksum log succ, cur_major_version=%lu cur_minor_version=%lu checksum=%lu",
    ...
    void ObUpsTableMgr::update_merged_version(ObUpsRpcStub &rpc_stub, const ObServer &root_server, const int64_t timeout_us)
    {
      ...
      while (merged_version <= last_frozen_version)
      {
        ...
        {
          YYSYS_LOG(WARN, "rpc check_table_merged fail ret=%d", tmp_ret);
          break;
        }
        if (!merged)
        {
          YYSYS_LOG(INFO, "major_version=%lu have not merged done", merged_version);
          break;
        }
        else
        {
          table_mgr_.set_merged_version(merged_version, yysys::CTimeUtil::getTime());
          YYSYS_LOG(INFO, "major_version=%lu have merged done, done_time=%ld", merged_version, table_mgr_.get_merged_timestamp());
          merged_version += 1;
        }
      }
      ...
      {
          //[676]
          bool delay_drop = false;
          ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
          if(NULL != ups_main)
          {
              delay_drop = ups_main->get_update_server().get_param().if_delay_drop_memtable;
          }
          if(delay_drop == false)
          {
    ...
    int ObUpsTableMgr::pre_process(const bool using_id, ObMutator& mutator, const IToken *token)
    {
          ...
          && NULL != token)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to check permission ret=%d", ret);
        }
      }
          ...
          && !using_id)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to trans_name2id, ret=%d", ret);
        }
      }
      (void)common_schema_mgr;
    ...
    {
      YYSYS_LOG(INFO, "replay checksum flag=%s", STR_BOOL(check_checksum_));
    ...
    void ObUpsTableMgr::get_ups_used_mem(int64_t &mem_size)
    {
    ...
    bool get_key_prefix(const TEKey &te_key, TEKey &prefix_key)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get schema handle fail ret=%d", tmp_ret);
        }
        else
        {
          if (NULL == (table_schema = tm.schema_mgr_.get_table_schema(schema_handle, te_key.table_id)))
          {
            YYSYS_LOG(ERROR, "get schema fail table_id=%lu", te_key.table_id);
          }
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "row key cannot be splited length=%ld split_pos=%d",
                                ...
                                const sql::ObLockFlag lock_flag/* = sql::LF_NONE*/)
    {
      ...
      if (OB_SUCCESS != (ret = get_(session_ctx, get_param, scanner, start_time,
                                    timeout, lock_flag)))
      {
        YYSYS_LOG(WARN, "ups get fail:ret[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = scanner.finish()))
      {
        YYSYS_LOG(WARN, "finish new scanner fail:ret[%d]", ret);
      }
      return ret;
    }
                                  ...
                                  const int64_t timeout)
    {
      ...
      {
        YYSYS_LOG(WARN, "ups scan fail:ret[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = scanner.finish()))
      {
        YYSYS_LOG(WARN, "finish new scanner fail:ret[%d]", ret);
      ...
      else
      {
        ...
        do
        {
          ...
          for (iter = table_list->begin(); iter != table_list->end(); iter++, index++)
          {
            ...
            if (OB_SUCCESS != (ret = table_entity->start_transaction(table_utils->get_trans_descriptor())))
            {
              YYSYS_LOG(WARN, "start transaction fail ret=%d scan_range=%s", ret, scan_range2str(*scan_range));
              break;
            }
            trans_start_counter++;
...
//mod:e
                                        ...
                                        const int64_t start_time, const int64_t timeout, const int64_t result_limit_size/* = UINT64_SIZE*/)
    {
      ...
      while (OB_SUCCESS == err && (OB_SUCCESS == (err = ups_merger.next_cell())))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get cell, err=%d", err);
          err = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(DEBUG, "from merger %s is_row_changed=%s", print_cellinfo(cell), STR_BOOL(is_row_changed));
          if (is_row_changed)
          {
            ++row_count;
          }
          ...
          {
            YYSYS_LOG(WARN, "get or scan too long time, start_time=%ld timeout=%ld timeu=%ld row_count=%ld",
                      start_time, timeout, yysys::CTimeUtil::getTime() - start_time, row_count - 1);
            err = OB_SIZE_OVERFLOW;
          }
          if (OB_SUCCESS != err)
          {
            ...
            if (1 > row_count)
            {
              YYSYS_LOG(WARN, "invalid row_count=%ld", row_count);
            }
          }
          if (OB_SIZE_OVERFLOW == err)
          {
            YYSYS_LOG(DEBUG, "scanner memory is not enough, rollback last row");
            // rollback last row
            if (OB_SUCCESS != scanner.rollback())
            {
              YYSYS_LOG(WARN, "failed to rollback");
              err = OB_ERROR;
            }
          }
          else if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to add cell, err=%d", err);
          }
        }
      }
    ...
    //add lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
    /*@param  ups_merger[in] 各个版本数据 迭代器
    * @param  has_merger_iter[in] 是否有多个版本的迭代器*/
    template<class T>
    int ObUpsTableMgr :: add_to_scanner_(ObIterator& ups_merger, T& scanner, int64_t& row_count,
                                         const int64_t start_time, const int64_t timeout, bool has_merger_iter, const int64_t result_limit_size/* = UINT64_SIZE*/)
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(DEBUG,"come my add to scanner");
      ObCellInfo* cell = NULL;
      bool is_row_changed = false;
      row_count = 0;

      ObMerger * merger_iter = NULL;
      ITableIterator * table_iter = NULL;
      if (has_merger_iter)
      {
        if (NULL == (merger_iter = dynamic_cast<ObMerger *>(&ups_merger)))
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "dynamic failed, not merger iter,ret=%d",err);
        }
      }
      else
      {
        if (NULL == (table_iter = dynamic_cast<ITableIterator*>(&ups_merger)))
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "dynamic failed, not table iter,ret=%d",err);
        }
      }
      /**
       * both ObScanner and ObCellNewScanner(ObNewScanner) use this interface,we only
       * need the row_version with ObNewScanner for DML select_stmt
       */
      YYSYS_LOG(DEBUG, "my scanner type is=%s",common::OB_NEW_SCANNER_TYPE==scanner.get_my_scanner_type()?"new_scanner":"old_scanner");
      if (common::OB_NEW_SCANNER_TYPE == scanner.get_my_scanner_type())
      {
        scanner.set_has_row_version(true);
      }
      ...
      while (OB_SUCCESS == err && (OB_SUCCESS == (err = ups_merger.next_cell())))
      {
        ...
        else
        {
          ...
          if (has_merger_iter)
          {
            if (NULL == (table_iter = dynamic_cast<ITableIterator *>(merger_iter->get_current_iter())))
            {
              err = OB_ERR_UNEXPECTED;
              YYSYS_LOG(WARN, "dynamic cast failed, get table iter from merger failed,ret=%d",err);
              break;
            }
          }
          if(ObExtendType == cell->value_.get_type())
          {
            ...
            {
              scanner.set_data_version(table_iter->get_version());//set current iter cell version
            }
            else if(ObActionFlag::OP_NOP == cell->value_.get_ext())
            {
              //for get,may return the empty row with max row version
              scanner.set_data_version(table_iter->get_version());
            }
            ...
            else
            {
              ...
              YYSYS_LOG(WARN, "not support ext value:[%ld]", cell->value_.get_ext());
              break;
            }
          }
          else
