...
int ObCommitLogReceiver::init(common::ObLogWriter *log_writer, common::ObBaseServer *base_server, ObUpsSlaveMgr *slave_mgr, const int64_t time_warn_us)
{
  ...
  {
    YYSYS_LOG(WARN, "Parameters are invalid, log_writer=%p base_server=%p", log_writer, base_server);
  }
  else
  {
    ...
    easy_request_t* req, const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  ...
  if (!log_writer_->get_is_log_start())
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "start log_writer error, ret=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "decomposite log entry error, ret=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "flush log error, ret=%d", ret);
    }
  }
...
int ObCommitLogReceiver::start_log(ObDataBuffer& buff)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObLogEntry deserialize error, ret=%d", ret);
  }
  else
  {
    if (OB_LOG_SWITCH_LOG != entry.cmd_)
    {
      YYSYS_LOG(ERROR, "when starting log, the log entry must be SWITCH_LOG");
    }
    else
    {
      uint64_t log_id = 0;
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "decode_i64 log_id error, ret=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "start_log error, log_id=%lu ret=%d", log_id, ret);
        }
      }
    }
  }
...
int ObCommitLogReceiver::write_data(const char* log_data, const int64_t data_len, uint64_t &log_id, uint64_t &log_seq)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObCommitLogReceiver has not initialized");
  }

  if (NULL == log_data || data_len <= 0)
  {
    YYSYS_LOG(WARN, "Parameter is invalid, log_data=%p data_len=%ld", log_data, data_len);
  ...
  while (OB_SUCCESS == ret && pos < data_len)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObLogEntry deserialize error, ret=%d", ret);
    }
    else
    {
      ...
      if (OB_LOG_SWITCH_LOG == log_ent.cmd_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "swith_log is not at the end, this should not happen, "
  ...
  {
    int64_t store_start_time = yysys::CTimeUtil::getTime();
    ret = log_writer_->store_log(log_data, data_len);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObLogWriter store_log error, ret=%d", ret);
    }
    else
    {
    ...
    int64_t store_elapse = yysys::CTimeUtil::getTime() - store_start_time;
    if (store_elapse > time_warn_us_)
    {
      YYSYS_LOG(WARN, "store log time is too long, store_time=%ld cur_time=%ld log_size=%ld",
                store_elapse, yysys::CTimeUtil::getTime(), data_len);
    }
  }
  ...
  //   if (OB_SUCCESS != ret)
  //   {
  //     YYSYS_LOG(WARN, "fail to send data to slave");
  //   }
  // }
  ...
  if (OB_SUCCESS == ret && switch_log_flag)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "switch_to_log_file error, log_id=%lu ret=%d", log_id, ret);
    }
    else
    {
      YYSYS_LOG(INFO, "switch_to_log_file log_id=%lu log_seq=%lu", log_id, log_seq);
      ++log_id;
    }
  }
...
int ObCommitLogReceiver::end_receiving_log(const int32_t version, easy_request_t* req, const int32_t channel_id, common::ObDataBuffer& out_buff)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObLogWriter flush_log error, ret=%d", ret);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObResultCode serialize error, ret=%d", ret);
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "send_response error, ret=%d", ret);
      }
    }
