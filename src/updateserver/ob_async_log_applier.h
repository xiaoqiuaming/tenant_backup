  ...
  namespace updateserver
  {
    ...
    struct ReplayTaskProfile
    {
      ...
      private:
        ...
        { return yysys::CTimeUtil::getTime(); }
      public:
      ...
      void on_submit()
      { if (enable_) submit_time_us_ = get_cur_time_us(); }
      void start_apply()
      { if (enable_) start_apply_time_us_ = get_cur_time_us(); }
      void end_apply()
      { if (enable_) end_apply_time_us_ = get_cur_time_us(); }
      void start_commit()
      { if (enable_) start_commit_time_us_ = get_cur_time_us(); }
      void end_commit()
      { if (enable_) end_commit_time_us_ = get_cur_time_us(); }
      void start_flush()
      { if (enable_) start_flush_time_us_ = get_cur_time_us(); }
      void end_flush()
      { if (enable_) end_flush_time_us_ = get_cur_time_us(); }
    ...
    struct ObLogTask
    {
      ...
      ~ObLogTask();
      void reset();
      bool is_last_log_of_batch()
      { return log_data_ + log_entry_.get_log_data_len() == batch_buf_ + batch_buf_len_; }
    ...
    class IAsyncLogApplier
    {
      ...
      public:
        ...
        virtual int handle_unsettled_trans() = 0;
        //add 20150816:e
    };
    ...
    class ObAsyncLogApplier: public IAsyncLogApplier
    {
      ...
      private:
        ...
        //prepare log has coordinator transid before normal mutator
        //int handle_normal_mutator(ObLogTask& task);
        int handle_normal_mutator(ObLogTask& task, int64_t update_pos = 0, ObTransID* trans_id = NULL);
        int handle_part_trans_commit(ObLogTask& task);
        int handle_part_trans_rollback(ObLogTask& task);
        int handle_unsettled_trans();
        //add 20150701:e
      ...
      private:
        ...
        static const int64_t TASK_NUM = 1000;
        //key: coordinator trans id (from replay log),
        //value: current session trans id (replay log start new session)
        common::hash::ObHashMap<common::ObTransID,common::ObTransID> task_map_;
        //add 20150818:e
    };
