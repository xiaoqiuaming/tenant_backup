...
namespace oceanbase
{
  ...
  namespace updateserver
  {
    ...
    int TransNode::start_mutation()
    {
      ...
      {
        YYSYS_LOG(WARN, "mutation is running, cannot start, start_trans_id=%ld cur_trans_id=%ld "
    ...
    int TransNode::end_mutation(const bool rollback)
    {
      ...
      if (!mutation_started_)
      {
        YYSYS_LOG(WARN, "mutation not start, start_trans_id=%ld cur_trans_id=%ld "
                  "rollback_list=%ld commit_list=%ld",
                  start_trans_id_, cur_trans_id_, rollback_list_.size(), commit_list_.size());
        ret = OB_NOT_INIT;
      }
          ...
          while (0 < rollback_counter_--)
          {
            ...
            if (iter != rollback_list_.end())
            {
              if (NULL != iter->rollbacker)
              {
                ...
                {
                  YYSYS_LOG(WARN, "rollback fail ret=%d", tmp_ret);
                  ret = tmp_ret;
                }
              }
    ...
    int TransNode::add_rollback_data(ITableEngine *rollbacker, void *data)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "push rollback info to list rollback list fail");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int TransNode::add_commit_data(ITableEngine *commiter, void *data)
    {
      ...
      if (!mutation_started_)
      {
        YYSYS_LOG(WARN, "mutation not start, start_trans_id=%ld cur_trans_id=%ld rollback_list=%ld commit_list=%ld",
                  start_trans_id_, cur_trans_id_, rollback_list_.size(), commit_list_.size());
        ret = OB_NOT_INIT;
      }
      else
    ...
    int TransNode::commit()
    {
      ...
      for (iter = commit_list_.begin(); iter != commit_list_.end(); iter++)
      {
        if (NULL != iter->commiter)
        {
          ...
          if (OB_SUCCESS != tmp_ret)
          {
            YYSYS_LOG(WARN, "commit fail ret=%d", tmp_ret);
            ret = tmp_ret;
          }
        }
      }
    ...
    int TransMgr::init(const int64_t max_trans_num)
    {
      ...
      {
        YYSYS_LOG(WARN, "init trans node map fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = trans_node_list_.init(max_trans_num)))
      {
        YYSYS_LOG(WARN, "init trans node list fail ret=%d", ret);
      }
      else
      {
        flying_trans_counter_ = 0;
    ...
    void TransMgr::destroy()
    {
      ...
      {
        YYSYS_LOG(ERROR, "some transaction have not end, counter=%ld",
    ...
    int TransMgr::start_transaction(const TETransType trans_type, uint64_t &trans_descriptor, const int64_t trans_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param trans_id=%ld commited_trans_id=%ld", trans_id, commited_trans_id_);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != trans_node_list_.pop(trans_node)
              || NULL == trans_node)
      {
        YYSYS_LOG(WARN, "to many flying transaction counter=%ld", flying_trans_counter_);
        ...
        while (OB_SUCCESS == ret)
        {
          switch (trans_type) 
          {
            case READ_TRANSACTION:
              ...
              YYSYS_LOG(DEBUG, "read transaction this=%p id=%ld", this, trans_node->get_trans_id());
              break;
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "assign from trans node map fail ret=%d", ret);
            }
          }
          if (OB_SUCCESS == ret)
          {
    ...
    int TransMgr::end_transaction(const uint64_t trans_descriptor, const bool rollback)
    {
      ...
      if (!inited_)
      {
        ret = OB_NOT_INIT;
              ...
              || NULL == trans_node)
      {
        YYSYS_LOG(WARN, "invalid trans descriptor, ret=%d trans_descriptor=%lu", ret, trans_descriptor);
        ret = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(WARN, "mutation is running or trans id not match, cannot end transaction, "
    ...
    {
      if (calc_timestamp_ + CALC_INTERVAL < yysys::CTimeUtil::getTime()
    ...
    {
      int64_t ret = (-1 == trans_id) ? yysys::CTimeUtil::getTime() : trans_id;
      if (ret <= commited_trans_id)
      {
        ret = commited_trans_id + 1;
        if (-1 == trans_id)
        {
          YYSYS_LOG(INFO, "time do not increase, will add 1 as next trans_id=%ld", ret);
        }
      }
      return ret;
    }
