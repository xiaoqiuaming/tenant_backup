...
int UpsMon::parse_cmd_line(const int argc, char *const argv[])
{
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "arguments parse succ, ups=%s:%d, timeout=%ldms, log_level=%s", host, port, ping_timeout_us_, log_level);
  }
...
int UpsMon::set_sync_limit()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "failed to initialize client, ret=%d", ret);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "encode_vi64 error, new_limit_rate_=%ld", new_sync_limit_);
    }
    else
    {
  ...
  int64_t timeu = yysys::CTimeUtil::getTime() - start_time;
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "set_new_limit ups(%s) succ, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
  else
  {
    YYSYS_LOG(ERROR, "set_new_limit ups(%s) failed, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
...
int UpsMon::set_new_vip()
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "encode_i32 error, new_vip_=%d", new_vip_);
    }
    else
    {
      ObClientManager& client_mgr = client.get_client_mgr();
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "set_new_vip ups(%s) succ, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
  else
  {
    YYSYS_LOG(ERROR, "set_new_vip ups(%s) failed, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
...
int UpsMon::ping_ups()
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "ping ups(%s) succ, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
  else
  {
    YYSYS_LOG(ERROR, "ping ups(%s) failed, timeu=%ldus", to_cstring(ups_server_), timeu);
  }
