...
int ObUpsLogMgr::load_log_from_disk(bool &is_append)
{
    ...
    {
        //some import comment
        start_id = max(replay_log_cache_.get_end_id(), replay_cursor.log_id_);
        if(last_flushed_id_ > 0 && last_flushed_id_ > start_id)
        {
            end_id = min(master_max_commit_log_id_, last_flushed_id_);
        }
        else
        {
        ...
        {
            YYSYS_LOG(INFO,"in async load thread, [start_id,end_id] = [%ld,%ld] cache_log_id=%ld, replay_log_id=%ld %s",
                      start_id,end_id,replay_log_cache_.get_end_id(),replay_cursor.log_id_,to_cstring(replay_log_cache_));
        }
        if(start_id >= end_id)
        {
            err = OB_NEED_WAIT;
        ...
        {
            YYSYS_LOG(ERROR,"reader.get_log(log_id=%ld)=>%d",start_location_.log_id_,err);
        }
