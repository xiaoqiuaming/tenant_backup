...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObUpsMutator :: set_freeze_memtable()
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid status, flag_=%d", flag_);
        err = OB_ERROR;
      }
      else
      {
    ...
    //add zhaoqiong [truncate table]:20190827
    ...
    int ObUpsMutator :: serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(WARN, "serialize_header(buf=%p[%ld], pos=%ld)=>%d", buf, buf_len, pos, err);
      }
      else if (OB_SUCCESS != (err = mutator_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(WARN, "failed to serialize mutator, err=%d", err);
      }
    ...
    int ObUpsMutator :: deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize_header(buf=%p[%ld], pos=%ld)=>%d", buf, buf_len, pos, err);
      }
      else if (OB_SUCCESS != (err = mutator_.deserialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(WARN, "failed to deserialize mutator, err=%d", err);
      }
    ...
    int PartCommitInfo:: serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      int err = OB_SUCCESS;
      ...
      else if (OB_SUCCESS != (err = coordinator_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(WARN, "failed to serialize mutator, err=%d", err);
      }
      else if ((pos + 3 * (int64_t) sizeof(int64_t)) >= buf_len)
      {
        YYSYS_LOG(WARN, "buf is not enough, pos=%ld, buf_len=%ld", pos, buf_len);
        err = OB_ERROR;
      }
    ...
    int PartCommitInfo:: deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      int err = OB_SUCCESS;
      ...
      else if (OB_SUCCESS != (err = coordinator_.deserialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(WARN, "failed to deserialize mutator, err=%d", err);
      }
      else
    ...
    int64_t PartCommitInfo:: get_serialize_size(void) const
    {
      return coordinator_.get_serialize_size() + 3 * sizeof(int64_t);
    }
    //add 20150818:e
  }
}
