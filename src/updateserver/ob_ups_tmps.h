  ...
  namespace updateserver
  {
    ...
    bool merge(const TEKey &te_key, TEValue &te_value, Allocator &allocator, int64_t stop_trans_id = INT64_MAX)
    {
      ...
      if (NULL == column_map)
      {
        ...
        if (NULL == column_map)
        {
          YYSYS_LOG(ERROR, "get tsi column map fail");
          bret = false;
        }
        else
        {
            ...
            && te_value.create_time < stop_trans_id)
      {
        ...
        if (NULL == iter)
        {
          YYSYS_LOG(ERROR, "te_value list iter null pointer");
          break;
        }
        ...
        if (common::ObExtendType == iter->cell_info.value_.get_type())
        {
          ...
          {
            YYSYS_LOG(ERROR, "invalid extend type cellinfo %s %s",
        ...
        else
        {
          ...
          else if (common::hash::HASH_EXIST == hash_ret)
          {
            if (NULL != cur_node)
            {
              ...
              {
                YYSYS_LOG(ERROR, "apply cellinfo fail %s %s %s",
      ...
      if (bret)
      {
        ...
        if (NULL != new_value.list_head)
        {
          ...
          YYSYS_TRACE_LOG("merge te_value succ [%s] [%s] ==> [%s] cell_info_cnt=%hd cell_info_size=%hdkb timeu=%ld",
      ...
      {
        YYSYS_LOG(ERROR, "merge te_value fail %s", te_value.log_str());
      }
