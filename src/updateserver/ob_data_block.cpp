...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "blocks[0--pos[%ld]--size[%ld]]}", end_pos_, block_size_);
      return err;
    }
    ...
    int ObDataBlock::read(const int64_t pos, char* buf, const int64_t len, int64_t& read_count) const
    {
      ...
      else if (0 > (read_count = min(len, end_pos_ - pos)))
      {
        ...
        YYSYS_LOG(DEBUG, "read(pos=%ld, len=%ld, end_pos=%ld, block_size=%ld): unexpected error.",
    ...
    int ObDataBlock::append(const char* buf, const int64_t len)
    {
      ...
      else if (get_end_pos() + len + BLOCK_RESERVED_BYTES > get_block_size())
      {
        ...
        YYSYS_LOG(ERROR, "append(end_pos=%ld, len=%ld, block_size=%ld): unexpected error.",
