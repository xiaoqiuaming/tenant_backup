...
namespace oceanbase
{
  ...
  namespace updateserver
  {
    ...
    int MultiFileUtils::open(const ObString &fname, const bool dio, const bool is_create, const bool is_trunc, const int64_t align_size)
    {
      ...
      if (NULL == fname.ptr())
      {
        YYSYS_LOG(WARN, "fname null pointer");
      ...
      else
      {
        ...
        while (NULL != iter)
        {
          ...
          if (NULL == file)
          {
            YYSYS_LOG(WARN, "allocate file appender fail [%s]", iter);
            ret = OB_ERROR;
            break;
          }
          else if (OB_SUCCESS != (ret = file->open(fname, dio, is_create, is_trunc, align_size)))
          {
            YYSYS_LOG(WARN, "open [%s] fail ret=%d", iter, ret);
            break;
          }
          else if (OB_SUCCESS != (ret = flist_.push_back(file)))
          {
            YYSYS_LOG(WARN, "push fileinfo to list fail [%s] ret=%d", iter, ret);
            file->close();
            break;
          }
          YYSYS_LOG(INFO, "open [%s] pos=[%ld] succ", iter, flist_.size() - 1);
          while ('\0' != *iter)
          {
            iter++;
          }
    ...
    int64_t MultiFileUtils::get_file_pos() const
    {
      ...
      if (0 == flist_.size())
      {
        YYSYS_LOG(WARN, "no file open");
      }
      else if (flist_.end() == (iter = flist_.begin()))
      {
        YYSYS_LOG(WARN, "flist begin equal end");
      }
      else if (NULL == (file = *iter))
      {
        YYSYS_LOG(WARN, "invalid file pos=[0]");
      }
      else
      {
    ...
    int MultiFileUtils::append(const void *buf, const int64_t count, bool is_fsync)
    {
      ...
      else
      {
        ...
        for (iter = flist_.begin(); iter != flist_.end(); iter++, pos++)
        {
          ...
          if (NULL == file)
          {
            YYSYS_LOG(WARN, "invalid file pos=[%ld] buf=%p count=%ld", pos, buf, count);
            ret = OB_ERROR;
            break;
          }
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "append file fail pos=[%ld] ret=%d buf=%p count=%ld", pos, ret, buf, count);
            break;
          }
        }
      }
    ...
    int MultiFileUtils::fsync()
    {
      ...
      else
      {
        ...
        for (iter = flist_.begin(); iter != flist_.end(); iter++, pos++)
        {
          ...
          if (NULL == file)
          {
            YYSYS_LOG(WARN, "invalid file pos=[%ld]", pos);
            ret = OB_ERROR;
            break;
          }
          ret = file->fsync();
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fsync fail pos=[%ld] ret=%d", pos, ret);
            break;
          }
        }
      }
