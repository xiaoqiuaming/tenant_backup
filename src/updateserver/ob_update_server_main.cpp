...
//      }
      ...
      //ups_config  cluster_id  paxos_id
	  //add pangtianze [Paxos Cluster.Balance] 20170715:b
      ...
      {
        YYSYS_LOG(INFO,"cmd_cluster_id_ %d",cmd_cluster_id_);
        ups_config_.cluster_id = cmd_cluster_id_;
      }
      if (cmd_paxos_id_ > 0)
      {
        ups_config_.paxos_id = cmd_paxos_id_;
      }
      //add 20150427:e
      //add peiouya [MultiUPS] [UPS_Manage_Function] 20150521:b
      if (cmd_start_major_version_ > static_cast<int64_t>(SSTableID::START_MAJOR_VERSION))
      {
        ups_config_.start_major_version = cmd_start_major_version_;
      }
      //add 20150521:e
      ...
      {
        YYSYS_LOG(WARN, "failed to load from conf, ret: [%d]", ret);
      }

      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "Start Update server failed, ret: [%d]", ret);
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "shadow_server port is %s",
    ...
    void ObUpdateServerMain::do_signal(const int sig)
    {
      ...
      case SIGINT:
        ...
        YYSYS_LOG(INFO, "KILLED by signal, sig=%d", sig);
        server_.req_stop();
        break;
