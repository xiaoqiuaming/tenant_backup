...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int BtreeEngine::init(MemTank *allocer)
    {
      ...
      {
        YYSYS_LOG(WARN, "allocer null pointer");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int BtreeEngine::set(const TEKey &key, const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "alloc value memory fail");
        ret = OB_ERROR;
      }
      else
      {
        ...
        *pvalue = value;
        ...
        {
          YYSYS_LOG(WARN, "put to keybtree fail btree_ret=%d %s %s", btree_ret, key.log_str(), value.log_str());
                        ...
                        const TEValue &value)
    {
      ...
      {
        YYSYS_LOG(WARN, "cur write handle null pointer");
        ret = OB_ERROR;
      }
      else if (NULL == (pvalue = (TEValue*)allocer_->indep_alloc(sizeof(value))))
      {
    ...
    int BtreeEngine::get(const TEKey &key, TEValue &value)
    {
              ...
              || NULL == pvalue)
      {
        ...
        {
          YYSYS_LOG(WARN, "get from keybtree fail btree_ret=%d pvalue=%p %s", btree_ret, pvalue, key.log_str());
        }
      ...
      else
      {
        ...
        switch (handle.trans_type_)
        {
          ...
          case WRITE_TRANSACTION:
            ...
            break;
          default:
            YYSYS_LOG(WARN, "invalid trans_type=%d", handle.trans_type_);
            ret = OB_ERROR;
            break;
        }
      }
      ...
      else
      {
        ...
        switch (type)
        {
          case READ_TRANSACTION:
            ...
            {
              YYSYS_LOG(WARN, "btree get_read_handle fail ret=%d", btree_ret);
              ret = OB_ERROR;
            }
            else
            {
          ...
          case WRITE_TRANSACTION:
            ...
            {
              YYSYS_LOG(WARN, "btree get_write_handle fail ret=%d", btree_ret);
              ret = OB_ERROR;
            }
            else
            {
    ...
    int BtreeEngine::start_mutation(BtreeEngineTransHandle &handle)
    {
      ...
      {
        YYSYS_LOG(WARN, "get write handle fail");
        ret = OB_ERROR;
        handle.cur_write_handle_ptr_ = &handle.write_handle_;
      }
      else
    ...
    int BtreeEngineIterator::next()
    {
      ...
      {
        //YYSYS_LOG(WARN, "invalid internal pointer keybtree=%p read_handle_ptr=%p", keybtree_, read_handle_ptr_);
        ret = OB_ITER_END;
      }
