...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    void StoreMgr::lookup_dir_(const char *root, Func &func)
    {
      ...
      {
        YYSYS_LOG(WARN, "open rootdir [%s] fail errno=%u", root, errno);
      }
      else
      {
              ...
              && NULL != dir_iter->d_name)
        {
          ...
          {
            YYSYS_LOG(INFO, "skip [%s/%s]", root, dir_iter->d_name);
          }
          else if (!func.match(dir_iter->d_name))
          {
            YYSYS_LOG(INFO, "dname [%s/%s] do not match regex", root, dir_iter->d_name);
          }
          else if (MAX_DIR_NAME_LENGTH <= snprintf(tmp_buffer, MAX_DIR_NAME_LENGTH, "%s/%s", root, dir_iter->d_name))
          {
            YYSYS_LOG(WARN, "build entire path [%s/%s] fail", root, dir_iter->d_name);
          }
          else if (!is_access(tmp_buffer)
                  || !is_dir(tmp_buffer))
          {
            YYSYS_LOG(INFO, "dname [%s/%s] cannot access or not dir", root, tmp_buffer);
          }
          else
          {
    ...
    bool StoreMgr::build_dir(const char *path, const char *dir)
    {
          ...
          && MAX_DIR_NAME_LENGTH > snprintf(dpath, MAX_DIR_NAME_LENGTH, "%s/%s", path, dir))
      {
        if (0 != mkdir(dpath, TRASH_DIR_MODE)
            && EEXIST != errno)
        {
          YYSYS_LOG(WARN, "mkdir fail dpath=[%s] errno=%u", dpath, errno);
        }
        else
        {
          bret = true;
