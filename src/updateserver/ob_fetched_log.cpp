...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObFetchLogReq::serialize(char* buf, int64_t len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "session_.serialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, tmp_pos, err);
      }
      else if(OB_SUCCESS != (err = serialization::encode_i64(buf, len, tmp_pos, start_id_)))
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, start_id_, err);
      }
      else if(OB_SUCCESS != (err = serialization::encode_i64(buf, len, tmp_pos, max_data_len_)))
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, max_data_len_, err);
      }
      else
      {
        pos = tmp_pos;
    ...
    int ObFetchLogReq::deserialize(const char* buf, int64_t len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "session_.deserialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, tmp_pos, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "decode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, max_data_len_, err);
      }
      else
      {
        pos = tmp_pos;
    ...
    int ObFetchedLog::set_buf(char* buf, int64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "set_buf(buf=%p[%ld]): INVALID ARGUMENT", buf, len);
      }
      else
      {
        log_data_ = buf;
    ...
    int ObFetchedLog::serialize(char* buf, int64_t len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "next_req.serialize(buf=%p, len=%ld, pos=%ld)=>%d", buf, len, tmp_pos, err);
      }
      else if(OB_SUCCESS != (err = serialization::encode_i64(buf, len, tmp_pos, start_id_)))
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, start_id_, err);
      ...
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, end_id_, err);
      }
      else if(OB_SUCCESS != (err = mem_chunk_serialize(buf, len, tmp_pos, log_data_, data_len_)))
      {
        YYSYS_LOG(ERROR, "mem_chunk_serialize(buf=%p[%ld], pos=%ld, data_len=%ld)=>%d", buf, len, tmp_pos, data_len_, err);
      }
      else
      {
        pos = tmp_pos;
    ...
    int ObFetchedLog::deserialize(const char* buf, int64_t len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "next_req.deserialize(buf=%p, len=%ld, pos=%ld)=>%d", buf, len, tmp_pos, err);
      }
      else if(OB_SUCCESS != (err = serialization::decode_i64(buf, len, tmp_pos, &start_id_)))
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, start_id_, err);
      ...
      {
        YYSYS_LOG(ERROR, "mem_chunk_deserialize(buf=%p[%ld], pos=%ld, data_len=%ld, max_data_len=%ld)=>%d",
