...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    {
      YYSYS_LOG(INFO, "construct SchemaMgrImp %p", this);
      if (0 != name_map_.create(SCHEMA_HASH_SIZE))
      {
        YYSYS_LOG(ERROR, "create schema map fail");
      }
    ...
    {
      YYSYS_LOG(INFO, "deconstruct SchemaMgrImp %p", this);
      clear_();
    }
    ...
    void SchemaMgrImp::analyse_column_(const ObSchema &schema, SchemaInfo &schema_info)
    {
          ...
          column_schema_iter++)
      {
        ...
        if (ObCreateTimeType == column_schema_iter->get_type())
        {
          ...
          {
            YYSYS_LOG(WARN, "there is already a create_time type column id=%lu, cur=%ld",
        ...
        else if (ObModifyTimeType == column_schema_iter->get_type())
        {
          ...
          {
            YYSYS_LOG(WARN, "there is already a modify_time type column id=%lu, cur=%ld",
    ...
    int SchemaMgrImp::set_schema_(const ObSchema &schema)
    {
      ...
      {
        YYSYS_LOG(WARN, "ob_malloc schema memory fail");
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "placement new SchemaInfo fail memory=%p", memory);
          ret = OB_ERROR;
        }
        else
        {
          ...
          *pschema = schema;
          ...
          {
            YYSYS_LOG(WARN, "update name map fail hash_ret=%d", hash_ret);
            ret = OB_ERROR;
          }
          else
          {
            ...
            if (HASH_INSERT_SUCC != hash_ret)
            {
              YYSYS_LOG(WARN, "update id map fail hash_ret=%d", hash_ret);
    ...
    void SchemaMgrImp::clear_()
    {
      if (0 != ref_cnt_)
      {
        YYSYS_LOG(WARN, "ref_cnt=%ld do not equal to 0", ref_cnt_);
      }
      else
      {
    ...
    int SchemaMgrImp::build_sstable_schema(sstable::ObSSTableSchema &sstable_schema) const
    {
      ...
      for (iter = id_map_.begin(); iter != id_map_.end(); iter++)
      {
        ...
        if (NULL == schema_info)
        {
          YYSYS_LOG(ERROR, "invalid schema_info table_id=%lu", iter->first);
          ret = OB_ERROR;
          break;
        }
        else
        {
          ...
          for (; column_schema != schema_info->schema.column_end(); column_schema++)
          {
            ...
            {
              YYSYS_LOG(ERROR, "invalid column_schema table_id=%lu", iter->first);
              ret = OB_ERROR;
              break;
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "add_column_def fail ret=%d group_id=%lu column_id=%lu value_type=%d table_id=%lu",
    ...
    void SchemaMgrImp::dump2text() const
    {
      ...
      snprintf(buffer, BUFFER_SIZE, "/tmp/ups_schemas.pid_%d.tim_%ld", getpid(), yysys::CTimeUtil::getTime());
    ...
    SchemaMgr::SchemaMgr() : cur_schema_mgr_imp_(NULL), has_schema_(false)
    {
      ...
      {
        YYSYS_LOG(ERROR, "new schema mgr imp fail");
      }
    }
    ...
    int SchemaMgr::set_schemas(const ObSchemaManagerWrapper &schema_manager)
    {
      ...
      {
        YYSYS_LOG(WARN, "new tmp schema mgr imp fail");
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = tmp_schema_mgr_imp->set_schemas(schema_manager)))
      {
        YYSYS_LOG(WARN, "set schemas to tmp schema mgr imp fail ret=%d", ret);
    ...
    uint64_t SchemaMgr::get_create_time_column_id(const uint64_t table_id) const
    {
      ...
      if (NULL == cur_schema_mgr_imp_)
      {
        YYSYS_LOG(WARN, "schema mgr imp null pointer");
      }
      else
      {
    ...
    void SchemaMgr::revert_schema_handle(SchemaHandle &schema_handle)
    {
      ...
      if (NULL != schema_handle)
      {
        ...
        if (0 > ref_cnt)
        {
          YYSYS_LOG(WARN, "schema_mgr=%p have a unexpected ref_cnt=%d", schema_handle, ref_cnt);
        }
        if (0 == ref_cnt
            && schema_handle != cur_schema_mgr_imp_)
        {
                                        ...
                                        sstable::ObSSTableSchema &sstable_schema) const
    {
      ...
      if (NULL == schema_handle)
      {
        YYSYS_LOG(WARN, "invalid schema_handle=%p", schema_handle);
        ret = OB_ERROR;
      }
      else
      {
