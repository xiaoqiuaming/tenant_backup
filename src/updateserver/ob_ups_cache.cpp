  ...
  namespace updateserver
  {
    ...
    int ObUpsCache::init(const int64_t cache_size)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"init kv_cache_ failed");
          ret = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO,"cache_size=%ldB",cache_size);
          inited_ = true;
        }
      }
      return ret;
    ...
    int ObUpsCache::destroy()
    {
      ...
      if(inited_)
      {
        ...
        {
          YYSYS_LOG(ERROR,"destroy cache fail");
          ret = OB_ERROR;
        }
        else
        {
      ...
      {
        YYSYS_LOG(WARN,"have not inited, no need to destroy");
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int ObUpsCache::clear()
    {
      ...
      {
        YYSYS_LOG(WARN,"clear cache fail");
        ret = OB_ERROR;
      }
      else
      {
        ...
        const uint64_t column_id, ObUpsCacheValue& value)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"invalid param table_id=%lu,column_id=%lu,row_key_ptr=%p,row_key_length=%ld",
        ...
        else
        {
          ...
          if(0 == column_id)
          {
            YYSYS_LOG(WARN,"use func is_row_exist to check if the row exist");
            ret = OB_ERROR;
          }
          else  //在缓存中查询列的值
          {
            ret = kv_cache_.get(date_key,value,buffer_handle.handle_);
            YYSYS_LOG(DEBUG, "get from kv cache, table_id=%lu column_id=%lu, row_key=%s ret=%d version=%ld value=[%s]",
                table_id, column_id, to_cstring(row_key), ret, value.version, print_obj(value.value));
            if (OB_ENTRY_NOT_EXIST != ret && OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN,"get kv_cache_ failed,ret=%d",ret);
            }
            else if(OB_SUCCESS == ret)
            {
              ...
              {
                  uint64_t *dec_value = NULL;
                  if(OB_SUCCESS != (ret = value.value.get_ttint(dec_value)))
                  {
                      YYSYS_LOG(ERROR,"get decimal pointer from value.value error, ret=%d",ret);
                  }
                  else
                  {
                      buffer_handle.buffer_ = reinterpret_cast<char *>(dec_value);
                  }
              }
        ...
        const ObUpsCacheValue& value)
    {
      ...
      else
      {
        if(OB_INVALID_ID == table_id || OB_INVALID_ID == column_id
            || NULL == row_key.ptr() || row_key.length() <= 0)
        {
          YYSYS_LOG(ERROR,"invalid param table_id=%lu,column_id=%lu,row_key_ptr=%p,row_key_length=%ld",table_id,column_id,row_key.ptr(),row_key.length());
          ret = OB_ERROR;
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(DEBUG, "put to kv cache, table_id=%lu column_id=%lu row_key=%s ret=%d",
            table_id, column_id, to_cstring(row_key), ret);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to add cache pair to kv cache, ret=%d", ret);
        }
      }
        ...
        ObBufferHandle& buffer_handle)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"invalid param table_id=%lu,row_ke=%s",table_id, to_cstring(row_key));
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObUpsCache::set_row_exist(const uint64_t table_id, const ObRowkey& row_key, const bool is_exist)
    {
      ...
      else
      {
        if(OB_INVALID_ID == table_id
            ...
            || row_key.length() <=0)
        {
          YYSYS_LOG(WARN,"invalid param,table_id=%lu,row_key_ptr=%p,row_key_length=%ld",
        ...
        else
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN,"set_row_exist failed,ret=%d",ret);
          }
        }
      }
      return ret;
    ...
    void ObUpsCache::set_cache_size(const int64_t cache_size)
    {
      ...
      {
        YYSYS_LOG(INFO,"set to min cache size, min_cache_size=%ldB",MIN_KVCACHE_SIZE);
