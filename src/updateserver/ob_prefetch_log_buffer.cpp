...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObPrefetchLogBuffer::reset()
    {
      ...
      {
        YYSYS_LOG(ERROR, "ObLogBuffer::reset()=>%d", err);
      }
      else
      {
        read_pos_ = end_pos_;
    ...
    int ObPrefetchLogBuffer::get_log(const int64_t align_bits, const int64_t start_id, int64_t& end_id, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      else if (OB_SUCCESS != (err = ObLogBuffer::get_log(align_bits, read_pos_, real_pos, real_start_id, end_id, buf, len, read_count)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "log_buf_.get_log(pos, buf, len)=>%d", err);
        }
      }
      else if (start_id != real_start_id)
      {
        err = OB_DISCONTINUOUS_LOG;
        YYSYS_LOG(WARN, "start_id[%ld] != real_start_id[%ld]: need reset prefetch_buffer", start_id, real_start_id);
      }
      else
      {
        read_pos_ = real_pos + read_count;
      }
      YYSYS_LOG(DEBUG, "get_log(read_pos_=%ld, start_id=%ld, end_id=%ld)=>%d", read_pos_, start_id, end_id, err);
      return err;
    }
                                        ...
                                        const char* buf, const int64_t len)
    {
      int err = OB_SUCCESS;
      SpinWLockGuard lock_guard(write_lock_);
      YYSYS_LOG(DEBUG, "append_log(log=[%ld,%ld], end_id=%ld)", start_id, end_id, get_end_id());
      if (!is_inited())
      {
        err = OB_NOT_INIT;
      }
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "append(buf=%p[%ld], log=[%ld,%ld]): invalid argument", buf, len, start_id, end_id);
      }
      // 检查是否会覆盖未读的日志
      else if (get_remain() < (1<<block_size_shift_))
      {
        err = OB_EAGAIN;
        YYSYS_LOG(DEBUG, "append_log(log=[%ld,%ld], end_id=%ld):WAIT_FOR_READER", start_id, end_id, get_end_id());
      }
      else if (OB_SUCCESS != (err = ObLogBuffer::append_log(start_id, end_id, buf, len))
               && OB_DISCONTINUOUS_LOG != err)
      {
        YYSYS_LOG(ERROR, "append_log(id=[%ld, %ld], buf=%p[%ld])=>%d", start_id, end_id, buf, len, err);
      }
      else if (OB_DISCONTINUOUS_LOG == err)
      {
        YYSYS_LOG(WARN, "append_log(log=[%ld,%ld], end_id=%ld):LOG_DISCONTINUOUS", start_id, end_id, get_end_id());
      }
      return err;
    }
