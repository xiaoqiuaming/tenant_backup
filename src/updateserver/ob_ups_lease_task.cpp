...
int ObUpsLeaseTask::init(common::ObRoleMgr *role_mgr,
                         ObUpsRpcStub * rpc_stub,
                         common::ObServer &root_server,
                         ObUpsLogMgr *log_mgr,
                         const int64_t inner_port,
                         common::ObServer &self_server,
                         const int64_t timeout_us)
{
  ...
  if (NULL == role_mgr || NULL == rpc_stub || NULL == log_mgr)
  {
    ...
    YYSYS_LOG(WARN, "invalid argument. role_mgr=%p", role_mgr);
  }
  else
  {
    role_mgr_ = role_mgr;
...
void ObUpsLeaseTask::update_lease(int64_t lease_time)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid lease. lease_time=%ld, cur_time=%ld", lease_time, cur_time);
  }
  else
  {
    self_lease_ = lease_time;
...
void ObUpsLeaseTask::runTimerTask(void)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to send keep_alive_msg to rootserver, err=%d", err);
  }
  else
  {
    YYSYS_LOG(DEBUG, "ups send keep alive to rootserver. time=%ld", cur_time_us);
  }

  //check lease
  if (0 != self_lease_ && self_lease_ < cur_time_us)
  {
    YYSYS_LOG(ERROR, "lease timeout, should reregister to rootserver, lease_time:%ld, cur_time:%ld", self_lease_, cur_time_us);
  ...
  {
    YYSYS_LOG(WARN,
              "lease will be timeout, retry to send renew lease to rootserver. lease_time:%ld, cur_time:%ld, remain lease[%ld] should big than %ld",
              self_lease_,
              cur_time_us,
              self_lease_ - cur_time_us,
              lease_renew_time_);
