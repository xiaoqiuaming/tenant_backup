...
namespace oceanbase
{
  namespace updateserver
  {
                                  ...
                                  const int64_t start_id, int64_t& end_id, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "rpc_stub->fetch_log(master=%s, start_id=%ld, timeout=%ld)=>%d",
      ...
      else if (OB_SUCCESS == err)
      {
        ...
        YYSYS_LOG(DEBUG, "rpc_stub->fetch_log(end_id=%ld, read_count=%ld)", end_id, read_count);
      }
      else if (OB_NEED_RETRY != err)
      {
        YYSYS_LOG(WARN, "rpc_stub->fetch_log(end_id=%ld)=>%d", end_id, err);
      }
      else
      {
        YYSYS_LOG(TRACE, "rpc_stub->fetch_log(): need retry");
      }
      return err;
    }
    ...
    int make_log_id_uniq(int64_t file_id, char* buf, int64_t len, int64_t& start_id, int64_t& end_id, bool& is_file_end)
    {
      ...
      while(OB_SUCCESS == err && pos < len && !is_file_end)
      {
        ...
        else if (old_pos != pos)
        {
          ...
          YYSYS_LOG(ERROR, "old_pos[%ld] != pos[%ld]", old_pos, pos);
        }
        if (OB_SUCCESS == err)
        {
      ...
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "is_file_end=%s, pos[%ld] != len[%ld]", is_file_end? "true": "false" , pos, len);
      }
      return err;
    }
                                  ...
                                  ObLogCursor& start_cursor, char*& buf, int64_t& len)
    {
      ...
      if (NULL == rpc_stub)
      {
        err = OB_INVALID_ARGUMENT;
               ...
               && OB_RESPONSE_TIME_OUT != err && OB_PACKET_NOT_SENT != err && OB_NEED_RETRY != err)
      {
        YYSYS_LOG(ERROR, "slave_register()=>%d", err);
      }
      else if (OB_SUCCESS != err)
      {
        err = OB_NEED_RETRY;
      ...
      {
        YYSYS_LOG(WARN, "rpc_stub->fetch_lsync(master=%s, start_cursor=%s, timeout=%ld)=>%d",
                                                  ...
                                                  ObLogCursor& end_cursor, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      else if (0 >= start_cursor.log_id_)
      {
        err = OB_INVALID_ARGUMENT;
               ...
               && OB_NEED_RETRY != err)
      {
        YYSYS_LOG(ERROR, "get_from_lsync()=>%d", err);
      }
      else if (OB_NEED_RETRY == err)
      {}
      ...
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "get_log_from_lsync(log_data=%p, log_cursor.file_id=%ld, start_cursor.file_id=%ld): UNEXPECTED ERROR", log_data, log_cursor.file_id_, start_cursor.file_id_);
      }
      else if (len < read_count)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "len[%ld] < read_count[%ld]", len, read_count);
      }
      ...
      {
        YYSYS_LOG(INFO, "receive no data from lsync[file_id=%ld, log_id=%ld].", start_cursor.file_id_, start_cursor.log_id_);
      }
      else if (OB_SUCCESS != (err = make_log_id_uniq(start_cursor.file_id_, buf, read_count, start_id, end_cursor.log_id_, is_file_end)))
      {
        YYSYS_LOG(ERROR, "make_log_id_uniq()=>%d", err);
      }
      else if (start_id != start_cursor.log_id_)
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "fetched_start_id[%ld] != request_log_id[%ld]", start_id, start_cursor.log_id_);
      }
      else if (is_file_end)
      {
    ...
    int ObRemoteLogSrc::fill_start_cursor(ObLogCursor& start_cursor)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_server()=>%d", err);
      }
      ...
      else if (FETCH_SERVER == server_getter_->get_type())
      {
        if (OB_SUCCESS != (err = fill_start_cursor_with_fetch_server(server, start_cursor))
            && OB_NEED_RETRY != err)
        {
          YYSYS_LOG(ERROR, "get_log_from_fetch_server(start_cursor=%s)=>%d", start_cursor.to_str(), err);
        }
      }
      else
      {
        if (OB_SUCCESS != (err = fill_start_cursor_with_lsync_server(server, start_cursor))
            && OB_NEED_RETRY != err)
        {
          YYSYS_LOG(ERROR, "get_log_from_lsync_server(start_cursor=%s)=>%d", start_cursor.to_str(), err);
        }
      }
      return err;
    }
    ...
    int ObRemoteLogSrc::fill_start_cursor_with_fetch_server(const ObServer& server, ObLogCursor& start_cursor)
    {
      ...
      {
        YYSYS_LOG(ERROR, "rpc->fill_log_cursor()=>%d", err);
      }
    ...
    int ObRemoteLogSrc::fill_start_cursor_with_lsync_server(const ObServer& server, ObLogCursor& start_cursor)
    {
      ...
      if (!is_inited())
      {
        err = OB_NOT_INIT;
               ...
               && OB_NEED_RETRY != err)
      {
        YYSYS_LOG(ERROR, "fill_log_cursor_with_lsync()=>%d", err);
      }
      else if (OB_NEED_RETRY == err)
      {}
      else if (0 >= read_count)
      ...
      {
        YYSYS_LOG(INFO, "fill_start_cursor(file_id=%ld, log_id=%ld)", start_cursor.file_id_, log_entry.seq_);
        start_cursor.log_id_ = log_entry.seq_ +
          ((OB_LOG_SWITCH_LOG == log_entry.cmd_) ?start_cursor.file_id_ : start_cursor.file_id_ - 1);
      }
      YYSYS_LOG(DEBUG, "fill_start_cursor(start_cursor=%s)=>%d", start_cursor.to_str(), err);
      return err;
    }
    ...
    int ObRemoteLogSrc::get_log(const ObLogCursor& start_cursor, ObLogCursor& end_cursor, char* buf, const int64_t len, int64_t& read_count)
    {
      ...
      else if (FETCH_SERVER == server_getter_->get_type())
      {
        ...
        if (OB_SUCCESS != (err = get_log_from_fetch_server(rpc_stub_, server, fetch_timeout_, copy_req(req),
            ...
            && OB_NEED_RETRY != err)
        {
          YYSYS_LOG(WARN, "get_log_from_fetch_server(start_cursor=%s)=>%d", start_cursor.to_str(), err);
          req.session_.reset();
          update_req(req);
          err = OB_NEED_RETRY;
        }
      ...
      else
      {
        if (0 == start_cursor.file_id_)
        {
          ...
          YYSYS_LOG(INFO, "fetch_log_with_lsync(start_cursor.file_id == 0): Maybe log src reconfigured.");
        }
      ...
      YYSYS_LOG(DEBUG, "get_log(start_cursor=%s, end_id=%ld, read_count=%ld)=>%d", start_cursor.to_str(), end_cursor.log_id_, read_count, err);
      return err;
    }
