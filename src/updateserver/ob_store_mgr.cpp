...
namespace oceanbase
{
  ...
  namespace updateserver
  {
    ...
    int StoreMgr::init(const char *store_root, const char *raid_regex, const char *dir_regex)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(WARN, "this=%p already inited", this);
        ret = OB_INIT_TWICE;
      }
              ...
              || NULL == dir_regex)
      {
        YYSYS_LOG(WARN, "invalid param store_root=%p dir_regex=%p", store_root, dir_regex);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (MAX_DIR_NAME_LENGTH <= (tmp_ret = snprintf(store_root_str_, MAX_DIR_NAME_LENGTH, "%s", store_root)))
      {
        YYSYS_LOG(WARN, "copy fail store_root=[%s] ret=%d", store_root, tmp_ret);
        ret = OB_ERROR;
      }
      else if (MAX_DIR_NAME_LENGTH <= (tmp_ret = snprintf(raid_regex_str_, MAX_DIR_NAME_LENGTH, "%s", raid_regex)))
      {
        YYSYS_LOG(WARN, "copy fail raid_regex=[%s] ret=%d", raid_regex, tmp_ret);
        ret = OB_ERROR;
      }
      else if (MAX_DIR_NAME_LENGTH <= (tmp_ret = snprintf(dir_regex_str_, MAX_DIR_NAME_LENGTH, "%s", dir_regex)))
      {
        YYSYS_LOG(WARN, "copy fail dir_regex=[%s] ret=%d", dir_regex, tmp_ret);
        ret = OB_ERROR;
      }
      else if (!raid_regex_.init(raid_regex_str_, REG_EXTENDED))
      {
        YYSYS_LOG(WARN, "init raid_regex fail raid_regex=[%s]", raid_regex_str_);
        ret = OB_ERROR;
      }
      else if (!dir_regex_.init(dir_regex_str_, REG_EXTENDED))
      {
        YYSYS_LOG(WARN, "init dir_regex fail dir_regex=[%s]", dir_regex_str_);
        ret = OB_ERROR;
      }
      else if (0 != (tmp_ret = pthread_mutex_init(&mutex_, NULL)))
      {
        YYSYS_LOG(WARN, "init mutex fail errno=%u ret=%d", errno, tmp_ret);
        ret = OB_ERROR;
      }
      else if (0 != (tmp_ret = store_dev_.create(DEVICE_SET_SIZE)))
      {
        YYSYS_LOG(WARN, "create store_dev fail ret=%d", tmp_ret);
        ret = OB_ERROR;
      }
      else
      {
    ...
    bool StoreMgr::load()
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "this=%p have not inited", this);
      }
      else
      {
        ...
        for (iter = valid_raid_.begin(); iter != valid_raid_.end();)
        {
          ...
          if (0 == (*tmp_iter)->node_list.size())
          {
            ...
            YYSYS_LOG(INFO, "erase empty raid=[%s]", (*tmp_iter)->path);
          }
          else
          {
            ...
            {
              YYSYS_LOG(INFO, "load succ raid=[%s] store=[%s]", (*tmp_iter)->path, (*jter)->path);
            }
          }
        }
        YYSYS_LOG(INFO, "end load new store_dev_num=%ld", store_dev_.size());
    ...
    StoreMgr::StoreNode *StoreMgr::get_store_node_(const Handle store_handle) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid store_handle=%lu all_store_size=%d", store_handle, all_store_.size());
      }
      else if (store_handle != ret->store_handle)
      {
        YYSYS_LOG(WARN, "store_handle=%lu not match store_node=%p handle=%lu", store_handle, ret, ret->store_handle);
        ret = NULL;
      }
      return ret;
    }
    ...
    bool StoreMgr::filt_store_(const RaidNode &raid_node, ObList<Handle> &store_list)
    {
      ...
      for (iter = raid_node.node_list.begin(); iter != raid_node.node_list.end(); iter++)
      {
        ...
        {
          YYSYS_LOG(INFO, "invalid store_node=%p path=[%s] stat=%d", store_node, store_node->path, store_node->stat);
        }
        // 检查是文件系统否打开文件过多
        else if (yysys::CTimeUtil::getTime() - store_node->last_enfile_time < disk_enfile_interval_time_)
        {
          YYSYS_LOG(INFO, "too many file have been opened store_node=%p path=[%s] last_enfile_time=%ld "
                    "cur_time=%ld disk_enfile_interval_time=%ld",
                    store_node, store_node->path, store_node->last_enfile_time,
                    yysys::CTimeUtil::getTime(), disk_enfile_interval_time_);
        }
        // 检查剩余空间容量
        ...
        {
          YYSYS_LOG(ERROR, "free space not enough store_node=%p path=[%s] free=%ld total=%ld disk_reserved_ratio=%lf",
    ...
    int StoreMgr::assign_stores(ObList<Handle> &store_list)
    {
      ...
      else
      {
        ...
        for (iter = valid_raid_.begin(); iter != valid_raid_.end(); iter++)
        {
          ...
          {
            YYSYS_LOG(WARN, "raid_node=%p path=[%s] have not valid store", raid_node, raid_node->path);
          }
          else
          {
        ...
        {
          YYSYS_LOG(WARN, "not valid store to be assigned");
          ret = OB_ERROR;
        }
      }
      return ret;
    ...
    void StoreMgr::report_broken(const Handle store_handle)
    {
      ...
      else
      {
        ...
        if (NULL != store_node)
        {
          if (USING != store_node->stat)
          {
            YYSYS_LOG(INFO, "invalid stat=%d store_node=%p path=[%s]",
    ...
    void StoreMgr::report_broken(const char *path)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get from store_dev fail dev_no=%lu path=[%s]", dev_no, path);
        }
        else
        {
          StoreNode *store_node = get_store_node_(store_handle);
    ...
    void StoreMgr::report_enfile(const Handle store_handle)
    {
      ...
      else
      {
        ...
        if (NULL != store_node)
        {
          if (USING != store_node->stat)
          {
            YYSYS_LOG(WARN, "invalid stat=%d store_node=%p path=[%s]",
                      store_node->stat, store_node, store_node->path);
          }
          else
          {
            store_node->last_enfile_time = yysys::CTimeUtil::getTime();
          }
        }
      }
    }
    ...
    void StoreMgr::umount(const Handle store_handle)
    {
      ...
      else
      {
        ...
        if (NULL != store_node)
        {
          ...
          {
            YYSYS_LOG(ERROR, "invalid stat=%d can not umount, store_node=%p path=[%s]",
                      store_node->stat, store_node, store_node->path);
          }
          else
          {
            ...
            YYSYS_LOG(INFO, "umount store_node=%p store_handle=%lu path=[%s]", store_node, store_handle, path);
          }
        }
      }
      if (NULL != path)
      {
        ...
        if (MAX_DIR_NAME_LENGTH <= snprintf(broken_path, MAX_DIR_NAME_LENGTH,
                                            "%s.%010ld.broken", path, yysys::CTimeUtil::getTime()))
        {
          YYSYS_LOG(ERROR, "build broken path fail");
        }
        else if (0 != rename(path, broken_path))
        {
          YYSYS_LOG(INFO, "rename broken path=[%s] to [%s] fail errno=%u", path, broken_path, errno);
        }
        else
        {
          YYSYS_LOG(INFO, "rename broken path=[%s] to [%s] success", path, broken_path);
        }
      }
      if (0 == store_dev_.size())
      {
        YYSYS_LOG(ERROR, "not valid store");
      }
    }
    ...
    void StoreMgr::log_store_info()
    {
      ...
      else
      {
        ...
        for (store_iter = all_store_.begin(); store_iter != all_store_.end(); store_iter++)
        {
          ...
          if (NULL != store_node)
          {
            YYSYS_LOG(INFO, "all store report path=[%s] handle=%lu stat=%d dev_no=%lu",
    ...
    void StoreMgr::StoreNodeBuilder::handle(const char *path)
    {
      ...
      if (!is_symlink(path))
      {
        YYSYS_LOG(INFO, "store [%s] is not symlink", path);
      }
      else if (!build_dir(path, TRASH_DIR))
      {
        YYSYS_LOG(WARN, "build trash dir fail store=[%s]", path);
      }
      else if (!build_dir(path, BYPASS_DIR))
      {
        YYSYS_LOG(WARN, "build bypass dir fail store=[%s]", path);
      }
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "store [%s] dev=%lu raid=[%s] have other dir in the same device",
                    path, dev_no, raid_node_.path);
        }
        else if (NULL == (store_node = (StoreNode*)store_mgr_.alloc_(sizeof(StoreNode))))
        {
          YYSYS_LOG(WARN, "alloc store_node fail store=[%s] raid=[%s]", path, raid_node_.path);
        }
        else if (OB_SUCCESS != store_mgr_.all_store_.push_back(store_node))
        {
          YYSYS_LOG(WARN, "push store_node to all_store_vector fail store=[%s] raid=[%s]", path, raid_node_.path);
          store_mgr_.free_(store_node);
        }
        else if (0 != store_mgr_.new_store_.push_back(store_node))
        {
          YYSYS_LOG(WARN, "push store_node to new_store_list fail store=[%s] raid=[%s]", path, raid_node_.path);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "set handle=%lu to store_dev fail dev_no=%lu", store_handle, dev_no);
          }
          else
          {
            ...
            YYSYS_LOG(INFO, "load store succ [%s] handle=%lu dev_no=%lu", path, store_node->store_handle, dev_no);
          }
        }
      }
    }
    ...
    void StoreMgr::RaidNodeBuilder::handle(const char *path)
    {
      ...
      if (true)
      {
        ...
        if (NULL == (raid_node = store_mgr_.get_raid_node_(path)))
        {
          ...
          {
            YYSYS_LOG(WARN, "alloc raid_node fail raid=[%s]", path);
          }
          else if (OB_SUCCESS != store_mgr_.valid_raid_.push_back(raid_node))
          {
            YYSYS_LOG(WARN, "push raid_node to valid_raid_list fail raid=[%s]", path);
