...
namespace oceanbase
{
  ...
  namespace updateserver
  {
    ...
    class ObUpsCompactCellWriter : public ObCompactCellWriter
    {
      ...
      public:
        void set_row_deleted(const bool deleted)
        {is_row_deleted_ = deleted;};
        void set_tab_truncated(const bool truncated)
        {is_tab_truncated_ = truncated;}; /*add zhaoqiong [Truncate Table]:20160318*/
        bool is_row_deleted()
        {return is_row_deleted_;};
        bool is_tab_truncated()
        {return is_tab_truncated_;}; /*add zhaoqiong [Truncate Table]:20160318*/
        //mod zhaoqiong [Truncate Table]:20160318:b
        //void reset() {ObCompactCellWriter::reset(); is_row_deleted_ = false; ctime_recorder_.reset(); };
        void reset()
        {
            ObCompactCellWriter::reset();
            is_row_deleted_ = false;
            is_tab_truncated_ = false;
            ctime_recorder_.reset();
        };
        //mod:e
        ObCTimeRecorder& get_ctime_recorder()
        { return ctime_recorder_; }
