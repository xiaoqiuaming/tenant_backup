    ...
    ObCommonRpcStub *rpc_stub, int64_t log_sync_timeout)
{
  ...
  if (NULL == role_mgr || NULL == rpc_stub)
  {
    ...
    YYSYS_LOG(WARN, "invalid argument, role_mgr_=%p, rpc_stub=%p, log_sync_timeout=%ld", role_mgr, rpc_stub, log_sync_timeout);
  }
  else if (OB_SUCCESS != (err = ack_queue_.init(callback, rpc_stub->get_client_mgr(), DEFAULT_ACK_QUEUE_LEN)))
  {
    YYSYS_LOG(ERROR, "ack_queue.init(callback=%p, client_mgr=%p, queue_len=%ld)=>%d",
...
int ObUpsSlaveMgr::get_slaves(ObServer* slaves, int64_t limit, int64_t& slave_count)
{
  ...
  while (OB_SUCCESS == ret && p != NULL && p != &slave_head_.server_list_link)
  {
    ...
    {
      YYSYS_LOG(ERROR, "too many slaves[%ld], limit=%ld", slave_count, limit);
      break;
    }
...
int ObUpsSlaveMgr::post_log_to_slave(const ObLogCursor& start_cursor, const ObLogCursor& end_cursor, const char* data, const int64_t length)
{
  ...
  {
    YYSYS_LOG(ERROR, "get_slaves(limit=%ld)=>%d", MAX_SLAVE_NUM, ret);
  }
  ...
  {
    YYSYS_LOG(ERROR, "post_to_servers(server_num=%ld, log=[%s,%s])=>%d", slave_num, to_cstring(start_cursor), to_cstring(end_cursor), send_err);
  }
  return ret;
}
...
int ObUpsSlaveMgr::grant_keep_alive()
{
  ...
  {
    slave_node = (ServerNode*)(p);
    YYSYS_LOG(DEBUG, "send keep alive msg to slave[%s], time=%ld", slave_node->server.to_cstring(), cur_time_us);
    err = rpc_stub_->send_keep_alive(slave_node->server);
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to send keep alive msg to slave[%s], err = %d", slave_node->server.to_cstring(), err);
    }
    p = p->next();
  }
  return err;
...
int ObUpsSlaveMgr::get_quorum_seq(int64_t& seq)
{
    int ret = OB_SUCCESS;
    int slave_count = 0;
    int64_t slave_seq[MAX_SLAVE_NUM + 1];
    ...
    while (OB_SUCCESS == ret && p != NULL && p != &slave_head_.server_list_link)
    {
      ...
      {
        YYSYS_LOG(ERROR, "too many slaves[%d], limit=%ld", slave_count, MAX_SLAVE_NUM);
        break;
      }
      slave_node = (ServerNode*)(p);
    ...
    slave_seq[slave_count++] = my_log_seq_;
