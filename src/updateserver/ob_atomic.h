...
//add support for arm platform by wangd 202106:b
  ...
  static __inline__ uint64_t atomic_exchange(volatile uint64_t * pv, const uint64_t nv)
  {
    register unsigned long __res = 0;
    __res = __sync_val_compare_and_swap(pv, __res, nv);
    if (*pv != __res)
    {
      //__sync_lock_test_and_set(&__res, *pv);
      return __res;
    }
  ...
  static __inline__ uint16_t atomic_exchange(volatile uint16_t * pv, const uint16_t nv)
  {
    register unsigned short __res = 0;
    __res = __sync_val_compare_and_swap(pv, __res, nv);
    if (*pv != __res)
    {
      //__sync_lock_test_and_set(&__res, *pv);
...
#else // #if defined(__i386__) || defined(__x86_64__) || defined(__aarch64_)

#error "must compiled on i386 or x86_64 or aarch64"
//add support for arm platform by wangd 202106:e


//del support for arm platform by wangd 202106:b
/*    #else // #if defined(__i386__) || defined(__x86_64__)

  #error "must compiled on i386 or x86_64"
  */
//del support for arm platform by wangd 202106:e
