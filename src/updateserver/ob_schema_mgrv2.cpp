...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    CommonSchemaManagerWrapper::CommonSchemaManagerWrapper() : schema_mgr_buffer_(NULL), schema_mgr_impl_(NULL)
    {
      ...
      {
        YYSYS_LOG(WARN, "new schema mgr fail");
      }
    }
    ...
    //int CommonSchemaManagerWrapper::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(WARN, "determine_serialize_pos fail, ret = %d",ret);
      }
      //add:e
      else
      {
    ...
    bool CommonSchemaManagerWrapper::parse_from_file(const char* file_name, yysys::CConfig& config)
    {
      bool bret = false;
      if (NULL != schema_mgr_impl_)
      {
    ...
    int UpsSchemaMgr::set_schema_mgr(const CommonSchemaManagerWrapper &schema_manager)
    {
      ...
      if (NULL == schema_mgr)
      {
        YYSYS_LOG(WARN, "get schema mgr from schema_mgr_wrapper fail");
        ret = OB_ERROR;
      }
    ...
    int UpsSchemaMgr::apply_schema_mutator(const ObSchemaMutator &schema_mutator)
    {
      ...
      else if (schema_mutator.get_refresh_schema())
      {
        ...
        YYSYS_LOG(ERROR, "mutator contain refresh schema, need fetch full schema");
      }
      else
      {        
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(TRACE, "==========print schema version[%ld] start==========",tmp_schema_mgr_imp->get_schema_mgr().get_version());
        tmp_schema_mgr_imp->get_schema_mgr().print_debug_info();
        YYSYS_LOG(TRACE, "==========print schema version[%ld] end==========",tmp_schema_mgr_imp->get_schema_mgr().get_version());
      }
      return ret;
    }
    //add:e
    ...
    int UpsSchemaMgr::build_sstable_schema(const SchemaHandle schema_handle, sstable::ObSSTableSchema &sstable_schema, const CommonSchemaManager *sm) const
    {
      int ret = OB_SUCCESS;
      if (INVALID_SCHEMA_HANDLE == schema_handle)
      {
      ...
      else
      {
        ...
        if(NULL != sm)
        {
            ret = oceanbase::sstable::build_sstable_schema(*sm, sstable_schema);
        }
        else
        {
            const CommonSchemaManager &schema_mgr = schema_handle->get_schema_mgr();
