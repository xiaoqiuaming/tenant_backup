...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObCachedLogSrc::init(IObLogSrc* cache, IObLogSrc* backup)
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "init()=>%d", err);
      }
      else if (NULL == cache || NULL == backup)
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "init(cache=%p, backup=%p): invalid argument", cache, backup);
      }
      else
      {
        cache_ = cache;
