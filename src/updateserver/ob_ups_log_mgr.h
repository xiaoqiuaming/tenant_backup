  ...
  namespace updateserver
  {
    ...
    class ObUpsLogMgr : public common::ObLogWriter
    {
      ...
      struct ReplayLocalLogTask
      {
        ...
        ReplayLocalLogTask(): lock_(0), log_mgr_(NULL), is_replay_finished_(false)
        {}
        ...
        bool is_done() const
        {
          //mod wangdonghui [ups_replcation] 20170816 :b
          //return lock_ >= 2;
          return lock_ >= 2 && is_replay_finished_;
          //mod :e
        }
        virtual int do_work()
        {
          ...
          else if (OB_SUCCESS != (err = log_mgr_->replay_local_log()))
          {
            if (OB_CANCELED == err)
            {
              YYSYS_LOG(WARN, "cancel replay local log");
            }
            else
            {
              YYSYS_LOG(ERROR, "replay_local_log()=>%d, will kill self", err);
              kill(getpid(), SIGTERM);
            }
          }
          else
          {
            ...
            //add :e
            YYSYS_LOG(INFO, "replay local log succ.");
          }
          return err;
        }
        ...
        //add wangdonghui [ups_replication] 20170816 :b
        //sometime replaying local log failed, but lock_ still advance to 2
      ...
      struct ReplayTmpLogTask
      {
        ...
        ReplayTmpLogTask(): lock_(0), log_mgr_(NULL), is_replay_finished_(false)
        {}
        ...
        {
          lock_ = 0;
          is_replay_finished_ = false;//add [async load log]
        }
        bool is_started() const
        {
          //mod wangdonghui [ups_replcation] 20170816 :b
          ...
          //return lock_ >= 2 && is_replay_finished_;
          //mod :e
          return lock_ >= 1;  //mod [async load log]
        }
        bool is_done() const
        {
          //return lock_ >= 2;
        ...
        virtual int do_work()
        {
          ...
          else if (OB_SUCCESS != (err = log_mgr_->replay_tmp_log()))
          {
            if (OB_CANCELED == err)
            {
              YYSYS_LOG(WARN, "cancel replay tmp log");
            }
            else
            {
              YYSYS_LOG(ERROR, "replay_tmp_log()=>%d, will kill self", err);
              kill(getpid(), SIGTERM);
            }
          }
          else
          {
            ...
            is_replay_finished_ = true;
            //add :e
            YYSYS_LOG(INFO, "replay tmp log succ.");
            //add chujiajia [Paxos ups_replication] 20160104:b
            //lock_ = 3;//del [async load log]
            //add:e
          }
          return err;
        }
        ...
        ObUpsLogMgr* log_mgr_;
        //add wangdonghui [ups_replication] 20170816 :b
        //sometime replaying tmp log failed, but lock_ still advance to 2
        bool is_replay_finished_;
        //add :e
      };
      //add :e
...
//               ObUpsSlaveMgr *slave_mgr, ObiRole* obi_role, ObUpsRoleMgr *role_mgr, int64_t log_sync_type);
      ...
      int add_log_replay_event(const int64_t seq, const int64_t mutation_time, ReplayType replay_type,common::LogCommand cmd);
      ...
      int64_t get_fill_log_max_log_id();
      int set_fill_log_max_log_id();
      int set_fill_log_max_log_id(int64_t log_id);
      //add:e

        // UPS刚启动，重放本地日志任务的函数
      int replay_local_log();
      //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
      //brief:used after replay local log
      int do_process_unsettled_trans();
      //add 20150701:e
      ...
      int replay_log_to_queue();
      int load_log_from_disk(bool &is_append);
      int reset_switch();
      ...
      int set_real_replay_cursor() const;
      int set_replay_cursor() const;
      int reset_real_replay_cursor() const; //add lxb [paxos bug for replay tmp log]
      ...
      ObLogCursor& get_tmp_cursor()
      {return tmp_log_cursor_;}
      int64_t get_flushed_cursor_log_id() const
      {return flushed_cursor_.log_id_;}
      int64_t get_tmp_cursor_log_id() const
      {return tmp_log_cursor_.log_id_;}
      ObLogReplayWorker* get_replay_worker()
      {return replay_worker_;}
      //add :e
      ...
      int64_t to_string(char* buf, const int64_t len) const;
        ...
        void set_load_log_config(const int64_t replay_block_num, const int64_t used_block_num);
        void set_async_load_thread_to_stop();

        //[377]
        void set_async_load_log_gap(const int64_t async_load_log_gap);

        ObLogReplayPoint &get_replay_point()
      ...
      public:
      int set_state_as_active();
      int set_state_as_replaying_log();//add wangdonghui [ups_replication] 20170816 :b:e
      int get_max_log_seq_in_buffer(int64_t& log_seq) const;//mod wangdonghui [Paxos] make the func public
      ...
      inline int64_t get_last_receive_log_time()
      {return last_receive_log_time_;}
      inline int64_t get_last_receive_log_id() const
      {return last_receive_log_id_;} // add lbzhong[Clog Monitor] 20151218:b:e

        inline ObClogStat& get_clog_stat()
        {return clog_stat_; }
      ...
      //add huangpeng [for cbtop]:b
        inline int64_t get_current_max_commit_log_id() const
        {
            return current_max_commit_log_id_;
        }
      ...
      //add pangtianze [paxos]
      inline void set_log_term_inited(const bool log_term_inited)
      {
          log_term_inited_ = log_term_inited;
      }
      ...
      inline int check_inner_stat() const
      {
        ...
        if (!is_initialized_)
        {
          YYSYS_LOG(ERROR, "ObUpsLogMgr has not been initialized");
      ...
      private:
        ...
        volatile int64_t current_max_commit_log_id_; //add huangpeng [for cbtop]
        ...
        ObLogBuffer replay_log_cache_; //add lxb [package miss optimizer]
        ...
        yysys::CThreadCond master_log_id_cond_;
      ...
      bool is_started_;
      bool log_term_inited_; //add pangtianze[paxos]
      //add shili [LONG_TRANSACTION_LOG]  20160926:b
      ObBigLogWriter *big_log_writer_;
      //add e

      //add hxlong [async load log]
      ObLogLocation start_location_;
      ObLogLocation end_location_;
      bool is_can_append_;
      bool is_stopping_async_load;
      ObSwitch  load_log_switch_;
      int64_t replay_log_cache_block_num;
      int64_t is_used_block_num_;
      //add:e
      int64_t async_load_log_gap_;  //[377]
      int64_t fill_log_max_log_id_;
      //int64_t last_flushed_id_; //add pangtianze [bugfix async load log] 20190918
