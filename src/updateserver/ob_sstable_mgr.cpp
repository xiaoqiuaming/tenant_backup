  ...
  namespace updateserver
  {
    ...
    int ObUpsFetchParam::clone(const ObUpsFetchParam& rp)
    {
      ...
      for (ssti = rp.sst_list_.begin(); ssti != rp.sst_list_.end(); ++ssti)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "add_sst_file_info error, ret=%d name=%s path=%s",
    ...
    int ObUpsFetchParam::add_sst_file_info(const SSTFileInfo &sst_file_info)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "ObStringBuf write_string error, err=%d", err);
      }
      else
      {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "ObVector push_back error, err=%d sst_list_.size()=%d",
              err, sst_list_.size());
        }
      }
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "err=%d sst_file_info.path=\"%.*s\"(path.length=%d path.size=%d)"
    ...
    int ObUpsFetchParam::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      if (err != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "ObFetchParam serialize error, err=%d buf=%p pos=%ld buf_len=%ld serialize_size=%ld",
            err, buf, pos, buf_len, get_serialize_size());
      }
      else
      {
        YYSYS_LOG(DEBUG, "min_log_id=%lu max_log_id=%lu fetch_log_=%d fetch_sstable_=%d sst_list_.size()=%d",
    ...
    int ObUpsFetchParam::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      if (OB_SUCCESS == err)
      {
        for (int32_t i = 0; OB_SUCCESS == err && i < sst_num; i++)
        {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "ObFetchParam add_sst_file_info error, err=%d", err);
            }
          }
        }
      }
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "ObFetchParam deserialize error, err=%d buf=%p buf_len=%ld pos=%ld",
            err, buf, buf_len, pos);
      }
      else
      {
          ...
          && NULL != (sstable_mgr = sstable_info_->get_sstable_mgr()))
      {
            ...
            && NULL != fname_substr)
        {
          ...
          trash_fpath = SSTableMgr::build_str("%s/%s/%s%s.%ld",
                        path, TRASH_DIR, fname_substr, SSTABLE_SUFFIX, yysys::CTimeUtil::getTime());
        }
        if (NULL == fpath
            || NULL == trash_fpath)
        {
          YYSYS_LOG(WARN, "build sstable fpath fail path=[%s] fpath=[%s] trash_fpath=[%s]",
                    path, fpath, trash_fpath);
        }
        else if (0 != rename(fpath, trash_fpath))
        {
          YYSYS_LOG(WARN, "rename from [%s] to [%s] fail errno=%u", fpath, trash_fpath, errno);
        }
        else
        {
          YYSYS_LOG(INFO, "rename from [%s] to [%s] succ", fpath, trash_fpath);
    ...
    void StoreInfo::remove_schema_file_(const char *path, const char *fname_substr)
    {
          ...
          && NULL != fname_substr)
      {
        ...
        trash_fpath = SSTableMgr::build_str("%s/%s/%s%s.%ld",
                      path, TRASH_DIR, fname_substr, SCHEMA_SUFFIX, yysys::CTimeUtil::getTime());
      }
      if (NULL == fpath
          || NULL == trash_fpath)
      {
        YYSYS_LOG(WARN, "build schema fpath fail path=[%s] fpath=[%s] trash_fpath=[%s]",
                  path, fpath, trash_fpath);
      }
      else if (0 != rename(fpath, trash_fpath))
      {
    ...
    SSTableInfo::SSTableInfo() : ref_cnt_(0), sstable_mgr_(NULL),
                                 ...
                                 last_clog_id_(OB_INVALID_ID),loop_pos_(0), store_num_(0)
    {
    ...
    int SSTableInfo::init(SSTableMgr *sstable_mgr, const uint64_t sstable_id, const uint64_t clog_id, const uint64_t last_clog_id)
    {
      int ret = OB_SUCCESS;
      if (NULL == sstable_mgr
        ...
        for (int64_t i = 0; i < store_num_; i++)
        {
              ...
              && store_info->store_handle_match(store_handle))
          {
            if (0 == store_info->dec_ref_cnt())
            {
              if (NULL == sstable_mgr_)
              {
                YYSYS_LOG(WARN, "sstable_mgr null pointer cannot deallocate store_info=%p", store_info);
              }
              else
              {
    ...
    uint64_t SSTableInfo::get_last_clog_id() const
    {
    ...
    {
      return SSTableMgr::sstable_id2str(sstable_id_, clog_id_, last_clog_id_);
    }
    ...
    void SSTableInfo::log_sstable_info() const
    {
      ...
      YYSYS_LOG(INFO, "%s store_num=%ld handle=[%s]", sst_id.log_str(), store_num_, buffer);
    }


    void SSTableInfo::get_memtable_store_info_string(ObString &store_info)
    {
        char result[MAX_STORE_NUM * 11 + 2] = {'\0'};
        char buffer[MAX_STORE_NUM * 11] = {'\0'};
        int tmp_pos = 0;
        int tmp_ret = 0;
        int tmp_size = MAX_STORE_NUM * 11;
        ...
        snprintf(result, tmp_size + 2, "[%s]", buffer);
        store_info = ObString::make_string(const_cast<char *>(result));
    }
    ...
    int SSTableMgr::init(const char *store_root, const char *raid_regex, const char *dir_regex)
    {
      ...
      if (inited_)
      {
        ret = OB_INIT_TWICE;
              ...
              || NULL == dir_regex)
      {
        YYSYS_LOG(WARN, "invalid param store_root=%p raid_regex=%p dir_regex=%p",
                  store_root, raid_regex, dir_regex);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (0 != sstable_info_map_.create(SSTABLE_NUM))
      {
        YYSYS_LOG(WARN, "create sstable_info_map fail");
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = store_mgr_.init(store_root, raid_regex, dir_regex)))
      {
        YYSYS_LOG(WARN, "init store_mgr fail ret=%d store_root=[%s] raid_regex=[%s] dir_regex=[%s]",
                  ret, store_root, raid_regex, dir_regex);
      }
      else if (!sstable_fname_regex_.init(SSTABLE_FNAME_REGEX, REG_EXTENDED))
      {
        YYSYS_LOG(WARN, "init sstable fname regex fail regex=[%s]", SSTABLE_FNAME_REGEX);
      }
      else
      {
        YYSYS_LOG(INFO, "init sstable mgr succ store_root=[%s] raid_regex=[%s] dir_regex=[%s]",
                  store_root, raid_regex, dir_regex);
        inited_ = true;
      }
      if (OB_SUCCESS != ret)
    ...
    bool SSTableMgr::copy_sstable_file_(const uint64_t sstable_id, const uint64_t clog_id, const uint64_t last_clog_id,
                                        const StoreMgr::Handle src_handle, const StoreMgr::Handle dest_handle)
    {
      ...
      const char *fname_substr = sstable_id2str(sstable_id, clog_id, last_clog_id);
      ...
      if (NULL == src_path
          ...
          || NULL == fname_substr)
      {
        YYSYS_LOG(WARN, "invalid src_path=[%s] dest_path=[%s] fname_substr=[%s]", src_path, dest_path, fname_substr);
      }
      ...
      {
        YYSYS_LOG(WARN, "build fpath fail src_fname=[%s] dest_fname=[%s] src_path=[%s] dest_path=[%s] fname_substr=[%s]",
                  src_fname, dest_fname, src_path, dest_path, fname_substr);
      }
      else if (OB_SUCCESS != FSU::cp(src_path, src_fname, dest_path, dest_fname))
      {
        YYSYS_LOG(WARN, "copy %s/%s to %s/%s fail errno=%u", src_path, src_fname, dest_path, dest_fname, errno);
      }
      else
      {
        YYSYS_LOG(INFO, "copy %s/%s to %s/%s succ", src_path, src_fname, dest_path, dest_fname);
        bret = true;
      }
      return bret;
    }

    bool SSTableMgr::build_sstable_file_(const uint64_t sstable_id, const ObString &fpaths, const int64_t time_stamp, IRowIterator &iter, const CommonSchemaManager *sm)
    {
      bool bret = false;
      ...
      if (!iter.get_sstable_schema(sstable_schema,sm))
      {
        YYSYS_LOG(WARN, "get sstable schema fail sstable_id=%lu", sstable_id);
      }
      else if (!iter.get_compressor_name(compressor_str))
      {
        YYSYS_LOG(WARN, "get compressor name fail sstable_id=%lu", sstable_id);
      }
      else if (!iter.get_store_type(store_type))
      {
        YYSYS_LOG(WARN, "get store type fail sstable_id=%lu", sstable_id);
      }
      else if (!iter.get_block_size(block_size))
      {
        YYSYS_LOG(WARN, "get block size fail sstable_id=%lu", sstable_id);
      }
      if (NULL == ups_main)
      {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "sstable create fail ret=%d sstable_id=%lu", tmp_ret, sstable_id);
        }
        else
        {
          ...
          {
            if ((OB_SUCCESS != (tmp_ret = iter.get_row(sstable_row, sm)))&&(OB_TABLE_UPDATE_LOCKED != tmp_ret))
            {
              if (OB_SCHEMA_ERROR == tmp_ret || OB_UPS_TABLE_NOT_EXIST == tmp_ret)//mod [224]
              {
                continue;
              }
              YYSYS_LOG(WARN, "get row fail ret=%d", tmp_ret);
              break;
            }
            ...
            else if (NULL == sm->get_table_schema(sstable_row.get_table_id()))
            {
              //YYSYS_LOG(WARN, "get rowkey info failed, table_id=%lu", sstable_row.get_table_id());
              YYSYS_LOG(WARN, "get_table_schema failed, table_id=%lu", sstable_row.get_table_id());
            ...
            {
              YYSYS_LOG(WARN, "set rowkey info failed");
            }
            */
            else if (OB_SUCCESS != (tmp_ret = sstable_writer.append_row(sstable_row, approx_space_usage)))
            {
              YYSYS_LOG(WARN, "append row fail ret=%d table_id=%lu", tmp_ret, sstable_row.get_table_id());
              break;
            }
            else
            {
              YYSYS_LOG(DEBUG, "append row succ ret=%d table_id=%lu approx_space_usage=%ld",
            ...
            {
              YYSYS_LOG(WARN, "invalid role mgr stat=%d", ups_main->get_update_server().get_role_mgr().get_state());
              tmp_ret = OB_ERROR;
              break;
            }
          }
          ...
          {
            YYSYS_LOG(WARN, "iterate row fail ret=%d", tmp_ret);
          }
          else if (OB_SUCCESS != (tmp_ret = sstable_writer.close_sstable(trailer_offset, sstable_size)))
          {
            YYSYS_LOG(WARN, "close sstable fail ret=%d", tmp_ret);
          }
          else
          {
            YYSYS_LOG(INFO, "build sstable succ trailer_offset=%ld sstable_size=%ld", trailer_offset, sstable_size);
            bret = true;
          }
        }
      }
                                              ...
                                              IRowIterator &iter, const CommonSchemaManager *sm)
    {
      ...
      {
        YYSYS_LOG(WARN, "malloc fpaths fail");
      }
      else
      {
        ...
        for (list_iter = store_list.begin(); list_iter != store_list.end(); list_iter++, pos++)
        {
          ...
          fname_substr = sstable_id2str(sstable_info.get_sstable_id(), sstable_info.get_clog_id(), sstable_info.get_last_clog_id());
          fpath_tmp = NULL;
          ...
          if (NULL != fpath_tmp)
          {
            ...
            YYSYS_LOG(INFO, "build fpath succ [%s]", fpath_tmp);
            buffer_pos += str_ret;
            buffer_size -= str_ret;
          }
        }
        if (0 < pos)
        {
          ...
          if (!build_sstable_file_(sstable_info.get_sstable_id(), fpath_str, time_stamp, iter, sm))
          {
            YYSYS_LOG(WARN, "build sstable file fail sstable_id=%lu clog_id=%lu",
        ...
        for (list_iter = store_list.begin(); i < pos && list_iter != store_list.end(); list_iter++, i++)
        {
          ...
          if (NULL != fpath_tmp && NULL != fpath)
          {
            ...
            {
              YYSYS_LOG(WARN, "rename [%s] to [%s] fail errno=%u", fpath_tmp, fpath, errno);
            }
            else
            {
              YYSYS_LOG(INFO, "rename [%s] to [%s] succ timeu=%ld", fpath_tmp, fpath, yysys::CTimeUtil::getTime() - timestamp);
        ...
        {
          YYSYS_LOG(WARN, "no valid sstable file has been build sstable_id=%lu clog_id=%lu",
                    sstable_info.get_sstable_id(), sstable_info.get_clog_id());
        }
        else
        {
    ...
    bool SSTableMgr::build_schema_file_(const char *path, const char *fname_substr, const CommonSchemaManager *sm)
    {
          ...
          && NULL != fname_substr)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "open file [%s] for dump schema fail ret=%d", fpath_tmp, ret);
        }
        else if (NULL == buffer)
        {
          ret = OB_ERROR; //add zhaoqiong [Schema Manager] 20150429:b
          YYSYS_LOG(ERROR, "alloc tmp buffer for dump schema fail");
        }
        else if (NULL == sm)
        {
          ret = OB_ERROR; //add zhaoqiong [Schema Manager] 20150429:b
          YYSYS_LOG(WARN, "get schema mgr fail");
        }
        ...
        //add:e
          ...
          {
            YYSYS_LOG(WARN, "append buffer to file fail ret=%d buffer=%p size=%ld", ret, buffer, buf_pos);
          }
          else
          {
            file.close();
            ...
            {
              YYSYS_LOG(INFO, "build schema file rename from [%s] to [%s] succ", fpath_tmp, fpath);
              bret = true;
            }
          }
        }
                                ...
                                IRowIterator &iter, const CommonSchemaManager *sm, uint64_t last_clog_id)
    {
      ...
      else
      {
          ...
          if((OB_INVALID_ID == last_clog_id) && (0 == sst_num))
          {
              //no sstable file in directory, get frozen log id from log_replay_point
              ...
              {
                  last_clog_id = static_cast<uint64_t>(last_clog_id_int);
              }
              YYSYS_LOG(INFO,"get last_clog_id from log_replay_point, last_clog_id=%lu", last_clog_id);
          }
      }
      //add:e
      ...
      if (!inited_)
      {
        ...
        ret = OB_NOT_INIT;
              ...
              || OB_INVALID_ID == clog_id)
      {
        YYSYS_LOG(WARN, "invalid param sstable_id=%lu clog_id=%lu", sstable_id, clog_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (sstable_info = sstable_allocator_.alloc()))
      {
        YYSYS_LOG(WARN, "allocate sstable_info fail sstable_id=%lu", sstable_id);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = sstable_info->init(this, sstable_id, clog_id, last_clog_id)))
      {
        YYSYS_LOG(WARN, "init sstable_info fail ret=%d sstable_id=%lu", ret, sstable_id);
      }
      else if (OB_SUCCESS != (ret = store_mgr_.assign_stores(store_list)))
      {
        YYSYS_LOG(WARN, "assign_stores fail ret=%d sstable_id=%lu", ret, sstable_id);
      }
      else
      {
        if (sstable_exist_(sstable_id))
        {
          ...
          YYSYS_LOG(WARN, "%s has already exist, will erase it", sst_id.log_str());
        ...
        {
          YYSYS_LOG(WARN, "no valid sstable file has been build sstable_id=%lu", sstable_id);
          ret = OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "set to sstable_info_map fail ret=%d sstable_id=%lu", hash_ret, sstable_id);
            ret = OB_ERROR;
          }
          else
          {
            YYSYS_LOG(INFO, "dump sstable succ sstable_id=%lu", sstable_id);
          }
        }
      }
      if (OB_SUCCESS != ret
    ...
    int SSTableMgr::erase_sstable(const uint64_t sstable_id, const bool remove_sstable_file)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "erase sstable_info fail ret=%d sstable_id=%lu", hash_ret, sstable_id);
          ret = OB_ERROR;
        }
        else
        {
          ...
          {
            sstable_allocator_.free(sstable_info);
            YYSYS_LOG(INFO, "erase sstable succ sstable_id=%lu", sstable_id);
          }
        }
      }
      return ret;
    ...
    void SSTableMgr::check_broken()
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "start check broken");
        ...
        YYSYS_LOG(INFO, "end check broken erase timeu=%ld erase_sstable_num=%ld",
                  yysys::CTimeUtil::getTime() - timeu, deleted_list.size());
    ...
    void SSTableMgr::reload_all()
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "reload all store_num=%ld timeu=%ld", store_list.size(), yysys::CTimeUtil::getTime() - timeu);
      }
    }
    ...
    bool SSTableMgr::load_new()
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "start load");
        while (NULL != (path = store_mgr_.get_new_dir(store_handle)))
        {
          load_dir_(store_handle);
        }
        YYSYS_LOG(INFO, "end load timeu=%ld add_sstable_num=%ld bret=%d",
                  yysys::CTimeUtil::getTime() - timeu, sstable_info_map_.size() - sstable_num, bret);
      }
      return bret;
    }
    ...
    void SSTableMgr::umount_store(const char *path)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param path=%p", path);
      }
      else
      {
    ...
    int SSTableMgr::reg_observer(ISSTableObserver *observer)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param observer=%p", observer);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = observer_list_.push_back(observer)))
      {
        YYSYS_LOG(WARN, "push observer to list fail ret=%d observer=%p", ret, observer);
      }
      else
      {
        // do nothing
                                     ...
                                     const uint64_t max_fill_major_num, ObUpsFetchParam &fetch_param)
    {
      ...
      {
        YYSYS_LOG(WARN, "get current dir name failed errno=%u", errno);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for (iter = sstable_info_map_.begin(); iter != sstable_info_map_.end(); iter++)
        {
          ...
          if (NULL == sstable_info)
          {
            YYSYS_LOG(ERROR, "invalid sstable_info=%p", sstable_info);
            ret = OB_ERROR;
            break;
          }
              ...
              && 0 <= SSTableID::compare(range_end, tmp_sst_id))
          {
            ...
            {
              YYSYS_LOG(ERROR, "add sstable_info to list fail ret=%d %s", ret, tmp_sst_id.log_str());
              break;
            }
          }
        }
        ...
        for (list_iter = list2fill.begin(); list_iter != list2fill.end(); list_iter++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "invalid sstable_info=%p or store_info=%p", sstable_info, store_info);
            ret = OB_ERROR;
            break;
          }
          ...
          if (0 > SSTableID::compare(slave_end, tmp_sst_id))
          {
            ...
            {
              YYSYS_LOG(INFO, "add sstable_info to fetch param succ %s fpath=%s/%s",
      ...
      if (NULL != path)
      {
        ...
        if (NULL != dd)
        {
                ...
                && NULL != dir_iter->d_name)
          {
            ...
            {
              YYSYS_LOG(DEBUG, "fname=[%s/%s] do not match regex", path, dir_iter->d_name);
              continue;
            }
            ...
            if (!sstable_str2id(dir_iter->d_name, sstable_id, clog_id, last_clog_id)
                || OB_INVALID_ID == sstable_id
                || OB_INVALID_ID == clog_id
                || OB_INVALID_ID == last_clog_id)
            {
              YYSYS_LOG(WARN, "fname=[%s/%s] trans to sstable_id fail", path, dir_iter->d_name);
              continue;
            }
            if (!check_sstable_(fpath, NULL))
            {
              YYSYS_LOG(WARN, "fpath=[%s] do not pass sstable check, will not load it", fpath);
              continue;
            }
            bool invoke_callback = true;
            add_sstable_file_(sstable_id, clog_id, last_clog_id, store_handle, invoke_callback);
          }
          closedir(dd);
          YYSYS_LOG(INFO, "load dir path=[%s] store_handle=%lu timeu=%ld",
                    path, store_handle, yysys::CTimeUtil::getTime() - timeu);
        }
      }
    }
    ...
    bool SSTableMgr::get_clog_and_last_clog(const uint64_t sstable_id, uint64_t &clog, uint64_t &last_clog)
    {
        bool bret = false;
        SSTableInfo *sstable_info = NULL;
        map_lock_.rdlock();
        ...
        if(bret)
        {
            clog = sstable_info->get_clog_id();
            last_clog = sstable_info->get_last_clog_id();
        }
        map_lock_.unlock();
        return bret;
    }
                                      ...
                                      const bool invoke_callback)
    {
      ...
      else if (hash::HASH_NOT_EXIST == hash_ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "allocate sstable_info fail sstable_id=%lu fname=[%s] store_handle=%lu",
                    sstable_id, sstable_id2str(sstable_id, clog_id, last_clog_id), store_handle);
        }
        else if (OB_SUCCESS != (tmp_ret = sstable_info->init(this, sstable_id, clog_id,last_clog_id)))
        {
          YYSYS_LOG(WARN, "init sstable_info fail ret=%d sstable_id=%lu fname=[%s] store_handle=%lu",
                    tmp_ret, sstable_id, sstable_id2str(sstable_id, clog_id, last_clog_id), store_handle);
        }
        else if (OB_SUCCESS != (tmp_ret = sstable_info->add_store(store_handle)))
        {
          YYSYS_LOG(WARN, "add_store to sstable_info fail ret=%d sstable_id=%lu fname=[%s] store_handle=%lu",
                    tmp_ret, sstable_id, sstable_id2str(sstable_id, clog_id, last_clog_id), store_handle);
        }
        else if (hash::HASH_INSERT_SUCC != (hash_ret = sstable_info_map_.set(sstable_id, sstable_info)))
        {
          YYSYS_LOG(WARN, "add sstable_info to map fail hash_ret=%d sstable_id=%lu fname=[%s] store_handle=%lu,",
                    hash_ret, sstable_id, sstable_id2str(sstable_id, clog_id, last_clog_id), store_handle);
        }
        else
        {
          add_succ = true;
                                        ...
                                        uint64_t &checksum)
    {
      ...
      {
        YYSYS_LOG(WARN, "get all valid stores fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = prepare_load_info_(store_list, allocator, info_list)))
      {
        YYSYS_LOG(WARN, "prepare load fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = info_list_uniq_(allocator, info_list)))
      {
        YYSYS_LOG(WARN, "info list uniq fail ret=%d", ret);
      }
      else
      {
        load_list_bypass_(info_list, major_version, minor_version_start, minor_version_end, clog_id, last_clog_id, table_list, tmp_checksum);
      }
      ...
      YYSYS_LOG(INFO, "load sstable bypass ret=%d loaded_num=%ld checksum=%lu timeu=%ld",
                ret, table_list.size(), checksum, yysys::CTimeUtil::getTime() - timeu);
      return ret;
    }
...

    {
      ...
      for (iter = store_list.begin(); iter != store_list.end(); iter++)
      {
        ...
        {
          YYSYS_LOG(WARN, "get store dir fail, store_handle=%lu", *iter);
        }
        else if (NULL == (bypass_path = build_str("%s/%s", store_path, BYPASS_DIR)))
        {
          YYSYS_LOG(WARN, "get bypass dir fail, bypass_path=[%s/%s]", store_path, BYPASS_DIR);
        }
        else if (NULL == (dd = opendir(bypass_path)))
        {
          YYSYS_LOG(WARN, "open dir fail bypass_path=[%s] errno=%u", bypass_path, errno);
        }
        else
        {
          struct dirent* dir_iter = NULL;
                ...
                && NULL != dir_iter->d_name)
          {
            ...
            if (NULL == info)
            {
              YYSYS_LOG(WARN, "alloc load bypass info fail");
              break;
            }
            else if (OB_MAX_FILE_NAME_LENGTH <= snprintf(info->fname, OB_MAX_FILE_NAME_LENGTH, "%s", dir_iter->d_name))
            {
              YYSYS_LOG(WARN, "file name too long will not load [%s]", dir_iter->d_name);
            }
            else
            {
                                    ...
                                    ObList<LoadBypassInfo*> &info_list)
    {
      ...
      if (NULL == sort_array)
      {
        YYSYS_LOG(WARN, "alloc array for sort fail info_num=%ld", info_list.size());
        ret = OB_MEM_OVERFLOW;
      }
      else
      {
        ...
        for (int64_t i = 0; i < counter; i++)
        {
          ...
          else
          {
            ...
            else
            {
              if (NULL != prev)
              {
                YYSYS_LOG(INFO, "[%s] same_count=%ld will not load", prev->fname, same_counter);
              }
            }
            prev = sort_array[i];
            same_counter = 1;
                                      ...
                                      uint64_t &checksum)
    {
      ...
      for (iter = info_list.begin(); cur_minor_version <= minor_version_end && iter != info_list.end(); iter++)
      {
        if (NULL == *iter)
        {
          YYSYS_LOG(WARN, "load bypass info null pointer minor_version_start=%lu cur_minor_version=%lu",
                    minor_version_start, cur_minor_version);
        }
        else if (NULL == (store_path = store_mgr_.get_dir((*iter)->store_handle)))
        {
          YYSYS_LOG(WARN, "get store dir fail, store_handle=%lu", (*iter)->store_handle);
        }
        else if (NULL == (bypass_path = build_str("%s/%s", store_path, BYPASS_DIR)))
        {
          YYSYS_LOG(WARN, "get bypass dir fail, bypass_path=[%s/%s]", store_path, BYPASS_DIR);
        ...
        else
        {
          ...
          const char *store_fpath = build_str("%s/%s%s", store_path, sstable_id2str(sstable_id, clog_id, last_clog_id), SSTABLE_SUFFIX);
          uint64_t sstable_checksum = 0;
          if (NULL == bypass_fpath
              || NULL == store_fpath)
          {
            YYSYS_LOG(WARN, "build entire fpath fail fname=[%s] bypass_path=[%s/%s] store_path=[%s]",
          ...
          {
            YYSYS_LOG(INFO, "fpath=[%s] do not pass sstable check, will not load it", bypass_fpath);
          }
          else if (0 != rename(bypass_fpath, store_fpath))
          {
            YYSYS_LOG(WARN, "rename from [%s] to [%s] fail, errno=%u", bypass_fpath, store_fpath, errno);
          }
          else
          {
            YYSYS_LOG(INFO, "rename from [%s] to [%s] succ", bypass_fpath, store_fpath);
            if (0 != sstable_list.push_back(sstable_id))
            {
              YYSYS_LOG(WARN, "push to sstable list fail %s", SSTableID::log_str(sstable_id));
            }
            else
            {
              bool invoke_callback = false;
              add_sstable_file_(sstable_id, clog_id, last_clog_id, (*iter)->store_handle, invoke_callback);
              cur_minor_version++;
      ...
      YYSYS_LOG(INFO, "load sstable_num=%ld timeu=%ld", sstable_list.size(), yysys::CTimeUtil::getTime() - timeu);
    }
    ...
    uint64_t SSTableMgr::get_last_major_frozen_clog_file_id()
    {
      ...
      {
        YYSYS_LOG(WARN, "get_last_frozen version err=%d", err);
      }
    ...
    const char *SSTableMgr::build_str(const char *fmt, ...)
    {
      ...
      va_list args;
      va_start(args, fmt);
      YYSYS_LOG(DEBUG, "build_str prev [%ld][%s]", i - 1, fpath);
      if (StoreMgr::MAX_DIR_NAME_LENGTH > vsnprintf(fpath, StoreMgr::MAX_DIR_NAME_LENGTH, fmt, args))
      {
        ret = fpath;
      }
      va_end(args);
      YYSYS_LOG(DEBUG, "build_str cur [%ld][%s] ret=%p", i - 1, fpath, ret);
      return ret;
    }

    const char *SSTableMgr::sstable_id2str(const uint64_t sstable_id, const uint64_t clog_id, const uint64_t last_clog_id)
    {
      char *ret = NULL;
      static __thread char str[SSTableInfo::MAX_SUBSTR_SIZE];
      SSTableID sst_id(sstable_id);
      if (SSTableInfo::MAX_SUBSTR_SIZE > snprintf(str, SSTableInfo::MAX_SUBSTR_SIZE, "%lu_%lu-%lu_%lu-%lu",
                                                  sst_id.major_version, sst_id.minor_version_start,
                                                  sst_id.minor_version_end, last_clog_id, clog_id))
      {
        ret = str;
      }
      return ret;
    ...
    bool SSTableMgr::sstable_str2id(const char *sstable_str, uint64_t &sstable_id, uint64_t &clog_id, uint64_t &last_clog_id)
    {
      bool bret = false;
      if (NULL != sstable_str)
      {
        ...
        uint64_t check_overflow[num] = {SSTableID::MAX_MAJOR_VERSION, SSTableID::MAX_MINOR_VERSION,
                                      SSTableID::MAX_MINOR_VERSION, SSTableID::MAX_CLOG_ID, SSTableID::MAX_CLOG_ID};
        bret = true;
    ...
    const IFileInfo *SSTableMgr::get_fileinfo(const uint64_t sstable_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "map_lock rdlock fail");
      }
      else
      {
        SSTableInfo *sstable_info = NULL;
        if (hash::HASH_EXIST != sstable_info_map_.get(sstable_id, sstable_info)
            || NULL == sstable_info)
        {
          YYSYS_LOG(WARN, "get from sstable_info_map fail sstable_id=%lu", sstable_id);
        }
        else
        {
    ...
    int SSTableMgr::revert_fileinfo(const IFileInfo *file_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param file_info=%p", file_info);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != map_lock_.rdlock())
      {
    ...
    int SSTableMgr::get_schema(const uint64_t sstable_id, CommonSchemaManagerWrapper &sm)
    {
      ...
      {
        YYSYS_LOG(WARN, "get file info of %s fail", SSTableID::log_str(sstable_id));
        ret = OB_ERROR;
      }
      else if (NULL == (store_info = dynamic_cast<const StoreInfo*>(file_info)))
      {
        YYSYS_LOG(WARN, "cast from file_info=%p to store_info fail", file_info);
        ret = OB_ERROR;
      }
      else if (NULL == (sstable_info = store_info->get_sstable_info()))
      {
        YYSYS_LOG(WARN, "get sstable info from store info fail");
        ret = OB_ERROR;
      }
      else if (NULL == (path = store_info->get_dir()))
      {
        YYSYS_LOG(WARN, "get dir from store info fail");
        ret = OB_ERROR;
      }
      else if (NULL == (fname_substr = sstable_id2str(sstable_id, sstable_info->get_clog_id(), sstable_info->get_last_clog_id())))
      {
        YYSYS_LOG(WARN, "build sstable id to string fail %s clog_id=%ld",
                  SSTableID::log_str(sstable_id), sstable_info->get_clog_id());
        ret = OB_ERROR;
      }
      else if (NULL == (fpath = build_str("%s/%s%s", path, fname_substr, SCHEMA_SUFFIX)))
      {
        YYSYS_LOG(WARN, "build schema fpath fail path=[%s] fname_substr=[%s]", path, fname_substr);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "open file [%s] fail ret=%d", fpath, ret);
      }
      else if (0 != stat(fpath, &st))
      {
        YYSYS_LOG(WARN, "stat file [%s] fail errno=%u", fpath, errno);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = file.pread(st.st_size, 0, buffer, ret_size)))
      {
        YYSYS_LOG(WARN, "pread file [%s] fail ret=%d size=%ld", fpath, ret, st.st_size);
      }
      else
      {
        YYSYS_LOG(INFO, "read from schema file [%s] succ, file_size=%ld ret_size=%ld", fpath, st.st_size, ret_size);
      }
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(INFO, "read from schema file fail, will get the local latest version");
    ...
    {
      SSTableInfoMap::iterator iter;
      YYSYS_LOG(INFO, "==========log sstable info start==========");
      map_lock_.rdlock();
      for (iter = sstable_info_map_.begin(); iter != sstable_info_map_.end(); iter++)
      {
        SSTableInfo *sstable_info = iter->second;
      ...
      map_lock_.unlock();
      YYSYS_LOG(INFO, "==========log sstable info end==========");
    }

    int64_t SSTableMgr::get_sstable_info_map_size()
    {
        return sstable_info_map_.size();
    }
  }
}
