...
//      {
//        str = "unknown";
//        YYSYS_LOG(WARN, "unknown role[%ld]", role);
//      }
//      return str;
//    }
...
//    }
    int get_role_as_int(const ObUpsRoleMgr::Role role, const ObiSlaveStat obi_slave = FOLLOWED_SLAVE)
    {
      UNUSED(obi_slave);
      return (role == ObUpsRoleMgr::MASTER ? 0: 1);
    }
    //mod 20150701:e
    ...
    int ObUpsCLogStatus::serialize(char* buf, int64_t len, int64_t& pos) const
    {
      ...
      //mod peiouya [MultiUPS] [DELETE_OBI] 20150701:b
              ...
              || OB_SUCCESS != sys_table_master_ups_.serialize (buf, len, pos)  //add peiouya [MultiUPS] [DELETE_OBI] 20150810
              ...
              || OB_SUCCESS != serialization::encode_i64(buf, len, pos, wait_response_))
      {
        ...
        YYSYS_LOG(ERROR, "clog_status.serialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, pos, err);
      }
      return err;
    }
    ...
    int ObUpsCLogStatus::deserialize(const char* buf, int64_t len, int64_t& pos)
    {
      ...
      if (NULL == buf || pos < 0 || pos > len)
      {
        err = OB_INVALID_ARGUMENT;
              ...
              //del 20150701:e
              || OB_SUCCESS != sys_table_master_ups_.deserialize (buf, len, pos)  //add peiouya [MultiUPS] [DELETE_OBI] 20150810
              ...
              || OB_SUCCESS != serialization::decode_i64(buf, len, pos, (int64_t*)&wait_response_))
      {
        ...
        YYSYS_LOG(ERROR, "clog_status.deserialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, pos, err);
      }
      return err;
    }
    ...
    int ObUpsCLogStatus::to_str(char* buf, const int64_t len, const bool detail /*=true*/) const
    {
      ...
      char sys_table_master_ups_addr[OB_MAX_SERVER_ADDR_SIZE];  //add peiouya [MultiUPS] [DELETE_OBI] 20150810
      //mod peiouya [MultiUPS] [DELETE_OBI] 20150701:b
      ...
      char* clog_src_addr_array[] = {self_addr, ups_master_addr};
      int64_t role = get_role_as_int(role_mgr_.get_role(), obi_slave_stat_);
      ...
      if (!rs_.to_string(rs_addr, sizeof(rs_addr)) || !self_.to_string(self_addr, sizeof(self_addr))
          || !ups_master_.to_string(ups_master_addr, sizeof(ups_master_addr)) /*|| !inst_ups_master_.to_string(inst_ups_master_addr, sizeof(inst_ups_master_addr))
          || !lsync_.to_string(lsync_addr, sizeof(lsync_addr))*/
          || !sys_table_master_ups_.to_string (sys_table_master_ups_addr, sizeof(sys_table_master_ups_addr)))  //add peiouya [MultiUPS] [DELETE_OBI] 20150810
      {
...
//                     || count > len)
                     ...
                     || count > len)
      {
        ...
        YYSYS_LOG(WARN, "snprintf(buf=%p[%ld]): count=%ld", buf, len, count);
      }
      ...
      //else if (detail && 0 >= (count2 = snprintf(buf + count, len - count,
      //                                           "rs=%s, self=%s, ups_master=%s\n",
      //                                           rs_addr, self_addr, ups_master_addr))
      //                         && count2 > len - count)
      else if (detail && 0 >= (count2 = snprintf(buf + count, len - count,
                                                 "rs=%s, self=%s, ups_master=%s, sys_table_master_ups=%s\n",
                                                 rs_addr, self_addr, ups_master_addr, sys_table_master_ups_addr))
                               && count2 > len - count)
      //mod 20150810:e
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(WARN, "snprintf(buf=%p[%ld]): count=%ld", buf + count, len - count, count2);
      }
      return err;
    }
