...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO,"pos=[%ld,%ld) read_pos =%ld,block_start_id=%ld,block_end_id=%ld,block_read_id=%ld",
                start_pos_, end_pos_, replay_pos_,
                get_block_id(start_pos_),
                get_block_id(end_pos_),
                get_block_id(replay_pos_));
      //YYSYS_LOG(INFO, "pos=[%ld,%ld)", start_pos_, end_pos_);
      YYSYS_LOG(INFO, "block=%ld*(1<<%ld)", n_blocks_, block_size_shift_);
      if (NULL != blocks_)
      {
    ...
    int ObRingDataBuffer::init(const int64_t n_blocks, const int64_t block_size_shift)
    {
      ...
      for (int64_t i = 0; OB_SUCCESS == err && i < n_blocks; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "blocks[%ld].init()=>%d", i, err);
        }
      }
      if (OB_SUCCESS == err)
      {
    ...
    //add[async load log]:b
    int64_t ObRingDataBuffer::get_replay_pos() const
    {
        return replay_pos_;
    }

    void ObRingDataBuffer::set_replay_pos(int64_t read_pos, int64_t read_count)
    {
        if(read_pos < replay_pos_ || read_count < 0)
        {
            YYSYS_LOG(WARN,"set ring data buff failed replay_pos=%ld,read_pos=%ld,read_count=%ld",
                      replay_pos_,read_pos, read_count);
        }
        else
        {
            replay_pos_ = read_pos + read_count;
    ...
    bool ObRingDataBuffer::is_can_append() const
    {
        return get_block_id(get_next_block_pos(end_pos_)) == get_block_id(replay_pos_)?false : true;
    }

    int64_t ObRingDataBuffer::get_use_blocks()
    {
        int64_t used_blocks = get_block_id(end_pos_) - get_block_id(replay_pos_);
        return used_blocks >=0 ? used_blocks : (n_blocks_ + used_blocks);
    }
    ...
    int ObRingDataBuffer::read(const int64_t pos, int64_t& real_pos, char* buf, const int64_t len, int64_t& read_count) const
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "read(buf=%p[%ld], pos=%ld):invalid argument", buf, len, pos);
      }
      ...
      {
        YYSYS_LOG(ERROR, "get_read_pos(pos=%ld)=>%d", pos, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "block[%ld].read(pos=%ld)=>%d", pos>>block_size_shift_, pos, err);
      }
    ...
    int ObRingDataBuffer::next_block_for_append()
    {
      ...
      {
        YYSYS_LOG(ERROR, "block[%ld].reset(new_pos=%ld)=>%d", end_pos_>>block_size_shift_, end_pos_, err);
      }
      __sync_synchronize();
      return err;
    }
    ...
    int ObRingDataBuffer::append(const char* buf, const int64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "append(buf=%p[%ld]):invalid argument", buf, len);
      }
      else if (blocks_[get_block_id(end_pos_)].get_remain_size() < len
               && OB_SUCCESS != (err = next_block_for_append()))
      {
        YYSYS_LOG(ERROR, "next_block_for_append()=%d", err);
      }
      else if (OB_SUCCESS != (err = blocks_[get_block_id(end_pos_)].append(buf, len)))
      {
        YYSYS_LOG(ERROR, "blocks[%ld].append(pos=%ld)=>%d", end_pos_>>block_size_shift_, end_pos_, err);
      }
      else
      {
        __sync_synchronize();
    ...
    int ObRingDataBuffer::append(const char *buf, const int64_t len, bool &is_append)
    {
        int err = OB_SUCCESS;
        is_append = true;
        if(!is_inited())
        ...
        else if(blocks_[get_block_id(end_pos_)].get_remain_size() < len && (!is_can_append()))
        {
            is_append = false;
            err = OB_EAGAIN;
            YYSYS_LOG(WARN, "the ring data buff is full,please wait,end_pos=%ld, replay_pos=%ld, start_pos=%ld"
                      ", end_block=%ld, replay_block=%ld, start_block=%ld",
                      end_pos_, replay_pos_, start_pos_,
                      get_block_id(end_pos_), get_block_id(replay_pos_), get_block_id(start_pos_));
        }
        else if(blocks_[get_block_id(end_pos_)].get_remain_size() < len
                && OB_SUCCESS != (err = next_block_for_append()))
        {
