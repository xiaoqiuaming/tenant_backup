...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int ObBigLogWriter::write_big_log(const ObTransID &tid, RWSessionCtx &session_ctx)
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "not init ,ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = unpackage_big_log(tid)))
      {
        YYSYS_LOG(WARN, "unpackage big log fail,ret=%d", ret);
      }
      else
      {
        //将拆分的小的包，分别刷盘
        for(int64_t i = 0; OB_SUCCESS == ret && i < package_num_; i++)
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "ups_mutator serialilze fail log_buffer=%p log_buffer_size=%ld "
          ...
          else
          {
            ...
            if(OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "log_mgr  flush log twice fail,ret=%d", ret);
            }
          }
          ...
          if(OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "write_and_flush_log fail, ret=%d",ret);
            }
            else
            {
    ...
    int ObBigLogWriter::apply(ObLogTask &task)
    {
      ...
      switch(log_cmd_)
      {
        ...
        case OB_UPS_MUTATOR_PREPARE:
            ...
            YYSYS_LOG(ERROR, "big log don't supported  distributed transaction,err=%d", err);
        default:
          err = OB_NOT_SUPPORTED;
          YYSYS_LOG(WARN, "no supported,ret=%d",err);
      }
      return err;
    }
    ...
    int ObBigLogWriter::check_data_log(const ObNormalLogData &log_data)
    {
      ...
      if(!log_data.trans_id_.is_valid())
      {
        ...
        YYSYS_LOG(WARN, " log data trans id is invalid,ret=%d",ret);
      }
      else if(is_first_package(log_data))
      {
        ...
        {
          ret = OB_INVALID_SESSION_ID;
          YYSYS_LOG(WARN, "last trans id  equal to this trans id,ret=%d",ret);
        }
        else if(last_seq_ + 1 != package_num_)/*上一个big_log 最后一个或者最后几个块 丢失，不需要处理,只报警报*/
        {
          YYSYS_LOG(WARN, "last big log seq  don't equal to package num");
        }
      }
      else
      {
        if(!last_trans_id_.equal(log_data.trans_id_))/*最开头的一个或者几个块 丢失*/
        {
          ...
          YYSYS_LOG(WARN, "big log is not continuous,ret=%d", ret);
        }
    ...
    int ObBigLogWriter::package_big_log(const char *log_data, const int64_t data_len, bool &is_completed)
    {
      ...
      {
        YYSYS_LOG(WARN, "log data deserialize fail,ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = check_data_log(tmp_log_data)))
      {
        YYSYS_LOG(WARN, "log data is invalid,ret=%d", ret);
      }
      else if(is_first_package(tmp_log_data))
      {
        ...
        if(ptr_ == NULL)
        {
          ...
          YYSYS_LOG(WARN, "alloc  log data memery fail,ret=%d", ret);
        }
        else
        {
          last_seq_ = 0;
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        if(0==(write_size = write(tmp_log_data.data_buf_, tmp_log_data.buf_size_)))
        {
          ...
          YYSYS_LOG(WARN, "write log data to log_writer fail,ret=%d", ret);
        }
        else if(OB_SUCCESS != (ret = package_store_.push_back(tmp_log_data)))
        {
          YYSYS_LOG(WARN, "push back fail,ret=%d", ret);
        }
    ...
    int ObBigLogWriter::handle_big_log(ObLogTask &task)
    {
      ...
      {
        YYSYS_LOG(WARN, "package_big_log fail,ret=%d",ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "apply  fail,ret=%d", ret);
      }
      return ret;
    }
    ...
    int ObBigLogWriter::unpackage_big_log(const ObTransID &tid)
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "ptr_ is null,ret=%d", ret);
      }
    ...
    int ObBigLogWriter::init()
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(WARN, "big log writer had inited, err=%d", err);
      }
    ...
    int ObNormalLogData::serialize(char *buf, const int64_t buf_len, int64_t &pos) const
    {
      ...
      if(res == OB_SUCCESS)
      {
        ...
        *(reinterpret_cast<int32_t*> (buf + pos)) = (int32_t) log_cmd_;
        ...
        {
          YYSYS_LOG(WARN,"serialize trans id fail,ret=%d",res);
        }
        else if(OB_SUCCESS != (res = serialization::encode_vstr(buf, buf_len, pos, data_buf_, buf_size_)))
        {
          YYSYS_LOG(WARN,"serialize vstr fail,ret=%d",res);
        }
      }
      return res;
    }
    ...
    int ObNormalLogData::deserialize(const char *buf, const int64_t data_len, int64_t &pos)
    {
      ...
      if(OB_SUCCESS == res)
      {
        ...
        {
          YYSYS_LOG(WARN,"deserialize trans id fail,ret=%d",res);
        }
      }
      ...
      if(OB_SUCCESS == res)
      {
        ...
        {
          res = OB_ERROR;
          YYSYS_LOG(WARN,"deserialize string fail,ret=%d",res);
        }
