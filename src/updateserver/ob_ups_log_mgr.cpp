...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    int64_t get_last_major_frozen_log_file_id(const char* log_dir)
    {
      ...
      //{
      //  YYSYS_LOG(ERROR, "scanner.init(log_dir=%s)=>%d", log_dir, err);
      //}
      else if (OB_SUCCESS != (err = scanner.init(log_dir)))
      {
      ...
      {
        YYSYS_LOG(ERROR, "scanner.get_min_log_file_id()=>%d", err);
      }
      return file_id;
    }
    ...
    int compare_log_buffer(const ObLogCursor& start_cursor, int64_t end_id, const char* buf1, const char* buf2, int64_t data_len, ObLogCursor& pass_cursor)
    {
      ...
      if (NULL == buf1 || NULL == buf2 || data_len <= 0 || !start_cursor.is_valid() || start_cursor.log_id_ >= end_id)
      {
        ...
        YYSYS_LOG(ERROR, "invalid argument, buf1=%p, buf2=%p, end_id=%ld, start_cursor=%s data_len=%ld", buf1, buf2, end_id, start_cursor.to_str(), data_len);
      }
      ...
      while(OB_SUCCESS == err && pos1 < data_len)
      {
        ...
        {
          YYSYS_LOG(ERROR, "log_entry1.deserialize(log_data=%p, data_len=%ld, pos=%ld)=>%d", buf1, data_len, pos1, err);
        }
        else if (OB_SUCCESS != (err = log_entry2.deserialize(buf2, data_len, pos2)))
        {
          YYSYS_LOG(ERROR, "log_entry2.deserialize(log_data=%p, data_len=%ld, pos=%ld)=>%d", buf2, data_len, pos2, err);
        }
        ...
        {
          YYSYS_LOG(WARN, "Checking TMP_LOG LOG not match @(%s)", pass_cursor.to_str());
          err = OB_INVALID_LOG;
        }
        //add:e
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "pass_cursor[%ld].advance(%ld)=>%d", pass_cursor.log_id_, log_entry1.seq_, err);
          }
        }
      }
      //YYSYS_LOG(INFO, "pass_cursor.log_id_[%ld], log_entry1.seq_[%ld], err=[%d]", pass_cursor.log_id_, static_cast<int64_t>(log_entry1.seq_), err);
      //YYSYS_LOG(INFO, "current_log_term[%ld]", UPS.get_log_mgr().get_current_log_term());
      return err;
    }
    //add :e
    ...
    int parse_log_buffer(const char* log_data, int64_t data_len, const ObLogCursor& start_cursor, ObLogCursor& end_cursor)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid argument, log_data=%p, data_len=%ld, start_cursor=%s",
      ...
      while (OB_SUCCESS == err && pos < data_len)
      {
        ...
        {
          YYSYS_LOG(ERROR, "decode switch_log failed(log_data=%p, data_len=%ld, pos=%ld)=>%d", log_data, data_len, tmp_pos, err);
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "end_cursor[%ld].advance(%ld)=>%d", end_cursor.log_id_, log_entry.seq_, err);
          }
        }
      }
      if (OB_SUCCESS == err && pos != data_len)
      {
        ...
        YYSYS_LOG(ERROR, "pos[%ld] != data_len[%ld]", pos, data_len);
      }

      if (OB_SUCCESS != err)
      {
        //hex_dump(log_data, static_cast<int32_t>(data_len), YYSYS_LOG_LEVEL_WARN);
      }
      return err;
    }
...
ObUpsLogMgr::ObUpsLogMgr(): log_buffer_for_get_(LOG_BUFFER_SIZE),log_buffer_for_fetch_(LOG_BUFFER_SIZE), log_buffer_for_replay_(LOG_BUFFER_SIZE)
{
  ...
  log_dir_[0] = '\0';
  is_started_ = false;
  log_term_inited_ = false;
  set_async_load_thread_to_stop();
  is_used_block_num_ = 3;
  replay_log_cache_block_num = 32;
  async_load_log_gap_ = 0;//[377]
  fill_log_max_log_id_ = 0;
  //last_flushed_id_ = 0;
}
...
int ObUpsLogMgr::init(const char* log_dir, const int64_t log_file_max_size,
                      ObLogReplayWorker* replay_worker, ObReplayLogSrc* replay_log_src, ObUpsTableMgr* table_mgr,
                      ObUpsSlaveMgr *slave_mgr, ObUpsRoleMgr *role_mgr,ObBigLogWriter *big_log_writer , int64_t log_sync_type)
{
  int ret = OB_SUCCESS;
  int64_t len = 0;
  ...
  if (is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObUpsLogMgr has been initialized");
    ret = OB_INIT_TWICE;
  }
//  else if (NULL == log_dir || NULL == replay_log_src || NULL == table_mgr
//           || NULL == slave_mgr || NULL == obi_role || NULL == role_mgr)
//  {
//    YYSYS_LOG(ERROR, "Invalid argument[log_dir=%p replay_log_src=%p, table_mgr=%p]"
//              "[slave_mgr=%p obi_role=%p, role_mgr=%p]",
//              log_dir, replay_log_src, table_mgr, slave_mgr, obi_role, role_mgr);
//    ret = OB_INVALID_ARGUMENT;
//  }
  else if (NULL == log_dir || NULL == replay_log_src || NULL == table_mgr
           || NULL == slave_mgr || NULL == role_mgr
            ||NULL== big_log_writer)//add shili [LONG_TRANSACTION_LOG]  20160926
  {
    YYSYS_LOG(ERROR, "Invalid argument[log_dir=%p replay_log_src=%p, table_mgr=%p]"
              "[slave_mgr=%p, role_mgr=%p]",
              log_dir, replay_log_src, table_mgr, slave_mgr, role_mgr);
    ret = OB_INVALID_ARGUMENT;
  }
  else if (0 >= (len = snprintf(log_dir_, sizeof(log_dir_), "%s", log_dir)) || len >= (int64_t)sizeof(log_dir_))
  {
    YYSYS_LOG(ERROR, "Argument is invalid[log_dir_len=%ld log_dir=%s]", len, log_dir);
    ret = OB_INVALID_ARGUMENT;
  }
  else if (OB_SUCCESS != (ret = recent_log_cache_.init(32,25)))//mod lxb [package miss optimizer]
  {
    YYSYS_LOG(ERROR, "recent_log_cache.init()=>%d", ret);
  }
  else if (OB_SUCCESS != (ret = replay_log_cache_.init(replay_log_cache_block_num, 25)))
  {
      YYSYS_LOG(ERROR,"replay_log_cache_.init()=>%d",ret);
  }
  else if (OB_SUCCESS != (ret = pos_log_reader_.init(log_dir)))
  {
    YYSYS_LOG(ERROR, "pos_log_reader.init(log_dir=%s)=>%d", log_dir, ret);
  }
  else if (OB_SUCCESS != (ret = cached_pos_log_reader_.init(&pos_log_reader_, &recent_log_cache_)))
  {
    YYSYS_LOG(ERROR, "cached_pos_log_reader_.init(pos_log_reader=%p, log_buf=%p)=>%d",
              &pos_log_reader_, &recent_log_cache_, ret);
  }
  else if (OB_SUCCESS != (ret = replay_local_log_task_.init(this)))
  {
    YYSYS_LOG(ERROR, "reaplay_local_log_task.init()=>%d", ret);
  }
  //add wangjiahao [Paxos] 20150719 :b
  else if (OB_SUCCESS != (ret = replay_tmp_log_task_.init(this)))
  {
    YYSYS_LOG(ERROR, "reaplay_tmp_log_task.init()=>%d", ret);
  }
  //add :e
  else if (OB_SUCCESS != (ret = replay_point_.init(log_dir_)))
  {
    YYSYS_LOG(ERROR, "replay_point_file.init(log_dir=%s)=>%d", log_dir_, ret);
  }
  //add wangjiahao [Paxos temp_log] 20150629 :b
  //***TODO_wangjiahao modify ups_commitpoint dir to a variable***
  else if (OB_SUCCESS != (ret = commit_point_.init("data/ts_commitlog", "commit_point")))
  {
    YYSYS_LOG(ERROR, "commit_point_file.init(log_dir=%s)=>%d", log_dir_, ret);
  }
  else if (OB_SUCCESS != (ret = disk_region_reader_.init(log_dir)))
  {
    YYSYS_LOG(ERROR, "pos_log_reader.init(log_dir=%s)=>%d", log_dir, ret);
  }
  else if (OB_SUCCESS != (ret = local_reader_.init(&disk_region_reader_, &replay_log_cache_)))
  {
    YYSYS_LOG(ERROR, "local_reader_.init(disk_region_reader_=%p, log_buf=%p)=>%d",
              &disk_region_reader_, &replay_log_cache_, ret);
  }
  else if (true != load_log_switch_.set_init_stat())
  {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR,"load_log_switch_.set_init_stat() failed");
  }
  //add :e
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObLogWriter init failed[ret=%d]", ret);
    }
    else
    {
      ...
      YYSYS_LOG(INFO, "ObUpsLogMgr[this=%p] init succ", this);
    }
  }
...
int ObUpsLogMgr::write_replay_point(uint64_t replay_point)
{
  ...
  {
    YYSYS_LOG(ERROR, "replay_point_file.write(%lu)=>%d", replay_point, ret);
  }
  return ret;
}
                           ...
                           )
{
  ...
  if (NULL == slave_mgr)
  {
    YYSYS_LOG(ERROR, "slave_mgr is NULL");
    ret = OB_ERROR;
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObSlaveMgreadd_server error[ret=%d]", ret);
    }
    else
    {
      if (true) // ���ڵ���־ͬ��������������־��
      ...
      else if (switch_log)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "switch_log_file error[ret=%d]", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "log mgr switch log file succ");
        }
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "fail to set_send_log_point to slave. slave=%s", server.to_cstring());
        }
        else
        {
          YYSYS_LOG(INFO, "slave [%s] send log point [%ld]", server.to_cstring(), new_log_file_id);
        }
      }
    }
  }
...
int ObUpsLogMgr::add_log_replay_event(const int64_t seq, const int64_t mutation_time, ReplayType replay_type, common::LogCommand cmd)
{
  return delay_stat_.add_log_replay_event(seq, mutation_time, master_log_id_, replay_type, cmd);
}

// һ����һ���ļ��Ŀ�ʼ�طţ�ֻ�ܵ���һ�Σ�
// ����û����־����־����������ʱ�طű�����־ʵ����ʲô����������
// �ط���֮��replayed_cursor_������Ȼ��Ч
int ObUpsLogMgr::replay_local_log()
{
  ...
  {
    YYSYS_LOG(WARN, "local log is already replayed: cur_cursor=%ld", replay_worker_->get_replayed_log_id());
    err = OB_ALREADY_DONE; // �Ѿ��طŹ���
  }
  //[706]
  else if (NULL == log_dir_)
  {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "log_dir == NULL");
  }
  else if (OB_SUCCESS != (err = scanner.init(log_dir_)))
  {
      if(OB_DISCONTINUOUS_LOG == err)
      {
          YYSYS_LOG(ERROR, "log_dir_scanner.init(%s): log file not continuous", log_dir_);
      }
      else
      {
          YYSYS_LOG(ERROR, "log_dir_scanner.init(%s)=>%d", log_dir_, err);
      }
  }
  else if (OB_SUCCESS != (err = scanner.get_max_log_id(max_log_file_id))
           && OB_ENTRY_NOT_EXIST != err)
  {
      YYSYS_LOG(ERROR, "get_max_log_file_id error[ret=%d]", err);
  }
  else if (OB_SUCCESS == err && OB_INVALID_ID != log_file_id_by_sst && log_file_id_by_sst > max_log_file_id)
  {
      err = OB_ERROR;
      YYSYS_LOG(ERROR, "replay start log_file_id from sst is large than max log_file_id, local replay failed. sst_file_id=%lu, commitlog_file_id=%lu, err=%d",
                log_file_id_by_sst, max_log_file_id, err);
  }
  ...
  {
    YYSYS_LOG(ERROR, "get_commit_point_func(log_dir=%s)=>%d", log_dir_, err);
  }
  else if (OB_SUCCESS != (err = slave_mgr_->set_acked_clog_id(clog_point)))
  {
    YYSYS_LOG(ERROR, "set_acked_clog_id(clog_point=%ld)=>%d", clog_point, err);
  }
  //add :e
  ...
  {
    YYSYS_LOG(ERROR, "get_replay_point_func(log_dir=%s)=>%d", log_dir_, err);
  }
  YYSYS_LOG(INFO, "get_commit_point(log_id=%ld)", clog_point);
  YYSYS_LOG(INFO, "get_replay_point(file_id=%ld)", start_cursor_.file_id_);
  ...
  //mod :e
  {
    if (OB_CANCELED == err)
    {
      YYSYS_LOG(WARN, "replay_log_func(log_dir=%s, start_cursor=%s): CANCELD", log_dir_, start_cursor_.to_str());
    }
    else
    {
      YYSYS_LOG(ERROR, "replay_log_func(log_dir=%s, start_cursor=%s)=>%d", log_dir_, start_cursor_.to_str(), err);
    }
  }
  else if (end_cursor.log_id_ <= 0
           && OB_SUCCESS != (err = get_local_max_log_cursor_func(log_dir_, get_max_file_id_by_sst(), end_cursor)))
  {
    YYSYS_LOG(ERROR, "get_local_max_log_cursor(log_dir=%s)=>%d", log_dir_, err);
  }
  else if (end_cursor.log_id_ <= 0)
  {
    YYSYS_LOG(WARN, "replayed_cursor.log_id[%ld] <= 0 after replay local log", end_cursor.log_id_);
  }
  else if (OB_SUCCESS != (err = start_log(end_cursor)))
  {
    YYSYS_LOG(ERROR, "start_log(cursor=%s)=>%d", end_cursor.to_str(), err);
  }
  else
  {
    YYSYS_LOG(INFO, "start_log_after_replay_local_log(replay_cursor=%s): OK.", end_cursor.to_str());
  }

  // ��UPS��ѭ���е���start_log_for_master_write()������״̬ΪACTIVE
  // if (OB_SUCCESS != err || !is_master_master())
  // {}
  // else if (OB_SUCCESS != (err = start_log_for_master_write()))
  // {
  //   YYSYS_LOG(ERROR, "start_log_for_master_write()=>%d", err);
  // }
  // else
  // {
  //   set_state_as_active();
...
int ObUpsLogMgr::do_process_unsettled_trans()
{
  return replay_worker_->handle_unsettled_trans();
}
//add 20150701:e
...
int ObUpsLogMgr::replay_tmp_log()
{
            ...
            && OB_ENTRY_NOT_EXIST != err)
    {
     if (OB_CANCELED == err)
     {
       YYSYS_LOG(WARN, "replay_log_func(log_dir=%s): CANCELD", log_dir_);
     }
     else
     {
       YYSYS_LOG(ERROR, "replay_log_func(log_dir=%s)=>%d", log_dir_, err);
     }
    }
    else if (end_cursor.log_id_ <= 0)
    {
      YYSYS_LOG(WARN, "replay_tmp_log.log_id[%ld] <= 0 after replay local log", end_cursor.log_id_);
    }
    else if (OB_SUCCESS != (err = start_log(end_cursor)))
    {
      YYSYS_LOG(ERROR, "start_log(cursor=%s)=>%d", end_cursor.to_str(), err);
    ...
    {
    YYSYS_LOG(WARN, "master write commit point(%ld) fail=>%d", end_cursor.log_id_, err);
    }
    //add:e
    replay_worker_->reset_real_replay_cursor(); //add lxb [paxos bug for replay tmp log]
    return err;
}
//add :e
...
int ObUpsLogMgr::start_log_for_master_write()
{
  ...
  {
    YYSYS_LOG(INFO, "start_log_for_master(replay_cursor=%s): ALREADY STARTED", start_cursor_.to_str());
  }
  else if (!allow_missing_log_file && start_cursor_.file_id_ > 0)
  {
    err = OB_LOG_MISSING;
    YYSYS_LOG(ERROR, "missing log_file[file_id=%ld]", start_cursor_.file_id_);
  }
  else if (OB_SUCCESS != (err = start_log(start_cursor)))
  {
    YYSYS_LOG(ERROR, "start_log()=>%d", err);
  }
  else
  {
    YYSYS_LOG(INFO, "start_log_for_master(replay_cursor=%s): STARTE OK.", start_cursor_.to_str());
  }
  return err;
}
...
bool ObUpsLogMgr::is_master() const
{
  return NULL != role_mgr_ && ObUpsRoleMgr::MASTER == role_mgr_->get_role();
}
//mod 20150701:e
...
//add lxb [paxos bug for replay tmp log]:b
int ObUpsLogMgr::set_real_replay_cursor() const
{
    return replay_worker_->set_real_replay_cursor(tmp_log_cursor_);
}
int ObUpsLogMgr::set_replay_cursor() const
{
    return replay_worker_->set_replay_cursor();
}
int ObUpsLogMgr::reset_real_replay_cursor() const
{
    return replay_worker_->reset_real_replay_cursor();
}
//add:e
...
int ObUpsLogMgr::set_state_as_active()
{
  ...
  //add wangdonghui [ups_replication] 20170816 :b
  else if (role_mgr_->get_state() == ObUpsRoleMgr::FATAL || role_mgr_->get_state() == ObUpsRoleMgr::STOP)
  {
    YYSYS_LOG(ERROR, "can not change state to ACTIVE, current stat=%d", role_mgr_->get_state());
    err = OB_ERROR;
  }
  //add :e
  else if (ObUpsRoleMgr::ACTIVE != role_mgr_->get_state())
  {
    YYSYS_LOG(INFO, "set ups state to be ACTIVE. master_log_id=%ld, log_cursor=%s",
...
//add wangdonghui [ups_replication] 20170816 :b
int ObUpsLogMgr::set_state_as_replaying_log()
{
  int err = OB_SUCCESS;
  ObLogCursor replay_cursor;
  if (NULL == role_mgr_)
  ...
  //add :e
  else if (ObUpsRoleMgr::REPLAYING_LOG != role_mgr_->get_state())
  {
    YYSYS_LOG(INFO, "set ups state to be replaying log. master_log_id=%ld, log_cursor=%s",
              master_log_id_, to_cstring(get_replayed_cursor_(replay_cursor)));
...
int ObUpsLogMgr::write_log_as_slave(const char* buf, const int64_t len)
{
  ...
  {
    YYSYS_LOG(ERROR, "store_log(%p[%ld], sync_to_slave=%s)=>%d", buf, len, STR_BOOL(need_send_log), err);
  }
  return err;
}

//add hxlong [async load log]:b
//find the location of log_id in buff
int ObUpsLogMgr::find_log_from_buf(const char *log_data, int64_t data_len, ObLogCursor &cursor, const int64_t log_id, int64_t &read_count)
{
    ObLogEntry log_entry;
    int64_t pos = 0;
    int err = OB_SUCCESS;
    while(OB_SUCCESS == err && pos < data_len)
    {
        ...
        {
            YYSYS_LOG(ERROR,"cursor.advance()=>%d",err);
        }
        else
        {
            pos += log_entry.get_log_data_len();
...
//xxxxx
int ObUpsLogMgr::switch_from_disk_to_cache()
{
    int err = OB_SUCCESS;
    ObLogCursor replay_cursor;
    replay_worker_->get_replay_cursor(replay_cursor);
    if(!is_can_append_ && replay_log_cache_.is_can_append()
            &&replay_log_cache_.get_use_blocks()<=is_used_block_num_
            &&(flushed_cursor_.log_id_ - replay_cursor.log_id_) < 100 )
    {
        if(load_log_switch_.wait_off())
        {
            set_async_load_thread_to_stop();
        }
        else
        {
            is_stopping_async_load = true;
        }
        YYSYS_LOG(INFO,"will notify the thread that async load log to stop");
    }
    else if(replay_log_cache_.get_use_blocks() > is_used_block_num_
            || (flushed_cursor_.log_id_ - replay_cursor.log_id_) >= 100)
    {
    }
    else
    {
        YYSYS_LOG(ERROR,"async load is doing but is_can_append_ is true");
        err = OB_NEED_RETRY;
    }
    return err;
}

void ObUpsLogMgr::set_load_log_config(const int64_t replay_block_num, const int64_t used_block_num)
{
    if(used_block_num > 0 && used_block_num < replay_block_num)
    {
        replay_log_cache_block_num = replay_block_num;
        is_used_block_num_ = used_block_num;
    }
    else
    {
        YYSYS_LOG(ERROR,"the replay_log_cache_block_num set failed,replay_log_cache_block_num = %ld,used_block_num=%ld",
                  replay_block_num,used_block_num);
    }
}


void ObUpsLogMgr::set_async_load_thread_to_stop()
{
    YYSYS_LOG(INFO,"init or async load log stopped, start direct push to cache");
    is_can_append_ = true;
    is_stopping_async_load = false;
    //last_flushed_id_ = 0;
}
...
//[377]
void ObUpsLogMgr::set_async_load_log_gap(const int64_t async_load_log_gap)
{
    if(async_load_log_gap > 0)
    {
        async_load_log_gap_ = async_load_log_gap;
    }
    else
    {
        YYSYS_LOG(WARN, "the async_load_log_gap set failed, async_load_log_gap = %ld", async_load_log_gap);
    }
}
...
int ObUpsLogMgr::replay_and_write_log(const int64_t start_id, const int64_t end_id, const char* buf, int64_t len)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "replay_and_write_log(buf=%p[%ld])", buf, len);
  }
  else if (0 == len)
  {}
...
//  {
//    YYSYS_LOG(ERROR, "write_log_as_slave(buf=%p[%ld])=>%d", buf, len, err);
//  }
//  else
//  {
...
//  {
//    YYSYS_LOG(WARN, "write_cpoint(master_max_commit_log_id_=%ld)=>", master_max_commit_log_id_);
//  }
//  else if (OB_SUCCESS != (err = replay_worker_->submit_batch(real_end_id, buf, len, RT_APPLY)))
//  {
//    YYSYS_LOG(ERROR, "replay_worker.submit_batch(buf=%p[%ld])=>%d", buf, len, err);
...
int ObUpsLogMgr::set_master_log_id(const int64_t master_log_id)
{
  ...
  {
    YYSYS_LOG(WARN, "master_log_id[%ld] < master_log_id_[%ld]", master_log_id, master_log_id_);
  }
                                   ...
                                   const WAIT_SYNC_TYPE wait_event_type)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "slave_receive_log(buf=%p[%ld]): invalid argument.", buf, len);
  }
  ...
  {
    YYSYS_LOG(ERROR, "parse_log_buffer(log_data=%p[%ld])=>%d", buf, len, err);
  }
  //add chujiajia [Paxos ups_replication] 20160106:b
  else if (end_id < master_max_commit_log_id_)
  {
    err = OB_INVALID_LOG;
    YYSYS_LOG(WARN, "received_log < master_commit_log, it is invalid. end_id=%ld, master_commit_log_id=%ld, err=%d",
        end_id, master_max_commit_log_id_,err);
  }
  else if (cmt_log_id >= 0 && OB_SUCCESS != (err = set_master_max_commit_log_id(cmt_log_id)))
  {
    err = OB_ERROR;
    YYSYS_LOG(WARN, "set_master_max_commit_log_id error! end_id=%ld, err=%d", end_id, err);
  }
  //add:e
  //add wangjiahao [Paxos ups_replication_tmplog] 20150804 :b
  else if (log_term_inited_ && OB_SUCCESS != (err = set_current_log_term(last_log_term)))
  {
    YYSYS_LOG(WARN, "fail to set_current_log_term(%ld)=>%d", last_log_term, err);
  }
  //add :e
  else if (OB_SUCCESS != (err = append_to_log_buffer(&recent_log_cache_, start_id, end_id, buf, len)))
  {
    YYSYS_LOG(ERROR, "append_to_log_buffer(log_id=[%ld,%ld))=>%d", start_id, end_id, err);
  }
  else
  {
  //del wangdonghui [ups_log_replication_optimize] 20161009 :b
    ...
    last_receive_log_time_ = yysys::CTimeUtil::getTime();
    ...
    else if (ObUpsRoleMgr::ACTIVE != role_mgr_->get_state())
    {
      ...
      YYSYS_LOG(WARN, "recv log(log=[%ld,%ld)). But slave state not ACTIVE, do not wait flush until log catch up master", start_id, end_id);
    ...
    else if (WAIT_FLUSH == wait_event_type)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "wangdonghui: log_range [%ld, %ld] slave whole time[%ld], begin:[%ld], end:[%ld]",
                  start_id, end_id, end_time - receive_time, receive_time, end_time);
      }
    }
    else
    {
      YYSYS_LOG(WARN, "unknown wait_event_type=%d", wait_event_type);
    }
    */
    //del :e
  }
...
int ObUpsLogMgr::get_log_for_slave_fetch(ObFetchLogReq& req, ObFetchedLog& result)
{
  ...
  if (NULL == my_buffer)
  {
    ...
    YYSYS_LOG(ERROR, "get_thread_buffer fail.");
  }
  else
  {
    my_buffer->reset();
  ...
  {
    YYSYS_LOG(ERROR, "result.set_buf(buf=%p[%d]): INVALID_ARGUMENT", my_buffer->current(), my_buffer->remain());
  }
  else if (!is_inited())
  {
    err = OB_NOT_INIT;
  ...
  //mod :e
  {
    YYSYS_LOG(DEBUG, "get_log_for_slave_fetch data not server. req.start_id_=%ld, replayed_cursor_=%ld",
  ...
  {
    YYSYS_LOG(ERROR, "cached_pos_log_reader.get_log(log_id=%ld)=>%d", req.start_id_, err);
  }
  if (OB_SUCCESS == err && result.data_len_ & (OB_DIRECT_IO_ALIGN-1))
  {
    ...
    // err = OB_LOG_NOT_ALIGN;
    YYSYS_LOG(WARN, "result.data_len[%ld] is not aligned. req='%s', result='%s'",
        result.data_len_, to_cstring(req), to_cstring(result));
  }
  YYSYS_LOG(DEBUG, "get_log_for_slave_fetch(start_id=%ld)=>%d", req.start_id_, err);
  return err;
}
...
int ObUpsLogMgr::fill_log_cursor(ObLogCursor& log_cursor)
{
  ...
  if (log_cursor.log_id_ == 0 && log_cursor.file_id_ > 0
      ...
      && OB_ENTRY_NOT_EXIST != err)
  {
    YYSYS_LOG(ERROR, "get_first_log_id_func()=>%d", err);
  }
  else if (OB_ENTRY_NOT_EXIST == err)
  {
    err = OB_SUCCESS;
  }
  YYSYS_LOG(TRACE, "fill_log_cursor[for slave](log_cursor=%s)=>%d", log_cursor.to_str(), err);
  return err;
}
...
int ObUpsLogMgr::start_log(const ObLogCursor& start_cursor)
{
  ...
  {
    YYSYS_LOG(ERROR, "start_log(start_cursor=%s)=>%d", start_cursor.to_str(), err);
  }
  else if (OB_SUCCESS != (err = replay_worker_->start_log(start_cursor)))
  {
    YYSYS_LOG(ERROR, "replay_worker_.start_log(start_cursor=%s)=>%d", start_cursor.to_str(), err);
  }
  else
  {
...
int ObUpsLogMgr::start_log_for_replay()
{
  ...
  if (start_cursor_.log_id_ > 0)
  {
    //YYSYS_LOG(INFO, "start_log_for_replay(replayed_cursor=%s): ALREADY STARTED.", replayed_cursor_.to_str() );
  }
  else if (OB_SUCCESS != (err = replay_log_src_->get_remote_log_src().fill_start_cursor(start_cursor_))
      && OB_NEED_RETRY != err)
  {
    YYSYS_LOG(ERROR, "fill_start_cursor(replayed_cursor=%s)=>%d", start_cursor_.to_str(), err);
  }
  ...
  {
    YYSYS_LOG(ERROR, "start_log(start_cursor=%s)=>%d", start_cursor_.to_str(), err);
  }
  else
  {
    YYSYS_LOG(INFO, "start_log_for_replay(replayed_cursor=%s): STARTED OK.", start_cursor_.to_str());
  }
  return err;
}
...
//add hxlong [fixbug change ups master]
int64_t ObUpsLogMgr::get_fill_log_max_log_id()
{
    return fill_log_max_log_id_;
}

//set fill_log_max_log_id after caculate checksum
int ObUpsLogMgr::set_fill_log_max_log_id()
{
   int ret = OB_SUCCESS;
   int64_t fill_log_log_id = get_max_log_id();
   if(fill_log_log_id < fill_log_max_log_id_)
   {
       ret = OB_ERROR;
   }
   else
   {
       fill_log_max_log_id_ = fill_log_log_id;
   }
   return ret;
}

int ObUpsLogMgr::set_fill_log_max_log_id(int64_t log_id)
{
    int ret = OB_SUCCESS;
    int64_t fill_log_log_id = log_id;
    if(fill_log_log_id < fill_log_max_log_id_)
    {
        ret = OB_ERROR;
    }
...
int ObUpsLogMgr::replay_log()
{
  ...
  if (NULL == my_buffer)
  {
    YYSYS_LOG(ERROR, "get thread specific buffer fail");
    err = OB_ALLOCATE_MEMORY_FAILED;
  }
  else
  {
...
*/
  ...
  is_master() &&  //mod lbzhong [Paxos ups_replication] 20160104
  ...
  else if (replay_worker_->get_replayed_log_id() < 0)
  {
    err = OB_ERR_UNEXPECTED;
           ...
           && OB_NEED_RETRY != err)
  {
    YYSYS_LOG(ERROR, "start_log_for_replay()=>%d", err);
  }
  else if (OB_NEED_RETRY == err)
  {
    err = OB_NEED_WAIT;
...
/*
  else if (OB_SUCCESS != (err = replay_worker_->get_replay_cursor(replay_cursor)))
  {
    YYSYS_LOG(ERROR, "get_replay_cursor()=>%d", err);
           ...
           && OB_DATA_NOT_SERVE != err)
  {
    YYSYS_LOG(ERROR, "get_log(replayed_cursor=%s)=>%d", replay_cursor.to_str(), err);
  }
  ...
  {
    YYSYS_LOG(ERROR, "set_state_as_active()=>%d", err);
  }
  else if (0 == read_count)  {
      err = OB_NEED_RETRY;
    //YYSYS_LOG(INFO, "master_max_commit_log_id_ =%ld, replay_worker_->get_next_submit_log_id()=%ld, flushed_cursor_:%s replay_worker_->get_next_submit_log_id():%ld", master_max_commit_log_id_, replay_worker_->get_next_submit_log_id(), flushed_cursor_.to_str(), replay_worker_->get_next_submit_log_id());
  }


  else if (OB_SUCCESS != (err = replay_and_write_log(replay_worker_->get_replayed_log_id(), end_id, buf, read_count)))
  {
    YYSYS_LOG(ERROR, "replay_and_write_log(buf=%p[%ld])=>%d", buf, read_count, err);
  }

  if (OB_SUCCESS != err && OB_NEED_RETRY != err && OB_NEED_WAIT != err && OB_CANCELED != err)
  {
    role_mgr_->set_state(ObUpsRoleMgr::FATAL);
    YYSYS_LOG(ERROR, "replay failed, ret=%d, will kill self", err);
    kill(getpid(), SIGTERM);
  }
  */
  ...
  //mod:e
  {
    ...
    //TODO_wangjiahao if write error many times
           ...
           && OB_DATA_NOT_SERVE != err)
  {
    YYSYS_LOG(ERROR, "get_log(replayed_cursor=%s)=>%d, flushed_cursor_=%s", replay_cursor.to_str(), err, flushed_cursor_.to_str());
  }
  else if (OB_DATA_NOT_SERVE == err)
  {
    err = OB_NEED_WAIT;
  ...
  {
    YYSYS_LOG(ERROR, "slave write log failed. err = %d buf_len=%ld", err, len);
  }
  else
  {
  ...
  */
  ...
  else  // not on disk
  {
      ...
      else if(is_can_append_ &&
              OB_SUCCESS != (err = append_to_log_buffer_2(&replay_log_cache_,flushed_cursor_.log_id_,end_id,buf,read_count,is_can_append_))
              && err != OB_EAGAIN)
      {
          YYSYS_LOG(ERROR,"slave append_to_log_buffer_2 failed. err=%d read_count=%ld",err,read_count);
      }
      else
      {
        flushed_cursor_ = en_cursor;
        tmp_log_cursor_ = en_cursor;
        replay_worker_->set_next_flush_log_id(flushed_cursor_.log_id_);
        ObUpdateServerMain::get_instance()->get_update_server().get_slave_mgr().set_my_log_seq(en_cursor.log_id_);
      }

      if(!is_can_append_)
      {
          err = OB_SUCCESS;
          if(!is_stopping_async_load && !load_log_switch_.is_on())
          {
              YYSYS_LOG(INFO,"can not append to replay_log_cache, start async load thread");
          }
          if(is_stopping_async_load)
          {
              if(load_log_switch_.wait_off())
              {
                  set_async_load_thread_to_stop();
          ...
          else if(load_log_switch_.wait_on())
          {
              switch_from_disk_to_cache();
          }
          else
          {
              YYSYS_LOG(ERROR,"failed to begin async load.");
          }
      }
      if(is_can_append_ && load_log_switch_.is_on())
      {
          YYSYS_LOG(ERROR,"is_can_append is true, but async load is still running");
      }
  }
...
int ObUpsLogMgr::replay_log_to_queue()
{
    int err = OB_SUCCESS;
    int64_t end_id = 0;
    ThreadSpecificBuffer::Buffer *my_buffer2 = log_buffer_for_replay_.get_buffer();
    ObLogCursor replay_cursor;
    char *buf_for_reply = NULL;
    int64_t len_for_reply = 0;
    int64_t read_count = 0;
    ...
    else if(ObUpsRoleMgr::FATAL == role_mgr_->get_state())
    {
        ...
        {
            YYSYS_LOG(WARN,"updateserver role is fatal, err[%d]",err);
        }
    }
    else if(!is_log_replay_finished())
    {
        ...
        if(REACH_TIME_INTERVAL(50*1000*1000))
        {
            YYSYS_LOG(WARN,"log replay has not finished, err[%d]",err);
        }
    }
    else if(is_master() && is_tmp_log_replay_started())
    {
        err = OB_NEED_RETRY;
        if(REACH_TIME_INTERVAL(50*1000*1000))
        {
            YYSYS_LOG(WARN,"tmp log replay of master ups has already started,err[%d]",err);
        }
    }
    else if(OB_SUCCESS != (err = replay_worker_->get_replay_cursor(replay_cursor)))
    {
    ...
    {
        YYSYS_LOG(ERROR,"local_reader_.get_log() failed.err=%d,pos=%ld",err,pos);
    }
    else if(OB_SUCCESS != (err = replay_and_write_log(replay_worker_->get_replayed_log_id(),end_id,buf_for_reply,len_for_reply)))
    {
        YYSYS_LOG(ERROR,"replay_and_write_log(buf=%p[%ld])=>%d",buf_for_reply,read_count,err);
    }
    if(OB_SUCCESS != err && OB_NEED_RETRY !=err && OB_NEED_WAIT != err && OB_CANCELED != err)
    {
        role_mgr_->set_state(ObUpsRoleMgr::FATAL);
        YYSYS_LOG(WARN, "replay occur error, get_log_start_id[%ld], replayed_log_id[%ld], end_id[%ld]=min(max_commit_log_id[%ld], max_flushed_log_id[%ld]), ret=%d",
                  replay_cursor.log_id_, replay_worker_->get_replayed_log_id(), end_id, master_max_commit_log_id_, flushed_cursor_.log_id_, err);
        YYSYS_LOG(ERROR,"replay failed, ret=%d, will kill self",err);
        kill(getpid(),SIGTERM);
    }
    return err;
...
int ObUpsLogMgr::load_log_from_disk(bool &is_append)
{
    int err = OB_SUCCESS;
    int64_t start_id = 0;
    int64_t end_id = 0;
    ...
    ObLogCursor replay_cursor;
    char *buf_for_load = NULL;
    int64_t len_for_load = 0;
    int64_t read_count = 0;
    ...
    {
        my_buffer->reset();
        buf_for_load = my_buffer->current();
        len_for_load = my_buffer->remain();
    }
    ...
    else if(replay_log_cache_.is_can_append() && !load_log_switch_.check_off(true))//buff is not full and thread is not closed
    {
        //some import comment

        start_id = replay_log_cache_.get_end_id();
        end_id = min(master_max_commit_log_id_, flushed_cursor_.log_id_);

        //last_flushed_id_ = flushed_cursor_.log_id_;


        YYSYS_LOG(INFO,"in async load thread, [start_id,end_id] = [%ld,%ld]."
                  "cache_log_id=%ld, replay_cursor_log_id=%ld, master_max_commit_log_id=%ld, flushed_id=%ld, %s",
                  start_id,end_id,replay_log_cache_.get_end_id(),replay_cursor.log_id_, master_max_commit_log_id_, flushed_cursor_.log_id_, to_cstring(replay_log_cache_));
        ...
        if(start_id >= end_id)
        {
            ...
            if(REACH_TIME_INTERVAL(50*1000*1000))
            {
                YYSYS_LOG(WARN,"start_id[%ld] is larger than end_id[%ld], err[%d]",start_id,end_id,err);
            }
        }
        //[377]
        else if (end_id - start_id <= async_load_log_gap_)
        {
            err = OB_NEED_WAIT;
            if(REACH_TIME_INTERVAL(50*1000*1000))
            {
                YYSYS_LOG(WARN,"end_id[%ld] - start_id[%ld] should need less than or equal to %ld, err[%d]", end_id, start_id, async_load_log_gap_, err);
            }
        }
        else if(OB_SUCCESS != (err = pos_log_reader_.get_log(start_id, start_location_, end_location_,
                                                             buf_for_load,len_for_load,read_count)))//read log from disk
        {
            YYSYS_LOG(ERROR,"reader.get_log(start_id=%ld,real_end_id=%ld,read_count=%ld)=>%d",
                      start_location_.log_id_, end_location_.log_id_, read_count, err);
        }
        else if(read_count <= 0)
        {
            ...
            if(REACH_TIME_INTERVAL(50*1000*1000))
            {
                YYSYS_LOG(WARN,"log read_count[%ld] < 0, err[%d]",read_count,err);
            }
        }
        else
        {
            if(end_location_.log_id_ > end_id && start_location_.log_id_ < end_id)
            {
                ObLogCursor tmp_cursor;
                tmp_cursor.file_id_ = start_location_.file_id_;
                tmp_cursor.log_id_ = start_location_.log_id_;
                tmp_cursor.offset_ = start_location_.offset_;
                int err1 = OB_SUCCESS;
                int64_t count = 0;
                if(OB_SUCCESS != (err1 = find_log_from_buf(buf_for_load, len_for_load, tmp_cursor, end_id, count)))
                {
                    YYSYS_LOG(ERROR,"failed to find log from buf(log_id=%ld)=>%d",end_id,err1);
                }
                else if(tmp_cursor.log_id_ == end_id)
                {
                    read_count = count;
                    end_location_.file_id_ = tmp_cursor.file_id_;
                    end_location_.log_id_ = tmp_cursor.log_id_;
                    end_location_.offset_ = tmp_cursor.offset_;
                }
                else
                {
                    YYSYS_LOG(ERROR,"WTF(log_id=%ld)=>%d",end_id, err);
                }
            }
            if(start_location_.log_id_ >= end_location_.log_id_)
            {
                YYSYS_LOG(ERROR,"async load log from disk failed.[start_id,end_id] = [%ld,%ld] end_id = %ld",
                          start_location_.log_id_,end_location_.log_id_,end_id);
            }
            else if(OB_SUCCESS != (err = append_to_log_buffer_2(&replay_log_cache_,start_location_.log_id_,end_location_.log_id_,buf_for_load,read_count,is_append)))
            {
                YYSYS_LOG(ERROR,"slave append_to_log_buffer_2 failed. err=%d buf_len=%ld",err,read_count);
            }
            else
            {
//                if(REACH_TIME_INTERVAL(1000*1000))
//                {
//                    YYSYS_LOG(DEBUG,"[start_id,end_id] = [%ld,%ld] %s",
//                              start_location_.log_id_,end_location_.log_id_,to_cstring(replay_log_cache_));
//                }
                YYSYS_LOG(INFO, "in async load thread, end print [start_id,end_id] = [%ld,%ld] %s",
                          start_location_.log_id_, end_location_.log_id_, to_cstring(replay_log_cache_));
                start_location_ = end_location_;
            }
        }
...
int ObUpsLogMgr::reset_switch()
{
    int ret = OB_SUCCESS;
    if(true != load_log_switch_.set_init_stat())
    {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR,"reset load log switch stat failed");
    }
    return ret;
}
...
int  ObUpsLogMgr::get_max_log_seq_in_file(int64_t& log_seq) const
{
  ...
  else if (is_log_replay_finished())
  {
    ...
    //mod :e
      log_seq = std::max(local_max_log_id_when_start_, tmp_log_cursor_.log_id_);
  }
  ...
  else
  {
    ...
    {
      YYSYS_LOG(INFO, "local log_dir has no log or log is not continuous.");
    }
  }
  return err;
}
...
int ObUpsLogMgr::get_max_log_seq_replayable(int64_t& max_log_seq, int32_t sync_mode) const  //= log_mgr_.tmp_cursor_.log_id
{
  ...
  else if (ObUpsLogMgr::WAIT_FLUSH == sync_mode)
  {
    ...
    {
      max_log_seq = max_log_seq_in_file;
    }
  }
  ...
  /*else*/if (OB_SUCCESS != (err = get_max_log_seq_in_file(max_log_seq_in_file)))
    ...
    {
      YYSYS_LOG(ERROR, "get_max_log_seq_in_buffer()=>%d", err);
    }
    else
    {
                                ...
                                const char* log_data, const int64_t data_len)
{
  ...
  if (is_master)
  {
    ...
    {
      YYSYS_LOG(ERROR, "append_to_log_buffer([%s,%s], data=%p[%ld])=>%d",
    ...
    {
      YYSYS_LOG(ERROR, "update_replay_cursor(log_cursor=%s)=>%d", end_cursor.to_str(), err);
    }*/
    //del :e
    else
    {
...
int ObUpsLogMgr::set_master_max_commit_log_id(const int64_t commit_id)
{
    ...
    {
        YYSYS_LOG(WARN, "slave set_master_max_commit_log_id waiting for __sync");
    }
...
int ObUpsLogMgr::write_cpoint(const int64_t cmt_point, bool is_master)
{
    ...
    if (cmt_point > 0)
    {
        ...
        current_max_commit_log_id_ = cmt_point;//add huangpeng for cptop
...
int ObUpsLogMgr::set_current_log_term(const int64_t term)
{
    ...
    {
        YYSYS_LOG(WARN, "set new log_term(%ld) waiting for __sync", term);
    }
    if (x > term)
    {
        if(REACH_TIME_INTERVAL(10*1000*1000))
        {
              YYSYS_LOG(ERROR, "set new log_term(%ld) < current log_term(%ld)", term, *p);
        }
        err = OB_ERR_TERM;
    }
    else if (x < term)
    {
      YYSYS_LOG(INFO, "new log_term setted. term=%ld", term);
    }
    return err;
}
...
int ObUpsLogMgr::restore_log_term_from_log(const int64_t log_id)
{
  ...
  if (log_id <= 0)
  {
    ...
    YYSYS_LOG(WARN, "local_max_log_id_when_start_ is not valid. set log_term_ as default term = 0");
  }
  else if (OB_SUCCESS != (err = local_reader_.get_log(log_id - 1, log_id, buf, read_count, len, pos)))
  {
    YYSYS_LOG(ERROR, "local_reader get log failed. err = %d pos=%ld", err, pos);
  }
  else if (OB_SUCCESS != (err = entry.deserialize(buf, read_count, pos1)))
  {
    YYSYS_LOG(ERROR, "log_entry.deserialize(log_data=%p, data_len=%ld, pos=%ld)=>%d", buf, read_count, pos1, err);
  }
  else
  {
    //mod chujiajia [Paxos ups_replication] 20160106:b
    //set_current_log_term(entry.header_.get_reserved());
    //YYSYS_LOG(INFO, "log_term get from max_log_id(%ld) succ. term = %ld", log_id, entry.header_.get_reserved());
    if (entry.header_.get_reserved() < get_current_log_term())
    {
      err = OB_INVALID_TERM;
      YYSYS_LOG(WARN, "log_term from max_log_id is invalid, err=%d. entry.header_.get_reserved(%ld) < get_current_log_term(%ld)",
                err, entry.header_.get_reserved(), get_current_log_term());
    }
    else
    {
      set_current_log_term(entry.header_.get_reserved());
      YYSYS_LOG(INFO, "restore_log_term_from_log()succ. max_log_id(%ld). term(%ld)", log_id, entry.header_.get_reserved());
    }
    //mod:e
  }
  return err;
...
//    {
//      YYSYS_LOG(ERROR, "scanner.init(log_dir=%s)=>%d", log_dir_, ret);
//    }
    else if (OB_SUCCESS !=(ret = scanner.init(log_dir_)))
    {
        if(OB_DISCONTINUOUS_LOG == ret)
        {
            YYSYS_LOG(WARN,"log_dir_scanner.init(%s):log file not continuous", log_dir_);
        }
        else
        {
            YYSYS_LOG(ERROR,"log_dir_scanner.init(%s)=>%d", log_dir_, ret);
        }
    }
    else if (OB_SUCCESS != (ret = scanner.get_max_log_id((uint64_t&)max_log_file_id)) && OB_ENTRY_NOT_EXIST != ret)
    {
      YYSYS_LOG(ERROR, "get_max_log_file_id error[ret=%d]", ret);
    }
    else
    {
      ret = OB_SUCCESS;
    ...
    {
      YYSYS_LOG(ERROR, "get_commit_point_func(log_dir=%s)=>%d", log_dir_, ret);
    }
    ...
    while(OB_SUCCESS == ret)
    {
        if(OB_SUCCESS != (ret = get_replay_cursor_from_file(log_dir_, start_cursor, clog_point, cursor))
                && OB_EAGAIN != ret)
        {
            YYSYS_LOG(WARN, "get_replay_cursor_from_file=>%d", ret);
        }
        else if(OB_EAGAIN == ret)
        {
            YYSYS_LOG(WARN, "replay cursor doesn't exist in file[%ld], read prior", start_cursor.file_id_);
        ...
        {
            ret = OB_INVALID_ARGUMENT;
            YYSYS_LOG(ERROR, "get_replay_cursor_for_disk fail, may be deleted by accident!");
        }
    }
    //add :e
    return ret;
