    ...
    class ObUpsRoleMgr
    {
      ...
      public:
        ObUpsRoleMgr()
        {
          ...
          lock_  = false; //add hongchen [Paxos set ups state lock] 20170920
        }
        ...
        inline void set_role(const Role role)
        {
          ...
          YYSYS_LOG(INFO, "set_role=%s state=%s", get_role_str(), get_state_str());
        }
        ...
        {
          //lock
		  //add hongchen [Paxos set ups state lock] 20170920:b
...
//add support for arm platform by wangd 202106:e
          ...
          if ((FATAL == state || STOP == state)
               || ((REPLAYING_LOG == state_ || ACTIVE == state_)
                   && (REPLAYING_LOG == state || ACTIVE == state)))
          {
          ...
          if (FATAL == state)
          {
            ...
            YYSYS_LOG(ERROR, "stop client_mgr, enter FATAL state.");
          }
          // else if (STOP == state)
          // {
          //   set_client_mgr_err(OB_IN_STOP_STATE);
          //   YYSYS_LOG(WARN, "stop client_mgr, enter STOP state.");
          // }
          YYSYS_LOG(INFO, "set_state=%s role=%s", get_state_str(), get_role_str());
          lock_ = false; //add hongchen [Paxos set ups state lock] 20170920
        }
        ...
        int serialize(char* buf, const int64_t len, int64_t& pos) const
        {
          ...
          {
            YYSYS_LOG(ERROR, "ups_role_mgr.serialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, pos, err);
          }
          return err;
        }
        int deserialize(const char* buf, const int64_t len, int64_t& pos)
        {
          ...
          {
            YYSYS_LOG(ERROR, "ups_role_mgr.deserialize(buf=%p[%ld], pos=%ld)=>%d", buf, len, pos, err);
          }
          return err;
        }
      ...
      private:
        Role  role_;
        State state_;
        bool  lock_; //add hongchen [Paxos set ups state lock] 20170920
    };
