...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    void ObClogStat::add_disk_us(int64_t start_id, int64_t end_id, int64_t time)
    {
      ...
      //{
      //  YYSYS_LOG(WARN, "flush_log: disk_delay=%ld, warn_us=%ld", time, disk_warn_us_);
      //}
      if (disk_warn_stat_.warn(time))
      {
        YYSYS_LOG(WARN, "flush_log: disk_delay=%ld %s", time, to_cstring(disk_warn_stat_));
      }
    }
    ...
    void ObClogStat::add_net_us(int64_t start_id, int64_t end_id, int64_t time)
    {
      ...
      //{
      //  YYSYS_LOG(WARN, "flush_log: net_delay=%ld, warn_us=%ld", time, net_warn_us_);
      //}
      if (net_warn_stat_.warn(time))
      {
        YYSYS_LOG(WARN, "flush_log: net_delay=%ld %s", time, to_cstring(net_warn_stat_));
      }
    }
    ...
    int ObClogStat::deserialize(const char* buf, int64_t len, int64_t& pos)
    {
      ...
      else if (version != STRUCT_VERSION)
      {
        ...
        YYSYS_LOG(ERROR, "version[%ld] != VERSION[%ld]", version, STRUCT_VERSION);
      }
