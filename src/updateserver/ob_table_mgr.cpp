  ...
  namespace updateserver
  {
    ...
    struct EmptyMemTableEntity
    {
      ...
      EmptyMemTableEntity(): memtable_entity_(table_item_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "init empty memtable_entity, err=%d", err);
        }
        else
        {
    ...
    //add lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
    void MemTableEntityIterator::set_version(int64_t version)
    {
      version_ = version;
    }

    int64_t MemTableEntityIterator::get_version(void)
    {
      return version_;
    }
    //add 20151228:e
    ...
    MemTableIterator &MemTableEntityIterator::get_memtable_iter()
    {
      ...
      rc_.set_frozen_schema(NULL); //add wangyao [403]
    ...
    ITableUtils *MemTableEntity::get_tsi_tableutils(const int64_t index)
    {
      ...
      {
        YYSYS_LOG(WARN, "get tsi table utils fail table_utils_set=%p index=%ld", table_utils_set, index);
      }
      return ret;
    }
                            ...
                            ITableIterator *iter)
    {
      ...
      ObReadAtomicDataMark *data_mark_ptr = NULL;
      if (NULL != column_filter
          && column_filter->get_read_atomic_param().is_valid())
      {
        data_mark_ptr = &data_mark;
      }
      //add duyr 20151207:e
      ...
      if (NULL == sub_iter)
      {
        YYSYS_LOG(WARN, "invalid param sub_iter=%p", sub_iter);
        ret = OB_INVALID_ARGUMENT;
      }
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
      else if (NULL != data_mark_ptr
               && OB_SUCCESS != (ret = fill_local_data_mark(*data_mark_ptr)))
      {
        YYSYS_LOG(WARN,"fail to set data mark!ret=%d",ret);
      }
      //add duyr 20151207:e
      else
      {
                            ...
                            ITableIterator *iter)
    {
      ...
      if (scan_param.get_read_atomic_param().is_valid())
      {
        data_mark_ptr = &data_mark;
      }
      //add duyr 20151207:e
      ...
      if (NULL == sub_iter
          || NULL == scan_range)
      {
        YYSYS_LOG(WARN, "invalid param sub_iter=%p scan_range=%p", sub_iter, scan_range);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        {
            query_version = scan_param.get_version_range().get_query_version();
          ret = memtable_.scan(session_ctx, *scan_range, reverse, sub_iter->get_memtable_iter(), column_filter
                               //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
                               ,data_mark_ptr
                               //add duyr 20151207:e
    ...
    void MemTableEntity::deref()
    {
      if (0 == memtable_.dec_ref_cnt())
      {
        ...
        memtable_.destroy();
        YYSYS_LOG(INFO, "clear memtable succ %s", sst_id.log_str());
      }
    }
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
    /*@berif ���� ��memtable ��data_mark
    * @param local_data_mark [out] */
    int MemTableEntity::fill_local_data_mark(ObReadAtomicDataMark &local_data_mark)
    {
      int ret = OB_SUCCESS;
      uint64_t cur_data_version = 0;
      ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
      local_data_mark.reset();
      if (NULL == ups_main)
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"table entity or ups_main is NULL!ups_main=%p,ret=%d",ups_main,ret);
      }
      else
      {
        local_data_mark.ups_paxos_id_ = ups_main->get_update_server().get_param().paxos_id;
        local_data_mark.data_store_type_ = common::ObReadAtomicDataMark::UPS_MEMTABLE_DATA;
        cur_data_version = get_memtable().get_version();
        local_data_mark.major_version_       = SSTableID::get_major_version(cur_data_version);
        local_data_mark.minor_version_start_ = SSTableID::get_minor_version_start(cur_data_version);
        local_data_mark.minor_version_end_   = SSTableID::get_minor_version_end(cur_data_version);
        if (!local_data_mark.is_valid())
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR,"fail to fill local data mark!mark=[%s],ret=%d",
                    to_cstring(local_data_mark),ret);
        }
      }
      YYSYS_LOG(DEBUG,"read_atomic::debug,finish get mem table data mark!"
                "cur_data_mark=[%s],ret=%d",
                to_cstring(local_data_mark),ret);
      return ret;
    }
                                                     ...
                                                     iter_counter_(0)
    {
      ...
      version_ = OB_INVALID_VERSION;
      //add 20151228:e
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160423:b
      has_store_real_data_state_     = false;
      is_outputing_read_atomic_cell_ = false;
      old_real_ci_ptr_      = NULL;
      read_atomic_cell_.value_.set_ext(ObActionFlag::OP_NOP);
      data_mark_.reset();
      output_paxos_id_       = 0;
      output_major_ver_      = 0;
      output_minor_ver_start_= 0;
      output_minor_ver_end_  = 0;
      output_data_store_type_= 0;
      read_atomic_param_.reset();
      is_strategy_init_      = false;
      strategy_state_        = NO_READ_STRATEGY;
    ...
    int SSTableEntityIterator::is_row_finished(bool *is_row_finished)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable_iter=%p", sstable_iter_);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = sstable_iter_->is_row_finished(is_row_finished)))
      {
        YYSYS_LOG(WARN, "get is_row_finished from sstable_iter fail ret=%d", ret);
      }
      else
      {
        // do nothing
    ...
    int SSTableEntityIterator::next_cell()
    {
      ...
      if (NULL == sstable_iter_
          || NULL == column_filter_)
      {
        YYSYS_LOG(WARN, "invalid sstable_iter=%p column_filter=%p", sstable_iter_, column_filter_);
        ret = OB_ERROR;
      }
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160423:b
      else if (!is_strategy_init_
               && read_atomic_param_.need_exact_data_mark_data_
               && read_atomic_param_.is_valid()
               && OB_SUCCESS != (ret = common::ObReadAtomicHelper::get_strategy_with_data_mark(read_atomic_param_,
                                                                                               data_mark_,
                                                                                               atomic_read_strategy_)))
      {
        YYSYS_LOG(WARN,"fail to get read atomic strategy!ret=%d,atomic_read_strategy_=%d",
                  ret,atomic_read_strategy_);
      }
      else if (!is_strategy_init_
               && read_atomic_param_.need_exact_data_mark_data_
               && read_atomic_param_.is_valid()
               && !(is_strategy_init_ = (common::ObReadAtomicParam::NO_READ == atomic_read_strategy_
                                        || common::ObReadAtomicParam::READ_WITH_COMMIT_DATA == atomic_read_strategy_
                                        || common::ObReadAtomicParam::READ_WITH_READ_ATOMIC_PARAM == atomic_read_strategy_)))
      {
        ret = OB_NOT_SUPPORTED;
        YYSYS_LOG(ERROR,"unknow strategy[%d],ret=%d",atomic_read_strategy_,ret);
      }
      else if (is_strategy_init_
               && ITER_MUST_END == strategy_state_)
      {
        ret = OB_ITER_END;
        YYSYS_LOG(INFO,"read_atomic::debug,sst iter won't output any data!"
                  "ret=%d,strategy_state_=%d",
                  ret,strategy_state_);
      }
      else if (is_outputing_read_atomic_cell_)
      {
        if (OB_SUCCESS != (ret = next_read_atomic_cell()))
        {
          YYSYS_LOG(WARN,"fail to get next read atomic cell!ret=%d",ret);
        }
      }
      //add duyr 20160423:e
      else
      {
        while (OB_SUCCESS == (ret = sstable_iter_->next_cell()))
        {
          ...
          // sstable������column_idΪOB_INVALID_ID����ת����0
          // ������������Ҫ���޸ĳ�OB_INVALID_ID
        ...
        //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160423:b
        if (NULL != cur_ci_ptr_ && read_atomic_param_.is_valid())
        YYSYS_LOG(DEBUG,"read_atomic::debug,get one real data cell!"
                  "ret=%d,is_row_changed_=%d,has_atomic_cell=%d,"
                  "tid=%ld,cid=%ld,key=[%s],val=[%s]",
                  ret,is_row_changed_,is_has_read_atomic_cell(),
                  cur_ci_ptr_->table_id_,cur_ci_ptr_->column_id_,
                  to_cstring(cur_ci_ptr_->row_key_),
                  to_cstring(cur_ci_ptr_->value_));

        if (OB_SUCCESS == ret
            && read_atomic_param_.is_valid())
        {
          //1:solve exact data mark data
          if (OB_SUCCESS == ret
              && read_atomic_param_.need_exact_data_mark_data_)
          {
            if (!is_strategy_init_ )
            {
              ret = OB_NOT_INIT;
              YYSYS_LOG(ERROR,"must first get the atomic read strategy!ret=%d",ret);
            }
            else if (common::ObReadAtomicParam::NO_READ == atomic_read_strategy_)
            {
              read_atomic_param_.reset();
              if (ITER_MUST_END == strategy_state_
                  || NULL == cur_ci_ptr_)
              {
                ret = OB_ITER_END;
              }
              ...
              {
                if (NULL != dynamic_cast<sstable::ObSSTableGetter *>(sstable_iter_))
                {//means use get!
                  strategy_state_ = ITER_LAST_CELL;
                  read_atomic_cell_.table_id_ = cur_ci_ptr_->table_id_;
                  read_atomic_cell_.row_key_  = cur_ci_ptr_->row_key_;
                  read_atomic_cell_.column_id_= OB_INVALID_ID;
                  read_atomic_cell_.value_.set_ext(ObActionFlag::OP_ROW_DOES_NOT_EXIST);
                  cur_ci_ptr_ = &read_atomic_cell_;
                }
                else
                {
                  strategy_state_ = ITER_MUST_END;
                  ret = OB_ITER_END;
                }
              }
              YYSYS_LOG(DEBUG,"read_atomic::debug,sst iter won't output any data!"
                        "ret=%d,strategy_state_=%d,cur_ci_ptr_=%p",
                        ret,strategy_state_,cur_ci_ptr_);
            }
            else if (common::ObReadAtomicParam::READ_WITH_COMMIT_DATA == atomic_read_strategy_)
            {
//              read_atomic_param_.reset();
              //TODO:READ_ATOMIC
              //need finish filter commit data!!!
              YYSYS_LOG(INFO,"read_atomic::debug,sst iter will read just commit data!");
            }
            else if (common::ObReadAtomicParam::READ_WITH_READ_ATOMIC_PARAM == atomic_read_strategy_)
            {
              //do nothing!
            }
          }

          //2:output read atomic data!
          //befor output one new row,
          //first output the read atomic cells of this row!
          if (OB_SUCCESS == ret
              && is_row_changed_
              && NULL != cur_ci_ptr_
              && read_atomic_param_.is_valid()
              && (ObExtendType != cur_ci_ptr_->value_.get_type()
                 || ObActionFlag::OP_ROW_DOES_NOT_EXIST != cur_ci_ptr_->value_.get_ext()))
          {//try output the read atomic cell!
            reset_read_atomic_cell();
            if (is_has_read_atomic_cell())
            {
              has_store_real_data_state_ = true;
              old_real_ci_ptr_      = cur_ci_ptr_;

              read_atomic_cell_.table_id_ = cur_ci_ptr_->table_id_;
              read_atomic_cell_.row_key_  = cur_ci_ptr_->row_key_;
              read_atomic_cell_.column_id_= OB_INVALID_ID;
              YYSYS_LOG(DEBUG,"read_atomic::debug,begin output read atomic cell!"
                        "old_real_ci_ptr=%p,iter_counter=%ld",
                        old_real_ci_ptr_,iter_counter_);
              if (OB_SUCCESS != (ret = next_read_atomic_cell(true)))
              {
                YYSYS_LOG(WARN,"fail to get next read atomic cell!ret=%d",ret);
              }
            }
      ...
      if (read_atomic_param_.is_valid() || is_strategy_init_)
      {
        YYSYS_LOG(DEBUG,"read_atomic::debug,finish next sst cell!"
                  "ret=%d,iter_counter=%ld,cur_ci_ptr=%p,is_row_change=%d,"
                  "is_strategy_init_=%d,strategy_state_=%d,atomic_read_strategy_=%d",
                  ret,iter_counter_,cur_ci_ptr_,is_row_changed_,
                  is_strategy_init_,strategy_state_,atomic_read_strategy_);
        if (NULL != cur_ci_ptr_)
          YYSYS_LOG(DEBUG,"read_atomic::debug,sst_tid=%ld,sst_cid=%ld,sst_key=[%s],sst_val=[%s]",
                    cur_ci_ptr_->table_id_,
                    cur_ci_ptr_->column_id_,
                    to_cstring(cur_ci_ptr_->row_key_),
                    to_cstring(cur_ci_ptr_->value_));
      }
      return ret;
    ...
    //            else
    //            {
    //              // �����ʾ�µ�һ�п�ʼ ������е�rowkey ����row_has_expected_column_��Ϊfalse
    ...
    //              {
    //                YYSYS_LOG(WARN, "write row_key fail ret=%d cell_info=[%s]", ret, print_cellinfo(cur_ci_ptr_));
    //                break;
    //              }
    ...
    //    }
    //  }
    //  YYSYS_LOG(DEBUG, "this=%p ret=%d", this, ret);
    //  return ret;
    //}
    ...
    int SSTableEntityIterator::get_cell(ObCellInfo **cell_info, bool *is_row_changed)
    {
      ...
      {
        ret = OB_ITER_END;
        YYSYS_LOG(INFO,"read_atomic::debug,sst iter won't read any data!ret=%d",ret);
      }
      //add duyr 20160423:e
      else
      {
        *cell_info = cur_ci_ptr_;
        ...
        bool irf = false;
        is_row_finished(&irf);
        YYSYS_LOG(DEBUG, "[SSTABLE_ITER] this=%p %s is_row_changed=%s is_row_finished=%s", this, print_cellinfo(*cell_info), STR_BOOL(is_row_changed_), STR_BOOL(irf));
      }
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160423:b
      if (OB_SUCCESS == ret
          && is_strategy_init_
          && ITER_LAST_CELL == strategy_state_)
      {//already output the last cell!
        strategy_state_ = ITER_MUST_END;
        YYSYS_LOG(DEBUG,"read_atomic::debug,alread output the last cell!strategy_state_=%d",strategy_state_);
      }

      if (read_atomic_param_.is_valid() || is_strategy_init_)
        YYSYS_LOG(DEBUG,"read_atomic::debug,finish get sstable cell!"
                  "ret=%d,is_row_changed=%d,iter_counter=%ld,"
                  "is_strategy_init_=%d,strategy_state_=%d,atomic_read_strategy_=%d",
                  ret,is_row_changed_,iter_counter_,
                  is_strategy_init_,strategy_state_,atomic_read_strategy_);
      if ((read_atomic_param_.is_valid() || is_strategy_init_) && NULL != cell_info)
        YYSYS_LOG(DEBUG,"read_atomic::debug,sst_tid=%ld,sst_cid=%ld,sst_key=[%s],sst_val=[%s]",
                  (*cell_info)->table_id_,
    ...
    //add lijianqiang [MultiUPS] [SELECT_MERGE] 20151228:b
    void SSTableEntityIterator::set_version(int64_t version)
    {
      version_ = version;
    }

    int64_t SSTableEntityIterator::get_version(void)
    {
      return version_;
    }
    //add 20151228:e
    ...
    void SSTableEntityIterator::set_column_filter(ColumnFilter *column_filter)
    {
      ...
      if (NULL != column_filter_ && column_filter_->get_read_atomic_param().is_valid())
      {
        ...
        YYSYS_LOG(DEBUG,"read_atomic::debug,finish set param to sstable iter!param=[%s]",
                  to_cstring(read_atomic_param_));
      }
      //add duyr 20160423:e
    }
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20160423:b
    void SSTableEntityIterator::set_data_mark(const ObReadAtomicDataMark &data_mark)
    {
      data_mark_ = data_mark;
      YYSYS_LOG(DEBUG,"read_atomic::debug,finish set data mark to sstable iter!mark=[%s]",
                to_cstring(data_mark));
    }

    bool SSTableEntityIterator::is_has_read_atomic_cell()
    {
      bool bret = false;
      if (read_atomic_param_.is_valid())
      {
        if ((read_atomic_param_.need_data_mark_ && data_mark_.is_valid() && output_major_ver_ == 0)
            ||(read_atomic_param_.need_data_mark_ && data_mark_.is_valid()  && output_minor_ver_start_ == 0)
            ||(read_atomic_param_.need_data_mark_ && data_mark_.is_valid()  && output_minor_ver_end_ == 0)
            ||(read_atomic_param_.need_data_mark_ && data_mark_.is_valid()  && output_paxos_id_ == 0)
            ||(read_atomic_param_.need_data_mark_ && data_mark_.is_valid()  && output_data_store_type_ == 0))
        {
          bret = true;
        }
      }
    ...
    void SSTableEntityIterator::reset_read_atomic_cell()
    {
      old_real_ci_ptr_ = NULL;
      output_paxos_id_       = 0;
      output_major_ver_      = 0;
    ...
    int SSTableEntityIterator::next_read_atomic_cell(const bool is_row_changed)
    {
      int ret = OB_SUCCESS;
      bool has_get_cell = false;

      if (!read_atomic_param_.is_valid()
          || !has_store_real_data_state_)
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(ERROR,"must have read atomic param or real data state!"
                  "has_store_real_data_state_=%d,ret=%d",
                  has_store_real_data_state_,ret);
      }
      else
      {
        is_outputing_read_atomic_cell_ = true;
        //TODO:READ_ATOMIC
        //we just output data mark yet!
        //other read atomic cell will output someday!
        ...
        {
          if (0 == output_major_ver_)
          {
            output_major_ver_ = static_cast<int64_t>(data_mark_.major_version_);
            read_atomic_cell_.column_id_ = read_atomic_param_.major_ver_cid_;
            read_atomic_cell_.value_.set_int(output_major_ver_);
            has_get_cell = true;
            output_major_ver_ = (0 == output_major_ver_) ? -1:output_major_ver_;
          }
          else if (0 == output_minor_ver_start_)
          {
            output_minor_ver_start_ = static_cast<int64_t>(data_mark_.minor_version_start_);
            read_atomic_cell_.column_id_ = read_atomic_param_.minor_ver_start_cid_;
            read_atomic_cell_.value_.set_int(output_minor_ver_start_);
            has_get_cell = true;
            output_minor_ver_start_ = (0 == output_minor_ver_start_) ? -1:output_minor_ver_start_;
          }
          else if (0 == output_minor_ver_end_)
          {
            output_minor_ver_end_ = static_cast<int64_t>(data_mark_.minor_version_end_);
            read_atomic_cell_.column_id_ = read_atomic_param_.minor_ver_end_cid_;
            read_atomic_cell_.value_.set_int(output_minor_ver_end_);
            has_get_cell = true;
            output_minor_ver_end_ = (0 == output_minor_ver_end_) ? -1:output_minor_ver_end_;
          }
          else if (0 == output_paxos_id_)
          {
            output_paxos_id_ = static_cast<int64_t>(data_mark_.ups_paxos_id_);
            read_atomic_cell_.column_id_ = read_atomic_param_.ups_paxos_id_cid_;
            read_atomic_cell_.value_.set_int(output_paxos_id_);
            has_get_cell = true;
            output_paxos_id_ = (0 == output_paxos_id_) ? -1:output_paxos_id_;
          }
          else if (0 == output_data_store_type_)
          {
            output_data_store_type_ = static_cast<int64_t>(data_mark_.data_store_type_);
            read_atomic_cell_.column_id_ = read_atomic_param_.data_store_type_cid_;
            read_atomic_cell_.value_.set_int(output_data_store_type_);
            has_get_cell = true;
            output_data_store_type_ = (0 == output_data_store_type_) ? -1:output_data_store_type_;
          }

          if (OB_SUCCESS == ret && has_get_cell)
          {
            cur_ci_ptr_ = &read_atomic_cell_;
            is_row_changed_ = is_row_changed;
          }
        }

        if (OB_SUCCESS == ret
            && !has_get_cell)
        {//finish output all read atomic cell!
          is_row_changed_ = is_row_changed;
          cur_ci_ptr_     = old_real_ci_ptr_;

          //it's important!!!!
          is_outputing_read_atomic_cell_ = false;
          has_store_real_data_state_     = false;
//          reset_next_row_atomic_cell();
        }
      }

      YYSYS_LOG(DEBUG,"read_atmic::debug,finish next atomic cell!"
                "has_get_cell=%d,"
                "is_outputing_read_atomic_cell_=%d,"
                "has_store_real_data_state_=%d"
                "is_row_changed_=%d,"
                "cur_ci_ptr_=%p,"
                "old_real_ci_ptr_=%p,iter_counter=%ld,ret=%d",
                has_get_cell,
                is_outputing_read_atomic_cell_,
                has_store_real_data_state_,
                is_row_changed_,
                cur_ci_ptr_,
                old_real_ci_ptr_,
                ...
                ret);
      if (NULL != cur_ci_ptr_)
        YYSYS_LOG(DEBUG,"read_atomic::debug,cur_ci_key=[%s],cur_ci_val=[%s],"
                  "tid=%ld,cid=%ld",
                  to_cstring(cur_ci_ptr_->row_key_),
                  to_cstring(cur_ci_ptr_->value_),
                  cur_ci_ptr_->table_id_,
                  cur_ci_ptr_->column_id_);
      if (NULL != old_real_ci_ptr_)
        YYSYS_LOG(DEBUG,"read_atomic::debug,old_real_ci_key=[%s],cur_ci_val=[%s],"
                  "tid=%ld,cid=%ld",
                  to_cstring(old_real_ci_ptr_->row_key_),
                  to_cstring(old_real_ci_ptr_->value_),
                  old_real_ci_ptr_->table_id_,
                  old_real_ci_ptr_->column_id_);
      return ret;
    }
    //add duyr 20160423:e
                          ...
                          ITableIterator *iter)
    {
      ...
      sub_iter->reset();
      ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
      bool is_table_truncated = false; // bugfix [322]

      if (NULL == ups_main)
      {
        YYSYS_LOG(ERROR, "get ups main fail");
        ret = OB_ERROR;
      }
      else if (NULL == sstable_reader_)
      {
        YYSYS_LOG(WARN, "invalid sstable_reader sstable_id=%lu", sstable_id_);
        ret = OB_ERROR;
      }
      else if (NULL == column_filter
              || NULL == sub_iter)
      {
        YYSYS_LOG(WARN, "invalid param column_filter=%p sub_iter=%p", column_filter, sub_iter);
        ret = OB_INVALID_ARGUMENT;
      }
      //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
      else if (column_filter->get_read_atomic_param().is_valid()
               && OB_SUCCESS != (ret = fill_local_data_mark_(data_mark)))
      {
        YYSYS_LOG(WARN,"fail to set data makr!ret=%d",ret);
      }
      //add duyr 20151207:e
      //bugfix 322
      else if (OB_SUCCESS != (ret = table_item_.get_table_truncate_stat(table_id, get_table_type(), is_table_truncated)))
      {
         YYSYS_LOG(WARN, "get_table_truncate_stat ret=[%d]",ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "add cell to get_param fail ret=%d cell_info=[%s]", ret, print_cellinfo(&cell_info));
        }
        else if (OB_SUCCESS != (ret = sstable_query.get(get_param, *sstable_reader_, sstable_iter))
                || NULL == sstable_iter)
        {
          YYSYS_LOG(WARN, "sstable query get fail ret=%d sstable_iter=%p cell_info=[%s]", ret, sstable_iter, print_cellinfo(&cell_info));
        }
        else
        {
          ...
          //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
          sub_iter->set_data_mark(data_mark);
          //add duyr 20151207:e
        }
      }
      return ret;
                            ...
                            ITableIterator *iter)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable_reader=%p", sstable_reader_);
        ret = OB_ERROR;
      }
      else if (NULL == (column_filter = ColumnFilter::build_columnfilter(scan_param, column_filter)))
      {
        YYSYS_LOG(WARN, "build column_filter fail");
        ret = OB_ERROR;
      }
      else if (NULL == sub_iter)
      {
        YYSYS_LOG(WARN, "invalid sub_iter=%p", sub_iter);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == scan_whole_row)
      {
        YYSYS_LOG(WARN, "get tsi scanparam fail");
      ...
      else
      {
        ...
        if (OB_SUCCESS != (ret = sstable_query.scan(*scan_whole_row, *sstable_reader_, sstable_iter))
            || NULL == sstable_iter)
        {
          YYSYS_LOG(WARN, "sstable query scan fail ret=%d sstable_iter=%p", ret, sstable_iter);
        }
        else
        {
          sub_iter->set_sstable_iter(sstable_iter);
    ...
    int SSTableEntity::init_sstable_meta(ITableEntity::SSTableMeta &sst_meta)
    {
      ...
      else
      {
          ...
          if (NULL == sstable_reader_)
          {
              ...
              {
                  sst_meta.sstable_loaded_time = yysys::CTimeUtil::getTime();
              }
              if (NULL != file_info)
              {
        ...
        if (NULL == sstable_reader_)
        {
          YYSYS_LOG(WARN, "invalid sstable_reader %s", sst_id.log_str());
          ret = OB_ERROR;
        }
        else if (OB_SUCCESS != (ret = sstable_reader_->open(ob_sstable_id, sst_id.major_version)))
        {
          destroy_sstable_meta();
          YYSYS_LOG(WARN, "sstable_reader open fail ret=%d %s", ret, sst_id.log_str());
        }
        else
        {
          YYSYS_LOG(INFO, "init sstable meta succ %s sstable_reader=%p", sst_id.log_str(), sstable_reader_);
          ...
          check_checksum = ups_main->get_update_server().get_replay_checksum_flag();//[523]
          if (check_checksum && 0 != remote_sstable_checksum_
              && remote_sstable_checksum_ != sstable_reader_->get_trailer().get_sstable_checksum())
          {
            YYSYS_LOG(ERROR, "sstable checksum not match, %s remote=%lu local=%lu",
                      sst_id.log_str(),
          ...
          {
            YYSYS_LOG(INFO, "sstable checksum match, %s remote=%lu local=%lu",
                      sst_id.log_str(),
                      remote_sstable_checksum_,
                      sstable_reader_->get_trailer().get_sstable_checksum());
          }
    ...
    void SSTableEntity::pre_load_sstable_block_index()
    {
      ...
      if (NULL == sstable_reader_)
      {
        YYSYS_LOG(ERROR, "sstable_reader null pointer");
      }
      ...
      {
        int64_t pre_load_timeu = yysys::CTimeUtil::getTime();
        ...
        {
          YYSYS_LOG(WARN, "try read from sstable once fail, ret=%d", tmp_ret);
        }
        else
        {
          ObCellInfo *ci = NULL;
          ...
          YYSYS_LOG(INFO, "try read from sstable for pre-load blockindex, next_cell ret=%d timeu=%ld",
                    tmp_ret, yysys::CTimeUtil::getTime() - pre_load_timeu);
        }
    ...
    int SSTableEntity::get_endkey(const uint64_t table_id, ObTabletInfo &ti)
    {
      ...
      if (NULL == sstable_reader_)
      {
        YYSYS_LOG(WARN, "invalid sstable_reader %s", SSTableID::log_str(sstable_id_));
        ret = OB_ERROR;
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "get endkey [%s] from sstable %s ret=%d, ret",
    ...
    int SSTableEntity::get_table_truncate_stat(uint64_t table_id, bool &is_truncated)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid sstable_reader");
        ret = OB_ERROR;
      }
    ...
    //add:e
    //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151207:b
    int SSTableEntity::fill_local_data_mark_(ObReadAtomicDataMark &local_data_mark)
    {
      int ret = OB_SUCCESS;
      uint64_t cur_data_version = 0;
      ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
      ...
      {
        local_data_mark.ups_paxos_id_ = ups_main->get_update_server().get_param().paxos_id;
        cur_data_version    = get_sstable_id();
        local_data_mark.data_store_type_ = common::ObReadAtomicDataMark::UPS_SSTABLE_DATA;
        local_data_mark.major_version_       = SSTableID::get_major_version(cur_data_version);
        local_data_mark.minor_version_start_ = SSTableID::get_minor_version_start(cur_data_version);
        local_data_mark.minor_version_end_   = SSTableID::get_minor_version_end(cur_data_version);
        if (!local_data_mark.is_valid())
      ...
      YYSYS_LOG(DEBUG,"read_atomic::debug,finish get sst table data mark!"
                "cur_data_mark=[%s],ret=%d",
                to_cstring(local_data_mark),ret);
      return ret;
    }
        ...
        else
        {
          ...
          switch (index)
          {
            ...
            case 1:
              ...
              break;
            default:
              YYSYS_LOG(WARN, "invalid percent index=%d sstable_percent=%ld", index, sstable_percent);
              ret = &memtable_entity_;
              break;
          }
        }
    ...
    int TableItem::get_table_truncate_stat(uint64_t table_id, int tabletype, bool &is_truncated)
    {
      is_truncated = false;
      int ret = OB_SUCCESS;
...
//      if (ACTIVE <= stat_
//          && DUMPED >= stat_)
      if(tabletype == ITableEntity::MEMTABLE)
      {
        ret = memtable_entity_.get_table_truncate_stat(table_id, is_truncated);
      }
//      else if (DROPING == stat_
//              || DROPED == stat_)
      else if(tabletype == ITableEntity::SSTABLE)
      {
    ...
    MemTable &TableItem::get_memtable()
    {
      if (DROPING < stat_)
      {
        YYSYS_LOG(WARN, "invalid stat=%d for get memtable", stat_);
      }
    ...
    int TableItem::load_inmemtable(ObUpsTableMgr& table_mgr, SessionMgr& session_mgr, LockMgr& lock_mgr)
    {
      ...
      {
        YYSYS_LOG(ERROR, "fail to get_schema, sstable_id=%s, err=%d", SSTableID::log_str(sstable_entity_.get_sstable_id()), err);
      }
      else if (NULL == (schema = schema_wrapper->get_impl()))
      {
        YYSYS_LOG(ERROR, "fail to get_schema, ");
      }
      else if (OB_SUCCESS != (err = inmemtable_entity_.get_memtable().init(hash_size)))
      {
        YYSYS_LOG(ERROR, "inmemtable_entity.init(%ld)=>%d", hash_size, err);
      }
          ...
          table_schema++)
      {
        if (!is_inmemtable(table_schema->get_table_id()))
        {
          ...
          if ((OB_SUCCESS == (err = sstable_entity_.get_table_truncate_stat(table_schema->get_table_id(), is_truncated))) && is_truncated)
          {

            YYSYS_LOG(INFO, "load table_id(%ld) truncate info", table_schema->get_table_id());


            ObMutator *mutator = GET_TSI_MULT(ObMutator, TSI_UPS_MUTATOR_1);
            if (mutator == NULL)
            {
            ...
            {
              mutator->reset();
              mutator->ups_trun_tab(table_schema->get_table_id());
              //mutator->ups_trun_tab(table_schema->get_table_id(),true);//del [312]
            }
            bool rollback = false;
            ...
            trans_id.reset();
            if (OB_SUCCESS != (err = session_guard.start_session(req, trans_id, session_ctx)))
            {
            ...
            else
            {
              ...
              session_ctx->set_trans_id(0); // set_trans_id(0)��֤end_sessionʱ���޸�ȫ�ֵ�commited_trans_id
            }

            if (OB_SUCCESS != err)
            {}
            else if (OB_SUCCESS != (err = inmemtable_entity_.get_memtable().set(*session_ctx, *lock_info, *mutator)))
            {
              YYSYS_LOG(ERROR, "inmemtable_entity->set(table_id=%ld)=>%d", table_schema->get_table_id(), err);
            }
            else
            {
              YYSYS_LOG(INFO, "load inmemtable %s %ld row", table_schema->get_table_name(), session_ctx->get_ups_result().get_affected_rows());
            ...
            {
              YYSYS_LOG(ERROR, "end_session(table_id=%ld, %s)=>%d", table_schema->get_table_id(), to_cstring(trans_id), err);
            }
          }
          //add:e
        }
        else
        {
          ...
          req.idle_time_ = INT64_MAX;
          trans_id.reset();
          YYSYS_LOG(INFO, "load %ldth inmemtable %s[%ld]", load_count++, table_schema->get_table_name(), table_schema->get_table_id());
          if (OB_SUCCESS != (err = scan_param.set(table_schema->get_table_id(), table_name, range)))
          {
            YYSYS_LOG(ERROR, "scan_param.set(%ld, %s)=>%d", table_schema->get_table_id(), table_schema->get_table_name(), err);
          }
          else if (NULL == (iter = sstable_entity_.alloc_iterator(table_mgr.get_table_mgr()->get_resource_pool(), guard)))
          {
            err = OB_MEM_OVERFLOW;
            YYSYS_LOG(ERROR, "table_entity->alloc_iterator(table_id=%ld) FAIL", table_schema->get_table_id());
          }
          else if (OB_SUCCESS != (err = session_guard.start_session(req, trans_id, session_ctx)))
          {
            YYSYS_LOG(WARN, "begin session fail ret=%d", err);
          }
          else if (OB_SUCCESS != (err = sstable_entity_.scan(*session_ctx, scan_param, iter)))
          {
            YYSYS_LOG(ERROR, "table_entity->scan(table_id=%ld)=>%d", table_schema->get_table_id(), err);
          }
          else
          {
            session_ctx->get_uc_info().host = &inmemtable_entity_.get_memtable();
      ...
      if (OB_SUCCESS != err)
      {
        ...
        YYSYS_LOG(ERROR, "load %ld inmemtable, %ld lines, btree %ld lines, hash %ld lines", load_count,
      ...
      {
        YYSYS_LOG(INFO, "load %ld inmemtable, %ld lines, btree %ld lines, hash %ld lines", load_count,
                  inmemtable_entity_.get_memtable().size(), inmemtable_entity_.get_memtable().btree_size(), inmemtable_entity_.get_memtable().hash_size());
        if (inmemtable_entity_.get_memtable().size() == 0)
        {
          YYSYS_LOG(INFO, "drop empty inmemtable");
          inmemtable_entity_.get_memtable().destroy();
        }
        __sync_synchronize();
    ...
    int TableItem::init_sstable_meta()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "erase sstable from sstable_mgr fail ret=%d sstable_id=%lu", tmp_ret, get_sstable_id());
          }
        }
      }
      return ret;
    ...
    void TableItem::set_last_clog_id(const uint64_t last_clog_id)
    {
        last_clog_id_ = last_clog_id;
    }
    ...
    int TableItem::do_freeze(const uint64_t clog_id, const int64_t time_stamp)
    {
      ...
      if (FREEZING != stat_)
      {
        YYSYS_LOG(WARN, "invalid status=%d for do_freeze", stat_);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (NULL == ups_main)
        {
          YYSYS_LOG(WARN, "get ups main fail");
          ret = OB_ERROR;
        }
        else if (OB_INVALID_ID == clog_id)
        {
          YYSYS_LOG(WARN, "invalid param clog_id=%lu", clog_id);
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "row_iter set schema handle fail ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(INFO, "memtable rowiter init succ");
          }
        }
        ...
        clog_id_ = clog_id;
        //add :b check the continuity of sstable
        int64_t sst_num = ups_main->get_update_server().get_sstable_mgr().get_sstable_info_map_size();
        TableMgr *table_mgr = ups_main->get_update_server().get_table_mgr().get_table_mgr();
        TableItem *table_item = table_mgr->get_active_memtable();
        if(NULL == table_item)
        {
            ret = OB_ERROR;
        }
        ...
        {
            table_item->set_last_clog_id(clog_id);
            table_mgr->revert_active_memtable(table_item);
        }

        if((OB_INVALID_ID == last_clog_id_) && (0 == sst_num))
        {
            YYSYS_LOG(WARN,"invalid param last_clog_id_=%lu", last_clog_id_);
            last_clog_id_ = 0;
            int64_t last_clog_id_int = 0;
            ups_main->get_update_server().get_log_mgr().get_replay_point().get(last_clog_id_int);
            if(last_clog_id_int > 0)
            {
                last_clog_id_ = static_cast<uint64_t>(last_clog_id_int);
            }
            YYSYS_LOG(INFO,"get last_clog_id from log_replay_point, last_clog_id_=%lu", last_clog_id_);
        }
    ...
    int TableItem::freeze_memtable()
    {
      ...
      if (ACTIVE != stat_
          && FREEZING != stat_)
      {
        YYSYS_LOG(WARN, "invalid status=%d for freeze", stat_);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int TableItem::do_dump(ObILogWriter &log_writer)
    {
      ...
      if (DUMPING!= stat_)
      {
        YYSYS_LOG(WARN, "invalid status=%d for do_dump", stat_);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          SSTableMgr &sstable_mgr = ups_main->get_update_server().get_sstable_mgr();
          if (OB_SUCCESS != (ret = sstable_mgr.add_sstable(sstable_entity_.get_sstable_id(), clog_id_, time_stamp_, row_iter_, schema_,last_clog_id_)))
          {
            YYSYS_LOG(ERROR, "add sstable fail ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = this->init_sstable_meta()))
          {
            YYSYS_LOG(ERROR, "init sstable_meta fail ret=%d", ret);
          }
          else
          {
            SSTableID sst_id = sstable_entity_.get_sstable_id();
            log_writer.write_replay_point(clog_id_);
            YYSYS_LOG(INFO, "dump sstable succ ret=%d %s", ret, sst_id.log_str());
            stat_ = DUMPED;
          }
        }
        if (OB_SUCCESS != ret)
    ...
    int TableItem::do_drop()
    {
      ...
      if (DROPING != stat_)
      {
        YYSYS_LOG(WARN, "invalid status=%d for do_drop", stat_);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "notice memtable still has been used ret_cnt=%ld", memtable_entity_.get_memtable().get_ref_cnt());
        }
        if (NULL != schema_)
        {
          schema_->~CommonSchemaManager();
    ...
    bool TableItem::erase_sstable()
    {
      ...
      if (DUMPED <= stat_)
      {
        ...
        YYSYS_LOG(INFO, "erase sstable succ table_item_stat=%d %s", stat_, sst_id.log_str());
        if (DROPED == stat_)
        {
          bret = true;
        }
    ...
    int TableItem::get_schema(CommonSchemaManagerWrapper &sm) const
    {
      ...
      if (FROZEN > stat_)
      {
        ...
        YYSYS_LOG(INFO, "%s stat=%d major version has not update, will return current latest schema, ret=%d",
                  SSTableID::log_str(sstable_entity_.get_sstable_id()), stat_, ret);
      }
      //else if (NULL != schema_)
      else if(NULL != schema_ && stat_ < DROPING)
      {
        sm = *schema_;
        YYSYS_LOG(INFO, "%s stat=%d get schema saved when do freeze",
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "%s stat=%d get schema read from file, ret=%d",
                  SSTableID::log_str(sstable_entity_.get_sstable_id()), stat_, ret);
      }
      return ret;
    }
    ...
    int TableItem::get_endkey(const uint64_t table_id, ObTabletInfo &ti)
    {
      ...
      if (DUMPED > stat_)
      {
        YYSYS_LOG(INFO, "%s has not dumped, cannot get endkey",
    ...
    uint64_t TableItem::get_clog_id() const
    {
        return clog_id_;
    }
    uint64_t TableItem::get_last_clog_id() const
    {
    ...
    //int TableMgr::init()
    int TableMgr::init(uint64_t start_major_version)
    {
      int ret = OB_SUCCESS;
      if (inited_)
      {
      ...
      else
      {
        ...
        //cur_major_version_ = SSTableID::START_MAJOR_VERSION;
        cur_major_version_ = start_major_version > SSTableID::START_MAJOR_VERSION ? start_major_version:SSTableID::START_MAJOR_VERSION;
        cur_minor_version_ = SSTableID::START_MINOR_VERSION;
        sstable_scan_finished_ = false;
        frozen_memused_ = 0;
        frozen_memtotal_ = 0;
    ...
    void TableMgr::destroy()
    {
      if (inited_)
      {
        if (true)
        {
          ...
          {
            YYSYS_LOG(WARN, "get read handle fail ret=%d", btree_ret);
          }
          else
          {
    ...
    int TableMgr::add_sstable(const uint64_t sstable_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param sstable_id=%lu", sstable_id);
        ret = OB_INVALID_ARGUMENT;
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "init sstable meta fail ret=%d sstable_id=%lu", ret, sstable_id);
          table_allocator_.free(table_item);
        }
        else
        {
          ...
          if (ERROR_CODE_OK != btree_ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "put table item fail sstable_id=%lu", sstable_id);
              ret = OB_ERROR;
            }
            else
            {
          ...
          else
          {
            ...
            YYSYS_LOG(INFO, "add sstable succ %s", sst_id.log_str());
          }
          map_lock_.unlock();
        }
      }
    ...
    int TableMgr::erase_sstable(const uint64_t sstable_id)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get from table_map fail sstable_id=%lu table_item_sstable_id=%lu",
        ...
        else
        {
          ...
          if (table_item->erase_sstable())
          {
            ...
            YYSYS_LOG(INFO, "erase sstable, remove from map %s", sst_id.log_str());
            if (0 == table_item->dec_ref_cnt())
            {
              table_allocator_.free(table_item);
              YYSYS_LOG(INFO, "erase sstable, delete table_item=%p %s", table_item, sst_id.log_str());
            }
          }
        }
        map_lock_.unlock();
    ...
    int TableMgr::sstable_scan_finished(const int64_t minor_num_limit)
    {
      ...
      else
      {
        ...
        if (sstable_scan_finished_)
        {
          YYSYS_LOG(INFO, "sstable scan has already finished");
        }
        else
        {
          SSTableID sst_id;
          ...
          {
            YYSYS_LOG(WARN, "put active_table_item to table_map fail ret=%d sstable_id=%lu", btree_ret, sst_id.id);
            ret = OB_ERROR;
          }
          else
          {
            YYSYS_LOG(INFO, "put active_table_item to table_mgr succ %s", sst_id.log_str());
        ...
        else
        {
          ...
          while (ERROR_CODE_OK == table_map_.get_next(handle, table_item))
          {
            if (NULL != table_item)
            {
              ...
              uint64_t clog_id = 0;
              uint64_t last_clog_id = 0;
              sstable_mgr.get_clog_and_last_clog(cur_sst_id.id, clog_id, last_clog_id);
              if(OB_INVALID_ID == last_clog_id)
              {
                  last_clog_id = 0;
              }
              ...
              {
                YYSYS_LOG(WARN, "sstable id do not continous %s <--> %s", sst_id_checker.log_str(), cur_sst_id.log_str());
                ret = OB_ERROR;
                break;
              }
              else if((0 != last_clog_id) && (0 != last_sst_clog_id) &&(last_sst_clog_id != last_clog_id))
              {
                  YYSYS_LOG(WARN,"sstable id do not continous last_clog_id=%lu, last_sst_clog_id=%lu, sst_id=%s",last_clog_id,last_sst_clog_id,cur_sst_id.log_str());
                  ret = OB_ERROR;
                  break;
              }
              else
    ...
    int TableMgr::get_table_time_stamp(const uint64_t major_version, int64_t &time_stamp)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid major_version=%ld cur_major_version=%ld",
          ...
          else
          {
            ...
            while (ERROR_CODE_OK == table_map_.get_next(handle, table_item))
            {
                  ...
                  && OB_SUCCESS == (ret = ti_list.allocator.write_string(ti.range_.end_key_, &(endkey_repl))))
              {
                ...
                {
                  YYSYS_LOG(INFO, "get and save endkey [%s] from %s succ",
                                ...
                                uint64_t table_id)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG,"start:%lu,%lu,%lu,end:%lu,%lu,%lu",sst_id_start.major_version,
        ...
        {
          YYSYS_LOG(WARN, "invalid version_range=%s", range2str(version_range));
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
          ...
          else
          {
            ...
            while (ERROR_CODE_OK == table_map_.get_next(handle, key, table_item))
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid table_item sstable_id=%lu", key.sst_id.id);
                ret = OB_UPS_ACQUIRE_TABLE_FAIL;
                break;
              }
                      ...
                      && !version_range.border_flag_.is_max_value())
              {
                YYSYS_LOG(WARN, "maybe acquire an active table for daily merge, will fail, version_range=[%s]", range2str(version_range));
              ...
              {
                YYSYS_LOG(ERROR, "request version is out of service, maybe slave cluster merge too slow, version_range=[%s]", range2str(version_range));
                ret = OB_NOT_MASTER;
                break;
              }
              else
              {
                ...
                {
                  YYSYS_LOG(WARN, "sstable id do not continous %s <--> %s", sst_id_checker.log_str(), key.sst_id.log_str());
                  ret = OB_UPS_ACQUIRE_TABLE_FAIL;
                  break;
                }
                else
                ...
                if (OB_SUCCESS == (ret = table_item->get_table_truncate_stat(table_id, table_entity->get_table_type(),is_truncated)))
                {
                  if (is_truncated)
                  {
                ...
                {
                    table_allocator_.free(table_item);
                    YYSYS_LOG(INFO,"erase sstable, delete table_item=%p %s",table_item, key.sst_id.log_str());
                }
              }
              first = false;
            }
        ...
        else
        {
          ...
          else
          {
            ...
            while (ERROR_CODE_OK == table_map_.get_next(handle, key, table_item))
            {
              ...
              // {
              //   YYSYS_LOG(WARN, "invalid version_range=[%s] min_major_version=%lu",
...
//              {
//                YYSYS_LOG(WARN, "push to list fail sstable_id=%lu", key.sst_id.id);
//                ret = OB_MEM_OVERFLOW;
//                break;
//              }
              else
              {
                  ...
                  if(OB_SUCCESS == (ret = table_item->get_table_truncate_stat(table_id, table_entity->get_table_type(), is_truncated)))
                  {
                      ...
                      if(0 != table_list.push_back(table_entity))
                      {
                          YYSYS_LOG(WARN, "push to list fail sstable_id=%lu", key.sst_id.id);
                          ret = OB_MEM_OVERFLOW;
                          break;
    ...
    TableItem *TableMgr::get_active_memtable()
    {
      ...
      else
      {
        ...
        if (NULL == active_table_item_)
        {
          YYSYS_LOG(WARN, "active_table_item null pointer");
        }
        else
        {
    ...
    void TableMgr::revert_memtable_(TableItem *table_item)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param table_item=%p", table_item);
      }
      else
      {
                                        ...
                                        const int64_t time_stamp)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param new_version=%lu clog_id=%lu", new_version, clog_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == active_table_item_)
      {
        YYSYS_LOG(WARN, "invalid active_table_item");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (0 == active_table_item_->get_sstable_id())
        {
          ...
          sstable_scan_finished_ = true;
          YYSYS_LOG(INFO, "first replay frozen log %s btree_ret=%d", sst_id.log_str(), btree_ret);
        }
        TableItem *table_item2freeze = freeze_active_(new_version);
        if (NULL == table_item2freeze)
        {
          YYSYS_LOG(WARN, "freeze memtable fail");
          ret = OB_ERROR;
        }
        else if (0 == table_item2freeze->get_memtable().size())
        {
          ...
          if (0 == sst_id.id || is_first_freeze)
          {
            ...
            YYSYS_LOG(INFO, "drop empty frozen table %s btree_ret=%d deallocated=%s",
                      sst_id.log_str(), btree_ret, STR_BOOL(deallocated));
          }
          else
          {
            table_item2freeze->inc_ref_cnt();
            YYSYS_LOG(INFO, "freeze an empty table %s", sst_id.log_str());
          }
        }
        else
        {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          active_table_item_->set_last_clog_id(clog_id);
        }
        map_lock_.unlock();
        if (NULL != table_item2freeze)
        {
          ...
          {
            YYSYS_LOG(ERROR, "do freeze fail ret=%d clog_id=%lu", ret, clog_id);
          }
          else
          {
              UPS.set_minor_freeze_stat(UPS_FROZEN);
            ...
            YYSYS_LOG(INFO, "replay freeze succ frozen_memused=%ld frozen_memtotal=%ld frozen_rowcount=%ld %s",
          ...
          {
            table_allocator_.free(table_item2freeze);
            YYSYS_LOG(INFO, "erase sstable, delete table_item=%p %s", table_item2freeze, sst_id.log_str());
          }
          map_lock_.unlock();
        }
      }
...
//    {
//        YYSYS_LOG(INFO, "active_mem_total=%ld need not freeze",
...
//    }
      else if (mem_limit > active_table_item_->get_memtable().total())
      {
        ret = OB_EAGAIN;
              ...
              && (yysys::CTimeUtil::getTime() - last_major_freeze_time_) < min_major_freeze_interval)
      {
        ret = OB_EAGAIN;
        YYSYS_LOG(WARN, "major freeze interval too small last_major_freeze_time=%ld cur_time=%ld min_major_freeze_interval=%ld",
            last_major_freeze_time_, yysys::CTimeUtil::getTime(), min_major_freeze_interval);
      }
      else if (!freeze_lock_.try_rdlock())
      {
        YYSYS_LOG(WARN, "someone has locked freeze action, loading bypass now, wait a moment");
        ret = OB_EAGAIN;
      }
      else if (OB_SUCCESS != (ret = log_writer_.switch_log_file(clog_id)))
      {
        YYSYS_LOG(WARN, "switch commit log fail ret=%d", ret);
        freeze_lock_.unlock();
      }
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          table_item2freeze = freeze_active_(new_version, limit_version);
        }
        if (NULL == table_item2freeze)
        {
          //YYSYS_LOG(WARN, "freeze memtable fail");
          //ret = OB_ERROR;
          SSTableID sst_id = active_table_item_->get_memtable().get_version();
          //mod peiouya [MultiUPS] [UPS_Manage_Function] 20150801:b
          //if (0 < limit_version && sst_id.major_version == limit_version + 1)
          //if (0 < limit_version && sst_id.major_version >= limit_version)
          if (0 < limit_version && sst_id.major_version == limit_version + 1)  //freeze bug fix 20150805
          //mod 20150801:e
          {
            major_version_changed = false;
            ret = OB_NOT_NEED_FROZEN;
          }
          else
          {
            YYSYS_LOG(WARN, "freeze memtable fail");
        ...
        if (NULL != table_item2freeze)
        {
          ...
          else
          {
            ...
            {
              last_major_freeze_time_ = yysys::CTimeUtil::getTime();
            }
            frozen_memused_ += table_item2freeze->get_memtable().used();
            frozen_memtotal_ += table_item2freeze->get_memtable().total();
            frozen_rowcount_ += table_item2freeze->get_memtable().size();
            YYSYS_LOG(INFO, "freeze succ last_major_freeze_time_=%ld frozen_memused=%ld frozen_memtotal=%ld frozen_rowcount=%ld %s",
    ...
    TableItem *TableMgr::freeze_active_(const uint64_t new_version, const uint64_t limit_version)
    {
      TableItem *table_item2freeze = NULL;
      TableItem *tmp_table_item = NULL;
      SSTableID sst_id = active_table_item_->get_memtable().get_version();
      ...
      //mod peiouya [MultiUPS] [UPS_Manage_Function] 20150801:b
      //if ((sst_id.major_version = limit_version && 0 < limit_version) || 0 == limit_version)
      //{
      //  tmp_table_item = table_allocator_.alloc();
      //}
      //if ((sst_id.major_version == limit_version -1 && 0 < limit_version) || 0 == limit_version)
      if ((sst_id.major_version == limit_version && 0 < limit_version) || 0 == limit_version)  //freeze bug fix 20150805
      {
        tmp_table_item = table_allocator_.alloc();
      }
      else
      ...
      if (NULL != tmp_table_item)
      {
        ...
        {
          YYSYS_LOG(WARN, "init memtable fail ret=%d", tmp_ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "freeze memtable fail ret=%d", tmp_ret);
          }
          else if (ERROR_CODE_OK != (btree_ret = table_map_.put(sst_id, tmp_table_item, false)))
          {
            YYSYS_LOG(WARN, "put to table_map fail ret=%d %s", btree_ret, sst_id.log_str());
          }
          else
          {
    ...
    bool TableMgr::try_dump_memtable()
    {
      ...
      else
      {
        ...
        if (NULL != table_item2dump)
        {
            //bugfix[258]
            int ret = OB_SUCCESS;
            if(OB_SUCCESS != (ret = table_item2dump->do_dump(log_writer_)))
            {
                YYSYS_LOG(WARN,"do dump failed,ret=%d",ret);
            }
            else
            {
                ...
                YYSYS_LOG(INFO,"dump succ frozen_memused=%ld frozen_memtotal=%ld frozen_rowcount=%ld",
                          frozen_memused_,frozen_memtotal_,frozen_rowcount_);
            }
          SSTableID sst_id = table_item2dump->get_sstable_id();
          if (0 == table_item2dump->dec_ref_cnt())
          {
            table_allocator_.free(table_item2dump);
            YYSYS_LOG(INFO, "erase sstable, delete table_item=%p %s", table_item2dump, sst_id.log_str());
          }
        }
      }
      return bret;
    ...
    bool TableMgr::try_drop_memtable(const int64_t mem_limit)
    {
      ...
      {
        //YYSYS_LOG(INFO, "need not drop memtable frozen_size=%ld limit=%ld", frozen_memtotal_, mem_limit);
      }
      else
      {
        ...
        if (NULL != table_item2drop)
        {
          ...
          if (0 == table_item2drop->dec_ref_cnt())
          {
            ...
            YYSYS_LOG(INFO, "erase sstable, delete table_item=%p %s", table_item2drop, sst_id.log_str());
          }
        }
        else
        {
          YYSYS_LOG(WARN, "need drop memtable for releasing memory, but no memtable can be dropped");
        }
        log_table_info();
      }
      return bret;
      ...
      else
      {
          ...
          else
          {
            ...
            while (ERROR_CODE_OK == table_map_.get_next(handle, table_item))
            {
              if (NULL != table_item)
              {
                ...
                if ((yysys::CTimeUtil::getTime() - table_item->get_sstable_loaded_time()) > time_limit
                    || (SSTableID::get_major_version(sstable_id) + MIN_MAJOR_VERSION_KEEP) <= merged_version_)
                {
                    //bugfix [333]
                    if(SSTableID::get_major_version(sstable_id)< merged_version_ ||
                            (SSTableID::get_major_version(sstable_id) == merged_version_
                             &&(yysys::CTimeUtil::getTime() - merged_timestamp_ > 600*1000*1000)))
                    {
                        major_version2erase = SSTableID::get_major_version(sstable_id);
                    ...
                    {
                        YYSYS_LOG(INFO,"erase sstable [%s] from map failed, merged_version_[%ld], merged_timestamp_[%ld]",
                                  SSTableID::log_str(sstable_id),merged_version_,merged_timestamp_);
                    }
                }
        ...
        for (iter = list2erase.begin(); iter != list2erase.end(); iter++)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "erase sstable from sstable_mgr fail ret=%d sstable_id=%lu", tmp_ret, *iter);
            }
          }
          erase_sstable(*iter);
        }
    ...
    void TableMgr::log_table_info()
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "==========log table info start==========");
          table_map_.set_key_range(handle, table_map_.get_min_key(), 0, table_map_.get_max_key(), 0);
          TableItem *table_item = NULL;
          while (ERROR_CODE_OK == table_map_.get_next(handle, table_item))
          {
            if (NULL != table_item)
            {
              ...
              if (TableItem::DUMPED >= table_item->get_stat())
              {
                ...
                YYSYS_LOG(INFO, "[table_info] stat=%d %s timestamp=%ld sstable_loaded_time=%ld "
              ...
              {
                YYSYS_LOG(INFO, "[table_info] stat=%d %s timestamp=%ld sstable_loaded_time=%ld, inmem_lines=%ld",
          ...
          YYSYS_LOG(INFO, "==========log table info end==========");
        }
        map_lock_.unlock();
      }
    }

    int TableMgr::serialize_ups_memtable_info(SSTableMgr &sstable_mgr, char *buf, const int64_t buf_len, int64_t &pos)
    {
        int ret = ERROR_CODE_OK;
        int32_t state = 0;
        int64_t frozen_timestamp = 0;
        int64_t dumped_timestamp = 0;
        int64_t memory_used = 0;
        int64_t memory_limit = 0;
        uint64_t start_commit_file_id = 0;
        uint64_t end_commit_file_id = 0;
        ObString store_info;
        ...
        else
        {
            map_lock_.rdlock();
            BtreeReadHandle handle;
            if(ERROR_CODE_OK != (ret = table_map_.get_read_handle(handle)))
            {
                YYSYS_LOG(WARN,"get read handle fail ret=%d",ret);
            }
            else
            {
                table_map_.set_key_range(handle, table_map_.get_min_key(),0,table_map_.get_max_key(),0);
                ...
                if(NULL != ups_main)
                {
                    memory_limit = ups_main->get_update_server().get_param().active_mem_limit;
                }
                ...
                while(ERROR_CODE_OK == table_map_.get_next(handle, table_item))
                {
                    ...
                    {
                        SSTableID sst_id = table_item->get_sstable_id();
                        state = table_item->get_stat();
                        frozen_timestamp = table_item->get_time_stamp();
                        dumped_timestamp = table_item->get_sstable_loaded_time();
                        start_commit_file_id = table_item->get_last_clog_id();
                        end_commit_file_id = table_item->get_clog_id();
                        memory_used = table_item->get_memtable().used();

                        if(TableItem::DUMPED > state) //ACTIVE, FREEZING, FROZEN, DUMPING
                        {
                            store_info = ObString::make_string("[]");
                        }
                        else //DUMPED, DROPING or DROPED
                        {
                            common::hash::ObHashMap<uint64_t, SSTableInfo *>::iterator iter;
                            for(iter = sstable_mgr.get_sstable_info_map().begin();
                                iter != sstable_mgr.get_sstable_info_map().end();
                                iter++)
                            {
                                if(iter->first == sst_id.id)
                                {
                                    SSTableInfo *sstable_info = iter->second;
                                    if(NULL != sstable_info)
                                    {
                                        sstable_info->get_memtable_store_info_string(store_info);
                                        start_commit_file_id = sstable_info->get_last_clog_id();
                                        end_commit_file_id = sstable_info->get_clog_id();
                                        break;
                                    }
                                }
                            }
                        ...
                        //serialize memtable info
                        if(OB_SUCCESS != (ret = serialization::encode_vi32(buf,buf_len,pos,state)))
                        {
                            YYSYS_LOG(WARN,"serialize memtable state error");
                        }
                        else if(OB_SUCCESS != (ret = sst_id.serialize(buf,buf_len,pos)))
                        {
                            YYSYS_LOG(WARN,"serialize sst_id error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,frozen_timestamp)))
                        {
                            YYSYS_LOG(WARN,"serialize frozen_timestamp error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,dumped_timestamp)))
                        {
                            YYSYS_LOG(WARN,"serialize dumped_timestamp error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,memory_used)))
                        {
                            YYSYS_LOG(WARN,"serialize memory_used error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,memory_limit)))
                        {
                            YYSYS_LOG(WARN,"serialize memory_limit error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,start_commit_file_id)))
                        {
                            YYSYS_LOG(WARN,"serialize start_commit_file_id error");
                        }
                        else if(OB_SUCCESS != (ret = serialization::encode_vi64(buf,buf_len,pos,end_commit_file_id)))
                        {
                            YYSYS_LOG(WARN,"serialize end_commit_file_id error");
                        }
                        else if(OB_SUCCESS != (ret = store_info.serialize(buf,buf_len,pos)))
                        {
                            YYSYS_LOG(WARN,"serialize store_info error");
                        }
                    }
                }
                if(OB_SUCCESS != (ret = serialization::encode_vi32(buf,buf_len,pos,9)))//mark the end of one ups memtables
                {
                    YYSYS_LOG(WARN,"serialize memtable state error");
                }
            }
    ...
    void TableMgr::get_ups_used_mem(int64_t &mem_size)
    {
        if(!inited_)
        {
            YYSYS_LOG(WARN,"have not inited this=%p", this);
            ...
            else
            {
                ...
                while(ERROR_CODE_OK == table_map_.get_next(handle,table_item))
                {
                    if(NULL != table_item)
                    {
                        if(TableItem::DUMPED > table_item->get_stat())
                        {
                            mem_size += table_item->get_memtable().used();
                        }
                        else if (TableItem::DUMPED == table_item->get_stat())
                        {
                            dumped_mem_size += table_item->get_memtable().used();
                        }
                        else
                        {
                            //droping or droped
    ...
    bool TableMgr::need_auto_freeze() const
    {
          ...
          && mem_limit < active_table_item_->get_memtable().total())
      {
        ...
        {
          YYSYS_LOG(INFO, "need auto freeze active_total=%ld active_limit=%ld",
    ...
    bool TableMgr::need_lock_partition() const
    {
      bool bret = false;
      ...
      {
        mem_limit = ups_main->get_update_server().get_param().active_mem_limit
                    - ups_main->get_update_server().get_param().partition_lock_mem_size;
        num_limit = ups_main->get_update_server().get_param().minor_num_limit;
      }

      if (NULL != active_table_item_
          && (uint64_t)num_limit < cur_minor_version_ + 1
          && mem_limit <= active_table_item_->get_memtable().total())
      {
        bret = true;
      }
    ...
    int TableMgr::try_freeze_memtable(const FreezeType freeze_type,
                                      uint64_t &new_version, uint64_t &frozen_version,
                                      uint64_t &clog_id, int64_t &time_stamp,
                                      bool &major_version_changed, const uint64_t limit_version)
    {
      int64_t mem_limit = 0;
      int64_t num_limit = 0;
      int64_t min_major_freeze_interval = 0;
      ...
      if (FORCE_MINOR == freeze_type)
      {
        ...
        YYSYS_LOG(INFO, "try froce minor freeze");
      }
      else if (FORCE_MAJOR == freeze_type)
      {
        mem_limit = 0;
        num_limit = 0;
        YYSYS_LOG(INFO, "try froce major freeze");
      }
      else if (MINOR_LOAD_BYPASS == freeze_type)
      {
        ...
        min_major_freeze_interval = INT64_MAX;
        YYSYS_LOG(INFO, "try froce minor freeze and load bypass");
      }
      else if (MAJOR_LOAD_BYPASS == freeze_type)
      {
        mem_limit = 0;
        num_limit = 0;
        YYSYS_LOG(INFO, "try froce major freeze and load bypass");
      }

      return try_freeze_memtable(mem_limit, num_limit, min_major_freeze_interval,
                                new_version, frozen_version, clog_id, time_stamp, major_version_changed, limit_version);
    }
    ...
    bool TableMgr::try_drop_memtable(const bool force)
    {
      ...
      else
      {
        ...
        if (NULL != table_item)
        {
          ...
          {
            YYSYS_LOG(INFO, "table_memory_total=%ld table_available_warn_size=%ld "
    ...
    int TableMgr::clear_active_memtable()
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "there is someone using memtable cannot clear ref_cnt=%ld",
        ...
        map_lock_.unlock();
        YYSYS_LOG(INFO, "clear active memtable ret=%d total_before=%ld total_after=%ld",
    ...
    int TableMgr::add_table_list()
    {
      ...
      {
        YYSYS_LOG(INFO, "there is not table to add, will do nothing");
        ret = OB_ENTRY_NOT_EXIST;
      }
      else if(ERROR_CODE_OK != (btree_ret = table_map_.get_write_handle(write_handle)))
      {
        YYSYS_LOG(WARN, "btree get write handle fail btree_ret=%d", btree_ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for (iter = table_list2add_.begin(); OB_SUCCESS == ret && iter != table_list2add_.end(); iter++)
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS != (ret = table_item->init_sstable_meta()))
            {
              YYSYS_LOG(WARN, "init sstable meta fail ret=%d %s", ret, sst_id.log_str());
              table_allocator_.free(table_item);
            }
            else if (ERROR_CODE_OK != (btree_ret = table_map_.put(write_handle, sst_id, table_item, false)))
            {
              YYSYS_LOG(WARN, "put table item fail btree_ret=%d %s", btree_ret, sst_id.log_str());
              table_allocator_.free(table_item);
              ret = OB_ERROR;
            }
            else
            {
              ...
              YYSYS_LOG(INFO, "put table item succ %s", sst_id.log_str());
            }
          }
        }
        if (OB_SUCCESS != ret)
        {
          ...
          for (uncommited_iter = uncommited_list.begin(); uncommited_iter != uncommited_list.end(); uncommited_iter++)
          {
            ...
            YYSYS_LOG(WARN, "load rollbacked %s", sst_id.log_str());
          }
        }
        else
        {
          ...
          for (iter = table_list2add_.begin(); OB_SUCCESS == ret && iter != table_list2add_.end(); iter++)
          {
            ...
            YYSYS_LOG(INFO, "load succ %s", sst_id.log_str());
          }
        }
        write_handle.end();
      }
                                        ...
                                        const int64_t minor_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "remove cur active table item from table map fail, btree_ret=%d sst_id=[%s]",
      ...
      {
        YYSYS_LOG(WARN, "add active table item with new version to table map fail, btree_ret=%d sst_id=[%s]",
                  btree_ret,new_sst_id.log_str());
        ret = OB_ERROR;
      }
      else
      {
        YYSYS_LOG(INFO, "modify active_version from %s to %s succ",
    ...
    void thread_read_prepare()
    {
      ...
      {
        YYSYS_LOG(ERROR, "cannot get tsi object of PageArena");
      }
      else
      {
    ...
    void thread_read_complete()
    {
      ...
      if (NULL != aio_buf_mgr_array)
      {
        ...
        if (OB_AIO_TIMEOUT == status)
        {
          ...
          YYSYS_LOG(WARN, "failed to wait all aio buffer manager free, stop current thread");
        }
      }
    }
    ...
    void MajorFreezeDuty::runTimerTask()
    {
      ...
      {
        YYSYS_LOG(WARN, "get local time fail errno=%u", errno);
      }
      else
      {
        ...
        */
        ...
        {
          YYSYS_LOG(DEBUG, "Major freeze disabled");
        }
        //mod peiouya [MULTI_DUTY_TIME] 20141125:b
        //del begin
        /*
        ...
        */
        ...
        //mod end
        {
          if (!same_minute_flag_)
          {
			...
			/*
			YYSYS_LOG(INFO, "submit major freeze cur_time=%d:%d duty_time=%d:%d",
                      human_time_ptr->tm_hour, human_time_ptr->tm_min, duty_hour, duty_min);
		    */
            YYSYS_LOG(INFO, "submit major freeze cur_time=%d:%d,equal:true, same_minute_flag_:false",
                      human_time_ptr->tm_hour, human_time_ptr->tm_min);
          }
          else
          {
		    /*
		    YYSYS_LOG(DEBUG, "need not do major freeze cur_time=%d:%d duty_time=%d:%d sec=%d",
                      human_time_ptr->tm_hour, human_time_ptr->tm_min, duty_hour, duty_min, human_time_ptr->tm_sec);
			*/
            YYSYS_LOG(DEBUG, "need not do major freeze cur_time=%d:%d sec=%d,,equal:true, same_minute_flag_:true",
        ...
        else
        {
		  ...
		  /*
		  YYSYS_LOG(DEBUG, "need not do major freeze cur_time=%d:%d duty_time=%d:%d",
                    human_time_ptr->tm_hour, human_time_ptr->tm_min, duty_hour, duty_min);
		  */
          YYSYS_LOG(DEBUG, "need not do major freeze cur_time=%d:%d, equal:false",
                    human_time_ptr->tm_hour, human_time_ptr->tm_min);
        }
      }
      return;
    ...
    void MinorFreezeDuty::runTimerTask()
    {
        ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
        if(NULL == ups_main)
        {
        ...
        else
        {
            ...
            {
                if(OB_SUCCESS != ups.submit_minor_freeze())
                {
                    YYSYS_LOG(WARN,"submit_minor_freeze() failed");
                }
                else
                {
                    YYSYS_LOG(INFO, "submit minor freeze task success");
                }
            }
        }
    }
    ...
    //[588]
    void MemtableLockDuty::runTimerTask()
    {
        ObUpdateServerMain *ups_main = ObUpdateServerMain::get_instance();
        if(NULL == ups_main)
        {
        ...
        {
            ObUpdateServer &ups = ups_main->get_update_server();
            TableMgr *table_mgr = ups.get_table_mgr().get_table_mgr();
            if(table_mgr->check_lock())
            {
                if(EXIST_LOCK == stat_)
                {
                    stat_ = NO_LOCK;
            ...
            else
            {
                ...
                else if(EXIST_LOCK == stat_)
                {
                    stat_ = LOCK_TIMEOUT;
                    while(table_mgr->exist_lock())
                    {
                        table_mgr->release_memtable_lock();
                    }
                    stat_ = NO_LOCK;
                }
                else
                {
                    YYSYS_LOG(INFO, "releasing memtable lock!");
                }
            }
        }
    }
