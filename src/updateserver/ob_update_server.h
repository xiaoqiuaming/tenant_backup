...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    struct ObSafeOnceGuard
    {
      ...
      void launch_success()
      {
        last_launch_time_ = yysys::CTimeUtil::getTime();
      }
      void launch_fail()
      {}
      void done()
      ...
      : public common::ObBaseServer, public ObPacketQueueHandler, public common::IBatchPacketQueueHandler, public ObUtilInterface
    {
      ...
      public:
        //yynet::IPacketHandler::HPRetCode handlePacket(yynet::Connection *connection, yynet::Packet *packet);
      ...
      //add e
        ...
        bool get_replay_checksum_flag()
        { return config_.replay_checksum_flag; }
        ...
        inline void set_root_server(const common::ObServer &rs)
        {
          ...
          if (!res)
          {
              YYSYS_LOG(ERROR, "root server address invalid: %s:%s",
          ...
          if (old_config_server != rs)
          {
              ...
              YYSYS_LOG(DEBUG, "update rootserver ip and port to config file [%s:%s]",
                 config_.root_server_ip.str(),
                 config_.root_server_port.str());
          }
          //add:e
...
//        }
        ...
        //[593]
        inline int64_t get_max_n_lagged_disk_log_allowed()
        {
            return config_.max_n_lagged_disk_log_allowed;
        }

        //add :e
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
        common::ObGeneralRpcStub & get_rpc_stub();
        const common::ObServer& get_sys_ups();
        void get_ms(ObServer &ms)
        {return config_mgr_.get_ms(ms);}
        //add:20150701:e
        void set_log_sync_delay_stat_param();
        void set_log_replay_thread_param();

        void set_replay_log_to_queue_param();//add lxb [slave ups optimizer]
        ...
        int submit_major_freeze_accord_hb();
        //add 20150521:e

        int submit_minor_freeze();//add wangyao [minor_freeze]
        ...
        bool check_if_permit_freeze();
        //add 20150615:e
        //add peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150521:b
        bool check_if_need_freeze_for_hb_major();
        //add 20150810:e
        //add peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150810:b
        void major_freeze_accord_hb_guard_unlock();
        //add 20150810:e
        //add peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150811:b
        bool can_process_with_disable_sws(const int pcode);
        //add 20150811:e
        //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160226:b
        bool is_ups_usable(const uint64_t cur_version);
        //add 20160226:e

        void set_minor_freeze_stat(const int32_t freeze_stat)
        { minor_freeze_done_ = freeze_stat; }

        //[588]
        int set_timer_check_memtable_lock();
        void cancel_timer_check_memtable_lock();
      ...
      private:
        ...
        //expr:@fn switch slave to master
        ...
        //int slave_change_role(const bool is_role_chenge);
        ////mod 20150701:e
        //del 20150723:e
        ...
        int master_switch_to_slave(const bool is_role_change);
        //mod 20150701:e
        ...
        int set_timer_get_sys_table_master_ups();
        //add 20150810:e

        int set_timer_minor_freeze();
        ...
        int response_published_transid(int32_t ret_code, ObPacket &pkt,int64_t published_transid, ObDataBuffer &out_buffer);
        // add by e
      ...
      private:
        ...
        int ups_rs_get_memory(const int32_t version, common::ObDataBuffer &in_buff,
                              easy_request_t *req, const uint32_t channel_id, common::ObDataBuffer &out_buff);
        int get_ups_mem_size(const common::ObServer &addr, int64_t &mem_size);

        int get_ups_tmplog_replay_finished(const int32_t version, common::ObDataBuffer &in_buff,
             easy_request_t *req, const uint32_t channel_id, common::ObDataBuffer &out_buff);//add wuwy [handle_unsetted_trans] 20190624
        ...
        int ups_get_memtable_info(const int32_t version, easy_request_t *req, const uint32_t channel_id, ObDataBuffer &out_buff);
        int ups_get_commit_log_status(const int32_t version, common::ObDataBuffer &in_buff,
                                      easy_request_t *req, const uint32_t channel_id, common::ObDataBuffer &out_buff);
      ...
      public:
        ...
        /**
         * @brief construct mutator by transid and commit,
         *        include seralize and deserialize mutator,
         *        mutator need deserialize before apply
         * @param [out]mutator
         * @param trans_id
         * @param commit
         * @return
         */
        int ui_set_mutator(ObMutator &mutator, const ObTransID &trans_id, const bool commit);
        /**
         * @brief construct mutator by transid and commit
         * @param trans_id
         * @param commit
      ...
      private:
        ...
        //yynet::PacketQueueThread read_thread_queue_; // for read task
        ...
        ObUpsReplayLogRunnable replay_log_to_queue_thread_;
        ObUpsLoadLogRunnable  async_load_log_thread_;
        ...
        //add peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150521:b
        //value only can be set by hb.last_frozen_version_
        volatile uint64_t last_frozen_version_;
        volatile uint64_t to_be_frozen_version_;  //add peiouya [MultiUPS] [UPS_Manage_Function] 20150615
        //add 20150521:e
        ...
        common::ObServer sys_ups_master_;
        //mod 20150701:e
        ...
        //add peiouya [MultiUPS] [DELETE_OBI] 20150701:b
        ObUpsGetSysUpsMaterDuty get_sys_ups_master_duty_;
        //add 20150701:e
        KillZombieDuty kill_zombie_duty_;
        MinorFreezeDuty  minor_freeze_duty_;//add wangyao[minor_freeze]��20181214
        ...
        //[588]
        MemtableLockDuty memtable_lock_duty_;
        ...
        //add peiouya [MultiUPS] [DELETE_OBI] 20150701:b
        ObSafeOnceGuard get_sys_ups_master_guard_;
        //add 20150701:e
        //add peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150810:b
        ObSafeOnceGuard major_freeze_accord_hb_guard_;
        //add 20150810:e
        ...
        //add pangtianze [Paxos] 20170811:b
        yysys::CThreadMutex regist_rs_mutex_;
        //add:e
        ...
        //add:e
		 common::ObGeneralRpcStub rpc_stub_;//add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701
        //add pangtianze [Paxos rs_election] 20150708:b
        ...
        //add:e
        uint64_t offline_version_;//add liuzy [MultiUPS] [take_paxos_offline_interface] 20160226
         //add shili [LONG_TRANSACTION_LOG]  20160926:b
