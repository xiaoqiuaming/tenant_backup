  ...
  namespace updateserver
  {
    int ObSingleTableScanQuery::query(BaseSessionCtx* session_ctx, ITableEntity* table_entity, ITableIterator* iter)
    {
      ...
      else if (NULL == session_ctx || NULL == table_entity || NULL == iter)
      {
        ...
        YYSYS_LOG(ERROR, "query(session=%p, table_entity=%p, iter=%p):INVALID_ARGUMENT", session_ctx, table_entity, iter);
      }
      else if (OB_SUCCESS != (err = table_entity->scan(*session_ctx, *scan_param_, iter)))
      {
        YYSYS_LOG(WARN, "table entity scan fail ret=%d scan_range=%s", err, scan_range2str(*scan_param_->get_range()));
      }
      return err;
    }
    ...
    int ObSingleTableGetQuery::query(BaseSessionCtx* session_ctx, ITableEntity* table_entity, ITableIterator* iter)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "query([%ld, %ld]):INVALID_ARGUMENT", start_idx_, end_idx_);
      }
      else if (NULL == (cell = (*get_param_)[start_idx_]))
      {
        YYSYS_LOG(WARN, "cellinfo null pointer idx=%ld", start_idx_);
        err = OB_ERROR;
      }
      ...
      else
      {
        ...
        if (column_filter->get_read_atomic_param().is_valid())
        {
          YYSYS_LOG(DEBUG,"read_atomic::debug,ObSingleTableGetQuery's read atomic param=[%s]!",
                    to_cstring(column_filter->get_read_atomic_param()));
        }
        //add duyr 20151207:e
                           ...
                           uint64_t& max_valid_version, bool& is_final_minor, uint64_t table_id)
    {
      ...
      if (OB_SUCCESS != (err = table_mgr.acquire_table(version_range, max_valid_version, table_list, is_final_minor, table_id))
          || 0 == table_list.size())
      {
        YYSYS_LOG(WARN, "acquire table fail version_range=%s, err=%d", range2str(version_range), err);
      ...
      else
      {
        ...
        for (iter = table_list.begin(); iter != table_list.end(); iter++, index++)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid table_entity version_range=%s", range2str(version_range));
            err = OB_ERROR;
            break;
          }
        }
    ...
    int ObTableListQuery::open(BaseSessionCtx* session_ctx, ObUpsTableMgr* table_mgr, const ObVersionRange& version_range, uint64_t table_id)
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "open():open twice.");
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "acquire table fail, err=%d", err);
        }
        else
        {
    ...
    int ObTableListQuery::query(IObSingleTableQuery* table_query, ObRowDesc* row_desc, ObPhyOperator*& result)
    {
      ...
      if (NULL == table_query || NULL == row_desc)
      {
        ...
        YYSYS_LOG(ERROR, "table_query == %p || NULL == %p", table_query, row_desc);
      }
      else if (NULL == ret_iter)
      {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(WARN, "merger null point, maybe allocate from resource_pool fail");
      }
      guard_->reset();
      merge_child_idx = 0;
      for (iter = table_list.begin(); OB_SUCCESS == ret && iter != table_list.end(); iter++, index++)
      {
        ...
        if (NULL == table_entity)
        {
          YYSYS_LOG(WARN, "invalid table_entity");
          ret = OB_ERROR;
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "alloc table row iterator fai index=%ld", index);
          ret = OB_MEM_OVERFLOW;
          break;
        }
        ...
        if (OB_SUCCESS != (ret = table_query->query(session_ctx_, table_entity, table_iter)))
        {
          if (!IS_SQL_ERR(ret))
          {
            YYSYS_LOG(WARN, "table entity query fail ret=%d", ret);
          }
          break;
        }
        sst_id = (NULL == table_entity) ? 0 : table_entity->get_table_item().get_sstable_id();
