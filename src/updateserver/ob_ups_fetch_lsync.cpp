         ...
         common::ObRoleMgr *role_mgr)
{
  ...
  {
    YYSYS_LOG(WARN, "Parameters are invalid in ObUpsFetchLsync::init, log_id=%lu log_seq=%lu rpc_stub=%p clog_receiver=%p",
...
void ObUpsFetchLsync::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  int err = OB_SUCCESS;
  ...
  if (!is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObUpsFetchLsync has not initialized");
  ...
  else
  {
    ...
    {
      YYSYS_LOG(DEBUG, "fetch lsync server: log_id_=%lu log_seq_=%lu", log_id_, log_seq_);
      ...
      {
        YYSYS_LOG(ERROR, "fetch_lsync error, err=%d log_id_=%lu log_seq_=%lu log_data=%p log_len=%ld",
      ...
      else if (OB_SUCCESS == err)
      {
        if (log_len > 0)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "ObCommitLogReceiver write_data error, err=%d log_data=%p log_len=%ld log_id_=%lu log_seq_=%lu",
                err, log_data, log_len, log_id_, log_seq_);
          }
          else
          {
            YYSYS_LOG(DEBUG, "write_data to log, log_len=%ld log_id_=%lu log_seq_=%lu", log_len, log_id_, log_seq_);
          }
        }
        else
        {
          YYSYS_LOG(INFO, "lsync returned 0 length log data");
        }
      }
      else if (OB_NOT_REGISTERED == err)
      {
        YYSYS_LOG(INFO, "The lsync server returned NOT_REGISTERED, go to register");
      ...
      {
        YYSYS_LOG(WARN, "The lsync server returned err=%d", err);
        err = OB_SUCCESS;
      }
    }
  }

  YYSYS_LOG(INFO, "ObUpsFetchLsync finished, err=%d role_state=%s", err, role_mgr_->get_state_str());
}
