  ...
  namespace common
  {
    namespace KVStoreCacheComponent
    {
                 ...
                 char *buffer,ObUpsCacheValueDeepCopyTag)
      {
        ...
        {
          common::ObDecimal value;
          other.value.get_decimal(value);
          buf = buf + sizeof(updateserver::ObUpsCacheValue);
          uint32_t p, s, vs, len;
          len = other.value.get_nwords();
          uint64_t *tmp = reinterpret_cast<uint64_t *>(buf);
          memcpy(tmp,value.get_words()->ToUInt_v2(),len * sizeof(uint64_t));
          p = value.get_precision();
          s = value.get_scale();
          vs = value.get_vscale();
          ret->value.set_decimal(tmp, p, s, vs, len);
        }
        //add:e
        else
        {
        ...
        explicit ObBufferHandle(char *buffer):ups_cache_(NULL),buffer_(buffer)
      {
        ...
        ObBufferHandle &operator=(const ObBufferHandle &other)
        {
          ...
          if(NULL != other.ups_cache_)
          {
            ...
            {
              YYSYS_LOG(ERROR,"copy handle fail this ups_cache=%p this buffer=%p other_ups_cache=%p other_buffer=%p",ups_cache_, buffer_, other.ups_cache_, other.buffer_);
              ups_cache_ = NULL;
              buffer_ = NULL;
            }
          }
