  ...
  namespace updateserver
  {
    ...
    int ObCellInfoNodeIterableWithCTime::next_cell()
    {
              ...
              && NULL != (list_iter_ = list_iter_->next))
      {
        ...
        if (OB_INVALID_ID == rkinfo_table_id_)
        {
          ret = OB_SCHEMA_ERROR;
          YYSYS_LOG(WARN,"fail to get rowkey info from cache, ret=%d", ret);
        }
        else if (rkinfo_.is_rowkey_column(column_id_))
        {
          is_from_te_key_ = true;
        }
    ...
    void ObCellInfoNodeIterableWithCTime::set_rowkey_column(const uint64_t table_id, const ObRowkey &row_key, const CommonSchemaManager *sm, int64_t query_version)
    {
      const ObRowkeyInfo *rki = get_rowkey_info(table_id, sm, query_version);
      if (NULL == row_key.ptr())
      {
        //YYSYS_LOG(WARN, "get rowkey info fail table_id=%lu rowkey_info=%p rowkey=%p %s", table_id, rki, row_key.ptr(), to_cstring(row_key));
        inner_err = OB_SCHEMA_ERROR;
      }
      //add [214]:b
      else if(NULL == rki)
      {
          inner_err = OB_UPS_TABLE_NOT_EXIST;
      }
      //add:e
      else
      {
        for (int64_t i = row_key.length() - 1; i >= 0; i--)
        {
          ...
          {
            YYSYS_LOG(WARN, "get rowkey column id fail index=%ld table_id=%lu", i, table_id);
            inner_err = OB_SCHEMA_ERROR;
          }
        }
      }
    ...
    int TEValueStmtCallback::cb_func(const bool rollback, void *data, BaseSessionCtx &session)
    {
      ...
      else if (!rollback)
      {
        ...
        // add by maosy [multi_delete_update do not read own query ]20170117
        ObCellInfoNode *iter = uci->uc_list_head;
        while (NULL != iter)
        {
            if (OB_INVALID_VERSION == iter->modify_time)
            {
                if (uci->uc_list_tail != iter)
                {
                    ret = OB_ERROR ;//todo
                    YYSYS_LOG(ERROR,"last tailf can be ob_invailed version ,cell node = %p",iter);
                }
                else
                {
                    iter->modify_time = INT64_MAX;
                }
                break;
            }
            if(uci->uc_list_tail == iter)
      ...
      {
        YYSYS_LOG(DEBUG, "rollback: uc_list_tail=%p", uci->uc_list_tail_before_stmt);
      ...
      else
      {
        ...
        if (!rollback)
        {
          ...
          while (NULL != iter)
          {
            ...
            {
              //YYSYS_LOG(INFO, "user supply ctime %ld", iter->modify_time);
            }
             //add duyr [MultiUPS] [READ_ATOMIC] [write_part] 20151117:b
             YYSYS_LOG(DEBUG,"read_atomic::debug,final distributed trans ver=[%s]",
                                to_cstring(*((common::ObTransVersion *)iter->trans_version_)));
             //add duyr 20151117:e
            YYSYS_LOG(DEBUG, "set value=%p node=%p modify_time=%ld", uci->value, iter, iter->modify_time);
            if (uci->uc_list_tail == iter)
            {
              break;
            }
          ...
          YYSYS_LOG(DEBUG, "commit value=%p %s "
          ...
          if (ATOMIC_CAS(&(uci->value->cur_uc_info), uci, NULL) != uci)
          {
            ...
            YYSYS_TRACE_LOG("reset cur_uc_info to NULL, te_value=%p uci=%p", uci->value, uci);
          }
        }
        else
        {
          YYSYS_LOG(DEBUG, "rollback value=%p %s "
                    "cur_uc_info=%p uc_list_head=%p uc_list_tail=%p "
                    "session=%p sd=%u",
                    uci->value, uci->value->log_list(),
                    uci->value->cur_uc_info, uci->uc_list_head, uci->uc_list_tail,
    ...
    //add duyr [MultiUPS] [READ_ATOMIC] [write_part] 20151117:b
    int TEValueTransverCallback::cb_func(const bool rollback, void *data, BaseSessionCtx &session)
    {
      int ret = OB_SUCCESS;
      common::ObTransVersion trans_ver;
      TEValueUCInfo *uci = (TEValueUCInfo*)data;
      const common::ObTransID& coor_sid = session.get_coordinator_info();

      if (!coor_sid.is_valid())
      {//it's a single transaction!
        trans_ver.ups_ = ObUpdateServerMain::get_instance()->get_update_server().get_self();
        trans_ver.descriptor_ = session.get_session_descriptor();
        trans_ver.start_time_us_ = session.get_session_start_time();
        ret = trans_ver.set_trans_dist_type(common::ObTransVersion::SINGLE_POINT_TRANS);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN,"fail to set single point trans type!ret=%d",ret);
        }
      }
      else
      {//it's a distribute transaction!
        trans_ver.assign(coor_sid);
        ret = trans_ver.set_trans_dist_type(common::ObTransVersion::DISTRIBUTED_TRANS);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN,"fail to set dist trans type!ret=%d",ret);
        }
      }
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"invalid trans ver!ret=%d,ver=[%s]",
                  ret,to_cstring(trans_ver));
      }
      else
      {
        bool need_unlock = false;
        ...
        if (!rollback)
        {
          ...
          {
            if (NULL == iter->trans_version_)
            {
              ret = OB_ERR_UNEXPECTED;
              YYSYS_LOG(ERROR,"trans_version_ should not be NULL!ret=%d",ret);
              break;
            }
            else
            {
              char *des_trans_ver_buf = iter->trans_version_;
              int64_t buf_length      = static_cast<int64_t>(sizeof(iter->trans_version_));
              if (OB_SUCCESS != (ret = common::ObReadAtomicHelper::transver_copy_to_buf(
                                                                  trans_ver,
                                                                  des_trans_ver_buf,
                                                                  buf_length)))
              {
                YYSYS_LOG(WARN,"fail to copy coor trans id to uc_list!ret=%d",ret);
                break;
              }
              else
              {
                YYSYS_LOG(DEBUG,"read_atomic::debug,callback function set distributed trans ver=[%s]",
                          to_cstring(*((common::ObTransVersion *)des_trans_ver_buf)));
              }
            }
