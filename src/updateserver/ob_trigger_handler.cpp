...
int ObTriggerCallback::init(ObTriggerHandler * handler)
{
  ...
  if (init_)
  {
    YYSYS_LOG(ERROR, "ObTriggerCallback %p has been initialized", this);
    ret = OB_INIT_TWICE;
  }
  else if (NULL == handler)
  {
    YYSYS_LOG(ERROR, "Parameter is invalid, handler is NULL");
    ...
    BaseSessionCtx &session)
{
  ...
  if (!rollback)
  {
    ...
    if (handler_ != NULL && session_ctx != NULL)
    {
      ...
      {
        YYSYS_LOG(ERROR, "notify the trigger handler failed. ret=%d", ret);
      }
    }
  }
  return ret;
...
int ObTriggerHandler::init(common::ObServer &rootserver, ObUpsRpcStub *rpc_stub, ObUpsRoleMgr *role_mgr)
{
      ...
      NULL == (role_mgr_ = role_mgr))
  {
    ...
    YYSYS_LOG(ERROR, "init trigger handler failed. invalid arguments. rpc_stub=%p, role_mgr=%p", rpc_stub, role_mgr);
  }
  else if (OB_SUCCESS != (err = callback_.init(this)))
  {
    YYSYS_LOG(ERROR, "ObTriggerCallback init error, err = %d", err);
  }
  return err;
}
...
int ObTriggerHandler::handle_trigger(ObUpsMutator &mutator) const
{
    ...
    // && ObUpsRoleMgr::ACTIVE == role_mgr_->get_state()) slave-master maybe not-sync state
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        // YYSYS_LOG(INFO, "GOT a trigger event on table %lu, OB_TRIGGER_EVENT_TABLE_TID=%lu",
        //    cell->cell_info.table_id_, OB_TRIGGER_EVENT_TID);
        if (OB_TRIGGER_EVENT_TID == cell->cell_info.table_id_)
        {
          ObTriggerMsg msg;
          // got a trigger event
          YYSYS_LOG(INFO, "GOT a trigger event, check if msg valid");
          // src
          if (OB_SUCCESS != (err = cell->cell_info.value_.get_varchar(msg.src)))
          {
            YYSYS_LOG(WARN, "fail to get trigger src. err=%d", err);
          }
          else if (OB_SUCCESS != (err = next_value_(mutator, msg.type)))
          {
            YYSYS_LOG(WARN, "fail to get trigger type. err=%d", err);
            if (OB_ITER_END == err)
            {
          ...
          {
            YYSYS_LOG(WARN, "fail to get trigger param. err=%d", err);
            if (OB_ITER_END == err)
            {
              err = OB_ITEM_NOT_SETTED;
            }
        ...
        {
          ObDdlTriggerMsg msg;
          YYSYS_LOG(INFO, "GOT a ddl trigger event, check if msg valid");
          ...
          if (cell->cell_info.row_key_.get_obj_cnt() == 1 )
          {
            ...
            {
              err = OB_ITEM_NOT_SETTED;
              YYSYS_LOG(WARN, "fail to get ddl trigger version. err=%d", err);
            }
            ...
            {
              YYSYS_LOG(INFO, "succeed to get ddl trigger version. version=%ld, param =%ld", msg.version, msg.param);
              err = handle_trigger(msg);
            }
          }
          else
          {
            ...
            YYSYS_LOG(WARN, "fail to get ddl trigger version, rowkey count[%ld]. err=%d", cell->cell_info.row_key_.get_obj_cnt(),err);
          }
        }
        //add:e
      }
...
int ObTriggerHandler::next_value_(ObUpsMutator &mutator, int64_t &value) const
{
  ...
  if (OB_SUCCESS != (err = mutator.next_cell()))
  {
    if (OB_ITER_END != err)
    {
      YYSYS_LOG(WARN, "fail to move to next cell. err=%d", err);
    }
  }
...
{
  YYSYS_LOG(INFO, "[TRIGGER][handle_trigger]send TriggerMsg to Rootserver. msg(src:%.*s, type:%ld, param:%ld)",
...
{
  YYSYS_LOG(INFO, "[TRIGGER][handle_trigger]send Ddl TriggerMsg to Rootserver. msg(version:%ld, type:%ld, param:%ld)",
