...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    //add wangdonghui [Paxos ups_replication] 20170215 :b
#define UPS ObUpdateServerMain::get_instance()->get_update_server()
    ...
    RWSessionCtx::RWSessionCtx(const SessionType type,
                               ...
                               const bool need_gen_mutator) : BaseSessionCtx(type, host),
                                                               ...
                                                               trans_version_callback_list_(),
                                                               writting_trans_stat_(false),
                                                               accept_prepare_ack_(false),
                                                               participant_num_(0),
                                                               mem_version_(0),
                                                               last_write_trans_stat_time_(0),
                                                               src_task_(NULL),
                                                               commit_stat_(false)
    {
      is_someone_prepare_failed_ = false;
    ...
    RWSessionCtx::~RWSessionCtx()
    {
      ...
      if (NULL != src_task_)
      {
        YYSYS_LOG(ERROR, "still some mem not free");
      }
      //add 20150701:e
    }
    ...
    void RWSessionCtx::end(const bool need_rollback)
    {
      if (!commit_done_)
      {
        ...
        backfill_trans_version(need_rollback);
        //add duyr 20151117:e
    ...
    {
      YYSYS_LOG(DEBUG, "session alloc %p size=%ld", this, size);
      //add zhaoqiong [fifo allocator bug] 20161206:b
      if (page_arena_.total() > 512*1024*1024)
      {
        YYSYS_LOG(TRACE, "session alloc memory too large: id[%d], trans_id[%ld] alloc %p cur_size=%ld",get_session_descriptor(), get_trans_id() , this, page_arena_.total());
      }
      //add:e
    ...
    void RWSessionCtx::reset()
    {
      ...
      //add duyr [MultiUPS] [READ_ATOMIC] [write_part] 20151117:b
      trans_version_callback_list_.reset();
      //add duyr 20151117:e
      ...
      accept_prepare_ack_ = false;
      table_array_.clear();
      coordinator_tid_.reset();
      participant_num_ = 0;
      mem_version_ = 0;
      last_write_trans_stat_time_ = 0;
      for(int i = 0; i< common::MAX_UPS_COUNT_ONE_CLUSTER; ++i)
      {
        participant_trans_id_[i].reset();
        participant_ack_[i]= false;
      }
      is_someone_prepare_failed_ = false;
      src_task_ =  NULL;
      commit_stat_ = false;
    ...
    int RWSessionCtx::check_truncate_conflict(uint64_t &table_id, bool &is_conflict)
    {
        ...
        {
            int64_t i = 0;
            for(; i < table_array_.count() && err == OB_SUCCESS && !is_conflict; i++)
            {
            ...
            if(err != OB_SUCCESS)
            {
                YYSYS_LOG(ERROR,"failed to check_truncate_conflict,ret=%d", err);
                ret = err;
            }
        }
        return ret;
    ...
    int RWSessionCtx::is_truncate_conflict(uint64_t table_id, bool &is_conflict)
    {
                          ...
                          (get_session_descriptor(), table_id, last_truncate_timestamp)))
        {
            ...
            {
                YYSYS_LOG(WARN,"[table_id=%ld] is currently being truncated, ret=%d", table_id, ret);
            }
            else
            {
                YYSYS_LOG(WARN,"failed to get_last_truncate_timestamp_in_active[table_id=%ld], ret=%d", table_id, ret);
            }
        }
        else if((last_truncate_timestamp != INT64_MAX) && (last_truncate_timestamp > get_trans_id())) //[631] mod
        {
            is_conflict = true;
            YYSYS_LOG(INFO,"[table_id=%ld]session_start_time_=%ld, last_truncate_timestamp=%ld", table_id, get_session_start_time(),last_truncate_timestamp);
        }
        return ret;
    }
    ...
    int RWSessionCtx::init_lock_info(LockMgr& lock_mgr, const IsolationLevel isolation)
    {
      ...
      {
        YYSYS_LOG(WARN, "assign lock_info fail");
        ret = OB_MEM_OVERFLOW;
      }
      else if (OB_SUCCESS != (ret = lock_info_->on_trans_begin()))
      {
        YYSYS_LOG(WARN, "invoke on_trans_begin fail ret=%d", ret);
      }
      return ret;
    }
    ...
    void RWSessionCtx::kill()
    {
      ...
      if (ObUpsRoleMgr::MASTER != UPS.get_role_mgr().get_role())
      {
          stat_ = ST_KILLING;
          alive_flag_ = false;
          YYSYS_LOG(INFO, "session is being killed sd=%u stat=%d session_start_time=%ld stmt_start_time=%ld session_timeout=%ld stmt_timeout=%ld",
      ...
      //mod:e
      {
        YYSYS_LOG(WARN, "session will not be killed sd=%u stat=%d session_start_time=%ld stmt_start_time=%ld session_timeout=%ld stmt_timeout=%ld",
                  get_session_descriptor(), stat_, get_session_start_time(), get_stmt_start_time(), get_session_timeout(), get_stmt_timeout());
      }
      else
      {
    ...
    void RWSessionCtx::set_frozen()
    {
      ...
      {
        YYSYS_LOG(INFO, "session has been set frozen, will not be killed, sd=%u", get_session_descriptor());
      }
    }
    ...
    //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
    void RWSessionCtx::set_prepare()
    {
      Stat old_stat = stat_;
      stat_ = ST_PREPARE;
      if (ST_KILLING == old_stat)
      {
        YYSYS_LOG(INFO, "session has been set prepare, will not be killed, sd=%u", get_session_descriptor());
      }
    }

    bool RWSessionCtx::is_prepare() const
    {
      return (ST_PREPARE == stat_);
    }

    int RWSessionCtx::set_participant_info(const common::ObEndTransReq &req)
    {
      int ret = OB_SUCCESS;
      if (req.participant_num_ <=0 || req.participant_num_ > common::MAX_UPS_COUNT_ONE_CLUSTER)
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "participant num is invalid, num=%d", req.participant_num_);
      }
      else
      {
        participant_num_ = req.participant_num_;
        mem_version_ = req.memtable_version_;
        for(int i = 0; i < participant_num_; ++i)
        {
          participant_trans_id_[i] = req.participant_trans_id_[i];
          participant_ack_[i] = false;
          YYSYS_LOG(DEBUG, "num:%d, req:%s", i, to_cstring(req.participant_trans_id_[i]));
        }
      }
      return ret;
    }

    void RWSessionCtx::set_participant_ack(const common::ObServer &participant)
    {
      for(int i = 0; i< participant_num_; ++i)
      {
        if (participant == participant_trans_id_[i].ups_)
        {
          participant_ack_[i] = true;
          break;
        }
      }
    }

    bool RWSessionCtx::all_participant_ack()
    {
      bool ret = true;
      for (int i = 0; i < participant_num_; ++i)
      {
        if(!participant_ack_[i])
        {
          ret = false;
          break;
        }
                                             ...
                                             ctx_allocator_()
    {
      ...
      {
        YYSYS_LOG(ERROR, "init allocator fail");
      }
      else
      {
    ...
    BaseSessionCtx *SessionCtxFactory::alloc(const SessionType type, SessionMgr &host)
    {
      ...
      case ST_READ_WRITE:
        ...
        break;
      default:
        YYSYS_LOG(WARN, "invalid session type=%d", type);
        break;
      }
      return ret;
    }
