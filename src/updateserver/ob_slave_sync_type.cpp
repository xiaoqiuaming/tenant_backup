...
namespace oceanbase
{
  namespace updateserver
  {
    ...
    DEFINE_SERIALIZE(ObSlaveSyncType)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to serialize slave_type. err=%d", ret);
      }
      return ret;
    }
    ...
    DEFINE_DESERIALIZE(ObSlaveSyncType)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to deserialize slave_type, err=%d", ret);
      }
      return ret;
    }
    ...
    {
       atomic_exchange(reinterpret_cast<uint32_t*>(&consistency_type_), static_cast<uint32_t>(type));//mod support for arm platform by wangd 202106
    }
