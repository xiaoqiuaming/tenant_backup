...
namespace oceanbase 
{
  namespace chunkserver 
  {
                               ...
                               ObIterator* it_out[],int64_t& it_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get data from local chunk server, ret=%d", ret);
      }
      else
      {
        it_out[idx++] = &scanner;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN,"alloc scanner failed"); //just warn
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN,"get data from compact sstable failed,err=%d",ret);
          }
          else if (idx < it_size)
          {
                                ...
                                ObIterator* it_out[],int64_t& it_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to scan data from local chunk server, ret=%d", ret);        
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to get thread local sstable scanner, "
                           "sstable_scanner=%p", 
                    sstable_scanner);
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get tablet range from scanner, ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (compactsstable_num > 0)
        {
          ...
          {
            YYSYS_LOG(WARN,"unexpect error,mem_node=%p,cf=%p,its=%p",mem_node,cf,its);
            ret = OB_ALLOCATE_MEMORY_FAILED;
          }
          else
          {
          ...
          {
            YYSYS_LOG(WARN,"build columnfilter failed");
            ret = OB_ERROR;
          }
          ...
          for(i=0; (mem_node != NULL) && (i < compactsstable_num) && (OB_SUCCESS == ret) && (it_num < it_size);++i)
          {
            ...
            if (NULL == mem)
            {
              YYSYS_LOG(WARN,"unexpect error,compact mem is null");
              ret = OB_ERROR;
            }
            else
            {
                                            ...
                                            ObScanner& orig_scanner,ObScanner& compact_scanner)
    {
      ...
      {
        YYSYS_LOG(WARN,"get thread context of get failed");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (ObVersion::compare(compactsstable_version,orig_scanner.get_data_version()) > 0)
        {
          ...
          for(int64_t i=0; (i < row_num) && (OB_SUCCESS == ret); ++i)
          {
            ...
            {
              YYSYS_LOG(WARN,"get data from compactsstable failed,ret=%d",ret);
            }
          }
        }
      }
                                        ...
                                        ObScanner& compact_scanner)
    {
            ...
            (mem_node != NULL) && (m < compactsstable_num); )
      {
        ...
        if (NULL == mem)
        {
          YYSYS_LOG(WARN,"unexpect error,compactsstable is null");
          ret = OB_ERROR;
        }
        else if (ObVersion::compare(mem->get_data_version(),compactsstable_version) <= 0)
        {
          ...
          if (!mem->is_row_exist(rowkey))
          {
            ...
            YYSYS_LOG(DEBUG,"row not exist,%s", to_cstring(rowkey));
            add_row_not_exist = true;
          }
          else if ((ret = its->iters_[m].init(mem)) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN,"init iterator failed,ret=%d",ret);
          }
          else if ((ret = its->iters_[m].set_get_param(rowkey,cf)) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN,"set get param failed,ret=%d",ret);
          }
          else if ((ret = merger.add_iterator(&(its->iters_[m]))) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN,"add iterator to merger failed,ret=%d",ret);
          }          
          else
          {
            ++m;
      ...
      if ((m > 0) && (OB_SUCCESS == ret))
      {
        ...
        {
          YYSYS_LOG(WARN,"fill compact data failed,ret=%d",ret);
        }
      }
      else if ((OB_SUCCESS == ret) && add_row_not_exist)
      {
        ...
        {
          YYSYS_LOG(WARN,"add not exist cell failed,ret=%d",ret);
        }
      }
      return ret;
    }
    ...
    int ObGetScanProxy::fill_compact_data(ObIterator& iterator,ObScanner& scanner)
    {
      ...
      while (OB_SUCCESS == (ret = iterator.next_cell()))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get cell, cell=%p, err=%d", cell, ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to add cell to scanner, ret=%d", ret);
          }
        }
      ...
      {
        YYSYS_LOG(WARN, "retor occurs while iterating, ret=%d", ret);
      }
      return ret;
    }
