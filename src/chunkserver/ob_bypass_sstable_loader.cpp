...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObBypassSSTableLoader::init(ObTabletManager* manager)
    {
      ...
      if (NULL == manager)
      {
        YYSYS_LOG(WARN, "invalid param, tablet manager is NULL");
      ...
      const ObTableImportInfoList& table_list)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "bypass sstable loader isn't initialized");
        ret = OB_ERROR;
      }
      else if (table_list.tablet_version_ < 1)
      {
        YYSYS_LOG(WARN, "invalid param, load_version=%ld",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "get disk no array failed, disk_no_array_=%p, "
                          "disk_count_=%d",
    ...
    int ObBypassSSTableLoader::finish_load()
    {
      ...
      YYSYS_LOG(INFO, "finish scanning bypass sstable directory, start load "
      ...
      if (tablet_array_.size() > 0)
      {
        ...
        if (is_load_succ_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to add bypass tablets into serving tablet image");
            is_load_succ_ = false;
          }
          else if (OB_SUCCESS != (ret = tablet_manager_->sync_all_tablet_images()))
          {
            YYSYS_LOG(WARN, "failed to sync all tablet images after load bypass sstables, "
        ...
        {
          YYSYS_LOG(WARN, "failed to report load result to rootserver after loading "
      ...
      {
        YYSYS_LOG(WARN, "no bypass sstable was imported, tablet_count=%d",
          tablet_array_.size());
      }
      is_finish_load_ = true;
      YYSYS_LOG(INFO, "finish loading bypass sstables, import_tables_info=%s",
        to_cstring(*table_list_));
    ...
    {
      YYSYS_LOG(INFO, "load failed, start rollback, import_tables_info=%s",
        to_cstring(*table_list_));
    ...
    int ObBypassSSTableLoader::recycle_tablet(ObTablet* tablet, bool only_recycle_removed_tablet)
    {
      ...
      if (NULL == tablet)
      {
        YYSYS_LOG(WARN, "invalid param, tablet is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
               ...
               || (only_recycle_removed_tablet && tablet->is_removed()))
      {
        ...
        if (tablet->get_sstable_id_list().count() > 0)
        {
          ...
          if (OB_SUCCESS == tablet_image.include_sstable(sstable_id))
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to remove bypass tablet, sstable_id=%lu, range=%s",
        ...
        if (0 == tablet->dec_ref())
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to remove sstable of tablet, range=%s",
    ...
    int ObBypassSSTableLoader::recycle_bypass_dir()
    {
      ...
      YYSYS_LOG(INFO, "recycle all sstables in bypass directory");
      ...
      int32_t disk_no, const char* file_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, disk_no=%d, file_name=%p",
          disk_no, file_name);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (need_import(file_name))
      {
        ...
        {
          YYSYS_LOG(ERROR, "can't get bypass sstable path, disk_no=%d, sstable_name=%s",
            disk_no, file_name);
        }
        else if (0 != ::unlink(byapss_sstable_path))
        {
          YYSYS_LOG(ERROR, "failed to unlink sstable file, sstable_file=%s, "
                           "errno=%d, err=%s",
    ...
    void ObBypassSSTableLoader::run(CThread* thread, void* arg)
    {
      ...
      YYSYS_LOG(INFO, "load bypass sstables thread start run, thread_index=%ld",
        thread_index);
      while(!_stop)
      {
        cond_.lock();
    ...
    int ObBypassSSTableLoader::load_bypass_sstables(const int64_t thread_index)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid disk no array or disk count, disk_no_array_=%p, "
      ...
      else
      {
        ...
        for (int64_t i = start_index; i < end_index && i < disk_count_; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to scan bypass sstable file in disk no=%d",
              disk_no_array_[i]);
          ...
          else if (static_cast<uint32_t>(disk_count_) == atomic_inc(&finish_load_disk_cnt_))
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to finish load, is_load_succ=%d", is_load_succ_);
            }
            if (i != end_index - 1)
            {
              YYSYS_LOG(ERROR, "expect that all sstable in all disks_per_disk are loaded, "
      ...
      const int32_t disk_no, Filter filter, Operate op)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "get byapss sstable directory error, disk_no=%d.", disk_no);
      }
      else if (!FileDirectoryUtils::is_directory(directory))
      {
        YYSYS_LOG(ERROR, "byapss sstable dir doesn't exist, dir=%s", directory);
        ret = OB_DIR_NOT_EXIST;
               ...
               || NULL == file_dirent)
      {
        YYSYS_LOG(INFO, "byapss directory=%s doesn't have sstable files.", directory);
      }
      else
      {
        /**
        ...
        for (int64_t n = 0; n < file_num; ++n)
        {
          ...
          {
            YYSYS_LOG(WARN, "scandir return null dirent[%ld]. directory=%s",
              n, directory);
            tmp_err = OB_IO_ERROR;
          }
          else
    ...
    int ObBypassSSTableLoader::do_load_sstable(const int32_t disk_no, const char* file_name)
    {
      ...
      if (is_continue_load_&& need_import(file_name))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "load_one_bypass_sstable failed, disk_no=%d, file_name=%s",
            disk_no, file_name);
      ...
      const int32_t disk_no, const char* file_name)
    {
      ...
      {
        YYSYS_LOG(ERROR, "can't create hard link for bypass sstable, "
                         "disk_no=%d, sstable_name=%s",
          disk_no, file_name);
      }
      else if (OB_SUCCESS != (ret = add_new_tablet(sstable_id, disk_no, tablet)))
      {
        YYSYS_LOG(ERROR, "can't add new tablet for bypass sstable into tablet image, "
                         "disk_no=%d, sstable_name=%s",
          disk_no, file_name);
      }
      else if (NULL != tablet)
      {
        YYSYS_LOG(INFO, "create hard link of bypass sstble=%s to dst sstable=%s, range=%s",
      ...
      const int64_t path_size, const int32_t disk_no, ObSSTableId& sstable_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, bypass_sstable_path=%p, "
      ...
      else if ((sstable_size = get_file_size(bypass_sstable_path)) <= 0)
      {
        if (sstable_size < 0)
        {
          YYSYS_LOG(ERROR, "get file size error, sstable_size=%ld, bypass_sstable_path=%s, err=%s",
              sstable_size, bypass_sstable_path, strerror(errno));
          ret = OB_IO_ERROR;
        }
        else if (0 == sstable_size)
        {
          YYSYS_LOG(ERROR, "can't load empty bypass sstable, bypass sstable size=%ld",
              sstable_size);
          ret = OB_ERROR;
        }
      }
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed create hard link for bypass sstable, "
      ...
      const ObSSTableId& sstable_id, const int32_t disk_no, ObTablet*& tablet)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, sstable_id=%lu, disk_no=%d",
      ...
      {
        YYSYS_LOG(ERROR, "alloc_tablet_object failed, sstable_id=%lu, disk_no=%d, "
                         "load_version=%ld",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "add sstable to tablet failed, sstable_id=%lu, disk_no=%d, "
                           "load_version=%ld",
            sstable_id.sstable_file_id_, disk_no, table_list_->tablet_version_);
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "add tablet to tmp tablet array failed, "
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to load sstable, sstable_id=%lu, disk_no=%d, "
                             "load_version=%ld",
              sstable_id.sstable_file_id_, disk_no, table_list_->tablet_version_);
          }
          else
    ...
    int ObBypassSSTableLoader::add_bypass_tablets_into_image()
    {
      ...
      for (; it != tablet_array_.end(); ++it)
      {
        if (NULL != *it)
        {
          ...
          {
            YYSYS_LOG(ERROR, "add tablet to tablet image failed, range=%s",
