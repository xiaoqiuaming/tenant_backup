...
//          const int64_t time_out = 0);
        ObCellStream(ObMergerRpcProxy * rpc = NULL,
                     const common::ObServerType server_type = common::MERGE_SERVER,
                     const int64_t time_out = 0,
                     const int64_t paxos_group_id = OB_INVALID_PAXOS_ID);
        //mod 20151209:e
    ...
    public:
      ...
      /**
       * for MultiUPS streams,scan all paxos group,each group will be return a stream
       * must be instantiated by subclass
       *
       * @param scan_param [IN]
       * @param it_out [OUT] all cell streams with data
       * @param size [OUT] the count of streams
       */
      virtual int scan(const common::ObScanParam& scan_param, ObIterator *it_out[], int64_t& size);
      //add 20151209:e
    ...
    protected:
      ...
      int rpc_scan_row_data(const common::ObScanParam & param,int64_t paxos_id);
	 //mod 20150701:e
