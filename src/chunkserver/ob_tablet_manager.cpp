...
#define LOG_CACHE_MEMORY_USAGE(header)                                  \
  do  {                                                                 \
    YYSYS_LOG(INFO, "%s cur_serving_idx_ =%ld, mgr_status_ =%ld,"       \
...
namespace oceanbase
{
  namespace chunkserver
  {
                              ...
                              const int64_t max_sstable_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid parameter, data_dir=%s, max_sstable_size=%ld",
        ...
        if (OB_SUCCESS == err)
        {
          if (sstable_row_cache_size > 0)
          {
            ...
            if (NULL != sstable_row_cache_)
            {
              ...
              {
                YYSYS_LOG(ERROR, "init sstable row cache failed");
              }
            }
            else
            {
              YYSYS_LOG(WARN, "failed to new sstable row cache");
              err = OB_ERROR;
            }
          }
        }
    ...
    //mod:e
    {
      ...
      if (NULL == config)
      {
        YYSYS_LOG(ERROR, "invalid parameter, param is NULL");
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      if (OB_SUCCESS == err)
      {
        if (config_->join_cache_size >= 1) // <= 0 will disable the join cache
        {
          ...
          {
            YYSYS_LOG(ERROR, "init join cache failed");
          }
        }
      }
    ...
    int ObTabletManager::build_statistics()
    {
        ...
        if(OB_SUCCESS == ret)
        {
            ...
            {
                YYSYS_LOG(WARN, "failed to schedule statistics collector, ret=[%d]", ret);
            }
        }
        return ret;
    }
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO,"start to build index");
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR,"failed to scan config datadir!");
        }
        else
        {
                                        ...
                                        uint64_t & crc_sum)
    {
      ...
      {
        YYSYS_LOG(ERROR, "error parameters, src_path=%p, dest_path=%p", src_path, dest_path);
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        {
          YYSYS_LOG(ERROR, "acquire tablet error.");
        }
        else if ( range.compare_with_startkey2(tablet->get_range()) != 0
            || range.compare_with_endkey2(tablet->get_range()) != 0)
        {
          YYSYS_LOG(INFO, "migrate tablet range = <%s>", to_cstring(range));
          YYSYS_LOG(INFO, "not equal to local tablet range = <%s>", to_cstring(tablet->get_range()));
          rc = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "acquire tablet  success, tablet=%p", tablet);
        }
      }
      ...
      if (OB_SUCCESS == rc && NULL != tablet)
      {
        ...
        {
          YYSYS_LOG(INFO, "get_migrate_dest_location succeed, dest_disk_no=%d, %s",
              dest_disk_no, dest_dir_buf);
        }
        else
        {
          YYSYS_LOG(ERROR, "get_migrate_dest_location failed, rc = %d, dest_disk_no=%d",
              rc, dest_disk_no);
        }
      }
      ...
      if (OB_SUCCESS == rc && NULL != tablet)
      {
        ...
        YYSYS_LOG(INFO, "migrate_tablet sstable file num =%ld , version=%ld, checksum=%lu",
      ...
      if (OB_SUCCESS == rc && NULL != tablet && num_file > 0)
      {
        ...
        for(int64_t idx = 0; idx < num_file && OB_SUCCESS == rc; idx++)
        {
          ...
          if ( OB_SUCCESS != rc )
          {
            YYSYS_LOG(WARN, "get sstable path error, rc=%d, sstable_id=%ld",
          ...
          {// generate dest path
            ...
            {
              YYSYS_LOG(WARN, "generate dest_filename_buf failed: length[%d] [%ld.%ld]",
            ...
            if (OB_SUCCESS == rc)
            {
              ...
              if (n < 0 || n >= OB_MAX_FILE_NAME_LENGTH)
              {
                YYSYS_LOG(WARN, "generate dest_path failed: length[%d] %s/%ld.%ld",
      ...
      if ( NULL != tablet )
      {
        ...
        if ( OB_SUCCESS != ret )
        {
          YYSYS_LOG(WARN, "release tablet error.");
          rc = OB_ERROR;
        }
        else
        {
      ...
      if (OB_SUCCESS == rc && NULL != src_path && num_file > 0)
      {
        ...
        for(int64_t idx = 0; idx < num_file && OB_SUCCESS == rc; idx++)
        {
          ...
          if (OB_SUCCESS != rc)
          {
            YYSYS_LOG(ERROR, "Transfer sstable failed [%ld]%s to "
        ...
        const int64_t tablet_version, int64_t &old_tablet_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "local merge in upgrade, cannot migrate "
          ...
          || (serving_version > 0 && tablet_version > serving_version + 1))
      {
         ...
         * version
         */
        YYSYS_LOG(WARN, "migrate in tablet = %s version =%ld, local serving version=%ld",
      ...
      {
        YYSYS_LOG(INFO, "tablet intersect with exist, input <%s> and exist <%s>",
            to_cstring(range), to_cstring(old_tablet->get_range()));
        if (tablet_version < old_tablet->get_data_version())
        {
          YYSYS_LOG(WARN, "migrate in tablet's version =%ld < local tablet's version=%ld",
        ...
        else if (tablet_version == old_tablet->get_data_version() + 1)
        {
          ...
          YYSYS_LOG(INFO, "migrate in tablet's version=%ld == "
        ...
        else
        {
           ...
           */
          YYSYS_LOG(WARN, "migrate in tablet's version=%ld > "
                                          ...
                                          const uint64_t crc_sum)
    {
              ...
              ObMultiVersionTabletImage::FROM_NEWEST_INDEX, 0, old_tablet)) )
      {
        ...
        else if (tablet_version == old_tablet->get_data_version())
        {
          ...
          {
            YYSYS_LOG(INFO, "migrate in tablet's version=%ld equal "
          ...
          {
            YYSYS_LOG(ERROR, "migrate in tablet's version=%ld equal "
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (OB_SUCCESS != rc)
        {
          YYSYS_LOG(ERROR, "alloc tablet failed.");
        }
        else
        {
        ...
        for( ; idx < num_file && OB_SUCCESS == rc; idx++)
        {
          ...
          if ( OB_SUCCESS != rc )
          {
            YYSYS_LOG(ERROR, "get sstable path error, rc=%d", rc);
          }
          else
          {
            YYSYS_LOG(INFO, "dest_load_tablet, rename %s -> %s", dest_path[idx], path);
            rc = rename(dest_path[idx], path);
            if (OB_SUCCESS != rc)
            {
              YYSYS_LOG(ERROR, "rename %s -> %s failed, error: %d, %s",
            ...
            if (OB_SUCCESS == rc)
            {
              ...
              if ( OB_SUCCESS != rc )
              {
                YYSYS_LOG(ERROR, "add sstable file  error.");
              }
            }
          }
        }
        ...
        if ( OB_SUCCESS == rc )
        {
          ...
          if(OB_SUCCESS != rc)
          {
            YYSYS_LOG(ERROR, "add tablet <%s> failed, rc=%d", input_range_buf, rc);
          }
          else
          {
            // save image file.
    ...
    int ObTabletManager::merge_multi_tablets(ObTabletReportInfoList& tablet_list)
    {
      ...
      {
        YYSYS_LOG(ERROR, "cannot get ObMultiTabletMerger object");
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to merge multi-tablets");
        }
    ...
    int ObTabletManager::sync_all_tablet_images()
    {
      ...
      if (disk_num > 0 && NULL != disk_no_array)
      {
        ...
        for(int32_t i = 0; i < disk_num; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "disk_no:%d is unavailable", disk_no_array[i]);
          }
        }
      ...
      {
        YYSYS_LOG(WARN, "sync all talbet images invalid argument, "
    ...
    int ObTabletManager::delete_table(const uint64_t table_id)
    {
          ...
          && bypass_sstable_loader_.is_loader_stoped())
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to delete table tablets in tablet image, table_id=%lu", table_id);
        }
      }
    ...
    int ObTabletManager::create_tablet(const ObNewRange& range, const int64_t data_version)
    {
      ...
      {
        YYSYS_LOG(ERROR, "create_tablet error, input range is empty "
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "tablet (%ld) >= input version(%ld) "
              "already exists! dump input and exist:",
              tablet->get_data_version(), data_version);
          range.hex_dump(YYSYS_LOG_LEVEL_ERROR);
          tablet->get_range().hex_dump(YYSYS_LOG_LEVEL_ERROR);
          err = OB_ENTRY_EXIST;
        }
        else
        {
      ...
      {
        YYSYS_LOG(ERROR, "allocate tablet object failed, ret=%d, version=%ld",
            err, data_version);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "create table failed add tablet to image error"
        ...
        else if (OB_SUCCESS != (err = tablet_image_.write(data_version, tablet->get_disk_no())))
        {
          ...
          YYSYS_LOG(ERROR, "create table failed write meta file error"
              ", version=%ld, disk=%d", data_version, tablet->get_disk_no());
        }
      }
    ...
    int ObTabletManager::load_tablets(const int32_t* disk_no_array, const int32_t size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, disk_no_array=%p, size=%d", disk_no_array, size);
        err = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (err =
            get_serving_tablet_image().load_tablets(disk_no_array, size, load_sstable)) )
      {
        YYSYS_LOG(ERROR, "read tablets from disk error, ret=%d", err);
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "load tablets, sstable seq:%ld, load sstable=%d", max_sstable_file_seq_, load_sstable);
      }
      return err;
    }
    ...
    int ObTabletManager::get(const ObGetParam& get_param, ObScanner& scanner)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to get_, err=%d", err);
        }
      }
                                      ...
                                      sstable::ObSSTableGetter *sstable_getter, int64_t &tablet_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "get thread local instance of sstable getter failed");
        err = OB_ERROR;
      }
      else if (row_size <= 0 || row_size > OB_MAX_GET_ROW_NUMBER)
      {
        YYSYS_LOG(WARN, "no cell or too many row to get, row_size=%ld", row_size);
        err = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to acquire tablet, row_size=%ld, "
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to init sstable getter, err=%d", err);
        }
      }
      return err;
    }
                                      ...
                                      ObScanner& scanner)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to gen sstable getter");
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to store cell array to scanner, err=%d", err);
        }
      ...
      {
        YYSYS_LOG(WARN, "failed to release tablets");
        err = OB_ERROR;
      }
        */
#ifdef OB_PROFILER
    ...
    int ObTabletManager::prepare_tablet_image(const int64_t memtable_frozen_version)
    {
      ...
      while (i++ < retry_times)
      {
        ...
        {
          YYSYS_LOG(ERROR, "prepare image version = %ld error ret = %d",
    ...
    int ObTabletManager::prepare_merge_tablets(const int64_t memtable_frozen_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "prepare_for_merge version = %ld error, err = %d",
            memtable_frozen_version, err);
      }
      else if ( OB_SUCCESS != (err = drop_unserving_cache()) )
      {
        YYSYS_LOG(ERROR, "drop unserving cache for migrate cache failed,"
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "prepare_recycle forzen version=%ld, recycle version=%ld",
            memtable_frozen_version, recycle_version);
        if (recycle_version > 0)
        {
          YYSYS_LOG(INFO, "prepare recycle version = %ld", recycle_version);
    ...
    int ObTabletManager::merge_tablets(const int64_t memtable_frozen_version)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        statistics_collector_.set_merging_tag(true);
        while(0 != statistics_collector_.get_active_threads_num())
        {
            usleep(5000*1000);
            retry_times++;
            //DEBUG
    ...
    int ObTabletManager::report_capacity_info()
    {
      ...
      if (!is_init_)
      {
        YYSYS_LOG(ERROR, "report_capacity_info not init");
        err = OB_NOT_INIT;
      }
      else if (cur_serving_idx_ >= TABLET_ARRAY_NUM)
      {
        YYSYS_LOG(ERROR, "report_capacity_info invalid status, cur_serving_idx=%ld", cur_serving_idx_);
        err = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "failed to report capacity info, err=%d", err);
      }
    ...
    //add zhuyanchao[secondary index static index build.report]20150413
     ...
     {
       //YYSYS_LOG(WARN, "test::zhuyanchao begin to report global index tablet info ");
       int err = OB_SUCCESS;
       ObTablet* tablet = NULL;
       ...
       */
       ...
       {
         YYSYS_LOG(WARN, "failed to scan tablets, tablet=%p, err=%d",
                   tablet, err);
         err = OB_ERROR;
       }
       else
       {
         while (OB_SUCCESS == err)
         {
           while (OB_SUCCESS == err && num > 0)
           {
             ...
             {
               YYSYS_LOG(WARN, "failed to get next tablet, err=%d, tablet=%p",
                   err, tablet);
               err = OB_ERROR;
             }
             ...
             {
               YYSYS_LOG(WARN, "report: ignore removed range = <%s>", to_cstring(tablet->get_range()));
               image.release_tablet(tablet);
               continue;
             }
             ...
             else
             {
               ...
               {
                 YYSYS_LOG(WARN, "report illegal tablet range = <%s>", to_cstring(tablet->get_range()));
               }
               /*del liumz, [NULL pointer coredump]20160406
               ...
               */
               ...
               YYSYS_LOG(INFO, "add tablet <%s>, row count:[%ld], size:[%ld], "
               ...
               //    YYSYS_LOG(INFO,"add tablet<%s>,column checksum[%s] to report list",to_cstring(tablet_info.tablet_info_.range_),
               //          tablet_info.column_checksum_->get_str());
               //add e
               err = report_info_list->add_tablet(tablet_info);
               if (OB_SUCCESS != err)
               {
                 YYSYS_LOG(WARN, "failed to add tablet info, num=%ld, err=%d", num, err);
               }
               else
               {
               ...
               {
                 YYSYS_LOG(WARN, "failed to release tablet, tablet=%p", tablet);
                 err = OB_ERROR;
               }
           ...
           if (OB_SUCCESS == err && cur_tablet_brother_cnt > 0)
           {
             if (serialize_size > max_serialize_size)
             {
                ...
                */
               YYSYS_LOG(WARN, "one tablet splited more than %ld tablets, "
             ...
             if (cur_tablet_brother_cnt < OB_MAX_TABLET_LIST_NUMBER)
             {
               ...
               if (OB_SUCCESS != err)
               {
                 YYSYS_LOG(WARN, "failed to rollback tablet info report list, err=%d", err);
               }
               else if (report_info_list->get_serialize_size() > max_serialize_size)
               {
                 YYSYS_LOG(ERROR, "report_info_list serialize_size: %ld still greater than %ld",
           ...
           if (OB_SUCCESS == err)
           {
             ...
             if (OB_SUCCESS != err)
             {
               YYSYS_LOG(WARN, "failed to send tablet info report, err=%d", err);
             }
           }
       ...
       if (OB_ITER_END == err)
       {
         ...
         if (OB_SUCCESS != err)
         {
           YYSYS_LOG(WARN, "failed to send tablet report in last round, ret=%d", err);
         }
       }
       ...
       if(OB_SUCCESS == err)
       {
         ...
         {
           YYSYS_LOG(WARN, "failed to releas schema_managerV2, err = %d",err);
         }
         else
         {
        ...
        const ObTablet* tablet, ObSSTableScanner& sstable_scanner)
    {
      ...
      {
        YYSYS_LOG(WARN, "find_sstable err=%d, size=%d", err, size);
      }
      else*/
      else
      {
        ...
        else
        {
          ...
          {
            //YYSYS_LOG(ERROR,"test::whx load local index,scan_param = %s,fake_range = %s",to_cstring(*scan_param.get_range()),to_cstring(*scan_param.get_fake_range()));
          }
        }
      }
      if(OB_SUCCESS == err)
      //modify e
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "sstable scanner set scan parameter error.");
        }
      }
      return err;
    }
    ...
    int ObTabletManager::internal_scan(const ObScanParam& scan_param, ObScanner& scanner)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread local sstable scanner, sstable_scanner=%p",
                  sstable_scanner);
        err = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if ((query_version = scan_param.get_version_range().get_query_version()) < 0)
      {
        YYSYS_LOG(ERROR, "empty version range to scan, version_range=%s",
      ...
      {
        YYSYS_LOG(WARN, "failed to acquire range(%s), tablet=%p, version=%ld, err=%d",
        ...
        else
        {
          if (OB_SUCCESS != (err =
                      tablet_image_.acquire_tablet(*scan_param.get_fake_range(),
                        scan_direction, query_version, tablet)))
                {
                  YYSYS_LOG(WARN, "failed to acquire fake range(%s), tablet=%p, version=%ld, err=%d",
      ...
      {
        YYSYS_LOG(INFO, "tablet is removed, can't scan, tablet_range=%s",
            to_cstring(tablet->get_range()));
        err = tablet_image_.release_tablet(tablet);
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to release tablet, tablet=%p, range:%s",
      ...
      {
        YYSYS_LOG(ERROR, "init_sstable_scanner error=%d", err);
      }
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG,"add cell failed(%d)",err);
        }

        if (YYSYS_LOGGER._level >= YYSYS_LOG_LEVEL_DEBUG)
        {
          YYSYS_LOG(DEBUG, "scan result: tablet's data version=%ld, range=%s",
          ...
          YYSYS_LOG(DEBUG, "thread local page arena hold memory usage,"
    ...
    int ObTabletManager::end_scan(bool release_tablet /*=true*/)
    {
      ...
      if (NULL != scan_tablet && release_tablet)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "failed to release tablet, tablet=%p", scan_tablet);
        }
        else
        {
          scan_tablet = NULL;
                                        ...
                                        int64_t* compactsstable_version /*=NULL*/)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, tablets=%p size=%ld", tablets, size);
        err = OB_INVALID_ARGUMENT;
      }
      else if (cur_serving_idx_ >= TABLET_ARRAY_NUM)
      {
        YYSYS_LOG(WARN, "invalid status, cur_serving_idx_=%ld", cur_serving_idx_);
        err = OB_ERROR;
      }
      else if (NULL == get_param.get_row_index() || get_param.get_row_size() <= 0)
      {
        YYSYS_LOG(WARN, "invalid get param");
        err = OB_ERROR;
      }
      else if ((query_version = get_param.get_version_range().get_query_version()) < 0)
      {
        YYSYS_LOG(ERROR, "empty version range to get, version_range=%s",
      ...
      else
      {
        ...
        for (i = 0; i < row_size && OB_SUCCESS == err; i++)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(DEBUG, "the tablet does not exist, err=%d, rowkey: %s", err, to_cstring(range.start_key_));
            tablets[i] = NULL;
            err = OB_SUCCESS;
            /**
          ...
          else if (NULL != tablets[i])
          {
                ...
                || tablets[i]->is_removed())
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "failed to release tablet, tablet=%p, err=%d",
                          tablets[i], err);
              }
                                             ...
                                             ObSSTableGetter& sstable_getter)
    {
      ...
      else
      {
        ...
        for (int64_t i = 0; i < size && OB_SUCCESS == err; ++i)
        {
          ...
          {
            YYSYS_LOG(DEBUG, "find_sstable reader=%p, reader_size=%d",
                      reader, reader_size);
          ...
          {
            YYSYS_LOG(WARN, "find sstable reader by rowkey return more than"
          ...
          {
            YYSYS_LOG(DEBUG, "tablet find sstable reader failed, "
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to set_get_param, err=%d", err);
          }
        }
      }
    ...
    int ObTabletManager::fill_get_data(ObIterator& iterator, ObScanner& scanner)
    {
      ...
      while (OB_SUCCESS == (err = iterator.next_cell()))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get cell, cell=%p, err=%d", cell, err);
          err = OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(INFO, "ObScanner size full, cannot add any cell.");
            scanner.rollback();
          ...
          {
            YYSYS_LOG(WARN, "failed to add cell to scanner, err=%d", err);
            err = OB_ERROR;
          }
        }
      ...
      {
        YYSYS_LOG(WARN, "error occurs while iterating, err=%d", err);
      }
      return err;
    }
 ...
 //add e
    ...
    int ObTabletManager::get_old_tablet_column_checksum(const ObNewRange new_range,col_checksum& column_checksum)
    {
      ...
      if(OB_SUCCESS != (ret = THE_CHUNK_SERVER.get_chunk_service().get_cchecksum_info(new_range, serving_version,THE_CHUNK_SERVER.get_self().cluster_id_,cchecksum)))
      {
          YYSYS_LOG(WARN, "failed to get schema_service_ cchecksum");
      }
      else
      {
          column_checksum.deepcopy(cchecksum.ptr(), cchecksum.length());
      }
    ...
    int ObTabletManager::send_tablet_column_checksum(const col_checksum column_checksum,const ObNewRange new_range,const int64_t version)
    {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "faild to add column check sum to mutator:ret[%d]",ret);
        }
        else
        {
          //
          for(int i = 0;i < retry_times;i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "insert column_checksum_failed");
              usleep(8000000);
            }
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "update column checksum failed");
      }
    ...
    int ObTabletManager::delete_tablet_on_rootserver(const common::ObVector<ObTablet*>& delete_tablets)
    {
      ...
      else
      {
        ...
        for(int32_t i = 0; i < delete_tablets.size() && OB_SUCCESS == ret; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "fill_tablet_info error.");
            break;
          }
              ...
              discard_tablet_list->get_tablet_size() >= OB_MAX_TABLET_LIST_NUMBER - 1)
          {
            ...
            {
              YYSYS_LOG(WARN, "delete tablets rpc error, ret= %d", ret);
              break;
            }
            else
            {
          ...
          {
            YYSYS_LOG(WARN, "add tablet(version=%ld) info to list error. ", delete_tablets[i]->get_data_version());
            break;
          }
          else
          {
    ...
    int ObTabletManager::uninstall_disk(int32_t disk_no)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get delete tablet, err=%d", ret);
      }
      else if (delete_tablet_list.size() > 0)
      {
            ...
            it != delete_tablet_list.end() && OB_SUCCESS == ret; ++it)
        {
          ...
          if (OB_SUCCESS == image.get_split_old_tablet((*it), old_tablet))
          {
            ...
            if (old_tablet->get_disk_no() != disk_no)
            {
              ...
              YYSYS_LOG(INFO, "split occur old_tablet:%s split_tablet:%s",
            ...
            else
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "release tablet failed, ret:%d", ret);
                break;
              }
            }
          }
        ...
        {
          YYSYS_LOG(WARN, "sync all images failed, ret:%d", ret);
        }

        //release all tablets
        if (OB_SUCCESS != image.release_tablets(delete_tablet_list))
        {
          YYSYS_LOG(WARN, "failed to release tablet");
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "disk_dir, %d > max_file_len:%ld", size, OB_MAX_FILE_NAME_LENGTH);
            ret = OB_SIZE_OVERFLOW;
          }
          else if(0 != ::unlink(disk_dir))
          {
            YYSYS_LOG(WARN, "failed to unlink disk:%s, errno:%d errno_str:%s",
                disk_dir, errno, strerror(errno));
            ret = OB_IO_ERROR;
          }
          else
          {
            YYSYS_LOG(ERROR, "unlink disk:%s succc", disk_dir);
          }
        }
      }

      YYSYS_LOG(ERROR, "uninstall the disk:%d succ remove tablets num:%d ret:%d",
    ...
    int ObTabletManager::install_disk(const char* mount_path, int32_t &disk_no)
    {
      ...
      YYSYS_LOG(INFO, "install_disk mount_path:%s", mount_path);
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "add mount_disk:%s failed, ret:%d", mount_path, ret);
        }
        else
        {
          ...
          YYSYS_LOG(INFO, "install the mount_path:%s to disk:%d ret:%d",
    ...
    void ObTabletManager::set_statistics_tid(const uint64_t tid)
    {
        if(statistics_tid_ != tid)
        {
            statistics_tid_ = tid;
        }
    }
    ...
    int ObTabletManager::serialize_cs_merge_stat(char *buf, const int64_t buf_len, int64_t &pos) const
    {
        return chunk_merge_.serialize_cs_merge_stat(buf, buf_len, pos);
    }
