...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObChunkServer::reload_config()
    {
      ...
      if (NULL != rpc_proxy)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "set update server black list param failed:ret=%d", ret);
          }
        }
      }
      else
    ...
    int ObChunkServer::init_merge_join_rpc()
    {
      ...
      ObCsPartitionManager *newest_part_mgr = NULL;
      //20150626:e
      ...
      if(OB_SUCCESS == ret)
      {
        newest_part_mgr = new(std::nothrow)ObCsPartitionManager(&service_, &config_mgr_);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          while (!stoped_)
          {
            ...
            YYSYS_LOG(INFO, "retry to fetch core schema:retry_times[%ld]", retry_times ++);
          }
          if (OB_SUCCESS == ret)
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "set update server black list config failed:ret=%d", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        while (!stoped_)
        {
          ...
          usleep(RETRY_INTERVAL_TIME);
          YYSYS_LOG(INFO, "retry to get ups list:retry_times[%ld]", retry_times ++);
        }
      }
      //add liu jun. [MultiUPS] [part_cache] 20150626:b
      if(OB_SUCCESS == ret)
      {
        ...
        else
        {
           ...
           * so delete it
           * */
//          for(int i = 0;i < config_.retry_times;i++)
//          {
//            if(OB_SUCCESS == (ret = newest_part_mgr->update_all_rules()))
//            {
...
//          ret = OB_SUCCESS;
          //del 20160113:e
          part_monitor_->add_partition_manager(newest_part_mgr);
        }
      }
      //20150626:e
      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150703:b
      if(OB_SUCCESS ==ret)
      {
        rpc_proxy_->set_cs_partition_monitor(part_monitor_);  //set  rpc_proxy,use for get (or scan) data from update server;
        rpc_proxy_->set_cluster_id(config_.cluster_id);
      }
      //add 20150703:e
    ...
    void ObChunkServer::destroy()
    {
      ...
      YYSYS_LOG(INFO, "single server stoped.");
      service_.destroy();
      YYSYS_LOG(INFO, "destory service_.");
      tablet_manager_.destroy();
      YYSYS_LOG(INFO, "destory tablet_manager_, server exit.");
    ...
    int ObChunkServer::do_request(ObPacket* base_packet)
    {
      ...
      else
      {
        ...
        {
          wait_time = yysys::CTimeUtil::getTime() - receive_time;
    ...
    int ObChunkServer::serialize_cs_merge_stat(char *buf, const int64_t buf_len, int64_t &pos) const
    {
        return tablet_manager_.serialize_cs_merge_stat(buf, buf_len, pos);
    }
