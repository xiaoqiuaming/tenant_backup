...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObGetParamCellIterator::init(const ObGetParam & get_param, int64_t cell_idx)
    {
      ...
      {
        YYSYS_LOG(ERROR, "logic error, cell index out of range [cell_idx:%ld,"
    ...
    ObCellInfo *ObGetParamCellIterator::operator->()
    {
      ...
      {
        YYSYS_LOG(ERROR, "logic error, try to access cell index out of range"
      ...
      else
      {
        ...
        if (NULL == result)
        {
          YYSYS_LOG(ERROR, "logic error, within range cell is null"
            "[get_param_:%p,cell_idex_:%ld,get_param_->get_cell_size:%ld]",
            get_param_, cell_idx_, get_param_ != NULL ? get_param_->get_cell_size() : 0);
          result = &fake_cell_;
        }
