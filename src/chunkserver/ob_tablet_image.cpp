  ...
  namespace chunkserver
  {
    ...
    int ObTabletBuilder::ensure_space(const int64_t size)
    {
      ...
      if (NULL == buf_ || (NULL != buf_ && size > buf_size_ - data_size_))
      {
        ...
        if (NULL == new_buf)
        {
          YYSYS_LOG(ERROR, "failed to alloc memory, new_buffer=%p", new_buf);
    ...
    int ObTabletRowkeyBuilder::append_range(const common::ObNewRange& range)
    {
      ...
      {
        YYSYS_LOG(ERROR, "rowkey size =%ld large than buf size(%ld,%ld)",
    ...
    int ObTabletExtendBuilder::append_tablet(const ObTabletExtendInfo& info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "tablet extend size =%ld large than buf size(%ld,%ld)",
            info.get_serialize_size(), data_size_, buf_size_);
      }
      else if (OB_SUCCESS != (ret = info.serialize(buf_, buf_size_, data_size_)))
      {
        YYSYS_LOG(ERROR, "serialize extend info error, buf size(%ld,%ld)",
    ...
    int ObTabletImage::destroy()
    {
      ...
      if (ref_count_ != 0)
      {
        YYSYS_LOG(ERROR, "ObTabletImage still been used ref=%ld, "
        ...
        ret = OB_SUCCESS;//[debug log for tabletimg ref count]???dduuhhtt
      }
      if (OB_SUCCESS == ret)
      {
        ...
        for (int32_t i = 0 ; i < tablet_count; ++i)
        {
          ...
          int64_t refcnt = tablet->get_refcnt();
          if(refcnt != 0)
          {
              YYSYS_LOG(WARN, "bugtest::tablet range = %s  refcnt = %ld", to_cstring(tablet->get_range()), refcnt);
          }
      ...
      ObTablet* &tablet) const
    {
      ...
      if (hash_map_inited_)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to find sstable_id=%lu in hash map, hash_ret=%d",
      ...
      const int32_t disk_no, ObVector<ObTablet*>& tablet_list)
    {
      ...
      if (0 > disk_no)
      {
        YYSYS_LOG(WARN, "tablet image acquire_tablets, invalid disk_no=%d",
          disk_no);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        for (; it != tablet_list_.end(); ++it)
        {
          if ((*it)->get_disk_no() == disk_no)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to push back tablet into "
      ...
      const uint64_t table_id, ObVector<ObTablet*>& table_tablets)
    {
      ...
      {
        YYSYS_LOG(WARN, "tablet image acquire_tablets, invalid table_id=%lu",
          table_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else
    ...
    int ObTabletImage::release_tablets(const ObVector<ObTablet*>& tablet_list)
    {
      ...
      for (; it != tablet_list.end(); ++it)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to release tablet, range=%s",
    ...
    int ObTabletImage::remove_table_tablets(const uint64_t table_id)
    {
      ...
      if (OB_INVALID_ID == table_id)
      {
        YYSYS_LOG(WARN, "tablet image delete table, invalid table_id=%lu", table_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
          ...
          if (cur_table_id == table_id)
          {
            ...
            if (hash_map_inited_)
            {
              ...
              for (int64_t i = 0; i < sstable_count; ++i)
              {
                ...
                {
                  YYSYS_LOG(WARN, "failed to erase (sstabl_id, tablet) pair frome hash map, "
    ...
    int ObTabletImage::delete_table(const uint64_t table_id, yysys::CRWLock& lock)
    {
      int ret = OB_SUCCESS;
      ...
      //acquire table tablets
      {
        ...
        {
          YYSYS_LOG(WARN, "disk_manager member of tablet image isn't set, it's NULL");
          ret = OB_INVALID_ARGUMENT;
        }
        else if (OB_SUCCESS != (ret = acquire_tablets(table_id, delete_table_tablet_list_)))
        {
          YYSYS_LOG(WARN, "failed to acquier table tablets from tablet image, table_id=%lu",
            table_id);
        }
      }
      ...
      {
        yysys::CWLockGuard guard(lock);
        if (OB_SUCCESS != (ret = remove_table_tablets(table_id)))
        {
          YYSYS_LOG(WARN, "failed to remove table tablets from table image, table_id=%lu", table_id);
        }
      }
      ...
      if (OB_SUCCESS == ret && delete_table_tablet_list_.size() > 0)
      {
        ...
        if (disk_num > 0 && NULL != disk_no_array)
        {
          ...
          for (int32_t i = 0; i < disk_num; ++i)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to sync tablet image, disk_no=%d", disk_no_array[i]);
            }
          }
        }
        else
        {
          YYSYS_LOG(WARN, "sync tablet images invalid argument, "
    ...
    const int ObTabletImage::delete_local_index_sstable() const
    {
      ...
      for (; it != tablet_list_.end(); ++it)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to delete local index sstable file_id:%ld", (*it)->get_sstable_id_list().at(1).sstable_file_id_);
        }
      }
      return ret;
    }
    ...
    int ObTabletImage::alloc_tablet_object(const ObNewRange& range, ObTablet* &tablet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "allocate tablet object failed.");
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
    ...
    int ObTabletImage::add_tablet(ObTablet* tablet)
    {
      ...
      if (!hash_map_inited_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to create hash map for sstable id "
      ...
      else
      {
        ...
        for (int64_t i = 0; i < sstable_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to insert (sstabl_id, tablet) pair into hash map, "
                            "sstable_id=%lu, tablet=%p, hash_ret=%d",
        ...
        const int32_t scan_direction, ObTablet* &tablet) const
    {
      ...
      {
        YYSYS_LOG(WARN, "chunkserver has no tablets, cannot find tablet.");
        ret = OB_CS_TABLET_NOT_EXIST;
      }
      else if (range.empty() || NULL == lookup_key.ptr() || 0 >= lookup_key.length())
      {
        YYSYS_LOG(WARN, "find invalid range:%s", to_cstring(range));
        ret = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(WARN, "cannot find range :%s", to_cstring(range));
      }
        ...
        ObTablet* &tablet) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(INFO, "find sstable sstable_id=%ld failed, ret=%d",
            sstable_id.sstable_file_id_, ret);
      }
      else if (NULL == tablet)
      {
        YYSYS_LOG(INFO, "reader is NULL");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObTabletImage::remove_sstable(ObTablet* tablet) const
    {
      ...
      int64_t hdlink_num = 0;

      if (NULL == tablet || NULL == fileinfo_cache)
      {
        YYSYS_LOG(WARN, "invalid param, tablet is NULL, tablet=%p, fileinfo_cache=%p",
      ...
      else
      {
        for (int64_t i = 0; i < tablet->get_sstable_id_list().count() && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get sstable path, sstable_id=%lu",
              sstable_id.sstable_file_id_);
          }
          else if (!FileDirectoryUtils::exists(sstable_path))
          {
            YYSYS_LOG(INFO, "sstable file=%s not exist.", sstable_path);
            ret = OB_ERROR;
          }
          else if(0 >= (hdlink_num = FileDirectoryUtils::get_hard_link_num(sstable_path)))
          {
              YYSYS_LOG(WARN, "failed to get sstable hard link number=%ld, sstable file=%s", hdlink_num, sstable_path);
              ret = OB_ERROR;
          }
          else if ((sstable_size = get_file_size(sstable_path)) <= 0)
          {
            YYSYS_LOG(WARN, "sstable file=%s not exist or invalid, sstable_size=%ld",
                  sstable_path, sstable_size);
          }
          else if (0 != ::unlink(sstable_path))
          {
            YYSYS_LOG(WARN, "failed to unlink sstable, sstable_file=%s, "
                            "errno=%d, err=%s",
          ...
          else if (NULL != disk_manager_)
          {
            ...
            YYSYS_LOG(INFO, "recycle tablet version = %ld, recycle sstable file = %s",
    ...
    int ObTabletImage::remove_tablet(const common::ObNewRange& range, int32_t &disk_no)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "remove_tablet : the specific range:%s not exist, ret=%d", range_buf, ret);
        }
      }
      return ret;
    }
        ...
        const int32_t scan_direction, ObTablet* &tablet) const
    {
      ...
      else if (NULL == tablet)
      {
        char range_buf[OB_RANGE_STR_BUFSIZ];
        range.to_string(range_buf, OB_RANGE_STR_BUFSIZ);
        YYSYS_LOG(ERROR, "found tablet:%s null, ret=%d", range_buf, ret);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObTabletImage::release_tablet(ObTablet* tablet, bool* is_remove_sstable) const
    {
      ...
      if (NULL == tablet)
      {
        YYSYS_LOG(WARN, "invalid param, tablet=%p", tablet);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (ref_count_ <= 0)
      {
        YYSYS_LOG(WARN, "invalid status, ref_count_=%ld", ref_count_);
        ret = OB_ERROR;
      }
      else
      {
        ...
        const int32_t disk_no, const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "check common record header failed, disk_no=%d", disk_no);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize meta header failed, disk_no=%d", disk_no);
        }
        else if (data_version_ != meta_header.data_version_)
        {
          YYSYS_LOG(ERROR, "data_version_=%ld != header.version=%ld",
        ...
        const int64_t buf_len, int64_t &pos)
    {
      ...
      if (NULL == writer)
      {
        YYSYS_LOG(ERROR, "failed to allocate memory for thread local meta writer, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "ObTabletImage::serialize error, disk no=%d", disk_no);
        }
      }
    ...
    int ObTabletImage::read(const char* idx_path, const int32_t disk_no, const bool load_sstable)
    {
      ...
      {
        YYSYS_LOG(INFO, "meta index file path=%s, disk_no=%d not exist", idx_path, disk_no);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        YYSYS_LOG(INFO, "read meta index file path=%s, disk_no=%d", idx_path, disk_no);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "invalid idx file =%s file_size=%ld", idx_path, file_size);
          ret = OB_ERROR;
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "open %s for read error, %s.", idx_path, strerror(errno));
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "read idx file = %s , ret = %d, read_size = %ld, file_size = %ld, %s.",
    ...
    int ObTabletImage::write(const char* idx_path, const int32_t disk_no)
    {
              ...
              tmp_idx_file, OB_MAX_FILE_NAME_LENGTH)))
      {
        ...
        YYSYS_LOG(WARN, "cannot get tmp meta file path disk_no=%d.", disk_no);
      }
      else
      {
        YYSYS_LOG(INFO, "write meta index file path=%s, disk_no=%d, tmp_file=%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "allocate memory for serialize image error, size =%ld", max_serialize_size);
            ret = OB_ALLOCATE_MEMORY_FAILED;
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "write row key buffer failed,%s.", strerror(errno));
          }
          ...
          {
            YYSYS_LOG(ERROR, "write extend info buffer failed,%s.", strerror(errno));
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "rename src meta = %s to dst meta =%s error.", tmp_idx_file, idx_path);
            ret = OB_IO_ERROR;
          }
        }
      }
    ...
    int ObTabletImage::write_meta(const char* idx_path, const int32_t disk_no)
    {
      ...
      {
        YYSYS_LOG(INFO, "write meta file, index file path=%s, disk_no=%d, tmp_file=%s",
            idx_path, disk_no, tmp_idx_file);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid meta data size, must call prepare_write_meta first, "
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(INFO, "write tablet image file %s succ.", idx_path);
          }
        }
      }
    ...
    int ObTabletImage::read(const int32_t* disk_no_array, const int32_t size, const bool load_sstable)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        for (int32_t i = 0; i < size; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "read meta info from disk %d failed", disk_no_array[i]);
            continue;
          }
        }
      }
    ...
    int ObTabletImage::write(const int32_t* disk_no_array, const int32_t size)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        for (int32_t i = 0; i < size; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "write meta info to disk %d failed", disk_no_array[i]);
            continue;
          }
        }
      }
    ...
    int ObTabletImage::begin_scan_tablets()
    {
      ...
      {
        YYSYS_LOG(ERROR, "scan in progress, cannot luanch another, cur_iter_idx_=%ld", cur_iter_idx_);
        ret = OB_CS_EAGAIN;
      }
      else
      {
        YYSYS_LOG(INFO, "begin_scan_tablets cur_iter_idx_ = %ld", cur_iter_idx_);
      }
    ...
    int ObTabletImage::get_next_tablet(ObTablet* &tablet)
    {
      ...
      if (cur_iter_idx_ == INVALID_ITER_INDEX)
      {
        YYSYS_LOG(ERROR, "not a initialized scan process, call begin_scan_tablets first");
        ret = OB_ERROR;
      }
    ...
    int ObTabletImage::end_scan_tablets()
    {
      ...
      if (cur_iter_idx_ == INVALID_ITER_INDEX)
      {
        YYSYS_LOG(INFO, "scans not begin or has no tablets, cur_iter_idx_=%ld", cur_iter_idx_);
        ret = OB_ERROR;
      }
      else
      {
        YYSYS_LOG(INFO, "end_scan_tablets cur_iter_idx_=%ld", cur_iter_idx_);
      }
      cur_iter_idx_ = INVALID_ITER_INDEX;
      return ret;
    }
    ...
    {
      YYSYS_LOG(INFO, "ref_count_=%ld, cur_iter_idx_=%ld, memory usage=%ld",
          ref_count_, cur_iter_idx_, allocator_.total());

      YYSYS_LOG(INFO, "----->begin dump tablets in image<--------");
      for (int32_t i = 0; i < tablet_list_.size(); ++i)
      {
        ObTablet* tablet = tablet_list_.at(i);
        YYSYS_LOG(INFO, "----->tablet(%d)<--------", i);
        tablet->dump(dump_sstable);
      }
      YYSYS_LOG(INFO, "----->end dump tablets in image<--------");
        ...
        ObTablet* &tablet) const
    {
      ...
      {
        YYSYS_LOG(WARN, "acquire_tablet invalid argument, "
      ...
      else
      {
        ...
        do
        {
          if (index < 0 || index >= MAX_RESERVE_VERSION_COUNT)
          {
            YYSYS_LOG(WARN, "image (index=%ld) not initialized, has no tablets", index);
            break;
          }
    ...
    int ObMultiVersionTabletImage::release_tablets(const ObVector<ObTablet*>& tablets) const
    {
      ...
      for(ObVector<ObTablet*>::iterator it = tablets.begin(); it != tablets.end(); ++it)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to release tablet, tablet=%s ret=:%d",
    ...
    int ObMultiVersionTabletImage::release_tablet(ObTablet* tablet) const
    {
...

      {
        ...
        if (NULL == tablet)
        {
          YYSYS_LOG(WARN, "release_tablet invalid argument tablet null");
          ret = OB_INVALID_ARGUMENT;
        }
        else if (!has_match_version(tablet->get_data_version()))
        {
          YYSYS_LOG(WARN, "release_tablet version=%ld dont match",
        ...
        ObTablet* &tablet) const
    {
      ...
      if (0 > version
          || (from_index != FROM_SERVICE_INDEX && from_index != FROM_NEWEST_INDEX))
      {
        YYSYS_LOG(ERROR, "acquire_sstable invalid argument, "
    ...
    int ObMultiVersionTabletImage::include_sstable(const ObSSTableId& sstable_id) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "include_sstable invalid argument, "
    ...
    int ObMultiVersionTabletImage::remove_tablets(ObVector<ObTablet*>& tablets, bool sync)
    {
          ...
          it != tablets.end() && OB_SUCCESS == ret; ++it)
      {
        ...
        {
          YYSYS_LOG(WARN, "remove tablet<%s> error, ret:%d", to_cstring((*it)->get_range()), ret);
          break;
        }
        else
        {
          YYSYS_LOG(INFO, "remove tablet<%s> on disk_no:%d data_version:%ld",
        ...
        const int64_t version, int32_t &disk_no, bool sync)
    {
...

      {
        ...
        if (range.empty() || 0 >= version)
        {
          YYSYS_LOG(WARN, "remove_tablet invalid argument, "
              "range is emtpy or version=%ld < 0", version);
          ret = OB_INVALID_ARGUMENT;
        }
        else if (!has_match_version_tablet(version))
        {
          YYSYS_LOG(WARN, "remove_tablet version=%ld dont match", version);
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
        ...
        const bool for_create /* = false */)
    {
      ...
      {
        YYSYS_LOG(WARN, "add_tablet invalid argument, "
      ...
      else
      {
        ...
        if(OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "prepare new version = %ld image error.", new_version);
          }
          else if ( OB_SUCCESS != (ret =
                get_image(new_version).add_tablet(tablet)) )
          {
            YYSYS_LOG(ERROR, "add tablet error with new version = %ld.", new_version);
          }
        ...
        ObTablet *old_tablet, ObTablet *new_tablet, const bool load_sstable)
    {
      ...
      {
        YYSYS_LOG(WARN, "upgrade_tablet invalid argument, "
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "upgrade_tablet range:(%s) old version = %ld, new version = %ld",
        ...
        const bool load_sstable)
    {
      ...
      else
      {
        ...
        char range_buf[OB_RANGE_STR_BUFSIZ];
        old_tablet->get_range().to_string(range_buf, OB_RANGE_STR_BUFSIZ);
        YYSYS_LOG(DEBUG, "upgrade old range:(%s), old version=%ld",
        ...
        {
          YYSYS_LOG(ERROR, "new version =%ld <= old version=%ld",
        ...
        for (int32_t i = 0; i < split_size && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "split tablet i=%d, version =%ld <> fisrt tablet version =%ld",
          ...
          else
          {
            ...
            YYSYS_LOG(DEBUG, "upgrade with new range:(%s), new version=%ld",
                                                        ...
                                                        const bool load_sstable)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "upgrade with new index range:(%s), new version=%ld",
                   range_buf, new_version);
      }
      if (OB_SUCCESS == ret && load_sstable)
      {
        //YYSYS_LOG(WARN, "test::zhuyanchao load sstable");
        ret = tablet->load_sstable();
      }
      return ret;
    }
    ...
    int ObMultiVersionTabletImage::upgrade_service(const bool is_really_merged)
    {
      int ret = OB_SUCCESS;
      ...
      if (!has_tablet(newest_index_))
      {
          if(is_really_merged)
          {
              YYSYS_LOG(WARN, "there is no tablets on version = %ld, still upgrade.", new_version);
              service_index_ = newest_index_;
          }
          else
          {
              YYSYS_LOG(WARN, "there is no tablets on version = %ld and no tablet has been successfully merged, cannot upgrade.", new_version);
              ret = OB_CS_EMPTY_MERGE;
          }
      }
      else if (new_version <= service_version)
      {
        YYSYS_LOG(WARN, "service version =%ld >= newest version =%ld, cannot upgrade.",
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "upgrade service version =%ld to new version =%ld",
            service_version, new_version);
        service_index_ = newest_index_;
      }
        ...
        const common::ObNewRange& range, const int64_t version, ObTablet* &tablet)
    {
      ...
      if (range.empty() || 0 >= version)
      {
        YYSYS_LOG(WARN, "alloc_tablet_object invalid argument, "
            "range is emtpy or version=%ld < 0", version);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "prepare new version = %ld image error.", version);
        }
        else if ( OB_SUCCESS != (ret =
              get_image(version).alloc_tablet_object(range, tablet)) )
        {
          YYSYS_LOG(ERROR, "cannot alloc tablet object new version = %ld", version);
        }
        else if (NULL != tablet)
        {
        ...
        const int64_t version, ObTablet* &tablet)
    {
      ...
      if (0 >= version)
      {
        YYSYS_LOG(WARN, "alloc_tablet_object invalid argument, version=%ld < 0",
          version);
        ret = OB_INVALID_ARGUMENT;
      }
      else
    ...
    int ObMultiVersionTabletImage::get_tablets_for_merge(
        const int64_t version, int64_t &size, ObTablet *tablets[],
            const bool need_unmerged
            ) const
    {
      int ret = OB_SUCCESS;
      ...
      YYSYS_LOG(DEBUG, "for merge, version=%ld,size=%ld,newest=%ld", version, size, newest_index_);
    ...
    int ObMultiVersionTabletImage::discard_tablets_not_merged(const int64_t version)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid version = %ld", version);
      }
      else
      {
        yysys::CWLockGuard guard(lock_);
    ...
    int ObMultiVersionTabletImage::prepare_tablet_image(const int64_t version, const bool destroy_exist)
    {
      ...
      if (index < 0 || index >= MAX_RESERVE_VERSION_COUNT)
      {
        YYSYS_LOG(ERROR, "index=%ld out of range.", index);
        ret = OB_ERROR;
      }
      else if (NULL == image_tracker_[index])
      {
        ...
        {
          YYSYS_LOG(ERROR, "cannot new ObTabletImage object, index=%ld", index);
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
      ...
      else if (version != image_tracker_[index]->data_version_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "new version=%ld not matched with old verion=%ld,"
    ...
    int ObMultiVersionTabletImage::tablets_all_merged(const int64_t index) const
    {
      ...
      else if (NULL != image_tracker_[index])
      {
        ...
        for (; it != tablet_list.end() ; ++it)
        {
          if (!(*it)->is_merged())
          {
            YYSYS_LOG(INFO, "tablet range=<%s> version=%ld has not merged", to_cstring((*it)->get_range()), (*it)->get_data_version());
            ret = OB_CS_EAGAIN;
            break;
          }
        }
        ...
        const int32_t disk_no)
    {
      ...
      {
        YYSYS_LOG(WARN, "write image invalid argument, "
            "disk_no =%d <0 or version=%ld < 0", disk_no, version);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!has_match_version(version))
      {
        YYSYS_LOG(WARN, "write image version=%ld don't match", version);
        ret = OB_ERROR;
      }
      else
      {
      ...
      const int32_t size)
    {
      ...
      if (NULL == disk_no_array || size <= 0)
      {
        YYSYS_LOG(WARN, "sync all images invalid argument, "
      ...
      else
      {
        ...
        if (0 < eldest_version)
        {
          for (int32_t i = 0; i < size; ++i)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "write eldest meta info to disk=%d failed, version=%ld",
        ...
        {
          YYSYS_LOG(WARN, "eldest_version:%ld <= 0", eldest_version);
        }
        ...
        if (newest_version != eldest_version)
        {
          if (0 < newest_version)
          {
            for (int32_t i = 0; i < size; ++i)
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "write newest meta info to disk=%d failed, version=%ld",
          ...
          {
            YYSYS_LOG(WARN, "newest_version:%ld <= 0", newest_version);
          }
        }
      }
        ...
        const int64_t version, const int32_t disk_no, const bool load_sstable)
    {
      ...
      if (0 >= version || 0 >= disk_no)
      {
        YYSYS_LOG(ERROR, "read image invalid argument, "
            "disk_no =%d <0 or version=%ld < 0", disk_no, version);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = prepare_tablet_image(version, true)))
      {
        YYSYS_LOG(ERROR, "cannot prepare image, version=%ld", version);
      }
      else if (OB_SUCCESS != (ret = get_image(version).read(
              idx_path, disk_no, load_sstable)) )
      {
        YYSYS_LOG(ERROR, "read idx file = %s , disk_no = %d, version = %ld error",
        ...
        const int32_t disk_no, const bool load_sstable)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get meta file path version = %ld, disk_no = %d error",
            version, disk_no);
      }
      else if ( OB_SUCCESS != (ret = read(idx_path,
              version, disk_no, load_sstable)) )
      {
        YYSYS_LOG(ERROR, "read idx file =%s version = %ld, disk_no = %d error",
        ...
        const int32_t size, const bool load_sstable)
    {
      ...
      else
      {
        ...
        for (int32_t i = 0; i < size && (OB_SUCCESS == ret || OB_CS_EAGAIN == ret); ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "get sstable directory disk %d failed", disk_no);
            ret = OB_ERROR;
            break;
          }
          ...
          if (idx_file_num <= 0)
          {
            YYSYS_LOG(INFO, "idx directory %s has no idx files.", idx_dir_path);
            continue;
          }
          ...
          for (int n = 0; n < idx_file_num; ++n)
          {
            ...
            {
              YYSYS_LOG(ERROR, "disk no = %d in idx file name cannot match with disk=%d ",
                  disk_no, disk_no_array[i]);
              ret = OB_ERROR;
            }
            else
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "search service tablet image failed,"
              "service_index_=%ld", service_index_);
          ret = OB_ERROR;
        }
        else if (OB_SUCCESS != (ret = adjust_inconsistent_tablets()))
        {
          YYSYS_LOG(ERROR, "failed to adjust inconsistent tablets");
        }
        else if (has_tablet(service_index_))
        {
          YYSYS_LOG(INFO, "current service tablet version=%ld",
          ...
          && has_tablet(service_index_) && has_tablet(newest_index_))
      {
        ...
        for (; it != tablet_list.end(); ++it)
        {
          if ((*it)->is_merged() && !(*it)->is_removed())
          {
            ...
            {
              (*it)->set_merged(0);
              YYSYS_LOG(WARN, "maybe chunkserver is killed in daily merge stage, "
      ...
      while (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err && NULL != tablet)
        {
          ...
          else if (next_split_range.compare_with_startkey2(tablet->get_range()) < 0)
          {
            ...
            YYSYS_LOG(WARN, "splited tablet range lost, lost_range=%s, old_range=%s, "
          ...
          else if (next_split_range.compare_with_endkey2(tablet->get_range()) < 0)
          {
            ...
            YYSYS_LOG(WARN, "splited tablet range of new version cross previous version "
    ...
    int ObMultiVersionTabletImage::begin_scan_tablets()
    {
      ...
      {
        YYSYS_LOG(WARN, "scan in progress, cannot luanch another, cur_vi_=%ld", iterator_.cur_vi_);
        ret = OB_CS_EAGAIN;
      }
      else if (OB_SUCCESS != (ret = iterator_.start()))
      {
        YYSYS_LOG(WARN, "start failed ret = %d", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "start succeed vi=%ld, ti=%ld", iterator_.cur_vi_, iterator_.cur_ti_);
      }
      return ret;
    }
    ...
    int ObMultiVersionTabletImage::end_scan_tablets()
    {
      ...
      if (iterator_.is_stop())
      {
        YYSYS_LOG(WARN, "scans not begin.");
        ret = OB_ERROR;
      }
      else
      {
        ...
        const int32_t index, const int32_t disk_no, char* buf, const int64_t buf_len, int64_t &pos)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument index= %d, disk_no=%d", index, disk_no);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!has_tablet(index))
      {
        YYSYS_LOG(WARN, "has no tablet index= %d, disk_no=%d", index, disk_no);
        ret = OB_ERROR;
      }
      else
      {
        ...
        const int32_t disk_no, const char* buf, const int64_t buf_len, int64_t &pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "prepare_tablet_image header.version=%ld error",
    ...
    int ObMultiVersionTabletImage::Iterator::start()
    {
      ...
      yysys::CRLockGuard guard(image_.lock_);
    ...
    //[schedule optimize
    void ObMultiVersionTabletImage::get_tablet_merge_status(int64_t &tablet_num, int64_t &merged_num) const
    {
        int64_t eldest_index = get_eldest_index();
        if(NULL != image_tracker_[eldest_index])
        {
    ...
    int ObMultiVersionTabletImage::delete_table(const uint64_t table_id)
    {
      ...
      if (OB_INVALID_ID == table_id)
      {
        YYSYS_LOG(WARN, "delete_table invalid argument, table_id = %lu",
          table_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else
