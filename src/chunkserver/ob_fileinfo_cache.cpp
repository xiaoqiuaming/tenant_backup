...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int FileInfo::init(const uint64_t sstable_id, const bool use_dio)
    {
      ...
      {
        YYSYS_LOG(WARN, "get sstable filename fail sstable_id=%lu", sstable_id);
        ret = OB_ERROR;
      }
    ...
    const IFileInfo *FileInfoCache::get_fileinfo(const uint64_t sstable_id)
    {
      ...
      if (OB_SUCCESS != (tmp_ret = cache_.get(sstable_id, ret, tmp_handle, false)))
      {
        ...
        {
          YYSYS_LOG(WARN, "init file info fail sstable_id=%lu", sstable_id);
          /**
          ...
          {
            YYSYS_LOG(WARN, "failed to delete fake file info, sstable_id=%lu, err=%d",
                      sstable_id, tmp_ret);
          }
          ret = NULL;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to put and fetch file info, sstable_id=%lu, "
                            "err=%d, ret=%p",
