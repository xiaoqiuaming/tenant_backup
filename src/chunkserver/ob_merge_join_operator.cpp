...
//                                          unmerge_if_unchanged);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (is_need_join_)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to start join, ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret && NULL != scan_param_)
      {
        ...
        if (comp_column_size > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to start compose, ret=%d", ret);
            if(OB_EXPIRE_CONDITION_ERROR == ret)
            {
                is_need_compose_ = false;
            }
          }
        ...
        {
          YYSYS_LOG(WARN, "the result array includes part row, "
    ...
    int ObMergeJoinOperator::do_merge_join()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to do merge, ret=%d", ret);
      }
      ...
      if (OB_SUCCESS == ret && is_need_join_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to do join, ret=%d", ret);
        }
      }
