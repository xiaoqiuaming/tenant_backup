...
namespace oceanbase
{
  namespace chunkserver
  {
        ...
        int64_t &sstable_type)
    {
      ...
      {
        YYSYS_LOG(ERROR, "serialize desc failed, %d", ret);
      }
...
//                                                                                      CS_FETCH_DATA_VERSION, THE_CHUNK_SERVER.get_config().datasource_timeout, out_buffer_, session_id_)))
      ...
      //mod:e
      {
        YYSYS_LOG(WARN, "send request to %s failed, %d ", to_cstring(src_server_), ret);
      ...
      {
        YYSYS_LOG(WARN, "deserialize response failed, %d", ret);
      }
    ...
    int ObTabletRemoteReader::read_next(common::ObDataBuffer& buffer)
    {
      ...
      else if(has_new_data_ && OB_ITER_END == ret)
      {
        ...
        //mod:e
        {
          YYSYS_LOG(WARN, "send request failed, %d", ret);
          ret = OB_DATA_SOURCE_TIMEOUT;
        }
        else if(OB_SUCCESS != (ret = rc.deserialize(out_buffer_.get_data(), out_buffer_.get_position(), pos_)))
        {
          YYSYS_LOG(ERROR, "finish flag deserialize failed, %d", ret);
        }
        else if(OB_SUCCESS != rc.result_code_)
        {
          ret = rc.result_code_;
          YYSYS_LOG(ERROR, "remote reader failed, ret:%d", ret);
        }
      ...
      {
        YYSYS_LOG(WARN, "remote read failed, ret:%d", ret);
      }
    ...
    int ObTabletFileReader::prepare(const ObNewRange& range, const int64_t req_tablet_version, const int16_t remote_sstable_version)
    {
          ...
          (NULL == tablet_ || req_tablet_version != tablet_->get_data_version()))
      {
        ...
        YYSYS_LOG(WARN, "not found the tablet range:%s "
    ...
    int ObTabletFileReader::open()
    {
      ...
      if(NULL == tablet_)
      {
        YYSYS_LOG(WARN, "not found tablet, tablet is NULL");
      }
      else
      {
        if(OB_SUCCESS != (ret != get_sstable_version(local_sstable_version)))
        {
          YYSYS_LOG(ERROR, "get sstable version failed, ret:%d", ret);
        }
        else if(remote_sstable_version_ != local_sstable_version)
        {
          ret = OB_SSTABLE_VERSION_UNEQUAL;
          YYSYS_LOG(ERROR, "sstable version not equal, remote_sstable_version:%d"
        ...
        {
          YYSYS_LOG(WARN, "not found sstable file");
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "sstable_file_id <= 0 not found the file");
          }
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "open %s for read error, %s.", path, strerror(errno));
            }
            else
            {
              YYSYS_LOG(INFO, "sstable version equal, local_sstable_version:%d remote_sstable_version:%d direct file copy, path:%s",
    ...
    int ObTabletFileReader::get_row_checksum(int64_t &row_checksum)
    {
      ...
      if(NULL == tablet_)
      {
        YYSYS_LOG(WARN, "not found the tablet");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObTabletFileReader::read_file_data(common::ObDataBuffer& buffer)
    {
      ...
      if(reader_.is_opened())
      {
        ...
        {
          YYSYS_LOG(ERROR, "file read error, %s", strerror(errno));
        }
      }
      ...
      {
        YYSYS_LOG(ERROR, "read result flag serialize, %d", serialize_ret);
      }
      ...
      YYSYS_LOG(DEBUG, "copy file size:%ld finish:%d ret:%d", read_size, read_result, ret);
      return ret;
    }
