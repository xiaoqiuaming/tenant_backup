...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObJoinCache::init(const int64_t max_mem_size)
    {
      ...
      {
        inited_ = true;
        YYSYS_LOG_US(DEBUG, "init join cache succ, cache_mem_size=%ld,",
                     max_mem_size);
      }
    ...
    int ObJoinCache::enlarg_cache_size(const int64_t cache_mem_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "enlarge total join cache size of kv cache fail");
      }
      else
      {
        YYSYS_LOG(INFO, "success enlarge join cache size to %ld", cache_mem_size);
      }
    ...
    int ObJoinCache::get_row(const ObJoinCacheKey& key, ObRowCellVec& row_cells)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid join cache key, =%s", to_cstring(key.row_key_));
