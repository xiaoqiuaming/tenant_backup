...
namespace oceanbase
{
  namespace chunkserver
  {
                                             ...
                                             ObSSTableReader* sstable_reader)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, src_buf=%p, src_len=%ld, dst_buf=%p,"
      ...
      {
        YYSYS_LOG(WARN, "sstable reader is NULL");
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to decompress");
        }
      }
                                                ...
                                                ObSSTableReader* sstable_reader) 
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "invalid block data");
        }
      }
          ...
          && OB_SUCCESS == (ret = uncompressed_buf_.ensure_space(header.data_length_)))
      {
        if (header.is_compress())
        {
          ...
          if (OB_SUCCESS != ret)
          {
            ...
            YYSYS_LOG(WARN, "decompress failed, data_zlength=%d data_length=%d,"
                                            ...
                                            ObSSTableReader* sstable_reader) 
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == reader)
        {
          YYSYS_LOG(INFO, "Problem get sstable reader, sstable id=%lu", 
        ...
        {
          YYSYS_LOG(INFO, "Problem get sstable scheam, sstable id=%lu, reader=%p, "
      ...
      else if (OB_ITER_END == ret)
      {
        //complete traversal
        handle.reset();
        YYSYS_LOG(INFO, "complete traversal block cache");
      }
      else
      {
                                                        ...
                                                        ObSSTableReader* sstable_reader)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "Block size is too small, block size=%ld", 
