...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    class ObIndexWorker : public yysys::CDefaultRunnable
    {
      public:
        ObIndexWorker();
        ~ObIndexWorker()
        ...
        int fetch_tablet_info(bool is_local_index = true, bool need_other_cs = false, bool need_all_range = false);
        ...
        bool if_current_round_finished();

        int parse_location_from_scanner(ObScanner &scanner, ObRowkey &row_key, uint64_t table_id, bool need_other_cs = false, bool need_all_range = false);
        ...
        inline int set_schedule_time(uint64_t tid, int64_t start_time, IndexPhase ip)
        {
          int ret = OB_SUCCESS;
          if (schedule_idx_tid_ == tid && OB_INVALID_ID != tid && 0 != start_time)
          {
            index_phase_ = ip;//add liumz, [static_index range_intersect]20170915
            ...
            {             
              schedule_time_2_ = start_time;
              YYSYS_LOG(WARN, "rs reboot or switch! schedule_time_1_=[%ld], schedule_time_2_=[%ld]", schedule_time_1_, schedule_time_2_);
            }
          }
          return ret;
        }
        ...
        inline int64_t get_local_mergerd_tablet_task_num()
        {
            return tablet_index_;
        }
        inline int64_t get_local_total_tablet_task_num()
        {
            return tablet_array_.count();
        }
        inline int64_t get_global_mergerd_tablet_task_num()
        {
            return range_index_;
        }
        inline int64_t get_global_total_tablet_task_num()
        {
            return range_array_.count();
        }
        inline IndexPhase get_index_build_phase()
        {
            return index_phase_;
        }
      ...
      private:
        ...
        const static int64_t RETRY_INTERVAL = 3000000;
