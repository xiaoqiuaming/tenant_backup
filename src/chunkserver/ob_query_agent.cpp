...
namespace oceanbase
{
  namespace chunkserver
  {
        ...
        const bool unmerge_if_unchanged, const bool is_static_truncated) /*add zhaoqiong [Truncate Table]:20160318 param:is_static_truncated*/
    {
      ...
      {
        YYSYS_LOG(WARN, "wrong get param or scan param, can't dynamic cast");
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to build query stage, ret=%d", ret);
        }
      }
       ...
       const int64_t timeout_time)
   {
    ...
    int ObQueryAgent::build_query_stage()
    {
      ...
      else if (NULL != scan_param_)
      {
        ...
        {
          YYSYS_LOG(WARN, "the groupby param includes filter info, "
        ...
        {
          YYSYS_LOG(WARN, "invalid scan param, the return info size "
          ...
          ret = OB_ERR_UNEXPECTED;
                 ...
                 || sharding_min_row_cnt_ < 0)
        {
          YYSYS_LOG(WARN, "invalid scan param, limit_offset_=%ld, limit_count_=%ld, "
        ...
        {
          YYSYS_LOG(WARN, "invalid scan param, orderby_count=%ld, "
    ...
    {
      int ret = OB_SUCCESS;
      int expire_cond_ret = OB_SUCCESS;

      //stage one
      if (MERGE_JOIN_COMPOSE != query_stage_[cur_stage_idx_])
      {
        YYSYS_LOG(WARN, "first stage is not MERGE_JOIN_COMPOSE");
        ret = OB_ERROR;
      }
      else
      {
...
//              unmerge_if_unchanged);
        ...
        //mod:e
        expire_cond_ret = ret;
        if ((OB_SUCCESS == ret|| ret == OB_EXPIRE_CONDITION_ERROR) && OB_SUCCESS == (ret = timeout_check()))
      ...
      if(expire_cond_ret == OB_EXPIRE_CONDITION_ERROR && ret == OB_SUCCESS)
      {
          ret = expire_cond_ret;
      }
    ...
    int ObQueryAgent::stage_two_machine()
    {
      ...
      while (!stop && OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "unexpected error, cur_stage_idx_=%ld, query_stage_size_=%ld",
        ...
        case FILTER:
          YYSYS_LOG(WARN, "chunk server doesn't support having operation");
          ret = OB_ERROR;
          stop = true;
          break;
        ...
        case STAGE_NULL:
        default:
          YYSYS_LOG(WARN, "invalid stage operation, stage=%d", 
    ...
    int ObQueryAgent::do_compose()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to do compose, ret=%d", ret);
      }      
    ...
    int ObQueryAgent::do_orderby()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to orderby result ret=%d", ret);
      }
    ...
    int ObQueryAgent::do_topk(const bool last_round)
    {
      ...
      {
        YYSYS_LOG(WARN, "unexpected error happens, needn't compute topk,"
      ...
      if (OB_SUCCESS == ret)
      {
        if (!last_round)
        {
          ...
          {
            YYSYS_LOG(WARN, "do more than one round topk, invalid param, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to topk result ret=%d", ret);
        }
      }
    ...
    int ObQueryAgent::do_return()
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to start return operator, row_width=%ld, "
    ...
    int ObQueryAgent::do_groupby(bool need_filter)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (all_record_is_one_group && (0 == cur_groupby_operator_->get_cell_size()))
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to init all in one group row ret=%d", ret);
          }
        }
      }
        ...
        bool& result)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "check row fail ret=%d", ret);
      }
      else
      {
        if (result
    ...
    int ObQueryAgent::switch_groupby_operator()
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to init ahead groupby operator ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to copy topk row from groupby operator ret=%d", ret);
        }
      }
    ...
    int ObQueryAgent::build_groupby_result(const bool need_filter)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid merge join result, merge_join_result_=%p, "
                        "merge_join_iter_=%p", 
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to init groupby operator ret=%d", ret);
        }
      }
      ...
      while (OB_SUCCESS == ret)
      {
             ...
             i += org_row_width)
        {
          if (need_filter)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "filter one row failed ret=%d", ret);
            }
          }
          ...
          if (OB_SUCCESS == ret && pass_filter)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              if (!need_fetch_all_result)
              {
                      ...
                      && (cur_groupby_operator_->get_real_memory_used() > mem_limit_.groupby_mem_size_)))
                {
                  if (!((limit_count_ > 0) && (limit_count_ + limit_offset_ <= got_row_count)))
                  {
                    YYSYS_LOG(WARN, "groupby result take too much memory "
            ...
            {
              YYSYS_LOG(WARN, "failed to add row to group by result ret=%d", ret);
            }
          }
          ...
          {
            YYSYS_LOG(WARN, "user limits scan rows from one tablet, only return "
        ...
        else if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get next cell from merge join "
                            "operator ret=%d", ret);
          }
          else if (OB_SUCCESS == ret && OB_SUCCESS != (ret = timeout_check()))
          {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "merge_join_operator result error "
            ...
            else
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "failed to unget one cell ret=%d", ret);
              }
            }
          }
        }
    ...
    int ObQueryAgent::timeout_check()
    {
      ...
      if (timeout_time_ > 0 && yysys::CTimeUtil::getTime() >= timeout_time_)
      {
    ...
    int ObQueryAgent::next_cell()
    {
      ...
      {
        YYSYS_LOG(WARN, "run start_agent() first, inited_=%d, pfinal_result_=%p", 
