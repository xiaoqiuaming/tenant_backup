...
namespace oceanbase
{
  ...
  namespace chunkserver
  {
    ...
    int ObRegularRecycler::prepare_recycle(const int64_t version)
    {
      ...
      if (0 >= version)
      {
        YYSYS_LOG(ERROR, "input invalid version = %ld ", version);
      ...
      {
        YYSYS_LOG(WARN, "cannot load tablets in image, version=%ld", version);
      }
      else if (version != expired_image_.get_data_version())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "cannot destroy previous recycle tablet "
      ...
      const bool do_recycle, const bool only_recycle_removed)
    {
      ...
      if (OB_ITER_END == ret)
      {
        YYSYS_LOG(INFO, "tablet image has no tablets.");
      }
      else if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "begin_scan_tablets error, ret = %d", ret);
      }
      else
      {
        while (OB_SUCCESS == (ret = tablet_image.get_next_tablet(tablet)) )
        {
          if (NULL != tablet && tablet->get_merge_count() == 0)
          {
            ...
            {
              YYSYS_LOG(WARN, "tablet not recycle..");
            }
          }
    ...
    int ObRegularRecycler::recycle_tablet(const common::ObNewRange & range, const int64_t version)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot prepare recycle tablet, version=%ld", version);
      }
      else if ( OB_SUCCESS != (ret = do_recycle_tablet(expired_image_, range)) )
      {
        YYSYS_LOG(WARN, "do recycle tablet error.");
      }
    ...
    int ObRegularRecycler::do_recycle_tablet(const ObTabletImage& image, const common::ObNewRange & range)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot acuqire tablet range = %s", to_cstring(range));
      }
      else
      {
      ...
      {
        YYSYS_LOG(ERROR, "release tablet error.");
      }
    ...
    int ObRegularRecycler::load_tablet(const int64_t version, const int32_t disk_no)
    {
      ...
      {
        YYSYS_LOG(ERROR, " get meta path error, version=%ld, disk_no=%d",
            version, disk_no);
      }
      else if (!FileDirectoryUtils::exists(idx_file))
      {
        YYSYS_LOG(INFO, "meta file = %s not exist, ignore.", idx_file);
        ret = OB_SUCCESS;
      }
      // do not load sstable files, important!
      else if ( OB_SUCCESS != (ret = expired_image_.read(idx_file, disk_no, false) ) )
      {
        YYSYS_LOG(WARN, " read idx file =%s error", idx_file);
      }
        ...
        int32_t disk_no, const char* dir, const char* filename)
    {
      ...
      YYSYS_LOG(DEBUG, "filename=%s,ret=%d,data_version=%ld, eldest v=%ld",
        ...
        int32_t disk_no, const char* dir, const char* filename)
    {
      ...
      if (OB_SUCCESS != manager_.get_serving_tablet_image().include_sstable(id)
          && get_mtime(filename) + SCAN_RECYCLE_SSTABLE_TIME_BEFORE < yysys::CTimeUtil::getTime())
      {
        ...
        int32_t disk_no, const char* dir, const char* filename)
    {
      ...
      YYSYS_LOG(INFO, "rename file = %s to %s.", file_path, dest_path);
      if (0 != ::rename(file_path, dest_path))
      {
        YYSYS_LOG(WARN, "rename file error %d, %s ", errno, strerror(errno));
        ret = OB_IO_ERROR;
      }
      return ret;
    }
    ...
    int ObScanRecycler::do_scan(int32_t disk_no, Filter filter, Predicate pred, Operate op)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "get sstable directory error.");
      }
      else if ( (file_num = ::scandir(directory,
              &file_dirent, filter, ::versionsort)) <= 0
          || NULL == file_dirent )
      {
        YYSYS_LOG(INFO, "directory=%s has nothing.", directory);
      }
      else
      {
        for (int32_t n = 0; n < file_num && OB_SUCCESS == ret; ++n)
        {
          ...
          {
            YYSYS_LOG(ERROR, "scandir return null dirent[%d]. directory=%s", n, directory);
            ret = OB_ERROR;
          }
          else
          {
            ...
            YYSYS_LOG(DEBUG, "check file %s,%s", directory, filename);
    ...
    int ObExpiredSSTablePool::get_expired_sstable(int32_t disk_no,char *path,int32_t size)
    {
      ...
      {
        YYSYS_LOG(WARN,"expired sstable pool not init");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (files_list_[disk_no - 1].files_num > 0)
        {
          ...
          YYSYS_LOG(DEBUG,"file [%s] will be reused",path);
        }
      }
      return ret;
    }
    ...
    int ObExpiredSSTablePool::scan_disk(int32_t disk_no)
    {
      ...
      else
      {
        ...
        if (FileDirectoryUtils::is_directory(path))
        {
          ...
          {
            YYSYS_LOG(WARN,"open %s failed",path);
            ret = OB_ERROR;
          }
          else
          {
