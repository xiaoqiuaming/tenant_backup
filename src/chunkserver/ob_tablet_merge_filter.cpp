...
namespace oceanbase
{
  namespace chunkserver
  {
      ...
      ObTablet* tablet, const int64_t frozen_version, const int64_t frozen_time)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, tablet=%p, frozen_version=%ld, "
                        "frozen_time=%ld",
      ...
      {
        YYSYS_LOG(WARN, "invalid table_id=%lu", table_id);
        ret = OB_ERROR;
      }
      else if (!hash_map_inited_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to create hash map for select column "
    ...
    void ObTabletMergerFilter::reset_for_expire_error()
    {
        expire_filter_.reset();
    }
      ...
      const int64_t last_do_expire_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, frozen_version=%ld, last_do_expire_version=%ld",
          frozen_version, last_do_expire_version);
      }
      else if (NULL == (table_schema = schema_->get_table_schema(table_id_)))
      {
        YYSYS_LOG(ERROR, "table=%lu not exist.", table_id_);
      }
      else if (frozen_version >= table_schema->get_expire_frequency() + last_do_expire_version)
      {
        ...
        if(0 == strcmp(expire_condition, "1=0"))
        {
            ret =true;
        }
        else
        {
            ...
            YYSYS_LOG(INFO, "expire info, frozen_version=%ld, expire_frequency=%ld, "
     ...
     //mod  peiouya [EXPIRE_INFO_NOT_ARITHMETIC_FIX] 20160608:e
    {
      ...
      ret = parser.parse(expr, expr_array, is_expire_info,true,schema_,table_schema.get_table_name());
      //mod  peiouya [EXPIRE_INFO_NOT_ARITHMETIC_FIX] 20160608:e
      if (OB_SUCCESS == ret)
      {
          ...
          else
          {
            if (ObExpression::COLUMN_IDX == type)
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(ERROR, "expire condition includes invalid column name, "
      ...
      const int64_t buf_size) const
    {
      ...
      else
      {
        ...
        if (NULL == tm)
        {
          YYSYS_LOG(WARN, "failed to transfer frozen time to tm, "
    ...
    */
                                                          ...
                                                          const bool                    is_expire_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "parse cond_expr:%.*s error, ret=%d",
            cond_expr.length(), cond_expr.ptr(), ret);
      }
      else if ((expr_obj_count = expr_array.get_array_index()) <= 0)
      {
        YYSYS_LOG(WARN, "no expr obj:expr_obj_count=%ld", expr_obj_count);
      }
      else
      {
        while (i < expr_obj_count && OB_SUCCESS == ret)
        {
          ...
          else if (type == ObExpression::COLUMN_IDX)
          {
            ...
            {
              YYSYS_LOG(WARN, "illegal postfix expr, type=%d, colname:%.*s",
            ...
            {
              YYSYS_LOG(WARN, "invalid column :%.*s", column_name.length(), column_name.ptr());
              ret = OB_INVALID_ARGUMENT;
            }
            else
            {
          ...
          if (OB_SUCCESS == ret && type != ObExpression::END)
          {
            ...
            {
              YYSYS_LOG(WARN, "add expr obj :%s error, ret=%d", to_cstring(*obj), ret);
            }
            else  if (OB_SUCCESS != (ret = sql_expr.add_expr_obj(obj_value1)))
            {
              YYSYS_LOG(WARN, "add expr obj :%s error, ret=%d", to_cstring(obj_value1), ret);
            }
            else if (type == ObExpression::COLUMN_IDX || type == ObExpression::OP)
            {
              if (OB_SUCCESS != (ret = sql_expr.add_expr_obj(obj_value2)))
              {
                YYSYS_LOG(WARN, "add expr obj :%s error, ret=%d", to_cstring(obj_value2), ret);
              }
            }
          }
    ...
    */
                                               ...
                                               const bool                    is_expire_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, objs=%p, obj_count=%ld",
          objs, obj_count);
        ret = OB_ERROR;
      }
      else
      {
        ...
        YYSYS_LOG(DEBUG, "parse done. expr_array len=%ld",
      ...
      const ObString& column_name, int64_t& column_index)
    {
      ...
      else
      {
        ...
        if (NULL == column_schema)
        {
          YYSYS_LOG(WARN, "failed to get column schema, table_name=%s, "
                          "column_name=%.*s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to add expire column into scan_param, "
                          "column_id=%lu, ret=%d",
      ...
      const ObObj* post_expr)
    {
      ...
      if (NULL == post_expr)
      {
        YYSYS_LOG(WARN, "invalid param, post_expr=%p", post_expr);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to add expire composite column <column_name, idx> pair "
                          "hash_err=%d, column_name=%s, expire_column_idx=%ld",
        ...
        {
          YYSYS_LOG(WARN, "failed to add expire composite column into scan_param, "
                          "column_name=%s, ret=%d",
            comp_cname, ret);
        }
        else
               ...
               && expire_condition[0] != '\0')
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(INFO, "success to transfer infix expression to postfix "
                              "expression, infix_expr=%s",
                infix_condition_expr);
            }
          }
    ...
    int ObTabletMergerFilter::fill_cname_to_idx_map(const uint64_t column_group_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "cann't find this column group=%lu, table_id=%ld", column_group_id, table_id_);
        ret = OB_ERROR;
      }
      else
      {
        for(int64_t i = 0; i < size; ++i)
        {
          ...
          {
            YYSYS_LOG(INFO, "id=%ld is rowkey column of table:%ld", col[i].get_id(), table_id_);
          ...
          {
            YYSYS_LOG(WARN, "fail to add column <column_name, idx> pair hash_err=%d",
              hash_err);
            ret = OB_ERROR;
            break;
          }
        ...
        const uint64_t column_group_id, ObScanParam& scan_param)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "tablet merge filter doesn't initialize");
        ret = OB_ERROR;
      }
      else if (column_group_idx < 0 || column_group_idx >= column_group_num_
          || OB_INVALID_ID == column_group_id)
      {
        YYSYS_LOG(WARN, "invalid param, column_group_idx=%ld, column_group_num_=%ld, "
            "column_group_id=%lu",
      ...
      else if (need_filter_ && 0 == column_group_idx)
      {
        ...
        {
          YYSYS_LOG(WARN, "column count in scan param is unexpected, "
        ...
        const int64_t row_num, ObSSTableRow& sstable_row, bool& is_expired)
    {
      ...
      else if (need_filter_ && 0 == column_group_idx)
      {
        ...
        {
          YYSYS_LOG(WARN, "column count in sstable row is unexpected, "
        ...
        else
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to filter expire sstable row, objs=%p, "
                            "obj_count=%ld, ret=%d",
        ...
        else
        {
          ...
          if (OB_SUCCESS == ret && is_expired && column_group_num_ > 1)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to set expire bitmap filter, column_group_idx=%ld, "
    ...
    int ObTabletMergerFilter::build_expire_condition_filter(sql::ObSqlExpression& sql_expr, bool& has_filter)
    {
      ...
      else if (NULL == (table_schema = schema_->get_table_schema(table_id_)))
      {
        ...
        ret = OB_ERROR;
          ...
          || expire_condition[0] == '\0')
      {
        YYSYS_LOG(INFO, "table %ld has no expire condition expression.", table_id_);
        ret = OB_SUCCESS;
      }
      else if (static_cast<int64_t>(strlen(expire_condition)) >= OB_MAX_EXPIRE_CONDITION_LENGTH)
      {
        YYSYS_LOG(WARN, "table %ld expire condition too large,"
        ...
        const bool has_column_filter, const bool has_condition_filter)
    {
      ...
      if (has_column_filter && has_condition_filter)
      {
        ...
        {
          YYSYS_LOG(WARN, "add AND op item error, ret=%d", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != (ret = sql_expr.add_expr_item(item)))
        {
          YYSYS_LOG(WARN, "add NE op item error, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = sql_expr.add_expr_item_end()))
        {
          YYSYS_LOG(WARN, "add_expr_item_end ret = %d", ret);
        }
      }
    ...
    int ObTabletMergerFilter::adjust_scan_param(sql::ObSqlScanParam& scan_param)
    {
      ...
      else if (OB_SUCCESS != (ret = build_expire_condition_filter(*sql_expr, has_condition_filter)))
      {
        ...
        YYSYS_LOG(WARN, "build_expire_condition_filter error, ret =%d", ret);
      }
      else if (OB_SUCCESS != (ret = finish_expire_filter(*sql_expr, false, has_condition_filter)))
      {
        ObSqlExpression::free(sql_expr);
        YYSYS_LOG(WARN, "finish_expire_filter ret = %d", ret);
      }
      else if (OB_SUCCESS != (ret = scan_param.add_filter(sql_expr)))
      {
        YYSYS_LOG(WARN, "add_filter to scan_param ret = %d, sql_expr=%s", ret, to_cstring(*sql_expr));
      }
      else
      {
        YYSYS_LOG(INFO, "add_filter to scan_param sql_expr=%s", to_cstring(*sql_expr));
      }
