...
//    : ObCellStream(rpc_proxy, server_type, time_out)
    ...
    : ObCellStream(rpc_proxy, server_type, time_out, paxos_group_id)
    //mod 20151209:e
    {
      finish_ = false;
      reset_inner_stat();
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "IIII am scaning");//for debug lijianqiang
    ...
    int ObScanCellStream::scan_row_data()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ret = ObCellStream::rpc_scan_row_data(cur_scan_param_, paxos_group_id_);
        if (ret != OB_SUCCESS)
        {
