  ...
  namespace chunkserver
  {
        ...
        const ObServerType type, const ObUpsList & list)
    {
          ...
          || (list.ups_count_ > MAX_LIST_COUNT))
      {
        ...
        YYSYS_LOG(WARN, "check max fail count or server list failed:fail[%d], timeout[%ld], count[%d]",
    ...
    bool ObUpsBlackList::check(const int32_t server_index)
    {
      ...
      if ((server_index < server_count_) && (fail_counter_[server_index].fail_count_ >= max_fail_count_))
      {
        ...
        if (yysys::CTimeUtil::getTime() - fail_counter_[server_index].fail_timestamp_ >= black_timeout_)
        {
          YYSYS_LOG(DEBUG, "pull the server to alive again:index[%d], count[%d], time[%ld], fail_count[%d]",
        ...
        {
          YYSYS_LOG(WARN, "check server is in blacklist:server[%s]", to_cstring(fail_counter_[server_index].server_));
          ret = false;
        }
      }
      return ret;
    ...
    void ObUpsBlackList::fail(const int32_t server_index, const common::ObServer & server)
    {
      if ((server_index < server_count_) && (fail_counter_[server_index].server_ == server))
      {
        ...
        {
          fail_counter_[server_index].fail_timestamp_ = yysys::CTimeUtil::getTime();
        }
        ++fail_counter_[server_index].fail_count_;
      }
      else
      {
        YYSYS_LOG(WARN, "check param failed:index[%d], count[%d], server[%u], port[%d]",
