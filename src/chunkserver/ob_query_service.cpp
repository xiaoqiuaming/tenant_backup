...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    : chunk_server_(chunk_server),
      ...
      multi_ups_scan_streams_(&rpc_proxy_, MERGE_SERVER),
      //add 20151210:e
                            ...
                            const int64_t timeout_time)
    {
      ...
      {
        YYSYS_LOG(WARN,"get param cell size error, cell_size=%ld, row_index=%p",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get the latest schema, unexpected error");
          ret = OB_SCHEMA_ERROR;
        }
        else
        {
          for (i = 0; i < row_count && OB_SUCCESS == ret; ++i)
          {
            if (table_id != get_param[row_index[i].offset_]->table_id_)
            {
              ...
              {
                YYSYS_LOG(WARN, "fail to get table schema, table_id=%lu", table_id);
                ret = OB_SCHEMA_ERROR;
              }
            }
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to store cell array to scanner, ret=%d", ret);
        }
      }
                                      ...
                                      ObScanner& scanner)
    {
      ...
      while (OB_SUCCESS == ret && OB_SUCCESS == (ret = query_agent_.next_cell()))
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "row cells count is too much, row_cells_cnt_=%ld, "
      ...
      {
        YYSYS_LOG(WARN, "error occurs while fill get data, ret=%d", ret);
      }
                             ...
                             const int64_t timeout_time)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get table schema, table_id=%lu",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        ret = query_agent_.start_agent(
              //mod lijianqiang [MultiUPS] [MERGE] 20151210:b
              //scan_param, ups_scan_cell_stream_,
              scan_param, multi_ups_scan_streams_,
              //mod 20151210:e
              ups_get_cell_stream_, *schema_mgr_,
              mem_limit, timeout_time);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        // scan_param.dump();
        // scanner.dump_all(YYSYS_LOG_LEVEL_DEBUG);
      }
    ...
    int ObQueryService::fill_scan_data(ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        scanner.set_data_version(multi_ups_scan_streams_.get_data_version());
        //mod 20151210:e
      ...
      {
        YYSYS_LOG(WARN, "error occurs while fill scan data, ret=%d", ret);
      }
