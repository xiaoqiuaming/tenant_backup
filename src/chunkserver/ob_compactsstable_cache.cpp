...
namespace oceanbase
{
  ...
  namespace chunkserver
  {
    ...
    int ObCompacSSTableMemGetter::get(ObTablet* tablet,int64_t data_version)
    {
      ...
      {
        YYSYS_LOG(WARN,"fill scan param failed,ret=%d",ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN,"scan from ups failed,ret=%d",ret);
        }
        else if (OB_SUCCESS == ret)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN,"init compact sstable failed,ret=%d",ret);
            }
            else if ((ret = fill_compact_sstable(cell_stream_,cache->mem_)) != OB_SUCCESS)
            {
              YYSYS_LOG(WARN,"write compact sstable failed,ret=%d",ret);
            }
            else
            {
              //success
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            //YYSYS_LOG(INFO,"add compact cache to tablet,tablet:[%p,%s],cache:[%p,row_num:%ld,version:%d-%hd-%hd]",
            YYSYS_LOG(INFO,"add compact cache to tablet,tablet:[%p,%s],cache:[%p,row_num:%ld,version:%ld-%ld-%hd]",
    ...
    int ObCompacSSTableMemGetter::fill_scan_param(ObTablet& tablet,int64_t data_version)
    {
      ...
      {
        YYSYS_LOG(ERROR,"add column id to scan_param_ error[%d]",ret);
      }
      return ret;
    }
                                                      ...
                                                      ObVersionRange& version_range) const
    {
      ...
      else
      {
        ...
        else if (ups_major > cs_major)
        {
          if (is_final_minor)
          {
            YYSYS_LOG(WARN,"there is some new frozen table,but we can hold only one major version");
          }
          else
          {
        ...
        else if (ups_major < cs_major)
        {
          ...
          YYSYS_LOG(WARN,"unexpect error,ups_major:%ld,cs_major:%ld",ups_major,cs_major);
        }
        else
        {
          //should not be here,ups_major == cs_major && ups_minor - cs_minor equal to 1
          YYSYS_LOG(WARN,"unexpect error:major(%ld,%ld),minor:(%ld,%ld)",cs_major,ups_major,cs_minor,ups_minor);
        }
      }
    }
    ...
    int ObCompacSSTableMemGetter::fill_compact_sstable(ObScanCellStream& cell_stream,ObCompactSSTableMem& sstable)
    {
      ...
      while((OB_SUCCESS == ret) && (OB_SUCCESS == (ret = cell_stream.next_cell())))
      {
        ...
        {
          YYSYS_LOG(WARN,"compactsstable cache size overflow,usage:%ld,size limit:%ld",
        ...
        if (OB_SUCCESS == (ret = cell_stream.get_cell(&cur_cell,&is_row_changed)))
        {
          if (is_row_changed && row_cell_num_ > 0)
          {
            ...
            {
              YYSYS_LOG(WARN,"append row to sstable failed,ret=%d",ret);
            }
          }
          ...
          if (OB_SUCCESS == ret)
          {
            if (0 == row_cell_num_)
            {
              ...
              {
                YYSYS_LOG(WARN,"add column to row failed,ret=%d",ret);
              }
              else
              {
                ++row_cell_num_;
    ...
    int ObCompacSSTableMemGetter::save_row(uint64_t table_id,ObCompactSSTableMem& sstable)
    {
      ...
      if (row_.get_col_num() > 1) //do not save row that have only rowkey column
      {
        ...
        {
          YYSYS_LOG(WARN,"set end of row_ failed,ret=%d",ret);
        }

        if ((OB_SUCCESS == ret) && ((ret = sstable.append_row(table_id,row_,size)) != OB_SUCCESS))
        {
          YYSYS_LOG(WARN,"add row_ to sstable failed,ret=%d",ret);
        }
      }
    ...
    int ObCompactSSTableMemThread::init(ObTabletManager* manager)
    {
      ...
      if ((NULL == manager))
      {
        YYSYS_LOG(WARN,"invalid argument,manager is null");
        ret = OB_INVALID_ARGUMENT;
      }
    ...
    int ObCompactSSTableMemThread::push(ObTablet* tablet,int64_t data_version)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"no left space to hold this tablet,read_idx_:%d,write_idx_:%d",read_idx_,write_idx_);
          ret = OB_SIZE_OVERFLOW;
        }
        mutex_.unlock();
      }
    ...
    void ObCompactSSTableMemThread::run(CThread *thread, void *arg)
    {
      ...
      if (NULL == getter)
      {
        YYSYS_LOG(WARN,"alloc getter failed");
      }
      else
      {
        while(!_stop)
        {
          ...
          if (NULL != tablet) //just in case
          {
            ...
            {
              YYSYS_LOG(WARN,"get data from ups failed");
            }
