...
int ObTabletService::open(const sql::ObSqlReadParam &sql_read_param)
{
  ...
  if (NULL == sql_scan_param)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "invalid param type");
    }
    else
    {
      YYSYS_LOG(DEBUG, "cs use GET method");
    }
  }
  else
  {
    YYSYS_LOG(DEBUG, "cs use SCAN method");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "chunk server get schema manager fail:ret[%d]", ret);
    }
    else if(NULL == (schema_mgr = merger_schema_mgr->get_user_schema(0)))
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "get user schema mgr fail:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to set tablet mgr:ret[%d]", ret);
      }
      else
      {
  ...
  if (schema_mgr != NULL)
  {
    ...
    {
      ret = err;
      YYSYS_LOG(WARN, "release schema mgr fail:ret[%d]", ret);
    }
  }
...
int ObTabletService::fill_scan_data(ObNewScanner &new_scanner)
{
  ...
  if (timeout <= 0)
  {
    ...
    YYSYS_LOG(WARN, "cs already timeout. timeout[%ld], timeout_us_[%ld], start_time[%ld]", timeout, timeout_us_, start_time);
  }
  ...
  while(OB_SUCCESS == ret)
  {
    if(NULL == cur_row_)
    {
      if(NULL != cur_rowkey_)
      {
        ...
        {
          YYSYS_LOG(WARN, "deep copy rowkey fail:ret[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if(OB_ITER_END == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "new scanner set last row key fail:ret[%d]", ret);
          }
          break;
        }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "get last rowkey fail:ret[%d]", ret);
        }
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      if ((start_time + max_time) < g_cur_time)
      {
        ...
        {
          YYSYS_LOG(WARN, "can't get any row, start_time=%ld timeout=%ld timeu=%ld",
...
int ObTabletService::close()
{
  ...
  {
    ret = err;
    YYSYS_LOG(WARN, "close tablet scan fail:err[%d]", err);
  }
  if (OB_SUCCESS != (err = tablet_get_.close()))
  {
    ret = err;
    YYSYS_LOG(WARN, "close tablet get fail:err[%d]", err);
  }
  return ret;
}
