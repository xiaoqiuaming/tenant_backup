...
int ObRootServerRpcStub::init(const ObServer & root_server, const ObClientManager * rpc_frame)
{
  ...
  {
    YYSYS_LOG(ERROR, "already inited or check input failed:inited[%s], rpc_frame[%p]",
...
ThreadSpecificBuffer::Buffer* ObRootServerRpcStub::get_thread_buffer(void) const
{
  ...
  if (NULL == obj)
  {
    YYSYS_LOG(ERROR, "get ObChunkServerMain instance failed.");
  }
  else
  {
...
int ObRootServerRpcStub::fetch_schema(const int64_t timestamp, ObSchemaManagerV2 & schema)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize timestamp failed:timestamp[%ld], ret[%d].",
          timestamp, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server(%s) for fetch schema failed"
...
int ObRootServerRpcStub::report_tablets(const ObTabletReportInfoList& tablets, int64_t time_stamp, bool has_more)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize ObServer(chunkserver) failed: ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize TabletInfoList failed: ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "report_tablets, timestamp:%ld", time_stamp);
    ret = serialization::encode_vi64(data_buff.get_data(),
        data_buff.get_capacity(), data_buff.get_position(), time_stamp);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "encode_vi64 time_stamp failed: ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "send_request(ReportTablets) to RootServer failed: ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize ObResultCode(report tablet result) failed: ret[%d].", ret);
    }
    else if (OB_SUCCESS != report_result.result_code_)
    {
      YYSYS_LOG(ERROR, "report Tablet failed, the RootServer returned error: ret[%d].",
  ...
  if (OB_SUCCESS == ret)
  {
    if (!has_more)  // at the end of report process
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "ObClientManager::send_request(schema_changed) to "
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "schema_changed request is sent: "
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize ObResultCode(schema changed result)"
              "failed: ret[%d].", ret);
        }
        else if (OB_SUCCESS != report_result.result_code_)
        {
          YYSYS_LOG(ERROR, "report schema changed failed, "
	  ...
	  )
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize server failed[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize is_merge_server failed=[%d]", ret);
    }
  }
  // step 2. send request for fetch new schema
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server for register failed"
          ",ret=[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize status from buff failed,"
    ...
    const int64_t capacity, const int64_t used)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize capacity failed=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize used failed=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server for report capacity failed"
          ",ret=[%d].", ret);
    }
  }
    ...
    const int64_t tablet_version)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize src_server failed[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize dest_server failed=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize keep_src failed=[%d]", ret);
    }
  }
  ...
  const bool is_merge_succ)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize is_merge_succ failed=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret && is_merge_succ)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize merged_tablet_list failed=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server for merge tablets over failed"
          ",ret=[%d].", ret);
    }
  }
...
int ObRootServerRpcStub::get_merge_delay_interval(int64_t &interval) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server for register server failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  return ret;
}
...
int ObRootServerRpcStub::async_heartbeat(const ObServer & client)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize client server addr failed:ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "post request to root server "
    ...
    const int64_t occupy_size, int32_t &dest_disk_no, ObString &dest_path)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize occupy_size failed[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize dest_disk_no failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize dest_path failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
...
int ObRootServerRpcStub::get_last_frozen_memtable_version(int64_t &last_version)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize last_version failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
                             ...
                             const ObScanParam & param, ObScanner & result)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "check get rpc buffer failed:ret[%d]", ret);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize get param failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get data failed from server:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR,"scan failed,ret[%d]",ret);
    }
  }
    ...
    while((OB_SUCCESS == ret) && index < size)
    {
      ...
      else if (cell != NULL)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (0 != port && 0 != ip && 0 != version && -1!= cluster_id)
          {
            ...
            ObTabletLocation addr(version, server, static_cast<int32_t>(cluster_id)); //mod zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b
            location[index++] = addr;
            ip = port = version = 0;
            cluster_id = -1;
          }
