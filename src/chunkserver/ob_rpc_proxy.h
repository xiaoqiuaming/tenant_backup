...
namespace oceanbase
{
  ...
  namespace chunkserver
  {
    ...
    public:
      ...
      common::ObSchemaServiceImpl* get_schema_service()
      ...
      virtual int check_incremental_data_range(
          int64_t table_id, ObVersionRange &version, ObVersionRange &new_range, bool is_strong_consistency = false); /*add zhaoqiong [Truncate Table]:20160318*/
      ...
      int fetch_master_update_server_list(int32_t &count);
      //add 20150630:e

      // fetch update server list
      int fetch_update_server_list(int32_t & count);

      //fetch sys master ups
      //[492]
      int force_fetch_sys_table_update_server_list(ObUpsList &list, int32_t &count);
    ...
    public:
      ...
      //        @scanner return result
        //      mod shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150701:b
      ...
      virtual int sql_ups_scan(const common::ObScanParam & scan_param,
        ...
        /**
         * for sql api rpc,get data from UPS using GET and paxos id,
         * the result is filling into ObNewScanner
         *
         */
        virtual int sql_ups_get(const ObGetParam & get_param,
                                ObNewScanner & new_scanner,
                                const int64_t timeout = 0,
                                const int64_t paxos_id = 0);
        /**
         * for sql api rpc,scan data from UPS using SCAN and paxos id,
         * the result is filling into ObNewScanner
         *
         */
        virtual int sql_ups_scan(const ObScanParam & scan_param,
                                 ObNewScanner & new_scanner,
                                 const int64_t timeout = 0,
                                 const int64_t paxos_id = 0);
        //add 20160104:e
        ...
        //add shili [MUTIUPS] [PHYSICAL_PLAN_TRANSFORM] 20150623:b
        inline void  set_cs_partition_monitor(common::ObPartitionMonitor *part_monitor);
        inline common::ObPartitionMonitor *get_cs_partition_monitor();
        inline  void set_cluster_id(int64_t cluster_id)
        {
          cluster_id_ = (int32_t)cluster_id;
        }
        int get_master_ups_list(common::ObUpsList  &master_ups_list, int64_t &master_ups_count);
        //add  20150623:e

        //add liuzy [Multiups] [UPS_List_Info] 20160427:b
        struct UsablePaoxsInfo
        {
          int64_t version_;
          int64_t use_paxos_num_;
          common::ObBitSet<MAX_UPS_COUNT_ONE_CLUSTER> offline_bitset_;
          UsablePaoxsInfo(): version_(OB_INVALID_VERSION), use_paxos_num_(OB_INVALID_VALUE)
          {}
        };
        int get_paxos_group_offline_info(ObServer &server, ObVersionRange range, const ObClientManager *& client_mgr,
    ...
    private:
      ...
      int ups_get_(RpcT *rpc_stub,
        ...
        //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150701:b
        template<class T, class RpcT>
        int ups_scan_by_paxos_id(RpcT *rpc_stub,
                                 const int64_t paxos_id,
      ...
      template<class T, class RpcT>
      int ups_scan_data(RpcT *rpc_stub,
                        const int64_t paxos_id,
      ...
      int get_update_server(const int64_t paxos_id,
                            const bool is_strong_consistency,
      ...
      int select_update_server(const int64_t paxos_id,
      ...
      //add e
      //add liuzy [Multiups] [UPS_List_Info] 20160427:b
      int refresh_usable_paxos_group_info_array(common::ObServer ms_server,
                                                const ObClientManager *& client_mgr,
                                                common::ObVersionRange range
                                               );
      //20160427:e
    ...
    private:
      ...
      yysys::CThreadMutex update_lock_;             // lock for fetch update server info
      ...
      yysys::CThreadMutex schema_lock_;             // lock for update schema manager

      // update server list
      yysys::CRWLock ups_list_lock_;                // lock for update server list
      ...
      yysys::CRWLock mm_ups_list_lock_;             // lock for update server list
      yysys::CThreadMutex mm_update_lock_;          // lock for fetch update server info
      common::ObServer master_master_ups_;          // update server vip addr
 //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150624:b
      int64_t fetch_master_ups_timestamp_;              // last fetch  master update server timestamp
      yysys::CRWLock master_ups_list_lock_;             // lock for master update server list
      yysys::CThreadMutex master_update_lock_;          // lock for fetch master update server info
      //add dyr [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150825:b
      uint64_t cur_master_ups_list_finger_print_;       //master ups list finger print
      //add dyr 20150825:e
      common::ObUpsList master_update_server_list_;        //master  update server list for read
      //add 20150624:e

      //add shili [MUTIUPS] [PHYSICAL_PLAN_TRANSFORM] 20150623:b
      //int64_t cluster_id_;
      common::ObPartitionMonitor *part_monitor_;
      //add  20150623:e

      //add liuzy [Multiups] [UPS_List_Info] 20160427:b
      common::ObArray<UsablePaoxsInfo> usable_paxos_group_info_array_;
      //20160427:e
      //add zhaoqiong [Schema Manager] 20150327:b
	...
	//add shili [MUTIUPS] [PHYSICAL_PLAN_TRANSFORM] 20150623:b
    inline void  ObMergerRpcProxy::set_cs_partition_monitor(common::ObPartitionMonitor *part_monitor)
    {
      part_monitor_ = part_monitor;
    }

    inline common::ObPartitionMonitor *ObMergerRpcProxy::get_cs_partition_monitor()
    {
      return  part_monitor_;
    }
    //add  20150623:e
