...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObIndexBuilder::init()
    {
      ...
      {
        YYSYS_LOG(ERROR, "null pointer of range_hash_pointer");
        ret = OB_ERROR;
      }
#if 0
      else if(OB_SUCCESS != (ret = sort_.set_run_filename(DEFAULT_SORT_DUMP_FILE)))
      {
        YYSYS_LOG(WARN, "set run file name failed,ret[%d]", ret);
      }
      else
      {
    ...
    {      
      ...
      {
        YYSYS_LOG(ERROR, "MergeSchemaManager pointer is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if(NULL == (current_schema_manager_ = manager_->get_schema(0)))
      {
        YYSYS_LOG(ERROR, "get null schema manager pointer!");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        YYSYS_LOG(INFO,"get tablet range[%s] to build index", to_cstring(new_range_));
        uint64_t table_id = worker_->get_process_tid();
        //IndexList il;
        /*if(OB_SUCCESS != (ret = gen_index_table_list(table_id, il)))
        {
          YYSYS_LOG(ERROR, "gen index table list error,data_table_id[%ld],ret[%d]",table_id,ret);
        }
        else */
        if(OB_SUCCESS != (ret = write_partitional_index_v1(new_range_, table_id, disk_no, tablet_row_count, sample_rate)))
        {
          YYSYS_LOG(ERROR, "gen static index test error,ret[%d]",ret);
        }
        else if(OB_SUCCESS != (ret = tablet->add_local_index_sstable_by_id(sstable_id_)))
        {
          YYSYS_LOG(ERROR, "add sstable_id[%ld] in tablet error",sstable_id_.sstable_file_id_);
        }
        //add liumz[sync local index meta]20161028
        else if(OB_SUCCESS != (ret = update_local_index_meta(tablet, true)))
        {
          YYSYS_LOG(ERROR, "update local index  error, sstable_file_id[%ld]", sstable_id_.sstable_file_id_);
        }
        //add:e
        if (OB_LIKELY(OB_SUCCESS == ret))
        {
          YYSYS_LOG(INFO, "build local index sstable[%ld] successed ! Add in tablet;",sstable_id_.sstable_file_id_);
        }
        //add liumz, [debug for checksum]20160615:b
        ...
        {
          YYSYS_LOG(ERROR,"test::liumz, checksum changed!");
          YYSYS_LOG(WARN, "test::liumz, before tablet <%s>, row count:[%ld], size:[%ld], "
          ...
          YYSYS_LOG(WARN, "test::liumz, after tablet <%s>, row count:[%ld], size:[%ld], "
              "crc:[%ld] row_checksum:[%lu] version:[%ld] sequence_num:[%ld] to report list",
      ...
      if(NULL != current_schema_manager_)
      {
        if(OB_LIKELY(OB_SUCCESS == manager_->release_schema(current_schema_manager_)))
        {
          ...
          //YYSYS_LOG(ERROR,"test whx:: release local index schema success!");
        }
      }
      return ret;
    }
    ...
    int ObIndexBuilder::start_total_index(ObNewRange *range)
    {
      ...
      {
        YYSYS_LOG(ERROR, "some pointer is null,invalid argument");
        ret = OB_ERROR;
      }
      else if(NULL == (current_schema_manager_ = manager_->get_schema(0)))
      {
        YYSYS_LOG(ERROR, "SchemaManagerV2 is NULL pointer!");
        ret = OB_ERROR;
      }
      else
      {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "create new sstable for table[%ld] failed, disk_no[%d]", new_range_.table_id_, disk_no);
        }
        else if(NULL == (range_server = worker_->get_range_info()))
        {
          YYSYS_LOG(WARN, "failed to get range server info");
          ret = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "create total index sstable success,table[%ld]", new_range_.table_id_);
          disk_no_ = disk_no;
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fill scan param for index failed,ret[%d]", ret);
        }
        else if(OB_SUCCESS != (ret = query_agent_.start_agent(param, *ms_wrapper_.get_cs_scan_cell_stream(), range_server)))
        {
          ...
          YYSYS_LOG(WARN, "start query agent failed,ret[%d]", ret);
        }
        else
        {
          //YYSYS_LOG(WARN, "test::whx scan_param = %s", to_cstring(param));
        }
        //YYSYS_LOG(ERROR,"test::whx param = [%s] new_range_[%s],fake_range[%s]", to_cstring(*(param.get_range())),to_cstring(new_range_),to_cstring(*(param.get_fake_range())));
      }
      ...
      if(OB_SUCCESS == ret || OB_ITER_END == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "write total index failed,ret[%d]", ret);
        }
        else
        {
      ...
      if(NULL != current_schema_manager_)
      {
        ...
        {
          current_schema_manager_ = NULL;
          //YYSYS_LOG(ERROR,"test whx:: release global index schema success!");
        }
      }
      return ret;
    }
    ...
    int ObIndexBuilder::trans_row_to_sstrow(ObRowDesc &desc, const ObRow &row, ObSSTableRow &sst_row)
    {
      ...
      //YYSYS_LOG(INFO, "test::whx row = %s",to_cstring(row));
      if(NULL == new_table_schema_)
      {
        YYSYS_LOG(WARN, "null pointer of table schema");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      for(int64_t i = 0; OB_SUCCESS == ret && i < desc.get_column_num();i++)
      {
        ...
        {
          YYSYS_LOG(WARN,"failed in get tid_cid from row desc,ret[%d]",ret);
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "get cell from obrow failed!tid[%ld],cid[%ld],ret[%d]",tid,cid,ret);
          break;
        }
        if(OB_SUCCESS == ret && OB_INDEX_VIRTUAL_COLUMN_ID != cid)
        {
          if(NULL == obj)
          {
            YYSYS_LOG(WARN, "obj pointer cannot be NULL!");
            ret = OB_INVALID_ARGUMENT;
            break;
          }
          else if(OB_SUCCESS != (ret = sst_row.add_obj(*obj)))
          {
            YYSYS_LOG(WARN, "set cell for index row failed,ret = %d",ret);
            break;
          }
          else
          {
            //YYSYS_LOG(INFO,"test::whx add obj[%s] success",to_cstring(*obj));
          }
        }
      }
      return ret;
    ...
    int ObIndexBuilder::write_total_index_v1()
    {
     ...
     // ObRow* row_ptr = &row;
      ...
      if(OB_SUCCESS != ret|| NULL == worker_ || NULL == worker_->get_tablet_manager())
      {
        ...
        YYSYS_LOG(WARN, "worker_ should not be null!");
      }
      else if(OB_SUCCESS != (ret = row_.set_table_id(table_id_)))
      {
        YYSYS_LOG(WARN,"failed to set sstable row tid[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = row_.set_column_group_id(0)))
      {
        YYSYS_LOG(WARN,"failed to set sstable row column_group_id[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = cons_row_desc(desc)))
      {
        YYSYS_LOG(WARN,"failed to construct row desc[%d]", ret);
      }
      else if(OB_SUCCESS != (ret = cons_row_desc_without_virtual(index_desc)))
      {
        YYSYS_LOG(WARN,"failed to construct index row desc[%d]", ret);
      }
      else
      {
      ...
      //YYSYS_LOG(ERROR,"test::liuxiao total cons_row_desc desc:%s",to_cstring(desc));
      ...
      if(OB_SUCCESS == ret)
      {
        for(int64_t i = 0; i < desc.get_rowkey_cell_count();i++)
        {
          ...
          if(OB_SUCCESS != (ret = desc.get_tid_cid(i, tid, cid)))
          {
            YYSYS_LOG(WARN, "get tid cid from row desc failed,i[%ld],ret[%d]",i, ret);
            break;
          }
          else if(OB_SUCCESS != (ret = sort_.add_sort_column(tid, cid, true)))
          {
            YYSYS_LOG(WARN, "set sort column failed,tid[%ld], cid[%ld],ret[%d]",tid, cid, ret);
            break;
          }
        }
      }
      ...
      if(OB_SUCCESS == ret)
      {
        while(OB_SUCCESS == (ret = sort_.get_next_row(row)))
        {
          ...
          {
            YYSYS_LOG(ERROR,"fail to calculate tablet column checksum index =%d",ret);
            break;
          }
          //add liuxiao [secondary index col checksum] 20150525
          else if(OB_SUCCESS != (ret = column_checksum_.sum(cc)))
          {
            YYSYS_LOG(ERROR,"checksum sum error =%d",ret);
            break;
          }
          cc.reset();
          //add e
          ...
          {
            YYSYS_LOG(WARN, "failed to trans row to sstable_row,ret = %d", ret);
            break;
          }
          else if(OB_SUCCESS != (ret = save_current_row()))
          {
            YYSYS_LOG(WARN, "failed to save current row,ret = %d", ret);
            break;
          }
        }
        if(OB_ITER_END == ret)
        {
          {
            ...
            //{
            //  YYSYS_LOG(ERROR,"failed to get ups for report index cchecksum");
            //}
            //delete e
            ...
            {
              YYSYS_LOG(ERROR,"send tablet column checksum failed =%d",ret);
            }
            else
            {
              //ret = OB_SUCCESS;
      ...
      if(OB_SUCCESS == ret)
      {
          ...
          {
            YYSYS_LOG(ERROR,"alloc_index_image_tablet_object failed.");
          }
          else
          {
            if((ret = construct_index_tablet_info(index_image_tablet)) == OB_SUCCESS)
            {
                ...
                if(OB_SUCCESS != (ret = sstable_writer_.close_sstable(trailer_offset, current_sstable_size_)))
                {
                    YYSYS_LOG(ERROR, "Build static index : close sstable failed. ret[%d]", ret);
                }
              else if(OB_SUCCESS != (ret = update_meta_index(index_image_tablet,sync_meta)))
              {
                YYSYS_LOG(WARN,"upgrade new index tablets error [%d]",ret);
              }
            }
          }
      }
      ...
      //if(OB_SUCCESS == ret)
      YYSYS_LOG(INFO,"write total index go to end!");
      sort_.reset();
      //current_sstable_size_ = 0;
    ...
    int ObIndexBuilder::cons_row_desc(ObRowDesc &desc)
    {
      ...
      {
        YYSYS_LOG(WARN, "get rowkey count failed,tid[%ld],ret[%d]", table_id_, ret);
      }
      else
      {
      ...
      for(int64_t i = 0;i < column_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get column def of table[%ld], i[%ld],ret[%d]", table_id_, i, ret);
          break;
        }
        else if(OB_SUCCESS != (desc.add_column_desc(table_id_, col_def->column_name_id_)))
        {
          YYSYS_LOG(WARN, "failed to set column desc table_id[%ld], ret[%d]", table_id_, ret);
          break;
        }
      }
                                                   ...
                                                   uint64_t tid ,uint64_t index_tid)
    {
      ...
      if(NULL == schema)
      {
        YYSYS_LOG(ERROR,"get data table schema error,ret=%d",ret);
        ret=OB_SCHEMA_ERROR;
      }
      else if(NULL == index_schema)
      {
        YYSYS_LOG(ERROR,"get index table schema error,ret=%d",ret);
      }
      else
      {
        ...
        for(int64_t i = 0;i < size;i++)
        {
          ...
          {
            YYSYS_LOG(ERROR,"failed to get rowkey column id from index info,ret=%d",ret);
            break;
          }
        ...
        */
        //YYSYS_LOG(ERROR,"test::liuxiao index desc %s",to_cstring(index_data_desc));
        //construct e
        if(OB_SUCCESS == ret)
        {
          for(int i = 0; i<=count;i++)
    ...
    int ObIndexBuilder::cons_row_desc_without_virtual(ObRowDesc &desc)
    {
        ...
        if(NULL == (index_schema = current_schema_manager_->get_table_schema(table_id_)))
        {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN, "get index table schema error, ret=%d", ret);
        }
        else if(OB_INVALID_ID == (data_tid = index_schema->get_index_helper().tbl_tid))
        {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN, "get data table id error, ret=%d", ret);
        }
        else if(NULL == (schema = current_schema_manager_->get_table_schema(data_tid)))
        {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN, "data table schema is null, ret=%d", ret);
        }
        else
        {
            max_data_table_cid = schema->get_max_column_id();
    ...
    int ObIndexBuilder::gen_index_table_list(uint64_t data_table_id, IndexList &list)
    {
     ...
     // int64_t version = manager_->get_latest_version();
      ...
      else
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR,"hash pointer is null");
        }
    ...
    int ObIndexBuilder::write_partitional_index_v1(ObNewRange range, uint64_t index_tid, int32_t disk, int64_t tablet_row_count, int64_t sample_rate)
    {
      ...
      int64_t row_count = 1;//当row_count等于sample_count的时候将此时的obindexsample加入onindexhistogram中，并且将count和sample初始�?
      int64_t temp_tablet_row_count =1;//记录当前的�?�的rowcount当与tablet的rowcount相等的时候则是最后一行这个时候要将range加入到sample
      ...
      {
        YYSYS_LOG(WARN, "construct row desc failed,ret = [%d]", ret);
      }
      if(OB_SUCCESS == ret)
      {
        if(OB_SUCCESS != (ret = create_new_sstable(index_tid, disk)))
        {
          YYSYS_LOG(WARN, "failed to create new sstable for index[%ld], disk[%d], ret = %d", index_tid, disk, ret);
        }
        else
        {
          basic_columns.clear();
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "table schema or new_table_schema_ error, null pointer!");
          ret = OB_SCHEMA_ERROR;
        }
        ...
        if(OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "push cid indesc failed,data_tid[%ld], index_data_desc = %s, desc = %s, ret = %d", data_tid, to_cstring(index_data_desc), to_cstring(desc), ret );
          }
          ...
          {
            YYSYS_LOG(WARN,"failed to open ObSsTableScan!");
          }
          else
          {
            while(OB_SUCCESS == ret && OB_SUCCESS == (ret = sort_.get_next_row(cur_row)))
            {
              if(NULL != cur_row)
              {
                //YYSYS_LOG(ERROR, "test::whx desc = [%s]",to_cstring(desc));
                if(OB_SUCCESS != (ret = trans_row_to_sstrow(desc, *cur_row, row_)))
                {
                  YYSYS_LOG(WARN,"failed to trans row to sstable row, ret = %d", ret);
                  break;
                }
                else
                {
                  if(1 == row_count || temp_tablet_row_count == tablet_row_count)//construct start key
                  {
                    if(1 == row_count)
                    {
                      for(int i = 0; i < rowkey_info->get_size(); i++)
                      {
                        ...
                        {
                          YYSYS_LOG(ERROR,"write obj faield,ret [%d]",ret);
                          break;
                        }
                      }
                    ...
                    if(temp_tablet_row_count == tablet_row_count)
                    {
                      ...
                      {
                        YYSYS_LOG(WARN,"fail to add sample to histogram =%d",ret);
                        break;
                      }
                      //YYSYS_LOG(ERROR,"test:whx output2 tablet_row_count[%ld],temp_tablet_row_count = %ld", tablet_row_count, temp_tablet_row_count);
                    }
                ...
                if(OB_SUCCESS == ret)
                {
                  ...
                  {
                    YYSYS_LOG(ERROR,"fail to calculate data table column checksum index =%d",ret);
                    break;
                  }
                  else if(OB_SUCCESS != (ret = column_checksum_.sum(cc)))
                  {
                ...
                if(OB_SUCCESS == ret)
                {
                  ...
                  {
                    YYSYS_LOG(ERROR,"write row error,ret=%d",ret);
                    break;
                  }
                }
              }
        ...
        {
          YYSYS_LOG(WARN, "close sstable scan failed!");
        }
      }
      if(true)
      {
    ...
    int ObIndexBuilder::save_current_row()
    {
      ...
      */
      ...
      {
        YYSYS_LOG(ERROR, "write_index : append row failed [%d], this row_, obj count:%ld, "
                         "table:%lu, group:%lu",
    ...
    int ObIndexBuilder::fill_scan_param(ObScanParam &param)
    {
      ...
      if(OB_SUCCESS == ret)
      {
        for(int64_t i = 0; i < sstable_schema_.get_column_count(); i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "get column define failed");
            ret = OB_ERROR;
            break;
          }
          else
          {
            ...
            //YYSYS_LOG(INFO, "test::whx add column %d",(int)def->column_name_id_);
          }
        }
      }
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fill scan param failed[%d]", ret);
      }
      return ret;
    }
    ...
    int ObIndexBuilder::create_new_sstable(uint64_t table_id, int32_t disk)
    {
      ...
      if(disk_no < 0)
      {
        YYSYS_LOG(ERROR,"does't have enough disk space for static index");
        sstable_id_.sstable_file_id_ = 0;
      ...
      if (OB_SUCCESS == ret)
      {
        do
        {
          ...
          {
            YYSYS_LOG(ERROR,"Create Index : can't get the path of new sstable");
            ret = OB_ERROR;
          }
          YYSYS_LOG(DEBUG,"test::whx create new sstable, sstable_path:%s", path_);
          if (OB_SUCCESS == ret)
          {
        ...
        if (OB_SUCCESS == ret)
        {
          //YYSYS_LOG(INFO,"test::whx create new sstable, sstable_path:%s,"
          ...
          YYSYS_LOG(DEBUG,"test::whx create new sstable, path_string_:%.*s", path_string_.length(), path_string_.ptr());
    ...
    {
     // YYSYS_LOG(WARN,"test::zhuyanchao test get into construct index tablet info");
      int ret = OB_SUCCESS;
      if(NULL != tablet && tablet_manager_ != NULL)
      {
       // YYSYS_LOG(WARN,"test::zhuyanchao test get into construct index tablet info");
        ...
        //tablet->set_range(new_range_);
          if(NULL == trailer)
          {
          YYSYS_LOG(ERROR, "null pointor of construct index tablet trailer");
            ret = OB_ERROR;
          }
        else if(OB_SUCCESS != (ret = tablet->add_sstable_by_id(sstable_id_)))
          {
            YYSYS_LOG(ERROR,"STATIC INDEX : add sstable to tablet failed.");
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          current_sstable_size_, true);
        }
      }
      else
      {
        YYSYS_LOG(ERROR,"index image tablet is null");
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int ObIndexBuilder::update_local_index_meta(ObTablet* tablet,const bool sync_meta)
    {
      ...
      if(tablet == NULL)
      {
          YYSYS_LOG(WARN,"update meta index error, null pointor of tablet");
          ret = OB_ERROR;
      }
      if(tablet_manager_ == NULL)
      {
          YYSYS_LOG(WARN,"update meta index error, null pointor of tabletmanager");
          ret = OB_ERROR;
      }
      if(OB_SUCCESS == ret)
      {
        ...
        if(OB_SUCCESS == ret)
        {
          if (sync_meta)
          {
            ...
            if (OB_SUCCESS != (ret = tablet_image.write(
                                 ...
                                 tablet->get_disk_no())))
            {
              YYSYS_LOG(ERROR,"write local index meta failed , version=%ld, disk_no=%d",
                        tablet->get_data_version(), tablet->get_disk_no());
            }
            else
            {
              YYSYS_LOG(INFO,"write local index meta success!");
            }
          }
        }
      }
          ...
          else
          {
            if (sync_meta)
            {
              ...
              if (OB_SUCCESS != (ret = tablet_image.write(
                      ...
                      tablet->get_disk_no())))
              {
                YYSYS_LOG(WARN,"write new index meta failed , version=%ld, disk_no=%d",
                    tablet->get_data_version(), tablet->get_disk_no());
    ...
    {
       //YYSYS_LOG(WARN,"test::zhuyanchao begin to calc checksum");
       int ret=OB_SUCCESS;
       int64_t count = 0;
       const ObRowDesc::Desc *desc_index = desc.get_cells_desc_array(count);
       // YYSYS_LOG(WARN,"test::zhuyanchao begin to calc checksum = %ld",count);
       if(OB_SUCCESS==ret)
       {
           int pos=0,len=0;
           const ObObj* obj=NULL;
    ...
    int ObIndexBuilder::calc_tablet_col_checksum_index_local_tablet(const ObRow& row, ObRowDesc &rowkey_desc, ObRowDesc &index_desc, char *column_checksum, int64_t tid)
    {
      ...
      //YYSYS_LOG(ERROR, "test::liuxiao cal cc in indexbuilder");
      int ret = OB_SUCCESS;
      ...
      if(OB_SUCCESS == ret)
      {
        for(i = 0;i < rowkey_count;i++)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "row obj error NULL");
            break;
          }
          else
          {
    ...
    void ObIndexBuilder::cal_sample_rate(const int64_t tablet_row_count, int64_t &sample_rate)
    {
        if(0 == sample_rate)
        {
            YYSYS_LOG(WARN, "sample_rate should not be zero!");
            sample_rate = 1;
        }
    ...
    int ObIndexBuilder::push_cid_in_desc_and_ophy(uint64_t index_tid, uint64_t data_tid, const ObRowDesc index_data_desc, ObArray<uint64_t> &basic_columns, ObRowDesc &desc)
    {
      ...
      for(int64_t j = 0; j < index_data_desc.get_rowkey_cell_count(); j++)
      {
        ...
        {
          YYSYS_LOG(WARN,"get column schema failed,idx[%ld] ret[%d]",j,ret);
          break;
        }
        else
        {
      ...
      if(OB_SUCCESS == ret)
      {
        for(int64_t j = index_data_desc.get_rowkey_cell_count(); j < index_data_desc.get_column_num();j++)
        {
          if(OB_SUCCESS != (ret = index_data_desc.get_tid_cid(j, tid, cid)))
          {
            YYSYS_LOG(WARN,"get column schema failed,cid[%ld]",cid);
            ret = OB_ERROR;
            break;
          }
          else if(OB_SUCCESS != (ret = basic_columns.push_back(cid)))
          {
            YYSYS_LOG(WARN, "push back basic columns failed, ret = %d", ret);
            break;
          }
          else if(OB_SUCCESS != (ret = sort_.add_sort_column(data_tid,cid,true)))
          {
            YYSYS_LOG(WARN, "add sort column failed ,data_tid[%ld],cid[%ld], ret = %d", data_tid, cid, ret);
            break;
          }
