  ...
  namespace chunkserver
  {
                                          ...
                                          const ObObj &right_value_in)
    {
      ...
      else if (ObCreateTimeType == right_value_in.get_type())
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to get create time from right value [err:%d]", err);
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to get precise time from left value [err:%d]", err);
          }
        }
      ...
      else if (ObModifyTimeType == right_value_in.get_type())
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to get modify time from right value [err:%d]", err);
        }
    ...
    int ObJoinOperator::prepare_join_param_()
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err && NULL != join_info)
        {
          ...
          {
            YYSYS_LOG(WARN,"unepxected error, fail to get ids from join info "
          ...
          else if (apply_join_right_cell.table_id_ != prev_join_right_table_id || need_change_row_key)
          {
            for (offset_idx = 0; offset_idx < join_info->left_column_count_; offset_idx++)
            {
              ...
              {
                YYSYS_LOG(WARN, "schema error, offset_in_rowkey_objs=%ld, left rowkey obj cnt=%ld",
            ...
            if (OB_SUCCESS == err)
            {
              ...
              {
                YYSYS_LOG(WARN, "fail to write string. No sufficient memory may cause this err. [err=%d]", err);
              }
              ...
              while (OB_SUCCESS == err)
              {
                ...
                if (OB_SUCCESS == err)
                {
                    ...
                    )
                  {
                    ...
                    {
                      YYSYS_LOG(WARN,"fail to get join mutation [tableid:%lu,rowkey:%s,"
      ...
      if (OB_SUCCESS == err && join_offset_vec_.size() > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "unexpected implementation algorithm error");
          err = OB_ERR_UNEXPECTED;
        }
        ...
        IteratorT & dst_off_end)
    {
      ...
      for (; dst_off_beg != dst_off_end && OB_SUCCESS == err; ++ dst_off_beg )
      {
        ...
        if (OB_SUCCESS == err && NULL != join_info)
        {
          ...
          {
            YYSYS_LOG(ERROR,"unexpected error, ups return unwanted result [expected_tableid:%lu,real_tableid:%lu]",
        ...
        if (OB_SUCCESS == err && NULL != join_info)
        {
          if (right_cell_column_id == cell.column_id_)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to get join value [left_value.type:%d,right_value.type:%d,err:%d]", affected_cell->value_.get_type(),
            ...
            if (OB_SUCCESS == err && affected_cell->value_.get_ext() != ObActionFlag::OP_ROW_DOES_NOT_EXIST)/// left table row exists
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN,"apply error [affect_cell->table_id:%lu,affect_cell->rowkey:%s,"
    ...
    int ObJoinOperator::join()
    {
        ...
        &&  src_cell_it_beg != join_param_end)
      {
        ...
        if (OB_SUCCESS == err && is_row_changed && !is_first_row)
        {
          ...
          {
            YYSYS_LOG(ERROR, "updateserver return more result than wanted [src_cell_idx:%ld,"
          ...
          {
            YYSYS_LOG(ERROR, "updateserver return result not wanted [src_cell_idx:%ld,"
        ...
        || (src_cell_it_beg  != join_param_array_.end())))
      {
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(ERROR, "%s", "unxpected error, update server return result not coincident with request");
          err = OB_ERR_UNEXPECTED;
        }
      }
      else if (OB_ITER_END == err)
