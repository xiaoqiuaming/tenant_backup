  ...
  namespace chunkserver
  {
        ...
        const ObSchemaManagerV2 &schema_mgr)
    {
      ...
      if (OB_SUCCESS == err)
      {
             ...
             cell_idx < column_size && OB_SUCCESS == err; cell_idx++)
        {
          ...
          {
            YYSYS_LOG(WARN, "unexpected error, fail to get column schema [table_id:%lu,"
    ...
    int ObMergeOperator::apply_whole_row(const ObCellInfo &cell, const int64_t row_beg)
    {
      ...
      if (OB_INVALID_ID != cell.column_id_ && NULL != scan_param_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "unexpected error, fail to get column schema [table_id:%lu,"
                           "column_id:%lu, column_idx:%d,column_schema:%p]",
        ...
        if (OB_SUCCESS == err)
        {
          ...
          for (int64_t i = 0; i < column_index_[column_idx].size_ && OB_SUCCESS == err; ++i)
          {
            ...
            {
              YYSYS_LOG(WARN,"dest idx err [dest_idx:%ld,column_id:%ld, "
            ...
            else
            {
              ...
              if (OB_SUCCESS == err)
              {
                if (cell.column_id_ == dst_cell->column_id_)
                {
                  ...
                  if (OB_SUCCESS != err)
                  {
                    YYSYS_LOG(WARN,"fail to apply mutation [err:%d]", err);
                  }
                }
                else
                {
                  YYSYS_LOG(WARN,"unexpected error, column id not coincident");
                  err = OB_ERR_UNEXPECTED;
                }
              }
              else
              {
                YYSYS_LOG(WARN,"fail to get dest cell from cell array [err:%d]", err);
              }
            }
            if (OB_SUCCESS == err)
            {
    ...
    int ObMergeOperator::append_fake_composite_column()
    {
      ...
      if (NULL != scan_param_)
      {
        ...
        if (comp_column_size > 0 && comp_column_size <= OB_MAX_COLUMN_NUMBER)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "fail to append fake composite column cell err=%d,"
                            "comp_column_size=%ld", err, comp_column_size);
          }
        }
        else if (comp_column_size > OB_MAX_COLUMN_NUMBER)
        {
          YYSYS_LOG(WARN, "composite columns size is bigger than max columns size, "
    ...
    int ObMergeOperator::merge_next_row(int64_t &cur_row_idx, int64_t &cur_row_beg, int64_t &cur_row_end)
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        if (!merger_iterator_moved_ && keep_move)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to move to next cell in merger [err:%d]", err);
          }
        ...
        if (OB_SUCCESS == err && keep_move)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to get cell from merger [err:%d]", err);
          }
          else
          {
            // do nothing
          ...
          || keep_deleted_row))
        {
          ...
          if (src_idx < cur_row_end) // initialize stage
          {
              ...
              && req_param_->operator [](src_idx)->column_id_ == cur_cell->column_id_)
            {
              ...
              {
                YYSYS_LOG(WARN,"fail to append cell to cell array [err:%d]", err);
              }
            }
            else
            {
          ...
          { // cells in current row are initialized
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN,"fail to apply whole row [err:%d]", err);
              }
            }
          }
        }
      ...
      if (OB_SUCCESS == err || OB_ITER_END == err)
      {
          ...
          && src_idx < cur_row_end)
        {
          for (;src_idx < cur_row_end && OB_SUCCESS == err; src_idx ++)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to append initialize cell [err:%d]", err);
            }
            else
            {
              if (exist_cell_num_got > 0)
      ...
      if (OB_SUCCESS == err || OB_ITER_END == err)
      {
        if (not_exist_cell_num > 0 && exist_cell_num_got > 0)
        {
          ...
          if (err >= 0)
          {
            ...
            {
              YYSYS_LOG(ERROR, "fatal error, algorithm error [not_exist_cell_num:%ld]", not_exist_cell_num);
              err = OB_ERR_UNEXPECTED;
            }
            else
            {
    ...
    int ObMergeOperator::scan_merge_row_fast_path()
    {
        ...
        if (OB_SUCCESS == err && !is_del_op(cur_cell->value_.get_ext()))
        {
          if (src_idx < row_width) // initialize stage
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN,"fail to batch append initialize cell, "
      ...
      if (OB_SUCCESS == err || OB_ITER_END == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to append_fake_composite_column, err=%d", err);
        }
    ...
    int ObMergeOperator::get_next_rpc_result()
    {
      ...
      {
        YYSYS_LOG(WARN, "cell stream was not initilized");
        err = OB_ITER_END;
      }
    ...
    int ObMergeOperator::get_next_get_rpc_result()
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        YYSYS_LOG(DEBUG, "OP:cs_get [got_cell_num:%ld,cur_get_cell_num:%ld,data_version:%ld,res:%d,it_size:%ld]",
    ...
    int ObMergeOperator::get_next_scan_rpc_result()
    {
      ...
      ObIterator *cur_ups_cell_it[OB_MAX_ITERATOR];
      int64_t ups_it_size = OB_MAX_ITERATOR;
      //add 20151209:e
      bool ups_scan_iter_end = false;
	  
      if (NULL == org_scan_range)
      {
        YYSYS_LOG(WARN, "%s", "unexpected error, fail to get range from scan param");
      ...
      if (OB_SUCCESS == err )
      {
        ...
        //YYSYS_LOG(DEBUG, "OP:cs_scan [data_version:%ld,res:%d]", cur_cs_result_.get_data_version(), err);
...
//          err = ups_stream_->scan(cur_scan_param_);
          err = ups_stream_->scan(cur_scan_param_, cur_ups_cell_it, ups_it_size);
          //mod 20151209:e
          if (OB_ITER_END == err)
          {
            err = OB_SUCCESS;
        ...
        else if (OB_ITER_END == err)
        {
          ...
          is_need_query_ups_ = false;
          ups_stream_->reset();
          ups_it_size = 0;  //add hongchen 20170906:b
        }
      }
...
//        merger_.add_iterator(ups_stream_);
        ...
        //for (int64_t j=0; j < ups_it_size; j++)
        for (int64_t j=0; is_need_query_ups_ && j < ups_it_size; j++)
        //add hongchen 20170906:e
        {
          YYSYS_LOG(DEBUG, "add one iter,size is[%ld]",ups_it_size);//for debug lijianqiang
          err = merger_.add_iterator(cur_ups_cell_it[j]);
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "add cur ups cell iter failed, ret=%d",err);
            break;
          }
        }
        //mod 20151209:e
    ...
    int ObMergeOperator::do_merge()
    {
      ...
      if (NULL == schema_mgr_)
      {
        YYSYS_LOG(WARN, "please set argument first");
        err = OB_INVALID_ARGUMENT;
      }
