...
namespace oceanbase
{
  ...
  ObIndexWorker::ObIndexWorker() : inited_(false),thread_num_(0),process_idx_tid_(OB_INVALID_ID),schedule_idx_tid_(OB_INVALID_ID),
                                     ...
                                     range_index_(0),hist_width_(0), index_phase_(INIT_PHASE), tablets_have_got_(0), active_thread_num_(0),min_work_thread_num_(0),
    ...
    int ObIndexWorker::init(ObTabletManager *manager)
    {
      ...
      else if(!inited_)
      {
        ...
        int64_t max_work_thread_num = chunk_server.get_config().max_index_build_thread_num;
        ...
        {
          YYSYS_LOG(ERROR, "failed to initialize thread for index[%d]",ret);
        }
        ...
        {
          YYSYS_LOG(ERROR, "failed to init black list array");
        }
      }
      else
      {
        YYSYS_LOG(WARN,"index worker has been inited!");
      }
    ...
    int ObIndexWorker::create_work_thread(const int64_t max_work_thread)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to start [%ld] threads to build index, there is [%ld] threads", max_work_thread, thread_num_);
        }
        min_work_thread_num_ = thread_num_ / 3;
        if(0 == min_work_thread_num_) min_work_thread_num_ = 1;
        YYSYS_LOG(INFO,"index work thread_num=%ld "
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "NOW START CREATE INDEX BUILDER");
      if(OB_SUCCESS != (ret = create_index_builders(builder_, MAX_WORK_THREAD)))
      {
        YYSYS_LOG(ERROR,"failed to create index builders");
      }
      return ret;
    }
    ...
    int ObIndexWorker::create_index_builders(ObIndexBuilder **builder, const int64_t size)
    {
      ...
      if(NULL == builder || 0 > size)
      {
        ...
        YYSYS_LOG(ERROR, "the pointer of index builder is null");
      }
      else if(NULL == (ptr = reinterpret_cast<char*>(ob_malloc(sizeof(ObIndexBuilder)*size, ObModIds::OB_STATIC_INDEX))))
      {
        YYSYS_LOG(WARN, "allocate memory for index builder object error");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if(NULL == schema_mgr_)
      {
        YYSYS_LOG(ERROR, "schema_manager pointer is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if(NULL == tablet_manager_)
      {
        ...
        ret = OB_INVALID_ARGUMENT;
        //modify e
        YYSYS_LOG(ERROR, "tablet_manager pointer is NULL");
      }
      else
      {
        for(int64_t i = 0; i < size; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "init index builder error, ret[%d]", ret);
            ret = OB_ERROR;
            break;
          }
          else
    ...
    int ObIndexWorker::schedule()
    {
      ...
      {
        YYSYS_LOG(INFO,"cannot launch next round.");
        ret = OB_CS_EAGAIN;
      }
      else if (0 == tablet_manager_->get_serving_data_version())
      {
        YYSYS_LOG(INFO,"empty chunkserver, wait for data");
        ret = OB_CS_EAGAIN;
      }
      else
      {
    ...
    int ObIndexWorker:: start_round()
    {
      ...
      {
        YYSYS_LOG(ERROR,"start build index round error");
      }
      ...
      */
      ...
      if(process_idx_tid_ != schedule_idx_tid_)
      {
        ...
        //YYSYS_LOG(ERROR, "test::whx process_idx_tid_[%ld]",process_idx_tid_);
      }
     ...
     */
    int ObIndexWorker::fetch_tablet_info(bool is_local_index, bool need_other_cs, bool need_all_range)
    {
      int ret = OB_SUCCESS;
      ...
      //YYSYS_LOG(INFO,"test::whx version[%ld]",version);
      ...
      if(NULL == schema)
      {
        YYSYS_LOG(ERROR, "failed in take schema!");
        ret = OB_SCHEMA_ERROR;
      }
      else if(NULL == (index_schema = schema->get_table_schema(process_idx_tid_)))
      {
        YYSYS_LOG(ERROR, "failed in find schema[%ld]",process_idx_tid_ );
        ret = OB_SCHEMA_ERROR;
      }
      ...
      if(OB_SUCCESS == ret)
      {
       // YYSYS_LOG(INFO,"test::whx size[%ld]",tablet_array_.count());
        {
          //ref_table_id = index_schema->get_index_helper().tbl_tid;
          YYSYS_LOG(DEBUG,"test::lmz, get tablet image version=[%ld]",tablet_manager_->get_serving_tablet_image().get_serving_image().get_data_version());
          ...
          if(!need_all_range &&
                  is_local_index &&
                  !need_other_cs &&
                  OB_SUCCESS != (ret = tablet_manager_->get_serving_tablet_image().get_serving_image().acquire_tablets_pub(ref_table_id,tablet_list)))
          {
            //释放tablet
            ...
            //add e
            YYSYS_LOG(ERROR,"get tablets error!");
            ret = OB_ERROR;
          }
          ...
          if(OB_SUCCESS == ret)
          {
            ...
            if(is_local_index && need_other_cs)
            {
              ...
              YYSYS_LOG(DEBUG,"test::lmz, data_multcs_range_hash_ released!!!");
            }
            else
            {
              range_hash_.clear();
              ...
              do
                {
                  ...
                  if(ret != OB_SUCCESS)
                  {
                    YYSYS_LOG(WARN,"fetch location failed,no tablet in root_table,ret=%d",ret);
                  ...
                  else
                  {
                    ...
                    ret = parse_location_from_scanner(scanner, start_key,ref_table_id, need_other_cs, need_all_range);
                  }
                  if (ret != OB_SUCCESS)
                  {
           ...
           if(!need_all_range && OB_SUCCESS == ret)
           {
             if(is_local_index && ! need_other_cs)
             {
               ...
               for(ObVector<ObTablet*>::iterator it = tablet_list.begin(); it != tablet_list.end(); ++it)
               {
                 ...
                 */
                 ...
                 {
                   YYSYS_LOG(DEBUG,"test::lmz, is_handled[%d]",is_handled);
                   if(!is_handled)
                   {
                     if(OB_SUCCESS != (ret = is_tablet_need_build_static_index(*it, list, need_index)))
                     {
                       YYSYS_LOG(ERROR,"error in is_need_static_index_tablet,ret[%d]",ret);
                       YYSYS_LOG(DEBUG,"test::lmz, need_index[%d]",need_index);
                     }
                     else if(need_index)
                     {
                       //YYSYS_LOG(INFO,"test::whx add tablet in array");
                       TabletRecord record;
                       record.tablet_ = *it;
                   ...
                   {
                     //YYSYS_LOG(ERROR,"test::whx release tablet");
                   }*/
                 }
                 //add zhuyanchao secondary index image ref count bug 20151231
                 else
             ...
             {
               YYSYS_LOG(DEBUG,"test::lmz, range_hash size[%ld]", range_hash_.size());
               ...
               {
                 YYSYS_LOG(DEBUG,"test::lmz, range_hash range:[%s]", to_cstring(iter->first));
                 if(0 == range_array_.count())
                 {
                   ...
                   {
                     YYSYS_LOG(ERROR, "deep copy range error!");
                   }
                 }
                 else
                 {
                 ...
                 //YYSYS_LOG(INFO,"test::whx range_array_count(%ld)",range_array_.count());
               }
               YYSYS_LOG(DEBUG,"test::lmz, range_array size[%ld]", range_array_.count());
               for(int64_t i = 0;i < range_array_.count();i++)
               {
                 YYSYS_LOG(DEBUG,"test::lmz, range_array range:[%s]", to_cstring(range_array_.at(i).range_));
               }
             }
           }
    ...
    int ObIndexWorker::set_config_param()
    {
      ...
      {
        YYSYS_LOG(ERROR,"init data range hash error,ret=%d",ret);
      }
      if(OB_SUCCESS != (ret = range_hash_.create(hash::cal_next_prime(512))))
      {
        YYSYS_LOG(ERROR,"init index range hash error,ret=%d",ret);
      }
      else
      {
        //reset_failed_record();
    ...
    int ObIndexWorker::parse_location_from_scanner(ObScanner &scanner, ObRowkey &row_key, uint64_t table_id, bool need_other_cs, bool need_all_range)
    {
      int ret = OB_SUCCESS;
      ObRowkey start_key;
      ...
      scanner.dump_all(YYSYS_LOG_LEVEL_ERROR);//test::lmz
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        for(++iter; iter != scanner.end(); ++iter)
        {
          ...
          else if (row_change) // && (iter != last_iter))
          {
            ...
            YYSYS_LOG(DEBUG,"test::lmz, list[0]'s ip [%d], self ip[%d]", list[0].server_.chunkserver_.get_ipv4(), THE_CHUNK_SERVER.get_self().get_ipv4());
            if(need_other_cs)
            {
              if(-1 ==  data_multcs_range_hash_.set(list.get_tablet_range(), list, 1))
              {
                YYSYS_LOG(ERROR,"insert data_multcs_range_hash_ error!");
              }
              else
              {
                YYSYS_LOG(DEBUG,"test::lmz, data_multcs_range_hash_: range[%s]", to_cstring(list.get_tablet_range()));
                //list.print_info();
              }
            }
            else if(need_all_range)
            {
        ...
        if ((OB_SUCCESS == ret) && (start_key != end_key))
        {
          ...
          //list.print_info();//test::lmz
          YYSYS_LOG(DEBUG,"test::lmz, list[0]'s ip [%d], self ip[%d]", list[0].server_.chunkserver_.get_ipv4(), THE_CHUNK_SERVER.get_self().get_ipv4());
          YYSYS_LOG(DEBUG,"test::lmz, need_other_cs[%d]", need_other_cs);
          if(need_other_cs)
          {
            if(-1 ==  data_multcs_range_hash_.set(list.get_tablet_range(), list, 1))
            {
              YYSYS_LOG(ERROR,"insert data_multcs_range hash error!");
              ret = OB_ERROR;
            }
          }
          else if(need_all_range)
          ...
          for (;iter != range_hash_.end(); ++iter)
          {
            YYSYS_LOG(DEBUG,"test::lmz, range:[%s]", to_cstring(iter->first));
          }
          YYSYS_LOG(DEBUG,"test::lmz, range_hash size[%ld]", range_hash_.size());
        }
      }
    ...
    int ObIndexWorker::is_tablet_need_build_static_index(ObTablet *tablet, ObTabletLocationList &list, bool &is_need_index)
    {
      ...
      else
      {
       ...
       // YYSYS_LOG(INFO,"test::whx range_str=%s",str);
      ...
      YYSYS_LOG(DEBUG,"test::whx sstable count=%ld",tablet->get_sstable_id_list().count());
      if(is_need_index && tablet->get_sstable_id_list().count() == ObTablet::MAX_SSTABLE_PER_TABLET)
      {
        YYSYS_LOG(DEBUG,"test::lmz, get_sstable_id_list().count()=[%ld]", ObTablet::MAX_SSTABLE_PER_TABLET);
        is_need_index = false;
      }
      return ret;
    }
    ...
    void ObIndexWorker::finish_phase1()
    {
      ...
      if(local_reported_ <= REPORT_FAILED)
      {
        if(0 == static_index_report_infolist->tablet_list_.get_array_index())
        {
          ...
          YYSYS_LOG(INFO,"partional tablet histogram info report success!");
        }
        //else if(OB_SUCCESS != (ret = send_local_index_info()))
        else if(OB_SUCCESS != send_local_index_info())
        {
          local_reported_ = REPORT_FAILED;
          YYSYS_LOG(ERROR,"partional tablet histogram info report failed!");
          /*
          static_index_report_infolist->reset();
          for(int i = 0; i<MAX_WORK_THREAD; i++)
          {
            builder_[i]->get_allocator()->reuse();   //发送失�?释放内存
            builder_[i]->reset_report_info();
          }
          YYSYS_LOG(WARN,"partional tablet histogram info report failed, memory reuse");
          */
        }
        else
        {
          ...
          for(int i = 0; i<MAX_WORK_THREAD; i++)
          {
            builder_[i]->get_allocator()->reuse();   //发送成�?释放内存
            builder_[i]->reset_report_info();
          }
          YYSYS_LOG(INFO,"partional tablet histogram info report success, memory reuse");
          */
        }
      }
    ...
    int ObIndexWorker::finish_phase2(bool & total_reported)
    {
      ...
      if(ROUND_FALSE >= round_end_)
      {
        ...
        {
          YYSYS_LOG(WARN,"send index tablets info failed = %d",ret);
        }
        else
        {
          total_reported = true;
      ...
      {
        //YYSYS_LOG(ERROR,"test::lmz, delete tablet image version=[%ld]",tablet_manager_->get_serving_tablet_image().get_serving_image().get_data_version());
        ...
        YYSYS_LOG(INFO,"test::whx delete local index sstable!");
        //tablet_manager_->get_serving_tablet_image().get_serving_image().dump();
      }*/
      if(OB_SUCCESS == ret && ROUND_FALSE >= round_end_)
      {
    ...
    int ObIndexWorker::add_tablet_info(ObTabletReportInfo *tablet)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "tablet is null");
      }
      ...
      //modify e
      {
        YYSYS_LOG(ERROR, "copy range failed.");
      }
      else
      {
      ...
      {
        YYSYS_LOG(ERROR, "failed to add tablet report info for sstable local index,null pointer");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObIndexWorker::send_local_index_info()
    {
     ...
     // YYSYS_LOG(WARN,"test::zhuyanchao histogram report info size=%ld",test);
      //add e
      ...
      while (OB_SUCCESS == ret &&histogram_index <tablet_histogram_report_info_list->tablet_list_.get_array_index())
      {
        while (OB_SUCCESS == ret &&histogram_index <tablet_histogram_report_info_list->tablet_list_.get_array_index())
        {
         // YYSYS_LOG(ERROR,"test::zhuyanchao begin to report info");
          ret=tablet_histogram_report_info_list->get_tablet_histogram(*report_info,histogram_index);
           //YYSYS_LOG(WARN,"test::zhuyanchao histogram report info table id =%ld",report_info->tablet_info_.range_.table_id_);
          //YYSYS_LOG(ERROR,"test::whx start dump,histogram_index = %ld,array_index = %ld", histogram_index, tablet_histogram_report_info_list->tablet_list_.get_array_index());
          //report_info->static_index_histogram_.dump();

          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to get tablet histogram info, err=%d", ret);
          }
          else
          {
            ...
            //YYSYS_LOG(ERROR,"test::zhuyanchao report table id=%ld",report_info->tablet_info_.range_.table_id_);
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to add tablet histogram info, err=%d", ret);
            }
            else
            {
        ...
        if(OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "send histogram info");
          ret =tablet_manager_->send_tablet_histogram_report(*report_list,true);
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to send tablet histogram info, err=%d", ret);
          }
        }
        if (OB_SUCCESS == ret)
        {
      ...
      if(OB_ITER_END == ret)
      {
        YYSYS_LOG(INFO, "send last histogram info");
        ret = report_list->add_tablet(*report_info);
        if (OB_SUCCESS != ret)
        {
        YYSYS_LOG(WARN, "failed to add tablet histogram info, err=%d", ret);
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to send tablet histogram report in last round, ret=%d", ret);
        }
      }
      /*for(int i=0;i<MAX_WORK_THREAD; i++)
      {
    ...
    int ObIndexWorker::get_tablets_ranges(TabletRecord *&tablet, RangeRecord *&range, int &err)
    {
      ...
      {
        YYSYS_LOG(INFO,"local_index_process:%.2f%%, local_finished:%ld, local_total:%ld", (double)tablet_index_ / (double)tablet_array_.count() * 100, tablet_index_, tablet_array_.count());
      ...
      else if(tablets_have_got_ == tablet_array_.count() && tablets_have_got_ != 0)
      {
        ...
        {
          YYSYS_LOG(INFO,"normal process report local index tablet!");
          finish_phase1();
          /*
          if(REPORT_SUCCESS == local_reported_)
          {
            YYSYS_LOG(INFO,"report local index tablet success!");
          }
          */
        }
        //todo
      ...
      if(OB_GET_NOTHING == err)
      {
        ...
        {
          YYSYS_LOG(INFO,"global_index_process:%.2f%%, global_finished:%ld, global_total:%ld", (double)range_index_ / (double)range_array_.count() * 100, range_index_, range_array_.count());
        ...
        else if(range_have_got_ == range_array_.count() && range_have_got_ != 0)
        {
          ...
          else if (is_phase_two_need_end() && OB_SUCCESS == (ret = finish_phase2(total_reported)))
          {
            if(total_reported)
            {
              YYSYS_LOG(INFO,"report total index tablet success!");
            }
          }
        }
      }
      //YYSYS_LOG(ERROR,"test::whx ret[%d],err[%d]",ret,err);
      return ret;
    }
    ...
    int ObIndexWorker::release_tablet_array()
    {
      ...
      {
        YYSYS_LOG(WARN, "release local index sstable failed, ret = [%d]", ret);
      }
      //add:e
      if(OB_SUCCESS == ret)
      {
    ...
    void ObIndexWorker::inc_get_tablet_count()
    {
      ...
      {
        tablets_have_got_++;
        if(tablets_have_got_ == tablet_array_.count() && tablets_have_got_ != 0 && check_if_tablet_range_failed(true))
        {
            YYSYS_LOG(INFO, "signal all threads to get failed tablets");
            pthread_cond_broadcast(&cond_);
        }
      }
      else
      {
        YYSYS_LOG(INFO, "all tablet has been consume, check if has failed record");
      }
    }
    ...
    void ObIndexWorker::inc_get_range_count()
    {
      ...
      {
        range_have_got_ ++;
        if(range_have_got_ == range_array_.count() && range_have_got_ != 0 && check_if_tablet_range_failed(false))
        {
            YYSYS_LOG(INFO, "signal all threads to get failed ranges");
            pthread_cond_broadcast(&cond_);
        }
      }
      else
      {
        YYSYS_LOG(INFO, "all range has been consume , check if has failed record");
      }
    }
    int ObIndexWorker::check_self()
    {
      ...
      {
        YYSYS_LOG(ERROR, "tablet manager cannot be NULL!");
        ret = OB_INVALID_ARGUMENT;
      }
      //comment liumz: 当前索引未建�? RS通知create新的索引�? 会在try_stop_mission方法中设置total_work_last_end_time_
      if(OB_SUCCESS == ret && total_work_last_end_time_ != 0 && 0 == active_thread_num_ )
      {
        ret = release_tablet_array();
        YYSYS_LOG(INFO,"index build failed, release all resource!");
        if(OB_SUCCESS == ret)
        {
          reset();
        }
      ...
      if(OB_SUCCESS == ret && 0 == active_thread_num_ && OB_INVALID_ID != process_idx_tid_)
      {
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN,"fetch index stat failed!");
        }
      ...
      else if(OB_SUCCESS == ret && OB_INVALID_ID != process_idx_tid_ && !is_current_index_complete(status) /*&& 0 == active_thread_num_*/)
      {
        ...
        {
          YYSYS_LOG(INFO,"retry report local index tablet after failed!");
        ...
        {
          YYSYS_LOG(INFO,"try report local index tablet after rs reboot or switch!");
        ...
        //mod:e
        {
          ...
          {
            YYSYS_LOG(INFO, "test::lmz, fetch local tablet info");
            //usleep(SLEEP_INTERVAL);
           // tablet_index_ = 0;
            if(OB_SUCCESS != (ret = fetch_tablet_info()))
            {
              YYSYS_LOG(WARN,"fetch talet info failed,ret[%d]", ret);
            }
            ...
            {
              YYSYS_LOG(INFO,"no tablet to build, sleep[%ld] wait it", SLEEP_INTERVAL);
            }
          }
          if(range_have_got_ == range_array_.count() && (OB_INVALID_ID != process_idx_tid_) && !need_signal)
          {
            YYSYS_LOG(INFO, "test::lmz, fetch global tablet info");
            //usleep(SLEEP_INTERVAL);
            is_local_index = false;
            need_other_cs = false;
            //range_index_ = 0;
            if(GLOABL_PHASE == index_phase_ && OB_SUCCESS != (ret = fetch_tablet_info(is_local_index,need_other_cs)))
            {
              YYSYS_LOG(WARN,"fetch talet info failed,ret[%d]", ret);
            }
            else if(range_counter < range_array_.count())
            {
              YYSYS_LOG(INFO, "test::lmz, fetch local tablet info for global index");
              is_local_index = true;
              need_other_cs = true;
            ...
            {
              YYSYS_LOG(INFO,"no range to build, sleep[%ld] wait it", SLEEP_INTERVAL);
            }
          }
          ...
          {
              YYSYS_LOG(DEBUG, "start handle blacklist!");
              if(GLOABL_PHASE == index_phase_)
              {
                  is_local_index = false;
                  need_other_cs = false;
                  if(OB_SUCCESS != (ret = fetch_tablet_info(is_local_index, need_other_cs, true)))
                  {
                      YYSYS_LOG(WARN, "fetch all range info failed,ret[%d]", ret);
                  }
                  is_local_index = true;
                  need_other_cs = true;
                  if(OB_SUCCESS != (ret = fetch_tablet_info(is_local_index, need_other_cs)))
                  {
                      YYSYS_LOG(WARN, "fetch all tablet info failed,ret[%d]", ret);
                  }
              }
            ...
            do
            {
              ...
              if(OB_SUCCESS ==  ret)
              {
                ...
                if(process_idx_tid_ == range.table_id_)
                {
                  ...
                  {
                      range_array_.at(range_index).wok_send_ = 0;
                      range_array_.at(range_index).fail_count_ = 1;
                      pthread_mutex_lock(&mutex_);
                      if(0 == total_work_last_end_time_)
                      {
                          range_array_.at(range_index).if_process_ = 0;
                      }
...
//add wenghaixing [secondary index static_index_build cluster.p2]20150630
    bool ObIndexWorker::is_current_index_failed(const int64_t status)
    {
      ...
      //YYSYS_LOG(WARN, "build local static index timeout, mission_start_time_=%ld, now=%ld,ternal[%ld]", local_work_start_time_, now, nernal);
      if(OB_INVALID_ID == process_idx_tid_)
      {
        ret = false;
        //YYSYS_LOG(INFO,"this is first round of index construction!");
      }
      /*else if(now > local_work_start_time_ + THE_CHUNK_SERVER.get_config().build_index_timeout)
      {
        ret = true;
        YYSYS_LOG(WARN, "build local static index timeout, mission_start_time_=%ld, now=%ld.", local_work_start_time_, now);
      }*/
      else
      {
    ...
    void ObIndexWorker::construct_index(const int64_t thread_no)
    {
      ...
      while(true)
      {
        ...
        //if(phase1_ && !phase2_)
        {
          //YYSYS_LOG(INFO,"test::whx try to get a tablet/range to build index");
        ...
        while(true)
        {
          ...
          //而实际情况里，直到确定索引表构建成功，才会阻塞线�?
          {
            --active_thread_num_;
            YYSYS_LOG(INFO,"there is no tablet need build static index,sleep wait for new index process.");
        ...
        {
          //YYSYS_LOG(INFO,"test::whx the tablet is not null and range is %s",to_cstring(tablet->tablet_->get_range()));
          if(OB_SUCCESS != (ret = get_index_builder(thread_no, builder)))
          {
            YYSYS_LOG(ERROR, "get index builder error, ret[%d]",ret);
          }
          else
          {
            int err_local = OB_SUCCESS;
            YYSYS_LOG(INFO,"now index builder begin to work!");
            //modify zhuyanchao [secondary index static_sstable_build.report]20150320
            //YYSYS_LOG(WARN,"test::zhuyanchao test histogram list size = %ld",static_index_report_infolist->tablet_list_.get_array_index());
            if(OB_SUCCESS != (err_local = builder->start(tablet->tablet_,hist_width_)))//将sample_rate加入了进去zhuyanchao 20150320
            {
              tablet->fail_count_++;
              YYSYS_LOG(WARN, "build partitional index failed,tablet[%s],fail count[%d],if_process[%d],err[%d]",
                        to_cstring(tablet->tablet_->get_range()),(int)tablet->fail_count_, (int)tablet->if_process_,err_local);
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN,"add histogram report info failed = %d",ret);
              }
              //add e
              tablet->fail_count_ = 0;
              tablet->if_process_ = -1;
            }
            //YYSYS_LOG(INFO,"test::whx err_local[%d]",err_local);
          }
          ///TODO,这段代码应该删除
#if 0
          ...
          //YYSYS_LOG(INFO,"test::whx tablets_have_got_++[%ld]",tablets_have_got_);
          pthread_mutex_unlock(&tablet_range_mutex_);
          if ( tablet_manager_->is_stoped() )
          {
            YYSYS_LOG(WARN,"stop in index");
            ret = OB_ERROR;
          }
        }
        else if(OB_GET_RANGES == err)
        {
          //todo 如果是得到一个range，则处于全局索引构建的阶�?
         // YYSYS_LOG(INFO,"test::whx the index range is not null and range is %s",to_cstring(range->range_));
          if(OB_SUCCESS != (ret = get_index_builder(thread_no, builder)))
          {
            YYSYS_LOG(ERROR, "get index builder error, ret[%d]",ret);
          }
          else
          {
            ...
            YYSYS_LOG(INFO,"now index builder begin to build total index [%s]!",to_cstring(range->range_));
            if(OB_SUCCESS != (err_local = builder->start_total_index(&range->range_)))
            {
              YYSYS_LOG(WARN, "build global index failed,err[%d]",err_local);
              if(OB_TABLET_HAS_NO_LOCAL_SSTABLE == err_local)
              {
                ...
                {
                  range->fail_count_++;
                  YYSYS_LOG(WARN, "cannot find local index sst.fake range[%s], fail_count[%ld]", to_cstring(fake_range), range->fail_count_);
                  usleep(RETRY_INTERVAL);//need to wait for other cs done
                  pthread_mutex_lock(&mutex_);
                  if(0 == total_work_last_end_time_)
                  {
                      range->if_process_ = 0;
              ...
              else
              {
                ...
                if(range->fail_count_ > MAX_FAILE_COUNT && !range->wok_send_)
                {
                  ...
                  YYSYS_LOG(WARN,"range failed too much");
                  YYSYS_LOG(WARN, "test::lmz, whipping_wok GLOBAL_INDEX_BUILD_FAILED");
                  if(OB_SUCCESS != (err_local = whipping_wok(GLOBAL_INDEX_BUILD_FAILED, NULL, range->range_)))
                  {
                    ret = OB_INDEX_BUILD_FAILED;
                    YYSYS_LOG(DEBUG, "whipping wok failed and try again in local, ret[%d]", ret);
                    usleep(RETRY_INTERVAL);
                    range->fail_count_ = 1;
                    range->wok_send_ = 0;
                    pthread_mutex_lock(&mutex_);
    ...
    void ObIndexWorker::run(yysys::CThread *thread, void *arg)
    {
      UNUSED(thread);
      SET_THD_NAME("cswork-idx");//[647]
    ...
    bool ObIndexWorker::can_launch_next_round()
    {
      bool ret = false;
     // int64_t now = yysys::CTimeUtil::getTime();
     // YYSYS_LOG(ERROR,"test::whx inited_[%d],active_thread_num_[%ld]",(int)inited_,active_thread_num_);
    ...
    bool ObIndexWorker::if_current_round_finished()
    {
        if(inited_ && 0 == active_thread_num_ && OB_INVALID_ID == process_idx_tid_)
        {
            return true;
        }
        else
    ...
    bool ObIndexWorker::check_if_tablet_range_failed(bool is_local_index)
    {
        bool ret = false;
        if(is_local_index)
        {
    ...
    bool ObIndexWorker::check_if_tablet_range_failed(bool is_local_index, TabletRecord *&tablet, RangeRecord *&range)
    {
      ...
      tablet = NULL;
      range = NULL;
      //YYSYS_LOG(ERROR,"test::whx check if failed here");
      if(is_local_index)
      {
        for(int64_t i = 0; i < tablet_array_.count(); i++)
        {
          //YYSYS_LOG(ERROR,"test::whx if_process set 1 pashe 1,range = [%s],if_process[%d],fail_count[%d]",to_cstring(tablet_array_.at(i).tablet_->get_range()),(int)tablet_array_.at(i).if_process_,(int)tablet_array_.at(i).fail_count_);
          if(tablet_array_.at(i).fail_count_> 0 && 0 == tablet_array_.at(i).if_process_)
          {
            tablet_array_.at(i).if_process_ = 1;
            tablet = &(tablet_array_.at(i));
            //YYSYS_LOG(ERROR,"test::whx if_process set 1 pashe 1");
            ret = true;
            //local_failed_[i]++;
            break;
          }
      ...
      else
      {
        for(int64_t i = 0; i < range_array_.count(); i++)
        {
          if(range_array_.at(i).fail_count_ > 0 && 0 == range_array_.at(i).if_process_)
          {
            ...
            //YYSYS_LOG(ERROR,"test::whx if_process set 1 pashe 2");
            ret = true;
            //local_failed_[i]++;
            break;
          }
...
//          break;
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_INVALID_ARGUMENT;
          YYSYS_LOG(ERROR, "range hash cannot find range,wok_range[%s],hash_size[%ld]",to_cstring(wok_range),range_info->size());
        }
        else
        {
          ...
          else if(in_list)
          {
            ...
            if(OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "get black list failed, ret = [%d]", ret);
            }
            else if(0 == black_list.get_wok_send())
            {
              ...
              while (OB_SUCCESS == (ret = black_list.next_replic_server(next_server)))
              {
                if(OB_SUCCESS != (ret = tablet_manager_->whipping_wok(black_list, next_server)))
                {
                  ...
                  YYSYS_LOG(WARN, "whipping wok failed,ret [%d]", ret);
                }
                else
                {
                  YYSYS_LOG(INFO, "whipping wok succeed!");
              ...
              {
                YYSYS_LOG(WARN, "all replicate failed!!!");
              }
          ...
          {
            ///todo 不在list里的时候，写BlackList并发�?
            if (OB_SUCCESS != (ret = black_list.write_list(wok_range, list)))//一次把所有的server写进�?
            {
              YYSYS_LOG(WARN, "write wok range in black list failed ret[%d]",ret);
            }
            else if (0 == black_list.get_wok_send())
            {
              //bug 20150420
              ...
              {
                YYSYS_LOG(WARN,"pus black list in array failed ret [%d] ",ret);
              }
              //add liumz, [loop whip until one cs success]20151231:b
              else
              {
    ...
    void ObIndexWorker::reset()
    {
      ...
      range_index_ = 0;
      hist_width_ = 0;
      index_phase_ = INIT_PHASE;//add liumz, [static_index range_intersect]20170915
    ...
    void ObIndexWorker::try_stop_mission(uint64_t index_tid)
    {
      ...
      //modify e
      {
        ...
        //modify e
        {
          YYSYS_LOG(WARN,"try stop index build work current idx_tid:[%ld] new idx_tid:[%ld]",process_idx_tid_,index_tid);
          ...
          //modify e
          {
            int64_t time = yysys::CTimeUtil::getTime();
