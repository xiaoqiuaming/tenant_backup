  ...
  namespace chunkserver
  {
    ...
    int ObGetCellStream::next_cell()
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "check get param cells count is 0");
          ret = OB_ITER_END;
        }
        else
        {
        ...
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(DEBUG, "get the next cell return finish");
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "check get next cell failed:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObGetCellStream::get_next_cell(void)
    {
      ...
      if (OB_ITER_END == ret)
      {
        ...
        if (is_cached_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "add cell to cache failed:err[%d]", add_cache_err);
          }
        }
        ...
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(DEBUG, "get cell data all finish");
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get cell data failed:ret[%d]", ret);
        }
      }
      // add cell to cache
      if (OB_SUCCESS == ret)
    ...
    int ObGetCellStream::add_cell_cache(const bool force)
    {
      ...
      if (!force)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get cell failed:ret[%d]", ret);
          ret = OB_ERROR;
        }
        else
        {
          if (row_change)
          {
            ...
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "add and clear result to cache failed:ret[%d]", ret);
            }
          }
          ...
          if (OB_SUCCESS == ret && cur_row_cache_result_valid_)
          {
            ...
            if (ret != OB_SUCCESS)
            {
              ...
              YYSYS_LOG(ERROR, "add cell to scanner failed:ret[%d]", ret);
            }
          }
        }
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "add cache and clear result failed:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObGetCellStream::add_cache_clear_result()
    {
      ...
      if (!row_result_.is_empty() && cur_row_cache_result_valid_)
      {
        if (OB_SUCCESS == ret)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "add row cache failed:ret[%d]", ret);
          }
        }
      }
      // clear
    ...
    int ObGetCellStream::get_new_cell_data(void)
    {
      ...
      if (!ObCellStream::first_rpc_)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "check finish get failed:ret[%d]", ret);
        }
        else if (true == finish)
        {
          YYSYS_LOG(DEBUG, "check cell data is already finished");
          ret = OB_ITER_END;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "modify get param failed:ret[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "check get cell data failed:ret[%d]", ret);
        }
        ...
        if (OB_ITER_END == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "unexpected error, request not fullfilled, but iterator end");
                                          ...
                                          ObGetParam & param)
    {
      ...
      if (end < start)
      {
        YYSYS_LOG(ERROR, "check input param failed:start[%ld], end[%ld]", start, end);
      ...
      else
      {
        ...
        for (int64_t i = start; i <= end; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "add cell to get param failed:pos[%ld], ret[%d]", i, ret);
            break;
          }
        }
      }
    ...
    int ObGetCellStream::check_finish_all_get(bool & finish)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "get scanner full filled status failed:ret[%d]", ret);
        }
        else if (item_count <= 0)
        {
          YYSYS_LOG(ERROR, "check item count failed:item_count[%ld], item_index[%ld]", 
        ...
        else
        {
          ...
          if (is_fullfill)
          {
            ...
            {
              ret = OB_ITEM_COUNT_ERROR;
              YYSYS_LOG(ERROR, "check index error:item_count[%ld], item_index[%ld], "
                               "get_count[%ld]",
          ...
          else if (item_index_ == get_cells_.get_cell_size())
          {
            is_finish_ = true;
            finish = is_finish_;
            YYSYS_LOG(WARN, "check item index succ but fulifill failed:"
    ...
    int ObGetCellStream::add_row_cache(const ObRowCellVec &row)
    {
      ...
      {
        ret = OB_INPUT_PARAM_ERROR;
        YYSYS_LOG(ERROR, "check row key[%s] failed:table_id[%lu]", to_cstring(row_key), table_id);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "put row to cache failed:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObGetCellStream::get_cache_row(const ObCellInfo & key, ObRowCellVec *& result)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check row key failed:table_id[%lu]", table_id);
        ret = OB_INPUT_PARAM_ERROR;
      }
      else if (key.row_key_.length() > OB_MAX_ROW_KEY_LENGTH)
      {
        YYSYS_LOG(WARN, "rowkey length too big [length:%ld, OB_MAX_ROW_KEY_LENGTH:%ld]", 
      ...
      else
      {
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              result = NULL;
              YYSYS_LOG(DEBUG, "find this row[%s] from cache failed:table_id[%lu], ret[%d]", 
                  to_cstring(row_key), table_id, ret); 
            }
            else
            {
              result = &row_result_;
              YYSYS_LOG(DEBUG, "find row[%s] from cache succ", to_cstring(row_key));
            }
          }
          else
          {
            YYSYS_LOG(WARN, "row data init failed:ret[%d]", ret);
          }
        }
      }
      return ret;
