  ...
  namespace chunkserver
  {
    ...
    int ObAgentScan::open()
    {
      ...
      if(NULL == query_agent_)
      {
        ...
        YYSYS_LOG(WARN, "query_agent cannot be a null pointer!");
      }
    ...
    int ObAgentScan::scan()
    {
      ...
      //YYSYS_LOG(ERROR,"test::whx in block 0 ");
	  //mod:e
      sst_scan_.reset();
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        //YYSYS_LOG(ERROR,"test::whx ObAgentScan fake_range[%s]",to_cstring(fake_range));
      }
      if(OB_SUCCESS == ret)
      {
        ...
        if(OB_SUCCESS != ret)
        {
          if(!query_agent_)
          {
            YYSYS_LOG(ERROR, "inner stat error, query agent cannot be null");
          }
          else
          {
      ...
      if(OB_ITER_END == ret)
      {
        ...
        //YYSYS_LOG(ERROR,"test::whx ObAgentScan local_idx_block_end_");
        //ret = OB_SUCCESS;
      }
      return ret;
    }
    ...
    int ObAgentScan::get_next_local_row(const ObRow *&row)
    {
      ...
      const ObRowkey *row_key = NULL;
      {
        ...
        //YYSYS_LOG(ERROR,"test::whx ObAgentScan row[%s]",to_cstring(*row));
        if(OB_ITER_END == ret)
        {
          do
          {
      ...
      if(OB_ITER_END == ret && local_idx_block_end_)
      {
        local_idx_scan_finishi_ = true;
		//mod whx, [bugfix: free memory]20161030:b
        //YYSYS_LOG(ERROR,"test::whx in block 1");
          sst_scan_.reset_scanner();
          //YYSYS_LOG(ERROR,"test::whx in block 0 ");
          sst_scan_.reset();
		//mod:e
      ...
      if(OB_ITER_END == ret && !query_agent_->has_no_use())
      {
        ...
        //YYSYS_LOG(ERROR,"test::whx in block 0 ");
        sst_scan_.reset();
        //YYSYS_LOG(ERROR,"test::whx in block 2");
		//mod:e
      }
    ...
    int ObAgentScan::get_next_local_range(ObNewRange &range)
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN,"should not be here");
      }
      if(OB_SUCCESS == ret)
      {
        ...
        for( ; itr != range_server_hash_->end(); ++itr)
        {
          ...
          else
          {
            ...
            if(find)
            {
              //YYSYS_LOG(INFO,"test:whx I find local range,range[%s]",to_cstring(range));
              break;
            }
          }
        }
    ...
    int ObAgentScan::get_next_row(const ObRow *&row)
    {
      ...
      {
        YYSYS_LOG(WARN,"null pointer of query agent!");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObAgentScan::build_sst_scan_param()
    {
      ...
      {
        YYSYS_LOG(WARN,"null pointer of scan_param!");
        ret = OB_ERROR;
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN,"set range failed!,ret[%d]", ret);
        }
        else
        {
          ...
          for(int64_t i = 0; i < row_desc_.get_column_num();i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "get tid cid failed! ret [%d]",ret);
              break;
            }
