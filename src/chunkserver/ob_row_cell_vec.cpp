...
int ObRowCellVec::init()
{
  ...
  if (NULL == row_buffer_)
  {
    YYSYS_LOG(WARN,"fail to allocate buffer [errno:%d]", errno);
...
int ObRowCellVec::add_cell(const ObCellInfo & cell)
{
  ...
  if (NULL == row_ )
  {
    ...
    YYSYS_LOG(WARN,"row not initialized");
  }
  if (OB_SUCCESS == err && row_key_.length() == 0)
  {
    ...
    {
      YYSYS_LOG(ERROR, "copy cell rowkey failed:%s, err=%d", 
  ...
  {
    YYSYS_LOG(WARN,"row key or tableid not coincident"
  ...
  if (OB_SUCCESS == err)
  {
    ...
    for (int32_t i = 0; i < row_->cell_num_ && OB_SUCCESS == err; i++)
    {
      if (row_->cells_[i].value_.get_type() == ObVarcharType)
      {
        ...
        {
          YYSYS_LOG(WARN,"unexpected error, get varchar [err:%d]",err);
        }
      }
    }
  }
...
int ObRowCellVec::deserialize(const char *buf, const int64_t buf_len, int64_t &pos)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN,"invalid argument[buf:%p, buf_len:%ld, pos:%ld]", buf, buf_len, pos);
    }
  }
