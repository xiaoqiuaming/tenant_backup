...
namespace 
{
  ...
  int allocate_range_buffer(ObNewRange &range, ObMemBuf &buffer)
  {
    ...
    if (0 < buf_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to allocate memory for range key");
  ...
  int get_ups_read_param(ObReadParam & param,  const ObScanner & cs_result)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "chunk server return all data needed");
        err = OB_ITER_END;
      }
    }
    if (OB_SUCCESS == err)
  ...
  namespace chunkserver
  {
        ...
        ObScanParam *scan_param, ObMemBuf &range_buffer)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "unexpected error, can't get range from org scan param");
      ...
      if (OB_SUCCESS == err)
      {
        if (request_fullfilled)
        {
          ...
          //YYSYS_LOG(INFO,"test::whx get_range [%s]",to_cstring(tablet_range));
          if (OB_SUCCESS == err)
          {
        ...
        else
        {
         ...
         // YYSYS_LOG(INFO,"test::whx last_row_key [%s]",to_cstring(tablet_range));
          ...
          {
            YYSYS_LOG(WARN, "unexpected error, scanner should contain at least one cell");
          }
        }
      }
      ...
      //YYSYS_LOG(INFO,"test::whx before set fake range [%s],range[%s]", to_cstring(*scan_param->get_fake_range()),to_cstring(*(scan_param->get_range())));
      ...
      if(scan_param->if_need_fake())
      {
        ...
        //modify e
        //YYSYS_LOG(INFO,"test::whx set fake range [%s]", to_cstring(*scan_param->get_fake_range()));
      }
      //YYSYS_LOG(INFO,"test::whx set fake range [%s],range[%s]", to_cstring(*scan_param->get_fake_range()),to_cstring(*(scan_param->get_range())));
      //add e
      return err;
    }
        ...
        ObMemBuf &range_buffer)
    {
      ...
      if (OB_SUCCESS == err )
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "unexpected error, there should be at least one cell in the scanner");
          }
          use_max_rowkey = true;
        }
      }
    ...
    int get_ups_param(ObGetParam & param, const ObScanner & cs_result)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err && fullfilled_cell_num < param.get_cell_size())
        {
          ...
          {
            YYSYS_LOG(WARN, "check param rollback failed:full_fill[%ld], param_size[%ld]",
