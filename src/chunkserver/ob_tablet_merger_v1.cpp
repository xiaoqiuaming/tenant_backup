...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObTabletMerger::check_tablet(const ObTablet* tablet)
    {
      ...
      if (NULL == tablet)
      {
        YYSYS_LOG(WARN, "tablet is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (!manager_.get_disk_manager().is_disk_avail(tablet->get_disk_no()))
      {
        YYSYS_LOG(WARN, "tablet:%s locate on bad disk:%d, sstable_id:%ld",
      ...
      else if ((tablet->get_sstable_id_list()).count() > 0)
      {
        ...
        {
          YYSYS_LOG(WARN, "can't get the path of sstable, tablet:%s sstable_id:%ld",
              to_cstring(tablet->get_range()), sstable_id);
        }
        else if (false == FileDirectoryUtils::exists(path))
        {
          YYSYS_LOG(WARN, "tablet:%s sstable file is not exist, path:%s, sstable_id:%ld",
        ...
        const common::ObVector<ObTablet*> & tablet_array, const bool sync_meta)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (!old_tablet->is_merged())
        {
          ...
          {
            YYSYS_LOG(WARN,"upgrade new merged tablets error [%d]",ret);
          }
          else
          {
            if (sync_meta)
            {
              ...
              for(int32_t i = 0; i < idx; ++i)
              {
                ...
                {
                  YYSYS_LOG(WARN,"write new meta failed i=%d, version=%ld, disk_no=%d", i ,
              ...
              {
                YYSYS_LOG(WARN,"write old meta failed version=%ld, disk_no=%d",
                    old_tablet->get_data_version(), old_tablet->get_disk_no());
              }
            }
        ...
        {
          YYSYS_LOG(INFO, "current tablet covered by migrated tablets, discard.");
        }
      }
        ...
        sstable::ObSSTableId& sstable_id, char* path, const int64_t path_size)
    {
      ...
      do
      {
        ...
        {
          YYSYS_LOG(WARN, "Merge : can't get the path of new sstable, ");
        }
    ...
    ObTabletMergerV1::ObTabletMergerV1(ObChunkMerge& chunk_merge, ObTabletManager& manager)
      ...
      ms_wrapper_(*(ObChunkServerMain::get_instance()->get_chunk_server().get_rpc_proxy()),
                  ObChunkServerMain::get_instance()->get_chunk_server().get_config().index_timeout),//uncertainty  ups
    ...
    int ObTabletMergerV1::check_row_count_in_column_group()
    {
      ...
      if (pre_column_group_row_num_ != 0)
      {
        ...
        {
          YYSYS_LOG(ERROR,"the row num between two column groups is difference,[%ld,%ld]",
    ...
    int ObTabletMergerV1::save_current_row(const bool current_row_expired)
    {
      ...
      {
        //YYSYS_LOG(DEBUG, "current row expired.");
        //hex_dump(row_.get_row_key().ptr(), row_.get_row_key().length(), false, YYSYS_LOG_LEVEL_DEBUG);
      }
      else if ((ret = writer_.append_row(row_,current_sstable_size_)) != OB_SUCCESS )
      {
        YYSYS_LOG(ERROR, "Merge : append row failed [%d], this row_, obj count:%ld, "
                         "table:%lu, group:%lu",
            ret, row_.get_obj_count(), row_.get_table_id(), row_.get_column_group_id());
        for(int32_t i=0; i<row_.get_obj_count(); ++i)
        {
          row_.get_obj(i)->dump(YYSYS_LOG_LEVEL_ERROR);
        }
      }
      //add liuxiao [secondary index col checksum]20150330
      ...
      else if(ret==OB_SUCCESS)
      {
        ...
        if(is_data_table_ && has_index_or_is_index_)
        {
          ...
          {
            YYSYS_LOG(ERROR,"calc column_checksum error,ret[%d]",ret);
          }
          else if(OB_SUCCESS != (ret = column_checksum_.sum(cc)))
          {
            YYSYS_LOG(ERROR,"sum column_checksum error,ret[%d]",ret);
          }
          //YYSYS_LOG(ERROR,"test::liuxiao table_id:%ld  rowkey desc:%s index_desc:%s",row_.get_table_id(),to_cstring(rowkey_desc_),to_cstring(index_desc_));
          //YYSYS_LOG(INFO,"test::lmz, range:%s, current cc:[%s], column_checksum_:[%s]", to_cstring(new_range_), cc.get_str_const(), column_checksum_.get_str_const());
        }
        ...
        bool& is_tablet_unchanged)
    {
      ...
      //mod lijianqiang [MultiUPS] [MERGE] 20151209:b
//     ms_wrapper_.get_ups_scan_cell_stream()->set_timeout(THE_CHUNK_SERVER.get_config().merge_timeout);
      ms_wrapper_.get_multi_ups_scan_cell_streams()->set_timeout(THE_CHUNK_SERVER.get_config().merge_timeout);
      //mod 20151209:e
      ...
      {
        YYSYS_LOG(ERROR, "wait aio buffer error, column_group_id = %ld", column_group_id);
      }
      else if (OB_SUCCESS != (ret = fill_scan_param( column_group_id )))
      {
        YYSYS_LOG(ERROR,"prepare scan param failed : [%d]",ret);
      }
      else if (OB_SUCCESS != (ret = tablet_merge_filter_.adjust_scan_param(
        column_group_idx, column_group_id, scan_param_)))
      {
        YYSYS_LOG(ERROR, "tablet merge filter adjust scan param failed : [%d]", ret);
      }
      else if (
               OB_SUCCESS != (ret = merge_join_agent_.start_agent(scan_param_,
              //mod lijianqiang [MultiUPS] [MERGE] 20151209:b
//              *ms_wrapper_.get_ups_scan_cell_stream(),
              *ms_wrapper_.get_multi_ups_scan_cell_streams(),
              //mod 20151209:e
      ...
      {
        YYSYS_LOG(ERROR,"set request param for merge_join_agent failed [%d]",ret);
        merge_join_agent_.clear();
        scan_param_.reset();
      }
      ...
      if (ret == OB_EXPIRE_CONDITION_ERROR)
      {
          tablet_merge_filter_.reset_for_expire_error();
          ret = OB_SUCCESS;
      }
      ...
      {
        YYSYS_LOG(ERROR,"create sstable failed.");
      }
      ...
      while(OB_SUCCESS == ret && !is_tablet_unchanged)
      {
        ...
        {
          YYSYS_LOG(WARN,"stop in merging column_group_id=%ld", column_group_id);
            ...
            (ret = merge_join_agent_.get_cell(&cell_, &is_row_changed)) != OB_SUCCESS)
        {
          ...
          if (OB_ITER_END == ret)
          {
            YYSYS_LOG(DEBUG,"Merge : end of file");
            ret = OB_SUCCESS;
            if (need_filter && ROW_GROWING == row_status)
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(ERROR, "tablet merge filter do check_and_trim_row failed, "
          ...
          {
            YYSYS_LOG(ERROR,"Merge : get data error,ret : [%d]", ret);
          }
        ...
        else if ( cell_ != NULL )
        {
          ...
          if ((ROW_GROWING == row_status && is_row_changed))
          {
            if (0 == (row_num_ % chunk_merge_.merge_pause_row_count_) )
            {
              if (chunk_merge_.merge_pause_sleep_time_ > 0)
              {
                if (0 == (row_num_ % (chunk_merge_.merge_pause_row_count_ * 20)))
                {
                  ...
                  YYSYS_LOG(INFO,"pause in merging,sleep %ld",
                ...
                && maybe_change_sstable())
            {
                  ...
                  && current_sstable_size_ > max_sstable_size)
              {
                ...
                YYSYS_LOG(INFO, "column group %ld splited by sstable size, "
              ...
              {
                YYSYS_LOG(ERROR,"update end range error [%d]",ret);
              }
              else
              {
          ...
          if (OB_SUCCESS == ret && ROW_GROWING == row_status)
          {
            ...
            {
              YYSYS_LOG(ERROR,"Merge : add_cell_to_row failed [%d]",ret);
            }
          }
        }
        else
        {
          YYSYS_LOG(ERROR,"get cell return success but cell is null");
          ret = OB_ERROR;
        }
      }

      YYSYS_LOG(INFO, "column group %ld merge completed, ret = %d, version_range=%s, "
    ...
    int ObTabletMergerV1::merge(ObTablet *tablet,int64_t frozen_version)
    {
      ...
      {
        YYSYS_LOG(ERROR,"merge : interal error ,param invalid frozen_version:[%ld]",frozen_version);
        ret = OB_ERROR;
      }
      else if ( OB_SUCCESS != (ret = reset()) )
      {
        YYSYS_LOG(ERROR, "reset query thread local buffer error.");
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (user_schema == NULL)
        {
          YYSYS_LOG(WARN, "get latest but local schema failed:latest[%ld]", schema_manager->get_latest_version());
        ...
        else if ( user_schema->get_version() > chunk_merge_.current_schema_.get_version())
        {
          ...
          {
            YYSYS_LOG(WARN,"table (%lu) schema is not found in frozen schema",tablet->get_range().table_id_);
...
//            chunk_merge_.current_schema_.get_table_schema(tablet->get_range().table_id_)))
      if (ret == OB_SUCCESS)
      {
          ObTableSchema *src_table_schema = NULL;
          uint64_t src_table_id = OB_INVALID_ID;
          new_table_schema_ = chunk_merge_.current_schema_.get_table_schema(tablet->get_range().table_id_);
          if(NULL != new_table_schema_ && OB_INVALID_ID != (src_table_id = new_table_schema_->get_index_helper().tbl_tid))
          {
              src_table_schema = chunk_merge_.current_schema_.get_table_schema(src_table_id);
          }
//        if (NULL == (new_table_schema_ =
//                          chunk_merge_.current_schema_.get_table_schema(tablet->get_range().table_id_)))
        if (NULL == new_table_schema_
                ||(OB_INVALID_ID != src_table_id && NULL == src_table_schema))
        {
          //This table has been deleted
          YYSYS_LOG(INFO,"table (%lu) has been deleted",tablet->get_range().table_id_);
          tablet->set_merged();
          ret = OB_CS_TABLE_HAS_DELETED;
        }
        else if (OB_SUCCESS != (ret = fill_sstable_schema(*new_table_schema_,sstable_schema_)))
        {
          YYSYS_LOG(ERROR, "convert table schema to sstable schema failed, table=%ld",
        ...
        {
          YYSYS_LOG(ERROR,"failed set flag for get old col checksum ret=%d",ret);
        }
        ...
        else
        {
          if(is_data_table_)
          {
            ...
            {
              YYSYS_LOG(ERROR,"failed to create row desc for cal col checksum,ret[%d]",ret);
            }
            //YYSYS_LOG(ERROR,"test::liuxiao table_id:%ld  rowkey desc:%s index_desc:%s",tablet->get_range().table_id_,to_cstring(rowkey_desc_),to_cstring(index_desc_));
          }
          else if(!is_data_table_ && has_index_or_is_index_)
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN,"no compressor with this sstable.");
        }
        ...
        {
          YYSYS_LOG(INFO, "schema doesn't specify max_sstable_size, "
         ...
         * split algorithm
         */
        if (over_size_percent > 0 && occupy_size > 0 && occupy_size <= split_size && row_count > 0)
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "start merge sstable_id:%ld, old_version=%ld, "
        ...
        if ( OB_SUCCESS != (ret = sstable_schema_.get_table_column_groups_id(
            ...
            || column_group_num <= 0)
        {
          YYSYS_LOG(ERROR,"get column groups failed, column_group_num=%ld, ret=%d",
        ...
        {
          YYSYS_LOG(ERROR, "failed to initialize tablet merge filter, table=%ld",
              tablet->get_range().table_id_);
        }
        ...
        while(OB_SUCCESS == ret)
        {
          ...
          for(int32_t group_index = 0; (group_index < column_group_num) && (OB_SUCCESS == ret); ++group_index)
          {
            ...
            {
              YYSYS_LOG(ERROR, "merge column group[%d] = %lu , group num = %ld,"
            ...
            {
              YYSYS_LOG(ERROR, "check row count column group[%d] = %lu",
                  group_index, column_group_ids[group_index]);
            }
          ...
          {
            YYSYS_LOG(ERROR,"close sstable failed [%d]",ret);
          }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(INFO,"this tablet has max flag,reset it");
        ...
        {
          YYSYS_LOG(ERROR,"merge failed,don't add these tablet");
    ...
    int ObTabletMergerV1::fill_sstable_schema(const ObTableSchema& common_schema,ObSSTableSchema& sstable_schema)
    {
      ...
      if (binary_format)
      {
        YYSYS_LOG(INFO, "table=%ld, rowkey info=%s, binary length=%ld",
    ...
    int ObTabletMergerV1::create_new_sstable()
    {
      ...
      if (disk_no < 0)
      {
        YYSYS_LOG(ERROR,"does't have enough disk space");
        sstable_id_.sstable_file_id_ = 0;
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        do
        {
          ...
          {
            YYSYS_LOG(ERROR,"Merge : can't get the path of new sstable");
            ret = OB_ERROR;
          }
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO,"create new sstable, sstable_path:%s, compressor:%.*s, "
                  ...
                  OB_SSTABLE_STORE_DENSE, sstable_block_size, element_count)) != OB_SUCCESS)
          {
            ...
            YYSYS_LOG(ERROR,"Merge : create sstable failed : [%d]",ret);
          }
        }
      }
      return ret;
    ...
    int ObTabletMergerV1::fill_scan_param(const uint64_t column_group_id)
    {
      ...
      if (NULL == col || size <= 0)
      {
        YYSYS_LOG(ERROR,"cann't find this column group:%lu, table_id=%ld",
      ...
      else
      {
        for(int32_t i = 0; i < size; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR,"add column id [%u] to scan_param_ error[%d]",(col + i)->column_name_id_, ret);
            break;
          }
        }
      }
    ...
    int ObTabletMergerV1::update_range_start_key()
    {
      ...
      {
        YYSYS_LOG(WARN, "set split_rowkey_(%s) as start key of new range error.", to_cstring(split_rowkey_));
      }
      else
      {
    ...
    int ObTabletMergerV1::update_range_end_key()
    {
      ...
      {
        YYSYS_LOG(INFO, "table: %ld has %ld column rowkey",
            new_range_.table_id_, rowkey_column_count);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = row_.get_rowkey(rowkey)))
      {
        YYSYS_LOG(ERROR, "get rowkey for current row error.");
      }
      else if (OB_SUCCESS != (ret = rowkey.deep_copy(split_rowkey_, allocator)))
      {
        YYSYS_LOG(ERROR, "deep copy current row:%s to split_rowkey_ error", to_cstring(rowkey));
      }
      else
      {
        YYSYS_LOG(INFO, "current rowkey:%s,split rowkey (%s), split pos=%d",
    ...
    int ObTabletMergerV1::create_hard_link_sstable(int64_t& sstable_size)
    {
      ...
      if (old_tablet_->get_sstable_id_list().count() > 0)
      {
        ...
        do
        {
          ...
          {
            YYSYS_LOG(ERROR, "create_hard_link_sstable: can't get the path of hard link sstable");
            ret = OB_ERROR;
          }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "create_hard_link_sstable: can't get the path of old sstable");
            ret = OB_ERROR;
          }
          else if (0 != ::link(old_path, path_))
          {
            YYSYS_LOG(ERROR, "failed create hard link for unchanged sstable, "
      ...
      ObTablet*& new_tablet)
    {
      ...
      new_tablet = NULL;
      YYSYS_LOG(DEBUG,"Merge : finish_sstable");
      if (!is_tablet_unchanged)
      {
        row_checksum = writer_.get_row_checksum();
        if (OB_SUCCESS != (ret = writer_.set_tablet_range(new_range_)))
        {
          YYSYS_LOG(ERROR, "failed to set tablet range for sstable writer");
        }
        else if (OB_SUCCESS != (ret = writer_.close_sstable(trailer_offset,sstable_size)))
        {
          YYSYS_LOG(ERROR,"Merge : close sstable failed.");
        }
        else if (sstable_size < 0)
        {
          YYSYS_LOG(ERROR, "Merge : close sstable failed, sstable_size=%ld", sstable_size);
            ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR,"alloc_tablet_object failed.");
        }
        else
        {
          if (is_tablet_unchanged)
          {
            ...
            {
              //是索引表或有有建索引表的数据�?
              //如果没有增量数据则需要从rs取旧的回�?is_tablet_unchanged)
              //如果有增量的话，finish tablet的时候就已经发�?
              //左边条件：如果是索引表，任何情况都去�?
              //右边条件：如果是数据表，如果有索引且没有init状态的索引表，那么有旧的列校验和可�?
              ...
              {
                YYSYS_LOG(ERROR,"failed to get ups for report cchecksum");
              }
              else
              {
                if(OB_SUCCESS != (ret = this->manager_.get_old_tablet_column_checksum(tablet->get_range(),column_checksum_)))
                {
                  YYSYS_LOG(ERROR,"merge v1 start get ccchecksum failed ret=%d  table_id=%ld",ret,tablet->get_range().table_id_);
                }
                else if(OB_SUCCESS != (ret = manager_.send_tablet_column_checksum(column_checksum_,new_range_,frozen_version_)))
                {
                  YYSYS_LOG(ERROR,"merge v1 start send ccchecksum failed ret=%d table_id=%ld",ret,tablet->get_range().table_id_);
                }
              }
              */
              ...
              if(!need_getold_cchecksum()
                      || THE_CHUNK_SERVER.get_config().skip_cchecksum_if_error) //add liumz, [add cchecksum switch]20170912
              {
                  //init index  or  data table without available index
                  ret = OB_SUCCESS;
                  YYSYS_LOG(WARN,"merge v1 start get ccchecksum failed ret=%d  table_id=%ld",ret,tablet->get_range().table_id_);
              }
              else if(OB_SUCCESS != (ret = this->manager_.get_old_tablet_column_checksum(new_range_,column_checksum_)))
              {
                  YYSYS_LOG(ERROR,"merge v1 start get ccchecksum failed ret=%d  table_id=%ld",ret,tablet->get_range().table_id_);
              }
              else if(OB_SUCCESS != (ret = manager_.send_tablet_column_checksum(column_checksum_,new_range_,frozen_version_)))
              ...
              YYSYS_LOG(DEBUG,"test::lmz, range:%s, old cc:[%s]", to_cstring(new_range_), column_checksum_.get_str_const());
              //modify e
            }
            //add e
          ...
          else
          {
            ...
            if((!is_data_table_ && has_index_or_is_index_) || (has_index_or_is_index_ && is_data_table_))
            {
              ...
              {
                ret = OB_ERROR;
                YYSYS_LOG(ERROR,"column checksum is null");
              }
              else
              {
                ...
                //add:e
                YYSYS_LOG(DEBUG,"test::lmz, range:%s, new cc:[%s]", to_cstring(new_range_), column_checksum_.get_str_const());
                if(OB_SUCCESS != (ret = manager_.send_tablet_column_checksum(column_checksum_,new_range_,frozen_version_)))
                {
                  YYSYS_LOG(ERROR,"lixuaio merge v1 start get ccchecksum");
                }
              }
            }
            //add e
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (sstable_size > 0)
            {
              if (NULL == trailer)
              {
                YYSYS_LOG(ERROR, "after close sstable, trailer=NULL, "
              ...
              {
                YYSYS_LOG(ERROR,"Merge : add sstable to tablet failed.");
              }
              else
              {
                int64_t pos = 0;
        ...
        if (OB_SUCCESS == ret && NULL != tablet)
        {
          ...
          if (is_tablet_unchanged)
          {
            ...
            YYSYS_LOG(INFO, "finish_sstable, create hard link sstable=%s, range=%s, "
    ...
    int ObTabletMergerV1::report_tablets(ObTablet* tablet_list[],int32_t tablet_size)
    {
      ...
      else
      {
        ...
        for(int32_t i=0; i < tablet_size && num > 0 && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to add tablet info, num=%ld, err=%d", num, ret);
          }
          else
          {
            --num;
        ...
        {
          YYSYS_LOG(WARN, "failed to send request report to rootserver err = %d",ret);
        }
      }
      return ret;
    }
    ...
    int ObTabletMergerV1::create_empty_tablet_and_upgrade(ObTablet *&new_tablet, int64_t frozen_version)
    {
      ...
      {
        ret = OB_INNER_STAT_ERROR;
        YYSYS_LOG(ERROR,"inner state error.");
      }
        ...
        if (!old_tablet_->is_merged())
        {
          ...
          if (ret == OB_SUCCESS)
          {
            if(sync_meta)
            {
              ...
              {
                YYSYS_LOG(ERROR, "write new meta failed, version=%ld, disk=%d", frozen_version, new_tablet->get_disk_no());
              }
              // sync old tablet meta files
              else if (OB_SUCCESS != (ret = manager_.get_serving_tablet_image().write(old_tablet_->get_data_version(), old_tablet_->get_disk_no())))
              {
                YYSYS_LOG(ERROR, "write old meta failed, version=%ld, disk=%d", old_tablet_->get_data_version(), old_tablet_->get_disk_no());
              }
            }
          }
        }
    ...
    int ObTabletMergerV1::set_has_index_or_is_index(const uint64_t table_id)
    {
      ...
      */
      ...
      {
        YYSYS_LOG(ERROR,"get merge schema manager error,ret=%d",ret);
        ret=OB_SCHEMA_ERROR;
      }
      else
      {
      ...
      */
      ...
      {
          ret = OB_SCHEMA_ERROR;
          YYSYS_LOG(ERROR, "get schema manager error,table_id:%ld,ret=%d", table_id, ret);
      }
      else if(NULL == (user_schema = schema_manager->get_user_schema(0)))
      {
          YYSYS_LOG(WARN, "get latest but local schema failed:latest[%ld]", schema_manager->get_latest_version());
          ret = OB_INNER_STAT_ERROR;
      }
      ...
      {
          //[571]
          YYSYS_LOG(WARN, "get_table_schema error,table_id:%ld,ret=%d", table_id, ret);
      }

      if(NULL == schemav2 || NULL == user_schema) //mod [571]
      {
        YYSYS_LOG(ERROR,"get schema manager v2 error,table_id:%ld,ret=%d",table_id,ret);
        ret = OB_SCHEMA_ERROR;
      }
      else if (NULL == (ts = schemav2->get_table_schema(table_id)))
      {
        YYSYS_LOG(ERROR,"get_table_schema error,table_id:%ld,ret=%d",table_id,ret);
        ret = OB_SCHEMA_ERROR;
      }
      /* del liumz, [use frozen version's schema manager]20160113
      if(ts == NULL)
      ...
      else
      {
        if(ts->get_index_helper().tbl_tid != OB_INVALID_ID)
        {
          ...
          {
              is_init_index_new_schema_ = is_or_have_init_index_;
          }
          else
          {
              is_init_index_new_schema_ = (INDEX_INIT == table_schema->get_index_helper().status);
          }
          ...
          if(NULL != ts_of_data_table)
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN,"index_column_num is not right = %ld",index_column_num);
            }
          }
          else
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN,"data_table is null! table_id:%ld",ts->get_index_helper().tbl_tid);
          }
        }
        else if(OB_SUCCESS != (ret = schemav2->get_index_list(table_id,index_list)))
        {
          YYSYS_LOG(ERROR,"failed to get index list ret = %d",ret);
        }
        else if(index_list.get_count() > 0)
        {
          //是数据表,判断索引表数量，大于0就是有索引表�?   //
          has_index_or_is_index_ = true;
          is_data_table_ = true;
          ...
          //YYSYS_LOG(INFO,"test:liuxiao this table need cal column checksum table_id = %ld",table_id);
          if(NULL == table_schema)
          {
              is_have_available_index_new_schema_ = is_have_available_index_;
          }
          else
          {
              is_have_available_index_new_schema_ = user_schema->is_have_available_index(table_id);
          }
        }
        else
        {
          ...
          is_data_table_ = false;
          //YYSYS_LOG(INFO,"test:liuxiao this table do not need cal column checksum table_id = %ld",table_id);
        }
      }
      ...
      if(NULL != schemav2 && NULL != merge_schema )
      {
          ...
          {
            ret = err;
            YYSYS_LOG(WARN, "release schema failed, ret = %d", ret);
          }
      }
      //add e
      */
    ...
    int ObTabletMergerV1::stop_invalid_index_tablet_merge(ObTablet *tablet, bool &invalid)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "null pointer of merge tablet");
      }
      if(OB_SUCCESS == ret)
      {
      ...
      if(NULL != schema && OB_INVALID_ID != schema->get_index_helper().tbl_tid && AVALIBALE != schema->get_index_helper().status)
      {
        tablet->set_removed();
        invalid = true;
        YYSYS_LOG(INFO, "unvaliabale index tablet cannot be merged!");
      }
      return ret;
    }
    //add e
    ...
    {
      //该函数只用于处理数据�?      //构造数据表的需要计算列校验和的row desc
      //rowkey_desc保存原表主键
      //index_desc保存除了原表主键的索引表中的�?//
      int ret = OB_SUCCESS;
      int64_t count = 0;
      ...
      */
      ...
      {
        YYSYS_LOG(ERROR,"get merge_schema error");
        ret = OB_SCHEMA_ERROR;
      }
      else if(NULL == (schemav2 = merge_schema->get_schema(tid)))
      {
        YYSYS_LOG(ERROR,"get schema manager v2 error,ret=%d",ret);
        ret = OB_SCHEMA_ERROR;
      }
      else
      {
      ...
      if(schemav2 == NULL)
      {
        YYSYS_LOG(ERROR,"get schema manager v2 error,table_id:%ld,ret=%d",tid,ret);
        ret = OB_SCHEMA_ERROR;
      }
      else if (NULL == (schema = schemav2->get_table_schema(tid)))  //获取表的table schema
      {
        YYSYS_LOG(ERROR,"get table schema error,ret=%d",ret);
        ret = OB_SCHEMA_ERROR;
      }
      //add:e
      /* del liumz, [use frozen version's schema manager]20160113
      ...
      {
        count = sstable_schema_.get_column_count(); //count保存原表列个�?
        ...
        for(int i = 0;i < count;i++)
        {
          ...
          {
            ret =OB_ERROR;
            YYSYS_LOG(WARN, "failed to get column def of table[%ld], i[%d],ret[%d]", tid, i, ret);
            break;
          }
          //遍历原表的所有列，并只保留在索引表中有的�?//
          if(OB_SUCCESS != (ret = schemav2->column_hint_index(tid,col_def->column_name_id_,hint_index)))
          {
            YYSYS_LOG(ERROR,"failed to check if hint index,ret=%d",ret);
            break;
          }
          else if(hint_index)
          {
            ...
            {
              //如果是主键则单独加入到rowkey_desc�? //
              if(OB_SUCCESS != (ret = rowkey_desc.add_column_desc(tid, i)))
              {
                YYSYS_LOG(WARN, "failed to set column rowkey_desc table_id[%ld], ret[%d]", tid, ret);
                break;
              }
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to set column index_desc table_id[%ld], ret[%d]", tid, ret);
                break;
              }
            }
          }
    ...
    //add [401]
    bool ObTabletMergerV1::need_getold_cchecksum()
    {
        bool ret = false;
        if(is_data_table_)
        {
            if(is_have_available_index_)
            {
                ret = true;
            }
            else if(is_have_available_index_new_schema_)
            {
                ret = true;
            }
        }
        ...
        {
            if(!is_or_have_init_index_)
            {
                ret = true;
            }
            else if(!is_init_index_new_schema_)
            {
                ret = true;
            }
        }
    ...
    int ObTabletMergerV1::cons_row_desc_without_virtual(ObRowDesc &rowkey_desc,ObRowDesc &index_desc,uint64_t &max_data_table_cid,uint64_t tid)
    {
      ...
      //rowkey_desc保存原表主键
      //index_desc保存除了原表主键的索引表中的�?
      //由于是obsstaberow不是obrow，无法用列id定位某一个obj，所以使用i表示，i�?开�?
      int ret = OB_SUCCESS;
      ...
      if(NULL == merge_schema)
      {
        YYSYS_LOG(ERROR, "merge_schema is null ");
        ret =OB_ERROR;
      }
      else if(NULL == (schemav2 = merge_schema->get_schema(tid)))
      {
        YYSYS_LOG(ERROR, "schemav2 is null ");
        ret =OB_ERROR;
      }*/
      // add liumz, [use frozen version's schema manager]20160113:b
      const ObSchemaManagerV2* schemav2 = &(chunk_merge_.current_schema_);
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to get index_schema");
          ret = OB_SCHEMA_ERROR;
        }
        else if(OB_INVALID_ID == (data_tid = index_schema->get_index_helper().tbl_tid))
        {
          YYSYS_LOG(ERROR, "failed to get data_table_id");
          ret =OB_ERROR;
        }
        else if(NULL == (data_schema = schemav2->get_table_schema(data_tid)))
        {
          YYSYS_LOG(ERROR, "failed to get data_table_schema");
          ret =OB_ERROR;
        }
        else
        {
          ...
          for(int64_t i = 0; i< column_count; i++)
          {
            if(NULL == (col_def = sstable_schema_.get_column_def((int)i)))
            {
              YYSYS_LOG(WARN, "failed to get column def of table[%ld], i[%ld],ret[%d]", tid, i, ret);
              ret =OB_ERROR;
              break;
            }
    ...
    int ObTabletMergerV1::calc_tablet_col_checksum_index_local_tablet(const sstable::ObSSTableRow& row, ObRowDesc rowkey_desc, ObRowDesc index_desc, char *column_checksum)
    {
      ...
      if(OB_SUCCESS == ret)
      {
        for(i = 0;i < rowkey_count;i++)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "get sstable row obj error,ret=%d", ret);
            break;
          }
          else
          {
        ...
        if(len < 0)
        {
          YYSYS_LOG(ERROR,"write column checksum error");
          ret = OB_ERROR;
        }
        else
        {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        for(i = 0;i < index_count;i++)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "failed to get column def in cal col checksum");
            break;
          }
          else
          {
    ...
    int ObTabletMergerV1::calc_tablet_col_checksum_index_index_tablet(const sstable::ObSSTableRow& row,
                                                                      ObRowDesc rowkey_desc,
                                                                      ObRowDesc index_desc,
                                                                      char *column_checksum,
                                                                      const uint64_t max_data_table_cid)
    {
      //该方法专门用于处理索引表的列校验�?
      int ret = OB_SUCCESS;
      int64_t rowkey_count = rowkey_desc.get_column_num(); //原表主键�?
