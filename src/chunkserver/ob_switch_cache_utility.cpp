  ...
  namespace chunkserver 
  {
                                            ...
                                            ObBlockIndexCache& index_cache)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to destroy block cache");
        }
      }
      else
      {
        YYSYS_LOG(WARN, "failed to destroy index cache");
      }
                                           ...
                                           ObBlockIndexCache &dst_index_cache)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, src_tablet_version=%ld, dst_tablet_version=%ld",
      ...
      while (OB_SUCCESS == status)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get start key of next block");
          }
          break;
        }
      }
      ...
      if (OB_ITER_END != status)
      {
        YYSYS_LOG(WARN, "Problem load new block cache");
        status = OB_ERROR;
      }
      else
      {
