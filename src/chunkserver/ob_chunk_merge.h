...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    class ObChunkMerge : public yysys::CDefaultRunnable
    {
      public:
        ObChunkMerge();
        ~ObChunkMerge()
      ...
      private:
        ...
        volatile bool is_really_merged_;
    ...
    template <typename Merger>
      int ObChunkMerge::create_tablet_mergers(ObTabletMerger** mergers, const int64_t size)
      {
        ...
        {
          YYSYS_LOG(WARN, "allocate memrory for merger object error.");
        ...
        else
        {
          for (int64_t i = 0 ; i < size; ++i)
          {
            ...
            {
              YYSYS_LOG(WARN, "init merger object error, merger=%p, ret=%d", merger, ret);
              ret = OB_ALLOCATE_MEMORY_FAILED;
              break;
            }
            else
