...
{ 
  namespace chunkserver 
  {
    ...
    void ObTablet::destroy()
    {
      ...
      if (0 != ref_count_)
      {
        YYSYS_LOG(ERROR, "try to destroy tablet, but ref count is not 0, "
                         "ref_count_=%ld",
          ref_count_);
      }
    ...
    int ObTablet::add_local_index_sstable_by_id(const ObSSTableId &sstable_id)
    {
      ...
      {
        YYSYS_LOG(ERROR,"there is already [%ld] sstable in tablet!",sstable_id_list_.count());
      }
      else if (OB_SUCCESS != (ret = sstable_id_list_.push_back(sstable_id)))
      {
        YYSYS_LOG(ERROR, "add sstable id :%ld in list error ret=%d",
    ...
    int ObTablet::delete_local_index_sstableid()
    {
      ...
      {
        YYSYS_LOG(DEBUG, "no local index sstable file need to delete");
      }
      else if(OB_SUCCESS != (ret = sstable_id_list_.remove(1)))
      {
        YYSYS_LOG(WARN, "delete local index sstable failed");
      }

      if(this->sstable_reader_list_.count() < 2)
      {
        YYSYS_LOG(DEBUG, "no local index reader need to delete");
      }
      else
      {
    ...
    int ObTablet::load_local_sstable(const int64_t tablet_version)
    {
      ...
      else
      {
        ...
        for (int64_t i = reader_size; i < size ; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "read sstable failed, sstable id=%ld, ret =%d",
                sstable_id_list_.at(i).sstable_file_id_, ret);
            break;
          }
          else if ( OB_SUCCESS != (ret = sstable_reader_list_.push_back(reader)))
          {
            YYSYS_LOG(ERROR, "add sstable reader in list error ret=%d", ret);
            break;
          }
      ...
      else
      {
        ...
        for (int64_t i = 0; i < size && reader_size < size; ++i)
        {
              ...
              && !(sstable_id_list_.at(i) == (sstable_reader_list_.at(i))->get_sstable_id())) 
          {
            ...
            YYSYS_LOG(WARN, "the order in sstable_id_list_ isn't the same as "
    ...
    {
      int64_t row_count = 0;
      const_cast<yysys::CThreadMutex&>(extend_info_mutex_).lock();
      ...
      const_cast<yysys::CThreadMutex&>(extend_info_mutex_).unlock();
      return row_count;
    }
    ...
    int ObTablet::calc_extend_info()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < size && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "calc extend_info error, sstable %ld,%ld reader = NULL",
    ...
    int ObTablet::dump(const bool dump_sstable) const
    {
      ...
      YYSYS_LOG(INFO, "range=%s, data version=%ld, disk_no=%d, merged=%d", 
      ...
      if (dump_sstable && OB_SUCCESS == sstable_loaded_)
      {
        ...
        for (int64_t i = 0; i < size; ++i)
        {
          ...
          if (NULL != reader) 
          {
            YYSYS_LOG(INFO, "sstable [%ld]: id=%ld, "
