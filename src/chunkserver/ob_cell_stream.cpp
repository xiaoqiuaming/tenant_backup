...
//        const int64_t time_out)
    ObCellStream::ObCellStream(ObMergerRpcProxy * rpc,
                               const ObServerType server_type,
                               const int64_t time_out,
                               const int64_t paxos_group_id)
    //mod 20151209:e
    {
      reset_inner_stat();
      //add lijianqiang [MultiUPS] [MERGE] 20151209:b
      paxos_group_id_ = paxos_group_id;
      //add 20151209:e
      rpc_proxy_ = rpc;
    ...
    {
      UNUSED(scan_param);
      YYSYS_LOG(ERROR, "%s", "not implement");
      return OB_ERROR;
    }

    //add lijianqiang [MultiUPS] [MERGE] 20151209:b
    int ObCellStream::scan(const common::ObScanParam & scan_param, ObIterator *it_out[], int64_t& size)
    {
      UNUSED(scan_param);
      UNUSED(it_out);
      UNUSED(size);
    ...
    int ObCellStream::check_scanner_result(const common::ObScanner & result)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get request is fullfilled failed:ret[%d]", ret);
      }
      else
      {
        if ((false == is_fullfill) && result.is_empty())
        {
          YYSYS_LOG(ERROR, "check scanner result empty but not fullfill:fullfill[%d], "
                           "item_count[%ld]",
    ...
    int ObCellStream::get_cell(ObCellInfo** cell)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check input cell or inner stat failed:cell[%p]", cell);
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "iterator get cell failed:ret[%d]", ret);
        }
      }
      return ret;
    }
...

    {
      int ret = OB_SUCCESS;
      int64_t timeout =
        timeout_time_ > 0 ? (timeout_time_ - yysys::CTimeUtil::getTime()) : time_out_;
      ret = rpc_proxy_->cs_cs_scan(param, chunkserver, cur_result_, server_type_, timeout);
      YYSYS_LOG(DEBUG,"test:whx cs_cs_scan = %s,server = %s", to_cstring(*param.get_fake_range()),to_cstring(chunkserver));
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "scan server data failed:ret[%d]", ret);
      }
      else
      {
        if (first_rpc_)
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "check scanner result failed:ret[%d]", ret);
        }
        else
        {
          first_rpc_ = false;
    ...
    /// add e
 //mod shili [MultiUPS] [MERGE] 20150701:b
    //int ObCellStream::rpc_scan_row_data(const ObScanParam & param)
    //paxos_id: ���ڲ��� update_server �ĵ�ַ
    int ObCellStream::rpc_scan_row_data(const ObScanParam & param, const int64_t paxos_id)
    //mod 20150701:e
    {
      int ret = OB_SUCCESS;
      int64_t timeout = 
        timeout_time_ > 0 ? (timeout_time_ - yysys::CTimeUtil::getTime()) : time_out_;
      //mod lijianqiang [MultiUPS] [MERGE] 20160316:b
      //ret = rpc_proxy_->ups_scan(param, cur_result_, server_type_, timeout);
      ret = rpc_proxy_->ups_scan(param, paxos_id, cur_result_, server_type_, timeout);
      //mod 20160316:e
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "scan server data failed:ret[%d]", ret);
    ...
    int ObCellStream::rpc_get_cell_data(const common::ObGetParam & param)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get data through rpc call failed:ret[%d]", ret);
      }
      else
      {
        if (first_rpc_)
        ...
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "check get ups first cell failed:ret[%d]", ret);
          }
        }
      }
      return ret;
