  ...
  namespace chunkserver
  {
        ...
        const ObTablet* tablet, const int64_t frozen_version)
    {
      ...
      if (NULL == (compressor_name = table_schema.get_compress_func_name()) 
          || 0 == *compressor_name)
      {
        YYSYS_LOG(WARN,"no compressor with this sstable. table id = (%lu)", 
      ...
      {
        YYSYS_LOG(WARN, "ret=%d,set_sstable_param error, major_version_=%ld,"
      ...
      {
        YYSYS_LOG(WARN, "set_table_info error, ret=%d, range=%s", ret, to_cstring(tablet->get_range()));
      }
      else
      {
    ...
    int ObTabletMergerV2::prepare_merge(ObTablet *tablet, int64_t frozen_version)
    {
      ...
      {
        YYSYS_LOG(ERROR,"merge : interal error, param invalid tablet[%p], frozen_version:[%ld]",
      ...
      {
        YYSYS_LOG(ERROR, "init_sstable_writer failed, ret=%d, table=%ld",
    ...
    int ObTabletMergerV2::open()
    {
      ...
      {
        YYSYS_LOG(WARN, "build_sql_scan_param: ret[%d]", ret);
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to create plan:ret[%d]", ret);
        }
        else if(OB_SUCCESS != (ret = tablet_scan_.open()))
        {
          YYSYS_LOG(WARN, "open tablet scan fail:ret[%d]", ret);
        }
      }
    ...
    int ObTabletMergerV2::do_merge()
    {
      ...
      {
        YYSYS_LOG(ERROR, "wait aio buffer error, ret= %d", ret);
      }
          ...
          && old_tablet_->get_sstable_id_list().count() > 0)
      {
        ...
        YYSYS_LOG(INFO, "tablet %s has no incremental data, finish.", to_cstring(old_tablet_->get_range()));
      ...
      while (OB_SUCCESS == ret && !is_tablet_unchanged)
      {
        ...
        if (OB_ITER_END == ret)
        {
          ...
          YYSYS_LOG(INFO, "scan row END, finish current sstable split=%d, unchanged=%d,ret=%d", 
        ...
        else if (OB_SUCCESS == ret && NULL != cur_row)
        {
          ...
          {
            YYSYS_LOG(WARN, "append row error, ret=%d, row=%s", ret, to_cstring(*cur_row));
          }
          else if (is_sstable_split)
          {
            YYSYS_LOG(INFO, "split tablet, range=%s, cur_row=%s", 
                to_cstring(old_tablet_->get_range()), to_cstring(*cur_row));
            if (OB_SUCCESS != (ret = finish_sstable(is_sstable_split, is_tablet_unchanged)))
            {
              YYSYS_LOG(WARN, "finish_sstable error ret=%d", ret);
            }
            else if (OB_SUCCESS != (ret = create_new_sstable()))
            {
              YYSYS_LOG(WARN, "create_new_sstable error,ret=%d", ret);
            }
            else
            {
        ...
        {
          YYSYS_LOG(ERROR, "tablet_scan_ get_next_row error, ret=%d, cur_row=%p", ret, cur_row);
        }
      }

      if (OB_SUCCESS !=  tablet_scan_.close())
      {
        YYSYS_LOG(WARN, "tablet_scan_ close error.");
      }
      CLEAR_TRACE_LOG();
    ...
    int ObTabletMergerV2::merge(ObTablet *tablet, int64_t frozen_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "save merge info failed, ret=%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "do_merge error, ret=%d", ret);

      }
      else if (OB_SUCCESS != (ret = update_meta(old_tablet_, tablet_array_, sync_meta)))
      {
        YYSYS_LOG(WARN, "update_meta error, ret=%d", ret);
      }
      ...
      YYSYS_LOG(INFO, "finish merge tablet, ret=%d", ret);
    ...
    int ObTabletMergerV2::cleanup_uncomplete_sstable_files()
    {
      ...
      for(ObVector<ObTablet *>::iterator it = tablet_array_.begin(); it != tablet_array_.end(); ++it)
      {
        if ( ((*it) != NULL) && ((*it)->get_sstable_id_list().count() > 0))
        {
          ...
          {
            unlink(path);
            YYSYS_LOG(WARN,"cleanup sstable %s",path);
    ...
    int ObTabletMergerV2::create_new_sstable()
    {
      ...
      {
        YYSYS_LOG(WARN, "gen_sstable_file_location disk_no=%d failed.", disk_no);
      }
      else 
      {
        ...
        {
          YYSYS_LOG(INFO,"create new sstable, sstable_path:%s ,version=%ld", path_, frozen_version_);
        }
      }
        ...
        const int64_t size, sql::ObProject& project)
    {
      ...
      for (int64_t i = 0; i < size && OB_SUCCESS == ret; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "add_expr_item ret=%d, tid=%ld, cid=%ld", ret, def[i].table_id_, def[i].column_id_);
        }
        else if (OB_SUCCESS != (ret = sql_expression.add_expr_item_end()))
        {
          YYSYS_LOG(WARN, "add_expr_item_end ret=%d, tid=%ld, cid=%ld", ret, def[i].table_id_, def[i].column_id_);
        }
        else if (OB_SUCCESS != (ret = project.add_output_column(sql_expression)))
        {
          YYSYS_LOG(WARN, "add_output_column ret=%d, tid=%ld, cid=%ld", ret, def[i].table_id_, def[i].column_id_);
        }
      }
      return ret;
    }
    ...
    int ObTabletMergerV2::build_project(const compactsstablev2::ObSSTableSchema& schema, sql::ObProject& project)
    {
      ...
      {
        YYSYS_LOG(WARN, "get rowkey schema error, table_id=%ld", table_id);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = build_project(def, rowkey_size, project)))
      {
        YYSYS_LOG(WARN, "build_project rowkey error, table_id=%ld", table_id);
      }
      ...
      {
        YYSYS_LOG(WARN, "build_project rowvalue error, table_id=%ld", table_id);
      }
    ...
    int ObTabletMergerV2::create_hard_link_sstable()
    {
      ...
      if (old_tablet_->get_sstable_id_list().count() > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed create hard link for unchanged sstable, disk=%d "
    ...
    int ObTabletMergerV2::build_new_tablet(const bool is_tablet_unchanged, ObTablet* &tablet)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot create_hard_link_sstable , ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = build_extend_info(is_tablet_unchanged, extend_info)))
      {
        YYSYS_LOG(WARN, "build_extend_info error. is_tablet_unchanged=%d, ret = %d", 
            is_tablet_unchanged, ret);
      }
      else if (OB_SUCCESS != (ret = tablet->add_sstable_by_id(sstable_id_)) )
      {
        YYSYS_LOG(ERROR,"Merge : add sstable to tablet failed. ret=%d", ret);
      }
      else
      {
    ...
    int ObTabletMergerV2::finish_sstable(const bool is_sstable_split, const bool is_tablet_unchanged)
    {
      ...
      if (!is_tablet_unchanged)
      {
        ...
        {
          YYSYS_LOG(ERROR,"Merge : finish sstable failed, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = writer_.get_table_range(new_range, 0)))
        {
          YYSYS_LOG(ERROR, "Merge : cannot get split range .");
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "finish current sstable path=%s, row_count=%ld, "
