  ...
  namespace chunkserver
  {
    ...
    int pipe_row(ObTabletManager& manager, ObTabletReader& reader, ObTabletWriter & writer)
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(INFO, "reach end of reader.");
          ret = OB_SUCCESS;
          break;
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "reader error =%d", ret);
        }
        else if (NULL == row)
        {
          YYSYS_LOG(WARN, "reader error of NULL row.");
          ret = OB_ERR_UNEXPECTED;
        }
        else if (OB_SUCCESS != (ret = writer.write_row(*row)))
        {
          YYSYS_LOG(WARN, "write error =%d row=%s.", ret, to_cstring(*row));
        }
      }
      return ret;
    }
    ...
    int pipe_buffer(ObTabletManager& manager, ObTabletReader& reader, ObTabletWriter & writer)
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "write error =%d buffer=%p,%ld", ret, buffer.get_data(), buffer.get_position());
        }
        else
        {
          if(buffer.get_capacity() > 0 && NULL != buffer.get_data() && band_limit > 0) 
          {
            ...
            sleep_time_us = 1000000L*total_read_size/band_limit - (yysys::CTimeUtil::getTime() - start_time_us);
      ...
      total_cost_time_us = yysys::CTimeUtil::getTime() - start_time_us;
       ...
       */
      YYSYS_LOG(INFO, "duration time of pipe buffer:"
    ...
    int ObTabletFileWriter::open()
    {
      ...
      {
        YYSYS_LOG(WARN, "gen_sstable_file_location disk_no=%d failed.", disk_no_);
      }
      else
      {
        ...
        if (OB_SUCCESS != (ret = appender_.open(path_string, true, true, true)))
        {
            if(OB_IO_ERROR == ret)
            {
                manager_->get_disk_manager().set_disk_status(disk_no_, DISK_ERROR);
            }
          YYSYS_LOG(ERROR, "open file path:%s failed, erro:%s", path, strerror(errno));
        }
        else
        {
          YYSYS_LOG(INFO, "prepare the open file, locate path:%s", path);
        }
      }
    ...
    int ObTabletFileWriter::close()
    {
      ...
      {
        YYSYS_LOG(ERROR, "finish sstable failed ret:%d", ret);
      }
    ...
    int ObTabletFileWriter::finish_sstable()
    {
      ...
      {
        YYSYS_LOG(ERROR,"build_new_tablet failed, range=%s, ret=%d",
            to_cstring(range_), ret);
      }
      else if(OB_SUCCESS != (ret = tablet_image.add_tablet(new_tablet, true, for_create)))
      {
        YYSYS_LOG(INFO, "add tablet failed ret:%d", ret);
      }
      else
      {
        if(sync_meta)
      ...
      if(is_empty_)
      {
        ...
        {
          YYSYS_LOG(WARN, "sstable file:%s is empty, will cleanup", path);
        }
      }
    ...
    int ObTabletFileWriter::write(const common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(ERROR, "append file erro:%s", strerror(errno));
      }
      else
      {
