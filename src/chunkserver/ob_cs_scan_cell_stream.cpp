...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObCSScanCellStream::next_cell(void)
    {
      ...
      if (NULL == scan_param_)
      {
        YYSYS_LOG(DEBUG, "check scan param is null");
        ret = OB_ITER_END;
      }
      ...
      else
      {
        ...
        do
        {
          ...
          if (OB_ITER_END == ret)
          {
            YYSYS_LOG(DEBUG, "scan cell finish");
            break;
          }
          else if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "scan next cell failed:ret[%d]", ret);
            break;
          }
      ...
      else
      {
        ...
        else
        {
          ...
          if (cur_result_.is_empty() && !ObCellStream::first_rpc_)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "check finish scan failed:ret[%d]", ret);
            }
            else if (finish_)
            {
              ret = OB_ITER_END;
              //YYSYS_LOG(ERROR,"test::whx here ret = [%d]",ret);
            }
          }
        }
      }
    ...
    int ObCSScanCellStream::get_next_cell(void)
    {
      ...
      {
        ret = OB_ITER_END;
        YYSYS_LOG(DEBUG, "check next already finish");
      }
      else
      {
        ...
        if (OB_ITER_END == ret)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "get next cell failed:ret[%d]", ret);
            }
            else if (OB_ITER_END == ret)
            {
              YYSYS_LOG(DEBUG, "finish the scan");
            }
          }
          else if (OB_ITER_END != ret)
          {
        ...
        {
          YYSYS_LOG(WARN, "next cell failed:ret[%d]", ret);
        }
      }
      return ret;
    }
    ...
    int ObCSScanCellStream::scan_row_data()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "modify scan param failed:ret[%d]", ret);
        }
      }
...
//        if(ObCellStream::first_rpc_)
//        {
//          YYSYS_LOG(DEBUG, "test::lmz, first_rpc_");
//          //chunkserver_.print_info();//add liumz, debug
          do
          {
            YYSYS_LOG(DEBUG,
                      "test:lmz cur_rep_index_ = %ld,cs_cs_scan = %s,server = %s",
                      cur_rep_index_,
                      to_cstring(*(cur_scan_param_.get_fake_range())),
                      to_cstring((chunkserver_)[cur_rep_index_].server_.chunkserver_));
...
//          if (ret != OB_SUCCESS)
//          {
//            YYSYS_LOG(WARN, "scan row data failed:ret[%d]", ret);
//          }
//        }
      }
      //if(cur_scan_param_.if_need_fake())
      //YYSYS_LOG(INFO, "test::whx cur_scan_param_.real =%s", to_cstring(*cur_scan_param_.get_range()));
    ...
    int ObCSScanCellStream::check_finish_scan(const ObScanParam & param)
    {
      ...
      if (!finish_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get scanner full filled status failed:ret[%d]", ret);
        }
        else if (is_fullfill)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "get result range failed:ret[%d]", ret);
          }
          else
          {
