...
//add:e
//add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150627:b
#include "ob_cs_partition_task.h"
//20150627:e
#include "ob_cs_partition_release_task.h"//add wuna [MultiUps] [part_cache] 20160118
  ...
  namespace chunkserver
  {
    ...
    class ObChunkService
    {
      ...
      public:
        ...
        //add liu jun. [MultiUPS] [part_cache] 20150520:b
        inline int64_t get_frozen_version() const
        {
          //return merge_task_.get_last_frozen_version();
          return frozen_version_sync_from_rs_;
        }

        int64_t get_serving_data_version() const;
        //20150520:e
        // add lqc [Multiups][index partition] 20173017 b
        common::ObMergerSchemaManager *get_schema_manag();
        //add e
      ...
      private:
        ...
        // because of the heartbeat handle will block yynet thread
        ...
        int cs_stop_gather_operation(
                const int32_t version,
                const int32_t channel_id,
                easy_request_t* req,
                common::ObDataBuffer& in_buf,
                common::ObDataBuffer& out_buf);
        int cs_fetch_collection_stat(
                const int32_t version,
                const int32_t channel_id,
                easy_request_t* req,
                common::ObDataBuffer& in_buf,
        ...
        int cs_fetch_merge_stat(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t* req,
            common::ObDataBuffer& in_buffer,
            common::ObDataBuffer& out_buffer);
        int cs_fetch_local_index_stat(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t* req,
            common::ObDataBuffer& in_buffer,
      ...
      private:
        ...
        class MergeTask : public common::ObTimerTask
        {
          public:
            ...
            {}
          public:
            inline int64_t get_last_frozen_version() const
        ...
        class StatisticsTask : public common::ObTimerTask
        {
          ...
          public:
            ...
            void set_finished_tasks_num(int64_t num);
            int64_t get_finished_tasks_num();
            void set_total_tasks_num(int64_t num);
            int64_t get_total_tasks_num();

            common::ObArrayHelper<common::ObGatherTableInfo>* get_gather_list();

            int64_t get_active_thread_num();
        ...
        class IndexTask : public common::ObTimerTask
        {
          ...
          public:
            ...
            int set_schedule_time(uint64_t tid, int64_t start_time, IndexPhase ip);
      ...
      private:
        ...
        int fetch_master_update_server_list();
        //add 20150825:e
      ...
      public:
        ...
        inline common::ObLocalCsMsList &get_local_cs_ms_list()
        {
            return ms_;
        }

        //add liumz, [secondary index version management]20160413:b
        inline void set_build_index_version(int64_t index_version)
        {build_index_version_ = index_version;}
        inline int64_t get_build_index_version()
        {return build_index_version_;}
        //add:e
      private:
        ...
        yysys::CRWLock del_tablet_rwlock_;
		 //add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150608:b
        ObCsPartitionTask part_task_;
        bool part_lock_;
        int64_t frozen_version_sync_from_rs_; //
        ObCsPartitionReleaseTask part_release_task_;//add wuna [MultiUps] [part_cache] 20160118
        //add:e
        //add zhaoqiong [Schema Manager] 20150327:b
        //for get schema mutator from sys table
        //oceanbase::common::ObLocalMsList ms_;
        oceanbase::common::ObLocalCsMsList ms_;
        ...
        //[401]
        int64_t last_build_index_version_;
        bool need_check_schema_version_;
