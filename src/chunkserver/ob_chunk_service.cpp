...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    : chunk_server_(NULL), inited_(false),
      ...
      del_tablet_rwlock_(),part_lock_(false),frozen_version_sync_from_rs_(OB_INVALID_VERSION),
      last_build_index_version_(0),
...
//                                false)))
                                ...
                                //mod:e
                                &chunk_server->get_client_manager(), OB_ALL_CLUSTER_FLAG,
                                false, chunk_server->get_config().cluster_id)))//[582]
      //mod:e
      {
      ...
      {
        YYSYS_LOG(ERROR, "init chunk server config error, ret: [%d]", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR,"common_rpc init error");
        }
        ...
        else
        {
          ...
          ms_.init(&ms_list_task_, chunk_server->get_self());
          ...
          scan_helper_->set_scan_retry_times(3);
    ...
    int ObChunkService::destroy()
    {
      ...
      if (inited_)
      {
        ...
        chunk_server_ = NULL;
        part_lock_ = false; //uncertainty   multiups 中含�?        //add zhaoqiong [Schema Manager] 20150327:b
        delete common_rpc_;
        common_rpc_ = NULL;
        delete scan_helper_;
        scan_helper_ = NULL;
    ...
    int ObChunkService::start()
    {
      ...
      else
      {
        ...
        if (rc != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "init merge join rpc failed.");
        }
        //add zhaoqiong [Schema Manager] 20150327:b
        else if (OB_SUCCESS != (rc = chunk_server_->get_rpc_proxy()->set_schema_service(schema_service_)))
        {
          YYSYS_LOG(ERROR, "rpc_proxy set_schema_service failed");
        }
        //add:e
      }
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (OB_SUCCESS != rc)
        {
          YYSYS_LOG(ERROR, "the compress lib in the list is not exist: rc=[%d]", rc);
        }
      }
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (OB_SUCCESS != rc)
        {
          YYSYS_LOG(ERROR, "load local tablets error, rc=%d", rc);
        }
      }
      if (OB_SUCCESS == rc)
      {
      ...
      if (OB_SUCCESS == rc)
      {
        //mod ptz [TIMER_OVERFLOW] 20170919:b
        rc = timer_.schedule(lease_checker_,
            chunk_server_->get_config().lease_check_interval, true);
        //mod 20170919:e
      }
      ...
      if(OB_SUCCESS == rc)
      {
        rc = part_release_task_.init(this,chunk_server_->get_partition_monitor());
        if(OB_SUCCESS == rc)
        {
          rc = timer_.schedule(part_release_task_,
            chunk_server_->get_config().partition_manager_release_interval, true);
          if(OB_SUCCESS == rc)
          {
            YYSYS_LOG(INFO, "release partition manager schedual succ");
          }
          else
          {
            YYSYS_LOG(ERROR, "release partition manager schedual failed, ret=%d", rc);
          }
        }
      }
      //add 20160118:e
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (rc != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR,"start merge thread failed.");
        }
        ...
        if ((OB_SUCCESS == rc) && (compactsstable_cache_size > 0))
        {
          ...
          {
            YYSYS_LOG(ERROR,"start cache thread failed");
          }
        }
        ...
        if (OB_SUCCESS == rc && bypass_loader_threads > 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "start bypass sstable loader threads failed, ret=%d", rc);
          }
        }
        //add wenghaixing [secondary index static_index_build]20150302
        if(OB_SUCCESS == rc)
        {
          ObTabletManager & tablet_manager = chunk_server_->get_tablet_manager();
          if(OB_SUCCESS == (rc = tablet_manager.start_index_work()))
          {
              YYSYS_LOG(INFO, " succeed to start index work! ret = %d", rc);
              if(OB_SUCCESS != (rc = tablet_manager.start_statistics_collector()))
              {
                  YYSYS_LOG(ERROR, "Fail to start statistics collector! ret = %d", rc);
              }
          }
          else
          {
              YYSYS_LOG(INFO, "Fail to start index work! ret = %d", rc);
          }
        }
        //add e
      }
    ...
    int ObChunkService::check_compress_lib(const char* compress_name_buf)
    {
      ...
      {
        YYSYS_LOG(ERROR, "compress name buf is NULL");
        rc = OB_INVALID_ARGUMENT;
      }
      else if (static_cast<int64_t>(strlen(compress_name_buf)) >= OB_MAX_VARCHAR_LENGTH)
      {
        YYSYS_LOG(ERROR, "compress name length >= OB_MAX_VARCHAR_LENGTH: compress name length=[%d]", static_cast<int>(strlen(compress_name_buf)));
        rc = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        while (NULL != ptr)
        {
          ...
          {
            YYSYS_LOG(ERROR, "create compressor error: ptr=[%s]", ptr);
            rc = OB_ERROR;
            break;
          }
          else
          {
            ...
            YYSYS_LOG(INFO, "create compressor success: ptr=[%s]", ptr);
    ...
    int ObChunkService::load_tablets()
    {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        {
          rc = OB_ERROR;
          YYSYS_LOG(ERROR, "get disk no array failed.");
        }
      }
    ...
    //mod:e
    {
      ...
      while (inited_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "register self to rootserver failed, rc=%d", rc);
          break;
        }*/
        if (OB_SUCCESS == rc)
        {
          YYSYS_LOG(INFO, "register self to rootserver successed, master rs=%s", chunk_server_->get_root_server().to_cstring());
          break;
        }
                 //add liuzy [MultiUPS] [take_cluster_offline_interface] 20160325:b
        /*Exp: when cluster is offline, kill self*/
        else if (OB_CURRENT_CLUSTER_OFFLINE == rc)
        {
          YYSYS_LOG(INFO, "cluster[%d], this cs belongs to, is offline, result_code:[%d]",
                    chunk_server_->get_self().cluster_id_, rc);
          this->kill_self();
                  break;
        }
        //add 20160325:e
        ...
        {
           YYSYS_LOG(WARN, "register self to rootserver failed, rc=%d", rc);
           //add bingo [Paxos ms/cs cluster_id] 20170401:b
           if(OB_CLUSTER_ID_ERROR == rc)
           {
             YYSYS_LOG(WARN, "register self to rootserver failed, stop self, err=%d", rc);
             chunk_server_->stop();
           }
           //add:e
           if (!is_first)
    ...
    int ObChunkService::report_tablets_busy_wait()
    {
      ...
      while (inited_ && (service_started_ || (in_register_process_ && !service_started_)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "report tablets to rootserver failed, rc=%d", rc);
          break;
        }
        usleep(static_cast<useconds_t>(chunk_server_->get_config().network_timeout));
      }
    ...
    int ObChunkService::fetch_schema_busy_wait(ObSchemaManagerV2 *schema)
    {
      ...
      if (NULL == schema)
      {
        YYSYS_LOG(ERROR,"invalid argument,sceham is null");
        rc = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    //mod:e
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(ERROR, "service not initialized, cannot register_self.");
        rc = OB_NOT_INIT;
      }

      if (in_register_process_)
      {
        YYSYS_LOG(ERROR, "another thread is registering.");
        rc = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        service_expired_time_ = yysys::CTimeUtil::getTime() + 10000000;
        int64_t current_data_version = tablet_manager.get_serving_data_version();
        if (0 == status)
        {
          YYSYS_LOG(INFO, "system startup on first time, wait rootserver start new schema,"
        ...
        {
          YYSYS_LOG(INFO, "chunk service start, current data version: %ld", current_data_version);
        ...
        const int64_t timeout_time)
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(ERROR, "service not initialized, cannot accept any message.");
        rc = OB_NOT_INIT;
      }
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        {
          YYSYS_LOG(ERROR, "service not started, only accept "
      ...
      if (OB_SUCCESS == rc)
      {
            ...
            && packet_code != OB_REQUIRE_HEARTBEAT)
        {
          ...
          YYSYS_LOG(WARN, "lease expired, wait timer schedule re-register self to rootserver.");
        }
      }

      if (OB_SUCCESS != rc)
      {
        YYSYS_LOG(ERROR, "call func error packet_code is %d return code is %d",
            packet_code, rc);
      ...
      else
      {
        switch(packet_code)
        {
          ...
          case OB_CS_CREATE_INDEX_SIGNAL:
            ...
            YYSYS_LOG(INFO,"This CS has recieved a signal for index creating");
            break;
          //add e
          ...
          case OB_WHIPPING_WOK:
            ...
            YYSYS_LOG(INFO,"This CS has recieved a wok!");
            break;
          //add e
          ...
          case OB_CS_SHOW_DISK:
            ...
            break;
          //[cs_merge_stat]
          case OB_CS_FETCH_MERGE_STAT:
            rc = cs_fetch_merge_stat(version, channel_id, req, in_buffer, out_buffer);
            break;
          case OB_CS_FETCH_LOCAL_INDEX_STAT:
            rc = cs_fetch_local_index_stat(version, channel_id, req, in_buffer, out_buffer);
            break;
          case OB_START_GATHER_OPERATION:
            rc = cs_start_gather_operation(version, channel_id, req, in_buffer, out_buffer);
            break;
          case OB_STOP_GATHER_OPERATION:
            rc = cs_stop_gather_operation(version, channel_id, req, in_buffer, out_buffer);
            break;
          case OB_FETCH_COLLECTION_STAT:
            rc = cs_fetch_collection_stat(version, channel_id, req, in_buffer, out_buffer);
            break;
          default:
            rc = OB_ERROR;
            YYSYS_LOG(WARN, "not support packet code[%d]", packet_code);
            break;
        }
      }
      return rc;
    }
    // add lqc [Multiups][index partition] 20173017 b
    ObMergerSchemaManager *ObChunkService::get_schema_manag()
    {
      return chunk_server_->get_schema_manager();
    }
    //add e

    //add liu jun. [MultiUPS] [part_cache] 20150528:b
    int64_t ObChunkService::get_serving_data_version() const
    {
      return chunk_server_->get_serving_data_version();
    }

    //20150528:e
    //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160302:b
    const common::ObServer ObChunkService::get_root_server() const
    {
      return chunk_server_->get_root_server();
    }
    //add 20160302:e

    int ObChunkService::get_cchecksum_info(const ObNewRange new_range, const int64_t required_version, const int32_t cluster_id, ObString &cchecksum)
    {
        int ret = OB_SUCCESS;
        ...
        else if(OB_SUCCESS != (ret = schema_service_->init(scan_helper_, false)))
        {
        ...
        {
            YYSYS_LOG(WARN, "failed to get old check sum for cs merge, tablet[%s], required_version[%ld], ret=%d", to_cstring(new_range), required_version, ret);
            if(OB_SUCCESS != (ret = schema_service_->get_checksum_info(new_range, required_version, cchecksum)))
            {
                YYSYS_LOG(ERROR, "failed to get old check sum for cs merge, tablet[%s], ret=%d", to_cstring(new_range), ret);
            }
        }
        return ret;
    }
    ...
    int ObChunkService::get_sql_query_service(ObSqlQueryService *&service)
    {
      ...
      {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(WARN, "get thread specific ObSqlQueryService fail:ret[%d]", ret);
      }
        ...
        const int64_t timeout_time)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread local get_param or scanner, "
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_get input param error.");
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid param, cell_size=%ld",get_param_ptr->get_cell_size());
          rc.result_code_ = OB_INVALID_ARGUMENT;
        }
        else if (NULL == get_param_ptr->get_row_index() || get_param_ptr->get_row_size() <= 0)
        {
          YYSYS_LOG(WARN, "invalid get param, row_index=%p, row_size=%ld",
      ...
      do
      {
        ...
        if (OB_SUCCESS == rc.result_code_ && !is_fullfilled)
        {
          ...
          rc.result_code_ = queue_thread.wait_for_next_request(
            session_id, next_request, timeout_time - yysys::CTimeUtil::getTime());
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(WARN, "failed to wait for next reques timeout, ret=%d",
              rc.result_code_);
            break;
          }
          else
      ...
      int64_t consume_time = yysys::CTimeUtil::getTime() - start_time;
      ...
      {
        YYSYS_LOG(WARN, "slow get: table_id:%lu, "
      ...
      if (get_context != NULL)
      {
        ...
        for (int64_t i=0; i< tablet_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN,"Problem of check frozen data of this tablet"); //just warn
          }
    ...
    int ObChunkService::reset_internal_status(bool release_table /*=true*/)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to end scan to release resources");
      }

      ret = reset_query_thread_local_buffer();
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to reset query thread local buffer");
      }

      ret = wait_aio_buffer();
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to wait aio buffer free");
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "serialize the rc failed, ret:%d", serialize_ret);
        }
        ...
        if (OB_SUCCESS == serialize_ret && NULL != disk_no_array)
        {
          for (int32_t i = 0; i < disk_num; i++)
          {
            ...
            if (OB_SUCCESS == serialize_ret)
            {
                serialize_ret = serialization::encode_i64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(),
                                                          disk_manager.get_disk_avail(disk_no_array[i]));
            }
          }
        }
        ...
        {
          YYSYS_LOG(WARN, "serialize failed, ret:%d", serialize_ret);
        }
      }
    ...
    int ObChunkService::cs_fetch_merge_stat(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t *req,
            common::ObDataBuffer &in_buffer,
            common::ObDataBuffer &out_buffer)
    {
        UNUSED(in_buffer);
        common::ObResultCode rc;
        const int32_t CS_FETCH_MERGE_STAT_VERSION = 1;
        rc.result_code_ = OB_SUCCESS;

        if(version != CS_FETCH_MERGE_STAT_VERSION)
        {
            rc.result_code_ = OB_ERROR_FUNC_VERSION;
        }
        else
        {
            ...
            else if(OB_SUCCESS != (serialize_ret = chunk_server_->serialize_cs_merge_stat(out_buffer.get_data(), out_buffer.get_capacity(),out_buffer.get_position())))
            {
                YYSYS_LOG(WARN, "serialize the cs merge stat failed, ret:%d", serialize_ret);
            }
            else
            {
                chunk_server_->send_response(
                            OB_CS_FETCH_MERGE_STAT_RESPONSE,
                            CS_FETCH_MERGE_STAT_VERSION,
                            out_buffer, req, channel_id);
            }
        }
        return rc.result_code_;
    ...
    int ObChunkService::cs_fetch_local_index_stat(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t *req,
            common::ObDataBuffer &in_buffer,
            common::ObDataBuffer &out_buffer)
    {
        UNUSED(in_buffer);
        common::ObResultCode rc;
        const int32_t CS_FETCH_LOCAL_INDEX_STAT_VERSION = 1;
        rc.result_code_ = OB_SUCCESS;

        if(version != CS_FETCH_LOCAL_INDEX_STAT_VERSION)
        {
            rc.result_code_ = OB_ERROR_FUNC_VERSION;
        }
        else
        {
            int32_t serialize_ret = 0;
            int64_t local_total_tablet_num = chunk_server_->get_tablet_manager().get_index_worker().get_local_total_tablet_task_num();
            int64_t local_merged_tablet_num = chunk_server_->get_tablet_manager().get_index_worker().get_local_mergerd_tablet_task_num();
            int64_t global_total_tablet_num = chunk_server_->get_tablet_manager().get_index_worker().get_global_total_tablet_task_num();
            int64_t global_merged_tablet_num = chunk_server_->get_tablet_manager().get_index_worker().get_global_mergerd_tablet_task_num();
            IndexPhase index_phase = chunk_server_->get_tablet_manager().get_index_worker().get_index_build_phase();

            if(OB_SUCCESS != (serialize_ret = rc.serialize(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position())))
            {
                YYSYS_LOG(WARN, "serialize the rc failed, ret:%d", serialize_ret);
            }
            else if(OB_SUCCESS != (serialize_ret = serialization::encode_vi64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(), local_total_tablet_num)))
            {
                YYSYS_LOG(WARN, "serialize the local total tablet num failed, ret:%d", serialize_ret);
            }
            else if(OB_SUCCESS != (serialize_ret = serialization::encode_vi64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(), local_merged_tablet_num)))
            {
                YYSYS_LOG(WARN, "serialize the local merged tablet num failed, ret:%d", serialize_ret);
            }
            else if(OB_SUCCESS != (serialize_ret = serialization::encode_vi64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(), global_total_tablet_num)))
            {
                YYSYS_LOG(WARN, "serialize the global total tablet num failed, ret:%d", serialize_ret);
            }
            else if(OB_SUCCESS != (serialize_ret = serialization::encode_vi64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(), global_merged_tablet_num)))
            {
                YYSYS_LOG(WARN, "serialize the global merged tablet num failed, ret:%d", serialize_ret);
            }
            else if(OB_SUCCESS != (serialize_ret = serialization::encode_vi64(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position(), index_phase)))
            {
                YYSYS_LOG(WARN, "serialize the index phase failed, ret:%d", serialize_ret);
            }
            else
            {
                chunk_server_->send_response(
                            OB_CS_FETCH_LOCAL_INDEX_STAT_RESPONSE,
                            CS_FETCH_LOCAL_INDEX_STAT_VERSION,
                            out_buffer, req, channel_id);
            }
        }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "decode mount path failed");
      }
      else
      {
      ...
      {
        YYSYS_LOG(WARN, "cs_load_disk rc.serialize error, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "cs_load_disk mount disk_no:%d serialize failed, ret:%d",
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "parse disk maintain param error.");
      }
      else
      {
        ...
        sql::ObSqlReadParam *sql_read_param_ptr)
    {
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_sql_scan input scan param error. ret=%d", rc.result_code_);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (table_id == OB_ALL_SERVER_STAT_TID)
        {
          if (NULL == sql_scan_param_ptr)
          {
            ...
            YYSYS_LOG(WARN, "get method is not supported for all server stat table ");
          }
          else
          {
            is_last_packet = true;
        ...
        else
        {
          if (OB_SUCCESS == rc.result_code_)
          {
            ...
            {
              YYSYS_LOG(WARN, "get sql query service fail:ret[%d]", rc.result_code_);
            }
            else
            {
          ...
          if (OB_SUCCESS == rc.result_code_)
          {
            ...
            //[290]
            if(OB_SUCCESS != rc.result_code_ && OB_UPS_TABLE_NOT_FROZEN != rc.result_code_)
            {
                YYSYS_LOG(WARN, "open query service fail:err[%d]", rc.result_code_);
            }
            if(OB_UPS_TABLE_NOT_FROZEN == rc.result_code_)
            {
                YYSYS_LOG(WARN, "acquire ups table maybe for daily merge:err[%d],need master ups acquire table", rc.result_code_);
                //clode failed plan, and rebuild plan
                if(NULL != sql_query_service)
                {
                    sql_query_service->close();
                    sql_query_service->reset();
                    sql_query_service->set_timeout_us(timeout_time);
                    sql_read_param_ptr->set_is_read_consistency(true);
                    rc.result_code_ = OB_SUCCESS;
                    rc.result_code_ = sql_query_service->open(*sql_read_param_ptr);
                    if(OB_SUCCESS != rc.result_code_)
                    {
                      YYSYS_LOG(WARN, "open master ups query service fail:err[%d]", rc.result_code_);
                    }
                }
            }
          ...
          if(OB_SUCCESS == rc.result_code_)
          {
            ...
            if (OB_SUCCESS != rc.result_code_)
            {
              YYSYS_LOG(WARN, "failed to fill_scan_data:err[%d]", rc.result_code_);
            }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        {
          YYSYS_LOG(WARN,"check update data failed"); //just warn
        }
      }
      ...
      if (consume_time >= chunk_server_->get_config().slow_query_warn_time)
      {
        if (is_scan)
        {
          ...
          YYSYS_LOG(WARN, "slow sql scan: table_id:%lu, scan_range=%s, data version=%ld, read mode=%d, peer=%s, rc=%d, scanner size=%ld, io_stat:%s consume=%ld",
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "slow sql get: table_id:%lu, row size=%ld, peer=%s, rc=%d, scanner size=%ld, io_stat:%s consume=%ld",
        ...
        const int64_t timeout_time)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread local scan_param or scanner, "
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_scan input scan param error.");
        }
        else
        {
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse drop_old_tablets input memtable_frozen_version param error.");
        }
      }

      YYSYS_LOG(INFO, "drop_old_tablets: memtable_frozen_version:%ld", memtable_frozen_version);

      int serialize_ret = rc.serialize(out_buffer.get_data(),
          out_buffer.get_capacity(), out_buffer.get_position());
      if (serialize_ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "rc.serialize error");
      }
    ...
    int ObChunkService::cs_recieve_wok(const int32_t version, const int32_t channel_id, easy_request_t *req, ObDataBuffer &in_buffer, ObDataBuffer &out_buffer)
    {
     ...
     // UNUSED(req);
      ...
      {
        rc.result_code_ = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "chunk server pointer can not be null");
      }
      if(OB_SUCCESS == rc.result_code_)
      {
        if(OB_SUCCESS != (rc.result_code_ = list.deserialize(in_buffer.get_data(),in_buffer.get_capacity(),in_buffer.get_position())))
        {
          YYSYS_LOG(WARN, "desirialize black list failed, ret = %d", rc.result_code_);
        }
        else
        {
          ObTabletManager& tablet_manager = chunk_server_->get_tablet_manager();
...
//add wenghaixing [secondary index static_index_build] 20150318
    int ObChunkService::cs_static_index(const int32_t version, const int32_t channel_id, easy_request_t *req, ObDataBuffer &in_buffer, ObDataBuffer &out_buffer)
    {
      ...
      */
      ...
      {
        YYSYS_LOG(ERROR,"seriliazation error,cannot decode index_tid to build index!ret[%d]",rc.result_code_);
      }
      ...
      {
        YYSYS_LOG(ERROR,"seriliazation error,cannot decode hist_width to build index!ret[%d]",rc.result_code_);
      }
      else if(!index_task_.is_scheduled())
      {
        index_task_.set_schedule_idx_tid(index_id);
        index_task_.set_hist_width(hist_width);
        YYSYS_LOG(INFO,"index_task: set_schedule_tid[%ld],hist_width[%ld]",index_id,hist_width);
        if(OB_SUCCESS != (rc.result_code_ = (timer_.schedule(index_task_, wait_time, false)))) //async
        {
          YYSYS_LOG(WARN, "cannot schedule index_task_ after wait %ld us", wait_time);
        }
        else
        {
          YYSYS_LOG(INFO, "launch a new index process after wait %ld us", wait_time);
    ...
    int ObChunkService::cs_start_gather_operation(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t *req,
            ObDataBuffer &in_buf,
            ObDataBuffer &out_buf)
    {
        ...
        {
            YYSYS_LOG(WARN, "deserialize count error, err=%d", ret);
        }
        else
        {
            for(int64_t i = 0; i < count; i=i+3)
            {
                uint64_t tmp_tid;
                uint64_t tmp_cid;
                ...
                {
                    YYSYS_LOG(WARN, "deserialize tmp_tid error, err=%d", ret);
                }
                else if(OB_SUCCESS != (ret = serialization::decode_i64(in_buf.get_data(), in_buf.get_capacity(), in_buf.get_position(), (int64_t*)&tmp_cid)))
                {
                    YYSYS_LOG(WARN, "deserialize tmp_cid error, err=%d", ret);
                }
                else if(OB_SUCCESS != (ret = serialization::decode_i64(in_buf.get_data(), in_buf.get_capacity(), in_buf.get_position(), (int64_t*)&replication_idx)))
                {
                    YYSYS_LOG(WARN, "deserialize replication_idx error, err=%d", ret);
                }
                else
                {
                    ...
                    gather_info.replication_idx_ = replication_idx;
                    YYSYS_LOG(INFO, "the push tid is %ld, cid is %ld", tmp_tid, tmp_cid);
                    if(OB_SUCCESS != (ret = statistics_task_.set_schedule_info(gather_info)))
                    {
                        YYSYS_LOG(WARN, "failed to push gather info into list! err=%d", ret);
                    }
                }
            }
            YYSYS_LOG(DEBUG, "The size of all tasks is %ld",statistics_task_.get_schedule_task_size());
            statistics_task_.set_total_tasks_num(statistics_task_.get_schedule_task_size());
        }
        result_msg.result_code_ = ret;
        if(!statistics_task_.get_collection_tag())
        {
            statistics_task_.set_collection_tag(true);
            statistics_task_.set_finished_tasks_num(0);
            if(statistics_task_.get_schedule_task_size() > 0)
            {
                if(OB_SUCCESS != (ret = timer_.schedule(statistics_task_, wait_time, false)))
                {
                    YYSYS_LOG(WARN, "can't schedule statistics_task_ after wait %ld us", wait_time);
                }
            }
            else
            {
                if(statistics_task_.get_schedule_task_size() == 0)
                {
                    YYSYS_LOG(WARN, "there is no gather info!");
                }
            }
        }
        else
        {
            YYSYS_LOG(WARN, "Chunkserver is gathering statistics! Abort this operation");
        }
        return ret;
    }

    int ObChunkService::cs_stop_gather_operation(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t *req,
            ObDataBuffer &in_buf,
            ObDataBuffer &out_buf)
    {
        ...
        if(statistics_task_.get_gather_list() == NULL)
        {
            YYSYS_LOG(WARN, "gather list is NULL!");
            ret = OB_ERROR;
        }
        else
        {
            statistics_task_.set_collection_tag(false);
            statistics_task_.get_gather_list()->clear();
        }
        return ret;
    }

    int ObChunkService::cs_fetch_collection_stat(
            const int32_t version,
            const int32_t channel_id,
            easy_request_t *req,
            ObDataBuffer &in_buf,
            ObDataBuffer &out_buf)
    {
        ...
        if(!statistics_task_.get_collection_tag() && statistics_task_.get_active_thread_num() == 0)
        {
            YYSYS_LOG(INFO, "gather state is DONE!");
        }
        else
        {
            result_msg.result_code_ = OB_STATISTICS_COLLECTION_RUNNING;
            YYSYS_LOG(INFO, "gather state is RUNNING!");
        }


        float progress;
        float total_tasks_num = static_cast<float>(statistics_task_.get_total_tasks_num());
        float finished_tasks_num = static_cast<float>(statistics_task_.get_finished_tasks_num());
        if(total_tasks_num == 0)//cluster is just started, no information could be gather
        {
            progress = 100;
        }
        else
        {
            progress = finished_tasks_num*100 / total_tasks_num;
        }

        if(OB_SUCCESS != (ret = result_msg.serialize(out_buf.get_data(),
                                                     out_buf.get_capacity(),out_buf.get_position())))
        {
            YYSYS_LOG(ERROR, "result_msg serialize error!");
        }
        else if(OB_SUCCESS != (ret = serialization::encode_float(   //dduuhhtt
                              out_buf.get_data(),out_buf.get_capacity(),
                              out_buf.get_position(), progress)))
        {
            YYSYS_LOG(ERROR, "progress serialize error!");
        }
        else if(OB_SUCCESS != (ret = chunk_server_->send_response(
                                   OB_FETCH_COLLECTION_STAT_REPONSE,DEFAULT_VERSION,out_buf,req,channel_id))) //dduuhhtt REPONSE-->RESPONSE?
        {
            YYSYS_LOG(WARN,"failed to send response to rs! ret = %d",ret);
        }
        return ret;
    }
    ...
    int ObChunkService::gen_index_table_list(int64_t &count, uint64_t *list)
    {
        ...
        {
            ret=OB_SCHEMA_ERROR;
            YYSYS_LOG(ERROR,"get user schema error!");
        }
        else
        {
            ...
            for (;iter != hash->end(); ++iter)
            {
                ...
                for(int64_t i=0;i<il.get_count();i++)
                {
                     ...
                     {
                        ret=OB_SCHEMA_ERROR;
                        YYSYS_LOG(ERROR,"failed to get table schema");
                     }
    ...
    int ObChunkService::gen_static_index(int64_t count, uint64_t *list)
    {
        ...
        for(int64_t i=0;i<count;i++)
        {
           ...
           // UNUSED(tid);
            ...
            {
                YYSYS_LOG(ERROR,"get table schema failed,tid[%ld]!",tid);
                ret=OB_SCHEMA_ERROR;
            }
            else
            {
                ...
                for(int64_t j=OB_APP_MIN_COLUMN_ID;j<=max_column_id;j++)
                {
                    ...
                    else
                    {
                        ...
                        //YYSYS_LOG(ERROR,"test::whx column id[%ld]",col_schema->get_id());
                    }
                }
                ...
                if(OB_SUCCESS==ret)
                {
                 ...
                 {
                     YYSYS_LOG(ERROR,"failed to build sstable scan param");
                 }
                 else if(OB_SUCCESS!=(ret=ob_sstable_scan.open_scan_context(sstable_scan_param,sc)))
                 {
                     YYSYS_LOG(ERROR,"failed to open scan context");
                 }
                 else if(OB_SUCCESS!=(ret=ob_sstable_scan.open()))
                 {
                     YYSYS_LOG(ERROR,"failed to open ObSsTableScan!");
                 }
                 else
                 {
                     while(ret==OB_SUCCESS)
                     {
                        ...
                        // if(ret==OB_SUCCESS)YYSYS_LOG(INFO,"test::whx get_next_row from index,%s",str);
                     }
                     if(ret==OB_ITER_END)
                     {
                         ret=OB_SUCCESS;
    ...
    int ObChunkService::handle_index_beat(IndexBeat beat)
    {
      ...
      if(OB_INVALID_ID == beat.idx_tid || ERROR == beat.status || 0 == beat.hist_width)
      {
        ...
        //TODO, CS锟节斤拷锟斤拷锟揭伙拷锟斤拷锟斤拷锟绞眔ffline, 然锟斤拷锟斤拷online, need release resources
        //TODO, 锟斤拷锟斤拷RS锟斤拷锟斤拷锟斤拷锟?
        //TODO, RS重启不能stop_mission,最后一张索引建�?需要stop_mission
      ...
      else if(OB_INVALID_ID != beat.idx_tid && INDEX_INIT == beat.status)
      {
        ...
        YYSYS_LOG(DEBUG, "test::whx index index_task_ schedule tid[%ld],boolean[%d]", index_task_.get_schedule_idx_tid(),index_task_.is_scheduled());
        if(!index_task_.is_scheduled()&& index_task_.get_round_end())
        {
          if(OB_SUCCESS == index_task_.set_schedule_idx_tid(beat.idx_tid)) //idx_tid
          {
            ...
            YYSYS_LOG(INFO,"index_task: set_schedule_tid[%ld],hist_width[%ld]",beat.idx_tid,beat.hist_width);
        ...
        {
          index_task_.try_stop_mission(beat.idx_tid);
          //YYSYS_LOG(ERROR, "test::whx try stop mission");
        }
      }
      if(OB_UNLIKELY(NULL == chunk_server_))
      {
        YYSYS_LOG(ERROR,"shuold not be here, null chunk ");
      }
      else
      {
    ...
    */
    ...
    int ObChunkService::handle_index_beat_v2(IndexBeat beat, int64_t frozen_version, int64_t build_index_version, bool is_last_finished)
    {
      ...
      if(OB_INVALID_VERSION != get_build_index_version()
              && chunk_server_->get_tablet_manager().get_index_worker().if_current_round_finished())
      {
          set_build_index_version(OB_INVALID_VERSION);
      }

      YYSYS_LOG(DEBUG,
                "beat.idx_tid[%lu], beat.status[%d], beat.hist_width[%ld], beat.start_time[%ld], beat.index_phase_[%d].",
                beat.idx_tid,
                beat.status,
                beat.hist_width,
                beat.start_time,
                beat.index_phase_);
      YYSYS_LOG(DEBUG, "is_merge_done[%d], frozen_version[%ld], build_index_version[%ld], building_version[%ld], data_version[%ld]",
          ...
          )
      {
            ...
            && OB_INVALID_VERSION != get_build_index_version())
        {
          ...
          if(!index_task_.get_round_end())
          {
            ...
            //index_task_.try_stop_mission(beat.idx_tid);
            YYSYS_LOG(DEBUG, "test::liumz force stop mission");
                     ...
                     || OB_INVALID_VERSION == get_build_index_version()))
        {
          ...
          {
            //NORMAL STATUS, DO NOTHING AT THIS TIME
            //TODO, CS?????????offline, ???online, need release resources
            //TODO, ??RS????
            //TODO, RS????stop_mission,????????,??stop_mission
          ...
          else if(OB_INVALID_ID != beat.idx_tid && INDEX_INIT == beat.status)
          {
            ...
            index_task_.set_schedule_time(beat.idx_tid, beat.start_time, beat.index_phase_);//set schedule time every time if need
            YYSYS_LOG(DEBUG,
                      "test::whx index index_task_ schedule tid[%ld],is_scheduled[%d],round_end[%d]",
                      index_task_.get_schedule_idx_tid(),
                      index_task_.is_scheduled(),
                      index_task_.get_round_end());
            if(!index_task_.is_scheduled()&& index_task_.get_round_end())
            {
              if(OB_SUCCESS == index_task_.set_schedule_idx_tid(beat.idx_tid))//idx_tid�?,????????
              {
                index_task_.set_schedule_time(beat.idx_tid, beat.start_time, beat.index_phase_);//set schedule time firstly
                index_task_.set_hist_width(beat.hist_width);
                YYSYS_LOG(INFO,"index_task: set_schedule_tid[%ld],hist_width[%ld]",beat.idx_tid,beat.hist_width);
                //add liumz, [clean old sstable firstly]20160624:b
                if(OB_SUCCESS != cs_delete_global_index(beat.idx_tid))
                {
                  YYSYS_LOG(WARN,"cs delete global index failed, idx_tid[%ld]",beat.idx_tid);
                }
                //add:e
                if(OB_SUCCESS != (ret = (timer_.schedule(index_task_, wait_time, false)))) //async
                {
            ...
            {
              YYSYS_LOG(DEBUG, "test::whx try stop mission");
              index_task_.try_stop_mission(beat.idx_tid);
            }
          }
        }
        else if(OB_INVALID_VERSION == build_index_version
                && chunk_server_->get_tablet_manager().get_chunk_merge().is_merge_done()
                && OB_INVALID_VERSION != get_build_index_version()
                && frozen_version == get_build_index_version())
        {
            if(OB_INVALID_ID == beat.idx_tid && ERROR == beat.status && 0 == beat.hist_width && beat.start_time == 0 && beat.index_phase_ == INIT_PHASE)
            {
                YYSYS_LOG(WARN, "index build timeout, try to stop mission!");
                if(is_last_finished && !index_task_.get_round_end())
                {
                    set_build_index_version(OB_INVALID_VERSION);
                }
        ...
        common::ObDataBuffer& out_buffer)
    {
        ...
        {
          YYSYS_LOG(WARN, "version not equal. version=%d, CS_FORCE_REGIST_VERSION=%d", version, CS_FORCE_REGIST_VERSION);
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "cs set new master rootserver[%s] after election", rs_master.to_cstring());

          ///make cs lease timeout
          service_expired_time_ = yysys::CTimeUtil::getTime();
          YYSYS_LOG(INFO, "reset lease, then will re-regist, lease=%ld", service_expired_time_);
          ...
          if (OB_SUCCESS == ret)
          {
            if (servers_count > 0) //count cannot be 0
            {
                ...
                if(OB_SUCCESS == ret)
                {
                  ...
                  {
                    YYSYS_LOG(WARN, "failed update all rootservers. err=%d", ret);
                  }
                }
                //mod:e
            }
            else
            {
                ...
                YYSYS_LOG(ERROR, "rs server count is invalid, count=%d, err=%d", servers_count, ret);
            }
            //mod:e
          }
        }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      UNUSED(req);
      UNUSED(out_buffer);
      //add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150605:b
      int64_t frozen_version = 0;
      //add:e

      //add [partition cache optimization]
      /*****
       this variable is used to handle the special case such as partition_lock_flag
       is true and last_mem_frozen_version_ has been promoted in current heartbeat
       package. Because we set end version for partition cache node according to
       frozen_version_sync_from_rs_ + 3, so we must check frozen_version_sync_from_rs_
       has been changed. if changed, we use frozen_version_sync_from_rs_+ 2 to set
       end version for partition cache node
      */
      bool frozen_version_sync_from_rs_has_changed = false;
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(WARN, "failed to async_heartbeat, ret=%d", rc.result_code_);
        }
      }
      ...
      if (OB_SUCCESS != rc.result_code_)
      {
        YYSYS_LOG(ERROR, "parse cs_heart_beat input lease_duration param error.");
      }
      else
      {
        service_expired_time_ = yysys::CTimeUtil::getTime() + lease_duration;
        YYSYS_LOG(DEBUG, "cs_heart_beat: lease_duration=%ld", lease_duration);
      }

      YYSYS_LOG(DEBUG,"cs_heart_beat,version:%d,CS_HEART_BEAT_VERSION:%d",version,CS_HEART_BEAT_VERSION);

      if (version >= CS_HEART_BEAT_VERSION)
      {
        //int64_t frozen_version = 0;mod liu jun.[MultiUPS] [PARTITION_LOCK_FUCTION] 20150609  //uncertainty  frozen_version作用域变�?
        if (OB_SUCCESS == rc.result_code_)
        {
          rc.result_code_ = common::serialization::decode_vi64(
              in_buffer.get_data(),in_buffer.get_capacity(),
          ...
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(ERROR, "parse cs_heart_beat input frozen_version param error.");
          }
          else
          {
            YYSYS_LOG(DEBUG, "cs_heart_beat: frozen_version=%ld", frozen_version);
//#####################################################################################
            if (frozen_version > frozen_version_sync_from_rs_)
            {
              frozen_version_sync_from_rs_ = frozen_version;
              frozen_version_sync_from_rs_has_changed = true;
            }
            else
            {
              /*
                * we think that frozen_version <= frozen_version_sync_from_rs_ is ok.
                * so ignore this case!
                */
            }
//######################################################################################
        ...
        if (OB_SUCCESS == rc.result_code_)
        {
          ...
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(ERROR, "parse cs_heart_beat input build_index_version param error.");
          }
          else
          {
            YYSYS_LOG(DEBUG, "cs_heart_beat: build_index_version=%ld", build_index_version);
          }
        }
        ...
        if (OB_SUCCESS == rc.result_code_)
        {
          ...
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(ERROR, "parse cs_heart_beat input is_last_finished param error.");
          }
          else
          {
            YYSYS_LOG(DEBUG, "cs_heart_beat: is_last_finished=%d", is_last_finished);
          }
        }
        if(OB_SUCCESS == rc.result_code_)
        {
          ...
          if (OB_SUCCESS != (rc.result_code_))
          {
            YYSYS_LOG(ERROR, "parse heartbeat index beat failed: ret[%d]",
                      rc.result_code_);
          }
          else
          {
        ...
        if(version > CS_HEART_BEAT_VERSION)
        {
            ...
            if(OB_SUCCESS == rc.result_code_)
            {
                ...
                if(OB_SUCCESS != rc.result_code_)
                {
                    YYSYS_LOG(ERROR, "parse heartbeat schema version failed:ret[%d]",
                              rc.result_code_);
                }
            }
            ...
            if((OB_SUCCESS == rc.result_code_) && service_started_)
            {
                ...
                if((local_version > schema_version) && (schema_version != 0))
                {
                    ...
                    YYSYS_LOG(ERROR, "check schema local version gt than new version:"
                ...
                if(need_check_schema_version_)
                {
                    if((local_version < schema_version) && (schema_version != 0))
                    {
                        YYSYS_LOG(DEBUG, "wait for local schema sync successed after building index or reboot");
                    }
                    else if(local_version == schema_version)
                    {
                        need_check_schema_version_ = false;
                        YYSYS_LOG(INFO, "local schema sync successed after building index or reboot");
                    }
                }
            }
        }
            ...
            && chunk_server_->get_config_mgr().get_current_version() > 1)
        {
          ...
          if ( frozen_version > tablet_manager.get_serving_data_version()
              && tablet_manager.get_serving_data_version() > 0   //uncerttianty   ups  中的心跳不加索引
              ...
              && OB_INVALID_VERSION == last_build_index_version_ && !need_check_schema_version_)
          {
            if (frozen_version > merge_task_.get_last_frozen_version())
            {
              YYSYS_LOG(INFO,"pending a new frozen version need merge:%ld,last:%ld",
                ...
                && tablet_manager.get_chunk_merge().can_launch_next_round(frozen_version))
            {
              ...
              {
                YYSYS_LOG(WARN, "cannot schedule merge_task_ after wait %ld us.", wait_time);
              }
              else
              {
                YYSYS_LOG(INFO, "launch a new merge process after wait %ld us.", wait_time);
        ...
        //[401]
        if(OB_INVALID_VERSION != last_build_index_version_ && OB_INVALID_VERSION == build_index_version)
        {
            need_check_schema_version_ = true;
        }
        last_build_index_version_ = build_index_version;
      }
...
//      }
      ...
      //mod peiouya [MultiUPS] [PARTITION_LOCK_FUCTION] 20150729
      //BUG FIX:after create user, the new user cann't connect db
      ////add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150605:b
      //if(OB_SUCCESS == rc.result_code_ && frozen_version > 1)
      //{
      //  bool partition_lock_flag = false;
      ...
      //  if(OB_SUCCESS != rc.result_code_)
      //  {
      //    YYSYS_LOG(ERROR, "parse heartbeat partition lock flag failed:ret[%d]",
      //              rc.result_code_);
      //  }
      //  else if(true == partition_lock_flag && !part_lock_)
      //  {
      //    part_lock_ = true;
      //    YYSYS_LOG(INFO, "partition tables has locked");
      //    part_task_.init(this, &(chunk_server_->get_config_mgr()), chunk_server_->get_partition_monitor());
      //    srand(static_cast<int32_t>(yysys::CTimeUtil::getTime()));
      //    rc.result_code_ = timer_.schedule(part_task_, random() % FETCH_SCHEMA_INTERVAL, false);
      //    if(OB_SUCCESS != rc.result_code_)
      //    {
      //      YYSYS_LOG(ERROR, "schedule cs partition manager failed");
      //    }
      //  }
      ///  else if(false == partition_lock_flag)
      //    part_lock_ = false;
      //}
      ////add:e
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        rc.result_code_ = serialization::decode_bool(in_buffer.get_data(),
                                                     ...
                                                     in_buffer.get_position(),
                                                     &partition_lock_flag);
        if(OB_SUCCESS == rc.result_code_ && frozen_version > 0)
        {
          if(OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(ERROR, "parse heartbeat partition lock flag failed:ret[%d]",
          ...
          else if(partition_lock_flag && !part_lock_)
          {
            part_lock_ = true;
            YYSYS_LOG(INFO, "partition tables has locked");
            part_task_.init(this, &(chunk_server_->get_config_mgr()), chunk_server_->get_partition_monitor(), frozen_version_sync_from_rs_has_changed);
            srand(static_cast<int32_t>(yysys::CTimeUtil::getTime()));
            rc.result_code_ = timer_.schedule(part_task_, random() % FETCH_SCHEMA_INTERVAL, false);
            if(OB_SUCCESS != rc.result_code_)
            {
      ...
      if (version > CS_HEART_BEAT_VERSION)
      {
        ...
        if (OB_SUCCESS == rc.result_code_)
        {
          ...
          if (OB_SUCCESS != (rc.result_code_))
          {
            YYSYS_LOG(ERROR, "parse heartbeat config version failed: ret[%d]",
                      rc.result_code_);
          }
          else if (OB_SUCCESS !=
                   (rc.result_code_ = config_mgr.got_version(config_version)))
          {
            YYSYS_LOG(WARN, "Process config failed, ret: [%d]", rc.result_code_);
          }
        }
      }
      /*
      ...
      */
      ...
      if(OB_SUCCESS == rc.result_code_)
      {
        ...
        else
        {
          ...
          //YYSYS_LOG(ERROR, "test::whx index beat tid[%ld]", beat.idx_tid);
        }
      }
      //add e
      */
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      if (OB_SUCCESS == err)
      {
        if (NULL == (schema = OB_NEW(ObSchemaManagerV2, ObModIds::OB_CS_SERVICE_FUNC)))
        {
          ...
          YYSYS_LOG(WARN, "fail to new ObSchemaManagerV2");
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(WARN, "not init:chunk_server_[%p], schema_manager[%p]", chunk_server_, schema_manager);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to deserialize schema:err[%d]", err);
        }
        else
        {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "schema version too old. old=%ld, latest=%ld",
        ...
        {
          YYSYS_LOG(INFO, "schema is not completion");
        }
        //add:e
        else if (OB_SUCCESS != (err = schema_manager->add_schema(*schema)))
        {
          YYSYS_LOG(WARN, "fail to add schema :err[%d]", err);
        }
      }
      //del zhaoqiong [Schema Manager] 20150327:b
      //OB_DELETE(ObSchemaManagerV2, ObModIds::OB_CS_SERVICE_FUNC, schema); //uncertainty multi_ups 中保留着�?      //del:e

      ret = rc.serialize(out_buffer.get_data(),
          out_buffer.get_capacity(), out_buffer.get_position());
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "fail to serialize: ret[%d]", ret);
      }
      ...
      if (OB_SUCCESS == ret && OB_SUCCESS == err && !schema->is_completion())
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to add schema :err[%d]", ret);
          }
        }
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "can not release schema, ret[%d]", err);
          }
          ...
          {
            YYSYS_LOG(INFO, "add schema success(mutator version[%ld->%ld])",
                     schema_mutator->get_start_version(), schema_mutator->get_end_version());
          }
        }
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
          YYSYS_LOG(WARN, "load bypass sstable is running, cannot create tablet");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_create_tablet input range param error.");
        }
      }

      YYSYS_LOG(INFO, "cs_create_tablet, dump input range %s", to_cstring(range));
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        YYSYS_LOG(DEBUG, "create tablet, last_frozen_memtable_version=%ld",
      ...
      if (serialize_ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "cs_create_tablet rc.serialize error");
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "merge running, cannot migrate in.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (!chunk_server_->get_tablet_manager().get_bypass_sstable_loader().is_loader_stoped())
      {
        YYSYS_LOG(WARN, "load bypass sstable is running, cannot migrate in");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_load_tablet range param error.");
        }
        else
        {
          char range_buf[OB_RANGE_STR_BUFSIZ];
          range.to_string(range_buf, OB_RANGE_STR_BUFSIZ);
          YYSYS_LOG(DEBUG,"cs_load_tablet dump range <%s>", range_buf);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse dest_disk_no range param error.");
        }
        else
        {
          YYSYS_LOG(INFO, "cs_load_tablet dest_disk_no=%d ", dest_disk_no);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_load_tablet tablet_version param error.");
        }
        else
        {
          YYSYS_LOG(INFO,"cs_load_tablet tablet_version = %ld ", tablet_version);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_load_tablet crc_sum param error.");
        }
        else
        {
          YYSYS_LOG(INFO,"cs_load_tablet crc_sum = %lu ", crc_sum);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_load_tablet number of sstable  param error.");
        }
        else
        {
          YYSYS_LOG(INFO,"cs_load_tablet num_file = %ld ", num_file);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_ && num_file > 0)
      {
        ...
        if ( NULL == path_buf )
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for path array.");
          rc.result_code_ = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
        ...
        if (OB_SUCCESS == rc.result_code_)
        {
          for( int64_t idx =0; idx < num_file; idx++)
          {
            ...
            {
              rc.result_code_ = OB_ERROR;
              YYSYS_LOG(ERROR, "parse cs_load_tablet dest_path param error.");
              break;
            }
            else
            {
              YYSYS_LOG(INFO, "parse cs_load_tablet dest_path [%ld] = %s", idx, path[idx]);
            }
          }
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_ && version  > 1)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_load_tablet tablet_seq_num param error.");
        }
        else
        {
          YYSYS_LOG(INFO,"cs_load_tablet tablet_seq_num = %ld ", tablet_seq_num);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        {
          YYSYS_LOG(WARN, "ObTabletManager::dest_load_tablet error, rc=%d", rc.result_code_);
        }
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "merge running, cannot remove tablets.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (NULL == (delete_tablet_list = GET_TSI_MULT(ObTabletReportInfoList, TSI_CS_TABLET_REPORT_INFO_LIST_1)))
      {
        YYSYS_LOG(ERROR, "cannot get ObTabletReportInfoList object.");
        rc.result_code_ = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_delete_tablets tablet info list param error.");
        }
      }
      ...
      if (version > CS_DELETE_TABLETS_VERSION)
      {
        ...
        {
          YYSYS_LOG(WARN, "load bypass sstable is running, cannot delete tablets");
          rc.result_code_ = OB_CS_EAGAIN;
        }
        if (OB_SUCCESS == rc.result_code_)
        {
          ...
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(ERROR, "parse cs_delete_tablets input is_force param error.");
          }
          else
          {
            YYSYS_LOG(DEBUG, "cs_heart_beat: is_force=%d", is_force);
          }
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        for (int64_t i = 0; i < size ;  ++i)
        {
              ...
              && tablet_info_array[i].tablet_info_.range_.compare_with_endkey2(src_tablet->get_range()) == 0)
          {
            ...
            {
              YYSYS_LOG(INFO, "delete tablet, version=%ld, disk=%d, range=<%s>",
                  src_tablet->get_data_version(), src_tablet->get_disk_no(), range_buf);
          ...
          {
            YYSYS_LOG(INFO, "cannot find tablet, version=%ld, range=<%s>, local range=<%s>",
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "merge running, cannot merge tablets.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (!chunk_server_->get_tablet_manager().get_bypass_sstable_loader().is_loader_stoped())
      {
        YYSYS_LOG(WARN, "load bypass sstables is running, cannot merge tablets.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_merge_tablets tablet info list param error.");
        }
      }
      ...
      if (serialize_ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "merge_tablets rc.serialize error");
      }
      ...
      if (OB_SUCCESS != rc.result_code_)
      {
        YYSYS_LOG(WARN, "report merge tablets over error, is_merge_succ=%d, seq_num=%ld",
            is_merge_succ, is_merge_succ ? merge_tablet_list->get_tablet()[0].tablet_location_.tablet_seq_ : -1);
      }
      else if (is_merge_succ)
      {
        YYSYS_LOG(INFO, "merge tablets <%s> over and success, seq_num=%ld",
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "merge or load bypass sstables running, cannot migrate in.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse cs_get_migrate_dest_loc occupy_size param error.");
        }
        else
        {
          YYSYS_LOG(INFO, "cs_get_migrate_dest_loc occupy_size =%ld", occupy_size);
        }
      }
      ...
      if (disk_no <= 0)
      {
        YYSYS_LOG(ERROR, "get wrong disk no =%d", disk_no);
        rc.result_code_ = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == rc.result_code_ && OB_SUCCESS == serialize_ret)
      {
        ...
        if (OB_SUCCESS != serialize_ret)
        {
          YYSYS_LOG(ERROR, "serialize disk_no failed.");
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_ && OB_SUCCESS == serialize_ret)
      {
        ...
        if (OB_SUCCESS != serialize_ret)
        {
          YYSYS_LOG(ERROR, "serialize dest_directory failed.");
        }
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "parse cs_dump_tablet_image index param error.");
      }
      else if (OB_SUCCESS != ( rc.result_code_ =
            serialization::decode_vi32(in_buffer.get_data(),
            in_buffer.get_capacity(), in_buffer.get_position(), &disk_no)))
      {
        YYSYS_LOG(WARN, "parse cs_dump_tablet_image disk_no param error.");
      }
      else if (disk_no <= 0)
      {
        YYSYS_LOG(WARN, "cs_dump_tablet_image input param error, "
            "disk_no=%d", disk_no);
        rc.result_code_ = OB_INVALID_ARGUMENT;
      }
      ...
      {
        rc.result_code_ = OB_ALLOCATE_MEMORY_FAILED;
        YYSYS_LOG(ERROR, "allocate memory for serialization failed.");
      }
      else if (OB_SUCCESS != (rc.result_code_ =
            chunk_server_->get_tablet_manager().get_serving_tablet_image().
            serialize(index, disk_no, dump_buf, dump_size, pos)))
      {
        YYSYS_LOG(WARN, "serialize tablet image failed. disk_no=%d", disk_no);
      }
      ...
      if (OB_SUCCESS == rc.result_code_ && OB_SUCCESS == serialize_ret)
      {
        ...
        if (OB_SUCCESS != serialize_ret)
        {
          YYSYS_LOG(ERROR, "serialize return_dump_obj failed.");
        }
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "deserialize table id error. pos=%ld, cap=%ld",
      ...
      common::ObDataBuffer& out_buffer)
    {
      ...
      */
      ...
      if (restart != 0)
      {
        ...
        YYSYS_LOG(INFO, "receive *restart server* packet from: [%ld]", peer_id);
      }
      else
      {
        YYSYS_LOG(INFO, "receive *stop server* packet from: [%ld]", peer_id);
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      YYSYS_LOG(INFO, "cs receive force_report_tablet to rs. maybe have some network trouble");
      if (MY_VERSION != version)
      {
        ret = OB_ERROR_FUNC_VERSION;
        YYSYS_LOG(WARN, "force to report tablet verion not equal. my_version=%d, receive_version=%d", MY_VERSION, version);
      }
      ObTabletManager & tablet_manager = chunk_server_->get_tablet_manager();
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "someone else is reporting. give up this process");
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to report tablets. err = %d", ret);
          }
          else
          {
            tablet_manager.report_capacity_info();
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      YYSYS_LOG(INFO, "chunkserver start sync all tablet images");
      ...
      {
        YYSYS_LOG(WARN, "can't sync tablet images now, please try again later");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      YYSYS_LOG(INFO, "finish sync all tablet images, ret=%d", rc.result_code_);
        ...
        const int64_t timeout_time)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread local migrate_reader, "
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(ERROR, "parse data source param error.");
        }
      }

      if (OB_SUCCESS == rc.result_code_)
      {
        YYSYS_LOG(INFO, "begin fetch_reader, %s", to_cstring(desc));
      ...
      {
        YYSYS_LOG(ERROR, "serialize sstable version object failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize sequence_num failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize row_checksum failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize sstable_type failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize rc code failed, ret:%d", serialize_ret);
      }
      ...
      do
      {
            ...
            !is_last_packet)
        {
          ...
          if (OB_SUCCESS != rc.result_code_)
          {
            YYSYS_LOG(WARN, "failed to prepare_for_next_request:err[%d]", rc.result_code_);
          }
        }
      ...
      {
        YYSYS_LOG(WARN, "tablet reader close fail, ret:%d", ret);
      }
      ...
      if(OB_SUCCESS != rc.result_code_)
      {
        YYSYS_LOG(WARN, "cs_tablet_read finish, rc:%d", rc.result_code_);
      }
      else
      {
        YYSYS_LOG(INFO, "cs_tablet_read finish, rc:%d", rc.result_code_);
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      yysys::CRLockGuard guard(del_tablet_rwlock_);
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread local tablet_writer, "
      ...
      if (serialize_ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "fetch_data rc.serialize error");
      }
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        YYSYS_LOG(INFO, "begin migrate data %s", to_cstring(desc));
      }

      if (!chunk_server_->get_config().merge_migrate_concurrency
          && (!chunk_server_->get_tablet_manager().get_chunk_merge().is_merge_stoped()))
      {
        YYSYS_LOG(WARN, "merge running, cannot migrate.");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (!chunk_server_->get_tablet_manager().get_bypass_sstable_loader().is_loader_stoped())
      {
        YYSYS_LOG(WARN, "load bypass sstable is running, cannot migrate");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      ...
      if (OB_SUCCESS == rc.result_code_ )
      {
        ...
        {
          YYSYS_LOG(WARN, "current migrate task count = %u, exceeded max = %ld",
              old_migrate_task_count, max_migrate_task_count);
          rc.result_code_ = OB_CS_EAGAIN;
        }
        else
        {
          YYSYS_LOG(INFO, "current migrate task count = %u", migrate_task_count_);
        }
      }
      ...
      if (OB_SUCCESS == rc.result_code_ )
      {
        ...
        {
          YYSYS_LOG(WARN, "daily merge is pending in upgrade");
        ...
        {
          YYSYS_LOG(WARN, "tablet range:%s data_versin:%ld check failed, ret:%d",
        ...
        {
          YYSYS_LOG(WARN, "prepare remote reader failed ret:%d", rc.result_code_);
        }
        else if(local_sstable_version != remote_sstable_version)
        {
          rc.result_code_ = OB_SSTABLE_VERSION_UNEQUAL;
          YYSYS_LOG(ERROR, "sstable version not equal local_sstable_version:%d remote_sstable_version:%d",
              local_sstable_version, remote_sstable_version);
        }
        else if(OB_SUCCESS != (rc.result_code_ = remote_reader.open()))
        {
          YYSYS_LOG(WARN, "remote reader open failed, ret:%d", rc.result_code_);
        }
        else if(OB_SUCCESS != (rc.result_code_ =
              tablet_writer->prepare(desc.range_, tablet_version, sequence_num, row_checksum, &tablet_manager)))
        {
          YYSYS_LOG(WARN, "remote writer prepare failed, ret:%d", rc.result_code_);
        }
        else if(OB_SUCCESS != (rc.result_code_ = tablet_writer->open()))
        {
          YYSYS_LOG(WARN, "remote writer open failed, ret:%d", rc.result_code_);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "remote reader close failed, ret:%d", ret);
          }
          ...
          if(OB_SUCCESS == rc.result_code_)
          {
            ...
            {
              YYSYS_LOG(WARN, "tablet writer close failed, ret:%d", rc.result_code_);
            }
          }
        ...
        if(OB_SUCCESS == rc.result_code_ && old_tablet_version > 0)
        {
          ...
          {
            YYSYS_LOG(WARN, "not found old tablet:%s old_tablet_version:%ld, maybe deleted",
        ...
        {
          YYSYS_LOG(WARN, "tablet writer cleanup failed, ret:%d", ret);
        }
      ...
      if(OB_SUCCESS == rc.result_code_)
      {
        ...
        {
          YYSYS_LOG(WARN, "add tablet succ, but is deleted, range:%s tablet_version:%ld",
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "report migrate over error, rc.code=%d", ret);
        if(OB_SUCCESS == rc.result_code_)
        {
          rc.result_code_ = ret;
        }
      ...
      {
        YYSYS_LOG(INFO,"report migrate tablet <%s> over to rootserver success, ret=%d",
          ...
          ObDataSourceDesc::OCEANBASE_INTERNAL == desc.type_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(WARN, "send delete tablet <%s> to cs:%s error, rc.code=%d",
              to_cstring(desc.range_), to_cstring(desc.src_server_), rc.result_code_);
        }
        else
        {
          YYSYS_LOG(INFO, "send delete tablet <%s> to cs:%s succ, rc.code=%d",
              to_cstring(desc.range_), to_cstring(desc.src_server_), rc.result_code_);
        }
      }
      ...
      if(OB_SUCCESS != rc.result_code_)
      {
        YYSYS_LOG(WARN, "fetch_data failed rc.code=%d",rc.result_code_);
      }
      else
      {
        YYSYS_LOG(INFO, "fetch_data finish rc.code=%d",rc.result_code_);
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to allocate memory for import table list info");
        rc.result_code_ = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (!tablet_manager.get_chunk_merge().is_merge_stoped())
      {
        YYSYS_LOG(WARN, "merge running, cannot load bypass sstables");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (!tablet_manager.get_bypass_sstable_loader().is_loader_stoped())
      {
        YYSYS_LOG(WARN, "load bypass sstable is running, cannot load bypass "
        ...
        in_buffer.get_position())))
      {
        YYSYS_LOG(WARN, "deserialize import table list info error, "
                        "load_version=%ld, err=%d",
          table_list->tablet_version_, rc.result_code_);
      }
      else if (serving_version < 0 || table_list->tablet_version_ < 1)
      {
        YYSYS_LOG(WARN, "invalid serving_version=%ld, load_version=%ld",
          serving_version, table_list->tablet_version_);
        rc.result_code_ = OB_ERROR;
      }
      else if (0 != serving_version && table_list->tablet_version_ != serving_version)
      {
        YYSYS_LOG(WARN, "load version is different from local serving verion, "
                        "load_verion=%ld, serving_version=%ld",
          table_list->tablet_version_, serving_version);
        rc.result_code_ = OB_ERROR;
      }

      YYSYS_LOG(INFO, "received load bypass sstable command, ret=%d", rc.result_code_);
      //response to root server first
      int serialize_ret = rc.serialize(out_buffer.get_data(),
          out_buffer.get_capacity(), out_buffer.get_position());
      if (serialize_ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "cs_load_bypass_sstables rc.serialize error");
      }
          ...
          && table_list->response_rootserver_)
      {
        ...
        if (OB_SUCCESS != rc.result_code_)
        {
          YYSYS_LOG(WARN, "report load bypass sstables over error, is_start_load_succ=%d",
              is_start_load_succ);
        }
      }
      YYSYS_LOG(INFO, "start load bypass sstables, load_version=%ld, "
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      yysys::CWLockGuard guard(del_tablet_rwlock_, is_block);
      ObTabletManager& tablet_manager = chunk_server_->get_tablet_manager();
      ...
      {
        rc.result_code_ = OB_CS_EAGAIN;
        YYSYS_LOG(WARN, "migrating tablet, cannot do delete now");
      }
      else if (!tablet_manager.get_chunk_merge().is_merge_stoped())
      {
        YYSYS_LOG(WARN, "merge running, cannot delete table");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (!tablet_manager.get_bypass_sstable_loader().is_loader_stoped())
      {
        YYSYS_LOG(WARN, "load bypass sstable is running, cannot delete table");
        rc.result_code_ = OB_CS_EAGAIN;
      }
      else if (in_register_process_ || tablet_manager.get_chunk_merge().is_pending_in_upgrade()
            || tablet_manager.get_bypass_sstable_loader().is_pending_upgrade())
      {
        YYSYS_LOG(WARN, "other thread is scanning tablet images, "
      ...
      {
        YYSYS_LOG(WARN, "deserialize table_id error, err=%d", rc.result_code_);
      }

      YYSYS_LOG(INFO, "received delete table command, table_id=%lu, ret=%d",
      ...
      if (OB_SUCCESS != rc.result_code_)
      {
        YYSYS_LOG(WARN, "report delete table over error, is_delete_succ=%d",
            is_delete_succ);
      }

      YYSYS_LOG(INFO, "load delete table over, table_id=%lu, "
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      if (in_register_process_ || tablet_manager.get_chunk_merge().is_pending_in_upgrade()
          ...
          || 0 != atomic_compare_exchange(&scan_tablet_image_count_, 1, 0))
      {
        YYSYS_LOG(WARN, "someone else is scanning tablets. give up this process");
        ret = OB_EAGAIN;
      }
      else
      {
        ...
        {
          ret = OB_ERROR_FUNC_VERSION;
          YYSYS_LOG(WARN, "server version of cs_fetch_sstable_dist is %d, "
        ...
        {
          YYSYS_LOG(WARN, "failed to decode table_version, cap=%ld pos=%ld ret=%d",
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to ensure space, buf size=%ld ret=%d", BUF_SIZE, ret);
          }
          else
          {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "serving version[%ld] of this cs is not same as request[%ld]",
                tablet_image.get_serving_version(), table_version);
            ret = OB_INVALID_DATA;
          }
          else if (OB_SUCCESS != (ret = tablet_image.begin_scan_tablets()))
          {
            YYSYS_LOG(WARN, "failed to begin scan tablets, ret=%d", ret);
          }
        }
      }
      ...
      do
      {
        if (OB_SUCCESS == ret)
        {
          ...
          else if (OB_SUCCESS == ret && NULL != tablet)
          {
            ...
            {
              YYSYS_LOG(INFO, "skip internal table %lu", range.table_id_);
              continue;
            }
            ...
            {
              ret = OB_INVALID_DATA;
              YYSYS_LOG(WARN, "tablet data version[%ld]  is not same as request[%ld]",
            ...
            else
            {
              ...
              {
                ret = OB_INVALID_DATA;
                YYSYS_LOG(WARN, "one tablet should only has one sstable, recent: %ld range: %s",
              ...
              else
              {
                ...
                if (OB_SUCCESS != ret)
                {
                  YYSYS_LOG(WARN, "failed to get sstable path with sstable id %ld range %s, ret=%d",
            ...
            {
              YYSYS_LOG(WARN, "failed to release tablet, tablet=%p, ret=%d", tablet, ret);
            }
          }
          else
          {
            YYSYS_LOG(WARN, "failed to get next tablet info, ret=%d", ret);
          }
        }
            ...
            || BUF_SIZE - buf_pos < range.get_serialize_size() + sstable_path.get_serialize_size())
        {
          ...
          if (OB_SUCCESS != (rpc_ret= rc.serialize(out_buffer.get_data(),
                  out_buffer.get_capacity(), out_buffer.get_position())))
          {
            YYSYS_LOG(WARN, "failed to serialize rc, cap=%ld pos=%ld ret=%d",
          ...
          if (OB_SUCCESS == ret || OB_ITER_END == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to encode sstable count, cap=%ld pos=%ld ret=%d",
                   out_buffer.get_capacity(), out_buffer.get_position(), rpc_ret);
              break;
            }
            else if (out_buffer.get_remain() < buf_pos)
            {
              YYSYS_LOG(WARN, "failed to write tablet info, buf remain %ld, buf_pos %ld",
          ...
          if (OB_SUCCESS != rpc_ret)
          {
            YYSYS_LOG(WARN, "failed to prepare next request: session_id=%ld, ret=%d", session_id, rpc_ret);
            break;
          }
          ...
          {
            YYSYS_LOG(WARN, "failed to send response, response_cid=%d, session_id=%ld, ret=%d",
          ...
          {
            rpc_ret = OB_SUCCESS;
            YYSYS_LOG(INFO, "client server stop fetching sstable dist");
            break;
          }
          else if (OB_SUCCESS != rpc_ret)
          {
            YYSYS_LOG(WARN, "failed to wait for next request, session_id=%ld timeout=%ld ret=%d",
        ...
        {
          YYSYS_LOG(WARN, "failed to serialize range: %s, buf size=%ld buf pos=%ld ret=%d",
              to_cstring(range), BUF_SIZE, buf_pos, ret);
        }
        else if (OB_SUCCESS != (ret = sstable_path.serialize(buf, BUF_SIZE, buf_pos)))
        {
          YYSYS_LOG(WARN, "failed to serialize path: %s, buf size=%ld buf pos=%ld ret=%d",
      ...
      if (is_increased_scan_tablet_image_count)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to end scan tablets, ret=%d", tmp_ret);
          if (OB_SUCCESS == ret)
          {
            ret = tmp_ret;
          }
      ...
      YYSYS_LOG(INFO, "complete to handle fetch_sstable_dist, ret=%d", ret);
      ...
      const int64_t timeout_time)
    {
      ...
      if (OB_SUCCESS != (ret = range.deserialize(allocator,
              ...
              in_buffer.get_position())))
      {
        YYSYS_LOG(WARN, "deserialize range error, cap:%ld, pos:%ld",
            in_buffer.get_capacity(), in_buffer.get_position());
      }
      ...
      {
        YYSYS_LOG(WARN, "deserialize tablet_version error, cap:%ld, pos:%ld",
            in_buffer.get_capacity(), in_buffer.get_position());
      }
      else if (OB_SUCCESS != (ret = serialization::decode_vi64(
              in_buffer.get_data(), in_buffer.get_capacity(),
      ...
      {
        YYSYS_LOG(WARN, "acquire_tablet ret=%d, range:%s, version:%ld, tablet=%p, tv=%ld",
      ...
      if (NULL != tablet)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "cache bloom filter memory:%ld too large, destroy current.",
              ob_get_mod_memory_usage(ObModIds::OB_BLOOM_FILTER));
        ...
        {
          YYSYS_LOG(WARN, "release tablet %p error.", tablet);
        }
      }
      ...
      do
      {
        ...
        if (OB_SUCCESS == serialize_ret && OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != (serialize_ret = response_buffer.serialize(
                  ...
                  out_buffer.get_position())))
          {
            YYSYS_LOG(WARN, "serialize response buffer error ret=%d,"
          ...
          {
            YYSYS_LOG(WARN, "serialize fullfilled error ret=%d,"
          ...
          {
            YYSYS_LOG(WARN, "serialize total size error ret=%d,"
        ...
        if (OB_SUCCESS != serialize_ret)
        {
          YYSYS_LOG(WARN, "serialize response failed. ret=%d", serialize_ret);
          break ;
        }
        else
        {
          YYSYS_LOG(TRACE, "SEND response::input range=%s, data=%ld, bf=%ld, "
    ...
    bool ObChunkService::have_frozen_table(const ObTablet& tablet,const int64_t ups_data_version) const
    {
      ...
      //杩欓噷鏈変袱绉嶆儏鍐碉紝??绉嶆槸cs杩樻病鏈塩ompactsstable,姝ゆ椂ups涓嶅彲鑳借繕鏈塩s_major鐗堟湰鐨勬暟鎹�?      //??瑕佸垽鏂璾ps鐨刴inor澶т簬OB_UPS_START_MINOR,姣斿cs鐨勭増鏈负1涓旀病鏈塩ompactsstable,閭ｄ箞ups鐗堟�??
      //2,2鐨勬椂鍊欐墠琛ㄦ槑鏈夋柊鐨刴inor freeze
      //鍙﹀�??绉嶆儏鍐垫槸cs宸茬粡鏈変簡??閮ㄤ唤compactsstable,cs_major鍜寀ps_major鐩哥瓑锛屽鏋渕inor鐨勭�??
      //鐩稿樊澶т�?鍒欐湁鏂扮殑鍐荤粨琛紝姣斿cs鐨勭増鏈负(2??1)锛寀ps鐗堟�??(2,3)銆傚鏋渦ps_major澶т簬cs_major??
      //涓攃s_major鐗堟湰涓嬬殑??鏈夋暟鎹凡缁忔媺鍙栧畬??(is_final_minor涓簍rue),璇存槑鏈夋柊鐨勫ぇ鐗堟湰浜х敓
      //鐩墠闄愬埗cs??澶氬彧鑳芥嫢鏈変竴涓ぇ鐗堟湰鐨勬暟�???

      YYSYS_LOG(DEBUG,"cs_version:%ld:%ld,ups_version:%ld:%ld,is_final_minor:%s",
      ...
      else
      {
        ...
        else if (ups_major > cs_major)
        {
          if (is_final_minor)
          {
            YYSYS_LOG(DEBUG,"there is some new frozen table,but we can hold only one major version");
          }
          else
          {
            ret = true;
    ...
    int ObChunkService::check_update_data(ObTablet& tablet,int64_t ups_data_version,bool& release_tablet)
    {
      ...
      if (compactsstable_cache_size > usage_size)
      {
            ...
            tablet.compare_and_set_compactsstable_loading())
        {
          ...
          {
            YYSYS_LOG(WARN,"put %p to cache thread failed,ret=%d,compactsstable num:%d",
          ...
          else
          {
            ...
            YYSYS_LOG(INFO,"push %p to cache thread,ups_data_version:%ld",&tablet,ups_data_version);
          }
        }
      }
      return ret;
    ...
    void ObChunkService::LeaseChecker::runTimerTask()
    {
      if (NULL != service_ && service_->inited_)
      {
        ...
        {
          YYSYS_LOG(INFO, "lease expired, re-register to root_server");
    ...
    void ObChunkService::DiskChecker::runTimerTask()
    {
      ...
      {
        YYSYS_LOG(ERROR, "disk:%d failed, will automatically unload", bad_disk);
        int i = 0;
        ...
        while (i++ < THE_CHUNK_SERVER.get_config().retry_times)
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "unload disk:%d, retry times:%d ret:%d",
                bad_disk, i, ret);
    ...
    void ObChunkService::StatUpdater::runTimerTask()
    {
      ...
      if (NULL == stat)
      {
        YYSYS_LOG(DEBUG,"get stat failed");
      }
      else
      {
    ...
    void ObChunkService::MergeTask::runTimerTask()
    {
      ...
      {
        YYSYS_LOG(ERROR,"mem frozen version (%ld) < newest local tablet version (%ld),exit",frozen_version_,
    ...
    int ObChunkService::fetch_update_server_list()
    {
      ...
      {
        YYSYS_LOG(ERROR, "rpc_proxy_ is NULL");
      }
      else
      {
        int32_t count = 0;
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fetch update server list failed:ret[%d]", err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "fetch update server list succ:count[%d]", count);
        }
      }
    ...
    int ObChunkService::fetch_master_update_server_list()
    {
      int err = OB_SUCCESS;
      ...
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(WARN, "chunk server is not init,ret=%d",err);
      }
      //add 20160316:e
      else
      {
        ...
        {
          int32_t count = 0;
          err = chunk_server_->get_rpc_proxy()->fetch_master_update_server_list(count);
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "fetch master update server list failed:ret[%d]", err);
          }
          else
          {
            YYSYS_LOG(DEBUG, "fetch master update server list succ:count[%d]", count);
          }
        }
      }
      return err;
    ...
    void ObChunkService::StatisticsTask::runTimerTask()
    {
        ...
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        unset_scheduled();
        YYSYS_LOG(INFO, "Start round to build index!");
        if(tablet_manager.get_statistics_collector().can_launch_next_round())
        {
            YYSYS_LOG(INFO, "Launch next round!");
            err = tablet_manager.build_statistics();
            if(OB_SUCCESS != err)
            {
                YYSYS_LOG(INFO, "Wait for next round!");
            }
        }
    }
    ...
    void ObChunkService::StatisticsTask::set_finished_tasks_num(int64_t num)
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        tablet_manager.get_statistics_collector().set_finished_tasks_num(num);
    }
    int64_t ObChunkService::StatisticsTask::get_finished_tasks_num()
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        return tablet_manager.get_statistics_collector().get_finished_tasks_num();
    }

    void ObChunkService::StatisticsTask::set_total_tasks_num(int64_t num)
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        tablet_manager.get_statistics_collector().set_total_tasks_num(num);
    }
    int64_t ObChunkService::StatisticsTask::get_total_tasks_num()
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        return tablet_manager.get_statistics_collector().get_total_tasks_num();
    }

    common::ObArrayHelper<common::ObGatherTableInfo>* ObChunkService::StatisticsTask::get_gather_list()
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        return tablet_manager.get_statistics_collector().get_gather_list();
    }

    int64_t ObChunkService::StatisticsTask::get_active_thread_num()
    {
        ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
        return tablet_manager.get_statistics_collector().get_active_threads_num();
    }
    ...
    void ObChunkService::IndexTask::runTimerTask()
    {
      ...
      ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
      unset_scheduled();
      YYSYS_LOG(INFO,"I want to start round to build index!");
      if(tablet_manager.get_index_worker().can_launch_next_round())
      {
        YYSYS_LOG(INFO,"I can launch next round!");
    ...
    int ObChunkService::IndexTask::set_schedule_time(uint64_t tid, int64_t start_time, IndexPhase ip)
    {
      ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
      return tablet_manager.get_index_worker().set_schedule_time(tid, start_time, ip);
    }
    ...
    {
      //todo ObIndexWorker鐨勫畾鏃惰嚜??浠诲�?      //鐩殑鏄负浜嗛槻姝㈡湁閬楁紡鐨則ablet/range娌℃湁澶勭悊
      ObTabletManager& tablet_manager = service_->chunk_server_->get_tablet_manager();
    ...
    void ObChunkService::FetchUpsTask::runTimerTask()
    {
      ...
      //add dyr [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150825:b
      service_->fetch_master_update_server_list();
      //add dyr 20150825:b
      // reschedule fetch updateserver list task with new interval.
      //YYSYS_LOG(INFO, "ObChunkService::FetchUpsTask::runTimerTask");
      service_->timer_.schedule(*this,
    ...
    void ObChunkService::ReportTabletTask::runTimerTask()
    {
      ...
      else if (tablet_manager.get_serving_data_version() > report_version_)
      {
        ...
        YYSYS_LOG(INFO, "service_version(%ld) upgrade, no need report current (%ld)",
            tablet_manager.get_serving_data_version(), report_version_);
      }
      else
      {
        YYSYS_LOG(INFO, "schedule retry report tablets, report version:%ld, wait_seconds_=%ld",
        ...
        if (OB_SUCCESS != ret)
        {
          if (OB_RESPONSE_TIME_OUT == ret)
          {
            ...
            YYSYS_LOG(WARN, "report tablets timeout, wait %ld seconds and rereport.", wait_seconds_);
          }
        }
        else
        {
