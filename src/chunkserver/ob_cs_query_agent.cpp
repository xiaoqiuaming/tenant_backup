...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObCSQueryAgent::start_agent(ObScanParam &scan_param, ObCSScanCellStream &cs_stream, const RangeServerHash *hash)
    {
      ...
      if(NULL == hash)
      {
        YYSYS_LOG(WARN, "null pointer of range server hash");
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        if(OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "fake range:[%s]", to_cstring(*(scan_param_->get_fake_range())));
          if(OB_SUCCESS != (ret = pfinal_result_->scan(*scan_param_)) && OB_ITER_END != ret)
          {
            ...
            //add e
            YYSYS_LOG(WARN, "failed to scan firstly batch data,[%d]", ret);
          }
          else if(OB_SUCCESS == ret || OB_ITER_END ==ret)
          {
            //YYSYS_LOG(ERROR,"test::whx ret start_agent first scan,hash_index_ [%d],fake_range(%s),range(%s) ret= [%d]", hash_index_,to_cstring(*(scan_param_->get_fake_range())),to_cstring(*(scan_param_->get_range())), ret);
            ret = OB_SUCCESS;
          }
        }
      }
    ...
    int ObCSQueryAgent::get_failed_fake_range(ObNewRange &range)
    {
      ...
      /*if(NULL == scan_param_)
      {
        YYSYS_LOG(WARN, "scan param is invalid");
        ret = OB_INVALID_ARGUMENT;
      }
      else if(!scan_param_->if_need_fake() || NULL == scan_param_->get_fake_range())
      {
        YYSYS_LOG(WARN, "scan_param is invalid");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      {
        YYSYS_LOG(WARN, "failed fake range is invalid");
        ret = OB_INNER_STAT_ERROR;
      }
      else
      {
      ...
      if(!not_used_)
      {
        do
        {
          ...
          if(OB_SUCCESS == ret)
          {
            ...
            {
             // YYSYS_LOG(INFO,"test::whx::row set cell ok, tid[%ld], cid[%ld],value[%s]", (cell)->table_id_, (cell)->column_id_,to_cstring((cell)->value_));
            }
            else
            {
              YYSYS_LOG(WARN,"row set cell failed, tid[%ld], cid[%ld]", (cell)->table_id_, (cell)->column_id_);
              break;
            }
          }
          else
        ...
        {
          YYSYS_LOG(WARN, "get_next_row failed[%d]", ret);
        }
      }
      else
      {
    ...
    int ObCSQueryAgent::get_cell(ObCellInfo **cell)
    {
      ...
      {
        YYSYS_LOG(WARN, "null pointer of ObCSScanCellStream");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObCSQueryAgent::next_cell()
    {
      ...
      else if(OB_ITER_END == ret)
      {
        do
        {
          if(hash_index_ < range_server_hash_->size())
          {
            ...
            ObTabletLocationList list;
            bool iter_end = true;
            //YYSYS_LOG(ERROR,"test::whx OB_ITER_END hash_index_ [%d],size[%ld]", hash_index_,range_server_hash_->size());
            for(;iter != range_server_hash_->end();iter++)
            {
              ...
              {
                loop ++;
                //YYSYS_LOG(ERROR,"test::whx show loop hash_index_[%d]",hash_index_);
                continue;
              }
              else
              {
            ...
            //YYSYS_LOG(ERROR,"test::whx ret start_agent next scan,hash_index_ [%d],fake_range(%s),range(%s) ret= [%d]", hash_index_,to_cstring(*(scan_param_->get_fake_range())),to_cstring(*(scan_param_->get_range())), ret);
            if(!iter_end)
            {
              ...
              else if(OB_ITER_END != ret)
              {
                ...
                set_failed_fake_range(*(scan_param_->get_fake_range()));
                //add e
                YYSYS_LOG(WARN, "get next cell failed![%d]",ret);

                break;//[global index build]
              }
            }
            else
            {
      ...
      else if(OB_TABLET_HAS_NO_LOCAL_SSTABLE == ret)
      {
          set_failed_fake_range(*(scan_param_->get_fake_range()));
          YYSYS_LOG(WARN, "get next cell failed![%d]", ret);
      }
