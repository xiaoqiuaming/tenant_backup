...
namespace oceanbase 
{
  namespace chunkserver 
  {
      ...
      const int64_t serving_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to check range list param, serving_version=%ld, ret=%d", 
          serving_version, ret);
      }
      else if (OB_SUCCESS != (ret = acquire_tablets_and_readers(tablet_list)))
      {
        YYSYS_LOG(WARN, "failed to acquire tablets and readers, serving_version=%ld, ret=%d", 
          serving_version, ret);
      }
      else if (OB_SUCCESS != (ret = get_new_tablet_range(new_range)))
      {
        YYSYS_LOG(WARN, "failed to get new tablet range");
      }
      else if (OB_SUCCESS != (ret = do_merge_sstable(new_range)))
      {
        YYSYS_LOG(WARN, "failed to merge sstable, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = create_new_tablet(new_range, new_tablet)) 
               || NULL == new_tablet)
      {
        YYSYS_LOG(WARN, "failed to create new tablet, new_tablet=%p, ret=%d", 
          new_tablet, ret);
      }
               ...
               && OB_SUCCESS != (ret = update_tablet_image(new_tablet)))
      {
        ...
        YYSYS_LOG(WARN, "failed to update tablet image, new_tablet=%p, ret=%d, range=%s", 
               ...
               && OB_SUCCESS != (ret = fill_return_tablet_list(tablet_list, *new_tablet)))
      {
        char range_buf[OB_RANGE_STR_BUFSIZ];
        new_tablet->get_range().to_string(range_buf, sizeof(range_buf));
        YYSYS_LOG(WARN, "failed to fill return tablet list, new_tablet=%p, "
                        "ret=%d, range=%s", 
          new_tablet, ret, range_buf);
      }
      ...
      const int64_t serving_version)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, merge tablet count must be greater "
      ...
      else 
      {
        for (int64_t i = 0; i < tablet_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "merge tabelt version isn't equal to current "
          ...
          {
            YYSYS_LOG(WARN, "the %ldth tablet range to merge is empty", i);
            ret = OB_INVALID_ARGUMENT;
            break;
          }
            ...
            tablet_infos[i].tablet_info_.range_.start_key_.length()))
          {
            ...
            YYSYS_LOG(WARN, "range isn't adjacent, prev_range=%s, cur_range=%s", 
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < tablet_list.get_tablet_size(); ++i)
        {
          ...
          YYSYS_LOG(INFO, "merge tablets[%ld]<%s>, "
      ...
      const ObTabletReportInfoList& tablet_list)
    {
      ...
      for (int64_t i = 0; i < tablet_count && OB_SUCCESS == ret; ++i)
      {
            ...
            ObMultiVersionTabletImage::SCAN_FORWARD, 0, tablet)))
        {
          ...
          YYSYS_LOG(WARN, "failed to qcquire tablet from tablet image, "
                          "index=%ld, range=%s",
             i, range_buf);
        }
        ...
        {
          tablet_infos[i].tablet_info_.range_.to_string(range_buf, sizeof(range_buf));
          YYSYS_LOG(WARN, "tablet image acquire tablet successfully, "
        ...
        {
          tablet_infos[i].tablet_info_.range_.to_string(range_buf, sizeof(range_buf));
          YYSYS_LOG(WARN, "tablet sequence num from rootserver is different "
        ...
        {
          YYSYS_LOG(WARN, "add tablet push to tablet array error, size=%ld",
        ...
        {
          tablet_infos[i].tablet_info_.range_.to_string(range_buf, sizeof(range_buf));
          YYSYS_LOG(WARN, "failed to load sstable, index=%ld, range=%s", 
            i, range_buf);
        }
        ...
        {
          tablet_infos[i].tablet_info_.range_.to_string(range_buf, sizeof(range_buf));
          YYSYS_LOG(WARN, "tablet has NULL sstable reader, index=%ld, range=%s", 
            i, range_buf);
          ret = OB_ERROR;
        }
        else if (NULL != reader && !sstable_array_.push_back(reader))
        {
          YYSYS_LOG(WARN, "add sstable reader push to sstable array error, size=%ld",
    ...
    int ObMultiTabletMerger::do_merge_sstable(const ObNewRange& new_range)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get sstable path for merged sstable");
      }
      else 
      {
        ...
        else 
        {
          ...
          YYSYS_LOG(INFO, "all tablets to merge is empty, tablet_count=%ld",
            tablet_array_.get_array_index());
        }
      }
    ...
    int ObMultiTabletMerger::get_new_sstable_path(ObString& sstable_path)
    {
      ...
      else
      {
        ...
        do
        {
          ...
          {
            YYSYS_LOG(WARN, "can't get the path of new sstable");
            ret = OB_ERROR;
          }
      ...
      const ObNewRange& new_range, ObTablet*& new_tablet)
    {
      ...
      else
      {
        ...
        if (sstable_size > 0)
        {
          if (NULL == trailer)
          {
            YYSYS_LOG(ERROR, "after merge sstable, trailer=NULL, "
                             "sstable_size=%ld", sstable_size);
            ret = OB_ERROR;
          }
          else if (OB_SUCCESS != (ret = tablet->add_sstable_by_id(sstable_id_)))
          {
            YYSYS_LOG(ERROR, "add sstable to tablet failed.");
          }
          else 
          {
            int64_t pos = 0;
    ...
    int ObMultiTabletMerger::get_new_tablet_range(ObNewRange& new_range)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid tablet in tablet array to merge, first_tablet=%p, "
                        "last_tablet=%p",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "this last mrege tablet has max flag");
    ...
    int ObMultiTabletMerger::update_tablet_image(ObTablet* tablet)
    {
      ...
      for (int64_t i = 0; i < tablet_array_.get_array_index() && OB_SUCCESS == ret; ++i)
      {
        if (NULL != merge_tablets_[i])
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to remove tablet, tablet=%p, ret=%d", 
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "failed to release tablet, tablet=%p, err=%d", 
                        merge_tablets_[i], ret);
            }
            else
            {
      ...
      {
        YYSYS_LOG(ERROR, "add new merged tablet into tablet iamge failed, "
      ...
      {
        YYSYS_LOG(WARN, "write new meta failed version=%ld, disk_no=%d",  
            tablet->get_data_version(), tablet->get_disk_no());
      }
