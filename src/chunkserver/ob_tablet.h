    ...
    class ObTablet
    {
      ...
      public:
        ...
        inline const common::ObSEArray<sstable::ObSSTableId, MAX_SSTABLE_PER_TABLET>&
          get_sstable_id_list() const
        { return sstable_id_list_; }
        inline const common::ObSEArray<sstable::SSTableReader*, MAX_SSTABLE_PER_TABLET>&
          get_sstable_reader_list() const
        { return sstable_reader_list_; }
      ...
      public:
        ...
        inline void set_merging(int status = -1)
        { merged_ = status; }
        inline bool is_merging() const
        { return merged_ < 0; }

        inline int get_merged() const
        { return merged_; }

        void set_merged(int status = 1);
        inline bool is_merged() const
        { return merged_ > 0; }
        inline void set_removed(int status = 1)
        { removed_ = status; }
        inline bool is_removed() const
        { return removed_ > 0; }
        inline void set_with_next_brother(int status = 0)
        { with_next_brother_ = status; }
        inline bool is_with_next_brother() const
        { return with_next_brother_ > 0; }
        inline int32_t get_merge_count() const
        { return merge_count_; }
        inline void inc_merge_count()
        { ++merge_count_; }
        ...
        inline int32_t get_compactsstable_num()
        {return compactsstable_num_;}
      ...
      private:
        ...
        yysys::CThreadMutex extend_info_mutex_;
        ...
        yysys::CThreadMutex load_sstable_mutex_;
    };
    ...
    template <typename Reader>
                                         ...
                                         Reader* sstable[], int32_t &size) const
      {
        ...
        if(1 == sstable_id_list_.count())
        {
          ...
          YYSYS_LOG(WARN, "get local sstable load failed[%d]", ret);
        }
        else
        {
          YYSYS_LOG(DEBUG,"test::whx sstable_id_list_.count() = [%ld], sstable_reader_list_.count() = [%ld]",
          ...
          if(OB_SUCCESS ==  ret && sstable_reader_list_.count() > 1)
          {
            ...
            YYSYS_LOG(ERROR,"test::whx sstable_reader_list_.count() = [%ld]",sstable_reader_list_.count());
          }
          */
          //add liumz, [check ret, add debug log]20170324:b
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "load sstable failed[%d]", ret);
          }
          //add:e
...
//            sstable[index] = dynamic_cast<Reader*> (reader);
            ...
            {
              ret = OB_TABLET_HAS_NO_LOCAL_SSTABLE;
              YYSYS_LOG(WARN,"test::whx sstable_id_list_.count() = [%ld], sstable_reader_list_.count() = [%ld]",
                        sstable_id_list_.count(), sstable_reader_list_.count());
            }
            //mod:e
            //YYSYS_LOG(ERROR,"test::whx load_local_sstable> 1");
          }        
          if(OB_SUCCESS == ret)
          {
            //YYSYS_LOG(ERROR,"test::whx reader range = %s",to_cstring(sstable[index]->get_range()));
          }
        }
        //modify e
        return ret;
