  ...
  namespace chunkserver
  {
      ...
      else
      {
        ...
        for(int n = 1; n <= OB_MAX_DISK_NUMBER; ++n) //disk index start from 1
        {
          ...
          if (FileDirectoryUtils::is_directory(disk_dir)/* && is_mount_point(disk_dir)*/)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "get sstable directory error, disk_no=%d", n);
              break;
            }
            else if ( 0 != statvfs(disk_dir,&svfs))
            {
              YYSYS_LOG(WARN, "stat vfs error, disk_dir=%s", disk_dir);
              continue;
            }
            else if (!FileDirectoryUtils::is_directory(sstable_dir))
            {
              YYSYS_LOG(WARN, "sstable dir =%s is not a directory.", sstable_dir);
              continue;
            }
            else
            {
        ...
        if (disk_num_ > 0)
        {
          YYSYS_LOG(INFO,"scan disk success.");
          dump();
          ...
          {
            YYSYS_LOG(ERROR, "disk accumulator init failed, ret:%d", ret);
          }
        }
        else
        {
          YYSYS_LOG(WARN,"no disk in dir %s, empty cs.", data_dir);
        }
      }
    ...
    int32_t ObDiskManager::get_disk_for_migrate()
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR,"disk space not enough");
        }
        else
        {
    ...
    int32_t ObDiskManager::get_dest_disk()
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR,"disk space not enough,avail_index:%ld,pending_files_ is %d",avail_index,
        ...
        const char* mount_path, int32_t& disk_no)
    {
      ...
      else
      {
        ...
        if (find_disk_no <= 0)
        {
          YYSYS_LOG(WARN, "cannot find a valid disk_no[%d]", disk_num_);
          dump();
        ...
        {
          YYSYS_LOG(WARN, "create symlink failed, old_path:%s new_path:%s, errno:%d str_errno:%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "scan data_dir:%s failed, ret:%d", data_dir, ret);
        }
        else
        {
    ...
    int ObDiskManager::check_disk(int32_t& bad_disk)
    {
      ...
      {
        uint64_t now = yysys::CTimeUtil::getTime();
        if (now - last_alarm_us_ > ALARM_INTERVAL &&
            alarm_times_++ < ALARM_TIMES)
        {
          YYSYS_LOG(ERROR, "found disk:%d bad", bad_disk);
          last_alarm_us_ = now;
        }
      ...
      if (disk_no > 0 && disk_no <= OB_MAX_DISK_NUMBER)
      {
        ...
        if (index != -1)
        {
          if (decr_pending_cnt)
          {
            ...
            {
              YYSYS_LOG(WARN, "disk_no:%d pending_files:%d reset to 0", disk_no, pending_files_[index]);
    ...
    int ObDiskManager::set_disk_status(const int32_t disk_no, const ObDiskStatus stat)
    {
      ...
      if (disk_no > 0 && disk_no <= OB_MAX_DISK_NUMBER)
      {
        ...
        {
          YYSYS_LOG(WARN, "not found disk:%d", disk_no);
        }
      }
      return ret;
    }
    ...
    bool ObDiskManager::is_mount_point(const char *path) const
    {
      ...
      if (NULL == path || '\0' == *path)
      {
        YYSYS_LOG(WARN,"path is null");
        err = OB_ERROR;
      }

      if ((OB_SUCCESS == err) && NULL == (fp = fopen(PROC_MOUNTS_FILE,"r")))
      {
        YYSYS_LOG(WARN,"open %s failed",PROC_MOUNTS_FILE);
        err = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == err)
      {
        if (lstat(path,&st) != 0)
        {
          YYSYS_LOG(WARN,"stat path failed");
          err = OB_ERROR;
        }
        else
        {
          if (S_ISLNK(st.st_mode))
          {
            ...
            {
              YYSYS_LOG(WARN,"readlink failed");
              ret = OB_ERROR;
            }
            else
            {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"read %s failed",PROC_MOUNTS_FILE);
        }
        else
        {
          buf[size] = '\0';
    ...
    void ObDiskManager::dump()
    {
      for(int i=0;i<disk_num_;++i)
      {
        YYSYS_LOG(INFO,"DISK %d: capacity:%ld,avail:%ld stat:%d",
            disk_[i].disk_no_,disk_[i].capacity_,disk_[i].avail_, disk_[i].status_);
      }
      return;
    }
    //[show disk]
    int64_t ObDiskManager::get_disk_avail(const int32_t disk_no)
    {
        int64_t avail = -1;
        if (disk_no > 0 && disk_no <= OB_MAX_DISK_NUMBER)
        {
