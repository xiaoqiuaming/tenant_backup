...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObMergerSchemaManager::init(const ObSchemaManagerV2 & schema)
    {
      ...
      if (true == init_)
      {
        YYSYS_LOG(WARN, "init schema manager twice");
        ret = OB_INIT_TWICE;
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "init schema manager succ:timestamp[%ld]", latest_version_);
        init_ = true;
      }
      return ret;
    }
        ...
        const ObSchemaManagerV2 ** manager)
    {
      ...
      {
        YYSYS_LOG(WARN, "check inner stat or schema version failed");
    ...
    int ObMergerSchemaManager::print_info(void) const
    {
      ...
      else
      {
        ...
        for (uint64_t i = 0; i < MAX_VERSION_COUNT; ++i)
        {
          ...
          {
            YYSYS_LOG(INFO, "schema ref_count[%lu]", _schemas[i].ref_count);
