  ...
  namespace proxyserver
  {
    ...
    int ProxyReader::fill_data(common::ObDataBuffer& buffer)
    {
      ...
      if(send_len > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "serialize finish flag failed, ret:%d", ret);
        }
      ...
      {
        YYSYS_LOG(ERROR, "has no remain data or argument invalid");
        ...
        int64_t req_tablet_version, int16_t remote_sstable_version, const char* sstable_dir)
    {
      ...
      {
        YYSYS_LOG(ERROR, "argument invalid");
        ret = OB_INVALID_ARGUMENT;
      }
      else if(OB_SUCCESS != (ret = proxy->open_rfile(fp_, range, sstable_dir)))
      {
        YYSYS_LOG(WARN, "yunti has some problem, or path:%s not exist", sstable_dir);
      }
      ...
      {
        YYSYS_LOG(ERROR, "version not equal, req_tablet_version:%ld local_sstable_version:%d remote_sstable_version:%d", req_tablet_version, local_sstable_version, remote_sstable_version);
        ret = OB_SSTABLE_VERSION_UNEQUAL;
      }

      open_file_time_us_ = yysys::CTimeUtil::getTime() - start_time_us;
    ...
    int YuntiFileReader::read_next(common::ObDataBuffer& buffer)
    {
      ...
      if(!has_remain_data())
      {
        ...
        {
          YYSYS_LOG(WARN, "yunti has some problem, read_size:%ld ret:%d", read_size, ret);
          ret = OB_DATA_SOURCE_READ_ERROR;
        }
        else
        {
          if(read_size < 0)
          {
            YYSYS_LOG(WARN, "yunti has some problem, read_size:%ld", read_size);
            ret = OB_DATA_SOURCE_READ_ERROR;
          }
          else
          {
            ...
            total_read_time_us_ += yysys::CTimeUtil::getTime() - start_time_us;
          }
        }
      }
    ...
    int YuntiFileReader::close()
    {
      ...
      YYSYS_LOG(INFO,
                "duration of read file: open_file_time_us[%ld] total_read_size[%ld] total_read_time_us[%ld] read_speed[%.2f]kb/s",
                open_file_time_us_,
                total_read_size_,
                total_read_time_us_,
                read_speed);
...
#define RANGE_META 9
      ...
      else
      {
        ...
        for(; offset_ < range_list_.count(); offset_++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize failed, ret:%d", ret);
            break;
          }
          else
          {
            YYSYS_LOG(INFO, "range:%s", to_cstring(*range));
            count++;
          }
        }
      }
      ...
      YYSYS_LOG(INFO, "range, count:%ld", count);
