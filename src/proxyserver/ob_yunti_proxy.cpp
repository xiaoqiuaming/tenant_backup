...
namespace oceanbase
{
  namespace proxyserver
  {
    ...
    int YuntiProxy::initialize()
    {
      ...
      {
        YYSYS_LOG(ERROR, "hdfs meta init failed, ret:%d", ret);
      }
    ...
    int YuntiProxy::open_rfile(FILE* &fp, const char* file_name)
    {
      ...
      int buf_size = snprintf(cmd, MAX_CMD_LEN, "hdfs dfs -cat %s", file_name);
      if(buf_size + 1 > MAX_CMD_LEN)
      {
        YYSYS_LOG(WARN, "cmd, cmd_len=%d <= bufsiz=%d", MAX_CMD_LEN, buf_size);
        ret = OB_SIZE_OVERFLOW;
      }
      else if(NULL == (fp = popen(cmd, "r")))
      {
        YYSYS_LOG(WARN, "get file:%s locate path failed, error:%d error_str:%s", file_name, errno, strerror(errno));
    ...
    int YuntiProxy::open_rfile(FILE* &fp, common::ObNewRange& range, const char* sstable_dir)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get range:%s locate path failed", to_cstring(range));
      }
      else if(!is_file_exist(sstable_file_name))
      {
        YYSYS_LOG(ERROR, "file %s not exist", sstable_file_name);
    ...
    int YuntiProxy::close_rfile(FILE* fp)
    {
      ...
      {
        YYSYS_LOG(WARN, "close file failed errno:%d error_str:%s ret:%d", errno, strerror(errno), ret);
      }
    ...
    int YuntiProxy::fetch_data(FILE* fp, char* buf, int64_t size, int64_t &read_size)
    {
      ...
      if(!fp)
      {
        YYSYS_LOG(WARN, "fp is null, maybe file not open");
        read_size = 0;
      }
      else if(ferror(fp))
      {
        YYSYS_LOG(WARN, "fp read failed, errno:%d, error_str:%s", errno, strerror(errno));
        ret = OB_DATA_SOURCE_SYS_ERROR;
      }
      else if(feof(fp))
      {
        YYSYS_LOG(INFO, "file reach end");
      }
      else
      {
    ...
    bool YuntiProxy::is_file_exist(const char* file_name)
    {
      ...
      memset(cmd, 0, sizeof(cmd));
      FILE* fp = NULL;
      int buf_size = snprintf(cmd, MAX_CMD_LEN, "hdfs dfs -du  %s 2>&1", file_name);
...
#define MAX_FILE_INFO_LEN 1024
        ...
        if(fgets(file_info, sizeof(file_info), fp))
        {
          ...
          {
            YYSYS_LOG(INFO, "file:%s is not exist", file_name);
            ret = false;
          }
        }
    ...
    int YuntiProxy::get_file_size(const char* file_name, int64_t &file_size)
    {
      ...
      int buf_size = snprintf(cmd, MAX_CMD_LEN, "hdfs dfs -du %s |gawk -F \" \" '{print $1}'", file_name);
