...
namespace oceanbase
{
  namespace proxyserver
  {
    ...
    int ObProxyService::initialize(ObProxyServer* proxy_server)
    {
      ...
      {
        YYSYS_LOG(ERROR, "yunti has some problem, check it, ret:%d", ret);
        ret = OB_ERROR;
      }
      else if(0 >= proxy_server->get_config().get_all_proxy_name(proxy_names))
      {
        YYSYS_LOG(ERROR, "config has some problem, check it, ret:%d", ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        const int64_t timeout_time)
    {
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        YYSYS_LOG(INFO, "begin fetch_data, %s", to_cstring(desc));
        ...
        else if(strcmp(data_source_name, ObProxyServerConfig::YUNTI_PROXY) == 0)
        {
          ...
          if(NULL == sstable_dir)
          {
            YYSYS_LOG(ERROR, "sstable dir miss");
            rc.result_code_ = OB_INVALID_ARGUMENT;
          }
          else
          {
        ...
        {
          YYSYS_LOG(ERROR, "not support this data source:%s", data_source_name);
          rc.result_code_ = OB_DATA_SOURCE_NOT_EXIST;
        }
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize sequence object failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize row_checksum object failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "serialize sstable type failed, ret:%d", serialize_ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "proxy reader close fail, ret:%d", ret);
      }


      YYSYS_LOG(INFO, "fetch data finish, rc:%d", rc.result_code_);
    ...
    int ObProxyService::get_yunti_range_reader(YuntiProxy* proxy, const char* table_name, const char* sstable_dir, ProxyReader* &proxy_reader)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "prepare yunti range reader failed, ret:%d", ret);
        }
        else
        {
        ...
        const int64_t timeout_time)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize the table_name error, ret:%d", rc.result_code_);
      ...
      {
        YYSYS_LOG(ERROR, "deserialize the uri error, ret:%d", rc.result_code_);
        rc.result_code_ = OB_DESERIALIZE_ERROR;
      }

      if (OB_SUCCESS == rc.result_code_)
      {
        YYSYS_LOG(INFO, "begin fetch_range, uri:%s table_name:%s", uri, table_name);
        ...
        else if(strcmp(data_source_name, ObProxyServerConfig::YUNTI_PROXY) == 0)
        {
          ...
          {
            YYSYS_LOG(INFO, "sstable dir:%s", sstable_dir);
