...
namespace oceanbase
{
  namespace proxyserver
  {
    ...
    int ObProxyServerMain::do_work()
    {
      ...
      {
        YYSYS_LOG(ERROR, "not set the java home");
        ret = OB_ERROR;
      }
      else if(0 != setenv("JAVA_HOME", java_home, 1))
      {
        YYSYS_LOG(ERROR, "JAVA_HOME set failed errno:%d, error_str:%s", errno, strerror(errno));
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(ERROR, "HADOOP_CONF_DIR set failed errno:%d, error_str:%s", errno, strerror(errno));
      }

      //check hadoop config whether exist
      char config_file[OB_MAX_FILE_NAME_LENGTH];
      memset(config_file, 0, sizeof(config_file));
      int size = snprintf(config_file, OB_MAX_FILE_NAME_LENGTH, "%s/core-site.xml", hdfs_config);
      if(size + 1 > OB_MAX_FILE_NAME_LENGTH || size < 0)
      {
        YYSYS_LOG(ERROR, "config, config_len=%ld <= bufsiz=%d", OB_MAX_FILE_NAME_LENGTH, size);
        ret = OB_SIZE_OVERFLOW;
      }
      else if(access(config_file, R_OK) != 0)
      {
        YYSYS_LOG(ERROR, "config_file:%s cannot access, errno:%d, error_str:%s", config_file, errno, strerror(errno));
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(ERROR, "not set the hadoop home");
        ret = OB_ERROR;
      }
      else if(0 != setenv("HADOOP_HOME", hadoop_home, 1))
      {
        YYSYS_LOG(ERROR, "HADOOP_HOME set failed errno:%d, error_str:%s", errno, strerror(errno));
        ret = OB_ERROR;
      }
      else
      {
        ...
        if(size + 1 > OB_MAX_FILE_NAME_LENGTH || size < 0)
        {
          YYSYS_LOG(ERROR, "hadoop, hadoop_len=%ld <= bufsiz=%d", OB_MAX_FILE_NAME_LENGTH, size);
          ret = OB_SIZE_OVERFLOW;
        }
        else if(access(hadoop_bin, X_OK) != 0)
        {
          YYSYS_LOG(ERROR, "hadoop_bin:%s cannot access, errno:%d, error_str:%s", hadoop_bin, errno, strerror(errno));
          ret = OB_ERROR;
        }
      }
...
#define MAX_ENV_LEN 1024
        ...
        {
          YYSYS_LOG(ERROR, "get path failed errno:%d, error_str:%s", errno, strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "env, env_len=%d <= bufsiz=%d", MAX_ENV_LEN, buf_size);
            ret = OB_SIZE_OVERFLOW;
          }
          else if(0 != setenv("PATH", env, 1))
          {
            YYSYS_LOG(ERROR, "PATH set failed errno:%d, error_str:%s", errno, strerror(errno));
            ret = OB_ERROR;
          }
        }
      }
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "Start proxy server failed, ret: [%d]", ret);
      }
      else
      {
