  ...
  namespace backupserver
  {
    ...
    int ObTabletBackupService::start()
    {
      ...
      else
      {
        ...
        if (rc != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "init backup rpc failed.");
        }
      }
    ...
    int ObTabletBackupService::start_backup_task(int mode, const char * db_name, const char * table_name)
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "start rs and ups fetch task failed.");
      }
      else
      {
        ...
        if (ret == OB_SUCCESS)
        {
          YYSYS_LOG(INFO, "start rs and ups fetch task succeed.");
        }
        else
        {
          YYSYS_LOG(ERROR, "start rs and ups fetch task failed.");
        }
      }
    ...
    int ObTabletBackupService::backup_fetch_data(
        ...
        const int64_t timeout_time)
    {
      YYSYS_LOG(INFO, "timeout_time =%ld", timeout_time);
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      YYSYS_LOG(INFO, "backupserver start sync all tablet images");
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        rc.result_code_ = OB_ERROR;
        YYSYS_LOG(WARN, "deserialize backup mode failed, ret:%d", rc.result_code_);
      }
      else if ( mode <=  MIN_BACKUP && mode >= MAX_BACKUP)
      {
        rc.result_code_ = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "backup mode is invalid, ret:%d", rc.result_code_);
      }
      else if (OB_SUCCESS != ( rc.result_code_ = start_backup_task(mode)))
      {
        YYSYS_LOG(WARN, "start backup task failed, ret:%d", rc.result_code_);
      }
      //response to root server first
      int serialize_ret = rc.serialize(out_buffer.get_data(),
          out_buffer.get_capacity(), out_buffer.get_position());
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        rc.result_code_ = OB_ERROR;
        YYSYS_LOG(WARN, "deserialize failed, ret:%d", rc.result_code_);
      }
      else if ( NULL == serialization::decode_vstr(in_buffer.get_data(),
      ...
      {
        YYSYS_LOG(WARN, "db_name [%s], table_name [%s]", db_name, table_name);
        YYSYS_LOG(WARN, "start backup task failed, ret:%d", rc.result_code_);
      }
      //response to root server first
      int serialize_ret = rc.serialize(out_buffer.get_data(),
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "abort backup task failed, ret:%d", rc.result_code_);
      }
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "check backup privilege failed, ret:%d", rc.result_code_);
      }
