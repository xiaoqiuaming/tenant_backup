...
namespace oceanbase
{
  namespace backupserver
  {
    ...
    void ObTabletBackupRunnable::run(yysys::CThread *thread, void *arg)
    {
      UNUSED(thread);
      UNUSED(arg);
      YYSYS_LOG(INFO, "[NOTICE] backup worker thread start, waiting [%s]",
      ...
      YYSYS_LOG(INFO, "[NOTICE] backup thread is working");
      while (!_stop)
      {
        ...
        YYSYS_LOG(INFO, "backup worker idle, sleep [%s]",
      ...
      YYSYS_LOG(INFO, "[NOTICE] backup worker thread exit");
    }
  }
}
