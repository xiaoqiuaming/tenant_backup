...
namespace oceanbase
{
  namespace backupserver
  {
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "dump the backup progress info start");
      bool do_print = false;
      if (backup_start_)
      {
        int64_t cur_time = yysys::CTimeUtil::getTime();
        YYSYS_LOG(INFO, "the backup is working, dumping [%ld]tables, start_time[%ld], cost_time [%ld]",task_array_.count(),start_time_,cur_time- start_time_);
        do_print = true;
      }
      else if (start_time_ == 0)
      {
        ret = OB_OP_NOT_ALLOW;
        YYSYS_LOG(INFO, "the backup is not started, start_time");
      }
      else
      {
        ret = OB_ITER_END;
        do_print = true;
        YYSYS_LOG(INFO, "the backup is finished, dumped [%ld]tables, start_time[%ld], end_time[%ld], cost_time[%ld]",task_array_.count(),start_time_, end_time_, end_time_-start_time_);
      }

      if (do_print)
      {
        yysys::CWLockGuard task_guard(task_thread_rwlock_);
        if (backup_start_)
        {
          YYSYS_LOG(INFO, "the [%ld]-th table is backup", doing_task_idx+1);
        }
        for (int64_t idx =0 ; idx <task_array_.count(); idx++)
        {
          ...
          {
            YYSYS_LOG(INFO,"table_id[%ld],status[%s]",
                  task_array_.at(idx).table_id,
                      parse_backup_status(task_array_.at(idx).status));
            if (task_array_.at(idx).status == BACKUP_DOING)
            {
              YYSYS_LOG(INFO,"start_time[%ld]",task_array_.at(idx).start_time_);
            }
            else
            {
              YYSYS_LOG(INFO,"start_time[%ld], end_time[%ld], cost_time[%ld]",
      ...
      YYSYS_LOG(INFO, "dump the backup progress info end");
      return ret;
    ...
    int ObTabletBackupManager::initialize(ObBackupServer * server)
    {
      ...
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "root rpc proxy init failed:ret[%d]", ret);
        }
      }
    ...
    int ObTabletBackupManager::do_start_backup_task( const char * db_name, const char * table_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "backup is already started");
        ret = OB_EAGAIN;
      }
      else if (tablet_version == 0 || tablet_version <= backup_server_->get_ups_frozen_version())
      //mod duyd linhx [desi] 20211208 before:tablet_version == backup_server_->get_ups_frozen_version()
      {
        clear();
        ObString dt;
      ...
      else
      {
        ...
        YYSYS_LOG(ERROR, "data version is not matched, failed to backup the chunkserver data. tablet_version=%ld, ups_frozen_version=%ld", tablet_version, backup_server_->get_ups_frozen_version());
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        yysys::CThreadGuard task_mutex_guard(&(task_thread_mutex_));
        yysys::CWLockGuard task_guard(task_thread_rwlock_);
    ...
    int ObTabletBackupManager::do_abort_backup_task()
    {
      ...
      {
        YYSYS_LOG(WARN, "backup service is not started, no backup task is aborted");
    ...
    int ObTabletBackupManager::do_tablet_backup_over(int code, ObDataSourceDesc & desc)
    {
      ...
      {
        YYSYS_LOG(WARN, "abandon the msg, tablet info [%s]",to_cstring(desc.range_));
      }
      else
      {
        //this for only one thread modify
        yysys::CThreadGuard mutex_guard(&(tablet_task_thread_mutex_));
        yysys::CWLockGuard guard(tablet_task_thread_rwlock_);
        ...
        else if (OB_SUCCESS == code)
        {
          ...
          {
            task_array_.at(doing_task_idx).status = BACKUP_DONE;
            task_array_.at(doing_task_idx).end_time_ = yysys::CTimeUtil::getTime();
            YYSYS_LOG(INFO, "set task_array.at(%ld) backup finish", doing_task_idx);
          }
        }
        ...
        {
          YYSYS_LOG(WARN, "failed to migrate tablet, ret[%d]", code);
          tablet_task_array_.at(idx).status_ = BACKUP_WAITING;
    ...
    int ObTabletBackupManager::do_dispatch_backup_task()
    {
      ...
      {
        YYSYS_LOG(ERROR, "backup service is not started, cannot dispatch backup task");
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          task_array_.at(doing_task_idx).start_time_= yysys::CTimeUtil::getTime();
          ret = generate_tablet_backup_task();
          if (ret == OB_SUCCESS || ret == OB_CANCELED)
          {
            YYSYS_LOG(INFO,"generate the backup task [doing_task_idx:%ld], ret [%d]", doing_task_idx, ret);
          }
          else
          {
            task_array_.at(doing_task_idx).status = BACKUP_ERROR;
            task_array_.at(doing_task_idx).end_time_= yysys::CTimeUtil::getTime();
            YYSYS_LOG(INFO,"failed to generate the backup task [ret:%d]", ret);
          }
        }
        else if (status == BACKUP_ERROR || status == BACKUP_DONE || status == BACKUP_CANCELLED)// sub tablet_backup is finished
        {
          ...
          if (doing_task_idx == task_array_.count())
          {
            ...
            print();
            YYSYS_LOG(INFO,"the backup task is finished [task number:%ld]", task_array_.count());
            ret = OB_SUCCESS;
          ...
          else
          {
            ...
            else
            {
              task_array_.at(doing_task_idx).end_time_= yysys::CTimeUtil::getTime();
              task_array_.at(doing_task_idx).status = BACKUP_ERROR;
              YYSYS_LOG(INFO,"failed to generate the backup task [doing_task_idx:%ld],[ret:%d]",doing_task_idx,ret);
            }
          }
        }
        else if (status == BACKUP_DOING)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            YYSYS_LOG(INFO,"send tablet backup task [count:%d] in this round", count);
          }
          else if(OB_ITER_END == ret)
          {
            YYSYS_LOG(ERROR,"send tablet backup task [ret:%d] in this round", ret);
            task_array_.at(doing_task_idx).status = BACKUP_ERROR;
            task_array_.at(doing_task_idx).end_time_= yysys::CTimeUtil::getTime();
          }
          else
  ...
  int ObTabletBackupManager::generate_tablet_backup_task()
  {
    ...
    int status = BACKUP_INIT;
    {
      ...
      {
        ret = OB_CANCELED;
        YYSYS_LOG(INFO,"the backup task has been cancelled [table_id:%ld]", table_id);
      }
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      do
      {
        ...
        if ((OB_SUCCESS == ret)
            ...
            && (OB_ITER_END != ret))
        {
          YYSYS_LOG(WARN,"fail to find replicas [ret:%d]", ret);
        }
        if (OB_ITER_END == ret)
        {
          YYSYS_LOG(WARN,"fail to find replicas, while retry [ret:%d]", ret);
          ret = OB_ERR_UNEXPECTED;
        }
        if ((OB_SUCCESS == ret) && (replica_count <= 0))
        {
          YYSYS_LOG(WARN,"fail to find replicas, while retry [ret:%d,replica_count:%d]", ret, replica_count);
          ret = OB_DATA_NOT_SERVE;
        }
        else
        {
          YYSYS_LOG(INFO,"succeed to find replicas, [ret:%d,replica_count:%d]", ret, replica_count);
        }
      ...
      {
        YYSYS_LOG(INFO," tablet_task_array_[%ld], rowkey [%s]", idx,to_cstring(tablet_task_array_.at(idx).range_));
        for (int64_t tmp = 0; tmp < tablet_task_array_.at(idx).replica_count_; tmp ++)
        {
          YYSYS_LOG(INFO," replica [%ld], chunkserver info [%s]",
  ...
  int ObTabletBackupManager::async_do_tablet_backup_task( int32_t & count)
  {
    ...
    count = 0;
    {
      ...
      YYSYS_LOG(INFO," async do tablet_backup_task min[%ld],max[%ld]",
      ...
      for (int64_t idx = min_done_tablet_task_idx +1; idx <= max_doing_tablet_task_idx; idx++)
      {
            ...
            || (task.status_ == BACKUP_DOING && cur_time - task.start_time_ > migrate_timeout))
        {
          if (task.status_ == BACKUP_DOING && task.retry_time_ > retry_time)
          {
            ...
            YYSYS_LOG(INFO," task.cur_replica_cs_[%d]",task.cur_replica_cs_);
          }
          else
          {
            YYSYS_LOG(INFO," async do tablet_backup_task, table_range[%s]",
        ...
        {
          YYSYS_LOG(INFO," task.retry_time[%ld]",task.retry_time_);
          YYSYS_LOG(INFO," task.cost_time[%ld],timeout[%ld]",cur_time - task.start_time_,migrate_timeout);
          YYSYS_LOG(INFO," task.cur_replicas_cs[%d],task.replicas_count[%d]",task.cur_replica_cs_,task.replica_count_);
          YYSYS_LOG(INFO," task.status[%d]",task.status_);
  ...
  int ObTabletBackupManager::inner_fill_backup_task(ObString & dt)
  {
    ...
    if ( 0 == dt.size())
    {
      YYSYS_LOG(INFO, "backup the whole database");
      ...
      for (uint64_t tid= 1; tid <=schema_mgr_.get_max_table_id(); tid++)
      {
        ...
        if (NULL != (table_schema = schema_mgr_.get_table_schema(tid)))
        {
              ...
              ((table_schema->get_index_helper().tbl_tid != OB_INVALID_ID)&&(table_schema->get_index_helper().status == common::AVALIBALE)))
          {
            ...
            YYSYS_LOG(INFO, "the table task [%s] is added ",task.table_name);
            ret = OB_SUCCESS;
          }
          //mod:e
        }
    ...
    {
      ret = OB_ENTRY_NOT_EXIST;
      YYSYS_LOG(INFO, "the table [%.*s] is not existed ",dt.length(),dt.ptr());
    }
