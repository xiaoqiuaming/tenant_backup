...
void ObBackupUpsFetchRunnable::run(yysys::CThread* thread, void* arg)
{
  int ret = OB_SUCCESS;
  is_backup_done_ = false;
  ...
  if (!is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObUpsFetchRunnable has not been init");
    ret = OB_NOT_INIT;
  }
  ...
  if (ret == OB_SUCCESS && !_stop)
  {
    if (ups_param_.fetch_sstable_)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "get_sstable error, ret=%d", ret);
      }
    }
    YYSYS_LOG(INFO, "log_dir = %s", log_dir_);
  ...
  YYSYS_LOG(INFO, " paxos%ld ObUpsFetchRunnable finished[stop=%d ret=%d]", paxos_id_, _stop, ret);
}

int ObBackupUpsFetchRunnable::init(const ObServer& master, const char* log_dir, const updateserver::ObUpsFetchParam &param,
                                   updateserver::SSTableMgr *sstable_mgr, ObBackupServer *server, int64_t paxos_id)
{

  int ret = OB_SUCCESS;
  YYSYS_LOG(WARN, "ObFetchRunnable init log_dir=%s", log_dir);
  if (NULL == sstable_mgr)
  {
    YYSYS_LOG(ERROR, "Invalid arguments, sstable_mgr=%p", sstable_mgr);
    ret = OB_INVALID_ARGUMENT;
  }

  if (OB_SUCCESS == ret)
  {
    ret = init(master, log_dir, param, paxos_id);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObFetchRunnable init error, ret=%d log_dir=%s",
          ret, log_dir);
    }
    else
    {
      YYSYS_LOG(INFO, "ObFetchRunnable init succeed, ret=%d log_dir=%s",
      ret, log_dir_);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObUpsFetchParam clone error, ret=%d", ret);
  ...
  YYSYS_LOG(INFO, "ObBackupUpsFetchRunnable init succeed, ret=%d log_dir=%s",ret, log_dir_);
  return ret;
}

int ObBackupUpsFetchRunnable::init(const ObServer& master,
                                   const char* log_dir,
                                   ...
                                   common::ObLogReplayRunnable *replay_thread)
{
  ...
  else
  {
    ...
    {
       YYSYS_LOG(INFO, "ObFetchRunnable role_mgr, replay_thread = NULL");
    }
    if (NULL == log_dir)
    {
      YYSYS_LOG(ERROR, "Parameter are invalid[log_dir=%p]",log_dir);
      ret = OB_INVALID_ARGUMENT;
    }
    else
    {
...
int ObBackupUpsFetchRunnable::set_fetch_param(const updateserver::ObUpsFetchParam& param)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "ObFetchParam clone error, ret=%d", ret);
  }
  else
  {
...
int ObBackupUpsFetchRunnable::gen_fetch_sstable_cmd_(const char* name, const char* src_path, const char* dst_path, char* cmd, const int64_t size) const
{
  ...
  {
    YYSYS_LOG(ERROR, "Parameters are invalid[name=%s src_path=%s dst_path=%s cmd=%p size=%ld", name, src_path, dst_path, cmd, size);
    ret = OB_INVALID_ARGUMENT;
  }
...
int ObBackupUpsFetchRunnable::remote_cp_sst_(const char* name, const char* src_path, const char* dst_path) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "gen_fetch_sstable_cmd_ error, ret=%d", ret);
    }
    else
    {
      ret = FSU::vsystem(cmd);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "vsystem_(cmd=%s) error, ret=%d", cmd, ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "remote_cp_sst succ, cmd=%s", cmd);
      }
    }
  }
...
int ObBackupUpsFetchRunnable::get_sstable_()
{
  ...
  for (ssti = ups_param_.sst_list_.begin(); !_stop && ssti != ups_param_.sst_list_.end(); ++ssti)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObStoreMgr assign_stores error, ret=%d", ret);
    }
    else if (store_list.empty())
    {
      YYSYS_LOG(WARN, "store list is empty, ret=%d", ret);
      ret = OB_ERROR;
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (NULL == dst_path1)
      {
        YYSYS_LOG(WARN, "1st dir in store list is NULL");
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "remote_cp_sst_ error, ret=%d", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "get sstable succ, path=%s name=%s dst_path=%s", ssti->path.ptr(), ssti->name.ptr(), dst_path1);
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "remote_cp_schema_ error, ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(INFO, "get schema succ, path=%s name=%s dst_path=%s", ssti->path.ptr(), fpath, dst_path1);
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      for (; stoi != store_list.end(); ++stoi, ++counter)
      {
        ...
        if (NULL == dst_path2)
        {
          YYSYS_LOG(WARN, "%d dir in store list is NULL", counter);
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "cp_safe error, ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(INFO, "copy sstable succ, src_path=%s name=%s dst_path=%s", dst_path1, ssti->name.ptr(), dst_path2);
          }
        }
      }
    }
