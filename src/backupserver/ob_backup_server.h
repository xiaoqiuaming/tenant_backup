  ...
  namespace backupserver
  {
    ...
    class ObBackupServer : public common::ObSingleServer
    {
      ...
      public:
        ...
        const ObArray<UpsBackupInfo> get_ups_backup_info() const;
        ObArray<UpsBackupInfo> get_ups_backup_info();
      ...
      private:
        ...
        int get_ups_fetch_param(common::ObString& sql_str, int mode, uint64_t& max_log_id, uint64_t& max_sstable_id);
        int update_inner_backup_table(bool is_finish = FALSE);
        int update_ups_backup_info(int idx, int mode = 0);
      private:
        ...
        //ups info
        ObUpsList master_ups_list_;
        SSTableMgr sstable_mgr_[MAX_UPS_COUNT_ONE_CLUSTER];
        int64_t paxos_id_;
        ObArray<UpsBackupInfo> ups_backup_info_;
        updateserver::ObUpsFetchParam param_;
        bool all_ups_backup_done;
        ...
        ObBackupUpsFetchRunnable *ups_fetch_thread_[MAX_UPS_COUNT_ONE_CLUSTER];
