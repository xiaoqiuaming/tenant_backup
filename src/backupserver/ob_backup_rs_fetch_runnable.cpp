  ...
  namespace backupserver
  {
    ...
    int ObBackupRootFetchRunnable::init(const ObServer& master, const char* log_dir)
    {
      ...
      else
      {
        if (NULL == log_dir )
        {
          YYSYS_LOG(ERROR, "Parameter are invalid[log_dir=%p]",
              log_dir);
    ...
    void ObBackupRootFetchRunnable::run(yysys::CThread* thread, void* arg)
    {
      int ret = OB_SUCCESS;
      ...
      if (!is_initialized_)
      {
        YYSYS_LOG(ERROR, "ObBackupFetchThread has not been initialized");
        ret = OB_NOT_INIT;
      }
    ...
    int ObBackupRootFetchRunnable::wait_fetch_done()
    {
      ...
      while (!is_backup_done_ && !_stop)
      {
        ...
        if ((count % 100) == 0)
        {
          YYSYS_LOG(INFO, "wait fetch rs log used %d seconds", (count/10));
        }
        usleep(WAIT_INTERVAL);
      }
      YYSYS_LOG(INFO, "wait fetch done:, backup_done[%d], stop[%d]", is_backup_done_, _stop);
      if (!is_backup_done_)
      {
        ret = OB_ERROR;
      }
    ...
    int ObBackupRootFetchRunnable::get_first_meta()
    {
      ...
      if (!is_initialized_)
      {
        YYSYS_LOG(ERROR, "ObBackupRootFetchRunnable has not been initialized");
        ret = OB_NOT_INIT;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "fetch first_tablet_meta");

        ret = gen_fetch_cmd_(ObBackupRootFetchRunnable::FIRST_META, NULL, cmd, OB_MAX_FETCH_CMD_LENGTH);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "gen_fetch_cmd_[file %s fn_ext=NULL] error[ret=%d]", ObBackupRootFetchRunnable::FIRST_META, ret);
          ret = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "fetch log[file=%s]: \"%s\"", ObBackupRootFetchRunnable::FIRST_META, cmd);
          ret = FSU::vsystem(cmd);
          if (0 != ret)
          {
            YYSYS_LOG(ERROR, "fetch[\"%s\"] error[ret=%d]", cmd, ret);
            ret = OB_ERROR;
          }
        }
      }
    ...
    int ObBackupRootFetchRunnable::gen_fetch_cmd_(const char* file_name, const char* fn_ext, char* cmd, const int64_t size)
    {
      ...
      if (NULL == cmd || size <= 0)
      {
        YYSYS_LOG(ERROR, "Parameters are invalid[file=%s fn_ext=%s cmd=%p size=%ld", file_name, fn_ext, cmd, size);
        ret = OB_INVALID_ARGUMENT;
      }
    ...
    bool ObBackupRootFetchRunnable::exists_(const char* file_name, const char* fn_ext) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "gen_full_name_ error[ret=%d file=%s fn_ext=%s]", ret, file_name, fn_ext);
      }
      else
      {
        YYSYS_LOG(WARN, "fn[file=%s]", fn);
    ...
    bool ObBackupRootFetchRunnable::remove_(const char * file_name, const char* fn_ext) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "gen_full_name_ error[ret=%d id=%s fn_ext=%s]", ret, file_name, fn_ext);
      }
      else
      {
