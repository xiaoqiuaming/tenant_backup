...
namespace oceanbase
{
  namespace obmysql
  {
    ...
    bool ObMySQLServer::has_too_many_sessions() const
    {
      ...
      {
        YYSYS_LOG(WARN, "there are too many sessions, session_count=%ld mem_limit=%ld",
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "initialize obmsql");
      //add bizb [slow_query]
      YYSYS_SLOWSQLLOGGER.setFileName("./log/slowquery.log",true);
      YYSYS_SLOWSQLLOGGER.setLogLevel("info");
      YYSYS_SLOWSQLLOGGER.setMaxFileSize(256 * 1024L * 1024L);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "set yaomysql io_thread_count=%s failed",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "create session mgr failed");
        }
        else if (OB_SUCCESS != (ret = timer_.schedule(session_mgr_, ObSQLSessionMgr::SCHEDULE_PERIOD, true)))
        {
          YYSYS_LOG(ERROR, "failed to schedule session timeout checker, ret=%d", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "YaoPsStore init error, ret = %d", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "YaoSQLIdMgr init error, ret: %d", ret);
        }
        else if (OB_SUCCESS != (ret = query_cache_.init(config_)))
        {
          YYSYS_LOG(ERROR, "YaoSQLQueryCache init error, ret: %d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to init FIFO stream, err=%d", ret);
        }
      }
      //start work thread
      if (OB_SUCCESS == ret)
      {
        ...
        {
          IpPort ip_port;
          uint32_t local_ip = yysys::CNetUtil::getLocalAddr(config_->devname);
          ...
          YYSYS_LOG(INFO, "yaomysql work thread count=%d", work_thread_count_);
        ...
        close_command_queue_thread_.setThreadParameter(1, this, NULL, true);
        YYSYS_LOG(INFO, "start thread to handle close request");
        close_command_queue_thread_.start();
        YYSYS_LOG(INFO, "start thread to get bloom filter");
      }
      return ret;
    }
    int ObMySQLServer::get_privilege(ObPrivilege * p_privilege)
    {
      int ret = OB_SUCCESS;
      int err = OB_SUCCESS;
      ObString get_users = ObString::make_string(
                  "SELECT /*+read_consistency(WEAK) */ u.user_name,u.user_id,u.pass_word,u.info,u.priv_all,u.priv_alter,u.priv_create,u.priv_create_user,u.priv_delete,u.priv_drop,u.priv_grant_option,u.priv_insert,u.priv_update,u.priv_select,u.priv_replace,u.is_locked  FROM __all_user u");
      ObString get_table_privileges = ObString::make_string(
                  "SELECT /*+read_consistency(WEAK) */ t.user_id,t.db_id,t.table_id,t.priv_all,t.priv_alter,t.priv_create,t.priv_create_user,t.priv_delete,t.priv_drop,t.priv_grant_option,t.priv_insert,t.priv_update,t.priv_select, t.priv_replace FROM __all_table_privilege t");//add liumz, [drop table -> clean table priv]20150902
      ObString get_privilege_version = ObString::make_string("SELECT /*+read_consistency(WEAK) */ value FROM __all_sys_stat where name='yb_current_privilege_version'");
      //add wenghaixing [database manage]20150607
      ObString sql_get_database = ObString::make_string(
                  "SELECT /*+read_consistency(WEAK) */d.db_name,d.db_id,d.stat FROM __all_database d");
      ...
      {
        YYSYS_LOG(WARN, "failed to init context, err=%d", ret);
      }
      ...
      else
      {
        ...
        while (!stop_)
        {
          ...
          {
            YYSYS_LOG(INFO, "privilege related table schema not ready,schema_version=%ld", schema_version);
            sleep(1);
          }
          else
          {
                ...
                NULL == context.schema_manager_->get_table_schema(OB_ALL_TABLE_PRIVILEGE_TABLE_NAME))
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "release schema manager failed,ret=%d", err);
              }
            ...
            {
              YYSYS_LOG(INFO, "privilege related table schema ready");
              break;
            }
          }
        }
        ...
        ObMsPartitionManager *temp = dynamic_cast<ObMsPartitionManager *>(env_.part_monitor_->get_partition_manager());
        if(NULL == temp)
        {
          ret = OB_ERROR;
        ...
        while (!stop_)
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to open result set, ret=%d", ret);
                // release resouce as early as possible, in case YOU FORGET
                err = result.close();
                if (OB_SUCCESS != err)
                {
                  YYSYS_LOG(WARN, "failed to close result set,ret=%d", err);
                }
              }
              else
              {
                ...
                while (OB_SUCCESS == ret)
                {
                  if (OB_SUCCESS != (ret = result.next_row(row)))
                  {
                    ...
                    {
                      YYSYS_LOG(ERROR, "next row from YaoMySQLResultSet failed, ret=%d", ret);
                    }
                    break;
                  }
                  else if(OB_SUCCESS != (ret = p_privilege->add_users_entry(*(row.get_inner_row()))))
                  {
                    YYSYS_LOG(ERROR, "add privilege row  to system failed,ret=%d", ret);
                  }
                  else
                  {
                    YYSYS_LOG(INFO, "get one user row, row=%s", to_cstring(*(row.get_inner_row())));
                    has_data = true;
                  }
                } // end while
                err = result.close();
                ...
                if (false == has_data)
                {
                  YYSYS_LOG(INFO, "no data in __users,retry");
            ...
            {
              YYSYS_LOG(ERROR,"execute sql %.*s, failed,ret=%d", get_users.length(), get_users.ptr(), ret);
            }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  while (OB_SUCCESS == ret)
                  {
                    ...
                    {
                      // do nothing
                      YYSYS_LOG(INFO, "get one table privilege row, row=%s", to_cstring(*(row.get_inner_row())));
                      has_data = true;
                    }
                  }
                  err = result2.close();
                  ...
                  if (false == has_data)
                  {
                    YYSYS_LOG(INFO, "no data in __all_table_privilege,retry");
              ...
              {
                YYSYS_LOG(ERROR,"execute sql %.*s, failed,ret=%d", get_table_privileges.length(), get_table_privileges.ptr(), ret);
              }
          ...
          if(OB_SUCCESS == ret)
          {
            ...
            else
            {
              ...
              if(OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  while (OB_SUCCESS == ret)
                  {
                    ...
                    {
                      YYSYS_LOG(INFO, "get one database row, row=%s", to_cstring(*(row.get_inner_row())));
                      has_data = true;
                    }
                  }
                  err = result_for_db.close();
                  ...
                  if (false == has_data)
                  {
                    YYSYS_LOG(INFO, "no data in __all_database,retry");
              ...
              {
                YYSYS_LOG(ERROR, "execute sql %.*s, failed, ret=%d", sql_get_database.length(),sql_get_database.ptr(),ret);
              }
              ...
              if(OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  while (OB_SUCCESS == ret)
                  {
                    ...
                    {
                      YYSYS_LOG(INFO, "get one database privilege row, row=%s", to_cstring(*(row.get_inner_row())));
                      has_data = true;
                    }
                  }
                  err = result_for_db_priv.close();
                  ...
                  if (false == has_data)
                  {
                    YYSYS_LOG(INFO, "no data in __all_database_privilege,retry");
            ...
            else
            {
              ...
              if (OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  if (OB_SUCCESS == ret)
                  {
                    ...
                    else
                    {
                      ...
                      while (OB_SUCCESS == ret)
                      {
                        ...
                        {
                          YYSYS_LOG(ERROR, "raw_get_cell failed,ret=%d", ret);
                        }
                        else
                        {
                          in.assign(*pcell);
                          ...
                          {
                            YYSYS_LOG(ERROR, "varchar cast to int failed, ret=%d", ret);
                          }
                          else
                          {
                      ...
                      if (OB_SUCCESS == ret)
                      {
                        YYSYS_LOG(INFO, "privilege related table ready");
                      }
                      break;
                    }
                  }
                  ...
                  {
                    YYSYS_LOG(ERROR,"execute sql %.*s, failed,ret=%d", get_privilege_version.length(), get_privilege_version.ptr(), ret);
                  }
        ...
        //add liu jun.[MultiUPS] [part_cache] 20150626:e
        if(NULL != context.partition_mgr_)
        {
          env_.part_monitor_->release_manager(context.partition_mgr_);
          context.partition_mgr_ = NULL;
        }
        //20150626:e
      }
      return ret;
    ...
    int ObMySQLServer::start(bool need_wait)
    {
      ...
      YYSYS_LOG(DEBUG, "yaomysql server start....");
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "easy_eio_create error");
      }
      else if (OB_SUCCESS != (ret = command_queue_thread_.init()))
      {
        YYSYS_LOG(ERROR, "command_queue_thread.init()=>%d", ret);
      }
      else if (OB_SUCCESS != (ret = close_command_queue_thread_.init()))
      {
        YYSYS_LOG(ERROR, "close_command_queue_thread.init()=>%d", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "yaomysql easy_eio_create successful");
        eio_->tcp_defer_accept = 0;
        ret = initialize();
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "initialize failed ret is %d", ret);
        }
        else
        {
          ...
          {
            YYSYS_LOG(INFO, "yaomysql listen start, port = %d", port_);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            ret = OB_SUCCESS;
            YYSYS_LOG(INFO, "yaomysql start io thread");
          }
          else
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "yaomysq start failed, call easy_eio_start return %d", rc);
          }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            if (NULL == p_privilege)
            {
              ...
              YYSYS_LOG(ERROR, "yao malloc YaoPrivilege failed, ret=%d", ret);
            }
            if (OB_SUCCESS == ret)
            {
              ...
              {
                YYSYS_LOG(ERROR, "init YaoPrivilege failed, ret=%d", ret);
              }
              else if (OB_SUCCESS != (ret = get_privilege(p_privilege)))
              {
                YYSYS_LOG(ERROR, "get privilege failed, ret=%d", ret);
              }
              else if (OB_SUCCESS != (ret = env_.privilege_mgr_->renew_privilege(*p_privilege)))
              {
                ...
                {
                  YYSYS_LOG(INFO, "initialize privilege information success");
                  login_handler_.set_privilege_manager(env_.privilege_mgr_);
                  ret = OB_SUCCESS;
                }
                else
                {
                  YYSYS_LOG(ERROR, "renew privilege failed, ret=%d", ret);
                }
              }
              else
              {
    ...
    void ObMySQLServer::stop()
    {
      ...
      {
        stop_ = true;
        YYSYS_LOG(WARN, "start to stop the mysql server");
        destroy();
        if (eio_ != NULL && NULL != eio_->pool)
        {
          YYSYS_LOG(WARN, "stop eio");
          easy_eio_stop(eio_);
          easy_eio_wait(eio_);
          easy_eio_destroy(eio_);
          eio_ = NULL;
        }
        YYSYS_LOG(INFO, "mysql server stoped.");
      }
    }
    ...
    void ObMySQLServer::destroy()
    {
      if (work_thread_count_ > 0)
      {
        YYSYS_LOG(WARN, "stop command queue thread");
        command_queue_thread_.stop();
        close_command_queue_thread_.stop();
        YYSYS_LOG(WARN, "wait command queue thread");
        wait_for_queue();
      }
      YYSYS_LOG(WARN, "stop timer");
      timer_.destroy();
      YYSYS_LOG(WARN, "stop package recorder");
    ...
    int ObMySQLServer::login_handler(easy_connection_t* c)
    {
      ...
      if (OB_SUCCESS != (ret = login_handler_.login(c, session)))
      {
        if (IS_SQL_ERR(ret))
        {
          YYSYS_LOG(WARN, "handler login failed ret is %d peer is %s", ret, inet_ntoa_r(addr));
        }
        else
        {
          YYSYS_LOG(WARN, "handler login failed ret is %d peer is %s", ret, inet_ntoa_r(addr));
      ...
      else if (PACKET_RECORDER_FLAG)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to record MySQL packet, err=%d", err);
        }
      }
      return ret;
    }
    ...
    int ObMySQLServer::load_system_params(sql::ObSQLSessionInfo& session)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Fail to get user schema. schema_version=%ld", schema_version);
        ret = OB_ERROR;
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "Get next row from YaoMySQLResultSet failed, ret=%d", ret);
        }
        if (result.close() != OB_SUCCESS)
        {
    ...
    int ObMySQLServer::set_io_thread_count(const int32_t io_thread_count)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument io_thread_count = %d", io_thread_count);
    ...
    int ObMySQLServer::set_work_thread_count(const int32_t work_thread_count)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument work_thread_count = %d", work_thread_count);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObMySQLServer::set_task_queue_size(const int32_t task_queue_size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument task_queue_size = %d", task_queue_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObMySQLServer::set_port(const int32_t port)
    {
      ...
      if (port <= 0)
      {
        YYSYS_LOG(ERROR, "invalid argument port = %d", port);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObMySQLServer::submit_session_delete_task(ObMySQLSessionKey key)
    {
      ...
      if (NULL == buffer)
      {
        YYSYS_LOG(ERROR, "failed to alloc mem");
        ret = OB_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "serialize key.fd_ failed, buffer=%p, len=%ld, key=%d, pos=%ld",
                    data, length, key, pos);
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "submit async task to thread queue fail task_queue_size=%s",
                      config_->obmysql_task_queue_size.str());
            ret = OB_ERROR;
          }
          else
    ...
    int ObMySQLServer::handle_packet(ObMySQLCommandPacket* packet)
    {
      ...
      if (NULL == packet)
      {
        YYSYS_LOG(ERROR, "invalid argument packet is %p", packet);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (COM_STMT_CLOSE == code)
        {
          ...
          {
            YYSYS_LOG(WARN, "close command queue thread is busy, size=%d", close_command_queue_thread_.get_queue_size());
          }
          ...
          {
            YYSYS_LOG(ERROR, "can not push packet(ptype is %u) to close command queue", code);
            ret = OB_ERROR;
          }
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "can not push packet(ptype is %u) to command queue", code);
            ret = OB_ERROR;
          }
        }
        if (OB_SUCCESS == ret)
    ...
    int ObMySQLServer::handle_packet_queue(ObMySQLCommandPacket* packet, void* args)
    {
      ...
      if (NULL == packet)
      {
        YYSYS_LOG(ERROR, "invalide argument packet is %p", packet);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (OB_LIKELY(COM_DELETE_SESSION != code))
        {
          ...
          int64_t wait_us = yysys::CTimeUtil::getTime() - packet->get_receive_ts();
        ...
        switch(code)
        {
          ...
          case COM_DELETE_SESSION:
            ...
            break;
          default:
            YYSYS_LOG(WARN, "unsupport command %u", code);
        ...
        if (COM_DELETE_SESSION != code)
        {
          ...
          {
            YYSYS_LOG(INFO, "server statistics: current tid=%ld, thread_counter=%ld, total_counter=%lu",
    ...
    int ObMySQLServer::do_unsupport(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to send error packet to mysql client(%s) ret is %d",
                    inet_ntoa_r(addr), ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "send error packet to mysql client(%s)", inet_ntoa_r(addr));
        }
      }
      return ret;
    }
    ...
    int ObMySQLServer::do_com_delete_session(ObMySQLCommandPacket* packet)
    {
      ...
      {
        YYSYS_LOG(WARN, "decode key from packet buffer failed");
      }
    ...
    int ObMySQLServer::do_com_quit(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "client quit, peer=%s",
    ...
    int ObMySQLServer::do_com_query(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        SET_FORCE_TRACE_LOG(force_trace_log || config_->force_print_sql);
        ...
        YYSYS_LOG(TRACE, "start query: \"%.*s\" real_query_len=%d, peer=%s",
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to init sql env ret is %d", ret);
            }
            else
            {
              YYSYS_LOG(ERROR, "init sql env failed ret is %d", ret);
            }
            if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
            {
              YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
            }
          }
          else
          {
           ...
           // is_iud=0;//add gaojt [Insert_Subquery_Function] [JHOBv0.1] 20141108 /*reset the signal of insert...select... before do the direct_execute*/
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "direct_execute failed, err=%d stmt=\"%.*s\" ret is %d",
            ...
            else
            {
              ...
              /**
               * version range is not serial,maybe major froze occured in ups, retry if not timeout,
               * only for insert,update,delete,replace
               */
              //mod liuzy [MultiUPS] [take_paxos_offline_interface] 20160414:b
              /*Exp: UPS return offline info, we should retry after version refreshed*/
/*del [570]:b
//              if (OB_INVALID_START_VERSION == ret)
              if (OB_INVALID_START_VERSION == ret || OB_CURRENT_PAXOS_GROUP_OFFLINE == ret)
              //mod 20160414:e
              {
                if (result.get_stmt_type() == ObBasicStmt::T_PREPARE)
                {
                  /// @todo for prepare,need be handled extra
                  result.set_errcode(OB_FORCE_TIME_OUT);
                  if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
                  {
                    YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
                  }
...
*/  //del:e
                ...
                {
                  ret = retry_com_query_stmt(packet, q, result, context);
                  YYSYS_LOG(INFO, "retry the plan ret=%d",ret);
                }
              //}
              //add 20151201:e
            }
            ...
            //add liu jun. [MultiUPS] [part_cache] 20150722:b
            if(NULL != context.partition_mgr_)
            {
              err = env_.part_monitor_->release_manager(context.partition_mgr_);
              if(OB_SUCCESS != err)
              {
                YYSYS_LOG(ERROR, "release partition manager failed, ret=%d", err);
              }
              context.partition_mgr_ = NULL;
            }
            //add:e
        ...
        const int64_t elapsed_time = yysys::CTimeUtil::getTime() - start_time;
        //mod [318]

        //add   bizb [slow_query]
        const int64_t threshold = config_->slow_query_threshold.ObConfigIntegralItem::get();
        if(threshold > 0 && elapsed_time > threshold)
        {
          YYSYS_SLOWSQLLOG(INFO, "query threshold:%ld", threshold);
          int32_t truncated_length = std::min(q.length(), 384);
          char buffer[truncated_length+1];
          //snprintf(buffer,truncated_length+1,"%s",q.ptr());
          strncpy(buffer, q.ptr(), sizeof(buffer)-1);
          buffer[sizeof(buffer)-1] = '\0';
          YYSYS_SLOWSQLLOG(ERROR, "stmt='%s  spent:%ldus  database:%s'",buffer, elapsed_time, context.session_info_->get_db_name().ptr());
        }
        //end

        if(config_->force_print_sql && elapsed_time > config_->slow_query_threshold)
        {
            FORCE_PRINT_TRACE_LOG();
        }
        else if (elapsed_time > config_->slow_query_threshold && force_trace_log) // 100ms
        {
          FORCE_PRINT_TRACE_LOG();
        }
        else
    ...
    int ObMySQLServer::do_com_prepare(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "start prepare: \"%.*s\" real_query_len=%d peer is %s",
                  truncated_length, q.ptr(), q.length(), inet_ntoa_r(addr));
        ...
        if (OB_SUCCESS != ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to init sql env ret is %d", ret);
          }
          if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
          {
            YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
        ...
        else
        {
          ...
          //add liu jun. [MultiUPS] [part_cache] 20150813:b
          if(NULL != context.partition_mgr_)
          {
            int err = env_.part_monitor_->release_manager(context.partition_mgr_);
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(ERROR, "release partition manager failed, ret=%d", err);
            }
               ...
               (result.get_field_cnt() > OB_MAX_PS_FIELD_COUNT || result.get_param_cnt() > OB_MAX_PS_PARAM_COUNT)))
          {
            if (OB_SUCCESS == ret)
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "failed to close prepare statement stmt_id=%lu", result.get_statement_id());
              }
            }
            YYSYS_LOG(WARN, "stmt_prepare failed, err=%d stmt=\"%.*s\" ret is %d retcode in result is %d",
                      ret, q.length(), q.ptr(), ret, result.get_errcode());
            if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
            {
              YYSYS_LOG(WARN, "fail to to send error packet. err=%d", err);
            }
          }
          else
          {
        ...
        PRINT_TRACE_LOG();
        YYSYS_LOG(DEBUG, "end do_com_prepare");
      }
      if (OB_SUCCESS == ret)
      {
    ...
    int ObMySQLServer::do_com_execute(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "Get param or param_type error, param: %p param_type: %p",
                param, param_type);
            ret = OB_ERROR;
            if (NULL != context.schema_manager_)
            {
          ...
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              ...
              //add:20150813e
              YYSYS_LOG(ERROR, "parse param from COM_STMT_EXECUTE failed");
              if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
              {
                YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
              }
            ...
            else
            {
              ...
              if (using_query_cache)
              {
                ...
                else
                {
                  ...
                  if (OB_ENTRY_NOT_EXIST != err)
                  {
                    YYSYS_LOG(WARN, "access_cache error, err: %d", err);
                    if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
                    {
                      YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
                    }
              ...
              if (!using_query_cache || OB_ENTRY_NOT_EXIST == err)
              {
                ...
                if (OB_SUCCESS != ret)
                {
                  YYSYS_LOG(WARN, "do_com_execute failed, err=%d stmt=%u", ret, stmt_id);
                  if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
                  {
                    YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
                  }
                ...
                else
                {
                  ...
                  if (PACKET_RECORDER_FLAG)//record ps packet with stmt_id
                  {
                    if (OB_SUCCESS == check_req(req))
                    {
                      ...
                      if (OB_SUCCESS == ps_store_.get_query(result.get_sql_id(), query_string))
                      {
                        ...
                        if (NULL == newbuf)
                        {
                          YYSYS_LOG(WARN, "can not alloc mem for new command str");
                        }
                        else
                        {
                      ...
                      {
                        YYSYS_LOG(WARN, "failed to get query, sql_id=%ld", result.get_sql_id());
                      }
                    }
                  }
                  ...
                  if (OB_SUCCESS == ret)
                  {
                    ...
                    if (OB_SUCCESS != err)
                    {
                      YYSYS_LOG(ERROR, "fill_cache error, err: %d", err);
                    }
                  }
                }
              }
      ...
      if(threshold > 0 && elapsed_time > threshold)
      {
        YYSYS_SLOWSQLLOG(INFO, "execute threshold:%ld", threshold);
        ObString q;
        if (OB_SUCCESS == ps_store_.get_query(result.get_sql_id(), q))
        {
              int32_t truncated_length = std::min(q.length(), 384);
              char buffer[truncated_length+1];
              //snprintf(buffer,truncated_length+1,"%s",q.ptr());
              strncpy(buffer, q.ptr(), sizeof(buffer)-1);
              buffer[sizeof(buffer)-1] = '\0';
              YYSYS_SLOWSQLLOG(ERROR, "stmt='%s  spent:%ldus'",buffer, elapsed_time);
        }
        else
        {
          YYSYS_SLOWSQLLOG(ERROR, "stmt='failed to get query, sql_id=%ld spent:%ldus'", result.get_sql_id(), elapsed_time);
        }
      }
      //end
      if (oceanbase::common::TraceLog::get_log_level() <= YYSYS_LOGGER._level
      ...
      YYSYS_LOG(DEBUG, "end do_com_execute");
      return ret;
    }
    ...
    int ObMySQLServer::do_com_close_stmt(ObMySQLCommandPacket* packet)
    {
      ...
      int64_t schema_version = 0;
      uint32_t stmt_id = 0;
      YYSYS_LOG(TRACE, "start do_com_close_stmt");
      ret = check_param(packet);
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "sql engine close statement(id is %u) faild ret is %d", stmt_id, ret);
            // No response is sent back to the client.
          }
          else
          {
      ...
      PRINT_TRACE_LOG();
      YYSYS_LOG(DEBUG, "end do_com_stmt_close");
    ...
    int ObMySQLServer::do_com_ping(ObMySQLCommandPacket* packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(DEBUG, "start do_com_ping peer=%s", inet_ntoa_r(addr));
        ret = send_ok_packet(packet, &result, 0);
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "send ok packet to client(%s) failed ret is %d",
                    inet_ntoa_r(addr), ret);
        }
      }
      PRINT_TRACE_LOG();
    ...
    int ObMySQLServer::post_packet(easy_request_t* req, ObMySQLPacket* packet, uint8_t seq)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument req is %p, packet is %p",
                  req, packet);
        ret = OB_INVALID_ARGUMENT;
      }
      else if((ret = check_request_cleanup(req, __FUNCTION__)) == OB_SUCCESS)
      {
        ObDataBuffer buffer;
        int32_t pkt_len = 0;
        int64_t len_pos = 0;
        ...
        if (NULL != buf)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "serialize packet failed packet is %p ret is %d", packet, ret);
          }
          else
          {
            len_pos = 0;
            ...
            //hex_dump(buf->pos,  static_cast<int32_t>(buf->last - buf->pos), true, YYSYS_LOG_LEVEL_INFO);
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "alloc buffer from req->ms->pool failed");
          ret = OB_ERROR;
        }
        easy_request_wakeup(req);
      }
      ...
      {
          YYSYS_LOG(WARN, "req(%p) post packet failed ret is %d.", req, ret);
      }
      return ret;
    }
                                     ...
                                     MYSQL_PROTOCOL_TYPE type, sql::ObSQLSessionInfo *session)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument packet is %p, result is %p, req is %p",
      ...
      else
      {
        ...
        if (OB_SUCCESS != (ret = result->open()))
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to open result set, err=%d, msg=%.*s", ret, err_msg.length(), err_msg.ptr());
          ...
          //mod liuzy [MultiUPS] [take_paxos_offline_interface] 20160414:b
          /*Exp: UPS return offline info, we should retry after version refreshed*/
          //mod [570] //if ((OB_INVALID_START_VERSION != ret && OB_CURRENT_PAXOS_GROUP_OFFLINE != ret)
          if((OB_CHECK_VERSION_RETRY != ret) && (OB_SUCCESS != (err = send_error_packet(packet, result))))
//          //mod lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20151201:b
//          //if (OB_SUCCESS != (err = send_error_packet(packet, result)))
//          if ((OB_INVALID_START_VERSION != ret) && (OB_SUCCESS != (err = send_error_packet(packet, result))))
//          //mod 20151201:e
          //mod 20160414:e
          {
            YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
          ...
          {
            YYSYS_LOG(WARN, "close result set failed");
          }
        }
        else
        {
          ...
          if (select)
          {
            ...
            if (OB_SUCCESS != ret && OB_CONNECT_ERROR != ret)
            {
              if (OB_SUCCESS != (err = send_error_packet(packet, result)))
              {
                YYSYS_LOG(ERROR, "send error packet to client(%s), failed,ret=%d", inet_ntoa_r(addr), err);
              }
          ...
          {
            YYSYS_LOG(ERROR, "close result set failed ret is %d", ret2);
            if (OB_SUCCESS == ret)
            {
              ret = ret2;
            }
                                    ...
                                    int64_t &schema_version, ObMySQLResultSet &result)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed get session key is %d ret is %d", key, ret);
      }
      else
      {
        info->mutex_lock(); // !!!lock
        YYSYS_LOG(DEBUG, "get session, addr=%p key=%d info=%s",
        ...
        //add liu jun. [MultiUPS] [part_cache] 20150525:b
        ObMsPartitionManager *part_mgr = NULL;
        context.part_monitor_ = env_.part_monitor_;
        if(NULL == (part_mgr = dynamic_cast<ObMsPartitionManager *>(env_.part_monitor_->get_partition_manager())))
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "dynamic cast to ms partition manager failed");
        }
        else
        {
          context.partition_mgr_ = part_mgr;
        }
        //20150525:e
        ...
        else
        {
          ...
          context.session_info_->get_parser_mem_pool().reuse();
          context.session_info_->get_transformer_mem_pool().start_batch_alloc();
          context.session_info_->set_query_start_time(yysys::CTimeUtil::getTime());
          context.session_info_->reset_retry_info();
          ...
          {
            YYSYS_LOG(ERROR, "store query string failed ret=%d", ret);
          }
        }
        //context.session_info_->get_user_name().length(), context.session_info_->get_user_name().ptr(),
        //context.session_info_->get_db_name().length(), context.session_info_->get_db_name().ptr(),
        //context.session_info_->get_peer_addr(), context.session_info_->get_session_id()
        //mod [543]
        //FILL_TRACE_LOG("session_id=%d", context.session_info_->get_session_id());
        FILL_TRACE_LOG("user_name=%.*s db_name=%.*s client=%s session_id=%d",
                       context.session_info_->get_user_name().length(), context.session_info_->get_user_name().ptr(),
                       context.session_info_->get_db_name().length(), context.session_info_->get_db_name().ptr(),
                       context.session_info_->get_peer_addr(), context.session_info_->get_session_id());
        ...
        if (ret != OB_SUCCESS)
        {
          ...
          if (NULL != context.schema_manager_)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(ERROR, "release schema failed, ret=%d", err);
            }
            context.schema_manager_ = NULL;
          }
          //add liu jun. [MultiUPS] [part_cache] 20150626:b
    ...
    void ObMySQLServer::cleanup_sql_env(ObSqlContext &context, ObMySQLResultSet &result, bool unlock)
    {
      ...
      YYSYS_LOG(DEBUG, "stack allocator reuse_mem=%c", reuse_mem?'Y':'N');
#ifdef __OB_MTRACE__
    ...
    int ObMySQLServer::send_error_packet(ObMySQLCommandPacket* packet, ObMySQLResultSet *result)
    {
      ...
      {
        YYSYS_LOG(ERROR, "should never reach here invalid argument packet is %p, req is %p",
      ...
      else
      {
        ...
        if (message.length() <= 0)
        {
          if (NULL != result && 0 < strlen(result->get_message()))
          {
              ...
              snprintf(msg_buf, 128, "YAO-%d: %s", -result->get_errcode(), result->get_message()); // add szj [Modify Welcome Info]
          ...
          else
          {
              ...
              snprintf(msg_buf, 128, "YAO-%d: %s", -result->get_errcode(), ob_strerror(result->get_errcode())); // add szj [Modify Welcome Info]
              //mod:e
        ...
        {
            snprintf(msg_buf, 128, "YAO-%d: %.*s", -result->get_errcode(), message.length(), message.ptr()); // add szj [Modify Welcome Info]
            message = ObString::make_string(msg_buf); // default error message
        }
        //add:e
            ...
            && OB_SUCCESS == (ret = epacket.set_message(message)))
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to send error packet to mysql client(%s) ret is %d", inet_ntoa_r(addr), ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "send error packet(seq=%u) to mysql client(%s) succ,", number, inet_ntoa_r(addr));
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "set member of MySQL error packet failed");
        }
      }
      return ret;
    }
    ...
    int ObMySQLServer::send_spr_packet(ObMySQLCommandPacket* packet, ObMySQLResultSet *result)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "send spr packet to mysql client(%s) succ", inet_ntoa_r(addr));
        }
      }
      return ret;
    }
    ...
    int ObMySQLServer::send_ok_packet(ObMySQLCommandPacket* packet, ObMySQLResultSet *result, uint16_t server_status)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "post ok packet to client(%s) failed ret is %d",
                    inet_ntoa_r(addr), ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "send result for update/insert/delete to client(%s) succ",
                    inet_ntoa_r(addr));
        }
      }
      return ret;
                                                  ...
                                                  ObSQLSessionInfo *session)
    {
      ...
      if (NULL == packet || NULL == result || NULL == packet->get_request() || NULL == session)
      {
        YYSYS_LOG(ERROR, "should not reache here, invalid argument"
                  "packet is %p, result is %p, req is %p", packet, result, packet->get_request());
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        YYSYS_LOG(TRACE, "query result=(%s)", to_cstring(*result));
        easy_request_t *req = packet->get_request();
        easy_addr_t addr = get_easy_addr(req);
        ...
        if (result->get_field_cnt() > 0 || result->get_param_cnt() > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "send stmt prepare result set to client %s failed ret is %d", inet_ntoa_r(addr), ret);
          }
        }
    ...
    int ObMySQLServer::parse_execute_params(ObSqlContext *context, ObMySQLCommandPacket *packet, ObIArray<ObObj>& params, uint32_t &stmt_id, ObIArray<EMySQLFieldType>& params_type)
    {
      ...
      if (NULL == packet)
      {
        YYSYS_LOG(ERROR, "should not reach here");
      }
      else
      {
        bool nv = false;
        ...
        //hex_dump(payload, length, true, YYSYS_LOG_LEVEL_INFO);
        int8_t bound_flag = 0;
        uint8_t type = 0;
        int8_t flag = 0;
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "can not find YaoSQLSessionInfo for stmt_id = %u", stmt_id);
        }
        else
        {
          param_count = sinfo->params_.count();
          const common::ObIArray<EMySQLFieldType> &types = sinfo->params_type_;
          YYSYS_LOG(DEBUG, "ps expected param_count=%ld, data_len=%d remain=%ld type_count=%ld",
          ...
          if (param_count > 0 && payload - start < length)
          {
            ...
            YYSYS_LOG(DEBUG, "bitmap_bytes=%ld bound_flag=%hhd bitmap0=%hhu",
              ...
              for (int32_t index = 0; index < param_count && OB_SUCCESS == ret; ++index)
              {
                ...
                else
                {
                  if (param_count != types.count())
                  {
                    ...
                    YYSYS_LOG(USER_ERROR, "Incorrect arguments number to EXECUTE, need %ld arguments but give %ld",
                ...
                {
                  YYSYS_LOG(WARN, "cast ob type from MySQL type failed MySQL type is %d", type);
                }
                else
                {
                  ...
                  if (false == nv)
                  {
                    ...
                    if (OB_SUCCESS != ret)
                    {
                      YYSYS_LOG(WARN, "get param value failed, type=%u i=%d ret is %d", type, index, ret);
                    }
                    else
                    {
                      YYSYS_LOG(DEBUG, "execute with param i=%d type=%d param=%s",
                                index, type, to_cstring(param));
                    }
                  }
                  else
                  {
                    YYSYS_LOG(DEBUG, "execute with param i=%d param=NULL", index);
                  }
...
//        param.set_int(value);
      ...
      case MYSQL_TYPE_TIMESTAMP:
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "parse timestamp value from client failed ret is %d", ret);
        }
        break;
      ...
      case MYSQL_TYPE_BLOB:
      {
        ...
        {
          YYSYS_LOG(ERROR, "decode varchar param value failed ret is %d", ret);
        }
        break;
      }
      default:
        YYSYS_LOG(USER_ERROR, "unsupport MySQL type %d. You should use java.sql.Timestamp instead of java.util.Date.", type);
        ret = OB_ERROR;
        break;
      }
      return ret;
    ...
    int ObMySQLServer::get_mysql_timestamp_value(char *&data, ObObj &param)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "invalid mysql timestamp value length %d", length);
      }
      ...
      YYSYS_LOG(DEBUG, "length is %d, time is %d-%d-%d %d:%d:%d %d value is %ld",
                                       ...
                                       MYSQL_PROTOCOL_TYPE type, uint16_t server_status, uint16_t charset)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument argument req is %p, result is %p", req, result);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (NULL != buf)
        {
          ...
          {
            YYSYS_LOG(WARN, "process resheasder packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
          }
          else if (OB_SUCCESS != (ret = process_field_packets(buf, buffer_pos, req, result, true, charset)))
          {
            YYSYS_LOG(WARN, "process field packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
          }
          else if (OB_SUCCESS != (ret = process_eof_packet(buf, buffer_pos, req, result, server_status)))
          {
            YYSYS_LOG(WARN, "process field eof packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
          }
          else if (OB_SUCCESS != (ret = process_row_packets(buf, buffer_pos,req, result, type)))
          {
            YYSYS_LOG(WARN, "process row packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
          }
          else if (OB_SUCCESS != (ret = process_eof_packet(buf, buffer_pos, req, result, server_status)))
          {
            YYSYS_LOG(WARN, "process row eof packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
          }
          if (OB_SUCCESS == ret)
          {
            YYSYS_LOG(DEBUG, "send result set to client %s", inet_ntoa_r(addr));
            buf->last = buf->pos + buffer_pos;
            req->opacket = reinterpret_cast<void*>(buf);
            ret = post_raw_packet(req);
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "post packet to client(%s) failed ret is %d", inet_ntoa_r(addr), ret);
            }
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "alloc buffer from req->yaos->pool failed");
          ret = OB_ERROR;
        }
      }
      return ret;
                                                    ...
                                                    uint16_t server_status, uint16_t charset)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument req is %p, result is %p", packet, result);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (NULL != buf && NULL != req)
        {
          ...
          if (result->get_field_cnt() > 0 || result->get_param_cnt() > 0)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              YYSYS_LOG(DEBUG, "send params%%fields field count is %ld, param count is %ld",
              ...
              if (result->get_param_cnt() > 0)
              {
                ...
                {
                  YYSYS_LOG(WARN, "process param packets failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
                }
                else if (OB_SUCCESS != (ret = process_eof_packet(buf, buffer_pos, req, result, server_status)))
                {
                  YYSYS_LOG(WARN, "process param eof failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
                }
              }
              ...
              if (OB_SUCCESS == ret && result->get_field_cnt() > 0)
              {
                ...
                {
                  YYSYS_LOG(WARN, "send field packet to client(%s) failed ret is %d", inet_ntoa_r(addr), ret);
                }
                else if (OB_SUCCESS != (ret = process_eof_packet(buf, buffer_pos, req, result, server_status)))
                {
                  YYSYS_LOG(WARN, "send field eof packet to client(%s) failed ret is %d", inet_ntoa_r(addr), ret);
                }
              }
            ...
            {
              YYSYS_LOG(ERROR, "process spr packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
            }
        ...
        {
          YYSYS_LOG(ERROR, "alloc buffer from req->yaos->pool failed request is %p", req);
          ret = OB_ERROR;
        }
      }
      return ret;
                                          ...
                                          easy_request_t *req, ObMySQLResultSet *result)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument buff is %p, req is %p, result is %p",
                  buff, req, result);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "process resheader packet failed ret is %d", ret);
        }
      }
      return ret;
    }
                              ...
                              easy_request_t *req, ObMySQLResultSet *result, bool is_field, uint16_t charset)
    {
      ...
      else
      {
               ...
               OB_SUCCESS == (ret = (result->*get_next)(field)))
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "process field packet dest is %s failed ret is %d",
                      inet_ntoa_r(addr), ret);
          }
        }
        if (OB_ITER_END == ret)
        ...
        {
          YYSYS_LOG(WARN, "libeasy connection error");
        }
        else
        {
          YYSYS_LOG(WARN, "failed to get next field, err=%d", ret);
          buff->last = buff->pos + buff_pos;
          req->opacket = reinterpret_cast<void*>(buff);
          int sret = send_raw_packet(req);
          if (OB_SUCCESS != sret)
          {
            YYSYS_LOG(ERROR, "send raw packet(dest is %s) failed ret is %d", inet_ntoa_r(addr), sret);
                                          ...
                                          uint16_t server_status)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "process eof packet failed dest is %s ret is %d", inet_ntoa_r(addr), ret);
        }
      }
      return ret;
    }
                                           ...
                                           easy_request_t *req, ObMySQLResultSet *result, MYSQL_PROTOCOL_TYPE type)
    {
      ...
      else
      {
               ...
               && OB_SUCCESS == (ret = result->next_row(row)))
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "process row packet failed, dest is %s ret is %d",
                      inet_ntoa_r(addr), ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "process row succ, dest is %s, seq=%d row_num=%ld",
        ...
        if (OB_ITER_END != ret)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "Query killed");
              result->set_message("Query execution was interrupted");
            }
            else if (ret == OB_ERR_SESSION_INTERRUPTED)
            {
              YYSYS_LOG(WARN, "Session killed peer is %s", get_peer_ip(req));
              result->set_message("Query execution was interrupted");
            }
            YYSYS_LOG(WARN, "failed to get next row, err=%d", ret);
          }
        }
        else
        {
                                             ...
                                             easy_request_t *req, ObMySQLPacket *packet)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument buff is %p, packet is %p", buff, packet);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if (OB_BUF_NOT_ENOUGH == ret || OB_ARRAY_OUT_OF_RANGE == ret || OB_SIZE_OVERFLOW == ret) //buff not enough to hold this packet
        {
          ...
          if (OB_SUCCESS == sret)
          {
            ...
            {
              YYSYS_LOG(ERROR, "do not support packet larger than %ld ret is %d", OB_MAX_PACKET_LENGTH, ret);
            }
          }
        }
    ...
    int ObMySQLServer::check_request_cleanup(easy_request_t *req, const char *fn)
    {
        int ret = OB_SUCCESS;
        ObMySQLCommandPacket *packet = (ObMySQLCommandPacket *)req->ipacket;
        if(packet == NULL || packet->is_cleanup() || req->ms->c->fd == -1)
        {
            YYSYS_LOG(ERROR, "call by(%s) request (%p) packet(%p) had cleanup.", fn, req, packet);
    ...
    int ObMySQLServer::send_raw_packet(easy_request_t *req)
    {
      ...
      if (NULL == req)
      {
        YYSYS_LOG(ERROR, "invalid argument req is %p", req);
        ret = OB_INVALID_ARGUMENT;
      }
      else if((ret = check_request_cleanup(req, __FUNCTION__)) == OB_SUCCESS)
      {
        ...
        req->waiting = 1;
        //send_response->easy_connection_request_done
        easy_request_wakeup(req);
        ...
        {
          YYSYS_LOG(WARN, "send error happen, quit current query");
      ...
      {
          YYSYS_LOG(WARN, "req(%p) send packet failed ret is %d", req, ret);
      }
      return ret;
    }
    ...
    int ObMySQLServer::post_raw_packet(easy_request_t *req)
    {
      ...
      else
      {
        ...
        if((ret = check_request_cleanup(req, __FUNCTION__)) == OB_SUCCESS)
        {
            req->retcode = EASY_OK;
    ...
    int ObMySQLServer::post_raw_bytes(easy_request_t * req, const char * bytes, int64_t len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid arguments, req: %p bytes: %p len: %ld", req, bytes, len);
        ret = OB_INVALID_ARGUMENT;
      }
      else if((ret = check_request_cleanup(req, __FUNCTION__)) == OB_SUCCESS)
      {
        ...
        if (NULL == buf)
        {
          YYSYS_LOG(ERROR, "easy_pool_alloc failed");
          ret = OB_ERROR;
        }
        else
        {
        ...
        int64_t consumed_time)
    {
      ...
      static int64_t real_start = yysys::CTimeUtil::getTime();
      ...
      {
        // log QPS/TPC
        int64_t real_end = yysys::CTimeUtil::getTime();
        ...
        YYSYS_LOG(INFO, "[QPS] qps=%ld tps=%ld rt=%ld select_count=%ld "
        ...
        ObSqlContext & context, const ObIArray<ObObj> & param)
    {
      ...
      if(NULL == (stored_result = context.session_info_->get_plan(stmt_id)))
      {
        ...
        YYSYS_LOG(USER_ERROR, "statement not prepared, stmt_id=%u", stmt_id);
      }
      ...
      else if (packet == NULL || packet->is_cleanup())
      {
          YYSYS_LOG(WARN, "request(%p) packet(%p) had cleanup.", packet->get_request(), packet);
          ret = OB_LIBEASY_ERROR;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "prepare_ongoing_res error, ret: %d", ret);
          query_cache_.stop_ongoing_res();
        }
        else if (OB_SUCCESS == ret)
        {
          ...
          if (NULL == req)
          {
            YYSYS_LOG(ERROR, "get_request failed");
            ret = OB_ERROR;
          }
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "post_raw_bytes error, ret: %d", ret);
            }
          }
          err = query_cache_.revert(cache_res);
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "YaoSQLQueryCache revert error, err: %d", err);
          }
        }
      }
      return ret;
    ...
    int ObMySQLServer::fill_cache()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "finish_ongoing_res error, ret: %d", ret);
      }
      return ret;
    }
    ...
    int ObMySQLServer::store_query_string(const ObMySQLCommandPacket& packet, ObSqlContext &context)
    {
      ...
      {
        if (COM_QUERY == code || COM_STMT_PREPARE == code)
        {
    ...
    int ObMySQLServer::trans_exe_command_with_type(ObSqlContext* context, ObMySQLCommandPacket* packet, char* buf, int32_t &len, bool& trans_flag)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument context=%p, packet=%p, should not reach here", context, packet);
      }
      else
      {
        trans_flag = false;
          ...
          if (param_count > 0 && payload - start < length)
          {
            ...
            if (payload -start < length) //if has param
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(WARN, "buf is not enough  buflen=%d, need%ld", len, length+param_count*2);
                }
              }
            }
          }
    ...
    //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20151201:b
    int ObMySQLServer::retry_com_query_stmt(ObMySQLCommandPacket *packet, const common::ObString& stmt,
                                            ObMySQLResultSet& result, ObSqlContext &context)
    {
      int ret = OB_SUCCESS;
      int err = OB_SUCCESS;
      int64_t retry_start_time = 0;
      //int64_t retry_end_time = 0;
      int retry_times = 6;
      unsigned int retry_wait_time = (unsigned int)get_config()->retry_sql_wait_time; //[20, 500]ms  [570]
      if (OB_SUCCESS != (ret = check_param(packet)))
      {
        YYSYS_LOG(WARN, "check param failed,ret=%d",ret);
      }
      else
      {
        YYSYS_LOG(DEBUG,"retry plan");
        ObPhysicalPlan * physical_plan = NULL;
        int64_t old_version = OB_INVALID_VERSION;
        int64_t cur_version = OB_INVALID_VERSION;
        if (NULL == (physical_plan = result.get_physical_plan()))
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "get physical plan failed,ret=%d",ret);
        }
        else
        {
          old_version = physical_plan->get_curr_frozen_version();
          while (OB_SUCCESS == ret && retry_times)
          {
            usleep(retry_wait_time*1000);//sleep 5ms for refresh the frozen version
            cur_version = context.session_info_->get_frozen_version();
        ...
        retry_start_time = yysys::CTimeUtil::getTime();
        YYSYS_LOG(DEBUG, "retry time out is[%ld]",context.session_info_->get_retry_plan_timeout());

        //do retry
        if (OB_SUCCESS == ret)
        {
            if(context.session_info_->get_retry_plan_timeout() > retry_start_time)
            {
                result.reset();
                YYSYS_LOG(DEBUG,"retry once");
                ret = ObSql::direct_execute(stmt, result, context);
                FILL_TRACE_LOG("direct_execute");
                if (OB_SUCCESS != ret)
                {
                    YYSYS_LOG(WARN, "direct_execute failed, err=%d stmt=\"%.*s\" ret is %d",
                              ret, stmt.length(), stmt.ptr(), ret);
                    if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
                    {
                        YYSYS_LOG(WARN, "fail to send error packet. err=%d", err);
                    }
                ...
                {
                    ret = send_response(packet, &result, TEXT, context.session_info_);
                    //retry_end_time = yysys::CTimeUtil::getTime();
                    //mod liuzy [MultiUPS] [take_paxos_offline_interface] 20160414:b
                    //            if (OB_INVALID_START_VERSION == ret && context.session_info_->get_retry_plan_timeout() < retry_end_time)
                    //if ((OB_INVALID_START_VERSION == ret || OB_CURRENT_PAXOS_GROUP_OFFLINE) && context.session_info_->get_retry_plan_timeout() < retry_end_time)
                    if(OB_CHECK_VERSION_RETRY == ret)
                        //mod 20160414:e
                    {
                        YYSYS_LOG(DEBUG, "no enough time to retry");
                        ret = OB_RESPONSE_TIME_OUT;
                        result.set_errcode(ret);
                        if (OB_SUCCESS != (err = send_error_packet(packet, &result)))
                        {
                    ...
                    YYSYS_LOG(DEBUG,"tiemout is[%ld]",context.session_info_->get_retry_plan_timeout());
                }
            }//end if
            else
            {
        ...
        context.session_info_->reset_retry_info();//do not forget reset after using the retry info
      }
      return ret;
    }//end
  }
