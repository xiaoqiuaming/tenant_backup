  ...
  namespace obmysql
  {
    ...
    class ObMySQLServer : public ObMySQLPacketQueueHandler
    {
      ...
      public:
        ...
        const mergeserver::ObMergeServerConfig *get_config() const
        {return config_;};
        const MergeServerEnv& get_env() const
        {return env_;};
        ...
        SessionPool& get_session_pool()
        {return session_pool_;};
        ObPsStore* get_ps_store()
        {
          return &ps_store_;
        }
        ObFIFOStream& get_packet_recorder()
        {return packet_recorder_;};
      private:
        int initialize();
      ...
      private:
        ...
        inline int check_param(ObMySQLCommandPacket *packet)
        {
          ...
          if (NULL == packet)
          {
            YYSYS_LOG(ERROR, "should not reach here invalid argument packet is %p", packet);
            ret = OB_INVALID_ARGUMENT;
          }
          else if (NULL == packet->get_request())
          {
            YYSYS_LOG(ERROR, "should not reach here invalid argument request is %p", packet->get_request());
            ret = OB_INVALID_ARGUMENT;
          }
          return ret;
        }
        ...
        void on_ioth_start()
        {
          ...
          SET_THD_NAME("mssql-io");
        }

        //add lijianqiang [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20151201:b
        int retry_com_query_stmt(ObMySQLCommandPacket *packet, const common::ObString& stmt,
                                 ObMySQLResultSet& result, ObSqlContext &context);
        //add 20151201:e
        int check_request_cleanup(easy_request_t* req, const char* fn);
