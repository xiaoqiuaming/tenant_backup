...
#include "packet/ob_mysql_switchauth_packet.h"
...
ObMySQLLoginer::LoginInfo::LoginInfo()
  :capability_(0), max_packet_size_(0), character_set_(0), user_name_(), db_name_(), auth_response_(), plugin_name_()//[736]
{
  db_name_.reset();
}
...
int ObMySQLLoginer::login(easy_connection_t* c, sql::ObSQLSessionInfo *& session)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "send hand shake packet failed");
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "parse client auth packet failed");
      }
      else
      {
        //[mysql8]
        if((login_info.plugin_name_.compare(OB_DEFAULT_AUTH_PLUGIN) != 0) && (login_info.plugin_name_.compare("") != 0))
        {
          if(OB_SUCCESS != (ret = switch_auth(c)))
          {
            YYSYS_LOG(WARN, "send switch auth packet failed");
          }
          else if(OB_SUCCESS != (ret = update_password(c, login_info)))
          {
            YYSYS_LOG(WARN, "update password failed");
          }
          else if(OB_SUCCESS != (ret = check_privilege(c, session, login_info, 4)))
          {
            YYSYS_LOG(WARN, "check privilege failed");
          }
        }
        //[736]
        else
        {
          ret = check_privilege(c, session, login_info, 2);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "login failed, err=%d", ret);
          }
        }
      }
    }
...
int ObMySQLLoginer::handshake(easy_connection_t* c)
{
  ...
  if (NULL == c)
  {
    YYSYS_LOG(ERROR, "invalide argument c is %p", c);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get thread buffer error, ignore");
      ret = OB_ERROR;
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "ObMySQLHandshakPacket serialize failed, buffer is %p, buffer len is %ld,"
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write packet data to client failed fd is %d, buffer is %p, length is %ld",
                    c->fd, out_buffer.get_data(), out_buffer.get_position());
          ret = OB_ERR_WRITE_AUTH_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "new client login, peer=%s, c->seq[%d]",
                    inet_ntoa_r(c->addr), c->seq);//[701]
        }
      }
    }
  }
...
int ObMySQLLoginer::parse_packet(easy_connection_t* c, LoginInfo& login_info)
{
  ...
  if (NULL == c)
  {
    YYSYS_LOG(ERROR, "invalid argument c is %p", c);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if (NULL == thread_buffer)
    {
      YYSYS_LOG(ERROR, "get thread buffer failed");
      ret = OB_ERROR;
    }
    else
    {
      ...
      YYSYS_LOG(DEBUG, "start read from %s %d bytes", inet_ntoa_r(c->addr),
                read_size);
      ret = read_data(c->fd, in_buffer.get_data(), read_size);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "read packet header failed");
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "start read from %s %u bytes", inet_ntoa_r(c->addr),
                  packet_len);
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "read packet data failed, ret=%d", ret);
          ret = OB_ERR_PROTOCOL_NOT_RECOGNIZE;
        }
        else
        {
          YYSYS_LOG(DEBUG, "readed from %s %u bytes", inet_ntoa_r(c->addr),
                    packet_len);
          ...
          //[mysql8]
          if(login_info.capability_.CapabilityFlags.CLIENT_CONNECT_WITH_DB)
          {
            db_len = static_cast<int32_t>(strlen(len_pos));
            login_info.db_name_.assign_ptr(len_pos, db_len);
            len_pos += db_len + 1;
          }
          if(login_info.capability_.CapabilityFlags.CLIENT_PLUGIN_AUTH)
          {
            plugin_name_len = static_cast<int32_t>(strlen(len_pos));
            login_info.plugin_name_.assign_ptr(len_pos, plugin_name_len);
            len_pos += plugin_name_len;
          }
          thread_buffer->advance(read_size + packet_len);
        }
      }
    }
  }
...
int ObMySQLLoginer::insert_new_session(easy_connection_t* c, sql::ObSQLSessionInfo *&session)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "set session peer addr failed");
  }
  else
  {
    //not over write
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "insert new session failed ret is %d, key is %d", ret, c->seq);
    }
    else
    {
      int64_t session_num = server_->get_session_mgr()->get_session_count();
      YYSYS_LOG(INFO, "[LOGIN] new session, session_key=%d session=%s username=%.*s total_sessions_num=%ld",
                                    ...
                                    )
{
  ...
  {
    //YYSYS_LOG(WARN, "before login");
    //ob_print_mod_memory_usage();
    if (server_->has_too_many_sessions())
    {
      YYSYS_LOG(WARN, "there are too many sessions, refuse this client");
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "set message failed, ret=%d", err);
      }
      packet = &err_packet;
    }
    else if (NULL == privilege_mgr_)
    {
      ...
      YYSYS_LOG(WARN, "privilege manager not set, equals null");
      err_packet.set_oberrcode(ret);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "get privilege failed, ret=%d", ret);
      ...
      {
        YYSYS_LOG(WARN, "user '%.*s' login failed,ret=%d", login_info.user_name_.length(), login_info.user_name_.ptr(), ret);
        err_packet.set_oberrcode(ret);
        ObString message;
      ...
      {
        YYSYS_LOG(WARN, "user [%.*s] has no access to db [%.*s]",login_info.user_name_.length(), login_info.user_name_.ptr(),
      ...
      else if (NULL == (session = server_->get_session_pool().alloc()))
      {
        ...
        YYSYS_LOG(ERROR, "ob malloc failed, ret=%d", OB_ALLOCATE_MEMORY_FAILED);
        packet = &err_packet;
      }
      else
      {
        ...
        {
          packet = &err_packet;
          YYSYS_LOG(WARN, "init session info failed, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret =session->set_ps_store(server_->get_ps_store())))
        {
          YYSYS_LOG(WARN, "set ps store to session failed, ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = session->set_username(login_info.user_name_)))
        {
          YYSYS_LOG(WARN, "add username %.*s to session failed, ret=%d", login_info.user_name_.length(), login_info.user_name_.ptr(), ret);
          err_packet.set_oberrcode(ret);
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "add dbname %.*s to session failed, ret=%d", db_name.length(), db_name.ptr(), ret);
            err_packet.set_oberrcode(ret);
            ObString message = ObString::make_string("internal server error");
            int err = err_packet.set_message(message);
            if (OB_SUCCESS != err)
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to load system params, ret=%d", ret);
            err_packet.set_oberrcode(ret);
          ...
          {
            YYSYS_LOG(ERROR, "failed to insert new session, ret=%d, key is %d", ret, c->seq);
            err_packet.set_oberrcode(ret);
            ObString message = ObString::make_string("login error, internal server error");
            int err = err_packet.set_message(message);
            if (OB_SUCCESS != err)
    ...
    if (NULL != thread_buffer)
    {
      ...
      {
        YYSYS_LOG(ERROR, "serialize packet failed, err=%d", send_err);
      }
      else
      {
        ...
        if (OB_SUCCESS != send_err)
        {
          YYSYS_LOG(WARN, "write packet to mysql client failed, fd=%d, buffer=%p,"
                    "length=%ld, err=%d", c->fd, out_buffer.get_data(), out_buffer.get_position(), send_err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "send packet to %s bytes", inet_ntoa_r(c->addr));
        }
      }
    }
    else
...
int ObMySQLLoginer::write_data(int fd, char* buffer, size_t length)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid argument fd=%d, buffer=%p, length=%zd", fd, buffer, length);
    ret = OB_ERROR;
  }
  else
  {
    ...
    while (OB_SUCCESS == ret && length > 0 && (count = write(fd, buff, length)) != 0)
    {
      if (-1 == count)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "write data faild, errno is %d, errstr is %s", errno, strerror(errno));
        }
...
int ObMySQLLoginer::read_data(int fd, char* buffer, size_t length)
{
  ...
  else
  {
    ...
    {
      trycount ++;
      if (trycount % 100 == 0 && yysys::CTimeUtil::getTime() - start_time > timeout)
      {
        YYSYS_LOG(WARN, "read data(fd=%d) timeout. try read count is %ld, count is %zu, length left is %zu, errno is %d,"
      ...
      if (OB_SUCCESS == ret)
      {
        if (-1 == count)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "read data faild, errno is %d, errstr is %s", errno, strerror(errno));
          }
        }
        buff += count;
        length -= count;
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "read not return enough data need %zu more bytes", length);
    }
  }
  return ret;
}
...
//[mysql8]
int ObMySQLLoginer::switch_auth(easy_connection_t *c)
{
  int ret = OB_SUCCESS;
  if (NULL == c)
  {
    YYSYS_LOG(ERROR, "invalid argument c in switch_auth is %p", c);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ObMySQLSwitchAuthPacket packet;
    packet.set_auth_plugin_name(OB_DEFAULT_AUTH_PLUGIN);
    packet.set_auth_plugin_data("aaaaaaaabbbbbbbbbbbb");

    ThreadSpecificBuffer::Buffer* thread_buffer = buffer_.get_buffer();
    if(NULL == thread_buffer)
    {
      YYSYS_LOG(ERROR, "get thread buffer error when sending switch auth packet");
      ret = OB_ERROR;
    }
    else
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "ObMySQLSwitchAuthPacket serialize failed, buffer is %p, buffer len is %ld,"
                  "pos id %ld", out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position());
      }
      else
      {
        ret = write_data(c->fd, out_buffer.get_data(), out_buffer.get_position());
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write switch_auth packet data to client failed, fd is %d, buffer is %p, length is %ld",
                    c->fd, out_buffer.get_data(), out_buffer.get_position());
          ret = OB_ERR_WRITE_AUTH_ERROR;
        }
        else
        {
          YYSYS_LOG(INFO, "switch_auth packet successfully send to client, peer=%s", inet_ntoa_r(c->addr));
        }
      }
    }
  }
...
int ObMySQLLoginer::update_password(easy_connection_t *c, LoginInfo &login_info)
{
  int ret = OB_SUCCESS;
  int read_size = OB_MYSQL_PACKET_HEADER_SIZE;
  char* pos = NULL;
  if (NULL == c)
  {
    YYSYS_LOG(ERROR, "invalid argument c in update_password is %p", c);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if(NULL == thread_buffer)
    {
      YYSYS_LOG(ERROR, "get thread buffer failed when recieving switch_auth response packet from client");
      ret = OB_ERROR;
    }
    else
    {
      ObDataBuffer in_buffer(thread_buffer->current(), thread_buffer->remain());
      YYSYS_LOG(INFO, "update_password:start read from %s %d bytes", inet_ntoa_r(c->addr), read_size);
      ret = read_data(c->fd, in_buffer.get_data(), read_size);
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "read packet header failed");
      ...
      {
        uint32_t packet_len = 0;
        pos = in_buffer.get_data();
        ObMySQLUtil::get_uint3(pos, packet_len);
        YYSYS_LOG(INFO, "start read from %s, %u bytes", inet_ntoa_r(c->addr), packet_len);
        in_buffer.get_position() = OB_MYSQL_PACKET_HEADER_SIZE;
        ret = read_data(c->fd, in_buffer.get_data() + in_buffer.get_position(), packet_len);
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "read switch_auth response packet failed");
          ret = OB_ERR_PROTOCOL_NOT_RECOGNIZE;
        }
        else
        {
          YYSYS_LOG(INFO, "readed from %s %u bytes", inet_ntoa_r(c->addr), packet_len);
          pos = in_buffer.get_data() + in_buffer.get_position();
          int auth_response_len = packet_len;
          login_info.auth_response_.assign_ptr(pos, static_cast<int32_t>(auth_response_len));
        }
      }
    }
  }
