  ...
  namespace obmysql
  {
    ...
    class ObMySQLLoginer
    {
      ...
      private:
        ...
        //[mysql8]
        int switch_auth(easy_connection_t* c);
        int update_password(easy_connection_t* c, LoginInfo& login_info);

        int check_privilege(easy_connection_t* c, sql::ObSQLSessionInfo *& session, LoginInfo& login_info, uint8_t seq);
