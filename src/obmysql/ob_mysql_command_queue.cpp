...
namespace oceanbase
{
  namespace obmysql
  {
      ...
      if (NULL != head_)
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          else
          {
            ...
            YYSYS_LOG(DEBUG, "pop out packet packet command length is %d total size is %ld",
        ...
        {
          YYSYS_LOG(ERROR, "pop tack from ring buffer failed, task: %p, err: %d", packet, err);
        }
      }
      return ret;
    }
    ...
    void ObMySQLCommandQueue::push(ObMySQLCommandPacket* packet)
    {
      ...
      {
        YYSYS_LOG(ERROR, "cast packet to ObMySQLCommandPacket failed");
      }
    }
