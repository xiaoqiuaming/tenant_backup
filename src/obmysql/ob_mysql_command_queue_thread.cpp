...
namespace  oceanbase
{
  namespace obmysql
  {
    ...
    void ObMySQLCommandQueueThread::setThreadParameter(int thread_count, ObMySQLPacketQueueHandler* handler,
                                                       void* args, bool nmclose/*=false*/)
    {
      setThreadCount(thread_count);
      handler_ = handler;
      thd_name_close_ = nmclose; //[647]
      UNUSED(args);
    }
    ...
    void ObMySQLCommandQueueThread::run(yysys::CThread* thread,void* args)
    {
      UNUSED(thread);
      UNUSED(args);
      ...
      *host = host_;
      ...
      //[647]
      if(thd_name_close_){
          SET_THD_NAME_ONCE("mssql-srvcls");
      }else{
          SET_THD_NAME("mssql-srv");
      }
      ...
      while (!_stop)
      {
        ...
        if (handler_)
        {
          int64_t pop_time = yysys::CTimeUtil::getTime();
          ...
          } else if (pop_time - packet->get_disconn_time() < OB_CONNECTION_FREE_TIME_S*1000*1000){
            easy_request_wakeup(packet->get_request());
            YYSYS_LOG(WARN, "pop packet[%p] code[%s] discard t[%ld], now[%ld],  ",
                      packet, packet->get_command().ptr(), packet->get_disconn_time(), pop_time);
          } else {
            YYSYS_LOG(WARN, "pop packet code:%s, disconnect timeout recv[%ld] conn-break[%ld], now[%ld],  ",
                      packet->get_command().ptr(), packet->get_receive_ts(), packet->get_disconn_time(), pop_time);
          }
