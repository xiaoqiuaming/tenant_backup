...
namespace oceanbase
{
  namespace obmysql
  {
    int ObMySQLCallback::encode(easy_request_t* r, void* packet)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument r=%p, packet=%p", r, packet);
        ret = EASY_ERROR;
      }
      else
      {
    ...
    void* ObMySQLCallback::decode(easy_message_t* m)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalide argument m->input is %p", m->input);
      }
      else
      {
        if ((len = static_cast<int32_t>(m->input->last - m->input->pos)) >= OB_MYSQL_PACKET_HEADER_SIZE)
        {
          ...
          if (pkt_len <= m->input->last - m->input->pos)
          {
            ...
            //����message����pool����Ӧ�ò��ڴ�ķ���
            buffer = reinterpret_cast<char*>(easy_pool_alloc(m->pool,
                                                              static_cast<uint32_t>(sizeof(ObMySQLCommandPacket) + pkt_len)));
            if (NULL == buffer)
            {
              YYSYS_LOG(ERROR, "alloc packet buffer(length=%lu) from m->pool failed", sizeof(ObMySQLCommandPacket) + pkt_len);
            }
            else
            {
              YYSYS_LOG(DEBUG, "alloc packet buffer length = %lu", sizeof(ObMySQLCommandPacket) + pkt_len);
              packet = new(buffer)ObMySQLCommandPacket();
              packet->set_header(pkt_len, pkt_seq);
              packet->set_type(pkt_type);
              packet->set_receive_ts(yysys::CTimeUtil::getTime());
              ...
              YYSYS_LOG(DEBUG, "decode comand packet command is \"%.*s\"", packet->get_command().length(),
                        packet->get_command().ptr());
              packet->set_conn_seq(m->c->seq); //[739]
          ...
          else
          {
            ...
            YYSYS_LOG(DEBUG, "not enough data in message, packet length = %u, data in message is %ld",
    ...
    int ObMySQLCallback::process(easy_request_t* r)
    {
      ...
      if (NULL == r)
      {
        YYSYS_LOG(ERROR, "request is NULL, r= %p", r);
        ret = EASY_BREAK;
      }
      else if (NULL == r->ipacket)
      {
        YYSYS_LOG(ERROR, "reqeust is NULL, r->ipacket is %p", r->ipacket);
        ret = EASY_BREAK;
      }
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "handle packet command=%.*s", packet->get_command().length(), packet->get_command().ptr());
        ...
        {
          //����post_packet�ڹ����߳���Ҳ���õ�������ֻ���ȼ����ü���
          r->ms->c->pool->ref++;
          easy_atomic_inc(&r->ms->pool->ref);
          YYSYS_LOG(ERROR, "packet is too large, greater than %d, size of request packet=%d, ret=%d",
          ...
          {
            YYSYS_LOG(WARN, "set err code to error packet failed, ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = epacket.set_message(err_msg)))
          {
            YYSYS_LOG(WARN, "set error message to error packet failed, ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = server->post_packet(r, &epacket, number)))
          {
            YYSYS_LOG(ERROR, "failed to send error packet to mysql client(%s)", get_peer_ip(r));
          }
          else
          {
            ret = EASY_AGAIN;
            YYSYS_LOG(DEBUG, "send error packet to mysql client(%s) succ", get_peer_ip(r));
          }
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            //����������������EASY_OK
            ret = EASY_OK;
            YYSYS_LOG(WARN, "can not push packet(src is %s, ptype is %u) to packet queue",
    ...
    int ObMySQLCallback::on_connect(easy_connection_t* c)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument c or c->handler is NULL");
        ret = EASY_ERROR;
      }
      else
      {
        int64_t begin_us = yysys::CTimeUtil::getTime();
        ObMySQLServer* server = reinterpret_cast<ObMySQLServer*>(c->handler->user_data);
        ret = server->login_handler(c);
        if (OB_SUCCESS != ret)
        {
          ret = EASY_ERROR;
          YYSYS_LOG(WARN, "login faild");
        }
        else
        {
        ...
        int64_t elapsed_us = yysys::CTimeUtil::getTime() - begin_us;
    ...
    //[739]
    void ObMySQLCallback::easy_disconn_cb(easy_connection_t *c, int64_t now)
    {
      //��mysql�ͻ��ˣ����������ϵ�ǰֻ��1������û���ڲ���·�ϻ��ж����ѹ������
      easy_message_t  *m, *m2;
      easy_request_t *r, *n;
      ...
      easy_list_for_each_entry_safe(m, m2, &c->message_list, message_list_node) {
        if(m->request_list_count > 0){
          easy_list_for_each_entry_safe(r, n, &m->all_list, all_node) {
            if (r->ipacket != NULL) {
              ObMySQLCommandPacket *ptask = reinterpret_cast<ObMySQLCommandPacket*>(r->ipacket);
              if (c->seq == ptask->get_conn_seq()){
                ptask->set_disconn_time(now);
                YYSYS_LOG(WARN, "disconn task set disconnect time[%ld] task[%p]", now, ptask);
              }
            }
          }
        }
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
            ...
            if(ret == OB_NEED_RETRY)
            {
                ...
                ret = server->get_session_mgr()->get(c->seq, info);
                if(OB_SUCCESS == ret)
                {
                    if(info->get_session_state() == sql::QUERY_ACTIVE)
                    {
                        YYSYS_LOG(WARN, "submit session delete task failed, key is %d, set state to SESSION_KILLED. ", c->seq);
                        info->set_session_state(sql::SESSION_KILLED);
                    }
                    info->unlock();
                }
            }

          YYSYS_LOG(INFO, "submit async delete request, session key is %d ret is %d", c->seq, ret);
          //generate a async request to delete session later
          ret = server->submit_session_delete_task(c->seq);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "submit session delete task failed session key is %d", c->seq);
            ret = EASY_ERROR;
          }
          else
          {
        ...
        else
        {
          ...
          YYSYS_LOG(INFO, "client disconnect, session key is %d, fd is %d", c->seq, c->fd);
        }
        if (PACKET_RECORDER_FLAG)
        {
          ObMySQLCommandPacketRecord record;
        ...
        if (c->type == EASY_TYPE_SERVER && c->pool->ref > 0){
          easy_disconn_cb(c, yysys::CTimeUtil::getTime());
        }
      }
      return ret;
    }
    ...
    int ObMySQLCallback::clean_up(easy_request_t *r, void *apacket)
    {
      ...
      if(NULL != packet){
          packet->set_cleanup();
          //YYSYS_LOG(INFO, "c= %p r=%p m=%p sql=%.*s", r->ms->c , r, r->ms, packet->get_command().length(), packet->get_command().ptr());
      }
