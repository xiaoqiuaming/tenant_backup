...
namespace oceanbase
{
  ...
  bool ObMySQLRow::check_decimal(ObString os,uint32_t pre,uint32_t scale)const
  {
   ...
   if(s==NULL)
   {
    YYSYS_LOG(WARN,"failed to check_decimal,err is NULL buffer pointer!");
   }
   else
   {
        if(*s=='+'||*s=='.'||(s[0]=='-'&&s[1]=='.'))
    ...
    int ObMySQLRow::serialize(char *buf, const int64_t len, int64_t &pos) const
    {
      ...
      for (cell_index = 0; cell_index < column_num; cell_index++)
      {
        ...
        else if (OB_SUCCESS != (ret = cell_str(*cell, buf, len, pos, cell_index)))
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to serialize cell, err=%d", ret);
          }
          break;
        }
      }
    ...
    int ObMySQLRow::cell_str(const ObObj &obj, char *buf, const int64_t len, int64_t &pos, int64_t cell_index) const
    {
      ...
      switch (obj.get_type())
      {
        ...
        case ObExtendType:
        case ObSeqType:
          YYSYS_LOG(WARN, "invalid ob type=%d", obj.get_type());
          // not goto here.
          ret = OB_ERROR;
          break;
        case ObNullType:
    ...
    int ObMySQLRow::decimal_cell_str(const ObObj &obj, char *buf, const int64_t len, int64_t &pos) const
    {
        ...
        if (OB_SUCCESS != (ret = obj.get_ttint(t1)))
        {
        ...
        else
        {
            ...
            length = dec.to_string(tmp, MAX_PRINTABLE_SIZE);
            s = tmp;
        }
        ...
        if (OB_SUCCESS == ret && length < len - pos)
        {
            ...
            {
                YYSYS_LOG(ERROR, "serialize data len failed len = %lu", length);
            }
        }
        else
        {
    ...
    int ObMySQLRow::fix_year_format(char *buf, const int64_t len, int64_t &pos, uint64_t &len_zero, const struct tm *tms) const
    {
      ...
      if (year < 1 || year >9999)
      {
        ...
        YYSYS_LOG(ERROR, "value of year is out of the rang of DATE, year = %d", tms->tm_year);
      }
      else
      {
        while(year > 0)
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "snprintf zero(%d) to buffer failed", zero_count);
        }
            ...
            else
            {
              ...
              if (0 != microsecond)
              {
                ...
                else
                {
                  ...
                  {
                    ret = OB_ERROR;
                    YYSYS_LOG(ERROR, "snprintf microsecond(%d) to buffer failed", microsecond);
                  }
                }
                if (OB_SUCCESS == ret)
                {
    ...
    int ObMySQLRow::float_cell_str(const ObObj &obj, char *buf, const int64_t len, int64_t &pos) const
    {
      ...
      if (OB_SUCCESS == ret && obj.get_type() == ObFloatType)
      {
        ...
        if (BINARY == type_)
        {
          YYSYS_LOG(DEBUG, "float_cell_str %f", value_f);
          ...
          {
            ret = OB_SIZE_OVERFLOW;
            //YYSYS_LOG(WARN, "Not enough space, err=%d", ret);
          }
        }
        else
        {
