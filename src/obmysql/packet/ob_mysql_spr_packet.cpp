...
namespace oceanbase
{
  namespace obmysql
  {
    ...
    int ObMySQLSPRPacket::serialize(char* buffer, int64_t len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument buffer=%p, len=%ld, pos=%ld",
                  buffer, len, pos);
      ...
      else
      {
        if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, status_, pos)))
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize status failed, buffer=%p, len=%ld, status=%u, pos=%ld, ret=%d",
        ...
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int4(buffer, len, statement_id_, pos)))
        {
          if (OB_UNLIKELY(OB_SIZE_OVERFLOW != ret))
          {
            YYSYS_LOG(ERROR, "serialize warning_count failed, buffer=%p, len=%ld, statement_id_=%u, pos=%ld, ret=%d",
        ...
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, column_num_, pos)))
        {
          if (OB_UNLIKELY(OB_SIZE_OVERFLOW != ret))
          {
            YYSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, column_num_=%u, pos=%ld, ret=%d",
        ...
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, param_num_, pos)))
        {
          if (OB_UNLIKELY(OB_SIZE_OVERFLOW != ret))
          {
            YYSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, param_num_=%u, pos=%ld, ret=%d",
        ...
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, reserved_, pos)))
        {
          if (OB_UNLIKELY(OB_SIZE_OVERFLOW != ret))
          {
            YYSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, reserved_=%u, pos=%ld, ret=%d",
        ...
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, warning_count_, pos)))
        {
          if (OB_UNLIKELY(OB_SIZE_OVERFLOW != ret))
          {
            YYSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, warning_count_=%u, pos=%ld, ret=%d",
