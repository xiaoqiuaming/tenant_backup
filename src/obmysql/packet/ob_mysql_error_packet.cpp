  ...
  namespace obmysql
  {
    ...
    int ObMySQLErrorPacket::encode(char* buffer, int64_t length, int64_t& pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "encode error packet data failed");
        }
      }
      return ret;
    ...
    int ObMySQLErrorPacket::serialize(char* buffer, int64_t len, int64_t& pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "serialize field_count failed, buffer=%p, len=%ld, field_count_=%u, pos=%ld",
                    buffer, len, field_count_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, errcode_, pos)))
        {
          YYSYS_LOG(ERROR, "serialize errcode failed, buffer=%p, len=%ld, errcode=%u, pos=%ld",
                    buffer, len, errcode_, pos);
        }
        else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, MARKER, pos)))
        {
          YYSYS_LOG(ERROR, "serialize marker failed, buffer=%p, len=%ld, marker=%c, pos=%ld",
                    buffer, len, '#', pos);
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "not enough buffer to serialize sqlstate, buffer=%p, len=%ld,"
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "serialize message failed, buffer=%p, len=%ld, message length=%d,"
