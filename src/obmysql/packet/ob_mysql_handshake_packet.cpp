...
namespace oceanbase
{
  namespace obmysql
  {
    ObMySQLHandshakePacket::ObMySQLHandshakePacket()
    {
      ...
      server_capabilities_2_ = 8;
      auth_plugin_data_len_ = 21;
      memset(reserverd_, 0, 10);
      memset(plugin2_, 'b', 12);
      auth_plugin_name_.assign(const_cast<char*>(plugin_name), static_cast<int32_t>(strlen(plugin_name)));
    }
    ...
    int ObMySQLHandshakePacket::serialize(char* buffer, int64_t len, int64_t &pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "serialize packet protocol_version failed, buffer=%p, length=%ld,"
        ...
        {
          YYSYS_LOG(ERROR, "serialize packet server_version failed, buffer=%p, length=%ld,"
        ...
        {
          YYSYS_LOG(ERROR, "serialize packet thread_id failed, buffer=%p, length=%ld,"
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "not enough buffer to serialize scramble_buff && filler, buffer=%p, len=%ld,"
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet filler_ failed, buffer=%p, length=%ld,"
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet server_capabilities failed, buffer=%p, length=%ld,"
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet server_language failed, buffer=%p, length=%ld,"
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet server_status failed, buffer=%p, length=%ld,"
                      "server_status=%d, pos=%ld", buffer, len, server_status_, pos);
          }
            //[mysql8]
            else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int2(buffer, len, server_capabilities_2_, pos)))
            {
              YYSYS_LOG(ERROR, "serialize packet server_capabilities_2_ failed, buffer=%p, length=%ld,"
                        "server_capabilities_2_=%d, pos=%ld", buffer, len, server_capabilities_2_, pos);
            }
            else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, auth_plugin_data_len_, pos)))
            {
              YYSYS_LOG(ERROR, "serialize packet auth_plugin_data_len_ failed, buffer=%p, length=%ld,"
                        "auth_plugin_data_len_=%d, pos=%ld", buffer, len, auth_plugin_data_len_, pos);
            }
        }
...
//          else
//          {
//            YYSYS_LOG(ERROR, "not enough buffer to serialize plugin, buffer=%p, len=%ld,"
...
//        }
        ...
        if (OB_SUCCESS == ret)
        {
          if ((buffer + pos + 10 < buffer + len))
          {
            memcpy(buffer + pos, reserverd_, 10);
            pos += 10;
          }
          else
          {
            YYSYS_LOG(ERROR, "not enough buffer to serialize plugin, buffer=%p, len=%ld,"
                      "scramble_buff length=%d,pos=%ld", buffer, len, 10, pos);
            ret = OB_ERROR;
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "not enough buffer to serialize filler2, buffer=%p, len=%ld,"
                      "terminated_ length=%c,pos=%ld", buffer, len, 1, pos);
            ret = OB_ERROR;
          }
        }

        //[mysql8]
        if(OB_SUCCESS != (ret = ObMySQLUtil::store_obstr_zt(buffer, len, auth_plugin_name_, pos)))
        {
            YYSYS_LOG(ERROR, "serialize packet auth_plugin_name_ failed, buffer=%p, length=%ld,"
                      "auth_plugin_name_ length=%d, pos=%ld", buffer, len, auth_plugin_name_.length(), pos);
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet haader size failed, buffer=%p, length=%ld, value=%d, pos=%ld",
                      buffer, len, pkt_len, start_pos);
          }
          else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, len, 0, start_pos)))
          {
            YYSYS_LOG(ERROR, "serialize packet haader seq failed, buffer=%p, length=%ld, value=%d, pos=%ld",
...
//      {
//        YYSYS_LOG(ERROR, "invalid argument version.ptr=%p, version.length()=%d",
...
//        if (OB_SUCCESS != ret)
//        {
//          YYSYS_LOG(ERROR, "write string to server_version_ failed");
//        }
//      }
//      return ret;
//    }
