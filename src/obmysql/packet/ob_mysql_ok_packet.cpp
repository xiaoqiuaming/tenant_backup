...
namespace oceanbase
{
  namespace obmysql
  {
    ...
    int ObMySQLOKPacket::set_message(ObString& message)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument message.ptr=%p, message.length=%d",
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "write string to messaget failed");
        }
      }
      return ret;
    }
    ...
    int ObMySQLOKPacket::encode(char* buffer, int64_t length, int64_t& pos)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize packet haader size failed, buffer=%p, buffer length=%ld, packet length=%d, pos=%ld",
                      buffer, length, pkt_len, start_pos);
          }
          else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, length, get_seq(), start_pos)))
          {
            YYSYS_LOG(ERROR, "serialize packet haader seq failed, buffer=%p, buffer length=%ld, seq number=%d, pos=%ld",
                      buffer, length, get_seq(), start_pos);
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "encode ok packet data failed");
        }
      }
      return ret;
    }
    ...
    int ObMySQLOKPacket::serialize(char* buffer, int64_t length, int64_t& pos)
    {
      ...
      else
      {
        ...
        else if (0 != message_.length())
        {
          ...
          {
            YYSYS_LOG(ERROR, "serialize message failed, buffer=%p, length=%ld, insert_id=%lu,"
                "pos=%ld", buffer, length, insert_id_, pos);
          }
        }
      }
