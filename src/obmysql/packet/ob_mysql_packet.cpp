...
int ObMySQLPacket::encode(char* buffer, int64_t length, int64_t& pos)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "serialize packet haader seq failed, buffer=%p, buffer length=%ld, seq number=%u, pos=%ld",
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "encode packet data failed, ret is %d", ret);
      }
    }
