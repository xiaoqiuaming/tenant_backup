...
namespace oceanbase
{
  namespace obmysql
  {
  ObMySQLCommandPacket::ObMySQLCommandPacket() : type_(0), req_(NULL), next_(NULL), receive_ts_(-1),cleanup_(false)
  , conn_seq_(0), disconn_time_(0) //[739]
    {
    ...
    int ObMySQLCommandPacket::set_request(easy_request_t* req)
    {
      ...
      if (NULL == req)
      {
        YYSYS_LOG(ERROR, "invalid argument reqeust is %p", req);
