...
bool ObConfigIntegralItem::parse_range(const char *range)
{
  ...
  if (NULL == range)
  {
    YYSYS_LOG(ERROR, "check range is NULL!");
    ret = false;
  }
  else if ('\0' == range[0])
  { }
...
int64_t ObConfigIntItem::get(const char* str, bool &valid) const
{
  ...
  else
  {
    ...
    {
      valid = false;
      YYSYS_LOG(ERROR, "set int error! name: [%s], str: [%s]", name(), str);
    }
  }
  return value;
}
...
int64_t ObConfigTimeItem::get(const char* str, bool &valid) const
{
  ...
  else
  {
    ...
    {
      valid = false;
      YYSYS_LOG(ERROR, "set time error! name: [%s], str: [%s]", name(), str);
    }
  }
  return value;
}
...
int64_t ObConfigCapacityItem::get(const char* str, bool &valid) const
{
  ...
  else
  {
    ...
    {
      valid = false;
      YYSYS_LOG(ERROR, "set capacity error! name: [%s], str: [%s]", name(), str);
    }
  }
...
bool ObConfigBoolItem::get(const char* str, bool &valid) const
{
  ...
  {
    valid = false;
    YYSYS_LOG(ERROR, "Get bool config item fail, str is NULL!");
  }
  ...
  {
    YYSYS_LOG(ERROR, "Get bool config item fail, str: [%s]", str);
    valid = false;
  }
  return value;
}
...
//    disable_ = true;
//    ret = false;
//    YYSYS_LOG(ERROR, "Not well-formed moment item value! str: [%s]", str);
//  }
//  else
//  {
//    disable_ = false;
...
//      ret = false;
//      //YYSYS_LOG(ERROR, "Not well-formed moment item value! str: [%s]", str);
//      YYSYS_LOG(ERROR, "Not well-formed moment item value or dutytime num larger than %d! str: [%s]", MAXNUM, str);
