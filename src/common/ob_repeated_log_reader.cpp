...
int ObRepeatedLogReader::read_log(LogCommand &cmd, uint64_t &log_seq, char *&log_data, int64_t & data_len)
{
  ...
  if (!is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObSingleLogReader has not been initialized, please initialize first");
    ret = OB_NOT_INIT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "read_header(entry, file_id=%ld, pos=%ld, seq=%ld)=>%d", file_id_, pos, last_log_seq_, ret);
    }
    else if (OB_LAST_LOG_RUINNED == ret)
    {
      ret = OB_READ_NOTHING;
      YYSYS_LOG(DEBUG, "read_log(): read end of file.");
    }
    ...
    if (OB_SUCCESS == ret)
    {
      if (OB_SUCCESS != entry.check_header_integrity())
      {
        ...
        if (all_zero(log_buffer_.get_data() + log_buffer_.get_position(), log_buffer_.get_remain_data_len()))
        {
          ...
          YYSYS_LOG(WARN, "read an invalid log_entry, data are allzero");
        }
        else
        {
          ret = OB_INVALID_LOG;
          //hex_dump(log_buffer_.get_data() + log_buffer_.get_position(), log_buffer_.get_remain_data_len(), false, YYSYS_LOG_LEVEL_WARN);
          YYSYS_LOG(WARN, "Log entry header is corrupted, file_id_=%lu pread_pos_=%ld pos=%ld last_log_seq_=%lu",
                    file_id_, pread_pos_, pos, last_log_seq_);
          YYSYS_LOG(WARN, "log_buffer_ position_=%ld limit_=%ld capacity_=%ld",
      ...
      {
        //YYSYS_LOG(DEBUG, "do not get a full entry, when checking log data");
        log_buffer_.get_position() -= entry.get_serialize_size();

        ret = read_log_();
        if (OB_READ_NOTHING == ret)
        {
          YYSYS_LOG(DEBUG, "do not get a full entry, when reading ObLogEntry");
        }
        else
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(DEBUG, "do not get a full entry, when checking log data");
              YYSYS_LOG(DEBUG, "log_data_len=%d remaining=%ld", entry.get_log_data_len(), log_buffer_.get_remain_data_len());
              YYSYS_LOG(DEBUG, "limit=%ld pos=%ld", log_buffer_.get_limit(), log_buffer_.get_position());
  ...
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        ...
        {
          ret = OB_INVALID_LOG;
          YYSYS_LOG(WARN, "data corrupt, when check_data_integrity, file_id_=%lu pread_pos_=%ld pos=%ld last_log_seq_=%lu",
                    file_id_, pread_pos_, pos, last_log_seq_);
          YYSYS_LOG(WARN, "log_buffer_ position_=%ld limit_=%ld capacity_=%ld",
                    log_buffer_.get_position(), log_buffer_.get_limit(), log_buffer_.get_capacity());
          hex_dump(log_buffer_.get_data(), static_cast<int32_t>(log_buffer_.get_limit()), true, YYSYS_LOG_LEVEL_WARN);
        }
        log_buffer_.get_position() -= entry.get_serialize_size();
        pread_pos_ -= log_buffer_.get_limit() - log_buffer_.get_position();
        log_buffer_.get_limit() = log_buffer_.get_position();
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "the log sequence is not continuous[%lu => %lu[cmd=%d]]",
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "LOG ENTRY: SEQ[%lu] CMD[%d] DATA_LEN[%ld] POS[%ld]", entry.seq_, cmd, data_len, pos);
    pos += entry.header_.header_length_ + entry.header_.data_length_;
  }
  if (OB_READ_ZERO_LOG == ret || OB_INVALID_LOG == ret)
  {
    ret = OB_READ_NOTHING;
    YYSYS_LOG(WARN, "read invalid log[%ld], reset err=OB_READ_NOTHING", last_log_seq_);
  }
