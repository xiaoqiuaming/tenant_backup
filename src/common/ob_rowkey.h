  ...
  namespace common
  {
    ...
    class ObRowkey
    {
      ...
      public:
        ...
        inline bool is_empty_row() const
        { return NULL == obj_ptr_ && 0 == obj_cnt_; }
        // is min rowkey or max rowkey
        inline bool is_min_row(void) const
        { return (*this == ObRowkey::MIN_ROWKEY); }
        inline bool is_max_row(void) const
        { return (*this == ObRowkey::MAX_ROWKEY); }
        ...
        //@berif ǳ���� ObRowkey
        inline void assign(const ObRowkey & other)
        {
          this->obj_ptr_=other.obj_ptr_;
          this->obj_cnt_=other.obj_cnt_;
        }
        //add 20150707:e
        ...
        int to_string_memcpy(char *buffer, const int64_t length, int64_t &pos) const;
    ...
    class RowkeyInfoHolder
    {
      public:
        ...
        inline const ObRowkeyInfo* get_rowkey_info() const
        { return rowkey_info_; }
      protected:
    ...
    int ObRowkey::deep_copy(ObRowkey& rhs, Allocator& allocator) const
    {
      ...
      if (obj_cnt_ > 0 && NULL != obj_ptr_)
      {
        ...
        for (int64_t i = 0; i < obj_cnt_ && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            long unsigned int decimal_len = sizeof(uint64_t) * obj_ptr_[i].get_nwords();
            if (OB_SUCCESS != (ret = obj_ptr_[i].get_ttint(decimal_val)))
            {
            ...
            else
            {
              ...
              obj_ptr[i].set_decimal(tmp, obj_ptr_[i].get_precision(),obj_ptr_[i].get_scale(),
                                     obj_ptr_[i].get_vscale(), obj_ptr_[i].get_nwords());
...
#define alloc_small_mem_return_ptr(varchar_mem, rowkey_count, ret) \
  if (NULL == (varchar_mem = (char*)ob_malloc(OB_MAX_VARCHAR_LENGTH*(rowkey_count), ObModIds::OB_SQL_EXECUTER)))  \
    {  \
  YYSYS_LOG(ERROR, "failed to malloc memory");  \
  ret = OB_ALLOCATE_MEMORY_FAILED;  \
  }
