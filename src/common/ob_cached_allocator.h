...
namespace oceanbase
{
  namespace common
  {
    ...
    ObCachedAllocator<T>::~ObCachedAllocator()
    {
      //YYSYS_LOG(DEBUG, "free cached objs, count=%ld", cached_objs_.count());
      T *obj = NULL;
      ...
      {
        YYSYS_LOG(WARN, "some allocated object is not freed, allocated_count=%d cached_count=%d",
    ...
    void ObCachedAllocator<T>::free(T *obj)
    {
      if (NULL != obj)
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to push obj into array or cached_count:%d > cached_limit:%d, err=%d", err,cached_count_,cached_limit);
          // free directly
          obj->~T();
          pool_.free(obj);
        }
