...
/*Exp:��Ҫʹ��cpp�еľ�̬����"varchar_timestamp()"*/
...
{
  reset_format_wrong_flag(); //add for [507-expire info enhancement]
  type_ = obj.get_type();
  switch(obj.get_type())
  {
    case ObNullType:
      ...
      //break;                 old code
    {
      ...
      if(OB_SUCCESS!= ret)
      {
        YYSYS_LOG(ERROR, "failed to do from in ObExprObj::assign, err=%d", ret);
      }
      break;
    }
      //modify:e
    ...
    case ObIntervalType:
      ...
      //add 20150908:e
    default:
      YYSYS_LOG(ERROR, "invalid value type=%d", obj.get_type());
      break;
  }
}
...
int ObExprObj::to(ObObj &obj) const
{
  ...
  switch(get_type())
  {
      ...
      //break;         old code
    {
      ...
      if(ret!=OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "failed to set decimal, err=[%d]", ret);
      }
      break;
    }
      //modify:e
    ...
    case ObExtendType:
      ...
      break;
    default:
      YYSYS_LOG(ERROR, "invalid value type=%d", get_type());
...
int ObExprObj::compare_same_type(const ObExprObj &other) const
{
  ...
  switch(get_type())
  {
    ...
    case ObBoolType:
      ...
      break;
    default:
      YYSYS_LOG(ERROR, "invalid type=%d", get_type());
      break;
  }
  return cmp;
}
                                     ...
                                     const ObExprObj *&p_this, const ObExprObj *&p_other)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid obj type for type promotion, this=%d other=%d", this_type, other_type);
    ret = OB_ERR_UNEXPECTED;
  }
  else
  {
    ...
    if (this_type != res_type)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to cast object, err=%d from_type=%d to_type=%d",
                                             ...
                                             const ObExprObj *&p_this, const ObExprObj *&p_other)
{
                                          ...
                                          promoted_obj1, promoted_obj2, p_this, p_other)))
  {
    ...
    {
      YYSYS_LOG(WARN, "invalid type promote for compare, err=%d", ret);
    }
  }
  return ret;
}
...
int ObExprObj::compare(const ObExprObj &other, int &cmp) const
{
  ...
  if (OB_SUCCESS != (ret = compare_type_promotion(*this, other, promoted1, promoted2, p_this, p_other)))
  {
    ...
    //ret = OB_RESULT_UNKNOWN;
    //del 20151010:e
    //YYSYS_LOG(ERROR, "failed to do compare_type_promotion");
...
int ObExprObj::lt(const ObExprObj &other, ObExprObj &res) const
{
  ...
  //add for [507-expire info if format incorrect]-b
  if(this->get_if_format_wrong() || other.get_if_format_wrong())
  {
    res.set_bool(false);
    YYSYS_LOG(USER_ERROR, "Format of TIMESTAMP value is incorrect.");
  }
  //add for [507-expire info if format incorrect]-e
  //modify for [507-expire info if format incorrect]-b
  //     if (OB_SUCCESS == (ret = this->compare(other, cmp)))
  else if (OB_SUCCESS == (ret = this->compare(other, cmp)))
    //modify for [507-expire info if format incorrect]-e
  {
    res.set_bool(cmp < 0);
  }
...
int ObExprObj::ge(const ObExprObj &other, ObExprObj &res) const
{
  int ret = OB_SUCCESS;
  int cmp = 0;
  //modify for [507-expire info if format incorrect]
  if(this->get_if_format_wrong() || other.get_if_format_wrong())
  {
    res.set_bool(false);
    YYSYS_LOG(USER_ERROR, "Format of TIMESTAMP value is incorrect.");
...
int ObExprObj::is(const ObExprObj &other, ObExprObj &res) const
{
  ...
  else if (ObBoolType != other.get_type())
  {
    ...
    YYSYS_LOG(WARN, "invalid type for is operator, type=%d", other.get_type());
                                           ...
                                           const ObExprObj *&p_this, const ObExprObj *&p_other)
{
                                          ...
                                          promoted_obj1, promoted_obj2, p_this, p_other)))
  {
    if (OB_RESULT_UNKNOWN != ret)
    {
      YYSYS_LOG(WARN, "invalid type promote for arithmetic, err=%d", ret);
    }
  }
  return ret;
}
...
int ObExprObj::coalesce_type_compatible(ObObjType& t1,const ObObjType& t2)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN,"parameters type %d and type %d cann't consistent!",t1,t2);
...
inline int ObExprObj::add_same_type(const ObExprObj &other, ObExprObj &res) const
{
  ...
  switch(get_type())
  {
    case ObIntType:
      ...
      //add for [sum int64 overflow]-b
      if((this->v_.int_ > 0 && other.v_.int_ > 0 && (res.v_.int_ > INT64_MAX || res.v_.int_ < 0))
         || (this->v_.int_ < 0 && other.v_.int_ < 0 && (res.v_.int_ < INT64_MIN || res.v_.int_ > 0)))
      {
        res.v_.int_ = 1;
        ret = OB_SUM_OVERFLOW;
        YYSYS_LOG(ERROR, "Arithmetic overflow or other arithmetic exception occurred");
      }
      //add for [sum int64 overflow]-e
      break;
      //add yushengjuan [INT_32] 20151012:b
    case ObInt32Type:
      res.v_.int32_ = this->v_.int32_ + other.v_.int32_;
      //add for [sum int32 overflow]-b
      if((this->v_.int32_ > 0 && other.v_.int32_ > 0 && (res.v_.int32_ > INT32_MAX || res.v_.int32_ < 0))
         || (this->v_.int32_ < 0 && other.v_.int32_ < 0 && (res.v_.int32_ < INT32_MIN || res.v_.int32_ > 0)))
      {
        res.v_.int32_ = 1;
        ret = OB_SUM_OVERFLOW;
        YYSYS_LOG(ERROR, "Arithmetic overflow or other arithmetic exception occurred");
      }
      //add for [sum int32 overflow]-e
      break;
      //add 20151012:e
    case ObFloatType:
      res.v_.float_ = this->v_.float_ + other.v_.float_;
      //add for [sum float overflow]-b
      if((this->v_.float_ > 0 && other.v_.float_ > 0 && (res.v_.float_ > FLT_MAX || res.v_.float_ <= 0))
         || (this->v_.float_ < 0 && other.v_.float_ < 0 && ((-1 * res.v_.float_) > FLT_MAX || res.v_.float_ >= 0)))
      {
        res.v_.float_ = 0;
        res.type_ = ObDoubleType;
        res.v_.double_ = static_cast<double>(this->v_.float_) + static_cast<double>(other.v_.float_);
      }
      //add for [sum float overflow]-e
      break;
    case ObDoubleType:
...
inline int ObExprObj::cast_to_int(int64_t &val) const
{
  ...
  {
    // YYSYS_LOG(WARN, "should not be null");
...
inline int ObExprObj::cast_to_varchar(ObString &varchar, ObStringBuf &mem_buf) const
{
  ...
  else if (OB_LIKELY(this->get_type() != ObVarcharType))
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to allocate memory for string. ret=%d", ret);
    }
  }
...
inline int ObExprObj::cast_to_decimal(ObString &varchar, ObStringBuf &mem_buf) const
{
  ...
  else if (OB_UNLIKELY(this->get_type() == ObDecimalType))
  {
    YYSYS_LOG(WARN, "type should not be ObDecimalType!");
    ret = OB_INVALID_ARGUMENT;
  }
  return ret;
}
...
int ObExprObj::cast_toV2(int32_t dest_type, ObExprObj &result, ObStringBuf &mem_buf,uint32_t precision,uint32_t scale)
{
  ...
  else if (dest_type > ObMinType && dest_type < ObMaxType)
  {
    ...
    {
      ObDecimal od;
      od=this->get_decimal();    //��ȷ��ֵ������ʱthis�ĳ�Աvarchar��һ������ȷ��
      ObDecimal od_v2;
      ...
      if((precision-scale)<(od_v2.get_precision()-od_v2.get_vscale()))
      {
        ...
        YYSYS_LOG(ERROR, "OB_DECIMAL_UNLEGAL_ERROR,od.get_precision()=%d,od.get_vscale()=%d",od.get_precision(),od.get_vscale());
      }
      else
      {
        ...
        if(this->get_type()==ObVarcharType)
        {
          ...
          if(os_v.ptr()==NULL||(int)(os_v.length())==0)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(ERROR, "fail to do from in ObExprObj::cast_toV2 ret=%d", err);
              }
              else
              {
                od.set_precision(precision);
...
inline int ObExprObj::div_same_type(const ObExprObj &other, ObExprObj &res) const
{
  ...
  switch(get_type())
  {
    ...
    default:
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "unexpected branch, type=%d", get_type());
      break;
  }
  return ret;
}
                                         ...
                                         const ObExprObj *&p_this, const ObExprObj *&p_other)
{
  ...
  if (OB_SUCCESS == ret && ObIntType != this_type)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to cast obj to int, err=%d this_type=%d",
                ret, this_type);
    }
    else
    {
...
int ObExprObj::concat(const ObExprObj &other, ObExprObj &result, ObStringBuf &mem_buf) const
{
  ...
  else
  {
    ...
    {
      // YYSYS_LOG(WARN, "fail to cast obj to varchar. ret=%d", ret);
      result.set_null();
    }
    ...
    else
    {
      ...
      else
      {
        ...
        {
          result.set_null();
          YYSYS_LOG(WARN, "fail to write string to membuf. ret=%d", ret);
        }
        else
        {
          result.set_varchar(varchar);
...
int ObExprObj::trim(const ObExprObj &trimType, const ObExprObj &trimPattern, ObExprObj &result, ObStringBuf &mem_buf) const
{
  ...
  {
    YYSYS_LOG(WARN, "fail to get trim type. err=%d", err);
  }
  else if (OB_SUCCESS != (err = trimPattern.cast_to_varchar(pattern, mem_buf)))
  {
    // YYSYS_LOG(WARN, "fail to get trim pattern. err=%d", err);
  }
  else if (pattern.length() <= 0)
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "trim pattern is empty");
  }
  else if (OB_SUCCESS != (err = this->cast_to_varchar(src, mem_buf)))
  {
    // YYSYS_LOG(WARN, "fail to get trim source. err=%d", err);
  }
  else
  {
    if (type == 0) // both
...
//add 20151225:e
//add lqc [MultiUps 1.0] [#type] 20170417 b
bool ObExprObj::is_equal_part_value_type(const ObObjType &t1, const ObObjType &t2)
{
  bool ret = false;
  if (
      (t1 == t2)
      || (is_datetime_compare(t1) && is_datetime_compare(t2))
      || (is_numeric(t1) && is_numeric(t2))
      || (( ObVarcharType == t1) && (ObVarcharType == t2))
      || (is_datetime_compare(t2) && is_datetime_compare(t1)))
  {
    ret = true;
  }
  return ret;
...
int ObExprObj::set_decimal(const char* dec_str)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to construct decimal from string, err=%d str=%s", ret, dec_str);
  }
  else
  {
    //add wenghaixing DECIMAL OceanBase_BankCommV0.3 2014_7_10:b
    ...
    //add e
    if (OB_SUCCESS != (ret = obj.set_decimal(os, TEST_PREC, TEST_SCALE,TEST_SCALE, len)))
    {
      YYSYS_LOG(WARN, "obj failed to set decimal, err=%d", ret);
    }
    else
    {
      this->assign(obj);
...
int ObExprObj::get_int(int64_t& value) const
{
  ...
  {
    YYSYS_LOG(WARN, "failed to convert to obj, err=%d", ret);
  }
  else
  {
...
int ObExprObj::unhex(ObExprObj &res, ObStringBuf & mem_buf)
{
  ...
  else if (varchar_.length() % 2 != 0)
  {
    res.set_null();
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "length is odd, ret=%d", ret);
  }
  else
  {
    int i = 0;
    ...
    if (NULL == buf)
    {
      ...
      YYSYS_LOG(ERROR, "alloc memory failed, ret=%d", ret);
    }
    else
    {
      for (i = 0;i < varchar_.length();i = i + 2)
      {
        ...
        else
        {
          ret = OB_ERR_UNEXPECTED;
          res.set_null();
          YYSYS_LOG(WARN, "data invalid, ret=%d", ret);
          break;
        }
      }
      if (OB_SUCCESS == ret)
...
int ObExprObj::ip_to_int(ObExprObj &res)
{
  ...
  else
  {
    ...
    if (varchar_.length() > 15)
    {
      res.set_null();
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN, "ip format invalid, ret=%d", ret);
    }
    else
    {
      ...
      {
        obj.set_null();
        YYSYS_LOG(WARN, "ip format invalid");
      }
      else
      {
        struct in_addr addr;
...
inline int ObExprObj::get_gtm(struct tm &gtm, int unexpected_type) const
{
  ...
  switch (get_type())
  {
    ...
    case ObVarcharType:
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "param's type is incorrect.");
      }
      else
      {
        ...
        if(ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "convert varchar to timestamp failed, ret=%d", ret);
        }
      }
      break;
    default:
...
int ObExprObj::year(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "year param's type not match, ret=%d", ret);
  }
  else if(OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    ...
    {
      ret = OB_OBJ_TYPE_ERROR;
      YYSYS_LOG(ERROR, "param's type not match, ret=%d", ret);
    }
    else
    {
  ...
  {
    YYSYS_LOG(ERROR, "year param is incorrect, ret=%d", ret);
  }
  return ret;
}
...
int ObExprObj::month(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "month param's type not match, ret=%d", ret);
  }
  else if(OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    if ((gtm.tm_year + 1900) < 0)
  ...
  {
    YYSYS_LOG(ERROR, "month param is incorrect, ret=%d", ret);
  }
  return ret;
}
...
int ObExprObj::day(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "day param's type not match, ret=%d", ret);
  }
  else if(OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    if ((gtm.tm_year + 1900) < 0)
  ...
  {
    YYSYS_LOG(ERROR, "day param is incorrect, ret=%d", ret);
  }
  return ret;
}
int ObExprObj::days(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "days param's type not match, ret=%d", ret);
  }
  else if (OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    if ((gtm.tm_year + 1900) < 0)
    ...
    else
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"days param is incorrect.");
      }
      else
      {
        ...
        YYSYS_LOG(DEBUG,"year: %d  month: %d  day: %d -> daynr: %ld",
  ...
  {
    YYSYS_LOG(ERROR, "days param is incorrect, ret=%d", ret);
  }
  return ret;
}
//add 20150831:e
...
int ObExprObj::hour(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "hour param's type not match, ret=%d", ret);
  }
  else if (OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    hour = gtm.tm_hour;
  ...
  {
    YYSYS_LOG(ERROR, "hour param is incorrect, ret=%d", ret);
  }
  return ret;
}
...
int ObExprObj::minute(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "minute param's type not match, ret=%d", ret);
  }
  else if (OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    minute = gtm.tm_min;
  ...
  {
    YYSYS_LOG(ERROR, "minute param is incorrect, ret=%d", ret);
  }
  return ret;
}
...
int ObExprObj::second(ObExprObj &res)
{
  ...
  {
    ret = OB_OBJ_TYPE_ERROR;
    YYSYS_LOG(ERROR, "second param's type not match, ret=%d", ret);
  }
  else if (OB_SUCCESS == (ret = get_gtm(gtm, unexpected_type)))
  {
    second = gtm.tm_sec;
  ...
  {
    YYSYS_LOG(ERROR, "second param is incorrect, ret=%d", ret);
  }
  return ret;
}
//add 20150901:e
...
ObObj ObExprObj::type_arithmetic(const ObObj& t1, const ObObj& t2)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to promote type, err=%d", err);
  }
  else
  {
...
ObObj ObExprObj::type_add_v2(const ObObj& t1, const ObObj& t2)
{
  ...
  type1=t1.get_type();
  type2=t2.get_type();
  //add for [598 array out of bounds]-b
  if(type1 <= ObMinType || type2 <= ObMinType)
  {
    ret.set_type(ObMinType);
  }
  else if(type1 >= ObMaxType || type2 >= ObMaxType)
  {
    ret.set_type(ObMaxType);
  }
  else
  {
    ret.set_type(ARITHMETIC_TYPE_PROMOTION[type1][type2]);
  }
  //add for [598 array out of bounds]-e
...
ObObj ObExprObj::type_mod(const ObObj& t1, const ObObj& t2)
{
  ...
  if (OB_SUCCESS != (err = mod_type_promotion(v1, v2, promoted_obj1, promoted_obj2,
                                              p_this, p_other)))
  {
    YYSYS_LOG(WARN, "failed to promote type, err=%d type1=%d type2=%d", err, t1.get_type(), t2.get_type());
  }
  else
  {
...
ObObj ObExprObj::type_negate(const ObObj& t1)
{
  ...
  switch(t1.get_type())
  {
    ...
    case ObDecimalType:
      ...
      break;
    default:
      YYSYS_LOG(WARN, "not supported type for negate, type=%d", t1.get_type());
      break;
  }
  return ret;
}
...
ObObj ObExprObj::type_varchar_length(const ObObj& t1)
{
  ...
  {
    YYSYS_LOG(WARN, "not supported type for varchar_length, type=%d", t1.get_type());
  }
  else
  {
    ret.meta_.type_ = ObIntType;
...
//[view]
int ObExprObj::cons_decimal_num(const uint32_t precision, const uint32_t scale, char num[40], ObDecimal &decimal_num, bool is_division)
{
    int ret = OB_SUCCESS;
    if(precision > static_cast<uint32_t> (MAX_DECIMAL_DIGIT) || scale > static_cast<uint32_t> (MAX_DECIMAL_SCALE) || scale >= precision)
    {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "invalid decimal arguments, precision[%u], scale[%u]", precision, scale);
    }
    else if (is_division)
    {
        memset(num, '0', precision + 1);
        num[1] = '.';
        num[scale + 1] = '1';
        num[scale + 2] = '\0';
        decimal_num.from(num);
    }
    else
    {
        memset(num, '9', precision + 1);
        num[precision - scale] = '.';
        num[precision + 1] = '\0';
        decimal_num.from(num);
    }
...
int ObExprObj::calc(const TypeDesc type[2], const OpType op, TypeDesc &result_type, const int64_t digit, const int64_t pos, const int64_t len)
{
    int ret = OB_SUCCESS;
    for (int32_t i = 0; i < 2; i++)
    {
        const TypeDesc &type_desc = type[i];
        if(type_desc.type_ == ObDecimalType && (type_desc.precision_ > static_cast<uint32_t> (MAX_DECIMAL_DIGIT)
               || type_desc.scale_ > static_cast<uint32_t> (MAX_DECIMAL_SCALE) || type_desc.scale_ >= type_desc.precision_))
        {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "invalid decimal arguments, precision[%u], scale[%u]", type_desc.precision_, type_desc.scale_);
            break;
        }
    }
    if(ret == OB_SUCCESS)
    {
        ...
        case OP_MUL:
        {
            ret = add_sub_mul(type[0], type[1], op, result_type);
            break;
        }
        case OP_DIV:
        {
            ret = div(type[0], type[1], result_type);
            break;
        }
        case OP_ROUND:
        {
            round(type[0], digit, result_type);
            break;
        }
        case OP_SUBSTR:
        {
            substr(type[0], pos, len, result_type);
            break;
        }
        case OP_CONCAT:
        {
            concat(type[0], type[1], result_type);
            break;
        }
        default:
        {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(WARN, "unknown type");
            break;
        }
        }
    }
...
/***
 exp:
 create view v1 (a) as select c1 * c2 from t1;
 �a�������:
 1.��c1�c2����
 2.�������c1���� + / * c2�������������������������
 ��������mysql�ob
 */
int ObExprObj::add_sub_mul(const TypeDesc &type_1, const TypeDesc &type_2, const OpType op, TypeDesc &result_type)
{
    int ret = OB_SUCCESS;
    ObExprObj expr_obj;
    bool is_double_type = false;
    ObObjType type1 = type_1.type_;
    ObObjType type2 = type_2.type_;
    if(((type1 == ObIntType || type1 == ObInt32Type) && type2 == ObDecimalType)
            ||(type1 == ObDecimalType && (type2 == ObIntType || type2 == ObInt32Type)))
    {
        //num1
        expr_obj.set_int(INT_MAX);
        const TypeDesc &type_desc = (type1 == ObDecimalType) ? type_1 : type_2;
        //num2
        char num[40] = {0};
        ObDecimal decimal_num;
        ret = cons_decimal_num(type_desc.precision_, type_desc.scale_, num, decimal_num);
        if( ret == OB_SUCCESS)
        {
            this->set_decimal(decimal_num);
        }
    }
    else if (type1 == ObDecimalType && type2 == ObDecimalType)
    {
        //num1
        char num1[40] = {0};
        ObDecimal decimal_num;
        ret = cons_decimal_num(type_1.precision_, type_1.scale_, num1, decimal_num);
        if( ret == OB_SUCCESS)
        {
            this->set_decimal(decimal_num);
        }
        ...
        char num2[40] = {0};
        decimal_num.reset();
        ret = cons_decimal_num(type_2.precision_, type_2.scale_, num2, decimal_num);
        if( ret == OB_SUCCESS)
        {
            expr_obj.set_decimal(decimal_num);
        }
    }
    else if((type1 == ObDecimalType && type2 == ObBoolType)
            ||(type1 == ObBoolType && type2 == ObDecimalType))
    {
        //num1
        expr_obj.set_int(1);
        //num2
    ...
    else if((type1 == ObDecimalType && type2 == ObVarcharType)
            ||(type1 == ObVarcharType && type2 == ObDecimalType))
    {
        result_type.type_ = ObDoubleType;
        result_type.precision_ = 0;
        result_type.scale_ = 0;
        is_double_type = true;
    }
    ...
    {
        ObExprObj res_obj;
        if(op == OP_ADD || op == OP_MINUS)
        {
            ret = this->add(expr_obj, res_obj);
        }
        else if(op == OP_MUL)
        {
            ret = this->mul(expr_obj, res_obj);
        }

        if (ret == OB_VALUE_OUT_OF_RANGE)
        {
            result_type.type_ = ObDecimalType;
            result_type.precision_ = MAX_DECIMAL_DIGIT;
            result_type.scale_ = std::max(type_1.scale_, type_2.scale_);
            ret = OB_SUCCESS;
        }
        else
        {
            const ObDecimal &res_num = res_obj.get_decimal();
            result_type.type_ = ObDecimalType;
            result_type.precision_ = res_num.get_precision();
            result_type.scale_ = res_num.get_scale();
        }
    }
    return ret;
}
...
/*
  exp: create view v1 (a) as select c1/c2 from t1;
  ��a���Լ��㷽��:
  1.����c1��c2�е����ֵ
  2.�������нӿ��� c1�����ֵ/c2����Сֵ �Ľ������ͼ�����е���ֵ��С ����������Χ֮�ڡ�
  �ο�mysql, ob
  */

int ObExprObj::div(const TypeDesc &type_1, const TypeDesc &type_2, TypeDesc &result_type)
{
    int ret = OB_SUCCESS;
    ObExprObj expr_obj;
    bool is_double_type = false;
    ...
    {
        //num1
        this->set_int(INT_MAX);
        //num2
        char num[40] = {0};
        ObDecimal decimal_num;
        ret = cons_decimal_num(type_2.precision_, type_2.scale_, num, decimal_num, true);
        if( ret == OB_SUCCESS)
        {
            expr_obj.set_decimal(decimal_num);
        }
    }
    else if (type1 == ObDecimalType &&
             (type2 == ObIntType || type2 == ObBoolType || type2 == ObInt32Type))
    {
        //num1
        char num[40] = {0};
        ObDecimal decimal_num;
        ret = cons_decimal_num(type_1.precision_, type_1.scale_, num, decimal_num);
        if( ret == OB_SUCCESS)
        {
            this->set_decimal(decimal_num);
        }
    ...
    else if(type1 == ObDecimalType && type2 == ObDecimalType)
    {
        ...
        char num2[40] = {0};
        decimal_num.reset();
        ret = cons_decimal_num(type_2.precision_, type_2.scale_, num2, decimal_num, true);
        if( ret == OB_SUCCESS)
        {
            expr_obj.set_decimal(decimal_num);
        }
    }
    else if(type1 == ObBoolType && type2 == ObDecimalType)
    {
        //num1
        this->set_int(1);
        //num2
    ...
    {
        ObExprObj res_obj;
        ret = this->div(expr_obj, res_obj, false);
...
/*
  exp: ��decimal(p,s)���ͣ�����round()����������;���
  digit > 0 ʱ�� �������뵽ָ����С��λ
          digit < scaleʱ�����Ϊdecimal(p-s+1+digit, digit)
          digit >=scaleʱ�����Ϊdecimal(p+digit-scale, digit)
             p+digit-scale>MAX_DECIMAL_DIGITʱ�����Ϊdecimal(MAX_DECIMAL_DIGIT,MAX_DECIMAL_SCALE)
  digit = 0 ʱ�� �������뵽��ӽ���С��λ�����Ϊdecimal(p-s+1, 0)
  digit < 0 ʱ�� ��С�����������������룬���Ϊdecimal(p-s+1, 0)
  */

void ObExprObj::round(const TypeDesc &type, const int64_t digit, TypeDesc &result_type)
{
    if(type.type_ == ObIntType || type.type_ == ObInt32Type || type.type_ == ObFloatType ||
            type.type_ == ObDoubleType || type.type_ == ObNullType)
    {
        result_type.type_ = type.type_ ;
    }
    else if(type.type_ == ObDecimalType)
    {
        result_type.type_ = ObDecimalType;
        if(digit > 0)
        {
            const uint32_t t_digit = static_cast<uint32_t>(digit);
            if(t_digit < type.scale_)
            {
                result_type.precision_ = type.precision_ - type.scale_ + 1 + t_digit;
                result_type.scale_ = t_digit;
            }
            else if (t_digit >= type.scale_)
            {
                uint32_t precision = type.precision_ + t_digit - type.scale_;
                result_type.precision_ = precision > static_cast<uint32_t>(MAX_DECIMAL_DIGIT) ? static_cast<uint32_t>(MAX_DECIMAL_DIGIT) : precision;
                result_type.scale_ = t_digit > static_cast<uint32_t>(MAX_DECIMAL_SCALE) ? static_cast<uint32_t>(MAX_DECIMAL_SCALE) : t_digit;
            }
        }
        else if(digit<=0)
        {
            result_type.precision_ = type.precision_ - type.scale_ + 1;
            result_type.scale_ = 0;
        }
    }
}
...
�ο�mysql
*/
void ObExprObj::concat(const TypeDesc &type1, const TypeDesc &type2, TypeDesc &result_type)
{
    ObObjType type[2];
    type[0] = type1.type_;
    type[1] = type2.type_;
    ...
    for(int32_t i = 0; i < 2; i++)
    {
        ...
        case ObFloatType:
            ...
            break;
        case ObVarcharType:
            len[i] = (i == 0) ? type1.length_ : type2.length_;
            break;
        case ObCreateTimeType:
        case ObModifyTimeType:
        case ObPreciseDateTimeType:
        ...
        case ObBoolType:
            ...
            break;
        case ObDecimalType:
            len[i] = (i == 0) ? type1.precision_ : type2.precision_;
            break;
        case ObDateType:
            len[i] = 10;
            break;
    ...
    result_type.length_ = len[0] + len[1];
}


/*
  exp:��substr(e1,pos,len)����
  ���ڷ���һ�����ַ�������ʼλ��pos������Ϊlen
  posΪ����ʱ��pos��λ�ô��ַ�����β����Ϊ0ʱ���ɿ���1
  lenС��0ʱ������posָʾ�ַ���λ�ò������ַ�ʱ�����ؿմ�
  */

void ObExprObj::substr(const TypeDesc &type, const int64_t pos, const int64_t len, TypeDesc &result_type)
{
    int64_t length = 0;
    switch (type.type_)
    {
    ...
    int64_t pos_value = (pos < 0) ? -pos : pos;
    uint32_t position = static_cast<uint32_t>(pos_value);
    position = (0 == position) ? 1 : position;

    if(len < 0 || length < position)
    {
        result_type.length_ = 0;
    }
    else if(len == 0)
    {
        result_type.length_ = static_cast<uint32_t>(length) - position + 1;
    }
    else
    {
        ...
        {
            result_type.length_ = static_cast<uint32_t>(len);
        }
    }
    result_type.type_ = ObVarcharType;
}
