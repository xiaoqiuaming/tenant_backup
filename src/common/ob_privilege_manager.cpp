  ...
  namespace common
  {
    ...
    int ObPrivilegeManager::release_privilege(const ObPrivilege ** pp_privilege)
    {
      ...
      if (NULL == pp_privilege)
      {
        ...
        YYSYS_LOG(WARN, "release privilege failed, ret=%d", ret);
      }
      else
      {
    ...
    int ObPrivilegeManager::renew_privilege(const ObPrivilege & privilege)
    {
      ...
      if (new_version < newest_privilege_version_)
      {
        ...
        YYSYS_LOG(WARN, "receive a older privilege version,local version=[%ld], incoming version=[%ld],ret=%d", newest_privilege_version_, new_version, ret);
      }
      else
      {
        int pos = -1;
        ...
        if (pos != -1)
        {
          ...
          YYSYS_LOG(INFO, "renew privilege success!!!");
