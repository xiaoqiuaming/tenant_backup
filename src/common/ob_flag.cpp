...
int oceanbase::common::string_copy(char* dst, const char* src, int64_t max_length)
{
  ...
  if(OB_SUCCESS == ret && NULL == src)
  {
    ...
    YYSYS_LOG(WARN, "src string is null");
  }
  else
  {
    src_len = strlen(src);
  ...
  if(OB_SUCCESS == ret && src_len >= max_length)
  {
    ...
    YYSYS_LOG(WARN, "src string length[%ld] is greater than dst buffer[%ld]", src_len, max_length);
  }
  if(OB_SUCCESS == ret)
  {
...
{ 
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "decoded string length[%ld] is greater than buf size[%ld]", len, max_length);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "decode string fail");
    }
  }
