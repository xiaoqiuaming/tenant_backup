...
namespace oceanbase
{
  namespace common
  {
    ...
    void get_table_name(const ObSchemaManagerV2& schema_mgr, const ObColumnSchemaV2& column, ObObj& value)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot get table schema with table id[%lu]", column.get_table_id());
      }
    }
    ...
    void get_start_pos(const ObSchemaManagerV2& schema_mgr, const ObColumnSchemaV2& column, ObObj& value)
    {
      ...
      {
        YYSYS_LOG(WARN, "column has no join info");
      }
    }
    ...
    void get_right_table_name(const ObSchemaManagerV2& schema_mgr, const ObColumnSchemaV2& column, ObObj& value)
    {
      ...
      if(NULL != joininfo)
      {
        ...
        {
          YYSYS_LOG(WARN, "cannot get table schema for table[%lu]", joininfo->join_table_);
        }
      }
      else
      {
...
int oceanbase::common::dump_joininfo(const ObSchemaManagerV2& schema_mgr, common::ObClientHelper* client_helper, ObMutator* mutator)
{
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "clean table[%s] fail:ret[%d]", OB_ALL_JOININFO_TABLE_NAME, ret);
  }
  ...
  for(const ObColumnSchemaV2* it=schema_mgr.column_begin();it != schema_mgr.column_end();it++)
  {
    ...
    for(uint32_t i=0;i<ARRAYSIZEOF(JOININFO_OR_MAP) && (OB_SUCCESS == ret);i++)
    {
      ...
      YYSYS_LOG(DEBUG, "table_name[%.*s], column[%.*s], %s", table_name.length(), table_name.ptr(),
      ...
      if(ObIntType == value.get_type())
      {
        int64_t tmp = 0;
        value.get_int(tmp);
        YYSYS_LOG(DEBUG, "value [%ld]", tmp);
      }
      else if(ObVarcharType == value.get_type())
      {
        ObString tmp;
        value.get_varchar(tmp);
        YYSYS_LOG(DEBUG, "value [%.*s]", tmp.length(), tmp.ptr());
      }

      ret = mutator->update(table_name, rowkey, column, value);
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add update to ob mutator fail:ret[%d]", ret);
      }
    }
  }
  return ret;
...
int oceanbase::common::dump_columns(const ObSchemaManagerV2& schema_mgr, common::ObClientHelper* client_helper, ObMutator* mutator)
{
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "clean table[%s] fail:ret[%d]", OB_ALL_COLUMN_TABLE_NAME, ret);
  }
...
int oceanbase::common::dump_tables(const ObSchemaManagerV2& schema_mgr, common::ObClientHelper* client_helper, ObMutator* mutator)
{
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "clean table[%s] fail:ret[%d]", FIRST_TABLET_TABLE_NAME, ret);
  }
  ...
  for(const ObTableSchema* it=schema_mgr.table_begin();(it != schema_mgr.table_end()) && (OB_SUCCESS == ret);it++)
  {
    ...
    //modify dolphin [database manager]@20150616:b  //[conflict_level B]
    ...
    for(uint32_t i = 0;i<ARRAYSIZEOF(TABLE_OR_MAP) && (OB_SUCCESS == ret);i++)
    {
      ...
      YYSYS_LOG(INFO, "table_name[%.*s], rowkey[%s], column[%.*s]",
...
int oceanbase::common::clean_table(const ObString& table_name, const ObString& select_column, common::ObClientHelper* client_helper, ObMutator* mutator)
{
  ...
  if(NULL == mutator)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "argument mutator is null");
    }
  }
  ...
  if(OB_SUCCESS == ret && NULL == client_helper)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "argument client_helper is null");
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "get thread specific ob scanner fail");
    }
    else
    {
      scanner->reset();
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "got %ld cells", scanner->get_size());
    }
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add del row to ob mutator fail:ret[%d]", ret);
      }
    }
  }
...
int oceanbase::common::dump_schema_manager(const ObSchemaManagerV2& schema_mgr, common::ObClientHelper* client_helper, const ObServer& update_server)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "arg client_helper is null");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "get thread specific ob mutator fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "dump tables fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "dump columns fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "dump joininfo fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "apply mutator schema table fail:ret[%d]", ret);
    }
  }
