...
int oceanbase::common::ob_init_memory_pool(int64_t block_size)
{
  ...
  {
    YYSYS_LOG(ERROR, "fixed_memory_pool.init(block_size=%ld)=>%d", block_size, ret);
  }
  else if (OB_SUCCESS != (ret = get_global_tsi_block_allocator().init(&sys_allocator)))
  {
    YYSYS_LOG(ERROR, "tsi_block_allocator.init()=>%d", ret);
  }
  else
  {
...
{
  YYSYS_LOG(DEBUG, "common memory_limit_callback");
}
...
//add support for arm platform by wangd 202106:e
    result = get_fixed_memory_pool_instance().malloc(nbyte_align,mod_id, got_size);//mod support for arm platform by wangd 202106
    ...
    int64_t *p = std::lower_bound(MON_BLOCK_SIZE_ARRAY, MON_BLOCK_SIZE_ARRAY+ARRAYSIZEOF(MON_BLOCK_SIZE_ARRAY), nbyte_align);//mod support for arm platform by wangd 202106
    ...
    BACKTRACE(
        ...
        "ob_malloc  addr=%p nbyte=%ld", result, nbyte_align
...
void oceanbase::common::ob_print_mod_memory_usage(bool print_to_std)
{
  ...
  YYSYS_LOG(INFO, "malloc size distribution\n%s", to_cstring(g_malloc_size_stat));
  for (uint32_t i = 0; i < ARRAYSIZEOF(MON_BLOCK_SIZE_ARRAY); ++i)
  {
    YYSYS_LOG(INFO, "[BLOCK_STAT] size=%ld times=%ld", MON_BLOCK_SIZE_ARRAY[i], MON_BLOCK_ARRAY[i]);
  } // end for 
}
...
#endif
//add support for arm platform by wangd 202106:e
  buf_ptr_ = ob_malloc(nbyte_align, ObModIds::OB_MEM_BUFFER); //mod support for arm platform by wangd 202106
  if (buf_ptr_ != NULL)
  {
    buf_size_ = nbyte_align;//mod support for arm platform by wangd 202106
  }
}
...
#endif
//add support for arm platform by wangd 202106:e
  if (nbyte_align <= buf_size_ && buf_ptr_ != NULL)//mod support for arm platform by wangd 202106
  {
    result = buf_ptr_;
  }
  else
  {
    ...
    buf_size_ = 0;
    buf_ptr_ = NULL;
    result = ob_malloc(nbyte_align,mod_id);//mod support for arm platform by wangd 202106
    if (result != NULL)
    {
      mod_id_ = mod_id;
      buf_size_ = nbyte_align;//mod support for arm platform by wangd 202106
...
#endif
//add support for arm platform by wangd 202106:e
  int64_t buf_len = size_align > buf_size_ ? size_align : buf_size_;//mod support for arm platform by wangd 202106

  if (size <= 0 || (NULL != buf_ptr_ && buf_size_ <= 0))
  {
    YYSYS_LOG(WARN, "invalid param, size=%ld, buf_ptr_=%p, "
                    "buf_size_=%ld",
              size_align, buf_ptr_, buf_size_);//mod support for arm platform by wangd 202106
    ret = OB_ERROR;
  }
  else if (NULL == buf_ptr_ || (NULL != buf_ptr_ && size_align > buf_size_))//mod support for arm platform by wangd 202106
  {
    new_buf = static_cast<char*>(ob_malloc(buf_len, mod_id));
    if (NULL == new_buf)
    {
      YYSYS_LOG(ERROR, "Problem allocate memory for buffer");
      ret = OB_ERROR;
    }
    else
    {
