  ...
  namespace common
  {
    ...
    int ObToken::set_username(const ObString &username)
    {
      ...
      if(username.ptr() == NULL)
      {
        ...
        YYSYS_LOG(WARN, "invlid username. err=%d", err);
      }
      if(OB_SUCCESS == err)
      {
        ...
        {
          err = OB_ERROR;
          YYSYS_LOG(WARN, "fail to write data to username");
        }
      }
      return err;
    }
    ...
    /*    int ObToken::set_password(const ObString &password)
          ...
          {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "invlid password. err=%d", err);
          }
          if(OB_SUCCESS == err)
          {
          ...
          YYSYS_LOG(WARN, "fail to allocate memory. err=%d", err);
          }
          else
          {
          ...
          {
          err = OB_ERROR;
          YYSYS_LOG(WARN, "fail to write data to password");
          }
          }
          }
          return err;
    ...
    int ObToken::set_timestamp(const int64_t timestamp)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument. err=%d", err);
      }
      if(err == OB_SUCCESS)
      {
    ...
    int ObToken::encrypt(const int64_t skey_version, const int64_t skey)
    {
      ...
      if(err == OB_SUCCESS)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to encrypt. err=%d", err);
        }
      }
      return err;
    }
    int ObToken::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      if(OB_SUCCESS == err)
      {
        if(buffer_ == NULL)
        {
          ...
          YYSYS_LOG(WARN, "data have't encrypt. err=%d", err);
        }
      }
      if(OB_SUCCESS == err)
      {
    ...
    int ObToken::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      if(OB_SUCCESS == err)
      {
        ...
        {
          err = OB_NOT_A_TOKEN;
          //YYSYS_LOG(INFO, "not have teken, err=%d", err);
        }
      }
      if(OB_SUCCESS == err)
      {
        ...
        if(cell_num != OB_CELL_NUM)
        {
          ...
          YYSYS_LOG(INFO, "token timeout, should renew it");
        }
      }
      if(OB_SUCCESS == err)
      {
    ...
    int ObToken::decrypt(const int64_t skey)
    {
      ...
      {
        err = OB_INNER_STAT_ERROR;
        YYSYS_LOG(WARN, "inner stat error.");
      }
      else
      {
        DES_crypt crypt;
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "decrypt fail .err=%d", err);
        }
      }
      return err;
    }
