...
namespace oceanbase
{
  namespace common
  {
      ...
      public:
                  ...
                  ObConfigItemType type, const char* info)
        {
          ...
          else
          {
            ...
            if (!set_value(def))
            {
              YYSYS_LOG(ERROR, "Set config item value fail! name: [%s], value: [%s]", name, def);
            }
            set_type(type);
            set_info(info);
          }
        ...
        : public ObConfigItem
    {
      public:
                  ...
                  const char* info)
        {
          ...
          if (NULL == range)
          {
            YYSYS_LOG(ERROR, "Range is NULL");
          }
          else if (!parse_range(range))
          {
            YYSYS_LOG(ERROR, "Parse check range fail! range: [%s]", range);
          }
        }
        ...
        {
          bool valid = true;
          return get() > get(str, valid) && valid;
        }
        bool operator >=(const char* str) const
        {
          bool valid = true;
    ...
    /**
      *@brief   ÿ�պϲ�ʱ����ɵ�һʱ����趨�����Ϊ����ϲ�ʱ���;Ŀǰ�ݶ�ΪMAXNUM
      *@author  ��ŷ��
      *@date    20141125
      *@version 0.1
      */
