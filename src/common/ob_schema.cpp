...
namespace
{
  ...
  //add 20151008:e
  const char* STR_COLUMN_TYPE_BOOL = "bool";
  ...
  const char *STR_WITH_CHECK_OPTION = "with_check_option";
  const char *STR_SQL_TEXT = "sql_text";
...
namespace oceanbase
{
  namespace common
  {
    ...
    {
      size_ = 0 ;//add by maosy [MultiUps 1.0] [secondary index optimize]20170509
    }
    ...
    int ObRowkeyInfo::get_column(const int64_t index, ObRowkeyColumn& column) const
    {
      ...
      {
        YYSYS_LOG(WARN, "Invalid argument index=%ld, split_size=%ld", index, size_);
    ...
    int ObRowkeyInfo::add_column(const ObRowkeyColumn& column)
    {
      ...
      {
        YYSYS_LOG(WARN, "No more column can be add, split_size=%ld, OB_MAX_ROWKEY_COLUMN_NUMBER=%ld",
    ...
    int ObRowkeyInfo::set_column(int64_t idx, const ObRowkeyColumn& column)
    {
      ...
      if (0 > idx || OB_MAX_ROWKEY_COLUMN_NUMBER <= idx)
      {
        ...
        YYSYS_LOG(WARN, "invalid idx=%ld", idx);
      }
      else
      {
        columns_[idx] = column;
      ...
      default_value_(), column_group_next_(NULL)
    {
      ...
      default_val_len_ = 0;
      has_sync_ = false;
      default_val_obj_.set_type(ObMinType);
    ...
    const char *ObColumnSchemaV2::get_default_val() const
    {
      return default_val_;
    }

    void ObColumnSchemaV2::set_default_val(const char *default_val)
    {
      if(NULL != default_val && '\0' != *default_val)
      {
        snprintf(default_val_, sizeof(default_val_), "%s", default_val);
        default_val_len_ = static_cast<int>(strlen(default_val_));; //[437]
      }
      default_value_is_null = false;
    }

    int ObColumnSchemaV2::calc_default_val_obj(ObObj &val)
    {
      int ret = OB_SUCCESS;
      if(default_val_len_ == 0)
      {
        val.set_varchar(ObString::make_string(default_val_));
      }
      else if(default_val_len_ > 0)
      {
        switch(type_)
        {
          case ObIntType:
          case ObInt32Type:
            val.set_int(atol(default_val_));
            break;
          case ObVarcharType:
            val.set_varchar(ObString::make_string(default_val_));
            break;
          case ObDecimalType:
            val.set_varchar(ObString::make_string(default_val_));
            val.set_type(ObDecimalType);
            break;
          case ObPreciseDateTimeType:
            val.set_varchar(ObString::make_string(default_val_));
            val.set_type(ObPreciseDateTimeType);
            break;
          case ObDateType:
            val.set_varchar(ObString::make_string(default_val_));
            val.set_type(ObDateType);
            break;
          case ObTimeType:
            val.set_varchar(ObString::make_string(default_val_));
            val.set_type(ObTimeType);
            break;
          case ObBoolType:
            val.set_bool(atoi(default_val_) > 0 ? true : false);
            break;
          case ObFloatType:
            val.set_float(static_cast<float>(atof(default_val_)));
            break;
          case ObDoubleType:
            val.set_double(atof(default_val_));
            break;
          default:
            ret = OB_ERR_ILLEGAL_TYPE;
            YYSYS_LOG(ERROR, "Illigeal type of default value");
            break;
        }
      }
      return ret;
    ...
    {
      YYSYS_LOG(INFO,"COLUMN:(%lu,%lu,%lu:%s)",table_id_,column_group_id_,column_id_,name_);
      YYSYS_LOG(INFO,"JOIN  :(%lu,%lu,%lu)",join_info_.join_table_,join_info_.left_column_count_,
                join_info_.correlated_column_);
    }
    //add zhaoqiong [Schema Manager] 20150327:b
    void ObColumnSchemaV2::print_debug_info() const
    {
      YYSYS_LOG(TRACE,"COLUMN:(%lu,%lu,%lu:%s)",table_id_,column_group_id_,column_id_,name_);
      YYSYS_LOG(TRACE,"JOIN  :(%lu,%lu,%lu)",join_info_.join_table_,join_info_.left_column_count_,
                join_info_.correlated_column_);
    }
    //add:e
    ...
    ColumnType ObColumnSchemaV2::convert_str_to_column_type(const char* str)
    {
      ...
      //add 20151008:e
      else if (strcmp(str, STR_COLUMN_TYPE_BOOL) == 0)
      {
        type = ObBoolType;
      }
      ...
      {
        YYSYS_LOG(ERROR, "column type %s not be supported", str);
      }
      return type;
    }
    ...
    DEFINE_SERIALIZE(ObColumnSchemaV2)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ret = serialization::encode_bool(buf, buf_len, tmp_pos, default_value_is_null);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ret = serialization::encode_vstr(buf, buf_len, tmp_pos, default_val_);
      }
    ...
    int ObColumnSchemaV2::deserialize_v5(const char* buf, const int64_t data_len, int64_t& pos)
    {
      int ret = OB_SUCCESS;
      int64_t tmp_pos = pos;
      ...
      if (OB_SUCCESS == ret)
      {
        ret = serialization::decode_bool(buf, data_len, tmp_pos, &default_value_is_null);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        int64_t len2 = 0;
        serialization::decode_vstr(buf, data_len, tmp_pos,
                                   default_val_, OB_MAX_DEBUG_MSG_LEN, &len2);
        if (-1 == len2)
        {
          ret = OB_ERROR;
        }
    ...
    {
      return deserialize_v5(buf, data_len, pos);
    }
    ...
    DEFINE_GET_SERIALIZE_SIZE(ObColumnSchemaV2)
    {
      ...
      len += serialization::encoded_length_bool(default_value_is_null);
      ...
      len += serialization::encoded_length_vstr(default_val_);
    ...
    ObTableSchema::ObTableSchema() : table_id_(OB_INVALID_ID),
      ...
      version_(OB_SCHEMA_VERSION_EIGHT_FIRST),
      //mod:e
      schema_version_(0),
    ...
    /*
     *ɾ����add_index_list�Ľӿ� //repaired from messy code by zhuxh 20151014
     **/
    /* int ObTableSchema::add_index_list(uint64_t tid)
    {
        ...
        if(OB_SUCCESS!=ret)
            YYSYS_LOG(ERROR,"add index_list_ error,one table has no more 5 index!");
            return ret;
    ...
    */
    ...
    //[view]
    void ObTableSchema::set_text(const char *text)
    {
        if(text != NULL && *text != '\0')
        {
    ...
    void ObTableSchema::set_text(const ObString &text)
    {
        if(text.ptr() != NULL && static_cast<uint32_t>(text.length()) < sizeof(text_))
        {
            snprintf(text_, text.length() + 1, "%s", text.ptr());
        }
    }

    void ObTableSchema::set_table_type(ObTableSchema::ObTableType type)
    {
        type_ = type;
    }

    const char *ObTableSchema::get_text() const
    {
        return text_;
    }

    ObTableSchema::ObTableType ObTableSchema::get_type() const
    {
        return type_;
    }

    void ObTableSchema::set_check_option(ObWithCheckOption with_check_option)
    {
        with_check_option_ = with_check_option;
    }

    ObTableSchema::ObWithCheckOption ObTableSchema::get_check_option() const
    {
        return with_check_option_;
    }

    void ObTableSchema::set_is_updatable(bool is_updatable)
    {
        is_updatable_ = is_updatable;
    }

    bool ObTableSchema::get_is_updatable() const
    {
    ...
    DEFINE_SERIALIZE(ObTableSchema)
    {
      ...
      if(OB_SUCCESS==ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to serialize ih_array_, err=%d", ret);
          //break;
        }
        //}
      }
      ...
      *ɾ����index_list_���������л��Ľӿڹر�
       if(OB_SUCCESS==ret)
       {
           for(int64_t i=0;i<OB_MAX_INDEX_NUMS;i++){
               ...
               {
                   YYSYS_LOG(WARN,"failed to serialized index_list,t_id[%ld],i[%ld]",index_list_[i],i);
               }
                YYSYS_LOG(ERROR,"test::whx index_list_[%ld]",index_list_[i]);

           }
       }
       */
      //[view]
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && rowkey_info_.get_size() <= 0)
      {
          ret = serialization::encode_vi32(buf, buf_len, tmp_pos, type_);
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
          ret = serialization::encode_vi32(buf, buf_len, tmp_pos, static_cast<int32_t>(with_check_option_));
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
          ret = serialization::encode_bool(buf, buf_len, tmp_pos, is_updatable_);
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
          ret = serialization::encode_vstr(buf, buf_len, tmp_pos, text_);
      }
      //add e
      if (OB_SUCCESS == ret)
      {
    ...
    int ObTableSchema::deserialize_v3(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        int64_t len = 0;
        serialization::decode_vstr(buf, data_len, tmp_pos,
                                   name_, OB_MAX_COMPLETE_TABLE_NAME_LENGTH, &len); //[339]
        if (-1 == len)
        {
          ret = OB_ERROR;
        }
    ...
    int ObTableSchema::deserialize_v4(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        int64_t len = 0;
        serialization::decode_vstr(buf, data_len, tmp_pos,
                                   name_, OB_MAX_COMPLETE_TABLE_NAME_LENGTH, &len);
        if (-1 == len)
        {
          ret = OB_ERROR;
        }
      ...
      if(OB_SUCCESS==ret && get_version() >= OB_SCHEMA_VERSION_SIX_FIRST)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to deserialize index helper:ret[%d]", ret);
        }
        else
        {
          ih_=ih;
       ...
       if(OB_SUCCESS==ret)
        {
            ...
            for(int64_t i=0;i<OB_MAX_INDEX_NUMS;i++)
            {
                ...
                {
                    YYSYS_LOG(WARN,"failed to decode idx_id,i[%ld]",idx_id);
                    break;
                }
                else
                {
                    index_list_[i]=idx_id;
                    YYSYS_LOG(ERROR,"test::whx idx_id idx_id,[%ld]",idx_id);
                }
            }
        }
        */
      ...
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && rowkey_info_.get_size() <= 0)
      {
          int32_t type = 0;
          ret = serialization::decode_vi32(buf, data_len, tmp_pos, &type);
          type_ = static_cast<ObTableType>(type);
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
          int32_t check_option = 0;
          ret = serialization::decode_vi32(buf, data_len, tmp_pos, &check_option);
          with_check_option_ = static_cast<ObWithCheckOption>(check_option);
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
          bool is_updatable = false;
          ret = serialization::decode_bool(buf, data_len, tmp_pos, &is_updatable);
          is_updatable_ = is_updatable;
      }
      if(OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_EIGHT_FIRST && type_ == ObTableSchema::VIEW)
      {
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0;i < count;++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "deserialize value failed, ret=%d", ret);
          }
          if (OB_SUCCESS != ret)
          {
            break;
    ...
    DEFINE_GET_SERIALIZE_SIZE(ObTableSchema)
    {
      ...
      //add e
      //[view]
      if(OB_SCHEMA_VERSION_EIGHT_FIRST <= version_ && rowkey_info_.get_size() <= 0)
      {
          len += serialization::encoded_length_vi32(type_);
          if (type_ == ObTableSchema::VIEW)
          {
            len += serialization::encoded_length_vi32(with_check_option_);
            len += serialization::encoded_length_bool(is_updatable_);
            len += serialization::encoded_length_vstr(text_);
          }
      }
    ...
    ObSchemaManagerV2::ObSchemaManagerV2(): schema_magic_(OB_SCHEMA_MAGIC_NUMBER),
      ...
      version_(OB_SCHEMA_VERSION_EIGHT),
      //mod:e
    ...
    ObSchemaManagerV2::~ObSchemaManagerV2()
    {
      ...
      if(ts_hash_inited_)
      {
        tid_hash_map_.destroy();
        tname_hash_map_.destroy();
      }
      //add wenghaixing [secondary index] 20141105
      if(index_hash_init_)
      {
    ...
    ObSchemaManagerV2& ObSchemaManagerV2::operator=(const ObSchemaManagerV2& schema) //ugly,for hashMap
    {
      if (this != &schema)
      {
        ...
        if (is_completion_)
        {
          //[602]
          int err = OB_SUCCESS;
          if(OB_SUCCESS != (ret = sort_table()))
          {
            YYSYS_LOG(ERROR, "failed to sort tables in schema manager, err=%d", ret);
          }
          else if(OB_SUCCESS != (ret = sort_column()))
          {
            YYSYS_LOG(ERROR, "failed to sort columns in schema manager, err=%d", ret);
          }
          if(OB_SUCCESS != (err = init_index_hash()))
          {
            YYSYS_LOG(ERROR, "init index hash failed:err=%d", err);
          }
    ...
    const ObTableSchema* ObSchemaManagerV2::get_table_schema(const char* table_name) const
    {
      ...
      {
        if(!ts_hash_inited_ || !use_ts_hash_)
        {
          ObTableSchema tmp;
        ...
        {
          ObString k;
          k.assign_ptr(const_cast<char *>(table_name), static_cast<int32_t>(strlen(table_name)));
          table = get_table_schema(k);
        }
      }
      return table;
    }
    ...
    const ObTableSchema* ObSchemaManagerV2::get_table_schema(const ObString& table_name) const
    {
      ...
      if (table_name.ptr() != NULL && table_name.length() > 0 && table_nums_ > 0)
      {
        ...
        else
        {
          ...
          ObString k;
          k = table_name;
          ObTableSchema *v = NULL;
          err = tname_hash_map_.get(k, v);
          ...
          {
            YYSYS_LOG(WARN, "using tname_hash_map_ found table but v is null");
          }
          //[694]
          if(table == NULL || -1 == err || hash::HASH_NOT_EXIST == err)
          {
              table = std::find(table_infos_, table_infos_ + table_nums_, table_name);
              if(table == (table_infos_ + table_nums_))
              {
    ...
    const ObTableSchema* ObSchemaManagerV2::get_table_schema(const uint64_t table_id) const
    {
      ...
      if (table_nums_ > 0)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "using tid_hash_map_ found table but v is null");
          }
          //[694]
          if(table == NULL || -1 == err || hash::HASH_NOT_EXIST == err)
          {
    ...
    uint64_t ObSchemaManagerV2::get_data_table_td(const uint64_t table_id)const
    {
      uint64_t data_tid = OB_INVALID_ID;
      const ObTableSchema *table = get_table_schema(table_id);
      if (NULL != table)
      {
        data_tid = table->get_index_helper().tbl_tid;
      }
      return data_tid;
    }
    ...
    struct __table_sort
    {
        __table_sort(yysys::CConfig& config): config_(config)
        {}
        ...
        yysys::CConfig& config_;
    };


    bool ObSchemaManagerV2::parse_from_file(const char* file_name, yysys::CConfig& config)
    {
      bool parse_ok = true;

      if (parse_ok && file_name != NULL && config.load(file_name) != EXIT_SUCCESS)
      {
        YYSYS_LOG(ERROR, "can not open config file, file name is %s", file_name);
        parse_ok = false;
      }

      YYSYS_LOG(DEBUG,"config:%p",&config);
      ...
      if (parse_ok)
      {
        ...
        {
          YYSYS_LOG(ERROR, "parse [%s]  %s error", STR_SECTION_APP_NAME, STR_KEY_APP_NAME);
          parse_ok = false;
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "we limit our table id less than %lu", MAX_ID_USED);
            parse_ok = false;
          }
          if (max_table_id_ < 1)
          {
            YYSYS_LOG(ERROR, "max_table_id is %lu", max_table_id_);
            parse_ok = false;
          }
          ...
          //          {
          //            YYSYS_LOG(ERROR, "we limit our schema version less than %ld",
          //                      OB_SCHEMA_VERSION_FOUR);
          version_ = config.getInt(STR_SECTION_APP_NAME, STR_SCHEMA_VERSION,
          ...
          {
            YYSYS_LOG(ERROR, "version_ is %d, must greater than %ld",
          ...
          {
            YYSYS_LOG(ERROR, "%ld error table number", table_nums_);
            parse_ok = false;
            table_nums_ = 0;
          }
          else if (table_nums_ < 1)
          {
            YYSYS_LOG(INFO, "not table schema in file.");
            table_nums_ = 0;
          }
              ...
              it != sections.end() && parse_ok; ++it)
          {
            ...
            YYSYS_LOG(DEBUG,"table name :%s ",it->c_str());
          ...
          //[639]
          if(parse_ok && sort_table() != OB_SUCCESS)
          {
              YYSYS_LOG(ERROR, "sort table failed");
              parse_ok = false;
              init_index_hash();
          }
          ...
          {
            YYSYS_LOG(ERROR,"sort column failed");
            parse_ok = false;
            init_index_hash();
          }
    ...
    bool ObSchemaManagerV2::parse_one_table(const char* section_name, yysys::CConfig& config, ObTableSchema& schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "table_name is missing");
        parse_ok = false;
      }
      if (parse_ok)
      {
        ...
        if (table_id == 0)
        {
          YYSYS_LOG(ERROR, "table id can not be 0");
          parse_ok = false;
        }
        if ( /*(table_id < OB_APP_MIN_TABLE_ID) ||*/ (table_id > max_table_id_))
        {
          YYSYS_LOG(ERROR, "table id %lu greater than max_table_id %lu or less than %lu",
      ...
      {
        YYSYS_LOG(ERROR, "table id %lu have been used", table_id);
        parse_ok = false;
      }
      ...
      if (parse_ok)
      {
        ...
        {
          YYSYS_LOG(ERROR, "%d is a invalid table type", type);
          parse_ok = false;
        }
      }
      ...
      if (parse_ok)
      {
        ...
        if (max_column_id < 1)
        {
          YYSYS_LOG(ERROR, "max_column_id is %lu", max_column_id);
          parse_ok = false;
        }
        if (max_column_id > MAX_ID_USED)
        {
          YYSYS_LOG(ERROR, "we limit our column id less than %lu", MAX_ID_USED);
          parse_ok = false;
        }
      }
      ...
      if (parse_ok)
      {
        ...
        if (block_size < 0)
        {
          YYSYS_LOG(ERROR, "block_size is %d, must >= 0",
                    block_size);
          parse_ok = false;
        }
      }
      ...
      if (parse_ok)
      {
          int table_type_parse = config.getInt(section_name, STR_TYPE, 0);
          switch (table_type_parse)
          {
          case ObTableSchema::NORMAL:
              schema.set_table_type(ObTableSchema::NORMAL);
              break;
          case ObTableSchema::INDEX:
              schema.set_table_type(ObTableSchema::INDEX);
              break;
          case ObTableSchema::META:
              schema.set_table_type(ObTableSchema::META);
              break;
          case ObTableSchema::VIEW:
              schema.set_table_type(ObTableSchema::VIEW);
              break;
          default:
              parse_ok = false;
              break;
      ...
      if (parse_ok)
      {
          int check_option = config.getInt(section_name, STR_WITH_CHECK_OPTION, 0);
          switch (check_option)
          {
          case ObTableSchema::OB_WITH_NONE_CHECK_OPTION:
              schema.set_check_option(ObTableSchema::OB_WITH_NONE_CHECK_OPTION);
              break;
          case ObTableSchema::OB_WITH_CASCADED_CHECK_OPTION:
              schema.set_check_option(ObTableSchema::OB_WITH_CASCADED_CHECK_OPTION);
              break;
          case ObTableSchema::OB_WITH_LOCAL_CHECK_OPTION:
              schema.set_check_option(ObTableSchema::OB_WITH_LOCAL_CHECK_OPTION);
              break;
          default:
              parse_ok = false;
              break;
      ...
      if (parse_ok)
      {
          const char *sql_text = config.getString(section_name, STR_SQL_TEXT, "none");
          schema.set_text(sql_text);
      }
      ...
      if (parse_ok)
      {
        ...
        //add e
        int64_t replica_num =
            config.getInt(section_name, STR_REPLICA_NUM, 2);
        //mod:e
        if (expire_frequency < 1)
        {
          YYSYS_LOG(ERROR, "expire_frequency is %ld, must greater than 0",
                    expire_frequency);
          parse_ok = false;
        }
        else if (max_sstable_size < 0)
        {
          YYSYS_LOG(ERROR, "max_sstable_size is %ld, must >= 0",
                    max_sstable_size);
          parse_ok = false;
        }
        else if (expire_time < 0)
        {
          YYSYS_LOG(ERROR, "query_cache_expire_second is %ld, must >= 0",
                    expire_time);
          parse_ok = false;
        }
        else if (expire_effect < 0)
        {
          YYSYS_LOG(ERROR, "is_expire_effect_immediately is %ld, must >= 0",
                    expire_time);
          parse_ok = false;
        }
        else if (max_scan_rows < 0)
        {
          YYSYS_LOG(ERROR, "max_scan_rows_per_tablet is %ld, must >= 0",
                    max_scan_rows);
          parse_ok = false;
        }
        else if (scan_size < 0 || scan_size >= OB_MAX_PACKET_LENGTH)
        {
          YYSYS_LOG(ERROR, "internal_ups_scan_size is %ld, must >= 0",
                    scan_size);
          parse_ok = false;
        }
        //add zhaoqiong[roottable tablet management]20150409:b
        ...
        {
          YYSYS_LOG(ERROR, "internal_replica_num is %ld, must > 0",
                    replica_num);
          parse_ok = false;
        }
        // add e
    ...
    bool ObSchemaManagerV2::parse_column_info(const char* section_name, yysys::CConfig& config, ObTableSchema& schema)
    {
      bool parse_ok = true;
      char *str = NULL;
      uint64_t id = OB_INVALID_ID;
      ...
      {
        YYSYS_LOG(ERROR,"alloc columns_info failed");
        parse_ok = false;
      }
          ...
          it != column_info_strs.end() && parse_ok; ++it)
      {
        ...
        str = str_trim(str);
        yysys::CStringUtil::split(str, ",", node_list);
        if (node_list.size() < POS_COLUM_TYPE + 1)
        {
          YYSYS_LOG(ERROR, "parse column |%s| error", str);
          parse_ok = false;
        }
        if (parse_ok)
        {
          ...
          {
            YYSYS_LOG(ERROR, "maintained %lu is unacceptable", maintained);
            parse_ok = false;
          }
          if (maintained == 1)
          {
        ...
        if (parse_ok)
        {
          ...
          if (id == 0 )
          {
            YYSYS_LOG(ERROR, "column id error id = %lu", id);
            parse_ok = false;
          }
        }
        if (parse_ok)
        ...
        if (parse_ok)
        {
          ...
          if (type == ObNullType)
          {
            YYSYS_LOG(ERROR, "column type error |%s|", node_list[POS_COLUM_TYPE]);
            parse_ok = false;
          }
        }
        ...
        if (parse_ok && type == ObVarcharType)
        {
          ...
          {
            YYSYS_LOG(ERROR, "column type need size |%s|", node_list[POS_COLUM_TYPE]);
            parse_ok = false;
          }
          if (parse_ok)
          {
            ...
            if (size <= 0)
            {
              YYSYS_LOG(ERROR, "column type size error |%s|", node_list[POS_COLUM_SIZE]);
              parse_ok = false;
            }
          }
        }
        ...
        if (parse_ok && type == ObCreateTimeType)
        {
          ...
          {
            YYSYS_LOG(ERROR, "more than one column have create time type");
            parse_ok = false;
          }
          else
          {
        ...
        if (parse_ok && type == ObModifyTimeType)
        {
          ...
          {
            YYSYS_LOG(ERROR, "more than one column have modify time type");
            parse_ok = false;
          }
          else
          {
      ...
      if (parse_ok)
      {
        ...
        {
          YYSYS_LOG(WARN, "schema file has no create_time column, and the default create_time column id is used");
          parse_ok = false;
        }
        if (!has_modify_time_column && has_modify_time_id)
        {
          YYSYS_LOG(WARN, "schema file has no modify_time column, and the default modify_time column id is used");
          parse_ok = false;
        }
      }
      ...
      if (parse_ok)
      {
        ...
        {
          YYSYS_LOG(ERROR,"we limit the column groups num less than %ld",OB_MAX_COLUMN_GROUP_NUMBER);
          parse_ok = false;
        }
      }
      ...
      if (parse_ok && !column_group_infos.empty() )
      {
            ...
            it != column_group_infos.end() && parse_ok; ++it)
        {
          ...
          yysys::CStringUtil::split(group_str, ",", column_list);
          if (column_list.size() < 2)
          {
            YYSYS_LOG(ERROR, "parse column group |%s| error", group_str);
            parse_ok = false;
          }
          ...
          if (parse_ok)
          {
            for(uint32_t i=1;i < column_list.size() && parse_ok; ++i)
            {
              ...
              if (NULL == col)
              {
                YYSYS_LOG(ERROR,"can't find column |%s|",column_list[i]);
                parse_ok = false;
              }
              else
              {
        ...
        for (int32_t index = 0; index < column_index; ++index)
        {
          if (0 == exist_in_column_group_info[index])
          {
            ...
            {
              YYSYS_LOG(ERROR,"can't add column |%s|",col->get_name());
              parse_ok = false;
            }
            else
            {
          ...
          if (parse_ok)
          {
            for(uint32_t i=1;i < column_list.size() && parse_ok; ++i)
            {
              ...
              else
              {
                ...
                if (add_column(*col) != OB_SUCCESS)
                {
                  YYSYS_LOG(ERROR,"can't add column |%s|",column_list[i]);
                  parse_ok = false;
                }
                else
                {
        ...
        {
          YYSYS_LOG(ERROR,"there is a column not belongs any column group");
          parse_ok = false;
        }
      }
      else if (parse_ok && column_group_infos.empty() )
      {
        for(int32_t i=0; i < column_index && parse_ok; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR,"add column |%s| failed",columns_info[i].get_name());
            parse_ok = false;
          }
        }
      }
    ...
    bool ObSchemaManagerV2::parse_rowkey_column(const char* column_str, ObRowkeyColumn& column, ObTableSchema& schema)
    {
      ...
      if (NULL != column_str)
      {
        ...
        {
          YYSYS_LOG(ERROR, "column (%s) not exist, cannot be part of rowkey.", s);
          parse_ok = false;
        }
        else
        {
        ...
        if (parse_ok && NULL != p)
        {
          ...
          if (NULL == p)
          {
            YYSYS_LOG(ERROR, "rowkey compatible format error:%s", s);
            parse_ok = false;
          }
    ...
    bool ObSchemaManagerV2::parse_rowkey_info(const char* section_name, yysys::CConfig&  config, ObTableSchema& schema)
    {
      bool parse_ok = true;
      ...
      if (NULL != rowkey_column_str)
      {
        ...
        yysys::CStringUtil::split(clone_column_str, ",", column_list);
    ...
    int ObSchemaManagerV2::set_app_name(const char* app_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "app name is NULL or empty");
    ...
    int ObSchemaManagerV2::add_index_in_map(ObTableSchema *tschema)
    {
      ...
      if(NULL == tschema)
      {
        YYSYS_LOG(WARN,"NULL pointer of tschema!");
        ret=OB_ERROR;
      }
      else if(OB_INVALID_ID != tschema->get_index_helper().tbl_tid)
      {
        ...
        if(hash::HASH_NOT_EXIST == id_index_map_.get(tid,il))
        {
          ...
          {
            YYSYS_LOG(WARN,"add in hash map faile tid[%ld]!",tid);
            ret = OB_ERROR;
          }
          else
          {
        ...
        else if(hash::HASH_EXIST == id_index_map_.get(tid,il))
        {
          if(OB_SUCCESS != il.add_index(tschema->get_table_id()))
          {
            YYSYS_LOG(WARN,"index_list is full!");
            ret = OB_ERROR;
          }
          else
          {
        ...
        // YYSYS_LOG(ERROR,"test,whx::whx::tid[%ld]",tid);
        //index_list l2;
        // id_index_map_.get(tid,il);
        //uint64_t id2;
        // il.get_idx_id(0,id2);
        // YYSYS_LOG(ERROR,"test,whx::whx::il_count[%ld],il(0)[%ld]",il.get_count(),id2);
        //add e
      }
    ...
    const int ObSchemaManagerV2::get_index_list(uint64_t table_id, IndexList &out) const
    {
      ...
      if(!index_hash_init_)
      {
        YYSYS_LOG(ERROR,"index hash is not inited!");
        ret = OB_ERROR;
      }
      else if(-1 == id_index_map_.get(table_id,out))
      {
        YYSYS_LOG(ERROR,"get index list for drop table err");
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int ObSchemaManagerV2::get_index_columns(uint64_t table_id, uint64_t index_table_id, common::ObArray<uint64_t> &column_array) const
    {
      ...
      if(NULL == schema)
      {
        YYSYS_LOG(WARN, "failed to get table schema,table[%ld]",table_id);
      ...
      YYSYS_LOG(INFO, "index table[%ld] has [%ld] columns", table_id, column_array.count());
      return ret;
    }
    ...
    int ObSchemaManagerV2::get_index_column_num(uint64_t& table_id, int64_t& num) const
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get index list for cal col num");
        ret = OB_ERROR;
      }
      else
      {
      ...
      YYSYS_LOG(INFO,"table[%ld] has [%ld] index columns",table_id, num);
      return ret;
    }
    ...
    int ObSchemaManagerV2::get_init_index(uint64_t *table_id, int64_t &size) const
    {
      ...
      for( ; iter != id_index_map_.end(); iter++)
      {
        ...
        for(int64_t loop = 0; loop < list.get_count(); loop ++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get index table id failed, ret[%d]", ret);
            ret = OB_SCHEMA_ERROR;
            break;
          }
                                                                 ...
                                                                 int32_t* idx /*=NULL*/) const
    {
      ...
      if (OB_INVALID_ID != table_id && OB_INVALID_ID != column_id)
      {
        ...
        {
          YYSYS_LOG(ERROR,"found column but v.head_ is NULL");
        }
      }
      return column;
    }
    ...
    const ObColumnSchemaV2* ObSchemaManagerV2::get_table_schema(const uint64_t table_id, int32_t& size) const
    {
      ...
      {
        YYSYS_LOG(ERROR,"table id invalid [%lu]",table_id);
        err = OB_ERROR;
      }
                                                                ...
                                                                const uint64_t column_group_id,int32_t& size) const
    {
      ...
      {
        YYSYS_LOG(ERROR,"column group id invalid [%lu]",column_group_id);
        err = OB_ERROR;
      }
    ...
    int ObSchemaManagerV2::prepare_column_storage(const int64_t column_num, bool need_reserve_space)
    {
      ...
      else if (column_capacity_ < column_num)
      {
        ...
        {
          YYSYS_LOG(ERROR, "allocate memory for copy schema failed. %ld", column_num);
    ...
    int ObSchemaManagerV2::add_column_without_sort(ObColumnSchemaV2& column)
    {
      ...
      if (NULL == table)
      {
        YYSYS_LOG(ERROR,"can't find this table:%lu",column.get_table_id());
        ret = OB_ENTRY_NOT_EXIST;
      }
      else if (column.get_id() < COLUMN_ID_RESERVED )
      {
        YYSYS_LOG(ERROR,"column id %ld reserved for internal usage.", column.get_id());
        ret = OB_INVALID_ARGUMENT;
      }
        ...
        // modify e
      {
        ...
        //YYSYS_LOG(ERROR,"column id %lu greater thean max_column_id %lu",
        //column.get_id(), table->get_max_column_id());
        YYSYS_LOG(ERROR,"column id %lu greater thean max_column_id %lu,table name=[%s]",
    ...
    int ObSchemaManagerV2::add_column(ObColumnSchemaV2& column)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR,"column %s has already added",column.get_name());
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
        ...
        //   )
        //{
        //  YYSYS_LOG(ERROR,"table id,column group id must in order,prev:(%lu,%lu),"
    ...
    int ObSchemaManagerV2::add_table(ObTableSchema& table)
    {
      ...
      for(int32_t i=0; i < table_nums_; ++i)
      {
        ...
        if (0 == strncmp(table_infos_[i].get_table_name(),table.get_table_name(),
            ...
            table.get_table_id() == table_infos_[i].get_table_id())
        {
          YYSYS_LOG(WARN,"can't add this table:[%ld,%s], table_infos[%d][table_id=%ld, table_name=%s]",
    ...
    void ObSchemaManagerV2::del_column(const ObColumnSchemaV2& column)
    {
      ...
      if ( (c = std::find(columns_,columns_ + column_nums_,column)) != columns_ + column_nums_)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "sort column failed:ret[%d]", ret);
        }
      }
      else
      {
        YYSYS_LOG(WARN,"column %s does not exist",column.get_name());
      }
    }
                                            ...
                                            int32_t index_array[],int32_t& size) const
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"%s:%s not found [%d]",table_name,column_name,ret);
        }
        else
        {
          int32_t i=0;
                                            ...
                                            int32_t index_array[],int32_t& size) const
    {
      ...
      else
      {
        ...
        if (-1 == ret || hash::HASH_NOT_EXIST  == ret)
        {
          YYSYS_LOG(WARN,"%lu:%lu not found [%d]",table_id,column_id,ret);
        }
        else
        {
          ObColumnSchemaV2 *tmp = info.head_;
    ...
    bool ObSchemaManagerV2::is_compatible(const ObSchemaManagerV2& schema_manager) const
    {
      ...
      for (int64_t i = 0; compatible && i < column_nums_; ++i)
      {
        ...
        if (NULL == column_left)
        {
          YYSYS_LOG(WARN, "should no be here. i=%ld", i);
          compatible = false;
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "should no be here, table_schema is NULL. table_id=%ld", left_table_id);
          compatible = false;
          break;
        }
        ...
        {
          YYSYS_LOG(INFO, "column is delete. table_name=%s, table_id=%lu, "
        ...
        {
          YYSYS_LOG(WARN, "column id has been change. column_name=%s, old_column_id=%lu, new_column_id=%lu, be cautious!",
        ...
        {
          YYSYS_LOG(WARN, "column type has been change. column_name=%s, old_type=%d, new_type=%d",
        ...
        if (column_left->get_type() == ObVarcharType)
        {
          ...
          {
            YYSYS_LOG(WARN, "varchar column name = %s has change str length. old_length=%ld, new_lenght=%ld, should not less than old",
      ...
      //�ж��ұ��������С�ͬ����column_id��Ҫ�� //repaired from messy code by zhuxh 20151014
      for (int64_t i = 0; i < right_column_count; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "right column is null. i=%ld", i);
          compatible = false;
          break;
        }
        ...
        {
          YYSYS_LOG(WARN, "right table schema is null. table_id=%ld", right_column->get_table_id());
          compatible = false;
          break;
        }
        ...
        if (NULL == left_table_schema)
        {
          YYSYS_LOG(INFO, "new table add. table_name=%s, table_id=%lu",
                    right_table_schema->get_table_name(), right_column->get_table_id());
          continue;
        }
        if (NULL == left_column)
        {
          YYSYS_LOG(INFO, "table add new column. table_name=%s, table_id=%lu, "
                    "column_name=%s, column_id=%lu",
                    right_table_schema->get_table_name(), right_column->get_table_id(),
                    right_column->get_name(), right_column->get_id());
          if (right_column->get_id() <= left_table_schema->get_max_column_id())
          {
            YYSYS_LOG(WARN, "column id =%lu not legal. should bigger than %lu",
                                              ...
                                              ObTableSchema& schema, const ObTableSchema& join_table_schema, uint64_t& column_offset)
    {
      ...
      {
        YYSYS_LOG(ERROR, "left column (%s,%ld) not a rowkey column of left table(%s)",
    ...
    bool ObSchemaManagerV2::parse_join_info(const char* section_name, yysys::CConfig& config, ObTableSchema& schema)
    {
          ...
          if (parse_ok)
          {
            ...
            for(uint32_t i = 0; parse_ok && i < node_list.size(); ++i)
            {
              ...
              *fp = '\0';
              ...
              int32_t l_column_size = static_cast<int32_t>(sizeof(l_column_index) / sizeof(l_column_index[0]));
              int32_t r_column_size = static_cast<int32_t>(sizeof(r_column_index) / sizeof(r_column_index[0]));
              ...
              {
                YYSYS_LOG(ERROR, "error column name %s %s ", node_list[i], fp);
                parse_ok = false;
                break;
              }
              ...
              for(int32_t l_index = 0; l_index < l_column_size && parse_ok; ++l_index)
              {
                ...
                if (lcs->get_type() != rcs->get_type())
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "join column have different types %s %s ", node_list[i], p);
                    parse_ok = false;
                    break;
                  }
                }
                ...
                {
                  YYSYS_LOG(ERROR, "column %s can not be jonined, it hase type ObCreateTimeType or ObModifyTimeType", node_list[i]);
                  parse_ok = false;
                  break;
                }
    ...
    const char* convert_column_type_to_str(ColumnType type)
    {
      ...
      //add 20151008:e
      else if (type == ObBoolType)
      {
        return STR_COLUMN_TYPE_BOOL;
      }
      ...
      {
        return STR_COLUMN_TYPE_FLOAT;
      }
      else if (type == ObDoubleType)
      {
        return STR_COLUMN_TYPE_DOUBLE;
      }
      else
      {
        YYSYS_LOG(ERROR,"column type %d not be supported", type);
        return NULL;
      }
    }
    ...
    int ObSchemaManagerV2::init_index_hash()
    {
      ...
      if(!index_hash_init_)
      {
        ...
        {
          YYSYS_LOG(ERROR,"create id_index_map_ failed!");
          return ret;//do not return like this in other code_block!
        }
        else
        {
          YYSYS_LOG(DEBUG,"create id_index_map_ success!");
        }
      }
      else
      {
      ...
      if(OB_SUCCESS == ret)
      {
        for(int64_t i = 0;i<table_nums_;i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to add index[%ld], err=%d",table_infos_[i].get_table_id() ,ret);
          }
          //add e
        }
      }
      ...
      else
      {
        for(int64_t i = 0;i<il.get_count();i++)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(DEBUG,"index_column_schema is NULL.tid[%ld],cid[%ld]",tid,cid);
            }
            ...
            {
              YYSYS_LOG(ERROR,"index_list is full,and tid[%ld] index cannot add in",tid);
            }
          }
    ...
    int ObSchemaManagerV2::sort_table()
    {
      int ret = OB_SUCCESS;
      int err = OB_SUCCESS;
      ...
      if(!ts_hash_inited_)
      {
        YYSYS_LOG(DEBUG, "create hash table");
        if((ret = tid_hash_map_.create(hash::cal_next_prime(512))) != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "create tid_hash_map_ failed");
        }
        else if((ret = tname_hash_map_.create(hash::cal_next_prime(512))) != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "create tname_hash_map_ failed");
          tid_hash_map_.destroy();
        }
        else
        {
          ts_hash_inited_ = true;
        }
      }
      else
      {
        YYSYS_LOG(DEBUG, "rebuild hash table,clear first");
        tid_hash_map_.clear();
        tname_hash_map_.clear();
      }
      ...
      if(OB_SUCCESS == ret)
      {
        uint64_t tid_key;
        ObString tname_key;
        ObTableSchema *table_schema_value = NULL;

        for(int i =0; i < table_nums_ && OB_SUCCESS == ret; ++i)
        {
          tid_key = table_infos_[i].get_table_id();
          tname_key.assign_ptr(const_cast<char *>(table_infos_[i].get_table_name()), static_cast<int32_t>(strlen(table_infos_[i].get_table_name())));
          table_schema_value = &table_infos_[i];

          ret = tname_hash_map_.set(tname_key, table_schema_value, 1);
          err = tid_hash_map_.set(tid_key, table_schema_value, 1);

          if(-1 == ret || -1 == err)
          {
            YYSYS_LOG(ERROR, "insert table schema into hash set failed");
            ret = OB_ERROR;
          }
          else if(ret != err)
          {
            use_ts_hash_ = true;
            YYSYS_LOG(ERROR, "tname->tid & tid->tname no match: [ret:%d,err:%d]", ret, err);
            ret = OB_SUCCESS;
          }
          else
          {
            use_ts_hash_ = true;
            YYSYS_LOG(DEBUG, "insert (%s,%lu) into hash succ", tname_key.ptr(), tid_key);
            ret = OB_SUCCESS;
          }
        }
      }
    ...
    int ObSchemaManagerV2::sort_column()
    {
      ...
      if (!hash_sorted_)
      {
        ...
        {
          YYSYS_LOG(ERROR,"create column_hash_map_ failed");
        }
        else if ((ret = id_hash_map_.create(hash::cal_next_prime(512))) != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR,"create id_hash_map_ failed");
          column_hash_map_.destroy();
        }
      }
      else
      ...
      {
        YYSYS_LOG(INFO,"drop all column group info");
        for(int64_t i=0; i < column_nums_; ++i)
        {
          columns_[i].set_column_group_id(0); //drop all column group
        }

        YYSYS_LOG(DEBUG,"old column nums:%ld",column_nums_);
        ...
        YYSYS_LOG(DEBUG,"new column nums:%ld",column_nums_);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for(int i = 0; i < column_nums_ && OB_SUCCESS == ret; ++i)
        {
          ...
          if (NULL == table_schema)
          {
            ...
            YYSYS_LOG(ERROR, "get_table_schema(id=%ld)=>NULL, this should not happen", columns_[i].get_table_id());
            break;
          }
          ...
          if ( -1 == ret || -1 == err)
          {
            YYSYS_LOG(ERROR,"insert column into hash set failed");
            ret = OB_ERROR;
          }
          else if (hash::HASH_EXIST == ret && hash::HASH_EXIST == err)
          {
            ...
            if (-1 == ret || -1 == err)
            {
              YYSYS_LOG(ERROR,"get (%s,%s) failed",name_key.table_name_.ptr(),name_key.column_name_.ptr());
              ret = OB_ERROR;
            }
            else
            {
          ...
          {
            YYSYS_LOG(ERROR,"name->id & id->name no match: [ret:%d,err:%d]",ret,err);
            ret = OB_ERROR;
          }
          else
          {
            YYSYS_LOG(DEBUG,"insert (%s,%s),(%lu,%lu) into hash succ",name_key.table_name_.ptr(),
          ...
          if (-1 == ret || -1 == err)
          {
            YYSYS_LOG(ERROR,"error happend in sort column ");
            ret = OB_ERROR;
          }
          else
          {
      ...
      {
        YYSYS_LOG(WARN,"sort column failed");
      }
        ...
        char* expire_condition, const int64_t buf_size) const
    {
      int ret = OB_SUCCESS;
      struct tm* tm = NULL;
      time_t sys_date = static_cast<time_t>(yysys::CTimeUtil::getTime() / 1000 / 1000);
      char replace_str_buf[32];

      if (NULL == expire_condition || buf_size <= 0)
      {
        YYSYS_LOG(WARN, "invalid param, expire_condition=%p, buf_size=%ld",
      ...
      else
      {
        ...
        if (NULL == tm)
        {
          YYSYS_LOG(WARN, "failed to transfer system date to tm, sys_date=%ld",
                    sys_date);
          ret = OB_ERROR;
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to replace $SYS_DATE in expire condition");
          }
          */
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "$SYS_DATE and $SYS_DAY, the two can only and must choose one!");
          }
          ...
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "failed to replace $SYS_DAY in expire condition");
            }
          }
          //modify 20140909:b
        }
        ...
        ObExpressionParser& parser) const
    {
      ...
      ret = parser.parse(expr, expr_array, true, true, this, table_schema.get_table_name());
      ...
      {
        YYSYS_LOG(WARN, "parse infix expression to postfix expression "
                  "error, ret=%d", ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "expire_info is wrong,ret=%d", ret);
        }
        else if(col_mark < 1)
        {
          ret = OB_EXPIRE_CONDITION_ERROR;
          YYSYS_LOG(WARN, "expire info doesn't have a column name, ret=%d", ret);
        }
      }
    ...
    int ObSchemaManagerV2::check_index_table_expire_column(const ObSchemaManagerV2 &schema_manager, const ObTableSchema &table_schema, ObString &column_name) const
    {
      int ret = OB_SUCCESS;
      IndexList idx_list;
      if(OB_SUCCESS != (ret = schema_manager.get_index_list(table_schema.get_table_id(), idx_list)))
      {
        YYSYS_LOG(WARN, "get index table failed, ret=%d", ret);
      }
      else if(0 < idx_list.get_count() && OB_SUCCESS == ret)
      {
        char col_name[OB_MAX_COLUMN_NAME_LENGTH] = {0};
        memcpy(col_name, column_name.ptr(), column_name.length());
        col_name[column_name.length()] = '\0';
        for(int64_t j = idx_list.get_count() - 1; j >= 0; j--)
        {
          uint64_t idx_tid = OB_INVALID_ID;
          idx_list.get_idx_id(j, idx_tid);
          const ObTableSchema *idx_table_schema = NULL;
          const ObColumnSchemaV2 *idx_column_schema = NULL;
          if(NULL == (idx_table_schema = schema_manager.get_table_schema(idx_tid)))
          {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN, "can not get index table schema,ret=%d", ret);
          }
          else if(NULL == (idx_column_schema = schema_manager.get_column_schema(idx_table_schema->get_table_name(), col_name)))
          {
            ret = OB_INDEX_NOT_EXIST;
            YYSYS_LOG(INFO, "index table does't have this column, column_name=%.*s, ret=%d", column_name.length(), column_name.ptr(), ret);
          }
        }
      }
      return ret;
    ...
    bool ObSchemaManagerV2::check_alter_column_in_expire_columns(const ObTableSchema &table_schema, const char *expire_condition, char *alter_column_name) const
    {
      int  ret = OB_SUCCESS;
      bool if_exist = false;
      int64_t type = 0;
      ...
      int col_mark = 0;
      char infix_condition_expr[OB_MAX_EXPIRE_CONDITION_LENGTH] = {0};
      strcpy(infix_condition_expr, const_cast<char *>(expire_condition));
      infix_condition_expr[static_cast<int32_t>(strlen(expire_condition))] = '\0';
      ret = replace_system_variable(infix_condition_expr, OB_MAX_EXPIRE_CONDITION_LENGTH);
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to check expire dependent columns, wrong column, infix_expr=%s", infix_condition_expr);
      }
      else
      {
        exp_cond.assign_ptr(const_cast<char *>(infix_condition_expr), static_cast<int32_t>(strlen(infix_condition_expr)));
        expr_array.init(OB_MAX_COMPOSITE_SYMBOL_COUNT, post_expr);
        ret = parser.parse(exp_cond, expr_array, true);
      }
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "parse infix expression to postfix expression "
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        while(i < postfix_size - 1)
        {
          if(OB_SUCCESS != (ret = expr_array.at(i)->get_int(type)))
          {
            YYSYS_LOG(WARN, "unexpected data type. int expected, but actual type is %d",
                      expr_array.at(i)->get_type());
            break;
          ...
          else
          {
            ...
            {
              col_mark++;
              if(OB_SUCCESS != (ret = expr_array.at(i + 1)->get_varchar(key_col_name)))
              {
                YYSYS_LOG(WARN, "unexpected data type. varchar expected, "
                          "but actual type is %d",
                          expr_array.at(i + 1)->get_type());
              ...
              {
                char *expire_col_name = NULL;
                table_name.assign_ptr(const_cast<char*>(table_schema.get_table_name()),
                                      static_cast<int32_t>(strlen(table_schema.get_table_name())));
                column_schema = get_column_schema(table_name, key_col_name);
                expire_col_name = const_cast<char *>(column_schema->get_name());
                if(NULL == column_schema)
                {
                  ret = OB_INVALID_ARGUMENT;
                  YYSYS_LOG(WARN, "expire condition includes invalid column name, "
                            "table_name=%s, column_name=%.*s, expire_condition=%.*s",
                            table_schema.get_table_name(), key_col_name.length(),key_col_name.ptr(),
                            exp_cond.length(),exp_cond.ptr());
                  break;
                }
                int comp = strcmp(alter_column_name, expire_col_name);
                if(comp == 0)
                {
                  if_exist = true;
                  break;
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "expire info is wrong, ret=%d", ret);
        }
      }
      return if_exist;
    }
    ...
    int ObSchemaManagerV2::check_expire_condition_column(const ObSchemaManagerV2 &schema_manager, const ObTableSchema &table_schema, const char *expire_condition) const
    {
      int  ret = OB_SUCCESS;
      int64_t type = 0;
      int64_t postfix_size = 0;
      ...
      else
      {
        ...
        ret = parser.parse(exp_cond, expr_array, true, true, this, table_schema.get_table_name());
          ...
          else
          {
            if(ObExpression::COLUMN_IDX == type)
            {
              ...
              else
              {
                ...
                else if(OB_SUCCESS != (ret = check_index_table_expire_column(schema_manager, table_schema, key_col_name)))
                {
                  YYSYS_LOG(WARN, "not all index table is satisfied, ret=%d", ret);
                  break;
                }
              }
            }
      ...
      if(index_hash_init_)
      {
        if(hash::HASH_EXIST == id_index_map_.get(table_id, idx_list))
        {
          for(int64_t i=0; i<idx_list.get_count(); i++)
          {
            ...
            {
              YYSYS_LOG(WARN,"fail to get table schema for table[%ld]", tmp_tid);
            }
            else
            {
              //del liumz, [modify index even if index is ERROR] 20170912:b
    ...
    int ObSchemaManagerV2::get_all_modifiable_index_tid(uint64_t main_tid, uint64_t *tid_array, uint64_t &count) const   //���tidΪmain_tid�ı������еĿ��õ����������浽����tid_array����
    {
      int ret = OB_SUCCESS;
      IndexList idx_list;
      uint64_t tmp_tid = OB_INVALID_ID;
        ...
        if(hash::HASH_EXIST == id_index_map_.get(main_tid, idx_list))
        {
          for(int64_t i=0; i<idx_list.get_count(); i++)
          {
            ...
            if (NULL == (main_table_schema = get_table_schema(tmp_tid)))
            {
              ...
              YYSYS_LOG(WARN,"fail to get table schema for table[%ld]", main_tid);
            }
            else
            {
              //del liumz, [modify index even if index is ERROR] 20170912:b
    ...
    //���tidΪmain_tid�ı������еĿ��õ�������id���浽����tid_array����
    {
      int ret = OB_SUCCESS;
      ...
      for (;iter != id_index_map_.end(); ++iter)
      {
        ...
        for(int64_t i = 0; i< tmp_list.get_count(); i++)
        {
          ...
          {
            ret = OB_ERR_ILLEGAL_ID;
            YYSYS_LOG(WARN,"fail to get table schema for table[%ld]", idx_id);
          }
          else
          {
    ...
    int ObSchemaManagerV2::get_all_init_index_tid(ObArray<uint64_t> &index_id_list) const
    {
      ...
      for (;iter != id_index_map_.end(); ++iter)
      {
        ...
        for(int64_t i = 0; i< tmp_list.get_count(); i++)
        {
          ...
          {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN,"fail to get table schema for index table[%lu]", idx_id);
          }
          else
          {
    ...
    //���tidΪmain_tid�ı������еĿ��õ����������浽����tid_array����
    {
      bool ret=false;
      IndexList il;
      uint64_t tmp_tid = OB_INVALID_ID;
        ...
        for(int64_t i = 0; i< tmp_list.get_count(); i++)
        {
          ...
          else
          {
            if(index_table_schema->get_index_helper().status == NOT_AVALIBALE)
            {
              ...
              {
                YYSYS_LOG(WARN, "push index table[%lu] into index_id_list failed", idx_id);
              }
            }
          }
        }
    ...
    int ObSchemaManagerV2::get_all_orphan_index(ObArray<uint64_t> &index_id_list, ObArray<ObString> &index_name_list) const
    {
      int ret = OB_SUCCESS;
      uint64_t idx_id = OB_INVALID_ID;
      const ObTableSchema *index_table_schema = NULL;
      uint64_t src_table_id = OB_INVALID_ID;
      hash::ObHashMap<uint64_t, IndexList, hash::NoPthreadDefendMode>::const_iterator iter = id_index_map_.begin();
      for(; iter != id_index_map_.end() && OB_SUCCESS == ret; ++iter)
      {
        IndexList tmp_list = iter->second;
        for(int64_t i = 0; i < tmp_list.get_count(); i++)
        {
          tmp_list.get_idx_id(i, idx_id);
          index_table_schema = get_table_schema(idx_id);

          if(NULL == index_table_schema)
          {
            ret = OB_SCHEMA_ERROR;
            YYSYS_LOG(WARN, "fail to get table schema for index table[%lu], ret=%d", idx_id, ret);
          }
          else if(OB_INVALID_ID != (src_table_id = index_table_schema->get_index_helper().tbl_tid)
                  && NULL == get_table_schema(src_table_id))
          {
            if(OB_SUCCESS != (ret = index_id_list.push_back(idx_id)))
            {
              YYSYS_LOG(WARN, "push orphan index table[%lu] into index_id_list failed, ret=%d", idx_id, ret);
            }
            else if(OB_SUCCESS != (ret = index_name_list.push_back(ObString::make_string(index_table_schema->get_table_name()))))
            {
              YYSYS_LOG(WARN, "push orphan index table name[%s] into index_name_list failed, ret=%d",
                        to_cstring(ObString::make_string(index_table_schema->get_table_name())), ret);
            }
          }
          else
          {
    ...
    //add e
    //add fanqiushi_index
    //�ж�tidΪtable_id�ı��Ƿ��п��õ����� //repaired from messy code by zhuxh 20151014
        ...
        if(hash::HASH_EXIST==id_index_map_.get(table_id,il))
        {
          for(int64_t i=0;i<il.get_count();i++)
          {
            ...
            /*else
                    ...
                    main_table_schema->get_index_helper().status == NOT_AVALIBALE ||
                    main_table_schema->get_index_helper().status == ERROR)
            {
              ret = true;
              break;
            }
          ...
          //YYSYS_LOG(ERROR,"test::fanqs,,table_id=%ld,,il.get_count()=%ld",table_id,il.get_count());
          // if(il.get_count()>0)
          //ret = true;
        }
      }
    ...
    int ObSchemaManagerV2::get_all_avalibale_index_tid(uint64_t main_tid,uint64_t *tid_array,uint64_t &count) const   //���tidΪmain_tid�ı������еĿ��õ����������浽����tid_array����
    {
      int ret=OB_SUCCESS;
      IndexList il;
      uint64_t tmp_tid=OB_INVALID_ID;
    ...
    bool ObSchemaManagerV2::is_this_table_avalibale(uint64_t tid) const   //�ж�tidΪ�����ı��Ƿ��ǿ��õ�������
    {
      bool ret=false;
      const ObTableSchema *main_table_schema = NULL;
      ...
      if(index_hash_init_)
      {
        if(hash::HASH_EXIST==id_index_map_.get(tid,il))
        {
          for(int64_t i=0;i<il.get_count();i++)
          {
            const ObTableSchema *index_table_schema = NULL;
            il.get_idx_id(i,tmp_tid);
            //YYSYS_LOG(ERROR,"test::fanqs4,,tmp_tid=%ld",tmp_tid);
            ...
            else
            {
              ...
              //YYSYS_LOG(ERROR,"test::fanqs4,,enter this,index_first_cid=%ld,cid=%ld",index_first_cid,cid);
              if(index_first_cid==cid&&index_table_schema->get_index_helper().status==AVALIBALE)
              {
                index_tid_array[array_index]=tmp_tid;
                //YYSYS_LOG(ERROR,"test::fanqs4,,enter this,tmp_tid=%ld,tid=%ld,,,array_index=%d",tmp_tid,tid,array_index);
                array_index++;
                ret=true;
                // break;
              }
          ...
          //YYSYS_LOG(ERROR,"test::fanqs4,,index_tid_array[0]=%ld,,array_index=%d",index_tid_array[0],array_index);
        }
      }
      else
        ret=false;
    ...
    bool ObSchemaManagerV2::check_compress_name() const
    {
      ...
      for (int64_t i = 0; i < table_nums_ && OB_SUCCESS == ret; ++i)
      {
            ...
            && compress_name[0] != '\0')
        {
          ...
          {
            bret = false;
            YYSYS_LOG(ERROR, "table compress function name check failed. table_name=%s, compress_name=%s",
                      table_infos_[i].get_table_name(), compress_name);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "fail to get tabel compress name, table_name=%s, compress_name=%p", table_infos_[i].get_table_name(), compress_name);
          bret = false;
          ret = OB_ERROR;
        }
      }
    ...
    bool ObSchemaManagerV2::check_table_expire_condition() const
    {
      ...
      if (NULL == parser)
      {
        YYSYS_LOG(WARN, "failed to new expression parser");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      ...
      for (int64_t i = 0; i < table_nums_ && OB_SUCCESS == ret; ++i)
      {
            ...
            && expire_condition[0] != '\0')
        {
          ...
          {
            YYSYS_LOG(WARN, "expire condition too large, expire_condition_len=%zu, "
          ...
          else
          {
            ...
            ret = replace_system_variable(infix_condition_expr, OB_MAX_EXPIRE_CONDITION_LENGTH);
            if (OB_SUCCESS == ret && 0 != strcmp(expire_condition, "1=0"))
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "failed to check expire dependent columns, infix_expr=%s",
    ...
    int ObSchemaManagerV2::check_new_expire_condition_for_alter(
        const ObSchemaManagerV2 &schema_manager, const ObTableSchema &table_schema, const ObString &expire_condition, const ObString &old_expire_condition, bool is_add) const
    {
      int ret = OB_SUCCESS;
      if(expire_condition.length() > OB_MAX_EXPIRE_CONDITION_LENGTH)
      {
        YYSYS_LOG(WARN, "expire condition is too long! expire_condition.length=%d, table_name=%s",
                  expire_condition.length(), table_schema.get_table_name());
        ret = OB_EXPIRE_CONDITION_ERROR;
      }
      else if(old_expire_condition.length() > OB_MAX_EXPIRE_CONDITION_LENGTH)
      {
        YYSYS_LOG(WARN, "expire condition is too long! expire_condition.length=%d, table_name=%s",
                  old_expire_condition.length(), table_schema.get_table_name());
        ret = OB_EXPIRE_CONDITION_ERROR;
      }

      if(expire_condition.ptr() && 0 != strcmp(expire_condition.ptr(), "1=0"))
      {
        ret = schema_manager.check_expire_condition_column(schema_manager, table_schema, expire_condition.ptr());
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "check expire column failed, ret=%d", ret);
        }
        else
        {
          if(!is_add && 0 != strcmp(old_expire_condition.ptr(), "1=0"))
          {
            if(!old_expire_condition.ptr())
            {
              ret = OB_EXPIRE_CONDITION_ERROR;
              YYSYS_LOG(WARN, "old_expire_info is null, ret=%d", ret);
            }
            else
            {
              ret = schema_manager.check_expire_condition_column(schema_manager, table_schema, old_expire_condition.ptr());
              if(OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "check expire column failed, ret=%d", ret);
              }
    ...
    int ObSchemaManagerV2::get_column_groups(uint64_t table_id,uint64_t column_groups[],int32_t& size) const
    {
      ...
      else
      {
        ...
        {
          size = 0;
          YYSYS_LOG(WARN,"not found column group in table %lu,column_group_nums_:%ld",table_id,column_group_nums_);
          ret = OB_ERROR;
        }
      }
      return ret;
    ...
    DEFINE_SERIALIZE(ObSchemaManagerV2)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        ret = serialization::encode_vi32(buf, buf_len, tmp_pos, OB_SCHEMA_VERSION_EIGHT);
        //mod:e   //[conflict_level A]
      }
      if (OB_SUCCESS == ret)
      {
    ...
    DEFINE_DESERIALIZE(ObSchemaManagerV2)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR,"schema magic numer is wrong, schema_magic_=%x, OB_SCHEMA_MAGIC_NUMBER=%x",
        ...
        else
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(ERROR, "bugs, table_nums_ %ld error", table_nums_);
              ret = OB_ERROR;
            }
          }
          //add zhaoqiong [Schema Manager] 20150327:b
          if (OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_FIVE)
          {
            ...
            YYSYS_LOG(DEBUG, "schema table_begin= %ld ",table_begin);
            if (table_begin != table_end_) //discontinuous
            {
              YYSYS_LOG(ERROR,"schema is discontinuous, expect_table_begin_=%ld, table_begin_=%ld",table_end_,table_begin_);
              ret = OB_ERROR;
            }
          }
          if (OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_FIVE)
          {
            ...
            YYSYS_LOG(DEBUG, "schema table_end= %ld ",table_end_);
            if (table_end_ > table_nums_) // illegal
            {
              YYSYS_LOG(ERROR,"schema is illegal, table_end_=%ld, table_nums_=%ld",table_end_,table_nums_);
              ret = OB_ERROR;
            }
          }
          ...
          if (OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_FIVE)
          {
            ...
            YYSYS_LOG(DEBUG, "schema column_begin= %ld ",column_begin);
            if (column_begin != column_end_) //discontinuous
            {
              YYSYS_LOG(ERROR,"schema is discontinuous, expect_column_begin_=%ld, column_begin_=%ld",column_end_,column_begin_);
              ret = OB_ERROR;
            }
          }
          if (OB_SUCCESS == ret && version_ >= OB_SCHEMA_VERSION_FIVE)
          {
            ...
            YYSYS_LOG(DEBUG, "schema column_end= %ld ",column_end_);
            if (column_end_ > column_nums_) // illegal
            {
              YYSYS_LOG(ERROR,"schema is illegal, column_end_=%ld, column_nums_=%ld",column_end_,column_nums_);
              ret = OB_ERROR;
            }
          }
          //add:e
          ...
          //mod zhaoqiong [Schema Manager] 20150327:b
          //          if (OB_SUCCESS == ret)                  //[conflict_level A]
          //          {
          ...
          else if (OB_SUCCESS == ret && version_ <= OB_SCHEMA_VERSION_SIX)
          {
          ...
          else if (OB_SUCCESS == ret && version_ > OB_SCHEMA_VERSION_SIX)
          {
            for (int64_t i = column_begin; i < column_end_; ++i)
            {
              ret = columns_[i].deserialize_v5(buf, data_len, tmp_pos);
              if (OB_SUCCESS != ret)
                break;
            }
          }
    ...
    int ObSchemaManagerV2::add_new_table_schema(const ObArray<TableSchema>& schema_array)
    {
      ...
      for (int i = 0; i < schema_array.count() && OB_SUCCESS == ret; i++)
      {
        ...
        //[view]
        old_tschema.set_table_type(static_cast<ObTableSchema::ObTableType>(tschema->table_type_));
        old_tschema.set_check_option(static_cast<ObTableSchema::ObWithCheckOption>(tschema->with_check_option_));
        old_tschema.set_is_updatable(tschema->is_updatable_);
        old_tschema.set_text(tschema->text_);
        ...
        {
          YYSYS_LOG(INFO, "expire condition =%s", tschema->expire_condition_);
        ...
        {
          YYSYS_LOG(WARN, "failed to add table, err=%d", ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "add table to schema_manager succ. table_id = %lu", tschema->table_id_);
          ObRowkeyInfo rowkey_info;
          ...
          {
            YYSYS_LOG(DEBUG, "this is a wide_table. add join info");
            ...
            {
              YYSYS_LOG(WARN, "fail to get offset array.");
            }
            //for debug
            for (int i = 0; i < right_rowkey_column_count; i++)
            {
              YYSYS_LOG(DEBUG, "left_column_offset_array[%d] = %ld", i, left_column_offset_array[i]);
            }
          }
          int count = 0;
          for (int64_t i = 0; i < tschema->columns_.count() && ret == OB_SUCCESS; i++)
          {
            ...
            //add 20131208:e
            if(!tcolumn.default_value_is_null)
            {
              old_tcolumn.set_default_val(tcolumn.default_value_);
            }
            // @todo join info
            if (0 < tcolumn.join_table_id_)
            {
              count++;
              YYSYS_LOG(DEBUG, "%d, column_name=%s, column_id=%lu have join info, join_table=%lu, join_column_id=%lu",
            ...
            if (0 < tcolumn.rowkey_id_)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to set rowkey column, err=%d", ret);
                break;
              }
            }
            if (OB_SUCCESS == ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "failed to add column, err=%d", ret);
                break;
              }
            }
          } // end for
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to get table schema");
            }
            else
            {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "add new table_schema success");
      }
      else
      {
        YYSYS_LOG(WARN, "fail to add new table schema");
      }
      return ret;
    }
    ...
    int ObSchemaManagerV2::add_new_table_schema(const TableSchema& tschema)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "push back to array fail");
      }
      else
      {
    ...
    int ObSchemaManagerV2::drop_one_table(uint64_t table_id)
    {
      ...
      if (NULL == tschema)
      {
        YYSYS_LOG(WARN,"can't drop this table's ObTableSchema:[%ld]",table_id);
        ret = OB_ENTRY_NOT_EXIST;
      }

      //[view]
      bool is_view = false;
      if (tschema != NULL && tschema->get_type() == ObTableSchema::VIEW)
      {
          is_view = true;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (tschema->get_table_id() == max_table_id_)
        {
          ...
          YYSYS_LOG(DEBUG,"update max table_id = %ld",max_table_id_);
        }
        ...
        if ( NULL == tcolumn && !is_view)
        {
          YYSYS_LOG(WARN,"can't drop this table's ObColumnSchema:[%ld]",table_id);
          ret = OB_ENTRY_NOT_EXIST;
        }
        if (OB_SUCCESS == ret && !is_view)
        {
          memmove(tcolumn,tcolumn+csize,sizeof(ObColumnSchemaV2) * (column_nums_ - (tcolumn + csize - columns_)));
          column_nums_ -= csize;
          YYSYS_LOG(DEBUG,"drop this table schema success:[%ld]",table_id);
        }
      }
      if (OB_ENTRY_NOT_EXIST == ret)
      {
        ret = OB_SUCCESS;
        YYSYS_LOG(WARN,"this table may not be created:[%ld]",table_id);
      }
      return ret;
    }
    ...
    {
      int ret = OB_SUCCESS;
      YYSYS_LOG(INFO, "write schema to file. file_name=%s", file_name);
      FILE *fd = NULL;
      if (NULL == file_name || (NULL == (fd = fopen(file_name, "w"))))
      {
        YYSYS_LOG(WARN, "can't open file. file_name=%p", file_name);
        ret = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "fprintf buf content to file fail. ret=%d", ret);
        }
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; i < table_nums_; i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to write table to file. table index =%ld, ret=%d", i, ret);
            break;
          }
        }
      }
    ...
    int ObSchemaManagerV2::write_table_to_file(FILE *fd, const int64_t table_index)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid index. index=%ld, table_num=%ld, fd=%p",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == table_schema)
        {
          YYSYS_LOG(WARN, "table_schema = %p, error here.!", table_schema);
          ret = OB_ERROR;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        //[view]
        else if (0 >= (ret = fprintf(fd, "%s=%d\n", STR_TYPE, table_schema->get_type())))
        {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "fprintf buf content to file fail. ret=%d", ret);
        }
        else if (0 >= (ret = fprintf(fd, "%s=%s\n", STR_SQL_TEXT, table_schema->get_text())))
        {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "fprintf buf content to file fail. ret=%d", ret);
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to write rowkey info to file. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        for (uint64_t i = 0; i <= table_schema->get_max_column_id(); i++)
        {
          ...
          if (NULL != column_schema)
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "fail to write column info to file. ret=%d", ret);
              break;
            }
            else
            {
              ...
              if (join_info != NULL)
              {
                ...
                {
                  YYSYS_LOG(WARN, "table schema =%p, column_schema=%p, table_id=%ld, column_id=%ld",
    ...
    int ObSchemaManagerV2::write_rowkey_info_to_file(FILE *fd, const uint64_t table_id, const ObRowkeyInfo &rowkey)
    {
      ...
      for (int64_t i = 0; i < rowkey.get_size(); i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "cann't find the column_schema. table_id=%ld, column_id=%ld",
                    table_id, column_id);
          ret = OB_ERROR;
          break;
        }
    ...
    int ObSchemaManagerV2::write_column_info_to_file(FILE *fd, const ObColumnSchemaV2 *column_schema)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument. fd=%p, column_schema=%p", fd, column_schema);
        ret = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == ret)
      {
        if (ObVarcharType != column_schema->get_type() && ObDecimalType != column_schema->get_type())
        {
          if (0 >= (ret = fprintf(fd, "%s=%d,%ld,%s,%s,%d\n", STR_COLUMN_INFO, column_schema->is_maintained(),
        ...
        else if(ObVarcharType == column_schema->get_type())
        {
          if (0 >= (ret = fprintf(fd, "%s=%d,%ld,%s,%s,%ld,%d\n", STR_COLUMN_INFO, column_schema->is_maintained(),
                                  column_schema->get_id(),
                                  column_schema->get_name(),
                                  convert_column_type_to_str(column_schema->get_type()),
        ...
        {
          if (0 >= (ret = fprintf(fd, "%s=%d,%ld,%s,%s,%d,%d,%d\n", STR_COLUMN_INFO, column_schema->is_maintained(),
                                  column_schema->get_id(),
                                  column_schema->get_name(),
                                  convert_column_type_to_str(column_schema->get_type()),
                                  column_schema->get_precision(),
                                  column_schema->get_scale(),
                                  column_schema->is_nullable())))
          {
            YYSYS_LOG(WARN, "fprintf buf content to file fail. ret=%d", ret);
            ret = OB_ERROR;
    ...
    int ObSchemaManagerV2::change_table_id(const uint64_t table_id, const uint64_t new_table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to find table_schema. table_id=%ld", table_id);
        ret = OB_ERROR;
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; i < column_nums_; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "error happened. i=%ld, column_num=%ld", i, column_nums_);
            break;
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if(OB_SUCCESS != (ret = sort_table()))
        {
          YYSYS_LOG(WARN, "fail to sort table. ert=%d", ret);
        }
        else if(OB_SUCCESS != (ret = sort_column()))
        {
          YYSYS_LOG(WARN, "fail to sort column. ert=%d", ret);
        }
      }
      return ret;
    }
    ...
    int ObSchemaManagerV2::apply_schema_mutator(const ObSchemaMutator &mutator)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "mutator contain refresh schema operation, need get full schema");
      }
      else if (timestamp_ != mutator.get_start_version())
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"mutator not match current schema,timestamp=%ld, mutator start_version=%ld",
                  timestamp_, mutator.get_start_version());
      }
      else if (OB_SUCCESS != (ret = drop_tables(mutator.get_droped_tables())))
      {
        YYSYS_LOG(WARN, "can not drop schema");
      }
      else if (OB_SUCCESS != (ret = add_new_table_schema(mutator.get_add_table_schema())))
      {
        YYSYS_LOG(WARN, "can not add schema");
      }
      if (OB_SUCCESS != (ret = sort_table()))
      {
        YYSYS_LOG(WARN, "fail to sort table. ert=%d", ret);
      ...
      {
        YYSYS_LOG(WARN, "can not init_index_hash");
      }
      //add e
      else
      {
        timestamp_ = mutator.get_end_version();
        YYSYS_LOG(INFO, "apply schema mutator(version[%ld->%ld])", mutator.get_start_version(), mutator.get_end_version());
      }
      return ret;
    }
    ...
    int ObSchemaManagerV2::determine_serialize_pos(int64_t table_begin_pos,int64_t column_begin_pos) const
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"invalid table_begin_pos, table_begin_pos=%ld,table_nums_=%ld",
                  table_begin_pos, table_nums_);
      }
      else if (column_begin_pos < 0 || column_begin_pos >= column_nums_)
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"invalid column_begin_pos, column_begin_pos=%ld,column_nums_=%ld",
      ...
      else
      {
        ...
        for (int64_t i = last_table_end_pos; i < table_nums_; ++i)
        {
          ...
          for (int64_t j = last_column_end_pos; j < column_nums_; ++j)
          {
            ...
            {
              YYSYS_LOG(ERROR, "error occurs,table[%ld] has no columns",current_table_id);
            }
            else
            {
              serialize_size = len;
          ...
          if (finished)
          {
            ...
            YYSYS_LOG(INFO, "wait for serialization in next round, table index begins at %ld,column index begins at %ld,serialize_size=%ld ",
        ...
        if (!finished)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "error occurs,totally serialize %ld tables, not equal to table_nums_=%ld",
          ...
          else
          {
            ...
            YYSYS_LOG(INFO, " finish serialization in current round, serialize %ld tables, %ld columns in total, serialize_size=%ld",
                      table_end_,column_end_, serialize_size);
          }
        }
      }
    ...
    int ObSchemaManagerV2::prepare_for_next_serialize()
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR,"maybe not after first serialize, this func should use after first serialize, table_end_=%ld,column_end_=%ld",
                  table_end_, column_end_);
      }
      else if (OB_SUCCESS != (ret = determine_serialize_pos(table_end_,column_end_)))
      {
        YYSYS_LOG(ERROR,"determine_serialize_pos error");
      }
      return ret;
    }
 ...
 *-----------------------------------------------------------------------------*/
    ...
    DEFINE_SERIALIZE(ObSchemaMutator)
    {
      ...
      else
      {
        for (int64_t i = 0; OB_SUCCESS == ret && i < droped_tables_.count(); ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to serialize column, err=%d", ret);
            break;
          }
        }
      }
    ...
    DEFINE_DESERIALIZE(ObSchemaMutator)
    {
      ...
      {
        YYSYS_LOG(WARN, "deserialize error here");
      }
      ...
      else
      {
        ...
        for (int64_t i = 0; i < drop_table_count; ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to deserialize column, err=%d", ret);
            break;
          }
          else if (OB_SUCCESS != (ret = droped_tables_.push_back(droped_table_id)))
          {
            YYSYS_LOG(WARN, "failed to push into array, err=%d", ret);
            break;
          }
        }
      }
