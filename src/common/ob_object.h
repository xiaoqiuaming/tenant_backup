  ...
  namespace common
  {
    ...
    class ObObj
    {
      ...
      public:
        ...
        /*
         *   �����������ͣ����date_timeΪOB_SYS_DATE����ʾ����Ϊ��������ǰʱ��
         */
        ...
        int to_string_memcpy(char *buffer, const int64_t length, int64_t &pos) const;//add for [cchecksum info range error]
        int64_t value_to_string(char *buffer, const int64_t length) const;  //add liu jun. [MultiUPS] [calculate] 20150709
        ...
        int get_varchar_for_precise_datetime(ObString &value) const;
        int get_varchar_for_date(ObString &value) const;
        int get_varchar_for_time(ObString &value) const;
        ...
        //add for [Decimal]-b
        int set_decimal(const ObString &value, uint32_t p, uint32_t s, uint32_t v, uint32_t len);
        void set_ttint(const uint64_t *ttint);
        int get_ttint(uint64_t *&res) const;
      ...
      private:
        ...
        friend class ObCellNewScanner;  //mod hongchen [MERGE_BUGFIX] 20170801
    ...
    inline int ObObj::set_add(const bool is_add /*=false*/)
    {
      ...
      switch (get_type())
      {
        ...
        case ObDoubleType:
          ...
          break;
        default:
          YYSYS_LOG(ERROR, "check obj type failed:type[%d]", get_type());
          ret = OB_ERROR;
      }
      return ret;
    }
    ...
    //add for [column default]-b
    inline int ObObj::get_varchar_for_precise_datetime(ObString &value) const
    {
      int res = OB_OBJ_TYPE_ERROR;
      if (meta_.type_ == ObPreciseDateTimeType)
      {
        value.assign_ptr(const_cast<char *>(value_.varchar_val), val_len_);
        res = OB_SUCCESS;
      }
    ...
    inline int ObObj::get_varchar_for_date(ObString &value) const
    {
      int res = OB_OBJ_TYPE_ERROR;
      if (meta_.type_ == ObDateType)
      {
        value.assign_ptr(const_cast<char *>(value_.varchar_val), val_len_);
        res = OB_SUCCESS;
      }
    ...
    inline int ObObj::get_varchar_for_time(ObString &value) const
    {
      int res = OB_OBJ_TYPE_ERROR;
      if (meta_.type_ == ObTimeType)
      {
        value.assign_ptr(const_cast<char *>(value_.varchar_val), val_len_);
        res = OB_SUCCESS;
      }
    ...
    int ob_write_obj(AllocatorT &allocator, const ObObj &src, ObObj &dst)
    {
      ...
      else
      {
        if(ObDecimalType == src.get_type())
        {
          ...
          if (OB_SUCCESS != (ret = src.get_ttint(t1)))
          {
            YYSYS_LOG(ERROR, "get decimal pointer from src error,ret = %d", ret);
          }
          else
          {
    ...
    int ob_write_obj_v2(AllocatorT &allocator, const ObObj &src, ObObj &dst)
    {
      ...
      else
      {
        if(ObDecimalType == src.get_type())
        {
          ...
          uint32_t len = src.get_nwords();
          ObDecimal dec2;
          if (OB_SUCCESS != (ret = src.get_ttint(src_val)))
          {
            YYSYS_LOG(ERROR, "get decimal pointer from src error,ret = %d", ret);
          }
          else
          {
            ...
            dec2.from(src.get_precision(), src.get_scale(), src.get_vscale(), tt);
          }
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to do from decimal");
          }
          ...
          else
          {
            ...
            if (OB_SUCCESS == (ret = ob_write_decimal(allocator, dec2.get_words()->ToUInt_v2(), len, dst_val)))
            {
              dst.set_decimal(dst_val, dec2.get_precision(), dec2.get_scale(), dec2.get_vscale(), len);
            }
          }
        }
        else
