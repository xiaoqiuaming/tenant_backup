    ...
    const int64_t timeout, const int64_t special_timeout)
{
  ...
  if (init_)
  {
    YYSYS_LOG(ERROR, "%s", "check cache already inited");
    ret = OB_INNER_STAT_ERROR;
  }
  else if ((timeout <= 0) || (special_timeout <= 0))
  {
    YYSYS_LOG(ERROR, "check cache timeout failed:timeout[%ld], vtimeout[%ld]",
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "cache init failed:mem[%lu], count[%lu], timeo[%ld], ret[%d]",
    ...
    ObTabletLocationList & location)
{
  ...
  if (!init_)
  {
    YYSYS_LOG(ERROR, "%s", "check init failed");
    ret = OB_INNER_STAT_ERROR;
  }
  else if ((0 == rowkey.length()) || (NULL == rowkey.ptr()))
  {
    YYSYS_LOG(ERROR, "%s", "check rowkey length failed");
    ret = OB_INPUT_PARAM_ERROR;
  }
  else
  {
    ...
    if (NULL == temp)
    {
      YYSYS_LOG(ERROR, "check ob malloc failed:size[%lu], pointer[%p]", size, temp);
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "serialize the search range failed:ret[%d]", ret);
      }
    }
    */
    ...
    //{
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(DEBUG, "find tablet from cache failed:table_id[%lu]",
            table_id);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize location failed:table_id[%lu], length[%ld], "
...
int ObTabletLocationCache::set(const ObNewRange & range, const ObTabletLocationList & location)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "check malloc failed:key_size[%ld], ret[%d]", key_size, ret);
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize range failed:ret[%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize locationlist failed:ret[%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "del the old item:table[%lu], ret[%d]", range.table_id_, ret);
      }
      else
      {
        ret = OB_SUCCESS;
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "add the pair failed:ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "%s", "set tablet location cache succ");
      }
    }
  }
  return ret;
    ...
    */
    ...
    //{
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize tablet range failed:table_id[%lu], ret[%d]",
              table_id, ret);
        }
        else
        {
  ...
  else
  {
    ...
    */
    ...
    //{
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(DEBUG, "find this row tablet failed:table_id[%lu]",
            table_id);
      }
      else
      {
        YYSYS_LOG(DEBUG, "%s", "del this location from cache succ");
      }
    //}
    // destory the temp buffer
    //ob_tc_free(temp);
...
{
  YYSYS_LOG(INFO, "[dump]tablet location cache size %lu", size());
}
