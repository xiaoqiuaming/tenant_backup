...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObBtreeMap<_key, _value>::create(int64_t num, const bool write_lock)
    {
      ...
      if (num <= 0)
      {
        YYSYS_LOG(ERROR, "check create num failed:num[%ld]", num);
        ret = common::OB_INPUT_PARAM_ERROR;
      }
      else if (tree_map_)
      {
        YYSYS_LOG(ERROR, "check tree map not null:map[%p]", tree_map_);
      ...
      else
      {
        ...
        if (NULL == tree_map_)
        {
          YYSYS_LOG(ERROR, "new KeyBtree failed:map[%p]", tree_map_);
        ...
        {
          YYSYS_LOG(DEBUG, "not enable write lock:map[%p]", tree_map_);
        }
      }
      return ret;
    }
    ...
    int64_t ObBtreeMap<_key, _value>::size() const
    {
      ...
      {
        YYSYS_LOG(ERROR, "check tree map is null:map[%p]", tree_map_);
      }
      return ret;
    }
    ...
    int ObBtreeMap<_key, _value>::erase(const _key & key, _value & value)
    {
      ...
      if (tree_map_)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "erase key value failed:ret[%d]", ret);
        }
      }
      else
      {
        ...
        _value & old_value, const bool okey /* = true */, const bool ovalue /* = true */)
    {
      ...
      if (tree_map_)
      {
        ...
        if (ret != common::OB_SUCCESS)
        {
          YYSYS_LOG(DEBUG, "set key value failed:ret[%d]", ret);
        }
      }
      else
      {
