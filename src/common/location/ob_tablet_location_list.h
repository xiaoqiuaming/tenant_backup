...
namespace oceanbase
{
  namespace common
  {
    ...
    public:
      ...
      // [563]
//      int sort(const common::ObServer & server, int32_t cluster_id = OB_MAX_CLUSTER_COUNT);
      int sort(const common::ObServer & server, int32_t cluster_id = OB_ALL_CLUSTER_FLAG);
      //mod:e
    ...
    inline int ObTabletLocationList::set_tablet_range(const common::ObNewRange & tablet_range)
    {
      ...
      /// if(OB_SUCCESS != ret)
      /// {
      ///   YYSYS_LOG(WARN, "reset tablet range rowkey fail:ret[%d]", ret);
      /// }
      ...
      if((OB_SUCCESS == ret) && (NULL != tablet_range_rowkey_buf_))
      {
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write tablet range start key fail:ret[%d]", ret);
        }
      }
      ...
      if((OB_SUCCESS == ret) && (NULL != tablet_range_rowkey_buf_))
      {
        ...
        if(OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write tablet range end key fail:ret[%d]", ret);
        }
      }
      return ret;
    }
