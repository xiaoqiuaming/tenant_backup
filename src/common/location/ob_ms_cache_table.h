...
namespace oceanbase
{
  namespace common
  {
    ...
    {
    public:
      ObCBtreeTable(): cache_lock_(yysys::WRITE_PRIORITY)
      {
      }
    ...
    public:
      ...
      struct MapKey
      {
        ...
        int operator - (const MapKey & key) const
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize range1 failed:ret[%d]", ret);
          }
          else
          {
            pos = 0;
            ...
            if (ret != common::OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "deserialize range2 failed:ret[%d]", ret);
            }
          }
          ...
          if (common::OB_SUCCESS == ret)
          {
            if (range1.start_key_ == range1.end_key_)
            {
              ...
              /*
               YYSYS_LOG(DEBUG, "table[%lu], key[%s], range[%s], cmp[%d]", range1.table_id_, to_cstring(range1.start_key_),
            ...
            else if (range2.start_key_ == range2.end_key_)
            {
              ...
              /*
              YYSYS_LOG(DEBUG, "table[%lu], key[%s], range[%s], cmp[%d]", range2.table_id_, to_cstring(range2.start_key_),
      ...
      private:
        ...
        yysys::CRWLock cache_lock_;
        //mutable yysys::CThreadMutex cache_lock_;
    ...
    {
      yysys::CWLockGuard lock(cache_lock_);
    ...
    int ObCBtreeTable<_key, _value>::add(common::CacheItemHead & item, common::CacheItemHead *& old_item)
    {
      ...
      if (ret != common::OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "check btree set failed:ret[%d]", ret);
      }
    ...
    common::CacheItemHead * ObCBtreeTable<_key, _value>::get(const common::ObNewRange & key)
    {
      ...
      if (ret != common::OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "construct MapKey failed through key:ret[%d]", ret);
      }
      else
      {
        Key.key_ = local;
        yysys::CRLockGuard lock(cache_lock_);
      ...
      common::CacheItemHead *& local)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check key ptr failed:key[%p]", key.ptr());
      ...
      */
      ...
      //{
        ...
        if (NULL == local)
        {
          YYSYS_LOG(ERROR, "ob_tc_malloc failed:size[%ld]", size);
      ...
      if (common::OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "serialize the range failed:ret[%d]", ret);
        }
        else
        {
