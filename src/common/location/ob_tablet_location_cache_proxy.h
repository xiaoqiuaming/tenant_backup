  ...
  namespace common
  {
    ...
    public:
          ...
          ObTabletLocationList & list)
      {
        ...
        {
          YYSYS_LOG(ERROR, "check scan param range failed:range[%p]", scan_param.get_range());
        ...
        {
          YYSYS_LOG(WARN, "get tablet location fail");
        }
    ...
    public:
      ...
      int del_cache_item(const T & scan_param)
      {
        ...
        if (NULL == range)
        {
          YYSYS_LOG(ERROR, "check scan param range failed:range[%p]", range);
          ret = OB_INPUT_PARAM_ERROR;
        }
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "get search key failed:ret[%d], range[%s]", ret, to_cstring(*range));
          }
          else
          {
        ...
        const ObTabletLocationList & list)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(DEBUG, "update cache item failed:table_id[%lu], ret[%d]", range->table_id_, ret);
            }
          }
          reset_search_key(search_key);
        }
        ...
        const ObTabletLocationItem & addr, ObTabletLocationList & list)
      {
        ...
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "get search key failed:ret[%d]", ret);
            range->hex_dump();
          }
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(DEBUG, "set item invalid failed:table_id[%lu], ret[%d]", range->table_id_, ret);
            }
          }
    ...
    private:
      ...
      yysys::CThreadMutex * acquire_lock(const uint64_t table_id);
      /// max len
