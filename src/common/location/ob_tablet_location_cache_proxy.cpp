...
int ObTabletLocationCacheProxy::init(const uint64_t id_lock_count, const uint64_t seq_lock_count)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "init array lock failed:count[%lu]", id_lock_count);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "init sequence lock failed:count[%lu]", seq_lock_count);
    }
  }
  return ret;
}

yysys::CThreadMutex * ObTabletLocationCacheProxy::acquire_lock(const uint64_t table_id)
{
  yysys::CThreadMutex * ret = NULL;
...
/*
 * add zhaoqiong [MultiUPS] [RootTable_Cache] 20150706
 * ɾȥObTabletLocationCache��<table_id, serach_key,ObTabletLocationList>
*/
int ObTabletLocationCacheProxy::del_cache_item(const uint64_t table_id, const ObRowkey & search_key)
{
  ...
  {
    YYSYS_LOG(WARN, "check search key failed:table[%lu], ptr[%p]", table_id, search_key.ptr());
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(DEBUG, "del cache item failed:table_id[%lu], ret[%d], search key[%s]",
          table_id, ret, to_cstring(search_key));
    }
    else
    {
      YYSYS_LOG(DEBUG, "del cache item succ:table_id[%lu]", table_id);
    }
  }
  return ret;
}
...
/*
 * add zhaoqiong [MultiUPS] [RootTable_Cache] 20150706
 * ����ObTabletLocationCache��<table_id, serach_key,ObTabletLocationList>
*/
  ...
  const ObTabletLocationList & list)
{
  ...
  {
    YYSYS_LOG(WARN, "check rowkey failed:table_id[%lu], rowkey[%p]", table_id, rowkey.ptr());
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(DEBUG, "update cache item failed:table_id[%lu], ret[%d]", table_id, ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "update cache item succ:table_id[%lu]", table_id);
    }
  }
  return ret;
}
...
/*
 * add zhaoqiong [MultiUPS] [RootTable_Cache] 20150706
 * ��ObTabletLocationCache��<table_id, serach_key,ObTabletLocationList>��server��ʧ�ܴ���+1, ������cache
*/
  ...
  ObTabletLocationList & list, const ObServer & server)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "fail to get tablet location cache:ret[%d]", ret);
  }
  else
  {
    for (int64_t i = 0; i < list.size(); i ++)
    {
      if (list[i].server_.chunkserver_ == server)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to update cache item:ret[%d]", ret);
        }
        break;
      }
    }
...
/*
 * add zhaoqiong [MultiUPS] [RootTable_Cache] 20150706
 * ��ObTabletLocationCache��<table_id, serach_key,ObTabletLocationList>��server��ʧ�ܴ�����Ϊ���, ������cache
*/
  ...
  const ObTabletLocationItem & addr, ObTabletLocationList & list)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "get cache item failed:table_id[%lu], ret[%d]",
      table_id, ret);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "set item invalid failed:table_id[%lu], ret[%d]",
        table_id, ret);
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(DEBUG, "update cache item failed:ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "%s", "set item invalid succ");
      }
    }
  }
  return ret;
  ...
  const common::ObNewRange * range, ObTabletLocationList & list)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get search key failed:range[%s], ret[%d]", to_cstring(*range), ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "get_search_key sd:%d, range:%s, sk:%s,ek:%s, search_key[%s]",
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "get tablet location failed:range[%s],  search_key[%s], ret[%d]",
          to_cstring(*range), to_cstring(search_key), ret);
      }
      else
      {
        YYSYS_LOG(DEBUG, "get tablet location list succ:table_id[%lu]", range->table_id_);
      }
    }
...
/*
 * add zhaoqiong [MultiUPS] [RootTable_Cache] 20150706
 * ��force_renewΪtrue,����rootserver fetch roottable
 * �����cache��get
*/
  ...
  const ObRowkey & row_key, ObTabletLocationList & location, bool force_renew)
{
  ...
  YYSYS_LOG(DEBUG, "local tablet location cache not exist:table_id[%lu]", table_id);
  // lock and check again
  yysys::CThreadGuard lock(acquire_lock(table_id));
  YYSYS_LOG(DEBUG, "cache not exist check local cache with lock:table_id[%lu]", table_id);
  int ret = tablet_cache_->get(table_id, row_key, location);
  if (ret != OB_SUCCESS || true == force_renew)
  {
    YYSYS_LOG(DEBUG, "cache not hit, begin scan root table with table_id[%ld],rowkey[%s]",
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get tablet location failed:table_id[%lu], ret[%d], rowkey[%s]",
          table_id, ret, to_cstring(row_key));
    }
    else if (0 == location.size())
    {
      ret = OB_INNER_STAT_ERROR;
      YYSYS_LOG(WARN, "check scan root table failed:table_id[%lu], count[%ld]",
        table_id, location.size());
    }
  }
  else
  {
    YYSYS_LOG(DEBUG, "cache item is fetched by other thread:table_id[%lu]", table_id);
  }
  return ret;
}
  ...
  const ObRowkey & row_key, ObTabletLocationList & location)
{
  ...
  {
    // lock and check again
    YYSYS_LOG(DEBUG, "cache timeout get local cache with lock:table_id[%lu]", table_id);
    yysys::CThreadGuard lock(acquire_lock(table_id));
      ...
      > tablet_cache_->get_timeout(table_id)))
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get tablet location from root failed:table_id[%lu], rowkey[%s], ret[%d]",
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "update cache item failed:table_id[%lu], rowkey[%s], ret[%d]",
              table_id, to_cstring(row_key), ret);
        }
      }
    }
    ...
    {
      YYSYS_LOG(DEBUG, "already update the cache item by other thread:table_id[%lu]", table_id);
    }
  }
  return ret;
}
  ...
  const ObRowkey & row_key, ObTabletLocationList & location)
{
  ...
  if (!check_inner_stat())
  {
    YYSYS_LOG(WARN, "%s", "check inner stat failed");
    ret = OB_INNER_STAT_ERROR;
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "fetch tablet location through rpc call failed:table_id[%lu], ret[%d]",
            table_id, ret);
      }
    }
    else
    {
      ...
      if (err != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "update timeout item failed:table_id[%lu], ret[%d]", table_id, err);
      }
      // check invalid location
      if (0 == location.get_valid_count() || 0 == location.size())
      {
        YYSYS_LOG(WARN, "check all the chunk server invalid:size[%ld]", location.size());
        err = del_cache_item(table_id, row_key);
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "del invalid cache item failed:table_id[%lu], ret[%d]", table_id, err);
        }
        // scan root table insert new location item to cache
        ret = renew_location_item(table_id, row_key, location);
        if (ret != OB_SUCCESS)
