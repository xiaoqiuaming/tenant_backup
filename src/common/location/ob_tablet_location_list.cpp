...
int ObTabletLocationList::sort(const ObServer & server, int32_t cluster_id)
//mod:e
{
  // asc 
  int ret = OB_SUCCESS;
  if (cur_count_ > 0)
  {
    ...
    for (int64_t i = cur_count_ - 1; i > 0; --i)
    {
      for (int64_t j = 0; j < i; ++j)
      {
        ...
        do_ip_compare = false;
        // [563]
//        if (cluster_id > MAX_CLUSTER_COUNT || cluster_id < 0)
        if (cluster_id > OB_MAX_CLUSTER_ID || cluster_id < 0)
        {
          ret = OB_INVALID_ARGUMENT;
        }//mod [multiups with paxos]
        //else if (cluster_id == MAX_CLUSTER_COUNT)
        else if (cluster_id == OB_ALL_CLUSTER_FLAG)
        {// mod e
          do_ip_compare = true;
        }
        else if (locations_[j].server_.cluster_id_ == cluster_id && locations_[j+1].server_.cluster_id_ == cluster_id)
        {
          do_ip_compare = true;
        }
        else if (locations_[j].server_.cluster_id_ != cluster_id && locations_[j+1].server_.cluster_id_ == cluster_id)
...
int ObTabletLocationList::del(const int64_t index, ObTabletLocationItem & location)
{
  ...
  {
    YYSYS_LOG(ERROR, "check index failed:index[%ld], count[%ld]", index, cur_count_);
...
int ObTabletLocationList::add(const ObTabletLocation & location)
{
  ...
  {
    YYSYS_LOG(ERROR, "the items is full:count[%ld]", cur_count_);
...
{
  YYSYS_LOG(DEBUG, "print tablet location servers:count[%ld], timestamp[%ld]",
    cur_count_, timestamp_);
  for (int64_t i = 0; i < cur_count_; ++i)
  {
    YYSYS_LOG(DEBUG, "check server error status:times[%lu]", locations_[i].err_times_);
...
DEFINE_SERIALIZE(ObTabletLocationList)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize cur count failed:count[%ld], ret[%d]", 
        cur_count_, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "serailize tablet range failed: ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = 0; i < cur_count_; ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize err times failed:index[%ld], ret[%d]", i, ret);
        break;
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "serailize location failed:index[%ld], ret[%d]", i, ret);
          break;
        }
      }
    }
...
DEFINE_DESERIALIZE(ObTabletLocationList)
{
  ...
  {
    YYSYS_LOG(ERROR, "deserialize timestamp failed:ret[%d], pos=%ld, data_len=%ld", ret, pos, data_len);
  }
  else if (OB_SUCCESS != (ret = serialization::decode_vi64(buf, data_len, pos, &cur_count_)))
  {
    YYSYS_LOG(ERROR, "deserialize cur count failed:ret[%d], pos=%ld, data_len=%ld", ret, pos, data_len);
  }
  else if (OB_SUCCESS != (ret = temp_range.deserialize(buf, data_len, pos)))
  {
    YYSYS_LOG(ERROR, "deserialize tablet range failed: ret[%d], pos=%ld, data_len=%ld", ret, pos, data_len);
  }
  if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = set_tablet_range(temp_range))))
  {
    YYSYS_LOG(WARN,"fail to deep copy tablet range [err:%d], pos=%ld, data_len=%ld", ret, pos, data_len);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = 0; i < cur_count_; ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize err times failed:index[%ld], ret[%d], pos=%ld, data_len=%ld", i, ret, pos, data_len);
        break;
      }
      else if (OB_SUCCESS != (ret = locations_[i].server_.deserialize(buf, data_len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize location failed:index[%ld], ret[%d], pos=%ld, data_len=%ld", i, ret, pos, data_len);
        break;
      }
    }
  }
