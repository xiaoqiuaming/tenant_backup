    ...
    const ObNewRange * iter_range, ScanFlag::Direction scan_direction, ObStringBuf * range_rowkey_buf)
{
  ...
  if (NULL == cache_proxy)
  {
    ...
    YYSYS_LOG(WARN, "cache_proxy is null");
  }

  if (OB_SUCCESS == ret && NULL == range_rowkey_buf)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "range_rowkey_buf is null");
  }

  if (OB_SUCCESS == ret && NULL == iter_range)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "scan_param_in is null");
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "string buf write fail:ret[%d]", ret);
    }
  }
  ...
  ObNewRange& r3, ObStringBuf& range_rowkey_buf) const
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "table id is not equal:r1.table_id_[%ld], r2.table_id_[%ld]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "range r1 or range r2 is emptry:r1 empty[%d], r2 empty[%d]",
  ...
  if (OB_SUCCESS == ret)
  {
    if (r1.intersect(r2)) // if r1 inersect r2 is not empty
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "write start key fail:ret[%d]", ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "write end key fail:ret[%d]", ret);
        }
      }
    }
    else
    {
      YYSYS_LOG(DEBUG, "dump range r1:%s, r2:%s", to_cstring(r1), to_cstring(r2));
    }
  }
  ...
  int32_t & replica_count_in_out, ObNewRange & tablet_range_out)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "ObTabletLocationRangeIterator not init yet");
  }

  if (OB_SUCCESS == ret && NULL == replicas_out)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "replicas_out is null");
  }

  if (OB_SUCCESS == ret && replica_count_in_out <= 0)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "replica_count_in_out should be positive: replica_count_in_out[%d]", replica_count_in_out);
  }
  ...
  {
    YYSYS_LOG(WARN, "get tablet location fail:ret[%d]", ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "replica count get from rootserver is greater than the expectation:replica_count_in_out[%d],\
    ...
    for (int i = 0; (i < size) && (fill_count < replica_count_in_out); ++i)
    {
      ...
      if (location_list[i].err_times_ < ObTabletLocationItem::MAX_ERR_TIMES)
      {
        ...
        replicas_out[fill_count].cluster_id_ = location_list[i].server_.cluster_id_;
        //add:e
        ++fill_count;
      }
    }
    ...
    {
      YYSYS_LOG(DEBUG, "dump location_list.get_tablet_range(%s)", to_cstring(location_list.get_tablet_range()));
      YYSYS_LOG(DEBUG, "dump next_range_(%s)", to_cstring(next_range_));
      YYSYS_LOG(WARN, "intersect next range and location list range fail:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "trim next range fail:ret[%d]", ret);
    }
  }
     ...
     && (scan_direction_ == ScanFlag::BACKWARD))
  {
    ...
    is_iter_end_ = true;
    YYSYS_LOG(INFO, "trigger seralizing access cs when backward scan");
  }
  return ret;
}
