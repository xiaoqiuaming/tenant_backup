...
int ObRowUtil::convert(const ObRow &row, ObString &compact_row, ObRow &out_row)
{
  ...
  for (int64_t i = 0; i < row.get_column_num(); ++i)
  {
    ...
    else if (OB_SUCCESS != (ret = cell_writer.append(column_id, *cell, const_cast<ObObj *>(clone_cell))))
    {
      if (OB_SIZE_OVERFLOW != ret)
      {
        YYSYS_LOG(WARN, "failed to append cell, err=%d size=%ld", ret, cell_writer.size());
      }
      break;
    }
  } // end for i
                       ...
                       )
{
  ...
  if(OB_SUCCESS == ret && NULL != rowkey)
  {
    if(NULL == rowkey_buf)
    {
      ...
      YYSYS_LOG(WARN, "rowkey_buf should not be null");
    }
    ...
    while(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "next cell fail:ret[%d]", ret);
      }
  ...
  {
    YYSYS_LOG(WARN, "fail to read next cell[%d]", ret);
  }
  if (cell_idx != row.get_column_num())
  {
    ...
    if (NULL == row_desc)
    {
      YYSYS_LOG(ERROR, "corrupted row data, row_desc is NULL cell_num=%ld", cell_idx);
    }
    else
    {
      YYSYS_LOG(ERROR, "corrupted row data, row_desc[%s] col=%ld cell_num=%ld", to_cstring(*row_desc), row.get_column_num(), cell_idx);
    }
...
int ObRowUtil::convert(uint64_t table_id, const ObString &compact_row, ObRow &row, bool is_ups_row)
{
  ...
  do
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to get cell, column_id=%lu err=%d", column_id, ret);
      break;
    }
    else if (is_row_finished)
    {
    ...
    else if(ObExtendType == value->get_type())
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to set action flag cell, table_id=%lu column_id=%lu "
    ...
    {
      YYSYS_LOG(WARN, "failed to set cell, table_id=%lu column_id=%lu err=%d",
...
int ObRowUtil::serialize_row(const common::ObRow& row, char* buf, int64_t buf_len, int64_t &pos)
{
  ...
  for (int64_t i = 0; i < row.get_column_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "serialize_row cell[%ld], ret=%d, cell=%p", i, ret, cell);
    }
    else
    {
