...
inline int ObNumber::mul_uint32(const ObNumber &multiplicand, uint32_t multiplier, ObNumber &res, bool did_carry)
{
  ...
  if (did_carry)
  {
    if (carry > 0)
    {
      ...
      {
        YYSYS_LOG(WARN, "multiply overflow, carry=%ld", carry);
...
inline int ObNumber::add_uint32(const ObNumber &n1, uint32_t n2, ObNumber &res)
{
  ...
  if (carry > 0)
  {
    ...
    {
      YYSYS_LOG(WARN, "add overflow, carry=%ld", carry);
      ret = OB_VALUE_OUT_OF_RANGE;
    }
  }
  return ret;
...
int ObNumber::from(const char* str, int64_t buf_len)
{
  ...
  for (int64_t i = 0; i < buf_len; ++i)
  {
    switch(*str)
    {
      ...
      case '.':
        ...
        break;
      default:
        YYSYS_LOG(WARN, "invalid numeric char=%c", *str);
  ...
  {
    YYSYS_LOG(WARN, "too many fractional digits, vscale=%hhd", vscale);
    ret = OB_VALUE_OUT_OF_RANGE;
  }
...
int ObNumber::to_int64(int64_t &i64) const
{
  ...
  {
    YYSYS_LOG(WARN, "the number cannot be converted to int64, vscale=%hhd nwords=%hhd",
              vscale_, nwords_);
    ret = OB_VALUE_OUT_OF_RANGE;
  }
  else
...
int ObNumber::round_to(int8_t precision, int8_t scale, int8_t &nwords, int8_t &vscale, uint32_t *words) const
{
  ...
  {
    ret = OB_VALUE_OUT_OF_RANGE;
    YYSYS_LOG(WARN, "value is not representable with the precision and scale, p=%hhd s=%hhd vp=%hhd vs=%hhd",
...
int ObNumber::add(const ObNumber &other, ObNumber &res) const
{
  ...
  {
    YYSYS_LOG(WARN, "number out of range");
    ret = OB_VALUE_OUT_OF_RANGE;
  }
  else
  {
...
int ObNumber::sub(const ObNumber &other, ObNumber &res) const
{
  ...
  {
    YYSYS_LOG(WARN, "value out of range for sub");
    ret = OB_VALUE_OUT_OF_RANGE;
  }
  else
  {
...
int ObNumber::mul_words(const ObNumber &n1, const ObNumber &n2, ObNumber &res)
{
  ...
  {
    YYSYS_LOG(WARN, "multiply precision overflow, res_nwords=%hhd", res.nwords_);
    ret = OB_VALUE_OUT_OF_RANGE;
  }
  else
  {
...
int ObNumber::div(const ObNumber &other, ObNumber &res) const
{
  ...
  if (other.is_zero())
  {
    YYSYS_LOG(WARN, "divisor is zero");
  ...
  else if (!this->is_zero())
  {
        ...
        || res.vscale_ > dividend.vscale_ - divisor.vscale_)
    {
      ...
      {
        YYSYS_LOG(WARN, "left shift overflow, err=%d res_vscale=%hhd other_vscale=%hhd this_vscale=%hhd",
...
int ObNumber::negate(ObNumber &res) const
{
  ...
  {
    YYSYS_LOG(WARN, "value out of range to do negate");
    ret = OB_VALUE_OUT_OF_RANGE;
  }
  else
  {
