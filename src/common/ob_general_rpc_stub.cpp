...
namespace oceanbase
{
  namespace common
  {
                                          ...
                                          const char* server_version) const
    {
      ...
      {
        YYSYS_LOG(WARN, "get_rpc_buffer failed with rpc call, ret =%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "send param to register server fail, ret: [%d]", ret);
      }
      else if (OB_SUCCESS != (ret = deserialize_result_1(data_buffer, pos, rc, status)))
      {
        YYSYS_LOG(ERROR, "deserialize server register result fail, ret: [%d]", ret);
      }
      else if (OB_SUCCESS != deserialize_result(data_buffer, pos, cluster_id)) /* succ though */
      {
        YYSYS_LOG(WARN, "deserialize cluster_id fail, maybe rootserver has low version.");
      }
      return ret;
    }
    ...
    int ObGeneralRpcStub::fetch_master_server_list(const int64_t timeout, const ObServer & root_server,
                                                   ObUpsList & server_list) const
    {
      return send_0_return_1(root_server, timeout, OB_GET_MASTER_UPDATE_SERVER_INFO_LIST, DEFAULT_VERSION, server_list);
    }
    //add 20150528:e

    //[492]
    int ObGeneralRpcStub::force_fetch_sys_table_update_server_list(const int64_t timeout, const ObServer &root_server,
                                                                   ObUpsList &server_list) const
    {
        return send_0_return_1(root_server, timeout, OB_FORCE_GET_SYS_TABLE_UPDATE_SERVER_INFO_LIST, DEFAULT_VERSION, server_list);
    }
        ...
        ) const
    {
      ...
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "scan root server for get chunk server location failed:"
                    "table_id[%lu], rowkey[%s], ret[%d]", table_id, to_cstring(row_key), ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "scan root server for get chunk server location succ:"
                                       ...
                                       bool if_not_exists, const common::TableSchema & tschema) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_2_return_0 failed ret[%d]", ret);
        YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
      }
      return ret;
    }
    ...
    int ObGeneralRpcStub::whipping_wok(const int64_t timeout, const ObServer &chunk_server, const BlackList list)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_1_return_0 failed: ret[%d]", ret);
        YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
      }
      return ret;
    }
    ...
    //add for [alter group]-b
    int ObGeneralRpcStub::alter_group(const int64_t timeout, const common::ObServer &root_server,
                                      const common::ObString &group_name, const int64_t idx) const
    {
      int ret = OB_SUCCESS;
      ObResultCode result_code;
      ret = send_2_return_0(root_server, timeout, OB_ALTER_GROUP, DEFAULT_VERSION,
                            result_code, group_name, idx);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_2_return_0 failed: ret[%d]", ret);
        if (OB_GROUP_NOT_EXIST == ret)
        {
          YYSYS_LOG(USER_ERROR, "input group name(%.*s) not exist", group_name.length(), group_name.ptr());
        }
        else if (OB_PAXOS_IDX_OUT_RANGE == ret)
        {
          YYSYS_LOG(USER_ERROR, "input paxos[%ld] more than exist paxos_num,please input right paxos id", idx);
        }
        else if (OB_PAXOS_IDX_OFFLINE == ret)
        {
          YYSYS_LOG(USER_ERROR, "input paxos id[%ld] have offline", idx);
        }
        else
        {
          YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
                                         ...
                                         bool if_exists, const common::ObStrings &tables, const common::ObString& user, const common::ObString & comment) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_4_return_0 failed: ret[%d]", ret);
        YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
      }
      return ret;
    }
    ...
    //[view]
    int ObGeneralRpcStub::create_view(const int64_t timeout, const common::ObServer &root_server,
                                      bool do_replace, const common::TableSchema &tschema) const

    {
      int ret = OB_SUCCESS;
      ObResultCode result_code;
      ret = send_2_return_0(root_server, timeout, OB_CREATE_VIEW, DEFAULT_VERSION,
                            result_code, do_replace, tschema);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_2_return_0 failed ret[%d]", ret);
        YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
    ...
    int ObGeneralRpcStub::drop_view(const int64_t timeout, const common::ObServer &root_server,
                                    bool if_exists, const common::ObStrings &views) const
    {
      int ret = OB_SUCCESS;
      ObResultCode result_code;
      ret = send_2_return_0(root_server, timeout, OB_DROP_VIEW, DEFAULT_VERSION,
                            result_code, if_exists, views);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "send_2_return_0 failed: ret[%d]", ret);
        YYSYS_LOG(USER_ERROR, "%.*s", result_code.message_.length(), result_code.message_.ptr());
    ...
    int ObGeneralRpcStub::set_obi_role(const ObServer &rs, const int64_t timeout, const ObiRole &obi_role) const
    {
      ...
      {
        YYSYS_LOG(WARN, "set obi role to rs[%s], failed, ret=%d", to_cstring(rs), ret);
      }
      return ret;
    }
    int ObGeneralRpcStub::set_master_rs_vip_port_to_cluster(const ObServer &rs, const int64_t timeout, const char *new_master_ip, const int32_t new_master_port) const
    {
      ...
      {
        YYSYS_LOG(WARN, "set new master rs vip port  to cluster[%s], failed, ret=%d", to_cstring(rs), ret);
      }
      return ret;
    }
        ...
        int64_t time_stamp, bool has_more)
    {
      ...
      {
        YYSYS_LOG(WARN, "send report tablets message failed, ret=%d", ret);
      }
      else if (!has_more)  // at the end of report process
      {
        ...
        YYSYS_LOG(INFO, "report tablets over, send OB_WAITING_JOB_DONE message.");
        ...
        int64_t time_stamp, bool has_more)
    {
      ...
      if (OB_SUCCESS != (ret = send_3_return_0(root_server, timeout, OB_REPORT_HISTOGRAMS, DEFAULT_VERSION + 1,
                                               client_server, tablets, time_stamp)))
      {
        YYSYS_LOG(WARN, "send report tablets histogram message failed, ret=%d", ret);
      }
      return ret;
    }
    //add e
    ...
    {
      return send_2_return_1(rootserver, timeout, OB_FETCH_INDEX_STAT, NEW_VERSION, index_tid, cluster_id, status);
    }
    //add e
        ...
        const char (*path)[common::OB_MAX_FILE_NAME_LENGTH])
    {
      ...
      {
        YYSYS_LOG(ERROR, "serialize range failed[%d]", ret);
      }
      // dest_disk_no
      ...
      {
        YYSYS_LOG(ERROR, "serialize dest_disk_no failed[%d]", ret);
      }
      // tablet_version
      ...
      {
        YYSYS_LOG(ERROR, "serialize tablet_version failed=[%d]", ret);
      }
      // crc checksum
      else if (OB_SUCCESS != (ret = serialization::encode_vi64(data_buff.get_data(),
                                                               data_buff.get_capacity(), data_buff.get_position(), crc_sum)))
      {
        YYSYS_LOG(ERROR, "serialize crc_sum failed=[%d]", ret);
      }
      // number of sstable files
      else if (OB_SUCCESS != (ret = serialization::encode_vi64(data_buff.get_data(),
                                                               data_buff.get_capacity(), data_buff.get_position(), size)))
      {
        YYSYS_LOG(ERROR, "serialize file num failed=[%d]", ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        for ( int64_t idx =0 ; idx < size; idx++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "serialize file path failed=[%d]", ret);
            break;
          }
        }
      }
      ...
      if (OB_SUCCESS == ret && CS_DEST_LOAD_TABLET_VERSION > 1)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "serialize tablet_seq_num failed=[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send request to chunkserver for dest load tablet"
                    ",ret=[%d].", ret);
        }
        else
        {
          YYSYS_LOG(INFO,"send request to destination server success");
        }
      }
    ...
    int ObGeneralRpcStub::fetch_rs_last_frozen_version(const int64_t timeout, const ObServer &root_server,
                                                       int64_t &last_version) const
    {
      return send_0_return_1(root_server, timeout, OB_RS_GET_LAST_FROZEN_VERSION, DEFAULT_VERSION, last_version);
    }
        ...
        ObTabletLocation location [], int32_t& size)
    {
      ...
      {
        YYSYS_LOG(ERROR,"invalid table id");
        ret = OB_ERROR;
      }
      ...
      if (NULL == table)
      {
        YYSYS_LOG(ERROR,"can not find table %lu",table_id);
        ret = OB_ERROR;
      }
      else
      {
      ...
      {
        YYSYS_LOG(ERROR,"get tablet from rootserver failed:[%d]",ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        while((OB_SUCCESS == ret) && index < size)
        {
          ...
          {
            YYSYS_LOG(DEBUG,"row :%s changed,ignore", to_cstring(cell->row_key_));
          ...
          else if (cell != NULL)
          {
              ...
              //add:e
            {
              ...
              YYSYS_LOG(DEBUG,"port is %ld",port);
            }
              ...
              //add:e
            {
              ...
              YYSYS_LOG(DEBUG,"tablet_version is %ld, rowkey:%s",version, to_cstring(cell->row_key_));
            }
            //add zhaoqiong [MultiUPS] [RootTable_Cache] 20150703:b
                     ...
                     || (cell->column_name_.compare("6_cluster_id") == 0))
            {
              ...
              YYSYS_LOG(DEBUG,"cluster_id is %ld, rowkey:%s",cluster_id, to_cstring(cell->row_key_));
            }
            //add:e
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              if (0 != port && 0 != ip && 0 != version && -1!= cluster_id) //mod zhaoqiong [MultiUPS] [RootTable_Cache] 20150703
              {
                server.set_ipv4_addr(static_cast<int32_t>(ip), static_cast<int32_t>(port));
                YYSYS_LOG(INFO,"add tablet s:%s,%ld", to_cstring(server), version);
                //ObTabletLocation addr(version, server);
                ObTabletLocation addr(version, server, static_cast<int32_t>(cluster_id)); //mod zhaoqiong [MultiUPS] [RootTable_Cache] 20150703
                location[index++] = addr;
                ip = port = version = 0;
                cluster_id = -1;
              }
      ...
      {
        size = index;
        YYSYS_LOG(INFO,"get %d tablets from rootserver",size);
      }
      return ret;
    }
                               ...
                               const ObScanParam & scan_param, ObScanner & scanner) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "serialize scan param failed:ret[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send scan request to server failed:ret[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "scan data succ from server:addr[%s]", to_cstring(server));
      }
      else
      {
        YYSYS_LOG(WARN, "scan data failed from server:addr[%s], ret[%d]", to_cstring(server), ret);
      }
                              ...
                              const ObGetParam & get_param, ObScanner & scanner) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "serialize get_param failed:ret[%d]", ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send get request to server failed:ret[%d]", ret);
        }
      }
      // step 3. deserialize the response result
      int64_t pos = 0;
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "get data succ from server:addr[%s]", to_cstring(server));
      }
      else
      {
        YYSYS_LOG(WARN, "get data failed from server:addr[%s], ret[%d]", to_cstring(server), ret);
      }
                               ...
                               ObScanner & scanner, bool & update_list) const
    {
      ...
      if (0 == list.size())
      {
        YYSYS_LOG(WARN, "%s", "check list size is zero");
      ...
      else
      {
        ...
        {
          list.set_item_valid(yysys::CTimeUtil::getTime());
        }
        ...
        for (int32_t i = 0; i < list.size(); ++i)
        {
          ...
          {
            YYSYS_LOG(DEBUG, "check server err times too much:times[%ld]", list[i].err_times_);
            continue;
          }
          scanner.clear();
          ...
          {
            YYSYS_LOG(WARN, "check chunk server position failed:pos[%d], count[%ld], ret[%d]",
                      i, list.size(), ret);
          ...
          {
            YYSYS_LOG(WARN, "scan from chunk server failed:pos[%d], count[%ld], ret[%d]",
                      i, list.size(), ret);
            ++list[i].err_times_;
            update_list = true;
            continue;
          ...
          {
            YYSYS_LOG(DEBUG, "scan from chunk server succ:pos[%d], count[%ld]", i, list.size());
                              ...
                              ObScanner & scanner, bool & update_list) const
    {
      ...
      else
      {
        ...
        for (int32_t i = 0; i < list.size(); ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "get from chunk server failed:pos[%d], count[%ld], ret[%d]",
                      i, list.size(), ret);
            ++list[i].err_times_;
            update_list = true;
            continue;
          ...
          {
            YYSYS_LOG(DEBUG, "get from chunk server succ:pos[%d], count[%ld]", i, list.size());
            if (list[i].err_times_ != 0)
            {
              list[i].err_times_ = 0;
              update_list = true;
    ...
    int ObGeneralRpcStub::ups_get_stmt_start_time(const int64_t timeout, const ObServer &ups, int64_t &publised_transid) const
    {
      int ret = OB_SUCCESS;
      ObResultCode result_code;
      ret = send_1_return_1(ups,timeout,OB_GET_PUBLICED_TRANCEID,DEFAULT_VERSION,result_code,publised_transid);
      return ret ;
    }
    // add by maosy e
    //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
    /*@berif �������׶��ύ����Э����ups �������׶��ύ����*/
    int ObGeneralRpcStub::ups_end_dis_trans(const int64_t timeout, const ObServer & ups,
                                            const ObEndTransReq &req) const
    {
      int ret = OB_SUCCESS;
      ObResultCode result_code;
      YYSYS_LOG(DEBUG, "send OB_END_DISTRIBUTED_TRANS packet to %s", to_cstring(ups));
      ret = send_1_return_0(ups, timeout, OB_END_DISTRIBUTED_TRANS, DEFAULT_VERSION,
                            result_code, req);
      return ret;
    }

    int ObGeneralRpcStub::ups_rollback_trans(const int64_t timeout, const ObServer & ups,
                                             const ObTransID &trans_id) const
    {
      int ret = OB_SUCCESS;
      ret = post_request_1(ups, timeout, OB_ROLLBACK_TRANSACTION, DEFAULT_VERSION,
                           ObTbnetCallback::default_callback, NULL, trans_id);
      return ret;
    }
    int ObGeneralRpcStub::ups_prepare_trans(const int64_t timeout, const ObServer & ups,
                                            const ObEndTransReq &req) const
    {
      int ret = OB_SUCCESS;
      ret = post_request_1(ups, timeout, OB_PREPARE_TRANSACTION, DEFAULT_VERSION,
                           ObTbnetCallback::default_callback, NULL, req);
      return ret;
    }
    int ObGeneralRpcStub::ups_commit_trans(const int64_t timeout, const ObServer & ups,
                                           const ObTransID &trans_id) const
    {
      int ret = OB_SUCCESS;
      ret = post_request_1(ups, timeout, OB_COMMIT_TRANSACTION, DEFAULT_VERSION,
                           ObTbnetCallback::default_callback, NULL, trans_id);
      return ret;
    }

    int ObGeneralRpcStub::ups_response_preapre_ack(const int64_t timeout, const ObServer & ups,
                                                   const ObTransID &trans_id, const ObServer &participant,
                                                   const bool is_rollback) const
    {
      int ret = OB_SUCCESS;
      //YYSYS_LOG(DEBUG, "send OB_TRANS_PREPARE_ACK packet");
      ret = post_request_3(ups, timeout, OB_TRANS_PREPARE_ACK, DEFAULT_VERSION,
                           ObTbnetCallback::default_callback, NULL, trans_id, participant, is_rollback);
      return ret;
    }

    int ObGeneralRpcStub::ups_write_trans_stat_ack(const int64_t timeout, const ObServer & ups,
                                                   const ObTransID &trans_id, const bool commit) const
    {
      int ret = OB_SUCCESS;
      //YYSYS_LOG(DEBUG, "send OB_WRITE_TRANS_STAT_ACK packet");
      ret = post_request_2(ups, timeout, OB_WRITE_TRANS_STAT_ACK, DEFAULT_VERSION,
                           ObTbnetCallback::default_callback, NULL, trans_id, commit);
      return ret;
    }

    int ObGeneralRpcStub::find_session_exist(const int64_t timeout, const common::ObTransID& trans_id, bool &coordinator_exist)
    {
      return send_1_return_1(trans_id.ups_, timeout, OB_CHECK_SESSION_EXIST, DEFAULT_VERSION, trans_id, coordinator_exist);
    }

    //add 20150701:e

    //add for [handle unsetted trans]-b
    int ObGeneralRpcStub::is_coordinator_master_ups_tmplog_replay_finished(const int64_t timeout, const ObServer &coordinator_master_ups, bool &tmplog_replay_finished)
    {
      return send_1_return_1(coordinator_master_ups, timeout, OB_GET_UPS_TMPLOG_REPLAY_FINISHED, DEFAULT_VERSION, coordinator_master_ups, tmplog_replay_finished);
    }
    //add for [handle unsetted trans]-e
    ...
    int ObGeneralRpcStub::get_ups_log_seq(const common::ObServer &ups, const int64_t timeout, int64_t & log_seq) const
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "get log seq from update server failed, ret=%d", ret);
      }
      return ret;
    }
        ...
        ObString &bf_buffer) const
    {
      ...
      {
        YYSYS_LOG(WARN, "get_rpc_buffer error with rpc call, ret =%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "send request to chunkserver for get_bloomfilter,ret[%d].", ret);
      }
      else if (OB_SUCCESS != (ret = deserialize_result_3(data_buff, pos,
                                                         result_code, response_buffer, is_fullfilled, total_size)))
      {
        YYSYS_LOG(WARN, "get_bloomfilter deserialize_result,ret[%d].", ret);
      }
      else if (OB_SUCCESS != (ret = result_code.result_code_))
      {
        YYSYS_LOG(WARN, "get_bloomfilter result_code:[%d].", ret);
      }
      else if (NULL == (ptr = reinterpret_cast<char*>(ob_tc_malloc(total_size, ObModIds::OB_BLOOM_FILTER))))
      {
        YYSYS_LOG(WARN, "allocate memory error, size[%ld].", total_size);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
        YYSYS_LOG(TRACE, "BF:first rpc return res[%p,%d], is_fullfilled[%d], total_size[%ld]",
        ...
        while (OB_SUCCESS == ret && !is_fullfilled && session_id != 0)
        {
          ...
          {
            YYSYS_LOG(INFO, "BF:next rpc return res[%p,%d], is_fullfilled[%d], total_size[%ld]",
                      response_buffer.ptr(), response_buffer.length(), is_fullfilled, total_size);
            memcpy(ptr, response_buffer.ptr(), response_buffer.length());
            ptr += response_buffer.length();
          }
