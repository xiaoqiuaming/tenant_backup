    ...
    class ObTriggerEvent
    {
      public:
        ...
        int set_rpc_stub(ObGeneralRpcStub *rpc_stub)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid rpc stub. null pointer.");
        ...
        int set_ms_provider(ObMsProvider *provider)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid ms provider. null pointer.");
            ret = OB_INVALID_ARGUMENT;
          }
          return ret;
        }
        ...
        int execute_sql(ObString &sql_str)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to get ms from ms_provider. ret=%d", ret);
            }
            else if (OB_SUCCESS != (ret = rpc_stub_->execute_sql(timeout, ms, sql_str)))
            {
              YYSYS_LOG(WARN, "fail to execute sql on ms(%s). ret=%d", to_cstring(ms), ret);
            }
          }
          return ret;
        }
