...
namespace oceanbase
{
  namespace common
  {
    ...
    void ObSingleServer::destroy()
    {
      ...
      YYSYS_LOG(WARN, "task threads stoped.");
      ObBaseServer::destroy();
      YYSYS_LOG(WARN, "libeasy timer thread stoped.");
    }
    ...
    int ObSingleServer::set_min_left_time(const int64_t left_time)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "left time should positive, you provide: %ld", left_time);
      }
      else
      {
    ...
    int ObSingleServer::set_thread_count(const int thread_count)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "thread count should positive, you provide: %d", thread_count);
      }
      else
      {
    ...
    int ObSingleServer::set_io_thread_count(const int io_thread_count)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "io thread count should bigger than one, you provide: %d", io_thread_count);
      }
      else
      {
    ...
    int ObSingleServer::set_default_queue_size(const int task_queue_size)
    {
      ...
      if (task_queue_size <= 0)
      {
        ...
        YYSYS_LOG(WARN, "default queue size should be positive, you provide %d", task_queue_size);
      }
      else
      {
        ...
        {
          YYSYS_LOG(TRACE, "huge task queue size(%d), make sure this is what you want.", task_queue_size_);
        }
      }
      return ret;
    }
    ...
    int ObSingleServer::handlePacket(ObPacket* packet)
    {
      int rc = OB_SUCCESS;
      ObPacket* req = packet;
      //YYSYS_LOG(DEBUG, "receive packet, pcode=%d channel=%d", req->get_packet_code(), req->get_channel_id());
      if (thread_count_ == 0)
      {
        handle_request(req);
      }
      else
      {
        ...
        if (!ps)
        {
          ...
          {
            YYSYS_LOG(WARN, "overflow packet dropped, packet code: %d", req->get_packet_code());
          }
    ...
    */
    ...
    bool ObSingleServer::handlePacketQueue(ObPacket *packet, void *args)
    {
      ...
      int64_t current_ts = yysys::CTimeUtil::getTime();
      if (source_timeout > 0)
      {
        ...
        if ((current_ts - receive_time) + min_left_time_ > source_timeout)
        {
          ...
          YYSYS_LOG(WARN, "packet block time: %ld(us), plus min left time: %ld(us) "
    ...
    void ObSingleServer::handle_request(ObPacket *request)
    {
      if (request == NULL)
      {
        YYSYS_LOG(WARN, "handle a NULL packet");
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "request process failed, pcode: %d, ret: %d, peer is %s",
    ...
    bool ObSingleServer::handle_overflow_packet(ObPacket* base_packet)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed initialize out_buff, buffer=%p", my_buffer);
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to send response to client(%s)", inet_ntoa_r(addr));
        }
      }
      else
      {
        easy_request_wakeup(base_packet->get_request());
        YYSYS_LOG(WARN, "easy_request_wakeup overflow packet(%p) request(%p)", base_packet, base_packet->get_request());
      }
      return false;
    }
    ...
    void ObSingleServer::handle_timeout_packet(ObPacket* base_packet)
    {
      ...
      {
        //[701]
        easy_request_t *req=base_packet->get_request();
        if (ObBaseServer::check_request_cleanup(req, __FUNCTION__) == OB_SUCCESS) {
          easy_request_wakeup(req);
          YYSYS_LOG(WARN, "easy_request_wakeup packet(%p) request(%p)", base_packet, req);
        } else {
          YYSYS_LOG(WARN, "packet(%p) cleanup request(%p)", base_packet, req);
        }
      }
    }
