...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObBypassTaskInfo::push_back(std::pair<common::ObString, uint64_t> info)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to write string to buf. err=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to push new item. ret=%d", ret);
        }
      }
      return ret;
    }
    uint64_t ObBypassTaskInfo::get_table_id(const char* table_name) const
    {
      ...
      if (NULL == table_name)
      {
        YYSYS_LOG(WARN, "invalid table_name. table_name=%p", table_name);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "not find table id. table_name=%s", table_name);
        }
      }
      return table_id;
    }
    ...
    DEFINE_SERIALIZE(ObBypassTaskInfo)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to encode operation_type. err=%d", ret);
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to encode count, pos=%ld", pos);
        }
      }
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; i < num; i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to serialize table_name. pos=%ld, table_name=%s", pos, to_cstring(table_name));
            break;
          }
          ret = serialization::encode_vi64(buf, buf_len, pos, name_id.second);
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to encode table_id. table_id=%lu, pos=%ld", name_id.second, pos);
            break;
          }
        }
      }
    ...
    DEFINE_DESERIALIZE(ObBypassTaskInfo)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to decode operation_type, ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to decode count. ret=%d", ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = 0; i < count; i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to deserilize table_name. ret=%d", ret);
            break;
          }
          int64_t table_id = 0;
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to decode table_id. ret=%ld", pos);
            break;
          }
          ret = push_back(std::make_pair(table_name, table_id));
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to push back item. table_name=%s, table_id=%lu", table_name.ptr(), table_id);
            break;
          }
        }
      }
