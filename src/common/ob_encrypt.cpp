  ...
  namespace common
  {
    ...
    int DES_crypt::stringTobyte(char* dest, const int64_t dest_len, const char* src, const int64_t src_len)
    {
      ...
      if(dest == NULL || src == NULL || dest_len < src_len * 8)
      {
        ...
        YYSYS_LOG(WARN, "invalid param. dest = %p, src = %p, dest_len = %ld, src_len = %ld", dest, src, dest_len, src_len);
      }
      int64_t tmp = 0;
      if(OB_SUCCESS == err)
      {
    ...
    int DES_crypt::byteTostring(char* dest, const int64_t dest_len, const char *src, const int64_t src_len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument. dest=%p, dest_len=%ld, src=%p, src_len=%ld", dest, dest_len, src, src_len);
      }
      int32_t op = 0x0000;
      if(OB_SUCCESS == err)
      {
        for(int64_t i = 0; i < src_len; i++)
        {
          ...
          {
            err = OB_INVALID_ARGUMENT;
            YYSYS_LOG(WARN, "invalid argument, char is [%c], but should be 0 or 1", src[i]);
            break;
          }
          else
          {
    ...
    int DES_crypt::pad(char* dest, const int64_t dest_len, int64_t &slen)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument. dest =%p, dest_len = %ld, slen = %ld, OB_MAX_TOKEN_BUFFER_LENGTH =%ld", dest, dest_len, slen, OB_MAX_TOKEN_BUFFER_LENGTH);
      }
      if(OB_SUCCESS == err)
      {
    ...
    int DES_crypt::unpad(char *src, int64_t &slen)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument, src =%p, slen = %ld, OB_MAX_TOKEN_BUFFER_LENGTH= %ld",src, slen, OB_MAX_TOKEN_BUFFER_LENGTH);
      }
      else
      {
        for(int i = original_txt_length_; i < OB_MAX_TOKEN_BUFFER_LENGTH; i++)
        {
          ...
          {
            err = OB_ERROR;
            YYSYS_LOG(WARN, "fail in unpad txt. the char is '%c', but should be '$'",src[i]);
          }
        }
      }
      if(OB_SUCCESS == err)
    ...
    int DES_crypt::des_encrypt(const ObString &user_name, const int64_t timestamp, const int64_t skey, char *buffer, int64_t &buffer_length)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument. buffer_length = %ld, should be 80", buffer_length);
      }
      if(OB_SUCCESS == err)
      {
        ...
        {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "user_name should not be null. user_name.length=%d", user_name.length());
        }
      }
      if(OB_SUCCESS == err)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to stringTobyte. err=%d", err);
        }
        else
        {
      ...
      if(OB_SUCCESS == err)
      {
        ...
        if(OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to pad array. err=%d", err);
        }
      }
      int64_t pos = 0;
      if(OB_SUCCESS == err)
      ...
      if(OB_SUCCESS == err)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to translate string to byte.err=%d", err);
        }
      }
      if(OB_SUCCESS == err)
      {
      ...
      if(OB_SUCCESS == err)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to translate to string.err=%d", err);
        }
        else
        {
    ...
    int DES_crypt::des_decrypt(ObString &user_name, int64_t &timestamp, const int64_t skey, char *buffer, int64_t buffer_length)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "invalid argument.err=%d, buffer=%p,buffer_length =%ld", err, buffer, buffer_length);
      }
      if(OB_SUCCESS == err)
      {
        data_.initialized = 0;
      ...
      if(OB_SUCCESS == err)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to tranlate to byte. err=%d", err);
        }
      }
      char *ptr = txtbyte_;
      if(OB_SUCCESS == err)
      ...
      if(OB_SUCCESS == err)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to tranlate to string. err=%d", err);
        }
      }
      if(OB_SUCCESS == err)
      {
        ...
        if(magic_number != OB_CRYPT_MAGIC_NUM)
        {
          ...
          YYSYS_LOG(WARN, "magic number not legal.maybe the wrong skey,or the wrong data, err = %d", err);
        }
        else
        {
          pos += sizeof(int32_t);
