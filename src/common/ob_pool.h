...
//add support for arm platform by wangd 202106:b
        ...
        LockT lock_  __attribute__((__aligned__(8)));
#else
...
#endif
//add support for arm platform by wangd 202106:e
          if (sz_align > block_pool_.get_obj_size())//mod support for arm platform by wangd 202106
          {
            YYSYS_LOG(ERROR, "Wrong block size, sz_align=%ld block_size=%ld", sz_align, block_pool_.get_obj_size());//mod support for arm platform by wangd 202106
          }
          else
          {
    ...
    class ObWrapperAllocator: public ObIAllocator
    {
      public:
        ...
        {};
        ObWrapperAllocator(int32_t mod_id):alloc_(NULL)
        { UNUSED(mod_id); };
        virtual ~ObWrapperAllocator()
        {};
        void *alloc(const int64_t sz)
        { return alloc_->alloc(sz); };
        void free(void *ptr)
        { alloc_->free(ptr); };
        void set_mod_id(int32_t mod_id)
        { alloc_->set_mod_id(mod_id); };
