...
int ObLogFile::assign(const char* filename, FileType &type)
{
  ...
  if (NULL == filename)
  {
    YYSYS_LOG(ERROR, "parameter filename=NULL. SHOULD NOT REACH HERE");
    ret = OB_ERROR;
  }
  else
  {
...
int ObLogDirScanner::init(const char* log_dir)
{
  ...
  if (is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObLogDirScanner has been initialized.");
    ret = OB_INIT_TWICE;
  }
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      YYSYS_LOG(INFO, "ObLogDirScanner initialize successfully[min_log_id_=%lu max_log_id_=%lu max_ckpt_id_=%lu]",
    ...
    {
      YYSYS_LOG(INFO, "search_log_dir_[log_dir=%s] error[ret=%d], ObLogDirScanner initialize failed", log_dir, ret);
    }
  }
...
int ObLogDirScanner::search_log_dir_(const char* log_dir)
{
  ...
  if (NULL == plog_dir)
  {
    ...
    if (err != 0
        && EEXIST != errno)
    {
      YYSYS_LOG(ERROR, "mkdir[\"%s\"] error[%s]", log_dir, strerror(errno));
      ret = OB_ERROR;
    }
    else
    {
      ...
      if (NULL == plog_dir)
      {
        YYSYS_LOG(ERROR, "opendir[\"%s\"] error[%s]", log_dir, strerror(errno));
        ret = OB_ERROR;
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    while (0 == err && NULL != pentry)
    {
      ...
      else
      {
        if (ObLogFile::LOG == file_type)
        {
          ...
          YYSYS_LOG(DEBUG, "find a valid log file(\"%s\")", log_file.name);
        }
        else if (ObLogFile::CKPT == file_type)
        {
          YYSYS_LOG(DEBUG, "find a valid checkpoint file(\"%s\")", log_file.name);
        ...
        {
          YYSYS_LOG(DEBUG, "ignore file(\"%s\"): \"%s\" is not valid log file",
    ...
    if (0 != err)
    {
      YYSYS_LOG(ERROR, "readdir_r error(ret[%d], pentry[%p], plog_dir[%p]", err, pentry, plog_dir);
      ret = OB_ERROR;
    }
    err = closedir(plog_dir);
    if (err < 0)
    {
      YYSYS_LOG(ERROR, "closedir[DIR=%p] error[%s]", plog_dir, strerror(errno));
      ret = OB_SUCCESS;
    }
  }
