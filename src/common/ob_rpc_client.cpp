...
int ObRpcClient::init(void)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "alloc param temp buffer failed");
  }
  /// init receive buffer
  if (OB_SUCCESS == ret)
  {
...
int ObRpcClient::construct_request(const int64_t timeout, const ObScanParam & param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "param serialize failed:ret[%d]", ret);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "serialize packet failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret) 
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "encode packet failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObRpcClient::parse_response(ObScanner & result, int & code)
{
  ...
  {
    YYSYS_LOG(WARN, "get packet info failed:ret[%d]", ret);
  }
  else
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "decode packet failed:ret[%d]", ret);
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "deserialize packet failed:ret[%d]", ret);
      }
    }
  }
  if (OB_SUCCESS == ret)
...
int ObRpcClient::parse_packet(ObScanner & result, int & code)
{
  ...
  else if (OB_SUCCESS == (code = result_code.result_code_))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "deserialize result failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObRpcClient::scan(const ObScanParam & param, const int64_t timeout, ObScanner & result)
{
  ...
  if (init_)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "rquest failed:ret[%d]", ret);
      }
    }
    // parse response
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "parse response failed:server[%s], ret[%d]", to_cstring(server_), ret);
        close();
      }
      if (code != OB_SUCCESS)
      {
...
int ObRpcClient::write(void)
{
  ...
  {
    close();
    YYSYS_LOG(WARN, "socket write failed:server[%s], len[%d], ret[%d]",
...
int ObRpcClient::read(const int64_t timeout)
{
  ...
  if (OB_SUCCESS == ret) 
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "socket read failed:server[%s], ret[%d]", to_cstring(server_), ret);
      close();
    }
  }
  return ret;
...
int ObRpcClient::connect(void)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "stauts error not init");
  }
  else if (!ready_)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "connect failed:server[%s]", to_cstring(server_));
    }
  }
  return ret;
}
