  ...
  namespace common
  {
    ...
    class ObBitmap
    {
      ...
      private:
        ...
        int for_each_block(Pred pred) const;
    ...
    int ObBitmap<Block, Allocator>::set(const size_type pos, const bool value)
    {
      ...
      if (NULL != mem_block)
      {
        if (value)
        {
          ...
          mem_block->bits_[block_index(inner_pos)] = static_cast<block_type>(bit_mask(inner_pos) | mem_block->bits_[(block_index(inner_pos))]);
