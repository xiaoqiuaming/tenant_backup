...
namespace oceanbase
{
  namespace common
  {
    ...
    inline ObRecursiveMutex::ObRecursiveMutex()
    {
      ...
      if (0 != ret)
      {
        YYSYS_LOG(ERROR, "init mutex attr failed, err=%s", strerror(ret));
      }
      else
      {
        ...
        if (0 != ret)
        {
          YYSYS_LOG(ERROR, "set mutex attr failed, err=%s", strerror(ret));
        }
        else
        {
          ...
          if (0 != ret)
          {
            YYSYS_LOG(ERROR, "mutex init failed, err=%s", strerror(ret));
          }
        }
      }
    }
