...
namespace oceanbase
{
  namespace common
  {
    void hex_dump(const void* data, const int32_t size,
                  const bool char_type /*= true*/, const int32_t log_level_const /*= YYSYS_LOG_LEVEL_DEBUG*/)
    {
      int32_t log_level = log_level_const;
      if (YYSYS_LOGGER._level < log_level) return;
      ...
      for(n = 1; n <= size; n++)
      {
        ...
        if (n % 16 == 0)
        {
          /* line completed */
          if (char_type)
            YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level), "[%ld] [%4.4s]   %-50.50s  %s\n",
                                    pthread_self(), addrstr, hexstr, charstr);
          else
            YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level), "[%ld] [%4.4s]   %-50.50s\n",
                                    pthread_self(), addrstr, hexstr);
          hexstr[0] = 0;
          charstr[0] = 0;
          if(errorCnt == 0 && log_level == YYSYS_LOG_LEVEL_ERROR)
          {
      ...
      else
      {
        while (pos < dlen)
        {
          if(data[pos] >= '0' && data[pos] <= '9')
          {
            ...
            {
              YYSYS_LOG(ERROR, "value overflow, legal range[%lu, %lu], input invalid numeric string[%.*s]", (uint64_t)0, UINT64_MAX, static_cast<int>(dlen), data);
              value = 0;
              ret = -1;
              break;
            }
          ...
          else
          {
            value = 0;
            ret = -1;
            YYSYS_LOG(ERROR, "input invalid numeric string[%.*s], invalid character(non numeric)=[%c]", (int)dlen, data, data[pos]);
            break;
          }
        }
      }
    ...
    int tokenize(const char *data, int64_t dlen, char delima, int &token_nr, Token *tokens)
    {
      ...
      {
        ret = -1;
        YYSYS_LOG(ERROR, "parameters must be well inited");
        return ret;
      }
      ...
      {
        YYSYS_LOG(ERROR, "token num mustn't be higher than %d", token_nr);
        ret = -1;
      }
      token_nr = token;
      return ret;
    ...
    int mem_chunk_serialize(char* buf, int64_t len, int64_t& pos, const char* data, int64_t data_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "encode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, data_len, err);
      }
    ...
    int mem_chunk_deserialize(const char* buf, int64_t len, int64_t& pos, char* data, int64_t data_len, int64_t& real_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "decode_i64(buf=%p, len=%ld, pos=%ld, i=%ld)=>%d", buf, len, tmp_pos, real_len, err);
      }
    ...
    //get esit distance of two strings src and dest using dynamic programing algorithm
    //not the length of string src and dest should less than 2048 bytes
    int64_t get_edit_distance(const char *src, const char *dest)
    {
      int64_t src_len = strlen(src);
      int64_t dest_len = strlen(dest);

      int64_t distance[OB_MAX_FETCH_CMD_LENGTH + 1][OB_MAX_FETCH_CMD_LENGTH + 1];
      ...
      return distance[src_len][dest_len];
    }
    ...
    int64_t convert_addr_to_server(easy_addr_t addr)
    {
      ...
      {
        YYSYS_LOG(WARN, "no port information %s", buffer);
      }
      uint64_t lport = port;
      server_id = lport << 32;
    ...
    void dump_scanner(const common::ObScanner &scanner, const int32_t log_level, const int32_t type)
    {
      ...
      else // dump by row
      {
        ...
        for (; iter != scanner.row_end(); ++iter, ++row_num)
        {
          ...
          YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level), "%s", row_buffer);
        }
      }
    }
    ...
    const char * print_role(const ObRole role)
    {
      ...
      {
        case OB_CHUNKSERVER:
          role_string = "yaodatasvr";
          break;
        case OB_MERGESERVER:
          role_string = "yaosqlsvr";
          break;
        case OB_ROOTSERVER:
          role_string = "yaoadminsvr";
          break;
        case OB_UPDATESERVER:
          role_string = "yaotxnsvr";
          break;
        default:
                    ...
                    const char* match_str, const char* replace_str)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, src_str=%p, src_str_buf_size=%ld, "
      ...
      else if (NULL != (find_pos = strstr(src_str, match_str)))
      {
        ...
        while(NULL != find_pos)
        {
          ...
          {
            YYSYS_LOG(WARN, "str after replace is too large, new_size=%ld, "
                           ...
                           oceanbase::common::ObExprObj &min)
    {
         ...
         max.get_type() == ObNullType)
      {
        ...
        ret = true;
              ...
              max.get_type() == ObNullType)
      {
        YYSYS_LOG(WARN, "min or max is null cause can't compute proportion.");
        ret = true;
      }
      else if(min.get_type() != max.get_type())
      {
        YYSYS_LOG(WARN, "min and max is not same type.");
      }
            ...
            else
            {
              ...
              switch(min.get_type())
              {
                ...
                case ObModifyTimeType:
                {
                  ...
                  {
                    YYSYS_LOG(WARN, "value obj cast fail.");
                  }
              ...
              switch(result.get_type())
              {
                ...
                case ObDecimalType:
                {
                  ...
                  {
                    proportion = 0.0;
                    YYSYS_LOG(WARN, "cast to double fail.");
                  }
                  break;
                }
                default:
      ...
      if(proportion > 1.0)
      {
        YYSYS_LOG(WARN, "compute may be happen error, proportion =%.20lf", proportion);
        proportion = 1.0;
      }
      ...
      YYSYS_LOG(DEBUG, "proportion = %.20lf", proportion);
      return proportion;
    }

    bool is_numerical_type(const oceanbase::common::ObExprObj &value)
    {
      bool flag = false;
