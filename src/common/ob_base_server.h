  ...
  namespace common
  {
    class ObBaseServer
    {
      public:
        ...
        int check_request_cleanup(easy_request_t *req, const char* fn);//[648]

        //[701]-b
        int disconn_clear_task(int32_t seq, int64_t ref) {
          int ret = 0;
          if (disconn_pkt_que_) ret = disconn_pkt_que_->disconn_clear(seq, ref);
          return ret;
        }
        void add_disconn_pkt_queue(ObPacketQueueThread &que) {    disconn_pkt_que_ = &que;};
        //[701]-e
      ...
      protected:
        ...
        uint32_t local_ip_;
        ObPacketQueueThread* disconn_pkt_que_; //[701]
