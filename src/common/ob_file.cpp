  ...
  namespace common
  {
    namespace FileComponent
    {
      ...
      int open(const ObString &fname, const T &file, int &fd)
      {
        ...
        if (-1 != fd)
        {
          YYSYS_LOG(WARN, "file has been open fd=%d", fd);
          ret = OB_INIT_TWICE;
        }
        ...
        else
        {
          ...
          if (NULL == fname_ptr)
          {
            YYSYS_LOG(WARN, "prepare fname string fail fname=[%.*s]", fname.length(), fname.ptr());
            ret = OB_INVALID_ARGUMENT;
          }
          else if (-1 == (fd = ::open(fname_ptr, file.get_open_flags(), file.get_open_mode())))
          {
            ...
            {
              YYSYS_LOG(WARN, "open fname=[%s] fail errno=%u", fname_ptr, errno);
              ret = OB_IO_ERROR;
            }
          }
          else
          {
            YYSYS_LOG(INFO, "open fname=[%s] fd=%d flags=%d succ", fname_ptr, fd, file.get_open_flags());
          }
        }
        return ret;
      }
      ...
      int BufferFileReader::pread_by_fd(const int fd, void *buf, const int64_t count, const int64_t offset, int64_t &read_size)
      {
        ...
        {
          YYSYS_LOG(WARN, "file has not been open");
          ret = OB_ERROR;
        }
        else if (0 > (read_ret = unintr_pread(fd, buf, count, offset)))
        {
          YYSYS_LOG(WARN, "read fail fd=%d count=%ld offset=%ld read_ret=%ld errno=%u",
      ...
      int BufferFileReader::pread_by_fd(const int fd, const int64_t count, const int64_t offset, IFileBuffer &file_buf, int64_t &read_size)
      {
        ...
        {
          YYSYS_LOG(WARN, "file_buf assign fail count=%ld ret=%d or get_buffer null pointer", count, ret);
      ...
      int DirectFileReader::pread_by_fd(const int fd, void *buf, const int64_t count, const int64_t offset, int64_t &read_size)
      {
        ...
        {
          YYSYS_LOG(WARN, "prepare buffer fail param_align=%s buffer=%p buf=%p count=%ld offset=%ld align_size=%ld buffer_size=%ld",
      ...
      int IFileAppender::create(const ObString &fname)
      {
        ...
        {
          YYSYS_LOG(WARN, "open file error:ret=%d,fname=%s,fd_=%d",
                    ret, fname.ptr(), fd_);
        }
        else
        {
      ...
      int BufferFileAppender::buffer_sync_()
      {
            ...
            && 0 != buffer_pos_)
        {
          ...
          {
            YYSYS_LOG(WARN, "write buffer fail fd=%d buffer=%p count=%ld offset=%ld write_ret=%ld errno=%u",
                      fd_, buffer_, buffer_pos_, file_pos_, write_ret, errno);
            ret = OB_IO_ERROR;
          }
          else
          {
            YYSYS_LOG(DEBUG, "write buffer succ fd=%d buffer_size=%ld file_pos=%ld", fd_, buffer_pos_, file_pos_);
      ...
      int BufferFileAppender::fsync()
      {
        ...
        else if (OB_SUCCESS == (ret = buffer_sync_()))
        {
          if (0 != ::fsync(fd_))
          {
            YYSYS_LOG(WARN, "fsync fail fd=%d errno=%u", fd_, errno);
            ret = OB_IO_ERROR;
          }
        }
        return ret;
      ...
      int BufferFileAppender::append(const void *buf, const int64_t count, const bool is_fsync)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid param buf=%p count=%ld", buf, count);
          ret = OB_INVALID_ARGUMENT;
        }
        else
        {
          ...
          if (OB_SUCCESS == ret)
          {
            if ((buffer_size_ - buffer_pos_) < count)
            {
              ...
              {
                YYSYS_LOG(WARN, "write fail fd=%d buffer=%p count=%ld offset=%ld write_ret=%ld errno=%u",
      ...
      int BufferFileAppender::prepare_buffer_()
      {
        ...
        {
          YYSYS_LOG(WARN, "prepare buffer fail buffer_size=%ld", buffer_size_);
          ret = OB_ERROR;
        }
        return ret;
      }
      ...
      int DirectFileAppender::fsync()
      {
        ...
        else if (OB_SUCCESS == (ret = buffer_sync_(&need_truncate)))
        {
          ...
          {
            YYSYS_LOG(WARN, "ftruncate fail fd=%d file_pos=%ld errno=%u", fd_, file_pos_, errno);
            ret = OB_IO_ERROR;
          }
        }
        if (OB_SUCCESS == ret)
      ...
      int DirectFileAppender::append(const void *buf, const int64_t count, const bool is_fsync)
      {
        ...
        else
        {
          ...
          {
            align_warn_++;
            YYSYS_LOG(WARN, "file_pos_=%ld do not match align_size=%ld", file_pos_, align_size_);
          }
      ...
      int DirectFileAppender::prepare_buffer_()
      {
        ...
        if (NULL == buffer_
            && NULL == (buffer_ = (char*)::memalign(align_size_, buffer_size_)))
        {
          YYSYS_LOG(WARN, "prepare buffer fail align_size=%ld buffer_size=%ld", align_size_, buffer_size_);
          ret = OB_ERROR;
        }
        else if (0 != (file_pos_ % align_size_))
        {
          ...
          {
            YYSYS_LOG(WARN, "read buffer fail fd=%d buffer=%p align_size=%ld offset2read=%ld size2read=%ld read_ret=%ld errno=%u",
      ...
      int DirectFileAppender::set_align_size(const int64_t align_size)
      {
        ...
        {
          YYSYS_LOG(INFO, "change align size from align_size=%ld to new_align_size=%ld",
                    align_size_, align_size);
        }
        if (-1 != fd_)
        {
          YYSYS_LOG(WARN, "file is open cannot modify align_size");
          ret = OB_INIT_TWICE;
        }
        else
        {
    ...
    int64_t unintr_pwrite(const int fd, const void *buf, const int64_t count, const int64_t offset)
    {
      ...
      while (length2write > 0)
      {
        for (int64_t retry = 0; retry < 3;)
        {
          ...
          if (0 >= write_ret)
          {
            if (errno == EINTR) // ����IO����Ҫ�ж�EAGAIN
            {
              continue;
            }
            YYSYS_LOG(ERROR, "pwrite fail ret=%ld errno=%u fd=%d buf=%p size2write=%ld offset2write=%ld retry_num=%ld",
      ...
      while (length2read > 0)
      {
        for (int64_t retry = 0; retry < 3;)
        {
          ...
          if (0 > read_ret)
          {
            ...
            YYSYS_LOG(ERROR, "pread fail ret=%ld errno=%u fd=%d buf=%p size2read=%ld offset2read=%ld retry_num=%ld",
    ...
    void ObFileBuffer::set_base_pos(const int64_t pos)
    {
      if (pos > buffer_size_)
      {
        YYSYS_LOG(WARN, "base_pos=%ld will be greater than buffer_size=%ld", pos, buffer_size_);
      }
      base_pos_ = pos;
    }
    ...
    int ObFileBuffer::assign(const int64_t size, const int64_t align)
    {
      ...
      if (0 >= size
          ...
          || 0 != size % align)
      {
        YYSYS_LOG(WARN, "invalid size=%ld or donot match align=%ld", size, align);
        ret = OB_INVALID_ARGUMENT;
      }
               ...
               || 0 != (int64_t)buffer_ % align)
      {
        ...
        {
          YYSYS_LOG(WARN, "memalign fail align=%ld alloc_size=%ld size=%ld errno=%u", align, alloc_size, size, errno);
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObFileBuffer::assign(const int64_t size)
    {
      ...
      if (0 >= size)
      {
        YYSYS_LOG(WARN, "invalid size=%ld", size);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == buffer_
               || buffer_size_ < size
               || (MIN_BUFFER_SIZE < buffer_size_ && MIN_BUFFER_SIZE > size))
      {
        ...
        {
          YYSYS_LOG(WARN, "malloc fail alloc_size=%ld size=%ld errno=%u", alloc_size, size, errno);
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObFileReader::open(const ObString &fname, const bool dio, const int64_t align_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid align_size=%ld", align_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      if (NULL == file_)
      {
        YYSYS_LOG(WARN, "construct file reader fail fname=[%.*s]", fname.length(), fname.ptr());
        ret = OB_ERROR;
      }
      else
      {
                                  ...
                                  IFileBuffer &file_buf)
    {
      ...
      if (NULL == file_info)
      {
        YYSYS_LOG(WARN, "get file info fail file_id=%lu offset=%ld size=%ld", file_id, offset, size);
        ret = OB_ERROR;
      }
      else
      {
                                  ...
                                  IFileBuffer &file_buf)
    {
      ...
      if (-1 == fd)
      {
        YYSYS_LOG(WARN, "invalid fd");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (-1 == (flags = fcntl(fd, F_GETFL)))
      {
        YYSYS_LOG(WARN, "fcntl F_GETFL fail fd=%d errno=%u", fd, errno);
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (size != read_size)
        {
          YYSYS_LOG(WARN, "read_size=%ld do not equal size=%ld", read_size, size);
          ret = (OB_SUCCESS == ret) ? OB_ERROR : ret;
        }
      }
      return ret;
    ...
    int ObFileAppender::open(const ObString &fname, const bool dio, const bool is_create, const bool is_trunc, const int64_t align_size)
    {
      ...
      if (NULL == file_)
      {
        YYSYS_LOG(WARN, "construct file appender fail fname=[%.*s]", fname.length(), fname.ptr());
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "set align_size=%ld fail", align_size);
      }
      else
      {
      ...
      cur_iocb_(NULL)
    {
      ...
      {
        YYSYS_LOG(ERROR, "io_setup fail ret=%d", tmp_ret);
      }
    }
                                  ...
                                  const int64_t align_size)
    {
      ...
      if (-1 != fd_)
      {
        ret = OB_INIT_TWICE;
               ...
               || !is2n(align_size))
      {
        YYSYS_LOG(WARN, "invalid param dio=%s align_size=%ld", STR_BOOL(dio), align_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = FileComponent::open(fname, op, fd_)))
      {
        YYSYS_LOG(WARN, "open file fail, ret=%d dio=%s is_create=%s is_trunc=%s is_excl=%s fname=[%.*s]",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "file_size=%ld do not align, align_size=%ld fd=%d", file_pos_, align_size, fd_);
          close();
        }
        else
        {
    ...
    void ObFileAsyncAppender::close()
    {
      if (-1 != fd_)
      {
        ...
        {
          // Fatal error
          YYSYS_LOG(ERROR, "fsync fail fd=%d, will set fd=-1, and the fd will leek", fd_);
        }
        else
        {
                                    ...
                                    const bool is_fsync)
    {
      ...
      else
      {
        ...
        {
          // Fatal error
          YYSYS_LOG(ERROR, "iocb timeout, to protect the file, will set fd=-1, and the fd will leek");
          fd_ = -1;
          cur_iocb_ = NULL;
        }
        if (OB_SUCCESS == ret
    ...
    int ObFileAsyncAppender::submit_iocb_(AIOCB *iocb)
    {
      ...
      {
        YYSYS_LOG(WARN, "io_submit fail ret=%d", tmp_ret);
    ...
    ObFileAsyncAppender::AIOCB *ObFileAsyncAppender::get_iocb_()
    {
          ...
          && NULL == ret->buffer)
      {
        ...
        {
          YYSYS_LOG(WARN, "alloc async buffer fail");
          pool_.free_obj(ret);
          cur_iocb_ = NULL;
          ret = NULL;
        }
    ...
    void ObFileAsyncAppender::wait()
    {
      ...
      for (int64_t i = 0; i < ret_num; i++)
      {
        ...
        {
          // Fatal error
          YYSYS_LOG(ERROR, "iocb return fail iocb=%p res=%ld res2=%ld, will set fd=-1, and the fd will leek",
