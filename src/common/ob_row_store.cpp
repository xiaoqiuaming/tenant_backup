...
struct ObRowStore::BlockInfo
{
   ...
   */
    ...
    {
      YYSYS_LOG(DEBUG, "cur=%ld, pos=%ld, remain=%ld", curr_data_pos_, pos, curr_data_pos_ - pos);
...
int ObRowStore::iterator::get_next_ups_row(const ObRowkey *&rowkey, ObUpsRow &row, common::ObString *compact_row /* = NULL */)
{
  ...
  {
    YYSYS_LOG(WARN, "get next ups row fail:ret[%d]", ret);
  }
  return ret;
}
...
int ObRowStore::iterator::get_next_row(ObRowkey *rowkey, ObObj *rowkey_obj, ObRow &row, common::ObString *compact_row)
{
  ...
  if (NULL == row_store_)
  {
    YYSYS_LOG(ERROR, "row_store_ is NULL, should not reach here");
    ret = OB_ERROR;
  }
  ...
  {
    YYSYS_LOG(DEBUG, "iter end.block=%p, pos=%ld", cur_iter_block_, cur_iter_pos_);
  }
  else if (OB_SUCCESS == ret)
  {
    ...
    //YYSYS_LOG(DEBUG, "stored_row->reserved_cells_count_=%d, stored_row->compact_row_size_=%d, sizeof(ObObj)=%lu, next_pos_=%ld",
    ...
    if (OB_SUCCESS == ret && NULL != stored_row)
    {
      ...
      if (NULL != ups_row)
      {
        ...
        if(OB_UNLIKELY(NULL == row_desc))
        {
          ...
          YYSYS_LOG(WARN, "row should set row desc first");
        }
        else if(OB_UNLIKELY(OB_SUCCESS != (ret = row_desc->get_tid_cid(0, table_id, column_id))))
        {
          YYSYS_LOG(WARN, "get tid cid fail:ret[%d]", ret);
        }
        else
        {
          if (OB_SUCCESS != (ret = ObUpsRowUtil::convert(table_id, stored_row->get_compact_row(), *ups_row, rowkey, rowkey_obj)))
          {
            YYSYS_LOG(WARN, "fail to convert compact row to ObUpsRow. ret=%d", ret);
          }
        }
      }
      else
    ...
    {
      YYSYS_LOG(WARN, "fail to get next row. stored_row=%p, ret=%d", stored_row, ret);
    }
  }
...
int ObRowStore::iterator::get_next_mergejoin_stored_row(ObRow &row, const MergejoinStoredRow *&stored_row, bool &has_convert, bool need_convert)
{
  int ret = OB_SUCCESS;
  stored_row = NULL;
  ...
  {
    const char *buffer = cur_iter_block_->get_buffer_head() + cur_iter_pos_;
    stored_row = reinterpret_cast<const MergejoinStoredRow *>(buffer);
    cur_iter_pos_ += (row_store_->get_mergejoin_reserved_cells_size(0) + stored_row->compact_row_size_);

    if(NULL == stored_row)
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "can not get stored merge join row");
    }
    if(OB_SUCCESS == ret)
    {
      bool start_convert = (need_convert)?true:(stored_row->join_status_ == ObRowStore::FAIL)?true:false;
      if(start_convert)
      {
...
int ObRowStore::rollback_last_row()
{
  ...
  {
    YYSYS_LOG(WARN, "only one row could be rollback after called add_row() once");
...
int ObRowStore::append_row(const ObRowkey *rowkey, const ObRow &row, BlockInfo &block, StoredRow &stored_row)
{
  ...
  if(OB_SUCCESS == ret && NULL != rowkey)
  {
    ...
    {
      YYSYS_LOG(WARN, "append rowkey fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret && NULL != ups_row)
  {
    if( ups_row->get_is_delete_row() )
    {
      ...
      {
        YYSYS_LOG(WARN, "append row delete flag fail:ret[%d]", ret);
      }
    }
  }
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < row.get_column_num(); ++i)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      if (ObExtendType == cell->get_type())
      {
        ...
        {
          YYSYS_LOG(WARN, "get ext value fail:ret[%d]", ret);
        }
        else if (ObActionFlag::OP_VALID == ext_value || ObActionFlag::OP_NEW_ADD == ext_value )
        {
          if (OB_SUCCESS != (ret = cell_writer.append_escape(ObCellMeta::ES_VALID)))
          {
            YYSYS_LOG(WARN, "fail to append escape:ret[%d]", ret);
          }
        }
        ...
        {
          ret = OB_NOT_SUPPORTED;
          YYSYS_LOG(WARN, "not supported ext value:ext[%ld]", ext_value);
        }
        else if(NULL == ups_row)
        {
          ret = OB_NOT_SUPPORTED;
          YYSYS_LOG(WARN, "OP_NOP can only used in ups row");
        }
        else //OP_NOP不需要序列化
        {
          cell_clone = *cell;
      ...
      else if (OB_SUCCESS != (ret = cell_writer.append(column_id, *cell, &cell_clone)))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to append cell, err=%d", ret);
        }
        break;
      }
    }
...
int ObRowStore::add_row(const ObRowkey *rowkey, const ObRow &row, const StoredRow *&stored_row, int64_t &cur_size_counter)
{
  ...
  {
    ret = OB_NOT_SUPPORTED;
    YYSYS_LOG(WARN, "read only ObRowStore, not allow add row, ret=%d", ret);
  }
  else
  {
    stored_row = NULL;
    ...
    while(OB_SUCCESS == ret && retry < 3)
    {
      ...
      if (OB_SUCCESS != (ret = append_row(rowkey, row, *block_list_head_, *reserved_cells)))
      {
        if (OB_BUF_NOT_ENOUGH == ret)
        {
          // buffer not enough
          block_list_head_->advance( -get_reserved_cells_size(reserved_columns_count) );
          YYSYS_LOG(DEBUG, "block buffer not enough, buff=%p remain_size=%ld block_count=%ld block_size=%ld",
          ...
          if (block_list_head_->is_fresh_block())
          {
            ...
            else
            {
              ...
              if (NULL == before_last)
              {
                YYSYS_LOG(ERROR, "This branch should not be reached, before_last = NULL");
                ret = OB_ERR_UNEXPECTED;
              }
              else
              {
        ...
        {
          YYSYS_LOG(WARN, "failed to append row, err=%d", ret);
        }
      }
      else
      {
...
{
  YYSYS_LOG(DEBUG, "ObRowStore::begin block_list_tail_ date_ = %p  curr_data_pos_ = %ld",
...
DEFINE_SERIALIZE(ObRowStore)
{
  ...
  while(NULL != block)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to serialize block size. ret=%d", ret);
      break;
    }
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "buffer not enough.");
    ...
    YYSYS_LOG(DEBUG, "serialize next block");
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != (ret = obj.serialize(buf, buf_len, pos)))
    {
      YYSYS_LOG(WARN, "fail to serialize end flag:ret[%d]", ret);
    }
  }
  return ret;
}
...
DEFINE_DESERIALIZE(ObRowStore)
{
        ...
        && (ObExtendType != obj.get_type()))
  {
    ...
    {
      YYSYS_LOG(WARN, "ObObj deserialize type error, ret=%d buf=%p data_len=%ld pos=%ld",
    ...
    {
      YYSYS_LOG(WARN, "fail to allocate new block. ret=%d", ret);
      break;
    }
    if (NULL != block_list_head_ && block_size <= block_list_head_->get_remain_size() && block_size  >= 0)
    {
      YYSYS_LOG(DEBUG, "copy data from scanner to row_store. buf=%p, pos=%ld, block_size=%ld",
                buf, pos, block_size);
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize scanner data into new block. block_list_head_=%p, block_size=%ld",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "obj should be OP_END_FLAG:obj[%s]", to_cstring(obj));
    }
  }
...
int ObRowStore::add_row(const ObRowkey &rowkey, const ObRow &row, const StoredRow *&stored_row)
{
  ...
  {
    YYSYS_LOG(WARN, "add row fail:ret[%d]", ret);
  }
  return ret;
}
...
int ObRowStore::add_ups_row(const ObRowkey &rowkey, const ObUpsRow &row, const StoredRow *&stored_row)
{
  ...
  {
    YYSYS_LOG(WARN, "add ups row fail:ret[%d]", ret);
  }
  return ret;
}
...
int ObRowStore::append_normal_row(const ObRowkey *rowkey, const ObRow &row, BlockInfo &block, StoredRow &stored_row)
{
  int ret = OB_SUCCESS;

  cell_writer_.reset();
  cell_writer_.init(block.get_buffer(), block.get_remain_size(),(NULL == rowkey)? SPARSE:DENSE_SPARSE);

  if(NULL != rowkey)
  {
    if(OB_SUCCESS != (ret = cell_writer_.append_rowkey(*rowkey)))
    {
      YYSYS_LOG(WARN, "append rowkey fail:ret[%d]", ret);
    }
  }
  ...
  for(int64_t i = 0; OB_SUCCESS == ret && i < row.get_column_num(); ++i)
  {
    ret = row.raw_get_cell(i, cell, table_id, column_id);
    if(OB_UNLIKELY(OB_SUCCESS != ret))
    {
      YYSYS_LOG(WARN, "failed to get cell, err=%d", ret);
      break;
    }
    else if(ObExtendType == cell->get_type())
    {
      if(OB_SUCCESS != (ret = cell->get_ext(ext_value)))
      {
        YYSYS_LOG(WARN, "get ext value fail:ret[%d]", ret);
      ...
      {
        if(OB_SUCCESS != (ret = cell_writer_.append_escape(ObCellMeta::ES_VALID)))
        {
          YYSYS_LOG(WARN, "fail to append escape:ret[%d]", ret);
        }
      }
      ...
      {
        if(OB_SUCCESS != (ret = cell_writer_.append_escape(ObCellMeta::ES_NOT_EXIST_ROW)))
        {
          YYSYS_LOG(WARN, "fail to append escape:ret[%d]", ret);
        }
      }
    ...
    else if(OB_SUCCESS != (ret = cell_writer_.append(column_id, *cell)))
    {
      if(OB_BUF_NOT_ENOUGH != ret)
      {
        YYSYS_LOG(WARN, "failed to append cell, err=%d", ret);
  ...
  if(OB_SUCCESS == ret)
  {
    if(OB_SUCCESS != (ret = cell_writer_.row_finish()))
    {
      if(OB_BUF_NOT_ENOUGH != ret)
      {
        YYSYS_LOG(WARN, "failed to append cell, err=%d", ret);
    ...
    {
      stored_row.compact_row_size_ = static_cast<int32_t>(cell_writer_.size());
      block.advance(cell_writer_.size());
      cur_size_counter_ += cell_writer_.size();
    }
  }
  cell_writer_.reset();
  return ret;
...
int ObRowStore::append_row_opt(const ObRowkey *rowkey, const ObRow &row, BlockInfo &block, StoredRow &stored_row)
{
  int ret = OB_SUCCESS;
  ObCompactCellWriter cell_writer;
  if(OB_LIKELY(NULL == rowkey))
  {
    cell_writer.init(block.get_buffer(), block.get_remain_size(), SPARSE);
  }
  else
  ...
  if(OB_SUCCESS == ret && OB_LIKELY(NULL != rowkey))
  {
    if(OB_SUCCESS != (ret = cell_writer.append_rowkey(*rowkey)))
    {
      YYSYS_LOG(WARN, "append rowkey fail:ret[%d]", ret);
  ...
  if(OB_SUCCESS == ret && NULL != ups_row)
  {
    if(ups_row->get_is_delete_row())
    {
      if(false == ups_row->get_is_all_nop())
      {
        YYSYS_LOG(WARN, "this is a deleted row ,but is not all nop row ,"\
                  "should not append into cell_writer as value->extend:8."\
                  "check if it was excuted after truncate");
      }
      if(OB_SUCCESS != (ret = cell_writer.row_delete()))
      {
        YYSYS_LOG(WARN, "append row delete flag fail:ret[%d]", ret);
  ...
  for(int64_t i = 0; OB_SUCCESS == ret && i < row.get_column_num(); ++i)
  {
    const bool& is_sort_col = sort_col_info_[i].is_sort_col_;
      ...
      if(ObExtendType == cell->get_type())
      {
        ...
        else if(is_sort_col)
        {
          cell_clone = *cell;
                ...
                ret = cell_writer.append_opt(sort_col_info_[i].column_id_, *cell, is_sort_col, &cell_clone)))
      {
        if(OB_BUF_NOT_ENOUGH != ret)
        {
          YYSYS_LOG(WARN, "failed to append cell, err=%d", ret);
    ...
    if(OB_SUCCESS == ret)
    {
      if(is_sort_col)
      {
        stored_row.reserved_cells_[sort_col_info_[i].index_in_sort_] = cell_clone;
      }
    }
  }//end for i
  if(OB_SUCCESS == ret)
...
int ObRowStore::add_row_opt(const ObRow &row, const StoredRow *&stored_row)
{
  int64_t cur_size_counter = 0;
  return add_row_opt(NULL, row, stored_row, cur_size_counter);
}

int ObRowStore::append_mergejoin_row(const ObRowkey *rowkey, const ObRow &row, ObRowStore::BlockInfo &block, ObRowStore::MergejoinStoredRow &stored_row)
{
  int ret = OB_SUCCESS;
...
int ObRowStore::add_mergejoin_row(const ObRow &row)
{
  const MergejoinStoredRow* stored_row = NULL;
  int64_t cur_size_counter = 0;
  return add_mergejoin_row(NULL, row, stored_row, cur_size_counter);
}

int ObRowStore::add_mergejoin_row(const ObRow &row, const MergejoinStoredRow *&stored_row)
{
  int64_t cur_size_counter = 0;
  return add_mergejoin_row(NULL, row, stored_row, cur_size_counter);
}

int ObRowStore::get_next_mergejoin_stored_row(ObRow &row, const MergejoinStoredRow *&stored_row
                                              , bool need_convert)
{
  bool has_convert = false;
  return inner_.get_next_mergejoin_stored_row(row, stored_row,has_convert,need_convert);
}
int ObRowStore::get_next_mergejoin_stored_row(ObRow &row, const ObRowStore::MergejoinStoredRow *&stored_row
                                              , bool &has_convert, bool need_convert)
{
  return inner_.get_next_mergejoin_stored_row(row, stored_row,has_convert,need_convert);
}

int ObRowStore::add_mergejoin_row(const ObRowkey *rowkey, const ObRow &row,
                                  const MergejoinStoredRow *&stored_row, int64_t &cur_size_counter)
{
  int ret = OB_SUCCESS;
  if(is_read_only_)
  {
  ...
  {
    stored_row = NULL;
    const int32_t reserved_columns_count = 0;

    if(NULL == block_list_head_
       || block_list_head_->get_remain_size() <= get_compact_row_min_size(row.get_column_num())
       + get_mergejoin_reserved_cells_size(reserved_columns_count))
    {
      ret = new_block();
    }
    int64_t retry = 0;
    while(OB_SUCCESS == ret && retry < 3)
    {
      ...
      OB_ASSERT(block_list_head_);
      MergejoinStoredRow *reserved_cells = reinterpret_cast<MergejoinStoredRow *>(block_list_head_->get_buffer());
      reserved_cells->join_status_ = static_cast<int32_t>(ObRowStore::FAIL);
      block_list_head_->advance(get_mergejoin_reserved_cells_size(reserved_columns_count));

      if(OB_SUCCESS != (ret = append_mergejoin_row(rowkey, row, *block_list_head_, *reserved_cells)))
      {
        if(OB_BUF_NOT_ENOUGH == ret)
        {
          //buff not enough
      ...
      {
        cur_size_counter_ += get_mergejoin_reserved_cells_size(reserved_columns_count);
        stored_row = reserved_cells;
        cur_size_counter = cur_size_counter_;
        break;
      }
...
int ObRowStore::add_normal_row(const ObRow &row, int64_t &cur_size_counter)
{
  const StoredRow *stored_row = NULL;
  return add_normal_row(NULL, row, stored_row, cur_size_counter);
}

int ObRowStore::add_normal_row(const ObRowkey &rowkey, const ObRow &row, int64_t &cur_size_counter)
{
  const StoredRow *stored_row = NULL;
  return add_normal_row(&rowkey, row, stored_row, cur_size_counter);
}


int ObRowStore::init_is_sort_col_array(const int32_t col_num)
{
  int ret = OB_SUCCESS;
  if(OB_UNLIKELY(NULL != sort_col_info_))
  {
    ret = OB_INIT_TWICE;
    YYSYS_LOG(ERROR, "has been inited,ret[%d]",ret);
  }
  else if(OB_UNLIKELY(col_num < 0 || col_num > OB_MAX_USER_DEFINED_COLUMNS_COUNT))
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "col_num is not invalied,col_num[%d],ret[%d]",col_num,ret);
  }
  else
  {
    YYSYS_LOG(DEBUG, "col_num[%d]",col_num);
    sort_col_info_ = new(std::nothrow)ObSortColIndex[col_num];
    has_inited_ = true;
    col_num_ = col_num;
  }
  return ret;
...
int ObRowStore::set_is_sort_col_array(const int32_t index, const int32_t index_in_sort)
{
  int ret = OB_SUCCESS;
  if(OB_UNLIKELY(NULL == sort_col_info_ || col_num_ < 0))
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "is_sort_col_ is NULL,ret[%d]", ret);
  }
  else if(OB_UNLIKELY(index < 0 || index > col_num_))
  {
    ret = OB_INVALID_INDEX;
    YYSYS_LOG(ERROR, "index is invalid,index[%d],col size[%d],ret[%d]",index,col_num_,ret);
  }
  else if(sort_col_info_[index].is_sort_col_)
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "is_sort_col[%d] has been setted true", index);
  }
  else
  {
    sort_col_info_[index].is_sort_col_ = true;
    sort_col_info_[index].index_in_sort_ = index_in_sort;
  }
  return ret;
}

int ObRowStore::set_column_id(const int32_t index, const uint64_t column_id)
{
  int ret = OB_SUCCESS;
  if(OB_UNLIKELY(NULL == sort_col_info_ || col_num_ < 0))
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "is_sort_col_ is NULL ,col_num[%d],ret[%d]",col_num_,ret);
  }
  else if(OB_UNLIKELY(index < 0 || index > col_num_))
  {
    ret = OB_INVALID_INDEX;
    YYSYS_LOG(ERROR, "index is invalid,index[%d],size[%d],ret[%d]",index,col_num_,ret);
  }
  else if(sort_col_info_[index].column_id_ != OB_INVALID_ID)
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "column_id_[%d] has been setted,ret[%d]", index, ret);
  }
  else
  {
    sort_col_info_[index].column_id_ = column_id;
  }
  return ret;
}

void ObRowStore::print_is_sort_col_array()
{
  for(int32_t i = 0; i < col_num_; i++)
  {
    YYSYS_LOG(DEBUG, "print_after_init.index[%d],is_sort_col[%d],cid[%lu],sort_index[%d]",
              i,sort_col_info_[i].is_sort_col_,sort_col_info_[i].column_id_,sort_col_info_[i].index_in_sort_);
  }
}

int ObRowStore::add_normal_row(const ObRowkey *rowkey, const ObRow &row, const StoredRow *&stored_row, int64_t &cur_size_counter)
{
  int ret = OB_SUCCESS;
  if(is_read_only_)
  {
  ...
  else
  {
    ...
    while(OB_SUCCESS == ret && retry < 3)
    {
      ...
      if(OB_SUCCESS != (ret = append_normal_row(rowkey, row, *block_list_head_, *reserved_cells)))
      {
        if(OB_BUF_NOT_ENOUGH == ret)
        {
          //buff not enough
...
int ObRowStore::add_row_opt(const ObRowkey *rowkey, const ObRow &row, const StoredRow *&stored_row, int64_t &cur_size_counter)
{
  int ret = OB_SUCCESS;
  if(is_read_only_)
  {
  ...
  else
  {
    ...
    while(OB_SUCCESS == ret && retry < 3)
    {
      ...
      if(OB_SUCCESS != (ret = append_row_opt(rowkey, row, *block_list_head_, *reserved_cells)))
      {
        if(OB_BUF_NOT_ENOUGH == ret)
        {
          //buff not enough
