...
namespace oceanbase
{
  namespace common
  {
    ...
    public:
      ...
      /// @fn get key
      /// @warning ����ObStringû��֧�������������ﷵ��const��������Ч�ģ�������Ϊ��ʾ
    ...
    public:
      ...
      ///   ��ʼ�� 
      /// 
      /// @param cache_mem_size         cache������ʹ�õ��ڴ�byte
      /// @param max_no_active_usec 
      ///   -# >=0 ����lru��̭
      ///   -# <0 ������̭����ʱcache_mem_size�޺��壬���ǿ�����ʾ�ڴ��ʹ��
      /// @param hash_slot_num  hash������
      /// 
      ...
      /// @fn ��cache������һ����øú�����cache_pair�е�ָ��Ϊ��;
      ///       ObCachePair�е��ڴ�ͨ��malloc��ȡ
      virtual int add(ObCachePair & cache_pair, ObCachePair &old_pair) = 0;

      /// @fn ��ѯcache��
      /// @warning cache_pair����ָ��cache�ڲ���ָ�룬�˴�Ӧ��get��const��cache_pair��
      ///          ���ǣ���ʱû���뵽�õ�ʵ�ַ��������ʹ���߲��ܹ��޸ĵõ���cache_pair
      virtual int get(const common::ObNewRange &key, ObCachePair &cache_pair) = 0;

      /// @fn �黹cache��Ŀ��cache_pair������ͨ��malloc�õ���Ҳ������ͨ��get�õ���
      ///       ǰһ����������ڻ�ȡ�ڴ�����cache_pair�����ݵĹ�����ʧ�ܣ�ʹ��������
      ///       ��ʾ���ã�ObCachePair�������������Զ�����
    ...
    public:
      ...
      /// @fn initialize, hash_slot_num hashͰ������
      int init(int32_t hash_slot_num)
      {
        int err = OB_SUCCESS;
        if (hash_slot_num <= 0)
        {
          YYSYS_LOG(WARN, "hash slot number must be greater than 0 [hash_slot_num:%d]", 
                    hash_slot_num );
          err = oceanbase::common::OB_INVALID_ARGUMENT;
        }
        if (hash_slot_array_ != NULL)
        {
          YYSYS_LOG(WARN, "hash table has been already initialized [hash_slot_array:%p]", 
                    hash_slot_array_ );
      ...
      int add(CacheItemHead & item, CacheItemHead *&old_item)
      {
        ...
        if (hash_slot_array_ == NULL)
        {
          ...
          YYSYS_LOG(WARN, "hashtable not initialized");
        }
        if (OB_SUCCESS == err )
        {
          ...
          if (&item == old_item)
          {
            ...
            YYSYS_LOG(WARN, "add an exist item [item:%p]",old_item);
            old_item = NULL;
          }
    ...
    public:
      ...
      ~ObVarCache()
      {
        if (total_ref_num_ != 0)
        {
          YYSYS_LOG(ERROR, "some cache pairs not reverted");
        }
        clear();
        total_ref_num_ = 0;
        available_mem_size_ = 0;
      ...
      inline int set_max_mem_size(const int64_t max_mem_size)
      {
        ...
        {
          err = oceanbase::common::OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "check max memory size failed:size[%ld]", max_mem_size);
        }
        else
        {
                       ...
                       const int32_t hash_slot_num)
      {
        ...
        {
          err = oceanbase::common::OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "param error [cache_mem_size:%ld,max_no_active_usec:%ld,"
                    "hash_slot_num:%d]", 
        ...
        {
          YYSYS_LOG(WARN, "cache has already been initialized");
          err = oceanbase::common::OB_INIT_TWICE;
        }
        if (OB_SUCCESS == err)
        {
                         ...
                         const int32_t value_size)
      {
        ...
        {
          YYSYS_LOG(WARN, "param error [key_size:%d,value_size:%d]", key_size, value_size);
          err = oceanbase::common::OB_INVALID_ARGUMENT;
        }
        /// @warning ����������revert������ᷢ������
        cache_pair.revert();
        ...
        if (OB_SUCCESS == err && !inited_)
        {
          YYSYS_LOG(WARN, "cache not initialized yet");
          err = oceanbase::common::OB_NOT_INIT;
        }
        if (OB_SUCCESS == err)
        {
      ...
      virtual int add(ObCachePair &cache_pair, ObCachePair &old_pair)
      {
        ...
        {
          err = oceanbase::common::OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "ObCachePair is empty [key:%p]", key.ptr());
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "cache memory overflow [item_ptr:%p]", item);
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (item->get_mother_block()->get_item_num() == 0)
          {
            ...
            else
            {
              ...
              /// @warning ���ܹ��޸�cache_pair�����ݣ���Ϊ�����߿�������ʹ��cache_pair��ָ��
              /* 
              cache_pair.revert();
      ...
      virtual int get(const common::ObNewRange &key, ObCachePair &cache_pair)
      {
        ...
        {
          YYSYS_LOG(WARN, "argument error [key.ptr:%p,key.length:%d]",key.ptr(), key.length());
          err = oceanbase::common::OB_INVALID_ARGUMENT;
        }
        */
        /// @warning ����������revert������ᷢ������
      ...
      ///     else
      ///     {
      ///       /// @warning ����ط�item�����ü������ܲ�Ϊ0������û�й�ϵ�����ﲢû��
      ///       ///          ���ڴ���ɾ��
      ///       /// item->hash_list_link_.remove();
      ...
      virtual int clear()
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "there exist item not reverted, clear fail, please revert all items first");
    ...
    private:
      ...
      /// @property ÿ��lru��̭��ʱ����������ô��������item����ֹÿ�α�������lru����
