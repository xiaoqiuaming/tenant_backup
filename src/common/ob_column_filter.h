  ...
  namespace common
  {
    class ColumnFilter
    {
      public:
        ...
        ,data_mark_param_()
        //add duyr 20160531:e
        //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151203:b    // [uncertainty ]
        ,read_atomic_param_()
        //add duyr 20151203:e
        {
        };
      ...
      public:
          ...
          if (NULL != column_filter)
          {
            ...
            else
            {
              ...
              for (int64_t i = 0; i < column_size; ++i)
              {
                ...
                {
                  YYSYS_LOG(WARN, "failed to add the %ld-th column, cf_id=%lu", i, cf_ids[i]);
                  ret = NULL;
                  break;
                }
              }
              ...
              //add duyr 20160531:e  //[conflict_level B]
              //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151223:b     // [uncertainty ]
              column_filter->set_read_atomic_param(scan_param.get_read_atomic_param());
              //add duyr 20151223:e
            }
          }
          return ret;
        ...
        //add duyr [MultiUPS] [READ_ATOMIC] [read_part] 20151203:b
        void set_read_atomic_param(const ObReadAtomicParam& param)
        {
          read_atomic_param_     = param;
        }
        const ObReadAtomicParam& get_read_atomic_param() const
        {
