    ...
    class ObResourcePool
    {
      ...
      public:
          ...
          allocated_num_(0)
        {
          ...
          int ret = common::PTHREAD_KEY_MGR->create_thread_key(thread_key_);
          //mod hongchen [TSD_USER_IMPL] 20170916:e
          if (0 != ret)
          {
            YYSYS_LOG(ERROR, "pthread_key_create fail ret=%d", ret);
          }
          array_list_.init(MAX_THREAD_NUM);
          free_list_.init(TOTAL_NUM);
          //YYSYS_LOG(INFO, "RP init type=%s local_num=%ld total_num=%ld", typeid(typeof(T)).name(), LOCAL_NUM, TOTAL_NUM);
        };
        ~ObResourcePool()
        {
          Node *node = NULL;
          ...
          common::PTHREAD_KEY_MGR->delete_thread_key(thread_key_);
          //mod hongchen [TSD_USER_IMPL] 20170916:e
        };
      public:
        T *get(Guard &guard)
        ...
        void free(T *ptr)
        {
          if (NULL != ptr)
          {
            ...
            {
              YYSYS_LOG(ERROR, "node=%p magic=%lx not match %lx", node, node->magic, ALLOC_MAGIC_NUM);
            }
            else
            {
              free_node_(node);
      ...
      private:
        ...
        void free_node_(Node *ptr)
        {
          if (NULL != ptr)
          {
            ...
            if (ALLOC_BY_PAGEARENA == ptr->flag)
            {
              ...
              {
                YYSYS_LOG(ERROR, "free node to list fail, size=%ld ptr=%p", free_list_.get_total(), ptr);
              }
            }
            ...
            {
              YYSYS_LOG(ERROR, "invalid flag=%lu", ptr->flag);
            }
            //int64_t cnt = 0;
            ...
            //  }
            //  if (TOTAL_NUM < cnt++
            //      && yysys::CTimeUtil::getTime() > (WARN_INTERVAL + last_warn_time))
            //  {
            //    last_warn_time = yysys::CTimeUtil::getTime();
            //    YYSYS_LOG(ERROR, "free node to list fail count to large %ld, free_list_size=%ld", cnt, free_list_.get_free());
            //  }
            //}
          }
        };
        ...
        NodeArray *get_node_array_()
        {
          ...
          NodeArray *ret = (NodeArray*)(common::PTHREAD_KEY_MGR->get_thread_specifi(thread_key_));
          if (NULL == ret)
          {
            if (NULL != (ret = alloc_array_()))
            {
              ...
              else
              {
                ...
                common::PTHREAD_KEY_MGR->set_thread_specifi(thread_key_, ret);
                YYSYS_LOG(INFO, "alloc thread specific node_array=%p", ret);
              }
            }
          }
          //mod hongchen [TSD_USER_IMPL] 20170916:e
