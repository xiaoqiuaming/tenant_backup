  ...
  namespace common
  {
    ...
    int ObSeqQueue::init(const int64_t limit, SeqItem* buf)
    {
      ...
      if (0 >= limit)
      {
        ...
        YYSYS_LOG(ERROR, "init(limit=%ld, buf=%p): INVALID_ARGUMENT", limit, buf);
      }
      ...
      else if (NULL == (items_ = (buf?: (SeqItem*)buf_holder_.get(sizeof(SeqItem) * limit))))
      {
        ...
        YYSYS_LOG(ERROR, "buf_holder.get(%ld)=>NULL", sizeof(SeqItem) * limit);
      }
      else
      {
        limit_ = limit;
    ...
    int ObSeqQueue::start(const int64_t seq)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "start(seq=%ld): INVALID_ARGUMENT", seq);
      }
      else if (NULL == items_)
      {
        err = OB_NOT_INIT;
    ...
    yysys::CThreadCond* ObSeqQueue::get_cond(const int64_t seq)
    {
    ...
    int ObSeqQueue::add(const int64_t seq, void* data)
    {
      ...
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "seq_[%ld] <= 0", seq_);
      }
      else if (seq < seq_)
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "add(seq[%ld] < seq_[%ld]): INVALID_ARGUMEN", seq, seq_);
      }
      ...
      {
        err = OB_ENTRY_EXIST;
        YYSYS_LOG(ERROR, "add(seq=%ld): ENTRY_EXIST", seq);
      }
      ...
      {
        yysys::CThreadCond* cond = get_cond(seq);
        cond->lock();
        pitem->data_ = data;
        __sync_synchronize();
        pitem->seq_ = seq;
    ...
    int ObSeqQueue::get(int64_t& seq, void*& data, const int64_t timeout_us)
    {
      int err = OB_EAGAIN;
      SeqItem* pitem = NULL;
      int64_t end_time_us = yysys::CTimeUtil::getTime() + timeout_us;
      int64_t wait_time_us = timeout_us;
      int wait_time_ms = (int)(wait_time_us/1000LL);
      yysys::CThreadCond* cond = NULL;
      seq = seq_;
      cond = get_cond(seq);
      if (!is_inited())
      {
    ...
    int ObSeqQueue::update(const int64_t seq)
    {
      ...
      if (seq < seq_)
      {
        ...
        YYSYS_LOG(ERROR, "seq[%ld] < seq_[%ld]", seq, seq_);
      }
      else
      {
        yysys::CThreadCond* cond = get_cond(seq_);
        cond->lock();
        seq_ = seq;
        cond->signal();
        cond->unlock();
