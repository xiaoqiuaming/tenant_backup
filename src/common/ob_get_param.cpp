  ...
  namespace common
  {
    ...
    ObGetParam::ObGetParam(const bool deep_copy_args)
      : deep_copy_args_(deep_copy_args), single_cell_(false),
        ...
        is_table_truncated_(false)    //add for [322-truncate sstable get]
    ...
    int ObGetParam::init()
    {
      ...
      if (NULL == cell_list_)
      {
        ...
        if (NULL == cell_list_)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for cell buffer of get param");
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret && NULL == row_index_ && cell_capacity_ > 0)
      {
        ...
        if (NULL == row_index_)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for row index of get param");
          ret = OB_ERROR;
        }
      }
    ...
    int ObGetParam::expand()
    {
      ...
      {
        YYSYS_LOG(WARN, "expand rowindex storage error, cell_size_=%ld, "
      ...
      {
        YYSYS_LOG(WARN, "expand cell storage error, cell_size_=%ld, "
                  "cell_capacity_=%ld, max=%ld", cell_size_, cell_capacity_, MAX_CELL_CAPACITY);
      }
      return ret;
    }
    ...
    {
      YYSYS_LOG(INFO, "ObGetParam use memory:%s, allocator:pages:%ld,used:%ld,total:%ld,",
    ...
    int ObGetParam::check_name_and_id(const ObCellInfo& cell_info, bool is_first)
    {
      ...
      if (is_first)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid table name and table id "
      ...
      else
      {
        ...
        if (id_only_ && (cell_info.column_id_ == OB_INVALID_ID
                         ...
                         || NULL != cell_info.column_name_.ptr()))
        {
          YYSYS_LOG(WARN, "not specify table id or cloumn id or specify table "
                                   ...
                                   && cell_info.table_id_ != OB_INVALID_ID)))
        {
           ...
           * illegal
           */
          YYSYS_LOG(WARN, "not specify table name or cloumn name, "
    ...
    int ObGetParam::copy(const ObGetParam& other)
    {
      ...
      for(int64_t i = 0; OB_SUCCESS == err && i < other.get_cell_size(); i++)
      {
        if (NULL == (cell = other[i]))
        {
          ...
          YYSYS_LOG(ERROR, "other[%ld] = NULL", i);
        }
        else if (OB_SUCCESS != (err = add_cell(*cell)))
        {
          YYSYS_LOG(ERROR, "add_cell()=>%d", err);
        }
      }
      return err;
    }
    //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150806:b
    int ObGetParam::add_row(const uint64_t table_id,const ObRowkey *row_key,const ObArray<uint64_t> & basic_columns)
    {
      int ret=OB_SUCCESS;
      ...
      for(int64_t col_id_idx = 0; OB_SUCCESS == ret && col_id_idx < basic_columns.count(); col_id_idx ++)
      {
        ...
        if(OB_SUCCESS != (ret = add_cell(cell_info)))
        {
    ...
    int ObGetParam::add_cell(const ObCellInfo& cell_info)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid row key of cell info, key_len=%ld, key_ptr=%p",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "get param is full, can't add cell anymore, "
    ...
    int ObGetParam::copy_cell_info(const ObCellInfo & cell_info, ObCellInfo & stored_cell)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to copy column name to local buffer [err:%d]", err);
      }
      else if (OB_SUCCESS != (err = cell_info.row_key_.deep_copy(stored_cell.row_key_, allocator_)))
      {
        YYSYS_LOG(WARN,"fail to copy rowkey to local buffer [err:%d]", err);
      }
      else if (OB_SUCCESS != (err = deep_copy_ob_obj(allocator_, cell_info.value_, stored_cell.value_)))
      {
        YYSYS_LOG(WARN,"fail to copy value to local buffer [err:%d]", err);
      }
      return err;
    }
    ...
    int ObGetParam::rollback()
    {
      ...
      {
        YYSYS_LOG(WARN, "there is no cells or only one row in get param, "
    ...
    int ObGetParam::rollback(const int64_t count)
    {
      ...
      else
      {
        ...
        if (index == -1)
        {
          YYSYS_LOG(WARN, "specify wrong rollback count, the count isn't in"
        ...
        {
          YYSYS_LOG(WARN, "after rollback there is no row"
                                          ...
                                          int64_t& pos) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        //        if (OB_SUCCESS == ret)
        //        {
        //            //YYSYS_LOG(INFO,"ObGetParam::serialize transaction id:%s",to_cstring(trans_id_));
                                            ...
                                            const int64_t data_len, int64_t& pos)
    {
      ...
      else
      {
        ...
        //              //add end
        //          }
        //         // YYSYS_LOG(INFO,"ObGetParam:deserialize transaction id:%s",to_cstring(temp));
        //        }
        //        //add:e
      }
      return ret;
                                         ...
                                         const uint64_t id, bool is_field)  const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "name and id is invalid, id=%lu name length=%d",
                    id, name.length());
          ret = OB_ERROR;
        }
      }
                                           ...
                                           uint64_t& id)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid object type for deserialize name or id,"
                                       ...
                                       int64_t& pos, ObRowkey& row_key)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "get rowkey obj array fail:ret[%d]", ret);
          }
          else
          {
                                              ...
                                              int64_t& pos, const ObRowkeyInfo& rowkey_info, ObRowkey& row_key)
    {
      ...
      {
        YYSYS_LOG(WARN, "allocate memory for rowkey's object array failed, size=%ld", size);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS != (ret = get_rowkey_compatible(buf, data_len, pos,
                                                          rowkey_info, array, size, is_binary_rowkey)))
      {
        YYSYS_LOG(WARN, "get binary rowkey and translate to ObRowkey failed=%d", ret);
      }
      else if (!is_binary_rowkey)
      {
        YYSYS_LOG(WARN, "unexpect rowkey format, not binary rowkey?");
    ...
    int64_t ObGetParam::get_serialize_rowkey_size(const ObRowkey& row_key) const
    {
      ...
      {
        YYSYS_LOG(WARN, "row key and id is invalid, row key length=%ld",
                  row_key.length());
      }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (ObExtendType == type)
          {
            ...
            switch (ext_val)
            {
              ...
              default:
                ...
                YYSYS_LOG(WARN, "unkonwn extend field, extend=%ld", ext_val);
                break;
            }
          }
          else if (obj.is_valid_type())
          {
                                ...
                                ))
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid object type for deserialize column "
            ...
            {
              ret = OB_ERROR;//add liumz, [set ret value, otherwise coredump may happens]20161219
              YYSYS_LOG(INFO, "unkonwn object, type=%d", type);
            }
          }
          else
          {
            YYSYS_LOG(WARN, "wrong object type=%d", type);
            ret = OB_ERROR;
          }
        }
      }
