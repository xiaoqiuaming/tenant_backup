                      ...
                      int64_t log_sync_type, MinAvailFileIdGetter* fufid_getter, const ObServer* server)
{
  ...
  if (is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObLogWriter has been initialized");
    ret = OB_INIT_TWICE;
  }
  else if (NULL == log_dir || NULL == slave_mgr)
  {
    YYSYS_LOG(ERROR, "Parameter are invalid[log_dir=%p slave_mgr=%p]", log_dir, slave_mgr);
    ret = OB_INVALID_ARGUMENT;
  }
  else if (OB_SUCCESS != (ret = log_generator_.init(LOG_BUFFER_SIZE, log_file_max_size, server)))
  {
    YYSYS_LOG(ERROR, "log_generator.init(buf_size=%ld, file_limit=%ld)=>%d", LOG_BUFFER_SIZE, log_file_max_size, ret);
  }
  else if (OB_SUCCESS != (ret = log_writer_.init(log_dir, log_file_max_size, du_percent, log_sync_type, fufid_getter)))
  {
    YYSYS_LOG(ERROR, "log_writer.init(log_dir=%s, file_limit=%ld, du_percent=%ld, sync_type=%ld, fufid_getter=%p)=>%d",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(INFO, "ObLogWriter initialize successfully[log_dir_=%s log_file_max_size_=%ld"
...
int ObLogWriter::reset_log()
{
  ...
  {
    YYSYS_LOG(ERROR, "log_writer.reset()=>%d", err);
  }
  else if (OB_SUCCESS != (err = log_generator_.reset()))
  {
    YYSYS_LOG(ERROR, "log_generator.reset()=>%d", err);
  }
  return err;
}
...
int ObLogWriter::start_log(const ObLogCursor& log_cursor)
{
  ...
  {
    YYSYS_LOG(ERROR, "log_writer.start_log(%s)=>%d", to_cstring(log_cursor), err);
  }
  else if (OB_SUCCESS != (err = log_generator_.start_log(log_cursor)))
  {
    YYSYS_LOG(ERROR, "log_generator.start_log(%s)=>%d", to_cstring(log_cursor), err);
  }
  return err;
}
...
int ObLogWriter::start_log_maybe(const ObLogCursor& start_cursor)
{
  ...
  if (log_generator_.is_log_start())
  {
    ...
    {
      YYSYS_LOG(ERROR, "log_generator.get_end_cursor()=>%d", err);
    }
    else if (!log_cursor.equal(start_cursor))
    {
      err = OB_DISCONTINUOUS_LOG;
      YYSYS_LOG(ERROR, "log_cursor.equal(start_cursor=%s)=>%d", to_cstring(start_cursor), err);
    }
  }
  else if (OB_SUCCESS != (err = start_log(start_cursor)))
  {
    YYSYS_LOG(ERROR, "start_log(%s)=>%d", to_cstring(start_cursor), err);
  }
  return err;
}
...
int ObLogWriter::write_log(const LogCommand cmd, const char* log_data, const int64_t data_len)
{
  ...
  {
    YYSYS_LOG(WARN, "log_generator.write_log(cmd=%d, buf=%p[%ld])=>%d", cmd, log_data, data_len, ret);
  }
  return ret;
}
...
int ObLogWriter::write_keep_alive_log()
{
  ...
  {
    YYSYS_LOG(ERROR, "write_keep_alive_log()=>%d", err);
  }
  return err;
}
...
int ObLogWriter::async_flush_log(int64_t& end_log_id, TraceLog::LogBuffer &tlog_buffer)
{
  ...
  {
    YYSYS_LOG(ERROR, "log_generator.get_log()=>%d", ret);
  }
  else if (len <= 0)
  {}
  else if (0 != (len & ObLogGenerator::LOG_FILE_ALIGN_MASK))
  {
    ...
    YYSYS_LOG(ERROR, "len=%ld cursor=[%s,%s], not align", len, to_cstring(start_cursor), to_cstring(end_cursor));
    while(1);
  }
  else
  {
    int64_t store_start_time_us = yysys::CTimeUtil::getTime();
    if (OB_SUCCESS != (send_err = slave_mgr_->post_log_to_slave(start_cursor, end_cursor, buf, len)))
    {
      YYSYS_LOG(WARN, "slave_mgr.send_data(buf=%p[%ld], %s)=>%d", buf, len, to_cstring(*this), send_err);
    }
    if (OB_SUCCESS != (ret = log_writer_.write(start_cursor, end_cursor,
                                               buf, len + ObLogGenerator::LOG_FILE_ALIGN_SIZE)))
    {
      YYSYS_LOG(ERROR, "log_writer.write_log(buf=%p[%ld], cursor=[%s,%s])=>%d, maybe disk FULL or Broken",
                buf, len, to_cstring(start_cursor), to_cstring(end_cursor), ret);
    }
    else
    {
      last_disk_elapse_ = yysys::CTimeUtil::getTime() - store_start_time_us;
  ...
  {
    YYSYS_LOG(ERROR, "log_generator.commit(end_cursor=%s)", to_cstring(end_cursor));
  }
  else if (OB_SUCCESS != (ret = write_log_hook(true, start_cursor, end_cursor, buf, len)))
  {
    YYSYS_LOG(ERROR, "write_log_hook(log_id=[%ld,%ld))=>%d", start_cursor.log_id_, end_cursor.log_id_, ret);
  }
  else if (len > 0)
  {
    last_flush_log_time_ = yysys::CTimeUtil::getTime();
  }
...
int ObLogWriter::flush_log(TraceLog::LogBuffer &tlog_buffer, const bool sync_to_slave, const bool is_master)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (last_disk_elapse_ > disk_warn_threshold_us_)
      {
        YYSYS_LOG(TRACE, "last_disk_elapse_[%ld] > disk_warn_threshold_us[%ld], cursor=[%s,%s], len=%ld",
                  last_disk_elapse_, disk_warn_threshold_us_, to_cstring(start_cursor), to_cstring(end_cursor), len);
      }
    }
    if (sync_to_slave)
    {
      ...
      {
        YYSYS_LOG(ERROR, "slave_mgr.send_data(buf=%p[%ld], cur_write=[%s,%s], %s)=>%d", buf, len, to_cstring(start_cursor), to_cstring(end_cursor), to_cstring(*this), send_err);
      }
      else if (delay >= 0)
      {
        ...
        {
          YYSYS_LOG(TRACE, "last_net_elapse_[%ld] > net_warn_threshold_us[%ld]", last_net_elapse_, net_warn_threshold_us_);
        }
      }
    }
  }
...
int ObLogWriter::write_and_flush_log(const LogCommand cmd, const char* log_data, const int64_t data_len)
{
  ...
  else if (!log_generator_.is_clear())
  {
    ...
    YYSYS_LOG(ERROR, "log_buffer not empty.");
  }
  else if (OB_SUCCESS != (ret = write_log(cmd, log_data, data_len)))
  {
    YYSYS_LOG(ERROR, "write_log(cmd=%d, log_data=%p[%ld])", cmd, log_data, data_len);
  }
  else if (OB_SUCCESS != (ret = flush_log(TraceLog::get_logbuffer())))
  {
    YYSYS_LOG(INFO, "log write write and flush log, log_cmd=%d", cmd);
  }
  return ret;
}
...
int ObLogWriter::store_log(const char* buf, const int64_t buf_len, const bool sync_to_slave)
{
  ...
  {
    YYSYS_LOG(ERROR, "log_generator.fill_batch(%p[%ld])=>%d", buf, buf_len, ret);
  }
  else if (OB_SUCCESS != (ret = flush_log(TraceLog::get_logbuffer(), sync_to_slave, false)))
  {
    YYSYS_LOG(ERROR, "flush_log(buf=%p[%ld],sync_slave=%s)=>%d", buf, buf_len, STR_BOOL(sync_to_slave), ret);
  }
  return ret;
}
...
int ObLogWriter::switch_log_file(uint64_t &new_log_file_id)
{
  int ret = check_inner_stat();
  new_log_file_id = 0;
  YYSYS_LOG(INFO, "log write switch log file");
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "check_inner_stat()=>%d", ret);
  }
  ...
  {
    YYSYS_LOG(ERROR, "flush_log()=>%d", ret);
  }
  else if (OB_SUCCESS != (ret = log_generator_.switch_log((int64_t&)new_log_file_id)))
  {
    YYSYS_LOG(ERROR, "log_generator.switch_log()=>%d", ret);
  }
  else if (OB_SUCCESS != (ret = flush_log(TraceLog::get_logbuffer())))
  {
    YYSYS_LOG(ERROR, "flush_log()=>%d", ret);
...
int ObLogWriter::write_checkpoint_log(uint64_t &cur_log_file_id)
{
  ...
  YYSYS_LOG(INFO, "wirte_checkpoint_log");
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "check_inner_stat()=>%d", ret);
  }
  ...
  {
    YYSYS_LOG(ERROR, "log_generator.check_point()=>%d", ret);
  }
  else if (OB_SUCCESS != (ret = switch_log_file(new_log_file_id)))
  {
    YYSYS_LOG(ERROR, "switch_log_file()=>%d", ret);
  }
  return ret;
}
