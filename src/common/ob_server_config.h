...
namespace oceanbase
{
  namespace common
  {
    class ObServerConfig
    {
      ...
      public:
        DEF_IP(root_server_ip, "0.0.0.0", "(default: range: ) root server ip address");
        DEF_INT(port, "0", "(1024,65536)", "(default: range:(1024,65536)) listen port");
        DEF_STR(devname, "bond0", "(default:bond0 range: ) listen device");
        DEF_INT(retry_times, "3", "[1,]", "(default:3 range:[1,]) retry times if failed");
        DEF_INT(cluster_id, "0", "(default:0 range: ) cluster id");
    };
  }
}
