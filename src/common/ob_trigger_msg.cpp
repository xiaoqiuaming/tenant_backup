...
DEFINE_SERIALIZE(ObDdlTriggerMsg)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to encode version:ret[%d]", ret);
  }
  if (OB_SUCCESS != ret)
  {
    pos = save_pos;
...
DEFINE_DESERIALIZE(ObDdlTriggerMsg)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to decode version:ret[%d]", ret);
  }
  if (OB_SUCCESS != ret)
  {
    pos = save_pos;
