...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObClientHelper::scan(const ObScanParam& scan_param,ObScanner& scanner)
    {
      ...
      {
        YYSYS_LOG(ERROR,"cann't get mergeserver,ret = %d",ret);
      }
      ...
      for(uint32_t i=0; i < sizeof(merge_server_) / sizeof(merge_server_[0]); ++i)
      {
        if ( 0 != merge_server_[i].get_ipv4() && 0 != merge_server_[i].get_port())
        {
          ...
          {
            YYSYS_LOG(INFO,"scan from (%s)", to_cstring(merge_server_[i]));
            if (OB_RESPONSE_TIME_OUT == ret || OB_PACKET_NOT_SENT == ret)
            {
              YYSYS_LOG(WARN,"scan from (%s) error,ret = %d", to_cstring(merge_server_[i]),ret);
              continue; //retry
            }
          }
          break;
    ...
    int ObClientHelper::get(const ObGetParam& get_param,ObScanner& scanner)
    {
      ...
      for(uint32_t i=0; i < sizeof(merge_server_) / sizeof(merge_server_[0]); ++i)
      {
        if ( 0 != merge_server_[i].get_ipv4() && 0 != merge_server_[i].get_port())
        {
          YYSYS_LOG(DEBUG,"get from (%s)", to_cstring(merge_server_[i]));
        
          if ((ret = get(merge_server_[i],get_param,scanner)) != OB_SUCCESS)
          {
            if (OB_RESPONSE_TIME_OUT == ret || OB_PACKET_NOT_SENT == ret)
            {
              YYSYS_LOG(WARN,"get from (%s) error,ret = %d", to_cstring(merge_server_[i]), ret);
              continue; //retry
            }
          }
          break;
    ...
    int ObClientHelper::scan(const ObServer& server,const ObScanParam& scan_param,ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to send request to (%s), ret=%d", to_cstring(server),ret);
          }
        }
    ...
    int ObClientHelper::apply(const ObServer& update_server, const ObMutator& mutator)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to send request to (%s), ret=%d", to_cstring(update_server),ret);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(ERROR,"apply failed : %d",result_code.result_code_);
            }
          }
        }
      }
    ...
    int ObClientHelper::get_tablet_info(const ObScanParam& scan_param)
    {
      ...
      {
        YYSYS_LOG(ERROR,"get tablet from rootserver(%s) failed:[%d]", to_cstring(root_server_),ret);
      }
    ...
    int ObClientHelper::parse_merge_server(ObScanner& scanner)
    {
      ...
      while((OB_SUCCESS == ret))
      {
        ...
        {
          YYSYS_LOG(DEBUG,"row changed,ignore, %s", to_cstring(cell->row_key_)); 
        ...
        else if (cell != NULL)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(DEBUG,"ip,port,version:%ld,%ld,%ld",ip,port,version);
