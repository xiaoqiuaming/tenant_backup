...
ObProbabilityRandom::ObProbabilityRandom():lock_(yysys::WRITE_PRIORITY)
{
  init_ = false;
  index_count_ = 0;
...
int ObProbabilityRandom::init(const int32_t percent[], const int32_t count)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "check the input percent failed:ret[%d]", ret);
  }
  else if (sum <= 0)
  {
    YYSYS_LOG(WARN, "check input percent sum failed:count[%d], sum[%d]", count, sum);
  }
  else
  {
    // must success
    yysys::CWLockGuard lock(lock_);
...
int32_t ObProbabilityRandom::random(void) const
{
  ...
  if (true == init_)
  {
    ...
    yysys::CRLockGuard lock(lock_);
    ret = index_[random_percent % index_count_];
  }
  else
  {
    YYSYS_LOG(WARN, "check status not init");
  }
  return ret;
}
    ...
    int32_t & percent_sum) const
{
  ...
  for (int32_t i = 0; i < count; ++i)
  {
    if (percent[i] < 0)
    {
      ...
      YYSYS_LOG(WARN, "check percent failed:index[%d], count[%d], percent[%d]", i, count, percent[i]);
      break;
    }
    else
    {
  ...
  if (percent_sum <= 0)
  {
    YYSYS_LOG(WARN, "check percent sum failed:count[%d], sum[%d]", count, percent_sum);
    ret = OB_INVALID_ARGUMENT;
  }
  return ret;
}
