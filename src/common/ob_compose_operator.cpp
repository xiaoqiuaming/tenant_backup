...
namespace oceanbase
{
  namespace common
  {
        ...
        const int64_t row_width)
    {
      ...
      {
        YYSYS_LOG(WARN, "the row column size is less than composite column size,"
    ...
    int ObComposeOperator::do_compose()
    {
      ...
      {
        YYSYS_LOG(WARN, "the result array includes part row, total_cell_size=%ld, "
      ...
      else
      {
             ...
             i += row_width_)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to caculate composite column for row=%ld", i);
            //add for [expire condition]-b
            ret = OB_EXPIRE_CONDITION_ERROR;
            break;
            //add for [expire condition]-e
          }
        }
      }
        ...
        const int64_t row_end)
    {
      ...
      for (int64_t i = 0; i < comp_column_size && OB_SUCCESS == ret; ++i)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to caculate composite column, row_beg=%ld, "
        ...
        {
          YYSYS_LOG(DEBUG, "success to calculate composite val. value is:");
          comp_val.dump();
        }
      }
