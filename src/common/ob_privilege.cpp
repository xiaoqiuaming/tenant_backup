...
int ObPrivilege::init()
{
  ...
  {
    YYSYS_LOG(WARN, "init username hash map failed, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = user_table_map_.create(USERTABLE_MAP_BUCKET_NUM)))
  {
    YYSYS_LOG(WARN, "init user table hash map failed, ret=%d", ret);
  }
  //add dolphin [database manager]@20150607
  else if (OB_SUCCESS != (ret = user_database_pri_map_.create(USERDB_MAP_BUCKET_NUM)))
  {
    YYSYS_LOG(WARN, "init  user database privilege hash map failed, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = name_database_map_.create(DBNAME_MAP_BUCKET_NUM)))
  {
    YYSYS_LOG(WARN, "init name database hash map failed, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = table_database_map_.create(TABLE_DATABASE_BUCKET_NUM)))
  {
    YYSYS_LOG(WARN, "init table database hash map failed, ret=%d", ret);
  }
  //add:e
  return ret;
}
int ObPrivilege::user_is_valid(const ObString & username, const ObString & password) const
{
  ...
  if (username.length() <= 0)
  {
    ...
    YYSYS_LOG(WARN, "usrname is empty string, ret=%d", ret);
  }
  else
  {
    User user;
    ...
    {
      YYSYS_LOG(WARN, "username:%.*s not exist, ret=%d", username.length(), username.ptr(), ret);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "system password=%.*s, provided password=%.*s", user.password_.length(), user.password_.ptr(), pwd.length(), pwd.ptr());
...
//add for [471-ob_export with username and password]-b
int ObPrivilege::user_is_valid_for_obexport(const ObString &username, const ObString &password) const
{
  int ret = OB_SUCCESS;
  if (username.length() <= 0)
  {
  ...
  else
  {
    ...
    else
    {
      ...
      ObEncryptedHelper::encrypt(encrypt_passwd, password);
      encrypt_passwd.assign_ptr(converted_password, SCRAMBLE_LENGTH * 2);

      if (user.password_ == encrypt_passwd)
      {
        if (user.locked_ == true)
        {
          ret = OB_ERR_USER_IS_LOCKED;
      ...
      {
        YYSYS_LOG(WARN, "system password=%.*s, provided password=%.*s", user.password_.length(), user.password_.ptr(), encrypt_passwd.length(), encrypt_passwd.ptr());
        ret = OB_ERR_WRONG_PASSWORD;
      }
    }
  }
...
//add for [471-ob_export with username and password]-e
...
int ObPrivilege::user_db_is_valid(const ObString &username, const ObString &db_name)const
{
  ...
  if(db_name.length() <= 0)
  {
    db_check = default_db;
    ret = OB_SUCCESS;
    YYSYS_LOG(INFO, "this user select no database...use default db");
  }
  else
  {
    db_check = db_name;
  ...
  if(true)
  {
    ...
    else
    {
      ...
      if(-1 == ret || hash::HASH_NOT_EXIST == ret)
      {
        YYSYS_LOG(WARN, "databasename:%.*s not exist, ret = %d", db_check.length(),db_check.ptr(),ret);
        ...
        if(OB_ERR_NO_ACCESS_TO_DATABASE == ret || -1 == ret || hash::HASH_NOT_EXIST == ret)
        {
          ...
          for(; user_prvi_iter != user_table_map_.end(); ++ user_prvi_iter)
          {
            ...
            if(user_table.user_id_ == user.user_id_)
            {
              ...
              if(-1 == ret || hash::HASH_NOT_EXIST == ret)
              {
                YYSYS_LOG(WARN,"can not find table[%ld] is belong to which database...",user_table.table_id_);
                ret = OB_ERROR;
              }
              else if(hash::HASH_EXIST == ret )
              {
...
int ObPrivilege::is_user_exist(const ObString &user_name, User &user) const
{
  ...
  if(-1 == ret || hash::HASH_NOT_EXIST == ret)
  {
    YYSYS_LOG(WARN, "user:%.*s not exist", user_name.length(), user_name.ptr());
    ret = OB_ERR_USER_NOT_EXIST;
  }
  else if(hash::HASH_EXIST == ret)
  {
...
int ObPrivilege::is_db_exist(const ObString &db_name, uint64_t &db_id) const
{
  ...
  if(-1 == ret || hash::HASH_NOT_EXIST == ret)
  {
    YYSYS_LOG(WARN, "database:%.*s not exist", db_name.length(), db_name.ptr());
    ret = OB_ERR_DATABASE_NOT_EXSIT;
  }
  else if(hash::HASH_EXIST == ret)
  {
...
int ObPrivilege::has_needed_privileges(const ObString &username, const ObArray<TablePrivilege> & table_privileges) const
{
  ...
  {
    ret = OB_ERR_USER_EMPTY;
    YYSYS_LOG(WARN, "username is empty string, ret=%d", ret);
  }
  else
  {
    ret = username_map_.get(username, user);
    ...
    for (int i = 0 ;i < table_privileges.count(); ++i)
    {
          ...
          || table_privilege.table_id_ == OB_INVALID_ID)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "username %.*s don't have global privilege, user_id=%ld, ret=%d",
      ...
      else
      {
        //ȫ��Ȩ�����Ҳ�����ʱ��ȥ�ֲ�Ȩ������
        // ���Ⱥ�__users��ȫ��Ȩ�޽��бȽ�
        // ��grantȨ�޵�ʱ��ʹ��grant priv_type on *�����ʱ�����Ȩ��ʹ�þ���ı���ʱ����Ҫ���������֧
        if (compare_privilege(table_privilege.privileges_, user.privileges_))
        {
          continue;
        }
        ...
        if (-1 == ret || hash::HASH_NOT_EXIST == ret)
        {
          YYSYS_LOG(WARN, "username %.*s don't have any privilege on table id=%ld, user_id = %ld, ret=%d",
        ...
        else
        {
          ...
          {
            ret = OB_ERR_NO_PRIVILEGE;
            YYSYS_LOG(WARN, "username %.*s don't have enough privilege on table id =%ld, user_id=%ld, ret=%d",
                      username.length(), username.ptr(), table_privilege.table_id_, user_id, ret);
            break;
          }
        }
...
int ObPrivilege::has_needed_db_privileges(const User &user, const TablePrivilege &needed_privilege, ObBitSet<> &fused_privilege) const
{
  ...
  {
    ret = OB_ERR_DATABASE_NOT_EXSIT;
    YYSYS_LOG(WARN, "database[%.*s] not exist.", needed_privilege.db_name_.length(), needed_privilege.db_name_.ptr());
  }
  else
  {
    ret = OB_SUCCESS;
    ...
    if (-1 == ret || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(WARN, "username %.*s don't have any privilege on database %.*s, ret=%d",
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "username %.*s don't have enough privilege on database %.*s, ret=%d",
                  user.user_name_.length(), user.user_name_.ptr(), needed_privilege.db_name_.length(), needed_privilege.db_name_.ptr(), ret);
        ret = OB_ERR_NO_PRIVILEGE;
      }
    }
...
int ObPrivilege::add_database_name_entry(const ObRow &row)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (pcell->get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to  write ObString(dbname) to string_buf_, ret=%d", ret);
      }
    }
    // Ŀǰϵͳ��֧��default�������������
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(dbname) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(dbid) from ObObj, ret=%d", ret);
      }
      else
      {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(dbid) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "add database failed, dbname:%.*s,ret=%d", database.dbname_.length(), database.dbname_.ptr(), ret);
    }
  }
  return ret;
}
...
int ObPrivilege::add_table_database_entry(const ObRow &row)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to  write ObString(tablename) to string_buf_, ret=%d", ret);
      }
    }
    // Ŀǰϵͳ��֧��default�������������
    else if (pcell->get_type() == ObNullType)
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(tablename) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "add table database failed, tablename:%.*s,ret=%d", td.table_name_.length(), td.table_name_.ptr(), ret);
    }
  }
  return ret;
}
int ObPrivilege::add_database_privilege_entry(const ObRow &row)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(user_id) from ObObj, ret=%d", ret);
      }
      else
      {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(user_id) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(1, pcell, table_id, column_id);
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(db_id) from ObObj, ret=%d", ret);
      }
      else
      {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(db_id) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(2, pcell, table_id, column_id);
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_all) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_all == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_ALL to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_all) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_alter) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_alter == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_ALTER to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_alter) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_create) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_create == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_CREATE to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_create) from row failed, ret=%d", ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_create_user) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_create_user == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_CREATE_USER to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_create_user) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_delete) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_delete == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_DELETE to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_delete) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_drop) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_drop == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_DROP to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_drop) from row failed, ret=%d", ret);
  }
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      else
      {
        if (priv_grant_option == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_GRANT_OPTION to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_grant_option) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_insert) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_insert == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_INSERT to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_insert) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_update) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_update == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_UPDATE to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_update) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_select) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_select == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_SELECT to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_select) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_replace) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_replace == 1)
        {
          ...
          {
            ret = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "add OB_PRIV_REPLACE to ObBitSet<> failed, ret=%d", ret);
          }
        }
      }
    }
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(priv_replace) from row failed, ret=%d", ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (pcell->get_type() == ObIntType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get_int(locked) from ObObj, ret=%d", ret);
      }
      else
      {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(locked) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "add user db privilege  failed, user_id:%ld,table_id:%ld,ret=%d", id.userid_, id.dbid_,ret);
    }
  }
...
int ObPrivilege::add_table_privileges_entry(const ObRow & row)
{
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(table_id) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(3, pcell, table_id, column_id);
  ...
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(9, pcell, table_id, column_id);//[conflict_level B]
    if (pcell->get_type() == ObIntType)
    {
      int64_t priv_grant_option = 0;
      if (OB_SUCCESS != (ret = pcell->get_int(priv_grant_option)))
      {
        YYSYS_LOG(WARN, "failed to get_int(priv_grant_option) from ObObj, ret=%d", ret);
      }
      else
      {
        if (priv_grant_option == 1)
  ...
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(11, pcell, table_id, column_id);//[conflict_level B]
    if (pcell->get_type() == ObIntType)
    {
      int64_t priv_update = 0;
      if (OB_SUCCESS != (ret = pcell->get_int(priv_update)))
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "add table privilege  failed, user_id:%ld,table_id:%ld,ret=%d", id.user_id_, id.table_id_,ret);
    }
  }
  return ret;
...
int ObPrivilege::add_users_entry(const ObRow & row)
{
  ...
  if (OB_SUCCESS == ret)
  {
    if (pcell->get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to  write ObString(username) to string_buf_, ret=%d", ret);
      }
    }
    // Ŀǰϵͳ��֧��default�������������
    else if (pcell->get_type() == ObNullType)
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(username) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(1, pcell, table_id, column_id);
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(userid) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(2, pcell, table_id, column_id);
    if (pcell->get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to  write ObString(password) to string_buf_, ret=%d", ret);
      }
    }
    else if (pcell->get_type() == ObNullType)
    {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(password) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(3, pcell, table_id, column_id);
    if (pcell->get_type() == ObVarcharType)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to  write ObString(comment) to string_buf_, ret=%d", ret);
      }
    }
    else if (pcell->get_type() == ObNullType)
    {
  ...
  {
    YYSYS_LOG(WARN, "raw_get_cell(comment) from row failed, ret=%d", ret);
  }
  if (OB_SUCCESS == ret)
  {
    ret = row.raw_get_cell(4, pcell, table_id, column_id);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "add user failed, username:%.*s,ret=%d", user.user_name_.length(), user.user_name_.ptr(), ret);
    }
  }
  return ret;
}
...
void ObPrivilege::print_info() const
{
  ...
  {
    YYSYS_LOG(INFO, "user[%.*s]", iter->first.length(), iter->first.ptr());
  }
}
