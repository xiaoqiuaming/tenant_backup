...
namespace oceanbase
{
  namespace common
  {
    ...
    ObScanParam::ObScanParam() : table_id_(OB_INVALID_ID),
      table_name_(),
      range_(), /*add wenghaixing [secondary index static_index_build]*/
      fake_range_(),
      need_fake_range_(false), /*add e*/
      scan_size_(0),
      scan_flag_(),
    ...
    bool *ObScanParam::is_return(const int64_t c_idx)const
    {
      ...
      {
        YYSYS_LOG(WARN,"try to access return info out of range [c_idx:%ld,return_info_size:%ld]",
    ...
    int ObScanParam::safe_copy(const ObScanParam & other)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to safe copy groupby param [err:%d]", err);
      }
      return err;
    }
    ...
    int ObScanParam::set_range(const ObNewRange& range)
    {
      ...
      if (deep_copy_args_)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to copy range.start_key_ to local buffer [err:%d]", err);
        }
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = buffer_pool_.write_string(range.end_key_,&(range_.end_key_)))))
        {
          YYSYS_LOG(WARN,"fail to copy range.end_key_ to local buffer [err:%d]", err);
        }
      }
      return err;
    }
    ...
    int ObScanParam::set(const uint64_t& table_id, const ObString& table_name, const ObNewRange& range, bool deep_copy_args)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to set range [err:%d]", err);
      }
      else if (deep_copy_args_)
      {
        if (OB_SUCCESS != (err = buffer_pool_.write_string(table_name, &table_name_)))
        {
          YYSYS_LOG(WARN,"fail to copy table name to local buffer [err:%d]", err);
        }
      }
    ...
    int ObScanParam::add_orderby_column(const ObString & column_name, Order order)
    {
      ...
      {
        YYSYS_LOG(WARN,"use only column id or column name, not both");
        err = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(WARN,"fail to copy column_name to local buffer [err:%d]", err);
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObScanParam::set_limit_info(const int64_t offset, int64_t count)
    {
      ...
      if (offset < 0 || count < 0)
      {
        YYSYS_LOG(WARN,"param error [offset:%ld,count:%ld]", offset, count);
        err = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObScanParam::set_topk_precision(const int64_t sharding_minimum_row_count, const double precision)
    {
      ...
      {
        YYSYS_LOG(WARN,"param error [sharding_minimum_row_count:%ld,precision:%f]", sharding_minimum_row_count, precision);
        err = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObScanParam::add_column(const ObString& column_name, bool is_return)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to copy column name to local buffer [err:%d]", rc);
      }
      return rc;
    }
    ...
    int ObScanParam::add_column(const ObString & expr, const ObString & as_name, bool is_return, bool is_expire_info)
    {
      ...
      {
        YYSYS_LOG(WARN,"select composite column list is full");
        err = OB_ARRAY_OUT_OF_RANGE;
      }

      if ((OB_SUCCESS == err) && (!comp_return_info_list_.push_back(is_return)))
      {
        YYSYS_LOG(WARN,"select return info list is full");
        err = OB_ARRAY_OUT_OF_RANGE;
      }
      return err;
    }
    ...
    int ObScanParam::add_column_if_notexists(uint64_t column_id)
    {
      int ret = OB_SUCCESS ;
      int64_t count = basic_column_id_list_.get_array_index();
      for(int64_t index = 0 ; index < count ; index++)
      {
        if(column_id == basic_column_ids_[index])
        {
          ret = OB_ENTRY_EXIST ;
          break;
        }
    ...
    int ObScanParam::add_where_cond(const ObString & expr, bool is_expire_cond/* = false*/)
    {
      ...
      {
        YYSYS_LOG(WARN,"argumen error, empty expr [expr.length():%d,expr.ptr():%p]", expr.length(), expr.ptr());
        err = OB_INVALID_ARGUMENT;
      }
    ...
    int ObScanParam::serialize_basic_param(char * buf, const int64_t buf_len, int64_t & pos) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Both table_name and table_id are set");
        }
        else
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        // YYSYS_LOG(ERROR,"test::whx fake range = %s, range = [%s], ret[%d]",to_cstring(fake_range_), to_cstring(range_), ret);
        if (OB_SUCCESS == ret)
        {
          ret = set_rowkey_obj_array(buf, buf_len, pos,
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        //YYSYS_LOG(ERROR,"test::whx phase 2 fake range  = %s, range = [%s], ret[%d]",to_cstring(fake_range_), to_cstring(range_), ret);
        if (OB_SUCCESS == ret)
        {
          ret = set_rowkey_obj_array(buf, buf_len, pos,
      ...
      //del by maosy [MultiUPS 1.0] [read uncommit]20170525 b:
      //add zhujun [transaction read uncommit]2016/3/24
      //trans_id
      //      if (OB_SUCCESS == ret)
      //      {
      //          //YYSYS_LOG(INFO,"ObScanParam:serialize transaction id:%s",to_cstring(trans_id_));
    ...
    int ObScanParam::deserialize_basic_param(const char * buf, const int64_t data_len, int64_t & pos)
    {
      ...
      //            //add end
      //        }
      //        //YYSYS_LOG(INFO,"ObScanParam::deserialize transaction id:%s",to_cstring(temp));
      //      }
      //add:e
      //add by maosy e
      return ret;
    ...
    int ObScanParam::serialize_column_param(char * buf, const int64_t buf_len, int64_t & pos) const
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "check column size failed");
          ret = OB_ERROR;
        }
    ...
    int ObScanParam::serialize_composite_column_param(char * buf, const int64_t buf_len, int64_t & pos) const
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to serialize SELECT_CLAUSE_COMP_COLUMN_FIELD ext obj");
      }
    ...
    int ObScanParam::serialize_return_info_param(char * buf, const int64_t buf_len, int64_t & pos) const
    {
      ...
      {
        YYSYS_LOG(WARN,"count of return info not coincident with count of columns [count_of_return_info:%ld,"
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to serialize SELECT_CLAUSE_RETURN_INFO_FIELD [err:%d]", err);
        }
        ObObj bool_obj;
        for (int64_t idx = 0; OB_SUCCESS == err && idx < get_return_info_size(); idx ++)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to serialize select return info [idx:%ld,err:%d]", idx, err);
          }
        }
      }
      return err;
    ...
    int ObScanParam::deserialize_return_info_param(const char * buf, const int64_t data_len, int64_t & pos)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to deserialize first obj [err:%d]", err);
      }
    ...
    DEFINE_SERIALIZE(ObScanParam)
    {
      ...
      {
        YYSYS_LOG(WARN,"use column id or column name, but not both");
      ...
      {
        YYSYS_LOG(WARN,"sizeof order list not coincident with column id/name list");
    ...
    DEFINE_DESERIALIZE(ObScanParam)
    {
      ...
      if ((OB_SUCCESS == ret) && (0 == get_return_info_size()))
      {
        ...
        for (int64_t i = 0; (OB_SUCCESS == ret) && i < basic_column_count; i++)
        {
          ...
          {
            YYSYS_LOG(WARN,"return info list is full");
    ...
    {
      YYSYS_LOG(INFO, "[dump] [table_name_=%.*s][table_id_=%lu]", table_name_.length(), table_name_.ptr(), table_id_);
      YYSYS_LOG(INFO, "[dump] scan range info: %s", to_cstring(range_));
      YYSYS_LOG(INFO, "[dump] [scan_direction=%s]", scan_flag_.direction_ == 0? "FORWARD" : "BACKWARD");
    }
    ...
    void ObScanParam::dump_column_param() const
    {
      ...
      if ((column_size > 0) && (column_id_size > 0))
      {
        YYSYS_LOG(ERROR, "[dump] check column size failed");
        ret = OB_ERROR;
      }
      ...
      if (ret == OB_SUCCESS)
      {
        for (int64_t i = 0; i < column_size; ++i)
        {
          YYSYS_LOG(INFO, "[dump] basic_column_names_[%ld]=%.*s",
                    i, basic_column_names_[i].length(), basic_column_names_[i].ptr());
        }

        for (int64_t i = 0; i < column_id_size; ++i)
        {
          YYSYS_LOG(INFO, "[dump] basic_column_ids_[%ld]=%lu",
    ...
    {
      YYSYS_LOG(INFO, "[dump] composite column size=%ld", select_comp_column_list_.get_array_index());
    }


    void ObScanParam::dump_filter_param() const
    {
      YYSYS_LOG(INFO, "[dump] condition filter count=%ld", condition_filter_.get_count());
    }


    void ObScanParam::dump_return_info_param() const
    {
      YYSYS_LOG(INFO, "[dump] return info size=%ld", get_return_info_size());
    }


    void ObScanParam::dump_groupby_param() const
    {
      YYSYS_LOG(INFO, "[dump] groupby info\n "
    ...
    {
      YYSYS_LOG(INFO, "[dump] [limit_offset_=%ld][limit_count=%ld]", limit_offset_, limit_count_);
    }
