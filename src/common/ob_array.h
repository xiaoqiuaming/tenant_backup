...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObArray<T, BlockAllocatorT, CallBack>::at(int64_t idx, T &obj) const
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "array in error state");
      }
    ...
    void ObArray<T, BlockAllocatorT, CallBack>::extend_buf(int64_t new_size)
    {
      ...
      if (NULL != new_data)
      {
        ...
        if (0 < data_size_)
        {
          YYSYS_LOG(DEBUG, "array extend buf, old_size=%ld new_size=%ld old_ptr=%p new_ptr=%p "
    ...
    int ObArray<T, BlockAllocatorT, CallBack>::push_back(const T &obj)
    {
      ...
      {
        YYSYS_LOG(WARN, "count_=%ld, data_size_=%ld, (int64_t)sizeof(T)=%ld, data_size_/(int64_t)sizeof(T)=%ld, ret=%d",
    ...
    ObArray<T, BlockAllocatorT, CallBack>::ObArray(const ObArray<T, BlockAllocatorT, CallBack> &other)
      : ObIArray<T>()
    {
      //add liu jun. [MultiUPS] [part_cache] 20150603:b
      data_size_ = 0;
      data_ = NULL;
      count_ = 0;
      //add:e
