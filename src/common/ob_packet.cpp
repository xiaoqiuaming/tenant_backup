...
namespace oceanbase
{
  namespace common
  {
    ...
    ObPacket::ObPacket() : no_free_(false), ob_packet_header_size_(0), api_version_(0), timeout_(0), data_length_(0),
        priority_(NORMAL_PRI), target_id_(0), receive_ts_(0), session_id_(0), alloc_inner_mem_(false), cleanup_(false),trace_id_(0), req_sign_(0)
    ,conn_seq_(0), disconn_time_(0) //[701]
    {
    }
    ...
    int ObPacket::serialize()
    {
      ...
      if (ret == OB_SUCCESS && target_id_ != OB_SELF_FLAG)
      {
        ...
        {
          ret = OB_MEM_OVERFLOW;
          YYSYS_LOG(ERROR, "alloc memory from out packet pool failed, buf size: %ld, errno: %d", buf_size, errno);
        }
        else
        {
    ...
    int ObPacket::serialize(ObDataBuffer* buffer)
    {
      ...
      if (NULL == buffer)
      {
        YYSYS_LOG(WARN, "invalid argument buffer is %p", buffer);
      ...
      else
      {
        ...
        if (ret == OB_SUCCESS && target_id_ != OB_SELF_FLAG)
        {
          ...
          {
            YYSYS_LOG(WARN, "buffer capacity is not enough, need %ld, but actual is %ld",
    ...
    int ObPacket::deserialize()
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize header_ failed, ret=%d", ret);
      }
    ...
    int ObPacket::do_sum_check()
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "packet header check failed");
        }
      }
      else
      {
        YYSYS_LOG(WARN, "packet magic number check failed");
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "packet body checksum invalid");
        }
      }
    ...
    bool ObPacket::encode(char* output, int64_t len)
    {
...

      {
        ...
        // lide.wd : ������ʱ����������ʱ����0.4 ����ģ���ô���Ǵ���trace_id, ������Ҫ����
      ...
      if (OB_SUCCESS != rc)
      {
        YYSYS_LOG(ERROR, "encode packet into request output buffer faild, output is %p, pos is %ld",
                  output, pos);
        ret = false;
      }
      return ret;
    ...
    void ObPacket::set_packet_priority(const int32_t priority)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid packet priority: %d", priority);
      }
    }
