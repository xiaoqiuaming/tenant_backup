  ...
  namespace common
  {
    ...
    int ObRowkey::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      if (obj_cnt_ < obj_cnt)
      {
        ...
        YYSYS_LOG(ERROR, "obj number greater than expected, expect_max_cnt=%ld real_cnt=%ld",
                  obj_cnt_, obj_cnt);
      }
      else
      {
    ...
    int ObRowkey::deserialize_objs(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      if (NULL == obj_ptr_)
      {
        YYSYS_LOG(ERROR, "obj array is NULL");
    ...
    int ObRowkey::deserialize_from_stream(const char* buf, const int64_t buf_len)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "deserialize objs error, ret=%d", ret);
      }
      else if (pos != buf_len)
      {
        YYSYS_LOG(ERROR, "obj count not match, pos=%ld, buf_len=%ld, i=%ld, obj_count = %ld",
    ...
    int ObRowkey::to_string_memcpy(char *buffer, const int64_t length, int64_t &pos) const
    {
      int ret = OB_SUCCESS;
      if (is_min_row())
      {
      ...
      else
      {
        for (int i = 0; i < obj_cnt_; ++i)
        {
          if (pos < length)
          {
            obj_ptr_[i].to_string_memcpy(buffer, length, pos);
            if (i < obj_cnt_ - 1)
            {
              databuff_printf(buffer, length, pos, ",");
            }
...
//        if (obj_ptr_[i].get_type() == ObVarcharType||obj_ptr_[i].get_type() == ObDecimalType)
        ...
        {
          total_len += obj_ptr_[i].get_nwords() * sizeof(uint64_t);
        }
      }
      return total_len;
    }
...
//        if (obj_ptr_[i].get_type() == ObVarcharType||obj_ptr_[i].get_type() == ObDecimalType)
                       ...
                       char* buf, int64_t buf_size, int64_t &used_buf_len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "wrong rowkey length, rowkey_info_len=%ld rowkey_len=%ld",
      ...
      else
      {
        ...
        for (int64_t i = 0; i < rowkey.get_obj_cnt(); ++i)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to cast obj, i=%ld", i);
            break;
          }
          else
          {
                       ...
                       ObObj *casted_objs, ObObj *rowkery_dst)
    {
      ...
      {
        YYSYS_LOG(ERROR, "wrong rowkey length, expect_rowkey_type_desc_len=%ld rowkey_len=%ld",
