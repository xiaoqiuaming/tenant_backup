  ...
  namespace common
  {
    ...
    class col_checksum
    {
      ...
      public:
        ...
        //add wenghaixing [secondary index col checksum] 20141208
        //modify for [2nd index col checksum]-b
        //         int sum (const col_checksum &col)
        int sum (const col_checksum &col, bool check_col_count = true)
        //modify for [2nd index col checksum]-e
        {
          int ret = OB_SUCCESS;
          //add liumz, [bugfix: empty tablet]20161121:b
          if (0 == strlen(col.get_str_const()))
          {
            YYSYS_LOG(WARN, "empty column checksum!");
          }
          //add:e
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "failed to parse src column checksum_str[%s]", checksum_str);
              ret = OB_ERROR;
            }
            else if (OB_SUCCESS != (ret = tokenize(col.checksum_str, strlen(col.checksum_str), ',', token_nr_add, tokens_add)))
            {
              YYSYS_LOG(ERROR, "failed to parse add column checksum_str[%s]", col.checksum_str);
              ret = OB_ERROR;
            }
            //modify for [2nd index col checksum]-b
            //                      else if (token_nr_src != token_nr_add)
            else if (check_col_count && token_nr_src != token_nr_add)
              //modify for [2nd index col checksum]-e
            {
              YYSYS_LOG(ERROR, "column checksum count is not equal, src column count[%d], add column count[%d], src column checksum_str[%s], add column checksum_str[%s]",
                        token_nr_src, token_nr_add, checksum_str, col.checksum_str);
              ret = OB_ERROR;
            }
            else
            {
              //add for [2nd index col checksum]-b
              if(!check_col_count && token_nr_src != token_nr_add)
              {
                YYSYS_LOG(WARN, "column checksum count is not equal, src column count[%d], add column count[%d], src column checksum_str[%s], add column checksum_str[%s]",
                          token_nr_src, token_nr_add, checksum_str, col.checksum_str);
              }
              //add for [2nd index col checksum]-e

              int idx = 0;
              std::map<uint64_t, uint64_t> col_sum_add;//�洢add column checksum_str������<cid, checksum>
              for (idx = 0; idx < token_nr_add && OB_SUCCESS == ret; idx++)
              {
                ...
                {
                  YYSYS_LOG(ERROR, "fialed to separate cid:checksum[%.*s], add column checksum_str[%s]", static_cast<int>(tokens_add[idx].len), tokens_add[idx].token, col.checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else if (attr_nr != 2)
                {
                  YYSYS_LOG(ERROR,
                            "column checksum must be 2 attributes, in manner of <cid:checksum>, column checksum[%.*s], add column checksum_str[%s]",
                            static_cast<int>(tokens_add[idx].len),
                            tokens_add[idx].token,
                            col.checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "fetch column id failed, column id str[%.*s], add column checksum_str[%s]", static_cast<int>(tokens_attr[0].len), tokens_attr[0].token, col.checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else if (OB_SUCCESS != (ret = transform_str_to_int(tokens_attr[1].token, tokens_attr[1].len, checksum)))
                  {
                    YYSYS_LOG(ERROR, "fetch checksum  failed, checksum str[%.*s], add column checksum_str[%s]", static_cast<int>(tokens_attr[1].len), tokens_attr[1].token, col.checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else
                  {
                    ...
                    {
                      YYSYS_LOG(ERROR, "equal column[cid = %lu] exists in add column checksum_str[%s]", cid, col.checksum_str);
                      ret = OB_ERROR;
                      break;
                    }
                    else
              ...
              std::vector<std::pair<uint64_t, uint64_t> > col_sum_src;//�洢src column checksum������<cid,checksum>
              for (idx = 0; idx < token_nr_src && OB_SUCCESS == ret; idx++)
              {
                ...
                {
                  YYSYS_LOG(ERROR, "fialed to separate cid:checksum[%.*s], src column checksum_str[%s]", static_cast<int>(tokens_src[idx].len), tokens_src[idx].token, checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else if (attr_nr != 2)
                {
                  YYSYS_LOG(ERROR,
                            "column checksum must be 2 attributes, in manner of <cid:checksum>, column checksum[%.*s], src column checksum_str[%s]",
                            static_cast<int>(tokens_src[idx].len),
                            tokens_src[idx].token,
                            checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else
                {
                  ...
                  if (OB_SUCCESS != (ret = transform_str_to_int(tokens_attr[0].token, tokens_attr[0].len, cid)))
                  {
                    YYSYS_LOG(ERROR, "fetch column id failed, column id str[%.*s], src column checksum_str[%s]", static_cast<int>(tokens_attr[0].len), tokens_attr[0].token, checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else if (OB_SUCCESS != (ret = transform_str_to_int(tokens_attr[1].token, tokens_attr[1].len, checksum)))
                  {
                    YYSYS_LOG(ERROR, "fetch checksum  failed, checksum str[%.*s], src column checksum_str[%s]", static_cast<int>(tokens_attr[1].len), tokens_attr[1].token, checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else
                  {
                    //�ҳ�cid��col_sum_add�ж�Ӧ��ֵ,����checksum���,����Ҳ�������
                    std::map<uint64_t, uint64_t>::iterator iter = col_sum_add.find(cid);
                    if (iter != col_sum_add.end())
                    {
                    ...
                    else if(check_col_count)
                    {
                      YYSYS_LOG(ERROR, "can't find equal column id[cid = %lu], src column checksum_str[%s], add column checksum_str[%s]", cid, checksum_str, col.checksum_str);
                      ret = OB_ERROR;
                      break;
                    }
                  }
        ...
        bool compare(const col_checksum &col)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to parse compare column checksum_str[%s]", col.checksum_str);
            ret = OB_ERROR;
          }
          else
          {
            ...
            for (idx = 0; idx < token_nr_src && OB_SUCCESS == ret; idx++)
            {
              ...
              else
              {
                ...
                else
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "equal column[cid = %lu] exists in src column checksum_str[%s]", cid, checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else
            ...
            for (idx = 0; idx < token_nr_cmp && OB_SUCCESS == ret; idx++)
            {
              ...
              {
                YYSYS_LOG(ERROR, "fialed to separate cid:checksum[%.*s], compare column checksum_str[%s]", static_cast<int>(tokens_cmp[idx].len), tokens_cmp[idx].token, col.checksum_str);
                ret = OB_ERROR;
                break;
              }
              else if (attr_nr != 2)
              {
                YYSYS_LOG(ERROR,
                          "column checksum must be 2 attributes, in manner of <cid:checksum>, column checksum[%.*s], compare column checksum_str[%s]",
                          static_cast<int>(tokens_cmp[idx].len),
                          tokens_cmp[idx].token,
                          col.checksum_str);
                ret = OB_ERROR;
                break;
              }
              else
              {
                ...
                if (OB_SUCCESS != (ret = transform_str_to_int(tokens_attr[0].token, tokens_attr[0].len, cid)))
                {
                  YYSYS_LOG(ERROR, "fetch column id failed, column id str[%.*s], compare column checksum_str[%s]", static_cast<int>(tokens_attr[0].len), tokens_attr[0].token, col.checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else if (OB_SUCCESS != (ret = transform_str_to_int(tokens_attr[1].token, tokens_attr[1].len, checksum)))
                {
                  YYSYS_LOG(ERROR, "fetch checksum  failed, checksum str[%.*s], compare column checksum_str[%s]", static_cast<int>(tokens_attr[1].len), tokens_attr[1].token, col.checksum_str);
                  ret = OB_ERROR;
                  break;
                }
                else
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "equal column[cid = %lu] exists in compare column checksum_str[%s]", cid, col.checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  else
              ...
              if (col_sum_cmp.size() <= col_sum_src.size())
              {
                ...
                while (iter != col_sum_cmp.end())
                {
                  ...
                  if (iter_temp != col_sum_src.end())
                  {
                    ...
                    {
                      YYSYS_LOG(ERROR, "equal column id[cid = %lu], but not equal checksum, src column checksum_str[%s], compare column checksum_str[%s]", iter->first, checksum_str, col.checksum_str);
                      ret = OB_ERROR;
                      break;
                    }
                  }
                  ...
                  {
                    YYSYS_LOG(ERROR, "can't find equal column id[cid = %lu], src column checksum_str[%s], compare column checksum_str[%s]", iter->first, checksum_str, col.checksum_str);
                    ret = OB_ERROR;
                    break;
                  }
                  iter++;
