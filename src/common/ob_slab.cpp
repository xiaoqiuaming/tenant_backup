...
namespace oceanbase
{
  namespace common
  {
    ...
    ObSlabCache* ObSlabCacheManager::slab_cache_create(const char *name,int64_t size,int64_t align)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        for(ObVector<ObSlabCache *>::iterator it = cache_list_.begin(); it != cache_list_.end();++it)
        {
          ...
          {
            YYSYS_LOG(ERROR,"slab %s has been created",name);
            err = OB_ERROR;
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        YYSYS_LOG(INFO,"create cache %s,size:%ld,align:%ld",name,size,align);
        char *buf = cache_cache_.slab_cache_alloc();
        if (NULL == buf)
        {
          YYSYS_LOG(ERROR,"alloc ObSlabCache failed");
          err = OB_ERROR;
        }
        else
        {
    ...
    void ObSlabCacheManager::slab_cache_destroy(ObSlabCache *slab_cache)
    {
      ...
      {
        YYSYS_LOG(ERROR,"clear slab failed,cannot destroy");
      }
      else
      {
                                                                             ...
                                                                             free_item_count_(0)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        YYSYS_LOG(INFO,"create slab %s success,obj_size_:%ld,obj_per_slab:%ld",slab_name_,obj_size_,obj_per_slab_);
      }
    }
    ...
    int ObSlabCache::clear()
    {
      ...
      {
        YYSYS_LOG(ERROR,"this slab is not empty,cann't clear");
        ret = OB_ERROR;
      }
      else
      {
    ...
    void ObSlabCache::slab_for_each_free(ObSlab *slab) const
    {
      if (NULL != slab)
      {
        ...
        {
          YYSYS_LOG(DEBUG,"slab:%p,slab->next_:%p",item,item->u_.next_);
        }
      }
    }
    ...
    void ObSlabCache::cache_init_objs(ObSlab *slab)
    {
      if (NULL != slab)
      {
        ...
        for(int64_t i=0;i < obj_per_slab_; ++i)
        {
          ...
          //YYSYS_LOG(INFO,"slab:%p,slab->mem_:%p,obj_size_:%ld,item:%p,item->magic_:%lu",
