...
int ObNewScannerHelper::print_new_scanner(ObNewScanner &new_scanner, ObRow &row, bool has_rowkey /* = false */)
{
  ...
  while (OB_SUCCESS == ret)
  {
    ...
    else if (OB_SUCCESS == ret)
    {
      if (has_rowkey)
      {
        YYSYS_LOG(INFO, "new scanner rowkey:%s row:%s", to_cstring(*rowkey), to_cstring(row));
      }
      else
      {
        YYSYS_LOG(INFO, "new scanner row:%s", to_cstring(row));
      }
    }
    else
    {
      YYSYS_LOG(WARN, "new scanner get next row fail");
    }
  }
...
int ObNewScannerHelper::add_cell(ObRow &row, const ObCellInfo &cell, bool is_ups_row)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to get action flag cell:ret[%d]", ret);
  }
    ...
    if (!is_ups_row) //normal row
    {
      if (ObExtendType == cell.value_.get_type())
      {
        if (ObActionFlag::OP_ROW_DOES_NOT_EXIST == cell.value_.get_ext())
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to put rowkey to row:ret[%d]", ret);
          }
        }
        else
        {
          YYSYS_LOG(WARN, "unsupported ext type[%ld]", cell.value_.get_ext());
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to apply cell to value:ret[%d]", ret);
        }
        else
        {
    ...
    else //ups row
    {
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "unsupport ext type[%ld]", cell.value_.get_ext());
        }
      }
    }
  }
...
int ObNewScannerHelper::put_rowkey_to_row(ObRow &row, const ObCellInfo &cell)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "row_desc is null");
  }
  else if(row_desc->get_rowkey_cell_count() > 0)
  {
    if (row_desc->get_rowkey_cell_count() != cell.row_key_.length())
    {
      ...
      YYSYS_LOG(WARN, "rowkey count is wrong row_desc rowkey count [%ld], actual rowkey count[%ld]",
...
/*@berif  ����row_version �� column_id �е� �汾*/
int ObNewScannerHelper::add_column_version(ObRowVersion &row_version, const ObCellInfo &cell,
                                           const int64_t column_version)
{
  int ret= OB_SUCCESS;
  int64_t  local_column_vesion = OB_INVALID_VERSION;
  int64_t  local_index= OB_INVALID_INDEX;
  //del hongchen [BUG_265_FIXED] 20170607:b
  //null must be set column version
  /*if( cell.value_.is_null())
  {
    //do nothing
  }
  else if(ObExtendType == cell.value_.get_type())
  */
  //del hongchen [BUG_265_FIXED] 20170607:e
  if(ObExtendType == cell.value_.get_type())
  {
    if(ObActionFlag::OP_DEL_ROW == cell.value_.get_ext())
    {
      row_version.reset();
      if(OB_SUCCESS!=(ret = row_version.add_column_version(OB_DELETED_FLAG_COLUMN_ID, column_version)))
      {
    ...
    else if(ObActionFlag::OP_NOP == cell.value_.get_ext())
    {
      //do nothing
      //add for [212]-b
      if (row_version.is_deleted_row())
      {
        if (OB_SUCCESS != (ret = row_version.add_column_version(OB_NOP_FLAG_COLUMN_ID, column_version)))
        {
          YYSYS_LOG(WARN, "add column version fail, ret=%d", ret);
        }
      }
  ...
  {
    if(OB_SUCCESS!=(ret=row_version.get_column_version(cell.column_id_, local_column_vesion, local_index)))
    {
      YYSYS_LOG(WARN, "get column version fail, ret=%d",ret);
    }
    else if(OB_INVALID_INDEX == local_index)  //not found
    {
      if(OB_SUCCESS!=(ret = row_version.add_column_version(cell.column_id_, column_version)))
      {
        YYSYS_LOG(WARN, "add column version fail, ret=%d",ret);
      }
    }
    //mod hongchen [VERSION_BUGFIX] 20170812:b
    else if (column_version == local_column_vesion)
    {
      //nothingtodo
    }
    else if(column_version < local_column_vesion)
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "version must be monotone increasing, but cur column column_id_[%lu],column_version[%ld] < local_column_vesion[%ld],ret=%d",
                cell.column_id_,
                column_version,
                local_column_vesion,
                ret);
    }
    //mod hongchen [VERSION_BUGFIX] 20170812:e
    else if(column_version>local_column_vesion) //�µ�version ���ھɵ�version
    {
      if(OB_SUCCESS!=(ret = row_version.update_column_version(cell.column_id_, column_version)))
      {
        YYSYS_LOG(WARN, "update  column version fail, ret=%d",ret);
      }
    }
  }
  return ret;
...
int ObNewScannerHelper::add_ups_cell(ObUpsRow &ups_row, const ObCellInfo &cell, ObStringBuf *str_buf /* = NULL */)
{
  ...
  if(ObExtendType == cell.value_.get_type())
  {
    if(ObActionFlag::OP_DEL_ROW == cell.value_.get_ext())
    {
      ...
      {
        YYSYS_LOG(WARN, "ups row set all nop fail:ret[%d]", ret);
      }
    }
    else if(ObActionFlag::OP_NOP == cell.value_.get_ext())
    {
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "ups row get cell fail:ret[%d]", ret);
    }
    else
    {
      value = *got_cell;
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "obj apply fail:ret[%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "ups set value fail:ret[%d]", ret);
      }
    }
  }
...
int ObNewScannerHelper::convert(ObScanner &scanner, const ObRowDesc *row_desc, ObNewScanner &new_scanner)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "get scanner fullfilled fail:ret[%d]", ret);
    }
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "scanner next cell fail:ret[%d]", ret);
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "scanner get cell fail:ret[%d]", ret);
      }
    }
    ...
    if(OB_SUCCESS == ret)
    {
      if(is_row_changed && !ups_row_empty)
      {
        ...
        {
          YYSYS_LOG(WARN, "new scanner add row fail:ret[%d]", ret);
        }
        else
        {
        ...
        if(OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "ups row reuse fail:ret[%d]", ret);
          }
          else
          {
            ups_row_empty = true;
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "add cell to ups row fail:ret[%d]", ret);
        }
        else
        {
          if(is_row_changed)
          {
            ...
            {
              YYSYS_LOG(WARN, "rowkey deep copy fail:ret[%d]", ret);
            }
            else
            {
              ups_row_empty = false;
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "when not fullfilled, fullfilled row num at least be one");
    }
    else if(OB_SUCCESS != (ret = new_scanner.set_is_req_fullfilled(is_fullfilled, fullfilled_row_num)))
    {
      YYSYS_LOG(WARN, "new scanner set is req fullfilled fail:ret[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = new_scanner.set_last_row_key(last_succ_added_rowkey)))
    {
      YYSYS_LOG(WARN, "set last rowkey fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "new scanner set range fail:ret[%d]", ret);
    }
  }
...
int ObNewScannerHelper::get_row_desc(const ObScanParam &scan_param, ObRowDesc &row_desc)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to add OB_ACTION_FLAG_COLUMN_ID:ret[%d]", ret);
    }
  }
...
int ObNewScannerHelper::get_row_desc(const ObGetParam &get_param, bool has_flag_column, ObRowDesc &row_desc)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "get param row should be positive:actual[%ld]", get_param.get_row_size());
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    for(int64_t i=0;OB_SUCCESS == ret && i<row_index.size_;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "add column desc fail:ret[%d], row_index.size_[%d], offset=%ld, row_num=%ld",
