    ...
    class TSIBlockCache
    {
      ...
      public:
        TSIBlockCache(): inited_(false), allocator_(NULL)
        {
          ...
          if (0 != (syserr = PTHREAD_KEY_MGR->create_thread_key(key_)))
          //mod hongchen [TSD_USER_IMPL] 20170916:e
          {
            err = OB_ERR_UNEXPECTED;
            YYSYS_LOG(ERROR, "pthread_key_create()=>%d", syserr);
          }
          else if (OB_SUCCESS != (err = block_list_pool_.init(MAX_THREAD_NUM)))
          {
            YYSYS_LOG(ERROR, "block_list_pool.init(%ld)=>%d", MAX_THREAD_NUM, err);
          }
          else if (OB_SUCCESS != (err = global_block_list_.init(MAX_BLOCK_NUM)))
          {
            YYSYS_LOG(ERROR, "global_block_list.init(%ld)=>%d", MAX_BLOCK_NUM, err);
          }
          for(int64_t i = 0; OB_SUCCESS == err && i < MAX_THREAD_NUM; i++)
          {
            ...
            {
              YYSYS_LOG(ERROR, "block_list_pool.push(i=%ld)=>%d", i, err);
            }
          }
          if (OB_SUCCESS == err)
          {
        ...
        ~TSIBlockCache()
        {
          if (inited_)
          {
            ...
            BlockList* ptr = (BlockList*)(PTHREAD_KEY_MGR->delete_thread_key(key_));
            call_clear_block_list(ptr);
            //mod hongchen [TSD_USER_IMPL] 20170916:e
          }
          inited_ = false;
        }
...
//      const static int64_t MEDIUM_BLOCK_SIZE = OB_TC_MALLOC_BLOCK_SIZE;
        const static int64_t MEDIUM_BLOCK_SIZE = (1 << 19) + (1 << 10) - BLOCK_RESERVED_SIZE;
        const static int64_t BIG_BLOCK_SIZE = (1<<21) + (1<<10) - BLOCK_RESERVED_SIZE;
        const static int64_t DEFAULT_NORMAL_BLOCK_TCLIMIT = 128 + 64;
        const static int64_t DEFAULT_MEDIUM_BLOCK_TCLIMIT = 8;
        const static int64_t DEFAULT_BIG_BLOCK_TCLIMIT = 8;
        const static int64_t DEFAULT_NORMAL_BLOCK_GLIMIT = 1024 + 512;
        const static int64_t DEFAULT_MEDIUM_BLOCK_GLIMIT = 64;
