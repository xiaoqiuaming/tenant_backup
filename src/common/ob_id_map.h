        ...
        void traverse(Callback &cb) const
        {
          if (NULL != array_)
          {
            for (ID_TYPE i = 0; i < num_; i++)
            {
              ...
              {
                  if(array_[i].data == NULL)//[620]
                  {
                      YYSYS_LOG(WARN,"data is null.");
                  }
                cb(array_[i].id);
              }
            }
    ...
    int ObIDMap<T, ID_TYPE>::init(const ID_TYPE num)
    {
      ...
      else
      {
        ...
        for (ID_TYPE i = 0; i < (num + 1); i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "push to free list fail ret=%d i=%lu", ret, (uint64_t)i);
            break;
          }
        }
        num_ = num + 1;
    ...
    int ObIDMap<T, ID_TYPE>::assign(T *value, ID_TYPE &id)
    {
      ...
      if (NULL == array_)
      {
        ...
        ret = common::OB_NOT_INIT;
               ...
               || NULL == node)
      {
        YYSYS_LOG(WARN, "no more id free list size=%ld", free_list_.get_total());
      ...
      if (NULL != array_)
      {
                 ...
                 && ST_USING == array_[pos].stat)
        {
          ...
          if(ret == NULL) //[620]
          {
              ...
              YYSYS_LOG(WARN,"ret is null,need unlock.");
          }
        }
        else
        {
    ...
    void ObIDMap<T, ID_TYPE>::revert(const ID_TYPE id, const bool erase)
    {
      if (NULL != array_)
      {
            ...
            && ST_USING == array_[pos].stat)
        {
          ...
          {
            YYSYS_LOG(ERROR, "push to free list fail ret=%d free list size=%ld", tmp_ret, free_list_.get_total());
          }
        }
        array_[pos].lock.unlock();
      }
