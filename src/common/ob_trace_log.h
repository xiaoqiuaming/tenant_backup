...
#define FILL_TRACE_LOG(_fmt_, args...) \
  if (oceanbase::common::TraceLog::get_log_level() <= YYSYS_LOGGER._level || oceanbase::common::TraceLog::get_logbuffer().force_log_) \
{ \
...
{                                                                     \
  YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(YYSYS_LOGGER._level), "[%ld][%ld][%ld][%ld] %stotal_timeu=%ld", \
  pthread_self(), GETTID(), yysys::CLogger::get_cur_tv().tv_sec, yysys::CLogger::get_cur_tv().tv_usec, \
  oceanbase::common::TraceLog::get_logbuffer().buffer, \
  yysys::CTimeUtil::getTime() - oceanbase::common::TraceLog::get_logbuffer().start_timestamp); \
  CLEAR_TRACE_LOG();                                                  \
  }
...
{ \
  YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(oceanbase::common::TraceLog::get_log_level()), "[%ld][%ld][%ld][%ld] %stotal_timeu=%ld", \
  pthread_self(), GETTID(), yysys::CLogger::get_cur_tv().tv_sec, yysys::CLogger::get_cur_tv().tv_usec, \
  oceanbase::common::TraceLog::get_logbuffer().buffer, \
  yysys::CTimeUtil::getTime() - oceanbase::common::TraceLog::get_logbuffer().start_timestamp); \
  }\
...
#define GET_TRACE_TIMEU() (yysys::CTimeUtil::getTime() - oceanbase::common::TraceLog::get_logbuffer().start_timestamp)
#define YYSYS_TRACE_LOG(_fmt_, args...) \
  if (oceanbase::common::TraceLog::get_log_level() <= YYSYS_LOGGER._level) \
{ \
  YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(oceanbase::common::TraceLog::get_log_level()), "[%ld][%ld][%ld][%ld] " _fmt_, \
  pthread_self(), GETTID(), yysys::CLogger::get_cur_tv().tv_sec, yysys::CLogger::get_cur_tv().tv_usec, ##args); \
  }
...
#define FILL_TRACE_BUF(log_buffer, _fmt_, args...) \
  if (oceanbase::common::TraceLog::get_log_level() <= YYSYS_LOGGER._level || log_buffer.force_log_) \
{ \
...
{ \
  if (oceanbase::common::TraceLog::get_log_level() <= YYSYS_LOGGER._level || (log_buffer.force_log_ && log_buffer.force_print_)) \
{ \
  YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(oceanbase::common::TraceLog::get_log_level()), "[%ld][%ld][%ld][%ld] %stotal_timeu=%ld", \
  pthread_self(), GETTID(), yysys::CLogger::get_cur_tv().tv_sec, yysys::CLogger::get_cur_tv().tv_usec, \
  log_buffer.buffer, \
  yysys::CTimeUtil::getTime() - log_buffer.start_timestamp); \
  } \
  CLEAR_TRACE_BUF(log_buffer); \
  }
#define GET_BUF_TRACE_TIMEU(log_buffer) (yysys::CTimeUtil::getTime() - log_buffer.start_timestamp)
...
namespace oceanbase
{
  namespace common
  {
    ...
    {
      if (get_log_level() > YYSYS_LOGGER._level)
      {
        if (0 == log_buffer.prev_timestamp)
        {
          log_buffer.start_timestamp = yysys::CTimeUtil::getTime();
