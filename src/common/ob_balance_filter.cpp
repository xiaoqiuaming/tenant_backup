...
namespace oceanbase
{
  namespace common
  {
      ...
      else
      {
        ...
        else
        {
          ...
          if (dynamic_rebalance
              && 1 != start())
          {
            YYSYS_LOG(WARN, "start thread to rebalance fail");
          ...
          else
          {
            bucket_round_robin_ = 0;
            inited_ = true;
            YYSYS_LOG(INFO, "init succ, bucket_node_num=%ld bucket_nodes=%p thread_node_num=%ld thread_nodes=%p dynamic_rebalance=%s",
    ...
    void ObBalanceFilter::run(yysys::CThread* thread, void* arg)
    {
      UNUSED(thread);
      UNUSED(arg);
      SET_THD_NAME("ups-balance");
      while (!_stop)
      {
            ...
            && thread_nodes_[max_thread_pos].cnt > (thread_nodes_[min_thread_pos].cnt * 2))
        {
          ...
          {
            YYSYS_LOG(INFO, "thread=%ld cnt=%ld has only one bucket=%ld cnt=%ld, need not rebalance",
                      max_thread_pos, thread_nodes_[max_thread_pos].cnt, min_bucket_pos, bucket_nodes_[min_bucket_pos].cnt);
          }
        }
        else
        {
          YYSYS_LOG(INFO, "max thread=%ld cnt=%ld, min thread=%ld cnt=%ld, need not rebalance",
    ...
    void ObBalanceFilter::migrate(const int64_t bucket_pos, const int64_t thread_pos)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, bucket_pos=%ld thread_pos=%ld", bucket_pos, thread_pos);
      }
      else
      {
        YYSYS_LOG(INFO, "migrate bucket_pos=%ld bucket_cnt=%ld thread_pos from %ld:%ld to %ld:%ld",
    ...
    {
      YYSYS_LOG(INFO, "==========balance filter info start==========");
      for (int64_t i = 0; i < bucket_node_num_; i++)
      {
        YYSYS_LOG(INFO, "[balance_filter][bucket] bucket_pos=%ld thread_pos=%ld cnt=%ld",
                  i, bucket_nodes_[i].thread_pos, bucket_nodes_[i].cnt);
      }
      for (int64_t i = 0; i < thread_node_num_; i++)
      {
        YYSYS_LOG(INFO, "[balance_filter][thread] thread_pos=%ld cnt=%ld",
                  i, thread_nodes_[i].cnt);
      }
      YYSYS_LOG(INFO, "==========balance filter info end==========");
    }
