...
//[647]
int g_curthreadno;
thd_name_t thdname[oceanbase::common::OB_MAX_THREAD_NUM];
void g_print_thrname()
{
    UNUSED(g_curthreadno);
    UNUSED(thdname);
}
  ...
  namespace common
  {
    ...
    void *ObMemList::get()
    {
      ...
      if (NULL != ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "maybe alloc too many memory from mem_list=%p hold_num=%ld free_num=%d",
    ...
    void ObMemList::put(void *ptr, const int32_t max_free_num)
    {
      if (NULL != ptr)
      {
        ...
        {
          alloc_.free(ptr);
          YYSYS_LOG(DEBUG, "free ptr=%p", ptr);
        }
        dec_ref_cnt();
      }
      return;
    ...
    ObThreadMempool::ObThreadMempool() : fixed_size_(0), max_free_num_(0)
    ...
    void ObThreadMempool::destroy_thread_data_(void *ptr)
    {
      if (NULL != ptr)
      {
        ...
        {
          YYSYS_LOG(INFO, "delete mem_list=%p", mem_list);
          delete mem_list;
          mem_list = NULL;
        }
        else
        {
          YYSYS_LOG(INFO, "will not delete mem_list=%p ref_cnt=%ld",
    ...
    int ObThreadMempool::init(const int32_t fixed_size, const int32_t max_free_num)
    {
      ...
      if (0 >= fixed_size
          || 0 > max_free_num)
      {
        YYSYS_LOG(WARN, "invalid param fixed_size=%d max_free_num=%d", fixed_size, max_free_num);
        ret = OB_INVALID_ARGUMENT;
      }
      //mod hongchen [TSD_USER_IMPL] 20170916:b
      //else if (0 != pthread_key_create(&key_, destroy_thread_data_))
      else if (0 != PTHREAD_KEY_MGR->create_thread_key(key_))
        //mod hongchen [TSD_USER_IMPL] 20170916:e
      {
        YYSYS_LOG(WARN, "pthread_key_create fail errno=%u", errno);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObThreadMempool::destroy()
    {
      ...
      if (key_.is_valid())
      {
        ...
        ObMemList *mem_list = (ObMemList*)(PTHREAD_KEY_MGR->get_thread_specifi(key_));
        if (NULL != mem_list)
        {
          if (0 == mem_list->dec_ref_cnt())
          {
      ...
      if (key_.is_valid())
      {
        ...
        if (NULL == mem_list)
        {
          ...
          if (NULL != mem_list)
          {
            ...
            if (0 != PTHREAD_KEY_MGR->set_thread_specifi(key_, mem_list))
            {
              //YYSYS_LOG(WARN, "pthread_setspecific fail errno=%u key=%d", errno, key_);
              YYSYS_LOG(WARN, "pthread_setspecific fail key=%ld", key_.index_);
              delete mem_list;
              mem_list = NULL;
            }
            else
            {
              YYSYS_LOG(INFO, "new mem_list=%p", mem_list);
    ...
    void ObThreadMempool::free(void *ptr)
    {
      if (NULL != ptr)
      {
        ...
        if (NULL == mem_list)
        {
          YYSYS_LOG(ERROR, "mem_list null pointer cannot free ptr=%p", ptr);
        }
        else
        {
    ...
    void ObThreadMempool::set_max_free_num(const int32_t max_free_num)
    {
      if (0 > max_free_num)
      {
        YYSYS_LOG(WARN, "invlaid max_free_num=%d", max_free_num);
      }
      else
      {
        YYSYS_LOG(INFO, "set max_free_num from %d to %d", max_free_num_, max_free_num);
        max_free_num_ = max_free_num;
      }
    }
