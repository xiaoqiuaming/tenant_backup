                                      ...
                                      const int64_t data_len, int64_t &pos, ObCellInfo &cell_info, ObObj &last_obj)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid arguments, buf=%p data_len=%ld pos=%ld", buf, data_len, pos);
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "deserialize error, ret=%d buf=%p data_len=%ld next_pos=%ld",
    ...
    else
    {
      if (ObExtendType == last_obj.get_type() && ObActionFlag::TABLE_NAME_FIELD == last_obj.get_ext())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "deserialize_int_or_varchar_ error, ret=%d", ret);
        }
        else
        {
          if (cur_table_name_.length() > 0 && NULL != cur_table_name_.ptr())
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN, "Inconsistent ID/NAME type, current=NAME before=ID");
            }
            else
            {
              id_name_type_ = NAME;
          ...
          else if (OB_INVALID_ID != cur_table_id_)
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN, "Inconsistent ID/NAME type, current=ID before=NAME");
            }
            else
            {
              id_name_type_ = ID;
          ...
          {
            YYSYS_LOG(WARN, "Table Name and Table ID are both invalid when deserializing");
          }
        }
    ...
    if (OB_SUCCESS == ret)
    {
              ...
              || ObActionFlag::FORMED_ROW_KEY_FIELD == last_obj.get_ext()))
      {
        ...
        if (ObActionFlag::ROW_KEY_FIELD == last_obj.get_ext())
        {
          ...
          {
            YYSYS_LOG(WARN, "deserialize binray rowkey, but not set compatibility schema,"
          ...
          {
            YYSYS_LOG(WARN, "cannot translate binary rowkey to ObRowkey, rowkey info size=%ld",
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "deserialize rowkey, ret=%d, last_obj type=%ld", ret, last_obj.get_ext());
        }
        else
        {
          if (size == 0)
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else if (ObIntType == last_obj.get_type() || ObVarcharType == last_obj.get_type())
      {
        if (ObIntType == last_obj.get_type())
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "deserialize column id error, ret=%d buf=%p data_len=%ld next_pos=%ld",
                      ret, buf, data_len, next_pos);
          }
          /*
          else if (OB_INVALID_ID == cell_info.column_id_)
          {
            YYSYS_LOG(WARN, "deserialized column id is invalid");
            ret = OB_ERROR;
          }
          */
          else
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "deserialize column name error, ret=%d buf=%p data_len=%ld next_pos=%ld",
                      ret, buf, data_len, next_pos);
          }
          else if (NULL == cell_info.column_name_.ptr() || 0 == cell_info.column_name_.length())
          {
            YYSYS_LOG(WARN, "deserialized column name is invalid");
          }
          else
          {
            if (ID == id_name_type_)
  ...
  cur_cell_info_(), new_row_cell_(false), is_end_(false)
{
  if (!is_iterator_end_())
  {
    ...
    {
      YYSYS_LOG(ERROR, "get cell fail next_pos=%ld cur_pos=%ld", next_pos_, cur_pos_);
    }
  }
}
...
ObScanner::Iterator &ObScanner::Iterator::operator ++ ()
{
  ...
  {
    YYSYS_LOG(ERROR, "get cell fail next_pos=%ld cur_pos=%ld", next_pos, cur_pos_);
  }
  else
  {
    cur_pos_ = next_pos;
                                   ...
                                   ObCellInfo &cell_info, int64_t &next_pos)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "read cell failed:ret[%d], len[%ld], pos[%ld]", ret, data_len, next_pos);
      is_end_ = true;
    }
    else
    {
...
ObScanner::RowIterator &ObScanner::RowIterator::operator ++ ()
{
  ...
  {
    YYSYS_LOG(ERROR, "get_row_ returned error, err=%d", err);
  }
  return *this;
}
...
int ObScanner::RowIterator::get_row_()
{
  ...
  if (OB_SUCCESS != err && OB_ITER_END != err)
  {
    YYSYS_LOG(ERROR, "ObScanner::Iterator get_cell returned error, "
              "err=%d cell=%p row_changed=%s",
              err, cell, row_changed?"true":"false");
  }
  else if (OB_ITER_END == err)
  {
    YYSYS_LOG(DEBUG, "ObScanner::Iterator get_cell returned iteration end");
    err = OB_SUCCESS;
  }
  else
  {
    ...
    while (!row_changed)
    {
      ...
      else if (!row_changed)
      {
        ...
        //if (OB_SUCCESS != err)
        //{
        //  YYSYS_LOG(ERROR, "get_cell_ error, err=%d next_pos_=%ld",
        //      err, next_pos_);
        //}
        //else
        //{
  ...
  mod_(ObModIds::OB_SCANNER), rowkey_allocator_(ModuleArena::DEFAULT_PAGE_SIZE, mod_)
{
  ...
  scanner_type_ = common::OB_SCANNER_TYPE;
  //add 20151228:e
  // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
  index_has_virtual_ =false;
  need_delete_ =false;
  is_serialize_column_ = false;
                        ...
                        const bool is_row_changed)
{
  ...
  if (NULL == cur_memblock_)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get_memblock_ error, ret=%d", ret);
    }
  }
  else
  {
  ...
  if (OB_SUCCESS == ret && !has_add)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "exceed the memory limit");
    }
    else if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "append_serialize_ error, cell_info=%s, ret=%d", print_cellinfo(&cell_info), ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      if (OB_INVALID_ID == cur_table_id_ && cur_table_name_.ptr() == NULL)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to serialize null_row_obj, buf=%p, buf_len=%ld, pos=%ld, ret=%d",
                    buf, buf_len, pos, ret);
        }
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to serialize rowkey, table_id=%lu, table_name=%s, last_row_key=%s, ret=%d",
...
int ObScanner::serialize_table_param(char * buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  if (cur_size_counter_ > 0)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (NULL != iter)
      {
        YYSYS_LOG(WARN, "ObScanner buffer to serialize is not enough, buf_len=%ld", buf_len);
                                                 ...
                                                 const uint64_t table_id, const ObString & table_name, const ObRowkey& rowkey) const
{
  ...
  {
    YYSYS_LOG(WARN, "cannot get rowkey of binary rowkey table_id=%lu, table_name=%.*s",
  ...
  if (OB_SUCCESS == ret && NULL != buffer)
  {
    ...
    else
    {
      ...
      if (0 != ret)
      {
        YYSYS_LOG(WARN, "failed to translate obj array to binary rowkey, rowkey=%s, ret=%d", to_cstring(rowkey), ret);
      }
    }
  }
...
//int ObScanner::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "serialize basic param error, err=%d", ret);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "serialize meta param error, err=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "serialize table param error, err=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "serialize end param error, err=%d", ret);
    }
    else
    {
      pos = new_pos;
  ...
  else
  {
    ...
    while (OB_SUCCESS == ret && new_pos <= data_len && !is_end)
    {
      ...
      while (OB_SUCCESS == ret && ObExtendType != obj_type)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "ObObj deserialize error, len=%ld, pos=%ld, ret=%d", data_len, new_pos, ret);
        }
        else
        {
          obj_type = param_id.get_type();
        ...
        else
        {
          switch (param_id_type)
          {
            ...
            case ObActionFlag::META_PARAM_FIELD:
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "deserialize meta info error, ret=%d buf=%p data_len=%ld new_pos=%ld",
                          ret, buf, data_len, new_pos);
              }
              break;
                                 ...
                                 const bool is_row_changed)
{
  ...
  // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
  bool is_addrowkey = false;
  is_addrowkey = is_add_rowkey(cell_info.column_id_);
  // add e

  if (is_addrowkey && rowkeys_add_.at(0) == cell_info.column_id_)
  {
    need_delete_ = false;
  }
  //  //add hongchen [SERIVILIZE_LOG_SUPRESS] 20170813:b
  //  if (cur_size_counter_ + DEFAULT_RESERVED_SIZE > mem_size_limit_)
  //  {
  //    ret = OB_SIZE_OVERFLOW;
  //  }
  //add hongchen [SERIVILIZE_LOG_SUPRESS] 20170813:e
  ...
  if (OB_SUCCESS == ret)
  {
        ...
        || cell_info.table_id_ != cur_table_id_)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "serialize_table_name_or_id_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
                  ret, buf, buf_len, pos);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
        ...
        || (is_compare_rowkey && cell_info.row_key_ != cur_row_key_))
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "serialize_row_key_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
                  ret, buf, buf_len, pos);
      }
      need_delete_ =false;    // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
      is_serialize_column_ = false;
    }
  }
  if (OB_SUCCESS == ret)
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(need_delete_)
    {
              ...
              || ObActionFlag::OP_DEL_TABLE == cell_info.value_.get_ext()))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "ObObj serialize error, ret=%d buf=%p buf_len=%ld pos=%ld",
                    ret, buf, buf_len, pos);
        }
      }
    // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
      else if(rowkey_info_.is_rowkey_column(cell_info.column_id_))
      {
        int64_t index= OB_INVALID_ID;
        ObRowkeyColumn rowkey_column;
        if(OB_SUCCESS == rowkey_info_.get_index(cell_info.column_id_,index,rowkey_column))
        {
          if(cell_info.row_key_.get_obj_ptr()[index]!= cell_info.value_)
          {
            need_delete_ = true;
            is_serialize_column_ = false;
            YYSYS_LOG(DEBUG,"the row is delete ,row key = %s,column_id = %lu,value= %s",
                      to_cstring(cur_row_key_),cell_info.column_id_,to_cstring(cell_info.value_));
            rollback();
            char* buf = cur_memblock_->memory;
            const int64_t buf_len = cur_memblock_->memory_size;
            pos = cur_memblock_->cur_pos;
            // table_id
            cur_memblock_->rollback_pos = cur_memblock_->cur_pos;
            rollback_memblock_ = cur_memblock_;
            rollback_size_counter_ = cur_size_counter_;
            ...
            if(OB_SUCCESS == ret )
            {
              ...
              //rowkey
              if(OB_SUCCESS !=(ret = serialize_row_key_(buf, buf_len, pos, cell_info)))
              {
                YYSYS_LOG(WARN, "serialize_row_key error, ret=%d buf=%p buf_len=%ld pos=%ld",
                          ret, buf, buf_len, pos);
              }
              else if(OB_SUCCESS !=(ret = obj.serialize(buf, buf_len, pos)))
              {
                YYSYS_LOG(WARN, "serialize action flag error, ret=%d buf=%p buf_len=%ld pos=%ld",
                          ret, buf, buf_len, pos);
              }
              else
              {
        ...
        if(!need_delete_ && !is_addrowkey)
        {
                      ...
                      || ObObj::MAX_OBJECT_VALUE == cell_info.value_.get_ext())))
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(WARN, "serialize_column_ error, ret=%d buf=%p buf_len=%ld pos=%ld",
                        ret, buf, buf_len, pos);
            }
            else
            {
              is_serialize_column_ = true;
            }
          }
        }
        //�ⲿ�ֵ�Ӱ����Ҫ�����������Ӷ������������е����
        if(index_has_virtual_ && is_serialize_row && !need_delete_ && !is_serialize_column_)//ֻ��������
        {
    ...
    // add by maosy e
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "ObCellInfo value extend type is invalid, Type=%d",
  ...
  if (OB_SUCCESS == ret)
  {
    if (0 < rowkey_info_.get_size()
        && 0 < rowkeys_add_.count()
        && !need_delete_
        && !is_serialize_column_
        && rowkeys_add_.at(rowkeys_add_.count() - 1) == cell_info.column_id_)
    {
      obj.set_ext(ObActionFlag::OP_NOP);
      ret = obj.serialize(buf, buf_len, pos);
      if (OB_SUCCESS != ret)
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
      //ֻ�������е�����£�����Ҫ�ߵ������֧
      if(!need_delete_ && !is_addrowkey)//����Ҫɾ�����Ҳ��������ӵ�ʱ���������cell�ĸ���
      {
        // add by maosy e
        ++cur_cell_num_;
      }
                                           ...
                                           const ObCellInfo &cell_info)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "table_name and table_id both exist, table_name=%.*s table_id=%lu",
    ...
    else
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (cell_info.table_name_.length() > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "ObObj deserialize error, ret=%d buf=%p pos=%ld cur_pos=%ld",
                      ret, buf, pos, cur_pos);
          }
          else
          {
...
int ObScanner::deserialize_table_param(const char* buf, const int64_t data_len, int64_t& pos, ObObj &last_obj)
{
  ...
  {
    YYSYS_LOG(WARN, "get valid data size fail buf=%p data_len=%ld pos=%ld", buf, data_len, pos);
    ret = OB_ERROR;
  }
  else
  {
...
int ObScanner::set_is_req_fullfilled(const bool &is_fullfilled, const int64_t fullfilled_item_num)
{
  ...
  {
    YYSYS_LOG(WARN,"param error [fullfilled_item_num:%ld]", fullfilled_item_num);
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
...
void ObScanner::dump(void) const
{
  ...
  {
    YYSYS_LOG(INFO, "[dump] last_row_key_=%s", to_cstring(last_row_key_));
  }
  else
  {
    YYSYS_LOG(INFO, "[dump] no last_row_key_ found!");
  }

  YYSYS_LOG(INFO, "[dump] is_request_fullfilled_=%d", is_request_fullfilled_);
  YYSYS_LOG(INFO, "[dump] fullfilled_item_num_=%ld", fullfilled_item_num_);
  YYSYS_LOG(INFO, "[dump] cell_num_=%ld", cur_cell_num_);
  YYSYS_LOG(INFO, "[dump] row_num_=%ld", cur_row_num_);
  YYSYS_LOG(INFO, "[dump] whole_result_row_num_=%ld", whole_result_row_num_);
...
void ObScanner::dump_all(int log_level) const
{
  ...
  if (OB_SUCCESS == err && log_level >= YYSYS_LOG_LEVEL_DEBUG)
  {
          ...
          (OB_SUCCESS == (err = iter.get_cell(&cell, &row_change))))
    {
      ...
      if (row_change)
      {
        YYSYS_LOG(INFO, "dump cellinfo rowkey :%s", to_cstring(cell->row_key_));//test::lmz DEBUG -> INFO
      }
      YYSYS_LOG(INFO, "cell->value:%s", to_cstring(cell->value_));//test::lmz DEBUG -> INFO
      ++iter;
    }
    if (err != OB_SUCCESS)
    {
