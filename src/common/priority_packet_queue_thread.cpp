...
namespace oceanbase
{
  namespace common
  {
    ...
    // ����
    PriorityPacketQueueThread::PriorityPacketQueueThread() : yysys::CDefaultRunnable()
    {
      _stop = 0;
      _waitFinish = false;
      _handler = NULL;
    ...
    bool PriorityPacketQueueThread::push(ObPacket *packet, int maxQueueLen, bool block, int priority)
    {
      ...
      if (NULL == packet)
      {
        YYSYS_LOG(WARN, "packet is NULL");
        return false;
      }
      else if (priority != NORMAL_PRIV && priority != LOW_PRIV)
      {
        YYSYS_LOG(WARN, "invalid param, priority=%d", priority);
        return false;
      }
    ...
    // Runnable �ӿ�
    void PriorityPacketQueueThread::run(yysys::CThread *, void *)
    {
      ObPacket *packet = NULL;
      int64_t priority = -1;
      static const int64_t TASK_WAIT_TIME = 1; // wait 1ms if there is no task
      SET_THD_NAME("ups-read");
