...
#include "ob_schema.h"    // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
...
namespace oceanbase
{
  namespace common
  {
    ...
    class ObScanner : public ObIterator, public ObInnerIterator
    {
        ...
        static const int64_t DEFAULT_RESERVED_SIZE = 128;
      ...
      public:
        ...
        // RowIteratorʵ����һ��ǰ����裬�ڶ�ȡcell��ʱ�����������Memblock��
        // ��һ�����ݺ������Memblockʱ������쳣��
        // ĿǰObScanner��ʵ�ֲ�����������⣬��Ϊÿ�������Mmeblock����2MB��
        // ����ObScanner��ǰ�ڴ����������2MB
      ...
      public:
        ...
        {
          YYSYS_LOG(ERROR,"shouldn't be here");
          UNUSED(has_row_version);
          //do nothing
        }
        //add 20151228:e
        // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
        inline void set_index_has_storing()
        {
          index_has_virtual_ = true;
        }
        void set_rowkey_info( ObRowkeyInfo rowkey)
        {
          rowkey_info_= rowkey ;
          YYSYS_LOG(DEBUG,"TEST=%s",to_cstring(rowkey_info_));
        }
        void set_add_rowkey(uint64_t column_id)
        {
          rowkeys_add_.push_back(column_id);
        }
        bool is_add_rowkey(uint64_t column_id)
        {
          bool ret = false;
          int64_t count = rowkeys_add_.count();
          for(int i = 0 ; i < count ;i++)
          {
            if(rowkeys_add_.at(i)== column_id)
            {
              ret = true ;
              break;
            }
      ...
      private:
        ...
        //add 20151228:e
        // add by maosy [MultiUps 1.0] [secondary index optimize]20170605 b:
        bool index_has_virtual_;
        ObRowkeyInfo rowkey_info_;
        bool need_delete_;
        bool is_serialize_column_;
        ObSEArray<uint64_t,OB_MAX_ROWKEY_COLUMN_NUMBER> rowkeys_add_ ;
        // add by maosy e
    };
