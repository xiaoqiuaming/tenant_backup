...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::deep_copy(const bloom_filter_t &other)
    {
      ...
      {
        YYSYS_LOG(WARN, "init fail ret=%d", ret);
      }
      else
      {
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::init(const int64_t nhash, const int64_t nbit)
    {
          ...
          || 0 >= nbit)
      {
        ...
        YYSYS_LOG(WARN, "invalid param nhash=%ld nbit=%ld", nhash, nbit);
      }
      else
      {
        destroy();
        ...
        if (NULL == bits_)
        {
          YYSYS_LOG(WARN, "bits_ null pointer, nbit_=%ld", nbit_);
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::insert(const T &element)
    {
      ...
      if (NULL == bits_
          ...
          || 0 >= nhash_)
      {
        YYSYS_LOG(WARN, "maybe have not inited bits_=%p nbit_=%ld nhash_=%ld", bits_, nbit_, nhash_);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param buf=%p buf_len=%ld pos=%ld serialize_size=%ld",
      ...
      else
      {
        if (NULL == bits_
            ...
            || 0 >= nhash_)
        {
          YYSYS_LOG(WARN, "maybe have not inited bits_=%p nbit_=%ld", bits_, nbit_);
        }
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::deserialize(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param buf=%p buf_len=%ld pos=%ld bin_data_size=%ld",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid param buf=%p buf_len=%ld pos=%ld total_data_size=%ld",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "alloc bits_ fail nbyte=%ld", nbyte);
            ret = OB_ERROR;
          }
          else
          {
    ...
    int ObBloomFilter<T, HashFunc, Alloc>::operator | (const bloom_filter_t& other)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "bloom_filter do not match");
      }
        ...
        const int64_t nbyte)
    {
      ...
      if (nbyte * 8 != nbit_)
      {
        YYSYS_LOG(WARN, "the buffer size is not equal:nbyte*8=%ld,nbit_=%ld",
            nbyte*8, nbit_);
        ret = OB_ERROR;
      }
      else
