...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObRingBuffer::clear()
    {
      ...
      if (is_init_)
      {
        ...
        while (1)
        {
          ...
          if (NULL == block)
          {
            YYSYS_LOG(ERROR, "the %ld-th block is NULL", idx);
          }
          else
          {
    ...
    int ObRingBuffer::init()
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to alloc block from system, err=%d, block=%p", err, block);
        err = OB_ERROR;
      }
      else
      {
    ...
    int ObRingBuffer::push_task(const void* ptr, const int64_t task_len, void*& ret_ptr)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, ptr=%p, task_len=%ld", ptr, task_len);
        err = OB_INVALID_ARGUMENT;
      }
      else if (!is_init_)
      {
        YYSYS_LOG(WARN, "not init");
        err = OB_NOT_INIT;
      }
      else
      {
        ...
        if (NULL == block)
        {
          YYSYS_LOG(ERROR, "the %ld-th block is NULL", queue_.ring_tail_idx);
          err = OB_ERROR;
        }
        else if (block->data_size + size > block->block_size)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to add block, err=%d", err);
          }
          else
          {
            block = queue_.blocks[queue_.ring_tail_idx];
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(ERROR, "too large task, data_size=%ld, task_size=%ld, block_size=%ld",
    ...
    int ObRingBuffer::pop_task(const void* ptr)
    {
      ...
      if (NULL == ptr)
      {
        YYSYS_LOG(WARN, "invalid param, ptr=%p", ptr);
        err = OB_INVALID_ARGUMENT;
      }
      else if (!is_init_)
      {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "invalid task magic, expected=%u, real=%u",
        ...
        else
        {
          ...
          if (NULL == block)
          {
            YYSYS_LOG(ERROR, "block is NULL");
            err = OB_ERROR;
          }
          else if (block->magic != BLOCK_MAGIC_NUM)
          {
            YYSYS_LOG(ERROR, "invalid block magic, expected=%u, real=%u",
                BLOCK_MAGIC_NUM, block->magic);
            err = OB_ERROR;
          }
          else if (block->task_num <= 0)
          {
            YYSYS_LOG(ERROR, "the block has no task, task_num=%d", block->task_num);
            err = OB_ERROR;
          }
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "invalid status, next_task_offset=%ld, data_size=%ld",
            ...
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "Pop an invalid task, pop_offset=%ld, next_read_offset_=%ld",
    ...
    int ObRingBuffer::add_a_block_()
    {
      ...
      {
        YYSYS_LOG(ERROR, "the ring buffer is full");
      ...
      else
      {
        ...
        if (OB_SUCCESS != err || NULL == block)
        {
          YYSYS_LOG(WARN, "failed to alloc block from system, err=%d", err);
          err = OB_ERROR;
        }
      }
    ...
    int ObRingBuffer::alloc_a_block_(RingBlock*& block)
    {
      ...
      if (NULL == ptr)
      {
        YYSYS_LOG(WARN, "not enough memory");
        err = OB_MEM_OVERFLOW;
      }
      else
      {
    ...
    int ObRingBuffer::get_next_preprocess_task(char* buf, const int64_t buf_len, int64_t& task_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, buf=%p, buf_len=%ld", buf, buf_len);
        err = OB_INVALID_ARGUMENT;
      }
      else if (!is_init_)
      {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get next offset, err=%d", err);
          }
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get next offset, ori_offset=%ld, err=%d", ori_offset, err);
            err = OB_ERROR;
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "buf is not enough, task_size=%d, buf_len=%ld", next_task->size, buf_len);
              err = OB_NEED_RETRY;
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid offset or block reused, new_offset=%ld, err=%d", new_offset, err);
              }
            }
          }
        }
    ...
    int ObRingBuffer::get_next_offset_(const int64_t offset, int64_t& next_offset)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "maybe the block is re-used, block=%p, version=%ld, expected_version=%ld",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid magic, expected=%u, real=%u", TASK_MAGIC_NUM, cur_task->magic);
            err = OB_NEED_RETRY;
          }
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "offset overflow, task_size=%ld, task_offset=%ld, block_data_size=%ld",
    ...
    int ObRingBuffer::translate_offset_(int64_t& offset, RingTask*& ret_next_task)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "preprocess too fast, offset=%ld, last_write_offset=%ld",
    ...
    int ObRingBuffer::is_offset_valid_(const int64_t offset)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid offset, offset=%ld, next_read_offset=%ld, last_write_offset=%ld",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid queue_idx or version, queue_idx=%ld, block=%p, "
                "version=%ld, expected_version=%ld", queue_idx, block, version, expected_version);
            err = OB_NEED_RETRY;
          }
          else if (task_offset > block->data_size)
          {
            YYSYS_LOG(WARN, "invalid offset, task_offset=%ld, data_size=%ld",
