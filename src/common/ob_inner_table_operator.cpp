...
//mod:e
{
  ...
  if ((sql.ptr() == NULL) || (0 == sql.size()))
  {
    ...
    YYSYS_LOG(WARN, "Check SQL buffer size failed! ptr[%p], size[%d]", sql.ptr(), sql.size());
  }
  else
  {
    ...
    else
    {
      ...
      if (size >= sql.size())
      {
        YYSYS_LOG(ERROR, "SQL buffer size not enough! size: [%d], need: [%d], sql: [%.*s]",
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "XXX=%.*s", sql.length(), sql.ptr());
      }
    }
  }
  return ret;
