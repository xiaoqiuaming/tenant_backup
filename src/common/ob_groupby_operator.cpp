...
namespace oceanbase
{
  namespace common
  {
    ...
    void ObGroupByOperator::clear()
    {
      ...
      {
        YYSYS_LOG(WARN, "clear groupby cellarray, memory_size_water_mark=%ld, "
                                ...
                                const int64_t max_groupby_mem_size)
    {
      ...
      if (!inited_)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to create hash table");
        }
        else
        {
          inited_ = true;
      ...
      if (NULL != param_)
      {
        ...
        if (comp_column_size > 0)
        {
          ...
          for (int64_t i = 0; i < comp_column_size && OB_SUCCESS == err; ++i)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "fail to append fake composite column cell [err:%d]", err);
            }
          }
        }
      }
        ...
        const ObRowkey& rowkey, const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN,"aggregate function must act all records and result must be empty "
           ...
           i++)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to init aggregate object [err:%d, idx:%ld]", err, i);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to init aggregate group key [err:%d]", err);
        }
      }
      if (OB_SUCCESS == err)
      {
                                   ...
                                   const int64_t row_beg, const int64_t row_end)
    {
      ...
      if (NULL == param_)
      {
        YYSYS_LOG(WARN,"initialize first [param_:%p]", param_);
        err = OB_INVALID_ARGUMENT;
      }
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"param error [row_beg:%ld,row_end:%ld,org_cells.get_cell_size():%ld]",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to initialize group key [err:%d]", err);
          }
          if (OB_SUCCESS == err)
          {
            ...
            if (HASH_EXIST == hash_err)
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN,"fail to aggregate org row [err:%d]", err);
              }
            }
            else
            {
        ...
        const ObCellArray & agg_cells, const int64_t row_beg, const int64_t row_end)
    {
      ...
      {
        YYSYS_LOG(WARN,"param error [row_beg:%ld,row_end:%ld,agg_cells.get_cell_size():%ld]",
      ...
      else
      {
        ...
        for (int64_t i = row_beg; i <= row_end && OB_SUCCESS == ret; i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN,"fail to append cell to cell array [err:%d]", ret);
          }
        }
        ...
        if (OB_SUCCESS == ret && param_->get_aggregate_row_width() > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN,"fail to init aggregate group key [ret:%d]", ret);
          }
          else
          {
            hash_err = group_hash_map_.set(agg_key, agg_row_beg);
        ...
        const ObCellArray& org_cells, const int64_t row_width)
    {
      ...
      {
        YYSYS_LOG(WARN, "orgin cell array exclude topk heap info, "
