  ...
  namespace common
  {
    ...
    class ObGlobalFreeList
    {
      ...
      public:
        ...
        {
          ObRecursiveMutexGuard guard(lock_);
          YYSYS_LOG(INFO, "[GFACTORY_STAT] freelist next_cache_line=%d", next_cache_line_);
          YYSYS_LOG(INFO, "[GFACTORY_STAT] freelist empty_blocks_num=%d", empty_blocks_.get_size());
          YYSYS_LOG(INFO, "[GFACTORY_STAT] freelist nonempty_blocks_num=%d", nonempty_blocks_.get_size());
          dlist_for_each(ObTCBlock, blk, nonempty_blocks_)
          {
            const int32_t obj_size = blk->get_obj_size();
            YYSYS_LOG(INFO, "[GFACTORY_STAT] nonempty_block, obj_size=%d in_use_count=%d total=%d",
    ...
    int ObGlobalFactory<T, MAX_CLASS_NUM, MODID>::register_class(int32_t type_id, create_method_t creator, int32_t obj_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid type_id=%d class_num=%ld", type_id, MAX_CLASS_NUM);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == creator || 0 >= obj_size)
      {
        YYSYS_LOG(WARN, "invalid arguments, creator=%p obj_size=%d", creator, obj_size);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL != create_methods_[type_id]
               || 0 != obj_size_[type_id])
      {
        YYSYS_LOG(WARN, "class already registered, type_id=%d", type_id);
        ret = OB_INIT_TWICE;
      }
      else
      {
    ...
    int ObGlobalFactory<T, MAX_CLASS_NUM, MODID>::init()
    {
      ...
      for (int32_t type_id = 0; type_id < MAX_CLASS_NUM; ++type_id)
      {
        ...
        {
          //YYSYS_LOG(WARN, "class not registered, type_id=%d", type_id);
        }
        else
        {
    ...
    ObGlobalFactory<T, MAX_CLASS_NUM, MODID>* ObGlobalFactory<T, MAX_CLASS_NUM, MODID>::get_instance()
    {
      if (NULL == INSTANCE)
      {
        ...
        if (NULL == INSTANCE)
        {
          YYSYS_LOG(ERROR, "failed to create singleton instance");
          abort();
        }
      }
      return INSTANCE;
    ...
    void ObGlobalFactory<T, MAX_CLASS_NUM, MODID>::stat()
    {
      for (int32_t type_id = 0; type_id < MAX_CLASS_NUM; ++type_id)
      {
        if (NULL != create_methods_[type_id])
        {
          YYSYS_LOG(INFO, "[GFACTORY_STAT] type_id=%d obj_size=%d batch_count=%d creator=%p",
