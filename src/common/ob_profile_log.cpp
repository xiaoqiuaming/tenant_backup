...
ObProfileLogger::ObProfileLogger()
  :log_dir_(strdup("./")), log_filename_(strdup("")), log_level_(INFO), log_fd_(2)
{
}
...
void ObProfileLogger::printlog(LogLevel level, const char *file, int line, const char *function, pthread_t tid, const char *fmt,...)
{
  ...
  ssize_t size = static_cast<ssize_t>(snprintf(buf,
                                               256,
                                               "[%04d-%02d-%02d %02d:%02d:%02d] %-5s %s (%s:%d) [%ld] trace_id=[%lu] source_chid=[%u] chid=[%u]",
                                               tm.tm_year+1900,
                                               tm.tm_mon+1,
                                               tm.tm_mday,
                                               tm.tm_hour,
    ...
    const int64_t consume, const char *fmt, ...)
{
  ...
  ssize_t size = static_cast<ssize_t>(snprintf(buf,
                                               256,
                                               "[%04d-%02d-%02d %02d:%02d:%02d] %-5s %s (%s:%d) [%ld] trace_id=[%lu] source_chid=[%u] chid=[%u], consume=[%ld] ",
                                               tm.tm_year+1900,
                                               tm.tm_mon+1,
                                               tm.tm_mday,
                                               tm.tm_hour,
