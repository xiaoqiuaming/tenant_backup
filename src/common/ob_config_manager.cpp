...
int ObConfigManager::base_init()
{
  ...
  {
    YYSYS_LOG(ERROR, "init system config, ret: [%d]", ret);
  }
  else if (OB_SUCCESS != (ret = server_config_.init(system_config_)))
  {
    YYSYS_LOG(ERROR, "init server config, ret: [%d]", ret);
  }
...
int ObConfigManager::reload_config()
{
  ...
  {
    YYSYS_LOG(WARN, "Check configuration failed, can't reload. ret: [%d]", ret);
  }
...
//modify for [491-check parameters when rs reboot]-b
//int ObConfigManager::load_config(const char *path)
int ObConfigManager::load_config(const char *path, bool check_exist)
//modify for [491-check parameters when rs reboot]-e
{
  int ret = OB_SUCCESS;
  FILE *fp = NULL;
  struct stat buf;
  ...
  if (0 != stat(path, &buf))
  {
    YYSYS_LOG(INFO, "No config file, read from command line! path: [%s]", path);
    ret = OB_FILE_NOT_EXIST;
  }
  else
  {
    YYSYS_LOG(INFO, "Using config file: [%s]", path);
  }

  if (OB_FILE_NOT_EXIST == ret)
  {
    YYSYS_LOG(INFO, "Config file doesn't exist, using command line options!");
...
//    ret = OB_SUCCESS; //delete for [491-check parameters when rs reboot]
  }
  else if ((fp = fopen(path, "rb")) == NULL)
  {
    YYSYS_LOG(ERROR, "Can't open file: [%s]", path);
    ret = OB_IO_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "Read config file error! file: [%s]", path);
      ret = OB_IO_ERROR;
    }
    else if (feof(fp) == 0)     /* not end of file */
    {
      YYSYS_LOG(ERROR, "Config file is too long! file: [%s]", path);
      ret = OB_BUF_NOT_ENOUGH;
    }
    else if(OB_SUCCESS != (ret = server_config_.deserialize(buf, len, pos)))
    {
      YYSYS_LOG(ERROR, "Deserialize server config failed! file: [%s], ret: [%d]", path, ret);
    }
    else if (pos != len)
    {
      YYSYS_LOG(ERROR, "Deserialize server config failed! file: [%s], ret: [%d]", path, ret);
    ...
    if (0 != fclose(fp))
    {
      YYSYS_LOG(ERROR, "Close config file failed!");
      ret = OB_IO_ERROR;
    }
  }
...
int ObConfigManager::dump2file(const char* path) const
{
  ...
  {
    YYSYS_LOG(WARN, "NO dump path specified!");
  ...
  else
  {
    ...
    if (fd <= 0)
    {
      YYSYS_LOG(WARN, "fail to create config file [%s], msg: [%s]",
                path, strerror(errno));
      ret = OB_ERROR;
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "Serialize server config fail! ret: [%d]", ret);
      }
      else if (pos != (size = write(fd, buf, pos)))
      {
        YYSYS_LOG(WARN, "Write server config fail!");
        ret = OB_IO_ERROR;
      }
      else
      {
        YYSYS_LOG(INFO, "Write server config successfully!");
      }
    }
    if (0 != close(fd))
    {
      YYSYS_LOG(WARN, "fail to close file fd: [%d]", fd);
    }
  }
  return ret;
}
...
void ObConfigManager::get_ms(ObServer &ms_server)
{
  ...
  else if (NULL != ms_provider_)
  {
    ...
    {
        //modify for [582-get local cluster ms]-b
//       ms_server = mslist_->get_one();
        ms_server = mslist_->get_cluster_one();
        //modify for [582-get local cluster ms]-e
    }
    //add e
  }
...
int ObConfigManager::update_local()
{
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      if (NULL == dump_path_)
      {
        YYSYS_LOG(ERROR, "Dump path doesn't set, stop read config.");
        ret = OB_NOT_INIT;
      }
      else if (OB_SUCCESS != (ret = server_config_.read_config()))
      {
        YYSYS_LOG(ERROR, "Read server config failed, ret: [%d]", ret);
      }
      ...
      {
        YYSYS_LOG(ERROR, "Dump to file failed!"
                  " path: [%s], ret: [%d]", dump_path_, ret);
      }
      else
      {
        YYSYS_LOG(INFO, "Reload server config successfully!");
      }
      server_config_.print();
    }
    else
    {
      YYSYS_LOG(WARN, "Read system config from inner table error, ret: [%d]", ret);
    }
  }
  return ret;
}
...
int ObConfigManager::got_version(int64_t version)
{
  ...
  {
    /* do nothing */
    YYSYS_LOG(DEBUG, "root server restarting");
  }
  ...
  {
    YYSYS_LOG(WARN, "Local config is newer than rs, weird!"
  ...
  else if (version > current_version_)
  {
    if (!mutex_.tryLock())
    {
      YYSYS_LOG(DEBUG, "Processed by other thread!");
    }
    else
    {
      if (version > newest_version_)
      {
        YYSYS_LOG(INFO, "Got new config version! local: [%ld], newest: [%ld], got: [%ld]",
      ...
      {
        YYSYS_LOG(WARN, "Receive weird config version! local: [%ld], newest: [%ld], got: [%ld]",
                  current_version_, newest_version_, version);
        /* impossible */
      }
      mutex_.unlock();
  ...
  if (schedule_task)
  {
    ...
    update_task_.scheduled_time_ = yysys::CTimeUtil::getMonotonicTime();
    if (NULL == timer_)
    {
      YYSYS_LOG(ERROR, "Couldn't update config because timer is NULL!");
      ret = OB_NOT_INIT;
    }
    else if (OB_SUCCESS != (ret = timer_->schedule(update_task_, 0, false)))
    {
      YYSYS_LOG(ERROR, "Update local config failed! ret: [%d]", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "Schedule update config task successfully!");
    }
  }
  return ret;
}
...
int ObConfigManager::UpdateTask::write2stat()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "config mgr is NULL, ret: [%d]", ret);
  }
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      YYSYS_LOG(INFO, "update config sys stat successfully!");
    }
  }
...
void ObConfigManager::UpdateTask::runTimerTask()
{
  ...
  if (update_local_)
  {
    ...
    {
      YYSYS_LOG(WARN, "Update local config failed! ret: [%d]", ret);
    }
    else if (OB_SUCCESS != (ret = write2stat()))
    {
      YYSYS_LOG(WARN, "update __all_sys_config_stat failed, ret: [%d]", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "update __all_sys_config_stat successfully");
    ...
    {
      YYSYS_LOG(ERROR, "Reschedule update local config failed! ret: [%d]", ret);
    }
  }
...
//add liu jun. [MultiUPS] [part_cache] 20150811:b
const ObClientManager *ObConfigManager::get_client_mgr() const
{
  return client_mgr_;
}
//add 20150811:e
