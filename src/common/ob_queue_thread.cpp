...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObCond::timedwait(const int64_t time_us)
    {
      int ret = OB_SUCCESS;
      bool need_wait = true;
      if ((last_waked_time_ + BUSY_INTERVAL) > yysys::CTimeUtil::getTime())
      {
        //return OB_SUCCESS;
...
//add support for arm platform by wangd 202106:e
      ...
      if (need_wait)
      {
        int64_t abs_time = time_us + yysys::CTimeUtil::getTime();
        struct timespec ts;
      ...
      if (OB_SUCCESS == ret)
      {
        last_waked_time_ = yysys::CTimeUtil::getTime();
      }
      return ret;
    }
    ...
    int S2MQueueThread::init(const int64_t thread_num, const int64_t task_num_limit, const bool queue_rebalance, const bool dynamic_rebalance)
    {
      ...
      if (0 != thread_num_)
      {
        ret = OB_INIT_TWICE;
               ...
               || 0 >= task_num_limit)
      {
        YYSYS_LOG(WARN, "invalid param, thread_num=%ld task_num_limit=%ld",
                  thread_num, task_num_limit);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = launch_thread_(thread_num, task_num_limit)))
      {
        YYSYS_LOG(WARN, "launch thread fail, ret=%d thread_num=%ld task_num_limit=%ld", ret, thread_num, task_num_limit);
      }
      else if (OB_SUCCESS != (ret = balance_filter_.init(thread_num_, dynamic_rebalance)))
      {
        YYSYS_LOG(WARN, "init balance_filter fail, ret=%d thread_num=%ld", ret, thread_num_);
      }
      else
      {
        // do nothing
    ...
    int S2MQueueThread::push(void *task, const uint64_t task_sign, const int64_t prio)
    {
      ...
      else
      {
        ...
        YYSYS_TRACE_LOG("req_sign=%lu,%lu hash=%lu", filted_task_sign % thread_num_, filted_task_sign, task_sign);
        if (OB_SUCCESS != (ret = tc.spec_task_queue.push(task)))
        {
          //YYSYS_LOG(WARN, "push task to queue fail, ret=%d", ret);
          if (OB_SIZE_OVERFLOW == ret)
          {
            ret = OB_EAGAIN;
          }
    ...
    int S2MQueueThread::add_thread(const int64_t thread_num, const int64_t task_num_limit)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, thread_num=%ld", thread_num);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int S2MQueueThread::sub_thread(const int64_t thread_num)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid param, thread_num=%ld thread_num_=%ld",
                    thread_num, thread_num_);
          ret = OB_INVALID_ARGUMENT;
        }
        else
      ...
      if (OB_SUCCESS == ret)
      {
        for (int64_t i = cur_thread_num; i < prev_thread_num; i++)
        {
          ...
          tc.comm_task_queue.destroy();
          tc.low_prio_task_queue.destroy();
          YYSYS_LOG(INFO, "join thread succ, index=%ld", i);
        }
      }
      return ret;
    }
    ...
    void *S2MQueueThread::rebalance_(const ThreadConf &cur_thread)
    {
      ...
      for (uint64_t i = 1; (int64_t)i <= thread_num_; i++)
      {
        ...
        if (!tc.using_flag
            )//&& (tc.last_active_time + THREAD_BUSY_TIME_LIMIT) < yysys::CTimeUtil::getTime())
        {
        ...
        if (NULL != ret)
        {
          ...
          {
            YYSYS_LOG(INFO, "task has been rebalance between threads rebalance_counter=%ld cur_idx=%ld balance_idx=%ld", rebalance_counter, cur_thread.index, balance_idx);
          }
          break;
        }
      }
      //add for [schedule optimize]-b
      if (NULL == ret)
      {
        for (uint64_t i = 1; (int64_t) i <= thread_num_; i++)
        {
          ...
          {
            tc.low_prio_task_queue.pop(ret);
          }
          if (NULL != ret)
          {
            ATOMIC_ADD(&low_prio_queued_num_, -1);
    ...
    int S2MQueueThread::launch_thread_(const int64_t thread_num, const int64_t task_num_limit)
    {
      ...
      for (int64_t i = 0; i < thread_num2launch; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "high prio task queue init fail, task_num_limit=%ld", task_num_limit);
          break;
        }
        if (OB_SUCCESS != (ret = tc.spec_task_queue.init(task_num_limit)))
        {
          YYSYS_LOG(WARN, "spec task queue init fail, task_num_limit=%ld", task_num_limit);
          break;
        }
        if (OB_SUCCESS != (ret = tc.comm_task_queue.init(task_num_limit)))
        {
          YYSYS_LOG(WARN, "comm task queue init fail, task_num_limit=%ld", task_num_limit);
          break;
        }
        if (OB_SUCCESS != (ret = tc.low_prio_task_queue.init(task_num_limit)))
        {
          YYSYS_LOG(WARN, "low prio task queue init fail, task_num_limit=%ld", task_num_limit);
          break;
        }
        int tmp_ret = 0;
        if (0 != (tmp_ret = pthread_create(&(tc.pd), NULL, thread_func_, &tc)))
        {
          YYSYS_LOG(WARN, "pthread_create fail, ret=%d", tmp_ret);
        ...
        YYSYS_LOG(INFO, "create thread succ, index=%ld tmp_ret=%d", thread_num_, tmp_ret);
        thread_num_ += 1;
      }
      return ret;
    }
    ...
    void *S2MQueueThread::thread_func_(void *data)
    {
      ...
      if (NULL == tc
          || NULL == tc->host)
      {
        YYSYS_LOG(WARN, "thread_func param null pointer");
      }
      else
      {
               ...
               || 0 != tc->low_prio_task_queue.get_total())
        {
                  ...
                  && (last_rebalance_got = (NULL != (task = tc->host->rebalance_(*tc))))))
          {
            ...
            tc->last_active_time = yysys::CTimeUtil::getTime();
          }
          else
          {
          ...
          tc->scheduler_.update(idx, yysys::CTimeUtil::getTime() - start_time, queue_len);
        }
                             ...
                             const int64_t idle_interval)
    {
      ...
      {
        YYSYS_LOG(WARN, "task_queue init fail, ret=%d task_num_limit=%ld", ret, task_num_limit);
      }
    ...
    void *M2SQueueThread::thread_func_(void *data)
    {
      ...
      else
      {
               ...
               || 0 != host->task_queue_.get_total())
        {
          ...
          else if ((host->last_idle_time_ + host->idle_interval_) <= yysys::CTimeUtil::getTime())
          {
            host->on_idle();
            host->last_idle_time_ = yysys::CTimeUtil::getTime();
          }
          else
          {
                             ...
                             const int64_t idle_interval)
    {
      ...
      {
        YYSYS_LOG(ERROR, "task_queue_.start(1)=>%d", ret);
      }
      else if (0 != (tmp_ret = pthread_create(&pd_, NULL, thread_func_, this)))
      {
        ret = OB_ERR_UNEXPECTED;
    ...
    int SeqQueueThread::push(void *task)
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "get_seq(task[%p])>%ld", task, seq);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "task_queue_.add(seq=%ld, task=%p)=>%d", seq, task, ret);
        }
      }
      if (OB_SUCCESS != ret)
      {
      ...
      else
      {
        ...
        while (host->run_flag_)
        {
          ...
          if (OB_SUCCESS != (err = host->task_queue_.get(seq, task, QUEUE_WAIT_TIME))
              && OB_EAGAIN != err)
          {
            YYSYS_LOG(ERROR, "get(seq=%ld)=>%d", seq, err);
            break;
          }
