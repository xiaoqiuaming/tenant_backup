...
namespace oceanbase
{
  namespace common
  {
    ...
    bool ObRegex::init(const char* pattern, int flags)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param pattern=%p", pattern);
      }
      else
      {
        ...
        if (0 != tmp_ret)
        {
          YYSYS_LOG(WARN, "regcomp fail ret=%d", tmp_ret);
        }
        else
        {
          ...
          if (NULL == match_)
          {
            YYSYS_LOG(WARN, "create the regmatch object fail");
            regfree(&reg_);
          }
          else
          {
    ...
    bool ObRegex::match(const char* text, int flags)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param text=%p", text);
      }
      else
      {
