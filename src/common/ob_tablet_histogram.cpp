...
//      range_.to_string(row_key_dump_buff, OB_MAX_ROW_KEY_LENGTH * 2);
      YYSYS_LOG(INFO, "sample range = %s", to_cstring(range_));
      YYSYS_LOG(INFO, "row_count = %ld", row_count_);
    }
    ...
    void ObTabletHistogram::dump() const
    {
      ...
      {
        YYSYS_LOG(DEBUG, "dump %d-th sample.", i);
        samples_[i].dump();
      }
    }
    ...
    int ObTabletHistogram::get_sample(const int64_t sample_index, ObTabletSample *&return_sample)
    {
      ...
      {
        YYSYS_LOG(WARN, "array index out of range.");
    ...
    int ObTabletHistogram::add_sample(const ObTabletSample &index_sample)
    {
      ...
      {
        ret = OB_ARRAY_OUT_OF_RANGE;
        YYSYS_LOG(WARN, "too many sample in one histogram, max is %ld, now is %ld.", MAX_SAMPLE_BUCKET, sample_helper_.get_array_index());
      }
      else if (!sample_helper_.push_back(index_sample))
      {
        ret = OB_ARRAY_OUT_OF_RANGE;
        YYSYS_LOG(WARN, "can not push index_sample into sample_list_");
      }
      return ret;
    }
    ...
    int ObTabletHistogram::add_sample_with_deep_copy(const ObTabletSample &index_sample)
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "allocator_ is null.");
      }
      ...
      {
        YYSYS_LOG(WARN, "deep copy index sample failed!");
      }
    ...
    int ObTabletHistogram::deep_copy(const ObTabletHistogram &other)
    {
      ...
      for (int64_t i = 0; i < size; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "get sample failed");
          break;
        }
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "sample is NULL");
          break;
        }
        //add:e
        else if (OB_SUCCESS != (ret = add_sample_with_deep_copy(*sample)))
        {
          YYSYS_LOG(WARN, "add sample failed");
          break;
        }
      }
    ...
    {
      yysys::CThreadGuard mutex_gard(&alloc_mutex_);
      module_arena_.reuse();
    ...
    int ObTabletHistogramManager::fill_all_samples()
    {
      ...
      for (int64_t hist_index = 0; hist_index < histogram_helper_.get_array_index() && OB_LIKELY(OB_SUCCESS == ret); hist_index++)
      {
        ...
        {
          ret = OB_ARRAY_OUT_OF_RANGE;
          YYSYS_LOG(WARN, "too many samples in one table, fill all samples failed.");
        }
        else
        {
    ...
    */
    ...
    int ObTabletHistogramManager::get_histogram(const int32_t hist_index, ObTabletHistogram *&out_hist) const
    {
      ...
      {
        ret = OB_ARRAY_OUT_OF_RANGE;
        YYSYS_LOG(WARN, "index out of range, index=%d, histogram_helper_.size()=%ld", hist_index, histogram_helper_.get_array_index());
      }
...
//      yysys::CThreadGuard mutex_gard(&alloc_mutex_);
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "added histogram is null.");
      }
      //20150512, tid in histogram must be consistent with index_tid_.
      else if (OB_SUCCESS != (ret = histogram.get_sample(0, first_sample)))
      {
        YYSYS_LOG(WARN, "get first sample failed.");
      }
      else if (NULL == first_sample)
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "first sample is NULL.");
      }
      else if (index_tid_ != (idx_tid = first_sample->range_.table_id_))
      {
        ret = OB_INVALID_DATA;
        YYSYS_LOG(WARN, "index tid is not consistent. index_tid_[%lu], sample's range tid[%lu]", index_tid_, first_sample->range_.table_id_);
      }
      else if (histogram_helper_.get_array_index() >= MAX_TABLET_COUNT_PER_TABLE)
      {
        ret = OB_ARRAY_OUT_OF_RANGE;
        YYSYS_LOG(WARN, "too many histogram in hist manager, max is %ld, now is %ld.", MAX_TABLET_COUNT_PER_TABLE, histogram_helper_.get_array_index());
      }
      else
      {
...
//        if (OB_SUCCESS != (ret = inner_hist->deep_copy(histogram)))
        ...
        {
          ret = OB_ERROR ;
          YYSYS_LOG(WARN, "alloc hist object failed.");
        }
        else if (OB_SUCCESS != (ret = inner_hist->deep_copy(module_arena_, histogram)))
        {
          ret = OB_ARRAY_OUT_OF_RANGE;
          YYSYS_LOG(WARN, "deep copy histogram with specific allocator failed.");
        }
        else if (!histogram_helper_.push_back(inner_hist))
        {
          ret = OB_ARRAY_OUT_OF_RANGE;
          YYSYS_LOG(WARN, "failed to push hist into hist manager.");
        }
      }
      if (OB_SUCCESS == ret)
      {
    ...
    //add for [bugfix too many histogram in hist manager]-b
    int ObTabletHistogramManager::pop_histogram(const int32_t hist_index)
    {
      int ret = OB_SUCCESS;
      ObTabletHistogram **pret = NULL;
      if (hist_index <= 0 || histogram_helper_.get_array_index() -1 != hist_index)
      {
        ret = OB_INVALID_DATA;
        YYSYS_LOG(WARN, "wrong element to be removed");
      }
      else if (NULL == (pret = histogram_helper_.pop()))
      {
        ret = OB_ERROR;
      }
      return ret;
    }
    //add for [bugfix too many histogram in hist manager]-e
...
//      YYSYS_LOG(ERROR, "test::liumz, sp_sorted_list_.size()=%d", sp_sorted_list_.size());
//      for (int32_t i = 0; i < sp_sorted_list_.size(); i++)
//      {
//        YYSYS_LOG(ERROR, "test::liumz, dump %d-th sample wrapper of sp_sorted_list_", i);
//        if (NULL != (sw = sp_sorted_list_.at(i)))
//        {
//          sw->sp_->dump();
//        }
//      }
      YYSYS_LOG(ERROR, "test::liumz, get_one_tablet_info cur_wrapper_=%p", cur_wrapper_);

      if (NULL == cur_wrapper_)
      {
        ret = OB_ERROR;//important
        YYSYS_LOG(WARN, "sp_sorted_list_ is empty");
      }
      else
      {
        ...
        if (cur_wrapper_ < end() - 1)//cur_wrapper_ used last time
        {
          ...
          if (sp_sorted_list_.begin() == cur_wrapper_)//����(MIN_ROWKEY, XXX]
          {
          ...
          if (sp_sorted_list_.end() - 1 == cur_wrapper_)//����(XXX, MAX_ROWKEY]
          {
    ...
    */
    ...
    DEFINE_DESERIALIZE(ObTabletSample)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to deserialize range, ret=%d, buf=%p, data_len=%ld, pos=%ld",
    ...
    DEFINE_DESERIALIZE(ObTabletHistogram)
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "allocator_ not set, deseriablize failed.");
      }
      ...
      if (ret == OB_SUCCESS && size > 0)
      {
        for (int64_t i=0; i<size; ++i)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to deserialize ObStaticIndexSample.");
            break;
          }
          else if (OB_SUCCESS != (ret = add_sample(sample)))//ǳ����
          {
            YYSYS_LOG(WARN, "fail to add sample into histogram.");
            break;
          }
        }
      }
