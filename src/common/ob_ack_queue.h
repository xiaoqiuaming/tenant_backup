  ...
  namespace common
  {
    ...
    class ObAckQueue
    {
      public:
        ...
        enum
        {
          TIMEOUT_DELTA = 10000, DEFAULT_DELAY_WARN_THRESHOLD_US = 50000
        };
        struct WaitNode
        {
          ...
          { return timeout_us_ > 0 && send_time_us_ + timeout_us_ < yysys::CTimeUtil::getTime(); }
          // modify wangdonghui [ups_replication] 20170323 :b
          //void done(int err);
          void done(int64_t& message_residence_time_us, int err);
          //modify :e
          int64_t get_delay()
          { return receive_time_us_ - send_time_us_; }
    ...
    class IObAsyncClientCallback
    {
      public:
        ...
        {}
        virtual int handle_response(ObAckQueue::WaitNode& node) = 0; // ɾ��ʧ�ܵ�slave
        virtual int on_ack(ObAckQueue::WaitNode& node) = 0; // ���ѵȴ��߳�
