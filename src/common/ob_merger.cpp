...
namespace oceanbase
{
  namespace common
  {
    ObMerger :: ObMerger(bool is_asc)
    {
      reset();
      is_asc_ = is_asc;
      //add for [truncate table]-b
      rdel_cell_.reset();
      trun_iter_idx_ = MAX_ITER_NUM;
      //add for [truncate table]-e
    }
    ...
    //modify for [truncate table]-b
    //    int ObMerger :: add_iterator(ObIterator* iterator)
    int ObMerger :: add_iterator(ObIterator* iterator, bool trun_status)
    //modify for [truncate table]-e
    {
      int err = OB_SUCCESS;

      if (NULL == iterator)
      {
        YYSYS_LOG(WARN, "invalid param, iterator=%p", iterator);
        err = OB_INVALID_ARGUMENT;
      }
      else if (iter_num_ >= MAX_ITER_NUM)
      {
        YYSYS_LOG(WARN, "exceeds limit, iter_num_=%ld", iter_num_);
        err = OB_ERROR;
      }
      else
      {
    ...
    ObIterator *ObMerger::get_current_iter(void)
    {
      ObIterator *ret = NULL;
      if (cur_iter_arr_idx_ < cur_iter_arr_num_)
      {
        ret = iters_[cur_iter_arr_[cur_iter_arr_idx_]];
        //        YYSYS_LOG(INFO, "cur iter arr num=%ld, cur iter arr idx=%ld, total iter num =%ld",
        //                  cur_iter_arr_num_,cur_iter_arr_idx_, iter_num_);
      }
      //add for [truncate table]-b
      else if (cur_iter_arr_idx_ == cur_iter_arr_num_ && cur_iter_arr_idx_ > 0)
      {
        ret = iters_[cur_iter_arr_[cur_iter_arr_num_ - 1]];
      }
      //add for [truncate table]-e
      else
      {
        YYSYS_LOG(WARN, "no iter can be get, please check,cur iter arr num=%ld, cur iter arr idx=%ld, total iter num =%ld",
                  cur_iter_arr_num_,cur_iter_arr_idx_, iter_num_);
      }
      return ret;
    }
    ...
    int ObMerger :: next_cell()
    {
      ...
      if (iter_num_ < 0)
      {
        YYSYS_LOG(ERROR, "error occurs, iter_num=%ld", iter_num_);
        err = OB_ERROR;
      }
      else if (0 == iter_num_)
      {
      ...
      else
      {
        ...
        if (OB_SUCCESS == err && cur_iter_arr_idx_ < cur_iter_arr_num_) // current row is iterating
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to call next_cell of the %ld-th iter, err=%d",
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to get cell, cur_iter_idx=%ld, err=%d",
                        cur_iter_arr_[cur_iter_arr_idx_], err);
              err = OB_ERROR;
            }
        ...
        if (OB_SUCCESS == err && !find_success)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to find smallest row, err=%d", err);
            }
            else
            {
    ...
    int ObMerger :: find_next_row_()
    {
      ...
      int64_t last_trun_row_idx = -1; //add for [truncate table]
      ...
      for (int64_t i = 0; OB_SUCCESS == err && i < iter_num_; ++i)
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != err || NULL == cur_cell)
          {
            YYSYS_LOG(WARN, "failed to get cell of the %ld-th iter, cur_cell=%p, err=%d",
                      i, cur_cell, err);
            err = OB_ERROR;
          }
          else
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "failed to cmp_row_key, min_max_cell=%p, cur_cell=%p, err=%d",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          //add for [truncate table]-b
          int64_t pop_idx = -1;
          if (last_trun_row_idx == -1 && trun_iter_idx_ < MAX_ITER_NUM)
          {
            pop_idx = cur_iter_arr_num_;
          }
          else if (last_trun_row_idx > last_del_row_idx)
          {
          ...
          for (int64_t i = 0; OB_SUCCESS == err && i < pop_idx; ++i) //modify for [truncate table]
          {
            // pop deleted cell of the current row
            err = pop_current_row_(cur_iter_arr_[i]);
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to pop current row, i=%ld, idx=%ld", i, cur_iter_arr_[i]);
            }
          }
                                 ...
                                 int& cmp_val)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, first_cell=%p, second_cell=%p", first_cell, second_cell);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObMerger :: next_cell_(const int64_t iter_idx)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, iter_idx=%ld, iter_num_=%ld",
                  iter_idx, iter_num_);
        err = OB_INVALID_ARGUMENT;
      }
    ...
    int ObMerger :: pop_current_row_(const int64_t iter_idx)
    {
      ...
      if (iter_idx < 0 || iter_idx >= iter_num_)
      {
        YYSYS_LOG(WARN, "invalid param, iter_idx=%ld, iter_num_=%ld", iter_idx, iter_num_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        while (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to call get_cell, iter_idx=%ld, err=%d", iter_idx, err);
            err = OB_ERROR;
          }
          else if (is_row_changed)
          {
        ...
        {
          YYSYS_LOG(WARN, "failed to pop current row, err=%d", err);
        }
      }
    ...
    int ObMerger :: get_cell_(const int64_t iter_idx, ObCellInfo** cell, bool* is_row_changed)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, iter_idx=%ld, iter_num_=%ld, cell=%p",
      ...
      else if (NULL != iters_[iter_idx]) // double check
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get_cell of the %ld-th iter, err=%d, cell=%p",
                    iter_idx, err, cell);
          err = OB_ERROR;
        }
      }
