...
namespace oceanbase
{
  namespace common
  {
    ...
    {
        TypeDesc()
            :type_(ObMinType), precision_(0), scale_(0), length_(0)
        {
        }
        ...
        void init(const uint32_t precision, const uint32_t scale, const uint32_t length, const ObObjType type)
        {
            type_ = type;
        ...
        {
            int64_t pos = 0;
            databuff_printf(buffer, length, pos, "type_ = [%d], precision_ = [%u], scale_ = [%u], length_ = [%u]", type_, precision_, scale_, length_);
            return pos;
        }
    };
    ...
    class ObExprObj
    {
      public:
        ...
        void set_if_format_wrong(const bool wrong);
        void reset_format_wrong_flag();
        ...
        const ObString& get_varchar() const
        { return varchar_; }
        ...
        const ObDecimal& get_decimal() const
        { return decimal_; }
        //modify:e

        const uint32_t get_len() const
        { return len; }
        const int64_t get_ext() const
        { return v_.ext_; }

        bool get_if_format_wrong() const
        {
          return if_format_wrong_;
        }
        ...
        //add:e
        int64_t to_string(char* buffer, const int64_t length) const;
        int64_t to_string_v2(std::string& s) const;//add gaojt [ListAgg][JHOBv0.1]20150104    //[conflict_level A] 
        ...
        //add 20151225:e
        //add lqc [MultiUps 1.0] [#] 20170417 b
        static bool is_equal_part_value_type(const ObObjType &t1, const ObObjType &t2);
        //add e

        //[view]
    public:
        int calc(const TypeDesc type[2], const OpType op, TypeDesc &result_type, const int64_t digit = 0, const int64_t pos = 0, const int64_t len = 0);
    private:
        int cons_decimal_num(const uint32_t precision, const uint32_t scale, char num[40], ObDecimal &decimal_num, bool is_division = false);
        int add_sub_mul(const TypeDesc &type_1, const TypeDesc &type_2, const OpType op, TypeDesc &result_type);
        int div(const TypeDesc &type_1, const TypeDesc &type_2, TypeDesc &result_type);
        void concat(const TypeDesc &type1, const TypeDesc &type2, TypeDesc &result_type);
        void round(const TypeDesc &type, const int64_t digit, TypeDesc &result_type);
        void substr(const TypeDesc &type, const int64_t pos, const int64_t len, TypeDesc &result_type);
      ...
      private:
        ...
        //add fanqiushi DECIMAL OceanBase_BankCommV0.3 2014_7_19:b
        inline int cast_to_decimal(ObString &dec, ObStringBuf &mem_buf) const;
        //add:e
   ...
   //modify:e
    ...
    //add for [507-expire info if format incorrect]-b
    inline void ObExprObj::set_if_format_wrong(const bool wrong)
    {
        if_format_wrong_ = wrong;
    }

    inline void ObExprObj::reset_format_wrong_flag()
    {
        if_format_wrong_ = false;
    }
