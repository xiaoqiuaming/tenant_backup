...
oceanbase::common::ObCounter::ObCounter(const int64_t interval_us)
{
  ...
  intervals_[cur_idx_].interval_beg_ =   yysys::CTimeUtil::getTime();
...
int oceanbase::common::ObCounterSet::init(const ObCounterInfos &counter_infos)
{
  ...
  {
    YYSYS_LOG(WARN,"there are two many counters in counter info [counter_infos.get_max_counter_size():%ld,"
...
void oceanbase::common::ObCounterSet::inc(const int64_t counter_id, const int64_t time_used_us)
{
  ...
  if (NULL == infos_)
  {
    YYSYS_LOG(WARN,"counter set has not been initialized yet");
    err = OB_INVALID_ARGUMENT;
  }
  else if ((counter_id < 0) || (counter_id >= MAX_COUNTER_SIZE) || (counter_id >= infos_->get_max_counter_size()))
  {
    YYSYS_LOG(WARN,"counter_id out of range [counter_id:%ld,infos_->get_max_counter_size():%ld,max_counter_size:%ld]", 
...
void oceanbase::common::ObCounterSet::print_static_info(const int64_t log_level )
{
  ...
  if (level < std::min<int>((YYSYS_LOG_LEVEL_ERROR), (YYSYS_LOG_LEVEL_DEBUG)))
  {
    level = std::min<int>((YYSYS_LOG_LEVEL_ERROR), (YYSYS_LOG_LEVEL_DEBUG));
  }
  if (level > std::max<int>((YYSYS_LOG_LEVEL_ERROR), (YYSYS_LOG_LEVEL_DEBUG)))
  {
    level = std::max<int>((YYSYS_LOG_LEVEL_ERROR), (YYSYS_LOG_LEVEL_DEBUG));
  }

  for (int64_t c_idx = 0; c_idx < infos_->get_max_counter_size(); c_idx++)
  {
    YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(static_cast<int32_t>(level)), 
