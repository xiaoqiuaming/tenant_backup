...
bool TableSchema::is_valid_index() const
{
  ...
  {
    YYSYS_LOG(USER_ERROR, "invalid replica num %d", replica_num_);
  }
  else
  {
    ...
    for (int32_t i = 0; i < columns_.count(); ++i)
    {
      if (columns_.at(i).rowkey_id_ != 0)
      {
        ...
        {
          YYSYS_LOG(USER_ERROR, "column '%s' with %s type as primary key not support",  columns_.at(i).column_name_,
        ...
        {
          YYSYS_LOG(USER_ERROR, "column id is greater than max_used_column_id, name=%s id=%ld max=%ld",
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN, "all columns=%ld are defined as rowkey column=%d, table=%s",
      ...
      {
        YYSYS_LOG(WARN, "rowkey_column_num=%d but defined_num=%d, table=%s",
...
bool TableSchema::is_valid() const
{
  ...
  //[view]
  else if (table_type_ == TableSchema::VIEW)
  {
      ret = true;
  }
...
int TableSchema::to_string(char* buf, const int64_t buf_len) const
{
  ...
  databuff_printf(buf, buf_len, pos,
                  ...
                  "partition_func_name_=%s"
                  "group_name_prefix=%s"
                  "param_list=%s"
                  "partition_func_rule_body=%s"
                  "table_partition_type=%d"
                  "partition_key_col_num=%ld"
                  //add 20150423:e
                  "partition_func_type=%ld"//add wuna[MultiUPS] [sql_api] 20151208
                  "partition_form=%ld"//add wuna[MultiUPS] [sql_api] 20151208
                  "replica_num=%d "
                  ...
                  rowkey_column_num_,
                  //add liu jun.[MultiUPS] [sql_api] 20150423:b
                  partition_func_name_,
                  group_name_prefix_,
                  param_list_,
                  partition_func_rule_body_,//add wuna[MultiUPS] [sql_api] 20151208
                  table_partition_type_,
                  partition_key_col_num_,
                  //add 20150423:e
                  partition_func_type_,//add wuna[MultiUPS] [sql_api] 20151208
                  partition_form_,//add wuna[MultiUPS] [sql_api] 20151208
                  replica_num_,
                  max_used_column_id_,
                  create_mem_version_,
                  tablet_block_size_,
...
DEFINE_SERIALIZE(ColumnSchema)
{
  ...
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, default_value_)))
  {
  }
  ...
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, default_value_is_null)))
  {
  }
...
DEFINE_DESERIALIZE(ColumnSchema)
{
  ...
  int64_t len2 = 0;
  serialization::decode_vstr(buf, data_len, pos, default_value_, OB_MAX_DEBUG_MSG_LEN, &len2);
  ...
  else if (OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &default_value_is_null)))
  {
  }
...
DEFINE_SERIALIZE(TableSchema)
{
  ...
  //add:e
  //add liu jun.[MultiUPS] [sql_api] 20150423:b
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, partition_func_name_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, group_name_prefix_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, param_list_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, partition_func_rule_body_)))
  {
  }
  //add 20150423:e
  ...
  //add liu jun.[MultiUPS] [sql_api] 20150423:b
  else if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, table_partition_type_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vi64(buf, buf_len, pos, partition_key_col_num_)))
  {
  }
  //add 20150423:e
  //add wuna[MultiUPS] [sql_api] 20151208:b
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_rule_modify_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vi64(buf, buf_len, pos, partition_func_type_)))
  {
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vi64(buf, buf_len, pos, partition_form_)))
  {
  }
  //add 20151208:e
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to serialize join info count:ret[%d]", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = 0; OB_SUCCESS == ret && i < join_info_.count(); i ++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to serialization join info:ret[%d], i[%ld]", ret, i);
      }
    }
  }
  ...
  //[737]
  //[view]
  if (OB_SUCCESS == ret && table_type_ == TableSchema::VIEW)
  {
    if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, text_)))
    {
      YYSYS_LOG(WARN, "failed to serialize text_, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, with_check_option_)))
    {
      YYSYS_LOG(WARN, "failed to serialize with_check_option_, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_updatable_)))
    {
      YYSYS_LOG(WARN, "failed to serialize is_updatable_, err=%d", ret);
    }
  }
  return ret;
}
...
DEFINE_DESERIALIZE(TableSchema)
{
  ...
  int64_t len7 = 0;
  int64_t len8 = 0;
  int64_t len9 = 0;//uncertainty len5±»Ê¹ÓÃ
  //add 20150423:e
  ...
  //add:e
   //add liu jun.[MultiUPS] [sql_api] 20150423:b
  serialization::decode_vstr(buf, data_len, pos, partition_func_name_, OB_MAX_TABLE_NAME_LENGTH, &len9);
  serialization::decode_vstr(buf, data_len, pos, group_name_prefix_, OB_MAX_VARCHAR_LENGTH, &len6);
  serialization::decode_vstr(buf, data_len, pos, param_list_, OB_MAX_VARCHAR_LENGTH, &len7);
  serialization::decode_vstr(buf, data_len, pos, partition_func_rule_body_, OB_MAX_VARCHAR_LENGTH, &len8);
  //add 20150423:e
  ...
  {
    YYSYS_LOG(WARN, "deserialize error, len1=%ld len5=%ld len2=%ld len3=%ld len4=%ld", len1, len5, len2, len3, len4);
  ...
  //add liu jun.[MultiUPS] [sql_api] 20150423:b
  else if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, data_len, pos, &table_partition_type_)))
  {
    YYSYS_LOG(WARN, "deserialize error here");
  }
  else if (OB_SUCCESS != (ret = serialization::decode_vi64(buf, data_len, pos, &partition_key_col_num_)))
  {
    YYSYS_LOG(WARN, "deserialize error here");
  }
  //add 20150423:e
  //add wuna [MultiUps] [sql_api] 20151208:b
  else if (OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_rule_modify_)))
  {
    YYSYS_LOG(WARN, "deserialize error here");
  }
  else if (OB_SUCCESS != (ret = serialization::decode_vi64(buf, data_len, pos, &partition_func_type_)))
  {
    YYSYS_LOG(WARN, "deserialize error here");
  }
  else if (OB_SUCCESS != (ret = serialization::decode_vi64(buf, data_len, pos, &partition_form_)))
  {
    YYSYS_LOG(WARN, "deserialize error here");
  }
  //add 20151208:e
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize join info count:ret[%d]", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == ret && i < join_info_count; i ++)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to deserialize join info:ret[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = join_info_.push_back(join_info)))
      {
        YYSYS_LOG(WARN, "fail to push join info to array;ret[%d]", ret);
      }
    }
  }
  //add wenghaixing [secondary index] 20141029
  ...
  {
    int64_t len10 = 0;
    serialization::decode_vstr(buf, data_len, pos, text_, OB_MAX_SQL_LENGTH, &len10);

    if ( len10 < 0)
    {
      YYSYS_LOG(WARN, "deserialize error len10=%ld", len10);
      ret = OB_DESERIALIZE_ERROR;
    }
    else if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, data_len, pos, reinterpret_cast<int32_t*>(&with_check_option_))))
    {
      YYSYS_LOG(WARN, "deserialize with_check_option_ error");
    }
    else if (OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_updatable_)))
    {
      YYSYS_LOG(WARN, "deserialize is_updatable_ error");
    }
  }
  return ret;
}
...
DEFINE_SERIALIZE(AlterTableSchema)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "new table name is empty");
  }
  //add e.
  //add wuna[MultiUPS] [sql_api] 20160111:b
  else if ('\0' == group_name_prefix_[0] && is_rule_modify_ == true)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "group prefix name is empty");
  }
  //add 20160111:e
  else if ('\0' != new_expire_info_[0] && is_drop_expire_info_ == true)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "new expire info is not empty");
  }
  else if ('\0' == new_expire_info_[0] && true == is_alter_expire_info_)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "new expire info is empty");
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, table_name_)))
  {
    YYSYS_LOG(WARN, "failed to serialize table name, err=%d", ret);
  }
  //add dolphin [database manager]@20150610:b
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, dbname_)))
  {
    YYSYS_LOG(WARN, "failed to serialize database name, err=%d", ret);
  }
  //add:e
  //add liuj [Alter_Rename] [JHOBv0.1] 20150104
  ...
  {
    YYSYS_LOG(WARN, "failed to serialize hash func name, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, group_name_prefix_)))
  {
    YYSYS_LOG(WARN, "failed to serialize group prefix name, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, param_list_)))
  {
    YYSYS_LOG(WARN, "failed to serialize param list name, err=%d", ret);
  }
  //add 20160111:e
  else if (OB_SUCCESS != (ret = serialization::encode_vstr(buf, buf_len, pos, new_expire_info_)))
  {
    YYSYS_LOG(WARN, "failed to serialize expire info, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vi64(buf, buf_len, pos, columns_.count())))
  {
    YYSYS_LOG(WARN, "failed to serialize column count, err=%d", ret);
  }
  //add wuna[MultiUPS] [sql_api] 20160111:b
  else if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, partition_type_)))
  {
    YYSYS_LOG(WARN, "failed to serialize partiton type, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, partition_key_col_num_)))
  {
    YYSYS_LOG(WARN, "failed to serialize hash column num, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_rule_modify_)))
  {
    YYSYS_LOG(WARN, "failed to serialize is rule modify, err=%d", ret);
  }
  //add 20160111:e
  //add liuj [Alter_Rename] [JHOBv0.1] 20150104
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, has_table_rename_)))
  {
    YYSYS_LOG(WARN, "failed to serialize has table rename, err=%d", ret);
  }
  //add e.
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_drop_expire_info_)))
  {
    YYSYS_LOG(WARN, "failed to serialize is drop expire info, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_alter_expire_info_)))
  {
    YYSYS_LOG(WARN, "failed to serialize is alter expire info, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_load_type_modify_)))
  {
    YYSYS_LOG(WARN, "failed to serialize is load type modify, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::encode_bool(buf, buf_len, pos, is_use_block_cache_)))
  {
    YYSYS_LOG(WARN, "failed to serialize is use block cache, err=%d", ret);
  }
  else
  {
    for (int64_t i = 0; i < columns_.count(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialize is add flag, err=%d", ret);
        break;
      }
...
DEFINE_SERIALIZE(JoinInfo)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to serialize:ret[%d]", ret);
  }
...
DEFINE_DESERIALIZE(JoinInfo)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize:ret[%d]", ret);
    }
    else
    {
      left_table_id_ = id;
...
DEFINE_DESERIALIZE(AlterTableSchema)
{
  ...
  //add:e
  //add wuna[MultiUPS] [sql_api] 20160111:b
  int64_t partition_func_name_len=0;
  int64_t group_name_prefix_len=0;
  int64_t param_list_len=0;
  //add 20160111:e
  int64_t column_count = 0;
  int64_t new_expire_info_len = 0;
  ...
  //add wuna[MultiUPS] [sql_api] 20160111:b
  serialization::decode_vstr(buf, data_len, pos, partition_func_name_, OB_MAX_TABLE_NAME_LENGTH, &partition_func_name_len);
  serialization::decode_vstr(buf, data_len, pos, group_name_prefix_, OB_MAX_VARCHAR_LENGTH, &group_name_prefix_len);
  serialization::decode_vstr(buf, data_len, pos, param_list_, OB_MAX_VARCHAR_LENGTH, &param_list_len);
  //add 20160111:e
  serialization::decode_vstr(buf, data_len, pos, new_expire_info_, OB_MAX_EXPIRE_CONDITION_LENGTH, &new_expire_info_len);
  if (table_name_len <= 0)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check table name len failed, err=%d", ret);
  }
  //add dolphin [database manager]@20150609:b
  if (table_name_len < 0)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "DESERIALIZE database name failed, err=%d", ret);
  }
  //add:e
  //add liuj [Alter_Rename] [JHOBv0.1] 20150104
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check new table name len failed, err=%d", ret);
  }
  //add e.
  //add wuna[MultiUPS] [sql_api] 20160111:b
  else if(group_name_prefix_len <= 0 && is_rule_modify_  == true)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check group name prefix len failed, err=%d", ret);
  }
  //add 20160111:e
  else if(new_expire_info_len < 0 && is_drop_expire_info_ == true)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check new expire info len failed, err=%d", ret);
  }
  else if(0 >= new_expire_info_len && true == is_alter_expire_info_)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check new expire info len failed, err=%d", ret);
  }
  ...
  {
    YYSYS_LOG(WARN, "failed to deserialize column count, err=%d", ret);
  }
  //add wuna[MultiUPS] [sql_api] 20160111:b
  else if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, data_len, pos, &partition_type_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize partition type, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, data_len, pos, &partition_key_col_num_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize hash col num, err=%d", ret);
  }
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_rule_modify_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize is rule modify, err=%d", ret);
  }
  //add 20160111:e
  //add liuj [Alter_Rename] [JHOBv0.1] 20150104
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &has_table_rename_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize has table rename, err=%d", ret);
  }
  //add e.
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_drop_expire_info_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize is drop expire info, err=%d", ret);
  }
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_alter_expire_info_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize is alter expire info, err=%d", ret);
  }
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_load_type_modify_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize is load type modify, err=%d", ret);
  }
  else if(OB_SUCCESS != (ret = serialization::decode_bool(buf, data_len, pos, &is_use_block_cache_)))
  {
    YYSYS_LOG(WARN, "failed to deserialize is use block cache, err=%d", ret);
  }
  else
  {
    int64_t alter_type;
    ...
    for (int64_t i = 0; i < column_count; ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialize is add flag, err=%d", ret);
        break;
      }
...
DEFINE_SERIALIZE(IndexBeat)
{
  ...
  else if (OB_SUCCESS != (ret = serialization::encode_i32(buf, buf_len, pos, index_phase_)))
  {
    YYSYS_LOG(WARN, "fail to serialize:ret[%d]", ret);
  }
  //add:e
...
DEFINE_DESERIALIZE(IndexBeat)
{
  ...
  //add liumz, [static_index range_intersect]20170915
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS != (ret = serialization::decode_i32(buf, data_len, pos, reinterpret_cast<int32_t*>(&ip))))
    {
      YYSYS_LOG(WARN, "deserialize error here");
    }
    else
