 ...
 *
 * ITEM_SIZE Ϊƽ��ÿ���������Ĵ�С
 * ��û�����������¿�����Ϊsizeof(Key) + sizeof(Value)
 *
 * ��������������̭ BLOCK_SIZE ����ȡĬ��ֵΪ1M
 * ������󲻿�������̭ BLOCK_SIZE ȡֵΪITEM_SIZE�Ĵ�С
 *
 * BLOCK_SIZE ȡֵС��128K��ʱ�� Ϊ�˱������С�ڴ�ķ���
 * ��Ҫʹ�� KVStoreCacheComponent::MultiObjFreeList ��Ϊ�ڴ������
 * �������ʹ��Ĭ�ϵ� KVStoreCacheComponent::SingleObjFreeList ��Ϊ�ڴ������
 *
 * Authors:
    ...
    namespace KVStoreCacheComponent
    {
      ...
      inline ObRowkey *do_copy(const ObRowkey &other, char *buffer, ObRowkeyDeepCopyTag)
      {
        ...
        if (NULL != ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "deep_copy rowkey fail ret=%d", tmp_ret);
            ret = NULL;
          }
        }
        return ret;
      ...
      {
          void *alloc(const int32_t nbyte)
          { return ob_tc_malloc(nbyte, ObModIds::OB_KVSTORE_CACHE); };
          void free(void *ptr)
          { ob_tc_free(ptr, ObModIds::OB_KVSTORE_CACHE); };
      };
        ...
        public:
          ...
          bool check_seq_num_and_inc_ref_cnt(const int32_t seq_num)
          {
            ...
            while (true)
            {
              ...
              {
                YYSYS_LOG(ERROR, "unexpect ref_cnt=%d seq_num=%d this=%p info_pos=%ld",
          ...
          int store(const Key &key, const Value &value, int32_t &pos)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              ...
              if (NULL == new_key
                  || NULL == new_value)
              {
                YYSYS_LOG_US(WARN, "copy fail, new_key=%p new_value=%p buffer_pos=%d pairs_pos=%d "
          ...
          int get_next(int32_t &pos, Key *&pkey, Value *&pvalue)
          {
            ...
            else
            {
              ...
              {
                YYSYS_LOG(ERROR, "invalid kv pair magic, expected=%u, real=%u",
          ...
          int alloc(const int64_t align_size)
          {
            ...
            if (!UseStaticBuf)
            {
              ...
              if (NULL == buffer_)
              {
                ...
                if (NULL == buffer_)
                {
                  YYSYS_LOG(ERROR, "allocate memory for memblock failed");
                  ret = OB_ERROR;
                }
              }
            }
            else
            {
              if (align_size > MEM_BLOCK_SIZE)
              {
                YYSYS_LOG(ERROR, "the static buffer size is not enough, alloc_size=%ld, "
      ...
      class MultiObjFreeListTemp
      {
        ...
        public:
          ...
          void clear()
          {
            if (0 != alloc_size_)
            {
              YYSYS_LOG(ERROR, "alloc_cnt=%ld not equal zero", alloc_size_);
            }
            else
            {
              allocator_.clear();
          ...
          T *alloc(const int64_t align_size)
          {
            ...
            {
              YYSYS_LOG(WARN, "invalid param, align_size=%ld, block_size=%ld",
      ...
      class SingleObjFreeListTemp
      {
        ...
        public:
          T *alloc(const int64_t align_size)
          {
            ...
            if (align_size <= 0)
            {
              YYSYS_LOG(WARN, "alloc size is less than 0, size = %ld", align_size);
            }
            else
            {
            ...
            //YYSYS_LOG_US(DEBUG, "alloc %p", ret);
            return ret;
          };
          void free(T *ptr)
          {
      ...
      public:
        int init(const int64_t total_size)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "failed to allocate memory for memblock info");
        ...
        int enlarge_total_size(const int64_t total_size)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(INFO, "cache size enlarged, new_cache_size=%ld, old_cache_size=%ld",
            ...
            {
              YYSYS_LOG(WARN, "can't resize cache size with less total size, "
            ...
            {
              YYSYS_LOG(INFO, "cache size doesn't change, old_cache_size=%ld, cache_size=%ld",
                        total_mb_num_ * MemBlock::MEM_BLOCK_SIZE, total_size);
            }
            else
            {
              YYSYS_LOG(INFO, "cache size can not change, max_cache_size=%ld, cache_size=%ld",
        ...
        int destroy()
        {
          ...
          {
            YYSYS_LOG(WARN, "not_revert_cnt=%ld someone has got key-value but not revert", not_revert_cnt_);
            ret = OB_ERROR;
          }
          else
          {
              ...
              switch (stat)
              {
                case StoreHandle::FREE:
                {
                           ...
                           && NULL != cur_memblock_)
                  {
                    ...
                    YYSYS_LOG(DEBUG, "start scan cur_memblock");
                  }
                  else
                  {
                    ret = OB_ITER_END;
                ...
                default:
                  YYSYS_LOG(WARN, "store_handle invalid stat=%d", stat);
                  ret = OB_INVALID_ARGUMENT;
                  loop = false;
                  break;
              }
      ...
      private:
        ...
        void update_mb_info_(MemBlock *memblock)
        {
          ...
          else if (-1 == info_pos)
          {
            ...
            memblock->set_last_time(yysys::CTimeUtil::getTime());
          }
          else
          {
            YYSYS_LOG_US(INFO, "maybe block has already been washed out, pos=%ld memblock=%p info_memblock=%p",
        ...
        int64_t wash_out_timeout_(const int64_t min_free_size)
        {
          ...
          int64_t sort_timeu = yysys::CTimeUtil::getTime();
          ...
          sort_timeu = yysys::CTimeUtil::getTime() - sort_timeu;

          int64_t free_timeu = yysys::CTimeUtil::getTime();
          for (int64_t i = 0, j = 0; i < num && j < HEAP_SIZE / 2; i++)
          {
                ...
                && old == (MemBlock*)atomic_compare_exchange((uint64_t*)&(mb_infos_[pos].mem_block), (uint64_t)NULL, (uint64_t)old))
            {
              ...
              YYSYS_LOG_US(DEBUG, "try to free memblock=%p", old);
          ...
          free_timeu = yysys::CTimeUtil::getTime() - free_timeu;

          YYSYS_LOG_US(DEBUG, "sort_timeu=%ld free_timeu=%ld", sort_timeu, free_timeu);
        ...
        MemBlock *get_cur_memblock_(int32_t &seq_num, const int64_t align_kv_size)
        {
          ...
          {
            YYSYS_LOG_US(WARN, "cann't allocate memblock from free list, kv size is bigger "
            ...
            while (true)
            {
                      ...
                      && (NULL == (ret = cur_memblock_) || !ret->check_and_inc_ref_cnt())))
              {
                ...
                while (NULL == new_memblock)
                {
                  ...
                  YYSYS_LOG_US(DEBUG, "wash out alloc_size_before=%ld alloc_size_after=%ld timeu=%ld",
                               alloc_cnt_before, free_list_.get_alloc_size(), yysys::CTimeUtil::getTime() - timeu);
                  new_memblock = free_list_.alloc(align_kv_size);
                  if (0 == wash_out_size && NULL == new_memblock)
                  {
                    YYSYS_LOG_US(WARN, "can't wash out some memblock, alloc_size=%ld, "
                ...
                // �ڶ��μ����ü��� ��ʾget_cur_memblock_Ҳ�Ƕ����block������
                // ���������߳���get_cur_memblock����ǰ�Ͱ����block���� ������̭�ͷŵ�
                new_memblock->inc_ref_cnt();
                if (align_kv_size > MemBlock::MEM_BLOCK_SIZE)
                {
                  /**
        ...
        void submit_memblock_(MemBlock *submit_memblock)
        {
          ...
          if (i < total_mb_num_)
          {
            ...
            mb_infos_[i].last_time = yysys::CTimeUtil::getTime();
            submit_memblock->set_info_pos(i);
          }
          else
          {
            YYSYS_LOG_US(WARN, "cannot find mb_info for cur_memblock=%p, will wash_out "
    ...
    class KeyValueCache
    {
      ...
      public:
        int init(const int64_t total_size)
        {
          ...
          {
            YYSYS_LOG(WARN, "init store fail ret=%d size=%ld", ret, total_size);
          }
          else if (0 != (hash_ret = map_.create(total_size / ItemSize)))
          {
            store_.destroy();
            YYSYS_LOG(WARN, "create map fail ret=%d num=%ld", hash_ret, total_size / ItemSize);
            ret = OB_ERROR;
          }
          else
          {
        ...
        int enlarge_total_size(const int64_t total_size)
        {
          ...
          {
            YYSYS_LOG(WARN, "enlarge total memory size of store cache fail, ret=%d, size=%ld",
                      ret, total_size);
          }
          return ret;
        }
        ...
        int internal_put(const Key &key, const Value &value, StoreHandle& store_handle, bool overwrite = true)
        {
          ...
          else if (!overwrite && hash::HASH_EXIST == (ret = map_.get(key, store_handle)))
          {
            ret = OB_ENTRY_EXIST;
                   ...
                   || NULL == pkey)
          {
            YYSYS_LOG(WARN, "store key-value fail ret=%d", ret);
            if (OB_SUCCESS == ret)
            {
      ...
      public:
                          ...
                          CacheHandle &handle, bool overwrite = true, bool only_cache = true)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to put key-value into kvcache, ret=%d", ret);
          }
          return ret;
        };
...
#endif
        {
          ...
          {
            //YYSYS_TRACE_LOG("kv_store_cache miss key=[%s]", KVStoreCacheComponent::log_str(key));
            ret = OB_ENTRY_NOT_EXIST;
          }
          else
          {
            ...
            do
            {
                  ...
                  || NULL == pkey || NULL == pvalue)
              {
                ...
                //YYSYS_TRACE_LOG("kv_store_cache miss and error key=[%s]", KVStoreCacheComponent::log_str(key));
              }
          ...
          if (OB_SUCCESS == ret)
          {
            //YYSYS_TRACE_LOG("kv_store_cache hit key=[%s]", KVStoreCacheComponent::log_str(key));
            //value = *pvalue;
#ifdef __DEBUG_TEST__
            //assert(value == cv);
#endif
