   ...
   freelist_(NULL)
{
  ...
  {
    YYSYS_LOG(WARN, "obj size larger than block size, obj_size=%ld block_size=%ld", obj_size_, block_size_);
...
ObPool::~ObPool()
{
  //YYSYS_LOG(DEBUG, "pool free blocks, count=%ld", blocks_.count());
...
void ObPool::alloc_new_block()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to push back block");
    }
    const int64_t obj_count = block_size_ / obj_size_;
    if (0 >= obj_count)
    {
      YYSYS_LOG(ERROR, "invalid block size=%ld", block_size_);
    }
    else
    {
