                              ...
                              const uint32_t max_concurrency_count)
{
  ...
  {
    ret = OB_INIT_TWICE;
    YYSYS_LOG(WARN, "ob_file_service is inited twice");
  }

  if(OB_SUCCESS == ret && NULL == server)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "server is null");
  }

  if(OB_SUCCESS == ret && NULL == queue_thread)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "queue_thread is null");
  }

  if (OB_SUCCESS == ret && network_timeout < MIN_NETWORK_TIMEOUT)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "Network_timeout should not less than %ld",
              MIN_NETWORK_TIMEOUT);
  }
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "max_concurrency_count for file_service should not less than 0");
  }
    ...
    common::ObDataBuffer& out_buffer)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "ob file client is not inited yet.");
  }

  if (OB_SUCCESS == ret && NULL == request)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "Connection param in receive_file_loop should not be null");
  }
  ...
  {
    src_ip = "";
    YYSYS_LOG(WARN, "can't get src ip for send_file_request");
  }

  if (DEFAULT_VERSION != version)
  {
    ret = OB_ERROR_FUNC_VERSION;
    YYSYS_LOG(ERROR, "Can'd handle send_file_request in different version:"
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Encode result code failed:ret=[%d]", err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "Send error message for OB_SEND_FILE_REQUEST failed:"
                  "ret[%d]", err);
      }
    }
  }
  ...
  int64_t t1 = yysys::CTimeUtil::getTime();
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "Prepare for receive_file failed: ret=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "Reveive_file_loop failed:ret=[%d]", ret);
    }
  }

  int64_t duration = yysys::CTimeUtil::getTime() - t1;
  if (OB_SUCCESS == ret && duration == 0)
  {
    YYSYS_LOG(ERROR, "duration of recieve time should not be zero");
    ret = OB_ERR_UNEXPECTED;
  }

  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "Recieve file from server[%s] to file_path[%.*s] "
  ...
  {
    YYSYS_LOG(ERROR, "Recieve file from server[%s] to file_path[%.*s] "
...
int ObFileService::inc_concurrency_count()
{
  ...
  {
    YYSYS_LOG(WARN, "Concurrency file trans count[%d] reach the threshold:[%d]. "
              "New send file request is refused.", old_concurrency_count, max_concurrency_count_);
    ret = OB_EAGAIN;
  }
  //YYSYS_LOG(INFO, "inc Concurrency file trans count[%d].", concurrency_count_);
  return ret;
}
...
void ObFileService::dec_concurrency_count()
{
  ...
  {
    YYSYS_LOG(ERROR, "can't decrease concurrency count[%u]", concurrency_count_);
  }
  else
  {
    atomic_dec(&concurrency_count_);
  }
  //YYSYS_LOG(INFO, "dec Concurrency file trans count[%d].", concurrency_count_);
}
                                    ...
                                    ObDataBuffer& out_buffer, int32_t& response_cid, const int64_t session_id)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Decode file_info failed: err=[%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Decode dest_dir failed:err=[%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Decode dest_file_name failed:err=[%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Check dir failed:err=[%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (count<0 || count >= OB_MAX_FILE_NAME_LENGTH)
    {
      ...
      YYSYS_LOG(WARN, "snprintf file name failed, return[%ld] [%.*s]/[%.*s]",
    ...
    {
      err = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "Write file_path_buf to ObString failed");
    }
    ...
    {
      err = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "snprintf tmp file name failed, return[%ld] [%.*s]/[%s][%.*s]",
    ...
    {
      err = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "Write tmp_file_path_buf to ObString failed");
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "Tmp file [%s] already exists", tmp_file_path_buf);
      err = OB_FILE_ALREADY_EXIST;
    }
    if (0 == access(file_path_buf, F_OK))
    {
      YYSYS_LOG(WARN, "File [%s] already exists", file_path_buf);
      err = OB_FILE_ALREADY_EXIST;
    }
  }
  // log operation
  if (OB_SUCCESS == err)
  {
    YYSYS_LOG(INFO, "start receive file: dir[%.*s] file_name[%.*s] "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "open file_appender failed:tmp_file[%.*s] err[%d]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "Serialize result code failed:ret=[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "prepare for next request fail:ret=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "Send send_file_request_response failed:ret=[%d]", ret);
    }
  }
                                      ...
                                      ObDataBuffer& out_buffer, int32_t & response_cid, const int64_t session_id)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Decode offset failed: err=[%d]", err);
    }
  }
  if (OB_SUCCESS == err)
  {
                                           ...
                                           block_buf, block_size_, &read_size))
    {
      ...
      YYSYS_LOG(WARN, "Decode block failed");
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Appender file block failed:err=[%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "Encode result code failed:ret=[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "Send send_file_end_response failed:ret[%d]", ret);
    }
  }
                                    ...
                                    int32_t& response_cid, const int64_t session_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to get tmp_file_path length[%d] [%.*s]",
              n, tmp_file_path.length(), tmp_file_path.ptr());
    ret = OB_SIZE_OVERFLOW;
  }
  else if (stat(tmp_path_buf, &file_stat) != 0)
  {
    YYSYS_LOG(ERROR, "stat tmp_file_path[%s] failed: %s",
  ...
  {
    YYSYS_LOG(ERROR, "failed to get path_buf length[%d] [%.*s]",
  ...
  {
    YYSYS_LOG(ERROR, "The size of received tmp file size[%ld], "
              "remote file size[%ld]", file_stat.st_size, file_size);
    ret = OB_INVALID_DATA;
  }
  if (OB_SUCCESS == ret && 0 != rename(tmp_path_buf, path_buf))
  {
    YYSYS_LOG(ERROR, "Rename [%s] to path[%s] failed: errno[%d] %s",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "Send send_file_end_response failed:ret[%d]", err);
    }
  }
                                     ...
                                     int32_t& response_cid, const int64_t session_id)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "File appender is not opened yet in receive_file_loop");
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if(NULL == block_buf)
    {
      ...
      YYSYS_LOG(WARN, "Allocate memory for block buffer in receive_file_loop"
                " failed:err[%d]", ret);
    }
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "wait for next request fail:ret[%d] "
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "Deserialize next request failed:ret[%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (NULL == in_buffer)
      {
        ...
        YYSYS_LOG(WARN, "Get buffer for next request failed");
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "Decode result code failed: ret=[%d]", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      if (OB_SUCCESS == rc.result_code_) // receive file block
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to receive_file_block");
        }
      ...
      else if (OB_ITER_END == rc.result_code_) // the whole file is received
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to send receive_file_end");
        }
...
int ObFileService::check_dir(const ObString& local_dir, const int64_t file_size)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_SIZE_OVERFLOW;
      YYSYS_LOG(ERROR, "local_dir length is overflow, length[%d] str[%.*s]",
                local_dir.length(), local_dir.length(), local_dir.ptr());
    }
    else if (stat(path_buf, &dir_stat) != 0)
    {
      YYSYS_LOG(WARN, "get local directory[%s] stat fail: %s",
                path_buf, strerror(errno));
      ret = OB_IO_ERROR;
    }
    else if (!(S_IWUSR & dir_stat.st_mode))
    {
      YYSYS_LOG(WARN, "local dir[%s] cannot be written", path_buf);
      ret = OB_IO_ERROR;
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "get disk free space of %s info fail: %s",
                path_buf, strerror(errno));
      ret = OB_IO_ERROR;
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "free disk space of %s is not enough: file_size[%ld]"
