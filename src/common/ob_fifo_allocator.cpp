...
namespace oceanbase
{
  namespace common
  {
    ...
    __thread uint64_t FIFOAllocator::thread_alloc_page_num_ = 0;
    __thread uint64_t FIFOAllocator::thread_free_page_num_ = 0;
    //add for [461-ups sessionctx mem log]-e
    FIFOAllocator::FIFOAllocator() : inited_(false),
      mod_id_(0),
      id_map_(),
      free_list_(),
      //thread_node_key_(0),  //del hongchen [TSD_USER_IMPL] 20170916
      thread_node_lock_(),
      ...
      allocated_size_(0)
    {
      ...
      PTHREAD_KEY_MGR->create_thread_key(thread_node_key_);
    ...
    FIFOAllocator::~FIFOAllocator()
    {
      ...
      PTHREAD_KEY_MGR->delete_thread_key(thread_node_key_);
      //mod hongchen [TSD_USER_IMPL] 20170916:e
    }
                            ...
                            const int64_t page_size)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(WARN, "have inited, this=%p", this);
        ret = OB_INIT_TWICE;
      }
      ...
      {
        YYSYS_LOG(WARN, "invalid param total_limit=%ld hold_limit=%ld page_size=%ld",
      ...
      {
        YYSYS_LOG(WARN, "init id_map fail, ret=%d total_limit=%ld page_size=%ld",
                  ret, total_limit, page_size);
      }
      else if (OB_SUCCESS != (ret = free_list_.init(hold_limit / page_size)))
      {
        YYSYS_LOG(WARN, "init free_list fail, ret=%d old_size=%ld page_size=%ld",
    ...
    void FIFOAllocator::destroy()
    {
      ...
      if (0 != id_map_.size())
      {
        YYSYS_LOG(ERROR, "still memory have not been free: ref_page_cnt=%d", id_map_.size());
    ...
    FIFOAllocator::Page *FIFOAllocator::get_page_(const int64_t require_size, uint64_t &id)
    {
      ...
      ThreadNode *thread_node = (ThreadNode*)PTHREAD_KEY_MGR->get_thread_specifi(thread_node_key_);
      if (NULL == thread_node)
      {
        ...
        if (NULL != thread_node)
        {
          ...
          //if (0 != tmp_ret)
          int tmp_ret = PTHREAD_KEY_MGR->set_thread_specifi(thread_node_key_, thread_node);
          if (OB_SUCCESS != tmp_ret)
          {
            //YYSYS_LOG(WARN, "setspecific fail, ret=%d key=%d thread_node=%p",
            //    tmp_ret, thread_node_key_, thread_node);
            YYSYS_LOG(WARN, "setspecific fail, ret=%d key.seq_=%ld thread_node=%p",
                      tmp_ret, thread_node_key_.seq_, thread_node);
          }
          else
          {
            while (true)
            ...
            {
              YYSYS_LOG(WARN, "total_limit cannot support at least one page for each thread, will force to modify total_limit from %ld to %ld",
      ...
      if (NULL != thread_node)
      {
        ...
        if (NULL == tmp_page)
        {
          ...
          if (NULL != tmp_page)
          {
            ...
            {
              YYSYS_LOG(WARN, "assign from id_map_ fail, page=%p", tmp_page);
              free_page_(tmp_page);
              tmp_page = NULL;
            }
          }
    ...
    void FIFOAllocator::revert_page_(const uint64_t id, void *ptr)
    {
      ...
      if (NULL == page)
      {
        YYSYS_LOG(ERROR, "get page fail, maybe double free, ptr=%p id=%lu", ptr, id);
      }
      else
      {
    ...
    FIFOAllocator::Page *FIFOAllocator::alloc_page_()
    {
      ...
      if (total_limit_ >= ATOMIC_ADD(&allocated_size_, page_size_))
      {
        ...
        if (NULL == ret)
        {
          ...
          if (NULL == ret)
          {
            YYSYS_LOG(WARN, "alloc from system fail size=%ld", page_size_ + sizeof(Page));
          }
        }
        if (NULL != ret)
        {
          ...
          ret->pos = 0;
          //add for [461-ups sessionctx mem log]-b
          ++thread_alloc_page_num_;
          if(ObModIds::OB_UPS_SESSION_CTX == mod_id_ && TC_REACH_TIME_INTERVAL(600L * 1000000L))
          {
            YYSYS_LOG(INFO, "OB_UPS_SESSION_CTX mod: free_list=%ld, alloc_page_num=%ld, allocated_size=%ld, "
                      "thread_alloc_page_num=%lu, thread_free_page_num=%lu"
                      ,free_list_.get_total(), allocated_size_/page_size_, allocated_size_
                      ,thread_alloc_page_num_, thread_free_page_num_);
          }
          //add for [461-ups sessionctx mem log]-e
        }
        else
        {//add zhaoqiong [fifo allocator bug] 20161118:b
          YYSYS_LOG(ERROR, "alloc failed, allocated_size=[%ld] and total_limit=[%ld]", allocated_size_,total_limit_);
          //add:e
        ...
        {
          YYSYS_LOG(ERROR, "allocated_size[%ld] has reached the warn threshold, total_limit[%ld]", allocated_size_,total_limit_);
        }
        //add:e
      }
      else
      {
        //add zhaoqiong [fifo allocator bug] 20161118:b
        YYSYS_LOG(ERROR, "alloc failed, allocated_size[%ld] exceeds total_limit[%ld]", allocated_size_,total_limit_);
        //add:e
        ATOMIC_ADD(&allocated_size_, -page_size_);
      }
      return ret;
...
//add support for arm platform by wangd 202106:b
      ...
      int64_t size_align = ((size + (int64_t)sizeof(id)) + 7) & ~(7);
#else
      int64_t size_align = size + (int64_t)sizeof(id);
#endif
//add support for arm platform by wangd 202106:e
      if (!inited_)
      {
        YYSYS_LOG(WARN, "have not inited, this=%p", this);
               ...
               || page_size_ < size_align) //mod support for arm platform by wangd 202106
      {
        YYSYS_LOG(WARN, "invalid param, size=%ld", size);
      }
      else
      {
        Page *page = get_page_(size_align, id); //mod support for arm platform by wangd 202106
        if (NULL != page)
        {
          *(uint64_t*)(page->buf + page->pos) = id;
          ret = page->buf + page->pos + sizeof(id);
          ATOMIC_ADD(&(page->ref_cnt), 1);
          page->pos += (uint32_t)(size_align); //mod support for arm platform by wangd 202106
        }
      }
      return ret;
    }
    ...
    void FIFOAllocator::free(void *ptr)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, null pointer");
      }
      else
      {
