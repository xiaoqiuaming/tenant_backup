...
#define ENABLE_MALLOC_BT 0
    char* parray(char* buf, int64_t len, int64_t* array, int size)
    {
      ...
      for(int64_t i = 0; i < size; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "buf not enough, len=%ld, array_size=%d", len, size);
          break;
        }
      }
      buf[pos + 1] = 0;
...
#undef BACKTRACE
#define BACKTRACE(level, cond, format...) { char buf[1024]; YYSYS_LOG(level, format); YYSYS_LOG(ERROR, "backtrace: %s", lbt(buf, 1024)); }
    void TSIBlockCache::Block::print_bt()
    {
      ...
      YYSYS_LOG(INFO, "alloc_bt: %s free_bt: %s", buf1, buf2);
    }
    ...
    void TSIBlockCache::clear_block_list(BlockList* block_list)
    {
      ...
      if (NULL != block_list)
      {
        ...
        while(NULL != (block = block_list->pop()))
        {
          ...
          {
            YYSYS_LOG(WARN, "global_block_list.push(%p)=>%d, free to OS", block, err);
            free_block(block);
          }
        }
        if (OB_SUCCESS != (err = block_list_pool_.push(block_list)))
        {
          YYSYS_LOG(ERROR, "block_list_pool.push(%p)=>%d", block_list, err);
        }
      }
    }
    ...
    TSIBlockCache::Block* TSIBlockCache::get()
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_block_list()=>NULL");
      }
    ...
    int TSIBlockCache::put(Block* block, const int64_t limit, const int64_t global_limit)
    {
      ...
      {
        YYSYS_LOG(ERROR, "block_list->push(%p, limit=%ld)=>%d", block, limit, err);
      }
      ...
      {
        free_block(block);
        YYSYS_LOG(WARN, "global_block_list->push(%p, %ld)=>%d", block, global_limit, err);
      }
      return err;
    }
    ...
    ObTSIBlockAllocator::Block* ObTSIBlockAllocator::alloc_block(const int64_t size)
    {
      ...
      {
        YYSYS_LOG(ERROR, "alloc(%ld)=>NULL", size + sizeof(*p));
      }
      else
      {
        new(p) Block(size);
...
//add support for arm platform by wangd 202106:e
      ...
      else if (size_align <= 0)//mod support for arm platform by wangd 202106
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "size_align[%ld] <= 0", size_align);//mod support for arm platform by wangd 202106
      }
      else if (normal_block_size_ >= size_align)//mod support for arm platform by wangd 202106
      {
        alloc_size = normal_block_size_;
        block = normal_block_cache_.get();
      }
      else if (medium_block_size_ >= size_align)//mod support for arm platform by wangd 202106
      {
        alloc_size = medium_block_size_;
        block = medium_block_cache_.get();
      }
      else if (big_block_size_ >= size_align)//mod support for arm platform by wangd 202106
      {
      ...
      {
        alloc_size = size_align;//mod support for arm platform by wangd 202106
        //YYSYS_LOG(TRACE, "alloc big block size[%ld]", size);
      }
      ...
      if (NULL != block)
      {
        block->real_size_ = size_align;//mod support for arm platform by wangd 202106
        block->mod_ = mod_id;
        after_alloc(block);
        g_malloc_size_stat.add(size_align);//mod support for arm platform by wangd 202106
    ...
    ObTSIBlockAllocator::Block* ObTSIBlockAllocator::mod_realloc_block(Block* old_block, int64_t size, const int32_t mod_id)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "realloc(block=%p, size[%ld] < 0): INVALID_ARGUMENT", old_block, size);
      }
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "block->check_checksum() fail");
      }
      ...
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(ERROR, "mod_alloc(%ld, %d): MEMORY_OVERFLOW", size, mod_id);
      }
    ...
    void ObTSIBlockAllocator::mod_free_block(Block* block, const int32_t mod_id)
    {
      ...
      {
        YYSYS_LOG(TRACE, "size[%ld] not equal %ld or %ld", block->size_, normal_block_size_, big_block_size_);
      }
      if (OB_SUCCESS == err && NULL != block)
      {
        free_block(block);
...
//add support for arm platform by wangd 202106:e
      ...
      Block* new_block = mod_realloc_block(old_block, size_align, mod_id);//mod support for arm platform by wangd 202106
