...
ObTableIdNameIterator::ObTableIdNameIterator()
  :need_scan_(false), only_core_tables_(true),
    table_idx_(-1), client_proxy_(NULL), res_(NULL), base_version_(0), end_version_(0), ddl_res_(NULL)//add zhaoqiong [Schema Manager] 20150327
{
}
...
bool ObTableIdNameIterator::check_inner_stat()
{
  ...
  else
  {
    if (need_scan_)
    {
      ...
      //      {
      //        YYSYS_LOG(WARN, "nb accessor scan fail:ret[%d]", err);
      //        ret = false;
      //      }
      //      else
      //      {
      //        YYSYS_LOG(DEBUG, "scan first_tablet_table success. scanner row count =%ld", res_->get_scanner()->get_row_num());
      //        need_scan_ = false;
      //      }
      int err = OB_SUCCESS;
...
//mod:e
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if (only_core_tables == false)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "init nb_accessor fail:ret[%d]", ret);
      }
      else
      {
...
int ObTableIdNameIterator::get_table_list(ObArray<int64_t> & create_table_id, ObArray<int64_t> & drop_table_id, bool &refresh_schema)
{
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "next row fail:ret[%d]", ret);
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(NULL != cell_info)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get operation_type fail, ret = %d",ret);
        }
        else if (REFRESH_SCHEMA == type)
        {
          refresh_schema = true;
          YYSYS_LOG(INFO, "refresh schema");
          break;
        }
      }
      else
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "get operation_type cell info fail");
      }
    }
    ...
    if(OB_SUCCESS == ret && !refresh_schema)
    {
      ...
      if(NULL != cell_info)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "get table_id fail, ret = %d",ret);
        }
        else
        {
          switch(type)
          {
            ...
            default:
              ret = OB_ERROR;
              YYSYS_LOG(ERROR, "unknow type:%ld",type);
              break;
          }
        }
      }
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "get table_id cell info fail");
      }
    }
  }
...
int ObTableIdNameIterator::get_latest_schema_version(int64_t& latest_schema_version)
{
  ...
  {
    YYSYS_LOG(WARN, "nb accessor scan fail:ret[%d]", ret);
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(NULL != cell_info)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get schema_version fail, ret = %d",ret);
        }
  ...
  {
    //for upgrade
    latest_schema_version = yysys::CTimeUtil::getTime();
    ret = OB_SUCCESS;
  }
...
int ObTableIdNameIterator::next()
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ++table_idx_;
      YYSYS_LOG(DEBUG, "table_idx=%d", table_idx_);
      //mod zhaoqiong [Schema Manager] 20150327:b
      //if (table_idx_ < 3)
...
int ObTableIdNameIterator::get(ObTableIdName** table_info)
{
  ...
  else
  {
    if (only_core_tables_ == true)
    {
      ...
      YYSYS_LOG(WARN, "get core tables but table_idx[%d]_ >= 3", table_idx_);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "table_name: [%.*s], table_id: [%ld]",
...
int ObTableIdNameIterator::normal_get(ObTableIdName** table_id_name)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(NULL != cell_info)
    {
          ...
          && cell_info->row_key_.get_obj_ptr()[0].get_type() == ObVarcharType)
      {
        ...
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "value is null,  get table name from cell rowkey:%s", print_cellinfo(cell_info));
        //modify dolphin [database manager]@20150611   //[conflict_level A]
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "get table_name cell info fail");
    }
  }
  //add dolphin [database manager]@20150611
  if(OB_SUCCESS == ret)
  {
    ...
    if(NULL != cell_info)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN, "db_name value is null,  get database name from cell rowkey:%s", print_cellinfo(cell_info));
      }
      else
      {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "get db_name cell info fail");
    }
  }
  //add:e
