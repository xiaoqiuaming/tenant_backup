...
namespace oceanbase
{
  namespace common
  {
    ...
    // ʹ��һ������ĸ���buffer ����dioд�� buffer���ܴ���Ϊ�˶�������������� �����Ҫ����ʹ���buffer_pos
    ...
    class ObFileAppender : public ObIFileAppender
    {
      ...
      public:
        // �����������append fsync
        // ���û������is_fsync ��ôĬ��ʹ���ڲ�buffer�������� ֱ��buffer����ˢ����
        int append(const void *buf, const int64_t count, const bool is_fsync);
        int fsync();
      public:
        // ��ʱ��ʵ��
...
#define __USE_AIO_FILE
      ...
      used_(0)
    {
      ...
      {
        YYSYS_LOG(ERROR, "alloc obj array fail");
      }
      else
      {
    ...
    ObWaitablePool<T, SIZE>::~ObWaitablePool()
    {
      ...
      if (NULL != objs_)
      {
        if (SIZE != counter)
        {
          YYSYS_LOG(ERROR, "still have %ld node not been free, memory=%p will leek", SIZE - counter, objs_);
        }
        else
        {
          ob_free(objs_);
