...
int ObMutatorHelper::add_column(const char* table_name, const ObRowkey& rowkey, const char* column_name, const ObObj& value, ObMutator* mutator)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "add value to mutator fail:table_name[%s], column_name[%s]", 
...
int common::kv_to_rowkey(const KV& kv, ObRowkey& rowkey, ObObj* rowkey_obj, int32_t obj_buf_count)
{
  ...
  if(OB_SUCCESS != kv.get_exec_status())
  {
    ...
    YYSYS_LOG(WARN, "kv.get_exec_status[%d]", kv.get_exec_status());
  }

  if(OB_SUCCESS == ret && kv.count() > obj_buf_count)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "kv.count[%ld] > obj_buf_count[%d]", kv.count(), obj_buf_count);
  }
  ...
  for(int32_t i=0;i<kv.count() && OB_SUCCESS == ret;i++)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "get element from kv array fail");
    }
    if(OB_SUCCESS == ret)
    {
...
int ObMutatorHelper::insert(const char* table_name, const ObRowkey& rowkey, const KV& kv)
{
  ...
  {
    ret = kv.get_exec_status();
    YYSYS_LOG(WARN, "kv array error:ret[%d]", ret);
  }
  ...
  for(int32_t i=0;i<kv.count() && OB_SUCCESS == ret;i++)
  {
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add column fail:ret[%d]", ret);
      }
    }
  }
  return ret;
...
int ObMutatorHelper::del_row(const char* table_name, const ObRowkey& rowkey)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "add del row info to mutator_ fail:ret[%d]", ret);
    }
  }
  return ret;
}
