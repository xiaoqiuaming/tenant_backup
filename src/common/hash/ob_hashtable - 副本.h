    ...
    namespace hash
    {
      ...
      class ObHashTable
      {
        ...
        private:
          ...
          {
              int ret = HASH_NOT_EXIST;
              hashnode *node = ((bucket_node != NULL) ? bucket_node->next : bucket_node);
              is_fake = false;
          ...
          inline int internal_get_multiple(const hashbucket &bucket, hashnode *&bucket_node, const_key_type &key, _value_type *&value, bool &is_fake) const
          {
              int ret = HASH_NOT_EXIST;
              hashnode *node = ((bucket_node != NULL) ? bucket_node->next : bucket_node);
              is_fake = false;
              ...
              while(NULL != node)
              {
                  if(_equal_(getkey_(node->data), key))
                  {
                      value = &(node->data);
                      is_fake = node->is_fake;
                      bucket_node = node;
          ...
          inline int internal_get_all(const hashbucket &bucket, hashnode*& bucket_node, _value_type &value, bool &is_fake) const
          {
              int ret = HASH_NOT_EXIST;
              hashnode *node = ((bucket_node != NULL) ? bucket_node->next : bucket_node);
              is_fake = false;
          ...
          inline int internal_get_all(const hashbucket &bucket, hashnode*& bucket_node, const _value_type *&value, bool &is_fake) const
          {
              int ret = HASH_NOT_EXIST;
              hashnode *node = ((bucket_node != NULL) ? bucket_node->next : bucket_node);
              is_fake = false;
          ...
          // add for [hash join single]
          inline int internal_modify_multiple(hashnode* bucket_node, const _value_type &value, const bool is fake)
          {
