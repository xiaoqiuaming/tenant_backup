...
#define HASH_WRITE_LOG(_loglevel_, _fmt_, args...) { \
  YYSYS_LOG(_loglevel_, _fmt_, ##args); \
}
#else
#define HASH_WRITE_LOG(_loglevel_, _fmt_, args...) { \
}
  ...
  namespace common
  {
    namespace hash
    {
      enum
      {
        ...
        HASH_INSERT_SUCC,
        HASH_GET_TIMEOUT,
        // add for [hash join single]
        HASH_MODIFY_SUCC,
        // add for [hash join single] end
      };
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&spin) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The SpinLocker address is not 8-bite alignment. Current address = [%p]", &spin);
            }
//add support for arm platform by wangd 202106:e
      ...
      class MutexLocker
      {
        ...
        public:
          ...
          {
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&mutex) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The MutexLocker address is not 8-bite alignment. Current address = [%p]", &mutex);
            }
//add support for arm platform by wangd 202106:e
      ...
      class ReadLocker
      {
        ...
        public:
          ...
          {
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&rwlock) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The ReadLocker address is not 8-bite alignment. Current address = [%p]", &rwlock);
            }
//add support for arm platform by wangd 202106:e
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&rwlock) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The WriteLocker address is not 8-bite alignment. Current address = [%p]", &rwlock);
            }
//add support for arm platform by wangd 202106:e
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&rwlock) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The RWLockIniter address is not 8-bite alignment. Current address = [%p]", &rwlock);
            }
//add support for arm platform by wangd 202106:e
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&rwlock) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The SpinReadLocker address is not 8-bite alignment. Current address = [%p]", &rwlock);
            }
//add support for arm platform by wangd 202106:e
            if (0 != rwlock.rdlock())
            {
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&rwlock) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The SpinWriteLocker address is not 8-bite alignment. Current address = [%p]", &rwlock);
            }
//add support for arm platform by wangd 202106:e
            if (0 != rwlock.wrlock())
            {
...
//            if ((reinterpret_cast<uint64_t>(&spin) % 8) != 0)
//            {
//              HASH_WRITE_LOG(HASH_FATAL, "The SpinIniter address is not 8-bite alignment. Current address = [%p]", &spin);
//            }
//add support for arm platform by wangd 202106:e
...
//add support for arm platform by wangd 202106:b
            if ((reinterpret_cast<uint64_t>(&mutex) % 8) != 0)
            {
              HASH_WRITE_LOG(HASH_FATAL, "The MutexIniter address is not 8-bite alignment. Current address = [%p]", &mutex);
            }
//add support for arm platform by wangd 202106:e
...
//            usr = &nlock;
      ...
      struct DefaultBigArrayAllocator
      {
        ...
        { return ob_tc_malloc(sz, ObModIds::OB_HASH_BUCKET); }
        void free(void *p)
        { ob_tc_free(p, ObModIds::OB_HASH_BUCKET); }
      };
      ...
      struct DefaultSimpleAllocerAllocator
      {
        public:
          ...
          void *alloc(const int64_t sz)
          { return ob_tc_malloc(sz, mod_id_); }
          void free(void *p)
          { ob_tc_free(p, mod_id_); }
          void set_mod_id (int32_t t)
          { mod_id_ = t; };
       private:
          int32_t mod_id_;
      };
      ...
      {
        static const int32_t NODE_NUM = static_cast<int32_t>((common::OB_TC_MALLOC_BLOCK_SIZE - 24/*=sizeof(SimpleAllocerBlock 's members except nodes_buffer)*/ - 128/*for robust*/)
...
//add support for arm platform by wangd 202106:b
          ...
          lock_type lock_ __attribute__((__aligned__(8)));
#else
//add support for arm platform by wangd 202106:e
          lock_type lock_;
