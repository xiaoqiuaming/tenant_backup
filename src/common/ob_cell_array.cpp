...
inline int ObCellArray::copy_obj_(ObObj &dst, const ObObj &src)
{
  ...
  if (src.get_type() == ObVarcharType)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (src_value.length() > 0)
      {
        ...
        if (NULL == tmp_buf)
        {
          YYSYS_LOG(WARN, "%s", "fail to malloc buffer for varchar value");
  ...
  else if(src.get_type() == ObDecimalType)
  {
    ...
    {
      //         dst = src;
      if (src_value.get_words() != NULL && len != 0)
      {
        uint64_t *ptr =reinterpret_cast<uint64_t*>( page_arena_.alloc(len));
        if (NULL == ptr)
        {
          YYSYS_LOG(WARN, "%s", "fail to malloc buffer for decimal value");
          err = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
          allocated_memory_size_ += len;
          memcpy(ptr, src_value.get_words()->ToUInt_v2(), len);
          dst.set_ttint(ptr);
...
int ObCellArray::copy_cell_(ObInnerCellInfo &dst, const ObCellInfo &src, const int64_t prev_cell_idx)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else if (src.row_key_.length() > 0)
    {
      ...
      if (err != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "deep copy new rowkey failed:err[%d]", err);
      }
      else
      {
...
int ObCellArray::ensure_current_block_space()
{
  ...
  if (NULL == current_block_)
  {
    ...
    {
      err = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "%s", "fail to allocate memory for cellinfo array");
    }
  }
    ...
    ObCellInfo **cells_in, ObInnerCellInfo **cells_out, const int64_t cell_size)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, cells_in=%p cells_out=%p, cell_size=%ld",
  ...
  for (int64_t i = 0; i < cell_size && OB_SUCCESS == err; i++)
  {
    ...
    if (OB_SUCCESS == err && NULL != cells_in[i])
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to copy cell, cell_size=%ld, index=%ld",
                  cell_size, i);
      }
    }
    else
    {
      YYSYS_LOG(WARN, "input cells to append is NULL or can't allocate memory "
...
int ObCellArray::apply(const ObCellInfo &src_cell, ObInnerCellInfo *& affected_cell)
{
  ...
  {
    YYSYS_LOG(ERROR, "should not goto this path:rowkey[%p], table[%lu], column[%lu]",
...
ObInnerCellInfo & ObCellArray::operator[](int64_t offset)
{
  ...
  if (OB_SUCCESS != err)
  {
    ...
    YYSYS_LOG(ERROR, "fail to get cell [offset:%ld,cell_num_:%ld,err:%d]", offset, cell_num_,
              err);
  }
  return *result;
}
...
int ObCellArray::get_cell(const int64_t offset, ObInnerCellInfo *&cell)const
{
  ...
  if (offset >= cell_num_ || offset < 0)
  {
    ...
    YYSYS_LOG(WARN,"try to access cell out of range [offset:%ld,cell_num_:%ld]",
              offset, cell_num_);
  }
  else
  {
    if (offset < O1_ACCESS_CELL_NUM)
    {
      ...
      {
        YYSYS_LOG(ERROR, "unexpected error, offset less than cell_num_, but out of range "
...
int  ObCellArray::unget_cell()
{
  ...
  {
    YYSYS_LOG(WARN,"cannot unget cell, param error [cur_row_consumed_cell_num_:%d,consumed_row_num_:%ld]",
...
int ObCellArray::next_cell(int64_t & cur_cell_offset)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to call next cell [err:%d]", err);
  }
  return err;
}
...
int ObCellArray::get_cell(ObInnerCellInfo **cell, bool * is_row_changed)
{
  ...
  if (NULL == cell)
  {
    YYSYS_LOG(WARN,"param error [cell:%p]", cell);
    err = OB_INVALID_ARGUMENT;
  }
    ...
    const int64_t desc_size, const int64_t sharding_row_cnt)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN,"param error [row_width:%ld,cell_num_:%ld]", row_width, cell_num_);
  }
  else if (row_width_ != cell_num_ && row_width_ != row_width)
  {
    YYSYS_LOG(WARN,"param error, row_width not coincident [row_width:%ld,row_width_:%ld,"
    ...
    err  = OB_INVALID_ARGUMENT;
           ...
           || sharding_row_cnt < 0)
  {
    YYSYS_LOG(WARN,"param error [order_desc:%p,desc_size:%ld,max_desc_size:%ld,"
              "sharding_row_cnt:%ld]",
  ...
  {
    YYSYS_LOG(WARN,"param error, desc_size should less or equal to row_width"
  ...
  else
  {
    for (int32_t i = 0; i < desc_size && OB_SUCCESS == err; i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"cell idx should less than row_width [order_desc_idx:%d,cell_idx:%d,"
  ...
  if (get_cell_size() > 0)
  {
    ...
    {
      YYSYS_LOG(WARN,"topk, orderby and reverse_rows can be only called once "
...
int oceanbase::common::ObCellArray::heap_topk(const int64_t sharding_row_cnt)
{
  ...
  {
    YYSYS_LOG(WARN, "heap topk is only used for case that "
  ...
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to allocate memory for heap buffer");
    }
    else
    {
...
int oceanbase::common::ObCellArray::reverse_rows(const int64_t row_width_in)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN,"param error [row_width_in:%ld,cell_num_:%ld]", row_width_in, cell_num_);
  }
  if (OB_SUCCESS == err && row_width_ != cell_num_ && row_width_ != row_width_in)
  {
    YYSYS_LOG(WARN,"param error, row_width not coincident [row_width_in:%ld,row_width_:%ld,"
  ...
  if (get_cell_size()>0)
  {
    if (1 != sorted_row_offsets_.size())
    {
      YYSYS_LOG(WARN,"orderby and reverse_rows can be only called once");
      err = OB_INVALID_ARGUMENT;
    }
    if (OB_SUCCESS == err)
    {
...
int ObCellArray::limit(int64_t offset, int64_t count, int32_t row_width)
{
  ...
  {
    YYSYS_LOG(WARN,"param error [offset:%ld,count:%ld,row_width:%d]", offset, count, row_width);
    err  = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err && row_width_ != cell_num_ && row_width_ != row_width)
  {
    YYSYS_LOG(WARN,"param error, row_width not coincident [row_width:%d,row_width_:%ld,"
              "cell_num_:%ld]", row_width, row_width_, cell_num_);
    err  = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err && cell_num_ % row_width != 0)
  {
    YYSYS_LOG(WARN,"param error, cells in array not compose rows [cell_num_:%ld,row_width:%d]",
              cell_num_, row_width);
    err  = OB_INVALID_ARGUMENT;
  }
...
bool ObCellArray::RowComp::operator()(int64_t off1, int64_t off2)
{
  ...
  while (0 == result && OB_SUCCESS == err && cur_column_idx < desc_size_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [cell_off:%ld,err:%d]", cell1_off, err);
      result = -1;
    }
    if (OB_SUCCESS == err)
    {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [cell_off:%ld,err:%d]", cell2_off, err);
      result = -1;
    }
    cur_column_idx ++;
  }
...
ObInnerCellInfo & ObCellArray::iterator::operator*()
{
  ...
  {
    YYSYS_LOG(ERROR, "logic error, try to access cell out of range "
  ...
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      ...
      YYSYS_LOG(ERROR, "unexpected error, fail to get cell [offset:%ld,cell_num_:%ld,err:%d]",
