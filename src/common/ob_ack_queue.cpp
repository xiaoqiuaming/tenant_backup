...
namespace oceanbase
{
  namespace common
  {
    ...
    {
      err_ = err;
      receive_time_us_ = yysys::CTimeUtil::getTime();
      ...
      if (err == OB_LOG_NOT_SYNC)
      {
        ...
        YYSYS_LOG(WARN, "wait response: err=%d, %s", err, to_cstring(*this));
      }
      else
      //add :e
      if (OB_SUCCESS != err)
      ...
      // {
      //   YYSYS_LOG(WARN, "slow response: %s", to_cstring(*this));
      // }
    }
    ...
    int ObAckQueue::init(IObAsyncClientCallback* callback, const ObClientManager* client_mgr, int64_t queue_len)
    {
      ...
      if (NULL == callback || NULL == client_mgr || queue_len <= 0)
      {
        ...
        YYSYS_LOG(ERROR, "invalid argument: callback=%p, client_mgr=%p, queue_len=%ld", callback, client_mgr, queue_len);
      }
      else if (OB_SUCCESS != (err = wait_queue_.init(queue_len)))
      {
        YYSYS_LOG(ERROR, "wait_queue.init()=>%d", err);
      }
      else
      {
        callback_ = callback;
    ...
    int ObAckQueue::set_next_acked_seq(const int64_t seq)
    {
      ...
      if (seq_is_setted_)
      {
        YYSYS_LOG(WARN, "next_acked_seq_ is setted already");
        err = OB_INIT_TWICE;
      }
      else
      {
        ...
        {
           YYSYS_LOG(WARN, "set new_ack_seq failed");
        }
      }
      return err;
    }
    ...
    int64_t ObAckQueue::get_next_acked_seq()
    {
      ...
      while(OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(ERROR, "wait_queue.pop()=>%d", err);
        }
      ...
      if (old_ack_seq != next_acked_seq_)
      {
        ...
        YYSYS_LOG(DEBUG, "ACK_QUEUE -> next_acked_seq_ = %ld", next_acked_seq_);
      }
    ...
    {
      int err = OB_SUCCESS;
      int64_t send_time_us = yysys::CTimeUtil::getTime();
      ObServer null_server;
      if (NULL == servers || n_server < 0 || start_seq < 0 || end_seq < start_seq || timeout_us <= TIMEOUT_DELTA)
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid argument: servers=%p[%ld], seq=[%ld,%ld], timeout=%ld", servers, n_server, start_seq, end_seq, timeout_us);
      }
      for(int64_t i = 0; OB_SUCCESS == err && i < n_server; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "post(%s)=>%d", to_cstring(servers[i]), err);
        }
      }
      if (OB_SUCCESS != err)
      {}
      ...
      {
        YYSYS_LOG(ERROR, "post(fake)=>%d", err);
      }
                           ...
                           const int32_t pcode, const int64_t timeout_us, const ObDataBuffer& pkt_buffer, int64_t idx)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid argument: servers=%s, seq=[%ld,%ld], timeout=%ld", to_cstring(server), start_seq, end_seq, timeout_us);
      }
      else
      {
      ...
      while(OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = wait_queue_.push(wait_idx, wait_node))
            && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "wait_queue_.push()=>%d", err);
        }
        ...
        else
        {
          ...
          if (idx >= 0 && OB_SUCCESS != (err = client_mgr_->post_request_using_dedicate_thread(server, pcode, RPC_VERSION, timeout_us, pkt_buffer,
                                                                                               ...
                                                                                               (int)idx)))
          {
            YYSYS_LOG(ERROR, "post_request()=>%d", err);
          }
    ...
    int ObAckQueue::callback(void* data)
    {
      ...
      {
        YYSYS_LOG(WARN, "request user_data == NULL");
      }
      */
      else
      {
        ...
        if (NULL == (packet =(ObPacket*)r->ipacket))
        {
          ...
          YYSYS_LOG(WARN, "receive NULL packet, timeout");
        }
        else if (NULL == (response_buffer = packet->get_buffer()))
        {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "response has NULL buffer");
        }
        ...
        {
          YYSYS_LOG(ERROR, "deserialize result_code failed:pos[%ld], err[%d], server=%s", pos, err, inet_ntoa_r(get_easy_addr(r)));
        }
        // add wangdonghui [ups_replication] 20170323 :b
        ...
        {
          YYSYS_LOG(ERROR, "deserialize message_residence_time_us failed:pos[%ld], err[%d], server=%s", pos, err, inet_ntoa_r(get_easy_addr(r)));
        }
        //add :e
        else
        {
        ...
        //mod :e
        {
          YYSYS_LOG(ERROR, "wait_queue.done()=>%d", tmperr);
        }
