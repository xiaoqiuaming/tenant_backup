  ...
  namespace common
  {
    ...
    int ObPermInfoKey::build_get_param(ObGetParam & get_param)
    {
      ...
      if (NULL != buffer_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "encode failed table_id=%ld", table_id_);
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "Get param add cell failed table_id[%lu]", cell_info.table_id_);
          }
          else
          {
      ...
      {
        YYSYS_LOG(ERROR, "malloc key buffer failed");
        ret = OB_ERROR;
      }
      return ret;
    }
    ...
    int ObPermInfoKey::set_table_id(uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "illegal argument table_id[%lu]", table_id);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObPermInfoKey::set_user_name(ObString name)
    {
      ...
      if (NULL == name.ptr())
      {
        YYSYS_LOG(WARN, "illegal argument name[%p]", name.ptr());
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObPermInfoKey::set_user_name(char * name)
    {
      ...
      if (NULL == name)
      {
        YYSYS_LOG(WARN, "illegal argument name[%p]", name);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObPermInfoKey:: set_length(int64_t length)
    {
      ...
      if (0 >= length)
      {
        YYSYS_LOG(WARN, "illegal argument length[%ld]", length);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSKeyInfoKey::build_get_param(ObGetParam& get_param)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "encode failed pkey_version=%ld", skey_version_);
      }
      else
      {
    ...
    int ObSKeyInfoKey::set_skey_version(int64_t version)
    {
      ...
      if (0 > version)
      {
        YYSYS_LOG(WARN, "illegal argument version[%ld]", version);
        ret = OB_ERROR;
      }
      else
      {
        ...
        else
        {
          ...
          switch (type)
          {
            case ObIntType:
              ...
              timestamp_ = yysys::CTimeUtil::getTime();
              break;
            case ObExtendType:
              ...
              {
                YYSYS_LOG(WARN, "unknow extend value=%ld", cell_info->value_.get_ext());
                ret = OB_ERROR;
              }
              break;
            default:
              YYSYS_LOG(WARN, "unexpected obj type");
              ret = OB_ERROR;
              break;
          }
        }
    ...
    int ObPermInfoValue::set_perm_mask(int64_t mask)
    {
      ...
      if (0 > mask)
      {
        YYSYS_LOG(WARN, "illegal argument perm_mask[%ld]", mask);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObPermInfoValue::set_time_stamp(int64_t timestamp)
    {
      ...
      if (0 > timestamp)
      {
        YYSYS_LOG(WARN, "illegal argument timestamp[%ld]", timestamp);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObUserInfoValue::set_passwd(ObString pass)
    {
      ...
      if (NULL == pass.ptr())
      {
        YYSYS_LOG(WARN, "illegal argument passwd[%p]", pass.ptr());
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObUserInfoValue::set_passwd(char* pass)
    {
      ...
      if (NULL == pass)
      {
        YYSYS_LOG(WARN, "illegal argument passwd[%p]", pass);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSKeyInfoValue::set_skey(ObString key)
    {
      ...
      if (NULL == key.ptr())
      {
        YYSYS_LOG(WARN, "illegal argument skey[%p]", key.ptr());
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSKeyInfoValue::set_skey(char* key)
    {
      ...
      if (NULL == key)
      {
        YYSYS_LOG(WARN, "illegal argument skey[%p]", key);
        ret = OB_ERROR;
      }
      else
      {
