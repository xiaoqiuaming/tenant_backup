...
namespace oceanbase
{
  namespace common
  {
        ...
        uint64_t *left_column_offset_array, int64_t &size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument, table_id[%lu] not legal, or left_table is not a wide table,join_count=%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get table by id. table_id=%ld", right_table_id);
        }
        else if (right_table_index == left_table_index)
        {
          ret = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "something wrong, right_table_index=left_table_index");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int64_t i = 0; i < left_table_schema->join_info_.count(); i++)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to get column rowkey index. column_id=%lu", right_column_id);
            break;
          }
          else if (OB_SUCCESS != (ret = left_table_schema->get_column_rowkey_index(left_column_id, offset)))
          {
            YYSYS_LOG(WARN, "fail to get column rowkey index. column_id=%lu", left_column_id);
            break;
          }
          ...
          {
            YYSYS_LOG(WARN, "rowkey column have rowkey_id less than 0. left_column_id=%lu," \
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to get left_column_offset_array");
      }
      return ret;
    }
                                        ...
                                        const uint64_t table_id, int64_t &index)
    {
      ...
      if (-1 == index)
      {
        YYSYS_LOG(WARN, "not find table, table_id=%lu", table_id);
    ...
    int ObSchemaHelper::transfer_manager_to_table_schema(const ObSchemaManagerV2 &schema_manager, ObArray<TableSchema> &table_schema_array)
    {
      ...
      {
        YYSYS_LOG(INFO, "schema_manager has 0 table.");
      }
      else
      {
        ...
        while (OB_SUCCESS == ret && table != schema_manager.table_end())
        {
          ...
          //[view]
          ObTableSchema::ObTableType type = table->get_type();
          switch(type)
          {
          ...
          case ObTableSchema::META:
              table_schema.table_type_ = TableSchema::META;
              break;
          ...
          for (int64_t i = 0; i < columns_size; i++)
          {
            ...
            if (NULL != tmp_join)
            {
              YYSYS_LOG(DEBUG, "column have join_info, join_table_id=%lu, join_column_id_=%lu",
            ...
            YYSYS_LOG(DEBUG, "column_name=%s", column->get_name());
            ...
            if (rowkey_info.is_rowkey_column(column->get_id()))
            {
              ...
              {
                ret = OB_ERROR;
                YYSYS_LOG(WARN, "fail to get column rowkey index. ret=%d", ret);
                break;
              }
              else
              {
                                                      ...
                                                      ObArray<JoinInfo>& join_array)
    {
      ...
      for (uint64_t i = 0; i < ob_join_info.left_column_count_; i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to find column_schema. column_id=%lu", join_info.left_column_id_);
          break;
        }
