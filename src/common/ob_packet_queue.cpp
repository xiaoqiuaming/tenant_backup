...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObPacketQueue::init()
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "ring buffer init failed, err: %d", ret);
      }
    ...
    int ObPacketQueue::pop_packets(ObPacket** packet_arr, const int64_t ary_size, int64_t& ret_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, packet_arr=%p, arr_size=%ld", packet_arr, ary_size);
        err = OB_ERROR;
      }
      else
      {
        ...
        if (tb == NULL)
        {
          YYSYS_LOG(ERROR, "get packet thread buffer failed, return NULL");
          err = OB_ERROR;
        }
        else
        {
      ...
      while (OB_SUCCESS == err && ret_size < ary_size)
      {
        ...
        else
        {
          ...
          if (tb->remain() < total_size)
          {
            if (0 == ret_size)
            {
              YYSYS_LOG(ERROR, "the thread buffer is too small, packet_size=%ld, buffer_size=%d",
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(ERROR, "failed to pop task, err=%d", err);
            }
    ...
    ObPacket* ObPacketQueue::pop()
    {
      ...
      {
        YYSYS_LOG(ERROR, "pop task from ring buffer failed, task: %p, err: %d", packet, err);
      }
    ...
    void ObPacketQueue::push(ObPacket* packet)
    {
      if (packet != NULL)
      {
        ...
        {
          YYSYS_LOG(WARN, "push packet into ring buffer failed, size:%ld, err: %d", size, err);
        }
      }
      else
      {
        YYSYS_LOG(ERROR, "cast packet to ObPacket failed");
      }
    }
