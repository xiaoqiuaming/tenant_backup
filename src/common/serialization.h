    ...
    namespace serialization
    {
      ...
      */
                                     ...
                                     int8_t precision, int8_t scale, int8_t vscale, int8_t nwords, const uint32_t *words)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to encode decimal. err = %d", err);
        }
        return err;
      }
                                     ...
                                     int8_t &precision, int8_t &scale, int8_t &vscale, int8_t &nwords, uint32_t *words)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to decode decimal, err = %d", err);
        }
        return err;
      }
      ...
      inline int64_t encoded_length_decimal_type(int8_t nwords, const uint32_t *words)
      {
        ...
        {
          YYSYS_LOG(ERROR, "null decimal words");
        }
        else
        {
          for (int8_t i = 0; i < nwords; ++i)
      ...
      inline int encode_createtime_type(char *buf,const int64_t buf_len,int64_t& pos,const ObCreateTime& val)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "fail to encode_createtime_type: buf[%p], buf_len[%ld], pos[%ld] val[%ld]",
                    buf, buf_len, pos, val);
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fail to __encode_time_type: ret = %d", ret);
          }
        }
        return ret;
      }
      ...
      */
      ...
      inline int encode_ttint_buf(char *buf,const int64_t buf_len,int64_t& pos, uint64_t *vbuf, int32_t len)
      {
        int ret = OB_SUCCESS;
        UNUSED(buf_len);
        if(buf==NULL)
        {
          ret=OB_ERROR;
          YYSYS_LOG(WARN,"the input buf pointer is NULL!");
        }
        if(buf_len-pos<len)
        {
          ret=OB_BUF_NOT_ENOUGH;
          YYSYS_LOG(WARN,"the buf is not enough");
        }
        if (OB_SUCCESS == ret && (NULL != vbuf) && (len > 0))
        {
          memcpy(buf + pos,vbuf,len);
      ...
      inline  char * decode_ttint_buf(const char *buf,const int64_t data_len,int64_t& pos,int32_t& lenp)
      {
        ...
        else
        {
          ...
          else
          {
            lenp = -1;
            str = NULL;
            YYSYS_LOG(ERROR,"failed to decode_ttint_buf");
          }
      ...
      inline int encode_comm_decimal(char* buf, const int64_t buf_len,
                                     int64_t &pos, bool is_add, int8_t precision, int8_t scale,
                                     int8_t vscale, uint64_t* word,int32_t len)
      {
        ...
        if (NULL == buf || buf_len <= 0 || pos >= buf_len)
        {
          //modify e
          ret = OB_ERROR;
          YYSYS_LOG(WARN,"string buffer for decimal is null!");
        }
                                     ...
                                     int8_t &vscale, char *&word,int32_t &len)
      {
        ...
        else
        {
          ...
          if(NULL==word)
          {
            ...
            YYSYS_LOG(WARN,"deserilization decimal error!null pointer");
          }
          if (OB_SUCCESS != ret)
          {

            //TODO WARNING FOR
            YYSYS_LOG(WARN,"deserilization decimal error!");
          }
