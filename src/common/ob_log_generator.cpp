...
namespace oceanbase
{
  namespace common
  {
    int DebugLog::advance()
    {
      int err = OB_SUCCESS;
      last_ctime_ = ctime_;
      ctime_ = yysys::CTimeUtil::getTime();
      return err;
    }
    ...
    int DebugLog::serialize(char* buf, int64_t limit, int64_t& pos) const
    {
      ...
      if (NULL == buf || limit < 0 || pos > limit)
      {
        ...
        //YYSYS_LOG(ERROR, "INVALID_ARGUMENT: buf=%p, limit=%ld, pos=%ld", buf, limit, pos);
      }
               ...
               || OB_SUCCESS != serialization::encode_i64(buf, limit, new_pos, last_ctime_))
      {
        ...
        //YYSYS_LOG(ERROR, "serialize(buf=%p, limit=%ld, pos=%ld): fail", buf, limit, pos);
      }
      else
      {
        pos = new_pos;
    ...
    int DebugLog::deserialize(const char* buf, int64_t limit, int64_t& pos)
    {
      ...
      {
        err = OB_DESERIALIZE_ERROR;
        //YYSYS_LOG(ERROR, "deserialize(buf=%p, limit=%ld, pos=%ld): fail", buf, limit, pos);
      }
      else
      {
        pos = new_pos;
    ...
    ObLogGenerator::ObLogGenerator(): is_frozen_(false), log_file_max_size_(1<<24), start_cursor_(), end_cursor_(),
      log_buf_(NULL), log_buf_len_(0), pos_(0), log_term_(OB_INVALID_TERM)//add wangdonghui [ups_replication] 20170801
    {
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "[start_cursor[%ld], end_cursor[%ld]]", start_cursor_.log_id_, end_cursor_.log_id_);
      return err;
    }
    ...
    int ObLogGenerator::init(int64_t log_buf_size, int64_t log_file_max_size, const ObServer* server)
    {
      ...
      else if (0 != (sys_err = posix_memalign((void**)&log_buf_, LOG_FILE_ALIGN_SIZE, log_buf_size + LOG_FILE_ALIGN_SIZE)))
      {
        ...
        YYSYS_LOG(ERROR, "posix_memalign(%ld):%s", log_buf_size, strerror(sys_err));
      }
      else
      {
        if (NULL != server)
        ...
        YYSYS_LOG(INFO, "log_generator.init(log_buf_size=%ld, log_file_max_size=%ld)", log_buf_size, log_file_max_size);
      }
      return err;
    }
    ...
    bool ObLogGenerator::check_log_size(const int64_t size,const int64_t max_mutator_size) const
    {
      ...
      if (!ret)
      {
        YYSYS_LOG(ERROR, "log_size[%ld] + reserved[%ld] + header[%ld] >= max_mutator_size[%ld]",
    ...
    bool ObLogGenerator::check_log_size(const int64_t size) const
    {
      ...
      if (!ret)
      {
        YYSYS_LOG(ERROR, "log_size[%ld] + reserved[%ld] + header[%ld] <= log_buf_len[%ld]",
    ...
    int ObLogGenerator::reset()
    {
      ...
      if (!is_clear())
      {
        ...
        YYSYS_LOG(ERROR, "log_not_clear, [%s,%s], len=%ld", to_cstring(start_cursor_), to_cstring(end_cursor_), pos_);
      }
      else
      {
        start_cursor_.reset();
    ...
    int ObLogGenerator::start_log(const ObLogCursor& log_cursor)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "log_cursor.is_valid()=>false");
      }
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "cursor=[%ld,%ld] already inited.", start_cursor_.log_id_, end_cursor_.log_id_);
      }*/
      //del
      else
      {
        ...
        YYSYS_LOG(INFO, "ObLogGenerator::start_log(log_cursor=%s)", to_cstring(log_cursor));
      }
      return err;
    }
    ...
    int ObLogGenerator:: update_cursor(const ObLogCursor& log_cursor)
    {
      ...
      else if (end_cursor_.newer_than(log_cursor))
      {
        ...
        YYSYS_LOG(ERROR, "end_cursor[%s].newer_than(log_cursor[%s])", to_cstring(end_cursor_), to_cstring(log_cursor));
      }
      else
      {
        start_cursor_ = log_cursor;
    ...
    int ObLogGenerator:: get_start_cursor(ObLogCursor& log_cursor) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_state()=>%d", err);
      }
      else
      {
                            ...
                            )
    {
      ...
      else if (entry.get_serialize_size() + data_len > len)
      {
        ...
        YYSYS_LOG(WARN, "header[%ld] + data_len[%ld] > len[%ld]", entry.get_serialize_size(), data_len, len);
      }
      ...
      {
        YYSYS_LOG(ERROR, "cursor[%s].next_entry()=>%d", to_cstring(cursor), err);
      }
                                      ...
                                      const int64_t reserved_len)
    {
      ...
      else if (is_frozen_)
      {
        ...
        YYSYS_LOG(ERROR, "log_generator is frozen, cursor=[%s,%s]", to_cstring(start_cursor_), to_cstring(end_cursor_));
      }
      ...
      {
        YYSYS_LOG(WARN, "generate_log(pos=%ld)=>%d", pos_, err);
      }
      return err;
    }
    ...
    static int parse_log_buffer(const char* log_data, int64_t data_len, const ObLogCursor& start_cursor, ObLogCursor& end_cursor, bool check_data_integrity = false)
    {
      ...
      while (OB_SUCCESS == err && pos < data_len)
      {
        ...
        else if (pos + log_entry.get_log_data_len() > data_len)
        {
          ...
          YYSYS_LOG(ERROR, "last_log broken, cursor=[%s,%s]", to_cstring(start_cursor), to_cstring(end_cursor));
        }
    ...
    int ObLogGenerator:: fill_batch(const char* buf, int64_t len)
    {
      ...
      else if (0 != (len & LOG_FILE_ALIGN_MASK))
      {
        ...
        YYSYS_LOG(ERROR, "len[%ld] is not align[mask=%lx]", len, LOG_FILE_ALIGN_SIZE);
      }
      else if (is_frozen_)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(WARN, "log_buf is frozen, end_cursor=%s", to_cstring(end_cursor_));
      }
      else if (pos_ != 0)
      {
        err = OB_LOG_NOT_CLEAR;
        YYSYS_LOG(ERROR, "fill_batch(pos[%ld] != 0, end_cursor=%s, buf=%p[%ld])",
                  pos_, to_cstring(end_cursor_), buf, len);
      }
      else if (len + reserved_len > log_buf_len_)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "len[%ld] + reserved_len[%ld] > log_buf_len[%ld]",
                  len, reserved_len, log_buf_len_);
      }
      else if (OB_SUCCESS != (err = parse_log_buffer(buf, len, start_cursor, end_cursor)))
      {
        YYSYS_LOG(ERROR, "parse_log_buffer(buf=%p[%ld], cursor=%s)=>%d", buf, len, to_cstring(end_cursor_), err);
      }
      else
      {
    ...
    int ObLogGenerator:: write_log(const LogCommand cmd, const char* log_data, const int64_t data_len)
    {
      ...
      else if (is_frozen_)
      {
        ...
        YYSYS_LOG(WARN, "log_buf is frozen, end_cursor=%s", to_cstring(end_cursor_));
               ...
               && OB_BUF_NOT_ENOUGH != err)
      {
        YYSYS_LOG(WARN, "do_write_log(cmd=%d, pos=%ld, len=%ld)=>%d", cmd, pos_, data_len, err);
      }
    ...
    int ObLogGenerator::switch_log()
    {
      ...
      {
        YYSYS_LOG(ERROR, "encode_i64(file_id_=%ld)=>%d", end_cursor_.file_id_, err);
      }
      else if (OB_SUCCESS != (err = do_write_log(OB_LOG_SWITCH_LOG, buf, buf_len, 0)))
      {
        YYSYS_LOG(ERROR, "write(OB_LOG_SWITCH_LOG, len=%ld)=>%d", end_cursor_.file_id_, err);
      }
      else
      {
        YYSYS_LOG(INFO, "switch_log(file_id=%ld, log_id=%ld)", end_cursor_.file_id_, end_cursor_.log_id_);
      }
      return err;
    }
    ...
    int ObLogGenerator::check_point(int64_t& cur_log_file_id)
    {
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "checkpoint(file_id=%ld, log_id=%ld)", end_cursor_.file_id_, end_cursor_.log_id_);
      }
      return err;
    }
    ...
    int ObLogGenerator::append_eof()
    {
      ...
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "no buf to append eof flag, pos=%ld, log_buf_len=%ld", pos_, log_buf_len_);
      }
      else
      {
    ...
    int ObLogGenerator:: check_log_file_size()
    {
      ...
      {
        YYSYS_LOG(ERROR, "switch_log()=>%d", err);
      }
      return err;
    }
    ...
    int ObLogGenerator:: write_nop(const bool force_write)
    {
      int err = OB_SUCCESS;
      int64_t pos = 0;
      YYSYS_LOG(TRACE, "try write nop: pos=%ld, force_write=%s", pos_, STR_BOOL(force_write));
      if (is_aligned(pos_, LOG_FILE_ALIGN_MASK) && !force_write)
      {
        YYSYS_LOG(TRACE, "The log is aligned");
      }
      else if (OB_SUCCESS != (err = debug_log_.advance()))
      {
        YYSYS_LOG(ERROR, "debug_log.advance()=>%d", err);
      }
      else if (OB_SUCCESS != (err = debug_log_.serialize(nop_log_, sizeof(nop_log_), pos)))
      {
        YYSYS_LOG(ERROR, "serialize_nop_log(%s)=>%d", to_cstring(end_cursor_), err);
      }
      else if (OB_SUCCESS != (err = do_write_log(OB_LOG_NOP, nop_log_, calc_nop_log_len(pos_, pos), 0)))
      {
        YYSYS_LOG(ERROR, "write_log(OB_LOG_NOP, len=%ld)=>%d", calc_nop_log_len(pos_, pos), err);
      }
      return err;
    }
    ...
    int ObLogGenerator:: switch_log(int64_t& new_file_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "write_nop()=>%d", err);
      }
      else if (OB_SUCCESS != (err = switch_log()))
      {
        YYSYS_LOG(ERROR, "switch_log()=>%d", err);
    ...
    int ObLogGenerator:: get_log(ObLogCursor& start_cursor, ObLogCursor& end_cursor, char*& buf, int64_t& len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_log_file_size()=>%d", err);
      }
      else if (OB_SUCCESS != (err = append_eof()))
      {
        YYSYS_LOG(ERROR, "write_eof()=>%d", err);
      }
      else
      {
        is_frozen_ = true;
    ...
    int ObLogGenerator:: commit(const ObLogCursor& end_cursor)
    {
      ...
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "end_cursor[%ld] != end_cursor_[%ld]", end_cursor.log_id_, end_cursor_.log_id_);
      }
      else
      {
