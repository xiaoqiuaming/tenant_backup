...
int ObUpsInfoMgr::get_master_ups(ObServer& master_ups, const bool is_update_list, bool& is_updated)
{
  ...
  if(OB_SUCCESS == ret)
  {
    yysys::CRLockGuard lock(ups_info_mgr_lock_);
...
int ObUpsInfoMgr::init(ObUpsInfoMgrRpcStub* ups_info_mgr_rpc_stub)
{
  ...
  {
    YYSYS_LOG(WARN, "ups_info_mgr_rpc_stub[%p]", ups_info_mgr_rpc_stub);
...
int ObUpsInfoMgr::fetch_update_server_list(int32_t& count, bool& is_updated)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "reset the percentage for all servers, server_type_[%d], list.ups_count_[%d]", server_type_, list.ups_count_);
    ...
    if (finger_print != cur_finger_print_)
    {
      ...
      list.print();
      yysys::CWLockGuard lock(ups_info_mgr_lock_);
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "there is no master ups in ups list:count [%d]", count);
        }
      }
    }
    else
...
ObUpsInfoMgr::ObUpsInfoMgr(const ObServerType type, const ObServer& root_server, int64_t rpc_timeout):ups_info_mgr_lock_(yysys::WRITE_PRIORITY)
{
