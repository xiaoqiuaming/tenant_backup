...
//         chunkserver_(server) { }
      ObTabletLocation() :tablet_version_(0), tablet_seq_(0),cluster_id_(-1)
      {}
      ObTabletLocation(int64_t tablet_version, const ObServer & server, int32_t cluster_id = OB_ALL_CLUSTER_FLAG) //mod [multiups with paxos]
        :tablet_version_(tablet_version), tablet_seq_(0),
         chunkserver_(server), cluster_id_(cluster_id)
      {}

      ObTabletLocation(int64_t tablet_version, int64_t tablet_seq,
        const ObServer & server ,int32_t cluster_id)
        :tablet_version_(tablet_version), tablet_seq_(tablet_seq),
         chunkserver_(server),cluster_id_(cluster_id)
      {}
      //mod:e
    ...
    struct ObTabletReportInfoList
    {
      ...
      inline int rollback(ObTabletReportInfoList& rollback_list, const int64_t rollback_count)
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid rollback count, rollback_count=%ld, tablet_size=%ld",
      ...
      inline int64_t get_tablet_size() const
      { return tablet_list_.get_array_index(); }
      inline const ObTabletReportInfo* const get_tablet() const
      { return tablets_; }
    ...
    struct ObIndexTabletInfoList
    {
      ...
      inline int64_t get_tablet_size() const
      { return tablet_list.get_array_index(); }
      inline const ObTabletInfo* const get_tablet() const
      { return tablets; }
    ...
    struct ObTableImportInfoList
    {
      ...
      inline int64_t get_table_size() const
      { return table_list_.get_array_index(); }
      inline const uint64_t* const get_table() const
      { return tables_; }
