  ...
  namespace common
  {
    ...
    int ObLogDataWriter::AppendBuffer::write(const char* buf, int64_t len, int64_t pos)
    {
               ...
               && 0 != (sys_err = posix_memalign((void**)&buf_, ObLogGenerator::LOG_FILE_ALIGN_SIZE, buf_limit_)))
      {
        ...
        YYSYS_LOG(ERROR, "posix_memalign(align=%ld, size=%ld): %s",
                  ObLogGenerator::LOG_FILE_ALIGN_SIZE, buf_limit_, strerror(sys_err));
      }
      else if (len > buf_limit_)
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "write_size[%ld] overflow: buf_limit=%ld", len, buf_limit_);
      }
      else
      {
      ...
      else if (pos < file_pos || pos > file_pos + buf_end_)
      {
        ...
        YYSYS_LOG(ERROR, "write(bufferd=[%ld,+%ld], append=[%ld,+%ld])", file_pos, buf_end_, pos, len);
      }
      else if (file_pos + buf_limit_ < pos + len)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(DEBUG, "AppendBuf not ENOUGH: file_pos[%ld] + buf_limit[%ld] < pos[%ld] + len[%ld]",
    ...
    int ObLogDataWriter::AppendBuffer::flush(int fd)
    {
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "uniter_pwrite(fd=%d, buf=%p[%ld], pos=%ld):%s",
    ...
    ObLogDataWriter::~ObLogDataWriter()
    {
      ...
      if (fd_ > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "write_buffer_.flush(fd=%d)=>%d", fd_, err);
        }
        if (0 != close(fd_))
        {
          YYSYS_LOG(ERROR, "close(fd=%d):%s", fd_, strerror(errno));
        }
      }
    }
                              ...
                              MinAvailFileIdGetter* min_avail_file_id_getter)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "log_dir == %p, file_size=%ld, du_percent=%ld, min_avail_fid_getter=%p",
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "statfs(dir=%s)=>%s", log_dir, strerror(errno));
      }
      else
      {
        ...
        YYSYS_LOG(INFO, "log_data_writer.init(log_dir=%s, fsize=%ld, dsize=%ld/%ld, du_limit=%ld%%, new_file_count=%ld)",
                  log_dir, file_size,
    ...
    int myfallocate_by_append(int fd, int mode, off_t offset, off_t len)
    {
      ...
      for(int64_t pos = offset; 0 == err && pos < offset + len; pos += count)
      {
        ...
        {
          err = -1;
          YYSYS_LOG(ERROR, "uniter_pwrite(pos=%ld, count=%ld) fail", pos, count);
          break;
        }
      }
      return err;
...
# if __linux && (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 10))
    int myfallocate(int fd, int mode, off_t offset, off_t len)
    {
      ...
      if (syscall_supported && 0 != (err = fallocate(fd, mode, offset, len)))
      {
        ...
        YYSYS_LOG(WARN, "glibc support fallocate(), but fallocate still fail with %s, fallback to call myfallocate_by_append()", strerror(errno));
      }
    ...
    int file_expand_by_fallocate(const int fd, const int64_t file_size)
    {
               ...
               || 0 != (file_size & ObLogGenerator::LOG_FILE_ALIGN_MASK))
      {
        ...
        YYSYS_LOG(ERROR, "file_size[%ld] or file_size[%ld] not align by %lx",
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "fallocate(fd=%d):%s", fd, strerror(errno));
      }
      else if (0 != fsync(fd))
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "fsync(fd=%d):%s", fd, strerror(errno));
      }
      return err;
    }
                               ...
                               const char* data, const int64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "write(cursor=[%s,%s], data=%p[%ld])",
      ...
      {
        err = OB_LOG_NOT_ALIGN;
        YYSYS_LOG(ERROR, "write_log(data=%p, len=%ld): NOT_ALIGN", data, len);
      }
      ...
      {
        YYSYS_LOG(ERROR, "prepare_fd(%ld)=>%d", start_cursor.file_id_, err);
      }
      else if (OB_LOG_NOSYNC == log_sync_type_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "writer_buffer_.write(%p[%ld], pos=%ld)=>%d", data, len, start_cursor.offset_, err);
        }
        else if (OB_SUCCESS == err)
        {}
        else if (OB_SUCCESS != (err = write_buffer_.flush(fd_)))
        {
          YYSYS_LOG(ERROR, "write_buffer_.flush(fd_=%d)=>%d", fd_, err);
        }
        else if (OB_SUCCESS != (err = write_buffer_.write(data, len, start_cursor.offset_)))
        {
          YYSYS_LOG(ERROR, "send write fail: start_cursor=%s, data=%p[%ld]", to_cstring(start_cursor), data, len);
        }
      }
      else
      {
        ...
        {
          err = OB_IO_ERROR;
          YYSYS_LOG(ERROR, "pwrite(fd=%d, buf=%p[%ld]), cursor=%s): %s",
                    fd_, data, len, to_cstring(start_cursor), strerror(errno));
        }
        else if (0 != fdatasync(fd_))
        {
          err = OB_IO_ERROR;
          YYSYS_LOG(ERROR, "fdatasync(%d):%s", fd_, strerror(errno));
        }
      }
      if (OB_SUCCESS == err)
      {
    ...
    int ObLogDataWriter::prepare_fd(const int64_t file_id)
    {
      ...
      if (cur_file_id_ == file_id)
      {
        if (fd_ < 0)
        {
          ...
          YYSYS_LOG(ERROR, "fd[%d] < 0, file_id=%ld", fd_, file_id);
        }
      }
      else
      {
        ...
        if (fd_ >= 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "file_expand_to(fd=%d, size=%ld):%s", fd_, file_size_, strerror(errno));
          }
          if (0 != close(fd_))
          {
            err = OB_IO_ERROR;
      ...
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "file name too long, log_dir=%s, log_id=%ld, buf_size=%ld",
                  log_dir_, file_id, sizeof(fname));
      }
      else if ((fd_ = open(fname, OPEN_FLAG, OPEN_MODE)) >= 0)
      {
        YYSYS_LOG(INFO, "old %s exist, append clog to it", fname);
      }
    ...
    int ObLogDataWriter::reuse(const char* pool_file, const char* fname)
    {
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(WARN, "rename(%s,%s):%s", pool_file, tmp_pool_file, strerror(errno));
      }
      else if ((fd = open(tmp_pool_file, OPEN_FLAG, OPEN_MODE)) < 0)
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "open(%s) failed: %s", tmp_pool_file, strerror(errno));
      }
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "write_eof_flag fail(%s): %s", tmp_pool_file, strerror(errno));
      }
      else if (0 != fsync(fd))
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "fdatasync(%s):%s", tmp_pool_file, strerror(errno));
      }
      else if (0 != rename(tmp_pool_file, fname))
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "rename(%s,%s):%s", pool_file, fname, strerror(errno));
      }
    ...
    const char* ObLogDataWriter::select_pool_file(char* buf, const int64_t buf_len)
    {
      ...
      {
        num_file_to_add_--;
        YYSYS_LOG(INFO, "min_file_id has not inited, can not reuse file, will create new file num_file_to_add=%ld", num_file_to_add_);
      }
      else if (min_file_id_ < 0)
      {
        YYSYS_LOG(ERROR, "min_file_id[%ld] < 0", min_file_id_);
      }
      else if (num_file_to_add_ > 0)
      {
        num_file_to_add_--;
        YYSYS_LOG(INFO, "num_file_to_add[%ld] >= 0 will create new file.", num_file_to_add_);
      }
      else if (min_file_id_ > min_avail_file_id_
               && min_file_id_ > (min_avail_file_id_ = min_avail_file_id_getter_->get()))
      {
        YYSYS_LOG(WARN, "can not select pool_file: min_file_id=%ld, min_avail_file_id=%ld",
                  min_file_id_, min_avail_file_id_);
      }
      else if ((len = snprintf(buf, buf_len, "%s/%ld", log_dir_, min_file_id_)) < 0
               || len >= buf_len)
      {
        YYSYS_LOG(ERROR, "gen fname fail: buf_len=%ld, fname=%s/%ld", buf_len, log_dir_, min_file_id_);
      }
      else
      {
        result = buf;
      ...
      YYSYS_LOG(INFO, "select_pool_file(num_file_to_add=%ld, min_file_id=%ld, max_reusable_fid=%ld)=>%s",
    ...
    int ObLogDataWriter::check_eof_after_log_cursor(const ObLogCursor& cursor)
    {
      ...
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "gen fname fail: buf_len=%ld, fname=%s/%ld", sizeof(fname), log_dir_, cursor.file_id_);
      }
      else if ((fd = open(fname, O_RDONLY | O_DIRECT | O_CREAT, OPEN_MODE)) < 0)
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "open %s fail: %s", fname, strerror(errno));
      }
      else if (::get_file_size(fd) == cursor.offset_)
      {
        YYSYS_LOG(WARN, "no eof mark after log_cursor[%s], but this is the end of file, maybe replay old version log_file", to_cstring(cursor));
      }
      else if (0 > (read_count = unintr_pread(fd, buf, len, cursor.offset_)))
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "pread(file=%s, fd=%d, buf=%p[%ld], cursor.offset=%ld)=>%s", fname, fd, buf, len, cursor.offset_, strerror(errno));
      }
      else if (!ObLogGenerator::is_eof(buf, read_count))
      {
        err = OB_LAST_LOG_RUINNED;
        YYSYS_LOG(ERROR, "%s not follow by eof, read_count=%ld", to_cstring(cursor), read_count);
      }
    ...
    int ObLogDataWriter::start_log(const ObLogCursor& cursor)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "cursor[%s].is_invalid", to_cstring(cursor));
      }
      /*
      else if (end_cursor_.is_valid())
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "ObLogWriter is init already init, cursor_=%s", to_cstring(end_cursor_));
      }
      */
      else if (0 != (cursor.offset_ & ObLogGenerator::LOG_FILE_ALIGN_MASK))
      {
        err = OB_LOG_NOT_ALIGN;
        YYSYS_LOG(WARN, "start_log(%s): LOG_NOT_ALIGNED", to_cstring(cursor));
      }
      ...
      {
        YYSYS_LOG(ERROR, "no eof after %s, maybe commitlog corrupt, err=%d", to_cstring(cursor), err);
