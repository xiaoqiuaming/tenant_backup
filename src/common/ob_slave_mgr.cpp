                     ...
                     bool exist_wait_lease_on/* = false*/)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "Parameters is invlid[role_mgr=%p rpc_stub=%p]",
          role_mgr, rpc_stub);
...
int ObSlaveMgr::set_send_log_point(const ObServer &server, const uint64_t send_log_point)
{
  ...
  {
    ret = OB_ERROR; 
    YYSYS_LOG(WARN, "server not exist");
  }
  return ret;
}
...
int ObSlaveMgr::set_last_reply_log_seq(const ObServer &server, const int64_t last_reply_log_seq)
{
  ...
  if (NULL != node)
  {
    ...
    while ((x = *p) < last_reply_log_seq && ! __sync_bool_compare_and_swap(p, x, last_reply_log_seq))
    {
       ...
       YYSYS_LOG(WARN, "slave set_last_reply_log_seq waiting for __sync lock release");
    }
    if (x >= last_reply_log_seq)
    {
      YYSYS_LOG(WARN, "receive slave[%s] log older than last_receive_log", to_cstring(server));
        ret = OB_EAGAIN;
    }
    else
    {
      node->last_log_sync_time = yysys::CTimeUtil::getTime();
    }
  }
  else
  {
...
int ObSlaveMgr::add_server(const ObServer& server)
{
  ...
  if (NULL != node)
  {
    YYSYS_LOG(INFO, "slave[%s] is already existed", to_cstring(server));
  }
  else
  {
    ...
    if (NULL == item)
    {
      YYSYS_LOG(ERROR, "slave_list_(ObVector) push_back error[%d]", ret);
      ret = OB_ERROR;
    }
    else
    {
      ...
      YYSYS_LOG(INFO, "add a slave[%s], remaining slave number[%d]", to_cstring(server), slave_num_);
    }
  }
  slave_info_mutex_.unlock();
...
int ObSlaveMgr::add_server_with_log_id(const ObServer& server, const int64_t log_id)
{
  ...
  if (NULL != node)
  {
    YYSYS_LOG(INFO, "slave[%s] is already existed, update log_id=%ld", to_cstring(server), log_id);
  }
  else
  {
    ServerNode* item = (ServerNode*)ob_malloc(sizeof(ServerNode), ObModIds::OB_SLAVE_MGR);
...
bool ObSlaveMgr::need_to_del(const ObServer& server, const int64_t wait_time)
{
  ...
  if (NULL == node)
  {
    YYSYS_LOG(WARN, "Server[%s] is not found", to_cstring(server));
    ret = false;
  }
  //add:e
  else
  {
    if (yysys::CTimeUtil::getTime() - node->last_log_sync_time > wait_time)
    {
      ret = true;
    }
  }
...
int ObSlaveMgr::delete_server(const ObServer& server)
{
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "delete server[%s], remaining slave number[%d]",  to_cstring(server), slave_num_);
  }
  slave_info_mutex_.unlock();
...
int ObSlaveMgr::set_obi_role(ObiRole obi_role)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    while (p != NULL && p != &slave_head_.server_list_link)
    {
      ...
      {
        ret = err;
        if (!yysys::CNetUtil::isLocalAddr(vip_))
        {
          YYSYS_LOG(ERROR, "VIP has gone");
          ret = OB_ERROR;
          p = p->next();
        }
        else
        {
          YYSYS_LOG(ERROR, "send obi_role to slave[%s] fail. err=%d, delete it", slave_node->server.to_cstring(), err);
          ObDLink *to_del = p;
          p = p->next();
          to_del->remove();
          slave_num_ --;
    ...
    if (NULL == p)
    {
      YYSYS_LOG(ERROR, "Server list encounter NULL pointer, this should not be reached");
      ret = OB_ERROR;
    }
  }
  slave_info_mutex_.unlock();
...
int ObSlaveMgr::send_data(const char* data, int64_t length)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "parameters are invalid[data=%p length=%ld]", data, length);
      ret = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    while (OB_SUCCESS == ret && p != NULL && p != &slave_head_.server_list_link)
    {
      ...
      for (int64_t i = 0; i < send_retry_times_; i++)
      {
        int64_t send_beg_time = yysys::CTimeUtil::getMonotonicTime();
        ...
        {
          int64_t send_elsp_time = yysys::CTimeUtil::getMonotonicTime() - send_beg_time;
      ...
      if (OB_SUCCESS != err)
      {
        ...
        {
          YYSYS_LOG(WARN, "send_data to slave[%s] error[err=%d]",  to_cstring(slave_node->server), err);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    while (OB_SUCCESS == ret && p != NULL && p != &failed_head.server_list_link)
    {
      ...
      {
        YYSYS_LOG(ERROR, "ERROR state found when waiting for lease expiring");
        ret = OB_ERROR;
      }
      else
      {
        if (slave_fail_wait_lease_on_)
        {
          YYSYS_LOG(WARN, "Slave[%s]'s lease is expired and has been removed",  to_cstring(slave_node->server));
        }
        else
        {
          YYSYS_LOG(WARN, "Slave[%s] has been removed without "
...
{
  int64_t start_time_us = yysys::CTimeUtil::getTime();
  int err = send_data(data, length);
  delay_us = yysys::CTimeUtil::getTime() - start_time_us;
  return err;
}
...
int ObSlaveMgr::extend_lease(const ObServer& server, ObLease& lease)
{
  ...
  {
    node->lease.lease_time = yysys::CTimeUtil::getTime();
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "grant_lease error, ret=%d", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "grant lease to Slave[%s], lease_time=%ld lease_internval=%ld renew_interval=%ld",
...
int ObSlaveMgr::check_lease_expiration()
{
  ...
  YYSYS_LOG(DEBUG, "TODO: check_lease_expiration");
  return OB_SUCCESS;
}
...
{
  //TODO: lease机制实现, yanran
  YYSYS_LOG(DEBUG, "TODO: is_lease_valid of Slave[%s]", to_cstring(server));
  return false;
}
...
int ObSlaveMgr::reset_slave_list()
{
  ...
  {
    YYSYS_LOG(INFO, "slave num =%d", slave_num_);
    res = (ServerNode*)(p);
    err = delete_server(res->server);
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "fail to delete server. err=%d", err);
    }
