...
namespace oceanbase
{
  namespace common
  {
    ...
    int obj_cast(const ObObj &orig_cell, const ObObj &expected_type,
                 ObObj &casted_cell, const ObObj *&res_cell, bool is_coalesce = false);
    ...
    int check_format_and_content(const ObExprObj &in, enum DateTimeType type); //add for [column default]
