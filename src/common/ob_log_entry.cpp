...
int ObLogEntry::check_header_integrity(bool dump_content) const
{
  ...
  {
    YYSYS_LOG(WARN, "check_header_integrity error: ");
    if (dump_content)
    {
      hex_dump(&header_, sizeof(header_), true, YYSYS_LOG_LEVEL_WARN);
    }
  }
...
int ObLogEntry::check_data_integrity(const char* log_data, bool dump_content) const
{
  ...
  else
  {
    ...
    else
    {
      if (dump_content)
      {
          YYSYS_LOG(WARN, "check_header:%d, check_magic:%d, check_data:%d",
                    header_.check_header_checksum(), header_.check_magic_num(MAGIC_NUMER),
                    crc_check_sum == header_.data_checksum_?OB_SUCCESS:OB_ERROR);
        YYSYS_LOG(WARN, "Header: ");
        hex_dump(&header_, sizeof(header_), true, YYSYS_LOG_LEVEL_WARN);
        YYSYS_LOG(WARN, "Body: ");
        hex_dump(log_data - sizeof(uint64_t) - sizeof(LogCommand), header_.data_length_, true, YYSYS_LOG_LEVEL_WARN);
      }
      ret = OB_ERROR;
    }
  }
