  ...
  namespace common
  {
    ...
    int ObDiskChecker::init(const int32_t* disk_no_array, const int32_t disk_num)
    {
      ...
      for (int32_t i = 0; i < disk_num && i < common::OB_MAX_DISK_NUMBER; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "disk_no:%d exceed max_disk_no:%ld",
    ...
    void ObDiskChecker::set_check_stat(const int32_t disk_no, const ObCheckStatus status)
    {
      ...
      if (CHECK_ERROR == status && has_bad_disk(bad_disk) &&
          bad_disk != disk_no)
      {
        YYSYS_LOG(WARN, "already has bad_disk:%d, cannot unload disk_no:%d",
                  bad_disk, disk_no);
      }
      else
      {
        ...
        if (index == disk_no)
        {
          ...
          YYSYS_LOG(WARN, "set disk:%d check_stat:%d", disk_no, status);
        }
        else
        {
          YYSYS_LOG(WARN, "set_check_stat disk_no:%d status:%d failed, not found",
                    disk_no, status);
        }
      }
    }
