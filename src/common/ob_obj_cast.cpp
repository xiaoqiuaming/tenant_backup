...
namespace oceanbase
{
  namespace common
  {
    ...
    static int varchar_printf(ObExprObj &out, const char* format, ...)
    {
      ...
      {
        YYSYS_LOG(WARN, "output buffer for varchar not enough, buf_len=%u", varchar.length());
    ...
    static int int_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to do from in int_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
          if(params.is_modify)
          {
            ...
            if((params.precision >0 && params.scale < params.precision ) && ((params.precision-params.scale)<(od.get_precision()-od.get_vscale())))
            {
              ...
              YYSYS_LOG(WARN, "OB_DECIMAL_UNLEGAL_ERROR,od.get_precision()=%d,od.get_vscale()=%d", od.get_precision(),od.get_vscale());
            }
            else
            {
    ...
    static int float_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to convert float to decimal, err=%d", ret);
      }
      else
      {
        out.set_decimal(num);
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in float_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int double_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in double_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int datetime_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //Exp:ԭ�е�ʽ���ǽ�΢������Ϊdecimal,�����޸�Ϊ��YYYYMMDDHHMMSS.SSSSSS����ʽ
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in datetime_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int pdatetime_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      OB_ASSERT(in.get_type() == ObPreciseDateTimeType);
      //mod liuzy [datetime to decimal format fix] 20151124:b
      //Exp:ԭ�е�ʽ���ǽ�΢������Ϊdecimalֵ,�����޸�Ϊ��YYYYMMDDHHMMSS.SSSSSS����ʽ
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in pdatetime_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static const char *patt_date_V2 = "\\s*[0-9]{1,4}([0-9]{1,2}){2}\\s*$"; //YYYYMMDD
    static const char *patt_date_V3 = "\\s*[0-9]{1,4}(/[0-9]{1,2}){2}\\s*$"; //YYYY/MM/DD
    ...
    //mod 20160517:e
    {
      ...
      if (boost::regex_match(test_str, r))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Failed to get values of TIMESTAMP.");
        }
        else
        {
          is_input_date = true;
    ...
    //mod 20160517:e
    {
      ...
      if (boost::regex_match(test_str, r))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Failed to get values of DATE.");
        }
        else
        {
          is_input_date = true;
    ...
    //mod 20160517:e
    {
      ...
      if (boost::regex_match(test_str, r))
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Failed to get values of TIME.");
        }
        else
        {
          gtm.tm_year = year;
    ...
    //add for [507-expire info enhancement]-b
    static bool check_format_date_V2(const char *test_str, const ObExprObj &in, bool &is_input_date, struct tm &gtm, int &ret, bool &is_match)
    {
      is_match = false;
      int year = 0, month = 0, day = 0, hour = 0, minute = 0, second = 0;
      boost::regex r(patt_date_V2);
      if (boost::regex_match(test_str, r))
      {
        is_match = true;
        ret = varchar_scanf(in, 3, "%4d%2d%2d",
    ...
    static bool check_format_date_V3(const char *test_str, const ObExprObj &in, bool &is_input_date, struct tm &gtm, int &ret, bool &is_match)
    {
      is_match = false;
      int year = 0, month = 0, day = 0, hour = 0, minute = 0, second = 0;
      boost::regex r(patt_date_V3);
      if (boost::regex_match(test_str, r))
      {
        is_match = true;
        ret = varchar_scanf(in, 3, "%4d/%2d/%2d",
    ...
    int check_format_and_content(const ObExprObj &in, enum DateTimeType type)
    {
      int ret = OB_SUCCESS;
      int year = 0, month = 0, day = 0, hour = 0, minute = 0, second = 0;
      struct tm gtm;
      ...
      switch (type)
      {
        case TIMESTAMP_CHECK:
          if (!check_format_timestamp(test_str.ptr(), in, is_input_date, gtm, ret, is_match))
          {
            ...
            {
              //modify for [507-expire info enhancement]-b
              //            check_format_time(test_str.ptr(), in, gtm, ret, is_match);
              if (!check_format_time(test_str.ptr(), in, gtm, ret, is_match))
              {
                if(!check_format_date_V2(test_str.ptr(), in, is_input_date, gtm, ret, is_match))
                {
                  check_format_date_V3(test_str.ptr(), in, is_input_date, gtm, ret, is_match);
                }
              }
              //modify for [507-expire info enhancement]-e
            }
          }
          if (!is_match)
          {
            ret = OB_ERR_TIMESTAMP_TYPE_FORMAT;
            YYSYS_LOG(WARN, "timestamp_str:[%.*s]", test_str.length(), test_str.ptr());
        ...
        case DATE_CHECK:
          ...
          if (!is_match)
          {
            ...
            YYSYS_LOG(WARN, "date_str:[%.*s]", test_str.length(), test_str.ptr());
            YYSYS_LOG(USER_ERROR, "Format of DATE value is incorrect.");
          }
          else if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "Failed to get values of DATE.");
        ...
        case TIME_CHECK:
          ...
          if (!is_match)
          {
            ...
            YYSYS_LOG(WARN, "time_str:[%.*s]", test_str.length(), test_str.ptr());
            YYSYS_LOG(USER_ERROR, "Format of TIME value is incorrect.");
          }
          else if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "Failed to get values of TIME.");
        ...
        default:
          ...
          YYSYS_LOG(WARN, "The second parameter of function check_format_and_content is unexpected.");
          break;
      }
      ...
      if (is_match && OB_SUCCESS == ret)
      {
        ...
        if (minute == 0 && second == 0)
        {
          if (hour < 0 || hour > 24)
          {
            ...
            YYSYS_LOG(WARN, "datetime_str:[%.*s]", test_str.length(), test_str.ptr());
            YYSYS_LOG(USER_ERROR, "Value of TIME part is incorrect.");
          }
        }
        else
        {
        ...
        if (OB_SUCCESS == ret)
        {
          if (is_input_date)
          {
                ...
                month < 1 || month > 12)
            {
              ...
              YYSYS_LOG(WARN, "datetime_str:[%.*s]", test_str.length(), test_str.ptr());
              YYSYS_LOG(USER_ERROR, "Value of DATE part is incorrect.");
            }
            else
            {
    ...
    static int varchar_datetime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "Format or content is incorrect.");
        //add for [507-expire info if format incorrect]-b
        out.set_if_format_wrong(true);
      ...
      else
      {
        ...
        {
          const ObString& varchar = in.get_varchar();
          YYSYS_LOG(WARN, "failed to convert string `%.*s' to datetime type",
    ...
    static int varchar_pdatetime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        const ObString& varchar = in.get_varchar();
        YYSYS_LOG(WARN, "failed to convert string `%.*s' to precise_datetime type",
                  varchar.length(), varchar.ptr());
        out.set_precise_datetime(static_cast<ObPreciseDateTime>(0));
        out.set_if_format_wrong(true); //add for [507-expire info if format incorrect]
        //del liuzy [datetime bug] 20150928:b
        /*Exp: should not assign ret OB_SUCCESS*/
        //        ret = OB_SUCCESS;
        //del 20150928:e
    ...
    static int varchar_ctime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        const ObString& varchar = in.get_varchar();
        YYSYS_LOG(WARN, "failed to convert string `%.*s' to createtime type",
                  varchar.length(), varchar.ptr());
    ...
    static int varchar_mtime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        const ObString& varchar = in.get_varchar();
        YYSYS_LOG(WARN, "failed to convert string `%.*s' to modifytime type",
                  varchar.length(), varchar.ptr());
    ...
    /*static int varchar_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to convert varchar to decimal, err=%d varchar=%.*s",
    ...
    static int ctime_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in ctime_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int mtime_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in mtime_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int bool_varchar(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if (in.get_bool())
      {
        ...
        varchar.assign_ptr(const_cast<char*>("true"), static_cast<oceanbase::common::ObString::obstr_size_t>(sizeof("true")-1));
      }
      else
      {
        //modify for [build warning]
        //        varchar.assign_ptr(const_cast<char*>("false"), sizeof("false")-1);
        varchar.assign_ptr(const_cast<char*>("false"), static_cast<oceanbase::common::ObString::obstr_size_t>(sizeof("false")-1));
      }
      out.set_varchar(varchar);
      return OB_SUCCESS;
    }
    ...
    static int bool_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS!=(ret=od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in bool_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          // YYSYS_LOG(WARN, "test::f2 ,,(p==%d,,s==%d,,,,od.p=%d,,,s=%d", params.precision,params.scale,od.get_precision(),od.get_vscale());
    ...
    static int decimal_int(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to cast to int64, err=%d", ret);
      }
      else
      {
        out.set_int(i64);
      ...
      //{
      // YYSYS_LOG(WARN, "failed to do from in decimal_int,os=%.*s", os.length(),os.ptr());
      // }
      // else
      // {
      ...
      //{
      // YYSYS_LOG(WARN, "failed to do modify_value in od_result.modify_value(p,s),od_result=%.*s,p=%d,s=%d", os.length(),os.ptr(),p,s);
      // }
      if (OB_SUCCESS != (ret = od_result.cast_to_int64(i64)))
      {
        YYSYS_LOG(WARN, "failed to do cast_to_int64 in od_result.cast_to_int64(i64)");
      }
      else
      {
        out.set_int(i64);
    ...
    static int decimal_float(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      //{
      //    YYSYS_LOG(WARN, "failed to do from in decimal_float,os=%.*s", os.length(),os.ptr());
      // }
      // else
      //{
      // uint32_t p=od.get_precision();
    ...
    static int decimal_double(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //os=in.get_varchar();
      //add fanqiushi
      //YYSYS_LOG(ERROR, "test:: os=%.*s", os.length(),os.ptr());
      // ObDecimal od=in.get_decimal();
      ObDecimal od_result=in.get_decimal();
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      //{
      // YYSYS_LOG(WARN, "failed to do from in decimal_double,os=%.*s", os.length(),os.ptr());
      //}
      // else
      // {
      // uint32_t p=od.get_precision();
    ...
    static int decimal_datetime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      // {
      // YYSYS_LOG(WARN, "failed to do from in decimal_datetime,os=%.*s", os.length(),os.ptr());
      //}
      // else
      // {
      //uint32_t p=od.get_precision();
    ...
    static int decimal_pdatetime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      // {
      //    YYSYS_LOG(WARN, "failed to do from in decimal_pdatetime,os=%.*s", os.length(),os.ptr());
      // }
      // else
      // {
      // uint32_t p=od.get_precision();
    ...
    static int decimal_varchar(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "output buffer for varchar not enough, buf_len=%d", varchar.length());
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
        //{
        //  YYSYS_LOG(WARN, "failed to do from in decimal_varchar,os=%.*s", os.length(),os.ptr());
        // }
        // else
        // {
    ...
    static int decimal_ctime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      // {
      //  YYSYS_LOG(WARN, "failed to do from in decimal_ctime,os=%.*s", os.length(),os.ptr());
      //}
      // else
      // {
      // uint32_t p=od.get_precision();
    ...
    static int decimal_mtime(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      // if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      //{
      //YYSYS_LOG(WARN, "failed to do from in decimal_mtime,os=%.*s", os.length(),os.ptr());
      //}
      //else
      // {
      //uint32_t p=od.get_precision();
    ...
    static int decimal_bool(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //if(OB_SUCCESS != (ret = od_result.from(os.ptr(),os.length())))
      //{
      //  YYSYS_LOG(WARN, "failed to do from in decimal_bool,os=%.*s", os.length(),os.ptr());
      //}
      //else
      // {
      //od_result.set_precision(od.get_precision());
    ...
    static int decimal_int32(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to do cast_to_int64 in od_result.cast_to_int32(i32)");
      }
      else
      {
        out.set_int32(i32);
    ...
    static int date_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //Exp:ԭ�е�ʽ���ǽ�΢������Ϊdecimal,�����޸�Ϊ��YYYYMMDD����ʽ
      int ret = OB_SUCCESS;
      time_t t = static_cast<time_t>(in.get_precise_datetime()/1000000L);
      struct tm gtm;
      localtime_r(&t, &gtm);
      ...
      if(ret==OB_SUCCESS)
      {
        ...
        else
        {
          ...
          {
            out.set_varchar(os);
            out.set_decimal(od);//add for [Decimal]-b
            uint32_t len = 1;
            if (od.get_words()->table[1] != 0)
            {
              len = 2;
    ...
    */
    ...
    static int time_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      //add liuzy [datetime to decimal format fix] 20151124:b
      //Exp:ԭ�е�ʽ���ǽ�΢������Ϊdecimal,�����޸�Ϊ��HHMMSS.SSSSSS����ʽ
      int ret = OB_SUCCESS;
      int32_t usec = static_cast<int32_t>(in.get_precise_datetime()%1000000L);
      if (usec != 0)
      {
        YYSYS_LOG(WARN, "Format of TIME value is incorrect.");
      }
      time_t t = static_cast<time_t>(in.get_precise_datetime()/1000000L);
      struct tm gtm;
      localtime_r(&t, &gtm);
    ...
    static int int32_decimal(const ObObjCastParams &params, const ObExprObj &in, ObExprObj &out)
    {
      ...
      if(ret == OB_SUCCESS)
      {
        ...
        if(OB_SUCCESS != (ret = od.from(os.ptr(),os.length())))
        {
          YYSYS_LOG(WARN, "failed to do from in int32_decimal,os=%.*s", os.length(),os.ptr());
        }
        else
        {
          if(params.is_modify)
    ...
    int obj_cast(const ObObj &orig_cell, const ObObj &expected_type,
                 ObObj &casted_cell, const ObObj *&res_cell, bool is_coalesce)
    {
      int ret = OB_SUCCESS;
      ObObj obj_tmp;
      //add fanqiushi
      ...
      {
        uint64_t *tt = NULL;
        if(OB_SUCCESS!=(ret= orig_cell.get_ttint(tt)))
        {
          YYSYS_LOG(ERROR, "get decimal pointer from orig_cell error, ret = %d", ret);
        }
        else
        {
          ...
          uint32_t len_int = static_cast<uint32_t>(str_for_int.length());

          if((schema_p - schema_s) < len_int)
          {
            ret=OB_DECIMAL_UNLEGAL_ERROR;
            YYSYS_LOG(WARN, "OB_DECIMAL_UNLEGAL_ERROR,schema_p=%d,schema_s=%d,"
                      "orig_cell.get_precision()=%d,orig_cell.get_vscale()=%d,orig_cell.get_scale()=%d,value=[%s]",
                      schema_p,schema_s,orig_cell.get_precision(),orig_cell.get_vscale(),orig_cell.get_scale(), to_cstring(orig_cell));
          }
          else
          {
            casted_cell.set_precision(schema_p);
            casted_cell.set_scale(schema_s);
            casted_cell.set_vscale(vscale);
            casted_cell.set_nwords(orig_cell.get_nwords());
      ...
      else
      {
        if (orig_cell.get_type() != expected_type.get_type())
        {
          ...
          if(orig_cell.get_type()==ObDecimalType)
          {
            ...
            {
              if (OB_SUCCESS != (ret = obj_tmp.set_decimal(tt, orig_cell.get_precision(), orig_cell.get_scale(),
                                                           orig_cell.get_vscale(), orig_cell.get_nwords())))
              {
                YYSYS_LOG(WARN, "failed to do set_decimal in obj_cast()");
              }
              else
              {
                from.assign(obj_tmp);
          ...
          if(OB_SUCCESS==ret)
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to type cast obj, err=%d", ret);
            }
            else if (OB_SUCCESS != (ret = to.to(casted_cell)))
            {
              YYSYS_LOG(WARN, "failed to convert expr_obj to obj, err=%d", ret);
            }
            else
            {
              //add for [fix other cast to decimal bug]-b
              if (ObDecimalType == expected_type.get_type())
              {
                if (casted_cell.get_type() == ObDecimalType)
                {
                  char *p = sstr.ptr();
                  memset(p, 0, sstr.length());
                  uint64_t *tt = reinterpret_cast<uint64_t *>(p);
                  uint64_t *src_tt = NULL;
                  if (OB_SUCCESS != (ret = casted_cell.get_ttint(src_tt)))
                  {
                    YYSYS_LOG(ERROR, "get decimal pointer from casted_cell error,ret = %d", ret);
                  }
                  else
                  {
                    int len = (int) sizeof(uint64_t) * casted_cell.get_nwords();
                    memcpy(tt, src_tt, len);
                    casted_cell.set_ttint(tt);
                  }
                }
              }
              //add for [fix other cast to decimal bug]-e
              res_cell = &casted_cell;
            }
          }
        }
      ...
      // YYSYS_LOG(WARN, "test::obj_cast::p=%d,s=%d, type=%d", res_cell->get_precision(),res_cell->get_scale(),res_cell->get_type());
      return ret;
    }
    ...
    int obj_cast(ObObj &cell, const ObObjType expected_type, char* buf, int64_t buf_size, int64_t &used_buf_len)
    {
      ...
      if (cell.get_type() != expected_type)
      {
        ...
        else
        {
          ...
          if (ObDecimalType == expected_type)
          {
            if (cell.get_type() == ObDecimalType)
            {
              uint64_t *tt = reinterpret_cast<uint64_t *>(buf);
              int32_t old_len = to.get_varchar().length();
              memset(tt, 0, old_len);
              uint64_t *src_tt = NULL;
              if (OB_SUCCESS != (ret = cell.get_ttint(src_tt)))
              {
                YYSYS_LOG(ERROR, "get decimal pointer from cell error,ret = %d", ret);
              }
              else
              {
                ...
                memcpy(tt, src_tt, len);
                cell.set_ttint(tt);
                used_buf_len = len; // used buffer length for casting to decimal type
              }
            }
            //modify for [Decimal]-e
