...
int ObServerConfig::read_config()
{
  ...
  ObServer server;
  server.set_ipv4_addr(yysys::CNetUtil::getLocalAddr(devname), 0);
  ...
  else
  {
    ...
    for (; it != container_.end(); it++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Read config error! name: [%s], ret: [%d]", it->first.str(), ret);
      }
    }
  }
  return ret;
...
int ObServerConfig::check_all() const
{
  ...
  for (; it != container_.end(); it++)
  {
    if (!it->second->check())
    {
      YYSYS_LOG(INFO, "Configure setting invalid! name: [%s], value: [%s]",
...
{
  YYSYS_LOG(INFO, "===================== *begin server config report * =====================");
  ObConfigContainer::const_iterator it = container_.begin();
  for (; it != container_.end(); it++)
  {
    YYSYS_LOG(INFO, "| %-36s = %s", it->first.str(), it->second->str());
  }
  YYSYS_LOG(INFO, "===================== *stop server config report* =======================");
}
...
int ObServerConfig::add_extra_config(const char* config_str, bool check_name /* = false */)
{
  ...
  {
    YYSYS_LOG(ERROR, "Extra config is too long!");
    ret = OB_BUF_NOT_ENOUGH;
  }
  else
  {
  ...
  while (NULL != token && OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "Invalid config string: [%s]", config_str);
      ret = OB_INVALID_CONFIG;
    }
    ...
    {
      YYSYS_LOG(WARN, "Invalid config string, no such config item!"
    ...
    else
    {
      ...
      YYSYS_LOG(INFO, "Load config succ! name: [%s], value: [%s]", name, value);
    }
    token = strtok_r(NULL, ",\n", &saveptr);
  }
...
//static const int HEADER_LENGTH = sizeof (uint32_t) + sizeof (uint64_t);
  static const int HEADER_LENGTH = static_cast<int>(sizeof (uint32_t) + sizeof (uint64_t));
...
DEFINE_DESERIALIZE(ObServerConfig)
{
  ...
  else
  {
    ...
    if (hash_r != hash)
    {
      YYSYS_LOG(ERROR, "Config file broken, deserialize server config error!");
    ...
    {
      YYSYS_LOG(ERROR, "Read server config failed!");
    }
    else
    {
      pos += length;
