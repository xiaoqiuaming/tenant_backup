...
{ 
  namespace common
  {
    ...
    DEFINE_SERIALIZE(ObVersionRange)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "buffer size is not enough, ret=[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = border_flag_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(ERROR, "serialize border_flag failed, ret=[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = start_version_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(ERROR, "serialize start_version failed, ret=[%d]", ret);
      }
      else if (OB_SUCCESS != (ret = end_version_.serialize(buf, buf_len, pos)))
      {
        YYSYS_LOG(ERROR, "serialize end_version failed, ret=[%d]", ret);
      }
      return ret;
    }
    ...
    DEFINE_DESERIALIZE(ObVersionRange)
    {
      ...
      {
        YYSYS_LOG(ERROR, "deserialize border_flag failed");
      }
      else if (OB_SUCCESS != (ret = start_version_.deserialize(buf, data_len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize start_version failed");
      }
      else if (OB_SUCCESS != (ret = end_version_.deserialize(buf, data_len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize end_version failed");
      }
      return ret;
    }
    //add:e
    ...
    bool ObRange::check(void) const
    {
      ...
      {
        YYSYS_LOG(WARN, "%s", "check start key gt than end key");
        ret = false;
      }
      else if (start_key_ == end_key_)
      {
        if (!border_flag_.is_min_value() && !border_flag_.is_max_value())
        {
          ...
          {
            YYSYS_LOG(WARN, "check border flag or length failed:length[%d], flag[%u]",
      ...
      if(OB_SUCCESS == ret)
      {
        if( 0 == compare_with_startkey2(r) ) // left interval equal
        {
          ...
          else
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(DEBUG, "dump this range");
              dump();
              YYSYS_LOG(DEBUG, "dump r range");
              r.dump();
    ...
    DEFINE_DESERIALIZE(ObRange)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to deserialize start key, buf=%p, data_len=%ld, pos=%ld, ret=%d",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to deserialize end key, buf=%p, data_len=%ld, pos=%ld, ret=%d",
                    buf, data_len, pos, ret);
        }
      }
