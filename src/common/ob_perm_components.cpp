...
namespace oceanbase
{
  namespace common
  {
    ...
    bool is_supper_account(const ObString &username)
    {
      ...
      {
        //YYSYS_LOG(WARN, "[%.*s] not supper account [%.*s]",
    ...
    int ObUserTable::userinfo_check(const ObString &username, const ObString &password) const
    {
      ...
      if (OB_SUCCESS != (ret = const_cast<ObUserTableCache&>(cache_).get(key, value, RPC_TIMEOUT, ref_guard)))
      {
        ...
        {
          YYSYS_LOG(WARN, "get from cache fail ret=%d username=[%.*s]", ret, username.length(), username.ptr());
          ret = (OB_ENTRY_NOT_EXIST == ret) ? OB_USER_NOT_EXIST : ret;
        }
      }
      else if (password != value.get_passwd())
      {
        //YYSYS_LOG(WARN, "password wrong [%.*s] [%.*s]", password.length(), password.ptr(), value.get_passwd().length(), value.get_passwd().ptr());
        ret = OB_PASSWORD_WRONG;
      }
      else
      {
    ...
    int ObSKeyTable::get_skey(const int64_t version, int64_t &skey) const
    {
      ...
      {
        YYSYS_LOG(WARN, "get from cache fail ret=%d version=%ld", ret, version);
      ...
      {
        YYSYS_LOG(WARN, "invalid skey_info ptr=%p length=%d", value.get_skey().ptr(), value.get_skey().length());
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObSKeyTable::get_cur_skey(int64_t &version, int64_t &skey) const
    {
      ...
      if (OB_SUCCESS != (ret = const_cast<ObSKeyTableCache&>(cache_).get(key, value, RPC_TIMEOUT, ref_guard)))
      {
        YYSYS_LOG(WARN, "get cur_version from cache fail ret=%d", ret);
      }
      else if (NULL == value.get_skey().ptr()
               || CUR_SKEY_INFO_LENGTH > value.get_skey().length())
      {
        YYSYS_LOG(WARN, "invalid cur_skey_info ptr=%p length=%d %.*s",
    ...
    int ObSKeyTable::update_cur_skey(ObMutator &mutator) const
    {
      ...
      {
        YYSYS_LOG(WARN, "mutator insert fail ret=%d", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "mutator update fail ret=%d", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "build mutator for skey switch succ, new version=%ld", timestamp);
        }
      }
      return ret;
    }
    ...
    int ObPermTable::check_table_writeable(const IToken &token, const uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "get username from token fail ret=%d", ret);
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "table_id=%lu username=[%.*s] perm_mask=%ld",
