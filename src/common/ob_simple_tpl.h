    ...
    struct TSI
    {
      ...
      public:
        TSI(Factory& factory): create_err_(0), factory_(factory)
        {
          ...
          create_err_ = PTHREAD_KEY_MGR->create_thread_key(key_);
        }
        ~TSI()
        {
          if (0 == create_err_)
        ...
        T* get()
        {
          ...
          else if (NULL != (val = (T*)(PTHREAD_KEY_MGR->get_thread_specifi(key_))))
          {}
          ...
          else if (0 != (err = PTHREAD_KEY_MGR->set_thread_specifi(key_, val)))
          {}
          if (0 != err)
          {
            destroy(val);
    ...
    class ObHandyAllocatorWrapper: public Allocator
    {
      ...
      public:
        ...
        int write_string(const ObString& str, ObString& stored_str)
        {
          ...
          {
            err = OB_MEM_OVERFLOW;
            YYSYS_LOG(ERROR, "alloc(%d)=>%d", str.length(), err);
          }
          else
          {
        ...
        int write_string(const ObRowkey& rowkey, ObRowkey& stored_rowkey)
        {
          ...
          {
            err = OB_MEM_OVERFLOW;
            YYSYS_LOG(ERROR, "alloc(len=%ld)=>%d", len, err);
          }
          else
          {
        ...
        int write_obj(const ObObj& obj, ObObj& stored_obj)
        {
          ...
          {
            YYSYS_LOG(ERROR, "obj.get_varchar()=>%d", err);
          }
          else if (OB_SUCCESS != (err = write_string(value, new_value)))
          {
            YYSYS_LOG(ERROR, "write_string(%*s)=>%d", value.length(), value.ptr(), err);
          }
          else
          {
