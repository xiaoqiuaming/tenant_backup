...
{ \
  ...
  { \
    YYSYS_LOG(WARN, "serialize_param args error. ret:%d, buffer cap:%ld, pos:%ld", \
  ...
  { \
    YYSYS_LOG(WARN, "serialize_param argn error. ret:%d, buffer cap:%ld, pos:%ld", \
        ret, data_buffer.get_capacity(), data_buffer.get_position()); \
  } \
  return ret; \
}
...
{ \
  ...
  { \
    YYSYS_LOG(WARN, "deserialize rets error. ret:%d, buffer length:%ld, pos:%ld", \
  ...
  { \
    YYSYS_LOG(WARN, "deserialize retn error. ret:%d, buffer length:%ld, pos:%ld", \
        ret, data_buffer.get_position(), pos); \
  } \
  return ret; \
}
...
{ \
  ...
  { \
    YYSYS_LOG(WARN, "serialize_param error."); \
  } \
  else if (OB_SUCCESS != (ret = rpc_frame_->send_request( \
          server, pcode, version, timeout, data_buffer))) \
  { \
    YYSYS_LOG(WARN, "send_request error, ret:%d, server=%s, pcode=%d, version=%d, timeout=%ld", \
...
{ \
  ...
  { \
    YYSYS_LOG(WARN, "send_param error, ret=%d, server=%s, pcode=%d, version=%d, timeout=%ld", \
        ret, to_cstring(server), pcode, version, timeout); \
  } \
  ...
  { \
    YYSYS_LOG(WARN, "deserialize_result error, ret=%d, server=%s, pcode=%d, version=%d, timeout=%ld", \
        ret, to_cstring(server), pcode, version, timeout); \
  } \
  return ret; \
}
