...
int ObSingleLogReader::init(const char* log_dir)
{
  ...
  if (is_initialized_)
  {
    YYSYS_LOG(ERROR, "ObSingleLogReader has been initialized");
    ret = OB_INIT_TWICE;
  }
  else
  {
    if (NULL == log_dir)
    {
      YYSYS_LOG(ERROR, "Parameter is invalid[log_dir=%p]", log_dir);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "log_dir is too long[len=%d log_dir=%s]", log_dir_len, log_dir);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
  ...
  if (OB_SUCCESS == ret)
  {
    if (NULL == log_buffer_.get_data())
    {
      ...
      if (NULL == buf)
      {
        YYSYS_LOG(ERROR, "ob_malloc for log_buffer_ failed");
        ret = OB_ERROR;
      }
      else
      {
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO, "ObSingleLogReader initialize successfully");
    last_log_seq_ = 0;
...
int ObSingleLogReader::get_max_log_file_id(uint64_t &max_log_file_id)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "fail to init scanner.");
  }
  else
  {
...
int ObSingleLogReader::open(const uint64_t file_id, const uint64_t last_log_seq/* = 0*/)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (err < 0)
    {
      YYSYS_LOG(ERROR, "snprintf file_name[log_dir_=%s file_id=[%lu] error[%s]", log_dir_, file_id, strerror(errno));
      ret = OB_ERROR;
    }
    else if (err >= OB_MAX_FILE_NAME_LENGTH)
    {
      YYSYS_LOG(ERROR, "snprintf file_name[file_id=%lu] error[%s]", file_id, strerror(errno));
      ret = OB_ERROR;
    }
    else
    {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "open log file(name=%s id=%lu)", file_name_, file_id);
      }
      else if (OB_FILE_NOT_EXIST == ret)
      {
        YYSYS_LOG(DEBUG, "log file(name=%s id=%lu) not found", file_name_, file_id);
      }
      else
      {
        YYSYS_LOG(WARN, "open file[name=%s] error[%s]", file_name_, strerror(errno));
      }
    }
  }
...
int ObSingleLogReader::open_with_lucky(const uint64_t file_id, const uint64_t last_log_seq)
{
  ...
  if (file_id == file_id_ )
  {
    if(last_log_seq != 0 && last_log_seq_ != 0 && last_log_seq != last_log_seq_)
    {
      ...
      YYSYS_LOG(ERROR, "open(file_id=%ld, req_log_seq=%ld, last_log_seq=%ld)=>%d",
                file_id, last_log_seq, last_log_seq_, err);
    }
  }
  else if (OB_SUCCESS != (err = close()))
  {
    YYSYS_LOG(ERROR, "close()=>%d", err);
  }
  else if (OB_SUCCESS != (err = open(file_id, last_log_seq)))
  {
    if (OB_FILE_NOT_EXIST == err)
    {
      YYSYS_LOG(WARN, "open(file_id=%ld, last_log_seq=%ld)=>%d", file_id, last_log_seq, err);
    }
    else
    {
      YYSYS_LOG(ERROR, "open(file_id=%ld, last_log_seq=%ld)=>%d", file_id, last_log_seq, err);
    }
  }
  return  err;
}
...
int ObSingleLogReader::close()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (last_log_seq_ == 0)
    {
      YYSYS_LOG(INFO, "close file[name=%s], read no data from this log", file_name_);
    }
    else
    {
      YYSYS_LOG(INFO, "close file[name=%s] successfully, the last log sequence is %lu",
...
int ObSingleLogReader::read_header(ObLogEntry& entry)
{
  ...
  if (log_buffer_.get_remain_data_len() < entry.get_serialize_size())
  {
    ...
    {
      YYSYS_LOG(ERROR, "read_log_()=>%d", err);
    }
  }
  if (OB_SUCCESS != err)
  {}
  else if (log_buffer_.get_remain_data_len() < entry.get_serialize_size())
  {
    ...
    {
      YYSYS_LOG(INFO, "Reach the end of log");
      err = OB_READ_NOTHING;
    }
    else
    {
      YYSYS_LOG(ERROR, "Get an illegal log entry, log file maybe corrupted");
                                  ...
                                  log_buffer_.get_limit() - log_buffer_.get_position()))
  {
    ...
    log_buffer_.get_limit() = log_buffer_.get_position();
    //YYSYS_LOG(WARN, "read eof, reset pread_pos=%ld, log_buf_pos=%ld", pread_pos_, log_buffer_.get_limit());
  }
  else if (OB_SUCCESS != (err = entry.deserialize(log_buffer_.get_data(), log_buffer_.get_limit(),
                                                  log_buffer_.get_position())))
  {
    err = OB_LAST_LOG_RUINNED;
    YYSYS_LOG(ERROR, "Log entry deserialize error");
  }
  return err;
}
...
int ObSingleLogReader::read_log_()
{
  ...
  YYSYS_LOG(DEBUG, "pread:: pread_pos=%ld read_size=%ld buf_pos=%ld buf_limit=%ld", pread_pos_, read_size, log_buffer_.get_position(), log_buffer_.get_limit());
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "read log file[file_id=%lu] ret=%d", file_id_, ret);
  }
  else
  {
    // comment this log due to too frequent invoke by replay thread
    //YYSYS_LOG(DEBUG, "read %dB amount data from log file[file_id=%lu fd=%d]", err, file_id_, log_fd_);
    ...
    {
      // comment this log due to too frequent invoke by replay thread
      //YYSYS_LOG(DEBUG, "reach end of log file[file_id=%d]", file_id_);
      ret = OB_READ_NOTHING;
    }
  }
