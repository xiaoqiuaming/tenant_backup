...
namespace oceanbase
{
  namespace common
  {
    ...
    volatile int TraceLog::log_level_ = YYSYS_LOG_LEVEL_TRACE;
    ...
    TraceLog::LogBuffer &TraceLog::get_logbuffer()
    {
      ...
      if (!inited)
      {
        YYSYS_LOG(INFO, "init trace log buffer tid=%ld buffer=%p", GETTID(), &log_buffer);
    ...
    int TraceLog::dec_log_level()
    {
      ...
      if (YYSYS_LOG_LEVEL_DEBUG >= modify_log_level
          && YYSYS_LOG_LEVEL_ERROR <= modify_log_level)
      {
        log_level_ = modify_log_level;
      }
      YYSYS_LOG(INFO, "dec_log_level prev_log_level=%d cur_log_level=%d", prev_log_level, log_level_);
      return log_level_;
    }
    ...
    int TraceLog::inc_log_level()
    {
      ...
      YYSYS_LOG(INFO, "inc_log_level prev_log_level=%d cur_log_level=%d", prev_log_level, log_level_);
      return log_level_;
    }
    ...
    int TraceLog::set_log_level(const char *log_level_str)
    {
      ...
      {
        int level_num = static_cast<int>(sizeof(level_strs_) / sizeof(const char*));
    ...
    {
      if (get_log_level() > YYSYS_LOGGER._level && !log_buffer.force_log_)
      {
    ...
    {
      //����
      YYSYS_LOG(DEBUG, "trace buff not enough, will reset trace buffer total[%ld] used[%ld]",
