  ...
  namespace common
  {
    ...
    class ObCompactCellWriter
    {
      ...
      public:
        ...
        /*
         * д��һ��cell
         * ����ϡ���ʽд�룬column_id����ΪOB_INVALID_IDҲ�����ڳ��ܸ�ʽ
         */
        int append(uint64_t column_id, const ObObj &value, ObObj *clone_value = NULL);

        //add for [369-full join]-b
        int append_opt(uint64_t column_id, const ObObj &value, const bool& need_copy/*=false*/, ObObj *clone_value = NULL );
        //add for [369-full join]-e

        /*
         * д��һ��cell�����ڳ��ܸ�ʽ����column_id
         */
      ...
      protected:
        ...
        //modify for [369-full join]-b
        //modify wenghaixing DECIMAL OceanBase_BankCommV0.3 2014_7_10:b
        //        int write_decimal(const ObObj &decimal, ObObj *clone_value);
        int write_decimal(const ObObj &decimal, ObObj *clone_value, bool need_copy = true);
        //modify:e
        int write_varchar_opt(const ObObj &value, ObObj *clone_value, bool need_copy);//add for [369-full join]
        //modify for [369-full join]-e
                                           ...
                                           common::ObObj *clone_value /* = NULL */)
    {
      ...
      {
        YYSYS_LOG(WARN, "append value fail:ret[%d]", ret);
      }
      return ret;
    }
  }
