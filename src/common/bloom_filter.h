    ...
    class ObBasicBloomFilter : public BloomFilter
    {
      ...
      public:
        ...
        int init(int64_t element_count, float false_positive_prob = BLOOM_FILTER_FALSE_POSITIVE_PROB)
        {
          ...
          if (element_count <= 0)
          { 
            YYSYS_LOG(ERROR, "bloom filter element_count should be > 0 ");
            ret = common::OB_INVALID_ARGUMENT;
          }
          else if (!(false_positive_prob < 1.0 || false_positive_prob > 0.0))
          { 
            YYSYS_LOG(ERROR, "bloom filter false_positive_prob should be < 1.0 and > 0.0");
            ret = common::OB_ERROR;
          }
          else
          {
        ...
        inline common::ObArray<common::ObObjType> get_rowkey_type_desc() const
        { return rowkey_type_desc_; }
        inline common::ObArray<common::ObObjType> * get_rowkey_type_desc_ptr()
        { return &rowkey_type_desc_; }
        inline void add_rowkey_type_desc(common::ObArray<common::ObObjType>& rowkey_type_desc)
        { rowkey_type_desc_ = rowkey_type_desc; }
        //add 20151225:e
        //add peiouya [IN_TYPEBUG_FIX] [Double_Free_fix] 20160223:b
        inline int64_t get_rowkey_type_desc_count() const
        { return rowkey_type_desc_.count(); }
        //add 20160223:e
        //mod peiouya [IN_TYPEBUG_FIX] 20151225:b
      ...
      //add peiouya [IN_TYPEBUG_FIX] 20151225:b
      private:
        inline int get_rowkey_type_desc_serialize_size() const
        { return static_cast<int>(rowkey_type_desc_.count () * serialization::encoded_length_i8 (static_cast<int8_t> (0)) + serialization::encoded_length_i32 (0)); }
