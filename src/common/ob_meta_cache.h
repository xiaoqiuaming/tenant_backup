    ...
    class ObMetaCache
    {
      ...
      public:
        int init(const int64_t cache_timeout, IRpcStub *rpc_stub, const int64_t max_cache_num)
        {
          ...
          {
            YYSYS_LOG(ERROR, "illegal param cache_timeout=%ld, rpc_stub=%p", cache_timeout, rpc_stub);
            ret = OB_ERROR;
          }
          ...
          if (OB_SUCCESS == ret)
          {
            ...
            inited_ = true;
            YYSYS_LOG(DEBUG, "init cache succ, max_cache_num=%ld, cache_timeout=%ld",
       ...
       */
        int get(Key& key, Value& value, const int64_t timeout, RefGuard<Key,Value> &refGuard)
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS == ret)
            {
              if (value.expired(yysys::CTimeUtil::getTime(),cache_timeout_))
              {
                kv_cache_.erase(key);
                need_rpc = true;
                ret = OB_ERROR;
            ...
            if (need_rpc && OB_SUCCESS != ret)
            {
              ...
              if (OB_SUCCESS != ret)
              {
                YYSYS_LOG(WARN, "build get param failed");
              }
              else
              {
                scanner->reset();
       ...
       */
        int set_cache_timeout(const int64_t cache_timeout)
        {
          ...
          if (0 > cache_timeout)
          {
            YYSYS_LOG(ERROR, "illegal param cache_timeout=%ld", cache_timeout);
            ret = OB_ERROR;
          }
