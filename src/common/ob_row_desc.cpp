...
int ObRowDesc::add_column_desc(const uint64_t table_id, const uint64_t column_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "too many column for a row, tid=%lu cid=%lu",
              table_id, column_id);
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to insert column desc, err=%d", ret);
    }
    else
    {
      ++cells_desc_count_;
...
int ObRowDesc::hash_insert(const uint64_t table_id, const uint64_t column_id, const int64_t index)
{
  ...
  {
    YYSYS_LOG(WARN, "duplicated cell desc, tid=%lu cid=%lu new_idx=%ld",
...
ObRowDesc & ObRowDesc::operator = (const ObRowDesc & r)
{
  ...
  for (int64_t i = 0; i < r.cells_desc_count_; i++)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "add_column_desc error, err: %d r: %s", err, to_cstring(r));
      break;
    }
  }
  return *this;
...
//DEFINE_SERIALIZE(ObRowDesc)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to encode rowkey cell count[%ld] :ret[%d]", rowkey_cell_count_, ret);
    }
  }
...
DEFINE_DESERIALIZE(ObRowDesc)
{
  ...
  while (column_cnt-- && OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "decode tid error, ret: %d", ret);
      break;
    }
    if (OB_SUCCESS != (ret = decode_vi64(buf, data_len, pos, &cid)))
    {
      YYSYS_LOG(ERROR, "decode cid error, ret: %d", ret);
      break;
    }
    if (OB_SUCCESS != (ret = add_column_desc(tid, cid)))
    {
      YYSYS_LOG(ERROR, "add column desc error, ret: %d", ret);
      break;
    }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to decode rowkey_cell_count_:ret[%d]", ret);
    }
  }
...
DEFINE_GET_SERIALIZE_SIZE(ObRowDesc)
{
  ...
  for (int64_t i = 0; i < column_cnt; i++)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get tid cid error, err: %d", err);
      break;
    }
