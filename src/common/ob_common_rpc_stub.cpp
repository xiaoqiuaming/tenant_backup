...
int ObCommonRpcStub :: init(const ObClientManager* client_mgr)
{
  ...
  if (NULL == client_mgr)
  {
    YYSYS_LOG(WARN, "invalid param, client_mgr=%p", client_mgr);
    ...
    const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send log data to slave failed "
    ...
    const common::ObServer& slave_addr, const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to serialize slave addr, err[%d].", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send renew_lease failed, err[%d].", err);
    }
  }
    ...
    const ObLease& lease, const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to serialize lease, err[%d].", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send grant lease failed, err[%d].", err);
    }
  }
    ...
    const int64_t timeout_us)
{
  ...
  YYSYS_LOG(INFO, "send slave(%s) quit info to Master(%s), err[%d].", to_cstring(slave_addr), to_cstring(master), err);
  return err;
}

//add peiouya [MultiUPS] [UPS_Manage_Function] 20150429:b 
int ObCommonRpcStub :: get_master_ups_info_by_paxos_id(const ObServer& rs, ObServer &master_ups, const int64_t paxos_id, const int64_t timeout_us)
{ 
  int err = OB_SUCCESS;
  ObDataBuffer data_buff;
  ObServer new_ups;
  ...
  if (OB_SUCCESS == err)
  {
    err = serialization::encode_i64(data_buff.get_data(), data_buff.get_capacity(), data_buff.get_position(), paxos_id);
    //YYSYS_LOG(WARN, "paxos_id:%ld serialization failed.", paxos_id); //del peiouya 20150811
  }

  if (OB_SUCCESS == err)
  {
    if (OB_SUCCESS != (err = client_mgr_->send_request(rs, OB_GET_UPDATE_SERVER_INFO_BY_PAXOS_ID, DEFAULT_VERSION, timeout_us, data_buff)))
    {//[uncertainty] :b
      YYSYS_LOG(WARN, "fail to get master ups. , rootserver=%s, err=%d", rs.to_cstring(), err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to deserialize master ups. err=%d", err);
    }
...
int ObCommonRpcStub :: get_obi_role(const common::ObServer& rs, common::ObiRole &obi_role, const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send get obi role failed, err[%d].", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "get obi response error, err=%d", err);
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "deserialize ObObiRole failed:pos[%ld], err[%d].", pos, err);
    }
  }
...
int ObCommonRpcStub :: send_obi_role(const common::ObServer& slave, const common::ObiRole obi_role)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to serialize obi_role, err[%d].", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "send send request failed, err[%d].", err);
    }
  }
...
int ObCommonRpcStub::scan(const ObServer& ms, const ObScanParam& scan_param, ObScanner& scanner, const int64_t timeout, int64_t* session_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to send request to (%s), ret=%d", to_cstring(ms), ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize result data failed:pos[%ld], ret[%d]", pos, ret);
          }
        }
      }
    }
 ...
 if (OB_SUCCESS == ret)
  {
    ObDataBuffer data_buff;
    get_thread_buffer_(data_buff);
    YYSYS_LOG(INFO,"MS IS %s SESSION_ID %ld", to_cstring(ssession.server_),ssession.session_id_);
    ret = client_mgr_->get_next(ssession.server_, ssession.session_id_, timeout, data_buff, data_buff);

    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "failed to send request to (%s), ret=%d", to_cstring(ssession.server_), ret);
    }
    else
    {
      // deserialize the response code
...
int ObCommonRpcStub::mutate(const ObServer& update_server, const ObMutator& mutator, const int64_t timeout)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to send mutate request to updateserver=%s, ret=%d", to_cstring(update_server),ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR,"apply failed, err=%d ups=%s",result_code.result_code_, to_cstring(update_server));
          ret = result_code.result_code_;
        }
      }
    }
...
int ObCommonRpcStub::mutate_distribute_stat(const ObServer& update_server, const ObTransID& trans_id,
                                            const bool commit, const int64_t memtable_version)
{
  int ret = OB_SUCCESS;
  static const int MY_VERSION = 1;
  ObDataBuffer data_buff;
  ...
  {
    if(OB_SUCCESS != (ret = trans_id.serialize(data_buff.get_data(), data_buff.get_capacity(), data_buff.get_position())))
    {
      YYSYS_LOG(ERROR, "failed to serialize trans id, ret = %d", ret);
    }
    else if(OB_SUCCESS != (ret = serialization::encode_bool(data_buff.get_data(), data_buff.get_capacity(),
                                                            data_buff.get_position(),commit)))
    {
      YYSYS_LOG(ERROR, "failed to serialize commit stat, ret = %d", ret);
    }
    else if(OB_SUCCESS != (ret = serialization::encode_i64(data_buff.get_data(), data_buff.get_capacity(),
                                                            data_buff.get_position(),memtable_version)))
    {
      YYSYS_LOG(ERROR, "failed to serialize memtable_version, ret = %d", ret);
    }
    else
    {
      ret = client_mgr_->post_request(update_server,OB_WRITE_TRANS_STAT,MY_VERSION,data_buff);
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to send mutate request to updateserver=%s, ret=%d", to_cstring(update_server),ret);
      }
