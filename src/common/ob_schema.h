...
namespace oceanbase
{
  namespace common
  {
    ...
    const int64_t OB_SCHEMA_VERSION_SEVEN = 7;

    //[view]
    const int64_t OB_SCHEMA_VERSION_EIGHT = 8;
    const int64_t OB_SCHEMA_VERSION_EIGHT_FIRST = 801;
    ...
    struct IndexList
    {
      ...
      public:
        ...
        inline int add_index(uint64_t tid)
        {
          ...
          {
            YYSYS_LOG(WARN,"the index num of list is full!");
          }
    ...
    class ObColumnSchemaV2
    {
      public:
        ...
        inline bool get_default_value_is_nullable() const
        { return default_value_is_null;}
        const char *get_default_val() const;
        void set_default_val(const char *default_val);

        const int get_default_val_len() const
        { return static_cast<int>(strlen(default_val_)); }
        const bool get_has_sync() const
        { return has_sync_; }
        const ObObj get_default_val_obj() const
        { return default_val_obj_; }

        int calc_default_val_obj(ObObj &val);
        ...
        int deserialize_v5(const char* buf, const int64_t data_len, int64_t& pos);

        inline bool is_nullable() const
        { return is_nullable_; }
        inline void set_nullable(const bool null)
        { is_nullable_ = null; }
        inline const ObObj & get_default_value()  const
        { return default_value_; }
        inline void set_default_value(const ObObj& value)
      ...
      private:
        ...
        char default_val_[OB_MAX_DEBUG_MSG_LEN];
        int default_val_len_;
        bool has_sync_;
        ObObj default_val_obj_;
    ...
    class ObTableSchema
    {
      public:
        ...
        void set_text(const ObString &text);
        void set_table_type(ObTableType type);
        const char *get_text() const;
        ObTableType get_type() const;
        void set_check_option(ObWithCheckOption with_check_option);
        ObWithCheckOption get_check_option() const;
        void set_is_updatable(bool is_updatable);
        bool get_is_updatable() const;
      ...
      private:
        ...
        /*
         *����ɾ����index_list_���飬ֻ����index_helper_
         *Ȼ����schema_manager���й���һ���ڴ��е�������Ӧ��ϵ
         **/
        //add wenghaixing [secondary index] 20141104
        IndexHelper ih_;
        ...
        ObWithCheckOption with_check_option_;
        bool is_updatable_;
    };
    ...
    class ObSchemaManagerV2
    {
      ...
      public:
        ...
        { return timestamp_; }//add zhaoqiong [Schema Manager] 20150327
        void set_version(const int64_t version);
        ...
        //add liumz, [MultiUPS]20170308:b
        uint64_t get_data_table_td(const uint64_t table_id)const;
        //add:e
        ...
        //add:e
      public:
        bool parse_from_file(const char* file_name, yysys::CConfig& config);
        bool parse_one_table(const char* section_name, yysys::CConfig& config, ObTableSchema& schema);
        bool parse_column_info(const char* section_name, yysys::CConfig& config, ObTableSchema& schema);
        bool parse_join_info(const char* section_name, yysys::CConfig& config, ObTableSchema& schema);
        bool parse_rowkey_info(const char* section_name, yysys::CConfig& config, ObTableSchema& schema);
      ...
      public:
        ...
        int get_all_orphan_index(ObArray<uint64_t> &index_id_list, ObArray<ObString> &index_name_list) const; //[571]
        ...
        int check_index_table_expire_column(const ObSchemaManagerV2 &schema_manager, const ObTableSchema &table_schema, ObString &column_name) const;

        int check_new_expire_condition_for_alter(const ObSchemaManagerV2 &schema_manager,
                                                 const ObTableSchema &table_schema,
                                                 const ObString &expire_condition,
                                                 const ObString &old_expire_condition,
                                                 bool is_add = false) const;

        int check_expire_condition_column(const ObSchemaManagerV2 &schema_manager,const ObTableSchema &table_schema, const char *expire_condition) const;
        bool check_alter_column_in_expire_columns(const ObTableSchema &table_schema, const char *expire_condition, char *column_name) const;
         ...
         */
        void get_next_start_pos(int64_t& table_start, int64_t& column_start) const
        {
          table_start = table_end_;
          column_start = column_end_;
        }
      ...
      private:
        ...
        //[602]
        volatile bool ts_hash_inited_;
        volatile bool use_ts_hash_;
        hash::ObHashMap<uint64_t, ObTableSchema*, hash::NoPthreadDefendMode> tid_hash_map_;
        hash::ObHashMap<ObString, ObTableSchema*, hash::NoPthreadDefendMode> tname_hash_map_;
    ...
    int ObSchemaManagerV2::fetch_schema_followed(RpcT& rpc_stub, const int64_t timeout, const ObServer & root_server,const int64_t version)
    {
      ...
      while (OB_SUCCESS == ret && !is_completion())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fetch next-schema error, ret = %d", ret);
        }
      }
      return ret;
    }
    ...
    class ObSchemaMutator
    {
      ...
      public:
        ...
        { return end_version_; }
        bool get_refresh_schema() const
        { return refresh_new_schema_; }
        void set_refresh_schema(bool refresh_schema)
        { refresh_new_schema_ = refresh_schema; }

        ObArray<int64_t>& get_droped_tables()
        { return droped_tables_; }
        ObArray<TableSchema>& get_add_table_schema()
        { return add_table_schema_; }
        const ObArray<int64_t>& get_droped_tables() const
        { return droped_tables_; }
        const ObArray<TableSchema>& get_add_table_schema() const
        { return add_table_schema_; }
    ...
    class ObSchemaSortByIdHelper
    {
      ...
      public:
        ...
        { return table_infos_index_; }
        const Item* end() const
        { return table_infos_index_ + table_nums_; }
