...
DEFINE_SERIALIZE(ObDataSourceDesc)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to serialize data source desc, %s: ret=%d", to_cstring(*this), ret);
  }
...
DEFINE_DESERIALIZE(ObDataSourceDesc)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to deserialize data source desc, ret=%d", ret);
  }
