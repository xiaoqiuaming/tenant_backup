...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObStringBufT<PageAllocatorT, PageArenaT> :: write_string(const ObString& str, ObString* stored_str)
    {
      ...
      else
      {
        ...
        if (OB_UNLIKELY(NULL == str_clone))
        {
          ...
          YYSYS_LOG(WARN, "failed to dup string");
        }
    ...
    //add for [Decimal]-b
    template <typename PageAllocatorT, typename PageArenaT>
    int ObStringBufT<PageAllocatorT, PageArenaT>::write_decimal(const uint64_t *dec, uint64_t*& stored_dec, uint32_t len)
    {
      int err = OB_SUCCESS;
      ...
      else
      {
        ...
        {
          stored_dec = reinterpret_cast<uint64_t *>(str_clone);
    ...
    int ObStringBufT<PageAllocatorT, PageArenaT> :: write_obj(const ObObj& obj, ObObj* stored_obj)
    {
      ...
      else if (ObDecimalType == type)
      {
        ...
        if (OB_SUCCESS != (err = obj.get_ttint(src_val)))
        {
          YYSYS_LOG(ERROR, "get decimal pointer from obj error, ret = %d", err);
        }
        else
        {
          err = write_decimal(src_val, dst_val, obj.get_nwords());
        }
        if (OB_SUCCESS == err)
        {
          if (NULL != stored_obj && dst_val != NULL)
          {
            stored_obj->set_decimal(dst_val,obj.get_precision(),obj.get_scale(),
                                    obj.get_vscale(), obj.get_nwords());
          }
        }
      }
      //add e
...
//            {
//                YYSYS_LOG(WARN, "Faild to do set decimal in write_obj_varchar_to_decimal");
//            }
//            else
//            {
