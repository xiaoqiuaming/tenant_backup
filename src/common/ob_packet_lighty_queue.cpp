...
#define futex(...) syscall(SYS_futex,__VA_ARGS__)
        ...
        int32_t inc(const timespec* timeout, bool block)
        {
          ...
          else
          {
            ...
            while(true)
            {
              ...
              {
                val = __sync_fetch_and_add(&val_, 1);
                YYSYS_LOG(WARN, "inc timeout");
                break;
              }
              //�����ѻ��ߣ�val_��futex_wait֮ǰ�����ˣ�futex_waitֱ�ӷ���
              if ((val = inc_if_le0(&val_)) <= 0)
              {
                break;
              }
    ...
    int ObPacketLightyQueue::init(const uint64_t len, void* buf)
    {
      ...
      if (0 == len || !is2n(len))
      {
        ...
        YYSYS_LOG(ERROR, "init(len=%ld):invalid argument", len);
      }
      else if (NULL != items_)
      {
        err = OB_INIT_TWICE;
      ...
      else if (NULL == (items = (Item*)(buf ? buf : (allocated_ = (ob_tc_malloc(get_item_size() * len, ObModIds::OB_LIGHTY_QUEUE))))))
      {
        ...
        YYSYS_LOG(ERROR, "init(len=%ld):allocate memory failed.", len);
      }
      else
      {
        pos_mask_ = len - 1;
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(ERROR, "init fifo allocator failed, err=%d", err);
        }
        else
        {
      ...
      {
        YYSYS_LOG(ERROR, "init ObPacketLightyQueue failed, err=%d", err);
      }
      return err;
    }
    ...
    int ObPacketLightyQueue::push(void* data, const timespec* timeout, bool block, bool deep_copy)
    {
      ...
      while(true)
      {
        ...
        if (OB_UNLIKELY(deep_copy == true))
        {
          ...
          {
            err = OB_ALLOCATE_MEMORY_FAILED;
            YYSYS_LOG(ERROR, "allocate memory failed, ret=%d", err);
            break;
          }
          else
          {
            ...
            {
              fifo_allocator_.free(ptr);
              YYSYS_LOG(WARN, "queue overflow err=%d", err);
              break;
            }
          }
        }
