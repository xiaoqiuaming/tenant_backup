...
int oceanbase::common::ObSPopSPushQueue::init(const int64_t queue_size, const int32_t mod_id)
{
  ...
  if (queue_size <= 0)
  {
    YYSYS_LOG(WARN,"arg error [queue_size:%ld]", queue_size);
  ...
  else
  {
    if (!inited_ || queue_size != queue_size_)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to allocate memory for queue array [errno:%d]", errno);
        err = OB_ALLOCATE_MEMORY_FAILED;
      }
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"[err:%d,queue_size:%ld]", err, queue_size);
      }
      else
      {
...
int oceanbase::common::ObSPopSPushQueue::push(void *msg)
{
  ...
  {
    YYSYS_LOG(WARN,"queue not initialized yet [queue_arr_:%p]",  queue_arr_);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && (push_count_ - pop_count_ >= static_cast<uint64_t>(queue_size_)))
  {
    YYSYS_LOG(WARN,"queue is full [push_count_:%lu,pop_count_:%lu,queue_size_:%ld]", push_count_, pop_count_, queue_size_);
...
int oceanbase::common::ObSPopSPushQueue::pop(const int64_t timeout_us, void *&msg)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to get clock time [errno:%d]", errno);
    err = OB_ERR_SYS;
  }
  if ((OB_SUCCESS == err) && (pop_count_ == push_count_) && (timeout_us > 0))
  {
    ...
    while ((pop_count_ == push_count_) && (ETIMEDOUT != (err = pthread_cond_timedwait(&cond_,&mutex_,&expired_time))))
    {
      ...
      {
        YYSYS_LOG(ERROR, "check cond timedwait failed:ret[%d], msg[%m]", err);
      }
      waiter_exist_ = true;
    }
    pthread_mutex_unlock(&mutex_);
  ...
  {
    msg = NULL;
    YYSYS_LOG(WARN,"queue is empty");
    err = OB_ARRAY_OUT_OF_RANGE;
  }
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(TRACE,"[pop err:%d]", err);
  }
  return err;
}
