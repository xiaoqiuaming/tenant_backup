...
int ObServerExt::init(char *hname, common::ObServer server)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, hname=%s", hname);
  ...
  else
  {
    ...
    else
    {
      ...
      *(reinterpret_cast<int64_t*> (buf + pos)) = str_len;
      ...
      if(OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "ObServer rs_server serialize fail");
      }
    }
  }
  return err;
...
int ObServerExt::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
{
  ...
  else
  {
    ...
    if(magic_num_ != magic_num)
    {
      ...
      YYSYS_LOG(WARN, "wrong magic num, can't deserilize the buffer to ObServerExt, err=%d", err);
    }
    else
    {
      pos += sizeof(int64_t);
      ...
      if(OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "ObServer rs_server deserialize fail.");
      }
    }
  }
  return err;
