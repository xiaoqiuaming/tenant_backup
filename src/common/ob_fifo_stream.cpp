...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObFIFOStream::init(const char *fifo_path, const int64_t max_buffer_size, const int64_t expired_time)
    {
      ...
      {
        YYSYS_LOG(WARN, "open fifo file fail fifo_path=%s errno=%u", fifo_path, errno);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = allocator_.init(max_buffer_size, max_buffer_size, MEMPOOL_PAGE_SIZE)))
      {
        YYSYS_LOG(WARN, "mempool create fail");
      }
      else if (NULL == (write_buffer_ = (char*)alloc_(WRITE_BUFFER_SIZE)))
      {
        YYSYS_LOG(WARN, "alloc write_buffer fail");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS != (ret = wakeup_.init()))
      {
        YYSYS_LOG(WARN, "wakeup init fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = queue_.init(max_buffer_size / AVG_PACKET_SIZE + 1)))
      {
        YYSYS_LOG(WARN, "queue init fail ret=%d max_num=%ld", ret, max_buffer_size / AVG_PACKET_SIZE + 1);
      }
      else
      {
        ...
        start();
        inited_ = true;
        YYSYS_LOG(INFO, "init succ fifo_path=%s max_buffer_size=%ld fifo_fd=%d", fifo_path, max_buffer_size, fifo_fd_);
      }
      if (OB_SUCCESS != ret)
      {
        queue_.destroy();
    ...
    void ObFIFOStream::destroy()
    {
      if (inited_)
      {
        ...
        stop();
        wait();
        YYSYS_LOG(INFO, "alloc_counter_=%ld free_counter_=%ld queue_total=%ld queue_free=%ld mempool_total=%ld",
    ...
    int ObFIFOStream::push(const ObPacket *packet)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid packet buffer capacity=%ld data_length=%d header_size=%ld",
      ...
      else
      {
        ...
        while (true)
        {
          ...
          else
          {
            ...
            pkt->timestamp = yysys::CTimeUtil::getTime();
            ...
            if (0 < buffer_size)
            {
              ...
              YYSYS_LOG(DEBUG, "packet buffer capacity=%ld data_length=%d header_size=%ld",
                        data_buffer ? data_buffer->get_capacity() : -1,
                        packet->get_data_length(), packet->get_header_size());
            }
            while (true)
    ...
    void ObFIFOStream::run(yysys::CThread *thread, void *arg)
    {
      UNUSED(thread);
      UNUSED(arg);
      FIFOPacket *pkt = NULL;
      int64_t buffer_pos = 0;
      SET_THD_NAME_ONCE("fifostream");
      while (!_stop)
      {
        ...
        else if ((yysys::CTimeUtil::getTime() - pkt->timestamp) > expired_time_)
        {
          // skip
        }
    ...
    int ObFIFOStream::mkfifo_(const char *fifo_path)
    {
      ...
      {
        YYSYS_LOG(WARN, "mkfifo fail fifo_path=%s errno=%u", fifo_path, errno);
      }
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "mkfifo succ fifo_fd=%d", fifo_fd);
      }
      return fifo_fd;
    }
  }
