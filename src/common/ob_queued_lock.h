  ...
  namespace common
  {
    ...
    class ObQueuedLock
    {
      public:
        ...
        {
          public:
            Guard(ObQueuedLock& lock): lock_(lock)
            { lock_.lock(ticket_); }
            ~Guard()
            { lock_.unlock(ticket_); }
          private:
            ObQueuedLock& lock_;
            uint64_t ticket_;
        };
      public:
        ObQueuedLock(): ticket_(0)
        {
          ...
          for(int64_t i = 0; OB_SUCCESS == err && i < OB_MAX_THREAD_NUM; i++)
          {
            ...
            {
              err = OB_ERR_SYS;
              YYSYS_LOG(ERROR, "pthread_mutex_init()=>%s", strerror(syserr));
            }
            else if (0 != (syserr = pthread_mutex_lock(mutex_ + i)))
            {
              err = OB_ERR_SYS;
              YYSYS_LOG(ERROR, "pthread_mutex_lock()=>%s", strerror(syserr));
            }
          }
          if (OB_SUCCESS != err)
          {}
          ...
          {
            err = OB_ERR_SYS;
            YYSYS_LOG(ERROR, "pthread_mutex_unlock()=>%s", strerror(syserr));
          }
      ...
      protected:
        pthread_mutex_t* get_mutex(uint64_t i)
        { return mutex_ + (i % OB_MAX_THREAD_NUM); }
      private:
        uint64_t ticket_;
