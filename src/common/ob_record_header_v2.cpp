...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObRecordHeaderV2::check_check_checksum(const char* buf, const int64_t len) const
    {
      ...
      {
        YYSYS_LOG(WARN, "chechk chekcksum error:data_zlength_=%ld,"
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "crc_check_sum != data_checksum_:"
                                       ...
                                       const char*& payload_ptr, int64_t& payload_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument:ptr=%p,size=%ld,record_header_len=%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (!check)
        {
          YYSYS_LOG(WARN, "check error:payload_size=%ld,magic=%d,"
