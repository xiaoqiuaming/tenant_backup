  ...
  namespace common
  {
    ...
    ObTCFactory<T, MAX_CLASS_NUM, MODID, MEM_LIMIT, MAX_FREE_LIST_LENGTH>* ObTCFactory<T, MAX_CLASS_NUM, MODID, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::get_instance()
    {
      ...
      if (OB_UNLIKELY(NULL == instance))
      {
        ...
        if (NULL == instance)
        {
          YYSYS_LOG(ERROR, "failed to init tc_factory");
          abort();
        }
        else
        {
    ...
    T* ObTCFactory<T, MAX_CLASS_NUM, MODID, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::get(int32_t type_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid class type id, id=%d", type_id);
      }
      else
      {
    ...
    void ObTCFactory<T, MAX_CLASS_NUM, MODID, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::put(T* ptr)
    {
      if (OB_LIKELY(NULL != ptr))
      {
        ...
        {
          //YYSYS_LOG(ERROR, "list too long, length=%d max_length=%d", list.get_length(), list.get_max_length());
          list_too_long(list, type_id, obj_size);
        }
        if (mem_size_ > MEM_LIMIT)
        {
          //YYSYS_LOG(ERROR, "cache too large, mem_size_=%ld", mem_size_);
          garbage_collect();
        }
        ++put_count_[type_id];
      }
    ...
    void ObTCFactory<T, MAX_CLASS_NUM, MODID, MEM_LIMIT, MAX_FREE_LIST_LENGTH>::stat()
    {
      ...
      {
        YYSYS_LOG(INFO, "[TCFACTORY_STAT] thread=%d type_id=ALL mem_size=%ld", i, tc_factory->mem_size_);
        ...
        for (int32_t type_id = 0; type_id < MAX_CLASS_NUM; ++type_id)
        {
          ...
          YYSYS_LOG(INFO, "[TCFACTORY_STAT] thread=%d type_id=%d length=%d "
      ...
      for (int32_t type_id = 0; type_id < MAX_CLASS_NUM; ++type_id)
      {
        YYSYS_LOG(INFO, "[TCFACTORY_STAT] thread=ALL type_id=%d length=%ld max_length=%ld allocated=%ld",
                  type_id, class_total_length[type_id],
                  class_total_max_length[type_id], allocated_count[type_id]);
      }
      YYSYS_LOG(INFO, "[TCFACTORY_STAT] thread=ALL type_id=ALL mem_size=%ld length=%ld max_length=%ld",
