...
namespace oceanbase
{
  namespace common
  {
    ...
    bool DLink::unlink()
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "prev_=%p and next_=%p must not null", prev_, next_);
      }
      else
      {
        prev_->next_ = next_;
    ...
    bool DLink::add(DLink *prev, DLink *e, DLink *next)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "prev=%p, e=%p and next=%p must not null", prev, e, next);
      }
      else
      {
        prev->next_ = e;
  ...
  //------------dlist define--------------
    ...
    bool DList::add_last(DLink *e)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "failed to add e before head, e=%p", e);
      }
      else
      {
        size_++;
    ...
    bool DList::add_first(DLink *e)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "failed to add e after head, e=%p", e);
      }
      else
      {
        size_++;
    ...
    bool DList::move_to_first(DLink *e)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "failed to move to first, e=%p", e);
      }
      else
      {
        size_--;
    ...
    bool DList::move_to_last(DLink *e)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "failed to move to last, e=%p", e);
      }
      else
      {
        size_--;
...
//        ret = false;
        ret = NULL;
        // modify for build warning-b
        YYSYS_LOG(ERROR, "failed to remove e=%p", e);
      }
      else
      {
        size_--;
