...
#define ATOMIC_CAS(val, cmpv, newv) __sync_val_compare_and_swap((val), (cmpv), (newv))
    namespace MultiWakeupComponent
    {
      bool fd_equal(const int a, const int b)
      {
            ...
            && 0 == fstat(b, &st_b))
        {
          ...
          if (!bret)
          {
            YYSYS_LOG(INFO, "fd not equal a=%d b=%d a.ino=%ld b.ino=%ld", a, b, st_a.st_ino, st_b.st_ino);
          }
        }
        return bret;
      }
      ...
      int FdPair::init(const int epoll_fd, const int64_t timestamp)
      {
        ...
        if (-1 == epoll_fd)
        {
          ...
          //else if (0 != pipe(fd_))
        {
          YYSYS_LOG(WARN, "create pipe pair fail errno=%u", errno);
          ret = OB_ERROR;
        }
        else if (-1 == (epoll_fd_ = dup(epoll_fd)))
        {
          YYSYS_LOG(WARN, "dup epoll_fd fail errno=%u", errno);
          ret = OB_ERROR;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "epoll_ctl add fail epoll_fd=%d read_fd=%d read_event=%p",
                      epoll_fd, fd_[READ_FD_POS], &read_event_);
            ret = OB_ERROR;
          }
          else
          {
            YYSYS_LOG(INFO, "fd_pair init succ epoll_fd=%d dup=%d read_fd=%d write_fd=%d",
      ...
      ThreadLocalInfo::ThreadLocalInfo()
      {
        ...
        {
          YYSYS_LOG(ERROR, "fd_set create fail");
        }
      }
      ...
      int ThreadLocalInfo::get_write_fd(const int epoll_fd, const int64_t timestamp)
      {
            ...
            && NULL != fd_pair)
        {
          ...
          {
            YYSYS_LOG(INFO, "epoll_fd=%d reused, pair_timestamp=%ld arg_timestamp=%ld, will rebuild fd_pair",
        ...
        if (-1 == write_fd)
        {
          ...
          if (NULL == fd_pair)
          {
            YYSYS_LOG(ERROR, "alloc fd_pair fail epoll_fd=%d", epoll_fd);
          }
          else if (OB_SUCCESS != fd_pair->init(epoll_fd, timestamp))
          {
            YYSYS_LOG(ERROR, "fd_pair init fail epoll_fd=%d", epoll_fd);
          }
          else
          {
                ...
                || hash::HASH_OVERWRITE_SUCC == tmp_ret)
            {
              ...
              {
                YYSYS_LOG(WARN, "overwrite fd_pair succ epoll_fd=%d", epoll_fd);
              }
              write_fd = fd_pair->get_write_fd();
            }
            else
    ...
    {
      timestamp_ = yysys::CTimeUtil::getMonotonicTime();
    }
    ...
    int ObMultiWakeup::init()
    {
      ...
      {
        YYSYS_LOG(WARN, "epoll_create errno=%u", errno);
        ret = OB_ERROR;
      }
      else
      {
        YYSYS_LOG(INFO, "epoll_create fd=%d", epoll_fd_);
      }
      return ret;
    }
    ...
    int ObMultiWakeup::signal()
    {
      ...
      else
      {
        ...
        //YYSYS_LOG(DEBUG, "write ret=%d errno=%u", write_ret, errno);
      }
      return ret;
    }
    ...
    int ObMultiWakeup::timedwait(const int64_t timeout_us)
    {
      ...
      else
      {
        ...
        while (true)
        {
          epoll_timeout = timeout_us - (yysys::CTimeUtil::getTime() - start_time);
          if (0 > epoll_timeout)
          {
          ...
          if (0 < event_num)
          {
            for (int i = 0; i < event_num; i++)
            {
              ...
              //YYSYS_LOG(DEBUG, "read ret=%d errno=%u", read_ret, errno);
            }
            break;
          }
        }
