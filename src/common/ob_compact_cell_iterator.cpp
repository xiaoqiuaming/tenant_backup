...
int ObCompactCellIterator::init(const char *buf, enum ObCompactStoreType store_type /* = SPARSE */)
{
  ...
  {
    YYSYS_LOG(WARN, "buf read assign fail:ret[%d]", ret);
  }
  else
  {
    row_start_ = 0;
...
int ObCompactCellIterator::init(const ObString &buf, enum ObCompactStoreType store_type /* = SPARSE */)
{
  ...
  {
    YYSYS_LOG(WARN, "buf read assign:ret[%d]", ret);
  }
  else
  {
    store_type_ = store_type;
    ...
    switch(store_type_)
    {
      ...
      case DENSE_DENSE:
        if (0 == step_)
        {
          ...
          {
            YYSYS_LOG(WARN, "parse cell fail:ret[%d]", ret);
          }
...
int ObCompactCellIterator::get_cell(const ObObj *&value, bool *is_row_finished /* = NULL */, ObString *row /* = NULL */)
{
  ...
  {
    YYSYS_LOG(WARN, "get cell fail:ret[%d]", ret);
  }
  else
  {
    if(OB_INVALID_ID != column_id)
    {
      ...
      YYSYS_LOG(WARN, "column_id should be OB_INVALID_ID. this function only is used in dense format!");
    }
  }
  return ret;
}
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    switch(cell_meta->type_)
    {
      ...
      case ObCellMeta::TP_DECIMAL:
        ...
        {
          YYSYS_LOG(WARN, "parse number fail:ret[%d]", ret);
        }
        else if(OB_SUCCESS != (ret = value.set_add(is_add)))
        {
          YYSYS_LOG(WARN, "set add fail:ret[%d]", ret);
        }
        break;
      ...
      case ObCellMeta::TP_ESCAPE:
        switch(cell_meta->attr_)
        {
          ...
          default:
            ret = OB_NOT_SUPPORTED;
            YYSYS_LOG(WARN, "unsupported escape %d", cell_meta->attr_);
        }
        break;
      case ObCellMeta::TP_EXTEND:
        switch(cell_meta->attr_)
        {
          ...
          default:
            ret = OB_NOT_SUPPORTED;
            YYSYS_LOG(WARN, "unsupported extend:cell_meta->attr_=%d",
                      cell_meta->attr_);
        }
        break;
        //add peiouya [DATE_TIME] 20150906:b
      ...
      default:
        ret = OB_NOT_SUPPORTED;
        YYSYS_LOG(WARN, "unsupported type %d", cell_meta->type_);
    }
  }
...
int ObCompactCellIterator::parse_decimal(ObBufferReader &buf_reader, ObObj &value) const
{
  ...
  {
    YYSYS_LOG(WARN, "read vscale fail:ret[%d]", ret);
  }
  else if(OB_SUCCESS != (ret = buf_reader.get<ObDecimalMeta>(dec_meta)))
  {
    YYSYS_LOG(WARN, "get decimal meta fail:ret[%d]", ret);
  }
  else
  {
  ...
  if(OB_SUCCESS == ret)
  {
    for(int8_t i=0;OB_SUCCESS == ret && i<nwords;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "get word fail:ret[%d]", ret);
      }
      else
      {
        words[i] = *word;
