  ...
  namespace common
  {
    ...
    int FileDirectoryUtils::vsystem(const char* cmd)
    {
      ...
      if (0 == pid)
      {
        ...
        if (-1 == err)
        {
          YYSYS_LOG(ERROR, "execlp error: %s", strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "program should not execute behind the \'execl\'");
        }
      }
      else if (-1 == pid)
      {
        YYSYS_LOG(ERROR, "vfork error: %s", strerror(errno));
        ret = OB_ERROR;
      }
      else
      {
        ...
        if (-1 == err)
        {
          YYSYS_LOG(ERROR, "waitpid error: %s", strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
    ...
    int FileDirectoryUtils::cp(const char* src_path, const char* src_name, const char* dst_path, const char* dst_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "Arguments are invalid, src_name=%s src_path=%s dst_naem=%s dst_path=%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == cmd)
        {
          YYSYS_LOG(WARN, "ob_malloc error, cp_cmd_len=%d", cp_cmd_len);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (err < 0)
        {
          YYSYS_LOG(ERROR, "snprintf cmd[cp_cmd_len=%d err=%d] error[%s]", cp_cmd_len, err, strerror(errno));
          ret = OB_ERROR;
        }
        else if (err >= cp_cmd_len)
        {
          YYSYS_LOG(ERROR, "cmd buffer is not enough[cp_cmd_len=%d err=%d]", cp_cmd_len, err);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(ERROR, "cp[\"%s\"] error[err=%d]", cmd, err);
          ret = OB_ERROR;
        }
      }
        ...
        const char* dst_path, const char* dst_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "Arguments are invalid, src_name=%s dst_name=%s src_path=%s dst_path=%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == tmp_name)
        {
          YYSYS_LOG(ERROR, "ob_malloc error, tmp_name_len=%ld", tmp_name_len);
          ret = OB_ERROR;
        }
        else
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (-1 == fd)
        {
          YYSYS_LOG(ERROR, "mkstemp error: %s", strerror(errno));
          ret = OB_ERROR;
        }
        else
        {
          ...
          if (-1 == err)
          {
            YYSYS_LOG(ERROR, "close file error: %s", strerror(errno));
            ret = OB_ERROR;
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "cp error, ret=%d", ret);
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "mv error, ret=%d", ret);
    ...
    int FileDirectoryUtils::mv(const char* src_path, const char* src_name, const char* dst_path, const char* dst_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "Arguments are invalid, src_path=%s dst_path=%s src_name=%s dst_name=%s",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == cmd)
        {
          YYSYS_LOG(WARN, "ob_malloc error, mv_cmd_len=%d", mv_cmd_len);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (err < 0)
        {
          YYSYS_LOG(ERROR, "snprintf cmd[mv_cmd_len=%d err=%d] error[%s]", mv_cmd_len, err, strerror(errno));
          ret = OB_ERROR;
        }
        else if (err >= mv_cmd_len)
        {
          YYSYS_LOG(ERROR, "cmd buffer is not enough[mv_cmd_len=%d err=%d]", mv_cmd_len, err);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(ERROR, "mv[\"%s\"] error[err=%d]", cmd, err);
          ret = OB_ERROR;
        }
      }
    ...
    int FileDirectoryUtils::rm(const char* path, const char* name)
    {
      ...
      {
        YYSYS_LOG(WARN, "Arguments are invalid, path=%s name=%s", path, name);
        ret = OB_INVALID_ARGUMENT;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == full_name)
        {
          YYSYS_LOG(WARN, "ob_malloc error, full_name_len=%d", full_name_len);
          ret = OB_ERROR;
        }
        else
        {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "unlink error, err=%d(%s)", err, strerror(errno));
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "rm_ ret=%d name=%s path=%s", ret, name, path);
      }

      return ret;
    }

    int64_t FileDirectoryUtils::get_hard_link_num(const char *filename)
    {
        if(filename == NULL)
        {
            return 0;
