...
namespace
{
  /// @brief  ������øû���������ֱ�ӵ���ϵͳmalloc��free�����ͷ��ڴ棬
  ///         Ŀ���Ǳ����ڴ�bug������Ϊ�����ڴ�ػ�����ڴ棬valgrindû�а취���
                           ...
                           const int64_t block_size, const int32_t ref_num)
  {
    ...
    (void)link; //add for [build warning]
...
{
  YYSYS_LOG(INFO, "[MEMORY] total=% '15ld direct_size=% '15ld direct_block=%'ld",
            mem_size_handled_, direct_allocated_mem_size_, direct_allocated_block_num_);
  if ((NULL == mod_set_)  || (NULL == mem_size_each_mod_))
  {
    YYSYS_LOG(INFO, "total used memory size:%'ld", mem_size_default_mod_.value());
    if (print_to_std)
    {
  ...
  else
  {
    for (int32_t mod_idx = 0; mod_idx < mod_set_->get_max_mod_num(); mod_idx ++)
    {
      if (mod_set_->get_mod_name(mod_idx) != NULL)
      {
        ...
        if (cv != 0)
        {
          YYSYS_LOG(INFO, "[MEMORY] size=% '15ld mod=%s",
  ...
  YYSYS_LOG(INFO, "mmap_chunks=%d", info.hblks);
  YYSYS_LOG(INFO, "mmap_bytes=%d", info.hblkhd);
  YYSYS_LOG(INFO, "sbrk_sys_bytes=%d", info.arena);
  YYSYS_LOG(INFO, "sbrk_used_chunk_bytes=%d", info.uordblks);
  YYSYS_LOG(INFO, "sbrk_not_in_use_chunks=%d", info.ordblks);
  YYSYS_LOG(INFO, "sbrk_not_in_use_chunk_bytes=%d", info.fordblks);
  YYSYS_LOG(INFO, "sbrk_top_most_releasable_chunk_bytes=%d", info.keepcost);
}
...
int oceanbase::common::ObBaseMemPool::init(const ObMemPoolModSet * mod_set)
{
  ...
  {
    YYSYS_LOG(ERROR, "ObBaseMemPool::init error, err=%d mod_set=%p",
              err, mod_set);
  }
  return err;
}
...
{
  yysys::CThreadGuard guard(&pool_mutex_);
...
void *oceanbase::common::ObBaseMemPool::malloc(const int64_t nbyte, int32_t mod_id, int64_t *got_size)
{
  ...
  if (mem_size_handled_ > mem_size_limit_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "memory over limited handled=%ld limit=%ld",
...
{
  //YYSYS_LOG(INFO, "ob_free ptr=%p", ptr);
  free_(ptr);
}
                                            ...
                                            const ObMemPoolModSet *mod_set/* = NULL*/)
{
  ...
  {
    YYSYS_LOG(WARN, "param invalid [fixed_item_size:%ld, item_num_each_block:%ld]",
  ...
  {
    YYSYS_LOG(WARN, "double initializing memory pool");
    err = EINVAL;
  }
  if ( 0 == err )
  {
...
int64_t oceanbase::common::ObFixedMemPool::get_block_num() const
{
  ...
  if (mem_block_size_ <= 0)
  {
    YYSYS_LOG(WARN,"memory pool not initialized");
  }
  else
  {
...
{
  YYSYS_LOG(INFO, "[MEMORY] mem_block_size=%ld item_size=%ld used_block=%ld free_block=%ld free_limit=%ld",
                                                                ...
                                                                bool check_unfreed_mem)
{
  ...
  if (info->ref_num_ != 0 )
  {
    ...
    {
      /*
      YYSYS_LOG(ERROR, "memory not released [begin_addr:%p,"
...
{
  /// @todo (wushi wushi.ly@taobao.com) �����κ��ڴ���䶼��ֱ�Ӹ�һ�����飬
  /// ���Ƕ���������hash�����ֶ����������С���ڴ���䣬
  /// �����������������Ż�������̫�˷��ڴ��ˣ���2010.08.31֮ǰ���
  char *result = NULL;
  int result_errno = 0;
  ...
  if (nbyte <= 0)
  {
    YYSYS_LOG(WARN, "cann't allocate memory less than 0 byte [nbyte:%ld]", nbyte);
    result_errno = EINVAL;
  }
//add support for arm platform by wangd 202106:b
#if defined(__aarch64__)
  int64_t nbyte_align = (nbyte + 7) & ~(7);
#else
  int64_t nbyte_align = nbyte;
#endif
//add support for arm platform by wangd 202106:e
  int64_t block_size = nbyte_align+sizeof(MemBlockInfo) + sizeof(MemPoolItemInfo);//mod support for arm platform by wangd 202106
  if (malloc_directly())
  {
    result = new(std::nothrow) char[nbyte_align];//mod support for arm platform by wangd 202106
    if (NULL == result)
    {
      result_errno = errno;
    }
  ...
  if (NULL == result && result_errno == 0
      && (nbyte_align > mem_fixed_item_size_ || ObModIds::OB_TSI_FACTORY == mod_id))//mod support for arm platform by wangd 202106
  {
    ...
    {
#ifdef __OB_MTRACE__
      YYSYS_LOG(WARN, "ALLOC BIG, ptr=%p nbyte_align=%ld mod_id=%d", result, nbyte_align, mod_id);//mod support for arm platform by wangd 202106
#endif
      yysys::CThreadGuard guard(&pool_mutex_);
    ...
    {
      YYSYS_LOG(ERROR, "try to allocate variable sized memory from fixed memory pool "
                "[nbyte_align:%ld,mem_fixed_item_size_:%ld]", nbyte_align, mem_fixed_item_size_);//mod support for arm platform by wangd 202106
    }
  }
...
void  oceanbase::common::ObFixedMemPool::free_(const void *ptr)
{
  ...
  }
  {
    ...
    if (0 == result_errno && !freed)
    {
      ...
      {
#ifdef __OB_MTRACE__
        YYSYS_LOG(WARN, "FREE BIG, ptr=%p size=%ld", ptr, block_info->block_size_);
#endif
...
void  oceanbase::common::ObVarMemPool::free_(const void *ptr)
{
  ...
  if (!freed)
  {
    ...
    if (!check_mem_pool_item_info(*item_info))
    {
      YYSYS_LOG(ERROR, "memory corrupt [addr:%p,"
                "MemPoolItemInfo::magic_:%u,"
                "MemPoolItemInfo::mother_block_:%p,"
                "MemBlockInfo::magic_:%u,"
                "correct magic number:%u]",ptr, item_info->magic_,
...
//add support for arm platform by wangd 202106:e
...

  {
    ...
    if (result_errno == 0 && result == NULL && block_info != NULL)
    {
      if (block_info->block_size_ - cur_block_used_nbyte_ >=
          static_cast<int64_t>(sizeof(MemPoolItemInfo)) + nbyte_align)//mod support for arm platform by wangd 202106
      {
        item_info = reinterpret_cast<MemPoolItemInfo*>(reinterpret_cast<char*>(block_info)
                                                       + cur_block_used_nbyte_);
        init_mem_pool_item_info(*item_info,block_info);
        cur_block_used_nbyte_ += nbyte_align + sizeof(MemPoolItemInfo);//mod support for arm platform by wangd 202106
        result  = item_info->buf_;
        block_info->ref_num_ ++;
      }
      else
  ...
  if (result_errno == 0 && result == NULL && block_info == NULL)
  {
    ...
    if (!free_mem_block_list_.is_empty())
    {
      ...
      cur_block_used_nbyte_ = sizeof(MemBlockInfo) + sizeof(MemPoolItemInfo) + nbyte_align;//mod support for arm platform by wangd 202106
      result = item_info->buf_;
    }
  }
