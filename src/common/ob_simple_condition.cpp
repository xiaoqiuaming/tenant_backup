...
int ObSimpleCond::set(const int64_t index, const ObLogicOperator oper, const ObObj & value)
{
  ...
  if (LIKE == logic_operator_)
  {
    ...
    {
      logic_operator_ = NIL;
      YYSYS_LOG(ERROR, "calculate sign failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
bool ObSimpleCond::check(const ObObjType & type) const
{
  ...
  {
    YYSYS_LOG(WARN, "%s", "check logic operator is nil");
    ret = false;
  }
  else
  {
    ...
    if ((operand_type != ObNullType) && (operand_type != type))
    {
      ...
      {
        ret = false;
        YYSYS_LOG(WARN, "check column data type not the same as oprand type:"
    ...
    {
      ret = false;
      YYSYS_LOG(WARN, "check operand type failed:operand_type[null], operator[%u]",
          logic_operator_);
    }
  }
  return ret;
...
int ObSimpleCond::calc_sign(const ObObj & operand, uint64_t & sign)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "get varchar pattern or check length failed:len[%d], ret[%d]",
        pattern.length(), ret);
  }
  else
  {
...
bool ObSimpleCond::calc(const ObObj & left_operand) const
{
  ...
  switch (logic_operator_)
  {
    ...
    default:
    {
      YYSYS_LOG(ERROR, "check condition logic type failed:type[%d]", logic_operator_);
    }
  }
  return ret;
}
...
int64_t ObSimpleCond::find(const ObObj & pattern, const uint64_t pattern_sign, const ObObj & src)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "get varchar type failed from pattern:[%d]", ret);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get varchar type failed from src:[%d]", ret);
    }
    else
    {
...
DEFINE_SERIALIZE(ObSimpleCond)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "serialize column_index or column name failed:index[%ld], name[%.*s], ret[%d]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "serialize operator failed:operator[%d], ret[%d]",
          logic_operator_, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "serialize operand failed:ret[%d]", ret);
    }
  }
...
DEFINE_DESERIALIZE(ObSimpleCond)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "deserialize column_index failed:ret[%d]", ret);
  }
  else if (ObIntType == obj.get_type())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get column index int value failed:ret[%d]", ret);
    }
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get column name value failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize operator failed:ret[%d]", ret);
    }
    else
    {
      int64_t value = 0;
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "get operator int value failed:ret[%d]", ret);
      }
      else
      {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize operand failed:ret[%d]", ret);
    }
