...
void get_package_and_git(char* server_version, int64_t buf_len)
{
const char* server_version_template = "%s";  
...
//      git_version(), build_date(), build_time());
	snprintf(server_version, buf_len, server_version_template,
        git_version());
}
