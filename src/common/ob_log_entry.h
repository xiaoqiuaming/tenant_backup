...
namespace oceanbase
{
  namespace common
  {
    ...
    enum LogCommand
    {
      ...
      OB_UPS_MUTATOR_PREPARE = 211,//for participant prepare
      OB_PART_TRANS_COMMIT = 222,//for participant commit
      OB_PART_TRANS_ROLLBACK = 205,//for participant rollback
      //add 20150701:e
    ...
    struct ObLogEntry
    {
       ...
       * @brief 设置日志序号
       */
      void set_log_seq(const uint64_t seq)
      { seq_ = seq; }

      /**
       * @brief 设置LogCommand
       */
      void set_log_command(const int32_t cmd)
      { cmd_ = cmd; }
      ...
      { return static_cast<int>(sizeof(ObRecordHeader) + sizeof(uint64_t) + sizeof(LogCommand)); } //modify for [build warning]
