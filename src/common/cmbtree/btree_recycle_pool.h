    ...
    namespace cmbtree
    {
      ...
      class BtreeRecyclePool
      {
        ...
        protected:
          ...
          TRecycleNode * volatile cur_recycle_node_;/// ��ǰ��RecycleNode
          TRecycleNode *      oldest_recycle_node_; /// ���ϵ�RecycleNode
          QLock               mutex_;               /// ���ղ���������
          TMemPool &          node_alloc_;          /// Btree�ڴ������
          TMemPool &          recycle_alloc_;       /// ���ճص��ڴ������
          bool                init_;
      };
      ...
      void BtreeRecyclePool<K, V, Alloc>::destroy()
      {
        if (init_)
        {
          YYSYS_LOG(DEBUG, "remained_recycle_node = %ld", get_recycle_node_num());
          do_free_(-1);
          init_ = false;
        }
      }
      ...
      int32_t BtreeRecyclePool<K, V, Alloc>::free_recycle_node()
      {
            ...
            && btree_node_num_ > DEFAULT_LOCK_BTREE_NODE_NUM))
        {
          err = mutex_.exclusive_lock(BtreeTID::gettid(),
              yysys::CTimeUtil::getTime() + DEFAULT_LOCK_TIME);
