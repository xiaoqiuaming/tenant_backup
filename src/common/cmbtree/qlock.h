...
#define CAS(x, old_v, new_v) __sync_bool_compare_and_swap(x, old_v, new_v)
      struct QLock
      {
        ...
        ~QLock()
        {}
        volatile uint32_t n_ref_ __attribute__((__aligned__(8)));
        volatile uint32_t uid_ __attribute__((__aligned__(8)));

        static inline bool is_timeout(const int64_t end_time)
        {
          return end_time > 0 && yysys::CTimeUtil::getTime() > end_time;
        }
        ...
        int try_exclusive_lock(const uint32_t uid)
        {
          ...
          else if (!CAS(&uid_, static_cast<uint32_t>(cur_uid), uid|EXCLUSIVE_BIT))
          {
            err = OB_EAGAIN;
          }
          return err;
        ...
        int share2exclusive_lock(const uint32_t uid, const int64_t end_time = -1)
        {
          ...
          while(!is_timeout(end_time) && OB_EAGAIN == err)
          {
            ...
            {
              err = OB_DEAD_LOCK;
              YYSYS_LOG(DEBUG, "dead_lock detected, uid=%u, n_ref=%u", uid_, n_ref_);
            }
            else if (!CAS(&uid_, static_cast<uint32_t>(cur_uid), uid|EXCLUSIVE_BIT))
            {
              err = OB_EAGAIN;
