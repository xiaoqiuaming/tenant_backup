  ...
  namespace common
  {
                           ...
                           const int64_t start_pos, const int64_t len)
    {
          ...
          || len > ring_buf_len || 0 > start_pos)
      {
        ...
        YYSYS_LOG(ERROR, "copy_from_ring_buf(dest=%p, src=%p[%ld], range=[%ld,+%ld]): invalid argument",
                           ...
                           const int64_t start_pos, const int64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "copy_to_ring_buf(dest=%p, src=%p[%ld], range=[%ld,+%ld]): invalid argument",
                  dest, src, ring_buf_len, start_pos, len);
      }
      else
      {
    ...
    int popen(const char* cmd, int flag)
    {
      ...
      if (NULL == cmd)
      {
        YYSYS_LOG(ERROR, "INVALID ARGUMENT: cmd == NULL");
      }
      else if (0 != pipe(pipefd))
      {
        YYSYS_LOG(ERROR, "pipe()=>%s", strerror(errno));
      }
      else if (0 > (pid = fork()))
      {
        YYSYS_LOG(ERROR, "fork()=>%s", strerror(errno));
      }
      else if (0 == pid)
      {
        ...
        {
          YYSYS_LOG(ERROR, "close(%d)=>%s", pipefd[fd_idx^1], strerror(errno));
        }
        else if (0 > dup2(pipefd[fd_idx], fd_idx))
        {
          YYSYS_LOG(ERROR, "dup2(%d)=>%s", pipefd[fd_idx], strerror(errno));
        }
        else if (0 != execlp("sh", "sh", "-c", cmd, (char*)NULL))
        {
          YYSYS_LOG(ERROR, "exec(cmd=%s)=>%s", cmd, strerror(errno));
        }
      }
      else if (0 < pid)
      {
    ...
    int64_t write_all(int fd, char* buf, int64_t len)
    {
      ...
      for(; OB_SUCCESS == err && total_write_size < len; total_write_size += write_size)
      {
        ...
        {
          err = OB_IO_ERROR;
          YYSYS_LOG(ERROR, "write(fd=%d, size=%ld)=>%s", fd, len - total_write_size, strerror(errno));
        }
      }
    ...
    int64_t read_all(int fd, char* buf, int64_t len)
    {
      ...
      for(; OB_SUCCESS == err && total_read_size < len; total_read_size += read_size)
      {
        ...
        {
          err = OB_IO_ERROR;
          YYSYS_LOG(ERROR, "read(fd=%d, size=%ld)=>%s[%d]", fd, len - total_read_size, strerror(errno), errno);
        }
      }
    ...
    IRLock::IRLock(uint64_t bs_shift, uint64_t bcount): bs_shift_(bs_shift), bcount_(bcount), start_(0), end_(0)
    {
      ...
      {
        YYSYS_LOG(ERROR, "INVALID ARGUMENT: bs_shif=%ld, bcount=%ld", bs_shift, bcount);
      }
      else
      {
    ...
    void IRLock::debug()
    {
      ...
      {
        YYSYS_LOG(ERROR, "irlock[%ld]=%ld", i, refs_[i]);
      }
    }
    ...
    int QBuf::cpytail(uint64_t pos, uint64_t len)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "INVALID_ARGUMENT: pos=%ld, len=%ld, reserved=%ld", pos, len, reserved_size_);
      }
    ...
    int QBuf::append(char* buf, int64_t len)
    {
      ...
      {
        YYSYS_LOG(ERROR, "write(pos=%ld, buf=%p[%ld])=>%d", push_, buf, len, err);
      }
      else
      {
    ...
    int QBuf::pop(uint64_t& pos, char*& buf, int64_t& data_len)
    {
      int err = OB_SUCCESS;
      {
        ...
        {
          YYSYS_LOG(ERROR, "read(pos=%ld, buf=%p[%ld])=>%d", pos, &data_len, sizeof(data_len), err);
        }
        else if (data_len <= 0 || (uint64_t)data_len > reserved_size_ || pos + (uint64_t)data_len > push_)
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "INVALID DATA LEN: pos=%ld, data_len=%ld, push_=%ld", pos, data_len, push_);
        }
        else if (OB_SUCCESS != (err = irlock_.use(pos)))
        {
          YYSYS_LOG(ERROR, "irlock_.use(pos=%ld)=>%d", pos, err);
        }
        else
        {
          //YYSYS_LOG(INFO, "data_len=%ld", data_len);
      ...
      {
        YYSYS_LOG(ERROR, "cpy2tail(pos=%ld, data_len=%ld)=>%d", pos, data_len, err);
      }
      else
      {
    ...
    int plain_decode_i64(char* buf, int64_t limit, int64_t& pos, int64_t& value)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "INVALID_ARGUMENT: buf=%p[%ld:%ld]", buf, pos, limit);
      }
      else if (new_pos + (int64_t)sizeof(int64_t) > limit)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "pos[%ld] + 8 > limit[%ld]", new_pos, limit);
      }
      else
      {
    ...
    int serialize_packet(char* buf, int64_t limit, int64_t& pos, ObPacket& pkt)
    {
      ...
      {
        YYSYS_LOG(ERROR, "encode_i64()=>%d", err);
      }
    ...
    int deserialize_packet(char* buf, int64_t limit, int64_t& pos, ObPacket& pkt)
    {
      ...
      {
        YYSYS_LOG(ERROR, "decode_i64()=>%d", err);
      }
      else
      {
      ...
      {
        YYSYS_LOG(ERROR, "decode_i32()=>%d", err);
      }
      ...
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "pos[%ld] + data_len[%ld] > limit[%ld]", new_pos, data_len, limit);
      }
      else if (pkt_buf->get_position() + data_len > pkt_buf->get_capacity())
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "pos[%ld] + data_len[%ld] > limit[%ld]", pkt_buf->get_position(), data_len, pkt_buf->get_capacity());
      }
      else
      {
                                           ...
                                           count_reporter_("pcap", 10000), total_size_(0)
    {
      if (NULL != pcap_cmd && pcap_cmd[0] != '\0')
      {
        ...
        {
          YYSYS_LOG(ERROR, "open(%s)=>%s", pcap_cmd, strerror(errno));
        }
      }
      YYSYS_LOG(INFO, "pcap.init: cmd=[%s], fd=%d", pcap_cmd, fd_);
    }
    ...
    int ObPCap::flush_buf()
    {
      ...
      {
        err = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "write_all(fd=%d, buf=%p[%ld])=>%d", fd_, buf_.get_data(), buf_.get_position(), err);
      }
      else
      {
        total_size_ += buf_.get_position();
        buf_.get_position() = 0;
        YYSYS_LOG(TRACE, "flush_buf(total_size=%ld)", total_size_);
      }
      return err;
    }
    ...
    int ObPCap::handle_packet(ObPacket* packet)
    {
      ...
      {
        YYSYS_LOG(ERROR, "do_capture_packet(pkt=%p)=>%d", packet, err);
      }
      return err;
    }
                                           ...
                                           count_reporter_("pfetch", 10000), total_size_(0)
    {
      if (NULL != pfetch_cmd && pfetch_cmd[0] != '\0')
      {
        ...
        {
          YYSYS_LOG(ERROR, "open(%s)=>%s", pfetch_cmd, strerror(errno));
        }
      }
      YYSYS_LOG(INFO, "pfetcher.init: cmd=[%s], fd=%d", pfetch_cmd, fd_);
    }
        ...
        else
        {
          ...
          while(OB_EAGAIN == err)
          {
            if (OB_SUCCESS != (err = qbuf_.append(cbuf_, read_size))
                && OB_EAGAIN != err)
            {
              YYSYS_LOG(ERROR, "qbuf.append()=>%d", err);
            }
          }
        }
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(TRACE, "read_buf(total_size=%ld)", total_size_ += read_size);
        }
      }
      return err;
    }
    ...
    int ObPFetcher::get_packet(ObPacket* packet)
    {
      ...
      {
        YYSYS_LOG(WARN, "read_buf()=>%d", err);
      }
      else if (OB_SUCCESS != (err = qbuf_.pop(pos, buf, len))
               && OB_EAGAIN != err)
      {
        YYSYS_LOG(ERROR, "qbuf.pop(pos=%ld, buf=%p[%ld])=>%d", pos, buf, len, err);
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "packet->deserialize(buf=%p[%ld])=>%d", buf, len, err);
        }
        else
        {
          count_reporter_.inc();
        ...
        {
          YYSYS_LOG(ERROR, "pop_done(pos=%ld)=>%d", pos, err);
        }
      }
      return err;
    }
    ...
    int ObPFetcher::get_match_packet(ObPacket* packet, int pkt_code)
    {
      ...
      while(OB_SUCCESS == err)
      {
        if (OB_SUCCESS != (err = get_packet(packet))
            && OB_EAGAIN != err)
        {
          YYSYS_LOG(ERROR, "get_packet()=>%d", err);
        }
        else if (OB_EAGAIN == err)
        {}
        ...
        YYSYS_LOG(DEBUG, "read packet: %d, err=%d", packet->get_packet_code(), err);
      }
      return err;
    }
