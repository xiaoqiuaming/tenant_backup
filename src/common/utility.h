...
{ \
  ...
  if (NULL != strings) \
{ \
  YYSYS_LOG(LEVEL, _fmt_ " BackTrace Start: ", ##args); \
  for (int i = 0; i < size; i++) \
{ \
  YYSYS_LOG(LEVEL, "BT[%d] @[%s]", i, strings[i]); \
  } \
  ::free(strings);                        \
  } \
  } \
...
namespace oceanbase
{
  namespace common
  {
    ...
    void hex_dump(const void* data, const int32_t size,
                  const bool char_type = true, const int32_t log_level = YYSYS_LOG_LEVEL_DEBUG);
    ...
    extern void dump_scanner(const common::ObScanner &scanner,
                             const int32_t log_level = YYSYS_LOG_LEVEL_DEBUG, const int32_t type = 1);
    ...
    int deep_copy_ob_obj(Allocator& allocator, const ObObj& src, ObObj& dst)
    {
      ...
      if (ObVarcharType == src.get_type())
      {
        ...
        {
          YYSYS_LOG(WARN, "get_varchar failed, obj=%s", to_cstring(src));
        }
        else if (OB_SUCCESS != (ret = deep_copy_ob_string(allocator, value, new_value)))
        {
          YYSYS_LOG(WARN, "deep_copy_ob_string failed, obj=%s", to_cstring(src));
        }
        else
        {
    ...
    struct CountReporter
    {
        ...
        ~CountReporter()
        {
          YYSYS_LOG(INFO, "%s=%ld", id_, count_);
        }
        void inc()
        {
    ...
    inline void ob_set_err_msg(const char* format, ...)
    {
      ...
      va_end(ap);
      yysys::WarningBuffer *wb = yysys::get_tsi_warning_buffer();
    ...
    extern bool is_numerical_type(const oceanbase::common::ObExprObj &value);
    ...
    class ObTSIArray
    {
      ...
      public:
        ObTSIArray() : thread_num_(0)
        {
          ...
          int tmp_ret = PTHREAD_KEY_MGR->create_thread_key(key_);
          //mod hongchen [TSD_USER_IMPL] 20170916:e
          if (0 != tmp_ret)
          {
            YYSYS_LOG(ERROR, "pthread_key_create fail ret=%d", tmp_ret);
          }
        };
        ~ObTSIArray()
        {
      ...
      public:
        T &get_tsi()
        {
          ...
          volatile ThreadNode* thread_node = (volatile ThreadNode*)(PTHREAD_KEY_MGR->get_thread_specifi(key_));
          if (NULL == thread_node)
          {
            thread_node = &array_[__sync_fetch_and_add(&thread_num_, 1) % N_THREAD];
            //int tmp_ret = pthread_setspecific(key_, (void*)thread_node);
            int tmp_ret = PTHREAD_KEY_MGR->set_thread_specifi(key_, (void*)thread_node);
            if (0 != tmp_ret)
            {
              YYSYS_LOG(ERROR, "pthread_setspecific fail ret=%d", tmp_ret);
            }
          }
          //mod hongchen [TSD_USER_IMPL] 20170916:e
...
#define REACH_TIME_INTERVAL_RANGE(i, j) \
  ({ \
  bool bret = false; \
  static volatile int64_t last_time = yysys::CTimeUtil::getTime(); \
  int64_t cur_time = yysys::CTimeUtil::getTime(); \
  int64_t old_time = last_time; \
