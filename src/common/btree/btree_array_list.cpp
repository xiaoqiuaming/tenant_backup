...
namespace oceanbase
{
  ...
  //modify for [build warning]-b
    const int32_t BtreeArrayList::NODE_SIZE = static_cast<int32_t>(CONST_NODE_OBJECT_COUNT * sizeof(BtreeKeyValuePair) + sizeof(BtreeNode));
    const int32_t BtreeArrayList::NODE_COUNT_PRE_PAGE = static_cast<int32_t>(NODE_SIZE / sizeof(void*) - 1);
