  ...
  namespace common
  {
    ...
    class ObTabletHistogram
    {
      public:
        ...
        NEED_SERIALIZE_AND_DESERIALIZE;//�����л�ʱ�����allocator_��ֵ!!!

      public:
        static const int64_t MAX_SAMPLE_BUCKET = 256;

      private:
        common::ModuleArena *allocator_;//add_sample_with_deep_copy���߷����л�ʱ����õ��ڴ������
    ...
    */
    ...
    class ObTabletHistogramManager
    {
      ...
      */
      public:
        ...
        int pop_histogram(const int32_t hist_index);//add for [bugfix too many histogram in hist manager]

        int64_t get_histogram_size()
        { return histogram_helper_.get_array_index(); }
        /*
        ...
        */
        ...
        int set_table_id(const uint64_t tid, const uint64_t index_tid)
        {
          ...
          if (OB_INVALID_ID == tid || OB_INVALID_ID == index_tid)
          {
            ...
            YYSYS_LOG(WARN, "invalid tid, tid=%lu, index_tid=%lu", tid, index_tid);
          }
          else
          {
            table_id_ = tid;
      ...
      */
      ...
      private:
        ...
        */
        ...
        uint64_t table_id_;
        uint64_t index_tid_;//add_histogramǰ���index_tid_�Ƿ�һ��
        mutable yysys::CThreadMutex alloc_mutex_;//�߳���
    };
        ...
        table_id_(OB_INVALID_ID), index_tid_(OB_INVALID_ID)
    {
      ...
      YYSYS_LOG(ERROR, "test::liumz, init cur_wrapper_=%p", cur_wrapper_);
      */
    }
