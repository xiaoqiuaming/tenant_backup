...
int ObMergerSchemaManager::init(const bool system, const ObSchemaManagerV2 & schema)
{
  ...
  if (true == init_)
  {
    YYSYS_LOG(WARN, "%s", "init schema manager twice");
    ret = OB_INIT_TWICE;
  }
  else if (false == system)
  {
    ...
    update_timestamp_ = yysys::CTimeUtil::getTime();
    latest_version_ = schema.get_version();
    YYSYS_LOG(INFO, "init user schema manager succ:timestamp[%ld]", latest_version_);
    init_ = true;
  }
  else
  {
    ...
    YYSYS_LOG(INFO, "init sys schema manager succ:timestamp[%ld]", schema.get_version());
    init_ = true;
  }
  return ret;
}
    ...
    const ObSchemaManagerV2 ** manager)
{
  ...
  {
    YYSYS_LOG(ERROR, "%s", "check inner stat or schema version failed");
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "check schema version failed:latest[%ld], schema[%ld]",
          latest_version_, schema.get_version());
...
int ObMergerSchemaManager::release_schema(const ObSchemaManagerV2 * schema)
{
  ...
  if (false == check_inner_stat())
  {
    YYSYS_LOG(ERROR, "check inner stat or release version failed");
    ret = OB_INNER_STAT_ERROR;
  }
  else if (NULL == schema)
  {
    YYSYS_LOG(WARN, "check input param schema failed");
    ret = OB_INVALID_ARGUMENT;
  }
  else if (schema != &(sys_schema_.schema))
  {
    bool find = false;
    (void)find; //add for [build warning]
    yysys::CThreadGuard lock(&lock_);
    for (uint64_t i = 0; i < MAX_VERSION_COUNT; ++i)
    {
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "fail to release schema. schema=%p", schema);
  }
  return ret;
}
...
int ObMergerSchemaManager::print_info(void) const
{
  ...
  {
    yysys::CThreadGuard lock(&lock_);
    YYSYS_LOG(INFO, "update[%ld], latest[%ld]", update_timestamp_, latest_version_);
    for (uint64_t i = 0; i < MAX_VERSION_COUNT; ++i)
    {
      if (0 == user_schemas_[i].ref_count)
      {
      ...
      {
        YYSYS_LOG(INFO, "schema ref_count[%ld]", user_schemas_[i].ref_count);
