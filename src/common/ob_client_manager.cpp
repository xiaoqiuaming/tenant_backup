...
namespace oceanbase
{
  namespace common
  {
                                    ...
                                    const int64_t max_request_timeout /*=5000000*/)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(ERROR, "ClientManager already initialized.");
        rc = OB_INIT_TWICE;
      }
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        {
          YYSYS_LOG(ERROR, "invalid arguments eio=%p, handler=%p", eio, handler);
    ...
    {
      error_ = err;
      YYSYS_LOG(WARN, "set_err(err=%d)", err);
    }
    ...
    int ObClientManager::set_dedicate_thread_num(const int num)
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "already set dedicate_thread_num, old_value=%d", dedicate_thread_num_);
      }
      else if (NULL == eio_)
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "eio == NULL");
      }
      else if (eio_->io_thread_pool->thread_count <= num)
      {
        err = OB_SIZE_OVERFLOW;
        YYSYS_LOG(ERROR, "request dedicate_thread_num[%d] >= max_avail_thread_num[%d]", num, eio_->io_thread_pool->thread_count);
      }
      else
      {
        ...
        easy_io_process_pt handler, void* args) const
    {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        YYSYS_LOG(DEBUG, "post packet, pcode=%d channel=%u session=%p session_id=%ld",
                  packet->get_packet_code(), packet->get_channel_id(), s, session_id);
        rc = post_session(s);
        if (OB_SUCCESS != rc)
        {
          //session post faild destroy it
          YYSYS_LOG(WARN, "post session faild destroy session s = %p", s);
        ...
        const int64_t timeout, ObDataBuffer& in_buffer, ObDataBuffer& out_buffer) const
    {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (OB_SUCCESS == rc)
        {
          ObPacket *packet = reinterpret_cast<ObPacket*>(s->r.opacket);
          packet->set_session_id(session_id);
          //YYSYS_LOG(DEBUG, "send packet code is %d, session timeout is %f", packet->get_packet_code(),s->timeout);
          response = reinterpret_cast<ObPacket*>(send_session(s));
          if (NULL == response && 1 == s->error)
          {
            YYSYS_LOG(WARN, "send packet to %s failed, easy_session(%p)_dispatch ret:%d",
                      to_cstring(server), s, s->error);
            rc = OB_RPC_SEND_ERROR;
          }
          else if (NULL == response)
          {
            YYSYS_LOG(WARN, "send packet to %s timeout", to_cstring(server));
      ...
      if (OB_SUCCESS == rc && NULL != response)
      {
        ...
        {
          YYSYS_LOG(ERROR, "insufficient memory in out_buffer, remain:%ld, length=%ld",
        ...
        if (OB_SUCCESS == rc)
        {
          ...
          *chid = packet->get_channel_id();
          ...
          if (pcode == OB_SQL_SCAN_REQUEST || pcode == OB_SQL_GET_REQUEST
              ...
              || pcode == OB_END_DISTRIBUTED_TRANS  || pcode == OB_TRANS_PREPARE_ACK
              || pcode == OB_ROLLBACK_TRANSACTION
              || pcode == OB_PREPARE_TRANSACTION || pcode == OB_COMMIT_TRANSACTION
              //add 20150701:e
              || pcode == OB_NEW_GET_REQUEST || pcode == OB_NEW_SCAN_REQUEST
                  //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
                  || pcode == OB_GET_PUBLICED_TRANCEID)
              //add by maosy e
          {
            PFILL_RPC_START(pcode,*chid);
          }
          YYSYS_LOG(DEBUG, "send packet, pcode=%d, channel=%u session=%p timeout=%f",
                                        ...
                                        int64_t size, easy_session_t *& session) const
    {
      ...
      if (NULL == session)
      {
        YYSYS_LOG(WARN, "create session failed");
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == trace_id)
        {
          ...
          YYSYS_LOG(ERROR, "no memory, alloc trace id failed, ret=%d", ret);
        }
        else
        {
    ...
    int ObClientManager::post_session_using_dedicate_thread(easy_session_t* s, int thread_idx /* =0 */) const
    {
      ...
      {
        rc = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "not set dedicate io thread");
      }
      else if (thread_idx >= dedicate_thread_num_)
      {
        rc = OB_SIZE_OVERFLOW;
        YYSYS_LOG(ERROR, "thread_idx[%d] > dedicate_thread_count[%d]", thread_idx, dedicate_thread_num_);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "post packet to server:%s faild", easy_inet_addr_to_str(&s->addr, buff, OB_SERVER_ADDR_STR_LEN));
          rc = OB_RPC_POST_ERROR;
        }
        else
        {
          YYSYS_LOG(DEBUG, "post packet to server:%s", easy_inet_addr_to_str(&s->addr, buff, OB_SERVER_ADDR_STR_LEN));
        }
      }
      return rc;
    }
    ...
    void* ObClientManager::send_session(easy_session_t* s) const
    {
      ...
      if (OB_SUCCESS == rc)
      {
        ...
        if (NULL == packet)
        {
          YYSYS_LOG(WARN, "send packet to server:%s faild", easy_inet_addr_to_str(&s->addr, buff, OB_SERVER_ADDR_STR_LEN));
        }
        else
        {
          YYSYS_LOG(DEBUG, "send packet succ, server=%s response_packet_code=%d",
