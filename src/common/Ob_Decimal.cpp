...
int ObDecimal::from(int64_t i64)
{
  ...
  if (fi >= (BASE) || fi <= -(BASE))
  {
    ...
    YYSYS_LOG(WARN, "error when covert int to decimal!");
  }
  else
    word[0] = fi;
  return ret;
...
int ObDecimal::from(const char* buff, int64_t buf_len)
{
  ...
  if(buff==NULL)
  {
    ...
    YYSYS_LOG(DEBUG, "failed to convert decimal from string buff,err=NULL buff ptr!");
  }
  ...
  {//[conflict_level B]
    ...
    //ret=OB_ERR_UNEXPECTED;
    ret = OB_VARCHAR_DECIMAL_INVALID;
    YYSYS_LOG(WARN,"failed to convert decimal from string buff,since buf_len is too long ");
  }
  if(OB_SUCCESS==ret)
  {
    //s=buff;
    ...
    {
      ret = OB_VARCHAR_DECIMAL_INVALID;
      YYSYS_LOG(WARN, "failed to convert char to decimal,invalid num=[%c]", *s);
    }
    //add:e
      ...
      for (;; ++s)
      {
        ...
        else
        {
          ...
          else if (*s != '\0')
          {
            ...
            //mod dolphin[coalesce return type]@20151201:b
            //ret=OB_ERR_UNEXPECTED;
            ret = OB_VARCHAR_DECIMAL_INVALID;//[conflict_level B]
            YYSYS_LOG(WARN, "failed to convert char to decimal,invalid num=%c", *s);
          }
        }
        i++;
  ...
  {
    ret = OB_DECIMAL_UNLEGAL_ERROR;
    YYSYS_LOG(USER_ERROR,"decimal overflow!");
    // YYSYS_LOG(ERROR, "decimal overflow!got_digit=%d,got_frac=%d", got_digit,got_frac);
  }
  if(OB_SUCCESS==ret)
  {
    //add wanglei [decimal fix] 20160428 ���������buff������temp_buff
...
int64_t ObDecimal::to_string(char* buf, const int64_t buf_len) const
{
  ...
  else
  {
    ...
    {
      strncpy(tmp + pos, str_for_word.c_str() + start, len_int);
      pos += len_int;
    }
    ...
    if (0 <= (int) vscale_ && scale_ != 0)
    {
      ...
      if (0 < vscale_)
      {
        if(len_int < 0)
        {
          ...
          strcpy(tmp + pos, str_for_word.c_str() + start);
          pos += len_word;
        }
        else
        {
          //      strcpy(tmp + pos, float_str);
          //      pos += (int) strlen(float_str);
          strcpy(tmp + pos, str_for_word.c_str() + len_int + start);
          pos += vscale_;
        }
      }
...
int ObDecimal::to_int64(int64_t &i64) const
{
  ...
  //    YYSYS_LOG(WARN, "the number cannot be converted to int64, vscale=%hhd ",
  //        vscale_);
...
int ObDecimal::cast_to_int64(int64_t &i64) const
{
  ...
  {
    ret=OB_VALUE_OUT_OF_RANGE;
    YYSYS_LOG(WARN, "failed to convert decimal to integer,err=overflowed!");
  }
  if(OB_SUCCESS==ret)
    i64=whole.ToInt();
  return ret;
...
int ObDecimal::round(ObDecimal &out,int64_t num_digits) const
{
  ...
  whole=word[0]/Base;
  rest = word[0]%Base;
  //YYSYS_LOG(ERROR,"testxionghuitest the vscale_ is %d, scale_ is %d",vscale_,scale_);
  if(num_digits == 0)
  {
  ...
  else if(num_digits > 0)
  {
    ...
    //add for [decimal]
    else if(num_digits < scale_)
    {
      out.from(38, static_cast<uint32_t>(num_digits), vscale_, word[0]);
    }
    else
    {
  ...
  else
  {
    ...
    if((-num_digits) <= digit)
    {
      ...
      if(see_num_2 >= 5)
      {
        ...
        //YYSYS_LOG(ERROR,"testxionghuitest mark1 the intege_part is %ld,base_2 is %ld",intege_part.ToInt(),base_2.ToInt());
      }
      else
      {
        out.from(38,0,0,intege_part);
        //YYSYS_LOG(ERROR,"testxionghuitest mark2 the intege_part is %ld,base_2 is %ld",intege_part.ToInt(),base_2.ToInt());
      }
    }
    else
    {
...
int ObDecimal::cast_to_int32(int32_t &i32) const
{
  ...
  {
    ret=OB_VALUE_OUT_OF_RANGE;
    YYSYS_LOG(WARN, "failed to convert decimal to int_32,err=overflowed!");
  }
  if(OB_SUCCESS==ret)
  {
    int64_t temp_int;
...
int ObDecimal::modify_value(uint32_t p, uint32_t s)
{
  ...
  if (OB_SUCCESS==ret&&s != vscale_) //[640]
  {
      if(s < vscale_)
      {
          if (0 == s)
          ...
          else
          {
              ...
              {
                  ret = OB_ERR_UNEXPECTED;
                  YYSYS_LOG(WARN, "failed to assigned str buff=%s",buf);
      ...
      else
      {
          ...
          len = vscale_ + point_pos + 1;
          for( ; vscale_ < s; vscale_++,len++)
              buf[len] = '0';
          buf[len] = '\0';
          strncpy(out, buf, len);
      }
    if(OB_SUCCESS!=ret)
    {
      YYSYS_LOG(ERROR,"failed to modify value");
    }
    else if (OB_SUCCESS != (ret = this->from(out)))
    {
      YYSYS_LOG(ERROR, "failed convert str to decimal!");
    }
  }
  if(OB_SUCCESS==ret)
  {
...
int ObDecimal::add(const ObDecimal &other, ObDecimal &res) const
{
  ...
  if (num1.Add(num2) == 0)
  {
    ...
    if(ret!=OB_SUCCESS)
    {
      ...
      YYSYS_LOG(WARN,"faild to add/sub decimal,over flow,n1 =%s,ttint1=%s,n2=%s,ttint2=%s,res ttint=%s",er1,er3,er2,er4,er5);
    }
  }
  return ret;
}
  ...
  if (num1.Mul(num2) == 0)
  {
    ...
    if(ret==OB_SUCCESS)
    {
      ...
      else
      {
        ...
        res.precision_ = len_int + res.scale_;
        if(res.precision_ <= res.scale_)
          res.precision_ = res.scale_ + 1;
      }
    }
    else
    {
      ...
      strcpy(er4,n2.get_words()[0].ToString().c_str());
      strcpy(er5,num1.ToString().c_str());
      YYSYS_LOG(WARN,"ERROR to mul decimal,over flow,n1 =%s,ttint1=%s,n2=%s,ttint2=%s,res ttint=%s",er1,er3,er2,er4,er5);
    }
  }
  else
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "failed to mul decimal");
  }
  return ret;
}
...
int ObDecimal::div(const ObDecimal &other, ObDecimal &res) const
{
  ...
  if (num1.Div(num2) == 0)
  {
    ...
    else
    {
      ...
      strcpy(er4,n2.get_words()[0].ToString().c_str());
      strcpy(er5,num1.ToString().c_str());
      YYSYS_LOG(WARN,"faild to mul decimal,over flow,n1 =%s,ttint1=%s,n2=%s,ttint2=%s,res ttint=%s",er1,er3,er2,er4,er5);
    }
  ...
  else
  {
    ...
    ret = OB_ERROR;
    //YYSYS_LOG(ERROR, "failed to div decimal,num1=%s,num2=%s",n1.to_string().c_str(),n2.to_string().c_str());
  }
