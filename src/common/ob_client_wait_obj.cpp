  ...
  namespace common
  {
    ...
    int ObClientWaitObj::wait(ObDataBuffer& response, const int64_t timeout_us)
    {
      ...
      else if (done_count_ <= 0 && wait_time_ms <= 0)
      {
        ...
        YYSYS_LOG(ERROR, "wait(timeout_us=%ld) TIMEOUT", timeout_us);
      }
      else
      {
        response = response_;
    ...
    int ObClientWaitObj::receive_packet(ObPacket* packet)
    {
      ...
      {
        err = err_;
        YYSYS_LOG(ERROR, "post packet error, err=%d", err_);
      }
      else
      {
        cond_.lock();
        ...
        else if ((int64_t)packet->get_data_length() > response_.get_capacity())
        {
          ...
          YYSYS_LOG(ERROR, "packet_size[%d] > buf_size[%ld]", packet->get_data_length(), response_.get_capacity());
        }
        else
        {
