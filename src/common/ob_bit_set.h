...
namespace oceanbase
{
  namespace common
  {
    ...
    public:
      ...
      int set_bitset_word(int32_t index, uint32_t word);
      //20160427:e
    ...
    bool ObBitSet<N>::add_member(int32_t index)
    {
      if (index < 0)
      {
        YYSYS_LOG(ERROR, "negative bitmapset member not allowed");
        return false;
      }
      if (index >= MAX_BITMAPWORD * PER_BITMAPWORD_BITS)
      {
        YYSYS_LOG(ERROR, "bitmap index exceeds the scope");
        return false;
      }
    ...
    typename ObBitSet<N>::BitSetWord ObBitSet<N>::get_bitset_word(int32_t index) const
    {
      ...
      {
        YYSYS_LOG(ERROR, "bitmap word index exceeds the scope");
        return 0;
      }
    ...
    //add liuzy [Multiups] [UPS_List_Info] 20160427:b
    template <int32_t N>
    int ObBitSet<N>::set_bitset_word(int32_t index, uint32_t word)
    {
      int ret = OB_SUCCESS;
      if (index < 0 || index >= MAX_BITMAPWORD)
      {
      ...
      {
        bitset_words_[index] = /*static_cast<ObBitSet<N>::BitSetWord>*/word;
      }
      return ret;
    }
    //20160427:e
    ...
    //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160229:b
    template <int32_t N>
    int ObBitSet<N>::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      int ret = OB_SUCCESS;
      for (int32_t i = 0; i < MAX_BITMAPWORD; ++i)
      {
        if (OB_SUCCESS != (ret = serialization::encode_vi32(buf, buf_len, pos, bitset_words_[i])))
        {
    ...
    template <int32_t N>
    int ObBitSet<N>::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      int ret = OB_SUCCESS;
      int32_t bitset_word;
      for (int32_t i = 0; i < MAX_BITMAPWORD; ++i)
      {
        if (OB_SUCCESS != (ret = serialization::decode_vi32(buf, buf_len, pos, &bitset_word)))
        {
        ...
        {
          bitset_words_[i] = static_cast<uint32_t>(bitset_word);
        }
      }
      return ret;
    }
