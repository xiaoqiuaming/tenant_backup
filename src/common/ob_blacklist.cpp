...
ObBlackList::ObBlackList():lock_(yysys::WRITE_PRIORITY)
{
  count_ = 0;
  alpha_ = DEFAULT_DETA;
    ...
    const int64_t alpha_deno, const int64_t threshold, const int64_t threshold_deno)
{
  ...
  if ((count <= 0) || (alpha <= 0) || (alpha_deno <= 0) || (threshold <= 0) || (threshold_deno <= 0))
  {
    ...
    YYSYS_LOG(WARN, "check input param failed:count[%d], alpha[%ld], deno[%ld], threshold[%ld], deno[%ld]",
        count, alpha, alpha_deno, threshold, threshold_deno);
  }
  else if (alpha > alpha_deno)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check input param failed:alpha[%ld], deno[%ld]", alpha, alpha_deno);
  }
  else
  {
    ...
    YYSYS_LOG(DEBUG, "reinit the scores of the blacklist:count[%d], old[%d], alpha[%2lf], threshold[%2lf]",
...
int ObBlackList::update(const bool succ, const int32_t index)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "check index failed:index[%d], count[%d]", index, count_);
  }
  else
  {
    yysys::CWLockGuard lock(lock_);
