      ...
      public:
        ...
        int init(int64_t len)
        {
          ...
          if (len < MIN_ARRAY_LEN || 0 != ((len-1) & len))
          {
            ...
            YYSYS_LOG(ERROR, "INVALID_ARGUMENT: len=%ld", len);
          }
          else if (NULL != items_)
          {
            err = OB_INIT_TWICE;
          ...
          {
            err = OB_MEM_OVERFLOW;
            YYSYS_LOG(ERROR, "ob_malloc(%ld): failed", sizeof(Item) * len);
          }
        ...
        Item* get(int64_t seq)
        { return items_? items_ + (seq & pos_mask_): NULL; }
        int64_t len() const
        { return pos_mask_ + 1; }
      protected:
        void destroy()
      ...
      public:
        ...
        int push(int64_t& seq, DataT& data)
        {
          ...
          else
          {
            ...
            {
              err = OB_EAGAIN;
              YYSYS_LOG(WARN, "wait_queue is full: push=%ld pop=%ld", push_, pop_);
            }
            else
            {
              item->data_ = data;
        ...
        // modify:e
        {
          ...
          else
          {
            ...
            {
              err = OB_ALREADY_DONE;
              YYSYS_LOG(WARN, "someone already mark done seq=%ld real_seq=%ld", seq, item->wait_seq_);
            }
            else if (item->wait_seq_ < seq + REGISTERED)
            {
              err = OB_ERR_UNEXPECTED;
              YYSYS_LOG(ERROR, "unexpected error: done before registered, done_seq=%ld real_seq=%ld", seq, item->wait_seq_);
            }
            else
            {
