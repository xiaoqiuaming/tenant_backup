...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObLoginInfo::set_username(const ObString &username)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param ptr=%p length=%d", username.ptr(), username.length());
    ...
    int ObLoginInfo::set_password(const ObString &password)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param ptr=%p length=%d", password.ptr(), password.length());
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObLoginInfo::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(WARN, "encode MAGIC_NUM fail ret=%d buf=%p buf_len=%ld cur_pos=%ld", ret, buf, buf_len, cur_pos);
      }
      else if (OB_SUCCESS != (ret = serialization::encode_i64(buf, buf_len, cur_pos, CUR_VERSION)))
      {
        YYSYS_LOG(WARN, "encode CUR_VERSION fail ret=%d buf=%p buf_len=%ld cur_pos=%ld", ret, buf, buf_len, cur_pos);
      }
      else if (OB_SUCCESS != (ret = username_str_.serialize(buf, buf_len, cur_pos)))
      {
        YYSYS_LOG(WARN, "encode username fail ret=%d buf=%p buf_len=%ld cur_pos=%ld", ret, buf, buf_len, cur_pos);
      }
      else if (OB_SUCCESS != (ret = password_str_.serialize(buf, buf_len, cur_pos)))
      {
        YYSYS_LOG(WARN, "encode password fail ret=%d buf=%p buf_len=%ld cur_pos=%ld", ret, buf, buf_len, cur_pos);
      }
      else
      {
        pos = cur_pos;
    ...
    int ObLoginInfo::deserialize(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(WARN, "decode magic_num fail ret=%d buf=%p data_len=%ld cur_pos=%ld", ret, buf, data_len, cur_pos);
      }
      ...
      {
        YYSYS_LOG(WARN, "decode version fail ret=%d buf=%p data_len=%ld cur_pos=%ld", ret, buf, data_len, cur_pos);
      }
      else if (CUR_VERSION != version)
      {
        YYSYS_LOG(WARN, "invalid version=%ld buf=%p data_len=%ld cur_pos=%ld", version, buf, data_len, cur_pos);
        ret = OB_ERROR;
      }
      else if (OB_SUCCESS != (ret = username_str_.deserialize(buf, data_len, cur_pos)))
      {
        YYSYS_LOG(WARN, "decode username fail ret=%d buf=%p data_len=%ld cur_pos=%ld", ret, buf, data_len, cur_pos);
      }
      else if (OB_SUCCESS != (ret = password_str_.deserialize(buf, data_len, cur_pos)))
      {
        YYSYS_LOG(WARN, "decode password fail ret=%d buf=%p data_len=%ld cur_pos=%ld", ret, buf, data_len, cur_pos);
      }
      else
      {
        pos = cur_pos;
    ...
    int ObLoginMgr::init(ISKeyTable *skey_table, IUserTable *user_table)
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(WARN, "have already inited this=%p", this);
        ret = OB_INIT_TWICE;
      }
      else if (NULL == skey_table
               || NULL == user_table)
      {
        YYSYS_LOG(WARN, "invalid param skey_table=%p user_table=%p", skey_table, user_table);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObLoginMgr::handle_login(const ObLoginInfo &login_info, IToken &token)
    {
      ...
      {
        YYSYS_LOG(WARN, "user info check fail ret=%d %s", ret, login_info.log_str());
      }
      else if (OB_SUCCESS != (ret = skey_table_->get_cur_skey(skey_version, skey)))
      {
        YYSYS_LOG(WARN, "get cur skey fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = token.set_username(username)))
      {
        YYSYS_LOG(WARN, "set username to token fail ret=%d username=%.*s", ret, username.length(), username.ptr());
      }
      else if (OB_SUCCESS != (ret = set_timestamp(token, timestamp)))
      {
        YYSYS_LOG(WARN, "set timestamp to token fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = token.encrypt(skey_version, skey)))
      {
        YYSYS_LOG(WARN, "encrypt token fail ret=%d skey_version=%ld", ret, skey_version);
      }
      else
      {
        YYSYS_LOG(INFO, "login succ %s skey_version=%ld timstamp=%ld", login_info.log_str(), skey_version, timestamp);
      }
      return ret;
    }
    ...
    int ObLoginMgr::check_token(char *buf, const int64_t data_len, int64_t &pos, IToken &token)
    {
      ...
      {
        //YYSYS_LOG(WARN, "deserialize token fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = skey_table_->get_skey(token.get_skey_version(), skey)))
      {
        YYSYS_LOG(WARN, "get skey fail ret=%d skey_version=%ld", ret, token.get_skey_version());
      }
      else if (OB_SUCCESS != (ret = token.decrypt(skey)))
      {
        YYSYS_LOG(WARN, "decrypt token fail ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = check_timestamp(token)))
      {
        YYSYS_LOG(WARN, "check timestamp fail ret=%d", ret);
      }
      else
      {
        // check succ
    ...
    {
      int ret = OB_SUCCESS;
      uint64_t local_ip = yysys::CNetUtil::getLocalAddr(NULL);
      int64_t time_sec = yysys::CTimeUtil::getTime() / 1000000;
    ...
    int ObLoginMgr::check_timestamp(IToken &token)
    {
      ...
      {
        YYSYS_LOG(WARN, "get timestamp from token fail ret=%d", ret);
      }
      else
      {
        ...
        if (((time_sec + TOKEN_EXPIRED_TIME) * 1000000) < yysys::CTimeUtil::getTime())
        {
          YYSYS_LOG(WARN, "timestamp=%ld time_sec=%ld cur_time=%ld timeout=%ld",
                    timestamp, time_sec, yysys::CTimeUtil::getTime(), TOKEN_EXPIRED_TIME);
