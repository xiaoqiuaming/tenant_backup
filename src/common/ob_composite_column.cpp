...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObCompositeColumn::set_expression(const ObObj *expr, oceanbase::common::ObStringBuf  & data_buf)
    {
      ...
      {
        YYSYS_LOG(WARN, "set postfix expression error.");
      }
      else
      {
        state = USE_POSTFIX;
                                          ...
                                          bool               is_expire_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "set infix expression error. [expr=%.*s][err=%d]",
                                              ...
                                              )
    {
      ...
      /*  FIXME: Ŀǰ��֧��"select 5+3"������䣬Ҫ��select�����ٳ���һ���С�
       *  ���ִ������ʱ���潫����
       */
      if (org_row_beg < 0
          || org_row_end < 0
      ...
      {
        YYSYS_LOG(WARN,"param error [org_row_beg:%ld,org_row_end:%ld,org_cells.get_cell_size():%ld]",
      ...
      if (OB_SUCCESS == err)
      {
        if (USE_POSTFIX == state)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to calculate composite column value, err=%d", err);
          }
        }
...
#else          
          err = OB_ERROR;
          YYSYS_LOG(WARN, "please convert infix expression to postfix expression first.");
#endif          
        }
        else
        {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN, "Invalid expression calculator.");
        }
      }
      return err;
    }
    ...
    DEFINE_SERIALIZE(ObCompositeColumn)
    {
      ...
      if (USE_POSTFIX == state)
      {
        YYSYS_LOG(DEBUG, "serialize::postfix mode");

        obj.set_int((int64_t)USE_POSTFIX); // indicates postfix
        if (OB_SUCCESS != (err = obj.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "serialize USE_POSTFIX flag failed. err[%d]", err);
        }
        else if (OB_SUCCESS != (err = postfix_.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "serialize postfix expression failed. err[%d]", err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "serialize postfix expression OK");
        }
      }
      else if(USE_INFIX == state)
      {
        YYSYS_LOG(DEBUG, "serialize::infix mode");

        obj.set_int((int64_t)USE_INFIX); // indicates infix
        if (OB_SUCCESS != (err = obj.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "serialize USE_INFIX flag failed. err[%d]", err);
        }
        else if (OB_SUCCESS != (err = infix_.serialize(buf, buf_len, pos)))
        {
          YYSYS_LOG(WARN, "serialize infix expression failed. err[%d]", err);
        }
        else
        {
          ...
          if (OB_SUCCESS != (err = obj.serialize(buf, buf_len, pos)))
          {
            YYSYS_LOG(WARN, "serialize as_column_name_ failed. err[%d]", err);
          }
        }
      }
      else
      {
        err = OB_NOT_INIT;
        YYSYS_LOG(ERROR, "unexpected expression type");
      }
      return err;
    }
    ...
    DEFINE_DESERIALIZE(ObCompositeColumn)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "deserialize composite column type failed:err[%d]", err);
      }
      else if (ObIntType == obj.get_type())
      {
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(WARN, "get composite column type int value failed:err[%d]", err);
        }
        if (USE_INFIX == column_type) // infix expression
        {
          YYSYS_LOG(DEBUG, "deserialize::infix mode");
          if(OB_SUCCESS != (err = infix_.deserialize(buf, data_len, pos)))
          {
            YYSYS_LOG(WARN, "deserialize infix expression failed:err[%d]", err);
          }
          else if(OB_SUCCESS != (err = obj.deserialize(buf, data_len, pos)))
          {
            YYSYS_LOG(WARN, "deserialize as_column_name_ failed:err[%d]", err);
          }
          else if(OB_SUCCESS != (err = obj.get_varchar(as_column_name_)))
          {
            YYSYS_LOG(WARN, "get as_column_name_ value failed:err[%d]", err);
          }
          else
          {
            state = USE_INFIX;
        ...
        {
          YYSYS_LOG(DEBUG, "deserialize::postfix mode");
          if(OB_SUCCESS != (err = postfix_.deserialize(buf, data_len, pos)))
          {
            YYSYS_LOG(WARN, "deserialize postfix expression failed:err[%d]", err);
          }
          else
          {
            state = USE_POSTFIX;
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(WARN, "not initialized.");
        }
      }
      else
      {
        pos = old_pos;
        err = OB_UNKNOWN_OBJ;
        /// YYSYS_LOG(WARN, "Int column type expected:[type:%d]", obj.get_type());
      }
      return err;
    }
