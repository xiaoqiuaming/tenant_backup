...
int ObTriggerEventUtil::format(ObString & sql, const int64_t type, const int64_t param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "update trigger event sql failed:type[%ld], param[%ld], err[%d]", type, param, ret);
  }
  return ret;
}
...
int ObTriggerEventUtil::execute(const int64_t type, const int64_t param, ObTriggerEvent & trigger)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "format trigger event sql failed:type[%ld], ret[%d]", type, ret);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "execute sql failed:sql[%.*s], ret[%d]", sql.length(), sql.ptr(), ret);
    }
  }
  return ret;
}
