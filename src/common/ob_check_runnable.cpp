...
void ObCheckRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  int err = OB_SUCCESS;
  ...
  while (!_stop)
  {
    if (yysys::CNetUtil::isLocalAddr(vip_))
    {
      if (ObRoleMgr::SLAVE == role_mgr_->get_role())
      {
        ...
        YYSYS_LOG(INFO, "vip found on slave, vip=%s", inet_ntoa(vip_addr));
        ...
        {
          YYSYS_LOG(ERROR, "vip found on slave, but slave lease is invalid, vip=%s", inet_ntoa(vip_addr));
        }
      }
    }
    else
    {
      if (ObRoleMgr::MASTER == role_mgr_->get_role())
      {
        struct in_addr vip_addr;
        vip_addr.s_addr = vip_;
        YYSYS_LOG(ERROR, "Incorrect vip, STOP server, vip=%s", inet_ntoa(vip_addr));
      ...
      else if (ObRoleMgr::SLAVE == role_mgr_->get_role() && lease_on_)
      {
        if (ObRoleMgr::ACTIVE == role_mgr_->get_state())
        {
          ...
          if (LEASE_INVALID == status)
          {
            YYSYS_LOG(ERROR, "Invalid lease, set server to INIT state");
          ...
          else if (LEASE_SHOULD_RENEW == status)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to renew_lease_, err=%d", err);
            }
          }
        }
      }
    ...
    ObCommonRpcStub* rpc_stub, ObServer* server, ObServer* slave_addr)
{
  ...
  if (NULL == role_mgr)
  {
    YYSYS_LOG(WARN, "Parameter is invalid[role_mgr=%p]", role_mgr);
...
int64_t ObCheckRunnable::get_lease_status_()
{
  ...
  YYSYS_LOG(DEBUG, "lease_time_ = %ld, lease_interval_ =%ld, cur_time_us=%ld", lease_time_, lease_interval_, cur_time_us);
  if (lease_time_ + lease_interval_ < cur_time_us)
  {
    YYSYS_LOG(WARN, "Lease expired, lease_time_=%ld, lease_interval_=%ld, cur_time_us=%ld",
  ...
  {
    YYSYS_LOG(DEBUG, "Lease will expire, lease_time_=%ld, lease_interval_=%ld, "
...
int ObCheckRunnable::renew_lease_()
{
  ...
  {
    YYSYS_LOG(WARN, "invalid status, rpc_stub_=%p, server_=%p", rpc_stub_, server_);
    err = OB_ERROR;
  }
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to renew lease, err=%d, timeout=%ld, server=%s", err, renew_lease_timeout_, server_->to_cstring());
    }
  }
...
int ObCheckRunnable::renew_lease(const ObLease& lease)
{
  ...
  YYSYS_LOG(DEBUG, "Renew lease, lease_time=%ld lease_interval=%ld renew_interval=%ld",
