    ...
    class TSINode : public TSINodeBase
    {
      public:
        ...
        virtual ~TSINode()
        {
          if (NULL != instance_)
          {
            YYSYS_LOG(INFO, "delete instance [%s] %p", typeid(T).name(), instance_);
            instance_->~T();
            instance_ = NULL;
          }
        };
    ...
    class ThreadSpecInfo
    {
      ...
      public:
        ...
        T *get_instance()
        {
          ...
          {
            YYSYS_LOG(WARN, "new instance [%s] fail", typeid(T).name());
          }
          else
          {
            ...
            if (NULL == node)
            {
              YYSYS_LOG(WARN, "new tsi_node fail [%s]", typeid(T).name());
              instance->~T();
              instance = NULL;
            }
            else
            {
              YYSYS_LOG(DEBUG, "new instance succ [%s] %p size=%ld, tsi=%p", typeid(T).name(), instance, sizeof(T), this);
              if (NULL == list_)
              {
                list_ = node;
                list_->next = NULL;
    ...
    class TSIFactory
    {
      ...
      public:
        int init()
        {
          ...
          */
          if (0 != ( ret = PTHREAD_KEY_MGR->create_thread_key(key_)))
          {
            YYSYS_LOG(WARN, "pthread_key_create fail ret=%d", ret);
            ret = common::OB_ERROR;
          }
          return ret;
        };
        ...
        T *get_instance()
        {
          ...
          if (NULL == instance && key_.is_valid())
          {
            //ThreadSpecInfo *tsi = (ThreadSpecInfo*)pthread_getspecific(key_);
            ThreadSpecInfo *tsi = (ThreadSpecInfo*)(PTHREAD_KEY_MGR->get_thread_specifi(key_));
            if (NULL == tsi)
            {
              tsi = new(std::nothrow) ThreadSpecInfo();
              if (NULL != tsi)
              {
                //YYSYS_LOG(INFO, "new tsi succ %p key=%d", tsi, key_);
                YYSYS_LOG(INFO, "new tsi succ %p key=%ld", tsi, key_.index_);
              }
              if (NULL != tsi
                  //&& 0 != pthread_setspecific(key_, tsi))
                  && 0 != PTHREAD_KEY_MGR->set_thread_specifi(key_, tsi))
              {
