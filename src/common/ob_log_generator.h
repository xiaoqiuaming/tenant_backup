...
#include "ob_transaction.h"  //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701
  ...
  namespace common
  {
    ...
    class ObLogGenerator
    {
      ...
      public:
        ...
        //add e
        int update_cursor(const ObLogCursor& log_cursor); // ����д��־ʱҲ�����log_generator��cursor
         ...
         */
        template<typename T>
        int write_log(const LogCommand cmd, T& data, const common::ObTransID& trans_id);
        //add 20150701:e
      ...
      public:
        ...
        int64_t get_max_log_id()
        {
          return end_cursor_.log_id_ - 1;
        }
                     ...
                     )
    {
      ...
      if (NULL == buf || 0 >= len || pos > len || !cursor.is_valid())
      {
        ...
        YYSYS_LOG(ERROR, "generate_log(buf=%p, len=%ld, pos=%ld, cursor=%s)=>%d",
      ...
      else if (OB_SUCCESS != (err = data.serialize(buf, len, end_pos)))
      {
        if (entry.get_serialize_size() + data.get_serialize_size() > len)
        {
          ...
          YYSYS_LOG(WARN, "log too large(size=%ld, limit=%ld)", data.get_serialize_size(), len);
        }
        else
        {
      ...
      {
        YYSYS_LOG(ERROR, "serialize_log_entry(buf=%p, len=%ld, entry[id=%ld], data_len=%ld)=>%d",
    ...
    template<typename T>
    int generate_log(char* buf, const int64_t len, int64_t& pos, ObLogCursor& cursor, const LogCommand cmd,
                     const T& data, const common::ObTransID& trans_id, const int64_t term)
    {
      int err = OB_SUCCESS;
      ObLogEntry entry;
      int64_t new_pos = pos;
      ...
      else if (OB_SUCCESS != (err = trans_id.serialize_for_prepare(buf, len, end_pos))
               || OB_SUCCESS != (err = data.serialize(buf, len, end_pos)))
      {
        if (entry.get_serialize_size() + trans_id.get_serialize_size_for_prepare()
            + data.get_serialize_size() > len)
        {
          err = OB_LOG_TOO_LARGE;
          YYSYS_LOG(WARN, "log too large(size=%ld, limit=%ld)", data.get_serialize_size(), len);
        }
      ...
      else if (OB_SUCCESS != (err = cursor.next_entry(entry, cmd, buf + data_pos, end_pos - data_pos, term)))
      {
        YYSYS_LOG(ERROR, "cursor[%s].next_entry()=>%d", cursor.to_str(), err);
      }
      else if (OB_SUCCESS != (err = entry.serialize(buf, new_pos + entry.get_serialize_size(), new_pos)))
      ...
      {
        YYSYS_LOG(DEBUG,"generate_log:cmd = %d, trans_id = %s",cmd, to_cstring(trans_id));
        pos = end_pos;
      }
      return err;
    }
    ...
    //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
    template<typename T>
    int ObLogGenerator::write_log(const LogCommand cmd, T& data, const common::ObTransID& trans_id)
    {
      int err = OB_SUCCESS;
      if (OB_SUCCESS != (err = check_state()))
      {
      ...
      else if (is_frozen_)
      {
        ...
        YYSYS_LOG(ERROR, "log_generator is frozen, cursor=[%s,%s]", to_cstring(start_cursor_), to_cstring(end_cursor_));
                                                 ...
                                                 end_cursor_, cmd, data, trans_id, log_term_))
               && OB_BUF_NOT_ENOUGH != err)
      {
        YYSYS_LOG(WARN, "generate_log(pos=%ld)=>%d", pos_, err);
      }
