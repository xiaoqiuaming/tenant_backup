...
namespace oceanbase
{
  namespace common
  {
    ...
    {
      int ret = OB_SUCCESS;
      yysys::CThreadGuard guard(&rs_mutex_);
      int32_t master_idx = get_master_rs_idx();
      if (master_idx < 0)
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "failed to find master rootserver");
      }
      else
      {
    ...
    void ObRsAddressMgr::set_master_rs(const common::ObServer &master)
    {
      ...
      if (-1 == (idx = find_rs_index(master)))
      {
        ...
        {
          YYSYS_LOG(WARN, "add rootserver failed, rs=%s", master.to_cstring());
        }
        else
        {
    ...
    int ObRsAddressMgr::update_all_rs(const common::ObServer *rs_array, const int32_t count)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "unexpected error, ret=%d", ret);
      }
      else if (list_equal(rs_array, count))
      {
        YYSYS_LOG(DEBUG, "rootserver list do not change, rs num=%d", count);
        for (int i = 0; i < count; i++)
        {
          YYSYS_LOG(DEBUG, "list[%d], rs=[%s]", i, rootservers_[i].to_cstring());
        }
      }
      else
      {
        //add:e
        YYSYS_LOG(INFO, "begin update rootserver list");
        ObServer tmp_server;
        if (master_rs_idx_ >= 0)
        {
        ...
        for (int32_t i = 0; i < count; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "add rootserver failed, rs=%s", rs_array[i].to_cstring());
          }
          else
          {
            YYSYS_LOG(INFO, "rs list, rs[%d]=%s", i, rootservers_[i].to_cstring());
          }
        }
        ...
        {
          YYSYS_LOG(ERROR, "master index in rs array is invalid");
        }
        else
        {
          set_master_rs_idx(master_idx);
          YYSYS_LOG(INFO, "end update rootserver list, count=%d, master_rs=%s",
