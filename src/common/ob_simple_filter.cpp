  ...
  const ObObj & cond_value)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "store column name failed:src[%.*s], ret[%d]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "copy obj failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add the condition item failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int ObSimpleFilter::copy_obj(const ObObj & cond_value, ObObj & store_value)
{
  ...
  if (cond_value.get_type() == ObVarcharType)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get obj string failed:ret[%d]", ret);
    }
    else
    {
      ObString store_ptr;
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "store obj string failed:src[%.*s], ret[%d]",
  ...
  const ObObj & cond_value)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "set condtion content failed:ret[%d]", ret);
    }
    else
    {
      ret = add_cond(cond);
...
int ObSimpleFilter::add_cond(const ObSimpleCond & cond)
{
  ...
  {
    YYSYS_LOG(ERROR, "push back the condition item failed:ret[%d]", ret);
...
const ObSimpleCond * ObSimpleFilter::operator [] (const int64_t index) const
{
  ...
  {
    YYSYS_LOG(ERROR, "check index failed:index[%ld], count[%ld]",
...
int ObSimpleFilter::operator = (const ObSimpleFilter & other)
{
  ...
  for (int64_t i = 0; i < size; ++i)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add condition failed:pos[%ld], ret[%d]", i, ret);
      break;
    }
  }
  return ret;
...
bool ObSimpleFilter::operator == (const ObSimpleFilter & other) const
{
  ...
  if (true == ret)
  {
    for (int64_t i = 0; i < size; ++i)
    {
      ...
      if (false == ret)
      {
        YYSYS_LOG(DEBUG, "check the %ldth condition failed", i);
        break;
      }
    }
  }
  ...
  const int64_t row_end, bool & result) const
{
  ...
  for (int64_t i = 0; i < size; ++i)
  {
    ...
    {
      YYSYS_LOG(ERROR, "check cell index failed:index[%ld], begin[%ld], end[%ld]",
...
int ObSimpleFilter::default_false_check(const ObObj* objs, const int64_t obj_count, bool& result)
{
  ...
  {
    YYSYS_LOG(WARN, "invlid param, objs=%p, obj_count=%ld", objs, obj_count);
    ret = OB_ERROR;
  }
  else
  {
    for (int64_t i = 0; i < size; ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check cell index failed:index=%ld, objs=%p, obj_count=%ld",
...
DEFINE_SERIALIZE(ObSimpleFilter)
{
  ...
  for (int64_t i = 0; i < size; ++i)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "the %ldth condition serialize failed:ret[%d]", i, ret);
      break;
    }
  }
  return ret;
...
DEFINE_DESERIALIZE(ObSimpleFilter)
{
    ...
    && (ObExtendType != temp.get_type()))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "the condition deserialize failed:ret[%d]", ret);
      break;
    }
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add condition failed:ret[%d]", ret);
      break;
    }
    temp_pos = pos;
  }
