    ...
    class CellInfoProcessor
    {
      ...
      public:
        ...
        inline bool analyse_syntax(ObMutatorCellInfo &mutator_cell_info)
        {
          ...
          if (ObExtendType == value.get_type())
          {
            switch (value.get_ext())
            {
              ...
              case ObActionFlag::OP_NOP:
                ...
                break;
              default:
                //YYSYS_LOG(WARN, "value invalid extend info %ld %s", value.get_ext(), print_cellinfo(&cell_info));
                bret = false;
                break;
            }
          }
                  ...
                  && value.is_valid_type())
          {
            ...
            switch (op_type.get_ext() & ObActionFlag::OP_ACTION_FLAG_LOW_MASK)
            {
              ...
              case ObActionFlag::OP_INSERT:
                ...
                break;
              default:
                //YYSYS_LOG(WARN, "op_type invalid extend info %ld %s", op_type.get_ext(), print_cellinfo(&cell_info));
                bret = false;
                break;
            }
          }
          ...
          {
            //YYSYS_LOG(WARN, "no extend info in op_type or value op_type=%d value_type=%d %s",
        ...
        inline bool cellinfo_check(const ObCellInfo &cell_info)
        {
          ...
          {
            //YYSYS_LOG(WARN, "invalid rowkey %s", to_cstring(cell_info.row_key_));
            bret = false;
          }
          else if (NULL == cell_info.table_name_.ptr() || 0 == cell_info.table_name_.length())
          {
            YYSYS_LOG(WARN, "invalid table name ptr=%p length=%d",
          ...
          {
            YYSYS_LOG(WARN, "invalid column name ptr=%p length=%d op_type=%ld",
          ...
          if (!bret)
          {
            //YYSYS_LOG(WARN, "invalid cell_info %s", print_cellinfo(&cell_info));
          }
          return bret;
        }
                                          ...
                                          const ObTableSchema &table_schema)
        {
          ...
          {
            YYSYS_LOG(WARN, "length=%ld not match schema, schema=%ld",
                      cell_info.row_key_.get_obj_cnt(), table_schema.get_rowkey_info().get_size());
            bret = false;
          }
          else if (NULL == objs)
          {
            YYSYS_LOG(WARN, "rowkey obj null pointer");
            bret = false;
          }
          else
          {
            for (int32_t i = 0; i < cell_info.row_key_.get_obj_cnt(); i++)
            {
			  ...
			  //modify:e
              {
                YYSYS_LOG(WARN, "rowkey column type does not match index=%d schema_type=%d rowkey_type=%d",
              ...
              {
                YYSYS_LOG(WARN, "varchar column length too long index=%d schema_length=%ld rowkey_length=%d",
                                          ...
                                          const ObColumnSchemaV2 &column_schema)
        {
          ...
          {
            YYSYS_LOG(WARN, "schema_type=%d value_type=%d not match", column_schema.get_type(), value.get_type());
            bret = false;
          }
          else if (NULL != column_schema.get_join_info())
          {
            YYSYS_LOG(WARN, "column_id=%lu only depend by join table, cannot update", column_schema.get_id());
            bret = false;
          }
          else
          {
            if (ObVarcharType == value.get_type())
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid varchar schema_max_length=%ld ptr=%p length=%d",
          ...
          if (bret)
          {
            switch (column_schema.get_type())
            {
              ...
              case ObModifyTimeType:
                YYSYS_LOG(WARN, "cannot update value_type=%d", column_schema.get_type());
                bret = false;
                break;
              case ObSeqType:
                YYSYS_LOG(WARN, "not support seq_type now");
                bret = false;
                break;
              default:
                break;
