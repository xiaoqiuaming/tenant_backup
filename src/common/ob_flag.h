...
namespace oceanbase
{
  namespace common
  {
    ...
    inline int ObFlag<const char*>::set(const char* v)
    {
      ...
      if (NULL == v)
      {
        YYSYS_LOG(ERROR, "invalid flag value, flag=%s v=NULL", flag_str_);
        ret = common::OB_INVALID_ARGUMENT;
      }
      else if (common::OB_MAX_FLAG_VALUE_LENGTH <= (len = static_cast<int32_t>(strlen(v))))
      {
        YYSYS_LOG(ERROR, "buffer not enough, flag=%s v=%s", flag_str_, v);
    ...
    {
      YYSYS_LOG(INFO, "flag %s=%d", flag.get_flag_str(), flag.get());
    }
    
    inline void g_print_flag(const ObFlag<int64_t>& flag)
    {
      YYSYS_LOG(INFO, "flag %s=%ld", flag.get_flag_str(), flag.get());
    }

    inline void g_print_flag(const ObFlag<const char*>& flag)
    {
      YYSYS_LOG(INFO, "flag %s=%s", flag.get_flag_str(), flag.get());
    }
...
{                                                                   \
  YYSYS_LOG(ERROR, "invalid flag value, set default value");        \
...
{                                                                   \
  YYSYS_LOG(WARN, "flag value is empty, flag=%s", flag.get_flag_str()); \
  ret = flag.set(flag.get_default());                               \
  }                                                                   \
  else                                                                \
{                                                                   \
...
{                                                                 \
  YYSYS_LOG(ERROR, "invalid flag value");                         \
  }                                                                 \
  else if ('\0' != *endptr)                                         \
{                                                                 \
  ret = flag.set(flag.get_default());                             \
  YYSYS_LOG(ERROR, "invalid flag value, flag=%s v=%s", flag.get_flag_str(), tmp_str); \
