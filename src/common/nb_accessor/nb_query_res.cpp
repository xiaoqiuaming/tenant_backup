...
int QueryRes::next_row()
{
  ...
  {
    YYSYS_LOG(WARN, "scanner iter get cell fail:ret[%d]", ret);
  }
  else if(!is_row_changed)
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "not the begin cell, is_row_changed=%d, cell=%s", 
        is_row_changed, print_cellinfo(cell));
  }
  else if (cur_row_.count() < 1)
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "need row cell count :%ld cannot got any cells.", cur_row_.count());
  }
  else
  {
    ...
    while (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot set to cur_row_ (%ld), cell=%p", column_index, cell);
      }
      ...
      else
      {
        ...
        { 
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "current cell stream end before got enough cell, index:(%ld), count:(%ld)",
              column_index, cur_row_.count());
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get cell from scanner error, index:(%ld), count:(%ld)",
              column_index, cur_row_.count());
        }
        else if (is_row_changed)
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "current row changed end before got enough cell, index:(%ld), count:(%ld)",
              column_index, cur_row_.count());
        }
      }
    }
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "column from scanner for one row is not enough, "
...
int QueryRes::init(const SC& sc)
{
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "SC exec fail:ret[%d]", ret);
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "create hash map fail:ret[%d]", ret);
    }
  }
  if(OB_SUCCESS == ret)
  {
    for(int32_t i=0;i<sc.count() && OB_SUCCESS == ret;i++)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "col_name is null");
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "hash map insert error:err[%d]", err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "add [%s] to cell map", col_name);
        }
      }
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(NULL == buf)
    {
      ...
      YYSYS_LOG(WARN, "allocate memory fail");
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "init table row fail:ret[%d]", ret);
    }
  }
