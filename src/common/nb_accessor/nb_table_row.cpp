...
void TableRow::dump() const
{
  ...
  {
    YYSYS_LOG(DEBUG, "cell:%ld, %s", i, print_cellinfo(&cells_[i]));
  }
}
...
int TableRow::init(hash::ObHashMap<const char*, int64_t>* cell_map, ObCellInfo* cells, int64_t cell_count)
{
  ...
  if(NULL == cell_map || NULL == cells || cell_count <= 0)
  {
    ...
    YYSYS_LOG(WARN, "cell_map[%p], cells[%p], cell_count[%ld]", cell_map, cells, cell_count);
  }
  else
  {
...
ObCellInfo* TableRow::get_cell_info(const char* column_name) const
{
  ...
  if(NULL == column_name)
  {
    ...
    YYSYS_LOG(WARN, "column_name is null");
  }

  if(OB_SUCCESS == err && !check_inner_stat())
  {
    err = OB_ERROR;
    YYSYS_LOG(WARN, "cells_[%p], cell_map_[%p], cell_count_[%ld]", cells_, cell_map_, cell_count_);
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(-1 == hash_rc)
    {
      ret = NULL;
      err = OB_ERROR;
      YYSYS_LOG(WARN, "cell_map_ get fail:column_name[%s]", column_name);
    }
    else if(hash::HASH_NOT_EXIST == hash_rc)
    {
      ret = NULL;
      err = OB_ERROR;
      YYSYS_LOG(WARN, "hash not exist:column_name[%s]", column_name);
    }
  }
...
ObCellInfo* TableRow::get_cell_info(int64_t index) const
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "index is out of range:index[%ld], cell_count_[%ld]", index, cell_count_);
  }
...
int TableRow::set_cell(ObCellInfo* cell, int64_t index)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "cell[%p], index[%ld], cell_count_[%ld]", cell, index, cell_count_);
  }
