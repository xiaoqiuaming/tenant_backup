...
int ObNbAccessor::get(QueryRes*& res, const char* table_name, const ObRowkey& rowkey, const SC& select_columns)
{
  ...
  if(NULL == table_name && NULL != res)
  {
    ...
    YYSYS_LOG(WARN, "table_name[%p], res[%p]", table_name, res);
  }

  if(OB_SUCCESS == ret && OB_SUCCESS != select_columns.get_exec_status())
  {
    ret = select_columns.get_exec_status();
    YYSYS_LOG(WARN, "select columns error:ret[%d]", ret);
  }

  if(OB_SUCCESS == ret && !check_inner_stat())
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check inner status fail");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(NULL == param)
    {
      ...
      YYSYS_LOG(WARN, "get thread specific get param fail");
    }
    else
    {
      param->reset();
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    for(int32_t i=0;i<select_columns.count() && OB_SUCCESS == ret;i++)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "get select column element fail:ret[%d]", ret);
      }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "allocate QueryRes fail");
    }
    else
    {
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN,"get from ms failed,ret = %d",ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "got %ld cells", res->get_scanner()->get_size());
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "scanner is not fullfilled, the result is too large, which is unsupported now");
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "create QueryRes fail:ret[%d]", ret);
    }
  }
...
int ObNbAccessor::get_next_scanner(QueryRes*& res)
{
  ...
  {
    YYSYS_LOG(WARN, "ObNbAccessor failed to receive next packet! ret = %d", ret);
  }
  else
  {
    res->reinit();
...
int ObNbAccessor::scan(QueryRes*& res, const char* table_name, const ObNewRange& range, const SC& select_columns, const ScanConds& scan_conds)
{
  ...
  if(OB_SUCCESS == ret && OB_SUCCESS != scan_conds.get_exec_status())
  {
    ...
    YYSYS_LOG(WARN, "scan conds error:ret[%d]", ret);
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "get thread specific scan param fail");
    }
    else
    {
      param->reset();
  ...
  for(int32_t i=0;i<select_columns.count() && OB_SUCCESS == ret;i++)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get sc array element fail:i[%d], ret[%d]", i, ret);
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add column to scan param fail:ret[%d]", ret);
      }
    }
  }
  ...
  for(int32_t i=0;i<scan_conds.count() && OB_SUCCESS == ret;i++)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get scan cond array element fail:i[%d], ret[%d]", i, ret);
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "add cond to scan param fail:ret[%d]", ret);
      }
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN,"scan from ms failed,ret = %d",ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "got %ld size, cell num:%ld, row num:%ld",
...
//[index bulid clusters on/offline]
int ObNbAccessor::scan(QueryRes *&res, const char *table_name, const ObNewRange &range, const SC &select_columns, const ScanConds &scan_conds, const char *groupby_column)
{
    int ret = OB_SUCCESS;
    ObScanParam* param = NULL;
    ObVersionRange version_range;
    ...
    for(int32_t i=0;i<select_columns.count() && OB_SUCCESS == ret;i++)
    {
      ...
      {
        ob_select_column.assign_ptr(const_cast<char*>(select_column), static_cast<int32_t>(strlen(select_column)));
        if(OB_SUCCESS != (ret = param->add_column(ob_select_column)))
        {
          YYSYS_LOG(WARN, "add column to scan param fail:ret[%d]", ret);
        }
        else if(OB_SUCCESS != (ret = param->get_group_by_param().add_return_column(ob_select_column)))
        {
          YYSYS_LOG(WARN, "add column to return_columns_ fail:ret[%d]", ret);
        }
      }
    }
    ...
    for(int32_t i=0;i<scan_conds.count() && OB_SUCCESS == ret;i++)
    {
      ...
      if(OB_SUCCESS == ret)
      {
          ret = param->add_column(scan_cond.get_column_name(),false);
          if(OB_SUCCESS != ret)
          {
              YYSYS_LOG(WARN, "add column to scan param fail:ret[%d]", ret);
          }
    ...
    if(OB_SUCCESS == ret)
    {
        if(OB_SUCCESS != (ret = param->get_group_by_param().add_groupby_column(ObString::make_string(groupby_column),false)))
        {
            YYSYS_LOG(WARN, "add groupby column to scan param fail:ret[%d]", ret);
        }
        else if(OB_SUCCESS != (ret = param->add_column(ObString::make_string(groupby_column),false)))
        {
            YYSYS_LOG(WARN, "add column to scan param fail:ret[%d]", ret);
        }
    }
...
int ObNbAccessor::delete_row(const char* table_name, const ObRowkey& rowkey)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "add del row info to mutator fail:ret[%d]", ret);
    }
  }
...
int ObNbAccessor::delete_row(const char* table_name, const SC& rowkey_columns, const ScanConds& scan_conds)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "table_name is null");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "scan fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "allocate memory for rowkey fail:ret[%d]", ret);
    }
  }
  ...
  while (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to get next row[%d]", ret);
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "unexpected error:row->count[%ld], rowkey_columns.count[%ld]", row->count(), rowkey_columns.count());
      }
    }
    ...
    for(int32_t i=0;(OB_SUCCESS == ret) && (i<row->count());i++)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "get cell info fail");
      }
    }
...
int ObNbAccessor::insert(const char* table_name, const ObRowkey& rowkey, const KV& kv)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "init mutator helper fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "insert fail:ret[%d]", ret);
    }
  }
...
int ObNbAccessor::init(ObScanHelper* client_proxy)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "client_proxy is null");
  }
  else
  {
