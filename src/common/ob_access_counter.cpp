...
int ObAccessCounter::inc(const int64_t server_id)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "find peer id failed:server[%ld], index[%ld]", server_id, index);
  }
  else
  {
...
{
  clear(yysys::CTimeUtil::getTime());
  YYSYS_LOG(DEBUG, "print all counter:count[%ld]", cur_count_);
  for (int64_t i = 0; i < cur_count_; ++i)
  {
    YYSYS_LOG(INFO, "access counter info:server[%ld], counter[%ld]",
