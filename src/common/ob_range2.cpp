...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObNewRange::trim(const ObNewRange& r, ObStringBuf & string_buf)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "table id is not equal table_id_[%ld] r.table_id_[%ld]",
      ...
      if(OB_SUCCESS == ret)
      {
        if( 0 == compare_with_startkey2(r) ) // left interval equal
        {
          ...
          else
          {
            ...
            {
              ret = OB_ERROR;
              YYSYS_LOG(WARN, "r should be included by this range");
            }
          }
        }
    ...
    bool ObNewRange::check(void) const
    {
      ...
      {
        YYSYS_LOG(WARN, "check start key gt than end key, start_key=%s, end_key=%s",
      ...
      else if (start_key_ == end_key_)
      {
        if (!start_key_.is_min_row() && !end_key_.is_max_row())
        {
          ...
          {
            YYSYS_LOG(WARN, "check border flag or length failed:length[%ld], flag[%u]",
    ...
    int ObNewRange::to_string_memcpy(char *buffer, const int64_t length, int64_t &pos) const
    {
      int ret = OB_SUCCESS;
      if (pos < length)
      {
        databuff_printf(buffer, length, pos, "table_id:%lu,%s", table_id_, border_flag_.inclusive_start() ? "[" : "(");
        start_key_.to_string_memcpy(buffer, length, pos);
        databuff_printf(buffer, length, pos, "; ");
        end_key_.to_string_memcpy(buffer, length, pos);
        databuff_printf(buffer, length, pos, "%s", border_flag_.inclusive_end() ? "]" : ")");
      }
      return ret;
    }
    ...
    int ObNewRange::deserialize(const char* buf, const int64_t data_len, int64_t& pos)
    {
      ...
      {
        YYSYS_LOG(WARN, "start_key or end_key ptr is NULL, start_key=%s, end_key=%s",
                  to_cstring(start_key_), to_cstring(end_key_));
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to deserialize start key, ret=%d, buf=%p, data_len=%ld, pos=%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to deserialize end key, ret=%d, buf=%p, data_len=%ld, pos=%ld",
                    ret, buf, data_len, pos);
        }
      }
