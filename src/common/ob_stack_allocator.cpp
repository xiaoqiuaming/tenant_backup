...
namespace oceanbase
{
  namespace common
  {
    ...
    DefaultBlockAllocator::~DefaultBlockAllocator()
    {
      if (allocated_ != 0)
      {
        YYSYS_LOG(WARN, "allocated_[%ld] != 0", allocated_);
      }
    }
    ...
    int DefaultBlockAllocator::set_limit(const int64_t limit)
    {
      ...
      {
        limit_ = limit;
        YYSYS_LOG(INFO, "block_allocator.set_limit(%ld)", limit );
      }
      return err;
    }
...
//add support for arm platform by wangd 202106:e
      ...
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(ERROR, "allocated[%ld] + size[%ld] > limit[%ld]", allocated_, size, limit_);
      }
      else if (__sync_add_and_fetch(&allocated_, alloc_size) > limit_)
      {
        err = OB_MEM_OVERFLOW;
        __sync_add_and_fetch(&allocated_, -alloc_size);
        YYSYS_LOG(ERROR, "allocated[%ld] + size[%ld] > limit[%ld]", allocated_, alloc_size, limit_);
      }
      else if (NULL == (p = ob_tc_malloc(alloc_size, mod_)))
      {
        err = OB_MEM_OVERFLOW;
        __sync_add_and_fetch(&allocated_, -alloc_size);
        YYSYS_LOG(ERROR, "ob_tc_malloc(size=%ld) failed", alloc_size);
      }
      else
      {
      ...
      (void)err; //add for [build warning]
      return p;
    }
    ...
    void DefaultBlockAllocator::free(void* p)
    {
      ...
      else if (__sync_add_and_fetch(&allocated_, -alloc_size) < 0)
      {
        ...
        YYSYS_LOG(ERROR, "free(size=%ld): allocated[%ld] < 0 after free", alloc_size, allocated_);
      }
      else
      {
    ...
    int StackAllocator::clear(const bool slow)
    {
      ...
      {
        YYSYS_LOG(ERROR, "restore_top(0)=>%d", err);
      }
      else if (OB_SUCCESS != (err = set_reserved_block(NULL)))
      {
        YYSYS_LOG(ERROR, "set_reserved_block()=>%d", err);
      }
      return err;
    }
    ...
    int StackAllocator::alloc_block(Block*& block, const int64_t size)
    {
      ...
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(WARN, "allocator->alloc(size=%ld)=>NULL", size);
      }
      else if (OB_SUCCESS != (err = block->init(limit)))
      {
        YYSYS_LOG(ERROR, "block->init()=>%d", err);
      }
      return err;
    }
    ...
    int StackAllocator::free_block(Block* block)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "free_block(block=NULL)");
      }
      else
      {
...
//add support for arm platform by wangd 202106:b
#if defined(__aarch64__)
      int64_t size_align = (size + 7) & ~(7);
#else
      int64_t size_align = size;
#endif
//add support for arm platform by wangd 202106:e
       if (new_block->remain() < size_align
          && OB_SUCCESS != (err = alloc_block(new_block, size_align)))//mod support for arm platform by wangd 202106
      {
        YYSYS_LOG(WARN, "allocator->alloc(size_align=%ld)=>%d", size_align, err);//mod support for arm platform by wangd 202106
      }
      else
      {
    ...
    int StackAllocator::free_head()
    {
      ...
      {
        YYSYS_LOG(ERROR, "free_block()=>%d", err);
      }
      return err;
    }
    ...
    int StackAllocator::set_reserved_block(Block* block)
    {
      ...
      {
        YYSYS_LOG(ERROR, "free(%p)=>%d", reserved_, err);
      }
      else
      {
        reserved_ = block;
...
#endif
//add support for arm platform by wangd 202106:e
      if (OB_SUCCESS != (err = alloc_block(new_block, size_align)))//mod support for arm platform by wangd 202106
      {
        YYSYS_LOG(WARN, "allocator->alloc(size_align=%ld)=>%d", size_align, err);//mod support for arm platform by wangd 202106
      }
      else if (OB_SUCCESS != (err = set_reserved_block(new_block)))
      {
        YYSYS_LOG(ERROR, "set_reserved_block(new_block=%p)=>%d", new_block, err);
      }
      if (OB_SUCCESS != err && OB_SUCCESS != (err = free_block(new_block)))
      {
        YYSYS_LOG(ERROR, "free_block(new_block=%p)=>%d", new_block, err);
      }
      return err;
    }
...
#endif
//add support for arm platform by wangd 202106:e
      if (head_->remain() >= size_align || reserved_->remain() >= size_align)//mod support for arm platform by wangd
      {}
      else if (OB_SUCCESS != (err = reserve_block(size_align)))//mod support for arm platform by wangd
      {
        YYSYS_LOG(ERROR, "reserve_block(size=%ld)=>%d", size_align, err);//mod support for arm platform by wangd
      }
      return err;
    }
...
#endif
//add support for arm platform by wangd 202106:e
      if (size_align <= 0)//mod support for arm platform by wangd
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "alloc(size_align=%ld): INVALID_ARGUMENT", size_align);//mod support for arm platform by wangd
      }
      else if (head_->remain() < size_align && OB_SUCCESS != (err = alloc_head(size_align)))//mod support for arm platform by wangd
      {
        YYSYS_LOG(ERROR, "alloc_block(size_align=%ld)=>%d", size_align, err);//mod support for arm platform by wangd
      }
      else
      {
        p = (char*)head_ + head_->pos_;
        head_->pos_ += size_align;//mod support for arm platform by wangd
        top_ += size_align;//mod support for arm platform by wangd
      }
      return p;
    }
    ...
    int StackAllocator::start_batch_alloc()
    {
      ...
      {
        YYSYS_LOG(ERROR, "save_top(top=%ld)=>%d", top_, err);
      }
      return err;
    }
    ...
    int StackAllocator::end_batch_alloc(const bool rollback)
    {
      ...
      {
        YYSYS_LOG(ERROR, "restor_top(saved_top=%ld)=>%d", saved_top_, err);
      }
      else
      {
        saved_top_ = -1;
    ...
    int StackAllocator::restore_top(const int64_t top, const bool slow)
    {
      ...
      while(OB_SUCCESS == err && top_ > top)
      {
        ...
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "restor_top(top=%ld): head_ == NULL", top);
        }
        ...
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(ERROR, "restore_top(head->pos_=%ld, top=%ld, top_=%ld)=>%d", head_->pos_, top, top_, err);
        }
        else
        {
          head_->pos_ -= top_ - top;
...
//add support for arm platform by wangd 202106:e
      ...
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(ERROR, "get_tsi_allocator() failed");
      }
      else
      {
        err = allocator->reserve(size_align);//mod support for arm platform by wangd 202106
      }
      return err;
    }
...
//add support for arm platform by wangd 202106:e
      ...
      else if (NULL == (p = allocator->alloc(size_align)))//mod support for arm platform by wangd 202106
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(ERROR, "allocator->alloc(%ld)=>NULL", size_align);//mod support for arm platform by wangd 202106
      }
      (void)err; //add for [build warning]
      return p;
    }
