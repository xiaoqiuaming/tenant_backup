...
namespace oceanbase
{
  namespace common
  {
      ...
      mutator_buf_(NULL)
    {
      ...
      is_stat_end_ = false;      //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
    }
    ...
    int ObMutator :: clear()
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to clear str_buf or page_arena, err=%d", err);
      }
      is_stat_end_ = false;      //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
      return err;
    }
    ...
    int ObMutator :: reset()
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to reset str_buf or page_arena, err=%d", err);
      }
      return err;
    }
    ...
    int ObMutator :: use_ob_sem()
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add ob sem op, err=%d", err);
      }
    ...
    int ObMutator :: use_db_sem()
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add db sem op, err=%d", err);
      }
                            ...
                            const ObString& column_name, const ObObj& value, const int return_flag)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add mutation, err=%d", err);
      }
    ...
    int ObMutator :: del_row(const ObString& table_name, const ObRowkey& row_key)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add del op, err=%d", err);
      }
    ...
    int ObMutator :: trun_tab(const uint64_t table_id, const common::ObString & table_name, const common::ObString & user_name, const common::ObString & comment)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add trun op, err=%d", err);
      }
      ObRowkey rowkey;
      ObObj value[2];
      value[0].set_precise_datetime(yysys::CTimeUtil::getTime());
    ...
    int ObMutator :: ups_trun_tab(const uint64_t table_id, const bool force_finish)
    {
      ...
      //modify for [merge not ending after truncate]-b
      //      err = add_cell(mutation,CHANGED,ROWKEY_BARRIER,OB_DML_UNKNOW);
      err = add_cell(mutation,CHANGED,NO_BARRIER,OB_DML_UNKNOW);
      //modify for [merge not ending after truncate]-e
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to add del op, err=%d", err);
      }
    ...
    //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
    int ObMutator::add_stat_barrier()
    {
      ObMutatorCellInfo cell;
      return add_cell(cell,CHANGED,STATEMENT_BARRIER,OB_DML_UNKNOW);
    }
    // add by e
                              ...
                              const ObDmlType dml_type)
    {
      ...
      if (NULL == cur_node)
      {
        YYSYS_LOG(WARN, "failed to alloc, cur_node=%p", cur_node);
        err = OB_MEM_OVERFLOW;
      }
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to copy cell, err=%d", err);
        }
        else
        {
          cur_node->next = NULL;
    ...
    int ObMutator :: next_cell()
    {
      ...
      if (OB_SUCCESS != (err = next_cell_())
          && OB_ITER_END != err)
      {
        YYSYS_LOG(ERROR, "next_cell_()=>%d", err);
      }
      ...
      else if (NULL == cur_iter_node_)
      {
        ...
        YYSYS_LOG(ERROR, "cur_iter_node = NULL");
      }
      else if (NO_BARRIER == cur_iter_node_->barrier_flag)
      {} // do nothing
      else
      {
        //YYSYS_LOG(INFO, "skip dml_type_cell: type=%d", cur_iter_node_->dml_type);
        //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
        //          err = next_cell_();
        while (OB_SUCCESS ==err && !is_stat_end_)
        {
          if(cur_iter_node_->barrier_flag == STATEMENT_BARRIER)
          {
            is_stat_end_ = true;
          }
          YYSYS_LOG(DEBUG,"barried = %d,node = %p",cur_iter_node_->barrier_flag,cur_iter_node_);
          err = next_cell_();
          if(NULL == cur_iter_node_ || NO_BARRIER== cur_iter_node_->barrier_flag )
          {
            break;
          }
        }
        if(cur_iter_node_ !=NULL)
        {
          YYSYS_LOG(DEBUG,"barried = %d,node = %p",cur_iter_node_->barrier_flag,cur_iter_node_);
        }
        //add by maosy e
    ...
    int ObMutator :: next_cell_()
    {
      ...
      else if (NULL == cur_iter_node_->next || cur_iter_node_->next->barrier_flag ==STATEMENT_BARRIER)
        // add by maosy e
      {
    ...
    int ObMutator :: get_cell(ObMutatorCellInfo** cell, bool* is_row_changed, bool* is_row_finished, ObDmlType *dml_type)
    {
      ...
      if (NULL == cell)
      {
        YYSYS_LOG(WARN, "invalid param, cell=%p", cell);
        err = OB_INVALID_ARGUMENT;
      }
      else if (NULL == cur_iter_node_)
      {
        YYSYS_LOG(WARN, "invalid status, cur_iter_node=%p", cur_iter_node_);
        err = OB_ERROR;
      }
      else
      {
    ...
    int ObMutator :: pre_serialize()
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "pre_serialize(mutator_buf=%p)", mutator_buf_);
      }
      else if (NULL == (buf = page_arena_.alloc(size)))
      {
        err = OB_MEM_OVERFLOW;
        YYSYS_LOG(ERROR, "page_arena.alloc(%ld)=>%d", size, err);
      }
      else if (OB_SUCCESS != (err = serialize(buf, size, pos)))
      {
        page_arena_.free(buf);
        YYSYS_LOG(ERROR, "serialize(%p[%ld])=>%d", buf, size, err);
      }
      else
      {
        mutator_buf_ = buf;
    ...
    int ObMutator :: serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, buf=%p, buf_len=%ld, pos=%ld", buf, buf_len, pos);
        err = OB_INVALID_ARGUMENT;
      }
      else if (NULL != mutator_buf_)
      {
        ...
        if (pos + size > buf_len)
        {
          ...
          YYSYS_LOG(WARN, "pos[%ld] + size[%ld] > buf_len[%ld]", pos, size, buf_len);
        }
        else
        {
      ...
      else
      {
        ...
        while (OB_SUCCESS == err && NULL != node)
        {
          if (NO_BARRIER != node->barrier_flag)
          {
            ...
            //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
            if(STATEMENT_BARRIER & node->barrier_flag)
            {
              YYSYS_LOG(DEBUG,"before serialize");
              err = serialize_flag_(buf,buf_len,pos,ObActionFlag::OB_STATEMENT_BARRIER);
            }
            // add by maosy e
            node = node->next;
            continue;
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to serialize, buf=%p, buf_len=%ld, ori_pos=%ld, pos=%ld, err=%d",
    ...
    int ObMutator :: deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to reset, err=%d", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        while (OB_SUCCESS == err && pos < buf_len && !end_flag)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to deserialize, err=%d", err);
          }
          else
          {
            int64_t ext_val = 0;
            ...
            if (ObExtendType != type)
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid obj type, obj_type=%ld", type);
                err = OB_ERROR;
              }
              ...
              switch (ext_val)
              {
                ...
                case ObActionFlag::MUTATOR_TYPE_FIELD:
                  ...
                  if (OB_SUCCESS == err)
                  {
                    ...
                    if (OB_SUCCESS != err)
                    {
                      YYSYS_LOG(WARN, "failed to get int for mutator type, err=%d", err);
                    }
                    else
                    {
                ...
                case ObActionFlag::OBDB_SEMANTIC_FIELD:
                  ...
                  if (OB_SUCCESS == err)
                  {
                    ...
                    if (OB_SUCCESS != err)
                    {
                      YYSYS_LOG(WARN, "failed to add_cell, err=%d", err);
                    }
                  }
                  break;
                ...
                case ObActionFlag::TABLE_NAME_FIELD:
                  ...
                  if (OB_SUCCESS == err)
                  {
                    ...
                    {
                      YYSYS_LOG(WARN, "invalid obj type, type=%d", tmp_obj.get_type());
                      err = OB_ERROR;
                    }
                    ...
                    /*
                    YYSYS_LOG(INFO, "get_rowkey_info, table_id=%lu, table_name=%s, rowkey_info_size=%ld, err=%d, schema_manager_=%p",
                ...
                case ObActionFlag::ROW_KEY_FIELD:
                  ...
                  {
                    YYSYS_LOG(WARN, "rowkey_info should be set, rowkey_info_size=%ld", rowkey_info_.get_size());
                    err = OB_NOT_SUPPORTED;
                  }
                  else
                  {
                    ...
                    if (OB_SUCCESS != err)
                    {
                      YYSYS_LOG(WARN, "binary_rowkey_to_obj_array_fail err=%d binary=[%s]", err, to_cstring(tmp_str));
                    }
                    else
                    {
                ...
                case ObActionFlag::UPDATE_COND_PARAM_FIELD:
                  err = OB_NOT_SUPPORTED;
                  YYSYS_LOG(ERROR, "not support UPDATE_COND_PARAM_FIELD[%ld]", ext_val);
                  break;
                case ObActionFlag::PREFETCH_PARAM_FIELD:
                  err = OB_NOT_SUPPORTED;
                  YYSYS_LOG(ERROR, "not support PREFETCH_PARAM_FIELD[%ld]", ext_val);
                  break;
                ...
                case ObActionFlag::DML_TYPE_FIELD:
                  ...
                  break;
                  //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
                case ObActionFlag::OB_STATEMENT_BARRIER:
                  YYSYS_LOG(DEBUG,"deserialize ");
                  err = add_stat_barrier();
                  break;
                  // add by maosy
                default:
                  YYSYS_LOG(WARN, "unknown format, omit it:type[%ld]", ext_val);
                  do
                  {
                    int64_t old_pos = pos;
                    do
                                ...
                                RowChangedStat row_changed_stat, int64_t& store_size)
    {
      ...
      else
      {
        ...
        if (0 != src_cell.cell_info.table_name_.length())
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid status, should use name");
            err = OB_ERROR;
          }
          else
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "failed to store table name, err=%d", err);
              }
              else
              {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "invalid status, should use id");
            err = OB_ERROR;
          }
          else
          {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to store row key, err=%d", err);
            }
            else
            {
        ...
        else
        {
          ...
          if (0 != src_cell.cell_info.column_name_.length())
          {
            ...
            else
            {
              ...
              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "failed to store column name, err=%d", err);
              }
              else
              {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to store column value, err=%d", err);
            }
            else
            {
    ...
    int ObMutator :: add_node_(CellInfoNode* cur_node)
    {
      ...
      if (NULL == cur_node)
      {
        YYSYS_LOG(WARN, "invalid param, cur_node=%p", cur_node);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        else
        {
          ...
          //add by maosy [MultiUps 1.0] [secondary index optimize] 20170722 b:
          if(STATEMENT_BARRIER == cur_node->barrier_flag)
          {
            list_tail_->row_finished_stat =FINISHED;
          }
          // add by maosy e
