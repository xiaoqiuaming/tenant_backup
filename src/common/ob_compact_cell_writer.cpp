...
int ObCompactCellWriter::append_row(const ObRowkey& rowkey, const ObRow& row)
{
  ...
  {
    YYSYS_LOG(WARN, "append rowkey error:ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret = append_rowvalue(row)))
  {
    YYSYS_LOG(WARN, "append rowvalue error:ret=%d", ret);
  }
  else
  {
    //do nothing
...
int ObCompactCellWriter::append_row(const ObRow& row)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid row column count: obj_array_cnt=[%ld]", obj_array_cnt);
    ret = OB_ERROR;
  }
  else if (NULL == obj_array_ptr)
  {
    YYSYS_LOG(WARN, "obj array ptr is NULL");
    ret = OB_ERROR;
  }
  else if (NULL == row_desc)
  {
    YYSYS_LOG(WARN, "row desc is NULL");
    ret = OB_ERROR;
  }
  else if (NULL == (desc_array_ptr = row_desc->get_cells_desc_array(desc_array_cnt)))
  {
    YYSYS_LOG(WARN, "desc array ptr is NULL");
    ret = OB_ERROR;
  }
  else if (0 == desc_array_cnt && desc_array_cnt > OB_ROW_MAX_COLUMNS_COUNT)
  {
    YYSYS_LOG(WARN, "invalid desc array cnt: desc_array_cnt=[%ld]", desc_array_cnt);
    ret = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "invalid rowkey column num: rowkey_column_num=[%ld]", rowkey_column_num);
      ret = OB_ERROR;
    }
  }
  ...
  for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey_column_num; i ++)
  {
    ...
    {
      YYSYS_LOG(WARN, "append rowkey faile: ret=[%d], obj=[%s]", ret, to_cstring(obj_array_ptr[i]));
      break;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "add rowkey finish flag fail: ret=[%d]", ret);
  }
  ...
  for(int64_t i = rowkey_column_num; OB_SUCCESS == ret && i < obj_array_cnt; i ++)
  {
    if (DENSE_SPARSE == store_type_)
    {
      ...
      {
        YYSYS_LOG(WARN, "append fail: i=[%ld], ret=[%d], column_id=[%lu], obj=[%s]",
    ...
    else if (DENSE_DENSE == store_type_)
    {
      if (OB_SUCCESS != (ret = append(obj_array_ptr[i])))
      {
        YYSYS_LOG(WARN, "append fail: i=[%ld], ret=[%d], obj=[%s]", i, ret, to_cstring(obj_array_ptr[i]));
      }
    }
    else
    {
      YYSYS_LOG(WARN, "unsupport store type: i=[%ld], store_type_=[%d]", i, store_type_);
    }
  }

  if (OB_SUCCESS == ret && OB_SUCCESS != (ret = row_finish()))
  {
    YYSYS_LOG(WARN, "row finish error: ret=[%d]", ret);
  }
...
int ObCompactCellWriter::append_rowvalue(const ObRow& row)
{
  ...
  for (int64_t i = 0; i < column_num; i ++)
  {
    ...
    {
      YYSYS_LOG(WARN, "raw get cell fail:ret=%d", ret);
      break;
    }
    else
    {
      if (DENSE_SPARSE == store_type_)
      {
        ...
        {
          YYSYS_LOG(WARN, "append fail:ret=%d", ret);
          break;
        }
      }
      else if (DENSE_DENSE == store_type_)
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "finish row error:ret=%d", ret);
    }
    else
    {
      //do nothing
...
int ObCompactCellWriter::init(char *buf, int64_t size, enum ObCompactStoreType store_type)
{
  ...
  if(NULL == buf)
  {
    ...
    YYSYS_LOG(WARN, "buf is null");
  }
  else if(OB_SUCCESS != (ret = buf_writer_.assign(buf, size)))
  {
    YYSYS_LOG(WARN, "buf write assign fail:ret[%d]", ret);
  }
  else
  {
...
int ObCompactCellWriter::write_decimal(const ObObj &decimal)
{
  ...
  {
    YYSYS_LOG(WARN, "write dec vscale fail:ret[%d]", ret);
  }
  else if(OB_SUCCESS != (ret = buf_writer_.write<ObDecimalMeta>(dec_meta)))
  {
    YYSYS_LOG(WARN, "write dec_meta fail:ret[%d]", ret);
  }
  else
  {
    if(nwords <= 3)
    ...
    for(uint16_t i=0;OB_SUCCESS == ret && i<=dec_meta.dec_nwords_;i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "write words fail:ret[%d]", ret);
      }
    }
  }
...
//modify for [369-full_join]-b
//int ObCompactCellWriter::write_decimal(const ObObj &decimal, ObObj *clone_value)
int ObCompactCellWriter::write_decimal(const ObObj &decimal, ObObj *clone_value, bool need_copy)
//modify for [369-full_join]-e
{
  int ret = OB_SUCCESS;
  ...
  if(OB_SUCCESS != (ret = decimal.get_ttint(src_val)))
  {
  ...
  else
  {
    ...
    else
    {
      ...
      if(OB_SUCCESS == ret)
      {
        ret = buf_writer_.write_Decimal(src_val, dst_val, decimal.get_nwords());
      }
      if(OB_SUCCESS == ret && NULL != dst_val && clone_value !=NULL && need_copy)
      {
        //ret = buf_writer_.write_varchar(str_deci, &str2);
        clone_value->set_decimal(dst_val, decimal.meta_.dec_precision_, decimal.meta_.dec_scale_,
                                 decimal.meta_.dec_vscale_, decimal.meta_.dec_nwords_);
        // YYSYS_LOG(WARN, "write_decimal=%.*s", str2.length(),str2.ptr());
...
//add for [369-full join]-b
int ObCompactCellWriter::write_varchar_opt(const ObObj &value, ObObj *clone_value, bool need_copy)
{
  int ret = OB_SUCCESS;
  ...
  if(varchar_value.length() > INT32_MAX)
  {
    ...
    YYSYS_LOG(WARN, "varchar is too long:[%d]", varchar_value.length());
  }
  if(OB_SUCCESS == ret)
  {
  ...
  if(OB_SUCCESS == ret && NULL != clone_value && need_copy)
  {
    clone_value->set_varchar(varchar_written);
  }
  return ret;
}
//add for [369-full join]-e
    ...
    switch(value.get_type())
    {
      ...
      case ObExtendType:
        ...
        if (OB_SUCCESS == value.get_ext(tmp_value))
        {
          ...
          {
            YYSYS_LOG(WARN, "extend type error:tmp_value=%ld", tmp_value);
            ret = OB_ERROR;
          }
        }
        if (OB_SUCCESS == ret)
        ...
        break;
      default:
        YYSYS_LOG(WARN, "unsupported type:type[%d]", value.get_type());
  ...
  if(OB_SUCCESS == ret)
  {
    if(column_id == OB_INVALID_ID)
    {
      if(SPARSE == store_type_)
      {
        ...
        YYSYS_LOG(WARN, "cannot append column_id OB_INVALID_ID");
      }
    }
    else if(column_id > UINT32_MAX)
    {
      ret = OB_SIZE_OVERFLOW;
      YYSYS_LOG(WARN, "column_id is too long:%lu", column_id);
    }
    else
    {
...
int ObCompactCellWriter::append_escape(int64_t escape)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "escape is invalid:[%ld]", escape);
  }
  else
  {
...
//add for [369-full join]-b
int ObCompactCellWriter::append_opt(uint64_t column_id, const ObObj &value, const bool &need_copy, ObObj *clone_value)
{
  int ret = OB_SUCCESS;
  ObCellMeta cell_meta;
  cell_meta.attr_ = ObCellMeta::AR_NORMAL;
  ...
  if(OB_SUCCESS == ret)
  {
    switch(value.get_type())
    {
      ...
      case ObDecimalType:
        ...
        if(OB_SUCCESS == ret)
        {
          ret = write_decimal(value, clone_value, need_copy);
        }
        break;
      case ObFloatType:
        cell_meta.type_ = ObCellMeta::TP_FLOAT;
      ...
      case ObVarcharType:
        ...
        if(OB_SUCCESS == ret)
        {
          ret = write_varchar_opt(value, clone_value, need_copy);
        }
        break;
      case ObCreateTimeType:
        cell_meta.type_ = ObCellMeta::TP_CREATE_TIME;
