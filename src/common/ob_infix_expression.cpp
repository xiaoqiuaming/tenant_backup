...
namespace oceanbase
{
  namespace common
  {
    DEFINE_SERIALIZE(ObInfixExpression)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "infix expression serialization error");
      }
      return err;
    }
    ...
    DEFINE_DESERIALIZE(ObInfixExpression)
    {
      ...
      else if (ObVarcharType != obj.get_type())
      {
        ...
        YYSYS_LOG(WARN, "unexpected infix expression element type. ObVarcharType expected"
            "[type:%d]", obj.get_type());
      }
      else if (OB_SUCCESS != (err = obj.get_varchar(expr_)))
      {
        YYSYS_LOG(WARN, "get varchar value failed [err:%d]", err);
      }
      return err;
    }
