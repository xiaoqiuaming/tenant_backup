...
namespace oceanbase
{
  namespace common
  {
    ...
    struct ModulePageAllocator: public ObIAllocator
    {
        ...
        { mod_id_ = mod_id; }
        void *alloc(const int64_t sz)
        { return (NULL == allocator_) ? ob_tc_malloc(sz, mod_id_) : allocator_->alloc(sz); }
        void free(void *p)
        { (NULL == allocator_) ? ob_tc_free(p, mod_id_) : allocator_->free(p); }
        void freed(const int64_t sz)
        { UNUSED(sz); /* mostly for effcient bulk stat reporting */ }
      private:
        int32_t mod_id_;
    ...
    class PageArena
    {
      ...
      private: // helpers
        ...
        Page * alloc_new_page(const int64_t sz)
        {
          ...
          {
            YYSYS_LOG(ERROR, "cannot allocate memory.sz=%ld, pages_=%ld,total_=%ld",
                      sz, pages_, total_);
          }
        ...
        Page * extend_page(const int64_t sz)
        {
          ...
          if (NULL != page)
          {
            ...
            else
            {
              ...
              if (NULL == page)
              {
                YYSYS_LOG(ERROR, "extend_page sz =%ld cannot alloc new page", sz);
              }
              insert_tail(page);
            }
          }
...
//add support for arm platform by wangd 202106:e
            if (NULL != cur_page_)
            {
              if (sz_align <= cur_page_->remain())//mod support for arm platform by wangd 202106
              {
                ret = cur_page_->alloc(sz_align);//mod support for arm platform by wangd 202106
              }
              else if (is_normal_overflow(sz_align))//mod support for arm platform by wangd 202106
              {
              ...
              {
                ret = alloc_big(sz_align);//mod support for arm platform by wangd 202106
              }

              if (NULL != ret)
              {
                used_ += sz_align;//mod support for arm platform by wangd 202106
              }
            }
            return ret;
          }
        ...
        T *new_object()
        {
          ...
          if (NULL == tmp)
          {
            YYSYS_LOG(WARN, "fail to alloc mem for T");
          }
          else
          {
            new(tmp)T();
...
//add support for arm platform by wangd 202106:e
            ...
            if (NULL != cur_page_)
            {
              if (sz_align <= cur_page_->remain())//mod support for arm platform by wangd 202106
              {
                ret = cur_page_->alloc_down(sz_align);//mod support for arm platform by wangd 202106
              }
              else if (is_normal_overflow(sz_align))//mod support for arm platform by wangd 202106
              {
                cur_page_ = extend_page(page_size_);
...
//add support for arm platform by wangd 202106:b
#if defined(__aarch64__)
            int64_t newsz_align = (newsz + 7) & ~(7);
#else
            int64_t newsz_align = newsz;
#endif
//add support for arm platform by wangd 202106:e
            // if we're the last one on the current page with enough space
            if (p + oldsz == cur_page_->alloc_end_
                && p + newsz_align  < cur_page_->page_end_)//mod support for arm platform by wangd 202106
            {
              cur_page_->alloc_end_ = (char *)p + newsz_align;//mod support for arm platform by wangd 202106
              ret = p;
            }
            else
            {
              ret = alloc(newsz_align);//mod support for arm platform by wangd 202106
              if (NULL != ret)
                ::memcpy(ret, p, newsz_align > oldsz ? oldsz : newsz_align);//mod support for arm platform by wangd 202106
            }
            return ret;
          }
...
//add support for arm platform by wangd 202106:b
#if defined(__aarch64__)
            int64_t len_align = (len + 7) & ~(7);
#else
            int64_t len_align = len;
#endif
//add support for arm platform by wangd 202106:e
            CharT *copy = alloc(len_align);//mod support for arm platform by wangd 20210
            if (NULL != copy)
              memcpy(copy, s, len_align);//mod support for arm platform by wangd 20210
            return copy;
          }
...
#endif
//add support for arm platform by wangd 202106:e
              copy = alloc(len_align);//mod support for arm platform by wangd 20210
              if (NULL != copy)
                memcpy(copy, s, len_align);//mod support for arm platform by wangd 20210
            }
    ...
    class ObArenaAllocator: public ObIAllocator
    {
      ...
      public:
        ...
        void reuse()
        { arena_.reuse(); };
        virtual void set_mod_id(int32_t mod_id)
        { UNUSED(mod_id); };
        void clear()
        { arena_.free(); };
      private:
...
#define ALLOC_OBJECT(allocator, obj, T, arg...) \
  ...
  if (NULL == tmp) \
    { \
  YYSYS_LOG(WARN, "alloc mem for %s", #T); \
  } \
  else \
    { \
  obj = new(tmp) T(##arg); \
