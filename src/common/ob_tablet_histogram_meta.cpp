...
namespace oceanbase
{
  namespace common
  {
    void ObTabletHistogramMeta::dump() const
    {
      ...
      {
        YYSYS_LOG(INFO, "server_info_index = %d, timestamp = %ld", report_cs_info_[i].server_info_index_, report_cs_info_[i].report_timestamp_);
      }
    }

    void ObTabletHistogramTable::dump() const
    {
      YYSYS_LOG(INFO, "begin dump hist table. meta_table_.size():%d", (int32_t)meta_table_.get_array_index());
      if (NULL != tablet_hist_manager_ && NULL != root_server_)
      {
        for (int32_t i = 0; i < meta_table_.get_array_index(); i++)
        {
          YYSYS_LOG(INFO, "dump the %d-th tablet info", i);
          root_server_->dump_root_table(data_holder_[i].root_meta_index_);
          YYSYS_LOG(INFO, "dump the %d-th tablet info end", i);

          YYSYS_LOG(INFO, "dump the %d-th histogram info", i);
          data_holder_[i].dump();
          tablet_hist_manager_->dump(data_holder_[i].hist_index_);
          YYSYS_LOG(INFO, "dump the %d-th histogram info end", i);
        }
      }
      YYSYS_LOG(INFO, "dump hist table complete");
    }
    ...
    int ObTabletHistogramTable::add_meta(const ObTabletHistogramMeta &meta)
    {
      ...
      {
        YYSYS_LOG(WARN, "too many histogram, max is %ld now is %ld", ObTabletHistogramManager::MAX_TABLET_COUNT_PER_TABLE, meta_table_.get_array_index());
        ret = OB_ARRAY_OUT_OF_RANGE;
      }
      else if (!meta_table_.push_back(meta))
      {
        YYSYS_LOG(WARN, "failed to push hist meta into meta_table_");
        ret = OB_ARRAY_OUT_OF_RANGE;
      }
      return ret;
    }
    ...
    int ObTabletHistogramTable::add_hist_meta(const ObTabletInfo &tablet_info, const ObTabletHistogram &hist, const int32_t server_index)
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "root_server_ or tablet_hist_manager_ is null.");
      }
      else if(OB_SUCCESS != (ret = tablet_hist_manager_->add_histogram(hist, hist_index)))
      {
        YYSYS_LOG(WARN, "add histogram to manager failed.");
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "can not get hist_meta.root_meta_ from root table.");
        }
        else if (OB_SUCCESS != (ret = add_meta(hist_meta)))
        {
          YYSYS_LOG(WARN, "add histogram meta to table failed.");
        }
    ...
    int ObTabletHistogramTable::add_hist_meta(const ObTabletInfo &tablet_info, const int32_t hist_index, const int32_t server_index, const uint64_t idx_tid, bool &is_repeat_report)
    {
      int ret = OB_SUCCESS;
      iterator find_pos = NULL;
      ObTabletHistogramMeta hist_meta;
      hist_meta.hist_index_ = hist_index;
      is_repeat_report = false;
      //      hist_meta.server_info_indexes_[0] = server_index;
      if (NULL == root_server_)
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "root_server_ is null.");
      }
      //add liumz, [check index tid consistency between hist table and reported sample]20151217:b
      else if (index_tid_ != idx_tid)
      {
        ret = OB_INVALID_DATA;
        YYSYS_LOG(WARN, "index tid is not consistent. index_tid_[%lu], sample's range tid[%lu]", index_tid_, idx_tid);
      }
      //add:e
      else if (NULL == (find_pos = find_tablet_pos(tablet_info)))
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "find_tablet_pos failed.");
      }
      //�µ�tablet_info, ��tablet��static indexͳ����Ϣ��δ�㱨
      else if (end() == find_pos)
      {
        //mod liumz, bugfix: �㱨һ��tablet��CS������OB_MAX_COPY_COUNTʱ, �µ�CS�㱨��Ϣ����д��hist table, 20150505:b
        //hist_meta.server_info_indexes_[0] = server_index;
        hist_meta.report_cs_info_[0].server_info_index_ = server_index;
        hist_meta.report_cs_info_[0].report_timestamp_ = yysys::CTimeUtil::getTime();
        //mod:e
      ...
      //��ͬ��tablet_info, ��tablet��static indexͳ����Ϣ֮ǰ�ѻ㱨��
      else
      {
        is_repeat_report = true;
        //find_pos->hist_index_Ҫ������
          ...
          //mod:e
        {
          ...
          find_pos->report_cs_info_[server_index_pos].report_timestamp_ = yysys::CTimeUtil::getTime();
          //mod:e
        }
      }
      return ret;
    ...
    int ObTabletHistogramTable::check_reported_hist_info(const ObTabletInfo *compare_tablet, bool &is_finished) const
    {
      ...
      {
        ret = OB_INVALID_DATA;
        YYSYS_LOG(WARN, "compared tablet info is null.");
      }
      ...
      else if (meta_table_.get_array_index() > 0)
      {
        ...
        for (int64_t i = 0; i < meta_table_.get_array_index(); i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "get_rt_tablet_info failed. ret=%d", ret);
            break;
          }
          else if (NULL == tablet_info)
          {
            ret = OB_INVALID_DATA;
            YYSYS_LOG(WARN, "tablet info is null. ret=%d", ret);
            break;
          }
    ...
    ObTabletHistogramTable::iterator ObTabletHistogramTable::find_tablet_pos(const ObTabletInfo &tablet_info)
    {
      ...
      else
      {
        for (find_pos = begin(); find_pos != end(); find_pos++)
        {
          ...
          {
            find_pos = NULL;
            YYSYS_LOG(WARN, "get_rt_tablet_info failed.");
            break;
          }
          else if (NULL == compare_tablet)
          {
            find_pos = NULL;
            YYSYS_LOG(WARN, "tablet info is null.");
            break;
          }
    ...
    int32_t ObTabletHistogramTable::find_suitable_pos(const_iterator it, const int32_t server_index) const
    {
      ...
      if (NULL == it)
      {
        YYSYS_LOG(WARN, "invalid parameter");
      }
      else
      {
        //add:e
        ...
        //add liumz, bugfix: �㱨һ��tablet��CS������OB_MAX_COPY_COUNTʱ, �µ�CS�㱨��Ϣ����д��hist table, 20150505:b
        if (OB_INVALID_INDEX == found_it_index)
        {
    ...
    int ObTabletHistogramTable::fill_all_ranges()
    {
      ...
      {
        ret = OB_NOT_INIT;
        YYSYS_LOG(WARN, "tablet_hist_manager_ is null");
      }
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "before alloc srw object, count[%ld]", tablet_hist_manager_->get_histogram_size()*(ObTabletHistogram::MAX_SAMPLE_BUCKET + 2));
        ...
        for (; it != end() && OB_LIKELY(OB_SUCCESS == ret); it++)
        {
          ...
          {
            YYSYS_LOG(WARN, "get histogram failed. hist index = %d", it->hist_index_);
          }
          else
          {
            for (int64_t i = 0; i < p_hist->get_sample_count() && OB_LIKELY(OB_SUCCESS == ret); i++)
            {
              ...
              {
                YYSYS_LOG(WARN, "get sample from histogram failed. sample index = %ld", i);
              }
              //add liumz, check if p_sample is null, 20150428:b
              else if (NULL == p_sample)
              {
                ret = OB_ERROR;
                YYSYS_LOG(WARN, "p_sample is NULL");
              }
              //add:e
              //              else if (NULL == (l_srw = alloc_srw_object()) || NULL == (r_srw = alloc_srw_object()))
              //              {
              //                ret = OB_MEM_OVERFLOW;
              //                YYSYS_LOG(ERROR, "alloc memory for srw failed.");
              //              }
              else
              {
                ...
                /*
                  ...
                  {
                    ret = OB_ARRAY_OUT_OF_RANGE;
                    YYSYS_LOG(WARN, "too many samples in one table, fill all ranges failed.");
                  }
                  else if (OB_SUCCESS != (ret = srw_array_.push_back(l_srw)) || OB_SUCCESS != (ret = srw_array_.push_back(r_srw)))
                  */
                //                YYSYS_LOG(DEBUG, "l_srw.rowkey_[%s], r_srw.rowkey_[%s]", to_cstring(l_srw.rowkey_), to_cstring(r_srw.rowkey_));
                ...
                //                  YYSYS_LOG(DEBUG, "l_srw_2.rowkey_[%s], r_srw_2.rowkey_[%s]", to_cstring(l_srw_2.rowkey_), to_cstring(r_srw_2.rowkey_));
                //                if (OB_SUCCESS != (ret = sorted_srw_list_.insert_unique(l_srw, insert_pos, compare_srw, unique_srw)))
                //                {
                //                  YYSYS_LOG(DEBUG, "can not insert left srw into sorted_srws_");
                //                }
                //                if (OB_CONFLICT_VALUE == ret)
                //                {
                //                  ret = OB_SUCCESS;
                //                  YYSYS_LOG(DEBUG, "duplicate sample range.");
                //                }
                //                if (OB_SUCCESS == ret && OB_SUCCESS != (ret = sorted_srw_list_.insert_unique(r_srw, insert_pos, compare_srw, unique_srw)))
                //                {
                //                  YYSYS_LOG(DEBUG, "can not insert right srw into sorted_srws_");
                //                }
                //                if (OB_CONFLICT_VALUE == ret)
                //                {
                //                  ret = OB_SUCCESS;
        ...
        YYSYS_LOG(INFO, "before insert sorted list, srw_idx[%ld]", srw_idx);
        common::ObSortedVector<ObTabletSampleRangeWrapper *>::iterator insert_pos = NULL;
        ...
        YYSYS_LOG(INFO, "after insert sorted list, size[%d]", sorted_srw_list_.size());
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int32_t idx = 0; idx < sorted_srw_list_.size(); idx++)
        {
          ...
          {
            YYSYS_LOG(INFO, "rowkey[%s], idx[%d]", to_cstring(srw->rowkey_), idx);
            srw->hist_meta_->dump();
          }
        }
      }
    ...
    //mod:e
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "wrong parameters, sample_num=%ld, server_index=%p", sample_num, server_index);
      }
      else if (sorted_srw_list_.size() <= cur_srw_index_ || 0 > cur_srw_index_)
      {
        ret = OB_ARRAY_OUT_OF_RANGE;
        YYSYS_LOG(WARN, "index out of range, cur_srw_index_=%d, sorted_srw_list_.size()=%d", cur_srw_index_, sorted_srw_list_.size());
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "null srw, cur_srw_index_=%d", cur_srw_index_);
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          YYSYS_LOG(INFO, "test::liumz, begin_index=%d, end_index=%d, sorted_srw_list_.size()=%d", begin_index, end_index, sorted_srw_list_.size());
          //mod liumz, [paxos static index]20170626:b
    ...
    //mod:e
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "wrong parameters, begin_index=%d, end_index=%d, server_index=%p.", begin_index, end_index, server_index);
      }
      if (OB_SUCCESS == ret)
      {
        ...
        for (int32_t i = begin_index; i < end_index && server_index_idx < copy_count && OB_LIKELY(OB_SUCCESS == ret); i++)
        {
          ...
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "null srw, index=%d", i);
          }
          else if (NULL == sorted_srw_list_.at(i)->hist_meta_)
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "srw's hist_meta_ is null, srw index=%d", i);
          }
          else
          {
        ...
        for (int32_t i = begin_index; i < end_index && server_index_idx < copy_count && OB_LIKELY(OB_SUCCESS == ret); i++)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "get root meta by index[%d] failed, ret=%d", sorted_srw_list_.at(i)->hist_meta_->root_meta_index_, ret);
            }
            else if (NULL == meta_data)
            {
              YYSYS_LOG(WARN, "root meta is NULL.");
            }
            else
            {
              int32_t server_idx = OB_INVALID_INDEX;
