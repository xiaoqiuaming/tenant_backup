...
namespace oceanbase
{
  namespace common
  {
    ...
    static ObString table_rule_name = OB_STR(OB_ALL_TABLE_RULES_NAME);
    static ObString partition_rule_name = OB_STR(OB_ALL_PARTITION_RULES_NAME);
    //add 20150324:e
    //add lqc [MultiUps 1.0] [#13] 20170405 b
    static ObString all_all_group_name = OB_STR(OB_ALL_ALL_GROUP_NAME);
    //add e

    //[view]
    static ObString all_view_name = OB_STR(OB_ALL_VIEW_TABLE_NAME);

    static ObString table_name_str = OB_STR("table_id");

    static ObString expire_info = OB_STR("expire_condition");
    static ObString table_rule_str = OB_STR("table_name");
    ...
    class ObSchemaServiceImpl : public ObSchemaService
    {
      public:
        ...
        virtual int create_table(const TableSchema& table_schema,
                                 ...
                                 common::ObArray<ObString> *group_name_list = NULL);
        //add 20150423:e
        virtual int drop_table(const ObString& table_name, uint64_t new_table_id = OB_INVALID_ID);
        virtual int alter_table(const AlterTableSchema& table_schema, const int64_t old_schema_version, ObSchemaManagerV2 *schema_manager);
        ...
        virtual int modify_table_id(TableSchema& table_schema, const int64_t new_table_id, const int64_t frozen_version);
        ...
        virtual int prepare_privilege_for_table(const nb_accessor::TableRow* table_row,
            ObMutator *mutator, const int64_t table_id, const int64_t db_id);
        //add zhaoqiong [Schema Manager] 20150327:b
        ...
        //add e
        virtual int get_checksum_info(const ObNewRange new_range,const int64_t required_version, ObString& cchecksum);
      ...
      private:
        ...
        /*mod by wuna[MultiUps] [sql_api] 20160111*/
//        int check_function_schema(const TableSchema& table_schema);//add liu jun.[MultiUps_Live] 20150324
        template <class T>
        int check_function_schema(const T& table_schema, const TableSchema &old_table_schema);//[416]
        int check_param_type(const char* func_rule_body, const TableSchema &table_schema);
        //mod 20160111:e
        // for read
        ...
        //add e  
        //add liu jun.[MultiUPS] [sql_api] 20150423:b
        int create_table_mutator(const TableSchema& table_schema,
                                 ObMutator* mutator,
                                 int64_t frozen_version = OB_INVALID_FROZEN_VERSION,
                                 common::ObArray<int32_t> *paxos_id_array = NULL,
                                 common::ObArray<ObString> *group_name_list = NULL);
        //add 20150423:e
        int alter_table_mutator(const AlterTableSchema& table_schema, ObMutator* mutator, const int64_t old_schema_version, ObSchemaManagerV2 *schema_manager);
        //add fyd [NotNULL_check] [JHOBv0.1] 20140108:b
        int alter_table_mutator(const AlterTableSchema& table_schema, ObMutator* mutator, TableSchema& old_table_schema, ObSchemaManagerV2 *schema_manager);
        //add 20140108:e
        ...
        //[view]
        int assemble_view(const nb_accessor::TableRow* table_row, TableSchema& table_schema);
        ...
        int reset_index_expire_info_mutator(ObMutator* mutator, ObRowkey & rowkey, const AlterTableSchema &alter_schema);
        int reset_expire_info(ObMutator *mutator, const AlterTableSchema &alter_schema, const TableSchema &old_schema);
        int mod_datatype_mutator(ObMutator* mutator, ObRowkey & rowkey, ColumnSchema & column);
        int mod_precision_mutator(ObMutator* mutator, ObRowkey & rowkey, ColumnSchema & column);
      ...
      int generate_new_table_name(char* buf, const uint64_t lenght, const char* table_name, const uint64_t table_name_length);
        ...
        //add 20140108:e
         ...
         void add_all_all_group_comment(const ObString &function_name, char *comment, const int64_t pos);

         //add lqc [MultiUps 1.0] [#13] 20170405 b
         int update_all_all_group_mutator(const TableSchema& table_schema,
                                          ObMutator* mutator,
                                          int64_t &frozen_version,
                                          common::ObArray<int32_t> *paxos_id_array,
                                          common::ObArray<ObString> *group_name_list);
         int alter_all_all_group_mutator(const AlterTableSchema& schema,
                                         TableSchema& old_table_schema,
                                         int64_t frozen_version,
                                         common::ObArray<int32_t> *paxos_id_array,
                                         common::ObArray<ObString> *group_name_list);
         //add e      
         int alter_all_table_rules_mutator(const AlterTableSchema& schema,
                                         TableSchema& old_table_schema,
                                         int64_t frozen_version);

         int renew_default_column_mutator(ObMutator* mutator,ObRowkey & rowkey, const ColumnSchema & column);
         int mod_load_type_mutator(ObMutator* mutator, const AlterTableSchema &alter_schema, const TableSchema & old_schema);

         //[view]
         int add_all_view(ObMutator* mutator, const TableSchema &table_schema);
