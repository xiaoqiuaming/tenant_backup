  ...
  switch (get_type())
  {
    ...
    case ObDecimalType:
    {
      ...
      if (OB_SUCCESS != ret)
      {
        ...
        YYSYS_LOG(ERROR,"failed in covert decimal from buf");
      }
      else
      {
        ret = !od.is_zero();
...
int ObObj::set_decimal(const ObString& value)
{
  ...
  if(OB_SUCCESS!=(ret=od.from(value.ptr(),value.length())))
  {
    ...
    YYSYS_LOG(WARN, "invalid buffer for string to decimal,buffer =%.*s,",value.length(),value.ptr());
  }
  else
  {
    uint32_t p,s,v;
...
//add for [Decimal]-b
int ObObj::set_decimal(const ObString &value, uint32_t p, uint32_t s, uint32_t v, uint32_t len)
{
  int ret = OB_SUCCESS;
  meta_.type_ = ObDecimalType;
  value_.word = value.ptr();
...
void ObObj::set_ttint(const uint64_t *ttint)
{
  meta_.type_ = ObDecimalType;
  value_.ii = const_cast<uint64_t *>(ttint);
}

int ObObj::get_ttint(uint64_t *&res) const
{
  int ret = OB_OBJ_TYPE_ERROR;
  if (meta_.type_ == ObDecimalType)
  {
...
int ObObj::set_decimal(const uint64_t *od, uint32_t p, uint32_t s, uint32_t v, uint32_t len)
{
  int ret = OB_SUCCESS;
  meta_.type_ = ObDecimalType;
  meta_.dec_nwords_ = static_cast<uint8_t>(len) & META_NWORDS_MASK;
  ...
  meta_.dec_vscale_ = static_cast<uint8_t>(v) & META_VSCALE_MASK;
  meta_.op_flag_ = INVALID_OP_FLAG;
  value_.ii = const_cast<uint64_t *>(od);
  return ret;
}
...
int ObObj::set_decimal(const ObDecimal &od)
{
  ...
  meta_.dec_vscale_ = static_cast<uint8_t>(v) & META_VSCALE_MASK;
  meta_.op_flag_ = INVALID_OP_FLAG;
  value_.ii = const_cast<ObDecimal &>(od).get_words()->ToUInt_v2();

  meta_.dec_nwords_ = 1;
  if (const_cast<ObDecimal &>(od).get_words()->table[1] != 0)
  {
    meta_.dec_nwords_ = 2;
  }
  return ret;
...
int ObObj::get_decimal(ObDecimal *value) const
{
  int res = OB_OBJ_TYPE_ERROR;
  if (meta_.type_ == ObDecimalType)
  {
...
int ObObj::compare(const ObObj &other) const
{
  ...
  {
    //modify for [obj type error]-b
    int64_t int_value = 0;
    if ((this_type == ObIntType && OB_SUCCESS == this->get_int(int_value))
        && (int_value < INT_MIN || int_value > INT_MAX))
    {
      YYSYS_LOG(WARN, "can not be compared, this_type=%d other_type=%d", get_type(), other.get_type());
    }
    else if ((other_type == ObIntType && OB_SUCCESS == other.get_int(int_value))
             && (int_value < INT_MIN || int_value > INT_MAX))
    {
      YYSYS_LOG(WARN, "can not be compared, this_type=%d other_type=%d", get_type(), other.get_type());
    }
    ...
    {
      YYSYS_LOG(ERROR, "can not be compared, this_type=%d other_type=%d", get_type(), other.get_type());
    }
...
int ObObj::apply(const ObObj &mutation)
{
  ...
  int org_ext = static_cast<int>(get_ext());
  ...
  if (ObSeqType == mut_type
      ...
      || ObMaxType <= mut_type)
  {
    YYSYS_LOG(WARN,"unsupported type [type:%d]", mut_type);
  ...
  if (OB_SUCCESS == err
      ...
      && org_type != mut_type)
  {
    YYSYS_LOG(WARN,"type not coincident [this->type:%d,mutation.type:%d]",
              org_type, mut_type);
    err = OB_INVALID_ARGUMENT;
  }
    ...
    switch (mut_type)
    {
      ...
      case ObExtendType:
        switch (mutation.get_ext())
        {
          ...
          case ObActionFlag::OP_COLUMN_DELETE:
            if (org_ext == ObActionFlag::OP_ROW_DOES_NOT_EXIST
                || org_ext == ObActionFlag::OP_DEL_ROW)
            {
              set_null();
            ...
            // add e
          default:
            YYSYS_LOG(ERROR,"unsupported ext value [value:%ld]", mutation.get_ext());
            err = OB_INVALID_ARGUMENT;
            break;
        }
      ...
      case ObDecimalType:
      {
        /*ObNumber num, mutation_num, res;
          ...
          */
        //modify wenghaixing DECIMAL OceanBase_BankCommV0.3 2014_7_10:b
        //modify for [Decimal]-b
        uint64_t *t1 = NULL;
        if (OB_SUCCESS != (err = mutation.get_ttint(t1)))
        {
          YYSYS_LOG(ERROR, "get decimal pointer from mutation error,err = %d", err);
        }
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "set_decimal failed,err=%d", err);
          }
        }
        //          *this=mutation;
      ...
      default:
        /* case ObSeqType: */
        YYSYS_LOG(ERROR,"unsupported type [type:%d]", mut_type);
        err = OB_INVALID_ARGUMENT;
        break;
    }
  }
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN,"fail to apply [this->type:%d,this->ext:%d,"
...
int ObObj::obj_copy(ObObj& nobj) const
{
  ...
  {
    YYSYS_LOG(WARN,"unsupported obobj copy type [type:%d]", type);
  ...
  switch(type)
  {
    ...
    case ObDecimalType:
    {
      ...
      //          nobj.set_decimal(ds);
      uint64_t *t1 = NULL;
      if (OB_SUCCESS != (ret = get_ttint(t1)))
      {
        YYSYS_LOG(ERROR, "get decimal pointer from obj error,ret = %d", ret);
      }
      nobj.set_decimal(t1, get_precision(), get_scale(), get_vscale(), get_nwords());
      //modify for [Decimal]-e
      break;
    }
    ...
    default:
      YYSYS_LOG(ERROR,"unsupported obobj copy type [type:%d]", type);
      ret = OB_INVALID_ARGUMENT;
      break;
  }
  return ret;
...
//add:e
void ObObj::dump(const int32_t log_level /*=YYSYS_LOG_LEVEL_DEBUG*/) const
{
  int64_t int_val = 0;
  //add lijianqiang [INT_32] 20151008:b
  int32_t int32_val = 0;
  ...
  {
    case ObNullType:
      YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level), "[%ld] type:ObNull",pthread_self());
      break;
    case ObIntType:
      get_int(int_val,is_add);
      YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level),
    ...
    case ObDecimalType:
      ...
      */
      ...
      {
        YYSYS_LOG(ERROR,"failed to convert decimal from buff!");
      }
      else
      {
        od.set_precision(get_precision());
        od.set_scale(get_scale());
        od.to_string(tmp_buf, MAX_PRINTABLE_SIZE);
      }
      //modify e
      YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level),
                              "[%ld] type:ObDecimalType, val:%s, is_add:%s",
                              pthread_self(), tmp_buf, is_add ? "true" : "false");
      break;
    default:
      YYSYS_LOGGER.logMessage(YYSYS_LOG_NUM_LEVEL(log_level)," [%ld] unexpected type (%d)",pthread_self(),get_type());
      break;
  }
}
...
void ObObj::print_value(FILE* fd)
{
  switch (get_type())
  {
    ...
    case ObDecimalType:
    {
      ...
      if(OB_SUCCESS!= get_decimal(od))
      {
        YYSYS_LOG(ERROR,"failed to get decimal!");
      }
      else
      {
        od.to_string(tmp_buf, MAX_PRINTABLE_SIZE);
...
int64_t ObObj::to_string_v2(std::string &s) const   //uncertainty
{
  int64_t int_val = 0;
  //add lijianqiang [INT_32] 20151008:b
  int32_t int32_val = 0;
  ...
  if(meta_.type_ == ObVarcharType)
  {
    ...
    snprintf(result,str_val.length()+1,"%.*s",str_val.length(), str_val.ptr());
    temp_result<<result;
    s = temp_result.str();
  }
  else
    ...
    switch(meta_.type_)
    {
      ...
      case ObDecimalType:
      {
        ...
        */
        ...
        if(OB_SUCCESS != get_decimal(od))
        {
          YYSYS_LOG(WARN,"failed to get decimal!");
        }
        else
        {
          od.to_string(res,MAX_PRINTABLE_SIZE);
        }
        //[738] databuff_printf(buffer, length, pos, "%s(%d,%d,%d)",res, od.get_precision(), od.get_scale(), od.get_vscale());//[640]
        //modify e
...
//add for [cchecksum info range error]-b
int ObObj::to_string_memcpy(char *buffer, const int64_t length, int64_t &pos) const
{
  int ret = OB_SUCCESS;
  static const char *obj_type_name[] =
  {
...

  {
    switch (meta_.type_)
    {
      ...
      case ObVarcharType:
        get_varchar(str_val);
        if (length - pos >= str_val.length())
        {
          copy_length = str_val.length();
        }
        else
        {
          copy_length = static_cast<int32_t>(length - pos);
        }
        ::memcpy(buffer + pos, str_val.ptr(), copy_length);
        pos += copy_length;
        break;
      case ObFloatType:
        get_float(float_val, is_add);
...
//add for [cchecksum info range error]-e
//add liu jun. [MultiUPS] [calculate] 20150709:b
int64_t ObObj::value_to_string(char *buffer, const int64_t length) const
{
  int64_t int_val = 0;
  float float_val = 0.0;
  double double_val = 0.0;
  ...
  switch(meta_.type_)
  {
    ...
    case ObDecimalType:
    {
      ...
      databuff_printf(buffer, length, pos, "%s", num_buf);
      */
      //mod wenghaixing DECIMAL OceanBase_BankCommV0.3 2014_7_10:b
      //      ObString num;
      //      get_decimal(num);
      ObDecimal od;
      char res[MAX_PRINTABLE_SIZE];
    ...
    switch (type)
    {
      ...
      case ObDecimalType:
        ...
        */
        ...
        break;
      default:
        YYSYS_LOG(ERROR, "invalid obj_type=%d", type);
          ...
          switch (sub_type)
          {
                    ...
                    case 3://other
                    {
                      ...
                      switch (sub_type)
                      {
                        case 0:
                          ...
                          break;
                        default:
                          YYSYS_LOG(ERROR, "invalid obj_type=%d", sub_type);
                          ret = OB_ERR_UNEXPECTED;
                          break;
                      }
                      break;
            ...
            case 3: //other
            {
              ...
              switch (sub_sub_type)
              {
                ...
                case 3: //Other
                {
                  ...
                  switch (sub_sub_sub_type)
                  {
                    ...
                    case 3: //obdecimaltype
                    {
                      ...
                      {
                        ret=OB_ERROR;
                        YYSYS_LOG(ERROR,"decode_comm_decimal error, the ob_string for decimal is NULL!");
                      }
                    ...
                    default:
                      YYSYS_LOG(ERROR, "invalid obj_type=%d", sub_sub_sub_type);
                      ret = OB_ERR_UNEXPECTED;
                      break;
                  }
                  break;
                ...
                default:
                  YYSYS_LOG(ERROR, "invalid obj_type=%d", sub_sub_type);
                  ret = OB_ERR_UNEXPECTED;
                  break;
              }
              break;
...
int ObObj::from_hash(TTCInt &tc,const char* buff, int64_t buf_len)const
{
  ...
  bool is_valid=false;
  short int sign;
  (void)sign; //add for [build warning]
  if(buff==NULL)
  {
    ret=OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "failed to convert decimal to string buff,err=NULL buff ptr!");
  }
  if(buf_len>(int)MAX_PRINTABLE_SIZE)
  {
    ret=OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR,"failed to convert decimal from string buff for hash,since buf_len is too long ");
  }
  if(OB_SUCCESS==ret)
  {
    s = buff;
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "failed to convert char to decimal,invalid num=%c", *s);
    }
  ...
  if (got_digit > MAX_DECIMAL_DIGIT || got_frac > MAX_DECIMAL_SCALE)
  {
    ...
    YYSYS_LOG(ERROR, "decimal overflow!got_digit=%d,got_frac=%d", got_digit,got_frac);
  }
  if(OB_SUCCESS==ret)
  {
    TTCInt whole;
  ...
  switch (type)
  {
    ...
    case ObDecimalType:
      ...
      */
      ...
      break;
    default:
      YYSYS_LOG(ERROR,"unexpected obj type [obj.type:%d]", type);
      break;
  }
  return len;
}
...
uint32_t ObObj::murmurhash2(const uint32_t hash) const
{
  ...
  switch (type)
  {
    ...
    case ObDecimalType:
    {
      ...
      {
        YYSYS_LOG(ERROR,"failed calculate ttint for hash!buff is=%.*s",os.length(),os.ptr());
        result=0;
      }
      else
      {
...
uint64_t ObObj::murmurhash64A(const uint64_t hash) const
{
  ...
  switch (type)
  {
    ...
    default:
      YYSYS_LOG(WARN, "invalid obj type: obj_type[%d]", type);
      result = 0;
  }
