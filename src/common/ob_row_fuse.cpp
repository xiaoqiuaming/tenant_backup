...
int ObRowFuse::assign(const ObUpsRow &incr_row, ObRow &result)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(WARN, "result row desc is null");
  }
  else
  {
    for(int64_t i=0;(OB_SUCCESS == ret) && i<incr_row.get_column_num();i++)
    {
      ...
      {
        YYSYS_LOG(WARN, "incr row get cell fail:ret[%d]", ret);
      }
      else 
      {
        ...
        {
          YYSYS_LOG(WARN, "result set_cell fail:ret[%d]", ret);
        }
      }
    }
  }
...
int ObRowFuse::join_row(const ObUpsRow *incr_row, const ObRow *sstable_row, ObRow *result)
{
  ...
  if(NULL == incr_row || NULL == sstable_row || NULL == result)
  {
    ...
    YYSYS_LOG(WARN, "incr_row[%p], sstable_row[%p], result[%p]", incr_row, sstable_row, result);
  }
...
int ObRowFuse::fuse_row(const ObUpsRow *incr_row, const ObRow *sstable_row, ObRow *result)
{
  ...
  if (OB_SUCCESS == ret && NULL != result_action_flag_cell)
  {
    if (ObExtendType != result_action_flag_cell->get_type())
    {
      ...
      YYSYS_LOG(WARN, "invalid type[%d]", result_action_flag_cell->get_type());
    }
    else
    {
...
int ObRowFuse::get_is_complete_row(const common::ObRow *& row, bool& is_complete_row, bool& is_empty_row)
{
  int ret = OB_SUCCESS;
  ...
  /**
   * @note the UpsRow is init with NopCells for all column,when fuse rows with different version,
   * just cover when the column is exist.
   */
  if (NULL == row)
  {
    ret = OB_NOT_INIT;
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "the current row is[%s]",to_cstring(*row));//for debug
    if (OB_INVALID_INDEX != row->get_row_desc()->get_idx(OB_INVALID_ID, OB_ACTION_FLAG_COLUMN_ID))
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "action_flag_cell_type[%d],ret=%d", action_flag_cell->get_type(), ret);
      }
      else
      {
    ...
    {
       ret = OB_ERROR;
       YYSYS_LOG(ERROR, "inc data must has the action flag column,ret=%d",ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    switch (row_action_flag)
    {
      ...
      case ObActionFlag::OP_VALID:
        is_empty_row = false;
        if (OB_SUCCESS != (ret = get_is_complete_row_in_cell_level(row, is_complete_row)))
        {
          YYSYS_LOG(WARN, "get is complete row in cell level failed,ret=%d",ret);
        }
        break;
      ...
      default:
        ...
        YYSYS_LOG(WARN, "unsupported ext type[%ld]", row_action_flag);
    }
  }
  return ret;
}

int ObRowFuse::get_is_complete_row_in_cell_level(const common::ObRow *& row, bool& is_complete_row)
{
  int ret = OB_SUCCESS;
  is_complete_row = true;
  if (NULL == row)
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    int64_t action_flag_index = row->get_row_desc()->get_idx(OB_INVALID_ID, OB_ACTION_FLAG_COLUMN_ID);
    for (int64_t i=0; OB_SUCCESS == ret && i<column_num; i++)
    {
      if (i != action_flag_index)
      {
        ...
        {
          type = cell->get_type();
          if ((type <= ObMinType) ||//invalid type
              (type >= ObMaxType) ||//invalid type
              (ObExtendType == type && ObActionFlag::OP_NOP == cell->get_ext()))//can't be nop cell
          {
            is_complete_row = false;
            YYSYS_LOG(DEBUG, "not complete row,may have nop cell");
            cell->dump();
            break;
          }
        }
...
int ObRowFuse::fuse_sstable_row(const ObRow &row, ObRow &result_row, bool &is_row_empty)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        ret = OB_ERR_UNEXPECTED;
        YYSYS_LOG(WARN, "action_flag_cell_type[%d]", action_flag_cell->get_type());
      }
      else
      {
        row_action_flag = action_flag_cell->get_ext();
      ...
      if (OB_SUCCESS == ret)
      {
        switch (row_action_flag)
        {
          ...
          case ObActionFlag::OP_DEL_ROW:
            ...
            {
              YYSYS_LOG(WARN, "reset result fail:ret[%d]", ret);
            }
          ...
          case ObActionFlag::OP_NEW_ADD:
            ...
            {
              YYSYS_LOG(WARN, "fail to reset result_row:ret[%d]", ret);
            }
...
int ObRowFuse::fuse_ups_row(const ObRow &row, ObRow &result_row, bool &is_row_empty)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to get result action flag cell:ret[%d]", ret);
  }
  else if (ObExtendType != action_flag_cell->get_type() || ObExtendType != result_action_flag_cell->get_type())
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "action_flag_cell_type[%d], result_row_action_flag_type[%d]", action_flag_cell->get_type(),
