...
namespace oceanbase
{
  namespace common
  {
    ...
    //����Ĳ�����server��ָ�룬ʹ�ô˺�����ʱ��ע������Ӧ�ñ�֤count������ĳ��ȱ����Ӧ
    ...
    int BlackList::add_in_black_list(ObNewRange &range, ObServer &server)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "this server is not in black list, we cant turn on server");
      }
      return ret;
    }
    ...
    int BlackList::next_replic_server(ObServer &server)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        //modify for [index build whip work]-b
        //always whip work in loop, if has a replica alive. so this function always return OB_SUCCESS
        //        YYSYS_LOG(WARN, "all replication is failed......DUMP SERVER:\n"
        ...
        YYSYS_LOG(WARN, "all replications failed, then start a new round.DUMP SERVER:");
        for(int8_t i = 0; i < server_count_; i++)
        {
          YYSYS_LOG(INFO, "dumped server[%s],unserved[%d]", to_cstring(server_[i]), unserved_[i]);
          unserved_[i] = 0;
        }
        //whip work to first replica again
        server = server_[0];
        ret = OB_SUCCESS;
        //modify for [index build whip work]-e
      }
      return ret;
    }
    ...
    //add for [index build whip work]-b
    int BlackList::set_unserved_array(BlackList &list)
    {
      int ret = OB_SUCCESS;
      if(list.server_count_ != server_count_)
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "server count in blacklist not equal.src server count[%d] ,dest server count[%d]", list.server_count_, server_count_);
      }
      else
      {
        for(int64_t i = 0; i < server_count_; ++i)
        {
          unserved_[i] = list.unserved_[i];
        }
      }
      return ret;
    }
    //add for [index build whip work]-e
    ...
    int BlackList::get_server(ObServer &server)
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "all replication is failed......");
      }
      return ret;
    }
#endif
    ...
    BlackList &BlackListArray::get_black_list(int64_t i, int &err)
    {
      ...
      if(i > list_array_.count())
      {
        ...
        YYSYS_LOG(WARN,"index is greater than black list count");
        return error_;
      }
      else
      {
    ...
    int BlackListArray::push(BlackList &list)
    {
      ...
      for(int64_t i = 0; i < list_array_.count(); i++ )
      {
        ...
        {
          YYSYS_LOG(ERROR, "get black list itr failed, ret[%d]", ret);
          break;
        }
        else
        {
        ...
        //modify for [index build whip work]-e
      {
        ...
        {
          YYSYS_LOG(ERROR, "deep copy start key failed,ret = [%d]", ret);
        }
        else if(OB_SUCCESS != (ret = range_src.end_key_.deep_copy(range_dst.end_key_, arena_)))
        {
          YYSYS_LOG(ERROR, "deep copy end key failed, ret = [%d]", ret);
        }
        else
        {
      ...
      if(OB_SUCCESS == ret && has_new_range)
      {
        ...
        for(int8_t i = 0; i < list.get_server_count(); i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "get server failed,ret [%d]", ret);
            break;
          }
          else
          {
        ...
        {
          YYSYS_LOG(WARN, "wite list failed,ret = %d", ret);
        }
        //add for [index build whip work]-b
        else if(OB_SUCCESS != (ret = list_to_push.set_unserved_array(list)))
        {
          YYSYS_LOG(WARN, "set unserved array failed,ret = %d", ret);
        }
        //add for [index build whip work]-e
        else if(OB_SUCCESS != (ret = list_array_.push_back(list_to_push)))
        {
          YYSYS_LOG(WARN, "push list into array failed,ret = %d", ret);
        }
        else
        {
      ...
      if(OB_SUCCESS ==  ret && !has_new_range)
      {
        ...
        //       {
        //         YYSYS_LOG(INFO, "this black list is already bleach!");
        //       }
        //       else
        //       {
        //         for(int8_t i = 0; i < list.get_server_count(); i++)
        ...
        //           {
        //             YYSYS_LOG(WARN, "add server into black list failed, ret[%d]", ret);
        //             break;
        //           }
        //         }
        //       }
        ...
        if(OB_SUCCESS != (ret = list_array_.at(index).write_list(range_dst, location_list)))
        {
          YYSYS_LOG(WARN, "write list failed,ret = %d", ret);
        }
        else if(OB_SUCCESS != (ret = list_array_.at(index).set_unserved_array(list)))
        {
          YYSYS_LOG(WARN, "set unserved array failed,ret = %d", ret);
        }
        else
        {
          list_bleach_.at(index) = 0;
          list_array_.at(index).set_wok_send(0);
        }
        //modify for [index build whip work]-e
      }
    ...
    int BlackList::get_rowkey_compatible(const char *buf, const int64_t buf_len, int64_t &pos,  ObObj *array, int64_t &size)
    {
      ...
      if ( OB_SUCCESS == (ret = obj.deserialize(buf, buf_len, pos)) )
      {
        ...
        {
          YYSYS_LOG(ERROR, "old fashion rowkey stream!");
          ret = OB_INVALID_ARGUMENT;
        }
      }
