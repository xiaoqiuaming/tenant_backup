...
namespace oceanbase
{
  namespace common
  {
     ...
     bool ObServer::set_ipv6_addr(const char* ip, const int32_t port)
    {
      UNUSED(ip);
      UNUSED(port);
      YYSYS_LOG(WARN, "set ipv6 address is not complete");
      return false;
    }
    ...
    int ObServer::parse_from_cstring(const char* target_server)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument, buff=%p, lenght=%ld", target_server, strlen(target_server));
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (target_server[0] == '[')
        {
          ...
          {
            ret = OB_INVALID_ARGUMENT;
            YYSYS_LOG(WARN, "fail to parse string to IPV6 address, buf=%s", target_server);
          }
          else
          {
            ...
            {
              ret = OB_INVALID_ARGUMENT;
              YYSYS_LOG(WARN, "invalid server string. buf=%s", target_server);
            }
          }
        }
        else  //IPV4
        {
          ...
          {
            ret = OB_INVALID_ARGUMENT;
            YYSYS_LOG(WARN, "fail to parse string to IPV4 address, buf=%s", target_server);
          }
          else
          {
