...
uint64_t ObMetaTableColumnSchema::get_cid(const CF cf, const int32_t i) const
{
  ...
  {
    YYSYS_LOG(ERROR, "BUG invalid index, i=%d", i);
  }
  else
  {
    ObString cname;
    ...
    switch(cf)
    {
      ...
      case CF_STARTKEY_COL:
        ...
        break;
      default:
        YYSYS_LOG(ERROR, "BUG invalid meta table column, cf=%d", cf);
        break;
    } // end switch

    if (OB_SUCCESS != get_cid_by_name(cname, ret))
    {
      YYSYS_LOG(ERROR, "meta table column not found, cname=%.*s tid=%lu",
...
uint64_t ObMetaTableColumnSchema::get_cid_by_idx(const int64_t i) const
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid column index, i=%ld tid=%lu", i, table_schema_.table_id_);
  }
  else
  {
    ObString cname;
...
int ObMetaTableColumnSchema::get_cf(const uint64_t cid, CF &cf, int32_t &idx) const
{
  ...
  {
    YYSYS_LOG(WARN, "no column found by id, cid=%lu", cid);
  }
  else
  {
