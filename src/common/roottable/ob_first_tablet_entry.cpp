...
int ObFirstTabletEntryTable::MyIterator::next(const ObTabletMetaTableRow *&row)
{
  ...
  {
    YYSYS_LOG(WARN, "scanner failed to get next row, err=%d", ret);
  }
  return ret;
}
...
int ObFirstTabletEntryTable::MyIterator::check_row(ObCellInfo *row_cells, int64_t cell_num) const
{
  ...
  if (SCHEMA.get_columns_num() != cell_num)
  {
    ...
    YYSYS_LOG(WARN, "invalid first tablet entry row, col_num=%ld expected=%lu",
  ...
  else
  {
    for (int64_t i = 0; i < cell_num && OB_SUCCESS == ret; ++i)
    {
      ...
      {
        ret = OB_ERR_DATA_FORMAT;
        YYSYS_LOG(WARN, "invalid first table entry row, tid=%lu",
      ...
      {
        ret = OB_ERR_DATA_FORMAT;
        YYSYS_LOG(WARN, "invalid first table entry row, rowkey_obj_count=%ld",
      ...
      {
        ret = OB_ERR_DATA_FORMAT;
        YYSYS_LOG(WARN, "invalid first table entry row, type=%d",
      ...
      {
        ret = OB_ERR_DATA_FORMAT;
        YYSYS_LOG(WARN, "invalid first table entry row, col=%ld cid=%lu",
      ...
      if (SCHEMA.get_cidx(SCHEMA.CF_META_TNAME, 0) == i)     // TNAME or META_TNAME
      {
        ...
        {
          ret = OB_ERR_DATA_FORMAT;
          YYSYS_LOG(WARN, "invalid first table entry row, col=%ld type=%d",
...
{
  YYSYS_LOG(DEBUG, "destroy ObFirstTabletEntryTable #iterators=%ld", iterators_.count());
  ...
  iterators_.clear();
  YYSYS_LOG(DEBUG, "destroy ObFirstTabletEntryTable #scanners=%ld", scanners_.count());
...
int ObFirstTabletEntryTable::get_location(ObScanner &scanner)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to search the first meta, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = it->next(row)))
  {
    YYSYS_LOG(ERROR, "failed to read the first meta, err=%d", ret);
  }
  else
  {
...
int ObFirstTabletEntryTable::cons_scan_param(const ObNewRange &range, ObScanParam &scan_param)
{
  ...
  for (int i = 0; i < SCHEMA.get_columns_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to add column name, err=%d", ret);
      break;
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to aquire scanner. err=%d", ret);
    }
...
int ObFirstTabletEntryTable::get_meta_tid(const uint64_t tid, ObString &tname, uint64_t &meta_tid, ObString& meta_tname)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid tid, tid=%lu", tid);
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to search key, err=%d tid=%lu", ret, tid);
    }
    else if (OB_SUCCESS != (ret = it->next(row)))
    {
      YYSYS_LOG(WARN, "table not exist, err=%d tid=%lu", ret, tid);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to copy table name");
      }
...
int ObFirstTabletEntryTable::scan_row(const ObString &tname, ObScanner *&out)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "out is not null");
  }
  else if (OB_SUCCESS != (ret = cons_scan_param(single_key_range, *scan_param)))
  {
    YYSYS_LOG(WARN, "failed to cons scan param, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = aquire_scanner(out)))
  {
    YYSYS_LOG(ERROR, "failed to aquire scanner");
  }
  else if (OB_SUCCESS != (ret = scan(*scan_param, *out)))
  {
    YYSYS_LOG(WARN, "failed to scan, err=%d", ret);
  }
  if (NULL != scan_param)
  {
...
int ObFirstTabletEntryTable::search(const KeyRange& key_range, ConstIterator *&first)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "invalid search key, tid=%lu", key_range.table_id_);
  }
  ...
  {
    YYSYS_LOG(WARN, "failed to get name from tid, err=%d tid=%lu", ret, key_range.table_id_);
  }
  else if (OB_SUCCESS != (ret = scan_row(tname, scanner)))
  {
    YYSYS_LOG(WARN, "failed to scan row, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = aquire_iterator(*scanner, it)))
  {
    YYSYS_LOG(ERROR, "failed to aquire iterator");
  }
  else
  {
    first = it;
...
//   {
//     ret = OB_INVALID_ARGUMENT;
//     YYSYS_LOG(WARN, "invalid table, tid=%lu", tid);
//   }
//   else
//   {
...
int ObFirstTabletEntryTable::table_id_to_name_in_scanner(ObScanner& scanner, const uint64_t tid, ObString &name, bool &found)
{
  ...
  while(OB_SUCCESS == ret && OB_SUCCESS == it.next(row))
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to copy table name, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = id_name_map_.push_back(std::make_pair(mrow->get_tid(), tname_copy))))
    {
      YYSYS_LOG(ERROR, "failed to push into array, err=%d", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "scan first tablet table, tname=%.*s tid=%lu",
...
int ObFirstTabletEntryTable::table_id_to_name_by_scan(const uint64_t tid, ObString &name)
{
  ...
  while(!found && OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to find in scanner, err=%d", ret);
    }
    else if (!found)
    {
      ...
      {
        YYSYS_LOG(WARN, "scanner get fullfilled error, err=%d", ret);
      }
      else
      {
        if (is_fullfilled)
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get last rowkey, err=%d", ret);
          }
        }
      }
    } // end if (!found)
                                                    ...
                                                    TableSchema &tschema) const
{
  ...
  for (int64_t i = 0; i < ObMetaTableColumnSchema::get_fixed_columns_num(); ++i)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to push into col array, err=%d", ret);
      break;
    }
  } // end for
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (int32_t j = 0; j < oschema.columns_.count(); ++j)
    {
      ...
      if (rowkey_id == ocschema.rowkey_id_)
      {
        ...
        if (OB_SUCCESS != (ret = tschema.columns_.push_back(col_schema)))
        {
          YYSYS_LOG(WARN, "failed to push into column array, err=%d", ret);
          break;
        }
    ...
    if (rowkey_id < tschema.rowkey_column_num_)
    {
      YYSYS_LOG(ERROR, "no rowkey column found, i=%d", rowkey_id);
      ret = OB_ERR_UNEXPECTED;
    }
  }
  return ret;
                                               ...
                                               uint64_t &meta_tid)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to get max used tid, err=%d", ret);
  }
           ...
           > OB_MAX_TABLE_NAME_LENGTH)
  {
    ...
    YYSYS_LOG(ERROR, "meta table name too long, tname=%s", oschema.table_name_);
  }
  else if (OB_SUCCESS != (ret = cons_meta_table_schema(max_used_tid+1, oschema, tschema)))
  {
    YYSYS_LOG(WARN, "failed to construct the new schema, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = schema_service_.create_table(tschema)))
  {
    YYSYS_LOG(WARN, "failed to create table, err=%d tname=%s", ret, tschema.table_name_);
  }
  else
  {
    YYSYS_LOG(INFO, "created meta table, tname=%s", tschema.table_name_);
    ...
    {
      YYSYS_LOG(ERROR, "meta_tname's buffer not enough, tname_len=%d", tname_len);
    }
                                                      ...
                                                      ObMutator &mutator)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to search, err=%d tid=%lu", ret, indexed_tid);
  }
  ...
  {
    YYSYS_LOG(DEBUG, "copy replicas from the only tablet for table, tid=%lu", crow->get_tid());
...
int ObFirstTabletEntryTable::insert(const Value &v)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to create meta table, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = mutator_init_row(meta_tname, meta_tid, mutator)))
  {
    YYSYS_LOG(WARN, "failed to fill mutator, err=%d", ret);
  }
...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(WARN, "try to delete table from the first_tablet_entry_table, tid=%lu", k.table_id_);
...
int ObFirstTabletEntryTable::update(const Value &v)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "invalid endkey or startkey, endkey=%s startkey=%s tname_len=%d",
...
int ObFirstTabletEntryTable::commit() // for testing only
{
  ...
  {
    mutator_.reset();
    YYSYS_LOG(WARN, "directly call first_tablet_entry_table's commit");
  }
  return ret;
}
