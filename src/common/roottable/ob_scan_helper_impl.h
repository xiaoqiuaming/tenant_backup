...
namespace oceanbase
{
  namespace common
  {
    ...
    // [secondary index get old checksum opt]
    // only for request from cs to ms
    class ObLocalCsMsList : public ObMsProvider
    {
      public:
          ObLocalCsMsList();
          ~ObLocalCsMsList();

          int init(MsList *ms_list, const ObServer &server);
    ...
    // [secondary index get old checksum opt] end
