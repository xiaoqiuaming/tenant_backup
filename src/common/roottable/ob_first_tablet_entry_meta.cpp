...
{
  YYSYS_LOG(DEBUG, "construct ObFirstTabletEntryMeta::MyIterator addr=%p", this);
}

ObFirstTabletEntryMeta::MyIterator::~MyIterator()
{
  // @note ����ʱӦ�ð�read_handle_ָ��Ԫ�ص����ü�����һ��������ΪMyIterator����TSI����ģ���aggr_�ǴӶѷ���ģ�
  // MyIterator����ʱaggr_ָ��Ķ����Ѿ���Ч�����ܷ���
  aggr_ = NULL;
  pos_ = -1;
  read_handle_ = INVALID_HANDLE;
  YYSYS_LOG(DEBUG, "destruct ObFirstTabletEntryMeta::MyIterator addr=%p", this);
}
...
int ObFirstTabletEntryMeta::MyIterator::next(const ObTabletMetaTableRow *&row)
{
  ...
  {
    YYSYS_LOG(ERROR, "iterator not init");
...
{
  // @note �Ӵ˴�������tagtag֮��Ĵ�����Ϊ�����ObFirstTabletEntryMetaTest�ĵ�������⴦���ģ�
  // ��ΪMyIterator��TSI�������Զ�����������������������Ч������������case��Ҫ�໥������ÿ��case
  // �½��Լ���ObFirstTabletEntryMeta�������������Ҫ����aggr_��ֵ
  ...
  while(true)
  {
    ...
    if (0 != oldref)
    {
      ...
      if (oldref == atomic_compare_exchange(&aggr_->first_meta_[read_handle].ref_count_, newref, oldref))
      {
        ...
        YYSYS_LOG(DEBUG, "init iterator with read handle=%u", read_handle_);
        break;
      }
      else
      {
        YYSYS_LOG(DEBUG, "oldref=%lu newref=%lu", oldref, newref);
      }
    }
    else
    {
      YYSYS_LOG(DEBUG, "readhandle_=%lu oldref=%lu", read_handle, oldref);
    }
  } // end while
}
   ...
   write_handle_(INVALID_HANDLE), first_meta_(NULL)
{
  ...
  {
    YYSYS_LOG(INFO, "init meta array size=%d", meta_array_size_);
  }
  int ret = OB_SUCCESS;
  if (OB_SUCCESS != (ret = set_filename(filename)))
  {
    YYSYS_LOG(WARN, "failed to set filename, err=%d", ret);
  }
  YYSYS_LOG(DEBUG, "construct ObFirstTabletEntryMeta, this=%p", this);
}
...
ObFirstTabletEntryMeta::~ObFirstTabletEntryMeta()
{
  ...
  YYSYS_LOG(DEBUG, "destruct ObFirstTabletEntryMeta, this=%p", this);
}
...
int ObFirstTabletEntryMeta::init(const Value &v)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid argument row, err=%d", ret);
  }
  else if (INVALID_HANDLE != read_handle_)
  {
    ret = OB_INIT_TWICE;
    YYSYS_LOG(WARN, "meta already init, handle=%u", read_handle_);
  }
  else
  {
    read_handle_ = 0;
    ...
    YYSYS_LOG(INFO, "first tablet entry meta inited");
  }
  return ret;
}
...
int ObFirstTabletEntryMeta::check_integrity()
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "first_meta_ is NULL");
  }
  else if (read_handle_ == INVALID_HANDLE)
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "read_handle is invalid");
  }
  else if ('\0' == filename_[0])
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "filename is empty");
  }
  return ret;
}
...
int ObFirstTabletEntryMeta::search(const KeyRange& key_range, ConstIterator *&first)
{
  ...
  if (OB_FIRST_TABLET_ENTRY_TID != key_range.table_id_)
  {
    ...
    YYSYS_LOG(ERROR, "invalid table id, tid=%lu", key_range.table_id_);
  }
  else if (OB_SUCCESS != (ret = check_integrity()))
  {
    YYSYS_LOG(ERROR, "integrity error");
  }
  else if (OB_SUCCESS != (ret = aquire_iterator(it)))
  {
    YYSYS_LOG(ERROR, "aquire iterator error, err=%d", ret);
  }
  else
  {
    first = it;
...
int ObFirstTabletEntryMeta::insert(const Value &v)
{
  ...
  UNUSED(v);
  YYSYS_LOG(ERROR, "the first tablet entry table can not split and should not be inserted");
  return ret;
}
...
int ObFirstTabletEntryMeta::erase(const Key &k)
{
  int ret = OB_ERR_UNEXPECTED;
  UNUSED(k);
  YYSYS_LOG(ERROR, "the first tablet entry table can not be erased");
  return ret;
}
...
int ObFirstTabletEntryMeta::check_first_meta_row(const Value &v)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid table id, tid=%lu", v.get_tid());
  }
  else if (ObString::make_string(FIRST_TABLET_TABLE_NAME) != v.get_table_name())
  {
    YYSYS_LOG(ERROR, "invalid table name, tname=%.*s",
  ...
  {
    YYSYS_LOG(ERROR, "invalid end_key, obj_cnt=%ld obj_ptr=%p",
  ...
  {
    YYSYS_LOG(ERROR, "invalid start_key, obj_cnt=%ld obj_ptr=%p",
...
int ObFirstTabletEntryMeta::aquire_write_handle()
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "can not aquire write_handle, read_handle=%u whandle=%u",
...
int ObFirstTabletEntryMeta::update(const Value &v)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid first meta row");
  }
  else if (OB_SUCCESS != (ret = check_integrity()))
  {
    YYSYS_LOG(ERROR, "read first meta error");
  }
  else if (OB_SUCCESS != (ret = aquire_write_handle()))
  {
    YYSYS_LOG(ERROR, "failed to aquire write handle");
  }
  else
  {
    YYSYS_LOG(DEBUG, "update with write handle=%u", write_handle_);
...
void ObFirstTabletEntryMeta::commit_write_handle()
{
  ...
  YYSYS_LOG(DEBUG, "commit write handle=%u", write_handle_);
  write_handle_ = INVALID_HANDLE;
}
...
int ObFirstTabletEntryMeta::set_filename(const char* filename)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "argument filename is NULL");
  }
  else
  {
    ...
    if (static_cast<int64_t>(sizeof(filename_)) < data_len)
    {
      ...
      YYSYS_LOG(WARN, "filename too long");
    }
    else
    {
...
int ObFirstTabletEntryMeta::init()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to load from file, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = init(row)))
  {
    YYSYS_LOG(WARN, "failed to init with row, err=%d", ret);
  }
  return ret;
}
...
int ObFirstTabletEntryMeta::load(const char* filename, ObTabletMetaTableRow &row)
{
  ...
  else
  {
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(WARN, "failed to openfile, file=%s err=%s", filename, strerror(errno));
    }
    else if (-1 == (readlen = read(fd, buf, max_data_size)))
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to read file, file=%s err=%s", filename, strerror(errno));
    }
    else if (OB_SUCCESS != (ret = row.deserialize(buf, readlen, pos)))
    {
      YYSYS_LOG(ERROR, "deserialize meta row error, err=%d", ret);
    }
    ...
    {
      YYSYS_LOG(INFO, "load the first meta from file, file=%s", filename);
...
int ObFirstTabletEntryMeta::backup(const char* filename)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid filename");
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "failed to generate backup filename");
    }
    else if (-1 == unlink(backup_filename)
             && ENOENT != errno)
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to remove file, err=%s file=%s", strerror(errno), backup_filename);
    }
    else if (-1 == rename(filename, backup_filename)
             && ENOENT != errno)
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to rename file, err=%s src=%s dest=%s",
...
int ObFirstTabletEntryMeta::store(const ObTabletMetaTableRow &row, const char* filename)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid row, err=%d", ret);
    ret = OB_ERR_UNEXPECTED;
  }
  else if (OB_SUCCESS != (ret = backup(filename)))
  {
    YYSYS_LOG(WARN, "failed to backup file, err=%d filename=%s", ret, filename);
  }
  else
  {
    const int64_t max_data_size = 64*1024;
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to openfile, file=%s err=%s", filename, strerror(errno));
    }
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to write file, file=%s err=%s", filename, strerror(errno));
    }
    else
    {
      YYSYS_LOG(INFO, "store the first meta to file, file=%s", filename);
    }
    if (-1 != fd)
    {
      close(fd);
