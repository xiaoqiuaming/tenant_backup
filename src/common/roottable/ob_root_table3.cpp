...
int ObRootTable3::search(const KeyRange& key, ConstIterator *&first, int32_t recursive_level)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid search range, range=%s", to_cstring(key));
  }
  else if (recursive_level > MAX_RECURSIVE_LEVEL)
  {
    ret = OB_MAYBE_LOOP;
    YYSYS_LOG(ERROR, "too many recursive funcation call");
  }
  else if (OB_SUCCESS != (ret = first_table_.get_meta_tid(key.table_id_, tname, meta_tid, meta_tname))
           || OB_INVALID_ID == meta_tid)
  {
    YYSYS_LOG(DEBUG, "table not exist, tid=%lu", key.table_id_);
    ret = OB_ENTRY_NOT_EXIST;
  }
  else
  {
    YYSYS_LOG(DEBUG, "get meta table, tid=%lu meta_tid=%lu", key.table_id_, meta_tid);
    ...
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to read meta table, tid=%lu", meta_tid);
      }
      else
      {
...
int ObRootTable3::aquire_meta_table_for_mutation(const uint64_t tid, common::ObTabletMetaTable* &meta_table)
{
  ...
  if (OB_SUCCESS != (ret = first_table_.get_meta_tid(tid, tname, meta_tid, meta_tname))
           || OB_INVALID_ID == meta_tid)
  {
    YYSYS_LOG(DEBUG, "table not exist, tid=%lu", tid);
    ret = OB_ENTRY_NOT_EXIST;
  }
  else
  {
    YYSYS_LOG(DEBUG, "get meta table, tid=%lu meta_tid=%lu", tid, meta_tid);
    if (OB_FIRST_META_VIRTUAL_TID == meta_tid)
    {
...
int ObRootTable3::insert(const Value &v)
{
  ...
  do
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to aquire meta table for mutation, err=%d tid=%lu",
                ret, v.get_tid());
    }
    else
    {
    ...
    if (loop > 2)
    {
      YYSYS_LOG(ERROR, "egain too many times, loop=%d", loop);
      ret = OB_MAYBE_LOOP;
      break;
    }
  } while (OB_EAGAIN == ret);
...
int ObRootTable3::commit()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to commit the meta");
    }
    else
    {
      mutator_.reset();
