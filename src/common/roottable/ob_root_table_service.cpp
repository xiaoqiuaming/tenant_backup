...
int ObRootTableService::aquire_root_table(ObScanHelper &scan_helper, ObRootTable3 *&root_table)
{
  ...
  if (NULL != root_table)
  {
    ...
    YYSYS_LOG(ERROR, "root table is not NULL");
  }
  else
  {
...
int ObRootTableService::report_tablets(ObScanHelper &scan_helper, const common::ObServer& cs, const common::ObTabletReportInfoList& rtablets)
{
  ...
  {
    YYSYS_LOG(INFO, "cs report tablets, cs=%s tablets_num=%ld", cs.to_cstring(), rtablets.get_tablet_size());
    ...
    for (int64_t i = 0; i < rtablets.get_tablet_size(); ++i)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to report");
      }
    }
  }
  return ret;
...
int ObRootTableService::report_tablet(ObScanHelper &scan_helper, const ObTabletReportInfo& tablet)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid tablet");
    ret = OB_INVALID_ARGUMENT;
  }
  else if (OB_SUCCESS != (ret = aquire_root_table(scan_helper, root_table)))
  {
    YYSYS_LOG(ERROR, "failed to aquire root table, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = root_table->search(tablet.tablet_info_.range_, first)))
  {
    YYSYS_LOG(WARN, "failed to search tablet, err=%d range=%s", ret, to_cstring(tablet.tablet_info_.range_));
  }
  else
  {
 ...
 int ObRootTableService::add_new_tablet(ObRootTable3& root_table, const ObTabletReportInfo& tablet)
 {
   ...
   {
     YYSYS_LOG(WARN, "failed to insert row, err=%d", ret);
   }
   else if (OB_SUCCESS != (ret = root_table.commit()))
   {
     YYSYS_LOG(WARN, "failed to commit change, err=%d", ret);
   }
   return ret;
 }
                                           ...
                                           const ObTabletReportInfo& tablet)
 {
   ...
   for (int i = 0; i < row.get_max_replica_count(); ++i)
   {
     ...
     if (replica.cs_ == tablet.tablet_location_.chunkserver_)
     {
       ...
       {
         YYSYS_LOG(WARN, "cs reported an old replica, cs=%s version=%ld report_v=%ld",
       ...
       else
       {
         ...
         {
           YYSYS_LOG(ERROR, "check sum error, ver=%ld old_crc=%lu new_crc=%lu cs=%s endkey=%s", 
   ...
   if (!found)
   {
     ...
     if (!found)
     {
       YYSYS_LOG(WARN, "no entry for replica, cs=%s endkey=%s",
                 tablet.tablet_location_.chunkserver_.to_cstring(),
                 to_cstring(tablet.tablet_info_.range_.end_key_));
     }
   }
   if (row.has_changed())
   {
     ...
     {
       YYSYS_LOG(ERROR, "failed to update root table");
     }
     else if (OB_SUCCESS != (ret = root_table.commit()))
     {
       YYSYS_LOG(WARN, "failed to commit change, err=%d", ret);
                                                       ...
                                                       const ObTabletReportInfo& tablet)
 {
   ...
   {
     YYSYS_LOG(ERROR, "failed to update root table, err=%d", ret);
   }
 ...
 int ObRootTableService::remove_replicas(ObScanHelper &scan_helper, const common::ObServer &cs)
 {
   ...
   if (0 == cs.get_port())
   {
     YYSYS_LOG(WARN, "invalid cs port");
     ret = OB_INVALID_ARGUMENT;
   }
 ...
 int ObRootTableService::remove_replicas_in_table(ObScanHelper &scan_helper, const common::ObServer &cs, const uint64_t tid)
 {
   ...
   {
     YYSYS_LOG(WARN, "failed to search tablet, err=%d tid=%lu", ret, tid);
   }
   else
   {
         ...
         if (replica.cs_ == cs)
         {
           ...
           else
           {
             ...
             if (updated_count % batch_count == 0)
             {
               ...
               {
                 YYSYS_LOG(ERROR, "failed to commit change, err=%d", ret);
               }
               else
               {
     ...
     if (OB_ITER_END != ret)
     {
       YYSYS_LOG(WARN, "failed to remove replicas, err=%d", ret);
     }
     else
     {
       ret = OB_SUCCESS;
     ...
     YYSYS_LOG(INFO, "delete replicas by cs, cs=%s replicas_num=%d", 
...
int ObRootTableService::migrate_replica(ObScanHelper &scan_helper, const ObNewRange &range, const int64_t version, 
                                         const common::ObServer &from, const common::ObServer &to, bool keep_src)
 {
   ...
   {
     YYSYS_LOG(WARN, "failed to search tablet, err=%d", ret);
   }
   else if (OB_SUCCESS != (ret = first->next(crow)))
   {
     YYSYS_LOG(WARN, "tablet not exist, err=%d range=%s", ret, to_cstring(range));
     ret = OB_ENTRY_NOT_EXIST;
   }
   else if (!is_same_range(*crow, range))
   {
     ret = OB_ENTRY_NOT_EXIST;
     YYSYS_LOG(WARN, "tablet not exist, range=%s", to_cstring(range));
   }
   else
   {
     for (int32_t i = 0; i < crow->get_max_replica_count(); ++i)
     {
       ...
       if (from == replica.cs_)
       {
         ...
         {
           YYSYS_LOG(WARN, "migrate tablet with wrong version, old_v=%ld new_v=%ld cs=%s range=%s",
         ...
         else
         {
           ...
           if (keep_src)
           {
             ...
             {
               YYSYS_LOG(WARN, "failed to add replica, err=%d", ret);
             }
           }
           else
           {
...
int ObRootTableService::remove_tables(ObScanHelper &scan_helper, const common::ObArray<uint64_t> &tables)
{
  ...
  else
  {
    for (int32_t i = 0; i < tables.count(); ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to remove table, err=%d tid=%ld",
                  ret, tid);
        break;
      }
      else
      {
        YYSYS_LOG(INFO, "removed table, tid=%ld", tid);
      }
    }
  }
  return ret;
...
int ObRootTableService::remove_table(ObScanHelper &scan_helper, const uint64_t &tid)
{
  ...
  else
  {
    ...
    if (OB_ITER_END != ret)
    {
      YYSYS_LOG(WARN, "failed to remove table, err=%d range=%s", ret, to_cstring(range_min_max));
    }
    else
    {
      ret = OB_SUCCESS;
    ...
    YYSYS_LOG(INFO, "remove table, tid=%lu removed_tablets_count=%d", tid, updated_count);
  }
  release_root_table(root_table);
...
int ObRootTableService::search_range(ObScanHelper &scan_helper, const common::ObNewRange& range, common::ObScanner& scanner)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to search range, err=%d range=%s", ret, to_cstring(range));
  }
  else
  {
          ...
          && (OB_SUCCESS == (ret = first->next(crow))))
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to add row into scanner, err=%d", ret);
        break;
      }
    }
    if (OB_ITER_END == ret)
...
int ObRootTableService::new_table(ObScanHelper &scan_helper, const uint64_t tid, const int64_t tablet_version, const ObArray<ObServer> &chunkservers)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid table id=%lu", tid);
    ret = OB_INVALID_ARGUMENT;
  }
  else if (0 > tablet_version)
  {
    YYSYS_LOG(WARN, "invalid tablet version=%ld", tablet_version);
    ret = OB_INVALID_ARGUMENT;
  }
  else if (0 >= chunkservers.count())
  {
    YYSYS_LOG(WARN, "invalid chunkserver count=%ld", chunkservers.count());
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "no row found for the new table, err=%d", ret);
    }
    else
    {
      ObTabletMetaTable::Value new_row = *crow; // copy
