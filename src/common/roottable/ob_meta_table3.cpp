...
//mod peiouys 20160119:b
//MyIterator's size is enlarged, so enlarge the default block size to avoid warnning
//carefully, if you want to modify it
//ObLockedPool ObMetaTable3::MyIterator::MY_ITERATOR_POOL(sizeof(MyIterator));
ObLockedPool ObMetaTable3::MyIterator::MY_ITERATOR_POOL(sizeof(MyIterator),
              ((sizeof(MyIterator) + OB_COMMON_MEM_BLOCK_SIZE - 1)
               / OB_COMMON_MEM_BLOCK_SIZE) * OB_COMMON_MEM_BLOCK_SIZE);
//mod 20160119:e
                                           ...
                                           const bool inclusive_startkey, ObScanner &location)
{
  ...
  if (OB_INVALID_ID == curr_meta_row_.get_tid())
  {
    ...
    if (OB_INVALID_ID == curr_meta_row_.get_tid())
    {
      YYSYS_LOG(WARN, "failed to find the location, startkey=%s", to_cstring(startkey));
      ret = OB_ENTRY_NOT_EXIST;
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "find the location, startkey=%s meta_range=%s:%s",
...
int ObMetaTable3::MyIterator::add_scan_columns(ObScanParam &scan_param)
{
  ...
  else
  {
    ...
    for (int64_t i = 0; i < col_schema.get_columns_num(); ++i)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get column id, i=%ld", i);
      }
...
int ObMetaTable3::MyIterator::scan_beginning(ConstIterator &meta_it, const KeyRange &search_range, ObScanner& scanner)
{
  ...
  if (NULL == scan_param)
  {
    YYSYS_LOG(ERROR, "failed to alloc scan param");
    ret = OB_ALLOCATE_MEMORY_FAILED;
  }
  else if (NULL == my_location)
  {
    YYSYS_LOG(ERROR, "failed to alloc scanner");
    ret = OB_ALLOCATE_MEMORY_FAILED;
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to set limit info, err=%d", ret);
    }
    // scan all columns
    else if (OB_SUCCESS != (ret = add_scan_columns(*scan_param)))
    {
      YYSYS_LOG(WARN, "failed to add scan columsn, err=%d", ret);
    }
    ...
    {
      YYSYS_LOG(WARN, "failed to get location, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = scan_param->set_location_info(*my_location)))
    {
      YYSYS_LOG(WARN, "failed to add location, err=%d", ret);
    }
    else if (OB_SUCCESS != (ret = aggr_.scan(*scan_param, scanner)))
    {
      YYSYS_LOG(WARN, "scan error, err=%d range=%s", ret, to_cstring(search_range));
    }
    else
    {
      scanner.reset_row_iter();
      read_scanner_row_count_ = 0;
      YYSYS_LOG(DEBUG, "scan meta table succ, range=%s", to_cstring(search_range));
    }
  }
  if (NULL != scan_param)
  {
...
int ObMetaTable3::MyIterator::next(const ObTabletMetaTableRow *&row)
{
  ...
  if (began_iterate_)
  {
    if (OB_ITER_END == (ret = scanner_.next_row())) // finished the current scanner
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get fullfilled, err=%d", ret);
      }
      else
      {
        ...
        else
        {
          if (OB_SUCCESS != (ret = scan_more()))
          {
            if (OB_ITER_END != ret)
            {
              YYSYS_LOG(WARN, "failed to scan more, err=%d", ret);
            }
          }
          else
          {
    ...
    {
      YYSYS_LOG(WARN, "scanner failed to scan next, err=%d", ret);
    }
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to scan the beginning, err=%d", ret);
    }
    else
    {
      ret = cons_next_row(row);
...
int ObMetaTable3::MyIterator::scan_more()
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to get range from scanner, err=%d", ret);
  }
  else if (last_rowkey == ObRowkey::MAX_ROWKEY) // safe check
  {
    YYSYS_LOG(DEBUG, "no more data to scan");
    ret = OB_ITER_END;
  }
  else
  {
    ...
    YYSYS_LOG(DEBUG, "scan more, read_scanner_row_count=%ld startkey=%s",
...
int ObMetaTable3::MyIterator::get_table_schema()
{
      ...
      || OB_INVALID_ID == table_schema_.table_id_)
  {
    ...
    else if (TableSchema::META != table_schema_.table_type_)
    {
      ...
      YYSYS_LOG(WARN, "wrong table type, tname=%.*s type=%d",
...
int ObMetaTable3::MyIterator::check_rowkey_obj_type(int32_t rowkey_obj_idx, const ObObj &obj) const
{
  ...
  for (int k = 0; k < table_schema_.columns_.count(); ++k)
  {
    ...
    if (rowkey_obj_idx + 1 == cschema.rowkey_id_) // rowkey_id start from 1
    {
      if (obj.get_type() != cschema.data_type_
          ...
          && (!obj.is_max_value()))
      {
        YYSYS_LOG(WARN, "invalid meta table row key, cid=%lu ctype=%d obj_type=%d",
  ...
  if (!found)
  {
    YYSYS_LOG(WARN, "rowkey obj not found in table schema, idx=%d", rowkey_obj_idx);
...
int ObMetaTable3::MyIterator::check_row(ObCellInfo *row_cells, int64_t cell_num)
{
  ...
  else
  {
    ...
    {
      ret = OB_ERR_DATA_FORMAT;
      YYSYS_LOG(WARN, "invalid meta table row, return_col_num=%ld schema_col_num=%ld",
    ...
    else
    {
      ...
      for (int64_t i = 0; i < cell_num && OB_SUCCESS == ret; ++i)
      {
        ...
        {
          ret = OB_ERR_DATA_FORMAT;
          YYSYS_LOG(WARN, "invalid meta table row, tid=%lu my_tid=%lu",
        ...
        if (0 == i)
        {
          ...
          {
            ret = OB_ERR_DATA_FORMAT;
            YYSYS_LOG(WARN, "invalid meta table row, rowkey_obj_count=%ld rowkey_col_num=%d",
                      row_cells[i].row_key_.get_obj_cnt(), table_schema_.rowkey_column_num_);
          }
          else
          {
            // check rowkey obj type
            YYSYS_LOG(DEBUG, "check meta table rowkey=%s", to_cstring(row_cells[i].row_key_));
            ...
            for (int j = 0; j < rowkey_obj_cnt; ++j)
            {
              ...
              {
                YYSYS_LOG(WARN, "rowkey column not found or invalid type, j=%d", j);
                break;
              }
            }   // end for
          }
        ...
        if (i < col_schema.get_cidx(col_schema.CF_STARTKEY_COL, 0))
        {
          ...
          {
            ret = OB_ERR_DATA_FORMAT;
            YYSYS_LOG(WARN, "invalid meta table row, col=%ld type=%d",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "start key column not found or invalid type");
          }
        }
        // check indexed_tid
        if (col_schema.get_cidx(col_schema.CF_TID, 0) == i)
        {
          ...
          {
            ret = OB_ERR_DATA_FORMAT;
            YYSYS_LOG(WARN, "invalid meta table content, col_tid=%ld", int_val);
          }
        }
      } // end for
    }
...
int ObMetaTable3::MyIterator::cons_next_row(const ObTabletMetaTableRow *&row)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to get row in scanner, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = check_row(row_cells, cell_num)))
  {
    YYSYS_LOG(WARN, "invalid first tablet entry row, err=%d", ret);
  }
  else
  {
    ObMetaTableColumnSchema col_schema(table_schema_);
    ...
    for (int i = 0; i < curr_row_.get_max_replica_count(); ++i)
    {
      ...
      {
        // @todo ipv6
        YYSYS_LOG(WARN, "ipv6 not used yet");
      }
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "too many rowkey columns, num=%d", table_schema_.rowkey_column_num_);
    }
    else
    {
      ...
      {
        YYSYS_LOG(DEBUG, "finished iterate");
        ret = OB_ITER_END;
      }
      else
      {
...
int ObMetaTable3::MyIterator::rewind()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to rewind the meta iterator, err=%d", ret);
  }
  else
  {
    began_iterate_ = false;
                         ...
                         const uint64_t indexed_tid, const ObString &indexed_tname)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "invalid table id, tid=%lu", tid);
  }
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "invalid table id, indexed_tid=%lu", indexed_tid);
  }
  else
  {
...
int ObMetaTable3::aquire_scanner(ObScanner *&scanner)
{
  ...
  if (NULL != scanner)
  {
    YYSYS_LOG(ERROR, "scanner is not NULL");
    ret = OB_ERR_UNEXPECTED;
  }
  else
  {
...
{
  YYSYS_LOG(DEBUG, "destruct meta table, #iterators=%ld", iterators_.count());
  ...
  iterators_.clear();
  YYSYS_LOG(DEBUG, "destruct meta table #scanners=%ld", scanners_.count());
...
int ObMetaTable3::check_tid(const uint64_t op_tid)
{
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "invalid table id, tid=%lu", my_tid_);
  }
  ...
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "invalid table id, tid=%lu", indexed_tid_);
  }
  else if (indexed_tid_ != op_tid)
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "the table is not indexed by this meta table, meta_tid=%lu indexed_tid=%lu op_tid=%lu",
...
int ObMetaTable3::get_table_schema(const ObString &tname, TableSchema &table_schema)
{
  ...
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "this table is not meta table, tname=%.*s type=%d",
              tname.length(), tname.ptr(), table_schema.table_type_);
  }
  else if (0 >= table_schema.rowkey_column_num_)
  {
    ret = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "BUG rowkey column num=%d", table_schema.rowkey_column_num_);
  }
  return ret;
}
...
int ObMetaTable3::search(const KeyRange& key_range, ConstIterator *&first)
{
  ...
  {
    YYSYS_LOG(ERROR, "invalid search range, range=%s", to_cstring(key_range));
  }
  else if (NULL == my_meta_)
  {
    ret = OB_NOT_INIT;
    YYSYS_LOG(ERROR, "my_meta_ is NULL");
  }
  else if (OB_SUCCESS != (ret = check_tid(key_range.table_id_)))
  {
    YYSYS_LOG(WARN, "check tid failed, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = aquire_scanner(scanner)))
  {
    YYSYS_LOG(WARN, "failed to get scanner, err=%d", ret);
  }
  else if (OB_SUCCESS != (ret = aquire_iterator(*my_meta_, key_range, *scanner, it)))
  {
    YYSYS_LOG(WARN, "failed to aquire iterator, err=%d", ret);
  }
  else
  {
    first = it;
...
int ObMetaTable3::insert(const Value &v)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid table id=%lu", v.get_tid());
  }
  else if (OB_SUCCESS != (ret = get_table_schema(my_tname_, table_schema)))
  {
    YYSYS_LOG(WARN, "failed to get table schema, err=%d tname=%.*s",
...
int ObMetaTable3::erase(const Key &k)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid table id=%lu", k.table_id_);
  }
  else if (OB_SUCCESS != (ret = mutator_.del_row(my_tid_, k.rowkey_)))
  {
    YYSYS_LOG(WARN, "failed to add into mutator, err=%d", ret);
  }
  else
  {
    YYSYS_LOG(DEBUG, "erase row, my_tid=%lu indexed_tid=%lu rowkey=%s",
...
int ObMetaTable3::commit()
{
  ...
  {
    YYSYS_LOG(WARN, "failed to apply mutator, err=%d", ret);
  }
  else
  {
    mutator_.reset();
    YYSYS_LOG(WARN, "directly call meta table's commit, tid=%lu", my_tid_);
  }
  return ret;
}
