  ...
  namespace common
  {
    ...
    struct ObTabletMetaTableRow
    {
      public:
        ...
        {
          this->tid_ = tid;
          modified_mask_ |= MODIFIED_MASK_TID;
        }
        const ObString& get_table_name() const
        { return this->table_name_; }
        void set_table_name(const ObString &table_name)
        { this->table_name_ = table_name; } // @note no modified mask on purpose
        const common::ObRowkey& get_end_key() const
        { return this->end_key_; }
        void set_end_key(const common::ObRowkey &end_key)
        { this->end_key_ = end_key;}
        const common::ObRowkey &get_start_key() const
        { return this->start_key_; }
        void set_start_key(const common::ObRowkey &start_key)
        {
          this->start_key_ = start_key;
          modified_mask_ |= MODIFIED_MASK_STARTKEY;
        }
        const ObTabletReplica& get_replica(const int32_t i) const
        { return this->replicas_[i]; }
        void set_replica(const int32_t i, const ObTabletReplica &replica)
        {
          this->replicas_[i] = replica;
        ...
        NEED_SERIALIZE_AND_DESERIALIZE;
        // 
        static int32_t get_max_replica_count()
