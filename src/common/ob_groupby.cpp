...
int oceanbase::common::ObAggregateColumn::calc_aggregate_val(ObObj & aggregated_val, const ObObj & new_val)const
{
  ...
  switch (func_type_)
  {
    case SUM:
      ...
      else if (ObNullType != new_val.get_type())
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"calculate aggregate value error [aggregated_val.type:%d,mutation.type:%d]",
                      aggregated_val.get_type(), mutation.get_type());
          }
        }
        else
        {
          YYSYS_LOG(WARN,"fail to set add property of mutation [mutation.type:%d]", mutation.get_type());
        }
      }
      break;
    case COUNT:
    ...
    case LISTAGG:
      ...
      //add f20150104:e
    default:
      YYSYS_LOG(WARN,"unsupported aggregate function type [type:%d]", func_type_);
...
int oceanbase::common::ObAggregateColumn::to_str(char *buf, int64_t buf_size, int64_t &pos)const
{
  ...
  {
    YYSYS_LOG(WARN,"argument error [buf:%p,buf_size:%ld, pos:%ld]", buf, buf_size, pos);
                                        ...
                                        const int64_t row_beg, const int64_t row_end, const int32_t type)
{
  ...
  {
    YYSYS_LOG(WARN,"param error, unknow key type [type:%d]", type);
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"param error [row_beg:%ld,row_end:%ld,cell_array.get_cell_size():%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to calc hash value of current key [err:%d]", err);
    }
  }
  if (OB_SUCCESS != err)
  {
...
bool oceanbase::common::ObGroupKey::equal_to(const ObGroupKey &other) const
{
  ...
  {
    YYSYS_LOG(WARN,"compared group key have different groupby param, "
  ...
  else if (hash_val_ == other.hash_val_)
  {
    ...
    for (i = 0; i < groupby_col.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"unrecogonized key type [type:%d]", key_type_);
        break;
      }
      ...
      {
        YYSYS_LOG(WARN,"unrecogonized key type [type:%d]", other.key_type_);
        break;
      }

      if (this_idx < 0 || that_idx < 0)
      {
        YYSYS_LOG(WARN,"unexpected error this_idx=%ld, that_idx=%ld",
                  this_idx, that_idx);
        break;
      }
      else
...
int64_t oceanbase::common::ObGroupKey::to_string(char* buffer, const int64_t length) const
{
  ...
  for (int64_t i = 0; i < group_by_columns.get_array_index(); i++)
  {
    ...
    {
      YYSYS_LOG(WARN,"param error [max_idx:%ld,groupby_column_idx:%ld,as_column_idx:%ld]", max_idx, i,
...
int oceanbase::common::ObGroupByParam::add_groupby_column(const ObString &column_name, bool is_return)
{
  ...
  if ((OB_SUCCESS == err) && deep_copy_args_)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to store column name into local buffer [err:%d]", err);
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    if (!group_by_columns_.push_back(group_by_column))
    {
      ...
      YYSYS_LOG(WARN,"fail to push_back group by column [err:%d]", err);
    }
    else
    {
      column_num_ ++;
  ...
  {
    YYSYS_LOG(WARN,"fail to add return info");
    err = OB_ARRAY_OUT_OF_RANGE;
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::add_return_column(const ObString & column_name, bool is_return)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      err =  OB_ARRAY_OUT_OF_RANGE;
      YYSYS_LOG(WARN,"fail to push_back return column [err:%d]", err);
    }
    else
    {
      column_num_ ++;
                                                            ...
                                                            const ObAggregateFuncType aggregate_func, bool is_return)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"param error, unrecogonized aggregate function type [type:%d]", aggregate_func);
      err = OB_INVALID_ARGUMENT;
    }
  }
  if (OB_SUCCESS == err && deep_copy_args_)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to store org_column_name to local buffer [err:%d]", err);
    }
  }
  if (OB_SUCCESS == err && deep_copy_args_)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to store as_column_name to local buffer [err:%d]", err);
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      err =  OB_ARRAY_OUT_OF_RANGE;
      YYSYS_LOG(WARN,"fail to push_back aggregate_column [err:%d,column_num_:%ld]", err, column_num_);
    }
    else
    {
      column_num_ ++;
...
int oceanbase::common::ObGroupByParam::malloc_composite_columns()
{
  ...
  if (NULL == groupby_comp_columns_buf_)
  {
    YYSYS_LOG(WARN,"fail to allocate memory");
...
int oceanbase::common::ObGroupByParam::add_column(const ObString & expr, const ObString & as_name, bool is_return)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to copy expr to local buffer [err:%d]", err);
  }
  if ((OB_SUCCESS == err)
      && deep_copy_args_
      && (OB_SUCCESS != (err = buffer_pool_.write_string(as_name, &stored_as_name))))
  {
    YYSYS_LOG(WARN,"fail to copy as_name to local buffer [err:%d]", err);
  }
  ObCompositeColumn comp_column;

  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = comp_column.set_expression(stored_expr, stored_as_name))))
  {
    YYSYS_LOG(WARN,"fail to set expression [err:%d,expr:%.*s, as_name:%.*s]", err,
  ...
  {
    YYSYS_LOG(WARN,"groupby composite column list is full");
    err = OB_ARRAY_OUT_OF_RANGE;
  }

  if ((OB_SUCCESS == err) && (!cc_return_infos_.push_back(is_return)))
  {
    YYSYS_LOG(WARN,"groupby return info list is full");
    err = OB_ARRAY_OUT_OF_RANGE;
  }
  if (OB_SUCCESS == err)
  {
...
int oceanbase::common::ObGroupByParam::add_column(const ObObj *expr, bool is_return)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to set expression [err:%d]", err);
  }
  if ((OB_SUCCESS == err) && (!groupby_comp_columns_.push_back(comp_column)))
  {
    YYSYS_LOG(WARN,"groupby composite column list is full");
...
int oceanbase::common::ObGroupByParam::add_having_cond(const ObString & expr)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to copy as column name [err:%d]", err);
  }

  if ((OB_SUCCESS == err) && deep_copy_args_ && (OB_SUCCESS != (err = buffer_pool_.write_string(expr, &stored_expr))))
  {
    YYSYS_LOG(WARN,"fail to copy expr [err:%d]", err);
  }

  if ((OB_SUCCESS == err ) && (OB_SUCCESS != (err = add_column(stored_expr,as_name,false))))
  {
    YYSYS_LOG(WARN,"fail to add composite column [err:%d,expr:%.*s]", err, stored_expr.length(), stored_expr.ptr());
  }
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = add_having_cond(as_name,NE, false_obj))))
  {
    YYSYS_LOG(WARN,"fail to add condition [err:%d]", err);
  }
  return err;
}
                                                       ...
                                                       const ObObj & cond_value)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to copy colum_name to local buffer [err:%d]", err);
  }
  if ((OB_SUCCESS == err) && deep_copy_args_ && (OB_SUCCESS != (err = buffer_pool_.write_obj(cond_value,&stored_cond_value))))
  {
    YYSYS_LOG(WARN,"fail to copy cond_value to local buffer [err:%d]", err);
  }
...
int oceanbase::common::ObGroupByParam::calc_group_key_hash_val(const ObGroupKey & key, uint32_t &val)
{
  ...
  {
    YYSYS_LOG(WARN,"param error [key.get_cell_array():%p,key.get_groupby_param():%p]",
  ...
  if (OB_SUCCESS == err)
  {
    switch (key.get_key_type())
    {
      ...
      case ObGroupKey::ORG_KEY:
        ...
        break;
      default:
        YYSYS_LOG(WARN,"param error, unregonized key type [key.get_key_type():%d]", key.get_key_type());
        err = OB_INVALID_ARGUMENT;
    }
  }
  val = hash_val;
...
bool oceanbase::common::ObGroupByParam::is_equal(const ObGroupKey & left, const ObGroupKey &right)
{
      ...
      )
  {
    ...
    YYSYS_LOG(WARN,
              ...
              "]",
              left.get_cell_array(),
              left.get_groupby_param(),
              left.get_row_beg(),
              left.get_row_end(),
              right.get_cell_array(),
              right.get_groupby_param(),
              right.get_row_beg(),
              right.get_row_end()
              );
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"invalid agument [left.get_cell_array()->get_cell_size():%ld,right.get_cell_array()->get_cell_size():%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == err && result && i < param->group_by_columns_.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"unexpected error [left_idx:%ld,right_idx:%ld]", left_idx, right_idx);
...
int64_t oceanbase::common::ObGroupByParam::get_target_cell_idx(const ObGroupKey & key, const int64_t groupby_idx)const
{
  ...
  {
    YYSYS_LOG(WARN,"param error [groupby_idx:%ld,group_by_columns_.size():%ld]", groupby_idx, group_by_columns_.get_array_index());
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    switch (key.get_key_type())
    {
      ...
      case ObGroupKey::ORG_KEY:
        ...
        break;
      default:
        YYSYS_LOG(WARN,"unrecogonized key type [type:%d]", key.get_key_type());
        err = OB_INVALID_ARGUMENT;
    }
    if (OB_SUCCESS == err)
    {
      if (idx > max_idx)
      {
        YYSYS_LOG(WARN,"unexpected error [idx:%ld,row_beg:%ld,row_end:%ld]", idx, key.get_row_beg(), key.get_row_end());
      }
      else
      {
        idx += key.get_row_beg();
  ...
  if (OB_SUCCESS == err)
  {
    if (idx >= key.get_cell_array()->get_cell_size())
    {
      ...
      YYSYS_LOG(WARN,"unexpected error, get idx bigger than size of cell array");
    }
    else
    {
      err = idx;
                                                                   ...
                                                                   const int64_t row_end, uint32_t &val)const
{
  ...
  {
    YYSYS_LOG(WARN,"param error [row_beg:%ld,row_end:%ld, cells.get_cell_size():%ld]", row_beg, row_end,
              cells.get_cell_size());
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < group_by_columns_.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"param error [max_idx:%ld,group_by_column_idx:%ld,org_column_idx:%ld]", max_idx, i,
                                                 ...
                                                 const int64_t aggregate_row_beg, const int64_t aggregate_row_end)const
{
  ...
  {
    YYSYS_LOG(WARN,"param error [expected_aggregate_row_width:%ld,real_aggregate_row_width:%ld]",
  ...
  if (OB_SUCCESS == err && first_row_of_group) /// append groupby return and aggregate columns
  {
    ...
    for (int64_t i = 0;  OB_SUCCESS == err && i < group_by_columns_.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"unexpected error [groupby_column_idx:%ld,org_column_idx:%ld,max_org_idx:%ld]",
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to append group by columns into aggregate cell array [err:%d]", err);
        }
      }
    }
    ...
    for (int64_t i = 0;  OB_SUCCESS == err && i < return_columns_.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"unexpected error [return_column_idx:%ld,org_column_idx:%ld,max_org_idx:%ld]",
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to append return columns into aggregate cell array [err:%d]", err);
        }
      }
    }
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < aggregate_columns_.get_array_index(); i++)
    {
      ...
      {
        YYSYS_LOG(WARN,"unexpected error [aggregate_column_idx:%ld,org_column_idx:%ld,max_org_idx:%ld]",
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to calc first aggregate value of aggregate cell [aggregeate_idx:%ld]", i);
        }
        else
        {
          first_agg_cell.row_key_ = fake_agg_rowkey;
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to append aggregate columns into aggregate cell array [err:%d]", err);
          }
        }
      }
    }
  ...
  if (OB_SUCCESS == err && !first_row_of_group)
  {
    ...
    for (int64_t i = 0;  OB_SUCCESS == err && i < aggregate_columns_.get_array_index(); i++)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to aggregate value [err:%d,org_idx:%ld,as_idx:%ld,func_type:%d]",
...
int oceanbase::common::ObGroupByParam::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
{
  ...
  {
    YYSYS_LOG(WARN,"return info count not coincident with total columns [return_info_count:%ld, column_num_:%ld]",
              return_infos_.get_array_index(), column_num_);
    err = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    err = static_cast<int>(serialize_helper(buf,buf_len,pos)); //modify for [build warning]
    if (0 <= err)
    {
      err = OB_SUCCESS;
    }
  ...
  while (OB_SUCCESS == err && pos < buf_len)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (using_id_)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to get int from obj [err:%d]", err);
        }
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = add_groupby_column(int_val))))
        {
          YYSYS_LOG(WARN,"fail to add groupby column [err:%d]", err);
        }
    ...
    {
      YYSYS_LOG(WARN,"fail to deserialize groupby column [column_idx:%ld,err:%d]", group_by_columns_.get_array_index(),err);
    }
  }
  return err;
}
  ...
  while (OB_SUCCESS == err && pos < buf_len)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (using_id_)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to add return column [err:%d]", err);
        }
        else if (OB_SUCCESS == err)
        {
    ...
    {
      YYSYS_LOG(WARN,"fail to deserialize return column [column_idx:%ld,err:%d]", return_columns_.get_array_index(),err);
    }
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::deserialize_aggregate_columns(const char *buf, const int64_t buf_len, int64_t & pos)
{
  ...
  while (OB_SUCCESS == err && pos < buf_len)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to get int value from obj [err:%d]", err);
      }
      /// as column name
      if (OB_SUCCESS == err)
      {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to get varchar from obj [err:%d]", err);
        }
      }
      /// org info
      if (OB_SUCCESS == err)
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to add aggregate column [idx:%ld,err:%d]", column_num_, err);
        }
        else
        {
    ...
    {
      YYSYS_LOG(WARN,"fail to deserialize aggregate column [column_idx:%ld,err:%d]", return_columns_.get_array_index(),err);
    }
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::deserialize_comp_columns(const char *buf, const int64_t buf_len, int64_t &pos)
{
  ...
  while ((OB_SUCCESS == err) && (pos < buf_len))
  {
    if ((OB_SUCCESS != (err = comp_column.deserialize(buf, buf_len, pos)))
        && (OB_UNKNOWN_OBJ != err))
    {
      YYSYS_LOG(WARN,"fail to decode composite column [err:%d]", err);
    }
    ...
    if ((OB_SUCCESS == err) && (!groupby_comp_columns_.push_back(comp_column)))
    {
      YYSYS_LOG(WARN,"composite column list is full");
      err = OB_ARRAY_OUT_OF_RANGE;
    }
    else if (OB_SUCCESS == err)
    {
...
int oceanbase::common::ObGroupByParam::deserialize_return_info(const char *buf, const int64_t buf_len, int64_t &pos)
{
       ...
       i++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to deserialize obj [err:%d]", err);
    }
    ...
    {
      YYSYS_LOG(WARN,"fail to get bool val from obj [err:%d,type:%d]", err, obj.get_type());
    }
...
int oceanbase::common::ObGroupByParam::deserialize_having_condition(const char *buf, const int64_t buf_len, int64_t &pos)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to deserialize having condition [err:%d]", err);
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::deserialize(const char* buf, const int64_t data_len, int64_t& pos)
{
  ...
  {
    YYSYS_LOG(WARN,"param error [buf:%p,data_len:%ld]", buf,data_len);
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
  ...
  ///   {
  ///     err = OB_INVALID_ARGUMENT;
  ///     YYSYS_LOG(WARN,"there is no aggregate column, but has groupby or return column");
  ///   }
  /// }
      ...
      && (column_num_ > 0))
  {
    for (int64_t idx = 0; idx < group_by_columns_.get_array_index() && OB_SUCCESS == err; idx ++)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to add default return info");
        err  = OB_ARRAY_OUT_OF_RANGE ;
      }
    }
...
int64_t oceanbase::common::ObGroupByParam::groupby_columns_serialize_helper(char* buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  for (int64_t i = 0; OB_SUCCESS == err && i < group_by_columns_.get_array_index(); i++)
  {
    ...
    if (using_name_)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"argument error, using only name [group_by_column_idx:%ld]", i);
      }
      else
      {
    ...
    else
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"argument error, using only id [group_by_column_idx:%ld]", i);
      }
      else
      {
...
int64_t oceanbase::common::ObGroupByParam::return_columns_serialize_helper(char* buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  for (int64_t i = 0; OB_SUCCESS == err && i < return_columns_.get_array_index(); i++)
  {
    ...
    if (using_name_)
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"argument error, using only name [return_column_idx:%ld]", i);
      }
      else
      {
    ...
    else
    {
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"argument error, using only id [return_column_idx:%ld]", i);
      }
      else
      {
  ...
  for (int64_t i = 0; OB_SUCCESS == err && i < aggregate_columns_.get_array_index(); i++)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (using_name_)
      {
        ...
        {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN,"argument error, using only name [aggregate_column_idx:%ld]", i);
        }
        else
        {
      ...
      else
      {
        ...
        {
          err = OB_INVALID_ARGUMENT;
          YYSYS_LOG(WARN,"argument error, using only id [aggregate_column_idx:%ld]", i);
        }
        else
        {
...
int oceanbase::common::ObGroupByParam::comp_columns_serialize_helper(char* buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  if (groupby_comp_columns_.get_array_index() > 0)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to serialize GROUPBY_CLAUSE_COMP_COLUMN_FIELD ext obj [err:%d]", err);
    }
  }
  for (int64_t idx = 0; OB_SUCCESS == err && idx < groupby_comp_columns_.get_array_index(); idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to serialize composite column [idx:%ld, err:%d]", idx, err);
    }
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::return_info_serialize_helper(char* buf, const int64_t buf_len, int64_t & pos) const
{
  ...
  if (return_infos_.get_array_index() > 0)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to serialize GROUPBY_CLAUSE_RETURN_INFO_FIELD [err:%d]", err);
    }
  }
  ObObj bool_obj;
  for (int64_t idx = 0; OB_SUCCESS == err && idx < return_infos_.get_array_index(); idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to serialize groupby return info [idx:%ld,err:%d]", idx, err);
    }
  }
  return err;
}
...
int oceanbase::common::ObGroupByParam::having_condition_serialize_helper(char* buf, const int64_t buf_len, int64_t & pos)const
{
  ...
  if (condition_filter_.get_count() > 0)
  {
    ...
    if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = obj.serialize(buf, buf_len, pos))))
    {
      YYSYS_LOG(WARN,"fail to serialize GROUPBY_CLAUSE_HAVING_FIELD ext field [err:%d]", err);
    }

    if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = condition_filter_.serialize(buf, buf_len, pos))))
    {
      YYSYS_LOG(WARN,"fail to serialize having condition [err:%d]", err);
    }
  }
  return err;
}
...
int64_t oceanbase::common::ObGroupByParam::serialize_helper(char* buf, const int64_t buf_len, int64_t& pos) const
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN,"there is only composite column, cannot calculate");
  }
  if (OB_SUCCESS == err)
  {
    err = static_cast<int>(groupby_columns_serialize_helper(buf,buf_len,pos));//modify for [build warning]
    if (0 <= err)
    {
      need_size += err;
      err = OB_SUCCESS;
    ...
    if (OB_SUCCESS == err)
    {
      err = static_cast<int>(return_columns_serialize_helper(buf,buf_len,pos));//modify for [build warning]
      if (0 <= err)
      {
        need_size += err;
        err = OB_SUCCESS;
    ...
    if (OB_SUCCESS == err)
    {
      err = static_cast<int>(aggregate_columns_serialize_helper(buf,buf_len,pos));//modify for [build warning]
      if (0 <= err)
      {
        need_size += err;
        err = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(WARN,"fail to serialize composite columns [err:%d]", err);
  }
...
int  oceanbase::common::ObGroupByParam::get_aggregate_column_name(const int64_t column_idx, ObString & column_name)const
{
  ...
  {
    YYSYS_LOG(WARN,"agument error [column_idx:%ld,column_num_:%ld]", column_idx, column_num_);
    err = OB_ARRAY_OUT_OF_RANGE;
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (column_count_returned < column_idx + 1)
    {
      YYSYS_LOG(WARN,"[column_num_need_return:%ld,column_idx:%ld]", column_count_returned, column_idx);
      err = OB_ARRAY_OUT_OF_RANGE;
    }
  }
  if (OB_SUCCESS == err)
...
int oceanbase::common::ObGroupByParam::safe_copy(const ObGroupByParam & other)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to deep copy condition filter");
  }
  ...
  if ((OB_SUCCESS == err) && (NULL == this_comp_columns) && (NULL != other.groupby_comp_columns_buf_))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to allocate composite columns [err:%d]", err);
    }
    else
    {
