  ...
  namespace common
  {
    ...
    class IRLock
    {
      ...
      public:
        ...
        void debug();
      protected:
        inline int64_t update_ref(uint64_t pos, int64_t n)
        { return __sync_add_and_fetch(&refs_[(pos>>bs_shift_) % bcount_], n); }
        inline int64_t get_ref(uint64_t pos)
        { return refs_[(pos>>bs_shift_) % bcount_]; }
      private:
        uint64_t bs_shift_;
        uint64_t bcount_;
