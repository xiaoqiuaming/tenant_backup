...
namespace oceanbase
{
  namespace common
  {
    ...
    int ObLogWriterV2::check_state() const
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_inited()=>%d", err);
      }
      else if (!log_cursor_.is_valid())
      {
        YYSYS_LOG(ERROR, "log_cursor[%s].is_valid()=>false", log_cursor_.to_str());
      }
      return err;
    }
    ...
    int ObLogWriterV2::init(const char* log_dir, const int64_t align_mask, int64_t log_sync_type)
    {
      ...
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "init twice");
      }
      else if (NULL == log_dir || (align_mask & (align_mask + 1)))
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "Parameter are invalid[log_dir=%p, align_mask=%ld]", log_dir, align_mask);
      }
      else if (NULL == (log_dir_ = strndup(log_dir, OB_MAX_FILE_NAME_LENGTH)))
      {
        YYSYS_LOG(ERROR, "strdup()=>NULL, log_dir=%s", log_dir);
      }
      else
      {
    ...
    int ObLogWriterV2::reset()
    {
      ...
      {
        YYSYS_LOG(ERROR, "check_inner_stat()=>%d", err);
      }
      else
      {
        if (file_.is_opened())
                                  ...
                                  const bool dio, const bool is_trunc)
    {
      ...
      {
        YYSYS_LOG(ERROR, "create \"%s\" directory error[%s]", log_dir, strerror(errno));
      }
      else if ((len = snprintf(file_name, OB_MAX_FILE_NAME_LENGTH, "%s/%lu", log_dir, log_file_id)) <= 0
               && len >= OB_MAX_FILE_NAME_LENGTH)
      {
        err = OB_ERROR;
        YYSYS_LOG(ERROR, "generate_file_name(len=%ld, log_dir=%s, log_file_id=%ld)=>%d", len, log_dir, log_file_id, err);
      }
      ...
      {
        YYSYS_LOG(ERROR, "open commit log file[file_name=%s] ret=%d", file_name, err);
      }
      return err;
    }
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "start_log(log_cursor=%s)", log_cursor.to_str());
      if (OB_SUCCESS != (err = check_inited()))
      {
        YYSYS_LOG(ERROR, "check_inner_stat()=>%d", err);
      }
      ...
      {
        err = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "log_cursor[%s].is_valid()=>false", log_cursor.to_str());
      }
      else if (log_cursor_.is_valid())
      {
        err = OB_INIT_TWICE;
        YYSYS_LOG(ERROR, "ObLogWriter is init already init, log_cursor_=%s", log_cursor_.to_str());
      }
      else if (OB_SUCCESS != (err = open_log_file_func(file_, log_dir_, log_cursor.file_id_, dio_, false)))
      {
        YYSYS_LOG(ERROR, "open_log_file_ error[ret=%d]", err);
      }
      else if (-1 == file_.get_file_pos())
      {
        err = OB_ERR_SYS;
        YYSYS_LOG(ERROR, "file_.get_file_pos(): %s", strerror(errno));
      }
      else if (log_cursor.log_id_ != 0 && file_.get_file_pos() != log_cursor.offset_)
      {
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "log file is broken");
      }
      else if (0 != (log_cursor.offset_ & align_mask_))
      {
        err = OB_LOG_NOT_ALIGN;
        YYSYS_LOG(WARN, "start_log(%s): LOG_NOT_ALIGNED", to_cstring(log_cursor));
      }
      else
      {
                                    ...
                                    const int64_t old_file_id, const int64_t new_file_id, const bool dio, const bool is_trunc)
    {
      ...
      if (old_file_id  != new_file_id)
      {
        ...
        {
          YYSYS_LOG(ERROR, "open_log_file(new_file_id=%ld)=>%d", new_file_id, err);
        }
        else
        {
          YYSYS_LOG(INFO, "open_log_file(new_file_id=%ld)", new_file_id);
        }
      }
      else
      {} // do nothing
    ...
    int ObLogWriterV2::write_log(const char* log_data, int64_t data_len)
    {
      ...
      {
        err = OB_LOG_NOT_ALIGN;
        YYSYS_LOG(ERROR, "write_log(data_len=%ld): NOT_ALIGN", data_len);
      }
      ...
      {
        YYSYS_LOG(WARN, "buf_len[%ld] not aligned align_mask=%lx", data_len, align_mask_);
      }
      ...
      {
        YYSYS_LOG(ERROR, "file_.append(log_data=%p, data_len=%ld, sync_type=%ld)=>%d", log_data, data_len, log_sync_type_, err);
      }
      else if (OB_SUCCESS != (err = open_log_file_maybe_(file_, log_dir_, log_cursor_.file_id_, new_cursor.file_id_, dio_, true)))
      {
        YYSYS_LOG(ERROR, "open_log_file_maybe(old_id=%ld, new_id=%ld)=>%d", log_cursor_.file_id_, new_cursor.file_id_, err);
      }
      else
      {
