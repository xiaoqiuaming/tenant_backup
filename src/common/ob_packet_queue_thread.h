...
namespace oceanbase
{
  namespace common
  {
    class ObPacketQueueThread : public yysys::CDefaultRunnable
    {
      public:
        ...
        */
        ...
        int prepare_for_next_request(int64_t session_id, int32_t conn_seq=0);//[701]
        ...
        //[701]-b
        bool wakeup_next_thread(int32_t conn_seq);
        int disconn_clear(int32_t conn_seq, int64_t ref);
        //[701]-e
      ...
      private:
        struct WaitObject
        {
          ...
          int32_t conn_seq_;
          int errno_;
          WaitObject() : thread_no_(0), packet_(NULL),session_end_(false), conn_seq_(0), errno_(0)
          {}
          WaitObject(int64_t no, ObPacket* p) : thread_no_(no), packet_(p),session_end_(false), conn_seq_(0), errno_(0)
          {}
          void end_session()
          { session_end_ = true; }
          bool is_session_end()const
          { return session_end_; }
        };
        ...
        yysys::CThreadCond next_cond_[MAX_THREAD_COUNT];
