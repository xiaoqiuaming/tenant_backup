      ...
      public:
        int init(const int64_t capacity, const int64_t wait_time_us = DEFAULT_WAIT_TIME_US)
        {
          ...
          if (0 >= capacity || !is2n(capacity) || wait_time_us < 0)
          {
            ...
            YYSYS_LOG(ERROR, "init(capacity=%ld, wait_time_us=%ld):INVALID_ARGUMENT", capacity, wait_time_us);
          }
          else if (NULL != buf_)
          {
            err = OB_INIT_TWICE;
          ...
          {
            err = OB_ERROR;
            YYSYS_LOG(ERROR, "memory not aligned buf_=%p", buf_);
          }
          else if (OB_SUCCESS != (err = free_.init(capacity, buf_)))
          {
            YYSYS_LOG(ERROR, "free_.init(capacity=%ld)=>%d", capacity, err);
          }
          else if (OB_SUCCESS != (err = queue_.init(capacity, buf_ + item_size * capacity)))
          {
            YYSYS_LOG(ERROR, "queue_.init(capacity=%ld)=>%d", capacity, err);
          }
