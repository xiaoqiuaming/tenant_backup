  ...
  const FieldArray &fields, ObNewScanner &scanner)
{
  ...
  YYSYS_LOG(DEBUG, "%s", to_cstring(fields));
  ...
  while (OB_SUCCESS == (ret = scanner.get_next_row(row)))
  {
         ...
         cell_idx++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "fail to get cell, ret = [%d]", ret);
        break;
      }
      else if (cell->get_type() == ObVarcharType)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get name column error, column = [%.*s],"
      ...
      else if (cell->get_type() == ObIntType)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get column error, obj: %s, ret = [%d]",
                    to_cstring(*cell), ret);
          break;
        }
        else if (OB_SUCCESS != (ret = key.set_int(column_name, intval)))
        {
          YYSYS_LOG(ERROR, "set int key error, ret = [%d]", ret);
          break;
        }
      }
      else if (cell->get_type() == ObModifyTimeType)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get modify time error, column: [%.*s], ret: [%d]",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "mismatch column type, column: %.*s",
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if(hash::HASH_INSERT_SUCC != hash_ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "sys config insert repeatly! key name: [%s]",
                    key.name());
        }
        else
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "sys config map set fail, ret = [%d],"
    ...
    {
      YYSYS_LOG(WARN, "ignore the row: %s", to_cstring(row));
      break;
    }
  }
...
int ObSystemConfig::dump2file(const char *path) const
{
  ...
  else
  {
    ...
    for ( ;it != last; it++ )
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to dump config to file: [%s], msg: [%s]",
                  path, strerror(errno));
        ret = OB_ERROR;
      }
...
int ObSystemConfig::reload(FILE *fp)
{
  ...
  if (NULL == fp)
  {
    YYSYS_LOG(ERROR, "Got NULL file pointer!");
    ret = OB_ERROR;
  }
  else
  {
    ...
    while (1)
    {
      if (1 != (cnt = fread(&key, sizeof (key), 1, fp)))
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to read config from file, msg: [%s]",
                    strerror(errno));
          ret = OB_ERROR;
        }
      }
      ...
      else
      {
        ...
        if(hash::HASH_INSERT_SUCC != hash_ret)
        {
          if(hash::HASH_EXIST == hash_ret)
          {
            YYSYS_LOG(WARN, "system config insert repeatly! key name: [%s]",
                      key.name());
          }
          else
          {
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "system config map set fail, ret = [%d],"
                      " key name = [%s]", ret, key.name());
          }
        }
      }
                               ...
                               const int32_t &def) const
{
  ...
  if (OB_SUCCESS == (ret = find_newest(key, pvalue)))
  {
    ...
    if (p == pvalue->value())
    {
      YYSYS_LOG(ERROR, "not integer, key name: [%s], value: [%s]",
                key.name(), pvalue->value());
    }
    else if (p == NULL || *p != '\0')
    {
      YYSYS_LOG(WARN, "config truncated, key name: [%s]", key.name());
    }
    else
    {
      YYSYS_LOG(INFO, "use internal conf: %s:[%d]", key.name(), value );
    }
  }
  else
  {
    value = def;
    YYSYS_LOG(INFO, "use default conf: %s:[%d]", key.name(), value);
    ret = OB_SUCCESS;
  }
  return ret;
}
                               ...
                               const int64_t &def) const
{
  ...
  if (OB_SUCCESS == (ret = find_newest(key, pvalue)))
  {
    ...
    {
      YYSYS_LOG(INFO, "use internal conf: %s:[%ld]", key.name(), value );
    }
  }
  else
  {
    value = def;
    YYSYS_LOG(INFO, "use default conf: %s:[%ld]", key.name(), value );
    ret = OB_SUCCESS;
  }
  return ret;
}
                             ...
                             const char* def) const
{
  ...
  if (OB_SUCCESS == (ret = find_newest(key, pvalue)))
  {
    ...
    {
      YYSYS_LOG(ERROR, "reload %s error", key.name());
    }
    else if (wlen >= len)
    {
      YYSYS_LOG(WARN, "config [%s] truncated", key.name());
    }
    else
    {
      YYSYS_LOG(INFO, "use internal conf: %s:[%.*s]", key.name(), static_cast<int>(len - 1), buf);
    }
    /* for safe */
    buf[len - 1] = '\0';
  }
  else
  {
    ...
    YYSYS_LOG(INFO, "use default conf: %s:[%s]", key.name(), def);
    ret = OB_SUCCESS;
  }
  return ret;
}
