  ...
  namespace common
  {
    ...
    class ObMalloc : public ObIAllocator
    {
      ...
      public:
        ...
        { return ob_malloc(sz, mod_id_); };
        void free(void *ptr)
        {ob_free(ptr);};
        void* mod_alloc(const int64_t size, const int32_t mod_id)
        { return ob_malloc(size, mod_id); }
        void mod_free(void* p, const int32_t mod_id)
        { return ob_free(p, mod_id); }
      private:
        int32_t mod_id_;
    };
