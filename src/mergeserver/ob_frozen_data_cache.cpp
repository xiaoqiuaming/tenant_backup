...
int64_t ObFrozenDataKey::to_string(char * buf, const int64_t buf_len) const
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "YaoSqlReadParam deserialize error, err: %d", err);
...
int ObFrozenDataKeyBuf::alloc_buf(int64_t bl)
{
  ...
  if (bl > buf_len || NULL == buf)
  {
    ...
    if (NULL == buf)
    {
      YYSYS_LOG(ERROR, "ob_tc_malloc failed");
...
int ObCachedFrozenData::get_next_row(const common::ObRow * & row)
{
  ...
  if (!has_data())
  {
    YYSYS_LOG(ERROR, "row_store_ is NULL");
    ret = OB_ERROR;
  }
  else
  {
...
int ObFrozenDataCache::init(int64_t total_size)
{
  ...
  {
    in_use_ = false;
    YYSYS_LOG(INFO, "ObFrozenDataCache switch off");
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "total_size(%ld) is smaller than on cache item size(%ld)",
    ...
    {
      in_use_ = true;
      YYSYS_LOG(INFO, "ObFrozenDataCache init OK");
    }
  }
  return ret;
}
...
int ObFrozenDataCache::get(const ObFrozenDataKey & key, ObCachedFrozenData & value)
{
  ...
  if (!in_use_)
  {
    YYSYS_LOG(ERROR, "ObFrozenDataCache is not initialized to use");
    ret = OB_NOT_INIT;
  }
  else
  {
    ...
    {
      YYSYS_LOG(INFO, "frozen data cache cached item number: %ld", cache_.count());
    }
  }
  return ret;
}
...
int ObFrozenDataCache::revert(ObCachedFrozenData & value)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "KeyValueCache revert error, ret: %d", ret);
    }
    else
    {
      value.reuse();
