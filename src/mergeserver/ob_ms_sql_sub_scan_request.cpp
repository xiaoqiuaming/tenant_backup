...
namespace oceanbase
{
  namespace mergeserver
  {
      ...
      ObChunkServerItem & selected_server)
    {
      ...
      {
        YYSYS_LOG(WARN, "no valid chunkserver selected. [sel_replica=%d, total_tried_replica_count_=%d, last_tried_replica_idx_[=%d]", sel_replica, 
          total_replica_count_, last_tried_replica_idx_);
        err = OB_ERROR;
      }
      else
      ...
      if (false)
      {
        ...
        {
          YYSYS_LOG(INFO, "cs=%s", to_cstring(cs_replicas_[i]));
        }
        YYSYS_LOG(INFO, "total_rep=%d, tried=%d, last=%d, cur=%s",
      ...
      ObMsSqlRequest *client_request, ObChunkServerItem & selected_server)
    {
      ...
      if (false == inited_)
      {
        YYSYS_LOG(WARN, "ObMsSqlSubScanRequest class not inited. call init() first");
        err = OB_NOT_INIT;
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "release allocated ObMsSqlRpcEvent object. [request_id:%lu]", client_request->get_request_id());
        }
      }
      if (OB_SUCCESS == err)
      {
      ...
      bool &belong_to_this)
    {
      ...
      if (OB_SUCCESS == err)
      {
        if (event_id_ == agent_event->get_event_id())
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to add scanner to result:err[%d]", err);
          }
        }
        else
        {
