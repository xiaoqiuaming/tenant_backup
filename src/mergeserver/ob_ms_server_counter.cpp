...
ObMergerServerCounter::ObMergerServerCounter()
{
  ...
  if (err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "init bucket count failed:ret[%d]", err);
  }
...
int ObMergerServerCounter::init(const int64_t count, const int64_t interval)
{
  ...
  if (count <= 0)
  {
    ...
    YYSYS_LOG(WARN, "check input param failed:count[%ld]", count);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "hashmap create failed:ret[%d]", ret);
    }
    else
    {
...
void ObMergerServerCounter::dump(void) const
{
  ...
  for (;iter != counter_map_.end(); ++iter)
  {
    ...
    YYSYS_LOG(TRACE, "dump server query counter:server[%s], count[%ld]", server.to_cstring(), count);
  }
}
...
int ObMergerServerCounter::inc(const ObServer & server, const int64_t count)
{
  ...
  {
    YYSYS_LOG(WARN, "get server count failed:server[%s], ret[%d]", server.to_cstring(), ret);
  }
  // no matter succ or fail
  ret = counter_map_.set(server.get_ipv4(), server_count, 1);
  if ((ret != HASH_OVERWRITE_SUCC) && (ret != HASH_INSERT_SUCC))
  {
    YYSYS_LOG(WARN, "overwrite or insert failed:server[%s], ret[%d]", server.to_cstring(), ret);
  }
  else
  {
    ret = OB_SUCCESS;
    YYSYS_LOG(DEBUG, "update server count succ:server[%s], count[%ld]",
...
int64_t ObMergerServerCounter::get(const common::ObServer & server) const
{
  ...
  {
    ret = OB_SUCCESS;
    YYSYS_LOG(DEBUG, "get server count succ:server[%s], count[%ld]", server.to_cstring(), count);
  }
  return count;
}
...
void ObMergerServerCounter::check_reset(void)
{
  ...
  if ((reset_interval_ > 0) && (timestamp - last_reset_timestamp_> reset_interval_))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "reset counter failed:last[%ld], interval[%ld], current[%ld], ret[%d]",
