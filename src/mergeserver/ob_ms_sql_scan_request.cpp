...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    ObMsSqlSubScanRequest * ObMsSqlScanRequest::alloc_sub_scan_request()
    {
          ...
          && (total_sub_request_count_ < MAX_SUBREQUEST_NUM))
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "ob malloc failed, ret=%d", OB_ALLOCATE_MEMORY_FAILED);
          }
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "push ObMsSqlSubScanRequest* to sub_requests failed, ret=%d", ret);
      ...
      {
        YYSYS_LOG(WARN, "allocate sub scan request failed. [total_sub_request_count_=%d][finished_sub_request_count_=%d]"
    ...
    int ObMsSqlScanRequest::set_request_param(ObSqlScanParam &scan_param, const common::ObRpcScanHint & hint)
    {
      ...
      {
        YYSYS_LOG(ERROR, "unexpected error, synmatic error [sharding_limit_offset:%ld]", sharding_limit_offset);
      }

      if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = merger_operator_.set_param(scan_param))))
      {
        YYSYS_LOG(WARN, "fail to set merger_operator's param. [err=%d]", err);
      }
      ...
      {
        YYSYS_LOG(WARN,"fail to init table range iterator [err:%d]", err);
      }
      return err;
    }
    ...
    int ObMsSqlScanRequest::set_request_param_only(ObSqlScanParam &scan_param)
    {
      ...
      scan_param_ = &scan_param;
      scan_param_->get_limit().get_limit(sharding_limit_count_, sharding_limit_offset);
      YYSYS_LOG(DEBUG, "sharding_limit_count_=%ld,sharding_limit_offset=%ld", sharding_limit_count_,sharding_limit_offset);
      if (sharding_limit_offset > 0)
      {
        err = OB_ERROR ;
        YYSYS_LOG(ERROR, "unexpected error, synmatic error [sharding_limit_offset:%ld],err=%d", sharding_limit_offset,err);
      }
                                           ...
                                           uint64_t * triggered_rpc_event_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to create rpc event. [err=%d]", err);
      }
      else
      {
        if (OB_SUCCESS != (err = sub_req->add_event(rpc_event, this, selected_server)))
        {
          YYSYS_LOG(WARN, "fail to init rpc event. [err=%d]", err);
        }
        else if (NULL == (async_rpc_stub = get_rpc()))
        {
          YYSYS_LOG(WARN, "fail to get rpc");
          err = OB_ERROR;
        }
        ...
        {
          YYSYS_LOG(WARN, "fail to scan cs %s. event id %ld [err=%d]",
        ...
        if (OB_SUCCESS != err)
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to destroy rpc_event. [err=%d]", e);
          }
          else
          {
            YYSYS_LOG(INFO, "rpc_event destroyed.");
          }
        }
      }
                                             ...
                                             const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        if (OB_UNLIKELY(sub_req == NULL))
        {
          ...
          YYSYS_LOG(WARN, "sub request is null, sub_req_idx=%d, ret=%d", sub_req_idx, err);
        }
        else
        {
          ...
          for (int64_t i = 0; i < replica_count; i++)
          {
            if (only_replica.addr_ == req_cs_replicas[i].addr_)
            {
              ...
              YYSYS_LOG(DEBUG,"zhaoqiong delete ObChunkServerItem [%s]",to_cstring(req_cs_replicas[i].addr_));
              memmove(&(req_cs_replicas[i]),&(req_cs_replicas[i+1]), (replica_count - i - 1) * sizeof(ObChunkServerItem) ); //size
      ...
      {
        YYSYS_LOG(WARN,"fail to get session next from %s [err:%d,session_id:%ld,req_id:%lu]",
      ...
      YYSYS_LOG(DEBUG, "[session next] timeout_us:%ld,"
                                       ...
                                       ObTabletLocationRangeIterator &range_iter,  const int64_t timeout_us)
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        if (OB_ITER_END == err)
        {
          //YYSYS_LOG(DEBUG, "end of range iteration. break");
          err = OB_SUCCESS;
          break;
        }
        else if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to get tablet locations.");
        }
        ...
        //             YYSYS_LOG(ERROR,"wanglei::count = %ld addr =%s",count,to_cstring(replicas[i].addr_));
        //         }
        /// 2. allocate a sub scan request
        if ((OB_SUCCESS == err) && (NULL == (sub_req = alloc_sub_scan_request())))
        {
          YYSYS_LOG(WARN, "fail to allocate sub scan request");
          err = OB_ERROR;
        }
        ...
        {
          YYSYS_LOG(WARN, "fail to init SubScanRequest. [err=%d]", err);
        }
        else if (OB_SUCCESS == err)
        {
          YYSYS_LOG(DEBUG, "sub query range info: %s", to_cstring(query_range));
        }
        /// 4. send request to cs
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = send_rpc_event(sub_req, timeout_us))))
        {
          YYSYS_LOG(WARN,"fail to send rpc event of ObMsSqlSubScanRequest [err:%d]", err);
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          // YYSYS_LOG(DEBUG, "success in sending a async scan request to selected server");
        }
      ...
      YYSYS_LOG(DEBUG, "[do_request end] timeout_us:%ld, triggered_rpc_event_count:%d,"
                                                  ...
                                                  bool &belong_to_this, int32_t &idx)
    {
      ...
      for (idx = 0; idx < total_sub_request_count_; idx++)
      {
        ...
        {
          err = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "sub request is null, idx = %d, ret=%d", idx, err);
          break;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to determine if agent event finished. [err=%d]", err);
            break;
          }
    ...
    int ObMsSqlScanRequest::check_if_need_more_req(const int32_t sub_req_idx,  const int64_t timeout_us, ObMsSqlRpcEvent &prev_rpc_event, bool &is_session_end)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "scanner from %s is NULL. no result.", to_cstring(prev_rpc_event.get_server()));
          err = OB_ERR_UNEXPECTED;
        }
        if ((OB_SUCCESS == err) && (NULL == (sub_scan_param = sub_requests_[sub_req_idx]->get_scan_param())))
        {
          YYSYS_LOG(WARN, "sub scan_param is NULL. no result.");
          err = OB_ERR_UNEXPECTED;
        }
        ...
        {
          YYSYS_LOG(WARN,"fail to get fullfilled info from scanner from server %s [err:%d]",to_cstring(prev_rpc_event.get_server()), err);
        }
        else
        {
          YYSYS_LOG(DEBUG,"scanner is_req_fullfilled:%d, fullfilled_count:%ld", is_req_fullfilled, fullfilled_count);
        }
        ...
        if (OB_SUCCESS == err)
        {
          if (is_req_fullfilled)
          {
            ...
            {
              YYSYS_LOG(WARN, "fail to get scanner tablet range:ret[%d]", err);
            }
            else if (sub_requests_[sub_req_idx]->get_sub_range().compare_with_endkey2( tablet_range ) > 0)
            {
              ...
              // ר�ŵ���(3,5]
              YYSYS_LOG(DEBUG, "tablet splited, query_range:%s, tablet_range:%s",
              ...
              {
                YYSYS_LOG(WARN, "fail to deep copy rowkey:err[%d]", err);
              }
              else
              {
                ...
                if (OB_SUCCESS != (err = range_iter.initialize(
                                     ...
                                     &get_buffer_pool())))
                {
                  YYSYS_LOG(WARN,"fail to initialize range iterator [err:%d]", err);
                }
                else if (OB_SUCCESS != (err = do_request(-1, range_iter,  timeout_us)))
                {
                  YYSYS_LOG(WARN, "fail to issue new scan request [err:%d]", err);
                }
              }
            }
            else
          ...
          else if (prev_rpc_event.is_session_end())
          {
            ...
            YYSYS_LOG(INFO, "prev_rpc_event from %s return packet code prefix PCODE_PREFIX_SESSION_END, end session, no need more request.",
                                    ...
                                    timeout_us)))
          {
            ...
            YYSYS_LOG(WARN, "fail to get session next from %s [err:%d]", to_cstring(prev_rpc_event.get_server()), err);
          }
    ...
    bool ObMsSqlScanRequest::check_if_location_cache_valid_(const ObNewScanner & scanner, const ObSqlScanParam & scan_param)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get tablet range from scanner [err:%d]", err);
      }
      YYSYS_LOG(DEBUG, "tablet range:%s, param range:%s", to_cstring(tablet_range), to_cstring(*scan_param.get_range()));
          ...
          && !(tablet_range.end_key_.is_max_row()))
      {
        ...
        {
          YYSYS_LOG(WARN, "param startkey=%s, tablet endkey=%s",
          ...
          && (!tablet_range.start_key_.is_min_row()))
      {
        ...
        {
          YYSYS_LOG(WARN, "tablet startkey=%s", to_cstring(tablet_range.start_key_));
          res = false;
        }
        else if (scan_param.get_range()->start_key_ < tablet_range.start_key_)
        {
          YYSYS_LOG(WARN, "param startkey=%s, tablet startkey=%s",
                                           ...
                                           ObMsSqlRpcEvent *rpc_event, bool& finish)
    {
      ...
      if (NULL != rpc_event)
      {
        int64_t scan_event_time_cost = yysys::CTimeUtil::getTime() - rpc_event->get_timestamp();
        OB_STAT_INC(MERGESERVER, SQL_SCAN_EVENT_TIME,  scan_event_time_cost);
        OB_STAT_INC(MERGESERVER, SQL_SCAN_EVENT_COUNT);
        YYSYS_LOG(TRACE, "rpc_event finised [us_used:%ld,timeout_us:%ld,request_event_id:%lu,rpc_event_id:%lu,"
      ...
      if (true == finish)
      {
        YYSYS_LOG(WARN, "request finished already. should not call process_result() anymore!");
        err = OB_ERROR;
      }
      else if (NULL == rpc_event || NULL == scan_param_)
      {
        YYSYS_LOG(WARN, "NULL pointer error. [rpc_event=%p][scan_param=%p]", rpc_event, scan_param_);
        err = OB_ERROR;
      }
      else
      {
      ...
      //YYSYS_LOG(DEBUG, "[process result begin]");
      if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = find_sub_scan_request(rpc_event,
                                                                             belong_to_this, sub_req_idx))))
      {
        YYSYS_LOG(WARN,"fail to find SubRequest for rpc event [from:%s,rpc_event:%p,rpc_event_id:%ld,"
      ...
      if ((OB_SUCCESS == err) && (OB_SUCCESS == rpc_event->get_result_code()))
      {
        ...
        {
          YYSYS_LOG(WARN, "Unexpected. rpc event to %s not found! The framework should deal with this case. not here",
                    to_cstring(rpc_event->get_server()));
          err = OB_ERR_UNEXPECTED;
        }
        else
            ...
            && (!check_if_location_cache_valid_(rpc_event->get_result(),*sub_requests_[sub_req_idx]->get_scan_param())))
        {
          ...
          YYSYS_LOG(WARN,"location cache invalid");
        }
        if ((OB_SUCCESS == err) && (true == belong_to_this)
            && (OB_SUCCESS == rpc_event->get_result_code()))
        {
          YYSYS_LOG(DEBUG, "got first finished rpc event of [from:%s,sub_request:%d,us_used:%ld,"
          ...
          if ((OB_SUCCESS == err) && sub_requests_[sub_req_idx]->get_result()->is_finish() && (OB_SUCCESS != (err = merger_operator_.add_sharding_result(
                                                                                                                *(sub_requests_[sub_req_idx]->get_result()),
                                                                                                                ...
                                                                                                                finish))))
          {
            YYSYS_LOG(WARN, "fail to add sharding result from %s. [err=%d]", to_cstring(rpc_event->get_server()), err);
          }
      ...
      {
        YYSYS_LOG(WARN, "rpc event return code not success. [err=%d,sever:%s,range:%s,event_id:%lu]",
        ...
        {
          YYSYS_LOG(WARN,"fail to update location cache");
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "this retry not success. server:%s", to_cstring(rpc_event->get_server()));
          }
          if (OB_ITER_END == err)
          {
            YYSYS_LOG(WARN, "tried all replica and not successful. last server:%s", to_cstring(rpc_event->get_server()));
      ...
      {
          YYSYS_LOG(WARN,"fail to trigger left requests [err:%d]", err);
      }
      else
      {
      ...
      */
      ...
      if (OB_SUCCESS == err)
      {
        YYSYS_LOG(DEBUG, "finished_sub_request_count_ =%d, total_sub_request_count_=%d",
                  finished_sub_request_count_ , total_sub_request_count_);
        if (finished_sub_request_count_ > total_sub_request_count_)
        {
          YYSYS_LOG(ERROR, "unexpected sub request finish count. [finished=%d][total=%d]",
                    finished_sub_request_count_, total_sub_request_count_);
          err = OB_ERR_UNEXPECTED;
        }
      }
      ...
      //YYSYS_LOG(DEBUG, "[process result end, finish=%d]", finish);
      return err;
    }
    ...
    void ObMsSqlScanRequest::end_sessions_()
    {
      for (int32_t i = 0; (i < total_sub_request_count_); i++)
      {
        if (sub_requests_[i]->get_session_id() > ObCommonSqlRpcEvent::INVALID_SESSION_ID)
        {
          ...
          {
            YYSYS_LOG(INFO,"end unfinished session [scan_event:%lu,idx:%d,session_id:%lu]", get_request_id(),
    ...
    int ObMsSqlScanRequest::retry(const int32_t sub_req_idx, ObMsSqlRpcEvent *rpc_event, int64_t timeout_us)
    {
      ...
      if (NULL == rpc_event)
      {
        ...
        YYSYS_LOG(WARN, "null pointer error. [rpc_event=%p][err=%d]", rpc_event, err);
      }
      ...
      if ((OB_SUCCESS == err) && (OB_DATA_NOT_SERVE == rpc_event->get_result_code()))
      {
        ...
        if ((OB_SUCCESS == err)
            ...
            && (OB_ITER_END != err))
        {
          YYSYS_LOG(WARN,"fail to find replicas [err:%d]", err);
        }

        if (OB_ITER_END == err)
        {
          YYSYS_LOG(WARN,"fail to find replicas, while retry [err:%d]", err);
          err = OB_ERR_UNEXPECTED;
        }

        if ((OB_SUCCESS == err) && (replica_count <= 0))
        {
          YYSYS_LOG(WARN,"fail to find replicas, while retry [err:%d,replica_count:%d]", err, replica_count);
        ...
        {
          YYSYS_LOG(WARN,"fail to set sub request's cs replicas [err:%d]", err);;
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "exceeds retry times. [total=%d][tried_replica_count=%d][total_replica_count=%d]",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to resend rpc event of ObMsSqlSubScanRequest [idx:%d,err:%d]", sub_req_idx, err);
        }
        else if (OB_SUCCESS == err)
        {
          YYSYS_LOG(INFO, "retry [prev_rpc:%lu,retry_rpc:%lu,request:%lu]", rpc_event->get_event_id(), retry_event_id,
                    get_request_id());
        }
      }
      return err;
    ...
    int ObMsSqlScanRequest::initialize()
    {
      ...
      if (NULL == (ptr = ob_tc_malloc(sizeof(ObMsSqlSubScanRequest), ObModIds::OB_MS_SUB_SCAN_REQUEST)))
      {
        ...
        YYSYS_LOG(WARN, "ob malloc failed, ret=%d", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "push ObMsSqlSubScanRequest* to sub_requests failed, ret=%d", ret);
    ...
    // reset �� ObMsSqlScanRequest::initialize֮ǰ��״̬
    // ʹ�÷�ʽ��
    // ���캯����initialize, reset��initialize, reset��initialize, reset
    // ����������ʹ�÷�ʽ��
    // ���캯����reset, do other than initialize ...
    ...
    //        {
    //          YYSYS_LOG(DEBUG, "total_sub_request_count_=%d, finished_sub_request_count_=%d, max_parallel_count_=%ld,"
    ...
    //          if (OB_SUCCESS != err)
    //          {
    //            YYSYS_LOG(WARN, "fail to trigger more event request. err=%d", err);
    //          }
    //        }
    //      }
    ...
    int ObMsSqlScanRequest::get_next_row(oceanbase::common::ObRow &row)
    {
      YYSYS_LOG(DEBUG, "is_finish =%d,total_sub_request_count_=%d, finished_sub_request_count_=%d, max_parallel_count_=%ld,"
      ...
      {
        // check if need to trigger more request
        // ÿ��alloc_sub_scan_request ��total_sub_request_count_++;
        //ÿprocess_result һ�Σ�total_sub_request_count_++  fyd 2014.3.3
        if ((total_sub_request_count_ - finished_sub_request_count_ < max_parallel_count_)
            && (merger_operator_.get_mem_size_used() < max_cs_result_mem_size_)
            /*&& false == org_req_range_iter_.end()*/)
        {
          ...
          {
            YYSYS_LOG(WARN, " should not go here!");
          ...
          else
          {
            ...
            if ( rpc_scan_->get_sql_read_trategy()->has_next_range() )
            {
              ...
              {
                err = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "fail to get next range %d",err);
              }
              else if ((err = rpc_scan_->init_next_scan_param(query_range)) != OB_SUCCESS)
              {
                err = OB_ERR_UNEXPECTED;
                YYSYS_LOG(WARN, "fail to init  next scan range  param %d",err);
              }
              else
              {
                YYSYS_LOG(DEBUG, "init  next scan range =%s  success!",to_cstring(*scan_param_));
                YYSYS_LOG(DEBUG, "current scan range finished =%d ",this->is_finish());
                this ->set_finish(false);
                YYSYS_LOG(DEBUG, "the next scan range finished =%d ",this->is_finish());
            ...
            {
              YYSYS_LOG(DEBUG, "get_next_row finds range_iter end,stop request!");
              this ->is_end_ = true;
            }
          }
        }
      ...
      if (OB_SUCCESS == ret)
        //add:e
      {
        YYSYS_LOG(DEBUG, "get_next_row finds row =%d ,row = %s!",ret,to_cstring(row));
      }
