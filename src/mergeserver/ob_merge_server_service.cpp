...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    ObMergeServerService::ObMergeServerService()
    {
      ...
      schema_mgr_ = NULL;
      //add liu jun.[MultiUPS] [part_cache] 20150519:b
      part_monitor_ = NULL;
      part_lock_ = false;
      //20150519:e
      schema_proxy_ = NULL;
      ...
      lease_expired_time_ = yysys::CTimeUtil::getTime() + DEFAULT_LEASE_TIME;
      sql_id_mgr_ = NULL;
    ...
    //mod:e
    {
      ...
      while (!merge_server_->is_stoped())
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to register merge server to root server! "
                    "root_server: [%s]",
                    to_cstring(merge_server_->get_root_server()));
          //add bingo [Paxos ms/cs cluster_id] 20170401:b
          if(OB_CLUSTER_ID_ERROR == err)
          {
            YYSYS_LOG(WARN, "register self to rootserver failed, stop self, err=%d", err);
            merge_server_->stop();
          }
          //add:e
          //add liuzy [MultiUPS] [take_cluster_offline_interface] 20160325:b
          /*Exp: when cluster,current ms register to, is offline, kill self*/
          if (OB_CURRENT_CLUSTER_OFFLINE == err)
          {
            YYSYS_LOG(INFO, "cluster[%d], this ms belongs to, is offline, result_code:[%d]",
                      merge_server_->get_self().cluster_id_, err);
            merge_server_->stop();
            break;
          }
          //add 20160325:e
          ...
          if (!is_first)
          {
            ...
            merge_server_->set_new_root_server(current_rs);
            YYSYS_LOG(INFO, "will regist to next rootserver=%s", merge_server_->get_root_server().to_cstring());
          }
          else
          {
        ...
        else
        {
          ...
          merge_server_->get_rs_mgr().set_master_rs(current_rs);
          YYSYS_LOG(INFO, "register self to rootserver successed, master rs=%s", current_rs.to_cstring());
          //add:e
          break;
        }
      }
    ...
    int ObMergeServerService::reload_config()
    {
      ...
      {
        YYSYS_LOG(ERROR, "Check merge server failed. server[%p], rpc[%p]", merge_server_, rpc_proxy_);
        ret = OB_NOT_INIT;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for (int32_t i = 0; i <= merge_server_->get_config().retry_times; ++i)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            YYSYS_LOG(INFO, "Fetch update server list succ. count[%d]", count);
            break;
          }
          usleep(RETRY_INTERVAL_TIME);
        }
      ...
      if(OB_SUCCESS == ret)
      {
        ret = merge_server_->get_timer().schedule(part_release_task_,
                                                  merge_server_->get_config().partition_manager_release_interval, true);
      }
      //add 20160118:e
      //add liuzy [MultiUPS] [BugFix] [FetchUpsList] 20160803:b
      if(OB_SUCCESS == ret)
      {
        ret = merge_server_->get_timer().schedule(ms_fetch_ups_task_,
                                                  merge_server_->get_config().fetch_ups_interval, true);
      }
      //add 20160803:e
      return ret;
    }
    ...
    int ObMergeServerService::init_ms_properties_()
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            if (err != OB_SUCCESS)
            {
              YYSYS_LOG(WARN, "server register to root failed:ret[%d]", err);
            }
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "init async rpc failed:ret[%d]", err);
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "root rpc proxy init failed:ret[%d]", err);
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          for (int32_t i = 0; i <= merge_server_->get_config().retry_times; ++i)
          {
            ...
            if (OB_SUCCESS == err)
            {
              YYSYS_LOG(DEBUG, "fetch core table schema success");
              schema_mgr_->init(true, *newest_schema_mgr);
              break;
            }
            else
            {
              YYSYS_LOG(DEBUG, "fetch core table schema failed, ret=%d", err);
            }
            usleep(RETRY_INTERVAL_TIME);
          }
        }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "set sql server port for stat manager failed");
            err = OB_ERROR;
          }
          else
          {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        //    if (OB_SUCCESS != err)
        //    {
        //      YYSYS_LOG(WARN, "initialize query cache failed:ret[%d]", err);
        //    }
        //  }
        //}
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "timer init failed:ret[%d]", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(INFO, "%s", "lease check timer schedule succ");
        }
        else
        {
          YYSYS_LOG(ERROR, "lease check timer schedule failed:ret[%d]", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS == err)
          {
            YYSYS_LOG(INFO, "%s", "monitor timer schedule succ");
          }
          else
          {
            YYSYS_LOG(ERROR, "monitor timer schedule failed:ret[%d]", err);
          }
        }
      }
      ...
      if(OB_SUCCESS == err)
      {
        part_monitor_ = new(std::nothrow) ObPartitionMonitor;
        if(NULL == part_monitor_)
        {
          err = OB_ALLOCATE_MEMORY_FAILED;
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        sql_proxy_.set_env(rpc_proxy_, root_rpc_,
                           async_rpc_, schema_mgr_, part_monitor_, cache_proxy_, this);
        //mod:e
      }
      if (OB_SUCCESS == err)
      {
      ...
      //add liu jun. [MultiUPS] [part_cache] 20150626:b
      ObMsPartitionManager *newest_part_mgr = NULL;
      if(OB_SUCCESS == err)
      {
        newest_part_mgr = new(std::nothrow)ObMsPartitionManager(this, get_config_mgr().get_client_mgr());
        if (NULL == newest_part_mgr)
        {
          err = OB_ALLOCATE_MEMORY_FAILED;
        }
      }
      if(OB_SUCCESS == err)
      {
        if(OB_SUCCESS != (err = part_monitor_->add_partition_manager(newest_part_mgr)))
        {
          YYSYS_LOG(ERROR, "add partition manager failed, err=%d", err);
        }
        //del peiouya  20160113:b
        /*
         * following code is bad optimization for acquiring rule.
         * so delete it.
         * */
        //        for(int32_t i = 0;OB_SUCCESS == err && i < merge_server_->get_config().retry_times;i++)
        //        {
        //          if(OB_SUCCESS == (err = newest_part_mgr->update_all_rules()))
        //          {
        //            YYSYS_LOG(INFO, "init partition manager succ");
        //            break;
        //          }
        //          else
        //          {
        //            YYSYS_LOG(WARN, "update rules failed");
        //          }
        //          usleep(RETRY_INTERVAL_TIME);
        //          err = OB_SUCCESS;
        //        }
      ...
      if(OB_SUCCESS == err)
      {
        err = part_release_task_.init(this,part_monitor_);
        if(OB_SUCCESS == err)
        {
          err = merge_server_->get_timer().schedule(part_release_task_,
                                                    merge_server_->get_config().partition_manager_release_interval, true);
          if(OB_SUCCESS == err)
          {
          ...
          {
            YYSYS_LOG(ERROR, "release partition manager schedual failed, err=%d", err);
          }
        }
      }
      //add 20160118:e
      ...
      if(OB_SUCCESS == err)
      {
        err = ms_fetch_ups_task_.init(rpc_proxy_);
        if(OB_SUCCESS == err)
        {
          err = merge_server_->get_timer().schedule(ms_fetch_ups_task_,
                                                    merge_server_->get_config().fetch_ups_interval, true);
          if(OB_SUCCESS == err)
          {
            YYSYS_LOG(INFO, "schedule ms fetch ups list success, err=%d", err);
          }
          else
          {
            YYSYS_LOG(ERROR, "schedule ms fetch ups list failed, err=%d", err);
          }
        }
      }
      //add 20160803:e
      ...
      {
        YYSYS_LOG(ERROR, "service init error, ret: [%d]", err);
      }
      __debug_init__();
      return err;
    }
    ...
    int ObMergeServerService::destroy()
    {
      ...
      {
        inited_ = false;
        YYSYS_LOG(WARN, "stop mergeserver timer");
        merge_server_->get_timer().destroy();
        merge_server_ = NULL;
        YYSYS_LOG(WARN, "destroy rpc proxy etc.");
        delete rpc_proxy_;
        rpc_proxy_ = NULL;
        delete rpc_stub_;
        rpc_stub_ = NULL;
    ...
    void ObMergeServerService::handle_failed_request(const int64_t timeout, const int32_t packet_code)
    {
      ...
      {
        YYSYS_LOG(WARN, "%s", "merge server has not inited or registered");
      }
      else
      {
        ...
        switch (packet_code)
        {
          ...
          case OB_GET_REQUEST:
            ...
            break;
          default:
            YYSYS_LOG(WARN, "handle overflow or timeout packet not include statistic info:packet[%d]", packet_code);
        }
      }
    }
      ...
      if (rc == OB_SUCCESS)
      {
        ...
        switch (packet_code)
        {
          ...
          case OB_FETCH_COLLECTION_LIST:
            ...
            break;
          case OB_FETCH_RANGE_COLLECTION_LIST:
            rc = ms_fetch_range_collection_list(receive_time, version, channel_id, req, in_buffer, out_buffer, timeout_us);
            break;
          case OB_PUT_ANALYSISED_TABLE_SINFO_INTO_CACHE:
            rc = ms_put_analysised_table_sinfo_into_cache(receive_time, version, channel_id, req, in_buffer, out_buffer, timeout_us);
            break;
          case OB_PUT_ANALYSISED_COLUMN_SINFO_INTO_CACHE:
            rc = ms_put_analysised_column_sinfo_into_cache(receive_time, version, channel_id, req, in_buffer, out_buffer, timeout_us);
            break;
          default:
            YYSYS_LOG(WARN, "check packet type failed:type[%d]", packet_code);
            rc = OB_ERROR;
        }
      }
      return rc;
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      if (NULL == buf)
      {
        YYSYS_LOG(WARN,"fail to allocate memory for session infos");
        err = OB_ALLOCATE_MEMORY_FAILED;
      }
      if (OB_SUCCESS == err)
      {
      ...
      {
        YYSYS_LOG(WARN,"fail to serialize result code [err:%d]", err);
      }
      if ((OB_SUCCESS == err)
          && (OB_SUCCESS != (err = result.serialize(out_buffer.get_data(),out_buffer.get_capacity(), out_buffer.get_position()))))
      {
        YYSYS_LOG(WARN,"fail to serialize result msg [err:%d]", err);
      }
      if ((OB_SUCCESS == err)
      ...
      {
        YYSYS_LOG(WARN,"fail to send list sessions response [send_err:%d]", send_err);
      }
      ob_free(buf);
        ...
        common::ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get session id from request [err:%d]", err);
      }
      if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = obj.get_int(session_id))))
      {
        YYSYS_LOG(WARN,"fail to get session id from reqeust [err:%d]", err);
      }
      if ((OB_SUCCESS == err) && (session_id <= 0))
      {
        YYSYS_LOG(WARN,"invalid aguemnt [session_id:%ld]", session_id);
      }
      if (OB_SUCCESS == err)
      {
      ...
      if (OB_SUCCESS == err)
      {
        YYSYS_LOG(INFO, "kill session [session_id:%ld]", session_id);
        ...
        )
    {
      ...
      if (OB_SUCCESS != (err = serialization::decode_vi32(in_buffer.get_data(),
                                                          ...
                                                          &session_id)))
      {
        YYSYS_LOG(WARN,"fail to decode session id from request [err:%d]", err);
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to decode is_kill_query from request [err:%d]", err);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        const int64_t timeout_us)
    {
      ...
      {
        YYSYS_LOG(WARN, "version not equal. version=%d, MS_FORCE_REGIST_VERSION=%d", version, MS_FORCE_REGIST_VERSION);
      ...
      else
      {
        ...
        merge_server_->set_new_root_server(rs_master);
        YYSYS_LOG(INFO, "ms set new master rootserver[%s] after election", rs_master.to_cstring());
        ///1. make ms lease timeout
        lease_expired_time_ = yysys::CTimeUtil::getTime();
        YYSYS_LOG(INFO, "reset lease, then will re-regist, lease=%ld", lease_expired_time_);
        ...
        if (NULL != location_cache_)
        {
          ...
          //YYSYS_LOG(INFO, "clear tablet location cache:ret[%d]", ret);
        }
        //add:e
      }
      // no response
        ...
        const int64_t timeout_us)
    {
      ...
      bool frozen_version_has_changed = false;
      ...
      if (version >= MS_HEARTBEAT_LEASE_SCHEMA_VERSION)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "parse heartbeat input lease_duration param failed:ret[%d]", err);
        }
        else
        {
          if (lease_duration <= 0)
          {
            err = OB_ERROR;
            YYSYS_LOG(ERROR, "check lease duration failed:duration[%ld]", lease_duration);
          }
        }
        //TODO frozen version
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "parse heartbeat frozen memory version failed:ret[%d]", err);
          }
          else
          {
            frozen_version_ = frozen_mem_version;
            frozen_version_has_changed = frozen_mem_version > frozen_version_ ? true : false;
          }
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "parse heartbeat schema version failed:ret[%d]", err);
          }
        ...
        ////add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150604:b
        //if(OB_SUCCESS == err && frozen_mem_version > 1)
        //{
        //  bool partition_lock_flag = false;
        //  err = serialization::decode_bool(in_buffer.get_data(), in_buffer.get_capacity(),
        //      in_buffer.get_position(), &partition_lock_flag);
        //  if(OB_SUCCESS != err)
        //  {
        //    YYSYS_LOG(ERROR, "parse heartbeat partition lock flag failed:ret[%d]", err);
        //  }
        //  //cause when frozen memory, only need update partition manager 1 time.
        //  else if(true == partition_lock_flag && !part_lock_)
        //  {
        //    part_lock_ = true;
        //    YYSYS_LOG(INFO, "partition tables has locked");
        //    part_task_.init(this, &get_config_mgr(), part_monitor_);
        //    srand(static_cast<int32_t>(yysys::CTimeUtil::getTime()));
        //    err = merge_server_->get_timer().schedule(part_task_,
        //          random() % FETCH_SCHEMA_INTERVAL, false);
        //    if(OB_SUCCESS != err)
        //    {
        //      YYSYS_LOG(ERROR, "schedual partition manager failed, err=%d", err);
        //    }
        //  }
        //  else if(false == partition_lock_flag)
        //  {
        //    part_lock_ = false;
        //    YYSYS_LOG(INFO, "unlock partition tables");
        //  }
        if (OB_SUCCESS == err)
        {
          bool partition_lock_flag = false;
          err = serialization::decode_bool(in_buffer.get_data(), in_buffer.get_capacity(),
                                           in_buffer.get_position(), &partition_lock_flag);
          if(OB_SUCCESS == err && frozen_mem_version > 0)
          {
            //cause when frozen memory, only need update partition manager 1 time.
            if(partition_lock_flag && !part_lock_)
            {
              part_lock_ = true;
              YYSYS_LOG(INFO, "partition tables has locked");
              part_task_.init(this, &get_config_mgr(), part_monitor_, frozen_version_has_changed);
              err = merge_server_->get_timer().schedule(part_task_, 0, false);
              if(OB_SUCCESS != err)
              {
                YYSYS_LOG(ERROR, "schedual partition manager failed:err=%d", err);
              }
          ...
          {
            YYSYS_LOG(WARN, "decode partition lock flag failed:ret[%d]", err);
          }
        }
        ////add:e
        //mod peiouya 20150729:e
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "heartbeat to root server failed:ret[%d]", err);
          }
          else
          {
            extend_lease(lease_duration);
            YYSYS_LOG(DEBUG, "%s", "heartbeat to root server succ");
          }
        }
        ...
        if ((OB_SUCCESS == err) && (local_version < schema_version) && (schema_version > 0))
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "schedule fetch new schema task failed:version[%ld], ret[%d]",
                      schema_version, err);
          }
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "parse heartbeat input role param failed:ret[%d]", err);
          }
        }
      }
      if (version >= MS_HEARTBEAT_PRIVILEGE_VERSION)
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "parse heartbeat privilege version failed:ret[%d]", err);
          }
          /* for slave ms, it's weird */
          /* else if (local_privilege_version > privilege_version) */
          /* { */
          /*   err = OB_ERR_OLDER_PRIVILEGE_VERSION; */
          /*   YYSYS_LOG(ERROR, "check privilege local version gt than new version:" */
          /*       "local[%ld], incoming[%ld], ret=%d", local_privilege_version, privilege_version, err); */
          /* } */
          else if (local_privilege_version < privilege_version)
          {
            YYSYS_LOG(INFO, "local_privilege_version=%ld, incoming privilege_version=%ld",
            ...
            if (err != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "schedule update privilege task failed, version=%ld, ret=%d", privilege_version, err);
            }
          }
        }
      }
        ...
        const int64_t timeout_us)
    {
      ...
      if (NULL != location_cache_)
      {
        ...
        YYSYS_LOG(INFO, "clear tablet location cache:ret[%d]", err);
      }
      else
      {
        err = OB_ERROR;
        ...
        const int64_t timeout_us)
    {
      ...
      if ((0 >= timeout_us) || (timeout_us > merge_server_->get_config().max_req_process_time))
      {
        ...
        YYSYS_LOG(DEBUG, "reset timeoutus %ld", req_timeout_us);
      }
      ...
      {
        YYSYS_LOG(WARN,"fail to allocate memory for request");
        err = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to init ObMergerGetEvent [err:%d]", err);
        }
      }
      /// decode request
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to parse ObGetParam [err:%d]",err);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to cast ObGetParam type to ObReadParam type. decoded_read_param=%p, org_read_param=%p",
                      decoded_read_param, org_read_param);
            err = OB_ERROR;
          }
        }
      ...
      {
        YYSYS_LOG(WARN,"get param cell size error, [org_get_param:%ld]", org_get_param->get_cell_size());
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get the latest schema, unexpected error:table[%.*s], ret[%d]",
      ...
      if (OB_SUCCESS == err && is_cache_hit)
      {
        ...
        if ((OB_SUCCESS == send_err) &&
            (OB_SUCCESS != (send_err = merge_server_->send_response(OB_GET_RESPONSE, MS_GET_VERSION,out_buffer,
                                                                    req,request_cid, 0))))
        {
          YYSYS_LOG(WARN,"fail to send response to client [err:%d]", send_err);
        }
        if (OB_SUCCESS == send_err)
        {
          packet_cnt ++;
      ...
      else
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to decode get param [err:%d]", err);
          }
        }
        if ((OB_SUCCESS == err)
        ...
        {
          YYSYS_LOG(WARN,"fail to create session in ObSessionManager [err:%d]", err);
        }
        if (OB_SUCCESS == err)
        {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "get request set param failed:err[%d]", err);
          }
          else
          {
            err = get_event->wait(timeout_time - yysys::CTimeUtil::getTime());
            if (err != OB_SUCCESS)
            {
              YYSYS_LOG(WARN, "get request wait failed:err[%d]", err);
            }
          }
        }
        ...
        {
          YYSYS_LOG(WARN, "fail to faill result [err:%d]", err);
        }
        ...
        do
        {
          if ((OB_SUCCESS == err) && (!got_all_result) && (packet_cnt > 0))
          {
            ...
            if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = get_event->fill_result(*result_scanner,
                                                                                    *get_param_with_name,got_all_result))))
            {
              YYSYS_LOG(WARN,"fail to fill result [err:%d]", err);
            }
          }
                                                      ...
                                                      out_buffer.get_position()))))
          {
            ...
            YYSYS_LOG(WARN,"fail to serialize result code [err:%d]", send_err);
          }
          if ((OB_SUCCESS == err) && (OB_SUCCESS == send_err)
              && (OB_SUCCESS !=(send_err = result_scanner->serialize(out_buffer.get_data(),out_buffer.get_capacity(),
                                                                     out_buffer.get_position()))))
          {
            YYSYS_LOG(WARN,"fail to serialize result scanner [err:%d]", send_err);
          }
          ...
          {
            YYSYS_LOG(WARN,"fail to prepare network session [err:%d]", err);
          }
          if ((OB_SUCCESS == err) && (OB_SUCCESS == send_err)
              ...
              && schema_mgr->get_table_query_cache_expire_time(first_table_name) > 0)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to put get result into query cache:err[%d]", err);
              err = OB_SUCCESS;
            }
          }
          ...
          {
            err = queue_thread.wait_for_next_request(net_session_id,next_request,timeout_time - yysys::CTimeUtil::getTime());
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to wait next request from session [err:%d,net_session_id:%ld]", err, net_session_id);
            }
            else
            {
              request_cid = next_request->get_channel_id();
      ...
      if (OB_SUCCESS == err)
      {
        ...
        //mod bizb [slow_query]
        int64_t threshold = merge_server_->get_config().slow_query_threshold.ObConfigIntegralItem::get();
        if (threshold > 0 && end_time - start_time > threshold)
        //end
        {
          const char *session_info = NULL;
          if (NULL == (session_info = session_mgr_.get_session(session_id)))
          {
            YYSYS_LOG(WARN,"get empty session info");
          }
          else
          {
            YYSYS_LOG(WARN, "slow query:%s; used_time:%ld; result_size:%ld", session_info, end_time - start_time,
      ...
      if (end_time > start_time + req_timeout_us)
      {
        ...
        YYSYS_LOG(WARN, "check process get task timeout:start[%ld], end[%ld], timeout[%ld], used[%ld]",
        ...
        const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to parse ObMutator [err:%d]",err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to decode mutator param [err:%d]", err);
        }
      }
      FILL_TRACE_LOG("step 2. finish parse the schema for ms_mutate:err[%d]", err);
      YYSYS_LOG(DEBUG, "need prefetch data:prefetch_cell[%ld], return_cell[%ld]",
      ...
      if (end_time > start_time + req_timeout_us)
      {
        ...
        YYSYS_LOG(WARN, "check process mutate task timeout:start[%ld], end[%ld], timeout[%ld], used[%ld]",
                  start_time, end_time, req_timeout_us, end_time - start_time);
      }
      /// send ups request
      if (OB_SUCCESS == err)
      ...
      // check process timeout
      end_time = yysys::CTimeUtil::getTime();
      if (end_time > start_time + req_timeout_us)
      {
        err_code = OB_PROCESS_TIMEOUT;
        YYSYS_LOG(WARN, "check process mutate task timeout:start[%ld], end[%ld], timeout[%ld], used[%ld]",
        ...
        const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          err = OB_NOT_INIT;
          YYSYS_LOG(WARN, "schema_mgr_ is null");
        }
      }
      ...
      //OB_DELETE(ObSchemaManagerV2, ObModIds::OB_MS_SERVICE_FUNC, schema); //[conflict_level C]
        ...
        int64_t timeout_us)
    {
      ...
      int64_t timeout_timestamp = yysys::CTimeUtil::getTime() + timeout_us;
      easy_connection_t *conn = req->ms->c;
      int icount = 0;
      //add 20141204:e
      do
      {
          if((ret = merge_server_->check_request_cleanup(req, __FUNCTION__))!= OB_SUCCESS)
          {
              YYSYS_LOG(WARN, "request(%p) error cleanup, req->ms->status=[%d], icount=%d",
                        req, req->ms->status, icount);
              break;
          }
          icount++;
        ...
        if (OB_SUCCESS == ret && OB_SUCCESS !=
            (ret = result.serialize(out_buffer.get_data(),
                                    ...
                                    out_buffer.get_position())))
        {
          YYSYS_LOG(ERROR, "serilize result buffer error, ret = [%d], icount=%d", ret, icount);
          break;
        }
        ...
        if (result.is_select() && !fullfilled)
        {
          ...
          ret = queue_thread.prepare_for_next_request(session_id, conn->seq); //[701]
        }
        ...
        else if (result.is_select() && !fullfilled)
        {
          ...
          /*ret = queue_thread.wait_for_next_request(
            ...
            timeout_us - yysys::CTimeUtil::getTime());
    */
          //del e
          ret = queue_thread.wait_for_next_request(
                  session_id, next_request,
                  timeout_timestamp - yysys::CTimeUtil::getTime());
          //mod 20141204:e
          ...
          {
            YYSYS_LOG(WARN, "failed to wait for next reques timeout, ret=%d @channel[%u], req[%p], c[%p]",
                      ret, channel_id, req, conn);
            break;
          }
          else
          {
      ...
      if (session_id > 0)
      {
        if (ret != OB_SUCCESS) result.set_errno(OB_ERR_SESSION_INTERRUPTED); //[701]
        ...
        const int64_t timeout_us)
    {
      ...
      ObString query_str;
      //[696]
      ObString query_str_deserialize;
      if (OB_SUCCESS !=
          (ret = query_str_deserialize.deserialize(in_buffer.get_data(),
                                       in_buffer.get_capacity(),
                                       in_buffer.get_position())))
      {
        YYSYS_LOG(WARN, "fail to deserialize query string.");
      }
      else
      {
        YYSYS_LOG(TRACE, "internal query: [%.*s], length: [%d]",
                  query_str_deserialize.length(), query_str_deserialize.ptr(), query_str_deserialize.length());
      }

      char *dup_buf = (char *)ob_malloc(query_str_deserialize.length(), ObModIds::LIB_OBSQL);
      if(dup_buf != NULL){
          query_str.assign_buffer(dup_buf, query_str_deserialize.length());
          query_str.clone(query_str_deserialize);
      }
      else
      {
          YYSYS_LOG(WARN, "fail to malloc duplicate query string.");
      }
      ...
      {
        if(OB_SUCCESS != (ret = db_name.deserialize(in_buffer.get_data(),
                                                    in_buffer.get_capacity(),
                                                    in_buffer.get_position())))
        {
          YYSYS_LOG(WARN,"fail to deserialize db name");
        }
        else
        {
          YYSYS_LOG(TRACE,"ob_export query use db name [%.*s]",
                    db_name.length(), db_name.ptr());
          session.set_db_name(db_name);
        }
        if(OB_SUCCESS == ret && OB_SUCCESS != (ret = username.deserialize(in_buffer.get_data(),
                                                                          in_buffer.get_capacity(),
                                                                          in_buffer.get_position())))
        {
          YYSYS_LOG(WARN,"fail to deserialize username");
        }
        else if(OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO,"ob_export query use username [%.*s]",username.length(),username.ptr());
          session.set_username(username);
        }
        if(OB_SUCCESS == ret && OB_SUCCESS != (ret = passwd.deserialize(in_buffer.get_data(),
                                                                        in_buffer.get_capacity(),
                                                                        in_buffer.get_position())))
        {
          YYSYS_LOG(WARN,"fail to deserialize passwd");
        }
        else if(OB_SUCCESS == ret)
        {
          //check username and passwd is valid or not
          const ObPrivilege **pp_privilege;

          if(OB_SUCCESS != (ret = privilege_mgr_->get_newest_privilege(pp_privilege)))
          {
            YYSYS_LOG(ERROR,"can't get privilege information, internal server error, ret=%d",ret);
          }
          else if (OB_SUCCESS != (ret = (*pp_privilege)->user_is_valid_for_obexport(username,passwd)))
          {
            YYSYS_LOG(WARN,"user is not valid, ret=%d",ret);
            if(OB_ERR_USER_EMPTY == ret)
            {
              YYSYS_LOG(WARN,"username is empty");
            }
            else if (OB_ERR_USER_NOT_EXIST == ret)
            {
              YYSYS_LOG(WARN,"username not exists");
            }
            else if (OB_ERR_USER_IS_LOCKED == ret)
            {
              YYSYS_LOG(WARN,"user is locked");
            }
            else if (OB_ERR_WRONG_PASSWORD == ret)
            {
              YYSYS_LOG(WARN,"wrong password");
            }
          }
          else
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(INFO, "DEBUG:set query_timeout is %ld", timeout_us);
        //add 20141120:e
        if (OB_SUCCESS !=
            (ret = sql_proxy_.init_sql_env(context, schema_version,
                                           rs, session)))
        {
          YYSYS_LOG(WARN, "init sql env error.");
        }
        //add tianz [EXPORT_TOOL] 20141120:b
        ...
        else if( OB_SUCCESS != (ret = set_tx_system_param(session)))
        {
          YYSYS_LOG(ERROR, "load tx system variable failed, ret=%d",ret);
        }
        //add 20141120:e
        else
        {
          if(version == 2)
          {
            context.disable_privilege_check_ = false;
          }
          if (OB_SUCCESS != (ret = sql_proxy_.execute(query_str, rs,
                                                      context, schema_version)))
          {
            YYSYS_LOG(WARN, "execute sql failed. ret = [%d] sql = [%.*s]",
                      ret, query_str.length(), query_str.ptr());
          }
          else if (OB_SUCCESS != (ret = rs.open()))
          {
            YYSYS_LOG(ERROR, "open sql result set error, ret = [%d]", ret);
          }
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "send sql result error, sql: [%.*s], ret: [%d]",
        ...
        //mod 20141204:e
      {
        ...
        if (OB_SUCCESS != (
              ret = rs.serialize(out_buffer.get_data(),
                                 ...
                                 out_buffer.get_position())))
        {
          YYSYS_LOG(WARN, "serilize result set error, ret: [%d]", ret);
        }
        //[701]
        else if (rs.get_errno() == OB_ERR_SESSION_INTERRUPTED){
          YYSYS_LOG(WARN, "one req can't send_response twice, ret: [%d]", ret);
        }
        else if (OB_SUCCESS != (
                   retcode = merge_server_->send_response(
                     OB_SQL_EXECUTE_RESPONSE, 1, out_buffer, req, channel_id)))
        {
          YYSYS_LOG(WARN, "send response error, ret: [%d]", retcode);
        }
      }
      //[632]
      if(NULL != context.pp_privilege_)
      {
          int err = privilege_mgr_->release_privilege(context.pp_privilege_);
          if(OB_SUCCESS != err)
          {
              YYSYS_LOG(ERROR, "release privilege failed, ret=%d", err);
          }
          context.pp_privilege_ = NULL;
      }

      //[696]
      if(dup_buf != NULL) ob_free(dup_buf);
      return retcode;
    }
        ...
        const int64_t timeout_us)
    {
      ...
      else if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to init ObMergerScanEvent [err:%d]", err);
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to deserialize ObScanParam [err:%d]",err);
          snprintf(rc.message_.ptr(), rc.message_.length(), "fail to deserialize YaoScanParam [err:%d]", err);
        }
        else
        {
          ObReadParam * org_read_param = NULL;
          ...
          {
            YYSYS_LOG(WARN, "fail to cast ObScanParam type to ObReadParam type. decoded_read_param=%p, org_read_param=%p",
                      decoded_read_param, org_read_param);
            err = OB_ERROR;
          }
        }
      ...
      else
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to decode scan param [err:%d]", err);
          }
        }
        ...
        if ((OB_SUCCESS == err)
            && (OB_SUCCESS != (err = session_mgr_.session_begin(*org_scan_param,
                                                                ...
                                                                syscall(SYS_gettid), static_cast<pid_t>(pthread_self())))))
        {
          YYSYS_LOG(WARN,"fail to create session in SessionManager [err:%d]", err);
        }
        if (OB_SUCCESS == err)
        {
        ...
        {
          YYSYS_LOG(WARN,"fail to set ObMergerScanParam [err:%d]", err);
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "scan request set request param failed [err:%d]", err);
          }
        }

        /// process result
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = scan_event->wait(timeout_time - yysys::CTimeUtil::getTime()))))
        {
          YYSYS_LOG(WARN, "scan request wait failed [err:%d]", err);
        }
        ...
        do
        {
              ...
              && schema_mgr->get_table_query_cache_expire_time(org_scan_param->get_table_name()) > 0)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to put scan result into query cache:err[%d]", err);
              err = OB_SUCCESS;
            }
          }
          FILL_TRACE_LOG("step 4.x. finish serialize scanner:err[%d]", send_err);
      ...
      {
        err_code = OB_PROCESS_TIMEOUT;
        YYSYS_LOG(WARN, "check process scan task timeout:start[%ld], end[%ld], timeout[%ld], used[%ld]",
                  start_time, end_time, req_timeout_us, end_time - start_time);
      }
        ...
        const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == rc.result_code_)
      {
        ...
        else if (OB_ALL_SERVER_SESSION_TID == table_id)
        {
          ...
          rc.result_code_ = sql_session_mgr_->get_scanner(*new_scanner,sql_scan_param_ptr);
        }
        ...
        const int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "reload configuration failed, ret: [%d]", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "reload configuration succeffully.");
        }
        ...
        if (OB_SUCCESS != send_err)
        {
          YYSYS_LOG(WARN, "Send reload config response failed, ret: [%d]", send_err);
        }
      }
        ...
        const int64_t timeout_us)
    {
      ...
      {
        if ((YYSYS_LOG_LEVEL_ERROR <= log_level) && (YYSYS_LOG_LEVEL_DEBUG >= log_level))
        {
    ...
    int ObMergeServerService::set_tx_system_param(ObSQLSessionInfo &session)
    {
      int ret = OB_SUCCESS;
      int timeout = 100000000;
      ObString isolation = ObString::make_string("tx_isolation");
      ObObj isolation_type;
      isolation_type.set_type(ObVarcharType);
      ObObj isolation_value;
      isolation_value.set_varchar(ObString::make_string("READ-COMMITTED"));
      ObString tx_timeout = ObString::make_string("yb_tx_timeout");
      ObObj tx_timeout_type;
      tx_timeout_type.set_type(ObIntType);
      ObObj tx_timeout_value;
      tx_timeout_value.set_int(timeout);
      ObString tx_idle_timeout = ObString::make_string("yb_tx_idle_timeout");
      ObObj tx_idle_timeout_type;
      tx_idle_timeout_type.set_type(ObIntType);
      ObObj tx_idle_timeout_value;
      tx_idle_timeout_value.set_int(timeout);

      if (OB_SUCCESS != (ret = session.load_system_variable(isolation, isolation_type, isolation_value)))
      {
        YYSYS_LOG(ERROR,"load system variable %.*s failed, ret=%d",isolation.length(),isolation.ptr(),ret);
      }
      else if (OB_SUCCESS != (ret = session.load_system_variable(tx_timeout,tx_timeout_type,tx_timeout_value)))
      {
        YYSYS_LOG(ERROR,"load system variable %.*s failed, ret=%d",tx_timeout.length(),tx_timeout.ptr(),ret);
      }
      else if (OB_SUCCESS != (ret = session.load_system_variable(tx_idle_timeout,tx_idle_timeout_type,tx_idle_timeout_value)))
      {
        YYSYS_LOG(ERROR,"load system variable %.*s failed, ret=%d",tx_idle_timeout.length(),tx_idle_timeout.ptr(),ret);
      }

      return ret;
    }

    int ObMergeServerService::ms_fetch_collection_list(
        const int64_t receive_time,
        const int32_t version,
        const int32_t channel_id,
        easy_request_t *req,
        ...
        const int64_t timeout_us)
    {
      ...
      if (pos > 1023)
      {
        ...
        YYSYS_LOG(WARN,"query collection list overflow, err = %d", err);
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"init sql env error, err=%d", err);
        }
        else
        {
          if(OB_SUCCESS != (err = sql_proxy_.execute(query_collection_list,collection_list, context, schema_version)))
          {
            YYSYS_LOG(WARN,"execute privilge sql failed,sql=%.*s, err = %d", query_collection_list.length(),query_collection_list.ptr(),err);
          }
          else if(OB_SUCCESS != (err = collection_list.get_result_set().open()))
          {
            YYSYS_LOG(ERROR, "open collection list failed, err = %d", err);
          }
          int32_t ret = err;
          if(OB_SUCCESS != (err = rc.serialize(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position())))
          {
            YYSYS_LOG(WARN, "failed to encode result buffer, err[%d]", err);
          }
          else if(OB_SUCCESS == ret)
          {
            ...
            while(OB_SUCCESS == err)
            {
              if(OB_SUCCESS != (err = collection_list.get_result_set().get_next_row(row)))
              {
                if(OB_ITER_END == err)
                {
                  YYSYS_LOG(INFO,"has itered to the end!, err=%d",err);
                  err = OB_SUCCESS;
                }
                else
                {
                  YYSYS_LOG(ERROR,"failed to get next row, err=%d",err);
                }
                break;
              }
              else if(row != NULL)
              {
                ...
                {
                  YYSYS_LOG(ERROR, "failed to get tid and cid, err=%d",err);
                  break;
                }
                if(OB_SUCCESS != (err = row->get_cell(table_id,column_id,cell1)))
                {
                  YYSYS_LOG(ERROR,"failed to get cell, err=%d",err);
                  break;
                }
                else
                {
            ...
            {
              YYSYS_LOG(WARN, "failed to serialize the list size, err=%ld", list_array.count());
            }
            else if(OB_SUCCESS != (err = sql_proxy_.cleanup_sql_env(context, collection_list)))
            {
              YYSYS_LOG(WARN, "clean sql env error, err:[%d]", err);
            }
            else
            {
              for(int64_t i = 0; i < list_array.count(); i++)
              {
                ...
                {
                  YYSYS_LOG(WARN, "failed to serialize the list id=%ld, err=%d", list_array.at(i), err);
                }
              }
            }
          }
      ...
      {
        pos = 0;
        memset(del_stat_buff,0,512);
        ObString del_stat;
        int64_t tmp = j;
        tmp++;
        databuff_printf(del_stat_buff,512,pos,"DELETE /*+ud_multi_batch*/ FROM __all_statistic_info WHERE table_id = %ld AND column_id = %ld",list_array.at(j),list_array.at(j+1));
        if(pos > 511)
        {
          err = OB_BUF_NOT_ENOUGH;
          YYSYS_LOG(WARN, "del_stat buff overflow! err = %d",err);
          break;
        }
        else
        {
          ObSQLResultSet sql_result;
          ObSQLSessionInfo session_;
          ObSqlContext context_;

          del_stat.assign_ptr(del_stat_buff, static_cast<ObString::obstr_size_t>(pos));
          if (OB_SUCCESS != (err = sql_proxy_.init_sql_env(context_, schema_version, sql_result, session_)))
          {
            YYSYS_LOG(WARN, "init sql env error, err=%d",err);
          }
          else
          {
            ...
            if (OB_SUCCESS != (err = sql_proxy_.execute(del_stat, sql_result, context_, schema_version)))
            {
              YYSYS_LOG(WARN,"execute privilge sql failed,sql=%.*s, err = %d",del_stat.length(),del_stat.ptr(),err);
            }
            else if (OB_SUCCESS != (err = sql_result.get_result_set().open()))
            {
              YYSYS_LOG(ERROR, "open collection list failed, err = %d", err);
            }
            else
            {
              int64_t affected_rows = sql_result.get_result_set().get_affected_rows();
              YYSYS_LOG(INFO,"clean statistics info affect [%ld] rows", affected_rows);
            }
          }
          if(OB_SUCCESS != (err = sql_proxy_.cleanup_sql_env(context_, sql_result)))
          {
            YYSYS_LOG(WARN, "clean sql env error, err:[%d]", err);
          }
        }
      ...
      {
        YYSYS_LOG(WARN, "send response error, err:[%d]",err);
      }
      if(context.schema_manager_!=NULL)
      {
        err = schema_mgr_->release_schema(context.schema_manager_);
        context.schema_manager_ = NULL;
      }
      return err;
    }

    int ObMergeServerService::ms_fetch_range_collection_list(
        const int64_t receive_time,
        const int32_t version,
        const int32_t channel_id,
        easy_request_t *req,
        ...
        const int64_t timeout_us)
    {
      ...
      ObSqlContext context;
      int64_t schema_version = 0;
      int64_t rec_version = 0;
      int64_t start = 0;
      int64_t end = 0;
      int32_t &err = rc.result_code_;
      if(OB_SUCCESS != (err = serialization::decode_i64(in_buffer.get_data(),in_buffer.get_capacity(),
                                                        in_buffer.get_position(), &rec_version)))
      {
        YYSYS_LOG(WARN, "deserialize error, err=%d",err);
      }
      else if(OB_SUCCESS != (err = serialization::decode_i64(in_buffer.get_data(),in_buffer.get_capacity(),
                                                             in_buffer.get_position(), &start)))
      {
        YYSYS_LOG(WARN, "deserialize error, err=%d",err);
      }
      else if(OB_SUCCESS != (err = serialization::decode_i64(in_buffer.get_data(),in_buffer.get_capacity(),
                                                             in_buffer.get_position(), &end)))
      {
        YYSYS_LOG(WARN, "deserialize error, err=%d",err);
      }
      databuff_printf(query_collection_list_buf, 1024, pos, "SELECT table_id, column_id FROM __all_udi_monitor_list limit %ld,%ld",start,end-start+1);
      if (pos > 1023)
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(WARN,"query collection list overflow, err = %d", err);
      ...
      {
        query_collection_list.assign_ptr(query_collection_list_buf, static_cast<ObString::obstr_size_t>(pos));
        YYSYS_LOG(DEBUG, "sql=%.*s, err = %d", query_collection_list.length(), query_collection_list.ptr(), err);
        if (OB_SUCCESS != (err = sql_proxy_.init_sql_env(context, schema_version, collection_list, session)))
        {
          YYSYS_LOG(WARN,"init sql env error, err=%d", err);
        }
      ...
      for(int64_t j = 0; j < list_array.count(); j = j+2)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "init result set failed,%d", err);
          }
          else if(OB_SUCCESS!=(err = ObSql::direct_execute(del_stat,result,context)))
          {
            YYSYS_LOG(WARN, "direct execute sql =%.*s failed, ret=%d",del_stat.length(),del_stat.ptr(),err);
            break;
          }
          else if(OB_SUCCESS != (err = result.open()))
          {
            YYSYS_LOG(WARN, "open result failed, ret=%d", err);
            break;
          }
          else
          {
      ...
      if(OB_SUCCESS!= (err = merge_server_->send_response(OB_FETCH_RANGE_COLLECTION_LIST_RESPONSE,
                                                          MY_FETCH_VERSION,out_buffer,req,channel_id)))
      {
        YYSYS_LOG(WARN, "send response error, err:[%d]",err);
      }
    ...
    int ObMergeServerService::ms_put_analysised_table_sinfo_into_cache(
        const int64_t receive_time,
        const int32_t version,
        const int32_t channel_id,
        easy_request_t *req,
        ...
        const int64_t timeout_us)
    {
      ...
      if(OB_SUCCESS != (err = serialization::decode_i64(in_buffer.get_data(), in_buffer.get_capacity(),
                                                        in_buffer.get_position(), &tid)))
      {
        YYSYS_LOG(WARN, "deserialize error, err=%d",err);
      }
      else
      {
        uint64_t table_id = static_cast<uint64_t>(tid);
        if(table_id!=OB_INVALID_ID)
        {
          ObStatisticInfoCache* sic = merge_server_->get_statistic_info_cache();
          if(sic == NULL)
          {
            YYSYS_LOG(WARN, "ObStatisticInfoCache is null!");
            err = OB_ERROR;
          }
          else if(OB_SUCCESS!=(err = sic->put_analysised_table_sinfo_into_cache(table_id)))
          {
            YYSYS_LOG(WARN, "put analysised table sinfo into cache failed!");
          }
          else
          {
            StatisticTableValue stv;
            if(OB_SUCCESS!=(err=sic->get_table_statistic_info_from_cache(tid,stv)))
            {
              YYSYS_LOG(WARN, "get table statistic info from cache failed,err=%d",err);
            }
            else
            {
              for(int i=0;i<stv.statistic_columns_num_;i++)
              {
                if(OB_SUCCESS!=(err=sic->put_analysised_column_sinfo_into_cache(table_id,stv.statistic_columns_[i])))
                {
                  YYSYS_LOG(WARN, "put analysised column sinfo into cache failed,err=%d",err);
                }
              }
            }
          }
        ...
        {
          YYSYS_LOG(WARN, "invalid table_id");
          err = OB_INVALID_ARGUMENT;
        }

        if(OB_SUCCESS!=(err = rc.serialize(out_buffer.get_data(),out_buffer.get_capacity(),
                                           out_buffer.get_position())))
        {
          YYSYS_LOG(WARN, "serialize error, err=%d",err);
        }
        else
        {
          send_err = merge_server_->send_response(OB_PUT_ANALYSISED_TABLE_SINFO_INTO_CACHE_RESPONSE, MY_VERSION, out_buffer,req, channel_id);
        }
      }
      easy_request_wakeup(req);
      return send_err;
    ...
    int ObMergeServerService::ms_put_analysised_column_sinfo_into_cache(
        const int64_t receive_time,
        const int32_t version,
        const int32_t channel_id,
        easy_request_t *req,
        ...
        const int64_t timeout_us)
    {
      ...
      {
        uint64_t table_id = static_cast<uint64_t>(tid);
        uint64_t column_id = static_cast<uint64_t>(cid);
        if (table_id!=OB_INVALID_ID && column_id!=OB_INVALID_ID)
        {
          if(OB_SUCCESS !=(err=merge_server_->get_statistic_info_cache()->put_analysised_column_sinfo_into_cache(table_id,column_id)))
          {
            YYSYS_LOG(WARN, "put analysised column sinfo into cache failed!");
          }
          else
          {
            YYSYS_LOG(INFO, "put analysised column sinfo into cache success");
          }
        }
        else
        {
          YYSYS_LOG(WARN, "invalid table_id or column_id");
          err = OB_INVALID_ARGUMENT;
        }
        ...
        {
          send_err = merge_server_->send_response(OB_PUT_ANALYSISED_COLUMN_SINFO_INTO_CACHE_RESPONSE, MY_VERSION, out_buffer,req,channel_id);
        }
      }
      easy_request_wakeup(req);
      return send_err;
    ...
    //add liu jun. [MultiUPS] [PARTITION_LOCK_FUCTION] 20150604:b
    void ObMergeServerService::kill_self()
    {
      kill(getpid(), SIGTERM);
    }

    const ObServer &ObMergeServerService::get_server() const
    {
      return merge_server_->get_self();
    }

    //add 20150604:e
    //add liuzy [MultiUPS] [take_paxos_offline_interface] 20160228:b
    const ObServer &ObMergeServerService::get_root_server() const
    {
      return merge_server_->get_root_server();
    }
    //add 20160228:e
