  ...
  else
  {
    ...
    else
    {
            ...
            && sharding_result_arr_[seamless_result_count_ - 1].sharding_query_range_->end_key_ == scan_range_.end_key_))
      {
        ...
        {
          YYSYS_LOG(ERROR, "fail to get fullfilled flag from sharding result.ret=%d", ret);
        ...
        else if (true == last_scanner_is_fullfilled)
        {
          ...
          {
            YYSYS_LOG(ERROR, "fail to get range");
            is_finish= true;
          }
  ...
  bool &is_finish)
{
  ...
  {
    YYSYS_LOG(WARN,"array is full [MAX_SHARDING_RESULT_COUNT:%ld,sharding_result_count_:%ld]",
  ...
  {
    YYSYS_LOG(WARN,"fail to get fullfilled info from sharding result [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get last rowkey from sharding result [err:%d]", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    if (is_fullfilled)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get tablet range from sharding result [err:%d]", err);
      }
      if (OB_SUCCESS == err)
      {
        if (ScanFlag::FORWARD == scan_param_->get_scan_direction())
  ...
  YYSYS_LOG(DEBUG, "add sharding result. is_finish=%d, err=%d", is_finish, err);
  return err;
}
...
int ObMergerSortedOperator::next_cell()
{
  ...
  while (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next cell from ObScanner [idx:%ld,err:%d]", cur_sharding_result_idx_, err);
      }
    }
  }
  return err;
...
int ObMergerSortedOperator::get_cell(ObInnerCellInfo ** cell, bool *is_row_changed)
{
  ...
  if ((OB_SUCCESS == err) && (cur_sharding_result_idx_ < 0))
  {
    YYSYS_LOG(WARN,"call next_cell first [cur_sharding_result_idx_:%ld]", cur_sharding_result_idx_);
    err = OB_INVALID_ARGUMENT;
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get cell from ObScanner [idx:%ld,err:%d]", cur_sharding_result_idx_, err);
    }
  }
  return err;
}
