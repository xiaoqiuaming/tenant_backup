...
int ObMergerOperator::set_param(const int64_t max_memory_size, const ObScanParam & scan_param)
{
    ...
    && (scan_param_->get_orderby_column_size() == 0))
  {
    if (scan_param_->get_scan_direction() == ScanFlag::FORWARD)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to set scan param to sorted_operator_ [err:%d]", err);
      }
    }
    else
    {
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to set scan param to groupby_operator_ [err:%d]", err);
    }
  }
  if (OB_SUCCESS != err)
  {
  ...
  const ObNewRange & query_range, const int64_t limit_offset, bool &is_finish, bool &can_free_res)
{
  ...
  {
    YYSYS_LOG(WARN,"please set param first");
    err = OB_INVALID_ARGUMENT;
  }
  else if (sealed_)
  {
    YYSYS_LOG(WARN,"seal operator has already sealed");
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    ...
    case USE_GROUPBY_OPERATOR:
      ...
      break;
    default:
      YYSYS_LOG(ERROR, "status error [status_:%d]", status_);
      err = OB_ERR_UNEXPECTED;
    }
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to add sharding result [err:%d,status_:%d]", err, status_);
    }
    else
    {
  ...
  {
    YYSYS_LOG(WARN, "find sharding res size exceed the max size:total[%ld], max[%ld]",
        total_sharding_res_size_, max_sharding_res_size_);
    err = OB_MEM_OVERFLOW;
  }
  return err;
...
int ObMergerOperator::seal()
{
  ...
  {
    YYSYS_LOG(WARN,"seal operator twice");
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to seal result [err:%d,status_:%d]", err, status_);
    }
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to start return [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
...
int ObMergerOperator::get_mem_size_used()const
{
  ...
  case USE_GROUPBY_OPERATOR:
    ...
    break;
  default:
    YYSYS_LOG(WARN, "status error [status_:%d]", status_);
  }
...
int ObMergerOperator::next_cell()
{
  ...
  if (NULL == scan_param_)
  {
    YYSYS_LOG(WARN,"please set request param first");
    err = OB_INVALID_ARGUMENT;
  }
  else if (!sealed_)
  {
    YYSYS_LOG(WARN,"operator has not sealed yet, please call seal first");
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && (NULL == res_iterator_))
  {
    YYSYS_LOG(WARN,"please seal this operator first");
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get next_cell from result [err:%d,status_:%d]", err, status_);
    }
  }
  return err;
}
...
int ObMergerOperator::get_cell(common::ObInnerCellInfo** cell, bool* is_row_changed)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get cell from result [err:%d,status_:%d]", err, status_);
    }
  }
  return err;
}
