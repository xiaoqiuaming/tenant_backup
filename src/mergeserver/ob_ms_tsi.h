  ...
  namespace mergeserver
  {
    ...
    public:
      ...
      inline int init()
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to create hash map for select column name to column index map [err:%d]", err);
        }
        if ((OB_SUCCESS == err) && !inited_
          && (OB_SUCCESS != (err = groupby_cname_to_idx_map_.create(hash_map_bucket_num_))))
        {
          YYSYS_LOG(WARN,"fail to create hash map for groupby column name to column index map [err:%d]", err);
        }
        if(OB_SUCCESS == err)
        {
          inited_ = true;
        ...
        int column_type, int overwrite_flag = 0)
      {
        ...
        {
          YYSYS_LOG(WARN,"unknow column type [type:%d]", column_type);
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to add column <cname,idx> pair [err:%d]", hash_err);
            err = OB_ERROR;
          }
        }
        return err;
