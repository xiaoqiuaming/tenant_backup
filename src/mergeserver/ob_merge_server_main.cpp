  ...
  namespace mergeserver
  {
    ...
    ObMergeServerMain * ObMergeServerMain::get_instance()
    {
      if (instance_ == NULL)
      {
        ...
        if (NULL == instance_)
        {
          YYSYS_LOG(ERROR, "check alloc YaoMergeServerMain failed");
        }
      }
    ...
    int ObMergeServerMain::init_sql_server()
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid env var.");
      ...
      else
      {
        ...
        env.part_monitor_ = server_.get_partition_monitor();
        //20150525:e
    ...
    int ObMergeServerMain::do_work()
    {
      ...
      YYSYS_LOG(INFO, "cbase-mergeserver start git_version=[%s] "
      ...
      //add shili [MUTIUPS] [START MS]  20150427:b
      //�ò����н����õ���cmd_cluster_id_ ������ ms_config_.cluster_id
      //if (cmd_cluster_id_ > 0)
      else
      {
        ms_config_.cluster_id = cmd_cluster_id_;
      }
      //add  20150427:e
      //add:e
      if (
          ...
          strlen(cmd_ms_type_) > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "cmd_ms_type is %s invalid", cmd_ms_type_);
          ret = OB_ERROR;
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (strlen(config_) > 0)
        {
          YYSYS_LOG(INFO, "config file path: [%s]", config_);
        }
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "Start merge server failed, ret: [%d]", ret);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "start mergeserver failed.");
        }
        else if (OB_SUCCESS != (ret = init_sql_server()))
        {
          YYSYS_LOG(ERROR, "init sql server error.");
        }
        else if (OB_SUCCESS != (ret = server_.set_sql_session_mgr(sql_server_.get_session_mgr())))
        {
          YYSYS_LOG(ERROR, "set sql session to mergeserver failed");
        }
        else if (OB_SUCCESS != (ret = sql_server_.start(false)))
        {
          YYSYS_LOG(ERROR, "obmysql server start failed,ret=%d", ret);
        }
        else
        {
          server_.wait();
        }
        YYSYS_LOG(WARN, "stop mysql server");
        sql_server_.stop();
        YYSYS_LOG(WARN, "stop mergeserver");
        server_.stop();
      }
      YYSYS_LOG(INFO, "exit do_work");
      return ret;
    }
    ...
    void ObMergeServerMain::do_signal(const int sig)
    {
      switch (sig)
      {
        ...
        case SIGINT:
          ...
          YYSYS_LOG(WARN, "KILLED by signal, sig=%d", sig);
          sql_server_.stop();
          server_.stop_eio();
          break;
        default:
