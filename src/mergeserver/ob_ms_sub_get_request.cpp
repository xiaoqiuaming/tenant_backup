...
int ObMergerSubGetRequest::add_cell(const int64_t cell_idx)
{
  ...
  if (NULL == pget_param_)
  {
    YYSYS_LOG(WARN,"set ObGetParam first [pget_param_:%p]", pget_param_);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && ((cell_idx < 0) || (cell_idx >= pget_param_->get_cell_size())))
  {
    YYSYS_LOG(WARN,"cell_idx out of range [cell_idx:%ld,pget_param_->get_cell_size():%ld]",
  ...
  {
    YYSYS_LOG(WARN,"fail to append cell index [err:%d]", err);
  }
...
int ObMergerSubGetRequest::add_result(ObScanner &res)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to appedn result [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
...
int ObMergerSubGetRequest::get_next_param_(ObGetParam & get_param)const
{
  ...
  for (int64_t cell_idx = received_cell_count_; (OB_SUCCESS == err) && (cell_idx < cell_idx_in_org_param_.size()); cell_idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to add cell to ObGetParam [err:%d]", err);
    }
...
int ObMergerSubGetRequest::reset_iterator()
{
  ...
  for (int32_t res_idx = 0; (OB_SUCCESS == err) && (res_idx < res_vec_.size()); res_idx++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to reset ObScanner's iterator [err:%d]", err);
    }
  }
  return err;
}
...
int ObMergerSubGetRequest::has_next()
{
  ...
  if ((OB_SUCCESS == err) && (received_cell_count_ != cell_idx_in_org_param_.size()))
  {
    YYSYS_LOG(ERROR, "cell count received not coincident with expected cell count [received_cell_count_:%ld,expect:%d]",
...
int  ObMergerSubGetRequest::next(oceanbase::common::ObInnerCellInfo *&cell, int64_t & org_cell_idx)
{
  ...
  if ((OB_SUCCESS == err) && (res_iterator_idx_ < res_vec_.size()))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get cell from ObScanner [err:%d,res_iterator_idx_:%ld]", err, res_iterator_idx_);
    }
    ...
    if (OB_SUCCESS == err && NULL != cell)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot copy rowkey(%s) to string buffer.", to_cstring(cell->row_key_));
      }
    ...
    {
      YYSYS_LOG(ERROR, "not all cells poped, but all indexes are poped [poped_cell_count_:%ld,res_iterator_idx_:%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "result not correct [param->table_id_:%lu, param->column_id_:%lu, param->row_key_:%s,"
  ...
  const ObGetParam & get_param)
{
  ...
  {
    YYSYS_LOG(WARN,"invalid argument [results:%p,res_count:%ld]", results, res_count);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && (res_count > MAX_RES_SIZE))
  {
    YYSYS_LOG(WARN,"result size too large [res_count:%ld,MAX_RES_SIZE:%ld]", res_count, MAX_RES_SIZE);
  ...
  for (int64_t res_idx = 0; (OB_SUCCESS == err) && (res_idx < res_size_); res_idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to call has_next of result [err:%d,res_idx:%ld]", err, res_idx);
    }
    else if (OB_ITER_END == err)
    {
      err = OB_SUCCESS;
    ...
    else if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next cell from result [err:%d,res_idx:%ld]", err, res_idx);
      }
    }
  }
  if (OB_SUCCESS == err)
...
int ObGetMerger::next_cell()
{
  ...
  if (NULL == res_vec_)
  {
    YYSYS_LOG(WARN,"ObGetMerger not init yet");
    err = OB_INVALID_ARGUMENT;
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [cur_cell_idx_:%ld,got_cell_idx:%ld,res_idx:%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to call has_next [err:%d,res_idx:%ld]", err, info_heap_[0].res_idx_);
    }
    else if (OB_ITER_END == err)
    {
    ...
    else if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next cell from result [res_idx:%ld,err:%d]", res_idx, err);
      }
      if (OB_SUCCESS == err)
      {
...
int ObGetMerger::get_cell(ObInnerCellInfo** cell, bool* is_row_changed)
{
  ...
  if (NULL == cell)
  {
    YYSYS_LOG(WARN,"invalid agument [cell:%p]", cell);
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err)
  {
