  ...
  oceanbase::common::ObGetParam *get_param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "get is req fullfilled failed:ret[%d]", ret);
  }
  // check the result cell size must be equal with fullfilled item count for get
  else if (result.get_cell_num() != item_count)
  {
    YYSYS_LOG(WARN, "check result cell size not coincident with fullfill num:"
      "size[%ld], fullfill[%ld]", result.get_cell_num(), item_count);
    ret = OB_INNER_STAT_ERROR;
  }
  else
  {
    YYSYS_LOG(INFO, "get next param:finish[%d], total[%ld], org[%ld], cur_count[%ld], cur_finish[%d]",
    ...
    {
      YYSYS_LOG(ERROR, "check cell size or finish status failed:total[%ld], org[%ld], "
  ...
  if ((OB_SUCCESS == ret) && (false == finish) && (NULL != get_param))
  {
    ...
    *read_param = org_param;
    ...
    while (count < cell_size)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check get param cell failed:cell[%ld], item[%ld], index[%ld]",
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add cell to new get param failed:cell[%ld], item[%ld], "
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "check add any cell failed:cell[%ld], item[%ld], index[%ld], add[%ld]",
...
namespace
{
  int allocate_range_buffer(ObNewRange &range, ObMemBuffer &buffer)
  {
    ...
    if (NULL != ptr)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to allocate memory to copy start_key");
      }

      if((OB_SUCCESS == err) && (OB_SUCCESS != (err = range.end_key_.deep_copy(range.end_key_, end_key_wrapper))))
      {
        YYSYS_LOG(WARN, "fail to allocate memory to copy end_key");
      }
    }
    return err;
  }
  ...
  ObScanParam *scan_param, ObMemBuffer &range_buffer)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "%s", "unexpected error, can't get range from org scan param");
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "%s", "unexpected error, scanner should contain at least one cell");
      }
    }
  }
  ...
  ObNewRange &cur_range)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get last rowkey from prev result [err:%d]", err);
      }
    }
  }
  ...
  ObStringBuf &buf)
{
  ...
  {
    YYSYS_LOG(WARN,"argument error [org_scan_param.get_group_by_param().get_aggregate_row_width():%ld,"
  ...
  {
    YYSYS_LOG(WARN,"argument error, backward scan are limited only for backward page browsing, "
  ...
  {
    YYSYS_LOG(WARN,"fail to get tablet range from prev result [err:%d]", err);
  }
  if ((OB_SUCCESS == err)  && (0 < prev_limit_offset))
  {
    ...
    {
      YYSYS_LOG(WARN,"cs tablet splitted during the whole request, please try this request again");
      err = OB_NEED_RETRY;
    }
  }
  ...
  else if ((OB_SUCCESS == err) && (org_scan_param.get_orderby_column_size() > 0))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get fullfill info from prev result [err:%d]", err);
    }
    if ((OB_SUCCESS == err) && (fullfilled_item_num != prev_scan_result.get_row_num()))
    {
      /// YYSYS_LOG(ERROR,"unexpected error [prev_result::fullfilled_item_num:%ld, prev_result::get_row_num():%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if ((OB_SUCCESS == err) && (str.length() > 0))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to deep copy cur_range.start_key_ [err:%d]", err);
      }
    }
    ...
    if ((OB_SUCCESS == err) && (str.length() > 0))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to deep copy cur_range.end_key_ [err:%d]", err);
      }
    }
  }
  return err;
