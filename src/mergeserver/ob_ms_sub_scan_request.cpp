...
namespace oceanbase
{
  namespace mergeserver
  {
      ...
      ObStringBuf *buffer_pool)
    {
      ...
      {
        YYSYS_LOG(WARN, "replica count exceeds max value. [replica_count:%d][max_count:%d]",
          replica_count, ObTabletLocationList::MAX_REPLICA_COUNT);
        err = OB_INVALID_ARGUMENT;
      }
      else if (replica_count <= 0)
      {
        YYSYS_LOG(WARN, "replica count exceeds min value. [replica_count:%d][min_count:%d]",
            replica_count, 1);
        err = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(WARN, "null pointer error! [scan_param=%p][bufffer_pool=%p]", scan_param, buffer_pool);
        err = OB_INVALID_ARGUMENT;
      }
    ...
    int ObMergerSubScanRequest::select_cs(ObChunkServerItem & selected_server)
    {
      ...
      {
        YYSYS_LOG(WARN, "no valid chunkserver selected. [sel_replica=%d]", sel_replica);
        err = OB_ERROR;
      }
      else
      {
        ...
        // YYSYS_LOG(DEBUG, "[tried_replica_count_:%d,total_replica_count_:%d]", tried_replica_count_, total_replica_count_);
      }
      return err;
    }
      ...
      ObMergerRequest *client_request, ObChunkServerItem & selected_server)
    {
      ...
      if (false == inited_)
      {
        YYSYS_LOG(WARN, "ObMergerSubScanRequest class not inited. call init() first");
        err = OB_NOT_INIT;
      }
      else if (NULL == client_request || NULL == rpc_event)
      {
        YYSYS_LOG(WARN, "NULL pointer error. client_request=%p, rpc_event=%p", client_request, rpc_event);
        err = OB_INVALID_ARGUMENT;
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to select chunkserver [request_id:%lu]",
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to init rpc_event. [request_id:%lu]", client_request->get_request_id());
          }
          else
          {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "release allocated ObMergerRpcEvent object. [request_id:%lu]", client_request->get_request_id());
        }
      }
      if (OB_SUCCESS == err)
      {
      ...
      bool &is_first)
    {
      ...
      {
        YYSYS_LOG(WARN, "NULL pointer error. agent_event=NULL");
        err = OB_INVALID_ARGUMENT;
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "check_event_id failed. [err=%d]", err);
        }
        else
        {
          if (true == exist)
          {
            ...
            {
              // YYSYS_LOG(DEBUG, "new agent event arrived. add to result set. scanner=%p", &(agent_event->get_result()));
              result_ = agent_event;
            }
          }
          else
    ...
    int ObMergerSubScanRequest::add_event_id(uint64_t event_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid event_id [event_id:%ld]", event_id);
        err = OB_INVALID_ARGUMENT;
      }
      ...
      {
        YYSYS_LOG(ERROR, "event_id[] size overflow. [event_id_pos_:%d][max size:%d][event_id:%ld]",
    ...
    int ObMergerSubScanRequest::check_event_id(uint64_t event_id, bool &exist)
    {
      ...
      {
        YYSYS_LOG(WARN, "event_id[] size overflow. [event_id_pos_:%d][max size:%d][event_id:%ld]",
          event_id_pos_, MAX_BACKUP_TASK_NUM, event_id);
        err = OB_SIZE_OVERFLOW;
      }
      else if (OB_INVALID_ID == event_id)
      {
        YYSYS_LOG(WARN, "invalid event id. [event_id=%lu]", event_id);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
