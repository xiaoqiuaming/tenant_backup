...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    int ObMergeCallback::process(easy_request_t* r)
    {
      ...
      if (NULL == r)
      {
        YYSYS_LOG(WARN, "request is empty, r = %p", r);
        ret = EASY_BREAK;
      }
      else if (NULL == r->ipacket)
      {
        YYSYS_LOG(WARN, "request is empty, r->ipacket = %p", r->ipacket);
        ret = EASY_BREAK;
      }
      else
      {
    ...
    int ObMergeCallback::rpc_process(easy_request_t* r)
    {
      ...
      if (NULL == r)
      {
        YYSYS_LOG(WARN, "async response is empty, r = %p", r);
        ret = EASY_ERROR;
      }
      else if (NULL == r->user_data)
      {
        YYSYS_LOG(WARN, "async response r->user_data = %p", r->user_data);
        ret = EASY_ERROR;
      }
      else //call handle_packet whatever r->ipacket is null or not
      {
        ...
        if (NULL == packet)
        {
          YYSYS_LOG(WARN, "r = %p r->ipacket = %p", r, r->ipacket);
        }
    ...
    int ObMergeCallback::sql_process(easy_request_t* r)
    {
      ...
      else //call handle_packet whatever r->ipacket is null or not
      {
        ...
        if (NULL == r->ipacket)
        {
          YYSYS_LOG(WARN, "message timeout,r = %p r->ipacket = %p", r, r->ipacket);
        }
        else
        {
          packet->set_receive_ts(yysys::CTimeUtil::getTime());
