...
namespace oceanbase
{
  ...
  namespace mergeserver
  {
    ...
    public:
      ...
      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150717:bsss
      int ups_plan_execute(const int64_t timeout,const sql::ObPhysicalPlan &plan, const ObServer &update_server,sql::ObUpsResult &result);
      //add 20150717:e
      ...
      int get_master_update_server(const int64_t paxos_id,common::ObServer & update_server);
      int fetch_master_update_server_list(int32_t & count);
      //add 20150706:e

      //add by maosy [MultiUps 1.0] [batch_iud_snapshot] 20170622 b:
      int32_t get_master_update_server_count()
      {
          return master_update_server_list_.ups_count_ ;
      }
      int get_master_ups(const int32_t index,int64_t & paxos_id,common::ObServer & update_server)
      {
          int ret = OB_SUCCESS;
          if(index < master_update_server_list_.ups_count_)
          {
              update_server = master_update_server_list_.ups_array_[index].addr_;
              paxos_id = master_update_server_list_.ups_array_[index].paxos_id_;
          }
          else
          {
      ...
      int get_stmt_start_time(ObServer ups_server ,int64_t &public_transid);
      // add by e
    protected:
      //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150701:b
      /**
       * @brief ms send rollback trans to every participants
       * @param req contain all participants info
       * @return
       */
      int ups_rollback_distributed_trans(const common::ObEndTransReq &req);
      //add 20150701:e
      // check inner stat
