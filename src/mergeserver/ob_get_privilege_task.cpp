...
const ObString & ObGetPrivilegeTask::get_users = ObString::make_string(
...
const ObString & ObGetPrivilegeTask::get_table_privileges = ObString::make_string(
            "SELECT /*+read_consistency(WEAK) */t.user_id,t.db_id,t.table_id,t.priv_all,t.priv_alter,t.priv_create,t.priv_create_user,t.priv_delete,t.priv_drop,t.priv_grant_option,t.priv_insert,t.priv_update,t.priv_select, priv_replace FROM __all_table_privilege t");//add liumz, [drop table -> clean table priv]20150902
...
const ObString & ObGetPrivilegeTask::get_database_privilege = ObString::make_string(
            "SELECT /*+read_consistency(WEAK) */dp.user_id,dp.db_id,dp.priv_all,dp.priv_alter,dp.priv_create,dp.priv_create_user,dp.priv_delete,dp.priv_drop,dp.priv_grant_option,dp.priv_insert,dp.priv_update,dp.priv_select,dp.priv_replace,dp.is_locked FROM __all_database_privilege dp");
...
void ObGetPrivilegeTask::runTimerTask()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "init privilege failed, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = load_user_table(p_privilege)))
    {
      YYSYS_LOG(WARN, "load user table failed, ret=%d", ret);
    }
    else if (OB_SUCCESS != (ret = load_privilege_table(p_privilege)))
    {
      YYSYS_LOG(WARN, "load privilege table failed, ret=%d", ret);
    }
    //add wenghaixing [database manage]20150607
    else if (OB_SUCCESS != (ret = load_database_table(p_privilege)))
    {
      YYSYS_LOG(WARN, "load database table failed, ret = %d", ret);
    }
    else if (OB_SUCCESS != (ret = load_database_priv_table(p_privilege)))
    {
      YYSYS_LOG(WARN, "load database priv table failed, ret = %d", ret);
    }
    else if (OB_SUCCESS != (ret = load_table_database(p_privilege)))
    {
      YYSYS_LOG(WARN, "load database table info failed,ret = %d", ret);
    }
    //add e
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "renew privilege failed, ret=%d", ret);
      }
    }
...
int ObGetPrivilegeTask::load_user_table(ObPrivilege *p_privilege)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "execute privilge sql failed,sql=%.*s, ret=%d", get_users.length(), get_users.ptr(), ret);
    }
    ...
    else
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "add user privilege to ObPrivilege failed, ret=%d", ret);
          break;
        }
      }
    }
...
int ObGetPrivilegeTask::load_privilege_table(ObPrivilege *p_privilege)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "execute privilge sql failed,sql=%.*s, ret=%d", get_table_privileges.length(), get_table_privileges.ptr(), ret);
    }
    else if (OB_SUCCESS != (ret = rs.open()))
    {
      YYSYS_LOG(ERROR, "open rs failed, ret=%d", ret);
    ...
    else
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "add table privilege to ObPrivilege failed, ret=%d", ret);
        }
      }
    }
    sql_proxy_->cleanup_sql_env(context, sql_rs);
...
int ObGetPrivilegeTask::load_database_table(ObPrivilege *p_privilege)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "execute database sql failed,sql = %.*s, ret = %d", get_databases.length(), get_databases.ptr(), ret);
    }
    else if(OB_SUCCESS != (ret = rs.open()))
    {
      YYSYS_LOG(WARN, "open rs failed, ret = %d", ret);
    }
    else
    {
      const ObRow *row = NULL;
      while (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "add database name to ObPrivilege failed, ret = %d", ret);
          break;
        }
      }
    }
...
int ObGetPrivilegeTask::load_database_priv_table(ObPrivilege *p_privilege)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "execute database sql failed,sql = %.*s, ret = %d", get_database_privilege.length(), get_database_privilege.ptr(), ret);
    }
    else if(OB_SUCCESS != (ret = rs.open()))
    {
      YYSYS_LOG(WARN, "open rs failed, ret = %d", ret);
...
int ObGetPrivilegeTask::load_table_database(ObPrivilege *p_privilege)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "execute database sql failed,sql = %.*s, ret = %d", get_first_tablet_entry.length(), get_first_tablet_entry.ptr(), ret);
    }
    else if(OB_SUCCESS != (ret = rs.open()))
    {
      YYSYS_LOG(WARN, "open rs failed, ret = %d", ret);
