...
int ObInsertCache::init(int64_t total_size)
{
  ...
  {
    in_use_ = false;
    YYSYS_LOG(INFO, "Bloom Filter Cache switch off");
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      ...
      {
        YYSYS_LOG(ERROR, "init bloom filter cache failed, ret=%d", ret);
      }
      in_use_ = false;
    }
    else
...
int ObInsertCache::put(const InsertCacheKey & key, const InsertCacheValue & value)
{
  ...
  if (!in_use_)
  {
    YYSYS_LOG(ERROR, "ObInsertCache is not initialized to use");
    ret = OB_NOT_INIT;
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "put item to bloom filter cache failed, ret=%d", ret);
    }
  }
  return ret;
}
int ObInsertCache::revert(InsertCacheWrapValue & value)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "revert item to bloom filter cache failed, ret=%d", ret);
    }
  }
  return ret;
}
int ObInsertCache::get(const InsertCacheKey & key, InsertCacheWrapValue & value)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get item from bloom filter cache failed, ret=%d", ret);
    }
    if (__sync_fetch_and_add(&get_counter_, 1) % ITEM_COUNT_PRINT_FREQUENCY == 0)
    {
      YYSYS_LOG(INFO, "bloom filter cache cached item number: %ld", bf_cache_.count());
    }
  }
  return ret;
}
