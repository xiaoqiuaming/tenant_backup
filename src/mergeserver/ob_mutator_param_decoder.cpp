...
int ObMutatorParamDecoder::add_cell(const ObCellInfo & cell, ObGetParam & get_param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "check add cell to get param failed:ret[%d]", ret);
  }
  return ret;
}
    ...
    ObGetParam & return_param, ObGetParam & get_param)
{
  ...
  while (OB_SUCCESS == (ret = mutator.next_cell()))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get cell failed:index[%ld], cell[%p], ret[%d]",
          count, org_cell, ret);
    }
    else if (NULL == org_cell)
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "check get cell failed:index[%ld], cell[%p]", count, org_cell);
    }
    ...
    else if (true == org_cell->is_return())
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add cell for return failed:index[%ld], ret[%d]", count, ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "decode cell failed:index[%ld], ret[%d]", count, ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "add decoded mutator failed:index[%ld], ret[%d]", count, ret);
      }
    }
    if (OB_SUCCESS != ret)
    {
    ...
    ObMutator & decoded_mutator, ObGetParam & get_param)
{
  ...
  if ((OB_SUCCESS == ret) && (true == decoded_cell.is_return()))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "check add cell failed:ret[%d]", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "add decoded mutator cell to get param succ");
    }
  }
  return ret;
}
    ...
    ObMutator & decoded_mutator, ObGetParam & return_param, ObGetParam & get_param)
{
  ...
  {
    YYSYS_LOG(WARN, "decode mutator cell failed:ret[%d]", ret);
  }
  return ret;
}
