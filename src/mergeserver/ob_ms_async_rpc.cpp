  ...
  const ObClientManager * rpc_frame) 
{
  ...
  {
    YYSYS_LOG(WARN, "check input failed:rpc_buffer[%p], rpc_frame[%p]",
...
int ObMergerAsyncRpcStub::get_rpc_buffer(ObDataBuffer & data_buffer) const
{
  ...
  else
  {
    ...
    if (NULL == rpc_buffer)
    {
      YYSYS_LOG(WARN, "get thread rpc buff failed:buffer[%p].", rpc_buffer);
      ret = OB_INNER_STAT_ERROR;
    }
    else
    {
  ...
  const ObGetParam & get_param, ObMergerRpcEvent & result) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send get request to server failed:client[%lu], event[%lu], "
  ...
  const int64_t session_id, const int32_t req_type, ObMergerRpcEvent & result)const
{
  ...
  {
    YYSYS_LOG(WARN,"fail to post next request of session [err:%d,session_id:%ld]", err, 
      session_id);
  }
  return err;
}
  ...
  const ObScanParam & scan_param, ObMergerRpcEvent & result) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "serialize scan param failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send scan request to server failed:client[%lu], event[%lu], "
        "server[%s], ret[%d]", result.get_client_id(), result.get_event_id(), to_cstring(server), ret);
    }
  }
  return ret;
  ...
  const ObSqlGetParam & get_param, ObMsSqlRpcEvent & result) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    result.start();
    result.set_timeout_us(timeout);
    YYSYS_LOG(DEBUG,"get_stmt  :%s",to_cstring(get_param));
    YYSYS_LOG(DEBUG,"get_stmt_type:%ld",(int64_t)get_param.get_stmt_type());
    ret = get_param.serialize(data_buff.get_data(), data_buff.get_capacity(), 
      data_buff.get_position());
    if (OB_SUCCESS != ret)
    {
