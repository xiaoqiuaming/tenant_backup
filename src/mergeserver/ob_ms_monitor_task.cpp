...
int ObMergerMonitorTask::init(const ObMergeServer * server)
{
  ...
  if (NULL == server)
  {
    ...
    YYSYS_LOG(WARN, "check merge server failed:server[%p]", server);
  }
  else
  {
    server_ = server;
...
void ObMergerMonitorTask::runTimerTask(void)
{
  ...
  {
    YYSYS_LOG(ERROR, "%s", "check monitor timer task inner stat failed");
  }
  else
  {
    ...
    if (new_drop_counter > old_drop_counter_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "check dropped packet count:drop[%lu], current[%lu], old[%lu], min[%ld]",
            new_drop_counter - old_drop_counter_, new_drop_counter, old_drop_counter_, min_drop_error_count_);
      }
      else
      {
        YYSYS_LOG(WARN, "check dropped packet count:drop[%lu], current[%lu], old[%lu], min[%ld]",
            new_drop_counter - old_drop_counter_, new_drop_counter, old_drop_counter_, min_drop_error_count_);
      }
    }
