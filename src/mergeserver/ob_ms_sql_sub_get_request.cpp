...
int ObMsSqlSubGetRequest::add_row(const int64_t row_idx)
{
  ...
  if (NULL == pget_param_)
  {
    YYSYS_LOG(WARN,"set ObSqlGetParam first [pget_param_:%p]", pget_param_);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && ((row_idx < 0) || (row_idx >= pget_param_->get_row_size())))
  {
    YYSYS_LOG(WARN,"row_idx out of range [row_idx:%ld,pget_param_->get_row_size():%ld]",
  ...
  {
    YYSYS_LOG(WARN,"fail to append row index [err:%d]", err);
  }
...
int ObMsSqlSubGetRequest::add_result(ObNewScanner &res)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    retry_times_ = 0;
    YYSYS_LOG(DEBUG, "got result. scanner row=%d, total received row=%ld", res_vec_.size(), received_row_count_);
  }
  return err;
}
...
int ObMsSqlSubGetRequest::get_next_param_(ObSqlGetParam & get_param)const
{
  ...
  for (int64_t row_idx = received_row_count_; (OB_SUCCESS == err) && (row_idx < row_idx_in_org_param_.size()); row_idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to add row to ObSqlGetParam [err:%d]", err);
    }
...
int  ObMsSqlSubGetRequest::get_next_row(oceanbase::common::ObRow &row, int64_t & org_row_idx)
{
  ...
  if ((OB_SUCCESS == err) && (res_iterator_idx_ < res_vec_.size()))
  {
    ...
    do
    {
      ...
      //YYSYS_LOG(INFO, "get next row.size=%d, err=%d, row=%s", res_vec_.size(), err, to_cstring(row));
      if (OB_ITER_END == err)
      {
        // YYSYS_LOG(INFO, "cur scanner used up. switch to next or end. res_iter_idx=%ld", res_iterator_idx_);
        res_iterator_idx_++;
      }
      else if (OB_SUCCESS == err)
      {
        // YYSYS_LOG(INFO, "row=%s", to_cstring(row));
        break;
      }
      else
      {
        YYSYS_LOG(WARN, "fail to get next row. %ld, %d", res_iterator_idx_, err);
        break;
      }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(ERROR, "not all rows poped, but all indexes are poped [poped_row_count_:%ld,res_iterator_idx_:%ld]",
      ...
      else
      {
...
#if 0
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to get rowkey. err=%d", err);
          }
          else if (*((*pget_param_)[org_row_idx]) != *verify_rowkey)
          {
            YYSYS_LOG(ERROR, "result not correct param->row_key_:%s, result->row_key_:%s]",
    ...
    {
      YYSYS_LOG(WARN,"get row from ObNewScanner [err:%d,res_iterator_idx_:%ld] fail", err, res_iterator_idx_);
    }
  }
    ...
    const ObSqlGetParam & get_param)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    for (int32_t res_idx = 0; (OB_SUCCESS == err) && (res_idx < res_size_); res_idx ++)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next row from result [err:%d,res_idx:%d]", err, res_idx);
      }
    }
    if (OB_SUCCESS == err)
    {
...
int ObSqlGetMerger::get_next_row(common::ObRow &row)
{
  ...
  if (NULL == res_vec_)
  {
    YYSYS_LOG(WARN,"ObSqlGetMerger not init yet");
    err = OB_INVALID_ARGUMENT;
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [cur_row_idx_:%ld,got_row_idx:%ld,res_idx:%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get next row from result [res_idx:%d,err:%d]", res_idx, err);
    }
  }
