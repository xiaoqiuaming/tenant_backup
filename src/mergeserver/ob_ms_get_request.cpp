...
int ObMergerGetRequest::distribute_request()
{
  ...
  for (int64_t row_idx = 0; (row_idx < row_count) && (OB_SUCCESS == err); ++row_idx)
  {
    ...
    {
      YYSYS_LOG(WARN, "wrong row range, cell_size=%ld, start=%ld, end=%ld",
    ...
    for (int64_t i = row_start; i < row_end && OB_SUCCESS == err; ++i)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to add cell to sub request [err:%d]", err);
      }
    }
  }
...
int ObMergerGetRequest::send_request(const int64_t sub_req_idx, const int64_t timeout_us)
{
  ...
  if ((OB_SUCCESS == err)
    ...
    && (OB_ITER_END != err))
  {
    YYSYS_LOG(WARN,"fail to get next param of sub request [request_id:%lu,sub_req_idx:%ld,err:%d]",
  ...
  for (;(svr_idx < loc_list.size()) && (OB_SUCCESS == err); svr_idx ++)
  {
    if (loc_list[svr_idx].server_.chunkserver_.get_ipv4() == sub_requests_[sub_req_idx].get_last_svr_ipv4())
    {
      if (sub_requests_[sub_req_idx].get_last_svr_ipv4() == sub_requests_[sub_req_idx].get_fail_svr_ipv4())
      {
        ...
        YYSYS_LOG(INFO, "sub req retry [request_id:%lu,sub_req_idx:%ld,tried_times:%ld,retry_svr:%d]", get_request_id(),
  ...
  {
    YYSYS_LOG(WARN,"fail to create ObMergerRpcEvent [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
  ...
  ObMergerRpcEvent & prev_rpc, const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      YYSYS_LOG(INFO, "session next [request_id:%lu,prev_rpc:%lu,cur_rpc:%lu]", get_request_id(),
  ...
  const int64_t max_parellel_count, const int64_t max_get_rows_per_subreq, const int64_t reserve_get_param_count)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to create hash map for request distribution");
...
int ObMergerGetRequest::process_result(const int64_t timeout_us, ObMergerRpcEvent *rpc_event, bool& finish)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(DEBUG, "rpc event finish [request_id:%lu,rpc_event->client_id:%lu,rpc_event_id:%lu,#cell:%ld,"
  ...
  {
    YYSYS_LOG(ERROR, "rpc event not belong to current request [request_id:%lu,rpc_event_id:%lu]", get_request_id(),
  ...
  {
    YYSYS_LOG(WARN,"fail to add result to sub request [request_id:%lu,rpc_event_id:%lu,err:%d]", get_request_id(),
  ...
  if ((OB_SUCCESS == err) && (OB_SUCCESS != rpc_event->get_result_code()))
  {
    ...
    {
      YYSYS_LOG(WARN,"sub request retry too many times [request_id:%lu,tried_times:%ld,MAX_RETRY_TIMES:%d]",
  ...
  {
    YYSYS_LOG(WARN,"fail to send request [request_id:%lu,err:%d]", get_request_id(), err);
  }
  if ((OB_SUCCESS == err) && (finished_sub_request_count_ > total_sub_request_count_))
  {
    YYSYS_LOG(ERROR, "unexpected error [finished_sub_request_count_:%d,total_sub_request_count_:%d,request_id:%lu]",
...
int ObMergerGetRequest::reset_iterator()
{
  ...
  if (!sealed_)
  {
    YYSYS_LOG(WARN,"get request haven't finished yet");
    err = OB_INVALID_ARGUMENT;
  }
  for (int64_t sub_req_idx = 0; (sub_req_idx < total_sub_request_count_) && (OB_SUCCESS == err); sub_req_idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to reset iterator of sub req [err:%d]", err);
    }
  }
...
int ObMergerGetRequest::next_cell()
{
  ...
  if (sealed_)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to call next_cell [err:%d]", err);
    }
  }
  else
  {
  ...
  bool &got_all_result)
{
  ...
  if (!sealed_)
  {
    YYSYS_LOG(WARN,"request not finished yet");
    err  = OB_INVALID_ARGUMENT;
  }
  ...
  while ((OB_SUCCESS == err) && (!size_over_flow))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get cell from ObGetMerger [err:%d]", err);
    }
    if (OB_SUCCESS == err)
    {
      got_cell_count ++;
    ...
    {
      YYSYS_LOG(WARN,"row cell count is too large [cur_row_cell_cnt_:%ld, MAX_ROW_COLUMN_COUNT:%ld]",
  ...
  {
    YYSYS_LOG(WARN,"fail to set fullfill infomation [err:%d]", err);
  }
  return err;
}
