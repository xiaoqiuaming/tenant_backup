...
int ObMsSqlGetRequest::open()
{
  ...
  {
    YYSYS_LOG(WARN,"fail to distribute request [err:%d]", err);
  }
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = check_if_need_more_req(-1,timeout_us_))))
  {
    YYSYS_LOG(WARN,"fail to send request to server [request_id:%lu,err:%d]", get_request_id(),err);
  }
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = wait(timeout_us_))))
  {
    YYSYS_LOG(WARN, "fail to wait result. err=%d", err);
  }
  return err;
}
...
int ObMsSqlGetRequest::distribute_request()
{
  ...
  for (int64_t row_idx = 0; (row_idx < row_count) && (OB_SUCCESS == err); ++row_idx)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get tablet location [err:%d,table_id:%lu,rowkey:%s]",
    ...
    {
      YYSYS_LOG(WARN,"fail to get tablet location [table_id:%lu,rowkey:%s]",
    ...
    if ((OB_SUCCESS == err) && (sub_req_idx < 0 || is_subreq_full))
    {
      ...
      if (total_sub_request_count_ < MAX_SUB_GET_REQUEST_NUM)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to alloc sub request. err=%d", err);
        }
        else if (OB_SUCCESS != (err = sub_requests_.push_back(sub_request)))
        {
          YYSYS_LOG(WARN, "fail to add subreq to vector. err=%d", err);
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "row desc not set.");
            err = OB_NOT_INIT;
          }
        }
        //说明这个cs之前没有被分配过子请求
        ...
        {
          YYSYS_LOG(WARN,"fail to insert hash map [hash_err:%d]", hash_err);
          err = OB_ERROR;
        }
        if (OB_SUCCESS == err)
        {
        ...
        if (true == is_subreq_full)
        {
          ...
          {
            YYSYS_LOG(WARN, "update the full cs failed:%ld, idx:%ld, ret:%d", ip, sub_req_idx, hash_err);
          }
        }
      }
      else
      {
        YYSYS_LOG(WARN,"sub request out of range [total_sub_request_count_:%d,"
    ...
    else if (OB_SUCCESS == err && sub_req_idx < sub_requests_.size()) // exsit already
    {
      ...
      {
        YYSYS_LOG(WARN, "sub request not found. sub_req_idx=%ld", sub_req_idx);
    ...
    {
      YYSYS_LOG(WARN,"fail to add row to sub request [err:%d]", err);
    }
    YYSYS_LOG(DEBUG, "distribute request: row[%ld] distribute to sub req[%ld]", row_idx, sub_req_idx);
  } /* end for */
...
int ObMsSqlGetRequest::alloc_sub_request(ObMsSqlSubGetRequest *&sub_req)
{
  ...
  {
    YYSYS_LOG(DEBUG, "alloc sub request, sub_req=%p, total_sub_request_count_=%d",sub_req, total_sub_request_count_);
  }
  return err;
}
...
int ObMsSqlGetRequest::send_request(const int32_t sub_req_idx, const int64_t timeout_us)
{
  ...
  if ((OB_SUCCESS == err)
    ...
    && (OB_ITER_END != err))
  {
    YYSYS_LOG(WARN,"fail to get next param of sub request [request_id:%lu,sub_req_idx:%d,err:%d]",
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get tablet location [err:%d,table_id:%lu,rowkey:%s]", err,
    ...
    for (svr_idx = 0;(svr_idx < loc_list.size()) && (OB_SUCCESS == err); svr_idx ++)
    {
      if (loc_list[svr_idx].server_.chunkserver_.get_ipv4() == sub_request.get_last_svr_ipv4())
      {
        if (sub_request.get_last_svr_ipv4() == sub_request.get_fail_svr_ipv4())
        {
          ...
          YYSYS_LOG(INFO, "sub req retry [request_id:%lu,sub_req_idx:%d,tried_times:%ld,retry_svr:%s]", get_request_id(),
    ...
    if ((OB_SUCCESS == err) && (svr_idx >= loc_list.size()))
    {
      ...
      YYSYS_LOG(WARN, "all servers get dirty. pick one randomly: loc_size[%ld], retry_svr:[%s]", 
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to create ObMsSqlRpcEvent [err:%d]", err);
    }
    else
    {
      rpc_event->set_server(cs_addr);
      //add shili [MultiUPS] [PHYSICAL_PLAN_TRANSFORM] 20150612:b
      cur_sub_get_param_.set_stmt_type(stmt_type_);
      YYSYS_LOG(DEBUG,"get_stmt_type:%ld",(int64_t)cur_sub_get_param_.get_stmt_type());
      //add 20150612:e
              ...
              cs_addr,cur_sub_get_param_,*rpc_event)))
      {
        ...
        if (OB_SUCCESS != e)
        {
          YYSYS_LOG(WARN, "fail to destroy rpc_event. [e=%d]", e);
        }
        else
        {
  ...
  ObMsSqlRpcEvent & prev_rpc, const int64_t timeout_us)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    else
    {
      ...
      YYSYS_LOG(DEBUG, "session next [request_id:%lu,prev_rpc:%lu,cur_rpc:%lu]", get_request_id(),
  ...
  const int64_t timeout_us, ObMsSqlRpcEvent *prev_rpc_event)
{
  ...
  if ((current_sub_req_idx >= 0) && (!sub_requests_[current_sub_req_idx]->finished()))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get fullfill info from resut [request_id:%lu,rpc_event_id:%lu,err:%d]",
    ...
    if ((OB_SUCCESS == err) && (NULL != prev_rpc_event) && (prev_rpc_event->get_session_id() > 0) && !fullfilled)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get_session_next [err:%d]", err);
      }
    }
    else if ((OB_SUCCESS == err)  && fullfilled && fullfilled_item_count <= 0)
    {
      YYSYS_LOG(ERROR, "unexpect filfilled item count %ld", fullfilled_item_count);
      err = OB_ERR_UNEXPECTED;
    }
...
int ObMsSqlGetRequest::process_result(const int64_t timeout_us, ObMsSqlRpcEvent *rpc_event, bool& finish)
{
  ...
  if (NULL == rpc_event)
  {
    YYSYS_LOG(WARN,"invalid argument [rpc_event:%p]", rpc_event);
    err = OB_INVALID_ARGUMENT;
  }
  else
  {
    int64_t get_event_time_cost = yysys::CTimeUtil::getTime() - rpc_event->get_timestamp();
    ...
    {
      YYSYS_LOG(DEBUG, "rpc event finish [request_id:%lu,rpc_event->client_id:%lu,rpc_event_id:%lu,#row:%ld,"
    ...
    {
      YYSYS_LOG(WARN, "rpc event finish [request_id:%lu,rpc_event->client_id:%lu,rpc_event_id:%lu,"
          "timeused:%ld,timeout:%ld,server:%s,session_id:%ld,err:%d]",
          get_request_id(), rpc_event->get_client_id(), rpc_event->get_event_id(),
  ...
  {
    YYSYS_LOG(ERROR, "rpc event not belong to current request [event server:%s,request_id:%lu,rpc_event_id:%lu]", 
  ...
  {
    YYSYS_LOG(WARN,"fail to update location cache for server %s", to_cstring(rpc_event->get_server()));
  }
  if ((OB_SUCCESS == err)
    && (OB_SUCCESS == rpc_event->get_result_code())
    && (OB_SUCCESS != (err = sub_requests_[sub_req_idx]->add_result(rpc_event->get_result()))))
  {
    YYSYS_LOG(WARN,"fail to add result to sub request [event server:%s, request_id:%lu,rpc_event_id:%lu,err:%d]", 
        to_cstring(rpc_event->get_server()),
        get_request_id(),
        rpc_event->get_event_id(), 
        err);
  ...
  if ((OB_SUCCESS == err) && (OB_SUCCESS != rpc_event->get_result_code()))
  {
    ...
    {
      YYSYS_LOG(WARN,"sub request retry too many times [last server:%s, request_id:%lu,tried_times:%ld,MAX_RETRY_TIMES:%d,result_code=%d]",
          to_cstring(rpc_event->get_server()),
          get_request_id(),
          tried_times,
          MAX_RETRY_TIMES,
  ...
  {
    YYSYS_LOG(WARN,"fail to send request [server:%s,request_id:%lu,err:%d]", to_cstring(rpc_event->get_server()), get_request_id(), err);
  }
  if ((OB_SUCCESS == err) && (finished_sub_request_count_ > total_sub_request_count_))
  {
    YYSYS_LOG(ERROR, "unexpected error [server:%s,finished_sub_request_count_:%d,total_sub_request_count_:%d,request_id:%lu]",
        to_cstring(rpc_event->get_server()),
        finished_sub_request_count_, 
        total_sub_request_count_,
        get_request_id());
  ...
  if ((OB_SUCCESS == err) && (finish))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to seal result [err:%d]", err);
    }
    else
    {
      sealed_ = true;
...
int ObMsSqlGetRequest::get_next_row(ObRow &row)
{
  ...
  {
    YYSYS_LOG(WARN,"get request not finished yet");
    err = OB_INVALID_ARGUMENT;
  }
  return err;
}
