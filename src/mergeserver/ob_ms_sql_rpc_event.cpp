...
ObMsSqlRpcEvent::~ObMsSqlRpcEvent()
{
  //YYSYS_LOG(INFO, "destroy event[%p] client_request_id_[%ld] client_request_[%p]"
...
int ObMsSqlRpcEvent::init(const uint64_t client_id, ObMsSqlRequest * request)
{
  ...
  if ((OB_INVALID_ID == client_id) || (NULL == request))
  {
    ...
    YYSYS_LOG(WARN, "check input failed:client[%lu], event[%lu], request[%p]",
  ...
  else
  {
    ...
    YYSYS_LOG(DEBUG, "init rpc event succ:client[%lu], event[%lu], request[%p]",
        client_id, get_event_id(), request);
  }
  return ret;
}
...
int ObMsSqlRpcEvent::parse_packet(ObPacket * packet, void * args)
{
  ...
  if (NULL == packet)
  {
    ...
    YYSYS_LOG(WARN, "message timeout, packet from chunkserver[%s] is NULL, request_id[%lu], event_id[%lu]",
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "deserialize packet from server[%s] failed, request_id[%lu], event_id[%lu], ret[%d]",
...
int ObMsSqlRpcEvent::handle_packet(ObPacket * packet, void * args)
{
  ...
  {
    YYSYS_LOG(WARN, "server stoped, cannot handle anything.");
    ret = OB_ERROR;
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "parse packet from server[%s] failed, request_id[%lu], event_id[%lu], rpc_event[%p]",
    ...
    if (NULL != obpacket)
    {
      YYSYS_LOG(DEBUG, "get packet from server[%s] success, event_id[%lu], time_used[%ld],"
    ...
    {
      YYSYS_LOG(WARN, "rpc event(%p) unknown status, destroy myself, event_id[%lu] "
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to push event to finish queue, destroy myself, req_type[%s], request_id[%lu], ret=%d",
...
int ObMsSqlRpcEvent::deserialize_packet(ObPacket & packet, ObNewScanner & result)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "deserialize the packet failed:ret[%d]", ret);
  }
  else
  {
    ...
    if (NULL == data_buff)
    {
      ...
      YYSYS_LOG(WARN, "check packet data buff failed:buff[%p]", data_buff);
    }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize result failed:pos[%ld], ret[%d]",
  ...
  if ((OB_SUCCESS == ret) && (OB_SUCCESS == code.result_code_))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "deserialize scanner failed:pos[%ld], ret[%d]",
          data_buff->get_position(), ret);
    }
  }
  return ret;
