  ...
  namespace mergeserver
  {
    ...
    inline ObSqlScanParam *ObMsSqlSubScanRequest::get_scan_param() const
    {
      ...
      if (NULL != scan_param_)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to set scan_param [err=%d]", err);
        }
        ...
        //else
        else if((scan_param_->get_table_id() != OB_ALL_SERVER_SESSION_TID) &&
                (scan_param_->get_table_id() != OB_ALL_SERVER_STAT_TID) &&
                (scan_param_->get_table_id() != OB_ALL_STATEMENT_TID))
        {
          YYSYS_LOG(WARN, "fail to set scan_param query version [err=%d]", err);
        }
        //add:e
/*
        if ((OB_SUCCESS == err) &&
        ...
        {
          YYSYS_LOG(WARN, "fail to set scan limit [limit_offset_=%ld][err=%d]", limit_offset_, err);
        }
*/
      }
      else
      {
        YYSYS_LOG(WARN, "null pointer error. [scan_param_=%p]", scan_param_);
      }
      return scan_param_;
    }
