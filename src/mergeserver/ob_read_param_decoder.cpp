...
namespace
{
    ...
    ObResultCode *rc)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get columns of table [table_name:%s,table_id:%lu,column_size:%d,column_beg:%p]",
    ...
    if (OB_SUCCESS == err)
    {
      ...
      for (int32_t i = 0; OB_SUCCESS == err && i < column_size; i ++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "unexpected error, fail to get column schema [table_id:%lu,column_id:%lu,idx:%d]",
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to add <column_name,idx> pair to hashmap [err:%d]", err);
          }
          else
          {
            err = OB_SUCCESS;
          ...
          {
            YYSYS_LOG(WARN,"fail to add column to decoded param [err:%d]", err);
          }
          else if (OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to add column to org param [err:%d]", err);
            }
          }
        }
      }
    ...
    ObResultCode *rc)
  {
      ...
      &&(org_param.get_column_name()[0] == ObGroupByParam::COUNT_ROWS_COLUMN_NAME))
    {
      ...
      if ((NULL != table_columns) && (0 < column_count))
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to decode column name to column id [table_id:%lu,column_id:%lu]",
      ...
      {
        err = OB_SCHEMA_ERROR;
        YYSYS_LOG(WARN,"fail to get column infos [table_name:%.*s, column_count:%d]",
    ...
    else if (OB_SUCCESS == err)
    {
      ...
      for (int64_t cell_idx = 0; OB_SUCCESS == err && cell_idx < org_param.get_column_name_size(); cell_idx ++)
      {
        ...
        if (NULL == column_info
          ...
          || column_idx_in_schema_mgr >= OB_MAX_COLUMN_NUMBER)
        {
          YYSYS_LOG(WARN,"fail to decode column name to column id [table_name:%.*s,column_name:%.*s]",
            org_param.get_table_name().length(), org_param.get_table_name().ptr(),
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to add <column_name,idx> pair to hashmap");
          }
          else
          {
            err = OB_SUCCESS;
    ...
    ObResultCode* rc)
  {
    ...
    {
      YYSYS_LOG(WARN, "invalid param, column_name_ptr=%p, column_name_len=%d",
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to add expire column <column_name, idx> pair "
      ...
      {
        YYSYS_LOG(WARN, "failed to add expire column into org_param, "
                        "column_name=%.*s, ret=%d",
          column_name.length(), column_name.ptr(), ret);
      }
      else if (OB_SUCCESS != (ret = decoded_param.add_column(column_schema->get_id(), false)))
      {
        YYSYS_LOG(WARN, "failed to add expire column into decoded_param, "
                        "column_id=%lu, ret=%d",
          ...
          if (ObExpression::COLUMN_IDX == type)
          {
            ...
            else
            {
              ...
              if (hash::HASH_EXIST != hash_ret)
              {
                ...
                {
                  YYSYS_LOG(WARN, "failed add extra column into scan param, "
    ...
    const int64_t buf_size)
  {
    ...
    struct tm* tm = NULL;
    time_t sys_date = 0;
    int64_t cur_time = yysys::CTimeUtil::getTime() / 1000 / 1000;
        ...
        && expire_condition[0] != '\0')
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to transfer infix expression to postfix "
    ...
    ObResultCode *rc)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "composite column: %ld", i);
      ...
      {
        YYSYS_LOG(WARN,"fail to set infix expression [err:%d, expr:%.*s, as_name:%.*s, comp_column_idx:%ld]",
      ...
      {
        YYSYS_LOG(WARN,"fail to add post expression to decoded scan param [err:%d]", err);
      }
      if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = schema_assis_in.add_column(as_name,cur_column_idx, ObMSSchemaDecoderAssis::SELECT_COLUMN))))
      {
        YYSYS_LOG(WARN,"fail to add composite column return info to cname to idx map [err:%d]", err);
      }
    }
    return err;
  }
    ...
    ObResultCode *rc)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < cond_count; i++)
    {
      ...
      if (NULL == cond)
      {
        YYSYS_LOG(ERROR,"unexpected error [org_filter.get_count():%ld,idx:%ld,cond:%p]", cond_count,
          i, cond);
        err = OB_ERR_UNEXPECTED;
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (hash::HASH_EXIST == (hash_err = schema_assis_in.get_select_cname_to_idx_map().get(cond->get_column_name(),column_idx_in_scan_param)))
        {
          ...
          {
            YYSYS_LOG(WARN,"filter column not in basic info [column_name:%.*s]", cond->get_column_name().length(),
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to add condition to decoded param [err:%d]", err);
            }
          }
        }
        else
        {
          YYSYS_LOG(WARN,"fail to find condition column name in cname to idx hash map [column_name:%.*s,condition_idx:%ld]",
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < groupby_columns.get_array_index(); i++, return_info_idx ++)
    {
        ...
        (hash_err = schema_assis_in.get_select_cname_to_idx_map().get(groupby_columns.at(i)->column_name_,column_idx_in_scan_param)))
      {
        ...
        if (column_idx_in_scan_param >= select_clause_table_row_width
          || column_idx_in_scan_param < 0)
        {
          YYSYS_LOG(WARN,"group by column not in basic info [column_name:%.*s]", groupby_columns.at(i)->column_name_.length(),
        ...
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to add group by column to decoded param [err:%d]", err);
          }
        }
      }
      else
      {
        YYSYS_LOG(WARN,"fail to find groupby column name in cname to idx hash map [column_name:%.*s,condition_idx:%ld]",
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < return_columns.get_array_index(); i++ , return_info_idx++)
    {
        ...
        (hash_err = schema_assis_in.get_select_cname_to_idx_map().get(return_columns.at(i)->column_name_,column_idx_in_scan_param)))
      {
        ...
        if (column_idx_in_scan_param >= select_clause_table_row_width
          || column_idx_in_scan_param < 0)
        {
          YYSYS_LOG(WARN,"return column not in basic info [column_name:%.*s]", return_columns.at(i)->column_name_.length(),
        ...
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to add return column to decoded param [err:%d]", err);
          }
        }
      }
      else
      {
        YYSYS_LOG(WARN,"fail to find return column name in cname to idx hash map [column_name:%.*s,condition_idx:%ld]",
    ...
    ObResultCode *rc)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < aggregate_columns.get_array_index(); i++, return_info_idx ++)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to find aggregate column name in cname to idx hash map [column_name:%.*s,condition_idx:%ld]",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (column_idx_in_scan_param >= select_clause_table_row_width
          || column_idx_in_scan_param < 0)
        {
          YYSYS_LOG(WARN,"aggregate column not in basic info [column_name:%.*s]",
            aggregate_columns.at(i)->get_org_column_name().length(),
            aggregate_columns.at(i)->get_org_column_name().ptr());
          err = OB_INVALID_ARGUMENT;
        }
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to add aggregate column to decoded param [err:%d]", err);
          }
        }
      }
    }
    ...
    ObResultCode *rc)
  {
      ...
      org_groupby_param.get_aggregate_columns().get_array_index() > 0)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to decode groupby columns [err:%d]", err);
      }
      if ((OB_SUCCESS == err)
        && (OB_SUCCESS !=
      ...
      for (int32_t idx = 0; (idx < groupby_columns.get_array_index()) && (OB_SUCCESS == err); idx++, cur_column_idx  ++)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to add column name to idx map [err:%d, column_idx:%d]", err, cur_column_idx);
        }
      }
      ...
      {
        YYSYS_LOG(WARN,"fail to decode groupby composite columns [err:%d]", err);
      }
      if ((OB_SUCCESS == err)
        && (OB_SUCCESS !=
        (err = ob_decode_scan_param_groupby_having_conditions(org_groupby_param,decoded_groupby_param,schema_assis_in, rc))))
      {
        YYSYS_LOG(WARN,"fail to decode having conditions [err:%d]", err);
      }
    }
    return err;
  }
    ...
    ObResultCode *rc)
  {
    ...
    for (int64_t i = 0; OB_SUCCESS == err && i < orderby_count; i++)
    {
      if (org_param.get_group_by_param().get_aggregate_row_width() > 0)
      {
        ...
        {
          YYSYS_LOG(WARN,"order by column was not in groupby columns [column_name:%.*s]",
        ...
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to add orderby column to decoded param [err:%d]", err);
          }
        }
      }
      else
      {
        ...
        if (hash::HASH_EXIST == (hash_err = schema_assis_in.get_select_cname_to_idx_map().get(orderby_columns[i], column_idx_in_scan_param)))
        {
          if (column_idx_in_scan_param >= decoded_param.get_column_id_size() + decoded_param.get_composite_columns().get_array_index()
            || column_idx_in_scan_param < 0)
          {
            YYSYS_LOG(WARN,"orderby column index outof range [column_name:%.*s, column_idx_in_scan_param:%ld, "
              "select_row_width:%ld]",
        ...
        {
          YYSYS_LOG(WARN,"fail to find orderby column name in cname to idx hash map [column_name:%.*s,condition_idx:%ld]",
      ...
      {
        YYSYS_LOG(WARN,"unrecogonized order [idx:%ld,order:%hhu]", i, order_desc[i]);
        err = OB_INVALID_ARGUMENT;
        if (NULL != rc)
        {
  ...
  bool check_scan_param_compatibility(const ObScanParam & scan_param_in , common::ObResultCode *rc)
  {
    ...
    if (ScanFlag::BACKWARD == scan_param_in.get_scan_direction())
    {
      ...
      {
        YYSYS_LOG(WARN,"backward scan do not support orderby operation");
        if (NULL != rc)
        {
      ...
      {
        YYSYS_LOG(WARN,"backward scan do not support grouby operation");
        result = false;
        if (NULL != rc)
        {
  ...
  common::ObResultCode *rc)
{
  ...
  for (int64_t i = 0; OB_SUCCESS == err && i < org_param.get_cell_size(); i++)
  {
    ...
    if (NULL == cur_cell)
    {
      YYSYS_LOG(WARN,"unexpected error [idx:%ld,org_param.get_cell_size():%ld,cur_cell:%p]",
    ...
    if ((OB_SUCCESS == err) && (cur_cell->column_name_ == ObGetParam::OB_GET_ALL_COLUMN_NAME))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get schema of table [table_name:%.*s]", cur_cell->table_name_.length(), cur_cell->table_name_.ptr());
        err = OB_SCHEMA_ERROR;
        if (NULL != rc)
        {
      ...
      for (int32_t j = 0; (j < column_size) && (OB_SUCCESS == err); j ++)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to add decoded cell to decoded param [err:%d]", err);
          if (NULL != rc)
          {
    ...
    else if (OB_SUCCESS == err)
    {
      ...
      if (NULL == column_schema)
      {
        YYSYS_LOG(WARN,"fail to decode column [table_name:%.*s,column_name:%.*s]",
          cur_cell->table_name_.length(), cur_cell->table_name_.ptr(),
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to add cell to decoded param [err:%d]", err);
          if (NULL != rc)
          {
            snprintf(rc->message_.ptr(),rc->message_.length(), "fail to add decoded cell to decoded param [err:%d]",  err);
          }
  ...
  common::ObResultCode *rc)
{
  ...
  if (NULL == table_schema)
  {
    YYSYS_LOG(WARN,"fail to decode table name to table id [table_name:%.*s]",
      org_param.get_table_name().length(), org_param.get_table_name().ptr());
    if (NULL != rc)
    {
      snprintf(rc->message_.ptr(), rc->message_.length(), "undecodable [table_name:%.*s]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "check scan param range failed:range[%p]", range);
    }
    else if (false == range->check())
    {
      err = OB_ERR_UNEXPECTED;
      YYSYS_LOG(WARN, "%s", "check param range rowkey and border flag failed");
      if (NULL != rc)
      {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to allocate memory for ObMSSchemaDecoderAssis");
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to initialize schema assiss [err:%d]", err);
      }
    }
  }
  if ((OB_SUCCESS == err)
  ...
  {
    YYSYS_LOG(WARN,"scan param compatibility check fail");
    err = OB_INVALID_ARGUMENT;
  }
  /// decode basic info
  if (OB_SUCCESS == err)
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to set limit info [err:%d, limit_offset:%ld, limit_count:%ld]", err, limit_offset, limit_count);
    }
    if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = decoded_param.set_topk_precision(sharding_minimum,precision))))
    {
      YYSYS_LOG(WARN,"fail to set topk precision [err:%d, sharding_minimum:%ld, precision:%f]", err, sharding_minimum,
        precision);
    }
  }
  /// decode scan flag
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"unrecogonized read_mode [org_param.get_scan_flag().read_mode_:%ld]",
    ...
    {
      YYSYS_LOG(WARN,"unrecogonized scan_direction [org_param.get_scan_direction():%d]",
  ...
  {
    YYSYS_LOG(WARN,"scan param must return some columns. return_column_count:%ld", decoded_param.get_returned_column_num());
    err = OB_INVALID_ARGUMENT;
  }
