...
void ObMergerSchemaTask::runTimerTask(void)
{
  ...
  {
    YYSYS_LOG(ERROR, "%s", "check schema timer task inner stat failed");
  }
  else if (remote_version_ > local_version_)
  {
    ...
    {
      YYSYS_LOG(WARN, "fetch new schema version failed:schema[%p], local[%ld], "
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "release schema failed:ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "fetch new schema succ:local[%ld], new[%ld]",
  ...
  {
    YYSYS_LOG(WARN, "check new version lt than local version:local[%ld], new[%ld]",
        local_version_, remote_version_);
  }
}
