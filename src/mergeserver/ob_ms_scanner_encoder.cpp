...
{
  YYSYS_LOG(DEBUG, "tableid:%lu, table_name:%.*s, rowkey:%s,"
    ...
    ObScanner & encoded_scanner)
{
  ...
  while (OB_SUCCESS == (ret = input.next_cell()))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get cell from org scanner failed:ret[%d]", ret);
      break;
    }
    // output(*cur_cell);
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "check encode cell failed:index[%ld]", cell_index);
      break;
    }
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add cell to encoded scanner failed:ret[%d]", ret);
      break;
    }
    ++cell_index;
  }
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check result scanner cell count not equal get param cell size:"
