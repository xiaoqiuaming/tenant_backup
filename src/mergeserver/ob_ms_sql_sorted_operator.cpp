...
{
  int err = OB_SUCCESS;
  //YYSYS_LOG(DEBUG, "ObMsSqlSortedOperator::set_param_only");
  scan_param_ = &scan_param;
  scan_range_ = *scan_param_->get_range();
  return err;
}
...
void oceanbase::mergeserver::ObMsSqlSortedOperator::sort(bool &is_finish, ObMsRequestResult * last_sharding_res)
{
  ...
  if (seamless_result_count_ <= 0)
  {
    ...
    //    }
    YYSYS_LOG(DEBUG,"start_scan_range=%s",to_cstring(first_scan_range_));
    YYSYS_LOG(DEBUG,"current_scan_range=%s",to_cstring(scan_range_));
  ...
  if (seamless_result_count_ > 0)
  {
         ...
         seamless_result_idx++)
    {
      ...
      if ( cmp_ret == 0 )
      {
        ...
        YYSYS_LOG(DEBUG, "1.prev last_row_key=%s, cur star_key_=%s, seamless=%ld",
      ...
      else if ( cmp_ret < 0 )
      {
        ...
        YYSYS_LOG(DEBUG,"last row key count =%ld,first count=%ld", last_row_key.get_obj_cnt(),last_row_key.get_obj_cnt());
        YYSYS_LOG(DEBUG,
                  "last row key obj =%s,first  obj2=%s",
                  to_cstring(*(last_row_key.get_obj_ptr() + last_row_key.get_obj_cnt() - 1)),
                  to_cstring(*(fist_row_key.get_obj_ptr() + last_row_key.get_obj_cnt() - 1)));
        //              const common::ObNewRange* publish =used_rowkey_.at(0);
        YYSYS_LOG(DEBUG,"count =%d,last pulished start  rowkey =%s",(int32_t)this->used_rowkey_.size(), to_cstring(used_rowkey_.at(0).start_key_));
        // change range?
        if (used_rowkey_.size() >1 && 0 == used_rowkey_.at(1).start_key_.compare(fist_row_key) && 0 == used_rowkey_.at(0).end_key_.compare(last_row_key))
        {
          ...
          YYSYS_LOG(DEBUG, "2.prev last_row_key=%s, cur star_key_=%s, seamless=%ld,used_rowkey_ count=%d",
                    to_cstring(sharding_result_arr_[seamless_result_idx - 1].sharding_query_range_->end_key_),
                    to_cstring(sharding_result_arr_[seamless_result_idx].sharding_query_range_->start_key_),
        ...
        else if (1 == used_rowkey_.size() && 0 == used_rowkey_.at(0).start_key_.compare(fist_row_key) )
        {
          seamless_result_count_ = seamless_result_idx + 1;
          used_rowkey_.erase(used_rowkey_.begin());
          YYSYS_LOG(DEBUG, "3.prev last_row_key=%s, cur star_key_=%s, seamless=%ld,used_rowkey_ count=%d",
                    to_cstring(sharding_result_arr_[seamless_result_idx - 1].sharding_query_range_->end_key_),
                    to_cstring(sharding_result_arr_[seamless_result_idx].sharding_query_range_->start_key_),
                    seamless_result_count_, (int32_t)used_rowkey_.size());
        }
      ...
      {
        YYSYS_LOG(ERROR, "start_key must be greater than or equal to end_key, prev last_row_key=%s, cur star_key_=%s, seamless=%ld",
                  to_cstring(sharding_result_arr_[seamless_result_idx - 1].sharding_query_range_->end_key_),
                  to_cstring(sharding_result_arr_[seamless_result_idx].sharding_query_range_->start_key_),
                  seamless_result_count_
                  );
  ...
  if (seamless_result_count_ > 0) // implicates that startkey matched already
  {
    ...
    YYSYS_LOG(DEBUG, "last seamless=%s", to_cstring(sharding_result_arr_[seamless_result_count_-1].sharding_query_range_->end_key_));
        ...
        sharding_result_arr_[seamless_result_count_-1].sharding_query_range_->end_key_.is_max_row())
    {
       ...
       */
      YYSYS_LOG(DEBUG, "biubiu. seamless_result_count_=%ld, start_key=%s, end_key=%s",
                seamless_result_count_,
                                               ...
                                               bool &is_finish)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to add sharding result, err=%d", err);
    }
  }
  ...
  YYSYS_LOG(DEBUG, "add sharding result. sharding_result_count_=%ld, is_finish=%d, err=%d", sharding_result_arr_.count(), is_finish, err);

  if (1)
  {
    static __thread int64_t total_row_num = 0;
    total_row_num += fullfilled_item_num;
    YYSYS_LOG(DEBUG, "query_range=%s,fullfilled_item_num=%ld,total=%ld, is_finish=%d",
...
int oceanbase::mergeserver::ObMsSqlSortedOperator::get_next_row(oceanbase::common::ObRow &row)
{
  ...
  while (OB_SUCCESS == err)
  {
    ...
    else if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next cell from ObNewScanner [idx:%ld,err:%d]", cur_sharding_result_idx_, err);
      }
    }
  }
  return err;
