    ...
    const ObSchemaManagerV2 & schema, ObCellInfo & decoded_cell)
{
  ...
  if (false == is_ext)
  {
    ...
    {
      ret = OB_SCHEMA_ERROR;
      YYSYS_LOG(WARN, "check column failed:table[%.*s], column[%.*s], ret[%d]",
  ...
  else
  {
    ...
    {
      ret = OB_SCHEMA_ERROR;
      YYSYS_LOG(WARN, "check table failed:table[%.*s], ret[%d]", org_cell.table_name_.length(),
    ...
    ObCellInfo & decoded_cell)
{
  ...
  if (org_cell.table_name_.length() <= 0) // || (org_cell.row_key_.length() <= 0))
  {
    ...
    YYSYS_LOG(WARN, "check cell table name or column name failed:table[%.*s], column[%.*s]",
        org_cell.table_name_.length(), org_cell.table_name_.ptr(),
  ...
  else if (false == org_cond.is_exist_type())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "decode cond cell failed:ret[%d]", ret);
    }
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "decode exist cond cell failed:ret[%d]", ret);
    }
  }
    ...
    ObCellInfo & decoded_cell)
{
  ...
  else if (ObActionFlag::OP_DEL_ROW != org_cell.value_.get_ext())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "decode org cell failed:ret[%d]", ret);
    }
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "decode ext cell failed:ret[%d]", ret);
    }
    else
    {
