...
// by the yynet thread so lightweight
int ObMergerRequest::signal(ObMergerRpcEvent & event)
{
  ...
  {
    YYSYS_LOG(INFO , "the event we do not expect from server, "
  ...
  if (ret != OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "push to finish queue failed:client[%lu], event[%lu], "
  ...
  {
    YYSYS_LOG(DEBUG, "push to finish queue succ:client[%lu], event[%lu], request[%lu]",
...
int ObMergerRequest::process_rpc_event(const int64_t timeout, ObMergerRpcEvent * event, bool & finish)
{
  ...
  else if (true == is_valid)
  {
    ...
    {
      YYSYS_LOG(WARN, "check result client id not equal with current request id:client[%lu], request[%lu], event[%lu]",
          event->get_client_id(), request_id_, event->get_event_id());
    }
  }
  else
...
int ObMergerRequest::remove_wait_queue(ObMergerRpcEvent * event, bool & is_valid)
{
  ...
  {
    YYSYS_LOG(DEBUG, "remove from waiting queue:client[%lu], event[%lu], result[%d], request[%lu]",
        event->get_client_id(), event->get_event_id(), event->get_result_code(), request_id_);
    yysys::CThreadGuard lock(&lock_);
    ret = waiting_queue_.erase(event);
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "find event not in waiting queue:client[%lu], event[%lu], result[%d], "
          "request[%lu], ret[%d]", event->get_client_id(), event->get_event_id(),
          event->get_result_code(), request_id_, ret);
    }
    else if (event->get_client_id() != request_id_)
    {
      YYSYS_LOG(WARN, "check the event client id not equal with request:"
...
int ObMergerRequest::wait(const int64_t timeout)
{
  ...
  YYSYS_LOG(DEBUG, "check all the event finish:request[%lu], remain finish queue size[%lu]",
      request_id_, finish_queue_.size());
  remove_invalid_event_in_finish_queue(remainder);
  if (remainder <= 0)
  {
    ret = OB_RESPONSE_TIME_OUT;
    YYSYS_LOG(WARN, "check remainder < zero:request[%lu], deadline[%ld], timeout[%ld], remainder[%ld]",
...
int ObMergerRequest::release_succ_event(uint64_t & event_count)
{
  ...
  YYSYS_LOG(DEBUG, "relase all the succ event:request[%lu], count[%lu]", request_id_, event_count);
