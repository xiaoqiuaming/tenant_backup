...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    int ObQueryCache::init(const int64_t max_mem_size)
    {
      ...
      {
        inited_ = true;
        YYSYS_LOG_US(DEBUG, "init query cache succ, cache_mem_size=%ld,",
                     max_mem_size);
      }
    ...
    int ObQueryCache::get(const ObString& key, ObDataBuffer& out_buffer)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid query cache get param, key_ptr=%p, key_len=%d", 
                  key.ptr(), key.length());
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (cache_val.expire_time_ < yysys::CTimeUtil::getTime())
          {
            if (OB_SUCCESS != kv_cache_.erase(key))
            {
              YYSYS_LOG(WARN, "failed to delete item from query cache");
            }
          ...
          else if (cache_val.size_ > 0)
          {
            ...
            {
              YYSYS_LOG(WARN, "out_buffer hasn't enough space to store the result data, "
    ...
    int ObQueryCache::put(const ObString& key, const ObQueryCacheValue& value)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid query cache put param, key_ptr=%p, key_len=%d", 
                  key.ptr(), key.length());
        ret = OB_INVALID_ARGUMENT;
      }
      else
