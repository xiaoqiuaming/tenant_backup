...
int  mergeserver::ObMergerReverseOperator::set_param(const ObScanParam & scan_param, ObCellArray &cells)
{
  ...
  {
    YYSYS_LOG(WARN,"set ObMergerSortedOperator fail [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
  ...
  bool &is_finish)
{
  ...
  {
    YYSYS_LOG(WARN,"operator not initalized yet [reversed_cells_:%p]", reversed_cells_);
  }
  else
  {
...
int mergeserver::ObMergerReverseOperator::seal()
{
  ...
  {
    YYSYS_LOG(WARN,"operator was not initialized yet [reversed_cells_:%p]", reversed_cells_);
    err = OB_INVALID_ARGUMENT;
  }

  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = sorted_operator_.seal())))
  {
    YYSYS_LOG(WARN,"fail to seal ObMergerSortedOperator [err:%d]", err);
  }
  /// jump limit offset
  ...
  if (limit_offset > 0)
  {
    ...
    for (int64_t i = 0; (i < jumped_cell_count) && (OB_SUCCESS == err); i ++)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get next cell from ObMergerSortedOperator [err:%d]", err);
      }
    }
  }
  ...
  while ((OB_SUCCESS == err) && (cell_count_got < cell_count_need_got))
  {
    if (OB_SUCCESS == (err = sorted_operator_.next_cell()))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get cell from sorted operator [err:%d]", err);
      }
      else
      {
        // TODO soon
      ...
      {
        YYSYS_LOG(WARN,"fail to append cell to reversed cell [err:%d]", err);
      }
      if (OB_SUCCESS == err)
      {
        cell_count_got ++;
    ...
    {
      YYSYS_LOG(WARN,"fail to get next_cell from SortedOperator [err:%d]", err);
    }
  }
  if (OB_ITER_END == err)
  {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to reverse result [err:%d]", err);
    }
  }
  return err;
}
...
int mergeserver::ObMergerReverseOperator::next_cell()
{
  ...
  {
    YYSYS_LOG(WARN,"fail to get next cell from cell array [err:%d]", err);
  }
  return err;
}
...
int mergeserver::ObMergerReverseOperator::get_cell(ObInnerCellInfo ** cell, bool *is_row_changed)
{
  ...
  if (NULL == cell)
  {
    YYSYS_LOG(WARN,"invalid argument [cell:%p]", cell);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && (NULL == reversed_cells_))
  {
    YYSYS_LOG(WARN,"operator not initialized yet [reversed_cells_:%p]", reversed_cells_);
    err = OB_INVALID_ARGUMENT;
  }
