...
int ObMsSqlRequest::close()
{
  ...
  {
    YYSYS_LOG(WARN, "fail to invalidate request id. ret=%d", ret);
  }
  if (OB_SUCCESS != (ret = remove_invalid_event_in_finish_queue(timeout_)))
  {
    YYSYS_LOG(WARN, "fail to remove invalid event in finish queue. ret=%d", ret);
  }
  return ret;
}
...
int ObMsSqlRequest::init(const uint64_t count, const uint32_t mod_id,  ObSessionManager * session_mgr)
{
  ...
  if (!finish_queue_inited_)
  {
    ...
    {
      finish_queue_inited_ = false;
      YYSYS_LOG(WARN,"fail to init finish_queue_ [err:%d]", err);
    }
    else
    {
...
int ObMsSqlRequest::reset()
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "release all processed event failed:request[%lu], ret[%d]",
              request_id_, ret);
  }
  buffer_pool_.clear();
  //finish_queue_.reset();
  // bugfix #224057, if queue is not clear, will cause deadlock
  yysys::CThreadGuard lock(&lock_);
...
int ObMsSqlRequest::create(ObMsSqlRpcEvent ** event)
{
  ...
  if (NULL == event)
  {
    ...
    YYSYS_LOG(WARN, "check input failed:request[%lu], event[%p]", request_id_, event);
  }
  else
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "create rpc event failed:request[%lu], event[%p], ret[%d]",
                request_id_, event, ret);
    }
    else
    {
      (*event)->set_timestamp(yysys::CTimeUtil::getTime());
      yysys::CThreadGuard lock(&lock_);
      ret = waiting_queue_.push_back(*event);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "push new event to waiting queue failed:request[%lu], event[%lu], "
      ...
      {
        YYSYS_LOG(DEBUG, "push new event succ:request[%lu], event[%lu], client[%lu]",
...
int ObMsSqlRequest::destroy(ObMsSqlRpcEvent * event)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "remove from the waiting queue failed:clinet[%lu], event[%lu], "
...
int ObMsSqlRequest::signal(ObMsSqlRpcEvent & event)
{
  ...
  {
    YYSYS_LOG(INFO , "rpc timeout event[%p] event_id[%ld], request_id[%ld], "
  ...
  if (ret != OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "push rpc_event[%p] to finish queue failed:request_id[%lu], event_id[%lu], "
             ...
             && event.get_session_id() >  ObCommonSqlRpcEvent::INVALID_SESSION_ID)
    {
      ...
      YYSYS_LOG(INFO,"end unexpect session [client:%ld, session_id:%lu]",
                event.get_client_id(), event.get_session_id());
    }
    else
    {
      YYSYS_LOG(WARN, "push to finish queue failed. destroy it:event_request_id[%lu], event_id[%lu], "
  ...
  {
    YYSYS_LOG(DEBUG, "push to finish queue succ:request_id[%lu], event_id[%lu], current_request_id[%lu]",
...
int ObMsSqlRequest::process_rpc_event(const int64_t timeout, ObMsSqlRpcEvent * event, bool & finish)
{
  ...
  if (ret != OB_SUCCESS)
  {
    ...
    destroy_rpc_event(event);
    YYSYS_LOG(ERROR, "remove the event failed:request[%lu], ret[%d]", request_id_, ret);
  }
  else if (true == is_valid)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "push the event to result queue failed:client[%lu], request[%lu], "
    ...
    {
      YYSYS_LOG(DEBUG, "push the event to result succ:client[%lu], request[%lu], event[%lu]",
                event->get_client_id(), request_id_, event->get_event_id());
      ret = process_result(timeout, event, finish);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "process the result failed:client[%lu], request[%lu], event[%lu], ret[%d]",
  ...
  else
  {
    ...
    YYSYS_LOG(WARN, "the event is not valid:client[%lu], event[%lu], request[%ld], valid[%d]",
...
int ObMsSqlRequest::remove_wait_queue(ObMsSqlRpcEvent * event, bool & is_valid)
{
  ...
  {
    ret = OB_INPUT_PARAM_ERROR;
    YYSYS_LOG(WARN, "check rpc event failed:request[%lu], event[%p]", request_id_, event);
  }
  else
  {
    yysys::CThreadGuard lock(&lock_);
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "event[%p] not found in waiting queue:request_id[%lu], event_id[%lu], rcode[%d], "
    ...
    {
      YYSYS_LOG(WARN, "the event request id not equal with current_request_id, destroy myself"
...
int ObMsSqlRequest::wait(const int64_t timeout)
{
  ...
  while (remainder > 0)
  {
    ...
    if (OB_SUCCESS == (ret = finish_queue_.pop((void *&)event, &to)))
    {
      if (NULL == event)
      {
        YYSYS_LOG(ERROR, "pop a NULL value from finish_queue_");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (request_id_ != event->get_client_id())
      {
        YYSYS_LOG(WARN, "expired rpc event[%p] timeout event_id[%ld], request_id[%ld], "
      ...
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "process rpc event failed:request[%lu], event[%p], ret[%d]",
                    request_id_, event, ret);
          break;
        }
        else if (true == finish)
    ...
    else
    {
      ...
      YYSYS_LOG(WARN, "pop result from finish queue failed:request[%lu], event[%p], ret[%d]",
                request_id_, event, ret);
    }
    // update the remainder time
    cur_timestamp = yysys::CTimeUtil::getTime();
    remainder = last_timestamp - cur_timestamp;
    YYSYS_LOG(DEBUG, "check remainder for next wait:request[%lu], cur[%ld], last[%ld], remainder[%ld]",
    ...
    if ((NULL != session_mgr_) && !session_mgr_->session_alive(session_id_))
    {
      ...
      YYSYS_LOG(INFO, "session was killed [session_id:%u]", session_id_);
      break;
    }
  }
  ...
  if (remainder <= 0)
  {
    ...
    YYSYS_LOG(WARN, "check remainder less than zero:request[%lu], remainder[%ld], timeout[%ld]",
...
int ObMsSqlRequest::release_succ_event(uint64_t & event_count)
{
  ...
  YYSYS_LOG(DEBUG, "relase all the succ event:request[%lu], count[%lu], finish_size[%d], result_size[%lu]",
  ...
  for (uint64_t i = 0; i < event_count; ++i)
  {
    ...
    if (NULL == rpc)
    {
      ...
      YYSYS_LOG(ERROR, "check finished result failed:request[%lu], index[%lu], event[%p]",
                request_id_, i, rpc);
    }
    else
    {
...
int ObMsSqlRequest::create_rpc_event(ObMsSqlRpcEvent ** event)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check rpc event allocate failed:rpc[%p]", rpc);
  }
  else
  {
    new (rpc) ObMsSqlRpcEvent;
    YYSYS_LOG(DEBUG, "create rpc event succ:request[%lu], event[%lu], ptr[%p]",
              request_id_, rpc->get_event_id(), rpc);
    ret = rpc->init(request_id_, this);
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "init rpc event failed:request[%lu], event[%lu], ptr[%p], ret[%d]",
...
void ObMsSqlRequest::destroy_rpc_event(ObMsSqlRpcEvent * rpc)
{
  ...
  {
    //rpc->lock();
    YYSYS_LOG(DEBUG, "destroy rpc event succ:client[%lu], request[%lu], event[%lu], "
  ...
  {
    YYSYS_LOG(WARN, "check destroy rpc event failed:request[%lu], event[%p]",
              request_id_, rpc);
  }
}
...
int ObMsSqlRequest::update_location_cache(const ObServer &svr, const int32_t result_code, const ObSqlScanParam & scan_param)
{
  ...
  if (OB_CS_TABLET_NOT_EXIST == result_code)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to invalidate cache item [err:%d], err", err);
    }
    else if (OB_SUCCESS != (err = cache_proxy_->renew_location_item(scan_param, loc_list, true)))
    {
      YYSYS_LOG(WARN, "fail to force renew location cache item. err=%d", err);
    }
  }
  ...
  else if (OB_SUCCESS != result_code)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to update cache item [err:%d], err", err);
    }
  }
  return err;
}
...
int ObMsSqlRequest::remove_invalid_event_in_finish_queue(const int64_t timeout)
{
  ...
  while (size > 0)
  {
    ...
    if (OB_SUCCESS == (ret = finish_queue_.pop((void *&)event, &to)))
    {
      ...
      {
        terminate_remote_session(event->get_server(), event->get_session_id());
        YYSYS_LOG(INFO,"end finished but not process session [client:%ld, session_id:%lu, event_id:%ld]",
...
int ObMsSqlRequest::wait_single_event(int64_t &timeout)
{
  ...
  YYSYS_LOG(DEBUG, "wait_single_event:timeout:%ld", timeout);
  ...
  do
  {
    ...
    else
    {
      ...
      YYSYS_LOG(WARN, "pop result from finish queue failed:request[%lu], event[%p], ret[%d], timeout[%ld]",
  ...
  YYSYS_LOG(DEBUG, "timeout:%ld", timeout);

  // wait finished, switch to next request and clean current queue_
  //request_id_ = atomic_inc(reinterpret_cast<volatile uint64_t*>(&id_allocator_));
  YYSYS_LOG(DEBUG, "check all the event finish:request[%lu], remain finish queue size[%d]",
