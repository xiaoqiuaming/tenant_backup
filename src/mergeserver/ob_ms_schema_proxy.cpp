...
int ObMergerSchemaProxy::get_schema(const ObString & table_name, const int64_t timestamp, const ObSchemaManagerV2 ** manager)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "check get schema failed:schema[%p], version[%ld], ret[%d]",
...
int ObMergerSchemaProxy::fetch_user_schema(const int64_t version, const ObSchemaManagerV2 ** manager)
{
  ...
  if (!check_inner_stat() || (NULL == manager))
  {
    YYSYS_LOG(WARN, "check inner stat or shema manager param failed:manager[%p]", manager);
    ret = OB_ERROR;
  }
  else if (yysys::CTimeUtil::getTime() - fetch_schema_timestamp_ < LEAST_FETCH_SCHEMA_INTERVAL)
  {
    YYSYS_LOG(WARN, "check last fetch schema timestamp is too nearby:version[%ld]", version);
    ret = OB_OP_NOT_ALLOW;
  }
  else
  {
    ...
    else if (schema_manager_->get_latest_version() == -1)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "fetch newest schema from root server failed:ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "fetch newest schema from root server success");
...
//      }
      ...
      else if (NULL != (schema_tmp = schema_manager_->get_user_schema(0)))
      {
        ...
        //YYSYS_LOG(ERROR,"schema_service_ %p",schema_service_); // uncertanty for debug
        ret = schema_service_->fetch_schema_mutator(schema_manager_->get_latest_version(), version, schema_mutator);
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fetch schema mutator from sys tables failed:ret[%d]", ret);
        }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          fetch_schema_timestamp_ = yysys::CTimeUtil::getTime();
          YYSYS_LOG(INFO, "fetch mutator and add new schema succ:version[%ld]", schema->get_version());
        }
      }
...
int ObMergerSchemaProxy::release_schema(const ObSchemaManagerV2 * manager)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "release scheam failed:schema[%p], timestamp[%ld]",
          manager, manager->get_version());
    }
  }
  return ret;
