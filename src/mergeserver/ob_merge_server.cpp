...
namespace oceanbase
{
  namespace mergeserver
  {
    ...
    {
      if(OB_SUCCESS != udi_index_.create(OB_MAX_TABLE_NUMBER))
      {
        YYSYS_LOG(WARN,"failed to create udi_counter!");
      }
      udi_counter_helper_.init(OB_MAX_TABLE_NUMBER,udi_counter_);
    }
    ...
    int ObMergeServer::reload_config()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "enlarge frozen_data_cache_size to: %ld",
        ...
        YYSYS_LOG(INFO, "enlarge bloom_filter_cache_size to: %ld",
        ...
        if (ms_config_.clean_statistic_info_cache)
        {
          statistic_info_cache_.clear();
        }

        YYSYS_LOG(INFO,"enlarge statistic_info_cache_size to: %ld",
        ...
        YYSYS_LOG(INFO,"set statistic_info_cache_timeout: %ld",
                  static_cast<int64_t>(ms_config_.statistic_info_cache_timeout));
        statistic_info_cache_.set_cache_timeout(ms_config_.statistic_info_cache_timeout);
    ...
    int ObMergeServer::initialize()
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        ret = sequence_frozen_data_cache_.init(256*1024*1024);
      }
      if (ret == OB_SUCCESS)
      {
        //add e
      ...
      if(ret == OB_SUCCESS)
      {
        ret = udi_index_.clear();
      }
      if(ret == OB_SUCCESS)
      {
        ret = statistic_info_cache_.init(service_.get_sql_proxy(),ms_config_.statistic_info_cache_size,ms_config_.statistic_info_cache_timeout);
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        ObBaseServer::add_disconn_pkt_queue(get_default_task_queue_thread()); //[701]
      }
    ...
    {
      YYSYS_LOG(WARN, "stop task timer");
      task_timer_.destroy();
      YYSYS_LOG(WARN, "stop bloomfilter queue thread");
      bloom_filter_queue_thread_.stop();
      bloom_filter_queue_thread_.wait();
      YYSYS_LOG(WARN, "stop single server");
      ObSingleServer::destroy();
      YYSYS_LOG(WARN, "stop mergeserver service");
      service_.destroy();
    }
    ...
    void ObMergeServer::handle_no_response_request(ObPacket * base_packet)
    {
      if (NULL == base_packet)
      {
        YYSYS_LOG(WARN, "packet is illegal, discard.");
      }
      else
      {
    ...
    int ObMergeServer::do_request(common::ObPacket* base_packet)
    {
      ...
      else
      {
        ...
        ret = service_.do_request(ob_packet->get_receive_ts(), packet_code, version,
                                  ...
                                  ob_packet->get_source_timeout() - (yysys::CTimeUtil::getTime() - ob_packet->get_receive_ts()));
      }
      return ret;
    }
