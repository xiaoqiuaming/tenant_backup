...
int ObMergerGroupByOperator::set_param(const int64_t max_memory_size, const ObScanParam & param)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to init ObGroupByOperator [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
    scan_param_ = &param;
  ...
  const ObNewRange & query_range, const int64_t limit_offset, bool &is_finish)
{
  ...
  if (NULL == scan_param_)
  {
    YYSYS_LOG(WARN,"operator was not initialized yet [scan_param_:%p]", scan_param_);
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [expect_row_width:%ld,res_cell_count:%ld,res_row_count:%ld]",
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to call next cell on sharding_res [err:%d]", err);
    }
  }
  ...
  while ((OB_SUCCESS == err) && (!sharding_res_it_end))
  {
      ...
      cur_row_width++, got_cell_count ++)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to got cell from ObScanner [err:%d,got_cell_count:%ld]", err, got_cell_count);
      }
      if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = row_cells_.append(*cur_cell, cell_out))))
      {
        YYSYS_LOG(WARN,"fail to append cell to temp row cell array [err:%d]", err);
      }
      if (OB_SUCCESS == err)
      {
        if (OB_SUCCESS == (err = sharding_res.next_cell()))
        ...
        else if (OB_ITER_END == err)
        {
          ...
          {
            YYSYS_LOG(ERROR,"sharding result cell number error [cur_row_width:%ld,expected_row_width:%ld]",
        ...
        {
          YYSYS_LOG(WARN,"fail to get next_cell from sharding result [err:%d]", err);
        }
      }
    }
    if ((OB_ITER_END == err) && (cur_row_width != expect_row_width))
    {
      YYSYS_LOG(ERROR,"unexpected error, current row not end while ObScanner is end [cur_row_width:%ld,expect_row_width:%ld]",
        cur_row_width, expect_row_width);
      err = OB_ERR_UNEXPECTED;
    }
    if ((cur_row_width == expect_row_width) && ((OB_SUCCESS == err) || (OB_ITER_END == err)))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to add row to ObGroupByOperator [err:%d]", err);
      }
    }
  }
  if (OB_ITER_END == err)
  ...
  {
    YYSYS_LOG(ERROR, "unexpected error, number of cell got is less than ObScanner::get_cell_num [got_cell_count:%ld,"
    ...
    const ObArrayHelper<ObCompositeColumn>& composite_columns, const int64_t row_width)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to compose result [err:%d]", err);
  }
  return err;
}
  ...
  const int64_t row_width, const ObGroupByParam &param)
{
  ...
  {
    YYSYS_LOG(WARN,"unexpected row_width:%ld", row_width);
    err = OB_ERR_UNEXPECTED;
  }
  if ((OB_SUCCESS == err) && (result.get_cell_size()%row_width != 0))
  {
    YYSYS_LOG(WARN,"argument error [result.get_cell_size():%ld,row_width:%ld]",
  ...
  for (int64_t cell_idx = 0; (OB_SUCCESS == err) && (cell_idx < result.get_cell_size()); cell_idx += row_width)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to filter row [row_idx:%ld, err:%d]", cell_idx/row_width, err);
    }
    if ((OB_SUCCESS == err)&&(!check_pass))
    {
      if (OB_SUCCESS != (err = g_key.init(result,param,cell_idx, cell_idx + row_width - 1,ObGroupKey::AGG_KEY)))
      {
        YYSYS_LOG(WARN,"fail to init ObGroupKey [cell_idx:%ld,err:%d]", cell_idx, err);
      }
      if (OB_SUCCESS == err)
      {
        int hash_err = 0;
        ...
        {
          YYSYS_LOG(ERROR, "unexpected error, row is in ObGroupOperator, but remove fail [row_idx:%ld]", cell_idx/row_width);
          err = OB_ERR_UNEXPECTED;
        }
        else
        {
          YYSYS_LOG(WARN,"remove key from hash table err [hash_err:%d]", hash_err);
          err = OB_ERROR;
        }
      }
    }
...
int ObMergerGroupByOperator::seal()
{
  ...
  if (NULL == scan_param_)
  {
    YYSYS_LOG(WARN,"operator was not initialized yet");
    err = OB_INVALID_ARGUMENT;
  }
  else
  {
    ...
    if ((OB_SUCCESS == err) && need_compose)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to compose composite columns [err:%d]", err);
      }
    }
    ...
    if ((OB_SUCCESS == err) && (need_filter_having_condition))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to filter having conditions [err:%d]", err);
      }
    }
    ...
    {
      YYSYS_LOG(WARN,"orderby_column_count too large [orderby_column_count:%ld]", orderby_column_count);
      err = OB_INVALID_ARGUMENT;
    }
    ...
    if ((OB_SUCCESS == err) && need_orderby)
    {
      if (agg_row_width > 0)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to orderby grouped result [err:%d]", err);
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to orderby select result [err:%d]", err);
        }
      }
    }
    if (OB_SUCCESS == err)
...
int ObMergerGroupByOperator::next_cell()
{
  ...
  if (!sealed_)
  {
    YYSYS_LOG(WARN,"operator was not yet sealed, seal it first");
    err = OB_INVALID_ARGUMENT;
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get next_cell from ObGroupByOperator [err:%d]", err);
    }
  }
  bool check_pass = false;
  int64_t agg_row_width = scan_param_->get_group_by_param().get_aggregate_row_width();
    ...
    && (cur_cell_offset%agg_row_width == 0))
  {
    ...
    {
      YYSYS_LOG(ERROR, "unexpected error [cur_cell_offset:%ld,agg_row_width:%ld,ObGroupByOperator::get_cell_size():%ld]",
    ...
    while ((OB_SUCCESS == err)  && !check_pass)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to init ObGroupKey [cell_idx:%ld,err:%d]", cur_cell_offset, err);
      }
      else
      {
      ...
      if ((OB_SUCCESS == err) && !check_pass)
      {
        ...
        {
          YYSYS_LOG(ERROR, "unexpected error, jumped cell count not enough [jumped_cell_count:%ld,agg_row_width:%ld]",
...
int ObMergerGroupByOperator::get_cell(ObInnerCellInfo** cell, bool* is_row_changed)
{
  ...
  {
    YYSYS_LOG(WARN,"agument error [cell:%p]", cell);
    err = OB_INVALID_ARGUMENT;
  }

  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = operator_.get_cell(cell,is_row_changed))))
  {
    YYSYS_LOG(WARN,"fail to get cell from ObGroupByIterator [err:%d,ObGroupByOperator.get_cell_size():%ld]",
