  ...
  namespace mergeserver
  {
      ...
      const int64_t timeout_us, const int64_t max_parellel_count)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to scan cs. [err=%d]", err);
        }
      }
      return err;
    }
      ...
      uint64_t * triggered_rpc_event_id)
    {
      ...
      /* @note range and offset set in get_scan_param() function */
        ...
        *rpc_event))))
      {
        YYSYS_LOG(WARN, "fail to send scan request. [err=%d]", err);
        ...
        {
          YYSYS_LOG(WARN, "fail to destroy rpc_event. [err=%d]", tmp_err);
        }
      }
        ...
        ObNewRange &query_range, const int64_t timeout_us,  const int64_t limit_offset)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "fail to allocate sub scan request, err = %d", err);
        }
      }
      if ((OB_SUCCESS == err)
      ...
      {
        YYSYS_LOG(WARN,"fail to get session next [err:%d,session_id:%ld,req_id:%lu]", err,
      ...
      YYSYS_LOG(INFO, "[session next] timeout_us:%ld, limit_offset:%ld, limit_count:%ld, "
      ...
      const int64_t limit_offset)
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to send rpc event of ObMergerSubScanRequest [err:%d]", err);
        }
      ...
      bool &belong_to_this, bool &is_first,  int64_t &idx)
    {
      ...
      for (idx = 0; idx < total_sub_request_count_; idx++)
      {
        ...
        {
          YYSYS_LOG(INFO, "not the first result of sub_scan[%ld]. ignore.", idx);
          break;
        }
    ...
    int ObMergerScanRequest::check_if_need_more_req(const int64_t sub_req_idx,  const int64_t timeout_us, ObMergerRpcEvent &prev_rpc_event)
    {
      ...
      {
        YYSYS_LOG(WARN, "scanner is NULL. no result.");
        err = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN,"fail to get fullfilled info from scanner [err:%d]", err);
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get next range, err=%d", err);
        }
        else
        {
          if ((OB_SUCCESS != (err= scan_param_->set_range(next_scan_range))))
          {
            YYSYS_LOG(WARN, "fail to set next scan range. [err:%d]", err);
          }
          ...
          else if (prev_rpc_event.is_session_end())
          {
            ...
            YYSYS_LOG(INFO, "prev_rpc_event return OB_SESSION_END, end session, no need more request.");
            err = OB_ITER_END;
          }
                  ...
                  timeout_us,  next_limit_offset)))
          {
            ...
            YYSYS_LOG(WARN, "fail to get session next [err:%d]", err);
          }
        }
    ...
    int ObMergerScanRequest::process_result(const int64_t timeout_us, ObMergerRpcEvent *rpc_event, bool & finish)
    {
      ...
      if (NULL != rpc_event)
      {
        ...
        YYSYS_LOG(DEBUG, "rpc_event finised [us_used:%ld,timeout_us:%ld,request_event_id:%lu,"
      ...
      {
        YYSYS_LOG(WARN,"fail to find SubRequest for rpc event [rpc_event:%p,rpc_event_id:%ld,"
      ...
      if ((OB_SUCCESS == err) && (OB_SUCCESS == rpc_event->get_result_code()))
      {
        if ((OB_SUCCESS == err) && (false == belong_to_this))
        {
          YYSYS_LOG(WARN, "Unexpected. rpc event not found! The framework should deal with this case. not here");
            ...
            && (OB_SUCCESS == rpc_event->get_result_code()))
        {
          ...
          if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = check_if_need_more_req(sub_req_idx, timeout_us,
            *rpc_event))))
          {
            YYSYS_LOG(WARN,"fail to check if need any more request for subreq [err:%d,sub_req_idx:%ld]",
              err, sub_req_idx);
          }
          ...
          if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = merger_operator_.add_sharding_result(*scanner,
            ...
            finish, can_free_res))))
          {
            YYSYS_LOG(WARN, "fail to add sharding result. [err=%d]", err);
          }
      ...
      {
        rpc_event->get_server().to_string(ip_addr, sizeof(ip_addr));
        YYSYS_LOG(WARN, "rpc event return code not success. [err=%d,event_id:%lu, server=%s]", rpc_event->get_result_code(),
        ...
        if ((OB_SUCCESS == err) && is_first)
        {
          ...
          {
            YYSYS_LOG(WARN, "this retry not success");
          }
          if (OB_ITER_END == err)
          {
            YYSYS_LOG(WARN, "tried all replica and not successful.");
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if ((finished_sub_request_count_ == total_sub_request_count_) || (finish))
        {
          ...
          {
            YYSYS_LOG(WARN, "fail to prepare getting cell. [err=%d]", err);
          }
        }
    ...
    void ObMergerScanRequest::end_sessions_()
    {
      for (int64_t i = 0; (i < total_sub_request_count_); i++)
      {
        if (sub_requests_[i].get_session_id() > ObCommonRpcEvent::INVALID_SESSION_ID)
        {
          ...
          YYSYS_LOG(INFO,"end unfinished session [scan_event:%lu,idx:%ld,session_id:%lu]", get_request_id(),
    ...
    int ObMergerScanRequest::retry(const int64_t sub_req_idx, ObMergerRpcEvent *rpc_event, int64_t timeout_us)
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (sub_requests_[sub_req_idx].tried_replica_count() >= sub_requests_[sub_req_idx].total_replica_count())
        {
          ...
          YYSYS_LOG(WARN, "exceeds retry times. [tried_replica_count=%d][total_replica_count=%d]",
        ...
        {
          YYSYS_LOG(WARN,"fail to resend rpc event of ObMergerSubScanRequest [idx:%ld,err:%d]", sub_req_idx, err);
        }
        else if (OB_SUCCESS == err)
        {
    ...
    int ObMergerScanRequest::prepare_get_cell()
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "fail to seal merger operator. [err=%d]", err);
      }
      else
      {
      ...
      bool &got_all_result)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid result_row_width: %ld", result_row_width);
        err = OB_ERR_UNEXPECTED;
      }
      for (int64_t i = 0; (i < cur_row_cell_cnt_) && (OB_SUCCESS == err); i ++, got_cell_count ++)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to add cell to scanner [err:%d]", err);
        }
      }
      ...
      while ((OB_SUCCESS == err) && (!size_over_flow))
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to get next cell from ObMergerScanRequest [err:%d, got_cell_count:%ld]", err, got_cell_count);
        }
        if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = get_cell(&cur_cell, &is_row_changed))))
        {
          YYSYS_LOG(WARN,"fail to get cell from ObMergerScanRequest [err:%d, got_cell_count:%ld]", err, got_cell_count);
        }
        if ((OB_SUCCESS == err) && (is_row_changed))
        {
          if (cell_remainder_num != 0)
          {
            YYSYS_LOG(WARN,"unexpected error, row changed unexpected[got_cell_count:%ld,result_row_width:%ld]",
        ...
        {
          YYSYS_LOG(WARN,"row cells count is too big, [cur_row_cell_cnt_:%ld,max_row_cell_cnt:%ld]",
        ...
        {
          YYSYS_LOG(WARN,"fail to get aggregate column_name [cell_idx:%ld,err:%d]", cell_remainder_num, err);
        }
        if ((OB_SUCCESS == err)
          && (org_scan_param.get_group_by_param().get_aggregate_row_width() <= 0)
          ...
          row_cells_[cur_row_cell_cnt_].column_name_))))
        {
          YYSYS_LOG(WARN,"fail to get select column_name [cell_idx:%ld,err:%d]", cell_remainder_num, err);
        }
        if (OB_SUCCESS == err)
        {
          ++cur_row_cell_cnt_;
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to add cell to result [err:%d]", err);
          }
          else if (OB_SIZE_OVERFLOW == err)
          {
            if (OB_SUCCESS != (err = scanner.rollback()))
            {
              YYSYS_LOG(WARN,"fail to rollback ObScanner [err:%d]", err);
            }
            else
            {
              size_over_flow = true;
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"ridiculous query which returned zero column");
        }
        else if (scanner.get_cell_num() % result_row_width != 0)
        {
          YYSYS_LOG(WARN,"unexpected err [got_cell_count:%ld,result_row_width:%ld]", scanner.get_cell_num(), result_row_width);
          err = OB_ERR_UNEXPECTED;
        }
      }
