...
int oceanbase::mergeserver::ObMergerScanParam::set_param(ObScanParam & param)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to set merge server scan param [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
    /// scan direction
    ...
    i++, return_info_idx ++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to add basic clumn [err:%d]", err);
    }
  }
  ...
  for (int32_t i = 0; (i < orderby_column_size) && (OB_SUCCESS == err); i++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to add orderby info to ms scan param [err:%d,idx:%d]", err, i);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
      ...
      && (decoded_org_param_->get_orderby_column_size() <= 0))
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to add orderby info to cs scan param [err:%d]", err);
      }
    }
  }
