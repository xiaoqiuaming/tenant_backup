...
int ObBloomFilterTaskQueueThread::handleTask(ObBloomFilterTask *task, void* arg)
{
  ...
  {
    YYSYS_LOG(ERROR, "task is NULL or ObMergeServer get_cache_proxy return NULL, fatal error."
  ...
  {
    YYSYS_LOG(ERROR, "get tablet location failed, err: %d  table_id: %lu  rowkey: %s",
        err, task->table_id, to_cstring(task->rowkey));
  }
  else if (loc_list.size() <= 0)
  {
    YYSYS_LOG(WARN, "tablet location size is %ld, no chunkserver could be selected, "
  ...
  else
  {
    ...
    {
      YYSYS_LOG(DEBUG, "bloomfilter already exists");
      int err = OB_SUCCESS;
      ...
      {
        YYSYS_LOG(WARN, "revert bloom filter failed, err=%d", err);
      }
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "get bloom filter failed, target_cs=%s, range=%s, "
      ...
      else if (OB_CS_EMPTY_TABLET == ret)
      {
        ...
        if (NULL != bf)
        {
          ...
          {
            YYSYS_LOG(ERROR, "put empty bloom filter failed, ret=%d", ret);
          }
          else
          {
            YYSYS_LOG(INFO, "put empty bloom filter success, tablet: %s tablet_version: %d",
                to_cstring(range), tablet_version);
          }
          int destroy_ret = OB_SUCCESS;
          if (OB_SUCCESS != (destroy_ret = destroy_bloom_filter(bf)))
          {
            YYSYS_LOG(WARN, "destroy bloom filter failed, ret=%d", destroy_ret);
          }
          else
          {
            YYSYS_LOG(DEBUG, "destroy bloom filter success");
          }
        }
      }
      else
      {
        ...
        if (buf == NULL)
        {
          ...
          YYSYS_LOG(ERROR, "allocate bloom filter failed,ret=%d", ret);
        }
        else
        {
          BloomFilter *bf = create_bloom_filter<AdapterAllocator>(bloom_filter_version);
          if (NULL != bf)
          {
            ...
            {
              YYSYS_LOG(WARN, "deserialize bloom filter failed ret=%d", ret);
            }
            else if (OB_SUCCESS != (ret = insert_cache.put(insert_cache_key, *bf_v2)))
            {
              YYSYS_LOG(WARN, "put to insert cache failed, ret=%d", ret);
            }
            else
            {
              YYSYS_LOG(TRACE, "update insert cache success, tablet: %s tablet_version: %d",
                  to_cstring(range), tablet_version);
            }
            int destroy_ret = OB_SUCCESS;
            if (OB_SUCCESS != (destroy_ret = destroy_bloom_filter(bf)))
...
int ObBloomFilterTaskQueueThread::push(ObBloomFilterTask *task)
{
  ...
  else
  {
    ...
    else
    {
      ...
      YYSYS_LOG(DEBUG, "too many task, task_limit_=%d, ret=%d", task_limit_, ret);
    }
  }
  return ret;
}
void ObBloomFilterTaskQueueThread::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  void *task = NULL;
