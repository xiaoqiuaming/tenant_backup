...
void ObMergerLeaseTask::runTimerTask(void)
{
  ...
  {
    YYSYS_LOG(ERROR, "%s", "check lease timer task inner stat failed");
  }
  else if (false == service_->check_lease())
  {
    // register lease
    YYSYS_LOG(WARN, "%s", "check lease expired register again");
    // if not registered will be blocked until stop or registered 
    ret = service_->register_root_server();
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "register root server failed:ret[%d]", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "%s", "register to root server succ");
    }
  }
  else
  {
    YYSYS_LOG(DEBUG, "check merge server lease ok");
  }
}
