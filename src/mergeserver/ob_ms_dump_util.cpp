...
void ObMergerDumpUtil::dump(const ObGetParam & param)
{
  ...
  for (int64_t i = 0; i < count; ++i)
  {
    ...
    if (NULL != cur_cell)
    {
      YYSYS_LOG(INFO, "tableid:%lu,rowkey:%.*s,column_id:%lu,ext:%ld,type:%d",
          cur_cell->table_id_,
          cur_cell->row_key_.length(), cur_cell->row_key_.ptr(), cur_cell->column_id_,
          cur_cell->value_.get_ext(), cur_cell->value_.get_type());
      hex_dump(cur_cell->row_key_.ptr(), cur_cell->row_key_.length(), true, YYSYS_LOG_LEVEL_INFO);
      cur_cell->value_.dump();
    }
    else
    {
      YYSYS_LOG(WARN, "get cell failed:index[%ld]", i);
      break;
    }
  }
}
...
void ObMergerDumpUtil::dump(const ObScanParam & param)
{
  ...
  if (NULL == range)
  {
    YYSYS_LOG(WARN, "check scan param range failed:table_id[%lu], range[%p]",
  ...
  else
  {
    ...
    YYSYS_LOG(INFO, "scan:table_id[%lu], range[%s], direction[%s]",
...
void ObMergerDumpUtil::dump(const ObScanner & scanner)
{
  ...
  YYSYS_LOG(INFO, "fullfill item count:fullfill[%d], count[%ld]", is_fullfilled, count);
