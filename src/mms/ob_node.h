...
namespace oceanbase
{
  namespace mms
  {
     ...
     * Node功能接口
     **************************************/
    class ObNode : public yynet::IPacketQueueHandler
    {
      public:
        ObNode();
        ~ObNode();
        ...
        int heartbeat(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);

        //通知本slave node下线,//int stop_slave();
        int stop_slave(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        ...
        int slave_down(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);

        //通知本slave，你要转变成master
        int transfer_2_master(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
