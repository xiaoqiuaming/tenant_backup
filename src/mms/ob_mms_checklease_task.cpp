...
void ObMMSCheckleaseTask::runTimerTask(void)
{
  if(node_ == NULL)
  {
    YYSYS_LOG(ERROR, "node_ should not be NULL");
  }
  else
  {
    ...
    {
      if(node_->get_lease().lease_time + node_->get_lease().renew_interval > yysys::CTimeUtil::getTime())
      {
        //do nothing
      }
      else
      {
        if(node_->get_lease().lease_time + node_->get_lease().lease_interval < yysys::CTimeUtil::getTime())
        {
          YYSYS_LOG(DEBUG, "lease time out.");
          status = UNKNOWN;
          node_->handle_lease_timeout();
        }
        else
        {
          if(node_->get_lease().lease_time + node_->get_lease().renew_interval < yysys::CTimeUtil::getTime())
          {
            YYSYS_LOG(DEBUG, "need to re_regiter");
