...
int ObMonitor::init(ObBaseServer *server, ObClientManager *client_manager, int64_t lease_interval, int64_t renew_interval, int64_t timeout, int64_t retry_times)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(heartbeat_task_ == NULL)
    {
      ...
      YYSYS_LOG(WARN, "fail to new heartbeat_task_, err=%d", err);
    }
  }
  if(OB_SUCCESS == err)
  {
    ...
    {
      err = OB_ERROR;
      YYSYS_LOG(ERROR, "create node map fail");
    }
  }
  return err;
}
...
int ObMonitor::start()
{
  ...
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "fail to schedule heasrbeat task, err=%d", err);
  }
  return err;
}
...
//����������İ� 
bool ObMonitor::handlePacketQueue(yynet::Packet *packet, void *args)
{
  UNUSED(args);
  bool ret = true;
        ...
        switch(packet_code)
        {
          ...
          case OB_MMS_SLAVE_DOWN:
            ...
            /*case OB_MMS_RENEW_LEASE:
              ...
              break;*/
          default:
            YYSYS_LOG(WARN, "UNKNOWN packet.packet_code=%d", packet_code);
...
int ObMonitor::node_register(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  ...
  {
    YYSYS_LOG(WARN, "version not equal. version=%d, MY_VERSION=%ld", version, MY_VERSION);
  ...
  if(OB_SUCCESS == err && result_msg.result_code_ == OB_SUCCESS)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to deserialize node server. err = %d", err);
    } 
  }
  ...
  if(OB_SUCCESS == err && result_msg.result_code_ == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to register node. err = %d", result_msg.result_code_);
    }
  ...
  { 
    ...
    { 
      YYSYS_LOG(ERROR, "have not set node_register_handler_");                                                 
      result_msg.result_code_ = OB_INNER_STAT_ERROR;                                                                               
    }
    if(result_msg.result_code_ != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to handleNodeRegister. err=%d", result_msg.result_code_);
    }
  }
  ...
  if(OB_SUCCESS == err && result_msg.result_code_ == OB_SUCCESS)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to serialize status, err =%d", err);
    }
  }
...
int ObMonitor::slave_failure(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  int err = OB_SUCCESS;
  ObResultCode result_msg;
  ...
  if(OB_SUCCESS == err && result_msg.result_code_ == OB_SUCCESS)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to deserialize slave node.err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err && result_msg.result_code_ == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to process slave failure, err=%d", result_msg.result_code_);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to serialize the result code .err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to send_response, err=%d", err);
    }
  }  
  return err;
}
...
bool ObMonitor::if_node_exist(const ObServer &node, const char *app_name, const char *instance_name, node_list_type * &pnodelist, node_list_type::iterator &it_server)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid arugment.");
  }
  else
  {
...
int ObMonitor::add_register_node(const ObServer &node, const char *app_name, const char *instance_name, const MMS_Status status)
{
  ...
  if(OB_SUCCESS == err)
  {
    if(HASH_EXIST == node_map_.get(key, pvalue))
    {
      ...
      if(node_exist == true)
      {
        YYSYS_LOG(DEBUG, "re_register node. reset the node status");
        (*it_server)->is_valid = true;
        (*it_server)->lease.lease_time = yysys::CTimeUtil::getTime();
        (*it_server)->lease.lease_interval = lease_interval_;
        (*it_server)->lease.renew_interval = renew_interval_;
      }
      else
      {
        YYSYS_LOG(DEBUG, "already have register node in the list, add node_info");
        node_info *node_server = new(std::nothrow) node_info;
        if(node_server == NULL)
        {
          YYSYS_LOG(WARN, "fail to new node_server.");
          err = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
          ...
          node_server->lease.lease_time = yysys::CTimeUtil::getTime();                                           
    ...
    {
      YYSYS_LOG(DEBUG, "a new app+instance node register. add a map pair");
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to insert to node_map");
          err = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(DEBUG, "add map pair succ~!");
        }
      }
    }
  }
...
int ObMonitor::node_register_(const ObServerExt &node, const char *app_name, const char *instance_name, MMS_Status &status)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(is_exist == false)
    {
      YYSYS_LOG(DEBUG, "no master exist, set the node be!");
      status = MASTER_ACTIVE;
    }
    else
    {
      YYSYS_LOG(DEBUG, "already have master. set the node be slave");
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to add register node. err=%d", err);
    }
  }
  return err;
}
...
int ObMonitor::slave_failure_(const ObServer &slave, const char *app_name, const char *instance_name)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "slave entity not exist");
  ...
  if(OB_SUCCESS == err)
  {
    if(NULL != slave_failure_handler_)
    {
      ...
      if(err != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "fail to handler slave failure");
      }
    }
    else
    {
      YYSYS_LOG(ERROR, "handleSlaveFailure should not be NULL");
...
int ObMonitor::heartbeat(const int64_t timeout, const ObServer &master, const ObServer &node, MMS_Status &status)
{
  ...
  ObLease lease;
  lease.lease_time = yysys::CTimeUtil::getTime();
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to serialize lease.err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to serialize master");
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to decode status.err=%d", err);
    }
  }
...
int ObMonitor::report_slave_failure(const int64_t timeout, const int64_t retry_times, const ObServer slave, const ObServer master)
{
  ...
  for(int64_t i = 0; (i < retry_times) && (err != OB_SUCCESS); i++)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "%ldth, report slave failure fail. err =%d", i, err);
    }
    else
    {
      YYSYS_LOG(INFO, "report slave failure to master succ!");
    }      
  }

  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(ERROR, "can't report failure to master node. master addr =%d", master.get_ipv4());
    err = OB_ERROR;
  }
  return err;
}
...
int ObMonitor::report_slave_failure_(const int64_t timeout, const ObServer &slave, const ObServer &master)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to send request to master. err=%d", err);
    }
  }
...
int ObMonitor::change_master(const int64_t timeout, const int64_t retry_times, const ObServer &new_master)
{
  ...
  { 
    ...
    if(OB_SUCCESS != err)                                                                                      
    { 
      YYSYS_LOG(WARN, "%ldth, report change master fail. err =%d", i, err);                                     
    }
    else                                                                                                       
    { 
      YYSYS_LOG(INFO, "report change master to master succ!");                                                 
    }                                                                                                          
  }                                                                                                            

  if(OB_SUCCESS != err)                                                                                        
  { 
    YYSYS_LOG(ERROR, "can't report to master node. master addr =%d, err=%d", new_master.get_ipv4(), err);               
  }
  return err;            
}
...
int ObMonitor::change_master_(const int64_t timeout, const ObServer& new_master)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to send request to new master .err = %d", err);
    }
  }
