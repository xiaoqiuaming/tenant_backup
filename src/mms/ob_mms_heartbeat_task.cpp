  ...
  while(it != node_map.end())
  {
    ...
    if(pvalue != NULL)
    {
      ...
      if(pvalue->has_master == false)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "select master fail.");
        }
        else
        {
        ...
        if(pnodelist != NULL)
        {
          ...
          while(it_list != pnodelist->end())
          {
            ...
            else
            {
              ...
              if((*it_list)->lease.lease_time + (*it_list)->lease.lease_interval < yysys::CTimeUtil::getTime())
              {
                if((*it_list)->status == MASTER_ACTIVE)
                {
                  ...
                  if(err != OB_SUCCESS)
                  {
                    YYSYS_LOG(ERROR, "fail to select master");
                  }
                  else
                  {
                    pvalue->has_master = true;
                  ...
                  if(OB_SUCCESS == err)
                  {
                    ...
                    if(err != OB_SUCCESS)
                    {
                      YYSYS_LOG(ERROR, "fail to change master.err=%d", err);
                    }
                ...
                else
                {
                  ...
                  if(err != OB_SUCCESS)
                  {
                    YYSYS_LOG(WARN, "fail to report slave failure.err=%d", err);
                  }
