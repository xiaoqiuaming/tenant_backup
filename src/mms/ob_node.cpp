...
int ObNode::init(ObBaseServer *server, int32_t ipv4, int32_t port, ObClientManager *client_manager, const ObLease &lease, ObServer *monitor_server)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to malloc space");
    ...
    if(OB_SUCCESS == err)
    {
      ...
      if(OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "fail to init timer");
      }
    }
    if(OB_SUCCESS == err)
    {
...
bool ObNode::handlePacket(ObPacket *packet)
{
  ...
  if(NULL == packet)
  {
    YYSYS_LOG(WARN, "invalid packet.");
  }
  else
  {
    if(!queue_thread_.push(packet, queue_size_, false))
    {
      YYSYS_LOG(WARN, "packet queue temporarily full");
      res = false;
    }
  }
  return res;
...
bool ObNode::handlePacketQueue(yynet::Packet *packet, void *args)
{
  UNUSED(args);
  bool ret = true;
  ...
  else
  {
    ...
    {
      YYSYS_LOG(WARN,
                "packet wait too long time, receive_time=%ld, cur_time=%ld, packet_max_timewait=%ld, packet_code = %d",
                ob_packet->get_receive_ts(),
    ...
    {
      YYSYS_LOG(WARN, "in_buff is NULL should not reach here");
      return_code = OB_INVALID_ARGUMENT;
    }
    else
    {
      yynet::Connection* conn = ob_packet->get_connection();
      ...
      if(thread_buffer != NULL)
      {
        ...
        if(OB_SUCCESS != return_code)
        {
          YYSYS_LOG(ERROR, "call func error packet_code is %d return code is %d", packet_code, return_code);
        }
      }
      else
      {
...
int ObNode::heartbeat(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, const uint32_t channel_id, common::ObDataBuffer& out_buffer)
{
  int err = OB_SUCCESS;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to deserialize lease. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to deserialize master_server. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "status serialize fail.err=%d", err);
    }
  }
...
int ObNode::stop_slave(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, const uint32_t channel_id, common::ObDataBuffer& out_buffer)
{
  int err = OB_SUCCESS;
  UNUSED(in_buff);
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_ )
  {
    ...
    {
      YYSYS_LOG(ERROR, "mms_stop_slave_handler not set");
...
int ObNode::transfer_state(const MMS_Status &new_state)
{
  int err = OB_SUCCESS;
  status_ = new_state;
  YYSYS_LOG(INFO, "transfer state to SLAVE_SYNC");
  return err;
}

int ObNode::slave_down(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  ...
  if(err == OB_SUCCESS && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to deserialize slave server. err=%d", err);
    }
  }
  ...
  if(err == OB_SUCCESS && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    {
      result_msg.result_code_ = OB_INNER_STAT_ERROR;
      YYSYS_LOG(ERROR, "have not set slave_down handler.");
    }
  }
...
int ObNode::transfer_2_master(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  UNUSED(in_buff);
  int err = OB_SUCCESS;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "can not transfer 2 master form %d", status_);
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "transfer_2_master_handler not set");
        result_msg.result_code_ = OB_INNER_STAT_ERROR;
      }
...
int ObNode::register_to_monitor(const int64_t timeout, const int64_t retry_times, const ObServer &monitor, MMS_Status &status)
{
  ...
  for(int64_t i = 0; (i < retry_times) && (err != OB_SUCCESS); i++)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "%ldth, register fail. err=%d", i, err);
      /* if(timeu < timeout)
         {
         ...
         {
         YYSYS_LOG(ERROR, "precise sleep, ret=%d", sleep_ret);
         }
         }*/
    }
    else
    {
      YYSYS_LOG(INFO, "regist to monitor succ!,start schedule");
      err = check_lease_timer_.schedule(check_lease_task_, DEFAULT_CHECK_LEASE_INTERVAL, true);
      if(OB_SUCCESS != err)
      { 
        YYSYS_LOG(WARN, "fail to schedule check_least_task, err = %d", err);                                     
      } 
    }
  }
  ...
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(ERROR, "can't register to monitor. monitor addr=%d", monitor.get_ipv4());
    err = OB_ERROR;
  }
  return err;
}
...
int ObNode::renew_lease(const int64_t timeout, const common::ObServer &monitor)
{
  ...
  /*ObDataBuffer data_buffer;
    ...
    if(err != OB_SUCCESS)
    {
    YYSYS_LOG(WARN, "fail to get data buffer. err=%d", err);
    }
    ...
    if(err != OB_SUCCESS)
    {
    YYSYS_LOG(ERROR, "fail to send request. err=%d", err);
    }
    }
    int64_t pos = 0;
    ObResultCode result_msg;
    ...
    if(OB_SUCCESS != err)
    {
    YYSYS_LOG(WARN, "fail to deserialize result_msg, err = %d", err);
    }
    else
    {
  ...
  YYSYS_LOG(DEBUG, "node should renew lease.port=%d", self_server_.get_port());
  MMS_Status status;
  err = register_to_monitor_(timeout, monitor, status);
  return err;
}
...
int ObNode::heartbeat_(const ObLease &lease, const ObServer &master_server)
{
  ...
  if(err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "fail to renew_lease_. err=%d", err);
  }
  ...
  if(OB_SUCCESS == err)
  {
    if(false == (master_server == master_server_))
    {
      ...
      {
        YYSYS_LOG(INFO, "I am the master now.");
      }
      else
      {
        if(master_changed_handler_ != NULL)
        {
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "fail to handleMasterChanged. err=%d", err);
          }
        }
        else
        {
...
int ObNode::register_to_monitor_(const int64_t timeout, const ObServer &monitor, MMS_Status &status)
{
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to serialize self_server_. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to encode str of app_name. app_name_=%s, err=%d", app_name_, err);
    }
  }
  FILL_TRACE_LOG("step2:serialize app_name, app_name=%s, err=%d", app_name_, err);

  //step3:���л�instance_name
  if(OB_SUCCESS == err)
  {
    err = serialization::encode_vstr(data_buffer.get_data(), data_buffer.get_capacity(), data_buffer.get_position(), instance_name_);
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to encode str of app_name. instance_name_=%s, err=%d", instance_name_, err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to send request to monitor, port=%d. err=%d", self_server_.get_port(), err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to deserialize result_code, err=%d", err);
    }
    else
    {
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to decode status_, err = %d", err);
    }
    else
    {
      status_ = status;
  ...
  if(OB_SUCCESS == err)
  {
    ...
    default_lease.lease_time = yysys::CTimeUtil::getTime();
...
int ObNode::get_rpc_buffer(common::ObDataBuffer &data_buffer)const
{
  ...
  if(NULL == rpc_buffer)
  {
    YYSYS_LOG(ERROR, "get thread rpc buff fail.buffer[%p]", rpc_buffer);
    err = OB_INNER_STAT_ERROR;
  }
  else
  {
    ...
    const common::ObServer &monitor, const common::ObServer &slave)
{
  ...
  { 
    ...
    if(OB_SUCCESS != err)
    { 
      YYSYS_LOG(WARN, "%ldth, report fail. err=%d, port=%d", i, err, monitor.get_port());     
    }
    else                                                                                  
    { 
      YYSYS_LOG(INFO, "report slave down succ!");                                         
    }                                                                                     
  }                                                                                       

  if(OB_SUCCESS != err)                                                                   
  { 
    YYSYS_LOG(ERROR, "can't report to monitor, monitor addr=%d", monitor.get_ipv4());   
    err = OB_ERROR;
  }
  return err;
}
...
int ObNode::report_slave_down_(const int64_t timeout, const ObServer &monitor, const ObServer &slave)
{
  ...
  if(err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "fail to get rpc buffer. err=%d", err);
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to serialize slave node .err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to serialize app_name.err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to serialzie instance_name. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to send request to monitor. err=%d, monitor addr=%di, port=%d", err, monitor.get_ipv4(), monitor.get_port());
    }
  }
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to deserialize result_code");
    }
    else
    {
      err = result_code.result_code_;
...
int ObNode::handle_lease_timeout()
{
  ...
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "fail to handle lease timeout");
  }
  }
  else
  {
    YYSYS_LOG(WARN, "have not set lease_timeout_handler_");
    err = OB_INNER_STAT_ERROR;
  }
  return err;
}
