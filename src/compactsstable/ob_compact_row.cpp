  ...
  namespace compactsstable
  {
    namespace
    {
      ...
      uint8_t get_time_type(int32_t vtype)
      {
        ...
        {
          YYSYS_LOG(WARN,"have no this type of time:%d",vtype);
        }
        return type;
      }
      ...
      int set_value(char* buffer, const int64_t size, int32_t& pos, const int64_t v, int& itype)
      {
        ...
        {
          YYSYS_LOG(WARN, "size overflow pos=%d size=%ld int_size=%d",
      ...
      inline int set_value(char* buffer, const int64_t size, int32_t& pos, const ObString &v)
      {
        ...
        {
          YYSYS_LOG(WARN, "size overflow pos=%d size=%ld varchar_size=%d",
                    pos, size, v.length());
          ret = OB_SIZE_OVERFLOW;
        }
        else
      ...
      inline int set_value(char* buffer, const int64_t size,int32_t& pos,const int64_t v)
      {
        ...
        {
          YYSYS_LOG(WARN,"size overflow,pos=%d size=%ld",pos,size);
          ret = OB_SIZE_OVERFLOW;
        }
        else
        {
      ...
      inline int get_follow_size(const ObCompactObjMeta* meta,const char* buf_end,int32_t& follow_size)
      {
        ...
        {
          YYSYS_LOG(WARN,"the address of meta invalid,meta:%p,buf_end:%p",meta,buf_end);
          ret = OB_INVALID_ARGUMENT;
        }
        else if (is_end_mark(*meta))
        {
          YYSYS_LOG(DEBUG,"end mark");
          ret = OB_ITER_END;
          follow_size = 0;
        }
        else
        {
          ...
          case COMPACT_OBJ_TYPE_ESCAPE:
            ...
            break;
          default:
            YYSYS_LOG(WARN,"unknow type:%hhu,%hhu,%hhu",meta->meta_,meta->type_,meta->attr_);
            ret = OB_INVALID_ARGUMENT;
            break;
          }
    ...
    int ObCompactRow::add_col(const uint64_t column_id, const ObObj &value)
    {
      ...
      if ((MAX_COL_ID < column_id))
      {
        ...
        {
          YYSYS_LOG(WARN,"invalid column_id:%lu",column_id);
          value.dump(YYSYS_LOG_LEVEL_WARN);
          ret = OB_SIZE_OVERFLOW;
        }
      }
      else if (NULL == buf_)
      ...
      {
        YYSYS_LOG(WARN,"not self alloc buffer,do not support add column");
        ret = OB_NOT_SUPPORTED;
      }
      else if (MAX_BUF_SIZE < static_cast<int64_t>((buf_pos_ + sizeof(ObCompactObjMeta) + COL_ID_SIZE))
              || MAX_COL_NUM <= col_num_)
      {
        YYSYS_LOG(WARN,"size overflow,buf_pos_:%d,col_num_:%d",buf_pos_,col_num_);
        ret = OB_SIZE_OVERFLOW;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else if (ObExtendType == vtype)
        {
          ...
          {
            YYSYS_LOG(WARN,"not supported exten:%ld,column id is %lu,buf_pos_ is %d",v,column_id,buf_pos_);
            hex_dump(buf_, buf_pos_,true,YYSYS_LOG_LEVEL_WARN);
            ret = OB_NOT_SUPPORTED;
          }
        }
        else
        {
          YYSYS_LOG(WARN,"not supported type:%d",vtype);
          ret = OB_NOT_SUPPORTED;
        }
    ...
    int ObCompactRow::set_end()
    {
      ...
      else if (!is_end_)
      {
        if (0 == buf_pos_)
        {
          YYSYS_LOG(WARN,"not data in this row");
          ret = OB_ERROR;
        }
    ...
    int ObCompactRow::set_row(const char* row,const int64_t row_len)
    {
      ...
      {
        YYSYS_LOG(WARN,"invalid row,row:%p,row_len:%ld",row,row_len);
        hex_dump(row,static_cast<int32_t>(row_len),true,YYSYS_LOG_LEVEL_WARN);
        ret = OB_ERROR;
      }
    ...
    int ObCompactRow::get_row_key(ObRowkey& rowkey)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"iterator row failed,ret=%d",ret);
        }
        ...
        {
          YYSYS_LOG(WARN,"invalid row");
        }
        /* TODO
        else if ((ret = cell->value_.get_varchar(rowkey)) != OB_SUCCESS)
        {
          YYSYS_LOG(WARN,"get varchar failed,ret=%d",ret);
        }
        */
        else if (rowkey.length() <= 0 || NULL == rowkey.ptr())
        {
          YYSYS_LOG(WARN,"rowkey is invalid");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObCompactRow::copy(const ObCompactRow& row)
    {
      ...
      if (NULL == buf_)
      {
        ...
        {
          ret = OB_ALLOCATE_MEMORY_FAILED;
          YYSYS_LOG(WARN,"alloc mem failed");
        }
        else
        {
          self_alloc_ = 1;
      ...
      for(int32_t i=0; (i < key_col_nums) && (OB_SUCCESS == ret); ++i)
      {
        if ((OB_SUCCESS == left.next_cell()) && (OB_SUCCESS == right.next_cell()))
        {
          ...
          if ((OB_SUCCESS == left.get_cell(&left_cell)) && (OB_SUCCESS == right.get_cell(&right_cell)))
          {
            ...
            {
              YYSYS_LOG(WARN,"can not compare");
            }
            else
            {
    ...
    int ObCompactRow::copy_key(char* buf,const int64_t buf_len,int64_t& pos,int32_t key_col_nums)
    {
      ...
      {
        int32_t key_size = static_cast<int32_t>(get_key_columns_len(key_col_nums));
    ...
    int32_t ObCompactRow::calc_col_num()
    {
      ...
      if ((buf_ != NULL) && (buf_pos_ > 0))
      {
        ...
        if (OB_ITER_END != ret)
        {
          YYSYS_LOG(WARN,"col_num:%d,col_pos:%p",col_num,col_meta);
          col_num = -1;
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN,"invalid row len:%d,actual len:%d",buf_pos_,total_len);
            col_num = -1;
          }
        }
      }
