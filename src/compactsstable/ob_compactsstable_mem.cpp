  ...
  namespace compactsstable
  {
                                  ...
                                  const int64_t frozen_time)
    {
      ...
      if (is_invalid_version_range(version_range) || (frozen_time < 0))
      {
        ...
        YYSYS_LOG(WARN,"invalid sstable argument");
      }
      else
      {
    ...
    int ObCompactSSTableMem::append_row(uint64_t table_id,ObCompactRow& row,int64_t& approx_space_usage)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(WARN,"only support one table");
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN,"get last row failed");
        }
        ...
        {
          YYSYS_LOG(WARN,"write row failed, ret = %d",ret);
        }
        //add index
    ...
    {
      YYSYS_LOG(INFO,"clear compactsstable mem");
      rows_.clear();
      row_count_ = 0;
      frozen_time_ = 0;
    ...
    bool ObCompactSSTableMem::Compare::operator()(const char* index,const common::ObRowkey& rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN,"invalid row,index:%p",index);
      }
      else if ((err = row.get_row_key(left_rowkey)) != OB_SUCCESS)
      {
        YYSYS_LOG(WARN,"get rowkey of row %p failed,ret=%d",index,err);
      }
      else
      {
    ...
    int ObCompactSSTableMem::locate_start_pos(const ObNewRange& range,const char**& start_pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"set row failed,row:%p,ret=%d",*start_pos,ret);
        }
        else if ((ret = row.get_row_key(found_key)) != OB_SUCCESS)
        {
          YYSYS_LOG(WARN,"invalid row,row:%p,ret=%d",*start_pos,ret);
        }
    ...
    int ObCompactSSTableMem::locate_end_pos(const ObNewRange& range,const char**& end_pos)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN,"set row failed,row:%p,ret=%d",*end_pos,ret);
          }
          else if ((ret = row.get_row_key(found_key)) != OB_SUCCESS)
          {
            YYSYS_LOG(WARN,"invalid row,row:%p,ret=%d",*end_pos,ret);
          }
    ...
    int ObCompactSSTableMem::find(const ObRowkey& rowkey,const char**& start_iterator)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"set row failed,row:%p,ret=%d",*pos,ret);
        }
    ...
    int ObCompactSSTableMem::add_index(const char* pos)
    {
      ...
      if (NULL == pos)
      {
        YYSYS_LOG(WARN,"pos is null,invalid argument");
        ret = OB_INVALID_ARGUMENT;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (size > 0)
        {
          ...
          {
            YYSYS_LOG(WARN,"malloc row index failed");
    ...
    int ObCompactSSTableMemIterator::set_scan_param(const ObNewRange& range,const ColumnFilter* cf,const bool is_reverse_scan)
    {
      ...
      {
        YYSYS_LOG(DEBUG,"locate start key out of range,ret=%d",ret);
      }
      else if ((ret = mem_->locate_end_pos(range,end_iterator_)) != OB_SUCCESS)
      {
        YYSYS_LOG(DEBUG,"locate end key out of range,ret=%d",ret);
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"set row failed,row:%p,ret=%d",*row_cursor_,ret);
        }
      }
    ...
    int ObCompactSSTableMemIterator::load_row(const char* row)
    {
      ...
      {
        YYSYS_LOG(WARN,"set row failed,row:%p,ret=%d",row,ret);
      }
      else if ((ret = current_row_.get_row_key(row_key_)) != OB_SUCCESS)
      {
        YYSYS_LOG(WARN,"invalid row,row:%p",row);
      }
      else
      {
