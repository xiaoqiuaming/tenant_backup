...
namespace oceanbase
{
  ...
  namespace compactsstable
  {
    ...
    public:
      const ObFrozenVersionRange& get_version_range() const
    ...
    public:
      ...
      inline int get_cell(oceanbase::common::ObCellInfo** cell, bool* is_row_changed)
      {
        ...
        if (NULL == cell)
        {
          YYSYS_LOG(ERROR, "invalid argument, cell=%p", cell);
          ret = common::OB_INVALID_ARGUMENT;
        }
        else if (common::OB_SUCCESS != iterator_status_)
        {
          YYSYS_LOG(ERROR, "initialize status error=%d", iterator_status_);
