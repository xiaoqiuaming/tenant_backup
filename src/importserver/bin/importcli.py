...
def ParseArgs():
  usage = ('usage: %prog [options] -t OVERWRITE[input_path1 table_name1 input_path2 table_name2 ...]\n'
           '       %prog [options] -t OVERWRITE_INDEX[input_path1 table_name1 index_name1 input_path2 table_name2 index_name2 ...]\n'
           '       %prog [options] -t OVERWRITE_GENERATE[input_path1 table_name1 input_path2 table_name2 ...]\n'
           '       %prog [options] -t OVERWRITE_GENERATE_INDEX[input_path1 table_name1 index_name1 input_path2 table_name2 index_name2 ...]\n'
           ...
           '       %prog [options] -t CREATE_BYPASS_CONFIG_INDEX table_name index_name [raw_data_field_count=1] [delim=1] [null_flag=2] [column_infos=1001-0,1002-1...]\n'
           '       %prog [options] -t DELETE_BYPASS_CONFIG table_name\n'
           '       %prog [options] -t DELETE_BYPASS_CONFIG_INDEX table_name index_name\n'
  ...
  parser.add_option('-t', '--cmd_type',
      help='Type of operation: OVERWRITE, OVERWRITE_INDEX, OVERWRITE_GENERATE, OVERWRITE_GENERATE_INDEX, STATE , KILL or MAJOR_FREEZE',
  ...
  elif cmd_type == 'CREATE_BYPASS_CONFIG_INDEX':
  ...
  elif cmd_type == 'DELETE_BYPASS_CONFIG_INDEX':
    if len(args)!=2:
      parser.print_help()
    else:
      for i in range(0,len(args)):
  ...
  elif cmd_type in ('OVERWRITE_INDEX','OVERWRITE_GENERATE_INDEX') and len(args) < 3:
    task_id = args[0]
    logging.debug('task_id      => ' + str(task_id))
    globals()['task_id'] = task_id 
  elif cmd_type in ('OVERWRITE_INDEX','OVERWRITE_GENERATE_INDEX'):
    if len(args) % 3 != 0 or len(args) == 0:
      parser.print_help()
      sys.exit(1)
    for i in range(0, len(args), 3):
      inputs.append([args[i], args[i+1], args[i+2]])
...
def SendRequest():
  #add qiuhm[failed retry]
  failed_retry_times = 5
  while failed_retry_times > 0:
  ...
  #add:e
    try:
      with SocketManager((server, int(port))) as sock:
        ...
        elif cmd_type.upper() == 'OVERWRITE_INDEX':
          if 'task_id' in globals():
            SendPacket(sock, OVERWRITE_CONTINUE_INDEX_TASK_PKG, task_id)
          else:
            SendPacket(sock, SUBMIT_OVERWRITE_INDEX_TASK_PKG, inputs)
            
        elif cmd_type.upper() == 'OVERWRITE_GENERATE':
          SendPacket(sock, OVERWRITE_GENERATE_TASK_PKG, inputs)
          
        elif cmd_type.upper() == 'OVERWRITE_GENERATE_INDEX':
          SendPacket(sock, OVERWRITE_GENERATE_INDEX_TASK_PKG, inputs)
        ...
        elif cmd_type.upper() == 'CREATE_BYPASS_CONFIG_INDEX':
          SendPacket(sock, CREATE_BYPASS_CONFIG_INDEX, inputs)
          
        elif cmd_type.upper() == 'DELETE_BYPASS_CONFIG':
          SendPacket(sock, DELETE_BYPASS_CONFIG, inputs)
          
        elif cmd_type.upper() == 'DELETE_BYPASS_CONFIG_INDEX':
          SendPacket(sock, DELETE_BYPASS_CONFIG_INDEX, inputs)
