...
def delete_invalid_dir(invalid_dir):
  if os.path.isdir(invalid_dir) == True:
    backup_dir_postfix = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    trash_dir = JoinPath(conf_dir, app_name, "_trash")
    dir_list = invalid_dir.split('/')
    last_dir_name=None
    for var in dir_list:
      last_dir_name = var;
    if os.path.isdir(trash_dir) == False:
      os.mkdir(trash_dir)
    log.debug("rename {3} {0}/{1}_{2}".format(trash_dir, last_dir_name, backup_dir_postfix, invalid_dir))
    os.rename(invalid_dir, "{0}/{1}_{2}".format(trash_dir, last_dir_name, backup_dir_postfix))
  return
      ...
      if ret == 0:
        ...
        if to_remove_count > 0:
          ...
          for k in sorted(self.trace.keys()):
            task = self.trace[k]
            if task['type'] in ('IMPORT_GENERATE', 'OVERWRITE_GENERATE', 'OVERWRITE_GENERATE_INDEX'):
              continue
    ...
    try:
      ...
      if task_id not in self.trace:
        ...
        if task['status'] == 'DONE' and task['type'] not in ('IMPORT_GENERATE', 'OVERWRITE_GENERATE'):
          ...
          log.error(msg)
        elif packettype == OVERWRITE_CONTINUE_TASK_PKG and task['type'] in ('OVERWRITE_GENERATE', 'OVERWRITE', 'OVERWRITE_GENERATE_INDEX', 'OVERWRITE_INDEX'):
          pass
        else:
          ret = 1
      ...
      if ret == 0:
        ...
        if ret != 0:
          ...
          if task['type'] == 'IMPORT_GENERATE':
            ...
            task['type'] = 'OVERWRITE'
          elif task['type'] == 'OVERWRITE_GENERATE_INDEX':
            task['type'] == 'OVERWRITE_INDEX'
    except Exception as err:
      log.exception(err)
      ret = 1
    finally:
...
class RequestHandler(SocketServer.StreamRequestHandler):
  ...
  def get_table_conf_param(self, table_name):
    ...
    try:
      ...
      column_info_desc = config.get(table_name, "column_info_desc_str")
      raw_data_field_count = config.get("public", "raw_data_field_cnt")
    ...
    return ret, rowkey_desc, partition_file, delim, column_info_desc, raw_data_field_count
  ...
  def pre_mr_table(self, task_id, table_name, table_id, hadoop_input_path, rs_ip, rs_port):
    ...
    try:
      ret, rowkey_desc, partition_file, delim, column_info_desc, raw_data_field_count = self.get_table_conf_param(table_name)
      if ret != 0:
      ...
      if ret == 0:
        ...
        hadoop_path_pre = JoinPath(hdfs_data_dir, app_name, task_id)
        hadoop_path = JoinPath(hdfs_data_dir, app_name, task_id, table_name)
        hadoop_output_path = JoinPath(hadoop_path, "sstables")
        hadoop_badfile_path = JoinPath(hadoop_path, "_bad_file")#add qiuhm[bug fix]20210826
        ...
        put_conf_cmd = R('cd {table_etc_dir} && {hadoop_cmd} fs -mkdir {hadoop_path_pre} && {hadoop_cmd} fs -mkdir {hadoop_path} && hdfs dfs -put {achived_conf} {hadoop_conf_path}', locals())
        achive_badfile_cmd = R('hdfs dfs -touch {hadoop_badfile_path}', locals())#add qiuhm[bug fix]20210826
        map_reduce_cmd = R('{hadoop_cmd} jar {mrsstable_jar} -conf {mr_configuration} '
            ' -archives {hadoop_conf_path} -table_id {table_id} -rowkey_desc {rowkey_desc} -column_desc {column_info_desc} -raw_data_field_count {raw_data_field_count}'
      ...
      if ret == 0:
        ...
        if ret !=0:
          ...
          output = Shell.popen(achive_badfile_cmd)#add qiuhm[bug fix]20210826
          log.debug('achive_badfile_cmd: {0}, output: {1}'.format(achive_badfile_cmd, output))
  ...
  def overwrite_load_table(self, table_name, table_id, hadoop_path):
    ret = 0
    # as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} import {table_name} {table_id} '
        # ' proxy://hadoop:/{hadoop_path}', locals())
    # log.debug(as_admin_cmd)
    for retry in range(start_load_table_retry):
      try:
        ...
        as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} import {table_name} {table_id} '
            ' proxy://hadoop:/{hadoop_path}', locals())
        log.debug(as_admin_cmd)
        msg = "call rs to load table {0}: {1}".format(retry, as_admin_cmd)
        SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
        output = Shell.popen(as_admin_cmd)
        log.debug(msg + ":\n" + output)
        ret = 0
        log.debug("succeed to call rs to load table {0} {1}".format(table_name, table_id))
        break
      except Exception as err:
        log.exception('ERROR: failed to call as_admin to import table {0}: {1}'.format(retry, err))
        ret = 1
        time.sleep(30)
      ...
      if ret != 0:
        ...
        try:
          ...
          for table_name in table_dict:
            ...
            if 0 != self.overwrite_load_table(table_name, table_id, hadoop_sstable_path):
              ret = 1
  ...
  def overwrite_op_generate_sstable(self, task_id, inputs):
    ...
    if ret == 0:
      ...
      for i in inputs:
        ...
        if ret == 0:
          ...
          expect_pos = table_name.find("__idx__")
          if expect_pos != -1:
            orig_table_id = table_name.split("__")[1]
            orig_database_name = table_name.split(".")[0]
            cmd = R('mysql --connect_timeout=30 -N -h {lms_ip} -P {ob_port} -u {ob_user} -p{ob_password} -e " select table_name from __first_tablet_entry where table_id= {orig_table_id}"', locals())
            log.debug(cmd)
            output = Shell.popen(cmd)
            line = output.split("\n")[0]
            log.debug(line)
            orig_table_name = orig_database_name + '.' + (line.split("\t")[0])
            idx_name = table_name.split("__idx__", 1)[1]
            result_path = JoinPath(conf_dir, "bypass", app_name, orig_table_name, idx_name)
          else:
            result_path = JoinPath(conf_dir, "bypass", app_name, table_name)
          if os.path.isdir(result_path) == False:
            os.makedirs(result_path)
          result_file = JoinPath(result_path, "sstable_result.ini")
          result_data = open(result_file, mode='w')
          result_data.write('table_name=')
          result_data.write(table_name)
          result_data.write('\n')
          result_data.write('table_id=')
          result_data.write(table_id)
          result_data.write('\n')
          result_data.close()
    ...
    if ret == 0:
      for table_name in table_names:
        ...
        log.info("hdfs dfs -ls {0}\n{1}".format(table_sstable_path, dir_list))
        result_data = open(result_file, mode='a')
        result_data.write('sstable_path=proxy://hadoop:/')
        result_data.write(table_sstable_path)
        result_data.write('\n')
        result_data.close()
  ...
  def index_name_handler(self, data):
    ret = 0
    table_infos = data[0].split(".")
    cmd = R('mysql --connect_timeout=30 -N -h {lms_ip} -P {ob_port} -u {ob_user} -p{ob_password} -e " select table_id from __first_tablet_entry where table_name= \'{table_infos[1]}\'"', locals())
    log.debug(cmd)
    output = Shell.popen(cmd)
    log.debug(output)
    line = output.split("\n")[0]
    log.debug(line)
    item = line.split("\t")[0]
    index_name = "__" + item + "__idx__" + data[1]
    
    cmd = R('mysql --connect_timeout=30 -N -h {lms_ip} -P {ob_port} -u {ob_user} -p{ob_password} -e " select count(*) from __first_tablet_entry where  table_name= \'{index_name}\'"', locals())
    log.debug(cmd)
    output = Shell.popen(cmd)
    log.debug(output)
    line = output.split("\n")[0]
    ...
    if item != '1':
      ret = 1
      msg = "have no index : {0}".format(index_name)
      log.error(msg)
      SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
    newdata =[]
    newdata.append(index_name)
    newdata[0] = table_infos[0] + "." + index_name
    if len(data) > 2:
      for i in range(2, len(data)):
        newdata.append(data[i])
        
    return ret,newdata,index_name
    
  def delete_bypass_config_index(self, data):
    ret = 0
    newdata = []
    ret,newdata,index_name = self.index_name_handler(data)
    if ret != 0:
      msg = "handle index name faild : {0}".format(index_name)
      log.error(msg)
      SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
    
    ret = self.delete_bypass_config(newdata)
    return ret
  ...
  def create_bypass_config_index(self, data):
    ret = 0
    newdata = []
    ...
    try:
      if len(data) < 1 or len(data) > 6:
        ret = 1
        msg = "wrong param for create_bypass_config_index: {0}".format(data)
        log.error(msg)
        SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
      else:
        orig_table_name = data[0]
        orig_index_name = data[1]
    except Exception as err:
      ret = 1
      msg = "exception happended during create_bypass_config_index: " + str(err)
      log.exception(msg)

    ret,newdata,index_name = self.index_name_handler(data)
    
    if ret != 0:
      msg = "handle index name failed : {0}".format(index_name)
      log.error(msg)
      SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
    
    ret = self.create_bypass_config(newdata, orig_table_name, orig_index_name)
    return ret

  def create_bypass_config(self, data, orig_table_name=None, orig_index_name=None):
    ret = 0
    ...
    try:
      if len(data) < 1 or len(data) > 5:
        ...
        for i in range(1, len(data)):
          ...
          m5 = re.match('^config_dir=(\S+)$', param)
          if m1 != None and len(m1.groups()) == 1:
            ...
            if null_flag < 1 or null_flag > 255:
              ...
              SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
              break
          elif m5 != None and len(m5.groups()) == 1:
            config_xml = m5.group(1)
          else:
            ret = 1
    ...
    try:
      ...
      if ret == 0:
        ...
        if ret !=0:
          log.error("failed to update schema, rs_ip={0} rs_port={1} schema={2}".format(rs_ip, rs_port, schema))
          delete_invalid_dir(table_dir)

      if ret == 0:
        #copy configuration.xml
        temp_conf_dir = JoinPath(conf_dir, "bypass", app_name, table_name)
        if orig_table_name != None and orig_index_name != None:
          temp_conf_dir = JoinPath(conf_dir, "bypass", app_name, orig_table_name, orig_index_name)
        if config_xml != None:
          temp_conf_dir = JoinPath(config_xml)
        if os.path.isfile(JoinPath(temp_conf_dir, "configuration.xml")) == False:
          if os.path.isdir(temp_conf_dir) == False:
            os.makedirs(temp_conf_dir)
          ret = 1
          msg = "config file {0} is not a file, please check if it exists.".format(JoinPath(temp_conf_dir, "configuration.xml"))
          log.error(msg)
          SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
        if ret == 0:
          shutil.copy(JoinPath(temp_conf_dir, "configuration.xml"), table_dir)
        if ret != 0:
          log.error("failed to copy configuration.xml, path={0}".format(temp_conf_dir))
          delete_invalid_dir(table_dir)
    ...
    try:
      with open(JoinPath(conf_dir, app_name, table_name, "schema.ini")) as f:
          for line in f:
            ...
            if len(items) == 5: # int or precise_datetime
              column_id = items[1]
              raw_data_idx = -1
              name = items[2]
              nullable = items[4]
              ...
              precision = -1
              scale = -1
              column_infos_result.append((column_id, raw_data_idx, name, type, length, precision, scale, matched, nullable))

            elif len(items) == 6: #varchar
              column_id = items[1]
              raw_data_idx = -1
              name = items[2]
              nullable = items[5]
    ...
    try:
      if column_infos == None:
        for idx in range(0, len(column_infos_result)):
          column_id, raw_data_idx, name, type, length, precision, scale, matched ,nullable= column_infos_result[idx]
          raw_data_idx = idx
          matched = True
          column_infos_result[idx] = (column_id, raw_data_idx, name, type, length, precision, scale, matched, nullable)
        if raw_data_field_count == 0:
        ...
        for rowkey_field in rowkey_fields:
          ...
          if m == None or len(m.groups()) != 3:
            ...
            if rowkey_type_name == "int":
              ...
              rowkey_type = 6
            elif rowkey_type_name == "int32":
              rowkey_type = 16
            elif rowkey_type_name == "float":
              rowkey_type = 2
            elif rowkey_type_name == "double":
              rowkey_type = 3
            elif rowkey_type_name == "datetime":
              rowkey_type = 4
            elif rowkey_type_name == "varchar":
              rowkey_type = 6
            elif rowkey_type_name == "bool":
              rowkey_type = 11
            elif rowkey_type_name == "decimal":
              rowkey_type = 12
            elif rowkey_type_name == "date":
              rowkey_type = 13
            elif rowkey_type_name == "time":
              rowkey_type = 14
            else:
              ret = 1
            ...
            for (column_id, raw_data_idx, name, type, length, precision, scale, matched, nullable) in column_infos_result:
              if name == rowkey_name:
                #if rowkey_type != type or (rowkey_type == 6 and rowkey_len != length): # type 6 "varchar"
                if rowkey_type != type: # type 6 "varchar"
                  ret = 1
                  ...
                  rowkey_desc.append((raw_data_idx, type, length, precision, scale, nullable))
    except Exception as err:
      ret = 1
  ...
  def write_data_syntax(self, table_name, raw_data_field_count, delim, null_flag, rowkey_desc, column_infos_result):
    ...
    try:
      ...
      f.write("[{0}]\n".format(table_name))
      rowkey_desc_str = ""
      for raw_data_idx, type, length, precision, scale, nullable in rowkey_desc:
        if len(rowkey_desc_str) > 0:
          rowkey_desc_str += ","
        if type == 6: #varchar
          rowkey_desc_str +="{0}-{1}-{2}-{3}".format(raw_data_idx, type, length, nullable)
        elif type == 12: #decimal
          rowkey_desc_str +="{0}-{1}-{2}-{3}-{4}".format(raw_data_idx, type, precision, scale, nullable)
        else:
          rowkey_desc_str +="{0}-{1}-{2}".format(raw_data_idx, type, nullable)
      f.write("rowkey_desc={0}\n\n".format(rowkey_desc_str))

      column_info_desc_str=""

      for (column_id, raw_data_idx, name, type, length, precision, scale, matched, nullable) in column_infos_result:
        f.write("column_info={0},{1},{3},{4},{5},{6} #{2}\n".format(column_id, raw_data_idx, name, length, precision, scale, nullable))
        if len(column_info_desc_str) > 0:
          column_info_desc_str += ","
        if type == 6:
          column_info_desc_str += "{0}-{1}-{2}-{3}".format(raw_data_idx, type, length, nullable)
        elif type == 12:
          column_info_desc_str += "{0}-{1}-{2}-{3}-{4}".format(raw_data_idx, type, precision, scale, nullable)
        else:
          column_info_desc_str += "{0}-{1}-{2}".format(raw_data_idx, type, nullable)
      f.write("column_info_desc_str={0}\n\n".format(column_info_desc_str))
  ...
  def handle(self):
    ...
    try:
      ...
      if packettype == None and data == None:
      ...
      elif packettype == DELETE_BYPASS_CONFIG_INDEX:
        ret = self.delete_bypass_config_index(data)        
      ...
      elif packettype == CREATE_BYPASS_CONFIG_INDEX:
        ret = self.create_bypass_config_index(data)        
        ...
        if ret != 0 or task == None:
          ...
          if packettype == OVERWRITE_CONTINUE_TASK_PKG and task_type == 'OVERWRITE':
            ...
            log.error(msg)
            SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
      elif packettype in (SUBMIT_OVERWRITE_TASK_PKG, OVERWRITE_GENERATE_TASK_PKG, SUBMIT_OVERWRITE_INDEX_TASK_PKG, OVERWRITE_GENERATE_INDEX_TASK_PKG):
        task_id = datetime.datetime.now().strftime('%Y%m%d%H%M%S-%f')
        table_name_list = []
        if packettype == SUBMIT_OVERWRITE_TASK_PKG:
          task_type = 'OVERWRITE'
        elif packettype == SUBMIT_OVERWRITE_INDEX_TASK_PKG:
          task_type = 'OVERWRITE_INDEX'
          for i in range(0, len(data)):
            ret,data[i][1:],index_name = self.index_name_handler(data[i][1:])
            if ret != 0:
              msg = "handle index name faile : {0}".format(index_name)
              log.error(msg)
              SendPacket(self.connection, RESPONSE_PKG, msg + '\n')
            data[i]=tuple(data[i])              
        elif packettype == OVERWRITE_GENERATE_TASK_PKG:
          task_type = 'OVERWRITE_GENERATE'
        elif packettype == OVERWRITE_GENERATE_INDEX_TASK_PKG:
          task_type = 'OVERWRITE_GENERATE_INDEX'
          for i in range(0, len(data)):
            ret,data[i][1:],index_name = self.index_name_handler(data[i][1:])
            if ret != 0:
              msg = "handle index name faile : {0}".format(index_name)
    ...
    if packettype == None and data == None:
      ...
      elif ret == 2: #add qiuhm[failed retry]
        SendPacket(self.connection, END_PKG, 'GEN_FAILED\n')
      else:
...
def GetUpsList(rs):
  '''
  Parsing updateserver addresses from output of 'as_admin stat -o ups' command
  '''
  try:
    rs_ip = rs['ip']
    rs_port = rs['port']
    as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} '
        'stat -o ups', locals())
...
def GetCsList(rs_ip, rs_port):
  '''
  Parsing chunkserver addresses from output of 'as_admin stat -o cs' command
  '''
  ret = 0
  cs_list = []
  try:
    as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} stat -o cs', locals())
    log.info(as_admin_cmd)
    output = Shell.popen(as_admin_cmd)
    cs_str_list = output.splitlines()[-1].strip().split(' ')
    if len(cs_str_list) < 2 or cs_str_list[0] != 'chunkservers:':
      ret = 1
      log.error("failed to get cs list from {0}:{1}".format(rs_ip, as_admin_cmd))
...
class OverwriteCheckLoadTable:
  ...
  def wait_load_table_result(self, rs_ip, table_name, table_id):
    ...
    if max_fail_count < 3:
      max_fail_count = 3
    cmd = R('mysql --connect_timeout=30 -N -h {lms_ip} -P {ob_port} -u {ob_user} -p{ob_password} -e "select * from bypass_import.load_data_history where table_id = {table_id} and table_name = \'{table_name}\' order by start_time desc"', locals())
    log.debug(cmd)
    time.sleep(10)
    for i in range(0, count):
      try:
        ...
        elif items[4] in ('NEW','DOING','PREPARE') :
          msg = "{3}: {0:>20}:{1:10} DOING     start_time={2:>20}".format(table_name, table_id, items[0], cur_time)
          log.info(msg)
          SendPacket(self.connection, RESPONSE_PKG, msg+"\n")
          if IsMaster(rs_ip,rs_port,self.connection) != True:
            msg = "master switch! the import task cant finish, please contact DBA to switch the master rs back to: rs_ip:{0},rs_port:{1}\n".format(rs_ip,rs_port)
            log.warning(msg)
            SendPacket(self.connection, RESPONSE_PKG, "WARN:"+msg+"\n")
...
def UpdateSchema(rs_ip, rs_port, schema_path):
  ...
  try:
    ...
    as_admin_cmd = R('rm -f {schema_path} && {bin_dir}/as_admin -r {rs_ip} -p {rs_port} '
        'print_schema -o location={schema_path}', locals())

    output = Shell.popen(as_admin_cmd)
    log.debug('as_admin_cmd: {0}, output: {1}'.format(as_admin_cmd, output))
...
def GetCSBypassTableIdList(rs, table_names):
  ...
  'ds_admin -i "fetch_bypass_table_id"' command
  ...
  try:
    ...
    ds_admin_cmd = R('{bin_dir}/ds_admin -t 60 -q -s {rs_ip} -p {rs_port} '
        '-i "fetch_bypass_table_id {table_names_str}"', locals())
    table_list = []
    table_dict = {}

    output = Shell.popen(ds_admin_cmd)
    log.debug('ds_admin_cmd: {0}, output: {1}'.format(ds_admin_cmd, output))
    expect_start_title = "fetch_bypass_table_id> "
    expect_pos = output.find("fetch_bypass_table_id>")
    if expect_pos == -1:
    #if not output.startswith(expect_start_title):
      log.exception("failed to get bypass table id list")
    else:
      table_str_list = output[expect_pos+len(expect_start_title):].split(' ')
...
def GetUpsBypassTableIdList(rs, table_names):
  ...
  try:
    rs_ip = rs['ip']
    rs_port = rs['port']
    as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} print_schema | grep table=', locals())
    table_list = []
    table_dict = {}

    output = Shell.popen(as_admin_cmd)
    log.debug('as_admin_cmd: {0}\noutput: {1}'.format(as_admin_cmd, output))
...
def IsMaster(vip, port, connection = None):
  ...
  try:
    ...
    rs_ip = vip
    rs_port = port
    #as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} stat', locals())
    as_admin_cmd = R('{bin_dir}/as_admin -r {rs_ip} -p {rs_port} get_rs_leader', locals())
    output = Shell.popen(as_admin_cmd)
    #if 'obi_role: MASTER' in output:
    count=output.count(rs_ip)
    if 'Connection refused' in output:
      is_master = False
      msg = 'can not connect to this root server:ip={0},port={1}'.format(rs_ip,rs_port)
      if connection != None:
        SendPacket(connection, RESPONSE_PKG, msg + '\n')
      log.error(msg)
    elif count == 1:
    ...
    msg = 'failed to call {0}'.format(as_admin_cmd)
    if connection != None:
      SendPacket(connection, RESPONSE_PKG, msg + '\n')
    log.exception(msg)
    return None
...
def ReloadRsConfig(connection = None):
  ...
  if obi_count <= 0:
    ...
    for i in range(obi_count):
      ...
      log.info(remote_rs_ip)
      rs_ip = None
      rs_port = None
      rs_ip_list = remote_rs_ip.split(',')
      for temp_rs_info in rs_ip_list:
        can_connect = True
        temp_ip_port = temp_rs_info.split(':', 1)
        temp_rs_ip = temp_ip_port[0]
        temp_rs_port = temp_ip_port[1]
        if len(temp_rs_ip) == 0 or len(temp_rs_port) == 0:
          ...
          tmp_ret = None
          try:
            sk.connect((temp_rs_ip, int(temp_rs_port)))
          except Exception:
            can_connect =  False
            msg = "failed to connect to this rs {0}:{1} ".format(temp_rs_ip, temp_rs_port)
            log.warning(msg)
          if can_connect == True:
            tmp_ret = IsMaster(temp_rs_ip, temp_rs_port, connection)
          if tmp_ret == None:
            continue
          elif tmp_ret == True:
            if rs_ip == None:
              rs_ip = temp_rs_ip
              rs_port = temp_rs_port
            else:
              msg = "more than one master cluster found, please check obi setting."\
                  "old {0} cur {1}".format(rs_ip, temp_rs_ip)
              log.error(msg)
              if connection != None:
                SendPacket(connection, RESPONSE_PKG, msg + '\n')
              ret = 1
            obi_rs.append(dict(ip=temp_rs_ip, port=temp_rs_port))
          else: #tmp_ret == False
            obi_rs.append(dict(ip=temp_rs_ip, port=temp_rs_port))
  ...
  if ret == 0:
    if 'rs_ip' not in globals() \
      ...
      msg = "update rs: master_rs_ip={0}, rs_port={1}, obi_count={2}, obi_rs={3}".format(
...
def ParseArgs():
  try:
    ...
    lms_ip = config.get('public', 'lms_ip')
...
SUBMIT_OVERWRITE_INDEX_TASK_PKG   = 301
OVERWRITE_GENERATE_INDEX_TASK_PKG = 302
OVERWRITE_CONTINUE_INDEX_TASK_PKG = 303
CREATE_BYPASS_CONFIG_INDEX        = 304
DELETE_BYPASS_CONFIG_INDEX        = 305
