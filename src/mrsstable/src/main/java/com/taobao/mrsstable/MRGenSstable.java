...
public class MRGenSstable extends Configured implements Tool {
  ...
  private static final String BAD_FILE = "_bad_file";
  ...
  public static class RowKeyDesc {
    ...
    public final int OB_NULL = 0;
    public final int FLOAT = 2;
    public final int DOUBLE = 3;
    public final int SEQ = 7;
    public final int DECIMAL = 12;
    public final int BOOL = 11;
    public final int DATE = 13;
    public final int TIME = 14;
    public final int INT32 = 16;
    ...
    private int[] isRowKeyColumn;
    private int[] allColumnInfoIndex;
    private int[] all_columnType;
    private int[] all_size;
    private int all_length;
    private int all_maxColumnId;
    private int[] all_precision;
    private int[] all_scale;
    private int[] all_nullable;
    ...
    private SimpleDateFormat dfList[] = {
        ...
        new SimpleDateFormat("HH:mm:ss"),             //5
        new SimpleDateFormat("HHmmss"),                 //6
        new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.ssssss"),  //7
    };
    ...
    public String getSimpleDesc() {
      ...
      for(int i=0; i<length; ++i){
        ...
        if (columnType[i] == VARCHAR) {
          desc.append(String.format("%d-%d-%d", columnType[i], size[i], nullable[i]));
        }
        else if(columnType[i] == DECIMAL)
        {
          desc.append(String.format("%d-%d-%d-%d", columnType[i], precision[i], scale[i], nullable[i]));
        }
        else {
          desc.append(String.format("%d-%d", columnType[i], nullable[i]));
        }
      }
    ...
    void checkRowKeyDesc(String rowkeyDescStr) {
      ...
      for (int i = 0; i < length; ++i) {
        ...
        if (columnType[i] == VARCHAR) {
          desc.append(String.format("%d-%d-%d-%d", orgColumnIndex[i], columnType[i], size[i], nullable[i]));
        }
        else if(columnType[i] == DECIMAL)
        {
          desc.append(String.format("%d-%d-%d-%d-%d", orgColumnIndex[i], columnType[i], precision[i], scale[i], nullable[i]));
        }
        else {
          desc.append(String.format("%d-%d-%d", orgColumnIndex[i], columnType[i], nullable[i]));
        }
      }
    ...
    void checkColumnDesc(String columnDescStr){
        StringBuffer desc = new StringBuffer();
        for(int i = 0; i < all_length; ++i){
            if (i != 0){
                desc.append(",");
            }
            if (all_columnType[i] == VARCHAR) {
                desc.append(String.format("%d-%d-%d-%d", allColumnInfoIndex[i], all_columnType[i], all_size[i], all_nullable[i]));
            }
            else if (all_columnType[i] == DECIMAL)
            {
                desc.append(String.format("%d-%d-%d-%d-%d", allColumnInfoIndex[i], all_columnType[i], all_precision[i], all_scale[i], all_nullable[i]));
            }
            else {
                desc.append(String.format("%d-%d-%d", allColumnInfoIndex[i], all_columnType[i], all_nullable[i]));
            }
        }
        String newColumnDescStr = desc.toString();
        if (!newColumnDescStr.equals(columnDescStr)) {
            throw new InvalidParameterException("Parameter mrsstable.column.desc[" + columnDescStr
                    + "] is not same as parsed column_desc[" + columnDescStr + "]");
        }
    }
    public RowKeyDesc buildRowKeyDesc(JobConf conf) {
      ...
      try {
        ...
        precision = new int[rowKeyDesc.length];
        scale = new int[rowKeyDesc.length];
        nullable = new int[rowKeyDesc.length];
        ...
        for (int i = 0; i < rowKeyDesc.length; i++) {
          ...
          if (columnDesc.length == 3 || columnDesc.length == 4 || columnDesc.length == 5) {
            orgColumnIndex[i] = Integer.parseInt(columnDesc[0]);
            columnType[i] = Integer.parseInt(columnDesc[1]);
            if (columnDesc.length == 3){
              nullable[i] = Integer.parseInt(columnDesc[2]);
              size[i] = 0;
              precision[i] = 0;
              scale[i] = 0;
            } else if(columnDesc.length == 4){
              nullable[i] = Integer.parseInt(columnDesc[3]);
              size[i] = Integer.parseInt(columnDesc[2]);
              precision[i] = 0;
              scale[i] = 0;
            }else {
              nullable[i] = Integer.parseInt(columnDesc[4]);
              size[i] = 0;
              precision[i] = Integer.parseInt(columnDesc[2]);
              scale[i] = Integer.parseInt(columnDesc[3]);
            }
            // check size
            int expectSize = -1;
            switch(columnType[i]){
      ...
      public RowKeyDesc buildColumnDesc(JobConf conf) {
          String AllColumnDescStr = conf.get("mrsstable.column.desc", "");
          String raw_data_field_count = conf.get("mrsstable.raw_data_field_count", "");
          int raw_field_count = Integer.parseInt(raw_data_field_count);
          try {
              if (AllColumnDescStr.isEmpty())
              {
                  throw new InvalidParameterException("not mrsstable.column.desc config found");
              }

              String[] columninfoDesc = AllColumnDescStr.split(",");
              if (columninfoDesc.length <= 0) {
                  throw new InvalidParameterException("wrong mrsstable.column.desc config");
              }
              allColumnInfoIndex = new int[columninfoDesc.length];
              isRowKeyColumn = new int[raw_field_count];
              all_columnType = new int[columninfoDesc.length];
              all_size = new int[columninfoDesc.length];
              all_precision = new int[columninfoDesc.length];
              all_scale = new int[columninfoDesc.length];
              all_nullable = new int[columninfoDesc.length];
              all_length = columninfoDesc.length;

              for (int i = 0; i < columninfoDesc.length; i++) {
                  String[] columnDesc = columninfoDesc[i].split(INDEX_DELIM, -1);
                  if (columnDesc.length == 3 || columnDesc.length == 4 || columnDesc.length == 5) {
                      allColumnInfoIndex[i] = Integer.parseInt(columnDesc[0]);
                      all_columnType[i] = Integer.parseInt(columnDesc[1]);
                      if (columnDesc.length == 3){
                          all_nullable[i] = Integer.parseInt(columnDesc[2]);
                          all_size[i] = 0;
                          all_precision[i] = 0;
                          all_scale[i] = 0;
                      } else if(columnDesc.length == 4){
                          all_nullable[i] = Integer.parseInt(columnDesc[3]);
                          all_size[i] = Integer.parseInt(columnDesc[2]);
                          all_precision[i] = 0;
                          all_scale[i] = 0;
                      }else {
                          all_nullable[i] = Integer.parseInt(columnDesc[4]);
                          all_size[i] = 0;
                          all_precision[i] = Integer.parseInt(columnDesc[2]);
                          all_scale[i] = Integer.parseInt(columnDesc[3]);
                      }
                      // check size
                      int expectSize = -1;
                      switch(all_columnType[i]){
                          case INT:
                              expectSize = 0;
                              break;
                          case PRECISE_DATETIME:
                          ...
                          default:
                              throw new InvalidParameterException("miss col type:" + all_columnType[i]);
                      }
                      if (expectSize != all_size[i]){
                          throw new InvalidParameterException("for columnDesc ("+columnDesc+")["+i+"] expect size["+expectSize+"] not equals to size["+all_size[i]+"]");
                      }
                  } else {
                      throw new InvalidParameterException("each column desc must include 3 part or 4 part");
                  }
              }
          } catch (NumberFormatException e) {
              throw new InvalidParameterException("Parameter mrsstable.column.desc is not set properly. "
                      + "Use comma seperated number list");
          }
          assert (allColumnInfoIndex.length > 0);

          checkColumnDesc(AllColumnDescStr);
          this.all_maxColumnId = 0;
          for (int column : this.allColumnInfoIndex) {
              if (this.all_maxColumnId < column) {
                  this.all_maxColumnId = column;
              }
          }
          for(int column: this.orgColumnIndex)
          {
              isRowKeyColumn[column] = 1;
          }
    ...
    public int getAllcolumnNums()
    {
        return all_length;
    }
    public int[] getAllColumnIndex()
    {
          return allColumnInfoIndex;
      }
    public int[] getAllColumnType()
    {
          return all_columnType;
      }

    public int getAllMaxColumnId() {
          return this.all_maxColumnId;
      }
    ...
    public SimpleDateFormat getDateFromat(String field){
      ...
      if (field.indexOf("-") > -1) {
        if(field.indexOf(":") > -1 && field.indexOf("-") > -1 && field.indexOf(".") > -1){
          df = dfList[7];
        ...
        if (field.indexOf(":") > -1 && field.length() <= 8) {
          df = dfList[5];
        } else if (field.indexOf(":") > -1 && field.indexOf("-") <= -1) {
    ...
    public boolean isAllColumnValid(String[] vec) {
        boolean is_valid = true;

        if (vec.length <= all_maxColumnId) {
            is_valid = false;
            System.out.println("the columns num is not correct, vec.length=[" + vec.length + "] and maxColumnId = [" + all_maxColumnId + "]");
        } else {
            System.out.println("length" + allColumnInfoIndex.length);
            for (int i = 0; i < allColumnInfoIndex.length; ++i) {
                if (!checkAllColumnField(vec[allColumnInfoIndex[i]], i)) {
                    is_valid = false;
                    break;
                }
                else
                {
                    if ("".equals(vec[allColumnInfoIndex[i]])) {
                        vec[allColumnInfoIndex[i]] = "\2";
                    }
                    // add [fix date] by wukunyang
                    else if(is_valid && vec[allColumnInfoIndex[i]].equalsIgnoreCase("NULL"))
                    {
                        vec[allColumnInfoIndex[i]] = "\2";
                    }
                    // add e
    ...
    public boolean isRowkeyValid(String[] vec) {
      ...
      if (vec.length <= maxColumnId){
        ...
        System.out.println("length" + orgColumnIndex.length);
        for (int i = 0; i < orgColumnIndex.length; ++i) {
          ...
          {
             if("".equals(vec[orgColumnIndex[i]])){
               vec[orgColumnIndex[i]] = "\2";
             }
             // add [fix date] by wukunyang
             else if(is_valid && vec[allColumnInfoIndex[i]].equalsIgnoreCase("NULL"))
             {
    ...
    public boolean checkAllColumnField(String field, int idx)
    {
        boolean is_valid = true;
        boolean is_rowkey = false;

        if (isRowKeyColumn[allColumnInfoIndex[idx]] == 1)
        {
            is_rowkey = true;
        }

            switch(all_columnType[idx]){
                case INT:
                {
                    try {
                        if(field.length() == 0 && all_nullable[idx] == 1)
                        {
                            is_valid = true;
                        }
                        else if(field.length() == 0 && all_nullable[idx] == 0){
                            is_valid = false;
                        }else{
                ...
                case VARCHAR:
                {
                    if (field.length() > all_size[idx]){
                        is_valid = false;
                        System.out.println("varchar is too long(limit=" + all_size[idx]+") [" + field + "]");
                    }
                    else if (field.length() == 1 && field.equals(NULL_FLAG) && is_rowkey)
                    {
                        is_valid = false;
                ...
                case PRECISE_DATETIME:
                {
                    ...
                    String null_str = "NULL"; // add [fix date] by wukunyang
                    if(field.length() == 0 && all_nullable[idx] == 1)
                    {
                        is_valid = true;
                    }
                    // add [fix date] by wukunyang
                            ...
                            null_str.equalsIgnoreCase(field))
                    {
                        field = "";
                        is_valid = true;
                    }
                ...
                case INT32:
                {
                    try{
                        ...
                        System.out.println("failed to parse int32 [" + field + "]");
                        is_valid = false;
                    }
                    break;
                }
                ...
                case FLOAT:
                {
                    try{
                        ...
                        System.out.println("failed to parse float [" + field + "]");
                        is_valid = false;
                    }
                    break;
                }
                ...
                case DOUBLE:
                {
                    try{
                        ...
                        System.out.println("failed to parse double [" + field + "]");
                        is_valid = false;
                    }
                    break;
                }
                ...
                case DECIMAL:
                {
                    try{
                        if(field.length() == 0 && all_nullable[idx] == 1)
                        {
                            ...
                            BigDecimal Max_value = new BigDecimal(+1.79E+308);
                            BigDecimal Min_value = new BigDecimal(-1.79E+308);
                            BigDecimal value = new BigDecimal(field);
                            if(1 == value.compareTo(Max_value) || -1 == value.compareTo(Min_value)){
                                is_valid = false;
                            }
                        }
                    }catch (NumberFormatException e){
                        System.out.println("failed to parse decimal [" + field + "]");
                        is_valid = false;
                    }
                    break;
                }
                ...
                case DATE:
                {
                    ...
                    // add e
                        ...
                        try {
                            ...
                            System.out.println("failed to parse date [" + field + "]");
                            is_valid = false;
                        }
                    }
                    break;
                ...
                case TIME:
                {
                    ...
                    // add e
                        ...
                        try {
                            ...
                            System.out.println("failed to parse time [" + field + "]");
                            is_valid = false;
                        }
                    }
                    break;
                ...
                case BOOL:
                {
                    try{
                        ...
                        System.out.println("failed to parse bool [" + field + "]");
                        is_valid = false;
                    }
                    break;
                }
                ...
                default:
                    ...
                    is_valid = false;
                    throw new InvalidParameterException("miss col type:" + all_columnType[idx]);
    ...
    public boolean checkField(String field, int idx)
    {
      ...
      else
      {
        ...
        {
          try {
            if(field.length() == 0 && nullable[idx] == 1)
            {
               is_valid = true;
            }
            else if(field.length() == 0 && nullable[idx] == 0){
               is_valid = false;
            }else{
                long value = Long.parseLong(field);
                if (value < (-9223372036854775807L-1) || value > 9223372036854775807L){
        ...
        case TIME:
        {
           ...
           else if(field.length() > 0 && nullable[idx] == 1 &&
                   null_str.equalsIgnoreCase(field))
           {
               field = "";
               is_valid = true;
  ...
  public static class TokenizerMapper implements Mapper<Object, Text, Text, Text> {
    ...
    public void map(Object key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
      ...
      for (int i = 1; i < keyColumnIndex.length; i++) {
        System.out.println("this is " + vec[keyColumnIndex[0]]);
  ...
  public static class TaggedKeyComparator extends Configured implements RawComparator<Text> {
    ...
    public float getTaggedFloat(String str) {
      float rtval;
      try {
        rtval = Float.parseFloat(str);
      } catch (NumberFormatException e) {
        rtval = 0;
        LOG.warn("Failed to parse integer [" + str + "]:" + e);
      }
    ...
    public double getTaggedDouble(String str) {
      double rtval;
      try {
    ...
    public double getTaggedDecimal(String str) { //dduuhhtt ??decimal use as double?
      double rtval;
      try {
        rtval = Double.parseDouble(str);
      } catch (NumberFormatException e) {
    ...
    public int compare(Text left, Text right) {
      ...
      for (int i = 0; i < rowKeyDesc.length; ++i) {
        ...
        else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.FLOAT) {
          float left_val = getTaggedFloat(left_columns[i]);
          float right_val = getTaggedFloat(right_columns[i]);
          if (left_val != right_val) {
            if (left_val < right_val){
              return -1;
            }
        ...
        }else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.DOUBLE) {
          double left_val = getTaggedDouble(left_columns[i]);
          double right_val = getTaggedDouble(right_columns[i]);
          if (left_val != right_val) {
            if (left_val < right_val){
              return -1;
            }
        ...
        }else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.DECIMAL) {
          double left_val = getTaggedDecimal(left_columns[i]);
          double right_val = getTaggedDecimal(right_columns[i]);
          if (left_val != right_val) {
            if (left_val < right_val){
              return -1;
            }
        ...
        }else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.DATE) {
          Date left_val = getTaggedDate(left_columns[i]);
          Date right_val = getTaggedDate(right_columns[i]);
          int ret = left_val.compareTo(right_val);
          if (0 != ret) {
        ...
        }else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.TIME) {
          Date left_val = getTaggedDate(left_columns[i]);
          Date right_val = getTaggedDate(right_columns[i]);
          int ret = left_val.compareTo(right_val);
          if (0 != ret) {
        ...
        }else if (rowKeyDesc.getColumnType()[i] == rowKeyDesc.INT32) {
          long left_val = getTaggedInteger(left_columns[i]);
          long right_val = getTaggedInteger(right_columns[i]);
          if (left_val != right_val) {
            if (left_val < right_val){
  ...
  private void initPartitioner(JobConf conf) throws IOException, URISyntaxException {
    ...
    rowKeyDesc.buildColumnDesc(conf);
    ...
    if (writePartsFile) {
      ...
      Path outputDir = FileOutputFormat.getOutputPath(conf).getParent();
      Path badFile = new Path(outputDir, BAD_FILE);
      conf.set("mrsstable.bad.file", badFile.toString());
  ...
  private void mvBadFile(JobConf conf) throws  IOException {
      Path badFile = new Path(conf.get("mrsstable.bad.file", ""));
      Path outputDir = FileOutputFormat.getOutputPath(conf);
      Path newBadFile = new Path(outputDir, BAD_FILE);
      FileSystem fs = badFile.getFileSystem(conf);
      if(!fs.rename(badFile, newBadFile)) {
          String msg = "failed mv Bad File from " + badFile.toString() + " to " + newBadFile.toString();
          LOG.error(msg);
          throw new IOException(msg);
      }
      else {
          LOG.info("mv Bad File from " + badFile.toString() + " to " + newBadFile.toString());
      }
  }
