...
public class MrsstableRunner {
  ...
  private static Options buildMrsstableOptions() {
    ...
    Option tableId = new Option("table_id",true,"specify the id of the table to run mrsstable tool.");
    Option rowkeyDesc = new Option("rowkey_desc",true,"rowkey description, it defines the rowkey formation"
                       + " format = \"clomn index in rowkey\",\"column index in origin line\",\"the column type\"");
    Option sampler = new Option("rowkey_desc",true,"rowkey description, it defines the rowkey formation"
                       + " format = \"clomn index in rowkey\",\"column index in origin line\",\"the column type\"");
    Option delim = new Option("delim", true, "delimeter in hdfs data file");
    Option null_flag = new Option("null_flag",true,"null_flag in hdfs data file");
    Option job_name = new Option("job_name",true,"job name for mapreduce");
    Option partition_file = new Option("partition_file",true,"partition file path in hdfs");
    Option ColumnDesc = new Option("column_desc", true, "column description, it defines the column formation"
    + " format = \"column index\", \"column type\", \"nullable\"");
    Option raw_data_field_count = new Option("raw_data_field_count", true, "specify the raw data field count of the table to run mrsstable tool.");
    ...
    options.addOption(ColumnDesc);
    options.addOption(raw_data_field_count);
    return options;
  }
...
*/
    ...
    if (line.hasOption("column_desc")) {
      String value = line.getOptionValue("column_desc");
      conf.set("mrsstable.column.desc", value);
      LOG.info("overload mrsstable.column.desc with cmd opertion column_desc: " + value);
    }

    if (line.hasOption("raw_data_field_count")) {
      String value = line.getOptionValue("raw_data_field_count");
      conf.set("mrsstable.raw_data_field_count", value);
      LOG.info("overload mrsstable.raw_data_field_count with cmd opertion raw_data_field_count: " + value);
    }
  ...
  public static void printMrsstableCommandUsage(PrintStream out) {
    ...
    out.println("-column_desc <column_desc>     overload mrsstable.column.desc");
    out.println("-raw_data_field_count <raw_data_field_count>     overload mrsstable.raw_data_field_count");
