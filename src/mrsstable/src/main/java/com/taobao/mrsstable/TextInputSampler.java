...
public class TextInputSampler extends InputSampler<Writable, Text> {
  private static final MRLogger LOG = MRLogger.getLogger(TextInputSampler.class);
  ...
  public static class RandomSampler implements Sampler {
    ...
    public Text[] getSample(InputFormat<Writable, Text> inf, JobConf job) throws IOException {
      ...
      if (splits.length == 0) {
        ...
        throw new IOException("numSamples=" + numSamples + "must less than numPartitions =" + numPartitions);
      }
      ...
      Path badFile = new Path(job.get("mrsstable.bad.file", ""));
      FileSystem bad_fs = badFile.getFileSystem(job);
      if(bad_fs.exists(badFile))
      {
        bad_fs.delete(badFile, false);
      }
      FSDataOutputStream fileOut = bad_fs.create(badFile);
        ...
        while (reader.next(key, value)) {
          ...
          if (samples.size() < numSamples || r.nextDouble() <= 1.0 * numSamples / total_line) {
            ...
            try {
              ...
              {
                writer.write(value, nullValue);
                System.out.println("skip invalid the line: " + e.getMessage());
                continue;
              }
              else
              {
...
//    }
    if(!rowKeyDesc.isAllColumnValid(vec))
    {
      StringBuilder err_sb = new StringBuilder("text: [" + value.toString() + "]");
      throw new IllegalArgumentException(err_sb.toString());
    }
