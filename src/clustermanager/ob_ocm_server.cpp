...
int OcmServer::initialize()
{
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "load config file fail.ret=%d", ret);
  }
  if(OB_SUCCESS == ret)
  {
  ...
  if(OB_SUCCESS == ret)
  {	
    YYSYS_LOG(INFO, "ocm server initted");
  }
  if(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "fail to init service, ret = %d", ret);
    }
  }
  return ret;
}
...
void OcmServer::destroy()
{
  ...
  if (NULL != queue_thread_)
  {
    queue_thread_->stop();
    queue_thread_->wait();
    YYSYS_LOG(INFO, "ocm server destroyed");
  }
  // delete members
  if (NULL != queue_thread_)
  {
...
yynet::IPacketHandler::HPRetCode OcmServer::handlePacket(yynet::Connection *connection, yynet::Packet *packet)
{
  yynet::IPacketHandler::HPRetCode ret = yynet::IPacketHandler::FREE_CHANNEL;
  ...
  else
  {
    ...
    if (!queue_thread_->push(req, 10, false))
    {
      ...
      ret = yynet::IPacketHandler::KEEP_CHANNEL;
    }
  }
  return ret;
}

bool OcmServer::handleBatchPacket(yynet::Connection *connection, yynet::PacketQueue &pq)
{
  bool ret = true;
  UNUSED(connection);
  UNUSED(pq);
