...
int OcmService::init(ObOcmParam *ocm_param, yynet::Transport* transport, yynet::IPacketStreamer* streamer)
{
  int err = OB_SUCCESS;
  if(true == init_)
  {
    YYSYS_LOG(WARN, "ocm service already inited. init_=%d", init_);
    err = OB_INIT_TWICE;
  }
  if(OB_SUCCESS == err)
  {
    if(ocm_param == NULL || transport == NULL || streamer == NULL)
    {
      ...
      YYSYS_LOG(ERROR, "invalid argument to init ocmservice");
    }
  }
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "init ocm meta manager fail, err=%d", err);
    }
  }
  /* if(OB_SUCCESS == err)
     {
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(NULL == rpc_stub_)
    {
      YYSYS_LOG(ERROR, "malloc rpc_stub fail. rpc_stub_=%p", rpc_stub_);
  ...
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "fail to init rpc_stub_, err=%d", err);
    }
  }
  if(OB_SUCCESS == err)
  {
    YYSYS_LOG(INFO, "ocm start init...");
  }
    //��һ��������Ҫ����һ����Ϣ������ocm
  return err;
}
...
int OcmService::start()
{
  ...
  if(OB_SUCCESS == err)
  {
    YYSYS_LOG(INFO, "start to schedule broadcast task succ!");
  }
  else
  {
    YYSYS_LOG(WARN, "fail to start schedule broadcast task");
  }
  if(OB_SUCCESS == err)
  {
    YYSYS_LOG(INFO, "ocm service start...");
    broad_cast_ocm_info();
  }
  return err;
}
...
bool OcmService::handlePacketQueue(yynet::Packet *packet, void *args)
{
  bool res = false;
  int ret = OB_SUCCESS;
  UNUSED(args);
  ...
  else
  {
    ...
    uint32_t channel_id = ob_packet->getChannelId();//yynet need this
    ...
    {
      YYSYS_LOG(WARN, "packet timeout, now=%ld, timeout=%ld, recive_time=%ld", 
          now, timeout, ob_packet->get_receive_ts());
    ...
    {
      YYSYS_LOG(WARN, "packet deserialize error, err=%d", ret);
      ret = OB_INVALID_ARGUMENT;
    }
    else if (NULL == (thread_buff = thread_buffer_.get_buffer()))
    {
      YYSYS_LOG(ERROR, "no memory in thread buffer");
      ret = OB_ERROR;
    }
    else 
    {
...
int OcmService::dispatch(int pcode, const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, 
...
int OcmService::on_get_instance_role_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  int err = OB_SUCCESS;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "app_name deserialize fail. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "instance_name deserialize fail. err=%d", err);
    }
  }
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "status serialize error");
    }
  }
...
int OcmService::on_set_instance_role_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  static const int MY_VERSION = 1;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "decode status error");
    }
  }
...
int OcmService::on_update_ocm_list_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  static const int MY_VERSION = 1;
  common::ObResultCode result_msg;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "deserialize ocm meta fail. err=%d",err);
    }
  }
...
int OcmService::on_ocm_change_status_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  static const int MY_VERSION = 1;
  common::ObResultCode result_msg;
  ...
  if(OB_SUCCESS == err && OB_SUCCESS == result_msg.result_code_)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "ip addr deserialize fail.ip_addr = %s, err=%d", ip_addr.ptr(), err);
    }
  }
...
int OcmService::on_no_matching_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, 
    const uint32_t channel_id, common::ObDataBuffer& out_buff)
{
  int ret = OB_SUCCESS;
  UNUSED(version);
    ...
    yynet::Connection* connection, const int32_t channel_id)
{
  ...
  {
    rc = OB_ERROR;
    YYSYS_LOG(WARN, "connection is NULL");
  }
  ...
  if (packet == NULL)
  {
    ...
    YYSYS_LOG(ERROR, "create packet failed");
  }
  else
  {
  ...
  if (rc == OB_SUCCESS)
  {
    rc = packet->serialize();
    if (rc != OB_SUCCESS)
      YYSYS_LOG(WARN, "packet serialize error, error: %d", rc);
  }
  if (rc == OB_SUCCESS)
  {
    if (!connection->postPacket(packet))
    {
      ...
      YYSYS_LOG(WARN, "send packet to [%s] failed", yysys::CNetUtil::addrToString(peer_id).c_str());
      rc = OB_ERROR;
    }
  }
...
int OcmService::broad_cast_ocm_info()
{
  ...
  for(int64_t i = 0; i < ocm_param_->get_cluster_manager_count(); i++)
  {
    if(ocm_param_->get_ocm_status(i))
    {
      ...
      if(!res)
      {
        YYSYS_LOG(ERROR, "fail to get ocm_server's addr. ip=%s, port=%ld", ocm_addr->ip_addr, ocm_addr->port);
      }
      else
      {  
        for(int64_t j = 0; OB_SUCCESS != err && j < retry_times; j++)
        {
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "%ldth, broadcast fail. err =%d", j, err);
            if(timeu < timeout)
            {
        ...
        if(OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "send ocm info to %s fail.", ocm_addr->ip_addr);
        }
      }
    }
  }
