...
namespace oceanbase
{
  namespace clustermanager
  {
    ...
    int ObOcmInstance::set_read_radio(const int64_t radio)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, radio=%ld", radio);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObOcmInstance::get_instance_master_rs(const char *instance_name, ObServerExt &serv_info) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid argument,instance_name=%p, instance_name_=%p", instance_name, instance_name_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObOcmInstance::get_rs_list(ObServList& serv_list)
    {
      ...
      if(err != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "get_rs_list fail.");
      }
      return err;
    }
      ...
      else
      {
        ...
        else
        {
          ...
          *(reinterpret_cast<int64_t*> (buf + pos)) = reserve2_;
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "ObServerExt master_rs serialize fail");
          }
        }
      }
      return err;
    ...
    int ObOcmInstance::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "ObServerExt master_rs deserialize fail");
          }
        }
      }
      return err;
