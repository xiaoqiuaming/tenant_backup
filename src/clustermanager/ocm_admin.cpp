    ...
    switch (opt)
	{
      ...
      case 'i':
       ...
       case 'e':
          ...
          {
            YYSYS_LOG(WARN, "invalid argument. eg: -e [yes][no]");
            print_usage();
            exit(1);
          }
break;
case 'o':
clp.ocm_addr = optarg;
YYSYS_LOG(INFO,"ocm addr is %s", clp.ocm_addr);
break;
case 'm':
...
{
  YYSYS_LOG(WARN, "invalid argument. eg: -m [master][slave]");
  print_usage();
  exit(1);
}
break;		  
...
{
  int err = OB_SUCCESS;
  YYSYS_LOG(INFO,"set instance role:\n");
  ...
  {
    YYSYS_LOG(INFO,"warn, serialize app_name fail. err = %d\n", err);
  }  
  if(OB_SUCCESS == err)
  {
    if(OB_SUCCESS != (err = instance_name.serialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position())))
	{
	  YYSYS_LOG(INFO,"warn, serialize instance_name fail. err = %d\n", err);
	}
  }
  if(OB_SUCCESS == err)
  {
	...
	{
	  YYSYS_LOG(INFO,"warn,  serialize status fail. err = %d\n", err);
	}
  }
  ...
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(INFO,"fail to send request\n");
  }
  ...
  if(OB_SUCCESS == err)
  {
	...
	if(err != OB_SUCCESS)
	{
	  YYSYS_LOG(INFO,"deserialze result code fail. err = %d\n", err);
	}
	else
	{
	  if(result_code.result_code_ != OB_SUCCESS)
	  {
	    YYSYS_LOG(INFO,"fail to set instance role, result_code =%d\n", result_code.result_code_);
	  }
	  else
	  {
	    YYSYS_LOG(INFO,"set instance role succ! \n");
	  }
	}
  }
  return err;
...
int get_instance_role(ObBaseClient& client, CmdLineParam clp)
{
  ...
  ObString app_name;
  app_name.assign_ptr(clp.app_name, static_cast<int32_t>(strlen(clp.app_name)));
  YYSYS_LOG(INFO,"app_name=%s\n", app_name.ptr());
  ObString instance_name;
  instance_name.assign_ptr(clp.instance_name, static_cast<int32_t>(strlen(clp.instance_name)));
  YYSYS_LOG(INFO,"instance name = %s\n", instance_name.ptr());
  if(OB_SUCCESS != (err = app_name.serialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position())))
  {
    YYSYS_LOG(INFO,"warn, serialize app_name fail. err = %d\n", err);
  }  
  ...
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(INFO,"warn, fail to send request\n");
  }
  ...
  if(OB_SUCCESS == err)
  {
	...
	if(err != OB_SUCCESS)
	{
	  YYSYS_LOG(INFO,"warn, fail to deserialize result code. err=%d\n", err);
	}
	else
	{
	  if(OB_SUCCESS != result_code.result_code_)
	  {
	    YYSYS_LOG(INFO,"warn,fail to get instance role. err = %d\n", result_code.result_code_);
	  }
    else
    {
      ...
      if(OB_SUCCESS == err)
      {
        YYSYS_LOG(INFO,"get instance role succ! role =%d\n", status);
      }
      else
      {
        YYSYS_LOG(INFO,"warn, deserialize status fail.err=%d\n", err);
      }
    }
  }
  }
...
int set_ocm_if_exist(ObBaseClient &client, CmdLineParam clp)
{
  ...
  YYSYS_LOG(INFO,"ip_addr=%s\n", ip_addr.ptr());
  YYSYS_LOG(INFO,"ocm if exist =%d\n", clp.ocm_if_exist);
  err = ip_addr.serialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position());
  if(OB_SUCCESS != err)
  {
    YYSYS_LOG(INFO,"warn, fail to serialize ip addr, err = %d\n", err);
  }
  if(OB_SUCCESS == err)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(INFO,"warn, fail to encode ocm_if_exist. err=%d\n", err);
    }
  }
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(INFO,"warn, fail to send request. err=%d\n", err);
    }
  }
  msgbuf.get_position() = 0;
  ObResultCode result_code;
  if(OB_SUCCESS == err)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(INFO,"warn ,fail to deserialize result_code,err=%d\n", err);
    }
    else
    {
      if(OB_SUCCESS != result_code.result_code_)
      {
        YYSYS_LOG(INFO,"warn, set ocm exist fail. err = %d\n", result_code.result_code_);
      }
      else
      {
        YYSYS_LOG(INFO,"set ocm exist succ!\n");
      }
    }
  }
  return err;
...
{
  int ret = OB_SUCCESS;
  //YYSYS_LOGGER.setFileName("ocm_admin.log", true);
  //YYSYS_LOG(INFO, "ocm_admin start!\n");
  YYSYS_LOG(INFO, "ocm_admin start");
  ob_init_memory_pool();
  ...
  {
    YYSYS_LOG(INFO,"fail to init client. ret = %d\n", ret);
  }
  else
  {
    ...
    else if(0 == strcmp("set_ocm_if_exist", clp.cmd_type))
    {
      ...
      YYSYS_LOG(INFO,"set ocm if exist ,err =%d\n", ret);
    }
  }
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(INFO,"do process fail. ret=%d\n", ret);
  }
  else
  {
    YYSYS_LOG(INFO,"do process succ!\n");
  }
  //client.destroy();
	return 0;
}
