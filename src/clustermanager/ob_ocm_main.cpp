...
int OcmMain::do_work()
{
  ...
  {
    YYSYS_LOG(ERROR, "ocm server start error, err=%d", ret);
  }
  return ret;
}
...
{
  YYSYS_LOG(INFO, "receive signal, sig=%d", sig);
  switch(sig)
  {
  case SIGTERM:
  case SIGINT:
    YYSYS_LOG(INFO, "start to stop the ocm server");
    if (NULL != ocm_server_)
    {
      ocm_server_->stop();
    }
    YYSYS_LOG(INFO, "the ocm server has stopped");
    break;
  default:
    break;
  }
