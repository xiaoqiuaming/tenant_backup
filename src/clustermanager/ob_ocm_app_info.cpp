...
namespace oceanbase
{
  namespace clustermanager
  {
    ...
    int ObOcmAppInfo::init(const char* app_name)
    {
      ...
      YYSYS_LOG(INFO, "magic_num=%ld", magic_num_);
      return err;
    }
    ...
    int ObOcmAppInfo::add_obi(ObOcmInstance &instance)
    {
      ...
      {
        YYSYS_LOG(WARN, "cannot add any instance more, obi_count_=%ld", obi_count_);
      ...
      else
      {
        ...
        for(; i < obi_count_; i++)
        {
          ...
          {
            YYSYS_LOG(WARN,"instance already exist.instance_name=%s", obi_[i].get_instance_name());
            err = OB_ENTRY_EXIST;
            break;
          }
        }
    ...
    int ObOcmAppInfo::add_register_node(const ObServerExt &node, const char* instance_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, instance_name=%s", instance_name);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if(obi_count_ == i)
        {
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "fail to init the register node. err=%d", err);
          }
    ...
    int ObOcmAppInfo::transfer_master(const ObServerExt &new_master, const char* instance_name)
    {
      ...
      else
      {
        ...
        if(obi_count_ == i)
        {
          YYSYS_LOG(ERROR, "the instance not exitst. instance_name=%s", instance_name);
    ...
    int ObOcmAppInfo::deep_copy(const ObOcmAppInfo &app_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, app_name_len =%ld", app_name_len);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        for(int i = 0; i < obi_count_; i++)
        {
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN ,"deep copy fail.i=%d", i);
            break;
          }
        }
      }
    ...
    const ObOcmInstance* ObOcmAppInfo::get_obi(int64_t index) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%ld, OB_MAX_OBI_COUNT=%ld", index, OB_MAX_OBI_COUNT);
        return NULL;
      }
      else
      {
        if(index >= obi_count_)
        {
          YYSYS_LOG(WARN, "this obi have no entity");
        }
        return (obi_ + index);
      }
    }
  ...
  int ObOcmAppInfo::set_instance_role(const char* instance_name, const Status status)
  {
    ...
    int ObOcmAppInfo::get_app_master_rs(const char *app_name, ObServerExt &serv_info) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, app_name=%s, app_name_=%s", app_name, app_name_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObOcmAppInfo::get_instance_master_rs_list(const char *app_name, ObServList &serv_list) const
    {
      ...
      if(0 != strcmp(app_name, app_name_))
      {
        YYSYS_LOG(WARN, "invalid param, app_name=%p, app_name_=%p", app_name, app_name_);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        for(int i = 0; i < obi_count_ ; i++)
        {
          ...
          if(err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "get rs list fail, obi_name=%p", obi_[i].get_instance_name());
          }
        }
      }
      return err;
    ...
    int ObOcmAppInfo::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      else
      {
        ...
        { 
          ...
          YYSYS_LOG(WARN, "wrong magic num, can't deserilize the buffer to ObOcmAppInfo, err=%d", err);       
        }
        else                                                                                                   
        { 
          pos += sizeof(int64_t);
