...
namespace oceanbase
{
  namespace clustermanager
  {
    ObOcmMeta::ObOcmMeta()
    {
      ...
      if(NULL == app_)
      {
        YYSYS_LOG(WARN,"init ObOcmMeta fail");
      }
      else
      {
        init_ = true;
      ...
      YYSYS_LOG(INFO, "magic num is %ld", magic_num_);
    }
    ...
    int ObOcmMeta::set_ocm_server(char *host_name, const common::ObServer server)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, hname=%s", host_name);
    ...
    {
      yysys::CRLockGuard guard(ocm_rwlock_);
      if(index >= app_count_)
      {
        return NULL;
      }
    ...
    {
      int err = OB_SUCCESS;
      // yysys::CWLockGuard guard(ocm_rwlock_);
      if(OB_MAX_APP_COUNT <= app_count_ || init_ == false)
      {
        YYSYS_LOG(INFO, "inner stat err, app_count_=%ld, init=%d", app_count_, init_);
      ...
      else
      {
        ...
        for(; i < app_count_; i++)
        {
          ...
          {
            YYSYS_LOG(WARN,"app already exist.app_name=%s", app_[i].get_app_name());
            err = OB_ENTRY_EXIST;
            break;
          }
        }
    ...
    int ObOcmMeta::add_register_node(const ObServerExt &node, const char* instance_name, const char* app_name)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, instance_name=%s, app_name=%s", instance_name, app_name);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        for(; i < app_count_ ; i++)
        {
          if(0 == strcmp(app_name, app_[i].get_app_name()))
          {
            ...
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "add register node to app fail. app_name=%s", app_name);
            }
            break;            
          }       
        }
        ...
        if(i == app_count_)
        {
          ...
          if(NULL == app)
          {
            YYSYS_LOG(WARN, "fail to new ObOcmAppInfo, app=%p", app);
          ...
          else
          {
            ...
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "fail to add app. err=%d", err);
            }
    ...
    int ObOcmMeta::set_location(const char * location)
    {
      ...
      {
        YYSYS_LOG(WARN,"invalid param, location=%s", location);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
      ...
      else
      {
        ...
        for(; i < app_count_; i++)
        {
          if(0 == strcmp(app_name, app_[i].get_app_name()))
          {
            ...
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "fail to transfer master, err=%d", err);
            }
            break;
          }
        }
        if(app_count_ == i)
        {
          YYSYS_LOG(ERROR, "can't find the app, app_name=%s", app_name);
    ...
    int ObOcmMeta::get_app(char * app_name, ObOcmAppInfo* &app)
    {
      ...
      {
        YYSYS_LOG(WARN,"invalid param,app_=%p,app_name=%s", app_, app_name);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        ...
        if( app_count_ == i)
        {
          YYSYS_LOG(INFO, "the app not exist.");
          err = OB_ITER_END;
        }
      }
      return err;
    ...
    int ObOcmMeta::get_app_master_rs(const char *app_name, ObServerExt &serv_info) const
    {
      ...
      {
        YYSYS_LOG(WARN,"invalid param,app_name=%s", app_name);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
    ...
    int ObOcmMeta::get_instance_master_rs(const char *app_name, const char *instance_name, ObServerExt &serv_info) const
    {
      ...
      if(NULL == app_name || NULL == instance_name
          ...
          || static_cast<int64_t>(strlen(instance_name)) >= OB_MAX_INSTANCE_NAME_LENGTH)
      {
        YYSYS_LOG(WARN,"invalid param, app_name=%s,instance_name=%s",
      ...
      else
      {
        ...
        for(int i = 0; i < app_count_; i++)
        {
          if(0 == strcmp(app_[i].get_app_name(), app_name))
          {
            ...
            if(err != OB_SUCCESS)
            {
              YYSYS_LOG(WARN, "get rs list fail. i = %d", i);
              break;
            }
          }
        }
    ...
    int ObOcmMeta::serialize(char* buf, const int64_t buf_len, int64_t& pos) const
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "buf is not enough, pos=%ld, buf_len=%ld", pos, buf_len);
          err = OB_ERROR;
        }
        else
        {
          ...
          *(reinterpret_cast<int64_t*> (buf + pos)) = str_len ;
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"ocm_server serialize fail");
          }
          else
          {
            *(reinterpret_cast<int64_t*> (buf + pos)) = app_count_;
            ...
            for(int i = 0; i < app_count_; i++)
            {
              ...
              if(OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "serialize the obi failed. i=%d, err=%d", i, err);
                break;
              }
            }
          }
    ...
    int ObOcmMeta::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      else
      {
        ...
        { 
          ...
          YYSYS_LOG(WARN, "wrong magic num, can't deserilize the buffer to ObOcmMeta, err=%d", err);       
        }
        else                                                                                                   
        { 
          pos += sizeof(int64_t);
          ...
          if(OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "ocm_server deserialize fail");
          }
          else
          {
            ...
            for(int i = 0; i < app_count_; i++)
            {
              ...
              if(OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN, "obinstance deserialize failed. i=%d, err=%d", i, err);
                break;
              }
            }
          }
