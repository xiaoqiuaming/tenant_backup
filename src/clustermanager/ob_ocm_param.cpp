...
namespace oceanbase
{
        ...
        const char* section, const char* name, bool not_null)
  {
    ...
    int ObOcmParam::load_from_config()
    {
      ...
      port_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_PORT, 0);
      if(port_ <= 0)
      {
        YYSYS_LOG(ERROR, "ocm server port (%ld) cannot <= 0.", port_);
        err = OB_ERROR;
      }
    ...
    if(OB_SUCCESS == err)
    {
      ...
      if(OB_SUCCESS == err)
      {
        log_size_mb_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_LOG_SIZE_MB,0);
      }
    ...
    /* if(OB_SUCCESS == err)
     ...
     if(OB_SUCCESS == err)
     {
       lease_on_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_LEASE_ON, 0); 
     }

     if(OB_SUCCESS == err)
     {
       lease_reserved_time_rs_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_EASE_RESERVED_TIME_RS, 0);
     }

     if(OB_SUCCESS == err)
     {
       lease_interval_rs_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_LEASE_INTERVAL_RS, 0);
     }

     if(OB_SUCCESS == err)
     {
       replay_wait_time_rs_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_REPLAY_WAIT_TIME_RS, 0);
     }

     if(OB_SUCCESS == err)
     {
       state_check_period_rs_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_STATE_CHECK_PERIOD_RS, 0);
     }

     if(OB_SUCCESS == err)
     {
       log_sync_limit_kb_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_LOG_SYNC_LIMIT_KB, 0);
     }

     if(OB_SUCCESS == err)
     {
       register_times_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_REGISTER_TIMES, 0);
     }

     if(OB_SUCCESS == err)
     {
       register_timeout_rs_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_REGISTER_TIMEOUT_RS, 0);
     }

     if(OB_SUCCESS == err)
     {
       cluster_manager_count_ = YYSYS_CONFIG.getInt(OBCM_SECTION, OBCM_CLUSTER_MANAGER_COUNT, 0);
     }
     ...
     if(OB_SUCCESS == err)
     {
       ...
       if(NULL == ocm_addr_array_)
       {
         YYSYS_LOG(WARN, "new ObclusterInfo array fail.");
       ...
       else
       {
         ...
         for(; (i < cluster_manager_count_) && (OB_SUCCESS == err); i++)
         {
          ...
          cluster_port_str[port_len + 1] = '\0';
           ...
           if(OB_SUCCESS == err)
           {  
             ocm_addr_array_[i].port = YYSYS_CONFIG.getInt(OBCM_SECTION, cluster_port_str, 0);
           }
  ...
  int ObOcmParam::set_ocm_status(const int64_t index, const bool is_exist)
  {
    ...
    {
      YYSYS_LOG(WARN, "invalid argument.index=%ld, cluster_manager_count_=%ld", index, cluster_manager_count_);
  ...
  int ObOcmParam::set_ocm_status(const ObString ocm_ip_addr, const bool is_exist)
  {
    ...
    if(NULL == ocm_addr_array_)
    {
      YYSYS_LOG(WARN, "ocm_addr_array not init.ocm_addr_array_=%p", ocm_addr_array_);
      err = OB_NOT_INIT;
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN, "find ocm fail.ocm_ip_addr=%s", ocm_ip_addr.ptr());
