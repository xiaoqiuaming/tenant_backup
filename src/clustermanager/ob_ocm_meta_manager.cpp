  ...
  namespace clustermanager
  {
    ...
    int ObOcmMetaManager::init_ocm_list(const char* ocm_location, const ObServerExt &ocm_server)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "invalid param ,ocm_location=%s", ocm_location);
        ...
        else
        {
          ...
          if(pmeta == NULL)
          {
            YYSYS_LOG(WARN, "fail to init ocm list, pmeta=%p", pmeta);
    ...
    {
      int err = OB_SUCCESS;
      yysys::CRLockGuard guard(ocm_list_rwlock_);
        ...
        else
        {
          ...
          *(reinterpret_cast<int64_t*> (buf + pos)) = ocm_count_;
          ...
          for(it = ocm_list_.begin(); it != ocm_list_.end(); it++)
          {
            ...
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "ocm list serialize fail, err= %d. ocm_location=%s", err, (*it).get_location());
              break;
            }
          }
        }
    ...
    int ObOcmMetaManager::deserialize(const char* buf, const int64_t buf_len, int64_t& pos)
    {
      ...
      if (NULL == buf || buf_len <= 0 || pos >= buf_len)
      {
        YYSYS_LOG(WARN, "invalid param, buf=%s, buf_len=%ld, pos=%ld", buf, buf_len, pos);
        err = OB_INVALID_ARGUMENT;
      }
      else
      {
        yysys::CWLockGuard guard(ocm_list_rwlock_);
        int64_t magic_num = 0;
        magic_num = *(reinterpret_cast <const int64_t*>(buf + pos));                                           
        if(magic_num != magic_num_)                                                                            
        { 
          err = OB_NOT_THE_OBJECT;
          YYSYS_LOG(WARN, "wrong magic num, can't deserilize the buffer to ObOcmInstance, err=%d", err);       
        }
        else                                                                                                   
        { 
          pos += sizeof(int64_t);
          ...
          for(int i = 0; i < ocm_count_ ; i++)
          {
            ...
            if(OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "ocm list deserialize fail. i=%d, err=%d", i, err);
              break;
            }
    ...
    int ObOcmMetaManager::get_instance_master_rs_list(const char *app_name, ObServList &serv_list) const
    {
      ...
      yysys::CRLockGuard guard(ocm_list_rwlock_);
        ...
        while(ocm_list_.end() != it)
        {
          ...
          if(err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN,"get rs list fail, ocm_location=%s", it->get_location());
            break;
          }
          it ++;
        }
  ...
  OcmList::iterator& ObOcmMetaManager::get_self()
    ...
    int ObOcmMetaManager::add_ocm_info(const ObOcmMeta &ocm_meta)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "add ocm fail. ocm_location=%s", ocm_meta.get_location());
        }
      }
      return err;
    }
