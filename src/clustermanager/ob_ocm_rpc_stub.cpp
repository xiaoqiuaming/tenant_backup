...
int ObOcmRpcStub::init(ThreadSpecificBuffer *rpc_buffer, ObClientManager *rpc_frame)
{
  ...
  {
    YYSYS_LOG(ERROR, "already inited or check input failed:inited[%s], "
...
int ObOcmRpcStub::broad_cast_ocm_info(const int64_t timeout, const common::ObServer &ocm_server, const ObOcmMeta &ocm_meta) const
{
  ...
  if(OB_SUCCESS == err)
  {
	...
	if(OB_SUCCESS != err)
	{
	  YYSYS_LOG(WARN, "serialize ocm meta fail. err=%d", err);
	}
  }
  if(OB_SUCCESS == err)
  {
    ...
    ocm_server.to_string(ip_addr, OB_IP_STR_BUFF);
	  YYSYS_LOG(WARN, "send request to ocm server fail.ip=%s, err=%d", ip_addr, err);
	}
  }
  int64_t pos = 0;
  if(OB_SUCCESS == err)
  {
	...
	if(OB_SUCCESS != err)
	{
	  YYSYS_LOG(ERROR, "deserialize resule_code failed, err=%d", err);
	}
	else
	{
