...
namespace oceanbase
{
  namespace clustermanager
  {
    ...
    class OcmService: public yynet::IPacketQueueHandler
    {
      public:
        OcmService();
        virtual ~OcmService();
      ...
      public:
        int init(ObOcmParam *ocm_param, yynet::Transport* transport, yynet::IPacketStreamer* streamer);
        ...
        virtual bool handlePacketQueue(yynet::Packet *packet, void *args);
		
        // on_xxx, packet handler functions
        int on_no_matching_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, 
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        int on_get_instance_role_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        int on_set_instance_role_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        int on_update_ocm_list_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        int on_ocm_change_status_handler(const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn,
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
        int broad_cast_ocm_info();							   
      private:
        int dispatch(int pcode, const int32_t version, common::ObDataBuffer& in_buff, yynet::Connection* conn, 
            const uint32_t channel_id, common::ObDataBuffer& out_buff);
      private:
        bool init_;
