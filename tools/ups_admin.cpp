...
void priv_queue_conf(const char* conf_str, MockClient &client)
{
  ...
  memset(&priv_queue_conf, 0, sizeof(priv_queue_conf));
...
void get_clog_status(MockClient &client)
{
  ...
  YYSYS_LOG(ERROR, "timeout=%ld", timeout);
  if (OB_SUCCESS != (err = client.get_clog_status(stat, timeout)))
  {
    YYSYS_LOG(ERROR, "client.get_clog_status()=>%d", err);
  }
...
void get_max_log_seq(MockClient &client)
{
  ...
  {
    YYSYS_LOG(ERROR, "client.get_max_log_id_replayable()=>%d", err);
  }
  fprintf(stdout, "[%s] err=%d\n", __FUNCTION__, err);
...
void get_clog_cursor(MockClient &client)
{
  ...
  {
    YYSYS_LOG(ERROR, "client.get_max_clog_id()=>%d", err);
  }
  fprintf(stdout, "[%s] err=%d\n", __FUNCTION__, err);
...
//mod peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150611:b
...
void print_usage()
{
  ...
  fprintf(stdout, "   -t|--cmd_type command type[apply|login_apply|get|param_get|get_clog_status|get_max_log_seq|get_clog_cursor|get_clog_master|get_log_sync_delay_stat|scan|total_scan|fetch_schema|drop|dump_memtable|dump_schemas|force_fetch_schema|"
          "reload_conf|memory_watch|memory_limit|priv_queue_conf|clear_active_memtable|get_last_frozen_version|fetch_ups_stat_info|get_bloomfilter|"
          "store_memtable|erase_sstable|load_new_store|reload_all_store|reload_store|umount_store|force_report_frozen_version|switch_commit_log|get_table_time_stamp|print_scanner|print_schema|"
          "enable_memtable_checksum|disable_memtable_checksum|get_slave_ups_info|"
          "delay_drop_memtable|immediately_drop_memtable|minor_load_bypass|major_load_bypass|change_log_level|list_sessions|kill_session|get_sstable_range_list|sql_query|ups_show_sessions|ups_kill_session]\n");
    ...
    case 'i':
      {
        ...
        fprintf(stdout, "ip=%ld str=%s\n", ip, yysys::CNetUtil::addrToString(ip).c_str());
        exit(1);
      }
    case 'e':
...
void build_cmd_map(std::map<std::string, std::string> &cmd_map)
{
  cmd_map.insert(std::map<std::string, std::string>::value_type("apply", "apply -f ini_file_name"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("login_apply", "login_apply"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get", "get -f ini_file_name|-r version_range"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("param_get", "param_get -f ini_file_name"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_clog_status", "get_clog_status"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_max_log_seq", "get_max_log_seq"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_clog_cursor", "get_clog_cursor"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_clog_master", "get_clog_master"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_log_sync_delay_stat", "get_log_sync_delay_stat"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("scan", "scan -f ini_file_name|-r version_range"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("total_scan", "total_scan -f ini_file_name|-r version_range"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("fetch_schema", "fetch_schema -s timestamp"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("drop", "drop"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_memtable", "dump_memtable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_schemas", "dump_schemas"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("force_fetch_schema", "force_fetch_schema"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("reload_conf", "reload_conf"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("memory_watch", "memory_watch"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("memory_limit", "memory_limit"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("priv_queue_conf", "priv_queue_conf"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("clear_active_memtable", "clear_active_memtable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_last_frozen_version", "get_last_frozen_version"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("fetch_ups_stat_info", "fetch_ups_stat_info"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_bloomfilter", "get_bloomfilter -s timestamp"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("store_memtable", "store_memtable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("erase_sstable", "erase_sstable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("load_new_store", "load_new_store"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("reload_all_store", "reload_all_store"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("reload_store", "reload_store -s timestamp"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("umount_store", "umount_store -f ini_file_name"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("force_report_frozen_version", "force_report_frozen_version"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("switch_commit_log", "switch_commit_log"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_table_time_stamp", "get_table_time_stamp -s timestamp"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("print_scanner", "print_scanner -f ini_file_name"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("print_schema", "print_schema -f ini_file_name"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("enable_memtable_checksum", "enable_memtable_checksum"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("disable_memtable_checksum", "disable_memtable_checksum"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_slave_ups_info", "get_slave_ups_info"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("delay_drop_memtable", "delay_drop_memtable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("immediately_drop_memtable", "immediately_drop_memtable"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("minor_load_bypass", "minor_load_bypass"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("major_load_bypass", "major_load_bypass"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("change_log_level", "change_log_level -g DEBUG|WARN|INFO|ERROR"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("list_sessions", "list_sessions"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("kill_session", "kill_session -n session_id"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_sstable_range_list", "get_sstable_range_list -s timestamp -n session_id"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("sql_query", "sql_query -x sql_query_string"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("ups_show_sessions", "ups_show_sessions"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("ups_kill_session", "ups_kill_session -n session_id"));
}
...
void print_error_prompt(const char *cmd, const char *host, const int port)
{
  ...
  build_cmd_map(cmd_map);

  for (std::map<std::string, std::string>::iterator it = cmd_map.begin();
       it != cmd_map.end(); it++)
  {
    distance = get_edit_distance(cmd, (it->first).c_str());
  ...
  {
    std::map<std::string, std::string>::iterator cmd_it = cmd_map.find(it->second);
    if (cmd_it != cmd_map.end())
    {
      printf("bin/ups_admin -a %s -p %d -t %s\n", host, port, (cmd_it->second).c_str());
    }
    count++;
  }
  printf("\n");
...
{
  YYSYS_LOGGER.setFileName("ups_admin.log", true);
...
int main(int argc, char** argv)
{
  setlocale(LC_ALL, "");
//  YYSYS_LOGGER.setFileName("ups_admin.log", true);
  YYSYS_LOG(INFO, "ups_admin start==================================================");
  ob_init_memory_pool();
  ...
  //del peiouya [MultiUPS] [UPS_SYNC_Frozen] 20150611:b
  ...
  {
//    print_usage();
    print_error_prompt(clp.cmd_type, clp.serv_addr, clp.serv_port);
  }

  if (!clp.quickly_exit)
  {
    client.destroy();
    YYSYS_LOG(INFO, "ups_admin end==================================================");
    return rc;
  }
  else
  {
    YYSYS_LOG(INFO, "ups_admin killed==================================================");
    fflush(stdout);
    fflush(stderr);
    _exit(rc);
  }
