                                                          ...
                                                          const int16_t magic)
{
  ...
  {
    YYSYS_LOG(ERROR, "param error [data_buf:%p,size:%ld]",data_buf, size);
  ...
  if (OB_SUCCESS == err)
  {
    ...
    = static_cast<int32_t>(sizeof(ObSCRecordHeader)/sizeof(int16_t));
    ...
    {
      YYSYS_LOG(ERROR, "there is not enough space");
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && magic != header->magic_ )
    {
      YYSYS_LOG(ERROR, "magic number coincident [real:%d,exp:%d]",
                header->magic_,magic);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err
    ...
    {
      YYSYS_LOG(ERROR, "header length error [real:%d,exp:%lu]",
                header->header_length_, sizeof(ObSCRecordHeader));
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err
    ...
    {
      YYSYS_LOG(ERROR, "version error [real:%d,exp:%d]",
    ...
    if (OB_SUCCESS == err
        && 0 != header->reserved_)
    {
      YYSYS_LOG(ERROR, "reserved error [real:%ld,exp:%d]",
                header->reserved_,0);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && header->data_zlength_ <= 0)
    {
      YYSYS_LOG(ERROR, "data_zlength should be greater than 0 [real:%d]",
                header->data_zlength_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && header->data_length_ <= 0)
    {
      YYSYS_LOG(ERROR, "data_length should be greater than 0 [real:%d]",
                header->data_length_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(ERROR, "header checksum check fail");
        err = OB_ERROR;
      }
    }
    if (OB_SUCCESS == err
    ...
    {
      YYSYS_LOG(ERROR, "data checksum check fail [real:%ld,exp:%ld]",
                                                            ...
                                                            int64_t &pos)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "trailer_offset should be greater than 0 [real:%ld]", offset->trailer_offset_);
      err = OB_ERROR;
    }
                                                              ...
                                                              int64_t &pos)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_ERROR;
      YYSYS_LOG(ERROR, "fail to deserialize sstable trailer's record head");
    }
  }
  if (OB_SUCCESS == err
      && record_header->data_length_ > static_cast<int64_t>(sizeof(ObSCSSTableTrailer)))
  {
    YYSYS_LOG(ERROR, "space not enough for ObSCSSTableTrailer, data_length=%d, "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "version error [real:%d,exp:%d]", trailer->version_, TRAILER_VERSION3);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->block_count_ < 0 )
    {
      YYSYS_LOG(ERROR, "block_count_ should be greater than 0 [real:%ld]", trailer->block_count_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->block_index_record_offset_ < 0 )
    {
      YYSYS_LOG(ERROR, "block_index_record_offset_ should be greater than 0 [real:%ld]",
                trailer->block_index_record_offset_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->block_index_record_size_ < 0 )
    {
      YYSYS_LOG(ERROR, "block_index_record_size_ should be greater than 0 [real:%ld]",
                trailer->block_index_record_size_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->bloom_filter_hash_count_ < 0 )
    {
      YYSYS_LOG(ERROR, "bloom_filter_hash_count_ should be greater than 0 [real:%ld]",
                trailer->bloom_filter_hash_count_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->bloom_filter_record_offset_ < 0 )
    {
      YYSYS_LOG(ERROR, "bloom_filter_record_offset_ should be greater than 0 [real:%ld]",
                trailer->bloom_filter_record_offset_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->bloom_filter_record_size_ < 0 )
    {
      YYSYS_LOG(ERROR, "bloom_filter_record_size_ should be greater than 0 [real:%ld]",
                trailer->bloom_filter_record_size_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->blocksize_ < 0 )
    {
      YYSYS_LOG(ERROR, "blocksize_ should be greater than 0 [real:%ld]",
                trailer->blocksize_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->row_count_ < 0 )
    {
      YYSYS_LOG(ERROR, "row_count_ should be greater than 0 [real:%ld]",
                trailer->row_count_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->first_table_id_ == OB_INVALID_ID)
    {
      YYSYS_LOG(ERROR, "first_table_id_ shouldn't be OB_INVALID_ID [real:%lu]",
                trailer->first_table_id_);
      err  = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->frozen_time_ < 0 )
    {
      YYSYS_LOG(ERROR, "frozen_time_ should be greater than or equal 0 [real:%ld]",
        ...
        i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "reserved64_ should be 0 [real:%ld,index:%d]", trailer->reserved64_[i], i);
        err = OB_ERROR;
      }
    }
    ...
    {
      YYSYS_LOG(ERROR, "compressor_name_ error [real:%.3s,exp:%.3s]",
    ...
    {
      YYSYS_LOG(ERROR, "row_value_store_style_ error [real:%hd,exp:%hd]",
                trailer->row_value_store_style_, OB_SSTABLE_STORE_DENSE);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && trailer->reserved_ != 0)
    {
      YYSYS_LOG(ERROR, "reserved_ should be 0 [real:%d]", trailer->reserved_);
      err = OB_ERROR;
    }
  }
                                                 ...
                                                 int64_t &pos)
{
  ...
  {
    YYSYS_LOG(ERROR, "space not enough for ObSCTableSchemaHeader");
    err = OB_INVALID_ARGUMENT;
  }
  if ( OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "column_count_ should be greater than or equal to 0 [real:%hd]",
    ...
    {
      YYSYS_LOG(ERROR, "reserved16_ should be 0 [real:%hd]", schema.reserved16_);
      err = OB_ERROR;
    }
    pos += sizeof(int16_t);
    ...
    {
      YYSYS_LOG(ERROR, "total_column_count_ should be greater than or equal to 0 [real:%hd]",
    ...
    {
      YYSYS_LOG(ERROR, "space not enough");
      err = OB_ERROR;
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      for (int16_t i = 0; i < column_count && OB_SUCCESS == err; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "schema column type error [columnid:%u,type:%d]",
        ...
        {
          YYSYS_LOG(ERROR, "shema column id not increase [index:%hd,current_columnid:%u,"
        ...
        {
          YYSYS_LOG(ERROR, "column id should not be 1 [real:%u]",
                    schema.column_defs_[i].column_name_id_);
          err = OB_ERROR;
        }
        ...
        {
          YYSYS_LOG(ERROR, "column def[%d] column_group_id_ should not be OB_INVALID_ID [real:%u]",
        ...
        {
          YYSYS_LOG(ERROR, "column def table_id_ should not be OB_INVALID_ID [real:%u]",
                    schema.column_defs_[i].table_id_);
          err = OB_ERROR;
        }
                                                                                 ...
                                                                                 int64_t &pos)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "sstable_block_count_ should be greater than 0 [real:%ld]",
                block_index->sstable_block_count_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && block_index->end_key_char_stream_offset_ <= 0)
    {
      YYSYS_LOG(ERROR, "end_key_char_stream_offset_ should be greater than 0 [real:%ld]",
                block_index->sstable_block_count_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && block_index->rowkey_flag_ != 1)
    {
      YYSYS_LOG(ERROR, "rowkey_flag_ should be 1[real:%d]",
                block_index->rowkey_flag_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && block_index->reserved16_ != 0)
    {
      YYSYS_LOG(ERROR, "reserved16_ should be 0 [real:%d]",  block_index->reserved16_);
      err = OB_ERROR;
    }
    for (uint32_t i = 0;
        ...
        i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "reserved64_ shoul be 0 [real:%ld,index:%u]",
    ...
    if (OB_SUCCESS == err)
    {
      ...
      for (int32_t i = 0; i < block_index->sstable_block_count_ && OB_SUCCESS == err; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "block index resreved16_ should be 0 [real:%d]",
        ...
        {
          YYSYS_LOG(ERROR, "block index column_group_id_ should not be OB_INVALID_ID [real:%u]",
        ...
        if (OB_SUCCESS == err && block_index->block_info_array_[i].table_id_
            == (uint32_t)(OB_INVALID_ID & 0xFFFFFFFF))
        {
          YYSYS_LOG(ERROR, "block index table_id_ should not be OB_INVALID_ID [real:%u]",
                    block_index->block_info_array_[i].table_id_);
          err = OB_ERROR;
        }
        if (OB_SUCCESS == err && block_index->block_info_array_[i].block_record_size_ <= 0)
        {
          YYSYS_LOG(ERROR, "block_record_size_ should be greater than 0 [real:%d,index:%d]",
        ...
        {
          YYSYS_LOG(ERROR, "block_end_key_size_ should be greater than 0 [real:%d,index:%d]",
                                                                       ...
                                                                       const char* compressor_name)
{
  ...
  if (OB_SUCCESS == err && record_header->data_length_ > record_header->data_zlength_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to allocate memory for uncompress buffer");
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err
        && NULL == (compressor = create_compressor(compressor_name)))
    {
      YYSYS_LOG(ERROR, "failed to create compressor");
      err = OB_ERROR;
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to compress block data");
        err = OB_ERROR;
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "row_index_array_offset_ should be greater than 0 [real:%d]",
                block->row_index_array_offset_);
      err = OB_ERROR;
    }
    if (OB_SUCCESS == err && block->row_count_ <= 0)
    {
      YYSYS_LOG(ERROR, "row_count_ should be greater than 0 [real:%d]",
                block->row_count_);
      err = OB_ERROR;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "block space not enough [need_size:%ld,actual_size:%d]",
  ...
  if (OB_SUCCESS == err)
  {
        ...
        rowid ++)
    {
      ...
      {
        YYSYS_LOG(WARN, "record overlapped with previous [current_rowid:%ld]", rowid);
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err && row_index_array[rowid] >= record_header->data_length_)
      {
        YYSYS_LOG(WARN, "record out of block area [rowid:%ld]", rowid);
        err = OB_ERROR;
      }
...
int oceanbase::chunkserver::ObSCSSTableChecker::check(const char *sstable_fname)
{
  ...
  {
    YYSYS_LOG(WARN, "param error [sstable_fname:%p]",sstable_fname);
    err = OB_INVALID_ARGUMENT;
  }
  if (OB_SUCCESS == err && (sstable_fd = open(sstable_fname, O_RDONLY)) < 0)
  {
    YYSYS_LOG(WARN, "fail to open disk file for read [fname:%s,errno:%d]",
              sstable_fname, errno);
    err = OB_ERR_SYS;
  }
  if (OB_SUCCESS == err && fstat(sstable_fd, &sstable_stat) < 0)
  {
    YYSYS_LOG(WARN, "fail to stat disk file [fname:%s,errno:%d]", sstable_fname, errno);
    err = OB_ERR_SYS;
  }
  ...
  if ( OB_SUCCESS == err )
  {
    ...
    {
      YYSYS_LOG( WARN, "fail to read trailer offset [offset:%ld,need_size:%ld,"
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (offset == NULL)
      {
        YYSYS_LOG( WARN, "fail to deserialize trailer offset" );
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err && pos != need_size)
      {
        YYSYS_LOG( ERROR, "internal error");
        err = OB_ERROR;
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG( WARN, "fail to allocate memory for trailer" );
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (readed_size != need_size)
      {
        YYSYS_LOG( WARN, "fail to read trailer [offset:%ld,need_size:%ld,"
                   "readed_size:%ld,errno:%d]", read_offset, need_size, readed_size, errno );
        err = OB_ERR_SYS;
      }
    }
    if (OB_SUCCESS == err)
    {
      ...
      if (NULL == trailer_ )
      {
        YYSYS_LOG( WARN, "fail to deserilize trailer" );
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err && pos != need_size)
      {
        YYSYS_LOG( ERROR, "check trailer internal error, pos=%ld, need_size=%ld", pos, need_size);
        err = OB_ERROR;
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to allocate memory for schema");
      err = OB_ALLOCATE_MEMORY_FAILED;
    }
    if (OB_SUCCESS == err)
    {
      ...
      if (readed_size != need_size)
      {
        YYSYS_LOG( WARN, "fail to read schema [offset:%ld,need_size:%ld,"
                   "readed_size:%ld,errno:%d]", read_offset, need_size, readed_size, errno );
        err = OB_ERR_SYS;
      }
    }
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "fail to deserialize schema");
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err && pos != need_size)
      {
        YYSYS_LOG( ERROR, "internal error, pos=%ld, need_size=%ld", pos, need_size);
        err = OB_ERROR;
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (need_size > 0)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to allocate memory for bloom filter");
        err = OB_ALLOCATE_MEMORY_FAILED;
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (readed_size != need_size)
        {
          YYSYS_LOG( WARN, "fail to read bloom filter [offset:%ld,need_size:%ld,"
                     "readed_size:%ld,errno:%d]", read_offset, need_size, readed_size, errno );
          err = OB_ERR_SYS;
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to deserialize bloom filter");
          err = OB_ERROR;
        }
        if (OB_SUCCESS == err && pos != need_size)
        {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(ERROR, "range_offset:%ld, range_size:%ld is illegal",
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to malloc range_buffer_");
      ...
      {
        YYSYS_LOG(WARN, "fail to read range [offset:%ld,need_size:%ld,"
      ...
      {
        err = OB_ERROR;
        YYSYS_LOG(ERROR, "fail to deserialize sstable key_stream:'s record head");
      }
      ...
      if (OB_SUCCESS == ret)
      {
        if (record_header->data_length_ == record_header->data_zlength_)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to deserialize range: payload_ptr %p,"
        ...
        else
        {
          ...
          YYSYS_LOG(ERROR, "range is in compressed format, "
  ...
  if (OB_SUCCESS == err && trailer_->block_index_record_size_ != 0)
  {
    ...
    {
      YYSYS_LOG(WARN, "fail to allocate memory for block index");
      err = OB_ALLOCATE_MEMORY_FAILED;
    }
    if (OB_SUCCESS == err)
    {
      ...
      if (readed_size != need_size)
      {
        YYSYS_LOG( WARN, "fail to read block index [offset:%ld,need_size:%ld,"
                   "readed_size:%ld,errno:%d]", read_offset, need_size, readed_size, errno );
        err = OB_ERR_SYS;
      }
    }
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to deserialize block index");
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err && pos != need_size)
      {
  ...
  {
    YYSYS_LOG(WARN, "block count in block index and trailer not coincident "
  ...
  if (OB_SUCCESS == err && trailer_->block_index_record_size_ != 0)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to check rowkey_order, ret=%d", err);
    }
  }
  if (sstable_fd >= 0)
  {
                                                          ...
                                                          common::ObRowkey& row_key, const int32_t column_count)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to deserialize row key,ret=%d", ret);
  }
  else
  {
    for(int32_t i = static_cast<int32_t>(row_key.get_obj_cnt()); i<column_count && OB_SUCCESS == ret; ++i)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to deserialize obj[%d] row_len=%ld pos=%ld ret=%d",
            i, row_len, pos, ret);
      }
    }
  }
...
int oceanbase::chunkserver::ObSCSSTableChecker::check_rowkey_order(const int sstable_fd)
{
  ...
  for (int64_t blockid = 0;
      ...
      blockid ++)
  {
    YYSYS_LOG(DEBUG, "begin to check block [blockid:%ld]", blockid);
    ...
    if (0 != err)
    {
      YYSYS_LOG(ERROR, "failed to deserialize end key of cur block, blockid=%ld, ret=%d", blockid,err);
    }
    ...
    {
      err = OB_ALLOCATE_MEMORY_FAILED;
      YYSYS_LOG(WARN, "fail to allocate memory for block");
    }
    ...
    if ( OB_SUCCESS == err )
    {
      ...
      {
        err = OB_ERR_SYS;
        YYSYS_LOG(WARN, "read block [blockid:%ld,block_size:%ld,readed_size:%ld]",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (NULL == block)
        {
          YYSYS_LOG(WARN, "fail to deserialize block [blockid:%ld]", blockid);
          err = OB_ERROR;
        }
        if (OB_SUCCESS == err && pos != block_size)
        {
          YYSYS_LOG(WARN, "internal error");
          err = OB_ERROR;
        }
      }
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      for (int64_t row_id = 0; row_id < block->row_count_ && OB_SUCCESS == err; row_id ++)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to check row, ret=%d", err);
        }
        ...
        {
          YYSYS_LOG(WARN, "failed to ensure space for bloom filter key buf, ret=%d", err);
        }
        ...
        {
          YYSYS_LOG(WARN, "failed to encode 0 of bloom filter, ret=%d", err);
        }
        else if (OB_SUCCESS !=
            (err = serialization::encode_i16(bf_key_buf, bf_key_size, pos, static_cast<int16_t>(cur_group_id))))
        {
          YYSYS_LOG(WARN, "failed to encode cur_groupd_id[%lu] of bloom filter, ret=%d", cur_group_id, err);
        }
        else if (OB_SUCCESS !=
            (err = serialization::encode_i32(bf_key_buf, bf_key_size, pos, static_cast<int32_t>(cur_table_id))))
        {
          YYSYS_LOG(WARN, "failed to encode cur_table_id[%lu] of bloom filter, ret=%d", cur_table_id, err);
        }
        else if (OB_SUCCESS !=
        ...
        {
          YYSYS_LOG(WARN, "bloom filter check fail [rowid:%ld], %s",row_id, to_cstring(row_key));
          err = OB_ERROR;
        }
        */
        ...
        {
          YYSYS_LOG(WARN, "current row key isn't bigger than previous one "
      ...
      if (OB_SUCCESS == err && (cur_table_id != prev_table_id
             ...
             || row_key != end_row_key))
      {
        YYSYS_LOG(WARN, "block's end key not coincident with block index [blockid:%ld],"
                        "cur_table_id=%lu, prev_table_id=%lu, "
                        "cur_group_id=lu, prev_group_id=%lu, "
  ...
  {
    YYSYS_LOG(WARN, "row count not coincident [total_row_count:%ld,trailer.row_count:%ld]",
...
int  oceanbase::chunkserver::ObSCSSTableChecker::check_offset_size(const int64_t sstable_file_size)
{
  ...
  if (trailer_->block_index_record_size_ != 0)
  {
        ...
        block_id ++)
    {
      ...
      {
        YYSYS_LOG(WARN, "sstable file size not enough [block_id:%ld,need_size:%ld,actual_size:%ld]",
  ...
  {
    YYSYS_LOG(INFO, "empty sstable, skip check block offset");
  }
  /// block index
  if (OB_SUCCESS == err && cur_offset > trailer_->block_index_record_offset_)
  {
    YYSYS_LOG(WARN, "content overlapped block and block index, "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (cur_offset > sstable_file_size)
    {
      YYSYS_LOG(WARN, "sstable file size not enough [block_index,need_size:%ld,actual_size:%ld]",
  ...
  {
    YYSYS_LOG(WARN, "content overlapped block index and bloom filter, "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (cur_offset > sstable_file_size)
    {
      YYSYS_LOG(WARN, "sstable file size not enough [bloom_filter,need_size:%ld,actual_size:%ld]",
                cur_offset, sstable_file_size);
      err = OB_ERROR;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "content overlapped bloom filter and schema, "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (cur_offset > sstable_file_size)
    {
      YYSYS_LOG(WARN, "sstable file size not enough [schema,need_size:%ld,actual_size:%ld]",
                cur_offset, sstable_file_size);
      err = OB_ERROR;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "content overlapped schema and trailer, "
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (cur_offset > sstable_file_size)
    {
      YYSYS_LOG(WARN, "sstable file size not enough [trailer,need_size:%ld,actual_size:%ld]",
                cur_offset, sstable_file_size);
      err = OB_ERROR;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "content overlapped trailer and trailer offset, "
...
#endif
  YYSYS_LOG(INFO, "range: %s", to_cstring(range_));
}
...
int main( int argc , char ** argv)
{
  ...
  for (int i = 1; i < argc && 0 == result; i++ )
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to new ObSCSSTableChecker");
    }
    if (OB_SUCCESS != result)
    {
      YYSYS_LOG(WARN, "%s check fail", argv[i]);
    }
    else
    {
      YYSYS_LOG(INFO, "%s check success", argv[i]);
    }
  }
  return 0;
}
