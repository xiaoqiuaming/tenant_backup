...
{ -1, 1, CB_DECIMAL_POS, 0, 0, 0, NULL, const_one_data } ;
...
void _decimal_set_nan( bson_decimal *decimal )
{
  ...
  decimal->ndigits = 0 ;
  decimal->weight  = 0 ;
  decimal->sign    = CB_DECIMAL_SPECIAL_SIGN ;
  decimal->dscale  = CB_DECIMAL_SPECIAL_NAN ;
}
  ...
  else
  {
    ...
    if ( ndigits <= decimal->ndigits )
    {
      ...
      if (di > 0)
      {
        ...
        digits[ndigits] = static_cast<short>(digits[ndigits] - static_cast<short>(extra)) ;
      }
    }
  }
}
                      ...
                      bson_decimal *result )
{
  ...
  res_buf = (short int*)bson_malloc( ( res_ndigits + 1 ) * static_cast<int>(sizeof( short )) ) ;
  if ( NULL == res_buf )
  {
    rc = -2 ;
    goto error ;
  ...
  for ( i = res_ndigits - 1 ; i >= 0 ; i-- )
  {
    ...
    if ( borrow < 0 )
    {
      res_digits[i] = static_cast<short>(borrow + DECIMAL_NBASE) ;
      borrow = -1 ;
    }
    else
    {
      res_digits[i] = static_cast<short>(borrow) ;
      borrow = 0 ;
    }
  }
                      ...
                      bson_decimal *result )
{
  ...
  for ( i = res_ndigits - 1 ; i >= 0 ; i-- )
  {
    ...
    {
      res_digits[i] = static_cast<short>(carry - DECIMAL_NBASE) ;
      carry = 1 ;
    }
    else
    {
      res_digits[i] = static_cast<short>(carry) ;
      carry = 0;
    }
  }
                  ...
                  bson_decimal *result )
{
  ...
  /*
    ...
    */
  if ( left->sign == CB_DECIMAL_POS )
  {
    if ( right->sign == CB_DECIMAL_NEG )
    {
      /* ----------
                  ...
                  bson_decimal *result, int rscale )
{
  ...
  {
    res_sign = CB_DECIMAL_POS ;
  }
  else
  {
    res_sign = CB_DECIMAL_NEG ;
  }
  ...
  /*
    ...
    */
  dig = (int *) bson_malloc( res_ndigits * static_cast<int>(sizeof( int )) ) ;
  if ( NULL == dig )
  {
    rc = -2 ;
    goto error ;
  ...
  for ( i = res_ndigits - 1 ; i >= 0 ; i-- )
  {
    ...
    res_digits[i] = static_cast<short>(newdig) ;
  }
                  ...
                  bson_decimal *result, int rscale, int isRound )
{
    ...
    */
  {
    ...
    dividend = ( short * )bson_malloc( tmpDividendLen * static_cast<int>(sizeof( short )) ) ;
    if ( NULL == dividend )
    {
      rc = -2 ;
      goto error ;
  ...
  if ( rightNdigits == 1 )
  {
    ...
    for ( i = 0 ; i < res_ndigits ; i++ )
    {
      ...
      res_digits[i] = static_cast<short>(carry / divisor1) ;
  ...
  else
  {
    ...
    if ( divisor[1] < DECIMAL_HALF_NBASE )
    {
      ...
      {
        carry     += divisor[i] * d ;
        divisor[i] = static_cast<short>(carry % DECIMAL_NBASE) ;
      ...
      {
        carry      += dividend[i] * d ;
        dividend[i] = static_cast<short>(carry % DECIMAL_NBASE) ;
        carry       = carry / DECIMAL_NBASE ;
      }
    }
    ...
    for ( j = 0 ; j < res_ndigits ; j++ )
    {
      ...
      if ( qhat > 0 )
      {
        ...
        for ( i = rightNdigits ; i >= 0 ; i-- )
        {
          ...
          if ( borrow < 0 )
          {
            dividend[j + i] = static_cast<short>(borrow + DECIMAL_NBASE) ;
            borrow          = -1 ;
          }
          else
          {
            dividend[j + i] = static_cast<short>(borrow) ;
            borrow          = 0 ;
          }
        }
        ...
        if ( borrow )
        {
          ...
          for ( i = rightNdigits ; i >= 0 ; i-- )
          {
            ...
            if ( carry >= DECIMAL_NBASE )
            {
              dividend[j + i] = static_cast<short>(carry - DECIMAL_NBASE) ;
              carry           = 1 ;
            }
            else
            {
              dividend[j + i] = static_cast<short>(carry) ;
              carry           = 0 ;
            }
          }
      ...
      res_digits[j] = static_cast<short>(qhat) ;
    }
  }
...
{
  int tmpSize = 0 ;
  if ( CB_DECIMAL_SPECIAL_SIGN == sign && CB_DECIMAL_SPECIAL_NAN == scale )
  {
    return ( 3 + 1 ) ;   // "NAN" + 1
  }

  if ( CB_DECIMAL_SPECIAL_SIGN == sign && CB_DECIMAL_SPECIAL_MIN == scale )
  {
    return ( 3 + 1 ) ;   // "MIN" + 1
  }

  if ( CB_DECIMAL_SPECIAL_SIGN == sign && CB_DECIMAL_SPECIAL_MAX == scale )
  {
    return ( 3 + 1 ) ;   // "MAX" + 1
  }
...
int _decimal_alloc( bson_decimal *decimal, int ndigits )
{
  ...
  decimal->buff = (short int*)bson_malloc( (ndigits + 1) * static_cast<short>(sizeof(short) )) ;
...
int decimal_is_special( const bson_decimal *decimal )
{
  ...
  if ( decimal->sign == CB_DECIMAL_SPECIAL_SIGN )
  {
    return 1 ;
  }
...
void decimal_set_min( bson_decimal *decimal )
{
  ...
  decimal->weight  = 0 ;
  decimal->sign    = CB_DECIMAL_SPECIAL_SIGN ;
  decimal->dscale  = CB_DECIMAL_SPECIAL_MIN ;
}
...
void decimal_set_max( bson_decimal *decimal )
{
  ...
  decimal->weight  = 0 ;
  decimal->sign    = CB_DECIMAL_SPECIAL_SIGN ;
  decimal->dscale  = CB_DECIMAL_SPECIAL_MAX ;
}
         ...
         ( ndigits == decimal->ndigits && di > 0 ) )
    {
      ...
      else
      {
        ...
        if ( extra >= pow10/2 )
        {
          ...
          digits[ ndigits ] = static_cast<short>(pow10) ;
        }
      }
      ...
      while (carry)
      {
        ...
        if ( carry >= DECIMAL_NBASE )
        {
          digits[ndigits] = static_cast<short>(carry - DECIMAL_NBASE) ;
          carry = 1;
        }
        else
        {
          digits[ ndigits ] = static_cast<short>(carry);
          carry = 0;
        }
      }
...
int64_t decimal_to_long( const bson_decimal *decimal )
{
  ...
  neg    = ( rounded.sign == CB_DECIMAL_NEG ) ;
  val    = digits[0] ;
  for ( i = 1 ; i <= weight ; i++ )
  {
    oldval = val ;
...
double decimal_to_double( const bson_decimal *decimal )
{
  ...
  strValue = (char *)bson_malloc( size ) ;
  if ( NULL == strValue )
  {
    rc = -2 ;
    goto error ;
...
int decimal_to_str( const bson_decimal *decimal, char *value, int value_size )
{
   ...
   */
  if ( decimal->sign == CB_DECIMAL_NEG )
  {
    *cp++ = '-' ;
  }
  ...
  else
  {
    ...
    {
      dig = static_cast<short>((d < decimal->ndigits) ? decimal->digits[d] : 0 );
      /* In the first digit, suppress extra leading decimal zeroes */
      {
        int putit = ( d > 0 ) ;

        d1    =  dig / 1000 ;
        dig   = static_cast<short>(dig - static_cast<short>(d1 * 1000)) ;
        putit |= ( d1 > 0 ) ;
        if ( putit )
        {
          *cp++ = static_cast<char>(d1 + '0') ;
        }

        d1    = dig / 100 ;
        dig   = static_cast<short>(dig - static_cast<short>(d1 * 100)) ;
        putit |= ( d1 > 0 ) ;
        if ( putit )
        {
          *cp++ = static_cast<char>(d1 + '0') ;
        ...
        d1    = dig / 10 ;
        dig   = static_cast<short>(dig - static_cast<int>(d1 * 10)) ;
        putit |= (d1 > 0) ;
        if ( putit )
        {
          *cp++ = static_cast<char>(d1 + '0') ;
  ...
  if ( dscale > 0 )
  {
    *cp++ = '.' ;
    ...
    {
      dig = static_cast<short>((d >= 0 && d < decimal->ndigits) ? decimal->digits[d] : 0) ;
      d1  = dig / 1000 ;
      dig = static_cast<short>(dig - static_cast<short>(d1 * 1000)) ;
      *cp++ = static_cast<char>(d1 + '0') ;
      ...
      *cp++ = static_cast<char>(d1 + '0') ;
      ...
      d1  = dig / 10 ;
      dig = static_cast<short>(dig - static_cast<short>(d1 * 10)) ;
      *cp++ = static_cast<char>(d1 + '0') ;
...
int decimal_from_long( int64_t value, bson_decimal *decimal )
{
  ...
  do
  {
    ...
    *ptr = static_cast<short>(uval - newuval * DECIMAL_NBASE) ;
    uval = newuval ;
  } while (uval) ;
...
{
  char buf[ CB_DECIMAL_DBL_DIG + 100 ] = "" ;
  ...
  bson_sprintf( buf, "%.*g", CB_DECIMAL_DBL_DIG, value ) ;
...

{
  ...
  len = static_cast<unsigned int>(strlen( cp )) ;
  ...
  decdigits = (unsigned char *) bson_malloc( static_cast<int>(strlen(cp)) +
  ...
  while (*cp)
  {
    if ( _decimal_is_digit( *cp ) )
    {
      decdigits[i++] = static_cast<unsigned char>(*cp++ - '0') ;
      if ( !have_dp )
      {
        dweight++ ;
  ...
  while (ndigits-- > 0)
  {
    *digits++ = static_cast<char>(( ( decdigits[i] * 10 + decdigits[i + 1] ) * 10 +
...
int decimal_from_bsonvalue( const char *value, bson_decimal *decimal )
{
  ...
  ndig = ( size - DECIMAL_HEADER_SIZE ) / static_cast<int>(sizeof( short )) ;
...
int decimal_copy( const bson_decimal *source, bson_decimal *target )
{
  ...
  newbuf    = (short int*)bson_malloc( (source->ndigits + 1) * static_cast<int>(sizeof(short)) );
  if ( NULL == newbuf )
  {
    rc = -2 ;
    goto error ;
                            ...
                            int typemod, int *size )
{
  ...
  tmpSize = static_cast<int>(strlen( decimal_str_start ) + simpleSize +
  ...
  if ( typemod != -1 )
  {
    ...
    tmpSize += static_cast<int>(strlen( precision_str_start ) +
                        ...
                        int value_size )
{
  ...
  value_size -= static_cast<int>(strlen( decimal_str_start )) ;
  ...
  if ( decimal->typemod != -1 )
  {
    ...
    value_size -= static_cast<int>(strlen( precision_str_start )) ;

    memcpy( value, preStr, strlen( preStr ) ) ;
    value += strlen( preStr ) ;
    value_size -= static_cast<int>(strlen( preStr )) ;

    memcpy( value, precision_str_end, strlen( precision_str_end ) ) ;
    value += strlen( precision_str_end ) ;
    value_size -= static_cast<int>(strlen( precision_str_end )) ;
  }

  memcpy( value, json_str_end, strlen( json_str_end ) ) ;
  value += strlen( json_str_end ) ;
  value_size -= static_cast<int>(strlen( json_str_end )) ;
...
CB_EXPORT int decimal_add( const bson_decimal *left,
                            const bson_decimal *right, bson_decimal *result )
{
  int rc = 0 ;
...
CB_EXPORT int decimal_sub( const bson_decimal *left,
                            const bson_decimal *right, bson_decimal *result )
{
  int rc = 0 ;
  if ( NULL == left || NULL == right || NULL == result )
...
CB_EXPORT int decimal_mul( const bson_decimal *left,
                            const bson_decimal *right, bson_decimal *result )
{
  int rc = 0 ;
  if ( NULL == left || NULL == right || NULL == result )
...
CB_EXPORT int decimal_div( const bson_decimal *left,
                            const bson_decimal *right, bson_decimal *result )
{
  int rc     = 0 ;
  int rscale = 0 ;
...
CB_EXPORT int decimal_abs( bson_decimal *decimal )
{
  int rc = 0 ;
...
CB_EXPORT int decimal_ceil( const bson_decimal *decimal,
                             bson_decimal *result )
{
  int rc = 0 ;
  ...
  if ( decimal->sign == CB_DECIMAL_POS && decimal_cmp( decimal, &tmp ) != 0 )
  {
...
CB_EXPORT int decimal_floor( const bson_decimal *decimal,
                              bson_decimal *result )
{
  int rc = 0 ;
  bson_decimal tmp = DECIMAL_DEFAULT_VALUE ;
  ...
  if ( decimal->sign == CB_DECIMAL_NEG && decimal_cmp(decimal, &tmp) != 0 )
  {
...
CB_EXPORT int decimal_mod( const bson_decimal *left,
                            const bson_decimal *right, bson_decimal *result )
{
  int rc = 0 ;
  bson_decimal tmp = DECIMAL_DEFAULT_VALUE ;
