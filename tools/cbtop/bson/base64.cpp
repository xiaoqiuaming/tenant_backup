...
namespace base64 {
    ...
    void encode( stringstream& ss , const char * data , int size ) {
        for ( int i=0; i<size; i+=3 ) {
            ...
            unsigned char temp = static_cast<unsigned char>(( start[0] << 4 ));
            if ( left == 1 ) {
                ss << alphabet.e(temp);
                break;
            }
            ...
            temp = static_cast<unsigned char>(( start[1] & 0xF ) << 2);
            if ( left == 2 ) {
                ss << alphabet.e(temp);
                break;
            }
    ...
    string encode( const string& s ) {
        return encode( s.c_str() , static_cast<int>(s.size()) );
    }
    ...
    void decode( stringstream& ss , const string& s ) {
        ...
        for ( int i=0; i<size; i+=4) {
            ...
            buf[0] = static_cast<unsigned char>(( ( alphabet.decode[start[0]] << 2 ) & 0xFC ) |
                     ( ( alphabet.decode[start[1]] >> 4 ) & 0x3 ));
            buf[1] = static_cast<unsigned char>(( ( alphabet.decode[start[1]] << 4 ) & 0xF0 ) |
                     ( ( alphabet.decode[start[2]] >> 2 ) & 0xF ));
            buf[2] = static_cast<unsigned char>(( ( alphabet.decode[start[2]] << 6 ) & 0xC0 ) |
