    ...
    */
    ...
    public:
        /** @param initsize this is just a hint as to the final size of the
            object */
        BSONObjBuilder(int initsize=512) : _b(_buf), _buf(initsize + static_cast<int>(sizeof(unsigned))),
        ...
        BSONObjBuilder( const BSONSizeTracker & tracker ) : _b(_buf) ,
          _buf(tracker.getSize() + static_cast<int>(sizeof(unsigned)) ), _offset( sizeof(unsigned) ),
