...
#pragma pack()
  ...
  void BSONObjBuilder::appendMinForType(const StringData& fieldName, int t) {
    switch ( t ) {
      ...
      case Date:
      {
        appendDate( fieldName , numeric_limits<int64_t>::min() ) ;
        return ;
      }
      ...
      case Timestamp:
      {
        OpTime t( numeric_limits<int32_t>::min(), 0 ) ;
  ...
  void BSONObjBuilder::appendMaxForType(const StringData& fieldName, int t) {
    switch ( t ) {
      ...
      case Date:
      {
        appendDate( fieldName , numeric_limits<int64_t>::max() ) ;
        break;
      }
      case Symbol:
      ...
      case Timestamp:
      {
        OpTime t( numeric_limits<int32_t>::max(), 999999 ) ;
        appendTimestamp( fieldName, t.asDate() ) ;
        break;
      }
      default:
