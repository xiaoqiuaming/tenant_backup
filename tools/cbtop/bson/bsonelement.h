...
//#pragma once
#ifndef CBTOP_BSONELEMENT_H_
#define CBTOP_BSONELEMENT_H_
...
namespace bson {
    ...
    */
    ...
    public:
        ...
        */
        ...
        int64_t Long()            const { return chk(NumberLong)._numberLong(); }
        bsonDecimal Decimal()       const { return chk(NumberDecimal).numberDecimal(); }
        int32_t Int()                   const { return chk(NumberInt)._numberInt(); }
        ...
        void OK_EOO()               const { chk(isOk()); }     // throw UserException if element DNE
   ...
   */
        ...
        */
        int32_t size( int32_t maxLen ) const;
        int32_t size() const;
        ...
        */
        ...
        const char *binData(int32_t& len) const {
            assert( type() == BinData );
            len = valuestrsize();
            return value() + 5;
        }
        /** Get binary data.  Element must be of type BinData. Handles type 2 */
        const char *binDataClean(int32_t& len) const {
        ...
        */
        int32_t woCompare( const BSONElement &e, bool considerFieldName = true )
          const;
        ...
        bool operator<( const BSONElement& other ) const {
            int32_t x = (int32_t)canonicalType() - (int32_t)other.canonicalType();
            if ( x < 0 ) return true;
    ...
    private:
        const char *data;
        mutable int32_t fieldNameSize_; // cached value
        int32_t fieldNameSize() const {
            if ( fieldNameSize_ == -1 )
                fieldNameSize_ = (int)strlen( fieldName() ) + 1;
            return fieldNameSize_;
        }
        mutable int32_t totalSize; /* caches the computed size */

        friend class BSONObjIterator;
        friend class BSONObj;
        const BSONElement& chk(int32_t t) const {
            if ( t != type() ) {
                StringBuilder ss;
                if( eoo() )
    ...
    inline int32_t BSONElement::canonicalType() const {
    ...
    inline double BSONElement::numberDouble() const {
        ...
        case NumberInt:
            return *reinterpret_cast< const int32_t* >( value() );
        case NumberLong:
            return (double) *reinterpret_cast< const int64_t* >( value() );
        case NumberDecimal:
            {
               bsonDecimal decimal ;
    ...
    /** Retrieve long value for the element safely.  Zero returned if not a
        number. */
    inline int64_t BSONElement::numberLong() const {
        switch( type() ) {
        case NumberDouble:
            return (int64_t) _numberDouble();
        case NumberInt:
            return _numberInt();
        case NumberLong:
            return _numberLong();
