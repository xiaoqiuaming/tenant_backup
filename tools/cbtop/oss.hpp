...
class CBObject
{
  ...
  public :
    ...
    throw ( const char * )
    {
      void *p = CB_OSS_MALLOC3(size, pFile, line ) ;
      if ( !p ) throw "allocation failure" ;
      return p ;
    }
    ...
    void operator delete ( void *p, const char *pFile, uint32_t line )
    {
      (void)pFile;
      (void)line;
      CB_OSS_FREE(p) ;
    ...
    void * operator new ( size_t size, const char *pFile,
                          uint32_t line, const std::nothrow_t & )
    {
      return CB_OSS_MALLOC3(size, pFile, line ) ;
    }
...
typedef class CBObject CBObject ;
