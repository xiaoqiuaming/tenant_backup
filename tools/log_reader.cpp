...
#include "common/ob_transaction.h" //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150707
#include "updateserver/ob_ups_mutator.h"
#include "updateserver/ob_ups_utils.h"
#include "updateserver/ob_big_log_writer.h"
//add hoongchen [DUMP_FIX] 20170815:b
#include "common/ob_expr_obj.h"
#include "common/ob_obj_cast.h"
//add hoongchen [DUMP_FIX] 20170815:e
...
class IDS
{
  public:
    ...
    static void init_log_command_str()
    {
      ...
      //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150707:b
      LogCommandStr[OB_UPS_MUTATOR_PREPARE]      = "OB_UPS_MUTATOR_PREPARE";
      LogCommandStr[OB_PART_TRANS_COMMIT]      = "OB_PART_TRANS_COMMIT";
      LogCommandStr[OB_PART_TRANS_ROLLBACK]      = "OB_PART_TRANS_ROLLBACK";
      //add 20150707:e
    ...
    static void init_action_flag_str()
    {
      ...
      ActionFlagStr[ObActionFlag::OP_NOP] = "";
      ActionFlagStr[ObActionFlag::OP_TRUN_TAB] = "TRUN_TAB";
    }
    ...
    static void init_obj_type_str()
    {
      ...
      ObjTypeStr[ObInt32Type]                     = "Int32";
      ObjTypeStr[ObDecimalType]                   = "Decimal";
      ObjTypeStr[ObDateType]                      = "Date";  //add hoongchen [DUMP_FIX] 20170815
      ObjTypeStr[ObTimeType]                      = "Time";  //add hoongchen [DUMP_FIX] 20170815
      ObjTypeStr[ObBoolType]                      = "Bool";
    }
...
void print_ups_mutator_body(int cell_idx, ObMutatorCellInfo* cell, const char* row_key_buf)
{
  ...
  if (ObExtendType == cell->cell_info.value_.get_type())
  {
    ...
    if (ObActionFlag::OP_TRUN_TAB == cell->cell_info.value_.get_ext())
    {
      if (!Param::get_instance().has_schema())
        fprintf(Param::get_instance().get_out_stream(), " Cell[%03d]: op=%s %s table[id=%lu]\n",
                cell_idx, iu_op.c_str(), row_op, cell->cell_info.table_id_);
      else
        fprintf(Param::get_instance().get_out_stream(), " Cell[%03d]: op=%s %s table[id=%lu name=%s]\n",
                cell_idx, iu_op.c_str(), row_op, cell->cell_info.table_id_, table_name.c_str());
    }
    else
    {
      if (!Param::get_instance().has_schema())
  ...
  else
  {
    ...
    else if (ObPreciseDateTimeType == cell->cell_info.value_.get_type())
    {
      int64_t val = 0;
      cell->cell_info.value_.get_precise_datetime(val);
      fprintf(Param::get_instance().get_out_stream(), " precisedatetime_value=%s\n", time2str(val));
    }
...
//      fprintf(Param::get_instance().get_out_stream(), " decimal_value=\"%.*s\"\n", val.length(), val.ptr());
      ...
      if (OB_SUCCESS != cell->cell_info.value_.get_decimal(od))
      {
    ...
    else if (ObBoolType == cell->cell_info.value_.get_type())
    {
      bool  val = 0;
      cell->cell_info.value_.get_bool(val);
      fprintf(Param::get_instance().get_out_stream(), " bool_value=%d\n", val);
    }
    //add hoongchen [DUMP_FIX] 20170815:b
    else if (ObInt32Type == cell->cell_info.value_.get_type())
    {
      int32_t val = 0;
      cell->cell_info.value_.get_int32(val);
      fprintf(Param::get_instance().get_out_stream(), " int32_value=%d\n", val);
    }
    else if (ObDateType == cell->cell_info.value_.get_type())
    {
      ObObjCastParams params;
      ObExprObj src_expr_obj,dst_expr_obj;
      src_expr_obj.assign(cell->cell_info.value_);
      char max_tmp_buf[128];
      memset(max_tmp_buf, '\0', sizeof(max_tmp_buf));
      ObString tmp_str(128, 128, max_tmp_buf);
      dst_expr_obj.set_varchar(tmp_str);
      OB_OBJ_CAST[ObDateType][ObVarcharType](params, src_expr_obj, dst_expr_obj);
      fprintf(Param::get_instance().get_out_stream(), " date_value=%s\n", max_tmp_buf);
    }
    else if (ObTimeType == cell->cell_info.value_.get_type())
    {
      ObObjCastParams params;
      ObExprObj src_expr_obj,dst_expr_obj;
      src_expr_obj.assign(cell->cell_info.value_);
      ...
      ObString tmp_str(128, 128, max_tmp_buf);
      dst_expr_obj.set_varchar(tmp_str);
      OB_OBJ_CAST[ObTimeType][ObVarcharType](params, src_expr_obj, dst_expr_obj);
      fprintf(Param::get_instance().get_out_stream(), " time_value=%s\n", max_tmp_buf);
    }
    //add hoongchen [DUMP_FIX] 20170815:e
    else
    {
...
//add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150707:b
void print_ups_mutator_prepare_head(int64_t seq, int64_t len, const ObUpsMutator &mut, const char* op)
{
  time_t tm = mut.get_mutate_timestamp() / 1000000;
  char time_buf[64];
  ctime_r(&tm, time_buf);
  ...
  fprintf(Param::get_instance().get_out_stream(), "SEQ: %lu\tPayload Length: %ld\tTYPE: %s\tMutatorTime: %s\t%s %ld "
          "ChecksumBefore=%lu ChecksumAfter=%lu\n",
          seq, len, IDS::str_log_cmd(OB_UPS_MUTATOR_PREPARE), time_buf, op, mut.get_mutate_timestamp(),
          mut.get_memtable_checksum_before_mutate(), mut.get_memtable_checksum_after_mutate());
}

int print_part_trans_commit(int64_t seq, const char* buf, int64_t len)
{
  int ret = 0;

  int64_t pos = 0;
  ObTransID coordinator_tid;
  ret = coordinator_tid.deserialize(buf, len, pos);
  if (OB_SUCCESS != ret)
  {
    fprintf(stderr, "Error occured when deserializing ObTransID");
  }
  if (!Param::get_instance().has_sub_row_key())
  {
    fprintf(stdout, "SEQ: %lu\tPayload Length: %ld\tTYPE: %s\t", seq, len, IDS::str_log_cmd(OB_PART_TRANS_COMMIT));
  }
  fprintf(Param::get_instance().get_out_stream(), "coordinator trans_id:%s\n",to_cstring(coordinator_tid));
  
  return ret;
}

int print_part_trans_rollback(int64_t seq, const char* buf, int64_t len)
{
  int ret = 0;
  ...
  if (!Param::get_instance().has_sub_row_key())
  {
    fprintf(stdout, "SEQ: %lu\tPayload Length: %ld\tTYPE: %s\t", seq, len, IDS::str_log_cmd(OB_PART_TRANS_ROLLBACK));
  }
  fprintf(Param::get_instance().get_out_stream(), "coordinator trans_id:%s\n",to_cstring(coordinator_tid));
...
int print_ups_mutator_prepare(int64_t seq, const char* buf, int64_t len)
{
  int ret = 0;
  ...
  int64_t pos = 0;
  ObTransID coordinator_tid;
  ret = coordinator_tid.deserialize_for_prepare(buf, len, pos);
    ...
    while (OB_SUCCESS == (ret = mut.next_cell()))
    {
      ...
      else
      {
        if (!Param::get_instance().has_sub_row_key() || strstr(row_key_buf, Param::get_instance().get_sub_row_key()) != NULL)
        {
          if (!print_flag)
          {
            print_ups_mutator_prepare_head(seq, len, mut, mutator_op);
            fprintf(Param::get_instance().get_out_stream(), "coordinator trans_id:%s\n",to_cstring(coordinator_tid));
            print_flag = true;
          }
  ...
  if (0 == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        else if(OB_UPS_BIG_LOG_DATA == cmd)
        {
          ...
          else
          {
            ...
            if(is_all_completed)
            {
              ...
              print_big_log_mutator(log_seq,data_len);//��ӡ��־ͷ
              //��ӡbig log������־
              ...
              if(OB_SUCCESS==ret&&read_next_file) //�л��ļ����� ��������־��ȡ��ϣ�����
              {
        ...
        else if(OB_LOG_SWITCH_LOG == cmd && is_reading_big_log)  //�л��ļ�
        {
          read_next_file = true;
          switch_log_seq = log_seq;
          switch_data_len = data_len;
          if(OB_SUCCESS!=(ret = reader.close())) //�ر���һ���ļ�
          {
            fprintf(stderr, "Error occured when close, ret=%d\n", ret);
          }
          else if(OB_SUCCESS!=(ret = reader.open(atoi(log_name)+1))) //��ȡ��һ���ļ�
          {
            fprintf(stderr, "Error occured when opening, ret=%d\n", ret);
          }
        }
        //add e
        //add peiouya [MultiUPS] [DISTRIBUTED_TRANS] 20150707:b
        else if (OB_UPS_MUTATOR_PREPARE == cmd)
        {
          ret = print_ups_mutator_prepare(log_seq, log_data, data_len);
        }
        else if (OB_PART_TRANS_COMMIT == cmd)
        {
          ret = print_part_trans_commit(log_seq, log_data, data_len);
        }
        else if (OB_PART_TRANS_ROLLBACK == cmd)
        {
          ret = print_part_trans_rollback(log_seq, log_data, data_len);
        }
        //add 20150707:e
        else
        {
