...
int get_result(ObDataBuffer & data_buff, ObResultCode& result_code, T &data)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize server list failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  return ret;
}
    ...
    const ObNewRange& range, ObTabletLocation location [],int32_t& size)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (; OB_SUCCESS == ret && iter != scanner.end() && index < size; ++iter)
    {
      ...
      if (OB_SUCCESS == ret && cell != NULL)
      {
                 ...
                 cell->column_name_.compare("3_tablet_version") == 0)
        {
          ...
          YYSYS_LOG(WARN, "%s", to_cstring(cell->row_key_));
          YYSYS_LOG(DEBUG,"tablet_version is %ld",version);
        }
    ...
    if (OB_SUCCESS == ret && ip != 0 && port != 0 && version != 0 && index < size) //last row
    {
      ...
      ObTabletLocation addr(version, server);
      server.to_string(tmp_buf,sizeof(tmp_buf));
      YYSYS_LOG(DEBUG,"add tablet s:%s,%ld",tmp_buf,version);
      location[index++] = addr;
    }
  }
  ...
  {
    size = index;
    YYSYS_LOG(DEBUG,"get %d tablets from rootserver",size);
  }
  return ret;
}
...
int ObClientRpcStub::show_param()
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO,"send show param success");
  }
...
int ObClientRpcStub::sync_all_tablet_images()
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO,"send sync_all_tablet_images success");
  }
...
int ObClientRpcStub::delete_table(const uint64_t table_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO,"send delete table success");
  }
...
int ObClientRpcStub::load_sstables(const ObTableImportInfoList& table_list)
{
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO,"send load bypass sstables success");
  }

  return ret;
}

int ObClientRpcStub::cs_show_disk(int32_t* disk_no, bool* avail, int32_t &disk_num, int64_t *avail_size)
{
  int ret = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(ERROR, "failed to alloc in_buffer, ret=%d", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to send request to server: %s, timeout=%ld ret=%d",
    ...
    {
      YYSYS_LOG(ERROR, "failed to deserialize rc, ret:%d", ret);
    }
    else if (OB_SUCCESS != rc.result_code_)
    {
      ret = rc.result_code_;
      YYSYS_LOG(ERROR, "failed show disk, ret:%d", ret);
    }
    else
    {
      if (OB_SUCCESS != (ret = serialization::decode_i32(data_buffer.get_data(), data_buffer.get_position(), pos, &disk_num)))
      {
        YYSYS_LOG(ERROR, "failed to deserialize disk num, ret:%d", ret);
      }
      else
      {
        for (int32_t i = 0; i < disk_num && i < common::OB_MAX_DISK_NUMBER; i++)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            ret = serialization::decode_i64(data_buffer.get_data(), data_buffer.get_position(), pos, &avail_size[i]);
          }
        }
      }
    }
...
int ObClientRpcStub::stop_server(bool restart)
{
  ...
  // change_log_level��Ϣ������Ϊһ��int32_t���͵�log_level
    ...
    std::list<std::pair<ObNewRange, ObString> >& sstable_list, oceanbase::common::ModuleArena& arena)
{
  ...
  YYSYS_LOG(INFO, "fetching sstable disst from %s with table_version_=%ld",
  ...
  {
    YYSYS_LOG(ERROR, "failed to alloc out_buffer, ret=%d", ret);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to encode table_version_, cap=%ld pos=%ld ret=%d",
    ...
    {
      YYSYS_LOG(WARN, "failed to send request to server: %s, timeout=%ld session_id=%ld ret=%d",
    ...
    while (OB_SUCCESS == ret && !last_flag)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to deserialize rc, cap=%ld pos=%ld ret=%d",
      ...
      {
        ret = rc.result_code_;
        YYSYS_LOG(WARN, "error happend in cs server[%s], ret=%d", to_cstring(server), ret);
        break;
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to decode sstable_count, cap=%ld pos=%ld ret=%d",
              out_buffer.get_position(), pos, ret);
        }
      }
      for(int64_t i = 0; OB_SUCCESS == ret && i < sstable_count; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to decode range, cap=%ld pos=%ld ret=%d",
              out_buffer.get_position(), pos, ret);
        }
        else if (OB_SUCCESS !=
            (ret = path.deserialize(out_buffer.get_data(), out_buffer.get_position(), pos)))
        {
          YYSYS_LOG(WARN, "failed to decode path, cap=%ld pos=%ld ret=%d",
              out_buffer.get_position(), pos, ret);
        }
        else
        {
          ...
          if (NULL == path_buf)
          {
            YYSYS_LOG(WARN, "failed to alloc path buf, len %ld", path_len);
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "failed to copy range: %s, ret=%d", to_cstring(range), ret);
            }
      ...
      YYSYS_LOG(INFO, "fetch sstable_count %ld, total_sstable_count from this cs: %ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get next from server: %s, ret=%d", to_cstring(server), ret);
        }
      }
      else if (OB_SUCCESS != (rpc_ret =
            rpc_frame_->post_end_next(server, session_id, timeout, in_buffer, NULL, NULL)))
      {
        YYSYS_LOG(WARN, "failed to post end to server %s, ret=%d", to_cstring(server), rpc_ret);
      }
    }
  }
...
int ObClientRpcStub::fetch_bypass_table_id(ObBypassTaskInfo& info)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "serialize import table task info into buffer failed,"
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "post request to root server for load_bypass_sstables failed"
          " ret[%d].",  ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "deserialize result_code failed:cap[%ld] pos[%ld], ret[%d].",
          data_buff.get_position(), pos, ret);
    }
    else if (OB_SUCCESS != result_code.result_code_)
    {
      ret = result_code.result_code_;
      YYSYS_LOG(ERROR, "root server return error code: %d", ret);
    }
    else if (OB_SUCCESS != (ret =
          info.deserialize(data_buff.get_data(), data_buff.get_position(), pos)))
    {
      YYSYS_LOG(ERROR, "deserialize bybpass table info failed: cap[%ld] pos[%ld] ret[%d]",
          data_buff.get_position(), pos, ret);
    }
  }
...
int ObClientRpcStub::backup_database(const int32_t backup_mode_id)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "serialize backup mode info into buffer failed,"
        "cap=%ld pos=%ld ret=%d", data_buff.get_capacity(), data_buff.get_position(), ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "post request to server for backup database failed"
          " ret[%d].",  ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = result_code.result_code_;
      YYSYS_LOG(ERROR, "backup server return error code: %d", ret);
    }
  }
  return ret;
}
                                  ...
                                  const oceanbase::common::ObString table_name)
{
  ...
  {
    YYSYS_LOG(ERROR, "serialize tablename info into buffer failed,"
        "cap=%ld pos=%ld ret=%d", data_buff.get_capacity(), data_buff.get_position(), ret);
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "post request to backup server for backup table failed"
          " ret[%d].",  ret);
    }
  }
  int64_t pos = 0;
...
int ObClientRpcStub::backup_abort()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "post request to root server for backup abort failed"
          " ret[%d].",  ret);
    }
  }
  int64_t pos = 0;
