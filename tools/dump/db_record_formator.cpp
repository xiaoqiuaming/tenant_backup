                                      ...
                                      const string &table_name, ObDataBuffer &buffer)
{
  ...
  if (len <= 0) {
    YYSYS_LOG(ERROR, "buffer size not enough, buff_size=%d", cap);
    ret = OB_ERROR;
  } else {
  ...
  if (ret == OB_SUCCESS) {
    if ((ret = rowkey.serialize(bin_rowkey_buffer, k2M, bin_rowkey_len)) != OB_SUCCESS) {
      YYSYS_LOG(WARN, "large rowkey exceed header format buffer[%d]", k2M);
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (len == 0) {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "no enough memory to append rowkey");
    } else {
...
int append_int64(int64_t value, ObDataBuffer &buffer)
{
  ...
  if (len > 0) {
    ...
    YYSYS_LOG(ERROR, "append_int64 error");
    ret = OB_ERROR;
  }
...
int append_double(double value, ObDataBuffer &buffer)
{
  ...
  if (len > 0) {
    ...
    YYSYS_LOG(ERROR, "append_double error");
    ret = OB_ERROR;
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (cfg->is_revise_column(column) && cell->value_.get_type() == ObIntType) {
      ...
      if (ret == OB_SUCCESS) {
        ...
        YYSYS_LOG(ERROR, "obcellinfo get value failed");
      }
    } else {
      ...
      if (len >= 0)
        ...
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "append item error,column=%s", column.c_str());
      }
    }
  }
  ...
  if (ret == OB_SUCCESS && is_rowkey) {
    ...
    if (ret != OB_SUCCESS && ret != OB_ENTRY_NOT_EXIST)
      ...
      if (ret == OB_ENTRY_NOT_EXIST) {
        ...
        buf[2 * len] = 0;
        YYSYS_LOG(ERROR, "can't get column:%s, table:%s, rowkey:%s", column.c_str(), cfg->table().c_str(), buf);
//        rec->dump();
      }
#endif
//      ret = OB_SUCCESS;
  ...
  if (ret == OB_SUCCESS) {
    ...
    for(size_t i = 0;i < output_columns.size(); i++) {
      if (output_columns[i].compare(kNullFiller) == 0) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "append column:%s failed", output_columns[i].c_str());
          break;
        }
      }
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "append deliam failed");
        break;
      }
    }
    ...
    YYSYS_LOG(ERROR, "DbRecordFormator can't find table id [%lu]", table_id);
  }
