...
void ComsumerQueue<T>::monitor(long id)
{
	...
	while (running_) {
    // add zhangcd [ob_import.ignore_merge] 20150721:b
    YYSYS_LOG(INFO, "Monitor is running...");
    // add:e
		...
		if (0 == (count % l_merge_sleep)) {//需要获取每日合并状态
          ...
          // add:e
				...
				if(1 == atomic_read(&is_merge_)) {//上一次是merge: DONE
					if (1 == state) {
						YYSYS_LOG(DEBUG, "current state [merge: DONE]");
					} else if (0 == state) {
						YYSYS_LOG(DEBUG, "current state [merge: DOING]");
						atomic_sub(1, &is_merge_);
					} else if (-1 == state) {
						YYSYS_LOG(DEBUG, "can't get merge state");
						atomic_sub(2, &is_merge_);
					} else {
						YYSYS_LOG(ERROR, "invalid merge state, state=%d", state);
						atomic_sub(2, &is_merge_);
					}
					...
					YYSYS_LOG(WARN, "forbid producer running, is_merge_=%d", atomic_read(&is_merge_));
					//修改全局状态,通知主线程异常状态,程序会异常退出
					YYSYS_LOG(INFO, "quit monitor id=%ld", id);
					g_cluster_state = false;
					break;
		...
		if (0 == (count % l_ups_sleep)) {//需要获取最新主UPS
			...
			if (ret != 0) {//获取主UPS失败,
				YYSYS_LOG(ERROR, "failed to get latest update server, err=%d", ret);
				atomic_set(&import_state_, -1);
				YYSYS_LOG(WARN, "forbid producer running, import_state_=%d", atomic_read(&import_state_));
				YYSYS_LOG(INFO, "quit monitor id=%ld", id);
				//修改全局状态,通知主线程异常状态,程序会异常退出
				g_cluster_state = false;
				break;
				...
				if (0 == ups_switch) {//主UPS未发生切换
					YYSYS_LOG(DEBUG, "master update server not switch");
					...
					YYSYS_LOG(ERROR, "invalid ups switch state, ups_switch=%d", ups_switch);
					atomic_set(&import_state_, -1);
					YYSYS_LOG(WARN, "forbid producer running, import_state_=%d", atomic_read(&import_state_));
					YYSYS_LOG(INFO, "quit monitor id=%ld", id);
					//修改全局状态,通知主线程异常状态,程序会异常退出
