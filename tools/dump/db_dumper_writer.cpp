...
DbDumperWriter::DbDumperWriter(uint64_t id, string path)
{
  ...
  YYSYS_LOG(INFO, "DbDumperWriter %s", path_.c_str());
  log_id_ = 0;
  file_ = NULL;
}
...
{
  YYSYS_LOG(INFO, "FILEID:%ld, PUSHEDLEN:%d, WRITENLEN:%d", id_, pushed_lines_,  writen_lines_);
  stop();
  if (file_ != NULL) {
    delete file_;
    file_ = NULL;
...
int DbDumperWriter::mv_output_file()
{
  ...
  if (len <= 0) {
    YYSYS_LOG(ERROR, "mv_out_file:src_buf size too short");
    ret = OB_ERROR;
  }
  ...
  if (ret == OB_SUCCESS) {
    len = snprintf(dst_buf, MAX_PATH_LEN, "%s/%s/%ld",
                   path_.c_str(), current_date_.c_str(), id);
    if (len <= 0) {
      YYSYS_LOG(ERROR, "mv_out_file:dst_buf size too small");
      ret = OB_ERROR;
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (!dir_utils.rename(src_buf, dst_buf)) {
      YYSYS_LOG(ERROR, "can't rename %s to %s", src_buf, dst_buf);
      ret = OB_ERROR;
    }
  }
...
int DbDumperWriter::init_output_files()
{
  ...
  if (exists) {
    ...
    if (!dir_utils.create_full_path(dir_path.c_str())) {
      YYSYS_LOG(ERROR, "create directory %s failed", dir_path.c_str());
      ret = OB_ERROR;
    } else {
      log_id_ = 0;
    }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (len < 0 || len > MAX_PATH_LEN) {
      YYSYS_LOG(ERROR, "when format path error");
      ret = OB_ERROR;
    } else {                                    /* create output file */
      ret = AppendableFile::NewAppendableFile(path, file_);
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "when open file %s", path);
      }
    }
  }
...
void DbDumperWriter::wait_completion(bool rotate_date)
{
  while(true) {
    {
      ...
      if (records_.empty()) {
        YYSYS_LOG(INFO, "waiting tableid:%lu complete writing records", id_);
        break;
      }
    }

    YYSYS_LOG(INFO, "DumperWrite Go to sleep");
    usleep(5000);
  }
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "can't rotate next out file, wait completion failed, quiting");
    stop();
  } else {
    YYSYS_LOG(INFO, "wait complete success");
  }
}
...
int64_t DbDumperWriter::get_next_file_id()
{
  ...
  if( NULL == log_dir ) {
    YYSYS_LOG(ERROR, "Can't open path, %s", log_dir_path.c_str());
  } else {
...
int DbDumperWriter::rotate_date_dir()
{
  ...
  if (current_date_.compare(datebuf)) {
    ...
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't write done file, path:%s, %d", tmp_path.c_str(), ret);
      report_msg(MSG_ERROR, "can't write done file, path:%s", tmp_path.c_str());
    }
    YYSYS_LOG(INFO, "max writen files number=[%ld]", log_id_);
    ...
    if (ret == OB_SUCCESS) {
      ...
      ret = mv_output_file();
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't mv_out_file ");
      }
    }
    ...
    if (ret == OB_SUCCESS) {
      ...
      YYSYS_LOG(INFO, "change to new date, %s", current_date_.c_str());
      ret = init_output_files();
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't change directory");
      }
    }
  } else {
    YYSYS_LOG(WARN, "can't roate date dir,because date hasn't changed");
  }
...
int DbDumperWriter::rotate_output_file()
{
  ...
  if (ret == OB_SUCCESS) {
    ...
    len = snprintf(file_buf, MAX_DATE_LEN, "%s/%s/%ld.TMP", path_.c_str(), 
                   current_date_.c_str(), ++log_id_);
    if (len <= 0) {
      YYSYS_LOG(ERROR, "file buffer is not enough");
      ret = OB_ERROR;
    } else {
      ret = AppendableFile::NewAppendableFile(file_buf, file_);
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "when open file %s", file_buf);
      }
    }
  }
...
int DbDumperWriter::start()
{
  ...
  if (sem_init(&sem_empty_, 0, 0) != 0) {
    YYSYS_LOG(ERROR, "error when calling sem_init");
    ret = OB_ERROR;
  }
  ...
  YYSYS_LOG(INFO, "[dump writer]:init date=%s", current_date_.c_str());
  if (ret == OB_SUCCESS) {
    ret = init_output_files();
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "when init output files");
    }
  }

  if (ret == OB_SUCCESS) {
    thr_ = new(std::nothrow)  yysys::CThread();
    if (thr_ == NULL)
      ret = OB_ERROR;
  }
...
void DbDumperWriter::run(yysys::CThread *thr_, void *arg)
{
  last_rotate_time_ = time(NULL);
  UNUSED(thr_);
  UNUSED(arg);
  ...
  while(running_ || !records_.empty()) {
    ...
    if (need_rotate_output_file()) {
      ret = rotate_output_file();
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't rotate file");
        break;
      }
    }
    ...
    if (ret == OB_SUCCESS) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "write record failed, %d", ret);
        sleep(1);
      }
    }
  }
...
int DbDumperWriter::push_record(const char *data, int lenth)
{
  ...
  if (rec == NULL) {
    YYSYS_LOG(ERROR, "Can't allocate records, len:%d", lenth);
    ret = OB_ERROR;
  } else {
...
int DbDumperWriter::write_record(bool flush)
{
  ...
  if (rec != NULL) {
    ...
    if (file_ == NULL || (ret = file_->Append(rec->buf, rec->length)) != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "Write record failed, ret:%d, path:%s, len:%d", ret, path_.c_str(), rec->length);
    } else {
      writen_lines_++;
      records_.pop_front();
      free_record(rec);
