...
int DbCheckpoint::load_checkpoint()
{
  ...
  if (fd < 0) {
    YYSYS_LOG(WARN, "open %s error", path_.c_str());
    YYSYS_LOG(INFO, "get checkpoint from rpc");
    ret = OB_ERROR;
    get_from_rpc = true;
  } else {
    ...
    if (len <= 0) {
      YYSYS_LOG(ERROR, "read %s error", ckp_name_.c_str());
      ret = OB_ERROR;
    } else {
      buff[len] = '\0';
      id_ = atol(buff);
...
int DbCheckpoint::do_checkpoint_local(int64_t log_id)
{
  ...
  if (fd < 0) {
    YYSYS_LOG(ERROR, "error when open %s", path_.c_str());
    ret = OB_ERROR;
  } else {
    char buff[kCheckpointLen];
    ...
    if (len < 0) {
      YYSYS_LOG(ERROR, "sprintf error");
      ret = OB_ERROR;
    } else if (len > kCheckpointLen) {
      YYSYS_LOG(ERROR, "id_ is too long for the buffer");
      ret = OB_ERROR;
    } else {
      if (file.write(buff, len, true) == len) {
        id_ = log_id;
        YYSYS_LOG(INFO, "write checkpoint successfully");
        ret = OB_SUCCESS;
      } else {
        YYSYS_LOG(ERROR, "write checkpoint failed");
        ret = OB_ERROR;
      }
    }
  }
...
int DbCheckpoint::write_checkpoint(int64_t log_id)
{
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "can't write remote checkpoint");
  } else {
    ret = do_checkpoint_local(log_id);
    if (ret != OB_SUCCESS)
      YYSYS_LOG(ERROR, "can't write local checkpoint");
  }
