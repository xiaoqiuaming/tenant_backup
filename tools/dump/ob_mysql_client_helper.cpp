...
yysys::CThreadMutex ObMysqlClientHelper::init_lock_; //add by zhuxh 20160922 [connection mutex] //Every static member is defined even with default constructor only.
                              ...
                              const char *db_name, unsigned short host_port, unsigned int timeout)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "failed to get tsi object from tsi_factory!");
  }

  yysys::CThreadGuard guard(&ObMysqlClientHelper::init_lock_); //add by zhuxh 20160922 [connection mutex] //A guard cannot be in a inner area otherwise it ends in advance unexpectedly.
  if(OB_SUCCESS == ret)
  {
    if(*link != NULL)
    {
    ...
    //yysys::CThreadGuard guard(&ObMysqlClientHelper::init_lock_); //del by zhuxh 20160922 [connection mutex]
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "mysql_real_connect failed! mysql error_code[%d] error_msg[%s]", mysql_errno(*link), mysql_error(*link));
      mysql_close(*link);
      *link = NULL;
    }
  }
...
int ObMysqlClientHelper::execute_dml_sql(const ObString& stmt, int &affected_row)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "failed to get MYSQL* from tsi buffer!");
  }
  ...
  {
    YYSYS_LOG(ERROR, "failed to get result while execute mysql_query! err_code[%d] err_msg[%s]", mysql_errno(*link), mysql_error(*link));
    ret = mysql_errno(*link);
//    mysql_close(*link);
//    *link = NULL;
  }
  if(ret == OB_SUCCESS)
