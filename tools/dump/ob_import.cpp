...
int ObRowBuilder::set_rowkey_desc(const std::vector<RowkeyDesc> &rowkeys)
{
  ...
  if (tab_schema == NULL) {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "no such table in schema");
  } else {
  ...
  if (ret == OB_SUCCESS) {
    ...
    for(size_t i = 0;i < rowkeys.size(); i++) {
      ...
      if (offset >= table_param_.input_column_nr) {
        YYSYS_LOG(ERROR, "wrong config, table=%s, offset=%d", table_param_.table_name.c_str(), offset);
        ret = OB_ERROR;
        break;
      }
    ...
    for (size_t i = 0;i < rowkey_desc_nr_;i++) {
      if (rowkey_desc_[i].pos == -1) {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "rowkey config error, intervals in it, pos=%ld, please check it", i);
        break;
      }
    }
  }
  ...
  while (block.next_record(slice)) {
    ...
    if (g_gbk_encoding) {
      if (RecordDelima::CHAR_DELIMA == table_param_.delima.delima_type() && table_param_.delima.get_char_delima() < 128u) {
        ...
        YYSYS_LOG(ERROR, "short delima or delima great than 128 is not support in gbk encoding");
        break;
      }
    }
    else {
...
//delete:e
	...
	//modify:end
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't rowkey, quiting");
    }
    ...
    if (ret == OB_SUCCESS) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't create insert mutator , table=%s", table_param_.table_name.c_str());
      }
    }

    if (table_param_.is_delete && ret == OB_SUCCESS) {
      if (OB_SUCCESS != (ret = mutator.delete_row())) {
        YYSYS_LOG(WARN, "fail to delete row:ret[%d]", ret);
      }
    }
    else {
      if (ret == OB_SUCCESS) {
		...
		//modify:end
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't transcation content, table=%s", table_param_.table_name.c_str());
          for (int i = 0; i < token_nr; i++) {
            YYSYS_LOG(ERROR, "token seq[%d] [%.*s]", i, static_cast<int>(tokens[i].len), tokens[i].token);
          }
        }
      }
    }
    ...
    if (OB_INVALID_ARGUMENT == ret) {
      ...
      if(bad_file->Append(line_buffer_, pos) != OB_SUCCESS) {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "can't write to bad_file name = %s", table_param_.bad_file_);
      }
    }
...
//modify:end
{
  ...
  if (mbuf == NULL || mbuf->ensure_space(rowkey_desc_nr_ * sizeof(ObObj))) {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "can't create object from TSI, or ObMemBuff can't alloc memory");
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    for(int64_t i = 0;i < rowkey_desc_nr_; i++) {
      ...
      if (table_param_.has_nop_flag) {
        if (table_param_.nop_flag == tokens[token_idx].token[0]) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "There is nop flag in rowkey. rowkey seq[%ld]", i);
          return ret;
        }
      }
	...
	if(table_param_.is_append_date_ && (columns_desc_[rowkey_offset_[i]].offset > token_nr - 1)){
		if((ret = make_obobj(columns_desc_[rowkey_offset_[i]], obj, table_param_.appended_date.c_str())) != OB_SUCCESS){
			...
			YYSYS_LOG(WARN, "create hds_lupt rowkey failed, pos = %ld", i);
			 break;
		}
		 buffer[i] = obj;
		continue;
	...
	//add:end
      if ((ret = make_obobj(columns_desc_[rowkey_offset_[i]], obj, tokens)) != OB_SUCCESS) {
		error_arr[3] = false; //add by pangtz:20141205
        YYSYS_LOG(WARN, "create rowkey failed, pos = %ld", i);
        break;
      }
      buffer[i] = obj;
    }
...
int ObRowBuilder::make_obobj(const ColumnInfo &column_info, ObObj &obj,  const char *accdate) const
{
  ...
  if (NULL == column_info.schema) {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "column_info schema is null");
    return ret;
  }
  ...
  if((token_len == 0) && !column_info.schema->is_nullable() && ((type != ObVarcharType) || is_decimal_to_varchar)) {
	...
	error_arr[4] = false;
	YYSYS_LOG(ERROR, "according to schema, the column is not allowed to be set null, column=%s, column offset[%d]", column_info.schema->get_name(), token_idx);
	//add e
	ret = OB_ERROR;
  }
  ...
  //mod:e
    ...
    switch(type) {
	 ...
	 //mod by liyongfeng
	    ...
	    {
		YYSYS_LOG(ERROR, "failed to transform_str_to_int, actual value[%.*s], column offset[%d]", token_len, token, token_idx);
	    }
         }
       }
       break;
     ...
     case ObDateTimeType:
       {
         ...
         if (OB_SUCCESS != ret)
         {
           YYSYS_LOG(ERROR,"failed to transform_date_to_time");
         }
         else
         {
     ...
     case ObVarcharType:
       {
		   if(is_decimal_to_varchar)
		   {
			   ...
			   if(ret == OB_ERROR) {
				   YYSYS_LOG(ERROR, "decimal field, but numeric string[%.*s] is invalid decimal string", token_len, token);
				   break;
			   }
		   }
         ObString bstring;
		...
		error_arr[7] = false;
		//add e
           YYSYS_LOG(ERROR, "varchar overflow max_len[%ld], actual size[%d], column offset[%d]",
			...
			if (token_len != 0)
	   		{
				...
				if((data_p - data_s) > (p - s)) {
					YYSYS_LOG(ERROR, "decimal value[%.*s] not satisfies decimal precision[%u, %u]", token_len, token, p, s);
					ret = OB_ERROR;
				}else {
		   			bstring.assign_ptr(const_cast<char *>(token), token_len);
		   			if(OB_SUCCESS != (ret = obj.set_decimal(bstring))){
			   			YYSYS_LOG(ERROR, "failed to covert ObString to Decimal,string=%.*s", bstring.length(),bstring.ptr());
		   			}else{
			   			obj.set_precision(p);
			   			obj.set_scale(s);
		   			}
     ...
     case ObPreciseDateTimeType:
       {
         ...
         if (OB_SUCCESS != ret)
         {
           YYSYS_LOG(ERROR,"failed to transform_date_to_time, actual value[%s], column offset[%d]", token, token_idx);
         }
         else
         {
       ...
       break;
     default:
       YYSYS_LOG(ERROR,"unexpect type index: %d", type);
       ret = OB_ERROR;
       break;
    }
  }
	...
	//add:end
	 // error_arr[3] = false; //add by pangtz:20141205
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't add column to row mutator");
          break;
        } else {
          YYSYS_LOG(DEBUG, "obj idx=%d, name=%s", columns_desc_[i].offset, schema->get_name());
          obj.dump();
        }
      } else {
		  error_arr[3] = false; //add by pangtz:20141219
        YYSYS_LOG(ERROR, "can't make obobj, column=%s", schema->get_name());
      }
    }
    else
    {
      YYSYS_LOG(DEBUG, "nop token, offset[%d]", columns_desc_[i].offset);
    }
  }
