...
void Tokenizer::tokenize(Slice &slice, const TableParam &table_param, int &token_nr, TokenInfo *tokens)
{
  ...
  const char *ptoken = NULL;
  const char *data = slice.data();
  bool is_start_with_quotation = false;
  ...
  int64_t dlen = static_cast<int64_t>(slice.size());
  int64_t first_rec_delima_pos = dlen;
  int64_t first_col_delima_pos = dlen;
  ...
  while (pos < dlen && token < token_nr) {
    ...
    if (*ptoken == '\"')
    {
      is_start_with_quotation = true;
    }
    //modify by pangtz:20141127
    //for ( ; pos < dlen && !delima.is_delima(data, pos, dlen); pos++);
    for ( ; pos < dlen &&
          ((!is_start_with_quotation && !table_param.delima.is_delima(data, pos, dlen)) ||
           (is_start_with_quotation && !(data[(pos > 0) ? (pos - 1) : 0] == '\"' && 
            (table_param.delima.is_delima(data, pos, dlen) || table_param.rec_delima.is_delima(data, pos, dlen)))));
          pos++)
      //add:end
    {
      if (is_start_with_quotation)
      {
        if(table_param.rec_delima.is_delima(data, pos, dlen))
        {
        ...
        else if(table_param.delima.is_delima(data, pos, dlen))
        {
    ...
    if(is_start_with_quotation && pos >= dlen)
    {
      if(first_col_delima_pos < first_rec_delima_pos)
      {
        pos = first_col_delima_pos;
        first_rec_delima_pos = dlen;
        first_col_delima_pos = dlen;
