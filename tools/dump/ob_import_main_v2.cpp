...
void usage(const char *prog)
{
  ...
  fprintf(stderr, //"Usage:%s \n"
          "===== Examples =====\n"
          "\t(1)\n"
          "\t./ob_import -h 182.119.80.67,182.119.80.68,182.119.80.69 -p 2500 --username db_user_name --passwd db_user_password --dbname database_name -t table_name -l t.log --badfile t.bad -f t.del --rep --progress\n"
          "\t(2)\n"
          "\t~/oceanbase/bin/ob_import -h 182.119.80.59,182.119.80.60,182.119.80.61 -p 5500 --dbname TANG -t students -l t.log --badfile t.bad -f t.del --case-sensitive --progress --concurrency 100 --record-persql 50 --trim 0 --column-mapping \"number,seq();first_name,3;last_name,1;hobby,2\"\n"
          ...
          "\t[--limit-ups-mem] (import process will pause when ups memory exceeds this value, unit:G)\n"
          /*
          ...
          //add:e
          "\t[--custom-column]\n"
          "\t --custom-column \"1:hex;3:hex\" means column 1 and column 3 should be imported as hex data(column index starts with 1)\n"
          "\t[-V] (version)\n"
          "\t[-H] (help)\n"
          //add by pangtz
          "===== Return Value =====\n"
...
//  if (queue.produce_and_comsume(1, param.ignore_merge_ ? 0 : 1, param.concurrency) != 0)
  if (queue.produce_and_comsume(1, 1, param.concurrency) != 0)
  {
    ret = OB_ERROR;
...
//add e
{
  ...
  if( OB_SUCCESS == ret ) //add by zhuxh:20150805
  {
    ...
    if( table_param.has_substr )
      if( OB_SUCCESS != (ret = parse_substr_grammar(table_param.substr_grammar,table_param.substr_map)) )
      {
        ...
        YYSYS_LOG(ERROR, "%s : substr grammar error.", table_param.substr_grammar);
      }
#endif
  }
  if( OB_SUCCESS == ret ) //add by zhuxh:20150805
  {
    ...
    table_param.custom_column_str = cmd_table_param.custom_column_str;
    if (NULL != table_param.custom_column_str)
    {
      char * & s = table_param.custom_column_str;
      for(int i = 0; s[i]; i++)
      {
        s[i] = static_cast<char>(tolower(s[i]));
      }
      vector<string> custom_column_vec;
      vector<string> hex_column_vec;
      Tokenizer::tokenize(table_param.custom_column_str, custom_column_vec, ';');
      for(size_t index = 0; index < custom_column_vec.size(); index++)
      {
        hex_column_vec.clear();
        Tokenizer::tokenize(custom_column_vec[index].c_str(), hex_column_vec, ':');
        if (hex_column_vec.size() == 2)
        {
          if (0 == strcmp("hex", hex_column_vec[1].c_str()))
          {
            table_param.hex_column.push_back(atoi(hex_column_vec[0].c_str()));
          }
        }
        else
        {
          fprintf(stdout, "parse command line parameter custom-column error!\n");
          YYSYS_LOG(ERROR, "parse command line parameter custom-column error!");
          return OB_ERROR;
        }
      }
      if (0 == custom_column_vec.size() || 0 == table_param.hex_column.size())
      {
        fprintf(stdout, "parse command line parameter custom-column error!\n");
        YYSYS_LOG(ERROR, "parse command line parameter custom-column error!");
        return OB_ERROR;
  ...
  if( OB_SUCCESS == ret ) //add by zhuxh:20150805
  {
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      if(ret == OB_SUCCESS)
      {
        ...
        for(int i = 0; i < column_desc_nr; i++)
        {
          table_param.is_hex_column.push_back(false);
        }
        for(unsigned int j = 0; j < table_param.hex_column.size(); j++)
        {
          if(table_param.hex_column[j] > 0)
          {
            table_param.is_hex_column[table_param.hex_column[j] - 1] = true;
          }
        }
      }
      ...
      {
        error_arr[COLUMN_DESC_SET_ERROR] = false;
        YYSYS_LOG(ERROR, "Dropping Sequence failed.");
      }
      */
      //add :e
    }
...
int main(int argc, char *argv[])
{
  ...
  //add by zhangcd 20150814:b
  cmd_table_param.user_name_ = NULL;
  cmd_table_param.passwd_ = NULL;
  ...
  static struct option long_options[] =
  {
    ...
    {"custom-column", 1, 0, e_custom_column},
    {0, 0, 0, 0}
  };
  int option_index = 0;
  ...
  while ((ret = getopt_long(argc, argv, "h:p:t:c:l:g:q:f:HV", long_options, &option_index)) != -1)
  {
    ...
    */
      ...
      case e_custom_column:
        cmd_table_param.custom_column_str = optarg;
        break;
  ...
  if (!table_name || !cmd_table_param.db_name_ || !host || !port || !cmd_table_param.user_name_ || !cmd_table_param.passwd_)
  {
    usage(argv[0]);
    exit(0);
  }
  ...
  YYSYS_LOG(INFO, "ob_import command line parameters are:\n%s", command_line_str.c_str());
  //modify by pangtz:20141206 添加ObImportLogInfo logInfo参数
