...
{
  YYSYS_LOG(INFO, "set max nolog interval");
...
int64_t DbLogMonitor::get_max_avail_log()
{
  ...
  if (file.open(file_name.c_str(), O_RDONLY) < 0) {
    YYSYS_LOG(ERROR, "can't open %s", file_name.c_str());
  } else {
    char buf[kLogIdMaxLen];
    if (file.read(buf, kLogIdMaxLen) <= 0) {
      YYSYS_LOG(ERROR, "can't read %s", file_name.c_str());
...
//  yysys::CThreadGuard guard(&logs_mutex_);
  logs_cond_.lock();
...
bool DbLogMonitor::is_lastlog_of_day(int64_t logid)
{
  ...
  if (len < 0) {
    YYSYS_LOG(ERROR, "%s is too long", path_.c_str());
    return false;
  }
  ...
  if (stat(file_path, &log_stat) == 0) {
    ...
    ret = false;
    YYSYS_LOG(ERROR, "stat file error, path=%s, retcode=%d", file_path, errno);
  }
...
void DbLogMonitor::start_servic(int64_t start_log)
{
  ...
  YYSYS_LOG(INFO, "[LOG MONITOR]:today = %s", today_.c_str());
  start();
}
...
void DbLogMonitor::run(yysys::CThread *thread, void *arg)
{
  UNUSED(thread);
  UNUSED(arg);
  while(running_) {
    YYSYS_LOG(INFO, "processing logs....., %ld", interval_);
    process_logs();
...
int DbLogMonitor::get_nas_log(int64_t log_id)
{
  ...
  if (ret == OB_SUCCESS) {
    if (file_util_.system(cmd) != 0) {
      YYSYS_LOG(ERROR, "error when get log from nas");
      ret = OB_ERROR;
    }
  }
...
void DbLogMonitor::copy_logs(std::vector<int64_t> &logs)
{
  ...
  for(size_t i = 0;i < logs.size(); i++) {
    ...
    if (retries <= 0) {
      YYSYS_LOG(ERROR, "copy logs failed, check %s", cmd);
...
//  yysys::CThreadGuard guard(&logs_cond_);
  ...
  while(itr != logs.end()) {
    ...
    YYSYS_LOG(INFO, "appending log=[%ld]", log_info.log_id);
    itr++;
  }
  //logs_cond_.broadcast();
  logs_cond_.signal();
...
int DbLogMonitor::process_logs()
{
  ...
  if (max_avail_log > last_max_logid_) {
    ...
    if (now - last_nolog_time_ > max_nolog_interval_) {
      YYSYS_LOG(ERROR, "waiting too long for commitlog");
    }
  }

  YYSYS_LOG(INFO, "max avail log id = %d", max_avail_log);
...
//    printf("search files failed\n");
    YYSYS_LOG(ERROR, "search files in nas failed");
    ret = OB_ERROR;
  }
...
bool DbLogMonitor::check_continuity(std::vector<int64_t> &vec, int64_t start)
{
  ...
  while (itr != vec.end()) {
    if (*itr != start++) {
      YYSYS_LOG(WARN, "continuity violated expected=%ld, found=%ld", start - 1, *itr);
