...
{
  int ret = OB_SUCCESS;
  YYSYS_LOG(INFO, "enter comsume");

  if (NULL == block) {
     YYSYS_LOG(DEBUG, "all tablets have scanned completly");
  ...
  if( code_conversion_flag != e_non )
  {
    ...
    if( charset.fail() )
    {
      YYSYS_LOG(ERROR, "Code conversion init error.");
  ...
  {
    YYSYS_LOG(ERROR, "failed to deserialize ObSQLResultSet, ret=[%d]", ret);
    err_code_->set_err_code(OB_ERROR);//add dinggh 20161201
  }
  else
  {
    ...
    while (OB_SUCCESS == (ret = scanner.get_next_row(row)))
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to format one row, row=[%s], ret=[%d]", to_cstring(row), ret);
        break;
      }
      if (bad_record != 0)//��ʽ��ʧ��,��bufferд��bad_file_
      {
        if (bad_file_ != NULL)
        {
          if (OB_SUCCESS != (ret = bad_file_->Append(formator_->get_line_buffer(), formator_->length())))//�˴�Ϊ���߳�,���ü�����append
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "can't write to bad file");
            break;
          }
        }
        else
        {
          YYSYS_LOG(DEBUG, "no bad file, no need to write bad file");
        }
      ...
      else//��ʽ���ɹ�,��bufferд�����������ļ�
      {
          ...
          if (tablet_writers_ != NULL)
          {
              ...
              {
                  ret = OB_ERROR;
                  YYSYS_LOG(ERROR, "failed to write buffer into data file, ret=[%d]", ret);
                  break;
              }
          }
          else
    ...
    if (OB_ITER_END == ret)
    {
      YYSYS_LOG(DEBUG, "fetch ObRow from ObNewScanner OK!");
      ret = OB_SUCCESS;
      inc_total_succ_rec_count(success_format_rec_num);
      inc_total_failed_rec_count(failed_format_rec_num);
      YYSYS_LOG(INFO, "consumer has consumer %d record, %d successfull, %d failed.", 
                      success_format_rec_num + failed_format_rec_num, 
                      success_format_rec_num, failed_format_rec_num);
    }
    else if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "failed to format ObNewScanner");
      err_code_->set_err_code(OB_ERROR);//add dinggh 20161201
      //��Ҫ����error_code
    }
    else
    {
      YYSYS_LOG(INFO, "format ObNewScanner success");
    }
  }
...
int64_t ExportConsumer::inc_total_succ_rec_count(int increment)
{
  yysys::CThreadGuard guard(&total_succ_rec_count_lock_);
  total_succ_rec_count_ += increment;
  return total_succ_rec_count_;
}

int64_t ExportConsumer::inc_total_failed_rec_count(int increment)
{
  yysys::CThreadGuard guard(&total_failed_rec_count_lock_);
