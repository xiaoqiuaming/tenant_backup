...
class DbThreadMgr : public CDefaultRunnable {
  public:
    ...
    ~DbThreadMgr() {
      YYSYS_LOG(INFO, "Inserted Lines = %ld, Writen Lines = %ld", inserted_lines_, writen_lines_);
      YYSYS_LOG(INFO, "all keys size %zu", all_keys_.size());
    }
    ...
    int insert_key(const ObRowkey& key, uint64_t table_id, int op, uint64_t timestamp, int64_t seq) {
      ...
      if (clone_rowkey(key, new_key) != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "clone rowkey failed, ret = %d", ret);
        ret = OB_ERROR;
      } else {
        TableRowkey tab_key;
    ...
    void dump_keys(TableRowkey *keys, int64_t key_nr, const char *msg) {
      ...
      for(int64_t i = 0;i < key_nr; i++) {
        ...
        YYSYS_LOG(INFO, "[%s]:rowkey=%s, op=%d, table_id=%ld", msg, buffer, keys[i].op, keys[i].table_id);
      }
    }
    ...
    int init(DbDumper *dumper, int muti_get_nr = kMutiGetKeyNr) {
      ...
      YYSYS_LOG(INFO, "[DbThreadMgr]:muti_get_nr=%d", muti_get_nr_);
      ...
      while(true) {
        ...
        if (do_work) {
          ...
          while(max_retries-- > 0) {
            ...
            YYSYS_LOG(INFO, "db_thread_mgr backoff sleeping, time=[%d]", back_off);
            sleep(back_off);
          ...
          if (ret != OB_SUCCESS) {
            //TODO:dump rowkey
            YYSYS_LOG(ERROR, "Max Retries, skipping records, num=%d", muti_key_nr);
    ...
    int start(int max_thread) {
      ...
      YYSYS_LOG(INFO, "start thread mgr with thread nr [%d]", max_thread);
    ...
    void wait_completion() {
      bool empty = false;
      YYSYS_LOG(INFO, "start waiting thread mgr");
      ...
      while (true) {
        ...
        if (empty) {
          YYSYS_LOG(INFO, "finish waiting thread mgr");
          break;
        }
        usleep(10000);
      }
