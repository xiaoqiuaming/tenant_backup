...
int FileWriter::init_file_name(const char *filename)
{
  int ret = 0;
  if (NULL == filename) {
    YYSYS_LOG(ERROR, "input filename is NULL pointer");
    return -1;
  }
  ...
  if (NULL == file_) {
    ret = -1;
    YYSYS_LOG(ERROR, "can't open file, name=%s", filename_);
  }
...
{
  yysys::CThreadGuard guard(&file_lock_);
  int ret = 0;

  if (NULL == buffer || length <= 0) {
    YYSYS_LOG(ERROR, "records buffer is NULL or empty");
    return -1;
  }
  //首先判断是否要分割新文件
  ...
  if (count != 1) {
    YYSYS_LOG(DEBUG, "write records buffer into file[%s] failed", filename_);
    ret = -1;
  } else {
    cur_file_size_ += length;
    cur_record_num_ += num;
    YYSYS_LOG(DEBUG, "write reocrds buffer into file, buffer length=[%ld], record num=[%ld], cur file size=[%ld], cur record num=[%ld]", length, num, cur_file_size_, cur_record_num_);
  }
  return ret;
}
