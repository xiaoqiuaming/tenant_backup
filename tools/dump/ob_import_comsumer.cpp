...
int ImportComsumer::comsume(RecordBlock &obj)
{
    ...
    if (NULL == tnx) {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "can't create new transcation");
    }
    else {
      tnx->set_db(db_);
      tnx->reset();
    ...
    if (ret == OB_SUCCESS) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "ObRowBuilder can't build tnx");
      }
    }
    ...
    if (ret == OB_SUCCESS) {
	  ...
	  error_arr[5] = false; //add by pangtz:20141205
        YYSYS_LOG(ERROR, "commit transcation error");
      }
    } else {
      YYSYS_LOG(ERROR, "error ocurrs, so aborting transcation");
