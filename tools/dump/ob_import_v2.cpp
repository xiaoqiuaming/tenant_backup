...
int ObRowBuilder::parse_decimal_2_varchar_grammar()
{
  ...
  for( ; it != vs.end(); it++ )
  {
    ...
    if( i>=columns_desc_nr_ )
    {
      ...
      YYSYS_LOG(ERROR, "Substring error! Fail to find column[%s].",it->c_str());
      break;
    }
    else
...
int ObRowBuilder::fill_columns_with_char_delima_set()
{
  ...
  if( OB_SUCCESS != ret )
  {
    ...
    YYSYS_LOG(ERROR, "A column with char delima doen't exist!");
  }
  else
  {
    std::vector<int>::iterator it = vi.begin();
...
int ObRowBuilder::adjust_substr_offset()
{
  ...
  for(; it != table_param_.substr_map.end(); it++)
  {
    ...
    if( i>=columns_desc_nr_ )
    {
      ...
      YYSYS_LOG(ERROR, "Substring error! Fail to find column[%s].",(it->second).column_name.c_str());
      break;
    }
  }
  return ret;
...
bool ObRowBuilder::check_valid()
{
  ...
  if (table_param_.input_column_nr <= 0) {
    YYSYS_LOG(ERROR, "input data file has 0 column");
    valid = false;
  }
#endif
                               ...
                               const int columns_desc_nr, char token_buf[]) const //mod by zhuxh:20170411 [performance optimization]
{
  ...
  for(int i=0; i<columns_desc_nr; i++)
    if( columns_desc[i].offset != ColumnInfo::DEFAULT_TOKEN_COLUMN && columns_desc[i].offset >= token_nr )//mod by zhuxh:20160307
    {
      ...
      YYSYS_LOG(ERROR,"The column number of datafile is only %d but there is a pair [table column %s,datafile column %d] in column map.", token_nr, columns_desc[i].schema->get_name(),columns_desc[i].offset+1);
      ret = OB_ERROR;
    }
  ...
  if(OB_SUCCESS == ret)
  {
    for(int i = 0; i < columns_desc_nr && OB_SUCCESS == ret; i++)
    {
      ...
      if( token_info.len > MAX_TOKEN_LENGTH )
      {
        ret = OB_ERROR;
        //mod by zhuxh:20160313 :b
        YYSYS_LOG(ERROR, "Token overflows. MAX_TOKEN_LENGTH is %d but Length of token [%.*s](1st 100 characters) is %d." ,MAX_TOKEN_LENGTH, static_cast<int>(min(token_info.len,100)),token_info.token, static_cast<int>(token_info.len));
        //mod :e
        break;
      }
      ...
      */
      ...
      switch((int)columns_desc[i].schema->get_type())
      {
        ...
        {
          int64_t value = 0;
          if (OB_SUCCESS != transform_str_to_int(token_info.token, (int)token_info.len, value))
          {
            ret = OB_ERROR;
          }
          break;
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "check format error! idx[%d], type[%s] content[%.*s]", i, ob_obj_type_str(columns_desc[i].schema->get_type()), (int)tokens[i].len, tokens[i].token);
      }
    }
  }
                                ...
                                char token_buf[]) const //mod by zhuxh:20170411 [performance optimization]
{
  ...
  for(int i = 0; i < columns_desc_nr && OB_SUCCESS == ret; i++)
  {
    ...
    if(column_info->offset == columns_desc_nr-1) //modify by zhuxh:20150811
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "config file may not be correct!");
        break;
      }
#endif
    }
...
//        const char * blanks=" \t\n\r";
        ...
        {
          while(l > 0){
            if(isspace(*p) ){ // if *p is a kind of blank char
              p++;
              l--;
            }else{
              break;
        ...
        if(table_param.has_substr)
        {
          ...
          if(it != table_param_.substr_map.end())
          {
            ...
            if( substrInfo.beg_pos<START_NUM )
            {
              ...
              YYSYS_LOG(ERROR, "Beginning of substring < %d !",START_NUM);
              break;
            }
            if( substrInfo.len<0 )
            {
              ...
              /*
              ret = OB_ERROR;
              YYSYS_LOG(ERROR, "Length of substring < 0 !");
              break;
              */
    ...
    if(OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "token format failed! columnname[%s] offset[%d] type[%s]", column_info->schema->get_name(), column_info->offset, ob_obj_type_str(column_info->schema->get_type()));
    }
  }
  ...
  while (block.next_record(slice))
  {
    ...
    if (g_gbk_encoding) {
      if (RecordDelima::CHAR_DELIMA == table_param_.delima.delima_type() && table_param_.delima.get_char_delima() < 128u) {
        ...
        YYSYS_LOG(ERROR, "Short delima or delima greater than 128 are not supported in gbk encoding.");
        break;
      }
    }
    else {
    */
    ...
    if (token_nr != table_param_.input_column_nr)
    {
      ...
      YYSYS_LOG(ERROR, "corrupted data files, please check it, input column number=%d, real nr=%d, slice[%.*s]",
    ...
    {
      ret = OB_INVALID_ARGUMENT;
      YYSYS_LOG(ERROR, "tokens_format error! row_content[%.*s]", (int)slice.size(), slice.data());
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "the real value is not mattching with the column_desc! input_values count[%d], column_desc count[%d]", tokens_out_nr, columns_desc_nr_);
      }
    }
    //mod:e
    // ƴ��sql���
    if(OB_SUCCESS == ret && table_param_.is_insert)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "construct insert sql error! row_content[%.*s]", (int)slice.size(), slice.data());
      }
    }
    else if(OB_SUCCESS == ret && table_param_.is_replace)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "construct replace sql error! row_content[%.*s]", (int)slice.size(), slice.data());
      }
    }
    else if(OB_SUCCESS == ret && table_param_.is_delete)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "construct delete sql error! row_content[%.*s]", (int)slice.size(), slice.data());
      }
    }
                                  ...
                                  const ObString table_name)
{
  ...
  if(ret)
  {
    ...
    for(int i = 0; i < rowkey_info.get_size() && ret; i++)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "find key name[%s] column_id[%ld] failed!", columns_desc[i].schema->get_name(), columns_desc[i].schema->get_id());
        break;
      }
    }
  }
    ...
    ObString &sql)
{
  ...
  else
  {
    ...
    int32_t pos = 0;
    int32_t tokens_len = 0;

    if( OB_SUCCESS != (ret = tokens_2_str(
                         tokens, token_nr, columns_desc,
                         rowkey_offset, rowkey_offset_nr, tokens_str, pos))
        )
    {
    ...
    else
    {
      ...
      if(
         static_cast<int64_t>(
           ...
           +strlen("\" WHERE ")+strlen(columns_str)+strlen(" IN (")+ tokens_len +strlen(")")
           )
         >=MAX_CONSTRUCT_SQL_LENGTH
         ||
         static_cast<int64_t>(
           sql.length()+strlen(",")+tokens_len+strlen(")")
           )
         >=MAX_CONSTRUCT_SQL_LENGTH
         )
      {
      ...
      else
      {
        ...
        else
        {
          ...
          SC(columns_str);
          SC(" IN (");
          pos += static_cast<int32_t>(strlen("DELETE FROM \"") + table_name.length() + strlen("\" WHERE ")
                                      + strlen(columns_str) + strlen(" IN ("));
        }

        MC(tokens_str, tokens_len);
        pos += tokens_len; // [729]
        MC(")", 1);
        pos++;
      }
      ObString::obstr_size_t val_len = pos;
      if( sql_not_empty )
      {
    ...
    )
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "Token[%.*s] is an unknown data.", static_cast<int>(token->len), token->token);
    //YYSYS_LOG(ERROR, "\nA0:%d A1:%d\nB0:%d B1:%d\nC0:%d C1:%d", A0, A1, B0, B1, C0, C1);
    //YYSYS_LOG(ERROR, "Token[%.*s]", static_cast<int>(token->len), token->token);
  }
  ...
  else
  {
    ...
    else
    {
      ...
      if (table_param_.is_hex_column[offset])
      {
        MC("x", 1);
        pos++;
        if (!add_quotation_mark)
        {
          MC("\'", 1);
          pos++;
        }
      ...
      for(int tl=static_cast<int>(token->len) - with_quotation_mark; ti < tl; ti++)
      {
      ...
      if(table_param_.is_hex_column[offset] && !add_quotation_mark)
      {
        MC("\'", 1);
        pos++;
      }
    ...
    )
{
  ...
  for(i=0;i<token_nr;i++)
  {
    ...
    if( OB_SUCCESS != add_token_2_str(tokens+i,columns_desc+i,i,str, pos) )
    {
      ret = OB_ERROR;
    ...
    int32_t &pos)
{
  ...
  for(j=0;j<offset_nr;j++)
  {
    ...
    if(/*i*/j>0) //mod by zhuxh:20161128
    {
      MC(",", 1);
      pos++;
    }
    if( OB_SUCCESS != add_token_2_str(tokens+i,columns_desc+i,static_cast<int>(i),str, pos) )
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "Fail when a token is transformed to string.");
      break;
    ...
    char str[])
{
  ...
  for(i=0;i<rowkey_offset_nr;i++)
  {
    ...
    {
      MC(" AND ", strlen(" AND "));
      pos += static_cast<int32_t>(strlen(" AND "));
    }
    MC(columns_desc[j].schema->get_name(), strlen(columns_desc[j].schema->get_name()));
    MC("=", 1);
    if( OB_SUCCESS != add_token_2_str(tokens+j,columns_desc+j,(int)j,str, pos) )
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "Fail when a token is transformed to string.");
      break;
    ...
    ObString &sql)
{
  ...
  else
  {
    ...
    int32_t tokens_len = 0;
    if( OB_SUCCESS !=
        (ret = tokens_2_str(tokens,token_nr,columns_desc,tokens_str, pos)) )
    {
    }
    else
    {
      ...
      if(
         static_cast<int64_t>(
           strlen(str)+strlen(",")+ tokens_len
           )>=MAX_CONSTRUCT_SQL_LENGTH
         ||
         static_cast<int64_t>(
           strlen(function_name)+strlen(" INTO ")+table_name.length()
           +strlen(columns_str)+strlen(" VALUES")+ tokens_len
           )>=MAX_CONSTRUCT_SQL_LENGTH
         )
      {
        YYSYS_LOG(ERROR, "SQL overflows");
      ...
      else
      {
        ...
        else
        {
          ...
          SC(columns_str);
          SC(" VALUES");
          pos += static_cast<int32_t>(strlen(function_name) + strlen(" INTO \"") + table_name.length()
                + strlen("\"") + strlen(columns_str) + strlen(" VALUES"));
        }
        MC(tokens_str, tokens_len);
        pos += tokens_len;
        ret = OB_SUCCESS;
      }
      //YYSYS_LOG(INFO, "%s", str);
      ObString::obstr_size_t val_len = pos;
      if(val_len != sql.write(str, val_len))
      {
        YYSYS_LOG(ERROR, "SQL overflows");
    ...
    switch( grammar[i] )
    {
      ...
      case SEMICOLON:
        ...
        else if( is_digit ) //datafile column id
        {
          ...
          {
            YYSYS_LOG(ERROR, "Datafile column can't not be set when sequence function is used!");
            return OB_ERROR;
          }
          //mod :e
          else
