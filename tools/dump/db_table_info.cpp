...
namespace oceanbase {
  namespace api {
    ...
    int TabletInfo::assign_end_key(const ObRowkey &key) 
    {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(WARN, "deep copy rowkey failed, ret = %d", ret);
      }
      return ret;
    }
    ...
    int TabletInfo::parse_one_cell(const common::ObCellInfo *cell) 
    {
      ...
      if ((ret = cell->value_.get_int(value)) != common::OB_SUCCESS) {
        YYSYS_LOG(ERROR, "Cell->value_.get_int failed");
        return ret;
      }
      ...
      if (!key.compare(ROOT_OCCUPY_SIZE)) {
        ...
        if (idx < 0 || idx > 5) {
        //mod 20160718:e
          YYSYS_LOG(ERROR, "Tablet slice number is wrong");
          return -1;
        }
    ...
    void TabletInfo::dump_slice(void) 
    {
      ...
      for(int i = 0; i < 6; i++) {
      //mod 20160718:e
        YYSYS_LOG(DEBUG, "ipv4:%x, cs_port:%d, ms_port:%d, tablet_vesion:%ld, avail:%d, key:%s, size=%ld, count=%ld", 
...
//          YYSYS_LOG(WARN, "error when get cs_port");
          continue;
        }
        ...
        if (ret != OB_SUCCESS || cell->value_.get_int(value) != OB_SUCCESS) {
//          YYSYS_LOG(INFO, "error when get ms_port");
          continue;
        }
...
//          YYSYS_LOG(INFO, "error when get version");
          continue;
        }
...
//          YYSYS_LOG(INFO, "error when get ipv4");
          continue;
        }
      ...
      if (ret != common::OB_SUCCESS ||
          cell->value_.get_int(value) != common::OB_SUCCESS) {
//        YYSYS_LOG(INFO, "error when get occupy_size");
        ret = OB_ERROR;
      } else {
        ocuppy_size_ = value;
        ...
        if (ret != common::OB_SUCCESS ||
            cell->value_.get_int(value)) {
          YYSYS_LOG(INFO, "error when get record_count");
          ret = OB_ERROR;
        } else
          record_count_ = value;
      }
