...
int64_t DbDumperMgr::get_start_log()
{
  ...
  YYSYS_LOG(INFO, "init log id = %d", log_id);
  int ret = ckp_.load_checkpoint();
  if (ret == OB_SUCCESS) {
    log_id = static_cast<int32_t>(ckp_.id());
  } else {
    YYSYS_LOG(INFO, "no checkpoint info, using init_log_id=%d", log_id);
  }
                            ...
                            int64_t parser_thread_nr, int64_t worker_thread_nr)
{
  ...
  if (db_ == NULL) {
    YYSYS_LOG(ERROR, "oceanbase database can't be NULL");
    ret = OB_ERROR;
  }

  if (log_dir == "" || worker_thread_nr == 0) {
    YYSYS_LOG(ERROR, "log dir is not specified");
    ret = OB_ERROR;
  }
...
//    ret = db_->fetch_schema(schema_mgr_);
  ...
  YYSYS_LOG(INFO, "UPS commitlog path is %s", log_dir_.c_str());
  return ret;
}
...
int DbDumperMgr::start()
{
  ...
  if (dumper_ == NULL) {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "can't allocate DbDumper");
  }

  if (ret == OB_SUCCESS) {
    ret = dumper_->init(db_);
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't init dumper");
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (monitor_ != NULL) {
      ...
      YYSYS_LOG(ERROR, "can't create monitor");
      ret = OB_ERROR;
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (parser_ == NULL) {
      YYSYS_LOG(ERROR, "can't allocate parser");
      ret = OB_ERROR;
    }
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(INFO, "error ocurrs, cleaning dumper mgr stuff");
    ...
    started_ = true;
    YYSYS_LOG(INFO, "log serveice started");
  }
...
void DbDumperMgr::stop()
{
  if (started_ == true) {
    started_ = false;
    YYSYS_LOG(INFO, "stop dumper process");
    monitor_->stop_service();

    YYSYS_LOG(INFO, "stop parser");
    parser_->stop_parse();
