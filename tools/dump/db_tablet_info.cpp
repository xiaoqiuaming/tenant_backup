...
namespace oceanbase {
    namespace api {
        ...
        static const char *kClusterId = "cluster_id";//add dingh [mulups] 20170821
        ...
        int ObExportTabletInfo::build_tablet_info(const ObRowkey &start_key, DbRecord *recp)
        {
            ...
            if (ret != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "can't find rowkey from record, ret=[%d]", ret);
                return ret;
            }
        ...
        int ObExportTabletInfo::build_tablet_info(const ObRowkey &start_key, const ObRowkey &end_key, DbRecord *recp)
        {
            ...
            if ((ret = assign_start_key(start_key)) != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "assign tablet start rowkey failed, ret=[%d]", ret);
            } else if ((ret = assign_end_key(end_key)) != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "assign tablet end rowkey failed, ret=[%d]", ret);
            } else if ((ret = parse_from_record(recp)) != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "parse tablet location info failed, ret=[%d]", ret);
            }
        ...
        int ObExportTabletInfo::parse_one_cell(const common::ObCellInfo *cell)
        {
            ...
            if ((ret = cell->value_.get_int(value)) != common::OB_SUCCESS) {
                YYSYS_LOG(ERROR, "ObCellInfo, get_int() failed, ret=[%d]", ret);
                return ret;
            }
            ...
            if (!column.compare(kOccupySize)) {
                ...
                if (column.length() < 2) {//length must be higher than 2
                    YYSYS_LOG(ERROR, "invalid column in cell, column name=[%s]", column.ptr());
                    return common::OB_ERROR;
                }

                // column is "1_port", "1_ms_port", "1_ipv4", "1_tablet_version","1_cluster_id" "2_port", "2_ms_port", "2_ipv4", "2_tablet_version","2_cluster_id" ...
                ...
                //mod e
                    YYSYS_LOG(ERROR, "tablet slice location index is wrong, idx=[%d]", idx);
                    return common::OB_ERROR;
                }
                ...
                //add dinggh [for mulups] 20170821:b
                } else if (!key.compare(kClusterId)) {
                    slice_[idx].cluster_id = value;
                //add:e
                } else {
                    YYSYS_LOG(ERROR, "invalid column in cell, column name=[%s]", column.ptr());
                    return common::OB_ERROR;
        ...
        int ObExportTabletInfo::parse_from_record(DbRecord *recp)
        {
            ...
            while (iter != recp->end()) {
                ...
                if (ret != OB_SUCCESS) {
                    YYSYS_LOG(ERROR, "parse one cell failed, ret=[%d]", ret);
                    break;
                }
                  iter++;
            }
        ...
        int ObExportTabletInfo::assign_start_key(const ObRowkey &start_key)
        {
            ...
            if (ret != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "deep copy start rowkey failed, ret=[%d]", ret);
            }
        ...
        int ObExportTabletInfo::assign_end_key(const ObRowkey &end_key)
        {
            ...
            if (ret != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "deep copy end rowkey failed, ret=[%d]", ret);
            }
