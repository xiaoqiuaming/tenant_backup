...
namespace oceanbase {
  ...
  namespace api {
    ...
    int DbTranscation::commit()
    {
...
#if 0
      {
        YYSYS_LOG(INFO, "dumping obmutator, mutator.size = %ld", mutator_.size());
        mutator_.reset_iter();
        while (mutator_.next_cell() == OB_SUCCESS)
        {
          ...
          if (mutator_.get_cell(&cell) == OB_SUCCESS)
          {
            ...
            YYSYS_LOG(INFO, "table[%.*s] column[%.*s]: value: %s", table_name.length(), table_name.ptr(), 
      ...
      {
        YYSYS_LOG(ERROR, "can't allocate from TSI");
        ret = OB_ERROR;
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "can't serialize mutator, due to %d. mutator size[%ld]", ret, mutator_.size());
        }
        else
        {
          int64_t retries = 0;
          YYSYS_LOG(DEBUG, "mutator serialize size[%ld]", buffer.get_position());

          while (true) 
          {
            if (retries > 2) //retry 2 times
            {
              YYSYS_LOG(ERROR, "fail to write to ups[%d]", ret);//mod by liyongfeng: modify WARN into ERROR
              break;
            }
            int64_t pos = 0;
            ...
            if (OB_ALLOCATE_MEMORY_FAILED == ret || OB_MEM_OVERFLOW == ret)//mod by liyongfeng for UPS memory overflow
            {
                ...
                YYSYS_LOG(WARN, "Updateserver memory is not enough, sleep 60s and wait ups release memory, then retry, ret=%d", ret);
                buffer = tmp_buffer;
                int err = mutator_.serialize(buffer.get_data(), buffer.get_capacity(), buffer.get_position());
                if(err != OB_SUCCESS) {
                    YYSYS_LOG(ERROR, "can't serialize mutator, due to %d", err);
                    ret = err;
                    break;
                }
            }
            ...
            {
                retries++;
                YYSYS_LOG(WARN, "Write to OB failed, ret=%d retry_time=%ld", ret, retries); // mod by liyongfeng: modify ERROR into WARN.
                sleep(5);//mod:e
                buffer = tmp_buffer;
                int err = mutator_.serialize(buffer.get_data(), buffer.get_capacity(), buffer.get_position());
                if (err != OB_SUCCESS) {
            ...
            YYSYS_LOG(WARN, "retring write to database, buffer size = %ld", pos);
          }
        }
        ...
        if (ret != OB_SUCCESS)
        {
          ...
          YYSYS_LOG(ERROR, "can't write to update server, due to [%d], server:%s", ret, server_buf);
        }
      }
...
//    root_server_.set_ipv4_addr(ip, port);
      ...
      YYSYS_LOG(INFO, "%s:%d, timeout is %ld, tablet_timeout is %ld", ip, port, timeout_, tablet_timeout_);
      db_ref_ = 1;
      consistency_ = true;
    ...
    int OceanbaseDb::set_master_rs()
    {
      int ret = OB_SUCCESS;
      bool is_master_rs;
      std::string rs_ips;
      std::vector<std::string> vec;
      rs_ips.assign(rs_ip_list_);
      splite_string(rs_ips, vec);

      for (int i = 0; i < static_cast<int>(vec.size()); i++)
      {
        YYSYS_LOG(DEBUG, "ip=%s", vec[i].c_str());
        if (OB_SUCCESS != (ret = check_is_master_rs(vec[i].c_str(), rs_port_, is_master_rs)))
        {
          YYSYS_LOG(WARN, "check is_master_rs failed,ip=[%s],ret=[%d]", vec[i].c_str(), ret);
        }
        else if (is_master_rs)
        {
          YYSYS_LOG(INFO, "ip=[%s] is_master_rs ip!", vec[i].c_str());
          master_rs_ip_ = vec[i].c_str();
          root_server_.set_ipv4_addr(master_rs_ip_, rs_port_);
          client_.set_server(root_server_);
          break;
        }
      }
      return ret;
    ...
    void OceanbaseDb::splite_string(std::string &info, std::vector<std::string> &vec)
    {
      char splitchar = ',';
      std::string empty("");
      ...
      for (size_t i = 0; i < length; i++)
      {
        ...
        {
          vec.push_back(empty);
          YYSYS_LOG(DEBUG, "vec=[%s]", vec[i].c_str());
          start += 1;
        }
        else if (info[i] == splitchar)
        {
          vec.push_back(info.substr(start, i - start));
          YYSYS_LOG(DEBUG, "vec=[%s]", info.substr(start, i - start).c_str());
          start = i + 1;
          if (i == length - 1)
          {
            vec.push_back(empty);
        ...
        else if (i == length - 1)
        {
          ...
          YYSYS_LOG(DEBUG, "vec=[%s]", info.substr(start, i + 1 - start).c_str());
        }
      }
    }

    int OceanbaseDb::check_is_master_rs(const char *ip, unsigned short port, bool &is_master_rs)
    {
      int ret = OB_SUCCESS;
      ObServer root_server;
      root_server.set_ipv4_addr(ip, port);

      char *buff = new(std::nothrow) char[k2M];
      if (buff == NULL)
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "can't allocate memory from heap");
      }
      else
      {
        ...
        {
          if (OB_SUCCESS == (ret = do_server_cmd(root_server, (int32_t)OB_CHECK_IS_MASTER_RS, buffer, pos)))
          {
            break;
          }
          else
          {
            YYSYS_LOG(WARN, "the %d time check is_master_rs failed", i + 1);
          }
          sleep(1);
        }
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "check is_master_rs failed, errorcode=%d", ret);
        }
        else if (OB_SUCCESS != (ret = serialization::decode_bool(buffer.get_data(), buffer.get_position(),
                                                                 pos, &is_master_rs)))
        {
          YYSYS_LOG(WARN, "deserialize is_master_rs failed!");
        }
      }
      if (buff != NULL)
      {
    ...
    int OceanbaseDb::set_limit_info(const int64_t offset, int64_t count)
    {
      ...
      if (offset < 0 || count < 0)
      {
        YYSYS_LOG(WARN,"set limit info error [offset:%ld,count:%ld]", offset, count);
        err = OB_ERROR;
      }
      if (OB_SUCCESS == err)
      {
    ...
    int OceanbaseDb::get(const std::vector<DbMutiGetRow> &rows, DbRecordSet &rs)
    {
      ...
      if (!rs.inited()) {
        YYSYS_LOG(INFO, "DbRecordSet Not init ,please init it first");
        ret = common::OB_ERROR;
      }
      ...
      while (retries--) {
        ...
        if (ret != common::OB_SUCCESS) {
          YYSYS_LOG(ERROR, "No Mergeserver available");
          break;
        }
        char server_str[128];
        ...
        if (ret == OB_SUCCESS) {
          YYSYS_LOG(DEBUG, "using merger server %s", server_str);
          break;
        } else {
          YYSYS_LOG(WARN, "failed when get data from %s", server_str);
          if (ret == OB_ERROR_OUT_OF_RANGE) {
            YYSYS_LOG(ERROR, "rowkey out of range");
            break;
          }
        ...
        const std::vector<ObRowkey> &rowkeys, DbRecordSet &rs)
    {
      ...
      while (retries--) {
        ...
        ret = do_muti_get(server, rows, rs.get_scanner(), rs.get_buffer());
        if (ret == OB_SUCCESS) {
          YYSYS_LOG(INFO, "using mergeserver %s", server_str);
          break;
        } else {
          YYSYS_LOG(WARN, "failed when get data from %s", server_str);
          if (ret == OB_ERROR_OUT_OF_RANGE) {
        ...
        const ObRowkey &rowkey, DbRecordSet &rs)
    {
      ...
      while (retries--) {
        ...
        if (ret == OB_SUCCESS) {
          ...
          YYSYS_LOG(WARN, "failed when get data from %s", err_msg);
          if (ret == OB_ERROR_OUT_OF_RANGE) {
            YYSYS_LOG(ERROR, "rowkey out of range");
            break;
          }
    ...
    int OceanbaseDb::init()
    {
      ...
      if (OB_SUCCESS != (ret = set_master_rs()))
      {
        YYSYS_LOG(ERROR, "can't set master root server");
      }
      else if ( OB_SUCCESS != ( ret = get_update_server(server) ) )
      {
        YYSYS_LOG(ERROR, "can't get update server");
      }
      //add by zhuxh:20160122 :b
      else if ( OB_SUCCESS != (ret = init_slave_cluster_info() ) )
      {
        YYSYS_LOG(ERROR, "Can't get slave clusters' info.");
      }
      //add zhuxh:e
      return ret;
    }
    ...
    int OceanbaseDb::global_init(const char *log_dir, const char * level)
    {
      UNUSED(log_dir);
      ob_init_memory_pool();
      //YYSYS_LOGGER.setFileName(log_dir);
      YYSYS_LOGGER.setMaxFileSize(kMaxLogSize);
      YYSYS_LOGGER.setLogLevel(level); 
      ob_init_crc64_table(OB_DEFAULT_CRC64_POLYNOM);
      YYSYS_LOG(INFO, "init oceanbase global config");
      return OB_SUCCESS;
    }

    int OceanbaseDb::search_tablet_cache(const std::string &table, const ObRowkey &rowkey, TabletInfo &loc)
    {
      yysys::CThreadGuard guard(&cache_lock_);
    ...
    void OceanbaseDb::mark_ms_failure(ObServer &server, const std::string &table, const ObRowkey &rowkey)
    {
      ...
      if (ret == OB_SUCCESS) {
        ...
        YYSYS_LOG(WARN, "tablet updated, no such rowkey");
      }
    }
        ...
        common::ObServer &server)
    {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(DEBUG, "Table %s not find in tablet cache, do root server get", table.c_str());

        ret = get_ms_location(rowkey, table);
        if (ret == OB_SUCCESS)
          ret = search_tablet_cache(table, rowkey, tablet_info);
        else {
          YYSYS_LOG(WARN, "get_ms_location faild , retcode:[%d]", ret);
        }
      }
      ...
      if (ret == common::OB_SUCCESS) {
        ...
        if (ret == common::OB_SUCCESS) {
          ...
          YYSYS_LOG(ERROR, "No available Merger Server");
        }
      }
    ...
    int OceanbaseDb::get_ms_location(const ObRowkey &row_key, const std::string &table_name)
    {
      ...
      if (!inited_) {
        YYSYS_LOG(ERROR, "OceanbaseDb is not Inited, plesase Initialize it first");
        ret = OB_ERROR;
      } else if ((ret = ds.init()) != common::OB_SUCCESS) {
        YYSYS_LOG(INFO, "DbRecordSet Init error");
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "do server get failed, ret=[%d]", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        while (itr != ds.end()) {
          ...
          itr.get_record(&recp);
          if (recp == NULL) {
            YYSYS_LOG(WARN, "NULL record skip line");
            itr++;
            continue;
          }
          ...
          if (ret != OB_SUCCESS) {
            YYSYS_LOG(ERROR, "pase from record failed");
            break;
          }
    ...
    void OceanbaseDb::insert_tablet_cache (const std::string &table, const ObRowkey &rowkey, TabletInfo &tablet)
    {
      ...
      if (set_itr == cache_set_.end()) {
        ...
        if (row_itr != set_itr->second.end()) {
          ...
          YYSYS_LOG(DEBUG, "deleting cache table is %s", table.c_str());
        } 

        YYSYS_LOG(DEBUG, "insert cache table is %s", table.c_str());
        ...
        ObScanner &scanner, ObDataBuffer& data_buff)
    {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(WARN, "can't init get_param");
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(WARN, "can't deserialize get param");
        }
      }
      ...
      for(int64_t i = 0;i < get_param->get_cell_size(); i++) {
        ...
        YYSYS_LOG(INFO, "tablet=%s,Column=%s, Rowkey=%s", table_name.c_str(), column_name.c_str(), buf);
        (*get_param)[i]->value_.dump(YYSYS_LOG_LEVEL_INFO);
      }
#endif
#if 0
      {
        ...
        YYSYS_LOG(INFO, "Assessing Merge server, %s", buf);
      }
#endif
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(WARN, "can't do server cmd, ret=%d", ret);
        } else {
          scanner.clear();

          ret = scanner.deserialize(data_buff.get_data(), data_buff.get_position(), pos);
          if (ret != OB_SUCCESS) {
            YYSYS_LOG(WARN, "ob scanner can't deserialize buffer, pos=%ld, ret=%d",pos, ret);
          }
        }
      }
    ...
    int OceanbaseDb::init_get_param(ObGetParam *&param, const std::vector<DbMutiGetRow> &rows)
    {
      ...
      if (get_param == NULL) {
        YYSYS_LOG(ERROR, "can't allocate memory from TSI");
        return OB_ERROR;
      }
      ...
      for(size_t i = 0;i < rows.size(); i++) {
        ...
        while(itr != rows[i].columns->end()) {
          ...
          if (ret != OB_SUCCESS) {
            YYSYS_LOG(ERROR, "add cell to get param failed:ret[%d]", ret);
            return ret;
          }
        ...
        const std::vector<std::string> &columns)
    {
      ...
      if (OB_SUCCESS == ret) {
        ...
        int32_t cluster_id = 0; // mod lqc [multiusp] 20170702
        if (OB_SUCCESS == ret)
        {
        ...
        YYSYS_LOG(WARN, "serialzie get param failed, ret=%d", ret);
      }
      ...
      YYSYS_LOG(DEBUG, "Merger server ip is %s, port is %d", ip_buf, server.get_port());
      ...
      if (OB_SUCCESS != ret)
        YYSYS_LOG(ERROR, "do_server_get deserialize result failed:pos[%ld], ret[%d]", pos, ret);
      else
      ...
      if (OB_SUCCESS == ret) {
        ...
        ret = scanner.deserialize(data_buff.get_data(), data_buff.get_position(), pos);
        if (OB_SUCCESS != ret)
          YYSYS_LOG(ERROR, "deserialize scanner from buff failed:pos[%ld], ret[%d]", pos, ret);
      }

      if (ret != OB_SUCCESS)
        YYSYS_LOG(ERROR, "do server get failed:%d, server=%s, table_name=%s", ret, ip_buf, table_name.c_str());
        ...
        bool inclusive_start, bool inclusive_end)
    {
      ...
      if (end_key.ptr() == NULL || end_key.length() == 0) {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "scan with null end key");
      }
        ...
        for(size_t i = 0;i < (size_t)kTabletDupNr; i++) {
          if (tablets.slice_[i].server_avail == true) {
            ...
            ret = param->serialize(data_buff.get_data(), data_buff.get_capacity(),
                data_buff.get_position());
            if (ret != OB_SUCCESS) {
              YYSYS_LOG(WARN, "can't serialize scan param to databuf, ret=%d", ret);
            } else {
              int64_t pos = 0;
              ...
              if (ret == OB_SUCCESS) {
                ...
                ret = scanner.deserialize(data_buff.get_data(), data_buff.get_position(), pos);
                if (ret != OB_SUCCESS) {
                  YYSYS_LOG(WARN, "can't deserialize obscanner, ret=%d", ret);
                }
              } else {
                YYSYS_LOG(WARN, "error when calling do_server_cmd, ret=[%d], server:%s", ret, server_str);
              }
            }

            if (ret == OB_SUCCESS) {
              YYSYS_LOG(DEBUG, "scan requesting server, %s", server_str);
              break;
            }
        ...
        int64_t data_version)
    {
      ...
      YYSYS_LOG(DEBUG, "scan param version is %ld", data_version);
      param->reset();
      ...
      for(size_t i = 0;i < columns.size(); i++) {
        ...
        if (param->add_column(column) != OB_SUCCESS) {
          param = NULL;
          YYSYS_LOG(ERROR, "can't add column to scan param");
          break;
        }
      }
    ...
    int OceanbaseDb::fetch_schema(common::ObSchemaManagerV2& schema_manager)
    {
      ...
      char *buff = new(std::nothrow) char[k2M];
      if (buff == NULL) {
        YYSYS_LOG(ERROR, "Fetch schema faild, due to memory lack");
      ...
      if (OB_SUCCESS != ret)
        YYSYS_LOG(ERROR, "deserialize result failed:pos[%ld], ret[%d]", pos, ret);
      else
        ret = result_code.result_code_;
      ...
      if (OB_SUCCESS == ret) {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "deserialize schema from buff failed:"
              "version[%d], pos[%ld], ret[%d]", 1, pos, ret);
        }
        else
        {
          YYSYS_LOG(DEBUG, "fetch schema succ:version[%ld]", schema_manager.get_version());
        }
      }
    ...
    int OceanbaseDb::get_merge_ups_memory(int32_t &state)
    {
      int ret = OB_SUCCESS;
      state = -1;
      char *buff = new(std::nothrow) char[k2M];
      if (buff == NULL) {
        YYSYS_LOG(ERROR, "get merge state failed, due to memory lack");
        return common::OB_MEM_OVERFLOW;
      }
      ...
      for (i = 0; OB_SUCCESS == ret && i < 1; i++)
      {
        ObDataBuffer msgbuf;
        msgbuf.set_data(buff, k2M);
        msgbuf.get_position() = 0;
        int64_t ups_memory = 0;
        if (OB_SUCCESS != (ret = serialization::encode_vi32(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position(), -1))) {
          YYSYS_LOG(ERROR, "failed to serialize ObDataBuffer, err=%d", ret);
        }
        else if (OB_SUCCESS != (ret = aoc[i]->send_recv(OB_SEND_GET_UPS_MEM, MY_VERSION, timeout_, msgbuf))) {
          YYSYS_LOG(ERROR, "failed to send request, pcode=%d, err=%d", OB_SEND_GET_UPS_MEM, ret);
        } else {
          ObResultCode result_code;
          msgbuf.get_position() = 0;
          if (OB_SUCCESS != (ret = result_code.deserialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position()))) {
            YYSYS_LOG(ERROR, "failed to deserialize result from response, pos=%ld, err=%d", msgbuf.get_position(), ret);
          }
          else if (OB_SUCCESS != (ret = result_code.result_code_)) {
            YYSYS_LOG(ERROR, "failed to get daily merge state, err=%d", result_code.result_code_);
          } else {
            serialization::decode_vi64(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position(),
                                       &ups_memory);
            YYSYS_LOG(INFO, "current ups memory [%ld]", ups_memory);
            YYSYS_LOG(INFO, "db limit ups memory [%ld]", import_limit_ups_memory);
            if (ups_memory > import_limit_ups_memory) {
              state = 0;
            } else {
              state = 1;
            }
    ...
    int OceanbaseDb::get_daily_merge_state(int32_t &state)
    {
        ...
        //mod dinggh [for multiups_paxos] 20170801:b
        //old code
        //for( i=0 ; OB_SUCCESS == ret && i<arr_slave_client_length+1; i++ )
        for(i = 0; OB_SUCCESS == ret && i < 1; i++)//only need check merge state of master rs
        //mod:e
        {
          ObDataBuffer msgbuf;
          msgbuf.set_data(buff, k2M);
          ...
          YYSYS_LOG(INFO, "Checking merge state: %s, merge checking array index=%d", (i==0?"cluster master":"cluster slave"), i);
          if(OB_SUCCESS != (ret = serialization::encode_vi32(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position(), 32))) {
              ...
              YYSYS_LOG(ERROR, "failed to send request, pcode=%d, err=%d", OB_RS_STAT, ret);
          } else {
              ObResultCode result_code;
              msgbuf.get_position() = 0;
              if(OB_SUCCESS != (ret = result_code.deserialize(msgbuf.get_data(), msgbuf.get_capacity(), msgbuf.get_position()))) {
                  ...
                  YYSYS_LOG(INFO, "current daily merge state [%s]", merge_info);
                  ...
                  if( i>0 )
                  {
                    ...
                    YYSYS_LOG(INFO, "cluster %d is thought to merge for testing by zhuxh on 20160119.",i+1);
                  }
                  */
                  //YYSYS_LOG(INFO, "cluster %d",i);
                  //ZXH TEST }
    ...
    {
      int ret = OB_SUCCESS;
      //modify huangcc 20170818:b
      const int64_t timeout = 3000000;
      if(opcode== OB_GET_UPDATE_SERVER_INFO
         || OB_CHECK_IS_MASTER_RS == opcode)
      {
        ret = client_.get_client_mgr().send_request(server, opcode, 1, timeout, inout_buffer);
      }
      else
      {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "deserialize result failed:pos[%ld], ret[%d]", pos, ret);
      }
      else
      {
        ret = result_code.result_code_;
    ...
    int OceanbaseDb::get_memtable_version(int64_t &version)
    {
      ...
      if (buff == NULL) {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "can't allocate memory for get_memtable_version");
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't get memtable version errorcode=%d", ret);
        } else {
          ret = serialization::decode_vi64(buffer.get_data(), 
              buffer.get_position(), pos, &version);
          if (ret != OB_SUCCESS) {
            YYSYS_LOG(ERROR, "can't decode version, errorcode=%d", ret);
          }
        }
      }
    ...
    int OceanbaseDb::get_update_server(ObServer &server)
    {
      ...
      if (ret == OB_SUCCESS)
      {
        ObDataBuffer buffer(buff, OB_MAX_PACKET_LENGTH);
        int64_t pos = 0;
        //modify huangcc[ob_export get_update_server timeout] 20170818:b
        int retry_times=3;
        for(int i=0;i<retry_times;i++)
        {
          if(OB_SUCCESS==(ret = do_server_cmd(root_server_, (int32_t)OB_GET_UPDATE_SERVER_INFO, buffer, pos)))
          {
            break;
          }
          else
          {
            YYSYS_LOG(WARN, "the %d time get updateserver failed", i+1);
          }
          usleep(50000);
        }
        //modify:e
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "can't get updateserver errorcode=%d", ret);
        }
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "can't get updateserver");
          }
          else
          {
    ...
    int OceanbaseDb::get_latest_update_server(int32_t &ups_switch)
    {
        ...
        if(NULL == buff) {
            ret = OB_MEM_OVERFLOW;
            YYSYS_LOG(ERROR, "can't allocate memory from heap, err=%d", ret);
        }
        ...
        if (OB_SUCCESS == ret) {
            ...
            if (ret != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "send request to get latest update server failed, err=%d", ret);
            } else {
                ret = latest_update_server.deserialize(buffer.get_data(), buffer.get_position(), pos);
                if (ret != OB_SUCCESS) {
                    YYSYS_LOG(ERROR, "failed to deserialize to get latest update server, err=%d", ret);
                } else {
                    //比较前后两次获取的UPS,包括成为主UPS的时间
                    if (latest_update_server == update_server_) {//如果前后两次UPS相同,需要判断前后两次成为主UPS的时间戳
                        ups_switch = 0;
                    } else {
                        YYSYS_LOG(ERROR, "master update server has switched from server[%s] to server[%s]", update_server_.to_cstring(), latest_update_server.to_cstring());
                        ups_switch = 1;
                    }
                }
            }
    ...
    int OceanbaseDb::start_tnx(DbTranscation *&tnx)
    {
      ...
      if (tnx == NULL) {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "can't allocat transcation class");
      } else {
        ref();
      }
    ...
    {
      YYSYS_LOG(INFO, "enter fetch ms list");
      int ret = OB_SUCCESS;
      ...
      {
        YYSYS_LOG(ERROR, "get the first ms from rs failed!");
        return ret;
      }
      ...
      select_sql_vec.push_back("select svr_ip, inner_port from __all_server a inner join __all_cluster b on a.cluster_id=b.cluster_id where svr_type='yaosqlsvr' and cluster_role=1");
      select_sql_vec.push_back("select svr_ip, inner_port from __all_server a inner join __all_cluster b on a.cluster_id=b.cluster_id where svr_type='yaosqlsvr' and cluster_role=2");
      ...
      for(unsigned int i = 0; i < select_sql_vec.size(); i++)
      {
        ...
        YYSYS_LOG(INFO, "begin send execute sql");
        YYSYS_LOG(INFO, "connect ms: %s", to_cstring(ms));
        if(OB_SUCCESS != (ret = send_execute_sql(ms, select_sql, 60000000, session_id, rs, out_buffer, has_next)))
        {
          YYSYS_LOG(ERROR, "send execute sql failed, ret=%d", ret);
          return ret;
        }
        while(true)
        {
          YYSYS_LOG(INFO, "has next=%d", has_next);
          ...
          while(OB_SUCCESS == (ret = scanner.get_next_row(row)))
          {
            ...
            {
              YYSYS_LOG(ERROR, "failed to get ip, ret=%d", ret);
              break;
            }
            ...
            {
              YYSYS_LOG(ERROR, "failed to get port, ret=%d", ret);
              break;
            }
            else
            {
              ...
              if(OB_SUCCESS != ret)
              {
                YYSYS_LOG(ERROR, "failed to construct a record! ret=%d", ret);
                break;
              }
            }
          }
          ...
          if(has_next && OB_SUCCESS == ret)
          {
            ...
            {
              YYSYS_LOG(ERROR, "get next result failed, ret=%d", ret);
              return ret;
            }
            YYSYS_LOG(INFO, "get next result success! ret=%d", ret);
          }
          ...
          {
            YYSYS_LOG(INFO, "the result is the last!");
            break;
          }
        }
        ...
        if(0 == i)
        {
          ...
          YYSYS_LOG(INFO, "zcd.master_count:%u", count);
        }
        else if(1 == i)
        {
          count = (unsigned int)(slave_percent * all_ms.size() / 100);
          YYSYS_LOG(INFO, "zcd.slave_count:%u", count);
        }
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "no available ms exist!");
      }

      for(unsigned int i = 0; i < array.size(); i++)
      {
        YYSYS_LOG(INFO, "selected ms use:%s", to_cstring(array[i]));
      }
    ...
    {
      YYSYS_LOG(INFO, "enter fetch_mysql_ms_list");
      int ret = OB_SUCCESS;
      int64_t session_id = 0;
      ObSQLResultSet rs;
      bool has_next = false;
      ...
      std::vector<ObServer> all_ms;
      //mod dinggh [optimize] 20170825:b
...
//    std::string select_sql = "select svr_ip, svr_port from __all_server a inner join __all_cluster b on a.cluster_id=b.cluster_id where svr_type='mergeserver'";
      std::string select_sql = "select svr_ip, svr_port from __all_server a inner join __all_cluster b on a.cluster_id=b.cluster_id where cluster_flow_percent>0 and svr_type='yaosqlsvr'";
      //mod:e
      //mod by zhangcd:20150925
      //"select svr_ip, svr_port from __all_server where svr_type='yaosqlsvr' and cluster_id=1"; /* 主集群中所有ms */
    ...
    int OceanbaseDb::get_result(ObSQLResultSet &rs, ObDataBuffer &out_buffer, bool &has_next)
    {
      ...
      YYSYS_LOG(INFO, "enter get_result!");
      // 使用线程私有单例的ExecuteSqlParam对象
      if(OB_SUCCESS != (ret = get_ts_execute_sql_param(param)))
      {
        YYSYS_LOG(ERROR, "get execute sql param failed!");
        return ret;
      }

      YYSYS_LOG(TRACE, "current exe sql client_timeout left is %ld", param->client_timeout_timestamp - yysys::CTimeUtil::getTime());
      // 判断是否超时，此超时时间是当前客户端的超时时间
      if(param->client_timeout_timestamp <= yysys::CTimeUtil::getTime())
      {
        YYSYS_LOG(ERROR, "current exe sql is timeout!");
      ...
      if(param->is_first)
      {
        ...
        if(OB_SUCCESS != (ret = send_execute_sql(param->ms, param->sql, param->ms_timeout, param->session_id, rs, out_buffer, has_next,
                                                 true)))
        {
          YYSYS_LOG(ERROR, "first send execute sql failed! ret=%d", ret);
          return ret;
        }
        else
        {
          YYSYS_LOG(INFO, "first send execute sql success!");
        }
      }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "next get_result failed! ret=%d", ret);
          return ret;
        }
        else
        {
          YYSYS_LOG(INFO, "next get_result success!");
        }
      }
      return ret;
    }
                                      ...
                                      , bool omit_db)
    {
      ...
      ObString usname;
      ObString uspasswd;
      if (omit_db && NULL != table_param_)
      {
        version = 2;
        string real_table_name = table_param_->table_name;
        string::size_type split_pos = real_table_name.find('.', 0);
        tmp_db = real_table_name.substr(0, split_pos);
        db_name = ObString::make_string(tmp_db.c_str());
        YYSYS_LOG(DEBUG, "db_name=%s", db_name.ptr());
        usname = ObString::make_string((table_param_->user_name).c_str());
        uspasswd = ObString::make_string((table_param_->passwd).c_str());
      }
      ...
      {
        YYSYS_LOG(ERROR, "out buffer is not correct!");
        ret = OB_ERROR;
      }
      else if(OB_SUCCESS != (ret = ob_sql.serialize(databuf.get_data(), databuf.get_capacity(), databuf.get_position())))
      {
        YYSYS_LOG(ERROR, "failed to serialize the sql into buf. ret=%d", ret);
      }
      else if (omit_db && OB_SUCCESS != (ret = db_name.serialize(databuf.get_data(), databuf.get_capacity(), databuf.get_position())))
      {
        YYSYS_LOG(ERROR, "failed to serialize the db name into buf, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = usname.serialize(databuf.get_data(), databuf.get_capacity(), databuf.get_position())))
      {
        YYSYS_LOG(ERROR, "failed to serialize the user name into buf, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = uspasswd.serialize(databuf.get_data(), databuf.get_capacity(), databuf.get_position())))
      {
        YYSYS_LOG(ERROR, "failed to serialize the user passwd into buf, ret=%d", ret);
      }
      // 发起sql查询，向ms发送带有sql的请求包，此函数是同步函数，返回结果包含结果集中的第一个数据包，会存储在输出缓存中
      else if(OB_SUCCESS != (ret = client_.get_client_mgr().send_request(ms, OB_SQL_EXECUTE, version, ms_timeout_us, databuf, session_id)))
      {
        YYSYS_LOG(ERROR, "failed to send request. ret=%d, ms_timeout_us=%ld", ret, ms_timeout_us);
      }
      if(OB_SUCCESS != ret)
      {
        return ret;
      ...
      {
        YYSYS_LOG(ERROR, "failed to deserialize result buffer, ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = rs.get_errno()))
      {
        YYSYS_LOG(ERROR, "execute query sql failed, sql=[%s], errno=[%d], errinfo=[%s]", sql.c_str(), rs.get_errno(), ob_strerror(rs.get_errno()));
      }
      else
      {
    ...
    int OceanbaseDb::get_next_result(const ObServer& ms, const int64_t packet_timeout, ObSQLResultSet &rs, common::ObDataBuffer &out_buffer, const int64_t session_id, bool &has_next)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get next packet. err=%d", ret);
      }
      if(OB_SUCCESS != ret)
      {
        return ret;
      ...
      {
        YYSYS_LOG(ERROR, "fetch next packet failed, sql=[%s], errno=[%d], errinfo=[%s]", rs.get_sql_str().ptr(), rs.get_errno(), ob_strerror(rs.get_errno()));
      }
      else
      {
        memcpy(out_buffer.get_data(), databuf.get_data(), databuf.get_capacity());
    ...
    int OceanbaseDb::init_execute_sql(const ObServer& ms, const std::string& sql, int64_t ms_timeout, int64_t client_timeout)
    {
      ...
      if(NULL == ptr)
      {
        ...
        if(NULL == param)
        {
          YYSYS_LOG(ERROR, "malloc memory failed!");
          ret = OB_ERROR;
        }
        else
        {
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        param->sql = sql;
        param->session_id = 0;
        param->client_timeout_timestamp = yysys::CTimeUtil::getTime() + client_timeout;
    ...
    int OceanbaseDb::get_ts_execute_sql_param(struct ExecuteSqlParam *&param)
    {
      ...
      if(NULL == ptr)
      {
        YYSYS_LOG(ERROR, "get thread specific sql param failed!");
        ret = OB_ERROR;
      }
    ...
    int OceanbaseDb::get_tablet_info(const std::string &table, const ObRowkey &rowkey, ObDataBuffer &data_buff, ObScanner &scanner)
    {
        ...
        if (!inited_) {
            YYSYS_LOG(ERROR, "OceanbaseDb is not inited, please initialize it first");
            return OB_ERROR;
        }
        ...
        if (OB_SUCCESS == ret) {
            ...
            if (ret != OB_SUCCESS) {
                YYSYS_LOG(ERROR, "do server get failed, server[%s], ret=[%d]", root_server_.to_cstring(), ret);
            }
        }
        return ret;
    }
    ...
    int OceanbaseDb::get_a_ms_from_rs(ObServer &out_ms)
    {
      ...
      if (OB_SUCCESS != ret) 
      {
        YYSYS_LOG(ERROR, "can't do server cmd, ret=%d", ret);
      }
      else 
      {
        data_buff.get_position() = 0;
        ...
        else
        {
          ...
          if(OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "decode ms num fail:ret[%d]", ret);
          }
          else if(ms_num <= 0)
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "no ms exist!");
          }
          else
          {
            ...
            for(int32_t i = 0;i < ms_num && OB_SUCCESS == ret; i++)
            {
              if (OB_SUCCESS != (ret = ms.deserialize(data_buff.get_data(),
                                                      ...
                                                      data_buff.get_position())))
              {
                YYSYS_LOG(ERROR, "deserialize merge server fail, ret: [%d]", ret);
              }
              else if (OB_SUCCESS !=
                       (ret = serialization::decode_vi64(data_buff.get_data(),
                                                         ...
                                                         &reserved)))
              {
                YYSYS_LOG(ERROR, "deserializ merge server"
    ...
    int OceanbaseDb::get_thread_server_session(ObServer &server, int64_t& session_id)
    {
      ...
      if(OB_SUCCESS != (ret = get_ts_execute_sql_param(param)))
      {
        YYSYS_LOG(ERROR, "get_ts_execute_sql_param failed! ret=%d", ret);
      }
      else
      {
        server = param->ms;
    ...
    int OceanbaseDb::post_end_next(ObServer server, int64_t session_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_thread_buffer failed! ret=%d", ret);
      }
      else if(OB_SUCCESS != (ret = client_.get_client_mgr().post_end_next(server, session_id, PACKET_TIMEOUT_US, buffer, NULL, NULL)))
      {
        YYSYS_LOG(ERROR, "post_end_next failed! server=%s, session_id=%ld ret=%d", to_cstring(server), session_id, ret);
      }
      return ret;
    }
    ...
    int OceanbaseDb::execute_dml_sql(const ObString& ob_sql, const ObServer &ms, int64_t& affected_row)
    {
      ...
      {
        YYSYS_LOG(ERROR, "execute query sql failed, sql=[%.*s], errno=[%d], errinfo=[%s]", ob_sql.length(), ob_sql.ptr(), rs.get_errno(), ob_strerror(rs.get_errno()));
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "this function should not execute query sql!");
        }
      }
      return ret;
    }
      ...
      while(true)
      {
        ...
        while(OB_SUCCESS == (ret = scanner.get_next_row(row)))
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(INFO, "server=%s, %.*s:%ld, merge checking array index=%d, will be checked merge state later.",
...
//mod by zhuxh:20160907 [cb sequence]
        ...
        bool default_start, bool default_increment )
    {
      ...
      if( NULL == data )
      {
        YYSYS_LOG(ERROR, "Allocating memory failed.");
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "Sending execute sql fails, ret=%d", ret);
        }
        else
        {
          int cell_idx = 0;
          ...
          {
            YYSYS_LOG(ERROR, "Getting next row fails, ret=%d", ret);
          }
          else if(OB_SUCCESS != (ret = row.raw_get_cell(cell_idx++, cell, tid, cid)))
          {
            YYSYS_LOG(ERROR, "Getting a cell fails, ret=%d", ret);
          }
          else if(OB_SUCCESS != (ret = cell->get_int(sequence_has_existed)))
          {
            YYSYS_LOG(ERROR, "Getting the value of cell fails, ret=%d", ret);
          }
        }
