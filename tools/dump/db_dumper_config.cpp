...
void DbTableConfig::parse_rowkey_item()
{
  ...
  for (int64_t i = 0;i < rowkey_info.get_size();i++) {
    ...
    YYSYS_LOG(INFO, "table rowkey item%ld: %s", i, item.key_name.c_str());
  }
}
...
void DbTableConfig::parse_output_columns(const std::string &out_columns)
{
  ...
  while ((delima_pos = out_columns.find(",", base_pos)) != std::string::npos) {
    ...
    YYSYS_LOG(DEBUG, "COLUMN: %s", column.c_str());
...
bool DbTableConfig::check_valid() const
{
  ...
  for(size_t i = 0;i < output_columns_.size(); i++) {
    if (schema_mgr_->get_column_schema(table_name, output_columns_[i].c_str()) == NULL) {
      YYSYS_LOG(ERROR, "schema does not contain such column[%s]", output_columns_[i].c_str());
      valid = false;
      break;
    }
  }
...
DbDumpConfigMgr* DbDumpConfigMgr::get_instance()
{
  ...
  if (instance_ == NULL) {
    YYSYS_LOG(ERROR, "error when get instance");
  }
...
void DbDumpConfigMgr::add_table_config(const std::string &table_name)
{
  ...
  columns = YYSYS_CONFIG.getStringList(table_name.c_str(), kConfigColumn);
  ...
  //set revise column
  std::vector<const char*> revise_column = YYSYS_CONFIG.getStringList(table_name.c_str(), kConfigReviseColumn);
  config.set_revise_columns(revise_column);
  YYSYS_LOG(INFO, "revise column size=%zu", config.get_revise_columns().size());

  const char *output_format_str = YYSYS_CONFIG.getString(table_name.c_str(), std::string(kOutputFormat));
  if (output_format_str != NULL) {
    std::string str = output_format_str;
    config.parse_output_columns(str);
  }

  const char *filter_str = YYSYS_CONFIG.getString(table_name.c_str(), std::string(kConfigColumnFilter));
  if (filter_str) {
  ...
  if (config.check_valid() == true) {
    ...
    YYSYS_LOG(ERROR, "check config valid failed, table[%s]", table_name.c_str());
  }
}

int DbDumpConfigMgr::load_sys_param()
{
  parser_thread_nr_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysParserNr), 1);
  output_dir_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kOutputDir), "data");
  worker_thread_nr_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysWorkerThreadNr), 6);
  if (worker_thread_nr_ <= 0) {
    YYSYS_LOG(WARN, "[%ld] is not a valid worker thread number, use 6 instead", worker_thread_nr_);
    worker_thread_nr_ = 6;
  }

  log_dir_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysLogDir), "obdump.log");
  log_level_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysLogLevel), "DEBUG");
  ob_log_dir_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kConfigObLog));
  if (ob_log_dir_.empty()) {
    YYSYS_LOG(ERROR, "please specify path of commlitlog");
    return OB_ERROR;
  }

  host_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysHost));
  port_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysPort), 2500);
  if (host_.empty() || port_ <= 0) {
    YYSYS_LOG(ERROR, "oceabase %s:%d config error", host_.c_str(), port_);
    return OB_ERROR;
  }

  consistency_ = (bool)YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysConsistency), 1);
  network_timeout_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysNetworkTimeout), 8 * kDefaultTimeout);
  max_file_size_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysMaxFileSize), 50 * 1024 * 1024);
  rotate_file_interval_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysRotateFileTime), 60);
  monitor_interval_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysMonitorInterval), 10);
  tmp_log_path_= YYSYS_CONFIG.getString(kConfigSys, std::string(kSysTmpLogDir), "tmp_log");
  init_log_id_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysInitLogId), 0);
  header_delima_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysHeaderDelima), 2);
  body_delima_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysBodyDelima), 1);
  app_name_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysAppName), "obdump");
  pid_file_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysPidFile), "obdump.pid");
  max_nolog_interval_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysMaxNologInvertal), 3600);
  init_date_ = YYSYS_CONFIG.getString(kConfigSys, std::string(kSysInitDate));
  muti_get_nr_ = YYSYS_CONFIG.getInt(kConfigSys, std::string(kSysMutiGetNr), 0);
  return OB_SUCCESS;
}

int DbDumpConfigMgr::load(const char *path)
{
  int ret = YYSYS_CONFIG.load(path);
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "Can't Load YYSYS config");
    return ret;
  }

  if ((ret = load_sys_param()) != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "load sys param failed, quiting");
    return ret;
  }

  OceanbaseDb db(host_.c_str(), port_);
  if ((ret = db.init()) != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "connect to %s:%d failed", host_.c_str(), port_);
    return ret;
  }

  RPC_WITH_RETIRES(db.fetch_schema(schema_mgr_), 5, ret);
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "dumper config fetch_schema failed, ret = %d", ret);
    return ret;
  }

  std::vector<std::string> sections;
  YYSYS_CONFIG.getSectionName(sections);
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "gen table id map failed");
  }
...
int DbDumpConfigMgr::gen_table_id_map()
{
  ...
  if (configs_.size() != config_set_.size()) {
    YYSYS_LOG(ERROR, "duplicate table id found");
    ret = OB_ERROR;
  }
