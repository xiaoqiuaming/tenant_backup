...
DbRowFilter *create_filter_from_str(std::string str)
{
  ...
  if (pos == std::string::npos) {
    YYSYS_LOG(WARN, "filter config error");
    return filter;
  }
   ...
   case MODIFY_TIME_FILTER_TYPE:
     {
       ...
       pos = str.find(',', start_pos);
       if (pos == std::string::npos) {
         YYSYS_LOG(WARN, "filter config error, skiped");
       }
       ...
       YYSYS_LOG(DEBUG, "TIME [%s]:[%s]", min.c_str(), max.c_str());
...
bool DbDateTimeFilter::operator()(DbRecord *record) const
{
  ...
  if (ret != OB_SUCCESS)
    ...
    if (ret == OB_SUCCESS) {
      ...
      YYSYS_LOG(DEBUG, "TIME:[%s]", time_str);
