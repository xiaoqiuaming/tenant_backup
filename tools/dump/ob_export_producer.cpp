...
int ExportProducer::produce_v2(TabletBlock* &block)//add qianzm
{
  ...
  YYSYS_LOG(DEBUG, "enter produce");
  block = NULL;
  db_->set_table_param(&table_param_);
  YYSYS_LOG(DEBUG, "real table name=%s", table_param_.table_name.c_str());

  //判断当前的全局err_code_是不是正常状态，如果是异常状态则结束当前线程
  if(OB_SUCCESS != err_code_->get_err_code())
  {
    YYSYS_LOG(INFO, "some other thread exit with fail code! current thread should exit!");
  ...
  else if(mark->next_sql_request && mark->next_packet)//前一个查询结束，进入下一个查询
  {
    ...
    if(!mark->is_last_timeout)//检查上次查询是否是超时失败，第一次为初始化超时时间
    {
      ...
      mark->last_timeout = timeout_us_;//(25min)
      //mark->last_timeout = 180L * 60L * 1000L * 1000L;(3hour)
    }
    else //如果上次查询是超时失败，则退出
    {
      YYSYS_LOG(ERROR, "last get_result is timeout, last_timeout is [%ld]", mark->last_timeout);
   ...
   //将mark中相关的标记量重新赋值
      ...
      {
        YYSYS_LOG(ERROR, "current timeout[%ld] is exceed the MAX_TIMEOUT_US[%ld], failed!", mark->last_timeout, max_limittime_);
        err_code_->set_err_code(OB_ERROR);
        return ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
      }
   */
    ...
    YYSYS_LOG(INFO, "query sql: %s", sql.c_str());

    //获取一台可用的ms
    ObServer ms = get_one_ms();
    YYSYS_LOG(INFO, "get one ms: %s, timeout is %ld", to_cstring(ms), mark->last_timeout);
    ...
    YYSYS_LOG(DEBUG, "init_execute_sql success!");
    ...
    YYSYS_LOG(DEBUG, "first tmp_block->init success!");
    ...
    {
      YYSYS_LOG(DEBUG, "first get a packet success!");
...
*/
      ...
      if(!is_rowkey_idx_inited_)
      {
        ...
        if(!is_rowkey_idx_inited_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "init_rowkey_idx failed!");
            rowkey_idx_inited_lock_.unlock();
            err_code_->set_err_code(OB_ERROR);
            return ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
          }
          YYSYS_LOG(INFO, "init_rowkey_idx successed!");
      ...
      if(!is_rowformator_result_field_inited_)
      {
        ...
        if(!is_rowformator_result_field_inited_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "rowformator->init_result_fields_map failed! ret=%d", ret);
            rowformator_result_field_lock_.unlock();
            err_code_->set_err_code(OB_ERROR);
            return ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
          }
          YYSYS_LOG(INFO, "rowformator->init_result_fields_map success!");
  ...
  else if(mark->next_packet && !mark->next_sql_request)//当前查询还没有结束，继续获取下一个包
  {
    ...
    if(OB_SUCCESS != (ret = tmp_block->init()))
    {
      YYSYS_LOG(ERROR, "next tmp_block init failed! ret=%d", ret);
      err_code_->set_err_code(OB_ERROR);
      delete tmp_block;
      thread_post_end_session();
      return ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
    ...
    YYSYS_LOG(DEBUG, "next tmp_block init success!");
    ...
    if(OB_SUCCESS != (ret = db_->get_result(result_set, out_buffer, has_next)))
    {
      ...
      if(OB_RESPONSE_TIME_OUT == ret)
      {
        ...
        mark->next_packet = true;
        thread_post_end_session();
        YYSYS_LOG(WARN, "current get_reuslt is timeout! timeout_us is %ld", mark->last_timeout);
      }
      else*/
      {
        YYSYS_LOG(ERROR, "get result from mergeserver failed! ret=%d", ret);
    ...
    {
      YYSYS_LOG(DEBUG, "next get a packet success!");
      //将新分配的TabletBlock内存地址传回
      block = tmp_block;
      ...
      if(OB_ITER_END == ret && !empty)
      {
        ...
        YYSYS_LOG(TRACE, "mark->start_rowkey = %s", to_cstring(mark->start_rowkey));
        ret = OB_SUCCESS;
      }
      else if(empty)
      {
        YYSYS_LOG(INFO, "current scanner is empty!");
        ret = OB_SUCCESS;
      }
      else
      {
        YYSYS_LOG(ERROR, "get next row failed! ret= [%d]", ret);
      }

      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "producer occurs failure, exit!");
        err_code_->set_err_code(OB_ERROR);
        thread_post_end_session();
        return ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
      }
      ...
      {
        YYSYS_LOG(INFO, "there are left packets should be received!");
        mark->next_sql_request = false;
        mark->next_packet = true;
        mark->is_last_timeout = false;
      }
      ...
      {
        YYSYS_LOG(INFO, "there are no left packet should be received!");
        //mark->next_sql_request = true;
        //mark->next_packet = true;
        //只有一个sql 收到所有数据包 就退出
        mark->next_sql_request = false;
  ...
  {
    YYSYS_LOG(INFO, "no execute sql left, work done!");
    ret = ComsumerQueue<TabletBlock*>::QUEUE_QUITING;
  }
  return ret;
}
...
int ExportProducer::produce(TabletBlock* &block)
{
  ...
  else if(mark->next_sql_request && mark->next_packet)//前一个查询结束，进入下一个查询
  {
    ...
    if(!mark->is_last_timeout)//检查上次查询是否是超时失败，如果不是就执行正常的获取tabletinfo流程来构造sql
    {
      ...
      {
        YYSYS_LOG(INFO, "tablet_info is empty! thread exit!");
      ...
      mark->is_last_timeout = false;
      mark->last_timeout = timeout_us_;
      //YYSYS_LOG(INFO, "qianzm-test timeout_us_ = [%ld]", timeout_us_);
      TabletCache::iterator iter = tablet_cache_.begin();
      ObExportTabletInfo ti = iter->second;
      tablet_cache_.erase(iter);
      if (table_param_.exist_limit)
      {
        tablet_cache_.clear();
        YYSYS_LOG(INFO, "exist limit in sql, so clear tablet_cache to avoid duplicate select");
      }
      else
      {
        YYSYS_LOG(DEBUG, "pop a tabletinfo from tablet_cache_, tabletinfo_.get_start_key_: %s, tabletinfo_.get_end_key_: %s",
      ...
      if(!table_param_.exist_limit &&
         OB_SUCCESS != (ret = parse_key_to_sql(ti.get_start_key(), ti.get_end_key(), sql_part2)))
      {
        YYSYS_LOG(ERROR, "parse_key_to_sql failed! ret=[%d]", ret);
        err_code_->set_err_code(OB_ERROR);
      ...
      else
      {
        if (table_param_.exist_limit)
        {
          sql_part2 = sql_part2 + " " + table_param_.limit;
        }
        //add qianzm [export_by_tablets] 20160415:b
        ...
        mark->deadline = yysys::CTimeUtil::getTime() + tablet_timeout;
        YYSYS_LOG(INFO, "Tablet %d, %ld B ( %ld MB ), has %ld us ( %ld min ) as max duration to export", mark->tablet_index_, ti.occupy_size_, ti.occupy_size_ >> 20, tablet_timeout, tablet_timeout / 1000000 / 60);
        //add by zhuxh [timeout mechanism] 20170512 :e
      }
    }
    else //如果上次查询是超时失败，则根据存储在mark中的rowkey信息构造sql，并把超时时间调大
    {
      //YYSYS_LOG(WARN, "last get_result is timeout, start a new query by last rowkey!");
      YYSYS_LOG(WARN, "last get_result of tablet[%d] is timeout, start a new query by last rowkey!", mark->tablet_index_);
      //将mark中相关的标记量重新赋值
      mark->next_sql_request = false;
      mark->next_packet = false;
      mark->is_last_timeout = false;
      ...
      //if(mark->last_timeout > max_limittime_)
      if( yysys::CTimeUtil::getTime() >= mark->deadline )
      {
        //YYSYS_LOG(ERROR, "current timeout[%ld] exceeds the MAX_TIMEOUT_US[%ld], failed!", mark->last_timeout, max_limittime_);
        YYSYS_LOG(ERROR, "Tablet export timeout.");
        ...
        */
        ...
        tablet_cache_lock_.unlock();
        YYSYS_LOG(ERROR, "tablet[%d] get_result failed!!!", mark->tablet_index_);
      ...
      YYSYS_LOG(TRACE, "mark->start_rowkey=%s mark->last_rowkey=%s", to_cstring(mark->start_rowkey), to_cstring(mark->last_rowkey));
    ...
    string sql = sql_part1 + " " + sql_part2;
    //YYSYS_LOG(INFO, "query sql: %s", sql.c_str());
    YYSYS_LOG(INFO, "tablet[%d]:query sql: %s", mark->tablet_index_, sql.c_str());
    ...
    if(OB_SUCCESS != (ret = db_->get_result(result_set, out_buffer, has_next)))
    {
      //add qianzm [export_by_tablets] 20160415:b
      //YYSYS_LOG(ERROR, "get result from mergeserver failed! ret=%d", ret);
      YYSYS_LOG(ERROR, "get result of tablet[%d] from mergeserver failed! ret=%d", mark->tablet_index_, ret);
      tablet_cache_lock_.lock();
      failed_sql_.push_back(mark->err_sql_);
      failed_tablets_index_.push_back( mark->tablet_index_);
      tablet_cache_lock_.unlock();
...
int ExportProducer::thread_add_server_session()
{
  ...
  {
    YYSYS_LOG(ERROR, "db_->get_thread_server_session(server, session_id) failed! ret=[%d]", ret);
  }
  producer_id = mark->producer_index;
  if(OB_EXPORT_SUCCESS != ac_server_session.push_by_id(producer_id, server, session_id))
  {
    YYSYS_LOG(ERROR, "ac_server_session.push_by_id(producer_id, server, session_id) failed!");
    ret = OB_ERROR;
  }
  return ret;
}
...
int ExportProducer::thread_remove_server_session()
{
  ...
  {
    YYSYS_LOG(WARN, "server session not exist, current producer has lost its server_session!");
  }
  return ret;
}
...
int ExportProducer::thread_post_end_session()
{
  ...
  if(OB_EXPORT_NOT_EXIST != ac_server_session.pop_by_producer_id(producer_id, server, session_id))
  {
    ...
    {
      YYSYS_LOG(ERROR, "current producer %d post end next failed! ret=[%d]", producer_id, ret);
    }
  }
  else
  {
...
int ExportProducer::init()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get_tablet_info failed! please check the table name! ret=[%d]", ret);
    }
    timeout_us_ = db_->get_timeout_us();
  }
  return ret;
...
int ExportProducer::parse_key_to_sql(const ObRowkey& startkey, const ObRowkey& endkey, string& out_range_str)
{
  ...
  {
    YYSYS_LOG(ERROR, "parse rowkey failed! ret=[%d]", ret);
  }
  else
  {
    startkey_str = buffer;
  ...
  if(OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "parse tabletinfo failed! ret=[%d]", ret);
  }
...
ObServer ExportProducer::get_one_ms()
{
  ...
  YYSYS_LOG(DEBUG, "ms=%s", to_cstring(ms));
  return ms;
}
...
int ExportProducer::get_tablet_info()
{
  ...
  do {
    ...
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "get_tablet_info failed! ret=[%d]", ret);
      break;
    }

    ret = parse_scanner(scanner);
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "parse tablet info from ObScanner failed, ret=[%d]", ret);
      break;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(INFO, "This table has %d tablets", tablet_cnt);
  }
  //add 20160415:e
  if (p_buffer != NULL) {
    delete[] p_buffer;
...
int ExportProducer::parse_scanner(ObScanner &scanner)
{
  ...
  YYSYS_LOG(DEBUG, "root server get rpc return succeed, cell num=%ld", scanner.get_cell_num());
  ...
  if (OB_SUCCESS == ret) {
    for (++iter; iter != scanner.end(); ++iter) {
      ...
      if (ret != OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "build tablet info failed, ret=[%d]", ret);
          break;
        }
        //add qianzm [export_by_tablets] 20160415:b
...
void ExportProducer::insert_tablet_cache(const ObRowkey &rowkey, ObExportTabletInfo &tablet)
{
  ...
  if (iter != tablet_cache_.end()) {
    tablet_cache_.erase(iter);
    YYSYS_LOG(DEBUG, "delete cache tablet, end rowkey=[%s]", to_cstring(iter->first));
  }

  YYSYS_LOG(DEBUG, "insert cache tablet, end rowkey=[%s]", to_cstring(rowkey));
  ...
  for(int64_t i = 0; i < rowkey_num; i++)
  {
    ...
    switch(ptr[i].get_type())
    {
      ...
      case ObVarcharType:
      {
        ...
        //          YYSYS_LOG(INFO, "ptr[i] is %s, ptr[i].get_varchar is %s, val.length() = %d", to_cstring(ptr[i]), val.ptr(), val.length());
        char tmp_sql_buffer[2048];
        if(OB_SUCCESS != (ret = add_escape_char(val.ptr(), val.length(), tmp_sql_buffer, 2048)))
        {
          YYSYS_LOG(ERROR, "add_escape_char failed! ret=[%d]", ret);
        }
        else
        {
      ...
      case ObDecimalType:            // aka numeric
      {
        ...
        int len = (int)val.to_string(res, MAX_PRINTABLE_SIZE);
        write_size = snprintf(pos, reserve_size, "%.*s", len, res);
        reserve_size -= write_size;
        pos += write_size;
      }
        break;
    ...
    if ( OB_SUCCESS != ret )
    {
      YYSYS_LOG(ERROR, "parse rowkey error! ret=[%d]", ret);
      break;
    }
...
int ExportProducer::init_rowkey_idx(const ObArray<ObResultSet::Field>& fields)
{
  ...
  for(size_t i = 0; i < rowkey_column_names.size(); i++)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "rowkey column %s is not assign in sql!", rowkey_column_names[i].c_str());
      break;
    }
  }
...
int ExportProducer::print_tablet_info_cache()
{
  ...
  if (failed_sql_.size() != 0 || tablet_cache_.begin() != tablet_cache_.end()){
    ...
    {
      YYSYS_LOG(ERROR, "FileWrite init tablet_info_file failed, ret=[%d]", ret);
    }
    else
    {
      if (failed_sql_.size() != failed_tablets_index_.size())
      {
        ...
        //为了防止有异常情况发生，写了这个分支
        YYSYS_LOG(ERROR, "failed_sql_.size(%d) != failed_tablets_index_.size(%d)",
      ...
      {
        YYSYS_LOG(ERROR, "failed_sql count is %d ", (int)failed_sql_.size());
        //将所有超时的sql写入一个新的日志文件中
        for(size_t i=0;i < failed_sql_.size(); i ++)
        {
          YYSYS_LOG(INFO, "The sql of failed_tablet[%d]: %s;", failed_tablets_index_[i], failed_sql_[i].c_str());
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (iter == tablet_cache_.end())
      {
        YYSYS_LOG(INFO, "tablet_cache_ is empty!");
      }
      ...
      while (iter != tablet_cache_.end())
      {
        ...
        {
          YYSYS_LOG(ERROR, "parse tablet_cache_ to tablet_info_file failed!");
          break;
        }
        else
        {
          ...
          YYSYS_LOG(WARN, "Not executed sql of tablet[%d]: %s",
...
//    tablet_cache_.erase(iter);
//    YYSYS_LOG(DEBUG, "pop a tablet from tablet cache, start key[%s], end key[%s]", to_cstring(tablet.get_start_key()), to_cstring(tablet.get_end_key()));
//    tablet_cache_lock_.unlock();
//  }
//
//  return ret;
