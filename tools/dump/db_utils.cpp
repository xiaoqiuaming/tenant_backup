...
//add by pangtz:20141205 [֧�ֳ���timestamp��ʽ����]
/*
 ...
 * YYYY/mm/dd HH:MM:SS.uuuuuu
 */
//add by zhuxh:20150714 [֧�ֳ���timestamp��ʽ����]
//Totally replaced
...
int is_decimal(const char *str, const int &len)//add by liyongfeng 20140818 for check whether string satisfies decimal format:b
{
    ...
    if((str != NULL) && (len != 0))
    {
        ...
        {
            if((str[i] < '0') || (str[i] > '9')) {
                YYSYS_LOG(WARN, "input invalid decimal format");
                ret = OB_ERROR;
            } else {
                while(i < len)
                {
                    if(str[i] >= '0' && str[i] <= '9') { i++; }
                        ...
                        if(points_num == 0) { points_num++; i++; }
                            ...
                            YYSYS_LOG(WARN, "decimal format is not correct, decimal points exceed 1");
                            ret = OB_ERROR;
                            break;
                        }
                    } else {
                        YYSYS_LOG(WARN, "invalid character(non numeric or point)=[%c]", str[i]);
                        ret = OB_ERROR;
                        break;
                    }
                }
            ...
            YYSYS_LOG(WARN, "invalid decimal format, only sign character or point(+, - or .)");
            ret = OB_ERROR;
        }
    } else {
        YYSYS_LOG(WARN, "input invalid decimal string, null or len < 0");
        ret = OB_ERROR;
    }
        ...
        if(str[i] != '\0' && i != len)
        {
            while(i < len)
            {
                if(str[i] >= '0' && str[i] <= '9')
                {
                    ...
                    YYSYS_LOG(DEBUG, "the last character is '.', numeric string: %.*s", len, str);
                    i++;
                }
                else
                {
   ...
   case ObVarcharType:
     {
       ...
       if (obj.get_varchar(str) != OB_SUCCESS ||
           cap < str.length()) {
         YYSYS_LOG(ERROR, "get_varchar error");
         break;
       }
   ...
   case ObPreciseDateTimeType:
     {
       ...
       if (obj.get_precise_datetime(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_precise_datetime error");
         break;
       }
   ...
   case ObDateTimeType:
     {
       ...
       if (obj.get_datetime(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_datetime error");
         break;
       }
       len = ObDateTime2MySQLDate(value, type, data + pos, static_cast<int32_t>(cap));
     }
   ...
   case ObModifyTimeType:
     {
       ...
       if (obj.get_modifytime(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_modifytime error ");
         break;
       }
       len = ObDateTime2MySQLDate(value, type, data + pos, static_cast<int32_t>(cap));
     }
   ...
   case ObCreateTimeType:
     {
       ...
       if (obj.get_createtime(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_createtime error");
         break;
       }
       len = ObDateTime2MySQLDate(value, type, data + pos, static_cast<int32_t>(cap));
     }
   ...
   case ObFloatType:
     {
       ...
       if (obj.get_float(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_float error");
         break;
       }
   ...
   case ObDoubleType:
     {
       ...
       if (obj.get_double(value) != OB_SUCCESS) {
         YYSYS_LOG(ERROR, "get_double error");
         break;
       }
       len = snprintf(data + pos, cap, "%f", value);
     }
     break;
   default:
     YYSYS_LOG(WARN, "Not Defined Type %d", obj.get_type());
     break;
  }
...
int ObDateTime2MySQLDate(int64_t ob_time, int time_type, char *outp, int size)
{
  ...
  switch (time_type) {
   ...
   case ObDateTimeType:
     ...
     break;
   default:
     YYSYS_LOG(ERROR, "unrecognized time format, type=%d", time_type);
     ret = OB_ERROR;
  }
...
int transform_date_to_time(const char *str, int len, ObPreciseDateTime &t)
{
  ...
  char back = str[len];
  const_cast<char*>(str)[len] = '\0';
  //YYSYS_LOG(INFO, "input str = %s", str);
  if (NULL != str && *str != '\0')
  {
    if (*str == 't' || *str == 'T')
    {
...
void dump_scanner(ObScanner &scanner)
{
  ...
  for (iter = scanner.begin(); iter != scanner.end(); iter++)
  {
    ...
    if (is_row_changed)
    {
      YYSYS_LOG(INFO, "table_id:%lu, rowkey:\n", cell_info->table_id_);
      //      hex_dump(cell_info->row_key_.ptr(), cell_info->row_key_.length());
    }
    YYSYS_LOG(INFO, "%s", print_cellinfo(cell_info, "DUMP-SCANNER"));
  }
}
