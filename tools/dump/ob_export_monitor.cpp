...
void ExportMonitor::run(yysys::CThread *thread, void *arg)
{
  UNUSED(thread);
  UNUSED(arg);
  int ret = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(WARN, "post end session failed!");
  }
  YYSYS_LOG(INFO, "monitor work done!");
}
...
int ExportMonitor::post_end_session_all()
{
  ...
  while(OB_EXPORT_EMPTY != (ret = ac_server_session.pop_one(server, session_id)))
  {
    ...
    {
      YYSYS_LOG(WARN, "post end next failed! server=%s, session_id=%ld, ret=%d", to_cstring(server), session_id, ret);
    }
    else
    {
      YYSYS_LOG(INFO, "post end next success! server=%s, session_id=%ld.", to_cstring(server), session_id);
    }
  }
  if(OB_SUCCESS != ret && OB_EXPORT_EMPTY != ret)
  {
    YYSYS_LOG(WARN, "post end next failed!");
  }
  else
  {
    ret = OB_SUCCESS;
