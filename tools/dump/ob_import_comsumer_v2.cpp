...
int ImportComsumer::comsume(RecordBlock &obj)
{
    ...
    //ObServer ms;
      if(OB_SUCCESS == ret)
      {
        if(OB_SUCCESS != (ret = ms_manager_.get_one_ms(ms_)))
        {
          error_arr[GET_ONE_MS_ERROR] = false; //add by zhuxh:20150722
          YYSYS_LOG(ERROR, "get one ms failed!");
        }
      }
      if(OB_SUCCESS == ret)
      {
        char ip_buffer[32];
        ms_.ip_to_string(ip_buffer, 32);
        YYSYS_LOG(DEBUG, "get one ms %s",to_cstring(ms_));
        //add by zhuxh:20160914 [retry mysql connection] :b
        //int retry_times=3;
        int retry_times=5; //mod by zhuxh:20160922 //try more times
        for( int i=0; i<retry_times; i++ )
        {
          ret = mysql_client.connect(ip_buffer, param_.user_name_, param_.passwd_, param_.db_name_, (short unsigned int)ms_.get_port(), param_.timeout_);
          if( i<retry_times-1 ) //No need to sleep at the last time
          {
            ...
            else
            {
              ...
              YYSYS_LOG(WARN, "mergeserver connection error. ret=%d. Sleep for %d us and try connection again.",ret,sleep_us);
              usleep( sleep_us );
            }
          }
        }
        ...
        if(ret != OB_SUCCESS)
        {
          ...
          YYSYS_LOG(ERROR, "connect to mergeserver[%s] failed!", to_cstring(ms_));
        }
      }
    }
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      //mod:e
      {
        ...
        YYSYS_LOG(ERROR, "execute sql error on mergeserver[%s] sql[%.*s]",
                  to_cstring(ms_), sql.length(), sql.ptr());
...
bool ImportComsumer::if_ignore_merge() const
{
  return param_.ignore_merge_;
}
