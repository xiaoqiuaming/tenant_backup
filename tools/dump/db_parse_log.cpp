...
DbLogParser::DbLogParser(std::string &path, DbLogMonitor *monitor, DbCheckpoint *ckp) 
{
  ...
  ckp_ = ckp;
  if (ckp_ == NULL) {
    YYSYS_LOG(WARN, "checkpoint disabled");
  }
}
...
int DbLogParser::parse_log(int64_t log_id)
{
  ...
  if (OB_SUCCESS != ret) {
    YYSYS_LOG(ERROR, "can't open directory, due to %d", ret);
    ret = OB_ERROR;
  }

  if (ret == OB_SUCCESS) {
    YYSYS_LOG(INFO, "Using LOG:%ld", log_id);
    ret = reader.open(log_id);
    if (OB_SUCCESS != ret) {
      YYSYS_LOG(ERROR, "can't open log %ld, due to %d", log_id, ret);
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "read log failed");
    }
  }
  ...
  while (OB_SUCCESS == ret) {
    ...
    if (OB_LOG_UPS_MUTATOR == cmd) {
      ...
      if (OB_SUCCESS != ret) {
        YYSYS_LOG(ERROR, "deserializing ObUpsMutator failed");
        break;
      }
      ...
      while (OB_SUCCESS == (ret = mut.next_cell())) {
        ...
        if (OB_SUCCESS != ret || NULL == cell) {
          YYSYS_LOG(ERROR, "ObUpsMutator get cell failed");
          break;
        }
...
//                uint64_t timestamp = mut.get_mutate_timestamp();
              ...
              YYSYS_LOG(ERROR, "OB/DB operation deserialize error, op_type=%ld, cell_type=%d", 
  ...
  YYSYS_LOG(INFO, "LOG_ID:%ld, ROWKEY COUNT:%ld, TIME ELAPSED(us):%ld", 
  ...
  if (OB_READ_NOTHING != ret) {
    YYSYS_LOG(ERROR, "parsing log[:%ld:] failed, [retcode = %d]", log_id, ret);
...
void DbLogParser::run(yysys::CThread *thread, void *args)
{
  int64_t log_id;
  int ret = OB_SUCCESS;
  UNUSED(thread);
  ...
  YYSYS_LOG(INFO, "starting log parser thread");
  while (running_ == true && (ret = monitor_->fetch_log(log_id)) == OB_SUCCESS) {
    YYSYS_LOG(INFO, "succeed fetch a log, id=%ld", log_id);
    ...
    if (ret != OB_SUCCESS) {
      ...
      if (ret == OB_SUCCESS) {
        ...
        ret = parse_log(log_id);
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't parse log file, id=%ld", log_id);
          report_msg(MSG_ERROR, "can't parse log file, id=%ld", log_id);
        }
      } else {
        YYSYS_LOG(ERROR, "can't get log[%ld] from nas", log_id);
      }
    } else {                                    /* parse log success */
      if (remove_log_file(log_id) != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't remove log id");
      }
    }
    ...
    if (ret == OB_SUCCESS && ckp_ != NULL) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't write checkpoint");
        report_msg(MSG_ERROR, "can't write checkpoint");
      }
    }
  }

  YYSYS_LOG(INFO, "parser thread quiting");
}
...
int DbLogParser::start_parse(RowkeyHandler *handler)
{
  ...
  if (thr_ != NULL) {
    ...
    YYSYS_LOG(ERROR, "can't alloc thread");
    ret = OB_ERROR;
  }
