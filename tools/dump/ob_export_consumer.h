...
public:
  ...
  int64_t get_total_succ_rec_count() { return total_succ_rec_count_; }
  int64_t get_total_failed_rec_count() { return total_failed_rec_count_; }

  int64_t inc_total_succ_rec_count(int increment);

  int64_t inc_total_failed_rec_count(int increment);
...
private:
  ...
  ErrorCode *err_code_;
  int64_t total_succ_rec_count_;
  int64_t total_failed_rec_count_;

  yysys::CThreadMutex total_succ_rec_count_lock_;
  yysys::CThreadMutex total_failed_rec_count_lock_;
