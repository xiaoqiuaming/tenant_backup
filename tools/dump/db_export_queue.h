...
template <class T>
class ExportQueue : public yysys::CDefaultRunnable {
private:
  std::list<T> queue_;
  size_t cap_;
  yysys::CThreadCond queue_cond_;
  yysys::CThreadCond queue_cond_full_;//如果队列装满且producer正在运行,该条件量阻塞等待
...
int ExportQueue<T>::produce_and_comsume(int nr_producer, int nr_consumer)
{
  ...
  {
     nr_producer_ = 0;
     YYSYS_LOG(ERROR, "export producer is NULL");
     return QUEUE_ERROR;
  }
  else
  {
  ...
  {
     YYSYS_LOG(ERROR, "can't init producer/consumer, quiting");
     ret = QUEUE_ERROR;
  }
  else
  {
     running_ = true;
     YYSYS_LOG(INFO, "ExportQueue: producer = %ld, consumer = %ld, cap = %ld", nr_producer_, nr_consumer_, cap_);
...
template <class T>
void ExportQueue<T>::run(yysys::CThread *thread, void *arg)
{
  long id = (long)arg;
  UNUSED(thread);
  UNUSED(arg);
  YYSYS_LOG(INFO, "ExportQueue: is running, id=%ld", id);
  if (id < nr_producer_)
  {
     produce(id);
  }
...
{
  YYSYS_LOG(INFO, "producer id = %ld", id);
