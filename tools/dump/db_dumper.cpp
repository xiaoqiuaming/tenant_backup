...
int DbDumper::process_rowkey(const ObCellInfo &cell, int op, uint64_t timestamp, int64_t seq) 
{
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(DEBUG, "no such table, table_id=%ld", cell.table_id_);
  } else {
...
int DbDumper::setup_dumpers()
{
  ...
  for(size_t i = 0; i < cfgs.size(); i++) {
    ...
    if (writer.dumper == NULL) {
      YYSYS_LOG(ERROR, "Can't create dumper, allocate memory failed");
      ret = OB_ERROR;
      break;
    }
  ...
  if (ret == OB_SUCCESS) {
    for(size_t i = 0;i < dump_writers_.size(); i++) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "start up dumper error, ret=%d", ret);
        break;
      }
    }
  }
...
int DbDumper::init(OceanbaseDb *db)
{
  ...
  if (db_ == NULL) {
    YYSYS_LOG(ERROR, "database or schema is NULL");
    ret = OB_ERROR;
  } else {
    ret = rs_.init();
  }
  ...
  if (ret == OB_SUCCESS) {
    if (pthread_spin_init(&seq_lock_, PTHREAD_PROCESS_PRIVATE) != 0) {
      YYSYS_LOG(ERROR, "can't init spin lock");
      ret = OB_ERROR;
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    YYSYS_LOG(ERROR, "failed when init DbDumper, [retcode=%d]", ret);
    ret= OB_ERROR;
  }
...
DbDumper::~DbDumper() 
{
  YYSYS_LOG(INFO, "waiting dump writer thread stop");
  destroy_dumpers();
  pthread_spin_destroy(&seq_lock_);
  YYSYS_LOG(INFO, "Total rowkyes = %ld", total_keys_);
  YYSYS_LOG(INFO, "del_lines= %ld, insert_lines=%ld", del_lines, insert_lines);
}
  ...
  for (int64_t i = size - 2;i >= 0; i--) {
    if (rowkeys[i].rowkey != rowkeys[i + 1].rowkey || 
      ...
      if (rowkeys[i].timestamp > rowkeys[i + 1].timestamp) {
        YYSYS_LOG(WARN, "currupted log files, mutations are not in order, time1=%ld, time=%ld",
...
int DbDumper::db_dump_rowkey(const TableRowkey *rowkeys, const int64_t size, DbRecordSet &rs)
{
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret == OB_SUCCESS) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "pack record error");
      }
    } else {
      YYSYS_LOG(ERROR, "can't get data from database");
    }
  }
...
int DbDumper::dump_del_key(const TableRowkey &key)
{
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "no table find, table_id=%lu", key.table_id);
  } else {
...
int DbDumper::handle_del_row(const DbTableConfig *cfg,const ObRowkey &rowkey, int op, uint64_t timestamp, int64_t seq)
{
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "Unable seiralize header, due to [%d], skip this line", ret);
  } 
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't push record to dumper writer queue, rec len=%ld", len);
    }
  }
                          ...
                          DbRecordSet &rs) 
{
  ...
  while(ret == OB_SUCCESS && itr != rs.end()) {
    ...
    if (ret != OB_SUCCESS || recp == NULL) {
      YYSYS_LOG(ERROR, "can't get record, due to[%d]", ret);
      break;
    }
    ...
    if (recp->get_rowkey(rowkey) != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "muti thread processing same DbRecordSet?");
      break;
    }
...
#if 0
    {
      ...
      buf[2 * len] = 0;
      YYSYS_LOG(INFO, "resp, key=%s", buf);
    }
#endif

    ret = find_table_key(rowkeys, size, rowkey, table_key);
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't find ds rowkey in req rowkeys, currupted memory?");
      break;
    }
    ...
    if (ret == OB_SUCCESS) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "Unable seiralize record, due to [%d], skip this line", ret);
      }
#if 1
      if (ret != OB_SUCCESS) {
        DbRecordSet::Iterator itr1 = rs.begin();
        while (itr1 != rs.end()) {
          YYSYS_LOG(INFO, "New Record Start");
          DbRecord *rec1;
          ret = itr1.get_record(&rec1);
          if (ret != OB_SUCCESS) {
            YYSYS_LOG(ERROR, "DBG:can't get record");
          } else {
            rec1->dump();
          }
                              ...
                              const TableRowkey *res_keys, const int64_t res_key_size)
{
  ...
  for(int64_t i = 0;i < req_key_size; i++) {
    ...
    if (idx == res_key_size) {
      ...
      buf[2 * len] = 0;
      YYSYS_LOG(INFO, "meet deleted key, key=%s", buf);
#endif
      ret = dump_del_key(req_keys[i]);
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(WARN, "can't dump_del_key");
        break;
      }
    }
  }
