...
void Charset::init(const char * _from_charset, const char * _to_charset)
{
  ...
  if( ctr >= 1 )
  {
    ...
    YYSYS_LOG(ERROR, "Only one class Charset permitted.");
  }
  else
  */
  if( OB_NOT_INIT == _errno_ )
  {
    ...
    if ( (cd = iconv_open( to_charset, from_charset) ) == 0 )
    {
      ...
      YYSYS_LOG(ERROR, "Conversion from %s to %s error.",from_charset,to_charset);
    }
    else
    {
      _errno_ = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(ERROR, "This object has been initialized.");
  }
}
...
int Charset::code_convert( char * buf, size_t capacity, size_t & len )
{
  ...
  if( ! fail() )
  {
    ...
    if(!tmpbuf)
    {
      YYSYS_LOG(ERROR, "fail to allocate memories during conversion");
    ...
    else
    {
      ...
      if( OB_SUCCESS != ret )
      {
        YYSYS_LOG(ERROR, "Conversion error");
      }
      else if( tmplen > capacity )
      {
        YYSYS_LOG(ERROR, "Memory over flow during conversion");
        ret = OB_MEM_OVERFLOW;
      }
      else
      {
