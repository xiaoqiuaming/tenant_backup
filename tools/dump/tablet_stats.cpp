...
void print_rowkey(const ObString &rowkey, string &str, string table, int64_t times)
{
  ...
  YYSYS_LOG(INFO, "table=%s, tablet.endkey=%s, times=%ld, rowkey=%s", table.c_str(), tablet_buf, times, rowkey_buf);
}
...
class TabletStatser : public RowkeyHandler {
  public:
    ...
    void do_force_search(std::string &table, const ObString &rowkey) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't fetch_tablet_info, table:%s", table.c_str());
        return;
      }
      ...
      if (itr != stats.end()) {
        ...
        times = 1;
        YYSYS_LOG(ERROR, "should not goes here");
      }
    }
    ...
    void print_update_count() {
      ...
      if (file == NULL) {
        YYSYS_LOG(ERROR, "can't create output file, name:%s", file_name);
        return;
      }
      fprintf(file, "%-25s%s\n", "[table id]", "[accesses]");
    ...
    void print_result() {
      ...
      if (fd < 0) {
        YYSYS_LOG(ERROR, "can't create output file, name:%s, ret=%d", file_name, fd);
        return;
      }
      ...
      while (itr != stats.end()) {
        ...
        if (file.write(buf, len, true) < 0) {
          YYSYS_LOG(ERROR, "can't write file,%d", len);
        }
    ...
    int fetch_tablet_info(string &table, const ObString &rowkey, TabletInfo &tablet_info)
    {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(INFO, "Table %s not find in tablet cache, do root server get", table.c_str());

        ret = db->get_ms_location(rowkey, table);
        if (ret == OB_SUCCESS)
          ret = db->search_tablet_cache(table, rowkey, tablet_info);
        else {
          YYSYS_LOG(ERROR, "get_ms_location faild , retcode:[%d]", ret);
        }
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS)
          YYSYS_LOG(ERROR, "can't get avail_sice");
      }
    ...
    void search_stats(string &table, std::string &rowkey) {
      ...
      if (itr != stats.end()) {
        ...
        YYSYS_LOG(ERROR, "strange rowkey");
      }
    }
    ...
    void calc_table_rows(const char *ip, unsigned port, const char *table_name, bool consistency, bool compatible) {
      ...
      if (!db || db->init()) {
        YYSYS_LOG(ERROR, "can't initialize database handle");
        return;
      }
...
//      print_result();
    ...
    int show_merge_process(const char *ip, unsigned port, int64_t dest_version, bool consistency) {
      ...
      if (schema_ == NULL) {
        YYSYS_LOG(ERROR, "can't allocate ObSchemaManagerV2");
        return -1;
      }

      int ret = db->fetch_schema(*schema_);
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't fetch schema from db");
        return -1;
      }
    ...
    void list_tablet_list(const char *ip, unsigned port, const char *table_name, bool consistency) {
      ...
      int ret = get_all_tablets(table_name);
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't get all tables");
        return;
      }
    ...
    int update_schema()
    {
      ...
      if (ret == OB_SUCCESS) {
        ret = db.init();
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't init database,%s:%d", DUMP_CONFIG->get_host(), DUMP_CONFIG->get_port());
        }
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't fetch schema from root server [%s:%d]", DUMP_CONFIG->get_host(), DUMP_CONFIG->get_port());
        }
      }
    ...
    int init(const char *ip, unsigned short port, int split, bool force_get, bool rotate_per_log) {
      ...
      if (!db || db->init() != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't init oceanbase db");
        ret = OB_ERROR;
      }
      ...
      if (ret == OB_SUCCESS) {
        ...
        while (true && split) {
          YYSYS_LOG(INFO, "spliting tablets");
          split_tablets(split);
          split = split / 2;
          if (split <= 1) {
            break;
    ...
    bool is_last_tablet(TabletInfo &tablet_info) {
...

      {
        ...
        YYSYS_LOG(INFO, "is last_tablet:%s", key_buf);
      }
    ...
    int init_all_tablets() {
      ...
      for(size_t i = 0;i < cfgs.size(); i++) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't get tables from table:%s", cfgs[i].table().c_str());
          break;
        }
      }
    ...
    int get_all_tablets(string table) {
      ...
      YYSYS_LOG(INFO, "get tablets from table:%s", table.c_str());
      do {
    ...
    int create_midkey(string &table, std::string &key1, std::string &key2, std::string *midkey) {
      ...
      if (ret != OB_SUCCESS) {
        ...
        if (key1.length() != key2.length()) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "key1:%zu and key2:%zu are not the same", key1.length(), key2.length());
        } else {
    ...
    void print_key(const std::string &table, std::string key) {
      ...
      buf[len * 2] = 0;
      YYSYS_LOG(INFO, "KEY:%s,TABLE:%s, len=%zu", buf, table.c_str(), key.length());
    }
    ...
    void dump_tablets(map<TabletKey, TabletValue, TabletKeyLesser> &stat) {
      ...
      while (itr != stat.end()) {
        ...
        buf[2 * len] = 0;
        YYSYS_LOG(INFO, "table=%s, key=%s", itr->first.table.c_str(), buf);
        itr++;
      }
    }
    ...
    void split_tablets(int split) {
      ...
      while (itr != stats.end()) {
        ...
        if (ret != OB_SUCCESS) {
          YYSYS_LOG(ERROR, "can't create mid key");
        } else {
          TabletKey key;
...
int setup_db_env(const char *config_file, bool start_deamon)
{
  ...
  if (ret == OB_SUCCESS && start_deamon) {
    ...
    YYSYS_LOG(ERROR, "initialize config file error, quiting");
  }
...
int do_stats(int split, bool force_get, bool rotate_per_log, bool count_update)
{
  ...
  if (count_update) {
    YYSYS_LOG(INFO, "counting updates during the day");
  ...
  if (ret != OB_SUCCESS) {
    YYSYS_LOG(ERROR, "can't start monitor service");
  } else {
    sleep(1);
...
int main(int argc, char *argv[])
{
  ...
  if (show_merge_process) {
    ...
    YYSYS_LOG(DEBUG, "memtable version = %ld", dest_version);
    OceanbaseDb::global_init(NULL, "ERROR");
    TabletStatser stater;
  ...
  if (name_to_id) {
    ...
    ret = db.init();
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(ERROR, "can't convert name to id, table_name=%s", table_name);
    } else {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't fetch schema from server, %s:%d", host, port);
      }
