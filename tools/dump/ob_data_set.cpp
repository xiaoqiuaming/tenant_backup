                    ...
                    const ObRowkey &end_key, int64_t version)
{
  ...
  if (end_key.ptr() == NULL || end_key.length() == 0) {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "error end key is specified");
  } else {
    start_key_ = start_key;
    end_key_ = end_key;
  ...
  if (db_ == NULL) {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "Database is not specified");
  }
  ...
  if (ret == OB_SUCCESS) {
    if (!ds_.inited()) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(ERROR, "can't init record set");
      }
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret != OB_SUCCESS) {
      YYSYS_LOG(WARN, "can't read more from oceanbase");
    }
  }
...
int ObDataSet::next()
{
  ...
  if (inited_ == true) {
    ...
    if (ds_itr_ != ds_.end()) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(WARN, "can't read more from db");
      }
    } else {
      has_more_ = false;
      YYSYS_LOG(DEBUG, "DataSet iterate to end");
    }
  } else {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "ObDataSet is not inited");
  }
...
{
  {
    ...
    YYSYS_LOG(INFO, "reading range--(%s:%s]", start_buff, end_buf);
  }
  ...
  if (ret == OB_SUCCESS) {
    if (ds_.empty()) {
      ...
      if (ret != OB_SUCCESS) {
        YYSYS_LOG(WARN, "can't get last_end_key");
      }
    ...
    YYSYS_LOG(WARN, "scan data from tablet failed");
  }
