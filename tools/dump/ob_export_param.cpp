...
int ExportParam::load(const char *file)
{
	int ret = OB_SUCCESS;
	if (NULL == file) {
		YYSYS_LOG(ERROR, "check conf file failed, file[%s]", file);
		ret = OB_ERROR;
	} else {
		ret = YYSYS_CONFIG.load(file);
		if (ret != OB_SUCCESS) {
			YYSYS_LOG(ERROR, "load conf failed, file[%s], ret[%d]", file, ret);
		}
	}

	if (OB_SUCCESS == ret) {
		vector<string> all_sections;
		YYSYS_CONFIG.getSectionName(all_sections);
		...
		for(size_t i = 0; i < all_sections.size(); i++) {
			...
			//init export query
			const char *str_sql = YYSYS_CONFIG.getString(param.table_name.c_str(), kExportSql);
                        if (str_sql != NULL) {
                                param.export_sql = str_sql;
                        } else {
                                YYSYS_LOG(WARN, "table[%s] export query is not specified, please check config file[%s]", param.table_name.c_str() ,file);
                                //break;
                        }
#if 0
			//init data_file
			const char *data_file = YYSYS_CONFIG.getString(param.table_name.c_str(),kDataFile);
			if (data_file != NULL) {
			...
			const char *str_delima = YYSYS_CONFIG.getString(param.table_name.c_str(), kDelima);
			if (str_delima != NULL) {
			...
			const char *str_rec_delima = YYSYS_CONFIG.getString(param.table_name.c_str(), kRecDelima);
			...
			if (param.delima.delima_type() != param.rec_delima.delima_type()) {
				YYSYS_LOG(WARN, "%s:delima and rec_delima length is not same, skiping config", param.table_name.c_str());
				continue;
			}
#endif
#if 0 //这部分参数已经移除,全部放置于命令行参数
			//init concurrency
			const char *str_concurr = YYSYS_CONFIG.getString(param.table_name.c_str(), kConcurrency);
			if (str_concurr != NULL) {
				param.concurrency = atoi(str_concurr);
			}

			//init max file size
			const char *str_filesize = YYSYS_CONFIG.getString(param.table_name.c_str(), kMaxFileSize);
			if (str_filesize != NULL) {
				param.max_file_size = atoi(str_filesize) * 1024 * 1024;
			}
			//init max reocrd num
			const char *str_recordnum = YYSYS_CONFIG.getString(param.table_name.c_str(), kMaxRecordNum);
			...
			const char *add_quotes_columns_str = YYSYS_CONFIG.getString(param.table_name.c_str(), kAddQuotes);
			if (add_quotes_columns_str != NULL) {
				...
				if (param.add_quotes.size() <= 0) {
					YYSYS_LOG(WARN, "error %s config, has no values", kAddQuotes);
				}
			}

			//init del point column
			const char *del_point_columns_str = YYSYS_CONFIG.getString(param.table_name.c_str(), kDelPoint);
			if (del_point_columns_str != NULL) {
				string value = del_point_columns_str;
				Tokenizer::tokenize(value, param.del_point, ',');
				if (param.add_quotes.size() <= 0) {
					YYSYS_LOG(WARN, "error %s config, has no values", kDelPoint);
				}
			}

			//init date/time/timestamp format column
			vector<const char *> format_columns_desc = YYSYS_CONFIG.getStringList(param.table_name.c_str(), kColumnFormat);
			for (size_t idx = 0; idx < format_columns_desc.size(); idx++) {
				...
				if (res.size() != 2) {
					ret = OB_ERROR;
					YYSYS_LOG(ERROR, "error column config, %s=%s", kColumnFormat, format_columns_desc[idx]);
					break;
				}
			...
			if (ret != OB_SUCCESS) {
				YYSYS_LOG(ERROR, "parse table[%s] config failed", param.table_name.c_str());
				continue;
			}
#endif
...
void ExportParam::PrintDebug()
{
	for(size_t i = 0; i < params_.size(); i++) {
		...
		YYSYS_LOG(DEBUG, "#######################################################################");
		YYSYS_LOG(DEBUG, "table name=[%s]", param.table_name.c_str());
		YYSYS_LOG(DEBUG, "delima type = %d, part1 = %d, part2 = %d", param.delima.type_, param.delima.part1_, param.delima.part2_);
		YYSYS_LOG(DEBUG, "rec_delima type = %d, part1 = %d, part2 = %d", param.rec_delima.type_, param.rec_delima.part1_, param.rec_delima.part2_);
		//YYSYS_LOG(DEBUG, "max file size = %ld", param.max_file_size);
		//YYSYS_LOG(DEBUG, "max record num = %ld", param.max_record_num);
		//YYSYS_LOG(DEBUG, "concurrency = %d", param.concurrency);
		YYSYS_LOG(DEBUG, "export sql = %s", param.export_sql.c_str());
		size_t idx;
		for (idx = 0; idx < param.col_formats.size(); idx++) {
			YYSYS_LOG(DEBUG, "column format = [%s,%s]", param.col_formats[idx].name.c_str(), param.col_formats[idx].format.c_str());
		}
		for (idx = 0; idx < param.add_quotes.size(); idx++) {
			YYSYS_LOG(DEBUG, "add quotes = [%s]", param.add_quotes[idx].c_str());
		}
		for (idx = 0; idx < param.del_point.size(); idx++) {
			YYSYS_LOG(DEBUG, "del point = [%s]", param.del_point[idx].c_str());
		}
	}
}
