...
//add:end
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret != OB_SUCCESS) {
       error_arr[5] = false;//add by pangtz:20141205
      YYSYS_LOG(ERROR, "can't fetch schema from root server [%s:%d]", host, port);
    }
  }
  ...
  if (ret == OB_SUCCESS) {
    ...
    if (ret != OB_SUCCESS) {
    error_arr[11] = false;//add by pangtz:20141205
      YYSYS_LOG(ERROR, "can't setup column descripes");
    }
...
//add e
    ...
    if (OB_SUCCESS == ret) {
        if(!g_cluster_state) {
            ...
            error_arr[10] = false;
            //add e
            YYSYS_LOG(ERROR, "root server has swithced or no root server, please re-run ob_import");
            ret = OB_ERROR;
        }
    }
    //add:end

    //add by liyongfeng:20141020 �ж��Ƿ�����UPS�л�
    /*
    if (OB_SUCCESS == ret) {
        //RPC_WITH_RETRIES_SLEEP(db.get_latest_update_server(), 3, ret);
        ret = db.get_latest_update_server();
        if (ret != OB_SUCCESS) {
            YYSYS_LOG(ERROR, "ob_import maybe lost some record, please re-run ob_import");
        }
    }
    */
    //add:end
