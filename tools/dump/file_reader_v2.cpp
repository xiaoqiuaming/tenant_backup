...
FileReader::FileReader(const char *file_name, bool g2u):
  file_(kReadBufferSize, kReadBufferSize), //add for [dio]
  g2u_(g2u),
...
int FileReader::open()
{
  ...
  if (dio) {
    int fd = OB_SUCCESS;
    ObString file_name;
    file_name.assign(const_cast<char *>(file_name_),
                     static_cast<ObString::obstr_size_t>(strlen(file_name_)));
    if (OB_SUCCESS != (fd = file_.open(file_name)))
    {
      ret = -1;
      YYSYS_LOG(ERROR, "can't open file, name=%s", file_name_);
    }
    else if (NULL == (reader_buffer_ = new(std::nothrow) char[kReadBufferSize]))
    {
      ret = -1;
      YYSYS_LOG(ERROR, "can't allocate memory for read_buffer, size=%d", kReadBufferSize);
    }
  } else {
    int fd = old_file_.open(file_name_, O_RDONLY);
    if (fd > 0) {
      reader_buffer_ = new(std::nothrow) char[kReadBufferSize];
      if (NULL == reader_buffer_) {
        ret = -1;
        YYSYS_LOG(ERROR, "can't allocate memory for read_buffer, size=%d", kReadBufferSize);
      }
    } else {
...
int FileReader::get_records(RecordBlock &block, const RecordDelima &rec_delima, const RecordDelima &col_delima, int64_t max_rec_extracted)
{
  ...
  if (buffer_size_ < valid_buff) {
    if (dio) {
      ret = static_cast<int32_t>(file_.pread(reader_buffer_ + buffer_size_ + with_last_gbk_char,
                                             valid_buff - buffer_size_ - with_last_gbk_char,
                                             file_offset_, len));
      if (ret == OB_SUCCESS && len != -1)
      {
        buffer_size_ += (len + with_last_gbk_char);
        file_offset_ += len;
      } else {
        YYSYS_LOG(ERROR, "can't read from file, name=%s", file_name_);
      }
    }
    else
    {
      len = (old_file_.read(reader_buffer_ + buffer_size_ + with_last_gbk_char, valid_buff - buffer_size_ - with_last_gbk_char)) + with_last_gbk_char;
      if (len != -1) {
         buffer_size_ += len;
      } else {
        ret = -1;
  ...
  //  YYSYS_LOG(INFO, "TEMP LOG BEGIN ====== the latter string\n%.*s\nTEMP LOG END ======\n",10,reader_buffer_ + buffer_size_ - len);
  ...
  if( ret == OB_SUCCESS && g2u_ )
  {
    ...
    {
      YYSYS_LOG(ERROR, "Inadequate buffer for transformation from gbk to utf-8!");
    ...
    else
    {
      ...
      {
        //Hypothetically code of �� is [0xAA][0xBB], separated by our file reader.
  ...
  if (ret == OB_SUCCESS) {
    ...
    //mod:e
      eof_ = true;
      YYSYS_LOG(DEBUG, "FileReader:read file end, name=%s", file_name_);
    } 

    if (buffer_size_ != 0) {
      YYSYS_LOG(DEBUG, "extract record in file, buffersize = %ld", buffer_size_);
      ret = extract_record(block, rec_delima, col_delima, max_rec_extracted);
	  if (len == 0 && ret == 1) {
		  YYSYS_LOG(WARN, "last line record is partial, add rec_delima");
		  rec_delima.append_delima(reader_buffer_, buffer_size_, kReadBufferSize);
		  rec_delima.skip_delima(buffer_size_);
	  }
    } else {
      YYSYS_LOG(DEBUG, "All data is processed");
    }
  }
...
bool FileReader::is_partial_record(int64_t &pos, const RecordDelima &rec_delima, const RecordDelima &col_delima)
{
  ...
  int64_t first_rec_delima_pos = buffer_size_;
  int64_t first_col_delima_pos = buffer_size_;
  int meet_rec_delima_count = 0;
  int meet_col_delima_count = 0;
  ...
  //mod by zhuxh:20160912 [quotation mark] :e
  {
    ...
    case 1: //between double quotation marks
      ...
      if( reader_buffer_[pos]==q )
        quotation_mark_ctr ++;
      else if((pos>0) && (reader_buffer_[pos-1]==q) && rec_delima.is_delima(reader_buffer_, pos, buffer_size_))
      //mod by zhuxh:20160912 [quotation mark] :e
      {
        running=false;
        first_rec_delima_pos = buffer_size_;
        first_col_delima_pos = buffer_size_;
        meet_rec_delima_count = 0;
        meet_col_delima_count = 0;
        break;
      }
      else if((pos>0) && (reader_buffer_[pos-1]==q) && col_delima.is_delima(reader_buffer_, pos, buffer_size_))
      {
        step=0;
        first_rec_delima_pos = buffer_size_;
        first_col_delima_pos = buffer_size_;
      ...
      else if (rec_delima.is_delima(reader_buffer_, pos, buffer_size_))
      {
        meet_rec_delima_count += 1;
        if (meet_rec_delima_count == 1)
        {
          first_rec_delima_pos = pos;
        }
      }
      else if (col_delima.is_delima(reader_buffer_, pos, buffer_size_))
      {
        meet_col_delima_count += 1;
        if (meet_col_delima_count == 1)
        {
          first_col_delima_pos = pos;
        }
      }
      break;
    }
    ...
    if ((meet_col_delima_count >= 10 && meet_rec_delima_count >= 1)
        || (buffer_size_ < kReadBufferSize && pos == buffer_size_ - 1
            && (meet_col_delima_count >= 1 || meet_rec_delima_count >= 1)))
    {
      if (first_col_delima_pos < first_rec_delima_pos)
      {
        pos = first_col_delima_pos;
        step = 0;
        first_rec_delima_pos = buffer_size_;
        first_col_delima_pos = buffer_size_;
        meet_rec_delima_count = 0;
      ...
      {
        pos = first_rec_delima_pos;
        running = false;
      }
      YYSYS_LOG(DEBUG, "in is_partial_record, first_rec_delima_pos = %ld, first_col_delima_pos = %ld", first_rec_delima_pos, first_col_delima_pos);
    }
    if(!running)break;
  }
  //add :e
...
int FileReader::skip_overflow_record(const RecordDelima &rec_delima, const RecordDelima &col_delima)
{
  ...
  while (0 == ret)
  {
    ...
    if (buffer_size_ < kReadBufferSize) {
      if (dio) {
        ret = static_cast<int32_t>(file_.pread(reader_buffer_ + buffer_size_,
                                               kReadBufferSize - buffer_size_,
                                               file_offset_, len));
        if (ret == OB_SUCCESS && len != -1)
        {
          buffer_size_ += len;
        ...
        len = (old_file_.read(reader_buffer_ + buffer_size_, kReadBufferSize - buffer_size_));
        if (len != -1)
        {
           buffer_size_ += len;
        }
    ...
    if (ret == 0) {
      ...
      if (buffer_size_ != 0) {
        ...
        ret = -1;
        YYSYS_LOG(WARN, "buffer_size_ should not be zero");
      }
    }
  }
  if (NULL != bad_file_)
  ...
  do
  {
    while (pos < buffer_size_) {
      ...
      if (rec_nr >= max_rec_extracted) {
        YYSYS_LOG(DEBUG, "extract rec count[%ld] reaches max_rec_extracted[%ld]", rec_nr, max_rec_extracted);
        break;
      }

      if ((has_partial_rec = is_partial_record(pos, rec_delima, col_delima)) == true) {
        YYSYS_LOG(DEBUG, "partial record meet, will be proceed next time");
        break;
      }
    ...
    YYSYS_LOG(DEBUG, "record block actual used size[%ld], rec_nr[%ld]", pos, rec_nr);
...
int EncodingFileReader::open()
{
  ...
  if (raw_buffer_ == NULL) {
    ret = -1;
    YYSYS_LOG(ERROR, "can't allocate memory for raw_buffer_, size=%d", kReadBufferSize);
  }
  if (ret == 0) {
    cd_ = iconv_open("utf-8", "gbk");
    if (NULL == cd_) {
      ret = -1;
      YYSYS_LOG(ERROR, "fail to open iconv gbk -> utf8");
    }
  }
  if (ret == 0) {
    ret = FileReader::open();
...
int EncodingFileReader::convert_string_encode()
{
  ...
  if (-1 == (int)iconv_ret && errno != E2BIG) {
    ret = -1;
    YYSYS_LOG(ERROR, "fail to iconv, errno[%d]", errno);
    return ret;
  }

  if (eof_ && 0 != raw_buf_size) {
    ret = -1;
    YYSYS_LOG(ERROR, "when file is end, raw_buf_size should be zero");
    //���ܷ�������������Ҳ����bug
  }
...
int g2u(char * buf, size_t & len)
{
  ...
  if(!tmpbuf)
  {
    YYSYS_LOG(ERROR, "fail to allocate memories in transform from gbk to utf-8!");
    ret = OB_ERROR;
  }
  else
  {
...
bool is_part_gbk(char * str, int len)
{
  ...
  for(;i>0;i--,ctr++)
    if( static_cast<unsigned int>(str[i]) <= 0x80 )
      break;
  //if( ctr&1 )YYSYS_LOG(INFO, "TEMP LOG BEGIN\n%d:\n%.*s\nTEMP LOG END\n",ctr,len,str);//ctr,str+i+1);
  return ctr&1;
}
#endif
