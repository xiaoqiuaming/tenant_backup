  ...
  public:
    ...
    virtual int Close() {
      ...
      if (unused > 0) {
        ...
        if (ftruncate(fd_, file_offset_ - unused) < 0) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "error ocurrs when ftruncate %s, due to %s", 
      ...
      if (close(fd_) < 0) {
        if (ret == OB_SUCCESS) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "error ocurrs when close %s, due to %s", 
                    filename_.c_str(), strerror(errno));
        }
      }
    ...
    virtual int Sync() {
      ...
      if (pending_sync_) {
        ...
        if (fdatasync(fd_) < 0) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "error ocurrs when fdatasync %s, due to %s", 
                    filename_.c_str(), strerror(errno));
        }
      }
      ...
      if (dst_ > last_sync_) {
        ...
        if (msync(base_ + p1, p2 - p1 + page_size_, MS_SYNC) < 0) {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "error ocurrs when msync%s, due to %s", 
                    filename_.c_str(), strerror(errno));
        }
      }
