...
#define DATE_MAX 64
  ...
  YYSYS_LOG(INFO, "TIME SECTION:%s %s", time_start, time_end);

  if (time_start == NULL || time_end == NULL || strcmp(time_end, time_start) < 0) 
  {
    YYSYS_LOG(ERROR, "Time section error");
    return -1;
  }
  ...
  if( NULL == log_dir )
  {
    YYSYS_LOG(ERROR, "Can't open path,%s", log_dir_path);
    return -1;
  }
  ...
  while( (file_in_dir=readdir(log_dir)) && NULL!=file_in_dir )
  {
    ...
    if( true == S_ISREG(file_stat.st_mode) )
    {
      ...
      YYSYS_LOG(DEBUG, "LOGID:%s, LOGTIME:%s", file_path, file_ctime_s);
  ...
  if (log_end == LONG_MAX || log_start == LONG_MAX) {
    YYSYS_LOG(INFO, "Time Interval Wrong");
    closedir(log_dir);
    return -1;
  }
...
//    bool print_flag = false;
    ...
    {
      YYSYS_LOG(DEBUG, "skiping log, timestamp=%ld, %s", mut_time, BINLOG_PARAM.get_start_time_str());
    }
    else
    {
  ...
  while ((opt = getopt(argc, argv, "l:k:s:t:d:b:e:co:m:")) != -1) 
  {
    switch (opt) 
    {
     ...
     case 't':
       ...
       YYSYS_LOG(INFO, "accept table id [%ld]", BINLOG_PARAM.get_table_id());
       break;
     case 'd':
       ret = 0;
     ...
     case 'm':
       ...
       YYSYS_LOG(INFO, "using column id ,[%ld]", BINLOG_PARAM.get_column_id());
       break;
     case 'o':
       fname = optarg;
       break;
...
int handle_commitlogs()
{
  ...
  if (search_logs(BINLOG_PARAM.get_commit_log_dir(), 
                  ...
                  begin_id, end_id) == 0) 
  {
    YYSYS_LOG(INFO, "Log Section need to be scand[%ld, %ld]", begin_id, end_id);
    while (begin_id <= end_id) 
    {
      YYSYS_LOG(INFO, "processing commitlog [%ld]", begin_id);
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    if (BINLOG_PARAM.get_start_time_str() != NULL)
    {
      YYSYS_LOG(INFO, "START:[%s--%ld], END:[%s--%ld]", BINLOG_PARAM.get_start_time_str(),
    ...
    else
    {
      ...
      {
        YYSYS_LOG(INFO, "processing one log [%ld]\n", BINLOG_PARAM.get_log_id());
