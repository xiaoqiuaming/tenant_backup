...
const int64_t SequenceInfo::UNDEFINED = LONG_LONG_MIN;
...
int ImportParam::load(const char *file)
{
  ...
  if (NULL == file)
  {
    error_arr[CONF_FILE_ERROR] = false;
  ...
  if (ret == OB_SUCCESS) {
    ...
    for(size_t i = 0;i < all_sections.size() && ret == OB_SUCCESS; i++) {
      ...
      param.bad_file_ = YYSYS_CONFIG.getString(param.table_name.c_str(), kBadFile);

      const char *str_input_column_nr =
          YYSYS_CONFIG.getString(param.table_name.c_str(), kInputColumnNr);
      if (str_input_column_nr == NULL) {
        YYSYS_LOG(WARN, "input columns nr is not specified for table=%s, skiping it",
...
//    param.input_column_nr = atoi(str_input_column_nr);
      ...
      const char *str_delima =
          YYSYS_CONFIG.getString(param.table_name.c_str(), kDelima);
      if (str_delima != NULL) {
      ...
      const char *str_rec_delima =
          YYSYS_CONFIG.getString(param.table_name.c_str(), kRecDelima);
      ...
      if (param.delima.delima_type() != param.rec_delima.delima_type()) {
        YYSYS_LOG(WARN, "%s:delima and rec_delima is not same, skiping config", param.table_name.c_str());
        continue;
      }
      ...
      const char *str_has_table_title = YYSYS_CONFIG.getString(param.table_name.c_str(), kHasTableTitle);
      ...
      const char *str_nop_flag = YYSYS_CONFIG.getString(param.table_name.c_str(), kNopFlag);
      ...
      const char *str_null_flag = YYSYS_CONFIG.getString(param.table_name.c_str(), kNullFlag);
      ...
      //add by pangtz:20141217 ����substr������Ϣ
      const char *str_substr = YYSYS_CONFIG.getString(param.table_name.c_str(), kSubstr);
      if(str_substr != NULL) {
        ...
        for(size_t index = 0; index < substr_info.size(); index++){
          ...
          if(substr_info_original.size() != 2){
            ret = OB_ERROR;
            error_arr[CONF_FILE_ERROR] = false;
            YYSYS_LOG(ERROR, "error substr config, %s", str_substr);
            break;
          }
...
//        param.substr_map[substr_struct.column_name] = substr_struct;
      ...
      const char *trim_str = YYSYS_CONFIG.getString(param.table_name.c_str(), kTrim);
      if(trim_str != NULL) {
        param.is_trim_ = (atoi(trim_str) != 0);
      }
#endif

      const char *decimal_to_varchar_str = YYSYS_CONFIG.getString(param.table_name.c_str(), kDecimalToVarchar);    //add by liyongfeng 20140807:e
...
//          param.decimal_to_varchar_fields_.push_back(column_offset);
      ...
      std::vector<const char *> columns_desc =
          YYSYS_CONFIG.getStringList(param.table_name.c_str(), kColumnDesc);
      ...
      for (;idx < columns_desc.size();idx++) {
        ...
        if (res.size() != 2) {                  /* name,[int]--offset */
          ret = OB_ERROR;
          error_arr[CONF_FILE_ERROR] = false;//add by pangtz:20141218
          YYSYS_LOG(ERROR, "error column config, %s", str.c_str());
          break;
        }
...
int ImportParam::get_table_param(const char *table_name, TableParam &param)
{
  ...
  error_arr[GET_TABLE_PARAM_ERROR] = false;
  return OB_ERROR;
}
