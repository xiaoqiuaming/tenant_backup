...
int DbRowFormator::init()
{
  ...
  //  {
  //     YYSYS_LOG(ERROR, "failed to allocate line buffer");
  //     ret = OB_ERROR;
  //  }
  //  buff_pos_ = 0;
  ...
  for (; idx < param_.del_point.size() && OB_SUCCESS == ret; idx++)
  {
    ...
    if (NULL == col_schema)
    {
      YYSYS_LOG(ERROR, "can't find column[%s] in table[%s], please check config file", param_.del_point[idx].c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else if (col_schema->get_type() != ObVarcharType && col_schema->get_type() != ObDecimalType)
    {
      YYSYS_LOG(ERROR, "column[%s] is not VARCHAR or DECIMAL in table[%s], should not delete last point", param_.del_point[idx].c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else
    {
      YYSYS_LOG(DEBUG, "find column[%s] in table[%s]", param_.del_point[idx].c_str(), param_.table_name.c_str());
      ...
      {
        YYSYS_LOG(ERROR, "column[%s] mustn't delete last point, maybe add quotes or formating, please check config file", param_.del_point[idx].c_str());
        ret = OB_ERROR;
      }
      else
      {
  ...
  for (idx = 0; idx < param_.col_formats.size() && OB_SUCCESS == ret; idx++)
  {
    ...
    if (NULL == col_schema)
    {
      YYSYS_LOG(ERROR, "can't find column[%s] in table[%s], please check config file", param_.col_formats[idx].name.c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else if (col_schema->get_type() != ObVarcharType && col_schema->get_type() != ObPreciseDateTimeType && col_schema->get_type() != ObDateTimeType)
    {
      YYSYS_LOG(ERROR, "column[%s] is not VARCHAR or TIMESTAMP in table[%s], should not format", param_.col_formats[idx].name.c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else
    {
      YYSYS_LOG(DEBUG, "find column[%s] in table[%s]", param_.col_formats[idx].name.c_str(), param_.table_name.c_str());
      //uint64_t cid = col_schema->get_id();
      ColumnCache::iterator itr = format_columns_.find(col_schema->get_name());
      if (itr != format_columns_.end())
      {
        YYSYS_LOG(ERROR, "column[%s] mustn't format, maybe add quotes or delete last point, please check config file", param_.col_formats[idx].name.c_str());
        ret =OB_ERROR;
      }
      else
      {
        ...
        /* 进行格式化格式匹配与转换 */
        int l = static_cast<int>(sizeof(user_format_style_) / sizeof(char *));
        int i = 0;
        for (; i < l; i++)
        {
        ...
        if (i == l)
        {
          YYSYS_LOG(ERROR, "user specify error format, not support format[%s], column[%s], please check config file",
  ...
  for (idx = 0; idx < param_.add_quotes.size() && OB_SUCCESS == ret; idx++)
  {
    ...
    if (NULL == col_schema)
    {
      YYSYS_LOG(ERROR, "can't find column[%s] in table[%s], please check config file", param_.add_quotes[idx].c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else if (col_schema->get_type() != ObVarcharType)
    {
      YYSYS_LOG(ERROR, "column[%s] is not VARCHAR in table[%s], should not add quotes", param_.add_quotes[idx].c_str(), param_.table_name.c_str());
      ret = OB_ERROR;
    }
    else
    {
      YYSYS_LOG(DEBUG, "find column[%s] in table[%s]", param_.add_quotes[idx].c_str(), param_.table_name.c_str());
      //uint64_t cid = col_schema->get_id();
      ColumnCache::iterator itr = format_columns_.find(col_schema->get_name());
      if (itr != format_columns_.end())
      {
        YYSYS_LOG(ERROR, "column[%s] mustn't add quotes, maybe formating or delete last point, please check config file", param_.add_quotes[idx].c_str());
        ret = OB_ERROR;
      }
      else
      {
...
DbRowFormator::TSIBuffer *DbRowFormator::get_tsi_buffer()
{
  ...
  if(NULL == tsi_buffer)
  {
    YYSYS_LOG(ERROR, "failed to allocate memory for thread tsi_buffer!");
  }
  else
  {
    if(NULL == tsi_buffer->get_line_buffer())
    {
      ...
      {
        YYSYS_LOG(ERROR, "tsi_buffer init failed!");
        tsi_buffer = NULL;
      }
    }
  }
...
const char* DbRowFormator::get_line_buffer()
{
  ...
  {
    YYSYS_LOG(ERROR, "get tsi_buffer failed!");
  }
  return buffer;
}
//add:e
...
void databuff_copy(char *dest_buff, const int64_t dest_size, int64_t &buff_pos, const int64_t src_len, const char *src)
{
  int64_t len = 0;
  if (dest_buff != NULL && src != NULL && 0 <= buff_pos && buff_pos < dest_size)
  {
    len = (src_len < dest_size - buff_pos) ? src_len : (dest_size - buff_pos);
    memcpy(dest_buff + buff_pos, src, len);
    buff_pos += len;
  }
}
...
int DbRowFormator::format(const ObRow &row, int32_t &bad, Charset & charset) //mod by zhuxh:20161018 [add class Charset]
{
  ...
  {
    if (param_.rowkey_index.size() > 0)
    {
      vector<int>::iterator it = find(param_.rowkey_index.begin(), param_.rowkey_index.end(), cell_idx);
      if (it != param_.rowkey_index.end())
      {
        YYSYS_LOG(DEBUG, "The column %d does not need to be output", *it);
        continue;
      }
  }
  if (OB_SUCCESS != (ret = row.raw_get_cell(cell_idx, cell, tid, cid)))
  {
    YYSYS_LOG(ERROR, "fialed to get one cell, tid=[%ld], cid=[%ld], ret=[%d]", tid, cid, ret);
    break;
  }
  else if (OB_SUCCESS != (ret = format_column(cell, (int)cell_idx, bad_format, charset))) //mod by zhuxh:20161018 [add class Charset]
  {
    YYSYS_LOG(ERROR, "failed to format one column into buffer, tid=[%ld], cid=[%ld], ret=[%d]", tid, cid, ret);
    break;
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to transform column in specified format");
      bad = 1;
    }
  ...
  switch (type)
  {
    ...
    case ObVarcharType:
    {
      ...
      */
      ...
      else
      {
        ...
        if( ! to_buff )
        {
          ...
          YYSYS_LOG(ERROR, "failed to allocate memory for code conversion buffer!");
        }
        else
        {
          ...
          if( OB_SUCCESS != ret )
          {
            YYSYS_LOG(ERROR, "Code conversion error.");
          }
          else
          {
            char_val.assign_ptr( to_buff, static_cast<oceanbase::common::ObString::obstr_size_t>(to_buff_size) );
            //YYSYS_LOG(INFO, "zxh: len=%d, str=[%.*s]",char_val.length(),char_val.length(),char_val.ptr());
          }
        }
      }
        ...
        if (is_format)
        {
          ...
          else//添加双引号分隔
          {
            ...
            for (int64_t i = 0; (OB_SUCCESS == ret) && i < char_val.length(); i++)
            {
              ...
              //YYSYS_LOG(INFO, "zxh: sum=%d,i=%ld",char_val.length(),i);
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "Buff over flows.");
              break;
            }
            //databuff_printf(line_buffer, kLineBufferSize, buff_pos, "\"%.*s\"", char_val.length(), char_val.ptr());
            //add 20160909:e
      ...
      else if (g2u_)
      {
        ...
        if (cd == 0){
          ...
          YYSYS_LOG(WARN, "charset:GBK convert to UTF-8 failed, do nothing!");
          databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%.*s", char_val.length(), char_val.ptr());
        }
      }
      else if (u2g_)
      {
        ...
        if (cd == 0){
          ...
          YYSYS_LOG(WARN, "charset:UTF-8 convert to GBK failed, do nothing!");
          databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%.*s", char_val.length(), char_val.ptr());
        }
      }
      //add 20160629:e
      */
        ...
        {
//        databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%.*s", char_val.length(), char_val.ptr());
          databuff_copy(line_buffer, kLineBufferSize, buff_pos, char_val.length(), char_val.ptr());
...
//        databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%.*s", static_cast<int>(strlen(res)), res);
//      }
      //delete for [Decimal]-e
      if (OB_SUCCESS != ret)
      {
//      databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%.*s", decimal_val.length(), decimal_val.ptr());
        databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%s", "get decimal error!");
        bad = 1;
        ret = OB_SUCCESS;
      }
    ...
    case ObDateTimeType:
      //add dinggh [add modifytime/createtime type] 20170905:b
    case ObModifyTimeType:
    case ObCreateTimeType:
      //add:e
    case ObPreciseDateTimeType:
    {
      //delete for [datetime buf]-b
...
//      localtime_r(&time_s, &time_struct);
      ...
      {
        cell->get_timestamp(int_val);
        time_t t = static_cast<time_t>(int_val/1000000L);
        int32_t usec = abs(static_cast<int32_t>(int_val % 1000000L));
        if ( 0 > int_val && 0 != usec)
        {
          t = t - 1;
          usec = 1000000 - usec;
        ...
        localtime_r(&t, &gtm);
        if (0 == export_col_info->idx)
          databuff_printf(line_buffer, kLineBufferSize, buff_pos, sys_format_style2_[0], gtm.tm_year + 1900, gtm.tm_mon + 1, gtm.tm_mday);
        else if (1 == export_col_info->idx)
          databuff_printf(line_buffer, kLineBufferSize, buff_pos, sys_format_style2_[1], gtm.tm_year + 1900, gtm.tm_mon + 1, gtm.tm_mday);
        else if (2 == export_col_info->idx)
...
//                        time_struct.tm_hour, time_struct.tm_min, time_struct.tm_sec);
        char tmp_column[32];
        int64_t tmp_used = 0;
        if (OB_SUCCESS != (ret = obj_cast(const_cast<ObObj&>(*cell), ObVarcharType, tmp_column, 32, tmp_used)))
        {
          YYSYS_LOG(WARN, "failed to cast obj,ret=%d", ret);
          break;
        }
        //if (time_us != 0) //delete dinggh 20160918
        databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%s", tmp_column);
      }
      break;
    }
    case ObNullType:
    ...
    case ObDateType:
    {
      char tmp_column[12];
      int64_t tmp_used = 0;
      if (OB_SUCCESS != (ret = obj_cast(const_cast<ObObj&>(*cell), ObVarcharType, tmp_column, 12, tmp_used)))
      {
        YYSYS_LOG(WARN, "failed to cast obj, ret=%d",ret);
        break;
      }
    ...
    case ObTimeType:
    {
      char tmp_column[20];
      int64_t tmp_used = 0;
      if (OB_SUCCESS != (ret = obj_cast(const_cast<ObObj&>(*cell), ObVarcharType, tmp_column, 20, tmp_used)))
      {
        YYSYS_LOG(WARN, "failed to cast obj, ret=%d",ret);
        break;
      }
    ...
    case ObBoolType:
    {
      cell->get_bool(bool_val);
      databuff_printf(line_buffer, kLineBufferSize, buff_pos, "%d", (bool_val ? 1 : 0));
      break;
    }
    default:
    {
      YYSYS_LOG(ERROR, "ob_export not support data type[%d]", type);
      ret = OB_ERROR;
      break;
    }
...
//  {
//    YYSYS_LOG(ERROR, "find column name by index error!");
//    return OB_ERROR;
//  }
//  cname = iter->second;
//  return OB_SUCCESS;
