...
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int ObMergeMetaNew::read_file_list(const char *file_list)
    {
      ...
      else
      {
        ...
        while( NULL != fgets(buf,sizeof(buf),fp) )
        {
          ...
          {
            YYSYS_LOG(ERROR,"parse %s failed [%d]",buf,ret);
            break;
          }
        }
        ...
        if (!just_merge_ && OB_SUCCESS == ret)
        {
          ...
          for(int32_t i=1;i<=10;++i)
          {
            ...
            {
              YYSYS_LOG(ERROR,"write failed [%d]",ret);
            }
            image_final_.dump();
          }
        }
    ...
    int ObMergeMetaNew::parse(const char *idx_file)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO,"deal %s\n",idx_file);

        if ( (fstat(fd,&st) != 0) || (st.st_size <= 0))
        {
          YYSYS_LOG(ERROR,"fstat failed");
          ret = OB_ERROR;
        }
      }
      ...
      YYSYS_LOG(INFO,"disk_no is %d,ret is %d",disk_no,ret);

      image_tmp_.set_data_version(version_);

      if ((OB_SUCCESS == ret) &&  (ret = image_tmp_.read(idx_file,disk_no)) != OB_SUCCESS)
      {
        YYSYS_LOG(INFO,"deserialize image failed.");
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == (ret = image_tmp_.begin_scan_tablets()))
        {
          ...
          while(OB_SUCCESS == ret && tablet != NULL)
          {
            ...
            {
              YYSYS_LOG(ERROR,"alloc tablet object failed : [%d]",ret);
            }

            YYSYS_LOG(INFO,"id is %ld",tablet->get_sstable_id_list().at(0).sstable_file_id_);
          ...
          YYSYS_LOG(INFO,"destroy image_tmp_");
          image_tmp_.destroy();
        }
      }
      ...
      YYSYS_LOG(INFO,"total tablet:%d",image_.get_tablets_num());
      return ret;
    }
    ...
    int ObMergeMetaNew::process_ring_range()
    {
      ...
      YYSYS_LOG(INFO,"process ring range");
    ...
    int ObMergeMetaNew::drop_not_exist_file()
    {
      ...
      YYSYS_LOG(INFO,"drop_not_exit_file");
      if (OB_SUCCESS == (ret = image_.begin_scan_tablets()))
      {
        ret = image_.get_next_tablet(tablet);
        while(OB_SUCCESS == ret && tablet != NULL)
        {
          ...
          {
            YYSYS_LOG(INFO,"file %s is not exist",path);
          }
          else
          {
            YYSYS_LOG(INFO,"file %s is exist",path);
            //copy meta
      ...
      YYSYS_LOG(INFO,"this host has %d tablets",image_.get_tablets_num());
      return ret;
    }
