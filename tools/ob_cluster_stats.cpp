 ...
 *
 *  Version: $Id: ob_cluster_stats.cpp 2010��12��22�� 15ʱ13��21�� qushan Exp $
 *
 *  Authors:
...
namespace oceanbase
{
  namespace tools
  {
    ...
    int load_string(yysys::CConfig& config, char* dest, const int32_t size,
        const char* section, const char* name, bool not_null)
    {
      ...
      if (OB_SUCCESS == ret && NULL != value)
      {
        ...
        {
          YYSYS_LOG(ERROR, "%s.%s too long, length (%d) > %d",
