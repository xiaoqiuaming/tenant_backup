...
namespace oceanbase
{
  namespace msync
  {
    ...
    {
      int err = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "client.initialize(config=%s)=>%d", config_.to_str(), err);
      ...
      {
        YYSYS_LOG(ERROR, "client.initialize(config=%s)=>%d", config_.to_str(), err);
      }
      else if (OB_SUCCESS != (err = client_.start_sync_mutator()))
      {
        YYSYS_LOG(ERROR, "client.start_send_mutator()=>%d", err);
      }
      else
      {
        client_.wait();
