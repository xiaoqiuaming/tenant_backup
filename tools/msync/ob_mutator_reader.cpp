...
namespace oceanbase
{
  namespace msync
  {
    ...
    //   YYSYS_LOG(INFO, "cell{op=%lu, table_id=%lu, column_id=%lu", op, table_id, column_id);
    // }
                                    ...
                                    const uint64_t last_seq_id, int64_t wait_time)
    {
      ...
      {
        YYSYS_LOG(WARN, "schemaNameMap.initialize(schema='%s')=>%d", schema, err);
      }
      else if (OB_SUCCESS != (err = reader_.init(&repeated_reader_, log_dir, log_file_start_id, last_seq_id, true)))
      {
        YYSYS_LOG(WARN, "reader.initialize(log_dir='%s', log_file_id=%lu, last_seq_id=%lu)=>%d",
    ...
    int ObMutatorReader::read_(ObMutator& mutator, uint64_t& seq, int64_t size_limit)
    {
      ...
      {
        //YYSYS_LOG(DEBUG, "mutator.get_serialize_size()=%ld", mutator.get_serialize_size());
        if (OB_SUCCESS != (err = get_mutator(cur_mutator, tmp_seq, timeout)))
        {
          if(OB_READ_NOTHING != err)
          {
            YYSYS_LOG(ERROR, "ObMutatorReader.get_mutator(last_seq=%ld)=>%d", last_seq_, err);
          }
          else
          {
            YYSYS_LOG(DEBUG, "ObMutatorReader.get_mutator(last_seq=%ld)=>%d", last_seq_, err);
          }
        }
        else if (OB_SUCCESS != (err = mutator_add(mutator, *cur_mutator, size_limit)))
        {
          ...
          {
            YYSYS_LOG(ERROR, "ObMutatorReader::mutator_add(last_seq=%ld)=>%d", last_seq_, err);
          }
          else
          {
            YYSYS_LOG(DEBUG, "ObMutatorReader::mutator_add(last_seq=%ld)=>%d", last_seq_, err);
          }
        }
        else
        {
          ...
          if (dec_size <= 0)
          {
            YYSYS_LOG(ERROR, "size inc after mutator merge: %ld", -dec_size);
    ...
    int ObMutatorReader::get_mutator(ObMutator*& mutator, uint64_t& seq, int64_t timeout)
    {
      ...
      else if (OB_SUCCESS != (err = get_mutator_(mutator, tmp_seq, timeout)))
      {
        if (OB_READ_NOTHING != err)
        {
          YYSYS_LOG(ERROR, "get_mutator_(last_seq=%ld)=>%d", last_seq_, err);
        }
      }
      else
      {
    ...
    int ObMutatorReader::get_mutator_(ObMutator*& mutator, uint64_t& seq, int64_t timeout)
    {
      ...
      if (OB_SUCCESS != (err = read_single_mutator_(tmp_mutator, seq, timeout)))
      {
        if(OB_READ_NOTHING != err)
        {
          YYSYS_LOG(ERROR, "ObMutatorReader.read()=>%d", err);
        }
        else
        {
          YYSYS_LOG(DEBUG, "ObMutatorReader.read()=>%d", err);
        }
      }
      ...
      { // after `ob_mutator_resolve_name()', `tmp_mutator.get_serialize_size()'
        ...
        YYSYS_LOG(ERROR, "ups_mutator_resolve_name()=>%d", err);
      }
      else if (OB_SUCCESS != (err = cur_mutator_.reset()))
      {
        YYSYS_LOG(ERROR, "cur_mutator_.reset()=>%d", err);
      }
    ...
    {
      int err = OB_SUCCESS;
      yyutil::Time now = yyutil::Time::now();
      ...
      while(!stop_)
      {
        ...
        if (OB_SUCCESS == err && OB_LOG_UPS_MUTATOR == cmd)
        {
          ...
          {
            YYSYS_LOG(ERROR, "ObUpsMutator.deserialize(buf=%p, len=%ld, pos=%ld)=>%d",
        ...
        {
          YYSYS_LOG(ERROR, "reader.read(seq=%ld)=>%d", seq, err);
          break; // error cause exit
        }
        
        if (OB_READ_NOTHING == err && (yyutil::Time::now() - now).toMilliSeconds() > timeout/1000)
        {
