...
int ValueRule::deserialize(const char* ori_buf, int64_t buf_size)
{
  ...
  if (NULL == token)
  {
    YYSYS_LOG(WARN, "no any token, rule_data=%s", buf + buf_pos);
    err = OB_ERROR;
  }
  else
  {
...
void ValueRule::get_value(uint64_t suffix, int64_t col_idx, int64_t& value)
{
  switch (rule_id_[col_idx])
  {
    ...
    case CYCLE_RULE:
      ...
      break;
    default:
      YYSYS_LOG(WARN, "invalid rule_id, rule_id=%ld", rule_id_[col_idx]);
      break;
  }
}
...
int ValueGenerator::generate_value_rule(uint64_t prefix, ValueRule& rule)
{
  ...
  switch (rand_val)
  {
    ...
    default:
      YYSYS_LOG(WARN, "not supported, rand_val=%d", rand_val);
      break;
  }
