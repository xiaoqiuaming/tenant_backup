...
int Array::init(int row_num, int column_num)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, row_num=%d column_num=%d", row_num, column_num);
...
int Array::set_value(int row_idx, int col_idx, const char* value, int type)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid row idx, row_idx=%d, row_num_=%d", row_idx, row_num_);
    err = OB_INVALID_ARGUMENT;
  }
  else if (col_idx < 0 || col_idx >= column_num_)
  {
    YYSYS_LOG(WARN, "invalid column idx, col_idx=%d, column_num_=%d", col_idx, column_num_);
    err = OB_INVALID_ARGUMENT;
  }
  ...
  {
    YYSYS_LOG(WARN, "the cell is set before, row_idx=%d, col_idx=%d", row_idx, col_idx);
    err = OB_ERROR;
  }
  else
  {
...
int Array::compare(Array& other) const
{
  ...
  {
    YYSYS_LOG(ERROR, "row num or column num is not equal, this[%d,%d], other[%d,%d]",
  ...
  else
  {
    for (int i = 0; i < row_num_ && 0 == cmp_val; ++i)
    {
      for (int j = 0; j < column_num_ && 0 == cmp_val; ++j)
      {
        ...
        {
          YYSYS_LOG(ERROR, "type is not equal, row_idx=%d, col_idx=%d, this_type=%d, other_type=%d", i, j, this_type, other_type);
        }
        else
        {
          switch (this_type)
          {
            ...
            case VARCHAR_TYPE:
              ...
              {
                YYSYS_LOG(ERROR, "value is not equal, row_idx=%d, col_idx=%d, this=%s, other=%s, len1=%d, len2=%d",
            ...
            case FLOAT_TYPE:
              {
                ...
                {
                  YYSYS_LOG(ERROR, "value is not equal, row_idx=%d, col_idx=%d, this=%s(%f), other=%s(%f), len1=%d, len2=%d",
                      i, j, this_val, this_double_val, other_val, other_double_val, (int) strlen(this_val), (int) strlen(other_val));
                  cmp_val = -1;
                }
                else
                {
                  YYSYS_LOG(INFO, "double value is equal, row_idx=%d, col_idx=%d, this=%s, other=%s, len1=%d, len2=%d",
                      i, j, this_val, other_val, (int) strlen(this_val), (int) strlen(other_val));
                }
              }
              break;
            case DATETIME_TYPE:
              YYSYS_LOG(ERROR, "datetime type is not supported");
              cmp_val = -1;
              break;
            case INVALID_TYPE:
              YYSYS_LOG(ERROR, "invalid type");
              cmp_val = -1;
              break;
            default:
              YYSYS_LOG(ERROR, "invalid type");
