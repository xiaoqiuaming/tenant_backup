...
int BigqueryTest::init()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(WARN, "failed to init client, server_addr=%p, addr_num=%ld, ret=%d",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(WARN, "failed to init prefix info, ret=%d", ret);
    }
  }
  ...
  while (NULL != token)
  {
    if (token[0] != '\0')
    {
      ...
      *tmp = '\0';
      ...
      YYSYS_LOG(INFO, "[SERVER_ADDR] host=%s, port=%d", addr[addr_num].host, addr[addr_num].port);
      ++addr_num;
    }
    token = strtok(NULL, " ");
  }
