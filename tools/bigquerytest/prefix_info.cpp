...
int PrefixInfo::set_read_write_status(uint64_t prefix, int status)
{
  ...
  if (status == 0)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to set status, prefix=%lu, err=%d", prefix, err);
    }
  }
  else
  {
    ...
    //YYSYS_LOG(INFO, "prefix=%lu, ori_status=%d", prefix, ori_status);

    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to get ori status, prefix=%lu, err=%d", prefix, err);
    }
    else
    {
      if (ori_status != 0)
      ...
      else
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "failed to set status, prefix=%lu, status=%d, err=%d", prefix, status, err);
        }
      }
    }
...
int PrefixInfo::set_status(uint64_t prefix, int status)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "failed to update ob, sql=%s, err=%d", ob_sql, err);
  }
...
int PrefixInfo::get_status(uint64_t prefix, int& status)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "failed to query ob, sql=%s, err=%d", ob_sql, err);
  }
  else
  {
...
int PrefixInfo::get_max_prefix(uint64_t client_id, uint64_t& prefix)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to get max prefix, key=%lu, err=%d",
        INVALID_PREFIX, err);
  }

  //YYSYS_LOG(INFO, "[get_max_prefix] prefix=%lu, err=%d", prefix, err);
  return err;
}
...
int PrefixInfo::set_max_prefix(uint64_t client_id, uint64_t prefix)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to set max prefix, key=%lu, err=%d",
        INVALID_PREFIX, err);
  }
  //YYSYS_LOG(INFO, "[set_max_prefix] prefix=%lu, err=%d", prefix, err);
  return err;
}
