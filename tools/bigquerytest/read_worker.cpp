...
int ReadWorker::init(KeyGenerator& key_gen, MysqlClient& ob_client, PrefixInfo& prefix_info)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to init bigquery reader, err=%d", err);
  }
  return err;
}
...
void ReadWorker::run(CThread* thread, void* arg)
{
  ...
  if (NULL != key_gen_)
  {
    YYSYS_LOG(INFO, "start read worker");
    while (!_stop && 0 == err)
    {
      ...
      else
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(ERROR, "failed to read data, prefix=%lu, err=%d", prefix, err);
        }
      }
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "key_gen_ is NULL");
  }
}
