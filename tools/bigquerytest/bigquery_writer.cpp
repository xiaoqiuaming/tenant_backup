...
int BigqueryWriter::write_data(uint64_t prefix)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set update flag to true, prefix=%lu, err=%d", prefix, err);
  }
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to generate value rule, prefix=%lu, err=%d",
          prefix, err);
    }
    else
    {
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to serialize rule, rule_data=%p, rule_len=%ld, err=%d",
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to set rule or row num, prefix=%lu, rule_data=%s, row_num=%ld, err=%d",
  ...
  if (0 == err)
  {
    ...
    for (int64_t batch = 0; batch < NUM_BATCH; ++batch)
    {
      ...
      //YYSYS_LOG(INFO, "batch=%ld, batch_row_num=%ld, row_num=%ld", batch, batch_row_num, row_num);
      char stmt_str[10240];
      char tmp_str[10240];
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to prepare, err=%d", err);
      }
      ...
      for (uint64_t i = batch * NUM_PER_MUTATION; i < (uint64_t) batch * NUM_PER_MUTATION + batch_row_num && 0 == err; ++i)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "failed to write data, prefix=%lu, suffix=%lu, err=%d",
              prefix, i, err);
        }
      }
      ...
      if (0 == err)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "failed to bind param, bind=%p, len=%ld", bind_ary, bind_ary_size);
        }
      }
      ...
      if (0 == err)
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "failed to commit, err=%d", err);
        }
      }
    }
  }

  YYSYS_LOG(INFO, "[write_data] rule=%s, err=%d", rule.to_string(), err);
    ...
    MYSQL_BIND* bind_ary, int64_t& bind_ary_size, int64_t* col_arr_store)
{
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to bind param, bind=%p, len=%ld", bind, arr_len + 2);
    }
  }
  */
...
int BigqueryWriter::commit()
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to execute, err=%d", err);
  }
  return err;
}
