...
int BigqueryTestParam::load_from_config()
{
  ...
  client_id_ = YYSYS_CONFIG.getInt(SQLTEST_SECTION, CLIENT_ID, 0);
  assert(client_id_ >= 0);

  /*
  if (0 == err)
  {
    ob_port_ = YYSYS_CONFIG.getInt(SQLTEST_SECTION, OB_PORT, 0);
    assert(ob_port_ > 0);
  ...
  if (0 == err)
  {
    ob_rs_port_ = YYSYS_CONFIG.getInt(SQLTEST_SECTION, OB_RS_PORT, 0);
  ...
  */
  ...
  if (0 == err)
  {
    read_thread_count_ = YYSYS_CONFIG.getInt(SQLTEST_SECTION, READ_THREAD_COUNT, 0);
    write_thread_count_ = YYSYS_CONFIG.getInt(SQLTEST_SECTION, WRITE_THREAD_COUNT, 0);
