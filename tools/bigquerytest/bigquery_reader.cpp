...
int BigqueryReader::read_data(uint64_t prefix)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set read flag, prefix=%lu, err=%d", prefix, err);
  }
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to get rule, prefix=%lu, err=%d", prefix, err);
    }
    else
    {
      uint64_t row_num = 0;
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to get row num, err=%d", err);
      }
      else
      {
        ...
        if (0 != err)
        {
          YYSYS_LOG(WARN, "failed to deserialize rule, err=%d", err);
        }
        else
        {
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to get big query, rule_data=%s, err=%d", rule_data, err);
    }
  }
  ...
  if (0 == err)
  {
    YYSYS_LOG(INFO, "row_num=%ld, rule_data=%s, bigquery sql=%s", rule.get_row_num(), rule_data, query.to_sql());
    err = ob_client_->exec_query(query.to_sql(), res);
    if (0 != err)
    {
      YYSYS_LOG(ERROR, "failed to exec query, sql=%s, err=%d", query.to_sql(), err);
    }
  }
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to reset flag, prefix=%lu, err=%d", prefix, err);
    }
  }
...
int BigqueryReader::get_bigquery(uint64_t prefix, ValueRule& rule, Bigquery& query, Array& result)
{
  ...
  switch (rule_num)
  {
    case 1:
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to get_bigquery_one_col, rule_num=%ld, err=%d", rule_num, err);
      }
      break;
    case 2:
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to get_bigquery_two_col, rule_num=%ld, err=%d", rule_num, err);
      }
      break;
    default:
      YYSYS_LOG(WARN, "invalid rule_num, rule_num=%ld", rule_num);
      break;
  }
...
int BigqueryReader::get_bigquery_one_col(uint64_t prefix, ValueRule& rule, Bigquery& query, Array& result)
{
  ...
  switch (rule_id)
  {
    ...
    default:
      YYSYS_LOG(WARN, "invalid rule_id, rule_id=%ld", rule_id);
      err = OB_ERROR;
      break;
  }
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to call get_bigquery_one_col, err=%d", err);
  }
  return err;
}
...
int BigqueryReader::get_bigquery_two_col(uint64_t prefix, ValueRule& rule, Bigquery& query, Array& result)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid rule_id, first_rule_id=%ld, second_rule_id=%ld",
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to call get bigquery_two_col, err=%d", err); 
  }
  return err;
}
...
int BigqueryReader::verify_result(Array& expected_result, Array& result)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to compare result, first=expected, second=real, err=%d", err);
  }
  return err;
}
