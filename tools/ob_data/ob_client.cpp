...
int ObClient::prepare()
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to get mergeserver, ret = %d", ret);
  }
  else if (OB_SUCCESS != (ret = rpc_->get_update_server(root_server_, timeout, update_server_)))
  {
    YYSYS_LOG(ERROR, "failed to get updateserver, ret = %d", ret);
  }
  return ret;
}
...
int ObClient::scan(const common::ObString & table_name, const common::ObRange & range, common::ObScanner & scanner)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      // do nothing
      //YYSYS_LOG(INFO, "scan finished");
    }
    else
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "scan failed");
    }
  }
  else
  {
    YYSYS_LOG(ERROR, "rpc scan table error, ret = %d",ret);
  }
  return ret;
}
...
int ObClient::del(const ObString & table_name, const ObRange & range, int64_t & row_count)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "set scan_param failed, ret = %d", ret);
  ...
  else
  {
    ...
    while (true)
    {
      ...
      for(; i < retry_times_ ; ++i)
      {
        ...
        {
          YYSYS_LOG(ERROR, "scan mergeserver[%s] failed, ret = %d", 
              ob_server_to_string(merge_server_[ms_idx]), ret);
        }
      }
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "scan mergeserver success");
        bool is_fullfilled = false;
        int64_t fullfilled_item_num = -1;
        scanner.get_is_req_fullfilled(is_fullfilled, fullfilled_item_num);
        if ( is_fullfilled == true && fullfilled_item_num == 0)
        {
          YYSYS_LOG(INFO, "mergeserver already no data, is_fullfilled = %d, fullfilled_item_num = %ld", is_fullfilled, fullfilled_item_num);
          break;
        }
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "del row from updateserver[%s] failed, ret = [%d]", ob_server_to_string(update_server_), ret);
          break;
        }
        else
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS != ret)
            {
              YYSYS_LOG(ERROR, "write last row key to string buf failed, ret = %d", ret);
              break;
            }
            else
            {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "check scan table error, ret = %d",ret);
        }
      }
    }
  }
