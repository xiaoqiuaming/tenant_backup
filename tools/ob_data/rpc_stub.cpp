    ...
    ObScanner & result)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "send scan data failed, server = %s, ret[%d]", server.to_cstring(),ret);
    }
    else
    {
...
int RpcStub::get_merge_server(const common::ObServer & server, const int64_t timeout,common::ObServer list[], const int64_t size, int32_t & count)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send request to root server for find merge server failed:ret[%d]", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize result_code failed:pos[%ld], ret[%d]", data_buff.get_position(), ret);
    }
    else
    {
      ret = result_code.result_code_;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      if (count == 0)
      {
        YYSYS_LOG(ERROR, "get mergeserver count = %d", count);
        ret = OB_ERROR;
      }
    }
  ...
  if (OB_SUCCESS == ret)
  {
    for (int32_t i = 0; i < count && OB_SUCCESS == ret; i++)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "Get %dth Mergeserver %s", i, ob_server_to_string(list[i]));
        int64_t reserve;
...
int RpcStub::del(const common::ObServer & server, const int64_t timeout, const common::ObMutator & mutator)
{
  ...
  {
    YYSYS_LOG(ERROR, "serilaize ob mutator failed, ret = %d", ret);
  }
  else if (OB_SUCCESS != (ret = frame_->send_request(server, OB_WRITE, DEFAULT_VERSION, timeout, data_buff )))
  {
    YYSYS_LOG(ERROR , "failed to send request to updateserver[%s], ret = %d", ob_server_to_string(server), ret);

  }
  else if(OB_SUCCESS != (ret = result_code.deserialize(data_buff.get_data(), data_buff.get_position(), pos )))
  {
    YYSYS_LOG(ERROR, "deserialize result code failed, ret = %d", ret);
  }
  else
  {
    ret = result_code.result_code_;
...
int RpcStub::del(const common::ObServer & server, const int64_t timeout, common::ObScanner & scanner)
{
      ...
      (OB_SUCCESS == (ret = scanner.next_cell())))
  {
	...
	if (OB_SUCCESS != ret)
	{
	  YYSYS_LOG(WARN, "add del row operation into ob_mutator failed, ret = %d",ret);
	}
      }
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "delete mutator from updateserver: %s, failed, ret = %d", ob_server_to_string(server), ret);
  }
  return ret;
