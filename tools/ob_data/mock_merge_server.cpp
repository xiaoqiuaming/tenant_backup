...
int MockMergeServer::initialize()
{
  int ret = OB_SUCCESS;
  /*
  if(YYSYS_CONFIG.load("./mock_merge_server.conf"))
  {
    fprintf(stderr, "load config file mock_merge_server.conf error");
    return EXIT_FAILURE;
  }
  ret = ms_params_.load_from_config(YYSYS_CONFIG);
  if (OB_SUCCESS == ret)
  {
...
int MockMergeServer::do_request(ObPacket * base_packet)
{
  ...
  yynet::Connection *connection = packet->get_connection();
  ObDataBuffer *in_buff = packet->get_buffer();
  ThreadSpecificBuffer::Buffer *thread_buff = response_packet_buffer_.get_buffer();
  //貌似刨去了网络时间？
  int64_t timeout_us = packet->get_source_timeout() - (yysys::CTimeUtil::getTime() - packet->get_receive_ts());
  if (NULL != thread_buff)
  {
    thread_buff->reset();
...
int MockMergeServer::ms_scan(const int64_t start_time,const int32_t version,const int32_t channel_id,yynet::Connection* connection,common::ObDataBuffer& in_buffer,common::ObDataBuffer& out_buffer,const int64_t timeout_us)
{
  ObResultCode rc;
