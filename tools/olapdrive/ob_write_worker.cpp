...
{ 
  namespace olapdrive 
  {
    ...
    int ObWriteWorker::batch_apply()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to insert row into ups");
        ...
        ObObj& obj_val)
    {
      ...
      default:
        YYSYS_LOG(WARN, "more than 24 cells to store for set lz data, column_idx=%ld", 
                  column_idx);
        ret = OB_ERROR;
        break;        
      }
    ...
    int ObWriteWorker::do_campaign_change(bool update_prev)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get campaign, ret=%d", ret);
            display_internal_status();
          }
        }
      }
    ...
    int ObWriteWorker::do_adgroup_change(bool update_prev)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to get adgroup, ret=%d", ret);
            display_internal_status();
          }
        }
      }
    ...
    int ObWriteWorker::do_write_work()
    {
      ...
      while (true)
      {
        ...
        if (OB_ITER_END == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "not write expected row count, wrote_count=%ld, "
        ...
        {
          YYSYS_LOG(WARN, "failed to write row, wrote_cnt=%ld, ret=%d", 
                    wrote_cnt, ret);
          break;
        }
      }
    ...
    void ObWriteWorker::run(CThread* thread, void* arg)
    {
      ...
      if (OB_SUCCESS == init())
      {
        while (!_stop)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to write row, err=%d", err);
            break;
          }
        }
      }
