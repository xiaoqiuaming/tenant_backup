...
{ 
  namespace olapdrive 
  {
    ...
    int64_t ObLzQuery::get_random_adgroup_id()
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to get current scan campaign, err=%d", err);
      }
      ...
      int64_t* array, const int64_t item_cnt, const int64_t value)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, array=%p, item_cnt=%ld",
                  array, item_cnt);
      }
      else
      {
      ...
      int64_t& adgroup_cnt)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, adgroup_id_array=%p, adgroup_cnt=%ld",
    ...
    int ObLzQuery::reset_extra_info()
    {
      ...
      if (NULL == extra_info)
      {
        YYSYS_LOG(WARN, "failed to thread local ObScanExtraInfo");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObLzQuery::add_scan_range(ObScanParam& scan_param)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread rowkey buffer");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObLzQuery::add_basic_scan_column(ObScanParam& scan_param)
    {
      ...
      {
        YYSYS_LOG(WARN, "lz table schema is invalid, lz_schema=%p", 
                  lz_schema);
        ret = OB_ERROR;
      }
      ...
      const ObCellInfo& cell_info, const int index, const int64_t sum_val)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid cell type, must be ObIntType, type=%d", 
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        case ALIPAY_INDIRECT_AMT:
          if (intv != sum_val)
          {
            YYSYS_LOG(WARN, "wrong result, column_index=%d, column_name=%.*s, "
        ...
        default:
          YYSYS_LOG(WARN, "unknown effect column type, type=%d", index);
          ret = OB_ERROR;
          break;
        }
      }
      ...
      const int64_t exp_val)
    {
      ...
      else 
      {
        ...
        {
          YYSYS_LOG(WARN, "wrong result, column_index=%ld, column_name=%.*s, "
                          "ret_val=%ld, exp_val=%ld", 
                    column_index, cell_info.column_name_.length(), 
    ...
    int ObLzQuery::get_cur_scan_campaign(ObCampaign& campaign)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread start rowkey");
        ret = OB_ERROR;
      }
      else
      {
    ...
    int ObLzQuery::get_scan_info(ObScanInfo& scan_info)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to get thread rowkey");
        ret = OB_ERROR;
      }
      else
      {
      ...
      const int64_t exp_cell_num)
    {
      ...
      {
        YYSYS_LOG(WARN, "returns wrong row count or column count, row_count=%ld, "
    ...
    int ObLzQuery::add_special_column(ObScanParam& scan_param)
    {
      ...
      UNUSED(scan_param);
      YYSYS_LOG(WARN, "add_special_column check not implementation");
    ...
    int ObLzQuery::scan(ObScanParam& scan_param, ObScanner& scanner)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid rowkey meta:");
    ...
    int ObLzQuery::check(const ObScanner& scanner, const int64_t time)
    {
      ...
      UNUSED(scanner);
      UNUSED(time);
      YYSYS_LOG(WARN, "check not implementation");
