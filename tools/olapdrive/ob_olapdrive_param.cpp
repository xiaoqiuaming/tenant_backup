...
{ 
  namespace olapdrive 
  {
    ...
    int ObOlapdriveParam::parse_merge_server(char* str)
    {
      ...
      for (begin_ptr = str; begin_ptr != end_ptr && OB_SUCCESS == ret;) 
      {
        ...
        if (NULL != merge_server_ && cur_index < merge_server_count_)
        {
          ...
          if (!res)
          {
            YYSYS_LOG(WARN, "invalid merge server index=%ld, ip=%s, port=%d.", 
        ...
        {
          YYSYS_LOG(WARN, "no space to store merge server, merge_server_=%p, "
    ...
    int ObOlapdriveParam::load_merge_server()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        merge_server_count_ = YYSYS_CONFIG.getInt(OBMS_SECTION, 
                                                  OBMS_MERGE_SERVER_COUNT, 0);
        if (merge_server_count_ <= 0)
        {
          YYSYS_LOG(WARN, "olapdrive merge server count=%ld can't <= 0." ,
              merge_server_count_);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == merge_server_)
        {
          YYSYS_LOG(ERROR, "failed allocate for merge servers array.");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObOlapdriveParam::load_from_config()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "olapdrive write thread count=%ld can't <= 0 or > 1." ,
              write_thread_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "olapdrive read thread count=%ld can't <= 0." ,
              read_thread_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "olapdrive network timeout=%ld can't <= 0." ,
                    network_time_out_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "olapdrive stat_dump_interval_=%ld can't < 0." ,
              stat_dump_interval_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        write_day_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                               OBSC_WRITE_DAY_COUNT, 
                                               DEFAULT_WRITE_DAY_COUNT);
        if (write_day_count_ < 0)
        {
          YYSYS_LOG(WARN, "olapdrive write_day_count_=%ld can't < 0." ,
              write_day_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        daily_unit_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                OBSC_DAILY_UNIT_COUNT, 
                                                DEFAULT_DAILY_UNIT_COUNT);
        if (daily_unit_count_ < 0)
        {
          YYSYS_LOG(WARN, "olapdrive daily_unit_count_=%ld can't < 0." ,
              daily_unit_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        read_day_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                              OBSC_READ_DAY_COUNT, 
                                              DEFAULT_READ_DAY_COUNT);
        if (read_day_count_ < 0)
        {
          YYSYS_LOG(WARN, "olapdrive read_day_count_=%ld can't < 0." ,
              read_day_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        log_level_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                         OBSC_LOG_LEVEL, 
                                         DEFAULT_LOG_LEVEL);
        if (log_level_ < 0)
        {
          YYSYS_LOG(WARN, "olapdrive log_level_=%ld can't < 0." ,
              log_level_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
