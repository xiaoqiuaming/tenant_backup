 ...
 *
 *  Version: $Id: cs_admin.cpp 2010��12��03�� 13ʱ48��14�� qushan Exp $
 *
 *  Authors:
...
int GFactory::initialize(const ObServer& remote_server, const int64_t timeout)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "initialize client object failed, ret:%d", ret);
    return ret;
  }
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "initialize rpc stub failed, ret:%d", ret);
  }
  timeout_ = timeout;
  return ret;
}
...
//  bool avail[common::OB_MAX_ROW_KEY_LENGTH];
  bool avail[common::OB_MAX_DISK_NUMBER];

  int32_t disk_num = 0;
  int64_t avail_size[common::OB_MAX_DISK_NUMBER];

  ret = GFactory::get_instance().get_rpc_stub().cs_show_disk(disk_no, avail, disk_num, avail_size);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      fprintf(stderr, "disk:%02d status:%s availsize:%ldB\n", disk_no[i], avail[i] ? "OK" : "error", avail_size[i]);
    }
  }
  else
  {
    ...
    ObRowkey& end_row_key, ObMemBufAllocatorWrapper& end_row_key_allocator)
{
  ...
  for (iter = scanner.begin(); iter != scanner.end(); iter++)
  {
    ...
    if (NULL == cell_info)
    {
      YYSYS_LOG(ERROR, "get null cell_info");
      ret = OB_ERROR;
      break;
    }
    ...
    YYSYS_LOG(DEBUG, "---------------------%d----------------------\n", row_count);
    hex_dump(cell_info->row_key_.ptr(), cell_info->row_key_.length());
    YYSYS_LOG(DEBUG,"table_id:%lu, table_name:%.*s, column_id:%ld, column_name:%.*s, is_row_changed:%d\n",
  ...
  if (OB_SUCCESS == ret && last_row_key.ptr())
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to copy last rowkey, ret=%d", ret);
    }
...
int change_log_level(const vector<string> &param)
{
  ...
  if (param[0].compare("ERROR") == 0)
    log_level = YYSYS_LOG_LEVEL_ERROR;
  else if (param[0].compare("WARN") == 0)
    log_level = YYSYS_LOG_LEVEL_WARN;
  else if (param[0].compare("INFO") == 0)
    log_level = YYSYS_LOG_LEVEL_INFO;
  else if (param[0].compare("DEBUG") == 0)
    log_level = YYSYS_LOG_LEVEL_DEBUG;
  else {
    fprintf(stderr, "change_log_level: param error!\n");
    fprintf(stderr, "usage: \n\tchange_log_level ERROR|WARN|INFO|DEBUG\n");
    return OB_ERROR;
    ...
    common::ObRowkey& cur_row_key)
{
  ...
  if (row_count >= 1)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "failed to copy range, ret=%d", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "insert range: %s", to_cstring(range));
    }
...
int SstableDistDumper::load_root_table(common::ObString& table_name, const uint64_t table_id)
{
  ...
  {
    YYSYS_LOG(INFO, "skip internal table %lu: %.*s",
  ...
  else
  {
    while (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "scan root server error, ret = %d", ret);
        break;
      }
          ...
          it != scanner.end() && OB_SUCCESS == ret;  ++it)
      {
        ...
        if (NULL == cell_info)
        {
          YYSYS_LOG(ERROR, "get null cell info");
          ret = OB_ERROR;
          break;
        }
        ...
        if (is_row_changed)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to insert range table_id %ld, row count %d,"
        ...
        if(column_index == 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to copy rowkey, ret=%d", ret);
          }
          else if (OB_SUCCESS !=
        ...
        {
          YYSYS_LOG(ERROR, "column value type %d expected, not %d", ObIntType, cell_info->value_.get_type());
        }
        ...
        if (column_index == 0)
        {
          ...
          {
            YYSYS_LOG(ERROR, "column_index=0, column_name should not be %s", column_name.c_str());
            ret = OB_INVALID_DATA;
            break;
          }
        }
        else if (column_index == 1)
        {
          ...
          {
            YYSYS_LOG(ERROR, "column_index=1, column_name should not be %s", column_name.c_str());
            ret = OB_INVALID_DATA;
            break;
          }
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "column_str_expect_buf not enough");
          ...
          else if (cs_info_id == 2)
          {
            ...
            if (cs_list_.find(server) == cs_list_.end())
            {
              ...
              YYSYS_LOG(INFO, "add server %s to server list, ip %ld, port %ld", to_cstring(server), ip, port);
            }
          }
          else if (cs_info_id == 3)
          {
            ...
            else if (table_version_ != version)
            {
              ...
              {
                YYSYS_LOG(WARN, "a tablet version of %.*s:%lu in root table is %ld, but current table_version_ is %ld",
                    table_name.length(), table_name.ptr(), table_id, version, table_version_);
              }
              else if (table_version_ < version)
              {
                YYSYS_LOG(INFO, "a tablet version of %.*s:%lu in root table is %ld, larger than current table_version_ %ld"
      ...
      if (cur_row_key.is_max_row())
      {
        ...
        column_index = 0;
        ++row_count;
        YYSYS_LOG(INFO, "dump root table finish, table id=%lu, table name=%.*s\n",
            table_id, table_name.length(), table_name.ptr());
        break;
      }
      else
...
int SstableDistDumper::check_sstable_dist()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "failed to check sstable info of range: %s", to_cstring(prev_range));
  }
  ...
  for(; OB_SUCCESS == ret && it != sstable_dist_.end(); ++it)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "failed to check sstable info of range: %s", to_cstring(range));
      break;
    }
    ...
    if (range.table_id_ == prev_range.table_id_)
    {
      ...
      {
        YYSYS_LOG(ERROR, "sstable dist is not continuous, range: %s, prev range: %s",
  ...
  {
    YYSYS_LOG(ERROR, "sstable dist is not continuous, last range: %s", to_cstring(range));
    ret = OB_ERROR;
  }
...
int SstableDistDumper::write_sstable_dist()
{
  ...
  if (NULL == fp_)
  {
    YYSYS_LOG(ERROR, "fp_ should not be NULL");
  ...
  {
    ret = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "failed to generate file_name_format, errmsg %s",
        strerror(errno));
  }
      ...
      OB_SUCCESS == ret && it != sstable_dist_.end(); ++it)
  {
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to append sstable info to %s, errmsg %s",
    ...
    {
      ret = OB_ERR_UNEXPECTED;
      YYSYS_LOG(ERROR, "failed to translate ip[%s] to string", to_cstring(info.server_));
      break;
    }
    ...
    if (write_range_info_)
    {
      ...
      {
        ret = OB_IO_ERROR;
        YYSYS_LOG(ERROR, "failed to append table id: %s", strerror(errno));
      }
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to append \\n to %s, errmsg %s",
          filename_, strerror(errno));
      break;
    }
  }
...
int SstableDistDumper::update_sstable_dist(const ObServer& server, std::list<pair<ObNewRange, ObString> >& sstable_list)
{
      ...
      OB_SUCCESS == ret && sstable_it != sstable_list.end(); ++sstable_it)
  {
    ...
    {
      YYSYS_LOG(WARN, "can't find range %s in sstable dist", to_cstring(range));
      continue;
    }
...
int SstableDistDumper::fetch_schema(common::ObSchemaManagerV2& schema)
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to init rpc stub, ret=%d", ret);
  }
  else if (OB_SUCCESS != (ret =
        (rpc_stub.fetch_schema(timeout, rs_server_, version, only_core_table, schema))))
  {
    YYSYS_LOG(ERROR, "failed to fetch schema, rs_server %s ret=%d", to_cstring(rs_server_), ret);
  }
  return ret;
}
...
int SstableDistDumper::dump()
{
  ...
  if (NULL == filename_)
  {
    YYSYS_LOG(ERROR, "the file path should not be NULL");
    ret = OB_INVALID_ARGUMENT;
  }
  else if (common::FileDirectoryUtils::exists(filename_))
  {
    YYSYS_LOG(ERROR, "the file %s has been existed", filename_);
    ret = OB_IO_ERROR;
  }
  else
  {
    ...
    {
      ret = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "failed to open %s, err msg %s", filename_, strerror(errno));
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to fetch schema, ret=%d", ret);
    }
    else
    {
          ...
          OB_SUCCESS == ret && table != schema.table_end(); ++table)
      {
        ...
        if (NULL == buf)
        {
          YYSYS_LOG(ERROR, "Failed to alloc mem, len %ld", len);
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "failed to load root table %.*s, table id %ld",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for(std::set<ObServer>::iterator cs_it = cs_list_.begin(); cs_it != cs_list_.end(); ++cs_it)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to load sstable list of cs %s, ret=%d",
            to_cstring(*cs_it), err);
      }
      else if (OB_SUCCESS != (err = update_sstable_dist(*cs_it, sstable_list)))
      {
        YYSYS_LOG(WARN, "failed to update sstable dist for cs %s, ret=%d",
            to_cstring(*cs_it), err);
      }
    }
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "sstable dist is not correct, ret=%d", ret);
    }
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to dump sstable dist, ret=%d", ret);
      }
    }
  }
  ...
  if (NULL != fp_)
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to close file %s, errmsg %s", filename_, strerror(errno));
      if (OB_SUCCESS == ret)
      {
        ret = tmp_ret;
      }
...
int fetch_bypass_table_id(const vector<string> & param)
{
  ...
  else
  {
    ...
    if (NULL == input)
    {
      YYSYS_LOG(ERROR, "failed to dup input param %s", param[0].c_str());
      ret = OB_ALLOCATE_MEMORY_FAILED;
    }
    ...
    while (OB_SUCCESS == ret && (token = strsep(&input, sep)) != NULL)
    {
      ...
      YYSYS_LOG(INFO, "add table name %.*s", table_name.length(),table_name.ptr());
      ret = info.push_back(pair<ObString, uint64_t>(table_name, OB_INVALID_ID));
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "failed to add table %s to bypass task info", token);
        break;
      }
    }
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "failed to fetch bypass table id, ret=%d", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (count <= 0)
      {
        YYSYS_LOG(ERROR, "failed to fetch bypass table id, count is %ld", count);
      }
      else
      {
...
//add for [optimize rs_cs_ups_admin_error_prompt]-b
void build_cmd_with_suboption_map(std::map<std::string, std::string> &cmd_map)
{
  cmd_map.insert(std::map<std::string, std::string>::value_type("install_disk", "install_disk mount_path"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("uninstall_disk", "uninstall_disk disk_no"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_tablet_image", "dump_tablet_image disk_no_1[, disk_no_2, ...] or dump_tablet_image disk_no_min~disk_no_max"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_app_stats", "dump_app_stats <server_type=rs/cs/ms/ups/dm> [interval=1] [count = 0] [show_header_count = 50] [table = 0/1001~1005/1001,1003] [show_index = 0/0~11/0,2,5]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_server_stats","dump_server_stats <server_type=rs/cs/ms/ups/dm> [interval=1] [count = 0] [show_header_count = 50] [table = 0/1001~1005/1001,1003] [show_index = 0/0~11/0,2,5]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_cluster_stats", "dump_cluster_stats <server_type=rs/cs/ms/ups/dm> [interval=1] [count = 0] [show_header_count = 50] [table = 0/1001~1005/1001,1003] [show_index = 0/0~11/0,2,5]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("manual_merge", "manual_merge memtable_frozen_version [init_flag=0]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("manual_drop_tablet", "manual_drop_tablet memtable_frozen_version"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("manual_gc", "manual_gc [copy_count]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("get_tablet_info", "get_tablet_info table_id table_name range_str"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("scan_root_table", "scan_root_table table_name or scan_root_table table_name table_id rowkey_split app_name [output_dir = .] [key_delim_assic = 1] [column_delim_assic = ' '] rowkey_split: type1-length1,type2-length2..."));
  cmd_map.insert(std::map<std::string, std::string>::value_type("migrate_tablet", "migrate_tablet table range range_format src_server keep_src data_source_type tablet_version sstable_version uri"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("create_tablet", "create_tablet table range range_format last_frozen_version"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("delete_tablet", "delete_tablet table range range_format tablet_version is_force, range_format: 0 : plain string, 1 : hex format string 'FACB012D', 2 : int64_t number '1232'"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("check_merge_process", "check_merge_process disk_no_1[, disk_no_2, ...] or check_merge_process disk_no_min~disk_no_max"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("execute_sql", "execute_sql sql_cmd"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("delete_table", "delete_table table_id"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("load_sstables", "load_sstables load_version [is_response_rootserver=0/1] [table = 0/1001~1005/1001,1003]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("dump_sstable_dist", "dump_sstable_dist sstable_dist_file [write_range]"));
  cmd_map.insert(std::map<std::string, std::string>::value_type("fetch_bypass_table_id", "fetch_bypass_table_id table_name1,table_name2..."));
  cmd_map.insert(std::map<std::string, std::string>::value_type("change_log_level", "change_log_level ERROR|WARN|INFO|DEBUG"));
}

void print_error_prompt(const char *cmd, const char *host, const int port)
{
  std::map<int64_t, std::string> distance_map;
  int64_t distance = 0;
  const std::map<std::string, int> cmd_map = GFactory::get_instance().get_cmd_map();
  const int CMD_MAX_LEN = 1024;
  char key_buf[CMD_MAX_LEN];
  snprintf(key_buf, CMD_MAX_LEN, "%s", cmd);
  char *key = key_buf;
  char *token = NULL;
  ...
  *token = '\0';
  ...
  for (std::map<std::string, int>::const_iterator it = cmd_map.begin();
       it != cmd_map.end(); it++)
  {
    distance = get_edit_distance(key, (it->first).c_str());
    distance_map.insert(std::map<int64_t, std::string>::value_type(distance, it->first));
  }
  std::map<std::string, std::string> cmd_with_suboption_map;
  build_cmd_with_suboption_map(cmd_with_suboption_map);

  int count = 0;
  printf("unknown command = %s\n", cmd);
  printf("\nthe most probably commands you may want to input are:\n");
  for (std::map<int64_t, std::string>::iterator it = distance_map.begin();
       it != distance_map.end() && count < 3; it++)
  {
    std::map<std::string, std::string>::iterator sub_it = cmd_with_suboption_map.find(it->second);
    if (sub_it != cmd_with_suboption_map.end())
    {
      printf("bin/cs_admin -s %s -p %d -i \"%s\"\n", host, port, (sub_it->second).c_str());
    }
    else
    {
      printf("bin/cs_admin -s %s -p %d -i \"%s\"\n", host, port, (it->second).c_str());
    }
    count++;
  }
  printf("\n");
...
{
  YYSYS_LOGGER.setFileName("cs_admin.log", true);
...
int main(const int argc, char *argv[])
{
  ...
  if (silent)
  {
    YYSYS_LOGGER.setLogLevel("ERROR");
  }
  else
  {
    YYSYS_LOGGER.setLogLevel("DEBUG");
  }
  ...
  {
    //usage();
    print_error_prompt(cmdstring, addr, port);
    return ret;
  }
