...
int parse_cmd_line(int argc, char** argv, CmdLineParam& clp)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "cmd_type is not set");
      usage();
      ret = OB_ERROR;
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    if (0 == strcmp("dump_sstable", clp.cmd_type_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "-d must be appear when the cmd_type is"
            "dump_sstable");
        usage();
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(ERROR, "-f must be appear when the cmd_type is:"
            "dump_sstable");
        usage();
        ret = OB_ERROR;
      }
      ...
      else if (0 == strcmp("dump_block", clp.dump_content_))
      {
        if (0 == clp.table_id_)
        {
          YYSYS_LOG(ERROR, "-t must be appear when thc cmd_tye is"
...
int DumpSSTableV2::open(const int64_t sstable_file_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "sstable_file_id <=0:sstable_file_id=%ld",
        sstable_file_id);
    ret = OB_ERROR;
  }
  ...
  {
    YYSYS_LOG(ERROR, "get sstable path error:ret=%d,sstable_file_id=%ld",
        ret, sstable_file_id);
  }
  else if (OB_SUCCESS != (ret = reader_.init(sstable_file_id)))
  {
    YYSYS_LOG(ERROR, "open sstable error:ret=%d,sstable_file_id=%ld",
        ret, sstable_file_id);
  }
  else if (util_.open(filename, O_RDONLY|O_EXCL) <= 0)
  {
    YYSYS_LOG(ERROR, "util open filename error:ret = %d,filename=%s",
        ret, filename);
  }
  else if (OB_SUCCESS != (ret = load_block_index()))
  {
    YYSYS_LOG(ERROR, "load block index error:ret=%d", ret);
  }
  else
  {
    YYSYS_LOG(DEBUG, "dumpsstablev2 open success");
  }
...
//int64_t record_endkey_offset = 0;
  ...
  {
    YYSYS_LOG(ERROR, "failed to malloc memory for block index:size=%ld",
        table_cnt * sizeof(ObSSTableBlockIndexMgr));
    ret = OB_ERROR;
  }
...
//      + sizeof(ObRecordHeaderV2);
      ...
      {
        YYSYS_LOG(ERROR, "failed to alloc memory for record index:"
        ...
        ret = OB_ERROR;
              ...
              record_endkey_size)))
      {
        YYSYS_LOG(ERROR, "failed to alloc memory for record endkey:"
        ...
        ret = OB_ERROR;
              ...
              + record_endkey_size)))
      {
        YYSYS_LOG(ERROR, "failed to alloc memory for total:size=%ld",
            sizeof(ObSSTableBlockIndexMgr) + record_index_size
            + record_endkey_size);
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(ERROR, "util read error:index_size=%ld,read_size=%ld",
      ...
      {
        YYSYS_LOG(ERROR, "util read error:record_endkey_size=%ld,"
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "new block index mgr error");
          ret = OB_ERROR;
        }
      }
    }//end for
...
int DumpSSTableV2::load_block_index()
{
  ...
  {
    YYSYS_LOG(ERROR, "failed to malloc memory for block endkey:size=%ld",
        table_cnt * sizeof(char*));
    ret = OB_ERROR;
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (int64_t i = 0; i < table_cnt; i ++)
    {
      ...
      if (OB_SUCCESS == ret) 
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to malloc memory for block index:"
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == (block_endkey_array_[i] = (char*)malloc(
                endkey_size)))
        {
          YYSYS_LOG(WARN, "failed to malloc memory for block endkey:"
        ...
        {
          YYSYS_LOG(ERROR, "util read error:endkey_size=%ld, read_size=%ld",
    ...
    const ObSSTableBlockIndexMgr& index, const int64_t block_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "block id error:block_id=%ld", block_id);
    ret = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to malloc memory:size=%ld", size);
      ret = OB_ERROR;
    }
    ...
    {
      YYSYS_LOG(ERROR, "check record header error:ret=%d", ret);
    }
    else if (NULL == (uncomp_buf = (char*)malloc(
            record_header.data_length_)))
    {
      YYSYS_LOG(ERROR, "failed to alloc memeory for uncomp_buf:"
    ...
    else
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "decompressor error:ret=%d", ret);
        }
      }
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != (ret = block_reader.init(block_data,
              row_store_type)))
      {
        YYSYS_LOG(ERROR, "block reader init error:ret=%d", ret);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for (int i = 0; tmp_index  < end_index; tmp_index ++, i ++)
        {
          ...
          if (OB_SUCCESS != (ret = block_reader.get_row(tmp_index,
                  row)))
          {
            YYSYS_LOG(ERROR, "block reader get row error:ret=%d", ret);
            break;
          }
          else
          {
            ...
            while (true)
            {
              ...
              {
                YYSYS_LOG(ERROR, "next cell error:ret=%d", ret);
                break;
              }
              else if (OB_SUCCESS != (ret = row.get_cell(obj,
                      &is_row_finished)))
              {
                YYSYS_LOG(ERROR, "row get cell error:ret=%d", ret);
                break;
              }
              else
              {
                ...
                else
                {
                  if (DENSE_DENSE == row_store_type)
                  {
                    if (OB_SUCCESS != (ret = row.get_cell(obj,
                            &is_row_finished)))
                    {
                      YYSYS_LOG(ERROR, "get cell error:ret=%d", ret);
                      break;
                    }
                    else
                    {
  ...
  const int64_t end_block_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "table is not exist:table_id=%lu", table_id);
    ret = OB_ERROR;
  }
  else if (start_block_id > end_block_id)
  {
    YYSYS_LOG(ERROR, "start_block_id > end_block_id:start_block_id=%ld,"
        "end_block_id=%ld", start_block_id, end_block_id);
    ret = OB_ERROR;
  }
  else if (end_block_id >= block_index_mgr_ptr->get_block_count())
  {
    YYSYS_LOG(ERROR, "end_block_id is overflow:end_block_id=%ld",
        end_block_id);
    ret = OB_ERROR;
  }
  else
  ...
  if (OB_SUCCESS == ret)
  {
    for (int64_t i = real_start_id; i <= real_end_id; i ++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "load block error:ret=%d,i=%ld", ret, i);
        break;
      }
    }
  }
...
int DumpSSTableV2::display_sstable_schema()
{
  ...
  for (int64_t i = 0; i < column_cnt; i ++)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get column def error:def==NULL");
      ret = OB_ERROR;
      break;
    }
  }
...
int main(const int argc, char** argv)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "parse cmd line error:argc=%d", argc);
    }
    else if (clp.quiet_)
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "ob_init_memory_pool error:ret=%d", ret);
    }
  }
  ...
  if (0 == strcmp("dump_sstable", clp.cmd_type_))
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to open sstable file:clp.file_id=%ld",
          clp.file_id_);
    }
    else if (0 == strcmp("dump_sstable_size", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump sstable size error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_sstable_trailer", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump trailer error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_sstable_schema", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump sstable schema error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_table_index", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump table index error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_table_bloomfilter", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump table bloomfilter error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_table_range", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump table range error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_block_index", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump block index error:ret=%d", ret);
      }
    }
    else if (0 == strcmp("dump_block", clp.dump_content_))
    {
      ...
      {
        YYSYS_LOG(ERROR, "dump load display blocks error:ret=%d", ret);
      }
    }
  }
