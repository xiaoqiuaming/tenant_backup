 ...
 *
 *  Version: $Id: test.cpp 2010��11��17�� 16ʱ12��46�� qushan Exp $
 *
 *  Authors:
...
int64_t random_number(int64_t min, int64_t max)
{
  ...
  /* ��Ϊ�Ǵ�/dev/random�ж�ȡ��read���ܻᱻ������һ�ζ�ȡ����ֻ�ܵõ�һ���ֽڣ�
   * ѭ����Ϊ�������Ƕ�ȡ�㹻���ֽ��������random_value.
   */
  do
  {
    int bytes_read;
...
int parse_object(const char* object_str, ObObj& obj)
{
  ...
  if (size == 1)
  {
    ...
    else if (strcasecmp(dst[0], "varchar") == 0) {
      ObString varchar = ObString::make_string('\0');
      obj.set_varchar(varchar);
    }
    else if (strcasecmp(dst[0], "null") == 0) {
      obj.set_null();
    }
    else
    {
    ...
    if (found)
    {
      ...
      switch(type)
      {
        ...
        case ObDecimalType:
        {
          ObDecimal od;
          od.from(dst[1]);
          uint32_t len = od.get_words()->table[1] != 0 ? 2 :1;
          uint64_t *tmp = (uint64_t*)ob_malloc(len * sizeof(uint64_t), ObModIds::TEST);
          memcpy(tmp, od.get_words()->ToUInt_v2(), len * sizeof(uint64_t));
          obj.set_decimal(tmp, od.get_precision(), od.get_scale(), od.get_vscale(), len);
        }
          break;
        default:
          break;
...
int parse_range_str(const char* range_str, int hex_format, oceanbase::common::ObNewRange &range)
{
  ...
  int ret = OB_SUCCESS;
  int64_t len = 0;
  const char *range_str_dst = range_str;
  ...
  else if (hex_format == 1)
  {
    int i = 0;
    char value = 0;
    char *range_str_buf = (char *)ob_malloc(strlen(range_str) / 2 + 1, ObModIds::TEST);
    for (i = 0; i < static_cast<int>(strlen(range_str)); i = i + 2)
    {
      const char &c1 = range_str[i];
      const char &c2 = range_str[i + 1];
    ...
    if (OB_SUCCESS == ret)
    {
      range_str_dst = range_str_buf;
      fprintf(stderr, "unhex range str=%s\n", range_str_dst);
    }
  }
  if(OB_SUCCESS == ret)
  {
    len = static_cast<int>(strlen(range_str_dst));
    if (len < 5)
      ret = OB_ERROR;
  }
  ...
  if(OB_SUCCESS == ret)
  {
    const char start_border = range_str_dst[0];
    if (start_border == '[')
  ...
  if(OB_SUCCESS == ret)
  {
    const char end_border = range_str_dst[len - 1];
    if (end_border == ']')
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    memcpy(rowkey_str, range_str_dst + 1, strlen(range_str_dst) - 2 );
...
int dump_tablet_info(ObScanner &scanner)
{
  ...
  for (; OB_SUCCESS == ret && iter != scanner.end() ; ++iter)
  {
    ...
    if (row_change && OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "failed to copy end key, ret=%d", ret);
      }
    }
    ...
    if (OB_SUCCESS == ret && cell != NULL)
    {
          ...
          cell->column_name_.compare("3_tablet_version") == 0)
      {
        ...
        //hex_dump(cell->row_key_.ptr(),cell->row_key_.length(),false,YYSYS_LOG_LEVEL_INFO);
        //YYSYS_LOG(DEBUG,"tablet_version is %d",version);
      }
...
int parse_rowkey_type_array(const char* rowkey_type_str, int* array, int32_t& size)
{
  ...
  yysys::CStringUtil::split(const_cast<char*>(rowkey_type_str), ",", list);
  int32_t cnt = 0;
...
int parse_rowkey_obj_array(const int* type_array, const int32_t size, const char* rowkey_value_str, ObObj* obj_array)
{
  ...
  std::vector<char*> list;
  yysys::CStringUtil::split(const_cast<char*>(rowkey_value_str), ",", list);
