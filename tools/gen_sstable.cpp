...
namespace oceanbase
{
  namespace chunkserver
  {
      ...
      if (!inited_)
      {
        ...
        if (NULL == config_file_)
        {
          ...
          if (is_join_table_)
          {
            ...
            if (max_rows_ <= 0)
            {
              YYSYS_LOG(ERROR, "max_rows_(=%ld/%ld) of join_table must larger than 0!",
        ...
        else
        {
          ...
          row_key_prefix_   = YYSYS_CONFIG.getInt(table_name,"rowkey_prefix",0) - 1;
          row_key_suffix_   = YYSYS_CONFIG.getInt(table_name,"rowkey_suffix",0) - 1;
          step_length_      = YYSYS_CONFIG.getInt(table_name,"step",1);

          set_min_       = false;
          set_max_       = false;
          int smax       = YYSYS_CONFIG.getInt(table_name,"set_max",1);
          int smin       = YYSYS_CONFIG.getInt(table_name,"set_min",1);
          set_max_       = 1 == smax ? true : false;
          set_min_       = 1 == smin ? true : false;

          curr_uid_       = YYSYS_CONFIG.getInt(table_name,"start_uid",0);
          curr_tid_       = YYSYS_CONFIG.getInt(table_name,"start_tid",0);
          curr_itype_     = YYSYS_CONFIG.getInt(table_name,"start_type",0);
          max_rows_       = YYSYS_CONFIG.getInt(table_name,"max_rows",1);
          max_sstable_num_ = YYSYS_CONFIG.getInt(table_name,"max_sstable",10);
      ...
      for(int32_t sstable_num = 0; OB_SUCCESS == ret && sstable_num < max_sstable_num_; ++sstable_num)
      {
        ...
        for(int32_t group_index=0; OB_SUCCESS == ret && group_index < column_group_num; ++group_index)
        {
          ...
          for(int i=0;(i < max_rows_) && (OB_SUCCESS == ret); ++i)
          {
            ...
            {
              YYSYS_LOG(ERROR, "fill row error: [%d]",ret);
            }
            ...
            {
              YYSYS_LOG(ERROR,"append row failed: [%d]",ret);
            }
        ...
        {
          YYSYS_LOG(ERROR,"finish_sstable failed: [%d]",tmp_ret);
          if (OB_SUCCESS == ret)
          {
            ret = tmp_ret;
          }
    ...
    int ObGenSSTable::TableGen::generate2()
    {
      ...
      {
        YYSYS_LOG(ERROR, "schem2_ get table schema error");
      }
      else
      {
        for (int64_t i = 0; i < row_size; i ++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "row_desc_ add column desc error:"
                "ret=%d", ret);
            break;
          }
        }
      ...
      {
        YYSYS_LOG(ERROR, "schema2_ get table schema error");
      }
      else
      {
        for (int64_t i = 0; i < row_size; i ++)
    ...
    int ObGenSSTable::TableGen::finish_sstable(bool set_max /*=false*/)
    {
      ...
      range_.dump();
      range_.hex_dump(YYSYS_LOG_LEVEL_DEBUG);
#endif
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "failed to serialization chunksum, ret=%d", ret);
          }
          if (0 == extend_info.row_count_ || 0 == extend_info.check_sum_)
          {
            YYSYS_LOG(WARN, "extend_info.row_count_ %ld and extend_info.check_sum_ %ld"
          ...
          else
          {
            switch(column->get_type())
            {
              ...
              case ObDoubleType:
                ...
                break;
              default:
                YYSYS_LOG(ERROR,"unexpect type : %d",column->get_type());
                ret = OB_ERROR;
                break;
            }
          }
    ...
    int ObGenSSTable::TableGen::create_rowkey_aux()
    {
      ...
      { // only for collect_info table
        ...
        else
        {
          ...
          {
            ret = OB_INVALID_ARGUMENT;
            YYSYS_LOG(ERROR, "item step[%ld] must larger than 0\n", item_step);
          }
          else
          {
...
int main(int argc, char *argv[])
{
  ...
  {
    if (YYSYS_CONFIG.load(args.config_file_))
    {
      fprintf(stderr, "load file %s error\n", args.config_file_);
      return EXIT_FAILURE;
    }

    schema_file = YYSYS_CONFIG.getString("config", "schema", "schema.ini");
    ...
    args.dest_dir_    = YYSYS_CONFIG.getString("config","dest_dir","data");
    args.file_no_     = YYSYS_CONFIG.getInt("config","sstable_id_base",1);
    args.disk_no_     = YYSYS_CONFIG.getInt("config","disk_no",1);
    args.block_size_ = YYSYS_CONFIG.getInt("config","block_size", sstable::ObSSTableBlockBuilder::SSTABLE_BLOCK_SIZE);
    args.comp_name_ = YYSYS_CONFIG.getString("config", "comp_name", "lzo_1.0");
  }
  else
  {
    if (argc < 6)
