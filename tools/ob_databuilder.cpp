...
namespace oceanbase 
{
  namespace chunkserver
  {
                            ...
                            ObRowDesc* row_desc)
    {
      ...
      {
        YYSYS_LOG(ERROR,"can't find this table:%lu",table_id);
        ret = OB_ERROR;
      }
      else
      {
        ...
        for (int col_index = 0; col_index < rowkey_info.get_size() && OB_SUCCESS == ret; ++col_index)
        {
          ...
          if (NULL == column) 
          {
            YYSYS_LOG(ERROR, "schema internal error, rowkey column %d not exist.", col_index);
            ret = OB_ERROR;
            break;
          }
          ...
          {
            YYSYS_LOG(ERROR,"can not add column_def to sstable_schema [%d],table_id[%lu],column_id[%lu]",ret,column_def.table_id_,column_def.column_id_);            
          }
          else
          {
        ...
        for(int col_index = 0; (col_index < size) && (OB_SUCCESS == ret); ++col_index)
        {
          ...
          YYSYS_LOG(INFO,"add column table_id[%lu],column_id[%lu]",column_def.table_id_,column_def.column_id_);
...
#ifdef BUILDER_DEBUG
            YYSYS_LOG(INFO,"data entry [%d], id:%2d,index:%2d,type:%2d,len:%2d",i,DATA_ENTRY[i].column_id,
    ...
    int ObDataBuilder::init(ObDataBuilderArgs& args)
    {
      ...
      {
        YYSYS_LOG(ERROR,"convert schema failed [%d]",ret);
      }
      else
      {
    ...
    int ObDataBuilder::build_with_no_cache()
    {
      ...
      {
        YYSYS_LOG(ERROR,"open file error:%s\n",strerror(errno));
        ret = OB_ERROR;
      }
      else 
      {
        ...
        {
          YYSYS_LOG(ERROR,"create sstable failed [%d]\n",ret);
        }
        else
        {
          bool is_split = false;
          while((OB_SUCCESS == ret) && read_line(fields) != NULL)
          {
            ...
            {
              YYSYS_LOG(WARN,"append_row failed:%ld\n",read_lines_);
            }
            else
            {
              ++wrote_keys_;
              ...
              if (is_split)
              {
                ...
                {
                  YYSYS_LOG(ERROR,"set sstable filepath failed");
                }
                wrote_keys_ = 0;
              }
            }
    ...
    char *ObDataBuilder::read_line(int &fields)
    {
      ...
      while(true)
      {
        if ( (line = fgets(buf_[buf_idx%2],sizeof(buf_[buf_idx%2]),fp_)) != NULL)
        {
          ...
          {
            YYSYS_LOG(ERROR,"%ld : raw data expect %d fields,but %d",read_lines_,RAW_DATA_FILED,fields);
            continue;
          }
          ...
          // {
          //   YYSYS_LOG(DEBUG,"dont deal the same line,%ld,%d,%ld\n",read_lines_,buf_idx,row_key_len);
          //   continue;
          // }
        }
...
//     create_rowkey(row_key_buf_[row_key_buf_index_++ % 2],ROW_KEY_BUF_LEN,pos);
        ...
        for(;i < DATA_ENTRIES_NUM; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR,"data format error?");
          }
          else
          {
            switch(ENTRY[i].type)
            {
              ...
              case ObPreciseDateTimeType:
                ...
                break;
              default:
                YYSYS_LOG(DEBUG,"unexpect type index: %d,type:%d",i,ENTRY[i].type);
                continue;
                break;
            }
          }
      ...
      for(int k=0;k<j;++k)
      {
        YYSYS_LOG(DEBUG,"%d,type:%d",k,row_value_[k].get_type());
        row_value_[k].dump(YYSYS_LOG_LEVEL_INFO);
      }
#endif
      ...
      for(int k=0;k<j;++k)
      {
        ...
        {
          YYSYS_LOG(WARN,"add_obj failed:%d\n",ret);
        }
      }
      return ret;
    }
    ...
    int ObDataBuilder::record_sstable_range()
    {
      ...
      {
        YYSYS_LOG(ERROR,"get table_range failed [%d]",ret);
      }
      else if (((ret = image_.alloc_tablet_object(*range,tablet)) != OB_SUCCESS) || NULL == tablet)
      {
        YYSYS_LOG(ERROR,"alloc_tablet_object failed [%d]",ret);
      }
      ...
      {
        YYSYS_LOG(ERROR,"add tablet failed [%d]",ret);
      }
    ...
    int ObDataBuilder::write_idx_info()
    {
      ...
      {
        YYSYS_LOG(ERROR,"write meta info failed");
      }
      return ret;
    }
    ...
    ObDateTime ObDataBuilder::transform_date_to_time(const char *str)
    {
      ...
      if (NULL != str && *str != '\0')
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR,"sscanf failed : [%s] ",str);
          YYSYS_LOG(ERROR,"line,%ld\n",read_lines_);
          t = atol(str);
        }
        else
        {
...
int main(int argc, char *argv[])
{
  ...
  ob_init_memory_pool();
  yysys::CConfig c1;
  ObSchemaManagerV2  *mm = new (std::nothrow)ObSchemaManagerV2(yysys::CTimeUtil::getTime());
  if ( !mm->parse_from_file(schema_file, c1) )
  {
    YYSYS_LOG(ERROR,"parse schema failed.\n");
    exit(1);
  }
  ...
  {
    YYSYS_LOG(ERROR,"new databulder failed");
    exit(1);
  }
  
  if (data_builder->init(args) != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR,"init failed");
  }
  else
  {
