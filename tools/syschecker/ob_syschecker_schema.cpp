  ...
  namespace syschecker 
  {
      ...
      const ObTableSchema* schema, const char* org_column_name)
    {
      ...
      if (NULL != schema && NULL != org_column_name)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get column schema, ret=%d", ret);
          column[0] = NULL;
        }
      }
      else
      {
        YYSYS_LOG(WARN, "invalid param, schema=%p, org_column_name=%p",
    ...
    int ObSyscheckerSchema::init_wt_column()
    {
      ...
      {
        YYSYS_LOG(WARN, "wide table or join table schema is NULL, "
      ...
      else
      {
        ...
        for (int64_t i = 0; i < column_size; ++i)
        {
          ...
          if (memcmp(column_name, AUX_NAME_PREFIX, AUX_NAME_PREFIX_SIZE) != 0)
          {
            ...
            YYSYS_LOG(INFO, "wide table: column=%s, aux_column=%s",
      ...
      {
        YYSYS_LOG(WARN, "no column in wide table, wt_column_count_=%ld", 
                  wt_column_count_);
        ret = OB_ERROR;
      }
    ...
    int ObSyscheckerSchema::init_jt_column()
    {
      ...
      else
      {
        ...
        for (int64_t i = 0; i < column_size; ++i)
        {
          ...
          if (memcmp(column_name, AUX_NAME_PREFIX, AUX_NAME_PREFIX_SIZE) != 0)
          {
            ...
            YYSYS_LOG(INFO, "join table: column=%s, aux_column=%s",
                column[i].get_name(), aux_column == NULL ? NULL : aux_column->get_name());
            jt_column_count_++;
      ...
      {
        YYSYS_LOG(WARN, "no column in join table, jt_column_count_=%ld", 
                  jt_column_count_);
        ret = OB_ERROR;
      }
    ...
    int ObSyscheckerSchema::init()
    {
      ...
      if (inited_)
      {
        YYSYS_LOG(WARN, "syschecker schema has inited before");
        ret = OB_ERROR;
      }
      ...
      {
        YYSYS_LOG(WARN, "expected 2 schema in schema manager, but get %ld schema",
                  schema_size);
        ret = OB_ERROR;
      }
