...
namespace oceanbase
{
  namespace syschecker
  {
    ...
    int ObWriteWorker::init(ObOpParam** write_param)
    {
      ...
      if (NULL != write_param && NULL == *write_param)
      {
        *write_param = reinterpret_cast<ObOpParam*>(ob_malloc(sizeof(ObOpParam), ObModIds::TEST));
        if (NULL == *write_param)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for write param");
          ret = OB_ERROR;
        }
        else
        {
                                  ...
                                  ObMutator& mutator)
    {
      ...
      {
        YYSYS_LOG(WARN, "run invalid operation, expect fail, but success, "
      ...
      {
        YYSYS_LOG(WARN, "run valid operation, expect success, but fail, "
                        "table_name=%.*s, is_wide_table=%s, op_type=%s, "
                        "row_count=%ld, ret=%d",
                  write_param.table_name_.length(), write_param.table_name_.ptr(),
                  write_param.is_wide_table_ ? "true" : "false",
      ...
      if (OB_SUCCESS == init(&write_param) && NULL != write_param)
      {
        while (!_stop)
        {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to run mixed operation");
              write_param->display();
              break;
            }
          }
          ...
          {
            YYSYS_LOG(WARN, "get next write param failed");
          }
        }
      }
