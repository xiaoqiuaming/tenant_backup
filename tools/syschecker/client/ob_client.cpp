...
{ 
  namespace client 
  {
    ...
    int ObClient::init(const int64_t timeout)
    {
      ...
      if (timeout <= 0)
      {
        YYSYS_LOG(WARN, "invalid param, timeout=%ld", timeout);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to init server rpc stub");
          }
        }
      }
    ...
    int ObClient::ups_apply(const ObMutator& mutator)
    {
      ...
      if (OB_RESPONSE_TIME_OUT == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to fetch update server addr, ret=%d", tmp_ret);
          ret = OB_ERROR;
        }
        else
        {
          servers_mgr_.set_update_server(tmp_server);
          YYSYS_LOG(INFO, "update ups location, ip[%d.%d.%d.%d] port[%d]",
    ...
    int ObClient::set_ups_by_rs()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "fetch_update_server ret=%d", ret);
      }
      else
      {
