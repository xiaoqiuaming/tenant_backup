...
{ 
  namespace client 
  {
        ...
        const int32_t pcode, common::ObArray<common::ObServer> &data_servers)
    {
      ...
      {
        YYSYS_LOG(ERROR, "send_param_0 error, root_server=%s, timeout=%ld, pcode=%d", 
            to_cstring(root_server), timeout, pcode);
      }
      else if (OB_SUCCESS != (ret = deserialize_result_1(data_buffer, pos, rc, num)))
      {
        YYSYS_LOG(ERROR, "deserialize_result error, root_server=%s, timeout=%ld, pcode=%d, pos=%ld", 
      ...
      else
      {
        for (int32_t i = 0; i < num && OB_SUCCESS == ret; ++i)
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize server error, ret=%d, root_server=%s, timeout=%ld, pcode=%d, pos=%ld", 
                ret, to_cstring(root_server), timeout, pcode, pos);
          }
          else if (OB_SUCCESS != (ret = data_servers.push_back(server)))
          {
            YYSYS_LOG(ERROR, "push back, ret=%d, server=%s, timeout=%ld, pcode=%d, pos=%ld", 
          ...
          {
            YYSYS_LOG(ERROR, "deserialize reserve error, ret= %d, root_server=%s, timeout=%ld, pcode=%d, pos=%ld", 
                ret, to_cstring(root_server), timeout, pcode, pos);
          }
        }
      }
