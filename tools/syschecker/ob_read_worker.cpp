...
namespace oceanbase
{
  namespace syschecker
  {
    ...
    int ObReadWorker::init(ObOpParam** read_param)
    {
      ...
      if (NULL != read_param && NULL == *read_param)
      {
        *read_param = reinterpret_cast<ObOpParam*>(ob_malloc(sizeof(ObOpParam), ObModIds::TEST));
        if (NULL == *read_param)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for read param");
          ret = OB_ERROR;
        }
        else
        {
        ...
        const ObCellInfo& cell_info)
    {
      ...
      {
        YYSYS_LOG(WARN, "check column name failed, "
      ...
      if (OB_SUCCESS == ret )
      {
        ...
        {
          YYSYS_LOG(WARN, "check column type failed, "
      ...
      if (ObNullType == cell_type)
      {
        switch (aux_cell_type)
        {
          ...
          case ObIntType:
            ...
            if (prefix != aux_intv)
            {
              YYSYS_LOG(WARN, "check int value failed, column name=%.*s, "
          ...
          case ObFloatType:
            ...
            {
              YYSYS_LOG(WARN, "check float value failed, column name=%.*s, "
          ...
          case ObDoubleType:
            ...
            {
              YYSYS_LOG(WARN, "check double value failed, column name=%.*s, "
                  "expected value=%.4f, actual value=%.4f",
                  cell_info.column_name_.length(),
          ...
          default:
            YYSYS_LOG(WARN, "invalid auxiliary cell info type %s",
        ...
        const ObCellInfo& aux_cell_info)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        switch (cell_type)
        {
          ...
          case ObIntType:
            ...
            {
              YYSYS_LOG(WARN, "expect auxiliary cell type is ObIntType, "
          ...
          case ObFloatType:
            ...
            {
              YYSYS_LOG(WARN, "expect auxiliary cell type is ObFloatType, "
                  "but it's %s", OBJ_TYPE_STR[aux_cell_type]);
              ret = OB_ERROR;
              break;
            }
          ...
          case ObDoubleType:
            ...
            {
              YYSYS_LOG(WARN, "expect auxiliary cell type is ObDoubleType, "
                  "but it's %s", OBJ_TYPE_STR[aux_cell_type]);
              ret = OB_ERROR;
              break;
            }
          ...
          case ObDateTimeType:
            ...
            {
              YYSYS_LOG(WARN, "check ObDateTime value failed, column name=%.*s, "
                  "expected value=%ld, actual value=%ld, "
          ...
          case ObPreciseDateTimeType:
            ...
            {
              YYSYS_LOG(WARN, "expect auxiliary cell type is ObIntType or "
            ...
            {
              YYSYS_LOG(WARN, "check ObPreciseDateTime value failed, "
          ...
          case ObVarcharType:
            ...
            {
              YYSYS_LOG(WARN, "check ObVarchar value failed, table=%ld, rowkey=%s, column name=%.*s, "
          ...
          case ObCreateTimeType:
            ...
            {
              YYSYS_LOG(WARN, "check createtime value failed, column name=%.*s, "
                  "expected value=%ld, actual value=%ld, ",
                  cell_info.column_name_.length(),
                  cell_info.column_name_.ptr(),
          ...
          case ObModifyTimeType:
            ...
            {
              YYSYS_LOG(WARN, "check modifytime value failed, column name=%.*s, "
                  "expected value=%ld, actual value=%ld, ",
                  cell_info.column_name_.length(),
                  cell_info.column_name_.ptr(),
          ...
          case ObExtendType:
          default:
            YYSYS_LOG(WARN, "invalid cell info type %s",
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "%s", to_cstring(cell_info.row_key_));
      }
    ...
    int ObReadWorker::display_scanner(const ObScanner& scanner) const
    {
          ...
          iter != scanner.end() && OB_SUCCESS == ret; iter++)
      {
        ...
        if (NULL == cell_info)
        {
          YYSYS_LOG(WARN, "get null cell_info");
          ret = OB_ERROR;
          break;
        }
        ...
        const bool is_get)
    {
          ...
          iter != scanner.end() && OB_SUCCESS == ret; iter++)
      {
        if (!read_aux_cell)
        {
          ...
          else if (is_row_changed)
          {
            ...
            if (OP_GET == read_param.op_type_)
            {
              ...
              {
                YYSYS_LOG(WARN, "row key isn't consistent");
                YYSYS_LOG(WARN, "%s", to_cstring(row_key));
                YYSYS_LOG(WARN, "%s", to_cstring(pcell->row_key_));
                ret = OB_ERROR;
                break;
              }
            }
          ...
          {
            YYSYS_LOG(WARN, "row doesn't exist, but we get other column "
              ...
              && pcell->value_.get_ext() == ObActionFlag::OP_ROW_DOES_NOT_EXIST)
          {
            ...
            {
              YYSYS_LOG(WARN, "scan operation return 'OP_ROW_DOES_NOT_EXIST' cell, "
          ...
          {
            YYSYS_LOG(WARN, "column index is greater than row cell count, "
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "get null aux_cell_info or row change:%p,%d, %s",
        ...
        ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        consume_time = yysys::CTimeUtil::getTime() - start_time;
      ...
      if (OB_SUCCESS == ret && param_.is_check_result())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "get error, ret=%d, \nget_param:%s", ret, to_cstring(get_param));
          dump_scanner(scanner, YYSYS_LOG_LEVEL_WARN);
        }
      }
        ...
        ObScanner& scanner)
    {
      ...
      {
        YYSYS_LOG(WARN, "wrong row count for scan, expect 2 row, but row_count=%ld",
      ...
      {
        YYSYS_LOG(WARN, "scan err=%d dump content of scan param:", ret);
        scan_param.dump();
      }
    ...
    void ObReadWorker::run(CThread* thread, void* arg)
    {
      ...
      if (OB_SUCCESS == init(&read_param) && NULL != read_param)
      {
        while (!_stop)
        {
          ...
          if (OB_SUCCESS == err)
          {
            ...
            switch (read_param->op_type_)
            {
              ...
              case OP_SQL_SCAN:
                ...
                break;
              default:
                YYSYS_LOG(WARN, "wrong op type :%d", read_param->op_type_);
                err = OB_ERROR;
            }
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to run read operation, err=%d", err);
              read_param->display();
          ...
          {
            YYSYS_LOG(WARN, "get next read param failed");
          }
        }
      }
    ...
    int ObReadWorker::fill_rowkey_project(const int64_t table_id, sql::ObProject &project, common::ObRowDesc& row_desc)
    {
      ...
      for (int64_t i = 0; i < rowkey_info.get_size() && OB_SUCCESS == ret; ++i)
      {
        ...
        {
          YYSYS_LOG(ERROR, "fill_project tid:[%ld],cid:[%ld]", table_id, column_id);
        }
        else if (OB_SUCCESS != (ret = row_desc.add_column_desc(table_id, column_id)))
        {
          YYSYS_LOG(ERROR, "add_column_desc tid:[%ld],cid:[%ld]", table_id, column_id);
        }
      }
      return ret;
    }
    ...
    int ObReadWorker::fill_sql_scan_param(ObOpParam& read_param, sql::ObSqlScanParam& scan_param, common::ObRowDesc& row_desc)
    {
      ...
      {
        YYSYS_LOG(WARN, "get_last_frozen_version ret=%d", ret);
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        //YYSYS_LOG(INFO, "scan range=%s, project=%s", to_cstring(range), to_cstring(project));
    ...
    int ObReadWorker::fill_sql_get_param(ObOpParam& read_param, sql::ObSqlGetParam& get_param, common::ObRowDesc& row_desc)
    {
      ...
      //YYSYS_LOG(INFO, "get_param tid=%ld, rename tid=%ld", get_param.get_table_id(), get_param.get_renamed_table_id());
      return ret;
    }
        ...
        const bool is_get)
    {
          ...
          OB_SUCCESS == const_cast<ObNewScanner&>(scanner).get_next_row(current_row))
      {
        ...
        {
          YYSYS_LOG(ERROR, "rowkey not correct=%d", ret);
        }
        else if (row_key->get_obj_ptr()[0].get_type() != ObIntType)
        {
          YYSYS_LOG(ERROR, "rowkey(%s) prefix not int type", to_cstring(*row_key));
        }
        else if (OB_SUCCESS != (ret = row_key->get_obj_ptr()[0].get_int(prefix)))
        {
          YYSYS_LOG(ERROR, "rowkey(%s) prefix error,ret=%d", to_cstring(*row_key), ret);
        }
        else
        {
          for (column_index = 0; column_index < row_param->cell_count_ && OB_SUCCESS == ret; column_index += MIN_OP_CELL_COUNT)
          {
            ...
            {
              YYSYS_LOG(ERROR, "get org cell ,tid=[%ld], cid=[%ld], ret=%d",
            ...
            {
              YYSYS_LOG(ERROR, "get aux cell ,tid=[%ld], cid=[%ld], ret=%d",
                  read_param.table_id_, cell_param->column_id_, ret);
            }
            else
            {
              ...
              {
                YYSYS_LOG(ERROR, "check_cell error, prefix=%ld, cid=%ld, cell=%s",
            ...
            /*
            YYSYS_LOG(INFO, "check_scanner_result, rowkey:%s,idx:%ld,tid:%ld,cid:%ld, pcell:%s",
