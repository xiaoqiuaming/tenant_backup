  ...
  namespace syschecker 
  {
        ...
        ObSchemaManagerV2& user_schema_mgr)
    {
          ...
          column_schema++)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to add column schema");
        }
      }
    ...
    int ObSyschecker::read_rowkey_range(ObScanner& scanner)
    {
      ...
      YYSYS_LOG(DEBUG, "read_rowkey_range");
           ...
           iter++, index++)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          if (0 == index)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              YYSYS_LOG(INFO, "get cur_max_prefix=%ld", prefix);
          ...
          else if (1 == index)
          {
            ...
            if (OB_SUCCESS == ret)
            {
              YYSYS_LOG(INFO, "get cur_max_suffix=%ld", suffix);
              rule_.add_cur_max_suffix(suffix + param_.get_syschecker_no());
            }
          }
          else
          {
            YYSYS_LOG(WARN, "more than 2 cell returned for get rowkey range");
            ret = OB_ERROR;
          }
        }
      }
    ...
    int ObSyschecker::init_rowkey_range()
    {
      ...
      {
        YYSYS_LOG(WARN, "wide table schema is invalid, wt_schema=%p", 
                  wt_schema);
        ret = OB_ERROR;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to get cell from merge server, ret=%d, rowkey:%s", 
    ...
    int ObSyschecker::init()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to fetch schema from root server");
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to translate schema manager, ret=%d", ret);
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to init syschecker schema manager");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to get current key range from oceanbase");
          }
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to init syschecker rule");
        }
      }
    ...
    int ObSyschecker::start()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (stat_dump_interval > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to init stat");
          }
        }
                                     ...
                                     char* value_buf, const int64_t value)
    {
      ...
      case ObMaxType:
      default:
        YYSYS_LOG(WARN, "wrong object type %d for store rowkey range", type);
        ret = OB_ERROR;
        break;
      }
