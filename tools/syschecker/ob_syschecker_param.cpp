...
{ 
  namespace syschecker 
  {
                                       ...
                                       bool not_null)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        value = YYSYS_CONFIG.getString(section, name);
        if (not_null && (NULL == value || 0 >= strlen(value)))
        {
          YYSYS_LOG(WARN, "%s.%s has not been set.", section, name);
          ret = OB_ERROR;
        }
      }
      ...
      if (OB_SUCCESS == ret && NULL != value)
      {
        ...
        {
          YYSYS_LOG(WARN, "%s.%s too long, length (%lu) > %ld", 
    ...
    int ObSyscheckerParam::parse_server(char* str, common::ObArray<common::ObServer>& servers)
    {
      ...
      if (NULL == str)
      {
        YYSYS_LOG(WARN, "invalid param, str=%p", str);
        ret = OB_ERROR;
      }
      ...
      for (begin_ptr = str; begin_ptr != end_ptr && OB_SUCCESS == ret;) 
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "invalid merge server ip=%s, port=%d.", buffer, port);
          break;
        }
    ...
    int ObSyscheckerParam::load_server()
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to parse merge server string");
      }
    ...
    int ObSyscheckerParam::load_from_config()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        root_server_port = YYSYS_CONFIG.getInt(OBRS_SECTION, 
                                               OBRS_PORT, 0);
        if (root_server_port <= 0)
        {
          YYSYS_LOG(WARN, "root server port=%d cann't <= 0.", 
                    root_server_port);
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (!res)
        {
          YYSYS_LOG(WARN, "root server ip=%s, port=%d is invalid.", 
      ...
      if (OB_SUCCESS == ret)
      {
        update_server_port = YYSYS_CONFIG.getInt(OBUPS_SECTION, 
                                                 OBUPS_PORT, 0);
        if (update_server_port <= 0)
        {
          YYSYS_LOG(WARN, "update server port=%d cann't <= 0.", 
                    update_server_port);
          ret = OB_INVALID_ARGUMENT;
        }
      }   
      if (OB_SUCCESS == ret)
      {
        ...
        if (!res)
        {
          YYSYS_LOG(WARN, "update server ip=%s, port=%d is invalid.", 
      ...
      if (OB_SUCCESS == ret)
      {
        write_thread_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                  OBSC_WRITE_THREAD_COUNT, 
                                                  DEFAULT_WRITE_THREAD_COUNT);
        if (write_thread_count_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker write thread count=%ld can't <= 0." ,
              write_thread_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        read_thread_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                 OBSC_READ_THREAD_COUNT, 
                                                 DEFAULT_READ_THERAD_COUNT);
        if (read_thread_count_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker read thread count=%ld can't <= 0." ,
              read_thread_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "both read_thread_count_ and write_thread_count_ is "
      ...
      if (OB_SUCCESS == ret)
      {
        network_time_out_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                OBSC_NETWORK_TIMEOUT, 
                                                DEFAULT_TIMEOUT);
        if (network_time_out_ <= 0)
        {
          YYSYS_LOG(WARN, "syschecker network timeout=%ld can't <= 0." ,
                    network_time_out_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        syschecker_count_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                OBSC_SYSCHECKER_COUNT, 
                                                DEFAULT_SYSCHECKER_COUNT);
        if (syschecker_count_ <= 0)
        {
          YYSYS_LOG(WARN, "syschecker syschecker count=%ld can't <= 0." ,
              syschecker_count_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        syschecker_no_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                             OBSC_SYSCHECKER_NO, 0);
        if (syschecker_no_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker syschecker no=%ld can't < 0." ,
              syschecker_no_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        specified_read_param_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                    OBSC_SPECIFIED_READ_PARAM, 0);
        if (specified_read_param_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker specified_read_param_=%ld can't < 0." ,
      ...
      if (OB_SUCCESS == ret)
      {
        operate_full_row_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                OBSC_OPERATE_FULL_ROW, 0);
        if (operate_full_row_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker operate_full_row_=%ld can't < 0." ,
              operate_full_row_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        stat_dump_interval_ = YYSYS_CONFIG.getInt(OBSC_SECTION, 
                                                OBSC_STAT_DUMP_INTERVAL, 0);
        if (stat_dump_interval_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker stat_dump_interval_=%ld can't < 0." ,
              stat_dump_interval_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        perf_test_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_PERF_TEST, 
                                         DEFAULT_PERF_TEST);
        if (perf_test_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker perf_test_=%ld can't < 0." ,
              perf_test_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        is_sql_read_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_SQL_READ, 
                                         DEFAULT_SQL_READ);
        if (is_sql_read_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker is_sql_read_=%ld can't < 0." ,
              is_sql_read_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        check_result_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_CHECK_RESULT, 
                                            DEFAULT_CHECK_RESULT);
        if (check_result_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker check_result_=%ld can't < 0." ,
              check_result_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        read_table_type_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_READ_TABLE_TYPE, 
                                               DEFAULT_READ_TABLE_TYPE);
        if (read_table_type_ < 0 || read_table_type_ > 2)
        {
          YYSYS_LOG(WARN, "syschecker read_table_type_=%ld can't < 0 or > 2, "
      ...
      if (OB_SUCCESS == ret)
      {
        write_table_type_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_WRITE_TABLE_TYPE, 
                                                DEFAULT_WRITE_TABLE_TYPE);
        if (write_table_type_ < 0 || write_table_type_ > 2)
        {
          YYSYS_LOG(WARN, "syschecker write_table_type_=%ld can't < 0 or > 2, "
                          "0:all_table, 1:wide_table, 2: join_table" ,
              write_table_type_);
          ret = OB_INVALID_ARGUMENT;
        }
      ...
      if (OB_SUCCESS == ret)
      {
        get_row_cnt_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_GET_ROW_CNT, 
                                          DEFAULT_GET_ROW_CNT);
        if (get_row_cnt_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker get_row_cnt_=%ld can't < 0." ,
              get_row_cnt_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        scan_row_cnt_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_SCAN_ROW_CNT, 
                                            DEFAULT_SCAN_ROW_CNT);
        if (scan_row_cnt_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker scan_row_cnt_=%ld can't < 0." ,
              scan_row_cnt_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        update_row_cnt_ = YYSYS_CONFIG.getInt(OBSC_SECTION, OBSC_UPDATE_ROW_CNT, 
                                              DEFAULT_UPDATE_ROW_CNT);
        if (update_row_cnt_ < 0)
        {
          YYSYS_LOG(WARN, "syschecker update_row_cnt_=%ld can't < 0." ,
              update_row_cnt_);
          ret = OB_INVALID_ARGUMENT;
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "specify perf_test_ is true, but not specify op cnt, "
