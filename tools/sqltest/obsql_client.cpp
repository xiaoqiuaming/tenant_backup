...
int ObSqlClient::init(char* hostaddr, int port)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, hostaddr=%p, port=%d", hostaddr, port);
    err = OB_INVALID_ARGUMENT;
  }
  else
  {
    int thread_safe = PQisthreadsafe();
    YYSYS_LOG(INFO, "pgsql thread_safety=%s", thread_safe ? "true" : "false");
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to connect, err=%d", err);
    }
    */
  }
...
int ObSqlClient::exec_query(char* query_sql, Array& array)
{
  ...
  {
    YYSYS_LOG(WARN, "query failed, query_sql=%s, err_msg=%s, err=%d", query_sql, PQerrorMessage(conn),
        PQresultStatus(res));
    err = OB_ERROR;
  }
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to translate res, err=%d", err);
    }
  }

  PQclear(res);

  YYSYS_LOG(INFO, "[OBSQL:exec_query], sql=%s, err=%d", query_sql, err);
  return err;
}
...
int ObSqlClient::exec_update(char* update_sql)
{
  ...
  {
    YYSYS_LOG(WARN, "update sql failed, update_sql=%s, err_msg=%s, err_code=%d", update_sql, PQerrorMessage(conn),
        PQresultStatus(res));
    err = OB_ERROR;
  }
  ...
  YYSYS_LOG(INFO, "[OBSQL:exec_update], sql=%s, err=%d", update_sql, err);
  return err;
}
...
int ObSqlClient::connect()
{
  ...
  YYSYS_LOG(INFO, "[GET_CONN] host_addr=%s, port=%d, conn_=%p", host_addr_, port_, conn);

  if (PQstatus(conn) != CONNECTION_OK)
  {
    YYSYS_LOG(WARN, "Failed to connnect to obconnector: hostaddr=%s, port=%d, conn_info=%s, err_msg=%s",
  ...
  else
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(ERROR, "pthread_setspecific failed, err=%d", err);
    }
  }
...
int ObSqlClient::translate_res(PGresult* res, Array& array)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to init result array, row_num=%d, col_num=%d, err=%d",
          row_num, col_num, err);
    }
    else
    {
        ...
        for (int j = 0; OB_SUCCESS == err && j < col_num; ++j)
        {
          ...
          switch (type_id)
          {
            ...
            case 1114:
              ...
              break;
            default:
              YYSYS_LOG(ERROR, "invalid type, type_id=%d", type_id);
              err = OB_ERROR;
              break;
          }
          ...
          if (0 == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to set value, i=%d, j=%d", i, j);
            }
          }
        }
      }
...
{
  YYSYS_LOG(DEBUG, "delete thread specific ptr:%p", ptr);
  if (NULL != ptr)
  {
    PQfinish((PGconn*) ptr);
  }
