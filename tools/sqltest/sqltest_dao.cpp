    ...
    char* mysql_user, char* mysql_pass, char* mysql_db)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to init ob client, err=%d", err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to init mysql client, err=%d", err);
      }
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "failed to init prefix info, err=%d", err);
      }
    }
...
int SqlTestDao::exec_query(char* obsql_query, char* mysql_query, uint64_t prefix)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set read flag, err=%d", err);
  }
  ...
  if (0 == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "failed to query ob, sql=%s, err=%d", obsql_query, err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "failed to query mysql, sql=%s, err=%d", mysql_query, err);
      }
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to compare result, err=%d", err);
      }
    }
  }
  ...
  if (0 == err)
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to reset flag, err=%d", err);
    }
  }
...
int SqlTestDao::exec_update(char* obsql_update, char* mysql_update, uint64_t prefix)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set update flag to true, err=%d", err);
  }
  ...
  if (0 == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "failed to update ob, update_sql=%s, err=%d", obsql_update, err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "failed to update mysql, update_sql=%s, err=%d", mysql_update, err);
      }
    }
  }
...
int SqlTestDao::exec_query_ob(char* obsql_query, Array& ob_res)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "failed to query ob, sql=%s, err=%d", obsql_query, err);
  }
...
int SqlTestDao::exec_update_ob(char* obsql_update)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "failed to update ob, update_sql=%s, err=%d", obsql_update, err);
  }
...
int SqlTestDao::compare_result(Array& ob_res, Array& mysql_res)
{
  ...
  if (cmp_val != 0)
  {
    YYSYS_LOG(ERROR, "result is not equal, cmp_val=%d", cmp_val);
    err = OB_ERROR;
    ob_res.print();
    mysql_res.print();
    ...
    {
      YYSYS_LOG(ERROR, "exit program");
      exit(0);
    }
  }

  YYSYS_LOG(INFO, "compare result, err=%d", err);
  return err;
}
...
int SqlTestDao::set_flag(uint64_t prefix, int flag)
{
  ...
  {
    YYSYS_LOG(WARN, "failed to set flag, prefix=%lu, flag=%d, err=%d",
        prefix, flag, err);
  }
