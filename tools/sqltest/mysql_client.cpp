...
int MysqlClient::init(char* host, int port, char* mysql_user, char* mysql_pass, char* mysql_db)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, host=%p, port=%d, mysql_user=%p, mysql_pass=%p, mysql_db=%p",
...
int MysqlClient::exec_query(char* query_sql, Array& res)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to exec query, query_sql=%s, err_msg=%s", query_sql, mysql_error(conn));
    err = OB_ERROR;
  }
  else
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to init array, row_num=%d, col_num=%d, err=%d",
          row_num, col_num, err);
    }
    else
    {
        ...
        for (int col_idx = 0; col_idx < col_num; ++col_idx)
        {
          ...
          switch (field->type)
          {
            ...
            case MYSQL_TYPE_VAR_STRING:
                ...
                break;
            default:
                YYSYS_LOG(ERROR, "invalid type, type=%d", field->type);
                err = OB_ERROR;
                break;
          }
  ...
  YYSYS_LOG(INFO, "[MYSQL:exec_query]: sql=%s, err=%d", query_sql, err);
  return err;
}
...
int MysqlClient::exec_update(char* update_sql)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to exec update, update_sql=%s, err_msg=%s", update_sql, mysql_error(conn));
    err = OB_ERROR;
  }

  YYSYS_LOG(INFO, "[MYSQL:exec_update]: sql=%s, err=%d", update_sql, err);
  return err;
}
...
int MysqlClient::connect()
{
  ...
  if (NULL == conn)
  {
    YYSYS_LOG(WARN, "failed to init conn");
    err = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "failed to connect, host_=%p, port_=%d, mysql_user_=%s, mysql_pass=%s, mysql_db=%s, err_msg=%s",
    ...
    else
    {
      ...
      if (0 != err)
      {
        YYSYS_LOG(WARN, "pthread_setspecific failed, err=%d", err);
      }
    }
  }
