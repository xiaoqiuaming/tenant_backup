...
int KeyGenerator::init(ObSqlClient& ob_client, KeyRule& key_rule)
{
  ...
  if (0 == err)
  {
    uint32_t local_addr = ntohl(yysys::CNetUtil::getLocalAddr(NULL));
    client_id_ = local_addr & 0xffff;

    YYSYS_LOG(INFO, "[CLIENT_ID] client_id=%lu", client_id_);
  }
...
int KeyGenerator::get_next_key(uint64_t& prefix, uint64_t& suffix, bool is_read)
{
  ...
  if (0 != err)
  {
    YYSYS_LOG(WARN, "failed to get prefix, type=%d, err=%d", prefix_type, err);
  }
  else
  {
    ...
    if (0 != err)
    {
      YYSYS_LOG(WARN, "failed to get suffix, type=%d, prefix=%lu, err=%d",
...
int KeyGenerator::get_next_key(uint64_t prefix, uint64_t& suffix, GenType type)
{
  ...
  switch (type)
  {
    ...
    default:
      YYSYS_LOG(WARN, "invalid gen type, type=%d", type);
      err = OB_ERROR;
      break;
  }
