...
int SqlTestSchema::init_table_schema()
{
      ...
      schema++)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to init column schema, table_id=%lu, err=%d",
...
const int SqlTestSchema::get_key_count(int table_idx) const
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, table_idx=%d, num_tables_=%d", table_idx, num_tables_);
  }
...
const char* SqlTestSchema::get_column_name(int table_idx, int column_idx) const
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, column_idx=%d, num_columns_=%d", column_idx, num_columns_[table_idx]);
  }
...
const char* SqlTestSchema::get_key_name(int table_idx, int key_idx) const
{
  ...
  {
    YYSYS_LOG(WARN, "invalid param, key_idx=%d, num_keys_=%d", key_idx, num_keys_[table_idx]);
  }
