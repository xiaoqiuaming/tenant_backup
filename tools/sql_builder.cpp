...
int SqlBuilder::ups_mutator_to_sql(uint64_t seq, ObUpsMutator &mut, vector<string> &vector, CommonSchemaManagerWrapper *schema_manager, TableIdSet *table_set)
{
  int ret = OB_SUCCESS;
  ObMutatorCellInfo* cell = NULL;
  int cell_idx = 0;

  const ObTableSchema *last_ts = NULL;
  uint64_t last_tableid = 0;
  string last_rowkey_sql = "";

  if(schema_manager == NULL)
  {
    ret = OB_SCHEMA_ERROR;
    YYSYS_LOG(WARN, "Empty schema, schema_manager is NULL");
    return ret;
  }
  ...
  while (OB_SUCCESS == (ret = mut.next_cell()))
  {
    ...
    {
      YYSYS_LOG(ERROR, "get cell error\n");
      continue;
    ...
    else if(IS_SYS_TABLE_ID(cell->cell_info.table_id_) && cell->cell_info.table_id_ != 103)
    {//103:__ups_session_info ������ ����
      continue;
    }
    else if (HASH_EXIST == table_set->exist(cell->cell_info.table_id_))
    {
      YYSYS_LOG(DEBUG, "cell->cell_info.table_id_ = %lu, skip", cell->cell_info.table_id_);
      continue;
    }
    const ObTableSchema *ts = NULL;
    if(last_tableid == cell->cell_info.table_id_)
    {
      ts = last_ts;
    }
    else
    {
      ts = schema_manager->get_impl()->get_table_schema(cell->cell_info.table_id_);
    }
    if(NULL == ts)
    {
      ret = OB_SCHEMA_ERROR;
      YYSYS_LOG(WARN, "ObTableSchema null error[table null], SEQ:%lu op:%s table_id=%lu table_column=%lu",
    ...
    if (NULL==cs && iu_op=="UPDATE")
    {
      ret = OB_SCHEMA_ERROR;
      YYSYS_LOG(WARN,"ObTableSchema null error[column null],SEQ:%lu op:%s table_id=%lu table_column=%lu\n",seq,iu_op.c_str(), cell->cell_info.table_id_,cell->cell_info.column_id_);
      //exit(0);
      return ret;
    }
    ...
    else
    {
      ...
      for (int64_t i = 0; i < ts->get_rowkey_info().get_size(); i++)
      {
        ...
        if(temp_cs == NULL)
        {
          sql += "error_rowkey";
          ret = OB_SCHEMA_ERROR;
          YYSYS_LOG(WARN,"ObTableSchema error rowkey not found,SEQ:%lu op:%s table_id=%lu table_rowkey=%lu\n",seq,iu_op.c_str(), cell->cell_info.table_id_,rowKey->column_id_);
          //exit(0);
          return ret;
        }
        else
        {
          tmp_rowkey_sql += to_cstring(ob_obj_type_str(rowKey->type_));
          tmp_rowkey_sql += ":";
          tmp_rowkey_sql += to_cstring(temp_cs->get_name());
        }

        if(i != ts->get_rowkey_info().get_size() -1)
        {
          tmp_rowkey_sql += char(15);
        }
      }
      sql += tmp_rowkey_sql;
      last_rowkey_sql = tmp_rowkey_sql;
    }
    sql += " value->" + obj_tostring(cell->cell_info.value_);
    vector.push_back(sql);

    last_tableid = cell->cell_info.table_id_;
    last_ts = ts;
