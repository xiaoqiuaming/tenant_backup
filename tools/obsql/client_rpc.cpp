    ...
    const ObClientManager * rpc_frame)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "can not get update server");
      return ret;
    }
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "can not get chunk/merge servers");
      return ret;
    }
...
int ObClientServerStub::cs_scan(const ObScanParam& scan_param, ObScanner& scanner)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize scan_param failed:ret[%d].", ret);
    }
  }
  ...
  { 
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to remote server for scan failed"
          " ret[%d].",  ret);
    }
  }
...
int ObClientServerStub::cs_get(const ObGetParam& get_param, ObScanner& scanner)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to remote server for get failed"
          " ret[%d].",  ret);
    }
  }
...
int ObClientServerStub::ups_apply(const ObMutator &mutator)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "serialize get_param failed, ret=%d.", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "send request to update server for apply failed, "
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "deserialize result_code failed, pos=%ld, ret=%d.", 
                pos, ret);
    }
    else
    {
                                               ...
                                               oceanbase::common::ObString &image_buf)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize disk_no failed:ret[%d].", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize data_version failed:ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to remote server for cs_dump_tablet_image failed"
          " ret[%d].",  ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize image_buf from buff failed"
          "pos[%ld], ret[%d].", pos, ret);
    }
  }
...
int ObClientServerStub::rs_dump_cs_info(ObChunkServerManager &obcsm)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "deserialize ObChunkServerManager from buff failed"
          "pos[%ld], ret[%d].", pos, ret);
    }
  }
...
int ObClientServerStub::fetch_schema(ObSchemaManagerV2 &schema)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to root server for getting schema");
    }
  }
...
int ObClientServerStub::start_merge(const int64_t frozen_memtable_version, const int32_t init_flag)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize frozen_memtable_version failed:ret[%d].", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize init_flag failed:ret[%d].", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "send request to remote server for start_merge failed"
          " ret[%d].",  ret);
    }
  }
...
int ObClientServerStub::start_gc(const int32_t reserve)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR,"encode failed ret[%d]",ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(INFO,"send start gc success");
  }
