...
namespace oceanbase
{
  namespace chunkserver
  {
                                      ...
                                      const int64_t element_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "init write buffer failed");
      }
                                      ...
                                      const int64_t element_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to set frozen time, frozen_time=%ld", 
                                  ...
                                  int64_t& approx_space_usage)
    {
      ...
      {
        YYSYS_LOG(WARN, "current row does not contains rowkey column count=%ld, table_id=%lu", 
      ...
      else if (!first_row_ && is_invalid_row_key(table_id, column_group_id, row_key))
      {
            ...
            && row_key == cur_key_)
        {
          ...
          YYSYS_LOG(DEBUG, "append same row");
          return ret;
        }
        /**
      ...
      if (OB_SUCCESS == ret && need_switch_block(table_id, column_group_id, row_size))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to write current block to disk, "
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          YYSYS_LOG(WARN, "Problem add row into block builder, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                    table_id_, row_count_, offset_);
        }
                                     ...
                                     int64_t& sstable_size)
    {
      ...
      if (row_count_ > 0)
      {
        ...
        if (OB_SUCCESS == ret && block_builder_.get_row_count() > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem write the last block, "
                            "table_id=%lu, row_count=%ld, offset=%ld",
                    table_id_, row_count_, offset_);
          }
        }
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem write block index, "
                            "table_id=%lu, row_count=%ld, offset=%ld",
                    table_id_, row_count_, offset_);
          }
        }
      ...
      if (OB_SUCCESS == ret && enable_bloom_filter_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write bloom filter, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                  table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write schema, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                    table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write range, "
              "table_id=%lu, row_count=%ld, offset=%ld",
              table_id_, row_count_, offset_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem write trailer, "
                          "table_id=%lu, row_count=%ld, offset=%ld",
                    table_id_, row_count_, offset_);
        }
        else
    ...
    */
                                             ...
                                             int64_t& wrote_len)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "Problem serialize block header to sstable");
          }
        }
                                          ...
                                          const bool need_compress)
    {
            ...
            compress_buf_len, ObModIds::OB_SSTABLE_WRITER)))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to compress, input=%p, "
    ...
    int SSTableWriter::write_schema()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize schema for sstable");
        }
      }
    ...
    int SSTableWriter::write_range()
    {
      ...
      {
        YYSYS_LOG(WARN, "Problem serialize range for sstable, ret=%d", ret);
      }
      else if (OB_SUCCESS != 
    ...
    int SSTableWriter::write_trailer()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize trailer for sstable");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "Problem serialize trailer offset for sstable");
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "Problem write trailer offset to sstable, error=%s",
                    strerror(errno));
        }
        else
        {
          offset_ += trailer_offset_size;
          YYSYS_LOG(INFO, "close sstable, checksum=%lu, table_id=%lu, "
                                       ...
                                       int64_t& wrote_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, data=%p, data_size=%ld", 
                  data, data_size);
        ret = OB_ERROR;
      }
    ...
    int SSTableWriter::init_write_buf()
    {
      ...
      if (NULL == write_buf_)
      {
        ...
        if (NULL == write_buf_)
        {
          YYSYS_LOG(ERROR, "failed to allocate memory for write buffer");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int SSTableWriter::ensure_write_buf(const int64_t size)
    {
      ...
      if (size <= 0)
      {
        YYSYS_LOG(WARN, "invalid param, size=%ld", size);
        ret = OB_ERROR;
      }
      else if (write_buf_size_ - data_size_ < size)
      {
        ...
        {
          YYSYS_LOG(ERROR, "alloc memory failed new_write_buffer=%p", new_write_buffer);
          ret = OB_ERROR;
        }
        else
        {
