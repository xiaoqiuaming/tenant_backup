...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    void print_log_position_info(int64_t checkpoint, int64_t timestamp, const char *mod, int64_t heartbeat_timestamp)
    {
      ...
      {
        int64_t cur_tm = yysys::CTimeUtil::getTime();
        ...
        {
          YYSYS_LOG(INFO, "STAT: [%s] CHECKPOINT=%ld TIMESTAMP=[%s] DELAY=%.0lf sec  HEARTBEAT=[%s]",
              mod, checkpoint, usec2str(timestamp, false), delta, usec2str(heartbeat_timestamp, false));
        }
        else
        {
          YYSYS_LOG(INFO, "STAT: [%s] CHECKPOINT=%ld TIMESTAMP=[%s] DELAY=%.0lf sec",
