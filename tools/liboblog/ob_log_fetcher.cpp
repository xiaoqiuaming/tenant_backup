...
namespace oceanbase
{
  ...
  namespace liboblog
  {
        ...
        const int64_t start_id)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, server_selector=%p rpc_stub=%p start_id=%ld",
      ...
      {
        YYSYS_LOG(WARN, "get_max_checkpoint_ fail, ret=%d", ret);
      }
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "start fetch log from checkpoint %ld", start_id_);

        if (OB_SUCCESS != (ret = init_mutator_list_(MAX_MUTATOR_NUM)))
        {
          YYSYS_LOG(WARN, "prepare mutator list fail, ret=%d", ret);
        }
        else if (1 != start())
        {
          YYSYS_LOG(WARN, "start thread to fetch log fail");
    ...
    {
      YYSYS_LOG(INFO, "stop log fetcher begin");
      CDefaultRunnable::stop();
      YYSYS_LOG(INFO, "stop log fetcher end");
    }
    ...
    void ObLogFetcher::release_mutator(ObLogMutator *mutator)
    {
      if (NULL != mutator)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "push mutator to v_list fail, ret=%d", ret);
        }
        else
        {
          v_cond_.signal();
    ...
    void ObLogFetcher::refresh_ups_list_(bool force_refresh)
    {
      ...
      if (force_refresh || UPS_ADDR_REFRESH_INTERVAL <= (yysys::CTimeUtil::getTime() - last_refresh_time))
      {
        server_selector_->refresh();
        last_refresh_time = yysys::CTimeUtil::getTime();
      }
    }
    ...
    int ObLogFetcher::get_ups_(ObServer &ups_addr, bool change_ups_addr)
    {
      ...
      if (OB_SUCCESS != (ret = server_selector_->get_ups(ups_addr, change_ups_addr)))
      {
        ...
        {
          YYSYS_LOG(WARN, "get_ups fail: no updateserver available, wait %ld usec and refresh updateserver list",
              NO_UPS_TIMEWAIT);
        ...
        {
          YYSYS_LOG(ERROR, "get_ups fail, ret=%d, change_ups_addr=%s", ret, change_ups_addr ? "Y" : "N");
        }
      }
    ...
    void ObLogFetcher::run(yysys::CThread* thread, void* arg)
    {
      UNUSED(thread);
      UNUSED(arg);
      ...
      if (NULL == buffer)
      {
        YYSYS_LOG(WARN, "allocate fetch buffer fail");
      }
      else
      {
        ...
        int64_t last_log_fetched_time = yysys::CTimeUtil::getTime();
        ...
        while (ob_log_alive(_stop))
        {
          ...
          else
          {
            ...
            last_log_fetched_time = yysys::CTimeUtil::getTime();
          }

          // Handle RETRY
          if (OB_NEED_RETRY == err)
          {
            int64_t log_not_served_interval = yysys::CTimeUtil::getTime() - last_log_fetched_time;
            ...
            {
              YYSYS_LOG(ERROR, "fetch log (checkpoint=%ld) timeout, timeout=%ld/%ld, change_ups_num=%ld, ups_num=%ld",
            ...
            {
              YYSYS_LOG(INFO, "fetch log fail from ALL UPS, force to wait %ld usec, ups_num=%ld, change_ups_num=%ld",
          ...
          {
            YYSYS_LOG(INFO, "current checkpoint is %ld", req.start_id_);
          }
        ...
        YYSYS_LOG(INFO, "log fetcher exit: stop=%s, ob_log_alive=%s, checkpoint=%ld, asked_min_checkpoint=%ld, err=%d",
        ...
        {
          YYSYS_LOG(INFO, "abort in fetcher");
          abort();
        }
      }
    }
        ...
        updateserver::ObFetchedLog &res)
    {
      ...
      do
      {
        ...
        {
          YYSYS_LOG(WARN, "fetch log from UPS(%s) fail: DATA_NOT_SERVE, checkpoint=%ld, retry_times=%ld",
        ...
        YYSYS_LOG(DEBUG, "LOG (checkpoint=%ld) not served on UPS(%s)", req.start_id_, to_cstring(ups_addr));
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "fetch log from UPS(%s) fail: checkpoint=%ld, result code = %d",
        ...
        else
        {
          ...
          YYSYS_LOG(DEBUG, "fetch done %s from %s, req='%s'", to_cstring(res), to_cstring(ups_addr), to_cstring(req));
        }
      }
        ...
        ObResultCode &rc)
    {
      ...
      while (ob_log_alive(_stop) && 0 < req.start_id_)
      {
        ...
        if (0 >= --req.start_id_)
        {
          YYSYS_LOG(ERROR, "fetch log from UPS(%s) fail: no aligned log between ID [%ld, %ld]",
      ...
      {
        YYSYS_LOG(WARN, "fetch log from UPS(%s) fail, checkpoint=%ld, req='%s', ret=%d",
      ...
      {
        YYSYS_LOG(WARN, "log (checkpoint=%ld) not aligned. forward range [%ld, %ld], UPS=%s",
    ...
    int ObLogFetcher::init_mutator_list_(const int64_t max_mutator_num)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "init mutator p_list fail, max_mutator_num=%ld", max_mutator_num);
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "init mutator v_list fail, max_mutator_num=%ld", max_mutator_num);
        }
      }
      for (int64_t i = 0; OB_SUCCESS == ret && i < max_mutator_num; i++)
      {
        ...
        if (NULL == buffer)
        {
          YYSYS_LOG(WARN, "allocator memory to build log_mutator fail, i=%ld", i);
        ...
        if (NULL == log_mutator)
        {
          YYSYS_LOG(WARN, "build log_mutator fail, i=%ld", i);
          ret = OB_ERR_UNEXPECTED;
          break;
        }
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "push mutator to list fail, i=%ld", i);
          break;
        }
      }
      return ret;
    ...
    void ObLogFetcher::destroy_mutator_list_()
    {
      ...
      if (0 != counter)
      {
        YYSYS_LOG(WARN, "still have mutator not release, counter=%ld", counter);
      }
    }
    ...
    int ObLogFetcher::handle_fetched_log_(updateserver::ObFetchedLog &log, int64_t &max_fetched_log_id)
    {
      ...
      if (0 == log.data_len_)
      {
        YYSYS_LOG(WARN, "fetch empty log: fetched_log='%s', wait %ld usec and retry",
      ...
      else
      {
        ...
        while (OB_SUCCESS == ret && ob_log_alive(_stop) && pos < log.data_len_)
        {
          ...
          {
            YYSYS_LOG(ERROR, "deserialize log_entry fail, ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = log_entry.check_header_integrity(false)))
          {
            YYSYS_LOG(ERROR, "check_header_integrity fail, ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = log_entry.check_data_integrity(log.log_data_ + pos, false)))
          {
            YYSYS_LOG(ERROR, "check_data_integrity fail, ret=%d", ret);
          }
          ...
          else if (OB_LOG_UPS_MUTATOR == log_entry.cmd_)
          {
            if (OB_SUCCESS != (ret = construct_mutator_(log_entry, log, pos)))
            {
              ...
              {
                YYSYS_LOG(ERROR, "construct_mutator_ fails. ret=%d", ret);
              }
            }
          }
        ...
        {
          YYSYS_LOG(WARN, "filter not aligned log [%ld, %ld], asked_min_checkpoint=%ld, "
        ...
        {
          YYSYS_LOG(ERROR, "handle_fetched_log_ fail, ret=%d, fetched_log='%s', "
    ...
    int ObLogFetcher::construct_mutator_(ObLogEntry &log_entry, updateserver::ObFetchedLog &log, const int64_t pos)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "deserialize log_mutator fail, ret=%d", ret);
        }
        else
        {
    ...
    void ObLogFetcher::set_mutator_min_process_timestamp_(ObLogMutator &mutator)
    {
      ...
      YYSYS_LOG(DEBUG, "MUTATOR INFO: TM=%ld, MIN_PROCESS_TM=%ld, SYS_TM=%ld, DELTA=%ld",
    ...
    int ObLogFetcher::get_max_checkpoint_(int64_t &max_checkpoint)
    {
      ...
      while (++index <= server_selector_->get_ups_num())
      {
        ...
        {
          YYSYS_LOG(WARN, "get ups addr failed, index=%ld, ret=%d", index, ret);
        }
        else if (OB_SUCCESS != (ret = rpc_stub_->get_max_log_seq(ups_addr, GET_MAX_LOG_SEQ_TIMEOUT, cp)))
        {
          YYSYS_LOG(WARN, "failed to get max checkpoint from UPS '%s', index=%ld, ret=%d",
              to_cstring(ups_addr), index, ret);
        }
        else
        {
          YYSYS_LOG(INFO, "UPS(%s) --> MAX CHECKPOINT(%ld)", to_cstring(ups_addr), cp);
      ...
      YYSYS_LOG(INFO, "max_checkpoint=%ld, fail_ups_num=%ld, available ups num=%ld, ret=%d",
