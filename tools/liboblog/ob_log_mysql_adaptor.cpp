...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    int ObLogMysqlAdaptor::TableSchemaContainer::init(const ObLogSchema *schema, uint64_t table_id)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_table_schema fail, table_id=%lu", table_id);
                                             ...
                                             allocator_()
    {
      ...
      {
        YYSYS_LOG(ERROR, "initialize FIFOAllocator fail, ret=%d, total_limit=%ld, "
        ...
        const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "GET_TSI_MULT get ObLogMysqlAdaptor fail, return NULL");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (! tmp_mysql_adaptor->is_inited())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "initialize MySQL Adaptor with schema (%ld,%ld) fail, ret=%d",
                total_schema->get_version(), total_schema->get_timestamp(), ret);
          }
          else
          {
            YYSYS_LOG(ERROR, "failed to initialize ObLogMysqlAdaptor, ret=%d", ret);
          }
    ...
    int ObLogMysqlAdaptor::init(const ObLogConfig &config, const ObLogSchema *total_schema)
    {
      ...
      if (inited_)
      {
        ret = OB_INIT_TWICE;
              ...
              || NULL == total_schema)
      {
        YYSYS_LOG(ERROR, "invalid configuration or arguments: mysql_addr=%s, mysql_port=%d "
      ...
      {
        YYSYS_LOG(ERROR, "init_ob_mysql_ fail, ret=%d", ret);
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(INFO, "Initialize MySQL Adaptor based on schema version=%ld, timestamp=%ld",
            total_schema->get_version(), total_schema->get_timestamp());

        // Create tc_info_map_
        if (0 != tc_info_map_.create(TC_INFO_MAP_SIZE))
        {
          YYSYS_LOG(WARN, "tc_info_map_ create fail, size=%ld", TC_INFO_MAP_SIZE);
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else if (OB_SUCCESS != (ret = ObLogConfig::parse_tb_select(config.get_tb_select(), *this, total_schema)))
        {
          YYSYS_LOG(WARN, "parse table selected fail with schema (%ld,%ld), ret=%d",
              total_schema->get_version(), total_schema->get_timestamp(), ret);
        }
        else
        {
    ...
    void ObLogMysqlAdaptor::destroy()
    {
      ...
      ob_mysql_thread_end();
      YYSYS_LOG(INFO, "Thread %lu: mysql adaptor calls mysql_thread_end", pthread_self());
    }
    ...
    int ObLogMysqlAdaptor::operator ()(const char *tb_name, const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument: tb_name=%s, total_schema=%p",
            NULL == tb_name ? "NULL" : tb_name, total_schema);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (table_schema = total_schema->get_table_schema(tb_name)))
      {
        YYSYS_LOG(WARN, "get_table_schema fail, tb_name=%s", tb_name);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = add_tc_info_(table_schema->get_table_id(), total_schema)))
      {
        if (OB_SCHEMA_ERROR != ret)
        {
          YYSYS_LOG(ERROR, "add_tc_info_ fail, tb_name=%s, table_id=%lu, total_schema=%p, ret=%d",
        ...
        const IObLogColValue *&list)
    {
      ...
      {
        ret = OB_INVALID_ARGUMENT;
        YYSYS_LOG(ERROR, "invalid arguments, table_id=%lu, asked_schema=%p", table_id, asked_schema);
      }
      else if (OB_INVALID_VERSION == schema_version_ || asked_schema->get_version() > schema_version_)
      {
        ...
        if (OB_SUCCESS != (ret = update_all_tc_info_(asked_schema)))
        {
          if (OB_SCHEMA_ERROR != ret)
          {
            YYSYS_LOG(ERROR, "update_all_tc_info_ fail, ret=%d", ret);
          }
      ...
      {
        YYSYS_LOG(ERROR, "invalid arguments: asked schema version %ld is less than local schema version %ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "query_data_ fail, table_id=%lu, rowkey='%s', ret=%d", table_id, to_cstring(rowkey), ret);
        }
      }

      if (OB_SCHEMA_ERROR == ret)
      {
        YYSYS_LOG(DEBUG, "query data fail with schema version=%ld, timestamp=%ld. table_id=%lu, "
        ...
        const IObLogColValue *&list)
    {
      ...
      while (MAX_MYSQL_FAIL_RETRY_TIMES >= ++retry_times)
      {
        ...
        {
          YYSYS_LOG(ERROR, "get_tc_info fail, table_id=%lu", table_id);
          break;
        }
        ...
        {
          YYSYS_LOG(ERROR, "execute_query_ fail, table_id=%lu, re-prepare retry times=%d/%d",
        ...
        if (OB_SUCCESS != (ret = update_all_tc_info_(asked_schema)))
        {
          if (OB_SCHEMA_ERROR != ret)
          {
            YYSYS_LOG(ERROR, "fail to re-prepare, update_all_tc_info_ fail, ret=%d", ret);
          }
      ...
      if (OB_SUCCESS == ret)
      {
        if (1 < retry_times)
        {
          YYSYS_LOG(WARN, "execute_query_ success after retry times=%d/%d", retry_times, MAX_MYSQL_FAIL_RETRY_TIMES);
        }
      ...
      {
        YYSYS_LOG(WARN, "execute_query_ fail, retry times=%d/%d, table_id=%lu, rowkey='%s', ret=%d",
    ...
    int ObLogMysqlAdaptor::add_tc_info_(uint64_t table_id, const ObLogSchema *total_schema)
    {
      ...
      if (OB_SUCCESS != (ret = construct_tc_info_(tc_info, table_id, total_schema)))
      {
        if (OB_SCHEMA_ERROR != ret)
        {
          YYSYS_LOG(ERROR, "construct_tc_info_ fail, table_id=%lu, tc_info=%p, total_schema=%p, ret=%d",
      ...
      else if (hash::HASH_INSERT_SUCC != (hash_ret = tc_info_map_.set(table_id, tc_info)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "set tc_info_map_ fail, table_id=%lu, tc_info=%p, hash_ret=%d",
      ...
      {
        YYSYS_LOG(DEBUG, "MYSQL ADAPTOR: ADD TABLE=%lu TCInfo", table_id);
      }
    ...
    int ObLogMysqlAdaptor::update_all_tc_info_(const ObLogSchema *total_schema)
    {
      ...
      YYSYS_LOG(INFO, "MYSQL ADAPTOR: UPDATE ALL TCInfo, TABLE COUNT=%ld, SCHEMA=%ld",
      ...
      {
        YYSYS_LOG(INFO, "MYSQL ADAPTOR: RE-INIT ObSQLMySQL HANDLE SUCCESS");
        ...
        for (int64_t index = 0; OB_SUCCESS == ret && index < table_id_array_->count(); index++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get table ID from table_id_array_ fail, index=%ld, count=%ld, ret=%d",
          ...
          else if (OB_SUCCESS != (ret = add_tc_info_(table_id, total_schema)))
          {
            if (OB_SCHEMA_ERROR != ret)
            {
              YYSYS_LOG(WARN, "add_tc_info_ fail, table_id=%lu, ret=%d", table_id, ret);
            }
    ...
    int ObLogMysqlAdaptor::get_tc_info_(const uint64_t table_id, TCInfo *&tc_info)
    {
      ...
      if (hash::HASH_NOT_EXIST == hash_ret)
      {
        ...
        YYSYS_LOG(ERROR, "table ID=%lu does not exist", table_id);
        ret = OB_NOT_SUPPORTED;
      }
      else if (-1 == hash_ret || NULL == tmp_tc_info)
      {
        YYSYS_LOG(ERROR, "get TCInfo from tc_info_map_ fail, table_id=%lu, tc_info=%p, hash_ret=%d",
    ...
    int ObLogMysqlAdaptor::set_mysql_stmt_params_(TCInfo *tc_info, const common::ObRowkey &rowkey)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid data: rowkey column number not match, rowkey number in mutator=%ld, "
      ...
      else
      {
        for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey.get_obj_cnt(); i++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "unknown rowkey column type=%s,%d, not support",
    ...
    int ObLogMysqlAdaptor::execute_query_(TCInfo *tc_info, const common::ObRowkey &rowkey)
    {
      ...
      {
        YYSYS_LOG(ERROR, "set_mysql_stmt_params_ fail, rowkey='%s', ps sql='%s', ret=%d",
            to_cstring(rowkey), tc_info->sql, ret);
      }
      else if (0 != ob_mysql_stmt_bind_param(tc_info->stmt, tc_info->params))
      {
        YYSYS_LOG(WARN, "mysql_stmt_bind_param fail, errno=%u, %s",
      ...
      {
        YYSYS_LOG(WARN, "mysql_stmt_execute fail, errno=%u, %s",
            ob_mysql_stmt_errno(tc_info->stmt), ob_mysql_stmt_error(tc_info->stmt));
      ...
      {
        YYSYS_LOG(WARN, "mysql_stmt_bind_result fail, errno=%u, %s",
            ob_mysql_stmt_errno(tc_info->stmt), ob_mysql_stmt_error(tc_info->stmt));
      ...
      else if (0 != (mysql_ret = ob_mysql_stmt_fetch(tc_info->stmt)))
      {
        ...
        {
          ret = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "invalid data: query back results are truncated. rowkey='%s', ps sql='%s'",
              to_cstring(rowkey), tc_info->sql);

          YYSYS_LOG(INFO, "************** TRUNCATED DATA INFORMATION BEGIN **************");
          YYSYS_LOG(INFO, "ROWKEY='%s'", to_cstring(rowkey));
          YYSYS_LOG(INFO, "SQL='%s'", tc_info->sql);
          int64_t index = 1;
          ...
          while (NULL != res)
          {
            ...
            YYSYS_LOG(INFO, "COLUMN[%ld]: BUF_LEN=%ld, DATA_LEN=%ld, DATA%s%s",
          ...
          YYSYS_LOG(INFO, "************** TRUNCATED DATA INFORMATION END **************");
        }
        else
        {
          YYSYS_LOG(WARN, "mysql_stmt_fetch fail, mysql_ret=%d, errno=%u, %s",
    ...
    int ObLogMysqlAdaptor::construct_tc_info_(TCInfo *&tc_info, uint64_t table_id, const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "alloc TCInfo fail");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (OB_SUCCESS != (ret = tsc.init(total_schema, table_id)))
      {
        YYSYS_LOG(ERROR, "set TableSchemaContainer fail, table_id=%lu, ret=%d", table_id, ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != (ret = init_ps_sql_(tmp_tc_info->sql,
                ...
                tsc)))
        {
          YYSYS_LOG(ERROR, "init_ps_sql_ fail, table_id=%lu, ret=%d", table_id, ret);
        }
                ...
                static_cast<int64_t>(strlen(tmp_tc_info->sql)))))
        {
          if (OB_SCHEMA_ERROR != ret)
          {
            YYSYS_LOG(ERROR, "init_mysql_stmt_ fail, sql=%s, ret=%d", tmp_tc_info->sql, ret);
          }
          tmp_tc_info->stmt = NULL;
                ...
                tsc)))
        {
          YYSYS_LOG(ERROR, "init_mysql_params_ fail, ret=%d", ret);
          ...
          tmp_tc_info->params_is_null = NULL;
                ...
                tsc)))
        {
          YYSYS_LOG(ERROR, "init_mysql_result_ fail, ret=%d", ret);
    ...
    int ObLogMysqlAdaptor::init_ps_sql_(char *sql, int64_t size, const TableSchemaContainer &tsc)
    {
      ...
      YYSYS_LOG(INFO, "build sql succ, table_id=%lu [%s]", table_id, sql);
    ...
    int ObLogMysqlAdaptor::init_mysql_stmt_(MYSQL_STMT *&stmt, const char *sql, int64_t sql_len)
    {
      ...
      {
        YYSYS_LOG(WARN, "ob_mysql_stmt_init_ fail, errno=%u, %s",
      ...
      {
        YYSYS_LOG(WARN, "ob_mysql_stmt_prepare fail, errno=%u, %s",
        ...
        const TableSchemaContainer &tsc)
    {
      ...
      else
      {
        ...
        for (int64_t i = 0; OB_SUCCESS == ret && i < rk_num; i++)
        {
          ...
          {
            YYSYS_LOG(WARN, "obmysql::ObMySQLUtil::get_mysql_type():  "
        ...
        const TableSchemaContainer &tsc)
    {
      ...
      {
        YYSYS_LOG(WARN, "alloc res_idx fail, size=%d", column_num);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
        ...
        for (int32_t i = column_num - 1; i >= 0; i--)
        {
          ...
          {
            YYSYS_LOG(ERROR, "unknown type: %s,%d, not supported",
                  ...
                  length)))
          {
            ...
            YYSYS_LOG(DEBUG, "init cv list, buffer_type=%d buffer_length=%ld buffer=%p length=%p is_null=%p",
    ...
    {
      YYSYS_LOG(DEBUG, "MYSQL ADAPTOR: destruct_all_tc_info");
    ...
    int ObLogMysqlAdaptor::init_ob_mysql_()
    {
      ...
      YYSYS_LOG(DEBUG, "MYSQL ADAPTOR: init_ob_mysql");

      if (NULL == (ob_mysql_ = ob_mysql_init(NULL)))
      {
        YYSYS_LOG(WARN, "ob_mysql_init fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        ...
        while (MAX_MYSQL_FAIL_RETRY_TIMES >= ++retry_times)
        {
          ...
          if (MAX_MYSQL_FAIL_RETRY_TIMES <= retry_times)
          {
            YYSYS_LOG(WARN, "ob_mysql_real_connect fail, retry_times=%d/%d, "
        ...
        {
          YYSYS_LOG(WARN, "ob_mysql_real_connect success after retry %d/%d times",
              retry_times, MAX_MYSQL_FAIL_RETRY_TIMES);
        }
      }
    ...
    {
      YYSYS_LOG(DEBUG, "MYSQL ADAPTOR: destroy_ob_mysql");
