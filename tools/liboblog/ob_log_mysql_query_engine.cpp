...
namespace oceanbase
{
  ...
  namespace liboblog
  {
        ...
        IObLogFilter *log_filter)
    {
      ...
      {
        YYSYS_LOG(ERROR, "initialize FIFOAllocator fail, ret=%d, total_limit=%ld, hold_limit=%ld, page_size=%ld",
      ...
      {
        YYSYS_LOG(ERROR, "invalid query back thread count %d which should be in (0, %d]",
    ...
    int ObLogMysqlQueryEngine::start()
    {
      ...
      else if (stop_flag_)
      {
        ...
        for (int64_t index = 0; index < config_->get_query_back_thread_num(); index++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "job_queue_.init fail, size=%ld, thread_index=%ld, ret=%d",
          ...
          if (0 != tmp_ret)
          {
            ...
            YYSYS_LOG(WARN, "create %ldth mysql query thread fail, ret=%d", index, tmp_ret);
            break;
          }
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "start %d mysql query threads success", thread_num_);
        }
        else
        {
          stop();
    ...
    {
      YYSYS_LOG(INFO, "stop mysql query engine begin, thread_num=%d, stop_flag=%s",
      ...
      for (int64_t index = 0; index < thread_num_; index++)
      {
        ...
        YYSYS_LOG(INFO, "mysql query thread %ld stoped", index);
      }
      YYSYS_LOG(INFO, "stop mysql query engine end");
    }
    ...
    void ObLogMysqlQueryEngine::run_query(ThreadInfo *ti)
    {
      ...
      if (NULL == ti)
      {
        YYSYS_LOG(ERROR, "invalid argument, thread_info=%p", ti);
        err = OB_INVALID_ARGUMENT;
      }
      else if (!inited_)
      {
        YYSYS_LOG(ERROR, "query thread %ld start fail: mysql query engine has not been initialized",
            ti->thread_index_);
        err = OB_NOT_INIT;
      }
      ...
      else
      {
        ...
        while (ob_log_alive(stop_flag_))
        {
          ...
          if (OB_SUCCESS != (err = ti->job_queue_.pop(query_job)))
          {
            ...
            {
              YYSYS_LOG(ERROR, "job_queue_.pop fail, ret=%d", err);
            }
          }
      ...
      YYSYS_LOG(INFO, "mysql query thread %ld exit, err=%d, stop=%s, ob_log_alive=%s",
    ...
    void ObLogMysqlQueryEngine::handle_query_job_(const MysqlQueryJob *query_job, ThreadInfo *ti)
    {
      ...
      if (OB_SUCCESS != (err = ObLogMysqlAdaptor::get_tsi_mysql_adaptor(mysql_adaptor, *config_, schema)))
      {
        ...
        {
          YYSYS_LOG(WARN, "get_tsi_mysql_adaptor fail with schema(%ld,%ld). It should be updated. ret=%d",
              schema->get_version(), schema->get_timestamp(), err);
        }
        else
        {
          YYSYS_LOG(ERROR, "get_tsi_mysql_adaptor fail, ret=%d, schema=%p", err, schema);
        }
      }
          ...
          || NULL == col_value)
      {
        ...
        {
          YYSYS_LOG(WARN, "query table (ID=%lu) data fail with schema (%ld,%ld). rowkey=%s, schema should be updated",
              table_id, schema->get_version(), schema->get_timestamp(), to_cstring(*rowkey));
        }
        else if (OB_ENTRY_NOT_EXIST != err)
        {
          YYSYS_LOG(ERROR, "query_whole_row fail, col_value=%p, query_job='%s', err=%d",
      ...
      int tmp_ret = OB_SUCCESS;
			int64_t query_interval = yysys::CTimeUtil::getTime() - start_time;
      if (OB_SUCCESS != (tmp_ret = query_result->fill_query_value(query_job->row_index_, col_value, err, query_interval)))
      {
        YYSYS_LOG(ERROR, "fill_query_value fail, ret=%d, col_value=%p, err=%d, query_result='%s', query_job='%s'",
            tmp_ret, col_value, err, to_cstring(*query_result), to_cstring(*query_job));
      }

      YYSYS_LOG(DEBUG, "QUERY_BACK: THREAD %ld: ROW[%ld], table=%lu, ITVL=%ld, queue.size=%ld, err=%d",
        ...
        ObLogMysqlQueryResult &query_result)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, schema=%p", schema);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (0 >= thread_num_)
      {
        YYSYS_LOG(ERROR, "no valid query back thread started, thread_num=%d, stop_flag=%s, ob_log_alive=%s",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "query_all_rows_ fail, err=%d, mutator=(CP=%lu,TM=%ld), schema=%p, query_result='%s'",
        ...
        if (OB_SUCCESS != (wait_err = wait_for_query_result_ready_(query_result, stop_flag)))
        {
          ...
          {
            YYSYS_LOG(ERROR, "wait_for_query_result_ready_ fail, err=%d, query_result='%s'",
        ...
        {
          YYSYS_LOG(WARN, "error occur during quering back mutator(CP=%lu,TM=%ld), "
        ...
        YYSYS_LOG(DEBUG, "QUERY_BACK: MUTATOR TM=%ld, ITVL=%ld, QUERY_ROWS=%ld/%ld, "
      ...
      if (REACH_TIME_INTERVAL(PRINT_QUERY_BACK_STAT_INFO_INTERVAL))
      {
        ...
        YYSYS_LOG(INFO, "STAT: [QUERY_BACK] TRANS_QUERY_TIME=%ld STMT_QUERY_TIME=%ld STMTS_PER_TRANS=%ld  NEED_QUERY_STMTS=%ld",
        ...
        ObLogMysqlQueryResult &query_result)
    {
          ...
          && OB_SUCCESS == (ret = mutator.get_mutator().next_cell()))
      {
        if (OB_SUCCESS != (ret = mutator.get_mutator_cell(&cell, &irc, &irf, &dml_type))
            || NULL == cell)
        {
          YYSYS_LOG(ERROR, "get_mutator_cell fail, ret=%d, cell=%p", ret, cell);
        ...
        {
          YYSYS_LOG(ERROR, "query_row_ fail, ret=%d, row_index=%ld, cell='%s', dml_type=%d, schema=%p",
        ...
        ObLogMysqlQueryResult &query_result)
    {
      ...
      if (OB_SUCCESS != (ret = query_result.add_row(cell->cell_info.table_id_,
              ...
              need_query)))
      {
        YYSYS_LOG(ERROR, "add_row into query result fail, rowkey='%s', dml_type=%d, ret=%d",
      ...
      else if (need_query)
      {
        ...
        if (NULL == query_job)
        {
          YYSYS_LOG(ERROR, "allocate memory for MysqlQueryJob fail, size=%ld", sizeof(MysqlQueryJob));
        ...
        else
        {
          ...
          YYSYS_LOG(DEBUG, "QUERY_BACK: push row{%s, %ld} into thread %ld queue(size=%ld)",
          ...
          {
            YYSYS_LOG(ERROR, "push query job into job_queue fail, ret=%d, size=%ld, query_job='%s'",
      ...
      YYSYS_LOG(DEBUG, "QUERY_BACK: ROW[%ld]='%s', table=%lu, need_query=%s, ret=%d",
        ...
        volatile bool &stop_flag)
    {
      ...
      while (ob_log_alive(stop_flag))
      {
        ...
        if (OB_PROCESS_TIMEOUT != ret)
        {
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "wait for query result ready fail, ret=%d, result='%s'",
        ...
        {
          YYSYS_LOG(WARN, "wait for query result ready timeout(%ld), query_result='%s'",
    ...
    void ObLogMysqlQueryEngine::clear_job_queue_()
    {
      ...
      {
        YYSYS_LOG(INFO, "clear mysql query engine job queue: still %d query jobs not handled", job_queue_.size());
