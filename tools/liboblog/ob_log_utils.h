    ...
    class ObLogClock
    {
      public:
        ObLogClock(const int64_t timeout_us, const char *func_name) : timeout_us_(timeout_us),
                                                                      ...
                                                                      timeu_(yysys::CTimeUtil::getTime())
        {
        };
        ~ObLogClock()
        {
          timeu_ = yysys::CTimeUtil::getTime() - timeu_;
          if (timeout_us_ < timeu_)
          {
            YYSYS_LOG(WARN, "call [%s] long, timeu=%ld", func_name_, timeu_);
          }
        };
      private:
    ...
    class ObLogSeqMap
    {
      ...
      public:
        int init(const uint64_t limit)
        {
          ...
          {
            YYSYS_LOG(INFO, "init seq_map succ, limit=%lu", limit);
