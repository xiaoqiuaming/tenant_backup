...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogSchemaGetter::init(IObLogServerSelector *server_selector, IObLogRpcStub *rpc_stub, ObLogConfig &config)
    {
      ...
      {
        YYSYS_LOG(ERROR, "initialize schema allocator fail, ret=%d, total_limit=%ld, "
      ...
      {
        YYSYS_LOG(ERROR, "fetch_schema_ fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = config.refresh_tb_select(tmp_schema)))
      {
        YYSYS_LOG(ERROR, "refresh_tb_select fail, tmp_schema=%p, schema version=%ld, ret=%d",
    ...
    int ObLogSchemaGetter::get_schema_newer_than(const ObLogSchema *old_schema, const ObLogSchema *&new_schema)
    {
      ...
      if (OB_SUCCESS == ret && need_refresh_schema)
      {
        ...
        // at the same time
        yysys::CThreadGuard guard(&multi_refresh_mutex_);
        ...
        {
          YYSYS_LOG(INFO, "force to refresh schema: current schema=(%ld,%ld), asked schema=(%ld,%ld)",
          ...
          {
            YYSYS_LOG(ERROR, "force_refresh fail, ret=%d", ret);
          }
          ...
          {
            YYSYS_LOG(ERROR, "refreshed schema (%ld,%ld) is not newer than old schema (%ld,%ld)",
                new_schema->get_version(), new_schema->get_timestamp(),
                old_schema->get_version(), old_schema->get_timestamp());
      ...
      {
        YYSYS_LOG(ERROR, "unexcepted error: fail to get newer schema");
        ret = OB_ERR_UNEXPECTED;
      }
    ...
    int ObLogSchemaGetter::refresh_schema_greater_than(int64_t base_version)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument: base_version=%ld", base_version);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (! inited_)
      {
        YYSYS_LOG(ERROR, "schema getter has not been initialized");
        ret = OB_NOT_INIT;
      }
      else if (get_current_schema_version_() <= base_version)
      {
        ...
        if (get_current_schema_version_() <= base_version)
        {
          ...
          {
            YYSYS_LOG(ERROR, "refresh schema greater than version=%ld fail, current version=%ld, ret=%d",
    ...
    int ObLogSchemaGetter::refresh_(int64_t base_version)
    {
      ...
      while (OB_SUCCESS == ret)
      {
        ...
        refresh_interval = yysys::CTimeUtil::getTime() - start_time;

        if (SCHEMA_REFRESH_FAIL_RETRY_TIMEOUT <= refresh_interval)
        {
          YYSYS_LOG(ERROR, "refresh schema to (version > %ld) fail, TIMEOUT, refresh_interval=%ld/%ld",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(ERROR, "refresh schema fail, refreshed schema (%ld,%ld) is not newer than "
        ...
        {
          YYSYS_LOG(INFO, "update schema from (%ld,%ld) to (%ld,%ld), base_version=%ld, refresh_interval=%ld",
              cur_schema_->get_version(), cur_schema_->get_timestamp(),
              next_schema->get_version(), next_schema->get_timestamp(),
    ...
    int ObLogSchemaGetter::fetch_schema_(ObLogSchema *&schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "alloc_schema_ fail, ret=%d", ret);
      }
      else
      {
        while (MAX_SCHEMA_FETCH_FAIL_RETRY_TIMES >= ++retry_times)
        {
          ...
          {
            YYSYS_LOG(WARN, "get rs addr failed, ret=%d", ret);
          }
          else if (OB_SUCCESS != (ret = rpc_stub_->fetch_schema(rs_addr, *tmp_schema, FETCH_SCHEMA_TIMEOUT)))
          {
            YYSYS_LOG(WARN, "fetch schema fail ret=%d", ret);
          }
          ...
          {
            YYSYS_LOG(ERROR, "fetch schema fail, retry_times=%ld/%ld, ret=%d",
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (1 < retry_times)
        {
          YYSYS_LOG(WARN, "fetch schema success after retry for times=%ld/%ld",
    ...
    int ObLogSchemaGetter::alloc_schema_(ObLogSchema *&schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "allocate memory form ObLogSchema fail");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else if (NULL == (tmp_schema = new(buffer) ObLogSchema()))
      {
        YYSYS_LOG(WARN, "construct schema fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
