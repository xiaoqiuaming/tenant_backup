...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogRouter::init(ObLogConfig &config, IObLogFilter *source, IObLogSchemaGetter *schema_getter)
    {
      ...
      {
        YYSYS_LOG(ERROR, "initialize heartbeat mutator allocator fail, ret=%d", ret);
      }
      else if (NULL == (source_ = source)
              || NULL == (schema_getter_ = schema_getter)
              || NULL == (config_ = &config))
      {
        YYSYS_LOG(WARN, "invalid param source=%p schema_getter=%p, config=%p", source, schema_getter, &config);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (0 >= (formater_thread_num_ = config.get_router_thread_num())
          || formater_thread_num_ > MAX_LOG_FORMATER_THREAD_NUM)
      {
        YYSYS_LOG(ERROR, "invalid router_thread_num %d, which should be in [1, %d]",
            formater_thread_num_, MAX_LOG_FORMATER_THREAD_NUM);
      ...
      {
        YYSYS_LOG(ERROR, "failed to initialize ObLogPerf, ret=%d", ret);
      }
      else if (0 != table_record_map_.create(DEFAULT_MAX_TABLE_NUM))
      {
        YYSYS_LOG(WARN, "init observer_set fail");
      ...
      {
        YYSYS_LOG(WARN, "init observer_map fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
    ...
    int ObLogRouter::register_observer(const uint64_t partition, IObLogObserver *observer)
    {
      ...
      {
        YYSYS_LOG(WARN, "thread has been launched, can not register observer partition=%lu observer=%p",
            partition, observer);
        ret = OB_NOT_SUPPORTED;
      }
      else if (OB_SUCCESS != (ret = log_perf_.register_db_partition(partition)))
      {
        YYSYS_LOG(ERROR, "ObLogPerf::register_db_partition() fails, partition=%lu, ret=%d",
            partition, ret);
      }
      else if (-1 == (hash_ret = observer_set_.set(reinterpret_cast<uint64_t>(observer))))
      {
        YYSYS_LOG(WARN, "add observer into observer_set_ fail, hash_ret=%d observer=%p",
            hash_ret, observer);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (hash::HASH_INSERT_SUCC != (hash_ret = observer_map_.set(partition, observer)))
      {
        YYSYS_LOG(WARN, "add observer fail, hash_ret=%d partition=%lu observer=%p",
    ...
    int ObLogRouter::launch()
    {
      ...
      {
        YYSYS_LOG(WARN, "no observer cannot launch");
        ret = OB_ENTRY_NOT_EXIST;
      }
      else if (launched_)
      {
        YYSYS_LOG(WARN, "thread has already been launched");
        ret = OB_INIT_TWICE;
      }
      // Start Perf
      else if (OB_SUCCESS != (ret = log_perf_.start()))
      {
        YYSYS_LOG(ERROR, "ObLogPerf::start() fails, ret=%d", ret);
      }
      ...
      {
        YYSYS_LOG(WARN, "init queue_thread fail, ret=%d thread_num=%d", ret, formater_thread_num_);
      }
      // Start router thread
      else if (1 != start())
      {
        YYSYS_LOG(WARN, "start thread to route log fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
    ...
    void ObLogRouter::stop()
    {
      if (launched_)
      {
        YYSYS_LOG(INFO, "stop log router begin");
        ...
        // Stop Perf
        log_perf_.stop();
        YYSYS_LOG(INFO, "stop log router end");
      }
    }

    void ObLogRouter::run(yysys::CThread* thread, void* arg)
    {
      UNUSED(thread);
      UNUSED(arg);
      ...
      {
        YYSYS_LOG(ERROR, "get_log_paritioner fail, log_partitioner=%p, ret=%d", log_partitioner, tmp_ret);
        abort();
      }
      ...
      while (!_stop)
      {
        ...
        if (REACH_TIME_INTERVAL(PERF_PRINT_INTERVAL))
        {
          ...
          {
            YYSYS_LOG(WARN, "ObLogPerf::set_total_trans_count fails, ret=%d", perf_ret);
          }
        ...
        else
        {
          ...
          if (NULL != mutator)
          {
            ...
            {
              YYSYS_LOG(INFO, "DDL operations appear, force refresh schema, mutator timestamp=%ld",
              ...
              {
                YYSYS_LOG(ERROR, "refresh_schema_greater_than() fail, DDL mutator timestamp=%ld, ret=%d",
        ...
        {
          YYSYS_LOG(WARN, "router skip dirty data, last mutator timestamp=%ld",
        ...
        {
          YYSYS_LOG(ERROR, "router aborted, last mutator timestamp=%ld, ret=%d",
    ...
    int ObLogRouter::construct_heartbeat_mutator_(ObLogMutator *&mutator, int64_t ht_tm, int64_t ht_sgnt)
    {
      ...
      if (NULL == buffer)
      {
        YYSYS_LOG(ERROR, "allocate heartbeat mutator fail, allocator hold=%ld, allocated=%ld",
      ...
      {
        YYSYS_LOG(ERROR, "new ObLogMutator fail, buffer=%p", buffer);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
    ...
    int ObLogRouter::route_heartbeat_mutator_()
    {
      ...
      int64_t heartbeat_signature = yysys::CTimeUtil::getTime();
      ...
      for (int index = 0; OB_SUCCESS == ret && index < formater_thread_num; index++)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            // unexpected error
            YYSYS_LOG(WARN, "push queue fail, queue index=%d, ret=%d", index, ret);
            ret = OB_ERR_UNEXPECTED;
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "construct_heartbeat_mutator_ fail, index=%d, ret=%d", index, ret);
          break;
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        YYSYS_LOG(DEBUG, "route HEARTBEAT (TM=%ld, SGNT=%ld) to %ld formators",
        ...
        ObLogMutator *mutator, uint64_t *db_partition)
    {
            ...
            && OB_SUCCESS == (ret = mutator->get_mutator().next_cell()))
      {
        ...
        {
          YYSYS_LOG(ERROR, "unexpected error, table (ID=%ld) contains in Filter but no in Partitioner, ret=%d",
              cell->cell_info.table_id_, ret);
          break;
        }
        else if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "ObLogPartitioner::partition() fails, ret=%d, cellinfo='%s'",
        ...
        {
          YYSYS_LOG(ERROR, "table_record_map_ get fails, hash_ret=%d", hash_ret);
          ret = OB_ERR_UNEXPECTED;
          break;
        }
        else
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(ERROR, "table_record_map_ set fails, hash_ret=%d", hash_ret);
          ret = OB_ERR_UNEXPECTED;
          break;
        }
      } // end of while
      ...
      if (OB_ITER_END == ret)
      {
        if (is_local_db_partition_valid)
        {
          ...
          *db_partition = local_db_partition;
          ...
          {
            YYSYS_LOG(WARN, "ObLogPerf::add_db_perf fails, db_partition=%lu, ret=%d",
          ...
          for (TableRecordMap::iterator iter = table_record_map_.begin(); iter != table_record_map_.end(); iter++)
          {
            ...
            {
              YYSYS_LOG(WARN, "ObLogPerf::add_tb_perf fails, table_id=%lu, ret=%d", iter->first, tmp_ret);
            }
          }
        ...
        else
        {
          ...
          YYSYS_LOG(ERROR, "Mutator does not contain selected tables");
          ret = OB_ERR_UNEXPECTED;
        }
      }
    ...
    int ObLogRouter::route_dml_mutator_(ObLogMutator *mutator, IObLogPartitioner *log_partitioner)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "partition mutator fail, ret=%d", ret);
      }
      else
      {
        ...
        if (hash::HASH_EXIST == hash_ret)
        {
          ...
          {
            // unexpected error
            YYSYS_LOG(WARN, "get observer fail, hash_ret=%d", hash_ret);
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
      ...
      {
        YYSYS_LOG(DEBUG, "push DML mutator=%p into formator (index=%ld), timestamp=%ld",
        ...
        {
          // unexpected error
          YYSYS_LOG(WARN, "push queue fail, ret=%d", ret);
          ret = OB_ERR_UNEXPECTED;
        }
        else
        {
          YYSYS_LOG(DEBUG, "push task db_partition=%ld timestamp=%ld",
              db_partition,
              mutator->get_mutate_timestamp());
        }
      }
    ...
    void ObLogRouter::handle_with_stopflag(void *task, void *pdata, volatile bool &stop_flag)
    {
      ...
      if (NULL != mutator)
      {
        ...
        YYSYS_LOG(DEBUG, "formator %ld handle mutator=%p, type=%s, CP=%lu, TM=%ld",
        ...
        if (skip_dirty_data_ && OB_INVALID_DATA == tmp_ret)
        {
          YYSYS_LOG(WARN, "Formator thread %ld: skip dirty data, mutator: TM=%ld, CP=%lu, is_heartbeat=%s",
        ...
        {
          YYSYS_LOG(ERROR, "Formator thread %ld aborted, add mutator fail, "
    ...
    int ObLogRouter::add_heartbeat_mutator_(ObLogMutator *mutator, volatile bool &stop_flag)
    {
      ...
      for (iter = observer_set_.begin(); OB_SUCCESS == ret && iter != observer_set_.end(); iter++)
      {
        ...
        YYSYS_LOG(DEBUG, "Thread %ld: push HEARTBEAT (TM=%ld, SGNT=%ld) to ObLogServer %p, ret=%d",
    ...
    int ObLogRouter::add_dml_mutator_(ObLogMutator *mutator, volatile bool &stop_flag)
    {
      ...
      {
        YYSYS_LOG(ERROR, "observer_map_ get observer fails, hash_ret=%d", hash_ret);
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        ...
        YYSYS_LOG(DEBUG, "Add DML mutator timestamp=%ld, DB=%ld, ret=%d",
