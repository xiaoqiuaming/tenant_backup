...
#include "yysys.h"                // YYSYS_LOG

#define BR_CALL(f, args...) \
  do {\
    if (NULL == data_) \
    { \
      YYSYS_LOG(ERROR, "%s() fails, IBinlogRecord has not been created", __FUNCTION__); \
      abort(); \
    } \
    data_->f(args); \
  } while (0)
    ...
    class ObLogBinlogRecord : public IBinlogRecord
    {
      ...
      public:
        ObLogBinlogRecord() : data_(NULL), next_(NULL), precise_timestamp_(0)
        {
          ...
          if (NULL == data_)
          {
            YYSYS_LOG(ERROR, "DRCMessageFactory::createBinlogRecord fails");
            abort();
          }
