      ...
      {\
        ...
        { \
          ...
          { \
            ...
            YYSYS_LOG(WARN, "construct std::string fail, ret=%d", ret);\
          }\
          else \
          { \
...
\
        ...
        IObLogFilter *log_filter)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, query_engine is null");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = init_binlogrecord_list_(MAX_BINLOG_RECORD_NUM)))
      {
        YYSYS_LOG(WARN, "prepare binglog record list fail, ret=%d", ret);
      }
      else if (0 != heartbeat_info_map_.create(MAX_CACHED_HEARTBEAT_NUM))
      {
        YYSYS_LOG(ERROR, "create heartbeat_info_map_ fail, size=%d", MAX_CACHED_HEARTBEAT_NUM);
    ...
    int ObLogDenseFormator::next_record(IBinlogRecord **record, const int64_t timeout)
    {
      ...
      else
      {
        ...
        while (ob_log_alive())
        {
          ...
          int64_t wait_time = timeout - (yysys::CTimeUtil::getTime() - start_time);
    ...
    int ObLogDenseFormator::add_mutator(ObLogMutator &mutator, volatile bool &loop_stop)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "failed to format %s mutator, ret=%d",
    ...
    int ObLogDenseFormator::init_binlogrecord_list_(const int64_t max_binlogrecord_num)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "init binlogrecord p_list fail, max_binlogrecord_num=%ld", max_binlogrecord_num);
      }
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "init binlogrecord v_list fail, max_binlogrecord_num=%ld", max_binlogrecord_num);
        }
      }
      for (int64_t i = 0; OB_SUCCESS == ret && i < max_binlogrecord_num; i++)
      {
        ...
        if (NULL == buffer)
        {
          YYSYS_LOG(WARN, "allocator memory to build binglog_record fail, i=%ld", i);
          ret = OB_ALLOCATE_MEMORY_FAILED;
          break;
        }
        ...
        if (NULL == binlog_record)
        {
          YYSYS_LOG(WARN, "build binlog_record fail, i=%ld", i);
          ret = OB_ERR_UNEXPECTED;
          break;
        }
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "push binlog_record to list fail, i=%ld", i);
          break;
        }
      }
    ...
    void ObLogDenseFormator::destroy_binlogrecord_list_()
    {
      ...
      if (0 != counter)
      {
        YYSYS_LOG(WARN, "still have binlog_record not release, counter=%ld", counter);
      }
    }
      ...
      if (NULL != oblog_br)
      {
        ...
        if (EUNKNOWN != oblog_br->recordType() && HEARTBEAT != oblog_br->recordType())
        {
          ...
          if (NULL != table_meta)
          {
            ...
            {
              YYSYS_LOG(WARN, "the UserData of ITableMeta is NULL, cannot revert table meta");
            }
          }
        }
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "push ObLogBinlogRecord to v_list fail, ret=%d", ret);
        }
        else
        {
          v_cond_.signal();
    ...
    int ObLogDenseFormator::format_heartbeat_mutator_(ObLogMutator &mutator, volatile bool &loop_stop)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid heartbeat mutator, timestamp=%ld, signature=%ld", timestamp, signature);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = update_and_check_heartbeat_info_(hb_info, can_generate_heartbeat)))
      {
        YYSYS_LOG(ERROR, "update_and_check_heartbeat_info_ fail, hb_info={%s}, ret=%d", to_cstring(hb_info), ret);
      }
      // Generate HEARTBEAT
      else if (can_generate_heartbeat)
      {
        YYSYS_LOG(DEBUG, "HEARTBEAT (TM=%ld, SGNT=%ld) is ready to show", timestamp, signature);
        ...
        if (NULL != binlog_record)
        {
          ...
          {
            YYSYS_LOG(WARN, "push to p_list fail, ret=%d", ret);
        ...
        {
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "unexpected error: failed to fetch binlog record");
        }
        else
        {
          ret = OB_IN_STOP_STATE;
        ...
        bool &can_generate_heartbeat)
    {
      ...
      if (-1 == hash_ret)
      {
        YYSYS_LOG(ERROR, "get from heartbeat_info_map_ fail, key={%s}, hash_ret=%d",
      ...
      if (OB_SUCCESS == ret)
      {
        if (count >= (formator_thread_num_ - 1))
        {
          ...
          {
            YYSYS_LOG(ERROR, "erase from heartbeat_info_map_ fail, key={%s}, hash_ret=%d",
                to_cstring(hb_info), hash_ret);
            ret = OB_ERR_UNEXPECTED;
          }
          else
        ...
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "set heartbeat_info_map_ fail, key={%s}, value=%d, hash_ret=%d",
      ...
      YYSYS_LOG(DEBUG, "FORMATOR: HEARTBEAT TM=%ld, SGNT=%ld, N=%d, READY=%s",
    ...
    void ObLogDenseFormator::print_heartbeat_info_()
    {
      if (inited_)
      {
        ...
        for (; iter != heartbeat_info_map_.end(); iter++)
        {
          ...
          YYSYS_LOG(INFO, "HEARTBEAT %ld: TM=%ld, SGNT=%ld, N=%d",
    ...
    int ObLogDenseFormator::format_dml_mutator_(ObLogMutator &mutator, volatile bool &loop_stop)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_schema fail, return NULL");
        ret = OB_ERR_UNEXPECTED;
      }
      // Transaction BEGIN
      else if (OB_SUCCESS != (ret = fill_trans_barrier_(EBEGIN, mutator, br_list_tail, 0, loop_stop)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "fill_trans_barrier_ fail, record_type=EBEGIN");
        }
      }
      else
      {
        ...
        while (ob_log_alive(loop_stop))
        {
          ...
          if (OB_SCHEMA_ERROR != ret && OB_NEED_RETRY != ret)
          {
            if (OB_SUCCESS != ret && OB_IN_STOP_STATE != ret)
            {
              YYSYS_LOG(ERROR, "format_all_dml_statements_ fail, ret=%d, mutator=(checkpoint=%ld, timestamp=%ld), "
                  ...
                  yysys::CTimeUtil::getTime() - start_time, total_schema, total_schema->get_version(),
          ...
          {
            YYSYS_LOG(WARN, "format mutator(checkpoint=%lu, timestamp=%ld) fail: NEED RETRY, wait %ld us and retry. "
                "retry_times=%ld, run_time=%ld",
                mutator.get_log_id(), mutator.get_mutate_timestamp(), FORMATOR_RETRY_TIMEWAIT, ++retry_times,
                yysys::CTimeUtil::getTime() - start_time);

            usleep(FORMATOR_RETRY_TIMEWAIT);
          }
          else    // OB_SCHEMA_ERROR == ret
          {
            YYSYS_LOG(WARN, "format mutator(checkpoint=%lu, timestamp=%ld) fail: SCHEMA ERROR, refresh schema "
                ...
                total_schema->get_timestamp(), ++retry_times, yysys::CTimeUtil::getTime() - start_time);
            ...
            if (OB_SUCCESS != (ret = update_schema_(total_schema, new_schema, loop_stop)))
            {
              if (OB_IN_STOP_STATE != ret)
              {
                YYSYS_LOG(ERROR, "update_schema_ fail, ret=%d", ret);
              }
              break;
            }

            YYSYS_LOG(INFO, "formator force update schema from (%ld,%ld) to (%ld,%ld)",
        ...
        uint64_t &stmt_count)
    {
      ...
      if (need_query_back_ && OB_SUCCESS != (ret = query_back_(mutator, total_schema, loop_stop, query_result)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "query_back_ fail, ret=%d", ret);
        }
      }
      else
      {
            ...
            && OB_SUCCESS == (ret = mutator.get_mutator().next_cell()))
        {
          ...
          if (OB_SUCCESS != (ret = filter_cells_(mutator)))
          {
            if (OB_ITER_END != ret)
            {
              YYSYS_LOG(ERROR, "filter_cells_ fail, ret=%d", ret);
            }
          ...
          if (OB_SUCCESS != ret)
          {
            if (OB_IN_STOP_STATE == ret)
            {
              YYSYS_LOG(INFO, "Loop stoped. stop to format dml statement");
            }
            else if (OB_SCHEMA_ERROR != ret)
            {
              YYSYS_LOG(WARN, "failed to format dml statement, ret=%d", ret);
            }
        ...
        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(ERROR, "unexcepted error: earlier exiting before iterating to the end of mutator");
          ret = OB_ERR_UNEXPECTED;
        }
        else if (OB_ITER_END == ret)
        {
          if (NULL == dml_br_list_head)
          {
            ...
            YYSYS_LOG(ERROR, "invalid mutator: it does not contain table data user selected");
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
        ...
        ObLogMysqlQueryResult *&query_result)
    {
      ...
      if (OB_SUCCESS != (ret = ObLogMysqlQueryResult::get_tsi_mysql_query_result(query_result,
              ...
              &string_pool_)))
      {
        YYSYS_LOG(ERROR, "get_tsi_mysql_query_result fail, default_row_count=%ld, string_pool=%p, ret=%d",
      ...
      else if (OB_SUCCESS != (ret = query_engine_->query_back(mutator, total_schema, loop_stop, *query_result)))
      {
        if (OB_SCHEMA_ERROR != ret && OB_IN_STOP_STATE != ret)
        {
          YYSYS_LOG(ERROR, "query back fail, ret=%d, mutator=(checkpoint=%ld, timestamp=%ld), "
    ...
    int ObLogDenseFormator::filter_cells_(ObLogMutator &mutator)
    {
      ...
      do
      {
        ...
        {
          YYSYS_LOG(WARN, "get_mutator_cell fail, ret=%d, cell=%p", ret, cell);
          ret = OB_SUCCESS == ret ? OB_ERR_UNEXPECTED : ret;
        }
        else if (!is_row_changed)
        {
          YYSYS_LOG(WARN, "get first changed cell fail, mutator: TM=%ld, CP=%lu, cell=%s",
        ...
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to iterate to the finish cell, ret=%d", ret);
          }
        }
      } while (OB_SUCCESS == ret
          && OB_SUCCESS == (ret = mutator.get_mutator().next_cell()));
    ...
    int ObLogDenseFormator::mutator_iterate_to_row_finish_cell_(ObLogMutator &mutator)
    {
      ...
      do
      {
        ...
        if (OB_SUCCESS != ret || NULL == cell)
        {
          YYSYS_LOG(WARN, "get_mutator_cell fail, ret=%d", ret);
          ret = OB_SUCCESS == ret ? OB_ERR_UNEXPECTED : ret;
          break;
        }
      ...
      if (OB_ITER_END == ret)
      {
        YYSYS_LOG(ERROR, "get row finished cell fail, mutator(TM=%ld,CP=%lu) last cell is not row finished cell",
        ...
        ObLogBinlogRecord *&binlog_record)
    {
      ...
      if (OB_SUCCESS != (ret = construct_dml_stmt_(dml_stmt, mutator, total_schema, &last_dml_meta, index + 1))
          || NULL == dml_stmt)
      {
        YYSYS_LOG(ERROR, "construct_dml_stmt_ fails, ret=%d", ret);
        ret = (OB_SUCCESS == ret ? OB_ERR_UNEXPECTED : ret);
      }
      ...
      else if (need_query_back_ && OB_SUCCESS != (ret = get_query_value_(index, dml_stmt, query_result, query_value)))
      {
        ...
        {
          YYSYS_LOG(ERROR, "get_query_value_ fail, ret=%d", ret);
        }
      }
      ...
      {
        YYSYS_LOG(ERROR, "fails to build row value, ret=%d", ret);
      }
              ...
              loop_stop)))
      {
        if (OB_IN_STOP_STATE != ret)
        {
          YYSYS_LOG(ERROR, "construct_binlog_record_ fail, ret=%d", ret);
        }
      }
      else
      {
        ...
        uint64_t stmt_index_in_trans)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_log_paritioner fail, ret=%d", ret);
      }
              ...
              tb_name_builder_)))
      {
        YYSYS_LOG(ERROR, "get_meta_manager fail, ret=%d", ret);
      }
      // Construct DML Statement
              ...
              stmt_index_in_trans)))
      {
        YYSYS_LOG(ERROR, "ObLogDmlStmt::construct_dml_stmt fails, ret=%d", ret);
      }
      else
      {
        ...
        ObLogStrColValue *&query_value)
    {
      ...
      if (really_need_query_back)
      {
        ...
        {
          YYSYS_LOG(WARN, "query back table (ID=%lu) data fail with schema (%ld,%ld). "
        ...
        {
          YYSYS_LOG(ERROR, "get_query_value fail, row[%ld]='%s', table_id=%lu, ret=%d, query_value=%p",
        ...
        ObLogStrColValue *query_value)
    {
      ...
      {
        YYSYS_LOG(ERROR, "alloc from row_value_pool fail, ret=%d", ret);
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "failed to fill RowValue, ret=%d", ret);
        }
      }
        ...
        ObLogStrColValue *query_value)
    {
      ...
      if (NULL != query_value)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "fill_row_value_query_value_ fail, ret=%d", ret);
        }
        else
        {
      ...
      {
        YYSYS_LOG(ERROR, "failed to fill rowkey into RowValue, ret=%d", ret);
      }
      ...
      if (OB_SUCCESS == ret && OB_DML_DELETE != dml_stmt->dml_type_)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to fill normal columns into RowValue, ret=%d", ret);
        }
      }
        ...
        ObLogStrColValue *query_value)
    {
      ...
      {
        YYSYS_LOG(ERROR, "query back column value number is bigger than OB_MAX_COLUMN_NUMBER=%ld",
        ...
        const ObLogDmlStmt *dml_stmt)
    {
      ...
      {
        YYSYS_LOG(WARN, "get table schema fail, table_id=%lu", table_id);
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey.get_obj_cnt(); i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "get_column_schema fail, table_id=%lu column_id=%lu idx=%d",
        ...
        if (fill_value)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to fill %ldth row key, table_id=%lu, column_id=%lu idx=%d",
        ...
        const ObLogDmlStmt *dml_stmt)
    {
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < normal_count; i++)
      {
        if (OB_SUCCESS != (ret = dml_stmt->normal_columns_.at(i, cell_info))
            || NULL == cell_info)
        {
          YYSYS_LOG(ERROR, "fails to get cell_info from normal_columns_, ret=%d", ret);
          ret = (OB_SUCCESS == ret) ? OB_ERR_UNEXPECTED : ret;
          break;
        }
        ...
        if (fill_value)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "fill_row_value_obj_() fails to fill Object into RowValue, column_index=%d, %s",
        ...
        const int32_t column_index)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "trans value to string fail, column_index=%d %s",
              column_index, print_obj(obj));
          ret = OB_ERR_UNEXPECTED;
        }
        else if (NULL == (vstr_str = (std::string*)string_pool_.alloc()))
        {
          YYSYS_LOG(WARN, "construct std::string fail");
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else
        {
    ...
    const char *ObLogDenseFormator::value2str_(const ObObj &v, int64_t &v_len)
    {
      ...
      if (ObVarcharType == v.get_type())
      {
        ...
        {
          YYSYS_LOG(WARN, "maybe varchar too long, length=%d", str.length());
        }
      }
      else
      {
        ...
        volatile bool &loop_stop)
    {
      ...
      if (OB_SUCCESS != (ret = init_binlog_record_(binlog_record, dml_stmt, loop_stop)))
      {
        if (OB_IN_STOP_STATE != ret)
        {
          YYSYS_LOG(ERROR, "failed to initialize binlog record, dml_type=%s ret=%d",
      ...
      {
        YYSYS_LOG(WARN, "failed to fill binlog record, dml_type=%s ret=%d",
            str_dml_type(dml_type), ret);
      }
        ...
        volatile bool &loop_stop)
    {
      ...
      {
        YYSYS_LOG(ERROR, "fails to initialize binlog record, ret=%d", ret);
      }
        ...
        ObDmlType dml_type)
    {
      ...
      switch (dml_type)
      {
        ...
        default:
          ret = OB_ERR_UNEXPECTED;
          YYSYS_LOG(WARN, "unexpected error: unknown DML type %d", dml_type);
          break;
      }

      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "failed to format dml %s, ret=%d", str_dml_type(dml_type), ret);
      }
    ...
    void ObLogDenseFormator::print_user_queue_state_()
    {
      ...
      YYSYS_LOG(INFO, "STAT: [USER_QUEUE] FREE=%ld FILLED=%ld OUT=%ld TOTAL=%ld", free, filled, out, total);
    }
    ...
    int ObLogDenseFormator::update_schema_(const ObLogSchema *total_schema, const ObLogSchema *&new_schema, volatile bool &loop_stop)
    {
      ...
      while (ob_log_alive(loop_stop))
      {
        ...
        {
          YYSYS_LOG(ERROR, "get_schema_newer_than fail, run_time=%ld, old_schema version=%ld, timestamp=%ld",
            yysys::CTimeUtil::getTime() - start_time, total_schema->get_version(), total_schema->get_timestamp());
        }
      }
      ...
      if (OB_SUCCESS != ret && OB_IN_STOP_STATE != ret)
      {
        YYSYS_LOG(ERROR, "get_schema_newer_than fail, old_schema version=%ld, timestamp=%ld",
            total_schema->get_version(), total_schema->get_timestamp());
      }
