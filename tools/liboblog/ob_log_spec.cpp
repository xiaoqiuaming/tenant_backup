...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogSpec::init(const char *config, const uint64_t position, ERROR_CALLBACK err_cb, ObLogPositionType pos_type)
    {
      ...
      {
        YYSYS_LOG(WARN, "ObLogConfig init fail, config=%s, ret=%d", config, ret);
      }
      else if (OB_SUCCESS != (ret = init_internal_(position, pos_type, err_cb)))
      {
        YYSYS_LOG(WARN, "init_internal_ fail, position=%lu, pos_type=%s, err_cb=%p, ret=%d",
            position, print_pos_type(pos_type), err_cb, ret);
      }

      YYSYS_LOG(INFO, "init log spec: config=%s, position=%lu, pos_type=%s, err_cb=%p, ret=%d",
        ...
        ObLogPositionType pos_type)
    {
      ...
      {
        YYSYS_LOG(WARN, "ObLogConfig init fail, config=%p, ret=%d", &configs, ret);
      }
      else if (OB_SUCCESS != (ret = init_internal_(position, pos_type, err_cb)))
      {
        YYSYS_LOG(WARN, "init_internal_ fail, position=%lu, pos_type=%s, err_cb=%p, ret=%d",
      ...
      YYSYS_LOG(INFO, "init log spec: configs=%p, position=%lu, pos_type=%s, err_cb=%p, ret=%d",
    ...
    void ObLogSpec::destroy()
    {
      ...
      YYSYS_LOG(INFO, "ObLogSpec destroyed");
    }
    ...
    int ObLogSpec::launch()
    {
      ...
      {
        YYSYS_LOG(ERROR, "start mysql query engine fail, ret=%d", ret);
      }
      else
      {
...
#define CONSTRUCT(v, type) \
    ...
    { \
      YYSYS_LOG(WARN, "construct %s fail", #type); \
...
#define RCALL(f, v, args...) \
    ...
    { \
      YYSYS_LOG(WARN, "call %s this=%p fail, ret=%d", #f, v, ret); \
    }
...
#define DECONSTRUCT(v) \
    ...
    int ObLogSpec::init_internal_(const uint64_t position, ObLogPositionType pos_type, ERROR_CALLBACK err_cb)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid position type: %d, which should be in (%d, %d)", pos_type, POS_TYPE_UNKNOWN, POS_TYPE_NUMBER);
        ret = OB_INVALID_ARGUMENT;
      }
      // NOTE: 目前仅支持位点启动方式
      else if (POS_TYPE_CHECKPOINT != pos_type)
      {
        YYSYS_LOG(ERROR, "position type %s(%d) not supported", print_pos_type(pos_type), pos_type);
        ret = OB_NOT_SUPPORTED;
      }
      else if (OB_SQL_SUCCESS != (ret = init_obsql_()))
      {
        YYSYS_LOG(ERROR, "init_obsql fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = construct_components_()))
      {
        YYSYS_LOG(ERROR, "construct components fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = init_components_(position)))
      {
        YYSYS_LOG(ERROR, "init components fail, ret=%d", ret);
      }
      else
      {
    ...
    int ObLogSpec::init_obsql_()
    {
      ...
      {
        YYSYS_LOG(WARN, "libobsql min_conn(%d) > max_conn(%d), set max_conn=%d",
      ...
      YYSYS_LOG(INFO, "load libobsql min_conn=%d, max_conn=%d", obsql_config.min_conn_, obsql_config.max_conn_);
      ...
      {
        YYSYS_LOG(ERROR, "invalid configuration: libobsql user name='%s' is invalid", user_name);
        ret = OB_INVALID_ARGUMENT;
      }
      // NOTE: Password can be empty
      else if (0 > snprintf(obsql_config.passwd_, sizeof(obsql_config.passwd_), "%s", passwd))
      {
        YYSYS_LOG(ERROR, "invalid configuration: libobsql passwd='%s' is invalid", passwd);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (0 >= snprintf(obsql_config.cluster_address_, sizeof(obsql_config.cluster_address_), "%s", cluster_address))
      {
        YYSYS_LOG(ERROR, "invalid configuration: libobsql cluster_address='%s' is invalid", cluster_address);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SQL_SUCCESS != ob_sql_init(obsql_config))
      {
        YYSYS_LOG(ERROR, "ob_sql_init fail");
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        YYSYS_LOG(INFO, "libobsql inited success");
        ret = OB_SUCCESS;
      }
    ...
    {
      ob_sql_destroy();
      YYSYS_LOG(INFO, "obsql destroyed");
    }
    ...
    {
      YYSYS_LOG(INFO, "destroy components");
    ...
    {
      YYSYS_LOG(INFO, "deconstruct components");
