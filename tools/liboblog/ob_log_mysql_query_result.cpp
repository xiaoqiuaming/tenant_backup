...
namespace oceanbase
{
  ...
  namespace liboblog
  {
        ...
        ObLogStringPool *string_pool)
    {
      ...
      {
        YYSYS_LOG(ERROR, "GET_TSI_MULT get ObLogMysqlQueryResult fail, return NULL");
      ...
      else if (! tmp_query_result->is_inited())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "initialize query result fail, default_row_count=%ld, string_pool=%p, ret=%d",
    ...
    int ObLogMysqlQueryResult::init(int64_t default_row_count, ObLogStringPool *string_pool)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, default_row_count=%ld, string_pool=%p",
      ...
      {
        YYSYS_LOG(ERROR, "alloc memory for mysql query result row array fail, size=%ld",
    ...
    int ObLogMysqlQueryResult::get_query_value(int64_t index, ObLogStrColValue *&query_value_list)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument, index=%ld, valid_row_count=%ld", index, valid_row_count_);
        ret = OB_INVALID_ARGUMENT;
      }
      // NOTE: Current result should be ready
      else if (! is_ready())
      {
        YYSYS_LOG(ERROR, "mysql query result is not ready, all_rows_added=%s, all_row_value_filled=%s, "
      ...
      else if (OB_SUCCESS != err_code_)
      {
        ...
        YYSYS_LOG(ERROR, "all query values are invalid. err_code=%d", err_code_);
      }
      else
      {
        SpinRLockGuard guard(row_array_lock_);
        ...
        {
          YYSYS_LOG(ERROR, "unexcepted error: there is reference value in ready result. index=%ld, "
        ...
        {
          YYSYS_LOG(ERROR, "unknown mysql query result value type: %d", rows_[index].value_type_);
        ...
        int64_t query_time)
    {
      ...
      {
        YYSYS_LOG(ERROR, "unexcepted error: the %ldth row type is %d but not MYSQL_QUERY_VALUE(%d)",
      ...
      else
      {
        ...
        if (OB_SUCCESS != err_code && OB_ENTRY_NOT_EXIST != err_code)
        {
          ...
          {
            YYSYS_LOG(INFO, "QUERY_BACK: set query result error code: %d, orignal error code: %d", err_code_, err_code);
            err_code_ = err_code;
          }
        }
        ...
        if (NULL != col_value_list)
        {
          ...
          {
            YYSYS_LOG(ERROR, "construct_query_value_list_ fail, ret=%d, col_value_list=%p", ret, col_value_list);
          }
          else
          {
        ...
        bool &need_query)
    {
      ...
      {
        YYSYS_LOG(ERROR, "operation not allowed: all rows have been added. no rows could be added any more. "
      ...
      {
        YYSYS_LOG(ERROR, "enlarge_row_array_ fail, ret=%d", ret);
      }
      else
      {
        SpinRLockGuard guard(row_array_lock_);
    ...
    int ObLogMysqlQueryResult::wait_for_ready(int64_t timeout)
    {
      ...
      while (! all_rows_added_)
      {
        ...
        if (0 >= wait_time)
        {
          YYSYS_LOG(WARN, "wait for all rows added timeout, timeout=%ld, wait=%ld",
      ...
      if (OB_SUCCESS == ret && ! all_row_value_filled_)
      {
        ...
        while (need_query_row_count_ > query_completed_row_count_)
        {
          ...
          if (0 >= wait_time)
          {
            YYSYS_LOG(WARN, "wait for all rows queried back timeout, timeout=%ld, wait=%ld, "
                "need_query_row_count=%ld, query_completed_row_count=%ld",
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(ERROR, "fill_all_reference_rows_ fail, ret=%d", ret);
          }
          else
          {
      ...
      if (OB_SUCCESS == err_code_)
      {
        for (int64_t index = 0; index < valid_row_count_; index++)
        {
          if (OB_SUCCESS == rows_[index].err_code_ && ResultRowValue::REFERENCE_VALUE == rows_[index].value_type_)
          {
            ...
            YYSYS_LOG(DEBUG, "QUERY_BACK: fill reference row[%ld] with row[%ld], table=%lu",
                 index, reference_index, rows_[index].table_id_);

            if (OB_SUCCESS != (ret = copy_row_value_(rows_[index], rows_[reference_index])))
            {
              YYSYS_LOG(ERROR, "copy_row_value_ fail, ret=%d, table_id=%lu, rowkey=%s, index=%ld, reference index=%ld",
    ...
    int ObLogMysqlQueryResult::copy_row_value_(ResultRowValue &dest, const ResultRowValue &src)
    {
      ...
      if (NULL != src_col_value)
      {
        ...
        while (NULL != src_col_value)
        {
          ...
          {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            YYSYS_LOG(ERROR, "allocate memory for ObLogStrColValue fail, size=%ld", sizeof(ObLogStrColValue));
            break;
          }
          ...
          if (NULL != src_col_value->str_val_)
          {
            ...
            {
              ret = OB_ALLOCATE_MEMORY_FAILED;
              YYSYS_LOG(WARN, "allocate std::string from string pool fail");
              break;
            }
    ...
    int ObLogMysqlQueryResult::enlarge_row_array_()
    {
      ...
      if (NULL == (new_rows = (ResultRowValue *)ob_malloc(new_count * sizeof(ResultRowValue),
              ObModIds::OB_LOG_MYSQL_QUERY_RESULT)))
      {
        YYSYS_LOG(ERROR, "allocate memory for ResultRowValue fail, count=%ld, size=%ld",
      ...
      YYSYS_LOG(INFO, "QUERY_BACK: ENLARGE ROW ARRAY: count(old=%ld,new=%ld,valid=%ld), ret=%d",
