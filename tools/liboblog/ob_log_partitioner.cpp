...
#define GET_LUA_VALUE(key, check_type, value, get_type, pop) \
  ...
  { \
    ...
    { \
      YYSYS_LOG(WARN, "key=%s is not check_type=%s", key, #check_type); \
      ret = OB_ERR_UNEXPECTED; \
    } \
    else \
    { \
      YYSYS_LOG(INFO, "get_lua_value succ key=%s check_type=%s get_type=%s", key, #check_type, #get_type); \
...
namespace oceanbase
{
  ...
  namespace liboblog
  {
        ...
        IObLogSchemaGetter *schema_getter)
    {
      ...
      {
        YYSYS_LOG(ERROR, "GET_TSI_MULT get IObLogPartitioner fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (! tmp_partitioner->is_inited())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to initialize IObLogPartitioner, ret=%d", ret);
    ...
    int ObLogPartitioner::init(const ObLogConfig &config, IObLogSchemaGetter *schema_getter)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid router thread number %d", router_thread_num_);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (tb_select = config.get_tb_select()))
      {
        YYSYS_LOG(WARN, "tb_select null pointer");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == schema_getter)
      {
        YYSYS_LOG(WARN, "schema_getter null pointer");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (total_schema = schema_getter->get_schema()))
      {
        YYSYS_LOG(WARN, "get total schema fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (lua_ = luaL_newstate()))
      {
        YYSYS_LOG(WARN, "luaL_newstate fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "prepare_partition_functions fail, ret=%d tb_select=[%s]", ret, tb_select);
        }
        else
        {
          inited_ = true;
    ...
    int ObLogPartitioner::partition(const ObMutatorCellInfo *cell, uint64_t *db_partition, uint64_t *tb_partition)
    {
      ...
      else
      {
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(ERROR, "table_id_map_ get table info fails, table_id=%lu, hash_ret=%d",
        ...
        {
          YYSYS_LOG(WARN, "get from table_id_map fail, hash_ret=%d table_id=%lu table_info=%p",
          ...
          ret = OB_ERR_UNEXPECTED;
                  ...
                  tb_partition)))
        {
          YYSYS_LOG(WARN, "calc_partition fail, ret=%d table_id=%lu rowkey=%s",
        ...
        PartitionFunctionType type)
    {
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey.get_obj_cnt(); i++)
      {
        ...
        case ObVarcharType:
          {
            ...
            if (VARCHAE_BUFFER_SIZE <= snprintf(buffer,
                  ...
                  varchar_value.ptr()))
            {
              YYSYS_LOG(WARN, "varchar length too long, value=%.*s",
        ...
        default:
          YYSYS_LOG(WARN, "rowkey column value type=%d not support", value.get_type());
          ret = OB_SCHEMA_ERROR;
          break;
        }
        if (ONE_LINE_LUA == type)
        ...
        uint64_t *tb_partition)
    {
      ...
      if (NULL != db_partition)
      {
        if (DEFAULT_PAR_FUNC == table_info.db.type)
        {
          ...
          {
            YYSYS_LOG(WARN, "calc_db_partition_default_() fail, ret=%d", ret);
          }
        }
        else
        {
          ...
          {
            YYSYS_LOG(WARN, "push_rowkey_values fail, ret=%d rowkey=%s", ret, to_cstring(rowkey));
          }
          else if (0 != (ret = lua_call_func_(table_info.db.type, (int)rowkey.get_obj_cnt())))
          {
            YYSYS_LOG(WARN, "lua_call_func_ fail, ret=%d", ret);
          }
          else
          {
          ...
          && NULL != tb_partition)
      {
        if (DEFAULT_PAR_FUNC == table_info.tb.type)
        {
          ...
          {
            YYSYS_LOG(WARN, "calc_tb_partition_default_() fail, ret=%d", ret);
          }
        }
        else
        {
        ...
        const ObLogSchema &total_schema)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        if (NULL != config.get_lua_conf())
        {
          ...
          {
            YYSYS_LOG(WARN, "luaL_loadfile fail, ret=%d conf=[%s] [%s]",
                      tmp_ret, config.get_lua_conf(), lua_tostring(lua_, -1));
            ret = OB_ERR_UNEXPECTED;
          }
          else if (0 != (tmp_ret = lua_pcall(lua_, 0, 0, 0)))
          {
            YYSYS_LOG(WARN, "lua_pcall fail, ret=%d [%s]", tmp_ret, lua_tostring(lua_, -1));
            ret = OB_ERR_UNEXPECTED;
          }
        }
      }
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "create table_id_map fail");
          ret = OB_ERR_UNEXPECTED;
        }
        else if (OB_SUCCESS != (ret = ObLogConfig::parse_tb_select(tb_select, *this, &parse_arg)))
        {
          YYSYS_LOG(WARN, "parse tb_select to build table_id_map fail, ret=%d", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "build table_id_map succ, tb_select=[%s]", tb_select);
          TableIDMap::iterator iter;
          for (iter = table_id_map_.begin(); iter != table_id_map_.end(); iter++)
          {
            YYSYS_LOG(INFO, "table_id=%lu db_partition_function=%s tb_partition_function=%s",
...
#define CHECK_LUA_VALUE(key, type) \
  ...
  { \
    ...
    { \
      YYSYS_LOG(INFO, "`%s' is not [%s]", key, #type); \
      ret = OB_ERR_UNEXPECTED; \
    } \
    else \
    { \
      YYSYS_LOG(INFO, "check_lua_value succ key=%s type=%s", key, #type); \
    } \
    lua_pop(lua_, 1); \
  }
...
#define FETCH_SCRIPT_PAR_FUNC(type, partition_format) \
  { \
    ...
    { \
      ...
      if (OB_SUCCESS == ret) \
      { \
        YYSYS_LOG(INFO, "`%s' " #type " partition function found in lua script %s", \
    ...
    { \
      ...
      { \
        ...
        { \
          ...
          YYSYS_LOG(INFO, "`%s' " #type " partition function found in configuration file (%s)", \
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          else if (OB_ENTRY_NOT_EXIST == ret)
          {
            ...
            YYSYS_LOG(INFO, "'%s' use default db partition function", tb_name);
            ret = OB_SUCCESS;
          }
          else
          {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          ...
          else if (OB_ENTRY_NOT_EXIST == ret)
          {
            ...
            YYSYS_LOG(INFO, "'%s' use default tb partition function", tb_name);
            ret = OB_SUCCESS;
          }
          else
          {
    ...
    int ObLogPartitioner::operator() (const char *tb_name, const ParseArg * arg)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        else
        {
          for (int64_t i = 0; i < table_schema->get_rowkey_info().get_size(); i++)
          {
            ...
            {
              YYSYS_LOG(WARN, "rowkey column type=(%s,%d) not support", ob_obj_type_str(type), type);
              ret = OB_SCHEMA_ERROR;
              break;
            }
          }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "set to table_id_map fail, hash_ret=%d table_id=%lu table_name=%s",
