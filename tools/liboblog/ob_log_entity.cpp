...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogEntity::init(IObLogSpec *log_spec, const std::vector<uint64_t> &partitions)
    {
      ...
      {
        YYSYS_LOG(ERROR, "There have been %lu IObLog instances. It will exceed MAX_OBSERVER_NUM limit %lu. "
        ...
        ret = OB_INIT_FAIL;
              ...
              || 0 >= partitions.size())
      {
        YYSYS_LOG(WARN, "invalid param log_spec=%p partitions_size=%ld", log_spec_, partitions.size());
        ret = OB_INVALID_ARGUMENT;
      }
      else if (MAX_DB_PARTITION_NUM < total_db_partition_num + partitions.size())
      {
        YYSYS_LOG(ERROR, "There have been %lu DB partitions. It will exceed MAX_DB_PARTITION_NUM limit %lu. "
            "Please check it again.",
      ...
      {
        YYSYS_LOG(WARN, "construct log_formator fail");
      ...
      {
        YYSYS_LOG(WARN, "init log_formator fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = seq_map_.init(SEQ_MAP_SIZE)))
      {
        YYSYS_LOG(WARN, "init seq_map fail, ret=%d start_checkpoint=%lu", ret, log_spec_->get_start_checkpoint());
      }
      else
      {
        ...
        for (; iter != partitions.end(); iter++)
        {
          ...
          {
            YYSYS_LOG(WARN, "register observer fail, ret=%d partition=%lu log_formator=%p", ret, *iter, log_formator_);
            break;
          }
          else
          {
            YYSYS_LOG(INFO, "register observer succ, ret=%d partition=%lu log_formator=%p", ret, *iter, log_formator_);
      ...
      else
      {
        *record = NULL;
        ...
        if (NULL != *record && HEARTBEAT != (*record)->recordType())
        {
          ...
          {
            YYSYS_LOG(ERROR, "unexpected error, set ck=%lu to seq_map fail, ret=%d", ck, ret);
          ...
          else
          {
            ...
            YYSYS_LOG(DEBUG, "num=%lu ck=%lu cur_ck=%lu", (*record)->id(), ck, cur_ck);
          }
        }
      }
      return ret;
