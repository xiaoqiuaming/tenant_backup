...
\
    YYSYS_LOG(INFO, str, ##__VA_ARGS__); \
  } while (0)
...
#define TABLE_PERF_DEBUG() \
  do { \
    ...
    { \
      ...
      YYSYS_LOG(INFO, "DEBUG: %s(): tb_perf_map_: key=%lu, name=%s value=(%lu,%lu)", \
 ...
 \
    ...
    { \
      const char *tb_name = total_schema->get_table_schema(iter->first)->get_table_name(); \
      YYSYS_LOG(INFO, "DEBUG: %s(): last_tb_perf_map_: key=%lu, name=%s value=(%lu,%lu)", \
          __FUNCTION__, iter->first, tb_name, iter->second.trans_count_, iter->second.record_count_); \
    } \
...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogPerf::init(ObLogConfig &config, IObLogSchemaGetter *schema_getter)
    {
      ...
      {
        YYSYS_LOG(ERROR, "Invalid argument: schema_getter = NULL");
      ...
      {
        YYSYS_LOG(ERROR, "failed to create DB perf map, size=%lu",
      ...
      {
        YYSYS_LOG(ERROR, "failed to create table perf map, size=%d",
      ...
      {
        YYSYS_LOG(ERROR, "failed to parse table selected, ret=%d", ret);
      }
      else
      {
        total_trans_count_ = 0;
    ...
    int ObLogPerf::register_db_partition(const uint64_t db_partition)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to add DB perf data, hash_ret=%d db_partition=%lu",
    ...
    int ObLogPerf::operator() (const char *tb_name, void * args)
    {
      ...
      if (NULL == tb_name)
      {
        YYSYS_LOG(ERROR, "%s(): invalid arguments", __FUNCTION__);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (total_schema = schema_getter_->get_schema()))
      {
        YYSYS_LOG(ERROR, "schema_getter_ get NULL schema");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (table_schema = total_schema->get_table_schema(tb_name)))
      {
        YYSYS_LOG(ERROR, "table %s schema is NULL", tb_name);
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(WARN, "failed to add perf data into table perf map, tb_name=%s, table_id=%lu hash_ret=%d",
    ...
    int ObLogPerf::start()
    {
      ...
      if (!inited_)
      {
        YYSYS_LOG(WARN, "ObLogPerf has not been initialized");
        ret = OB_NOT_INIT;
      }
      else
      {
    ...
    int ObLogPerf::add_perf_(PerfMap &perf_map, const uint64_t key, const PerfUnit &pu)
    {
      ...
      if (! started_)
      {
        YYSYS_LOG(WARN, "ObLogPerf has not been started");
      ...
      else
      {
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(WARN, "failed to get perf data, key=%lu, hast_ret=%d", key, hash_ret);
          ret = OB_ERR_UNEXPECTED;
        }
        else
        {
          ...
          if (-1 == hash_ret)
          {
            YYSYS_LOG(WARN, "failed to add perf data, key=%lu, hash_ret=%d", key, hash_ret);
            ret = OB_ERR_UNEXPECTED;
          }
        }
      }
    ...
    int ObLogPerf::set_total_trans_count(const uint64_t trans_count)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "%s(): total transaction count is inverted: "
    ...
    void ObLogPerf::print_perf_info()
    {
      ...
      {
        last_print_time_ = yysys::CTimeUtil::getTime();

        ret = sync_perf_data_to_last_();
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "failed to sync perf data to the last, ret=%d", ret);
        }
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "%s(): print too fast! Interval=%ld, MIN_PRINT_INTERVAL_TIME=%ld",
    ...
    int ObLogPerf::sync_perf_data_to_last_()
    {
      ...
      for (iter = db_perf_map_.begin(); OB_SUCCESS == ret && iter != db_perf_map_.end(); iter++)
      {
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(ERROR, "failed to add perf data into last DB perf map, "
      ...
      for (iter = tb_perf_map_.begin(); OB_SUCCESS == ret && iter != tb_perf_map_.end(); iter++)
      {
        ...
        if (-1 == hash_ret)
        {
          YYSYS_LOG(ERROR, "failed to add perf data into last table perf map, "
      ...
      for (iter = perf_map.begin(); OB_SUCCESS == ret && iter != perf_map.end(); iter++)
      {
        ...
        if (hash::HASH_EXIST == hash_ret)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "table %lu schema is NULL", iter->first);
              ret = OB_ERR_UNEXPECTED;
              break;
            }
            ...
            {
              YYSYS_LOG(ERROR, "table %lu name is NULL", iter->first);
              ret = OB_ERR_UNEXPECTED;
              break;
            }
        ...
        {
          YYSYS_LOG(WARN, "%s %lu does not exist in last perf map",
              is_db_perf_map ? "DB" : "Table",
              iter->first);
        }
        else
        {
          YYSYS_LOG(ERROR, "failed to get %s %lu last perf data",
              is_db_perf_map ? "DB" : "Table",
              iter->first);
