...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogDBNameBuilder::operator ()(const char *tb_name, const ObLogConfig *config)
    {
      ...
      {
        YYSYS_LOG(WARN, "allocate tb_name_cstr or dbn_format_cstr fail");
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "insert into name map fail, hash_ret=%d key=%s value=%s",
        ...
        const int64_t dest_buffer_size)
    {
      ...
      if (!inited_)
      {
        ret = OB_NOT_INIT;
              ...
              || NULL == format)
      {
        YYSYS_LOG(WARN, "get from name map fail, tb_name=%s", src_name);
        ret = OB_ENTRY_NOT_EXIST;
      }
      else if (dest_buffer_size <= snprintf(dest_name, dest_buffer_size, format, db_partition))
      {
        YYSYS_LOG(WARN, "buffer size not enough, partition=%lu", db_partition);
    ...
    int ObLogTBNameBuilder::operator ()(const char *tb_name, const ObLogConfig *config)
    {
      ...
      {
        YYSYS_LOG(WARN, "allocate tb_name_cstr or tbn_format_cstr fail");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
        ...
        const int64_t dest_buffer_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "buffer size not enough, partition=%lu", tb_partition);
        ret = OB_BUF_NOT_ENOUGH;
      }
      else
      {
        ...
        IObLogTBNameBuilder *tb_name_builder)
    {
      ...
      {
        YYSYS_LOG(ERROR, "GET_TSI_MULT get IObLogMetaManager fail, return NULL");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (! tmp_meta_manager->is_inited())
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "initialize IObLogMetaManager fail, ret=%d", ret);
        }
      }
        ...
        IObLogTBNameBuilder *tb_name_builder)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, schema_getter=%p, db_name_builder=%p tb_name_builder=%p",
                  schema_getter, db_name_builder, tb_name_builder);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (NULL == (total_schema = schema_getter->get_schema()))
      {
        YYSYS_LOG(ERROR, "schema_getter get invalid schema");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (OB_INVALID_VERSION == (version_ = total_schema->get_version()))
      {
        YYSYS_LOG(ERROR, "actual version of schema is invalid");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (0 != table_meta_map_.create(MAX_TABLE_META_ENTRY_NUM))
      {
        YYSYS_LOG(ERROR, "create table_meta_map fail, size=%ld", MAX_TABLE_META_ENTRY_NUM);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (OB_SUCCESS != (ret = create_meta_collect_(meta_collect_)))
      {
        YYSYS_LOG(WARN, "create meta collection fails, ret=%d", ret);
      }
      else
      {
        reserved_version_count_ = 0;
        ...
        const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid argument: table_id=%lu, total_schema=%p, asked_schema_version=%ld",
            table_id, total_schema, asked_schema_version);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (version_ > asked_schema_version)
      {
        YYSYS_LOG(ERROR, "invalid schema: current meta version=%ld, asked schema version=%ld",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(INFO, "Meta version updates from %ld to %ld", version_, asked_schema_version);

          if (OB_SUCCESS != (ret = update_meta_version_(asked_schema_version)))
          {
            YYSYS_LOG(ERROR, "update_meta_version_ fail, asked schema version=%ld, ret=%d",
        ...
        if (OB_SUCCESS == ret)
        {
          if (hash::HASH_EXIST == (hash_ret = table_meta_map_.get(table_meta_key, tmp_table_meta)))
          {
            ...
            {
              YYSYS_LOG(WARN, "unexpected error, get from table_meta_map_ fail, %s",
          ...
          {
            YYSYS_LOG(WARN, "unexpected error, get from table_meta_map_ fail, hash_ret=%d", hash_ret);
            ret = OB_ERR_UNEXPECTED;
          }
          else if (OB_SUCCESS != (ret = build_table_meta_(tmp_table_meta, table_meta_key, total_schema)))
          {
            YYSYS_LOG(ERROR, "build table_meta fail, %s, ret=%d", table_meta_key.to_cstring(), ret);
          }
          else if (hash::HASH_INSERT_SUCC != (hash_ret = table_meta_map_.set(table_meta_key, tmp_table_meta)))
          {
            YYSYS_LOG(WARN, "add table_meta fail, hash_ret=%d %s", hash_ret, table_meta_key.to_cstring());
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
            YYSYS_LOG(INFO, "build and add table_meta succ, %s table_meta=%p",
      ...
      {
        YYSYS_LOG(ERROR, "get_table_meta fail, return NULL");
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        YYSYS_LOG(DEBUG, "get_table_meta: table_meta=%p, meta_collect_=%p, ref=%ld",
    ...
    void ObLogMetaManager::revert_table_meta(ITableMeta *table_meta)
    {
      if (NULL != table_meta)
      {
        ...
        {
          YYSYS_LOG(WARN, "can not revert ITableMeta %p, IDBMeta is NULL",
              table_meta);
        }
        else if (NULL == (meta_collect = db_meta->getMetaDataCollections()))
        {
          YYSYS_LOG(WARN, "can not revert ITableMeta %p, IMetaDataCollections is NULL",
              table_meta);
        }
        else
        {
          ...
          YYSYS_LOG(DEBUG, "revert table meta=%p, meta_collect=%p, ref=%ld",
    ...
    int ObLogMetaManager::create_meta_collect_(IMetaDataCollections *&meta_collect) const
    {
      ...
      {
        YYSYS_LOG(WARN, "createMetaDataCollections fails, return NULL");
        ret = OB_ALLOCATE_MEMORY_FAILED;
      }
      else
      {
    ...
    void ObLogMetaManager::destroy_meta_data_of_one_version_(IMetaDataCollections *meta_collect) const
    {
      ...
      {
        YYSYS_LOG(INFO, "Destroy meta data: addr=%p, ref=%ld",
    ...
    {
      YYSYS_LOG(INFO, "Recycle meta data begin: current version=%ld, reserved_version_count=%d",
      ...
      if (NULL != meta_collect_)
      {
        ...
        YYSYS_LOG(INFO, "Recycle meta data %d: addr=%p, ref=%ld",
            index--, meta_collect_,
            reinterpret_cast<int64_t>(meta_collect_->getUserData()));
        ...
        while (NULL != mc)
        {
          ...
          YYSYS_LOG(INFO, "Recycle meta data %d: addr=%p, ref=%ld", index--, mc, ref);
      ...
      YYSYS_LOG(INFO, "Recycle meta data end: current version=%ld, reserved_version_count=%d",
          version_, reserved_version_count_);
    }
        ...
        const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_table_schema fail, table_id=%lu", table_meta_key.table_id);
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      {
        YYSYS_LOG(ERROR, "get db and table name fail, ret=%d, %s", ret, table_meta_key.to_cstring());
      }
      else if (OB_SUCCESS != (ret = get_db_meta_(tmp_db_meta, db_partition_name_buffer, db_exist)))
      {
        YYSYS_LOG(WARN, "get db_meta fail, db_name=%s, ret=%d", db_partition_name_buffer, ret);
      }
              ...
              table_exist)))
      {
        YYSYS_LOG(WARN, "get table_meta fail, db_partition_name=%s table_partition_name=%s, ret=%d",
      ...
      {
        YYSYS_LOG(WARN, "set table schema fail, table_partition_name=%s, ret=%d",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "put db_meta to meta_collect fail, db_partition_name=%s", db_partition_name_buffer);
          ret = OB_ERR_UNEXPECTED;
        }
        if (!table_exist
            && 0 != tmp_db_meta->put(tmp_table_meta))
        {
          YYSYS_LOG(ERROR, "put table_meta to db_meta fail, table_partition_name=%s db_partition_name=%s",
    ...
    int ObLogMetaManager::get_db_meta_(IDBMeta *&db_meta, const char *db_partition_name, bool &exist)
    {
      ...
      if (NULL == (db_meta = meta_collect_->get(db_partition_name)))
      {
        ...
        {
          YYSYS_LOG(WARN, "createDBMeta fails, return NULL");
          ret = OB_ERR_UNEXPECTED;
        }
        ...
        bool &exist)
    {
      ...
      if (NULL == (table_meta = meta_collect_->get(db_partition_name, table_partition_name)))
      {
        ...
        {
          YYSYS_LOG(WARN, "createTableMeta fail, return NULL");
          ret = OB_ERR_UNEXPECTED;
        }
        ...
        char *tb_partition_name_buffer, const int64_t tb_partition_name_buffer_size)
    {
      ...
      {
        YYSYS_LOG(WARN, "build db name fail, ret=%d table_name=%s table_id=%lu db_partition=%lu",
      ...
      {
        YYSYS_LOG(WARN, "build table name fail, ret=%d table_name=%s table_id=%lu tb_partition=%lu",
        ...
        ITableMeta &table_meta)
    {
      ...
      {
        YYSYS_LOG(WARN, "get column schemas fail table_id=%lu", table_schema.get_table_id());
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      for (int32_t i = 0; OB_SUCCESS == ret && i < column_num; i++)
      {
        ...
        {
          YYSYS_LOG(INFO, "col_meta has been already add to table_meta succ, table_name=%s column_name=%s col_num=%d col_meta=%p",
        ...
        {
          YYSYS_LOG(WARN, "createColMeta fails, col_name=%s", column_schema.get_name());
          ret = OB_ALLOCATE_MEMORY_FAILED;
        }
        else if (OB_SUCCESS != (ret = type_trans_mysql_(column_schema.get_type(), mysql_type)))
        {
          YYSYS_LOG(WARN, "type trans to mysql fail, ret=%d ob_type=%d, mysql_type=%d", ret, column_schema.get_type(), mysql_type);
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "append col_meta to table_meta fail, table_name=%s column_name=%s", table_schema.get_table_name(), column_schema.get_name());
            ret = OB_ERR_UNEXPECTED;
          }
          else
          {
            YYSYS_LOG(DEBUG, "append col_meta to table_meta succ, table_name=%s column_name=%s", table_schema.get_table_name(), column_schema.get_name());
          }
        }
      }
      ...
      for (int64_t i = 0; OB_SUCCESS == ret && i < rowkey_info.get_size(); i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "get rowkey column fail index=%ld", i);
          ret = OB_ERR_UNEXPECTED;
        }
        ...
        {
          YYSYS_LOG(WARN, "get column schema fail table_id=%lu column_id=%lu",
