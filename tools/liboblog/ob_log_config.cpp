...
#define STR_CONFIG_CHECK(section, name) \
    ...
    { \
      ...
      { \
        YYSYS_LOG(INFO, "load string config succ %s=%s", name, v); \
      } \
    }
...
#define INT_CONFIG_CHECK(section, name, min, max) \
    ...
    { \
      ...
      { \
        YYSYS_LOG(INFO, "load int config succ %s=%d", name, v); \
      } \
    }
...
#define BOOL_CONFIG_CHECK(section, name) \
    ...
    { \
      ...
      { \
        YYSYS_LOG(INFO, "load bool config succ %s=%d", name, v); \
      } \
    }
    ...
    int ObLogConfig::init(const char * config_file)
    {
      ...
      else if (0 != yysys_config_.load(config_file))
      {
        YYSYS_LOG(WARN, "load config fail config_file=%s", config_file);
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        const char *log_fpath = yysys_config_.getString(LOG_CONFIG_SECTION, LOG_CONFIG_LOG_FPATH);
        const char *log_level = yysys_config_.getString(LOG_CONFIG_SECTION, LOG_CONFIG_LOG_LEVEL);

        // Start Logger
        start_logger_(log_fpath, log_level);

        if (OB_SUCCESS != (ret = init_map_config_(map_config_, yysys_config_)))
        {
          YYSYS_LOG(ERROR, "init_map_config_ fail, ret=%d", ret);
        }
        // Load configuration
        else if (OB_SUCCESS != (ret = load_config_()))
        {
          YYSYS_LOG(WARN, "load config fail, ret=%d config_file=%s", ret, config_file);
        }
        else
        {
          inited_ = true;
    ...
    int ObLogConfig::init(const std::map<std::string, std::string>& configs)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "load config fail, ret=%d config map=%p", ret, &configs);
        }
        else
        {
          inited_ = true;
    ...
    int ObLogConfig::refresh_tb_select(const ObLogSchema *schema)
    {
      ...
      if (NULL == schema)
      {
        YYSYS_LOG(ERROR, "invalid argument: schema is NULL");
        ret = OB_INVALID_ARGUMENT;
      }
      else if (! inited_)
      {
        YYSYS_LOG(ERROR, "ObLogConfig is not initialized");
        ret = OB_NOT_INIT;
      }
      else
      {
        YYSYS_LOG(INFO, "=============== REFRESH TABLE SELECT BEGIN ===============");
        YYSYS_LOG(INFO, "system table count = %ld", schema->get_table_count());
        YYSYS_LOG(INFO, "select table count = %ld", table_name_array_.count());
        YYSYS_LOG(INFO, "tb_select = [%s]", get_tb_select());
        ...
        for (int64_t index = 0; OB_SUCCESS == ret && index < table_name_array_.count(); index++)
        {
          ...
          {
            YYSYS_LOG(ERROR, "get table name from table name array fail, index=%ld", index);
          }
          else if (NULL == tb_name)
          {
            YYSYS_LOG(ERROR, "table name in array is invalid, index=%ld", index);
            ret = OB_INVALID_CONFIG;
          }
          else if (NULL == (table_schema = schema->get_table_schema(tb_name)))
          {
            YYSYS_LOG(ERROR, "table '%s' does not exist. schema version=%ld", tb_name, schema->get_version());
            ret = OB_INVALID_CONFIG;
          }
          else if (OB_SUCCESS != (ret = table_id_array_.push_back(table_schema->get_table_id())))
          {
            YYSYS_LOG(ERROR, "push table (%ld) into table_id_array_ fail, count=%ld, ret=%d",
                table_schema->get_table_id(), table_id_array_.count(), ret);
          }
          else
          {
            YYSYS_LOG(INFO, "TABLE SELECT[%ld]  NAME=[%s]  ID=%lu", index + 1, tb_name, table_schema->get_table_id());
          }
        }

        if (OB_SUCCESS == ret)
        {
          YYSYS_LOG(INFO, "refresh table select success");
          table_id_array_inited_ = true;
        }
        else
        {
          YYSYS_LOG(ERROR, "refresh table select fail, ret=%d, tb_select=%s", ret, get_tb_select());
        }

        YYSYS_LOG(INFO, "=============== REFRESH TABLE SELECT END ===============");
      }
    ...
    int ObLogConfig::init_map_config_(ConfigMap &map_config, yysys::CConfig &yysys_config) const
    {
      int ret = OB_SUCCESS;

      YYSYS_LOG(INFO, "===================== LIBOBLOG CONFIG BEGIN =====================");

      if (OB_SUCCESS != (ret = init_map_config_section_(map_config, yysys_config, LOG_CONFIG_SECTION)))
      {
        YYSYS_LOG(ERROR, "init_map_config_section_ fail, section=%s, ret=%d", LOG_CONFIG_SECTION, ret);
      }
      else if (OB_SUCCESS != (ret = init_map_config_section_(map_config, yysys_config, LOG_PARTITION_SECTION)))
      {
        YYSYS_LOG(ERROR, "init_map_config_section_ fail, section=%s, ret=%d", LOG_PARTITION_SECTION, ret);
      }
      else if (OB_SUCCESS != (ret = init_map_config_section_(map_config, yysys_config, LOG_OBSQL_SECTION)))
      {
        YYSYS_LOG(ERROR, "init_map_config_section_ fail, section=%s, ret=%d", LOG_OBSQL_SECTION, ret);
      }

      YYSYS_LOG(INFO, "===================== LIBOBLOG CONFIG END ======================");

      return ret;
    }

    int ObLogConfig::init_map_config_section_(ConfigMap &map_config,
        yysys::CConfig &yysys_config,
        const char *section) const
    {
      int ret = OB_SUCCESS;
      std::vector<std::string> keys;
      yysys_config.getSectionKey(section, keys);

      YYSYS_LOG(INFO, "------ CONFIG SECTION [%s]  SIZE=%ld ------", section, keys.size());

      for (uint64_t i = 0; i < keys.size(); i++)
      {
        std::string key = keys[i];
        std::string value = yysys_config.getString(section, key.c_str());
        ...
        const std::string &orig_value) const
    {
      ...
      YYSYS_LOG(INFO, "%s=%s", key.c_str(), value.c_str());
      if (changed)
      {
        YYSYS_LOG(INFO, "ORG CONFIG: %s=%s", orig_key.c_str(), orig_value.c_str());
      }
    ...
    const char *ObLogConfig::format_url_(const char *url)
    {
      ...
      if (NULL == url)
      {
        YYSYS_LOG(ERROR, "cluster_url is NULL");
      }
      else
      {
        std::string str(url);
        ...
        if (0 < str.length())
        {
          ...
          YYSYS_LOG(INFO, "format url from [%s] to [%s]", url, BUFFER);

          ret = BUFFER;
        }
        else
        {
          YYSYS_LOG(WARN, "cluster URL is empty");
        }
      }
    ...
    int ObLogConfig::load_config_()
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(WARN, "parse_cluster_url_ fail, cluster_url=%s, ret=%d", cluster_url, ret);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "set config: %s=%d micro seconds", LOG_CONFIG_QUERY_BACK_TIMEWAIT_US, query_back_timewait_us_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "set config: %s=%d", LOG_CONFIG_QUERY_BACK_THREAD_NUM, query_back_thread_num_);
        }
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        {
          YYSYS_LOG(INFO, "set config: %s=%d seconds", LOG_CONFIG_FETCH_LOG_TIMEOUT_SEC, fetch_log_timeout_sec_);
        }
      }
    ...
    int ObLogConfig::get_int_config_(const char *config_section, const char *config_name, int invalid_value) const
    {
      ...
      if (map_config_.end() != iter)
      {
        ret = yysys::CStringUtil::strToInt(iter->second.c_str(), invalid_value);
      }
    ...
    void ObLogConfig::start_logger_(const char *log_fpath, const char *log_level)
    {
      ...
      if (NULL != p)
      {
        ...
        yysys::CFileUtil::mkdirs(dir_buffer);
      }
      YYSYS_LOGGER.setFileName(log_fpath, true);

      log_level = (NULL != log_level) ? log_level : DEFAULT_LOG_LEVEL;
      YYSYS_LOGGER.setLogLevel(log_level);

      // Set max log file size
      YYSYS_LOGGER.setMaxFileSize(256 * 1024L * 1024L); /* 256M */

      YYSYS_LOG(INFO, "logger=%p", &(YYSYS_LOGGER));
      YYSYS_LOG(INFO, "====================liboblog start====================");
      YYSYS_LOG(INFO, "GIT_VERSION: %s", git_version());
      YYSYS_LOG(INFO, "BUILD_TIME: %s %s", build_date(), build_time());
      YYSYS_LOG(INFO, "BUILD_FLAGS: %s", build_flags());
    }
    ...
    int ObLogConfig::parse_cluster_url_(const char *cluster_url)
    {
      ...
      {
        YYSYS_LOG(WARN, "str config [%s] is invalid", LOG_CONFIG_OB_URL);
        ret = OB_ENTRY_NOT_EXIST;
      }
      else if (BUFFER_SIZE <= snprintf(config_file_buffer, BUFFER_SIZE, "/tmp/liboblog.curl.%d", getpid()))
      {
        YYSYS_LOG(WARN, "assemble config_file_buffer fail, buffer=./liboblog.curl.%d", getpid());
        ret = OB_BUF_NOT_ENOUGH;
      }
      else if (NULL == (config_file_fd = fopen(config_file_buffer, "w+")))
      {
        YYSYS_LOG(WARN, "fopen config_file fail, file=%s errno=%u", config_file_buffer, errno);
        ret = OB_ERR_UNEXPECTED;
      }
            ...
            config_file_fd))
      {
        YYSYS_LOG(WARN, "write session string to config file fail, errno=%u", errno);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (curl = curl_easy_init()))
      {
        YYSYS_LOG(WARN, "curl_easy_init fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (CURLE_OK != (curl_ret = curl_easy_setopt(curl, CURLOPT_URL, cluster_url)))
      {
        YYSYS_LOG(WARN, "curl_easy_setopt set CURLOPT_URL fail, error=%s", curl_easy_strerror(curl_ret));
        ret = OB_ERR_UNEXPECTED;
      }
      else if (CURLE_OK != (curl_ret = curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L)))
      {
        YYSYS_LOG(WARN, "curl_easy_setopt set CURLOPT_FOLLOWLOCATION fail, error=%s", curl_easy_strerror(curl_ret));
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      {
        YYSYS_LOG(WARN, "curl_easy_perform fail, error=%s url=[%s]", curl_easy_strerror(curl_ret), cluster_url);
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        fflush(config_file_fd);
        // NOTE: Load URL config into yysys CConfig
        if (0 != yysys_config_.load(config_file_buffer))
        {
          YYSYS_LOG(WARN, "load config fail fpath=%s", config_file_buffer);
          ret = OB_ERR_UNEXPECTED;
        }
        else
        {
          const char *cluster_address = yysys_config_.getString(LOG_CONFIG_SECTION, LOG_CONFIG_OB_ADDR);
          if (NULL == cluster_address)
          {
            YYSYS_LOG(ERROR, "clusterAddress is not provided in cluster URL: %s, invalid argument", cluster_url);
            ret = OB_INVALID_ARGUMENT;
          }
          ...
          {
            YYSYS_LOG(ERROR, "\"%s\" exists both in configuration and cluster URL(%s). Please check it again",
                LOG_CONFIG_OB_ADDR, cluster_url);
            ret = OB_INVALID_ARGUMENT;
          }
          else
          {
            YYSYS_LOG(INFO, "load string config from cluster_url succ, %s=%s",
    ...
    int ObLogConfig::operator ()(const char *tb_name, void *arg)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid select table '%s'", tb_name);
        ret = OB_INVALID_ARGUMENT;
      }
      else
      {
        YYSYS_LOG(INFO, "parse from tb_name=[%s]", tb_name);
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          if (NULL == new_tb_name)
          {
            YYSYS_LOG(ERROR, "allocator memory for table name fail, tb_name=%s, buf_len=%ld", tb_name, buf_len);
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "push back table name into table_name_array_ fail, tb_name=%s, ret=%d, count=%ld",
