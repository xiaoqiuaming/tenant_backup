...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogServerSelector::init(const ObLogConfig &config)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param from config, ob_mysql_addr=%s ob_mysql_port=%d",
                  ob_mysql_addr_, ob_mysql_port_);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = query_server_addr_()))
      {
        YYSYS_LOG(WARN, "query server addr from ob_mysql fail, ret=%d", ret);
      }
      else
      {
        cur_ups_idx_ = 0;
      ...
      else
      {
        ...
        else
        {
          ...
          if (change)
          {
            ...
            {
              YYSYS_LOG(ERROR, "get_different_ups_ fail, ret=%d", ret);
            }
          }
          else
          {
        ...
        {
          YYSYS_LOG(INFO, "get_ups: ups=%s, index=%ld", to_cstring(server), cur_ups_idx_);
          last_get_ups = server;
        }
      }
    ...
    int ObLogServerSelector::get_different_ups_(ObServer &ups, ServerHistory &ups_history)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "SERVER SELECTOR: clear ups list history");
      ...
      {
        YYSYS_LOG(WARN, "UPS history has been full, clear it.");
        ups_history.clear();
    ...
    int ObLogServerSelector::get_rs(common::ObServer &server, const bool change)
    {
      ...
      else
      {
            ...
            && cur_ups_idx_ < ups_vector_.count())
        {
          ...
          {
            YYSYS_LOG(INFO, "get_rs addr=%s cur_rs_idx=%ld", to_cstring(server), cur_rs_idx_);
          }
        }
        else
        {
    ...
    int ObLogServerSelector::query_server_addr_()
    {
      ...
      {
        YYSYS_LOG(WARN, "mysql init fail");
        ret = OB_ERR_UNEXPECTED;
            ...
            NULL, 0))
      {
        YYSYS_LOG(WARN, "connect to ob_mysql %s:%d fail, %s", ob_mysql_addr_, ob_mysql_port_, ob_mysql_error(mysql));
        ret = OB_ERR_UNEXPECTED;
      }
      else if (0 != cluster_id2role.create(ID_ROLE_MAP_SIZE))
      {
        YYSYS_LOG(WARN, "create cluster_id2role fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else if (OB_SUCCESS != (ret = query_cluster_info_(mysql, cluster_id2role)))
      {
        YYSYS_LOG(WARN, "query_cluster_info fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = query_ups_info_(mysql, cluster_id2role)))
      {
        YYSYS_LOG(WARN, "query_ups_info fail, ret=%d", ret);
      }
      else if (OB_SUCCESS != (ret = query_rs_info_(mysql, cluster_id2role)))
      {
        YYSYS_LOG(WARN, "query_rs_info fail, ret=%d", ret);
      }
      else
      {
        // do nothing
    ...
    int ObLogServerSelector::query_cluster_info_(ObSQLMySQL *mysql, IdRoleMap &cluster_id2role)
    {
      ...
      {
        YYSYS_LOG(WARN, "query sql [%s] fail, mysql_ret=%d", SQL_QUERY_CLUSTER_INFO, mysql_ret);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (mysql_results = ob_mysql_store_result(mysql)))
      {
        YYSYS_LOG(WARN, "mysql_store_result fail");
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        ...
        while(NULL != (mysql_record = ob_mysql_fetch_row(mysql_results)))
        {
          ...
          {
            YYSYS_LOG(WARN, "set cluster_id=%lu cluster_role=%lu to id2role map fail, hash_ret=%d",
                      cluster_id, cluster_role, hash_ret);
          }
          else
          {
            YYSYS_LOG(INFO, "set cluster_id=%lu cluster_role=%lu to id2role map succ",
    ...
    int ObLogServerSelector::query_ups_info_(ObSQLMySQL *mysql, IdRoleMap &cluster_id2role)
    {
      ...
      {
        YYSYS_LOG(WARN, "query sql [%s] fail, mysql_ret=%d", SQL_QUERY_UPS_ADDR, mysql_ret);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (mysql_results = ob_mysql_store_result(mysql)))
      {
      ...
      else
      {
        ...
        while(NULL != (mysql_record = ob_mysql_fetch_row(mysql_results)))
        {
          ...
          {
            YYSYS_LOG(WARN, "cluster_id=%lu not found in cluster_id2role map, hash_ret=%d",
                      cluster_id, hash_ret);
          }
          else
          {
            //YYSYS_LOG(INFO, "cluster_id=%lu found in cluster_id2role map, cluster_role=%lu",
            //          cluster_id, cluster_role);
            ObLogServer serv;
        ...
        for (iter = slave_ups_list_.begin(); iter != slave_ups_list_.end(); iter++)
        {
          ...
          YYSYS_LOG(INFO, "prepare ups list addr=%s role=%lx", to_cstring(iter->addr), iter->role);
        }
    ...
    int ObLogServerSelector::query_rs_info_(ObSQLMySQL *mysql, IdRoleMap &cluster_id2role)
    {
      ...
      {
        YYSYS_LOG(WARN, "query sql [%s] fail, mysql_ret=%d", SQL_QUERY_RS_ADDR, mysql_ret);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (NULL == (mysql_results = ob_mysql_store_result(mysql)))
      {
      ...
      else
      {
        ...
        while(NULL != (mysql_record = ob_mysql_fetch_row(mysql_results)))
        {
          ...
          else
          {
            ...
            YYSYS_LOG(INFO, "prepare rs list addr=%s role=%lx", to_cstring(serv.addr), serv.role);
          }
        }
