...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    int ObLogMutator::get_mutator_cell(ObMutatorCellInfo **cell)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "mutator get_cell fail, invalid data, timestamp=%ld, checkpoint=%lu, ret=%d",
    ...
    int ObLogTableFilter::init(IObLogFilter *filter, const ObLogConfig &config, IObLogSchemaGetter *schema_getter)
    {
      ...
      if (inited_)
      {
        ret = OB_INIT_TWICE;
              ...
              || NULL == schema_getter)
      {
        YYSYS_LOG(WARN, "invalid param filter=%p schema_getter=%p", filter, schema_getter);
        ret = OB_INVALID_ARGUMENT;
      }
      else if (OB_SUCCESS != (ret = init_tid_filter_(config, *schema_getter)))
      {
        YYSYS_LOG(WARN, "init_tid_filter fail, ret=%d", ret);
      }
      else
      {
      ...
      else
      {
        ...
        while (true)
        {
          ...
          if (OB_SUCCESS == ret)
          {
            if (NULL == tmp_mutator)
            {
              YYSYS_LOG(ERROR, "unexpected error: next_mutator return true but with NULL mutator value");
            ...
            {
              YYSYS_LOG(ERROR, "filter_mutator_ fail, ret=%d", ret);
              break;
            }
            else if (! tmp_mutator->is_filter_info_valid())
            {
              YYSYS_LOG(ERROR, "filter_mutator_ fail, mutator filter info is invalid");
              ret = OB_ERR_UNEXPECTED;
              break;
            }
          ...
          cur_timeout = end_time - yysys::CTimeUtil::getTime();
          // TIMEOUT
          if (0 >= cur_timeout)
          {
    ...
    int ObLogTableFilter::operator ()(const char *tb_name, const ObLogSchema *total_schema)
    {
      ...
      {
        YYSYS_LOG(WARN, "table_name=%s not find in schema", tb_name);
        ret = OB_ERR_UNEXPECTED;
      }
      else if (hash::HASH_INSERT_SUCC != (hash_ret = tid_filter_.set(table_schema->get_table_id()))
              && hash::HASH_EXIST != hash_ret)
      {
        YYSYS_LOG(WARN, "set to tid_filter fail, hash_ret=%d table_name=%s table_id=%lu",
                  hash_ret, tb_name, table_schema->get_table_id());
        ret = OB_ERR_UNEXPECTED;
      }
      else
      {
        YYSYS_LOG(INFO, "add tid_filter succ, table_name=%s table_id=%lu",
    ...
    int ObLogTableFilter::filter_mutator_(ObLogMutator &mutator) const
    {
      ...
      else
      {
            ...
            && OB_SUCCESS == (ret = mutator.get_mutator().next_cell()))
        {
          ...
          {
            YYSYS_LOG(WARN, "unexpected error cell is null pointer");
            ret = OB_ERR_UNEXPECTED;
          }
    ...
    void ObLogTableFilter::wait_to_process_mutator_(const ObLogMutator &mutator) const
    {
      ...
      total_count++;
      while (min_process_timestamp > (cur_timestamp = yysys::CTimeUtil::getTime()))
      {
        ...
        YYSYS_LOG(DEBUG, "MUTATOR INFO: WAIT=%u, MIN_PROCESS_TM=%ld, SYS_TM=%ld, "
        ...
        {
          YYSYS_LOG(INFO, "ROUTER WAIT: AVG=%ld us, RATIO=%ld%% (%ld/%ld) SUM=%ld sec",
        ...
        {
          YYSYS_LOG(INFO, "ROUTER WAIT: RESET, SUM=%ld sec, TOTAL_COUNT=%ld, WAIT_COUNT=%ld",
