  ...
  namespace liboblog
  {
    ...
    void ObLogCommon::handle_error(ObLogError::ErrLevel level, int err_no, const char *fmt, ...)
    {
      ...
      if (0 == ATOMIC_CAS(&handle_error_flag_, 0, 1))
      {
        ...
        YYSYS_LOG(INFO, "%s(): err_cb_=%p, level=%d, errno=%d, errmsg=\"%s\"",
        ...
        if (NULL != err_cb_)
        {
          ...
          YYSYS_LOG(INFO, "ERROR_CALLBACK %p begin", err_cb_);
          err_cb_(err);
          YYSYS_LOG(INFO, "ERROR_CALLBACK %p end", err_cb_);
        }
        else
        {
          YYSYS_LOG(INFO, "No ERROR CALLBACK function available, abort now");
          abort();
        }
