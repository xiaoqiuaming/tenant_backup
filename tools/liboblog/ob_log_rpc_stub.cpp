...
namespace oceanbase
{
  ...
  namespace liboblog
  {
    ...
    int ObLogRpcStub::init()
    {
      ...
      {
        YYSYS_LOG(WARN, "easy_eio_create fail");
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "easy_eio_start fail");
          ret = OB_ERR_UNEXPECTED;
        }
        else if (OB_SUCCESS != (ret = client_.initialize(eio_, &client_handler_)))
        {
          YYSYS_LOG(WARN, "initialize client fail ret=%d", ret);
        }
        else if (OB_SUCCESS != (ret = ObRpcStub::init(&buffer_, &client_)))
        {
          YYSYS_LOG(WARN, "rpc_stub init fail ret=%d", ret);
        }
        else
        {
          inited_ = true;
