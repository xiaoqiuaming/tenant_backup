...
namespace oceanbase
{
  ...
  namespace liboblog
  {
        ...
        uint64_t stmt_index_in_trans)
    {
      ...
      if (NULL != dml_stmt)
      {
        ...
        if (OB_SUCCESS != (ret = dml_stmt->init(total_schema,
                ...
                stmt_index_in_trans)))
        {
          YYSYS_LOG(ERROR, "fail to initialize ObLogDmlStmt, ret=%d", ret);
        }
      }
      else
      {
        YYSYS_LOG(ERROR, "GET_TSI_MULT fail: TSI_LIBOBLOG_DML_STMT, return NULL");
        ...
        uint64_t stmt_index_in_trans)
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid arguments: total_schema=%p, log_partitioner=%p, "
      ...
      {
        YYSYS_LOG(ERROR, "fails to initialize data, ret=%d", ret);
      }
      else
      {
        ...
        IObLogMetaManager *meta_manager)
    {
      ...
      {
        YYSYS_LOG(ERROR, "init_basic_data_ fail, ret=%d", ret);
      }
      else if (OB_INVALID_ID != first_cell->cell_info.column_id_)
      {
        ...
        do
        {
          if (NULL != total_schema_->get_column_schema(table_id_, cell->cell_info.column_id_))
          {
            ...
            {
              YYSYS_LOG(ERROR, "normal_columns_.push_back fails, ret=%d", ret);
              break;
            }
            else if (! cur_dml_meta_.add_modified_column(cell->cell_info.column_id_))
            {
              YYSYS_LOG(ERROR, "ObLogDmlMeta::add_modified_column fail, column_id=%lu",
          ...
          else
          {
            ...
            YYSYS_LOG(INFO, "mutator (TM=%ld, CP=%lu): ignore column %ld of table %ld, schema=(%ld,%ld) ",
          ...
          {
            YYSYS_LOG(ERROR, "iterate to row finished cell fail, next_cell() return %d"
                "mutator: TM=%ld, CP=%lu",
          ...
          {
            YYSYS_LOG(ERROR, "cell column ID is invalid, mutator: TM=%ld, CP=%lu, cell='%s'",
        ...
        bool &irf)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get_first_cell_ fail, ret=%d", ret);
      }
      // Get column number
      else if (NULL == (total_schema_->get_table_schema(first_cell->cell_info.table_id_, column_num_)))
      {
        // TODO: support drop table
        YYSYS_LOG(ERROR, "get_table_schema fail, table_id=%lu", first_cell->cell_info.table_id_);
        ret = OB_ERR_UNEXPECTED;
      }
      ...
      {
        YYSYS_LOG(ERROR, "ObLogPartitioner::partition() fails, schema=(%ld,%ld), mutator TM=%ld, CP=%lu"
            "ret=%d, cell_info='%s'",
            total_schema_->get_version(), total_schema_->get_timestamp(),
      ...
      // Get ITableMeta based on specific schema
              ...
              total_schema_)))
      {
        YYSYS_LOG(ERROR, "get_table_meta fail, table_id=%lu, db_partition=%lu, "
        ...
        ObDmlType *dml_type_ptr)
    {
      ...
      {
        YYSYS_LOG(ERROR, "mutator cell is not the first changed cell, TM=%ld, CP=%lu, cell='%s'",
      ...
      {
        YYSYS_LOG(ERROR, "mutator cell table id is invalid, TM=%ld, CP=%lu, cell='%s'",
            mutator.get_mutate_timestamp(), mutator.get_log_id(), print_cellinfo(&cell->cell_info));
        ret = OB_INVALID_DATA;
      }
      ...
      {
        YYSYS_LOG(ERROR, "mutator has NOP column but has multiple cells, TM=%ld, CP=%lu, "
