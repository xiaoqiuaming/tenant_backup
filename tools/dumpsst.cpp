...
void print_obj(int o,const ObObj& obj)
{
  ...
  switch(obj.get_type())
  {
    ...
    case ObExtendType:
      ...
      break;
    case ObBoolType:
      obj.get_bool(bool_val);
      fprintf(stderr, "type:ObBool, val:%d\n", bool_val);
      break;
    case ObDecimalType:
      obj.get_decimal(dec_val);
      fprintf(stderr, "type:ObDecimal, val:%s\n", to_cstring(dec_val));
      fprintf(stderr, "get_precision():%d, get_scale():%d, get_vscale():%d\n", dec_val.get_precision(), dec_val.get_scale(), dec_val.get_vscale());
      break;
    case ObDateType:
      obj.get_date(int_val);
      fprintf(stderr, "type:ObDate, val:%ld\n", int_val);
      break;
    case ObTimeType:
      obj.get_time(int_val);
      fprintf(stderr, "type:ObTime, val:%ld\n", int_val);
      break;
    case ObIntervalType:
      obj.get_interval(int_val);
      fprintf(stderr, "type:ObInterval, val:%ld\n", int_val);
      break;
    case ObInt32Type:
      obj.get_int32(int32_val);
      fprintf(stderr, "type:ObInt32, val:%d\n", int32_val);
      break;
    default:
  ...
  if(OB_SUCCESS == ret)
  {
    if (header.is_compress())
    {
      ...
      if (NULL != dec)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "decompress failed...\n");
      ...
      {
        YYSYS_LOG(ERROR, "get_decompressor failed, maybe decompress library install incorrectly.");
...
//        if (ltype == ObCreateTimeType || ltype == ObModifyTimeType) { continue; }
          ...
          if (0 == ret && src_obj_array[j] != dst_obj_array[j])
          {
            fprintf(stderr, "row index %ld column %ld not equal, dump rowkey :\n",
                i, j);
            YYSYS_LOG(WARN, "%s", to_cstring(src_key));
            src_obj_array[j].dump(YYSYS_LOG_LEVEL_WARN);
            dst_obj_array[j].dump(YYSYS_LOG_LEVEL_WARN);
            ret = -1;
          }
          if(-1 == ret)
          {
  ...
  if(0 == ret)
  {
    while (0 == ret && dst_it != dst_block_reader.end())
    {
      ...
      if(0 == ret)
      {
        YYSYS_LOG(WARN, "%s", to_cstring(dst_key));
        ++dst_it;
        ++i;
      }
    }
...
//          src_data_checksum, dst_data_checksum);
      fprintf(stderr, "block[%ld] 's checksum is inconsistent, src_data_checksum=%lu, dst_data_checksum=%lu table_id[%ld]:src key[%s]\n",
              block_num, src_data_checksum, dst_data_checksum, src_block_pos->table_id_, to_cstring(src_block_pos->rowkey_));
      //ret = OB_ERROR;
      //goto error;
    }
  }
    ...
    bool ignore_size)
{
  ...
  if(0 == ret)
  {
    while (0 == ret && src_it != src_index.end() && dst_it != dst_index.end())
    {
      ...
      if(0 == ret)
      {
        ret = compare_block(src_file_name, dst_file_name, src_reader, dst_reader, src_it, dst_it, ignore_size, i);
        if (OB_SUCCESS != ret)
        {
  ...
  if(0 == ret)
  {
    ...
    while (src_it != src_index.end())
    {
      ...
      src_it->rowkey_.dump(YYSYS_LOG_LEVEL_WARN);
      ++src_it;
      ++i;
    }
    ...
    while (dst_it != dst_index.end())
    {
      ...
      dst_it->rowkey_.dump(YYSYS_LOG_LEVEL_WARN);
      ++dst_it;
      ++i;
    }
...
void usage()
{
  ...
  printf("   -c| --cmd_type,command type:[dump_sstable|cmp_sstable|dump_meta|search_meta|change_meta] \n");
  printf("   -D| --sstable_directory,sstable directory \n");
  printf("   -I| --index_file_name,must be set while cmd_type is change_meta\n");
  printf("   -f| --file_id,sstable file id,must be set while cmp_type is cmp_sstable\n");
  printf("   -p| --target_file_id,must be set while cmp_type is cmp_sstable\n");
  printf("   -d| --dump_content:[dump_index|dump_schema|dump_trailer|dump_block],could be set while cmd_type is dump_sstable\n");
  printf("   -b| --block_id,show rows number in block,could be set while cmd_type is dump_sstable and dump_content is dump_block\n");
  printf("   -i| --disk_no,disk number\n");
  printf("   -r| --search_range,must be set while cmp_type is search_meta\n");
  printf("   -t| --table_id,must be set while cmp_type is search_meta\n");
  printf("   -a| --app_name,must be set while cmp_type is search_meta\n");
  printf("   -P| --sstable_file_directory,sstable file directory\n");
  printf("   -n| --new_range,must be set while cmp_type is change_meta and action is change_range \n");
  ...
  printf("   -c search_meta -t table_id -r search_range -a app_name -x hex_format -I idx_file_name -D sstable_directory \n");
  printf("   -c change_meta -t table_id -r search_range -d remove_range -x hex_format -I idx_file_name -D sstable_directory "
         "[-i disk_no -v data_version -f dump_sstable -n new_range] \n");
  printf("   -c change_meta -t table_id -r search_range -d set_merged_flag -x hex_format -I idx_file_name -D sstable_directory "
         "[-i disk_no -v data_version -f dump_sstable -n new_range] \n");
  printf("   -c change_meta -t table_id -r search_range -d clear_merged_flag -x hex_format -I idx_file_name -D sstable_directory "
         "[-i disk_no -v data_version -f dump_sstable -n new_range] \n");
  printf("   -c change_meta -t table_id -r search_range -d find_tablet -x hex_format -I idx_file_name -D sstable_directory "
         "[-i disk_no -v data_version -f dump_sstable -n new_range] \n");
  printf("   -c change_meta -t table_id -r search_range -d change_range -x hex_format -I idx_file_name -D sstable_directory"
         "-n new_range [-i disk_no -v data_version -f dump_sstable ] \n");
  printf("\n");
  exit(0);
}
...
int main(const int argc, char **argv)
{
  ...
  if (clp.quiet) YYSYS_LOGGER.setLogLevel("ERROR");
