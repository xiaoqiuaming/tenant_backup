...
namespace oceanbase
{
  namespace tools
  {
        ...
        TableConf::ColumnIterator &col_itr, bool add_delima) const
    {
      ...
      while (col_itr != conf_->column_end())// && cell->column_name_.compare(*col_itr))
      {
        ...
        if (add_delima)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "apped delima failed");
            break;
          }
        }
        ...
        if (conf_->is_rowkey(*col_itr))
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "can't encode rowkey");
            break;
          }
        }
#if DUMP_DEBUG
    ...
    int TaskOutputFile::set_table_conf(const TableConf *conf)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "call set_table_conf with NULL");
      }
      else
      {
        delima_ = conf->delima();
        rec_delima_ = conf->rec_delima();
        YYSYS_LOG(DEBUG, "Using table conf, conf string = %s", conf->DebugString().c_str());
      }
    ...
    int TaskOutputFile::open()
    {
      ...
      if (env_ != NULL && env_->valid())
      {
        ...
        {
          YYSYS_LOG(ERROR, "can't new writable_file");
          ret = OB_ERROR;
        }
      }
      else                                      /* create a console output */
      {
        ...
        if (file_ == NULL)
        {
          YYSYS_LOG(ERROR, "can't create console output");
          ret = OB_ERROR;
        }
        else
        {
          YYSYS_LOG(DEBUG, "using console as output file");
        }
      }
    ...
    int TaskOutputFile::convert_result(const ObScanner &result, ObDataBuffer &buffer) const
    {
      ...
      {
        YYSYS_LOG(WARN, "please specify table conf");
        return OB_ERROR;
      }
      ...
      while (itr != result.end())
      {
        ...
        else
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "append delima or end or record failed");
            break;
          }
        }
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "task_worker conf is diff with master conf, please check it");
          break;
        }
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(ERROR, "column name mismatch ,cell column name = %s, expected name = %s",
        ...
        if (ret == OB_SUCCESS)
        {
          if (conf_->is_revise_column(cell->column_name_))
          {
            ...
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "revise column failed, column_id=%ld", cell->column_id_);
              break;
            }
          }
          else
          {
            ...
            if (len < 0)
            {
              YYSYS_LOG(ERROR, "serialize cell failed");
              ret = OB_ERROR;
              break;
            }
          }
      ...
      if (ret == OB_SUCCESS)
      {
        if (col_itr != conf_->column_end())
        {
          YYSYS_LOG(ERROR, "error happens column does not all proceeded");
        }
        ...
        if (ret == OB_SUCCESS)
        {
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "last row, append end rec failed");
          }
        }
      }
      return ret;
    ...
    int TaskOutputFile::get_buffer(ObDataBuffer &buffer)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "no enough memory for buffer");
      }
      else
      {
    ...
    int TaskOutputFile::append(common::ObScanner &scanner)
    {
      ...
      YYSYS_LOG(DEBUG, "in task output file append");
      if (file_ == NULL)
      {
        YYSYS_LOG(ERROR, "output file is not opened");
        ret = OB_ERROR;
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "can't get internal buffer");
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        if (ret == OB_SUCCESS)
        {
          YYSYS_LOG(DEBUG, "result buffer len = %ld", buffer.get_position());
          if (file_->append(buffer) == -1)
          {
            ret = OB_ERROR;
            YYSYS_LOG(ERROR, "can't append buffer to writable file");
          }
        }
        else
        {
          YYSYS_LOG(ERROR, "convert_result failed");
        }
      }
