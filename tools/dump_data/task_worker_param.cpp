...
int TaskWorkerParam::load(const char *file)
{
  ...
  if (file == NULL)
  {
    YYSYS_LOG(ERROR, "no configuration file is specified");
    ret = OB_ERROR;
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      task_file_type_ = YYSYS_CONFIG.getInt(kSysConfig, kOutputFileType, 1);
...
int TaskWorkerParam::load_hdfs_config()
{
  ...
  hdfs_param_.replication = YYSYS_CONFIG.getInt(kHdfsConfig, kHdfsReplication, 0);
  hdfs_param_.port = YYSYS_CONFIG.getInt(kHdfsConfig, kHdfsPort, 9000);
  hdfs_param_.host = YYSYS_CONFIG.getString(kHdfsConfig, kHdfsHost);
  hdfs_param_.user = YYSYS_CONFIG.getString(kHdfsConfig, kHdfsUser);

  /* default 4K io buffer */
  hdfs_param_.io_buffer_size = YYSYS_CONFIG.getInt(kHdfsConfig, kHdfsIoBufferSize, 4096); 
  hdfs_param_.blocksize = YYSYS_CONFIG.getInt(kHdfsConfig, kHdfsBlockSize, 0);
  need_auth = YYSYS_CONFIG.getInt(kHdfsConfig, kHadoopAuth, 0); /* default--do not need auth */

  const char *group_str = YYSYS_CONFIG.getString(kHdfsConfig, kHdfsGroup);
  if (group_str == NULL) 
  {
    YYSYS_LOG(WARN, "group is null");
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "group string is too len, max availabe length is %d", kHdfsGroupBuffSize);
      ret = OB_ERROR;
    }
    ...
    while (str != NULL && ret == OB_SUCCESS)
    {
      ...
      {
        YYSYS_LOG(ERROR, "too much nr group, max is %d", kHdfsMaxGroupNr);
        ret = OB_ERROR;
        break;
      }
