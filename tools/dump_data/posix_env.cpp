  ...
  namespace tools {
    ...
    WritableFile *PosixEnv::console()
    {
      ...
      if (new_stdout != -1) 
      {
        ...
        if (file == NULL) 
        {
          YYSYS_LOG(ERROR, "can't allocate posix writable file");
        }
      }
    ...
    int PosixEnv::new_writable_file(const std::string &file_name, WritableFile *&file)
    {
      ...
      if (fd < 0) 
      {
        YYSYS_LOG(ERROR, "can't crate output file %s", file_name.c_str());
        ret = -1;
      } 
      else 
      {
        ...
        if (file == NULL) {
          YYSYS_LOG(ERROR, "can't allocate PosixWritableFile");
          ret = -1;
        }
      }
