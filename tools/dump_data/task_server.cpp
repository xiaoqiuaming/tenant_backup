  ...
  const char * dev, const int32_t port)
{
  ...
  {
    YYSYS_LOG(ERROR, "check status or rpc or dev or port failed:"
  ...
  else
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "base server start failed:ret[%d]", ret);
        }
        else
        {
          YYSYS_LOG(INFO, "start task server succ");
        }
      }
    }
  }
...
int TaskServer::initialize(void)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "check service status failed:status[%d]", status_);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "initialize base server failed:ret[%d]", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "base server init succ");
    }
  }
  return ret;
}
...
int TaskServer::start_service(void)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "get newest schema failed:ret[%d]", ret);
  }
#else
  yysys::CConfig cc1;
  ...
  if (!succ) {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "can't load schema from file");
  }
#endif
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get update server failed:ret[%d]", ret);
    }
    else
    {
      ...
      YYSYS_LOG(INFO, "get update server succ:server[%s]", server_addr);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get memtable version failed:ret[%d]", ret);
    }
    else
    {
      YYSYS_LOG(INFO, "get memtable version succ:version[%ld]", memtable_version_);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "factory init failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "get all tablet failed:ret[%d]", ret);
    }
    else if (true == task_manager_.check_tasks(count))
    {
      ...
      {
        status_ = READY_STAT;
        YYSYS_LOG(INFO, "get all tablet succ:count[%lu]", count);
      }
    }
    else
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "check all tablet failed:count[%lu]", count);
    }
  }
  return ret;
}
...
int TaskServer::do_request(ObPacket* base_packet)
{
  ...
  if ((OB_SUCCESS == ret) && (READY_STAT == status_) && check_inner_stat())
  {
    ...
    default:
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "wrong packet code:%d", packet_code);
        break;
      }
    }
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check handle failed:ret[%d]", ret);
    }
  }
  else if (FINISH_STAT == status_)
  {
    YYSYS_LOG(INFO, "all task finished succ:tablet_count[%lu]", task_manager_.get_count());
    stop();
  }
  else
  {
...
int TaskServer::handle_fetch_task(ObPacket * packet)
{
  ...
  if (NULL == data)
  {
    YYSYS_LOG(ERROR, "check get buffer failed:data[%p]", data);
    ret = OB_ERROR;
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send response to client failed:ret[%d], task[%lu]",
          old_ret, task.get_id());
      // TODO rollback the doing or waiting counter
    }
    else
    {
      YYSYS_LOG(DEBUG, "handle fetch task succ:ret[%d], task[%lu], index[%ld]",
...
int TaskServer::handle_finish_task(ObPacket * packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize finish result failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize finish task packet failed:ret[%d]", ret);
    }
  }
  ...
  if ((OB_SUCCESS == ret) && (OB_SUCCESS == result))
  {
    ...
    if (NULL == file_name)
    {
      YYSYS_LOG(ERROR, "%s", "deserialize file name failed");
      ret = OB_ERROR;
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "report finish task failed:task[%lu], server[%s], ret[%d]",
        task.get_id(), peer_ip, ret);
    }
    else if (OB_SUCCESS == result)
    {
      YYSYS_LOG(INFO, "finish task succ:task[%lu], server[%s], output_file[%s]",
        task.get_id(), peer_ip, file_name);
    }
    else
    {
      YYSYS_LOG(WARN, "report finish task failed:task[%lu], server[%s], result[%ld]",
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send response to client failed:task[%lu], server[%s], result[%ld], ret[%d]",
          task.get_id(), peer_ip, result, old_ret);
    }
    else
    {
      YYSYS_LOG(INFO, "handle finish task succ:task[%lu], server[%s], result[%ld], ret[%d]",
          task.get_id(), peer_ip, result, old_ret);
    }
  }
  ...
  if ((OB_SUCCESS == ret) && (OB_SUCCESS == result))
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "add finish task failed:task[%lu], ret[%d]", task.get_id(), ret);
    }
  }
...
TaskServer::~TaskServer()
{
  ...
  if ((status_ == FINISH_STAT) && (task_output_.size() > 0) && (NULL != result_file_))
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "check fopen output file failed:file[%s]", result_file_);
    }
    else
    {
      ...
      if (err != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "task output print file failed:file[%s], ret[%d]", result_file_, err);
      }
      ret = fclose(file);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "fclose file failed:file[%s], ret[%d]", result_file_, ret);
      }
      else if (OB_SUCCESS == err)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "rename temp file to result file failed:temp[%s], result[%s], ret[%d]",
      ...
      if (ret != OB_SUCCESS)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "remove temp file failed:temp[%s], result[%s], ret[%d]",
              temp_file, result_file_, ret);
        }
      }
    }
