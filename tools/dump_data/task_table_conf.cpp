...
#define ITEM_BUFFER_SIZE 128
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "rowkey item is too len, max len is %d", ITEM_BUFFER_SIZE);
  }
  else
  {
  ...
  YYSYS_LOG(DEBUG, "input_str=%s", input_str);
  //parse start_pos
  if (ret == OB_SUCCESS)
  {
    p = strtok(str, delima);
    if (p == NULL)
    {
      YYSYS_LOG(ERROR, "can't parse start_pos");
      ret = OB_ERROR;
    }
    else
    {
      YYSYS_LOG(DEBUG, "start_pos=%s", p);
      start_pos = atoi(p);
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (p == NULL)
    {
      YYSYS_LOG(ERROR, "can't parse length");
      ret = OB_ERROR;
    }
    else
    {
      YYSYS_LOG(DEBUG, "length=%s", p);
      item_len = atoi(p);
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (p == NULL)
    {
      YYSYS_LOG(ERROR, "can't parse type");
      ret = OB_ERROR;
    }
    else
    {
      ...
      {
        YYSYS_LOG(DEBUG, "rkitem type = %d", type);
      }
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (p == NULL)
    {
      YYSYS_LOG(ERROR, "can't parse rowkey item name");
      ret = OB_ERROR;
    }
    else
    {
...
int RowkeyWrapper::load(const std::vector<const char*> &conf)
{
  ...
  for(size_t i = 0;i < conf.size(); i++)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "can't parse string");
      break;
    }
...
int RowkeyWrapper::encode(const ObString &rowkey, ObDataBuffer &buff, const RecordDelima &delima) const
{
  ...
  while (itr != end())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "can't append rowkey item, start_pos=%d, type=%d",
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "can't append delima, start_pos=%d, type=%d",
          itr->start_pos, itr->type);
      break;
    }
    ...
    case INT16:
      {
        ...
        {
          YYSYS_LOG(ERROR, "rowkey item config error, start_pos=%d, type=%d, key_part_len = %d",
    ...
    case INT8:
      {
        ...
        if (key.length() < (item.start_pos + 1) ||
            item.item_len != 1)
        {
          YYSYS_LOG(ERROR, "rowkey item config error, start_pos=%d, type=%d",
    ...
    case STR:
      {
        if (key.length() < item.start_pos + item.item_len)
        {
          YYSYS_LOG(ERROR, "configuration error, item.start_pos=%d, item.item_len=%d",
    ...
    case VSTR:
      {
        ...
        {
          YYSYS_LOG(ERROR, "configuration error, item.start_pos=%d",
              item.start_pos);
          break;
        }
      ...
      break;
    default:
      YYSYS_LOG(ERROR, "such type [%d] is not supported now", type);
      break;
  }
  ...
  revise_cols_(revise_columns), table_id_(table_id), delima_(1), rec_delima_(10)
{
  ...
  {
    YYSYS_LOG(ERROR, "can't load rowkey items, please your config");
  }
}
...
int TableConf::revise_column(const ObCellInfo &cell, ObDataBuffer &data) const
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      ...
      // YYSYS_LOG(ERROR, "append_double error, column:%s", column.c_str());
    }
  }
  else
  {
    ...
    buf[2 * len] = 0;
    YYSYS_LOG(ERROR, "obcellinfo get value failed, column=%s, rowkey=%s, ret=%d" ,column.c_str(), buf, ret);
  }
                       ...
                       ObDataBuffer & buffer) const
{
  ...
  {
    YYSYS_LOG(ERROR, "No such rowkey column %s, please check it", rk_item_name);
    ret = OB_ERROR;
  }
...
int TableConf::loadConf(const char *table_name, TableConf &conf)
{
  ...
  vector<const char *> rowkey_items =
    YYSYS_CONFIG.getStringList(table_name, kTabRowkeyItem);
  vector<const char *> revise_cols =
    YYSYS_CONFIG.getStringList(table_name, kTabReviseCol);
  vector<const char *> columns =
    YYSYS_CONFIG.getStringList(table_name, kColumnList);

  int64_t table_id = YYSYS_CONFIG.getInt(table_name, kTableId);
  TableConf tmp_conf(revise_cols, rowkey_items, columns, table_name, table_id);
  if (tmp_conf.valid())
  {
    const char *out = YYSYS_CONFIG.getString(table_name, kTableOutputPath);
    tmp_conf.set_output_path(out);

    const char *str_delima =
      YYSYS_CONFIG.getString(table_name, kDelima);
    if (str_delima != NULL)
    {
      RecordDelima delima;
    ...
    const char *str_rec_delima =
      YYSYS_CONFIG.getString(table_name, kRecDelima);
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "invalid table conf finded, table:%s", table_name);
  }
