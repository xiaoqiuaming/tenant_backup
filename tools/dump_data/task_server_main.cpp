...
int main(int argc, char ** argv)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "load conf failed:conf[%s], ret[%d]", conf, ret);
  }
  else
  {
    if (param.get_log_name() != NULL)
    {
      YYSYS_LOGGER.setFileName(param.get_log_name());
    }
    YYSYS_LOGGER.setLogLevel(param.get_log_level());
    YYSYS_LOGGER.setMaxFileSize(param.get_max_log_size() * 1024L * 1024L);
...
int main_routine(const set<string> & tables, const TaskServerParam & param)
{
  ...
  for (it = tables.begin();it != tables.end(); ++it)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add table failed:table[%s], ret[%d]", (*it).c_str(), ret);
    }
  }

  // init run
  BaseServerRunner server_run(server);
  yysys::CThread server_thread;
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check server init failed:ret[%d]", ret);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "%s", "check no task for doing");
    }
    else
    {
      server_thread.join();
