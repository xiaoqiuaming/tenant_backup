...
int HdfsWritableFile::append(const common::ObDataBuffer &data)
{
  ...
  YYSYS_LOG(DEBUG, "HDFS fs handle = %ld, fd = %ld", (long)env_->fs(), (long)fd_);
  while (remain > 0)
  {
    int nwrite = hdfsWrite(env_->fs(), fd_, outp, remain);
    if (nwrite < 0)
    {
      YYSYS_LOG(ERROR, "hdfs write error already writen = %d", len);
      len = -1;
      break;
    }
...
int HdfsWritableFile::close()
{
  ...
  if (fd_ != NULL)
  {
    ...
    if (ret != 0)
    {
      YYSYS_LOG(WARN, "hdfsCloseFile error");
    }
  }
  fd_ = NULL;
...
HdfsEnv::~HdfsEnv()
{
  if (fs_ != NULL)
  {
    ...
    hdfsDisconnect(fs_);
    YYSYS_LOG(DEBUG, "call hdfs env destructor");
  }
}
...
int HdfsEnv::new_writable_file(const std::string &file_name, WritableFile *&file)
{
  ...
  YYSYS_LOG(DEBUG, "file name = %s, buffersize=%ld, replica=%ld,bs=%ld",
            file_name.c_str(), buffersize_, replica_, blocksize_);

  if (fd == NULL)
  {
    YYSYS_LOG(ERROR, "can't open hdfs file, name=%s", file_name.c_str());
    ret = -1;
  }
  else
  {
    ...
    if (file == NULL)
    {
      YYSYS_LOG(ERROR, "can't allocate memory for HdfsWritableFile");
      ret = -1;
    }
  }
...
int HdfsEnv::setup(const Env::Param *param)
{
  ...
  YYSYS_LOG(INFO, "host=%s,user=%s, port=%d, group_size=%d", hparam->host, hparam->user, hparam->port, hparam->group_size);
  ...
  {
    ret = -1;
    YYSYS_LOG(ERROR, "calling hdfsConnectAsUser failed");
  }
  else
  {
    ref();
    valid_ = true;
    YYSYS_LOG(DEBUG, "Hdfs FS handle = %ld", (long)fs_);
  }
