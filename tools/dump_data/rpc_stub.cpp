    ...
    TaskCounter & count, TaskInfo & task)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send request to task server for fetch new task failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize uncompleted task failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize new task failed:pos[%ld], ret[%d]", pos, ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "fetch new task succ:id[%lu]", task.get_id());
    }
  }
  return ret;
}
    ...
    const int64_t result_code, const char * file_name, const TaskInfo & task)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize result code failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize task info failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    if (OB_SUCCESS == result_code)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize filename failed:ret[%d]", ret);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send report task info failed:ret[%d]", ret);
    }
  }
    ...
    ObScanner & result)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "send scan data failed to server:ret[%d]", ret);
    }
    else
    {
      // step 3. deserialize the response result
    ...
    const ObScanParam & param, ObScanner & result)
{
  ...
  for (int64_t i = index; count < size; i = (i + 1) % size, ++count)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "skip version:index[%ld], size[%ld], server[%s], tablet_ver=%ld, version=%ld",
    ...
    if (OB_SUCCESS == ret)
    {
      YYSYS_LOG(DEBUG, "scan from server succ:index[%ld], size[%ld], server[%s]",
          i, size, server_addr);
      break;
    }
    else
    {
      YYSYS_LOG(WARN, "scan from server failed:index[%ld], size[%ld], server[%s], ret[%d]",
    ...
    common::ObServer & update_server)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send request to root server for find update server failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize server failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  return ret;
}
    ...
    ObSchemaManagerV2 & schema)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "serialize timestamp failed:version[%ld], ret[%d]",
          version, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send request to root server for fetch schema failed:"
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "fetch schema succ:version[%ld]", schema.get_version());
    }
  }
  return ret;
}
...
int RpcStub::get_version(const ObServer & server, const int64_t timeout, int64_t & version)
{
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send get memtable version request failed:ret[%d]", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "result code deserialize failed:ret[%d]", ret);
    }
    else
    {
      ret = result_code.result_code_;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "decode last frozen memtable version failed:ret[%d]", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "get frozen version succ:version[%ld]", version);
    }
  }
  return ret;
}
...
int RpcStub::response_finish(const int ret_code, const ObPacket * packet)
{
  ...
  {
    YYSYS_LOG(ERROR, "check packet or inner stat failed:packet[%p]", packet);
    ret = common::OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "get rpc buffer failed:ret[%d]", ret);
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize result message failed:ret[%d]", ret);
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (ret != common::OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "send response packet failed:ret[%d]", ret);
        }
      }
    }
  }
...
int RpcStub::response_fetch(const int ret_code, const TaskCounter & couter, const TaskInfo & task, ObPacket * packet)
{
  ...
  else
  {
    ...
    if ((OB_SUCCESS == ret_code) && (OB_SUCCESS == ret))
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "serialize counter failed:ret[%d]", ret);
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "serialize task faile:ret[%d]", ret);
        }
      }
    }
