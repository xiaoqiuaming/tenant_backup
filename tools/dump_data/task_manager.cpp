...
{
  YYSYS_LOG(DEBUG, "max times[%ld], max_count[%ld]", avg_times, max_count);
  if ((avg_times <= 0) || max_count <= 0)
  {
    YYSYS_LOG(WARN, "check max count failed:times[%ld], count[%ld]", avg_times, max_count);
  ...
  task_id_alloc_ = 0;
  total_task_count_ = 0;
  task_token_ = yysys::CTimeUtil::getTime();
  total_finish_time_ = MAX_WAIT_TIME;
  total_finish_count_ = 1;
  tablet_version_ = 0;
  YYSYS_LOG(INFO, "task token is created:token[%ld]", task_token_);
}
...
bool TaskManager::check_tasks(const uint64_t tablet_count)
{
  ...
  {
    ret = false;
    YYSYS_LOG(ERROR, "check tablet count failed:tablet[%lu], task[%lu]",
  ...
  else
  {
    ...
    for (it = wait_queue_.begin(); it != wait_queue_.end(); ++it)
    {
      ...
      if ((NULL != last_range ) && (last_task.get_param().get_table_name() == cur_task.get_param().get_table_name()))
      {
        ...
        {
          ret = false;
          YYSYS_LOG(ERROR, "check range failed:last[%p], cur[%p]", last_range, cur_range);
          break;
        }
        if (last_range->end_key_ != cur_range->start_key_)
        {
          YYSYS_LOG(ERROR, "check last tablet end_key not equal with cur tablet start_key:last[%s], cur[%s]",
        ...
        else
        {
          last_task = cur_task;
          last_range = last_task.get_param().get_range();
          YYSYS_LOG(DEBUG, "check last tablet end_key equal with cur tablet start_key:last[%s], cur[%s]",
              to_cstring(*last_range), to_cstring(*cur_range));
          continue;
        }
      }
      ...
      {
        ret = false;
        YYSYS_LOG(WARN, "check table frist tablet start_key is not min value:range[%s]", to_cstring(*cur_range));
        break;
      }
      if (last_range != NULL)
      {
        ...
        {
          ret = false;
          YYSYS_LOG(WARN, "check table last tablet start_key is not max value:range[%s]", to_cstring(*last_range));
          break;
        }
      }
      last_task = cur_task;
  ...
  if (true == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "repair task server list failed");
      ret = false;
    }
  }
  return ret;
...
bool TaskManager::get_tablet_version(int64_t memtable_version, int64_t &version)
{
  ...
  for (int64_t ver = memtable_version; ver > memtable_version - CS_HIST_VERSION; --ver)
  {
    ...
    while (it != wait_queue_.end())
    {
      ...
      {
        YYSYS_LOG(DEBUG, "task_id=%ld, task_param=[table:%.*s, range:%s], version=%ld",
      ...
      if (i == loc.size())
      {
        YYSYS_LOG(DEBUG, "version:%ld can't be dumped from task[%ld]", ver, it->first);
        break;
      }
      it++;
    }
    if (it == wait_queue_.end())
    {
      ...
      find = true;
      YYSYS_LOG(INFO, "version:%ld is selected", version);
      break;
    }
  }
...
int TaskManager::repair_all_tasks(int64_t & count)
{
  ...
  if (find_invalid_server)
  {
    ...
    {
      YYSYS_LOG(ERROR, "find invalid server list but not find valid list");
      ret = OB_ERROR;
    }
  }
  YYSYS_LOG(INFO, "repair task count:count[%lu]", count);
  return ret;
}
...
int TaskManager::insert_task(const TabletLocation & location, TaskInfo & task)
{
  ...
  YYSYS_LOG(DEBUG, "insert task succ:id[%lu], table_name[%.*s], range[%s], count[%lu]", task_id_alloc_,
  ...
  {
    YYSYS_LOG(TRACE, "dump task range:task[%ld], range[%p:%s], key_obj[%p:%p]", temp_it->first,
...
int TaskManager::fetch_task(TaskCounter & counter, TaskInfo & task)
{
  ...
  for (it = wait_queue_.begin(); it != wait_queue_.end(); ++it)
  {
    for (int64_t i = 0; i < it->second.get_location().size(); ++i)
    {
          ...
          it->second.get_location()[i].tablet_version_ != (tablet_version_ + 1))
      {
        ...
        YYSYS_LOG(DEBUG, "skip task[%ld], due to version compatiablility", it->first);
#endif
        continue;
      }

      YYSYS_LOG(DEBUG, "server:%s is selected, task_count = %ld",
      ...
      else
      {
        ...
        task.set_index(i);
        find_task = true;
        task.set_timestamp(yysys::CTimeUtil::getTime());
    ...
    for (it = doing_queue_.begin(); it != doing_queue_.end(); ++it)
    {
      if ((timestamp - it->second.get_timestamp()) > (avg_times_ * avg_finish_time))
      {
        ...
        for (int64_t i = 0; i < it->second.get_location().size(); ++i)
        {
          ...
          {
            YYSYS_LOG(INFO, "check task timeout:task[%lu], avg_time[%ld], timeout_times[%ld], "
  ...
  if (false == find_task)
  {
    YYSYS_LOG(DEBUG, "not find suitable task");
    ret = OB_ERROR;
  }
  else
  {
...
void TaskManager::print_access_server()
{
  ...
  {
    YYSYS_LOG(INFO, "server info:ip[%s], count[%ld]", it->first.to_cstring(), it->second);
  }
}
...
{
  YYSYS_LOG(INFO, "task token:%ld", task_token_);
  YYSYS_LOG(INFO, "max task id:%lu", task_id_alloc_);
  YYSYS_LOG(INFO, "total count:%lu", total_task_count_);
  YYSYS_LOG(INFO, "wait task count:%ld", wait_queue_.size());
  YYSYS_LOG(INFO, "doing task count:%ld", doing_queue_.size());
  YYSYS_LOG(INFO, "finish task count:%ld", complete_queue_.size());
  YYSYS_LOG(INFO, "visit server count:%ld", working_queue_.size());
  YYSYS_LOG(INFO, "merge server count:%ld", server_manager_.size());
  return OB_SUCCESS;
}
...
int TaskManager::finish_task(const bool result, const TaskInfo & task)
{
  ...
  {
    YYSYS_LOG(ERROR, "check task token failed:token[%ld], task[%ld]",
  ...
  else
  {
    ...
    if (task_count < 1)
    {
      YYSYS_LOG(WARN, "check server task count failed:task[%lu], count[%ld]", task.get_id(), task_count);
    }
    else
    {
      YYSYS_LOG(DEBUG, "server ip = %s, task_count = %ld",
    ...
    else
    {
      ...
      if (it != complete_queue_.end())
      {
        ...
        YYSYS_LOG(WARN, "find the task already finished:task[%lu]", task.get_id());
      }
      else
      {
        YYSYS_LOG(ERROR, "not find this task in doing and complete queue:task[%lu]", task.get_id());
        ret = OB_ERROR;
      }
    }
  }
  YYSYS_LOG(INFO, "finish monitor task [id=%lu] stat:wait[%lu], doing[%lu], finish[%lu], avg_time[%ld]", task.get_id(),
...
void TaskManager::dump_tablets(const char *file)
{
  ...
  if (NULL == f)
  {
    YYSYS_LOG(ERROR, "can't write tablets list file, %s", file);
    return;
  }
  ...
  if (NULL == key_buf)
  {
    YYSYS_LOG(ERROR, "can't allocate memory");
    fclose(f);
    return;
  }
