...
DEFINE_SERIALIZE(TabletLocation)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "serialize cur count failed:count[%ld], ret[%d]", cur_count_, ret); 
  }
  else
  {
    for (int64_t i = 0; i < cur_count_; ++i)
...
DEFINE_DESERIALIZE(TabletLocation)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "deserialize cur count failed:ret[%d]", ret);
  }
  else
  {
    for (int64_t i = 0; i < cur_count_; ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "deserialize location failed:index[%ld], ret[%d]", i, ret);
        break;
      }
    }
  }
