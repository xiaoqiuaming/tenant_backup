...
int TaskServerParam::load_from_config(const char * file)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      server_listen_port_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_PORT, 0);
      if (server_listen_port_ < 1024)
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check task server conf failed:port[%d]", server_listen_port_);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    log_name_ = YYSYS_CONFIG.getString(OBTS_SECTION, OBTS_LOGFILE);

    ret = load_string(log_level_, OB_MAX_LOG_LEVEL, OBTS_SECTION, OBTS_LOGLEVEL);
    if (OB_SUCCESS == ret)
    {
      max_log_size_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_MAX_LOGSIZE, 1024);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    max_visit_count_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_VISIT_COUNT, 20);
    max_timeout_times_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_TIMEOUT_TIMES, 3);
    task_queue_size_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_TASK_QUEUE_SIZE, 512);
    task_thread_count_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_TASK_THREAD_COUNT, 20);
    network_timeout_ = YYSYS_CONFIG.getInt(OBTS_SECTION, OBTS_NETWORK_TIMEOUT, 1000 * 1000);
    if ((max_visit_count_ <= 0) || (max_timeout_times_ <= 0) || (task_queue_size_ <= 0)
      || (task_thread_count_ <= 0) || (network_timeout_ <= 0))
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "check task server conf failed:count[%d], times[%d], queue[%d], "
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      root_server_port_ = YYSYS_CONFIG.getInt(OBTS_RS_SECTION, OBTS_RS_PORT, 0);
      if (root_server_port_ < 1024)
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check root server conf failed:port[%d]", root_server_port_);
      }
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    for(size_t i = 0;i < sections.size(); i++)
    {
      ...
      {
        TableConf conf;
        YYSYS_LOG(DEBUG, "section=%s", sections[i].c_str());
        ret = TableConf::loadConf(sections[i].c_str(), conf);
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "can't load conf table_name = %s", sections[i].c_str());
          break;
        }
    ...
    const char * name, bool require)
{
  ...
  if (OB_SUCCESS == ret && NULL != value)
  {
    ...
    {
      YYSYS_LOG(ERROR, "%s.%s too long, length (%d) > %d", section, name, int(strlen(value)), size);
