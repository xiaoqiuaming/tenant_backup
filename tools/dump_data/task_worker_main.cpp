...
int main(int argc, char ** argv)
{
  ...
  if ((NULL != hostname) && (NULL != log_file) && (0 != port) && (config != NULL))
  {
    ...
    YYSYS_LOGGER.setMaxFileSize(1024L * 1024L);
    ...
    if (ret == OB_SUCCESS)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "setup env failed");
      }
      else
      {
    ...
    {
      YYSYS_LOG(ERROR, "can't load config file, path=%s", config);
    }
  }
  else
  {
...
int main_routine(Env *env, const ObServer & task_server, const char * file_path, const TaskWorkerParam *param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "client init failed:ret[%d]", ret);
  }
  else
  {
    TaskInfo task;
    ...
    while (true)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        ...
        YYSYS_LOG(WARN, "fetch new task failed:ret[%d]", ret);
        usleep(RETRY_TIME);
      }
      else if (count.finish_count_ < count.total_count_)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "finish task failed:task[%ld], ret[%d]", task.get_id(), ret);
        }
        else
        {
          YYSYS_LOG(INFO, "finish task succ:task[%ld], file[%s], table_id=%ld",
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "report task failed:task[%ld], ret[%d]", task.get_id(), ret);
        }
      }
      else
      {
        ret = OB_SUCCESS;
        YYSYS_LOG(INFO, "finish all the task");
        break;
      }
    }
  }
  ...
  *envp = NULL;
  ...
  else
  {
    *envp = Env::Hadoop();
    ...
    {
      YYSYS_LOG(ERROR, "can't setup hdfs env");
      ret = OB_ERROR;
    }
  }
