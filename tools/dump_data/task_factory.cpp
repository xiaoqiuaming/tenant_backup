...
TaskFactory::TaskFactory()
{
  ...
  rpc_ = NULL;
  task_manager_ = NULL;
  YYSYS_LOG(DEBUG, "in constructor task factory");
  confs_ = NULL;
}
  ...
  const ObSchemaManagerV2 * schema, RpcStub * rpc, TaskManager * manager)
{
  ...
  {
    YYSYS_LOG(ERROR, "check rpc or task manager failed:rpc[%p], task[%p], schema[%p]",
      rpc, manager, schema);
    ret = OB_ERROR;
  }
  else
...
int TaskFactory::add_table(const char * table_name)
{
  ...
  if (NULL == table_name)
  {
    YYSYS_LOG(ERROR, "check table name failed:name[%s]", table_name);
    ret = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "table already exist:table[%s]", table_name);
      ret = OB_ERROR;
    }
    else
    {
...
bool TaskFactory::is_max_rowkey(const int64_t max_len, const char rowkey[], const int64_t key_len)
{
  ...
  {
    ret = false;
    YYSYS_LOG(ERROR, "check table max_len or rowkey failed:max[%ld], key[%s], len[%ld]",
...
int TaskFactory::get_all_tablets(uint64_t & tablet_count)
{
  ...
  else
  {
    ...
    for (it = dump_tables_.begin();it != dump_tables_.end(); ++it)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check table schema failed:table[%s]", (*it).c_str());
        break;
      }
      uint64_t count = 0;
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "get table's tablet failed:table[%s], id[%lu]", (*it).c_str(),
            table->get_table_id());
        break;
      }
      else
      {
        YYSYS_LOG(DEBUG, "get table tablet succ:table[%s], count[%lu]", (*it).c_str(), count);
        tablet_count += count;
      }
    }
  }
...
int TaskFactory::init_scan_param(const char *table_name, const uint64_t table_id, uint64_t & max_len, ObScanParam & param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "get table max rowkey len failed:table[%lu], ret[%d]", table_id, ret);
  }
  else
  {
//    ret = add_all_columns(table_id, param);
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add all columns failed:table[%lu], ret[%d]", table_id, ret);
    }
  }
  return ret;
}
...
int TaskFactory::get_max_len(const uint64_t table_id, uint64_t & max_len)
{
  ...
  if (NULL == schema)
  {
    YYSYS_LOG(ERROR, "get table schema failed:table[%lu]", table_id);
    ret = OB_ERROR;
  }
  else
  {
...
int TaskFactory::add_columns_conf(const uint64_t table_id, common::ObScanParam & param, const TableConf *conf)
{
  ...
  while (itr != conf->column_end())
  {
    ...
    YYSYS_LOG(DEBUG, "columns with conf, column = %s", column_name);
    column.assign(const_cast<char *>(*itr), (int32_t)strlen(column_name));
    ret = param.add_column(column);
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "add column failed:table[%lu], column[%s], ret[%d]",
...
int TaskFactory::add_columns(const char *table_name, const uint64_t table_id, common::ObScanParam & param)
{
  ...
  {
    YYSYS_LOG(DEBUG, "add columns, using conf %s", conf->DebugString().c_str());
...
int TaskFactory::add_all_columns(const uint64_t table_id, ObScanParam & param)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "check column schema or schema count failed:column[%p], count[%d]",
        column_schema, count);
  }
  else
  {
    ...
    for (int32_t i = 0; i < count; ++i)
    {
      ...
      if (column_name != NULL)
      {
        YYSYS_LOG(DEBUG, "TableId:%ld, COLUMN=%s", table_id, column_name);
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check column name failed:table[%lu], name[%s]", table_id, column_name);
        break;
      }
      ++column_schema;
    }
...
int TaskFactory::setup_tablets_version()
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "Oceanbase Merging in process, Please dump in another time");
  }
...
int TaskFactory::get_table_tablet(const char * table_name, const uint64_t table_id, uint64_t & count)
{
  ...
  if (NULL == table_name)
  {
    YYSYS_LOG(ERROR, "check table name failed:table[%s], id[%lu]", table_name, table_id);
    ret = OB_ERROR;
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "init scan param failed:table[%s], ret[%d]", table_name, ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    while ((OB_SUCCESS == ret) && (!row_key.is_max_row()))
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "add cell failed:ret[%d]", ret);
        break;
      }
      ret = rpc_->get(root_server_, timeout_, param, scanner);
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "get root table for tablet failed:table[%lu], ret[%d]", table_id, ret);
        break;
      }
      else
      {
            ...
            && (OB_SUCCESS == (ret = iter.get_cell(&cell, &row_change))) && !row_change)
        {
          if (NULL == cell)
          {
            YYSYS_LOG(ERROR, "%s", "check cell failed");
          ...
          if (ret != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "deep copy the rowkey failed:ret[%d]", ret);
          }
          ++iter;
        }
      }
      ...
      if (ret == OB_SUCCESS)
      {
        ...
        for (++iter; iter != scanner.end(); ++iter)
        {
          ...
          else if (row_change) // && (iter != last_iter))
          {
            ...
            if (ret != OB_SUCCESS)
            {
              YYSYS_LOG(ERROR, "init new task failed:ret[%d]", ret);
              break;
            }
            else
            {
              ...
              if (ret != OB_SUCCESS)
              {
                YYSYS_LOG(ERROR, "insert new task failed:ret[%d]", ret);
                break;
              }
              ++count;
            }
          ...
          else
          {
                ...
                || (cell->column_name_.compare("3_tablet_version") == 0))
            {
              ...
              if (OB_SUCCESS == ret)
              {
                if (0 == port || port == 0)
                {
                  YYSYS_LOG(WARN, "%s", "check port or ip failed");
                }
                else
                {
                  ...
                  {
                    YYSYS_LOG(ERROR, "add addr failed:server[%d], port[%d], ret[%d]",
                        server.get_ipv4(), server.get_port(), ret);
                  }
                  else
                  {
                    YYSYS_LOG(DEBUG, "add addr succ:server[%s], version:%ld", server.to_cstring(), version);
                  }
                }
                ip = port = version = 0;
              }
    ...
    const ObRowkey & end_key, ObScanParam & scan_param, TaskInfo & task) const
{
  ...
  {
    YYSYS_LOG(WARN, "check table name or end key failed:name[%.*s]",
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "set param failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int TaskFactory::insert_new_task(const TabletLocation & list, TaskInfo & task)
{
  ...
  if (0 == list.size())
  {
    YYSYS_LOG(ERROR, "check task server list count failed:task[%lu], count[%lu]",
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "insert a new task failed:ret[%d]", ret);
  }
  else
  {
    YYSYS_LOG(TRACE, "insert a new task succ:task[%lu], table_name[%.*s], tablet[%s]",
