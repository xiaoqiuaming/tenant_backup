...
int TaskWorker::init(RpcStub * rpc, const char * data_path)
{
  ...
  {
    YYSYS_LOG(ERROR, "check data path or rpc stub failed:path[%s], temp_result[%p], stub[%p]",
  ...
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "init rpc failed:server[%s], ret[%d]", server_.to_cstring(), ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (NULL == temp_result_)
    {
      YYSYS_LOG(ERROR, "check new memory failed:mem[%p]", temp_result_);
      ret = OB_ERROR;
    }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "Env is invalid");
      ret = OB_ERROR;
    }
  }
  return ret;
...
int TaskWorker::fetch_task(TaskCounter & count, TaskInfo & task)
{
  ...
  if (check_inner_stat())
  {
    for (uint64_t i = 0; i <= MAX_RETRY_TIMES; ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "check fetch task failed:ret[%d]", ret);
        usleep(__useconds_t(RETRY_INTERVAL * (i + 1)));
      }
      else
      {
        YYSYS_LOG(INFO, "fetch task succ:task[%lu]", task.get_id());
        // the param is from local rpc buffer so deserialize to its own buffer
        ret = task.set_param(task.get_param());
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "set task its own param failed:ret[%d]", ret);
        }
        break;
      }
    }
...
int TaskWorker::report_task(const int64_t result_code, const char * file_name, const TaskInfo & task)
{
  ...
  if ((NULL != file_name) && check_inner_stat())
  {
    for (uint64_t i = 0; i <= MAX_RETRY_TIMES; ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "check report task failed:ret[%d]", ret);
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "check inner stat or file_name failed:file[%s]", file_name);
  }
  return ret;
}
    ...
    ObScanner & result)
{
  ...
  if (check_inner_stat())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "check scan task failed:ret[%d]", ret);
    }
  }
  else
  {
...
int TaskWorker::do_task(TaskOutputFile * file, const TaskInfo & task)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (conf != NULL)
    {
      YYSYS_LOG(DEBUG, "using table conf, table_id=%ld", table_id);
      file->set_table_conf(conf);
    }
    else
    {
      YYSYS_LOG(DEBUG, "do not use table conf, table_id=%ld", table_id);
    }
  }
  else
  {
    YYSYS_LOG(WARN, "safe copy scan param failed:ret[%d]", ret);
  }
  ...
  while (ret == OB_SUCCESS)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      ...
      YYSYS_LOG(ERROR, "scan failed:task[%lu], ret[%d]", task.get_id(), ret);
      break;
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "output result failed:task[%lu], ret[%d]", task.get_id(), ret);
        break;
      }
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "check result finish failed:task[%lu], ret[%d]", task.get_id(), ret);
        break;
      }
      else if (!finish)
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "modify param for next query failed:task[%lu], ret[%d]",
              task.get_id(), ret);
          break;
        }
      }
      ...
      {
        YYSYS_LOG(INFO, "do the task succ:task[%lu]", task.get_id());
        YYSYS_LOG(INFO, "Average Scan Time: [%ld]", total_scan_time / scan_times);
        YYSYS_LOG(INFO, "Average Output Time: [%ld]", total_output_time / scan_times);
        break;
      }
    }
  }
...
int TaskWorker::do_task(const char * file_name, const TaskInfo & task)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "check fopen failed:file[%s]", temp_name);
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check do task failed:task[%lu], ret[%d]", task.get_id(), ret);
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "fclose failed:file[%s], err[%d], do_task ret[%d]", temp_name, err, ret);
      err = OB_ERROR;
    }
    else if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "rename failed from temp to dest:temp[%s], dest[%s], err[%d]",
    ...
    if (ret != OB_SUCCESS)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "remove temp file failed:temp[%s], err[%d]", temp_name, err);
        err = OB_ERROR;
      }
    }
  }
...
int TaskWorker::make_file_name(const TaskInfo &task, char file_name[], const int64_t len)
{
  ...
  {
    YYSYS_LOG(ERROR, "can't get host name, due to [%d]", errno);
    ret = OB_ERROR;
  }
  else
  {
    ...
    {
      YYSYS_LOG(DEBUG, "no table conf is specified");
    }
    else
    {
      YYSYS_LOG(DEBUG, "get table_conf ,conf=%s", conf->DebugString().c_str());
    ...
    if (blen <= 0)
    {
      YYSYS_LOG(ERROR, "file name too long");
      ret = OB_ERROR;
    }
  }
...
int TaskWorker::do_task(const TaskInfo & task, char file_name[], const int64_t len)
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "can't open concole for output");
    }
  }
  return ret;
}
...
int TaskWorker::check_finish(const ObScanParam & param, const ObScanner & result, bool & finish)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "get is fullfilled failed:ret[%d]", ret);
  }
  else
  {
    YYSYS_LOG(INFO, "%s, Finished count=%ld", finish ? "Finished" : "Still has", item_count);
    INC_STAT(TASK_TOTAL_RECORD_COUNT, item_count); /* update row counts */
    ret = check_result(param, result);
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check result failed:ret[%d]", ret);
    }
  }
  return ret;
}
...
int TaskWorker::modify_param(const ObScanner & result, ModuleArena & allocator, ObNewRange & new_range, ObScanParam & param)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(ERROR, "get last row key failed:ret[%d]", ret);
  }
  else
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "get range failed:range[%p]", range);
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "deep copy new range failed:ret[%d]", ret);
      }
    }
    if (OB_SUCCESS == ret)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "deep copy last rowkey failed:ret[%d]", ret);
      }
      else
      {
        YYSYS_LOG(INFO, "construct the next scan range:%s", to_cstring(new_range));
...
int TaskWorker::output(TaskOutputFile * file, ObScanner & result) const
{
  ...
  if (NULL == file)
  {
    YYSYS_LOG(ERROR, "check output file failed:file[%p]", file);
    ret = OB_ERROR;
  }
  ...
  {
    YYSYS_LOG(INFO, "output the result");
    UNUSED(result);
    // TODO
