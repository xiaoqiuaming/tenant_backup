...
DEFINE_SERIALIZE(TaskStats)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "can't serialize vector size, pos=%ld, size=%ld", pos, buf_len);
  }
  ...
  for(size_t i = 0;ret == OB_SUCCESS && i < stats_.size(); i++)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "can't serialize TaskStats pos=%ld, size=%ld", pos, buf_len);
    }
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "can't serialize item.stat_value, pos=%ld, size=%ld", pos, buf_len);
      }
    }
  }
...
DEFINE_DESERIALIZE(TaskStats)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "can't serialize vector size, pos=%ld, size=%ld", pos, data_len);
  }
  else
  {
    for(int64_t i = 0;i < vec_size; i++)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "can't deserialize TaskStats pos=%ld, size=%ld", pos, data_len);
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "can't serialize item.stat_value, pos=%ld, size=%ld", pos, data_len);
        }
      }
      if (ret != OB_SUCCESS)
      {
...
{
  YYSYS_LOG(INFO, "Outputing Task Stats, task id [%ld]", task_id);

  for(size_t i = 0;i < stats_.size(); i++)
  {
    if (stats_[i].stat_type == TASK_SUCC_SCAN_COUNT)
    {
      YYSYS_LOG(INFO, "TASK_SUCC_SCAN_COUNT = %ld", stats_[i].stat_value);
    }
    if (stats_[i].stat_type == TASK_FAILED_SCAN_COUNT)
    {
      YYSYS_LOG(INFO, "TASK_FAILED_SCAN_COUNT = %ld", stats_[i].stat_value);
    }
    if (stats_[i].stat_type == TASK_TOTAL_RECORD_COUNT)
    {
      YYSYS_LOG(INFO, "TASK_TOTAL_RECORD_COUNT = %ld", stats_[i].stat_value);
    }
    if (stats_[i].stat_type == TASK_TOTAL_WRITE_BYTES)
    {
      YYSYS_LOG(INFO, "TASK_TOTAL_WRITE_BYTES = %ld", stats_[i].stat_value);
    }
    if (stats_[i].stat_type == TASK_TOTAL_REPONSE_BYTES)
    {
      YYSYS_LOG(INFO, "TASK_TOTAL_REPONSE_BYTES = %ld", stats_[i].stat_value);
    }
    if (stats_[i].stat_type == TASK_TOTAL_REQUEST_BYTES)
    {
      YYSYS_LOG(INFO, "TASK_TOTAL_REQUEST_BYTES = %ld", stats_[i].stat_value);
    }
  }
}
