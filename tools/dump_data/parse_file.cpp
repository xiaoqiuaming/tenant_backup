...
int main(int argc, char ** argv)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "fopen file failed:file[%s]", file_path);
    }
    else
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "check alloc memory buffer failed:buffer[%p]", buffer);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check process file failed:file[%s], ret[%d]", file_path, ret);
    }
  }
...
int process_file(char * buffer, const int64_t len, FILE * file)
{
  ...
  {
    YYSYS_LOG(ERROR, "check buffer or file failed:buff[%p], len[%ld], file[%p]", buffer, len, file);
    ret = OB_ERROR;
  }
  else
  {
    ...
    while ((read_pos = fread(buffer, 1, len, file)) > 0)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "process scanner result failed:ret[%d]", ret);
        break;
      }
      else
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "check fseek failed:file_pos[%ld], ret[%d]", file_pos, ret);
          break;
        }
      }
    }
...
int print_scanner(const char * buffer, const int64_t len, int64_t & pos, FILE * file)
{
  ...
  {
    YYSYS_LOG(ERROR, "check buffer failed:buff[%p], file[%p]", buffer, file);
    ret = OB_ERROR;
  }
  else
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check deserialize scanner failed:pos[%ld], ret[%d]", pos, ret);
    }
    else
    {
