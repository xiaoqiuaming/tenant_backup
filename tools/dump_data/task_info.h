  ...
  namespace tools
  {
    ...
    public:
      ...
      TaskInfo(const TaskInfo & other)
      {
        ...
        {
          yysys::CThreadGuard lock(&other.param_buffer_->lock_);
        ...
        /*
        YYSYS_LOG(TRACE, "construct the task info:[%p->%p], src_range[%p:%s], new_range[%p:%s]",
      ...
      void dec_ref()
      {
        if (param_buffer_)
        {
          yysys::CThreadGuard lock(&param_buffer_->lock_);
          ...
          {
            YYSYS_LOG(ERROR, "not release buffer[%p], ref_count[%ld]",
