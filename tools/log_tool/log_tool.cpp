...
int dump_log_location(ObOnDiskLogLocator& log_locator, int64_t log_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "log_locator.get_location(%ld)=>%d", log_id, err);
  }
  else
  {
...
int read_file_test(const char* path, int64_t offset)
{
  ...
  {
    YYSYS_LOG(ERROR, "file_reader.open(%s)=>%d", path, err);
  }
  else if (OB_SUCCESS != (err = file_reader.pread(buf, len, offset, read_count)))
  {
    YYSYS_LOG(ERROR, "file_reader.pread(buf=%p[%ld], offset=%ld)=>%d", buf, len, offset, err);
  }
  ...
  YYSYS_LOG(INFO, "read file '%s', offset=%ld, readcount=%ld", path, offset, read_count);
  return err;
}
...
int locate(const char* log_dir, int64_t log_id)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "find_log(log_dir=%s, log_id=%ld): INVALID ARGUMENT.", log_dir, log_id);
  }
  ...
  {
    YYSYS_LOG(ERROR, "dump_log_location(%ld)=>%d", log_id, err);
  }
  return err;
}
...
static int file_expand_to(const int fd, const int64_t file_size)
{
  ...
  {
    err = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "fstat(%d):%s", fd, strerror(errno));
  }
  for(int64_t pos = st.st_size; OB_SUCCESS == err && pos < file_size; pos += count)
  {
    ...
    {
      err = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "uniter_pwrite(pos=%ld, count=%ld)", pos, count);
    }
  }
  return err;
}
...
static int expand(const char* path, const int64_t size)
{
  ...
  {
    YYSYS_LOG(ERROR, "file_expand_to(fd=%d, size=%ld)=>%d", fd, size, err);
  }
  if (fd >= 0)
  {
    close(fd);
...
int find_align_log_id(const char* log_data, int64_t data_len, int64_t log_id, int64_t end_log_id, int64_t last_log_id)
{
  int err = OB_SUCCESS;
  int64_t offset = 0;
  int64_t previous_log_id = log_id - 1;
  bool previous_log_id_flag = false;
  if (NULL == log_data || data_len <= 0)
  {
    err = OB_INVALID_ARGUMENT;
  }
  end_log_id > 0 ? fprintf(stderr, "Given log_id [%ld] is not align.", log_id) : fprintf(stderr, "real_end_log_id [%ld] is not align.", log_id);
  while (OB_SUCCESS == err && previous_log_id > 0)
  {
    if (OB_SUCCESS != (err = locate_in_buf(log_data, data_len, previous_log_id, offset)))
    {
      YYSYS_LOG(ERROR, "dump_log(buf=%p[%ld])=>%d", log_data, data_len, err);
    }
    else if (is_align(offset, OB_DIRECT_IO_ALIGN_BITS))
    {
      end_log_id > 0 ? fprintf(stderr, "The previous align log_id is [%ld].\n", previous_log_id) : fprintf(stderr, "end_log_id [%ld] is align.", previous_log_id - last_log_id);
      previous_log_id_flag = true;
      break;
    }
    previous_log_id--;
  }
  if (!previous_log_id_flag)
  {
    fprintf(stderr, "there is no align log_id previous log_id %ld.\n", log_id);
  }
  return err;
}

//static int cat(const char* log_file, const int64_t end_log_id)
static int cat(const char* log_file, const int64_t end_log_id, const char* is_force="")
{
  int err = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(ERROR, "get_last_end_log_id()=>%d", err);
  }
...
//  {
//    YYSYS_LOG(ERROR, "write(stdout, count=%ld)=>%s", offset, strerror(errno));
//  }
//  else if (end_log_id <= 0)
//  {
//    fprintf(stderr, "dump: real_end_log_id=%ld\n", last_log_id + end_log_id);
//  }
  else if (strcmp(is_force, "force") != 0)
  {
    if (is_align(offset, OB_DIRECT_IO_ALIGN_BITS))
    {
      if (offset != write(fileno(stdout), src_buf, offset))
      {
        YYSYS_LOG(ERROR, "write(stdout, count=%ld)=>%s", offset, strerror(errno));
      ...
      {
        fprintf(stderr, "Okay,cat success!\n");
      }
      end_log_id > 0 ? fprintf(stderr, "log_id %ld is align.\n", end_log_id) : fprintf(stderr, "dump: real_end_log_id=%ld is align.\n", last_log_id + end_log_id);
    }
    else
    {
      if (OB_SUCCESS != (err = find_align_log_id(src_buf, len, end_log_id > 0 ? end_log_id : last_log_id + end_log_id, end_log_id, last_log_id)))
      {
        YYSYS_LOG(ERROR, "find_align_log_id()=>%d", err);
      }
      fprintf(stderr, "cat failed!\n");
    }
  }
  else if (strcmp(is_force, "force") == 0)
  {
    ...
    else if (OB_SUCCESS != (err = find_align_log_id(src_buf, len, end_log_id > 0 ? end_log_id : last_log_id + end_log_id, end_log_id, last_log_id)))
    {
      YYSYS_LOG(ERROR, "find_align_log_id()=>%d", err);
    }
    if (offset != write(fileno(stdout), src_buf, offset))
...
int get_align_log_id(const char* log_data, int64_t data_len, int64_t& first_align_log_id, int64_t& end_align_log_id)
{
  int err = OB_SUCCESS;
  int64_t pos = 0;
  ObLogEntry log_entry;
  ...
  while (OB_SUCCESS == err && pos < data_len)
  {
    ...
    {
      err = OB_LAST_LOG_RUINNED;
      YYSYS_LOG(ERROR, "last log broken, last_log_id=%ld, offset=%ld", end_align_log_id, pos - log_entry.get_serialize_size());
    }
    else if (check_data_integrity && OB_SUCCESS != (err = log_entry.check_data_integrity(log_data + pos)))
    {
      YYSYS_LOG(ERROR, "log_entry.check_data_integrity()=>%d", err);
    ...
    if (is_align(offset, OB_DIRECT_IO_ALIGN_BITS))
    {
      if (0 == first_align_log_id)
      {
        first_align_log_id = (int64_t)log_entry.seq_;
      }
      end_align_log_id = (int64_t)log_entry.seq_;
    }
    offset = pos + log_entry.get_log_data_len();
    pos += log_entry.get_log_data_len();
  }
  if (end_align_log_id > 0 && OB_LAST_LOG_RUINNED != err)
  {
    err = OB_SUCCESS;
  }
  return err;
...
static int find_log_id(const char* log_file)
{
  int err = OB_SUCCESS;
  const char *src_buf = NULL;
  int64_t len = 0;
  int64_t first_align_log_id = 0;
  int64_t end_align_log_id = 0;
  fprintf(stderr, "find_log_id(src=%s)\n", log_file);
  if (NULL == log_file)
  {
    err = OB_INVALID_ARGUMENT;
  }
  ...
  else if (OB_SUCCESS != (err = get_align_log_id(src_buf, len, first_align_log_id, end_align_log_id)))
  {
    YYSYS_LOG(ERROR, "get_align_log_id()=>%d", err);
  }
  else
  {
    fprintf(stderr, "[first_align_log_id=%ld, end_align_log_id=%ld]\n", first_align_log_id, end_align_log_id);
  }
  return err;
}
...
int64_t calc_new_file_num_to_add(const char* dir, int64_t du_percent, int64_t file_size)
{
  ...
  {
    err = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "statfs(dir=%s)=>%s", dir, strerror(errno));
  }
  else
  {
...
int precreate(const char* log_dir, int64_t du_percent, int64_t file_size)
{
  ...
  for(int64_t id = 1; OB_SUCCESS == err && id <= max_id; id++)
  {
    ...
    {
      err = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "open(%s):%s", fname, strerror(errno));
    }
    else if (OB_SUCCESS != (err = serialization::encode_i64(log_buf, sizeof(log_buf), data_pos, id + 1)))
    {
      YYSYS_LOG(ERROR, "encode_i64(id=%ld)=>%d", id + 1, err);
    }
    else
    {
    ...
    {
      YYSYS_LOG(ERROR, "entry.fill_header()=>%d", err);
    }
    else if (OB_SUCCESS != (err = entry.serialize(log_buf, sizeof(log_buf), pos)))
    {
      YYSYS_LOG(ERROR, "entry.serialize()=>%d", err);
    }
    else if (unintr_pwrite(fd, log_buf, sizeof(log_buf), 0) != sizeof(log_buf))
    {
      err = OB_IO_ERROR;
      YYSYS_LOG(ERROR, "unintr_pwrite(fd=%d,buf=%p[%ld])=>%s", fd, log_buf, sizeof(log_buf), strerror(errno));
    }
    else if (OB_SUCCESS != (err = file_expand_to(fd, file_size)))
    {
      YYSYS_LOG(ERROR, "file_expand_to(%ld)=>%d", file_size, err);
    }
    if (fd >= 0)
    {
      close(fd);
...
int mutator_size(ObMutator& src, int64_t& n_cell, int64_t& n_row)
{
  ...
  while ((OB_SUCCESS == err) && (OB_SUCCESS == (err = src.next_cell())))
  {
    ...
    {
      YYSYS_LOG(ERROR, "mut.get_cell()=>%d", err);
    }
...
int cell_info_resolve_table_name(ObSchemaManagerV2& sch_mgr, ObCellInfo& cell)
{
  ...
  if (OB_INVALID_ID != table_id)
  {
    ...
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(DEBUG, "sch_mge.get_table_schema(table_id=%lu)=>NULL", table_id);
    }
    else if (NULL == (table_name = table_schema->get_table_name()))
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(DEBUG, "get_table_name(table_id=%lu) == NULL", table_id);
    }
    else
    {
...
int cell_info_resolve_column_name(ObSchemaManagerV2& sch_mgr, ObCellInfo& cell)
{
  ...
  if (OB_INVALID_ID != table_id && OB_INVALID_ID != column_id)
  {
    ...
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(DEBUG, "sch_mgr.get_column_schema(table_id=%lu, column_id=%lu) == NULL", table_id, column_id);
    }
    else if(NULL == (column_name = column_schema->get_name()))
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(DEBUG, "get_column_name(table_id=%lu, column_id=%lu) == NULL", table_id, column_id);
    }
    else
    {
...
{
  int err = OB_SUCCESS;
  YYSYS_LOG(DEBUG, "dump_ob_mutator");
  mut.reset_iter();
  ...
  while (OB_SUCCESS == err && OB_SUCCESS == (err = mut.next_cell()))
  {
    ...
    if (OB_SUCCESS != (err = mut.get_cell(&cell)))
    {
      YYSYS_LOG(ERROR, "mut.get_cell()=>%d", err);
             ...
             && OB_SCHEMA_ERROR != err)
    {
      YYSYS_LOG(ERROR, "resolve_column_name(table_id=%lu, column_id=%lu)=>%d",
    ...
    {}
             ...
             && OB_SCHEMA_ERROR != err)
    {
      YYSYS_LOG(ERROR, "resolve_table_name(table_id=%lu)=>%d", cell->cell_info.table_id_, err);
    }
    else if (OB_SCHEMA_ERROR == err)
    {}
  }
...
int dump_freeze_log(ObUpsMutator &ups_mutator)
{
  ...
  {
    YYSYS_LOG(ERROR, "next cell for freeze ups_mutator fail ret=%d", err);
  }
  else if (OB_SUCCESS != (err = ups_mutator.get_mutator().get_cell(&mutator_ci)))
  {
    YYSYS_LOG(ERROR, "get mutator cell fail, err=%d", err);
  }
  else if (NULL == mutator_ci)
  {
    YYSYS_LOG(WARN, "get NULL mutator cell");
    err = OB_ERR_UNEXPECTED;
  }
  else
  {
  ...
  {
    YYSYS_LOG(ERROR, "get freeze_param from freeze ups_mutator fail ret=%d header=%p length=%d",
  ...
  else
  {
    switch (header->version)
    {
      ...
      case 4:
      {
        ...
        {
          YYSYS_LOG(ERROR, "schema_manager.deserialize()=>%d", tmp_ret);
        }
        else
        {
      ...
      default:
        err = OB_ERR_UNEXPECTED;
        YYSYS_LOG(ERROR, "freeze_param version error %d", header->version);
        break;
    }
  }
  ups_mutator.get_mutator().reset_iter();
...
int dump_mutator_header(uint64_t seq, ObUpsMutator& mutator, int64_t& n_cell, int64_t& n_row)
{
  ...
  {
    YYSYS_LOG(ERROR, "mutator_size()=>%d", err);
  }
  else
  {
...
inline int64_t memNchr(const char* s, int64_t s_n, const char* c, int64_t c_n)
{
  ...
  {
    YYSYS_LOG(ERROR, "memNchr() INVALID_ARGUMENT");
  }
  else
  {
...
int64_t next_header(const char* buf, int64_t len, int64_t pos)
{
  ...
  {
    YYSYS_LOG(ERROR, "encode_i16() FAIL");
  }
  else
  {
...
int dump_first_log_if_freeze(const char* log_file, const char* buf, const int64_t len)
{
  ...
  {
    YYSYS_LOG(ERROR, "deserialize()=>%d", err);
  }
  ...
  else
  {
    ...
    if (OB_SUCCESS != (err = dump_freeze_log(mutator)))
    {
      YYSYS_LOG(ERROR, "dump_freeze_log()=>%d", err);
    }
  }
  return err;
}
...
int dump_tablets_list(const char* buf, int64_t len)
{
  ...
  {
    YYSYS_LOG(ERROR, "decode_vi64()=>%d", err);
  }
  else if (OB_SUCCESS != (err = server.deserialize(buf, len, pos)))
  {
    YYSYS_LOG(ERROR, "server.deserialize()=>%d", err);
  }
  else if (OB_SUCCESS != (err = tablets.deserialize(buf, len, pos)))
  {
    YYSYS_LOG(ERROR, "tablets.deserialize()=>%d", err);
  }
...
int dump_log(const char* log_file, const char* buf, const int64_t len, const char* type)
{
  ...
  if (strcmp(type, "freeze") == 0)
  {
    if (OB_SUCCESS != (err = dump_first_log_if_freeze(log_file, buf, len))
      && OB_READ_NOTHING != err)
    {
      YYSYS_LOG(ERROR, "dump_first_log_if_freeze()=>%d", err);
    }
  ...
  while((dump_all_data || OB_SUCCESS == err) && pos < len)
  {
    ...
    {
      YYSYS_LOG(INFO, "read eof(buf=%p, pos=%ld, len=%ld)", buf, pos, len);
      read_eof = true;
      break;
    }
    else if (OB_SUCCESS != (err = entry.deserialize(buf, len, pos)))
    ...
    else
    {
      ...
      else if (OB_UPS_SWITCH_SCHEMA == entry.cmd_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "schema_mgr.deserialize()=>%d", err);
        }
        else
        {
      ...
      else if (OB_LOG_UPS_MUTATOR == entry.cmd_)
      {
        if (OB_SUCCESS != (err = mutator.deserialize(buf + pos, entry.get_log_data_len(), tmp_pos)))
        {
          ...
          //YYSYS_LOG(ERROR, "mutator.deserialize(seq=%ld)=>%d", (int64_t)entry.seq_, err);
        }
        else if (OB_SUCCESS != (err = dump_mutator(entry.seq_, mutator, n_cell, n_row, verbose)))
        {
          YYSYS_LOG(ERROR, "dump_mutator()=>%d", err);
        }
      }
      else if (OB_RT_REPORT_TABLETS == entry.cmd_)
      {
        ...
        {
          YYSYS_LOG(ERROR, "dump_tablets_list()=>%d", err);
        }
      }
      else if (OB_LOG_NOP == entry.cmd_)
      {
        ...
        {
          YYSYS_LOG(DEBUG, "deserialize nop log fail: log_id=%ld, pos=%ld", entry.seq_, pos);
  ...
  {
    err = OB_ERR_UNEXPECTED;
    YYSYS_LOG(WARN, "pos[%ld] != len[%ld]", pos, len);
  }
...
int dump(const char* log_file, const char* type="all")
{
  ...
  const char* src_buf = NULL;
  int64_t len = 0;
  YYSYS_LOG(DEBUG, "dump(src=%s)", log_file);
  if (NULL == log_file)
  {
    err = OB_INVALID_ARGUMENT;
  }
...
int find(const char* dir, const char* type="freeze")
{
  ...
  {
    YYSYS_LOG(ERROR, "scandir(%s)=>%s", dir, strerror(errno));
  }
  else if (0 == n)
  {
    YYSYS_LOG(WARN, "%s is empty, can not select a file", dir);
  }
  for(int64_t i = 0; OB_SUCCESS == err && i < n; i++)
  {
    ...
    {
      YYSYS_LOG(ERROR, "generate file_name error, buf=%p[%ld], dir=%s, fname=%s",
                buf, sizeof(buf), dir, list[i]->d_name);
    }
    else if (OB_SUCCESS != (err = dump(buf, type))
             && OB_READ_NOTHING != err)
    {
      YYSYS_LOG(ERROR, "dump_freeze_log_in_file(%s)=>%d", buf, err);
    }
    else if (OB_READ_NOTHING == err)
    {
      err = OB_SUCCESS;
...
int mutator_add_(ObMutator& dst, ObMutator& src)
{
  ...
  while ((OB_SUCCESS == err) && (OB_SUCCESS == (err = src.next_cell())))
  {
    ...
    {
      YYSYS_LOG(ERROR, "dst.add_cell()=>%d", err);
    }
  }
  if (OB_ITER_END == err)
  {
...
int mutator_add(ObMutator& dst, ObMutator& src, int64_t size_limit)
{
  ...
  if (dst.get_serialize_size() + src.get_serialize_size() > size_limit)
  {
    ...
    YYSYS_LOG(DEBUG, "mutator_add(): size overflow");
  }
  else if (OB_SUCCESS != (err = mutator_add_(dst, src)))
  {
    YYSYS_LOG(ERROR, "mutator_add()=>%d", err);
  }
  return err;
}
...
int serialize_mutator_log(int64_t seq, char* buf, const int64_t limit, int64_t& pos, ObUpsMutator& mutator)
{
  ...
  {
    YYSYS_LOG(ERROR, "mutator.serialize()=>%d", err);
  }
  ...
  {
    err = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "mutator.get_serialize_size()[%ld] != tmp_pos[%ld] - pos[%ld]", mutator.get_serialize_size(), tmp_pos, pos);
  }
  else
  {
    pos = tmp_pos;
...
int merge_mutator(const char* src_buf, const int64_t src_len, char* dest_buf, const int64_t dest_len, int64_t& ret_len)
{
  ...
  {
    int64_t tmp_pos = 0;
    if (ObLogGenerator::is_eof(src_buf + src_pos, src_len - src_pos))
    {
      YYSYS_LOG(INFO, "read eof(buf=%p, pos=%ld, len=%ld)", src_buf, src_pos, src_len);
      read_eof = true;
      break;
    }
    else if (OB_SUCCESS != (err = entry.deserialize(src_buf, src_len, src_pos)))
    {
      YYSYS_LOG(ERROR, "log_entry.deserialize()=>%d", err);
    }
    else if (OB_LOG_UPS_MUTATOR != entry.cmd_)
  ...
  if (OB_SUCCESS == err && !read_eof && src_pos != src_len)
  {
    err = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "pos[%ld] != len[%ld]", src_pos, src_len);
  }
  if (OB_SUCCESS == err)
  {
    ret_len = dest_pos;
...
int merge(const char* src_log_file, const char* dest_log_file)
{
  ...
  int64_t src_len = 0;
  int64_t dest_len = 0;
  YYSYS_LOG(DEBUG, "merge(src=%s, dest=%s)", src_log_file, dest_log_file);
  ...
  {
    YYSYS_LOG(ERROR, "get_file_len(%s)=>%d", src_log_file, err);
  }
  else if (OB_SUCCESS != (err = file_map_read(src_log_file, src_len, src_buf)))
  {
    YYSYS_LOG(ERROR, "file_map_read(%s)=>%d", src_log_file, err);
  }
  else if (OB_SUCCESS != (err = file_map_write(dest_log_file, src_len, dest_buf)))
  {
    YYSYS_LOG(ERROR, "file_map_write(%s)=>%d", dest_log_file, err);
  }
  else if (OB_SUCCESS != (err = merge_mutator(src_buf, src_len, dest_buf, src_len, dest_len)))
  {
    YYSYS_LOG(ERROR, "merge_mutator(src_len=%ld, dest_len=%ld)=>%d", src_len, dest_len, err);
  }
  else if (OB_SUCCESS != (err = truncate(dest_log_file, dest_len)))
  {
    YYSYS_LOG(ERROR, "truncate(%s, len=%ld)=>%d", dest_log_file, dest_len, err);
  }
  return err;
}

// ÿ��log��seq = seq + file_id
int do_convert_log_seq_2to3(const int64_t file_id, char* buf, const int64_t len)
{
  ...
  while(OB_SUCCESS == err && pos < len)
  {
    ...
    else if (OB_SUCCESS != (err = log_entry.deserialize(buf, len, pos)))
    {
      YYSYS_LOG(ERROR, "log_entry.deserialize()=>%d", err);
    }
    else
    {
      //YYSYS_LOG(DEBUG, "log_entry[seq=%ld, file_id=%ld]", log_entry.seq_, file_id);
    ...
    {
      YYSYS_LOG(ERROR, "log_entry.fill_header()=>%d", err);
    }
    else if (OB_SUCCESS != (err = log_entry.serialize(buf, len, old_pos)))
    {
      YYSYS_LOG(ERROR, "log_entry.serialize()=>%d", err);
    }
    if (OB_SUCCESS == err)
    {
      pos += log_entry.get_log_data_len();
  ...
  {
    err = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "pos[%ld] != len[%ld]", pos, len);
  }
  return err;
}
...
int reset_log_id(char* buf, int64_t len, const int64_t start_id)
{
  ...
  while(OB_SUCCESS == err && pos < len)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "set_log_seq(%ld)", real_log_id);
...
int trans2to3(const char* src_log_file, const char* dest_log_file)
{
  ...
  char* dest_buf = NULL;
  int64_t len = 0;
  YYSYS_LOG(DEBUG, "trans2to3(src=%s, dest=%s)", src_log_file, dest_log_file);
  if (NULL == src_log_file || NULL == dest_log_file)
  {
    err = OB_INVALID_ARGUMENT;
  }
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "src_log_file[%s] should end with a number:path_sep=%s, strlen(path_sep)=%ld", src_log_file, path_sep, strlen(path_sep));
  }
  ...
  {
    YYSYS_LOG(ERROR, "do_convert_log_seq_2to3(file_id=%ld)=>%d", src_file_id, err);
  }
  return err;
}
...
int gen_nop(const char* dest_log_file, int64_t n_logs)
{
  ...
  YYSYS_LOG(DEBUG, "gen_nop(dest=%s, n_logs=%ld)", dest_log_file, n_logs);
  ...
  {
    YYSYS_LOG(ERROR, "fill_nop_log(n_logs=%ld)=>%d", n_logs, err);
  }
  return err;
}
...
int reset_id(const char* src_log_file, const char* dest_log_file, int64_t start_id)
{
  ...
  char* dest_buf = NULL;
  int64_t len = 0;
  YYSYS_LOG(DEBUG, "reset_id(src=%s, dest=%s, start_id=%ld)", src_log_file, dest_log_file, start_id);
  ...
  {
    YYSYS_LOG(ERROR, "reset_log_id(start_id=%ld)=>%d", start_id, err);
  }
  return err;
}
...
int dumpi(const char* src_file)
{
  ...
  const char* src_buf = NULL;
  int64_t len = 0;
  YYSYS_LOG(DEBUG, "dumpi(src=%s)", src_file);
  if (NULL == src_file)
  {
    err = OB_INVALID_ARGUMENT;
  }
  ...
  {
    YYSYS_LOG(ERROR, "get_file_len(%s)=>%d", src_file, err);
  }
  else if (OB_SUCCESS != (err = file_map_read(src_file, len, src_buf)))
  {
    YYSYS_LOG(ERROR, "file_map_read(%s)=>%d", src_file, err);
  }
  if (0 != (len % sizeof(int64_t)))
  {
    YYSYS_LOG(WARN, "len[%ld] %% sizeof(int64_t)[%ld] != 0", len, sizeof(int64_t));
  }
...
int a2i(const char* src_file, const char* dest_file)
{
  ...
  YYSYS_LOG(DEBUG, "atoi(src=%s, dest=%s)", src_file, dest_file);
  ...
  {
    YYSYS_LOG(ERROR, "file_map_write(%s)=>%d", dest_file, err);
  }
  else
  {
  ...
  {
    err = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "truncate(file=%s, len=%ld): %s", dest_file, int_file_len, strerror(errno));
  }
  return err;
}
                     ...
                     const int64_t* id_seq, int64_t n_id)
{
  ...
  else
  {
    ...
    while(OB_SUCCESS == err && src_pos < src_len)
    {
      ...
      {
        YYSYS_LOG(DEBUG, "ignore this log[seq=%ld]", entry.seq_);
      }
      else
      {
        ...
        {
          err = OB_BUF_NOT_ENOUGH;
          YYSYS_LOG(ERROR, "dest_buf is not enough[dest_limit=%ld, cur_log_seq=%ld]", dest_limit, entry.seq_);
        }
        else
        {
...
int select(const char* src_log_file, const char* dest_log_file, const char* id_seq_file)
{
  ...
  YYSYS_LOG(DEBUG, "select(src=%s, dest=%s, id_file=%s)", src_log_file, dest_log_file, id_seq_file);
  ...
  {
    YYSYS_LOG(ERROR, "get_file_len(%s)=>%d", id_seq_file, err);
  }
  else if (OB_SUCCESS != (err = file_map_read(src_log_file, src_len, src_buf)))
  {
    YYSYS_LOG(ERROR, "file_map_read(%s)=>%d", src_log_file, err);
  ...
  {
    YYSYS_LOG(ERROR, "select_log_by_id()=>%d", err);
  }
  if (0 != (truncate(dest_log_file, dest_len)))
  {
    YYSYS_LOG(ERROR, "truncate(file=%s, len=%ld):%s", dest_log_file, dest_len, strerror(errno));
  }
  return err;
}
...
//  {
//    YYSYS_LOG(ERROR, "ob_init_memory_pool()=>%d", err);
//  }
}
...
{
  int err = 0;
  YYSYS_LOGGER.setLogLevel(getenv("log_level")?:"WARN");
  ob_init_memory_pool();
  init_log_cmd_str();
  init_action_flag_str();
  if (OB_SUCCESS != (err = g_cfg.init()))
  {
    YYSYS_LOG(ERROR, "g_cfg.init()=>%d", err);
  }
  ...
  else if (OB_NEED_RETRY != (err = CmdCall(argc, argv, cat, StrArg(log_file), IntArg(end_log_id, "0"), StrArg(is_force, "")):OB_NEED_RETRY))
  {
  ...
  else if (OB_NEED_RETRY != (err = CmdCall(argc, argv, find_log_id, StrArg(log_file)):OB_NEED_RETRY))
  {
    report_error(err, "find_log_id()=>%d", err);
  }
  else
  {
