...
//add support for arm platform by wangd 202106:e
      if (NULL == buf_)
      {
        YYSYS_LOG(ERROR, "buf_ == NULL");
      }
      else if (buf_->get_remain() < size_align)//mod support for arm platform by wangd 202106
      {
        YYSYS_LOG(ERROR, "buf.alloc(remain=%ld, size=%ld):MEM_OVERFLOW", buf_->get_remain(), size_align);//mod support for arm platform by wangd 202106
      }
      else
      {
        p = buf_->get_data() + buf_->get_position();
        buf_->get_position() += size_align;//mod support for arm platform by wangd 202106
      }
      return p;
    }
  private:
