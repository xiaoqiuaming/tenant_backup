...
struct ServerList
{
  ...
  int deserialize(char* buf, int64_t len, int64_t& pos)
  {
    ...
    {
      YYSYS_LOG(ERROR, "deserialize server_num error, err=%d", err);
    }
    for(int64_t i = 0; OB_SUCCESS == err && i < n_servers_; i++)
    {
      if (OB_SUCCESS != (err = servers_[i].deserialize(buf, len, pos)))
      {
        YYSYS_LOG(ERROR, "deserialize %ldth SERVER error, ret=%d", i, err);
      }
      else if (OB_SUCCESS != (err = serialization::decode_vi64(buf, len, pos, &reserved)))
      {
        YYSYS_LOG(ERROR, "deserialize reserve field error, ret=%d", err);
      }
    }
    return err;
  }
  char* to_str(char* buf, int64_t len, int64_t& pos)
  {
    ...
    {
      YYSYS_LOG(ERROR, "strformat()=>%d", err);
    }
...
struct RpcCfg
{
  ...
  int print() {
    YYSYS_LOG(INFO, "server[%s], slave_ups[%s]", to_cstring(server_), to_cstring(slave_ups_));
    return 0;
  }
  int err_;
...
class ClientWorker
{
 ...
 || write_thread_count + scan_thread_count + mget_thread_count > MAX_N_THREADS)
      ...
      {
        YYSYS_LOG(ERROR, "cfg_getter_.init(rs=%s, server=%s)=>%d", rs, server, err);
      }
      else if (OB_SUCCESS != (err = cfg_src_.init(&cfg_getter_, atoll(_cfg("cfg_update_interval", "1000000")))))
      {
        YYSYS_LOG(ERROR, "schema_getter.init()=>%d", err);
      }
      else
      {
        rpc_ = rpc;
    ...
    int start() {
      ...
      for(int64_t i = 0; OB_SUCCESS == err && i < get_thread_count(); i++) {
        if (0 != pthread_create(threads_ + i, NULL, (void*(*)(void*))ClientWorker::do_work, this)){
          ...
          YYSYS_LOG(ERROR, "pthread_create(%ld): %s", i, strerror(errno));
        }
      }
      return err;
    }
    ...
    int wait() {
      ...
      for(int64_t i = 0; i < get_thread_count(); i++)
      {
        ...
        if (OB_SUCCESS != err){
          YYSYS_LOG(ERROR, "threads[%ld]=>%ld", i, err);
          n_err ++;
        }
      }
      YYSYS_LOG(INFO, "all worker finish: n_err=%ld", n_err);
  ...
  protected:
    ...
    int run() {
      ...
      {
        YYSYS_LOG(ERROR, "invalid thread idx[%ld]", idx);
      }
      else if (idx < write_thread_count_)
      {
        YYSYS_LOG(INFO, "write_worker[%ld] start.", idx);
        err = write(get_thread_idx());
        __sync_fetch_and_add(&finish_write_thread_count_, 1);
        YYSYS_LOG(INFO, "write_worker[%ld] finished[%d]", idx, err);
      }
      else if (idx < write_thread_count_ + scan_thread_count_)
      {
        YYSYS_LOG(INFO, "scan_worker[%ld] start.", idx);
        err = scan(get_thread_idx());
        __sync_fetch_and_add(&finish_scan_thread_count_, 1);
        YYSYS_LOG(INFO, "scan_worker[%ld] finished[%d]", idx, err);
      }
      else
      {
        YYSYS_LOG(INFO, "mget_worker[%ld] start.", idx);
        err = mget(get_thread_idx());
        __sync_fetch_and_add(&finish_mget_thread_count_, 1);
        YYSYS_LOG(INFO, "mget_worker[%ld] finished[%d]", idx, err);
      }
      return err;
    }
    ...
    class Monitor
    {
      ...
      {}
        ...
        int report(){
          ...
          last_time_ = yysys::CTimeUtil::getTime();
          ...
          YYSYS_LOG(INFO, "worker report: write=[total=%ld:%ld][tps=%ld:%ld][thread=%ld:%ld][rt=%ld:%ld], scan=[total=%ld:%ld][qps=%ld:%ld][thread=%ld:%ld][rt=%ld:%ld], mget=[total=%ld:%ld][qps=%ld:%ld][thread=%ld:%ld][rt=%ld:%ld]",
        ...
        int monitor(int64_t duration) {
          ...
          start_time_ = yysys::CTimeUtil::getTime(); 
...
int desc_tables(ObDataBuffer& buf, ObSchemaManagerV2* schema_mgr, const char* table_name, const char*& result)
{
  ...
  YYSYS_LOG(INFO, "desc_tables(table_name='%s')", table_name);
  if (NULL == schema_mgr)
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "schema_mgr == NULL");
  }
  else if (NULL == table_name || 0 == strcmp(table_name, "*"))
  {
         ...
         OB_SUCCESS == err && it != schema_mgr->table_end(); ++it)
    {
      ...
      {
        YYSYS_LOG(ERROR, "strfomat()=>%d", err);
      }
    }
  }
  else
  {
    ...
    {
      err = OB_SCHEMA_ERROR;
      YYSYS_LOG(ERROR, "schema_mgr->get_table_schema(table_name=%s)=>NULL", table_name);
    }
...
int add_columns_to_scan_param(ObDataBuffer& buf, ObScanParam& scan_param, int n_columns, char** const columns)
{
  ...
  for (int i = 0; OB_SUCCESS == err && i < n_columns; i++)
  {
    ...
    {
      YYSYS_LOG(ERROR, "alloc_str(%s)=>%d", columns[i], err);
    }
    else if (OB_SUCCESS != (err = scan_param.add_column(str)))
    {
      YYSYS_LOG(ERROR, "scan_param.add_column(%.*s)=>%d", str.length(), str.ptr(), err);
    }
  }
  return err;
}
               ...
               int n_columns, char** const columns, char* start_key, char* end_key, int64_t limit)
{
  ...
  {
    YYSYS_LOG(ERROR, "alloc_str(%s)=>%d", _table_name, err);
  }
  else if (OB_SUCCESS != (err = set_range(buf, scan_param, table_name, start_version, start_key, end_key)))
  {
    YYSYS_LOG(ERROR, "set_range(table_name=%.*s, start_version=%ld)=>%d",
  ...
  {
    YYSYS_LOG(ERROR, "add_columns_to_scan_param()=>%d", err);
  }
  else if (OB_SUCCESS != (err = scan_param.set_limit_info(0, limit)))
  {
    YYSYS_LOG(ERROR, "scan_param.set_limit_info(offset=%d, limit=%ld)=>%d", 0, limit, err);
  }
  return err;
}
               ...
               const char* table, const char* columns_spec, const char* rowkey_range, int64_t limit)
{
  ...
  char* start_key = NULL;
  char* end_key = NULL;
  YYSYS_LOG(INFO, "scan(start_version=%ld, table='%s', columns='%s', rowkey='%s')", start_version, table, columns_spec, rowkey_range);
  if (NULL == table || NULL == columns_spec || NULL == rowkey_range)
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "select(table=%s, columns=%s, rowkey=%s)=>%d", table, columns_spec, rowkey_range, err);
  }
  else if (OB_SUCCESS != (err = split(buf, columns_spec, ", ", static_cast<int>(ARRAYSIZEOF(columns)), n_columns, columns)))
  {
    YYSYS_LOG(ERROR, "split(column_spec=%s)=>%d", columns_spec, err);
  }
  else if (OB_SUCCESS != (err = parse_range(buf, rowkey_range, start_key, end_key)))
  {
    YYSYS_LOG(ERROR, "parse_range(%s)=>%d", rowkey_range, err);
  }
  else if (OB_SUCCESS != (err = scan_func(buf, scan_param, start_version, table, n_columns, columns, start_key, end_key, limit)))
  {
    YYSYS_LOG(ERROR, "scan_table(table_schema=%s, columns=%s, rowkey=[%s,%s])=>%d", table, columns_spec, start_key, end_key, err);
  }
  return err;
}
...
int set_obj(ObDataBuffer& buf, ObObj& obj, ColumnType type, const char* value)
{
  ...
  switch(type)
  {
    ...
    case ObVarcharType:
      ...
      {
        YYSYS_LOG(ERROR, "alloc_str(%s)=>%d", value, err);
      }
      else
      {
...
struct RPC : public BaseClient
{
  ...
  int desc(const char* server, const char* table, int64_t major_version)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_request()=>%d", err);
    }
    else if (OB_SUCCESS != (err = desc_tables(buf, &schema_mgr, table, result)))
    {
      YYSYS_LOG(ERROR, "desc_table()=>%d", err);
    }
    else
    {
      printf("%s\n", result);
  ...
  int malloc_stress(const char* svr, const int64_t limit)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_request(MALLOC_STRESS)=>%d", err);
    }
    return err;
  }
  ...
  int send(const char* svr, const int64_t packet, const char* file)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get_file_len(%s)=>%d", file, err);
    }
    else if (OB_SUCCESS != (err = file_map_read(file, len, cbuf)))
    {
      YYSYS_LOG(ERROR, "file_map_read(%s, %ld)=>%d", file, len, err);
    }
    else
    {
    ...
    {
      YYSYS_LOG(ERROR, "send_request(GET_MS_LIST)=>%d", err);
    }
    return err;
  }
  ...
  int get_ms(const char* rs)
  {
    ...
    {
      YYSYS_LOG(ERROR, "ms_list.to_str()=>%d", err);
    }
    else
    {
      printf("%s\n", result);
  ...
  int choose_server(const char* rs, const char* _server, ObServer& server)
  {
        ...
        && 0 != strcmp("slave_ups", _server))
    {
      ...
      {
        YYSYS_LOG(ERROR, "to_server(%s)=>%d", _server, err);
      }
    }
    else if (0 == strcmp("ms", _server))
    {
      ...
      {
        YYSYS_LOG(ERROR, "ms_list.n_servers[%d] <= 0", ms_list.n_servers_);
      }
      else
      {
    ...
    else if (0 == strcmp("mmups", _server))
    {
      ...
      {
        YYSYS_LOG(WARN, "fail to get client config, err = %d", err);
      }
      for (int32_t i = 0; OB_SUCCESS == err && i < client_config.obi_list_.obi_count_ && i < client_config.obi_list_.MAX_OBI_COUNT; ++i)
      {
        ...
        {
          YYSYS_LOG(WARN, "fail to get obi role .rs_addr=%s, err=%d", to_cstring(tmp_rs), tmp_err);
        }
  ...
  int get_server(const char* rs, const char* type)
  {
    ...
    {
      YYSYS_LOG(ERROR, "choose_server(rs=%s, type=%s)=>%d", rs, type, err);
    }
    else
    {
      printf("%s\n", server.to_cstring());
  ...
  int scan(const char* rs, const char* table, const char* columns, const char* rowkey, int64_t limit, const char* _server)
  {
    ...
    {
      YYSYS_LOG(ERROR, "repr(scanner)=>%d", err);
    }
    else
    {
      printf("%s\n", result);
  ...
  int set(const char* rs, const char* table, const char* column, const char* rowkey, const char* value, const char* _server)
  {
    ...
    {
      YYSYS_LOG(ERROR, "mutate_func()=>%d", err);
    }
    else if (OB_SUCCESS != (err = send_request(server, OB_WRITE, mutator, _dummy_)))
    {
      YYSYS_LOG(ERROR, "send_request(server=%s, write)=>%d", server.to_cstring(), err);
    }
    else
    {
      printf("%s mutate OK!\n", server.to_cstring());
  ...
  int randset(const char* rs, const char* table, const char* _server, const int64_t n_row)
  {
    ...
    for(int64_t i = 0; OB_SUCCESS == err && (n_row < 0 || i < n_row); i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "send_write(server=%s, iter=%ld)=>%d", server.to_cstring(), i, send_err);
      ...
      {
        YYSYS_LOG(INFO, "%s mutate[iter=%ld] OK!", server.to_cstring(), i);
      }
    }
    return err;
  }
  ...
  int ping_req(ObServer& server, int64_t idx, int64_t worker_id, int64_t& rt)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_ping(server=%s, iter=[%ld])=>%d", server.to_cstring(), idx, err);
    }
    else
    {
      YYSYS_LOG(DEBUG, "%s ping[%ld] OK!", server.to_cstring(), idx);
    }
    return err;
  }
  ...
  int sync_delay(const char* rs)
  {
    ...
    {
      YYSYS_LOG(ERROR, "get_master_ups()=>%d", err);
    }
    else if (OB_SUCCESS != (err = choose_server(rs, "slave_ups", slave)))
    {
      YYSYS_LOG(ERROR, "get_slave_ups()=>%d", err);
    }
    else if (OB_SUCCESS != (err = test_sync_delay(master, slave, schema_mgr, time(NULL), 0, rt)))
    {
      YYSYS_LOG(ERROR, "test_sync_delay(master=%s, slave=%s)=>%d", to_cstring(master), to_cstring(slave), err);
    }
    else
    {
      char result[1024];
  ...
  int test_sync_delay(ObServer& master, ObServer& slave, ObSchemaManagerV2& schema_mgr, int64_t start, int64_t worker_id, int64_t& rt)
  {
    ...
    {
      YYSYS_LOG(ERROR, "choose_table(table=%s)=>%d", _table_name, err);
    }
    else if (OB_SUCCESS != (err = choose_rowkey(buf, schema_mgr, table_name, rowkey, seed)))
    {
      YYSYS_LOG(ERROR, "choose_rowkey(table=%*s)=>%d", table_name.length(), table_name.ptr(), err);
    }
    else if (OB_SUCCESS != (err = choose_column(buf, schema_mgr, table_name, column_name, seed, true)))
    {
      YYSYS_LOG(ERROR, "choose_column(table=%*s)=>%d", table_name.length(), table_name.ptr(), err);
    }
    else if (OB_SUCCESS != (err = choose_value(buf, schema_mgr, table_name, column_name, cell_value, seed)))
    {
      YYSYS_LOG(ERROR, "choose_value(table=%*s, column=%*s)=>%d", table_name.length(), table_name.ptr(),
    ...
    {
      YYSYS_LOG(ERROR, "get_table_id(%.*s)=>%d", table_name.length(), table_name.ptr(), err);
    }
    else if (OB_SUCCESS != (err = get_column_id(schema_mgr, table_name, column_name, column_id)))
    {
      YYSYS_LOG(ERROR, "get_table_id(%.*s, column_name=%.*s)=>%d", table_name.length(), table_name.ptr(),
                column_name.length(), column_name.ptr(), err);
    }
    else if (OB_SUCCESS != (err = add_cell_to_get_param(get_param, table_id, rowkey, column_id)))
    {
      YYSYS_LOG(ERROR, "get_param.add_cell()=>%d", err);
    }
    else
    {
      start_version++;
      YYSYS_LOG(INFO, "start_version=%ld", start_version);
    }
    YYSYS_LOG(INFO, "mutator[table=%.*s, column_name=%.*s, rowkey=%s]",  table_name.length(), table_name.ptr(),
              column_name.length(), column_name.ptr(), to_cstring(rowkey));
    if (OB_SUCCESS != err)
    {}
    else if (0 >= start_version)
    {
      YYSYS_LOG(DEBUG, "no need to set start_version");
    }
    else if (OB_SUCCESS != (err = make_version_range(version_range, start_version)))
    {
      YYSYS_LOG(ERROR, "make_version_range(%ld)=>%d", start_version, err);
    }
    else
    {
    ...
    {
      err = send_err;
      YYSYS_LOG(ERROR, "send_write(server=%s, iter=%ld)=>%d", to_cstring(master), start, send_err);
    }
    else
    {
      ObScanner scanner;
      ObCellInfo* cell_info = NULL;
      int64_t start_us = yysys::CTimeUtil::getTime();
      ...
      for(i = 0; OB_SUCCESS == err && i < max_retry_time; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "send_request(server=%s, iter=%ld)=>%d", to_cstring(slave), start, err);
        }
        else if (OB_SUCCESS != (err = get_last_cell_from_scanner(scanner, cell_info)) && OB_ITER_END != err)
        {
          YYSYS_LOG(ERROR, "get_last_cell_from_scanner()=>%d", err);
        }
        else if (OB_ITER_END == err)
        {
          YYSYS_LOG(ERROR, "no cell in scanner");
        }
        ...
        cur_us = yysys::CTimeUtil::getTime();
      }
      if (max_retry_time == i)
      {
        err = OB_LOG_NOT_SYNC;
        YYSYS_LOG(WARN, "log_not_sync within %ld retry", max_retry_time);
      }
      else
      {
        YYSYS_LOG(INFO, "sync within %ld retry", i);
      }
    }
    return err;
  }
  ...
  int pfetch(int64_t limit)
  {
    ...
    while(OB_EAGAIN == err || OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != (err = pf.get_match_packet(&pkt, OB_PHY_PLAN_EXECUTE))
          && OB_EAGAIN != err)
      {
        YYSYS_LOG(ERROR, "get_match_packet(PHY_PLAN_EXECUTE)=>%d, stop client", err);
        err = OB_CANCELED;
      }
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "hash=%ld", pkt.get_req_sign());
      }
    }
    YYSYS_LOG(INFO, "pkt_cnt=%ld", count);
    return err;
  }
  ...
  int write_req(ObServer& server, ObSchemaManagerV2& schema_mgr, int64_t start, int64_t end, int64_t worker_id, int64_t& rt)
  {
    ...
    if (0 == strcmp(write_type, "pcap"))
    {
      ...
      else if (need_send && OB_SUCCESS != (send_err = post_request(server, packet.get_packet_code(), *packet.get_buffer())))
      {
        ...
        {
          YYSYS_LOG(ERROR, "send_write(server=%s, iter=[%ld,%ld])=>%d", to_cstring(server), start, end, send_err);
        }
      }
      else
      {
        YYSYS_LOG(DEBUG, "%s replay[%ld, %ld] OK!", to_cstring(server), start, end);
      }
    }
    else if (0 == strcmp(write_type, "mutator"))
    {
      ...
      {
        YYSYS_LOG(DEBUG, "%s mutate[%ld, %ld] OK!", to_cstring(server), start, end);
      }
    }
    if (0 == strcmp(write_type, "msmutate"))
    {
      ...
      {
        YYSYS_LOG(DEBUG, "%s msmutate[%ld, %ld] OK!", to_cstring(server), start, end);
      }
    }
...
#undef ROWKEY_IS_OBJ
  ...
  int mget_req(ObServer& server, ObSchemaManagerV2& schema_mgr, int64_t start, int64_t end, int64_t worker_id, int64_t start_version, int64_t& rt)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_request(server=%s, iter=[%ld, %ld])=>%d", server.to_cstring(), start, end, err);
    }
    else
    {
      YYSYS_LOG(DEBUG, "%s mget[%ld, %ld] OK!", server.to_cstring(), start, end);
    }
    return err;
  }
             ...
             const int64_t write_size, const int64_t scan_size, const int64_t mget_size)
  {
    ...
    YYSYS_LOG(INFO, "stress(rs=%s, server=%s, duration=%ld, range=[%ld,%ld), thread=%ld:%ld:%ld, req_size=%ld:%ld:%ld, table=%s, write_type=%s)",
    ...
    {
      YYSYS_LOG(ERROR, "worker.init()=>%d", err);
    }
    else if (OB_SUCCESS != (err = worker.start()))
    {
      YYSYS_LOG(ERROR, "worker.start()=>%d", err);
    }
    else if (OB_SUCCESS != (err = monitor.monitor(duration)))
    {
      YYSYS_LOG(ERROR, "monitor.monitor(%ld)=>%d", duration, err);
    }
    return err;
  }
  ...
  int post_stress(const char* svr, int64_t size)
  {
    ...
    else
    {
      ...
      while(true)
      {
        ...
        {
          YYSYS_LOG(ERROR, "post_request(%s)=>%d", svr, err);
          usleep(1000000);
        }
      }
      free(cbuf);
  ...
  int send_mutator(const char* rs, const char* log_file, const char* _ups)
  {
    ...
    YYSYS_LOG(DEBUG, "send_mutator(ups=%s, src=%s)", _ups, log_file);
    ...
    {
      YYSYS_LOG(ERROR, "send_request(OB_FETCH_SCHEMA)=>%d", err);
    }
    else if (OB_SUCCESS != (err = choose_server(rs, _ups, ups)))
    {
      YYSYS_LOG(ERROR, "choose_server(rs=%s, _ups=%s)=>%d", rs, _ups, err);
    }
    ...
    while(OB_SUCCESS == err && pos < len)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "ignore non mutator[seq=%ld, cmd=%d]", entry.seq_, entry.cmd_);
        }
        ...
        {
          YYSYS_LOG(DEBUG, "ignore special mutator[seq=%ld, cmd=%d]", entry.seq_, entry.cmd_);
        }
        else if (use_name && OB_SUCCESS != (err = ob_mutator_resolve_name(schema_mgr, mutator.get_mutator())))
        {
          YYSYS_LOG(ERROR, "mutator_resolve_name()=>%d", err);
        }
        ...
        {
          YYSYS_LOG(ERROR, "final_mutator.reset()=>%d", err);
        }
        ...
        {
          YYSYS_LOG(ERROR, "FAIL TO SEND MUTATOR: server=%s, seq=%ld, err=%d", ups.to_cstring(), entry.seq_, send_err);
          if (!keep_going_on_err)
          {
            err = send_err;
          }
        ...
        {
          YYSYS_LOG(DEBUG, "SUCCESS TO SEND MUTATOR: server=%s, seq=%ld", ups.to_cstring(), entry.seq_);
        }
        if (OB_SUCCESS == err)
        {
  ...
  int get_clog_status(const char* ups)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_request(ups=%s, OB_GET_CLOG_STATUS)=>%d", ups, err);
    }
    else if (OB_SUCCESS != (err = stat.to_str(buf, sizeof(buf))))
    {
      YYSYS_LOG(ERROR, "stat.to_str()=>%d", err);
    }
    else
    {
      printf("%s\n", buf);
  ...
  int get_log_sync_delay_stat(const char* ups)
  {
    ...
    {
      YYSYS_LOG(ERROR, "send_request(ups=%s, OB_GET_LOG_SYNC_DELAY_STAT)=>%d", ups, err);
    }
    else
    {
  ...
  {
    int err = OB_SUCCESS;
    printf("%s\n", yysys::CNetUtil::addrToString(ip).c_str());
    return err;
  }
...
int RpcCfgGetter::get(RpcCfg& cfg) {
  ...
  {
    YYSYS_LOG(ERROR, "send_request(rs=%s, FETCH_SCHEMA)=>%d", rs_, err);
  }
  else if (OB_SUCCESS != (err = rpc_->choose_server(rs_, type_, cfg.server_)))
  {
    YYSYS_LOG(ERROR, "choose_server(rs=%s, type=%s)=>%d", rs_, type_, err);
  }
  else if (OB_SUCCESS != (err = rpc_->choose_server(rs_, "slave_ups", cfg.slave_ups_))
           && OB_ENTRY_NOT_EXIST != err)
  {
    YYSYS_LOG(ERROR, "choose_server(rs=%s, 'slave_ups')=>%d", rs_, err);
  }
  else if (cfg.server_ == null_server)
  {
    err = OB_NEED_RETRY;
    YYSYS_LOG(ERROR, "cfg.server[%s]", to_cstring(cfg.server_));
    YYSYS_LOG(ERROR, "cfg.slave_ups[%s]", to_cstring(cfg.slave_ups_));
  }
  else
  {
    err = OB_SUCCESS;
  ...
  {
    YYSYS_LOG(INFO, "cfg change: server[%s->%s]", to_cstring(old_server), to_cstring(cfg.server_));
  }
  return err;
}
...
int ClientWorker::is_ups(bool& ret, ObServer& server) {
  ...
  {
    YYSYS_LOG(ERROR, "send_request(OB_GET_UPS)=>%d", err);
  }
...
int ClientWorker::write(int64_t idx) {
      ...
      || 0 == strcmp(write_type, "insert") || 0 == strcmp(write_type, "upcond"))
  {
    ...
    for(; !stop_ && OB_SUCCESS == err && (end_ < 0 || write_seq_ < end_);){
      ...
      else if (OB_SUCCESS != (err = rpc_->write_req(cfg->server_, cfg->schema_mgr_, write_seq, write_seq + write_req_size, idx, rt)))
      {
        ...
        if (OB_ERR_PRIMARY_KEY_DUPLICATE != err)
        {
          YYSYS_LOG(ERROR, "write_req([%ld-%ld], id=%ld)=>%d", write_seq, write_seq + write_req_size, idx, err);
        }
      }
      else
      {
  ...
  else if (0 == strcmp(write_type, "mget"))
  {
    ...
    for(; !stop_ && OB_SUCCESS == err && (end_ < 0 || write_seq_ < end_);){
      ...
      else if (OB_SUCCESS != (err = rpc_->mget_req(cfg->server_, cfg->schema_mgr_, write_seq, write_seq + write_req_size_, idx, start_version, rt)))
      {
        ...
        if (OB_ERR_PRIMARY_KEY_DUPLICATE != err)
        {
          YYSYS_LOG(ERROR, "write_req([%ld-%ld], id=%ld)=>%d", write_seq, write_seq + write_req_size_, idx, err);
        }
      }
      else
      {
  ...
  else if (0 == strcmp(write_type, "ping"))
  {
    for(; !stop_ && OB_SUCCESS == err && (end_ < 0 || write_seq_ < end_);){
      ...
      {
        __sync_fetch_and_add(&write_fail_count_, write_req_size_);
        YYSYS_LOG(ERROR, "ping_req([%ld], id=%ld)=>%d", write_seq, idx, err);
      }
      else
      {
        __sync_fetch_and_add(&write_us_, rt);
  ...
  else if (0 == strcmp(write_type, "test_sync_delay"))
  {
    for(; !stop_ && OB_SUCCESS == err && (end_ < 0 || write_seq_ < end_);) {
      ...
      else if (OB_SUCCESS != (err = rpc_->test_sync_delay(cfg->server_, cfg->slave_ups_, cfg->schema_mgr_, write_seq, idx, rt)))
      {
        ...
        YYSYS_LOG(ERROR, "test_sync_delay([%ld], id=%ld)=>%d", write_seq, idx, err);
      }
      else
      {
        __sync_fetch_and_add(&write_us_, rt);
  ...
  {
    YYSYS_LOG(ERROR, "write(type=%s): not known write_type", write_type);
  }
  return err;
}
...
int ClientWorker::mget(int64_t idx) {
  ...
  {
    YYSYS_LOG(ERROR, "send_request(get_update_server)=>%d", err);
  }
  else if (OB_SUCCESS != (err = is_ups(server_is_ups, server)))
  {
    YYSYS_LOG(ERROR, "is_ups()=>%d", err);
  }
  else if (!server_is_ups)
  {
    YYSYS_LOG(INFO, "server[%s] is not ups, don't need to set start version", server.to_cstring());
  }
  ...
  for(; !stop_ && OB_SUCCESS == err;){
    ...
    else if (OB_SUCCESS != (err = rpc_->mget_req(cfg->server_, cfg->schema_mgr_, mget_req, mget_req + mget_req_size_, idx, start_version, rt)))
    {
      ...
      YYSYS_LOG(ERROR, "mget_req([%ld-%ld], id=%ld)=>%d", mget_seq, mget_seq + mget_req_size_, idx, err);
    }
    else
    {
...
int ClientWorker::scan(int64_t idx) {
  ...
  for(; !stop_ && OB_SUCCESS == err;){
    ...
    if (scan_seq < 0)
    {
      YYSYS_LOG(INFO, "scan_seq[%ld] <= 0, need wait", scan_seq);
      usleep(100000);
    }
    else if (NULL == cfg)
    {
    ...
    else if (OB_SUCCESS != (err = rpc_->scan_req(cfg->server_, cfg->schema_mgr_, scan_seq, scan_seq + scan_req_size_, idx, start_version, rt)))
    {
      ...
      YYSYS_LOG(ERROR, "scan_req([%ld-%ld], id=%ld)=>%d", scan_seq, scan_seq + scan_req_size_, idx, err);
    }
    else
    {
...
int main(int argc, char *argv[])
{
  int err = 0;
  RPC rpc;
  YYSYS_LOGGER.setLogLevel(getenv("log_level")?:"INFO");
  if (getenv("log_file"))
    YYSYS_LOGGER.setFileName(getenv("log_file"));
  ...
  {
    YYSYS_LOG(ERROR, "rpc.initialize()=>%d", err);
  }
