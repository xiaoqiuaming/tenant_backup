...
int alloc_str(char* buf, const int64_t len, int64_t& pos, ObString& str, const char* _str)
{
  ...
  {
    YYSYS_LOG(ERROR, "strformat(buf=%p, len=%ld, pos=%ld, _str=%s)=>%d", buf, len, pos, _str, err);
  }
  else
  {
...
int rand_obj(ObDataBuffer& buf, ObObjType type, ObObj& value, int64_t seed)
{
  ...
  switch(type)
  {
    ...
    case ObVarcharType:
      ...
      {
        YYSYS_LOG(ERROR, "rand_str(len=%ld)=>%d", sizeof(_str), err);
      }
      else if (OB_SUCCESS != (err = alloc_str(buf, str, _str)))
      {
        YYSYS_LOG(ERROR, "alloc_str(str=%s)=>%d", _str, err);
      }
      else
      {
...
int rand_obj(ObDataBuffer& buf, ObObjType type, int64_t size, ObObj& value, int64_t seed)
{
  ...
  switch(type)
  {
    ...
    default:
      err = OB_NOT_SUPPORTED;
      YYSYS_LOG(ERROR, "not support type[%d]", type);
      break;
  }
  return err;
}
...
int vstrformat(char* buf, const int64_t len, int64_t& pos, const char* format, va_list ap)
{
  ...
  if (NULL == buf || 0 > len || len < pos)
  {
    ...
    YYSYS_LOG(ERROR, "strformat(buf=%s, len=%ld, pos=%ld, format='%s')=>%d", buf, len, pos, format, err);
  }
  else if (len - pos <= 0)
  {
  ...
  else
  {
    ...
    {
      err = OB_BUF_NOT_ENOUGH;
      YYSYS_LOG(ERROR, "strformat(buf=%s, len=%ld, pos=%ld, format='%s')=>[count=%ld,err=%d]",
...
int time_format(char* buf, const int64_t len, int64_t& pos, const int64_t time_us, const char *format)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "time_format(buf=%p, format=%p)=>%d", buf, format, err);
  }
  else if(NULL == localtime_r(&time_s, &time_struct))
  {
    err = OB_ERR_UNEXPECTED;
    YYSYS_LOG(ERROR, "localtime(time_us=%ld)=>NULL", time_us);
  }
  else if (len - pos <(count = strftime(buf + pos, len - pos, format, &time_struct)))
  {
    err = OB_BUF_NOT_ENOUGH;
    YYSYS_LOG(ERROR, "strftime(buf_size=%ld, format='%s')=>%d", len-pos, format, err);
  }
  return err;
}
...
const char* time2str(const int64_t time_us, const char* format)
{
  ...
  {
    YYSYS_LOG(ERROR, "time_format()=>%d", err);
  }
...
int repr(char* buf, const int64_t len, int64_t& pos, const ObObj& value)
{
  ...
  switch(_type)
  {
    ...
    case ObIntType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_int()=>%d", err);
      }
    ...
    case ObVarcharType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_varchar()=>%d", err);
      }
    ...
    case ObFloatType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_float()=>%d", err);
      }
    ...
    case ObDateTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_datetime()=>%d", err);
      }
    ...
    case ObPreciseDateTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_precise_datetime()=>%d", err);
      }
    ...
    case ObCreateTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_createtime()=>%d", err);
      }
    ...
    case ObModifyTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "value.get_modifytime()=>%d", err);
      }
...
int repr(char* buf, const int64_t len, int64_t& pos, const char* _str)
{
  ...
  if (OB_SUCCESS != (err = strformat(buf, len, pos, "%s", _str)))
  {
    YYSYS_LOG(ERROR, "strformat(_str=%p)=>%d", &_str, err);
  }
  return err;
}
...
int obstring2cstr(char* buf, const int64_t len, int64_t& pos, const char*& dest, ObString& src)
{
  ...
  {
    YYSYS_LOG(ERROR, "strformat(_str=%p)=>%d", &src, err);
  }
  else
  {
    dest = buf + old_pos;
...
int to_server(ObServer& server, const char* spec)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "spec == NULL");
  }
  else if (NULL == (p = strchr(const_cast<char*> (spec), ':')))
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "strchr(spec='%s', ':')=>NULL", spec);
  }
  else
  {
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "to_server(spec=%s)=>%d", spec, err);
  }
  else
  {
    YYSYS_LOG(DEBUG, "to_server(ip=%s, port=%d)=>%d", ip, port, err);
          ...
          int max_n_secs, int& n_secs, char** secs)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "split(buf='%s', str='%s', delim='%s', max_n_secs='%d', secs=%p)=>%d",
              buf, str, delim, max_n_secs, secs, err);
  }
  else if (OB_SUCCESS != (err = alloc_str(buf, len, pos, _str, str)))
  {
    YYSYS_LOG(ERROR, "alloc_str(buf=%p, len=%ld, pos=%ld, str='%s')=>%d", buf, len, pos, str, err);
  }
  else if (NULL == (secs[0] = strtok_r(_str.ptr(), delim, &saveptr)))
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "strtok_r(str='%s')=>NULL", _str.ptr());
  }
  else
  {
...
int parse_range(char* buf, const int64_t len, int64_t& pos, const char* _range, char*& start, char*& end)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "parse_range(buf=%p[%ld], range=%s): INVALID_ARGUMENT", buf, len, _range);
  }
  else if (OB_SUCCESS != (err = copy_str(buf, len, pos, range, _range)))
  {
    YYSYS_LOG(ERROR, "alloc_str(buf=%p, len=%ld, pos=%ld, str='%s')=>%d", buf, len, pos, _range, err);
  }
  else if ('[' != range[0] || ']' != range[strlen(range)-1])
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "ill formated range: %s", range);
  }
  else if (NULL == (start = strtok_r(range + 1, delim, &saveptr)))
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "strtok_r(str='%s')=>NULL", range);
  }
...
int rand_rowkey_obj(ObDataBuffer& buf, ObObj& obj, const ObObjType type, const int64_t len, const int64_t seed)
{
  ...
  switch(type)
  {
    ...
    case ObVarcharType:
      ...
      {
        err = OB_BUF_NOT_ENOUGH;
        YYSYS_LOG(ERROR, "pos[%ld] + len[%ld] > limit[%ld]", buf.get_position(), vchar_len, buf.get_capacity());
      }
      else
      {
      ...
      {
        YYSYS_LOG(ERROR, "rand_str(len=%ld)=>%d", vchar_len, err);
      }
      else
      {
    ...
    default:
      err = OB_NOT_SUPPORTED;
      YYSYS_LOG(WARN, "not supported type[%d]", type);
  }
  return err;
}
...
int rand_rowkey(ObDataBuffer& buf, ObRowkey& rowkey, const ObRowkeyInfo& rowkey_info, const int64_t seed)
{
  ...
  {
    err = OB_MEM_OVERFLOW;
    YYSYS_LOG(ERROR, "allocator.alloc(obj_count=%ld)=>%d", obj_count, err);
  }
  for(int64_t i = 0; OB_SUCCESS == err && i < obj_count; i++)
  {
    ...
    {
      YYSYS_LOG(ERROR, "rowkey_info.get_column(i=%ld)=>%d", i, err);
    }
    else if (OB_SUCCESS != (err = rand_rowkey_obj(buf, obj_ptr[i], column.type_, min(column.length_, max_rowkey_len), seed + i)))
    {
      YYSYS_LOG(ERROR, "rand_obj(len=%ld)=>%d", min(column.length_, max_rowkey_len), err);
    }
  }
  if (OB_SUCCESS == err)
  {
...
int rand_rowkey(ObDataBuffer& buf, ObString& rowkey, const int64_t& rowkey_len, const int64_t seed)
{
  ...
  {
    err = OB_BUF_NOT_ENOUGH;
    YYSYS_LOG(ERROR, "pos[%ld] + len[%ld] > limit[%ld]", buf.get_position(), rowkey_len, buf.get_capacity());
  }
  else
  {
    p = buf.get_data() + buf.get_position();
  ...
  {
    YYSYS_LOG(ERROR, "rand_str(len=%ld)=>%d", rowkey_len, err);
  }
  else
  {
...
int parse_servers(const char* tablet_servers, const int max_n_servers, int& n_servers, ObServer *servers)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "parse_servers(table_server=%s, max_n_servers=%d, servers=%p)=>%d",
              tablet_servers, max_n_servers, servers, err);
  }
  else if ((int)ARRAYSIZEOF(server_specs) > max_n_servers)
  {
    err = OB_BUF_NOT_ENOUGH;
    YYSYS_LOG(ERROR, "ARRAYSIZEOF(server_specs)[%ld] > max_n_servers[%d]", (int64_t)ARRAYSIZEOF(server_specs), max_n_servers);
  }
  else if (OB_SUCCESS != (err = split(buf, tablet_servers, ", ", max_n_servers, server_count, server_specs)))
  {
    YYSYS_LOG(ERROR, "split(tablet_servers='%s', max_n_servers=%d)=>%d", tablet_servers, max_n_servers, err);
  }
  else
  {
    for(int i = 0; OB_SUCCESS == err && i < server_count; i++)
    {
      ...
      {
        YYSYS_LOG(ERROR, "to_server(spec='%s')=>%d", server_specs[i], err);
      }
    }
  }
  if (OB_SUCCESS == err)
...
int expand(char* buf, int64_t len, int64_t& pos, const char* spec_)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "buf=%p[%ld:%ld], spec=%p", buf, pos, len, spec_);
  }
