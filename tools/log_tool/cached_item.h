...
class CachedItem
{
  ...
  public:
    ...
    ~CachedItem() {
      ...
      {
        YYSYS_LOG(ERROR, "pthread_rwlock_trywrlock()=>%d", err);
        abort();
      }
      else
      {
  ...
  public:
    int init(Getter* getter, int64_t update_interval_us){
      ...
      else
      {
        ...
        YYSYS_LOG(INFO, "init(update_interval_us=%ld)", update_interval_us);
      }
      return err;
    }
    ...
    int release(Item* item) {
      ...
      {
        err = OB_ERROR;
        YYSYS_LOG(ERROR, "release(): NOT MATCH");
      }
      else
      {
        pthread_rwlock_unlock(&lock_);
    ...
    int update(){
      ...
      {
        YYSYS_LOG(DEBUG, "update(%ld): already in process", seq);
      }
      else
      {
        pthread_rwlock_wrlock(&lock_);
        ...
        {
          YYSYS_LOG(ERROR, "update_cfg())=>%d", err);
        }
        else
        {
          last_update_time_us_ = yysys::CTimeUtil::getTime();
        }
        pthread_rwlock_unlock(&lock_);
