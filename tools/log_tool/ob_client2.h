...
int dump_to_file(const char* file, const char* buf, int64_t len)
{
  ...
  {
    err = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "open(%s)=>%s", file, strerror(errno));
  }
  else if ((write_cnt = write(fd, buf, len)) != len)
  {
    err = OB_IO_ERROR;
    YYSYS_LOG(ERROR, "write fail, request count=%ld, done count=%ld", len, write_cnt);
  }
  if (fd >= 0)
  {
    close(fd);
...
int uni_serialize<ObDataBuffer>(const ObDataBuffer &data, char *buf, const int64_t data_len, int64_t& pos)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "INVALID_ARGUMENT buf=%p[%ld:%ld]", buf, pos, data_len);
  }
  else if (pos + data.get_position() > data_len)
  {
    err = OB_BUF_NOT_ENOUGH;
    YYSYS_LOG(ERROR, "pos[%ld] + data_len[%ld] > buf_limit[%ld]", pos, data.get_position(), data_len);
  }
  else
  {
                 ...
                 ObDataBuffer& buf, const int64_t timeout)
{
  ...
  rt = yysys::CTimeUtil::getTime() - start_time;
  return err;
}
                 ...
                 ObDataBuffer& buf, const int64_t timeout)
{
  ...
  {
    YYSYS_LOG(ERROR, "serialize(buf=%p[%ld:%ld])=>%d", buf.get_data(), buf.get_position(), buf.get_capacity(), err);
  }
  else
  {
  ...
  {
    YYSYS_LOG(DEBUG, "result_code.result_code = %d", err);
    if (OB_PACKET_CHECKSUM_ERROR == result_code.result_code_)
    {
      ...
      YYSYS_LOG(ERROR, "packet body checksum error: pcode=%d server=%s checksum=%ld", pcode, to_cstring(server), crc);
      if (OB_SUCCESS != dump_to_file(formater.format("crc_error.%x", crc), data_buf, data_len))
      {
        YYSYS_LOG(ERROR, "write crc fail");
      }
    }
  }
...
class TbnetBaseClient
{
  ...
  public:
    virtual int initialize(int64_t n_transport = 1) {
      ...
      YYSYS_LOG(INFO, "client[n_transport=%ld]", n_transport);
      if (0 >= n_transport)
      {
        err = OB_INVALID_ARGUMENT;
      }
      for(int64_t i = 0; i < n_transport; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "client_mgr.initialize()=>%d", err);
        }
        else if (!transport_[i].start())
        {
          err = OB_ERROR;
          YYSYS_LOG(ERROR, "transport.start()=>%d", err);
        }
      }
      if (OB_SUCCESS == err)
      {
    ...
    int send_request(const Server server_spec, const int pcode,
      ...
      {
        YYSYS_LOG(ERROR, "invalid server");
      }
  ...
  protected:
    ...
    yynet::DefaultPacketStreamer streamer_;
    ObPacketFactory factory_;
    yynet::Transport transport_[MAX_N_TRANSPORT];
...
struct AsyncReqMonitor: public IObAsyncClientCallback
{
  ...
  int handle_response(ObAckQueue::WaitNode& node) {
    ...
    else if (!(node.server_ == null_server))
    {
      ...
      __sync_fetch_and_add(&wait_time_, yysys::CTimeUtil::getTime() - node.send_time_us_);
    }
    return OB_SUCCESS;
  }
  ...
  //add lbzhong [Ob Paxos] 20160224:b
...
struct AsyncReqReporter {
  ...
  void report() {
    ...
    YYSYS_LOG(INFO, "AsyncReqReport: tps=[%ld][%ld], fail=[%ld][%ld]",
...
class EasyBaseClient
{
  ...
  public:
    ...
    virtual int initialize(int64_t io_thread_count = 1, int64_t dedicate_thread_count=1, int64_t n_client = 1, int64_t ack_queue_len = 4096) {
      int err = OB_SUCCESS;
      YYSYS_LOG(INFO, "client[io_thread_count=%ld, n_client=%ld]", io_thread_count, n_client);
      ...
      for(int64_t i = 0; OB_SUCCESS == err && i < n_client; i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "easy_eio_start(i=%ld)=>%d", i, err);
        }
      }
      if (OB_SUCCESS != err)
      {}
      ...
      {
        YYSYS_LOG(ERROR, "ack_queue.init()=>%d", err);
      }
      else
      {
        n_client_ = n_client;
    ...
    int send_request(const Server server_spec, const int pcode,
      ...
      if (OB_SUCCESS != (err = to_server(server, server_spec)))
      {
        YYSYS_LOG(ERROR, "invalid server(%s)", to_cstring(server_spec));
      }
    ...
    int post_request(const Server server_spec, const int pcode,
      ...
      {
        YYSYS_LOG(ERROR, "post_to_servers(pkt=%d, seq=%ld)=>%d", pcode, seq, err);
      }
      return err;
    }
