...
void ObGetQueryTest::loop()
{
  ...
  YYSYS_LOG(INFO, "get query test stopped. bye");
}
...
void ObGetQueryTest::start_read_threads(int thread_num)
{
  ...
  for (int i = 0; i < thread_num; i++)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(WARN, "fail to start write thread. ret=%d", ret);
    }
  }
}
...
int ObGetQueryTest::start_thread(void *(*routine)(void*), void *param)
{
  ...
  if (0 != ret)
  {
    YYSYS_LOG(WARN, "fail to create thread");
  }
  return ret;
}
...
{
  YYSYS_LOG(INFO, "read thread routine");
  ...
  while (true)
  {
    if ( query->param_.write_idx_ > 1)
    {
      ...
      if (0 != ret)
      {
        YYSYS_LOG(WARN, "fail to read data. exit");
        query->stop();
        break;
      }
    }
...
{
  YYSYS_LOG(INFO, "write thread routine");
  ObGetQueryTest * query = reinterpret_cast<ObGetQueryTest *>(param);
  ObSqlWriter writer;
  writer.open();  
  ...
  while (start_idx < query->param_.expect_count_ )
  {
    ...
    if (0 != idx)
    {
      YYSYS_LOG(WARN, "fail to write row[%ld]. exit", idx);
      query->stop();
      break;
    }
    ...
    if (0 == anti_counter--)
    {
      YYSYS_LOG(INFO, "wrote row:%ld", query->param_.write_idx_);
      anti_counter = 10; //query->param_.batch_write_count_;
    }
  }
  writer.close();  
  YYSYS_LOG(INFO, "exit write thread");
  return NULL;
}
