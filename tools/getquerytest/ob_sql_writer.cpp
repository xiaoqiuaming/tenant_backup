...
int ObSqlWriter::write(int64_t start, int64_t count)
{
  ...
  YYSYS_LOG(DEBUG, "WRITE: start=%ld, count=%ld", start, count);
  // start transaction
  ret = ObSqlGenerator::make_batch_insert_sql(buf_index, buf_index_len, buf_get, buf_get_len, start, count);
  if (0 == ret)
  {
    YYSYS_LOG(DEBUG, "insert into index and get table sql: %s,%s", buf_index, buf_get);
    if (0 == ret)
    {
  ...
  {
    YYSYS_LOG(WARN, "fail to make batch sql! ret=%d", ret);
  }
  // commit or rollback
  return ret;
}
