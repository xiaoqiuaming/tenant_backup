    ...
    if (0 == ObSqlGenerator::make_select_index_table_sql(buf, buf_len, first, last))
    {
      ...
      else
      {
        ...
        if (0 == ObSqlGenerator::make_select_get_table_sql(buf, buf_len, first, last, mysql_, row_cnt))
        {
          if (row_cnt > 0)
          {
            YYSYS_LOG(DEBUG, "select get table sql: %s", buf);
            ret = mysql_query(&mysql_, buf);
            if (0 != ret) 
            {
              YYSYS_LOG(WARN, "fail to execute %s. \nERR:%s", buf, mysql_error(&mysql_));
        ...
        {
          YYSYS_LOG(WARN, "fail to make_select_get_table_sql. fail count=%d", ObSqlGenerator::fail_counter());
        }
      }
    }
    else
    {
      YYSYS_LOG(WARN, "fail to make_select_index_table_sql. fail count=%d", ObSqlGenerator::fail_counter());
    }
  }
  return ret;
}
