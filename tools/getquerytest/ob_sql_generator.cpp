...
int ObSqlGenerator::verify(int64_t index_first, int64_t index_last, MYSQL &mysql)
{
  ...
  while(NULL != (row = mysql_fetch_row(res)))
  {
    ...
    {
      YYSYS_LOG(WARN, "NULL row. check if OB internal request timeout");
      ret = -1;
      break;
    }
    row_cnt++;
  ...
  if (0 == ret && row_cnt > 0)
  {
    ...
    {
      YYSYS_LOG(WARN, "unexpected: row=%ld, expect=%ld", row_cnt, (index_last - index_first + 1));
    }
    assert(row_cnt == (index_last - index_first + 1));
  }
  if (0 != ret)
