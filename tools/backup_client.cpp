...
int backup_database(const vector<string> &param)
{
  ...
  YYSYS_LOG(INFO,"backup database [%s], mode [%d]", backup_mode, backup_mode_id);
...
int backup_table(const vector<string> &param)
{
  ...
  YYSYS_LOG(INFO,"db_name [%.*s]", db_name.length(),db_name.ptr());
  ...
  YYSYS_LOG(INFO,"table_name [%.*s]", table_name.length(),table_name.ptr());
