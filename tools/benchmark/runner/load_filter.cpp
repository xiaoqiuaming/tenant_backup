...
bool LoadFilter::check(const int32_t type, const char packet[], const int64_t data_len) const
{
  ...
  if (FIFO_PKT_REQUEST == type)
  {
    ...
    {
      YYSYS_LOG(WARN, "check request packet failed");
    }
    else
    {
      // request type filter
      ...
      if ((true == ret) && (false == enable_read_master_))
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "check packet is consistency read failed:filter[%d], err[%d]", ret, err);
        }
        else
        {
          ret = !read_master;
      ...
      YYSYS_LOG(DEBUG, "check request:code[%d], enable[%d], master[%d], filter[%d], len[%d]",
