  ...
  if (true == check_inner_stat())
  {
    switch (packet->type)
    {
      case FIFO_PKT_REQUEST:
      {
        ...
        if (NULL == request)
        {
          YYSYS_LOG(WARN, "check request failed:req[%p]", request);
        }
      ...
      default:
      {
        YYSYS_LOG(WARN, "not supported packet type:type[%d]", packet->type);
        break;
      }
    }
  }
...
int LoadRpc::fetch_query(const ObServer & server, const int64_t timeout, QueryInfo & query)
{
  ...
  if (true == check_inner_stat())
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "get rpc buffer failed:ret[%d]", ret);
    }
  }
  else
  {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "send request to server for get request packet failed:ret[%d]", ret);
    }
    else
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "deserialize query packet failed:pos[%ld], ret[%d]", pos, ret);
    }
  }
  return ret;
}
...
int LoadRpc::send_request(const ObServer & server, Request * request, ObScanner & result)
{
  ...
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "send request failed to server:code[%d], version[%d], ret[%d]",
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(INFO, "check result code:code[%d]", ret);
      }
    }
  }
    ...
    && ((request->pcode == OB_GET_REQUEST) || (request->pcode == OB_SCAN_REQUEST)))
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(ERROR, "deserialize result failed:pos[%ld], ret[%d]", pos, err);
    }
  }

  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "check ret failed:ret[%d], code[%d], pos[%ld]", ret, result_code.result_code_, pos);
  }
  return ret;
}
