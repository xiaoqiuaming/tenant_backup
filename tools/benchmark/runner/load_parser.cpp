...
int LoadParser::is_consistency_read(const common::Request * request, bool & read_master) const
{
  ...
  if (NULL == request)
  {
    YYSYS_LOG(WARN, "check request failed:ptr[%p]", request);
    ...
    case OB_GET_REQUEST:
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "get param deserialize failed:pos[%ld], len[%d], ret[%d]", pos, request->buf_size, ret);
        }
        else
        {
    ...
    case OB_SCAN_REQUEST:
      {
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "scan param deserialize failed:pos[%ld], len[%d], ret[%d]", pos, request->buf_size, ret);
        }
        else
        {
