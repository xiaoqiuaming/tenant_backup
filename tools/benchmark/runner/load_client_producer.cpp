...
void LoadClientProducer::run(yysys::CThread* thread, void* arg)
{
  int ret = OB_SUCCESS;
  if (manager_ != NULL)
  {
...
int LoadClientProducer::get_request(const ObServer & server, const int64_t count)
{
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "check get request failed:ret[%d]", ret);
  }
  else if (filter_ != NULL)
  {
    ...
    else
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "push request failed:ret[%d]", ret);
      }
  ...
  do
  {
    ...
    else
    {
      ...
      while (cur_time - last_time_ < std_time_)
      {
        ...
        //YYSYS_LOG(ERROR, "std_ms:%ld sleep ms:%ld", std_time, sleep_ms_time);
