...
bool LoadUtil::compare_cell(const ObCellInfo & first, const ObCellInfo & second)
{
  ...
  if (!ret)
  {
  YYSYS_LOG(ERROR, "unequal object data.first.table_name=%s, first.row_key_=%s, second.row_key_=%s, column_name=%s, second.column_name=%s, first.column_id=%ld, second.column_id=%ld",
        ...
        first.value_.dump(YYSYS_LOG_LEVEL_DEBUG);
        second.value_.dump(YYSYS_LOG_LEVEL_DEBUG);
  }
  return ret;
}
...
bool LoadUtil::compare_result(ObScanner & first, ObScanner & second)
{
  ...
  {
    YYSYS_LOG(ERROR, "fail to compare.first.is_empty=%d ,second.is_empty=%d, first fullfill=%d, second fullfill=%d, first_item=%ld, second_item=%ld",
        first.is_empty(),second.is_empty(),first_fullfill,second_fullfill,first_item,second_item);
    first.dump_all(YYSYS_LOG_LEVEL_WARN);
    second.dump_all(YYSYS_LOG_LEVEL_WARN);
    ret = false;
  }
  ...
  if (ret == false)
  {
    if (OB_SUCCESS == second.next_cell())
    {
      if (OB_SUCCESS == second.get_cell(&second_cell, &second_change))
      {
        ...
        YYSYS_LOG(WARN, "fail to compare. table_name=%s, table_id=%ld, rowkey=%s",
  ...
  else
  {
    ...
    while (OB_ITER_END != first.next_cell())
    {
      if (OB_SUCCESS != second.next_cell())
      {
        YYSYS_LOG(ERROR, "unequal cell num. first cell =%ld, second cell=%ld", first.get_cell_num(), second.get_cell_num());
        ret = false;
        break;
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "fail to get first cell.");
          ret = false;
          break;
        }
        if (OB_SUCCESS != second.get_cell(&second_cell, &second_change))
        {
          YYSYS_LOG(ERROR, "fail to get second cell");
          ret = false;
          break;
        }
        if ((first_change != second_change) || (false == compare_cell(*first_cell, *second_cell)))
        {
          YYSYS_LOG(WARN, "fail to compare cell.");
          ret = false;
          break;
        }
      }
    ...
    if (true == ret)
    {
      ...
      if (!ret)
      {
        YYSYS_LOG(ERROR, "fail to compare.");
      }
    }
  }
  first.reset_iter();
...
int LoadUtil::dump_param(const ObGetParam & param)
{
  ...
  for (int64_t i = 0; i < cell_size; ++i)
  {
    ...
    if (cur_cell != NULL)
    {
      //YYSYS_LOG(INFO, "table_name:%.*s, tableid:%lu, rowkey:%.*s,"
    ...
    {
      YYSYS_LOG(ERROR, "check the cell failed:index[%ld]", i);
      ret = OB_ERROR;
      break;
    }
  }
...
bool LoadUtil::check_request(Request * request)
{
  ...
  else
  {
    ...
    if (request->pcode == 101)
    {
      ...
      {
        ret = false;
        YYSYS_LOG(ERROR, "deserialize param failed:err[%d]", err);
      }
      else if (0 == param.get_cell_size())
      {
        ret = false;
        YYSYS_LOG(ERROR, "check param cell size failed");
      }
      else
      {
