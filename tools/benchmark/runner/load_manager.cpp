...
int LoadManager::get_statement_id(const int socket_fd, uint32_t &statement_id)
{
  ...
  while (!find && retry_time < 10)
  {
    ...
    for (; it != load_queue_.array_[packet_fd].queue_.end(); ++it)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "fail to ge tiovect, ret=%d", ret);
        break;
      }
      ...
      if (record->cseq_ == socket_fd)
      {
        ...
        {
          YYSYS_LOG(ERROR, "input file error. the first packet in socket_df[%d] is not prepare_response. type=%d", socket_fd, record->obmysql_type_);
          ret = OB_ERROR;
          find = false;
        }
        else
        {
          ...
          YYSYS_LOG(DEBUG, "get statement id =%d, socked_fd=%d", statement_id, socket_fd);
          find = true;
        }
        break;
      }
  ...
  {
    YYSYS_LOG(WARN, "fail to get statement id. socket_fd=%d", socket_fd);
  }
  return ret;
}
void LoadManager::dump()
{
  ...
  {
    YYSYS_LOG(DEBUG, "queue index=%ld, packet count=%ld", i, load_queue_.array_[i].queue_.size());
  }
}
int LoadManager::pop(const int64_t pid, QueryInfo & query, const int64_t timeout)
{
  ...
  do
  {
    ...
    if (size > 0)
    {
      ...
      ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "pop packet. packet queue index=%ld, remain_count=%ld, total_count=%ld",
