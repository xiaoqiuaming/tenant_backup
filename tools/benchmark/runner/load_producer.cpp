...
void LoadProducer::run(yysys::CThread* thread, void* arg)
{
  int ret = OB_SUCCESS;
  UNUSED(thread);
  UNUSED(arg);
  if (manager_ != NULL)
  {
    ...
    for (int64_t i = 0; (i < param_.iter_times_) && !_stop; ++i)
    {
      ...
      if (ret != OB_SUCCESS)
      {
        YYSYS_LOG(WARN, "load input load file failed:ret[%d]", ret);
        break;
      }
      else
      {
        YYSYS_LOG(INFO, "load one file request succ:count[%ld]", count);
        //break;
      }
    }
    manager_->set_finish();
...
int LoadProducer::load_file(const char * file, const ObServer & server, int64_t & count)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check file failed:file[%s]", file);
  }
  else
  {
    QueryInfo query;
    ...
    count = 0;
    int64_t last_time = yysys::CTimeUtil::getTime();
    int64_t cur_time = 0;
    int64_t tmp_count = 0;
    if (pfile != NULL)
    {
      while (!_stop)
      {
        ...
        {
          YYSYS_LOG(WARN, "read packet failed:ret[%d]", ret);
          break;
        }
        ret = construct_request(header, buffer, data_len, query);
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "construct task failed:ret[%d]", ret);
          break;
        }
        ...
        if (ret != OB_SUCCESS)
        {
          YYSYS_LOG(DEBUG, "push new task failed:ret[%d]", ret);
        }
        else
        {
          ++count;
        ...
        {
          YYSYS_LOG(INFO, "producer: read count = %ld per 10Second", count - tmp_count);
          manager_->dump();
          last_time = cur_time;
          tmp_count = count;
        }
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "fopen failed:file[%s], ret[%d]", file, ret);
    }
    if (pfile != NULL)
    {
      fclose(pfile);
    ...
    char buffer[], const uint32_t max_len, uint32_t & data_len)
{
  ...
  if (len <= 0)
  {
    if (true == feof(file))
    {
      YYSYS_LOG(INFO, "read file end");
      ret = FILE_END;
    }
    else
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "fread header size failed:size[%u]", len);
    }
  }
  else if ((data_len = header.buf_size) > max_len)
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check data len too long:len[%u], max[%u]", data_len, max_len);
  }
  else
  {
    ...
    {
      YYSYS_LOG(WARN, "fread data size failed:len[%u]", data_len);
    }
  }
  return ret;
}
    ...
    const int64_t data_len, QueryInfo & query)
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "malloc memory for packet failed:buf[%p], len[%ld]", buffer, data_len);
  }
  return ret;
}
