...
{
  //YYSYS_LOG(DEBUG, "reset queryinfo. packet=%p", packet_);
  id_ = 0;
  packet_ = NULL;
}
...
DEFINE_SERIALIZE(QueryInfo)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "serialize id failed:id[%ld], ret[%d]", id_, ret);
  }
  else
  {
...
DEFINE_DESERIALIZE(QueryInfo)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "deserialize id failed:id[%ld], ret[%d]", id_, ret);
  }
  else
  {
