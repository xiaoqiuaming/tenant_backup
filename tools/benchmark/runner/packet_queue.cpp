...
int64_t PacketQueue::size(void)const
{
  ...
  //YYSYS_LOG(INFO, "packet queue size is %ld", ret);
  return ret;
}
int PacketQueue::init(const int64_t thread_count)
{
  ...
  if (array_ == NULL)
  {
    YYSYS_LOG(WARN, "fail to alloc array.");
    ret = OB_ERROR;
  }
  else
  {
    YYSYS_LOG(INFO, "thread count = %ld", thread_count_ );
  }
  //else if (OB_SUCCESS != (ret = fd_thread_map_.create(hash::cal_next_prime(512))) != OB_SUCCESS )
  //{
  //  YYSYS_LOG(WARN, "fail to create fd_thread_map, ret=%d", ret);
  //}
  return ret;
}
...
int PacketQueue::get_iovect(const QueryInfo* query_info, struct iovec *buffer, int &count)
{
  ...
  if (NULL == query_info)
  {
    YYSYS_LOG(WARN, "INVALID ARGUMENT. query_info is null");
...
int PacketQueue::filter_packet(struct iovec *buffer, const int count)
{
  ...
  if (NULL == buffer)
  {
    YYSYS_LOG(WARN, "INVALID ARGUMENT. buffer is NULL");
    ret = OB_INVALID_ARGUMENT;
  }
  char *buff = NULL;
  int64_t length = 0;
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "discard packet. str=%s", tmpbuff);
    }
  }
  if (OB_ERROR == ret)
  {
...
void PacketQueue::print_iovect(struct iovec *buffers, int count)
{
  ...
  if (count == 2)
  {
    ...
    YYSYS_LOG(INFO, "socket fd = %d, addr = %s, packet length = %d, packet seq = %d, sesseion seq =%d, login type = %d, cmd type = %d, count=%d, sql content =%s",
  ...
  else if (count == 3)
  {
    ...
    YYSYS_LOG(INFO, "socket fd = %d, addr = %s, packet length = %d, packet seq = %d, sesseion seq =%d, login type = %d, cmd type = %d, count=%d, excute sql =%s",
        record->socket_fd_, inet_ntoa_r(record->addr_), record->pkt_length_, record->pkt_seq_, record->cseq_, record->obmysql_type_, record->cmd_type_, count, ptr);
  }
}
int PacketQueue::select_queue(int &fd, struct iovec *buffer, int count)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (count == 1)
    {
      //YYSYS_LOG(DEBUG, "dispatch packet. socket_fd=%d, packet_index=%d, queue_total_size=%ld, cmd_type=%d",
    ...
    {
      //YYSYS_LOG(TRACE, "dispatch packet. socket_fd=%d, packet_index=%d, queue_total_size=%ld, cmd_type=%d, str=%.*s",
...
int PacketQueue::push(const QueryInfo& query_info, const bool read_only)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to get iovec. ret=%d", ret);
    }
    else
    {
 ...
 UNUSED(read_only);
  if (OB_SUCCESS == ret)
  {
    ...
    {
      discart_count_ ++;
      //YYSYS_LOG(DEBUG, "discart packet. discart_count=%ld", discart_count_);
      //print_iovect(buffers, count);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to select queue. ret=%d", ret);
    }
    else
    {
    }
