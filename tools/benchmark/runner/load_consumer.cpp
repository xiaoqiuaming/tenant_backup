...
void LoadConsumer::run(yysys::CThread* thread, void* arg)
{
  int ret = OB_SUCCESS;
  UNUSED(arg);
  UNUSED(thread);
  int64_t start_time= yysys::CTimeUtil::getTime();
  int64_t begin = yysys::CTimeUtil::getTime();
  int64_t end = yysys::CTimeUtil::getTime();
  int64_t process_count = 0;
  if (NULL == manager_)
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "check load manager failed:load[%p]", manager_);
  }
  else
  {
    YYSYS_LOG(DEBUG, "consumer thread start.");
    QueryInfo query;
...
//  int64_t tmp_count = 0;
    ...
    int64_t tid = index_;
    index_ ++;
    YYSYS_LOG(DEBUG, "thread = %ld index = %ld", syscall(__NR_gettid), tid);
...
//  int64_t retry_times = 0;
    ...
    while (!_stop)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        end = yysys::CTimeUtil::getTime();
        time = yysys::CTimeUtil::getTime();
//      retry_times = 0;
        process_count ++;
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "fail to get iovect.");
        }
        else
        {
          ...
          case COM_STMT_PREPARE:
            {
              ...
              {
                YYSYS_LOG(DEBUG, "start to do prepare, prepare str=%s", reinterpret_cast<char*>(iovect[1].iov_base));
                prepare_count++;
          ...
          case COM_STMT_EXECUTE:
            {
              ...
              {
                YYSYS_LOG(DEBUG, "start to do execute. prepare stmt =%s", reinterpret_cast<char*>(iovect[2].iov_base));
                execute_count++;
            ...
            break;
          case COM_STMT_CLOSE:
            YYSYS_LOG(DEBUG, "start to do close stmt.");
            ret = do_com_close_stmt(iovect, count, fd_connect, stmt_map);
            break;
          case COM_QUERY:
            {
              YYSYS_LOG(DEBUG, "start to query: socket_fd=%d, str=%s",
            ...
            break;
          default:
            YYSYS_LOG(DEBUG, "unsupport type. tpye=%d", cmd);
            break;
        }
      ...
      else
      {
        ...
        {
          YYSYS_LOG(DEBUG, "retry 1 minute, and fetch no packet. exit;");
          break;
        }
      }
      ...
      if (cur_time - start_time > 10 * 1000 * 1000)
      {
        ...
        YYSYS_LOG(INFO, "statistic: thread_index=%ld, consumer count =%ld, query_count=%ld, prepare_count=%ld, execute_count=%ld, filter_count=%ld, err_count=%ld, total_count=%ld",
...
//      tmp_count = process_count;
  ...
  YYSYS_LOG(INFO, "consumer thread is stop!, count=%ld, total_time=%ld", process_count, (end - begin)/1000000);
}
...
int LoadConsumer::do_login(const struct iovec *iovect, const int count, hash::ObHashMap<int, MYSQL> &fd_connect)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid argument. iovect is %p, login_type=%d", iovect, record->obmysql_type_);
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "new connect need build. fd=%d", (reinterpret_cast<ObMySQLCommandPacketRecord *>(iovect[0].iov_base))->cseq_);
      ...
      {
        YYSYS_LOG(DEBUG, "build connect success! socket_fd=%d, pipe_fd lable=%d", record->cseq_, my_.net.vio->sd);
        ret = OB_SUCCESS;
    ...
    {
      YYSYS_LOG(WARN, "mysql connect already in map. check it. fd=%d", record->cseq_);
      ret = OB_ERROR;
    }
  }
  return ret;
...
int LoadConsumer::do_logout(const struct iovec *iovect, const int count, hash::ObHashMap<int, MYSQL> &fd_connect)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "fail to find mysql connect. ret=%d", ret);
    }
    else if (ret == hash::HASH_EXIST)
    {
      ret = OB_SUCCESS;
      MYSQL tmp_my_;
      YYSYS_LOG(DEBUG, "erase hash item in fd_connect. cseq=%d", record->cseq_);
      fd_connect.erase(record->cseq_, &tmp_my_);
      //double check
      if (hash::HASH_EXIST == fd_connect.get(record->cseq_, tmp_my_))
      {
        YYSYS_LOG(ERROR, "fail to earse hash item in fd_connect. cseq=%d", record->cseq_);
      }
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "logout, socket_fd=%d, pipe_fd=%d", record->cseq_, my_.net.vio->sd);
   // mysql_close(&my_);
  }
  return ret;
}
    ...
    hash::ObHashMap<std::pair<int, uint32_t>, uint32_t> &statement_map)
{
  ...
  if (NULL == iovect)
  {
    YYSYS_LOG(WARN, "invalid argument. iovect is null");
    ret = OB_INVALID_ARGUMENT;
  }
  MYSQL my_;
  char host[128];
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "new connect need build. fd=%d", record->cseq_);
      ...
      {
        YYSYS_LOG(DEBUG, "prepare: socket_fd=%d, pipe_fd=%d, not find in map. str=%s", record->cseq_, my_.net.vio->sd, reinterpret_cast<char*>(iovect[1].iov_base));
        ret = OB_SUCCESS;
        fd_connect.set(record->cseq_, my_);
      }
    }
    ...
    {
      YYSYS_LOG(DEBUG, "prepare: socket_fd=%d, pipe_fd=%d, find in map. str=%.*s", record->cseq_, my_.net.vio->sd, int32_t(iovect[1].iov_len), reinterpret_cast<char*>(iovect[1].iov_base));
      ret = OB_SUCCESS;
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "write prepare packet to fd, pipe_fd=%d", my_.net.vio->sd);
   ret = write_packet_to_fd(my_.net.vio->sd, iovect ,count);
   if (OB_SUCCESS != ret)
   {
     YYSYS_LOG(WARN, "fail to write prepare statement to fd, ret=%d", ret);
   }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "start to read prepare result.fd=%d", my_.net.vio->sd);
    ret = read_prepare_result(my_.net.vio->sd, real_statement_id);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to get statememt id from fd. ret=%d", ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "get statement id for real prepare. real statement_id=%d", real_statement_id);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to get statement id. socket_fd=%d,ret=%d", record->cseq_, ret);
    }
    else
    {
      YYSYS_LOG(DEBUG, "get statement id for replay process. real statement id=%d, replay statement_id=%d", real_statement_id, replay_statement_id);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "save fd: socket id=%d, replay_statement_id=%d, real_statement_id=%d",
        record->cseq_, replay_statement_id, real_statement_id);
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "prepare success. socket_id=%d, replay_statement_id=%d, real_statement_id=%d, str=%.*s",
    ...
    hash::ObHashMap<std::pair<int, uint32_t>, uint32_t> &statement_map)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "get content from execute packet.replay_statement_id=%d",
        replay_statement_id);
    ret = statement_map.get(std::make_pair(record->cseq_, replay_statement_id), statement_id);
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "fail to get statement id. socket_fd=%d, replay statement id = %d, retry to prepare first",
    ...
    {
      ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "get statement_id in map, socket_fd=%d, replay statement id = %d, real statement id =%d, prepare stmt=%s",
  ...
  if (OB_FIND_OUT_OF_RANGE == ret && record->obmysql_type_ == OBMYSQL_EXECUTE)
  {
    ...
    YYSYS_LOG(DEBUG, "prepare statement_id not found, need to prepare first, prepare_str=%s, str=%s",
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to do prepare. ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(WARN, "fail to find my instance. record_fd=%d, ret=%d", record->cseq_, ret);
      ret = OB_ERROR;
    }
    else if (ret == hash::HASH_EXIST)
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(ERROR, "fail to get statement id. socket_fd=%d, replay statement id = %d",
          record->cseq_, replay_statement_id);
      ret = OB_FIND_OUT_OF_RANGE;
    }
    else if (ret == hash::HASH_EXIST)
    {
      ret = OB_SUCCESS;
      YYSYS_LOG(DEBUG, "get statement_id, socket_fd=%d, replay statement id = %d, real statement id =%d, prepare stmt = %s",
          record->cseq_, replay_statement_id, statement_id, reinterpret_cast<char*>(iovect[2].iov_base));
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "write execute packet to fd. fife_fd=%d, stmt=%s", my_.net.vio->sd, reinterpret_cast<char*>(iovect[2].iov_base));
    ret = write_stmt_packet_to_fd(my_.net.vio->sd, iovect, count, statement_id);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to write stmt packet to fd. ret=%d", ret);
    }
  }
  //get response
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "read execute result, fife_fd=%d", my_.net.vio->sd);
    ret = read_execute_result(my_.net.vio->sd);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to read execute result from fd, ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "execute success. satement_id=%d, replay_statement_id=%d, prepare stmt=%s",
    ...
    hash::ObHashMap<std::pair<int, uint32_t>, uint32_t> &statement_map)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "fail to find my instance. ret=%d", ret);
      ret = OB_ERROR;
    }
    else if (ret == hash::HASH_EXIST)
    {
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "fail to find statement_id. ret=%d", ret);
      ret = OB_ERROR;
    }
    else if (ret == hash::HASH_EXIST)
    {
      ...
      YYSYS_LOG(DEBUG, "get replay statement id = %d, real statement id =%d",
          replay_statement_id, statement_id);
      YYSYS_LOG(DEBUG, "earse hash item in statement_map, cseq=%d, replay_statemet_id=%d", record->cseq_, replay_statement_id);
      ...
      {
        YYSYS_LOG(ERROR, "fail to earse hash item in statement_map. cseq=%d, replay_statement_id=%d", record->cseq_, replay_statement_id);
      }
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "write close stmt packet to fd. fd=%d", my_.net.vio->sd);
    ret = write_stmt_packet_to_fd(my_.net.vio->sd, iovect, count, statement_id);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to write stmt packet to fd. ret=%d, statement_id=%d", ret, statement_id);
    }
  }
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to read close stmt result. socket fd=%d, ret=%d", my_.net.vio->sd, ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "close stmt success.statement_id=%d, replay statement_id=%d",
...
int LoadConsumer::write_stmt_packet_to_fd(int fd, const struct iovec *iovect, const int count, const uint32_t statement_id)
{
  ...
  {
    YYSYS_LOG(ERROR, "serialize packet header size failed, buffer=%p, buffer length=%ld, packet length=%d, pos=%ld",
        buffer, length, pkt_len, start_pos);
  }
  else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int1(buffer, length, record->pkt_seq_, start_pos)))
  {
    YYSYS_LOG(ERROR, "serialize packet header seq failed, buffer=%p, buffer length=%ld, seq number=%u, pos=%ld",
  ...
  {
    YYSYS_LOG(ERROR, "serialize packet command failed, buffer=%p, buffer length=%ld, cmd_type=%d, pos=%ld", buffer, length, record->cmd_type_, start_pos);
  }
  else if (OB_SUCCESS != (ret = ObMySQLUtil::store_int4(buffer, length, statement_id, start_pos)))
  {
    YYSYS_LOG(ERROR, "serialize statement id failed. buffer=%p, buffer_length=%ld, statement_id=%d, pso=%ld", buffer, length, statement_id, start_pos);
  }
  else
  {
    ...
    YYSYS_LOG(DEBUG, "write stmp packet info to fd success, packet seq=%d", record->pkt_seq_);
...
int LoadConsumer::write_packet_to_fd(int fd, const struct iovec *iovect, const int count)
{
  ...
  int64_t length = OB_MAX_PACKET_LENGTH;
  uint32_t pkt_len = uint32_t(iovect[1].iov_len) + 1;
  YYSYS_LOG(DEBUG, "packet length=%d", pkt_len);
  int64_t start_pos = 0;
  ObMySQLCommandPacketRecord *record;
  record = reinterpret_cast<ObMySQLCommandPacketRecord *>(iovect[0].iov_base);
  if (OB_SUCCESS == ret)
  {
    //YYSYS_LOG(DEBUG, "start to serialize to buffer.");
    if (OB_SUCCESS != (ret = ObMySQLUtil::store_int3(buffer, length, pkt_len, start_pos)))
    {
      YYSYS_LOG(ERROR, "serialize packet header size failed, buffer=%p, buffer length=%ld, packet length=%d, pos=%ld",
          buffer, length, pkt_len, start_pos);
    ...
    else
    {
      ...
      YYSYS_LOG(DEBUG, "write:  socket_fd=%d, pipe_fd=%d, write_len=%ld, packet_seq=%d, str=%s", record->cseq_, fd, len, record->pkt_seq_, reinterpret_cast<char*>(iovect[1].iov_base));
      hex_dump(buffer, (int32_t)OB_MYSQL_PACKET_HEADER_SIZE + pkt_len);
    }
  }
  return ret;
...
int LoadConsumer::read_packet(const int fd, char* buf, const int64_t buf_length)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "fail to read packet header from fd. fd=%d", fd);
  }
  //get packet length
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to read packet content from fd. fd=%d, content length=%d, seq=%d",
          fd, header.length_, header.seq_);
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "dump:read packet from fd. fd=%d", fd);
...
int LoadConsumer::read_fd(const int fd, char* buf, const int64_t size, const int64_t buf_length)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "invalid argument. buf is %p, fd = %d, length=%ld, size=%ld", buf, fd, buf_length, size);
  }
  int64_t count = 0;
  int64_t read_size = 0;
  ...
  if (OB_SUCCESS == ret)
  {
    //YYSYS_LOG(DEBUG, "read_time =%ld, read size =%ld", times_, size);
    times_++;
    while (true)
    {
...
int LoadConsumer::read_close_stmt_result(const int fd)
{
  int ret = OB_SUCCESS;
  UNUSED(fd);
  YYSYS_LOG(DEBUG, "No response is sent back to the client.");
  return ret;
}
int LoadConsumer::read_execute_result(const int fd)
{
  ...
  ObMySQLUtil::get_uint3(pos, header.length_);
  ObMySQLUtil::get_uint1(pos, header.seq_);
  YYSYS_LOG(DEBUG, "packet length=%d, packet seq=%d", header.length_, header.seq_);
  ...
  if (result_type == 0x00 || result_type == 0xff)
  {
    ...
    YYSYS_LOG(DEBUG, "result type is ok, or err. result_type=%d", result_type);
  }
  ...
  if (!is_finish && OB_SUCCESS == ret)
  {
    for (int64_t i = 0; i < count; i++)
    {
      ...
      YYSYS_LOG(DEBUG, "column definition [%ld], str=%s, seq=%d", i, buffer + 1, header.seq_);
    }
  }
  //eof
  if (OB_SUCCESS == ret && !is_finish && count > 0)
  {
    ...
    YYSYS_LOG(DEBUG, "packet length=%d, packet seq=%d", header.length_, header.seq_);
    //read_fd(fd, buffer, header.length_, OB_MAX_PACKET_LENGTH);
    YYSYS_LOG(DEBUG, "read execute result eof. length=%d, seq=%d", header.length_, header.seq_);
  }
  ...
  if (OB_SUCCESS == ret && !is_finish)
  {
    while(true)
    {
      ...
      ObMySQLUtil::get_uint3(pos, header.length_);
      ObMySQLUtil::get_uint1(pos, header.seq_);
      YYSYS_LOG(DEBUG, "packet header: length=%d, seq=%d,", header.length_, header.seq_);
      //read_ret = read_fd(fd, buffer, 1, OB_MAX_PACKET_LENGTH);
      //if (read_ret != OB_SUCCESS)
      //{
        //break;
      ...
      //pos = buffer;
      ObMySQLUtil::get_uint1(pos, result_type);
      YYSYS_LOG(DEBUG, "result type: type=%d", result_type);
      //read_ret = read_fd(fd, buffer, header.length_ - 1, OB_MAX_PACKET_LENGTH);
      //if (read_ret != OB_SUCCESS)
      //{
      //  break;
      //}
      YYSYS_LOG(DEBUG, "resultset: type=%d, length=%d, seq=%d, str=%s", result_type, header.length_, header.seq_, buffer);
      if (result_type == 0xfe)
      {
        YYSYS_LOG(DEBUG, "end of the read process.");
        is_finish = true;
        break;
      }
    }
...
int LoadConsumer::read_prepare_result(const int fd, uint32_t &statement_id)
{
  ...
  ObMySQLUtil::get_uint3(pos, header.length_);
  ObMySQLUtil::get_uint1(pos, header.seq_);
  YYSYS_LOG(DEBUG, "read prepare reuslt, fd=%d, packet length=%d, packet seq=%d", fd, header.length_, header.seq_);
  uint8_t status = 0;  //1
  ...
  YYSYS_LOG(DEBUG, "read prepare prarm: pipe_fd=%d, total_lenght=%d, status=%d, statement_id=%d, column_count=%d, param_count=%d", fd, header.length_, status, statement_id, column_count, param_count);
  if (status == 0)
  {
    // param group
    for (int i = 0; i < param_count; i++)
    {
      ...
      ObMySQLUtil::get_uint3(pos, header.length_);
      ObMySQLUtil::get_uint1(pos, header.seq_);
      YYSYS_LOG(DEBUG, "read prepare param group[%d]: packet length=%d, packet seq=%d", i, header.length_, header.seq_);
      //read_fd(fd, buffer, header.length_, OB_MAX_PACKET_LENGTH);
    }
    //eof
    if (param_count > 0)
    {
      ...
      ObMySQLUtil::get_uint3(pos, header.length_);
       ObMySQLUtil::get_uint1(pos, header.seq_);
        YYSYS_LOG(DEBUG, "read prepare eof: packet length=%d, packet seq=%d", header.length_, header.seq_);
      //read_fd(fd, buffer, header.length_, OB_MAX_PACKET_LENGTH);
    }
    //column group
    for (int i = 0; i < column_count; i++)
    {
      ...
      ObMySQLUtil::get_uint3(pos, header.length_);
      ObMySQLUtil::get_uint1(pos, header.seq_);
      YYSYS_LOG(DEBUG, "read prepare column group[%d]:packet length=%d, packet seq=%d", i, header.length_, header.seq_);
      //read_fd(fd, buffer, header.length_, OB_MAX_PACKET_LENGTH);
    }
    if (column_count > 0)
    {
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(ERROR, "prepare failed. state=%d, pipe_fd=%d, statement_id=%d, str=%s",
...
int LoadConsumer::read_query_result(const int fd)
{
  ...
  if (result_type == 0x00 || result_type == 0xff)
  {
    is_finish = true;
    //read_fd(fd, buffer, header.length_ - 1, OB_MAX_PACKET_LENGTH);
    YYSYS_LOG(DEBUG, "result: type is %d, pipe fd=%d, 0x00 is ok, and 0xff is error, finish read result.", result_type, fd);
  }
  else if (result_type == 0Xfb)
  {
    is_finish = true;
     //read_fd(fd, buffer, header.length_ - 1, OB_MAX_PACKET_LENGTH);
    YYSYS_LOG(DEBUG, "not support result type. result_type=%d", result_type);
  }
  else if (result_type < 0xfb)
  {
    count = result_type;
  ...
  YYSYS_LOG(DEBUG, "is_finish =%s, ret=%d, column_count=%ld", is_finish?"true":"false", ret, count);
  if (is_finish)
  {
    ret = OB_SUCCESS;
  }
  if (OB_SUCCESS == ret && !is_finish)
  {
    ...
    for (int32_t i = 0; i < count; i++)
    {
      ...
      YYSYS_LOG(DEBUG, "packet length=%d, packet seq=%d", header.length_, header.seq_);
      //read_fd(fd, buffer, header.length_, OB_MAX_PACKET_LENGTH);
      YYSYS_LOG(DEBUG, "column define[%d]: %s", i, buffer + 1);
    }
  }
  //eof
  if (OB_SUCCESS == ret && !is_finish)
  {
    ...
    ObMySQLUtil::get_uint3(pos, header.length_);
    ObMySQLUtil::get_uint1(pos, header.seq_);
    YYSYS_LOG(DEBUG, "eof, packet length=%d, packet seq=%d", header.length_, header.seq_);
    //pos = buffer;
    //read_fd(fd, buffer, 1, OB_MAX_PACKET_LENGTH);
    ObMySQLUtil::get_uint1(pos, result_type);
    //read_fd(fd, buffer, header.length_ - 1, OB_MAX_PACKET_LENGTH);
  ...
  if (OB_SUCCESS == ret && !is_finish)
  {
    ...
    while(true)
    {
      ...
      YYSYS_LOG(DEBUG, "result rows[%ld]: %s", ++i, buffer);
    }
  }
  return ret;
}
...
char host[128];
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret == -1 || hash::HASH_NOT_EXIST == ret)
    {
      YYSYS_LOG(DEBUG, "new connect need build. fd=%d, str=%s",
      ...
      {
        //YYSYS_LOG(DEBUG, "build real connect success!");
        ret = OB_SUCCESS;
  ...
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "write query to fd. fd=%d", my_.net.vio->sd);
    ret = write_packet_to_fd(my_.net.vio->sd, iovect, count);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to write packet to fd, ret=%d", ret);
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "read query result from fd. fd=%d, query str=%s", my_.net.vio->sd, reinterpret_cast<char*>(iovect[1].iov_base));
    ret = read_query_result(my_.net.vio->sd);
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "fail to get response. ret=%d", ret);
    }
    else
    {
      //YYSYS_LOG(DEBUG, "read query result from fd success. fd=%d, query str=%s", my_.net.vio->sd, reinterpret_cast<char*>(iovect[1].iov_base));
    }
  }
  if (OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "query success: str=%.*s", int32_t(iovect[1].iov_len), reinterpret_cast<char*>(iovect[1].iov_base));
  }
  return ret;
}
