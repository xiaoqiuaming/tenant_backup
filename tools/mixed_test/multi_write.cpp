...
struct CmdLine
{
  ...
  void log_all()
  {
    YYSYS_LOG(INFO, "write_addr=%s write_port=%ld schema_addr=%s schema_port=%ld read_addr=%s read_port=%ld using_id=%s "
...
int main(int argc, char **argv)
{
  ...
  for (int64_t i = 0; i < clp.mutator_num; i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "build_mutator fail ret=%d\n", ret);
      break;
    }
    while (true)
    {
      ...
      YYSYS_LOG(INFO, "apply ret=%d timeu=%ld\n", ret, yysys::CTimeUtil::getTime() - timeu);
      if (OB_SUCCESS == ret)
      {
        break;
      }
