                  ...
                  const bool using_id)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "client.get(get_param.cell_size()=>%ld, scanner.cell_size()=>%ld)",
                get_param.get_cell_size(), scanner.get_cell_num());
      if (get_param.get_cell_size() > 0)
      {
        YYSYS_LOG(WARN, "%s", print_cellinfo(get_param[0]));
      }
      scanner.dump();
    }
    RowChecker rc;
    while (OB_SUCCESS == scanner.next_cell())
    {
      ...
      if (OB_SUCCESS == scanner.get_cell(&ci))
      {
        ...
        {
          bret = false;
          YYSYS_LOG(ERROR, "[%s] row not exist!", to_cstring(row_key));
          break;
        }
        rc.add_cell(ci);
      }
