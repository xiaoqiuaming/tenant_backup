                              ...
                              ClientWrapper *client4write)
{
  ...
  {
    YYSYS_LOG(WARN, "schema cannot pass check");
  }
  else
  {
    ...
    for (int64_t i = 0; i < table_num_; i++)
    {
      ...
      if (0 == tmp_suffix_length
          ...
          || 0 == tmp_prefix_end)
      {
        YYSYS_LOG(WARN, "suffix_length=%ld suffix_num=%ld prefix_end=%ld",
...
int64_t MutatorBuilder::get_cur_seed_(const seed_map_t &seed_map, const ObSchema &schema, const ObString &row_key, const bool using_id)
{
  ...
  if (HASH_EXIST != (hash_ret = seed_map.get(te_key, cur_seed)))
  {
    ...
    if (OB_SUCCESS == client_.get(get_param, scanner))
    {
      ...
      if (0 != result_set.size())
      {
        ...
        YYSYS_LOG(DEBUG, "seed get from oceanbase %s", print_cellinfo(ret_ci));
      }
    }
  }
  if (0 == cur_seed)
  {
    cur_seed = SEED_START;
    YYSYS_LOG(DEBUG, "first seed from cur_seed=%ld row_key=[%.*s]", cur_seed, row_key.length(), row_key.ptr());
  }
  else
  {
    cur_seed += 1;
  }
  YYSYS_LOG(DEBUG, "get cur_seed=%ld hash_ret=%d row_key=[%.*s]", cur_seed, hash_ret, row_key.length(), row_key.ptr());
  return cur_seed;
}
