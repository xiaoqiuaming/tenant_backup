...
struct CmdLine
{
  ...
  void log_all()
  {
    YYSYS_LOG(INFO, "test_type=%s cluster_addr=%s cluster_port=%ld thread_num=%ld sign=%ld check=%s",
...
int join_path(char* result_path, const int64_t len_limit, const char* base, const char* path)
{
  ...
  if (NULL == result_path || 0 >= len_limit || NULL == path)
  {
    ...
    YYSYS_LOG(ERROR, "join_path(result_path=%s, limit=%ld, base=%s, path=%s)=>%d",
  ...
  else if ('/' == path[0] || NULL == base)
  {
    if (0 >= (len = snprintf(result_path, len_limit, "%s", path)) || len > len_limit)
    {
      ...
      YYSYS_LOG(ERROR, "snprintf(path=%s)=>%ld", path, len);
    }
  }
  else
  {
    ...
    {
      err = OB_BUF_NOT_ENOUGH;
      YYSYS_LOG(ERROR, "snprintf(base=%s, path=%s)=>%d", base, path, err);
    }
  }
  return err;
}
...
int get_absolute_path(char* abs_path, const int64_t len_limit, const char* path)
{
  ...
  {
    err = OB_INVALID_ARGUMENT;
    YYSYS_LOG(ERROR, "get_absolute_path(abs_path=%p, limit=%ld, path=%s)=>%d", abs_path, len_limit, path, err);
  }
  ...
  {
    YYSYS_LOG(ERROR, "join_path(len_limit=%ld, base=%s, path=%s)=>%d", len_limit, cwd, path, err);
  }
  return err;
}
...
const char* get_absolute_path(const char* path)
{
  ...
  {
    YYSYS_LOG(ERROR, "get_absolute_path(path=%s)=>%d", path, err);
  }
...
{
  int64_t local_ip = abs(yysys::CNetUtil::getLocalAddr(NULL));
