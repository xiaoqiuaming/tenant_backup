...
bool RowChecker::check_row(CellinfoBuilder &cb, const ObSchema &schema)
{
  ...
  if (NULL == seed_ci
      || NULL == cell_num_ci)
  {
    YYSYS_LOG(WARN, "not seed column or cell_num column cellinfo [%s] [%s]",
    ...
    for (iter = read_result_.begin(); iter != read_result_.end(); ++iter)
    {
      ...
      else
      {
        ...
        if (NULL == tmp_ci)
        {
          ...
          {
            YYSYS_LOG(WARN, "value not equal [%s] [%s]", ::print_cellinfo(iter->second), ::print_cellinfo(tmp_ci));
            bret = false;
            break;
          }
        }
...
bool RowChecker::check_rowkey(RowkeyBuilder &rb, const int64_t *prefix_ptr)
{
  ...
  for (iter = rowkey_read_set_.begin(); iter != rowkey_read_set_.end(); iter++)
  {
    ...
    {
      bret = false;
      YYSYS_LOG(WARN, "read_rowkey=[%s] not found in check_set", to_cstring(iter->first));
    }
    else
    {
  ...
  if (0 != rowkey_check_set.size())
  {
    ...
    {
      YYSYS_LOG(WARN, "rowkey=[%s] not in result_set", to_cstring(iter->first));
    }
  }
  assert(bret);
                                          ...
                                          const bool using_id, const int64_t table_start_version, const int64_t timestamp)
{
  ...
  if (HASH_EXIST != hash_set_.exist(prefix))
  {
    ...
    if (OB_SUCCESS == tmp_ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "scan get nothing (%s)", to_cstring(new_range));
        tmp_ret = OB_ERROR;
      }
      else
      {
