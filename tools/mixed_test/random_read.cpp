...
struct CmdLine
{
  ...
  void log_all()
  {
    YYSYS_LOG(INFO, "serv_addr=%s serv_port=%ld schema_addr=%s schema_port=%ld using_id=%s get2read=%s table_start_version=%ld "
...
int random_scan_check(MutatorBuilder &mb, ClientWrapper &client, const int64_t table_start_version, const bool using_id, const bool check)
{
        ...
        && OB_SUCCESS == ret)
  {
        ...
        if (OB_SUCCESS == scanner.get_cell(&ci, &is_row_changed))
        {
          ...
          if (is_row_changed && 0 != rc.cell_num())
          {
            ...
            if (!get_row_bret
                || !cell_info_bret)
            {
              YYSYS_LOG(ERROR, "[%s] [get_row check_ret=%s] [cell_info check_ret=%s]",
              ...
              && rc.is_prefix_changed(ci->row_key_))
          {
            ...
            if (!bret)
            {
              YYSYS_LOG(ERROR, "[%s] [row_key check_ret=%s]", row_key_str.c_str(), STR_BOOL(bret));
            }
          }
          rc.add_cell(ci);
          if (is_row_changed)
    ...
    YYSYS_LOG(INFO, "random_scan ret=%d timeu=%ld row_counter=%ld", ret, timeu, cur_row_counter);
  ...
  YYSYS_LOG(DEBUG, "table_id=%lu row_counter=%ld", mb.get_schema(i).get_table_id(), row_counter);
  return ret;
}
                      ...
                      const int64_t row_num, const int64_t cell_num_per_row, const bool check)
{
    ...
    while (OB_SUCCESS == scanner.next_cell())
    {
      ...
      if (OB_SUCCESS == scanner.get_cell(&ci, &is_row_changed))
      {
            ...
            && ObActionFlag::OP_ROW_DOES_NOT_EXIST == ci->value_.get_ext())
        {
          ...
          {
            YYSYS_LOG(ERROR, "[%s] maybe un-exist but not, timeu=%ld", to_cstring(ci->row_key_), timeu);
          }
          continue;
        }
  ...
  YYSYS_LOG(INFO, "random_mget ret=%d timeu=%ld row_counter=%ld", ret, timeu, row_counter);

  bool is_fullfilled = false;
  int64_t fullfilled_item_num = 0;
  scanner.get_is_req_fullfilled(is_fullfilled, fullfilled_item_num);
  YYSYS_LOG(DEBUG, "row_counter=%ld get_param_row_size=%ld fullfilled_item_num=%ld get_param_cell_size=%ld "
...
int main(int argc, char **argv)
{
  ...
  if (clp.get2read)
  {
    ...
    YYSYS_LOGGER.rotateLog(buffer);
    YYSYS_LOGGER.setFileName(buffer);
  }
  else
  {
    char buffer[1024];
  ...
  YYSYS_LOGGER.setLogLevel(getenv("log_level")? getenv("log_level"): clp.log_level);
  clp.log_all();

  ob_init_memory_pool();
  YYSYS_LOG(INFO, "cmdline: %s", my_str_join(argc, argv));
  YYSYS_LOG(INFO, "getpid()=%d", getpid());
