...
struct CmdLine
{
  ...
  void log_all()
  {
    YYSYS_LOG(INFO, "serv_addr=%s serv_port=%ld schema_addr=%s schema_port=%ld using_id=%s table_start_version=%ld "
...
void scan_check_all(MutatorBuilder &mb, ClientWrapper &client, const int64_t table_start_version, const bool using_id, const bool check)
{
  for (int64_t i = 0; i < mb.get_schema_num(); ++i)
  {
          ...
          && OB_SUCCESS == ret)
    {
      ...
      YYSYS_LOG(INFO, "total_scan ret=%d timeu=%ld row_counter=%ld last_prefix_row_counter=%ld",
    ...
    YYSYS_LOG(INFO, "table_id=%lu total_row_counter=%ld", mb.get_schema(i).get_table_id(), row_counter);
  }
}
...
int main(int argc, char **argv)
{
  ...
  YYSYS_LOGGER.setLogLevel(clp.log_level);
  clp.log_all();
