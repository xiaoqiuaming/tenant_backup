...
void *thread_func(void *data)
{
  ...
  for (int64_t m = 0; m < tp->mutator_num; m++)
  {
    ...
    YYSYS_LOG(INFO, "[%d] apply ret=%d timeu=%ld", gettid(), ret, timeu);
    total_timeu += timeu;
  }
  YYSYS_LOG(INFO, "[%d] total timeu=%ld", gettid(), total_timeu);
  delete[] vc_buf;
  delete[] rk_buf;
  return NULL;
}
...
void run(const pid_t pid, MainParam &param)
{
  ...
  YYSYS_LOGGER.rotateLog(log_file_buffer);
  YYSYS_LOGGER.setFileName(log_file_buffer);
...
int main(int argc, char **argv)
{
  ...
  for (int64_t i = 0; i < param.process_num; i++)
  {
    ...
    {
      YYSYS_LOG(INFO, "fork child process succ cid=%d", cid);
    }
  }
}
