 ...
 *
 *  Version: $Id: cs_press.cpp 2010��11��16�� 11ʱ01��16�� qushan Exp $
 *
 *  Authors:
    ...
    const char* schema_file, const int32_t tc)
{
  ...
  if (!parse_ok) 
  {
    YYSYS_LOG(ERROR, "open schema_file failed, ret:%d", ret);
    return OB_ERROR;
  }
...
{
  YYSYS_LOG(INFO, "worker done, total consume:%ld, total case:%ld,  "
...
// class Worker
//--------------------------------------------------------------------
void Worker::run(yysys::CThread *thread, void *arg)
{
  long index = (long) arg;
...
int Worker::seq_scan_case(const Param &param)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "get_column_array failed=%d", ret);
    return ret;
  }
  else
  {
    YYSYS_LOG(DEBUG, "get_column_array size=%ld, is_whole_row=%d", column_id_size, is_whole_row);
  }
  ...
  while (!_stop)
  {
    ...
    {
      YYSYS_LOG(DEBUG, "start_seed:%ld > max_seed_:%ld, thread exit.", 
    ...
    YYSYS_LOG(DEBUG, "sequence generator, start_seed=%ld, end_seed=%ld ",
        start_seed, end_seed);
...
int Worker::random_scan_case(const Param &param)
{
  ...
  {
    YYSYS_LOG(DEBUG, "get_column_array size=%ld", column_id_size);
  }
  ...
  while (!_stop)
  {
    ...
    YYSYS_LOG(DEBUG, "random generator, start_seed=%ld, end_seed=%ld ",
        start_seed, end_seed);
    ...
    const bool is_whole_row)
{
  ...
  YYSYS_LOG(DEBUG, "unit_scan_case dump range start");
  range.hex_dump();
  YYSYS_LOG(DEBUG, "unit_scan_case dump range end");
  ...
  YYSYS_LOG(DEBUG, "cs_scan time consume:%ld", end - start);
  metrics_.tick(end - start, 1, OB_SUCCESS == ret ? true : false);
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "cs_scan error ret:%d, start seed=%ld, end seed=%ld", ret, start_seed, end_seed);
  }
  else
  {
    ...
    int64_t *column_id_array, int64_t &column_id_size, bool &is_whole_row)
{
  ...
  if (column_id_size == 1 && column_id_array[0] == 0)
  {
    ...
    is_whole_row = true;
    YYSYS_LOG(DEBUG, "query all coluumns, param.columns_=%s,  column_id_size=%ld" 
    ...
    uint32_t & hash_value)
{
  ...
  {
    YYSYS_LOG(ERROR, "check cell failed,  "
  ...
  {
    YYSYS_LOG(ERROR, "check cell failed, column id=%lu, "
  ...
  if (OB_SUCCESS != ret) 
  {
    YYSYS_LOG(ERROR, "decode prefix error=%d", ret);
    return ret;
  }

  int64_t base_value = prefix * column_id * cell_info.value_.get_type() * 991;
  YYSYS_LOG(DEBUG, "column id(%ld) prefix(%ld), base value(%ld), type(%d)", 
  ...
  switch (cell_info.value_.get_type())
  {
    ...
    case ObIntType:
      ...
      if (intv != base_value) 
      {
        YYSYS_LOG(ERROR, "check int value failed, column id=%ld, "
    ...
    case ObFloatType:
      ...
      {
        YYSYS_LOG(ERROR, "check float value failed, column id=%ld, "
    ...
    case ObDoubleType:
      ...
      {
        YYSYS_LOG(ERROR, "check double value failed, column id=%ld, "
            "base value=%ld, actual value=%10.2f", column_id,
            base_value, doublev);
        ret = OB_ERROR;
      }
    ...
    case ObDateTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "check datetime value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
    ...
    case ObDateType:
      ...
      if (date != base_value)
      {
        YYSYS_LOG(ERROR, "check  date value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
            base_value, date);
        ret = OB_ERROR;
      }
    ...
    case ObTimeType:
      ...
      if (time != base_value)
      {
        YYSYS_LOG(ERROR, "check time value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
            base_value, time);
        ret = OB_ERROR;
      }
    ...
    case ObPreciseDateTimeType:
      ...
      {
        YYSYS_LOG(ERROR, "check precise datetime value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
            base_value, precisev);
        ret = OB_ERROR;
      }
    ...
    case ObVarcharType:
      ...
      if (trs != base_value) 
      {
        YYSYS_LOG(ERROR, "check varchar value failed, column id=%ld, "
    ...
    case ObCreateTimeType:
      ...
      if (precisev != base_value) 
      {
        YYSYS_LOG(ERROR, "check create datetime value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
            base_value, precisev);
        ret = OB_ERROR;
      }
    ...
    case ObModifyTimeType:
      ...
      if (precisev != base_value) 
      {
        YYSYS_LOG(ERROR, "check modify datetime value failed, column id=%ld, "
            "base value=%ld, actual value=%ld", column_id,
            base_value, precisev);
        ret = OB_ERROR;
      }
...
int Worker::random_get_case(const Param &param)
{
  ...
  while (!_stop)
  {
    ...
    while (count < param.row_count_)
    {
      ...
      YYSYS_LOG(DEBUG, "random generator, current_seed=%ld ", current_seed);
      seed_array[i++] = current_seed;
    }
    seed_array_size = i;
    ...
    const bool is_whole_row)
{
  ...
  int64_t end = yysys::CTimeUtil::getTime();
  YYSYS_LOG(DEBUG, "cs_get time consume:%ld", end - start);
  metrics_.tick(end - start, 1, OB_SUCCESS == ret ? true : false);

  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "cs_get error ret:%d", ret);
  }
  else
  {
    ret = check_query_result(param, scanner, seed_array, 
    ...
    const bool is_whole_row)
{
  ...
  for (iter = scanner.begin(); iter != scanner.end(); iter++)
  {
    ...
    if (is_row_changed)
    {
      ...
      {
        YYSYS_LOG(ERROR, "get row count=%d > seed_array_size=%ld ", row_count, seed_array_size);
        ret = OB_ERROR;
        break;
      }
      ...
      if (cmp != 0)
      {
        YYSYS_LOG(ERROR, "check_scan_result error, cell_info key not equal base_key");
        hex_dump(base_key.ptr(), base_key.length(), YYSYS_LOG_LEVEL_ERROR);
        hex_dump(cell_info->row_key_.ptr(), cell_info->row_key_.length(),
            YYSYS_LOG_LEVEL_ERROR);
        ret = OB_ERROR;
      }
      else
      {
    ...
    YYSYS_LOG(DEBUG, "---------------------%d----------------------\n", row_count);
    YYSYS_LOG(DEBUG,"table_id:%lu, column_id:%ld, is_row_changed:%d\n", 
        cell_info->table_id_, cell_info->column_id_, is_row_changed);
    hex_dump(cell_info->row_key_.ptr(), cell_info->row_key_.length());

    if (column_index >= column_id_size) 
    {
      YYSYS_LOG(ERROR, "get column_index:%d > column_id_size:%ld",
    ...
    else
    {
      ...
      YYSYS_LOG(DEBUG, "column_index=%d, check_sum=%ld", column_index, check_sum);
      total_hash_value += check_sum;

      if (base_hash_value != total_hash_value)
      {
        YYSYS_LOG(ERROR, "check cell mismatch , base_hash_value=%ld not equal total_hash_value=%ld",
            base_hash_value, total_hash_value);
        ret = OB_ERROR;
      }
      else
      {
        YYSYS_LOG(DEBUG, "check cell succeed, base_hash_value=%ld equal total_hash_value=%ld",
            base_hash_value, total_hash_value);
      }
    }
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "check cell mismatch, index=%d, id=%ld", column_index, 
...
{
  int thread_count = YYSYS_CONFIG.getInt(section_name, "thread_count", 1);
  int query_type = YYSYS_CONFIG.getInt(section_name, "query_type", 0);
  const char* rowkey_seed_str = YYSYS_CONFIG.getString(section_name, "rowkey_seed");
  int64_t rowkey_seed = strtoll(rowkey_seed_str, NULL, 10);
  const char* rowkey_max_seed_str = YYSYS_CONFIG.getString(section_name, "rowkey_max_seed");
  int64_t rowkey_max_seed = strtoll(rowkey_max_seed_str, NULL, 10);
  int32_t rowkey_step =  YYSYS_CONFIG.getInt(section_name, "rowkey_step", 1);
  int32_t row_count = YYSYS_CONFIG.getInt(section_name, "row_count", 1);
  const char* column_id_array = YYSYS_CONFIG.getString(section_name, "column_id_array");
  int64_t table_id =  YYSYS_CONFIG.getInt(section_name, "table_id", 0);
  int32_t use_cache = YYSYS_CONFIG.getInt(section_name, "use_cache", 1);
  int32_t read_mode = YYSYS_CONFIG.getInt(section_name, "read_mode", 1);

  const char* remote_server_addr = YYSYS_CONFIG.getString(section_name, "remote_server_addr");
  int32_t remote_port = YYSYS_CONFIG.getInt(section_name, "remote_port", 0);
  const char* schema_file = YYSYS_CONFIG.getString(section_name, "schema_file");
...
int main(const int argc, char *argv[])
{
  ...
  if(YYSYS_CONFIG.load(config_file)) 
  {
  ...
  const char* sz_pid_file =
    YYSYS_CONFIG.getString(section_name, PID_FILE, "server.pid");
  const char* sz_log_file =
    YYSYS_CONFIG.getString(section_name, LOG_FILE, "server.log");

  int pid = 0;
  if((pid = yysys::CProcess::existPid(sz_pid_file))) {
    fprintf(stderr, "program has been exist: pid=%d\n", pid);
    return EXIT_FAILURE;
  }

  const char * sz_log_level =
    YYSYS_CONFIG.getString(section_name, LOG_LEVEL, "info");
  YYSYS_LOGGER.setLogLevel(sz_log_level);
  int max_file_size = 
    YYSYS_CONFIG.getInt(section_name, MAX_LOG_FILE_SIZE, 1024); //1G
  YYSYS_LOGGER.setMaxFileSize(max_file_size * 1024L * 1024L);

  int ret = initialize("client");
  if (OB_SUCCESS != ret) 
  { 
    YYSYS_LOG(ERROR, "initialize failed, ret=%d", ret);
    return ret; 
  }
  ...
  if (use_deamon_) 
  {
    start_ok = (yysys::CProcess::startDaemon(sz_pid_file, sz_log_file) == 0);
  }
  if(start_ok) 
  {
