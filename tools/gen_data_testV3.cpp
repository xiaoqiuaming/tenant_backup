...
//TODO ÿ���л�column group��ʱ�� ��Ҫ��ס���й���rowkey����
//TODO �ѹ���rowkow����ó��� range��start key����һ��
namespace oceanbase
{
  namespace chunkserver
  {
    ...
    int GenDataTestV3::generate_table_jdatetime()
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "create sstable failed table_id = %lu", table_id_);
      }
      else
      {
        ...
        {
          YYSYS_LOG(ERROR,"close_sstable failed: [%d]",ret);
        }
      }
      return ret;
    }
     ...
     int GenDataTestV3::generate_table_cust()
     {
       ...
       for (int32_t sstable = 0; OB_SUCCESS == ret && sstable < 50; ++sstable)
       {
         ...
         if (sstable == 0)
         {
           ...
           {
             YYSYS_LOG(ERROR,"create rowkey failed: [%d]",ret);
             return ret;
           }
           else
           {
         ...
         for (int32_t group = 0; OB_SUCCESS == ret && group < column_group_num; ++group)
         {
           ...
           for (int32_t row = 0; row < 10000/*TODO change the number*/ && OB_SUCCESS == ret; ++row)
           {
             ...
             if ((ret = fill_row_cust(column_group_ids[group], sstable*10000+row))
                 != OB_SUCCESS)
             {
               YYSYS_LOG(ERROR,"fill row error");
             }
             ...
             if (OB_SUCCESS == ret)
             {
               ...
               if (OB_SUCCESS != ret)
               {
                 YYSYS_LOG(ERROR,"append_row error disk_no_=%d, ret=%d", disk_no_, ret);
                 YYSYS_LOG(ERROR,"row number is %d", row);
               }
             }
           }
         }
    ...
    int GenDataTestV3::generate()
    {
      ...
      for(int index = 0; index < 6; ++index)
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "convert schema error, table_id is %lu", table_id_);
        }
        else
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "generate_table failed, table id = %lu", table_id_);
          }
        }
      }
      //write idx file
      ...
      for(int index = 1; index <= arg_->disk_num_; ++index)
      {
        ...
        {
          YYSYS_LOG(ERROR, "write meta file faied, ret=%d", ret);
        }
      }
      return ret;
    }
    ...
    int GenDataTestV3::fill_sstable_schema(const ObSchemaManagerV2& schema,uint64_t table_id,ObSSTableSchema& sstable_schema)
    {
      ...
      if (NULL == col || size <= 0)
      {
        YYSYS_LOG(ERROR,"cann't find this table:%lu",table_id);
        ret = OB_ERROR;
      }
      else
      {
    ...
    int GenDataTestV3::close_sstable(bool set_min, bool set_max)
    {
      ...
      if ((ret = writer_.close_sstable(trailer_offset, sstable_size)) != OB_SUCCESS
          || sstable_size < 0)
      {
        YYSYS_LOG(ERROR, "close sstable error ret%d sstable size=%ld", ret, sstable_size);
      }
      ...
      if (sstable_size > 0)
      {
        ...
        {
          YYSYS_LOG(ERROR, "alloc tablet object failed.\n");
        }
        else
        {
        ...
        {
          YYSYS_LOG(ERROR, "add tablet failed, ret=%d", ret);
        }
    ...
    int GenDataTestV3::fill_row_search(uint64_t group_id, int32_t index)
    {
      ...
      {
        YYSYS_LOG(ERROR,"set row key failed: [%d]",ret);
      }
      else
      {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        for(int32_t i=0; i < size && (OB_SUCCESS == ret);++i,++column)
        {
...
#ifdef GEN_SSTABLE_DEBUG
            YYSYS_LOG(DEBUG,"%ld | %s | %d",column_value, column->get_name(), column->get_type());
#endif
            break;
          case ObPreciseDateTimeType:
            obj.set_precise_datetime(column_value);
#ifdef GEN_SSTABLE_DEBUG
            YYSYS_LOG(DEBUG,"%ld | %s",column_value, column->get_name());
#endif
            break;
          ...
          case ObVarcharType:
            ...
            obj.set_varchar(str);
#ifdef GEN_SSTABLE_DEBUG
            YYSYS_LOG(DEBUG,"%s | %s",var, column->get_name());
#endif
            break;
          default:
