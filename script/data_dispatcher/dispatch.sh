...
usage()
{
	...
	echo "  -m                    dispatch sstable to yaodatasvr with multiple gateway"
	echo "  -b                    map reduce build sstable in hadoop"
	echo "  -f                    major freeze the only one OB cluster in config file"
	echo "  -o YAODATASVR_IP    each gateway only copy the sstable belong to the specified yaodatasvr"
}
...
read_param_from_config_file()
{
	...
	ROOTSERVER_IP=$(get_conf "$conf" "ADMINSERVER_IP" "")
	ROOTSERVER_PORT=$(get_conf "$conf" "ADMINSERVER_PORT" "2500")

	UPDATESERVER_IP=$(get_conf "$conf" "TRANSSERVER_IP" "")
	UPDATESERVER_PORT=$(get_conf "$conf" "TRANSSERVER_PORT" "2900")
...
global_var_init()
{
	...
	FINISH_UPS_FREEZE="$LOCAL_TABLE_DIR/ts_freeze.done"
...
build_range_file()
{
	...
	if [ "X$ROOTSERVER_IP" == "X" ]; then
		mlog "INFO: not specify adminserver ip, needn't scan root table to build range file: $RANGE_FILE" $LOG_FILE
		echo "INFO: not specify adminserver ip, needn't scan root table to build range file: $RANGE_FILE"
		return 0
	fi
...
scp_data_to_cs()
{
	infolog "INFO: starting scp data to  yaodatasvr ......"
	declare flag_file=${FINISH_DISP_FLAG_FILE}
	if [ -f $flag_file ]; then
		infolog "INFO: $flag_file is existent, needn't scp data to yaodatasvr again"
		return 0
	fi
	...
	infolog "INFO: finish scp file to yaodatasvr, succ_count=$SCP_FILE_SUCC_COUNT, failed_count=$SCP_FILE_FAIL_COUNT"
}
...
major_freeze()
{
	...
	if [ "X$ROOTSERVER_IP" == "X" -o "X$UPDATESERVER_IP" == "X" ]; then
		mlog "INFO: not specify adminserver ip ($ROOTSERVER_IP) or transserver ip ($UPDATESERVER_IP), needn't major freeze" $LOG_FILE
		echo "INFO: not specify adminserver ip ($ROOTSERVER_IP) or transserver ip ($UPDATESERVER_IP), needn't major freeze"
		return 1
	fi
	...
	if [ "X$?" != "X0" ]; then
		errorlog "ERROR: failed to insert ctl data to transserver by ob_import"
		return 2
	fi

	declare ups_admin_file="${BIN_DIR}ts_admin"
	sleep 10
