        ...
        public IChangeMaster
      {
        public:
          ...
          int handleNodeRegister(const ObServerExt &node, const char* app_name, const char *inst_name, bool is_master)
          {
            ...
            UNUSED(is_master);
            int err = OB_SUCCESS;
            YYSYS_LOG(INFO, "a new node register. should add to list");
            return err;
          }
          ...
          {
            int err = OB_SUCCESS;
            YYSYS_LOG(INFO, "a slave node failure. ip=%d", slave.get_ipv4());
            return err;
          }
          ...
          {
            int err = OB_SUCCESS;
            YYSYS_LOG(INFO, "change master. new master ip=%d", new_master.get_ipv4());
            return err;
          }
