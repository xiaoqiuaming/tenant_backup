  ...
  namespace mms
  {
    namespace tests
    {
      ...
      int MockMonitorServer::initialize()
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "fail to call MockServer initialize");
        }
        ...
        if(err == OB_SUCCESS)
        {
          ...
          if(err != OB_SUCCESS)
          {
            YYSYS_LOG(WARN, "fail to init monitor");
          }
        }
        ...
        if(err == OB_SUCCESS)
        {
          ...
          if(handler_ == NULL)
          {
            ...
            YYSYS_LOG(WARN, "fail to allocate handler. err=%d", err);
          }
        }
        return err;
      }
      ...
      int MockMonitorServer::start_service()
      {
        ...
        {
          YYSYS_LOG(INFO, "start monitor succ~!");
        }
        return err;
      }

      yynet::IPacketHandler::HPRetCode MockMonitorServer::handlePacket(yynet::Connection* connection, yynet::Packet *packet)
      {
       ...
       bool MockMonitorServer::handlePacketQueue(yynet::Packet *packet, void *args)
       {
       ...
       int MockMonitorServer::do_request(ObPacket* base_packet)
      {
        ...
        if(err != OB_SUCCESS)
        {
          YYSYS_LOG(WARN, "handle failed:ret[%d]", err);
        }
        return err;
      }
    }
