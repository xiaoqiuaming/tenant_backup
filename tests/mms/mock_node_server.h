    ...
    namespace tests
    {
      ...
      class MockNodeServer : public MockServer
      {
        public:
          ...
          bool handlePacketQueue(yynet::Packet *packet, void *args);
        ...
        public ILeaseTimeout
      {
        public:
          ...
          {
            YYSYS_LOG(INFO, "stop slave handler");
            return OB_SUCCESS;
          }

          int handleSlaveDown(const common::ObServer &server)
          {
            YYSYS_LOG(INFO, "slave down. need not to write log to %d", server.get_ipv4());
            return OB_SUCCESS;
          }

          int handleTransfer2Master()
          {
            YYSYS_LOG(INFO, "tranfer my role to master.");   
            MMS_Status &status = mock_node_->get_status();
            status = MASTER_ACTIVE;
            YYSYS_LOG(INFO, "I am master now");
            return OB_SUCCESS;
          }

          int handleMasterChanged()
          {
            YYSYS_LOG(INFO, "master have changed.");
            YYSYS_LOG(INFO, "I need to ask register to the new master");
            return OB_SUCCESS;
          }

          int handleLeaseTimeout()
          {
            YYSYS_LOG(INFO, "oh, lease timeout!!");
            YYSYS_LOG(INFO, "i am died !");
            mock_node_->stop();
            return OB_SUCCESS;
          }
