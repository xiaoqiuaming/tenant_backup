...
int MockNodeServer::initialize()
{
  ...
  if(NULL == handler_)
  {
    ...
    if(handler_ == NULL)
    {
      ...
      YYSYS_LOG(WARN, "fail to new handler. err= %d", err);
    }
  }
  return err;
}
...
int MockNodeServer::start_service()
{
  ...
  else
  {
    ...
    YYSYS_LOG(ERROR, "hanlder should not be NULL, err=%d", err);
  } 
  if(OB_SUCCESS == err)
  {
    err = start_();
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to start. err=%d", err);
    }
  }   
  return err;
}
...
yynet::IPacketHandler::HPRetCode MockNodeServer::handlePacket(yynet::Connection* connection, yynet::Packet *packet)
{
  yynet::IPacketHandler::HPRetCode rc = yynet::IPacketHandler::FREE_CHANNEL;          
  ...
  else
  {
    ...
    if (!ps)                                                                        
    {
      YYSYS_LOG(WARN, "overflow packet dropped, packet code: %d", req->getPCode());
      rc = yynet::IPacketHandler::KEEP_CHANNEL;
    }                                                                               
  }     
...
bool MockNodeServer::handlePacketQueue(yynet::Packet *packet, void *args)
{
...
int MockNodeServer::do_request(ObPacket* base_packet)
{
  ...
  if(OB_SUCCESS == err)
  {
    switch(packet_code)
    {
      ...
      default:
        err = OB_ERROR;
        YYSYS_LOG(ERROR, "wrong packet code:packet[%d]", packet_code);
        break;
    }
  }
  if(err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "check handle failed:ret[%d]", err);
  } 
  return err;
}
...
int MockNodeServer::start_()
{
  ...
  if(err != OB_SUCCESS)
  {
    YYSYS_LOG(WARN, "fail to init node.");
  }
  if(err == OB_SUCCESS)
  {
    ...
    if(err != OB_SUCCESS)
    {
      YYSYS_LOG(WARN, "fail to start node, err=%d", err);
    }
  }
  if(err == OB_SUCCESS)
  {
    ...
    if(OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "fail to register to monitor, err=%d", err);
    }
  }
  if(err == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(INFO, "register succ,  as a slave");
    }
    else
    {
      if(status_ == MASTER_ACTIVE)
      {
        YYSYS_LOG(INFO, "register succ, as a master");
      }
    }
  }
  return err;
