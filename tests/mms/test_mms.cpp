...
TEST_F(TestMMS, start)
{
  ...
  yysys::CThread monitor_server_thread;
  monitor_server_thread.start(&test_monitor_server, NULL);
  sleep(1);
  //node start
  int node_port = 5678;
  ObServer monitor;
  uint32_t ip = yysys::CNetUtil::getLocalAddr("bond0");
  ...
  yysys::CThread node_server_thread;
  ...
  yysys::CThread node_server_thread_3;
  ...
  yysys::CThread node_server_thread_2;
  ...
  yysys::CThread node_server_thread_4;
...
TEST_F(TestMMS, master_report_slave_down)
{
  ...
  YYSYS_LOG(INFO, "send to monitor, addr=%d, port=%d", monitor.get_ipv4(), monitor.get_port());
