  ...
  namespace rootserver
  {
    ...
    class ObRootWorkerForTest : public ObRootWorker
    {
      public:
        ...
        {
          YYSYS_LOG(INFO, "will do cs_migrate");
          char t1[100];
          char t2[100];
          src_server.to_string(t1, 100);
          dest_server.to_string(t2, 100);
          range.hex_dump(YYSYS_LOG_LEVEL_INFO);
          YYSYS_LOG(INFO, "src server = %s dest server = %s keep src = %d", t1, t2, keep_src);
        ...
        int async_sql(const common::ObString &sqlstr, const common::ObString &table_name, int64_t try_times)
        {
          ...
          UNUSED(table_name);
          UNUSED(try_times);
          YYSYS_LOG(INFO, "async sql. sql_str=%s, table_name=%s", to_cstring(sqlstr), to_cstring(table_name));
          return OB_SUCCESS;
        }
