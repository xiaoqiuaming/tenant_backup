...
class BalanceTestRpc : public oceanbase::rootserver::ObRootRpcStub, public yysys::CDefaultRunnable
{
  public:
  ...
  private:
    ...
    int64_t msg_count_;
    yysys::CThreadMutex migrate_msg_mutex_;
...
int BalanceTestRpc::push_migrate_msg(const common::ObServer& src_cs, const common::ObServer& dest_cs, const common::ObNewRange& range, bool keep_src)
{
  ...
  yysys::CThreadGuard guard(&migrate_msg_mutex_);
  if (NULL == msg_head_)
  {
    msg_head_ = msg;
  }
  ...
  YYSYS_LOG(DEBUG, "push migrate msg, src=%s dest=%s range=%s",
...
int BalanceTestRpc::pop_migrate_msg(MigrateMsg *&msg)
{
  ...
  if (NULL != msg_head_)
  {
    ...
    msg->range_.to_string(range_buf, OB_MAX_ROW_KEY_LENGTH*2);
    YYSYS_LOG(DEBUG, "pop migrate msg, src=%s dest=%s range=%s",
              addr_buf1, addr_buf2, range_buf);
    msg_count_--;
    ret = OB_SUCCESS;
  }
  ...
  {
    YYSYS_LOG(DEBUG, "msg head is NULL, count=%ld", msg_count_);
  }
  return ret;
}
...
void BalanceTestRpc::run(yysys::CThread *thread, void *arg)
{
  UNUSED(thread);
  UNUSED(arg);
  YYSYS_LOG(INFO, "balance test rpc thread begin");
  while(!_stop)
  {
    YYSYS_LOG(DEBUG, "balance test rpc thread running");
    MigrateMsg *msg = NULL;
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (blocked_cs_ == msg->src_cs_)
      {
        ...
        YYSYS_LOG(DEBUG, "drop migrate msg, src_cs=%s", addr_buf1);
      }
      else if (blocked_dest_cs_ == msg->dest_cs_)
      {
        char addr_buf1[OB_IP_STR_BUFF];
        blocked_dest_cs_.to_string(addr_buf1, OB_IP_STR_BUFF);
        YYSYS_LOG(DEBUG, "drop migrate msg, dest_cs=%s", addr_buf1);
      }
      else
      {
  ...
  YYSYS_LOG(INFO, "balance test rpc thread finished");
}
...
void ObBalanceTest::register_cs(int32_t cs_num)
{
  for (int i = 0; i < cs_num; ++i)
  {
    ...
    YYSYS_LOG(INFO, "register cs, id=%d status=%d", i, status);
  }
  YYSYS_LOG(INFO, "register %d cs end.", cs_num);
}
...
void ObBalanceTest::reset_report_info_list(ObTabletReportInfoList &rlist)
{
  ...
  YYSYS_LOG(INFO, "report info addr = %p, size=%ld", report_info, rlist.get_tablet_size());
  //// free keys
  //for (int k = 0; k < rlist.get_tablet_size(); ++k)
  //{
  //  if (NULL != report_info[k].tablet_info_.range_.start_key_.ptr())
  //  {
  //    YYSYS_LOG(INFO, "report_info[%d].tablet_info_.range_.start_key_.ptr()=%p", k, report_info[k].tablet_info_.range_.start_key_.ptr());
...
{
  YYSYS_LOG(INFO, "start to report tablet.");
  ...
  for (int i = 0; i < params.table_num_; ++i)
  {
    ...
    YYSYS_LOG(INFO, "table_id=%lu sstable_count=%d total_dist=%d\n", get_table_id(i), sstables_count, total_dist);
    for (int k = 0; k < params.cs_num_; ++k)
    {
      YYSYS_LOG(INFO, "sstables dist per cs, cs_index=%d, table_index=%d, sstable_count=%d",
      ...
      for (int k = 0; k < params.cs_num_ && j < sstables_count; ++k)
      {
        ...
        if (posts[k] >= total_dist)
        {
          ...
          if (OB_ARRAY_OUT_OF_RANGE == ret2)
          {
            ...
            YYSYS_LOG(INFO, "report tablets, cs=%d count=%ld", k, report_lists[k].get_tablet_size());
    ...
    for (int k = 0; k < params.cs_num_; ++k)
    {
      YYSYS_LOG(INFO, "reported tablets, table=%lu cs_idx=%d tablets=%ld expected=%d",
...
void ObBalanceTest::report_tablets_2replicas(BalanceTestParams &params)
{
  ...
  {
    int32_t sstables_count = params.sstables_per_table_[i];
    YYSYS_LOG(INFO, "table_id=%lu sstables=%d\n", get_table_id(i), sstables_count);
    for (int j = 0; j < params.cs_num_; ++j)
    {
      YYSYS_LOG(INFO, "sstables dist per cs, %d", params.sstables_dist[i][j]);
      reset_report_info_list(report_lists[j]);
      tablets_per_cs[j] = 0;
    }
    int k = 0;
    ...
    for (int k = 0; k < params.cs_num_; ++k)
    {
      YYSYS_LOG(INFO, "reported tablets, table=%lu cs_idx=%d tablets=%ld",
...
{
  ob_init_memory_pool();
  //YYSYS_LOGGER.setFileName("a.log");
