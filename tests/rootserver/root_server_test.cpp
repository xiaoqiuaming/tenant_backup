...
TEST(ObRootServer2Test2, init_report)
{
  ...
  YYSYS_LOG(INFO, "will start test");
  //tester.init_root_table_by_report();

  //while (worker.start_new_send_times < 2)
  //{
  //  YYSYS_LOG(INFO, "wait send start_new_schema now %d have sended",worker.start_new_send_times);
  //  sleep(1);
  //}
  YYSYS_LOG(INFO, "over send start_new_schema now %d have sended",worker.start_new_send_times);
  ...
  //{
  //  YYSYS_LOG(INFO, "wait send unload_old_table now %d have sended", worker.unload_old_table_times);
  //  sleep(1);
  //}
  sleep(5);
  {
...
TEST(ObRootServer2Test2, migrate_over)
{
  ...
  YYSYS_LOG(INFO, "check migrate result s2=%d s3=%d", server2_index, server3_index);
  ...
  ASSERT_EQ(0, root_server->migrate_over(migrate_result, desc1, occupy_size, crc_sum, row_checksum, row_count));
  YYSYS_LOG(INFO, "check migrate 2->4 ");
...
TEST(ObRootServer2Test2, cs_stop_start_data_keep)
{
  ...
  root_server->get_boot()->set_boot_ok();
  YYSYS_LOG(INFO, "will start test cs_stop_start_data_keep");
  //tester.init_root_table_by_report();
  ...
  rt_q->dump();
  //cs start again
  YYSYS_LOG(INFO," ------------------- server start again -------------------");
  root_server->regist_chunk_server(server3, "0.4.1.2", status);
  ObTabletReportInfoList report_list1;
  ObTabletReportInfoList report_list2;
  ObTabletReportInfoList report_list3;
  ...
  report_info.tablet_location_ = location;
  report_list3.add_tablet(report_info);
  YYSYS_LOG(INFO, "start to report tablets");
  root_server->report_tablets(server3, report_list3,0);
  sleep(1);
  rt_q->dump();
  root_server->stop_threads();
...
{
  ob_init_memory_pool();
  //YYSYS_LOGGER.setFileName("b.log");
