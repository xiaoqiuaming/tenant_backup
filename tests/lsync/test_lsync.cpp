      ...
      int init(const char* host, int port)
        {
          ...
          if (OB_SUCCESS != err){
            YYSYS_LOG(WARN, "failed to initialize client_manager, err=%d", err);
          } else {
            transport.start();
          }
          return err;
      ...
      int call(const int32_t pcode, char* buf, int len)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to post request, err=%d", err);
          }
          return err;
        }
    private:
      ...
      yynet::DefaultPacketStreamer packet_streamer;
