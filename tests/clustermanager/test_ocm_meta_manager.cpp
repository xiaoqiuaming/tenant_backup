...
TEST(TestOcmMetaManager, add_ocm_info)
{
  ...
  it++;
  YYSYS_LOG(INFO, "%s", it->get_location());
  ...
  for(int i = 0; i < 1; i++)
  {
    ...
    YYSYS_LOG(INFO, "app %d name=%s", i, tmp_app->get_app_name());

    for(int j = 0; j < 3; j++)
    {
      tmp_instance = tmp_app->get_obi(j);
      YYSYS_LOG(INFO, "instance %d name = %s", j, tmp_instance->get_instance_name());
    }
  }
}
TEST(TestOcmMetaManager, serialize)
{
  ...
  int64_t buf_len = 12;
  int64_t pos = 0;
  YYSYS_LOG(INFO, "only two ocm ,get_serialize_size =%ld", ocm_meta_manager.get_serialize_size());
...
TEST(TestOcmMetaManager, deserialize)
{
  ...
  it++;
  YYSYS_LOG(INFO, "with one ocm, ocm location=%s", it->get_location());
  YYSYS_LOG(INFO,"app count=%ld", it->get_app_count());
  YYSYS_LOG(INFO, "ocm_server name=%s", it->get_ocm_server()->get_hostname());
  const ObOcmAppInfo* app = it->get_app(0);
  for(int i = 0; i < app->get_obi_count(); i++)
  {
    YYSYS_LOG(INFO, "%dth obi name=%s", i, app->get_obi(i)->get_instance_name());
  }
  ...
  it = list->begin();
  it++;
  YYSYS_LOG(INFO, "the first ocm_meta");
  YYSYS_LOG(INFO, "ocm location=%s", it->get_location());
  YYSYS_LOG(INFO,"app count=%ld", it->get_app_count());
  YYSYS_LOG(INFO, "ocm_server name=%s", it->get_ocm_server()->get_hostname());
  app = it->get_app(0);
  for(int i = 0; i < app->get_obi_count(); i++)
  ...
  it ++;
  YYSYS_LOG(INFO, "the second ocm_meta");
  YYSYS_LOG(INFO, "ocm location=%s", it->get_location());
  YYSYS_LOG(INFO,"app count=%ld", it->get_app_count());
  YYSYS_LOG(INFO, "ocm_server name=%s", it->get_ocm_server()->get_hostname());
  app = it->get_app(0);
