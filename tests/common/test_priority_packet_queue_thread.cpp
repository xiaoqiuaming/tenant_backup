...
class TestPacket : public yynet::Packet
{
  public:
    TestPacket()
    {
...
TEST_F(TestPriorityPacketQueueThread, test_push)
{
  ...
  EXPECT_TRUE(((yynet::Packet*) packet1) == priority_thread.head(priority));
  EXPECT_TRUE(((yynet::Packet*) packet2) == priority_thread.tail(priority));
