    ...
    class ObLogGeneratorTest: public ::testing::Test, public Config, public RWT {
      ...
      protected:
        ...
        virtual void TearDown(){
          YYSYS_LOG(INFO, "consumed_cursor=%s", to_cstring(consumed_cursor));
        }
        ...
        int consume_log(const char* buf, int64_t len) {
          ...
          {
            YYSYS_LOG(ERROR, "parse_log_buffer()=>%d", err);
          }
          else
          {
        ...
        int report() {
          ...
          YYSYS_LOG(INFO, "generator=[%s,%s],consumed_cursor=%s", to_cstring(start_cursor),
        ...
        int write(const int64_t idx) {
          ...
          while(!stop_ && OB_SUCCESS == err) {
            ...
            {
              YYSYS_LOG(ERROR, "fill_log()=>%d", err);
            }
            ...
            {
              YYSYS_LOG(ERROR, "consume_log(buf=%p[%ld])=>%d", buf, len, err);
            }
            else if (OB_SUCCESS != (err = log_generator.commit(end_cursor)))
            {
              YYSYS_LOG(ERROR, "commit_log(%s)=>%d", to_cstring(end_cursor), err);
            }
          }
          return err;
        }
