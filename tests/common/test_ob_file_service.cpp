...
class ObFileServer : public common::ObSingleServer
{
  public:
    int initialize()
    {
      ...
      if(OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "init file service fail:ret[%d]", ret);
      }
    ...
    static int process(easy_request_t *r)
    {
      ...
      if (NULL == r)
      {
        YYSYS_LOG(WARN, "request is NULL, r = %p", r);
        ret = EASY_BREAK;
      }
      else if (NULL == r->ipacket)
      {
        YYSYS_LOG(WARN, "request is NULL, r->ipacket = %p", r->ipacket);
        ret = EASY_BREAK;
      }
      else
      {
    ...
    int do_request(ObPacket* base_packet)
    {
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == in_buffer)
        {
          YYSYS_LOG(ERROR, "%s", "in_buffer is NUll should not reach this");
        }
        else
        {
          ...
          {
            YYSYS_LOG(ERROR, "%s", "get thread buffer error, ignore this packet");
          }
        }
      }
      return ret;
...
int start_file_service()
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(ERROR, "failed to start ob file service thread");
  }
  return (ret);
}
