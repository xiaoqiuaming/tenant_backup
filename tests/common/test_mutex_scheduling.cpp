...
#define cfgi(k, v) atoll(cfg(k ,v))
    ...
    class ObMutexSchedulingTest: public ::testing::Test, public yysys::CDefaultRunnable, public Config {
      public:
      ...
      protected:
        ...
        void run(yysys::CThread *thread, void* arg){
          int err = OB_SUCCESS;
          int64_t last_ts = yysys::CTimeUtil::getTime();
          ...
          for(int i = 0; OB_SUCCESS == err; i++) {
            if (using_queued_lock)
            {
              ...
              cur_ts = yysys::CTimeUtil::getTime();
              ...
              {
                YYSYS_LOG(WARN, "cur_ts-last_ts=%ldus\n", cur_ts - last_ts);
              }
              last_ts = cur_ts;
            ...
            {
              yysys::CThreadGuard guard(&mutex);
              cur_ts = yysys::CTimeUtil::getTime();
              if (cur_ts - start_ts > duration)
              {
                break;
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "thread[%ld]=>%d", idx, err);
