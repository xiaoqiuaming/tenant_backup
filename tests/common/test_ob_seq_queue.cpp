...
namespace oceanbase
{
  namespace test
  {
    ...
    class ObSeqQueueTest: public ::testing::Test, public yysys::CDefaultRunnable, public Config {
      public:
        ObSeqQueueTest(){}
      protected:
      ...
      protected:
        ...
        int run_as_get_worker(){
          ...
          while((OB_SUCCESS == err || OB_EAGAIN == err) && !_stop && seq < max_num_items)
          {
            if (OB_SUCCESS == (err = queue.get(cur_seq, (void*&)data, timeout)))
            {
              ...
              {
                YYSYS_LOG(INFO, "cur_seq=%ld", cur_seq);
              }
              if ((cur_seq ^ data) != -1)
              {
                err = OB_INVALID_DATA;
                YYSYS_LOG(ERROR, "cur_seq[%ld] ^ data[%ld] != -1", cur_seq, data);
              }
            }
          }
          YYSYS_LOG(INFO, "get_worker, end_seq=%ld", cur_seq);
          return err;
        }
        ...
        void run(yysys::CThread *thread, void* arg){
          ...
          UNUSED(thread);
          YYSYS_LOG(INFO, "worker[%ld] start", idx);
          if (idx > n_thread)
          {}
          else if(0 == idx)
          {
          ...
          YYSYS_LOG(INFO, "worker[%ld] stop, err=%d", idx, err);
          if (OB_EAGAIN == err)
          {
            err = OB_SUCCESS;
          }
