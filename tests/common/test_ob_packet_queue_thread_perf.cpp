...
class DummyHandler : public ObPacketQueueHandler
{
  public:
    virtual bool handlePacketQueue(ObPacket *packet, void* args)
    {
      ...
      //YYSYS_LOG(INFO, "packet = %p", packet);
      return true;
    }
};

class TestStress : public yysys::CDefaultRunnable
{
  public:
    typedef int64_t                        K;
    typedef int64_t                        V;
  ...
  public:
    ...
    void run(yysys::CThread * thread, void * arg)
    {
      ...
      YYSYS_LOG(INFO, "counter=%ld", counter);
...
TEST(ObPacketQueueThread, perf)
{
  ...
  YYSYS_LOG(INFO, "push_thread: %d  pop_thread: %d  running time: %fs  qps: %f",
