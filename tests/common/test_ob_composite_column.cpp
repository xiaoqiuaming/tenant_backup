...
{  
  ...
  if (NULL == fp)
  {
    YYSYS_LOG(WARN, "fail to open file. filename=%s", filename);
  }
}
...
int ObCellReaderUtil::next_cell()
{
  ...
  if (NULL != fp)
  {
    ...
    if (EOF == c)
    {
      //YYSYS_LOG(WARN, "EOF. filename=%s", filename);
      err = OB_ERROR;
    }
  }
  else
  {
     YYSYS_LOG(WARN, "null file. filename=%s", filename);
     err = OB_ERROR;
  }
  return err;
}
  ...
  if (i < len)
  {
    if (isdigit(data[i]))
    {
      ...
      if (1 == type)
      {
        ...
        //YYSYS_LOG(DEBUG, "int. data=%s", data);
      }
      else if(0 == type)
      {
        cell.value_.set_double(atof(data));
        //YYSYS_LOG(DEBUG, "double. data=%s", data);
      }
    }
    else
    {
      ...
      //YYSYS_LOG(DEBUG, "string. data=%s", data);
      //cell.value_.dump();
    }
  }
  return OB_SUCCESS;
  ...
  while(OB_SUCCESS == cu.next_cell())
  {
    ...
    if(OB_SUCCESS == cu.get_cell(cell, buf))
    {
      ...
      if (cell.value_.get_type() == ObVarcharType)
      {
       ...
       //YYSYS_LOG(DEBUG, "varchar =[%.*s]", str.length(), str.ptr());
        cell.value_.dump();
      }
      else
      {
...
TEST(Test_ob_composite_column, efficiency_test_int)
{
  ...
  YYSYS_LOG(DEBUG, "total rows = %d", 10000 * 1000 / 4);
}
...
TEST(Test_ob_composite_column, single_element_test)
{
  ...
  YYSYS_LOG(DEBUG, "c3 content:[%.*s]", str.length(), str.ptr());
}
