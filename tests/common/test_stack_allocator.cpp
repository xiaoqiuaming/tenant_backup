...
#define cfgi(k, v) atoll(cfg(k ,v))
    ...
    class ObStackAllocatorTest: public ::testing::Test, public yysys::CDefaultRunnable, public Config {
      public:
      ...
      protected:
        ...
        int alloc_once(int64_t idx) {
          ...
          {
            start_batch = false;
            YYSYS_LOG(ERROR, "allocator.start_batch_alloc()=>%d", err);
          }
          else if (need_reserve && OB_SUCCESS != (err = allocator.reserve(size)))
          {
            YYSYS_LOG(ERROR, "reserver(size=%ld)=>%d", size, err);
          }
          else if (need_reserve_big && OB_SUCCESS != (err = allocator.reserve(size + block_size)))
          {
            YYSYS_LOG(ERROR, "reserver(size=%ld)=>%d", size + block_size, err);
          }
          else if (OB_SUCCESS != (err = alloc(size)))
          {
            YYSYS_LOG(ERROR, "allocator.alloc(size=%ld)=>%d", size, err);
          }
          else if (alloc_big && OB_SUCCESS != (err = alloc(size + block_size)))
          {
            YYSYS_LOG(ERROR, "allocator.alloc(size=%ld)=>%d", size + block_size, err);
          }
          if (start_batch && OB_SUCCESS != (err = allocator.end_batch_alloc(rollback)))
          {
            YYSYS_LOG(ERROR, "allocator.end_batch_alloc(rollback=%s)=>%d", STR_BOOL(rollback), err);
          }
          return err;
        }
        void run(yysys::CThread *thread, void* arg){
          ...
          for(int i = 0; OB_SUCCESS == err; i++) {
            ...
            {
              YYSYS_LOG(ERROR, "alloc_once(idx=%ld)=>%d", idx, err);
            }
    ...
    TEST_F(ObStackAllocatorTest, AllocAndClear){
      ...
      start();
      wait();
      YYSYS_LOG(INFO, "total alloc: %ld", block_allocator.get_allocated());
