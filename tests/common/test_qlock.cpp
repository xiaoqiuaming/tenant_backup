  ...
  namespace test
  {
    ...
    struct Node
    {
      ...
      int check(int32_t uid){
        ...
        {
          err = OB_INVALID_DATA;
          YYSYS_LOG(ERROR, "check(uid=%d): ERROR", uid);
        }
        if (OB_SUCCESS == err)
        {
      ...
      int set(int32_t uid, int64_t id){
        ...
        {
          is_locked = false;
          //YYSYS_LOG(ERROR, "lock.try_exclusive_lock(uid=%ld)=>%d", uid, err);
        }
        else if (use_lock_ && OB_SUCCESS != (err = lock_.wait_shared_lock_release(uid)))
        {
          YYSYS_LOG(ERROR, "lock.wait_shared_lock_release(uid=%d)=>%d", uid, err);
        }
        else if (x != y)
        {
          err = OB_INVALID_DATA;
      ...
      {
        YYSYS_LOG(INFO, "uid[%ld]: check=%ld, set=%ld, set2=%ld, total=%ld, n_err=%ld, n_dead_lock=%ld", uid, n_check_, n_set_, n_set2_, n_call_, n_err_, n_dead_lock_);
      }
    };
    class ObQLockTest: public ::testing::Test, public yysys::CDefaultRunnable, public Config {
      public:
        ObQLockTest() {}
        ~ObQLockTest() {}
      protected:
      ...
      protected:
        ...
        int do_work(yysys::CThread *thread, int64_t idx){
          int err = OB_SUCCESS;
          int64_t start_id = 1;
