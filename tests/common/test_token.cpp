...
TEST_F(TestToken, des_encrypt)
{
  ...
  //(4)满足要求
    ...
    YYSYS_LOG(INFO, "**********start to decrypt************");
     //////开始解密
    //des_decrypt(ObString &user_name, int64_t &timestamp, const int64_t skey, char *buffer, int64_t buffer_length)
...
//  for(int i = 0; i < 1000; i++)
  ...
  //  srandom(i);
    ...
    YYSYS_LOG(INFO, "buffer = %s", buffer);
    ObToken token;
    ...
    YYSYS_LOG(INFO, "out_str.length=%d", out_str.length());
    YYSYS_LOG(INFO, "out_str.ptr =%.*s", out_str.length(),out_str.ptr());
    YYSYS_LOG(INFO, "out_str.ptr =%s", out_str.ptr());
    ...
    //YYSYS_LOG(INFO, "length = %d", out_serialize.length());
    //YYSYS_LOG(INFO, "ptr=%.*s", out_serialize.length(), out_serialize.ptr());
    delete[] buf; 
  }
}
