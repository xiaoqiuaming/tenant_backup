...
TEST(ObGetMerger, basic)
{
  ...
  for (int64_t row_idx = 0; row_idx < row_count; row_idx++)
  {
    for (int64_t sub_req_idx = 0; sub_req_idx < sub_req_count; sub_req_idx ++)
    {
      ...
      YYSYS_LOG(INFO, "==== set (sub[%ld],rowkey[%ld]) ====", sub_req_idx, idx);
  ...
  for (int64_t sub_req_idx = 0; sub_req_idx < sub_req_count; sub_req_idx ++)
  {
    // YYSYS_LOG(INFO, "add result %ld", sub_req_idx);
  ...
  for (int64_t sub_req_idx = 0; sub_req_idx < sub_req_count; sub_req_idx ++)
  {
    for (int64_t i = 0; i < row_count; i++)
    {
      ...
      YYSYS_LOG(INFO, " >>>> get (rowkey=%s) <<<<", to_cstring(cur_row));
      //ObRowkey rowkey;
