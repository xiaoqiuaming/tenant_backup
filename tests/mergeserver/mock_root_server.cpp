...
int MockRootServer::handle_fetch_schema(ObPacket * ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "check timestamp failed:timestamp[%ld]", timestamp);
      ret = OB_ERROR;
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "decode only_core_tables failed.");
    }
  }

  YYSYS_LOG(DEBUG, "handle_fetch_schema timestamp=%ld, only_core_tables=%d", timestamp, only_core_tables);
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "%s", "parse from file failed");
      ret = OB_ERROR;
    }
    ...
    {
      YYSYS_LOG(ERROR, "serialize result_msg failed.");
    }
    else if (OB_SUCCESS != (ret = schema.serialize(out_buffer.get_data(), 
            out_buffer.get_capacity(), out_buffer.get_position())))
    {
      YYSYS_LOG(ERROR, "serialize schema failed.");
    }
    else if (OB_SUCCESS != (ret = send_response(OB_FETCH_SCHEMA_RESPONSE, 
            1, out_buffer, connection, channel_id)))
    {
      YYSYS_LOG(ERROR, "send response failed.");
    }
    else
    {
      YYSYS_LOG(DEBUG, "send_response succeed, pos=%ld", out_buffer.get_position());
    }
  }
  return ret;
}
...
int MockRootServer::handle_register_server(ObPacket * ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "%s", "merge server registered");
    }
  }
