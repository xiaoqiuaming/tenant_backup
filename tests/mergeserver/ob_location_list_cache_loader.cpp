...
int ObLocationListCacheLoader::get_decoded_location_list_cache(ObTabletLocationCache & cache)
{
  ...
  if (config_loaded_)
  {
    YYSYS_LOG(INFO, "decoding table location list");
    /*
       if(OB_SUCCESS != cache.init(50000 * 5, 1000, 100000))
       {
       YYSYS_LOG(WARN, "fail to init cache");
       }
    ...
    cache.init(50000 * 5, 1000, 100000);
    {
      ...
      YYSYS_LOG(INFO, "%ld ranges added to the location cache", range_num_);
    }
  }
  return OB_SUCCESS;
}
...
int ObLocationListCacheLoader::load_from_config(const char *location_cache_section_name)
{
  ...
  for (i = 0; i < max_range_count; i++)
  {
    ...
    if(OB_SUCCESS != (ret = load_string((char*)buf2, OB_MAX_ROW_KEY_LENGTH,
            location_cache_section_name, name_buf, 0)))
    {
      YYSYS_LOG(WARN, "should set range_end_");
      continue;
    }
    ...
    if(OB_SUCCESS != (ret = load_string((char*)buf3, OB_MAX_ROW_KEY_LENGTH,
            location_cache_section_name, name_buf, 0)))
    { 
      YYSYS_LOG(WARN, "should set server_list_");
      continue;
    }
    ...
    if (table_id <= 0)
    {
      YYSYS_LOG(WARN, "should set table_id");
      continue;
    }
  ...
  if (range_num_ > 0)
  {
    YYSYS_LOG(INFO, "%ld ranges loaded", range_num_);
    ret = OB_SUCCESS;
  }
  else
  {
    ret = OB_ERROR;
    YYSYS_LOG(INFO, "no range loaded. Fail!");
  }
  return ret;
}
...
{
  int i = 0;
  YYSYS_LOG(INFO, "%s", "location list config:");
  for (i = 0; i < range_num_; i++)
  {
    YYSYS_LOG(INFO, "[%d][%d][%.*s][%.*s][%.*s]", 
        i, 
        table_id_[i], 
