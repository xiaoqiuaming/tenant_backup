...
#define SETUP_ENV()   YYSYS_LOG(INFO, "( 1 )");    \
...
class ObMockMergerAsyncRpcStub : public ObMergerAsyncRpcStub
{
  ...
  public:
        ...
        const ObScanParam & scan_param, ObMergerRpcEvent & result) const
    {
      ...
      {
        YYSYS_LOG(INFO, "currently totally %ld results saved....", rpc_events_list_.get_array_index());
...
TEST(ObMergerScanEvent, parallel_count_limit_with_scanner_fulfill)
{
  ...
  YYSYS_LOG(DEBUG, "rpc_events_list_.size()=%ld", async.rpc_events_list_.get_array_index());
  ...
  YYSYS_LOG(INFO, "totally %ld results saved", async.rpc_events_list_.get_array_index());
