...
public:
  ...
  int get(const int64_t , const common::ObServer & , const common::ObSqlGetParam & get_param, ObMsSqlRpcEvent & rpc_event) const
  {
    ...
    YYSYS_LOG(INFO, "get_param=%s", to_cstring(get_param));
    return OB_SUCCESS;
  }
...
int build_env(MockEnv &env)
{
  ...
  if (rs_proxy == NULL)
  {
    YYSYS_LOG(WARN, "fail to rs_proxy"); 
...
{
  YYSYS_LOG(INFO, "distribute_requests(0)");

  YYSYS_LOG(INFO, "distribute_requests(0.1)");
  MockEnv env;
  build_env(env);
  YYSYS_LOG(INFO, "distribute_requests(0.2)");
  ...
  YYSYS_LOG(INFO, "distribute_requests(1)");
