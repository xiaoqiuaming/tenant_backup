    ...
    const ObString & table_name, const ObRowkey& rowkey, ObScanner & scanner)
{
  ...
  {
    err = OB_ERROR;
    YYSYS_LOG(ERROR, "check return scanner result is empty");
  }
  while ((OB_SUCCESS == err) && (scanner.next_cell() == OB_SUCCESS))
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(ERROR, "check value failed:cell[%ld], check[%ld], rowkey[%ld], column[%lu], count[%ld]",
...
int apply(CParam &param, MockClient &client)
{
  ...
  if (NULL == schema)
  {
    YYSYS_LOG(ERROR, "fail to find table:name[%s]", param.table_name_);
    err = OB_ERROR;
  }
  else
  {
    ...
    for (int64_t i = param.start_key_; i <= param.end_key_; ++i)
    {
      ...
      for (int32_t j = 0; j < size; ++j)
      {
        if (NULL == temp_column)
        {
          YYSYS_LOG(ERROR, "check column info failed:table[%lu], rowkey[%lu]", schema->get_table_id(), i);
          err = OB_ERROR;
          break;
        }
        ...
        if (temp_column->get_type() == ObIntType)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "update table:%.*s rowkey:%ld column:%.*s err:%d",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if ((i != 0) && (0 == i % param.fail_base_))
        {
          ...
          {
            err = OB_ERROR;
            YYSYS_LOG(ERROR, "update failed:talble[%.*s] rowkey[%ld] err[%d]",
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "check result failed:table[%.*s], rowkey[%ld], ret[%d]",
              table_name.length(), table_name.ptr(), i, err);
          break;
        }
        else
        {
          YYSYS_LOG(INFO, "check reusult succ:table[%.*s], rowkey[%ld]",
              table_name.length(), table_name.ptr(), i);
        }
      }
      if (err != OB_SUCCESS)
...
int main(int argc, char **argv)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check apply mutation failed:ret[%d]", err);
    }
  }
  client.destroy();
