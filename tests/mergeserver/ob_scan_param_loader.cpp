...
int ObScanParamLoader::load(const char *config_file_name, const char *section_name)
{
  ...
  {
    YYSYS_LOG(WARN, "fail to load config file [%s]", config_file_name);
    ret = OB_ERROR;
  }
  return ret;
}
...
int ObScanParamLoader::get_org_scan_param(ObScanParam &param)
{
  ...
  if (config_loaded_)
  {
    YYSYS_LOG(INFO, "creating org scan param");
...
int ObScanParamLoader::decoded_scan_param(ObScanParam &org_param, ObScanParam &decoded_param)
{
  ...
  {
    YYSYS_LOG(ERROR, "fail to load schema file [test_schema.ini]");
    ret = OB_ERROR;
  }
...
{
  YYSYS_LOG(INFO, "DUMP ObScanParam:");
  YYSYS_LOG(INFO, "table_name:[%.*s]", param.get_table_name().length(), param.get_table_name().ptr());
  YYSYS_LOG(INFO, "range:%s", to_cstring(*param.get_range()));
}
...
int ObScanParamLoader::load_from_config(const char *scan_param_section_name)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (table_id_ <= 0)
    {
      YYSYS_LOG(ERROR, "invalid table id (%d)", table_id_);
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid/empty table name. err=%d", ret);
      ret = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid/empty range start. err=%d", ret);
      ret = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid/empty %s. err=%d", OBSP_RANGE_END, ret);
      ret = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_RANGE_START_INCLUSIVE, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint != 0 && tmpint != 1)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_RANGE_START_MIN, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint != 0 && tmpint != 1)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_RANGE_END_INCLUSIVE, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint != 0 && tmpint != 1)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_RANGE_END_MAX, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint != 0 && tmpint != 1)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_SCAN_DIRECTION, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint < 0)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_LIMIT_OFFSET, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint < 0)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_LIMIT_COUNT, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  for (i = 0; i < OB_MAX_COLUMN_NUMBER; i++)
  {
    ...
    if(OB_SUCCESS != (ret = load_string((char*)buf, OB_MAX_ROW_KEY_LENGTH,
            scan_param_section_name, name_buf, 0)))
    {
      YYSYS_LOG(WARN, "no as name found for aggregate column %.*s",
  ...
  if (ret != OB_SUCCESS)
  {
    YYSYS_LOG(INFO, "load config return value not SUCCESS. rewrite to success!");
    ret = OB_SUCCESS;
  }
  return ret;
}
    ...
    const char* section, const char* name, bool require)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "%s.%s has not been set.", section, name);
      ret = OB_ERROR;
    }
  ...
  if (OB_SUCCESS == ret && NULL != value)
  {
    ...
    {
      YYSYS_LOG(ERROR, "%s.%s too long, length (%ld) > %d",
...
{
  YYSYS_LOG(INFO, "%s", "scan param config:");

  YYSYS_LOG(INFO, "%s => %d", OBSP_TABLE_ID, table_id_);
  YYSYS_LOG(INFO, "%s => %.*s", OBSP_TABLE_NAME, table_name_.length(), table_name_.ptr());

  if (range_start_min_)
  {
    YYSYS_LOG(INFO, "%s => %s:%s:%s", OBSP_RANGE_START, "",
  ...
  {
    YYSYS_LOG(INFO, "%s => %s:%s:%s", OBSP_RANGE_START, to_cstring(range_start_),
        range_start_inclusive_?"inclusive":"exclusive",
        range_start_min_?"MIN":"");
  }
  ...
  if (range_end_max_)
  {
    YYSYS_LOG(INFO, "%s => %s:%s:%s", OBSP_RANGE_END, "",
  ...
  {
    YYSYS_LOG(INFO, "%s => %s:%s:%s", OBSP_RANGE_END, to_cstring(range_end_),
        range_end_inclusive_?"inclusive":"exclusive",
        range_end_max_?"MAX":"");
  }

  YYSYS_LOG(INFO, "%s => %.*s", OBSP_WHERE_COND, where_cond_.length(), where_cond_.ptr());
  YYSYS_LOG(INFO, "%s => %.*s", OBSP_HAVING_COND, having_cond_.length(), having_cond_.ptr());
  YYSYS_LOG(INFO, "%s => %s", OBSP_SCAN_DIRECTION, scan_direction_==0?"forward":"backward");
  YYSYS_LOG(INFO, "%s:%s => %d:%d", OBSP_LIMIT_OFFSET, OBSP_LIMIT_COUNT, limit_offset_, limit_count_);

  int i = 0;
  for (i = 0; i < column_id_count_; i++)
  {
    YYSYS_LOG(INFO, "%s%d => %.*s:%s", OBSP_COLUMN_ID, i, column_id_[i].length(), column_id_[i].ptr(),
        column_is_return_[i]?"return":"not return");
  }
  for (i = 0; i < complex_column_id_count_; i++)
  {
    YYSYS_LOG(INFO, "%s%d => %.*s:%s", OBSP_COMPLEX_COLUMN_ID, i,
        complex_column_id_[i].length(), complex_column_id_[i].ptr(),
        complex_column_is_return_[i]?"return":"not return");
  }
  for (i = 0; i < agg_column_id_count_; i++)
  {
    YYSYS_LOG(INFO, "%s%d => %.*s:%s", OBSP_AGG_COLUMN_ID, i,
        agg_column_id_[i].length(), agg_column_id_[i].ptr(),
        agg_column_is_return_[i]?"return":"not return");
  }
  for (i = 0; i < orderby_column_id_count_; i++)
  {
    YYSYS_LOG(INFO, "%s%d => %.*s:%s", OBSP_ORDERBY_COLUMN_ID, i,
