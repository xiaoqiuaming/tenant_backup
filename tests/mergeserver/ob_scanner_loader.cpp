...
int ObScannerLoader::load(const char *config_file_name, const char *section_name)
{
  ...
  if (ret == EXIT_SUCCESS)
  {
    YYSYS_LOG(DEBUG, "succ to load config file [%s][%s]", config_file_name, section_name);
...
int ObScannerLoader::get_decoded_scanner(ObScanner &param)
{
  ...
  if (config_loaded_)
  {
    YYSYS_LOG(INFO, "creating scanner");
    ...
    YYSYS_LOG(DEBUG, "range_start_=%s", to_cstring(range_start_));
    YYSYS_LOG(DEBUG, "range_end_=%s", to_cstring(range_end_));
    ...
    if ((fp = fopen(data_file_name_, "rb")) == NULL) {
      YYSYS_LOG(ERROR, "Fail to open %s", data_file_name_);
    }
    else
    {
...
int ObScannerLoader::build_cell_info(int table_id, const ObRowkey& row_key, char *buf, ObScanner &param)
{
  ...
  YYSYS_LOG(DEBUG, "[%lu][%s] [%lu]:[%s]", info.table_id_,
...
void ObScannerLoader::auto_set_val(ObObj &val, char *buf)
{
  if (NULL == buf)
  {
    YYSYS_LOG(ERROR, "invalid scanner data input");
  }
  else
  {
...
int ObScannerLoader::load_from_config(const char* scanner_section_name)
{
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    else
    {
      ...
      YYSYS_LOG(DEBUG, "************%.*s**************", tmpstr.length(), tmpstr.ptr());
    }
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid/empty %s. err=%d", OB_DATA_FILE_NAME, ret);
      ret = OB_INVALID_ARGUMENT;
    }
  }
  ...
  if (tmpint != 0 && tmpint != 1)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_IS_FULLFILLED, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint < 0)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_FULLFILLED_ITEM_NUM, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
  ...
  if (tmpint < 0)
  {
    YYSYS_LOG(ERROR, "invalid %s (%d)", OBSP_DATA_VERSION, tmpint);
    ret = OB_INVALID_ARGUMENT;
  }
  else
  {
...
void ObScannerLoader::dump_config()
{
  ...
  YYSYS_LOG(INFO, "%s:%s => %d:%ld", OBSP_IS_FULLFILLED, OBSP_FULLFILLED_ITEM_NUM, is_fullfilled_, fullfilled_item_num_);
  YYSYS_LOG(INFO, "%s => %ld", OBSP_DATA_VERSION, data_version_);
