...
class ObMockMergerAsyncRpcStub : public ObMergerAsyncRpcStub
{
  ...
  public:
        ...
        const ObScanParam & scan_param, ObMergerRpcEvent & result) const
    {
      ...
      YYSYS_LOG(INFO, "+==============DUMP===============+");
      scanner.dump();
      YYSYS_LOG(INFO, "+=============END DUMP============+");

      if(!rpc_events_list_.push_back(&result))
      {
        YYSYS_LOG(ERROR, "fail to save rpc result");
      }
      else
      {
        YYSYS_LOG(INFO, "currently totally %d results saved....", rpc_events_list_.get_array_index());
        YYSYS_LOG(INFO, "a RPC result save to rpc_event_list_");
      }
      return OB_SUCCESS;
    }
...
TEST(ObMergerScanEvent, no_parallel_count_limit_scanner_fulfill)
{
  ...
  for (i = 0; i < async.rpc_events_list_.get_array_index(); i++)
  {
    ...
    YYSYS_LOG(INFO, "total=%d, finished=%d", 
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "fail to process result %d!", i);
    }
    else
    {
      YYSYS_LOG(INFO, "succ to process result %d ;) finish=%s", i, finish?"true":"false");
    }
  }
...
/*
  YYSYS_LOG(INFO, "first_batch_count=%ld; async.rpc_events_list_length=%ld", 
  ...
  for (i = first_batch_count; i < async.rpc_events_list_.get_array_index(); i++)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "second round: fail to process result %d!", i);
    }
    else
    {
      YYSYS_LOG(INFO, "second round: succ to process result %d ;) finish=%s", i, finish?"true":"false");
    }    
  }
*/
  YYSYS_LOG(INFO, "totally %d results saved", async.rpc_events_list_.get_array_index());
