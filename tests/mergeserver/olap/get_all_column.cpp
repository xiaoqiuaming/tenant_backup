  ...
  void *arg)
{
  ...
  {
    YYSYS_LOG(WARN,"result cell number error [got:%ld,expcted:%ld]", result.get_cell_num(), 
  ...
  for (got_cell_num = 0; (got_cell_num < result.get_cell_num()) && (OB_SUCCESS == err); got_cell_num++)
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to get next cell from result [err:%d]", err);
    }
    ...
    {
      YYSYS_LOG(WARN,"cell content error");
      err = OB_ERR_UNEXPECTED;
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"intvalue error [column_name:%c,expect_val:%ld,got_val:%ld,rowkey:%u]", c_name, 
  ...
  {
    YYSYS_LOG(WARN,"fail to get enough cells from result");
    err = OB_ERR_UNEXPECTED;
  }
