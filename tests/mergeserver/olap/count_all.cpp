  ...
  void *arg)
{
  ...
  {
    YYSYS_LOG(WARN,"expect there is just one cell in the result, but got [%ld]", result.get_cell_num());
    err = OB_ERR_UNEXPECTED;
  }
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = result.next_cell())))
  {
    YYSYS_LOG(WARN,"fail to get first cell [err:%d]", err);
  }
  ...
  {
    YYSYS_LOG(WARN,"unexpected value type [err:%d,type:%d]",err, cell->value_.get_type());
  }
  if ((OB_SUCCESS == err) && (intval != max_key_include - min_key_include + 1))
  {
    YYSYS_LOG(WARN,"count result error [expected:%d,got:%ld]", max_key_include - min_key_include + 1, intval);
    err = OB_ERR_UNEXPECTED;
  }
  if (OB_SUCCESS != err)
  {
  ...
  void *&arg)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to add * column [err:%d]", err);
  }
