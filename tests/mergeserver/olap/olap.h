...
namespace msolap
{
  ...
  inline bool olap_check_cell(const oceanbase::common::ObCellInfo & cell)
  {
    ...
    {
      YYSYS_LOG(WARN,"rowkey size error [size:%ld]",  cell.row_key_.length());
      res = false;
    }
    ...
    {
      res = false;
      YYSYS_LOG(WARN,"fail to get int val from cell");
    }
    if (res && (val != static_cast<int64_t>(olap_get_column_val(cell.column_name_,big_endian_row_key))))
    {
      YYSYS_LOG(WARN,"value not correct [expect:%u,got:%ld]", olap_get_column_val(cell.column_name_,big_endian_row_key), val);
      res = false;
    }
    return res;
  }
