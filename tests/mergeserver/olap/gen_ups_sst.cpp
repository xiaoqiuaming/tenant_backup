...
int fill_sstable_schema(ObSchemaManagerV2 &mgr, const uint64_t table_id, ObSSTableSchema& sstable_schema)
{
  ...
  else
  {
    for (int col_index = 0; col_index < size && OB_SUCCESS == ret; ++col_index)
    {
      ...
      {
        YYSYS_LOG(ERROR,"add column_def(%u,%u,%u) failed col_index : %d",column_def.table_id_,
...
int gen_sst(const gen_param &param)
{
  ...
  if ((NULL == mgr) || (NULL == sst_schema))
  {
    ...
    YYSYS_LOG(WARN,"fail to allocate memory for ObSchemaManagerV2");
  }
  if ((OB_SUCCESS == err) && (!mgr->parse_from_file(param.schema_,config)))
  {
    YYSYS_LOG(WARN,"fail to load schema from file [err:%d,schema.ini:%s]", err, param.schema_);
    err = OB_INVALID_ARGUMENT;
  }
  if ((OB_SUCCESS == err) && (OB_SUCCESS != (err = fill_sstable_schema(*mgr,msolap::target_table_id,*sst_schema))))
  {
    YYSYS_LOG(WARN,"fail to fill sst schema [err:%d, table_id:%lu]", err, msolap::target_table_id);
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to create sst [err:%d,sst_path:%s]", err, param.sst_name_);
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to call sst_row.set_obj_count [err:%d]", err);
  }
  for (uint32_t row_key = param.start_include_; (OB_SUCCESS == err) && (row_key <= param.end_include_); row_key++)
  {
    ...
    for (uint64_t column_id = msolap::min_column_id; (OB_SUCCESS == err)&&(column_id <= msolap::max_column_id); column_id ++)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to add obj to sst row [err:%d]", err);
      }
    }
    ...
    {
      YYSYS_LOG(WARN,"fail to append row to sst writer [err:%d]", err);
    }
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to close sst [err:%d]", err);
  }
  else
  {
    YYSYS_LOG(INFO, "sst write success [trailer_offset:%ld,sst_size:%ld]", trailer_offset, sst_size);
  }
  delete sst_schema;
  delete mgr;
  return err;
