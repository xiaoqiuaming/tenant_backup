...
int OlapConf::parse_merge_server(char* str)
{
  ...
  for (begin_ptr = str; begin_ptr != end_ptr && OB_SUCCESS == ret;)
  {
    ...
    {
      YYSYS_LOG(WARN,"unrecogonizable ip addr:%s", buffer);
...
int OlapConf::init(const char *conf_file)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to load configuration");
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (NULL == ms_list)
    {
      YYSYS_LOG(WARN,"fail to get ms_list from configuration");
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to parse merge server list:%s", ms_list_buf);
      }
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (NULL == log_path)
    {
      YYSYS_LOG(WARN,"fail to get log_path");
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (NULL == log_level)
    {
      YYSYS_LOG(WARN,"fail to get log_level");
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == err)
  {
    YYSYS_LOGGER.setFileName(log_path_.c_str());
    YYSYS_LOGGER.setLogLevel(log_level_.c_str());
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"read_thread_count should bigger than 0 [read_thread_count:%ld]", read_thread_count_);
      err  = OB_INVALID_ARGUMENT;
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"network_timeout_us should bigger than 0 [network_timeout_us:%ld]", network_timeout_us_);
      err  = OB_INVALID_ARGUMENT;
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (val <= 0)
    {
      YYSYS_LOG(WARN,"start_key should bigger than 0 [start_key:%ld]", val);
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (val <= 0)
    {
      YYSYS_LOG(WARN,"end_key should bigger than 0 [end_key:%ld]", val);
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
    ...
    {
      YYSYS_LOG(WARN,"end_key must bigger than start_key [start_key:%u,end_key:%u]", start_key_, end_key_);
      err  = OB_INVALID_ARGUMENT;
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (val <= 0)
    {
      YYSYS_LOG(WARN,"scan_count_min should bigger than 0 [scan_count_min:%ld]", val);
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (val <= 0)
    {
      YYSYS_LOG(WARN,"scan_count_max should bigger than 0 [scan_count_max:%ld]", val);
      err  = OB_INVALID_ARGUMENT;
    }
    else
    {
    ...
    {
      YYSYS_LOG(WARN,"scan_count_max must bigger than scan_count_min [scan_count_min:%u,scan_count_max:%u]", 
  ...
  if (OB_SUCCESS == err)
  {
    YYSYS_LOG(INFO,"merge_server_count:%zu", ms_server_vec_.size());
    for (size_t i = 0; i < ms_server_vec_.size(); i++)
    {
      YYSYS_LOG(INFO, "ms_:%zu,ip:%s,port:%u", i, ms_server_vec_[i].ip_str_, ms_server_vec_[i].port_);
    }

    YYSYS_LOG(INFO, "log_path:%s", log_path_.c_str());
    YYSYS_LOG(INFO, "log_level:%s", log_level_.c_str());

    YYSYS_LOG(INFO, "read_thread_count:%ld", read_thread_count_);
    assert(end_key_ > start_key_);
    YYSYS_LOG(INFO, "start_key:%u", start_key_);
    YYSYS_LOG(INFO, "end_key:%u", end_key_);

    YYSYS_LOG(INFO, "scan_count_min:%u", scan_count_min_);
    YYSYS_LOG(INFO, "scan_count_max:%u", scan_count_max_);
    assert(scan_count_max_ > scan_count_min_);


    YYSYS_LOG(INFO, "network_timeout_us:%ld", network_timeout_us_);
  }
  return err;
}
