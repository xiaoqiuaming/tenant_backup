  ...
  void *arg)
{
  ...
  {
    YYSYS_LOG(WARN,"whole row count not correct [expect:%ld,got:%ld]", whole_row_count, 
    ...
    row_key --)
  {
    ...
    if (where_c_val == where_right_operand)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to get row from scanner [err:%d]", err);
      }
      else if (row_width != expect_row_width)
      {
        YYSYS_LOG(WARN,"row width not correct [row_width:%ld, expect_row_width:%ld]", row_width, expect_row_width);
        err = OB_ERR_UNEXPECTED;
      }
      else
      {
        for (uint64_t cid = msolap::min_column_id;(cid <= msolap::max_column_id) && (OB_SUCCESS == err); cid ++)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to get int val from obj [err:%d, cid:%lu]", err, cid);
          }
          else if (intval != msolap::olap_get_column_val(cid,big_endian_key))
          {
            YYSYS_LOG(WARN,"column value not correct [cid:%lu,expect_val:%u,real_val:%ld]", cid, 
          ...
          {
            YYSYS_LOG(WARN,"unexpect column_name [cid:%lu,cname:%.*s]", cid, 
          ...
          {
            YYSYS_LOG(WARN,"unexpect table name [table_name:%.*s]", row_cells[cid - msolap::min_column_id].table_name_.length(), 
          ...
          {
            YYSYS_LOG(WARN,"unexpect rowkey");
            err = OB_ERR_UNEXPECTED;
          }
        }
        if (OB_SUCCESS == err)
  ...
  {
    YYSYS_LOG(WARN,"row count not correct [hit_row_count:%ld,expect_hit_row_count:%ld]", hit_row_count, expect_hit_row_count);
    res = false;
  }
  return res;
}
  ...
  void *&arg)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to init scan param [err:%d]", err);
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to add composite column");
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to add where condition [err:%d]", err);
  }
  ...
  {
    YYSYS_LOG(WARN,"fail to add orderby column [err:%d]", err);
  }
  if (OB_SUCCESS == err)
  {
