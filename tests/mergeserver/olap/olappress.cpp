...
void * worker(void * arg)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to allocate memory for ObScanParam");
    err = OB_ALLOCATE_MEMORY_FAILED;
  }
  if ((OB_SUCCESS == err) && (NULL == (get_param = new(std::nothrow)ObGetParam)))
  {
    YYSYS_LOG(WARN,"fail to allocate memory for ObGetParam");
    err = OB_ALLOCATE_MEMORY_FAILED;
  }
  ...
  int64_t beg = yysys::CTimeUtil::getTime();
  while ((OB_SUCCESS == err) && !g_stopped)
  {
    ...
    if (OlapBaseCase::SCAN == cur_case->get_req_type())
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to form ObScanParam [err:%d]", err);
      }
      if ((OB_SUCCESS == err) 
        && (OB_SUCCESS != ( err = g_client.ups_scan(*param,result, conf->get_network_timeout(), mss[ms_idx]))))
      {
        YYSYS_LOG(WARN,"fail to get result from ms [err:%d,start_key:%u,endkey:%u,case_name:%s,ms_ip:%s,ms_port:%u]", err,start_key, 
    ...
    else
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to form ObGetParam [err:%d]", err);
      }
      if ((OB_SUCCESS == err)
    ...
    {
      YYSYS_LOG(WARN,"case fail [start_key:%u,endkey:%u,case_name:%s,ms_ip:%s,ms_port:%u]", start_key, 
        end_key,cur_case->get_name(), conf->get_ms_servers()[ms_idx].ip_str_, conf->get_ms_servers()[ms_idx].port_);
    ...
    if (OB_SUCCESS == err)
    {
      ...
      YYSYS_LOG(INFO, "case success [start_key:%u,endkey:%u,case_name:%s,ms_ip:%s,ms_port:%u,used_us:%ld,type:%s]",  start_key, 
        end_key,cur_case->get_name(), conf->get_ms_servers()[ms_idx].ip_str_, conf->get_ms_servers()[ms_idx].port_, 
...
int main(int argc, char **argv)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to load configuration [err:%d]", err);
  }

  if (OB_SUCCESS != (err =  g_client.init()))
  {
    YYSYS_LOG(WARN,"fail to init MockClient [err:%d]", err);
  }
