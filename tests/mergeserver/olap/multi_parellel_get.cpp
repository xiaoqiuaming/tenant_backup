  ...
  void *arg)
{
  ...
  {
    YYSYS_LOG(WARN,"result cell count not correct [got_cell_count:%ld,request_cell_count:%ld]", result.get_cell_num(), 
  ...
  {
    YYSYS_LOG(WARN,"result not ended as expected");
    err = OB_ERR_UNEXPECTED;
  }
  if (OB_SUCCESS != err)
  {
  ...
  void *&arg)
{
    ...
    cell_idx ++,start_key = static_cast<int32_t>(start_key + random_selector))
  {
    ...
    {
      YYSYS_LOG(WARN,"fail to add cell to ObGetParam [err:%d,c_name:%c]", err, c_name);
    }
  }
  if (OB_SUCCESS == err)
  {
