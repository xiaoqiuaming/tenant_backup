    ...
    const ObString & table_name, ObScanner & scanner)
{
  ...
  {
    err = OB_ERROR;
    YYSYS_LOG(ERROR, "check read scanner result is empty");
  }
  while ((OB_SUCCESS == err) && (scanner.next_cell() == OB_SUCCESS))
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "get cell failed:ret[%d]", err);
      break;
    }
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "check table name rowkey or type failed");
      ...
      else
      {
        ...
        if (NULL == column)
        {
          YYSYS_LOG(ERROR, "find column info failed");
          err = OB_ERROR;
          break;
        }
        ...
        if (row_change)
        {
          YYSYS_LOG(INFO, "test scan succ:table[%lu], rowkey[%ld]", column->get_table_id(), row_key);
        }
        cur_cell->value_.get_int(value);
        // YYSYS_LOG(INFO, "check value:value[%ld], rowkey[%ld], column[%lu]", value, row_key, column_id);
        if (value != (((row_key << 24) | ((int64_t)column_id << 16)) | (count + 1)))
        {
          YYSYS_LOG(ERROR, "check value failed:value[%ld], new[%ld], rowkey[%ld], column[%lu], count[%ld]",
...
int scan(CParam &param, MockClient &client)
{
  ...
  if (NULL == schema)
  {
    YYSYS_LOG(ERROR, "check schema failed:id[%s]", param.table_name_);
    err = OB_ERROR;
  }
  else
  {
    ...
    for (int32_t j = 0; j < size; ++j)
    {
      if (NULL == column_info)
      {
        YYSYS_LOG(ERROR, "check column info failed:table[%lu]", schema->get_table_id());
        err = OB_ERROR;
        break;
      }
      ...
      if (column_info->get_type() == ObIntType)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "add column failed:table[%lu], ret[%d]", schema->get_table_id(), err);
          break;
        }
      }
      ++column_info;
    ...
    while (err == OB_SUCCESS)
    {
      ...
      if (err != OB_SUCCESS)
      {
        YYSYS_LOG(ERROR, "check scan failed:table[%lu], ret[%d]", schema->get_table_id(), err);
      }
      else
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "check result failed:table[%lu], ret[%d]", schema->get_table_id(), err);
        }
        else
        {
          YYSYS_LOG(INFO, "check result succ:table[%lu]", schema->get_table_id());
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        if (true == param.backward_)
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "check get first rowkey failed:ret[%d]", err);
          }
        }
        else
        {
          ...
          if (err != OB_SUCCESS)
          {
            YYSYS_LOG(ERROR, "check get last rowkey failed:ret[%d]", err);
          }
        }
      }
      ...
      if (OB_SUCCESS == err)
      {
        ...
        if (err != OB_SUCCESS)
        {
          YYSYS_LOG(ERROR, "get fullfilled status failed:ret[%d]", err);
        }
        else if (is_fullfill)
        {
          break;
...
int main(int argc, char **argv)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (err != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check get failed:ret[%d]", err);
    }
  }
  client.destroy();
