...
int check_schema(CParam &param)
{
  ...
  if (NULL == left_schema)
  {
    YYSYS_LOG(WARN,"left table not exist [table_name:%.*s]", param.left_table_.length(), param.left_table_.ptr());
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"right table not exist [table_name:%.*s]", param.right_table_.length(), param.right_table_.ptr());
      err = OB_INVALID_ARGUMENT;
    }
    else
    {
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN,"left table's rowkey len must be %zu", sizeof(left_rowkey_t));
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN,"left table's rowkey len must be fixed");
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    */
    for (;column_begin < column_end && OB_SUCCESS == err; column_begin++)
    {
      ...
      if (NULL != join_info)
      {
        ...
        {
          YYSYS_LOG(WARN,"join info not correct [left.column_id:%s,right.table_id:%lu,real_right.table_id:%lu]",
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"wrong join rowkey range [start_pos:%d,end_pos:%d,expedted_start_pos:%zu,expected_end_pos:%zu]",
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"left id and right id not equal [left_id:%lu,right_id:%lu]", column_begin->get_id(), right_column_id);
            err = OB_INVALID_ARGUMENT;
          }
        }
      }
      ...
      {
        YYSYS_LOG(WARN,"column id too max, MAX_COLUMN_ID:%ld", MAX_COLUMN_ID);
        err = OB_INVALID_ARGUMENT;
      }
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN,"right table's rowkey len must be %zu", sizeof(rowkey_t));
    }
  }
  if (OB_SUCCESS == err)
  {
    ...
    {
      err = OB_INVALID_ARGUMENT;
      YYSYS_LOG(WARN,"right table's rowkey len must be fixed");
    }
  }
  return err;
}
...
int parse_cmd_args(int argc, char **argv, CParam & param)
{
  ...
  if (OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN, "%s", "fail to allocate memory for schema manager");
  ...
  {
    YYSYS_LOG(WARN,"fail to load schema from file [fname:%s]", param.schema_fname_);
    err = OB_SCHEMA_ERROR;
  }
  if (OB_SUCCESS == err)
  {
...
int delete_rows(CParam &param, MockClient &client)
{
  ...
  for (int64_t i = param.start_key_; OB_SUCCESS == err && i < param.end_key_; i++)
  {
    ...
    if (OB_SUCCESS == err && i > 0 && ( i % each_update_row_num == 0 || i == param.end_key_ - 1))
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to update ups [err:%d]", err);
      }
      else
      {
        YYSYS_LOG(WARN,"update [i:%ld,param.start_key_:%ld,param.end_key_:%ld]", i,
          param.start_key_, param.end_key_);
        req_sended = true;
        mutator.reset();
      }
  ...
  if (!req_sended)
  {
    ...
    {
      YYSYS_LOG(WARN,"update [param.start_key_:%ld,param.end_key_:%ld]",
        param.start_key_, param.end_key_);
      req_sended = true;
      mutator.reset();
    }
    ...
    while (OB_SUCCESS == err && column_beg < column_end)
    {
      if (column_beg->get_type() == ObIntType)
      {
        if ((join_info =  column_beg->get_join_info())!= NULL)
        {
          ...
          {
            YYSYS_LOG(WARN,"argumet error, cann't find right column info [columnid:%lu]", column_beg->get_id());
            err = OB_INVALID_ARGUMENT;
          }
          else
          {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to add muatator [err:%d]", err);
          }
          else
          {
            req_sended = false;
...
int check_result(CParam &param, ObScanner &result, uint32_t start_key, ObScanParam::Direction direction)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to next cell from result [err:%d]", err);
  }
  if (OB_SUCCESS == err && ObScanParam::BACKWARD == direction)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to get first cell from ObScanner [err:%d]", err);
    }
    else
    {
  ...
  while (OB_SUCCESS == err)
  {
      ...
      column_beg < column_end && OB_SUCCESS == err; column_beg ++)
    {
      if (column_beg->get_type() == ObIntType)
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to get cell from scanner [err:%d]", err);
        }
        if (OB_SUCCESS == err)
        {
          err = result.next_cell();
          ...
          {
            YYSYS_LOG(WARN,"result err, not row width");
            err = OB_ERR_UNEXPECTED;
          }
        }
      }
...
int scan(CParam &param, MockClient &client)
{
  ...
  for (;column_beg < column_end && OB_SUCCESS == err; column_beg++)
  {
    if (column_beg->get_type() == ObIntType)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to add column to scan param, [err:%d]", err);
      }
    }
  }
  ...
  while (cur_key < param.end_key_ && OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to scan from server [err:%d]", err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"check result fail");
      }
      else
      {
        ...
        {
          YYSYS_LOG(WARN,"fail to get last key from scanner [err:%d,last_row_key.length():%ld]", err,
            last_row_key.length());
        }
        else
        {
    ...
    YYSYS_LOG(WARN,"scan one round [cur_key:%d,param.start_key_:%ld,param.end_key_:%ld]", cur_key,
      param.start_key_, param.end_key_);
  }
  /// scan backward
  YYSYS_LOG(WARN,"scan backward");
    ...
    else
    {
      ...
      if (OB_SUCCESS == err)
      {
        ...
        else
        {
          ...
          {
            YYSYS_LOG(WARN,"ms error [expect_last_rowkey:%u,real:%u]", cur_key,((rowkey_t*)&last_key_val)->val_);
            err = OB_ERR_UNEXPECTED;
          }
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN,"fail to get first cell from result [err:%d]", err);
          }
        }
        else
        {
          YYSYS_LOG(WARN,"fail to reset iterator of result [err:%d]", err);
        }
      }
    }
    YYSYS_LOG(WARN,"scan one round [cur_key:%d,param.start_key_:%ld,param.end_key_:%ld]", cur_key,
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to add aggregate column [err:%d]", err);
    }
    ...
    {
      YYSYS_LOG(WARN,"fail to add having condition [err:%d]", err);
    }
    ...
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to scan server [err:%d]", err);
      }
      else
      {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        if (OB_SUCCESS != err && OB_ITER_END != err)
        {
          YYSYS_LOG(WARN,"fail to get next cell [err:%d]", err);
        }
        if (OB_SUCCESS == err)
        {
          err = result.get_cell(&cur_cell,&is_row_changed);
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN,"fail to get cell [err:%d]", err);
          }
          else
          {
            cell_num ++;
            ...
            {
              YYSYS_LOG(WARN,"fail to get int val from result");
              err = OB_ERR_UNEXPECTED;
            }
            else
            {
              ...
              {
                YYSYS_LOG(WARN,"count result error, [count:%ld,expect_count:%ld,param.end_key_:%ld,param.start_key_:%ld]", count,
                  expect_count, param.end_key_, param.start_key_);
                err = OB_ERR_UNEXPECTED;
              }
              else
              {
                YYSYS_LOG(WARN,"count:%ld,time_us:%ld", count,time_us);
              }
            }
          }
        }
      ...
      if (cell_num != 1)
      {
        YYSYS_LOG(WARN,"result cell number wrong [cell_num:%ld]", cell_num);
      }
    }
    if (OB_ITER_END == err)
    {
