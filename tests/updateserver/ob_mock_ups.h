  ...
  namespace test
  {
    class MockLogMgr : public ObILogWriter
    {
      public:
        ...
        {
          YYSYS_LOG(INFO, "[MOCK] new_log_file_id=%lu", new_log_file_id);
          return OB_SUCCESS;
        };
        int write_replay_point(uint64_t replay_point)
        {
          YYSYS_LOG(INFO, "[MOCK] replay_point=%lu", replay_point);
          return OB_SUCCESS;
        };
    };
    ...
    class ObMockUps
    {
      public:
        ...
        updateserver::TableItem* get_active_table_item() {
          ...
          {
            YYSYS_LOG(ERROR, "table_mgr == NULL");
          }
          else if (NULL == (table_item = table_mgr->get_active_memtable()))
          {
            YYSYS_LOG(ERROR, "get_active_memtable()=>NULL");
          }
          return table_item;
        }
        ...
        struct SessionGuard
        {
          ...
          SessionGuard(ObMockUps& ups): session_ctx_(NULL), session_descriptor_(0), session_mgr_(ups.get_session_mgr()) {
            int err = OB_SUCCESS;
            updateserver::TableItem* table_item = NULL;
            if (OB_SUCCESS != (err = session_mgr_.begin_session(type, yysys::CTimeUtil::getTime(), INT64_MAX, INT64_MAX, session_descriptor_)))
            {
              YYSYS_LOG(ERROR, "begin_session()=>%d", err);
            }
            else if (NULL == (session_ctx_ = (RWSessionCtx*)session_mgr_.fetch_ctx(session_descriptor_)))
            {
              YYSYS_LOG(ERROR, "fetch_ctx():FAILED");
            }
            else if (NULL == ups.get_lock_mgr().assign(lock_type, *session_ctx_))
            {
              YYSYS_LOG(ERROR, "assign_lock()=>NULL");
            }
      ...
      public:
        int init(BaseConfig& cfg) {
          ...
          {
            YYSYS_LOG(ERROR, "schema_wrapper.set_impl()=>%d", err);
          }
          else if (OB_SUCCESS != (err = table_mgr_.set_schemas(schema_wrapper)))
          {
            YYSYS_LOG(ERROR, "table_mgr.set_schema()=>%d", err);
          }
          return err;
        }
