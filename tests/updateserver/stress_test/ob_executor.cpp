...
int ObExecutor::exec(const ObTestBomb &bomb)
{
  ...
  switch (bomb.get_type())
  {
    case UPDATE_BOMB:
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "exec_update error, ret=%d", ret);
      }
      break;
    ...
    case SCAN_BOMB:
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "exec_scan error, ret=%d", ret);
      }
      break;
...
int ObExecutor::exec_scan(const ObTestBomb &bomb)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "ups_scan error, ret=%d", ret);
  }
  else
  {
...
//        YYSYS_LOG(DEBUG, "cell column_name=%.*s type=%d value=%ld",
//            cell->column_name_.length(), cell->column_name_.ptr(),
//            cell->value_.get_type(), value);
      }
      else
      {
//        YYSYS_LOG(DEBUG, "cell column_name=%.*s type=%d",
//            cell->column_name_.length(), cell->column_name_.ptr(),
//            cell->value_.get_type());
      ...
      if (ObExtendType == cell->value_.get_type())
      {
        ...
        YYSYS_LOG(DEBUG, "cell ext=%d", ext);
      }
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "ObScanner get_cell error, ret=%d", ret);
      }
      else
      {
        if (row_changed)
        {
          ...
          else
          {
            ...
            if (rk_p2 != sum)
            {
              ...
              YYSYS_LOG(ERROR, "INCORRECT ROW[%s], rk_p1=%ld rk_p2=%ld sum=%ld", str, rk_p1, rk_p2, sum);
              ret = OB_ERROR;
            }
        ...
        if (OB_SUCCESS != ret)
        {
          ...
          {
            YYSYS_LOG(WARN, "cell->value_ get_int error, ret=%d", ret);
          }
          else
          {
            ret = OB_SUCCESS;
    ...
    if (first_time)
    {
      YYSYS_LOG(ERROR, "NO DATA"); ret = OB_ERROR;
    }
    else
    {
      int64_t rk_p1;
      ...
      else
      {
        ...
        YYSYS_LOG(DEBUG, "SCAN RES [%s] #%d#", str, cell_no);
      }
    }
