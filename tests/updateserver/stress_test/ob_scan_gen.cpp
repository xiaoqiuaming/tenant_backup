...
int ObScanGen::gen(ObTestBomb &bomb)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "encode_i64 rk_p1 error, ret=%d rk_p1=%ld", ret, rk_p1);
  }
  else
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "encode_i64 rk_p2_start error, ret=%d rk_p2_start=%ld", ret, rk_p2_start);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "encode_i64 rk_p2_end error, ret=%d rk_p2_end=%ld", ret, rk_p2_end);
      }
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (int i = 0; OB_SUCCESS == ret && i < schema.get_column_num(); i++)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "ObStringBuf write_string error, ret=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "ObScanParam add_column error, ret=%d", ret);
        }
      }
    }
    ...
    YYSYS_LOG(DEBUG, "SCAN GEN [%s]", str);
