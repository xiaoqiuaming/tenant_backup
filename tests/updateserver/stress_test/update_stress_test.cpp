...
class UpdateStressTest
{
  ...
  public:
    ...
    int init(ObSchemaManagerWrapper& schema_wrapper)
    {
      ...
      {
        YYSYS_LOG(WARN, "num_int_columns should be >= 2, num_int_columns_=%ld", param_.num_int_columns_);
        err = OB_ERROR;
      }

      YYSYS_LOG(INFO, "table_name=%s, num_int_columns=%ld, num_other_columns=%ld, err=%d",
  ...
  public:
        ...
        const int64_t num_int_columns, const ObString* int_columns_name, ObMutator& mutator)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to build int write params, err=%d", err);
      }
        ...
        const ColumnType* other_columns_type, ObMutator& mutator)
    {
      ...
      for (int64_t i = 0; OB_SUCCESS == err && i < num_other_columns; ++i)
      {
        ...
        switch (type)
        {
          case ObVarcharType:
            ...
            break;
          default:
            //YYSYS_LOG(WARN, "unknown type, omit it, type=%d", type);
            found = false;
            break;
        }
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to build other write params, err=%d", err);
      }
      return err;
    }
        ...
        const ObVersionRange& version_range, ObGetParam& get_param)
    {
      ...
      while (OB_SUCCESS == err && OB_SUCCESS == (err = mutator.next_cell()))
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to get_cell, err=%d", err);
          err = OB_ERROR;
        }
        else
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to add cell to get_param, err=%d", err);
          }
        }
      }
      ...
      {
        YYSYS_LOG(WARN, "error occurs when translate mutator to get_param, err=%d", err);
      }
    ...
    static int verify_int_res(ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS == err)
      {
        if (0 != sum)
        {
          YYSYS_LOG(ERROR, "invalid sum, sum=%ld", sum);
          err = OB_ERROR;
        }
      }
    ...
    static int verify_res(ObMutator& mutator, ObScanner& scanner)
    {
      ...
      while (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN, "failed to call next, err1=%d, err2=%d", err1, err2);
          err = OB_ERROR;
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(WARN, "failed to call next cell, err1=%d, err2=%d, "
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN, "table name or row key not equal");
              err = OB_ERROR;
            }
            else if (mutator_cell->cell_info.column_name_ != scanner_cell->column_name_)
            {
              YYSYS_LOG(WARN, "column name not equal");
              err = OB_ERROR;
            }
            else
            {
              ...
              if (type1 != type2)
              {
                YYSYS_LOG(WARN, "cell type not equal");
                err = OB_ERROR;
              }
              else
              {
                  ...
                  case ObVarcharType:
                    {
                      ...
                      if (str1 != str2)
                      {
                        YYSYS_LOG(WARN, "value is not equal, mutator_str=%.*s, scanner_str=%.*s",
                    ...
                    break;
                  default:
                    YYSYS_LOG(WARN, "unknown type, omit it, type=%d", type1);
                    err = OB_ERROR;
                    break;
                }
              }
    ...
    static int get_data(ObGetParam& get_param, ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to get data ,err=%d", err);
      }
    ...
    static int scan_data(ObScanParam& scan_param, ObScanner& scanner)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to scan data, err=%d", err);
      }
    ...
    static int write_data(ObMutator& mutator)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to call ups_apply, err=%d", err);
      }
    ...
    static int translate_row_key(const int64_t row_id, ObString& row_key)
    {
      ...
      if (num != 8)
      {
        YYSYS_LOG(WARN, "error occurs, num=%ld", num);
        err = OB_ERROR;
      }
      return err;
    }
    ...
    static void* read_thread_routine(void* arg)
    {
      ...
      while (!is_stop_)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to build get param, err=%d", err);
        }
        else
        {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          while (OB_SUCCESS == err && OB_SUCCESS == (err=scanner.next_cell()))
          {
            ...
            if (OB_SUCCESS != err || NULL == cell)
            {
              YYSYS_LOG(WARN, "failed to get_cell, cell=%p, err=%d", cell, err);
              err = OB_ERROR;
            }
            else
            {
              ...
              if (ObExtendType == type)
              {
                ...
                {
                  YYSYS_LOG(WARN, "invalid ext_val, exe_val=%ld", tmp_val);
                  err = OB_ERROR;
                }
                else
                {
              ...
              {
                YYSYS_LOG(WARN, "invalid type, obj_type=%d", type);
                err = OB_ERROR;
              }
            }
          }
          ...
          {
            YYSYS_LOG(INFO, "verify read success, row_key=%ld, row_exist=%s", row_idx,
                row_exist ? "true" : "false");
          }
          else
          {
            YYSYS_LOG(WARN, "verify read failed, row_key=%ld, sum=%ld, err=%d",
                row_idx, sum, err);
            int64_t cell_size = get_param.get_cell_size();
            for (int64_t i = 0; i < cell_size; ++i)
            {
              YYSYS_LOG(WARN, "the %ld-th column is %.*s", i, (int) get_param[i]->column_name_.length(),
      ...
      YYSYS_LOG(INFO, "read thread STOP, err=%d", err);
      return NULL;
    }
    ...
    static void* scan_routine(void* arg)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to build scan param, err=%d", err);
      }
      else
      {
        bool is_end = false;
        while (OB_SUCCESS == err && false == is_end)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to scan data, table_name=%.*s, err=%d",
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to verify int res, err=%d", err);
            }
          }
          ...
          if (OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to get is_req_fullfilled, err=%d", err);
            }
            else
            {
              is_end = is_full;
          ...
          if (OB_SUCCESS == err && !is_end)
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to get last row key, err=%d", err);
            }
            else
            {
    ...
    static void* int_update_thread_routine(void* arg)
    {
      ...
      while (!is_stop_)
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN, "failed to build int write param, err=%d", err);
        }
        else
        {
          mutator.reset_iter();
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to build get param from mutator, err=%d", err);
          }
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "failed to write data, err=%d", err);
          }
        }
        ...
        if (OB_SUCCESS == err)
        {
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN, "failed to verify res, err=%d", err);
            }
          }
        }
        */
        ...
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(INFO, "write int success, row_key=%ld", row_idx);
        }
        else
        {
          YYSYS_LOG(WARN, "write int failed, row_key=%ld, err=%d",
              row_idx, err);
        }
      }

      YYSYS_LOG(INFO, "int write thread STOP, err=%d", err);
      return NULL;
    }
    ...
    static void* other_update_thread_routine(void* arg)
    {
      ...
      while (!is_stop_)
      {
        ...
        if (OB_SUCCESS == err)
        {
          YYSYS_LOG(INFO, "write other type success, row_key=%ld", row_idx);
        }
        else
        {
          YYSYS_LOG(WARN, "write other type failed, row_key=%ld, err=%d",
              row_idx, err);
        }
      }

      YYSYS_LOG(INFO, "other type write thread STOP, err=%d", err);
      return NULL;
    }
};
...
static void sign_handler(const int sig)
{
  switch (sig) {
    ...
    case SIGINT:
      ...
      break;
    default:
      YYSYS_LOG(WARN, "unknown signal, sig=%d", sig);
      break;
  }
}
...
int main(int argc, char** argv)
{
  ...
  YYSYS_LOGGER.setFileName("./stress.log");
  YYSYS_LOGGER.setMaxFileSize(100 * 1024L * 1024L); // 100M per log file
  ...
  if (false == ret_val)
  {
    YYSYS_LOG(WARN, "failed to parse schema file, file_name=%s", param.schema_file_);
    exit(1);
  }
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN, "failed to init, err=%d", err);
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to init merge client, err=%d", err);
    }
    else
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to init update client, err=%d", err);
      }
    }
  }
