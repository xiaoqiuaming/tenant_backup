...
int main(int argc, char* argv[])
{
  ...
  if (0 == ret)
  {
    YYSYS_LOGGER.setFileName(param.log_name.ptr(), true);
    YYSYS_LOGGER.setLogLevel(param.log_level);
  }
  ...
  if (0 == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "Runnable is NULL");
    }
    elapsed_time = yysys::CTimeUtil::getTime() - start_time;
  }
...
#define STAT_FORMAT "elapsed time %lds, " \
    ...
    fprintf(stderr, STAT_FORMAT, elapsed_time / 1000000,
                                 ...
                                 stat.get_max_rt(),
                                 stat.get_min_rt());
    YYSYS_LOG(INFO, STAT_HEAD, param.str_tr_type(param.tr_type), param.ip, param.port);
    YYSYS_LOG(INFO, STAT_FORMAT, elapsed_time / 1000000,
                                 stat.get_sum_num() + stat.get_sum_retry_num(),
                                 stat.get_sum_succ_num(),
                                 failed_percent,
                                 average_response_time,
