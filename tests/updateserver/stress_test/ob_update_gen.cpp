...
int ObUpdateGen::gen(ObTestBomb &bomb)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "ObMutator reset error, ret=%d", ret);
  }
  else
  {
    int n_sum = 0;
    for (int i = 0; OB_SUCCESS == ret && i < line_num; i++)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "encode_i64 row_key_part1 error, ret=%d row_key_part1=%ld", ret, row_key_part1);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "encode_i64 row_key_part2 error, ret=%d row_key_part2=%ld", ret, row_key_part1);
        }
      }
...
int ObUpdateGen::gen_line(int n, ObString& row_key, int64_t sum, ObRandom& ran, common::ObMutator& mut)
{
  ...
  for (int j = 0; j < n; j++)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "ObMutator update error, ret=%d", ret);
    }
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "line_sum is not equal sum: line_sum=%ld sum=%ld", line_sum, sum);
  }
  else
  {
    if (log_buf_pos > 1)
    ...
    YYSYS_LOG(DEBUG, "UPDATE %s #%d#%s", rs, n, log_buf);
  }
