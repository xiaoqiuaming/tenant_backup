  ...
  while((opt = getopt_long(argc, argv, opt_string, longopts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'o':
        ...
        else {YYSYS_LOG(ERROR, "Unknown argument: %s", optarg); ret = 1;}
        break;
...
int Param::fill_default_param(const int argc,  char *const argv[])
{
  ...
  if (NULL == ip)
  {
    YYSYS_LOG(ERROR, "Host Address needed");
    goto error_h;
  }
  else YYSYS_LOG(INFO, "Host Address: %s", ip);
  ...
  else YYSYS_LOG(INFO, "Host Port: %d", port);

  ////////// Schema File //////////
  if (0 == schema_file.length())
  {
    YYSYS_LOG(ERROR, "Schema File needed");
    goto error_h;
  }
  else YYSYS_LOG(INFO, "Schema File: %s", schema_file.ptr());

  ////////// Thread Number //////////
  if (INVALID_VAL == thread_no)
  {
    thread_no = DEFAULT_THREAD_NO;
    YYSYS_LOG(INFO, "Thread Number using DEFAULT: %d", thread_no);
  }
  else YYSYS_LOG(INFO, "Thread Number: %d", thread_no);

  ////////// TPS Limit //////////
  if (INVALID_VAL == tps_limit)
  {
    tps_limit = DEFAULT_TPS_LIMIT;
    YYSYS_LOG(INFO, "TPS Limit using DEFAULT: %ld", tps_limit);
  }
  else YYSYS_LOG(INFO, "TPS Limit: %ld", tps_limit);

  ////////// Total Transaction Limit //////////
  if (INVALID_VAL == total_tr_limit)
  {
    total_tr_limit = DEFAULT_TOTAL_TR_LIMIT;
    YYSYS_LOG(INFO, "Total Transaction Limit using DEFAULT: %ld", total_tr_limit);
  }
  else YYSYS_LOG(INFO, "Total Transaction Limit: %ld", total_tr_limit);

  ////////// Request Timeout in us //////////
  if (INVALID_VAL == timeout_us)
  {
    timeout_us = DEFAULT_TIMEOUT_US;
    YYSYS_LOG(INFO, "Request Timeout using DEFAULT: %ldus", timeout_us);
  }
  else YYSYS_LOG(INFO, "Request Timeout: %ldus", timeout_us);

  ////////// Transaction Type //////////
  if (INVALID_VAL == tr_type)
  {
    tr_type = DEFAULT_TR_TYPE;
    YYSYS_LOG(INFO, "Transaction Type using DEFAULT: %s", str_tr_type(tr_type));
  }
  else YYSYS_LOG(INFO, "Transaction Type: %s", str_tr_type(tr_type));

  ////////// Transaction Size Limit //////////
  if (INVALID_VAL == size_p_tr)
  {
    size_p_tr = DEFAULT_SIZE_P_TR;
    YYSYS_LOG(INFO, "Transcation Size Limit using DEFAULT: %ld", size_p_tr);
  }
  else YYSYS_LOG(INFO, "Transcation Size Limit: %ld", size_p_tr);

  ////////// Log Name //////////
  if (0 == log_name.length())
  {
    log_name.assign(argv[0]).append(".log");
    YYSYS_LOG(INFO, "Log Name using DEFAULT: %s", log_name.ptr());
  }
  else YYSYS_LOG(INFO, "Log Name: %s", log_name.ptr());

  ////////// Log Level //////////
  if (NULL == log_level)
  {
    log_level = DEFAULT_LOG_LEVEL;
    YYSYS_LOG(INFO, "Log Level using DEFAULT: %s", log_level);
  }
  else YYSYS_LOG(INFO, "Log Level: %s", log_level);

  ////////// Log Ptype //////////
  if (INVALID_VAL == log_ptype)
  {
    log_ptype = DEFAULT_LOG_PTYPE;
    YYSYS_LOG(INFO, "Whether printing log per transaction using DEFAULT: %s", str_log_ptype(log_ptype));
  }
  else YYSYS_LOG(INFO, "Whether printing log per transaction: %s", str_log_ptype(log_ptype));
...
int OBAPI::init(const ObServer &server)
{
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "init_us_addr_ error, ret=%d", ret);
  }
  else
  {
//    ret = init_ms_addr_();
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "init_ms_addr_ error, ret=%d", ret);
    }
  }
...
int OBAPI::init_us_addr_()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    else
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "ObServer deserialize error, ret=%d", ret);
      }
    }
  }
...
int OBAPI::init_ms_addr_()
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (ss = ser_mgr.begin(); ss != ser_mgr.end(); ++ss)
    {
      if (NULL == ss)
      {
        YYSYS_LOG(WARN, "Server in ObChunkServerManager is NULL, ret=%d", ret);
        ret = OB_ERROR;
      }
      else
      {
    ...
    const int64_t timeout, common::ObScanner& scanner)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(ERROR, "ObScanParam serialize error, err=%d", err);
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN, "failed to send request, err=%d", err);
    }
  }
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS == err)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(ERROR, "scan result_code_ is %d", err);
        }
      }
    }
    // deserialize scanner
    if (OB_SUCCESS == err)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(ERROR, "deserialize scanner from buff failed, "
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(ERROR, "get result_code_ is %d", ret);
        }
      }
    }
    // deserialize scanner
...
int SchemaProxy::init(const char* schema_file)
{
  ...
  {
    YYSYS_LOG(ERROR, "ObSchemaManager parse_from_file error");
    ret = 1;
  }
  else
  {
    ...
    common::ObMutator &mutator) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(ERROR, "ObMutator use_ob_sem error, ret=%d", ret);
    }
  }
  ...
  if (OB_SUCCESS == ret)
  {
    for (ColumnArr::iterator ci = columns.begin(); OB_SUCCESS == ret && ci != columns.end(); ++ci)
    {
      ...
      {
        YYSYS_LOG(ERROR, "unknown column type: %d", ct);
        ret = 1;
      }
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "ObMutator update error, ret=%d", ret);
      }
    }
  }
    ...
    common::ObScanParam &scan_param) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (ColumnArr::iterator ci = columns.begin(); OB_SUCCESS == ret && ci != columns.end(); ++ci)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "ObScanParam add_column error, ret=%d column_id=%lu",
            ret, (*ci)->get_id());
      }
    }
  }
    ...
    common::ObGetParam &get_param) const
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    for (ColumnArr::iterator ci = columns.begin(); OB_SUCCESS == ret && ci != columns.end(); ++ci)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(ERROR, "ObGetParam add_cell error, ret=%d column_id=%lu",
            ret, (*ci)->get_id());
      }
    }
  }
        ...
        const common::ObScanner &scanner) const
{
  ...
  for (; OB_SUCCESS == ret && iter != scanner.end(); ++iter)
  {
    ...
    if (ObExtendType == cell->value_.get_type())
      ...
      {
        YYSYS_LOG(ERROR, "Scanner find an illegal ext type: %ld", cell->value_.get_ext());
        ret = OB_ERROR;
      }
      else
        row_not_exist = true;
    ...
    {
      YYSYS_LOG(ERROR, "Scanner find an illegal cell type: %ld", cell->value_.get_type());
      ret = OB_ERROR;
    }
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(ERROR, "Sum of line is not zero: line_sum=%ld", line_sum);
      ret = OB_ERROR;
    }
  }
  ...
  if (OB_SUCCESS != ret)
  {
    if (NULL != cell)
    {
      ...
      for (; iter != scanner.end(); ++iter)
      {
        ...
        YYSYS_LOG(ERROR, "value=%ld", value);
      }

      YYSYS_LOG(ERROR, "scanner_size=%ld rowkey=%.*s Columns={%.*s}",
          scanner.get_size(),
...
int FlowLimiter::wait()
{
  ...
  if (0 != tr_unit_limit_)
  {
    ...
    if (time_elapsed > TIME_UNIT)
    {
      ...
      stat_.get()->last_timestamp_ = yysys::CTimeUtil::getTime();
    }
    else
    {
...
{
  ++(stat_.get()->num_);
  stat_.get()->start_timestamp_ = yysys::CTimeUtil::getTime();
}
...
void StatCollector::succ()
{
  ...
  {
    ++(stat_.get()->succ_num_);
    int64_t et = yysys::CTimeUtil::getTime() - stat_.get()->start_timestamp_;
...
void ApplyRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  ...
  while (!_stop)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(ERROR, "gen_mutator_ error, ret=%d", ret);
      break;
    }
    else
    {
      do
      {
        ...
        else
        {
          ...
          {
            stat.fail(ret);
            YYSYS_LOG(ERROR, "apply error, ret=%d", ret);
          }
          else stat.succ();
        }
      }
...
int ApplyRunnable::gen_mutator_()
{
  ...
  if (0 != ret)
  {
    YYSYS_LOG(ERROR, "random_table error, ret=%d", ret);
  }
  else
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(ERROR, "random_column error, ret=%d", ret);
    }
  }
  ...
  if (0 == ret)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(ERROR, "fill_mutator error, ret=%d", ret);
    }
  }
...
void USScanRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  ...
  while (!_stop)
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(ERROR, "gen_scan_param_ error, ret=%d", ret);
      break;
    }
    else
    {
      do
      {
        ...
        else
        {
          ...
          {
            stat.fail(ret);
            YYSYS_LOG(ERROR, "scan error, ret=%d", ret);
          }
          else
          {
            ...
            {
              stat.fail(ret);
              YYSYS_LOG(ERROR, "scanner check error, ret=%d", ret);
            }
            else
            {
              stat.succ();
...
void USGetRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
    ...
    else
    {
      do
      {
        ...
        else
        {
          ...
          {
            stat.fail(ret);
            YYSYS_LOG(ERROR, "get error, ret=%d", ret);
          }
          else
          {
...
int GI::init()
{
  ...
  if (0 != ret)
  {
    YYSYS_LOG(ERROR, "ObServer init error, ret=%d", ret);
  }
  else
  {
    ...
    if (0 != ret)
    {
      YYSYS_LOG(ERROR, "SchemaProxy init error, ret=%d", ret);
    }
    else
    {
