...
void ObAddRunnable::run(yysys::CThread* thread, void* arg)
{
  UNUSED(thread);
  UNUSED(arg);
  ...
  if (OB_SUCCESS != ret)
  {
    YYSYS_LOG(WARN, "ObExecutor init error, ret=%d", ret);
  }
  ...
  while (OB_SUCCESS == ret && !_stop)
  {
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "ObUpdateGen gen error, ret=%d", ret);
      }
    }
  ...
  YYSYS_LOG(INFO, "TestAddRunnable finish, ret=%d _stop=%d", ret, _stop);
  abort();
}
