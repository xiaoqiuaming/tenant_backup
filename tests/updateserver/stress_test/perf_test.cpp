...
class PerformanceTest
{
  ...
  public:
    ...
    int init(ObSchemaManagerWrapper& schema_wrapper, ObSchemaManagerWrapper& read_schema_wrapper)
    {
      ...
      YYSYS_LOG(INFO, "table_name=%s, table_id=%lu, num_int_columns=%ld, num_other_columns=%ld, "
  ...
  public:
    ...
    */
        ...
        const ColumnType* other_columns_type, const int64_t* other_columns_size, ObMutator& mutator)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN, "failed to build write param, err=%d", err);
      }
    ...
    */
    ...
    static void* write_thread_routine(void* arg)
    {
      ...
      YYSYS_LOG(INFO, "START write thread, write_thread_num=%ld, write_thread_idx=%ld "
      ...
      while (!is_stop_)
      {
        ...
        if (OB_SUCCESS == err)
        {
          ...
          {
            YYSYS_LOG(INFO, "Successfully write 10000 rows");
          }
        }
        else
        {
      ...
      YYSYS_LOG(INFO, "write thread STOP, succeed_row_num=%ld, failed_row_num=%ld",
    ...
    static void* read_thread_routine(void* arg)
    {
      ...
      YYSYS_LOG(INFO, "START read thread, read_start_row=%ld, read_end_row=%ld "
      ...
      while (!is_stop_ && --read_times >= 0)
      {
        ...
        //YYSYS_LOG(INFO, "Scan data, start_val=%ld, end_val=%ld", start_val, end_val);
        ...
        else
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(INFO, "Successfully scan 10000000 rows");
            }
            ...
            while (OB_SUCCESS == (err = scanner.next_cell()))
            {
              ...
              {
                YYSYS_LOG(WARN, "invalid scanner");
                err = OB_ERROR;
              }
              else
              {
                ...
                YYSYS_LOG(INFO, "read_times=%ld column_id=%lu str len=%ld", read_times, cell_info->column_id_, str_val.length());
              }
            }
            */
          }
      ...
      YYSYS_LOG(INFO, "read thread STOP, succeed_row_num=%ld, read_size=%ld, failed_row_num=%ld "
...
void parse_cmd_line(int argc, char **argv, ConfigParam &clp)
{
  ...
  while ((opt = getopt_long(argc, argv, opt_string, longopts, NULL)) != -1)
  {
    ...
    case 'l':
      ...
      YYSYS_LOGGER.setFileName(log_file_name);
      break;
    case 'n':
...
int main(int argc, char** argv)
{
  ...
  //YYSYS_LOGGER.setFileName("./perf_test.log");
  ...
  yysys::CConfig read_config;
  ret_val = read_schema_wrapper.parse_from_file(param.read_schema_file_, read_config);
  if (false == ret_val)
  {
    YYSYS_LOG(WARN, "failed to parse read schema file, file_name=%s", param.read_schema_file_);
    exit(1);
  }
