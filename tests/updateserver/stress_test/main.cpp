...
int init_ups()
{
  ...
  for (int i = 0; OB_SUCCESS == ret && i <= row_dis.get_row_p1_num(); i++)
  {
    for (int j = 0; OB_SUCCESS == ret && j <= row_dis.get_row_p2_num(); j++)
    {
      ...
      if (OB_SUCCESS != ret)
      {
        YYSYS_LOG(WARN, "gen_line error, ret=%d", ret);
      }
      else
      {
        ...
        if (OB_SUCCESS != ret)
        {
          YYSYS_LOG(WARN, "ObExecutor exec error, ret=%d", ret);
        }
      }
    }
  }
...
int parse_cmd_line(const int argc,  char *const argv[])
{
  ...
  while((opt = getopt_long(argc, argv, opt_string, longopts, NULL)) != -1) {
    switch (opt) {
      case 'u':
        upsvip = optarg;
        YYSYS_LOG(INFO, "UPS VIP = %s", upsvip);
        break;
      case 'p':
        upsport = atoi(optarg);
        YYSYS_LOG(INFO, "UPS PORT = %d", upsport);
        break;
      case 'm':
        msip = optarg;
        YYSYS_LOG(INFO, "MS VIP = %s", msip);
        break;
      case 'q':
        msport = atoi(optarg);
        YYSYS_LOG(INFO, "MS PORT = %d", msport);
        break;
      case 'i':
        init = true;
        YYSYS_LOG(INFO, "Enable init UPS");
        break;
      case 't':
        thread_no = atoi(optarg);
        YYSYS_LOG(INFO, "Thread Number = %d", thread_no);
        break;
      case 'w':
        write_weight = atoi(optarg);
        YYSYS_LOG(INFO, "Write Weight = %d", write_weight);
        break;
      case 'r':
        read_weight = atoi(optarg);
        YYSYS_LOG(INFO, "Read Weight = %d", read_weight);
        break;
      case 'l':
        log_name = optarg;
        YYSYS_LOG(INFO, "Log Name = %s", log_name);
        break;
      case 'e':
        log_level = optarg;
        YYSYS_LOG(INFO, "Log Level = %s", log_level);
        break;
      case 'c':
        update_max_cell_no = atoi(optarg);
        YYSYS_LOG(INFO, "Maximum Number of Cells of Update = %ld", update_max_cell_no);
        break;
      case 's':
        scan_max_line_no = atoi(optarg);
        YYSYS_LOG(INFO, "Maximum Number of Lines of Scan = %ld", scan_max_line_no);
        break;
      case 'h':
        print_usage(argv[0]);
        exit(0);
...
int main(int argc, char* argv[])
{
  ...
  if (NULL != log_name)
  {
    YYSYS_LOGGER.setFileName(log_name);
  }
  else
  {
    YYSYS_LOGGER.setFileName("test_add.log");
  }
