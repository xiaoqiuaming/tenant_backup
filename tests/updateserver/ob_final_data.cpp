                              ...
                              const oceanbase::common::ObJoinInfo &join_info)
{
  ...
  {
    YYSYS_LOG(WARN,"unexpected error, fail to get right table schema");
  ...
  if (OB_SUCCESS == err)
  {
    ...
    if (NULL == column_info)
    {
      YYSYS_LOG(WARN,"unexpected error, fail to get right column schema");
      err = OB_ERR_UNEXPECTED;
    }
    if (OB_SUCCESS == err)
    {
  ...
  while (OB_SUCCESS == err)
  {
    ...
    if (OB_SUCCESS == err)
    {
      YYSYS_LOG(DEBUG, "apply one cell");
...
int ObFinalResult::parse_from_file(const char *  filename, const char *schema_fname)
{
  ...
  {
    YYSYS_LOG(WARN,"fail to load schema from file [fname:%s]", schema_fname);
    err = OB_SCHEMA_ERROR;
  }
  ...
  while (getline(input,line) && OB_SUCCESS == err)
  {
    ...
    {
      YYSYS_LOG(WARN,"line format error [line_number:%lld,line:%s]", line_number, line.c_str());
    ...
    {
      YYSYS_LOG(WARN,"value type error [line_number:%lld,value_type:%s]",
    ...
    {
      YYSYS_LOG(WARN,"optype error [line_number:%lld,optype:%s]",
                  ...
                  ObFinalResult & local_result)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN,"fail to get local result");
  }
      ...
      get_idx ++)
  {
    ...
    {
      err = ob_err;
      YYSYS_LOG(WARN,"ob next cell err");
      break;
    }
    if (OB_SUCCESS != local_err)
    {
      err = local_err;
      YYSYS_LOG(WARN,"local next cell err");
      break;
    }
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(WARN,"fail to get cell from ob result [idx:%lld]", get_idx);
    }
    if (OB_SUCCESS == err)
    {
      if (OB_SUCCESS == err)
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to get cell from local result [idx:%lld]", get_idx);
      }
    }
    if (OB_SUCCESS == err)
    {
      ...
      {
        YYSYS_LOG(WARN,"ob result table name error [idx:%lld,ob.table_name_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"ob result rowkey error [idx:%lld,ob.row_key_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"ob result column name error [idx:%lld,ob.column_name_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"local result table name error [idx:%lld,ob.table_name_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"local result rowkey error [idx:%lld,local.row_key_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"local result column_name error [idx:%lld,local.column_name_:%.*s,"
      ...
      */
      YYSYS_LOG(DEBUG, "ob.table_name:%.*s,ob.rowkey:%.*s,ob.column_name:%.*s",
      ...
      YYSYS_LOG(DEBUG, "local.table_name:%.*s,local.rowkey:%.*s,local.column_name:%.*s",
      ...
      {
        YYSYS_LOG(WARN,"table name not coincident [idx:%lld,ob.table_name_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"rowkey not coincident [idx:%lld,ob.row_key_:%.*s,"
      ...
      {
        YYSYS_LOG(WARN,"column name not coincident [idx:%lld,ob.column_name_:%.*s,"
      ...
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"ext info not coincident [idx:%lld,ob.ext:%lld,local.ext:%lld]",
        ...
        if (OB_SUCCESS == err && ObExtendType != ob_cell->value_.get_type())
        {
          ...
          if (ob_cell->value_.get_ext() != local_cell->value_.get_ext())
          {
            YYSYS_LOG(WARN,"ext not coincident [idx:%lld,ob.ext:%lld,"
          ...
          if (ob_cell->value_.get_ext() == 0  && OB_SUCCESS == err)
          {
            ...
            else
            {
              if (OB_SUCCESS == err)
              {
                ...
                if (OB_SUCCESS != err)
                {
                  YYSYS_LOG(WARN,"fail to get int from local, ext:%lld,type:%d",
                            local_cell->value_.get_ext(),local_cell->value_.get_type());
                }
                else
                {
                  YYSYS_LOG(DEBUG, "local value:%lld", local_value);
                }
              }
              if (OB_SUCCESS == err)
              {
                ...
                if (OB_SUCCESS != err)
                {
                  YYSYS_LOG(WARN,"fail to get int from ob, ext:%lld,type:%d",
                            ob_cell->value_.get_ext(),ob_cell->value_.get_type());
                }
                else
                {
                  YYSYS_LOG(DEBUG, "ob value:%lld", ob_value);
                }
              }

              if (OB_SUCCESS != err)
              {
                YYSYS_LOG(WARN,"fail to get int value [idx:%lld]", get_idx);
                err = OB_INVALID_ARGUMENT;
              }
              else
              {
                ...
                {
                  YYSYS_LOG(WARN,"value not coincident [idx:%lld,ob.value:%lld,"
  ...
  {
    YYSYS_LOG(WARN,"ob return more result than expected");
  }

  if (OB_SUCCESS == err && local_scanner.next_cell() != OB_ITER_END)
  {
    YYSYS_LOG(WARN,"local return more result than expected");
  }

  if (OB_SUCCESS != err)
  {
    res = false;
    YYSYS_LOG(DEBUG, "check fail");
  }
  else
  {
    res = true;
    YYSYS_LOG(DEBUG, "check pass");
  }
  return res;
}
                  ...
                  ObFinalResult & local_result)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(WARN,"fail to scan local result");
  }
  ...
  while (OB_SUCCESS == err)
  {
    for (int64_t scan_idx = 0; scan_idx < scan_param.get_column_name_size(); scan_idx ++)
    {
      ...
      if (OB_SUCCESS != err)
      {
        YYSYS_LOG(WARN,"fail to get cell from ob result [row_idx:%lld,idx:%lld]",
                  row_number, scan_idx);
      }
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"local result return more cells than ob result");
          err = OB_INVALID_ARGUMENT;
        }
        if ( !ob_end && OB_ITER_END == err)
        {
          YYSYS_LOG(WARN,"ob result return more cells than local result");
          err = OB_INVALID_ARGUMENT;
        }
        if (ob_end && OB_ITER_END == err)
        {
        ...
        if (OB_SUCCESS != err)
        {
          YYSYS_LOG(WARN,"fail to get cell from local result [row_idx:%lld,idx:%lld]",
                    row_number, scan_idx);
        }
      }
      if (OB_SUCCESS == err)
      {
        ...
        {
          YYSYS_LOG(WARN,"ob result table name error [row_idx:%lld,idx:%lld,ob.table_name_:%.*s,"
        ...
        {
          YYSYS_LOG(WARN,"ob result rowkey error [row_idx:%lld,idx:%lld,ob.column_name_:%.*s,"
        ...
        if (local_cell->row_key_ != ob_cell->row_key_)
        {
          YYSYS_LOG(WARN,"rowkey error [row_idx:%lld,idx:%lld,ob.row_key_:%.*s,"
        ...
        if (OB_SUCCESS == err)
        {
          if (ob_cell->value_.get_ext() != local_cell->value_.get_ext())
          {
            YYSYS_LOG(WARN,"ext info not coincident [row_idx:%lld,idx:%lld,"
          ...
          else
          {
            ...
            if (OB_SUCCESS != err)
            {
              YYSYS_LOG(WARN,"fail to get int value [row_idx:%lld,idx:%lld]",row_number, scan_idx);
              err = OB_INVALID_ARGUMENT;
            }
            else
            {
              if (ob_value != local_value)
              {
                YYSYS_LOG(WARN,"value not coincident [row_idx:%lld,idx:%lld,ob.value:%lld,"
