...
class RWSessionCtx : public BaseSessionCtx, public CallbackMgr
{
  public:
    RWSessionCtx(SessionMgr &host) : BaseSessionCtx(ST_READ_WRITE, host),
                                     ...
                                     is_killed_(false)
    {
      YYSYS_LOG(INFO, "use mock RWSessionCtx");
    };
  public:
    void reset()
    {
    ...
    void kill()
    {
      if (is_frozen_)
      {
        YYSYS_LOG(WARN, "session is frozen, can not kill, sd=%u", get_session_descriptor());
      }
      else
      {
        is_killed_ = true;
...
TEST(TestSessionMgr, init)
{
  ...
  uint32_t sd = 0;
  ret = sm.begin_session(ST_READ_WRITE, yysys::CTimeUtil::getTime(), 0, 0, sd);
  EXPECT_EQ(OB_SUCCESS, ret);
