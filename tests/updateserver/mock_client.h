...
inline int BaseClient::initialize()
{
  ...
  else
  {
    ...
    {
      YYSYS_LOG(ERROR, "failed to init client_mgr, err=%d", ret);
    }
    else
    {
      //start io thread
...
class MockClient : public BaseClient
{
  ...
  public:
    ...
    int kill_session(const int64_t timeout, const int64_t session_id)
    {
      ...
      {
        YYSYS_LOG(WARN,"fail to serialize sessionid [ret:%d]", ret);
      }
      ...
      if(OB_SUCCESS == ret)
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          {
            YYSYS_LOG(WARN,"kill session:%d", result_code.result_code_);
          }
        }
      }
      return ret;
      ...
      else
      {
        ...
        if (OB_SUCCESS == ret)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(WARN,"fail to deserialize result [ret:%d]", ret);
            }
            if ((OB_SUCCESS == ret) && (OB_SUCCESS != (ret = result.get_varchar(msg))))
            {
              YYSYS_LOG(WARN,"fail to get result msg [ret:%d]", ret);
            }
            if (OB_SUCCESS == ret)
            {
              fprintf(stderr, "%.*s\n", msg.length(), msg.ptr());
