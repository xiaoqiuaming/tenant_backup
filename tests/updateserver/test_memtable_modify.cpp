...
class MockUpsTableMgr : public ObIUpsTableMgr
{
  ...
  public:
    int apply(RWSessionCtx &session_ctx, ObIterator &iter, const ObDmlType dml_type)
    {
      ...
      {
        YYSYS_LOG(WARN, "add trans cb info to session ctx fail, ret=%d", ret);
      }
      else
      {
...
TEST(TestObInsertDBSemFilter, iter_succ)
{
  ...
  MockUpsTableMgr tm;
  uint32_t sd = 0;
  sm.begin_session(ST_READ_WRITE, yysys::CTimeUtil::getTime(), INT64_MAX, INT64_MAX, sd);
  ...
  session->set_trans_id(yysys::CTimeUtil::getTime());
  sm.revert_ctx(sd);
  sm.end_session(sd);
