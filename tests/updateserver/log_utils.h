...
namespace oceanbase
{
  namespace test
  {
    ...
    int get_log_and_handle(IObBatchLogHandler* log_handler, ObLogGenerator& log_generator) {
      ...
      {
        YYSYS_LOG(ERROR, "get_log()=>%d", err);
      }
      else if (OB_SUCCESS != (err = log_handler->handle_log(start_cursor, end_cursor, buf, len))
               && OB_NEED_RETRY != err)
      {
        YYSYS_LOG(ERROR, "handle_log(log_range=[%ld,%ld], len=%ld)=>%d",
      ...
      {
        YYSYS_LOG(ERROR, "log_generator.commit_log(end_cursor=%s)=>%d", end_cursor.to_str(), err);
      }
      return err;
    }
    ...
    class ObUpsLogMgr2
    {
      ...
      public:
        ...
        int init(const char* log_dir, const int64_t log_file_max_size, int64_t log_buf_size) {
          ...
          {
            YYSYS_LOG(ERROR, "log_generator.init(log_dir=%s)=>%d", log_dir, err);
          }
          else if (OB_SUCCESS != (err = log_writer_.init(log_dir, LOG_ALIGN-1, log_sync_type)))
          {
            YYSYS_LOG(ERROR, "log_writer.init(log_dir=%s)=>%d", log_dir, err);
          }
          else
          {
            log_dir_ = log_dir;
        ...
        int start_log(const ObLogCursor& start_cursor) {
          ...
          {
            YYSYS_LOG(ERROR, "log_generator.start_log(start_cursor)=>%d", err);
          }
          else if (OB_SUCCESS != (err = log_writer_.start_log(start_cursor)))
          {
            YYSYS_LOG(ERROR, "log_writer.start_log(start_cursor)=>%d", err);
          }
          else
          {
        ...
        int write_log(LogCommand cmd, char* buf, int64_t len) {
          ...
          else if (NULL == buf || 0 > len)
          {
            err = OB_INVALID_ARGUMENT;
                   ...
                   && OB_BUF_NOT_ENOUGH != err)
          {
            YYSYS_LOG(ERROR, "log_generator.write_log()=>%d", err);
          }
          else if (OB_BUF_NOT_ENOUGH != err)
          {}
          else if (OB_SUCCESS != (err = flush_log()))
          {
            YYSYS_LOG(ERROR, "flush_log()=>%d", err);
          }
          return err;
        }
        ...
        int flush_log() {
          ...
          {
            YYSYS_LOG(ERROR, "log_generator.get_log()=>%d", err);
          }
          else if (OB_SUCCESS != (err = log_writer_.write_log(log_buf, log_buf_len)))
          {
            YYSYS_LOG(ERROR, "log_writer.write_log(buf=%p[%ld])=>%d", log_buf, log_buf_len, err);
          }
          else if (OB_SUCCESS != (err = log_generator_.commit(end_cursor)))
          {
            YYSYS_LOG(ERROR, "log_generator.commit(end_cursor=%s)=>%d", end_cursor.to_str(), err);
          }
          else if (OB_SUCCESS != (err = log_writer_.get_cursor(end_cursor_)))
          {
            YYSYS_LOG(ERROR, "log_writer.get_log()=>%d", err);
          }
          else
          {
            YYSYS_LOG(INFO, "write log: %s", end_cursor_.to_str());
          }
          return err;
        }
    ...
    class WriteLogHandler : public IObBatchLogHandler
    {
      ...
      public:
        ...
        virtual int handle_log(const ObLogCursor& start_cursor, const ObLogCursor& end_cursor,
          ...
          UNUSED(start_cursor);
          UNUSED(end_cursor);
          //YYSYS_LOG(INFO, "write_log([%ld,%ld])=>%d", start_cursor.log_id_, end_cursor.log_id_);
