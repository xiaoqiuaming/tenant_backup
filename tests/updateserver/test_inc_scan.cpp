    ...
    class ObIncScanTest: public ::testing::Test, public Config, public ObMockUps, public RWT {
      ...
      protected:
        ...
        int read_rows() {
          ...
          {
            YYSYS_LOG(ERROR, "rand_values()=>%d", err);
          }
          else if (false && OB_SUCCESS != (err = test_values(values)))
          {
            YYSYS_LOG(ERROR, "test_values()=>%d", err);
          }
          ...
          else if (NULL == session_ctx)
          {
            ...
            YYSYS_LOG(ERROR, "session_ctx == NULL");
          }
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "build_get_param()=>%d", err);
            }
          }
          if (OB_SUCCESS != err)
          {}
          ...
          {
            YYSYS_LOG(ERROR, "scan_op.close()=>%d", err);
          }
          else if (OB_SUCCESS != (err = values.close()))
          {
            YYSYS_LOG(ERROR, "values.close()=>%d", err);
          }
          if (NULL != scan_op)
          {
            delete scan_op;
        ...
        int write_rows() {
          ...
          {
            YYSYS_LOG(ERROR, "modify.close()=>%d", err);
          }
          if (NULL != modify)
          {
            delete modify;
        ...
        int report() {
          int err = OB_SUCCESS;
          YYSYS_LOG(INFO, "thread[%ld:%ld],read=[%ld:%ld:%ld],write=[%ld:%ld:%ld]",
