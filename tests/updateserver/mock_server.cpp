      ...
      class MockServer : public ObSingleServer
      {
        public:
          ...
          virtual int do_request(ObPacket* base_packet)
          {
            ...
            YYSYS_LOG(INFO, "recv packet with packet_code[%d] version[%d] channel_id[%d]",
          ...
          int handle_send_log(ObPacket* ob_packet)
          {
            ...
            if (NULL == data_buffer)
            {
              YYSYS_LOG(ERROR, "data_buffer is NUll should not reach this");
              ret = OB_ERROR;
            }
            else
            {
              ...
              YYSYS_LOG(DEBUG, "recv send log request, ret=%d", ret);

              int32_t version = 1;
              int32_t channel_id = ob_packet->getChannelId();
              ret = send_response(OB_SEND_LOG_RES, version, out_buffer, connection, channel_id);
              YYSYS_LOG(DEBUG, "send log response, ret=%d", ret);
            }
          ...
          int handle_renew_lease(ObPacket* ob_packet)
          {
            ...
            else
            {
              ...
              YYSYS_LOG(DEBUG, "recv renew lease request, slave_addr=%s, ret=%d", addr_buf, ret);
              yynet::Connection* connection = ob_packet->get_connection();
              char buf[10 * 1024];
              ObDataBuffer out_buffer(buf, sizeof(buf));
              ...
              YYSYS_LOG(DEBUG, "send renew lease response, ret=%d", ret);
            }
          ...
          int handle_grant_lease(ObPacket* ob_packet)
          {
            ...
            else
            {
              ...
              ret = lease.deserialize(data_buffer->get_data(), data_buffer->get_capacity(),
                 data_buffer->get_position());
              YYSYS_LOG(DEBUG, "recv grant lease request, lease_time=%ld, lease_interval=%ld, "
              ...
              YYSYS_LOG(DEBUG, "send grant lease response, ret=%d", ret);
            }
