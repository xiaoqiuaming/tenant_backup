...
void *thread_func(void *arg)
{
  ...
  for(uint64_t i = 1; i < g_times_per_thread + 1; i++)
  {
    ...
    if(OB_SUCCESS == uc.get(g_key_array[j].table_id,rowkey,*handle,g_key_array[j].column_id,g_value_result[j]))
    {
      ...
      {
        YYSYS_LOG(ERROR,"the error version");
        YYSYS_LOG(INFO,"j=%d,version=%ld",j,g_value_result[j].version);
      }
      g_value_result[j].value.get_int(int_val);
      if(j != int_val)
      {
        YYSYS_LOG(ERROR,"the error value");
        YYSYS_LOG(INFO,"j=%d,int_val=%d",j,int_val);
      }
    }
    else
    {
      YYSYS_LOG(WARN,"get failed");
    }
    if(NULL != handle)
      delete handle;
    ...
    {
      YYSYS_LOG(WARN,"is_row_exist failed");
    }
...
int main(int argc, char **argv)
{
  ...
  for(; i < g_key_num; i++)
  {
 //   YYSYS_LOG(INFO,"%d",g_value_array[i].version);
  }
