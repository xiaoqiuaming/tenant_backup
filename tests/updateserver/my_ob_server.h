    ...
    class MyObServer : public ObBaseServer
    {
      ...
      public:
        ...
        virtual int handlePacket(ObPacket *packet){
          ...
          YYSYS_LOG(DEBUG,"get packet code is %d, priority=%d", req->get_packet_code(), req->get_packet_priority());
          if (!packet)
          {
          ...
          {
            YYSYS_LOG(ERROR, "req->deserialize()=>%d", err);
          }
          else if (OB_SUCCESS != (handle_err = do_request(req->get_packet_code(), *req->get_buffer(), ret_pcode, handle_buf)))
          {
            YYSYS_LOG(ERROR, "do_request(pcode=%d)=>%d", req->get_packet_code(), handle_err);
          }
          ...
          {
            YYSYS_LOG(ERROR, "serialize_result()=>%d", err);
          }
          else if (OB_SUCCESS != (err = send_response(ret_pcode, req->get_api_version(), outbuf, req->get_request(), req->get_channel_id())))
          {
            YYSYS_LOG(ERROR, "send_response(pcode=%d)=>%d", ret_pcode, err);
          }

          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(WARN, "packet %d can not be distribute to queue", req->get_packet_code());
        ...
        int serialize_result(int err_code, ObDataBuffer& buf, ObDataBuffer& outbuf) {
          ...
          {
            YYSYS_LOG(ERROR, "result_mgr.serialize()=>%d", err);
          }
          else if (outbuf.get_remain() < buf.get_position())
          {
            err = OB_BUF_NOT_ENOUGH;
            YYSYS_LOG(ERROR, "outbuf.get_remain()[%ld] < buf.get_position()[%ld], err=%d", outbuf.get_remain(), buf.get_position(), err);
          }
        ...
        virtual int handleBatchPacket(ObPacketQueue &packetQueue) {
          UNUSED(packetQueue);
          YYSYS_LOG(WARN, "You should not reach here!");
          return 0;
        }
      private:
