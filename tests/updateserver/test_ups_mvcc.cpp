    ...
    class CONST
    {
      public:
        ...
        static int64_t GetUniqTime()
        {
          ...
          while (true)
          {
            int64_t newv = yysys::CTimeUtil::getTime();
...
#define DEFINE_TABLE_SCHEMA(TableName, Rowkey) \
          ...
          if (OB_SUCCESS == ret \
            ...
            if (OB_SUCCESS == ret \
              ...
              YYSYS_LOG(ERROR, "SchemaMgr add_table " #TableName " error");\
            } else {\
              YYSYS_LOG(INFO, "Schema New Table: " #TableName "(ID=%ld)",\
                  TableName##TID);\
            }\
          }
#define DEFILE_COLUMN_SCHEMA(TableName, ColumnName) \
          ...
          if (OB_SUCCESS == ret \
            ...
            YYSYS_LOG(ERROR, "SchemaMgr add_column " #TableName "." #ColumnName " error");\
          } else {\
            YYSYS_LOG(INFO, "Schema New Column: " #TableName "." #ColumnName \
...
#define DEFILE_COLUMN_SCHEMA_V(TableName, ColumnNameV, ColumnIDV, ColumnTypeV) \
          ...
          if (OB_SUCCESS == ret \
            ...
            YYSYS_LOG(ERROR, "SchemaMgr add_column " #TableName ".%s error", ColumnNameV);\
          } else {\
            YYSYS_LOG(INFO, "Schema New Column: " #TableName ".%s(ID=%ld)", \
    ...
    class ErrMsgException
    {
      public:
        ...
        void logException() const
        {
          ...
          YYSYS_LOG(ERROR, "Exception: %04d%02d%02d%02d%02d%02d%06ld "
...
#define THROW_EXCEPTION_AND_LOG(iErrCode, pszMsg, ...) \
    YYSYS_LOG(ERROR, "Throw Exception: ErrCode: %d ErrMsg: " pszMsg, \
        iErrCode, ##__VA_ARGS__); \
    throw ErrMsgException(__FILE__, __LINE__, __FUNCTION__, pthread_self(), \
                          iErrCode, pszMsg, ##__VA_ARGS__)
#define RETHROW(ex) \
    YYSYS_LOG(ERROR, "Rethrow Exception: ErrCode: %d ErrMsg: %s", \
    ...
    class CopyA
    {
      public:
          ...
          : oA_(oDefaultA_)
        {
          ...
          if (OB_SUCCESS != err)
          {
            YYSYS_LOG(ERROR, "FIFOAllocator init error");
          }
        }
        CopyA(A & oA)
          : oA_(oA)
    ...
    class Singleton
    {
      public:
                                       ...
                                       bool bIsRollBack)
        {
          ...
          int ret = OB_SUCCESS;
          int64_t iCurTimestamp = yysys::CTimeUtil::getTime();
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(ERROR, "end_session error");
          }
          else
          {
            //YYSYS_LOG(INFO, "[END] trans_id=%ld", tid);
          }
          return ret;
        }
      protected:
          ...
          : oStorage_(oLogWriter_), iLastTimestamp_(0)
        {
          ...
          else
          {
            ...
            {
              YYSYS_LOG(ERROR, "SchemaGen error");
            }
            else
            {
...
#define S Singleton::getInstance()
      ...
      public:
        ...
        void beginTransaction()
        {
          int err = oSessionMgr_.begin_session(getSessionType(),
              yysys::CTimeUtil::getTime(), CONST::TransactionTimeout,
    ...
    class DAO
    {
      ...
      protected:
           ...
           throw(ErrMsgException)
        {
          ...
          int64_t iStartTime = yysys::CTimeUtil::getTime();
            ...
            AutoString & pszCheckDataValue) throw(ErrMsgException)
        {
          ...
          else
          {
            ...
            //YYSYS_LOG(INFO, "Rowkey: %ld  RowStat: %ld  WriteThreadID: %ld  "
    ...
    class ApplyThread : public yysys::CDefaultRunnable
    {
      protected:
        class Applier
        {
            ...
            void randomInfoRowkey_(RWTransaction::RWGuard & oGuard) throw(ErrMsgException)
            {
              ...
              do
              {
                ...
                catch (ErrMsgException & ex)
                {
                  ...
                  {
                    err = ex.getErrorCode();
                    //YYSYS_LOG(INFO, "1: iInfoRowkey_: %ld iRowStat_: %ld err: %d",
                ...
                //YYSYS_LOG(ERROR, "1: iInfoRowkey_: %ld iRowStat_: %ld err: %d",
                //    iInfoRowkey_, iRowStat_, err);
              }
              while (!(OB_ENTRY_NOT_EXIST == err
                     || (OB_SUCCESS == err && iRowStat_ == 0)
                     || (++iErrorCounter >= CONST::GetInfoMaxRowkey())));
              //YYSYS_LOG(ERROR, "2: iInfoRowkey_: %ld iRowStat_: %ld err: %d",
              //    iInfoRowkey_, iRowStat_, err);
              ...
              //if (iErrorCounter > 0)
              //{
              //  YYSYS_LOG(INFO, "2: iInfoRowkey_: %ld iRowStat_: %ld err: %d",
              //      iInfoRowkey_, iRowStat_, err);
              //}
            }
        ...
        virtual void run(yysys::CThread *thread, void *arg)
        {
          ...
          while (!_stop && OB_SUCCESS == err)
          {
            if (OB_SUCCESS != (err = oApplier_.applyRandom()))
            {
              ...
              {
                YYSYS_LOG(ERROR, "applyRandom error");
              }
            }
          ...
          fprintf(stderr, "\n");
          YYSYS_LOG(INFO, "===========");
          YYSYS_LOG(INFO, "Thread(%d) did %ld tests changed %ld rows",
    ...
    class CheckThread : public yysys::CDefaultRunnable
    {
      protected:
        class Checker
        {
          ...
          public:
            ...
            int check(int64_t iInfoRowkey)
            {
              ...
              {
                YYSYS_LOG(ERROR, "check error: iInfoRowkey_: %ld  "
      ...
      public:
        ...
        virtual void run(yysys::CThread *thread, void *arg)
        {
          ...
          while (!_stop && OB_SUCCESS == err)
          {
            ...
            if (OB_SUCCESS == oQueue_.pop(piTemp))
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(ERROR, "check error: iInfoRowkey_: %ld", iInfoRowkey);
                }
                else
                {
                  iSuccCounter++;
          ...
          iErr_ = err;
          YYSYS_LOG(INFO, "STAT: SuccCounter: %ld", iSuccCounter);
        }
        void setThreadID(int iThreadID)
        {
          iThreadID_ = iThreadID;
        ...
        LockMgr & lock_mgr_, ObUpsTableMgr & storage_)
    {
      ...
      oDao.getDataLine(ro.getSessionCtx(), 10, iThreadID, iSelfCheck1,
          iMultiCheck);
      YYSYS_LOG(INFO, "iSelfCheck1: %ld  iSelfCheck2: %ld  iMultiCheck: %ld",
      ...
      oDao.getDataLine(ro.getSessionCtx(), 10, iSelfCheck2, aiMultiChecks,
          iMultiCheckNum);
      YYSYS_LOG(INFO, "iSelfCheck2: %ld  iMultiCheckNum: %ld  iMultiCheck0: %ld  "
    ...
    TEST(UPS_MVCC, apply) {
          ...
          && oMemTable.btree_size() < CONST::getMaxLineNum())
      {
        ...
        YYSYS_LOG(INFO, "TEST IS RUNNING. BTREE LINE NUMBER: %-10ld  MEM: %-12ld  "
            "RESV: %-12ld  %%: %ld",
      ...
      int64_t total = oMemTable.btree_alloc_memory();
      int64_t reserved = oMemTable.btree_reserved_memory();
      YYSYS_LOG(INFO, "TEST IS FINISHED. BTREE LINE NUMBER: %-10ld  MEM: %-12ld  "
          "RESV: %-12ld  %%: %ld",
          oMemTable.btree_size(), total, reserved, reserved * 100 / total);
      YYSYS_LOG(INFO, "=================");
    }
...
{
  YYSYS_LOG(INFO, "start main");
  ::testing::InitGoogleTest(&argc, argv);
  int err = OB_SUCCESS;
  //YYSYS_LOGGER.setFileName("test_ups_mvcc.log");
