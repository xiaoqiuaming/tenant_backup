  ...
  namespace updateserver
  {
    class OBS : public ISSTableObserver
    {
      ...
      public:
        ...
        {
          YYSYS_LOG(INFO, "add sstable id=%lu", sstable_id);
          return OB_SUCCESS;
        };
        virtual int erase_sstable(const uint64_t sstable_id)
        {
          YYSYS_LOG(INFO, "erase sstable id=%lu", sstable_id);
          return OB_SUCCESS;
        };
        virtual int add_sstable(const common::ObList<uint64_t> &sstable_list)
        {
          YYSYS_LOG(INFO, "add sstable list size=%lu", sstable_list.size());
          return OB_SUCCESS;
        };
    };
    ...
    class FP// : public IFetchParam
    {
      ...
      public:
        ...
        {
          YYSYS_LOG(INFO, "add sst path=[%s] name=[%s]", sst_file_info.path.ptr(), sst_file_info.name.ptr());
          return OB_SUCCESS;
        };
        virtual void set_max_clog_id(const uint64_t max_clog_id)
        {
          YYSYS_LOG(INFO, "set clog id=%lu", max_clog_id);
        };
    };
...
int main(int argc, char **argv)
{
  ...
  ob_init_memory_pool();
  //YYSYS_LOGGER.setFileName("test_sstablemgr.log");
