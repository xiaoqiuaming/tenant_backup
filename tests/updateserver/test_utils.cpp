...
{
  char *ret = NULL;
  const char *str = YYSYS_CONFIG.getString(section, key);
  if (NULL == str)
  {
    len = 0;
  }
...
void read_get_param(const char *fname, const char *section, PageArena<char> &allocer, ObGetParam &get_param)
{
  init_opt_map_();
  init_obj_type_map_();
  YYSYS_CONFIG.load(fname);
  const char *str = NULL;

  int32_t cell_infos_num = YYSYS_CONFIG.getInt(section, CELL_INFOS_NUM);
  str = YYSYS_CONFIG.getString(section, GET_FROZEN_ONLY);
  if (0 == strcmp("YES", str))
  {
  ...
  for (int32_t i = 0; i < cell_infos_num; i++)
  {
    ...
    if (NULL == str)
    {
      ...
      ci->table_id_ = YYSYS_CONFIG.getInt(section, buffer);
    }
    else
    {
    ...
    rowkey_desc = YYSYS_CONFIG.getString(section, buffer);

    sprintf(buffer, ROW_KEY_KEY_FORMAT, i);
    str = YYSYS_CONFIG.getString(section, buffer);
    ...
    if (NULL == str)
    {
      ...
      ci->column_id_ = YYSYS_CONFIG.getInt(section, buffer);
    }
    else
    {
...
void read_scan_param(const char *fname, const char *section, PageArena<char> &allocer, ObScanParam &scan_param)
{
  ...
  str = YYSYS_CONFIG.getString(section, SCAN_FROZEN_ONLY);
  if (0 == strcmp("YES", str))
  {
    //scan_param.set_is_read_frozen_only(true);
  }

  str = YYSYS_CONFIG.getString(section, SCAN_REVERSE);
  if (NULL == str
      || 0 != strcmp("YES", str))
  {
  ...
  if (NULL == str)
  {
    table_id = YYSYS_CONFIG.getInt(section, SCAN_TABLE_ID);
  }
  else
  {
  ...
  int32_t columns_num = YYSYS_CONFIG.getInt(section, SCAN_COLUMNS_NUM);
  for (int32_t i = 0; i < columns_num; i++)
  {
    ...
    else
    {
      ...
      uint64_t column_id = YYSYS_CONFIG.getInt(section, buffer);
      scan_param.add_column(column_id);
    }
  }

  const char* rowkey_desc_start = YYSYS_CONFIG.getString(section, "ROW_KEY_DESC");
  char* rowkey_desc_end = strdup(rowkey_desc_start);


  ObNewRange range;
  range.table_id_ = table_id;
  str = YYSYS_CONFIG.getString(section, SCAN_START_KEY);
  ...
  str = YYSYS_CONFIG.getString(section, SCAN_END_KEY);
...
void read_cell_infos(const char *fname, const char *section, PageArena<char> &allocer, ObMutator &mutator, ObMutator &result)
{
  ...
  for (int32_t i = 0; i < cell_infos_num; i++)
  {
    ...
    {
      int64_t value = YYSYS_CONFIG.getInt(section, buffer);
...
void prepare_cell_new_scanner(ObCellNewScanner &scanner)
{
  ...
  if (OB_SUCCESS != err)
  {
    YYSYS_LOG(ERROR, "ObCellNewScanner serialize error, err: %d", err);
  }
  else
  {
    pos = 0;
    ...
    if (OB_SUCCESS != err)
    {
      YYSYS_LOG(ERROR, "ObCellNewScanner deserialize error, err: %d", err);
    }
  }
  delete[] buffer;
}
