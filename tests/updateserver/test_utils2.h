...
#define core() (*(char*)(0) = 0)
    ...
    struct Vcalc
    {
      public:
        ...
        int start() {
          count_ = 0;
          start_ = yysys::CTimeUtil::getTime();
          end_ = 0;
          return 0;
        }
        int end(int64_t count) {
          count_ = count;
          end_ = yysys::CTimeUtil::getTime();
          return 0;
        }
        int report(const char* msg) {
          YYSYS_LOG(INFO, "%s: %ld/(%ld-%ld)=%.4lf/ms", msg, count_, end_, start_, 1e3 * (double)count_/(double)(end_ - start_));
          return 0;
        }
