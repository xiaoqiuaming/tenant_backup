...
int MockRootServer::do_request(ObPacket* base_packet)
{
  ...
  {   
    ...
    default:
      {
        ret = OB_ERROR;
        YYSYS_LOG(ERROR, "wrong packet code");
        break;
      }
    }
    //
...
int MockRootServer::handle_get_updater(ObPacket * ob_packet)
{
  ...
  yynet::Connection* connection = ob_packet->get_connection();
...
int MockRootServer::handle_waiting_job_done(ObPacket * ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    YYSYS_LOG(DEBUG, "recv schema changed request, timestamp=%ld ret=%d", timestamp, ret);
  }
...
int MockRootServer::handle_fetch_schema(ObPacket * ob_packet)
{
  ...
  {
    ret = serialization::decode_vi64(data->get_data(), data->get_capacity(), data->get_position(), &timestamp);
    YYSYS_LOG(DEBUG, "recv fetch schema request, timestamp=%ld ret=%d", timestamp, ret);
  }
...
int MockRootServer::handle_register_server(ObPacket * ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(INFO, "merge server registered");
    }
  }
  ...
  YYSYS_LOG(INFO, "handle register server result:ret[%d]", ret);
  return ret;
}
...
int MockRootServer::handle_scan_root(ObPacket * ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "check param failed");
    }
    else
    {
      //
      ...
      {
        YYSYS_LOG(ERROR, "check param failed:table[%lu], size[%lu]", param.get_table_id(), param.get_column_id_size());
        ret = OB_ERROR;
      }
    }
  }
  ...
  YYSYS_LOG(INFO, "handle scan root table result:ret[%d]", ret);
  return ret;
}
...
int MockRootServer::handle_get_root(ObPacket * ob_packet)
{
  ...
  else
  {
    ...
    YYSYS_LOG(INFO,"row_size=%d",get_param.get_row_size());

    ObCellInfo *cell_ptr = get_param[0];
    YYSYS_LOG(INFO,"column_id=%u",cell_ptr->column_id_);
    YYSYS_LOG(INFO,"table_id=%u",cell_ptr->table_id_);
...
*/
    ...
    ret = scanner.serialize(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position());
    ret = send_response(OB_REPORT_TABLETS_RESPONSE, 1, out_buffer, connection, channel_id);
    YYSYS_LOG(INFO, "send get root response:ret[%d]", ret);
  }

  YYSYS_LOG(INFO, "handle get root table result:ret[%d]", ret);
  return ret;
}
/*
