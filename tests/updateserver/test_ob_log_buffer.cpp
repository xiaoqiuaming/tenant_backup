...
namespace oceanbase
{
  namespace test
  {
    ...
    class ObLogCacheTest: public ::testing::Test, public yysys::CDefaultRunnable, public Config {
      public:
        ObLogCacheTest() {}
        ~ObLogCacheTest() {}
      protected:
      ...
      protected:
        ...
        int get(const int64_t& start, int64_t& end){
          ...
          {
            YYSYS_LOG(ERROR, "log_cache.get_log(start_id=%ld)=>%d", start, err);
          }
          return err;
        }
        ...
        void run_as_reader(yysys::CThread *thread, int64_t idx){
          ...
          for (; !_stop && (OB_SUCCESS == err || OB_DATA_NOT_SERVE == err) && start_id < max_num_items; start_id = end_id)
          {
            ...
            {
              YYSYS_LOG(ERROR, "get(start_id=%ld)=>%d", start_id, err);
            }
          ...
          YYSYS_LOG(INFO, "reader[%ld]: %ld/(%ld+%ld)=%ld/10000", idx, hit, hit, miss, 10000 * hit/(hit+miss));
        }
    ...
    TEST_F(ObLogCacheTest, PushAndGetSeq){
      ...
      {
        //YYSYS_LOG(DEBUG, "push(end_id=%ld)", end_id);
        ...
        {
          err = OB_SUCCESS;
          YYSYS_LOG(INFO, "get(start_id=%ld): OB_DATA_NOT_SERVE", start_id);
        }
        else
        {
          start_id = end_id2;
