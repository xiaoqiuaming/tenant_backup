...
namespace oceanbase
{
  namespace sstable
  {
    ...
    DEFINE_SERIALIZE(ObTableTrailerInfoV1)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie trailer table info, buf=%p, "
                        "buf_len=%ld, pos=%ld", buf, buf_len, pos);
        ret = OB_ERROR;
      }
    ...
    DEFINE_DESERIALIZE(ObTableTrailerInfoV1)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialzie trailer table info, buf=%p, "
                        "buf_len=%ld, pos=%ld", buf, data_len, pos);
        ret = OB_ERROR;
      }
    ...
    DEFINE_SERIALIZE(ObSSTableTrailerV1)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to serialzie first part of trailer, buf=%p, "
                        "buf_len=%ld, pos=%ld", buf, buf_len, pos);
        ret = OB_ERROR;
      }
    ...
    DEFINE_DESERIALIZE(ObSSTableTrailerV1)
    {
                                       ...
                                       reinterpret_cast<int64_t*> (&sstable_checksum_))))
      {
        ...
        if (OB_SUCCESS == ret && table_count_ > 0)
        {
          ...
          if (OB_SUCCESS != ret)
          {
            YYSYS_LOG(WARN, "failed to init table info");
          }
        }
    ...
    uint64_t ObSSTableTrailerV1::get_table_id(const int64_t index) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%ld, table_count=%d",
                  index, table_count_);
      }
    ...
    int ObSSTableTrailerV1::set_table_id(const int64_t index, const uint64_t table_id)
    {
          ...
          && table_id != OB_INVALID_ID && table_id != 0)
      {
        ...
        {
          YYSYS_LOG(WARN, "table id isn't in ascending ording, prev_table_id=%lu, "
                          "cur_table_id=%lu",
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%ld, table_id=%lu, table_count=%d",
                                           ...
                                           const int64_t column_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%ld, column_count=%ld, "
    ...
    int ObSSTableTrailerV1::set_table_count(const int32_t count)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_count=%d", count);
        ret = OB_ERROR;
      }
    ...
    int ObSSTableTrailerV1::init_table_info(const int64_t table_count)
    {
      ...
      {
        YYSYS_LOG(WARN, "table info is inited, no need init again");
        ob_free(table_info_);
        table_info_ = NULL;
      }
      ...
      if (OB_SUCCESS == ret)
      {
        ...
        if (NULL == table_info_)
        {
          YYSYS_LOG(ERROR, "Problem allocating memory for table trailer info");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObSSTableTrailerV1::ensure_key_buf_space(const int64_t size)
    {
      ...
      else if (NULL == key_buf_ || (NULL != key_buf_ && size > reamin_size))
      {
        ...
        if (NULL == new_buf)
        {
          YYSYS_LOG(ERROR, "Problem allocating memory for key buffer");
          ret = OB_ERROR;
        }
        else
        {
    ...
    int ObSSTableTrailerV1::set_start_key(const int64_t table_index, ObString& start_key)
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_index=%ld, table_count=%d, "
                        "key_len=%d, key_ptr=%p",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "no enough memory to store start key, key_len=%d, "
                          "key_ptr=%p", size, ptr);
        }
      }
    ...
    int ObSSTableTrailerV1::set_end_key(const int64_t table_index, ObString& end_key)
    {
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "no enough memory to store end key, key_len=%d, "
                          "key_ptr=%p", size, ptr);
        }
      }
    ...
    const ObString ObSSTableTrailerV1::get_start_key(const uint64_t table_id) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, table_id=%lu, table_count=%d, table_info=%p",
      ...
      else
      {
        ...
        {
          YYSYS_LOG(WARN, "can't find table id in trailer, table_id=%lu, "
                          "table_count=%d, table_info=%p",
                    table_id, table_count_, table_info_);
        }
      }
    ...
    int ObSSTableTrailerV1::set_key_buf(ObString key_buf)
    {
      ...
      if (NULL == ptr || len == 0)
      {
        YYSYS_LOG(WARN, "invalid parameter, key bufer length=%ld, ptr=%p",
                  len, ptr);
        ret = OB_ERROR;
      }
      else
