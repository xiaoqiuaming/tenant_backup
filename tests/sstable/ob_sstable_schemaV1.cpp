...
{ 
  namespace sstable 
  {
      ...
      const int32_t index) const
    {
      ...
      {
        YYSYS_LOG(WARN, "invalid param, index=%d, column_count=%d", 
    ...
    DEFINE_DESERIALIZE(ObSSTableSchemaColumnDefV1)
    {
      ...
      {
        YYSYS_LOG(WARN, "failed to deserialize schema column def");
    ...
    int ObSSTableSchemaV1::add_column_def(ObSSTableSchemaColumnDefV1& column_def)
    {
      ...
      if (schema_header_.column_count_ < OB_MAX_COLUMN_NUMBER)
      {
        ...
        {
          YYSYS_LOG(WARN, "column id must be in ascending order, prev_column_id=%lu, "
      ...
      {
        YYSYS_LOG(WARN, "no space to store more column def, "
