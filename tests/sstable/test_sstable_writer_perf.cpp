    ...
    namespace sstable 
    {
      ...
      void test_write_one_file()
      {
        ...
        if (OB_ERROR == (ret = writer.create_sstable(schema, file_name,
                                  compressor, 2)))
        {
          YYSYS_LOG(ERROR, "Failed to create sstable file: %s", sstable_path);
        }
        ...
        for (int64_t i = 0; i < 5000000; ++i) 
        {
          ...
          {
            YYSYS_LOG(ERROR, "add row failed, i=%ld", i);
            return;
          }
        }
        ...
        {
          YYSYS_LOG(ERROR, "close sstable failed ------------------");
        }
      ...
      void test_write_patch_file_with_check()
      {
        ...
        for (int64_t i = 0; i < 5000000; ++i) {
          ...
          {
            YYSYS_LOG(ERROR, "add row failed");
            return;
          }
        }
