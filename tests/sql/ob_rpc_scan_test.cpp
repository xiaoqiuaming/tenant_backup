  ...
  public:
    MOCK_CONST_METHOD7(post_request,
        int (const ObServer& server,
          ...
          yynet::IPacketHandler* handler,
          void* args));

    //    MOCK_METHOD2(handlePacket,
    //         yynet::IPacketHandler::HPRetCode(yynet::Packet* packet, void * args));
};
...
int handle_scan_table(ObDataBuffer &out_buffer)
{
  ...
  {
    YYSYS_LOG(WARN, "invalid times!!! times=%d", times);
  }

  /* end add by xiaochu */
  if (OB_SUCCESS != (ret = scanner.serialize(out_buffer.get_data(), out_buffer.get_capacity(), out_buffer.get_position())))
  {
    YYSYS_LOG(WARN, "fail to serialize scanner");
  }

  YYSYS_LOG(INFO, "handle scan table result:ret[%d]", ret);
  return ret;
}
...
class AsyncPacketQueueHandler : public Runnable{
  public:
    void push(ObMsSqlRpcEvent *result, ObPacket* packet)
    {
      ...
      YYSYS_LOG(INFO, "push a packet into result queue [queue.size=%d]", packet_queue.size());
    }
    ...
    void run(CThread *thread, void *arg)
    {
      while(1)
      {
        ...
        {
          YYSYS_LOG(INFO, "proccessing packet in the queue [queue.size=%d]", packet_queue.size());
          handle_packet();
    ...
    void handle_packet()
    {
      ...
      if (NULL != pair.first)
      {
        ...
        YYSYS_LOG(INFO, "event->handlePacket called [queue.size=%d]", packet_queue.size());
      }
      else
      {
        YYSYS_LOG(WARN, "invalid event pointer. is NULL!!!");
      }
    }
  private:
...
{
  YYSYS_LOG(WARN, "callback invoiked~ <SCAN> ~~~~~~~~~~~");
...
{
  YYSYS_LOG(WARN, "callback invoiked~ <SESSION> ~~~~~~~~~~~~~~~~~");
  handle_async_request(result);
  return OB_SUCCESS;
}
...
int callback_func(Unused, Unused,Unused,Unused,Unused,  yynet::IPacketHandler* handler, void* args)
{
  YYSYS_LOG(WARN, "callback invoiked~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
  YYSYS_LOG(WARN, "%p, %p", handler, args);
  EXPECT_NE((yynet::IPacketHandler*)NULL, handler);
...
TEST_F(TestObRpcScan, test_single_table_scan)
{
  ...
  YYSYS_LOG(INFO, "( 1 )");
  yynet::Transport * transport = new yynet::Transport();
  yynet::DefaultPacketStreamer * streamer = new yynet::DefaultPacketStreamer();
  ...
  EXPECT_CALL(client_manager, handlePacket(_,_))
    ...
    .WillOnce(Return((yynet::IPacketHandler::HPRetCode)0));
#endif
  ...
  // start root server
  MockRootServer root;
  yysys::CThread root_server_thread;
  ...
  while(1)
  {
    ...
    YYSYS_LOG(INFO, "get_next_row() - begin");
    if (OB_ITER_END == (ret = scan.get_next_row(cur_row)))
    {
      YYSYS_LOG(INFO, "get_next_row() - iter_end");
      break;
    }
    else if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "get next row failed. ret=%d", ret);
      break;
    }
    /*YYSYS_LOG(DEBUG, "[id:%lu][key:%.*s][obj:dumped bellow]", cur_row->table_id_, cur_row->row_key_.length(),
      cur_row->row_key_.ptr());
      cur_row->value_.dump();
      */
    if (NULL != cur_row)
    {
      YYSYS_LOG(INFO, "row info:[SEE_DEBUG_LEVEL]");
      cur_row->dump();
    }
    else
    {
      YYSYS_LOG(WARN, "no current row");
    }
    YYSYS_LOG(INFO, "get_next_row() - end");
  }
  YYSYS_LOG(INFO, "scannnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnning - terminatet");
  //sleep(10);

  scan.close();

  usleep(1000 * 1000 * 2);
  YYSYS_LOG(INFO, "( request sent )");
  transport->stop();
