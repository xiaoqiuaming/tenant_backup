...
int MockUpdateServer::handle_mock_scan(ObPacket *ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "%s", "check scan_param failed");
    }
  }
  ...
  else
  {
    ...
    if (mock::join_table_id == cell.table_id_)
    {
      for (int32_t i = 0; i < scan_param.get_column_id_size(); i++)
      {
        ...
        {
          YYSYS_LOG(ERROR, "unepxected column id [tableid:%lu,columnid:%lu]", cell.table_id_,
    ...
    {
      YYSYS_LOG(ERROR, "unexpected table id [tableid:%lu]", cell.table_id_);
      ret = OB_ERR_UNEXPECTED;
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      pos = 0;
      YYSYS_LOG(INFO, "pos:%ld,ret:%d",pos, 
...
int MockUpdateServer::handle_mock_get(ObPacket *ob_packet)
{
  ...
  if (OB_SUCCESS == ret)
  {
    ...
    if (ret != OB_SUCCESS)
    {
      YYSYS_LOG(ERROR, "%s", "check param failed");
    }
  }
  ...
  YYSYS_LOG(INFO, "handle get table result:ret[%d]", ret);
  return ret;
}
...
int MockUpdateServer::handle_mutate_table(ObPacket * ob_packet)
{
  ...
  YYSYS_LOG(INFO, "handle mutate table result:ret[%d]", ret);
  return ret;
}
...
int MockUpdateServer::handle_frozen_version(ObPacket * ob_packet)
{
  ...
  YYSYS_LOG(INFO, "handle get frozen version result:ret[%d]", ret);
  return ret;
}
