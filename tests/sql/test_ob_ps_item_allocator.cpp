...
namespace oceanbase
{
  namespace tests
  {
    namespace sql
    {
      ...
      class AllocFreeRunnable: public yysys::CDefaultRunnable
      {
      public:
        void run(yysys::CThread *thread, void *arg)
        {
          ...
          for (int i = 0; i < 10000; ++i)
          {
            ...
            YYSYS_LOG(ERROR, "alloc item %p", item);
            EXPECT_TRUE(NULL != item);
            alloc->free(item);
            YYSYS_LOG(ERROR, "free item %p", item);
            item = reinterpret_cast<ObPsStoreItem*>(alloc->alloc(sizeof(ObPsStoreItem)));
            YYSYS_LOG(ERROR, "alloc item %p", item);
            EXPECT_TRUE(NULL != item);
            alloc->free(item);
...
int main(int argc, char **argv)
{
  ...
  YYSYS_LOGGER.setFileName("test.log");
