    ...
    namespace sql
    {
      ...
      void do_get(TestObPsStore *psstore)
      {
        ...
        for (int i = 0; i < TestObPsStore::sqlnum; ++i)
        {
            ...
            else if (PS_ITEM_INVALID == item->get_status())
            {
              while(true && OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  {
                    YYSYS_LOG(DEBUG, "item %p wlock failed", item);
                  }
                }
              }
            }
          ...
          {
            YYSYS_LOG(ERROR, "Get item failed ret=%d, sql is %.*s", ret, sql.length(), sql.ptr());
          }
        }
      }
            ...
            else if (PS_ITEM_INVALID == item->get_status())
            {
              while(true && OB_SUCCESS == ret)
              {
                ...
                else
                {
                  ...
                  {
                    //YYSYS_LOG(ERROR, "item %p wlock failed", item);
                  }
                }
              }
            }
      ...
      class GetRunnable:public yysys::CDefaultRunnable
      {
      public:
        void run(yysys::CThread *thread, void *arg)
        {
          TestObPsStore *tstore = reinterpret_cast<TestObPsStore*>(arg);
          YYSYS_LOG(DEBUG, "start thread %p", thread);
          do_get(tstore);
        }
      };

      class GetPlanRunnable:public yysys::CDefaultRunnable
      {
      public:
        void run(yysys::CThread *thread, void *arg)
        {
      ...
      class RemovePlanRunnable:public yysys::CDefaultRunnable
      {
      public:
        void run(yysys::CThread *thread, void *arg)
        {
      ...
      class MixRunnable:public yysys::CDefaultRunnable
      {
      public:
        void run(yysys::CThread *thread, void *arg)
        {
...
void sigdump(int s)
{
  ...
  YYSYS_LOG(ERROR, "Obtained %zd stack frames\n", size);
  for (i = 0; i < size; i++)
    YYSYS_LOG(ERROR,"%s\n", strings[i]);
  free (strings);
  exit(0);
