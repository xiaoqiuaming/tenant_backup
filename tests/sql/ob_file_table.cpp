...
int ObFileTable::parse_line(const ObRow *&row)
{
  ...
  if (column_count_ == count_)
  {
    for (int32_t i=0;(OB_SUCCESS == ret) && i<count_;i++)
    {
      ...
      else
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "unsupport type [%d]", column_type_[i]);
        }
      }
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "column_count_[%d], count_[%d]", column_count_, count_);
  }
...
int ObFileTable::get_next_row(const common::ObRowkey *&rowkey, const ObRow *&row)
{
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    if (OB_SUCCESS != ret)
    {
      YYSYS_LOG(WARN, "parse line fail");
    }
  }
...
int ObFileTable::open()
{
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "file_name_ is null");
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "open file [%s] fail", file_name_);
    }
    else
    {
      YYSYS_LOG(INFO, "open file [%s]", file_name_);
    }
  }
  ...
  //  YYSYS_LOG(WARN, "rowkey item count too large[%d]", rowkey_item_count_);
  //}
  ...
  if(OB_SUCCESS == ret)
  {
    if ( 2 == count_ )
    {
      ...
      {
        YYSYS_LOG(WARN, "convert int fail:[%s]", tokens_[0]);
      }
      else if(OB_SUCCESS != (ret = convert(tokens_[1], column_count_)))
      {
        YYSYS_LOG(WARN, "convert int fail:[%s]", tokens_[1]);
      }
    }
    else
    {
      YYSYS_LOG(WARN, "should be two num [%d]", count_);
      ret = OB_ERROR;
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "column_count_[%d], count_[%d], line_[%s]", column_count_, count_, line_copy_);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    if( column_count_ == count_ )
    {
      ...
      for(int32_t i=0; (OB_SUCCESS == ret) && i<count_;i++)
      {
        ...
        {
          YYSYS_LOG(WARN, "convert [%s] fail", tokens_[i]);
        }
        else
        {
          switch( type )
          ...
          default:
            ret = OB_ERROR;
            YYSYS_LOG(WARN, "unsupport type[%s]", tokens_[i]);
          }
        }
      }
    }
