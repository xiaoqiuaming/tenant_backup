  ...
  namespace sql
  {
    namespace test
    {
      ...
      void ObTabletJoinTest::test_get_next_row(ObTabletJoin &tablet_join)
      {
        ...
        for(int i=0;i<7;i++)
        {
          ...
          for(int j=1;j<4;j++)
          {
            ...
            if(!eq)
            {
              YYSYS_LOG(WARN, "================row idx[%d] column idx[%d]", i, j);
              YYSYS_LOG(WARN, "cell:%s", print_obj(*cell));
              YYSYS_LOG(WARN, "result_cell:%s", print_obj(*result_cell));
            }
            ASSERT_TRUE(eq);
          }
        }
      ...
      int ObTabletJoinTest::get_rowkey(const ObRow &row, ObString &rowkey)
      {
        ...
        {
          YYSYS_LOG(WARN, "get varchar:ret[%d]", ret);
        }
      ...
      int ObTabletJoinTest::read_join_info(ObTabletJoin::TableJoinInfo &tablet_join_info, const char *file_name)
      {
        ...
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "file_name is null");
        }
        else if(NULL == (fp = fopen(file_name, "r")))
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "open file[%s] fail", file_name);
        }
        else if(NULL == fgets(buf, 1024, fp))
        {
          ret = OB_ERROR;
          YYSYS_LOG(WARN, "fgets fail");
        }
        else
        {
          int num = atoi(buf);
          for(int i=0;i<num;i++)
          {
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  YYSYS_LOG(WARN, "add join column fail:ret[%d]", ret);
                }
              }
            }
          }
