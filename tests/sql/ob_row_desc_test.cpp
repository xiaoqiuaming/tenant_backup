...
TEST_F(ObRowDescTest, basic_test)
{
  ...
  for (uint64_t tid = 1000; tid <= 1001; ++tid)
  {
    for (uint64_t cid = 16; cid <= 19; ++cid)
    {
      ...
      YYSYS_LOG(DEBUG, "get_idx tid=%lu cid=%lu index=%ld", tid, cid, count);
      ++count;
    }
  }
  ...
  for (uint64_t tid = 1000; tid <= 1001; ++tid)
  {
    for (uint64_t cid = 16; cid <= 19; ++cid)
    {
      ...
      YYSYS_LOG(DEBUG, "get_tid_cid tid=%lu cid=%lu index=%ld", out_tid, out_cid, count);
      ++count;
    }
  }
...
TEST_F(ObRowDescTest, perf_test)
{
  ...
  YYSYS_LOG(WARN, "hash_collisions=%lu", row_desc.get_hash_collisions_count());
  // get idx
  ...
  int64_t finish_time = yysys::CTimeUtil::getTime();
  int64_t elapsed_ms = (finish_time-start_time)/1000;
  YYSYS_LOG(INFO, "ops=%ld elapsed_ms=%ld ops/ms=%ld",
