...
int ObFakeUpsMultiGet::open()
{
  ...
  YYSYS_LOG(DEBUG, "open fake ups multi get operator");
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "file table open fail:ret[%d]", ret);
    }
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      YYSYS_LOG(WARN, "file table close fail:ret[%d]", ret);
    }
  }
...
int ObFakeUpsMultiGet::get_next_row(const ObRowkey *&rowkey, const ObRow *&row)
{
  ...
  if(OB_SUCCESS == ret)
  {
    YYSYS_LOG(DEBUG, "ups multi get rowkey:[%s]", to_cstring(*rowkey));
  }
...
int ObFakeUpsMultiGet::convert(const ObUpsRow &from, ObUpsRow &to)
{
  ...
  for(int64_t i=0;OB_SUCCESS == ret && i<to.get_column_num();i++)
  {
    ...
    {
      YYSYS_LOG(WARN, "get_tid_cid fail:ret[%d]", ret);
    }
    else if(OB_SUCCESS != (ret = from.get_cell(table_id, column_id, cell)))
    {
      YYSYS_LOG(WARN, "get_cell fail:ret[%d]", ret);
    }
