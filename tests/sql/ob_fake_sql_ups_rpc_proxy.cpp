...
int ObFakeSqlUpsRpcProxy::sql_ups_get(const ObGetParam & get_param, ObNewScanner & new_scanner, const int64_t timeout)
{
  ...
  if(NULL == ups_multi_get_file_)
  {
    ...
    YYSYS_LOG(WARN, "ups multi get file is null");
    return ret;
  }

  YYSYS_LOG(DEBUG, "fake sql ups rpc proxy:[%s]", to_cstring(get_param));
  ...
  {
    ret = OB_ERROR;
    YYSYS_LOG(WARN, "row desc is invalid");
    return ret;
  }
  else
  {
  ...
  {
    YYSYS_LOG(WARN, "open multi get fail:ret[%d]", ret);
  }
...
int ObFakeSqlUpsRpcProxy::sql_ups_scan(const ObScanParam & scan_param, ObNewScanner & new_scanner, const int64_t timeout)
{
  ...
  {
    ret = OB_INVALID_ARGUMENT;
    YYSYS_LOG(WARN, "ups scan file is null");
    return ret;
  }
  ...
  if(OB_SUCCESS == ret)
  {
    ...
    {
      ret = OB_ERROR;
      YYSYS_LOG(WARN, "table id is invalid");
      return ret;
    }
  }
  ...
  {
    YYSYS_LOG(WARN, "open ups scan fail:ret[%d]", ret);
  }
  ...
  while(OB_SUCCESS == ret)
  {
    ...
    if(OB_SUCCESS == ret)
    {
      ...
      {
        ret = OB_ERROR;
        YYSYS_LOG(WARN, "must be ObUpsRow");
      }
      else
      {
