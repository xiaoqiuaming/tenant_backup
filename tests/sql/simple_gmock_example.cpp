...
class TargetClass
{
  public:
  TargetClass(){
    YYSYS_LOG(INFO, "construct targetClass");
  }

  ~TargetClass(){
    YYSYS_LOG(INFO, "destruct targetClass");
  }
  ...
  virtual int sendPacket(const char *data, char *result_buf, int result_buf_len)
  {
    ...
    // 3. wait for response
    // 4. return value
    YYSYS_LOG(WARN, "input data=[%s]", data);
...
TEST(ABC, abc)
{
  ...
  YYSYS_LOG(INFO, "return buf=[%s]", buf);
}
