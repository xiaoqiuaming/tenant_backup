...
int32_t run_sql_test_file(const char* file_name)
{
  ...
  if (ret == OB_SUCCESS && yysys::CConfig::getCConfig().load("MyTestSchema.ini") == EXIT_FAILURE)
  {
    YYSYS_LOG(ERROR, "load schema error");
    ret = OB_ERROR;
  }
  if (ret == OB_SUCCESS && (sql_fd = fopen(file_name, "r")) == NULL)
  {
    YYSYS_LOG(ERROR, "can not open sql file");
    ret = OB_ERROR;
  }
  ...
  if (ret == OB_SUCCESS)
  {
    ...
    if (chdir(cmd))
    {
      YYSYS_LOG(ERROR, "chdir error[%s]", strerror(errno));
      ret = OB_ERROR;
    }
  }
  ...
  {
    YYSYS_LOG(ERROR, "parse_init error!!!");
    ret = OB_ERROR;
  }
  ...
  while (ret == OB_SUCCESS && fgets(sql_string, OB_MAX_SQL_LENGTH, sql_fd) != NULL)
  {
    ...
    if (strncmp(ptr, "@@", 2) == 0)
    {
      ...
      {
        YYSYS_LOG(ERROR, "open %s error", cmd);
        ret = OB_ERROR;
        break;
      }
      continue;
