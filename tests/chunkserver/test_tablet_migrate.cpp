      ...
      public:
        ...
        static  int cs_migrate(const common::ObRange& range,
                               const common::ObServer& src_server, const common::ObServer& dest_server, bool keep_src)
          {
            ...
            if (my_buffer != NULL)
            {
              ...
              if (ret == OB_SUCCESS)
              {
                ...
                if (ret == OB_SUCCESS)
                {
                  ...
                  if (ret != OB_SUCCESS)
                  {
                    YYSYS_LOG(INFO, "rpc return error code is %d msg is %s", result_msg.result_code_, result_msg.message_.ptr());
                  }
                }
              }
            }
            ...
            {
              YYSYS_LOG(ERROR, "can not get thread buffer");
              ret = OB_ERROR;
            }
            return ret;
          }
        ...
        static int create_tablet(const ObRange& range, const ObSSTableId& sst_id)
          {
            ...
            if (OB_SUCCESS == err)
            {
              ...
              if (OB_SUCCESS == err)
              {
                YYSYS_LOG(ERROR, "tablet already exists! dump input and exist:");
