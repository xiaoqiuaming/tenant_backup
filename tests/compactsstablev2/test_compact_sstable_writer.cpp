...
public:
      ...
      const int version_flag)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid version flag:version_flag=%d",
          version_flag);
      ret = OB_ERROR;
    }
  ...
  int make_comp_name(ObString& comp_name, const int comp_flag)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invlid comp_flag:comp_flag=%d", comp_flag);
      ret = OB_ERROR;
    }
    else if (0 == comp_flag)
    {
  ...
  int make_compressor(ObCompressor*& compressor, const int comp_flag)
  {
    ...
    if (NULL == compressor_lzo)
    {
      YYSYS_LOG(ERROR, "create compressor error:NULL==compressor_");
      ret = OB_ERROR;
    }
    ...
    if (OB_SUCCESS == ret)
    {
      ...
      {
        YYSYS_LOG(ERROR, "not support:comp_flag=%d", comp_flag);
        ret = OB_ERROR;
      }
    }
  ...
  int make_file_path(ObString& file_path, const int64_t file_num)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid file_num:file_num=%ld", file_num);
      ret = OB_ERROR;
    }
      ...
      const uint64_t table_id, const int flag)
  {
    ...
    {
      YYSYS_LOG(ERROR, "invalid flag:flag=%d", flag);
      ret = OB_ERROR;
    }
  }
      ...
      const int start_flag, const int end_flag)
  {
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid end_flag:start_flag=%d", start_flag);
        ret = OB_ERROR;
      }
    ...
    else
    {
      ...
      {
        YYSYS_LOG(ERROR, "invalid end_flag:end_flag=%d", end_flag);
        ret = OB_ERROR;
      }
